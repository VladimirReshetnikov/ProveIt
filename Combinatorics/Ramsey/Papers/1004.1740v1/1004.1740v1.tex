...
example, $M(4)=10$, corresponding to the permutations $(1342), \,
...
In \cite{Sha09} the question whether $\lim_{n \to \infty}
\frac{M(n+1)}{M(n)} = 2$ was attributed
...
$M(15)=74904$ (see \cite{Dav77}), the inequality holds for $8 \le n \le
...
\proof We first show that any permutation
$\sigma=(t_1,t_2,\ldots,t_{11})$ of $\{1,2,\ldots,11\}$ with $t_1=2$ and 
$t_2=1$ must contain a $3$-term AP with odd common difference as a
subsequence. Indeed, $4$ must appear in $\sigma$ before $3$ to avoid the
$3$-term AP $(2,3,4)$; similar considerations force $4$ before $5$, $7$
before $4$, $6$ before $5$, $6$ before $7$, $11$ before $6$, $8$ before
$11$, $8$ before $9$, and $7$ before $10$. Thus, both $8$ and $11$ appear
in $\sigma$ before either $9$ or $10$ appears. Now we have the subsequence
$(8,9,10)$ if $9$ occurs before $10$ in $\sigma$ and the subsequence
$(11,10,9)$ otherwise. This proves our claim. \\

Let $a_1,a_2,\ldots$ be a permutation of the positive integers. Subtracting
$a_1-1$ from every term and ignoring nonpositive terms, we can assume that
$a_1=1$. Let $k$ be
...
let $\sigma_i$ be a 3AP-free permutation of the following set of $2^{2i-1}$
consecutive even numbers: $$\{(4^i+2)/3, (4^i+8)/3, \ldots, 
(4^{i+1}-4)/3\}$$ Similarly, let $\pi_i$ be a 3AP-free permutation of the 
following set of $2^{2i-2}$ consecutive odd numbers: $$\{(4^i+2)/6,
(4^i+14)/6, \ldots, (4^{i+1}-10)/6\}$$ Observe that the concatenated
...
\liminf_{n \rightarrow \infty} \frac{A(n)}{n} \mbox{ where } A(n)=|S \cap
...
\in T_{\ell}$ with $\ell < k$. But $x_3 - x_2 < q_{k-1} \le x_2 - x_1$,
...
and Results in Combinatorial Number Theory}}, L'Enseignement Mathematique,
