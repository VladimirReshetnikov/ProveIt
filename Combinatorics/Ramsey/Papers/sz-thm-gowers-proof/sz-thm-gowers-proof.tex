...
which implies, after a small calculation (using the assumption that $t \geqslant$ $2^{2^{40 k^{3}}}$ ), that $q \leqslant 2 t^{1 / 2 k}$.
...
Let us choose $u=r^{1 / k^{2} 2^{k+2}}$, with the result that $u^{k} r^{-1 / k 2^{k+1}}=r^{-1 / k 2^{k+2}}$. It is easy to check that $u \geqslant 2^{2^{40(k-1)^{3}}}$. Therefore, by induction, if $v \leqslant u^{1 / L}$, where $L=((k-1)!)^{2} 2^{k^{2}}$, then every $Q_{j}$ can be partitioned into arithmetic progressions $P_{j t}$ of length $v-1$ or $v$ in such a way that $\operatorname{diam}\left(\phi_{j}\left(P_{j t}\right)\right) \leqslant$ $u^{-1 / L} N$ for every $t$. It is not hard to check that this, with our choice of $u$ above, gives us the inductive hypothesis for $k$. \(\square\)

% ed.: Corollary 5.7 uses the target-length form of Corollary 5.6, with an
% emergent positive number of nonempty cells.
Corollary 5.7. Let $\phi: \mathbb{Z}_{N} \rightarrow \mathbb{Z}_{N}$ be a polynomial of degree $k$, let $K=$ $(k!)^{2} 2^{(k+1)^{2}}$, let $\alpha>0$ and let $r\leqslant N$ be an integer exceeding $\max \left\{2^{2^{40 k^{3}}},(4 \pi / \alpha)^{K}\right\}$. Then, for any integer $v$ with $1 \leqslant v \leqslant r^{1 / K}$, there is a partition of the set $\{0,1, \ldots, r-1\}$ into a positive number of nonempty arithmetic progressions $P_{1}, \ldots, P_{M}$, each of length $v-1$ or $v$, and if $f: \mathbb{Z}_{N} \rightarrow D$ is any function such that
...
Lemma 5.9. Let $\phi_{1}, \ldots, \phi_{q}$ be polynomials from $\mathbb{Z}_{N}$ to $\mathbb{Z}_{N}$ of degree at most $k$, let $K=(k!)^{2} 2^{(k+1)^{2}}$ and let $r\leqslant N$ be an integer exceeding $2^{2^{40 k^{3}} K^{q-1}}$. Then for every integer $v$ with $1 \leqslant v \leqslant r^{1 /(2 K^{q})}$ the set $\{0,1,2, \ldots, r-1\}$ can be partitioned into a positive number of nonempty arithmetic progressions $P_{1}, \ldots, P_{M}$, each of length $v-1$ or $v$, such that the diameter of $\phi_{i}\left(P_{j}\right)$ is at most $r^{-1 / K^{q}} N$ for every $i$ and every $j$.

Proof. First we prove by induction that for every $p \leqslant q$ we can partition the set $\{0,1, \ldots, r-1\}$ into some positive number of nonempty arithmetic progressions of size at least $r^{1 / K^{p}}$ such that $\operatorname{diam} \phi_{i}\left(P_{j}\right)$ is at most $r^{-1 / K^{p}} N$ for every $i \leqslant p$ and every cell $P_j$. When $p=1$ this follows immediately from Corollary 5.6. If we know it for $p-1$, let $Q_{1}, \ldots, Q_{l}$ be the arithmetic progressions obtained. The size of each $Q_{i}$ is at least $r^{1 / K^{p-1}} \geqslant 2^{2^{40 k^{3}}}$, so by Corollary 5.6 each $Q_{i}$ can be partitioned into further arithmetic progressions $P_{j}$ of cardinality at least $\left(r^{1 / K^{p-1}}\right)^{1 / K}=r^{1 / K^{p}}$, such that, for every $j$, the diameter of $\phi_{p}\left(P_{j}\right)$ is at most $\left(r^{-1 / K^{p-1}}\right)^{1 / K} N=r^{-1 / K^{p}} N$. This is clearly enough to give us the statement for $p$.
...
Lemma 5.10. Let $k \geqslant 2$, let $K=k^{2} 2^{k+3}$, let $m \geqslant 2^{K^{2^{k}} 2^{40 k^{3}+1}}$, let $P$ be a box in $\mathbb{Z}_{N}^{k}$ of width at least $m$ and let $\mu$ be a $k$-linear function from $P$ to $\mathbb{Z}_{N}$. Then $P$ can be partitioned into boxes $P_{1}, \ldots, P_{M}$, such that each $P_{j}$ has width at least $m^{K^{-2^{k}}}$ and the diameter of $\mu\left(P_{j}\right)$ is at most $2 m^{-K^{-2^{k}}} N$ for every $j$.\\
Proof. As noted above, $\mu$ can be written as a sum of terms of the form $c_{A} \prod_{j \in A} x_{j}$. Take any total ordering on the subsets of $[k]$ which extends the partial ordering by inclusion, and define the height of $\mu$ to be the largest position in this ordering of a set $A$ such that the coefficient $c_{A}$ is non-zero. We shall prove the result by induction on the height. The precise inductive hypothesis is that if $\mu$ has height at most $p$, then any box $Q$ of width $t \geqslant 2^{K^{p} 2^{40 k^{3}+1}}$ can be partitioned into boxes $Q_{j}$ of width at least $t^{K^{-p}}$ such that for every $j$ the diameter of $\mu\left(Q_{j}\right)$ is at most $\left(1+2^{-k} p\right) t^{-K^{-p}} N$.
...
Corollary 5.11. Let $k \geqslant 2$, let $K=k^{2} 2^{k+3}$, let $P$ be a box in $\mathbb{Z}_{N}^{k}$ of width at least $m \geqslant 2^{K^{2^{k}q} 2^{40 k^{3}+1}}$ and let $\mu_{1}, \ldots, \mu_{q}$ be $k$-linear functions from $P$ to $\mathbb{Z}_{N}$. Then $P$ can be partitioned into boxes $P_{1}, \ldots, P_{M}$, such that each $P_{j}$ has width at least $m^{K^{-2^{k}q}}$ and the diameter of $\mu_{i}\left(P_{j}\right)$ is at most $2 m^{-K^{-2^{k}q}} N$ for every $i$ and $j$.
