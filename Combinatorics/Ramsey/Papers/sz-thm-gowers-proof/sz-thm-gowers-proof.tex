...
% ed.: The summation variable is $x$, not the free symbol $s$.  The proof
% chooses $m=(16\pi r/\alpha)^{1/2}$, so the printed factor 8 is corrected too.
...
then there is a partition of $\{0,1, \ldots, r-1\}$ into $m \leqslant(16 \pi r / \alpha)^{1 / 2}$ arithmetic\\
...
Lemma 3.1. Let $d \geqslant 1$ and let $f$ be a function from $\mathbb{Z}_{N}$ to $D$. The following are equivalent.
...
\quad \cdot\left(\sum_{x \in \mathbb{Z}_{N}^{d-1}}\left|\sum_{s} \prod_{\epsilon \in\{0,1\}^{d-1}} C^{|\epsilon|} f_{\epsilon, 1}(s-\epsilon . x)\right|^{2}\right)^{\frac{1}{2}} .
\end{array}
\end{aligned}
$$
...
Corollary 5.6. Let $\phi: \mathbb{Z}_{N} \rightarrow \mathbb{Z}_{N}$ be any polynomial of degree $k$, let $K=(k!)^{2} 2^{(k+1)^{2}}$ and let $r$ be an integer exceeding $2^{2^{40 k^{2}}}$. Then for every integer $m$ with $r^{1-1 / K} \leqslant m \leqslant r$ the set $\{0,1,2, \ldots, r-1\}$ can be partitioned into arithmetic progressions $P_{1}, \ldots, P_{m}$ such that the diameter of $\phi\left(P_{j}\right)$ is at most $r^{-1 / K} N$ for every $j$ and the lengths of any two $P_{j}$ differ by at most 1 .
...
Corollary 5.7. Let $\phi: \mathbb{Z}_{N} \rightarrow \mathbb{Z}_{N}$ be a polynomial of degree $k$, let $K=$ $(k!)^{2} 2^{(k+1)^{2}}$, let $\alpha>0$ and let $r$ be an integer exceeding $\max \left\{2^{2^{40 k^{2}}},(4 \pi / \alpha)^{K}\right\}$. Then, for any integer $m$ with $r^{1-1 / K} \leqslant m \leqslant r$, there is a partition of the set $\{0,1, \ldots, r-1\}$ into arithmetic progressions $P_{1}, \ldots, P_{m}$ such that the sizes of the $P_{j}$ differ by at most one, and if $f: \mathbb{Z}_{N} \rightarrow D$ is any function such that
...
Lemma 5.9. Let $\phi_{1}, \ldots, \phi_{q}$ be polynomials from $\mathbb{Z}_{N}$ to $\mathbb{Z}_{N}$ of degree at most $k$, let $K=(k!)^{2} 2^{(k+1)^{2}}$ and let $r$ be an integer exceeding $2^{2^{40 k^{2}} K^{q-1}}$. Then for every integer $m$ with $r^{1-1 / 2 K^{q}} \leqslant m \leqslant r$ the set $\{0,1,2, \ldots, r-1\}$ can be partitioned into arithmetic progressions $P_{1}, \ldots, P_{m}$ such that the diameter of $\phi_{i}\left(P_{j}\right)$ is at most $r^{-1 / K^{q}} N$ for every $i$ and every $j$, and the lengths of any two $P_{j}$ differ by at most 1.
...
Lemma 12.5. Let $\eta>0$, let $B \subset \mathbb{Z}_{N}^{2}$ be a set of size $\beta N^{2}$ and let $\phi$ : $B \rightarrow \mathbb{Z}_{N}$ be a function that respects at least $\alpha \beta^{15} N^{32}$ 8-arrangements. If $N$ is sufficiently large (depending on $\alpha$, $\beta$ and $\eta$ ) then there is a subset $B^{\prime} \subset B$ containing at least $(\alpha \eta / 4)^{2^{36}} \beta^{15} N^{32}$ 8-arrangements, such that the proportion of 8-arrangements respected by $\phi$ is at least $1-\eta$.
...
Proof. By Lemma 12.4 there are at least $\beta^{112} \gamma^{336} N^{32}$ 8-arrangements respected by $\phi$. This allows us to take $\alpha=\beta^{97} \gamma^{336}$ in Lemma 12.5. It is not hard to check that $\left(\beta^{97} \gamma^{336} \eta / 4\right)^{2^{36}} \beta^{15} \geqslant 2^{-2^{37}} \beta^{2^{43}} \gamma^{2^{45}} \eta^{2^{36}}$, so the lemma is proved. \(\square\)
...
Therefore, if $\lambda$ is non-degenerate, the single-stage sum over $t$ and the $s_{A}$ is $2^{-2^{k+4}} N^{2^{k+1}+1}$ if $\phi$ does not respect $\lambda$ and $2^{-2^{k+4}} N^{2^{k+1}+1}\left(1+2.2^{-2^{k+4}}\right)$ if it does. It follows that the probability that $\lambda$ belongs to $B^{\prime}$ is $2^{-2^{k+4} r}$ if $\phi$ does not respect $\lambda$ and $2^{-2^{k+4} r}\left(1+2.2^{-2^{k+4}}\right)^{r}$ if it does. Therefore, our hypotheses imply that the expected number $X$ of 8-arrangements respected by $\phi$ is at least $2^{-2^{k+4} r}\left(1+2.2^{-2^{k+4}}\right)^{r} \alpha \beta^{15} N^{17 k+15}$, and the expected number $Y$ of non-degenerate 8-arrangements not respected by $\phi$ is at most $2^{-2^{k+4} r} \beta^{15} N^{17 k+15}$. Using the fact that
...
\begin{itemize}
  ...
  % ed.: Lemma 15.6 applied to $|A''|\geqslant\theta N^{k+1}/2$
  % gives the factor $\theta\gamma/2$, as the proof below states.
  \item[(ii)] $B$ contains at least $(\theta \gamma / 2)^{2^{2^{k+5}}} N^{17 k+15}$ 8-arrangements;
...
% ed.: The printed statement leaves $K$ and $q$ undefined and loses the
% nested exponent $2^{k+1}$ inherited from Lemma 16.1.
Lemma 16.6. Let $P=Q \times I$ be a box in $\mathbb{Z}_{N}^{k+1}$ of width at least $m$, let $t=\delta^{-2} s\left(\theta_{1} / 8, \delta, k\right)$, let $\sigma>0$, let $K=(k+1)^{2}2^{k+4}$ and let $q=q(\sigma/t,\delta,k)^{t}$. Let $l=(\zeta / 2) m^{c\left(t^{-1} \sigma, \delta, k\right)^{t} / \left(2 K^{2^{k+1} q}\right)}$. Then there is a subset $G \subset Q$ of size at least $(1-\sigma)|Q|$ and a partition of $P$ into boxes $S_{u}=T_{u} \times J_{u}$ of width at least $l$, such that, for every $u$ and every $h \in G \cap H_{1} \cap T_{u}$, the function from $J_{u}$ to $\mathbb{Z}_{N}$ defined by $x \mapsto \phi^{\prime}(h, x)$ is linear.
...
% ed.: The arguments of $q(\theta,\gamma,k)$ were reversed in print; $K$
% was omitted, and the exponent inherited from Lemma 16.6 was flattened.
Lemma 16.9. Let $\sigma>0$, let $t=\delta^{-2} s\left(\theta_{1} / 8, \delta, k\right)$, let $r=\left(2^{k}-1\right) \gamma^{-2} s\left(2^{-(k+2)} \theta, \gamma, k\right)$, let $q=q(\sigma / (2 r), \gamma, k)^{r}$ and let $K=(k+1)^{2}2^{k+4}$. Let $P=Q \times I$ be any box in $\mathbb{Z}_{N}^{k+1}$ of width at least $m$. Then there is a subset $E \subset P$ of size at least $(1-\sigma)|P|$ and a partition of $P$ into boxes $S_{u}=T_{u} \times J_{u}$ with the following property. Given $u$ and $h \in T_{u}$ let $\psi_{u, h}$ be the function $x \mapsto \phi_{1}(h, x)$, where the domain is the set of all $x$ such that $(h, x) \in B_{1} \cap E \cap S_{u}$. Then for every $u$ and $h \in T_{u}$ the graph of $\psi_{u, h}$ is contained in the union of the graphs of at most $q$ linear functions. The width of each box $S_{u}$ is at least

$$
l=(\zeta / 2) m^{c(\sigma / (2 r), \gamma, k)^{r} c(\sigma / (2 t), \delta, k)^{t} / \left(2 K^{2^{k+1}q}\right)}
$$
...
$$
\sigma\left(x_{1}, \ldots, x_{k}\right)=\sum_{\epsilon \in\{0,1\}^{k}}(-1)^{|\epsilon|} \phi_{\epsilon}(s-\epsilon . x) .
$$
...
$$
x_{k+1} \sigma(x)=\sum_{\epsilon \in\{0,1\}^{k+1}}(-1)^{|\epsilon|}\phi_{\epsilon}(s-\epsilon . x) .
$$
...
$$
\sum_{x \in \mathbb{Z}_{N}^{k}}\left|\Delta(f ; x)^{\wedge}(\sigma(x))\right|^{2}=\sum_{x \in \mathbb{Z}_{N}^{k+1}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{k+1}} C^{|\epsilon|}\left(f(s-\epsilon . x) \omega^{-\phi_{\epsilon}(s-\epsilon . x)}\right) .
$$
...
Theorem 18.1. Let $\alpha \leqslant 1 / 2$ and let $A \subset \mathbb{Z}_{N}$ be a set which fails to be $\alpha$-uniform of degree $k$. There exists a partition of $\mathbb{Z}_{N}$ into arithmetic progressions $P_{1}, \ldots, P_{M}$ of average size at least $N^{\alpha^{2^{2^{k+10}}}}$ such that
...
The mod- $N$ progressions $S_{i j}$ with $i \in I$, together with those $Q_{i}$ for which $i \notin I$, partition $\mathbb{Z}_{N}$. From the way we chose $I$, the average size of a cell in this partition is at least $r=(2 k)^{-1} p^{\left(2^{-(k+4)^{3}} \beta / 2 k\right)^{2^{2^{k+9}}}}$. By Lemma 5.13 we can find a refinement of this partition into proper arithmetic progressions of average size at least $r^{1 / 2} / 4$. Let us call these progressions $T_{1}, \ldots, T_{L}$. Since $\sum_{i \in I}\left|Q_{i}\right| \geqslant 2^{-2(k+4)^{3}} \beta N$ we have the inequality
