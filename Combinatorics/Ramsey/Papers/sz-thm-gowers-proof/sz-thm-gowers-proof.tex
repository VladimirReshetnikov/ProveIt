...
Let us define functions $j_{1}$ and $j_{2}$ (from $\{0,1\}^{k}$ to $\mathbb{Z}_{M}$ ) to be equivalent if they are translations of each other. Corollary 17.5 implies that the number of equivalence classes is at most $2^{k(k+1)^{2}}$. Let us call them $J_{1}, \ldots, J_{L}$. By the Cauchy-Schwarz inequality, we can deduce from our calculations above that
