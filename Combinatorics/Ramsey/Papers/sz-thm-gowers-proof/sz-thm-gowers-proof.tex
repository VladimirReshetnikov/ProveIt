...
Moreover, if $t \geqslant 2^{2^{40 k^{3}}}$, then the above inequality is valid with $\epsilon=1 / k 2^{k+1}$ and $C_{\epsilon}=1000$. \(\square\)

% ed.: Vaughan's cited proof absorbs a generalized-divisor multiplicity into
% an unspecified t^epsilon and does not justify the printed uniform constants
% at the original 2^(2^(32k^2)) threshold.  Keeping that multiplicity explicit
% with divisor parameter m=8k^3 proves the stated constant 1000 at the safe
% threshold 2^(2^(40k^3)).  The dependent thresholds below are raised
% coherently; the displayed 500-bound is retained as the printed claim.
...
Lemma 5.5. Let $k \geqslant 2$, let $t \geqslant 2^{2^{40 k^{3}}}$, let $N \geqslant t$ and let $a \in \mathbb{Z}_{N}$. Then there exists $p$ with $1\leqslant p \leqslant t$ such that $\left|p^{k} a\right| \leqslant t^{-1 / k 2^{k+1}} N$.\\
...
% ed.: With the displayed constants, the claimed bound
% q <= 2 t^(1/(2k)) does not follow: at that endpoint the q^{-1} term
% has exactly the critical t-exponent, so the factor 2000 cannot be absorbed
% by making t larger.  The proof-supported consequence is q <= t^(1/k).
% Together with the collision-safe weighted Fourier argument (which gives
% |r| <= 16 t^(1/(k 2^k))), this is sufficient for the stated Lemma 5.5.
which implies, after a small calculation (using the assumption that $t \geqslant 2^{2^{40 k^{3}}}$), that $q \leqslant t^{1/k}$.

We may now argue directly. We know that $|\alpha-b / q| \leqslant 1 / q t$. Multiplying both sides by $(r q)^{k} N / r$ we find that

$$
\left|-a(r q)^{k}-b(r q)^{k-1} N\right| \leqslant(|r| q)^{k-1} N / t \leqslant t^{-1 / k 2^{k+1}} N,
$$

so we can set $p=|r| q$ (the harmless possible sign is absorbed by modular
absolute value), contradicting the initial assumption that the result was
false.  Here the fixed factors $16$ and $16^{k-1}$, as well as the inequalities
$p\leqslant t$ and the final displayed exponent, are absorbed by the stated
double-exponential threshold. \(\square\)

% ed.: The preceding contradiction proves Lemma 5.5 but does not logically
% yield the stronger p <= sqrt(t) assertion used below: its hypothesis excludes
% every recurrence up to t, whereas the negation of the stronger assertion
% excludes only recurrences up to sqrt(t).  A direct repetition of the weighted
% Fourier/Weyl argument with u=floor(sqrt(t)) does prove the stronger assertion,
% with target scale t^{-1/(k 2^{k+1})}, provided u itself meets the Weyl
% threshold.  We therefore use the proof-supported squared threshold
% t >= (2^{2^{40 k^3}})^2 = 2^{2^{40 k^3+1}} whenever the square-root
% strengthening is invoked.  Quantitatively, the direct proof obtains
% |r| <= 64 u^{4/(k2^{k+1})}, q <= u^{1/k}, and p=|r|q <= u; its remaining
% exponent margin absorbs the factor 4*64^{k-1} and gives
% (N/u)p^{k-1} <= t^{-1/(k2^{k+1})}N.
...
% on $p$ comes from the direct auxiliary argument above, not from Lemma 5.5's
% existential conclusion.
Proof. The case $k=1$ follows immediately from Lemma 2.3. Given $k>1$, let us write $\phi(x)=a x^{k}+\psi_{1}(x)$, in such a way that $\psi_{1}$ is a polynomial of degree $k-1$. By the square-root auxiliary to Lemma 5.5 described above, we can find $p$ with $p^{2}\leqslant r$ such that $\left|a p^{k}\right| \leqslant$ $r^{-1 / k 2^{k+1}} N$. Then for any $s$ we have
...
% The extra factor 2 in the base of the threshold is rounding slack: at every
% refinement a cell may have length one less than its integral target.
Lemma 5.9. Let $\phi_{1}, \ldots, \phi_{q}$ be polynomials from $\mathbb{Z}_{N}$ to $\mathbb{Z}_{N}$ of degree at most $k$, let $K=(k!)^{2} 2^{(k+1)^{2}}$ and let $r\leqslant N$ be an integer exceeding $2^{\left(2^{40 k^{3}+1}+1\right) K^{q-1}}$. Then for every integer $v$ with $1 \leqslant v \leqslant r^{1 /(2 K^{q})}$ the set $\{0,1,2, \ldots, r-1\}$ can be partitioned into a positive number of nonempty arithmetic progressions $P_{1}, \ldots, P_{M}$, each of length $v-1$ or $v$, such that the diameter of $\phi_{i}\left(P_{j}\right)$ is at most $r^{-1 / K^{q}} N$ for every $i$ and every $j$.

Proof. First we prove by induction that for every $p \leqslant q$ we can partition the set $\{0,1, \ldots, r-1\}$ into some positive number of nonempty arithmetic progressions of the required root scale such that $\operatorname{diam} \phi_{i}\left(P_{j}\right)$ is at most $r^{-1 / K^{p}} N$ for every $i \leqslant p$ and every cell $P_j$. When $p=1$ this follows immediately from Corollary 5.6. If we know it for $p-1$, let $Q_{1}, \ldots, Q_{l}$ be the arithmetic progressions obtained. Choose the integral target length for the next application of Corollary 5.6 from the corresponding $K$-th root.  Its cells may have that length minus one; the repaired threshold above dominates the recursive budget $R_{0}=2^{2^{40 k^{3}+1}}$, $R_{n+1}=(R_{n}+2)^{K}$, so every $Q_i$ is still strictly above the one-polynomial threshold when it is refined.  The diameter estimate is at most $r^{-1 / K^{p}} N$. This gives the induction, with ample rounding slack for the final hypothesis $v\leqslant r^{1/(2K^q)}$.
...
% ed.: The additive $+1$ next to $2^{40k^3+1}$ is the same integral-rounding
% slack as in Lemma 5.9; the inner exponent $40k^3+1$ itself comes from the
% squared polynomial-recurrence threshold.
Lemma 5.10. Let $k \geqslant 2$, let $K=k^{2} 2^{k+3}$, let $m \geqslant 2^{\left(2^{40 k^{3}+1}+1\right)K^{2^{k}}}$, let $P$ be a box in $\mathbb{Z}_{N}^{k}$ of width at least $m$ and let $\mu$ be a $k$-linear function from $P$ to $\mathbb{Z}_{N}$. Then $P$ can be partitioned into boxes $P_{1}, \ldots, P_{M}$, such that each $P_{j}$ has width at least $m^{K^{-2^{k}}}$ and the diameter of $\mu\left(P_{j}\right)$ is at most $2 m^{-K^{-2^{k}}} N$ for every $j$.\\
Proof. As noted above, $\mu$ can be written as a sum of terms of the form $c_{A} \prod_{j \in A} x_{j}$. Take any total ordering on the subsets of $[k]$ which extends the partial ordering by inclusion, and define the height of $\mu$ to be the largest position in this ordering of a set $A$ such that the coefficient $c_{A}$ is non-zero. We shall prove the result by induction on the height. The precise inductive hypothesis is that if $\mu$ has height at most $p$, then any box $Q$ of width $t \geqslant 2^{\left(2^{40 k^{3}+1}+1\right)K^{p}}$ can be partitioned into boxes $Q_{j}$ of width at least $t^{K^{-p}}$ such that for every $j$ the diameter of $\mu\left(Q_{j}\right)$ is at most $\left(1+2^{-k} p\right) t^{-K^{-p}} N$.
...
By the square-root auxiliary to Lemma 5.5, we can find $r$ with $r^{2}\leqslant t$, such that, setting $d=r d_{0}$, we have the inequality $\left|c_{A} d^{|A|}\right| \leqslant t^{-1 / k 2^{k+2}} N$. Now, for any $\left(x_{1}, \ldots, x_{k}\right) \in Q$ we can define a function $\nu$ by
...
Corollary 5.11. Let $k \geqslant 2$, let $K=k^{2} 2^{k+3}$, let $P$ be a box in $\mathbb{Z}_{N}^{k}$ of width at least $m \geqslant 2^{\left(2^{40 k^{3}+1}+1\right)K^{2^{k}q}}$ and let $\mu_{1}, \ldots, \mu_{q}$ be $k$-linear functions from $P$ to $\mathbb{Z}_{N}$. Then $P$ can be partitioned into boxes $P_{1}, \ldots, P_{M}$, such that each $P_{j}$ has width at least $m^{K^{-2^{k}q}}$ and the diameter of $\mu_{i}\left(P_{j}\right)$ is at most $2 m^{-K^{-2^{k}q}} N$ for every $i$ and $j$.
...
% ed.: The conclusion has positive formal size, so the empty set is an
% immediate counterexample unless the paper's implicit convention $m>0$ is
% made explicit.
Theorem 7.1. Let $C$ be a constant. There exist constants $d_{0}$ and $K$ depending only on $C$ such that whenever $A$ is a nonempty subset of $\mathbb{Z}$ with $|A|=m$ and $|A+A| \leqslant C m$, there exist $d \leqslant d_{0}$, an integer $x_{0}$ and positive integers $x_{1}, \ldots, x_{d}$ and $k_{1}, \ldots, k_{d}$ such that $k_{1} k_{2} \ldots k_{d} \leqslant K m$ and
...
% ed.: As in Theorem 7.1, nonemptiness is necessary: for $A=\varnothing$
% the hypothesis is vacuous while a positive-size progression cannot have
% size at most $K|A|=0$.
Theorem 7.2. Let $A$ be a nonempty subset of $\mathbb{Z}^{D}$ of cardinality $m$ and suppose that $\|A * A\|_{2}^{2} \geqslant c_{0} m^{3}$. Then there are constants $c, K$ and $d_{0}$ depending only on $c_{0}$ and an arithmetic progression $P$ of dimension $d \leqslant d_{0}$ and size at most $K m$ such that $|A \cap P| \geqslant c m$.
...
% hypotheses used in Lemmas 13.4--13.10.  The proof below also retains the
% Fourier-threshold losses omitted in the printed fibre-restriction argument.
% The resulting weaker arrangement bound still gives the stated set-size
% bound via the sharp estimate that the number of 8-arrangements in $A$ is at
% most $|A|^{15}N^2$.
...
Proof. By Lemma 3.1 (using the implications (vii)$\Rightarrow$(vi)$\Rightarrow$(iii)$\Rightarrow$(ii)) there is a set $A_{0} \subset \mathbb{Z}_{N}^{2}$ of size at least $\alpha N^{2} / 2$ and a function $\phi: A_{0} \rightarrow \mathbb{Z}_{N}$ such\\
...
Put $a=\alpha/2$.  Fixing $k$, the Fourier lower bound gives

$$
\sum_{l\in A_{0,k}}\left|\Delta(f;k,l)^{\wedge}(\phi_k(l))\right|^2
 \geqslant \alpha_k a^2N^3.
$$

Thus Proposition 6.1, applied to $\Delta(f;k)$ with parameter
$\tau_k=\alpha_k a^2$, gives at least
$\tau_k^4N^3=\alpha_k^4a^8N^3$ $\phi_k$-additive quadruples.  Corollary 7.6
must therefore be applied with density $\alpha_k$ and additive parameter
$\gamma_k=\alpha_ka^8$.  It produces $A_{1,k}\subset A_{0,k}$ of size at
least

$$
2^{-1882}a^{9312}\alpha_k^{1165}N
$$

on which $\phi_k$ is a Freiman homomorphism of order 8.  Since the average
of the $\alpha_k$ is at least $a$, convexity shows that the union $A_1$ of
these sets has density at least $2^{-1882}a^{10477}$.  As $a\leqslant1/2$,
this is at least $a^{12359}$.  The restriction of $\phi(k,l)$ to $A_1$ is a
homomorphism of order 8 in $l$ for every fixed $k$.

Repeating the corrected argument in the second variable, and using that
subsets preserve the Freiman property already obtained in the first
variable, gives a further subset $A_2\subset A_1$ whose density is at least

$$
2^{-1882}a^{9312}(a^{12359})^{1165}
 \geqslant a^{14409429}\geqslant a^{2^{24}}.
$$

The restriction of $\phi$ to $A_2$ is therefore a homomorphism of order 8 in
each variable separately.  Let $\beta=a^{2^{24}}$.

We now apply Lemma 12.6 with $B=A_{2}, \gamma=\alpha / 2$ and $\eta=2^{-44}$. This yields a set $A$ with at least

$$
2^{-2^{37}} \beta^{2^{43}} \gamma^{2^{45}} \eta^{2^{36}} N^{32}
 \geqslant a^{2^{68}}N^{32}
$$

8-arrangements, such that the proportion respected by $\phi$ is at least
$1-2^{-44}$.  Here the displayed estimate follows from
$2^{-2^{37}}\geqslant a^{2^{37}}$, $\eta^{2^{36}}\geqslant
a^{44\cdot2^{36}}$, and
$2^{67}+2^{45}+44\cdot2^{36}+2^{37}<2^{68}$.  Finally, the number of
8-arrangements in any $A$ is at most $|A|^{15}N^2$.  If
$|A|<a^{2^{66}}N^2$, this upper bound is strictly less than
$a^{15\cdot2^{66}}N^{32}\leqslant a^{2^{68}}N^{32}$, a contradiction.
Thus $|A|\geqslant(\alpha/2)^{2^{66}}N^2$, and the lemma is proved. \(\square\)
...
Lemma 16.1. Let $k$ be an integer, let $K=(k+1)^{2} 2^{k+4}$ and let $m \geqslant$ $2^{\left(2^{40(k+1)^{3}+1}+1\right)K^{2^{k+1} q}}$. Let $P$ be a box in $\mathbb{Z}_{N}^{k}$ of width at least $m$, and let $\mu_{1}, \ldots, \mu_{q}$ be $k$-linear functions defined on $P$. Then $P$ can be partitioned into boxes $P_{1}, \ldots, P_{M}$ of width at least $m^{K^{-2^{k+1} q}}$ with the following property. For every $i$ and $j$ and every $x \in P_{j}$ we have the inequality $\left|\mu_{i}(x) d_{j}\right| \leqslant$ $2 m^{-K^{-2^{k+1} q}} N$, where $d_{j}$ is the common difference of the box $P_{j}$.\\
