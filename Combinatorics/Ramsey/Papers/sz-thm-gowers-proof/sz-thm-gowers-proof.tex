...
It is natural to wonder how quickly the least such $M$ grows as a function of $k$ and $r$, but this has turned out to be a surprisingly difficult question. The original proof of van der Waerden bounds $M$ above by an Ackermanntype function in $k$, even when $r=2$, and it was a major advance when Shelah, in 1987, gave the first primitive recursive upper bound (with a beautifully transparent proof). His bound can be described as follows. Define a tower function $T$ inductively by letting $T(1)=2$ and $T(k)=2^{T(k-1)}$ for $k>1$. Then define a function $W$ by $W(1)=2$ and $W(k)=T(W(k-1))$ for $k>1$. Shelah obtained a bound of the form $M(k, 2) \leqslant W(C k)$ (with $C$ an absolute constant). Although this was a huge improvement on the previous bound, it still left an enormous gap, as the best known lower bound was, and still is, exponential in $k$.
...
A second proof of Szemerédi's theorem was given by Furstenberg in 1977, using ergodic theory, which provides an extremely useful conceptual framework for discussing the result. This proof was also a major breakthrough, partly because of the difficulty of Szemerédi's original proof, and partly because Furstenberg's techniques have since been extended to prove many natural generalizations of the theorem which do not seem to follow from Szemerédi's approach. These include a density version of the HalesJewett theorem [FK] and a "polynomial Szemerédi theorem" [BL].
...
\sum_{r} \hat{f}(r) \hat{\hat{g}(r)}=N \sum_{s} f(s) \overline{g(s)},  \tag{2}\\
...
For technical reasons it is also useful to consider functions of mean zero. Given a set $A$ of cardinality $\delta N$, let us define the balanced function of $A$ to be $f_{A}: \mathbb{Z}^{N} \rightarrow[-1,1]$ where
...
Corollary 2.4. Let $f$ be a function from the set $\{0,1, \ldots, r-1\}$ to the closed unit disc in $\mathbb{C}$, let $\phi: \mathbb{Z}_{N} \rightarrow \mathbb{Z}_{N}$ be linear and let $\alpha>0$. If
...
and such that the lengths of the $P_{j}$ all lie between $(\alpha r / \pi)^{\frac{1}{2}} / 4$ and $(\alpha r / \pi)^{\frac{1}{2}} / 2$.\\
Proof. Let $s \leqslant \alpha N / 4 \pi$ and let $m=(16 \pi r / \alpha)^{1 / 2}$. By Lemma 2.3 we can find a partition of $\{0,1, \ldots, r-1\}$ into arithmetic progressions $P_{1}, \ldots, P_{m}$ such that the diameter of $\phi\left(P_{j}\right)$ is at most $s$ for every $j$ and the length of each $P_{j}$ lies between $r / m$ and $2 r / m$. By the triangle inequality,
...
Proof. Define $\phi(x)=r x$ and let $f$ be the balanced function of $A$ (regarded as a function on $\{0,1, \ldots, N-1\}$ ). By Corollary 2.4 we can partition the set $\{0,1, \ldots, N-1\}$ into $m \leqslant(16 \pi N / \alpha)^{1 / 2}$ arithmetic progressions $P_{1}, \ldots, P_{m}$ of lengths between $N / m$ and $2 N / m$ such that
...
Therefore, we can find $j$ such that $\sum_{x \in P_{j}} f(x) \geqslant \alpha N / 4 m$. But $\left|P_{j}\right| \leqslant 2 N / m$, so $\sum_{x \in P_{j}} f(x) \geqslant \alpha\left|P_{j}\right| / 8$, which implies that $\left|A \cap P_{j}\right| \geqslant(\delta+\alpha / 8)\left|P_{j}\right|$. \(\square\)
...
Averaging arguments show that (iii) implies (vi) as long as $c_{1} \leqslant c_{2}^{2}$, and that (vi) implies (iii) as long as $c_{1} \geqslant 2 c_{2}$. Finally, the equivalence of (i) and (ii) in Lemma 2.1 shows that in this lemma (vi) implies (vii) if $c_{3} \geqslant c_{2}^{1 / 4}$ and (vii) implies (vi) if $c_{2} \geqslant c_{3}$. \(\square\)
...
Theorem 3.2. Let $k \geqslant 2$ and let $f_{1}, \ldots, f_{k}$ be functions from $\mathbb{Z}_{N}$ to $D$ such that $f_{k}$ is $\alpha$-uniform of degree $k-2$. Then
...
Since $\Delta\left(f_{k} ;(k-1) u\right)$ is $\alpha((k-1) u)$-uniform of degree $k-3$, our inductive hypothesis implies that this is at most $N \sum_{u} \alpha((k-1) u)^{1 / 2^{k-2}} N^{2}$, and since $\sum_{u} \alpha((k-1) u) \leqslant \alpha N$, this is at most $\alpha^{1 / 2^{k-2}} N^{4}$, which proves the result for $k$. \(\square\)
...
Corollary 3.3. Let $A_{1}, \ldots, A_{k}$ be subsets of $\mathbb{Z}_{N}$, such that $A_{i}$ has cardinality $\delta_{i} N$ for every $i$, and is $\alpha^{2^{i-1}}$-uniform of degree $i-2$ for every $i \geqslant 3$. Then
...
Lemma 3.5. Let $A$ be an $\alpha$-uniform subset of $\mathbb{Z}_{N}$ of cardinality $\delta N$, and let $P$ be an interval of the form $\{a+1, \ldots, a+M\}$, where $M=\beta N$. Then $||A \cap P|-\beta \delta N| \leqslant \alpha^{1 / 4} N$.
...
This estimate implies that $\sum_{r \neq 0}|\hat{P}(r)|^{4 / 3} \leqslant N^{4 / 3}$. Therefore,
...
& \leqslant\left(\sum_{r \neq 0}|\hat{A}(r)|^{4}\right)^{1 / 4} \leqslant \alpha^{1 / 4} N,
\end{aligned}
$$

using property (iv) of Lemma 3.1. \(\square\)

Corollary 3.6. Let $A \subset \mathbb{Z}_{N}$ be $\alpha$-uniform of degree $k-2$ and have cardinality $\delta N$. If $\alpha \leqslant(\delta / 2)^{k 2^{k}}$ and $N \geqslant 32 k^{2} \delta^{-k}$, then $A$ contains an arithmetic progression of length $k$.

Proof. Let $A_{1}=A_{2}=A \cap[(k-2) N /(2 k-3),(k-1) N /(2 k-3)]$, and let $A_{3}=\cdots=A_{k}=A$. By Lemma $3.4 A$ is $\alpha^{1 / 2^{k-3}}$-uniform (of degree one), so by Lemma 3.5 the sets $A_{1}$ and $A_{2}$ both have cardinality at least $\delta N / 4 k$ since, by the first inequality we have assumed, we know that $\alpha^{1 / 2^{k-1}} \leqslant \delta / 4 k$.

Therefore, by Corollary 3.3, $A$ contains at least $\left(\left(\frac{\delta^{k}}{16 k^{2}}\right)-2^{k} \alpha^{1 / 2^{k-1}}\right) N^{2}$ arithmetic progressions modulo $N$ with the first two terms belonging to the interval $[(k-2) N /(2 k-3),(k-1) N /(2 k-3)]$. The only way such a progression can fail to be genuine is if the common difference is zero, and there are at most $\delta N$ such degenerate progressions. Thus the corollary is proved, since the two inequalities we have assumed imply that $\left(\delta^{k} / 16 k^{2}\right)-2^{k} \alpha^{1 / 2^{k-1}} \geqslant \delta^{k} / 32 k^{2}$ and $\delta^{k} N^{2} / 32 k^{2}>\delta N$. \(\square\)
...
The next lemma is little more than the Cauchy-Schwarz inequality and some notation. It will be convenient to use abbreviations such as $x$ for $\left(x_{1}, \ldots, x_{k}\right)$ and $x . y$ for $\sum_{i=1}^{k} x_{i} y_{i}$. If $\epsilon \in\{0,1\}^{k}$ then we shall write $|\epsilon|$ for $\sum_{i=1}^{k} \epsilon_{i}$. Once again, $C$ is the operation of complex conjugation.

Lemma 3.8. For every $\epsilon \in\{0,1\}^{k}$ let $f_{\epsilon}$ be a function from $\mathbb{Z}_{N}$ to $D$. Then

$$
\left|\sum_{x \in \mathbb{Z}_{N}^{d}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{d}} C^{|\epsilon|} f_{\epsilon}(s-\epsilon . x)\right| \leqslant \prod_{\epsilon \in\{0,1\}^{d}}\left|\sum_{x \in \mathbb{Z}_{N}^{d}} \sum_{s} \prod_{\eta \in\{0,1\}^{d}} C^{|\eta|} f_{\epsilon}(s-\eta . x)\right|^{\frac{1}{2^{d}}} .
$$
...
$$
\begin{aligned}
& \left|\sum_{x \in \mathbb{Z}_{N}^{d}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{d}} C^{|\epsilon|} f_{\epsilon}(s-\epsilon . x)\right| \\
& =\left|\sum_{x \in \mathbb{Z}_{N}^{d-1}}\left(\sum_{s} \prod_{\epsilon \in\{0,1\}^{d-1}} C^{|\epsilon|} f_{\epsilon, 0}(s-\epsilon . x)\right)\left(\sum_{t} \prod_{\epsilon \in\{0,1\}^{d-1}} C^{|\epsilon|} f_{\epsilon, 1}(t-\epsilon . x)\right)\right| \\
& \begin{array}{r}
\leqslant\left(\sum_{x \in \mathbb{Z}_{N}^{d-1}}\left|\sum_{s} \prod_{\epsilon \in\{0,1\}^{d-1}} C^{|\epsilon|} f_{\epsilon, 0}(s-\epsilon . x)\right|^{2}\right)^{\frac{1}{2}} \\
\quad \cdot\left(\sum_{x \in \mathbb{Z}_{N}^{d}}\left|\sum_{s} \prod_{\epsilon \in\{0,1\}^{d-1}} C^{|\epsilon|} f_{\epsilon, 1}(s-\epsilon . x)\right|^{2}\right)^{\frac{1}{2}} .
\end{array}
\end{aligned}
$$
...
$$
\sum_{x \in \mathbb{Z}_{N}^{d-1}}\left|\sum_{s} \prod_{\epsilon \in\{0,1\}^{d-1}} C^{|\epsilon|} f_{\epsilon, 0}(s-\epsilon . x)\right|^{2}=\sum_{x \in \mathbb{Z}_{N}^{d}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{d}} C^{|\epsilon|} f_{P_{d}(\epsilon)}(s-\epsilon . x)
$$
...
$$
\|f\|_{d}=\left|\sum_{x \in \mathbb{Z}_{N}^{d}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{d}} C^{|\epsilon|} f(s-\epsilon . x)\right|^{1 / 2^{d}} .
$$
...
If we expand the product we obtain $2^{2^{d}}$ terms of the form $\prod_{\epsilon \in\{0,1\}^{d}} C^{|\epsilon|} f_{\epsilon}(s-\epsilon . x)$, where each function $f_{\epsilon}$ is either $f$ or $g$. For each one of these terms, if we take the sum over $x_{1}, \ldots, x_{d}$ and $s$ and apply
...
Lemma 5.5. Let $k \geqslant 2$, let $t \geqslant 2^{2^{32 k^{2}}}$, let $N \geqslant t$ and and let $a \in \mathbb{Z}_{N}$. Then there exists $p \leqslant t$ such that $\left|p^{k} a\right| \leqslant t^{-1 / k 2^{k+1}} N$.\\
...
Corollary 5.6. Let $\phi: \mathbb{Z}_{N} \rightarrow \mathbb{Z}_{N}$ be any polynomial of degree $k$, let $K=(k!)^{2} 2^{(k+1)^{2}}$ and let $r$ be an integer exceeding $2^{2^{40 k^{2}}}$. Then for every $m \geqslant r^{1-1 / K}$ the set $\{0,1,2, \ldots, r-1\}$ can be partitioned into arithmetic progressions $P_{1}, \ldots, P_{m}$ such that the diameter of $\phi\left(P_{j}\right)$ is at most $r^{-1 / K} N$ for every $j$ and the lengths of any two $P_{j}$ differ by at most 1 .

Proof. The case $k=1$ follows immediately from Lemma 2.2. Given $k>1$, let us write $\phi(x)=a x^{k}+\psi_{1}(x)$, in such a way that $\psi_{1}$ is a polynomial of degree $k-1$. By Lemma 5.5 we can find $p \leqslant r^{1 / 2}$ such that $\left|a p^{k}\right| \leqslant$ $r^{-1 / k 2^{k+2}} N$. Then for any $s$ we have
...
where $\psi_{2}$ is, for any fixed $x$, a polynomial of degree at most $k-1$ in $p$.\\
For any $u$, the diameter of the set $\left\{s^{k}\left(a p^{k}\right): 0 \leqslant s<u\right\}$ is at most $u^{k}\left|a p^{k}\right| \leqslant u^{k} r^{-1 / k 2^{k+2}} N$. Therefore, for any $u \leqslant r^{1 / 4}$, we can partition the set $\{0,1, \ldots, r-1\}$ into arithmetic progressions of the form
...
Corollary 5.7. Let $\phi: \mathbb{Z}_{N} \rightarrow \mathbb{Z}_{N}$ be a polynomial of degree $k$, let $K=$ $(k!)^{2} 2^{(k+1)^{2}}$, let $\alpha>0$ and let $r$ be an integer exceeding $\max \left\{2^{2^{40 k^{2}}},(4 \pi / \alpha)^{K}\right\}$. Then, for any $m \geqslant r^{1-1 / K}$, there is a partition of the set $\{0,1, \ldots, r-1\}$ into arithmetic progressions $P_{1}, \ldots, P_{m}$ such that the sizes of the $P_{j}$ differ by at most one, and if $f: \mathbb{Z}_{N} \rightarrow D$ is any function such that
...
Proof. By Corollary 5.6 we can choose $P_{1}, \ldots, P_{m}$ such that $\operatorname{diam}\left(\phi\left(P_{j}\right)\right) \leqslant$ $N r^{-1 / K}$ for every $j$. By the second lower bound for $r$, this is at most $\alpha N / 4 \pi$. Exactly as in the proof of Corollary 2.3, this implies the result. \(\square\)
...
Suppose also that $\left|P_{i}\right| \leqslant 2\left|P_{j}\right|$ for all $i, j$. Then there is an arithmetic progression $Q$ of cardinality at least $(N / M)^{1 / K} / 8$ such that $|A \cap Q| \geqslant$ $(\delta+\alpha / 8)|Q|$.
...
$$
\sum_{i=1}^{M} \sum_{j=1}^{m}\left|\sum_{s \in P_{i j}} f(s)\right| \geqslant \alpha N .
$$

Since $A$ is contained in the union of the $P_{i j}$ we also know that

$$
\sum_{i=1}^{M} \sum_{j=1}^{m} \sum_{s \in P_{i j}} f(s)=0 .
$$
...
Lemma 5.9. Let $\phi_{1}, \ldots, \phi_{q}$ be polynomials from $\mathbb{Z}_{N}$ to $\mathbb{Z}_{N}$ of degree at most $k$, let $K=(k!)^{2} 2^{(k+1)^{2}}$ and let $r$ be an integer exceeding $2^{2^{40 k^{2}} K^{q-1}}$. Then for every $m \geqslant r^{1-1 / 2 K^{q}}$ the set $\{0,1,2, \ldots, r-1\}$ can be partitioned into arithmetic progressions $P_{1}, \ldots, P_{m}$ such that the diameter of $\phi_{i}\left(P_{j}\right)$ is at most $r^{-1 / K^{q}} N$ for every $i$ and every $j$, and the lengths of any two $P_{j}$ differ by at most 1.

Proof. First we prove by induction that for every $p \leqslant q$ we can partition the set $\{0,1, \ldots, r-1\}$ into arithmetic progressions $P_{1}, \ldots, P_{m}$ of size at least $r^{1 / K^{p}}$ such that $\operatorname{diam} \phi_{i}\left(P_{j}\right)$ is at most $r^{-1 / K^{p}} N$ for every $i \leqslant p$ and $j \leqslant m$. When $p=1$ this follows immediately from Corollary 5.6. If we know it for $p-1$, let $Q_{1}, \ldots, Q_{l}$ be the arithmetic progressions obtained. The size of each $Q_{i}$ is at least $r^{1 / K^{p-1}} \geqslant 2^{2^{40 k^{2}}}$, so by Corollary 5.6 each $Q_{i}$ can be partitioned into further arithmetic progressions $P_{j}$ of cardinality at least $\left(r^{1 / K^{p-1}}\right)^{1 / K}=r^{1 / K^{p}}$, such that, for every $j$, the diameter of $\phi_{q}\left(P_{j}\right)$ is at most $\left(r^{-1 / K^{p-1}}\right)^{1 / K} N=r^{-1 / K^{p}} N$. This is clearly enough to give us the statement for $p$.

In particular, we have the statement when $p=q$. To obtain the lemma, notice that if $k^{2} \leqslant m$, then an arithmetic progression of length $m$ can be partitioned into subprogressions each of which has length $k$ or $k+1$. \(\square\)
...
Corollary 5.11. Let $k \geqslant 2$, let $K=k^{2} 2^{k+3}$, let $P$ be a box in $\mathbb{Z}_{N}^{k}$ of width at least $m \geqslant 2^{K^{2}{ }^{k} 2^{32 k^{2}+1}}$ and let $\mu_{1}, \ldots, \mu_{q}$ be $k$-linear functions from $P$ to $\mathbb{Z}_{N}$. Then $P$ can be partitioned into boxes $P_{1}, \ldots, P_{M}$, such that each $P_{j}$ has width at least $m^{K^{-2^{k}} q}$ and the diameter of $\mu_{i}\left(P_{j}\right)$ is at most $2 m^{-K^{-2^{k} q}} N$ for every $i$ and $j$.
...
Proof. Let $Q_{i}$ have cardinality $m_{i}$. By Lemma 18.1, one can partition $Q_{i}$ into at most $4 m_{i}^{1 / 2}$ proper arithmetic progressions. Since $m_{1}+\ldots+m_{M}=N$, the Cauchy-Schwarz inequality tells us that $4\left(m_{1}^{1 / 2}+\ldots+m_{M}^{1 / 2}\right) \leqslant 4 \sqrt{M N}$. \(\square\)

Lemma 5.14. Let $\phi: \mathbb{Z}_{N} \rightarrow \mathbb{Z}_{N}$ be a polynomial of degree $k$ and let $K=(k!)^{2} 2^{k^{2}}$. Let $f: \mathbb{Z}_{N} \rightarrow[-1,1]$ and let $Q_{1}, \ldots, Q_{M}$ be arithmetic progressions such that

$$
\sum_{i=1}^{M}\left|\sum_{s \in Q_{i}} f(s) \omega^{-\phi(s)}\right| \geqslant \alpha N
$$

There is a refinement of $Q_{1}, \ldots, Q_{M}$ consisting of arithmetic progressions $R_{1}, \ldots, R_{L}$ such that $L \leqslant C M^{1 / K} N^{1-1 / K}$ and
...
$$
\sum\left\{a_{j}: a_{j}<\alpha\left|Q_{j}\right| / 4\right\}<\alpha N / 4
$$

and

$$
\sum\left\{a_{j}:\left|Q_{j}\right|<\alpha N / 4 m\right\}<\alpha N / 4
$$

(as $a_{j} \leqslant\left|Q_{j}\right|$ ) so there must be other values of $j$ contributing to $\sum_{j=1}^{M} a_{j}$. This proves the lemma. \(\square\)
...
$$
\sum_{u, r}\left|\hat{g}_{u}(r)\right|^{4}=\sum_{u, r}\left|\sum_{k \in B} \omega^{\phi(s) u-r s}\right|^{4} \geqslant \alpha^{4} N^{5} .
$$
...
Lemma 7.5. Let $B \subset \mathbb{Z}_{N}$ and let $\phi: B \rightarrow \mathbb{Z}_{N}$ be a function with graph $\Gamma$. Suppose that $|\Gamma-\Gamma| \leqslant C|\Gamma|$. Then there is a subset $B^{\prime} \subset B$ of size at least $|B| / 8 k C^{4 k}$ such that the restriction of $\phi$ to $C$ is a Freiman homomorphism of order $k$.

Proof. First, a theorem of Ruzsa [Ru2] (deduced from a result of Plünnecke [P] for which Ruzsa discovered a simpler proof) implies that $|4 k \Gamma-4 k \Gamma| \leqslant$ $C^{4 k}|\Gamma|$. If for some $x$ we could find more than $C^{4 k}$ distinct values of $y$ such\\
that $(x, y) \in 2 k \Gamma-2 k \Gamma$, then for every $(z, w) \in 2 k \Gamma-2 k \Gamma$ there would be more than $C^{4 k}$ distinct values of $u$ such that $(z-x, u) \in 4 k \Gamma-4 k \Gamma$. But the number of $z$ such that $(z, w) \in 2 k \Gamma-2 k \Gamma$ for some $w$ is certainly at least $|\Gamma|$, so this would contradict the upper bound for $|4 k \Gamma-4 k \Gamma|$.

Therefore, there are in particular at most $C^{4 k}$ distinct values of $y$ such that $(0, y) \in 2 k \Gamma-2 k \Gamma$. If $(x, y),\left(x, y^{\prime}\right) \in k \Gamma-k \Gamma$ then $\left(0, y-y^{\prime}\right) \in 2 k \Gamma-2 k \Gamma$. Hence, there is a set $K$ of size at most $C^{4 k}$ such that, writing $K_{x}$ for the set $\{y:(x, y) \in k \Gamma-k \Gamma\}$, we have $K_{x}-K_{x} \subset K$ for every $x$.

Now let $0 \leqslant M<N / 2$ be even. For every $w \in \mathbb{Z}_{N}$, there are exactly $2 M$ non-zero values of $d$ such that

$$
w \in\{-M d,-(M-1) d, \ldots,-2 d,-d\} \cup\{d, 2 d, \ldots, M d\},
$$
...
Let $d$ be such that if we define $P$ to be $\{d y:-M \leqslant y \leqslant M\}$ then $K \cap P=\{0\}$. Let $P^{\prime}=\{d y:-M / 2 \leqslant y \leqslant M / 2\}$, let $L \leqslant M / 2 k$ and let $Q=\{d y: 0 \leqslant y \leqslant L\}$. Define $\Gamma_{a}$ to be the set $\{(x, y) \in \Gamma: y \in a+Q\}$.

We claim that $\Gamma_{a}$ is the graph of a homomorphism of order $k$. If not, then we can find $\left(x_{1}, y_{1}\right), \ldots,\left(x_{2 k}, y_{2 k}\right)$ and $\left(x_{1}^{\prime}, y_{1}^{\prime}\right), \ldots,\left(x_{2 k}^{\prime}, y_{2 k}^{\prime}\right) \in \Gamma_{a}$ such that

$$
x_{1}+\cdots+x_{k}-x_{k+1}-\cdots-x_{2 k}=x_{1}^{\prime}+\cdots+x_{k}^{\prime}-x_{k+1}^{\prime}-\cdots-x_{2 k}^{\prime},
$$

but

$$
y_{1}+\cdots+y_{k}-y_{k+1}-\cdots-y_{2 k} \neq y_{1}^{\prime}+\cdots+y_{k}^{\prime}-y_{k+1}^{\prime}-\cdots-y_{2 k}^{\prime},
$$

and hence $x, y, y^{\prime}$ such that $y \neq y^{\prime}$ and $(x, y),\left(x, y^{\prime}\right) \in k \Gamma_{a}-k \Gamma_{a}$. However, $k \Gamma_{a}-k \Gamma_{a}$ is the set of all points of the form

$$
\left(x_{1}+\cdots+x_{k}-x_{k+1}-\cdots-x_{2 k}, y_{1}+\cdots+y_{k}-y_{k+1}-\cdots-y_{2 k}\right)
$$

such that $\left(x_{i}, y_{i}\right) \in \Gamma$ and $y_{i} \in a+Q$ for every $i$. This is a subset of $\left\{(x, y) \in k \Gamma-k \Gamma: y \in P^{\prime}\right\}=\left\{(x, y): y \in K_{x} \cap P^{\prime}\right\}$. It follows that $\left(K_{x}-K_{x}\right) \cap\left(P^{\prime}-P^{\prime}\right)$ is non-empty and hence that $K \cap P$ is non-empty, which is a contradiction.

Therefore, as long as $2 M C^{4 k}<N-1$, we can find a value of $d$ such that $\Gamma_{a}$ is the graph of a homomorphism for every $a$. The average size of $\Gamma_{a}$ is $(L+1)|\Gamma| / N$, so if we choose $M$ to be at least $N / 4 C^{4 k}$ and $L$ to be at least $(M / 2 k)-1$, as we may, then we can find $a$ such that the size of $\Gamma_{a}$ is at least $|\Gamma| / 8 k C^{4 k}$. \(\square\)
...
Proof. Let $g$ be the function $A * A * A * A$. Then $\hat{g}(r)=|\hat{A}(r)|^{4}$ and $g(r)=N^{-1} \sum_{r}|\hat{A}(r)|^{4} \omega^{r x}$ for every $r \in \mathbb{Z}_{N}$. Let $\lambda=\alpha^{3 / 2} / 4$ so that\\
...
Corollary 7.9. Let $A$ and $K$ be as in Lemma 7.8 and let $m$ be a positive integer. For every $d \in B(K, \alpha / 32 \pi m)$ there exists $c$ such that $\phi(x)-\phi(y)=$ $c(x-y)$ whenever $x-y$ belongs to the set $\{j d:-m \leqslant j \leqslant m\}$.
...
Corollary 7.10. Let $N$ be sufficiently large, let $B_{0} \subset \mathbb{Z}_{N}$ have cardinality $\alpha N$ and let $\phi: B_{0} \rightarrow \mathbb{Z}_{N}$ have $\gamma(\alpha N)^{3}$ additive quadruples. Then there exist a mod- $N$ arithmetic progression $P$ of length at least $N^{2^{-3770}} \gamma^{2328} \alpha^{2}$, a subset $H \subset P$ of cardinality at least $2^{-1849} \gamma^{1164} \alpha|P|$ and constants $\lambda, \mu \in \mathbb{Z}_{N}$ such that $\phi(s)=\lambda s+\mu$ for every $s \in H$.
...
be a homomorphism of order 8. As long as $m \leqslant|R|^{2^{-14} \alpha^{2} q^{-1}}$ it is possible to partition $R$ into arithmetic progressions $S_{1}, \ldots, S_{M}$, all of size $m$ or $m+1$ and all with the same common difference, such that the restriction of any $\phi_{i}$ to any $A_{i} \cap S_{j}$ is linear.\\
...
$$
\left|\sum_{i=1}^{T} \sum_{j=1}^{T} f_{1}(i) \omega^{-\left(a i+c i^{2}\right)} f_{2}(i) \omega^{-\left(b j+c j^{2}\right)} f_{3}(i+j) \omega^{c(i+j)^{2}}\right| \geqslant c T^{2}
$$
...
$$
\sum_{u, r}\left|\hat{f}_{u}(r)\right|^{16} \geqslant\left(\gamma N^{5} . N^{-18 / 7}\right)^{7}=\gamma^{7} N^{17}
$$

Hence, the number of sequences with the desired properties is, as stated, at least $\gamma^{7} N^{15}$. \(\square\)

Lemma 9.3. Let $\eta>0$, let $B \subset \mathbb{Z}_{N}$ be a set of size $\beta N$ and let $\phi: B \rightarrow \mathbb{Z}_{N}$ be a function with at least $\alpha \beta^{15} N^{15}$ sequences $a_{1}, \ldots, a_{16}$ such that
...
Corollary 9.4. Let $B \subset \mathbb{Z}_{N}$ have size $\beta N$, let $\phi: B \rightarrow \mathbb{Z}_{N}$ be $\gamma \beta^{3}$ -additive and let $\eta>0$. There is a subset $B^{\prime} \subset B$ containing at least $\left(\gamma^{7} \beta^{6} \eta / 4\right)^{2^{19}} \beta^{15} N^{15}$ sequences $\left(a_{1}, \ldots, a_{16}\right)$ with $a_{1}+\cdots+a_{8}=a_{9}+\cdots+a_{16}$, such that the restriction of $\phi$ to $B^{\prime}$ is a $(1-\eta)$-homomorphism of order eight.\\
...
\begin{itemize}
  ...
  \item[(iii)] For at least $\left(1-5 \eta^{1 / 2}\right)|W|$ of the points $y \in W$ we have $\epsilon(y) \leqslant 300 \eta^{1 / 2}$.
...
& \cap\left\{y:(\lambda+\rho-\sigma) \alpha M^{2} N \leqslant q(y) \leqslant(\lambda+\rho) \alpha M^{2} N\right\} \\
& \cap\{y:(\mu-\rho) M \leqslant R(y) \leqslant(\mu-\rho+\sigma) M\} \\
& \cap\{y:(\mu+\rho-\sigma) M \leqslant R(y) \leqslant(\mu+\rho) M\} .
\end{aligned}
$$
...
Lemma 10.6. If $\sigma \leqslant \eta \rho \alpha^{2} / 16$ then there exist a subset $B^{\prime}$ of $B$ of cardinality at least $\left(1-10 \eta^{1 / 5}\right)|B|$ and a function $\psi: B^{\prime} \rightarrow \mathbb{Z}_{N}$ such that, for every $d \in B^{\prime}$,
...
Lemma 10.9. Let $\theta=10 \eta^{1 / 5}$ and assume that $6 \theta^{1 / 2}<1$. Then the function $\psi: B^{\prime} \rightarrow \mathbb{Z}_{N}$ constructed in Lemma 10.6 is a Freiman homomorphism.
...
If $\theta^{1 / 2} \leqslant 1 / 8$ (as we shall assume), then Lemma 10.7 implies that $W_{1}$ has cardinality at least $7|W| / 8$.

Lemma 10.12. Assume that $\theta^{1 / 2} \leqslant 1 / 8$. Let $B=B(K, \delta)$ and let $B^{\prime}$ and $\psi$ be given by Lemma 10.6. Let $C$ and $\psi_{1}$ be as in Corollary 10.11 and let $w_{1}, w_{2} \in W_{1}$ with $r\left(w_{1}\right)-r\left(w_{2}\right)=c \in C$. Then $\phi\left(w_{1}\right)-\phi\left(w_{2}\right)=\psi_{1}(c)$.

Proof. By the definition of $W_{1}$ and the assumption that $\theta^{1 / 2} \leqslant 1 / 8$, we have the statements


\begin{equation*}
\text { (7/8 a.e. } \left.d \in B^{\prime}\right)\left((1-\theta) \text { a.e. } z \in X_{r\left(w_{1}\right)+d}\right) \quad \phi(z)-\phi\left(w_{1}\right)=\psi(d) \tag{1}
\end{equation*}
...
\begin{equation*}
\text { (7/8 a.e. } \left.d \in B^{\prime}\right)\left((1-\theta) \text { a.e. } z \in X_{r\left(w_{2}\right)+d}\right) \quad \phi(z)-\phi\left(w_{2}\right)=\psi(d) \text {. } \tag{2}
\end{equation*}
...
\begin{gather*}
\left(1 / 2 \text { a.e. } d \in B^{\prime}\right)\left((1-\theta) \text { a.e. } z \in X_{d+r\left(w_{1}\right)}\right)  \tag{4}\\
\phi(z)-\phi\left(w_{2}\right)=\psi\left(d+r\left(w_{1}\right)-r\left(w_{2}\right)\right) .
\end{gather*}


From (1) and (4) it follows that for 3/8-almost every $d \in B^{\prime}$, for $(1-\theta)$ -almost every $z \in X_{r\left(w_{1}\right)+d}$ we have both
...
$$
\phi\left(w_{2}\right)-\phi\left(w_{1}\right)=\psi\left(d+r\left(w_{1}\right)-r\left(w_{2}\right)\right)-\psi(d)=\psi(d+c)-\psi(c)=\psi(c) .
$$
...
We know that $\phi$ induces a ( $1-\eta$ )-homomorphism $\phi^{\prime \prime}$ (of order two) on $2 X-2 X$. Therefore, we can find a set $W$ of cardinality at least $\rho \alpha^{2} L N / 16=\alpha^{7} M^{4} N^{4} / 512$ with the properties claimed in Lemma 10.5. Corollary 10.11, Lemma 10.12 and the definition in between then give us a set $W_{1}$ of cardinality at least $\alpha^{7} M^{4} N^{4} / 1000=\alpha^{3}|2 X-2 X| / 1000$ and a homomorphism $\psi_{1}: W_{1} \rightarrow \mathbb{Z}_{N}$ such that, whenever $w_{1}, w_{2} \in W_{1}$ and $r\left(w_{1}\right)-r\left(w_{2}\right) \in C$, we have $\phi^{\prime \prime}\left(w_{1}\right)-\phi^{\prime \prime}\left(w_{2}\right)=\psi_{1}\left(w_{1}-w_{2}\right)$.

Now choose $\left(x_{2}, x_{3}, x_{4}\right) \in X^{3}$ uniformly at random. The expected number of $y \in X$ such that $\left(y, x_{2}, x_{3}, x_{4}\right) \in W$ is at least $\alpha^{3}|X| / 1000$, so let us fix $\left(x_{2}, x_{3}, x_{4}\right)$ such that the set $Y$ of all $y$ such that $\left(y, x_{2}, x_{3}, x_{4}\right) \in W$ has cardinality at least $\alpha^{3}|X| / 1000$. If $y, z \in Y$ and $r(y)-r(z)=c \in C$, then $r\left(y, x_{2}, x_{3}, x_{4}\right)-r\left(z, x_{2}, x_{3}, x_{4}\right)=c$, so

$$
\phi(y)-\phi(z)=\phi^{\prime \prime}\left(y, x_{2}, x_{3}, x_{4}\right)-\phi^{\prime \prime}\left(z, x_{2}, x_{3}, x_{4}\right)=\psi(y-z) .
$$
...
Proposition 12.1. For each $k \in \mathbb{Z}_{N}$, let $\lambda_{k} \geqslant 0$. Let $f_{1}, \ldots, f_{p}$ be functions from $\mathbb{Z}_{N}$ to $D$ and let $\phi_{1}, \ldots, \phi_{p}$ be functions from $\mathbb{Z}_{N}$ to $\mathbb{Z}_{N}$ such that
...
Lemma 12.5. Let $\eta>0$, let $B \subset \mathbb{Z}_{N}^{2}$ be a set of size $\beta N^{2}$ and let $\phi$ : $B \rightarrow \mathbb{Z}_{N}$ be a function that respects at least $\alpha \beta^{15} N^{32}$ 8-arrangements. If $N$ is sufficiently large (depending on $\beta$ and $\eta$ ) then there is a subset $B^{\prime} \subset B$ containing at least $(\alpha \eta / 4)^{2^{36}} \beta^{15} N^{32}$ 8-arrangements, such that the proportion of 8-arrangements respected by $\phi$ is at least $1-\eta$.
...
$$
2(\alpha \eta / 4)^{2^{36}} \leqslant 2^{-64 k} \leqslant(\alpha \eta / 2)^{2^{36}} .
$$
...
Lemma 12.6. Let $\beta, \gamma, \eta>0$. Let $f: \mathbb{Z}_{N} \rightarrow D$, let $B \subset \mathbb{Z}_{N}^{2}$ be a set of cardinality at least $\beta N^{2}$ and let $\phi: B \rightarrow \mathbb{Z}_{N}$ be such that $\left|\Delta(f ; k, l)^{\wedge}(\phi(k, l))\right| \geqslant \gamma N$ for every $(k, l) \in B$. Then there is a subset $B^{\prime} \subset B$ containing at least $2^{-2^{37}} \beta^{2^{43}} \gamma^{2^{45}} \eta^{2^{36}} N^{32}$ 8-arrangements, such that the proportion of them respected by $\phi$ is at least $1-\eta$.\\
Proof. By Lemma 12.4 there are at least $\beta^{112} \gamma^{336} N^{32}$ 8-arrangements respected by $\phi$. This allows us to take $\alpha=\beta^{97} \gamma^{336}$ in Lemma 12.5. It is not hard to check that $\left(\beta^{97} \gamma^{336} \eta / 4\right)^{2^{36}} \beta^{15} \geqslant 2^{-2^{37}} \beta^{2^{43}} \gamma^{2^{45}} \eta^{2^{36}} N^{32}$, so the lemma is proved. \(\square\)
...
Lemma 13.1. Let $f: \mathbb{Z}_{N}^{2} \rightarrow D$ be a function into the closed unit disc. For any $h$, define
...
Corollary 13.2. Let $A \subset \mathbb{Z}_{N}^{2}$. For any $h \in \mathbb{Z}_{N}$, define a domain $X_{h}=X_{h, 0} \cup X_{h, 1} \cup \cdots \cup X_{h, N-1}$ by letting $X_{h, x}$ be the set of all pairs $((x, y),(x, y+h))$, for which both $(x, y)$ and $(x, y+h)$ belong to $A$. Let $f_{h}(x)$ be the cardinality of $X_{h, x}$. Let $B \subset \mathbb{Z}_{N}$ and let $\sigma: B \rightarrow \mathbb{Z}_{N}$ be any function such that $\sum_{h \in B}\left|\hat{f}_{h}(\sigma(h))\right|^{2} \geqslant \alpha N^{5}$. Then there are at least $\alpha^{4} N^{3}$ quadruples $(a, b, c, d) \in B^{4}$ such that $a+b=c+d$ and $\sigma(a)+\sigma(b)=\sigma(c)+\sigma(d)$.

Proof. This follows immediately from Lemma 13.1 applied to the characteristic function of $A$. \(\square\)

Corollary 13.3. Let $A \subset \mathbb{Z}_{N}^{2}$, and for $h \in \mathbb{Z}_{N}$ let $X_{h}$ and $f_{h}$ be as in Corollary 13.2. Let $\theta>0$. Then there exist Freiman homomorphisms $\sigma_{1}, \ldots, \sigma_{q}$ of order eight, defined on subsets $B_{1}, \ldots, B_{q}$ of $\mathbb{Z}_{N}$, and a set $G \subset \mathbb{Z}_{N}$ of cardinality at least $(1-\theta) N$, such that whenever $h \in G$ and $\left|\hat{f}_{h}(r)\right| \geqslant \theta N^{2}$ there exists $i \leqslant q$ such that $r=\sigma_{i}(h)$. The sets $B_{i}$ have cardinality at least $2^{-1882} \theta^{10477} N$ and $q \leqslant 2^{1882} \theta^{-10479}$.

Proof. Suppose that $B \subset \mathbb{Z}_{N}$ is a set of size $\theta N$ and that $\sigma: B \rightarrow \mathbb{Z}_{N}$ is a function with the property that $\left|\hat{f}_{h}(\sigma(h))\right| \geqslant \theta N^{2}$ for every $h \in B$. Then $\sum_{h \in B}\left|\hat{f}_{h}(\sigma(h))\right|^{2} \geqslant \theta^{3} N^{5}$. Hence, by Corollary 13.2, $B$ contains at least $\theta^{12} N^{3} \sigma$-additive quadruples. It follows from Corollary 7.6 (with $\alpha=\theta$ and $\gamma=\theta^{9}$ ) that there is a subset $C$ of $B$ of cardinality at least $2^{-1882} \theta^{10477} N$ such that the restriction of $\sigma$ to $C$ is a Freiman homomorphism of order eight.

Now let $\Gamma_{0}$ be the set of all pairs $(h, r)$ such that $\left|\hat{f}_{h}(r)\right| \geqslant \theta N^{2}$. If the projection of $\Gamma$ to the $h$-axis has size less than $\theta N$, then we are done. Otherwise, we can choose $B$ and $\sigma$ satisfying the hypotheses of the previous paragraph and hence can find $B_{1} \subset B$ of cardinality at least $2^{-1882} \theta^{10477} N$ such that the restriction of $\sigma$ to $B_{1}$ is a homomorphism of order eight. Let $\sigma_{1}$ be this restriction and let $\Gamma_{1}=\Gamma \backslash\left\{\left(h, \sigma_{1}(h)\right): h \in B_{1}\right\}$.
...
Lemma 13.4. Let $\theta>0, \theta_{1}=2^{-1882} \theta^{10477}, q=2^{1882} \theta^{-10479}$ and $m=$ $\left\lfloor\left(\theta_{1} / 64 \pi\right) N^{\theta_{1}^{2} / 16 q}\right\rfloor$. Then there exist an arithmetic progression $P$ of length $m_{0} \in\{m-1, m\}$ and a subset $H \subset P$ such that

$$
\sum\{C(h): h \in H, G(h) \geqslant(1-2 \eta) C(h)\} \geqslant \alpha^{32} N^{31} m_{0} / 8,
$$

and there exist constants $a_{1}, \ldots, a_{q}$ and $b_{1}, \ldots, b_{q}$ such that, whenever $h \in$ $H$ and $r \in \mathbb{Z}_{N}$ have the property that $\left|\hat{f}_{h}(r)\right| \geqslant \theta N$, we have $r=a_{i} h+b_{i}$ for some $i \leqslant q$.
...
Let us define $I$ to be the set of all $h \in Q \cap H$ such that $C(h) \geqslant \alpha^{32} N^{15} / 16$ and $G(h) \geqslant(1-2 \eta) C(h)$. Notice that if $h \in I$ then $\phi_{h}$ is a $(1-2 \eta)$ -homomorphism of order 8 on the domain $X_{h}$. Lemma 13.5 asserts that $I$ has cardinality at least $\alpha^{32} m_{1} / 20$. For the next lemma, recall that a typical element of the domain $X_{h}$, which was defined in the statement of Corollary 13.2, is a pair $v=((x, y),(x, y+h))$ and that $r(v)$ is defined to be $x$.
...
Proof. We know that $B$ has size at least $2^{-31} \alpha^{160} m_{1} m_{3}=$ $2^{-31} \alpha^{160} m_{1} m_{3}|S||Q+y|$. We also know (from our main assumption) that\\
...
Theorem 13.12. Let $f$ be a function from $\mathbb{Z}_{N}$ to the closed unit disc. If $f$ is not cubically $\alpha$-uniform then there exist arithmetic progressions $P$ and $Q$ of size at least $N^{(1 / 2)^{(1 / \alpha)^{2}}}$ and with the same common difference, a subset $B \subset P \times Q$ of size at least $(\alpha / 2)^{2^{76}}|P \| Q|$ and a bilinear function $\phi: P \times Q \rightarrow \mathbb{Z}_{N}$, such that $\Delta(f ; k, l)^{\wedge}(\phi(k, l)) \geqslant \alpha N / 2$ for every $(k, l) \in B$.\\
...
$$
\sum_{r} \sum_{x^{\prime}, w^{\prime}, u, z}\left|\sum_{h} \lambda_{h} \prod_{i=1}^{p} \omega^{\sigma_{i}(h) u_{i}-r h}\right|^{4} \geqslant \alpha^{4} N^{4 p+4} .
$$
...
Lemma 14.2. Let $f: \mathbb{Z}_{N} \rightarrow D$, let $B \subset \mathbb{Z}_{N}^{k}$ and let $\phi: B \rightarrow \mathbb{Z}_{N}$ be such that $\left|\Delta\left(f ; r_{1}, \ldots, r_{k}\right)^{\sim}\left(\phi\left(r_{1}, \ldots, r_{k}\right)\right)\right| \geqslant \gamma N$ for every $\left(r_{1}, \ldots, r_{k}\right) \in B$. Then $\phi$ has the product property with parameter $\gamma$.\\
Proof. Let $y_{1}, \ldots, y_{p}$ be elements of $\mathbb{Z}_{N}^{k-1}$ and let $E$ be the set of all $r \in$ $\mathbb{Z}_{N}$ such that $\left(y_{i}, r\right) \in B$ for every $i$. Then if we are given a function $\theta: E \rightarrow \mathbb{R}_{+}$, we can set $\theta(k)=0$ for $k \notin E$ and apply Proposition 12.1 to the functions $f_{i}=\Delta\left(f ; y_{i}\right)$. Since $\Delta\left(f_{i} ; r\right)=\Delta\left(f ;\left(y_{i}, r\right)\right)$ these functions satisfy the hypothesis of Lemma 12.1 with $\alpha=\gamma^{2 p} \sum_{k} \theta(k) N^{-1}$ and $\sigma_{i}(r)=$ $\phi\left(y_{i}, r\right)$. The conclusion of the lemma then gives us exactly what we want, at least for $k$-restrictions. By symmetry, the result is true for the other $j$-restrictions as well, and $\phi$ has the product property with parameter $\gamma$. \(\square\)
...
Proof. For any $y=\left(y_{1}, \ldots, y_{k-1}\right) \in \mathbb{Z}_{N}^{k-1}$ we can define a function $g_{y}$ : $\mathbb{Z}_{N}^{2} \rightarrow D$ by the formula
...
Lemma 14.4. Let $B \subset \mathbb{Z}_{N}^{k}$ be a set of size $\beta N^{k}$, let $\phi: B \rightarrow \mathbb{Z}_{N}$ and suppose that $\phi$ has the product property with parameter $\gamma$. Then $\phi$ respects at least $\beta^{4^{k}} \gamma^{2 k .4^{k}} N^{3 k}$ configurations in $B$.
...
Because $\phi$ respects the configuration $\lambda$, we know that for all choices of $\eta_{j}$ for $j \neq i$, the additive quadruple
...
where $\epsilon_{i}$ and $\eta_{i}$ take the values 0 or 1, is $\phi$-additive. This implies that $\phi\left(\kappa_{j-1}\right)=\phi\left(\kappa_{j}\right)$, and the argument works for every $j$ between 1 and $k$. Therefore, $\phi\left(\kappa_{0}\right)=\phi\left(\kappa_{k}\right)$, which is the required result. \(\square\)

Corollary 14.6. Let $B \subset \mathbb{Z}_{N}^{k+1}$ be a set of size $\beta N^{k+1}$, let $\phi: B \rightarrow \mathbb{Z}_{N}$ and suppose that $\phi$ has the product property with parameter $\gamma$. Then $\phi$ respects at least $\beta^{4^{k+1}} \gamma^{k .4^{k+1}} N^{5 k+3}$ parallelepiped pairs in $B$.

Proof. The proof of this result is similar to that of Lemma 14.4. For each $r \in \mathbb{Z}_{N}$ let $\beta(r) N^{k}$ be the size of the cross-section $B_{r}$ of $B$. By Corollary $14.5, \phi$ respects at least $\beta(r)^{4^{k}} \gamma^{2 k .4^{k}} N^{3 k}$ cube pairs in $B_{r}$. The average of this number over $r$ is at least $\beta^{4^{k}} \gamma^{2 k .4^{k}} N^{3 k}$. Therefore, if we choose $r$ at random and choose a cube $\kappa$ in $\mathbb{Z}_{N}^{k}$ at random, the expected number of cubes $\kappa^{\prime}$ in $\mathbb{Z}_{N}^{k}$ for which $\left((\kappa, r),\left(\kappa^{\prime}, r\right)\right)$ is a cube pair in $B_{r}$ respected by $\phi$ is at least $\beta^{4^{k}} \gamma^{2 k \cdot 4^{k}} N^{k}$.
...
for every $\epsilon \in\{0,1\}^{k}$ is at least $\theta^{4} \gamma^{8.2^{k}} N^{4 k+3}$. But this sum counts the number of parallelepiped pairs $\left(\left(\kappa_{i}^{\prime}, r_{i}\right)\right)_{i=1}^{4}$ in $B$ such that, for each $i,\left(\kappa_{i}, r_{i}\right)$ lies in $B_{r_{i}}$ and $\left(\kappa_{i}^{\prime}, r_{i}\right)$ is congruent to it. It is certainly a lower bound for the number of parallelepiped pairs such that each of the four cubes is congruent to $\kappa$.

If we now choose randomly, for every $h=\left(h_{1}, \ldots, h_{k}\right)$, some cube $\kappa(h)$ with sidelengths $\left(h_{1}, \ldots, h_{k}\right)$ and apply the above argument, we shall obtain, on average, at least $\left(\beta^{4^{k}} \gamma^{2 k .4^{k}}\right)^{4} \gamma^{8.2^{k}} N^{5 k+3}$ distinct parallelepiped pairs, since the average of $\theta$ (which still depends on $\kappa$ ) is at least $\beta^{4^{k}} \gamma^{2 k \cdot 4^{k}}$. This proves the corollary (where, just for the sake of neatness, we have stated a weaker bound). \(\square\)
...
Lemma 14.8. Let $B \subset \mathbb{Z}_{N}^{k+1}$ be a set of size $\beta N^{k+1}$, let $\phi: B \rightarrow \mathbb{Z}_{N}$ and suppose that $\phi$ has the product property with parameter $\gamma$. Then $\phi$ respects at least $\beta^{7.4^{k+1}} \gamma^{7 k .4^{k+1}} N^{17 k+15}$ 8-arrangements in $B$. \(\square\)
...
Lemma 15.4. The number of degenerate $d$-arrangements in $\mathbb{Z}_{N}^{k+1}$ is at most $3^{2 d .2^{k}} k N^{(2 d+1) k+2 d-2}$.
...
If $\eta$ is not a multiple of $\eta_{0}$, then $\left(\eta_{1}, \ldots, \eta_{2 d}\right)$ is not a multiple of the sequence $(1, \ldots, 1,-1, \ldots,-1)$ ( $d$ ones followed by $d$ minus ones). Therefore the equation $\sum_{j} \eta_{j} r_{j}$ places a further linear restriction on the sequence $\left(r_{1}, \ldots, r_{2 d}\right)$, meaning that the number of choices for this sequence is at most $N^{2 d-2}$.

There are (strictly) fewer than $3^{2 d .2^{k}}$ functions $\eta:\{0,1\}^{k} \times\{1, \ldots, 2 d\} \rightarrow$ $\{-1,0,1\}$ that are not multiples of $\eta_{0}$. For each such function, the arguments we have just given show that the proportion of $d$-arrangements such that $\sum_{\epsilon, t} \eta(\epsilon) \prod_{i \in A} \rho(\epsilon, t)_{i}=0$ for every $A \subset\{1,2, \ldots, k+1\}$ is at most $k / N$. Finally, the proportion of $d$-arrangements for which at least one of the sidelengths $h_{i}$ is zero is also at most $k / N$. The lemma is proved. \(\square\)
...
Lemma 15.5. Let $\beta, \eta>0$, let $B \subset \mathbb{Z}_{N}^{k+1}$ be a set of size $\beta N^{k+1}$ and let $\phi: B \rightarrow \mathbb{Z}_{N}$ be a function respecting at least $\alpha \beta^{15} N^{17 k+15}$ 8-arrangements in $B$. Then there is a subset $B^{\prime} \subset B$ containing at least $(\alpha \eta / 4)^{2^{2^{k+4}+k+3}} \beta^{15} N^{17 k+15} 8$-arrangements, such that the proportion of them that are respected by $\phi$ is at least $1-\eta$.
...
$$
N^{-\left(2^{k+1}+1\right) r} \sum_{t_{1}, \ldots, t_{r}} \sum_{S_{1}, \ldots, S_{r}} \prod_{\epsilon, h} 2^{-r} \prod_{j=1}^{r}\left(1+\cos \frac{2 \pi}{N}\left(t_{j} \phi(\lambda(\epsilon, h))+\sum_{A} s_{A, j} \lambda(\epsilon, h)_{i}\right)\right)
$$

which equals

$$
N^{-\left(2^{k+1}+1\right) r}\left(\sum_{t} \sum_{S} \prod_{\epsilon, h} 2^{-r}\left(1+\cos \frac{2 \pi}{N}\left(t \phi(\lambda(\epsilon, h))+\sum_{A} s_{A} \lambda(\epsilon, h)_{i}\right)\right)^{r} .\right.
$$

By rewriting $1+\cos \frac{2 \pi}{N}\left(t \phi(\lambda(\epsilon, h))+\sum_{A} s_{A} \lambda(\epsilon, h)_{i}\right)$ as

$$
\frac{1}{2}\left(1+1+\omega^{t \phi(\lambda(\epsilon, h))+\sum_{A} s_{A} \lambda(\epsilon, h)_{i}}+\omega^{-t \phi(\lambda(\epsilon, h))-\sum_{A} s_{A} \lambda(\epsilon, h)_{i}}\right)
$$

we see that the product over $(\epsilon, h)$ is a sum of $4^{2^{k+4}}$ terms of the form

$$
2^{-2^{k+4}(r+1)} \prod_{\epsilon, h} \omega^{\eta(\epsilon, h)\left(t \phi(\lambda(\epsilon, h))+\sum_{A} s_{A} \lambda(\epsilon, h)_{i}\right)}
$$

which equals

$$
2^{-2^{k+4}(r+1)} \omega^{t \sum_{\epsilon, h} \eta(\epsilon, h) \phi(\lambda(\epsilon, h))+\sum_{A} s_{A} \sum_{\epsilon, h} \eta(\epsilon, h) \prod_{i \in A} \lambda(\epsilon, h)_{i}} .
$$

A term contributes to the sum over all the $s_{A}$ if and only if all the sums $\sum_{\epsilon, h} \eta(\epsilon, h) \prod_{i \in A} \lambda(\epsilon, h)_{i}$ are zero, and if $\lambda$ is non-degenerate, then this can happen only when $\eta$ is a multiple of $\eta_{0}$. If $\lambda$ is non-degenerate, $\eta \neq 0$ and the term contributes to the sum over $t$, then we must have in addition that $\sum_{\epsilon, h} \eta_{0}(\epsilon, h) \phi(\lambda(\epsilon, h))=0$. It is not hard to see that this is precisely the definition of $\phi$ respecting the 8-arrangement $\lambda$. The contribution of a non-zero term to the sum over $t$ and the $s_{A}$ is $2^{-2^{k+4}(r+1)} N^{2^{k}+1}$ and the number of multiples of $\eta_{0}$, counted with multiplicity, is $2^{2^{k+4}}+2$, since 0 can be produced in $2^{2^{k+4}}$ ways, and $\pm \eta_{0}$ in one way each.

Therefore, if $\lambda$ is non-degenerate, the sum over $t$ and the $s_{A}$ is $2^{-2^{k+4} r} N^{2^{k}+1}$ if $\phi$ does not respect $\lambda$ and $2^{-2^{k+4} r} N^{2^{k}+1}\left(1+2.2^{-2^{k+4}}\right)$ if it does. It follows that the probability that $\lambda$ belongs to $B^{\prime}$ is $2^{-2^{k+4} r}$ if $\phi$ does not respect $\lambda$ and $2^{-2^{k+4} r}\left(1+2.2^{-2^{k+4}}\right)^{r}$ if it does. Therefore, our hypotheses imply that the expected number $X$ of 8-arrangements respected by $\phi$ is at least $2^{-2^{k+4} r}\left(1+2.2^{-2^{k+4}}\right)^{r} \alpha \beta^{15} N^{17 k+15}$, and the expected number $Y$ of non-degenerate 8-arrangements not respected by $\phi$ is at most $2^{-2^{k+4} r} \alpha \beta^{15} N^{17 k+15}$. Using the fact that
...
If $N$ is large enough that $(\alpha \eta / 4)^{2^{2^{k+4}+k+3}} \beta^{15} N \geqslant 3^{2^{k+4}} k$, then Lemma 15.4 and the values of the above expectations imply that a set $B^{\prime}$ exists such that $X \geqslant(\alpha \eta / 4)^{2^{2^{k+4}+k+3}} \beta^{15} N^{17 k+15}, \eta X \geqslant 2 Y$ and $Y \geqslant Z$, where $Z$ is the number of degenerate 8-arrangements. This proves the lemma. \(\square\)
...
Lemma 15.6. Let $\beta, \gamma>0$. Let $B \subset \mathbb{Z}_{N}^{k+1}$ be a set of size $\beta N^{k+1}$ and let $\phi$ : $B \rightarrow \mathbb{Z}_{N}$ be a function satisfying the product property with parameter $\gamma$. Then $B$ has a subset $B^{\prime}$ containing at least $(\beta \gamma / 2)^{2^{2^{k+5}}} N^{17 k+15}$ 8-arrangements such that the proportion of them that are respected by $\phi$ is at least $1-2^{-44}$.\\
Proof. By Lemma 14.8, $\phi$ respects at least $\beta^{7.4^{k}} \gamma^{7 k .4^{k+1}} N^{17 k+15}$ 8-arrangements, and therefore, by Lemma 15.5, $B$ has a subset $B^{\prime}$ containing at least $\left(2^{-46} \beta^{7.4^{k}} \gamma^{7 k .4^{k+1}}\right)^{2^{2^{k+4}+k+3}} N^{17 k+15}$ 8-arrangements, such that the proportion respected by $\phi$ is at least $1-2^{-44}$. The lemma follows from a simple numerical check. \(\square\)
...
\begin{itemize}
  ...
  \item[(ii)] $B$ contains at least $(\theta \gamma)^{2^{2^{k+5}}} N^{17 k+15}$ 8-arrangements;
...
We shall write $\theta_{1}$ for the number $(\theta \gamma / 2)^{2^{2^{k+5}}}$ that appeared in the last lemma. Recall that the Bohr neighbourhood $B(K, \zeta)$ is defined to be the set of all $s \in \mathbb{Z}_{N}$ such that $|r s| \leqslant \zeta N$ for every $r \in K$. Given a function $\phi$\\
...
The statement that $G(h) \geqslant(1-2 \eta) C(h)$ is equivalent to the statement that the function induced by $\phi$ on $X_{h}$ is a $(1-2 \eta)$-homomorphism of order eight. We know that $X_{h}(y)$ has cardinality at most $N^{k}$ for each $y$, and it is not hard to show that if $C(h) \geqslant\left(\theta_{1} / 4\right) N^{16 k+15}$ then the cardinality of $X_{h}$ is at least $\theta_{1} N^{k+1} / 4$. Therefore, for every $h \in H$ we may apply Theorem 10.13 (with $\alpha=\theta_{1} / 4$ and $g=f_{h}$ ) and find a subset $Y_{h} \subset X_{h}$ of cardinality at least $2^{-16} \theta_{1}^{3}\left|X_{h}\right|$ such that the restriction of $\phi$ to $Y_{h}$ is a $B\left(K_{h}, \zeta\right)$ -homomorphism, where $\zeta$ is determined (as a function of $\theta$ ) by the equations $\theta_{1}=(\theta \gamma / 2)^{2^{2^{k+5}}}, \alpha=\theta_{1} / 4, k_{0}=2^{74} \alpha^{-10}$ and $\zeta=2^{-155 k_{0}} \alpha^{18 k_{0}} k_{0}$. It can be checked that the resulting number $\zeta$ exceeds $2^{-s(\theta, \gamma, k)}$. \(\square\)
...
constant. Let us write $\phi^{\prime}(h, x)$ for this value, when it is defined. Corollary 10.14 tells us that if $d \in B\left(K_{h}, \zeta / l\right)$ then the restriction of $\phi^{\prime}(h,$.$) to$ any arithmetic progression with common difference $d$ and length at most $l$ is linear. This fact will be used in the next lemma. We shall also adopt the convention that if a box in $\mathbb{Z}_{N}^{k+1}$ is written as a Cartesian product $A \times B$, then $A$ and $B$ are boxes in $\mathbb{Z}_{N}^{k}$ and $\mathbb{Z}_{N}$ respectively.
...
Lemma 16.6. Let $P=Q \times I$ be a box in $\mathbb{Z}_{N}^{k+1}$ of width at least $m$, let $t=\delta^{-2} s\left(\theta_{1} / 8, \delta, k\right)$, let $\sigma>0$ and let $l=(\zeta / 2) m^{c\left(t^{-1} \sigma, \delta, k\right)^{t} / 2 K^{2 k+1} q}$. Then there is a subset $G \subset Q$ of size at least $(1-\sigma)|Q|$ and a partition of $P$ into boxes $S_{u}=T_{u} \times J_{u}$ of width at least $l$, such that, for every $u$ and every $h \in G \cap H_{1} \cap T_{u}$, the function from $J_{u}$ to $\mathbb{Z}_{N}$ defined by $x \mapsto \phi^{\prime}(h, x)$ is linear.

Proof. Because $\Delta_{1}$ is $(\delta, t)$-multiply $k$-linear, we can find a subset $G \subset Q$ of size at least $(1-\sigma)|Q|$ and a partition of $Q$ into subboxes $Q_{j}$ of width at least $m_{1}=m^{c(\sigma / t, \delta, k)^{t}}$ such that for each $j$ there are $k$-linear functions $\mu_{1}, \ldots, \mu_{q}$ from $Q_{j}$ to $\mathbb{Z}_{N}$, with $q \leqslant q(\sigma / t, \delta, k)^{t}$ such that $\Delta_{1} \cap\left(\left(G \cap Q_{j}\right) \times \mathbb{Z}_{N}\right)$ is contained in the union of the graphs of the $\mu_{i}$. This says that, for any $h \in G \cap Q_{j}$, the set $K_{h}=\left\{r \in \mathbb{Z}_{N}:\left|\hat{f}_{h}(r)\right| \geqslant \delta N^{k+1}\right\}$ is a subset of $\left\{\mu_{1}, \ldots, \mu_{q}\right\}$.

By Lemma 16.1, each $Q_{j}$ may be partitioned into subboxes $R_{t}$ of width\\
at least $m_{2}=m_{1}^{1 / K^{2 k+1} q} \geqslant l^{2}$ and common difference $d_{t}$ such that, given any $h \in R_{t}$ and any $i,\left|\mu_{i}(h) d_{t}\right| \leqslant 2 m_{1}^{-1 / K^{2 k+1} q} \leqslant \zeta N /(l+1)$. By the conclusion of the last paragraph, this implies that, whenever $h \in G \cap R_{t}, d_{t}$ belongs to the Bohr neighbourhood $B\left(K_{h}, \zeta /(l+1)\right)$. For each $t, R_{t} \times I$ can be partitioned into boxes $S_{u}=T_{u} \times J_{u}$ of width $l$ or $l+1$. As remarked after the statement of Lemma 16.5, the restriction of $\phi^{\prime}(h,$.$) to an arithmetic$ progression with common difference $d \in B\left(K_{h}, \zeta /(l+1)\right)$ and length at most $l+1$ is linear. In other words, the function from $J_{u}$ to $\mathbb{Z}_{N}$ defined by $x \mapsto \phi^{\prime}(h, x)$ is linear, as stated. \(\square\)
...
Lemma 16.7. Let $\theta_{2}=2^{-21} \theta_{1}^{5}$. Then there exist elements $x_{1}, \ldots, x_{k}$ of $\mathbb{Z}_{N}$ such that, for at least $\theta_{2} N^{k+1}$ choices of $\left(h_{1}, \ldots, h_{k}, x\right)$ with $h \in H_{1}$, $\phi^{\prime}(h, x)$ is defined and equals

$$
\sum_{\epsilon \in\{0,1\}^{k}}(-1)^{|\epsilon|} \phi\left(x_{1}+\epsilon_{1} h_{1}, x_{2}+\epsilon_{2} h_{2}, \ldots, x_{k}+\epsilon_{k} h_{k}, x\right) .
$$

Proof. The expression given for $\phi^{\prime}(h, x)$ is valid whenever $Y_{h}(x)$ contains the cube $\left[x_{1}, \ldots, x_{k} ; h_{1}, \ldots, h_{k}\right]$. Since $\left|H_{1}\right| \geqslant\left(\theta_{1} / 8\right) N^{k}$ and $\left|X_{h}\right| \geqslant$ $\left(\theta_{1} / 4\right) N^{k+1}$ for every $h \in H_{1}$, we find that $\sum_{h \in H}\left|Y_{h}\right| \geqslant 2^{-21} \theta_{1}^{5} N^{2 k+1}=$ $\theta_{2} N^{2 k+1}$ (by the estimate for the sizes of the sets $Y_{h}$ in Lemma 16.5). Therefore, if we choose the $x_{j}$ randomly, the expected number of choices of $\left(h_{1}, \ldots, h_{k}, x\right)$ with $h \in H_{1}$ for which the equality holds is at least $\theta_{2} N^{k+1}$. The lemma follows. \(\square\)

Lemma 16.8. Suppose that $\Gamma_{1}, \ldots, \Gamma_{r}$ are $(\gamma, s)$-multiply $(k+1)$-linear subsets of $\mathbb{Z}_{N}^{k+1} \times \mathbb{Z}_{N}$. Then $\Gamma_{1} \cup \cdots \cup \Gamma_{r}$ is $(\gamma, r s)$-multiply $(k+1)$-linear. If $\phi_{1}, \ldots, \phi_{r}$ are $(\gamma, s)$-multiply $(k+1)$-linear functions defined on a subset $B \subset \mathbb{Z}_{N}^{k+1}$, then $\phi_{1}+\cdots+\phi_{r}$ is $(\gamma, r s)$-multiply $(k+1)$-linear.\\
Proof. Let $P$ be a box. We can find a subset $H_{1} \subset P$ of size at least $(1-\theta / r s)|P|$ and a partition of $P$ into boxes $Q$ of width at least $m^{c\left((r s)^{-1} \theta, \gamma, k\right)^{s}}$ such that $\Gamma_{1}$ restricted to any $Q \cap H_{1}$ is contained in the union of the graphs of $q\left((r s)^{-1} \theta, \gamma, k,\right)^{s}$ multilinear functions. Now repeat this argument inside each $Q$ for the set $\Gamma_{2}$ and so on. At each of the $r$ stages of this process, the width of the boxes is raised to the power\\
$c\left((r s)^{-1} \theta, \gamma, k\right)^{s}$, the number of new multilinear functions introduced inside each box is at most $q\left((r s)^{-1} \theta, \gamma, k\right)^{s}$ and $\theta|P| / r s$ points are thrown away. Therefore, at the end of the process we have a width of at least $m^{c\left((r s)^{-1} \theta, \gamma, k\right)^{r s}}$ and $r q\left((r s)^{-1} \theta, \gamma, k\right)^{s}$ multilinear functions for each set $\Gamma_{i}$. The result about unions follows (and in fact we have overestimated the number of multilinear functions needed). The result for sums of functions also follows, once we notice that there are $q\left((r s)^{-1} \theta, \gamma, k\right)^{r s}$ functions of the form $\mu_{1}+\cdots+\mu_{r}$, with each $\mu_{i}$ one of the multilinear functions chosen at the $i^{\text {th }}$ stage. \(\square\)

Let us now fix a choice of $x_{1}, \ldots, x_{k}$ satisfying the conclusion of Lemma 16.7 and write $\phi_{\epsilon}(h, x)$ for $\phi\left(x_{1}+\epsilon_{1} h_{1}, \ldots, x_{k}+\epsilon_{k} h_{k}, x\right)$. Write also $\phi_{1}(h, x)$ for the function $\phi_{\epsilon}(h, x)$ when $\epsilon=(1,1, \ldots, 1)$. Regard all these functions as being defined on the set $B_{1}$ of $(h, x)$ that satisfy the conclusion of Lemma 16.7, which can be rephrased as $\phi_{1}(h, x)=$ $\phi^{\prime}(h, x)-\sum_{\epsilon \neq 1}(-1)^{|\epsilon|} \phi_{\epsilon}(h, x)$ and $h \in H_{1}$. We now show that something like Lemma 16.6, but weaker, holds for the function $\phi_{1}$ as well.

Lemma 16.9. Let $t=\delta^{-2} s\left(\theta_{1} / 8, \delta, k\right), r=\left(2^{k}-1\right) \gamma^{-2} s\left(2^{-(k+2)} \theta, \gamma, k\right)$ and $q=q(\gamma, \sigma / 2 r, k)^{r}$. Let $P=Q \times I$ be any box in $\mathbb{Z}_{N}^{k+1}$ of width at least $m$ and let $\sigma>0$. Then there is a subset $E \subset P$ of size at least $(1-\sigma)|P|$ and a partition of $P$ into boxes $S_{u}=T_{u} \times J_{u}$ with the following property. Given $u$ and $h \in T_{u}$ let $\psi_{u, h}$ be the function $x \mapsto \phi_{1}(h, x)$, where the domain is the set of all $x$ such that $(h, x) \in B_{1} \cap E \cap S_{u}$. Then for every $u$ and $h \in T_{u}$ the graph of $\psi_{u, h}$ is contained in the union of the graphs of at most $q$ linear functions. The width of each box $S_{u}$ is at least

$$
l=\left(\zeta / 2 C_{k+1}\right) m^{c(\sigma / 2 r, \gamma, k)^{r} c(\sigma / 2 t, \delta, k)^{t} / 2 K^{2 k+1} q}
$$
...
and a partition of $P$ into boxes $P_{j}$ of width at least $m_{1}=m^{c\left((2 r)^{-1} \sigma, \gamma, k\right)^{r}}$ such that for every $j$ there are $(k+1)$-linear functions $\mu_{1}, \ldots, \mu_{q}$ from $P_{j}$ to $\mathbb{Z}_{N}$ with the property that $\phi^{\prime \prime}(h, x)=\mu_{i}(h, x)$ for some $i$, whenever $(h, x) \in B_{1} \cap F \cap P_{j}$.

By Lemma 16.6, each of the boxes $P_{j}=Q_{j} \times I_{j}$ gives a subset $G_{j} \subset Q_{j}$ of size at least $(1-\sigma / 2)\left|Q_{j}\right|$ and a further partition into boxes $S_{j u}=T_{j u} \times J_{j u}$ of width at least $m_{2}=\left(\zeta / 2 C_{k+1}\right) m_{1}^{c\left((2 t)^{-1} \sigma, \delta, k\right)^{t} / 2 K^{2 k+1} q}$ such that for every $h \in H_{1} \cap G_{j} \cap T_{j u}$ (recall that $(h, x) \in B_{1}$ implies that $h \in H_{1}$, which was defined just before the statement of Lemma 16.6), the restriction of $\phi^{\prime}(h, x)$ to $B_{1} \cap S_{j u}$ is linear in $x$. The lemma now follows on adding $\phi^{\prime}$ and $\phi^{\prime \prime}$ and taking $E$ to be $F \cap \bigcup_{j}\left(G_{j} \times I_{j}\right)$. \(\square\)
...
Now let $\rho>0$, let $\sigma=\rho / 4$ and let $P=Q \times I$ be a box of width at least $m$. Applying Lemma 16.9, we can find a subset $E \subset P$ of size at least $(1-\sigma)|P|$ and a partition of $P$ into boxes $S_{u}=T_{u} \times J_{u}$ of width at least $l$ satisfying the conclusion of that lemma. Let $S=T \times J$ be one of these boxes, and write $B_{1}(h)$ for the set $\left\{\left(h^{\prime}, x\right) \in B_{1} \cap E \cap S: h^{\prime}=h\right\}$. Each set $B_{1}(h)$ can be partitioned into subsets $C_{1}(h), \ldots, C_{q}(h)$ such that the restriction of $\phi_{1}$ to any $C_{t}(h)$ is linear. An easy averaging argument shows that

$$
\sum_{j=1}^{q} \sum_{h \in S}\left\{\left|C_{t}(h)\right|:\left|C_{t}(h)\right| \geqslant \sigma|J| / q\right\} \geqslant(1-\sigma)|S| .
$$

Hence, there is a subset $D \subset S$ of size at least $(1-\sigma)|S|$ such that, for every $t, C_{t}(h) \cap D$ is either empty or of size at least $\sigma|J| / q$.

Now let $r=q \sigma^{-2}$ and choose $x_{1}, \ldots, x_{r}$ randomly from $J$. If $\left|C_{t}(h)\right| \geqslant$ $\sigma|J| / q$, then the probability that $C_{t}(h)$ does not contain two distinct points $\left(h, x_{i}\right)$ and $\left(h, x_{j}\right)$ is at most $(1-\sigma / q)^{r}+r(\sigma / q)(1-\sigma / q)^{r-1}$, which is much\\
...
$$
\lambda_{t}(h)=\left(x_{i}-x_{j}\right)^{-1}\left(\phi_{1}\left(h, x_{i}\right)-\phi_{2}\left(h, x_{j}\right)\right),
$$
...
To complete the proof of the lemma, it is necessary only to check that $q(\sigma / p, \gamma, k)^{p} \leqslant q(\rho, \gamma, k+1)$ and that $l^{c(\sigma / p, \gamma, k)^{p}} \geqslant m^{c(\rho, \gamma, k+1)}$. This is a back-of-envelope calculation left to the reader. \(\square\)
...
Corollary 16.11. If $f: \mathbb{Z}_{N} \rightarrow D$ fails to be $\alpha$-uniform of degree $k+1$ then there is a box $P \subset \mathbb{Z}_{N}^{k}$ of width at least $N^{(\alpha / 2)^{2^{2^{k+9}}}}$ and a multilinear function $\mu: P \rightarrow \mathbb{Z}_{N}$ such that, for at least $(\alpha / 2)^{2^{2^{k+9}}}|P|$ values of $\left(y_{1}, \ldots, y_{k}\right)$, we have $\left|\Delta\left(f ; y_{1}, \ldots, y_{k}\right)^{\wedge}\left(\mu\left(y_{1}, \ldots, y_{k}\right)\right)\right| \geqslant(\alpha / 2) N$.

Proof. Since $f$ is not $\alpha$-uniform of degree $k+1$, we find, using the implication of (ii) from (vi) in Lemma 3.1, that there is a set $B \subset \mathbb{Z}_{N}^{k}$ of size at least $(\alpha / 2) N^{k}$ and a function $\phi: B \rightarrow \mathbb{Z}_{N}$ such that $\left|\Delta\left(f ; a_{1}, \ldots, a_{k}\right)^{\wedge}\left(\phi\left(a_{1}, \ldots, a_{k}\right)\right)\right| \geqslant(\alpha / 2) N$ for every $\left(a_{1}, \ldots, a_{k}\right) \in B$. Lemma 14.2 then implies that $\phi$ has the product property with parameter $\alpha / 2$. Next, Theorem 16.2 implies that $B$ has a subset $C$ of size at least $(\alpha / 4) N^{k}$ such that the restriction of $\phi$ to $C$ is $(\alpha / 2, r)$-multiply $k$-linear, where $r=4 \alpha^{-2} s(\alpha / 4, \alpha / 2, k)$. Applying the definition of multiple multilinearity in the case where the box $P$ is the whole of $\mathbb{Z}_{N}^{k}$ and $\theta=\alpha / 8$, we find a set $H \subset \mathbb{Z}_{N}^{k}$ of size at least $(1-\alpha / 8) N^{k}$ and partition of $\mathbb{Z}_{N}^{k}$ into boxes $P_{1}, \ldots, P_{M}$ of width at least $N^{c(\alpha / 8 r, \alpha / 2, k)^{r}}$ such that for every $j$ the restriction of $\phi$ to $C \cap P_{j} \cap H$ is contained in the graph of at most $q(\alpha / 8 r, \alpha / 2, k)^{r}$ multilinear functions. By averaging, we can find a box $P_{j}$ such that $\left|C \cap P_{j} \cap H\right| \geqslant(\alpha / 8)\left|P_{j}\right|$. By further averaging, we can find a subset $D \subset P_{j}$ of size at least $\left(q(\alpha / 8 r, \alpha / 2, k)^{r}\right)^{-1}(\alpha / 8)\left|P_{j}\right|$ such that the restriction of $\phi$ to $D$ is multilinear. A straightforward calculation shows that this implies the corollary. \(\square\)
...
Lemma 17.1. Let $\sigma$ be any $k$-linear function (over $\mathbb{Z}_{N}$ ) in variables $x_{1}, \ldots, x_{k}$. Then there are polynomials $\phi_{\epsilon}\left(\epsilon \in\{0,1\}^{k}\right)$ of degree at most $k$ giving the identity

$$
\phi\left(x_{1}, \ldots, x_{k}\right)=\sum_{\epsilon \in\{0,1\}^{k}}(-1)^{|\epsilon|} \phi_{\epsilon}(s-\epsilon . x) .
$$
...
$$
k!x_{1} \ldots x_{k}=\sum_{\epsilon \in\{0,1\}^{k}}(-1)^{|\epsilon|}(s-\epsilon . x)^{k}
$$
...
Proposition 17.2. Let $f: \mathbb{Z}_{N} \rightarrow D$. Suppose that there is a $k$-linear function $\sigma: \mathbb{Z}_{N}^{k} \rightarrow \mathbb{Z}_{N}$ such that
...
$$
\sum_{x \in \mathbb{Z}_{N}^{k}}\left|\Delta(f ; x)^{\wedge}(\sigma(x))\right|^{2}=\sum_{x \in \mathbb{Z}_{N}^{k+1}} \sum_{s} \sum_{\epsilon \in\{0,1\}^{k+1}} C^{|\epsilon|}\left(f(s-\epsilon . x) \omega^{-\phi_{\epsilon}(s-\epsilon . x)}\right) .
$$

By Lemma 17.1, we can find some $\epsilon$ such that the function $g(s)=f(s) \omega^{-\phi_{\epsilon}(s)}$ satisfies the inequality
...
Corollary 17.5. Given real numbers $\alpha_{1}, \ldots, \alpha_{k}$, set $\alpha=\left(\alpha_{1}, \ldots, \alpha_{k}\right)$ and define a function $f:\{0,1\}^{k} \rightarrow \mathbb{Z}$ by $f(\epsilon)=\lfloor\epsilon . \alpha\rfloor$. If $r$ is an integer and the $\alpha_{i}$ are allowed to vary in the interval ( $-r, r$ ), then the number of distinct such functions that can result is at most $2^{2 r k^{3}}$.

Proof. The possible values taken by $f$ are the integers between $\lfloor-r k\rfloor$ and $\lfloor r k\rfloor$, and the set of $\epsilon$ such that $f(\epsilon) \leqslant j$ is the set of $\epsilon$ such that $\epsilon . \alpha<j+1$. Let us estimate how many distinct such sets can be obtained as $\alpha$ varies. Two real numbers $\alpha$ and $\alpha^{\prime}$ give distinct sets if and only if there exists some $\epsilon$ such that $\epsilon . \alpha<j+1$ and $\epsilon . \alpha^{\prime} \geqslant j+1$, that is, if and only if the hyperplane $\{\beta: \epsilon . \beta=j+1\}$ separates $\alpha$ from $\alpha^{\prime}$. There are $2^{k}$ different such hyperplanes, so the previous lemma tells us that the number of distinct sets of the given form is at most $k\binom{2^{k}}{k} \leqslant 2^{k^{2}}$. The function $f$ is determined by the $2 r k$ sets $\{\epsilon: f(\epsilon) \leqslant j\}$ with $\left\lfloor-k^{2} / 2\right\rfloor<j \leqslant\left\lfloor k^{2} / 2\right\rfloor$, so the result follows. \(\square\)

Corollary 17.6. Let $\alpha_{0}, \alpha_{1}, \ldots, \alpha_{k}$ be real numbers, let $\alpha=\left(\alpha_{1}, \ldots, \alpha_{k}\right)$ and define a function $f:\{0,1\}^{k} \rightarrow \mathbb{Z}_{M}$ by $f(\epsilon)=\left\lfloor\alpha_{0}+\alpha . \epsilon\right\rfloor(\bmod M)$. If $\alpha_{0}$ can be arbitrary and the $\alpha_{i}$ are allowed to vary in the interval $(-r, r)$, then the number of distinct such functions that can result is at most $M .2^{2 r(k+1)^{3}}$.
...
Before stating the next proposition, we define a concept which is similar to $\alpha$-uniformity but designed for situations where we are given a multilinear function on a small domain. Let $f: \mathbb{Z}_{N} \rightarrow D$. Suppose that we can partition $\mathbb{Z}_{N}$ into $\bmod -N$ arithmetic progressions $Q_{1}, \ldots, Q_{M}$, each of length at most $m$, such that, defining $Q_{i} f(s)$ to be $f(s)$ when $s \in Q_{i}$ and 0 otherwise, we have
...
Proposition 17.7. Let $f: \mathbb{Z}_{N} \rightarrow D$. Suppose that there is a product $P=P_{1} \times \cdots \times P_{k}$ of arithmetic progressions $P_{i}$ of common difference $d$ and odd length $m \leqslant N^{1 / 2}$, and a $k$-linear function $\sigma: P \rightarrow \mathbb{Z}_{N}$ such that
...
$0 \leqslant j \leqslant M-1$ define $Q_{j}$ to be the interval $\left\{x \in \mathbb{Z}_{N}: j w \leqslant x<(j+1) w\right\}$. Notice that the cardinality of $Q_{j}$ is always $\lfloor l / k\rfloor-1$ or $\lfloor l / k\rfloor$. Let a sequence $r=\left(r_{1}, \ldots, r_{k}\right)$ be defined by $P_{i}=\left\{r_{i}-l, r_{i}-l+1, \ldots, r_{i}+l\right\}$ and let $I$ be the interval $\{-l,-l+1, \ldots, l\}$. Then any $x \in P$ can be written uniquely as $a+r$ for some $a \in I^{k}$. Given $\epsilon \in\{0,1\}^{k}$, we shall write $f_{\epsilon}$ for the function that takes $s$ to $f(s-\epsilon . r)$. Let us also write $\tau(a)$ for $\sigma(a+r)$.

Then

$$
\begin{aligned}
\sum_{x \in P}\left|\Delta(f ; x)^{\wedge}(\sigma(x))\right|^{2} & =\sum_{a \in I^{k}}\left|\sum_{s} \omega^{-s \sigma(a+r)} \prod_{\epsilon} C^{|\epsilon|} f(s-\epsilon . a-\epsilon . r)\right|^{2} \\
& =\sum_{a \in I^{k}}\left|\sum_{s} \omega^{-s \tau(a)} \prod_{\epsilon} C^{|\epsilon|} f_{\epsilon}(s-\epsilon . a)\right|^{2}
\end{aligned}
$$
...
$$
\sum_{a \in I^{k}}\left|\sum_{s} \sum_{j} \omega^{-s \tau(a)} \prod_{\epsilon \in\{0,1\}^{k}}\left(Q_{j(\epsilon)} C^{|\epsilon|} f_{\epsilon}\right)(s-\epsilon . a)\right|^{2},
$$
...
This we can do using Corollary 17.5. If $Q_{j(\epsilon)} f_{\epsilon}(s-\epsilon . a)$ is non-zero, then $s-\epsilon . a \in Q_{j(\epsilon)}$ which implies that $j(\epsilon) w \leqslant s-\epsilon . a<j(\epsilon) w$ and therefore that $j(\epsilon)$ is exactly the integer part of $w^{-1} s-\epsilon .\left(w^{-1} a\right)$. Since $-k-1<w^{-1} a_{i}<k+1$ for every $i$, Corollary 17.5 implies that the number of functions $j$ for which the product over $\epsilon$ can ever be non-zero is at most $M .2^{2(k+1)^{4}}$.

Let us define functions $j_{1}$ and $j_{2}$ (from $\{0,1\}^{k}$ to $\mathbb{Z}_{M}$ ) to be equivalent if they are translations of each other. Corollary 17.5 with $M=1$ implies that the number of equivalence classes is at most $2^{k(k+1)^{2}}$. Let us call them $J_{1}, \ldots, J_{L}$. By the Cauchy-Schwarz inequality, we can deduce from our calculations above that

$$
\alpha N^{2} m^{k} \leqslant L \sum_{r=1}^{L} \sum_{a \in I^{k}}\left|\sum_{s} \sum_{j \in J_{r}} \omega^{-s \tau(a)} \prod_{\epsilon}\left(Q_{j(\epsilon)} C^{|\epsilon|} f_{\epsilon}\right)(s-\epsilon . a)\right|^{2} .
$$

We can therefore find $r$ such that, choosing some representative $j$ of $J_{r}$, we have

$$
L^{-2} \alpha N^{2} m^{k} \leqslant \sum_{a \in I^{k}}\left|\sum_{s} \sum_{i=1}^{M} \omega^{-s \tau(a)} \prod_{\epsilon}\left(Q_{j(\epsilon)+i} C^{|\epsilon|} f_{\epsilon}\right)(s-\epsilon . a)\right|^{2} .
$$
...
Obviously this still exceeds $L^{-2} \alpha N^{2} m^{k}$ if we replace the sum over $a \in I_{k}$ above by a sum over all of $\mathbb{Z}_{N}^{k}$. Expanding the modulus squared and substituting in the usual way, the resulting inequality can be rewritten

$$
M \sum_{i=1}^{M} \sum_{a \in \mathbb{Z}_{N}^{k+1}} \sum_{s} \omega^{-\rho(a)} \prod_{\epsilon \in\{0,1\}^{k+1}}\left(Q_{j(\epsilon)+i} C^{|\epsilon|} f_{\epsilon}\right)(s-\epsilon . a) \geqslant L^{-2} \alpha N^{2} m^{k},
$$
...
$$
M \sum_{i=1}^{M} \sum_{a \in \mathbb{Z}_{N}^{k+1}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{k+1}}\left(Q_{j(\epsilon)+i} C^{|\epsilon|} f_{\epsilon}\right)(s-\epsilon . a) \omega^{-\phi_{\epsilon}(s-\epsilon . a)} \geqslant L^{-2} \alpha N^{2} m^{k} .
$$
...
$$
\sum_{a \in \mathbb{Z}_{N}^{k+1}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{k+1}}\left(Q_{j(\epsilon)+i} C^{|\epsilon|} f_{\epsilon}\right)(s-\epsilon . a) \omega^{-\phi_{\epsilon}(s-\epsilon . a)}
$$

is bounded above by the average over $\eta \in\{0,1\}^{k+1}$ of

$$
\sum_{a \in \mathbb{Z}_{N}^{k+1}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{k+1}}\left(Q_{j(\eta)+i} C^{|\epsilon|} f_{\eta}\right)(s-\epsilon . a) \omega^{-\phi_{\eta}(s-\epsilon . a)} .
$$
...
$$
M \sum_{i=1}^{M} \sum_{a \in \mathbb{Z}_{N}^{k+1}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{k+1}}\left(Q_{j(\epsilon)+i} C^{|\epsilon|} g\right)(s-\epsilon . a) \geqslant L^{-2} \alpha N^{2} m^{k},
$$

and this may be rewritten

$$
\sum_{i=1}^{M} \sum_{a \in \mathbb{Z}_{N}^{k+1}} \sum_{s} \prod_{\epsilon \in\{0,1\}^{k+1}}\left(C^{|\epsilon|} Q_{i} g\right)(s-\epsilon . a) \geqslant L^{-2} \alpha M^{-2} N^{2} m^{k} M,
$$
...
This implies that the function $g$ is not $2^{-2(k+1)^{3}} \alpha$-uniform with respect to the partition $Q_{1}, \ldots, Q_{M}$.

This is not quite the statement of the proposition. To obtain it, recall that $f_{\eta}(s)=f(s-\eta . r)$. Therefore, the statement about $g$ implies that the function $f(s) \omega^{-\phi_{\eta}(s+\eta . r)}$ is not $2^{-2(k+1)^{3}} \alpha$-uniform with respect to the partition $\left(Q_{i}+\eta . r\right)_{i=1}^{m}$. Since $\phi_{\eta}(s+\eta . r)$ is still a polynomial in $s$ of degree at most $k+1$, the proposition is proved. \(\square\)
...
Theorem 18.1. Let $\alpha \leqslant 1 / 2$ and let $A \subset \mathbb{Z}_{N}$ be a set which fails to be $\alpha$-uniform of degree $k$. There exists a partition of $\mathbb{Z}_{N}$ into arithmetic progressions $P_{1}, \ldots, P_{M}$ of average size at least $N^{\alpha^{2^{k+10}}}$ such that

$$
\sum_{j=1}^{M}\left|\sum_{s \in P_{j}} f(s)\right| \geqslant \alpha^{2^{2^{k+10}}} N .
$$

Proof. The result will be proved by induction on $k$. First, Corollary 16.11 gives us a box $P \subset \mathbb{Z}_{N}^{k}$ of width at least $N^{(\alpha / 2)^{2^{2 k+8}}}$ and a multilinear function $\mu: P \rightarrow \mathbb{Z}_{N}$ such that, for at least $(\alpha / 2)^{2^{2^{k+8}}}|P|$ values of $\left(y_{1}, \ldots, y_{k}\right) \in P$, we have
...
The mod- $N$ progressions $S_{i j}$ with $i \in I$, together with those $Q_{i}$ for which $i \notin I$, partition $\mathbb{Z}_{N}$. From the way we chose $I$, the average size of a cell in this partition is at least $r=(2 k)^{-1} p^{\left(2^{-(k+4)^{3}} \beta / 2 k\right)^{2^{2}+9}}$. By Lemma 5.13 we can find a refinement of this partition into proper arithmetic progressions of average size at least $r^{1 / 2} / 4$. Let us call these progressions $T_{1}, \ldots, T_{L}$. Since $\sum_{i \in I}\left|Q_{i}\right| \geqslant 2^{-2(k+4)^{3}} \beta N$ we have the inequality
...
Corollary 18.7. Let $k$ be a positive integer and let $N \geqslant$ 2 ↑ 2 ↑ 2 ↑ 2 ↑ 2 ↑ $(k+9)$. Then however the set $\{1,2, \ldots, N\}$ is coloured with two colours, there will be a monochromatic arithmetic progression of length $k$. \(\square\)
