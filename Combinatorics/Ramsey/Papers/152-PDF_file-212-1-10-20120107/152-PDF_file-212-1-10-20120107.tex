...
\end{aligned}
\]
How large can $\left|\gamma_{o} \otimes \gamma_{e}\right|$ be? We already know that if $b_{1}$ does not commute with $c_{1}$, then $\left|\gamma_{o} \otimes \gamma_{e}\right|=1$. Thus, we first make the following assumptions; the other commuting orientation will be reduced to this one before Corollary 2.7.1.\\
...
We also dispose of a boundary range omitted from the printed case split. Suppose $6q<b_{1}<8q$, and write $b_{1}=6q+z$, where $0<z<2q$. Four distinct positive terminal entries, all congruent to $b_{1}$ modulo $2q$ and at most $b_{1}$, force $b_{1}-2q=4q+z$ to belong to $\operatorname{epi}(\gamma_o)$. If $16q+z\leq n$, Theorem 2.4 applied to $4q+z,10q+z,16q+z$ forces $10q+z$ into $\operatorname{epi}(\gamma_o)$, contradicting the maximality of $b_{1}$. Hence $n<16q+z$. On the other hand, the first two entries $c_{1}<c_{2}$ of $\operatorname{pro}(\gamma_e)$ are congruent modulo $4q$, while the commuting boundary conditions give $2b_{1}\leq c_{1}$. Consequently $n\geq c_{2}\geq2b_{1}+4q=16q+2z$, again a contradiction. (The equality $b_{1}=8q$ is excluded by parity.)
...
It remains to justify the commuting orientation overlooked by assumption (3). Suppose that $b_{1}$ is in the upper half and $c_{1}$ is in the lower half. Mirror the odd and even traces separately in their normalized intervals, sending each odd entry $x$ and even entry $y$, respectively, to
\[
2\left(\left\lceil\frac n2\right\rceil+1-\frac{x+1}{2}\right)-1
\quad\text{and}\quad
2\left(\left\lfloor\frac n2\right\rfloor+1-\frac y2\right).
\]
Complementation preserves the $\Theta$ property of each normalized trace, so concatenating the mirrored odd trace and mirrored even trace gives a $\Theta_{12}$ permutation. Its boundary entries commute by Theorem 2.2 and have the orientation in (3). If $u',v'$ are its active end-block lengths, then the corresponding original lengths satisfy $u\leq u'$ and $v\leq v'$. The bounds in Theorems 2.6 and 2.7 therefore transfer to the original pair (both original end blocks are nonempty). Finally, the fixed-block argument in Lemma 2.5 gives the general upper bound $\left|\gamma_o\otimes\gamma_e\right|\leq\binom{u+v}{v}$. Together with $u,v\leq6$ and the transferred bounds, this is at most $20$. Thus the opposite commuting orientation gives the same bound.
