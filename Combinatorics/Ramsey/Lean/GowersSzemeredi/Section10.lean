...
Bohr element, the local homomorphism is scalar multiplication.  The paper's
standing prime-modulus hypothesis is explicit because the scalar is obtained
by division by a nonzero progression step. -/
def corollary_10_14 : Prop :=
  forall (N : Nat) [NeZero N] (X : Type*) [Fintype X] [DecidableEq X]
      (D : MultifunctionDomain N X) (phi : X -> ZMod N)
      (K : Finset (ZMod N)) (zeta : Real) (Y : Finset X)
      (psi1 : ZMod N -> ZMod N),
    Nat.Prime N -> HasBohrDifferenceModel D phi K zeta Y psi1 ->
