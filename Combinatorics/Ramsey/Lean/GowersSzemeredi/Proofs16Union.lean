...
Lemma 16.8.  A single globally multilinear graph is multiply linear with
...
private lemma multiplyLinear_const_zero {N k : Nat} [NeZero N]
  ...
  intro theta htheta P
  refine ⟨1, 1, P.carrier, (fun _ : Fin 1 => P),
    (fun _ _ _ => c), Finset.Subset.rfl, ?_, singleton_box_partition P, ?_, ?_, ?_, ?_⟩
...
private def pointSingletonBox {N d : Nat} (x : Point N d) : Box N d where
  axis := fun i => { start := x i, step := 0, length := 1 }
  commonDiff := 0
  axis_step := by intro i; rfl

@[simp] private lemma pointSingletonBox_carrier {N d : Nat} [NeZero N]
    (x : Point N d) : (pointSingletonBox x).carrier = {x} := by
  classical
  ext y
  simp only [Box.carrier, pointSingletonBox, Finset.mem_filter,
...
theorem lemma_16_8_zero_iteration_counterexample :
  ...
  obtain ⟨M, q, H, Q, mu, hHP, hHcard, hpartition, hq, hwidth,
      hmultilinear, hcover⟩ := hunion (1 / 2 : Real) (by norm_num) lemma168Box
...
private structure UnionRefinement {N d r : Nat} [NeZero N]
...
private def initialUnionRefinement {N d r : Nat} [NeZero N]
    (Gamma : Fin r → Finset (Point N d × ZMod N)) (P : Box N d)
    (eta c s : Real) (q0 : Nat) :
    UnionRefinement Gamma P eta c s q0 0 (Nat.zero_le r) where
  M := 1
  Q := fun _ => P
  H := P.carrier
...
private noncomputable def extendUnionRefinement {N d r t q0 : Nat} [NeZero N]
    (Gamma : Fin r → Finset (Point N d × ZMod N)) (P : Box N d)
    (gamma eta s : Real) (htr : t ≤ r) (hlt : t < r)
    (heta : 0 < eta) (hs : 0 < s) (D : UnionRefinement Gamma P eta
      (multipleC (s⁻¹ * eta) gamma d) s q0 t htr)
    (hnext : MultiplyLinear gamma s (Gamma ⟨t, hlt⟩))
    (hq0 : ∀ q : Nat,
      (q : Real) ≤ (multipleQ (s⁻¹ * eta) gamma d) ^ s → q ≤ q0) :
    UnionRefinement Gamma P eta (multipleC (s⁻¹ * eta) gamma d) s
      q0 (t + 1) (by omega) := by
  classical
  choose m q H R nu hHsub hHcard hpart hq hwidth hmulti hcover using
    fun j : Fin D.M => hnext eta heta (D.Q j)
  ...
  refine ⟨Fintype.card I, Q', H', mu', ?_, ?_, ?_, ?_, ?_, ?_⟩
...
private noncomputable def iterateUnionRefinement {N d r t q0 : Nat} [NeZero N]
    (Gamma : Fin r → Finset (Point N d × ZMod N)) (P : Box N d)
    (gamma eta s : Real) (heta : 0 < eta) (hs : 0 < s)
    (hGamma : ∀ i, MultiplyLinear gamma s (Gamma i))
    (hq0 : ∀ q : Nat,
      (q : Real) ≤ (multipleQ (s⁻¹ * eta) gamma d) ^ s → q ≤ q0)
    (htr : t ≤ r) :
    UnionRefinement Gamma P eta (multipleC (s⁻¹ * eta) gamma d) s
  ...
  induction t with
  | zero =>
      exact initialUnionRefinement Gamma P eta
        (multipleC (s⁻¹ * eta) gamma d) s q0
  | succ t ih =>
      exact extendUnionRefinement Gamma P gamma eta s (by omega) (by omega)
        heta hs (ih (by omega)) (hGamma ⟨t, by omega⟩) hq0

private lemma abs_gamma_le_iteration_of_nonempty {N d : Nat} [NeZero N]
    {gamma s : Real} {Gamma : Finset (Point N d × ZMod N)}
    (hs : 0 < s) (hGamma : MultiplyLinear gamma s Gamma)
  ...
  obtain ⟨M, q, H, Q, mu, hHP, hHcard, hpartition, hq, hwidth,
      hmultilinear, hcover⟩ := hGamma rho hrho (pointSingletonBox z.1)
  have hPcard : (pointSingletonBox z.1).carrier.card = 1 := by simp
  have hHpos : 0 < H.card := by
  ...
  have hxP := hHP hxH
  have hx : x = z.1 := by
    simpa only [pointSingletonBox_carrier, Finset.mem_singleton] using hxP
  subst x
...
private lemma gamma_ne_zero_of_nonempty {N d : Nat} [NeZero N]
  ...
  obtain ⟨M, q, H, Q, mu, hHP, hHcard, hpartition, hq, hwidth,
      hmultilinear, hcover⟩ :=
    hGamma (1 / 2 : Real) (by norm_num) (pointSingletonBox z.1)
  have hPcard : (pointSingletonBox z.1).carrier.card = 1 := by simp
  have hHpos : 0 < H.card := by
    have hreal : (0 : Real) < H.card := by
      rw [hPcard] at hHcard
...
private theorem multiplyLinear_finsetUnion {N d r : Nat} [NeZero N]
    (gamma s : Real) (hs : 1 ≤ s)
    (Gamma : Fin r → Finset (Point N d × ZMod N))
    (hr : 0 < r) (hGamma : ∀ i, MultiplyLinear gamma s (Gamma i)) :
    MultiplyLinear gamma ((r : Real) * s) (section16FinsetUnion Gamma) := by
  intro theta htheta P
  have hsPos : 0 < s := zero_lt_one.trans_le hs
  have hrReal : (0 : Real) < r := by exact_mod_cast hr
  ...
  let D := iterateUnionRefinement Gamma P gamma eta s heta hsPos hGamma
  ...
  · by_cases hUnionNe : (section16FinsetUnion Gamma).Nonempty
    · obtain ⟨z, hz⟩ := hUnionNe
      ...
      refine ⟨D.M, r * q0, D.H, D.Q, mu, D.H_subset, hDcard,
        D.partition, hcounts.1, hDwidth, ?_, ?_⟩
      · intro j a
        let p := finProdFinEquiv.symm a
    ...
    · refine ⟨D.M, 0, D.H, D.Q, (fun _ i => Fin.elim0 i),
        D.H_subset, hDcard, D.partition, (by simpa using htargetBoundNonneg),
        hDwidth, ?_, ?_⟩
      · intro j i
        exact Fin.elim0 i
      · intro j x hxQ hxH y hxy
  ...
  · refine ⟨D.M, 0, ∅, D.Q, (fun _ i => Fin.elim0 i),
      Finset.empty_subset _, ?_, D.partition,
...
private theorem multiplyLinear_fin_sum {N d r : Nat} [NeZero N]
  ...
  · by_cases hBNe : B.Nonempty
    · obtain ⟨x0, hx0⟩ := hBNe
      ...
      refine ⟨D.M, q0 ^ r, D.H, D.Q, mu, D.H_subset, hDcard,
        D.partition, hcounts.2, hDwidth, ?_, ?_⟩
      · intro j a
...
private lemma multiplyLinear_empty_zero {N d : Nat} [NeZero N]
  ...
  refine ⟨1, 0, P.carrier, (fun _ : Fin 1 => P),
    ...
    singleton_box_partition P, ?_, ?_, ?_, ?_⟩
  · have hcard : (0 : Real) ≤ P.carrier.card := by positivity
    nlinarith
...
private lemma multiplyLinearFunction_zero_zero {N d : Nat} [NeZero N]
  ...
  intro theta htheta P
  refine ⟨1, 1, P.carrier, (fun _ : Fin 1 => P),
    (fun _ _ _ => 0), Finset.Subset.rfl, ?_, singleton_box_partition P,
    ?_, ?_, ?_, ?_⟩
  · have hcard : (0 : Real) ≤ P.carrier.card := by positivity
    nlinarith
  · simp [multipleQ, multipleC]
  ...
  · intro j i
    exact isMultilinear_const 0
  · intro j x hxQ hxH y hxy
    have hy := (mem_partialGraph_iff B (fun _ => 0) x y).1 hxy |>.2
    exact ⟨0, hy⟩
...
theorem lemma_16_8_holds : lemma_16_8 := by
  ...
  · intro N k r _ gamma s Gamma hs hGamma
    ...
    · exact multiplyLinear_finsetUnion gamma s hs Gamma (Nat.pos_of_ne_zero hr) hGamma
  · intro N k r _ gamma s B phi hs hphi
    by_cases hr : r = 0
    · subst r
      simpa using (multiplyLinearFunction_zero_zero
        (N := N) (d := k + 1) gamma B)
    · exact multiplyLinear_fin_sum gamma s hs B phi (Nat.pos_of_ne_zero hr) hphi
