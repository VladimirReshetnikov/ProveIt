...
reconstruction of five candidate outputs.  It proves the normalization
identity, the equivalence of the monic and fraction-free resolvent equations,
preservation of $\eps^2$ by the correction, nonvanishing consequences of the
branch certificates, coherence of all four quadratic sign branches, and
soundness of finite branch selection.  A separate generic theorem proves that
all five outputs are roots once four cyclic Fourier identities are supplied.
The development does \emph{not} yet derive those identities from the raw
Lazard invariant/radical data, construct the required certificates, or prove
the five-factor completeness statement.
\end{abstract}
...
\begin{center}
  \fouriercodepath.
\end{center}
Both are imported by the public \lean{PolynomialFormulas} module.  The source
...
\begin{quote}
The Lean definitions are a type-correct transcription of the corrected
algebraic algorithm.  The kernel proves the final root identity from four
explicit cyclic Fourier relations.  Deriving those relations from the raw
Lazard certificates, constructing certificates from solvable coefficients,
and proving complete root coverage remain outside the present claim.
\end{quote}
...
interface is \lean{FourierRelations}, whose four cyclic equations are exactly
the additional evidence used by
\lean{solveGeneral_root_of_fourierRelations}.  Lazard's sourced irreducibility
and $-1$ nonsquare hypotheses remain relevant to deriving that evidence and to
the applicability/existence bridge.
...
relations are useful algebraic data, but do not by themselves certify that a
supplied tuple came from roots.  Establishing that origin, or directly deriving
the four \lean{FourierRelations}, remains part of the Lazard-specific bridge.
...
When the four \lean{FourierRelations} hold, the proved theorem
\lean{solveGeneral_root_of_fourierRelations} establishes that every $x_k$
annihilates the original quintic.
...
The formula-independent endgame is now proved.  For components
...
the Tschirnhaus translation.  This proof uses only symbolic field identities.

The main remaining Lazard-specific soundness task is to derive those four
cyclic identities for the concrete components computed from the intended
invariants and radical choices.  The raw \lean{InvariantRelations} and
\lean{RadicalCertificate} interfaces are insufficient on their own because
they admit extraneous solutions.  Accordingly, the older
...
hypotheses omit necessary evidence.

Pointwise soundness also does not express that all five roots, with their
multiplicities, have been recovered.  A future completeness result should
establish the polynomial identity
\[
 \text{\lean{GeneralQuintic.polynomial}}
   =a\prod_{k=0}^{4}\bigl(X-\text{\lean{solveGeneral}}(k)\bigr).
\]
The proof-carrying Gaussian quintic dispatcher already uses the proved
pointwise theorem by requiring \lean{FourierRelations} in every Lazard witness.
It makes no degree-five completeness claim, and automatic witness construction
remains absent.
...
\item identify $P_1,\ldots,P_4$ with the four Fourier sums, or equivalently
  prove their four \lean{FourierRelations};
\item construct the required proof-carrying witness from solvable input
  coefficients; and
\item prove a five-factor identity for completeness and multiplicities.
\end{enumerate}

Such a factorization theorem would give exhaustiveness and, under
irreducibility and separability, pairwise distinctness.  None of these
unfinished statements is admitted with \lean{axiom},
