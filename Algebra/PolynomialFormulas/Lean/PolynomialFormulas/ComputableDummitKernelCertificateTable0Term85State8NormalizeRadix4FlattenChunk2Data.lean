import PolynomialFormulas.ComputableDummitCoefficientsCore

namespace LeanProofs.PolynomialFormulas.ComputableDummitCoefficients.KernelCertificate

open LeanProofs.PolynomialFormulas.ComputableDummitCoefficients

set_option maxRecDepth 1000000

set_option maxHeartbeats 20000000

def table0Term85State8NormalizeRadix4Bucket2Normal : SparsePolynomial := [
  ⟨44, ⟨1, 2, 7, 8, 2⟩⟩,
  ⟨44, ⟨1, 2, 8, 7, 2⟩⟩,
  ⟨396, ⟨1, 3, 6, 8, 2⟩⟩,
  ⟨880, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨44, ⟨0, 3, 7, 8, 2⟩⟩,
  ⟨44, ⟨1, 2, 7, 8, 2⟩⟩,
  ⟨44, ⟨1, 3, 6, 8, 2⟩⟩,
  ⟨44, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨396, ⟨1, 3, 8, 6, 2⟩⟩,
  ⟨44, ⟨0, 3, 8, 7, 2⟩⟩,
  ⟨44, ⟨1, 2, 8, 7, 2⟩⟩,
  ⟨44, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨44, ⟨1, 3, 8, 6, 2⟩⟩,
  ⟨968, ⟨1, 4, 5, 8, 2⟩⟩,
  ⟨3652, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨176, ⟨0, 4, 6, 8, 2⟩⟩,
  ⟨176, ⟨1, 3, 6, 8, 2⟩⟩,
  ⟨176, ⟨1, 4, 5, 8, 2⟩⟩,
  ⟨176, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨3652, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨396, ⟨0, 4, 7, 7, 2⟩⟩,
  ⟨396, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨396, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨396, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨968, ⟨1, 4, 8, 5, 2⟩⟩,
  ⟨176, ⟨0, 4, 8, 6, 2⟩⟩,
  ⟨176, ⟨1, 3, 8, 6, 2⟩⟩,
  ⟨176, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨176, ⟨1, 4, 8, 5, 2⟩⟩,
  ⟨968, ⟨1, 5, 4, 8, 2⟩⟩,
  ⟨5632, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨264, ⟨0, 5, 5, 8, 2⟩⟩,
  ⟨264, ⟨1, 4, 5, 8, 2⟩⟩,
  ⟨264, ⟨1, 5, 4, 8, 2⟩⟩,
  ⟨264, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨9504, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨968, ⟨0, 5, 6, 7, 2⟩⟩,
  ⟨968, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨968, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨968, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨5632, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨968, ⟨0, 5, 7, 6, 2⟩⟩,
  ⟨968, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨968, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨968, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨968, ⟨1, 5, 8, 4, 2⟩⟩,
  ⟨264, ⟨0, 5, 8, 5, 2⟩⟩,
  ⟨264, ⟨1, 4, 8, 5, 2⟩⟩,
  ⟨264, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨264, ⟨1, 5, 8, 4, 2⟩⟩,
  ⟨396, ⟨1, 6, 3, 8, 2⟩⟩,
  ⟨3652, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨176, ⟨0, 6, 4, 8, 2⟩⟩,
  ⟨176, ⟨1, 5, 4, 8, 2⟩⟩,
  ⟨176, ⟨1, 6, 3, 8, 2⟩⟩,
  ⟨176, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨9504, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨968, ⟨0, 6, 5, 7, 2⟩⟩,
  ⟨968, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨968, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨968, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨9504, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨1584, ⟨0, 6, 6, 6, 2⟩⟩,
  ⟨1584, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨1584, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨1584, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨3652, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨968, ⟨0, 6, 7, 5, 2⟩⟩,
  ⟨968, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨968, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨968, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨396, ⟨1, 6, 8, 3, 2⟩⟩,
  ⟨176, ⟨0, 6, 8, 4, 2⟩⟩,
  ⟨176, ⟨1, 5, 8, 4, 2⟩⟩,
  ⟨176, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨176, ⟨1, 6, 8, 3, 2⟩⟩,
  ⟨44, ⟨1, 7, 2, 8, 2⟩⟩,
  ⟨880, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨44, ⟨0, 7, 3, 8, 2⟩⟩,
  ⟨44, ⟨1, 6, 3, 8, 2⟩⟩,
  ⟨44, ⟨1, 7, 2, 8, 2⟩⟩,
  ⟨44, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨3652, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨396, ⟨0, 7, 4, 7, 2⟩⟩,
  ⟨396, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨396, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨396, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨5632, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨968, ⟨0, 7, 5, 6, 2⟩⟩,
  ⟨968, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨968, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨968, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨3652, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨968, ⟨0, 7, 6, 5, 2⟩⟩,
  ⟨968, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨968, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨968, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨880, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨396, ⟨0, 7, 7, 4, 2⟩⟩,
  ⟨396, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨396, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨396, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨44, ⟨1, 7, 8, 2, 2⟩⟩,
  ⟨44, ⟨0, 7, 8, 3, 2⟩⟩,
  ⟨44, ⟨1, 6, 8, 3, 2⟩⟩,
  ⟨44, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨44, ⟨1, 7, 8, 2, 2⟩⟩,
  ⟨44, ⟨1, 8, 2, 7, 2⟩⟩,
  ⟨396, ⟨1, 8, 3, 6, 2⟩⟩,
  ⟨44, ⟨0, 8, 3, 7, 2⟩⟩,
  ⟨44, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨44, ⟨1, 8, 2, 7, 2⟩⟩,
  ⟨44, ⟨1, 8, 3, 6, 2⟩⟩,
  ⟨968, ⟨1, 8, 4, 5, 2⟩⟩,
  ⟨176, ⟨0, 8, 4, 6, 2⟩⟩,
  ⟨176, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨176, ⟨1, 8, 3, 6, 2⟩⟩,
  ⟨176, ⟨1, 8, 4, 5, 2⟩⟩,
  ⟨968, ⟨1, 8, 5, 4, 2⟩⟩,
  ⟨264, ⟨0, 8, 5, 5, 2⟩⟩,
  ⟨264, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨264, ⟨1, 8, 4, 5, 2⟩⟩,
  ⟨264, ⟨1, 8, 5, 4, 2⟩⟩,
  ⟨396, ⟨1, 8, 6, 3, 2⟩⟩,
  ⟨176, ⟨0, 8, 6, 4, 2⟩⟩,
  ⟨176, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨176, ⟨1, 8, 5, 4, 2⟩⟩,
  ⟨176, ⟨1, 8, 6, 3, 2⟩⟩,
  ⟨44, ⟨1, 8, 7, 2, 2⟩⟩,
  ⟨44, ⟨0, 8, 7, 3, 2⟩⟩,
  ⟨44, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨44, ⟨1, 8, 6, 3, 2⟩⟩,
  ⟨44, ⟨1, 8, 7, 2, 2⟩⟩,
  ⟨44, ⟨2, 1, 7, 8, 2⟩⟩,
  ⟨44, ⟨2, 1, 8, 7, 2⟩⟩,
  ⟨1012, ⟨2, 2, 6, 8, 2⟩⟩,
  ⟨2244, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨132, ⟨1, 2, 7, 8, 2⟩⟩,
  ⟨132, ⟨2, 1, 7, 8, 2⟩⟩,
  ⟨132, ⟨2, 2, 6, 8, 2⟩⟩,
  ⟨132, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨1012, ⟨2, 2, 8, 6, 2⟩⟩,
  ⟨132, ⟨1, 2, 8, 7, 2⟩⟩,
  ⟨132, ⟨2, 1, 8, 7, 2⟩⟩,
  ⟨132, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨132, ⟨2, 2, 8, 6, 2⟩⟩,
  ⟨4444, ⟨2, 3, 5, 8, 2⟩⟩,
  ⟨16676, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨1012, ⟨1, 3, 6, 8, 2⟩⟩,
  ⟨1012, ⟨2, 2, 6, 8, 2⟩⟩,
  ⟨1012, ⟨2, 3, 5, 8, 2⟩⟩,
  ⟨1012, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨16676, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨2244, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨2244, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨2244, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨2244, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨4444, ⟨2, 3, 8, 5, 2⟩⟩,
  ⟨1012, ⟨1, 3, 8, 6, 2⟩⟩,
  ⟨1012, ⟨2, 2, 8, 6, 2⟩⟩,
  ⟨1012, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨1012, ⟨2, 3, 8, 5, 2⟩⟩,
  ⟨6952, ⟨2, 4, 4, 8, 2⟩⟩,
  ⟨40700, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨2376, ⟨1, 4, 5, 8, 2⟩⟩,
  ⟨2376, ⟨2, 3, 5, 8, 2⟩⟩,
  ⟨2376, ⟨2, 4, 4, 8, 2⟩⟩,
  ⟨2376, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨68948, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨8844, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨8844, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨8844, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨8844, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨40700, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨8844, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨8844, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨8844, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨8844, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨6952, ⟨2, 4, 8, 4, 2⟩⟩,
  ⟨2376, ⟨1, 4, 8, 5, 2⟩⟩,
  ⟨2376, ⟨2, 3, 8, 5, 2⟩⟩,
  ⟨2376, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨2376, ⟨2, 4, 8, 4, 2⟩⟩,
  ⟨4444, ⟨2, 5, 3, 8, 2⟩⟩,
  ⟨40700, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨2376, ⟨1, 5, 4, 8, 2⟩⟩,
  ⟨2376, ⟨2, 4, 4, 8, 2⟩⟩,
  ⟨2376, ⟨2, 5, 3, 8, 2⟩⟩,
  ⟨2376, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨107360, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨13464, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨13464, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨13464, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨13464, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨107360, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨22528, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨22528, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨22528, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨22528, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨40700, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨13464, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨13464, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨13464, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨13464, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨4444, ⟨2, 5, 8, 3, 2⟩⟩,
  ⟨2376, ⟨1, 5, 8, 4, 2⟩⟩,
  ⟨2376, ⟨2, 4, 8, 4, 2⟩⟩,
  ⟨2376, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨2376, ⟨2, 5, 8, 3, 2⟩⟩,
  ⟨1012, ⟨2, 6, 2, 8, 2⟩⟩,
  ⟨16676, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨1012, ⟨1, 6, 3, 8, 2⟩⟩,
  ⟨1012, ⟨2, 5, 3, 8, 2⟩⟩,
  ⟨1012, ⟨2, 6, 2, 8, 2⟩⟩,
  ⟨1012, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨68948, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨8844, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨8844, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨8844, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨8844, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨107360, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨22528, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨22528, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨22528, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨22528, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨68948, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨22528, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨22528, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨22528, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨22528, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨16676, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨8844, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨8844, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨8844, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨8844, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨1012, ⟨2, 6, 8, 2, 2⟩⟩,
  ⟨1012, ⟨1, 6, 8, 3, 2⟩⟩,
  ⟨1012, ⟨2, 5, 8, 3, 2⟩⟩,
  ⟨1012, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨1012, ⟨2, 6, 8, 2, 2⟩⟩,
  ⟨44, ⟨2, 7, 1, 8, 2⟩⟩,
  ⟨2244, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨132, ⟨1, 7, 2, 8, 2⟩⟩,
  ⟨132, ⟨2, 6, 2, 8, 2⟩⟩,
  ⟨132, ⟨2, 7, 1, 8, 2⟩⟩,
  ⟨132, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨16676, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨2244, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨2244, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨2244, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨2244, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨40700, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨8844, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨8844, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨8844, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨8844, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨40700, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨13464, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨13464, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨13464, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨13464, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨16676, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨8844, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨8844, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨8844, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨8844, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨2244, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨2244, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨2244, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨2244, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨2244, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨44, ⟨2, 7, 8, 1, 2⟩⟩,
  ⟨132, ⟨1, 7, 8, 2, 2⟩⟩,
  ⟨132, ⟨2, 6, 8, 2, 2⟩⟩,
  ⟨132, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨132, ⟨2, 7, 8, 1, 2⟩⟩,
  ⟨44, ⟨2, 8, 1, 7, 2⟩⟩,
  ⟨1012, ⟨2, 8, 2, 6, 2⟩⟩,
  ⟨132, ⟨1, 8, 2, 7, 2⟩⟩,
  ⟨132, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨132, ⟨2, 8, 1, 7, 2⟩⟩,
  ⟨132, ⟨2, 8, 2, 6, 2⟩⟩,
  ⟨4444, ⟨2, 8, 3, 5, 2⟩⟩,
  ⟨1012, ⟨1, 8, 3, 6, 2⟩⟩,
  ⟨1012, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨1012, ⟨2, 8, 2, 6, 2⟩⟩,
  ⟨1012, ⟨2, 8, 3, 5, 2⟩⟩,
  ⟨6952, ⟨2, 8, 4, 4, 2⟩⟩,
  ⟨2376, ⟨1, 8, 4, 5, 2⟩⟩,
  ⟨2376, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨2376, ⟨2, 8, 3, 5, 2⟩⟩,
  ⟨2376, ⟨2, 8, 4, 4, 2⟩⟩,
  ⟨4444, ⟨2, 8, 5, 3, 2⟩⟩,
  ⟨2376, ⟨1, 8, 5, 4, 2⟩⟩,
  ⟨2376, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨2376, ⟨2, 8, 4, 4, 2⟩⟩,
  ⟨2376, ⟨2, 8, 5, 3, 2⟩⟩,
  ⟨1012, ⟨2, 8, 6, 2, 2⟩⟩,
  ⟨1012, ⟨1, 8, 6, 3, 2⟩⟩,
  ⟨1012, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨1012, ⟨2, 8, 5, 3, 2⟩⟩,
  ⟨1012, ⟨2, 8, 6, 2, 2⟩⟩,
  ⟨44, ⟨2, 8, 7, 1, 2⟩⟩,
  ⟨132, ⟨1, 8, 7, 2, 2⟩⟩,
  ⟨132, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨132, ⟨2, 8, 6, 2, 2⟩⟩,
  ⟨132, ⟨2, 8, 7, 1, 2⟩⟩,
  ⟨396, ⟨3, 1, 6, 8, 2⟩⟩,
  ⟨880, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨44, ⟨2, 1, 7, 8, 2⟩⟩,
  ⟨44, ⟨3, 0, 7, 8, 2⟩⟩,
  ⟨44, ⟨3, 1, 6, 8, 2⟩⟩,
  ⟨44, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨396, ⟨3, 1, 8, 6, 2⟩⟩,
  ⟨44, ⟨2, 1, 8, 7, 2⟩⟩,
  ⟨44, ⟨3, 0, 8, 7, 2⟩⟩,
  ⟨44, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨44, ⟨3, 1, 8, 6, 2⟩⟩,
  ⟨4444, ⟨3, 2, 5, 8, 2⟩⟩,
  ⟨16676, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨1012, ⟨2, 2, 6, 8, 2⟩⟩,
  ⟨1012, ⟨3, 1, 6, 8, 2⟩⟩,
  ⟨1012, ⟨3, 2, 5, 8, 2⟩⟩,
  ⟨1012, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨16676, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨2244, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨2244, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨2244, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨2244, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨4444, ⟨3, 2, 8, 5, 2⟩⟩,
  ⟨1012, ⟨2, 2, 8, 6, 2⟩⟩,
  ⟨1012, ⟨3, 1, 8, 6, 2⟩⟩,
  ⟨1012, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨1012, ⟨3, 2, 8, 5, 2⟩⟩,
  ⟨12408, ⟨3, 3, 4, 8, 2⟩⟩,
  ⟨73040, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨4444, ⟨2, 3, 5, 8, 2⟩⟩,
  ⟨4444, ⟨3, 2, 5, 8, 2⟩⟩,
  ⟨4444, ⟨3, 3, 4, 8, 2⟩⟩,
  ⟨4444, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨124036, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨16676, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨16676, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨16676, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨16676, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨73040, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨16676, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨16676, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨16676, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨16676, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨12408, ⟨3, 3, 8, 4, 2⟩⟩,
  ⟨4444, ⟨2, 3, 8, 5, 2⟩⟩,
  ⟨4444, ⟨3, 2, 8, 5, 2⟩⟩,
  ⟨4444, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨4444, ⟨3, 3, 8, 4, 2⟩⟩,
  ⟨12408, ⟨3, 4, 3, 8, 2⟩⟩,
  ⟨115544, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨6952, ⟨2, 4, 4, 8, 2⟩⟩,
  ⟨6952, ⟨3, 3, 4, 8, 2⟩⟩,
  ⟨6952, ⟨3, 4, 3, 8, 2⟩⟩,
  ⟨6952, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨308396, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨40700, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨40700, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨40700, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨40700, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨308396, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨68948, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨68948, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨68948, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨68948, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨115544, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨40700, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨40700, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨40700, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨40700, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨12408, ⟨3, 4, 8, 3, 2⟩⟩,
  ⟨6952, ⟨2, 4, 8, 4, 2⟩⟩,
  ⟨6952, ⟨3, 3, 8, 4, 2⟩⟩,
  ⟨6952, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨6952, ⟨3, 4, 8, 3, 2⟩⟩,
  ⟨4444, ⟨3, 5, 2, 8, 2⟩⟩,
  ⟨73040, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨4444, ⟨2, 5, 3, 8, 2⟩⟩,
  ⟨4444, ⟨3, 4, 3, 8, 2⟩⟩,
  ⟨4444, ⟨3, 5, 2, 8, 2⟩⟩,
  ⟨4444, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨308396, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨40700, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨40700, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨40700, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨40700, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨484704, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨107360, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨107360, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨107360, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨107360, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨308396, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨107360, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨107360, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨107360, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨107360, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨73040, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨40700, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨40700, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨40700, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨40700, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨4444, ⟨3, 5, 8, 2, 2⟩⟩,
  ⟨4444, ⟨2, 5, 8, 3, 2⟩⟩,
  ⟨4444, ⟨3, 4, 8, 3, 2⟩⟩,
  ⟨4444, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨4444, ⟨3, 5, 8, 2, 2⟩⟩,
  ⟨396, ⟨3, 6, 1, 8, 2⟩⟩,
  ⟨16676, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨1012, ⟨2, 6, 2, 8, 2⟩⟩,
  ⟨1012, ⟨3, 5, 2, 8, 2⟩⟩,
  ⟨1012, ⟨3, 6, 1, 8, 2⟩⟩,
  ⟨1012, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨124036, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨16676, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨16676, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨16676, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨16676, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨308396, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨68948, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨68948, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨68948, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨68948, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨308396, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨107360, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨107360, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨107360, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨107360, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨124036, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨68948, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨68948, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨68948, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨68948, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨16676, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨16676, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨16676, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨16676, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨16676, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨396, ⟨3, 6, 8, 1, 2⟩⟩,
  ⟨1012, ⟨2, 6, 8, 2, 2⟩⟩,
  ⟨1012, ⟨3, 5, 8, 2, 2⟩⟩,
  ⟨1012, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨1012, ⟨3, 6, 8, 1, 2⟩⟩,
  ⟨880, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨44, ⟨2, 7, 1, 8, 2⟩⟩,
  ⟨44, ⟨3, 6, 1, 8, 2⟩⟩,
  ⟨44, ⟨3, 7, 0, 8, 2⟩⟩,
  ⟨44, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨16676, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨2244, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨2244, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨2244, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨2244, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨73040, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨16676, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨16676, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨16676, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨16676, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨115544, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨40700, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨40700, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨40700, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨40700, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨73040, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨40700, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨40700, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨40700, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨40700, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨16676, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨16676, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨16676, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨16676, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨16676, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨880, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨2244, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨2244, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨2244, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨2244, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨44, ⟨2, 7, 8, 1, 2⟩⟩,
  ⟨44, ⟨3, 6, 8, 1, 2⟩⟩,
  ⟨44, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨44, ⟨3, 7, 8, 0, 2⟩⟩,
  ⟨396, ⟨3, 8, 1, 6, 2⟩⟩,
  ⟨44, ⟨2, 8, 1, 7, 2⟩⟩,
  ⟨44, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨44, ⟨3, 8, 0, 7, 2⟩⟩,
  ⟨44, ⟨3, 8, 1, 6, 2⟩⟩,
  ⟨4444, ⟨3, 8, 2, 5, 2⟩⟩,
  ⟨1012, ⟨2, 8, 2, 6, 2⟩⟩,
  ⟨1012, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨1012, ⟨3, 8, 1, 6, 2⟩⟩,
  ⟨1012, ⟨3, 8, 2, 5, 2⟩⟩,
  ⟨12408, ⟨3, 8, 3, 4, 2⟩⟩,
  ⟨4444, ⟨2, 8, 3, 5, 2⟩⟩,
  ⟨4444, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨4444, ⟨3, 8, 2, 5, 2⟩⟩,
  ⟨4444, ⟨3, 8, 3, 4, 2⟩⟩,
  ⟨12408, ⟨3, 8, 4, 3, 2⟩⟩,
  ⟨6952, ⟨2, 8, 4, 4, 2⟩⟩,
  ⟨6952, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨6952, ⟨3, 8, 3, 4, 2⟩⟩,
  ⟨6952, ⟨3, 8, 4, 3, 2⟩⟩,
  ⟨4444, ⟨3, 8, 5, 2, 2⟩⟩,
  ⟨4444, ⟨2, 8, 5, 3, 2⟩⟩,
  ⟨4444, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨4444, ⟨3, 8, 4, 3, 2⟩⟩,
  ⟨4444, ⟨3, 8, 5, 2, 2⟩⟩,
  ⟨396, ⟨3, 8, 6, 1, 2⟩⟩,
  ⟨1012, ⟨2, 8, 6, 2, 2⟩⟩,
  ⟨1012, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨1012, ⟨3, 8, 5, 2, 2⟩⟩,
  ⟨1012, ⟨3, 8, 6, 1, 2⟩⟩,
  ⟨44, ⟨2, 8, 7, 1, 2⟩⟩,
  ⟨44, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨44, ⟨3, 8, 6, 1, 2⟩⟩,
  ⟨44, ⟨3, 8, 7, 0, 2⟩⟩,
  ⟨968, ⟨4, 1, 5, 8, 2⟩⟩,
  ⟨3652, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨176, ⟨3, 1, 6, 8, 2⟩⟩,
  ⟨176, ⟨4, 0, 6, 8, 2⟩⟩,
  ⟨176, ⟨4, 1, 5, 8, 2⟩⟩,
  ⟨176, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨3652, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨396, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨396, ⟨4, 0, 7, 7, 2⟩⟩,
  ⟨396, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨396, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨968, ⟨4, 1, 8, 5, 2⟩⟩,
  ⟨176, ⟨3, 1, 8, 6, 2⟩⟩,
  ⟨176, ⟨4, 0, 8, 6, 2⟩⟩,
  ⟨176, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨176, ⟨4, 1, 8, 5, 2⟩⟩,
  ⟨6952, ⟨4, 2, 4, 8, 2⟩⟩,
  ⟨40700, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨2376, ⟨3, 2, 5, 8, 2⟩⟩,
  ⟨2376, ⟨4, 1, 5, 8, 2⟩⟩,
  ⟨2376, ⟨4, 2, 4, 8, 2⟩⟩,
  ⟨2376, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨68948, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨8844, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨8844, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨8844, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨8844, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨40700, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨8844, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨8844, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨8844, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨8844, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨6952, ⟨4, 2, 8, 4, 2⟩⟩,
  ⟨2376, ⟨3, 2, 8, 5, 2⟩⟩,
  ⟨2376, ⟨4, 1, 8, 5, 2⟩⟩,
  ⟨2376, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨2376, ⟨4, 2, 8, 4, 2⟩⟩,
  ⟨12408, ⟨4, 3, 3, 8, 2⟩⟩,
  ⟨115544, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨6952, ⟨3, 3, 4, 8, 2⟩⟩,
  ⟨6952, ⟨4, 2, 4, 8, 2⟩⟩,
  ⟨6952, ⟨4, 3, 3, 8, 2⟩⟩,
  ⟨6952, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨308396, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨40700, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨40700, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨40700, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨40700, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨308396, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨68948, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨68948, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨68948, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨68948, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨115544, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨40700, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨40700, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨40700, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨40700, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨12408, ⟨4, 3, 8, 3, 2⟩⟩,
  ⟨6952, ⟨3, 3, 8, 4, 2⟩⟩,
  ⟨6952, ⟨4, 2, 8, 4, 2⟩⟩,
  ⟨6952, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨6952, ⟨4, 3, 8, 3, 2⟩⟩,
  ⟨6952, ⟨4, 4, 2, 8, 2⟩⟩,
  ⟨115544, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨6952, ⟨3, 4, 3, 8, 2⟩⟩,
  ⟨6952, ⟨4, 3, 3, 8, 2⟩⟩,
  ⟨6952, ⟨4, 4, 2, 8, 2⟩⟩,
  ⟨6952, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨492624, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨65208, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨65208, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨65208, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨65208, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨777128, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨174328, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨174328, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨174328, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨174328, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨492624, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨174328, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨174328, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨174328, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨174328, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨115544, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨65208, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨65208, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨65208, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨65208, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨6952, ⟨4, 4, 8, 2, 2⟩⟩,
  ⟨6952, ⟨3, 4, 8, 3, 2⟩⟩,
  ⟨6952, ⟨4, 3, 8, 3, 2⟩⟩,
  ⟨6952, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨6952, ⟨4, 4, 8, 2, 2⟩⟩,
  ⟨968, ⟨4, 5, 1, 8, 2⟩⟩,
  ⟨40700, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨2376, ⟨3, 5, 2, 8, 2⟩⟩,
  ⟨2376, ⟨4, 4, 2, 8, 2⟩⟩,
  ⟨2376, ⟨4, 5, 1, 8, 2⟩⟩,
  ⟨2376, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨308396, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨40700, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨40700, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨40700, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨40700, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨777128, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨174328, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨174328, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨174328, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨174328, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨777128, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨275088, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨275088, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨275088, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨275088, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨308396, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨174328, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨174328, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨174328, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨174328, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨40700, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨40700, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨40700, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨40700, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨40700, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨968, ⟨4, 5, 8, 1, 2⟩⟩,
  ⟨2376, ⟨3, 5, 8, 2, 2⟩⟩,
  ⟨2376, ⟨4, 4, 8, 2, 2⟩⟩,
  ⟨2376, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨2376, ⟨4, 5, 8, 1, 2⟩⟩,
  ⟨3652, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨176, ⟨3, 6, 1, 8, 2⟩⟩,
  ⟨176, ⟨4, 5, 1, 8, 2⟩⟩,
  ⟨176, ⟨4, 6, 0, 8, 2⟩⟩,
  ⟨176, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨68948, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨8844, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨8844, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨8844, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨8844, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨308396, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨68948, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨68948, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨68948, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨68948, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨492624, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨174328, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨174328, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨174328, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨174328, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨308396, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨174328, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨174328, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨174328, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨174328, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨68948, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨68948, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨68948, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨68948, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨68948, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨3652, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨8844, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨8844, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨8844, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨8844, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨176, ⟨3, 6, 8, 1, 2⟩⟩,
  ⟨176, ⟨4, 5, 8, 1, 2⟩⟩,
  ⟨176, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨176, ⟨4, 6, 8, 0, 2⟩⟩,
  ⟨3652, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨396, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨396, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨396, ⟨4, 7, 0, 7, 2⟩⟩,
  ⟨396, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨40700, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨8844, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨8844, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨8844, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨8844, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨115544, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨40700, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨40700, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨40700, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨40700, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨115544, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨65208, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨65208, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨65208, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨65208, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨40700, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨40700, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨40700, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨40700, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨40700, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨3652, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨8844, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨8844, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨8844, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨8844, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨396, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨396, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨396, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨396, ⟨4, 7, 7, 0, 2⟩⟩,
  ⟨968, ⟨4, 8, 1, 5, 2⟩⟩,
  ⟨176, ⟨3, 8, 1, 6, 2⟩⟩,
  ⟨176, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨176, ⟨4, 8, 0, 6, 2⟩⟩,
  ⟨176, ⟨4, 8, 1, 5, 2⟩⟩,
  ⟨6952, ⟨4, 8, 2, 4, 2⟩⟩,
  ⟨2376, ⟨3, 8, 2, 5, 2⟩⟩,
  ⟨2376, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨2376, ⟨4, 8, 1, 5, 2⟩⟩,
  ⟨2376, ⟨4, 8, 2, 4, 2⟩⟩,
  ⟨12408, ⟨4, 8, 3, 3, 2⟩⟩,
  ⟨6952, ⟨3, 8, 3, 4, 2⟩⟩,
  ⟨6952, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨6952, ⟨4, 8, 2, 4, 2⟩⟩,
  ⟨6952, ⟨4, 8, 3, 3, 2⟩⟩,
  ⟨6952, ⟨4, 8, 4, 2, 2⟩⟩,
  ⟨6952, ⟨3, 8, 4, 3, 2⟩⟩,
  ⟨6952, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨6952, ⟨4, 8, 3, 3, 2⟩⟩,
  ⟨6952, ⟨4, 8, 4, 2, 2⟩⟩,
  ⟨968, ⟨4, 8, 5, 1, 2⟩⟩,
  ⟨2376, ⟨3, 8, 5, 2, 2⟩⟩,
  ⟨2376, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨2376, ⟨4, 8, 4, 2, 2⟩⟩,
  ⟨2376, ⟨4, 8, 5, 1, 2⟩⟩,
  ⟨176, ⟨3, 8, 6, 1, 2⟩⟩,
  ⟨176, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨176, ⟨4, 8, 5, 1, 2⟩⟩,
  ⟨176, ⟨4, 8, 6, 0, 2⟩⟩,
  ⟨968, ⟨5, 1, 4, 8, 2⟩⟩,
  ⟨5632, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨264, ⟨4, 1, 5, 8, 2⟩⟩,
  ⟨264, ⟨5, 0, 5, 8, 2⟩⟩,
  ⟨264, ⟨5, 1, 4, 8, 2⟩⟩,
  ⟨264, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨9504, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨968, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨968, ⟨5, 0, 6, 7, 2⟩⟩,
  ⟨968, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨968, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨5632, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨968, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨968, ⟨5, 0, 7, 6, 2⟩⟩,
  ⟨968, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨968, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨968, ⟨5, 1, 8, 4, 2⟩⟩,
  ⟨264, ⟨4, 1, 8, 5, 2⟩⟩,
  ⟨264, ⟨5, 0, 8, 5, 2⟩⟩,
  ⟨264, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨264, ⟨5, 1, 8, 4, 2⟩⟩,
  ⟨4444, ⟨5, 2, 3, 8, 2⟩⟩,
  ⟨40700, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨2376, ⟨4, 2, 4, 8, 2⟩⟩,
  ⟨2376, ⟨5, 1, 4, 8, 2⟩⟩,
  ⟨2376, ⟨5, 2, 3, 8, 2⟩⟩,
  ⟨2376, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨107360, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨13464, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨13464, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨13464, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨13464, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨107360, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨22528, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨22528, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨22528, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨22528, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨40700, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨13464, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨13464, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨13464, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨13464, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨4444, ⟨5, 2, 8, 3, 2⟩⟩,
  ⟨2376, ⟨4, 2, 8, 4, 2⟩⟩,
  ⟨2376, ⟨5, 1, 8, 4, 2⟩⟩,
  ⟨2376, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨2376, ⟨5, 2, 8, 3, 2⟩⟩,
  ⟨4444, ⟨5, 3, 2, 8, 2⟩⟩,
  ⟨73040, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨4444, ⟨4, 3, 3, 8, 2⟩⟩,
  ⟨4444, ⟨5, 2, 3, 8, 2⟩⟩,
  ⟨4444, ⟨5, 3, 2, 8, 2⟩⟩,
  ⟨4444, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨308396, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨40700, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨40700, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨40700, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨40700, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨484704, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨107360, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨107360, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨107360, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨107360, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨308396, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨107360, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨107360, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨107360, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨107360, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨73040, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨40700, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨40700, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨40700, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨40700, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨4444, ⟨5, 3, 8, 2, 2⟩⟩,
  ⟨4444, ⟨4, 3, 8, 3, 2⟩⟩,
  ⟨4444, ⟨5, 2, 8, 3, 2⟩⟩,
  ⟨4444, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨4444, ⟨5, 3, 8, 2, 2⟩⟩,
  ⟨968, ⟨5, 4, 1, 8, 2⟩⟩,
  ⟨40700, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨2376, ⟨4, 4, 2, 8, 2⟩⟩,
  ⟨2376, ⟨5, 3, 2, 8, 2⟩⟩,
  ⟨2376, ⟨5, 4, 1, 8, 2⟩⟩,
  ⟨2376, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨308396, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨40700, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨40700, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨40700, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨40700, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨777128, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨174328, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨174328, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨174328, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨174328, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨777128, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨275088, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨275088, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨275088, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨275088, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨308396, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨174328, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨174328, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨174328, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨174328, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨40700, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨40700, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨40700, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨40700, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨40700, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨968, ⟨5, 4, 8, 1, 2⟩⟩,
  ⟨2376, ⟨4, 4, 8, 2, 2⟩⟩,
  ⟨2376, ⟨5, 3, 8, 2, 2⟩⟩,
  ⟨2376, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨2376, ⟨5, 4, 8, 1, 2⟩⟩,
  ⟨5632, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨264, ⟨4, 5, 1, 8, 2⟩⟩,
  ⟨264, ⟨5, 4, 1, 8, 2⟩⟩,
  ⟨264, ⟨5, 5, 0, 8, 2⟩⟩,
  ⟨264, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨107360, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨13464, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨13464, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨13464, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨13464, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨484704, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨107360, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨107360, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨107360, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨107360, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨777128, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨275088, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨275088, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨275088, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨275088, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨484704, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨275088, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨275088, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨275088, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨275088, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨107360, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨107360, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨107360, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨107360, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨107360, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨5632, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨13464, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨13464, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨13464, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨13464, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨264, ⟨4, 5, 8, 1, 2⟩⟩,
  ⟨264, ⟨5, 4, 8, 1, 2⟩⟩,
  ⟨264, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨264, ⟨5, 5, 8, 0, 2⟩⟩,
  ⟨9504, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨968, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨968, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨968, ⟨5, 6, 0, 7, 2⟩⟩,
  ⟨968, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨107360, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨22528, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨22528, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨22528, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨22528, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨308396, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨107360, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨107360, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨107360, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨107360, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨308396, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨174328, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨174328, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨174328, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨174328, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨107360, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨107360, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨107360, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨107360, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨107360, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨9504, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨22528, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨22528, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨22528, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨22528, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨968, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨968, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨968, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨968, ⟨5, 6, 7, 0, 2⟩⟩,
  ⟨5632, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨968, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨968, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨968, ⟨5, 7, 0, 6, 2⟩⟩,
  ⟨968, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨40700, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨13464, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨13464, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨13464, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨13464, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨73040, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨40700, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨40700, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨40700, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨40700, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨40700, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨40700, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨40700, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨40700, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨40700, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨5632, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨13464, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨13464, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨13464, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨13464, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨968, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨968, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨968, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨968, ⟨5, 7, 6, 0, 2⟩⟩,
  ⟨968, ⟨5, 8, 1, 4, 2⟩⟩,
  ⟨264, ⟨4, 8, 1, 5, 2⟩⟩,
  ⟨264, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨264, ⟨5, 8, 0, 5, 2⟩⟩,
  ⟨264, ⟨5, 8, 1, 4, 2⟩⟩,
  ⟨4444, ⟨5, 8, 2, 3, 2⟩⟩,
  ⟨2376, ⟨4, 8, 2, 4, 2⟩⟩,
  ⟨2376, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨2376, ⟨5, 8, 1, 4, 2⟩⟩,
  ⟨2376, ⟨5, 8, 2, 3, 2⟩⟩,
  ⟨4444, ⟨5, 8, 3, 2, 2⟩⟩,
  ⟨4444, ⟨4, 8, 3, 3, 2⟩⟩,
  ⟨4444, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨4444, ⟨5, 8, 2, 3, 2⟩⟩,
  ⟨4444, ⟨5, 8, 3, 2, 2⟩⟩,
  ⟨968, ⟨5, 8, 4, 1, 2⟩⟩,
  ⟨2376, ⟨4, 8, 4, 2, 2⟩⟩,
  ⟨2376, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨2376, ⟨5, 8, 3, 2, 2⟩⟩,
  ⟨2376, ⟨5, 8, 4, 1, 2⟩⟩,
  ⟨264, ⟨4, 8, 5, 1, 2⟩⟩,
  ⟨264, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨264, ⟨5, 8, 4, 1, 2⟩⟩,
  ⟨264, ⟨5, 8, 5, 0, 2⟩⟩,
  ⟨396, ⟨6, 1, 3, 8, 2⟩⟩,
  ⟨3652, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨176, ⟨5, 1, 4, 8, 2⟩⟩,
  ⟨176, ⟨6, 0, 4, 8, 2⟩⟩,
  ⟨176, ⟨6, 1, 3, 8, 2⟩⟩,
  ⟨176, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨9504, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨968, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨968, ⟨6, 0, 5, 7, 2⟩⟩,
  ⟨968, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨968, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨9504, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨1584, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨1584, ⟨6, 0, 6, 6, 2⟩⟩,
  ⟨1584, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨1584, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨3652, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨968, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨968, ⟨6, 0, 7, 5, 2⟩⟩,
  ⟨968, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨968, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨396, ⟨6, 1, 8, 3, 2⟩⟩,
  ⟨176, ⟨5, 1, 8, 4, 2⟩⟩,
  ⟨176, ⟨6, 0, 8, 4, 2⟩⟩,
  ⟨176, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨176, ⟨6, 1, 8, 3, 2⟩⟩,
  ⟨1012, ⟨6, 2, 2, 8, 2⟩⟩,
  ⟨16676, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨1012, ⟨5, 2, 3, 8, 2⟩⟩,
  ⟨1012, ⟨6, 1, 3, 8, 2⟩⟩,
  ⟨1012, ⟨6, 2, 2, 8, 2⟩⟩,
  ⟨1012, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨68948, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨8844, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨8844, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨8844, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨8844, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨107360, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨22528, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨22528, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨22528, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨22528, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨68948, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨22528, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨22528, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨22528, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨22528, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨16676, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨8844, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨8844, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨8844, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨8844, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨1012, ⟨6, 2, 8, 2, 2⟩⟩,
  ⟨1012, ⟨5, 2, 8, 3, 2⟩⟩,
  ⟨1012, ⟨6, 1, 8, 3, 2⟩⟩,
  ⟨1012, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨1012, ⟨6, 2, 8, 2, 2⟩⟩,
  ⟨396, ⟨6, 3, 1, 8, 2⟩⟩,
  ⟨16676, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨1012, ⟨5, 3, 2, 8, 2⟩⟩,
  ⟨1012, ⟨6, 2, 2, 8, 2⟩⟩,
  ⟨1012, ⟨6, 3, 1, 8, 2⟩⟩,
  ⟨1012, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨124036, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨16676, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨16676, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨16676, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨16676, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨308396, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨68948, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨68948, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨68948, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨68948, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨308396, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨107360, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨107360, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨107360, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨107360, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨124036, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨68948, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨68948, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨68948, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨68948, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨16676, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨16676, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨16676, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨16676, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨16676, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨396, ⟨6, 3, 8, 1, 2⟩⟩,
  ⟨1012, ⟨5, 3, 8, 2, 2⟩⟩,
  ⟨1012, ⟨6, 2, 8, 2, 2⟩⟩,
  ⟨1012, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨1012, ⟨6, 3, 8, 1, 2⟩⟩,
  ⟨3652, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨176, ⟨5, 4, 1, 8, 2⟩⟩,
  ⟨176, ⟨6, 3, 1, 8, 2⟩⟩,
  ⟨176, ⟨6, 4, 0, 8, 2⟩⟩,
  ⟨176, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨68948, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨8844, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨8844, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨8844, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨8844, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨308396, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨68948, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨68948, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨68948, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨68948, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨492624, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨174328, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨174328, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨174328, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨174328, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨308396, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨174328, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨174328, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨174328, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨174328, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨68948, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨68948, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨68948, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨68948, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨68948, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨3652, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨8844, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨8844, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨8844, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨8844, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨176, ⟨5, 4, 8, 1, 2⟩⟩,
  ⟨176, ⟨6, 3, 8, 1, 2⟩⟩,
  ⟨176, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨176, ⟨6, 4, 8, 0, 2⟩⟩,
  ⟨9504, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨968, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨968, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨968, ⟨6, 5, 0, 7, 2⟩⟩,
  ⟨968, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨107360, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨22528, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨22528, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨22528, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨22528, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨308396, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨107360, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨107360, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨107360, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨107360, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨308396, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨174328, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨174328, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨174328, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨174328, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨107360, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨107360, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨107360, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨107360, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨107360, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨9504, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨22528, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨22528, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨22528, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨22528, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨968, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨968, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨968, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨968, ⟨6, 5, 7, 0, 2⟩⟩,
  ⟨9504, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨1584, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨1584, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨1584, ⟨6, 6, 0, 6, 2⟩⟩,
  ⟨1584, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨68948, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨22528, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨22528, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨22528, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨22528, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨124036, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨68948, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨68948, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨68948, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨68948, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨68948, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨68948, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨68948, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨68948, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨68948, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨9504, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨22528, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨22528, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨22528, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨22528, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨1584, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨1584, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨1584, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨1584, ⟨6, 6, 6, 0, 2⟩⟩,
  ⟨3652, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨968, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨968, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨968, ⟨6, 7, 0, 5, 2⟩⟩,
  ⟨968, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨16676, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨8844, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨8844, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨8844, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨8844, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨16676, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨16676, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨16676, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨16676, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨16676, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨3652, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨8844, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨8844, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨8844, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨8844, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨968, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨968, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨968, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨968, ⟨6, 7, 5, 0, 2⟩⟩,
  ⟨396, ⟨6, 8, 1, 3, 2⟩⟩,
  ⟨176, ⟨5, 8, 1, 4, 2⟩⟩,
  ⟨176, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨176, ⟨6, 8, 0, 4, 2⟩⟩,
  ⟨176, ⟨6, 8, 1, 3, 2⟩⟩,
  ⟨1012, ⟨6, 8, 2, 2, 2⟩⟩,
  ⟨1012, ⟨5, 8, 2, 3, 2⟩⟩,
  ⟨1012, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨1012, ⟨6, 8, 1, 3, 2⟩⟩,
  ⟨1012, ⟨6, 8, 2, 2, 2⟩⟩,
  ⟨396, ⟨6, 8, 3, 1, 2⟩⟩,
  ⟨1012, ⟨5, 8, 3, 2, 2⟩⟩,
  ⟨1012, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨1012, ⟨6, 8, 2, 2, 2⟩⟩,
  ⟨1012, ⟨6, 8, 3, 1, 2⟩⟩,
  ⟨176, ⟨5, 8, 4, 1, 2⟩⟩,
  ⟨176, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨176, ⟨6, 8, 3, 1, 2⟩⟩,
  ⟨176, ⟨6, 8, 4, 0, 2⟩⟩,
  ⟨44, ⟨7, 1, 2, 8, 2⟩⟩,
  ⟨880, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨44, ⟨6, 1, 3, 8, 2⟩⟩,
  ⟨44, ⟨7, 0, 3, 8, 2⟩⟩,
  ⟨44, ⟨7, 1, 2, 8, 2⟩⟩,
  ⟨44, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨3652, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨396, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨396, ⟨7, 0, 4, 7, 2⟩⟩,
  ⟨396, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨396, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨5632, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨968, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨968, ⟨7, 0, 5, 6, 2⟩⟩,
  ⟨968, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨968, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨3652, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨968, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨968, ⟨7, 0, 6, 5, 2⟩⟩,
  ⟨968, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨968, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨880, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨396, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨396, ⟨7, 0, 7, 4, 2⟩⟩,
  ⟨396, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨396, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨44, ⟨7, 1, 8, 2, 2⟩⟩,
  ⟨44, ⟨6, 1, 8, 3, 2⟩⟩,
  ⟨44, ⟨7, 0, 8, 3, 2⟩⟩,
  ⟨44, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨44, ⟨7, 1, 8, 2, 2⟩⟩,
  ⟨44, ⟨7, 2, 1, 8, 2⟩⟩,
  ⟨2244, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨132, ⟨6, 2, 2, 8, 2⟩⟩,
  ⟨132, ⟨7, 1, 2, 8, 2⟩⟩,
  ⟨132, ⟨7, 2, 1, 8, 2⟩⟩,
  ⟨132, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨16676, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨2244, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨2244, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨2244, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨2244, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨40700, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨8844, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨8844, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨8844, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨8844, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨40700, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨13464, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨13464, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨13464, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨13464, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨16676, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨8844, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨8844, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨8844, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨8844, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨2244, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨2244, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨2244, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨2244, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨2244, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨44, ⟨7, 2, 8, 1, 2⟩⟩,
  ⟨132, ⟨6, 2, 8, 2, 2⟩⟩,
  ⟨132, ⟨7, 1, 8, 2, 2⟩⟩,
  ⟨132, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨132, ⟨7, 2, 8, 1, 2⟩⟩,
  ⟨880, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨44, ⟨6, 3, 1, 8, 2⟩⟩,
  ⟨44, ⟨7, 2, 1, 8, 2⟩⟩,
  ⟨44, ⟨7, 3, 0, 8, 2⟩⟩,
  ⟨44, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨16676, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨2244, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨2244, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨2244, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨2244, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨73040, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨16676, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨16676, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨16676, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨16676, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨115544, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨40700, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨40700, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨40700, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨40700, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨73040, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨40700, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨40700, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨40700, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨40700, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨16676, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨16676, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨16676, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨16676, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨16676, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨880, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨2244, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨2244, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨2244, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨2244, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨44, ⟨6, 3, 8, 1, 2⟩⟩,
  ⟨44, ⟨7, 2, 8, 1, 2⟩⟩,
  ⟨44, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨44, ⟨7, 3, 8, 0, 2⟩⟩,
  ⟨3652, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨396, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨396, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨396, ⟨7, 4, 0, 7, 2⟩⟩,
  ⟨396, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨40700, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨8844, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨8844, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨8844, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨8844, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨115544, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨40700, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨40700, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨40700, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨40700, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨115544, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨65208, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨65208, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨65208, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨65208, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨40700, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨40700, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨40700, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨40700, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨40700, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨3652, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨8844, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨8844, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨8844, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨8844, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨396, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨396, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨396, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨396, ⟨7, 4, 7, 0, 2⟩⟩,
  ⟨5632, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨968, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨968, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨968, ⟨7, 5, 0, 6, 2⟩⟩,
  ⟨968, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨40700, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨13464, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨13464, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨13464, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨13464, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨73040, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨40700, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨40700, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨40700, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨40700, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨40700, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨40700, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨40700, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨40700, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨40700, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨5632, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨13464, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨13464, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨13464, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨13464, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨968, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨968, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨968, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨968, ⟨7, 5, 6, 0, 2⟩⟩,
  ⟨3652, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨968, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨968, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨968, ⟨7, 6, 0, 5, 2⟩⟩,
  ⟨968, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨16676, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨8844, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨8844, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨8844, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨8844, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨16676, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨16676, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨16676, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨16676, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨16676, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨3652, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨8844, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨8844, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨8844, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨8844, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨968, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨968, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨968, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨968, ⟨7, 6, 5, 0, 2⟩⟩,
  ⟨880, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨396, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨396, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨396, ⟨7, 7, 0, 4, 2⟩⟩,
  ⟨396, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨2244, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨2244, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨2244, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨2244, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨2244, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨880, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨2244, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨2244, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨2244, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨2244, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨396, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨396, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨396, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨396, ⟨7, 7, 4, 0, 2⟩⟩,
  ⟨44, ⟨7, 8, 1, 2, 2⟩⟩,
  ⟨44, ⟨6, 8, 1, 3, 2⟩⟩,
  ⟨44, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨44, ⟨7, 8, 0, 3, 2⟩⟩,
  ⟨44, ⟨7, 8, 1, 2, 2⟩⟩,
  ⟨44, ⟨7, 8, 2, 1, 2⟩⟩,
  ⟨132, ⟨6, 8, 2, 2, 2⟩⟩,
  ⟨132, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨132, ⟨7, 8, 1, 2, 2⟩⟩,
  ⟨132, ⟨7, 8, 2, 1, 2⟩⟩,
  ⟨44, ⟨6, 8, 3, 1, 2⟩⟩,
  ⟨44, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨44, ⟨7, 8, 2, 1, 2⟩⟩,
  ⟨44, ⟨7, 8, 3, 0, 2⟩⟩,
  ⟨44, ⟨8, 1, 2, 7, 2⟩⟩,
  ⟨396, ⟨8, 1, 3, 6, 2⟩⟩,
  ⟨44, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨44, ⟨8, 0, 3, 7, 2⟩⟩,
  ⟨44, ⟨8, 1, 2, 7, 2⟩⟩,
  ⟨44, ⟨8, 1, 3, 6, 2⟩⟩,
  ⟨968, ⟨8, 1, 4, 5, 2⟩⟩,
  ⟨176, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨176, ⟨8, 0, 4, 6, 2⟩⟩,
  ⟨176, ⟨8, 1, 3, 6, 2⟩⟩,
  ⟨176, ⟨8, 1, 4, 5, 2⟩⟩,
  ⟨968, ⟨8, 1, 5, 4, 2⟩⟩,
  ⟨264, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨264, ⟨8, 0, 5, 5, 2⟩⟩,
  ⟨264, ⟨8, 1, 4, 5, 2⟩⟩,
  ⟨264, ⟨8, 1, 5, 4, 2⟩⟩,
  ⟨396, ⟨8, 1, 6, 3, 2⟩⟩,
  ⟨176, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨176, ⟨8, 0, 6, 4, 2⟩⟩,
  ⟨176, ⟨8, 1, 5, 4, 2⟩⟩,
  ⟨176, ⟨8, 1, 6, 3, 2⟩⟩,
  ⟨44, ⟨8, 1, 7, 2, 2⟩⟩,
  ⟨44, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨44, ⟨8, 0, 7, 3, 2⟩⟩,
  ⟨44, ⟨8, 1, 6, 3, 2⟩⟩,
  ⟨44, ⟨8, 1, 7, 2, 2⟩⟩,
  ⟨44, ⟨8, 2, 1, 7, 2⟩⟩,
  ⟨1012, ⟨8, 2, 2, 6, 2⟩⟩,
  ⟨132, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨132, ⟨8, 1, 2, 7, 2⟩⟩,
  ⟨132, ⟨8, 2, 1, 7, 2⟩⟩,
  ⟨132, ⟨8, 2, 2, 6, 2⟩⟩,
  ⟨4444, ⟨8, 2, 3, 5, 2⟩⟩,
  ⟨1012, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨1012, ⟨8, 1, 3, 6, 2⟩⟩,
  ⟨1012, ⟨8, 2, 2, 6, 2⟩⟩,
  ⟨1012, ⟨8, 2, 3, 5, 2⟩⟩,
  ⟨6952, ⟨8, 2, 4, 4, 2⟩⟩,
  ⟨2376, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨2376, ⟨8, 1, 4, 5, 2⟩⟩,
  ⟨2376, ⟨8, 2, 3, 5, 2⟩⟩,
  ⟨2376, ⟨8, 2, 4, 4, 2⟩⟩,
  ⟨4444, ⟨8, 2, 5, 3, 2⟩⟩,
  ⟨2376, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨2376, ⟨8, 1, 5, 4, 2⟩⟩,
  ⟨2376, ⟨8, 2, 4, 4, 2⟩⟩,
  ⟨2376, ⟨8, 2, 5, 3, 2⟩⟩,
  ⟨1012, ⟨8, 2, 6, 2, 2⟩⟩,
  ⟨1012, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨1012, ⟨8, 1, 6, 3, 2⟩⟩,
  ⟨1012, ⟨8, 2, 5, 3, 2⟩⟩,
  ⟨1012, ⟨8, 2, 6, 2, 2⟩⟩,
  ⟨44, ⟨8, 2, 7, 1, 2⟩⟩,
  ⟨132, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨132, ⟨8, 1, 7, 2, 2⟩⟩,
  ⟨132, ⟨8, 2, 6, 2, 2⟩⟩,
  ⟨132, ⟨8, 2, 7, 1, 2⟩⟩,
  ⟨396, ⟨8, 3, 1, 6, 2⟩⟩,
  ⟨44, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨44, ⟨8, 2, 1, 7, 2⟩⟩,
  ⟨44, ⟨8, 3, 0, 7, 2⟩⟩,
  ⟨44, ⟨8, 3, 1, 6, 2⟩⟩,
  ⟨4444, ⟨8, 3, 2, 5, 2⟩⟩,
  ⟨1012, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨1012, ⟨8, 2, 2, 6, 2⟩⟩,
  ⟨1012, ⟨8, 3, 1, 6, 2⟩⟩,
  ⟨1012, ⟨8, 3, 2, 5, 2⟩⟩,
  ⟨12408, ⟨8, 3, 3, 4, 2⟩⟩,
  ⟨4444, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨4444, ⟨8, 2, 3, 5, 2⟩⟩,
  ⟨4444, ⟨8, 3, 2, 5, 2⟩⟩,
  ⟨4444, ⟨8, 3, 3, 4, 2⟩⟩,
  ⟨12408, ⟨8, 3, 4, 3, 2⟩⟩,
  ⟨6952, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨6952, ⟨8, 2, 4, 4, 2⟩⟩,
  ⟨6952, ⟨8, 3, 3, 4, 2⟩⟩,
  ⟨6952, ⟨8, 3, 4, 3, 2⟩⟩,
  ⟨4444, ⟨8, 3, 5, 2, 2⟩⟩,
  ⟨4444, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨4444, ⟨8, 2, 5, 3, 2⟩⟩,
  ⟨4444, ⟨8, 3, 4, 3, 2⟩⟩,
  ⟨4444, ⟨8, 3, 5, 2, 2⟩⟩,
  ⟨396, ⟨8, 3, 6, 1, 2⟩⟩,
  ⟨1012, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨1012, ⟨8, 2, 6, 2, 2⟩⟩,
  ⟨1012, ⟨8, 3, 5, 2, 2⟩⟩,
  ⟨1012, ⟨8, 3, 6, 1, 2⟩⟩,
  ⟨44, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨44, ⟨8, 2, 7, 1, 2⟩⟩,
  ⟨44, ⟨8, 3, 6, 1, 2⟩⟩,
  ⟨44, ⟨8, 3, 7, 0, 2⟩⟩,
  ⟨968, ⟨8, 4, 1, 5, 2⟩⟩,
  ⟨176, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨176, ⟨8, 3, 1, 6, 2⟩⟩,
  ⟨176, ⟨8, 4, 0, 6, 2⟩⟩,
  ⟨176, ⟨8, 4, 1, 5, 2⟩⟩,
  ⟨6952, ⟨8, 4, 2, 4, 2⟩⟩,
  ⟨2376, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨2376, ⟨8, 3, 2, 5, 2⟩⟩,
  ⟨2376, ⟨8, 4, 1, 5, 2⟩⟩,
  ⟨2376, ⟨8, 4, 2, 4, 2⟩⟩,
  ⟨12408, ⟨8, 4, 3, 3, 2⟩⟩,
  ⟨6952, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨6952, ⟨8, 3, 3, 4, 2⟩⟩,
  ⟨6952, ⟨8, 4, 2, 4, 2⟩⟩,
  ⟨6952, ⟨8, 4, 3, 3, 2⟩⟩,
  ⟨6952, ⟨8, 4, 4, 2, 2⟩⟩,
  ⟨6952, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨6952, ⟨8, 3, 4, 3, 2⟩⟩,
  ⟨6952, ⟨8, 4, 3, 3, 2⟩⟩,
  ⟨6952, ⟨8, 4, 4, 2, 2⟩⟩,
  ⟨968, ⟨8, 4, 5, 1, 2⟩⟩,
  ⟨2376, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨2376, ⟨8, 3, 5, 2, 2⟩⟩,
  ⟨2376, ⟨8, 4, 4, 2, 2⟩⟩,
  ⟨2376, ⟨8, 4, 5, 1, 2⟩⟩,
  ⟨176, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨176, ⟨8, 3, 6, 1, 2⟩⟩,
  ⟨176, ⟨8, 4, 5, 1, 2⟩⟩,
  ⟨176, ⟨8, 4, 6, 0, 2⟩⟩,
  ⟨968, ⟨8, 5, 1, 4, 2⟩⟩,
  ⟨264, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨264, ⟨8, 4, 1, 5, 2⟩⟩,
  ⟨264, ⟨8, 5, 0, 5, 2⟩⟩,
  ⟨264, ⟨8, 5, 1, 4, 2⟩⟩,
  ⟨4444, ⟨8, 5, 2, 3, 2⟩⟩,
  ⟨2376, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨2376, ⟨8, 4, 2, 4, 2⟩⟩,
  ⟨2376, ⟨8, 5, 1, 4, 2⟩⟩,
  ⟨2376, ⟨8, 5, 2, 3, 2⟩⟩,
  ⟨4444, ⟨8, 5, 3, 2, 2⟩⟩,
  ⟨4444, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨4444, ⟨8, 4, 3, 3, 2⟩⟩,
  ⟨4444, ⟨8, 5, 2, 3, 2⟩⟩,
  ⟨4444, ⟨8, 5, 3, 2, 2⟩⟩,
  ⟨968, ⟨8, 5, 4, 1, 2⟩⟩,
  ⟨2376, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨2376, ⟨8, 4, 4, 2, 2⟩⟩,
  ⟨2376, ⟨8, 5, 3, 2, 2⟩⟩,
  ⟨2376, ⟨8, 5, 4, 1, 2⟩⟩,
  ⟨264, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨264, ⟨8, 4, 5, 1, 2⟩⟩,
  ⟨264, ⟨8, 5, 4, 1, 2⟩⟩,
  ⟨264, ⟨8, 5, 5, 0, 2⟩⟩,
  ⟨396, ⟨8, 6, 1, 3, 2⟩⟩,
  ⟨176, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨176, ⟨8, 5, 1, 4, 2⟩⟩,
  ⟨176, ⟨8, 6, 0, 4, 2⟩⟩,
  ⟨176, ⟨8, 6, 1, 3, 2⟩⟩,
  ⟨1012, ⟨8, 6, 2, 2, 2⟩⟩,
  ⟨1012, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨1012, ⟨8, 5, 2, 3, 2⟩⟩,
  ⟨1012, ⟨8, 6, 1, 3, 2⟩⟩,
  ⟨1012, ⟨8, 6, 2, 2, 2⟩⟩,
  ⟨396, ⟨8, 6, 3, 1, 2⟩⟩,
  ⟨1012, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨1012, ⟨8, 5, 3, 2, 2⟩⟩,
  ⟨1012, ⟨8, 6, 2, 2, 2⟩⟩,
  ⟨1012, ⟨8, 6, 3, 1, 2⟩⟩,
  ⟨176, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨176, ⟨8, 5, 4, 1, 2⟩⟩,
  ⟨176, ⟨8, 6, 3, 1, 2⟩⟩,
  ⟨176, ⟨8, 6, 4, 0, 2⟩⟩,
  ⟨44, ⟨8, 7, 1, 2, 2⟩⟩,
  ⟨44, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨44, ⟨8, 6, 1, 3, 2⟩⟩,
  ⟨44, ⟨8, 7, 0, 3, 2⟩⟩,
  ⟨44, ⟨8, 7, 1, 2, 2⟩⟩,
  ⟨44, ⟨8, 7, 2, 1, 2⟩⟩,
  ⟨132, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨132, ⟨8, 6, 2, 2, 2⟩⟩,
  ⟨132, ⟨8, 7, 1, 2, 2⟩⟩,
  ⟨132, ⟨8, 7, 2, 1, 2⟩⟩,
  ⟨44, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨44, ⟨8, 6, 3, 1, 2⟩⟩,
  ⟨44, ⟨8, 7, 2, 1, 2⟩⟩,
  ⟨44, ⟨8, 7, 3, 0, 2⟩⟩
]

def table0Term85State8NormalizeRadix4FlattenChunk2BucketsNormal : List SparsePolynomial :=
  [table0Term85State8NormalizeRadix4Bucket2Normal]

def table0Term85State8NormalizeRadix4FlattenChunk2Normal : SparsePolynomial :=
  table0Term85State8NormalizeRadix4Bucket2Normal

end LeanProofs.PolynomialFormulas.ComputableDummitCoefficients.KernelCertificate
