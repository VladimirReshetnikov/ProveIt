...
theorem row126_product_coefficients_direct :
    ∀ j : Fin 132,
      degreeSevenProductRow ⟨126, by decide⟩ j =
        productCoefficient (degreeSevenProductSource ⟨126, by decide⟩).2
          (degreeSevenProductSource ⟨126, by decide⟩).1
          (degreeSevenRepresentative j) := by
  decide

theorem row126_literalProduct_eq_encoded_direct :
    degreeSevenLiteralProduct ⟨126, by decide⟩ =
      degreeSevenEncodedProduct ⟨126, by decide⟩ := by
  apply degreeSevenLiteralProduct_eq_encoded_of_productCoefficients
  exact row126_product_coefficients_direct
