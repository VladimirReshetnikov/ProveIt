...
theorem row124_product_coefficients_direct :
    ∀ j : Fin 132,
      degreeSevenProductRow ⟨124, by decide⟩ j =
        productCoefficient (degreeSevenProductSource ⟨124, by decide⟩).2
          (degreeSevenProductSource ⟨124, by decide⟩).1
          (degreeSevenRepresentative j) := by
  decide

theorem row124_literalProduct_eq_encoded_direct :
    degreeSevenLiteralProduct ⟨124, by decide⟩ =
      degreeSevenEncodedProduct ⟨124, by decide⟩ := by
  apply degreeSevenLiteralProduct_eq_encoded_of_productCoefficients
  exact row124_product_coefficients_direct
