import PolynomialFormulas.ComputableDummitCoefficientsCore

namespace LeanProofs.PolynomialFormulas.ComputableDummitCoefficients.KernelCertificate

open LeanProofs.PolynomialFormulas.ComputableDummitCoefficients

set_option maxRecDepth 1000000

set_option maxHeartbeats 20000000

def table0Term111State8NormalizeRadix4Bucket2Normal : SparsePolynomial := [
  ⟨1, ⟨1, 1, 8, 8, 2⟩⟩,
  ⟨17, ⟨1, 2, 7, 8, 2⟩⟩,
  ⟨17, ⟨1, 2, 8, 7, 2⟩⟩,
  ⟨2, ⟨0, 2, 8, 8, 2⟩⟩,
  ⟨2, ⟨1, 1, 8, 8, 2⟩⟩,
  ⟨2, ⟨1, 2, 7, 8, 2⟩⟩,
  ⟨2, ⟨1, 2, 8, 7, 2⟩⟩,
  ⟨81, ⟨1, 3, 6, 8, 2⟩⟩,
  ⟨172, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨17, ⟨0, 3, 7, 8, 2⟩⟩,
  ⟨17, ⟨1, 2, 7, 8, 2⟩⟩,
  ⟨17, ⟨1, 3, 6, 8, 2⟩⟩,
  ⟨17, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨81, ⟨1, 3, 8, 6, 2⟩⟩,
  ⟨17, ⟨0, 3, 8, 7, 2⟩⟩,
  ⟨17, ⟨1, 2, 8, 7, 2⟩⟩,
  ⟨17, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨17, ⟨1, 3, 8, 6, 2⟩⟩,
  ⟨165, ⟨1, 4, 5, 8, 2⟩⟩,
  ⟨565, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨50, ⟨0, 4, 6, 8, 2⟩⟩,
  ⟨50, ⟨1, 3, 6, 8, 2⟩⟩,
  ⟨50, ⟨1, 4, 5, 8, 2⟩⟩,
  ⟨50, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨565, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨105, ⟨0, 4, 7, 7, 2⟩⟩,
  ⟨105, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨105, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨105, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨165, ⟨1, 4, 8, 5, 2⟩⟩,
  ⟨50, ⟨0, 4, 8, 6, 2⟩⟩,
  ⟨50, ⟨1, 3, 8, 6, 2⟩⟩,
  ⟨50, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨50, ⟨1, 4, 8, 5, 2⟩⟩,
  ⟨165, ⟨1, 5, 4, 8, 2⟩⟩,
  ⟨820, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨70, ⟨0, 5, 5, 8, 2⟩⟩,
  ⟨70, ⟨1, 4, 5, 8, 2⟩⟩,
  ⟨70, ⟨1, 5, 4, 8, 2⟩⟩,
  ⟨70, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨1320, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨230, ⟨0, 5, 6, 7, 2⟩⟩,
  ⟨230, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨230, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨230, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨820, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨230, ⟨0, 5, 7, 6, 2⟩⟩,
  ⟨230, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨230, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨230, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨165, ⟨1, 5, 8, 4, 2⟩⟩,
  ⟨70, ⟨0, 5, 8, 5, 2⟩⟩,
  ⟨70, ⟨1, 4, 8, 5, 2⟩⟩,
  ⟨70, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨70, ⟨1, 5, 8, 4, 2⟩⟩,
  ⟨81, ⟨1, 6, 3, 8, 2⟩⟩,
  ⟨565, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨50, ⟨0, 6, 4, 8, 2⟩⟩,
  ⟨50, ⟨1, 5, 4, 8, 2⟩⟩,
  ⟨50, ⟨1, 6, 3, 8, 2⟩⟩,
  ⟨50, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨1320, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨230, ⟨0, 6, 5, 7, 2⟩⟩,
  ⟨230, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨230, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨230, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨1320, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨360, ⟨0, 6, 6, 6, 2⟩⟩,
  ⟨360, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨360, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨360, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨565, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨230, ⟨0, 6, 7, 5, 2⟩⟩,
  ⟨230, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨230, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨230, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨81, ⟨1, 6, 8, 3, 2⟩⟩,
  ⟨50, ⟨0, 6, 8, 4, 2⟩⟩,
  ⟨50, ⟨1, 5, 8, 4, 2⟩⟩,
  ⟨50, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨50, ⟨1, 6, 8, 3, 2⟩⟩,
  ⟨17, ⟨1, 7, 2, 8, 2⟩⟩,
  ⟨172, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨17, ⟨0, 7, 3, 8, 2⟩⟩,
  ⟨17, ⟨1, 6, 3, 8, 2⟩⟩,
  ⟨17, ⟨1, 7, 2, 8, 2⟩⟩,
  ⟨17, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨565, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨105, ⟨0, 7, 4, 7, 2⟩⟩,
  ⟨105, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨105, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨105, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨820, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨230, ⟨0, 7, 5, 6, 2⟩⟩,
  ⟨230, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨230, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨230, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨565, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨230, ⟨0, 7, 6, 5, 2⟩⟩,
  ⟨230, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨230, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨230, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨172, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨105, ⟨0, 7, 7, 4, 2⟩⟩,
  ⟨105, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨105, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨105, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨17, ⟨1, 7, 8, 2, 2⟩⟩,
  ⟨17, ⟨0, 7, 8, 3, 2⟩⟩,
  ⟨17, ⟨1, 6, 8, 3, 2⟩⟩,
  ⟨17, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨17, ⟨1, 7, 8, 2, 2⟩⟩,
  ⟨1, ⟨1, 8, 1, 8, 2⟩⟩,
  ⟨17, ⟨1, 8, 2, 7, 2⟩⟩,
  ⟨2, ⟨0, 8, 2, 8, 2⟩⟩,
  ⟨2, ⟨1, 7, 2, 8, 2⟩⟩,
  ⟨2, ⟨1, 8, 1, 8, 2⟩⟩,
  ⟨2, ⟨1, 8, 2, 7, 2⟩⟩,
  ⟨81, ⟨1, 8, 3, 6, 2⟩⟩,
  ⟨17, ⟨0, 8, 3, 7, 2⟩⟩,
  ⟨17, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨17, ⟨1, 8, 2, 7, 2⟩⟩,
  ⟨17, ⟨1, 8, 3, 6, 2⟩⟩,
  ⟨165, ⟨1, 8, 4, 5, 2⟩⟩,
  ⟨50, ⟨0, 8, 4, 6, 2⟩⟩,
  ⟨50, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨50, ⟨1, 8, 3, 6, 2⟩⟩,
  ⟨50, ⟨1, 8, 4, 5, 2⟩⟩,
  ⟨165, ⟨1, 8, 5, 4, 2⟩⟩,
  ⟨70, ⟨0, 8, 5, 5, 2⟩⟩,
  ⟨70, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨70, ⟨1, 8, 4, 5, 2⟩⟩,
  ⟨70, ⟨1, 8, 5, 4, 2⟩⟩,
  ⟨81, ⟨1, 8, 6, 3, 2⟩⟩,
  ⟨50, ⟨0, 8, 6, 4, 2⟩⟩,
  ⟨50, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨50, ⟨1, 8, 5, 4, 2⟩⟩,
  ⟨50, ⟨1, 8, 6, 3, 2⟩⟩,
  ⟨17, ⟨1, 8, 7, 2, 2⟩⟩,
  ⟨17, ⟨0, 8, 7, 3, 2⟩⟩,
  ⟨17, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨17, ⟨1, 8, 6, 3, 2⟩⟩,
  ⟨17, ⟨1, 8, 7, 2, 2⟩⟩,
  ⟨1, ⟨1, 8, 8, 1, 2⟩⟩,
  ⟨2, ⟨0, 8, 8, 2, 2⟩⟩,
  ⟨2, ⟨1, 7, 8, 2, 2⟩⟩,
  ⟨2, ⟨1, 8, 7, 2, 2⟩⟩,
  ⟨2, ⟨1, 8, 8, 1, 2⟩⟩,
  ⟨17, ⟨2, 1, 7, 8, 2⟩⟩,
  ⟨17, ⟨2, 1, 8, 7, 2⟩⟩,
  ⟨2, ⟨1, 1, 8, 8, 2⟩⟩,
  ⟨2, ⟨2, 0, 8, 8, 2⟩⟩,
  ⟨2, ⟨2, 1, 7, 8, 2⟩⟩,
  ⟨2, ⟨2, 1, 8, 7, 2⟩⟩,
  ⟨172, ⟨2, 2, 6, 8, 2⟩⟩,
  ⟨371, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨36, ⟨1, 2, 7, 8, 2⟩⟩,
  ⟨36, ⟨2, 1, 7, 8, 2⟩⟩,
  ⟨36, ⟨2, 2, 6, 8, 2⟩⟩,
  ⟨36, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨172, ⟨2, 2, 8, 6, 2⟩⟩,
  ⟨36, ⟨1, 2, 8, 7, 2⟩⟩,
  ⟨36, ⟨2, 1, 8, 7, 2⟩⟩,
  ⟨36, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨36, ⟨2, 2, 8, 6, 2⟩⟩,
  ⟨565, ⟨2, 3, 5, 8, 2⟩⟩,
  ⟨2007, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨172, ⟨1, 3, 6, 8, 2⟩⟩,
  ⟨172, ⟨2, 2, 6, 8, 2⟩⟩,
  ⟨172, ⟨2, 3, 5, 8, 2⟩⟩,
  ⟨172, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨2007, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨371, ⟨1, 3, 7, 7, 2⟩⟩,
  ⟨371, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨371, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨371, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨565, ⟨2, 3, 8, 5, 2⟩⟩,
  ⟨172, ⟨1, 3, 8, 6, 2⟩⟩,
  ⟨172, ⟨2, 2, 8, 6, 2⟩⟩,
  ⟨172, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨172, ⟨2, 3, 8, 5, 2⟩⟩,
  ⟨820, ⟨2, 4, 4, 8, 2⟩⟩,
  ⟨4345, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨350, ⟨1, 4, 5, 8, 2⟩⟩,
  ⟨350, ⟨2, 3, 5, 8, 2⟩⟩,
  ⟨350, ⟨2, 4, 4, 8, 2⟩⟩,
  ⟨350, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨7140, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨1220, ⟨1, 4, 6, 7, 2⟩⟩,
  ⟨1220, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨1220, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨1220, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨4345, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨1220, ⟨1, 4, 7, 6, 2⟩⟩,
  ⟨1220, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨1220, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨1220, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨820, ⟨2, 4, 8, 4, 2⟩⟩,
  ⟨350, ⟨1, 4, 8, 5, 2⟩⟩,
  ⟨350, ⟨2, 3, 8, 5, 2⟩⟩,
  ⟨350, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨350, ⟨2, 4, 8, 4, 2⟩⟩,
  ⟨565, ⟨2, 5, 3, 8, 2⟩⟩,
  ⟨4345, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨350, ⟨1, 5, 4, 8, 2⟩⟩,
  ⟨350, ⟨2, 4, 4, 8, 2⟩⟩,
  ⟨350, ⟨2, 5, 3, 8, 2⟩⟩,
  ⟨350, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨10640, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨1770, ⟨1, 5, 5, 7, 2⟩⟩,
  ⟨1770, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨1770, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨1770, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨10640, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨2860, ⟨1, 5, 6, 6, 2⟩⟩,
  ⟨2860, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨2860, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨2860, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨4345, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨1770, ⟨1, 5, 7, 5, 2⟩⟩,
  ⟨1770, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨1770, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨1770, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨565, ⟨2, 5, 8, 3, 2⟩⟩,
  ⟨350, ⟨1, 5, 8, 4, 2⟩⟩,
  ⟨350, ⟨2, 4, 8, 4, 2⟩⟩,
  ⟨350, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨350, ⟨2, 5, 8, 3, 2⟩⟩,
  ⟨172, ⟨2, 6, 2, 8, 2⟩⟩,
  ⟨2007, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨172, ⟨1, 6, 3, 8, 2⟩⟩,
  ⟨172, ⟨2, 5, 3, 8, 2⟩⟩,
  ⟨172, ⟨2, 6, 2, 8, 2⟩⟩,
  ⟨172, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨7140, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨1220, ⟨1, 6, 4, 7, 2⟩⟩,
  ⟨1220, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨1220, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨1220, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨10640, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨2860, ⟨1, 6, 5, 6, 2⟩⟩,
  ⟨2860, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨2860, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨2860, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨7140, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨2860, ⟨1, 6, 6, 5, 2⟩⟩,
  ⟨2860, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨2860, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨2860, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨2007, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨1220, ⟨1, 6, 7, 4, 2⟩⟩,
  ⟨1220, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨1220, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨1220, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨172, ⟨2, 6, 8, 2, 2⟩⟩,
  ⟨172, ⟨1, 6, 8, 3, 2⟩⟩,
  ⟨172, ⟨2, 5, 8, 3, 2⟩⟩,
  ⟨172, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨172, ⟨2, 6, 8, 2, 2⟩⟩,
  ⟨17, ⟨2, 7, 1, 8, 2⟩⟩,
  ⟨371, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨36, ⟨1, 7, 2, 8, 2⟩⟩,
  ⟨36, ⟨2, 6, 2, 8, 2⟩⟩,
  ⟨36, ⟨2, 7, 1, 8, 2⟩⟩,
  ⟨36, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨2007, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨371, ⟨1, 7, 3, 7, 2⟩⟩,
  ⟨371, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨371, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨371, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨4345, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨1220, ⟨1, 7, 4, 6, 2⟩⟩,
  ⟨1220, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨1220, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨1220, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨4345, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨1770, ⟨1, 7, 5, 5, 2⟩⟩,
  ⟨1770, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨1770, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨1770, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨2007, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨1220, ⟨1, 7, 6, 4, 2⟩⟩,
  ⟨1220, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨1220, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨1220, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨371, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨371, ⟨1, 7, 7, 3, 2⟩⟩,
  ⟨371, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨371, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨371, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨17, ⟨2, 7, 8, 1, 2⟩⟩,
  ⟨36, ⟨1, 7, 8, 2, 2⟩⟩,
  ⟨36, ⟨2, 6, 8, 2, 2⟩⟩,
  ⟨36, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨36, ⟨2, 7, 8, 1, 2⟩⟩,
  ⟨17, ⟨2, 8, 1, 7, 2⟩⟩,
  ⟨2, ⟨1, 8, 1, 8, 2⟩⟩,
  ⟨2, ⟨2, 7, 1, 8, 2⟩⟩,
  ⟨2, ⟨2, 8, 0, 8, 2⟩⟩,
  ⟨2, ⟨2, 8, 1, 7, 2⟩⟩,
  ⟨172, ⟨2, 8, 2, 6, 2⟩⟩,
  ⟨36, ⟨1, 8, 2, 7, 2⟩⟩,
  ⟨36, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨36, ⟨2, 8, 1, 7, 2⟩⟩,
  ⟨36, ⟨2, 8, 2, 6, 2⟩⟩,
  ⟨565, ⟨2, 8, 3, 5, 2⟩⟩,
  ⟨172, ⟨1, 8, 3, 6, 2⟩⟩,
  ⟨172, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨172, ⟨2, 8, 2, 6, 2⟩⟩,
  ⟨172, ⟨2, 8, 3, 5, 2⟩⟩,
  ⟨820, ⟨2, 8, 4, 4, 2⟩⟩,
  ⟨350, ⟨1, 8, 4, 5, 2⟩⟩,
  ⟨350, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨350, ⟨2, 8, 3, 5, 2⟩⟩,
  ⟨350, ⟨2, 8, 4, 4, 2⟩⟩,
  ⟨565, ⟨2, 8, 5, 3, 2⟩⟩,
  ⟨350, ⟨1, 8, 5, 4, 2⟩⟩,
  ⟨350, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨350, ⟨2, 8, 4, 4, 2⟩⟩,
  ⟨350, ⟨2, 8, 5, 3, 2⟩⟩,
  ⟨172, ⟨2, 8, 6, 2, 2⟩⟩,
  ⟨172, ⟨1, 8, 6, 3, 2⟩⟩,
  ⟨172, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨172, ⟨2, 8, 5, 3, 2⟩⟩,
  ⟨172, ⟨2, 8, 6, 2, 2⟩⟩,
  ⟨17, ⟨2, 8, 7, 1, 2⟩⟩,
  ⟨36, ⟨1, 8, 7, 2, 2⟩⟩,
  ⟨36, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨36, ⟨2, 8, 6, 2, 2⟩⟩,
  ⟨36, ⟨2, 8, 7, 1, 2⟩⟩,
  ⟨2, ⟨1, 8, 8, 1, 2⟩⟩,
  ⟨2, ⟨2, 7, 8, 1, 2⟩⟩,
  ⟨2, ⟨2, 8, 7, 1, 2⟩⟩,
  ⟨2, ⟨2, 8, 8, 0, 2⟩⟩,
  ⟨81, ⟨3, 1, 6, 8, 2⟩⟩,
  ⟨172, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨17, ⟨2, 1, 7, 8, 2⟩⟩,
  ⟨17, ⟨3, 0, 7, 8, 2⟩⟩,
  ⟨17, ⟨3, 1, 6, 8, 2⟩⟩,
  ⟨17, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨81, ⟨3, 1, 8, 6, 2⟩⟩,
  ⟨17, ⟨2, 1, 8, 7, 2⟩⟩,
  ⟨17, ⟨3, 0, 8, 7, 2⟩⟩,
  ⟨17, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨17, ⟨3, 1, 8, 6, 2⟩⟩,
  ⟨565, ⟨3, 2, 5, 8, 2⟩⟩,
  ⟨2007, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨172, ⟨2, 2, 6, 8, 2⟩⟩,
  ⟨172, ⟨3, 1, 6, 8, 2⟩⟩,
  ⟨172, ⟨3, 2, 5, 8, 2⟩⟩,
  ⟨172, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨2007, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨371, ⟨2, 2, 7, 7, 2⟩⟩,
  ⟨371, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨371, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨371, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨565, ⟨3, 2, 8, 5, 2⟩⟩,
  ⟨172, ⟨2, 2, 8, 6, 2⟩⟩,
  ⟨172, ⟨3, 1, 8, 6, 2⟩⟩,
  ⟨172, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨172, ⟨3, 2, 8, 5, 2⟩⟩,
  ⟨1320, ⟨3, 3, 4, 8, 2⟩⟩,
  ⟨7140, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨565, ⟨2, 3, 5, 8, 2⟩⟩,
  ⟨565, ⟨3, 2, 5, 8, 2⟩⟩,
  ⟨565, ⟨3, 3, 4, 8, 2⟩⟩,
  ⟨565, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨11821, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨2007, ⟨2, 3, 6, 7, 2⟩⟩,
  ⟨2007, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨2007, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨2007, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨7140, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨2007, ⟨2, 3, 7, 6, 2⟩⟩,
  ⟨2007, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨2007, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨2007, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨1320, ⟨3, 3, 8, 4, 2⟩⟩,
  ⟨565, ⟨2, 3, 8, 5, 2⟩⟩,
  ⟨565, ⟨3, 2, 8, 5, 2⟩⟩,
  ⟨565, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨565, ⟨3, 3, 8, 4, 2⟩⟩,
  ⟨1320, ⟨3, 4, 3, 8, 2⟩⟩,
  ⟨10640, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨820, ⟨2, 4, 4, 8, 2⟩⟩,
  ⟨820, ⟨3, 3, 4, 8, 2⟩⟩,
  ⟨820, ⟨3, 4, 3, 8, 2⟩⟩,
  ⟨820, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨26715, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨4345, ⟨2, 4, 5, 7, 2⟩⟩,
  ⟨4345, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨4345, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨4345, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨26715, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨7140, ⟨2, 4, 6, 6, 2⟩⟩,
  ⟨7140, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨7140, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨7140, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨10640, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨4345, ⟨2, 4, 7, 5, 2⟩⟩,
  ⟨4345, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨4345, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨4345, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨1320, ⟨3, 4, 8, 3, 2⟩⟩,
  ⟨820, ⟨2, 4, 8, 4, 2⟩⟩,
  ⟨820, ⟨3, 3, 8, 4, 2⟩⟩,
  ⟨820, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨820, ⟨3, 4, 8, 3, 2⟩⟩,
  ⟨565, ⟨3, 5, 2, 8, 2⟩⟩,
  ⟨7140, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨565, ⟨2, 5, 3, 8, 2⟩⟩,
  ⟨565, ⟨3, 4, 3, 8, 2⟩⟩,
  ⟨565, ⟨3, 5, 2, 8, 2⟩⟩,
  ⟨565, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨26715, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨4345, ⟨2, 5, 4, 7, 2⟩⟩,
  ⟨4345, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨4345, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨4345, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨40500, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨10640, ⟨2, 5, 5, 6, 2⟩⟩,
  ⟨10640, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨10640, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨10640, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨26715, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨10640, ⟨2, 5, 6, 5, 2⟩⟩,
  ⟨10640, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨10640, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨10640, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨7140, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨4345, ⟨2, 5, 7, 4, 2⟩⟩,
  ⟨4345, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨4345, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨4345, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨565, ⟨3, 5, 8, 2, 2⟩⟩,
  ⟨565, ⟨2, 5, 8, 3, 2⟩⟩,
  ⟨565, ⟨3, 4, 8, 3, 2⟩⟩,
  ⟨565, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨565, ⟨3, 5, 8, 2, 2⟩⟩,
  ⟨81, ⟨3, 6, 1, 8, 2⟩⟩,
  ⟨2007, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨172, ⟨2, 6, 2, 8, 2⟩⟩,
  ⟨172, ⟨3, 5, 2, 8, 2⟩⟩,
  ⟨172, ⟨3, 6, 1, 8, 2⟩⟩,
  ⟨172, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨11821, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨2007, ⟨2, 6, 3, 7, 2⟩⟩,
  ⟨2007, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨2007, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨2007, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨26715, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨7140, ⟨2, 6, 4, 6, 2⟩⟩,
  ⟨7140, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨7140, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨7140, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨26715, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨10640, ⟨2, 6, 5, 5, 2⟩⟩,
  ⟨10640, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨10640, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨10640, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨11821, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨7140, ⟨2, 6, 6, 4, 2⟩⟩,
  ⟨7140, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨7140, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨7140, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨2007, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨2007, ⟨2, 6, 7, 3, 2⟩⟩,
  ⟨2007, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨2007, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨2007, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨81, ⟨3, 6, 8, 1, 2⟩⟩,
  ⟨172, ⟨2, 6, 8, 2, 2⟩⟩,
  ⟨172, ⟨3, 5, 8, 2, 2⟩⟩,
  ⟨172, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨172, ⟨3, 6, 8, 1, 2⟩⟩,
  ⟨172, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨17, ⟨2, 7, 1, 8, 2⟩⟩,
  ⟨17, ⟨3, 6, 1, 8, 2⟩⟩,
  ⟨17, ⟨3, 7, 0, 8, 2⟩⟩,
  ⟨17, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨2007, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨371, ⟨2, 7, 2, 7, 2⟩⟩,
  ⟨371, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨371, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨371, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨7140, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨2007, ⟨2, 7, 3, 6, 2⟩⟩,
  ⟨2007, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨2007, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨2007, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨10640, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨4345, ⟨2, 7, 4, 5, 2⟩⟩,
  ⟨4345, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨4345, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨4345, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨7140, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨4345, ⟨2, 7, 5, 4, 2⟩⟩,
  ⟨4345, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨4345, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨4345, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨2007, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨2007, ⟨2, 7, 6, 3, 2⟩⟩,
  ⟨2007, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨2007, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨2007, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨172, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨371, ⟨2, 7, 7, 2, 2⟩⟩,
  ⟨371, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨371, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨371, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨17, ⟨2, 7, 8, 1, 2⟩⟩,
  ⟨17, ⟨3, 6, 8, 1, 2⟩⟩,
  ⟨17, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨17, ⟨3, 7, 8, 0, 2⟩⟩,
  ⟨81, ⟨3, 8, 1, 6, 2⟩⟩,
  ⟨17, ⟨2, 8, 1, 7, 2⟩⟩,
  ⟨17, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨17, ⟨3, 8, 0, 7, 2⟩⟩,
  ⟨17, ⟨3, 8, 1, 6, 2⟩⟩,
  ⟨565, ⟨3, 8, 2, 5, 2⟩⟩,
  ⟨172, ⟨2, 8, 2, 6, 2⟩⟩,
  ⟨172, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨172, ⟨3, 8, 1, 6, 2⟩⟩,
  ⟨172, ⟨3, 8, 2, 5, 2⟩⟩,
  ⟨1320, ⟨3, 8, 3, 4, 2⟩⟩,
  ⟨565, ⟨2, 8, 3, 5, 2⟩⟩,
  ⟨565, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨565, ⟨3, 8, 2, 5, 2⟩⟩,
  ⟨565, ⟨3, 8, 3, 4, 2⟩⟩,
  ⟨1320, ⟨3, 8, 4, 3, 2⟩⟩,
  ⟨820, ⟨2, 8, 4, 4, 2⟩⟩,
  ⟨820, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨820, ⟨3, 8, 3, 4, 2⟩⟩,
  ⟨820, ⟨3, 8, 4, 3, 2⟩⟩,
  ⟨565, ⟨3, 8, 5, 2, 2⟩⟩,
  ⟨565, ⟨2, 8, 5, 3, 2⟩⟩,
  ⟨565, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨565, ⟨3, 8, 4, 3, 2⟩⟩,
  ⟨565, ⟨3, 8, 5, 2, 2⟩⟩,
  ⟨81, ⟨3, 8, 6, 1, 2⟩⟩,
  ⟨172, ⟨2, 8, 6, 2, 2⟩⟩,
  ⟨172, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨172, ⟨3, 8, 5, 2, 2⟩⟩,
  ⟨172, ⟨3, 8, 6, 1, 2⟩⟩,
  ⟨17, ⟨2, 8, 7, 1, 2⟩⟩,
  ⟨17, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨17, ⟨3, 8, 6, 1, 2⟩⟩,
  ⟨17, ⟨3, 8, 7, 0, 2⟩⟩,
  ⟨165, ⟨4, 1, 5, 8, 2⟩⟩,
  ⟨565, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨50, ⟨3, 1, 6, 8, 2⟩⟩,
  ⟨50, ⟨4, 0, 6, 8, 2⟩⟩,
  ⟨50, ⟨4, 1, 5, 8, 2⟩⟩,
  ⟨50, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨565, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨105, ⟨3, 1, 7, 7, 2⟩⟩,
  ⟨105, ⟨4, 0, 7, 7, 2⟩⟩,
  ⟨105, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨105, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨165, ⟨4, 1, 8, 5, 2⟩⟩,
  ⟨50, ⟨3, 1, 8, 6, 2⟩⟩,
  ⟨50, ⟨4, 0, 8, 6, 2⟩⟩,
  ⟨50, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨50, ⟨4, 1, 8, 5, 2⟩⟩,
  ⟨820, ⟨4, 2, 4, 8, 2⟩⟩,
  ⟨4345, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨350, ⟨3, 2, 5, 8, 2⟩⟩,
  ⟨350, ⟨4, 1, 5, 8, 2⟩⟩,
  ⟨350, ⟨4, 2, 4, 8, 2⟩⟩,
  ⟨350, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨7140, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨1220, ⟨3, 2, 6, 7, 2⟩⟩,
  ⟨1220, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨1220, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨1220, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨4345, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨1220, ⟨3, 2, 7, 6, 2⟩⟩,
  ⟨1220, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨1220, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨1220, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨820, ⟨4, 2, 8, 4, 2⟩⟩,
  ⟨350, ⟨3, 2, 8, 5, 2⟩⟩,
  ⟨350, ⟨4, 1, 8, 5, 2⟩⟩,
  ⟨350, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨350, ⟨4, 2, 8, 4, 2⟩⟩,
  ⟨1320, ⟨4, 3, 3, 8, 2⟩⟩,
  ⟨10640, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨820, ⟨3, 3, 4, 8, 2⟩⟩,
  ⟨820, ⟨4, 2, 4, 8, 2⟩⟩,
  ⟨820, ⟨4, 3, 3, 8, 2⟩⟩,
  ⟨820, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨26715, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨4345, ⟨3, 3, 5, 7, 2⟩⟩,
  ⟨4345, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨4345, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨4345, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨26715, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨7140, ⟨3, 3, 6, 6, 2⟩⟩,
  ⟨7140, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨7140, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨7140, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨10640, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨4345, ⟨3, 3, 7, 5, 2⟩⟩,
  ⟨4345, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨4345, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨4345, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨1320, ⟨4, 3, 8, 3, 2⟩⟩,
  ⟨820, ⟨3, 3, 8, 4, 2⟩⟩,
  ⟨820, ⟨4, 2, 8, 4, 2⟩⟩,
  ⟨820, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨820, ⟨4, 3, 8, 3, 2⟩⟩,
  ⟨820, ⟨4, 4, 2, 8, 2⟩⟩,
  ⟨10640, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨820, ⟨3, 4, 3, 8, 2⟩⟩,
  ⟨820, ⟨4, 3, 3, 8, 2⟩⟩,
  ⟨820, ⟨4, 4, 2, 8, 2⟩⟩,
  ⟨820, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨40500, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨6480, ⟨3, 4, 4, 7, 2⟩⟩,
  ⟨6480, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨6480, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨6480, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨61775, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨16110, ⟨3, 4, 5, 6, 2⟩⟩,
  ⟨16110, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨16110, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨16110, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨40500, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨16110, ⟨3, 4, 6, 5, 2⟩⟩,
  ⟨16110, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨16110, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨16110, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨10640, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨6480, ⟨3, 4, 7, 4, 2⟩⟩,
  ⟨6480, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨6480, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨6480, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨820, ⟨4, 4, 8, 2, 2⟩⟩,
  ⟨820, ⟨3, 4, 8, 3, 2⟩⟩,
  ⟨820, ⟨4, 3, 8, 3, 2⟩⟩,
  ⟨820, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨820, ⟨4, 4, 8, 2, 2⟩⟩,
  ⟨165, ⟨4, 5, 1, 8, 2⟩⟩,
  ⟨4345, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨350, ⟨3, 5, 2, 8, 2⟩⟩,
  ⟨350, ⟨4, 4, 2, 8, 2⟩⟩,
  ⟨350, ⟨4, 5, 1, 8, 2⟩⟩,
  ⟨350, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨26715, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨4345, ⟨3, 5, 3, 7, 2⟩⟩,
  ⟨4345, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨4345, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨4345, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨61775, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨16110, ⟨3, 5, 4, 6, 2⟩⟩,
  ⟨16110, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨16110, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨16110, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨61775, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨24360, ⟨3, 5, 5, 5, 2⟩⟩,
  ⟨24360, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨24360, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨24360, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨26715, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨16110, ⟨3, 5, 6, 4, 2⟩⟩,
  ⟨16110, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨16110, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨16110, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨4345, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨4345, ⟨3, 5, 7, 3, 2⟩⟩,
  ⟨4345, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨4345, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨4345, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨165, ⟨4, 5, 8, 1, 2⟩⟩,
  ⟨350, ⟨3, 5, 8, 2, 2⟩⟩,
  ⟨350, ⟨4, 4, 8, 2, 2⟩⟩,
  ⟨350, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨350, ⟨4, 5, 8, 1, 2⟩⟩,
  ⟨565, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨50, ⟨3, 6, 1, 8, 2⟩⟩,
  ⟨50, ⟨4, 5, 1, 8, 2⟩⟩,
  ⟨50, ⟨4, 6, 0, 8, 2⟩⟩,
  ⟨50, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨7140, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨1220, ⟨3, 6, 2, 7, 2⟩⟩,
  ⟨1220, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨1220, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨1220, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨26715, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨7140, ⟨3, 6, 3, 6, 2⟩⟩,
  ⟨7140, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨7140, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨7140, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨40500, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨16110, ⟨3, 6, 4, 5, 2⟩⟩,
  ⟨16110, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨16110, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨16110, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨26715, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨16110, ⟨3, 6, 5, 4, 2⟩⟩,
  ⟨16110, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨16110, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨16110, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨7140, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨7140, ⟨3, 6, 6, 3, 2⟩⟩,
  ⟨7140, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨7140, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨7140, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨565, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨1220, ⟨3, 6, 7, 2, 2⟩⟩,
  ⟨1220, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨1220, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨1220, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨50, ⟨3, 6, 8, 1, 2⟩⟩,
  ⟨50, ⟨4, 5, 8, 1, 2⟩⟩,
  ⟨50, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨50, ⟨4, 6, 8, 0, 2⟩⟩,
  ⟨565, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨105, ⟨3, 7, 1, 7, 2⟩⟩,
  ⟨105, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨105, ⟨4, 7, 0, 7, 2⟩⟩,
  ⟨105, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨4345, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨1220, ⟨3, 7, 2, 6, 2⟩⟩,
  ⟨1220, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨1220, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨1220, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨10640, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨4345, ⟨3, 7, 3, 5, 2⟩⟩,
  ⟨4345, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨4345, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨4345, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨10640, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨6480, ⟨3, 7, 4, 4, 2⟩⟩,
  ⟨6480, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨6480, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨6480, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨4345, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨4345, ⟨3, 7, 5, 3, 2⟩⟩,
  ⟨4345, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨4345, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨4345, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨565, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨1220, ⟨3, 7, 6, 2, 2⟩⟩,
  ⟨1220, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨1220, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨1220, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨105, ⟨3, 7, 7, 1, 2⟩⟩,
  ⟨105, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨105, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨105, ⟨4, 7, 7, 0, 2⟩⟩,
  ⟨165, ⟨4, 8, 1, 5, 2⟩⟩,
  ⟨50, ⟨3, 8, 1, 6, 2⟩⟩,
  ⟨50, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨50, ⟨4, 8, 0, 6, 2⟩⟩,
  ⟨50, ⟨4, 8, 1, 5, 2⟩⟩,
  ⟨820, ⟨4, 8, 2, 4, 2⟩⟩,
  ⟨350, ⟨3, 8, 2, 5, 2⟩⟩,
  ⟨350, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨350, ⟨4, 8, 1, 5, 2⟩⟩,
  ⟨350, ⟨4, 8, 2, 4, 2⟩⟩,
  ⟨1320, ⟨4, 8, 3, 3, 2⟩⟩,
  ⟨820, ⟨3, 8, 3, 4, 2⟩⟩,
  ⟨820, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨820, ⟨4, 8, 2, 4, 2⟩⟩,
  ⟨820, ⟨4, 8, 3, 3, 2⟩⟩,
  ⟨820, ⟨4, 8, 4, 2, 2⟩⟩,
  ⟨820, ⟨3, 8, 4, 3, 2⟩⟩,
  ⟨820, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨820, ⟨4, 8, 3, 3, 2⟩⟩,
  ⟨820, ⟨4, 8, 4, 2, 2⟩⟩,
  ⟨165, ⟨4, 8, 5, 1, 2⟩⟩,
  ⟨350, ⟨3, 8, 5, 2, 2⟩⟩,
  ⟨350, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨350, ⟨4, 8, 4, 2, 2⟩⟩,
  ⟨350, ⟨4, 8, 5, 1, 2⟩⟩,
  ⟨50, ⟨3, 8, 6, 1, 2⟩⟩,
  ⟨50, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨50, ⟨4, 8, 5, 1, 2⟩⟩,
  ⟨50, ⟨4, 8, 6, 0, 2⟩⟩,
  ⟨165, ⟨5, 1, 4, 8, 2⟩⟩,
  ⟨820, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨70, ⟨4, 1, 5, 8, 2⟩⟩,
  ⟨70, ⟨5, 0, 5, 8, 2⟩⟩,
  ⟨70, ⟨5, 1, 4, 8, 2⟩⟩,
  ⟨70, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨1320, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨230, ⟨4, 1, 6, 7, 2⟩⟩,
  ⟨230, ⟨5, 0, 6, 7, 2⟩⟩,
  ⟨230, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨230, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨820, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨230, ⟨4, 1, 7, 6, 2⟩⟩,
  ⟨230, ⟨5, 0, 7, 6, 2⟩⟩,
  ⟨230, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨230, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨165, ⟨5, 1, 8, 4, 2⟩⟩,
  ⟨70, ⟨4, 1, 8, 5, 2⟩⟩,
  ⟨70, ⟨5, 0, 8, 5, 2⟩⟩,
  ⟨70, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨70, ⟨5, 1, 8, 4, 2⟩⟩,
  ⟨565, ⟨5, 2, 3, 8, 2⟩⟩,
  ⟨4345, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨350, ⟨4, 2, 4, 8, 2⟩⟩,
  ⟨350, ⟨5, 1, 4, 8, 2⟩⟩,
  ⟨350, ⟨5, 2, 3, 8, 2⟩⟩,
  ⟨350, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨10640, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨1770, ⟨4, 2, 5, 7, 2⟩⟩,
  ⟨1770, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨1770, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨1770, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨10640, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨2860, ⟨4, 2, 6, 6, 2⟩⟩,
  ⟨2860, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨2860, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨2860, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨4345, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨1770, ⟨4, 2, 7, 5, 2⟩⟩,
  ⟨1770, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨1770, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨1770, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨565, ⟨5, 2, 8, 3, 2⟩⟩,
  ⟨350, ⟨4, 2, 8, 4, 2⟩⟩,
  ⟨350, ⟨5, 1, 8, 4, 2⟩⟩,
  ⟨350, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨350, ⟨5, 2, 8, 3, 2⟩⟩,
  ⟨565, ⟨5, 3, 2, 8, 2⟩⟩,
  ⟨7140, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨565, ⟨4, 3, 3, 8, 2⟩⟩,
  ⟨565, ⟨5, 2, 3, 8, 2⟩⟩,
  ⟨565, ⟨5, 3, 2, 8, 2⟩⟩,
  ⟨565, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨26715, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨4345, ⟨4, 3, 4, 7, 2⟩⟩,
  ⟨4345, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨4345, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨4345, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨40500, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨10640, ⟨4, 3, 5, 6, 2⟩⟩,
  ⟨10640, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨10640, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨10640, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨26715, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨10640, ⟨4, 3, 6, 5, 2⟩⟩,
  ⟨10640, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨10640, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨10640, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨7140, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨4345, ⟨4, 3, 7, 4, 2⟩⟩,
  ⟨4345, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨4345, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨4345, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨565, ⟨5, 3, 8, 2, 2⟩⟩,
  ⟨565, ⟨4, 3, 8, 3, 2⟩⟩,
  ⟨565, ⟨5, 2, 8, 3, 2⟩⟩,
  ⟨565, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨565, ⟨5, 3, 8, 2, 2⟩⟩,
  ⟨165, ⟨5, 4, 1, 8, 2⟩⟩,
  ⟨4345, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨350, ⟨4, 4, 2, 8, 2⟩⟩,
  ⟨350, ⟨5, 3, 2, 8, 2⟩⟩,
  ⟨350, ⟨5, 4, 1, 8, 2⟩⟩,
  ⟨350, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨26715, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨4345, ⟨4, 4, 3, 7, 2⟩⟩,
  ⟨4345, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨4345, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨4345, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨61775, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨16110, ⟨4, 4, 4, 6, 2⟩⟩,
  ⟨16110, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨16110, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨16110, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨61775, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨24360, ⟨4, 4, 5, 5, 2⟩⟩,
  ⟨24360, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨24360, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨24360, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨26715, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨16110, ⟨4, 4, 6, 4, 2⟩⟩,
  ⟨16110, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨16110, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨16110, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨4345, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨4345, ⟨4, 4, 7, 3, 2⟩⟩,
  ⟨4345, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨4345, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨4345, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨165, ⟨5, 4, 8, 1, 2⟩⟩,
  ⟨350, ⟨4, 4, 8, 2, 2⟩⟩,
  ⟨350, ⟨5, 3, 8, 2, 2⟩⟩,
  ⟨350, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨350, ⟨5, 4, 8, 1, 2⟩⟩,
  ⟨820, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨70, ⟨4, 5, 1, 8, 2⟩⟩,
  ⟨70, ⟨5, 4, 1, 8, 2⟩⟩,
  ⟨70, ⟨5, 5, 0, 8, 2⟩⟩,
  ⟨70, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨10640, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨1770, ⟨4, 5, 2, 7, 2⟩⟩,
  ⟨1770, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨1770, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨1770, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨40500, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨10640, ⟨4, 5, 3, 6, 2⟩⟩,
  ⟨10640, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨10640, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨10640, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨61775, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨24360, ⟨4, 5, 4, 5, 2⟩⟩,
  ⟨24360, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨24360, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨24360, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨40500, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨24360, ⟨4, 5, 5, 4, 2⟩⟩,
  ⟨24360, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨24360, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨24360, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨10640, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨10640, ⟨4, 5, 6, 3, 2⟩⟩,
  ⟨10640, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨10640, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨10640, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨820, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨1770, ⟨4, 5, 7, 2, 2⟩⟩,
  ⟨1770, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨1770, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨1770, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨70, ⟨4, 5, 8, 1, 2⟩⟩,
  ⟨70, ⟨5, 4, 8, 1, 2⟩⟩,
  ⟨70, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨70, ⟨5, 5, 8, 0, 2⟩⟩,
  ⟨1320, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨230, ⟨4, 6, 1, 7, 2⟩⟩,
  ⟨230, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨230, ⟨5, 6, 0, 7, 2⟩⟩,
  ⟨230, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨10640, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨2860, ⟨4, 6, 2, 6, 2⟩⟩,
  ⟨2860, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨2860, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨2860, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨26715, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨10640, ⟨4, 6, 3, 5, 2⟩⟩,
  ⟨10640, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨10640, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨10640, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨26715, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨16110, ⟨4, 6, 4, 4, 2⟩⟩,
  ⟨16110, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨16110, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨16110, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨10640, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨10640, ⟨4, 6, 5, 3, 2⟩⟩,
  ⟨10640, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨10640, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨10640, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨1320, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨2860, ⟨4, 6, 6, 2, 2⟩⟩,
  ⟨2860, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨2860, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨2860, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨230, ⟨4, 6, 7, 1, 2⟩⟩,
  ⟨230, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨230, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨230, ⟨5, 6, 7, 0, 2⟩⟩,
  ⟨820, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨230, ⟨4, 7, 1, 6, 2⟩⟩,
  ⟨230, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨230, ⟨5, 7, 0, 6, 2⟩⟩,
  ⟨230, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨4345, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨1770, ⟨4, 7, 2, 5, 2⟩⟩,
  ⟨1770, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨1770, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨1770, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨7140, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨4345, ⟨4, 7, 3, 4, 2⟩⟩,
  ⟨4345, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨4345, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨4345, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨4345, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨4345, ⟨4, 7, 4, 3, 2⟩⟩,
  ⟨4345, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨4345, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨4345, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨820, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨1770, ⟨4, 7, 5, 2, 2⟩⟩,
  ⟨1770, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨1770, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨1770, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨230, ⟨4, 7, 6, 1, 2⟩⟩,
  ⟨230, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨230, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨230, ⟨5, 7, 6, 0, 2⟩⟩,
  ⟨165, ⟨5, 8, 1, 4, 2⟩⟩,
  ⟨70, ⟨4, 8, 1, 5, 2⟩⟩,
  ⟨70, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨70, ⟨5, 8, 0, 5, 2⟩⟩,
  ⟨70, ⟨5, 8, 1, 4, 2⟩⟩,
  ⟨565, ⟨5, 8, 2, 3, 2⟩⟩,
  ⟨350, ⟨4, 8, 2, 4, 2⟩⟩,
  ⟨350, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨350, ⟨5, 8, 1, 4, 2⟩⟩,
  ⟨350, ⟨5, 8, 2, 3, 2⟩⟩,
  ⟨565, ⟨5, 8, 3, 2, 2⟩⟩,
  ⟨565, ⟨4, 8, 3, 3, 2⟩⟩,
  ⟨565, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨565, ⟨5, 8, 2, 3, 2⟩⟩,
  ⟨565, ⟨5, 8, 3, 2, 2⟩⟩,
  ⟨165, ⟨5, 8, 4, 1, 2⟩⟩,
  ⟨350, ⟨4, 8, 4, 2, 2⟩⟩,
  ⟨350, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨350, ⟨5, 8, 3, 2, 2⟩⟩,
  ⟨350, ⟨5, 8, 4, 1, 2⟩⟩,
  ⟨70, ⟨4, 8, 5, 1, 2⟩⟩,
  ⟨70, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨70, ⟨5, 8, 4, 1, 2⟩⟩,
  ⟨70, ⟨5, 8, 5, 0, 2⟩⟩,
  ⟨81, ⟨6, 1, 3, 8, 2⟩⟩,
  ⟨565, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨50, ⟨5, 1, 4, 8, 2⟩⟩,
  ⟨50, ⟨6, 0, 4, 8, 2⟩⟩,
  ⟨50, ⟨6, 1, 3, 8, 2⟩⟩,
  ⟨50, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨1320, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨230, ⟨5, 1, 5, 7, 2⟩⟩,
  ⟨230, ⟨6, 0, 5, 7, 2⟩⟩,
  ⟨230, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨230, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨1320, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨360, ⟨5, 1, 6, 6, 2⟩⟩,
  ⟨360, ⟨6, 0, 6, 6, 2⟩⟩,
  ⟨360, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨360, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨565, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨230, ⟨5, 1, 7, 5, 2⟩⟩,
  ⟨230, ⟨6, 0, 7, 5, 2⟩⟩,
  ⟨230, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨230, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨81, ⟨6, 1, 8, 3, 2⟩⟩,
  ⟨50, ⟨5, 1, 8, 4, 2⟩⟩,
  ⟨50, ⟨6, 0, 8, 4, 2⟩⟩,
  ⟨50, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨50, ⟨6, 1, 8, 3, 2⟩⟩,
  ⟨172, ⟨6, 2, 2, 8, 2⟩⟩,
  ⟨2007, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨172, ⟨5, 2, 3, 8, 2⟩⟩,
  ⟨172, ⟨6, 1, 3, 8, 2⟩⟩,
  ⟨172, ⟨6, 2, 2, 8, 2⟩⟩,
  ⟨172, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨7140, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨1220, ⟨5, 2, 4, 7, 2⟩⟩,
  ⟨1220, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨1220, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨1220, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨10640, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨2860, ⟨5, 2, 5, 6, 2⟩⟩,
  ⟨2860, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨2860, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨2860, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨7140, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨2860, ⟨5, 2, 6, 5, 2⟩⟩,
  ⟨2860, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨2860, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨2860, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨2007, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨1220, ⟨5, 2, 7, 4, 2⟩⟩,
  ⟨1220, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨1220, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨1220, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨172, ⟨6, 2, 8, 2, 2⟩⟩,
  ⟨172, ⟨5, 2, 8, 3, 2⟩⟩,
  ⟨172, ⟨6, 1, 8, 3, 2⟩⟩,
  ⟨172, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨172, ⟨6, 2, 8, 2, 2⟩⟩,
  ⟨81, ⟨6, 3, 1, 8, 2⟩⟩,
  ⟨2007, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨172, ⟨5, 3, 2, 8, 2⟩⟩,
  ⟨172, ⟨6, 2, 2, 8, 2⟩⟩,
  ⟨172, ⟨6, 3, 1, 8, 2⟩⟩,
  ⟨172, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨11821, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨2007, ⟨5, 3, 3, 7, 2⟩⟩,
  ⟨2007, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨2007, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨2007, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨26715, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨7140, ⟨5, 3, 4, 6, 2⟩⟩,
  ⟨7140, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨7140, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨7140, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨26715, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨10640, ⟨5, 3, 5, 5, 2⟩⟩,
  ⟨10640, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨10640, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨10640, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨11821, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨7140, ⟨5, 3, 6, 4, 2⟩⟩,
  ⟨7140, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨7140, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨7140, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨2007, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨2007, ⟨5, 3, 7, 3, 2⟩⟩,
  ⟨2007, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨2007, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨2007, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨81, ⟨6, 3, 8, 1, 2⟩⟩,
  ⟨172, ⟨5, 3, 8, 2, 2⟩⟩,
  ⟨172, ⟨6, 2, 8, 2, 2⟩⟩,
  ⟨172, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨172, ⟨6, 3, 8, 1, 2⟩⟩,
  ⟨565, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨50, ⟨5, 4, 1, 8, 2⟩⟩,
  ⟨50, ⟨6, 3, 1, 8, 2⟩⟩,
  ⟨50, ⟨6, 4, 0, 8, 2⟩⟩,
  ⟨50, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨7140, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨1220, ⟨5, 4, 2, 7, 2⟩⟩,
  ⟨1220, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨1220, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨1220, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨26715, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨7140, ⟨5, 4, 3, 6, 2⟩⟩,
  ⟨7140, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨7140, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨7140, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨40500, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨16110, ⟨5, 4, 4, 5, 2⟩⟩,
  ⟨16110, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨16110, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨16110, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨26715, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨16110, ⟨5, 4, 5, 4, 2⟩⟩,
  ⟨16110, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨16110, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨16110, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨7140, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨7140, ⟨5, 4, 6, 3, 2⟩⟩,
  ⟨7140, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨7140, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨7140, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨565, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨1220, ⟨5, 4, 7, 2, 2⟩⟩,
  ⟨1220, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨1220, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨1220, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨50, ⟨5, 4, 8, 1, 2⟩⟩,
  ⟨50, ⟨6, 3, 8, 1, 2⟩⟩,
  ⟨50, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨50, ⟨6, 4, 8, 0, 2⟩⟩,
  ⟨1320, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨230, ⟨5, 5, 1, 7, 2⟩⟩,
  ⟨230, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨230, ⟨6, 5, 0, 7, 2⟩⟩,
  ⟨230, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨10640, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨2860, ⟨5, 5, 2, 6, 2⟩⟩,
  ⟨2860, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨2860, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨2860, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨26715, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨10640, ⟨5, 5, 3, 5, 2⟩⟩,
  ⟨10640, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨10640, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨10640, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨26715, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨16110, ⟨5, 5, 4, 4, 2⟩⟩,
  ⟨16110, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨16110, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨16110, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨10640, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨10640, ⟨5, 5, 5, 3, 2⟩⟩,
  ⟨10640, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨10640, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨10640, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨1320, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨2860, ⟨5, 5, 6, 2, 2⟩⟩,
  ⟨2860, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨2860, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨2860, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨230, ⟨5, 5, 7, 1, 2⟩⟩,
  ⟨230, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨230, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨230, ⟨6, 5, 7, 0, 2⟩⟩,
  ⟨1320, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨360, ⟨5, 6, 1, 6, 2⟩⟩,
  ⟨360, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨360, ⟨6, 6, 0, 6, 2⟩⟩,
  ⟨360, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨7140, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨2860, ⟨5, 6, 2, 5, 2⟩⟩,
  ⟨2860, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨2860, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨2860, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨11821, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨7140, ⟨5, 6, 3, 4, 2⟩⟩,
  ⟨7140, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨7140, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨7140, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨7140, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨7140, ⟨5, 6, 4, 3, 2⟩⟩,
  ⟨7140, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨7140, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨7140, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨1320, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨2860, ⟨5, 6, 5, 2, 2⟩⟩,
  ⟨2860, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨2860, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨2860, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨360, ⟨5, 6, 6, 1, 2⟩⟩,
  ⟨360, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨360, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨360, ⟨6, 6, 6, 0, 2⟩⟩,
  ⟨565, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨230, ⟨5, 7, 1, 5, 2⟩⟩,
  ⟨230, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨230, ⟨6, 7, 0, 5, 2⟩⟩,
  ⟨230, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨2007, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨1220, ⟨5, 7, 2, 4, 2⟩⟩,
  ⟨1220, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨1220, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨1220, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨2007, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨2007, ⟨5, 7, 3, 3, 2⟩⟩,
  ⟨2007, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨2007, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨2007, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨565, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨1220, ⟨5, 7, 4, 2, 2⟩⟩,
  ⟨1220, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨1220, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨1220, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨230, ⟨5, 7, 5, 1, 2⟩⟩,
  ⟨230, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨230, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨230, ⟨6, 7, 5, 0, 2⟩⟩,
  ⟨81, ⟨6, 8, 1, 3, 2⟩⟩,
  ⟨50, ⟨5, 8, 1, 4, 2⟩⟩,
  ⟨50, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨50, ⟨6, 8, 0, 4, 2⟩⟩,
  ⟨50, ⟨6, 8, 1, 3, 2⟩⟩,
  ⟨172, ⟨6, 8, 2, 2, 2⟩⟩,
  ⟨172, ⟨5, 8, 2, 3, 2⟩⟩,
  ⟨172, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨172, ⟨6, 8, 1, 3, 2⟩⟩,
  ⟨172, ⟨6, 8, 2, 2, 2⟩⟩,
  ⟨81, ⟨6, 8, 3, 1, 2⟩⟩,
  ⟨172, ⟨5, 8, 3, 2, 2⟩⟩,
  ⟨172, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨172, ⟨6, 8, 2, 2, 2⟩⟩,
  ⟨172, ⟨6, 8, 3, 1, 2⟩⟩,
  ⟨50, ⟨5, 8, 4, 1, 2⟩⟩,
  ⟨50, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨50, ⟨6, 8, 3, 1, 2⟩⟩,
  ⟨50, ⟨6, 8, 4, 0, 2⟩⟩,
  ⟨17, ⟨7, 1, 2, 8, 2⟩⟩,
  ⟨172, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨17, ⟨6, 1, 3, 8, 2⟩⟩,
  ⟨17, ⟨7, 0, 3, 8, 2⟩⟩,
  ⟨17, ⟨7, 1, 2, 8, 2⟩⟩,
  ⟨17, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨565, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨105, ⟨6, 1, 4, 7, 2⟩⟩,
  ⟨105, ⟨7, 0, 4, 7, 2⟩⟩,
  ⟨105, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨105, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨820, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨230, ⟨6, 1, 5, 6, 2⟩⟩,
  ⟨230, ⟨7, 0, 5, 6, 2⟩⟩,
  ⟨230, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨230, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨565, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨230, ⟨6, 1, 6, 5, 2⟩⟩,
  ⟨230, ⟨7, 0, 6, 5, 2⟩⟩,
  ⟨230, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨230, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨172, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨105, ⟨6, 1, 7, 4, 2⟩⟩,
  ⟨105, ⟨7, 0, 7, 4, 2⟩⟩,
  ⟨105, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨105, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨17, ⟨7, 1, 8, 2, 2⟩⟩,
  ⟨17, ⟨6, 1, 8, 3, 2⟩⟩,
  ⟨17, ⟨7, 0, 8, 3, 2⟩⟩,
  ⟨17, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨17, ⟨7, 1, 8, 2, 2⟩⟩,
  ⟨17, ⟨7, 2, 1, 8, 2⟩⟩,
  ⟨371, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨36, ⟨6, 2, 2, 8, 2⟩⟩,
  ⟨36, ⟨7, 1, 2, 8, 2⟩⟩,
  ⟨36, ⟨7, 2, 1, 8, 2⟩⟩,
  ⟨36, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨2007, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨371, ⟨6, 2, 3, 7, 2⟩⟩,
  ⟨371, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨371, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨371, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨4345, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨1220, ⟨6, 2, 4, 6, 2⟩⟩,
  ⟨1220, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨1220, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨1220, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨4345, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨1770, ⟨6, 2, 5, 5, 2⟩⟩,
  ⟨1770, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨1770, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨1770, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨2007, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨1220, ⟨6, 2, 6, 4, 2⟩⟩,
  ⟨1220, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨1220, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨1220, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨371, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨371, ⟨6, 2, 7, 3, 2⟩⟩,
  ⟨371, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨371, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨371, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨17, ⟨7, 2, 8, 1, 2⟩⟩,
  ⟨36, ⟨6, 2, 8, 2, 2⟩⟩,
  ⟨36, ⟨7, 1, 8, 2, 2⟩⟩,
  ⟨36, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨36, ⟨7, 2, 8, 1, 2⟩⟩,
  ⟨172, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨17, ⟨6, 3, 1, 8, 2⟩⟩,
  ⟨17, ⟨7, 2, 1, 8, 2⟩⟩,
  ⟨17, ⟨7, 3, 0, 8, 2⟩⟩,
  ⟨17, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨2007, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨371, ⟨6, 3, 2, 7, 2⟩⟩,
  ⟨371, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨371, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨371, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨7140, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨2007, ⟨6, 3, 3, 6, 2⟩⟩,
  ⟨2007, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨2007, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨2007, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨10640, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨4345, ⟨6, 3, 4, 5, 2⟩⟩,
  ⟨4345, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨4345, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨4345, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨7140, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨4345, ⟨6, 3, 5, 4, 2⟩⟩,
  ⟨4345, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨4345, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨4345, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨2007, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨2007, ⟨6, 3, 6, 3, 2⟩⟩,
  ⟨2007, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨2007, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨2007, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨172, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨371, ⟨6, 3, 7, 2, 2⟩⟩,
  ⟨371, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨371, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨371, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨17, ⟨6, 3, 8, 1, 2⟩⟩,
  ⟨17, ⟨7, 2, 8, 1, 2⟩⟩,
  ⟨17, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨17, ⟨7, 3, 8, 0, 2⟩⟩,
  ⟨565, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨105, ⟨6, 4, 1, 7, 2⟩⟩,
  ⟨105, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨105, ⟨7, 4, 0, 7, 2⟩⟩,
  ⟨105, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨4345, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨1220, ⟨6, 4, 2, 6, 2⟩⟩,
  ⟨1220, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨1220, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨1220, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨10640, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨4345, ⟨6, 4, 3, 5, 2⟩⟩,
  ⟨4345, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨4345, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨4345, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨10640, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨6480, ⟨6, 4, 4, 4, 2⟩⟩,
  ⟨6480, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨6480, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨6480, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨4345, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨4345, ⟨6, 4, 5, 3, 2⟩⟩,
  ⟨4345, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨4345, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨4345, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨565, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨1220, ⟨6, 4, 6, 2, 2⟩⟩,
  ⟨1220, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨1220, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨1220, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨105, ⟨6, 4, 7, 1, 2⟩⟩,
  ⟨105, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨105, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨105, ⟨7, 4, 7, 0, 2⟩⟩,
  ⟨820, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨230, ⟨6, 5, 1, 6, 2⟩⟩,
  ⟨230, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨230, ⟨7, 5, 0, 6, 2⟩⟩,
  ⟨230, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨4345, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨1770, ⟨6, 5, 2, 5, 2⟩⟩,
  ⟨1770, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨1770, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨1770, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨7140, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨4345, ⟨6, 5, 3, 4, 2⟩⟩,
  ⟨4345, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨4345, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨4345, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨4345, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨4345, ⟨6, 5, 4, 3, 2⟩⟩,
  ⟨4345, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨4345, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨4345, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨820, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨1770, ⟨6, 5, 5, 2, 2⟩⟩,
  ⟨1770, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨1770, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨1770, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨230, ⟨6, 5, 6, 1, 2⟩⟩,
  ⟨230, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨230, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨230, ⟨7, 5, 6, 0, 2⟩⟩,
  ⟨565, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨230, ⟨6, 6, 1, 5, 2⟩⟩,
  ⟨230, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨230, ⟨7, 6, 0, 5, 2⟩⟩,
  ⟨230, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨2007, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨1220, ⟨6, 6, 2, 4, 2⟩⟩,
  ⟨1220, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨1220, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨1220, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨2007, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨2007, ⟨6, 6, 3, 3, 2⟩⟩,
  ⟨2007, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨2007, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨2007, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨565, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨1220, ⟨6, 6, 4, 2, 2⟩⟩,
  ⟨1220, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨1220, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨1220, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨230, ⟨6, 6, 5, 1, 2⟩⟩,
  ⟨230, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨230, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨230, ⟨7, 6, 5, 0, 2⟩⟩,
  ⟨172, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨105, ⟨6, 7, 1, 4, 2⟩⟩,
  ⟨105, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨105, ⟨7, 7, 0, 4, 2⟩⟩,
  ⟨105, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨371, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨371, ⟨6, 7, 2, 3, 2⟩⟩,
  ⟨371, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨371, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨371, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨172, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨371, ⟨6, 7, 3, 2, 2⟩⟩,
  ⟨371, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨371, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨371, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨105, ⟨6, 7, 4, 1, 2⟩⟩,
  ⟨105, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨105, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨105, ⟨7, 7, 4, 0, 2⟩⟩,
  ⟨17, ⟨7, 8, 1, 2, 2⟩⟩,
  ⟨17, ⟨6, 8, 1, 3, 2⟩⟩,
  ⟨17, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨17, ⟨7, 8, 0, 3, 2⟩⟩,
  ⟨17, ⟨7, 8, 1, 2, 2⟩⟩,
  ⟨17, ⟨7, 8, 2, 1, 2⟩⟩,
  ⟨36, ⟨6, 8, 2, 2, 2⟩⟩,
  ⟨36, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨36, ⟨7, 8, 1, 2, 2⟩⟩,
  ⟨36, ⟨7, 8, 2, 1, 2⟩⟩,
  ⟨17, ⟨6, 8, 3, 1, 2⟩⟩,
  ⟨17, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨17, ⟨7, 8, 2, 1, 2⟩⟩,
  ⟨17, ⟨7, 8, 3, 0, 2⟩⟩,
  ⟨1, ⟨8, 1, 1, 8, 2⟩⟩,
  ⟨17, ⟨8, 1, 2, 7, 2⟩⟩,
  ⟨2, ⟨7, 1, 2, 8, 2⟩⟩,
  ⟨2, ⟨8, 0, 2, 8, 2⟩⟩,
  ⟨2, ⟨8, 1, 1, 8, 2⟩⟩,
  ⟨2, ⟨8, 1, 2, 7, 2⟩⟩,
  ⟨81, ⟨8, 1, 3, 6, 2⟩⟩,
  ⟨17, ⟨7, 1, 3, 7, 2⟩⟩,
  ⟨17, ⟨8, 0, 3, 7, 2⟩⟩,
  ⟨17, ⟨8, 1, 2, 7, 2⟩⟩,
  ⟨17, ⟨8, 1, 3, 6, 2⟩⟩,
  ⟨165, ⟨8, 1, 4, 5, 2⟩⟩,
  ⟨50, ⟨7, 1, 4, 6, 2⟩⟩,
  ⟨50, ⟨8, 0, 4, 6, 2⟩⟩,
  ⟨50, ⟨8, 1, 3, 6, 2⟩⟩,
  ⟨50, ⟨8, 1, 4, 5, 2⟩⟩,
  ⟨165, ⟨8, 1, 5, 4, 2⟩⟩,
  ⟨70, ⟨7, 1, 5, 5, 2⟩⟩,
  ⟨70, ⟨8, 0, 5, 5, 2⟩⟩,
  ⟨70, ⟨8, 1, 4, 5, 2⟩⟩,
  ⟨70, ⟨8, 1, 5, 4, 2⟩⟩,
  ⟨81, ⟨8, 1, 6, 3, 2⟩⟩,
  ⟨50, ⟨7, 1, 6, 4, 2⟩⟩,
  ⟨50, ⟨8, 0, 6, 4, 2⟩⟩,
  ⟨50, ⟨8, 1, 5, 4, 2⟩⟩,
  ⟨50, ⟨8, 1, 6, 3, 2⟩⟩,
  ⟨17, ⟨8, 1, 7, 2, 2⟩⟩,
  ⟨17, ⟨7, 1, 7, 3, 2⟩⟩,
  ⟨17, ⟨8, 0, 7, 3, 2⟩⟩,
  ⟨17, ⟨8, 1, 6, 3, 2⟩⟩,
  ⟨17, ⟨8, 1, 7, 2, 2⟩⟩,
  ⟨1, ⟨8, 1, 8, 1, 2⟩⟩,
  ⟨2, ⟨7, 1, 8, 2, 2⟩⟩,
  ⟨2, ⟨8, 0, 8, 2, 2⟩⟩,
  ⟨2, ⟨8, 1, 7, 2, 2⟩⟩,
  ⟨2, ⟨8, 1, 8, 1, 2⟩⟩,
  ⟨17, ⟨8, 2, 1, 7, 2⟩⟩,
  ⟨2, ⟨7, 2, 1, 8, 2⟩⟩,
  ⟨2, ⟨8, 1, 1, 8, 2⟩⟩,
  ⟨2, ⟨8, 2, 0, 8, 2⟩⟩,
  ⟨2, ⟨8, 2, 1, 7, 2⟩⟩,
  ⟨172, ⟨8, 2, 2, 6, 2⟩⟩,
  ⟨36, ⟨7, 2, 2, 7, 2⟩⟩,
  ⟨36, ⟨8, 1, 2, 7, 2⟩⟩,
  ⟨36, ⟨8, 2, 1, 7, 2⟩⟩,
  ⟨36, ⟨8, 2, 2, 6, 2⟩⟩,
  ⟨565, ⟨8, 2, 3, 5, 2⟩⟩,
  ⟨172, ⟨7, 2, 3, 6, 2⟩⟩,
  ⟨172, ⟨8, 1, 3, 6, 2⟩⟩,
  ⟨172, ⟨8, 2, 2, 6, 2⟩⟩,
  ⟨172, ⟨8, 2, 3, 5, 2⟩⟩,
  ⟨820, ⟨8, 2, 4, 4, 2⟩⟩,
  ⟨350, ⟨7, 2, 4, 5, 2⟩⟩,
  ⟨350, ⟨8, 1, 4, 5, 2⟩⟩,
  ⟨350, ⟨8, 2, 3, 5, 2⟩⟩,
  ⟨350, ⟨8, 2, 4, 4, 2⟩⟩,
  ⟨565, ⟨8, 2, 5, 3, 2⟩⟩,
  ⟨350, ⟨7, 2, 5, 4, 2⟩⟩,
  ⟨350, ⟨8, 1, 5, 4, 2⟩⟩,
  ⟨350, ⟨8, 2, 4, 4, 2⟩⟩,
  ⟨350, ⟨8, 2, 5, 3, 2⟩⟩,
  ⟨172, ⟨8, 2, 6, 2, 2⟩⟩,
  ⟨172, ⟨7, 2, 6, 3, 2⟩⟩,
  ⟨172, ⟨8, 1, 6, 3, 2⟩⟩,
  ⟨172, ⟨8, 2, 5, 3, 2⟩⟩,
  ⟨172, ⟨8, 2, 6, 2, 2⟩⟩,
  ⟨17, ⟨8, 2, 7, 1, 2⟩⟩,
  ⟨36, ⟨7, 2, 7, 2, 2⟩⟩,
  ⟨36, ⟨8, 1, 7, 2, 2⟩⟩,
  ⟨36, ⟨8, 2, 6, 2, 2⟩⟩,
  ⟨36, ⟨8, 2, 7, 1, 2⟩⟩,
  ⟨2, ⟨7, 2, 8, 1, 2⟩⟩,
  ⟨2, ⟨8, 1, 8, 1, 2⟩⟩,
  ⟨2, ⟨8, 2, 7, 1, 2⟩⟩,
  ⟨2, ⟨8, 2, 8, 0, 2⟩⟩,
  ⟨81, ⟨8, 3, 1, 6, 2⟩⟩,
  ⟨17, ⟨7, 3, 1, 7, 2⟩⟩,
  ⟨17, ⟨8, 2, 1, 7, 2⟩⟩,
  ⟨17, ⟨8, 3, 0, 7, 2⟩⟩,
  ⟨17, ⟨8, 3, 1, 6, 2⟩⟩,
  ⟨565, ⟨8, 3, 2, 5, 2⟩⟩,
  ⟨172, ⟨7, 3, 2, 6, 2⟩⟩,
  ⟨172, ⟨8, 2, 2, 6, 2⟩⟩,
  ⟨172, ⟨8, 3, 1, 6, 2⟩⟩,
  ⟨172, ⟨8, 3, 2, 5, 2⟩⟩,
  ⟨1320, ⟨8, 3, 3, 4, 2⟩⟩,
  ⟨565, ⟨7, 3, 3, 5, 2⟩⟩,
  ⟨565, ⟨8, 2, 3, 5, 2⟩⟩,
  ⟨565, ⟨8, 3, 2, 5, 2⟩⟩,
  ⟨565, ⟨8, 3, 3, 4, 2⟩⟩,
  ⟨1320, ⟨8, 3, 4, 3, 2⟩⟩,
  ⟨820, ⟨7, 3, 4, 4, 2⟩⟩,
  ⟨820, ⟨8, 2, 4, 4, 2⟩⟩,
  ⟨820, ⟨8, 3, 3, 4, 2⟩⟩,
  ⟨820, ⟨8, 3, 4, 3, 2⟩⟩,
  ⟨565, ⟨8, 3, 5, 2, 2⟩⟩,
  ⟨565, ⟨7, 3, 5, 3, 2⟩⟩,
  ⟨565, ⟨8, 2, 5, 3, 2⟩⟩,
  ⟨565, ⟨8, 3, 4, 3, 2⟩⟩,
  ⟨565, ⟨8, 3, 5, 2, 2⟩⟩,
  ⟨81, ⟨8, 3, 6, 1, 2⟩⟩,
  ⟨172, ⟨7, 3, 6, 2, 2⟩⟩,
  ⟨172, ⟨8, 2, 6, 2, 2⟩⟩,
  ⟨172, ⟨8, 3, 5, 2, 2⟩⟩,
  ⟨172, ⟨8, 3, 6, 1, 2⟩⟩,
  ⟨17, ⟨7, 3, 7, 1, 2⟩⟩,
  ⟨17, ⟨8, 2, 7, 1, 2⟩⟩,
  ⟨17, ⟨8, 3, 6, 1, 2⟩⟩,
  ⟨17, ⟨8, 3, 7, 0, 2⟩⟩,
  ⟨165, ⟨8, 4, 1, 5, 2⟩⟩,
  ⟨50, ⟨7, 4, 1, 6, 2⟩⟩,
  ⟨50, ⟨8, 3, 1, 6, 2⟩⟩,
  ⟨50, ⟨8, 4, 0, 6, 2⟩⟩,
  ⟨50, ⟨8, 4, 1, 5, 2⟩⟩,
  ⟨820, ⟨8, 4, 2, 4, 2⟩⟩,
  ⟨350, ⟨7, 4, 2, 5, 2⟩⟩,
  ⟨350, ⟨8, 3, 2, 5, 2⟩⟩,
  ⟨350, ⟨8, 4, 1, 5, 2⟩⟩,
  ⟨350, ⟨8, 4, 2, 4, 2⟩⟩,
  ⟨1320, ⟨8, 4, 3, 3, 2⟩⟩,
  ⟨820, ⟨7, 4, 3, 4, 2⟩⟩,
  ⟨820, ⟨8, 3, 3, 4, 2⟩⟩,
  ⟨820, ⟨8, 4, 2, 4, 2⟩⟩,
  ⟨820, ⟨8, 4, 3, 3, 2⟩⟩,
  ⟨820, ⟨8, 4, 4, 2, 2⟩⟩,
  ⟨820, ⟨7, 4, 4, 3, 2⟩⟩,
  ⟨820, ⟨8, 3, 4, 3, 2⟩⟩,
  ⟨820, ⟨8, 4, 3, 3, 2⟩⟩,
  ⟨820, ⟨8, 4, 4, 2, 2⟩⟩,
  ⟨165, ⟨8, 4, 5, 1, 2⟩⟩,
  ⟨350, ⟨7, 4, 5, 2, 2⟩⟩,
  ⟨350, ⟨8, 3, 5, 2, 2⟩⟩,
  ⟨350, ⟨8, 4, 4, 2, 2⟩⟩,
  ⟨350, ⟨8, 4, 5, 1, 2⟩⟩,
  ⟨50, ⟨7, 4, 6, 1, 2⟩⟩,
  ⟨50, ⟨8, 3, 6, 1, 2⟩⟩,
  ⟨50, ⟨8, 4, 5, 1, 2⟩⟩,
  ⟨50, ⟨8, 4, 6, 0, 2⟩⟩,
  ⟨165, ⟨8, 5, 1, 4, 2⟩⟩,
  ⟨70, ⟨7, 5, 1, 5, 2⟩⟩,
  ⟨70, ⟨8, 4, 1, 5, 2⟩⟩,
  ⟨70, ⟨8, 5, 0, 5, 2⟩⟩,
  ⟨70, ⟨8, 5, 1, 4, 2⟩⟩,
  ⟨565, ⟨8, 5, 2, 3, 2⟩⟩,
  ⟨350, ⟨7, 5, 2, 4, 2⟩⟩,
  ⟨350, ⟨8, 4, 2, 4, 2⟩⟩,
  ⟨350, ⟨8, 5, 1, 4, 2⟩⟩,
  ⟨350, ⟨8, 5, 2, 3, 2⟩⟩,
  ⟨565, ⟨8, 5, 3, 2, 2⟩⟩,
  ⟨565, ⟨7, 5, 3, 3, 2⟩⟩,
  ⟨565, ⟨8, 4, 3, 3, 2⟩⟩,
  ⟨565, ⟨8, 5, 2, 3, 2⟩⟩,
  ⟨565, ⟨8, 5, 3, 2, 2⟩⟩,
  ⟨165, ⟨8, 5, 4, 1, 2⟩⟩,
  ⟨350, ⟨7, 5, 4, 2, 2⟩⟩,
  ⟨350, ⟨8, 4, 4, 2, 2⟩⟩,
  ⟨350, ⟨8, 5, 3, 2, 2⟩⟩,
  ⟨350, ⟨8, 5, 4, 1, 2⟩⟩,
  ⟨70, ⟨7, 5, 5, 1, 2⟩⟩,
  ⟨70, ⟨8, 4, 5, 1, 2⟩⟩,
  ⟨70, ⟨8, 5, 4, 1, 2⟩⟩,
  ⟨70, ⟨8, 5, 5, 0, 2⟩⟩,
  ⟨81, ⟨8, 6, 1, 3, 2⟩⟩,
  ⟨50, ⟨7, 6, 1, 4, 2⟩⟩,
  ⟨50, ⟨8, 5, 1, 4, 2⟩⟩,
  ⟨50, ⟨8, 6, 0, 4, 2⟩⟩,
  ⟨50, ⟨8, 6, 1, 3, 2⟩⟩,
  ⟨172, ⟨8, 6, 2, 2, 2⟩⟩,
  ⟨172, ⟨7, 6, 2, 3, 2⟩⟩,
  ⟨172, ⟨8, 5, 2, 3, 2⟩⟩,
  ⟨172, ⟨8, 6, 1, 3, 2⟩⟩,
  ⟨172, ⟨8, 6, 2, 2, 2⟩⟩,
  ⟨81, ⟨8, 6, 3, 1, 2⟩⟩,
  ⟨172, ⟨7, 6, 3, 2, 2⟩⟩,
  ⟨172, ⟨8, 5, 3, 2, 2⟩⟩,
  ⟨172, ⟨8, 6, 2, 2, 2⟩⟩,
  ⟨172, ⟨8, 6, 3, 1, 2⟩⟩,
  ⟨50, ⟨7, 6, 4, 1, 2⟩⟩,
  ⟨50, ⟨8, 5, 4, 1, 2⟩⟩,
  ⟨50, ⟨8, 6, 3, 1, 2⟩⟩,
  ⟨50, ⟨8, 6, 4, 0, 2⟩⟩,
  ⟨17, ⟨8, 7, 1, 2, 2⟩⟩,
  ⟨17, ⟨7, 7, 1, 3, 2⟩⟩,
  ⟨17, ⟨8, 6, 1, 3, 2⟩⟩,
  ⟨17, ⟨8, 7, 0, 3, 2⟩⟩,
  ⟨17, ⟨8, 7, 1, 2, 2⟩⟩,
  ⟨17, ⟨8, 7, 2, 1, 2⟩⟩,
  ⟨36, ⟨7, 7, 2, 2, 2⟩⟩,
  ⟨36, ⟨8, 6, 2, 2, 2⟩⟩,
  ⟨36, ⟨8, 7, 1, 2, 2⟩⟩,
  ⟨36, ⟨8, 7, 2, 1, 2⟩⟩,
  ⟨17, ⟨7, 7, 3, 1, 2⟩⟩,
  ⟨17, ⟨8, 6, 3, 1, 2⟩⟩,
  ⟨17, ⟨8, 7, 2, 1, 2⟩⟩,
  ⟨17, ⟨8, 7, 3, 0, 2⟩⟩,
  ⟨1, ⟨8, 8, 1, 1, 2⟩⟩,
  ⟨2, ⟨7, 8, 1, 2, 2⟩⟩,
  ⟨2, ⟨8, 7, 1, 2, 2⟩⟩,
  ⟨2, ⟨8, 8, 0, 2, 2⟩⟩,
  ⟨2, ⟨8, 8, 1, 1, 2⟩⟩,
  ⟨2, ⟨7, 8, 2, 1, 2⟩⟩,
  ⟨2, ⟨8, 7, 2, 1, 2⟩⟩,
  ⟨2, ⟨8, 8, 1, 1, 2⟩⟩,
  ⟨2, ⟨8, 8, 2, 0, 2⟩⟩
]

def table0Term111State8NormalizeRadix4FlattenChunk2BucketsNormal : List SparsePolynomial :=
  [table0Term111State8NormalizeRadix4Bucket2Normal]

def table0Term111State8NormalizeRadix4FlattenChunk2Normal : SparsePolynomial :=
  table0Term111State8NormalizeRadix4Bucket2Normal

end LeanProofs.PolynomialFormulas.ComputableDummitCoefficients.KernelCertificate
