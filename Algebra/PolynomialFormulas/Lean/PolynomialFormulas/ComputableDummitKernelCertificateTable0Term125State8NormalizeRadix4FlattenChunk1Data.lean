import PolynomialFormulas.ComputableDummitCoefficientsCore

namespace LeanProofs.PolynomialFormulas.ComputableDummitCoefficients.KernelCertificate

open LeanProofs.PolynomialFormulas.ComputableDummitCoefficients

set_option maxRecDepth 1000000

set_option maxHeartbeats 20000000

def table0Term125State8NormalizeRadix4Bucket1Normal : SparsePolynomial := [
  ⟨-78, ⟨1, 6, 8, 8, 1⟩⟩,
  ⟨-169, ⟨1, 7, 7, 8, 1⟩⟩,
  ⟨-169, ⟨1, 7, 8, 7, 1⟩⟩,
  ⟨-13, ⟨0, 7, 8, 8, 1⟩⟩,
  ⟨-13, ⟨1, 6, 8, 8, 1⟩⟩,
  ⟨-13, ⟨1, 7, 7, 8, 1⟩⟩,
  ⟨-13, ⟨1, 7, 8, 7, 1⟩⟩,
  ⟨-78, ⟨1, 8, 6, 8, 1⟩⟩,
  ⟨-169, ⟨1, 8, 7, 7, 1⟩⟩,
  ⟨-13, ⟨0, 8, 7, 8, 1⟩⟩,
  ⟨-13, ⟨1, 7, 7, 8, 1⟩⟩,
  ⟨-13, ⟨1, 8, 6, 8, 1⟩⟩,
  ⟨-13, ⟨1, 8, 7, 7, 1⟩⟩,
  ⟨-78, ⟨1, 8, 8, 6, 1⟩⟩,
  ⟨-13, ⟨0, 8, 8, 7, 1⟩⟩,
  ⟨-13, ⟨1, 7, 8, 7, 1⟩⟩,
  ⟨-13, ⟨1, 8, 7, 7, 1⟩⟩,
  ⟨-13, ⟨1, 8, 8, 6, 1⟩⟩,
  ⟨-390, ⟨2, 5, 8, 8, 1⟩⟩,
  ⟨-1404, ⟨2, 6, 7, 8, 1⟩⟩,
  ⟨-1404, ⟨2, 6, 8, 7, 1⟩⟩,
  ⟨-78, ⟨1, 6, 8, 8, 1⟩⟩,
  ⟨-78, ⟨2, 5, 8, 8, 1⟩⟩,
  ⟨-78, ⟨2, 6, 7, 8, 1⟩⟩,
  ⟨-78, ⟨2, 6, 8, 7, 1⟩⟩,
  ⟨-1404, ⟨2, 7, 6, 8, 1⟩⟩,
  ⟨-3055, ⟨2, 7, 7, 7, 1⟩⟩,
  ⟨-169, ⟨1, 7, 7, 8, 1⟩⟩,
  ⟨-169, ⟨2, 6, 7, 8, 1⟩⟩,
  ⟨-169, ⟨2, 7, 6, 8, 1⟩⟩,
  ⟨-169, ⟨2, 7, 7, 7, 1⟩⟩,
  ⟨-1404, ⟨2, 7, 8, 6, 1⟩⟩,
  ⟨-169, ⟨1, 7, 8, 7, 1⟩⟩,
  ⟨-169, ⟨2, 6, 8, 7, 1⟩⟩,
  ⟨-169, ⟨2, 7, 7, 7, 1⟩⟩,
  ⟨-169, ⟨2, 7, 8, 6, 1⟩⟩,
  ⟨-390, ⟨2, 8, 5, 8, 1⟩⟩,
  ⟨-1404, ⟨2, 8, 6, 7, 1⟩⟩,
  ⟨-78, ⟨1, 8, 6, 8, 1⟩⟩,
  ⟨-78, ⟨2, 7, 6, 8, 1⟩⟩,
  ⟨-78, ⟨2, 8, 5, 8, 1⟩⟩,
  ⟨-78, ⟨2, 8, 6, 7, 1⟩⟩,
  ⟨-1404, ⟨2, 8, 7, 6, 1⟩⟩,
  ⟨-169, ⟨1, 8, 7, 7, 1⟩⟩,
  ⟨-169, ⟨2, 7, 7, 7, 1⟩⟩,
  ⟨-169, ⟨2, 8, 6, 7, 1⟩⟩,
  ⟨-169, ⟨2, 8, 7, 6, 1⟩⟩,
  ⟨-390, ⟨2, 8, 8, 5, 1⟩⟩,
  ⟨-78, ⟨1, 8, 8, 6, 1⟩⟩,
  ⟨-78, ⟨2, 7, 8, 6, 1⟩⟩,
  ⟨-78, ⟨2, 8, 7, 6, 1⟩⟩,
  ⟨-78, ⟨2, 8, 8, 5, 1⟩⟩,
  ⟨-780, ⟨3, 4, 8, 8, 1⟩⟩,
  ⟨-4095, ⟨3, 5, 7, 8, 1⟩⟩,
  ⟨-4095, ⟨3, 5, 8, 7, 1⟩⟩,
  ⟨-195, ⟨2, 5, 8, 8, 1⟩⟩,
  ⟨-195, ⟨3, 4, 8, 8, 1⟩⟩,
  ⟨-195, ⟨3, 5, 7, 8, 1⟩⟩,
  ⟨-195, ⟨3, 5, 8, 7, 1⟩⟩,
  ⟨-6708, ⟨3, 6, 6, 8, 1⟩⟩,
  ⟨-14469, ⟨3, 6, 7, 7, 1⟩⟩,
  ⟨-663, ⟨2, 6, 7, 8, 1⟩⟩,
  ⟨-663, ⟨3, 5, 7, 8, 1⟩⟩,
  ⟨-663, ⟨3, 6, 6, 8, 1⟩⟩,
  ⟨-663, ⟨3, 6, 7, 7, 1⟩⟩,
  ⟨-6708, ⟨3, 6, 8, 6, 1⟩⟩,
  ⟨-663, ⟨2, 6, 8, 7, 1⟩⟩,
  ⟨-663, ⟨3, 5, 8, 7, 1⟩⟩,
  ⟨-663, ⟨3, 6, 7, 7, 1⟩⟩,
  ⟨-663, ⟨3, 6, 8, 6, 1⟩⟩,
  ⟨-4095, ⟨3, 7, 5, 8, 1⟩⟩,
  ⟨-14469, ⟨3, 7, 6, 7, 1⟩⟩,
  ⟨-663, ⟨2, 7, 6, 8, 1⟩⟩,
  ⟨-663, ⟨3, 6, 6, 8, 1⟩⟩,
  ⟨-663, ⟨3, 7, 5, 8, 1⟩⟩,
  ⟨-663, ⟨3, 7, 6, 7, 1⟩⟩,
  ⟨-14469, ⟨3, 7, 7, 6, 1⟩⟩,
  ⟨-1404, ⟨2, 7, 7, 7, 1⟩⟩,
  ⟨-1404, ⟨3, 6, 7, 7, 1⟩⟩,
  ⟨-1404, ⟨3, 7, 6, 7, 1⟩⟩,
  ⟨-1404, ⟨3, 7, 7, 6, 1⟩⟩,
  ⟨-4095, ⟨3, 7, 8, 5, 1⟩⟩,
  ⟨-663, ⟨2, 7, 8, 6, 1⟩⟩,
  ⟨-663, ⟨3, 6, 8, 6, 1⟩⟩,
  ⟨-663, ⟨3, 7, 7, 6, 1⟩⟩,
  ⟨-663, ⟨3, 7, 8, 5, 1⟩⟩,
  ⟨-780, ⟨3, 8, 4, 8, 1⟩⟩,
  ⟨-4095, ⟨3, 8, 5, 7, 1⟩⟩,
  ⟨-195, ⟨2, 8, 5, 8, 1⟩⟩,
  ⟨-195, ⟨3, 7, 5, 8, 1⟩⟩,
  ⟨-195, ⟨3, 8, 4, 8, 1⟩⟩,
  ⟨-195, ⟨3, 8, 5, 7, 1⟩⟩,
  ⟨-6708, ⟨3, 8, 6, 6, 1⟩⟩,
  ⟨-663, ⟨2, 8, 6, 7, 1⟩⟩,
  ⟨-663, ⟨3, 7, 6, 7, 1⟩⟩,
  ⟨-663, ⟨3, 8, 5, 7, 1⟩⟩,
  ⟨-663, ⟨3, 8, 6, 6, 1⟩⟩,
  ⟨-4095, ⟨3, 8, 7, 5, 1⟩⟩,
  ⟨-663, ⟨2, 8, 7, 6, 1⟩⟩,
  ⟨-663, ⟨3, 7, 7, 6, 1⟩⟩,
  ⟨-663, ⟨3, 8, 6, 6, 1⟩⟩,
  ⟨-663, ⟨3, 8, 7, 5, 1⟩⟩,
  ⟨-780, ⟨3, 8, 8, 4, 1⟩⟩,
  ⟨-195, ⟨2, 8, 8, 5, 1⟩⟩,
  ⟨-195, ⟨3, 7, 8, 5, 1⟩⟩,
  ⟨-195, ⟨3, 8, 7, 5, 1⟩⟩,
  ⟨-195, ⟨3, 8, 8, 4, 1⟩⟩,
  ⟨-780, ⟨4, 3, 8, 8, 1⟩⟩,
  ⟨-5720, ⟨4, 4, 7, 8, 1⟩⟩,
  ⟨-5720, ⟨4, 4, 8, 7, 1⟩⟩,
  ⟨-260, ⟨3, 4, 8, 8, 1⟩⟩,
  ⟨-260, ⟨4, 3, 8, 8, 1⟩⟩,
  ⟨-260, ⟨4, 4, 7, 8, 1⟩⟩,
  ⟨-260, ⟨4, 4, 8, 7, 1⟩⟩,
  ⟨-13650, ⟨4, 5, 6, 8, 1⟩⟩,
  ⟨-29315, ⟨4, 5, 7, 7, 1⟩⟩,
  ⟨-1235, ⟨3, 5, 7, 8, 1⟩⟩,
  ⟨-1235, ⟨4, 4, 7, 8, 1⟩⟩,
  ⟨-1235, ⟨4, 5, 6, 8, 1⟩⟩,
  ⟨-1235, ⟨4, 5, 7, 7, 1⟩⟩,
  ⟨-13650, ⟨4, 5, 8, 6, 1⟩⟩,
  ⟨-1235, ⟨3, 5, 8, 7, 1⟩⟩,
  ⟨-1235, ⟨4, 4, 8, 7, 1⟩⟩,
  ⟨-1235, ⟨4, 5, 7, 7, 1⟩⟩,
  ⟨-1235, ⟨4, 5, 8, 6, 1⟩⟩,
  ⟨-13650, ⟨4, 6, 5, 8, 1⟩⟩,
  ⟨-47580, ⟨4, 6, 6, 7, 1⟩⟩,
  ⟨-1950, ⟨3, 6, 6, 8, 1⟩⟩,
  ⟨-1950, ⟨4, 5, 6, 8, 1⟩⟩,
  ⟨-1950, ⟨4, 6, 5, 8, 1⟩⟩,
  ⟨-1950, ⟨4, 6, 6, 7, 1⟩⟩,
  ⟨-47580, ⟨4, 6, 7, 6, 1⟩⟩,
  ⟨-4095, ⟨3, 6, 7, 7, 1⟩⟩,
  ⟨-4095, ⟨4, 5, 7, 7, 1⟩⟩,
  ⟨-4095, ⟨4, 6, 6, 7, 1⟩⟩,
  ⟨-4095, ⟨4, 6, 7, 6, 1⟩⟩,
  ⟨-13650, ⟨4, 6, 8, 5, 1⟩⟩,
  ⟨-1950, ⟨3, 6, 8, 6, 1⟩⟩,
  ⟨-1950, ⟨4, 5, 8, 6, 1⟩⟩,
  ⟨-1950, ⟨4, 6, 7, 6, 1⟩⟩,
  ⟨-1950, ⟨4, 6, 8, 5, 1⟩⟩,
  ⟨-5720, ⟨4, 7, 4, 8, 1⟩⟩,
  ⟨-29315, ⟨4, 7, 5, 7, 1⟩⟩,
  ⟨-1235, ⟨3, 7, 5, 8, 1⟩⟩,
  ⟨-1235, ⟨4, 6, 5, 8, 1⟩⟩,
  ⟨-1235, ⟨4, 7, 4, 8, 1⟩⟩,
  ⟨-1235, ⟨4, 7, 5, 7, 1⟩⟩,
  ⟨-47580, ⟨4, 7, 6, 6, 1⟩⟩,
  ⟨-4095, ⟨3, 7, 6, 7, 1⟩⟩,
  ⟨-4095, ⟨4, 6, 6, 7, 1⟩⟩,
  ⟨-4095, ⟨4, 7, 5, 7, 1⟩⟩,
  ⟨-4095, ⟨4, 7, 6, 6, 1⟩⟩,
  ⟨-29315, ⟨4, 7, 7, 5, 1⟩⟩,
  ⟨-4095, ⟨3, 7, 7, 6, 1⟩⟩,
  ⟨-4095, ⟨4, 6, 7, 6, 1⟩⟩,
  ⟨-4095, ⟨4, 7, 6, 6, 1⟩⟩,
  ⟨-4095, ⟨4, 7, 7, 5, 1⟩⟩,
  ⟨-5720, ⟨4, 7, 8, 4, 1⟩⟩,
  ⟨-1235, ⟨3, 7, 8, 5, 1⟩⟩,
  ⟨-1235, ⟨4, 6, 8, 5, 1⟩⟩,
  ⟨-1235, ⟨4, 7, 7, 5, 1⟩⟩,
  ⟨-1235, ⟨4, 7, 8, 4, 1⟩⟩,
  ⟨-780, ⟨4, 8, 3, 8, 1⟩⟩,
  ⟨-5720, ⟨4, 8, 4, 7, 1⟩⟩,
  ⟨-260, ⟨3, 8, 4, 8, 1⟩⟩,
  ⟨-260, ⟨4, 7, 4, 8, 1⟩⟩,
  ⟨-260, ⟨4, 8, 3, 8, 1⟩⟩,
  ⟨-260, ⟨4, 8, 4, 7, 1⟩⟩,
  ⟨-13650, ⟨4, 8, 5, 6, 1⟩⟩,
  ⟨-1235, ⟨3, 8, 5, 7, 1⟩⟩,
  ⟨-1235, ⟨4, 7, 5, 7, 1⟩⟩,
  ⟨-1235, ⟨4, 8, 4, 7, 1⟩⟩,
  ⟨-1235, ⟨4, 8, 5, 6, 1⟩⟩,
  ⟨-13650, ⟨4, 8, 6, 5, 1⟩⟩,
  ⟨-1950, ⟨3, 8, 6, 6, 1⟩⟩,
  ⟨-1950, ⟨4, 7, 6, 6, 1⟩⟩,
  ⟨-1950, ⟨4, 8, 5, 6, 1⟩⟩,
  ⟨-1950, ⟨4, 8, 6, 5, 1⟩⟩,
  ⟨-5720, ⟨4, 8, 7, 4, 1⟩⟩,
  ⟨-1235, ⟨3, 8, 7, 5, 1⟩⟩,
  ⟨-1235, ⟨4, 7, 7, 5, 1⟩⟩,
  ⟨-1235, ⟨4, 8, 6, 5, 1⟩⟩,
  ⟨-1235, ⟨4, 8, 7, 4, 1⟩⟩,
  ⟨-780, ⟨4, 8, 8, 3, 1⟩⟩,
  ⟨-260, ⟨3, 8, 8, 4, 1⟩⟩,
  ⟨-260, ⟨4, 7, 8, 4, 1⟩⟩,
  ⟨-260, ⟨4, 8, 7, 4, 1⟩⟩,
  ⟨-260, ⟨4, 8, 8, 3, 1⟩⟩,
  ⟨-390, ⟨5, 2, 8, 8, 1⟩⟩,
  ⟨-4095, ⟨5, 3, 7, 8, 1⟩⟩,
  ⟨-4095, ⟨5, 3, 8, 7, 1⟩⟩,
  ⟨-195, ⟨4, 3, 8, 8, 1⟩⟩,
  ⟨-195, ⟨5, 2, 8, 8, 1⟩⟩,
  ⟨-195, ⟨5, 3, 7, 8, 1⟩⟩,
  ⟨-195, ⟨5, 3, 8, 7, 1⟩⟩,
  ⟨-13650, ⟨5, 4, 6, 8, 1⟩⟩,
  ⟨-29315, ⟨5, 4, 7, 7, 1⟩⟩,
  ⟨-1235, ⟨4, 4, 7, 8, 1⟩⟩,
  ⟨-1235, ⟨5, 3, 7, 8, 1⟩⟩,
  ⟨-1235, ⟨5, 4, 6, 8, 1⟩⟩,
  ⟨-1235, ⟨5, 4, 7, 7, 1⟩⟩,
  ⟨-13650, ⟨5, 4, 8, 6, 1⟩⟩,
  ⟨-1235, ⟨4, 4, 8, 7, 1⟩⟩,
  ⟨-1235, ⟨5, 3, 8, 7, 1⟩⟩,
  ⟨-1235, ⟨5, 4, 7, 7, 1⟩⟩,
  ⟨-1235, ⟨5, 4, 8, 6, 1⟩⟩,
  ⟨-19890, ⟨5, 5, 5, 8, 1⟩⟩,
  ⟨-69030, ⟨5, 5, 6, 7, 1⟩⟩,
  ⟨-2730, ⟨4, 5, 6, 8, 1⟩⟩,
  ⟨-2730, ⟨5, 4, 6, 8, 1⟩⟩,
  ⟨-2730, ⟨5, 5, 5, 8, 1⟩⟩,
  ⟨-2730, ⟨5, 5, 6, 7, 1⟩⟩,
  ⟨-69030, ⟨5, 5, 7, 6, 1⟩⟩,
  ⟨-5720, ⟨4, 5, 7, 7, 1⟩⟩,
  ⟨-5720, ⟨5, 4, 7, 7, 1⟩⟩,
  ⟨-5720, ⟨5, 5, 6, 7, 1⟩⟩,
  ⟨-5720, ⟨5, 5, 7, 6, 1⟩⟩,
  ⟨-19890, ⟨5, 5, 8, 5, 1⟩⟩,
  ⟨-2730, ⟨4, 5, 8, 6, 1⟩⟩,
  ⟨-2730, ⟨5, 4, 8, 6, 1⟩⟩,
  ⟨-2730, ⟨5, 5, 7, 6, 1⟩⟩,
  ⟨-2730, ⟨5, 5, 8, 5, 1⟩⟩,
  ⟨-13650, ⟨5, 6, 4, 8, 1⟩⟩,
  ⟨-69030, ⟨5, 6, 5, 7, 1⟩⟩,
  ⟨-2730, ⟨4, 6, 5, 8, 1⟩⟩,
  ⟨-2730, ⟨5, 5, 5, 8, 1⟩⟩,
  ⟨-2730, ⟨5, 6, 4, 8, 1⟩⟩,
  ⟨-2730, ⟨5, 6, 5, 7, 1⟩⟩,
  ⟨-111540, ⟨5, 6, 6, 6, 1⟩⟩,
  ⟨-8970, ⟨4, 6, 6, 7, 1⟩⟩,
  ⟨-8970, ⟨5, 5, 6, 7, 1⟩⟩,
  ⟨-8970, ⟨5, 6, 5, 7, 1⟩⟩,
  ⟨-8970, ⟨5, 6, 6, 6, 1⟩⟩,
  ⟨-69030, ⟨5, 6, 7, 5, 1⟩⟩,
  ⟨-8970, ⟨4, 6, 7, 6, 1⟩⟩,
  ⟨-8970, ⟨5, 5, 7, 6, 1⟩⟩,
  ⟨-8970, ⟨5, 6, 6, 6, 1⟩⟩,
  ⟨-8970, ⟨5, 6, 7, 5, 1⟩⟩,
  ⟨-13650, ⟨5, 6, 8, 4, 1⟩⟩,
  ⟨-2730, ⟨4, 6, 8, 5, 1⟩⟩,
  ⟨-2730, ⟨5, 5, 8, 5, 1⟩⟩,
  ⟨-2730, ⟨5, 6, 7, 5, 1⟩⟩,
  ⟨-2730, ⟨5, 6, 8, 4, 1⟩⟩,
  ⟨-4095, ⟨5, 7, 3, 8, 1⟩⟩,
  ⟨-29315, ⟨5, 7, 4, 7, 1⟩⟩,
  ⟨-1235, ⟨4, 7, 4, 8, 1⟩⟩,
  ⟨-1235, ⟨5, 6, 4, 8, 1⟩⟩,
  ⟨-1235, ⟨5, 7, 3, 8, 1⟩⟩,
  ⟨-1235, ⟨5, 7, 4, 7, 1⟩⟩,
  ⟨-69030, ⟨5, 7, 5, 6, 1⟩⟩,
  ⟨-5720, ⟨4, 7, 5, 7, 1⟩⟩,
  ⟨-5720, ⟨5, 6, 5, 7, 1⟩⟩,
  ⟨-5720, ⟨5, 7, 4, 7, 1⟩⟩,
  ⟨-5720, ⟨5, 7, 5, 6, 1⟩⟩,
  ⟨-69030, ⟨5, 7, 6, 5, 1⟩⟩,
  ⟨-8970, ⟨4, 7, 6, 6, 1⟩⟩,
  ⟨-8970, ⟨5, 6, 6, 6, 1⟩⟩,
  ⟨-8970, ⟨5, 7, 5, 6, 1⟩⟩,
  ⟨-8970, ⟨5, 7, 6, 5, 1⟩⟩,
  ⟨-29315, ⟨5, 7, 7, 4, 1⟩⟩,
  ⟨-5720, ⟨4, 7, 7, 5, 1⟩⟩,
  ⟨-5720, ⟨5, 6, 7, 5, 1⟩⟩,
  ⟨-5720, ⟨5, 7, 6, 5, 1⟩⟩,
  ⟨-5720, ⟨5, 7, 7, 4, 1⟩⟩,
  ⟨-4095, ⟨5, 7, 8, 3, 1⟩⟩,
  ⟨-1235, ⟨4, 7, 8, 4, 1⟩⟩,
  ⟨-1235, ⟨5, 6, 8, 4, 1⟩⟩,
  ⟨-1235, ⟨5, 7, 7, 4, 1⟩⟩,
  ⟨-1235, ⟨5, 7, 8, 3, 1⟩⟩,
  ⟨-390, ⟨5, 8, 2, 8, 1⟩⟩,
  ⟨-4095, ⟨5, 8, 3, 7, 1⟩⟩,
  ⟨-195, ⟨4, 8, 3, 8, 1⟩⟩,
  ⟨-195, ⟨5, 7, 3, 8, 1⟩⟩,
  ⟨-195, ⟨5, 8, 2, 8, 1⟩⟩,
  ⟨-195, ⟨5, 8, 3, 7, 1⟩⟩,
  ⟨-13650, ⟨5, 8, 4, 6, 1⟩⟩,
  ⟨-1235, ⟨4, 8, 4, 7, 1⟩⟩,
  ⟨-1235, ⟨5, 7, 4, 7, 1⟩⟩,
  ⟨-1235, ⟨5, 8, 3, 7, 1⟩⟩,
  ⟨-1235, ⟨5, 8, 4, 6, 1⟩⟩,
  ⟨-19890, ⟨5, 8, 5, 5, 1⟩⟩,
  ⟨-2730, ⟨4, 8, 5, 6, 1⟩⟩,
  ⟨-2730, ⟨5, 7, 5, 6, 1⟩⟩,
  ⟨-2730, ⟨5, 8, 4, 6, 1⟩⟩,
  ⟨-2730, ⟨5, 8, 5, 5, 1⟩⟩,
  ⟨-13650, ⟨5, 8, 6, 4, 1⟩⟩,
  ⟨-2730, ⟨4, 8, 6, 5, 1⟩⟩,
  ⟨-2730, ⟨5, 7, 6, 5, 1⟩⟩,
  ⟨-2730, ⟨5, 8, 5, 5, 1⟩⟩,
  ⟨-2730, ⟨5, 8, 6, 4, 1⟩⟩,
  ⟨-4095, ⟨5, 8, 7, 3, 1⟩⟩,
  ⟨-1235, ⟨4, 8, 7, 4, 1⟩⟩,
  ⟨-1235, ⟨5, 7, 7, 4, 1⟩⟩,
  ⟨-1235, ⟨5, 8, 6, 4, 1⟩⟩,
  ⟨-1235, ⟨5, 8, 7, 3, 1⟩⟩,
  ⟨-390, ⟨5, 8, 8, 2, 1⟩⟩,
  ⟨-195, ⟨4, 8, 8, 3, 1⟩⟩,
  ⟨-195, ⟨5, 7, 8, 3, 1⟩⟩,
  ⟨-195, ⟨5, 8, 7, 3, 1⟩⟩,
  ⟨-195, ⟨5, 8, 8, 2, 1⟩⟩,
  ⟨-78, ⟨6, 1, 8, 8, 1⟩⟩,
  ⟨-1404, ⟨6, 2, 7, 8, 1⟩⟩,
  ⟨-1404, ⟨6, 2, 8, 7, 1⟩⟩,
  ⟨-78, ⟨5, 2, 8, 8, 1⟩⟩,
  ⟨-78, ⟨6, 1, 8, 8, 1⟩⟩,
  ⟨-78, ⟨6, 2, 7, 8, 1⟩⟩,
  ⟨-78, ⟨6, 2, 8, 7, 1⟩⟩,
  ⟨-6708, ⟨6, 3, 6, 8, 1⟩⟩,
  ⟨-14469, ⟨6, 3, 7, 7, 1⟩⟩,
  ⟨-663, ⟨5, 3, 7, 8, 1⟩⟩,
  ⟨-663, ⟨6, 2, 7, 8, 1⟩⟩,
  ⟨-663, ⟨6, 3, 6, 8, 1⟩⟩,
  ⟨-663, ⟨6, 3, 7, 7, 1⟩⟩,
  ⟨-6708, ⟨6, 3, 8, 6, 1⟩⟩,
  ⟨-663, ⟨5, 3, 8, 7, 1⟩⟩,
  ⟨-663, ⟨6, 2, 8, 7, 1⟩⟩,
  ⟨-663, ⟨6, 3, 7, 7, 1⟩⟩,
  ⟨-663, ⟨6, 3, 8, 6, 1⟩⟩,
  ⟨-13650, ⟨6, 4, 5, 8, 1⟩⟩,
  ⟨-47580, ⟨6, 4, 6, 7, 1⟩⟩,
  ⟨-1950, ⟨5, 4, 6, 8, 1⟩⟩,
  ⟨-1950, ⟨6, 3, 6, 8, 1⟩⟩,
  ⟨-1950, ⟨6, 4, 5, 8, 1⟩⟩,
  ⟨-1950, ⟨6, 4, 6, 7, 1⟩⟩,
  ⟨-47580, ⟨6, 4, 7, 6, 1⟩⟩,
  ⟨-4095, ⟨5, 4, 7, 7, 1⟩⟩,
  ⟨-4095, ⟨6, 3, 7, 7, 1⟩⟩,
  ⟨-4095, ⟨6, 4, 6, 7, 1⟩⟩,
  ⟨-4095, ⟨6, 4, 7, 6, 1⟩⟩,
  ⟨-13650, ⟨6, 4, 8, 5, 1⟩⟩,
  ⟨-1950, ⟨5, 4, 8, 6, 1⟩⟩,
  ⟨-1950, ⟨6, 3, 8, 6, 1⟩⟩,
  ⟨-1950, ⟨6, 4, 7, 6, 1⟩⟩,
  ⟨-1950, ⟨6, 4, 8, 5, 1⟩⟩,
  ⟨-13650, ⟨6, 5, 4, 8, 1⟩⟩,
  ⟨-69030, ⟨6, 5, 5, 7, 1⟩⟩,
  ⟨-2730, ⟨5, 5, 5, 8, 1⟩⟩,
  ⟨-2730, ⟨6, 4, 5, 8, 1⟩⟩,
  ⟨-2730, ⟨6, 5, 4, 8, 1⟩⟩,
  ⟨-2730, ⟨6, 5, 5, 7, 1⟩⟩,
  ⟨-111540, ⟨6, 5, 6, 6, 1⟩⟩,
  ⟨-8970, ⟨5, 5, 6, 7, 1⟩⟩,
  ⟨-8970, ⟨6, 4, 6, 7, 1⟩⟩,
  ⟨-8970, ⟨6, 5, 5, 7, 1⟩⟩,
  ⟨-8970, ⟨6, 5, 6, 6, 1⟩⟩,
  ⟨-69030, ⟨6, 5, 7, 5, 1⟩⟩,
  ⟨-8970, ⟨5, 5, 7, 6, 1⟩⟩,
  ⟨-8970, ⟨6, 4, 7, 6, 1⟩⟩,
  ⟨-8970, ⟨6, 5, 6, 6, 1⟩⟩,
  ⟨-8970, ⟨6, 5, 7, 5, 1⟩⟩,
  ⟨-13650, ⟨6, 5, 8, 4, 1⟩⟩,
  ⟨-2730, ⟨5, 5, 8, 5, 1⟩⟩,
  ⟨-2730, ⟨6, 4, 8, 5, 1⟩⟩,
  ⟨-2730, ⟨6, 5, 7, 5, 1⟩⟩,
  ⟨-2730, ⟨6, 5, 8, 4, 1⟩⟩,
  ⟨-6708, ⟨6, 6, 3, 8, 1⟩⟩,
  ⟨-47580, ⟨6, 6, 4, 7, 1⟩⟩,
  ⟨-1950, ⟨5, 6, 4, 8, 1⟩⟩,
  ⟨-1950, ⟨6, 5, 4, 8, 1⟩⟩,
  ⟨-1950, ⟨6, 6, 3, 8, 1⟩⟩,
  ⟨-1950, ⟨6, 6, 4, 7, 1⟩⟩,
  ⟨-111540, ⟨6, 6, 5, 6, 1⟩⟩,
  ⟨-8970, ⟨5, 6, 5, 7, 1⟩⟩,
  ⟨-8970, ⟨6, 5, 5, 7, 1⟩⟩,
  ⟨-8970, ⟨6, 6, 4, 7, 1⟩⟩,
  ⟨-8970, ⟨6, 6, 5, 6, 1⟩⟩,
  ⟨-111540, ⟨6, 6, 6, 5, 1⟩⟩,
  ⟨-14040, ⟨5, 6, 6, 6, 1⟩⟩,
  ⟨-14040, ⟨6, 5, 6, 6, 1⟩⟩,
  ⟨-14040, ⟨6, 6, 5, 6, 1⟩⟩,
  ⟨-14040, ⟨6, 6, 6, 5, 1⟩⟩,
  ⟨-47580, ⟨6, 6, 7, 4, 1⟩⟩,
  ⟨-8970, ⟨5, 6, 7, 5, 1⟩⟩,
  ⟨-8970, ⟨6, 5, 7, 5, 1⟩⟩,
  ⟨-8970, ⟨6, 6, 6, 5, 1⟩⟩,
  ⟨-8970, ⟨6, 6, 7, 4, 1⟩⟩,
  ⟨-6708, ⟨6, 6, 8, 3, 1⟩⟩,
  ⟨-1950, ⟨5, 6, 8, 4, 1⟩⟩,
  ⟨-1950, ⟨6, 5, 8, 4, 1⟩⟩,
  ⟨-1950, ⟨6, 6, 7, 4, 1⟩⟩,
  ⟨-1950, ⟨6, 6, 8, 3, 1⟩⟩,
  ⟨-1404, ⟨6, 7, 2, 8, 1⟩⟩,
  ⟨-14469, ⟨6, 7, 3, 7, 1⟩⟩,
  ⟨-663, ⟨5, 7, 3, 8, 1⟩⟩,
  ⟨-663, ⟨6, 6, 3, 8, 1⟩⟩,
  ⟨-663, ⟨6, 7, 2, 8, 1⟩⟩,
  ⟨-663, ⟨6, 7, 3, 7, 1⟩⟩,
  ⟨-47580, ⟨6, 7, 4, 6, 1⟩⟩,
  ⟨-4095, ⟨5, 7, 4, 7, 1⟩⟩,
  ⟨-4095, ⟨6, 6, 4, 7, 1⟩⟩,
  ⟨-4095, ⟨6, 7, 3, 7, 1⟩⟩,
  ⟨-4095, ⟨6, 7, 4, 6, 1⟩⟩,
  ⟨-69030, ⟨6, 7, 5, 5, 1⟩⟩,
  ⟨-8970, ⟨5, 7, 5, 6, 1⟩⟩,
  ⟨-8970, ⟨6, 6, 5, 6, 1⟩⟩,
  ⟨-8970, ⟨6, 7, 4, 6, 1⟩⟩,
  ⟨-8970, ⟨6, 7, 5, 5, 1⟩⟩,
  ⟨-47580, ⟨6, 7, 6, 4, 1⟩⟩,
  ⟨-8970, ⟨5, 7, 6, 5, 1⟩⟩,
  ⟨-8970, ⟨6, 6, 6, 5, 1⟩⟩,
  ⟨-8970, ⟨6, 7, 5, 5, 1⟩⟩,
  ⟨-8970, ⟨6, 7, 6, 4, 1⟩⟩,
  ⟨-14469, ⟨6, 7, 7, 3, 1⟩⟩,
  ⟨-4095, ⟨5, 7, 7, 4, 1⟩⟩,
  ⟨-4095, ⟨6, 6, 7, 4, 1⟩⟩,
  ⟨-4095, ⟨6, 7, 6, 4, 1⟩⟩,
  ⟨-4095, ⟨6, 7, 7, 3, 1⟩⟩,
  ⟨-1404, ⟨6, 7, 8, 2, 1⟩⟩,
  ⟨-663, ⟨5, 7, 8, 3, 1⟩⟩,
  ⟨-663, ⟨6, 6, 8, 3, 1⟩⟩,
  ⟨-663, ⟨6, 7, 7, 3, 1⟩⟩,
  ⟨-663, ⟨6, 7, 8, 2, 1⟩⟩,
  ⟨-78, ⟨6, 8, 1, 8, 1⟩⟩,
  ⟨-1404, ⟨6, 8, 2, 7, 1⟩⟩,
  ⟨-78, ⟨5, 8, 2, 8, 1⟩⟩,
  ⟨-78, ⟨6, 7, 2, 8, 1⟩⟩,
  ⟨-78, ⟨6, 8, 1, 8, 1⟩⟩,
  ⟨-78, ⟨6, 8, 2, 7, 1⟩⟩,
  ⟨-6708, ⟨6, 8, 3, 6, 1⟩⟩,
  ⟨-663, ⟨5, 8, 3, 7, 1⟩⟩,
  ⟨-663, ⟨6, 7, 3, 7, 1⟩⟩,
  ⟨-663, ⟨6, 8, 2, 7, 1⟩⟩,
  ⟨-663, ⟨6, 8, 3, 6, 1⟩⟩,
  ⟨-13650, ⟨6, 8, 4, 5, 1⟩⟩,
  ⟨-1950, ⟨5, 8, 4, 6, 1⟩⟩,
  ⟨-1950, ⟨6, 7, 4, 6, 1⟩⟩,
  ⟨-1950, ⟨6, 8, 3, 6, 1⟩⟩,
  ⟨-1950, ⟨6, 8, 4, 5, 1⟩⟩,
  ⟨-13650, ⟨6, 8, 5, 4, 1⟩⟩,
  ⟨-2730, ⟨5, 8, 5, 5, 1⟩⟩,
  ⟨-2730, ⟨6, 7, 5, 5, 1⟩⟩,
  ⟨-2730, ⟨6, 8, 4, 5, 1⟩⟩,
  ⟨-2730, ⟨6, 8, 5, 4, 1⟩⟩,
  ⟨-6708, ⟨6, 8, 6, 3, 1⟩⟩,
  ⟨-1950, ⟨5, 8, 6, 4, 1⟩⟩,
  ⟨-1950, ⟨6, 7, 6, 4, 1⟩⟩,
  ⟨-1950, ⟨6, 8, 5, 4, 1⟩⟩,
  ⟨-1950, ⟨6, 8, 6, 3, 1⟩⟩,
  ⟨-1404, ⟨6, 8, 7, 2, 1⟩⟩,
  ⟨-663, ⟨5, 8, 7, 3, 1⟩⟩,
  ⟨-663, ⟨6, 7, 7, 3, 1⟩⟩,
  ⟨-663, ⟨6, 8, 6, 3, 1⟩⟩,
  ⟨-663, ⟨6, 8, 7, 2, 1⟩⟩,
  ⟨-78, ⟨6, 8, 8, 1, 1⟩⟩,
  ⟨-78, ⟨5, 8, 8, 2, 1⟩⟩,
  ⟨-78, ⟨6, 7, 8, 2, 1⟩⟩,
  ⟨-78, ⟨6, 8, 7, 2, 1⟩⟩,
  ⟨-78, ⟨6, 8, 8, 1, 1⟩⟩,
  ⟨-169, ⟨7, 1, 7, 8, 1⟩⟩,
  ⟨-169, ⟨7, 1, 8, 7, 1⟩⟩,
  ⟨-13, ⟨6, 1, 8, 8, 1⟩⟩,
  ⟨-13, ⟨7, 0, 8, 8, 1⟩⟩,
  ⟨-13, ⟨7, 1, 7, 8, 1⟩⟩,
  ⟨-13, ⟨7, 1, 8, 7, 1⟩⟩,
  ⟨-1404, ⟨7, 2, 6, 8, 1⟩⟩,
  ⟨-3055, ⟨7, 2, 7, 7, 1⟩⟩,
  ⟨-169, ⟨6, 2, 7, 8, 1⟩⟩,
  ⟨-169, ⟨7, 1, 7, 8, 1⟩⟩,
  ⟨-169, ⟨7, 2, 6, 8, 1⟩⟩,
  ⟨-169, ⟨7, 2, 7, 7, 1⟩⟩,
  ⟨-1404, ⟨7, 2, 8, 6, 1⟩⟩,
  ⟨-169, ⟨6, 2, 8, 7, 1⟩⟩,
  ⟨-169, ⟨7, 1, 8, 7, 1⟩⟩,
  ⟨-169, ⟨7, 2, 7, 7, 1⟩⟩,
  ⟨-169, ⟨7, 2, 8, 6, 1⟩⟩,
  ⟨-4095, ⟨7, 3, 5, 8, 1⟩⟩,
  ⟨-14469, ⟨7, 3, 6, 7, 1⟩⟩,
  ⟨-663, ⟨6, 3, 6, 8, 1⟩⟩,
  ⟨-663, ⟨7, 2, 6, 8, 1⟩⟩,
  ⟨-663, ⟨7, 3, 5, 8, 1⟩⟩,
  ⟨-663, ⟨7, 3, 6, 7, 1⟩⟩,
  ⟨-14469, ⟨7, 3, 7, 6, 1⟩⟩,
  ⟨-1404, ⟨6, 3, 7, 7, 1⟩⟩,
  ⟨-1404, ⟨7, 2, 7, 7, 1⟩⟩,
  ⟨-1404, ⟨7, 3, 6, 7, 1⟩⟩,
  ⟨-1404, ⟨7, 3, 7, 6, 1⟩⟩,
  ⟨-4095, ⟨7, 3, 8, 5, 1⟩⟩,
  ⟨-663, ⟨6, 3, 8, 6, 1⟩⟩,
  ⟨-663, ⟨7, 2, 8, 6, 1⟩⟩,
  ⟨-663, ⟨7, 3, 7, 6, 1⟩⟩,
  ⟨-663, ⟨7, 3, 8, 5, 1⟩⟩,
  ⟨-5720, ⟨7, 4, 4, 8, 1⟩⟩,
  ⟨-29315, ⟨7, 4, 5, 7, 1⟩⟩,
  ⟨-1235, ⟨6, 4, 5, 8, 1⟩⟩,
  ⟨-1235, ⟨7, 3, 5, 8, 1⟩⟩,
  ⟨-1235, ⟨7, 4, 4, 8, 1⟩⟩,
  ⟨-1235, ⟨7, 4, 5, 7, 1⟩⟩,
  ⟨-47580, ⟨7, 4, 6, 6, 1⟩⟩,
  ⟨-4095, ⟨6, 4, 6, 7, 1⟩⟩,
  ⟨-4095, ⟨7, 3, 6, 7, 1⟩⟩,
  ⟨-4095, ⟨7, 4, 5, 7, 1⟩⟩,
  ⟨-4095, ⟨7, 4, 6, 6, 1⟩⟩,
  ⟨-29315, ⟨7, 4, 7, 5, 1⟩⟩,
  ⟨-4095, ⟨6, 4, 7, 6, 1⟩⟩,
  ⟨-4095, ⟨7, 3, 7, 6, 1⟩⟩,
  ⟨-4095, ⟨7, 4, 6, 6, 1⟩⟩,
  ⟨-4095, ⟨7, 4, 7, 5, 1⟩⟩,
  ⟨-5720, ⟨7, 4, 8, 4, 1⟩⟩,
  ⟨-1235, ⟨6, 4, 8, 5, 1⟩⟩,
  ⟨-1235, ⟨7, 3, 8, 5, 1⟩⟩,
  ⟨-1235, ⟨7, 4, 7, 5, 1⟩⟩,
  ⟨-1235, ⟨7, 4, 8, 4, 1⟩⟩,
  ⟨-4095, ⟨7, 5, 3, 8, 1⟩⟩,
  ⟨-29315, ⟨7, 5, 4, 7, 1⟩⟩,
  ⟨-1235, ⟨6, 5, 4, 8, 1⟩⟩,
  ⟨-1235, ⟨7, 4, 4, 8, 1⟩⟩,
  ⟨-1235, ⟨7, 5, 3, 8, 1⟩⟩,
  ⟨-1235, ⟨7, 5, 4, 7, 1⟩⟩,
  ⟨-69030, ⟨7, 5, 5, 6, 1⟩⟩,
  ⟨-5720, ⟨6, 5, 5, 7, 1⟩⟩,
  ⟨-5720, ⟨7, 4, 5, 7, 1⟩⟩,
  ⟨-5720, ⟨7, 5, 4, 7, 1⟩⟩,
  ⟨-5720, ⟨7, 5, 5, 6, 1⟩⟩,
  ⟨-69030, ⟨7, 5, 6, 5, 1⟩⟩,
  ⟨-8970, ⟨6, 5, 6, 6, 1⟩⟩,
  ⟨-8970, ⟨7, 4, 6, 6, 1⟩⟩,
  ⟨-8970, ⟨7, 5, 5, 6, 1⟩⟩,
  ⟨-8970, ⟨7, 5, 6, 5, 1⟩⟩,
  ⟨-29315, ⟨7, 5, 7, 4, 1⟩⟩,
  ⟨-5720, ⟨6, 5, 7, 5, 1⟩⟩,
  ⟨-5720, ⟨7, 4, 7, 5, 1⟩⟩,
  ⟨-5720, ⟨7, 5, 6, 5, 1⟩⟩,
  ⟨-5720, ⟨7, 5, 7, 4, 1⟩⟩,
  ⟨-4095, ⟨7, 5, 8, 3, 1⟩⟩,
  ⟨-1235, ⟨6, 5, 8, 4, 1⟩⟩,
  ⟨-1235, ⟨7, 4, 8, 4, 1⟩⟩,
  ⟨-1235, ⟨7, 5, 7, 4, 1⟩⟩,
  ⟨-1235, ⟨7, 5, 8, 3, 1⟩⟩,
  ⟨-1404, ⟨7, 6, 2, 8, 1⟩⟩,
  ⟨-14469, ⟨7, 6, 3, 7, 1⟩⟩,
  ⟨-663, ⟨6, 6, 3, 8, 1⟩⟩,
  ⟨-663, ⟨7, 5, 3, 8, 1⟩⟩,
  ⟨-663, ⟨7, 6, 2, 8, 1⟩⟩,
  ⟨-663, ⟨7, 6, 3, 7, 1⟩⟩,
  ⟨-47580, ⟨7, 6, 4, 6, 1⟩⟩,
  ⟨-4095, ⟨6, 6, 4, 7, 1⟩⟩,
  ⟨-4095, ⟨7, 5, 4, 7, 1⟩⟩,
  ⟨-4095, ⟨7, 6, 3, 7, 1⟩⟩,
  ⟨-4095, ⟨7, 6, 4, 6, 1⟩⟩,
  ⟨-69030, ⟨7, 6, 5, 5, 1⟩⟩,
  ⟨-8970, ⟨6, 6, 5, 6, 1⟩⟩,
  ⟨-8970, ⟨7, 5, 5, 6, 1⟩⟩,
  ⟨-8970, ⟨7, 6, 4, 6, 1⟩⟩,
  ⟨-8970, ⟨7, 6, 5, 5, 1⟩⟩,
  ⟨-47580, ⟨7, 6, 6, 4, 1⟩⟩,
  ⟨-8970, ⟨6, 6, 6, 5, 1⟩⟩,
  ⟨-8970, ⟨7, 5, 6, 5, 1⟩⟩,
  ⟨-8970, ⟨7, 6, 5, 5, 1⟩⟩,
  ⟨-8970, ⟨7, 6, 6, 4, 1⟩⟩,
  ⟨-14469, ⟨7, 6, 7, 3, 1⟩⟩,
  ⟨-4095, ⟨6, 6, 7, 4, 1⟩⟩,
  ⟨-4095, ⟨7, 5, 7, 4, 1⟩⟩,
  ⟨-4095, ⟨7, 6, 6, 4, 1⟩⟩,
  ⟨-4095, ⟨7, 6, 7, 3, 1⟩⟩,
  ⟨-1404, ⟨7, 6, 8, 2, 1⟩⟩,
  ⟨-663, ⟨6, 6, 8, 3, 1⟩⟩,
  ⟨-663, ⟨7, 5, 8, 3, 1⟩⟩,
  ⟨-663, ⟨7, 6, 7, 3, 1⟩⟩,
  ⟨-663, ⟨7, 6, 8, 2, 1⟩⟩,
  ⟨-169, ⟨7, 7, 1, 8, 1⟩⟩,
  ⟨-3055, ⟨7, 7, 2, 7, 1⟩⟩,
  ⟨-169, ⟨6, 7, 2, 8, 1⟩⟩,
  ⟨-169, ⟨7, 6, 2, 8, 1⟩⟩,
  ⟨-169, ⟨7, 7, 1, 8, 1⟩⟩,
  ⟨-169, ⟨7, 7, 2, 7, 1⟩⟩,
  ⟨-14469, ⟨7, 7, 3, 6, 1⟩⟩,
  ⟨-1404, ⟨6, 7, 3, 7, 1⟩⟩,
  ⟨-1404, ⟨7, 6, 3, 7, 1⟩⟩,
  ⟨-1404, ⟨7, 7, 2, 7, 1⟩⟩,
  ⟨-1404, ⟨7, 7, 3, 6, 1⟩⟩,
  ⟨-29315, ⟨7, 7, 4, 5, 1⟩⟩,
  ⟨-4095, ⟨6, 7, 4, 6, 1⟩⟩,
  ⟨-4095, ⟨7, 6, 4, 6, 1⟩⟩,
  ⟨-4095, ⟨7, 7, 3, 6, 1⟩⟩,
  ⟨-4095, ⟨7, 7, 4, 5, 1⟩⟩,
  ⟨-29315, ⟨7, 7, 5, 4, 1⟩⟩,
  ⟨-5720, ⟨6, 7, 5, 5, 1⟩⟩,
  ⟨-5720, ⟨7, 6, 5, 5, 1⟩⟩,
  ⟨-5720, ⟨7, 7, 4, 5, 1⟩⟩,
  ⟨-5720, ⟨7, 7, 5, 4, 1⟩⟩,
  ⟨-14469, ⟨7, 7, 6, 3, 1⟩⟩,
  ⟨-4095, ⟨6, 7, 6, 4, 1⟩⟩,
  ⟨-4095, ⟨7, 6, 6, 4, 1⟩⟩,
  ⟨-4095, ⟨7, 7, 5, 4, 1⟩⟩,
  ⟨-4095, ⟨7, 7, 6, 3, 1⟩⟩,
  ⟨-3055, ⟨7, 7, 7, 2, 1⟩⟩,
  ⟨-1404, ⟨6, 7, 7, 3, 1⟩⟩,
  ⟨-1404, ⟨7, 6, 7, 3, 1⟩⟩,
  ⟨-1404, ⟨7, 7, 6, 3, 1⟩⟩,
  ⟨-1404, ⟨7, 7, 7, 2, 1⟩⟩,
  ⟨-169, ⟨7, 7, 8, 1, 1⟩⟩,
  ⟨-169, ⟨6, 7, 8, 2, 1⟩⟩,
  ⟨-169, ⟨7, 6, 8, 2, 1⟩⟩,
  ⟨-169, ⟨7, 7, 7, 2, 1⟩⟩,
  ⟨-169, ⟨7, 7, 8, 1, 1⟩⟩,
  ⟨-169, ⟨7, 8, 1, 7, 1⟩⟩,
  ⟨-13, ⟨6, 8, 1, 8, 1⟩⟩,
  ⟨-13, ⟨7, 7, 1, 8, 1⟩⟩,
  ⟨-13, ⟨7, 8, 0, 8, 1⟩⟩,
  ⟨-13, ⟨7, 8, 1, 7, 1⟩⟩,
  ⟨-1404, ⟨7, 8, 2, 6, 1⟩⟩,
  ⟨-169, ⟨6, 8, 2, 7, 1⟩⟩,
  ⟨-169, ⟨7, 7, 2, 7, 1⟩⟩,
  ⟨-169, ⟨7, 8, 1, 7, 1⟩⟩,
  ⟨-169, ⟨7, 8, 2, 6, 1⟩⟩,
  ⟨-4095, ⟨7, 8, 3, 5, 1⟩⟩,
  ⟨-663, ⟨6, 8, 3, 6, 1⟩⟩,
  ⟨-663, ⟨7, 7, 3, 6, 1⟩⟩,
  ⟨-663, ⟨7, 8, 2, 6, 1⟩⟩,
  ⟨-663, ⟨7, 8, 3, 5, 1⟩⟩,
  ⟨-5720, ⟨7, 8, 4, 4, 1⟩⟩,
  ⟨-1235, ⟨6, 8, 4, 5, 1⟩⟩,
  ⟨-1235, ⟨7, 7, 4, 5, 1⟩⟩,
  ⟨-1235, ⟨7, 8, 3, 5, 1⟩⟩,
  ⟨-1235, ⟨7, 8, 4, 4, 1⟩⟩,
  ⟨-4095, ⟨7, 8, 5, 3, 1⟩⟩,
  ⟨-1235, ⟨6, 8, 5, 4, 1⟩⟩,
  ⟨-1235, ⟨7, 7, 5, 4, 1⟩⟩,
  ⟨-1235, ⟨7, 8, 4, 4, 1⟩⟩,
  ⟨-1235, ⟨7, 8, 5, 3, 1⟩⟩,
  ⟨-1404, ⟨7, 8, 6, 2, 1⟩⟩,
  ⟨-663, ⟨6, 8, 6, 3, 1⟩⟩,
  ⟨-663, ⟨7, 7, 6, 3, 1⟩⟩,
  ⟨-663, ⟨7, 8, 5, 3, 1⟩⟩,
  ⟨-663, ⟨7, 8, 6, 2, 1⟩⟩,
  ⟨-169, ⟨7, 8, 7, 1, 1⟩⟩,
  ⟨-169, ⟨6, 8, 7, 2, 1⟩⟩,
  ⟨-169, ⟨7, 7, 7, 2, 1⟩⟩,
  ⟨-169, ⟨7, 8, 6, 2, 1⟩⟩,
  ⟨-169, ⟨7, 8, 7, 1, 1⟩⟩,
  ⟨-13, ⟨6, 8, 8, 1, 1⟩⟩,
  ⟨-13, ⟨7, 7, 8, 1, 1⟩⟩,
  ⟨-13, ⟨7, 8, 7, 1, 1⟩⟩,
  ⟨-13, ⟨7, 8, 8, 0, 1⟩⟩,
  ⟨-78, ⟨8, 1, 6, 8, 1⟩⟩,
  ⟨-169, ⟨8, 1, 7, 7, 1⟩⟩,
  ⟨-13, ⟨7, 1, 7, 8, 1⟩⟩,
  ⟨-13, ⟨8, 0, 7, 8, 1⟩⟩,
  ⟨-13, ⟨8, 1, 6, 8, 1⟩⟩,
  ⟨-13, ⟨8, 1, 7, 7, 1⟩⟩,
  ⟨-78, ⟨8, 1, 8, 6, 1⟩⟩,
  ⟨-13, ⟨7, 1, 8, 7, 1⟩⟩,
  ⟨-13, ⟨8, 0, 8, 7, 1⟩⟩,
  ⟨-13, ⟨8, 1, 7, 7, 1⟩⟩,
  ⟨-13, ⟨8, 1, 8, 6, 1⟩⟩,
  ⟨-390, ⟨8, 2, 5, 8, 1⟩⟩,
  ⟨-1404, ⟨8, 2, 6, 7, 1⟩⟩,
  ⟨-78, ⟨7, 2, 6, 8, 1⟩⟩,
  ⟨-78, ⟨8, 1, 6, 8, 1⟩⟩,
  ⟨-78, ⟨8, 2, 5, 8, 1⟩⟩,
  ⟨-78, ⟨8, 2, 6, 7, 1⟩⟩,
  ⟨-1404, ⟨8, 2, 7, 6, 1⟩⟩,
  ⟨-169, ⟨7, 2, 7, 7, 1⟩⟩,
  ⟨-169, ⟨8, 1, 7, 7, 1⟩⟩,
  ⟨-169, ⟨8, 2, 6, 7, 1⟩⟩,
  ⟨-169, ⟨8, 2, 7, 6, 1⟩⟩,
  ⟨-390, ⟨8, 2, 8, 5, 1⟩⟩,
  ⟨-78, ⟨7, 2, 8, 6, 1⟩⟩,
  ⟨-78, ⟨8, 1, 8, 6, 1⟩⟩,
  ⟨-78, ⟨8, 2, 7, 6, 1⟩⟩,
  ⟨-78, ⟨8, 2, 8, 5, 1⟩⟩,
  ⟨-780, ⟨8, 3, 4, 8, 1⟩⟩,
  ⟨-4095, ⟨8, 3, 5, 7, 1⟩⟩,
  ⟨-195, ⟨7, 3, 5, 8, 1⟩⟩,
  ⟨-195, ⟨8, 2, 5, 8, 1⟩⟩,
  ⟨-195, ⟨8, 3, 4, 8, 1⟩⟩,
  ⟨-195, ⟨8, 3, 5, 7, 1⟩⟩,
  ⟨-6708, ⟨8, 3, 6, 6, 1⟩⟩,
  ⟨-663, ⟨7, 3, 6, 7, 1⟩⟩,
  ⟨-663, ⟨8, 2, 6, 7, 1⟩⟩,
  ⟨-663, ⟨8, 3, 5, 7, 1⟩⟩,
  ⟨-663, ⟨8, 3, 6, 6, 1⟩⟩,
  ⟨-4095, ⟨8, 3, 7, 5, 1⟩⟩,
  ⟨-663, ⟨7, 3, 7, 6, 1⟩⟩,
  ⟨-663, ⟨8, 2, 7, 6, 1⟩⟩,
  ⟨-663, ⟨8, 3, 6, 6, 1⟩⟩,
  ⟨-663, ⟨8, 3, 7, 5, 1⟩⟩,
  ⟨-780, ⟨8, 3, 8, 4, 1⟩⟩,
  ⟨-195, ⟨7, 3, 8, 5, 1⟩⟩,
  ⟨-195, ⟨8, 2, 8, 5, 1⟩⟩,
  ⟨-195, ⟨8, 3, 7, 5, 1⟩⟩,
  ⟨-195, ⟨8, 3, 8, 4, 1⟩⟩,
  ⟨-780, ⟨8, 4, 3, 8, 1⟩⟩,
  ⟨-5720, ⟨8, 4, 4, 7, 1⟩⟩,
  ⟨-260, ⟨7, 4, 4, 8, 1⟩⟩,
  ⟨-260, ⟨8, 3, 4, 8, 1⟩⟩,
  ⟨-260, ⟨8, 4, 3, 8, 1⟩⟩,
  ⟨-260, ⟨8, 4, 4, 7, 1⟩⟩,
  ⟨-13650, ⟨8, 4, 5, 6, 1⟩⟩,
  ⟨-1235, ⟨7, 4, 5, 7, 1⟩⟩,
  ⟨-1235, ⟨8, 3, 5, 7, 1⟩⟩,
  ⟨-1235, ⟨8, 4, 4, 7, 1⟩⟩,
  ⟨-1235, ⟨8, 4, 5, 6, 1⟩⟩,
  ⟨-13650, ⟨8, 4, 6, 5, 1⟩⟩,
  ⟨-1950, ⟨7, 4, 6, 6, 1⟩⟩,
  ⟨-1950, ⟨8, 3, 6, 6, 1⟩⟩,
  ⟨-1950, ⟨8, 4, 5, 6, 1⟩⟩,
  ⟨-1950, ⟨8, 4, 6, 5, 1⟩⟩,
  ⟨-5720, ⟨8, 4, 7, 4, 1⟩⟩,
  ⟨-1235, ⟨7, 4, 7, 5, 1⟩⟩,
  ⟨-1235, ⟨8, 3, 7, 5, 1⟩⟩,
  ⟨-1235, ⟨8, 4, 6, 5, 1⟩⟩,
  ⟨-1235, ⟨8, 4, 7, 4, 1⟩⟩,
  ⟨-780, ⟨8, 4, 8, 3, 1⟩⟩,
  ⟨-260, ⟨7, 4, 8, 4, 1⟩⟩,
  ⟨-260, ⟨8, 3, 8, 4, 1⟩⟩,
  ⟨-260, ⟨8, 4, 7, 4, 1⟩⟩,
  ⟨-260, ⟨8, 4, 8, 3, 1⟩⟩,
  ⟨-390, ⟨8, 5, 2, 8, 1⟩⟩,
  ⟨-4095, ⟨8, 5, 3, 7, 1⟩⟩,
  ⟨-195, ⟨7, 5, 3, 8, 1⟩⟩,
  ⟨-195, ⟨8, 4, 3, 8, 1⟩⟩,
  ⟨-195, ⟨8, 5, 2, 8, 1⟩⟩,
  ⟨-195, ⟨8, 5, 3, 7, 1⟩⟩,
  ⟨-13650, ⟨8, 5, 4, 6, 1⟩⟩,
  ⟨-1235, ⟨7, 5, 4, 7, 1⟩⟩,
  ⟨-1235, ⟨8, 4, 4, 7, 1⟩⟩,
  ⟨-1235, ⟨8, 5, 3, 7, 1⟩⟩,
  ⟨-1235, ⟨8, 5, 4, 6, 1⟩⟩,
  ⟨-19890, ⟨8, 5, 5, 5, 1⟩⟩,
  ⟨-2730, ⟨7, 5, 5, 6, 1⟩⟩,
  ⟨-2730, ⟨8, 4, 5, 6, 1⟩⟩,
  ⟨-2730, ⟨8, 5, 4, 6, 1⟩⟩,
  ⟨-2730, ⟨8, 5, 5, 5, 1⟩⟩,
  ⟨-13650, ⟨8, 5, 6, 4, 1⟩⟩,
  ⟨-2730, ⟨7, 5, 6, 5, 1⟩⟩,
  ⟨-2730, ⟨8, 4, 6, 5, 1⟩⟩,
  ⟨-2730, ⟨8, 5, 5, 5, 1⟩⟩,
  ⟨-2730, ⟨8, 5, 6, 4, 1⟩⟩,
  ⟨-4095, ⟨8, 5, 7, 3, 1⟩⟩,
  ⟨-1235, ⟨7, 5, 7, 4, 1⟩⟩,
  ⟨-1235, ⟨8, 4, 7, 4, 1⟩⟩,
  ⟨-1235, ⟨8, 5, 6, 4, 1⟩⟩,
  ⟨-1235, ⟨8, 5, 7, 3, 1⟩⟩,
  ⟨-390, ⟨8, 5, 8, 2, 1⟩⟩,
  ⟨-195, ⟨7, 5, 8, 3, 1⟩⟩,
  ⟨-195, ⟨8, 4, 8, 3, 1⟩⟩,
  ⟨-195, ⟨8, 5, 7, 3, 1⟩⟩,
  ⟨-195, ⟨8, 5, 8, 2, 1⟩⟩,
  ⟨-78, ⟨8, 6, 1, 8, 1⟩⟩,
  ⟨-1404, ⟨8, 6, 2, 7, 1⟩⟩,
  ⟨-78, ⟨7, 6, 2, 8, 1⟩⟩,
  ⟨-78, ⟨8, 5, 2, 8, 1⟩⟩,
  ⟨-78, ⟨8, 6, 1, 8, 1⟩⟩,
  ⟨-78, ⟨8, 6, 2, 7, 1⟩⟩,
  ⟨-6708, ⟨8, 6, 3, 6, 1⟩⟩,
  ⟨-663, ⟨7, 6, 3, 7, 1⟩⟩,
  ⟨-663, ⟨8, 5, 3, 7, 1⟩⟩,
  ⟨-663, ⟨8, 6, 2, 7, 1⟩⟩,
  ⟨-663, ⟨8, 6, 3, 6, 1⟩⟩,
  ⟨-13650, ⟨8, 6, 4, 5, 1⟩⟩,
  ⟨-1950, ⟨7, 6, 4, 6, 1⟩⟩,
  ⟨-1950, ⟨8, 5, 4, 6, 1⟩⟩,
  ⟨-1950, ⟨8, 6, 3, 6, 1⟩⟩,
  ⟨-1950, ⟨8, 6, 4, 5, 1⟩⟩,
  ⟨-13650, ⟨8, 6, 5, 4, 1⟩⟩,
  ⟨-2730, ⟨7, 6, 5, 5, 1⟩⟩,
  ⟨-2730, ⟨8, 5, 5, 5, 1⟩⟩,
  ⟨-2730, ⟨8, 6, 4, 5, 1⟩⟩,
  ⟨-2730, ⟨8, 6, 5, 4, 1⟩⟩,
  ⟨-6708, ⟨8, 6, 6, 3, 1⟩⟩,
  ⟨-1950, ⟨7, 6, 6, 4, 1⟩⟩,
  ⟨-1950, ⟨8, 5, 6, 4, 1⟩⟩,
  ⟨-1950, ⟨8, 6, 5, 4, 1⟩⟩,
  ⟨-1950, ⟨8, 6, 6, 3, 1⟩⟩,
  ⟨-1404, ⟨8, 6, 7, 2, 1⟩⟩,
  ⟨-663, ⟨7, 6, 7, 3, 1⟩⟩,
  ⟨-663, ⟨8, 5, 7, 3, 1⟩⟩,
  ⟨-663, ⟨8, 6, 6, 3, 1⟩⟩,
  ⟨-663, ⟨8, 6, 7, 2, 1⟩⟩,
  ⟨-78, ⟨8, 6, 8, 1, 1⟩⟩,
  ⟨-78, ⟨7, 6, 8, 2, 1⟩⟩,
  ⟨-78, ⟨8, 5, 8, 2, 1⟩⟩,
  ⟨-78, ⟨8, 6, 7, 2, 1⟩⟩,
  ⟨-78, ⟨8, 6, 8, 1, 1⟩⟩,
  ⟨-169, ⟨8, 7, 1, 7, 1⟩⟩,
  ⟨-13, ⟨7, 7, 1, 8, 1⟩⟩,
  ⟨-13, ⟨8, 6, 1, 8, 1⟩⟩,
  ⟨-13, ⟨8, 7, 0, 8, 1⟩⟩,
  ⟨-13, ⟨8, 7, 1, 7, 1⟩⟩,
  ⟨-1404, ⟨8, 7, 2, 6, 1⟩⟩,
  ⟨-169, ⟨7, 7, 2, 7, 1⟩⟩,
  ⟨-169, ⟨8, 6, 2, 7, 1⟩⟩,
  ⟨-169, ⟨8, 7, 1, 7, 1⟩⟩,
  ⟨-169, ⟨8, 7, 2, 6, 1⟩⟩,
  ⟨-4095, ⟨8, 7, 3, 5, 1⟩⟩,
  ⟨-663, ⟨7, 7, 3, 6, 1⟩⟩,
  ⟨-663, ⟨8, 6, 3, 6, 1⟩⟩,
  ⟨-663, ⟨8, 7, 2, 6, 1⟩⟩,
  ⟨-663, ⟨8, 7, 3, 5, 1⟩⟩,
  ⟨-5720, ⟨8, 7, 4, 4, 1⟩⟩,
  ⟨-1235, ⟨7, 7, 4, 5, 1⟩⟩,
  ⟨-1235, ⟨8, 6, 4, 5, 1⟩⟩,
  ⟨-1235, ⟨8, 7, 3, 5, 1⟩⟩,
  ⟨-1235, ⟨8, 7, 4, 4, 1⟩⟩,
  ⟨-4095, ⟨8, 7, 5, 3, 1⟩⟩,
  ⟨-1235, ⟨7, 7, 5, 4, 1⟩⟩,
  ⟨-1235, ⟨8, 6, 5, 4, 1⟩⟩,
  ⟨-1235, ⟨8, 7, 4, 4, 1⟩⟩,
  ⟨-1235, ⟨8, 7, 5, 3, 1⟩⟩,
  ⟨-1404, ⟨8, 7, 6, 2, 1⟩⟩,
  ⟨-663, ⟨7, 7, 6, 3, 1⟩⟩,
  ⟨-663, ⟨8, 6, 6, 3, 1⟩⟩,
  ⟨-663, ⟨8, 7, 5, 3, 1⟩⟩,
  ⟨-663, ⟨8, 7, 6, 2, 1⟩⟩,
  ⟨-169, ⟨8, 7, 7, 1, 1⟩⟩,
  ⟨-169, ⟨7, 7, 7, 2, 1⟩⟩,
  ⟨-169, ⟨8, 6, 7, 2, 1⟩⟩,
  ⟨-169, ⟨8, 7, 6, 2, 1⟩⟩,
  ⟨-169, ⟨8, 7, 7, 1, 1⟩⟩,
  ⟨-13, ⟨7, 7, 8, 1, 1⟩⟩,
  ⟨-13, ⟨8, 6, 8, 1, 1⟩⟩,
  ⟨-13, ⟨8, 7, 7, 1, 1⟩⟩,
  ⟨-13, ⟨8, 7, 8, 0, 1⟩⟩,
  ⟨-78, ⟨8, 8, 1, 6, 1⟩⟩,
  ⟨-13, ⟨7, 8, 1, 7, 1⟩⟩,
  ⟨-13, ⟨8, 7, 1, 7, 1⟩⟩,
  ⟨-13, ⟨8, 8, 0, 7, 1⟩⟩,
  ⟨-13, ⟨8, 8, 1, 6, 1⟩⟩,
  ⟨-390, ⟨8, 8, 2, 5, 1⟩⟩,
  ⟨-78, ⟨7, 8, 2, 6, 1⟩⟩,
  ⟨-78, ⟨8, 7, 2, 6, 1⟩⟩,
  ⟨-78, ⟨8, 8, 1, 6, 1⟩⟩,
  ⟨-78, ⟨8, 8, 2, 5, 1⟩⟩,
  ⟨-780, ⟨8, 8, 3, 4, 1⟩⟩,
  ⟨-195, ⟨7, 8, 3, 5, 1⟩⟩,
  ⟨-195, ⟨8, 7, 3, 5, 1⟩⟩,
  ⟨-195, ⟨8, 8, 2, 5, 1⟩⟩,
  ⟨-195, ⟨8, 8, 3, 4, 1⟩⟩,
  ⟨-780, ⟨8, 8, 4, 3, 1⟩⟩,
  ⟨-260, ⟨7, 8, 4, 4, 1⟩⟩,
  ⟨-260, ⟨8, 7, 4, 4, 1⟩⟩,
  ⟨-260, ⟨8, 8, 3, 4, 1⟩⟩,
  ⟨-260, ⟨8, 8, 4, 3, 1⟩⟩,
  ⟨-390, ⟨8, 8, 5, 2, 1⟩⟩,
  ⟨-195, ⟨7, 8, 5, 3, 1⟩⟩,
  ⟨-195, ⟨8, 7, 5, 3, 1⟩⟩,
  ⟨-195, ⟨8, 8, 4, 3, 1⟩⟩,
  ⟨-195, ⟨8, 8, 5, 2, 1⟩⟩,
  ⟨-78, ⟨8, 8, 6, 1, 1⟩⟩,
  ⟨-78, ⟨7, 8, 6, 2, 1⟩⟩,
  ⟨-78, ⟨8, 7, 6, 2, 1⟩⟩,
  ⟨-78, ⟨8, 8, 5, 2, 1⟩⟩,
  ⟨-78, ⟨8, 8, 6, 1, 1⟩⟩,
  ⟨-13, ⟨7, 8, 7, 1, 1⟩⟩,
  ⟨-13, ⟨8, 7, 7, 1, 1⟩⟩,
  ⟨-13, ⟨8, 8, 6, 1, 1⟩⟩,
  ⟨-13, ⟨8, 8, 7, 0, 1⟩⟩
]

def table0Term125State8NormalizeRadix4FlattenChunk1BucketsNormal : List SparsePolynomial :=
  [table0Term125State8NormalizeRadix4Bucket1Normal]

def table0Term125State8NormalizeRadix4FlattenChunk1Normal : SparsePolynomial :=
  table0Term125State8NormalizeRadix4Bucket1Normal

end LeanProofs.PolynomialFormulas.ComputableDummitCoefficients.KernelCertificate
