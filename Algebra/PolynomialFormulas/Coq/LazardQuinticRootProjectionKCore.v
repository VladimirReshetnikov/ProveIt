...
(** The isolated degree-ten root coefficient certificate for Lazard's fourth
    standard projection. *)
Module PolynomialFormulasLazardQuinticRootProjectionKCore.
...
Lemma lazard_root_projection_K_ring_addE (x y : F) :
  x + y = ring_add x y. Proof. reflexivity. Qed.
Lemma lazard_root_projection_K_ring_mulE (x y : F) :
  x * y = ring_mul x y. Proof. reflexivity. Qed.
Lemma lazard_root_projection_K_ring_subE (x y : F) :
  x - y = ring_sub x y. Proof. reflexivity. Qed.
Lemma lazard_root_projection_K_ring_oppE (x : F) :
  - x = ring_opp x. Proof. reflexivity. Qed.
Lemma lazard_root_projection_K_ring_zeroE :
  (0 : F) = ring_zero. Proof. reflexivity. Qed.
Lemma lazard_root_projection_K_ring_oneE :
  (1 : F) = ring_one. Proof. reflexivity. Qed.

Lemma lazard_root_projection_K_ring_theory :
...
Add Ring lazard_root_projection_K_ring : lazard_root_projection_K_ring_theory.
...
Lemma lazard_root_projection_seventy_five_natrE :
  (75%:R : F) = 3%:R * 25%:R.
Proof. exact: (@natrM F 3 25). Qed.
Lemma lazard_root_projection_sixty_three_natrE :
  (63%:R : F) = 3%:R * 20%:R + 3%:R.
Proof.
rewrite -(@natrM F 3 20).
exact: (@natrD F 60 3).
Qed.
Lemma lazard_root_projection_sixty_seven_natrE :
  (67%:R : F) = 51%:R + 16%:R.
Proof. exact: (@natrD F 51 16). Qed.
Lemma lazard_root_projection_seventy_nine_natrE :
  (79%:R : F) = 70%:R + 7%:R + 2%:R.
Proof.
rewrite -(@natrD F 70 7).
exact: (@natrD F 77 2).
Qed.
Lemma lazard_root_projection_hundred_nine_natrE :
  (109%:R : F) = 100%:R + 7%:R + 2%:R.
Proof.
rewrite -(@natrD F 100 7).
exact: (@natrD F 107 2).
Qed.
Lemma lazard_root_projection_two_hundred_ten_natrE :
  (210%:R : F) = 3%:R * 70%:R.
Proof. exact: (@natrM F 3 70). Qed.
Lemma lazard_root_projection_four_hundred_twelve_natrE :
  (412%:R : F) = 300%:R + 100%:R + 10%:R + 2%:R.
Proof.
rewrite -(@natrD F 300 100) -(@natrD F 400 10).
exact: (@natrD F 410 2).
Qed.
Lemma lazard_root_projection_four_hundred_fifteen_natrE :
  (415%:R : F) = 300%:R + 100%:R + 10%:R + 5%:R.
Proof.
rewrite -(@natrD F 300 100) -(@natrD F 400 10).
exact: (@natrD F 410 5).
Qed.
Lemma lazard_root_projection_four_hundred_twenty_natrE :
  (420%:R : F) = 300%:R + 100%:R + 20%:R.
Proof.
rewrite -(@natrD F 300 100).
exact: (@natrD F 400 20).
Qed.
Lemma lazard_root_projection_four_hundred_ninety_six_natrE :
  (496%:R : F) = 300%:R + 160%:R + 28%:R + 8%:R.
Proof.
rewrite -(@natrD F 300 160) -(@natrD F 460 28).
exact: (@natrD F 488 8).
Qed.
Lemma lazard_root_projection_six_hundred_seventy_six_natrE :
  (676%:R : F) = 300%:R + 300%:R + 70%:R + 5%:R + 1.
Proof.
rewrite -(@natrD F 300 300) -(@natrD F 600 70)
  -(@natrD F 670 5).
exact: (@natrD F 675 1).
Qed.
Lemma lazard_root_projection_seven_hundred_fifty_natrE :
  (750%:R : F) = 5%:R * (125%:R + 25%:R).
Proof.
rewrite -(@natrD F 125 25).
exact: (@natrM F 5 150).
Qed.
Lemma lazard_root_projection_one_thousand_hundred_seventy_five_natrE :
  (1175%:R : F) = 1000%:R + 125%:R + 50%:R.
Proof.
rewrite -(@natrD F 1000 125).
exact: (@natrD F 1125 50).
Qed.
Lemma lazard_root_projection_twenty_seven_natrE :
  (27%:R : F) = 24%:R + 3%:R.
Proof. exact: (@natrD F 24 3). Qed.

Ltac finish_lazard_root_projection_K_ring :=
  repeat first
    [ rewrite lazard_root_projection_one_thousand_hundred_seventy_five_natrE
    | rewrite lazard_root_projection_seven_hundred_fifty_natrE
    | rewrite lazard_root_projection_six_hundred_seventy_six_natrE
    | rewrite lazard_root_projection_four_hundred_ninety_six_natrE
    | rewrite lazard_root_projection_four_hundred_twenty_natrE
    | rewrite lazard_root_projection_four_hundred_fifteen_natrE
    | rewrite lazard_root_projection_four_hundred_twelve_natrE
    | rewrite lazard_root_projection_two_hundred_ten_natrE
    | rewrite lazard_root_projection_hundred_nine_natrE
    | rewrite lazard_root_projection_seventy_nine_natrE
    | rewrite lazard_root_projection_seventy_five_natrE
    | rewrite lazard_root_projection_sixty_seven_natrE
    | rewrite lazard_root_projection_sixty_three_natrE
    | rewrite lazard_root_projection_twenty_seven_natrE
    ...
    | rewrite expr1
    | rewrite lazard_root_projection_K_ring_addE
    | rewrite lazard_root_projection_K_ring_mulE
    | rewrite lazard_root_projection_K_ring_subE
    | rewrite lazard_root_projection_K_ring_oppE
    | rewrite lazard_root_projection_K_ring_zeroE
    | rewrite lazard_root_projection_K_ring_oneE ];
  match goal with
  | |- ?lhs = ?rhs => change (ring_eq lhs rhs)
  end;
  ring.

(** Root-only coefficient identity underlying the K projection. *)
Lemma lazard_root_K_component_core roots
    (hsum : lazard_root_esymm1 roots = 0) :
  2%:R * lazard_root_K_component roots =
    5%:R * lazard_root_invariant_K
      (@lazard_depressed_of_roots F roots) (@lazard_root_invariants F roots).
Proof.
have hx4 := lazard_root_sum_zero_last hsum.
rewrite /lazard_root_K_component /lazard_cyclic_fourier_seed.
...
rewrite /lazard_root_T_prime /lazard_root_U_prime
  /lazard_root_invariant_K /lazard_depressed_of_roots /=
...
finish_lazard_root_projection_K_ring.
Qed.


End RootProjectionK.

End PolynomialFormulasLazardQuinticRootProjectionKCore.
