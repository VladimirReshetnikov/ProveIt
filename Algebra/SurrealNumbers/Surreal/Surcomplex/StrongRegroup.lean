...
/-!
# Regrouping actual complex strong sums
...
sum. The two coordinate regroupings of any jointly summable double family
therefore agree. These are the regrouping and interchange assertions after
`a:def:summable`, with joint summability retained throughout.
-/
...
/-- Both orders of a jointly strongly summable double family have the same actual sum.
The inner sums are indexed by the corresponding projection fibers. -/
theorem strongSum_fubini [Small.{u} κ] {F : ι × κ → Surcomplex.{u}}
    (hF : StronglySummable F) :
    strongSum
      (fun i => strongSum (fun p : {p : ι × κ // p.1 = i} => F p.val) (hF.restrict _))
      (hF.regroup Prod.fst) =
    strongSum
      (fun j => strongSum (fun p : {p : ι × κ // p.2 = j} => F p.val) (hF.restrict _))
      (hF.regroup Prod.snd) :=
  (strongSum_regroup hF Prod.fst).trans (strongSum_regroup hF Prod.snd).symm
