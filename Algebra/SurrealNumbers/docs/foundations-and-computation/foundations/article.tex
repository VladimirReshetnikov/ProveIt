...
\vspace{0.5cm}
{\large Source audits: September 21, 2026\par}
{\small Revised exposition: September 22, 2026\par}
\vspace{0.5cm}
...
\smallskip
\emph{For the original merged document.} Three of the five shipped Lean files
were compiled and axiom-audited, under a toolchain different from the one the
sources pin. Those historical checks are recorded in
\cref{found:sub:compiled}. Subsequent repository formalization is a separate
body of evidence, summarized in \cref{found:sub:currentimplementation} and
tracked theorem by theorem in \path{docs/FORMALIZATION.md}. The present
exposition is not a claim that all its results have been formalized.
\end{minipage}
\clearpage
...
The three September 21 source audits were source inspections: none compiled
a Lean file, rebuilt a repository, or ran \texttt{\#print axioms}. The original
merge additionally compiled three illustrative files, as recorded in
\cref{found:sub:compiled}. The repository has since developed a larger Lean
library. These are three different verification records, with different
revisions and scopes; the later work does not retroactively turn the historical
source inspections into executed audits.

\subsection{The current implementation and the historical proposal}
\label{found:sub:currentimplementation}
At repository checkpoint \texttt{ebda0e0}, the checked-development ledger
\path{docs/FORMALIZATION.md} records the constructed sign-sequence ordered
field, its game-quotient comparison, small-cut and fine-topology results, the
concrete surcomplex pair field, and many generic Hahn-field results. It also
records canonical evaluation of small formal normal forms as an order
isomorphism with inverse extraction. That last result does not yet supply the
arithmetic and strong-sum preservation required by the full bridge in
\cref{found:sub:bridge}. Real closedness of the concrete sign carrier and the
resulting surcomplex algebraic-closure specialization remain separate
obligations at this checkpoint.

The implementation uses Lean \texttt{4.32.0} and the dependency revisions in
\path{lake-manifest.json}, rather than the source audits' toolchain. Its
\path{SurrealAudit.lean} target imports the library and checks transitive axiom
dependencies of every imported declaration in the \texttt{Surreal} and
\texttt{SurrealHahnSeries} namespaces, allowing only \texttt{propext},
\texttt{Quot.sound}, and \texttt{Classical.choice}. This is broader than the
three illustrative-file checks, but it still requires a statement-level mapping
to each manuscript claim. The ledger distinguishes complete mappings from
prerequisites, conditional results and pending work; neither a successful build
nor an order isomorphism alone verifies the whole article.

The layers and suggested module paths in \cref{found:sec:architecture} retain
their role as a plan. For implementation status, follow the ledger and actual
imports in \path{Surreal.lean}; do not infer completion from a proposed module
name. The files in this report's \path{code/} and \path{data/} directories
remain the historical originals. The source manuscripts and their READMEs are
retained in Git history at checkpoint \texttt{251bd32}, under the report's
former \path{sources/} directory; they are no longer duplicated in the working
tree.
...
\noindent Every one of those six remains true of the three source audits. The original
merged report changed the first and the third only for the three files named in
\cref{found:sub:compiled}. Its own checks did not rebuild the external
repository, audit all its declarations, run Mizar, or verify a complete supplied
manuscript. Subsequent work in this repository is recorded separately in
\cref{found:sub:currentimplementation}.
...
M1 \texttt{FoundationsSketch} & \texttt{Init} only & compiles, exit 0 &
  \path{noUniversalStrictBound}: none at all\\[0.35em]
...
M2 \texttt{LogicalGuards} & Mathlib & \textbf{does not compile} &
  after repair: \path{noUnrestrictedUpperBounds} none; two others
  \texttt{propext}, \texttt{Quot.sound}\\[0.35em]
M1 \path{InspectCurrentLibrary} & \path{CombinatorialGames} & not attempted &
  repository absent\\[0.35em]
M2 \texttt{SourceAudit} & \path{CombinatorialGames} & not attempted &
  repository absent\\
\bottomrule
\end{tabular}
\end{center}

\textbf{Neither \texttt{sorryAx} nor \texttt{Classical.choice} occurs in the
reported dependencies of these illustrative files.} Every axiom reported is \texttt{propext} or \texttt{Quot.sound}, both ordinary
...
Assume existence and uniqueness of set-supported surreal normal forms, with
evaluation preserving coefficientwise addition and Hahn convolution and sending
$t^\gamma$ to $\omega^{-\gamma}$. Assume also the algebraic-closure theorem
for Hahn fields with divisible exponent group and algebraically closed
coefficient field. Then, in the full-class interpretation:
\begin{enumerate}
...
\end{enumerate}
For $\U=V_\kappa$ as in \cref{found:prop:universe}, the same statements hold
inside $\SC_\U=\No_\U[i]$ when the input family in (1), and the family of
$\U$-small workspaces in (2), are indexed by members of $\U$. The resulting
$\Gamma$, $F_\Gamma$ and $K_\Gamma$ are $\U$-small. An arbitrary external set
of elements of $\SC_\U$ is not sufficient for this relative conclusion.
\end{theorem}
\begin{proof}
...
repeat the rational-span construction. The coefficient-extension maps agree
with normal-form evaluation, so this gives inclusions of the specified
workspaces, not merely abstract isomorphic copies.

For the universe-relative version, each input sign sequence and its normal-form
support have codes in $\U$. Regularity of $\kappa$ bounds the ranks of the
codes in a $\U$-small indexed family strictly below $\kappa$; their union is
therefore in $\U$. The finite-list and rational-span constructions above stay
in $\U$. Since $\Gamma,\R,\C\in\U$, the coefficient-function sets
$\R^\Gamma$ and $\C^\Gamma$, and their Hahn subfields, also belong to $\U$.
The internal normal-form theorem evaluates them in $\No_\U$ and $\SC_\U$.
This is the step that fails for an externally arbitrary family whose support
codes can have ranks cofinal in $\kappa$.
...
Let $A$ be a commutative coefficient ring, $m\in\N$, $V=A^m$, and let
$\Gamma$ be an ordered abelian group. Fix a \emph{set} $E\subseteq\Gamma_{>0}$
that is well ordered in the increasing exponent order. Let $L:V\to V$ be an
invertible $A$-linear map, and let $N:V\to V$ be given by finitely many
polynomials over $A$, each with no terms of total degree zero or one. Extend
these maps coefficientwise and by finite Hahn convolution, respectively. If
$X$ is a $V$-valued Hahn series with $\supp X\subseteq E$, then
\[
 LY+N(Y)=X
\]
...
\end{proposition}
\begin{proof}
Use well-founded recursion on $E^*\setminus\{0\}$. Its well-ordering follows
from the stated hypotheses on $E$ and the Hahn--Neumann lemma. For each
polynomial monomial of degree $d\ge2$, a contribution at exponent $\nu$ uses
positive exponents $\nu_1,\ldots,\nu_d\in E^*$ with
$\nu_1+\cdots+\nu_d=\nu$. Each $\nu_j$ is strictly smaller than $\nu$.
There are finitely many such tuples: expand their entries into words in $E$;
the Hahn--Neumann lemma gives finitely many nondecreasing words with total
$\nu$, and each finite word has finitely many permutations and partitions
into $d$ nonempty groups. There are also only finitely many polynomial
monomials and coordinates. Thus the following coefficient is a finite sum of
...
The two source examples are retained below. A third shows that rank one alone
does not identify strong summation with topological summation.
...
\begin{example}[Strong summation can fail to be a limit even in rank one]
\label{found:ex:boundedrankone}
In $\C((t^\Q))$ the family $A_n=t^{n/(n+1)}$, $n\ge0$, is strongly summable:
its union of supports is a strictly increasing sequence, hence well ordered,
and every exponent occurs once. Let $H$ be its formal sum. For
$P_N=\sum_{n=0}^N A_n$ one has
\[
 v(H-P_N)=\frac{N+1}{N+2}<1.
\]
Thus no partial sum lies in the neighbourhood $\{z:v(H-z)>1\}$. Even the net
of all finite subsums misses that neighbourhood, since some exponent below
$1$ is always omitted. Rank one alone therefore does not identify arbitrary
strong sums with topological sums. What matters for a particular family is
whether its remaining exponents eventually escape every valuation bound.
\end{example}

\emph{Related (batch 32).} These examples concern particular valuation
topologies. The companion report \emph{Omnific-Preserving Automorphisms}, held
in this repository at \path{docs/surreal/omnific-preserving-automorphisms/},
proves the general statement for full Hahn fields in its part on topological
collapse: for a field $k$ and a set-sized ordered abelian group $\Gamma$, the
field $k((t^\Gamma))$ admits a Hausdorff ring topology in which every
Hahn-summable family converges to its sum along the net of its finite partial
sums if and only if $\Gamma=0$ or $\Gamma\cong\Z$; and on $\No$ and on $\No[i]$
the only such ring topology is the indiscrete one.
\Cref{found:ex:boundedrankone} shows the failure for the valuation topology with
$\Gamma=\Q$; that theorem excludes every Hausdorff ring topology.
...
finely (M2). The canonical coefficient-extension embeddings of workspaces
preserve algebra and strong sums but need not preserve the intended ambient
topology in the naive subspace sense.
...
\paragraph{Which intrinsic topology?}
For these nontrivial workspaces, intrinsic order-modulus balls and intrinsic
valuation balls generate the \emph{same} topology, although the individual
balls are different as \cref{found:eq:separator} shows. Write
$V_\gamma=\{z:v(z)>\gamma\}$. If $r\in(F_\Gamma)_{>0}$ and
$\rho=v(r)$, then $V_\rho\subseteq B(0,r)$: higher valuation means a smaller
positive leading magnitude. Conversely, given $\gamma\in\Gamma$, choose
$\eta>\gamma$, possible because $\Gamma\ne0$. Then
$B(0,t^\eta)\subseteq V_\gamma$, since $|z|<t^\eta$ implies
$v(z)\ge\eta>\gamma$. On the complex field the same argument uses
$v(|z|)=v(z)$: in $|z|^2=x^2+y^2$ the first nonzero real coefficients are
squares, so the leading terms cannot cancel. Translation gives the comparison
at every centre. None of these comparisons identifies the topology with the
\emph{external} fine subspace topology.

For a geometric family this gives an exact convergence criterion. If
$\delta\in\Gamma_{>0}$, the strong sum
$H=\sum_{n\ge0}t^{n\delta}=(1-t^\delta)^{-1}$ always exists, and
\[
 v\!\left(H-\sum_{n=0}^N t^{n\delta}\right)=(N+1)\delta.
\]
Consequently its partial sums converge intrinsically if and only if the
multiples $n\delta$ are cofinal in $\Gamma$: for every $\gamma\in\Gamma$
some $n$ satisfies $n\delta>\gamma$. This holds for every $\delta>0$ in an
Archimedean ordered group such as $\Q$, but fails for $\delta=1$ in
$\Q+\Q\omega$. The criterion concerns this geometric family;
\cref{found:ex:boundedrankone} explains why rank one does not suffice for an
arbitrary strongly summable family.
...
\section{The Lean ecosystem in the source audits: one inspection, three readers}
...
a claim that files with those paths exist.} The implementation has progressed
since the source audits; see \cref{found:sub:currentimplementation} and the
repository ledger for the current correspondence. Historical expressions such
as ``the next target'' below describe the proposal's dependency order.
...
The September 21 source inspections, the original merge's three illustrative
Lean checks, and the later repository implementation are separated in
\cref{found:sec:boundary}. The last is tracked by
\path{docs/FORMALIZATION.md}; it does not verify every claim in this report.
The historical per-audit build evidence and per-archive inventories remain in
\cref{found:sub:executed,found:sub:archives}. In particular, checking a changed
dependency graph after \texttt{lake update} would not be a check of the original
pinned revision.
...
This appendix preserves the three source audits' limitation lists, deduplicated,
and the original merge's verification boundary. Statements about what was
compiled refer to that historical work; subsequent repository formalization is
tracked separately in \cref{found:sub:currentimplementation}. The mathematical
and size qualifications continue to apply to the revised exposition.
...
five files, in \cref{found:sub:compiled}. Each \texttt{\#print axioms} result includes the named declaration's
transitive dependencies; it does not audit every other declaration in the
imported libraries.
...
\item Of the claims in the original merged report, the illustrative checks
covered only \cref{found:prop:allcuts}, the unrestricted-cut obstruction, in
the two Lean encodings of \cref{found:sub:compiled}, each depending on no
axioms. The later implementation mappings in
\cref{found:sub:currentimplementation} have a different and explicitly recorded
scope.
\item Where the three audits differ in scope, sources or emphasis, the
differences are printed rather than averaged.
\item The historical synthesis does not promote agreement among source audits
into stronger verification evidence. Later corrections and expanded arguments
are identified in the report's README; the source originals remain available
