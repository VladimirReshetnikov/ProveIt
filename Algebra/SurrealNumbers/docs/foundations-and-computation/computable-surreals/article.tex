...
\label{prop:topology}
For the full surreal class $\No$, a set-indexed net converging in the full
order topology is eventually equal to its limit, and a set-indexed Cauchy net
for the ordered-field uniformity is eventually constant. For a
universe-relative carrier $\No_u$ living one universe above its admissible
option types, both conclusions require the directed index type to be
\emph{small in the lower universe}.
\end{proposition}
\begin{proof}
...
differences zero. Empty distance sets cause no difficulty. At a fixed
universe level, images and finite products of a lower-universe-small index
type are still small there, so the same proof uses admissible option families.
\end{proof}

The size qualifier is necessary. On $\No_u$, index the net $x_r=r$ by all
positive elements of that carrier, ordered in reverse numerical order. For
any $\varepsilon>0$, the tail beyond $r=\varepsilon/2$ satisfies
$0<x_s\le\varepsilon/2<\varepsilon$. The net converges to zero but is never
zero. Its index is a legitimate type in the carrier's larger universe, not
an admissible lower-universe-small type. In the full-class reading the
analogous index is a proper class, so it is not a counterexample to the
set-indexed statement.
...
Use lower bound zero and the finite candidate cover
\[
 F_G(B)=[0,B)\cap
 \bigcup_{\substack{n\in\N\\n\delta<B}}F_{h^n}(B).
\]
The intersection is part of the algorithm. The input can have a negative
supplied lower bound and negative candidates whose coefficients are zero;
ring operations need not remove those redundant candidates. For example,
$h=0$ may have lower bound $-1$ and the valid cover
$F_h(B)=\{-1/2\}$ when $B>-1/2$, empty otherwise. The unfiltered union at
$B=2$ could retain $-1/2$ even though $G(h)=1$. Intersecting with $[0,B)$
removes it without a coefficient-zero test. This filtering preserves every
nonzero term because all powers have nonnegative support, and for $B\le0$
the output cover is empty.
...
without network access. The September 22, 2026 rerun, recorded with its
Python version and UTC timestamp in \texttt{data/summary.json}, gives:
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
...
Merged & 4,366 cross-model exact checks & PASS\\
\bottomrule
\end{tabular}
\end{center}
...
identities, and denominator examples. Its geometric-cover regression includes
negative zero candidates and checks both support completeness and the output
cover range; an exact sparse-series identity would not check that contract. Synthetic halting fixtures do not
