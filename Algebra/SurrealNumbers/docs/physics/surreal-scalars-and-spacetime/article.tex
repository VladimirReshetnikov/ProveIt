...
For Schwarzschild with $m\ne0$, the Kretschmann invariant is exactly
$48m^{2}/r^{6}$. Under a formal ramification $r=s^{q}w(s)$, where $q\ge1$
is an integer and $w\in\R[[s]]$ has $w(0)\ne0$, the pole order becomes
exactly $6q$ and the leading coefficient is $48m^{2}w(0)^{-6}$, so no
radial reparametrization of this form removes the blow-up; the proper time from rest at $R$ to
...
\path{docs/foundations-and-computation/foundations/}
...
$r=s^{q}w(s)$ with $q\ge1$ an integer and $w\in\R[[s]]$ having
$w_0=w(0)\ne0$, one has
\[
 f(s^{q}w(s))=s^{-pq}\widetilde u(s),\qquad
 \widetilde u\in\R[[s]],\qquad \widetilde u(0)=c\,w_0^{-p}\ne0.
\]
Thus the pole order is exactly $pq$, and its leading Laurent coefficient
is $c\,w_0^{-p}$. The pole persists although its coefficient can change.
\end{proposition}
\begin{proof}
...
\]
The substitution into $u$ is defined as a formal power series because
$s^q w(s)$ has positive order. Since $w_0\ne0$, the factor $w(s)^{-p}$
is a power-series unit with constant term $w_0^{-p}$, while
$u(s^q w(s))$ has constant term $c$. Their product is the required
$\widetilde u(s)$, with nonzero constant term $c\,w_0^{-p}$. No term of
higher $s$-order can cancel the resulting leading term at exponent $-pq$.
\end{proof}
...
For $m\ne0$, \cref{phys:prop:pole} applies to \eqref{phys:eq:Kschwarz}
with $p=6$ and $c=48m^{2}$. In the checked special case $w(s)=1+s$,
\[
 \Kret(s^{q}(1+s))=48m^{2}s^{-6q}(1+s)^{-6}.
\]
Expansion for $q=1,2,3$ gives pole order exactly $6q$ and leading coefficient
$48m^{2}$. Its preservation in these tests comes from the normalization
$w(0)=1$. For a general unit $w$, the proposition instead gives
$48m^{2}w(0)^{-6}$; for example, the constant unit $w=2$ gives
$3m^{2}/4$. The pole remains in each case.

The finite verification reconstructed the Christoffel symbols and the
Riemann tensor from the metric and contracted them to obtain
$\Kret=48M^{2}/r^{6}$ before making the normalized substitutions.
Those three tests illustrate the general formal-series argument; they do
not assert an unchanged coefficient for arbitrary $w(0)\ne0$.
\emph{No formal radial reparametrization satisfying the proposition's
hypotheses removes the pole.}
\end{computation}
...
\TA\ Two precise notions of countable addition for Hahn-valued weights are
worked out in the companion report held in
\path{docs/surreal/hahn-valued-measures-and-probability/}. Under strong Hahn
countable additivity each Hahn coefficient is carried by finitely many points
(\texttt{meas:thm:atomic}); a coefficientwise notion that admits diffuse laws
is in its Part~V. Neither is a physical interpretation: that report does not
say how records relate to field-valued weights. Finite surreal-valued probability,
with conditioning on infinitesimal events, a fine-ultrafilter law
(\texttt{fsp:sub:ultrafilter}) and a Loeb bridge, is worked out in
\path{docs/surreal/finite-surreal-probability/}, under the same caveat.
...
\path{docs/surcomplex/differential-equations/}
...
\TA\ For one precisely delimited class the admissibility question has an exact
answer elsewhere in the collection. For the Wick diagram families of a finite
polynomial interaction with a symmetric covariance over $K_\Gamma$, $\Gamma$
divisible, the companion report held in
\path{docs/surcomplex/wick-summability-certificates/} proves that strong Hahn
summability is equivalent to finitely many strict valuation inequalities (its
Theorem~6.1, \texttt{wick:thm:main}), and exhibits a mixed quartic family that
fails although each interaction alone passes. That is a theorem about formal
Gaussian coefficient data. It does not concern the display above and is not a
statement about any physical expansion.
...
\cref{phys:prop:pole} the $r=0$ pole persists under every radial
reparametrization of the stated form; its order is multiplied by $q$ and
its leading coefficient is rescaled by $w(0)^{-6}$.
...
In \path{docs/foundations-and-computation/foundations/}.
...
In \path{docs/surcomplex/differential-equations/}.
...
remains exactly $48m^{2}/r^{6}$ for $m\ne0$. Under a formal radial change
$r=s^{q}w(s)$ with integer $q\ge1$ and $w\in\R[[s]]$, $w(0)\ne0$, the pole
order becomes exactly $6q$ and the leading coefficient becomes
$48m^{2}w(0)^{-6}$. The finite checks used $w=1+s$ and $q=1,2,3$, so their
leading coefficient remained $48m^{2}$. The general proof is
\cref{phys:prop:pole}: no radial reparametrization of this stated form
removes the pole. The proper-time invariant remains $64/(27u^{4})$; the
...
exactly $6q$ with leading coefficient $48M^{2}$ for $q=1,2,3$ and $M\ne0$,
the normalized case $w(0)=1$ of the general coefficient
$48M^{2}w(0)^{-6}$;
...
Kerr interiors. For finite polynomial Gaussian models over $K_\Gamma$ the
diagramwise admissible domain is determined exactly in
\path{docs/surcomplex/wick-summability-certificates/}; that result concerns
formal coefficient data and does not extend to the display.
\end{enumerate}
...
\texttt{Surreal}, \path{docs/surcomplex/differential-equations/}. Owns the
...
\texttt{Surreal}, \path{docs/foundations-and-computation/foundations/}.
