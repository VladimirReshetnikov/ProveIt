...
If $A$ has no largest element, there is one surreal $\delta_A>0$ such that
\begin{equation}\label{eq:uniform-general}
 |h|<\delta_A\quad\Longrightarrow\quad
 B_A(x+h)=B_A(x),\quad T_A(x+h)=T_A(x)
 \qquad(x\in\No).
\end{equation}
In particular, both functions are locally constant everywhere and have zero
derivative everywhere. If $A$ has a largest element $m$, they are
discontinuous at $m$ and are not differentiable there.
\end{theorem}
\begin{proof}
...
Suppose first that $A$ has no largest element. Form the set of all its
positive differences and one separating cut:
\[
 \Delta_A=\{b-a:a,b\in A,\ a<b\},\qquad
 \delta_A=\cut{0}{\Delta_A}.
\]
Replacement makes $\Delta_A$ a set, every member is positive, and the cut
therefore gives $0<\delta_A<d$ for every $d\in\Delta_A$. The radius is
chosen once from $A$, independently of the point $x$.

If $x\in\D_A$, choose $a\in A$ with $x\le a$ and then $b\in A$ with
$b>a$. For $|h|<\delta_A$,
\[
 x+h<x+\delta_A\le a+\delta_A<b,
\]
so $x+h\in\D_A$. If $x\in\U_A$, fix any $a\in A$ and choose $b\in A$
with $b>a$. Since $x>b$ and $\delta_A<b-a$, the same radius gives
\[
 x+h>x-\delta_A>b-\delta_A>a.
\]
This holds for every $a\in A$, so $x+h\in\U_A$. No simultaneous selection
of the witnesses $b$ is needed: each inequality is proved for an arbitrary
fixed $a$. The two indicator formulas now give \eqref{eq:uniform-general},
and every punctured difference quotient is exactly zero on this radius.
...
The indicator depends only on the cut encoded by $A$, not on an enumeration
or on the magnitudes of the rational options. The displayed choice of
$\delta_A$ is sufficient, but not intrinsic to the indicator or necessarily
largest: cofinal sets can give the same indicator with different such radii. No real supremum of
$A$ is used. The uniform radius comes from a cut with a \emph{set} of right
options; this argument does not supply one radius simultaneously for a
proper class of indexing sets.
...
Two openness conventions occur here and must be named separately. The
local order-field convention declares a class open when each point has an
interval contained in it; it is the convention relevant to the
$\varepsilon$--$\delta$ derivative in \eqref{eq:derivative}.
The original article instead uses a global openness convention built from
...
can be seen directly here. It is not a distinction between two choices of
small numerical tolerance: it is a restriction on the allowed global unions.
