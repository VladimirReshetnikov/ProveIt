...
### Compressed fields and denominator transfer review (2026-09-24)

Reviewed the first eight standard results of the internal-field section:
`osq:if:prop:closed`, `osq:if:lem:compression`,
`osq:if:thm:compression`, `osq:if:lem:gf`, `osq:if:thm:transfer`,
`osq:if:cor:unitconstant`, `osq:if:cor:primefield` and
`osq:if:prop:reservoir`. Also checked the bounded-field and transfer
remarks, the explicit compression and nilpotent examples, and the
composed-embedding comparison. No numbered statement is changed.

Expanded closedness into the separate support, algebraic and real
steps. Each inverse is a Neumann geometric series in the set-sized
subgroup generated by its input support. Each polynomial has a set-sized
divisible workspace obtained as the finite rational span of its coefficient
supports; the trivial group is included. Checked Poonen's Corollary 4 in
the author-hosted primary PDF and added its direct bibliography link.
Real and imaginary coefficient parts give the complexification and its
fixed real field; the positive-square and odd-degree-root arguments then
prove real closedness. No classical closedness theorem is applied to a
proper-class exponent group directly.

Made the real scalar structure on H_a explicit using an ordinary integer
larger than the absolute value of the real scalar. The bounds omega^(a)
and omega^(-a) lie outside the compressed field; leading-exponent gaps
prove the two inequalities without bounding the real leading coefficient.
The additive map Psi and the field embedding Theta now have exact
coefficient rules. The support image is a set, and every nonempty subset
has its largest element transported by the order embedding. Strong sums
preserve exactly the same finite contributing-index sets; the inverse
order map proves reflection of strong summability as well. Additivity of
the exponent map identifies the finite product-convolution pairs.
The complex extension is checked through its real and imaginary parts.

Added the exact image description: Theta_a(No) consists of precisely the
series supported in Psi_a(No). The inverse class map on a set support
has set image by replacement, so the converse really yields a surreal
normal form. This is a proper subfield of F_a: h=a*omega^(-3) belongs to
H_a but not Psi_a(No), since every exponent of a nonzero Psi(x) lies
between -2 and -1. Therefore omega^(a*omega^(-3)) is an explicit missing
monomial. The complex embedding has the analogous proper image.

The transfer and residue arguments retain their precise hypotheses.
Inversion, not mere survival, of a purely infinite element is needed.
All calculations in a possibly noncommutative target take place in the
commutative ring generated by the source image and the chosen inverse.
Added a compatibility calculation: (g1*f)*g2=(g2*f)*g1 implies equality
of the two transfer formulas wherever both are defined. Their images
commute with the source image, without a claim of centrality in the whole
target or preservation of a target Hahn summation. Conditional prime
quotients and the broader residue fraction-field hypothesis remain
distinct; neither proof assumes existence of the indicated prime ideals.

Updated root README, catalogue, report guide, notation and formalization
scope/anchors. These eight manuscript results and the added consequences
remain Pending in Lean. Exact binomial kernels, subsequent internal
algebras and full source reconciliation remain pending manuscript review.
Validation: three clean final TeX passes for the 316-page article and
35-page catalogue; no warnings or box diagnostics. Inspected the revised
closedness, compression, proper-image and transfer pages and catalogue.
All 418 standard/principal statements, 828 labels, 1,656 auxiliary label/
number pairs, 66 questions, 60 remarks and 28 numbered examples are
unchanged. The new consequences occur in unnumbered proof/commentary text.
All 4,626 source anchors across 63 reports, 5,220 source-label references,
...
### Support descent and radical countable-support fibres (2026-09-24)

Reviewed the first nine standard branching results, `osq:bb:lem:certificate`
through `osq:bb:lem:quadratic`. Expanded the finite determinant proof with
a module generating list containing 1, explicitly avoiding a free-basis
assumption and cancellation. Its coefficients lie in powers of the ideal,
and substituting X^q gives the desired positive-degree certificate. The
exact certificate separately invokes global radicality. Support projection
now compares coefficients both inside and outside the retained subgroup;
its image equalities use that local elements are fixed. The descent proof
projects one monic-equation term at a time and treats both directions of
the local radical equality. Quotient-kernel calculations prove the reduced
fibre injections.

For the countable-support model, explained why the group generated by the
element's real and imaginary supports is countable and why the full Hahn
field on that group stays in the model. Projection to the entire model
exponent group would not ensure countable witness supports. The rational
function proof eliminates a coprime denominator before evaluating its
projected monic equation at zero. The dominant Laurent-block argument
bounds integer block indices by the support maximum, treats positive and
negative blocks separately, and handles the zero series. Quadratic-branch
independence uses odd orders at T−1 and T−2, then a two-step field basis
to prove injectivity of the polynomial presentation. Specializations are
only maps of that polynomial algebra.

Added an unnumbered strengthening after `osq:bb:cor:descent03`: both extended
normalization ideals in the two-armed countable-support model are radical.
A finite power witness has countably many positive coefficient exponents.
Since cf(κ)>ℵ₀, their strict positive lower bound h gives d=X^(h/m), with
a_j/d^m still in the positive-support ideal. Integral closedness puts x/d
back in the normalization, so x is in the extended ideal. Consequently the
two model descent formulas need no radical, and the unreduced model fibres
embed in the full fibres. This uses the specific model's positive-gap
property and does not imply radicality for arbitrary full Hahn workspaces.
All nine reviewed results and this consequence remain Pending in Lean.

Updated root/report guides, shared notation, catalogue and coverage ledger.
Three final diagnostic-free TeX passes give a 333-page article and 35-page
catalogue. Inspected article PDF pages 212–216 and catalogue pages 12–13;
rechecked article page 213 after removing a redundant rendered ring name.
...
unchanged. Source 20's byte-identical verifier was run from scratch so its
output could not overwrite historical files: all 6,674 assertions in five
groups pass, and the complete JSON matches except Python 3.13.14 versus
3.13.5 (SymPy 1.14.0 in both). These finite diagnostics do not prove
arbitrary-support descent, integrality or the new radicality consequence.
...
### Finite-test transfer, image ideals and fixed workspaces (2026-09-24)

Reviewed the five remaining standard results in Source 22's finite-test
section: tensor base change, lifting ordinary test sets, image-ideal
generation, birational rigidity and fixed-workspace transfer. Together
with the prior seven, this completes the twelve standard-result proofs;
question/status review and full parallel-source reconciliation are still
separate tasks.

The tensor proof now exhibits surjectivity on monomials and injectivity
by clearing one denominator across a finite tensor sum. It uses neither
a basis nor finite generation of Int(D^m), distinguishes the additive
splitting from a ring product, and supplies the evaluation identity and
ring section explicitly. The lifting proof gives the full-column-rank
argument, an invertible minor over the ordinary fraction field that stays
invertible over the ambient field, the dimension lower bound and the
constant-degree boundary. A universal set cannot be empty.

The image-ideal proof gives the three finite ideal inclusions through
Newton expansion and finite differences. An added unnumbered consequence
identifies ct(I_F) with the ordinary ideal generated by ct_*(F)(D^m), in
both the real and Gaussian cases. Finite grid constants and their ordinary
gcd compute that image; this supplies no principal generator for the
original class ideal. Birational rigidity retains an assumed rational
inverse and uses injection into the rational function field to extend
both inverse identities to all ambient points.

For fixed workspaces the proof constructs a positive fresh exponent
outside the convex closure, verifies the reverse well ordering and
summability of separated Laurent blocks, and checks finite outer-index
convolution. The real binomial argument and Gaussian-coordinate Newton
argument transfer without divisibility of the exponent group. Abstract
monomials are written X^b, with omega^b reserved for the surreal realization.
Updated root/report guides, notation, ledger and catalogue. These full
results and the new image-ideal consequence remain pending in Lean.

Three final TeX passes per PDF are diagnostic-free: article 358 pages,
catalogue 37 pages. Inspected article PDF pages 295–299 and catalogue
page 10. All 418 numbered results, 828 labels, 1,656 AUX numbering pairs,
66 questions, 60 remarks and 28 examples remain unchanged. Audits pass
4,626 anchors in 63 reports, 5,244 cited source labels and 2,022 local
