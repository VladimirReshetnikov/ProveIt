...
In the monomial basis $T_1$ is the Newton-sum Hankel matrix $(s_{i+j})$. Over
$\R((t^\Gamma))$, with $\Gamma$ not assumed divisible, its leading principal
minors give the exact splitting field and the number of irreducible factors of
a squarefree real-rooted $P$: see \texttt{spec:hank:thm:field} and
\texttt{spec:hank:thm:profile} in the companion report held in
...
For the forest polynomial of a positive rate matrix every coefficient point
lies on the lower boundary of the Newton polygon; see
\texttt{markov:thm:convexity} and \texttt{markov:rem:newton} in
...
For moment input instead of coefficient input see the companion report held in
\path{docs/surcomplex/prony-reconstruction-at-surreal-scales/}, Theorem~1.1
(\texttt{prony:thm:main}). For $n$ distinct integral nodes with weight
valuations $\varpi_i$, and with $d_i$, $\delta_i$ as in
\eqref{polynomial:eq:stabilitydata}, its exact threshold for the strict-ball
labelling is $\Theta=\max_i\bigl((\varpi_i+d_i)+(d_i+\delta_i)\bigr)$, and
perturbing the last moment attains it at every configuration. Its Remark~6.2
(\texttt{prony:rem:coefficient}) compares $\Theta$ with $T(P)$, and its
Lemma~4.1 (\texttt{prony:lem:localroots}) is a nodewise form of the matching
step above: its hypothesis $v(E(\alpha_i))>d_i+\delta_i$ for every $i$ follows
from $\varepsilon>T(P)$, but not conversely.
