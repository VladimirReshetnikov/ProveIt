...
\emph{Related (batch 33).} The omnific continued-fraction report
(\texttt{docs/\allowbreak surreal/\allowbreak omnific-\allowbreak
continued-\allowbreak fractions/}) proves a fiber theorem of the same shape
for a different coding, the continued-fraction digits given by the omnific
floor: the fiber of an admissible digit sequence is one point plus a convex
precision ideal (\texttt{ocf:thm:fiber}), and an eventually periodic sequence
determines a unique point of a full Hahn field exactly when its period degree
is an order unit of the exponent group (\texttt{ocf:thm:orderunit}). For such
a sequence with positive period degree $E$, whose preperiodic digits have
total degree at most an ordinary multiple of $E$, that report identifies the precision ideal with
$\Ik$ at $\kappa=E$. The two proofs are independent; neither report uses the
other.
