...
`article.tex` / `article.pdf` — one merged research article, 70 pages, 92 numbered statements, self-contained LaTeX with an internal bibliography. Build with
...
**Part II (§§16–23), base a compact connected ordinary Riemann surface `X` of genus `g`.** A closed discrete set of cluster centres is finite, so the well-ordering question is vacuous and what remains is cohomological: the Picard group is classified completely, sections are obstructed by a valuation monodromy, and a finite matrix computes the rest.

§16.1 states, and the article repeats at each point of use, that **the global theorems retain their stated base hypotheses**. Local preparation and stalk algebra do transfer through ordinary coordinate charts; the maintained remark after the compact classification gives the proof. The compact comparison theorem (Thm 17.7) uses compactness twice — finite-dimensional ordinary harmonic spaces, and one global well-ordered support bound for smooth Hahn forms (Lem 17.2) — and *must not be applied to the plane by setting `X = ℂ`, `g = 0`* (Rem 17.8). Conversely the plane obstructions come precisely from infinitely many centres, which a compact base cannot carry.
...
- **What one scale can see** (Thm 23.3). `Pic_lf(X, ℋ_{Γ,≥0}) → lim_n Pic_lf(X, ℋ_{Γ,≥0}/t^{nδ})` is split surjective onto `Pic(X) ⊕ (H¹(X, 𝒪_X) ⊗ 𝔪_Δ)` with kernel exactly `H¹(X, 𝒪_X) ⊗ I_δ`, injective **iff** `g = 0` or `Δ = Γ`. Ex 23.5, the `t^ω` witness in `Γ = ℚ + ℚω`: nontrivial over both sheaves, trivial mod `t^n` for every integer `n`. Cor 23.6: for *any* prescribed set `B` of positive surreal exponents there is a set-sized `Γ ⊇ B` and a nontrivial integral bundle invisible modulo every `t^β` and every integer multiple thereof. The workspace is chosen after `B`: in a fixed nonzero `Γ`, all positive cutoffs together detect every integral Picard class, as the maintained text now proves.
...
**C, independently, declines the same identification.** Written without access to A or B, C defines `Pic_lf` by actual rank-one local trivializations, says explicitly that an ordinary-base Hahn stalk may not be a local ring, and states that it "does not assume the repository's unrefereed claims about principal ideal stalks". That is recorded as Rem 18.5 and noted in Rem 6.4: it is an independent confirmation of Prop 6.2 (nonlocality), not a competing claim. The compact classification proof still needs no PID hypothesis. The maintained review now records a local consequence of the merged text: an ordinary coordinate chart identifies each Hahn stalk on `X` with a plane stalk, preserving all coefficient supports. Thm 6.1 and the dual-generator argument therefore give `Pic = Pic_lf` on any ordinary Riemann surface, without compactness. This does not transfer either global classification to the other base.

**Cofinality, used for two different jobs.** Part I's Rem 2.2 and Part II's Rem 17.5 both record that `nδ` need **not** eventually exceed every positive element of `Γ` — the witness in both is `Γ = ℚ + ℚω`, where `n < ω` for every integer `n`. C's version is kept verbatim in Part II because it is what prevents its Neumann inverse from being misread as a convergence argument: the mechanism is finite decomposability of an *exact* exponent, not domination. Cofinality enters Part II in Thm 23.3: for positive genus, `Δ = Γ` decides whether a one-scale test is faithful; in genus zero the test is always faithful.
...
- For `Γ ≠ 0`, reduction does not extend to a ring map on the full `ℋ_Γ`; positive monomials are units there, so reduction is performed on the `ℋ_{Γ,≥0}` integral model. For `Γ = 0` it is the ordinary identity map.
...
- Divisibility of `Γ` makes `K_Γ` algebraically closed and guarantees that every cluster splits in that field. Without it, individual clusters can still split and all roots exist in a divisible Hahn enlargement. The root-free criterion needs neither splitting nor enlargement.
...
- **No uniform decision procedure** is asserted for unrestricted surcomplex symbolic inputs; each fixed Hahn coefficient uses finitely many ordinary operations, which can include noneffective interpolation or analytic solution operators.
- The original Manuscript C did not assume or verify this repository's other unrefereed claims — it named the principal-ideal stalk theorem specifically. The maintained coordinate-chart argument above uses that theorem as proved in the merged report; it is not a claim that every source manuscript or Lean file was audited.
...
The 22 September 2026 review followed the main chain from support calculus,
preparation and division through the plane divisor and Cousin criteria, the
compact comparison and finite cohomology matrix, the Abel functional and the
one-scale inverse limit. It corrected the complementary divisor's support
union to `ℚ ∩ (0,1)`, qualified the trivial-group reduction and root-splitting
claims, and made the genus-zero exception and workspace quantifiers explicit.
The coordinate-chart proof extending PID stalks and `Pic = Pic_lf` to ordinary
Riemann surfaces received a second independent review. The statements of the
global plane and compact classification theorems keep their base hypotheses.

The ordinary torsor and invertible-sheaf distinctions were checked against
[Stacks Tag 02FQ](https://stacks.math.columbia.edu/tag/02FQ) and
[Tag 09NT](https://stacks.math.columbia.edu/tag/09NT). The finite-matrix proof
remains a direct two-term elimination; its comparison with
[Crainic v1, §2](https://arxiv.org/abs/math/0403266v1) now explains the opposite
homotopy sign convention. These targeted source checks do not audit all
classical analytic imports.

## Reproducing the checks

Run the unchanged scripts on temporary copies, because the compact script
writes `verification_results.json` beside itself:

```sh
check_dir=$(mktemp -d)
cp code/*.py "$check_dir/"
python "$check_dir/09-moving-divisors-picard-dichotomy.py"
python "$check_dir/10-cohomological-obstructions.py"
python "$check_dir/11-compact-curve-picard-classification-verification.py"
```

The maintained review reran all three with Python 3.13.14 and SymPy 1.14.0:
the first passed all six check groups, the second passed 38 checks, and the
compact script passed 53/53. These are separately counted finite examples.
Historical scripts and recorded data were preserved. After ingestion the
compact script's generated sibling file is separate from the historical
results under `data/`; a failed run can still leave an older generated
all-pass file in place, so inspect the process result as well as the JSON.

## Build result

The maintained PDF was rebuilt with three `pdflatex` passes in a temporary
output directory: **70 pages**, no errors, warnings, unresolved references or
box issues. The 69-page baseline had three overfull boxes and one underfull
box; those have been removed. Changed pages were rendered and inspected.
