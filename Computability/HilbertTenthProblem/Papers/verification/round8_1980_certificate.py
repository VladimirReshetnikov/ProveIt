#!/usr/bin/env python3
"""110-operation certificate with a decoded unit coordinate.
...
import round7_1980_certificate as previous
...
OUT = HERE / "round8_1980_certificate.json"
L = previous.L
...
K = LAST_ROW + 1
CODE_COORDINATES = LAST_VARIABLE + ROWS + 1  # includes the unit, excludes input
...
def make_schedule():
    ...
    for target, operation, left, right in previous.SCHEDULE:
        if target == "c0":
            baseline.need((operation, left, right) == ("+", "xb5", 1),
                          "the unique explicit constant-coordinate addition")
            continue
        if left == "c0":
            left = "xb5"
        if right == "c0":
            right = "xb5"
        if target == "b5m2":
            baseline.need((operation, left, right) == ("-", "B", 2), "old third mask")
            right = 4
        schedule.append((target, operation, left, right))
    baseline.need(len(schedule) == 110, "one constant-coordinate addition removed")
    return schedule
...
def verify_support_bounds():
    ...
    baseline.need(MOMENT_MODULUS == 6963 and VARIABLE_WEIGHTS[0] == 1,
                  "input weight and exact Sidon modulus")
    ...
    return {"original_witnesses": ORIGINAL_WITNESSES, "guard_witnesses": 1,
            ...
            "code_coordinates_including_unit_excluding_input": CODE_COORDINATES,
            "coordinates_including_input": CODE_COORDINATES + 1,
            "coefficient_bound": COEFFICIENT_BOUND, "unit_weight": 0,
...
def verify_certificate():
    ...
    baseline.need(len(primitives) == sum(histogram.values()) == 110, "110 arithmetic checks")
    baseline.need(primitive_histogram == {"+": 49, "*": 61}, "final histogram")
    constants = quadratic.verify_constants()
    return {"status": "PASS", "operations": 110, "straight_line_histogram": histogram,
            "additions_and_multiplications_only": {"additions": 49, "multiplications": 61},
            ...
            "input_admissibility": {
                ...
                "special_target": "Ordinary targets 2*F are even; special target delta^2 may be 0 or 1",
                "support_code": "ell_0(B)=1+sum(true coordinate powers)+sum(1832 row powers)",
            ...
            "proofs": ["../1980/UNIT_DIGIT_PROOF.md", "../1980/REVERSED_PACKING_PROOF.md",
            ...
            "operations_with_numerals_from_one": 110 + constants["operations"],
            "scope": "arithmetic identities for the decoded unit coordinate and reversed quadratic packing",
