#!/usr/bin/env python3
"""Exact48 conditional native-positive ternary component; no universal claim."""
...
def verify_certificate():
    need = baseline.need
    need(len(OUTER) == 5 and len(CORE) == 43, 'five outer and43 core operations')
    env = dict(SYM)
    ...
    need(len(primitives) == 48 and counts == {'+':19,'*':29}, 'exact48 conditional operations')
    ...
    return dict(
        status='ARITHMETIC_PASS_CONDITIONAL_NATIVE_POSITIVE_TERNARY_COMPONENT',
        arithmetic_status='PASS', universality_status='NOT_CLAIMED',
        operations=48, outer_operations=5, retained_pell_operations=43,
        ...
        input_domains={'q':'integer >=3','P0':'integer,q^3<=P0<q^4',
                      'core_witnesses':'positive integers','intermediate_registers':'integers'},
        converse_extra_hypotheses='q is a power of three; every one of the log_3(q^4) ternary digits of P0 is1 or2; P0 is even',
        ...
        residuals=records, proof='../1980/EXPLORATION_BASE_THREE_POSITIVE_KERNEL.md',
        scope='Conditional power-of-three and native-positive ternary digit component; excludes packed-word construction/bounds, parity enforcement, local computation, raw input and halting interfaces. Its scale3q^4 is not assumed square. No universal operation bound.')
...
def check_mask_hypotheses():
    """Exact complete native mask cases and separate pre-power inequalities."""
    ...
    domains=[]
    for N in range(1,11):
        L=3**N;scale=3*L;subtotal=0
        for packed in range(L):
            expected=all(1<=packed//3**i%3<=2 for i in range(N))
            r=3*packed+2
            valuation=factorial_valuation(2*r)-2*factorial_valuation(r)
            assert valuation<=N+1 and (valuation==N+1)==expected
            assert r%2==packed%2
            complete+=1;subtotal+=1;accepted+=expected
        domains.append(dict(length=N,complete_words=subtotal))

    prepower=[]
    for q in range(3,31):
        for packed in (q**3,(q**3+q**4)//2,q**4-1):
            scale=3*q**4;r=3*packed+2
            assert scale>=243 and r>=83 and r<scale<r*r
            assert scale*scale>r+1 and scale*(scale+1)>2*r+1
            assert 12*r<scale*(scale+1)
            prepower.append(dict(q=q,P0=packed,r=r,D0=scale))

    rows=[]
    for q in (3,9,27):
        L=q**4;scale=3*L
        for packed in ((L-1)//2,L-1):
            r=3*packed+2
            assert q**3<=packed<L and packed%2==r%2==0
            exponent=0;rest=scale
            while rest>1:
                assert rest%3==0
                exponent+=1;rest//=3
            valuation=factorial_valuation(2*r)-2*factorial_valuation(r)
            ...
            rows.append(dict(q=q,P0=packed,r=r,D0=scale,
                             ...
                             even_native_positive_input=True))
    return dict(complete_mask_cases=complete,accepted_mask_cases=accepted,
                complete_domains=domains,prepower_cases=prepower,
                positive_converse_hypothesis_cases=rows)
...
def verify():
    ...
    result['regression'] = dict(
        ...
        mask=check_mask_hypotheses(),
        canonical_ratio_cases=[check_canonical_ratio(r) for r in (83,84)],
        arithmetic='Exact integer powers and cross-multiplied rational inequalities',
        auxiliary_witnesses_materialized=False,
        scope='The complete native mask tests and preliminary inequalities are distinct from standalone canonical ratio cases at the changed lower endpoint. No huge relaxed auxiliary tuple is materialized; the general proof establishes the unbounded positive converse.')
    return result
