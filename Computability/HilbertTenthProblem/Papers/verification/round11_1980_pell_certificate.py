#!/usr/bin/env python3
"""108 operations: factor the odd Pell root and halve its index modulus.
...
OUT = HERE / "round11_1980_pell_certificate.json"
L = previous.L
K = previous.K
...
def make_schedule():
    ...
    baseline.need(len(schedule) == 109, "109-operation predecessor")
    ...
    baseline.need(len(schedule) == 108, "108-operation odd-root schedule")
    return schedule
...
def verify_certificate():
    ...
    baseline.need(len(source) == len(EQUALITIES) == len(EQUATION_LABELS) == 21,
                  "21 source equations and equality tests")
    records = []
    ...
    baseline.need(len(primitives) == sum(histogram.values()) == 108, "108 arithmetic instructions")
    baseline.need(primitive_histogram == {"+": 49, "*": 59}, "final primitive histogram")
    receipt.update({
        "operations": 108, "straight_line_histogram": histogram,
        "additions_and_multiplications_only": {"additions": 49, "multiplications": 59},
        ...
        "operations_with_numerals_from_one": 108 + receipt["numerals_from_one"]["operations"],
        "scope": "unit-position homogeneous packing; factored odd Pell root and halved index modulus",
    })
    return receipt
