#!/usr/bin/env python3
"""Independent finite audit of the bounded radix-8 two-auxiliary history."""
...
def check_complete_tuple(m,t,B,Y,U,V,I,F):
    ...
    return dict(m=m,t=t,I=I,F=F,q=q,B=B,C=C,Y=Y,U=U,V=V,
                alpha=q-rhs,alphaI=v-I,r_bit_count=r.bit_count(),
...
def verify():
    ...
    for I in (1,9,65,73,513):
        for t in (1,2,3,4,8,16):
            ...
            U,V = auxiliaries(15*B+4*Y-B//8,q)
            canonical.append(check_complete_tuple(m,t,B,Y,U,V,I,outputs[-1]))
    assert accepted and no_111 > 0
    return {'status':'PASS',
            'scope':'Independent complete bounded T/final-Y enumeration, exact local truth table and positive canonical outer histories for the proposed80 component; no full Pell witnesses or universal interface instantiated.',
