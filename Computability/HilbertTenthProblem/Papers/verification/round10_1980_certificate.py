#!/usr/bin/env python3
"""108-operation certificate using a positive computed coefficient.
...
import round9_1980_certificate as previous
...
OUT = HERE / "round10_1980_certificate.json"
L = previous.L
...
NAMES = list(previous.NAMES) + ["Omega"]
SYM = {name: sp.Symbol(name) for name in NAMES}
EQUALITIES = list(previous.EQUALITIES) + [("t2", "Omega")]
EQUATION_LABELS = list(previous.EQUATION_LABELS) + ["coefficient_positive"]
...
def make_schedule():
    ...
    for target, operation, left, right in previous.SCHEDULE:
        if target == "bound_sum":
            baseline.need((operation, left, right) == ("+", "S2", "C2"),
                          "the addition of the redundant square bound")
            continue
        if left == "bound_sum":
            left = "S2"
        if right == "bound_sum":
            right = "S2"
        if target == "t2":
            baseline.need((operation, left, right) == ("-", "twice_e", "zl"),
                          "the old negative computed coefficient")
            left, right = right, left
        if target == "S3":
            baseline.need((operation, left, right) == ("+", "P1", "P2"),
                          "the old signed third-block sum")
            operation, left, right = "-", "P2", "P1"
        schedule.append((target, operation, left, right))
    baseline.need(len(schedule) == 108, "one redundant bound addition removed")
    return schedule
...
def source_residuals():
    ...
    source[-1] = S3 - s["sigma"]
    source.append(s["Z"]*s["la"] - 2*e - s["Omega"])
    return source
...
def verify_certificate():
    ...
    baseline.need(len(NAMES) == len(set(NAMES)) == 38, "38 distinct positive inputs")
    baseline.need(len(set(NAMES) - set(PARAMETERS)) == 34, "34 positive unknowns")
    env = dict(SYM)
    ...
    baseline.need(len(source) == len(EQUALITIES) == len(EQUATION_LABELS) == 22,
                  "22 explicit source equations and tests")
    records = []
    ...
    baseline.need(len(primitives) == sum(histogram.values()) == 108, "108 arithmetic checks")
    baseline.need(primitive_histogram == {"+": 48, "*": 60}, "final histogram")
    constants = quadratic.verify_constants()
    return {"status": "PASS", "operations": 108, "straight_line_histogram": histogram,
            "additions_and_multiplications_only": {"additions": 48, "multiplications": 60},
            ...
            "positive_coefficient": {"register": "t2", "positive_unknown": "Omega",
                                     "value": "Z*lambda-2*e", "enforcement": "the free equality t2=Omega"},
            ...
            "proofs": ["../1980/POSITIVE_COEFFICIENT_BOUND_PROOF.md",
                       "../1980/INPUT_UNIT_PROOF.md", "../1980/PELL_RELAXED_RADIX_PROOF.md",
            ...
            "operations_with_numerals_from_one": 108 + constants["operations"],
            "scope": "arithmetic identities using positive computed coefficient Z*lambda-2*e",
