...
/-!
# The Pell block of the 90-operation system

`Papers/1980/BASE_TWO_PELL_90_PROOF.md` and `HALF_PARAMETER_PELL_92_PROOF.md`.
From `Pos90` (including `0 < x`), `Sys90`, and the bootstrap bounds, before any
coefficient row is decoded:

* the main norm `E15` classifies `(d, c)` as a Pell pair of `A = a + 2`, and the
  first norm `E9` classifies `(2τ + 1, k)` as a Pell pair of `P = 2UY² + 1`; the
  index congruence `E11` gives a first index `≥ r + 1`, and growth forces the main
  index to be `≥ r + 2`, so that `c > A (A² − 1)²`;
* the relaxed auxiliary norm `E16` then supplies the integral auxiliary pair
  (`relaxed_aux`), and the half-parameter block `E17`, `E17b` fixes the main index
  to `J = 2r + 1` (`half_index`), so `c = ψ_A(J)`, `d = χ_A(J)`;
* the ratio block gives `k = ψ_P(r + 1)`, `Y ≥ U^r`, `a > U^(r+1)`;
* the exponent congruences give `U = 2^J` (`Exp90.exp_U`) and `q = B^L`
  (`Exp90.exp_q`), `B = H + b + 2`, with `κ = ψ_A(L)`;
* the ratio estimates with the exact binomial tail give `n² ∣ C(2r, r)`
  (`Ratio90.Y_eq_floor`, `Ratio90.central_dvd`);
* hence `q` and `B` are powers of two.

The index hypotheses used are `64 ≤ H`, `0 < L`, `3L ≤ H` and `Tindex = ψ₂(L)`.
-/

namespace Jones1980

open Diophantine Pell Exp90 Ratio90

/-- `36 r < 4^r` for `r ≥ 5`. -/
theorem thirty_six_mul_lt_pow (r : ℕ) (hr : 5 ≤ r) : 36 * r < 4 ^ r := by
  induction r, hr using Nat.le_induction with
  | base => norm_num
  | succ r _ ih => rw [pow_succ]; omega

/-- The conclusions of the Pell block, for the fixed proofs `hA : 1 < a + 2` and
`hPp : 1 < P`. -/
structure PellBlock90 (a b c d k n q r s w κ H L : ℕ) (hA : 1 < a + 2)
...
variable {x V H Tindex L a b c d e f g h i j k l n o q r s t w α γ η θ lam τ φ κ μ ρ Δ β ζ σ y : ℕ}
...
theorem pell_block90 (hH : 64 ≤ H) (hL : 0 < L) (hLH : 3 * L ≤ H)
    (hT : Tindex = yn two_lt_one L) (hA : 1 < a + 2)
    (hPp : 1 < 2 * (w * n ^ 2 * (s * n ^ 2) ^ 2) + 1) :
    PellBlock90 a b c d k n q r s w κ H L hA hPp := by
  -- sizes
  have hq9 := nine_le_q90 hP hS hH
  obtain ⟨hnr, hr2n⟩ := r_bounds90 hP hS (by omega)
  have hn := hS.E6
  ...
  have hcJ : 2 * r + 1 < c := by
    ...
    omega
  -- the main Pell pair
  have hDeq : (a + 2) ^ 2 - 1 = a ^ 2 + 4 * a + 3 := sq_sub_one_eq a
  ...
  have ht'ge : r + 1 ≤ t' := by
    ...
    omega
  -- `P > 2A`: `2(a + 2) < 2P − 1`
  have hPA : 2 * (a + 2) < 2 * (2 * (U * Y ^ 2) + 1) - 1 := by
    have h1 : 4 * (Y * (U + 1) + 2) < 2 * (2 * (U * Y ^ 2) + 1) - 1 := two_A_lt (by omega) hY2
    calc 2 * (a + 2) ≤ 4 * (a + 2) := by omega
      _ = 4 * (Y * (U + 1) + 2) := by rw [ha]
      _ < _ := h1
  -- the main index is at least `r + 2`
  have hp'2 : r + 2 ≤ p' := by
    by_contra hlt; push Not at hlt
    have h1 : c ≤ (2 * (a + 2)) ^ (p' - 1) := by
      rw [hcp]
      have := ψ_succ_le_pow hA (p' - 1)
      rwa [Nat.sub_add_cancel hp'0] at this
    have h2 : (2 * (a + 2)) ^ (p' - 1) ≤ (2 * (a + 2)) ^ r :=
      Nat.pow_le_pow_right (by omega) (by omega)
    have h3 : (2 * (a + 2)) ^ r < (2 * (2 * (U * Y ^ 2) + 1) - 1) ^ r :=
    ...
    omega
  -- `c > A (A² − 1)²`
  have hbig : (a + 2) * ((a + 2) ^ 2 - 1) ^ 2 < c := by
    have h1 : (2 * (a + 2) - 1) ^ (p' - 1) ≤ c := by
      rw [hcp]
      have := pow_le_ψ_succ hA (p' - 1)
      rwa [Nat.sub_add_cancel hp'0] at this
    have h2 : (a + 2) ^ 6 ≤ (2 * (a + 2) - 1) ^ (p' - 1) :=
      calc (a + 2) ^ 6 ≤ (2 * (a + 2) - 1) ^ 6 := Nat.pow_le_pow_left (by omega) 6
        _ ≤ (2 * (a + 2) - 1) ^ (p' - 1) := Nat.pow_le_pow_right (by omega) (by omega)
    have h3 : (a + 2) * ((a + 2) ^ 2 - 1) ^ 2 < (a + 2) ^ 6 := by
      have hlt : ((a + 2) ^ 2 - 1) ^ 2 < ((a + 2) ^ 2) ^ 2 :=
        Nat.pow_lt_pow_left (by have := Nat.one_le_pow 2 (a + 2) (by omega); omega) two_ne_zero
      calc (a + 2) * ((a + 2) ^ 2 - 1) ^ 2 < (a + 2) * ((a + 2) ^ 2) ^ 2 :=
            Nat.mul_lt_mul_of_pos_left hlt (by omega)
        _ = (a + 2) ^ 5 := by ring
        _ ≤ (a + 2) ^ 6 := Nat.pow_le_pow_right (by omega) (by norm_num)
    omega
  ...
  have hpJ : p' = 2 * r + 1 :=
    half_index hA (by omega) hcJ ⟨r, rfl⟩ hp'0 hd hcp hfm hpm hcm hicm hP.i rfl hS.E17b hP.y
      (by
        push_cast
        rw [show ((a : ℤ) + 2) ^ 2 - 1 = (a : ℤ) ^ 2 + 4 * a + 3 by ring]
        exact hS.E17)
  ...
  -- `U = 2^J`
  have hU2 : U = 2 ^ (2 * r + 1) := by
    apply exp_U hA hcJ' hdJ' (by omega) ?_ ?_ hS.E14
    · calc 2 ^ (3 * (2 * r + 1)) ≤ 2 ^ (6 * (r + 1)) := Nat.pow_le_pow_right (by norm_num) (by omega)
        _ = (2 ^ 6) ^ (r + 1) := by rw [pow_mul]
        _ ≤ U ^ (r + 1) := Nat.pow_le_pow_left (by norm_num at hU64 ⊢; omega) _
        _ < a + 2 := by omega
    · calc U ^ 3 ≤ U ^ (r + 1) := Nat.pow_le_pow_right (by omega) (by omega)
        _ < a + 2 := by omega
  -- `q = B^L`
  have hBn : H + b + 2 < n := B_lt_n90 hP hS
  obtain ⟨hq_eq, hκ_eq, _⟩ := exp_q (B := H + b + 2) hA hcJ' hL (by omega)
    (by have := hS.E13; have := hP.φ; omega) hP.κ hT
    (by
      calc 4 ^ (2 * r + 1) ≤ 4 ^ (2 * (r + 1)) := Nat.pow_le_pow_right (by norm_num) (by omega)
        _ = 16 ^ (r + 1) := by rw [pow_mul]; norm_num
        _ ≤ U ^ (r + 1) := Nat.pow_le_pow_left (by omega) _
        _ ≤ a := hagt.le)
    (by omega) hP.q
    (by
      calc (H + b + 2) ^ (3 * L) ≤ n ^ (3 * L) := Nat.pow_le_pow_left hBn.le _
  ...
  have hU2' : 2 ^ (2 * r + 1) ≤ U := hU2.ge
  have hlow := lower_estimate (U := w * n ^ 2) (Y := s * n ^ 2) (by omega) (by omega) (by omega) rfl
    hA ha hPp hcJ' hk_eq
  have h72 : 72 * r < U + 1 := by
    have h1 := thirty_six_mul_lt_pow r (by omega)
    have : U = 2 * 4 ^ r := by rw [hU2, pow_succ, mul_comm, pow_mul]; norm_num
    omega
  ...
  obtain ⟨w', hw'⟩ := Y_eq_floor (U := w * n ^ 2) (Y := s * n ^ 2) (by omega) hU2' hP.k hP.η hS.E10a
  ...
  have hqpow : ∃ i, q = 2 ^ i := by
    ...
    exact ⟨j, hj⟩
  have hBpow : ∃ i, H + b + 2 = 2 ^ i := by
    obtain ⟨i, hi⟩ := hqpow
    have : H + b + 2 ∣ 2 ^ i := by
      rw [← hi, hq_eq]; exact dvd_pow_self (H + b + 2) hL.ne'
    obtain ⟨j, _, hj⟩ := (Nat.dvd_prime_pow Nat.prime_two).1 this
    exact ⟨j, hj⟩
  exact ⟨hcJ', hdJ', hk_eq, hYr, hagt, hU2, hq_eq, hκ_eq, hcentral, hqpow, hBpow⟩
