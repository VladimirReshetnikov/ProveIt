...
/-!
# The ratio block of the base-three Pell kernel (Pell base `A = a + 3`)

The alternative universal constructions of `Papers/1980/` (the counter-machine
family `EXPLORATION_STATE_TOP_DOUBLED_GRID.md` and the tag-system family
`EXPLORATION_PRODUCT_COORDINATE_TAG.md`) share the base-three Pell kernel of
`EXPLORATION_UNIT_TWO_TERNARY_KERNEL.md` / `EXPLORATION_BASE_THREE_PELL_KERNEL.md`:
`U = w D₀`, `Y = s D₀`, `Q = U Y²`, `P = 2Q + 1`, `a = Y(U + 1)`, `A = a + 3`,
`J = 2r + 1`.  This file is the ratio block of `PellRatio90.lean` with the main
parameter shifted from `a + 2` to `a + 3`:

1. `k = ψ_P(r + 1)` (`k_index`), by the congruence `ψ_P(t) ≡ t (mod U Y)` and growth;
2. `U^r ≤ Y` (`Y_lower`), since `c/k > ξ := (U+1)^(2r)/U^r` and `c/k < Y + 1`;
3. `c/k < ξ + 1/2` once `72 r < U + 1` (`upper_estimate`), so that
   `Ratio90.Y_eq_floor` (which does not depend on the Pell base) gives
   `Y = ⌊ξ⌋ ≡ C(2r, r) (mod U)`.

The only changes are the three elementary inequalities `lower_key`, `two_A_lt`,
`upper_key`, whose constants are redone for `A = Y(U+1) + 3`, and the bound
`hbig : 3 ≤ Y(U+1)` used by `upper_key` (the 90-operation version used `16`).
-/
...
/-- `2P (U+1)² < (2A − 1)² U` for `A = Y(U+1) + 3`, `P = 2UY² + 1`. -/
theorem lower_key {U Y : ℕ} (hU : 1 ≤ U) (hY : 1 ≤ Y) :
    2 * (2 * (U * Y ^ 2) + 1) * (U + 1) ^ 2 < (2 * (Y * (U + 1) + 3) - 1) ^ 2 * U := by
  have e : 2 * (Y * (U + 1) + 3) - 1 = 2 * (Y * (U + 1)) + 5 := by omega
  rw [e]
...
theorem k_index {U Y r k h c η ζ τ a J : ℕ} (hU : 4 ≤ U) (hY : 2 ≤ Y) (hr : 1 ≤ r)
  ...
  · exfalso
    ...
    have hk : (2 * (2 * (U * Y ^ 2) + 1) - 1) ^ (2 * r) ≤ k := by
      ...
      exact h2.trans h1
    have hcb : c ≤ (2 * (a + 3)) ^ (2 * r) := by
      rw [hc, hJ]; exact ψ_succ_le_pow hA (2 * r)
    have h4A := two_A_lt hU hY
    rw [← ha] at h4A
    have hlt2 : (2 * (a + 3)) ^ (2 * r) < (2 * (2 * (U * Y ^ 2) + 1) - 1) ^ (2 * r) := by
...
theorem lower_estimate {U Y r k c a J : ℕ} (hU : 1 ≤ U) (hY : 1 ≤ Y) (hr : 1 ≤ r)
  ...
  calc k * (U + 1) ^ (2 * r) ≤ (2 * (2 * (U * Y ^ 2) + 1)) ^ r * (U + 1) ^ (2 * r) :=
    ...
    _ < ((2 * (a + 3) - 1) ^ 2 * U) ^ r := hpow
    _ = (2 * (a + 3) - 1) ^ (2 * r) * U ^ r := by rw [mul_pow, ← pow_mul]
...
theorem Y_lower {U Y r k c η ζ a J : ℕ} (hU : 1 ≤ U) (hY : 1 ≤ Y) (hr : 1 ≤ r)
    ...
    (E10a : c = k * Y + η) (E10b : k = η + ζ) (hζ : 0 < ζ) :
    U ^ r ≤ Y := by
  have h1 := lower_estimate hU hY hr hJ hA ha hP hc hk
  -- `c < k (Y + 1)`
...
/-- `(2A)² U ⋅ Y(U+1) ≤ (2P − 1)(U+1)² ⋅ (Y(U+1) + 9)` for `A = Y(U+1) + 3`, `Y(U+1) ≥ 3`. -/
theorem upper_key {U Y : ℕ} (h3 : 3 ≤ Y * (U + 1)) :
    (2 * (Y * (U + 1) + 3)) ^ 2 * U * (Y * (U + 1)) ≤
      (2 * (2 * (U * Y ^ 2) + 1) - 1) * (U + 1) ^ 2 * (Y * (U + 1) + 9) := by
  have e : 2 * (2 * (U * Y ^ 2) + 1) - 1 = 4 * (U * Y ^ 2) + 1 := by omega
  rw [e]
  have e2 : (4 * (U * Y ^ 2) + 1) * (U + 1) ^ 2 = 4 * U * (Y * (U + 1)) ^ 2 + (U + 1) ^ 2 := by
    ring
  rw [e2]
  obtain ⟨a', ha'⟩ : ∃ a', Y * (U + 1) = a' + 3 := ⟨Y * (U + 1) - 3, by omega⟩
  rw [ha']
  have key : (2 * (a' + 3 + 3)) ^ 2 * U * (a' + 3) + (12 * U * a' * (a' + 3) + (U + 1) ^ 2 * (a' + 3 + 9)) =
      (4 * U * (a' + 3) ^ 2 + (U + 1) ^ 2) * (a' + 3 + 9) := by ring
  omega

/-- The upper estimate `c/k < ξ + 1/2` once `72 r < U + 1`, `U^r ≤ Y` and `3 ≤ Y(U+1)`. -/
theorem upper_estimate' {U Y r k c a J : ℕ} (hU : 1 ≤ U) (hY : 1 ≤ Y) (hr : 1 ≤ r)
    (hJ : J = 2 * r + 1) (hA : 1 < a + 3) (ha : a = Y * (U + 1))
    (hP : 1 < 2 * (U * Y ^ 2) + 1) (hc : c = ψ hA J) (hk : k = ψ hP (r + 1))
    (hξY' : ((U : ℝ) + 1) ^ (2 * r) / (U : ℝ) ^ r < Y + 1)
    (h72 : 72 * r < U + 1) (hYr : U ^ r ≤ Y) (h3 : 3 ≤ Y * (U + 1)) :
  ...
  obtain ⟨X, hX⟩ : ∃ X : ℝ, X = ((2 * (2 * (U * Y ^ 2) + 1) - 1 : ℕ) : ℝ) * ((U : ℝ) + 1) ^ 2 :=
    ⟨_, rfl⟩
  obtain ⟨Z, hZ⟩ : ∃ Z : ℝ, Z = (2 * ((a : ℝ) + 3)) ^ 2 * U := ⟨_, rfl⟩
  ...
  have hcZ : (c : ℝ) * (U : ℝ) ^ r ≤ Z ^ r := by
    have e : Z ^ r = (2 * ((a : ℝ) + 3)) ^ (2 * r) * (U : ℝ) ^ r := by
  ...
  have hrε : (r : ℝ) * ε ≤ 1 / 2 := by
    ...
    nlinarith
  have hpow := Ratio90.one_add_pow_le r hε0 hrε
...
theorem upper_estimate {U Y r k c η ζ a J : ℕ} (hU : 1 ≤ U) (hY : 1 ≤ Y) (hr : 1 ≤ r)
  ...
  exact upper_estimate' hU hY hr hJ hA ha hP hc hk hξY' h72 hYr h3

/-- `72 r < 3^(2r+1)` for `r ≥ 2`. -/
theorem seventy_two_mul_lt_pow (r : ℕ) (hr : 2 ≤ r) : 72 * r < 3 ^ (2 * r + 1) := by
  induction r, hr using Nat.le_induction with
  | base => norm_num
  | succ r _ ih =>
    have e : 3 ^ (2 * (r + 1) + 1) = 9 * 3 ^ (2 * r + 1) := by
      rw [show 2 * (r + 1) + 1 = 2 * r + 1 + 2 by ring, pow_add]; ring
    rw [e]; omega

/-- The central-binomial divisibility `D₀ ∣ C(2r, r)` from `Y = C(2r,r) + wU`, `D₀ ∣ U`,
`D₀ ∣ Y`. -/
theorem central_dvd {D₀ U Y r w : ℕ} (hU : D₀ ∣ U) (hY : D₀ ∣ Y)
    (h : Y = (2 * r).choose r + w * U) : D₀ ∣ (2 * r).choose r := by
  have h1 : D₀ ∣ w * U := Dvd.dvd.mul_left hU w
