...
/-!
# The ratio block of the 99-operation system

With `U = w n²`, `Y = s n²`, `Q = U Y²`, `P = 2Q + 1`, `a = Y(U + 1)`, `A = a + 4`,
`J = 2r + 1`, the block
...
parameters `A = Y(U+1) + 4` and `P = 2UY² + 1`.
-/
...
/-- `2P (U+1)² < (2A − 1)² U` for `A = Y(U+1) + 4`, `P = 2UY² + 1`. -/
theorem lower_key {U Y : ℕ} (hU : 1 ≤ U) (hY : 1 ≤ Y) :
    2 * (2 * (U * Y ^ 2) + 1) * (U + 1) ^ 2 < (2 * (Y * (U + 1) + 4) - 1) ^ 2 * U := by
  have e : 2 * (Y * (U + 1) + 4) - 1 = 2 * (Y * (U + 1)) + 7 := by omega
  rw [e]
...
/-- `4A < 2P − 1` for `Y ≥ 2`, `U ≥ 4`. -/
theorem four_A_lt {U Y : ℕ} (hU : 4 ≤ U) (hY : 2 ≤ Y) :
    4 * (Y * (U + 1) + 4) < 2 * (2 * (U * Y ^ 2) + 1) - 1 := by
...
theorem k_index {U Y r k h c η ζ τ a J : ℕ} (hU : 4 ≤ U) (hY : 2 ≤ Y) (hr : 1 ≤ r)
    (hJ : J = 2 * r + 1) (hA : 1 < a + 4) (ha : a = Y * (U + 1))
  ...
  · exfalso
    ...
    have hk : (2 * (2 * (U * Y ^ 2) + 1) - 1) ^ (2 * r) ≤ k := by
      ...
      exact h2.trans h1
    have hcb : c ≤ (2 * (a + 4)) ^ (2 * r) := by
      rw [hc, hJ]; exact ψ_succ_le_pow hA (2 * r)
    have h4A := four_A_lt hU hY
    rw [← ha] at h4A
    have hlt2 : (2 * (a + 4)) ^ (2 * r) < (2 * (2 * (U * Y ^ 2) + 1) - 1) ^ (2 * r) := by
...
theorem Y_lower {U Y r k c η ζ a J : ℕ} (hU : 1 ≤ U) (hY : 1 ≤ Y) (hr : 1 ≤ r)
  ...
  have h1 : k * (U + 1) ^ (2 * r) < c * U ^ r := by
    calc k * (U + 1) ^ (2 * r) ≤ (2 * (2 * (U * Y ^ 2) + 1)) ^ r * (U + 1) ^ (2 * r) :=
      ...
      _ < ((2 * (a + 4) - 1) ^ 2 * U) ^ r := hpow
      _ = (2 * (a + 4) - 1) ^ (2 * r) * U ^ r := by rw [mul_pow, ← pow_mul]
...
theorem upper_estimate' {U Y r k c a J : ℕ} (hU : 1 ≤ U) (hY : 1 ≤ Y) (hr : 1 ≤ r)
  ...
  obtain ⟨X, hX⟩ : ∃ X : ℝ, X = ((2 * (2 * (U * Y ^ 2) + 1) - 1 : ℕ) : ℝ) * ((U : ℝ) + 1) ^ 2 :=
    ⟨_, rfl⟩
  obtain ⟨Z, hZ⟩ : ∃ Z : ℝ, Z = (2 * ((a : ℝ) + 4)) ^ 2 * U := ⟨_, rfl⟩
  ...
  have hcZ : (c : ℝ) * (U : ℝ) ^ r ≤ Z ^ r := by
    have e : Z ^ r = (2 * ((a : ℝ) + 4)) ^ (2 * r) * (U : ℝ) ^ r := by
...
/-- `Y = ⌊ξ⌋ ≡ C(2r, r) (mod U)` once `4 ⋅ 2^(2r) < U`. -/
theorem Y_eq_floor {U Y r k c η ζ : ℕ} (hr : 1 ≤ r) (hU4 : 4 * 2 ^ (2 * r) < U)
    (hk0 : 0 < k) (hη : 0 < η) (E10a : c = k * Y + η) (E10b : k = η + ζ) (hζ : 0 < ζ)
    (hlow : k * (U + 1) ^ (2 * r) < c * U ^ r)
    (hup : (c : ℝ) / k < ((U : ℝ) + 1) ^ (2 * r) / (U : ℝ) ^ r + 1 / 2) :
    ∃ w : ℕ, Y = (2 * r).choose r + w * U := by
  obtain ⟨F, v, w, hsplit, hv, hF⟩ := Jones1982.lemma_2_24 hr (by norm_num : 0 < 4) hU4
  refine ⟨w, ?_⟩
  rw [← hF]
  have hkR : (0 : ℝ) < k := by exact_mod_cast hk0
