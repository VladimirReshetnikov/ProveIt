...
/-!
# Necessity: the Pell witnesses of the 90-operation system

Section 6 of `Papers/1980/BASE_TWO_PELL_90_PROOF.md` (and Section 7 of
`BINARY_PRODUCT_90_PROOF.md`).  Given the central number `r` with
`n² ∣ C(2r, r)`, `n = q⁸`, `q = 2^jq`, `n ≤ r`, `r` even, the radix `B ≤ q = B^L`
and `Tindex = ψ₂(L)`, all Pell unknowns are constructed: `J = 2r + 1`,
`U = 2^J`, `Y = ⌊(U+1)^{2r}/U^r⌋`, `w = U/n²`, `s = Y/n²`, `a = Y(U+1)`,
`A = a + 2`, `P = 2UY² + 1`, `k = ψ_P(r+1)`, `2τ + 1 = χ_P(r+1)`, `c = ψ_A(J)`,
`d = χ_A(J)`, `η = c − kY`, `ζ = k − η`, `h = (k − r − 1)/(UY)`, `κ = ψ_A(L)`,
`μ = χ_A(L)`, `φ = c − κ`, the exponential quotients `γ, ρ`,
`Δ = (ψ_A(L) − ψ₂(L))/a`, and the half-parameter witnesses `f, i, o, j, y`
(`half_witnesses`, which needs `J ≡ 1 (mod 4)`, i.e. `r` even).
-/
...
/-- The Pell witnesses. -/
theorem pell_witnesses90 {r n q B L Tindex jq : ℕ} (hr : 2 ≤ r) (hreven : 2 ∣ r)
    (hn : n = q ^ 8) (hq : q = 2 ^ jq) (hjq : 1 ≤ jq) (hnr : n ≤ r) (hB : 64 ≤ B) (hBq : B ≤ q)
    (hL : 2 ≤ L) (hLB : 3 * L ≤ B) (hqL : q = B ^ L) (hT : Tindex = yn two_lt_one L)
    (hcentral : n ^ 2 ∣ (2 * r).choose r) :
    ∃ a c d f h i j k o s w γ η τ φ κ μ ρ Δ ζ y : ℕ,
      0 < a ∧ 0 < c ∧ 0 < d ∧ 0 < f ∧ 0 < h ∧ 0 < i ∧ 0 < j ∧ 0 < k ∧ 0 < o ∧ 0 < s ∧ 0 < w ∧
      0 < γ ∧ 0 < η ∧ 0 < τ ∧ 0 < φ ∧ 0 < κ ∧ 0 < μ ∧ 0 < ρ ∧ 0 < Δ ∧ 0 < ζ ∧ 0 < y ∧
      τ * (τ + 1) = (w * n ^ 2 * (s * n ^ 2) ^ 2) * (w * n ^ 2 * (s * n ^ 2) ^ 2 + 1) * k ^ 2 ∧
      c = k * (s * n ^ 2) + η ∧ k = η + ζ ∧ k = r + 1 + h * (w * n ^ 2) * (s * n ^ 2) ∧
      a = s * n ^ 2 * (w * n ^ 2 + 1) ∧ c = κ + φ ∧
      d = w * n ^ 2 + a * c + γ * (4 * a + 3) ∧
      d ^ 2 = 1 + (a ^ 2 + 4 * a + 3) * c ^ 2 ∧
      (i * c ^ 2) ^ 2 = (a ^ 2 + 4 * a + 3) * (f ^ 2 - 1) ∧
      (((a : ℤ) ^ 2 + 4 * a + 3) * (f ^ 2 - 1)) * ((2 * r + 1 + j * c) ^ 2 - (y : ℤ) ^ 2) =
        1 - (y : ℤ) ^ 2 ∧
      2 * r + 1 + j * c = c + o * f ∧
      (μ : ℤ) = q + κ * ((a : ℤ) + 2 - B) + ρ * (2 * ((a : ℤ) + 2) * B - (B : ℤ) ^ 2 - 1) ∧
      μ ^ 2 = 1 + (a ^ 2 + 4 * a + 3) * κ ^ 2 ∧
      κ = Tindex + Δ * a := by
  -- sizes
  have hq0 : 0 < q := by rw [hq]; positivity
  have hq2 : 2 ≤ q := by
    rw [hq]; calc 2 = 2 ^ 1 := by norm_num
      _ ≤ 2 ^ jq := Nat.pow_le_pow_right (by norm_num) hjq
  have hn0 : 0 < n := by rw [hn]; positivity
  have hqn : q ≤ n := by rw [hn]; exact Nat.le_self_pow (by norm_num) q
  have hn256 : 256 ≤ n := by
    rw [hn]; calc 256 = 2 ^ 8 := by norm_num
      _ ≤ q ^ 8 := Nat.pow_le_pow_left hq2 8
  have hr256 : 256 ≤ r := le_trans hn256 hnr
  have hn8 : 8 * jq < n := by
  ...
  obtain ⟨U, hU⟩ : ∃ U, U = 2 ^ J := ⟨_, rfl⟩
  have hU0 : 0 < U := by rw [hU]; positivity
  have hU2r : 2 ^ (2 * r + 1) ≤ U := by rw [hU, hJ]
  have hU64 : 64 ≤ U := by
    rw [hU]
  ...
  have hUr : r < U := by
    rw [hU]
    calc r < J := by omega
      _ < 2 ^ J := Nat.lt_two_pow_self
  -- `Y`
  obtain ⟨Y, v, w', hsplit, hv, hY⟩ := binomial_floor_two (R := r) (U := U) (by omega) hU2r
  -- `U^r ≤ Y`
  have hYU : U ^ r ≤ Y := by
    by_contra hlt
    push Not at hlt
  ...
  have hDeq : (a + 2) * (a + 2) - 1 = a ^ 2 + 4 * a + 3 := by
    have : (a + 2) * (a + 2) = a ^ 2 + 4 * a + 4 := by ring
    omega
  have E15 : d ^ 2 = 1 + (a ^ 2 + 4 * a + 3) * c ^ 2 := by
    have h := χ_sq hA J
    simp only [Diophantine.χ, Diophantine.ψ] at h
    rw [← hc, ← hd, hDeq] at h
    rw [sq, h]; ring
  ...
  have h72 : 72 * r < U + 1 := by
    ...
    have : U = 2 * 4 ^ r := by rw [hU, hJ, pow_succ, mul_comm, pow_mul]; norm_num
    omega
  ...
  -- `γ`: the base-two exponent congruence
  obtain ⟨γ, hγ0, hγ⟩ := Jones1982.exists_positive_pell_power_quotient_int (A := a + 2) (B := 2)
    (L := J) hA (by norm_num) (by omega) hJ2
  have E14 : d = U + a * c + γ * (4 * a + 3) := by
    have h1 : (d : ℤ) = U + a * c + γ * (4 * a + 3) := by
      rw [hd, hU]
  ...
  obtain ⟨ρ, hρ0, hρ⟩ := Jones1982.exists_positive_pell_power_quotient_int (A := a + 2) (B := B)
    (L := L) hA (by omega) hBA hL
  obtain ⟨μ, hμ⟩ : ∃ μ, μ = xn hA L := ⟨_, rfl⟩
  have hμ0 : 0 < μ := by rw [hμ]; exact xn_pos' hA L
  have E18 : (μ : ℤ) = q + κ * ((a : ℤ) + 2 - B) +
      ρ * (2 * ((a : ℤ) + 2) * B - (B : ℤ) ^ 2 - 1) := by
    ...
    push_cast
    ring
  have E19 : μ ^ 2 = 1 + (a ^ 2 + 4 * a + 3) * κ ^ 2 := by
    have h := χ_sq hA L
    simp only [Diophantine.χ, Diophantine.ψ] at h
    rw [← hκ, ← hμ, hDeq] at h
    rw [sq, h]; ring
  -- `Δ`: the fixed index
  have hA4 : 4 ^ (L + 1) ≤ a + 2 := by
    calc 4 ^ (L + 1) ≤ 64 ^ L := by
          rw [show (64 : ℕ) = 4 ^ 3 by norm_num, ← pow_mul]
      ...
      _ ≤ r := hnr
      _ ≤ a + 2 := by omega
  have hκT : yn two_lt_one L < κ := by
    rw [hκ]
    show yn two_lt_one L < ψ hA L
    ...
    have hLL : L ≤ (L + 1) * (L - 1) := by
      ...
      omega
    calc yn two_lt_one L < 4 ^ L := yn_two_lt L
      _ ≤ (4 ^ (L + 1)) ^ (L - 1) := by
          rw [← pow_mul]
          exact Nat.pow_le_pow_right (by norm_num) hLL
      _ ≤ (2 * (a + 2) - 1) ^ (L - 1) := Nat.pow_le_pow_left (by omega) _
      _ ≤ ψ hA L := hψ
  have hmodΔ : (yn hA L : ℤ) ≡ yn two_lt_one L [ZMOD a] := yn_add_two_modEq a hA L
  obtain ⟨Δz, hΔz⟩ := (Int.modEq_iff_dvd.1 hmodΔ.symm)
  have hΔpos : 0 < Δz := by
    have h1 : (0 : ℤ) < (yn hA L : ℤ) - yn two_lt_one L := by
      have : (yn two_lt_one L : ℤ) < yn hA L := by rw [hκ] at hκT; exact_mod_cast hκT
      linarith
    rw [hΔz] at h1
  ...
  have hD : (a + 2) ^ 2 - 1 = a ^ 2 + 4 * a + 3 := sq_sub_one_eq a
  rw [hD] at E16'
  have E17 : (((a : ℤ) ^ 2 + 4 * a + 3) * (f ^ 2 - 1)) * ((2 * r + 1 + j * c) ^ 2 - (y : ℤ) ^ 2) =
      1 - (y : ℤ) ^ 2 := by
    have e : (((a + 2 : ℕ) : ℤ) ^ 2 - 1) = (a : ℤ) ^ 2 + 4 * a + 3 := by push_cast; ring
    have hJZ : (J : ℤ) = 2 * r + 1 := by rw [hJ]; push_cast; ring
    rw [e, hJZ] at E17'
    exact E17'
  have E17b : 2 * r + 1 + j * c = c + o * f := by rw [← hJ]; exact E17b'
  refine ⟨a, c, d, f, h, i, j, k, o, s, w, γ, η, τ, φ, κ, μ, ρ, Δ, ζ, y,
    ha0, hc0, hd0, hf0, hh0, hi0, hj0, hk0, ho0, hs0, hw0, hγ0, hη0, hτ0, hφ0, hκ0, hμ0, hρ0,
    hΔ0, hζ0, hy0, ?_, ?_, E10b, ?_, ?_, E13, ?_, E15, E16', E17, E17b, E18, E19, E20⟩
  · rw [← hU', ← hY']; exact E9
  · rw [← hY']; exact E10a
  · rw [← hU', ← hY']; exact E11
  · rw [← hU', ← hY']; exact ha
