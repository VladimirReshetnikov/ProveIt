...
\label{sec:changes}
Round~1 consisted of nine independent research reports, all of which chose C1. They are archived in \path{docs/coarse-degrees/research-reports/01}--\path{09}, and their deduplicated content is the synthesis \cite{Synthesis}. The outcome has three parts.

\emph{C1 is settled, and was not open.} Every nonuniform (and uniform) coarse class of a $1$-generic set, of the dyadic code $n\mapsto\emptyset''(\nu_2(n+1))$, and of an explicit many-one complete c.e. set lacks a least Turing degree. All nine reports found independently that this bare answer already follows from Hirschfeldt--Jockusch--Kuyper--Schupp \cite[Theorems 4.2 and 4.3]{HJKS}, with \cite[Theorem 2.19]{JS} for the dyadic code. The first edition's rating of C1 as ``B, O25'' was therefore wrong as a \emph{status}: the question was listed in a 2025 source, but it was not unresolved. Section~\ref{sec:stale} records the correction and Section~\ref{sec:plan} adds a protocol step designed to catch this failure mode earlier. The negative answer is also formalized, three times over, in the Lean~4 library \path{CoarseDegrees} at the repository root: through a $1$-generic set (admitting \cite[Theorem 4.2]{HJKS}), through a c.e.\ set (admitting \cite[Theorem 2.26]{JS} and \cite[Theorem 4.3]{HJKS}), and through the dyadic code (admitting one consequence of Cooper's theorem that every degree above $\zero'$ is the jump of a minimal degree). The three derivations share no admitted statement. The dyadic route needed the coarse-description criterion for $\R(A)$, which is proved there in limit form because neither Mathlib nor \path{C:\ProveIt} has a jump operator.
...
These are directions, not catalogue entries; none has been checked against the literature, and the status audit of Section~\ref{sec:plan} applies to each before any work. \emph{Update.} Report~10 \cite{ReportTen} settles (O1) and (O4), both negatively, and proves the observation (O2); the items are kept below as written, each followed by what was shown. Its proofs are conventional and unrefereed, and the proof for (O1) quotes standard facts about Cohen forcing over $L_{\omega_1^{\mathrm{CK}}}$ from memory. They are, however, partly formalized: the Lean~4 library \path{CoarseDegrees} at the repository root contains the budget forcing and the bridge lemma with nothing admitted, and the whole of the dyadic-code calibration of (O2) --- including the failure of cone-avoiding compactness for $\le_h$, which depends on no admitted statement at all.
\begin{enumerate}[label=\textup{(O\arabic*)},leftmargin=3em]
\item \emph{Hyperarithmetic analogues of the coarse track.} Replace $\leT$ by $\le_h$ in Section~\ref{sec:coarse}: does every coarse class have a representative of least \emph{hyperdegree}? The round-1 counterexamples evaporate at this level, which is what makes the question interesting. For the dyadic code, every description $B$ of $\R(A)$ satisfies $A\leT B'$ by (K4), hence $A\le_h B$, while $\R(A)\eqT A$ is itself a representative; so the class of $\R(A)$ has least hyperdegree $\deg_h(A)$, although it has no least Turing degree when $A\nleq_{\mathrm T}\emptyset'$. The $\emptyset'$-computable $1$-generic set and the guarded c.e. set of (K5) are hyperarithmetic, so their classes trivially have least hyperdegree $\zero_h$. A counterexample must therefore be a non-hyperarithmetic $X$ with no hyperarithmetic coarse description whose hyperarithmetic core $\{A:A\le_h D\text{ for every coarse description }D\text{ of }X\}$ is just the hyperarithmetic sets; the candidates are $\Sigma^1_1$-generic and $\Pi^1_1$-random reals. The category method of round~1 rests on the local decoder of \cite[Lemma 4.3]{Synthesis}, which uses the finite-use property of Turing functionals; $\le_h$ has no such property, so this is a real test of the method and not a relabelling. Gandy's basis theorem and Sacks's measure theorem are the natural replacements for the genericity and approximation arguments, and the proposal C4 has an obvious analogue for countable ideals of hyperdegrees. \emph{Settled negatively} \cite[Theorem 1.1]{ReportTen}: for every $Z$ and every disagreement budget there are $A\approx B$, each Cohen generic over $L_{\omega_1^Z}[Z]$, with $\Delta^1_1(Z\oplus A)\cap\Delta^1_1(Z\oplus B)=\Delta^1_1(Z)$; so there are coarse classes with no least hyperdegree, and every hyperdegree is an unattained infimum of a hyperdegree spectrum. The category method was not needed: the one-bit bridge lemma survives with ``the condition decides the sentence'' in place of ``the computation converges''. A caution recorded there, and worth repeating here, is that minimal pairs of hyperdegrees are themselves classical --- Feferman embeds every countable partial order into the hyperdegrees below $\degT(\mathcal O)$, and two distinct Gandy--Sacks minimal hyperdegrees form a minimal pair --- and that for the unrestricted product forcing the minimal-pair property of a mutually generic pair is a one-line argument. The content of the new theorem is the disagreement budget alone, which is what places the pair inside a single coarse class; a Cohen generic is at upper density $1$ from every hyperarithmetic set, so no classical construction does that. Report~10 also shows that the two methods act on the same space: a set lies within distance $r$ of another in the prefix-density metric of \cite{Synthesis} exactly when their difference obeys a \emph{density budget}, so the balls of that metric are conditions of the forcing. Budgets being closed under intersection, the pair can be required to be coarsely equal \emph{and} within distance $r$ for any prescribed $r>0$, so a hyperdegree minimal pair lies inside every such ball.

\emph{Whether the category method itself extends to $\le_h$} is question (Q1) of that report, and it is \emph{not} settled there; two routes are ruled out. The Turing proof cannot be repaired, because it runs through the local decoder whose conclusion is the robust-radius characterization, and that is false for $\le_h$. The obvious forcing proof is circular: one can build a pair inside a ball that dodges the closed nowhere dense sets covering the meager complement of the cone, so that both coordinates lie in the cone and the bridge lemma applies, but the ground model must contain the parameters defining those dense sets, and the cone is defined from the avoided set itself; the conclusion is then vacuous. This is the same circularity that makes the classical statement --- a set hyperarithmetic in comeager many reals is hyperarithmetic --- a theorem about $\Sigma^1_1$ sets rather than a genericity argument, and it is the reason (Q1) is harder than its Turing analogue. A transfer from Cantor space is also ruled out: the space of coarse descriptions is nowhere locally compact, so the image of a Cantor-space parametrization is nowhere dense.
\item \emph{Coarse information can force the hyperjump.} This is an observation rather than a question, recorded because it calibrates (O1). Since $\omega_1^X$ depends only on the degree, each coarse class has a least value $\min\{\omega_1^g:g\equiv_{\mathrm{nc}}f\}$. For $\R(\mathcal O)$ every representative computes a description $B$ with $\mathcal O\leT B'$ by (K4), so $\mathcal O\le_h g$ and $\omega_1^g>\omega_1^{\mathrm{CK}}$ by Spector's criterion: no representative is ``hyperarithmetically low'', although by (K4) a representative need not compute $\mathcal O$: it is enough that its jump does. Conversely the block code $J(X)$ of a real with $\omega_1^X=\alpha$ shows, with Sacks's realization theorem, that every countable admissible $\alpha>\omega$ is the least value of $\omega_1^g$ on some coarse class. So the ordinal invariant is nontrivial on coarse classes but is always attained; what it does not decide is whether a class can lack a least \emph{hyperdegree}, which is (O1). \emph{Proved} in \cite[Theorem 1.4]{ReportTen}, together with its consequence that cone-avoiding compactness and the $\gamma(X)=1$ theorem of \cite{HJKS} fail for $\le_h$. Parts (a) and (b) are formalized in \path{CoarseDegrees}, which sidesteps the missing jump operator by stating the criterion in limit form: $B$ computes a coarse description of $\R(A)$ if and only if $A$ is the limit of a $B$-computable approximation, which is (K4) read through Shoenfield's limit lemma. The hard direction is majority decoding over a column, and what makes it work is that a column has density $2^{-k-1}>0$ while the errors have density $0$, so the errors in a fixed column are eventually a minority. The failure of compactness is then explicit: the first $K$ bits of $A$, replicated along the columns below $K$, form a \emph{computable} set within $2^{-K}$ of $\R(A)$, while $A$ itself lies in the hyper-core.
...
\bibitem{Synthesis}
\emph{Coarse Classes Without Least Turing Degrees: Deduplicated results of the nine research reports on target C1}. Round-1 synthesis prepared for Vladimir Reshetnikov, 18 September 2026. \path{docs/coarse-degrees/research-synthesis/Turing_Degrees_Synthesis.tex}; the nine reports are in \path{docs/coarse-degrees/research-reports/01}--\path{09}. Conventional proofs; not refereed, not formalized, novelty unverified.

\bibitem{ReportTen}
\emph{Coarse Classes, Hyperdegrees, and the Cone Filter}. Research report 10, 18 September 2026. \path{docs/coarse-degrees/research-reports/10/coarse_hyperdegrees.tex}. Conventional proofs; not refereed, not formalized.
...
Jump inversion by minimal degrees: every degree above $\zero'$ is the jump of a minimal degree. Statement and page range checked 19 September 2026.
