#!/usr/bin/env python3
"""Independently verify the POISONED <4,6,9> certificate by exact finite games.
...
Conventions.  Every board handled here has its least element 0 deleted, so a
label 2 means "Grundy value at least 2" for the POISONED board.  The report's
own certificates/s469.json uses the unpoisoned convention (labels one larger,
saturation symbol 3) and orders the 17 gap ideals by ascending bit mask, while
this file orders them by cardinality and then lexicographically.  The function
cross_check_unpoisoned below asserts that the two certificates agree entry for
entry under the stated column permutation and the +1 shift.
"""
import argparse
import csv
...
def check_certificate(path: Path, output_dir: Path = None) -> Dict:
    ...
    if output_dir is not None:
        ...
        with (output_dir/"s469_poisoned_verification.json").open("w", encoding="utf-8", newline="\n") as handle:
            handle.write(json.dumps(result, indent=2)+"\n")
        with (output_dir/"s469_poisoned_boundary_values.csv").open("w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["x"]+["C"+str(i) for i in range(17)])
            writer.writerows([x]+list(exact[x]) for x in range(first, last+1))
        with (output_dir/"s469_poisoned_apery_values.csv").open("w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
...
# The unpoisoned companion certificate, certificates/s469.json, holds the same
# 253 rows over the same 17 gap ideals, but orders the ideals by ascending bit
# mask and records unpoisoned labels, which are one larger.  This function makes
# that dictionary machine-checked rather than a claim in the prose.
COLUMN_PERMUTATION = (0, 1, 2, 4, 3, 5, 7, 8, 6, 9, 10, 12, 11, 13, 14, 15, 16)


def cross_check_unpoisoned(poisoned: Dict, path: Path) -> int:
    """Check base column j <- poisoned column COLUMN_PERMUTATION[j], labels +1."""
    other = json.loads(path.read_text(encoding="utf-8"))
    require(other["generators"] == [4, 6, 9], "Companion certificate is for another semigroup")
    gaps = other["gaps"]
    ideals = [sorted(c) for c in poisoned["ideals"]]
    masks = other["pattern_masks"]
    derived = []
    for mask in masks:
        derived.append(ideals.index(sorted(g for i, g in enumerate(gaps) if mask >> i & 1)))
    require(tuple(derived) == COLUMN_PERMUTATION, "Column permutation is not the documented one")
    require((other["row_start"], other["row_end"])
            == (poisoned["row_first"], poisoned["row_last"]), "Row ranges differ")
    checked = 0
    for unpoisoned_row, poisoned_row in zip(other["rows"], poisoned["rows"]):
        for j, column in enumerate(COLUMN_PERMUTATION):
            require(int(unpoisoned_row[j]) == int(poisoned_row[column]) + 1,
                    "Unpoisoned and poisoned certificates disagree")
            checked += 1
    return checked
...
def main() -> None:
    ...
    parser.add_argument("certificate", nargs="?", type=Path, default=root/"certificates"/"s469_poisoned.json")
    parser.add_argument("--output-dir", type=Path, default=root/"data")
    parser.add_argument("--companion", type=Path,
                        default=root/"certificates"/"s469.json")
    args = parser.parse_args()
    result = check_certificate(args.certificate, args.output_dir)
    cells = cross_check_unpoisoned(json.loads(args.certificate.read_text(encoding="utf-8")),
                                   args.companion)
    print("VERIFIED")
    ...
    print("Cross-checked against the unpoisoned certificate:", cells, "cells")
