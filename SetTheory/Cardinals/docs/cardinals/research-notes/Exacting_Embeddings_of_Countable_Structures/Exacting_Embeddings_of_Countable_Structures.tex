...
\end{enumerate}
\textbf{Status.} Conventional proofs, not refereed. Items 1, 3 and 5, and Propositions~\ref{prop:interleave} and~\ref{prop:root}, are formalized in Lean over an arbitrary transitive set model (Section~\ref{sec:lean}), including the two clauses --- Theorems~\ref{thm:free}(4) and~\ref{thm:cofinal}(3) --- that the first version of this note left out; items 2 and 4 are not formalized, because they quantify over $L$. I searched for ``virtually exacting'' and found nothing, but exacting cardinals date from 2024 and I cannot exclude unpublished work. The results are modest: they are what one obtains by running the arguments of the synthesis through the Gitman--Schindler machinery, and their main value is to locate precisely where the Kunen inconsistency enters.
\end{assessment}
...
\toprule
\textbf{Statement}&\textbf{Lean}&\textbf{Status}\\\midrule\endfirsthead
\toprule
\textbf{Statement}&\textbf{Lean}&\textbf{Status}\\\midrule\endhead
...
Theorem~\ref{thm:free}(4)&\nolinkurl{relTo}, \nolinkurl{relTo_spec}, \nolinkurl{vA_fixed}, \nolinkurl{fixed_below_crit}, \nolinkurl{restriction_elementary}, \nolinkurl{restriction_moves_crit}&proved, with one further admitted input (below). The clause ``hence $\kappa$ is virtually rank-into-rank'' is the Gitman--Schindler \emph{definition} applied to the conclusion, and is not formalized\\
...
Theorem~\ref{thm:cofinal}(3)&\nolinkurl{no_short_cofinal_low_rank}, \nolinkurl{no_small_family_low_rank}, \nolinkurl{fixed_param}, \nolinkurl{least_ordinal_fixed}&proved for parameters of rank below $\lambda$ (which includes every ordinal $<\lambda$) together with $\lambda$ itself, using the same admitted input as Theorem~\ref{thm:free}(4) and nothing else. \nolinkurl{least_ordinal_fixed} supplies the device for a parameter that is a \emph{large} ordinal, and is itself free of admitted statements\\
...
Theorem~\ref{thm:sharp}(1)&\nolinkurl{SilverData}, \nolinkurl{SilverData.witness}, \nolinkurl{witness_crit}, \nolinkurl{witness_critSeq}, \nolinkurl{witness_cofinalType}, \nolinkurl{virtually_exacting_cofinal}, \nolinkurl{hasHighWitnesses}, \nolinkurl{crit_inaccessible_in_L}, \nolinkurl{no_short_cofinal_in_L}&derived from the interface below, nothing further admitted; \nolinkurl{no_short_cofinal_in_L} inherits the hierarchy input of Theorem~\ref{thm:free}(4) through Theorem~\ref{thm:cofinal}(3)\\
Theorem~\ref{thm:sharp}(2)&---&not derived: the interface is specialized to $\lambda=i_\omega$\\
Theorem~\ref{thm:sharp}(3)&\nolinkurl{crit_inaccessible_in_L}&only the mathematical content; the consistency statement is metamathematical\\
Theorem~\ref{thm:ordinal}(1)&\nolinkurl{HasGraphIn}, \nolinkurl{lExacting_absolute}&admitted: Shoenfield absoluteness, applied to the $\Sigma^1_2$ statement in question; the complexity computation is left to the reader\\
Theorem~\ref{thm:ordinal}(2)&\nolinkurl{StableData}, \nolinkurl{least_LExacting_lt_stable}&derived from the interface\\
Theorem~\ref{thm:ordinal}(4)&\nolinkurl{LExacting_of_silver}, \nolinkurl{LExacting}&derived from the interface\\
\bottomrule
\end{longtable}
\endgroup

\emph{The constructible hierarchy.} Theorems~\ref{thm:sharp} and~\ref{thm:ordinal} quantify over $L$, the Silver indiscernibles and the least stable ordinal, and neither Mathlib nor ProveIt has the constructible hierarchy, so their statements could not even be written down. \nolinkurl{Cardinals/Virtual/Constructible.lean} supplies the missing layer as an \emph{interface}: the structures \nolinkurl{SilverData} and \nolinkurl{StableData}, whose fields are precisely the published facts used by the proofs --- the levels $L_\alpha$ and their transitivity, the increasing continuous enumeration of the indiscernibles, the Skolem hulls with their elementary inclusions, Silver's theorem that shifting the indiscernibles induces an elementary embedding, the fact that everything of rank below $i_\omega$ lies in the hull, and $\Sigma_1$-reflection to the least stable ordinal --- each carrying its citation to Jech or Kanamori. Assuming a \nolinkurl{SilverData} is assuming $0^\#$; we take the machinery itself as the hypothesis rather than introducing an opaque constant for $0^\#$. The theorems are then \emph{derived}: assembling the witness from the hull and the shift, computing that its critical point is $i_m$ and its critical sequence $i_m,i_{m+1},\dots$, and checking cofinality in $i_\omega$. As a by-product the interface discharges the hypothesis \nolinkurl{HasHighWitnesses} of Section~\ref{sec:cofinal}, so Theorem~\ref{thm:cofinal}(3) applies verbatim at $i_\omega$ inside $L$ (\nolinkurl{no_short_cofinal_in_L}). Building $L$ and proving the fields of the interface is a separate project, and is what remains between this and a self-contained formalization.

Two devices made the last two clauses possible, and neither needs a satisfaction predicate inside $A$ --- which is what the first version of this note reported as the obstruction.

\emph{Relativization.} Elementarity of the restriction $j\restr V^A_\lambda$ is a statement about one formula at a time, so instead of a truth predicate it is enough to translate each formula: \nolinkurl{relTo k φ} restricts every quantifier of $\varphi$ to the variable $v_k$, and \nolinkurl{relTo_spec} proves that $A\models(\mathtt{relTo}\ k\ \varphi)[\vec x]$ iff $(x_k,\in)\models\varphi[\vec x]$. Combined with the fixedness of the stage $V^A_\lambda$ this gives Theorem~\ref{thm:free}(4). The one admitted ingredient is that the cumulative hierarchy is definable in a transitive model of $\ZF$, so that $V^A_\rho$ is the unique solution of a formula with parameter $\rho$; it is a textbook fact (Jech, Chapters~6 and~12).

\emph{Making parameters fixed.} Theorem~\ref{thm:cofinal}(3) needs the defining data of a counterexample to be fixed by $j$. Two mechanisms do this, and the Lean development has both. First, \nolinkurl{fixed_below_crit}: $j$ fixes \emph{every} set of rank below $\crit(j)$ --- proved by induction on the rank, using the fixed stages again, and false at $\crit(j)$ itself. So a parameter becomes fixed as soon as the witness is chosen after it, which is the hypothesis \nolinkurl{HasHighWitnesses} (for genuine ultraexacting cardinals, \cite[Lemma~3.2]{abl}; for virtual ones, part of the hypothesis, exactly as in this note). This is the minimal-rank-parameter device of report~R18 in the synthesis, and it yields the theorem for all parameters of rank below $\lambda$. Secondly, \nolinkurl{least_ordinal_fixed}: if $\varphi$, with parameters already fixed, is satisfied in $A$ only by ordinals, then the \emph{least} ordinal satisfying it is fixed --- however large, and although $j$ moves cofinally many ordinals. The formula \nolinkurl{leastF φ} expressing ``$v_0$ satisfies $\varphi$ and no element of $v_0$ does'' is built from $\varphi$ at the meta-level, so again no truth predicate appears. That is the least-counterexample device of the proof of Theorem~\ref{thm:cofinal}(3) in isolation. What is \emph{not} carried out is the bookkeeping that iterates it over the finitely many ordinal parameters and the defining rank of an arbitrary $\OD_{V_\lambda}$ definition; that step is routine and adds nothing mathematical.

The axiom audit (\nolinkurl{Cardinals/Audit.lean}) records exactly which admitted statements each theorem uses, and the picture is the following. Theorem~\ref{thm:free}(1)--(3), Theorem~\ref{thm:cofinal}(2), Lemma~\ref{lem:abs}, Propositions~\ref{prop:root} and~\ref{prop:interleave}, and the two devices \nolinkurl{relTo_spec} and \nolinkurl{least_ordinal_fixed} depend only on \nolinkurl{propext}, \nolinkurl{Classical.choice} and \nolinkurl{Quot.sound} --- on no admitted statement, and in particular on no form of the Kunen inconsistency. That is the precise content of the claim that the core of Sections~\ref{sec:free} and~\ref{sec:cofinal} is Kunen-free: it is not an inspection of the paper proofs but a property of the machine-checked ones. Theorem~\ref{thm:free}(4) and Theorem~\ref{thm:cofinal}(3) depend, through \nolinkurl{fixed_below_crit}, on the definability of the cumulative hierarchy, and on nothing else; \nolinkurl{restriction_elementary} taken by itself, with the fixedness of $V^A_\lambda$ as a hypothesis, does not. Proposition~\ref{prop:ultra} depends on the admitted in-model Kunen inconsistency, as it must.
