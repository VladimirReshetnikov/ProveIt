...
$x$-th powers are rational, the exclusion of algebraic outputs at independent natural
auxiliary bases, and the equivalence with both fractional-part gaps and a localized-radical
...
classifications, the number-field determinant theorem for algebraic third outputs,
...
injectivity, and the uniform finite-difference API now have
...
The main theorem and its corollaries are in \lean{AlgebraicThirdBase} and
\lean{AlgebraicConsequences}, respectively.
...
``algebraic''; outside the $2,3$-smooth monoid the output is transcendental.  In particular,
at least one of $2^{x^2},3^{x^2}$ is transcendental, and in fact $6^{x^2}$ is
transcendental.  \statuslean
...
\[
 ...
 \qquad
 \operatorname{Trans}_{\Q}(6^{x^2}).
\]
For the second assertion, write $M=2^x$, $B=3^x$.  If
$(MB)^x=6^{x^2}$ were algebraic, eqref{eq:natural-algebraic-output} would make $MB$
$2,3$-smooth; its positive divisors $M$ and $B$ would then both be $2,3$-smooth, forcing
both first iterates to be algebraic, contrary to the simultaneous-core obstruction.
...
\]
Notice that the classified quantity is $(E^d)^\beta$, not $E^\beta$.
...
and characterizes exactly when $(E^d)^\beta$ is rational in terms of the reduced numerator, while
...
same interpolation margin.  The next extension is to algebraic outputs at arbitrary positive
rational bases, and ultimately to a fully algebraic-base sharp six-exponentials interface.
...
rational-output determinants under monomial injectivity, the exact rational-base and
canonical-core classifications, and the uniform
higher-difference obstruction are kernel-verified.  Full blockwise equidistribution for
arbitrary continuous tests and intervals remains a paper theorem; algebraic (rather than
rational) third outputs remain beyond the determinant lower bound currently formalized.
