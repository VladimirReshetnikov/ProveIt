...
exclusion.  Exact unit-block counts and their cumulative finite sum are kernel-verified;
blockwise Weyl equidistribution and the cumulative asymptotic are proved here at the paper
level.  An independent interval computation excludes all nonintegral candidates with
...
an exact conditional solution monoid with least generator, sharp arbitrary-order
...
$\Nzero+\Nzero\beta$; exact dyadic-block counts and their cumulative finite sum are
kernel-verified, while blockwise Weyl equidistribution and the cumulative asymptotic remain
paper corollaries.  Third, it records the now kernel-verified general higher-difference
...
The current feature wave adds directly kernel-checked structure modules
\lean{PrimitiveOddCore}, \lean{LeastSolution}, \lean{ThreeDenominatorNormalization},
\lean{RationalPowerIndex}, \lean{PrimitiveGenerator}, and \lean{ExactCounting}.
...
$\lfloor(N+1)/\beta\rfloor$ and cumulative finite sum are kernel-verified.  \statuslean{}
Blockwise Weyl equidistribution and the cumulative asymptotic are paper corollaries.
\statusnew
...
core, least-solution primitivity, power-of-three denominator normalization, rational-power
index and exact primitive generator, exact conditional counting, primitive localized-radical
reduction and exact radical degree, and uniform higher finite differences. \\
\bottomrule
\end{longtable}
...
\statuscomp{} level by design.  The later feature modules listed in the final row are
distinguished from the still-paper-level cumulative asymptotic and blockwise
equidistribution throughout this report; exact counts, radical irreducibility, and uniform
higher differences have subsequently been kernel-verified.
...
exact structure of the next section yields the paper-level strengthening to blockwise Weyl
...
solution, affine primitivity, exact free-monoid classification, exact unit-block counts, and
cumulative finite sum are now \statuslean{}.  The cumulative asymptotic and blockwise Weyl
equidistribution deduced from that classification remain \statusnew{}.
...
\lean{cumulativeTwoBaseCandidateCount_eq}.  The final $T^2/(2\beta)+O(T)$ asymptotic in
\eqref{eq:exact-cumulative} remains a paper corollary.}
\end{statusbox}
...
noninteger solution and affine primitivity, rational-power index, denominator support, and
exact primitive-generator theorem are now complete in \lean{PrimitiveOddCore},
\lean{LeastSolution}, \lean{ThreeDenominatorNormalization}, \lean{RationalPowerIndex}, and
\lean{PrimitiveGenerator}.  \lean{ExactCounting} now proves the exact unit-block and dyadic
block counts and their cumulative finite sum.  The remaining work in this priority is the
$T^2/(2\beta)+O(T)$ asymptotic and blockwise Weyl equidistribution in
Theorem~\ref{thm:block-equidist}; a plausible module is
\lean{FractionalPartEquidistribution}.  This supersedes the earlier
...
The free-monoid classification, exact block counts and cumulative finite sum, primitive
localized-radical reduction, exact radical degree, and uniform higher-difference obstruction
are kernel-verified.  The cumulative asymptotic and blockwise-equidistribution consequence
remain paper theorems.
...
Cumulative $T^2/(2\beta)+O(T)$ asymptotic & \statusnew &
Equation~\eqref{eq:exact-cumulative} \\
