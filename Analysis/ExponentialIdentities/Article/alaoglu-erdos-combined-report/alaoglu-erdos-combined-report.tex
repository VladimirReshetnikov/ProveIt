...
$\lfloor(N+1)/\beta\rfloor$, and blockwise Weyl equidistribution.  \statusnew
...
$u>1$ satisfies $u^{\thetaq}\in\Zthree$.  \statusnew{} (equivalence); the statement itself is
open even for $u=3$.
\end{enumerate}
...
This section develops, at the \statusnew{} level, the exact rigidity theory of a
...
\begin{statusbox}{\statusnew{} --- the principal structural result of this report.  It
...
\begin{statusbox}{\statusnew{} --- a compact target for future work: prove
...
Formalize the primitive normalization of the odd core, the rational-power index, and the
primitive-generator theorem, with exact counting and blockwise equidistribution as
corollaries.  A plausible module split:
\lean{OddCore} (extend with primitivity and uniqueness),
\lean{RationalPowerIndex}, \lean{PrimitiveGenerator}, \lean{ExactCounting},
\lean{FractionalPartEquidistribution}.  Engineering pressure points: gcd and B\'ezout over
a \lean{Finsupp} support; identifying $\{j:E^j\in\Q\}$ with $d\Z$; denominator-support
lemmas over \lean{Rat}; and cast bookkeeping among $\N,\Z,\Q,\R$ powers.  No new
transcendence theory is required.  This supersedes the earlier ``exponent semigroup''
direction: there are no Frobenius-type gaps to study.

\subsection*{Priority B: formalize the sharper finite differences}
Formalize \eqref{eq:finite-diff-integral} as a reusable identity, then
Theorem~\ref{thm:higher-difference}; replace $h^5\le n$ by the sharp \eqref{eq:sharp-3ap} in the
density pipeline.  Schematic signature:
...
$1048576$ analogue of Theorem~ef{thm:finite4096}.  Every descent constant then improves to $20$
by bookkeeping.
...
Exact solution monoid $\Nzero+\Nzero\beta$ & \statusnew & Theorem~\ref{thm:generator} \\
...
Sharp and higher-order AP obstructions & \statusnew & Theorem~\ref{thm:higher-difference} \\
Localization and radical reformulations & \statusnew &
Theorem~\ref{thm:localization}, Proposition~\ref{prop:radical-degree} \\
No nonintegral candidate with $2^x<2^{20}$ & \statuscomp & Proposition~ef{prop:finite20} \\
