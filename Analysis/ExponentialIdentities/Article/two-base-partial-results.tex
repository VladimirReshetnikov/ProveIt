...
\begin{remark}[Temporary note on verification status]\label{rem:pending}
At the time this revision was typeset, the Lean kernel replay of the $4096$-certificate
table was still running; Theorem~\ref{thm:finite} and each downstream use of the numeral
$12$ should be read as pending final acceptance by Lean. The certificate table has already
passed two independent exact validations (PARI/GP and Python big-integer arithmetic), and
the previously accepted region $2^x<256$ (hence $x\ge 8$) is unaffected. This remark will
be removed once the build completes.
\end{remark}
\end{theorem}
