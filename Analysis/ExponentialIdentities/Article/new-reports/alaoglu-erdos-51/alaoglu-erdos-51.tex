...
\large A nonredundant continuation toward the \AEname\ conjecture}
...
the consecutive pure-axis family is factored in closed form.  Its sign hierarchy is automatic
and therefore does not test compatibility: genuinely mixed minors are indispensable.  Third,
scalar Stieltjes divided differences are lifted to positive
...
No unconditional proof of the \AEname\ conjecture is asserted.
\end{minipage}}
\end{center}
...
Let $k\ge1$.  Choose distinct exponent vectors $e_0,\ldots,e_k\in\N_0^2$ and order them so that
...
Consequently this consecutive pure-axis sign hierarchy cannot test the compatibility of
$A^\theta=2$ with $B^\theta=3$.
\end{theorem}
...
After clearing the single factor $\Delta(\UU)$, the leading consecutive Hankel determinants
used in \cref{thm:momentlattice} can be computed in either of two equivalent exact ways,
with the Laurent expansion taken at infinity and the corresponding reversed-polynomial
...
Here $\deg Q_m=2m$, so an application of the moment lattice requires $d\ge2m$ and
$n\ge2d+1$; choosing $d=2m$ requires $n\ge4m+1$.
...
Any argument deriving the algebraicity of $5^x$ from $2^x,3^x\in\Z$ proves the \AEname\
conjecture.  The same holds with $5$ replaced by any positive algebraic number $c\ne1$ whose
logarithm is $\Q$-independent of $\log2$ and $\log3$.
\end{criterion}
...
would likely need one of the output-specific cancellations isolated in
\cref{thm:axis,cor:L2lattice,prop:pade}.
...
\subsection{\texorpdfstring{$p$}{p}-adic automatic continuity}
...
The strongest negative advance is \cref{thm:axis}: the consecutive pure-axis sign hierarchy
is automatic and cannot settle compatibility.  Together these facts focus subsequent effort
