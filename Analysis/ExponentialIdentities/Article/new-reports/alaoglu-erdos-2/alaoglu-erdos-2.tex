...
\hypersetup{
  ...
  citecolor=newgreen,
  urlcolor=leanblue,
  pdftitle={The Alaoglu--Erdos Integer-Exponent Conjecture: Unified Report and 2026 Continuation},
  pdfauthor={The ProveIt project},
  pdfsubject={Machine-checked partial results, matrix coefficients, sparse interpolation,
critical counting thresholds, and the August 2026 research frontier}
}
\urlstyle{same}
...
\vspace{0.12cm}
{\LARGE Matrix Coefficients, Sparse Interpolation, and the 2026 Frontier\par}
\vspace{1.0cm}
...
At paper level the report develops two determinant methods, a prime-log-product criterion,
and now a third continuation organized around the 2026 Matrix Coefficient frontier.  It proves
that the conjecture is exactly a restricted $2\times2$ Matrix Coefficient statement; that a
counterexample would give a nonzero irreducible sparse quadratic in prime-log symbols which
vanishes at the actual prime logarithms; and that its semigroup points have maximal possible
sparse interpolation complexity.  On a box of $4N^2$ points the least support of a nonzero
vanishing polynomial would be exactly $4N^2+1$, so a one-monomial saving over trivial
interpolation would already prove the conjecture.  The continuation also derives the sharp
point-counting thresholds $o((\log H)^2)$ on the unbounded power curve and $o(\log H)$ on the
compact synchronized arc, a six-exponentials monodromy consequence, and a literature audit
through August 21, 2026.  No unconditional proof is claimed.
\end{minipage}
\vfill
...
formulation, and the prime-log-product independence criterion.  Fifth, the August 2026
continuation gives an exact restricted Matrix Coefficient equivalence, an irreducible
prime-symbol determinant attached to every hypothetical counterexample, quantitative
rank-one anisotropy at the critical Dirichlet exponent, an exact sparse-support dichotomy for
the counterexample semigroup, the sharp rational-point thresholds needed from current
polylogarithmic counting technology, a six-exponentials monodromy consequence, and a detailed
audit of literature from 2023 through August 2026.  It closes with a prioritized research
program and a formalization blueprint, while explicitly recording that the full conjecture
remains unresolved.
\end{abstract}
...
The third continuation, Section~\ref{sec:third-continuation}, sharpens the endpoint in four
ways.  It identifies the conjecture with a restricted form of the Matrix Coefficient
Conjecture for the singular logarithm matrix
$\bigl(\begin{smallmatrix}\log2&\log A\\\log3&\log B\end{smallmatrix}\bigr)$; packages a
counterexample as a nonzero irreducible quadratic in formal prime-log variables; proves that
the associated $4N^2$ semigroup points require exactly $4N^2+1$ monomials in any nonzero
vanishing polynomial; and computes the exact point-counting exponents which a future
Pfaffian or denominator-sensitive theorem must beat.  The same section specializes the
published Dasgupta--Kakde fewnomial program, audits the 2023--2026 literature (including
recent arXiv work), and isolates a one-monomial sparse-compression target which is weaker than
the general Matrix Coefficient auxiliary-polynomial conjecture but still sufficient here.
...
{\scriptsize
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.26\textwidth}>{\raggedright\arraybackslash}p{0.35\textwidth}>{\raggedright\arraybackslash}p{0.29\textwidth}@{}}
\toprule
...
\clearpage
\section{Third continuation: matrix coefficients, sparse interpolation, and the August 2026 frontier}
\label{sec:third-continuation}

\subsection{Scope, claim ledger, and the honest endpoint}
\label{third:scope}

This continuation was written after a fresh search of the literature through August 21,
2026, with special attention to arXiv, current journal metadata, and statements which could
conceivably cross the final $2\times2$ transcendence barrier.  The most relevant development
is the Matrix Coefficient Conjecture of Dasgupta and Kakde, now published in
\emph{Advances in Mathematics}~\cite{DasguptaKakde2026}.  In dimension two that conjecture is
equivalent to the four exponentials conjecture.  Its fewnomial strategy turns out to fit the
Alaoglu--Erd\H{o}s problem exceptionally well, because a hypothetical counterexample gives a
rank-one logarithm matrix whose two kernel slopes are both irrational.

The outcome is not an unconditional proof.  It is, however, considerably sharper than the
statement ``the problem reduces to four exponentials.''  We obtain three exact finite or
algebraic avatars of the missing step:
\begin{enumerate}
\item a restricted Matrix Coefficient assertion which is \emph{equivalent} to the original
conjecture;
\item a nonzero irreducible homogeneous quadratic in formal prime-logarithm variables which
would vanish at the actual prime logarithms;
\item a maximal sparse-interpolation phenomenon: on every finite counterexample orbit of
$R$ points, a nonzero vanishing polynomial needs exactly $R+1$ monomials.
\end{enumerate}
The last item yields a particularly concrete target.  Trivial univariate interpolation uses
$R+1$ monomials.  Saving even \emph{one} monomial, by any arithmetic construction, would rule
out the counterexample.

\begin{statusbox}{No unconditional proof of Conjecture~\ref{conj:AE} is claimed.}
Statements proved below are marked \statusnew{}; quoted transcendence, point-counting, and
Matrix Coefficient results are marked \statusknown{}; and all sufficient hypotheses whose
missing implication is open are marked \statusopen{}.  The purpose is to replace a vague
frontier by exact equivalences, exact exponents, and auditable finite targets.
\end{statusbox}
...
\toprule
Statement & Status & Role in the continuation \\
\midrule
Restricted Matrix Coefficient principle $\Longleftrightarrow$ Alaoglu--Erd\H{o}s
  & \statusnew & Exact reformulation of the conjecture. \\
Formal prime-symbol determinant and its vanishing classification
  & \statusnew & Sparse Structural Rank avatar. \\
Irreducibility of the counterexample quadratic
  & \statusnew & Excludes factorization into known Baker-type linear relations. \\
Effective polynomial repulsion for rational matrix coefficients
  & \statusknown{} + \statusnew & Baker--W\"ustholz/Matveev specialized to this rank-one matrix. \\
Quadratic Dirichlet approach scale $O(Q^{-2})$
  & \statusnew & Identifies the critical exponent for lower-bound methods. \\
Exact support minimum $R+1$ on $R$ counterexample points
  & \statusnew & Sharp sparse-interpolation obstruction and one-term target. \\
Dasgupta--Kakde condition $(m')$ specialized to the semigroup
  & \statusknown{} + \statusopen & Most focused modern auxiliary-polynomial route. \\
Unbounded point-counting threshold $o((\log H)^2)$
  & \statusnew & Exact exponent required from rational-point counting. \\
Compact synchronized threshold $o(\log H)$
  & \statusnew & Denominator-sensitive version on a fixed compact arc. \\
Six-exponentials monodromy consequence
  & \statusnew & New unconditional property forced by any counterexample. \\
Full conjecture
  & Open & None of the audited literature supplies the missing implication. \\
\bottomrule
\end{tabularx}
\end{center}

\subsection{The singular logarithm matrix}
\label{third:matrix}
...
\begin{equation}
 ...
 \begin{pmatrix}
  ...
  \log 3 & \log B
 \end{pmatrix}.
 \label{third:eq-logmatrix}
\end{equation}
The determinant condition
\begin{equation}
 \det \mathsf L(A,B)=0
 \label{third:eq-singular}
\end{equation}
is equivalent to the existence of a common real exponent
\begin{equation}
 \beta=\frac{\log A}{\log2}=\frac{\log B}{\log3},
 \qquad A=2^\beta,
 \qquad B=3^\beta.
 \label{third:eq-beta}
\end{equation}
Because $A$ and $B$ are positive integers, $\beta\ge0$.  We repeatedly use the elementary
rational-exponent lemma already present in the formal corpus.

\begin{lemma}[Rational exponents are integral]
\label{third:lem-rational-integral}
If $\beta\in\Q$ and $2^\beta\in\Z$, then $\beta\in\Z$.  Consequently, under
\eqref{third:eq-beta}, either $\beta\in\Z$ or $\beta\notin\Q$.
\end{lemma}

\begin{proof}
Write $\beta=r/s$ in lowest terms with $s>0$.  From $2^{r/s}=A\in\Z$ one obtains
$2^r=A^s$.  Unique factorization forces $A$ to be a power of two, say $A=2^m$, and then
$r=ms$.  Since $(r,s)=1$, this gives $s=1$.
\end{proof}

When \eqref{third:eq-singular} holds, the matrix factors as
\begin{equation}
 \mathsf L(A,B)=
 \binom{\log2}{\log3}
 \begin{pmatrix}1&\beta\end{pmatrix}.
 \label{third:eq-rankonefactor}
\end{equation}
Thus a nonintegral solution would give a real rank-one matrix with an irrational left kernel
slope and an irrational right kernel slope.  More explicitly, its real left and right kernels
are generated by
\begin{equation}
 \binom{\log3}{-\log2}
 \quad\text{and}\quad
 \binom{-\beta}{1},
 \label{third:eq-kernels}
\end{equation}
respectively.  Neither line contains a nonzero rational point.

\subsection{An exact restricted Matrix Coefficient equivalence}
\label{third:mcc}

Dasgupta and Kakde formulate the Matrix Coefficient Conjecture as follows: if a square matrix
of logarithms of algebraic numbers is singular, then there are nonzero rational vectors on the
left and right whose matrix coefficient vanishes~\cite{DasguptaKakde2026}.  We isolate only
the matrices \eqref{third:eq-logmatrix} arising from positive integers.

\begin{definition}[AE-restricted Matrix Coefficient principle]
\label{third:def-rmcc}
The principle $\mathrm{MCC}_{2,3}^{\Z}$ is the assertion that, for every $A,B\in\N$ with
$\det\mathsf L(A,B)=0$, there exist nonzero vectors $u,v\in\Q^2$ such that
\begin{equation}
 u^{\mathsf T}\mathsf L(A,B)v=0.
 \label{third:eq-mcccoefficient}
\end{equation}
\end{definition}

\begin{theorem}[Restricted Matrix Coefficient equivalence]
\label{third:thm-rmcc-equivalence}
Conjecture~\ref{conj:AE} is equivalent to $\mathrm{MCC}_{2,3}^{\Z}$.
\end{theorem}

\begin{proof}
Assume Conjecture~\ref{conj:AE}.  If $\det\mathsf L(A,B)=0$, define $\beta$ by
\eqref{third:eq-beta}.  Then $2^\beta=A$ and $3^\beta=B$ are integers, so the conjecture gives
$\beta=m\in\Z$.  Taking $v=(-m,1)^{\mathsf T}$ gives $\mathsf L(A,B)v=0$, and any nonzero
$u\in\Q^2$ proves \eqref{third:eq-mcccoefficient}.

Conversely, assume $\mathrm{MCC}_{2,3}^{\Z}$ and suppose that $\beta$ satisfies
$2^\beta=A\in\Z$ and $3^\beta=B\in\Z$.  If $\beta\notin\Z$, then
Lemma~\ref{third:lem-rational-integral} gives $\beta\notin\Q$.  For nonzero
$u=(u_1,u_2)^{\mathsf T}$ and $v=(v_1,v_2)^{\mathsf T}$ in $\Q^2$, the factorization
\eqref{third:eq-rankonefactor} gives
\begin{equation}
 u^{\mathsf T}\mathsf L(A,B)v
 =\bigl(u_1\log2+u_2\log3\bigr)\bigl(v_1+\beta v_2\bigr).
 \label{third:eq-coefffactor}
\end{equation}
The first factor is nonzero because $2$ and $3$ are multiplicatively independent.  The second
is nonzero because $\beta$ is irrational.  Hence every nonzero rational rank-one matrix
coefficient is nonzero, contradicting $\mathrm{MCC}_{2,3}^{\Z}$.
\end{proof}

\begin{corollary}[Rational rank-one anisotropy]
\label{third:cor-anisotropy}
A hypothetical counterexample produces a singular matrix $\mathsf L(A,B)$ satisfying
\begin{equation}
 u^{\mathsf T}\mathsf L(A,B)v\ne0
 \qquad
 \text{for all }u,v\in\Q^2\setminus\{0\}.
 \label{third:eq-anisotropy}
\end{equation}
Conversely, any singular matrix of the form \eqref{third:eq-logmatrix} satisfying
\eqref{third:eq-anisotropy} comes from a nonintegral counterexample.
\end{corollary}

This formulation is more precise than saying merely that the conjecture is ``a case of four
exponentials.''  The matrix is not generic: its first column is fixed, its second column is
made from logarithms of integers, all entries are real and positive, and the formal prime
support discussed below has nonnegative integral coefficients.  A proof may exploit any of
these restrictions without resolving the full Matrix Coefficient Conjecture.

\subsection{The formal prime-symbol determinant}
\label{third:formal-determinant}

Let
\[
 S=\{2,3\}\cup\supp(A)\cup\supp(B),
 \qquad
 a_p=v_p(A),
 \qquad
 b_p=v_p(B).
\]
Introduce algebraically independent variables $X_p$ for $p\in S$ and define
\begin{equation}
 \mathcal L_{A,B}(\mathbf X)=
 \begin{pmatrix}
  X_2 & \displaystyle\sum_{p\in S}a_pX_p\\[0.5em]
  X_3 & \displaystyle\sum_{p\in S}b_pX_p
 \end{pmatrix}.
 \label{third:eq-formalmatrix}
\end{equation}
Its determinant is the sparse homogeneous quadratic
\begin{equation}
 \Phi_{A,B}(\mathbf X)
 =X_2\sum_{p\in S}b_pX_p-X_3\sum_{p\in S}a_pX_p.
 \label{third:eq-phi}
\end{equation}
Substitution $X_p=\log p$ gives
\begin{equation}
 \Phi_{A,B}\bigl((\log p)_{p\in S}\bigr)=\det\mathsf L(A,B).
 \label{third:eq-phispecialization}
\end{equation}

\begin{lemma}[Formal vanishing classification]
\label{third:lem-formalzero}
The polynomial $\Phi_{A,B}$ is identically zero if and only if
\begin{equation}
 A=2^m,
 \qquad
 B=3^m
 \label{third:eq-formaltrivial}
\end{equation}
for some $m\in\Nzero$.
\end{lemma}

\begin{proof}
Expanding by monomial type gives
\begin{align*}
 \Phi_{A,B}
 &=b_2X_2^2+(b_3-a_2)X_2X_3-a_3X_3^2\\
 &\quad+\sum_{p\in S\setminus\{2,3\}}b_pX_2X_p
       -\sum_{p\in S\setminus\{2,3\}}a_pX_3X_p.
\end{align*}
If this polynomial is zero, coefficient comparison yields
\[
 b_2=0,
 \quad a_3=0,
 \quad b_p=0\ (p\ne3),
 ...
 \quad b_3=a_2.
\]
Writing the common value as $m$ gives \eqref{third:eq-formaltrivial}.  The converse is immediate.
\end{proof}

\begin{theorem}[Sparse Structural Rank formulation]
\label{third:thm-sparse-structural}
Conjecture~\ref{conj:AE} is equivalent to the following special prime-logarithm statement:
for all positive integers $A,B$,
\begin{equation}
 \Phi_{A,B}\bigl((\log p)_{p\in S}\bigr)=0
 \quad\Longrightarrow\quad
 \Phi_{A,B}\equiv0.
 \label{third:eq-sparseSR}
\end{equation}
Equivalently, no nonzero polynomial of the signed sparse form
\eqref{third:eq-phi}, with valuation coefficients in $\Nzero$, vanishes at the prime-logarithm
point.
\end{theorem}

\begin{proof}
By \eqref{third:eq-phispecialization}, the hypothesis is exactly
$\det\mathsf L(A,B)=0$, hence gives a common exponent $\beta$ as in
\eqref{third:eq-beta}.  If the conjecture holds, $\beta=m\in\Z$ and
Lemma~\ref{third:lem-formalzero} gives $\Phi_{A,B}\equiv0$.  Conversely,
\eqref{third:eq-sparseSR} and Lemma~\ref{third:lem-formalzero} force
$A=2^m,B=3^m$, so the common exponent is $m$.
\end{proof}

Thus a counterexample is an exceptionally small failure of the Structural Rank principle of
Dasgupta~\cite{Dasgupta2023}: the formal matrix \eqref{third:eq-formalmatrix} has rank two over
$\Q(\mathbf X)$, while its specialization at $X_p=\log p$ has rank one.  The failure is
encoded by a homogeneous quadratic with two signed stars of monomials, one centered at $X_2$
and one at $X_3$.

\begin{proposition}[Irreducibility of the counterexample quadratic]
\label{third:prop-irreducible}
If $(A,B)$ comes from a nonintegral counterexample, then the nonzero polynomial
$\Phi_{A,B}$ is irreducible in $\Q[X_p:p\in S]$.
\end{proposition}

\begin{proof}
A nontrivial factorization of a nonzero homogeneous quadratic over $\Q$ is a product of two
nonzero homogeneous rational linear forms.  Evaluating such a factorization at
$X_p=\log p$ and using \eqref{third:eq-phispecialization}, one of the two linear forms would
vanish at the prime-logarithm point.  This would give a nontrivial rational relation
\[
 \sum_{p\in S}c_p\log p=0.
\]
After clearing denominators and exponentiating, one obtains a nontrivial multiplicative
relation among distinct primes, contrary to unique factorization.
\end{proof}

The proposition explains why ordinary linear-forms-in-logarithms technology does not split the
problem into easier pieces.  The quadratic relation is not a product of two hidden rational
linear relations; its rank drop is genuinely bilinear.

\begin{remark}[How special the missing algebraic independence is]
The prime-log-product criterion of Section~\ref{sec:logproduct} assumes linear independence of
a broad family of products $\log p\log q$.  Theorem~\ref{third:thm-sparse-structural} needs far
less: only nonvanishing for quadratics of the signed valuation form
$X_2L_B-X_3L_A$.  A direct proof for this cone of sparse polynomials would settle the
Alaoglu--Erd\H{o}s conjecture without establishing general algebraic independence of logarithm
products.
\end{remark}

\subsection{Quantitative rational matrix coefficients}
\label{third:quantitative-mcc}

The qualitative anisotropy \eqref{third:eq-anisotropy} admits two useful quantitative
descriptions.  First, every integral matrix coefficient is itself a logarithm of a positive rational
number:
\begin{equation}
 \begin{split}
 u^{\mathsf T}\mathsf L(A,B)v
 &=u_1v_1\log2+u_1v_2\log A+u_2v_1\log3+u_2v_2\log B\\
 &=\log\!\left(
  2^{u_1v_1}A^{u_1v_2}3^{u_2v_1}B^{u_2v_2}
 \right)
 \end{split}
 \label{third:eq-logrational}
\end{equation}
for $u,v\in\Z^2$.  Under a counterexample, the rational number in parentheses is never one
when $u$ and $v$ are both nonzero.

Second, the rank-one factorization separates this logarithm into two classical linear forms:
\begin{equation}
 u^{\mathsf T}\mathsf L(A,B)v
 =\frac{(u_1\log2+u_2\log3)(v_1\log2+v_2\log A)}{\log2}.
 \label{third:eq-productlinearforms}
\end{equation}
The numbers $2,3$ are multiplicatively independent.  In a counterexample, $2$ and $A$ are
also multiplicatively independent, since multiplicative dependence would force $A$ to be a
power of two and $\beta$ to be integral.

For $z=(z_1,z_2)\in\Z^2$, set
\[
 H(z)=\max\{3,|z_1|,|z_2|\}.
\]

\begin{proposition}[Effective Baker repulsion]
\label{third:prop-baker-repulsion}
Fix a hypothetical counterexample $(A,B)$.  There are effectively computable constants
$c>0$ and $C_1,C_2>0$, depending only on $A$ and the fixed bases, such that
\begin{equation}
 \bigl|u^{\mathsf T}\mathsf L(A,B)v\bigr|
 \ge c\,H(u)^{-C_1}H(v)^{-C_2}
 \label{third:eq-bakerrepulsion}
\end{equation}
for all nonzero $u,v\in\Z^2$.
\end{proposition}

\begin{proof}
Both factors in \eqref{third:eq-productlinearforms} are nonzero linear forms in logarithms of
fixed positive algebraic numbers.  The Baker--W\"ustholz theorem, or Matveev's explicit form,
gives effective lower bounds polynomial in the heights of the integer coefficient vectors;
see \cite{BakerWustholz1993,Matveev2000}.  Multiplying the two bounds and dividing by the fixed
number $\log2$ gives \eqref{third:eq-bakerrepulsion}.
\end{proof}

The elementary estimate obtained only from \eqref{third:eq-logrational} is far weaker.  If
$H(u),H(v)\le Q$, numerator and denominator heights of the rational number in
\eqref{third:eq-logrational} are at most $\exp(O_{A,B}(Q^2))$, yielding only
$\exp(-O_{A,B}(Q^2))$ as a lower bound for a nonzero logarithm.  Baker theory upgrades this to
a power of $Q$, but its exponent is not close to the critical value described next.

\begin{proposition}[Double-Dirichlet upper scale]
\label{third:prop-dirichlet}
Let $(A,B)$ be a hypothetical counterexample, with
$\alpha=\log3/\log2$ and $\beta$ as in \eqref{third:eq-beta}.  There is a constant $C>0$ such
that for every integer $Q\ge2$ one can find nonzero $u,v\in\Z^2$ satisfying
\begin{equation}
 H(u),H(v)\le CQ,
 \qquad
 0<\bigl|u^{\mathsf T}\mathsf L(A,B)v\bigr|\le \frac{C}{Q^2}.
 \label{third:eq-dirichletproduct}
\end{equation}
\end{proposition}

\begin{proof}
Dirichlet approximation gives integers $1\le q\le Q$ and $p$ with
$|q\alpha-p|\le Q^{-1}$.  Put $u=(-p,q)$.  Then
\[
 |u_1\log2+u_2\log3|
 =\log2\,|q\alpha-p|
 \le \frac{\log2}{Q}.
\]
Since $p=q\alpha+O(Q^{-1})$, the coordinates of $u$ are $O(Q)$.  Applying Dirichlet
independently to $\beta$ gives $1\le s\le Q$ and $r\in\Z$ with
$|s\beta-r|\le Q^{-1}$.  Put $v=(-r,s)$; then $|v_1+\beta v_2|\le Q^{-1}$ and
$H(v)=O_\beta(Q)$.  Equation \eqref{third:eq-coefffactor} gives the upper bound.  The value is
nonzero by Corollary~\ref{third:cor-anisotropy}.
\end{proof}

\begin{remark}[The critical exponent is two]
Proposition~\ref{third:prop-dirichlet} is an absolute obstruction to any generic claim that all
nonzero rational matrix coefficients are much larger than $Q^{-2}$.  A contradiction from
lower bounds would require a genuinely coupled estimate of strength
$Q^{-2}L(Q)$ with $L(Q)\to\infty$, or an exponent strictly smaller than two.  Separate
one-dimensional irrationality estimates cannot cross this threshold: even optimal irrationality
measures would at best approach $Q^{-2-o(1)}$ after multiplication.  Current explicit Baker
exponents are much larger than two.  The required improvement must use the simultaneous
integrality of $A=2^\beta$ and $B=3^\beta$, not only the irrationality of the two kernel slopes.
\end{remark}

\subsection{Exact sparse interpolation on the counterexample semigroup}
\label{third:sparse-interpolation}

The Matrix Coefficient program becomes especially concrete when restricted to the semigroup
of integer points supplied by a counterexample.  For a finite set
$\Omega\subset\Nzero^2$, define
\begin{equation}
 \mathcal X_\Omega(A,B)
 =\left\{
   \bigl(2^a3^b,A^aB^b\bigr):(a,b)\in\Omega
  \right\}\subset(\N)^2.
 \label{third:eq-Xomega}
\end{equation}
If \eqref{third:eq-beta} holds, then every point has the one-parameter form
\begin{equation}
 \bigl(2^a3^b,A^aB^b\bigr)
 =\bigl(e^{z_{a,b}},e^{\beta z_{a,b}}\bigr),
 \qquad
 z_{a,b}=a\log2+b\log3.
 \label{third:eq-paramsemigroup}
\end{equation}
Distinct pairs $(a,b)$ give distinct $z_{a,b}$ by multiplicative independence of $2$ and $3$.

The following zero-counting lemma is classical, but its exact consequence for this problem is
new and decisive for auxiliary-polynomial design.

\begin{lemma}[Chebyshev zero bound for real exponentials]
\label{third:lem-chebyshev}
Let $\lambda_1<\cdots<\lambda_M$ be real numbers and let
\[
 F(z)=\sum_{j=1}^M c_j e^{\lambda_jz}
\]
with all $c_j\ne0$.  Then $F$ has at most $M-1$ distinct real zeros.
\end{lemma}

\begin{proof}
Induct on $M$.  The case $M=1$ is immediate.  Multiplying by $e^{-\lambda_1z}$ does not
change the zeros.  If the resulting function had $M$ distinct zeros, Rolle's theorem would
give at least $M-1$ distinct zeros of its derivative, which is a nonzero sum of $M-1$
exponentials with distinct exponents $\lambda_j-\lambda_1$ for $j\ge2$.  This contradicts the
induction hypothesis.
\end{proof}

For a polynomial $P(t_1,t_2)$, let $S(P)\subset\Nzero^2$ denote its monomial support.

\begin{theorem}[Exact sparse-support dichotomy]
\label{third:thm-support-dichotomy}
Assume $A,B\in\N$ and $\det\mathsf L(A,B)=0$.  Let
$\Omega\subset\Nzero^2$ have cardinality $R\ge2$, and define
\begin{equation}
 \sigma_\Omega(A,B)
 =\min\left\{|S(P)|:
 0\ne P\in\R[t_1,t_2],\ P|_{\mathcal X_\Omega(A,B)}=0\right\}.
 \label{third:eq-sigmadef}
\end{equation}
Then exactly one of the following holds:
\begin{enumerate}
\item $\beta=m\in\Nzero$, in which case $\sigma_\Omega(A,B)=2$;
\item $\beta\notin\Q$, in which case $\sigma_\Omega(A,B)=R+1$.
\end{enumerate}
In particular, a counterexample has the largest sparse interpolation complexity permitted for
any $R$ points with distinct first coordinates.
\end{theorem}

\begin{proof}
If $\beta=m\in\Nzero$, every point satisfies $t_2=t_1^m$, so the binomial
$P=t_2-t_1^m$ vanishes.  A single nonzero monomial cannot vanish on a point of
$(\R_{>0})^2$, hence the minimum is two.

Suppose now that $\beta$ is irrational.  Write a nonzero polynomial with $M$ monomials as
\[
 P(t_1,t_2)=\sum_{(r,s)\in S(P)}c_{r,s}t_1^rt_2^s,
 \qquad c_{r,s}\ne0.
\]
Restricting to the curve in \eqref{third:eq-paramsemigroup} gives
\begin{equation}
 F(z)=P(e^z,e^{\beta z})
 =\sum_{(r,s)\in S(P)}c_{r,s}e^{(r+s\beta)z}.
 \label{third:eq-restrictedfewnomial}
\end{equation}
The frequencies $r+s\beta$ are distinct, since equality for two integer pairs would make
$\beta$ rational.  If $P$ vanishes on the $R$ points, then $F$ has the $R$ distinct zeros
$z_{a,b}$.  Lemma~\ref{third:lem-chebyshev} gives $R\le M-1$, hence $M\ge R+1$.

For the reverse inequality, list the distinct first coordinates as $x_1,\dots,x_R$ and take
\[
 Q(t_1)=\prod_{j=1}^R(t_1-x_j).
\]
All $x_j$ are positive, so every elementary symmetric coefficient is nonzero; therefore
$Q$ has exactly $R+1$ monomials.  It vanishes on the required points.  In the present
integer setting, $Q\in\Z[t_1]$.
\end{proof}

For the box used by Dasgupta and Kakde, put
\begin{equation}
 \mathcal X(2N)=
 \left\{(2^a3^b,A^aB^b):0\le a,b<2N\right\}.
 \label{third:eq-X2N}
\end{equation}
It has exactly $4N^2$ points.  The theorem gives the sharp formula
\begin{equation}
 \sigma_{[0,2N)^2}(A,B)=
 \begin{cases}
 2,&\beta\in\Z,\\
 4N^2+1,&\beta\notin\Q.
 \end{cases}
 \label{third:eq-boxsupport}
\end{equation}

\begin{corollary}[One-monomial saving criterion]
\label{third:cor-one-term}
Conjecture~\ref{conj:AE} follows from the following auxiliary-polynomial assertion:
for every $A,B\in\N$ with $\det\mathsf L(A,B)=0$, there exist an integer $N\ge1$ and a
nonzero polynomial $P\in\R[t_1,t_2]$ such that
\begin{equation}
 |S(P)|\le4N^2,
 \qquad
 P|_{\mathcal X(2N)}=0.
 \label{third:eq-one-term-target}
\end{equation}
The coefficient field may be replaced by $\Q$ or $\Z$, producing stronger sufficient
conditions.
\end{corollary}

\begin{proof}
A nonintegral counterexample would have support minimum $4N^2+1$ by
\eqref{third:eq-boxsupport}, contradicting \eqref{third:eq-one-term-target}.
\end{proof}

This criterion is deliberately calibrated against trivial interpolation.  The polynomial
$\prod_{x\in\pi_1\mathcal X(2N)}(t_1-x)$ has $4N^2+1$ terms.  The conjecture would follow from
a construction which cancels just one of those terms, or from any genuinely bivariate
polynomial with no more than $4N^2$ monomials.

\begin{corollary}[Generalized evaluation determinants]
\label{third:cor-evaluationdet}
Assume a counterexample.  Let $z_1<\cdots<z_M$ be parameters of $M$ distinct semigroup
points, and let $(r_i,s_i)$ be $M$ distinct monomial exponents.  Then
\begin{equation}
 D=\det\!\left(e^{(r_i+s_i\beta)z_j}\right)_{1\le j,i\le M}
 =\det\!\left(x_j^{r_i}y_j^{s_i}\right)_{1\le j,i\le M}
 \label{third:eq-evaldet}
\end{equation}
is a nonzero integer.  In particular, $|D|\ge1$.
\end{corollary}

\begin{proof}
If the determinant vanished, a nontrivial linear combination of its columns would produce an
a nonzero exponential sum with at most $M$ distinct terms and $M$ distinct zeros, contradicting
Lemma~\ref{third:lem-chebyshev}.  The entries $x_j^{r_i}y_j^{s_i}$ are positive integers.
\end{proof}

This is the exact finite-dimensional form of the barrier encountered by the determinant
methods earlier in the report.  Every square evaluation matrix is nonsingular; a proof must
use arithmetic to make a nonzero integer determinant too small, or too divisible, to survive.

\subsection{The Dasgupta--Kakde fewnomial target, specialized exactly}
\label{third:DK}

For the matrix \eqref{third:eq-logmatrix}, define row generators
\[
 x_1=(2,A),
 \qquad
 x_2=(3,B)
 \quad\text{in }(\R_{>0})^2.
\]
The multiplicative pairing of Dasgupta and Kakde becomes
\begin{equation}
 \begin{split}
 \langle u,v\rangle_X
 &=2^{u_1v_1}A^{u_1v_2}3^{u_2v_1}B^{u_2v_2}\\
 &=\exp\!\left(u^{\mathsf T}\mathsf L(A,B)v\right).
 \end{split}
 \label{third:eq-DKpairing}
\end{equation}
Their condition $(o)$ is exactly the existence of nonzero integral $u,v$ for which this
pairing equals one.  Integral and rational versions are equivalent by bilinearity and
clearing denominators; by Theorem~\ref{third:thm-rmcc-equivalence}, this is exactly the
desired rational matrix coefficient.

Their real $2\times2$ fewnomial condition $(m')$ asks that, for every sufficiently large $N$,
there be a polynomial $P_N(t_1,t_2)$ satisfying
\begin{equation}
 |S(P_N)|<N^2,
 \qquad
 P_N|_{\mathcal X(2N)}=0.
 \label{third:eq-mprime}
\end{equation}
Theorem~7.1 of \cite{DasguptaKakde2026} proves, for a singular real $2\times2$ logarithm
matrix,
\begin{equation}
 (m')\Longrightarrow(o).
 \label{third:eq-mprime-to-o}
\end{equation}
The unproved direction in their program is the construction
\begin{equation}
 (w)\Longrightarrow(m'),
 \label{third:eq-w-to-mprime}
\end{equation}
where $(w)$ is singularity of the logarithm matrix.  They explicitly describe this implication
as remaining mysterious.

\begin{theorem}[Conditional Dasgupta--Kakde route]
\label{third:thm-DKconditional}
Conjecture~\ref{conj:AE} follows if singularity of every integer matrix
\eqref{third:eq-logmatrix} implies the fewnomial condition \eqref{third:eq-mprime}.
It already follows from the weaker AE-specific one-time assertion
\eqref{third:eq-one-term-target}.
\end{theorem}

The exact support dichotomy shows how much specialization buys.  Dasgupta--Kakde ask for
fewer than $N^2$ monomials on $4N^2$ points.  For this problem, any polynomial with at most
$4N^2$ monomials would suffice.  Thus the AE-specific target permits asymptotically four times
as many monomials and needs only one value of $N$.  Nevertheless, under a counterexample even
that one-term compression is impossible: the support minimum is exactly $4N^2+1$.

\subsubsection{An adelic determinant program}

The preceding results suggest a precise way integrality might be used.  Choose $R$ semigroup
points and $R$ monomials and form the integer determinant $D$ in
\eqref{third:eq-evaldet}.  A contradiction follows if one can establish
\begin{equation}
 \sum_p v_p(D)\log p>\log U,
 \qquad
 |D|\le U,
 \label{third:eq-adeliccriterion}
\end{equation}
for some explicit archimedean upper bound $U$, because a nonzero integer cannot be divisible
by an integer larger than its absolute value.  Equivalently, one may seek an integer
polynomial whose values on the box are all divisible by a modulus exceeding their
archimedean size; exact vanishing then follows.

Three mechanisms are available in principle:
\begin{enumerate}
\item choose rows which collide to high order modulo primes dividing $2,3,A,$ or $B$;
\item choose a support whose finite differences produce high $p$-adic divisibility;
\item combine archimedean smallness with nonarchimedean divisibility through the product
formula.
\end{enumerate}
The obstruction is quantitative.  Congruence collapse based only on orders modulo primes
reduces to common divisors such as $\gcd(2^d-1,3^d-1)$, for which known arithmetic
unlikely-intersection and gcd estimates are subexponential in $d$; meanwhile raw evaluation
heights on a box grow like $\exp(O(Nd))$.  Any successful construction must exploit the
second coordinates and the singular real-logarithm relation in a coupled way, rather than
relying only on independent periodicity modulo primes.

\begin{target}[AE-special sparse auxiliary polynomial]
\label{third:target-sparse}
Construct, from the exact integer data $A,B$ and the singularity
$\det\mathsf L(A,B)=0$, one nonzero $P\in\Z[t_1,t_2]$ and one $N\ge1$ with
$|S(P)|\le4N^2$ such that $P$ vanishes on \eqref{third:eq-X2N}.  A construction with
$|S(P)|<N^2$ would prove the full real $2\times2$ Matrix Coefficient case by the
Dasgupta--Kakde theorem; the weaker bound $4N^2$ is already sufficient for
Alaoglu--Erd\H{o}s.
\end{target}

\subsection{The exact rational-point exponent on the unbounded power curve}
\label{third:unbounded-counting}
...
\begin{equation}
 ...
 =\{(t,t^{\thetaq}):t\ge1\},
 \qquad
 \thetaq=\frac{\log3}{\log2}>1.
 \label{third:eq-gammacurve}
\end{equation}
For a rational number $q=a/b$ in lowest terms, write
$H(q)=\max\{|a|,|b|\}$, and for a rational point $P=(q_1,q_2)$ put
$H(P)=\max\{H(q_1),H(q_2)\}$.  Define
\[
 N_{\Gamma_{\thetaq}}(H)
 =\#\{P\in\Gamma_{\thetaq}(\Q):H(P)\le H\}.
\]

A hypothetical counterexample $(A,B,\beta)$ produces the two-parameter family
\begin{equation}
 P_{n,k}=(2^nA^k,3^nB^k)
 =\bigl(2^{n+k\beta},3^{n+k\beta}\bigr),
 \qquad n,k\in\Nzero.
 \label{third:eq-unboundedpoints}
\end{equation}
The points are distinct: equality would give
$(n-n')+(k-k')\beta=0$, and irrationality of $\beta$ forces both differences to vanish.

\begin{theorem}[Exact counterexample point count]
\label{third:thm-unbounded-count}
If a counterexample with exponent $\beta$ exists, then there is a constant $C_\beta>0$ such that
\begin{equation}
 N_{\Gamma_{\thetaq}}(H)
 \ge
 \frac{(\log H)^2}{2\beta(\log3)^2}
 -C_\beta\log H
 \qquad(H\ge2).
 \label{third:eq-unboundedlower}
\end{equation}
More precisely, the number of points in \eqref{third:eq-unboundedpoints} of height at most
$H$ equals
\begin{equation}
 \#\{(n,k)\in\Nzero^2:n+k\beta\le\log_3H\}
 =\frac{T^2}{2\beta}+O_\beta(T),
 \qquad T=\log_3H.
 \label{third:eq-latticecount}
\end{equation}
\end{theorem}

\begin{proof}
Since $\thetaq>1$ and both coordinates in \eqref{third:eq-unboundedpoints} are positive
integers, their height is the second coordinate:
$H(P_{n,k})=3^{n+k\beta}$.  This identifies the family count with the lattice count in
\eqref{third:eq-latticecount}.  Summing over $0\le k\le T/\beta$ gives
\begin{align*}
 \sum_{k=0}^{\lfloor T/\beta\rfloor}
 \bigl(\lfloor T-k\beta\rfloor+1\bigr)
 &=\frac{T^2}{2\beta}+O_\beta(T),
\end{align*}
by the elementary area estimate for a right triangle.  Substituting
$T=(\log H)/(\log3)$ proves \eqref{third:eq-unboundedlower}.
\end{proof}

\begin{corollary}[Critical unbounded counting threshold]
\label{third:cor-unbounded-threshold}
Any estimate
\begin{equation}
 N_{\Gamma_{\thetaq}}(H)=o((\log H)^2)
 \label{third:eq-criticalcount}
\end{equation}
proves Conjecture~\ref{conj:AE}.  In particular, an upper bound
$O((\log H)^{2-\varepsilon})$ for any $\varepsilon>0$ suffices.
\end{corollary}

This exponent is the exact benchmark against which current point-counting theorems must be
measured.  Pila's explicit result for the related surface and curves
$W_\alpha:y=x^\alpha$ gives a power such as $(\log H)^{20}$ in the relevant specialization
\cite{Pila2010Exponential}.  Binyamini--Novikov--Zak prove that sets definable in
$\R_{\exp}$ satisfy a bound by \emph{some} polynomial in $\log H$
\cite{BinyaminiNovikovZak2024}.  Their theorem is a major qualitative advance, but it does not
state a specialization with exponent below two for \eqref{third:eq-gammacurve}.  A generic
statement $O((\log H)^C)$ is insufficient unless $C<2$.

The issue is not definability.  The curve is definable in $\R_{\exp}$ and has no positive
dimensional semialgebraic part.  The issue is the numerical exponent and the special arithmetic
of its rational points.

\subsection{The compact synchronized denominator threshold}
\label{third:compact-counting}

The unbounded family can be folded into the fixed compact arc
\begin{equation}
 \mathcal A
 =\{(2^t,3^t):0\le t<1\}.
 \label{third:eq-compactarc}
\end{equation}
For $r\in\Nzero$, call a point $(x,y)\in\mathcal A(\Q)$ \emph{synchronized at level $r$} if
\begin{equation}
 2^r x\in\Z,
 \qquad
 3^r y\in\Z.
 \label{third:eq-syncdef}
\end{equation}
With the same height convention, let
$N_{\mathcal A}(H)=\#\{P\in\mathcal A(\Q):H(P)\le H\}$, and let
$N_{\mathrm{sync}}(H)$ count rational points on $\mathcal A$ of height at most $H$ which
are synchronized at at least one level.

For a counterexample, set $r_k=\lfloor k\beta\rfloor$ and
\begin{equation}
 Q_k=
 \left(\frac{A^k}{2^{r_k}},\frac{B^k}{3^{r_k}}\right)
 =\left(2^{\{k\beta\}},3^{\{k\beta\}}\right).
 \label{third:eq-compactpoints}
\end{equation}
These points lie on $\mathcal A$, satisfy \eqref{third:eq-syncdef} at the common level $r_k$,
and are distinct because $\beta$ is irrational.

\begin{theorem}[Compact synchronized lower bound]
\label{third:thm-compactlower}
If a counterexample $(A,B,\beta)$ exists, then for all sufficiently large $H$,
\begin{equation}
 N_{\mathrm{sync}}(H)
 \ge \left\lfloor\frac{\log H}{\log B}\right\rfloor.
 \label{third:eq-compactlower}
\end{equation}
Consequently, either of the estimates
\begin{equation}
 N_{\mathcal A}(H)=o(\log H)
 \qquad\text{or}\qquad
 N_{\mathrm{sync}}(H)=o(\log H)
 \label{third:eq-compactthreshold}
\end{equation}
would prove Conjecture~\ref{conj:AE}.
\end{theorem}

\begin{proof}
The rational height of the first coordinate in \eqref{third:eq-compactpoints} is at most
$\max(A^k,2^{r_k})\le A^k$, and the second is at most
$\max(B^k,3^{r_k})\le B^k$.  Since $B>A$ for $\beta>0$, one has
$H(Q_k)\le B^k$.  Hence every $1\le k\le(\log H)/(\log B)$ contributes a distinct
synchronized point of height at most $H$.
\end{proof}

This compact version isolates the arithmetic information which generic point-counting loses.
After reduction, the first denominator is supported only at $2$, the second only at $3$, and
both displayed denominators arise from the same integer $r_k$.  A theorem sensitive to that
synchronization needs only beat $\log H$, not $(\log H)^2$.

\begin{target}[Denominator-sensitive compact counting]
\label{third:target-compact}
Prove $N_{\mathrm{sync}}(H)=o(\log H)$ for the arc \eqref{third:eq-compactarc}.  It is enough
to count points admitting the common-level representation \eqref{third:eq-syncdef}; no bound
for all rational points on arbitrary definable curves is required.
\end{target}

\subsection{A six-exponentials monodromy consequence}
\label{third:monodromy}

Six exponentials does not settle the two-base problem, but it forces an additional and rather
strong property of any counterexample.

\begin{theorem}[Transcendental rational monodromy]
\label{third:thm-monodromy}
Let $\beta$ be a hypothetical nonintegral solution.  For every nonzero $q\in\Q$,
\begin{equation}
 \exp(2\pi i q\beta)
 \label{third:eq-monodromy}
\end{equation}
is transcendental.  Consequently both
$\cos(2\pi q\beta)$ and $\sin(2\pi q\beta)$ are transcendental.
\end{theorem}

\begin{proof}
The two numbers $1,\beta$ are linearly independent over $\Q$.  The three complex numbers
$\log2,\log3,2\pi iq$ are also $\Q$-linearly independent: taking imaginary parts kills the
coefficient of $2\pi iq$, and multiplicative independence of $2$ and $3$ kills the remaining
two coefficients.  Apply the six exponentials theorem to these two rows and three columns.
Five of the six exponentials are algebraic:
\[
 2,\quad3,\quad e^{2\pi iq},\quad A,\quad B.
\]
Here $e^{2\pi iq}$ is a root of unity.  Therefore the sixth exponential
$e^{2\pi iq\beta}$ must be transcendental.  If its cosine or sine were algebraic, then
$e^{2\pi iq\beta}$ would satisfy a quadratic polynomial with algebraic coefficients, which is
impossible.
\end{proof}

This ``monodromy obstruction'' is not a contradiction, but it is a new unconditional
consequence of a counterexample.  It says that every nontrivial rational rotation of the
counterexample exponent avoids the algebraic unit circle in the strongest possible way.

\subsection{Literature audit: 2023 through August 21, 2026}
\label{third:literature}

The search was aimed not merely at papers mentioning four exponentials, but at developments
which might supply one of the exact missing ingredients above: rational matrix coefficients,
sparse auxiliary polynomials, an exponent below two in rational-point counting, or an
arithmetic bridge from real logarithmic dependence to congruence or gcd information.
...
\toprule
Work & Main relevant result & Consequence for the present problem \\
\midrule
\endfirsthead
\toprule
Work & Main relevant result & Consequence for the present problem \\
\midrule
\endhead
Dasgupta (2023)~\cite{Dasgupta2023}
& Develops Structural Rank, Baker, and Waldschmidt--Masser background; announces the Matrix
Coefficient viewpoint.
& Supplies the correct formal-rank language for \eqref{third:eq-sparseSR}, but no
$2\times2$ proof. \\
\addlinespace
Dasgupta--Kakde (arXiv 2024; published 2026)~\cite{DasguptaKakde2026}
& Formulates Matrix Coefficient; proves Structural Rank $\Rightarrow$ Matrix Coefficient;
proves $(m)\Rightarrow(o)$ and, in the real $2\times2$ case,
$(m')\Rightarrow(o)$.
& Directly relevant.  The missing $(w)\Rightarrow(m')$ is exactly an auxiliary-polynomial
construction; Section~\ref{third:DK} gives its AE specialization and weaker one-term target. \\
\addlinespace
Binyamini--Novikov--Zak (2024)~\cite{BinyaminiNovikovZak2024}
& Proves Wilkie's conjecture for Pfaffian structures and hence polylogarithmic counting in
$\R_{\exp}$.
& Gives $O((\log H)^C)$ for some $C$, but no specialization with $C<2$ or with the
synchronized denominator structure required by
Corollary~\ref{third:cor-unbounded-threshold}. \\
\addlinespace
Pila (2007, 2010)~\cite{Pila2007Pfaff,Pila2010Exponential}
& Explicit rational-point bounds for Pfaff curves and a certain exponential-algebraic
surface; the power-curve bound has a large logarithmic exponent.
& Historically closest counting route, but the explicit exponent (for example $20$) is far
above the critical value $2$. \\
\addlinespace
Massold (2025 v1)~\cite{Massold2025}
& Claims lower bounds for transcendence degrees generated by exponentials of products.
& The displayed parameter choice for the corollary does not satisfy the theorem's strict
inequality when $m=n=2$; see Section~\ref{third:massold}.  It therefore supplies no proof of
the four exponentials case as written. \\
\addlinespace
Dill--Gallinaro (2025)~\cite{DillGallinaro2025}
& Finiteness and intersection results for algebraic subvarieties and translates of subtori in
powers of $\mathbb G_m$.
& The graph $y=x^\beta$ with transcendental $\beta$ is not an algebraic subvariety, so the
results do not directly apply to the counterexample curve. \\
\addlinespace
Vermeulen (2025)~\cite{Vermeulen2025}
& Pila--Wilkie-type rational-point bounds for transcendental curves in $1$-$h$-minimal valued
fields.
& Important nonarchimedean analogue, but it does not furnish the archimedean exponent or the
common-denominator synchronization in \eqref{third:eq-syncdef}. \\
\addlinespace
Binyamini et al. (2026)~\cite{CountingRelations2026}
& Formulates counting for algebraic relations; proves a weaker covering theorem for compact
trajectories of polynomial differential equations satisfying an arithmetic D-property, with a
dimensional restriction.
& The full counting conjecture is stated to imply difficult algebraic-independence problems.
The present exponential trajectory has differential coefficients $\log2,\log3$ (or the
transcendental ratio $\thetaq$), not a polynomial vector field over a number field in the
stated setup, and the proved output is a tiny-ball cover rather than the exponent
\eqref{third:eq-criticalcount}. \\
\addlinespace
Harrison (2026)~\cite{Harrison2026}
& Studies additive relations in irrational powers using functional transcendence,
Binyamini--Novikov--Zak counting, and linear forms in logarithms.
& Strong adjacent evidence that irrational powers can be controlled in additive problems,
but it does not address simultaneous exact integrality in two multiplicatively independent
bases. \\
\addlinespace
Brisebarre--Hanrot (2026)~\cite{BrisebarreHanrot2026}
& Gives an algorithmic framework for integer points close to transcendental curves, with
complexity analysis and experiments.
& Useful for finite searches and near-miss certification; it does not yield a global
subquadratic logarithmic count on \eqref{third:eq-gammacurve}. \\
\addlinespace
Campagna--Dill--Wilms (2026)~\cite{CampagnaDillWilms2026}
& Develops arithmetic likely/unlikely intersections in powers of $\mathbb{G}_m$, including
partial results in dimensions one and two and gcd-type applications.
& Suggests an adelic route to \eqref{third:eq-adeliccriterion}, but no theorem converts the
real logarithmic rank-one relation into the required congruence or gcd growth. \\
\bottomrule
\end{longtable}
}

The audit leaves the Dasgupta--Kakde implication \eqref{third:eq-w-to-mprime} as the closest
published formulation of the missing step.  The counting literature supplies the right
qualitative scale ``polynomial in $\log H$'' but not the numerical exponent below two, while
the arithmetic-intersection literature supplies strong congruence and gcd estimates only
after one has entered an algebraic torus setting.  The present problem lies exactly between
those worlds: its curve is transcendental, but its special points are integral and have
extremely rigid prime support.

\subsection{The Massold v1 parameter check}
\label{third:massold}

Because a valid $m=n=2$ specialization of \cite{Massold2025} would settle the four
exponentials conjecture, its numerical hypotheses require explicit checking rather than a
citation by title or abstract.

Theorem~II.2(2) in arXiv:2506.01123v1 assumes positive integers $t,\mu,\nu$ satisfying, among
other bounds,
\begin{equation}
 \frac{\mu\nu}{\mu+\nu}>t,
 \qquad
 \mu\le\frac{m-t}{\max(t-1,1)},
 \qquad
 \nu\le\frac{n-t}{\max(t-1,1)}.
 \label{third:eq-massoldhyp}
\end{equation}
The displayed proof of Corollary~1.6 chooses
\begin{equation}
 t=\left\lfloor\sqrt{\frac{\min(m,n)+1}{2}}\right\rfloor,
 \qquad
 \mu=\nu=2t^2-1.
 \label{third:eq-massoldchoice}
\end{equation}
For $m=n=2$, this gives $t=1$ and $\mu=\nu=1$.  The first strict inequality in
\eqref{third:eq-massoldhyp} becomes
\begin{equation}
 \frac12>1,
 \label{third:eq-massoldfailure}
\end{equation}
which is false.

In fact no positive $t$ can satisfy the first inequality when $m=n=2$ under the theorem's
bounds $\mu\le m$ and $\nu\le n$: for $1\le\mu,\nu\le2$,
\[
 \frac{\mu\nu}{\mu+\nu}\le1,
\]
and the required inequality is strict with $t\ge1$.  Thus Theorem~II.2, with its displayed
hypotheses, cannot yield a positive transcendence-degree lower bound in the $2\times2$ case.

\begin{warning}
This is a narrow audit of arXiv v1 and its displayed parameter substitution.  It does not
assert that every result or possible corrected version of the paper is false.  As of August
21, 2026, arXiv lists only v1.  The point relevant here is simply that the cited corollary
cannot be used as a proof of Alaoglu--Erd\H{o}s or four exponentials without a corrected
argument that genuinely covers $m=n=2$.
\end{warning}

\subsection{What the newest counting theorems would have to improve}
\label{third:counting-audit}

The critical exponents derived above allow a more discriminating reading of recent counting
results.

\begin{enumerate}
\item \textbf{A polylogarithmic theorem is not automatically enough.}
A statement $N(H)\le(\log H)^C$ closes the problem only if $C<2$ on the unbounded curve, or
$C<1$ on the compact synchronized set.  The generic notation $\operatorname{poly}(\log H)$
hides precisely the datum that matters here.

\item \textbf{Counting all rational points may be harder than counting the relevant ones.}
The points \eqref{third:eq-compactpoints} have denominators supported at $2$ and $3$ and a
common level $r_k$.  A theorem using this local information can aim directly at
$o(\log H)$ without controlling arbitrary rational points on the arc.

\item \textbf{Tiny-ball covers do not by themselves count exact points.}
The 2026 algebraic-relations work proves, under an arithmetic D-property and dimensional
hypotheses, a cover by polynomially many balls of exponentially small radius
\cite{CountingRelations2026}.  Turning such a cover into an exact count here would require a
separation theorem for synchronized $S$-integral points at a scale smaller than those balls.
No such separation result is presently available.

\item \textbf{The vector-field arithmetic is mismatched.}
The natural parametrization $z\mapsto(e^z,e^{\thetaq z})$ satisfies a linear differential
equation with coefficient $\thetaq$, which is transcendental; equivalently
$z\mapsto(2^z,3^z)$ uses coefficients $\log2,\log3$.  This does not directly fit a polynomial
vector field over a number field, one of the arithmetic inputs in the 2026 trajectory theorem.

\item \textbf{The lower bound is genuinely sharp.}
The triangular lattice count \eqref{third:eq-latticecount} is not a rough estimate: it gives
the exact leading constant.  Any future theorem with leading order $C(\log H)^2$ would need
$C<1/(2\beta(\log3)^2)$ for the unknown counterexample $\beta$, whereas an
$o((\log H)^2)$ statement is uniform and immediately decisive.
\end{enumerate}

\subsection{A prioritized research program}
\label{third:program}

The exact reductions suggest the following order of attack.

\subsubsection{Priority 1: one-term sparse compression}

The finite target \eqref{third:eq-one-term-target} is weaker than the general
Dasgupta--Kakde condition and is calibrated exactly against the trivial polynomial.
Approaches worth testing are:
\begin{itemize}
\item integer or Laurent-polynomial interpolation combined with divisibility at primes in
$\supp(6AB)$;
\item determinant constructions in which row congruences force a large $p$-adic valuation;
\item Siegel-lemma coefficients producing archimedean smallness, followed by a product-formula
argument which upgrades small algebraic integers to zero;
\item supports adapted to the valuation vectors $(v_p(A))_p$ and $(v_p(B))_p$, rather than
rectangular degree boxes.
\end{itemize}
A successful construction needs only one $N$ and at most $4N^2$ monomials.

\subsubsection{Priority 2: prove the restricted Matrix Coefficient statement directly}

Theorem~\ref{third:thm-rmcc-equivalence} shows that one need not attack arbitrary logarithm
matrices.  The input matrix has positive real entries, fixed first column, integer second
column after exponentiation, and nonnegative prime valuations.  A proof that every singular
matrix in this narrow family has a rational rank-one zero would settle the conjecture.

\subsubsection{Priority 3: exploit the irreducible signed quadratic}

Instead of assuming independence of all products $\log p\log q$, target only
\[
 X_2L_B(\mathbf X)-X_3L_A(\mathbf X)
\]
with $L_A,L_B$ having nonnegative integral coefficients.  Positivity, sparse support, and the
asymmetry of the distinguished primes $2$ and $3$ are absent from generic Structural Rank
statements and may permit a direct argument.

\subsubsection{Priority 4: denominator-sensitive counting}

The compact target $N_{\mathrm{sync}}(H)=o(\log H)$ is more arithmetic and numerically easier
than a global $o((\log H)^2)$ theorem.  Potential inputs include $S$-unit equations, local
heights, and determinant methods which keep the common level $r$ visible instead of clearing
denominators independently.

\subsubsection{Priority 5: an arithmetic-intersection bridge}

Recent work on arithmetic unlikely intersections controls gcds and intersections with subgroup
schemes in powers of $\mathbb{G}_m$~\cite{CampagnaDillWilms2026}.  To apply it here one needs a
lemma converting
\[
 \log2\log B=\log3\log A
\]
into unexpectedly large congruence intersections for a family of subgroup schemes.  The
real-logarithm equality alone does not currently provide such a bridge.

\subsubsection{Priority 6: coupled critical-scale approximation}

Proposition~\ref{third:prop-dirichlet} fixes the scale $Q^{-2}$.  Further separate irrationality
measures for $\alpha$ and $\beta$ cannot suffice.  A viable approximation theorem must couple
the two errors and extract an arithmetic penalty from $A=2^\beta$ and $B=3^\beta$, for
example a factor tending to infinity at the critical exponent.

\subsection{Formalization blueprint}
\label{third:formalization}

Most new elementary statements are well suited to Lean and would strengthen the trustworthy
boundary of the project without requiring immediate formalization of deep transcendence
theorems.

\begin{description}[leftmargin=4.8cm,style=nextline]
\item[\lean{RestrictedMatrixCoefficient}]
Define $\mathsf L(A,B)$ over $\R$ and prove
Theorem~\ref{third:thm-rmcc-equivalence}, using the existing rational-exponent and
multiplicative-independence lemmas.

\item[\lean{FormalPrimeDeterminant}]
Represent valuation vectors with finite support; prove
Lemma~\ref{third:lem-formalzero} by coefficient comparison and derive the sparse Structural
Rank equivalence.

\item[\lean{PrimeDeterminantIrreducible}]
Formalize Proposition~\ref{third:prop-irreducible} from unique factorization of primes and the
fact that a reducible homogeneous quadratic factors into linear forms.

\item[\lean{ExponentialChebyshev}]
Prove Lemma~\ref{third:lem-chebyshev} by induction and Rolle's theorem.  This is the main
analytic ingredient in the exact support theorem.

\item[\lean{SparseSupportDichotomy}]
Formalize restriction of monomials to $z\mapsto(e^z,e^{\beta z})$, injectivity of
$(r,s)\mapsto r+s\beta$, and Theorem~\ref{third:thm-support-dichotomy}.

\item[\lean{CounterexampleCurveCount}]
Formalize the triangular lattice sum in \eqref{third:eq-latticecount} and its
$T^2/(2\beta)+O(T)$ estimate.

\item[\lean{SynchronizedCompactOrbit}]
Build on the existing compact-orbit files to prove the explicit height inequality
$H(Q_k)\le B^k$ and Theorem~\ref{third:thm-compactlower}.

\item[Named transcendence interfaces]
Keep Baker--W\"ustholz, Matveev, and six exponentials behind explicitly named axiomatic
interfaces until formal libraries exist.  The elementary consequences can be proved once the
interfaces are assumed, preserving a clear trust boundary.
\end{description}

\subsection{Conclusion of the continuation}
\label{third:conclusion}

A hypothetical counterexample is now seen to impose all of the following simultaneously:
\begin{enumerate}
\item a singular $2\times2$ logarithm matrix with no zero rational rank-one coefficient;
\item a nonzero irreducible signed quadratic in prime-log symbols vanishing at the actual
prime logarithms;
\item polynomial Baker repulsion but unavoidable matrix coefficients of size $O(Q^{-2})$;
\item maximal sparse interpolation complexity $R+1$ on every $R$-point semigroup sample;
\item at least a quadratic logarithmic population of rational points on the unbounded curve
and a linear logarithmic synchronized population on the compact arc;
\item transcendental monodromy $e^{2\pi iq\beta}$ for every $q\in\Q^\times$.
\end{enumerate}

None is presently contradictory.  Their value is that each converts the original question
into an exact, testable frontier.  The closest modern general framework is the Matrix
Coefficient/fewnomial program of Dasgupta--Kakde; the closest arithmetic finite target is the
one-monomial saving in Corollary~\ref{third:cor-one-term}; and the closest counting targets are
\eqref{third:eq-criticalcount} and \eqref{third:eq-compactthreshold}.  The literature through
August 21, 2026 does not cross any of these thresholds.  Accordingly, the rigorous endpoint
of this report remains: Conjecture~\ref{conj:AE} is open, but its remaining obstruction has
been compressed to several equivalent or sharply sufficient statements with no hidden
asymptotic slack.
...
Section~\ref{sec:third-continuation} \\
AE-restricted Matrix Coefficient principle $\Longleftrightarrow$ full conjecture
& \statusnew & Theorem~\ref{third:thm-rmcc-equivalence} \\
\midrule
...
Section~\ref{lp:twoprime} \\
Formal prime-symbol determinant and sparse Structural Rank equivalence & \statusnew &
Section~\ref{third:formal-determinant} \\
Counterexample quadratic irreducible over $\Q$ & \statusnew &
Proposition~\ref{third:prop-irreducible} \\
Six-exponentials monodromy consequence & \statusnew &
Theorem~\ref{third:thm-monodromy} \\
\midrule
...
\midrule
\multicolumn{3}{@{}l}{\emph{Matrix coefficients, sparse interpolation, and critical counting}}\\
Effective Baker repulsion for rational matrix coefficients & \statusknown{} + \statusnew &
Proposition~\ref{third:prop-baker-repulsion} \\
Double-Dirichlet $O(Q^{-2})$ coefficient scale & \statusnew &
Proposition~\ref{third:prop-dirichlet} \\
Exact support minimum $R+1$ on $R$ counterexample points & \statusnew &
Theorem~\ref{third:thm-support-dichotomy} \\
One-monomial sparse compression $\Rightarrow$ conjecture & \statusnew &
Corollary~\ref{third:cor-one-term} \\
Dasgupta--Kakde $(m')\Rightarrow(o)$ specialization & \statusknown{} + \statusopen &
Section~\ref{third:DK} \\
$o((\log H)^2)$ unbounded point count $\Rightarrow$ conjecture & \statusnew &
Corollary~\ref{third:cor-unbounded-threshold} \\
$o(\log H)$ synchronized compact count $\Rightarrow$ conjecture & \statusnew &
Theorem~\ref{third:thm-compactlower} \\
\midrule
...
% --- added with the August 2026 Matrix Coefficient continuation ---
...
\newblock DOI:
\href{https://doi.org/10.2140/ent.2023.2.93}{10.2140/ent.2023.2.93};
arXiv:2303.02037.
...
\newblock Annales de la Facult\'e des Sciences de Toulouse, S\'erie 6,
\textbf{16}(3) (2007), 635--645.
\newblock DOI: \href{https://doi.org/10.5802/afst.1162}{10.5802/afst.1162}.

\bibitem{Pila2010Exponential}
J.~Pila,
...
\bibitem{DillGallinaro2025}
G.~A.~Dill and F.~Gallinaro,
\newblock \emph{Likely intersections in powers of the multiplicative group},
\newblock arXiv:2506.07550v2, June 2025.

\bibitem{Vermeulen2025}
F.~Vermeulen,
\newblock \emph{Counting rational points on transcendental curves in valued fields},
\newblock arXiv:2506.19411v2, July 2025, accepted version.

\bibitem{CountingRelations2026}
G.~Binyamini, N.~Hirata-Kohno, M.~Kawashima, and Y.~Salant,
\newblock \emph{Counting Theorems for Algebraic Relations},
\newblock arXiv:2604.15189v1, April 2026.
...
\newblock \emph{Integer points close to a transcendental curve: an algorithmic approach},
\newblock arXiv:2606.04858v1, June 2026.

\bibitem{CampagnaDillWilms2026}
F.~Campagna, G.~A.~Dill, and R.~Wilms,
\newblock \emph{Arithmetic unlikely intersections in powers of the multiplicative group},
\newblock arXiv:2607.15741v1, July 2026.
