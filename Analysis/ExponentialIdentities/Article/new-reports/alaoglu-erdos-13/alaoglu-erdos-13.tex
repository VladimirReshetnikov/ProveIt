...
\hypersetup{
  ...
  pdfsubject={Machine-checked partial results, exact conditional structure, determinant and matrix
methods, exterior powers, boundary realizations, and research directions}
}
\urlstyle{same}
...
% The continuation prefixes reach three characters (for example M10).  Widen the
% table-of-contents number boxes so these labels never intrude into the titles.
\makeatletter
\renewcommand*\l@section[2]{%
  \ifnum \c@tocdepth >\z@
    ...
    \endgroup
  \fi}
\renewcommand*\l@subsection{\@dottedtocline{2}{2.4em}{3.2em}}
\renewcommand*\l@subsubsection{\@dottedtocline{3}{5.6em}{4.0em}}
\makeatother
...
through $2^{29}$.  The fourth continuation develops a full compound/Smith hierarchy, proves an explicit
density-dependent exterior-power ceiling, isolates the canonical finite-difference factorial
content of every Smith divisor, realizes the conditional grid as two commuting rank-$K$
boundary perturbations of nilpotent shifts, and introduces rational strictly positive
Kraus--Loewner determinant families.  It also proves a projective-density principle showing
that any finite-dimensional open, scale-invariant Hadamard-power obstruction can be realized
integrally under a counterexample.
\end{minipage}
\vfill
...
A fourth continuation studies the matrix problem beyond the top determinant.  It proves the
adelic compound sandwich $Q_k\mid d_k\le\|\bigwedge^kE\|_2$, computes the complete
triangular-grid compound transport profile $\pi\rho/(8-4\rho)$, and shows that every proper
compound is worse than the full determinant under termwise valuation and largest-term bounds.
It also proves a projective arithmetic-density principle, exact rational-direction alternant
factorizations, and a conditional Hadamard-power no-go theorem.  On the square grid it separates
a universal finite-difference Smith contribution of size only $\exp(O(K^3\log K))$ from the
$K^4$-scale arithmetic still needed, derives rank-$K$ boundary-displacement and transfer-
determinant formulas, and constructs a strictly totally positive family of rational
Kraus--Loewner matrices.
\end{abstract}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.48\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}@{}}
\toprule
...
{\footnotesize
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.31\textwidth}>{\raggedright\arraybackslash}p{0.33\textwidth}>{\raggedright\arraybackslash}p{0.26\textwidth}@{}}
\toprule
...
\subsection{All unimodular bases of \texorpdfstring{$\langle2,3\rangle$}{the group generated by 2 and 3}}
...
\subsection{Catalan, \texorpdfstring{$abc$}{abc}, and small differences}
...
\begin{center}
\begin{tabularx}{0.97\textwidth}{@{}>{\raggedright\arraybackslash}p{0.45\textwidth}X@{}}
\toprule
Reading & Status \\
\midrule
...
\begin{description}[leftmargin=4em,style=nextline,itemsep=0.9em]
...
\renewcommand{\theHsection}{third.\arabic{section}}
\renewcommand{\theHsubsection}{third.\arabic{section}.\arabic{subsection}}
\renewcommand{\theHsubsubsection}{third.\arabic{section}.\arabic{subsection}.\arabic{subsubsection}}
\renewcommand{\theHtheorem}{third.\arabic{section}.\arabic{theorem}}
\renewcommand{\theHequation}{third.\arabic{section}.\arabic{equation}}
...
\renewcommand{\theHsection}{matrix.\arabic{section}}
\renewcommand{\theHsubsection}{matrix.\arabic{section}.\arabic{subsection}}
\renewcommand{\theHsubsubsection}{matrix.\arabic{section}.\arabic{subsection}.\arabic{subsubsection}}
\renewcommand{\theHtheorem}{matrix.\arabic{section}.\arabic{theorem}}
\renewcommand{\theHequation}{matrix.\arabic{section}.\arabic{equation}}

\section{Fourth continuation: matrix analysis, exterior powers, and boundary realizations}
\label{sec:matrix-fourth}

This continuation takes the matrix viewpoint as far as possible without assuming the Matrix
Coefficient Conjecture.  It does not produce an unconditional proof of the Alaoglu--Erd\H{o}s
conjecture.  It does, however, add several exact results that substantially sharpen the
matrix-theoretic diagnosis of the problem.

Throughout this continuation, a hypothetical counterexample is written
\begin{equation}
  M=2^\beta\in\Z_{>0},\qquad A=3^\beta\in\Z_{>0},
  \qquad \beta\notin\Q,
  \label{eq:matrix-counterexample}
\end{equation}
and the fixed exponent of the original power curve is
\begin{equation}
  \thetaq=\frac{\log 3}{\log 2}\in(1,2).
  \label{eq:matrix-theta}
\end{equation}
Thus every element of the two-sided multiplicative group
\begin{equation}
  G:=\{2^rM^s:r,s\in\Z\}\subset\Q_{>0}
  \label{eq:matrix-candidate-group}
\end{equation}
has rational \(\thetaq\)-th power,
\begin{equation}
  (2^rM^s)^{\thetaq}=3^rA^s\in\Q_{>0}.
  \label{eq:matrix-group-power}
\end{equation}
Because \(\beta\) is irrational, \(G\) is dense in \(\R_{>0}\).

\subsection{Outcome and claim ledger}

\begin{center}
\small
\begin{tabularx}{0.97\textwidth}{@{}p{0.25\textwidth}X p{0.18\textwidth}@{}}
\toprule
Item & Outcome & Status\\
\midrule
Rational basis conditioning & The smallest coefficient obtainable from the rank-one logarithm
matrix by bounded integral left and right vectors factors exactly into two one-dimensional
Diophantine minima.  Dirichlet gives an \(O(B^{-2})\) near-zero coefficient, while Baker theory
only supplies a much weaker polynomial lower bound. & \statusnew\\
Rational-direction alternants & Every determinant whose exponent support lies on a rational
kernel direction factors into an ordinary Vandermonde and a Schur factor.  Consecutive support
factors completely into pairwise integer binomials, so continued-fraction alignment creates no
collective determinant gain. & \statusnew\\
Projective matrix density & Conditional on a counterexample, every finite positive real matrix
can be approximated, up to one common scalar, by a positive integer matrix whose entrywise
\(\thetaq\)-th power is also integral.  Symmetry can be preserved.  Hence every open,
scale-invariant finite-dimensional matrix obstruction transfers to the arithmetic grid. &
\statusnew\\
Hadamard powers & Conditional on a counterexample, there are positive-definite integer matrices
whose entrywise \(\thetaq\)-th powers are integer but indefinite.  Thus the classical critical
exponent for entrywise powers is compatible with a counterexample and cannot by itself prove the
conjecture. & \statusnew\\
Adelic compounds & Determinantal divisors, Smith factors, exterior powers, and termwise
primewise divisibility are assembled into one exact hierarchy
\(Q_k\mid d_k\le\|\bigwedge^kE\|_2\). & \statusnew\\
Triangular compound ceiling & For \(k/N_T\to\rho\), the termwise all-place ceiling divided by
the largest-term exterior-power budget tends at most
\(\pi\rho/(8-4\rho)\), strictly increasing to \(\pi/4\).  Every proper compound is therefore
worse than the full determinant under these estimates. & \statusnew\\
\bottomrule
\end{tabularx}
\end{center}
...
Item & Outcome & Status\\
\midrule
Finite-difference Smith baseline & A unimodular two-variable difference transform factors the
square-grid evaluation matrix through the diagonal weights \(a!b!\).  The resulting explicit
factorial divisor of every compound is only \(\exp(O(K^3\log K))\), lower order than the
\(K^4\)-scale determinant competition. & \statusnew\\
Boundary displacement & The square multiplicative-grid evaluation matrix has exact displacement
rank \(K\) in each coordinate.  Its two commuting multiplication operators are rank-\(K\)
boundary perturbations of the two nilpotent grid shifts, and each \(K^2\)-point spectrum has an
exact \(K\times K\) transfer-determinant representation. & \statusnew\\
Kraus--Loewner matrices & Second divided differences of \(t^{\thetaq}\) at candidate nodes form
strictly totally positive rational matrices with an exact positive multiple-integral determinant.
A complete denominator-clearing identity is proved, together with a homogeneity obstruction to
a naive small-determinant argument. & \statusnew\\
Literature through August 2026 & Recent work on the Matrix Coefficient Conjecture, effective
Diophantine approximation, compounds and total positivity, HCIZ asymptotics, and Gaussian Cobham
rigidity is incorporated.  No unconditional resolution of the relevant four-exponentials case
was found. & \statusknown{} and literature survey\\
\bottomrule
\end{tabularx}
\end{center}

The most important negative conclusion is precise.  Passing from the determinant to arbitrary
proper compounds does not improve the inherited tropical comparison; it makes the leading
constant smaller.  A successful exterior-power argument must therefore estimate the \emph{true}
singular-value free energy or uncover additional Smith divisibility not visible term by term.

\section{The rank-one logarithm matrix under rational changes of basis}
\label{sec:matrix-rational-basis}

The logarithm matrix attached to \eqref{eq:matrix-counterexample} is
\begin{equation}
  L=
  \begin{pmatrix}
   \log2&\log M\\
  ...
  =ab^{\mathsf T},
  \qquad
  a=\binom{\log2}{\log3},\quad
  b=\binom{1}{\beta}.
  \label{eq:matrix-rank-one-log}
\end{equation}
It is singular over \(\R\), but neither factor has a nonzero rational orthogonal vector.  This
is the fixed-base failure of the Matrix Coefficient conclusion recorded in the third
continuation.

\subsection{A factorized bilinear condition number}

For a nonzero vector \(c\in\R^2\) and \(B\ge1\), define
\begin{equation}
 \mu_c(B):=
 \min_{\substack{z\in\Z^2\setminus\{0\}\\\|z\|_\infty\le B}}
 |z^{\mathsf T}c|.
 \label{eq:matrix-one-sided-minimum}
\end{equation}
For the matrix \(L\), define the bounded bilinear minimum
\begin{equation}
 \eta_L(B):=
 \min_{\substack{u,v\in\Z^2\setminus\{0\}\\
                  \|u\|_\infty,\|v\|_\infty\le B}}
 |u^{\mathsf T}Lv|.
 \label{eq:matrix-bilinear-minimum}
\end{equation}

\begin{theorem}[Exact factorization of the bounded bilinear minimum]
\label{thm:matrix-bilinear-factorization}
Under \eqref{eq:matrix-counterexample}, for every \(B\ge1\),
\begin{equation}
  \eta_L(B)=\mu_a(B)\mu_b(B).
  \label{eq:matrix-bilinear-factorization}
\end{equation}
In particular there is a constant \(C=C(M)>0\) such that
\begin{equation}
  \eta_L(B)\le \frac{C}{B^2}
  \qquad(B\ge2).
  \label{eq:matrix-dirichlet-upper}
\end{equation}
On the other hand, effective linear-forms-in-logarithms estimates give computable constants
\(c>0\) and \(\kappa>0\), depending on \(M\), for which
\begin{equation}
  \eta_L(B)\ge cB^{-\kappa}
  \qquad(B\ge2).
  \label{eq:matrix-baker-lower}
\end{equation}
\end{theorem}

\begin{proof}
The rank-one factorization gives
\[
  u^{\mathsf T}Lv=(u^{\mathsf T}a)(b^{\mathsf T}v).
\]
Both finite sets of absolute values in \eqref{eq:matrix-one-sided-minimum} are positive: a zero
for \(a\) would make \(\log3/\log2\) rational, while a zero for \(b\) would make \(\beta\)
rational.  The minimum of all products from two finite positive sets is the product of their
separate minima, proving \eqref{eq:matrix-bilinear-factorization}.

Dirichlet approximation applied separately to \(\thetaq=\log3/\log2\) and to \(\beta\) gives
\(\mu_a(B)\ll B^{-1}\) and \(\mu_b(B)\ll_M B^{-1}\), proving
\eqref{eq:matrix-dirichlet-upper}.  For the lower bound, Baker--W\"ustholz or Matveev gives a
polynomial lower bound for every nonzero form
\(u_1\log2+u_2\log3\).  Likewise
\[
 v_1+\beta v_2=
 \frac{v_1\log2+v_2\log M}{\log2},
\]
and \(2,M\) are multiplicatively independent because \(\beta\notin\Q\).  Multiplying the two
effective bounds proves \eqref{eq:matrix-baker-lower}.
\end{proof}

Every primitive integral vector in dimension two can be completed to a unimodular matrix.
Consequently \(\eta_L(B)\), up to harmless changes in the height parameter, measures how small
one coefficient of \(ULV\) can be made with \(U,V\in\operatorname{GL}_2(\Z)\) of bounded
height.  Approximate vanishing is therefore abundant: coefficients of size \(O(B^{-2})\) are
forced by ordinary continued fractions.  Exact vanishing is a qualitatively different demand.

\subsection{Exponentiating a near-zero coefficient}

For \(u,v\in\Z^2\), put
\begin{equation}
 R(u,v):=
 2^{u_1v_1}3^{u_2v_1}M^{u_1v_2}A^{u_2v_2}\in\Q_{>0}.
 \label{eq:matrix-rational-R}
\end{equation}
Then
\begin{equation}
  \log R(u,v)=u^{\mathsf T}Lv.
  \label{eq:matrix-log-R}
\end{equation}
If \(\|u\|_\infty,\|v\|_\infty\le B\), the logarithmic height of \(R(u,v)\) is
\(O_{M,A}(B^2)\).  The elementary separation of two distinct rationals therefore yields only
\begin{equation}
  R(u,v)\ne1
  \quad\Longrightarrow\quad
  |u^{\mathsf T}Lv|\ge \exp(-C_0B^2)
  \label{eq:matrix-elementary-separation}
\end{equation}
for a fixed \(C_0\).  Baker theory improves the right side to a negative power of \(B\), but
the exponent is far larger than the Dirichlet exponent \(2\) supplied by
\eqref{eq:matrix-dirichlet-upper}.  Thus a numerically tiny coefficient of a rationally
transformed logarithm matrix is fully compatible with exact nonvanishing.

\begin{warning}[Numerical rank is not arithmetic rank]
\label{warn:matrix-numerical-rank}
The singular value of \(L\) is exactly zero, and bounded rational basis changes can make an
individual coefficient polynomially tiny.  Neither fact approaches the Matrix Coefficient
conclusion.  The obstruction is not conditioning; it is the absence of an exact rational
orthogonality relation in either factor of \(ab^{\mathsf T}\).
\end{warning}

\section{Projective arithmetic density and Hadamard powers}
\label{sec:matrix-hadamard}

A tempting matrix-analysis idea is to exploit the fact that nonintegral entrywise powers do not
preserve positive semidefiniteness in all dimensions.  For \(1<\alpha<2\), the classical
FitzGerald--Horn critical-exponent theorem already gives failure in dimension four
\cite{FitzGeraldHorn1977}.  In the
present problem this failure does not contradict integrality.  In fact a hypothetical
counterexample supplies enough rational powers to realize it with integer matrices.

For a positive-entry matrix \(C=(c_{ij})\), write
\[
  C^{\circ\alpha}:=(c_{ij}^{\alpha})
\]
for its Hadamard power.

\subsection{A projective density principle}

The density of \(G\) is stronger than entrywise density: finitely many denominators can always
be cleared by one common element of \(G\).  This gives a general transfer principle for
finite-dimensional matrix analysis.

\begin{theorem}[Projective arithmetic density]
\label{thm:matrix-projective-density}
Assume \eqref{eq:matrix-counterexample}.  Let \(n\ge1\), let
\(B\in M_n(\R_{>0})\), and let \(\varepsilon>0\).  There exist a positive integer
\(\lambda\) with \(\lambda^{\thetaq}\in\Z_{>0}\) and a matrix
\(C\in M_n(\Z_{>0})\) such that
\begin{align}
 \left\|\lambda^{-1}C-B\right\|_{\max}&<\varepsilon,
 \label{eq:matrix-projective-density-base}\\
 \left\|\lambda^{-\thetaq}C^{\circ\thetaq}-B^{\circ\thetaq}\right\|_{\max}
 &<\varepsilon,
 \label{eq:matrix-projective-density-power}
\end{align}
with \(C^{\circ\thetaq}\in M_n(\Z_{>0})\).  If \(B\) is symmetric, \(C\) may be
chosen symmetric.
\end{theorem}

\begin{proof}
By continuity of \(t\mapsto t^{\thetaq}\), choose \(\delta>0\) such that entrywise
\(\delta\)-approximation to \(B\) implies both inequalities after replacing
\(\varepsilon\) by a smaller tolerance if necessary.  Since \(G\) is dense in
\(\R_{>0}\), choose
\[
 B_0=(2^{r_{ij}}M^{s_{ij}})_{i,j}\in M_n(G)
\]
with \(\|B_0-B\|_{\max}<\delta\); for symmetric \(B\), make the choices symmetrically.
Choose nonnegative integers \(R,S\) so large that
\(R+r_{ij}\ge0\) and \(S+s_{ij}\ge0\) for every entry, and put
\[
 \lambda=2^RM^S,
 \qquad
 C=\lambda B_0.
\]
Then \(\lambda\in\Z_{>0}\), \(\lambda^{\thetaq}=3^RA^S\in\Z_{>0}\), and
\[
 C_{ij}=2^{R+r_{ij}}M^{S+s_{ij}}\in\Z_{>0},
 \qquad
 (C_{ij})^{\thetaq}=3^{R+r_{ij}}A^{S+s_{ij}}\in\Z_{>0}.
\]
Finally \(\lambda^{-1}C=B_0\) and
\(\lambda^{-\thetaq}C^{\circ\thetaq}=B_0^{\circ\thetaq}\), giving
\eqref{eq:matrix-projective-density-base}--\eqref{eq:matrix-projective-density-power}.
\end{proof}

\begin{corollary}[Transfer of open homogeneous matrix phenomena]
\label{cor:matrix-open-homogeneous-transfer}
Assume \eqref{eq:matrix-counterexample}.  Let \(\mathcal U,\mathcal V\) be open subsets of
positive-entry matrices, each invariant under multiplication by a positive scalar.  If some
matrix \(B\) satisfies \(B\in\mathcal U\) and
\(B^{\circ\thetaq}\in\mathcal V\), then there is a positive integer matrix \(C\) with
\(C^{\circ\thetaq}\) integral, \(C\in\mathcal U\), and
\(C^{\circ\thetaq}\in\mathcal V\).  The same statement holds inside the symmetric
subspace.
\end{corollary}

\begin{proof}
Take the approximations in Theorem~\ref{thm:matrix-projective-density} close enough to remain
in the two open sets, then use their scale invariance.
\end{proof}

\begin{lemma}[Explicit four-dimensional sign change]
\label{lem:matrix-hadamard-sign}
Let \(1<\alpha<2\) and let \(r_1,r_2,r_3,r_4\) be distinct positive reals.  For sufficiently
small \(t>0\), the matrix
\begin{equation}
 H_\alpha(t)=\bigl((1+tr_ir_j)^\alpha\bigr)_{i,j=1}^4
 \label{eq:matrix-hadamard-family}
\end{equation}
has negative determinant.
\end{lemma}

\begin{proof}
For \(t\) small enough, expand
\[
 (1+tr_ir_j)^\alpha
 =\sum_{k\ge0}\binom{\alpha}{k}t^kr_i^kr_j^k.
\]
Cauchy--Binet gives
\begin{align}
 \det H_\alpha(t)
 &=t^6
   \left(\prod_{k=0}^{3}\binom{\alpha}{k}\right)
   \prod_{1\le i<j\le4}(r_j-r_i)^2
   +O(t^7).
 \label{eq:matrix-hadamard-leading}
\end{align}
The coefficients for \(k=0,1,2\) are positive, while
\(\binom{\alpha}{3}<0\) for \(1<\alpha<2\).  The leading coefficient is therefore negative.
\end{proof}

The unpowered matrix \((1+tr_ir_j)\) is positive semidefinite of rank at most two.  Adding a
small positive number to its diagonal makes it positive definite while preserving, by
continuity, the negative determinant of its Hadamard \(\alpha\)-power.

\begin{theorem}[A counterexample realizes Hadamard-power failure integrally]
\label{thm:matrix-integral-hadamard-failure}
Assume \eqref{eq:matrix-counterexample}.  Then there exists a symmetric positive-definite
matrix \(C\in M_4(\Z_{>0})\) such that
\begin{equation}
 C^{\circ\thetaq}\in M_4(\Z_{>0})
 \label{eq:matrix-integral-hadamard}
\end{equation}
and \(C^{\circ\thetaq}\) is indefinite.
\end{theorem}

\begin{proof}
Apply Lemma~\ref{lem:matrix-hadamard-sign} with \(\alpha=\thetaq\), then add a sufficiently
small positive diagonal perturbation.  This produces a symmetric positive-definite
positive-entry matrix \(B\) for which \(B^{\circ\thetaq}\) is nonsingular and indefinite.
Positive definiteness and nonsingular indefiniteness are open, scale-invariant conditions in the
symmetric subspace.  Corollary~\ref{cor:matrix-open-homogeneous-transfer} now supplies the
required integral matrix \(C\).
\end{proof}

\begin{resultbox}
The critical-exponent obstruction is therefore a genuine no-go route.  A counterexample would
not force entrywise \(\thetaq\)-powers to preserve positive semidefiniteness; instead it would
supply a dense arithmetic subgroup on which the known failure can be realized exactly, even
after clearing to positive integers.
\end{resultbox}

\section{Alternants aligned with rational kernel directions}
\label{sec:matrix-rational-alternants}

The rank-one factor \(b=(1,\beta)\) has very good rational approximate orthogonals
\((p,-q)\) when \(p/q\) is a convergent to \(\beta\).  One might therefore choose monomials
whose exponent vectors lie on a lattice line parallel to \((p,-q)\), hoping that the resulting
evaluation determinant is collectively tiny.  Such determinants factor exactly, and the hoped
for collective gain disappears.

\begin{theorem}[Rational-direction alternant factorization]
\label{thm:matrix-line-alternant}
Let \(X_i,Y_i\in\Z_{>0}\) for \(1\le i\le n\), let \(p,q\in\Z_{>0}\), and let
\(0\le m_1<\cdots<m_n\).  Define
\begin{equation}
 V_{ij}=X_i^{pm_j}Y_i^{q(m_n-m_j)},
 \qquad
 z_i=\frac{X_i^p}{Y_i^q}.
 \label{eq:matrix-line-matrix}
\end{equation}
Then
\begin{equation}
 \det V=
 \left(\prod_{i=1}^nY_i^{qm_n}\right)
 \det(z_i^{m_j})_{i,j=1}^n.
 \label{eq:matrix-line-general-factor}
\end{equation}
Moreover
\begin{equation}
 \det(z_i^{m_j})=
 \prod_{1\le i<j\le n}(z_j-z_i)\,S_m(z_1,\ldots,z_n),
 \label{eq:matrix-line-schur}
\end{equation}
where \(S_m\) is a symmetric polynomial with integer coefficients.  In the consecutive case
\(m_j=j-1\), one has \(S_m=1\) and the denominators cancel completely:
\begin{equation}
 \boxed{
 \det\!\left(X_i^{p(j-1)}Y_i^{q(n-j)}\right)_{i,j=1}^n
 =\prod_{1\le i<j\le n}
   \left(X_j^pY_i^q-X_i^pY_j^q\right).}
 \label{eq:matrix-line-pair-factor}
\end{equation}
\end{theorem}

\begin{proof}
Factor \(Y_i^{qm_n}\) from row \(i\), giving
\eqref{eq:matrix-line-general-factor}.  Every alternant
\(\det(z_i^{m_j})\) is divisible by the ordinary Vandermonde; the quotient is the Schur
polynomial associated with the strictly increasing exponent sequence, proving
\eqref{eq:matrix-line-schur}.  For \(m_j=j-1\), the quotient is one.  Substituting
\(z_i=X_i^p/Y_i^q\), the denominator of the Vandermonde is
\(\prod_iY_i^{q(n-1)}\), exactly the row factor in the consecutive case, which gives
\eqref{eq:matrix-line-pair-factor}.
\end{proof}

If \(Y_i=X_i^\beta\) and \(X_1<\cdots<X_n\), then
\begin{equation}
 X_j^pY_i^q-X_i^pY_j^q
 =X_i^{p+q\beta}
  \left(\frac{X_j}{X_i}\right)^{q\beta}
  \left[
   \left(\frac{X_j}{X_i}\right)^{p-q\beta}-1
  \right].
 \label{eq:matrix-line-factor-curve}
\end{equation}
All factors have the same nonzero sign because \(\beta\notin\Q\).  Since they are nonzero
integers, their absolute values are at least one individually.

\begin{corollary}[No collective gain from a convergent direction]
\label{cor:matrix-line-no-gain}
For supports on a rational line, choosing \(p/q\) extremely close to \(\beta\) merely makes the
pairwise binomials in \eqref{eq:matrix-line-pair-factor} small relative to their monomial
summands.  The determinant contains no further cancellation: it is exactly their product,
possibly multiplied by a Schur factor for a support with gaps.
\end{corollary}

For example, if \(|p-q\beta|\log(X_j/X_i)\le1\), the inequality
\(|e^t-1|\le e|t|\) and the integrality of the factor give only
\begin{equation}
 |p-q\beta|\log\frac{X_j}{X_i}
 \ge
 \frac{1}{eX_i^{p+q\beta}(X_j/X_i)^{q\beta}}.
 \label{eq:matrix-line-binary-bound}
\end{equation}
This is a binary gap estimate, not an \(n\)-point amplification.  Any useful near-kernel support
must therefore be genuinely two-dimensional.

\section{Determinantal divisors and exterior powers}
\label{sec:matrix-compounds}

The inherited determinant arguments use only the top determinantal divisor
\(d_N(E)=|\det E|\).  Matrix analysis naturally supplies the whole hierarchy of compounds and
Smith invariant factors.  This section records the exact adelic bridge.

Let \(E\in M_N(\Z)\) be nonsingular.  For \(0\le k\le N\), let \(d_k(E)\) be the greatest
common divisor of all \(k\times k\) minors, with \(d_0(E)=1\).  If
\(s_1\mid s_2\mid\cdots\mid s_N\) are the Smith invariant factors, then
\begin{equation}
 d_k(E)=s_1s_2\cdots s_k.
 \label{eq:matrix-determinantal-divisor}
\end{equation}
For a prime \(p\), use the convention \(v_p(0)=+\infty\) and define the termwise
compound minimum
\begin{equation}
 \tau_{p,k}(E):=
 \min_{\substack{I,J\subseteq\{1,\ldots,N\}\\|I|=|J|=k}}
 \ \min_{\sigma:I\overset{\sim}{\longrightarrow}J}
 \sum_{i\in I}v_p(E_{i,\sigma(i)}),
 \label{eq:matrix-tau-p-k}
\end{equation}
and put
\begin{equation}
 ...
 \label{eq:matrix-Q-k}
\end{equation}
Only finitely many primes occur for the positive-entry matrices used here.

\begin{theorem}[Adelic compound hierarchy]
\label{thm:matrix-adelic-compound}
Let \(\sigma_1(E)\ge\cdots\ge\sigma_N(E)>0\) be the singular values of \(E\).  Then, for every
\(1\le k\le N\),
\begin{align}
 v_p(d_k(E))
  &=\min_{|I|=|J|=k}v_p(\det E_{I,J}),
  \label{eq:matrix-dk-local}\\
 Q_k(E)&\mid d_k(E),
  \label{eq:matrix-Q-divides-d}\\
 d_k(E)&\le\|\textstyle\bigwedge^kE\|_2
       =\prod_{r=1}^k\sigma_r(E).
  \label{eq:matrix-dk-wedge}
\end{align}
Consequently
\begin{equation}
  Q_k(E)\le d_k(E)\le\prod_{r=1}^k\sigma_r(E).
  \label{eq:matrix-compound-sandwich}
\end{equation}
\end{theorem}

\begin{proof}
Equation~\eqref{eq:matrix-dk-local} is the valuation of a gcd.  In every Leibniz term of every
\(k\)-minor, the \(p\)-adic valuation is at least \(\tau_{p,k}(E)\); hence that power of \(p\)
divides every minor, proving \eqref{eq:matrix-Q-divides-d}.  The matrix of
\(\bigwedge^kE\) in the standard orthonormal wedge bases has the \(k\)-minors as its entries.
Thus its operator norm dominates the absolute value of each minor and hence their gcd.  The
singular values of \(\bigwedge^kE\) are all products of \(k\) singular values of \(E\), so its
operator norm is the product of the largest \(k\).
\end{proof}

The Frobenius identity
\begin{equation}
 \|\textstyle\bigwedge^kE\|_F^2
 =\sum_{|I|=|J|=k}|\det E_{I,J}|^2
 \label{eq:matrix-wedge-frobenius}
\end{equation}
is often useful for an upper bound.  It is, however, still a largest-minor estimate unless one
controls cancellation or the singular-value distribution.

\begin{remark}[Where local-to-global loss enters]
For a positive integer matrix, summing the primewise minima gives
\[
 \log Q_k(E)
 =\sum_p\min_{I,J,\sigma}
   \sum_{i\in I}v_p(E_{i,\sigma(i)})\log p.
\]
Moving the minimum outside the sum over places can only increase this quantity.  This is the
same local-to-global loss as for \(k=N\), now present at every exterior degree.
\end{remark}

\section{The entire compound profile on the triangular grid}
\label{sec:matrix-triangular-compounds}

Return to the complete triangular evaluation matrix \(E_T=(e^{x_iy_j})\) of
Section~\ref{sec:fullgrid}, of order \(N_T\).  Order \(x_i\) and \(y_j\) increasingly.  For
\(1\le k\le N_T\), define the partial assignment energies
\begin{align}
 E_{T,k}^-&:=
 \min_{\substack{I,J\subseteq\{1,\ldots,N_T\}\\|I|=|J|=k}}
 \ \min_{\sigma:I\overset{\sim}{\longrightarrow}J}
 \sum_{i\in I}x_i y_{\sigma(i)},
 \label{eq:matrix-partial-Eminus}\\
 E_{T,k}^+&:=
 \max_{\substack{I,J\subseteq\{1,\ldots,N_T\}\\|I|=|J|=k}}
 \ \max_{\sigma:I\overset{\sim}{\longrightarrow}J}
 \sum_{i\in I}x_i y_{\sigma(i)}.
 \label{eq:matrix-partial-Eplus}
\end{align}
Let \(Q_{T,k}=Q_k(E_T)\).

\begin{lemma}[Finite partial-assignment bounds]
\label{lem:matrix-partial-assignment}
For every \(T\) and \(k\),
\begin{align}
 \log Q_{T,k}&\le E_{T,k}^- ,
 \label{eq:matrix-partial-Q-upper}\\
 \|\textstyle\bigwedge^kE_T\|_2
 &\le \binom{N_T}{k}k!\,e^{E_{T,k}^+}.
 \label{eq:matrix-partial-wedge-upper}
\end{align}
Moreover, \(E_{T,k}^-\) is obtained by selecting the \(k\) smallest \(x\)'s and \(y\)'s and
pairing them in reverse order, whereas \(E_{T,k}^+\) is obtained by selecting the \(k\)
largest of each and pairing them in the same order.
\end{lemma}

\begin{proof}
The all-place argument of Lemma~\ref{lem:all-place-transport} applies to every minor and gives
\eqref{eq:matrix-partial-Q-upper}.  Every term of a \(k\)-minor is at most
\(e^{E_{T,k}^+}\), so every such minor is at most \(k!e^{E_{T,k}^+}\).  There are
\(\binom{N_T}{k}^2\) entries in the compound matrix; apply
\eqref{eq:matrix-wedge-frobenius} and take a square root to obtain
\eqref{eq:matrix-partial-wedge-upper}.

All variables are nonnegative.  Replacing a selected coordinate by a smaller unselected one
cannot increase a minimizing cost, and replacing it by a larger one cannot decrease a
maximizing cost.  The rearrangement inequality then gives the reverse and same-order pairings.
\end{proof}

\begin{theorem}[Triangular-grid compound transport profile]
\label{thm:matrix-compound-profile}
Let \(k=k(T)\) satisfy
\begin{equation}
 \frac{k}{N_T}\longrightarrow\rho\in(0,1].
 \label{eq:matrix-rho}
\end{equation}
Then
\begin{align}
 \frac{E_{T,k}^-}{N_TTL_T\log2}
 &\longrightarrow
 \int_0^\rho\sqrt{t(\rho-t)}\,dt
 =\frac{\pi}{8}\rho^2,
 \label{eq:matrix-compound-min-limit}\\
 \frac{E_{T,k}^+}{N_TTL_T\log2}
 &\longrightarrow
 \int_{1-\rho}^{1}t\,dt
 =\rho-\frac{\rho^2}{2}.
 \label{eq:matrix-compound-max-limit}
\end{align}
Consequently
\begin{equation}
 \limsup_{T\to\infty}
 \frac{\log Q_{T,k}}
      {E_{T,k}^++\log\!\left(\binom{N_T}{k}k!\right)}
 \le
 R(\rho):=\frac{\pi\rho}{8-4\rho}.
 \label{eq:matrix-compound-ratio}
\end{equation}
The function \(R\) is strictly increasing on \((0,1]\), with
\begin{equation}
 R(\rho)<R(1)=\frac\pi4
 \qquad(0<\rho<1).
 \label{eq:matrix-proper-compounds-worse}
\end{equation}
\end{theorem}

\begin{proof}
The common normalized empirical quantile of the row and column lists is
\(Q(t)=\sqrt t\) by Lemma~\ref{lem:triangular-law}.  The \(k\) smallest entries correspond to
global quantile parameters in \([0,\rho]\); reverse pairing sends \(t\) to \(\rho-t\).  This
gives the first integral.  The \(k\) largest entries occupy \([1-\rho,1]\), and monotone
pairing gives the second.  The beta integral is
\[
 \int_0^\rho\sqrt{t(\rho-t)}\,dt
 =\rho^2B(3/2,3/2)=\frac\pi8\rho^2.
\]
Since \(N_T\asymp T^2\) while \(N_TTL_T\asymp T^4\),
\[
 \log\binom{N_T}{k}+\log(k!)=O(N_T\log N_T)=o(N_TTL_T).
\]
Apply Lemma~\ref{lem:matrix-partial-assignment}.  Division of the two limits gives
\(R(\rho)\).  Direct differentiation shows \(R'(\rho)>0\).
\end{proof}
...
\(\rho\) & \(1/4\) & \(1/2\) & \(3/4\) & \(1\)\\
\midrule
\(R(\rho)\) & \(0.11220\ldots\) & \(\pi/12=0.26179\ldots\) &
\(0.47123\ldots\) & \(\pi/4=0.78539\ldots\)\\
\bottomrule
\end{tabular}
\end{center}

\begin{resultbox}
Under the inherited termwise \(p\)-adic divisor and largest-Leibniz-term archimedean bounds,
there is no favorable exterior degree.  Sparse compounds are drastically worse, and the best
constant occurs only at the full determinant.  Compounds can help only through information not
present in these two bounds: the true singular-value free energy, cancellation among minors, or
extra divisibility of the determinantal divisors beyond \(Q_{T,k}\).
\end{resultbox}

\subsection{A precise singular-value target}

Define, whenever the limit exists,
\begin{equation}
 \Phi(\rho):=
 \lim_{T\to\infty}
 \frac{\log\|\bigwedge^{\lfloor\rho N_T\rfloor}E_T\|_2}
      {N_TTL_T\log2},
 \label{eq:matrix-compound-free-energy}
\end{equation}
and define the arithmetic profile
\begin{equation}
 \Psi(\rho):=
 \liminf_{T\to\infty}
 \frac{\log Q_{T,\lfloor\rho N_T\rfloor}}
      {N_TTL_T\log2}.
 \label{eq:matrix-compound-arithmetic-profile}
\end{equation}
The compound sandwich \eqref{eq:matrix-compound-sandwich} would contradict a counterexample if
\begin{equation}
  \Psi(\rho)>\Phi(\rho)
  \label{eq:matrix-compound-decisive-inequality}
\end{equation}
for any \(\rho\).  The present work proves only
\begin{equation}
 \Psi(\rho)\le\frac\pi8\rho^2,
 \qquad
 \Phi(\rho)\le\rho-\frac{\rho^2}{2},
 \label{eq:matrix-compound-known-bounds}
\end{equation}
which are in the wrong direction for comparison.  Computing either profile sharply is a
well-posed matrix-analysis problem.  The recent coupled-HCIZ work discussed in
Section~\ref{sec:matrix-literature} suggests analytic tools, but its rank regime does not yet
cover this full-rank triangular model.

\section{Finite differences, boundary displacement, and transfer determinants}
\label{sec:matrix-displacement}

The polynomial boundary defects in Section~\ref{subsec:boundary-compression} have a direct
matrix realization.  It makes the \(O(K)\)-boundary versus \(K^2\)-interior compression exact
at the level of rank.

Let \(\mathcal I_K=\{0,\ldots,K-1\}^2\), set \(N=K^2\), and form the square integer matrix
\begin{equation}
 \mathcal E_K[(i,j),(u,v)]
 :=(2^i3^j)^u(M^iA^j)^v.
 \label{eq:matrix-grid-evaluation}
\end{equation}
Equivalently, with
\begin{equation}
 d_{2;u,v}=2^uM^v,
 \qquad
 d_{3;u,v}=3^uA^v,
 \label{eq:matrix-column-multipliers}
\end{equation}
one has
\begin{equation}
 \mathcal E_K[(i,j),(u,v)]
 =d_{2;u,v}^{\,i}d_{3;u,v}^{\,j}.
 \label{eq:matrix-grid-separable}
\end{equation}
The row parameters \(i\log2+j\log3\) and column frequencies \(u+\beta v\) are pairwise
distinct, so strict total positivity of \(e^{xy}\), after sorting, implies that
\(\mathcal E_K\) is invertible.

\subsection{A unimodular finite-difference factorization}

There is a canonical source of Smith divisibility in every integer evaluation matrix: integer
finite differences convert monomials into the binomial-polynomial basis.  On the present
square grid this contribution can be computed in every exterior degree.

For \(0\le r,i<K\), let
\begin{equation}
 \mathcal B_K[r,i]=
 \begin{cases}
 (-1)^{r-i}\binom ri,&i\le r,\\
 0,&i>r.
 \end{cases}
 \label{eq:matrix-binomial-difference}
\end{equation}
This is a unit lower-triangular integer matrix and hence is unimodular.  Put
\begin{align}
 \mathcal F_K&:=(\mathcal B_K\otimes\mathcal B_K)\mathcal E_K,
 \label{eq:matrix-finite-difference-transform}\\
 \mathcal H_K[(a,b),(u,v)]
 &:=\binom{d_{2;u,v}-1}{a}\binom{d_{3;u,v}-1}{b},
 \label{eq:matrix-binomial-evaluation}\\
 \mathcal D_K[(a,b),(a,b)]&:=a!b!,
 \label{eq:matrix-factorial-diagonal}
\end{align}
with all off-diagonal entries of \(\mathcal D_K\) zero.

\begin{lemma}[Finite differences and the binomial basis]
\label{lem:matrix-finite-difference-factorization}
For all grid indices,
\begin{equation}
 \mathcal F_K[(r,s),(u,v)]
 =(d_{2;u,v}-1)^r(d_{3;u,v}-1)^s.
 \label{eq:matrix-finite-difference-entry}
\end{equation}
Moreover \(\mathcal H_K\in M_{K^2}(\Z)\), and there is a unimodular integer matrix
\(\mathcal U_K\) such that
\begin{equation}
 \mathcal F_K=\mathcal U_K\mathcal D_K\mathcal H_K.
 \label{eq:matrix-stirling-factorization}
\end{equation}
Consequently \(\mathcal E_K\) is left-unimodularly equivalent to
\(\mathcal D_K\mathcal H_K\).
\end{lemma}

\begin{proof}
The binomial theorem gives
\[
 \sum_{i=0}^r(-1)^{r-i}\binom ri X^i=(X-1)^r;
\]
applying it in both row coordinates proves
\eqref{eq:matrix-finite-difference-entry}.  The entries of \(\mathcal H_K\) are integers
because the two arguments of the binomial coefficients are nonnegative integers.  Finally,
\[
 X^r=\sum_{a=0}^r
 \genfrac\{\}{0pt}{}{r}{a}\,a!\binom Xa,
\]
where the braces denote Stirling numbers of the second kind.  Applying this identity in each
coordinate gives \eqref{eq:matrix-stirling-factorization}; the tensor product of the two
Stirling matrices is unit lower triangular, hence unimodular.  Multiplying on the left by its
inverse and by \(\mathcal B_K\otimes\mathcal B_K\) proves the final assertion.
\end{proof}

For each prime \(p\), arrange the multiset
\begin{equation}
 \bigl\{v_p(a!b!):0\le a,b<K\bigr\}
 =\{\omega_{p,1}\le\cdots\le\omega_{p,K^2}\}
 \label{eq:matrix-factorial-valuations}
\end{equation}
in nondecreasing order, with multiplicity, and define
\begin{equation}
 \mathfrak F_{K,k}:=
 \prod_p p^{\sum_{\ell=1}^k\omega_{p,\ell}}
 \qquad(1\le k\le K^2).
 \label{eq:matrix-factorial-divisor}
\end{equation}
Only primes below \(K\) occur.

\begin{theorem}[Factorial divisors of every compound]
\label{thm:matrix-factorial-smith}
For every \(1\le k\le K^2\),
\begin{equation}
 \mathfrak F_{K,k}\mid d_k(\mathcal E_K).
 \label{eq:matrix-factorial-divides-dk}
\end{equation}
At the top degree,
\begin{equation}
 \left(\prod_{r=0}^{K-1}r!\right)^{2K}
 =\mathfrak F_{K,K^2}
 \mid\det\mathcal E_K.
 \label{eq:matrix-full-factorial-divisor}
\end{equation}
Furthermore
\begin{equation}
 \log\mathfrak F_{K,K^2}
 =K^3\log K-\frac32K^3+O(K^2\log K),
 \label{eq:matrix-factorial-asymptotic}
\end{equation}
and, uniformly in \(k\),
\begin{equation}
 0\le\log\mathfrak F_{K,k}=O(K^3\log K).
 \label{eq:matrix-factorial-uniform}
\end{equation}
\end{theorem}

\begin{proof}
Determinantal divisors are invariant under left multiplication by unimodular integer matrices,
so Lemma~\ref{lem:matrix-finite-difference-factorization} reduces the question to
\(\mathcal D_K\mathcal H_K\).  A \(k\)-minor using a row set \(I\) factors as
\[
 \det(\mathcal D_K\mathcal H_K)_{I,J}
 =\left(\prod_{(a,b)\in I}a!b!\right)\det(\mathcal H_K)_{I,J}.
\]
Its \(p\)-adic valuation is therefore at least the sum of the \(k\) smallest values in
\eqref{eq:matrix-factorial-valuations}.  This holds for every minor and every prime, proving
\eqref{eq:matrix-factorial-divides-dk}.  Taking all rows gives
\[
 \prod_{0\le a,b<K}a!b!
 =\left(\prod_{r=0}^{K-1}r!\right)^{2K},
\]
which proves \eqref{eq:matrix-full-factorial-divisor}.  Finally, the standard Barnes--Stirling
estimate
\[
 \sum_{r=0}^{K-1}\log(r!)
 =\frac12K^2\log K-\frac34K^2+O(K\log K)
\]
gives \eqref{eq:matrix-factorial-asymptotic}.  Since all valuations are nonnegative,
\(\mathfrak F_{K,k}\le\mathfrak F_{K,K^2}\), proving the uniform estimate.
\end{proof}

\begin{resultbox}
The finite-difference transform removes a universal source of hidden Smith content from the
search.  Its full contribution is only \(\exp(O(K^3\log K))\), whereas the determinant-height
comparisons for the \(K^2\)-point grid live on the \(K^4\) scale.  Thus factorial divisibility
cannot close the argument by itself.  Any decisive extra Smith growth must come from the
conditional arithmetic of the residual integer matrix \(\mathcal H_K\), not merely from the
integer-valued-polynomial basis.
\end{resultbox}

\subsection{Exact rank-\texorpdfstring{$K$}{K} boundary displacement}

Let \(D_2,D_3\) be the diagonal matrices with entries \(d_{2;u,v}\) and \(d_{3;u,v}\).
Define truncated shifts on row-indexed vectors by
\begin{align}
 (S_2z)_{i,j}&=
 \begin{cases}z_{i+1,j},&i<K-1,\\0,&i=K-1,\end{cases}
 \label{eq:matrix-S2}\\
 (S_3z)_{i,j}&=
 \begin{cases}z_{i,j+1},&j<K-1,\\0,&j=K-1.\end{cases}
 \label{eq:matrix-S3}
\end{align}
Let \(P_2\) and \(P_3\) be the coordinate projections onto the terminal boundaries
\(i=K-1\) and \(j=K-1\), respectively.  Both have rank \(K\).

\begin{theorem}[Exact displacement-rank identities]
\label{thm:matrix-displacement-rank}
The evaluation matrix satisfies
\begin{align}
 S_2\mathcal E_K-\mathcal E_KD_2
  &=-P_2\mathcal E_KD_2,
 \label{eq:matrix-displacement-2}\\
 S_3\mathcal E_K-\mathcal E_KD_3
  &=-P_3\mathcal E_KD_3.
 \label{eq:matrix-displacement-3}
\end{align}
Hence each displacement has rank exactly \(K\).  The rational multiplication operators
\begin{equation}
 T_2:=\mathcal E_KD_2\mathcal E_K^{-1},
 \qquad
 T_3:=\mathcal E_KD_3\mathcal E_K^{-1}
 \label{eq:matrix-T2T3}
\end{equation}
have the boundary-feedback forms
\begin{equation}
 T_2=S_2+R_2,
 \qquad
 T_3=S_3+R_3,
 \qquad
 \rank R_2=\rank R_3=K.
 \label{eq:matrix-boundary-feedback}
\end{equation}
They commute, and therefore
\begin{equation}
 [S_2,R_3]+[R_2,S_3]+[R_2,R_3]=0.
 \label{eq:matrix-boundary-commutator}
\end{equation}
\end{theorem}

\begin{proof}
For \(i<K-1\), shifting the row index from \(i\) to \(i+1\) multiplies column \((u,v)\) by
\(d_{2;u,v}\); on the terminal boundary the truncated shift is zero.  This is exactly
\eqref{eq:matrix-displacement-2}, and the triadic identity is identical.  Since all other
factors are invertible, the right sides have the ranks of \(P_2\) and \(P_3\), namely \(K\).
Rearranging and multiplying by \(\mathcal E_K^{-1}\) gives
\eqref{eq:matrix-boundary-feedback}.  Finally \(D_2D_3=D_3D_2\), so \(T_2T_3=T_3T_2\);
expand the commutator and use \([S_2,S_3]=0\).
\end{proof}

The operators \(T_2,T_3\) preserve the full-rank lattice
\(\mathcal E_K\Z^N\), even though their matrices in the standard basis are generally only
rational.  The low-rank boundary corrections are precisely what makes the two nilpotent shifts
carry the integer spectra \(\{2^uM^v\}\) and \(\{3^uA^v\}\).

\subsection{Compression of a \texorpdfstring{$K^2$}{K-squared}-point spectrum to a \texorpdfstring{$K\times K$}{K-by-K} transfer determinant}

Choose an isometric coordinate injection \(J_2:\R^K\to\R^N\) onto the boundary
\(i=K-1\), so \(P_2=J_2J_2^{\mathsf T}\), and put
\begin{equation}
 C_2:=J_2^{\mathsf T}T_2,
 \qquad R_2=J_2C_2.
 \label{eq:matrix-C2}
\end{equation}
Define \(J_3,C_3\) similarly.

\begin{theorem}[Boundary transfer determinant]
\label{thm:matrix-transfer-determinant}
For \(z\ne0\),
\begin{align}
 \prod_{0\le u,v<K}(z-2^uM^v)
 &=z^{K^2}
 \det\!\left(
 I_K-C_2(zI_N-S_2)^{-1}J_2
 \right),
 \label{eq:matrix-transfer-2}\\
 \prod_{0\le u,v<K}(z-3^uA^v)
 &=z^{K^2}
 \det\!\left(
 I_K-C_3(zI_N-S_3)^{-1}J_3
 \right).
 \label{eq:matrix-transfer-3}
\end{align}
Furthermore
\begin{equation}
 (zI_N-S_r)^{-1}
 =\sum_{h=0}^{K-1}z^{-h-1}S_r^h,
 \qquad r\in\{2,3\}.
 \label{eq:matrix-shift-resolvent}
\end{equation}
Thus a characteristic polynomial of degree \(K^2\) is encoded by a \(K\times K\) determinant
whose entries are Laurent polynomials of depth at most \(K\).
\end{theorem}

\begin{proof}
The left side of \eqref{eq:matrix-transfer-2} is \(\det(zI_N-T_2)\).  Since
\(T_2=S_2+J_2C_2\), the matrix determinant lemma and Sylvester's identity give
\begin{align*}
 \det(zI_N-T_2)
 &=\det(zI_N-S_2)
   \det\!\left(I_K-C_2(zI_N-S_2)^{-1}J_2\right).
\end{align*}
The shift \(S_2\) is nilpotent, so \(\det(zI_N-S_2)=z^N\).  Also \(S_2^K=0\), which gives
the finite resolvent expansion.  The proof for \(T_3\) is the same.
\end{proof}

There is also a unique polynomial \(\widehat Q_K\in\Q[X]\) of degree below \(K^2\) satisfying
\begin{equation}
 \widehat Q_K(2^uM^v)=3^uA^v
 \qquad(0\le u,v<K),
 \label{eq:matrix-dual-interpolant}
\end{equation}
and simultaneous diagonalization gives the exact matrix identity
\begin{equation}
 T_3=\widehat Q_K(T_2).
 \label{eq:matrix-polynomial-operator-relation}
\end{equation}
This is the operator dual of the graph interpolant in
Section~\ref{subsec:boundary-compression}.  Equations
\eqref{eq:matrix-boundary-commutator}, \eqref{eq:matrix-transfer-2}, and
\eqref{eq:matrix-polynomial-operator-relation} isolate the remaining arithmetic in two
rank-\(K\) boundary feedback matrices.

\begin{target}[Smith theory of the boundary transfer matrices]
\label{target:matrix-boundary-smith}
Determine the primitive denominators, local Smith factors, and \(2\)- and \(3\)-adic Newton
polygons of \(C_2\) and \(C_3\), preferably after a lattice-adapted basis change.  Any
leading-order divisibility forced by the commuting relation
\eqref{eq:matrix-boundary-commutator} would be invisible to the termwise full-grid ceiling and
would act on only \(O(K)\) boundary data.
\end{target}

\section{Kraus--Loewner matrices from operator convexity}
\label{sec:matrix-kraus}

The function
\begin{equation}
 f(t)=t^{\thetaq},\qquad 1<\thetaq<2,
 \label{eq:matrix-power-function}
\end{equation}
is operator convex on \((0,\infty)\) \cite{BhatiaMatrixAnalysis}.  More concretely, it has the integral representation
\begin{equation}
 t^{\thetaq}
 =c_{\thetaq}\int_0^\infty
   \frac{t^2}{t+s}s^{\thetaq-2}\,ds,
 \qquad
 c_{\thetaq}:=
 \frac{\sin(\pi(\thetaq-1))}{\pi}>0.
 \label{eq:matrix-power-integral}
\end{equation}
The second divided-difference kernel therefore has a positive Cauchy-transform
representation.

For pairwise distinct \(x,a,y>0\), write
\begin{equation}
 K_a(x,y):=f[x,a,y]
 =\sum_{\xi\in\{x,a,y\}}
   \frac{f(\xi)}{\prod_{\eta\in\{x,a,y\}\setminus\{\xi\}}(\xi-\eta)}.
 \label{eq:matrix-second-divided-difference}
\end{equation}
When nodes coincide, \(K_a(x,y)\) is understood by continuous extension; the integral formula
below provides that extension directly.

\begin{lemma}[Positive integral representation]
\label{lem:matrix-kraus-integral}
For \(x,a,y>0\),
\begin{equation}
 K_a(x,y)=
 c_{\thetaq}\int_0^\infty
 \frac{s^{\thetaq}}
 {(x+s)(a+s)(y+s)}\,ds.
 \label{eq:matrix-kraus-kernel}
\end{equation}
\end{lemma}

\begin{proof}
The beta integral evaluates \eqref{eq:matrix-power-integral}.  For fixed \(s\),
\[
 \frac{t^2}{t+s}=t-s+\frac{s^2}{t+s}.
\]
The second divided difference kills the affine part and sends \((t+s)^{-1}\) to
\(((x+s)(a+s)(y+s))^{-1}\).  Multiplication by \(s^{\thetaq-2}s^2\) gives
\eqref{eq:matrix-kraus-kernel}.
\end{proof}

\begin{theorem}[Strict total positivity and an exact determinant integral]
\label{thm:matrix-kraus-determinant}
Let
\[
 0<x_1<\cdots<x_n,
 \qquad
...
and let \(a>0\).  Then
\begin{align}
 &\det\bigl(K_a(x_i,y_j)\bigr)_{i,j=1}^n
 \notag\\
 &\quad=
 \frac{c_{\thetaq}^{\,n}\Delta(x)\Delta(y)}{n!}
 \int_{(0,\infty)^n}
 \frac{\Delta(s)^2
       \prod_{r=1}^n s_r^{\thetaq}/(a+s_r)}
      {\prod_{i,r}(x_i+s_r)\prod_{j,r}(y_j+s_r)}
 \,ds_1\cdots ds_n
 >0,
 \label{eq:matrix-kraus-determinant-integral}
\end{align}
where \(\Delta(x)=\prod_{i<j}(x_j-x_i)\).  In particular the cross-kernel
\(K_a(x,y)\) is strictly totally positive.
\end{theorem}

\begin{proof}
Apply the continuous Cauchy--Binet, or Andr\'eief, identity to
\eqref{eq:matrix-kraus-kernel} with
\[
 \phi_i(s)=\frac1{x_i+s},\qquad
 \psi_j(s)=\frac1{y_j+s},\qquad
 w(s)=\frac{s^{\thetaq}}{a+s}.
\]
The two Cauchy determinants are
\[
 \det\!\left(\frac1{x_i+s_r}\right)
 =\frac{\Delta(x)\Delta(s)}{\prod_{i,r}(x_i+s_r)},
\]
and the analogous expression for \(y\).  Their product gives
\eqref{eq:matrix-kraus-determinant-integral}, whose integrand is positive off the diagonals.
The same calculation for every square submatrix proves strict total positivity.
\end{proof}

\subsection{Arithmetic denominator clearing}

Suppose \(a,x_i,y_j\) are positive integers and their \(\thetaq\)-th powers are integers.  Then
every entry \(K_a(x_i,y_j)\) is rational.  Define
\begin{equation}
 \mathcal D_a(X,Y):=
 \left(\prod_{i=1}^n(x_i-a)\right)
 \left(\prod_{j=1}^n(a-y_j)\right)
 \left(\prod_{i,j=1}^n(x_i-y_j)\right).
 \label{eq:matrix-kraus-denominator}
\end{equation}

\begin{proposition}[Integral Kraus determinant]
\label{prop:matrix-kraus-integral-integer}
If all nodes in \eqref{eq:matrix-kraus-denominator} are distinct, then
\begin{equation}
 \mathcal D_a(X,Y)
 \det\bigl(K_a(x_i,y_j)\bigr)_{i,j=1}^n
 \in\Z\setminus\{0\}.
 \label{eq:matrix-kraus-integer}
\end{equation}
\end{proposition}

\begin{proof}
Formula~\eqref{eq:matrix-second-divided-difference} shows that the denominator of entry
\((i,j)\) divides
\((x_i-a)(a-y_j)(x_i-y_j)\).  In a determinant term, the first factors occur once per row,
the second once per column, and the cross factors occur for one perfect matching.  The product
\(\mathcal D_a(X,Y)\) contains every required factor and therefore clears every term.
Nonvanishing follows from Theorem~\ref{thm:matrix-kraus-determinant}.
\end{proof}

A small unconditional calibration uses only powers of two.  With
\begin{equation}
 a=2,
 \qquad X=(1,4),
 \qquad Y=(8,16),
 \label{eq:matrix-kraus-example-nodes}
\end{equation}
one obtains exactly
\begin{equation}
 \bigl(K_2(x_i,y_j)\bigr)=
 \begin{pmatrix}
 2/7&5/21\\[2pt]
 1/4&3/14
 \end{pmatrix},
 ...
 \det=\frac1{588},
 \qquad
 \mathcal D_2(X,Y)\det=-1440.
 \label{eq:matrix-kraus-example}
\end{equation}

\subsection{Why crude denominator clearing cannot finish}

If all nodes are scaled by \(R>0\), homogeneity gives
\begin{equation}
 K_{Ra}(Rx,Ry)=R^{\thetaq-2}K_a(x,y).
 \label{eq:matrix-kraus-homogeneity-kernel}
\end{equation}
The denominator product in \eqref{eq:matrix-kraus-denominator} has degree \(n^2+2n\).
Therefore
\begin{equation}
 \mathcal D_{Ra}(RX,RY)
 \det\bigl(K_{Ra}(Rx_i,Ry_j)\bigr)
 =R^{n^2+n\thetaq}
  \mathcal D_a(X,Y)
  \det\bigl(K_a(x_i,y_j)\bigr).
 \label{eq:matrix-kraus-homogeneity-integer}
\end{equation}
The exponent \(n^2+n\thetaq\) is positive.  The derivative decay of the divided differences is
therefore overwhelmed by the naive common denominator.

\begin{target}[Primitive Kraus numerators]
\label{target:matrix-kraus-primitive}
Determine the primitive denominator and the primewise content of the determinant before the
full cross-product \(\prod_{i,j}(x_i-y_j)\) is imposed.  The positive integral
\eqref{eq:matrix-kraus-determinant-integral} supplies an exact archimedean object, while the
candidate condition supplies rationality at every entry.  A proof would require systematic
cancellation in the denominator or additional divisibility in the numerator; homogeneity alone
cannot provide it.
\end{target}

\section{Recent literature and its precise relevance}
\label{sec:matrix-literature}

The literature search for this continuation covered arXiv and journal records through
August~21, 2026.  The following items are the most relevant to the matrix routes above.

\subsection{Matrix Coefficient and structural rank}

Dasgupta and Kakde's Matrix Coefficient paper, first posted in 2024 and published in
\emph{Advances in Mathematics} in 2026, remains the closest general conjectural framework
\cite{DasguptaKakdeMCC}.  It predicts that a singular square matrix of logarithms of algebraic
numbers acquires a zero coefficient after rational changes of basis.  In dimension two it is
equivalent to the Four Exponentials Conjecture.  Their support condition~(m) implies the
required multiplicative orthogonality, but the analytic implication from singularity to the
existence of the auxiliary polynomials remains the missing step.  The exact factor-four
interpolation obstruction in Section~\ref{sec:newton-threshold} shows how severe that step is
for the present grid.

No arXiv preprint located in the search claims a proof of the Matrix Coefficient Conjecture, the
Four Exponentials Conjecture, or this fixed-base Alaoglu--Erd\H{o}s case.

\subsection{New effective Diophantine approximation}

Calegari, Dimitrov, and Tang develop arithmetic-holonomy methods yielding effective
irrationality and linear-independence measures and recovering classical effective results such
as two-variable \(S\)-unit and Thue--Mahler finiteness by a route different from standard
linear forms in logarithms \cite{CalegariDimitrovTang2025}.  This is important methodological
progress, but the quantities controlled are still linear or multiplicative Diophantine
approximations.  The obstruction here is the bilinear coefficient
\((u\cdot a)(b\cdot v)\) or, equivalently, a product of two logarithmic linear forms.  No bound
in that paper supplies the exact rational orthogonality required by
\eqref{eq:matrix-rank-one-log}.

Orr gives explicit height bounds for unexpected global polynomial relations among values of
G-functions and applies them to unlikely intersections of lines with algebraic tori
\cite{Orr2026}.  The global-relations philosophy is relevant to the search for an adelic
certificate, but the graph \(y=x^\beta\) generated by a hypothetical transcendental \(\beta\)
is not an algebraic line in a torus and is not presently covered by the G-function framework.

\subsection{Compounds, total positivity, and HCIZ asymptotics}

Fallat, Gupta, and Johnson show that compounds of totally positive matrices need not themselves
retain the expected stronger total-positivity properties and study Dodgson condensation in
this setting \cite{FallatGuptaJohnson2024}.  Their negative results reinforce the need to work
with exact exterior-power norms and Smith factors rather than assuming that total positivity
automatically becomes stronger after taking compounds.  Johnson and Richards extend the
classical total positivity of \(e^{xy}\) to hyperdeterminantal kernels
\(e^{x_1\cdots x_{2m}}\) \cite{JohnsonRichards2025}.  This broadens the available positivity
architecture, but no current hyperdeterminantal construction changes the arithmetic density
mismatch in the present two-variable grid.

Bohnacker and M\"uller study asymptotic exponents of coupled HCIZ integrals using
operator-valued free-probability transforms \cite{BohnackerMuller2026}.  Their results concern
finitely many coupled rank-one integrals and ranks up to
\(O(N^{1/2-\varepsilon})\).  The triangular determinant and its compound profile require a
full-rank joint spectral limit.  The paper is therefore a useful pointer toward analytic
technology, not a ready-made evaluation of \(\Phi(\rho)\) in
\eqref{eq:matrix-compound-free-energy}.

\subsection{Additive relations in irrational powers}

Harrison proves near-minimal additive energy for irrational powers of sufficiently dense
subsets of arithmetic progressions, uniformly in the irrational exponent, using functional
transcendence and Pila--Wilkie counting \cite{Harrison2026}.  He also gives a Diophantine
approximation criterion ensuring nonvanishing of certain rational linear forms in irrational
powers of multiplicatively independent integers.  These are unusually close in theme to the
present problem, but their hypotheses do not match the conditional candidate set: the square
multiplicative grid \(\{2^uM^v\}\) is not a dense subset of an arithmetic progression, and the
determinants here encode many-term multilinear cancellation rather than a single additive
relation.  The paper nevertheless suggests a concrete future test: seek a definable-family
point-counting theorem directly for the two-parameter multiplicative grid, rather than first
embedding it in an additive progression.

\subsection{Recent formalization infrastructure}

Karatarakis and Wiedijk formalize the Gelfond--Schneider theorem in Lean~4, including the
number-field version of Siegel's lemma and the auxiliary-function estimates
\cite{KaratarakisWiedijk2026}.  This does not supply the linear-forms-in-logarithms or
four-exponentials input missing here, but it materially changes the formalization landscape:
parts of the analytic and algebraic infrastructure needed for transcendence proofs now have a
public Mathlib-oriented implementation that can be compared with, and potentially reused by,
the ProveIt development.

\subsection{Rigidity that bypasses Four Exponentials by extra structure}

Bustos-Gajardo, Fokkink, and Yassawi prove a Cobham theorem for Gaussian integer numeration
without assuming Four Exponentials, in a setting where earlier work used that conjecture
\cite{BustosFokkinkYassawi2026}.  Their proof exploits automaticity and pumping-generated
periods, structure absent from a bare pair of integer powers.  The lesson is nevertheless
constructive: a four-exponentials barrier can sometimes be bypassed when an independent
finite-state or recurrence structure manufactures exact periods.  No analogous automatic
structure is presently known for the conditional monoid
\(\Nzero+\Nzero\beta\).

\section{Updated matrix research program}
\label{sec:matrix-research-program}

The matrix work above converts several vague possibilities into exact, falsifiable targets.
They are ordered here by their apparent leverage.

\subsection{Priority 1: compute a joint arithmetic/singular-value compound profile}

For one or more fixed densities \(\rho\), determine both \(\Phi(\rho)\) and
\(\Psi(\rho)\) from \eqref{eq:matrix-compound-free-energy}--
\eqref{eq:matrix-compound-arithmetic-profile}.  The decisive inequality is
\eqref{eq:matrix-compound-decisive-inequality}.  There are two separate tasks:
\begin{enumerate}
\item obtain a genuine singular-value or HCIZ/free-energy bound, rather than the largest-term
      estimate \(\rho-\rho^2/2\);
\item compute primewise assignment minima sharply enough to get a lower bound for
      \(Q_{T,k}\), rather than the all-place upper ceiling \(\pi\rho^2/8\).
\end{enumerate}
Theorem~\ref{thm:matrix-compound-profile} proves that changing \(k\) alone cannot substitute
for either task.  Theorem~\ref{thm:matrix-factorial-smith} also removes the canonical
finite-difference contribution from consideration: its \(O(K^3\log K)\) logarithm is too small,
so the desired arithmetic profile must detect genuinely conditional content in
\(\mathcal H_K\) or in another lattice-adapted factor.

\subsection{Priority 2: exploit the commuting boundary feedback}

The transfer matrices in \eqref{eq:matrix-transfer-2}--\eqref{eq:matrix-transfer-3} reduce
\(K^2\) spectral data to rank-\(K\) feedback.  Compute their Smith forms over
\(\Z[1/(6MA)]\), their primitive common denominators, and the effect of the exact commutator
\eqref{eq:matrix-boundary-commutator}.  A useful theorem would have the form
\begin{equation}
  \text{forced local content of }C_2,C_3
  \ >\ \text{archimedean boundary norm}.
  \label{eq:matrix-boundary-target-form}
\end{equation}
This route is genuinely different from the factor-four auxiliary-polynomial obstruction,
because it acts on the boundary realization rather than on a polynomial vanishing at all
\(K^2\) points.

\subsection{Priority 3: primitive Kraus determinants}

The Kraus matrix simultaneously supplies strict positivity, rational entries, and an exact
multiple integral.  Search for node configurations from the conditional grid for which the
primitive denominator of \eqref{eq:matrix-kraus-integer} is exponentially smaller than the
crude cross-product denominator.  Exact experiments should record:
\begin{itemize}
\item the reduced numerator and denominator of every minor;
\item their \(2\)-, \(3\)-, and primitive-odd valuations;
\item the comparison between the integral determinant and the reduced arithmetic height.
\end{itemize}
The homogeneity identity \eqref{eq:matrix-kraus-homogeneity-integer} should be used as a guard:
any experiment explained solely by common scaling cannot close the problem.

\subsection{Priority 4: genuinely two-dimensional near-kernel supports}

Theorem~\ref{thm:matrix-line-alternant} eliminates all supports contained in a rational line.
The next support class should have bounded width transverse to a continued-fraction direction,
not width zero.  For a strip of width \(w\), determine whether the alternant decomposes into a
block resultant or a matrix-valued orthogonal polynomial whose determinant has extra
arithmetic content.  Width one is the first nontrivial case.  A useful outcome would be either
an improved divisor/height ratio or another exact factorization no-go theorem.

\subsection{Priority 5: search for extra exact dynamics}

The Gaussian Cobham result suggests looking for finite-state or substitution structure in the
conditional block word
\[
  T\longmapsto\#\bigl(\Sol\cap[T,T+1)\bigr)
  =\left\lfloor\frac{T+1}{\beta}\right\rfloor
   -\left\lfloor\frac{T}{\beta}\right\rfloor.
\]
This is Sturmian rather than automatic for irrational \(\beta\).  A proof route would need an
additional arithmetic recurrence tying this word to the integer outputs \(2^x,3^x\).  Pure
symbolic complexity is already fully compatible with a counterexample.

\section{Formalization blueprint for the new matrix results}
\label{sec:matrix-formalization}

Most of the exact algebra in this continuation is suitable for Lean without new transcendence
infrastructure.

\begin{center}
\small
\begin{tabularx}{0.97\textwidth}{@{}p{0.36\textwidth}X@{}}
\toprule
Proposed module & Main content\\
\midrule
\lean{RationalDirectionAlternant} & Row-factorization, ordinary Vandermonde cancellation,
consecutive pair-product identity \eqref{eq:matrix-line-pair-factor}, and nonvanishing on an
irrational power curve.\\
\lean{DeterminantalDivisorCompound} & Determinantal divisors, valuation of a gcd, termwise
compound divisibility, and the finite inequality \(Q_k\mid d_k\).\\
\lean{ExteriorPowerMinorBound} & Compound matrix entries, Frobenius sum of squared minors, and
finite largest-term bounds.  The singular-value equality may be isolated behind a standard
linear-algebra interface if Mathlib lacks the required theorem.\\
\lean{TriangularCompoundTransport} & Finite subset/rearrangement lemmas and exact beta-integral
constants.  The empirical-quantile limit can initially remain paper-level, matching the
existing full-grid module plan.\\
\lean{FiniteDifferenceSmith} & Unimodularity of the binomial-difference and Stirling
transforms, the factorization \eqref{eq:matrix-stirling-factorization}, and the finite divisibility
\(\mathfrak F_{K,k}\mid d_k(\mathcal E_K)\).\\
\lean{BoundaryDisplacement} & Shift/projector identities
\eqref{eq:matrix-displacement-2}--\eqref{eq:matrix-displacement-3}, exact rank \(K\), commuting
feedback, and the finite matrix determinant lemma.\\
\lean{ProjectiveCandidateDensity} & Density of \(\Z+\beta\Z\), entrywise approximation in the
positive cone, and simultaneous denominator clearing by one element of \(G\).\\
\lean{HadamardPowerNoGo} & Formal power-series leading term for the \(4\times4\) determinant.
The continuity transfer can use \lean{ProjectiveCandidateDensity}; only the open-cone spectral
facts require additional topological linear algebra.\\
\lean{KrausDividedDifference} & Algebraic divided-difference denominator clearing.  The
positive integral and Andr\'eief identity should remain an analytic interface until the needed
measure-theory determinant infrastructure exists.\\
\bottomrule
\end{tabularx}
\end{center}

A useful first formalization wave is therefore entirely finite: the line alternant, the
compound divisor hierarchy, the finite-difference Smith factorization, and the displacement-rank
identities.  These results would enlarge the kernel-checked no-go ledger even before any
asymptotic or integral theorem is formalized.

\section{Conclusions of the matrix continuation}
\label{sec:matrix-conclusions}

The matrix-analysis program did not close the Alaoglu--Erd\H{o}s conjecture.  It did produce a
more exact map of the remaining terrain.

First, rational basis reduction is quantitatively understood.  The smallest bounded coefficient
of the singular logarithm matrix factors into two ordinary one-dimensional approximation
problems and is automatically \(O(B^{-2})\).  Numerical near-orthogonality is therefore cheap;
exact rational orthogonality remains the Matrix Coefficient barrier.

Second, a counterexample would make its arithmetic matrix locus projectively dense in every
finite dimension: after one common scalar, arbitrary positive matrices and their entrywise
\(\thetaq\)-th powers can be approximated simultaneously by integer matrices.  Consequently
entrywise-power non-preservation is compatible with a counterexample even for positive-definite
integer matrices.  Determinants supported on rational near-kernel lines also factor completely
into pairwise binomials and Schur factors.

Third, the determinant method now has a full compound/Smith hierarchy.  On the complete
triangular grid, the density-\(\rho\) termwise ceiling is
\[
  R(\rho)=\frac{\pi\rho}{8-4\rho},
\]
strictly below \(\pi/4\) for every proper compound.  Thus exterior powers do not improve the
known argument unless one obtains true singular-value cancellation or new determinantal-divisor
content.  A unimodular finite-difference transform identifies the most obvious such content:
the factorial divisor of every compound.  Its logarithm is only \(O(K^3\log K)\), so it is
lower order and leaves the required \(K^4\)-scale gain inside a residual integer matrix.

Fourth, the conditional grid has exact matrix displacement rank \(K\).  Two commuting
rank-\(K\) boundary feedbacks carry \(K^2\) integer eigenvalues, and their characteristic
polynomials admit \(K\times K\) transfer determinants.  This is the sharpest matrix
compression found here and the most promising new arithmetic object.

Finally, Kraus--Loewner matrices give a second exact positivity architecture: rational entries,
strictly positive minors, an explicit multiple integral, and an integer after denominator
clearing.  Their crude homogeneity is unfavorable, so any success must come from primitive
arithmetic cancellation.

\begin{resultbox}
\textbf{Bottom line.}  No unconditional proof is claimed.  The continuation adds exact theorems
that eliminate several natural matrix shortcuts and isolates three concrete surviving targets:
(1) a joint Smith/singular-value compound profile beyond the universal factorial baseline,
(2) arithmetic rigidity of the commuting boundary transfer matrices, and (3) primitive
denominator cancellation in Kraus--Loewner determinants.  Of these, the boundary-transfer formulation appears to offer the cleanest way to
use the full \(K^2\)-point conditional grid without paying the exact factor-four interpolation
cost.
\end{resultbox}
...
% --- imported with the matrix-analysis continuation ---

\bibitem{FitzGeraldHorn1977}
C.~H.~FitzGerald and R.~A.~Horn,
\newblock \emph{On fractional Hadamard powers of positive definite matrices},
\newblock Journal of Mathematical Analysis and Applications \textbf{61}(3) (1977), 633--642.
\newblock DOI:
\href{https://doi.org/10.1016/0022-247X(77)90167-6}{10.1016/0022-247X(77)90167-6}.
...
\newblock DOI:
\href{https://doi.org/10.1007/978-1-4612-0653-8}{10.1007/978-1-4612-0653-8}.

\bibitem{FallatGuptaJohnson2024}
S.~Fallat, H.~Gupta, and C.~R.~Johnson,
\newblock \emph{Sufficient conditions for total positivity, compounds, and Dodgson condensation},
\newblock arXiv:2405.06069 [math.CO], 2024.
\newblock \url{https://arxiv.org/abs/2405.06069}.

\bibitem{JohnsonRichards2025}
K.~W.~Johnson and D.~St.~P.~Richards,
\newblock \emph{Hyperdeterminantal total positivity},
\newblock SIGMA \textbf{21} (2025), Article~055, 18 pages.
\newblock DOI:
\href{https://doi.org/10.3842/SIGMA.2025.055}{10.3842/SIGMA.2025.055};
\href{https://arxiv.org/abs/2412.03000}{arXiv:2412.03000}.

\bibitem{CalegariDimitrovTang2025}
F.~Calegari, V.~Dimitrov, and Y.~Tang,
\newblock \emph{Arithmetic holonomy bounds and effective Diophantine approximation},
\newblock arXiv:2510.04156 [math.NT], 2025; contribution to the ICM Proceedings 2026.
\newblock \url{https://arxiv.org/abs/2510.04156}.

\bibitem{Orr2026}
M.~Orr,
\newblock \emph{Explicit height bounds for G-functions and unlikely intersections with lines in tori},
\newblock arXiv:2607.09565 [math.NT], 2026.
\newblock \url{https://arxiv.org/abs/2607.09565}.

\bibitem{BohnackerMuller2026}
L.-P.~V.~G.~Bohnacker and R.~R.~M\"uller,
\newblock \emph{On the asymptotic of coupled spherical integrals: an operator-valued extension of the free Fourier transform},
\newblock arXiv:2608.03273 [math.PR], 2026.
\newblock \url{https://arxiv.org/abs/2608.03273}.

\bibitem{Harrison2026}
J.~Harrison,
\newblock \emph{Additive relations in irrational powers},
\newblock arXiv:2512.04081v3 [math.NT], revised July 2026.
...
\newblock arXiv:2603.24823v1 [cs.LO], 2026.
...
\newblock arXiv:2510.01440v3 [math.NT], revised August 2026.
\newblock \url{https://arxiv.org/abs/2510.01440}.
