...
\hypersetup{
  ...
  citecolor=newgreen,
  urlcolor=leanblue,
  pdftitle={The Alaoglu--Erdos Integer-Exponent Conjecture: Unified Report with p-adic Continuation},
  pdfauthor={The ProveIt project},
  pdfsubject={Machine-checked partial results, determinant methods, p-adic rank,
boundary residuals, and the remaining adelic barrier}
}
\urlstyle{same}
...
\vspace{0.12cm}
{\LARGE Determinant Methods, $p$-adic Rank, and the Adelic Barrier\par}
\vspace{1.0cm}
...
follows.  A third continuation identifies the conjecture with a fixed-base case of the Matrix
Coefficient Conjecture, proves an exact $K^2-1$ Newton-polytope interpolation threshold on the
conditional $K\times K$ grid, and extends the directed finite exclusion through $2^{29}$.  The
new $p$-adic continuation proves a rank jump at the structural primes $2,3$, exact
structural-prime normalization of the boundary residuals, and sharp odd-degree formulas for
natural dilation resultants.  These results isolate the surviving obstacle as genuinely
adelic: local divisibility must be coupled to a sufficiently small archimedean height.
\end{minipage}
\vfill
...
A fourth continuation develops the $p$-adic and adelic side.  It proves that a hypothetical
counterexample has real logarithm rank one and structural rank two, while its Iwasawa-logarithm
matrices at $2$ and $3$ are automatically nonsingular; the associated principal-unit subgroup
therefore has open local closure.  For the degree-$n$ boundary residuals obtained from the
minimal-denominator graph interpolant, it proves coefficientwise structural-prime floors,
explicit scaled reductions, and exact total root depth in two primitive residue branches.  The
unit-dilation resultants satisfy
\[
 v_2(\mathcal R_{2,K})\ge n^3+n+v_2(M-1),
 \qquad
 v_3(\mathcal R_{3,K})\ge n^3+v_3(A-1),
\]
and when $n$ is odd they are nonzero and both inequalities are equalities.  Exact symbolic
checks verify the endpoint formulas, Smith invariants, coefficient valuations, and small
resultants.  The conjecture remains open; the remaining task is to remove inherited
interpolation height or construct one adelic auxiliary value whose archimedean and finite-place
bounds cross the product-formula threshold.
\end{abstract}
...
None of them closes the final gap.  The fourth continuation adds a non-archimedean audit: it
rules out a naive $p$-adic copy of the real rank-one relation at the structural primes, but
extracts exact structural-prime scaling and sharp odd-degree resultant valuations from the
boundary-defect polynomials.
...
\item \textbf{New: structural-prime rank jump and exact boundary scaling.}  For a
hypothetical least generator $\beta$, with $M=2^\beta$ and $A=3^\beta$, the real logarithm
matrix has analytic rank one and structural rank two, but the Iwasawa-logarithm matrices at
$p=2,3$ have rank two.  After principal-unit normalization the corresponding local subgroup
has open closure.  In two primitive residue branches, the minimal-denominator boundary
residuals satisfy
\[
 2^{-n^2}F_K(2^nZ)\in\Z_2[Z]
 \quad\text{or}\quad
 3^{-n^2}G_K(3^nZ)\in\Z_3[Z]
\]
with unit leading coefficient and explicit reduction.  Every root has depth at least $n$, the
total excess depth is exact, and for odd $n$ the associated dilation resultant is nonzero with
an exact cubic-plus-linear valuation (Section~\ref{sec:fourth-continuation}).  \statusnew
...
\begin{resultbox}
\textbf{$p$-adic rank and boundary resultants (Sections~\ref{sec:padic-rank-jump}
--\ref{sec:fourth-conclusion}).}
A counterexample would exhibit the sharp mismatch
$\rank L_\infty=1<\operatorname{srank}L_\infty=2$ but
$\rank L_2=\rank L_3=2$.  Nevertheless the boundary residuals admit exact structural-prime
normalization.  In every odd boundary degree the natural unit-dilation resultants are nonzero
and satisfy
\[
 v_2(\mathcal R_{2,K})=n^3+n+v_2(M-1),
 \qquad
 v_3(\mathcal R_{3,K})=n^3+v_3(A-1)
\]
in their respective residue branches.  Paper proof and exact symbolic audit in this report;
not yet formalized in Lean.  \statusnew
\end{resultbox}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.28\textwidth}>{\raggedright\arraybackslash}p{0.48\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}@{}}
\toprule
...
\begin{center}
\begin{tabularx}{0.96\textwidth}{@{}>{\raggedright\arraybackslash}p{0.48\textwidth}>{\raggedright\arraybackslash}X@{}}
\toprule
Reading & Status \\
\midrule
...
\section{Fourth continuation: $p$-adic outcome and claim ledger}
\label{sec:fourth-continuation}

This continuation takes the exact conditional structure and the boundary-defect compression of
Section~\ref{subsec:boundary-compression} as its starting point.  The main objective was to
turn the identities
\[
  2^\beta=M\in\N,
  \qquad
  3^\beta=A\in\N
\]
for a hypothetical nonintegral least generator $\beta$ into a contradiction at one or more
finite places.  No complete proof was obtained.  The $p$-adic analysis nevertheless changes
our understanding of the obstruction in three material ways.

First, the real logarithm matrix of a counterexample has rank one, but its Iwasawa-logarithm
matrices at the two structural primes $2$ and $3$ are automatically nonsingular.  Thus the
most obvious local analogue of the real argument points in the opposite direction: at both
primes built into the problem, the two generators span an open subgroup of the
principal-unit torus.  The conjectural relation is archimedean, not a relation in
$\Q_2$ or $\Q_3$.

Second, the interpolation residuals $U_K,V_K$ admit a much more rigid local normalization than
was visible from their endpoint coefficients alone.  In two natural residue branches, after
clearing the \emph{minimal} denominator of the graph interpolant and putting $n=K-1$, one has
\[
  2^{-n^2}F_K(2^nZ)\in\Z_2[Z]
  \qquad\text{or}\qquad
  3^{-n^2}G_K(3^nZ)\in\Z_3[Z],
\]
with unit leading coefficient, where $F_K=\delta_KU_K$ and
$G_K=\delta_KV_K$.  Consequently every residual root lies at structural-prime depth at least
$n$, the total excess depth is known exactly, and every coefficient satisfies a sharp linear
valuation floor.  The reductions of the scaled polynomials also describe the residue clusters
of the roots; in odd boundary degree they determine the complete depth distribution.

Third, the scaled residuals yield natural integer resultants comparing a residual with its
unit dilation.  In the dyadic branch the nonzero resultant satisfies
\[
 v_2(\mathcal R_{2,K})\ge n^3+n+v_2(M-1),
\]
and in the triadic branch
\[
 v_3(\mathcal R_{3,K})\ge n^3+v_3(A-1).
\]
When $n$ is odd, the exact-dilation alternative is impossible in both branches and equality
holds in the corresponding formula.  The remaining obstacle is therefore no longer finding a
$p$-adically small integer.  It is making its archimedean height small enough, or coupling the
resulting local objects for different $K$ and different places.

\begin{center}
\begin{tabularx}{0.97\textwidth}{@{}>{\raggedright\arraybackslash}p{0.27\textwidth}X>{\raggedright\arraybackslash}p{0.16\textwidth}@{}}
\toprule
Result & Content & Status\\
\midrule
Structural-prime rank jump & The real logarithm matrix has rank one, whereas the Iwasawa-logarithm matrices at $p=2$ and $p=3$ have determinant respectively $-\log_2(M)\log_2(3)$ and $\log_3(2)\log_3(A)$, both nonzero. & \statusnew\\
\addlinespace
Exact structural rank & The real rank-one matrix has structural rank two: structural rank one would force a rational matrix coefficient to vanish, which the inherited factorization excludes. & \statusnew\\
\addlinespace
Open local closure & After passing to principal units, the subgroup generated by $(2,3)$ and $(M,A)$ has open closure in a two-dimensional $p$-adic torus for $p=2,3$.  Rectangular parameter averages converge to Haar measure on that closure. & \statusnew\\
\addlinespace
Quantitative recurrence ceiling & At every full-rank prime, simultaneous congruence depth of two parameter-grid points is only $O(\log K)$; over finitely many primes the weighted depths satisfy a product-formula-like budget. & \statusnew\\
\addlinespace
Good-prime shadow exponent & A rank drop at a good prime $\ell$ would create a $p$-adic exponent $\beta_\ell$ that is transcendental by Baker--Brumer, but no theorem identifies it with the real $\beta$.  A Fermat-quotient determinant gives a first finite certificate for local full rank. & \statusnew\\
\addlinespace
Denominator lattice & The minimal interpolation denominator is the order of the value vector in the Vandermonde cokernel.  The universal denominator is both an lcm of barycentric denominators and the largest Smith invariant factor. & \statusnew\\
\addlinespace
Structural-prime scaling & In the branches $M$ odd, $A$ even and $3\mid M$, $3\nmid A$, the entire normalized residual has an exact $p^{n^2}$ scaling and a unit leading coefficient.  Every root has valuation at least $n$, with exact total excess and explicit residue clusters. & \statusnew\\
\addlinespace
Dilation resultant & The corresponding unit-dilation resultant is either zero because of an exact dilation pair, or has valuation at least $n^3+n+v_2(M-1)$ at $2$ and $n^3+v_3(A-1)$ at $3$.  For odd $n$ it is nonzero and equality holds. & \statusnew\\
\addlinespace
Exact symbolic audit & The $K=2$ formulas, Smith forms for $K=2,3$, denominator extremes through $K=8$, and sample Newton polygons and resultants were checked over exact integers and rationals. & \statuscomp\\
\bottomrule
\end{tabularx}
\end{center}

The paper proofs below are independent of the exact symbolic audit; the computations are used
to test formulas, reveal the correct normalizations, and calibrate the remaining height gap.
None of the new statements has yet been formalized in Lean.

\section{The real rank-one relation and the structural-prime rank jump}
\label{sec:padic-rank-jump}

\subsection{Iwasawa logarithms and their rational kernel}

For every prime $p$, let
\[
  \log_p:\overline{\Q}_p^{\,\times}\longrightarrow\overline{\Q}_p
\]
denote Iwasawa's branch of the $p$-adic logarithm, normalized by $\log_p(p)=0$ and by
vanishing on roots of unity.  On $\Q_p^\times$ this is obtained by writing a unit as its
Teichmuller part times a principal unit and applying the convergent logarithm series to the
principal-unit part.

\begin{lemma}[Kernel on positive rationals]
\label{lem:padic-log-rational-kernel}
For $r\in\Q_{>0}$,
\[
  \log_p r=0
  \quad\Longleftrightarrow\quad
...
\begin{proof}
The kernel of Iwasawa's logarithm on $\Q_p^\times$ is $p^\Z$ times the finite group of roots
of unity in $\Q_p$.  The only positive rational root of unity is $1$.  Intersecting the local
kernel with $\Q_{>0}$ therefore leaves exactly $p^\Z$.
\end{proof}

Assume from now on that $\beta\notin\Z$ is a hypothetical counterexample and use the inherited
normalization
\[
  M=2^\beta\in\N,
  \qquad
...
\begin{equation}
  ...
  \begin{pmatrix}\log2\\\log3\end{pmatrix}
  \begin{pmatrix}1&\beta\end{pmatrix}.
  \label{eq:padic-real-rank-one}
\end{equation}
It has analytic rank one.  At a finite place define instead
\begin{equation}
  L_p(M,A)
  :=
  \begin{pmatrix}
  ...
  \end{pmatrix},
  \qquad
  \Delta_p(M,A):=\det L_p(M,A).
  \label{eq:padic-log-matrix}
\end{equation}

\begin{theorem}[Structural-prime rank jump]
\label{thm:structural-prime-rank-jump}
For a hypothetical nonintegral counterexample,
\begin{align}
  \Delta_2(M,A)&=-\log_2(M)\log_2(3)\ne0,
  \label{eq:delta2-nonzero}\\
  \Delta_3(M,A)&= \log_3(2)\log_3(A)\ne0.
  \label{eq:delta3-nonzero}
\end{align}
Consequently $L_2(M,A)$ and $L_3(M,A)$ both have rank two, although
$L_\infty(M,A)$ has rank one.
\end{theorem}

\begin{proof}
Because $\log_2 2=0$, expansion of the determinant gives the first displayed identity.
Now $\log_2 3\ne0$ by Lemma~\ref{lem:padic-log-rational-kernel}.  If
$\log_2M=0$, then $M=2^m$ for some $m\in\Z$; positivity gives $m\ge0$, and
$2^\beta=M=2^m$ forces $\beta=m$, contrary to hypothesis.  Hence
$\Delta_2\ne0$.

The proof at $p=3$ is symmetric.  Here $\log_3 3=0$, while $2$ is not a power of $3$ and
$A$ cannot be a power of $3$ without forcing $\beta\in\Z$.
\end{proof}

The same statement holds for a two-base problem with distinct prime bases $p,q$: the real
matrix attached to $p^\beta,q^\beta$ is singular, while the Iwasawa matrices at the two base
primes are automatically nonsingular.  This elementary observation is important because it
rules out an entire family of tempting arguments.  There is no local identity
\[
  \frac{\log_p M}{\log_p2}=\frac{\log_p A}{\log_p3}
\]
at either structural prime.  The real rank-one factorization cannot simply be copied into
$\Q_2$ or $\Q_3$.

\subsection{Structural rank is nevertheless two}

Write the entries of a logarithm matrix in a $\Q$-basis and replace the basis elements by
independent variables.  The rank of the resulting generic matrix is its \emph{structural
rank}, as in Dasgupta~\cite{Dasgupta2023} and Dasgupta--Kakde~\cite{DasguptaKakdeMCC}.
Theorem~2.2 of the latter says that a rational vector space of square matrices consisting
entirely of singular matrices has a common rational matrix coefficient equal to zero.

\begin{proposition}[Maximal structural defect]
\label{prop:real-structural-rank-two}
The matrix $L_\infty(M,A)$ of a hypothetical counterexample has structural rank two.  Thus it
has analytic rank one but the largest possible structural rank for a $2\times2$ matrix.
\end{proposition}

\begin{proof}
Its structural rank is at least one.  If it were one, the rational coefficient space obtained
from its entries would consist entirely of singular matrices.  The cited linear-algebra
theorem would then give nonzero $v,w\in\Q^2$ such that
$w^TL_\infty(M,A)v=0$.  Proposition~\ref{prop:mc-factorization} of the inherited report
shows that every such coefficient factors as
\[
  \bigl(w_1\log2+w_2\log3\bigr)
  \bigl(v_1+\beta v_2\bigr),
\]
and that neither factor can vanish for nonzero rational $v,w$.  This contradiction proves
that the structural rank is two.
\end{proof}

A hypothetical counterexample is therefore a particularly sharp instance of the structural
rank problem:
\[
 \rank L_\infty=1
 \quad<\quad
 \operatorname{srank}L_\infty=2,
 \qquad
 \rank L_2=\rank L_3=2.
\]
At infinity, analytic rank falls below structural rank; at the two base primes, actual rank
returns to structural rank.  This is the local-global mismatch that an adelic proof would have
to exploit.

\subsection{Why no product formula applies directly to $\Delta_p$}

The numbers $\Delta_p(M,A)$ are not the $p$-adic absolute values of one algebraic number.
They are determinants built from different transcendental period maps at different places.
In particular, $\Delta_\infty=0$ supplies no algebraic quantity whose product formula would
force $\Delta_2$ or $\Delta_3$ to vanish.  Any product-formula argument must first construct a
single algebraic auxiliary value---typically a polynomial or determinant evaluated at
algebraic points---that is small at all selected places.  Applying a product formula directly
to the local logarithm determinants is a category error.

\section{$p$-adic closures, Haar measure, and recurrence ceilings}
\label{sec:padic-closures}

\subsection{Principal-unit normalization}
...
\[
  ...
  \end{cases}
  \qquad
  \langle u\rangle_p:=\bigl(p^{-v_p(u)}u\bigr)^{e_p}
  \quad(u\in\Q_{>0}).
\]
For odd $p$, $\langle u\rangle_p\in1+p\Z_p$; for $p=2$, an odd square lies in
$1+8\Z_2$.  In either case the logarithm and exponential are inverse analytic isomorphisms on
a sufficiently small principal-unit group and
\[
  \log_p\langle u\rangle_p=e_p\log_pu.
\]
Consider the continuous homomorphism
\begin{equation}
 \Phi_p:\Z_p^2\longrightarrow (1+p^{\nu_p}\Z_p)^2,
 \qquad
 (z,w)\longmapsto
 \left(
  \langle2\rangle_p^z\langle M\rangle_p^w,
  \langle3\rangle_p^z\langle A\rangle_p^w
 \right),
 \label{eq:padic-Phi}
\end{equation}
where $\nu_p=1$ for odd $p$ and $\nu_2=3$.  Its logarithmic differential is
$e_pL_p(M,A)$.

\begin{theorem}[Dimension of the local closure]
\label{thm:padic-closure-dimension}
The closure of the subgroup generated by
\[
  (\langle2\rangle_p,\langle3\rangle_p),
  \qquad
  (\langle M\rangle_p,\langle A\rangle_p)
\]
has $p$-adic Lie dimension $\rank L_p(M,A)$.  If $\Delta_p(M,A)\ne0$, the closure is an open
subgroup of the two-dimensional principal-unit torus.  In particular this closure is open for
$p=2$ and $p=3$.
\end{theorem}

\begin{proof}
Apply the logarithm coordinatewise.  The closure becomes the closure of the $\Z$-span of the
two columns of $e_pL_p$ in a two-dimensional additive $p$-adic Lie group.  Its Lie algebra is
the $\Q_p$-span of those columns and therefore has the stated dimension.  If the determinant
is nonzero, the image of a full-rank $\Z_p$-lattice is again a full-rank lattice, hence open;
exponentiating returns an open subgroup.
\end{proof}

This is the toric special case of the closure-dimension perspective emphasized by
Prasad~\cite{Prasad2025} and, in a different setting, by the structural-rank discussion of
Burungale--Skinner--Wan~\cite{BurungaleSkinnerWan2026}.  Here it is elementary because the
logarithm linearizes the principal-unit torus completely.

\subsection{A $p$-adic measure-theoretic consequence}

Let $H_p$ be the compact closure in Theorem~\ref{thm:padic-closure-dimension}, and let $\mu_p$
be its normalized Haar measure.

\begin{proposition}[Rectangular Haar equidistribution]
\label{prop:padic-haar-equidistribution}
For every continuous $f:H_p\to\C$,
\begin{equation}
 \frac1{N^2}\sum_{0\le i,j<N}
 f\!\left(\Phi_p(i,j)\right)
 \longrightarrow
 \int_{H_p}f\,d\mu_p.
 \label{eq:padic-haar-average}
\end{equation}
If $\Delta_p\ne0$, this applies in particular to every clopen congruence box in an open
subgroup of a two-dimensional principal-unit torus.
\end{proposition}

\begin{proof}
By the character criterion for compact abelian groups, it suffices to test continuous
characters $\chi$ of $H_p$.  The average factors as
\[
 \left(\frac1N\sum_{i=0}^{N-1}\chi(\Phi_p(i,0))\right)
 \left(\frac1N\sum_{j=0}^{N-1}\chi(\Phi_p(0,j))\right).
\]
For a nontrivial character, density of the two generated elements implies that at least one of
the two character values is not $1$, and the corresponding geometric average tends to zero.
The trivial character gives one.  Approximation by finite linear combinations of characters
completes the proof.
\end{proof}

Thus the $2$-adic and $3$-adic pictures are locally \emph{thick}, not sparse: the two-parameter
orbit fills an open group with the expected Haar frequencies.  This measure-theoretic fact is
fully compatible with the real orbit lying on the one-dimensional curve $y=x^\beta$.
Equidistribution at one completion does not identify the exponent used by another completion.
The missing ingredient is a common algebraic observable whose real and non-archimedean sizes
can be compared.

\subsection{A quantitative local collision ceiling}

For $z=(z_1,z_2)\in\Q_p^2$ put
\[
  \nu_p(z):=\min\{v_p(z_1),v_p(z_2)\}.
\]
If $B_p=e_pL_p(M,A)$ is invertible, equivalence of norms in finite-dimensional $p$-adic vector
spaces gives an integer $c_p\ge0$ such that
\begin{equation}
  \bigl|\nu_p(B_pz)-\nu_p(z)\bigr|\le c_p
  \qquad(z\in\Q_p^2).
  \label{eq:padic-norm-equivalence}
\end{equation}

\begin{proposition}[Logarithmic collision ceiling]
\label{prop:padic-collision-ceiling}
Assume $\Delta_p(M,A)\ne0$.  If distinct parameter points
$u,u'\in\{0,1,\ldots,K-1\}^2$ have logarithmic orbit difference of depth at least $r$, meaning
\[
  \nu_p\!\left(B_p(u-u')\right)\ge r,
\]
then
\begin{equation}
  r\le \left\lfloor\log_p(K-1)\right\rfloor+c_p.
  \label{eq:padic-collision-log}
\end{equation}
More generally, for a finite set $S$ of full-rank primes and depths $r_p$,
\begin{equation}
  \sum_{p\in S}r_p\log p
  \le
  \log(K-1)+\sum_{p\in S}c_p\log p.
  \label{eq:padic-multiplace-budget}
\end{equation}
\end{proposition}

\begin{proof}
Equation~\eqref{eq:padic-norm-equivalence} implies
$\nu_p(u-u')\ge r-c_p$.  A nonzero coordinate of $u-u'$ has absolute value at most $K-1$,
so it cannot be divisible by a power of $p$ exceeding
$p^{\lfloor\log_p(K-1)\rfloor}$, proving the first assertion.

For the second, $p^{\max(0,r_p-c_p)}$ divides both coordinates of $u-u'$ for every $p\in S$.
Their product therefore divides both coordinates.  At least one coordinate is nonzero and has
absolute value at most $K-1$, so
\[
  \prod_{p\in S}p^{\max(0,r_p-c_p)}\le K-1.
\]
Taking logarithms and weakening the left side gives~\eqref{eq:padic-multiplace-budget}.
\end{proof}

This ceiling is the finite-place analogue of a packing statement.  Full local rank prevents
many parameter pairs from being simultaneously extraordinarily close.  It does not contradict
the existence of the orbit; rather, it says that any successful $p$-adic determinant must use
structured cancellation, not merely repeated near-collisions of orbit points.

\section{Good primes, shadow exponents, and Fermat quotients}
\label{sec:good-prime-shadows}

At a prime $\ell\nmid6MA$, all four numbers $2,3,M,A$ are $\ell$-adic units.  A local rank
drop is then equivalent to
\begin{equation}
  \Delta_\ell(M,A)
  =\log_\ell2\log_\ell A-\log_\ell M\log_\ell3
  =0.
  \label{eq:good-prime-rank-drop}
\end{equation}
Neither $\log_\ell2$ nor $\log_\ell3$ vanishes, so such a prime would define a common local
ratio
\begin{equation}
  \beta_\ell
  :=\frac{\log_\ell M}{\log_\ell2}
  =\frac{\log_\ell A}{\log_\ell3}
  \in\C_\ell.
  \label{eq:padic-shadow-beta}
\end{equation}

\begin{proposition}[Transcendence of a good-prime shadow]
\label{prop:shadow-beta-transcendental}
If~\eqref{eq:good-prime-rank-drop} holds, then $\beta_\ell$ is transcendental over the embedded
copy of $\overline\Q$ in $\C_\ell$.
\end{proposition}

\begin{proof}
The positive rationals $2$ and $M$ are multiplicatively independent.  Indeed, a relation
$2^aM^b=1$ with $a,b\in\Z$ would give $a+b\beta=0$ after taking real logarithms; irrationality
of $\beta$ forces $a=b=0$.  Hence $\log_\ell2$ and $\log_\ell M$ are $\Q$-linearly
independent.  Baker--Brumer upgrades this to linear independence over $\overline\Q$
\cite{Brumer1967,Dasgupta2023}.
If $\beta_\ell$ were algebraic, the relation
$\log_\ell M-\beta_\ell\log_\ell2=0$ would contradict that independence.
\end{proof}

This is a genuine $p$-adic Gelfond--Schneider-type shadow, but it is not yet a bridge.  The
real number $\beta$ and the local number $\beta_\ell$ live in different fields and are defined
by different logarithm homomorphisms.  No known theorem forces them to be images of one
algebraic number, or even to satisfy a common algebraic relation.

\subsection{The first finite shadow: Fermat quotients}

For an odd good prime $\ell$ and an $\ell$-adic unit $u$, define the Fermat quotient
\[
  q_\ell(u):=\frac{u^{\ell-1}-1}{\ell}\pmod\ell.
\]
For rational units the definition is interpreted in $\Z_\ell$; denominators prime to $\ell$
are harmless.  The standard first-order expansion gives
\[
  \frac{\log_\ell u}{\ell}\equiv-q_\ell(u)\pmod\ell.
\]

\begin{proposition}[Fermat-quotient rank certificate]
\label{prop:fermat-quotient-certificate}
For every odd prime $\ell\nmid6MA$,
\begin{equation}
 \frac{\Delta_\ell(M,A)}{\ell^2}
 \equiv
 q_\ell(2)q_\ell(A)-q_\ell(M)q_\ell(3)
 \pmod\ell.
 \label{eq:fermat-quotient-determinant}
\end{equation}
If the right side is nonzero, then $v_\ell(\Delta_\ell)=2$ and $L_\ell(M,A)$ has full rank.
\end{proposition}

\begin{proof}
Each logarithm of an $\ell$-adic unit is divisible by $\ell$.  Divide the determinant by
$\ell^2$ and substitute the four first-order congruences.  The two minus signs in each product
cancel.
\end{proof}

The congruence supplies a cheap exact test for local full rank.  Its vanishing does not prove a
rank drop; it only says that the determinant has valuation at least three.  More importantly,
the real identity $\Delta_\infty=0$ does not force the Fermat-quotient expression to vanish.
A proof based on good primes would need a new theorem producing infinitely many unusually
deep local determinant congruences from the real relation, or an adelic auxiliary construction
that packages all places at once.

\section{The interpolation denominator as an arithmetic lattice}
\label{sec:boundary-denominator-lattice}

Return to the graph interpolant $Q_K$ of~\eqref{eq:boundary-Q}.  Put
\[
  R=K^2,
  \qquad
  n=K-1,
  ...
  \quad(0\le i,j\le n).
\]
Let $E_K$ be the $R\times R$ evaluation matrix
\begin{equation}
  E_K=\bigl(x_{ij}^{\,r}\bigr)_{(i,j),\,0\le r<R}.
  \label{eq:boundary-evaluation-matrix}
\end{equation}
If $q=(q_0,\ldots,q_{R-1})^T$ is the coefficient vector of $Q_K$ and
$y=(y_{ij})$, then $E_Kq=y$.

Define the minimal denominator
\begin{equation}
  \delta_K:=\min\{d\in\N_{>0}:dQ_K\in\Z[X]\},
  \qquad
  P_K:=\delta_KQ_K,
  \label{eq:boundary-minimal-denominator}
\end{equation}
and the integer residuals
\begin{equation}
  F_K:=\delta_KU_K,
  \qquad
  G_K:=\delta_KV_K.
  \label{eq:boundary-integer-residuals}
\end{equation}
Divisibility in Theorem~\ref{thm:dilation-defect} is by monic integer polynomials, so
$F_K,G_K\in\Z[X]$.

\subsection{Smith normal form and the universal denominator}

For a node $s=(i,j)$ put
\begin{equation}
  d_{ij}^{(K)}
  :=\prod_{(a,b)\ne(i,j)}(x_{ij}-x_{ab}),
  \qquad
  D_K^*:=\operatorname{lcm}_{0\le i,j<K}|d_{ij}^{(K)}|.
  \label{eq:boundary-bary-denominators}
\end{equation}

\begin{proposition}[Three descriptions of the denominator]
\label{prop:boundary-denominator-descriptions}
The integer $D_K^*$ is the least positive integer for which
$D_K^*E_K^{-1}$ has integral entries.  Equivalently, it is the largest invariant factor in the
Smith normal form of $E_K$.  For the specialized value vector $y$, the minimal denominator
$\delta_K$ is the order of the class of $y$ in the finite group
$\Z^R/E_K\Z^R$.

More explicitly, if
\[
  U E_K V=\operatorname{diag}(s_1,\ldots,s_R),
  \qquad s_1\mid s_2\mid\cdots\mid s_R,
\]
...
\begin{equation}
  ...
  \label{eq:boundary-delta-smith}
\end{equation}
\end{proposition}

\begin{proof}
The rows of $E_K^{-1}$ are the coefficient vectors of the Lagrange basis polynomials
\[
  \ell_{ij}(X)=
  \frac{\prod_{(a,b)\ne(i,j)}(X-x_{ab})}{d_{ij}^{(K)}}.
\]
All numerator coefficients are integers, so the lcm in~\eqref{eq:boundary-bary-denominators}
clears the inverse matrix.  Conversely, the leading coefficient of $\ell_{ij}$ is exactly
$1/d_{ij}^{(K)}$, so every common inverse denominator must be divisible by every
$|d_{ij}^{(K)}|$.  This proves the first description.

The Smith decomposition gives
$E_K^{-1}=V\operatorname{diag}(s_r^{-1})U$, so the least scalar clearing the inverse is
$s_R$.  Finally, $dq\in\Z^R$ is equivalent to $dUy$ being coordinatewise divisible by the
corresponding $s_r$, which gives~\eqref{eq:boundary-delta-smith} and identifies $\delta_K$ as
the order of $[y]$ in the cokernel.
\end{proof}

This makes the denominator dependence on $M,A$ transparent.  The node lattice determines the
finite abelian group; the counterexample values determine one class in it.  Large cancellation
in $\delta_K$ is precisely a statement that this class has unexpectedly small order.

\subsection{Two adjacent generalized Vandermonde numerators}

Let
\[
  \mathcal V_K:=\det(1,x,\ldots,x^{R-1}),
\]
the ordinary Vandermonde determinant on the ordered node list.  Define
\begin{align}
 \mathcal C_K(M,A)
 &:=\det(1,x,\ldots,x^{R-2},y),
 \label{eq:boundary-C-determinant}\\
 \mathcal D_K(M,A)
 &:=\det(x,x^2,\ldots,x^{R-1},y).
 \label{eq:boundary-D-determinant}
\end{align}
Here each displayed symbol denotes a column of the evaluation matrix.

\begin{proposition}[Endpoint Cramer witnesses]
\label{prop:boundary-cramer-witnesses}
The leading and constant coefficients of $Q_K$ are
\begin{equation}
 [X^{R-1}]Q_K=\frac{\mathcal C_K}{\mathcal V_K},
 \qquad
 Q_K(0)=(-1)^{R-1}\frac{\mathcal D_K}{\mathcal V_K}.
 \label{eq:boundary-cramer-endpoints}
\end{equation}
Both numerators are nonzero under the irrational power-curve hypothesis.  Moreover,
$\mathcal C_K$ vanishes after the formal specialization $(M,A)=(2^m,3^m)$ for
$0\le m\le R-2$, while $\mathcal D_K$ vanishes for $1\le m\le R-1$.
\end{proposition}

\begin{proof}
The formulas are Cramer's rule, moving the replaced first column past $R-1$ columns in the
constant-coefficient case.  If either endpoint coefficient vanished, the polynomial
$Y-Q_K(X)$ would have at most $R$ monomials, contradicting
Corollary~\ref{cor:full-support-interpolant}.  Under $(M,A)=(2^m,3^m)$ the value column becomes
$x^m$, duplicating one of the displayed monomial columns in exactly the stated ranges.
\end{proof}

These are adjacent generalized Vandermonde determinants.  They show that the two endpoint
coefficients are not arbitrary fractions: all of their primitive divisibility is encoded in
explicit determinant numerators that vanish at consecutive blocks of integral exponents.

\subsection{Exact endpoint identities for the residuals}
...
\[
  ...
  e_K:=P_K(0).
\]
By full support, both are nonzero.  Comparing leading and constant terms in the two defect
factorizations gives the following exact formulas.

\begin{proposition}[Residual endpoints]
\label{prop:boundary-residual-endpoints}
Let
\[
  a_n:=\frac{n(n-1)(n+1)}2,
  \qquad
  b_n:=\frac{n^2(n+1)}2.
\]
Then
\begin{align}
 [X^n]F_K&=(2^{R-1}-M)c_K,
 &2^{a_n}3^{b_n}F_K(0)&=(1-M)e_K,
 \label{eq:boundary-F-endpoints}\\
 [X^n]G_K&=(3^{R-1}-A)c_K,
 &2^{b_n}3^{a_n}G_K(0)&=(1-A)e_K.
 \label{eq:boundary-G-endpoints}
\end{align}
\end{proposition}

\begin{proof}
The leading formulas are~\eqref{eq:boundary-leading} after multiplying by $\delta_K$.  At
$X=0$, the constant of $\Pi_{n,n+1}$ is
$2^{a_n}3^{b_n}$ because its degree $n(n+1)$ is even; the constant of
$\Pi_{n+1,n}$ is $2^{b_n}3^{a_n}$.  Substitute $X=0$ in the defect identities.
\end{proof}

The remaining task is to understand $c_K$ and $e_K$ at the structural primes.  That can be
done exactly in two residue branches by locating the extreme barycentric denominators.

\section{Extreme barycentric valuations}
\label{sec:boundary-extreme-valuations}

\subsection{The dyadic and triadic denominator formulas}

For $0\le i,j\le n$, direct comparison of node valuations gives
\begin{align}
 v_2(d_{ij}^{(K)})
 &=K\frac{i(i-1)}2+Ki(n-i)+ni
   +\sum_{b\ne j}v_2(3^{|b-j|}-1),
 \label{eq:boundary-v2-dij}\\
 v_3(d_{ij}^{(K)})
 &=K\frac{j(j-1)}2+Kj(n-j)+nj
   +\sum_{a\ne i}v_3(2^{|a-i|}-1).
 \label{eq:boundary-v3-dij}
\end{align}
For example, in~\eqref{eq:boundary-v2-dij}, a node in a lower $2$-adic row contributes its
row index, a node in a higher row contributes $i$, and a node in the same row contributes
$i+v_2(3^{|b-j|}-1)$.

Define prefix sums
\[
 A_2(t):=\sum_{d=1}^t v_2(3^d-1),
 \qquad
 A_3(t):=\sum_{d=1}^t v_3(2^d-1).
\]
The lifting-the-exponent lemma gives
\begin{align}
 A_2(t)&=t+2\left\lfloor\frac t2\right\rfloor
          +v_2\!\left(\left\lfloor\frac t2\right\rfloor!\right),
 \label{eq:A2-prefix}\\
 A_3(t)&=\left\lfloor\frac t2\right\rfloor
          +v_3\!\left(\left\lfloor\frac t2\right\rfloor!\right).
 \label{eq:A3-prefix}
\end{align}
Both functions are superadditive: the floor term is superadditive and
$v_p((r+s)!)\ge v_p(r!)+v_p(s!)$.  Hence
$A_p(j)+A_p(n-j)\le A_p(n)$, so the within-row or within-column contribution is maximal at an
endpoint.

The part of~\eqref{eq:boundary-v2-dij} depending only on $i$ increases strictly, since its
increment from $i$ to $i+1$ is
\[
  K(n-i)-1>0
  \qquad(0\le i<n).
\]
The triadic formula is symmetric.

\begin{lemma}[Extreme denominator locations]
\label{lem:boundary-extreme-denominators}
The maximum of $v_2(d_{ij}^{(K)})$ occurs only in the top row $i=n$, and $(n,0)$ is a
maximizer.  The maximum of $v_3(d_{ij}^{(K)})$ occurs only in the rightmost column $j=n$,
and $(0,n)$ is a maximizer.
\end{lemma}

\begin{proof}
Strict monotonicity of the row part forces $i=n$ in the dyadic case, and superadditivity of
$A_2$ shows that the endpoint $j=0$ attains the maximum within that row.  Interchanging
$2,i$ with $3,j$ proves the triadic statement.
\end{proof}

Let
\[
  T_n:=\sum_{0\le i,j\le n}v_2(x_{ij})
      =\sum_{0\le i,j\le n}v_3(x_{ij})
      =\frac{n(n+1)^2}{2},
  \qquad
  \lambda_n:=T_n-n.
  \label{eq:boundary-T-lambda}
\]
A short calculation gives
\begin{equation}
  \lambda_n-a_n=n^2.
  \label{eq:boundary-lambda-minus-a}
\end{equation}

\subsection{Leading and constant coefficients in the favorable branches}

The two branches singled out below are not claimed to exhaust all possibilities.  They are
exactly the branches in which the off-axis output kills every competing extreme barycentric
term modulo the structural prime.

\begin{theorem}[Structural-prime endpoint theorem]
\label{thm:boundary-structural-endpoints}
Let $c_K,e_K$ be as above.

\begin{enumerate}
\item If $M$ is odd and $A$ is even, then
\[
  v_2(c_K)=0,
  \qquad
  v_2(e_K)=\lambda_n.
\]
Consequently the leading coefficient of $F_K$ is a $2$-adic unit and
\begin{equation}
  v_2(F_K(0))=n^2+v_2(M-1).
  \label{eq:boundary-F-constant-exact}
\end{equation}

\item If $3\mid M$ and $3\nmid A$, then
\[
  v_3(c_K)=0,
  \qquad
  v_3(e_K)=\lambda_n.
\]
Consequently the leading coefficient of $G_K$ is a $3$-adic unit and
\begin{equation}
  v_3(G_K(0))=n^2+v_3(A-1).
  \label{eq:boundary-G-constant-exact}
\end{equation}
\end{enumerate}
\end{theorem}

\begin{proof}
Write $w_{ij}=D_K^*/d_{ij}^{(K)}$.  The barycentric formulas give
\begin{align*}
 D_K^*[X^{R-1}]Q_K&=\sum_{i,j}w_{ij}M^iA^j,\\
 D_K^*Q_K(0)&=(-1)^{R-1}
 \sum_{i,j}w_{ij}\frac{\prod_{a,b}x_{ab}}{x_{ij}}M^iA^j.
\end{align*}

Suppose first that $M$ is odd and $A$ is even.  Modulo $2$, every leading-coefficient term
outside the extreme row has even $w_{ij}$, every term in that row with $j>0$ has even $A^j$,
and the endpoint $(n,0)$ survives with unit coefficient by
Lemma~\ref{lem:boundary-extreme-denominators}.  Thus the universal leading numerator is odd.
Since $\delta_K\mid D_K^*$ and $\delta_K[X^{R-1}]Q_K$ is integral, the denominator needed by
the leading coefficient already has the full dyadic valuation of $D_K^*$.  Therefore
$v_2(\delta_K)=v_2(D_K^*)$ and $c_K$ is odd.

In the constant formula, the term $(i,j)$ has valuation
\[
 v_2(w_{ij})+T_n-i+jv_2(A).
\]
The endpoint $(n,0)$ has valuation $T_n-n=\lambda_n$.  Every $i<n$ term is at least one
larger because $T_n-i\ge\lambda_n+1$, and every $(n,j)$ term with $j>0$ is at least one
larger because $A$ is even.  The minimum is unique, so
$v_2(e_K)=\lambda_n$.  Equations~\eqref{eq:boundary-F-endpoints} and
\eqref{eq:boundary-lambda-minus-a} give the assertions for $F_K$.

The triadic proof uses the endpoint $(0,n)$, with $M^i$ killing the competing terms in the
extreme column, and then applies~\eqref{eq:boundary-G-endpoints}.
\end{proof}

This theorem already shows that large primitive content is not the mechanism: the relevant
residual has a unit leading coefficient.  The divisibility is instead distributed through its
coefficients and roots.  The next theorem gives the natural structural-prime normalization and
an explicit reduction; in odd boundary degree this determines the full root-depth multiset.

\section{Structural-prime scaling and Newton-polygon normalization}
\label{sec:boundary-scaling-newton}

\begin{theorem}[Exact structural-prime scaling]
\label{thm:boundary-exact-scaling}
Let $n=K-1$.
...
\begin{equation}
  ...
  \label{eq:boundary-scaled-F}
\end{equation}
Its degree is $n$, its leading coefficient is a $2$-adic unit, and
\begin{equation}
  v_2(\mathcal F_K(0))=v_2(M-1).
  \label{eq:boundary-scaled-F-constant}
\end{equation}
Moreover, up to multiplication by a nonzero element of $\F_2$,
\begin{equation}
  \overline{\mathcal F_K}(Z)=1-(Z-1)^n
  \quad\text{in }\F_2[Z].
  \label{eq:boundary-scaled-F-reduction}
\end{equation}

\item If $3\mid M$ and $3\nmid A$, then
\begin{equation}
  \mathcal G_K(Z):=3^{-n^2}G_K(3^nZ)\in\Z_3[Z].
  \label{eq:boundary-scaled-G}
\end{equation}
Its degree is $n$, its leading coefficient is a $3$-adic unit, and
\begin{equation}
  v_3(\mathcal G_K(0))=v_3(A-1).
  \label{eq:boundary-scaled-G-constant}
\end{equation}
If
$R_n(Z)=\prod_{i=1}^n(Z-2^i)\in\F_3[Z]$, then, up to a nonzero scalar,
\begin{equation}
  \overline{\mathcal G_K}(Z)=R_n(0)-\overline A\,R_n(Z).
  \label{eq:boundary-scaled-G-reduction}
\end{equation}
\end{enumerate}
\end{theorem}

\begin{proof}
We prove the dyadic statement.  Start with the universal integer interpolant
$P_K^*=D_K^*Q_K$.  In its barycentric expansion, substitute $X=2^nZ$.  The term indexed by
$(i,j)$ is
\[
  w_{ij}M^iA^j
  \prod_{(a,b)\ne(i,j)}(2^nZ-2^a3^b).
\]
Extracting $2^a$ from every factor in row $a$ shows that this term is divisible in
$\Z_2[Z]$ by
\[
  2^{\,v_2(w_{ij})+T_n-i+jv_2(A)}.
\]
By the argument in Theorem~\ref{thm:boundary-structural-endpoints}, the unique term of minimum
valuation is $(n,0)$ and that minimum is $\lambda_n$.  Hence
\[
  H_K(Z):=2^{-\lambda_n}P_K(2^nZ)\in\Z_2[Z],
\]
because $\delta_K/D_K^*$ is a $2$-adic unit.  Reducing the unique minimum term modulo $2$
leaves, up to a unit,
\[
  M^n\prod_{b=1}^n(Z-3^b)=(Z-1)^n
  \quad\text{in }\F_2[Z].
\]
In particular $H_K(0)$ is a unit.

Next write
\[
  \Pi_{n,n+1}(2^nZ)=2^{a_n}J_K(Z),
  \qquad
  J_K(Z)=\prod_{i=0}^{n-1}\prod_{j=0}^n(2^{n-i}Z-3^j).
\]
The constant $J_K(0)$ is odd, so $J_K$ is invertible in $\Z_2[[Z]]$.  The dyadic defect
identity becomes
\[
  2^{\lambda_n}\bigl(H_K(2Z)-M H_K(Z)\bigr)
  =2^{a_n}J_K(Z)F_K(2^nZ).
\]
Using $\lambda_n-a_n=n^2$ gives
\[
  \mathcal F_K(Z)
  =\frac{H_K(2Z)-M H_K(Z)}{J_K(Z)}.
\]
The right side is integral as a formal power series and is already known to be a polynomial;
hence it lies in $\Z_2[Z]$.  Its leading coefficient is a unit by
Theorem~\ref{thm:boundary-structural-endpoints}.  At $Z=0$ its valuation is exactly
$v_2(1-M)$ because $H_K(0)$ and $J_K(0)$ are units.  Finally, modulo $2$ one has
$H_K(2Z)=H_K(0)$, $M=1$, and $J_K(Z)$ constant and nonzero, giving
\eqref{eq:boundary-scaled-F-reduction}.

For the triadic statement, substitute $X=3^nZ$, use the unique minimum endpoint $(0,n)$,
and factor
\[
  \Pi_{n+1,n}(3^nZ)=3^{a_n}
  \prod_{j=0}^{n-1}\prod_{i=0}^n(3^{n-j}Z-2^i).
\]
The same argument gives~\eqref{eq:boundary-scaled-G}; the surviving endpoint term reduces to
$R_n(Z)$, and evaluating the defect at $3Z$ gives
\eqref{eq:boundary-scaled-G-reduction}.
\end{proof}

\begin{corollary}[Coefficientwise valuation floors]
\label{cor:boundary-coefficient-floors}
Write
\[
 F_K(X)=\sum_{r=0}^n f_{K,r}X^r,
 \qquad
 G_K(X)=\sum_{r=0}^n g_{K,r}X^r.
\]
In the dyadic branch $M$ odd, $A$ even,
\begin{equation}
 v_2(f_{K,r})\ge n(n-r)
 \qquad(0\le r\le n),
 \label{eq:boundary-F-coefficient-floor}
\end{equation}
and in the triadic branch $3\mid M$, $3\nmid A$,
\begin{equation}
 v_3(g_{K,r})\ge n(n-r)
 \qquad(0\le r\le n).
 \label{eq:boundary-G-coefficient-floor}
\end{equation}
The leading inequalities are equalities, while the constant inequalities have excess
$v_2(M-1)$ and $v_3(A-1)$, respectively.
\end{corollary}

\begin{proof}
Compare coefficients in
$F_K(2^nZ)=2^{n^2}\mathcal F_K(Z)$ and
$G_K(3^nZ)=3^{n^2}\mathcal G_K(Z)$, then use the unit leading coefficients and
\eqref{eq:boundary-scaled-F-constant}--\eqref{eq:boundary-scaled-G-constant}.
\end{proof}

\begin{corollary}[Complete root-depth statement]
\label{cor:boundary-root-depths}
Valuations are extended to $\overline\Q_p$ with $v_p(p)=1$ and roots are counted with
multiplicity.

\begin{enumerate}
\item In the branch $M$ odd, $A$ even, every root $\rho$ of $F_K$ satisfies
$v_2(\rho)\ge n$, and
\begin{equation}
  \sum_{F_K(\rho)=0}\bigl(v_2(\rho)-n\bigr)=v_2(M-1).
  \label{eq:boundary-F-root-excess}
\end{equation}

\item In the branch $3\mid M$, $3\nmid A$, every root $\sigma$ of $G_K$ satisfies
$v_3(\sigma)\ge n$, and
\begin{equation}
  \sum_{G_K(\sigma)=0}\bigl(v_3(\sigma)-n\bigr)=v_3(A-1).
  \label{eq:boundary-G-root-excess}
\end{equation}
\end{enumerate}
\end{corollary}

\begin{proof}
The roots of $\mathcal F_K$ and $\mathcal G_K$ are algebraic integers because the scaled
polynomials have integral coefficients and unit leading coefficient.  Rescaling returns the
lower bounds.  The sum of root valuations equals the valuation of the constant coefficient
minus that of the leading coefficient, giving the exact excess formulas.
\end{proof}

In particular, at least one root satisfies
\[
  v_2(\rho)\ge n+\frac{v_2(M-1)}n
  \quad\text{or}\quad
  v_3(\sigma)\ge n+\frac{v_3(A-1)}n,
\]
but the corollary is much stronger: it accounts for \emph{all} roots.

\begin{corollary}[Dyadic residue clusters]
\label{cor:boundary-dyadic-clusters}
Assume $M$ is odd and $A$ is even.  Write $n=2^s m$ with $m$ odd.  Over
$\overline{\F}_2$,
\begin{equation}
 \overline{\mathcal F_K}(Z)
 =\bigl(1-(Z-1)^m\bigr)^{2^s}
 \quad\text{up to a nonzero scalar}.
 \label{eq:boundary-dyadic-clusters}
\end{equation}
Thus the $n$ scaled roots, counted with multiplicity, fall into $m$ distinct residue classes,
each containing $2^s$ roots.  Only the class reducing to $0$ can contain roots of positive
valuation, and the sum of the valuations in that class is $v_2(M-1)$.
\end{corollary}

\begin{proof}
In characteristic two, Frobenius gives
$1-T^{2^sm}=(1-T^m)^{2^s}$.  Since $m$ is odd, $1-T^m$ is squarefree.  Apply this with
$T=Z-1$ in~\eqref{eq:boundary-scaled-F-reduction}.  A scaled root has positive valuation
exactly when its reduction is $0$, and the total scaled-root valuation is
$v_2(M-1)$ by~\eqref{eq:boundary-F-root-excess}.
\end{proof}

\begin{corollary}[Odd-$n$ root distributions]
\label{cor:boundary-odd-n-roots}
Assume $n$ is odd.
\begin{enumerate}
\item If $M$ is odd and $A$ is even, exactly one root $\rho_0$ of $F_K$ satisfies
\[
  v_2(\rho_0)=n+v_2(M-1),
\]
and every other root satisfies $v_2(\rho)=n$.

\item If $3\mid M$ and $3\nmid A$, then:
\begin{itemize}
\item when $A\equiv1\pmod3$, exactly one root $\sigma_0$ of $G_K$ satisfies
\[
 v_3(\sigma_0)=n+v_3(A-1),
\]
and every other root has valuation $n$;
\item when $A\equiv2\pmod3$, every root of $G_K$ has valuation $n$.
\end{itemize}
\end{enumerate}
\end{corollary}

\begin{proof}
The dyadic assertion is the case $s=0$ of
Corollary~\ref{cor:boundary-dyadic-clusters}: the reduction is squarefree, its zero root is
simple, and all other residue roots are nonzero.

For the triadic assertion write $n=2m+1$.  In $\F_3[Z]$,
\[
 R_n(Z)=(Z+1)^{m+1}(Z-1)^m.
\]
The scaled constant term is zero modulo $3$ exactly when $A\equiv1\pmod3$.  In that case the
zero is simple because
\[
 \frac{R_n'(0)}{R_n(0)}=(m+1)-m=1
 \quad\text{in }\F_3.
\]
All other reduction roots are nonzero.  The exact total excess
\eqref{eq:boundary-G-root-excess} gives the stated valuations.  If
$A\equiv2\pmod3$, then $v_3(A-1)=0$ and the scaled constant is a unit, so every scaled root is
a unit.
\end{proof}

\subsection{Dilation resultants}

Define
\begin{equation}
  \mathcal R_{2,K}:=\operatorname{Res}_X(F_K(X),F_K(3X)),
  \qquad
  \mathcal R_{3,K}:=\operatorname{Res}_X(G_K(X),G_K(2X)).
  \label{eq:boundary-dilation-resultants}
\end{equation}
These are ordinary integers.

\begin{theorem}[Sharp odd-degree dilation resultants]
\label{thm:boundary-resultant-divisibility}
Let $n=K-1$.

\begin{enumerate}
\item If $M$ is odd and $A$ is even, then either $\mathcal R_{2,K}=0$, in which case
$F_K$ has nonzero roots $\rho,3\rho$, or
\begin{equation}
  v_2(\mathcal R_{2,K})
  \ge n^3+n+v_2(M-1).
  \label{eq:boundary-resultant-v2}
\end{equation}
If $n$ is odd, the zero-resultant alternative is impossible and equality holds in
\eqref{eq:boundary-resultant-v2}.

\item If $3\mid M$ and $3\nmid A$, then either $\mathcal R_{3,K}=0$, in which case
$G_K$ has nonzero roots $\sigma,2\sigma$, or
\begin{equation}
  v_3(\mathcal R_{3,K})
  \ge n^3+v_3(A-1).
  \label{eq:boundary-resultant-v3}
\end{equation}
If $n$ is odd, the zero-resultant alternative is impossible and equality holds in
\eqref{eq:boundary-resultant-v3}.
\end{enumerate}
\end{theorem}

\begin{proof}
Let $a$ be the leading coefficient of $F_K$, a $2$-adic unit, and write its roots as
$\rho_i=2^nz_i$ with $z_i\in\overline\Z_2$.  Put
\[
 D_{2,K}(Z):=
 \frac{\mathcal F_K(3Z)-\mathcal F_K(Z)}{2Z}\in\Z_2[Z].
\]
The quotient is integral because the constant term cancels and $2\mid3^r-1$ for every
$r\ge1$.  At a root $z_i$ of $\mathcal F_K$,
\[
 v_2\bigl(\mathcal F_K(3z_i)\bigr)
 \ge1+v_2(z_i).
\]
If the resultant is nonzero, its root-product formula and
\eqref{eq:boundary-F-root-excess} give
\begin{align*}
 v_2(\mathcal R_{2,K})
 &=v_2\!\left(a^n\prod_{i=1}^nF_K(3\rho_i)\right)\\
 &\ge n^3+\sum_{i=1}^n\bigl(1+v_2(z_i)\bigr)\\
 &=n^3+n+v_2(M-1).
\end{align*}
When $n$ is odd, reduction modulo $2$ gives
\[
 \overline{D_{2,K}}=(\overline{\mathcal F_K})'.
\]
The reduction $1-(Z-1)^n$ is squarefree, so $D_{2,K}(z_i)$ is a unit for every root.
Every displayed inequality is then an equality.  The same unit statement also rules out
$\mathcal F_K(3z_i)=0$; hence the resultant cannot vanish.

For the triadic branch write the roots of $G_K$ as $\sigma_i=3^nz_i$ and put
\[
 D_{3,K}(Z):=
 \frac{\mathcal G_K(2Z)-\mathcal G_K(Z)}{Z}\in\Z_3[Z].
\]
Thus
$v_3(\mathcal G_K(2z_i))\ge v_3(z_i)$, and the root-product formula with
\eqref{eq:boundary-G-root-excess} gives
\[
 v_3(\mathcal R_{3,K})
 \ge n^3+v_3(A-1)
\]
whenever the resultant is nonzero.

It remains to prove exactness for odd $n$.  Write $n=2m+1$.  Over $\F_3$,
\[
 R_n(Z)=(Z+1)^{m+1}(Z-1)^m,
 \qquad
 R_n(-Z)=-(Z-1)^{m+1}(Z+1)^m.
\]
Let $\bar g=R_n(0)-\bar A R_n$; multiplication by the harmless unit in
\eqref{eq:boundary-scaled-G-reduction} does not affect the argument.  If
$\bar g(r)=\bar g(-r)=0$, then $R_n(r)$ and $R_n(-r)$ are nonzero and equal.  Their quotient
forces
\[
 -\frac{r-1}{r+1}=1,
\]
so $r=0$.  This can occur only when $\bar A=1$, and then $0$ is a simple root because
$R_n'(0)/R_n(0)=1$.  Consequently
$\overline{D_{3,K}}(r)\ne0$ at every root $r$ of $\bar g$: for $r\ne0$ this follows from
$\bar g(-r)\ne0$, and for $r=0$ from simplicity.  Hence $D_{3,K}(z_i)$ is a unit for every
root, all valuation inequalities are equalities, and an exact dilation pair is impossible.
\end{proof}

This is a concrete product-formula target.  The resultant is a single algebraic integer, not a
collection of unrelated local periods, and in every odd boundary degree its structural-prime
valuation is now known exactly.  Unfortunately its archimedean height is presently much larger
than the guaranteed local divisor.  The theorem therefore supplies the correct kind of object
but not yet the inequality needed to force it to vanish.

\section{Exact small cases and symbolic audit}
\label{sec:padic-symbolic-audit}

\subsection{The complete $K=2$ residuals}

For $K=2$ put
\[
  \mathfrak c_2:=AM-10A+15M-6,
  \qquad
  \mathfrak d_2:=AM-20A+45M-36.
\]
Direct exact interpolation gives
\begin{align}
 60U_2(X)&=-(M-8)\mathfrak c_2X+2(M-1)\mathfrak d_2,
 \label{eq:K2-U-exact}\\
 60V_2(X)&=-(A-27)\mathfrak c_2X+3(A-1)\mathfrak d_2.
 \label{eq:K2-V-exact}
\end{align}
The formal pair $(M,A)=(5,8)$ obeys the primitive valuation and common-power restrictions but
not the real logarithmic relation.  It gives
\[
  \mathfrak c_2=29,
  \qquad
  \mathfrak d_2=69,
...
\[
  ...
  60V_2=551X+1449.
\]
The dyadic content of the first polynomial is $1$ and the triadic content of the second is
$1$.  Thus the low-degree boundary identities and primitive arithmetic restrictions alone do
not force common content; the Newton-polygon mechanism is genuinely more refined.

\subsection{Smith forms}

Exact Smith-normal-form computation gives
\begin{align*}
 \operatorname{SNF}(E_2)&=(1,1,2,60),\\
 \operatorname{SNF}(E_3)&=(1,1,2,6,120,720,90720,
  407151360,439723468800).
\end{align*}
In particular
\[
  D_2^*=60,
  ...
  D_3^*=439723468800.
\]
The rapid growth is one reason a raw resultant height bound is weak: even after reducing the
boundary degree from $K^2-1$ to $K-1$, the coefficient lattice remembers the full grid.

\subsection{Sample coefficient valuations}

For the formal pair $(M,A)=(5,8)$, the exact coefficient valuations of the integer residuals
are as follows; entries run from the constant coefficient to the leading coefficient.

\begin{center}
\begin{tabular}{ccll}
\toprule
$K$&$n$&$v_2(F_K)$ coefficient list&$v_3(G_K)$ coefficient list\\
\midrule
2&1&$(3,0)$&$(2,0)$\\
3&2&$(6,3,0)$&$(4,2,0)$\\
4&3&$(11,6,3,0)$&$(10,6,4,0)$\\
5&4&$(18,13,9,9,0)$&$(16,12,8,4,0)$\\
\bottomrule
\end{tabular}
\end{center}

The dyadic lists lie on or above the line
$v_2([X^r]F_K)\ge n(n-r)$, exactly as
Theorem~\ref{thm:boundary-exact-scaling} predicts.  For $K=4$, where $n=3$ is odd, the lower
Newton polygon gives root valuations $5,3,3$, agreeing with
Corollary~\ref{cor:boundary-odd-n-roots} because $v_2(5-1)=2$.

For the same pair the exact dyadic resultants have
\begin{center}
\begin{tabular}{cccc}
\toprule
$K$&$n$&$v_2(\mathcal R_{2,K})$&decimal digits of $|\mathcal R_{2,K}|$\\
\midrule
2&1&4&5\\
3&2&14&41\\
4&3&32&163\\
...
\end{tabular}
\end{center}
The strengthened theorem gives lower bounds $4,12,32$, respectively; for the odd values
$n=1,3$ it predicts the observed valuations exactly.  In the triadic branch, the formal pair
$(M,A)=(9,2)$ gives valuations $1,10,27$ for $K=2,3,4$, against the bounds $1,8,27$; again the
odd-degree cases are exact.  The pair $(M,A)=(3,4)$, for which $v_3(A-1)=1$, gives
$2,11,28$, against the sharpened bounds $2,9,28$.  These computations test both the excess term
and the exact odd-degree formula.

\subsection{Reproducible exact-integer core}

The following is the essential part of the symbolic audit.  It uses no floating-point
arithmetic.

\begin{lstlisting}[style=pythonstyle,caption={Exact barycentric and Smith-form audit.}]
...
def barycentric_denominators(K):
    data = []
    for i, j, x in nodes(K):
        d = 1
        for a, b, y in nodes(K):
            if (a, b) != (i, j):
                d *= x - y
        data.append((i, j, x, d))
    D = 1
...
def smith_invariants(K):
    ns = nodes(K)
    R = K*K
    E = Matrix([[Integer(x)**r for r in range(R)]
                for _, _, x in ns])
    S = smith_normal_form(E, domain=ZZ)
    return [abs(int(S[i, i])) for i in range(R)]


def check_extreme_rows(K, valuation):
    data, D = barycentric_denominators(K)
    max2 = max(valuation(d, 2) for _, _, _, d in data)
    max3 = max(valuation(d, 3) for _, _, _, d in data)
    assert all(i == K-1 for i, j, x, d in data
               if valuation(d, 2) == max2)
    assert all(j == K-1 for i, j, x, d in data
               if valuation(d, 3) == max3)
    return D
\end{lstlisting}

The full audit additionally constructs $Q_K,U_K,V_K$ over exact rational-function fields,
checks~\eqref{eq:K2-U-exact}--\eqref{eq:K2-V-exact}, verifies the extreme locations through
$K=8$, and evaluates the sample coefficient floors, root-depth polygons, and resultants above.

\section{How the $p$-adic results interact with the Matrix Coefficient program}
\label{sec:padic-matrix-coefficient}

Dasgupta--Kakde's Matrix Coefficient Conjecture is stated simultaneously for complex and
$p$-adic logarithm matrices~\cite{DasguptaKakdeMCC}.  Their $2\times2$ fewnomial implication
uses a rank-one factorization of the local matrix and a zero bound for a $p$-adic exponential
sum.  Theorem~\ref{thm:structural-prime-rank-jump} shows why that local mechanism does not
apply at the primes $2$ and $3$: the matrices $L_2,L_3$ are full rank and have no rank-one
factorization.

At a good prime $\ell$ with $\Delta_\ell=0$, a rank-one factorization would reappear, and the
fewnomial zero bound could in principle be used.  But such a prime is not known to exist.
Even if it did, Proposition~\ref{prop:shadow-beta-transcendental} says that the resulting
factorization is controlled by a new $\ell$-adic transcendental exponent $\beta_\ell$, not by
the real $\beta$ in any known sense.

The most relevant positive clue in Dasgupta--Kakde is their Question~7.4: construct an integer
auxiliary polynomial that is simultaneously small at an archimedean place and a $p$-adic
place, with the product of the two bounds below one.  Roy's adelic refinement of Hermite
approximation~\cite{Roy2020Adelic} demonstrates that such simultaneous optimization can be
sharp when all local values arise from one exponential-period construction.  The current
problem differs in that the real relation and the full-rank structural-prime logarithms do not
share a single local exponent.  The boundary resultants are useful precisely because they are
honest integers and therefore restore access to the product formula.

A successful synthesis would have to combine four properties:
\begin{enumerate}
\item support below, or effectively below, the factor-four threshold of
Section~\ref{sec:matrix-coefficient};
\item archimedean height small enough to compete with structural-prime divisibility;
\item boundary-sensitive cancellation beyond the inherited min-plus ceiling;
\item compatibility across $K$, so that the deep local roots or resultants are not unrelated
objects at each interpolation level.
\end{enumerate}
The present continuation obtains the third property in a precise local form and identifies a
candidate for the fourth, but does not yet control the first two simultaneously.

\section{Other $p$-adic proof attempts and their exact failure points}
\label{sec:padic-attempt-audit}

\subsection{$p$-adic linear forms in logarithms}

Results of Brumer, Yu, Bugeaud--Laurent, and related authors give strong lower bounds for a
nonzero form
\[
  b_1\log_p\alpha_1+\cdots+b_t\log_p\alpha_t
\]
with algebraic $\alpha_i$ and integer or algebraic coefficients $b_i$ of controlled height.
They are invaluable once a concrete near-relation is available.  Here the real identity has
coefficient $\beta$, which is transcendental, and at $p=2,3$ the corresponding determinant is
nonzero.  Replacing $\beta$ by rational approximants creates a local linear form, but its
smallness is controlled by the local ratio $\log_pM/\log_p2$, not by the real approximation to
$\beta$.  There is no reason for the same rational approximant to be good in both metrics.

\subsection{The $p$-adic analytic subgroup theorem}

The analytic subgroup theorem and its modern variants are qualitative engines for detecting
algebraic subgroups from algebraic points whose logarithms lie in an algebraic subspace; see
Fuchs--Pham~\cite{FuchsPham2015}, Mohajer--Sebbar~\cite{MohajerSebbar2025}, and the recent
application of Burungale--Skinner--Wan~\cite{BurungaleSkinnerWan2026}.  In the present torus,
Theorem~\ref{thm:structural-prime-rank-jump} says that the structural-prime logarithms span the
full Lie algebra.  There is no proper local analytic subspace to which the theorem can be
applied.  At infinity the span is a nonalgebraic line of irrational slope, exactly the
four-exponentials obstruction.

\subsection{$S$-units and the Subspace Theorem}

The pairs $(2^iM^j,3^iA^j)$ form a rank-two subgroup of $\mathbf G_m^2$ consisting of
$S$-units.  The Subspace Theorem is powerful for additive equations or inequalities among
$S$-units, but the defining relation here is a shared real exponent, not a small algebraic
linear form in the coordinates.  Applying an algebraic polynomial to the orbit either yields
a standard exponential polynomial, whose exact zeros are already controlled by
multiplicative independence, or returns to the auxiliary-polynomial support barrier.

\subsection{Finite-field and Fermat-quotient descent}

Equation~\eqref{eq:fermat-quotient-determinant} gives a hierarchy of congruence conditions for
local rank drop.  One could continue to higher Fermat quotients, but doing so merely computes
successive digits of $\Delta_\ell$.  The real relation supplies no known congruence forcing
those digits.  An averaging theorem over $\ell$ for the determinant expression would be
interesting, but a generic nonzero value only confirms local full rank and does not contradict
the real counterexample.

\subsection{Boundary resultants and the archimedean wall}

Theorem~\ref{thm:boundary-resultant-divisibility} is closer to a product-formula argument:
$\mathcal R_{p,K}$ is an integer with a cubic local divisor.  Standard resultant bounds,
however, involve roughly $2n$ powers of the coefficient height of $F_K$ or $G_K$.  The
universal interpolation denominator already has logarithmic height growing much faster than
$n^2$, and the values $M^iA^j$ add a further $K^2\log(MA)$ scale.  The resulting archimedean
upper bound is nowhere near $p^{n^3}$ with current estimates.  The exact computations in
Section~\ref{sec:padic-symbolic-audit} confirm the mismatch: the structural-prime valuation is
large but occupies only a small fraction of the integer's bit length.

The useful conclusion is not that resultants are a dead end.  It is that the \emph{raw}
resultant still carries too much inherited interpolation height.  A successful version must
remove predictable factors, use a subresultant attached only to the deep root cluster, or show
compatibility across $K$ that makes the same algebraic factor recur with increasing
$p$-divisibility.

\section{Literature update through August 21, 2026}
\label{sec:padic-literature-update}

The literature search was directed at the fixed-base conjecture, four exponentials, matrices
of logarithms, $p$-adic structural rank, topological closures, adelic approximation, and
$p$-adic period relations.  No paper located in that search proves the Alaoglu--Erd\H{o}s
conjecture or the four exponentials conjecture.  The following developments are nevertheless
directly relevant to the architecture of a possible proof.

\begin{description}
\item[Dasgupta, 2023.]
Part~I of the logarithm-matrix program~\cite{Dasgupta2023} gives a modern account of
Baker--Brumer, structural rank, and Waldschmidt--Masser.  It makes explicit that the same
structural-rank language applies to complex and $p$-adic logarithm matrices.

\item[Dasgupta--Kakde, 2024/2026.]
The Matrix Coefficient Conjecture~\cite{DasguptaKakdeMCC} packages singular logarithm matrices
into the assertion that a rational matrix coefficient vanishes after changes of basis.  In
size two it is equivalent to four exponentials.  Its support and fewnomial strategy, and its
question about simultaneous archimedean/$p$-adic smallness, are the closest current framework
to the exact factor-four and product-formula barriers isolated in this report.

\item[Prasad, 2025.]
Prasad~\cite{Prasad2025} studies closures of finitely generated algebraic-point subgroups in
real and $p$-adic groups, with emphasis on tori and abelian varieties, and explicitly places
the Alaoglu--Erd\H{o}s question among four-exponentials phenomena.  The viewpoint motivates
Theorem~\ref{thm:padic-closure-dimension}: a counterexample would have one-dimensional real
logarithmic closure but two-dimensional structural-prime closure.

\item[Betina--Maksoud--Pozzi, 2025.]
In their work on eigencurves and Gross--Stark regulators~\cite{BetinaMaksoudPozzi2025}, the
authors isolate the Baker--Brumer kernel statement for the $\Q$-linear extension of
$\log_p$ and formulate a $p$-adic four-exponentials variant under independence of all matrix
entries.  That hypothesis does not apply directly to $L_2$ or $L_3$, because a structural
prime forces one entry to be zero; the remaining entries nevertheless make the determinant
nonzero by Theorem~\ref{thm:structural-prime-rank-jump}.

\item[Roy, 2020.]
Roy's adelic Hermite approximation theorem~\cite{Roy2020Adelic} shows that simultaneous
approximation to exponential values can be nearly optimal when the local data arise from one
adelic construction.  It is evidence that the right missing object may be adelic rather than
purely archimedean or purely $p$-adic.

\item[Mohajer--Sebbar, 2025.]
Their theory of $p$-adic periods of $1$-motives~\cite{MohajerSebbar2025} develops new
$p$-adic integration and subgroup-theorem machinery, proving linear-relation statements at
several depths for canonical rational structures.  It does not settle the toric
four-exponentials problem, but it is part of a broader movement toward comparing structural
and actual ranks of $p$-adic period matrices.

\item[Burungale--Skinner--Wan, 2026.]
Their refined nonvanishing theorem for $p$-adic logarithms of rational points on abelian
varieties~\cite{BurungaleSkinnerWan2026} uses the $p$-adic analytic subgroup theorem and
formulates structural-rank consequences for logarithm matrices and closure dimensions.  The
setting is abelian varieties rather than the two-dimensional torus here, but the structural
rank language closely matches Proposition~\ref{prop:real-structural-rank-two}.
\end{description}

The recent papers sharpen the conceptual map but do not provide a hidden local transfer
principle.  In particular, none turns a real rank drop into a rank drop at a finite place.  The
natural modern formulation of the missing step is adelic: construct one algebraic auxiliary
object whose local smallness is forced for different reasons at different places.

\section{A focused research program}
\label{sec:padic-research-program}

The continuation suggests the following order of attack.

\subsection{Compatibility of the scaled residuals}

The polynomials $\mathcal F_K$ and $\mathcal G_K$ have explicit reductions, while their
degrees increase by one with $K$.  The first question is whether
the interpolation nesting from the $K\times K$ grid to the $(K+1)\times(K+1)$ grid induces a
congruence, a divisibility relation, or a stable factor among the scaled residuals.  Any
nonconstant algebraic factor recurring for infinitely many $K$ would inherit unbounded
structural-prime divisibility from Theorem~\ref{thm:boundary-resultant-divisibility}, creating a
credible product-formula contradiction.

A concrete target is to compute the subresultant sequence of
$F_K(X)$ and $F_K(3X)$ after removing the predictable $2^{n^3+n+v_2(M-1)}$ factor, and similarly after removing
$3^{n^3+v_3(A-1)}$ in the triadic branch.  Stabilization of even one primitive factor would be far more useful than growth of the
raw resultant.

\subsection{Exploit the odd-$n$ exact formulas}

For even $K$ the scaled dyadic residual has squarefree reduction and a completely determined
root-depth multiset.  This gives a canonical distinguished root---the unique root deeper than
$n$.  In the triadic branch the same phenomenon holds when $A\equiv1\pmod3$, while
Theorem~\ref{thm:boundary-resultant-divisibility} gives an exact dilation-resultant valuation
for every odd $n$ in either branch.  Hensel lifting makes each distinguished root an analytic
function of the scaled coefficients.  One should derive its first several structural-prime
digits and compare them as $K$ varies.  A recurrence carrying a distinguished root from $K$ to
$K+2$ would turn the current family of unrelated local roots into one $p$-adic analytic object.

\subsection{Complete the residue branches}

The exact scaling theorem uses $A$ even at $p=2$ or $3\mid M$ at $p=3$.  The complementary
branches exhibit cancellation among several extreme barycentric terms.  Rather than treating
that cancellation as an inconvenience, it may provide still deeper divisibility.  The correct
objects are the top-row polynomial
\[
  \sum_{j\in J_{2,K}}\overline w_{n,j}\,\overline A^{\,j}
\]
and the right-column analogue over $\F_3$, where $J_{p,K}$ indexes the extreme denominators.
Determining these polynomials and their multiplicities could produce a complete case split for
all primitive generators.

\subsection{Good-prime determinant statistics}

For good primes, compute the Fermat-quotient determinant
\[
  q_\ell(2)q_\ell(A)-q_\ell(M)q_\ell(3)
\]
and its higher lifts.  The immediate goal is not to find a rank-drop prime by experiment, but
to determine whether the values display a systematic bias forced by the real power relation.
Any theorem giving anomalously many zeros or high valuations would create an adelic input not
visible to one-place transcendence theory.

\subsection{An adelic auxiliary polynomial at the exact threshold}

The factor-four theorem in the third continuation specifies the support gain required.  The
new local scaling theorem specifies a finite-place gain available on the boundary.  A useful
auxiliary construction should interpolate only approximately over the real grid, exactly or
very deeply over a structural-prime boundary cluster, and use a geometry-of-numbers lattice
whose covolume includes both contributions.  The benchmark is explicit: after evaluation at
an algebraic grid point, the product of the archimedean bound and all selected $p$-adic bounds
must be below one.  This is the fixed-base version of Dasgupta--Kakde's analytic-vanishing
question.

\subsection{Formalization targets}

The most suitable statements for near-term Lean formalization are:
\begin{enumerate}
\item the rational kernel of Iwasawa logarithm, assuming or importing the required $p$-adic
logarithm API;
\item the elementary determinant identities~\eqref{eq:delta2-nonzero} and
\eqref{eq:delta3-nonzero};
\item the Smith/cokernel description of $\delta_K$;
\item the formulas~\eqref{eq:boundary-v2-dij}--\eqref{eq:boundary-v3-dij} and the endpoint
maximization via LTE;
\item the polynomial scaling theorem, phrased coefficientwise as
$v_2([X^r]F_K)\ge n(n-r)$ and its triadic analogue;
\item the resultant divisibility theorem, once the polynomial resultant API is sufficient.
\end{enumerate}
These are finite algebraic and valuation statements.  Unlike four exponentials itself, they do
not require formalizing deep transcendence theory.

\section{Conclusions of the fourth continuation}
\label{sec:fourth-conclusion}

The $p$-adic investigation does not prove the Alaoglu--Erd\H{o}s conjecture, but it eliminates
a misleading picture and replaces it with a more exact one.

A counterexample would not be locally rank one at the primes $2$ and $3$.  Its real logarithm
matrix would have analytic rank one and structural rank two, while both structural-prime
Iwasawa matrices would have actual rank two.  After principal-unit normalization, the two
generators would span open $2$-adic and $3$-adic subgroups and equidistribute with respect to
Haar measure.  The local geometry is therefore maximally two-dimensional at the very primes
one might first hope would detect the real dependence.

The boundary residuals nevertheless remember the structural primes in a strong arithmetic
way.  In favorable residue branches, the minimal-denominator residuals have an exact
$p^{n^2}$ rescaling, all their roots lie at depth at least $n$, and the total excess depth and
residue clusters are known.  The resulting dilation resultants satisfy the sharpened bounds
\[
 v_2(\mathcal R_{2,K})\ge n^3+n+v_2(M-1),
 \qquad
 v_3(\mathcal R_{3,K})\ge n^3+v_3(A-1),
\]
unless they vanish through an exact dilation pair.  For odd boundary degree they do not vanish,
and both displayed inequalities are equalities.

These results move the problem from ``find some $p$-adic divisibility'' to a sharper target:
remove the predictable interpolation height or prove compatibility across $K$.  A raw
resultant is too large archimedeanly, and a local logarithm determinant is not an algebraic
number to which the product formula applies.  The surviving route is genuinely adelic: one
must manufacture a single algebraic auxiliary value whose real smallness comes from the
rank-one power curve and whose finite-place smallness comes from the structural-prime boundary
scaling.

\begin{resultbox}
\textbf{Bottom line.}
No complete proof was found.  The continuation proves that the naive local rank-one strategy
cannot work at $2$ or $3$, establishes open structural-prime closures and quantitative
recurrence ceilings, identifies transcendental good-prime shadow exponents, and derives exact
coefficient and root-depth normalization for the boundary residuals.  The strongest new
arithmetic object is a unit-dilation resultant whose valuation is known exactly in every odd
boundary degree.  Closing the conjecture now requires either an archimedean height reduction
for that object, a stable factor linking different interpolation levels, or an adelic auxiliary
polynomial that crosses the exact factor-four support barrier.
\end{resultbox}
...
% --- imported with the fourth ($p$-adic) continuation ---

\bibitem{Brumer1967}
A.~Brumer,
\newblock \emph{On the units of algebraic number fields},
\newblock Mathematika \textbf{14}(2) (1967), 121--124.
\newblock DOI: \href{https://doi.org/10.1112/S0025579300003703}{10.1112/S0025579300003703}.

\bibitem{Dasgupta2023}
S.~Dasgupta,
\newblock \emph{Ranks of matrices of logarithms of algebraic numbers I: the theorems of Baker
and Waldschmidt--Masser},
\newblock Essential Number Theory \textbf{2}(1) (2023), 93--138.
...
\newblock preprint \href{https://arxiv.org/abs/2505.15744}{arXiv:2505.15744}, submitted
May~21, 2025; manuscript dated August~11, 2026.

\bibitem{BetinaMaksoudPozzi2025}
A.~Betina, A.~Maksoud, and A.~Pozzi,
\newblock \emph{The eigencurve at crystalline points with scalar Frobenius and Gross--Stark
regulators},
\newblock preprint \href{https://arxiv.org/abs/2502.00876}{arXiv:2502.00876}, 2025.

\bibitem{Roy2020Adelic}
D.~Roy,
\newblock \emph{Simultaneous approximation to values of the exponential function over the
adeles},
\newblock Mathematische Annalen \textbf{377} (2020), 1057--1093.
\newblock DOI: \href{https://doi.org/10.1007/s00208-020-02005-5}{10.1007/s00208-020-02005-5};
\href{https://arxiv.org/abs/1905.01678}{arXiv:1905.01678}.

\bibitem{FuchsPham2015}
C.~Fuchs and D.~H.~Pham,
\newblock \emph{The $p$-adic analytic subgroup theorem revisited},
\newblock p-Adic Numbers, Ultrametric Analysis and Applications \textbf{7}(2) (2015), 143--156.
\newblock DOI: \href{https://doi.org/10.1134/S2070046615020065}{10.1134/S2070046615020065};
\href{https://arxiv.org/abs/1502.00768}{arXiv:1502.00768}.

\bibitem{MohajerSebbar2025}
M.~Mohajer and A.~Sebbar,
\newblock \emph{$p$-adic Period Conjectures for 1-motives: Integration and Linear Relations},
\newblock preprint \href{https://arxiv.org/abs/2507.15020}{arXiv:2507.15020}, 2025.

\bibitem{BurungaleSkinnerWan2026}
A.~Burungale, C.~Skinner, and X.~Wan,
\newblock \emph{A refined non-vanishing of the $p$-adic logarithm of a rational point on an
abelian variety},
\newblock Annales math\'ematiques du Qu\'ebec, to appear;
\href{https://arxiv.org/abs/2603.20886}{arXiv:2603.20886v2}, 2026.
