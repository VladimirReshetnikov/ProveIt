...
\begin{statusbox}{precise research status}
...
\texttt{alaoglu-erdos-unified-report.tex.gz}.
...
only $O(Q^{-1})$.  Standard irrationality measures for $\log_2 3$ therefore give
...
The exact scaling in \eqref{eq:zero-block-scaling} implies a family of exact
...
The number $\beta=\log_2 M$ is Diophantine-generic by the same logarithmic-form
...
If $\alpha=\log_B A$ for multiplicatively independent positive integers
$A,B>1$, then
...
Approximate $\log_2 3$ by the residual ratio &
...
theorem lift23_le_rpow (m : Nat) :
  (lift23 m : Real) <= (m : Real) ^ (Real.logb 2 3)

theorem lift32_ge_rpow (n : Nat) :
  (n : Real) ^ (Real.logb 3 2) <= (lift32 n : Real)
...
attached LaTeX source supplied for this continuation, 20,483 lines,
