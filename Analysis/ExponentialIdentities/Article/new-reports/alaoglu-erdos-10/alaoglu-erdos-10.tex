...
#pragma omp for schedule(static)
  ...
  printf("mpfr_version=printf("range=[printf("precision_bits=printf("checked_nonpowers=printf("failures=printf("min_lower_log_margin=total.min_lower, total.min_lower_m, total.min_lower_n);
  printf("min_upper_log_margin=total.min_upper, total.min_upper_m, total.min_upper_n);
  if (total.failures) {
    printf("first_failure=(m,guess)=(total.first_fail_m, total.first_fail_guess);
  }
...
\caption{Classical transcendence input consumed by each family of results.}\label{a4:tab:deps}\\
\toprule
...
\section{Matrix theory, Loewner determinants, and the arithmetic obstruction}
\label{sec:matrix-continuation}

This section records a further attempt to attack the Alaoglu--Erd\H{o}s
conjecture by matrix methods.  The outcome is deliberately separated into
three levels.

\begin{enumerate}
\item There is an exact $2\times2$ integral-matrix reformulation of the
      conjecture.  It is useful conceptually, but by itself it merely repackages
      the two original integrality assumptions.
\item Loewner matrices and generalized Vandermonde determinants do produce
      genuinely new arithmetic certificates: certain analytic determinants
      become nonzero integers, and this gives explicit cluster-exclusion and
      candidate-repulsion inequalities.
\item The quantitative scale of those inequalities exposes a precise barrier.
      Determinants made only from the two available exponential directions have
      a limiting ``one-half exponent'' and do not beat the height cost of the
      auxiliary integers.  This is the matrix shadow of the four-exponentials
      obstruction, not an unconditional proof of the conjecture.
\end{enumerate}

Thus the present continuation does not claim a complete unconditional proof.
It does give several rigorous results that can be reused in a future auxiliary-
function or transcendence argument, and it identifies exactly which seemingly
promising matrix constructions are formally incapable of adding information.

\subsection{Notation and the irreducible transcendence bottleneck}

Put
\[
  \mathcal C=\{t\in\mathbb R:2^t,3^t\in\mathbb Z\},
  \qquad
  \mathcal S=\{2^a3^b:a,b\in\mathbb Z_{\ge0}\}.
\]
If $t\in\mathcal C$, then $t\ge0$, and every $s\in\mathcal S$ satisfies
$s^t\in\mathbb Z$.  Moreover $\mathcal C$ is an additive semigroup and
$\mathbb Z_{\ge0}\subseteq\mathcal C$.

The elementary reductions remain decisive:

\begin{itemize}
\item If $t\in\mathbb Q$ and $2^t\in\mathbb Z$, then $t\in\mathbb Z$ by the
      $2$-adic valuation.
\item If $t$ is algebraic and irrational, then $2^t$ is transcendental by the
      Gelfond--Schneider theorem.  Hence a nonintegral member of $\mathcal C$
      would have to be transcendental.
\item Writing $m=2^t$ and $n=3^t$, a counterexample is exactly a second
      multiplicatively independent integral point $(m,n)$ on the real monomial
      curve
      \[
        y=x^{\log 3/\log 2},
      \]
      besides the points $(2^k,3^k)$.
\end{itemize}

Equivalently, all four entries of
\begin{equation}
  E(t)=
  \begin{pmatrix}
    \exp((\log2)\cdot1) & \exp((\log2)t)\\
    \exp((\log3)\cdot1) & \exp((\log3)t)
  \end{pmatrix}
  =
  \begin{pmatrix}2&2^t\\3&3^t\end{pmatrix}
  \label{eq:four-exp-grid}
\end{equation}
are algebraic.  The numbers $\log2,\log3$ are linearly independent over
$\mathbb Q$, and $1,t$ are linearly independent over $\mathbb Q$ precisely
when $t$ is irrational.  Consequently the Four Exponentials Conjecture would
immediately force $t\in\mathbb Q$, hence $t\in\mathbb Z$.  Matrix language does
not remove this bottleneck; the useful question is whether integrality, as
opposed to mere algebraicity, supplies enough extra determinant size to cross
it.

\subsection{An exact $2\times2$ integral-matrix reformulation}
\label{subsec:2x2-reformulation}
...
\begin{equation}
  ...
  \label{eq:matrix-A}
\end{equation}
Its eigenvalues are $2$ and $3$, and its integral spectral projectors are
\[
  P_2=3I-A,
  \qquad
  P_3=A-2I.
\]
They satisfy $P_i^2=P_i$, $P_2P_3=0$, and $P_2+P_3=I$.  Since the spectrum is
positive, the real power $A^t$ is unambiguously defined by the real functional
calculus, and
\begin{equation}
  A^t=2^tP_2+3^tP_3
     =(3\,2^t-2\,3^t)I+(3^t-2^t)A.
  \label{eq:A-real-power}
\end{equation}
...
\begin{equation}
  ...
  \quad\Longleftrightarrow\quad
  2^t,3^t\in\mathbb Z.
  \label{eq:matrix-equivalence}
\end{equation}
Indeed, the reverse implication follows directly from
\eqref{eq:A-real-power}.  Conversely, if $B=A^t$ is integral, then
\[
  c=B_{21}=3^t-2^t\in\mathbb Z,
  \qquad
  d=B_{11}=3\,2^t-2\,3^t\in\mathbb Z.
\]
Solving gives $2^t=d+2c$ and $3^t=d+3c$, both integers.

The reformulation can be made even more transparent.  With
\[
...
one has
\[
  A=S\begin{pmatrix}2&0\\0&3\end{pmatrix}S^{-1}.
\]
Thus $S$ is unimodular and $A^t$ is integral exactly when the diagonal matrix
$\operatorname{diag}(2^t,3^t)$ is integral.  The Alaoglu--Erd\H{o}s conjecture
is therefore equivalent to the assertion
\begin{equation}
  \{t\in\mathbb R:A^t\in M_2(\mathbb Z)\}=\mathbb Z_{\ge0}.
  \label{eq:integral-times}
\end{equation}

There is a useful structural reason why the eigenvalue gap $3-2=1$ makes this
reformulation exact but arithmetically nonrestrictive.  More generally, if an
integral matrix $T$ satisfies
\[
  (T-2I)(T-3I)=0,
\]
then $3I-T$ and $T-2I$ are complementary integral idempotents.  They split the
underlying lattice as a direct sum, so an integral change of basis makes $T$ a
block diagonal matrix with a $2I$ block and a $3I$ block.  There is no hidden
lattice gluing from which an additional congruence could be extracted.

The logarithmic form is
\[
  A^t=\exp(t\log A),
  \qquad
  \log A=(\log2)P_2+(\log3)P_3.
\]
Hence the conjecture asks for the integral points on one specific analytic
one-parameter subgroup of the split real torus in the algebra
$\mathbb R[A]\cong\mathbb R\times\mathbb R$.  Its Zariski closure is
$2$-dimensional because $2$ and $3$ are multiplicatively independent, whereas
the analytic subgroup itself is one-dimensional.  This mismatch is precisely
why algebraic-group methods naturally lead back to four exponentials.

\paragraph{Conditional matrix theorem.}
Assume the Four Exponentials Conjecture.  Let $T$ be a diagonalizable matrix
with algebraic entries and with two positive, multiplicatively independent
algebraic eigenvalues $\lambda_1,\lambda_2$.  If $T^u$ has algebraic entries,
then $u\in\mathbb Q$.  In fact the eigenvalues
$\lambda_1^u,\lambda_2^u$ of $T^u$ are algebraic.  If $u$ were irrational,
then the two rows $\log\lambda_1,\log\lambda_2$ and the two columns $1,u$
would give four algebraic exponentials in the forbidden $2\times2$ grid.
For $T=A$, integrality of $A^u$ then implies $u\in\mathbb Z$.

\subsection{The single-matrix and representation-theoretic barriers}
\label{subsec:single-matrix-barrier}

The algebra generated by $A$ is
\[
  \mathbb Q[A]\cong\mathbb Q\times\mathbb Q,
  \qquad
  f(A)\longmapsto(f(2),f(3)).
\]
Under this isomorphism, $A^t$ corresponds to $(m,n)=(2^t,3^t)$.  Therefore
any matrix obtained from the two endpoint matrices $A$ and $A^t$ by rational
algebraic operations has entries in $\mathbb Q(m,n)$.  If only integral
polynomial operations are used, its entries lie in $\mathbb Z[m,n]$.
Once $m,n$ are assumed integral, every resulting integrality statement holds
formally for an arbitrary pair of integers $(m,n)$; it does not remember that
$m$ and $n$ arose at the same real time $t$.

This observation covers most natural matrix amplifications.

\begin{itemize}
\item Tensor, symmetric, and exterior powers give weights $2^a3^b$, whose
      $t$th powers are $m^an^b$.  They add many integral eigenvalues but no new
      logarithmic direction.
\item For every polynomial representation $\rho$ defined over $\mathbb Z$,
      \[
        \rho(A^t)=\rho(A)^t.
      \]
      The left side is an integral polynomial in the entries of $A^t$, and its
      spectral data again consist only of monomials in $m,n$.
\item Direct sums and block triangular extensions produce linear recurrences
      with characteristic roots $2$ and $3$.  The shifted sequence
      $A^{t+k}=A^tA^k$ is integral for every $k\ge0$, but every pair of integers
      $m,n$ already produces such an integral shifted recurrence.
\item The tempting Kronecker sum does not evade the barrier.  Although
      $A\otimes I+I\otimes A$ has eigenvalues $4,5,5,6$, its real $t$th power is
      not determined by $A^t$.  In particular
      $(A\otimes I+I\otimes A)^t$ is not
      $A^t\otimes I+I\otimes A^t$ unless $t=1$ or in other exceptional cases.
      The new base $5$ has simply been smuggled in without an integrality
      hypothesis for $5^t$.
\end{itemize}

This gives a precise no-go statement for a broad class of arguments: a proof
must use more than formal algebra in the pair $(A,A^t)$.  It must exploit
analytic dependence on $t$, inequalities, denominator growth, or a genuinely
new transcendence input.  The Loewner construction below does use analytic
dependence, through divided differences, and consequently yields nontrivial
quantitative information.

\subsection{Generalized Vandermonde alternants}
\label{subsec:generalized-vandermonde}

For pairwise distinct positive numbers $z_1,\dots,z_N$ and pairwise distinct
real exponents $\beta_1,\dots,\beta_N$, define
\[
  V_{\boldsymbol\beta}(\mathbf z)
  =\det\bigl(z_i^{\beta_j}\bigr)_{1\le i,j\le N}.
\]
This determinant never vanishes.  One way to see this is to put
$z_i=e^{u_i}$: the functions $e^{\beta_j u}$ form an extended complete
Chebyshev system.  Equivalently, their Wronskian is
\[
  \det\left(\frac{d^{i-1}}{du^{i-1}}e^{\beta_j u}\right)
  =e^{u\sum_j\beta_j}\prod_{j<k}(\beta_k-\beta_j),
\]
which has a fixed nonzero sign.  If both the $u_i$ and the $\beta_j$ are in
increasing order, then $V_{\boldsymbol\beta}(\mathbf z)>0$; this is the strict
total positivity of the exponential kernel $e^{uv}$.

For the present problem the important exponent list is
\begin{equation}
  0,1,\dots,r-1,
  \qquad
  t,t+1,\dots,t+r-1.
  \label{eq:two-block-exponents}
\end{equation}
If $t$ is nonintegral, all $2r$ exponents are distinct.  If
$z_i\in\mathcal S$ and $t\in\mathcal C$, then every entry of the associated
alternant is an integer.  Consequently
\begin{equation}
  \left|
  \det\begin{pmatrix}
    1&z_1&\cdots&z_1^{r-1}&z_1^t&z_1^{t+1}&\cdots&z_1^{t+r-1}\\
    \vdots&\vdots&&\vdots&\vdots&\vdots&&\vdots\\
    1&z_{2r}&\cdots&z_{2r}^{r-1}&z_{2r}^t&z_{2r}^{t+1}&\cdots&z_{2r}^{t+r-1}
  \end{pmatrix}
  \right|\ge1
  \label{eq:integer-alternant-lower-bound}
\end{equation}
for every nonintegral candidate $t$ and every choice of distinct
$z_i\in\mathcal S$.

There is also an exact finite witness for integrality versus nonintegrality.
Choose $r>t$.  If $t$ is an integer, the two lists in
\eqref{eq:two-block-exponents} overlap and the determinant vanishes.  If $t$
is nonintegral, it is a nonzero integer.  This does not by itself yield a
contradiction, but it converts the conjecture into the problem of forcing one
of these nonzero integer alternants to have absolute value $<1$.

\subsection{The cross-Loewner identity}
\label{subsec:cross-loewner}

Let $X=(x_1,\dots,x_r)$ and $Y=(y_1,\dots,y_r)$ consist of distinct numbers,
with $x_i\ne y_j$, and let $f$ be any function defined on these $2r$ points.
The cross-Loewner matrix is
\[
  L_f(X,Y)=\left(\frac{f(x_i)-f(y_j)}{x_i-y_j}\right)_{1\le i,j\le r}.
\]
Define the $2r\times2r$ alternant
\[
  M_f(X,Y)=
  \begin{pmatrix}
    1&x_1&\cdots&x_1^{r-1}&f(x_1)&x_1f(x_1)&\cdots&x_1^{r-1}f(x_1)\\
    \vdots&\vdots&&\vdots&\vdots&\vdots&&\vdots\\
    1&x_r&\cdots&x_r^{r-1}&f(x_r)&x_rf(x_r)&\cdots&x_r^{r-1}f(x_r)\\
    1&y_1&\cdots&y_1^{r-1}&f(y_1)&y_1f(y_1)&\cdots&y_1^{r-1}f(y_1)\\
    \vdots&\vdots&&\vdots&\vdots&\vdots&&\vdots\\
    1&y_r&\cdots&y_r^{r-1}&f(y_r)&y_rf(y_r)&\cdots&y_r^{r-1}f(y_r)
  \end{pmatrix}.
\]
Then the Sylvester--Loewner determinant identity is
\begin{equation}
  \det M_f(X,Y)
  =(-1)^{r(r+1)/2}
    \left(\prod_{i=1}^r\prod_{j=1}^r(x_i-y_j)\right)
    \det L_f(X,Y).
  \label{eq:sylvester-loewner}
\end{equation}
A direct proof is obtained by Laplace-expanding the left side in its last
$r$ columns, expanding the determinant on the right in the values
$f(x_i),f(y_j)$, and applying the Cauchy determinant formula to each matching
minor.  The overall sign can be calibrated by taking $f(u)=u^r$, when the left
side is an ordinary Vandermonde determinant and
$L_f(x,y)=\sum_{k=0}^{r-1}x^{r-1-k}y^k$ factors into two Vandermonde matrices
and a reversal matrix.

For $f(u)=u^t$, the left side is exactly the generalized Vandermonde determinant
with exponents \eqref{eq:two-block-exponents}.  Hence, for a nonintegral
candidate $t$ and semigroup nodes $X,Y\subset\mathcal S$,
\begin{equation}
  0\ne
  \left(\prod_{i,j}(x_i-y_j)\right)\det L_{u^t}(X,Y)
  \in\mathbb Z.
  \label{eq:loewner-integer-certificate}
\end{equation}
Every Loewner entry is rational, but the cleared determinant is not merely
rational: it is an explicitly identified nonzero integer generalized
Vandermonde determinant.  This is the strongest purely algebraic matrix
certificate obtained in the present continuation.

\subsection{A positive rational determinant from the fractional part}
\label{subsec:positive-loewner}

Suppose now that $t=k+\alpha$ with $k=\lfloor t\rfloor$ and
$0<\alpha<1$.  For $u,v>0$ put
\[
  K_\alpha(u,v)=\frac{u^\alpha-v^\alpha}{u-v},
\]
with the continuous value $\alpha u^{\alpha-1}$ on the diagonal.  The classical
integral representation
\begin{equation}
  K_\alpha(u,v)
  =\frac{\sin(\pi\alpha)}{\pi}
    \int_0^\infty
      \frac{\lambda^\alpha}{(u+\lambda)(v+\lambda)}\,d\lambda
  \label{eq:loewner-integral}
\end{equation}
shows that this kernel is strictly totally positive.  Indeed, Andreief's
identity expresses a cross minor as an integral of a product of two Cauchy
determinants:
\begin{align}
 \det\bigl(K_\alpha(x_i,y_j)\bigr)_{i,j=1}^r
  & =\left(\frac{\sin(\pi\alpha)}{\pi}\right)^r
     \int_{0<\lambda_1<\cdots<\lambda_r}
       \det\left(\frac1{x_i+\lambda_j}\right)
       \det\left(\frac1{y_i+\lambda_j}\right)
       \prod_{j=1}^r\lambda_j^\alpha\,d\boldsymbol\lambda.
  \label{eq:andreief-loewner}
\end{align}
For increasing $X$ and $Y$, the two Cauchy determinants have the same strict
sign, so the integral is positive.

If $t\in\mathcal C$ and $s\in\mathcal S$, then
\[
  s^\alpha=\frac{s^t}{s^k}\in\mathbb Q.
\]
Thus every off-diagonal cross-Loewner entry $K_\alpha(x_i,y_j)$ is rational.
Combining strict total positivity with \eqref{eq:sylvester-loewner} gives an
arithmetic strengthening.  For increasing, disjoint $X,Y\subset\mathcal S$,
set
\begin{equation}
 \mathfrak I_{r,t}(X,Y)=
 \left(\prod_{i=1}^r x_i^k y_i^k\right)
 \left|\prod_{i,j}(x_i-y_j)\right|
 \det\bigl(K_\alpha(x_i,y_j)\bigr)_{i,j=1}^r.
 \label{eq:positive-integer-loewner}
\end{equation}
Then
\begin{equation}
  \mathfrak I_{r,t}(X,Y)\in\mathbb Z_{>0}.
  \label{eq:positive-integer-conclusion}
\end{equation}
To verify integrality, multiply the row belonging to a node $z$ in the
alternant $M_{u^\alpha}$ by $z^k$.  The first block becomes
$z^k,\dots,z^{k+r-1}$ and the second becomes
$z^t,\dots,z^{t+r-1}$, all integers.  Nonvanishing and positivity follow from
total positivity.  Formula \eqref{eq:positive-integer-conclusion} is a useful
bridge between operator-monotone matrix theory and the arithmetic problem: an
analytic Gram determinant, involving the transcendental parameter $\alpha$,
is forced onto a discrete positive lattice after a completely explicit
normalization.

It is important not to overread the positivity.  The kernel
$K_\alpha$ is strictly totally positive for every $0<\alpha<1$, whether or not
$2^{k+\alpha}$ and $3^{k+\alpha}$ are integers.  The arithmetic content lies
entirely in the denominator normalization in
\eqref{eq:positive-integer-loewner}; positivity alone cannot select integral
$\alpha$.

\subsection{A quantitative alternant bound}
\label{subsec:quantitative-alternant}

The nonzero integer in \eqref{eq:integer-alternant-lower-bound} becomes useful
only when accompanied by an analytic upper bound.  The following elementary
bound is convenient because all constants are explicit.

Let $0\le\beta_1,\dots,\beta_N\le B$, put
\[
  \Sigma=\sum_{j=1}^N\beta_j,
  \qquad
  M=\frac{N(N-1)}2,
  \qquad
  B_*=\max(1,B),
\]
and let $z_i=e^{u_i}$ with
\[
  T\le u_i\le T+\delta,
  \qquad 0<\delta\le1.
\]
Then
\begin{equation}
  \left|\det(z_i^{\beta_j})_{i,j=1}^N\right|
  \le
  e^{T\Sigma}\delta^M
  \frac{N^{N/2}e^{NB}B_*^M}{\prod_{\ell=0}^{N-1}\ell!}.
  \label{eq:alternant-upper-bound}
\end{equation}

To prove this, factor $e^{T\beta_j}$ from column $j$ and write
$v_i=u_i-T\in[0,\delta]$.  Passing from function values
$e^{\beta_jv_i}$ to Newton divided differences factors out the ordinary
Vandermonde $\prod_{i<j}(v_j-v_i)$.  A divided difference of order $\ell$ is
bounded by
\[
  \frac{\sup_{0\le v\le\delta}
  |(d/dv)^\ell e^{\beta_jv}|}{\ell!}
  \le \frac{\beta_j^\ell e^{\beta_j\delta}}{\ell!}.
\]
Hadamard's inequality bounds the norm of the row of order $\ell$ by
$N^{1/2}B_*^\ell e^B/\ell!$, while the Vandermonde is at most $\delta^M$.
Multiplication over $\ell=0,\dots,N-1$ yields
\eqref{eq:alternant-upper-bound}.

\subsection{An explicit $2r$-point logarithmic gap bound}
\label{subsec:2r-gap}

Apply \eqref{eq:alternant-upper-bound} to the two-block exponents
\eqref{eq:two-block-exponents}.  Here
\[
  N=2r,
  \quad
  M=r(2r-1),
...
Define
\begin{equation}
  C_{r,t}=\frac{(2r)^r e^{2r(t+r-1)}
  \max(1,t+r-1)^{r(2r-1)}}
  {\prod_{\ell=0}^{2r-1}\ell!}.
  \label{eq:Crt}
\end{equation}

\paragraph{$2r$-point cluster exclusion.}
Let $t\in\mathcal C\setminus\mathbb Z$ and let
$z_1,\dots,z_{2r}$ be distinct elements of $\mathcal S$ contained in
\[
  [R,Re^\delta],
  \qquad R\ge1,
  \qquad 0<\delta\le1.
...
\begin{equation}
  ...
  C_{r,t}^{-1/[r(2r-1)]}
  R^{-(t+r-1)/(2r-1)}.
  \label{eq:2r-gap-bound}
\end{equation}
Indeed, the determinant is a nonzero integer by
\eqref{eq:integer-alternant-lower-bound}, hence has absolute value at least
$1$; the upper bound \eqref{eq:alternant-upper-bound} gives
\eqref{eq:2r-gap-bound}.

This theorem is nontrivial: a hypothetical counterexample would impose an
explicit exclusion zone on every $2r$-tuple of $3$-smooth numbers.  However,
for fixed $t$ the exponent in \eqref{eq:2r-gap-bound} tends only to $1/2$ as
$r\to\infty$:
\[
  \frac{t+r-1}{2r-1}\longrightarrow\frac12.
\]
That limiting scale is much smaller than the logarithmic spacings supplied by
simple counting of $3$-smooth numbers, so the theorem does not force a
contradiction.

For comparison, the number of elements of $\mathcal S$ not exceeding $R$ is
\begin{equation}
  \#\{s\in\mathcal S:s\le R\}
  =\frac{(\log R)^2}{2\log2\log3}+O(\log R).
  \label{eq:smooth-count}
\end{equation}
This follows by counting lattice points $(a,b)\in\mathbb Z_{\ge0}^2$ in the
triangle $a\log2+b\log3\le\log R$.  Counting alone therefore guarantees
logarithmic gaps only on the order of $1/\log R$, whereas a contradiction from
\eqref{eq:2r-gap-bound} would need clusters on a power scale close to
$R^{-1/2}$.  The mismatch is exponential in $\log R$.

\subsection{Mixed alternants and the one-half barrier}
\label{subsec:mixed-alternants}

The equal block sizes in the Loewner identity are not forced if one works
directly with generalized Vandermonde matrices.  Take $p,q\ge1$,
$N=p+q$, and the exponent set
\begin{equation}
  0,1,\dots,p-1,
  \qquad
  t,t+1,\dots,t+q-1.
  \label{eq:mixed-exponents}
\end{equation}
For a nonintegral candidate $t$ and distinct nodes from $\mathcal S$, the
associated determinant is again a nonzero integer.  Its total exponent is
\begin{equation}
  \Sigma(p,q;t)
  =\frac{p(p-1)}2+qt+\frac{q(q-1)}2.
  \label{eq:mixed-sum}
\end{equation}
With $M=N(N-1)/2$ and $p=N-q$, this can be rewritten as
\begin{equation}
  \Sigma(p,q;t)=M+q(q+t-N).
  \label{eq:mixed-sum-quadratic}
\end{equation}
For fixed $N$, the continuous minimum occurs at
$q=(N-t)/2$, subject to the endpoint constraints.  When $t$ is fixed and
$N\to\infty$, the optimal ratio satisfies
\begin{equation}
  \min_{p+q=N}\frac{\Sigma(p,q;t)}{M}
  =\frac12+O_t\!\left(\frac1N\right).
  \label{eq:one-half-barrier}
\end{equation}
The initial consecutive shifts in \eqref{eq:mixed-exponents} minimize the sum
among all $p$ nonnegative integer exponents and $q$ exponents in
$t+\mathbb Z_{\ge0}$.  Thus \eqref{eq:one-half-barrier} is intrinsic to the
entire two-coset alternant family, not an artifact of taking equal blocks.

This is the precise ``one-half barrier'' encountered by the matrix method.
The determinant vanishes to order $M\asymp N^2/2$ when the logarithmic nodes
coalesce, but clearing integrality forces a total monomial height
asymptotic to at least $M/2$.  Merely increasing the matrix dimension cannot
make the height-to-vanishing ratio tend to zero.

Allowing rational bases $2^a3^{-b}$ does not evade the barrier in a formal
way.  Multiplying rows to clear denominators translates all lattice exponent
vectors into the first quadrant, and this translation contributes exactly the
height that the apparent cancellation in $a\log2-b\log3$ had removed.  Any
successful signed-exponent construction would therefore need determinant-level
denominator cancellation beyond a common monomial row scaling.

\subsection{Alternants built from candidate exponents}
\label{subsec:candidate-repulsion}

There is a dual determinant hierarchy in which the nodes are candidate
exponents rather than semigroup bases.  Let
\[
  \tau_1<\cdots<\tau_N,
  \qquad \tau_i\in\mathcal C,
\]
and let $1\le s_1<\cdots<s_N$ be distinct elements of $\mathcal S$.  Then
\begin{equation}
  D(\boldsymbol\tau,\mathbf s)
  =\det(s_j^{\tau_i})_{i,j=1}^N
  \label{eq:candidate-alternant}
\end{equation}
is a positive integer.  Integrality follows from
$s_j^{\tau_i}\in\mathbb Z$, while positivity follows from strict total
positivity of $e^{uv}$ after putting $u=\tau_i$ and $v=\log s_j$.
...
\[
  ...
  M=N(N-1)/2.
\]
If all $\tau_i$ lie in $[T,T+\delta]$ with $0<\delta\le1$, then the same
divided-difference argument gives
\begin{equation}
  \delta\ge
  \left(
    \frac{\prod_{\ell=0}^{N-1}\ell!}
    {N^{N/2}e^{NL}\max(1,L)^M}
  \right)^{1/M}
  \exp\left(-\frac{T\Lambda}{M}\right).
  \label{eq:candidate-repulsion-bound}
\end{equation}
Thus candidates cannot form arbitrarily tight finite clusters without paying
an exponential height cost.

The best choice of bases for this bound is to take the smallest elements of
$\mathcal S$.  If $\lambda_1\le\cdots\le\lambda_N$ are the corresponding
logarithms, lattice-point integration in the triangle
$a\log2+b\log3\le L$ gives
\begin{align}
  \lambda_N
    &=\sqrt{2\log2\log3}\,N^{1/2}+O(1),
    \label{eq:lambdaN-asymptotic}\\
  \sum_{j=1}^N\lambda_j
    &=\frac{2}{3}\sqrt{2\log2\log3}\,N^{3/2}+O(N).
    \label{eq:lambda-sum-asymptotic}
\end{align}
Ignoring the lower-order explicit constant in
\eqref{eq:candidate-repulsion-bound}, the exponential rate is therefore
\begin{equation}
  \frac{\Lambda}{M}
  =\frac{4}{3}\sqrt{2\log2\log3}\,N^{-1/2}+O(N^{-1}).
  \label{eq:candidate-rate}
\end{equation}
Increasing the determinant size weakens the exponential rate, but a
hypothetical nonintegral candidate supplies only a rank-two additive semigroup
$\mathbb Z_{\ge0}+t\mathbb Z_{\ge0}$, whose local point count near height $T$
is only of order $T$.  This density is not enough to make
\eqref{eq:candidate-repulsion-bound} contradictory with presently available
Diophantine approximation.

\subsection{An exact Vandermonde identity from a rational approximation}
\label{subsec:approximation-vandermonde}

The candidate semigroup nevertheless gives a particularly clean exact
identity.  Let $t\in\mathcal C\setminus\mathbb Q$, choose integers $p,q\ge1$,
and put
\[
  \varepsilon=qt-p.
\]
For an integer $d\ge2$, the $d$ numbers
\begin{equation}
  \tau_i=(d-1-i)p+iqt=(d-1)p+i\varepsilon,
  \qquad i=0,\dots,d-1,
  \label{eq:clustered-candidates}
\end{equation}
all belong to $\mathcal C$.  For distinct
$s_1<\cdots<s_d$ in $\mathcal S$, the corresponding integer determinant
factors exactly:
\begin{equation}
  \det(s_j^{\tau_i})_{0\le i\le d-1,\,1\le j\le d}
  =\left(\prod_{j=1}^d s_j^{(d-1)p}\right)
    \prod_{1\le j<k\le d}(s_k^\varepsilon-s_j^\varepsilon).
  \label{eq:exact-approx-vandermonde}
\end{equation}
The left side is a nonzero integer.  Writing $\lambda_j=\log s_j$ and assuming
$|\varepsilon|\le1$, the mean-value theorem yields
\[
  |s_k^\varepsilon-s_j^\varepsilon|
  \le |\varepsilon|\,|\lambda_k-\lambda_j|e^{\lambda_d}.
\]
Consequently, with $M=d(d-1)/2$ and
$\Lambda=\sum_j\lambda_j$,
\begin{equation}
  |qt-p|
  \ge
  \left(\prod_{j<k}|\lambda_k-\lambda_j|\right)^{-1/M}
  \exp\left(-\frac{(d-1)p\Lambda}{M}-\lambda_d\right).
  \label{eq:candidate-irrationality-bound}
\end{equation}
This is an explicit irrationality bound that every nonintegral candidate would
have to satisfy.  It is exponential rather than polynomial in $q$ and hence is
far weaker than what would be required to contradict Dirichlet's theorem.
The case $d=2$ is already qualitatively present in the elementary nonzero
integer $2^{qt}-2^p$; higher dimensions organize the same phenomenon into a
total-positive determinant but do not change its exponential nature.

\subsection{Matrix functional calculus interpretation}
\label{subsec:frechet-matrix}

The Loewner matrix is not an artificial determinant device.  It is the matrix
of the first divided-difference operator in matrix functional calculus.  If
$X=\operatorname{diag}(x_1,\dots,x_r)$,
$Y=\operatorname{diag}(y_1,\dots,y_r)$, and $E=(e_{ij})$, then
\begin{equation}
 f\!\left(\begin{array}{cc}X&E\\0&Y\end{array}\right)
 =\begin{pmatrix}
    f(X)&\bigl(f[x_i,y_j]e_{ij}\bigr)_{i,j}\\
    0&f(Y)
  \end{pmatrix},
 \label{eq:block-functional-calculus}
\end{equation}
where $f[x,y]=(f(x)-f(y))/(x-y)$.  For $f(u)=u^t$ and semigroup spectra,
the candidate hypothesis makes the diagonal blocks integral and the
off-diagonal divided differences rational.  Multiplying $e_{ij}$ by
$x_i-y_j$ clears the first divided difference and yields an integral power.
Iterated upper-triangular block constructions similarly realize higher divided
differences.

This interpretation also explains the limitation.  Choosing off-diagonal
entries that clear the denominators builds integrality into the original
matrix lattice; choosing unit off-diagonal entries would impose new divisibility
conditions, but the hypotheses $2^t,3^t\in\mathbb Z$ do not imply that those
new matrices have integral $t$th powers.  The matrix method can package every
valid consequence elegantly, but it cannot assume preservation of a lattice
that is not functorially inherited from the original split torus.

\subsection{Why standard positivity-preserver results do not settle the problem}
\label{subsec:positivity-limitations}

Several mature branches of matrix theory lie nearby but stop short of the
needed arithmetic statement.

\begin{itemize}
\item Loewner's theorem characterizes operator-monotone functions through
      positivity of divided-difference matrices.  The fractional power
      $u^\alpha$ is operator monotone for $0\le\alpha\le1$, exactly the range
      used in \eqref{eq:loewner-integral}.  This is a real-order property and
      contains no rationality information.
\item Results on entrywise powers and critical exponents determine when
      $[(a_{ij})^\alpha]$ preserves positive semidefiniteness or total
      nonnegativity.  Again, they hold on continuous intervals of $\alpha$ and
      therefore cannot isolate integral exponents.
\item Perron--Frobenius theory controls the dominant eigenvalue of a
      nonnegative integral matrix, but the $2$--$3$ spectral lattice splits
      integrally because the eigenvalue difference is $1$.  There is no
      irreducible nonnegative coupling that forces an extra congruence between
      $2^t$ and $3^t$.
\item Exterior powers turn minors into eigenvalue products.  In the present
      setting those products are only $m^an^b$, already integral by assumption.
\end{itemize}

The missing ingredient is an arithmetic strengthening of positivity: one
needs an auxiliary determinant whose denominator-sensitive lower bound grows
more slowly than its analytic upper bound.  The certificates above provide the
right form of lower bound, but their current height exponents do not close the
gap.

\subsection{Promising matrix-driven directions}
\label{subsec:matrix-directions}

The preceding barriers suggest several focused next steps rather than a blind
search over larger matrices.

\paragraph{1. Denominator cancellation before absolute values.}
The one-half barrier comes from clearing each row by a common monomial.  A
useful advance would be a determinantal identity in which signed semigroup
exponents cancel denominators globally, before taking absolute values, leaving
an integer with substantially smaller height than any entrywise clearing.
This is naturally a problem about Smith normal forms of generalized Cauchy or
Loewner matrices over the localization
$\mathbb Z[1/6]$.

\paragraph{2. Confluent determinants without derivatives.}
Confluent Vandermonde determinants have much higher vanishing order, but true
derivatives introduce factors such as $t2^{t-1}$, which are not known to be
rational.  One should seek finite-difference substitutes supported entirely on
$\mathcal S$.  Block Loewner matrices associated with carefully chosen chains
of nearby $3$-smooth numbers may approximate confluence while retaining exact
integrality.

\paragraph{3. A product-formula matrix.}
The real determinant becomes small when nodes coalesce, while its denominators
are supported at finitely many rational primes.  A simultaneous real and
$p$-adic analysis of the same determinant could, in principle, use the product
formula instead of clearing denominators crudely.  The obstacle is that the
relation defining $t$ is archimedean; nevertheless, the matrices themselves
have rational entries, so their determinants have well-defined $p$-adic
valuations.  Computing those valuations exactly for structured Loewner nodes
is a concrete task.

\paragraph{4. Matrix Pad\'e approximation to $u^\alpha$.}
The Stieltjes representation \eqref{eq:loewner-integral} admits rational Pad\'e
approximants with positive poles and residues.  Evaluating such approximants on
semigroup nodes gives rational matrices with controlled sign.  A successful
construction would need an error determinant that vanishes to quadratic order
in the matrix size while its common denominator grows subquadratically.

\paragraph{5. Arithmetic one-parameter subgroups.}
The exact reformulation \eqref{eq:integral-times} suggests studying the set of
real times at which an analytic one-parameter subgroup of a split torus meets
an integral order.  Existing algebraic-subgroup theorems apply to algebraic
parameter subgroups or to integer times; here the parameter line has slope
$\log3/\log2$.  A theorem asserting cyclicity of this integral intersection
for a rank-two split torus would contain the present conjecture and would be a
natural matrix/arithmetic-group analogue of four exponentials.

\paragraph{6. Formal verification of determinant identities.}
The Sylvester--Loewner identity, the positivity integral, and the quantitative
bound are finite-dimensional and suitable for formalization.  A Lean
formalization would be valuable not because these steps are especially
mysterious, but because future auxiliary-determinant experiments will involve
large signs, denominator exponents, and optimization formulas where a silent
indexing error can invalidate an apparent gain.

\subsection{Literature update through August 2026}
\label{subsec:matrix-literature-update}

A fresh search of the mathematical literature and arXiv was carried out for
the exact Alaoglu--Erd\H{o}s statement, the Four Exponentials Conjecture,
Schanuel-type refinements, Loewner matrices, and generalized Vandermonde total
positivity.  The audit did not locate an unconditional resolution of the exact
$2^t,3^t$ problem.  The relevant literature still separates into the following
layers.

\begin{enumerate}
\item The original source is L. Alaoglu and P. Erd\H{o}s, ``On highly
      composite and similar numbers,'' \emph{Transactions of the American
      Mathematical Society} \textbf{56} (1944), 448--469.  The conjecture
      appears in the study of the exponent structure of highly composite
      numbers.
\item Gelfond--Schneider excludes every algebraic irrational candidate.  The
      remaining obstruction is a special $2\times2$ case of the Four
      Exponentials Conjecture; the proved Six Exponentials Theorem needs a third
      linearly independent logarithmic direction or a third independent
      exponent and therefore does not apply to the grid
      \eqref{eq:four-exp-grid}.
\item Modern work on strong forms of four exponentials and on Schanuel's
      conjecture sharpens the conceptual framework and supplies conditional
      consequences, but no currently verified theorem forces one of the four
      entries in \eqref{eq:four-exp-grid} to be transcendental.
\item On the matrix side, Loewner's theory of matrix-monotone functions,
      strict total positivity of Cauchy and exponential kernels, and the theory
      of generalized Vandermonde matrices justify the determinant and
      positivity tools used above.  Recent work on entrywise positivity
      preservers and critical exponents is structurally relevant but does not
      add the denominator-sensitive arithmetic input required here.
\end{enumerate}

The automatically generated metadata table immediately following this
paragraph records the most recent arXiv items returned by the targeted keyword
queries.  It is included to make the date and scope of the literature refresh
reproducible; inclusion in the table does not imply that every item proves a
new result directly about the Alaoglu--Erd\H{o}s conjecture.

\begin{quote}\small
No parseable arXiv records were returned by the archived keyword queries; the static literature discussion above remains the audit record.
\end{quote}

\subsection{Assessment of the matrix approach}
\label{subsec:matrix-assessment}

The matrix investigation changes the status of the project in a useful but
limited way.

\begin{itemize}
\item The conjecture is exactly an integral-real-power problem for a fixed
      $2\times2$ integral matrix, not merely analogous to one.
\item All polynomial representation-theoretic amplifications are now known to
      be information-neutral: they produce only monomials in the two assumed
      integers.
\item Cross-Loewner matrices produce a nonzero integer generalized
      Vandermonde determinant, and fractional powers produce an explicitly
      normalized positive integer Gram determinant.
\item These determinants yield rigorous $2r$-point gap bounds and finite
      candidate-repulsion inequalities.
\item Optimizing all two-coset alternants reveals an asymptotic one-half
      height barrier.  The natural density of $3$-smooth nodes and the
      Diophantine clusters supplied by an irrational candidate are much too
      coarse to violate the resulting bounds.
\end{itemize}

Accordingly, the most plausible route from matrix theory to a proof is not a
larger tensor power or another positivity theorem.  It is a new arithmetic
identity that lowers denominator height through global cancellation, or a
product-formula argument that can exploit the same rational determinant in
several absolute values at once.  Absent such an ingredient, the conjecture
remains pinned to the unresolved four-exponentials threshold.

\subsection{Concluding status}
\label{subsec:matrix-conclusion}

No unconditional proof of the Alaoglu--Erd\H{o}s conjecture is obtained here.
The report does, however, establish the following additional rigorous facts.

\begin{enumerate}
\item The conjecture is equivalent to the absence of nonintegral integral
      times for the matrix \eqref{eq:matrix-A}.
\item Any formal construction from one matrix and its real power, including
      polynomial representations, tensor powers, symmetric powers, and
      exterior powers, cannot add an independent arithmetic constraint.
\item For every hypothetical nonintegral candidate and every $2r$ distinct
      semigroup nodes, the cross-Loewner determinant clears to a nonzero
      integer generalized Vandermonde determinant.
\item After reduction to the fractional part, the corresponding Loewner
      determinant is strictly positive and has the explicit positive-integer
      normalization \eqref{eq:positive-integer-loewner}.
\item The explicit cluster bound \eqref{eq:2r-gap-bound}, the general
      candidate-repulsion bound \eqref{eq:candidate-repulsion-bound}, and the
      exact approximation determinant \eqref{eq:exact-approx-vandermonde}
      hold for every hypothetical counterexample.
\item The optimized mixed-alternant family has the asymptotic one-half barrier
      \eqref{eq:one-half-barrier}, explaining quantitatively why this entire
      first generation of matrix determinants falls short.
\end{enumerate}

These results narrow the search: any successful continuation must either beat
the one-half barrier by nonlocal denominator cancellation, import a genuinely
new transcendence theorem, or use arithmetic information not visible in the
split two-eigenvalue matrix algebra.
