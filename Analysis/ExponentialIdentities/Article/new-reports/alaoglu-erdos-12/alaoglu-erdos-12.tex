...
through $2^{29}$.  The fourth continuation reconstructs the original colossally abundant
prime-switch mechanism, proves a second-order logarithmic-mean resonance inequality and a
microscopic one-prime window, reduces the $2$--$3$ switch family to a continued-fraction sieve,
and gives an exact finite exclusion through switch level $10000$.
\end{minipage}
\vfill
...
A fourth continuation investigates prime-distribution routes.  It derives the exact
colossally abundant switch formula $p^\varepsilon=1+1/(p+\cdots+p^a)$, proves a global
second-order approximation for $1/\log(1+1/H)$, and shows that any level-$(a,b)$ switch
collision forces one prescribed prime into a unique interval of radius $O_b(q^{1-3b})$.
For the bases $2$ and $3$, a collision would force
$(2^{a+2}-3)/(3^{b+1}-2)$ to be a continued-fraction convergent of $\log3/\log2$ and would
satisfy a lacunary floor-power identity.  An exact integer verifier excludes all switch levels
$b\le10000$.  Modern Guth--Maynard short-interval estimates improve the historical Ingham
error term but operate at a scale fundamentally larger than the collision window.
\end{abstract}
...
\item \textbf{New: prime-switch resonance and an exact $2$--$3$ sieve.}  In the
historical colossally abundant family, a level-$a$ switch at a prime $p$ is exactly
$p^\varepsilon=1+1/(p+\cdots+p^a)$.  A simultaneous level-$(a,b)$ switch forces a
second-order logarithmic resonance and places the second prime in a unique interval of radius
$O_b(q^{1-3b})$, already $O(q^{-2})$ at $b=1$.  For $p=2,q=3$, the shifted quotient
$(2^{a+2}-3)/(3^{b+1}-2)$ must be a continued-fraction convergent of $\thetaq$, and
$\lfloor\thetaq(3^{b+1}-2)\rfloor+3$ must be a power of two.  Exact integer interval
arithmetic excludes every $b\le10000$.  This is a theorem about the stronger historical
rational-output switch family, not a proof of the integer-output conjecture.  \statusnew{}
\statuscomp
...
\begin{resultbox}
\textbf{Prime-switch scale separation (fourth continuation).}
The colossally abundant switch equations admit a new logarithmic-mean normal form.  For fixed
levels $a>b$, a collision prime must hit a unique window of radius $O_b(q^{1-3b})$, far below
the scale of any uniform or almost-all theorem on primes in short intervals.  In the special
$2$--$3$ family, the window collapses to a continued-fraction and shifted-power sieve, exactly
verified through level $10000$.  This advances the original application and identifies a
sharp scale barrier, while remaining logically separate from the integer-output conjecture.
\statusnew{} \statuscomp
\end{resultbox}
...
\clearpage
\section{Fourth continuation: prime switches, short intervals, and microscopic resonance}
\label{sec:prime-continuation}

\subsection{Outcome and claim ledger}
\label{ps:outcome}

This continuation attacks the problem from the prime-distribution side requested in the
brief.  It begins at the historical source of the conjecture---the discontinuities in the
prime exponents of colossally abundant numbers---and then asks whether the substantial recent
progress on primes in short intervals can resolve the relevant simultaneous-discontinuity
problem.  It also tests prime-divisor, primitive-divisor, smooth-number, and common-order
routes directly against the integer-output conjecture.

The answer is sharply divided.  Prime distribution materially improves the asymptotic
factorization estimates in the 1944 paper: the Ingham exponent $5/8$ can now be replaced, in
the all-interval prime-number-theorem input, by any fixed exponent greater than $17/30$ using
Guth--Maynard \cite{GuthMaynard2024}.  It does not resolve the exact prime-switch collision.
The latter is not a prime gap of positive length.  After an exact logarithmic-mean expansion,
a collision at levels $(a,b)$ forces one prescribed prime to lie in a window of length
$O_b(q^{1-3b})$, already $O(q^{-2})$ when $b=1$.  Thus the nearest integer is determined
before primality enters; contemporary short-interval theorems operate on a categorically
larger scale.

The continuation nevertheless produces several exact results.

\begin{center}
\begin{tabularx}{0.96\textwidth}{@{}p{0.23\textwidth}X p{0.18\textwidth}@{}}
\toprule
Result & Content & Status \\
\midrule
Prime-switch formula & A level-$a$ switch at a prime $p$ occurs at
$p^{\varepsilon}=1+1/(p+\cdots+p^a)$. & \statusknown{} / \statusnew{} derivation \\
Logarithmic-mean enclosure & For $H\ge1$, the reciprocal
$1/\log(1+1/H)$ lies in an explicit interval of width $1/(24H^2)$. & \statusnew \\
Microscopic resonance & A simultaneous switch forces a unique real resonance root to lie
within $O_b(q^{1-3b})$ of the integer prime $q$. & \statusnew \\
Continued-fraction sieve & A simultaneous $2$--$3$ switch forces
$(2^{a+2}-3)/(3^{b+1}-2)$, after reduction, to be a convergent of
$\log3/\log2$. & \statusnew \\
Finite switch exclusion & Exact integer arithmetic excludes a simultaneous $2$--$3$ switch
for every $1\le b\le10000$. & \statuscomp{} (exact rational enclosure) \\
Modernized 1944 estimate & The prime-block error in Alaoglu--Erd\H{o}s Theorem~4 improves
from $q^{-3/8}$ to $q^{-13/30+\delta}$. & \statusknown{} input / \statusnew{} transference \\
Fermat synchronization barrier & Primes forced to divide both $M^n-1$ and $A^n-1$ merely
from $\ell-1\mid n$ contribute only $\exp(n^{o(1)})$. & \statusnew \\
Full conjecture & No unconditional proof of $2^x,3^x\in\Z\Rightarrow x\in\Z$ is obtained. &
\statusknown{} open \\
\bottomrule
\end{tabularx}
\end{center}

The distinction between the last two columns is essential.  The prime-switch problem belongs
to the \emph{rational-output} form
\begin{equation}
 p^x,q^x\in\Q\quad\Longrightarrow\quad x\in\Z,
 \label{ps:eq-rational-form}
\end{equation}
which is stronger than the integer-output conjecture of this report.  As emphasized in
Section~\ref{sec:logproduct}, no reduction from arbitrary rational outputs to integer outputs
is known.  The historical application is therefore an important structured test case, not a
back door that silently proves Conjecture~\ref{conj:AE}.

\subsection{Recent placement}
\label{ps:recent-placement}

The current literature still places the basic $2^x,3^x$ problem at the four-exponentials
frontier.  Prasad's 2025 notes single out precisely the challenge of proving that simultaneous
integrality of $2^t$ and $3^t$ forces $t\in\Z$ \cite{Prasad2025}.  Dasgupta--Kakde's Matrix
Coefficient Conjecture gives the wider determinant framework developed in the third
continuation \cite{DasguptaKakdeMCC}.  On the analytic-prime side, Guth--Maynard's new large
value estimates yield prime-number-theorem asymptotics in intervals of length
$x^{17/30+o(1)}$ \cite{GuthMaynard2024}; Gafni--Tao recover the all-interval consequence and
obtain an almost-all result down to every exponent greater than $2/15$
\cite{GafniTao2026}.  The sections below identify exactly where those advances enter and why
they do not cross the transcendence wall.

\section{The colossally abundant prime-switch mechanism}
\label{sec:prime-switch}

\subsection{Local maximization and critical parameters}
\label{ps:local-max}

For $\varepsilon>0$, a colossally abundant number is a maximizer of
\[
 \frac{\sigma(n)}{n^{1+\varepsilon}}.
\]
Because the objective is multiplicative, the exponent of each prime is chosen independently.
For a prime $p$ and $k\ge0$, put
\[
 L_{p,k}(\varepsilon)
 :=\frac{1+p+\cdots+p^k}{p^{k(1+\varepsilon)}}.
\]
The ratio between consecutive local choices is
\begin{equation}
 \frac{L_{p,k}(\varepsilon)}{L_{p,k-1}(\varepsilon)}
 =\frac{p^{k+1}-1}{p^k-1}\,p^{-(1+\varepsilon)}
 \qquad(k\ge1).
 \label{ps:eq-local-ratio}
\end{equation}
Thus the exponent changes between $k-1$ and $k$ exactly when this ratio is $1$.

\begin{definition}[Prime-switch height]
\label{ps:def-H}
For a prime $p$ and an integer $a\ge1$, define
\[
 H_{p,a}:=p+p^2+\cdots+p^a=\frac{p^{a+1}-p}{p-1}
\]
and
\begin{equation}
 \varepsilon_{p,a}
 :=\frac{\log(1+H_{p,a}^{-1})}{\log p}.
 \label{ps:eq-switch-epsilon}
\end{equation}
\end{definition}

\begin{proposition}[Exact switch formula]
\label{ps:prop-switch-formula}
The local exponents $a-1$ and $a$ tie at the prime $p$ exactly at
$\varepsilon=\varepsilon_{p,a}$.  Equivalently,
\begin{equation}
 p^{\varepsilon_{p,a}}
 =\frac{p^{a+1}-1}{p^{a+1}-p}
 =\frac{H_{p,a}+1}{H_{p,a}}
 =1+\frac1{H_{p,a}}.
 \label{ps:eq-rational-switch-output}
\end{equation}
Away from the set of switch parameters, the local maximizing exponent is unique.
\end{proposition}

\begin{proof}
Set $k=a$ in \eqref{ps:eq-local-ratio}.  Equality of the ratio with $1$ gives
\[
 p^{1+\varepsilon}=\frac{p^{a+1}-1}{p^a-1},
\]
and division by $p$ gives
\[
 p^\varepsilon
 =\frac{p^{a+1}-1}{p(p^a-1)}
 =\frac{p^{a+1}-1}{p^{a+1}-p}.
\]
After dividing numerator and denominator by $p-1$, this is
$(H_{p,a}+1)/H_{p,a}$.  Strict monotonicity of the ratio in $\varepsilon$ gives uniqueness
away from equality.
\end{proof}

Formula~\eqref{ps:eq-rational-switch-output} is the cleanest form of the historical
connection.  It also shows
\[
 0<\varepsilon_{p,a}<\frac1{H_{p,a}\log p}<1.
\]
The reduced denominator of $p^{\varepsilon_{p,a}}$ has $p$-adic valuation exactly $1$, since
$v_p(H_{p,a})=1$ and $p\nmid H_{p,a}+1$.

\subsection{Simultaneous switches and the stronger rational conjecture}
\label{ps:simultaneous}

A simultaneous switch at two distinct primes is an equality
\begin{equation}
 \varepsilon_{p,a}=\varepsilon_{q,b}.
 \label{ps:eq-collision}
\end{equation}
At such a parameter $\varepsilon$, both $p^\varepsilon$ and $q^\varepsilon$ are rational by
\eqref{ps:eq-rational-switch-output}.  Since $0<\varepsilon<1$, the exponent is not an integer.
Consequently the rational-output assertion \eqref{ps:eq-rational-form} rules out every
simultaneous switch.  Conversely, ruling out switches only addresses the special rationals
$1+1/H_{p,a}$ and does not prove \eqref{ps:eq-rational-form} in general.

When switch parameters are all distinct, the one-parameter colossally abundant family changes
one prime exponent at a time, so the quotient between consecutive values is a prime.  A tie at
two primes permits a two-prime jump and is exactly the obstruction identified by Alaoglu and
Erd\H{o}s \cite{AE1944}.  The six exponentials theorem excludes a triple switch, explaining
their prime-or-semiprime conclusion; the two-prime collision remains at the four-exponentials
threshold.

\begin{proposition}[Monotonicity and the antichain condition]
\label{ps:prop-antichain}
For fixed $a$, the map $p\mapsto\varepsilon_{p,a}$ is strictly decreasing on $p>1$; for fixed
$p$, the map $a\mapsto\varepsilon_{p,a}$ is strictly decreasing.  If $p<q$ and
\eqref{ps:eq-collision} holds, then
\begin{equation}
 H_{p,a}>H_{q,b}
 \qquad\text{and}\qquad
 a>b.
...
\begin{proof}
The quantity $H_{p,a}$ increases strictly in either variable, while
$H\mapsto\log(1+1/H)$ decreases strictly.  Increasing $p$ also increases the denominator
$\log p$, proving the two monotonicity statements.

For a collision with $p<q$,
\[
 \frac{\log(1+1/H_{p,a})}{\log(1+1/H_{q,b})}
 =\frac{\log p}{\log q}<1.
\]
Hence the numerator on the left is smaller, so $H_{p,a}>H_{q,b}$.  If $a\le b$, then
\[
 H_{p,a}<H_{q,a}\le H_{q,b},
\]
a contradiction.  Thus $a>b$.
\end{proof}

This antichain geometry is the first point at which prime size and exponent level interact:
a larger prime can collide only at a strictly smaller local exponent.

\section{Logarithmic means and exact resonance inequalities}
\label{sec:logmean-resonance}

\subsection{The logarithmic mean behind a switch}
\label{ps:logmean}

Put
\begin{equation}
 R(H):=\frac1{\log(1+1/H)}
 =\frac{(H+1)-H}{\log(H+1)-\log H}.
 \label{ps:eq-logmean}
\end{equation}
Thus $R(H)$ is the logarithmic mean of the consecutive integers $H$ and $H+1$.  A collision
\eqref{ps:eq-collision} is equivalent to the exact resonance
\begin{equation}
 R(H_{p,a})\log p=R(H_{q,b})\log q.
 \label{ps:eq-R-resonance}
\end{equation}
The elementary bounds $H<R(H)<H+1$ already give a useful integer-coefficient near relation.

\begin{proposition}[First resonance strip]
\label{ps:prop-first-strip}
If \eqref{ps:eq-collision} holds, then
\begin{equation}
 -\log p
 <H_{p,a}\log p-H_{q,b}\log q
 <\log q.
 \label{ps:eq-first-strip}
\end{equation}
\end{proposition}

\begin{proof}
Write $R(H)=H+u_H$ with $0<u_H<1$.  Equation~\eqref{ps:eq-R-resonance} gives
\[
 H_{p,a}\log p-H_{q,b}\log q
 =u_{H_{q,b}}\log q-u_{H_{p,a}}\log p,
\]
which lies in the displayed interval.
\end{proof}

The width in \eqref{ps:eq-first-strip} is bounded, while the two coefficients are of size
$p^a$ and $q^b$.  The next theorem shrinks the width from a constant to the reciprocal square
of those coefficients.

\subsection{A two-term rational enclosure}
\label{ps:rational-enclosure}
...
\begin{theorem}[Sharp elementary enclosure for consecutive logarithmic means]
\label{ps:thm-logmean-bounds}
For every real $H\ge1$,
\begin{equation}
 J(H)<R(H)<J(H)+\frac1{24H^2}.
 \label{ps:eq-logmean-bounds}
\end{equation}
Equivalently,
\begin{equation}
 \frac1{12H}-\frac1{24H^2}
 <H+\frac12-R(H)
 <\frac1{12H}.
 \label{ps:eq-delta-bounds}
\end{equation}
\end{theorem}

\begin{proof}
Put $x=1/H$, so $0<x\le1$.  The lower inequality $J(H)<R(H)$ is equivalent to
\begin{equation}
 \log(1+x)<\frac{12x}{12+6x-x^2}.
 \label{ps:eq-log-upper-rational}
\end{equation}
The difference between the right side and $\log(1+x)$ vanishes at $x=0$, and its derivative is
\[
 \frac{x^3(24-x)}{(1+x)(12+6x-x^2)^2}>0
 \qquad(0<x\le1).
\]
This proves \eqref{ps:eq-log-upper-rational}.

The upper inequality in \eqref{ps:eq-logmean-bounds} is equivalent to
\begin{equation}
 \log(1+x)>\frac{24x}{24+12x-2x^2+x^3}.
 \label{ps:eq-log-lower-rational}
\end{equation}
Again the difference vanishes at $0$, and now
\[
 \frac{d}{dx}\left(
 \log(1+x)-\frac{24x}{24+12x-2x^2+x^3}
 \right)
 =\frac{x^4(x^2-4x+76)}{(1+x)(24+12x-2x^2+x^3)^2}>0.
\]
Thus \eqref{ps:eq-log-lower-rational} holds.  Taking reciprocals yields
\eqref{ps:eq-logmean-bounds}, and subtracting from $H+1/2$ gives
\eqref{ps:eq-delta-bounds}.
\end{proof}

The coefficients $1/2,-1/12,1/24$ are the first terms in the asymptotic expansion of the
logarithmic mean.  What matters here is not the formal expansion but the globally valid,
one-sided remainder in \eqref{ps:eq-logmean-bounds}.

\begin{theorem}[Second-order switch resonance]
\label{ps:thm-second-resonance}
Let $H=H_{p,a}$ and $K=H_{q,b}$.  If
$\varepsilon_{p,a}=\varepsilon_{q,b}$, then
\begin{equation}
 \left|J(H)\log p-J(K)\log q\right|
 <\frac{\log p}{24H^2}+\frac{\log q}{24K^2}.
 \label{ps:eq-second-resonance}
\end{equation}
In the coarser half-shift form,
\begin{equation}
 \left|\left(H+\frac12\right)\log p
       -\left(K+\frac12\right)\log q\right|
 <\frac{\log p}{12H}+\frac{\log q}{12K}.
 \label{ps:eq-halfshift-resonance}
\end{equation}
Equivalently,
\begin{equation}
 \left|(2H+1)\log p-(2K+1)\log q\right|
 <\frac{\log p}{6H}+\frac{\log q}{6K}.
 \label{ps:eq-integer-resonance}
\end{equation}
\end{theorem}

\begin{proof}
By Theorem~\ref{ps:thm-logmean-bounds}, write
\[
 R(H)=J(H)+e_H,
 \qquad 0<e_H<\frac1{24H^2},
\]
and similarly for $K$.  Insert these expressions into
\eqref{ps:eq-R-resonance}; moving the two error terms to the right gives
\eqref{ps:eq-second-resonance}.

For the half-shift version, write
$R(H)=H+\frac12-\delta_H$ and use
$0<\delta_H<1/(12H)$ from \eqref{ps:eq-delta-bounds}.  Multiplication by $2$ gives
\eqref{ps:eq-integer-resonance}.
\end{proof}

This theorem turns a transcendental equality into a highly constrained linear form in two
ordinary logarithms with explicitly structured integer coefficients.  General irrationality
measures for $\log p/\log q$ cannot by themselves exploit the structure of those coefficients:
\eqref{ps:eq-integer-resonance} corresponds to an approximation of Dirichlet order $2$, the
natural scale available for every irrational number.  The special geometric-sum form of the
coefficients is therefore indispensable.


\section{Microscopic prime windows}
\label{sec:microscopic-prime-window}

\subsection{The unique real resonance root}
\label{ps:real-root}

For an integer $b\ge1$ and a real variable $t>1$, put
\[
 H_b(t):=t+t^2+\cdots+t^b
\]
and
\begin{equation}
 \Phi_b(t):=J(H_b(t))\log t.
 \label{ps:eq-Phi}
\end{equation}
Since
\[
...
\begin{equation}
 \Phi_b'(t)
 =J'(H_b(t))H_b'(t)\log t+\frac{J(H_b(t))}{t}
 >b t^{b-1}\log t.
 \label{ps:eq-Phi-derivative}
\end{equation}
Thus $\Phi_b$ is strictly increasing from $0$ to $+\infty$ on $(1,\infty)$.

\begin{definition}[Real switch-resonance root]
\label{ps:def-real-root}
For fixed $p,a,b$, let $Q_{p,a,b}>1$ be the unique real number satisfying
\begin{equation}
 \Phi_b(Q_{p,a,b})=J(H_{p,a})\log p.
 \label{ps:eq-root-def}
\end{equation}
\end{definition}

This is the real point at which the two rational centers in
Theorem~\ref{ps:thm-second-resonance} agree.  A simultaneous switch does not merely ask for a
prime near this point; it asks for the prime $q$ itself to lie extraordinarily close to it.

\begin{theorem}[Microscopic prime-window theorem]
\label{ps:thm-microscopic-window}
Assume $p<q$ are primes, $a,b\ge1$, and
$\varepsilon_{p,a}=\varepsilon_{q,b}$.  Put $H=H_{p,a}$, $K=H_{q,b}$ and
$Q=Q_{p,a,b}$.  Then $a>b$, $H>K$, and
\begin{equation}
 |q-Q|
 <\frac{\displaystyle \frac{\log p}{24H^2}+\frac{\log q}{24K^2}}
 {\displaystyle b(q-\tfrac12)^{b-1}\log(q-\tfrac12)}.
 \label{ps:eq-window-exact}
\end{equation}
In particular, for fixed $a,b$,
\begin{equation}
 |q-Q_{p,a,b}|=O_b(q^{1-3b}).
 \label{ps:eq-window-asymptotic}
\end{equation}
The interval in \eqref{ps:eq-window-exact} has length less than $1$, so for each fixed triple
$(p,a,b)$ there is at most one integer---and hence at most one prime---that can survive this
necessary condition.
\end{theorem}

\begin{proof}
The antichain conclusions are Proposition~\ref{ps:prop-antichain}.  By
Theorem~\ref{ps:thm-second-resonance} and the definition of $Q$,
\[
 |\Phi_b(q)-\Phi_b(Q)|
 <E:=\frac{\log p}{24H^2}+\frac{\log q}{24K^2}.
\]
Because $p<q$ and $H>K$, both summands are bounded by
$\log q/(24K^2)$, so
\begin{equation}
 E<\frac{\log q}{12K^2}\le\frac{\log q}{12q^{2b}}.
 \label{ps:eq-E-bound}
\end{equation}
On the other hand, by \eqref{ps:eq-Phi-derivative}, moving a half unit from $q$ changes
$\Phi_b$ by more than
\[
 \frac12 b(q-\tfrac12)^{b-1}\log(q-\tfrac12).
\]
For $q\ge3$ this is larger than the right side of \eqref{ps:eq-E-bound}.  Hence
$Q\in(q-1/2,q+1/2)$.  The mean value theorem and
\eqref{ps:eq-Phi-derivative} now give \eqref{ps:eq-window-exact}.

Finally, $K\ge q^b$, while the denominator in \eqref{ps:eq-window-exact} is
$\asymp_b q^{b-1}\log q$; the second-order numerator is
$O(q^{-2b}\log q)$.  This proves \eqref{ps:eq-window-asymptotic}.
\end{proof}

For $b=1$, the allowed displacement is already $O(q^{-2})$; for $b=2$ it is $O(q^{-5})$.
This is the precise scale mismatch with primes-in-short-interval theory.  A theorem asserting a
prime in every interval of length $q^\eta$ with $\eta>0$ cannot decide whether the single
nearest integer to $Q_{p,a,b}$ is prime, much less whether it satisfies the original exact
switch equation.

\subsection{Location of the real root}
\label{ps:root-asymptotic}

The root has a simple first-order description.  It lies near a scaled rational power of $p$,
not near an arbitrary point.

\begin{proposition}[Asymptotic location]
\label{ps:prop-root-asymptotic}
Fix integers $a>b\ge1$.  As $p\to\infty$ through primes,
\begin{equation}
 Q_{p,a,b}
 =\left(\frac ba\right)^{1/b}p^{a/b}
 \left(1+O_{a,b}\!\left(\frac1{\log p}\right)\right).
 \label{ps:eq-root-asymptotic}
\end{equation}
\end{proposition}
...
\[
 H_r(t)=t^r(1+O_r(t^{-1})),
 \qquad
 J(H_r(t))=H_r(t)(1+O_r(t^{-r})).
\]
Hence the defining equation is
\[
 Q^b\log Q\,(1+o(1))=p^a\log p\,(1+o(1)).
\]
Write $Q=Cp^{a/b}(1+u)$ with $u=o(1)$.  The leading coefficient is determined by
\[
 C^b\frac ab=1,
\]
so $C=(b/a)^{1/b}$.  The constant term $\log C$ in $\log Q$ changes the product by relative
size $O_{a,b}(1/\log p)$, and monotonicity of $Q^b\log Q$ then gives the stated error.
\end{proof}

Thus a collision would place a prime at the nearest integer to a very rigid smooth curve
\[
 p\longmapsto \left(\frac ba\right)^{1/b}p^{a/b}
\]
with corrections that must ultimately be computed to accuracy $O(q^{1-3b})$.  This resembles
prime values of Piatetski--Shapiro or Beatty-type sequences only at a superficial level.  The
usual questions permit intervals or floors of macroscopic size; the switch problem prescribes
one integer to sub-unit accuracy and then imposes a second transcendental equality.

\section{The special \texorpdfstring{$2$--$3$}{2--3} switch and a continued-fraction sieve}
\label{sec:two-three-switch}

\subsection{A Dirichlet-scale approximation with exponential coefficients}
\label{ps:two-three-approx}

Set
\[
 \thetaq=\frac{\log3}{\log2}.
\]
For a possible $2$--$3$ collision, write
\[
 H:=H_{2,a}=2^{a+1}-2,
 \qquad
 K:=H_{3,b}=\frac{3^{b+1}-3}{2},
\]
and define the odd integers
\begin{equation}
 P:=2H+1=2^{a+2}-3,
 \qquad
 Q:=2K+1=3^{b+1}-2.
 \label{ps:eq-PQ}
\end{equation}
The next theorem turns the switch equality into a classical continued-fraction condition.

\begin{theorem}[Legendre-scale switch sieve]
\label{ps:thm-legendre-sieve}
If $\varepsilon_{2,a}=\varepsilon_{3,b}$, then $a>b$ and
\begin{equation}
 0<\thetaq-\frac PQ<\frac1{2Q^2}.
 \label{ps:eq-legendre-window}
\end{equation}
Consequently, after reducing $P/Q$ to lowest terms, the resulting rational number is a
continued-fraction convergent of $\thetaq$.
\end{theorem}

\begin{proof}
The inequality $a>b$ is Proposition~\ref{ps:prop-antichain}.  Put
\[
 \delta(T):=T+\frac12-R(T).
\]
By \eqref{ps:eq-delta-bounds},
\begin{equation}
 \frac1{12T}-\frac1{24T^2}<\delta(T)<\frac1{12T}.
 \label{ps:eq-delta-again}
\end{equation}
The resonance $R(H)\log2=R(K)\log3$ gives
\begin{equation}
 \frac PQ-\thetaq
 =\frac{\delta(H)-\thetaq\delta(K)}{K+1/2}.
 \label{ps:eq-PQ-error}
\end{equation}

We use the elementary rational bounds
\begin{equation}
 \frac32<\thetaq<\frac85,
 \label{ps:eq-theta-rational-bounds}
\end{equation}
which follow from $2^3<3^2$ and $3^5<2^8$.  Since $K\ge3$,
\[
 \thetaq\delta(K)
 >\frac{\thetaq}{12K}\left(1-\frac1{2K}\right)
 >\frac1{12K}
 >\delta(H),
\]
where the last inequality uses $H>K$.  Thus $P/Q<\thetaq$.

For the upper bound, note first that $H<R(H)=\thetaq R(K)<\thetaq(K+1)$, hence
$H<(8/5)(K+1)$.  The function
\[
 g(t):=\frac1{12t}-\frac1{24t^2}
\]
is decreasing for $t\ge1$.  Therefore \eqref{ps:eq-PQ-error} and
\eqref{ps:eq-delta-again} give
\[
 \thetaq-\frac PQ
 <\frac1{K+1/2}
 \left[
 ...
 \right].
\]
A direct common-denominator calculation shows that, for $K\ge3$,
\begin{align*}
 &\frac{2}{15K}
  -\frac5{96(K+1)}
  +\frac{25}{1536(K+1)^2}
  -\frac1{8(K+1/2)}\\
 &\hspace{2cm}
 =-\frac{672K^3-330K^2-1901K-1024}
 {7680K(K+1)^2(2K+1)}<0.
\end{align*}
The cubic numerator is positive at $K=3$ and strictly increasing thereafter.  Since
$Q=2K+1=2(K+1/2)$, this proves
\[
 \thetaq-\frac PQ<\frac1{8(K+1/2)^2}=\frac1{2Q^2}.
\]
Legendre's theorem now says that the reduced fraction is a convergent of $\thetaq$.
\end{proof}

The theorem is stronger than a generic linear-form estimate in one very specific way: the
numerator and denominator are shifted pure powers.  It yields two exact corollaries.

\begin{corollary}[Floor-power sieve]
\label{ps:cor-floor-power}
A simultaneous $2$--$3$ switch forces
\begin{equation}
 \left\lfloor\thetaq(3^{b+1}-2)\right\rfloor+3=2^{a+2}.
 \label{ps:eq-floor-power}
\end{equation}
\end{corollary}

\begin{proof}
Equation~\eqref{ps:eq-legendre-window} gives
$0<\thetaq Q-P<1/(2Q)<1$.  Hence $P=\lfloor\thetaq Q\rfloor$, and
\eqref{ps:eq-PQ} gives the result.
\end{proof}

\begin{corollary}[Convergent exponential sieve]
\label{ps:cor-convergent-sieve}
Let $P/Q=r/s$ in lowest terms and put $d=\gcd(P,Q)$.  A simultaneous switch forces $r/s$ to be
a convergent of $\thetaq$ and
\begin{equation}
 dr+3=2^{a+2},
 \qquad
 ds+2=3^{b+1}.
 \label{ps:eq-convergent-exponential}
\end{equation}
Equivalently,
\begin{equation}
 s2^{a+2}-r3^{b+1}=3s-2r.
 \label{ps:eq-fixed-convergent-Sunit}
\end{equation}
\end{corollary}

For each fixed convergent, \eqref{ps:eq-fixed-convergent-Sunit} is an effective two-base
exponential Diophantine equation.  The unresolved point is uniformity as the convergent varies.
This is a considerably narrower target than the original four-exponentials statement and is a
natural place for modular sieves, $S$-unit estimates, and exact computation.

\subsection{Exact finite exclusion through switch level \texorpdfstring{$10^4$}{10000}}
\label{ps:finite-switch}

The floor-power sieve admits a proof-producing finite scan without floating-point arithmetic.
For $z>1$, use
\begin{equation}
 \log z
 =2\sum_{k=0}^{N}\frac{y^{2k+1}}{2k+1}+T_N,
 \qquad
 y=\frac{z-1}{z+1},
 \label{ps:eq-atanh-series}
\end{equation}
where
\begin{equation}
 0<T_N<\frac{2y^{2N+3}}{(2N+3)(1-y^2)}.
 \label{ps:eq-atanh-tail}
\end{equation}
For $z=2,3$, the parameters are $y=1/3,1/2$.  Summing each positive term downward at a fixed
binary scale and adding one unit for every term plus the exact tail ceiling gives rational
lower and upper bounds for both logarithms.  Division of the positive intervals yields a
rational enclosure for $\thetaq$.

\begin{theorem}[Exact computational exclusion of low switch levels]
\label{ps:thm-switch-10000}
At the exact-integer-computation trust level, there is no simultaneous $2$--$3$ colossally
abundant switch with
\begin{equation}
 1\le b\le10000.
 \label{ps:eq-b-range}
\end{equation}
Thus any such switch would have $b>10000$, $a>b$, and
\begin{equation}
 0<\varepsilon<\frac1{3^{10001}\log3}.
 \label{ps:eq-epsilon-tiny}
\end{equation}
\end{theorem}

\begin{proof}
The verifier uses a scale of $2^{40000}$.  It retains $12614$ terms for $\log2$ and $19993$
terms for $\log3$, with the remainder bounded by \eqref{ps:eq-atanh-tail}.  For each
$1\le b\le10000$, it proves that the lower and upper rational bounds for
$\thetaq(3^{b+1}-2)$ have the same integer part.  Corollary~\ref{ps:cor-floor-power} then
requires that this floor plus $3$ be a power of two.

There is exactly one floor-power hit:
\[
 (a,b,P,Q)=(5,3,125,79).
\]
It fails the stronger Legendre window.  Indeed, $3^{12}>2^{19}$ gives
$\thetaq>19/12$, while
\[
 \frac{19}{12}-\frac{125}{79}=\frac1{948}>
 \frac1{2\cdot79^2}.
\]
Thus $\thetaq-125/79>1/(2\cdot79^2)$, contradicting the necessary bound in
Theorem~\ref{ps:thm-legendre-sieve}.  The diagnostic value
$2\cdot79^2(\thetaq-125/79)=33.501934\ldots$ is reported by the verifier but is not used
in the exact rejection.  All other levels fail the power shape already.  Hence no level in
\eqref{ps:eq-b-range} survives the necessary conditions of
Theorem~\ref{ps:thm-legendre-sieve}.

The literal verifier has SHA-256 digest
\begin{center}
\sha{a7aef69a62ce5b07bd8f909966244b3a5e92e23ac9bb2e1b951433d1f6902ce8},
\end{center}
and the JSON output has digest
\begin{center}
\sha{2e769c599f338c5eb5b4cbeca201928b38735de1eed35be32747d60eb5907a8c}.
\end{center}
The run checked $10000$ unique floors, found one power-shape hit, and found zero Legendre-window
survivors.  No binary or decimal floating-point operation occurs in the certification path;
the trusted software base is the Python interpreter and arbitrary-precision integer
arithmetic.

If $b>10000$, then $H_{3,b}\ge3^b$ and
$\log(1+1/H)<1/H$, giving \eqref{ps:eq-epsilon-tiny}.
\end{proof}

This computation is not evidence for the integer-output conjecture at large $x$; it is a
separate exact exclusion in the historical rational-output switch family.  Its conceptual
value is the sieve: the transcendental equality first collapses to a convergent condition and
then to a lacunary floor-power event.


\section{What modern prime distribution changes}
\label{sec:modern-prime-distribution}

\subsection{Ingham's role in the 1944 paper}
\label{ps:ingham-role}

Alaoglu and Erd\H{o}s begin their analytic argument with Ingham's theorem that, for each fixed
$c>0$ and every $\eta\ge5/8$,
\begin{equation}
 \pi(t+ct^\eta)-\pi(t)
 \sim\frac{ct^\eta}{\log t}.
 \label{ps:eq-ingham-pnt}
\end{equation}
They use it to construct blocks of consecutive primes near two scales and compare the effect of
replacing one block by another in a superabundant or highly composite integer
\cite{AE1944,Ingham1937}.  It is the principal analytic input for their estimates of the
prime exponents.  It is \emph{not} used to decide whether two exact switch parameters
\eqref{ps:eq-switch-epsilon} coincide.

That separation is visible already in the dimensions.  The prime blocks have length
$t^\eta\to\infty$; the exact switch window of Theorem~\ref{ps:thm-microscopic-window} has
length tending to $0$.

\subsection{A parameterized transference lemma}
\label{ps:transference}

The original proof is modular in its short-interval exponent.  The following statement records
the resulting update.

\begin{proposition}[Short-interval transference for the Alaoglu--Erd\H{o}s exponent estimate]
\label{ps:prop-transference}
Assume that for a fixed $\eta<1$ and every fixed $c>0$ one has the uniform asymptotic
\begin{equation}
 \pi(t+ct^\eta)-\pi(t)
 \sim\frac{ct^\eta}{\log t}.
 \label{ps:eq-generic-short-pnt}
\end{equation}
Let $n$ be superabundant, let $p$ be its largest prime factor, and let $q$ be the greatest
prime occurring to exponent $k$.  In the range
\begin{equation}
 q^{1-\eta}>\log p,
 \label{ps:eq-transference-range}
\end{equation}
Alaoglu--Erd\H{o}s's block-replacement argument gives
\begin{equation}
 q^k
 =\frac{p\log p}{\log q}
 \left(
  1+O\!\left(\frac{\log p}{q^{1-\eta}\log q}\right)
 \right).
 \label{ps:eq-modern-AE4}
\end{equation}
The same substitution of $\eta$ for the historical parameter propagates through the analogous
prime-block estimates for highly composite numbers.
\end{proposition}

\begin{proof}
In Lemmas~3--4 and Theorem~4 of the 1944 paper, the exponent denoted there by $\theta$ enters
only through two consequences of \eqref{ps:eq-generic-short-pnt}: the number of primes in a
block of length $ct^\eta$ is asymptotic to $ct^\eta/\log t$, and the logarithms of all primes in
that block equal $\log t+O(t^{\eta-1})$.  The replacement ratios therefore have the same main
terms as in the original proof, while the relative error contributed by a block near $q$ is
$O(q^{\eta-1})$ per logarithmic comparison.  The number of terms needed to match the block
near $p$ introduces the factor $\log p/\log q$.  Under
\eqref{ps:eq-transference-range}, the resulting error tends to zero and is exactly the one in
\eqref{ps:eq-modern-AE4}.  No other use of $5/8$ occurs in that argument.
\end{proof}

Guth--Maynard prove the needed asymptotic for intervals of length
$x^{17/30+o(1)}$ \cite{GuthMaynard2024}.  Hence, for every fixed $\delta>0$, one may take
\[
 \eta=\frac{17}{30}+\delta
\]
in Proposition~\ref{ps:prop-transference}, obtaining
\begin{equation}
 q^k
 =\frac{p\log p}{\log q}
 \left(
  1+O_\delta\!\left(
   \frac{\log p}{q^{13/30-\delta}\log q}
  \right)
 \right),
 \qquad
 q^{13/30-\delta}>\log p.
 \label{ps:eq-GM-AE4}
\end{equation}
This replaces the historical decay exponent $3/8$ by any exponent below $13/30$.

Baker--Harman--Pintz prove the stronger exponent $21/40=0.525$ for the existence of at least
one prime in a short interval \cite{BakerHarmanPintz2001}.  That theorem improves arguments
that need only a successor or predecessor prime.  It does not replace
\eqref{ps:eq-generic-short-pnt} where the asymptotic number and product of primes in an entire
block are used.

\subsection{All intervals versus almost all intervals}
\label{ps:all-vs-almost}

Gafni--Tao give a streamlined deduction of the all-interval result for every
$\eta>17/30$ and prove an almost-all asymptotic for every $\eta>2/15$
\cite{GafniTao2026}.  The almost-all theorem is a major advance in prime distribution, but it
cannot be inserted into a deterministic switch argument without new information on the
resonance roots.  A sparse structured sequence may lie entirely in the exceptional set of a
measure-theoretic theorem.  Here the points
\[
 Q_{p,a,b}
\]
are defined by a logarithmic equation and are exactly the points one would need to show avoid
the exceptional set.  That avoidance is at least as arithmetic as the original collision.

\subsection{The scale table}
\label{ps:scale-table}

The relevant scales can be compared directly.

\begin{center}
\begin{tabularx}{0.96\textwidth}{@{}p{0.28\textwidth}p{0.25\textwidth}X@{}}
\toprule
Input or target & Interval length near $q$ & What it controls \\
\midrule
Ingham in the 1944 paper & $q^{5/8}$ & Prime blocks in superabundant-factorization estimates \\
Baker--Harman--Pintz & $q^{21/40+o(1)}$ & Existence of a prime in every sufficiently large interval \\
Guth--Maynard / Gafni--Tao, all intervals & $q^{17/30+o(1)}$ & Prime-number-theorem asymptotic \\
Gafni--Tao, almost all intervals & $q^{2/15+\delta}$ & Prime-number-theorem asymptotic outside a density-zero exceptional set \\
Simultaneous switch, level $b$ & $O_b(q^{1-3b})$ & Necessary location of the one prescribed prime \\
\bottomrule
\end{tabularx}
\end{center}

For every $b\ge1$, the final exponent is at most $-2$.  Even conjectural bounds for ordinary
maximal prime gaps, such as a polylogarithmic Cram\'er scale, are enormously larger.  The issue
is not a lack of density estimates; it is that an interval of length below $1$ contains either
the nearest integer or nothing.  One must analyze the arithmetic of that integer, not the
average distribution of nearby primes.

\section{Other prime-distribution routes and their exact barriers}
\label{sec:other-prime-routes}

\subsection{Piatetski--Shapiro primes do not see exact candidate hits}
\label{ps:PS-barrier}

The candidate formulation asks for integers $m$ with
\[
 m^{\thetaq}\in\Z,
 \qquad \thetaq=1.5849625007\ldots.
\]
Piatetski--Shapiro theory studies primes among $\lfloor n^c\rfloor$.  The current v2 of
Guo--Guo--Lu proves an asymptotic formula for $1<c<1.1612$ \cite{GuoGuoLu2025}, while the
Rivat--Wu lower-bound theorem gives infinitude for $1<c\le243/205=1.1853\ldots$
\cite{RivatWu2001}; both ranges lie far below $c=\thetaq$.  More importantly, even a theorem at
$c=\thetaq$ would count prime values of a floor function.  It would not control the boundary
event
\[
 \{m^{\thetaq}\}=0.
\]
An average theorem about fractional parts in intervals cannot exclude a single exact zero.
The same zero-measure obstruction already appeared in the shrinking-target analysis of the
inherited report.

\subsection{Smooth numbers in short intervals}
\label{ps:smooth-barrier}

The primitive-generator theorem says that a counterexample would produce a fixed rank-two
multiplicative semigroup
\[
 \{2^nM^k:n,k\ge0\}
\]
of candidate inputs and a corresponding semigroup $\{3^nA^k\}$ of outputs.  This invites a
comparison with the distribution of smooth or friable integers.  Recent work proves strong
results for smooth numbers in short intervals, including all-interval results at approximately
square-root scale and almost-all results in much shorter ranges \cite{Jain2025}.

The comparison does not close.  Smooth-number theorems guarantee integers whose prime factors
are small relative to their size.  They do not force membership in one prescribed finitely
generated semigroup, and they do not connect the chosen smooth integer to its
$\thetaq$-th power.  Conversely, a hypothetical $M$ or $A$ may contain a very large prime
factor; the exact conditional theory imposes rank and primitivity, not a uniform smoothness
bound.

\subsection{Primitive prime divisors and synchronized gcds}
\label{ps:gcd-route}

Assume for this subsection that a nonintegral primitive solution $\beta$ exists, and write
\[
 M=2^\beta,\qquad A=3^\beta.
\]
The integers $M$ and $A$ are multiplicatively independent: a relation $M^r=A^s$ would give
$r\log2=s\log3$, impossible for nonzero integers $r,s$.  Zsigmondy's theorem therefore gives
new prime divisors in each of the individual sequences $M^n-1$ and $A^n-1$ outside the
classical exceptional cases \cite{Zsigmondy1892}.  The problem is synchronization.  A prime
new for one sequence need not divide the other.

The theorem of Bugeaud--Corvaja--Zannier gives the sharp qualitative upper barrier
\begin{equation}
 \gcd(M^n-1,A^n-1)<\exp(\epsilon n)
 \label{ps:eq-BCZ}
\end{equation}
for every $\epsilon>0$ and all sufficiently large $n$
\cite{BugeaudCorvajaZannier2003}.  Thus an exponential lower bound for the common gcd along an
infinite sequence would prove the conjecture.  Prime distribution alone supplies only a much
smaller universal lower bound.

\begin{proposition}[Fermat-synchronized common divisors]
\label{ps:prop-fermat-sync}
Let $U,V\ge2$ be fixed integers and define
\begin{equation}
 F_{U,V}(n)
...
\begin{equation}
 ...
 \label{ps:eq-Fn-divides}
\end{equation}
and
\begin{equation}
 \log F_{U,V}(n)
 \le\tau(n)\log(n+1)=n^{o(1)}.
 \label{ps:eq-Fn-upper}
\end{equation}
Moreover, if $n_y=\operatorname{lcm}(1,2,\ldots,\lfloor y\rfloor)$, then
\begin{equation}
 (1+o(1))y
 \le \log F_{U,V}(n_y)
 \le \exp\left(O\!\left(\frac{y}{\log y}\right)\right),
 \qquad
 \log n_y=(1+o(1))y.
 \label{ps:eq-Fn-lcm}
\end{equation}
In particular $\log F_{U,V}(n_y)=n_y^{o(1)}=o(n_y)$.  Thus this universal
construction remains subexponential in the exponent $n_y$ and cannot reach a lower bound
$\exp(cn_y)$.
\end{proposition}

\begin{proof}
If $\ell-1\mid n$ and $\ell\nmid UV$, Fermat's theorem gives
$U^n\equiv V^n\equiv1\pmod\ell$, proving \eqref{ps:eq-Fn-divides}.
Every such prime has the form $d+1$ with $d\mid n$, so there are at most $\tau(n)$ of them and
each is at most $n+1$.  This proves \eqref{ps:eq-Fn-upper}; the classical divisor bound gives
$\tau(n)=\exp(O(\log n/\log\log n))=n^{o(1)}$.

For $n_y$, every prime $\ell\le y+1$ outside the fixed support of $UV$ satisfies
$\ell-1\mid n_y$.  Therefore
\[
 \log F_{U,V}(n_y)\ge\sum_{\substack{\ell\le y+1\\ \ell\nmid UV}}\log\ell
 =(1+o(1))y
\]
by the prime number theorem.  For the upper bound, $\log \tau(n_y)=O(y/\log y)$, while
$\log(n_y+1)=(1+o(1))y$; inserting this in \eqref{ps:eq-Fn-upper} gives the stated
subexponential estimate.  Finally, the classical identity
$\log\operatorname{lcm}(1,\ldots,\lfloor y\rfloor)=\psi(y)=(1+o(1))y$ gives the second
formula.
\end{proof}

This calculation isolates a tempting but insufficient use of prime distribution.  Choosing an
exponent divisible by many $\ell-1$ does force many common prime divisors, but the exponent is
itself exponentially large in the prime cutoff.  The forced divisor has logarithm $n_y^{o(1)}=o(n_y)$, entirely consistent with
\eqref{ps:eq-BCZ}.  To contradict Bugeaud--Corvaja--Zannier,
one must exploit counterexample-specific orders of $M$ and $A$, not Fermat synchronization
available for every pair of integers.

\subsection{Congruence primes in interpolation determinants}
\label{ps:determinant-primes}

The second continuation already quantified the universal determinant gain from primes not
dividing the fixed bases.  Modulo such a prime $\ell$, rows indexed by exponent pairs depend
only on residues modulo $\ell-1$, giving an aggregate lower bound of order $N^{3/2}$ after
summing over $\ell\le\sqrt N$.  The determinant's leading archimedean and valuation budgets are
of order $N^2$.  Replacing the prime number theorem by a sharper error term or inserting the
Guth--Maynard short-interval exponent changes only lower-order terms; it cannot promote
$N^{3/2}$ to $N^2$.

A useful conclusion is therefore negative but exact: generic distribution of congruence
primes is not the missing source of leading divisibility.  A successful $p$-adic determinant
argument must use cancellation tied to the actual valuation vectors of $M$ and $A$, precisely
the residual problem isolated in the third continuation.

\subsection{Why prime factors do not manufacture a third exponential}
\label{ps:third-base-primes}

The rational-third-base theorem in the inherited corpus would finish the conjecture if one
could produce a rational value $r^\beta$ for a prime $r\notin\{2,3\}$.  Factoring $M$, $A$,
or values such as $M^n-1$ produces many such primes $r$, but divisibility of an integer by $r$
does not imply algebraicity, let alone rationality, of $r^\beta$.  This is the precise place
where a distribution theorem ceases to interact with the exponential parameter.  A norm,
resultant, or $p$-adic construction must first convert divisibility information into a third
algebraic exponential; no current prime theorem performs that conversion.


\section{Exact criteria extracted from the prime analysis}
\label{sec:prime-criteria}

\subsection{A prime-switch separation criterion}
\label{ps:switch-separation}

The second-order inequality can be isolated as an elementary target involving only primes,
geometric sums, and ordinary logarithms.

\begin{definition}[Switch separation]
\label{ps:def-switch-separation}
For distinct primes $p<q$, let $\mathrm{SS}(p,q)$ be the assertion that for every $a>b\ge1$,
with $H=H_{p,a}$ and $K=H_{q,b}$,
\begin{equation}
 \left|J(H)\log p-J(K)\log q\right|
 \ge\frac{\log p}{24H^2}+\frac{\log q}{24K^2}.
 \label{ps:eq-SS}
\end{equation}
\end{definition}

\begin{corollary}[Separation excludes simultaneous switches]
\label{ps:cor-SS}
If $\mathrm{SS}(p,q)$ holds, the primes $p$ and $q$ never switch simultaneously in the
colossally abundant family.  If $\mathrm{SS}(p,q)$ holds for every pair of distinct primes,
then every consecutive parameter step changes by a single prime.
\end{corollary}

\begin{proof}
Theorem~\ref{ps:thm-second-resonance} is the contrapositive of the first assertion.  The second
follows by applying the first to every pair.
\end{proof}

This criterion is much weaker than the rational-output four-exponentials statement, because it
only tests the special coefficients $H_{p,a}$.  It is also more concrete: a proof may use the
geometric-sum structure, modular restrictions on $H_{p,a}$, or the arithmetic of the unique
root $Q_{p,a,b}$.

\subsection{A restricted continued-fraction criterion for \texorpdfstring{$2,3$}{2,3}}
\label{ps:restricted-CF}

\begin{corollary}[A sufficient restricted-denominator statement]
\label{ps:cor-restricted-CF}
Suppose that no convergent $r/s$ of $\thetaq$ admits integers $d\ge1$, $a>b\ge1$ satisfying
\[
 dr+3=2^{a+2},
 \qquad
 ds+2=3^{b+1}.
\]
Then $2$ and $3$ never switch simultaneously in the colossally abundant family.
\end{corollary}

This is an infinite but discrete $S$-unit sieve.  For a fixed convergent it can be attacked by
congruences and linear forms in logarithms.  What is missing is a uniform argument over all
convergents of $\log3/\log2$.  The finite computation of
Theorem~\ref{ps:thm-switch-10000} is the first direct implementation of this sieve.

\subsection{A shrinking-target formulation}
\label{ps:lacunary-target}

Corollary~\ref{ps:cor-floor-power} rewrites the special collision as a lacunary shrinking
target.  Put
\[
 Q_b=3^{b+1}-2.
\]
A collision forces an integer $a>b$ for which
\begin{equation}
 0<\thetaq Q_b-(2^{a+2}-3)<\frac1{2Q_b}.
 \label{ps:eq-lacunary-shrinking}
\end{equation}
Thus the orbit $\thetaq(3^{b+1}-2)$ must approach the shifted powers of two at the exponentially
shrinking scale $3^{-b}$.  Ordinary equidistribution of $\{3^b\thetaq\}$, even if available for
this specific $\thetaq$, would not exclude a single hit at such a target.  A successful route
would need an effective lacunary avoidance theorem with arithmetic information about the
centers $2^a-3$.

\subsection{A common-gcd criterion for the integer-output conjecture}
\label{ps:gcd-criterion}

\begin{corollary}[Exponential common-gcd growth would settle the conjecture]
\label{ps:cor-gcd-criterion}
Suppose every hypothetical primitive pair $(M,A)=(2^\beta,3^\beta)$ with nonintegral $\beta$
would satisfy, for some $c>0$ and infinitely many $n$,
\begin{equation}
 \log\gcd(M^n-1,A^n-1)\ge cn.
 \label{ps:eq-gcd-criterion}
\end{equation}
Then Conjecture~\ref{conj:AE} holds.
\end{corollary}

\begin{proof}
The pair $M,A$ is multiplicatively independent, so
\eqref{ps:eq-gcd-criterion} contradicts the Bugeaud--Corvaja--Zannier estimate
\eqref{ps:eq-BCZ} with $\epsilon<c$.
\end{proof}

Proposition~\ref{ps:prop-fermat-sync} shows exactly why generic prime distribution is
insufficient for this criterion.  The universal order argument can force only a common divisor whose logarithm is
$n^{o(1)}$.  The desired linear growth would have to come from an unexpectedly large set of
primes at which the two unknown integers $M$ and $A$ have correlated small orders.

\section{A branch-sensitive prime-distribution research program}
\label{sec:prime-research-program}

The analysis above changes the priority order.  It retires broad requests for ``more primes in
short intervals'' as a direct solution, while leaving several sharply formulated arithmetic
tasks.

\subsection{Priority P1: the continued-fraction exponential sieve}
\label{ps:priority-CF}

Compute the convergents $r_n/s_n$ of $\thetaq$ and study simultaneous shifted-power equations
\[
 d r_n+3=2^u,
 \qquad
 d s_n+2=3^v.
\]
For fixed $n$, congruences modulo small primes can often eliminate $d$ or force it into a thin
progression, after which a two-logarithm bound is effective.  A uniform proof would likely need
one of:
\begin{enumerate}
\item a theorem excluding shifted powers in both coordinates of convergents to a logarithm
ratio;
\item a primitive-divisor argument for the recurrence of convergent matrices;
\item a modular covering whose moduli grow through the continued-fraction recurrence.
\end{enumerate}
The key gain over a direct four-exponentials attack is discreteness: no unknown real exponent
appears in the sieve equations.

\subsection{Priority P2: exact arithmetic of the real resonance root}
\label{ps:priority-root}

For general primes $p<q$, compute the nearest integer to $Q_{p,a,b}$ using certified rational
bounds and test its congruence classes before testing primality.  Theorem~\ref{ps:thm-microscopic-window}
shows there is never a need to search an interval.  A promising experiment is to identify
families for which the nearest integer has a forced small divisor.  Such a theorem would prove
switch separation for those families without any global prime-gap input.

\subsection{Priority P3: higher logarithmic-mean approximants}
\label{ps:priority-logmean}

Theorem~\ref{ps:thm-logmean-bounds} uses two terms of the logarithmic-mean expansion.  Rational
Pad\'e bounds for
\[
 \frac1{\log(1+1/H)}
\]
can produce centers with errors $O(H^{-m})$ for larger fixed $m$.  At order $m$, the prime
window contracts by another factor of approximately $q^{-b}$.  This does not solve the problem
by itself, but it may expose integral coefficient combinations whose residues are rigid enough
for a modular contradiction.

\subsection{Priority P4: correlated multiplicative orders}
\label{ps:priority-orders}

For a hypothetical primitive pair $M,A$, isolate primes $\ell$ for which
\[
 \operatorname{ord}_\ell(M)\mid n,
 \qquad
 \operatorname{ord}_\ell(A)\mid n.
\]
Fermat-synchronized primes give only the subexponential contribution of
Proposition~\ref{ps:prop-fermat-sync}.  Any route to
Corollary~\ref{ps:cor-gcd-criterion} must prove that the logarithmic relation between $M$ and
$A$ creates many additional common-order primes.  No theorem of this form is currently known;
it would be a genuinely new bridge between real logarithms and finite-field orders.

\subsection{Priority P5: use prime distribution only after an arithmetic reduction}
\label{ps:priority-after-reduction}

Prime-distribution theorems become relevant after one has reduced the problem to the primality
or factorization of a prescribed integer sequence.  The switch roots and the convergent sieve
provide exactly such sequences.  Before that reduction, a density theorem about anonymous
primes has no channel through which to constrain $2^x$ and $3^x$.  This sequencing principle
should govern future experiments: first derive a rigid integer; then invoke sieve or prime
technology.

\subsection{Priority P6: keep the integer-output core separate}
\label{ps:priority-core}

The original conjecture is not settled by proving all prime-switch collisions impossible.  A
prime-based proof of the integer-output statement still needs one of the bridges already
identified in the inherited report:
\begin{enumerate}
\item manufacture a third algebraic exponential and invoke six exponentials;
\item obtain leading $p$-adic cancellation in a determinant;
\item prove the required fixed-base Matrix Coefficient instance;
\item establish an independence statement for the relevant products of prime logarithms.
\end{enumerate}
The prime-switch work is valuable because it supplies a more structured laboratory for the
same rank-two obstruction, not because it erases this logical distinction.

\section{Lean formalization blueprint for the prime continuation}
\label{sec:prime-lean-blueprint}

The new paper proofs are elementary enough to formalize without importing analytic number
theory, except for the optional historical transference and the Bugeaud--Corvaja--Zannier
barrier.

\subsection{Proposed module graph}
\label{ps:module-graph}

A natural layout under
\lean{ExponentialIdentities/TwoBaseIntegerExponent/} is:
\begin{center}
\lean{ColossallyAbundantSwitch} $\longrightarrow$
\lean{ConsecutiveLogMeanBounds} $\longrightarrow$
\lean{SwitchResonance}\\
$\hspace{2cm}\searrow$ \lean{TwoThreeSwitchConvergent}
$\longrightarrow$ \lean{PrimeSwitchFiniteCertificate},\\
\lean{FermatSynchronizedGCD}.
\end{center}

\paragraph{\lean{ColossallyAbundantSwitch}.}
Define the local objective $L_{p,k}(\varepsilon)$, prove the consecutive-ratio formula, and
derive
\[
 p^{\varepsilon_{p,a}}=1+H_{p,a}^{-1}.
\]
The development can avoid a full definition of colossally abundant numbers at first; the local
maximization theorem is independent and is the only input needed for resonance.

\paragraph{\lean{ConsecutiveLogMeanBounds}.}
Define
\[
 R(H)=1/\log(1+1/H),\qquad J(H)=H+1/2-1/(12H)
\]
on positive reals.  Formalize the two rational derivative identities in the proof of
Theorem~\ref{ps:thm-logmean-bounds}.  All denominators are manifestly positive on
$0<x\le1$, so the proof is a sequence of field normalizations and positivity checks.

\paragraph{\lean{SwitchResonance}.}
Package Theorem~\ref{ps:thm-second-resonance}, the antichain theorem, and the monotonicity of
$\Phi_b$.  The microscopic-window theorem then uses only the mean value theorem and the
finite-sum derivative of $H_b(t)$.

\paragraph{\lean{TwoThreeSwitchConvergent}.}
Specialize the resonance to $2,3$, prove the rational bounds
$3/2<\thetaq<8/5$ by monotonicity of powers, normalize the polynomial inequality in
Theorem~\ref{ps:thm-legendre-sieve}, and invoke the existing continued-fraction API for
Legendre's theorem.

\paragraph{\lean{PrimeSwitchFiniteCertificate}.}
The exact scan can be converted from a Python computation into a compact certificate consisting
of:
\begin{enumerate}
\item dyadic integer endpoints for $\log2$ and $\log3$ proved by finite sums of
\eqref{ps:eq-atanh-series};
\item a list of floor values generated recursively from $Q_{b+1}=3Q_b+4$;
\item a Boolean proof that only $b=3$ gives a shifted power of two;
\item one exact rational inequality excluding $(a,b)=(5,3)$.
\end{enumerate}
A range of $10000$ should be split into chunks, exactly as in the existing finite-check
modules, to keep elaboration and proof-term size manageable.

\paragraph{\lean{FermatSynchronizedGCD}.}
Formalize the finite product in \eqref{ps:eq-Fn}, Fermat divisibility, and the elementary bound
by the divisor count.  The asymptotic statements can remain classical interfaces; the exact
finite divisibility theorem is already useful.

\subsection{Suggested theorem signatures}
\label{ps:lean-signatures}

The key interfaces can be stated without the full colossally abundant infrastructure:

\begin{lstlisting}[style=leanstyle,caption={Proposed logarithmic-mean enclosure.}]
theorem consecutive_logMean_bounds (H : R) (hH : 1 <= H) :
    H + 1 / 2 - 1 / (12 * H) < 1 / Real.log (1 + 1 / H) and
    1 / Real.log (1 + 1 / H) <
      H + 1 / 2 - 1 / (12 * H) + 1 / (24 * H ^ 2)
\end{lstlisting}

\begin{lstlisting}[style=leanstyle,caption={Proposed $2$--$3$ switch sieve.}]
theorem two_three_switch_forces_convergent
    (a b : N) (ha : 1 <= a) (hb : 1 <= b)
    (hswitch : switchEpsilon 2 a = switchEpsilon 3 b) :
    let P := 2 ^ (a + 2) - 3
    let Q := 3 ^ (b + 1) - 2
    0 < logThreeDivLogTwo - (P : R) / Q and
    logThreeDivLogTwo - (P : R) / Q < 1 / (2 * (Q : R) ^ 2)
\end{lstlisting}

\begin{lstlisting}[style=leanstyle,caption={Proposed Fermat synchronization theorem.}]
theorem prime_product_dvd_common_gcd
    (U V n : N) (hU : 2 <= U) (hV : 2 <= V) :
    Dvd.dvd (primeFermatProduct U V n) (Nat.gcd (U ^ n - 1) (V ^ n - 1))
\end{lstlisting}



\section{Exact verifier source and reproducibility}
\label{sec:prime-switch-source}

The finite theorem above is reproducible from the literal source below.  The command used was
\begin{center}
\sha{python3 prime_switch_exact_scan.py --B 10000 --bits 40000 > prime_switch_exact_scan_B10000.json}.
\end{center}
The two logarithm intervals are built from positive rational series terms and explicit rational
tails.  Every later operation is an integer comparison after cross multiplication.  The
elapsed-time field in the JSON is informational; the mathematical conclusion is determined by
the interval and survivor fields.

The decisive output fields are:
\begin{center}
\begin{tabular}{@{}lr@{}}
\toprule
Maximum switch level $B$ & $10000$\\
Binary scale & $2^{40000}$\\
Terms for $\log2$ & $12614$\\
Terms for $\log3$ & $19993$\\
Unique certified floors & $10000$\\
Shifted-power hits & $1$\\
Legendre-window survivors & $0$\\
\bottomrule
\end{tabular}
\end{center}

\begin{lstlisting}[style=pythonstyle,caption={Exact integer verifier for the $2$--$3$ prime-switch sieve.},label={ps:lst-exact-switch}]
#!/usr/bin/env python3
"""Exact fixed-point scan for the 2--3 colossally-abundant switch sieve.

For levels b <= B, a simultaneous switch would force
  P = 2^(a+2)-3 = floor(theta * Q), Q=3^(b+1)-2,
where theta=log(3)/log(2).  We enclose theta by rational intervals built
only from the positive atanh series for logarithms, then check the floor.
No floating-point arithmetic is used in the proof-producing scan.
"""
from __future__ import annotations
...
def log_atanh_interval(num: int, den: int, scale_bits: int) -> FixedInterval:
    """Enclose log((den+num)/(den-num)) = 2 atanh(num/den).

    Here 0 < num < den.  The positive series is
      2 sum_{k>=0} (num/den)^(2k+1)/(2k+1).
    Each retained fixed-point term is rounded down.  We stop once the
    geometric tail contributes less than one ulp; the upper endpoint adds
    one ulp for every retained rounding plus a rigorous tail ceiling.
    """
    S = 1 << scale_bits
    total_floor = 0
    k = 0
    # Maintain exact odd powers.
    num_pow = num
    den_pow = den
    while True:
        odd = 2 * k + 1
        term_num = 2 * S * num_pow
        term_den = odd * den_pow
        total_floor += term_num // term_den

        # Tail after this term: for j >= k+1, denominator 2j+1 >= 2k+3.
        # Hence tail < 2*y^(2k+3)/((2k+3)*(1-y^2)).
        next_odd = odd + 2
        next_num_pow = num_pow * num * num
        next_den_pow = den_pow * den * den
        tail_num = 2 * S * next_num_pow * den * den
        tail_den = next_odd * next_den_pow * (den * den - num * num)
        tail_ceil = ceil_div(tail_num, tail_den)

        k += 1
        if tail_ceil <= 1:
            # The strict tail bound is below or equal to one fixed-point ulp.
            return FixedInterval(
                lo=total_floor,
                hi=total_floor + k + tail_ceil,
                scale_bits=scale_bits,
                terms=k,
            )
        num_pow = next_num_pow
        den_pow = next_den_pow
...
def floor_ratio(num: int, den: int) -> int:
    return num // den


def scan(B: int, scale_bits: int) -> dict:
    start = time.time()
    # log 2 = 2 atanh(1/3), log 3 = 2 atanh(1/2).
    log2 = log_atanh_interval(1, 3, scale_bits)
    log3 = log_atanh_interval(1, 2, scale_bits)

    # theta lower = log3_lo/log2_hi; theta upper = log3_hi/log2_lo.
    theta_lo_num, theta_lo_den = log3.lo, log2.hi
    theta_hi_num, theta_hi_den = log3.hi, log2.lo

    Q = 3 * 3 - 2  # b=1 -> 3^(b+1)-2 = 7
    ambiguous = []
    power_shape_hits = []
    legendre_survivors = []
    closest_exp_gap = None
    closest_record = None

    checked = 0
    for b in range(1, B + 1):
        if b > 1:
            # If Q_b=3^(b+1)-2, then Q_{b+1}=3Q_b+4.
            Q = 3 * Q + 4

        lo_floor = floor_ratio(theta_lo_num * Q, theta_lo_den)
        hi_floor = floor_ratio(theta_hi_num * Q, theta_hi_den)
        if lo_floor != hi_floor:
            ambiguous.append((b, lo_floor, hi_floor))
            continue
        P = lo_floor
        checked += 1
        candidate = P + 3
        if is_power_of_two(candidate):
            a = candidate.bit_length() - 3
            # Collision also forces 0 < theta-P/Q < 1/(2Q^2).
            # Certify the direction and the Legendre-scale gap from the theta interval.
            power_shape_hits.append((b, a, P, Q))
            # theta_lo - P/Q >= 1/(2Q^2) iff
            # 2Q(theta_lo_num*Q - P*theta_lo_den) >= theta_lo_den.
            left_num = theta_lo_num * Q - P * theta_lo_den
            if left_num > 0 and 2 * Q * left_num < theta_lo_den:
                # The lower endpoint does not exclude the Legendre window; use upper
                # information only to record a survivor for further direct checking.
                legendre_survivors.append((b, a, P, Q))
            elif left_num <= 0:
                # The lower endpoint is not enough to prove P/Q < theta; if even the upper
                # endpoint is <= P/Q, the direction is wrong and the pair is excluded.
                upper_num = theta_hi_num * Q - P * theta_hi_den
                if upper_num > 0:
                    legendre_survivors.append((b, a, P, Q))

        # Reconnaissance statistic: exponent distance from the nearest power 2.
        lower_pow = 1 << (candidate.bit_length() - 1)
        upper_pow = lower_pow << 1
        gap = min(candidate - lower_pow, upper_pow - candidate)
        if closest_exp_gap is None or gap < closest_exp_gap:
            closest_exp_gap = gap
            closest_record = (b, candidate.bit_length() - 1, gap)

    elapsed = time.time() - start
    payload = {
        "B": B,
        "scale_bits": scale_bits,
        "log2_terms": log2.terms,
        "log3_terms": log3.terms,
        "checked_unique_floors": checked,
        "ambiguous_count": len(ambiguous),
        "ambiguous_first": ambiguous[:10],
        "power_shape_hit_count": len(power_shape_hits),
        "power_shape_hits_first": power_shape_hits[:10],
        "legendre_survivor_count": len(legendre_survivors),
        "legendre_survivors_first": legendre_survivors[:10],
        "closest_power_gap_record": closest_record,
        "elapsed_seconds": elapsed,
        "conclusion": (
            f"For every 1 <= b <= {B}, the exact floor/power and Legendre-window "
            "necessary conditions exclude a simultaneous 2--3 colossally-abundant "
            "switch.  Power-shape hits are listed separately and are all outside the "
            "required 1/(2Q^2) approximation window."
            if not ambiguous and not legendre_survivors
            else "The requested conclusion was not certified; inspect ambiguous/power hits."
        ),
    }
...
def main() -> None:
    ...
    ap.add_argument("--bits", type=int, default=40_000)
    args = ap.parse_args()
    result = scan(args.B, args.bits)
    print(json.dumps(result, indent=2, sort_keys=True))
...
\begin{statusbox}{\statuscomp{} --- exact rational interval computation implemented with
Python arbitrary-precision integers.  The source and output digests are stated in
Theorem~\ref{ps:thm-switch-10000}; the source is reproduced verbatim in
Listing~\ref{ps:lst-exact-switch}.}
\end{statusbox}


\section{Recent literature reconnaissance for the prime continuation}
\label{sec:prime-literature}

This section records the literature check performed for the fourth continuation, with a cutoff
of August~21, 2026.  The distinction between a theorem used in a proof, a theorem used only to
calibrate scale, and a recent preprint claim is kept explicit.

\subsection{The conjecture and the logarithm-matrix frontier}

The original source is Alaoglu--Erd\H{o}s~\cite{AE1944}.  Their colossally abundant exponent
formula is the source of the switch values \eqref{ps:eq-switch-epsilon}, and their paper explicitly
states that simultaneous rationality for two prime bases was not proved.  A recent problem list
of Prasad again singles out the concrete challenge that $2^t$ and $3^t$ integral should force
$t$ integral~\cite{Prasad2025}; it supplies no solution and confirms that the example remains a
live test problem.

Dasgupta--Kakde's Matrix Coefficient Conjecture is the most directly relevant recent structural
development~\cite{DasguptaKakdeMCC}.  The third continuation proved that the fixed first column
$(\log2,\log3)^T$ already contains the present problem.  The prime-switch family does not evade
that frontier: it replaces arbitrary rational outputs by the special rational numbers
$1+1/H_{p,a}$, but the resulting $2\times2$ logarithm determinant is still exactly zero.

\subsection{Uniform and almost-all primes in short intervals}

Ingham's 1937 theorem supplied the exponent $5/8$ used in the 1944 paper
\cite{Ingham1937}.  The peer-reviewed lower-bound benchmark of Baker--Harman--Pintz gives a
prime in intervals at exponent $21/40=0.525$~\cite{BakerHarmanPintz2001}.  A 2025 eighth-version
preprint of Runbo Li claims a further existence/lower-bound improvement to exponent
$0.52$~\cite{Li2025}; no step in this report depends on that claim.

For the full prime-number-theorem asymptotic needed by the Alaoglu--Erd\H{o}s prime-block
comparison, the decisive current input is Guth--Maynard.  Their revised April~2026 preprint
proves asymptotics in intervals of length $x^{17/30+o(1)}$ and the zero-density exponent
$30/13$~\cite{GuthMaynard2024}.  Gafni--Tao quantify the exceptional set and record the sharp
distinction between the all-$x$ threshold $17/30$ and the almost-all threshold
$2/15$~\cite{GafniTao2026}.  Turnage-Butterbaugh's July~2026 survey places this first
improvement over the relevant Ingham-era zero-density input in its historical context
\cite{TurnageButterbaugh2026}.

These advances genuinely improve the inherited colossally abundant error term, as
equation~\eqref{ps:eq-GM-AE4} records.  They do not approach the collision window of
Theorem~\ref{ps:thm-microscopic-window}, which has length $O(q^{1-3b})$ and is already
$O(q^{-2})$ for $b=1$.  Even the conjectural Cram\'er scale concerns intervals growing roughly
like a power of $\log q$, not a prescribed interval shrinking below one integer.

\subsection{Primes and smooth numbers on nonlinear sequences}

Guo--Guo--Lu prove a Piatetski--Shapiro prime number theorem for
$1<c<1.1612\ldots$~\cite{GuoGuoLu2025}, and Guo--Guo study such primes inside short intervals
in a 2026 preprint~\cite{GuoGuo2026}.  Both developments are far from the exponent
$\thetaq=1.5849\ldots$, and more importantly they concern primality of
$\lfloor n^c\rfloor$, not the exact event $\{n^c\}=0$.  They therefore provide useful scale
comparisons but no implication for the candidate set $\Cand$.

Jain's 2025 work improves the distribution of smooth numbers in short intervals
\cite{Jain2025}.  It likewise addresses anonymous smooth values in moving intervals.  A
counterexample would require the prescribed map $M^k\mapsto A^k$ and the fixed rank-two
semigroup $\{2^nM^k\}$, a much more rigid condition than smoothness.  This is why neither
friable-number density nor prime density can substitute for a transcendence or valuation
bridge.

\subsection{Prime divisors, orders, and gcd estimates}

Zsigmondy's theorem guarantees primitive prime divisors for most terms of $U^n-V^n$
\cite{Zsigmondy1892}.  It is effective at creating new prime factors but does not correlate the
separate sequences $M^n-1$ and $A^n-1$.  The theorem of
Bugeaud--Corvaja--Zannier gives the opposite global constraint: for multiplicatively
independent $M,A$, the common gcd is smaller than $\exp(\epsilon n)$ for every fixed
$\epsilon>0$ and all large $n$~\cite{BugeaudCorvajaZannier2003}.  The exact criterion
\eqref{ps:eq-gcd-criterion} therefore identifies what a prime-order approach would have to
produce and Proposition~\ref{ps:prop-fermat-sync} proves that generic Fermat synchronization
falls short.

\subsection{Literature verdict}

No located paper proves the integer-output Alaoglu--Erd\H{o}s conjecture, the stronger
rational-output conjecture, the switch-separation condition of
Definition~\ref{ps:def-switch-separation}, or the restricted convergent sieve of
Corollary~\ref{ps:cor-restricted-CF}.  The recent prime-distribution breakthroughs improve the
historical analytic application but do not address the fixed logarithmic determinant.  The
recent logarithm-matrix work explains the obstruction but remains conjectural in the
$2\times2$ case needed here.

\begin{statusbox}{\statusknown{} for cited external theorems; \statusnew{} for the deductions
made from them in this continuation.  The Runbo Li item is cited explicitly as an arXiv
preprint claim and is not used.}
\end{statusbox}


\section{Conclusions of the fourth continuation}
\label{sec:fourth-conclusion}

The requested unconditional proof was not obtained.  The conjecture remains open, and the
prime-distribution investigation clarifies why: distribution theorems control the abundance of
unspecified primes, whereas a counterexample is an exact rank-one logarithm relation between
two prescribed integer outputs.  There is no automatic channel from one kind of information to
the other.

The continuation nevertheless adds several exact results.

First, it reconstructs the historical colossally abundant mechanism in a form adapted to
modern Diophantine analysis.  A level-$a$ switch at $p$ is exactly
\[
 p^{\varepsilon_{p,a}}=1+\frac1{p+p^2+\cdots+p^a}.
\]
The new consecutive-logarithmic-mean estimate
\[
 H+\frac12-\frac1{12H}
 <\frac1{\log(1+1/H)}
 <H+\frac12-\frac1{12H}+\frac1{24H^2}
\]
turns a simultaneous switch into the second-order resonance inequality
\eqref{ps:eq-second-resonance}.

Second, that inequality produces a sharp scale diagnosis.  For fixed levels $a>b$, a collision
prime $q$ must lie in a unique interval around $Q_{p,a,b}$ of radius
$O_b(q^{1-3b})$.  This interval is shorter than one integer and shrinks like $q^{-2}$ already
at the first level.  Guth--Maynard and every other known short-interval prime theorem operate
at a positive power of $q$; their improvement is mathematically important but orthogonal to
this exact hit problem.

Third, the special pair $2,3$ collapses further.  A collision forces
\[
 \frac{2^{a+2}-3}{3^{b+1}-2}
\]
to be a convergent of $\thetaq=\log3/\log2$, and forces the lacunary floor identity
\[
 \left\lfloor\thetaq(3^{b+1}-2)\right\rfloor+3=2^{a+2}.
\]
The exact integer verifier eliminates every $1\le b\le10000$.  This is a rigorous finite theorem
about the historical rational-output switch family, not a finite verification of the original
integer-output conjecture.

Fourth, the prime-factor routes for a hypothetical primitive output pair $(M,A)$ now have an
exact target.  Bugeaud--Corvaja--Zannier rules out linear exponential growth of
$\log\gcd(M^n-1,A^n-1)$, while generic Fermat synchronization forces only a common divisor with
sublinear logarithm.  A successful prime-order argument would have to extract
counterexample-specific correlation, not merely invoke the distribution of primes.

The best next steps are therefore discrete rather than density based: analyze the shifted-power
conditions on convergents of $\thetaq$; compute and factor the primitive residual numerators in
the logarithmic-mean expansion; search for congruence obstructions to the unique roots
$Q_{p,a,b}$; and formalize the elementary switch, logarithmic-mean, and convergent-sieve
lemmas.  For the original integer-output conjecture, the remaining decisive bridges are still
the fixed-base Matrix Coefficient Conjecture, a third algebraic exponential, leading
$p$-adic determinant cancellation, or independence of the relevant products of prime
logarithms.

\begin{resultbox}
\textbf{Bottom line.}
No complete proof is claimed.  Prime-distribution theory improves the 1944 asymptotic machinery
but not the transcendence core.  The new contribution is a reduction of the historical
prime-switch collision to a microscopic one-prime window and, for $2$ and $3$, to an exact
continued-fraction/lacunary-power sieve, together with a proof-producing exclusion through
switch level $10000$.  The scale mismatch is now quantitative rather than heuristic.
\end{resultbox}
...
% --- imported with the fourth (prime-distribution) continuation ---

\bibitem{Ingham1937}
A.~E.~Ingham,
\newblock \emph{On the difference between consecutive primes},
\newblock Quarterly Journal of Mathematics, Oxford Series \textbf{8} (1937), 255--266.
\newblock DOI: \href{https://doi.org/10.1093/qmath/os-8.1.255}{10.1093/qmath/os-8.1.255}.

\bibitem{BakerHarmanPintz2001}
R.~C.~Baker, G.~Harman, and J.~Pintz,
\newblock \emph{The difference between consecutive primes, II},
\newblock Proceedings of the London Mathematical Society (3) \textbf{83}(3) (2001), 532--562.
\newblock DOI: \href{https://doi.org/10.1112/plms/83.3.532}{10.1112/plms/83.3.532}.

\bibitem{Li2025}
R.~Li,
\newblock \emph{The number of primes in short intervals and numerical calculations for
Harman's sieve},
\newblock arXiv:2308.04458v8, October 2025.
\newblock \url{https://arxiv.org/abs/2308.04458}.

\bibitem{GuthMaynard2024}
L.~Guth and J.~Maynard,
\newblock \emph{New large value estimates for Dirichlet polynomials},
\newblock arXiv:2405.20552v2, April 2026.
\newblock \url{https://arxiv.org/abs/2405.20552}.

\bibitem{GafniTao2026}
A.~Gafni and T.~Tao,
\newblock \emph{On the number of exceptional intervals to the prime number theorem in short
intervals},
\newblock Essential Number Theory \textbf{5} (2026), 221--241;
arXiv:2505.24017.
\newblock DOI: \href{https://doi.org/10.2140/ent.2026.5.221}{10.2140/ent.2026.5.221}.

\bibitem{TurnageButterbaugh2026}
C.~L.~Turnage-Butterbaugh,
\newblock \emph{A decades-long breakthrough in zero-density estimates and primes in short
intervals},
\newblock arXiv:2607.04632, July 2026; to appear in the Bulletin of the American Mathematical
Society.
\newblock \url{https://arxiv.org/abs/2607.04632}.

\bibitem{Prasad2025}
D.~Prasad,
\newblock \emph{Some questions in Diophantine approximation: real and $p$-adics},
\newblock arXiv:2505.15744, May 2025.
\newblock \url{https://arxiv.org/abs/2505.15744}.

\bibitem{RivatWu2001}
J.~Rivat and J.~Wu,
\newblock \emph{Prime numbers of the form $\lfloor n^c\rfloor$},
\newblock Glasgow Mathematical Journal \textbf{43}(2) (2001), 237--254.
\newblock DOI: \href{https://doi.org/10.1017/S0017089501020080}{10.1017/S0017089501020080}.

\bibitem{GuoGuoLu2025}
L.~Guo, V.~Z.~Guo, and L.~Lu,
\newblock \emph{The Piatetski--Shapiro prime number theorem},
\newblock arXiv:2505.10391v2, December 2025.
\newblock \url{https://arxiv.org/abs/2505.10391}.

\bibitem{GuoGuo2026}
L.~Guo and V.~Z.~Guo,
\newblock \emph{Piatetski--Shapiro primes in short intervals},
\newblock arXiv:2606.01115, May 2026.
\newblock \url{https://arxiv.org/abs/2606.01115}.

\bibitem{Jain2025}
S.~Jain,
\newblock \emph{Smooth numbers in short intervals},
\newblock arXiv:2502.10530, February 2025.
\newblock \url{https://arxiv.org/abs/2502.10530}.

\bibitem{Zsigmondy1892}
K.~Zsigmondy,
\newblock \emph{Zur Theorie der Potenzreste},
\newblock Monatshefte f\"ur Mathematik und Physik \textbf{3} (1892), 265--284.
\newblock DOI: \href{https://doi.org/10.1007/BF01692444}{10.1007/BF01692444}.
...
\newblock DOI: \href{https://doi.org/10.1007/s00209-002-0448-z}{10.1007/s00209-002-0448-z}.
