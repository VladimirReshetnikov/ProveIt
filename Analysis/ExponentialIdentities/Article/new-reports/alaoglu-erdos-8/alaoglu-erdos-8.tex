...
\hypersetup{
  ...
  pdfsubject={Machine-checked partial results, exact conditional structure, determinant and
approximation methods, exact obstructions, and research directions}
}
\urlstyle{same}
...
\vspace{0.65cm}
{\LARGE Machine-Checked Partial Results and Exact Conditional Structure,\par}
\vspace{0.10cm}
{\LARGE Determinant, Transcendence, and Approximation Methods,\par}
\vspace{0.10cm}
{\LARGE and the Remaining Barrier\par}
\vspace{1.0cm}
...
through $2^{29}$.  The fourth continuation proves an all-degree obstruction to the generic
normalized rational-polynomial strategy, an exact contact-equals-multiplicity theorem, and an
arithmetic M\"untz certificate, leaving global adelic cancellation as the viable approximation
target.
\end{minipage}
\vfill
...
A fourth continuation investigates uniform, rational, integer-valued, M\"untz, Pad\'e, and
Hermite--Pad\'e approximation with every coefficient denominator included in the arithmetic
budget.  It proves that the generic normalized rational-polynomial forcing inequality is
impossible in every degree for dyadic blocks $T\ge4$; that rational contact with the
irrational-power curve at a positive rational point equals ordinary algebraic multiplicity;
and that a norm-below-one integral M\"untz polynomial gives an exact candidate-count
certificate.  Direct-scale approximation is shown to require degree linear in $T$, far above
the conditional block population.  The recent 2024--2026 literature on constrained Remez
algorithms, effective Hermite--Pad\'e methods, transcendence measures, and arithmetic holonomy
is audited.  No complete proof is claimed: the remaining approximation target is a genuinely
adelic auxiliary residual whose smallness comes from global arithmetic cancellation.
...
\renewcommand{\theHsection}{P\arabic{section}}

\section{Fourth continuation: polynomial approximation, outcome, and claim ledger}
\label{pa:sec-outcome}

This continuation investigates whether approximation theory can convert the integrality of
\(m^{\thetaq}\) at candidate integers into an auxiliary-polynomial contradiction.  The
starting point is deliberately broader than ordinary Lagrange interpolation.  Uniform
minimax approximation, direct-scale rational polynomials, integer-valued polynomials,
M\"untz systems, Pad\'e and Hermite--Pad\'e contact, and constrained Remez methods are all
examined with the arithmetic denominator included in the estimate rather than appended at the
end.

No complete proof of the Alaoglu--Erd\H{o}s conjecture was obtained.  The approximation
program nevertheless produces three exact results which materially sharpen the inherited
failure analysis.

\begin{enumerate}
\item A rational polynomial approximant on the normalized interval \([1,2]\) must beat the
lattice spacing
\[
  \frac{1}{H3^T2^{Td}}
\]
when its degree is \(d\) and its coefficient denominator is \(H\).  A finite-difference lower
bound proves that this is impossible for every degree as soon as \(T\ge4\).  Thus the most
natural ``normalize, approximate, clear denominators'' strategy is not merely quantitatively
weak; its required inequality has the wrong sign in every degree.

\item If \(P\in\Q[X,Y]\) and \((a,b)\in\Q_{>0}^2\), then restriction to the transcendental
one-parameter curve
\[
  z\longmapsto(ae^z,be^{\thetaq z})
\]
has order of vanishing exactly equal to the ordinary algebraic multiplicity of \(P\) at
\((a,b)\).  Consequently rational polynomials have no anomalous tangency to
\(Y=X^{\thetaq}\) at rational points.  In a bidegree \((d,e)\) rectangle, the largest possible
rational Hermite--Pad\'e contact is exactly \(d+e\), not
\((d+1)(e+1)-1\).  This is an exact area-to-diameter collapse caused by the transcendence of
\(\thetaq\).

\item There is an exact arithmetic M\"untz certificate: an integer linear combination of the
functions \(x^{i+j\thetaq}\) which has norm below one on a dyadic interval vanishes at every
candidate there, and hence bounds their number by the size of its support minus one.  This is
the approximation-theoretic dual of the generalized-Vandermonde determinant method.  It
identifies the viable remaining target: not ordinary real minimax approximation, but a small
vector in an integral coefficient lattice with simultaneous archimedean and finite-prime
control.
\end{enumerate}
...
\begin{center}
\begin{tabularx}{0.97\textwidth}{@{}>{\raggedright\arraybackslash}p{0.28\textwidth}X>{\raggedright\arraybackslash}p{0.16\textwidth}@{}}
\toprule
Item & Content & Status\\
\midrule
Dyadic arithmetic-resolution lemma & Exact integer obtained by clearing a rational approximant at a candidate point; both polynomial and rational-residual versions are given. & \statusnew\\
\addlinespace
Finite-difference approximation floor & An explicit all-degree lower bound for \(\|t^{\thetaq}-p(t)\|_{[1,2]}\). & \statusnew\\
\addlinespace
Normalization-tax theorem & For every \(T\ge4\), every degree \(d\), and every rational coefficient denominator \(H\), the approximation error is at least the arithmetic forcing threshold. & \statusnew\\
\addlinespace
Direct-scale degree floor & Any rational polynomial which is close enough on \([2^T,2^{T+1}]\) to force equality at candidates must have degree linear in \(T\); the sharp asymptotic slope is governed by the Bernstein ellipse through the branch point at zero. & \statusnew{} plus classical approximation theory\\
\addlinespace
Contact equals multiplicity & Rational algebraic curves have no excess contact with \(Y=X^{\thetaq}\) at positive rational points. & \statusnew\\
\addlinespace
Area-to-diameter collapse & Rational bidegree \((d,e)\) Hermite--Pad\'e contact is exactly \(d+e\), while coefficients in \(\Q(\thetaq)\) permit the generic order \((d+1)(e+1)-1\). & \statusnew\\
\addlinespace
Integer M\"untz certificate & A norm-below-one integral exponential polynomial gives an exact candidate-count bound. & \statusnew\\
\addlinespace
Recent literature audit & Approximation, Hermite--Pad\'e, effective Diophantine approximation, and modern transcendence-measure papers from 2024--2026 are compared with the fixed-base obstruction. & \statusknown\\
\addlinespace
Arithmetic minimax construction & Construct a nonzero integer M\"untz polynomial or a rational numerator residual which is small enough adelically to cross the product-formula threshold. & \statusopen\\
\bottomrule
\end{tabularx}
\end{center}

The proofs below use only the already established transcendence of
\(\thetaq=\log_2 3\), elementary finite differences, standard Chebyshev-system zero counting,
and classical polynomial approximation.  No unproved transcendence conjecture enters any
statement tagged \statusnew{}.

\section{Arithmetic resolution on a dyadic block}
\label{pa:sec-resolution}

\subsection{The normalized candidate equation}

Fix an integer \(T\ge0\), and let \(m\) be a candidate in the half-open dyadic block
\([2^T,2^{T+1})\).  Put
\begin{equation}
  u:=\frac{m}{2^T}\in[1,2),
  \qquad n:=m^{\thetaq}\in\N.
  \label{pa:eq-normalization}
\end{equation}
Since \(2^{T\thetaq}=3^T\), the candidate identity becomes
\begin{equation}
  n=3^T u^{\thetaq}.
  \label{pa:eq-normalized-candidate}
\end{equation}
Analytically this normalization is ideal: the approximation interval no longer grows with
\(T\).  Arithmetically it is expensive: a degree-\(d\) monomial in \(u=m/2^T\) introduces a
denominator as large as \(2^{Td}\).  The next lemma records the exact cost.

\begin{lemma}[Normalized polynomial arithmetic resolution]
\label{pa:lem-normalized-resolution}
Let \(p\in\Q[t]\) have degree at most \(d\), and let \(H\in\N\) satisfy
\(Hp\in\Z[t]\).  For every candidate pair \((m,n)\) satisfying
\eqref{pa:eq-normalization},
\begin{equation}
  H2^{Td}\bigl(n-3^T p(u)\bigr)\in\Z.
  \label{pa:eq-normalized-integer}
\end{equation}
Consequently, if
\begin{equation}
 \bigl\|t^{\thetaq}-p(t)\bigr\|_{[1,2]}
 <\frac{1}{H3^T2^{Td}},
 \label{pa:eq-normalized-threshold}
\end{equation}
then \(p(u)=u^{\thetaq}\) at every candidate in the block.
\end{lemma}

\begin{proof}
Write
\[
 Hp(t)=\sum_{k=0}^{d}a_kt^k,
 \qquad a_k\in\Z.
\]
Then
\[
 H2^{Td}p(m/2^T)
 =\sum_{k=0}^{d}a_km^k2^{T(d-k)}\in\Z.
\]
The first term \(H2^{Td}n\) is also integral, proving
\eqref{pa:eq-normalized-integer}.  By \eqref{pa:eq-normalized-candidate}, the
absolute value of that integer is
\[
 H3^T2^{Td}\,\bigl|u^{\thetaq}-p(u)\bigr|.
\]
Under \eqref{pa:eq-normalized-threshold} it is strictly below one, hence is zero.
\end{proof}

Because the functions
\[
 1,t,t^2,\ldots,t^d,t^{\thetaq}
\]
form a Chebyshev system on \((0,\infty)\), a nonzero difference
\(t^{\thetaq}-p(t)\) has at most \(d+1\) positive zeros.  Thus a polynomial meeting
\eqref{pa:eq-normalized-threshold} would imply
\begin{equation}
 \#\bigl(\Cand\cap[2^T,2^{T+1}]\bigr)\le d+1.
 \label{pa:eq-normalized-count}
\end{equation}
The point of Section~\ref{pa:sec-poly-lower} is that the required polynomial does not exist
for \(T\ge4\), regardless of \(d\).

\subsection{Direct-scale rational polynomials}

Normalizing first is not the only possible arithmetic arrangement.  One may instead choose a
rational polynomial in the physical variable \(x\), where evaluation at an integer does not
create the factor \(2^{Td}\).

\begin{lemma}[Direct-scale arithmetic forcing]
\label{pa:lem-direct-resolution}
Let \(q\in\Q[X]\), \(\deg q\le d\), and suppose \(Hq\in\Z[X]\).  If
\begin{equation}
 \bigl\|X^{\thetaq}-q(X)\bigr\|_{[2^T,2^{T+1}]}
 <\frac1H,
 \label{pa:eq-direct-threshold}
\end{equation}
then \(q(m)=m^{\thetaq}\) for every candidate integer \(m\) in that block.  Necessarily,
\begin{equation}
 H3^T E_d(\thetaq)<1,
 \qquad
 E_d(\thetaq):=
 \inf_{\deg p\le d}\|t^{\thetaq}-p(t)\|_{[1,2]}.
 \label{pa:eq-direct-necessary}
\end{equation}
\end{lemma}

\begin{proof}
At a candidate \(m\), the number
\[
 H\bigl(m^{\thetaq}-q(m)\bigr)
\]
is an integer.  Inequality \eqref{pa:eq-direct-threshold} makes its absolute value less than
one, so it vanishes.  For the necessary condition, put
\[
 p(t):=3^{-T}q(2^Tt).
\]
...
\[
 \|t^{\thetaq}-p(t)\|_{[1,2]}
 =3^{-T}\|X^{\thetaq}-q(X)\|_{[2^T,2^{T+1}]}
 <\frac{1}{H3^T}.
\]
Taking the infimum over all real polynomials proves \eqref{pa:eq-direct-necessary}.
\end{proof}

The direct-scale formulation avoids the generic \(2^{Td}\) denominator tax, so it is not
excluded by the normalization theorem below.  It pays in a different currency: degree.  To
make an error of order one while the function has size \(3^T\), a polynomial must have degree
linear in \(T\).  Section~\ref{pa:sub-direct-degree} shows that this degree is far larger than
the conditional candidate population it is supposed to contradict.

\subsection{The correct residual for rational approximation}

A quotient error \(|t^{\thetaq}-A(t)/B(t)|\) is not itself an algebraic integer.  The
quantity compatible with the candidate lattice is the numerator residual.

\begin{lemma}[Rational numerator-residual resolution]
\label{pa:lem-rational-residual}
Let \(A,B\in\Z[t]\) have degree at most \(d\).  For a candidate
\(m=2^Tu\), \(n=m^{\thetaq}\),
\begin{equation}
 2^{Td}\bigl(B(u)n-3^TA(u)\bigr)
 =3^T2^{Td}\bigl(B(u)u^{\thetaq}-A(u)\bigr)
 \in\Z.
 \label{pa:eq-rational-residual-integer}
\end{equation}
Hence
\begin{equation}
 \bigl\|B(t)t^{\thetaq}-A(t)\bigr\|_{[1,2]}
 <\frac{1}{3^T2^{Td}}
 \label{pa:eq-rational-residual-threshold}
\end{equation}
forces \(B(u)u^{\thetaq}=A(u)\) at every candidate in the block.
\end{lemma}

\begin{proof}
Both \(2^{Td}A(m/2^T)\) and \(2^{Td}B(m/2^T)\) are integers.  Multiply the first by \(3^T\)
and the second by the integer \(n\), then use \(n=3^Tu^{\thetaq}\).  The strict inequality
\eqref{pa:eq-rational-residual-threshold} makes the resulting integer have absolute value less
than one.
\end{proof}

If \(A\) and \(B\) have rational coefficients with a common denominator \(H\), the right
side of \eqref{pa:eq-rational-residual-threshold} is divided by \(H\).  The analytic quotient
error satisfies
\[
 \left|t^{\thetaq}-\frac{A(t)}{B(t)}\right|
 =\frac{|B(t)t^{\thetaq}-A(t)|}{|B(t)|};
\]
therefore poles or a large denominator polynomial may make the quotient look excellent while
the arithmetic numerator remains much too large.  Any rational-approximation proof must
control the numerator residual and coefficient height simultaneously.

\section{Uniform polynomial approximation and the normalization tax}
\label{pa:sec-poly-lower}

\subsection{An explicit finite-difference lower bound}

The next estimate is elementary but has exactly the dependence needed for the arithmetic
comparison.  Recall
\[
 \fall{\thetaq}{r}=\thetaq(\thetaq-1)\cdots(\thetaq-r+1).
\]

\begin{theorem}[All-degree finite-difference lower bound]
\label{pa:thm-fd-lower}
For \(d\ge1\), every real polynomial \(p\) of degree at most \(d\) satisfies
\begin{equation}
 \bigl\|t^{\thetaq}-p(t)\bigr\|_{[1,2]}
 \ge c_d,
 \qquad
 c_d:=
 \frac{|\fall{\thetaq}{d+1}|\,2^{\thetaq-2d-2}}
 {(d+1)^{d+1}}.
 \label{pa:eq-cd}
\end{equation}
For \(d=0\), the lower bound \(E_0(\thetaq)\ge1\) holds.
\end{theorem}

\begin{proof}
Let \(h=1/(d+1)\), let \(f(t)=t^{\thetaq}\), and put
\(E=\|f-p\|_{[1,2]}\).  Since \(\Delta_h^{d+1}p(1)=0\),
\[
 |\Delta_h^{d+1}f(1)|
 =|\Delta_h^{d+1}(f-p)(1)|
 \le\sum_{j=0}^{d+1}\binom{d+1}{j}E
 =2^{d+1}E.
\]
...
\[
 \Delta_h^{d+1}f(1)
 =h^{d+1}f^{(d+1)}(\xi)
 =h^{d+1}\fall{\thetaq}{d+1}\xi^{\thetaq-d-1}.
\]
For \(d\ge1\), the exponent \(\thetaq-d-1\) is negative, so
\(
 \xi^{\thetaq-d-1}\ge2^{\thetaq-d-1}
\).
Combining the two displays and dividing by \(2^{d+1}\) gives
\eqref{pa:eq-cd}.  If \(p\) is constant, the endpoint values of \(f\) are \(1\) and \(3\),
so one of the two endpoint errors is at least one.
\end{proof}

The bound is weaker than the sharp Bernstein-ellipse asymptotics, but it is explicit, valid
for every degree, and strong enough to settle the normalized arithmetic comparison.

\subsection{No normalized rational polynomial crosses the lattice threshold}

The simple rational bounds
\begin{equation}
 \frac32<\thetaq<\frac85
 \label{pa:eq-theta-rational-bounds}
\end{equation}
follow from \(2^3<3^2\) and \(3^5<2^8\).  They are sufficient for a completely rational proof
of the next theorem.

\begin{theorem}[The normalization-tax obstruction]
\label{pa:thm-normalization-tax}
Let \(T\ge4\), \(d\ge0\), \(H\ge1\), and let \(p\in\Q[t]\) have degree at most \(d\) with
\(Hp\in\Z[t]\).  Then
\begin{equation}
 H3^T2^{Td}
 \bigl\|t^{\thetaq}-p(t)\bigr\|_{[1,2]}
 \ge1.
 \label{pa:eq-tax-obstruction}
\end{equation}
In particular, the arithmetic forcing inequality
\eqref{pa:eq-normalized-threshold} is impossible in every degree once \(T\ge4\).
\end{theorem}

\begin{proof}
The case \(d=0\) follows from Theorem~\ref{pa:thm-fd-lower}, since the left side is at least
\(3^T\ge81\).  For \(d\ge1\), it is enough to treat \(T=4\) and \(H=1\).  Define
\begin{equation}
 L_d:=3^4 2^{4d}c_d
 =81\,2^{\thetaq-2}
 \frac{4^d|\fall{\thetaq}{d+1}|}{(d+1)^{d+1}}.
 \label{pa:eq-Ld}
\end{equation}
For \(d\ge1\),
\begin{equation}
 \frac{L_{d+1}}{L_d}
 =4(d+1-\thetaq)
 \frac{(d+1)^{d+1}}{(d+2)^{d+2}}.
 \label{pa:eq-L-ratio}
\end{equation}
Using \(\thetaq>3/2\), this ratio is less than one for \(d=1,2,3,4\): respectively it is
bounded above by
\[
 \frac8{27},\qquad
 \frac{162}{256},\qquad
 \frac{2560}{3125},\qquad
 \frac{43750}{46656}.
\]
Thus \(L_1>\cdots>L_5\).

Next, \(2^{\thetaq-2}>2^{-1/2}>7/10\).  From
\eqref{pa:eq-theta-rational-bounds},
\begin{align*}
 L_5
 &>81\cdot\frac7{10}\cdot\frac{4^5}{6^6}
   \left(\frac32\right)\left(\frac12\right)
   \left(\frac25\right)\left(\frac75\right)
   \left(\frac{12}{5}\right)\left(\frac{17}{5}\right)\\
 &=\frac{13328}{3125}>4.
\end{align*}
For the next ratio, \(\thetaq<8/5\) gives
\[
 \frac{L_6}{L_5}
 >4\left(6-\frac85\right)\frac{6^6}{7^7}
 =\frac{4105728}{4117715}>\frac{99}{100}.
\]
Hence \(L_6>1\).  Also
\[
 \frac{L_7}{L_6}
 >4\left(7-\frac85\right)\frac{7^7}{8^8}
 =\frac{22235661}{20971520}>1.
\]
For \(d\ge7\), put \(n=d+2\).  The inequality
\[
 \log(1-y)>-\frac{y}{1-y}\qquad(0<y<1)
\]
gives \((1-1/n)^{n-1}>e^{-1}\).  Moreover
\[
 e=\sum_{k=0}^{\infty}\frac1{k!}
 <2+\frac12+\frac16+\frac1{24}\sum_{j=0}^{\infty}4^{-j}
 =\frac{49}{18}<\frac{11}{4},
\]
so \(e^{-1}>4/11\).  Therefore
\begin{align*}
 \frac{L_{d+1}}{L_d}
 &>4\frac{d+1-8/5}{d+2}\cdot\frac4{11}\\
 &=\frac{16(5d-3)}{55(d+2)}
 \ge\frac{512}{495}>1.
\end{align*}
The first five terms satisfy \(L_1>\cdots>L_5>4\); independently, the displayed lower
bound gives \(L_6>1\), and every ratio from \(L_6\) onward is greater than one.  Hence
\(L_d>1\) for every \(d\ge1\).  Increasing \(T\) or \(H\) can only increase the left side
of \eqref{pa:eq-tax-obstruction}.
\end{proof}

\begin{resultbox}
\textbf{Interpretation.}
Uniform approximation of \(t^{\thetaq}\) on a fixed interval is exponentially good in the
degree.  But after the substitution \(t=m/2^T\), clearing a generic rational degree-\(d\)
polynomial costs \(2^{Td}\).  Theorem~\ref{pa:thm-normalization-tax} proves that the arithmetic
resolution decays faster than the approximation error for every degree, already at \(T=4\).
The hypothetical counterexample range begins beyond \(T=29\) by
Theorem~\ref{thm:finite29}, so this route fails with an enormous margin where it matters.
\end{resultbox}

For calibration, set \(c_0:=1\).  Table~\ref{pa:tab-tax29} gives the base-ten logarithm of the lower bound
\(3^{29}2^{29d}c_d\).  The table is not used in the proof.
...
\bottomrule
\end{tabular}
\caption{The normalized arithmetic gap at the first currently admissible block scale.}
\label{pa:tab-tax29}
\end{table}

\subsection{The direct-scale degree floor}
\label{pa:sub-direct-degree}

The direct-scale strategy of Lemma~\ref{pa:lem-direct-resolution} avoids the factor
\(2^{Td}\), but Theorem~\ref{pa:thm-fd-lower} still gives an explicit necessary degree
condition:
\begin{equation}
 H3^T c_d<1.
 \label{pa:eq-direct-cd-condition}
\end{equation}
The gamma identity
\begin{equation}
 |\fall{\thetaq}{d+1}|
 =\frac{\Gamma(d+1-\thetaq)}{|\Gamma(-\thetaq)|}
 \label{pa:eq-falling-gamma}
\end{equation}
and Stirling's formula yield
\begin{equation}
 c_d=C_{\thetaq}\,d^{-\thetaq-1/2}(4e)^{-d}
 \bigl(1+O(d^{-1})\bigr)
 \label{pa:eq-cd-asymptotic}
\end{equation}
for an explicit positive constant \(C_{\thetaq}\).  Consequently any direct-scale polynomial
meeting \eqref{pa:eq-direct-threshold} must satisfy
\begin{equation}
 d\ge
 \frac{\log3}{\log(4e)}T-O(\log T)
 =0.460384\ldots\,T-O(\log T).
 \label{pa:eq-direct-degree-floor}
\end{equation}
A larger denominator \(H\) only raises the required degree.

This elementary slope can be sharpened by classical complex approximation.  Map
\([1,2]\) to \([-1,1]\) by \(s=2t-3\).  The branch point \(t=0\) becomes \(s=-3\).  The
largest Bernstein ellipse avoiding that singularity has parameter
\begin{equation}
 \rho_* =3+2\sqrt2.
 \label{pa:eq-rho-star}
\end{equation}
Indeed, the leftmost point of the ellipse of parameter \(\rho\) is
\(-\tfrac12(\rho+\rho^{-1})\), and equality with \(-3\) gives
\(\rho=3+2\sqrt2\).  Bernstein's theorem therefore gives
\begin{equation}
 \limsup_{d\to\infty} E_d(\thetaq)^{1/d}=\rho_*^{-1}
 \label{pa:eq-Bernstein-rate}
\end{equation}
(and in this algebraic-branch-point case the root rate is a limit).  See
\cite{TrefethenApprox}.  The sharp asymptotic degree floor for
\(3^T E_d(\thetaq)<1/H\) is consequently
\begin{equation}
 d\ge
 \frac{\log3}{\log(3+2\sqrt2)}T-o(T)
 =0.623238\ldots\,T-o(T).
 \label{pa:eq-sharp-direct-degree}
\end{equation}

For the half-open block
\(B(T):=\#(\Cand\cap[2^T,2^{T+1}))\), failure of the conjecture gives the inherited exact formula
\begin{equation}
 B(T)=1+\left\lfloor\frac{T+1}{\beta}\right\rfloor,
 \label{pa:eq-block-count-beta}
\end{equation}
where \(\beta\) is the least nonintegral solution.  The directed finite exclusion gives
\(\beta>29\), so
\[
 B(T)\le1+\frac{T+1}{29}.
\]
A direct-scale polynomial of degree \(d\) which forces equality at candidates only gives
\(B(T)\le d+1\).  But the necessary degree has slope at least
\(0.460384\ldots\), and the true approximation slope is
\(0.623238\ldots\), whereas the conditional population has slope below \(1/29\).  Thus the
finite-difference calibration misses by a factor exceeding \(13.35\), and the sharp
Bernstein calibration misses by a factor exceeding \(18.07\).  Better minimax constants
cannot reverse a linear-slope mismatch of this size.

At \(T=29\), the explicit lower bound already requires \(d\ge11\).  Indeed,
\(c_0>c_1>c_2>\cdots\) (use the quotient of consecutive terms in
\eqref{pa:eq-cd}), and
\[
 3^{29}c_{10}>7.39,
 \qquad
 3^{29}c_{11}<0.557.
\]
The exact numerical values are merely illustrative.  The asymptotic comparison is the
structural point.

\subsection{Using all trivial powers as interpolation nodes}

There is a more aggressive polynomial idea.  Instead of working block by block, interpolate
through every known trivial solution
\[
  (1,1),(2,3),(2^2,3^2),\ldots,(2^N,3^N).
\]
Let \(Q_N\in\Q[X]\) be the unique polynomial of degree at most \(N\) with
\begin{equation}
 Q_N(2^j)=3^j,
 \qquad 0\le j\le N.
 \label{pa:eq-trivial-interpolant}
\end{equation}
Its Newton expansion is
\begin{equation}
 Q_N(X)=\sum_{r=0}^{N}a_r\prod_{j=0}^{r-1}(X-2^j),
 \qquad
 a_r=\frac{\prod_{j=0}^{r-1}(3-2^j)}
 {\prod_{j=0}^{r-1}(2^r-2^j)},
 \label{pa:eq-geometric-newton}
\end{equation}
with the empty products equal to one.  Formula \eqref{pa:eq-geometric-newton} is the Newton
divided-difference formula for geometric nodes.  One quick proof observes that, as a
polynomial in a formal value \(q\), the \(r\)-th divided difference of the data
\((2^k,q^k)\) vanishes at \(q=1,2,2^2,\ldots,2^{r-1}\), because in those cases the data come
from a polynomial in \(X\) of degree below \(r\); its leading coefficient fixes the displayed
denominator.

The system
\[
 1,X,\ldots,X^N,X^{\thetaq}
\]
is Chebyshev, so \(X^{\thetaq}-Q_N(X)\) has at most \(N+1\) positive zeros.  It already has
exactly the \(N+1\) trivial zeros in \eqref{pa:eq-trivial-interpolant}.  Hence
\begin{equation}
 m^{\thetaq}\ne Q_N(m)
 \quad\text{for every positive nonpower of two }m.
 \label{pa:eq-no-extra-intersection}
\end{equation}
This saturates the zero budget perfectly: an arithmetic estimate forcing equality at one
additional candidate would prove the conjecture.

The obstruction is capacity.  On the expanding interval \([1,X]\), the singular point zero
maps to
\[
 -\frac{X+1}{X-1}
\]
...
\begin{equation}
 \rho_X=\frac{\sqrt X+1}{\sqrt X-1}
 =1+\frac{2}{\sqrt X-1}.
 \label{pa:eq-expanding-rho}
\end{equation}
For \(X=2^N\), this tends to one exponentially fast in \(N\).  More precisely,
\(\rho_X^{-N}=\exp(-O(N/\sqrt X))=1-o(1)\), so degree \(N=\log_2X\) obtains no geometric
decay from analyticity.  Equivalently, after dividing by \(X^{\thetaq}\) and rescaling to
\([1/X,1]\), the problem approaches approximation of the endpoint-singular function
\(y^{\thetaq}\) on \([0,1]\), where classical best-polynomial convergence is algebraic rather
than geometric.  The exact Newton denominators in \eqref{pa:eq-geometric-newton} grow rapidly
at the same time.  Thus the scheme spends its entire degree budget creating the known zeros
while the approximation interval becomes exponentially ill-conditioned.  This is a capacity
diagnostic, not a new sharp asymptotic theorem; it shows that the algebraically optimal
interpolant comes with no available arithmetic smallness estimate.

\section{Local rigidity: contact equals multiplicity}
\label{pa:sec-contact}

The approximation routes above are global.  Pad\'e and Hermite--Pad\'e methods instead seek
high-order contact at one point.  Here the transcendence of \(\thetaq\) produces an exact and
unexpectedly strong rigidity theorem.

\subsection{The local contact theorem}

For a polynomial \(P\) and a point \((a,b)\), write
\(\operatorname{mult}_{(a,b)}P\) for the least total degree of a nonzero term in the Taylor
expansion of \(P(a+U,b+V)\).

\begin{theorem}[Rational contact equals algebraic multiplicity]
\label{pa:thm-contact-multiplicity}
Let \(\tau\) be transcendental, let \(0\ne P\in\Q[X,Y]\), and let
\((a,b)\in\Q_{>0}^2\).  If
\[
 s=\operatorname{mult}_{(a,b)}P,
\]
then
\begin{equation}
 \operatorname{ord}_{z=0}P(ae^z,be^{\tau z})=s.
 \label{pa:eq-contact-equals-mult}
\end{equation}
In particular the statement applies to \(\tau=\thetaq\).
\end{theorem}
...
\]
where \(H_s\ne0\) is homogeneous of degree \(s\) and has rational coefficients.  Since
\[
 a(e^z-1)=az+O(z^2),
 \qquad
 b(e^{\tau z}-1)=b\tau z+O(z^2),
\]
the coefficient of \(z^s\) in \(P(ae^z,be^{\tau z})\) is
\begin{equation}
 H_s(a,b\tau).
 \label{pa:eq-leading-contact-coeff}
\end{equation}
Writing
\[
 H_s(U,V)=\sum_{j=0}^{s}h_jU^{s-j}V^j
\]
shows that \(H_s(a,b\tau)\) is a nonzero polynomial in the transcendental number \(\tau\)
with rational coefficients: not all \(h_j a^{s-j}b^j\) vanish.  Hence
\eqref{pa:eq-leading-contact-coeff} is nonzero, and the order is exactly \(s\).
\end{proof}

The theorem says that a rational algebraic curve cannot be tangent to the irrational-power
curve to an order not already visible in its algebraic singularity.  Transcendence eliminates
all cancellation among the different powers of \(\thetaq\).

\begin{corollary}[Simple intersections at candidate points]
\label{pa:cor-simple-candidate}
Let \(a>0\) and \(b=a^{\thetaq}\) be rational.  If \(Q\in\Q[X]\) and \(Q(a)=b\), then
\begin{equation}
 \operatorname{ord}_{X=a}\bigl(X^{\thetaq}-Q(X)\bigr)=1.
 \label{pa:eq-simple-graph-intersection}
\end{equation}
Equivalently, the graph polynomial \(Y-Q(X)\) meets \(Y=X^{\thetaq}\) transversely at every
rational point of intersection.
\end{corollary}

\begin{proof}
The polynomial \(P(X,Y)=Y-Q(X)\) has multiplicity one at \((a,b)\), because
\(\partial P/\partial Y=1\).  Apply Theorem~\ref{pa:thm-contact-multiplicity} after writing
\(X=ae^z\).  Directly, the derivative of the difference at \(a\) is
\[
 \thetaq\frac ba-Q'(a),
\]
a nonzero number because its first term is a nonzero rational multiple of the transcendental
number \(\thetaq\), while the second is rational.
\end{proof}

Thus rational Hermite interpolation cannot double a candidate node ``for free.''  Every
additional prescribed derivative consumes genuine algebraic multiplicity and therefore
ordinary degree.

\subsection{Exact rectangular contact}

\begin{theorem}[Exact rational bidegree contact]
\label{pa:thm-rectangle-contact}
Let \(a,b\in\Q_{>0}\), let \(\tau\) be transcendental, and let
\(0\ne P\in\Q[X,Y]\) satisfy
\[
 \deg_XP\le d,
 \qquad
 \deg_YP\le e.
...
Then
\begin{equation}
 \operatorname{ord}_{z=0}P(ae^z,be^{\tau z})\le d+e.
 \label{pa:eq-rectangle-contact-bound}
\end{equation}
The bound is attained by \((X-a)^d(Y-b)^e\), and equality determines that polynomial up to a
nonzero rational scalar.
\end{theorem}

\begin{proof}
The total degree of \(P\) is at most \(d+e\), so its multiplicity at \((a,b)\) is at most
\(d+e\).  Theorem~\ref{pa:thm-contact-multiplicity} gives
\eqref{pa:eq-rectangle-contact-bound}.  The displayed product has multiplicity \(d+e\), so the
bound is sharp.  If equality holds, the shifted polynomial \(P(a+U,b+V)\) contains no term of
total degree below \(d+e\).  Since its \(U\)-degree is at most \(d\) and its \(V\)-degree at
most \(e\), the only possible term of total degree \(d+e\) is a scalar multiple of
\(U^dV^e\).
\end{proof}

\begin{corollary}[No nondegenerate rational Pad\'e contact]
\label{pa:cor-rational-pade}
Let \(A,B\in\Q[t]\) have degree at most \(d\).  Then
\begin{equation}
 \operatorname{ord}_{t=1}\bigl(B(t)t^{\thetaq}-A(t)\bigr)\le d+1.
 \label{pa:eq-pade-dplus1}
\end{equation}
Equality holds only when
\[
 A(t)=B(t)=c(t-1)^d
\]
for some \(c\in\Q^\times\).  In particular, if \(A\) and \(B\) are coprime,
\(B(1)\ne0\), and \(A(1)=B(1)\), then the residual has order exactly one at \(t=1\).
\end{corollary}

\begin{proof}
Apply Theorem~\ref{pa:thm-rectangle-contact} to
\[
 P(X,Y)=B(X)Y-A(X),
\]
which has bidegree at most \((d,1)\).  For the equality case, expand at \((1,1)\):
\[
 P(1+U,1+V)=B(1+U)V+\bigl(B(1+U)-A(1+U)\bigr).
\]
Multiplicity \(d+1\) forces \(B(1+U)=cU^d\) and forces \(B-A\), a polynomial of degree at
most \(d\), to vanish to order \(d+1\); hence \(A=B=c(t-1)^d\).  If \(A,B\) are coprime and
\(B(1)\ne0\), the derivative of the residual is
\[
 B(1)\thetaq+B'(1)-A'(1),
\]
which is nonzero by transcendence of \(\thetaq\).
\end{proof}

Standard Pad\'e approximants to \((1+z)^\alpha\) achieve high order because their coefficients
are allowed to depend algebraically or rationally on the parameter \(\alpha\).  When the
coefficient field is fixed to \(\Q\) and \(\alpha=\thetaq\) is transcendental, even matching
the first derivative at a rational point is impossible without inserting a common vanishing
factor.  This is not a defect of a particular Pad\'e table; it is an exact field-of-coefficients
obstruction.

\section{Rational Hermite--Pad\'e approximation: area collapses to diameter}
\label{pa:sec-area-diameter}

\subsection{Moment equations over \texorpdfstring{\(\Q\)}{Q}}

Take
\begin{equation}
 P(X,Y)=\sum_{0\le i\le d}\sum_{0\le j\le e}c_{ij}X^iY^j,
 \qquad c_{ij}\in\Q,
 \label{pa:eq-rectangle-P}
\end{equation}
and restrict it to \((e^z,e^{\thetaq z})\):
\begin{equation}
 F(z)=P(e^z,e^{\thetaq z})
 =\sum_{i,j}c_{ij}e^{(i+j\thetaq)z}.
 \label{pa:eq-HP-restriction}
\end{equation}
The \(k\)-th derivative at zero is
\begin{align}
 F^{(k)}(0)
 &=\sum_{i,j}c_{ij}(i+j\thetaq)^k\\
 &=\sum_{r=0}^{k}\binom{k}{r}\thetaq^r
   \sum_{i,j}c_{ij}i^{k-r}j^r.
 \label{pa:eq-moment-unfolding}
\end{align}
Because \(\thetaq\) is transcendental, the scalar equation \(F^{(k)}(0)=0\) is equivalent to
all \(k+1\) rational moment equations
\begin{equation}
 \sum_{i,j}c_{ij}i^{k-r}j^r=0,
 \qquad 0\le r\le k.
 \label{pa:eq-rational-moments}
\end{equation}
Thus a single formal jet condition unfolds into an entire homogeneous layer of bivariate
moment conditions over \(\Q\).

Theorem~\ref{pa:thm-rectangle-contact} now gives the exact answer:
\begin{equation}
 \max_{\substack{0\ne P\in\Q[X,Y]\\ \deg_XP\le d,\ \deg_YP\le e}}
 \operatorname{ord}_{z=0}P(e^z,e^{\thetaq z})
 =d+e.
 \label{pa:eq-rational-HP-max}
\end{equation}
The rectangle contains \((d+1)(e+1)\) coefficient slots, but rational coefficients extract
only the sum of the side lengths as contact order.

\subsection{What changes over \texorpdfstring{\(\Q(\thetaq)\)}{Q(theta)}}

Over the coefficient field \(\Q(\thetaq)\), the frequencies
\(i+j\thetaq\) are distinct and the ordinary Vandermonde dimension count applies.  With
\(M=(d+1)(e+1)\) unknown coefficients one can annihilate the first \(M-1\) derivatives of
\eqref{pa:eq-HP-restriction}; the \(M\)-th derivative cannot vanish as well because the full
frequency Vandermonde is nonsingular.  Therefore
\begin{equation}
 \max_{\substack{0\ne P\in\Q(\thetaq)[X,Y]\\ \deg_XP\le d,\ \deg_YP\le e}}
 \operatorname{ord}_{z=0}P(e^z,e^{\thetaq z})
 =(d+1)(e+1)-1.
 \label{pa:eq-theta-field-HP-max}
\end{equation}
For a square \(d=e=K-1\), the contrast is
\begin{equation}
 \boxed{
 \begin{aligned}
  \text{coefficients in }\Q &: \quad \operatorname{ord}\le2K-2,\\
  \text{coefficients in }\Q(\thetaq) &: \quad \operatorname{ord}\le K^2-1.
 \end{aligned}}
 \label{pa:eq-area-diameter-box}
\end{equation}
This is the \emph{area-to-diameter collapse}.  The coefficient count is quadratic, but the
rational contact order is only linear.

The high-contact object over \(\Q(\thetaq)\) is analytically legitimate but arithmetically
useless for the present number-field argument: \(\Q(\thetaq)\) is a transcendental function
field, not a number field.  It has no finite set of algebraic conjugates and no finite-degree
algebraic norm to \(\Q\); in particular there is no finite-rank number-field integer lattice on
which the usual norm-of-a-nonzero-algebraic-integer contradiction can be based.  Replacing
\(\thetaq\) by an algebraic parameter restores those tools, but then one must control the
perturbation back to the transcendental value with enough precision to survive the rapidly
growing coefficient height.

\subsection{Triangles, arbitrary support, and the one-logarithm deficit}

For triangular support
\[
 \{(i,j)\in\Nzero^2:i+j\le L\},
\]
there are \((L+1)(L+2)/2\) monomials, while rational local contact is at most \(L\).  The
extremizer is simply \((X-1)^L\), or any homogeneous shifted polynomial of multiplicity
\(L\).  More generally, a rational polynomial of total degree \(D\) has contact at most
\(D\) at every positive rational point.

This local fact mirrors the global determinant ceiling of the inherited report.  A
\(K\times K\) support contains \(K^2\) monomials, but rational jet conditions behave as if the
useful complexity were only the boundary length.  In the semigroup determinant, quadratic
support complexity buys only a linear scale in the logarithm of the nodes; here quadratic
coefficient count buys only linear contact.  These are two manifestations of the same field
collapse: the irrational parameter supplies many distinct real frequencies but no algebraic
coefficient field in which they can be canceled cheaply.

\section{Integer-valued and M\"untz approximation}
\label{pa:sec-muntz}

\subsection{Why the physical and normalized intervals pull in opposite directions}

On a physical interval, integer coefficients have excellent arithmetic but terrible
capacity.

\begin{lemma}[No small low-degree integer polynomial on a long integral interval]
\label{pa:lem-small-integer-poly}
Let \(N\in\N\) and let \(0\ne P\in\Z[X]\) have degree \(d<N\).  Then
\begin{equation}
 \|P\|_{[N,2N]}\ge1.
 \label{pa:eq-integer-poly-floor}
\end{equation}
\end{lemma}

\begin{proof}
If \(\|P\|_{[N,2N]}<1\), then \(P(k)\) is an integer of absolute value below one for each of
the \(N+1\) integers \(k=N,N+1,\ldots,2N\).  Hence all those values vanish, giving more roots
than the degree.
\end{proof}

On the normalized interval \([1,2]\), analytic capacity is favorable, but substituting
\(X=m/2^T\) creates the denominator tax quantified by
Theorem~\ref{pa:thm-normalization-tax}.  This is a genuine capacity--integrality
incompatibility: the scale on which approximation is efficient is not the scale on which the
coefficient lattice is integral.

Integer-valued polynomials do not remove the conflict.  If
\(G\in\operatorname{Int}(\Z)\) has degree at most \(d\), then the binomial-basis theorem gives
\begin{equation}
 G(X)=\sum_{k=0}^{d}b_k\binom{X}{k},
 \qquad b_k\in\Z,
 \label{pa:eq-intZ-binomial}
\end{equation}
so \(d!G\in\Z[X]\).  In the normalized arithmetic-resolution lemma one may therefore take
\(H=d!\), which makes the required threshold smaller, not larger.

There is also a global mismatch of targets.  By the classical uniform-distribution theorem
for nonintegral powers, the sequence \(\{n^{\thetaq}\}\) is uniformly distributed modulo one
\cite{KuipersNiederreiter}.  Hence for all sufficiently large dyadic intervals there is an
integer \(n\) with
\[
 \operatorname{dist}(n^{\thetaq},\Z)\ge\frac14.
\]
For every integer-valued polynomial \(G\),
\[
 |n^{\thetaq}-G(n)|\ge\frac14
\]
at such an input.  Thus uniform integer-valued approximation spends most of its effort on the
noncandidate integers, where no exact arithmetic identity is available.  The candidate set is
too sparse for global integer-valued approximation to be the correct object.

\subsection{An exact arithmetic M\"untz certificate}

The mixed exponents already present in the determinant method are better adapted to candidate
integrality.  Let \(S\subset\Nzero^2\) be finite, and for
\(c=(c_{ij})_{(i,j)\in S}\in\Z^S\) define
\begin{equation}
 F_c(x):=\sum_{(i,j)\in S}c_{ij}x^{i+j\thetaq}.
 \label{pa:eq-muntz-polynomial}
\end{equation}
At a candidate \(m\), with \(n=m^{\thetaq}\in\Z\),
\begin{equation}
 F_c(m)=\sum_{(i,j)\in S}c_{ij}m^in^j\in\Z.
 \label{pa:eq-muntz-integrality}
\end{equation}

\begin{theorem}[Arithmetic M\"untz certificate]
\label{pa:thm-muntz-certificate}
Let \(S\subset\Nzero^2\) have \(r\) elements, and let
\(0\ne c\in\Z^S\).  If for some interval \(I\subset(0,\infty)\)
\begin{equation}
 \|F_c\|_I<1,
 \label{pa:eq-muntz-small}
\end{equation}
then
\begin{equation}
 \#(\Cand\cap I)\le r-1.
 \label{pa:eq-muntz-count}
\end{equation}
\end{theorem}

\begin{proof}
At every candidate in \(I\), equations \eqref{pa:eq-muntz-integrality} and
\eqref{pa:eq-muntz-small} force \(F_c(m)=0\).  The exponents \(i+j\thetaq\) are distinct
because \(\thetaq\) is irrational.  The functions \(x^\lambda\) with distinct real exponents
form an extended complete Chebyshev system on \((0,\infty)\), so a nonzero combination with
\(r\) terms has at most \(r-1\) positive zeros.
\end{proof}

The theorem is exactly the approximation-theoretic dual of the generalized-Vandermonde
method.  A determinant lower bound says that evaluations of the mixed monomials at too many
candidate nodes cannot be linearly dependent over the integers.  Theorem
\ref{pa:thm-muntz-certificate} says that a short integer vector in the uniform evaluation norm
would create precisely such a dependence.

The unresolved problem is therefore a shortest-vector problem in an anisotropic function
lattice:
\begin{equation}
 \text{find }0\ne c\in\Z^S
 \quad\text{with}\quad
 \sup_{x\in[X,2X]}
 \left|\sum_{(i,j)\in S}c_{ij}x^{i+j\thetaq}\right|<1.
 \label{pa:eq-arithmetic-minimax-target}
\end{equation}
Ordinary Remez theory minimizes over real coefficient vectors.  It does not control the
integral coefficient lattice or the finite-prime size of the resulting vector.  The recent
work of Protasov and Kamalov develops generalized alternance and Remez algorithms for arbitrary
finite systems, constraints, complex exponentials, and lacunary polynomials
\cite{ProtasovKamalov}; it is well suited to numerical reconnaissance for
\eqref{pa:eq-arithmetic-minimax-target}, but an arithmetic transference theorem is still
needed before its output can enter a product-formula proof.

\subsection{A convex-body formulation}

Choose \(M\ge |S|\) distinct sample points
\(x_1,\ldots,x_M\in[X,2X]\), and form the evaluation matrix
\[
 V_{\ell,(i,j)}=x_\ell^{i+j\thetaq}.
\]
Distinct positive nodes make \(V\) full column rank, so the set
\[
 \mathcal K(\varepsilon)=
 \{c\in\R^S:|Vc|_\infty\le\varepsilon\}
\]
is a bounded symmetric convex body.  Minkowski's theorem can produce a nonzero integer point
only when its volume is large relative to the coefficient lattice.  But the singular values of
\(V\) encode the same generalized-Vandermonde determinants already estimated in the report.
Using more sample points improves uniform control while shrinking the convex body; using fewer
points enlarges the body but loses control between samples.  Any successful transference must
therefore exploit special prime divisibility or coefficient sparsity not visible in the real
condition number alone.

A concrete computational program is nevertheless available:
\begin{enumerate}
\item use generalized Remez iterations to identify supports \(S\) and real minimax sign
patterns with unusually small norm;
\item solve the corresponding closest-vector problem in \(\Z^S\) using exact interval
evaluations;
\item compute the complete content and local Newton polygons of the coefficient vector;
\item accept a candidate only if its archimedean gain survives the product of all coefficient
heights and denominator factors.
\end{enumerate}
This is substantially narrower than a blind search over auxiliary polynomials.

\section{Rational approximation and the residual-height barrier}
\label{pa:sec-rational}

\subsection{Why a better quotient is not yet a better arithmetic object}

Rational approximation can converge much faster than polynomial approximation when poles are
placed near a branch singularity.  For the present problem, however, Lemma
\ref{pa:lem-rational-residual} shows that the decisive object is
\[
 R_{A,B}(t):=B(t)t^{\thetaq}-A(t),
\]
not \(t^{\thetaq}-A(t)/B(t)\).  To force equality at a candidate in the \(T\)-th block one
needs
\[
 \|R_{A,B}\|_{[1,2]}<3^{-T}2^{-Td}
\]
for integral \(A,B\) of degree at most \(d\), with an additional common-denominator factor for
rational coefficients.  A small quotient error is useful only if \(B\) remains small in norm
...
\[
 0,1,\ldots,d,
 \qquad
 \thetaq,\thetaq+1,\ldots,\thetaq+d.
\]
Unless it is identically zero, it has at most \(2d+1\) positive zeros.  Thus a residual meeting
the arithmetic threshold would give a block count bound of \(2d+1\).  This can improve the
constant relative to ordinary polynomials only if the numerator residual converges at a rate
which survives both \(\|B\|\) and coefficient height.

\subsection{Local Pad\'e versus nonlocal rational approximation}

Corollary~\ref{pa:cor-rational-pade} rules out high-order rational Pad\'e contact over \(\Q\)
at a rational point.  It does not rule out a nonlocal rational minimax approximant with many
alternating extrema.  The distinction is important:

\begin{itemize}
\item \emph{local route:} exact Taylor cancellation requires coefficients in
\(\Q(\thetaq)\), and rational coefficients collapse to first-order contact after common
factors are removed;
\item \emph{global route:} rational coefficients can approximate real minimax coefficients
arbitrarily well, but the common denominator and numerator residual must be included in the
arithmetic inequality;
\item \emph{adelic route:} one seeks coefficients chosen simultaneously for small real norm
and favorable \(p\)-adic divisibility.  This is the only version not already blocked by the
local theorem or the normalization tax.
\end{itemize}

The appropriate target is therefore not a classical Pad\'e table.  It is a rational or integral
residual with a product-formula budget:
\begin{equation}
 \log\|B(t)t^{\thetaq}-A(t)\|_{[1,2]}
 +\sum_p \mathcal H_p(A,B)
 <-T\log3-Td\log2,
 \label{pa:eq-adelic-residual-target}
\end{equation}
where \(\mathcal H_p\) records exactly the loss incurred when evaluations are cleared at
candidate points.  The inherited min-plus ceiling proves that termwise valuation minima cannot
supply the missing amount.  Any gain in \eqref{pa:eq-adelic-residual-target} must come from
cancellation among terms or from the boundary residuals of the multiplicative interpolant.

\section{Consequences for the inherited grid and boundary residuals}
\label{pa:sec-boundary-consequences}

The third continuation constructed the degree-\(K^2-1\) graph interpolant \(Q_K\) on the
conditional \(K\times K\) multiplicative grid and factored its dyadic and triadic dilation
defects as
\[
 Q_K(2X)-M Q_K(X)=\Pi_{K-1,K}(X)U_K(X),
\]
\[
 Q_K(3X)-A Q_K(X)=\Pi_{K,K-1}(X)V_K(X),
\]
with \(\deg U_K=\deg V_K=K-1\).  A hypothetical least generator \(\beta\) is
transcendental by the inherited results, so
Theorem~\ref{pa:thm-contact-multiplicity} applies here with \(\tau=\beta\).  It adds three
exact pieces of information.

\begin{enumerate}
\item Every interpolation node is a simple zero of
\(X^{\beta}-Q_K(X)\), because \(Q_K\) has rational coefficients and each node has rational
coordinates.  No squared grid divisor can be recovered from hidden tangency.

\item More generally, if a rational auxiliary polynomial \(P(X,Y)\) vanishes to algebraic
multiplicity \(s\) at a grid point, then its restriction to the irrational-power curve has
exactly order \(s\).  Confluent conditions do not acquire any bonus from the curve parameter;
each derivative condition must be paid for by ordinary local multiplicity.

\item Therefore any extra divisibility of \(U_K\) or \(V_K\) is necessarily global.  It must
come from cancellation among boundary interpolation data, from a common arithmetic content,
or from a local Newton-polygon phenomenon.  It cannot be explained by an analytic
high-contact zero inherited from the interior grid.
\end{enumerate}

This focuses the proposed boundary computation.  For moderate \(K\), one should clear
\(Q_K,U_K,V_K\) primitively and record:
\begin{equation}
 \gcd\text{ of coefficients},\qquad
 v_p\text{-Newton polygons},\qquad
 \operatorname{Res}(U_K,V_K),
 \qquad
 \operatorname{Res}(U_K,L_2^\pm L_3^\pm),
 \label{pa:eq-boundary-audit}
\end{equation}
for the primes dividing \(2\cdot3\cdot MA\) and for the large primes appearing in the node
differences.  A systematic excess over the termwise min-plus prediction would be genuine new
information.  Absence of such excess would retire the boundary route just as decisively as
Theorem~\ref{pa:thm-normalization-tax} retires generic normalized polynomial approximation.

\section{Recent literature, 2024--2026}
\label{pa:sec-literature}

The literature search was updated through August 21, 2026, with emphasis on primary sources
and arXiv revisions.  No paper located claims a proof of the fixed-base Alaoglu--Erd\H{o}s
conjecture.  The relevant developments sharpen auxiliary-function methods around it but retain
algebraicity hypotheses absent here.

\subsection{Matrix coefficients and logarithm matrices}

Dasgupta and Kakde's Matrix Coefficient Conjecture asks that a singular matrix of logarithms of
algebraic numbers acquire a zero coefficient after rational changes of basis
\cite{DasguptaKakdeMCC}.  Their paper appeared in \emph{Advances in Mathematics} in 2026.  As
proved in the third continuation, the present conjecture is exactly a positive-integral,
fixed-first-column instance in dimension two.  Polynomial approximation does not bypass that
identification: a successful arithmetic minimax construction would amount to an auxiliary
object strong enough to prove this fixed-base matrix-coefficient case.

\subsection{Constrained Chebyshev and Remez algorithms}

Protasov and Kamalov's revised 2025 paper develops best uniform approximation for arbitrary
finite systems, including non-Chebyshev systems and linear constraints, by a generalized
alternance criterion and modified Remez iteration \cite{ProtasovKamalov}.  Their examples
include complex exponential systems and lacunary polynomial families.  This is directly useful
for exploring real minimax versions of \eqref{pa:eq-arithmetic-minimax-target}.  The missing
ingredient is arithmetic: the algorithm optimizes over a real affine space, whereas the
conjecture requires an integral coefficient vector whose complete height and \(p\)-adic
content survive the final inequality.

\subsection{Hermite--Pad\'e constructions with algebraic parameters}

F\"urnsinn and Pannier give an effective version of the order-one \(p\)-curvature theorem by
making a Chudnovsky Hermite--Pad\'e argument explicit \cite{FuernsinnPannier}.  Their
construction uses approximants to consecutive powers of a binomial function with an algebraic
parameter, coefficients in the corresponding algebraic field, and a norm from a splitting
field to \(\Q\).  The algebraic norm is the final discreteness mechanism.  For the present
parameter \(\thetaq\), Theorem~\ref{pa:thm-rectangle-contact} shows what happens if one insists
on rational coefficients, while \eqref{pa:eq-theta-field-HP-max} shows that the high-contact
coefficients naturally lie in the transcendental field \(\Q(\thetaq)\), where the norm
argument no longer exists.

Fischler and Rivoal obtain new transcendence measures for \(e^\alpha\) at algebraic arguments
by optimizing an accessory parameter in Siegel's determinant method with Hermite--Pad\'e
approximants to powers of the exponential function \cite{FischlerRivoalExp}.  Their work is a
strong demonstration that auxiliary-parameter optimization can materially improve exponents.
It does not apply directly here: a hypothetical counterexample forces the relevant exponent
and logarithmic products into the transcendental, bilinear regime rather than the algebraic
argument regime.

Their 2024 work on logarithms of \(E\)-function values proves transcendence and, in rational
strict cases, quantitative irrationality for many values \cite{FischlerRivoalLogE}.  Faverjon
and Adamczewski establish Liouville-type polynomial inequalities for values of arbitrary
\(E\)-functions at nonzero algebraic points and analogous \(M\)-function results
\cite{FaverjonAdamczewski}.  These papers enlarge the class of special values admitting
quantitative lower bounds, but they do not supply lower bounds for the bilinear logarithm
relation
\[
 \log2\log A-\log3\log M=0
\]
or for rational-coefficient Hermite--Pad\'e cancellation involving the transcendental ratio
\(\thetaq\).

\subsection{Arithmetic holonomy and effective approximation on \texorpdfstring{\(\mathbb G_m\)}{Gm}}

Calegari, Dimitrov, and Tang propose arithmetic holonomy bounds yielding effective irrationality
and linear-independence measures on the projective line and the multiplicative group, with
applications to high roots of algebraic numbers and classical Diophantine equations
\cite{CalegariDimitrovTang}.  This is relevant methodologically because it packages analytic
continuation and arithmetic height into a single effective framework.  Its stated applications
still begin with algebraic data in a number field or a finitely generated algebraic
multiplicative group.  A useful future test is whether the boundary residuals
\(U_K,V_K\) can be embedded into such an algebraic holonomy problem before the transcendental
parameter is introduced.  No such embedding is presently known.

\subsection{Literature comparison table}

\begin{center}
\begin{tabularx}{0.97\textwidth}{@{}>{\raggedright\arraybackslash}p{0.24\textwidth}X X@{}}
\toprule
Source & What it supplies & Why it does not presently close the conjecture\\
\midrule
Dasgupta--Kakde & Structural conjecture and support-based route for singular logarithm matrices. & The fixed-base \(2\times2\) instance is essentially the present problem.\\
\addlinespace
...
\addlinespace
F\"urnsinn--Pannier & Effective Hermite--Pad\'e plus algebraic field norm. & High-contact coefficients and norm require an algebraic parameter; \(\thetaq\) is transcendental.\\
\addlinespace
Fischler--Rivoal & Improved transcendence measures using optimized Hermite--Pad\'e determinants; logarithms of many \(E\)-function values. & Arguments concern algebraic inputs and do not control bilinear products of logarithms.\\
\addlinespace
...
\addlinespace
Calegari--Dimitrov--Tang & Effective arithmetic holonomy and Diophantine approximation on \(\mathbb P^1\) and \(\mathbb G_m\). & Current framework remains algebraic; no fixed-base bilinear logarithm specialization is known.\\
\bottomrule
\end{tabularx}
\end{center}

\section{A focused approximation-theoretic research program}
\label{pa:sec-program}

The negative theorems above do not eliminate approximation theory.  They identify which
version could still matter.

\subsection{Priority one: arithmetic minimax in a M\"untz lattice}

For supports \(S\) adapted to the conditional grid, solve the exact optimization problem
\[
 \lambda_1(S;X):=
 \min_{0\ne c\in\Z^S}
 \left\|\sum_{(i,j)\in S}c_{ij}x^{i+j\thetaq}\right\|_{[X,2X]}.
\]
The target is \(\lambda_1(S;X)<1\) with \(|S|\) smaller than the conditional block count.  A
proof needs both an upper construction and a nonvanishing certificate for the coefficient
vector.  Real Remez output should be treated only as a guide to the active alternation set;
integer-relation or lattice reduction must be followed by rigorous interval verification and
full coefficient-height accounting.

Supports should not be chosen only by cardinality.  Three families deserve separate tests:
\begin{enumerate}
\item thin rectangles, which maximize distinct mixed frequencies for a fixed side length;
\item boundary strips, suggested by the residuals \(U_K,V_K\);
\item valuation-adapted staircases whose monomials share large powers of two or three at the
conditional nodes.
\end{enumerate}
The inherited termwise min-plus theorem predicts failure for generic shapes.  The search is
specifically for exceptional cancellation beyond that prediction.

\subsection{Priority two: rational residuals with controlled denominator polynomial}

Search directly for primitive \(A,B\in\Z[t]\) minimizing
\[
 \|B(t)t^{\thetaq}-A(t)\|_{[1,2]}
\]
subject to simultaneous bounds on \(\|B\|_{[1,2]}\), coefficient height, and local contents.
The comparison object is the exact threshold
\(3^{-T}2^{-Td}\), not the quotient error.  Candidate supports found by rational Remez or
Zolotarev-type experiments must be rejected unless their numerator residual survives this
threshold after clearing every coefficient.

A particularly sharp test is to require \(B\) to be nonvanishing on \([1,2]\) and primitive,
then compare
\[
 -\log\|Bt^{\thetaq}-A\|,
 \qquad
 dT\log2+T\log3+\log H(A,B).
\]
Only a positive asymptotic gap is relevant.  Constant-factor improvements in quotient error
are immaterial if coefficient height grows at the same exponential rate.

\subsection{Priority three: algebraic-parameter deformation}

Choose algebraic \(\alpha\) close to \(\thetaq\), construct Hermite--Pad\'e approximants over
\(K=\Q(\alpha)\), take norms, and then estimate the perturbation from \(\alpha\) to
\(\thetaq\).  The local theorem predicts the central tradeoff.  Contact of quadratic size is
available over \(K\), but the height and degree of \(\alpha\) must grow as the approximation
improves.  A viable deformation theorem would need an inequality of the form
\begin{equation}
 \text{analytic gain from contact}
 >
 \text{number-field norm cost}
 +
 \text{parameter perturbation cost}.
 \label{pa:eq-deformation-target}
\end{equation}
No current algebraic approximation theorem for \(\thetaq\) gives such a balance, but
\eqref{pa:eq-deformation-target} is precise enough for experimental optimization.

\subsection{Priority four: boundary cancellation before approximation}

Compute the primitive boundary residuals first, then approximate them, rather than approximating
\(x^{\thetaq}\) directly.  They already contain the complete interior grid factor, so any
remaining smallness is concentrated in \(O(K)\) coefficients.  If their local Newton polygons
show systematic excess, use those valuations as constraints in a generalized Remez or
Hermite--Pad\'e problem.  If not, the calculation supplies a rigorous reason to stop investing
in that branch.
...
\begin{enumerate}
\item Lemma~\ref{pa:lem-normalized-resolution} and
Lemma~\ref{pa:lem-rational-residual}, which are exact denominator-clearing identities;
\item Theorem~\ref{pa:thm-fd-lower}, using the existing higher finite-difference module;
\item the rational inequalities in Theorem~\ref{pa:thm-normalization-tax};
\item Theorem~\ref{pa:thm-contact-multiplicity}, once multivariate polynomial multiplicity and
formal power-series order are connected;
\item Theorem~\ref{pa:thm-muntz-certificate}, reusing the generalized Chebyshev-system and
exponential-polynomial zero-count interfaces already needed by the corpus.
\end{enumerate}
The Bernstein-ellipse rate can remain a named analytic interface.  The all-degree
normalization-tax theorem does not depend on it.

\section{Conclusions of the fourth continuation}
\label{pa:sec-conclusion}

The Alaoglu--Erd\H{o}s conjecture remains open.  Polynomial approximation does not presently
supply the missing proof, but the exploration resolves several ambiguities that remained in
the inherited report.

First, the most obvious uniform strategy is now closed exactly.  After normalizing a dyadic
block to \([1,2]\), a degree-\(d\) rational polynomial with denominator \(H\) must approximate
\(t^{\thetaq}\) to better than \((H3^T2^{Td})^{-1}\) before integrality can force equality.
The finite-difference lower bound proves that no degree can meet this threshold for
\(T\ge4\).  This is stronger than saying that known estimates are insufficient: the desired
inequality itself is false.

Second, avoiding normalization does not repair the zero-count method.  Direct-scale
polynomials need degree at least \(0.460384\ldots T\) by the explicit lower bound and
asymptotically \(0.623238\ldots T\) by the sharp Bernstein ellipse.  Under a hypothetical
counterexample, the exact candidate count has slope below \(1/29\).  The approximation degree
therefore exceeds the population to be contradicted by more than an order of magnitude.
Interpolating through all trivial powers saturates the zero budget, but the expanding interval
has Bernstein parameter tending exponentially to one, destroying analytic control.

Third, local Pad\'e theory over rational coefficients collapses completely.  At every positive
rational point, contact with the irrational-power curve equals ordinary algebraic multiplicity.
A \(K\times K\) rational coefficient rectangle has maximal contact \(2K-2\), while allowing
coefficients in \(\Q(\thetaq)\) restores \(K^2-1\).  The high-contact field is transcendental
and has no algebraic norm.  This exact area-to-diameter collapse explains why importing
Hermite--Pad\'e machinery from algebraic-parameter problems cannot be done formally by
substitution.

Finally, the viable approximation-theoretic object is no longer vague.  It is an integral
M\"untz polynomial or a rational numerator residual which is small in the real norm and
simultaneously favorable at every finite prime.  Theorem~\ref{pa:thm-muntz-certificate} shows
exactly what such an object would prove.  The inherited determinant ceilings and the new local
rigidity theorem show exactly where ordinary real optimization loses the arithmetic battle.
The remaining opportunity is global adelic cancellation, most plausibly in the low-degree
boundary residuals of the multiplicative grid.

\begin{resultbox}
\textbf{Bottom line.}
No complete proof was found.  The continuation contributes three exact advances: an all-degree
normalization-tax obstruction for rational polynomial approximation; a local theorem that
rational contact with \(Y=X^{\thetaq}\) equals algebraic multiplicity; and an arithmetic
M\"untz certificate dual to the semigroup determinant.  Together they retire generic
polynomial and rational Pad\'e approaches and reduce the approximation program to one precise
open task: construct an auxiliary residual whose archimedean smallness comes from genuine
adelic cancellation rather than from real minimax approximation alone.
\end{resultbox}
...
\bibitem{TrefethenApprox}
L.~N.~Trefethen,
\newblock \emph{Approximation Theory and Approximation Practice}, extended edition,
\newblock SIAM, 2019.

\bibitem{ProtasovKamalov}
V.~Yu.~Protasov and R.~Kamalov,
\newblock \emph{Chebyshev approximation by non-Chebyshev systems},
\newblock arXiv:2403.16330v3 (revised 2025).
\newblock \href{https://arxiv.org/abs/2403.16330}{arXiv:2403.16330}.

\bibitem{FuernsinnPannier}
F.~F\"urnsinn and L.~Pannier,
\newblock \emph{An effective version of the $p$-curvature conjecture for order one differential equations},
\newblock arXiv:2510.00892v2 (revised 2026).
\newblock \href{https://arxiv.org/abs/2510.00892}{arXiv:2510.00892}.

\bibitem{FischlerRivoalExp}
...
\newblock arXiv:2502.17992 (2025).
\newblock \href{https://arxiv.org/abs/2502.17992}{arXiv:2502.17992}.

\bibitem{FischlerRivoalLogE}
S.~Fischler and T.~Rivoal,
\newblock \emph{Transcendence of values of logarithms of $E$-functions},
\newblock arXiv:2409.18537 (2024).
\newblock \href{https://arxiv.org/abs/2409.18537}{arXiv:2409.18537}.

\bibitem{FaverjonAdamczewski}
C.~Faverjon and B.~Adamczewski,
\newblock \emph{Algebraic independence measures for values of $E$-functions and $M$-functions},
\newblock arXiv:2502.09999 (2025).
\newblock \href{https://arxiv.org/abs/2502.09999}{arXiv:2502.09999}.

\bibitem{CalegariDimitrovTang}
F.~Calegari, V.~Dimitrov, and Y.~Tang,
\newblock \emph{Arithmetic holonomy bounds and effective Diophantine approximation},
\newblock arXiv:2510.04156 (2025), submission for the 2026 ICM Proceedings.
\newblock \href{https://arxiv.org/abs/2510.04156}{arXiv:2510.04156}.
