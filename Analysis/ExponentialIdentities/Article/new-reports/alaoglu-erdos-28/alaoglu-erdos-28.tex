...
\]
Writing $\vtheta=\log_2 3$, so that $A=M^{\vtheta}$, we introduce moving
...
with exact sign, fresh arithmetic support, and critical decay rate $\log_3 2$ relative
to height.
...
$\log_M A=\vtheta$ has no immediate meaning in the finite cyclic group
...
dyadic supports.  It is also properly scoped: no currently cited theorem supplies the required
