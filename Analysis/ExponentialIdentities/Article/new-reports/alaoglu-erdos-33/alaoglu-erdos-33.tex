...
The input archive was parsed before this addendum was written. Its compressed SHA--256 digest is
\begin{center}
\texttt{e9f6dc782809d6e048a8782dccfd688af26f8a87e94eb87ca93e448a3211dd3c}
\end{center}
and the decompressed source contains \texttt{20,483} lines and \texttt{485} section-level headings. The role of these statistics is reproducibility: they identify the exact baseline against which ``new'' is being used here.
...
\begin{equation}\label{eq:gap-lower}
 \delta_n\ge
 \max\left\{
 \frac{\log(1+2^{-M_n})}{\log2},
 \frac{\log(1+3^{-M_n})}{\log3}
 \right\}.
\end{equation}
\item With
\[
 R(t)=\frac{3^t-1}{2^t-1},\qquad R(0)=\frac{\log3}{\log2}=\alpha,
\]
...
\[
 H_d(\log2,\log3)
 =(\log2)^d H_d\!\left(1,\frac{\log3}{\log2}\right).
\]
The polynomial $H_d(1,T)\in\QQ[T]$ is nonzero. Since $\alpha=\log3/\log2$ is transcendental, this coefficient cannot vanish. Hence the order is exactly $d$. The bidegree bound gives $d\le D_1+D_2$.
...
The curve
\[
 y=t^\alpha,\qquad \alpha=\frac{\log3}{\log2},
\]
...
\begin{center}
\begin{tabular}{@{}p{38mm}p{30mm}p{83mm}@{}}
\toprule
...
\item \texttt{JetRigidity.lean}: represent the translated polynomial by homogeneous components and prove the leading coefficient is nonzero from transcendence of $\log3/\log2$. This module needs a theorem packaging the Gelfond--Schneider consequence.
...
Second, exact auxiliary-polynomial contact has a rigid ceiling. Because $\log3/\log2$ is transcendental, the exponential curve cannot have anomalously high rational-algebraic contact at a rational point. This removes an entire class of seemingly promising ``many coefficients, many vanishing derivatives'' arguments and explains exactly why they fail.
...
\item Compute $d_j(T)=\prod_{i\ne j}(t_j-t_i)$ exactly and set $L(T)=\lcm_j|d_j(T)|$.
