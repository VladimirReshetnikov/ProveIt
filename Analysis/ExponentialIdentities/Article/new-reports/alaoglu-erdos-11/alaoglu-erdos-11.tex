...
\vspace{0.12cm}
{\LARGE Determinant and $p$-Adic Methods, and the Remaining Barrier\par}
\vspace{1.0cm}
...
follows.  The third continuation adds a finite-place audit: the real logarithm matrix of a
counterexample would be singular, while its $2$-adic and $3$-adic analogues are provably
nonsingular; it also gives an exact support lower bound for the Dasgupta--Kakde multiplicative
grid and an explicit Fermat-quotient obstruction at auxiliary primes.
\end{minipage}
\vfill
...
formulation, and the prime-log-product independence criterion.  Fifth, its $p$-adic
continuation places the real rank drop in the Matrix Coefficient framework, proves exact
support rigidity for the associated multiplicative grid, establishes nonsingularity at the
structural primes $2$ and $3$, and derives a Fermat-quotient certificate for the first local
rank-drop layer.  It closes with a prioritized research program and a formalization blueprint.
\end{abstract}
...
\clearpage
\section{Third continuation: the \texorpdfstring{$p$}{p}-adic and matrix-coefficient frontier}
\label{sec:continuation3}

\subsection{Outcome, scope, and claim ledger}

This continuation was written after a new literature search through August~21, 2026 and a
fresh audit of the exact counterexample structure, the determinant ceilings, and the first
$t$-adic cancellation layer established earlier in this report.  Its purpose is not to rename
the four-exponentials obstruction, but to place that obstruction simultaneously at the real
and finite places and to determine what a genuinely $p$-adic proof would have to transfer.
The exact problem is still open: Prasad's 2025 survey singled out this very implication as a
striking unresolved form of the Four Exponentials Conjecture
\cite{Prasad2025}.  No complete proof is claimed below.

The strongest new unconditional statements are the following.
\begin{enumerate}
\item A hypothetical primitive pair $(M,A)=(2^\beta,3^\beta)$ determines a nonzero sparse
      quadratic polynomial in the prime logarithms which vanishes at the real logarithm
      vector.  The polynomial is identically zero exactly for the ordinary integer solutions.
\item The associated Dasgupta--Kakde multiplicative grid lies on the single real curve
      $y=x^\beta$.  A generalized-Vandermonde argument proves an exact support lower bound:
      a nonzero polynomial vanishing on $X(L)$ has at least $L^2+1$ monomials.  This gives a
      self-contained specialization of the auxiliary-polynomial bottleneck in the Matrix
      Coefficient Conjecture.
\item The Iwasawa-logarithm determinants at the two structural primes satisfy
      $D_2(M,A)\ne0$ and $D_3(M,A)\ne0$ for every nonintegral counterexample.  Thus the
      real logarithm matrix would be singular while both most natural finite-place matrices
      are provably nonsingular.
\item For every auxiliary prime $p\nmid6MA$, a Fermat-quotient determinant gives the first
      residue-level obstruction to $D_p(M,A)=0$.  Exact modular experiments show that the
      strongest real near-misses found by the finite scan do not systematically satisfy this
      local obstruction.
\item Real continued-fraction errors and $p$-adic errors obey an exact wedge identity.  At
      $p=2$ and $p=3$, the numerator of a real convergent has no effect at all on one of the
      two local logarithmic errors; only divisibility of the denominator can increase its
      $p$-adic depth.
\item A quantified cascade lemma shows that any factorized tropical-cancellation proof using
      $o(T/(\log T)^2)$ active layers remains lower order relative to the determinant height
      $H_T\asymp T^{5/2}$.
\end{enumerate}

The current literature makes the boundary unusually explicit.  Dasgupta and Kakde's Matrix
Coefficient Conjecture, published in 2026, says that a singular square matrix of complex or
$p$-adic logarithms acquires a zero matrix coefficient after rational changes of bases; in
size two this is equivalent to Four Exponentials \cite{DasguptaKakde2026}.  Their proposed
proof architecture is
\[
   \text{rank deficiency}\ \Longrightarrow\
   \text{small Newton support}\ \Longrightarrow\
   \text{a multiplicative relation}.
\]
They prove the second implication, while the first remains the missing step.  The
specialization below shows exactly why that first implication would prove the present
conjecture and exactly how sharp its support threshold is.

\begin{center}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.42\textwidth}
>{\raggedright\arraybackslash}p{0.18\textwidth}X@{}}
\toprule
Statement & Status & Principal input \\
\midrule
Sparse prime-log quadratic obstruction & \statusnew & unique factorization \\
No rational matrix coefficient can vanish & \statusnew & irrationality of $\beta$ and $\log2/\log3$ \\
Grid support lower bound $|\supp P|\ge L^2+1$ & \statusnew & generalized Vandermonde positivity \\
Failure of the Dasgupta--Kakde conditions (m) and (m$'$) for a counterexample & \statusnew & support bound and Pick's theorem \\
$D_2(M,A)\ne0$ and $D_3(M,A)\ne0$ & \statusnew & kernel of the Iwasawa logarithm on $\Q_{>0}$ \\
$D_p(M,A)\ne0$ for all $p\nmid6MA$ & conditional & $p$-adic Matrix Coefficient/Four Exponentials \\
Fermat-quotient first-layer certificate & \statusnew & $p$-adic logarithm modulo $p^2$ \\
Near-miss prime scan through $50{,}000$ & software experiment & exact modular arithmetic \\
No shallow factorized cancellation cascade & conditional proposition & existing two-logarithm valuation bounds \\
Complete Alaoglu--Erd\H{o}s conjecture & open & would follow from real Matrix Coefficient/Four Exponentials \\
\bottomrule
\end{tabularx}
\end{center}

\subsection{The real logarithm matrix and a sparse prime-log polynomial}
\label{sec:c3-real-matrix}

Assume in this section that the conjecture fails and use the least nonintegral generator from
\eqref{eq:exact-structure}:
\[
  \beta\notin\Q,\qquad M=2^\beta\in\N,\qquad A=3^\beta\in\N.
\]
The relation is the rank drop
\begin{equation}\label{eq:c3-real-matrix}
 L_\infty(M,A):=
 \begin{pmatrix}
   \log2&\log M\\
   \log3&\log A
 ...
 \det L_\infty(M,A)=0.
\end{equation}
The two rows are $\Q$-independent, and the two columns are $\Q$-independent.  Row
independence is unique factorization.  For the columns, the counterexample identities give
$(\log M,\log A)^t=\beta(\log2,\log3)^t$, and a rational dependence would therefore
force $\beta\in\Q$, contrary to the least-generator construction.  Thus
\eqref{eq:c3-real-matrix} is precisely a size-two Four Exponentials configuration rather than
a degenerate one.

Let $\mathcal P$ be the finite set of primes dividing $6MA$, and put
\[
  m_q=v_q(M),\qquad a_q=v_q(A)\qquad(q\in\mathcal P).
\]
Introduce independent variables $X_q$ indexed by $\mathcal P$ and the homogeneous quadratic
\begin{equation}\label{eq:c3-prime-polynomial}
 \mathcal F_{M,A}(X)
  :=\left(\sum_{q\in\mathcal P}m_qX_q\right)X_3
    -\left(\sum_{q\in\mathcal P}a_qX_q\right)X_2.
\end{equation}

\begin{theorem}[Sparse prime-log obstruction]\label{thm:c3-sparse-quadratic}
For every positive integer pair $(M,A)$ satisfying
$\log M\log3=\log A\log2$ one has
\begin{equation}\label{eq:c3-F-real-zero}
 \mathcal F_{M,A}\bigl((\log q)_{q\in\mathcal P}\bigr)=0.
\end{equation}
Moreover, $\mathcal F_{M,A}$ is the zero polynomial if and only if
\[
  (M,A)=(2^n,3^n)
\]
for some $n\in\Nzero$.  Consequently every nonintegral counterexample gives a
\emph{nonzero} rational quadratic polynomial, supported on at most
$2|\mathcal P|-1$ monomials, which vanishes at a vector of logarithms of distinct primes.
\end{theorem}

\begin{proof}
Evaluation at $X_q=\log q$ turns the two linear factors into $\log M$ and $\log A$, so
\eqref{eq:c3-F-real-zero} is the assumed determinant relation.  To identify when the
polynomial itself vanishes, inspect its coefficients.  The coefficient of $X_2^2$ is
$-a_2$, that of $X_3^2$ is $m_3$, and for $q\notin\{2,3\}$ the coefficients of
$X_qX_3$ and $X_qX_2$ are $m_q$ and $-a_q$.  The remaining coefficient of $X_2X_3$
is $m_2-a_3$.  Hence $\mathcal F_{M,A}\equiv0$ forces
\[
 a_2=m_3=0,\qquad m_q=a_q=0\ (q\notin\{2,3\}),\qquad m_2=a_3.
\]
Thus $M=2^n$ and $A=3^n$ with $n=m_2=a_3$.  The converse is immediate.
\end{proof}

\begin{corollary}[A very small Schanuel target]\label{cor:c3-schanuel-target}
It would suffice to prove that no nonzero polynomial of the special form
\eqref{eq:c3-prime-polynomial} vanishes at the corresponding prime-log vector.  In particular,
Schanuel's conjecture implies the Alaoglu--Erd\H{o}s conjecture, because the logarithms of
distinct primes are $\Q$-linearly independent and hence would be algebraically independent.
\end{corollary}

The point of Theorem~\ref{thm:c3-sparse-quadratic} is not to replace the product-of-logarithms
criterion already isolated in Section~\ref{sec:logproduct}.  It records the exact polynomial
shape forced by integrality.  Generic quadratic algebraic independence is far more than is
needed: only one star-shaped family of coefficients, with two distinguished variables
$X_2,X_3$ and nonnegative valuation data, must be excluded.

\subsection{The matrix-coefficient obstruction is multiplicative and exact}
\label{sec:c3-matrix-coefficient}

For vectors $r=(r_1,r_2)$ and $s=(s_1,s_2)$ in $\Q^2$, define the outer-product monomial
\begin{equation}\label{eq:c3-Pi}
 \Pi_{M,A}(r,s)
 :=2^{r_1s_1}M^{r_1s_2}3^{r_2s_1}A^{r_2s_2},
\end{equation}
interpreted through the positive real branches.  Clearing denominators replaces
\eqref{eq:c3-Pi} by an ordinary rational monomial with integer exponents.  Its real logarithm
factors completely:
\begin{equation}\label{eq:c3-Pi-factor}
 \log\Pi_{M,A}(r,s)
  =(s_1+\beta s_2)(r_1\log2+r_2\log3).
\end{equation}

\begin{proposition}[No vanishing rational matrix coefficient]
\label{prop:c3-no-coefficient}
If $\beta$ is nonintegral and $M=2^\beta$, $A=3^\beta$ are integers, then
\[
 \Pi_{M,A}(r,s)\ne1
\]
for all nonzero $r,s\in\Q^2$.  Equivalently, no nonzero rational vectors satisfy
\[
 r^{\mathsf T}L_\infty(M,A)s=0.
\]
\end{proposition}

\begin{proof}
The first factor on the right of \eqref{eq:c3-Pi-factor} cannot vanish for nonzero $s$:
otherwise $s_2\ne0$ and $\beta=-s_1/s_2\in\Q$.  The second cannot vanish for nonzero $r$,
because after clearing denominators it would give $2^u3^v=1$ with $(u,v)\ne(0,0)$.
Thus the logarithm is nonzero.
\end{proof}

Dasgupta and Kakde's Matrix Coefficient Conjecture says that a singular square matrix with
entries in the complex or $p$-adic logarithm space has nonzero rational vectors $r,s$ for
which the corresponding matrix coefficient vanishes \cite{DasguptaKakde2026}.  In dimension
two it is equivalent to Four Exponentials.  Proposition~\ref{prop:c3-no-coefficient} therefore
gives the following particularly transparent reduction.

\begin{corollary}[Matrix Coefficient implies Alaoglu--Erd\H{o}s]
\label{cor:c3-MCC-implies-AE}
The complex Matrix Coefficient Conjecture for the matrix \eqref{eq:c3-real-matrix} implies
Conjecture~\ref{conj:AE}.
\end{corollary}

\begin{proof}
A counterexample makes the matrix singular, so Matrix Coefficient would produce a vanishing
rational coefficient.  Proposition~\ref{prop:c3-no-coefficient} forbids one.
\end{proof}

This formulation is stronger diagnostically than the bare statement ``Four Exponentials would
solve the problem.''  It identifies the impossible output of the conjecture: not merely a
new algebraic exponential, but an explicit rank-one multiplicative relation among the four
integers $2,M,3,A$.

\subsection{The Dasgupta--Kakde grid specializes to one transcendental curve}
\label{sec:c3-DK-grid}

Write the two row-vectors of algebraic numbers as
\[
 x_1=(2,M),\qquad x_2=(3,A).
\]
In the notation of Dasgupta--Kakde, the finite multiplicative grid is
\begin{equation}\label{eq:c3-grid}
 X(L)=\{x_1^ux_2^v:0\le u,v<L\}
 =\{(2^u3^v,M^uA^v):0\le u,v<L\}.
\end{equation}
Under the counterexample relation this becomes
\begin{equation}\label{eq:c3-grid-curve}
 X(L)=\{(z,z^\beta):z=2^u3^v,\ 0\le u,v<L\}.
\end{equation}
All $L^2$ first coordinates are distinct by unique factorization.

For a polynomial $P(T_1,T_2)$, let $\supp(P)$ be its set of exponent pairs with nonzero
coefficient.  Multiplying by a monomial allows the same discussion for Laurent polynomials.

\begin{theorem}[Exact support rigidity on the counterexample grid]
\label{thm:c3-support-rigidity}
Let $P\in\C[T_1,T_2]$ be nonzero and suppose that it vanishes at every point of $X(L)$.
Then
\begin{equation}\label{eq:c3-support-lower}
  |\supp(P)|\ge L^2+1.
\end{equation}
The same conclusion holds for Laurent polynomials.
\end{theorem}

\begin{proof}
Write
\[
 P(T_1,T_2)=\sum_{(i,j)\in S}c_{ij}T_1^iT_2^j,
 \qquad S=\supp(P),\quad s=|S|.
\]
On the curve $T_2=T_1^\beta$ this restricts to
\[
 f(z)=\sum_{(i,j)\in S}c_{ij}z^{i+\beta j}.
\]
The exponents $i+\beta j$ are pairwise distinct: equality for two integer pairs would make
$\beta$ rational.  If $f$ vanished at $s$ distinct positive points
$0<z_1<\cdots<z_s$, the coefficient vector would lie in the kernel of the generalized
Vandermonde matrix
\[
  (z_k^{\lambda_h})_{1\le k,h\le s},
  \qquad \lambda_1<\cdots<\lambda_s.
\]
That determinant is nonzero (indeed strictly positive after the usual ordering), by the
generalized-Vandermonde theorem proved earlier in this report.  Hence a nonzero $s$-term
restriction has at most $s-1$ distinct positive zeros.  The grid supplies $L^2$ such zeros,
so $s\ge L^2+1$.
\end{proof}

The support threshold in Dasgupta--Kakde's condition (m$'$) is $N^2$ for a polynomial
vanishing on $X(2N)$.  Theorem~\ref{thm:c3-support-rigidity} says that in the present
specialization any such polynomial requires at least $4N^2+1$ monomials.  Thus condition
(m$'$) is not merely unavailable: it is false under the counterexample hypothesis.

Their stronger geometric condition (m) asks, for large $N$, for a polynomial vanishing on
$X(2N)$ whose Bernstein--Kushnirenko degree is less than $N^2$.  In two variables this means
both
\begin{equation}\label{eq:c3-m-condition}
 \deg P<N^2,
 \qquad
 2\operatorname{Area}(\operatorname{conv}\supp P)<N^2.
\end{equation}
The same grid rigidity also excludes this condition directly.

\begin{proposition}[The critical Newton polygon is too small]
\label{prop:c3-m-fails}
For a counterexample matrix, no nonzero polynomial satisfying \eqref{eq:c3-m-condition} can
vanish on $X(2N)$.
\end{proposition}

\begin{proof}
Let $\Delta$ be the convex hull of the support and put $d=\deg P<N^2$.  If $\Delta$ is a
point or a segment, its lattice points number at most $2d+1<4N^2+1$, because the total
horizontal-plus-vertical displacement between its endpoints is at most $2d$; this already
contradicts Theorem~\ref{thm:c3-support-rigidity}.  We may therefore assume that $\Delta$ is a
two-dimensional lattice polygon, with area $A_\Delta$ and $B_\Delta$ boundary lattice points.
It lies in the degree triangle $x\ge0$, $y\ge0$, $x+y\le d$.  Along the boundary of any convex polygon,
the sum of $|\Delta x|$ is twice its horizontal width and the sum of $|\Delta y|$ is twice its
vertical width.  Since the lattice length of an edge is at most $|\Delta x|+|\Delta y|$,
\[
 B_\Delta\le2(\operatorname{width}_x\Delta+\operatorname{width}_y\Delta)\le4d.
\]
Pick's theorem therefore gives
\[
 |\Delta\cap\Z^2|
 =A_\Delta+\frac{B_\Delta}{2}+1
 <\frac{N^2}{2}+2N^2+1
 =\frac52N^2+1.
\]
Hence $|\supp P|<\frac52N^2+1<4N^2+1$ for every $N\ge1$, contradicting
Theorem~\ref{thm:c3-support-rigidity} with $L=2N$.
\end{proof}

\begin{statusbox}{\statusnew{} --- Theorem~\ref{thm:c3-support-rigidity} and
Proposition~\ref{prop:c3-m-fails} are self-contained specializations.  They do not prove the
conjecture.  Instead they isolate the exact missing implication in the Dasgupta--Kakde
program: rank deficiency would have to \emph{construct} a polynomial whose existence the
one-dimensional grid geometry then rules out.}
\end{statusbox}

This explains why the classical Waldschmidt--Masser theorem does not already close the
problem.  For a $2\times2$ matrix its rank hypothesis is strictly below one, whereas the
counterexample matrix has rank exactly one.  The desired auxiliary polynomial lies precisely
at the critical rank where the standard dimension count loses one full power of $N$.
Dasgupta's 2023 exposition gives the common complex/$p$-adic structural-rank framework and the
Waldschmidt--Masser bound \cite{Dasgupta2023}; Dasgupta--Kakde replace ordinary degree by
Newton support, but the critical construction remains open.

\subsection{Iwasawa logarithms and the finite-place determinant}
\label{sec:c3-local-determinant}

For every prime $p$, let $\log_p:\Q_p^*\to\Q_p$ be the Iwasawa logarithm, normalized by
$\log_p p=0$.  Define
\begin{equation}\label{eq:c3-Dp}
 L_p(M,A)=
 \begin{pmatrix}
   \log_p2&\log_pM\\
   \log_p3&\log_pA
 ...
 \qquad
 D_p(M,A):=-\det L_p(M,A)
 =\log_pM\,\log_p3-\log_pA\,\log_p2.
\end{equation}
Equivalently,
\begin{equation}\label{eq:c3-Fp-eval}
 D_p(M,A)=
 \mathcal F_{M,A}\bigl((\log_pq)_{q\in\mathcal P}\bigr).
\end{equation}
The formal resemblance to \eqref{eq:c3-F-real-zero} is exact, but there is no transfer
principle forcing the two evaluations to vanish together.

We use the elementary kernel description
\begin{equation}\label{eq:c3-Iwasawa-kernel}
 \log_p u=0\quad(u\in\Q_{>0})
 \qquad\Longleftrightarrow\qquad
 u=p^k\quad\text{for some }k\in\Z.
\end{equation}
Indeed, the kernel on $\Q_p^*$ is generated by the uniformizer and roots of unity; the only
positive rational root of unity is $1$.  This rational kernel is also a direct consequence of
the Baker--Brumer theorem in its standard algebraic formulation.

\begin{theorem}[The structural primes are locally transverse]
\label{thm:c3-base-primes}
For every nonintegral counterexample pair $(M,A)$,
\begin{align}
 D_2(M,A)&=\log_2M\,\log_23\ne0,\label{eq:c3-D2}\\
 D_3(M,A)&=-\log_3A\,\log_32\ne0.\label{eq:c3-D3}
\end{align}
In particular, $L_\infty(M,A)$ has rank one while both $L_2(M,A)$ and $L_3(M,A)$ have rank
two.
\end{theorem}

\begin{proof}
Since $\log_22=0$, the determinant at two reduces to the product in
\eqref{eq:c3-D2}.  The factor $\log_23$ is nonzero by
\eqref{eq:c3-Iwasawa-kernel}.  The factor $\log_2M$ would vanish only if $M$ were a power of
two, which would force $\beta=\log_2M\in\Z$.  Thus it is nonzero.  The proof at three is
symmetric.
\end{proof}

This is the most concrete finite-place conclusion of the continuation.  A counterexample does
not produce the same rank drop at the primes built into its definition; it produces the
opposite phenomenon.  Any adelic proof must therefore manufacture a mechanism that compares
a real zero with finite nonzeros, rather than hope that the real relation simply persists
$p$-adically.

For auxiliary primes the expected statement is stronger but presently conditional.

\begin{proposition}[Conditional nonsingularity at every auxiliary prime]
\label{prop:c3-aux-MCC}
Let $p\nmid6MA$.  The $p$-adic Matrix Coefficient Conjecture for $L_p(M,A)$ implies
\[
 D_p(M,A)\ne0.
\]
Consequently the $p$-adic Four Exponentials Conjecture implies nonsingularity for every such
$p$.
\end{proposition}

\begin{proof}
If $D_p=0$, Matrix Coefficient gives nonzero $r,s\in\Q^2$ with
$r^{\mathsf T}L_p s=0$.  Multiply $r$ and $s$ by separate common denominators.  We obtain
nonzero integer vectors $u,v$ such that the $p$-adic logarithm of
\[
  2^{u_1v_1}M^{u_1v_2}3^{u_2v_1}A^{u_2v_2}
\]
vanishes.  The displayed number is a positive rational $p$-adic unit, so
\eqref{eq:c3-Iwasawa-kernel} makes it equal to $1$.  This contradicts
Proposition~\ref{prop:c3-no-coefficient}.
\end{proof}

Recent arithmetic applications confirm that this finite-place boundary is active rather than
artificial.  Betina, Maksoud, and Pozzi use matrices of $p$-adic logarithms in the geometry of
the eigencurve and show that relevant regulator hypotheses follow in specified cases from a
weak $p$-adic Schanuel conjecture or from a $p$-adic Four Exponentials Conjecture
\cite{BetinaMaksoudPozzi2026}.  Their setting is much richer, but the logical obstruction is
the same: present $p$-adic transcendence theorems control algebraic linear coefficients, not
determinants whose cofactors are themselves logarithms.

\subsection{The local closure of the cyclic subgroup}
\label{sec:c3-local-closure}

The determinant $D_p$ has a direct dynamical meaning.  Let
\[
 \Gamma_p=\overline{\{(2^n,3^n):n\in\Z\}}
 \subset(\Q_p^*)^2.
\]

\begin{proposition}[Local closure forces local rank drop]
\label{prop:c3-closure-implies-D}
For every prime $p$, if $(M,A)\in\Gamma_p$, then $D_p(M,A)=0$.
Consequently a nonintegral counterexample pair lies in neither $\Gamma_2$ nor $\Gamma_3$.
\end{proposition}

\begin{proof}
Choose integers $n_j$ with $(2^{n_j},3^{n_j})\to(M,A)$.  Continuity of the Iwasawa logarithm
gives
\[
 n_j\log_p2\to\log_pM,
 \qquad
 n_j\log_p3\to\log_pA.
\]
Taking the cross product yields $D_p=0$.  The last assertion is
Theorem~\ref{thm:c3-base-primes}.
\end{proof}

For $p\nmid6MA$ one can make the closure criterion exact.  Write a $p$-adic unit as
$u=\omega_p(u)\langle u\rangle$, where $\omega_p(u)$ is its Teichm\"uller component and
$\langle u\rangle\in1+p\Z_p$ (with the standard harmless modification at $p=2$).  Let $h_p$
be the order of the torsion pair $(\omega_p(2),\omega_p(3))$.

\begin{proposition}[Exact auxiliary-prime closure criterion]
\label{prop:c3-exact-closure}
Assume $p\nmid6MA$.  Then $(M,A)\in\Gamma_p$ if and only if there is a residue
$r\in\{0,\ldots,h_p-1\}$ and a $t\in\Z_p$ such that
\begin{align}
 (\omega_p(M),\omega_p(A))
   &=(\omega_p(2)^r,\omega_p(3)^r),\label{eq:c3-torsion-match}\\
 \log_pM&=(r+h_pt)\log_p2,
 &\log_pA&=(r+h_pt)\log_p3.
 \label{eq:c3-principal-match}
\end{align}
In particular, local closure requires both a finite residue-class match and the tangent
condition $D_p=0$.
\end{proposition}

\begin{proof}
Split the integer sequence $n$ into residue classes modulo $h_p$.  In the class
$n=r+h_pk$, the Teichm\"uller pair is constant, while $k\in\Z$ is dense in $\Z_p$ and the
principal-unit powers extend continuously to $k=t\in\Z_p$ through the $p$-adic exponential.
This gives the displayed parametrization of the closure of each residue class, and the union
of those finitely many parametrizations is $\Gamma_p$.
\end{proof}

The exact research target can now be stated without metaphor.

\begin{target}[A place-transfer target]
\label{target:c3-place-transfer}
Prove that the real collinearity
\[
 (\log M,\log A)=\beta(\log2,\log3)
\]
together with integrality forces $(M,A)$ into $\Gamma_p$ for at least one prime $p$ for which
$D_p(M,A)$ is known to be nonzero.  The structural primes $2$ and $3$ would give an immediate
unconditional contradiction.  No present theorem supplies this transfer, and
Proposition~\ref{prop:c3-exact-closure} shows that it must control both torsion residues and a
$p$-adic tangent parameter.
\end{target}

Prasad's real/$p$-adic closure perspective for finitely generated subgroups of algebraic
groups is closely aligned with this formulation \cite{Prasad2025}.  It does not presently
imply the target: the required bridge compares closures at different places for one highly
special point, while the available conjectures describe each closure internally.

\subsection{Fermat quotients: the first local rank-drop layer}
\label{sec:c3-fermat}

For an odd prime $p\nmid u$, define the Fermat quotient
\begin{equation}\label{eq:c3-fermat-quotient}
 q_p(u)=\frac{u^{p-1}-1}{p}\pmod p.
\end{equation}

\begin{lemma}[Iwasawa logarithm modulo $p^2$]
\label{lem:c3-log-fermat}
For odd $p\nmid u$,
\begin{equation}\label{eq:c3-log-fermat}
 \frac{\log_pu}{p}\equiv-q_p(u)\pmod p.
\end{equation}
\end{lemma}

\begin{proof}
Write $u^{p-1}=1+pq_p(u)$ modulo $p^2$.  Then
\[
 (p-1)\log_pu=\log_p(u^{p-1})\equiv pq_p(u)\pmod{p^2}.
\]
After division by $p$, use $(p-1)^{-1}\equiv-1\pmod p$.
\end{proof}

\begin{theorem}[Fermat-quotient determinant certificate]
\label{thm:c3-fermat-determinant}
Let $p\ge5$ and $p\nmid6MA$.  Put
\begin{equation}\label{eq:c3-Delta-p}
 \Delta_p(M,A)
 :=q_p(M)q_p(3)-q_p(A)q_p(2)\pmod p.
\end{equation}
Then
\begin{equation}\label{eq:c3-Dp-modp}
...
\[
 ...
 v_p(D_p(M,A))=2.
\]
Every integer-exponent pair $(M,A)=(2^n,3^n)$ has
$\Delta_p(M,A)=0$ for every $p\ge5$.
\end{theorem}

\begin{proof}
Apply Lemma~\ref{lem:c3-log-fermat} to the four entries of
\eqref{eq:c3-Dp}; the two minus signs cancel in the determinant.  The last assertion follows
from
$q_p(u^n)\equiv n q_p(u)\pmod p$.
\end{proof}

Thus a local rank drop is a generalized Wieferich phenomenon.  It must first pass
\eqref{eq:c3-Delta-p}, then an infinite tower of congruences modulo higher powers of $p$.
The first layer is especially inexpensive to test and provides a clean way to compare real
near-integrality with finite-place alignment.

\subsubsection*{Exact modular reconnaissance}

The two strongest real near-misses in the directed-rounding scan are
\begin{align*}
 (M_-,A_-)&=(132{,}833{,}765,\ 7{,}501{,}348{,}219{,}569),\\
 (M_+,A_+)&=(129{,}799{,}345,\ 7{,}231{,}571{,}292{,}851).
\end{align*}
They are not counterexamples; they are used only to test whether exceptional real proximity
tends to create the first local rank-drop congruence.  The following exact-integer program
examines every eligible prime through $50{,}000$.

\begin{lstlisting}[style=pythonstyle,caption={Fermat-quotient determinant scan.}]
def primes_up_to(n: int) -> list[int]:
    sieve = bytearray(b"\x01") * (n + 1)
    sieve[:2] = b"\x00\x00"
    for r in range(2, int(n**0.5) + 1):
        if sieve[r]:
            sieve[r*r:n+1:r] = b"\x00" * (((n-r*r)//r) + 1)
    return [r for r in range(2, n + 1) if sieve[r]]
...
def delta(M: int, A: int, p: int) -> int:
    return (fermat_quotient(M, p) * fermat_quotient(3, p)
            - fermat_quotient(A, p) * fermat_quotient(2, p)) % p


PAIRS = [
    ("lower near-miss", 132_833_765, 7_501_348_219_569),
    ("upper near-miss", 129_799_345, 7_231_571_292_851),
    ("integer control 17", 2**17, 3**17),
    ("integer control 31", 2**31, 3**31),
]

for label, M, A in PAIRS:
    eligible = [p for p in primes_up_to(50_000)
                if p >= 5 and M % p and A % p]
    zeros = [p for p in eligible if delta(M, A, p) == 0]
    print(label, len(eligible), zeros)
\end{lstlisting}
...
\begin{tabular}{lrrl}
\toprule
Pair & Eligible primes & $\Delta_p=0$ & Exceptional primes \\
\midrule
Lower real record $(M_-,A_-)$ & $5{,}130$ & $2$ & $167,\ 27077$ \\
Upper real record $(M_+,A_+)$ & $5{,}126$ & $0$ & none \\
Control $(2^{17},3^{17})$ & $5{,}131$ & $5{,}131$ & every eligible prime \\
Control $(2^{31},3^{31})$ & $5{,}131$ & $5{,}131$ & every eligible prime \\
\bottomrule
\end{tabular}
\end{center}

At the two exceptional primes for the lower record, a second exact expansion gives
$v_p(D_p)=3$, so the congruence fails at the next level.  To verify this, compute
$\log_p(u)/p$ modulo $p^2$ from
\begin{equation}\label{eq:c3-second-level}
 \frac{\log_pu}{p}
 \equiv(p-1)^{-1}\left(Q-\frac p2Q^2\right)\pmod{p^2},
 \qquad
 Q=\frac{u^{p-1}-1}{p}\pmod{p^2}.
\end{equation}
More precisely, the residues modulo $p^2$ are
\begin{align*}
 \frac{D_{167}(M_-,A_-)}{167^2}
   &\equiv 5678=34\cdot167\pmod{167^2},\\
 \frac{D_{27077}(M_-,A_-)}{27077^2}
   &\equiv 289453130=10690\cdot27077\pmod{27077^2}.
\end{align*}
Thus $D_p/p^2$ is divisible by $p$ but not by $p^2$ at both exceptional primes.

\begin{statusbox}{Software-level exact modular experiment.  The computation is not evidence
that a counterexample cannot have exceptional primes, and no probabilistic model is used in a
proof.  Its limited conclusion is negative but useful: exceptional archimedean proximity does
not automatically produce even the first $p$-adic rank-drop layer.}
\end{statusbox}

\subsection{Continued fractions versus finite-place errors}
\label{sec:c3-convergents}

Let $r/s$ be any rational approximation to $\beta$, with $s>0$, and put
\[
 \varepsilon=s\beta-r.
\]
The two rational multiplicative errors are
\begin{equation}\label{eq:c3-ratios}
 R_2(r,s)=\frac{M^s}{2^r},
 \qquad
 R_3(r,s)=\frac{A^s}{3^r}.
\end{equation}
At the real place they are perfectly synchronized:
\begin{equation}\label{eq:c3-real-errors}
 \log R_2(r,s)=\varepsilon\log2,
 \qquad
 \log R_3(r,s)=\varepsilon\log3.
\end{equation}
Thus convergents make both ratios close to one archimedeanly.

At a finite prime $p$, define the logarithmic error vector
\begin{equation}\label{eq:c3-Ep}
 E_p(r,s)=
 \bigl(s\log_pM-r\log_p2,\ s\log_pA-r\log_p3\bigr).
\end{equation}

\begin{theorem}[Exact adelic wedge identity]
\label{thm:c3-wedge}
For every prime $p$ and integers $r,s$,
\begin{equation}\label{eq:c3-wedge}
 E_{p,1}(r,s)\log_p3-E_{p,2}(r,s)\log_p2
 =sD_p(M,A).
\end{equation}
At the structural primes this specializes to
\begin{align}
 \log_2R_2(r,s)&=s\log_2M,
 &v_2(\log_2R_2(r,s))&=v_2(s)+v_2(\log_2M),
 \label{eq:c3-two-error}\\
 \log_3R_3(r,s)&=s\log_3A,
 &v_3(\log_3R_3(r,s))&=v_3(s)+v_3(\log_3A).
 \label{eq:c3-three-error}
\end{align}
\end{theorem}

\begin{proof}
Expand the left side of \eqref{eq:c3-wedge}.  The terms containing $r$ cancel, leaving
$s(\log_pM\log_p3-\log_pA\log_p2)$.  Equations
\eqref{eq:c3-two-error} and \eqref{eq:c3-three-error} use $\log_22=0$ and $\log_33=0$,
followed by additivity of the valuation.
\end{proof}

The theorem gives a precise anti-transfer statement.  Improving the real approximation by
choosing a better numerator $r$ has no effect whatever on the $2$-adic depth of $R_2$ or the
$3$-adic depth of $R_3$.  Those depths increase only when the denominator $s$ contains high
powers of $2$ or $3$.  A continued-fraction argument would therefore need strong, simultaneous
arithmetic control of the denominators, not merely the usual estimate
$|s\beta-r|\ll1/s$.

The synchronized block nodes from Section~\ref{sec:sturmian} make this explicit.  Put
\[
 m_k=2^{T-\lfloor k\beta\rfloor}M^k,
 \qquad
 a_k=3^{T-\lfloor k\beta\rfloor}A^k.
\]
Whenever $k,k+s,\ldots,k+ts$ stay in a no-wrap interval satisfying
\[
 \lfloor(k+js)\beta\rfloor=\lfloor k\beta\rfloor+jr
 \qquad(0\le j\le t),
\]
one has the exact geometric progression
\begin{equation}\label{eq:c3-no-wrap}
 m_{k+js}=m_kR_2(r,s)^j,
 \qquad
 a_{k+js}=a_kR_3(r,s)^j.
\end{equation}
For an auxiliary prime $p\nmid6MA$, restricting $j$ to one residue class modulo $p-1$
places suitable powers of $R_2,R_3$ in $1+p\Z_p$, so the progression becomes a
$p$-adic analytic exponential curve.  A monomial $x^uy^v$ restricts to a geometric sequence
with ratio
\[
  R_2(r,s)^{(p-1)u}R_3(r,s)^{(p-1)v}.
\]
The resulting interpolation determinant is a generalized Vandermonde.  Pairwise valuation
gains are governed by nonzero forms
\[
 (p-1)\bigl(\Delta u\log_pR_2(r,s)+\Delta v\log_pR_3(r,s)\bigr),
\]
which current $p$-adic logarithmic-form estimates bound only polylogarithmically in the
integer coefficients.  Thus local analyticity reorganizes the determinant but does not create
the missing leading-order divisor.

\subsection{How deep would a successful cancellation cascade have to be?}
\label{sec:c3-cascade-depth}

The first tied layer in Section~\ref{sec:firstlayer} factors into
$O(T^{3/2})$ two-logarithm differences, each of valuation
$O((\log T)^2)$, and therefore contributes only
$O(T^{3/2}(\log T)^2)$ against $H_T\asymp T^{5/2}$.  The following conditional bookkeeping
statement quantifies how many similarly factorized layers would be needed.

\begin{proposition}[Necessary depth of a factorized cascade]
\label{prop:c3-cascade-depth}
Suppose that each of the first $J(T)$ active tropical cancellation layers can increase the
running determinant valuation by no more than the valuation of a product of at most
$CT^{3/2}$ nonzero fixed-base logarithmic factors, and suppose each factor has valuation at
most $C'(\log T)^2$.  Then the cumulative valuation exposed by those layers is
\begin{equation}\label{eq:c3-cascade-bound}
 O\bigl(J(T)T^{3/2}(\log T)^2\bigr).
\end{equation}
Consequently, if
\[
 J(T)=o\left(\frac{T}{(\log T)^2}\right),
\]
those layers contribute $o(H_T)$.  Any proof of a leading-order excess within this factorized
model requires
\begin{equation}\label{eq:c3-cascade-necessary}
 J(T)=\Omega\left(\frac{T}{(\log T)^2}\right)
\end{equation}
genuinely active layers.
\end{proposition}

\begin{proof}
Sum the assumed per-factor bound over the $CT^{3/2}$ factors and then over the $J(T)$ layers.
Divide by $H_T\asymp T^{5/2}$.
\end{proof}

\begin{warning}
Proposition~\ref{prop:c3-cascade-depth} is not a bound on arbitrary cancellation.  A higher
initial form might possess a global determinant identity that does not factor into pairwise
fixed-base logarithmic forms.  The proposition says that repeatedly applying the mechanism of
the first tied layer cannot work unless it persists through almost linearly many layers.
That is a much stronger structural demand than ``find one more congruence.''
\end{warning}

A useful computational project is therefore not merely to measure $v_p(D_T)$, but to compute
the tropical associated graded determinant layer by layer and test whether a stable global
identity emerges.  The exact threshold \eqref{eq:c3-cascade-necessary} provides a scale against
which such experiments can be judged.

\subsection{An algebraic-integer Kummer determinant and why its norm does not close}
\label{sec:c3-kummer}

One might try to convert the real determinant zero into a small nonzero algebraic integer and
then invoke the product formula.  The simplest version already reveals the obstruction.
For $N\ge1$, choose the positive real $N$th roots and set
\begin{equation}\label{eq:c3-Xi}
 \Xi_N=
 (M^{1/N}-1)(3^{1/N}-1)
 -(A^{1/N}-1)(2^{1/N}-1).
\end{equation}
Each $N$th root is an algebraic integer, hence so is $\Xi_N$.

\begin{proposition}[Third-order Kummer cancellation]
\label{prop:c3-kummer-asymptotic}
For a nonintegral counterexample,
\begin{equation}\label{eq:c3-Xi-asymptotic}
 \Xi_N=
 \frac{\beta\log2\log3\,(\beta-1)(\log2-\log3)}{2N^3}
 +O_{M,A}(N^{-4}).
\end{equation}
In particular $\Xi_N\ne0$ for all sufficiently large $N$ and
$|\Xi_N|\asymp_{M,A}N^{-3}$ in the distinguished real embedding.
\end{proposition}

\begin{proof}
For fixed $z$,
\[
 e^{z/N}-1=\frac zN+\frac{z^2}{2N^2}+O_z(N^{-3}).
\]
Writing $a=\log M=\beta\log2$, $b=\log3$, $c=\log A=\beta\log3$, and $d=\log2$, the
$N^{-2}$ coefficient in \eqref{eq:c3-Xi} is $ab-cd=0$.  The $N^{-3}$ coefficient is
\[
 \frac12\{ab(a+b)-cd(c+d)\}
 =\frac{ab}{2}\{(a+b)-(c+d)\}.
\]
Now
$(a+b)-(c+d)=(\beta-1)(\log2-\log3)$, which is nonzero because a counterexample has
$\beta\ne1$.
\end{proof}

If $K_N$ contains the four radicals, then a nonzero $\Xi_N$ has nonzero integral norm, so
$|N_{K_N/\Q}(\Xi_N)|\ge1$.  The distinguished conjugate is only polynomially small, while
$[K_N:\Q]$ grows with $N$ and the other conjugates replace the positive roots by independent
root-of-unity twists.  For those embeddings the leading determinant cancellation normally
disappears, and a crude uniform bound greater than one raised to the growing degree overwhelms
$N^{-3}$.  Thus the norm argument has the wrong sign of complexity: adjoining more radicals
creates many large conjugates faster than the real embedding becomes small.

This failed construction is nevertheless informative.  A viable adelic algebraic-integer
argument would need either exponentially small size at one place or simultaneous smallness at
a positive proportion of the Kummer embeddings.  The determinant relation supplies neither
without an additional symmetry correlating the root-of-unity twists.

\subsection{Recent literature and what changed after the earlier survey}
\label{sec:c3-literature}

The literature search was centered on arXiv, journal metadata, and primary papers through
August~21, 2026.  The following developments are the ones most directly connected to the
remaining barrier.

\paragraph{Matrix coefficient and structural rank.}
Dasgupta's first paper gives an expository common treatment of complex and $p$-adic logarithm
matrices, Baker--Brumer, the Structural Rank Conjecture, and the Waldschmidt--Masser lower
bound \cite{Dasgupta2023}.  Dasgupta--Kakde then formulate the Matrix Coefficient Conjecture
and a Newton-polytope auxiliary-polynomial strategy; the paper appeared in
\emph{Advances in Mathematics} in 2026 \cite{DasguptaKakde2026}.  For the present problem,
Sections~\ref{sec:c3-matrix-coefficient}--\ref{sec:c3-DK-grid} identify the exact matrix,
prove condition (o) impossible, and show directly that their conditions (m) and (m$'$) are
incompatible with the counterexample grid.  The missing implication is therefore the critical
construction from rank one to a forbidden small-support polynomial.

\paragraph{$p$-adic regulator applications.}
Betina--Maksoud--Pozzi, published in Crelle's Journal in 2026, show that hypotheses governing
$p$-adic eigencurve geometry and Gross--Stark regulators follow in different cases from weak
$p$-adic Schanuel or the weaker $p$-adic Four Exponentials Conjecture
\cite{BetinaMaksoudPozzi2026}.  This demonstrates that the same determinant nonvanishing
problem is already a live input in arithmetic geometry.  It supplies no new unconditional
size-two determinant theorem, but it validates the matrix formulation as the natural local
one.

\paragraph{Hensel minimality and $p$-adic exponentiation.}
Eterovi\'c and Vermeulen prove that $p$-adic exponentiation on $\C_p$ lives in a
$1$-h-minimal structure and show that the $p$-adic Schanuel conjecture implies a uniform
version of itself \cite{EterovicVermeulen2026}.  This is important model-theoretic progress,
but its transcendence conclusions remain conditional on $p$-adic Schanuel.  It does not
transfer the real rank drop \eqref{eq:c3-real-matrix} to any finite place.

\paragraph{Valued-field determinant counting.}
Vermeulen proves Pila--Wilkie-type $H^\varepsilon$ bounds for rational points on
transcendental curves in broad $1$-h-minimal valued fields using parametrization and a
non-archimedean determinant estimate \cite{Vermeulen2025}.  This substantially enlarges the
scope of valued-field counting, but the scale remains too coarse here: a counterexample
produces only $\asymp\log H$ synchronized points, and an $H^\varepsilon$ upper bound does not
exclude them.  Nor does the theorem encode the separated $2$-power and $3$-power denominator
conditions.

\paragraph{Current status and formalization.}
Prasad's 2025 survey explicitly records the implication
$2^t,3^t\in\Z\Rightarrow t\in\Z$ as unknown and places it in the Four Exponentials framework
\cite{Prasad2025}.  Karatarakis and Wiedijk's 2026 Lean formalization of
Gelfond--Schneider confirms that the one-exponential transcendence input can now be
machine-checked end to end \cite{KaratarakisWiedijk2026}; the determinant and $p$-adic
logarithmic-form layers needed here remain unformalized.

The search found no 2025--2026 result proving the needed complex or $p$-adic size-two matrix
nonvanishing.  The new papers sharpen the architecture, applications, and model theory around
the frontier, but do not move the unconditional boundary past Four Exponentials.

\subsection{A revised research program}
\label{sec:c3-program}

The combined real and $p$-adic audit changes the priority order.

\begin{enumerate}
\item \textbf{Specialize the critical auxiliary-polynomial construction.}
      Work directly with the grid \eqref{eq:c3-grid}.  The target is to derive, from
      $\det L_\infty=0$ and integrality of $M,A$, a nonzero polynomial on $X(2N)$ with fewer
      than $4N^2+1$ monomials, or preferably satisfying the Dasgupta--Kakde
      $\operatorname{BK}$ bound.  Theorem~\ref{thm:c3-support-rigidity} would then give the
      contradiction.  This is the cleanest route because the zero estimate is already exact.

\item \textbf{Search for a real-to-structural-prime closure mechanism.}
      Proposition~\ref{prop:c3-closure-implies-D} and
      Theorem~\ref{thm:c3-base-primes} show that either
      $(M,A)\in\Gamma_2$ or $(M,A)\in\Gamma_3$ is enough.  Any proposed mechanism must meet
      the exact torsion and tangent constraints of Proposition~\ref{prop:c3-exact-closure};
      merely producing congruences modulo infinitely many unrelated primes is not sufficient.

\item \textbf{Exploit the Fermat-quotient determinant statistically but certify it
      algebraically.}
      Factor candidate $M,A$, compute $\Delta_p(M,A)$ over large prime ranges, and test
      whether the valuation vectors forced by the primitive odd core make
      $\Delta_p\ne0$ at some bounded prime.  A theorem of the form
      ``every nontrivial primitive pair has an auxiliary prime with $\Delta_p\ne0$'' would
      prove that at least one local tangent is nonsingular, but an additional place-transfer
      argument would still be needed to contradict the real zero.

\item \textbf{Compute tropical associated-graded determinants, not only valuations.}
      The first-layer factorization is too small.  Experiments should retain the actual
      initial forms across successive layers and search for a representation-theoretic or
      symmetric-function identity.  Proposition~\ref{prop:c3-cascade-depth} says that a
      repeated pairwise mechanism needs almost linearly many layers; a successful identity
      should therefore be visible globally rather than as sporadic extra divisibility.

\item \textbf{Pursue an adelic auxiliary object with many small conjugates.}
      Proposition~\ref{prop:c3-kummer-asymptotic} gives one small conjugate but too many
      uncontrolled twists.  The right object would correlate the twists, perhaps by taking a
      determinant over a structured orbit rather than four independent radicals.  The product
      formula can help only after the local logarithm determinant is embedded into one common
      algebraic number.

\item \textbf{Formalize the elementary new layer.}
      The sparse-polynomial theorem, no-coefficient proposition, grid support rigidity,
      base-prime nonsingularity, and Fermat-quotient congruence require only unique
      factorization, generalized Vandermonde nonvanishing, and elementary modular algebra.
      They are realistic Lean targets independent of any unformalized transcendence theorem.
\end{enumerate}

\subsection{Lean blueprint for the new statements}
\label{sec:c3-lean}

A minimal module graph would be
\begin{center}
\begin{tabular}{ll}
\toprule
Proposed module & Main statement \\
\midrule
\lean{PrimeLogQuadratic} & coefficient characterization of $\mathcal F_{M,A}$ \\
\lean{MatrixCoefficientObstruction} & factorization \eqref{eq:c3-Pi-factor} and nonvanishing \\
\lean{CounterexampleGridSupport} & $|\supp P|\ge L^2+1$ from generalized Vandermonde \\
\lean{IwasawaRationalKernel} & rational specialization \eqref{eq:c3-Iwasawa-kernel} as an interface \\
\lean{StructuralPrimeDeterminants} & formulas \eqref{eq:c3-D2}--\eqref{eq:c3-D3} \\
\lean{FermatQuotientDeterminant} & congruence \eqref{eq:c3-Dp-modp} \\
\lean{AdelicErrorWedge} & identity \eqref{eq:c3-wedge} \\
\lean{KummerTaylorDeterminant} & asymptotic coefficient in \eqref{eq:c3-Xi-asymptotic} \\
\bottomrule
\end{tabular}
\end{center}

The Iwasawa-logarithm modules would initially expose algebraic interfaces rather than attempt
to construct the full $p$-adic analytic library.  For example,
\lean{IwasawaRationalKernel} can assume a homomorphism with the rational kernel theorem as a
named hypothesis, while the Fermat-quotient congruence can be formalized entirely in modular
arithmetic.  The grid support theorem can reuse the existing
\lean{GeneralizedVandermonde} development proposed earlier in this report.

\subsection{Conclusion of the \texorpdfstring{$p$}{p}-adic continuation}

The $p$-adic analysis does not prove the Alaoglu--Erd\H{o}s conjecture.  It produces a sharper
and somewhat counterintuitive picture:
\begin{itemize}
\item the real place is the unique place where singularity is known;
\item the two primes defining the problem are places where singularity is impossible;
\item all auxiliary finite places are conjecturally nonsingular by the $p$-adic Matrix
      Coefficient Conjecture;
\item the first local obstruction is the explicit Fermat-quotient determinant
      \eqref{eq:c3-Delta-p}; and
\item real continued-fraction synchronization does not propagate to finite places without
      strong denominator divisibility.
\end{itemize}

The most promising new formulation is therefore not ``prove a $p$-adic analogue of the real
relation.''  It is: construct an adelic or auxiliary-polynomial object that is forced by the
real rank drop but whose finite-place behavior is constrained by the proven transversality at
$2$ and $3$.  Dasgupta--Kakde's support framework supplies an exact endpoint for such a
construction, and Theorem~\ref{thm:c3-support-rigidity} shows that the endpoint is already
strong enough.  The remaining task is the critical construction itself.

\begin{resultbox}
\textbf{Final status after the third continuation.}
The conjecture remains open.  No unsupported proof claim is made.  The continuation adds
rigorous reductions and no-go theorems: a sparse prime-log polynomial, an exact
matrix-coefficient obstruction, sharp support rigidity for the full counterexample grid,
provable $2$-adic and $3$-adic nonsingularity, a Fermat-quotient local certificate, an exact
adelic error identity, a quantified lower bound on cancellation-cascade depth, and a Kummer
norm diagnosis.  Together they reduce a viable $p$-adic strategy to one missing bridge:
transfer enough information from the real rank drop to an algebraic or finite-place object
without assuming Four Exponentials itself.
\end{resultbox}
...
Sparse prime-log quadratic obstruction & \statusnew & Theorem~\ref{thm:c3-sparse-quadratic} \\
No vanishing rational matrix coefficient for a counterexample & \statusnew & Proposition~\ref{prop:c3-no-coefficient} \\
Exact counterexample-grid support bound $|\supp P|\ge L^2+1$ & \statusnew & Theorem~\ref{thm:c3-support-rigidity} \\
Failure of the critical Dasgupta--Kakde Newton bounds under a counterexample & \statusnew & Proposition~\ref{prop:c3-m-fails} \\
Structural-prime nonsingularity $D_2D_3\ne0$ & \statusnew & Theorem~\ref{thm:c3-base-primes} \\
Auxiliary-prime Fermat-quotient determinant certificate & \statusnew & Theorem~\ref{thm:c3-fermat-determinant} \\
Exact real/$p$-adic error wedge identity & \statusnew & Theorem~\ref{thm:c3-wedge} \\
Third-order Kummer determinant asymptotic & \statusnew & Proposition~\ref{prop:c3-kummer-asymptotic} \\
\bottomrule
\end{longtable}
}
...
% --- added with the p-adic and matrix-coefficient continuation ---
...
\newblock \url{https://arxiv.org/abs/2303.02037}.
...
\newblock arXiv:2505.15744 (2025).
\newblock \url{https://arxiv.org/abs/2505.15744}.

\bibitem{BetinaMaksoudPozzi2026}
A.~Betina, A.~Maksoud, and A.~Pozzi,
\newblock \emph{The eigencurve at crystalline points with scalar Frobenius and
Gross--Stark regulators},
\newblock Journal f\"ur die reine und angewandte Mathematik \textbf{834} (2026), 169--228.
\newblock DOI:
\href{https://doi.org/10.1515/crelle-2026-0023}{10.1515/crelle-2026-0023}.

\bibitem{EterovicVermeulen2026}
S.~Eterovi\'c and F.~Vermeulen,
\newblock \emph{Hensel minimality, $p$-adic exponentiation and Tate uniformization},
\newblock arXiv:2602.16433 (2026).
\newblock \url{https://arxiv.org/abs/2602.16433}.

\bibitem{Vermeulen2025}
F.~Vermeulen,
\newblock \emph{Counting rational points on transcendental curves in valued fields},
\newblock arXiv:2506.19411, version 2 (2025).
\newblock \url{https://arxiv.org/abs/2506.19411}.
