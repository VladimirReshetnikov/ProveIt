...
\hypersetup{
  ...
  pdfsubject={Machine-checked partial results, matrix-coefficient reformulation, exact sparse
frontiers, determinant divisibility, and research directions}
}
\urlstyle{same}
...
\newcommand{\BKd}{\operatorname{BKdeg}}
\newcommand{\SF}[1]{\mathfrak F_{#1}}
...
At paper level the report develops arbitrary-node and mixed-semigroup determinant methods,
a prime-log-product sufficient criterion, and a 2026 continuation built around the Matrix
Coefficient Conjecture.  The continuation proves full spark for every finite monomial
evaluation matrix on an irrational power curve; determines the exact support, total-degree,
bidegree, multiplicity, coding-theoretic, and Bernstein--Kushnirenko interpolation frontiers;
strengthens ordinary-power repulsion by the universal divisor
$\prod_{j<r}j!$; and isolates the primitive Smith divisor, including a node-only factor whose
$2$- and $3$-adic masses on an $L\times L$ grid are each
$L^5/6-L^4/4+O(L^3)$; the corresponding dilation defects compress to degree $L-1$.
These are genuine
new partial results, but they locate rather than cross the final one-monomial/product-formula
barrier.
\end{minipage}
\vfill
...
\[
 \prod_{0\le i<j\le r}(m_j-m_i)\ \ge\ \SF{r}\frac{r!}{|\fall{\thetaq}{r}|}\,m_0^{\,r-\thetaq},
\]
...
formulation, and the prime-log-product independence criterion.  Fifth, it specializes the
recent Matrix Coefficient/fewnomial framework to the exact $2,3$ grid, proves full-spark and
sharp sparse/BK interpolation frontiers, identifies universal and node-dependent Smith
divisors, audits the 2023--2026 literature, and closes with exact sufficient targets and a
...
The third continuation, Section~\ref{sec:continuation3}, places the problem inside the 2026
Matrix Coefficient framework and then computes the special auxiliary-polynomial obstruction
exactly.  On an irrational power curve, $M$ points require $M+1$ monomials and BK-degree
$M-1$; the primitive relation is unique and full-support.  The same continuation strengthens
the ordinary interpolation determinant by $\SF r=\prod_{j<r}j!$ and decomposes its primitive
Smith normalization into a node-only deleted-Vandermonde factor and a genuinely
output-dependent quotient.  These results sharpen the proof target to one monomial, one unit
of BK-degree, or one product-formula unit.
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.51\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}@{}}
\toprule
...
\addlinespace
Effective irrationality of $\beta$ & Baker--W\"ustholz/Matveev gives $|\beta-p/q|\ge c(M)q^{-\mu(M)}$ and hence polynomial, rather than exponential, recurrence bounds for continued-fraction denominators. & \statusknown{}\par\statusnew\\
\addlinespace
...
\addlinespace
HCIZ reformulation & The full-grid archimedean problem is expressed by the Harish--Chandra--Itzykson--Zuber integral, identifying a concrete free-energy inequality as the remaining analytic target. & \statusknown{}\par\statusnew\\
\bottomrule
\end{longtable}
...
{\footnotesize
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.29\textwidth}>{\raggedright\arraybackslash}p{0.34\textwidth}>{\raggedright\arraybackslash}p{0.27\textwidth}@{}}
\toprule
...
\section{Third continuation: the matrix-coefficient bridge and the exact sparse frontier}
\label{sec:continuation3}

\subsection{Scope, outcome, and trust boundary}

This continuation was written after a fresh literature search through August 21, 2026,
including arXiv, journal metadata, and recent work on matrices of logarithms, auxiliary
polynomials, transcendence measures, rational-point counting, irrational powers, and formalized
transcendence theory.  The most directly relevant development is the Matrix Coefficient
Conjecture of Dasgupta and Kakde, whose article appeared in \emph{Advances in Mathematics} in
2026~\cite{DasguptaKakde2026}.  Their framework does not prove the four exponentials
conjecture; rather, it isolates an auxiliary-polynomial implication that remains open.  In the
special Alaoglu--Erd\H{o}s family, however, that framework can be specialized much more
sharply than the general discussion suggests.

No proof of Conjecture~\ref{conj:AE} is claimed.  The new unconditional results are instead:
\begin{enumerate}
\item an exact equivalence between the Alaoglu--Erd\H{o}s conjecture and the Matrix
      Coefficient Conjecture restricted to one explicit family of singular $2\times2$
      logarithm matrices;
\item a full-spark theorem for every finite monomial evaluation matrix on an irrational
      power curve, with exact support, multiplicity, degree, bidegree, and generalized
      Hamming-weight consequences;
\item an exact Bernstein--Kushnirenko frontier: $M$ distinct points on an irrational power
      curve require BK-degree at least $M-1$, and this is attained by the primitive
      interpolation polynomial;
\item a universal superfactorial divisor in the ordinary-power interpolation determinant,
      strengthening all fixed-order arbitrary-node repulsion constants from
      Section~\ref{sec:arbnode};
\item a primitive-cofactor/Smith-normal-form package that gathers all archimedean and
      $p$-adic interpolation information into one last determinantal divisor, including an
      exact maximal node-only factor and its two-term grid valuation asymptotics;
\item an exact dilation-defect factorization compressing the two rectangular-grid scaling
      failures to nonzero degree-$(L-1)$ integer boundary polynomials; and
\item sharply stated sufficient targets showing what an analytic, adelic, or Smith-divisor
      argument would have to construct in order to finish the conjecture.
\end{enumerate}

Every theorem below marked \statusnew{} has a complete paper proof in this continuation.  The
literature statements are marked \statusknown{}.  No new Lean verification is claimed.  The
formalization plan in Section~\ref{sec:thirdformalization} separates elementary linear algebra,
integer-valued polynomial divisibility, convex lattice geometry, and the analytic
Chebyshev-system input so that each can be checked independently.

\subsection{Claim ledger}

\begin{center}
\small
\begin{tabularx}{0.96\textwidth}{>{\raggedright\arraybackslash}X
                                  >{\raggedright\arraybackslash}p{0.17\textwidth}
                                  >{\raggedright\arraybackslash}p{0.31\textwidth}}
\toprule
Claim & Status & Principal input \\
\midrule
Restricted Matrix Coefficient Conjecture $\Longleftrightarrow$ Alaoglu--Erd\H{o}s &
\statusnew & multiplicative independence of $2,3$ and rational-exponent integrality \\
Every finite monomial evaluation matrix on $y=x^\beta$, $\beta\notin\Q$, is full spark &
\statusnew & strict total positivity / exponential-polynomial zero count \\
Minimum support through $M$ distinct points is exactly $M+1$ &
\statusnew & full spark and univariate interpolation \\
Minimum total degree and bidegree are exact binomial-count thresholds &
\statusnew & full spark and monomial counting \\
Minimum BK-degree through $M\ge2$ points is exactly $M-1$ &
\statusnew & Pick's theorem and primitive interpolation \\
Ordinary interpolation determinant is divisible by $\prod_{j=0}^{r-1}j!$ &
\statusnew & the binomial-coefficient basis of integer-valued polynomials \\
Strengthened arbitrary-node repulsion &
\statusnew & the preceding divisor plus divided differences \\
Primitive interpolation polynomial and last determinantal divisor &
\statusnew & maximal minors and Smith normal form \\
Maximal node-only divisor and two-term $2,3$-adic grid mass &
\statusnew & deleted Vandermondes and weighted valuation graphs \\
Rectangular-grid dilation defects compress to degree $L-1$ &
\statusnew & primitive interpolation and commuting dilation operators \\
A complete contradiction & Open & requires a sparse/low-BK auxiliary polynomial or
an additional divisor exceeding the archimedean bound \\
\bottomrule
\end{tabularx}
\end{center}

\section{The exact matrix-coefficient formulation}
\label{sec:mcc}

\subsection{The recent conjectural framework}

Let
\[
 \LL=\{z\in\C:e^z\in\Abar\}
\]
be the $\Q$-vector space of logarithms of algebraic numbers.  Dasgupta and Kakde formulate
the following conjecture~\cite{DasguptaKakde2026}.

\begin{conjecture}[Matrix Coefficient Conjecture]
\label{c3:conj-mcc}
If $L\in M_n(\LL)$ is singular, then there are nonzero vectors
$w,v\in\Q^n$ such that
\[
 w^{\mathsf T}Lv=0.
\]
\end{conjecture}

For $n=2$ this is equivalent to the four exponentials conjecture.  Its form is weaker than
requiring a rational left or right kernel: after independent rational changes of bases on the
left and right, it asks only that one coefficient vanish.  In the Alaoglu--Erd\H{o}s family,
that apparent flexibility disappears completely.

\subsection{One explicit singular family}

For positive integers $U,V$, put
\begin{equation}
 L(U,V):=
 \begin{pmatrix}
  \log 2&\log U\\
  \log 3&\log V
 \end{pmatrix}.
 \label{c3:eq-logmatrix}
\end{equation}
The determinant condition is
\begin{equation}
 \log2\log V-\log3\log U=0.
 \label{c3:eq-detzero}
\end{equation}
When it holds, define
\[
 \beta:=\frac{\log U}{\log2}=\frac{\log V}{\log3}.
\]
Then $U=2^\beta$, $V=3^\beta$, and
\begin{equation}
 L(U,V)=
 \binom{\log2}{\log3}
 \begin{pmatrix}1&\beta\end{pmatrix}.
 \label{c3:eq-rankonefactor}
\end{equation}

\begin{theorem}[Exact restricted Matrix Coefficient equivalence]
\label{c3:thm-restricted-mcc}
The following are equivalent.
\begin{enumerate}
\item The Alaoglu--Erd\H{o}s conjecture is true.
\item For every $U,V\in\N$ satisfying $\det L(U,V)=0$, there exist nonzero
      $w,v\in\Q^2$ with $w^{\mathsf T}L(U,V)v=0$.
\item For every such $U,V$, the matrix $L(U,V)$ has a nonzero rational right-kernel vector.
\end{enumerate}
Moreover, in this family a nonzero rational left-kernel vector never exists.
\end{theorem}

\begin{proof}
Suppose first that $\det L(U,V)=0$.  By \eqref{c3:eq-rankonefactor}, for
$w=(w_1,w_2)^{\mathsf T}$ and $v=(v_1,v_2)^{\mathsf T}$ one has the exact factorization
\begin{equation}
 w^{\mathsf T}L(U,V)v
 =\bigl(w_1\log2+w_2\log3\bigr)\bigl(v_1+\beta v_2\bigr).
 \label{c3:eq-mccfactor}
\end{equation}
If $w\in\Q^2\setminus\{0\}$, the first factor is nonzero.  Indeed, after clearing
denominators, a vanishing relation would give $a\log2+b\log3=0$ with integers $a,b$ not
both zero, hence $2^a3^b=1$, contrary to unique factorization.  Thus any Matrix Coefficient
witness must satisfy $v_1+\beta v_2=0$.  Necessarily $v_2\ne0$, so $\beta\in\Q$.

Write $\beta=p/q$ in lowest terms with $q>0$.  Since $U=2^{p/q}\in\N$, raising to the
$q$-th power gives $U^q=2^p$.  Applying the $2$-adic valuation gives
$qv_2(U)=p$, and coprimality of $p,q$ forces $q=1$.  Hence $\beta\in\Z$.  This proves that the
restricted Matrix Coefficient assertion implies the Alaoglu--Erd\H{o}s conclusion.

Conversely assume the Alaoglu--Erd\H{o}s conjecture.  The determinant condition produces
$U=2^\beta$, $V=3^\beta$ as above, so $\beta\in\Z$.  Then
$v=(-\beta,1)^{\mathsf T}\in\Q^2\setminus\{0\}$ satisfies $L(U,V)v=0$.  Thus (1) implies
(3), and (3) trivially implies (2).  The first-factor argument also proves the final sentence.
\end{proof}

\begin{statusbox}{\statusnew{} --- this is not merely another implication from four
exponentials.  It identifies the conjecture with the Matrix Coefficient Conjecture on the
single arithmetic surface \eqref{c3:eq-detzero}, and proves that the ``mixed coefficient''
freedom collapses there to an ordinary rational right kernel.}
\end{statusbox}

\begin{corollary}[The obstruction has only one side]
\label{c3:cor-one-side}
A hypothetical counterexample $\beta$ produces a singular matrix $L(2^\beta,3^\beta)$ for
which
\[
 w^{\mathsf T}Lv\ne0
 \qquad
 (w,v\in\Q^2\setminus\{0\}).
\]
Thus it is a counterexample to the $2\times2$ Matrix Coefficient Conjecture in the strongest
possible rank-one form: both factors in \eqref{c3:eq-mccfactor} are rationally anisotropic.
\end{corollary}

The value of Theorem~\ref{c3:thm-restricted-mcc} is diagnostic.  Any proof of the full Matrix
Coefficient Conjecture proves Alaoglu--Erd\H{o}s immediately, but the restricted family is far
more structured than a generic singular logarithm matrix: its left factor is fixed,
its right factor is $(1,\beta)$, and the entire multiplicative grid generated by the rows is
integral under a counterexample.  The rest of this continuation asks exactly how much that
integral grid buys in the auxiliary-polynomial strategy.

\section{Full spark on an irrational power curve}
\label{sec:fullspark}

\subsection{Evaluation matrices}

Fix $\beta\in\R\setminus\Q$ and distinct positive real numbers
\[
 0<x_1<\cdots<x_M.
\]
For a finite set $S\subset\Z^2$, write
\[
 S=\{(a_1,b_1),\ldots,(a_H,b_H)\}
\]
and form the evaluation matrix
\begin{equation}
 E_{X,S}:=\bigl(x_i^{a_j+\beta b_j}\bigr)_{
  1\le i\le M,\ 1\le j\le H}.
 \label{c3:eq-evalmatrix}
\end{equation}
It evaluates Laurent monomials $X^aY^b$ at the points
$(x_i,x_i^\beta)$.  Irrationality of $\beta$ makes the spectral parameters
\[
 \lambda_{a,b}:=a+\beta b
\]
pairwise distinct on distinct lattice points $(a,b)$.

\begin{theorem}[Full-spark evaluation]
\label{c3:thm-fullspark}
Every square minor of $E_{X,S}$ of order at most $\min(M,H)$ is nonzero.  More precisely,
after arranging the selected nodes $x_i$ and selected spectral parameters $a+\beta b$ in
increasing order, the minor is strictly positive.  Consequently
\begin{equation}
 \rank E_{X,S}=\min(M,H).
 \label{c3:eq-fullrank}
\end{equation}
\end{theorem}

\begin{proof}
Put $t_i=\log x_i$.  Every selected square minor is, after row and column permutations,
\[
 \det\bigl(e^{t_i\lambda_j}\bigr)_{1\le i,j\le h}.
\]
Arrange $t_1<\cdots<t_h$ and $\lambda_1<\cdots<\lambda_h$.  The kernel
$(t,\lambda)\mapsto e^{t\lambda}$ is strictly totally positive.  Equivalently, the functions
$e^{\lambda_1t},\ldots,e^{\lambda_ht}$ form an extended complete Chebyshev system: a nonzero
linear combination has at most $h-1$ real zeros counted with multiplicity.  Nonvanishing of
the determinant follows immediately, and its sign is positive by the generalized
Vandermonde sign theorem proved in Appendix~\ref{app:gvproof}.  Taking a largest square minor
gives \eqref{c3:eq-fullrank}.
\end{proof}

\begin{remark}[No genericity qualification]
The theorem is exact for every choice of distinct positive nodes.  There is no exceptional
Zariski-closed set, no height threshold, and no asymptotic qualifier.  The only hypothesis is
$\beta\notin\Q$, which separates the monomial weights $a+\beta b$.
\end{remark}

\subsection{The exact support threshold}

For a Laurent polynomial
\[
 P(X,Y)=\sum_{(a,b)\in S}c_{a,b}X^aY^b,
 \qquad c_{a,b}\ne0,
\]
write $|\supp P|=|S|$.

\begin{theorem}[Exact fewnomial interpolation threshold]
\label{c3:thm-support-threshold}
Let $\beta\notin\Q$ and let $(x_i,x_i^\beta)$, $1\le i\le M$, be distinct positive points.
Then a nonzero Laurent polynomial vanishing at all $M$ points has at least $M+1$ monomials.
This is sharp.  If all $x_i$ and $x_i^\beta$ are rational, the polynomial attaining the
minimum may be chosen in $\Q[X^{\pm1},Y^{\pm1}]$; if they are integral, it may be chosen in
$\Z[X,Y]$ and primitive.
\end{theorem}

\begin{proof}
If $P$ has $H\le M$ monomials, its coefficient vector lies in the kernel of an
$M\times H$ submatrix of \eqref{c3:eq-evalmatrix}.  Theorem~\ref{c3:thm-fullspark} says that
this matrix has rank $H$, so the coefficient vector is zero, a contradiction.

For sharpness, take
\[
 P_0(X)=\prod_{i=1}^{M}(X-x_i).
\]
Because every $x_i>0$, every elementary symmetric coefficient is nonzero, so $P_0$ has
exactly $M+1$ monomials and vanishes at all points.  Rational or integral nodes give rational
or integral coefficients; division by the coefficient gcd makes the latter primitive.
\end{proof}

\begin{corollary}[A discontinuous rational/irrational phase transition]
\label{c3:cor-phase-transition}
Fix $M\ge2$ distinct positive nodes.  For $\beta>0$, the minimum support of a nonzero Laurent
polynomial vanishing on $(x_i,x_i^\beta)$ is
\[
 \begin{cases}
  2,&\beta\in\Q,\\
  M+1,&\beta\notin\Q.
...
\begin{proof}
The irrational case is Theorem~\ref{c3:thm-support-threshold}.  If $\beta=p/q$ in lowest
terms with $p,q>0$, then $Y^q-X^p$ has two monomials and vanishes identically on the curve.
A monomial has no positive zero, so one monomial is impossible.
\end{proof}

This phase transition is the finite interpolation shadow of the conjecture itself.  A
rational exponent creates a two-term algebraic equation on the whole curve.  An irrational
exponent makes every finite set of $M$ points maximally independent with respect to monomial
support: the first relation occurs only when linear algebra forces it at $M+1$ columns.

\subsection{Multiplicity and confluent constraints}

\begin{corollary}[Multiplicity costs one monomial per condition]
\label{c3:cor-multiplicity}
Let $P\ne0$ be a Laurent polynomial and put
$F(t)=P(e^t,e^{\beta t})$.  If $F$ has zeros of total multiplicity $R$ on the real line, then
\[
 |\supp P|\ge R+1.
\]
In particular, imposing value and derivative conditions does not beat the support threshold:
each independent confluent condition consumes one additional monomial.
\end{corollary}

\begin{proof}
If $P$ has $H$ monomials, then $F$ is an exponential polynomial with $H$ distinct real
exponents.  Such a nonzero function has at most $H-1$ zeros counted with multiplicity by the
extended Chebyshev property used in Theorem~\ref{c3:thm-fullspark}.
\end{proof}

\subsection{The associated MDS code}

Let $K$ be a subfield of $\R$ containing all entries of $E_{X,S}$, and suppose $H>M$.
The coefficient relations form the linear code
\[
 \mathcal C_{X,S}:=\ker(E_{X,S}:K^H\to K^M),
 \qquad \dim\mathcal C_{X,S}=H-M.
\]
For a subspace $W\subset K^H$, its support is the union of coordinate positions on which
some vector of $W$ is nonzero.

\begin{theorem}[Exact generalized Hamming weights]
\label{c3:thm-mds}
For $1\le d\le H-M$, every $d$-dimensional subspace
$W\subset\mathcal C_{X,S}$ satisfies
\[
 |\supp W|\ge M+d,
\]
and equality is attained.  Thus the generalized Hamming weights are
\[
 d_d(\mathcal C_{X,S})=M+d.
\]
\end{theorem}

\begin{proof}
Let $T=\supp W$.  Restricting to the columns in $T$ embeds $W$ in
$\ker E_{X,T}$.  Full spark gives
\[
 \dim\ker E_{X,T}=\max(0,|T|-M),
\]
so $d\le|T|-M$.  Conversely, for any $T$ of size $M+d$, the same formula gives a kernel of dimension
exactly $d$.  No coordinate of this kernel vanishes identically: imposing one chosen
coordinate to be zero identifies the subspace with the kernel on $T$ with that column
removed, which has dimension $d-1$ by full spark.  Hence the union of supports of the
full $d$-dimensional kernel is exactly $T$, attaining equality.
\end{proof}

The terminology is not decorative.  The point configuration on an irrational power curve
produces a generalized Reed--Solomon phenomenon for \emph{every} finite lattice support.
There is no sparse subcode hidden in a clever choice of monomials: every $d$ independent
relations collectively use at least $M+d$ coordinates.

\subsection{The critical circuit}

The first possible relation occurs at $H=M+1$.

\begin{corollary}[Alternating primitive circuit]
\label{c3:cor-circuit}
Suppose $H=M+1$, and order the monomials so that their spectral weights
$a_j+\beta b_j$ increase.  Then $\ker E_{X,S}$ is one-dimensional.  Its signed cofactor
vector has no zero coordinate and, after multiplication by one global sign, its coordinates
alternate in sign.  Over rational evaluations the primitive integer relation is unique up to
sign.
\end{corollary}

\begin{proof}
The rank is $M$, so the kernel is one-dimensional.  The $j$-th coordinate of the cofactor
vector is $(-1)^j$ times an $M\times M$ maximal minor.  All such minors are strictly positive
in the ordered chamber by Theorem~\ref{c3:thm-fullspark}.  Rationality and primitivity follow
by clearing denominators and dividing by the gcd.
\end{proof}

This is the oriented-matroid circuit of the configuration.  It explains a recurring feature
of all interpolation constructions below: at the critical $M+1$ support size, no coefficient
can be deleted by accidental cancellation.  The support bound is not merely dimensional; the
unique relation sits in the interior of every coordinate orthant compatible with its forced
alternating sign pattern.

\subsection{Exact degree and bidegree thresholds}

\begin{theorem}[Total-degree threshold]
\label{c3:thm-degree-threshold}
Let $d_{\min}(M)$ be the smallest total degree of a nonzero polynomial in $\R[X,Y]$ vanishing
at $M$ distinct points of an irrational power curve.  Then
\begin{equation}
 d_{\min}(M)=
 \left\lceil\frac{\sqrt{8M+9}-3}{2}\right\rceil.
 \label{c3:eq-dmin}
\end{equation}
If the points are rational, a polynomial of this degree exists over $\Q$; if integral, over
$\Z$.
\end{theorem}

\begin{proof}
The number of monomials $X^aY^b$ with $a,b\ge0$ and $a+b\le d$ is
\[
 H_d=\binom{d+2}{2}.
\]
The evaluation map on these monomials has rank $\min(M,H_d)$ by
Theorem~\ref{c3:thm-fullspark}; hence a nonzero kernel exists exactly when $H_d>M$.  The least
$d$ satisfying $\binom{d+2}{2}\ge M+1$ is \eqref{c3:eq-dmin}.  The field-of-definition
claims follow because the kernel matrix has rational or integral entries.
\end{proof}

\begin{theorem}[Bidegree threshold]
\label{c3:thm-bidegree-threshold}
For integers $r,s\ge0$, there is a nonzero polynomial $P$ with
$\deg_XP\le r$, $\deg_YP\le s$ vanishing at all $M$ points if and only if
\begin{equation}
 (r+1)(s+1)>M.
 \label{c3:eq-bidegree}
\end{equation}
The same rational/integral coefficient conclusion holds.
\end{theorem}

\begin{proof}
The relevant rectangle contains exactly $(r+1)(s+1)$ monomials, and full spark again makes
the evaluation map have maximal possible rank.
\end{proof}

Theorems~\ref{c3:thm-support-threshold}--\ref{c3:thm-bidegree-threshold} give the complete
finite-dimensional Hilbert function of these points for all monomial support families.  No
choice of a skew Newton polygon, weighted degree, or sparse rectangle can improve on the
plain count of available monomials.

\section{The exact Bernstein--Kushnirenko frontier}
\label{sec:bkfrontier}

\subsection{The two-dimensional complexity used by Dasgupta--Kakde}

For a nonzero polynomial $P\in\C[X,Y]$, let $\Delta(P)$ be its Newton polygon.  In two
variables the Bernstein--Kushnirenko number is
\[
 \BK(P)=2\operatorname{Area}\Delta(P).
\]
After dividing out the largest common monomial factor, the BK-degree of
Dasgupta--Kakde~\cite{DasguptaKakde2026} becomes
\begin{equation}
 \BKd(P)=\max\{\deg P,\,2\operatorname{Area}\Delta(P)\}.
 \label{c3:eq-bkd2}
\end{equation}
The common-monomial normalization is harmless on $(\C^*)^2$, because a monomial has no
zero there.

The following elementary lattice-geometric estimate turns the support theorem into an exact
BK-degree theorem.

\begin{lemma}[Support forces BK-degree]
\label{c3:lem-support-bkd}
Let $P\in\C[X,Y]$ be nonzero and not divisible by a monomial.  If
$h=|\supp P|\ge3$, then
\begin{equation}
 \BKd(P)\ge h-2.
 \label{c3:eq-support-bkd}
\end{equation}
\end{lemma}

\begin{proof}
If $\Delta(P)$ is two-dimensional, let $I$ and $B$ be the numbers of interior and boundary
lattice points of the polygon.  The support is contained among these lattice points, so
$h\le I+B$.  Pick's theorem gives
\[
 2\operatorname{Area}\Delta(P)=2I+B-2\ge I+B-2\ge h-2.
\]

If $\Delta(P)$ is a line segment, write its lattice length as $k$, so that it contains at most
$k+1$ lattice points and hence $k\ge h-1$.  Its direction is $k(u,v)$ for a primitive
integer vector $(u,v)$.  Since the common monomial factor has been removed,
$\min a=\min b=0$ on the support.  Therefore the usual total degree is at least the larger
coordinate width, which is at least $k\max(|u|,|v|)\ge k$.  Thus
$\deg P\ge h-1$, stronger than \eqref{c3:eq-support-bkd}.
\end{proof}

\subsection{Exact minimization}

\begin{theorem}[Minimum BK-degree through $M$ points]
\label{c3:thm-min-bkd}
Let $M\ge2$ and let $(x_i,x_i^\beta)$ be $M$ distinct positive points with
$\beta\notin\Q$.  Among nonzero polynomials $P\in\R[X,Y]$ vanishing at all the points, the
minimum BK-degree is exactly
\begin{equation}
 \min\BKd(P)=M-1.
 \label{c3:eq-min-bkd}
\end{equation}
For rational points the minimum is attained over $\Q$, and for integral points by a primitive
polynomial over $\Z$.
\end{theorem}

\begin{proof}
Divide out any common monomial.  The support theorem gives
$h=|\supp P|\ge M+1$, so Lemma~\ref{c3:lem-support-bkd} yields
$\BKd(P)\ge M-1$.

For the reverse inequality, use the $M+1$ monomials
\[
 Y,1,X,X^2,\ldots,X^{M-1}.
\]
By Theorem~\ref{c3:thm-fullspark} there is a unique relation up to scale,
\begin{equation}
 P_{\mathrm{int}}(X,Y)=c_YY+\sum_{j=0}^{M-1}c_jX^j,
 \label{c3:eq-interppoly}
\end{equation}
vanishing at the $M$ points, and Corollary~\ref{c3:cor-circuit} says every coefficient is
nonzero.  Its Newton polygon is the triangle with vertices
$(0,0)$, $(M-1,0)$, $(0,1)$.  Hence
\[
 \deg P_{\mathrm{int}}=M-1,
 \qquad
 2\operatorname{Area}\Delta(P_{\mathrm{int}})=M-1,
\]
so $\BKd(P_{\mathrm{int}})=M-1$.  Rationality, integrality, and primitivity follow from the
cofactor construction.
\end{proof}

\begin{corollary}[The first relation is simultaneously support- and BK-optimal]
\label{c3:cor-double-optimal}
The primitive interpolation relation \eqref{c3:eq-interppoly} has the least possible support
$M+1$ and the least possible BK-degree $M-1$.  No alternative Newton polygon can trade one
complexity measure against the other to improve either threshold.
\end{corollary}

\subsection{Specialization to the $2^i3^j$ grid}

Suppose a counterexample $\beta$ exists, and put
\[
 U=2^\beta\in\N,
 \qquad
 V=3^\beta\in\N.
\]
For an integer $L\ge2$ define
\begin{equation}
 \mathcal X(L):=
 \{(2^i3^j,U^iV^j):0\le i,j<L\}.
 \label{c3:eq-grid}
\end{equation}
Unique factorization makes the first coordinates distinct, and every point lies on
$Y=X^\beta$.  Thus $|\mathcal X(L)|=L^2$.

\begin{corollary}[Exact complexity of the counterexample grid]
\label{c3:cor-grid-complexity}
If $\beta\notin\Q$, every nonzero polynomial vanishing on $\mathcal X(L)$ satisfies
\[
 |\supp P|\ge L^2+1,
 \qquad
 \BKd(P)\ge L^2-1,
...
Dasgupta--Kakde's condition $(m')$ asks, for the $2\times2$ problem, for a polynomial with
fewer than $N^2$ monomials vanishing on $\mathcal X(2N)$; their condition $(m)$ asks for
BK-degree below $N^2$ on the same grid.  Here $M=4N^2$, so
Corollary~\ref{c3:cor-grid-complexity} gives the exact opposite lower bounds
\begin{equation}
 |\supp P|\ge4N^2+1,
 \qquad
 \BKd(P)\ge4N^2-1.
 \label{c3:eq-dk-factor-four}
\end{equation}

\begin{remark}[Two different gaps]
There are two scales which must not be conflated.
\begin{enumerate}
\item For an arbitrary set of $M$ points, linear algebra automatically constructs a relation
      at $M+1$ monomials, while a contradiction by the zero-count theorem would require at
      most $M$.  This is an intrinsic \emph{one-monomial gap}.  In BK-degree the corresponding
      exact frontier is $M-1$ versus the impossible target $M-2$.
\item Dasgupta--Kakde normalize their $2\times2$ auxiliary-polynomial condition using
      $N^2$ while evaluating on $M=(2N)^2=4N^2$ points.  In that normalization the exact
      obstruction is a \emph{factor of four}, as in \eqref{c3:eq-dk-factor-four}.
\end{enumerate}
The first is the irreducible interpolation threshold.  The second is the quantitative room
needed by their global proof that $(m)$ or $(m')$ implies a rational matrix coefficient.
\end{remark}

\begin{statusbox}{\statusnew{} --- this specialization does not prove the missing implication
$(w)\Rightarrow(m)$ or $(w)\Rightarrow(m')$.  It proves exactly why a counterexample would
make that implication difficult: on the resulting integral grid, the auxiliary polynomial
must beat an interpolation optimum that is rigid in support, Newton area, total degree, and
coefficient support.}
\end{statusbox}

\section{A universal superfactorial divisor}
\label{sec:superfactorial}

\subsection{Integer-valued polynomial basis}

Define the Barnes superfactorial segment
\begin{equation}
 \SF{r}:=\prod_{j=0}^{r-1}j!
 \qquad(r\ge1).
 \label{c3:eq-superfactorial}
\end{equation}
Thus $\SF{1}=\SF{2}=1$, $\SF{3}=2$, $\SF{4}=12$, $\SF{5}=288$.

\begin{theorem}[Universal divisor of the interpolation determinant]
\label{c3:thm-superfactorial-divisor}
Let $r\ge1$, let $m_0,\ldots,m_r\in\Z$, and let $y_0,\ldots,y_r\in\Z$.  Then
\begin{equation}
 D_r:=\det
 \begin{pmatrix}
  1&m_0&\cdots&m_0^{r-1}&y_0\\
  1&m_1&\cdots&m_1^{r-1}&y_1\\
  \vdots&\vdots&&\vdots&\vdots\\
  1&m_r&\cdots&m_r^{r-1}&y_r
 \end{pmatrix}
 \quad\text{is divisible by }\SF{r}.
 \label{c3:eq-D-divisible}
\end{equation}
The divisor is best possible uniformly in the integer data.
\end{theorem}

\begin{proof}
Replace the first $r$ columns by
\[
 \binom{m_i}{0},\binom{m_i}{1},\ldots,\binom{m_i}{r-1}.
\]
These entries are integers for every integer $m_i$.  The change from the binomial basis to the
power basis is triangular because
\[
 m^k=\sum_{j=0}^{k}S(k,j)j!\binom mj,
\]
where $S(k,j)$ is a Stirling number of the second kind.  The diagonal entries of the column
transformation are $0!,1!,\ldots,(r-1)!$, so its determinant is $\SF{r}$.  Therefore
$D_r=\SF{r}D_r^{\mathrm{bin}}$ with $D_r^{\mathrm{bin}}\in\Z$.

For sharpness take $m_i=i$ for $0\le i\le r$ and
$y_0=\cdots=y_{r-1}=0$, $y_r=1$.  Expansion along the last column gives the Vandermonde on
$0,1,\ldots,r-1$, whose absolute value is
$\prod_{j=1}^{r-1}j!=\SF{r}$.
\end{proof}

\begin{remark}[The old lower bound was not arithmetically sharp]
Section~\ref{sec:arbnode} used only that a nonzero integral determinant has absolute value at
least $1$.  Theorem~\ref{c3:thm-superfactorial-divisor} shows that the determinant is forced
into the sublattice $\SF{r}\Z$.  This gain is universal: it uses neither the special value
$y_i=m_i^{\thetaq}$ nor congruences among the nodes.
\end{remark}

\subsection{Strengthened arbitrary-node repulsion}

Recall $\thetaq=\log_2 3$ and
$\fall{\thetaq}{r}=\thetaq(\thetaq-1)\cdots(\thetaq-r+1)$.

\begin{theorem}[Superfactorial Vandermonde repulsion]
\label{c3:thm-super-repulsion}
Let $r\ge2$ and let
\[
 1\le m_0<m_1<\cdots<m_r
\]
be candidates, so $m_i^{\thetaq}\in\N$.  Then
\begin{equation}
 \prod_{0\le i<j\le r}(m_j-m_i)
 \ge
 \SF{r}\frac{r!}{|\fall{\thetaq}{r}|}
 m_0^{\,r-\thetaq}.
 \label{c3:eq-super-repulsion}
\end{equation}
\end{theorem}

\begin{proof}
Use Theorem~\ref{c3:thm-superfactorial-divisor} with $y_i=m_i^{\thetaq}$.  The determinant is
nonzero by Theorem~\ref{c3:thm-fullspark}, applied to the exponents
$0,1,\ldots,r-1,\thetaq$.  Hence $|D_r|\ge\SF{r}$.  The divided-difference identity
\eqref{df:det-divdiff} and mean-value theorem \eqref{df:divdiff-mvt} give
\[
 |D_r|
 =V(m_0,\ldots,m_r)
...
\]
because $r>\thetaq$ and $\xi>m_0$.  Rearrangement proves the claim.
\end{proof}
...
\]
and define the improved constant
\begin{equation}
 \widetilde C_r:=
 \left(\SF{r}\frac{r!}{|\fall{\thetaq}{r}|}\right)^{1/R_r}.
 \label{c3:eq-Ctilde}
\end{equation}

\begin{corollary}[Improved local span and packing]
\label{c3:cor-super-span}
If $r+1$ candidates lie in $[N,N+H]$, then
\begin{equation}
 H\ge\widetilde C_rN^{\alpha_r}.
 \label{c3:eq-super-span}
\end{equation}
Consequently
\begin{equation}
 \#(\Cand\cap[N,N+H])
 \le r+\frac{r}{\widetilde C_r}HN^{-\alpha_r}.
 \label{c3:eq-super-count}
\end{equation}
\end{corollary}

\begin{proof}
The first statement follows from
$V(m_0,\ldots,m_r)\le H^{R_r}$ and
Theorem~\ref{c3:thm-super-repulsion}.  The same disjoint-window argument as in
Corollary~\ref{df:cor-ordinary-count} gives the count.
\end{proof}
...
\begin{tabular}{rrrrr}
\toprule
$r$ & $\alpha_r$ & old $C_r$ & $\widetilde C_r$ & $\SF{r}$ \\
\midrule
2 & 0.1383458331 & 1.2920946431 & 1.2920946431 & $1$ \\
3 & 0.2358395832 & 1.5805909191 & 1.7741533206 & $2$ \\
4 & 0.2415037499 & 1.4602287461 & 1.8721429996 & $12$ \\
5 & 0.2276691666 & 1.3510881062 & 1.9708027974 & $288$ \\
6 & 0.2102398809 & 1.2735045942 & 2.0947059908 & $34560$ \\
7 & 0.1933941964 & 1.2187063909 & 2.2389201079 & $24883200$ \\
8 & 0.1781954861 & 1.1790125192 & 2.3977591062 & $125411328000$ \\
\bottomrule
\end{tabular}
\caption{The superfactorial divisor improves every fixed-order constant from $r=3$ onward.
The exponent remains maximal at $r=4$.}
\label{c3:tab-superconstants}
\end{table}

The best fixed-order exponent remains
\[
 \alpha_4=\frac{4-\thetaq}{10}=0.2415037499\ldots,
\]
but its span constant improves by about $28.2\%$, from $1.4602287461\ldots$ to
$1.8721429996\ldots$.  The three-point constant is unchanged because $\SF{2}=1$.

\subsection{Why the universal divisor still cannot close the conjecture}

The Barnes asymptotic gives
\begin{equation}
 \log\SF{r}
 =\frac{r^2}{2}\log r-\frac{3r^2}{4}+O(r\log r).
 \label{c3:eq-barnes-asymp}
\end{equation}
Since $R_r\sim r^2/2$ and the factor
$r!/|\fall{\thetaq}{r}|$ is only polynomial in $r$,
\begin{equation}
 \widetilde C_r\sim r e^{-3/2}.
 \label{c3:eq-Ctilde-asymp}
\end{equation}
At the same time $\alpha_r\sim2/r$.  Thus the lower span supplied by order $r$ has the scale
\[
 \widetilde C_rX^{\alpha_r}
 \asymp r e^{-3/2}X^{2/r}.
\]
Optimizing this expression alone gives $r\sim2\log X$ and only a constant multiple of
$\log X$.  This is a useful consistency check: the universal integer-valued-polynomial
content recovers a logarithmic natural scale, but an interval $[X,2X]$ has length $X$, and a
hypothetical counterexample branch supplies only $\asymp\log X$ candidates there.  The
divisor improves finite-order repulsion but does not create a contradiction with the exact
conditional monoid.

\begin{statusbox}{\statusnew{} --- Theorem~\ref{c3:thm-superfactorial-divisor} and its
repulsion consequences are unconditional.  The asymptotic diagnosis is deliberately stated
as a limitation: the new divisor changes constants, not the best fixed-order exponent or the
one-logarithm population barrier.}
\end{statusbox}

\section{Primitive interpolation and the last determinantal divisor}
\label{sec:smithinterp}

The exact support frontier says that $M+1$ monomials produce a unique relation.  For integral
curve points, that relation has a canonical primitive arithmetic normalization.  The correct
invariant is not one selected determinant but the gcd of \emph{all} maximal minors.

\subsection{The cofactor polynomial}
...
\[
 ...
 \beta\notin\Q,
\]
and form the $M\times(M+1)$ integer matrix
\begin{equation}
 E_M:=
 \begin{pmatrix}
  v_1&1&u_1&\cdots&u_1^{M-1}\\
  v_2&1&u_2&\cdots&u_2^{M-1}\\
  \vdots&\vdots&\vdots&&\vdots\\
  v_M&1&u_M&\cdots&u_M^{M-1}
 \end{pmatrix}.
 \label{c3:eq-EM}
\end{equation}
Full spark gives $\rank E_M=M$.  Let $C_Y,C_0,\ldots,C_{M-1}$ be the signed maximal minors obtained by deleting,
respectively, the columns $Y,1,X,\ldots,X^{M-1}$, and define the last determinantal
divisor
\begin{equation}
 \Delta_M:=\gcd(C_Y,C_0,\ldots,C_{M-1})>0.
 \label{c3:eq-DeltaM}
\end{equation}
For $1\le i\le M$, let
\[
 V_M^{(i)}:=\prod_{\substack{1\le a<b\le M\\a,b\ne i}}(u_b-u_a),
 \qquad
 G_M:=\gcd\bigl(V_M^{(1)},\ldots,V_M^{(M)}\bigr),
\]
with the empty product interpreted as $1$.  Thus $G_M$ depends only on the abscissae, not on
the values $v_i$.

Then
\[
 (c_Y,c_0,\ldots,c_{M-1})
 :=\frac1{\Delta_M}(C_Y,C_0,\ldots,C_{M-1})
\]
is the primitive integer generator of $\ker E_M$.  Write
\begin{equation}
 P_M(X,Y)=c_YY+\sum_{j=0}^{M-1}c_jX^j.
 \label{c3:eq-PM}
\end{equation}

\begin{theorem}[Primitive cofactor interpolation package]
\label{c3:thm-smith-package}
With the notation above:
\begin{enumerate}
\item $P_M$ is primitive, vanishes at every $(u_i,v_i)$, and is unique up to sign among
      primitive integer polynomials supported on
      $\{Y,1,X,\ldots,X^{M-1}\}$.
\item Every coefficient is nonzero.  In spectral order the coefficients alternate in sign.
\item If
      \[
       V_M:=\prod_{1\le i<j\le M}(u_j-u_i),
      \]
      then
      \begin{equation}
       |c_Y|=\frac{V_M}{\Delta_M}.
       \label{c3:eq-cY}
      \end{equation}
\item The stronger node-deletion divisor satisfies
      \begin{equation}
       \SF{M-1}\mid G_M\mid\Delta_M.
       \label{c3:eq-F-divides-Delta}
      \end{equation}
\item If $(u,v)=(u,u^\beta)$ is any further integral curve point with
      $u\notin\{u_1,\ldots,u_M\}$, then
      \begin{equation}
       D_{M+1}(u,v)=\pm\Delta_M P_M(u,v),
       \label{c3:eq-augdet-factor}
      \end{equation}
      where $D_{M+1}(u,v)$ is the determinant obtained by appending $(v,1,u,\ldots,u^{M-1})$
      as a final row to $E_M$.  In particular
      \begin{equation}
       P_M(u,v)\in\Z\setminus\{0\},
       \qquad
       |D_{M+1}(u,v)|\ge\Delta_M.
       \label{c3:eq-augdet-lower}
      \end{equation}
\item Writing $f(t)=t^\beta$, one has the exact interpolation remainder
      \begin{equation}
       P_M(u,f(u))
       =c_Y f[u_1,\ldots,u_M,u]
        \prod_{i=1}^{M}(u-u_i),
       \label{c3:eq-primitive-remainder}
      \end{equation}
      and therefore the Diophantine inequality
      \begin{equation}
       \left|c_Y f[u_1,\ldots,u_M,u]
        \prod_{i=1}^{M}(u-u_i)\right|\ge1.
       \label{c3:eq-primitive-ineq}
      \end{equation}
\end{enumerate}
\end{theorem}

\begin{proof}
The signed maximal-minor vector is a nonzero kernel vector of every full-row-rank
$M\times(M+1)$ matrix.  Dividing by the coefficient gcd makes it primitive, proving (1).
Full spark says none of the maximal minors vanishes, and their positivity in spectral order
gives the alternating signs, proving (2).

Deleting the $Y$ column leaves the ordinary Vandermonde matrix
$(u_i^j)_{1\le i\le M,0\le j\le M-1}$, whose determinant has absolute value $V_M$.
The corresponding primitive coefficient is that cofactor divided by $\Delta_M$, proving
\eqref{c3:eq-cY}.

For (4), first delete the $Y$ column.  Its determinant is $V_M$, and every
$V_M^{(i)}$ divides $V_M$, so $G_M\mid V_M$.  If instead one deletes a power column, expand
the resulting maximal minor along its $Y$ column.  The cofactor of row $i$ is a generalized
Vandermonde determinant on the $M-1$ nodes other than $u_i$, with $M-1$ distinct
nonnegative power exponents.  Such a determinant is $V_M^{(i)}$ times an integer Schur
polynomial.  Hence every summand is divisible by $G_M$, and so is every maximal minor.
Therefore $G_M\mid\Delta_M$.  Finally, each $V_M^{(i)}$ is an ordinary Vandermonde on
$M-1$ integer nodes and is divisible by $\SF{M-1}$, proving
$\SF{M-1}\mid G_M$.

Laplace expansion of the appended square determinant along its final row pairs that row with
the signed cofactor vector
$(C_Y,C_0,\ldots,C_{M-1})=\Delta_M(c_Y,c_0,\ldots,c_{M-1})$, giving
\eqref{c3:eq-augdet-factor}.  The augmented matrix is again a square monomial evaluation
matrix at $M+1$ distinct nodes, so Theorem~\ref{c3:thm-fullspark} makes its determinant
nonzero.  Since $P_M(u,v)$ is an integer, (5) follows.

Finally, $-\sum c_jX^j/c_Y$ is the degree-$M-1$ interpolation polynomial for $f$ at the
nodes $u_i$.  The standard Newton remainder formula gives
\eqref{c3:eq-primitive-remainder}; combine it with (5) to obtain
\eqref{c3:eq-primitive-ineq}.
\end{proof}

\begin{corollary}[Maximality of the node-only divisor]
\label{c3:cor-node-maximal}
For fixed integer nodes $u_1,\ldots,u_M$, let $\Delta_M(y)$ denote the last determinantal
divisor of the matrix \eqref{c3:eq-EM} when the first column is an arbitrary ordinate vector
$y=(y_1,\ldots,y_M)^{\mathsf T}\in\Z^M$.  Then $G_M$ is the greatest positive integer that
divides $\Delta_M(y)$ for every $y\in\Z^M$.  Equivalently,
\begin{equation}
 G_M=\gcd_{y\in\Z^M}\Delta_M(y)
     =\gcd_{1\le i\le M}\Delta_M(e_i),
 \label{c3:eq-G-universal}
\end{equation}
where $e_i$ are the standard basis vectors.
\end{corollary}

\begin{proof}
Theorem~\ref{c3:thm-smith-package} proves that $G_M$ divides every maximal minor for every
integer ordinate vector, hence $G_M\mid\Delta_M(y)$ for all $y$.  Conversely, specialize to
$y=e_i$ and delete the highest-power column $X^{M-1}$.  Expansion along the first column
leaves, up to sign, the ordinary Vandermonde $V_M^{(i)}$ on all nodes except $u_i$.
Therefore any integer dividing every $\Delta_M(y)$ divides every $V_M^{(i)}$, and hence their
gcd $G_M$.  The same argument shows that the finite family $y=e_i$ already has gcd $G_M$.
\end{proof}

This maximality statement makes the quotient $\Delta_M/G_M$ canonical: it is exactly the
part of the primitive normalization that cannot be forced from the nodes alone and must come
from the special ordinates.

\begin{corollary}[Exact node-only divisor]
\label{c3:cor-node-divisor}
Put
\[
 R_i:=\prod_{\substack{1\le j\le M\\j\ne i}}|u_i-u_j|.
\]
Then
\begin{equation}
 G_M=\frac{V_M}{\operatorname{lcm}(R_1,\ldots,R_M)}.
 \label{c3:eq-G-lcm}
\end{equation}
For every prime $p$,
\begin{equation}
 v_p(G_M)=
 \sum_{1\le a<b\le M}v_p(u_b-u_a)
 -\max_{1\le i\le M}\sum_{j\ne i}v_p(u_i-u_j).
 \label{c3:eq-G-edge}
\end{equation}
If all $u_i$ are congruent modulo a positive integer $q$, then
\begin{equation}
 q^{\binom{M-1}{2}}\SF{M-1}\mid G_M\mid\Delta_M.
 \label{c3:eq-congruence-G}
\end{equation}
\end{corollary}

\begin{proof}
Since $V_M^{(i)}=V_M/R_i$ and every $R_i$ divides $V_M$, the elementary identity
$\gcd(V/a_1,\ldots,V/a_M)=V/\operatorname{lcm}(a_1,\ldots,a_M)$ proves
\eqref{c3:eq-G-lcm}.  Taking $p$-adic valuations gives \eqref{c3:eq-G-edge}: the first sum is
the total weight of the complete graph on the nodes and the subtracted term is the heaviest
incident star.  For \eqref{c3:eq-congruence-G}, write $u_i=a+qz_i$.  Every deleted
Vandermonde equals $q^{\binom{M-1}{2}}$ times the Vandermonde on the corresponding $M-1$
integer nodes $z_i$, and the latter is divisible by $\SF{M-1}$.
\end{proof}

\begin{proposition}[Leading node-only Smith mass on the $2^i3^j$ grid]
\label{c3:prop-grid-node-mass}
Let $G_L$ be the divisor $G_M$ for the $M=L^2$ abscissae
$u_{i,j}=2^i3^j$, $0\le i,j<L$.  As $L\to\infty$,
\begin{equation}
 v_2(G_L)=\frac{L^5}{6}-\frac{L^4}{4}+O(L^3),
 \qquad
 v_3(G_L)=\frac{L^5}{6}-\frac{L^4}{4}+O(L^3).
 \label{c3:eq-grid-node-mass}
\end{equation}
In particular,
\begin{equation}
 \log G_L\ge
 (\log6)\left(\frac{L^5}{6}-\frac{L^4}{4}\right)-O(L^3).
 \label{c3:eq-grid-node-log}
\end{equation}
\end{proposition}

\begin{proof}
For $p=2$, let $W_2$ be the sum of $v_2(u-u')$ over all unordered pairs of grid nodes.
Pairs with distinct first indices $i<k$ have
$v_2(2^i3^j-2^k3^\ell)=i$, so their contribution is
\[
 L^2\sum_{i=0}^{L-2}i(L-1-i)
 =\frac{L^3(L-1)(L-2)}6
 =\frac{L^5}{6}-\frac{L^4}{2}+O(L^3).
\]
For pairs with the same first index, the common factor $2^i$ contributes exactly
\[
 \binom L2\sum_{i=0}^{L-1}i
 =\frac{L^2(L-1)^2}{4}
 =\frac{L^4}{4}+O(L^3).
\]
The remaining contribution is
\[
 L\sum_{d=1}^{L-1}(L-d)v_2(3^d-1)=O(L^3).
\]
Indeed, LTE gives $v_2(3^d-1)=1$ for odd $d$ and
$2+v_2(d)$ for even $d$, while
$\sum_{d\le L}v_2(d)=O(L)$.  Hence
\[
 W_2=\frac{L^5}{6}-\frac{L^4}{4}+O(L^3).
\]
The $2$-adic weight of any incident star is $O(L^3)$: it has $L^2-1$ edges, and summing the
explicit minimum valuations over the other first indices gives $O(L^3)$; the equal-index
edges contribute only $O(L^2)$.  Formula~\eqref{c3:eq-G-edge} proves the first asymptotic.

For $p=3$, interchange the two grid indices.  The distinct-second-index and common-power
terms are identical.  For the residual equal-index terms, LTE gives
$v_3(2^d-1)=0$ for odd $d$ and $1+v_3(d/2)$ for even $d$, whose weighted sum is again
$O(L^2)$ before multiplication by the outer factor $L$.  The largest star remains $O(L^3)$.
This proves the second asymptotic, and \eqref{c3:eq-grid-node-log} follows by retaining only
the primes $2$ and $3$ in the prime factorization of $G_L$.
\end{proof}

\begin{remark}[What remains output-dependent]
The automatic divisor $G_L$ already has logarithm of order $L^5$, the same order as the
logarithm of the full grid Vandermonde.  It is nevertheless determined entirely by the
abscissae and would be present for arbitrary integer ordinates.  The genuinely new arithmetic
needed for a proof is therefore the excess factor $\Delta_M/G_M$, forced by the special
relations $v_{i,j}=U^iV^j$.  Proposition~\ref{c3:prop-grid-node-mass} converts ``look for
more divisibility'' into a leading-order residual problem rather than a merely qualitative
one.
\end{remark}

\subsection{Smith normal form and local meaning}

Let the Smith normal form of $E_M$ have nonzero invariant factors
\[
 d_1\mid d_2\mid\cdots\mid d_M.
\]
The standard determinantal-divisor theorem gives
\begin{equation}
 \Delta_M=d_1d_2\cdots d_M.
 \label{c3:eq-smith-product}
\end{equation}
Consequently, for every prime $p$,
\begin{equation}
 v_p(\Delta_M)=\sum_{j=1}^{M}v_p(d_j),
 \label{c3:eq-smith-vp}
\end{equation}
and
\begin{equation}
 p\mid\Delta_M
 \quad\Longleftrightarrow\quad
 \rank(E_M\bmod p)<M.
 \label{c3:eq-rankdrop}
\end{equation}
More generally, $v_p(\Delta_M)$ measures the full depth of the rank defect over
$\Z_p$, not just rank loss modulo $p$.

This is the cleanest package for any future adelic argument.  The archimedean side controls
the maximal minors by divided differences and Vandermonde products.  Every $p$-adic
congruence gain appears as a contribution to $v_p(\Delta_M)$.  Dividing the cofactor vector by
$\Delta_M$ simultaneously shrinks all coefficients of the primitive auxiliary polynomial.
The node-only contribution \eqref{c3:eq-F-divides-Delta}, quantified on the rectangular
grid by Proposition~\ref{c3:prop-grid-node-mass}, is only the baseline; a proof must show
that the special output relation forces substantial excess in $\Delta_M/G_M$.

\begin{target}[Smith growth on the exact counterexample grid]
\label{c3:target-smith-growth}
For the points
\[
 u_{i,j}=2^i3^j,
 \qquad
 v_{i,j}=U^iV^j
\]
in a large rectangular or triangular grid, obtain a lower bound for the output-dependent quotient $\Delta_M/G_M$ (or, equivalently,
a lower bound for $\Delta_M$ beyond the node-only baseline) that exceeds the best simultaneous archimedean upper bound for the relevant
maximal minors.  Equivalently, force enough $p$-adic Smith mass that the primitive cofactor
polynomial has coefficients too small to interpolate $x^\beta$ at the next grid point.
\end{target}

\begin{remark}[Why the gcd, not one determinant]
A selected determinant may have a large accidental divisor that disappears when another
column is deleted.  Such a divisor does not make the primitive relation smaller.  Only the
gcd of all maximal minors survives normalization and therefore feeds into
$P_M$.  This is why $\Delta_M$, rather than the valuation of one aesthetically convenient
minor, is the invariant a proof must control.
\end{remark}

\begin{statusbox}{\statusnew{} --- all statements in this section are exact integer linear
algebra except the already established strict-total-positivity input.  The Smith formulation
turns the vague request for ``more $p$-adic divisibility'' into the concrete quantitative
target \ref{c3:target-smith-growth}.}
\end{statusbox}

\section{Sharp sufficient targets for a complete proof}
\label{sec:auxiliarytarget}

The preceding theorems make it possible to state proof targets at the exact threshold, without
hiding a dimension count in asymptotic notation.

\subsection{The one-monomial target}

\begin{theorem}[Sparse small-value criterion]
\label{c3:thm-small-value-criterion}
Assume that a nonintegral counterexample $\beta$ exists and let
$(u_i,v_i)=(u_i,u_i^\beta)\in\N^2$, $1\le i\le M$, be distinct candidate points.  If one can
construct a nonzero polynomial $P\in\Z[X,Y]$ with
\begin{equation}
 |\supp P|\le M
 \qquad\text{and}\qquad
 |P(u_i,v_i)|<1\quad(1\le i\le M),
 \label{c3:eq-small-support-target}
\end{equation}
then a contradiction follows.
\end{theorem}

\begin{proof}
Each $P(u_i,v_i)$ is an integer, so the strict bound makes it zero.  A nonzero polynomial with
at most $M$ monomials cannot vanish at $M$ distinct points by
Theorem~\ref{c3:thm-support-threshold}.
\end{proof}

The strict inequalities in \eqref{c3:eq-small-support-target} may be replaced by a product
formula: it suffices to make the product of the archimedean and all relevant nonarchimedean
norms of each value strictly below $1$.  This is the two-variable analogue of the
``vanishes for analytic reasons'' program raised by Dasgupta--Kakde in their
Question~7.4~\cite{DasguptaKakde2026}.

\begin{corollary}[Exact BK small-value criterion]
\label{c3:cor-bk-small-value}
Under the same hypotheses, a nonzero $P\in\Z[X,Y]$ with
\[
 \BKd(P)\le M-2,
 \qquad
 |P(u_i,v_i)|<1\quad(1\le i\le M)
\]
is impossible and therefore yields a contradiction.
\end{corollary}

\begin{proof}
The values are integral and hence zero.  Theorem~\ref{c3:thm-min-bkd} says that every
nonzero polynomial vanishing at the $M$ points has BK-degree at least $M-1$.
\end{proof}

\subsection{The determinant target}

Choose any $M$ polynomial monomials
$S=\{(a_j,b_j)\}_{j=1}^M\subset\Nzero^2$.  The square determinant
\begin{equation}
 \mathcal D(X,S):=
 \det\bigl(u_i^{a_j}v_i^{b_j}\bigr)_{1\le i,j\le M}
 \label{c3:eq-square-evaldet}
\end{equation}
is a positive integer after ordering spectral weights.  A contradiction follows from any
proof that
\begin{equation}
 0<|\mathcal D(X,S)|<1.
 \label{c3:eq-det-target}
\end{equation}
The archimedean half must exploit more than an entrywise maximum, and the arithmetic half must
exploit more than a divisor already included in that same maximum.  Sections
\ref{sec:minplus}, \ref{sec:smithinterp}, and the second continuation show three increasingly
strict audits of this requirement.

There is an equivalent primitive-interpolation form.  With $M+1$ chosen monomials, let
$P_{\mathrm{prim}}$ be the primitive cofactor relation and $\Delta_M$ its last determinantal
divisor.  If one can prove at a further integral point that
\[
 |P_{\mathrm{prim}}(u,v)|<1,
\]
then integrality contradicts full spark.  By \eqref{c3:eq-augdet-factor}, this asks precisely
for an upper bound on the appended determinant stronger than $\Delta_M$.

\subsection{Three prioritized routes}

\begin{enumerate}
\item \textbf{Adelic sparse auxiliary polynomial.}
Construct $P$ with support at or below the exact $M$-term threshold and make its values small
simultaneously at the real and selected $p$-adic places.  Hermite--Pad\'e methods are relevant
only if they preserve this support budget; a high-degree dense polynomial does not address
the correct obstruction.

\item \textbf{Grid Smith growth.}
Prove a leading-order lower bound for $\Delta_M$ on a rectangular or triangular
$2^i3^j$ grid.  The bound must survive the gcd over all column deletions, not merely one
Vandermonde minor.  A promising decomposition is prime by prime via
\eqref{c3:eq-smith-vp}, with the exact conditional output valuations supplying the local
rank defects.

\item \textbf{Archimedean cancellation beyond total positivity.}
Strict total positivity controls sign but not smallness.  One needs an asymptotic for
\eqref{c3:eq-square-evaldet} or for all maximal minors that is exponentially below the
largest Leibniz term after the arithmetic divisor is removed.  The HCIZ/free-energy program
of the first continuation remains the natural continuous limit, now with the sharp
Smith-divisor normalization identified.
\end{enumerate}

\subsection{What cannot close the one-monomial gap by itself}

Several transformations preserve the exact obstruction.

\begin{itemize}
\item \textbf{Changing the monomial shape.}  Full spark depends only on the number of distinct
      spectral weights, not on whether the support is a triangle, rectangle, thin strip, or
      disconnected set.
\item \textbf{Adding derivative conditions.}  Corollary~\ref{c3:cor-multiplicity} charges one
      monomial per unit of total multiplicity.
\item \textbf{Taking finite differences or translates.}  These may lower values or create
      extra zeros, but each translate also moves or erodes the finite grid.  Unless an analytic
      estimate supplies an additional smallness gain, the number of surviving independent
      conditions never overtakes the number of resulting spectral terms.
\item \textbf{Clearing denominators after rational interpolation.}  Clearing denominators
      restores integrality but enlarges coefficients.  The primitive normalization is already
      exactly measured by $\Delta_M$; no further free gcd is available.
\item \textbf{Pila--Wilkie-type counting alone.}  Polylogarithmically many integral points are
      compatible with every $H^\varepsilon$ bound.  Counting can constrain density but cannot
      remove the first exceptional point.
\end{itemize}

The resulting target is narrow but honest: one must beat exact interpolation by one monomial,
one unit of BK-degree, or one unit in the product formula.  Any claimed proof that does not
identify where that unit comes from is merely repackaging linear algebra.

\section{Recent literature reconnaissance, 2023--2026}
\label{sec:recentlit}

\subsection{Search protocol and negative result}

The search was conducted through August 21, 2026 using combinations of
``Alaoglu--Erd\H{o}s'', ``four exponentials'', ``matrices of logarithms'', ``matrix
coefficient conjecture'', ``exponentials of products'', ``irrational powers'',
``transcendence measure'', and ``auxiliary polynomial'', with follow-up inspection of cited
and citing papers.  arXiv records and journal metadata were checked separately where a paper
had moved to publication.  No credible announcement or proof of the four exponentials
conjecture, the restricted Alaoglu--Erd\H{o}s instance, or an equivalent $2\times2$
logarithm-matrix statement was found.  The recent papers below sharpen the surrounding
technology or reformulate the bottleneck; none supplies the missing final implication.

This is necessarily a dated literature statement, not a mathematical theorem.  The exact
new deductions in Sections~\ref{sec:mcc}--\ref{sec:smithinterp} are independent of it.

\subsection{Dasgupta--Kakde: the central structural development}

Dasgupta's 2023 expository paper places the four exponentials conjecture inside the
Structural Rank Conjecture for matrices of logarithms and reviews the Waldschmidt--Masser
partial rank theorem~\cite{Dasgupta2023}.  Dasgupta and Kakde then introduced the Matrix
Coefficient Conjecture in arXiv:2408.08178; the paper was published as article 110753 in
volume 487 of \emph{Advances in Mathematics} in March 2026~\cite{DasguptaKakde2026}.

Three points are directly relevant here.
\begin{enumerate}
\item In dimension two, their Matrix Coefficient Conjecture is equivalent to four
      exponentials.  Theorem~\ref{c3:thm-restricted-mcc} above proves that its restriction to
      the matrices \eqref{c3:eq-logmatrix} is already exactly Alaoglu--Erd\H{o}s.
\item Their proof strategy factors the problem into a transcendence-to-auxiliary-polynomial
      implication $(w)\Rightarrow(m)$ and an algebraic-geometric implication
      $(m)\Rightarrow(o)$.  They prove the latter.  In two variables they also show that a
      still weaker fewnomial condition $(m')$ would suffice, but explicitly leave
      $(w)\Rightarrow(m')$ open.
\item Their Section~7.2 asks for auxiliary polynomials that vanish for genuinely analytic or
      adelic reasons: $p$-adic smallness and archimedean control should combine through the
      product formula.  The exact support and BK frontiers in this continuation quantify the
      special $2^i3^j$ instance of that request.
\end{enumerate}

The specialization is somewhat sobering.  On a hypothetical counterexample grid
$\mathcal X(2N)$, every exact vanishing polynomial has at least $4N^2+1$ monomials and
BK-degree at least $4N^2-1$, while the Dasgupta--Kakde sufficient conditions ask for a budget
below $N^2$.  Thus the missing construction cannot be obtained by ordinary interpolation plus
a better choice of Newton polygon.  It must exploit analytic smallness before integrality is
invoked, or force an arithmetic collapse that is itself tantamount to the desired rational
coefficient.

\subsection{Effective approximation and new determinant technology}

Calegari, Dimitrov, and Tang develop quantitative arithmetic holonomy bounds and derive
effective Diophantine approximation results on $\mathbf P^1$ and $\mathbf G_m$, including
new routes to classical effective $S$-unit, Thue--Mahler, and power-approximation
consequences~\cite{CalegariDimitrovTang2025}.  The work is conceptually relevant because it
extracts effective irrationality and linear-independence measures from a geometric
construction rather than beginning with a conventional Baker lower bound.  It does not,
however, furnish a bound for the bilinear logarithmic identity
\[
 \log U\log3-\log V\log2=0,
\]
whose ``coefficients'' are themselves logarithms of algebraic numbers.  No specialization
found in the paper crosses that coefficient barrier.

Fischler and Rivoal give an improved transcendence measure for $e^\alpha$ at nonzero
algebraic $\alpha$, using an optimized Hermite--Pad\'e determinant construction
~\cite{FischlerRivoal2025}.  This is relevant methodologically: it demonstrates that a
seemingly secondary accessory parameter in a Siegel determinant can materially improve the
final exponent.  Direct application fails here because the exponents
$\beta\log2=\log U$ and $\beta\log3=\log V$ are logarithms of algebraic numbers, not nonzero
algebraic arguments.  The useful lesson is architectural rather than a ready-made estimate:
any successful sparse auxiliary construction should optimize support, multiplicity, height,
and local smallness jointly rather than sequentially.

\subsection{Point counting and additive structure of irrational powers}

Vermeulen proves Pila--Wilkie-type rational-point bounds for transcendental curves in
arbitrary $1$-h-minimal valued fields of mixed characteristic, by parametrization and a
determinant method~\cite{Vermeulen2025}.  This broadens the nonarchimedean counting toolkit,
but it does not settle the present problem.  Over the reals, the graph $y=x^{\thetaq}$ is a
transcendental definable curve and polylogarithmically many integral points are fully
compatible with any $H^\varepsilon$ upper bound.  Over a valued field, the fixed real
irrational exponent does not automatically define the same analytic curve.  The obstacle is
not an excess of rational points of bounded height; it is the possible existence of the first
nontrivial integral point and the exact multiplicative branch it would generate.

Harrison studies additive relations in sets of irrational powers and proves, among other
results, asymptotically maximal additive expansion for broad classes of irrational exponents,
with effective nonvanishing criteria near suitable rational approximants
~\cite{Harrison2026}.  This strengthens the general principle behind the progression
obstructions in Section~\ref{sec:differences}: irrational powers usually destroy additive
structure.  It does not exclude $m^{\thetaq}\in\Z$ for an isolated $m$, because that condition
is multiplicative and one-term rather than an additive relation among several independent
bases.  A possible future use would be to combine Harrison-type additive expansion with the
exact conditional monoid to forbid unusually structured subsets of candidates; by itself it
cannot remove the primitive generator.

\subsection{A proof audit of a potentially decisive 2025 preprint}

Massold's arXiv:2506.01123 studies transcendence degrees of fields generated by
$\exp(\theta_i\kappa_j)$ and proves lower bounds under quantitative genericity hypotheses
~\cite{Massold2025}.  Corollary~1.6 of the posted version states a lower bound which, if its
written proof applied at $m=n=2$, would imply that at least one of the four exponentials is
transcendental and hence would settle the present problem.

The displayed parameter check does not cover that boundary case as written.  The proof sets
\[
 t=\left\lfloor\sqrt{\frac{n+1}{2}}\right\rfloor,
 \qquad
 \mu=\nu=2t^2-1,
\]
and invokes Theorem~II.2, whose first hypothesis is
\[
 \frac{\mu\nu}{\mu+\nu}>t.
\]
For $m=n=2$ one gets $t=1$ and $\mu=\nu=1$, so the left side is $1/2$, not greater than
$1$.  Therefore the displayed derivation does not establish the $2\times2$ case.  Only
version~1 was found on arXiv during this search, and no correction resolving this boundary
substitution was located.  This observation is deliberately limited: it identifies a gap in
the written deduction of the decisive special case; it is not a judgment on the paper's
higher-dimensional results.

The episode is a useful audit warning.  Any transcendence-degree estimate whose rounded lower
bound first becomes positive at $m=n=2$ must check the strict inequalities at that endpoint
separately.  The four exponentials boundary is exactly where asymptotic parameter choices are
least reliable.

\subsection{Formalized transcendence theory}

Karatarakis and Wiedijk posted a 2026 preprint describing their Lean~4 formalization of the
Gelfond--Schneider theorem, including the auxiliary-function, growth,
algebraic-number-field, and complex-analytic machinery needed for Hilbert's seventh
problem~\cite{KaratarakisWiedijk2026}.  This supplies a citable account of the development
from which the ProveIt corpus already carries a pinned port (Mathlib PR~\#42911), as recorded
in Section~\ref{co:transcendence} and Appendix~\ref{app:inventory}.  The present literature
audit checked the preprint's stated scope; the repository audit and kernel status of the port
remain those documented earlier in this report.

The work does not formalize four or six exponentials, Baker's theorem, or the
Waldschmidt--Masser machinery.  It nevertheless confirms that Gelfond--Schneider is no longer
a greenfield formalization item; the unresolved formal gap begins at the higher-rank
logarithm-matrix input.

\subsection{An August 2026 application-side bypass}

Bustos-Gajardo, Fokkink, and Yassawi prove a Cobham theorem for Gaussian-integer
numeration systems in arXiv:2510.01440v3~\cite{BustosGajardoFokkinkYassawi2026}.
Earlier work of Hansel and Safer had used the four exponentials conjecture to obtain density of
a multiplicative subgroup of $\C^\times$ and thereby a syndeticity conclusion.  The new proof
observes that the automata-theoretic argument needs only that $1$ is not isolated in the subgroup,
a much weaker fact that follows elementarily from multiplicative independence.  It then proves
the stronger eventual-periodicity statement without four exponentials.

This does not weaken, refute, or prove Conjecture~\ref{conj:AE}.  It is nevertheless relevant to
proof strategy: some problems historically advertised as applications of four exponentials may
use far less than its full transcendence content.  Such applications should therefore be audited
for the weakest local input before being treated as routes back to the conjecture.  Conversely,
the direct Gaussian-integer equivalence proved in Theorem~\ref{va:thm-gaussian} is not bypassed by
this automata argument; it is an arithmetic statement about simultaneous powers themselves.

\subsection{Comparative relevance table}

\begin{center}
\small
\begin{tabularx}{0.98\textwidth}{>{\raggedright\arraybackslash}p{0.25\textwidth}
                                  >{\raggedright\arraybackslash}X
                                  >{\raggedright\arraybackslash}X}
\toprule
Work & What it adds & Why it does not finish Alaoglu--Erd\H{o}s \\
\midrule
Dasgupta--Kakde~\cite{DasguptaKakde2026} & Matrix Coefficient Conjecture; BK/fewnomial
auxiliary-polynomial program & the decisive $(w)\Rightarrow(m)$ and
$(w)\Rightarrow(m')$ implications remain open \\
Calegari--Dimitrov--Tang~\cite{CalegariDimitrovTang2025} & new effective approximation and
linear-independence mechanisms on $\mathbf G_m$ & no estimate for bilinear forms with
logarithmic coefficients \\
Fischler--Rivoal~\cite{FischlerRivoal2025} & improved Hermite--Pad\'e transcendence measures
for $e^\alpha$, $\alpha$ algebraic & the relevant exponent arguments are logarithmic, not
algebraic \\
Vermeulen~\cite{Vermeulen2025} & valued-field Pila--Wilkie counting for transcendental curves
& polylogarithmic candidate growth is compatible with $H^\varepsilon$ counting \\
Harrison~\cite{Harrison2026} & strong additive expansion/nonvanishing for irrational powers &
integrality of one irrational power is a multiplicative one-term condition \\
Massold~\cite{Massold2025} & new transcendence-degree bounds for exponentials of products &
the posted endpoint parameter check fails at $m=n=2$ \\
Karatarakis--Wiedijk~\cite{KaratarakisWiedijk2026} & Lean formalization of
Gelfond--Schneider & does not reach four exponentials or logarithm-matrix rank \\
Bustos-Gajardo--Fokkink--Yassawi~\cite{BustosGajardoFokkinkYassawi2026} &
Gaussian-integer Cobham theorem without four exponentials & bypasses one former application by
using a strictly weaker non-isolation input; it does not constrain simultaneous irrational powers \\
\bottomrule
\end{tabularx}
\end{center}

\begin{statusbox}{\statusknown{} for the cited literature; \statusnew{} for the explicit
specializations and endpoint audit given here.  The literature search found a much cleaner
modern language for the obstruction, but no theorem that removes it.}
\end{statusbox}

\section{Further deductions and proof-route audits}
\label{sec:thirdattempts}

\subsection{Boundary-defect compression under the two dilations}

The primitive interpolation polynomial also remembers that the rectangular grid is stable
under multiplication by $2$ and $3$ away from its two outer faces.  This yields an exact
degree compression not visible in a raw cofactor determinant.

\begin{theorem}[Exact rectangular-grid dilation factorization]
\label{c3:thm-dilation-defect}
Assume a counterexample $\beta\notin\Q$ exists, put $U=2^\beta$ and $V=3^\beta$, and let
$L\ge2$, $M=L^2$.  Choose the sign of the primitive interpolation polynomial on
$\mathcal X(L)$ so that
\begin{equation}
 P_L(X,Y)=c_YY-R_L(X),
 \qquad c_Y>0,
 ...
 \label{c3:eq-PL-RL}
\end{equation}
Then $\deg R_L=M-1$.  Define the two monic interior-node polynomials
\begin{align}
 A_{2,L}(X)&:=
 \prod_{i=0}^{L-2}\prod_{j=0}^{L-1}(X-2^i3^j),
 \\
 A_{3,L}(X)&:=
 \prod_{i=0}^{L-1}\prod_{j=0}^{L-2}(X-2^i3^j).
 \label{c3:eq-interior-node-polys}
\end{align}
There are unique nonzero polynomials $B_{2,L},B_{3,L}\in\Z[X]$, each of degree exactly
$L-1$, such that
\begin{align}
 R_L(2X)-U R_L(X)&=A_{2,L}(X)B_{2,L}(X),
 \label{c3:eq-2-defect}\\
 R_L(3X)-V R_L(X)&=A_{3,L}(X)B_{3,L}(X).
 \label{c3:eq-3-defect}
\end{align}
They satisfy the exact commutator syzygy
\begin{align}
&A_{2,L}(3X)B_{2,L}(3X)-V A_{2,L}(X)B_{2,L}(X)
\notag\\
&\qquad =A_{3,L}(2X)B_{3,L}(2X)-U A_{3,L}(X)B_{3,L}(X),
 \label{c3:eq-defect-syzygy}
\end{align}
and, writing $r_{M-1}=\operatorname{lc}(R_L)$,
\begin{equation}
 \operatorname{lc}(B_{2,L})=(2^{M-1}-U)r_{M-1},
 \qquad
 \operatorname{lc}(B_{3,L})=(3^{M-1}-V)r_{M-1}.
 \label{c3:eq-defect-leading}
\end{equation}
\end{theorem}

\begin{proof}
At every interior node $x=2^i3^j$ with $0\le i<L-1$ one has
\[
 R_L(2x)-UR_L(x)
 =c_YU^{i+1}V^j-Uc_YU^iV^j=0.
\]
Thus $A_{2,L}$ divides the first defect over $\Q[X]$.  The divisor is monic with integer
coefficients and the defect lies in $\Z[X]$, so monic polynomial division shows that the
quotient $B_{2,L}$ lies in $\Z[X]$.  The same argument gives \eqref{c3:eq-3-defect}.

Full spark makes every coefficient of the critical relation nonzero, hence
$\deg R_L=M-1$.  Since $\beta$ is irrational, $U\ne2^{M-1}$ and $V\ne3^{M-1}$.
Consequently both defects have degree $M-1$.  Each interior-node polynomial has degree
$L(L-1)=M-L$, proving that both quotients are nonzero of degree $L-1$ and giving
\eqref{c3:eq-defect-leading} because the divisors are monic.

Finally the dilation operators $T_2:F(X)\mapsto F(2X)$ and
$T_3:F(X)\mapsto F(3X)$ commute.  Therefore
$(T_3-V)(T_2-U)R_L=(T_2-U)(T_3-V)R_L$; substituting
\eqref{c3:eq-2-defect} and \eqref{c3:eq-3-defect} gives
\eqref{c3:eq-defect-syzygy}.
\end{proof}

The theorem compresses each degree-$(L^2-1)$ failure of exact scaling covariance to a
nonzero degree-$(L-1)$ boundary polynomial.  This is a square-root reduction in auxiliary
dimension, but not yet a contradiction: the factorization follows from interpolation and
rectangular closure alone.  Its possible arithmetic value is that the two small boundary
defects are coupled by \eqref{c3:eq-defect-syzygy} and inherit the primitive normalization
of $P_L$.

\begin{target}[Boundary-defect content]
Use the integral syzygy \eqref{c3:eq-defect-syzygy} to force output-dependent common content,
resultant divisibility, or local rank loss in $B_{2,L}$ and $B_{3,L}$ beyond the maximal
node-only factor $G_M$.  Any such gain must survive primitive normalization and therefore
contribute to $\Delta_M/G_M$.
\end{target}

\subsection{No monomial projection escape}

A common hope is to map the curve by carefully chosen monomials and then find a low-degree
relation in the image.  Full spark rules out every finite version of this idea at once.

\begin{corollary}[Monomial reparametrizations preserve maximal independence]
\label{c3:cor-no-projection}
Let $\phi:(\R_{>0})^2\to(\R_{>0})^H$ be any monomial map
\[
 \phi(X,Y)=(X^{a_1}Y^{b_1},\ldots,X^{a_H}Y^{b_H})
\]
with distinct exponent pairs.  On any $M$ distinct points of
$Y=X^\beta$, $\beta\notin\Q$, the homogeneous evaluation matrix has rank
$\min(M,H)$.  If none of the exponent pairs is $(0,0)$, then adjoining the constant
coordinate gives affine-evaluation rank $\min(M,H+1)$.  In particular, no finite monomial
reparametrization creates a linear or affine dependence before the corresponding dimension
count forces one.
\end{corollary}

\begin{proof}
The first statement is Theorem~\ref{c3:thm-fullspark}.  For the second, adjoin the distinct
spectral weight $0$ corresponding to the constant monomial and apply the same theorem to the
$H+1$ columns.
\end{proof}

Thus the exact algebraic-output loci developed in Sections~\ref{sec:spectrum} and
\ref{sec:valuationlattice} cannot be strengthened merely by selecting a more ingenious finite
list of monomial outputs.  A successful output detector must use a genuinely transcendental
operation, a local norm, or an infinite family with uniform control.

\subsection{The rational approximation route revisited}

A hypothetical primitive generator has
\[
 \beta=\frac{\log U}{\log2}=\frac{\log V}{\log3}.
\]
Baker--Matveev theory gives a finite irrationality measure for each fixed such ratio, as
recorded in the second continuation.  Harrison's recent nonvanishing criterion shows that
irrational exponents lying sufficiently close to selected rationals can forbid bounded
additive relations among multiplicatively independent integers~\cite{Harrison2026}.  It is
tempting to apply this with convergents $p/q$ to $\thetaq$.

The scale is wrong for direct integrality.  Writing
\[
 m^{\thetaq}=m^{p/q}\exp\bigl((\thetaq-p/q)\log m\bigr),
\]
a gap of less than $1$ to a nearby integer would require control at scale roughly
$m^{-\thetaq}$, exponentially small in $\log m$.  Continued fractions provide
$|\thetaq-p/q|\asymp q^{-2}$ along good subsequences, while Baker-type lower bounds are
polynomial in $q$.  Neither compares to the exponentially shrinking integer gap uniformly in
$m$.  The exact conditional branch can choose $m$ exponentially large relative to $q$, so no
contradiction follows.

The useful residual target is not a better irrationality measure for $\thetaq$ alone, but a
\emph{correlated} measure involving the candidate height:
\begin{target}[Height-coupled approximation]
Prove that whenever $m^{\thetaq}\in\Z$ and $m$ is not a power of $2$, every rational $p/q$
satisfies a lower bound for
$|\thetaq-p/q|\log m$ strong enough to exceed the nearest-integer tolerance forced by
$m^{\thetaq}$.  Such a bound must use the integrality of the output, not merely the fixed
number $\thetaq$.
\end{target}
This target is another form of the missing logarithmic-coefficient estimate.

\subsection{Why exact interpolation is a theorem about failure, not a candidate proof}

The polynomial $P_{\mathrm{int}}=c_YY-Q(X)$ of
Theorem~\ref{c3:thm-min-bkd} is sometimes psychologically misleading.  It has the smallest
possible BK-degree and vanishes on every selected candidate point.  Could one compare it with
the function $Y-X^\beta$ and use convexity to obtain a further zero?  The answer is no without
...
\[
 E(x)=c_Yx^\beta-Q(x)
\]
is an exponential polynomial with exactly $M+1$ spectral terms and has exactly the permitted
$M$ prescribed zeros.  Total positivity determines the sign of $E$ between and outside the
nodes, but it does not force another zero.  Indeed, the alternating cofactor signs are exactly
the sign pattern of an extremal Chebyshev interpolation error.  The construction saturates,
rather than violates, the zero-count theorem.

One therefore needs an arithmetic statement incompatible with this extremal error: for
example, that $E(u)$ is a nonzero integer of absolute value $<1$ at one more candidate, or
that its primitive coefficient $c_Y=V_M/\Delta_M$ is too small for the divided-difference
remainder.  This is precisely the Smith target of Section~\ref{sec:smithinterp}.

\subsection{A local-global rank formulation}

The matrix $E_M$ has full rank over $\Q$, yet a large $\Delta_M$ means it is close to rank
deficient at many $p$-adic precisions.  This suggests the following local-global dichotomy.

\begin{target}[Accumulated local rank defect]
For a counterexample grid, prove that either
\begin{enumerate}
\item some fixed small set of primes contributes
      $\sum_pv_p(\Delta_M)\log p$ above the archimedean cofactor budget; or
\item the contributing primes proliferate with enough total logarithmic weight to force an
      external prime in the primitive output cores contrary to the exact valuation-lattice
      constraints of Sections~\ref{sec:lattice} and~\ref{sec:valuationlattice}.
\end{enumerate}
\end{target}

The first branch is a strengthened $2,3$-adic determinant argument.  The second attempts to
turn failure at the structural primes into forbidden prime support.  The min-plus ceilings in
Section~\ref{sec:minplus} show that entrywise valuation estimates alone cannot establish the
first branch; the Smith formulation makes clear that one needs correlated row/column rank
defects, not independent termwise divisibility.

\subsection{Critical-support rigidity under several relations}

The generalized Hamming theorem supplies one additional no-go result that is easy to miss.
Suppose an auxiliary construction produces not one but $d$ independent polynomials vanishing
on the same $M$ points, all drawn from one ambient monomial set.  Their combined support has
at least $M+d$ monomials.  Thus producing many relations does not amortize the support cost:
each new independent relation costs one new monomial globally.  Any multi-polynomial version
of the Dasgupta--Kakde strategy must gain through mixed volume or local smallness, not through
sharing a fixed sparse dictionary.

\begin{corollary}[No support amortization]
\label{c3:cor-no-amortization}
Let $P_1,\ldots,P_d$ be linearly independent Laurent polynomials vanishing at $M$ distinct
points of an irrational power curve.  Then
\[
 \left|\bigcup_{j=1}^{d}\supp P_j\right|\ge M+d.
\]
The bound is sharp.
\end{corollary}

\begin{proof}
Apply Theorem~\ref{c3:thm-mds} to the $d$-dimensional coefficient subspace.  Sharpness is the
attainment statement in that theorem.
\end{proof}

\subsection{A compact statement of the remaining wall}

All new routes can be summarized in one diagram:
\[
\begin{array}{c}
\det L(U,V)=0
\quad\Longleftrightarrow\quad
L(U,V)=\binom{\log2}{\log3}(1,\beta)
\\[3pt]
\Downarrow\quad\text{(counterexample)}\\[3pt]
\text{integral grid }\mathcal X(L)\subset\{Y=X^\beta\},\quad\beta\notin\Q
\\[3pt]
\Downarrow\\[3pt]
\begin{array}{c}
\text{full spark: }M\text{ points require }M+1\text{ monomials},\\
\text{BK frontier: minimum }M-1,\\
\text{primitive circuit normalized by }\Delta_M
\end{array}
\\[3pt]
\Downarrow\\[3pt]
\boxed{\text{prove analytic smallness, excess Smith divisibility, or a rational kernel.}}
\end{array}
\]
The last box is not three unrelated options.  Through the product formula they are three
languages for the same missing unit of complexity.

\section{Lean formalization blueprint for the new continuation}
\label{sec:thirdformalization}

The new results divide naturally into six modules.  None requires Baker theory or six
exponentials; only the sign/full-spark theorem requires real analysis beyond finite-dimensional
algebra.

\subsection{Module \lean{RestrictedMatrixCoefficient}}

Suggested definitions:
\begin{lstlisting}[style=leanstyle]
def logMatrix (U V : Nat) : Matrix (Fin 2) (Fin 2) Real := ...

def RestrictedMCC : Prop :=
  forall U V : Nat, 0 < U -> 0 < V ->
    Matrix.det (logMatrix U V) = 0 ->
    exists w v : Fin 2 -> Rat,
      And (Not (w = 0)) (And (Not (v = 0)) ...)
\end{lstlisting}

Principal theorems:
\begin{enumerate}
\item factorization of $w^{\mathsf T}L(U,V)v$ under determinant zero;
\item nonvanishing of $w_1\log2+w_2\log3$ for nonzero rational $w$;
\item rationality of $\beta$ from a Matrix Coefficient witness;
\item integrality of a rational $\beta$ from $2^\beta\in\N$;
\item equivalence of \lean{RestrictedMCC} with the existing conjecture statement.
\end{enumerate}
The only transcendence-free analytic facts needed are positivity and injectivity of the real
logarithm.  Multiplicative independence is an integer valuation argument.

\subsection{Module \lean{PowerCurveFullSpark}}

Parameterize monomials by $s:\Z\times\Z$ and define their spectral weight
$\lambda_s=a+\beta b$.  Irrationality proves injectivity of this weight map.  Two proof routes
are available.

\begin{description}
\item[Reuse route.] Instantiate the existing generalized Vandermonde positivity theorem from
\lean{ExponentialPolynomialZeros} and the ordered-chamber proof already mirrored in
Appendix~\ref{app:gvproof}.
\item[Confluent route.] Formalize the extended Chebyshev zero count by induction using Rolle's
theorem, then deduce nonvanishing of every square minor.  This route also yields
Corollary~\ref{c3:cor-multiplicity} directly.
\end{description}

The output should be a theorem saying that every square submatrix selected by injective row
and column maps has nonzero determinant.  Rank maximality then follows by choosing a square
submatrix of order $\min(M,H)$.

\subsection{Module \lean{SparsePowerCurveInterpolation}}

This module is finite linear algebra on top of full spark:
\begin{enumerate}
\item support lower bound $H\ge M+1$;
\item exact kernel dimension $\max(0,H-M)$;
\item primitive cofactor vector at $H=M+1$;
\item no zero coefficient and alternating signs;
\item generalized Hamming weights $M+d$;
\item exact total-degree and bidegree thresholds by finite-cardinality formulas.
\end{enumerate}

For coefficient fields $\Q$ and $\Z$, avoid a separate rational-kernel proof: use signed
maximal minors directly.  Primitivity is division by the gcd of a finite nonzero integer
family.

\subsection{Module \lean{BKFrontierTwo}}

The key lattice lemma can be kept independent of analysis:
\begin{lstlisting}[style=leanstyle]
theorem support_card_sub_two_le_bkDegree
    (P : MvPolynomial (Fin 2) Complex)
    (hmono : noCommonMonomial P) :
    P.support.ncard - 2 <= bkDegreeTwo P := ...
\end{lstlisting}

The two-dimensional branch uses the formalized Pick theorem if available; otherwise a
special-purpose lemma for lattice polygons may be isolated behind a named interface.  The
collinear branch needs only primitive lattice segment length and a coordinate-width bound.
The upper construction is the cofactor polynomial on
$\{Y,1,X,\ldots,X^{M-1}\}$, whose Newton polygon is an explicit triangle.  This proves the
exact minimum $M-1$ without a general mixed-volume library.

\subsection{Module \lean{SuperfactorialDeterminant}}

This is entirely algebraic.  Define the binomial evaluation matrix and a triangular change of
basis using Stirling numbers.  Prove
\[
 \det(\text{power columns},y)=\SF{r}\det(\text{binomial columns},y).
\]
An alternative avoids explicit Stirling matrices: prove by induction that the $k$-th power
column is an integer combination of $j!\binom mj$, $j\le k$, with leading coefficient $k!$.
Then instantiate the existing divided-difference theorem to obtain
Theorem~\ref{c3:thm-super-repulsion}.

The constants in Table~\ref{c3:tab-superconstants} should be generated as rational interval
certificates around $\thetaq$, rather than trusted decimal literals.

\subsection{Module \lean{PrimitiveInterpolationSmith}}

Use the existing Smith-normal-form API for integer matrices if sufficient; otherwise separate
the module into:
\begin{enumerate}
\item cofactor kernel and primitive gcd normalization;
\item the exact node-deletion divisor $G_M$, its weighted-edge valuation formula, and
      divisibility of generalized Vandermonde minors;
\item the abstract theorem identifying the gcd of maximal minors with the product of Smith
      invariant factors;
\item augmented determinant factorization and Newton interpolation remainder.
\end{enumerate}

The generalized Vandermonde divisibility can be formalized without Schur polynomials.  For
$n$ integer nodes and any $n$ integer polynomials whose degrees are distinct, apply repeated
finite differences after sorting the nodes, or express each monomial in the binomial basis and
use determinantal divisors.  This gives the needed $\SF n$ factor in a form more compatible
with Mathlib's integer-valued-polynomial infrastructure.

\subsection{Recommended dependency order}

\[
\begin{array}{c}
\lean{RestrictedMatrixCoefficient}
\qquad
\lean{SuperfactorialDeterminant}
\\[4pt]
\lean{PowerCurveFullSpark}
\\[4pt]\downarrow\\[-2pt]
\lean{SparsePowerCurveInterpolation}
\\[4pt]\swarrow\qquad\searrow\\[-2pt]
\lean{BKFrontierTwo}
\qquad
\lean{PrimitiveInterpolationSmith}
\end{array}
\]
The restricted Matrix Coefficient equivalence and the superfactorial divisor can be completed
first: they are small, self-contained, and do not depend on the analytic full-spark module.

\begin{statusbox}{\statusopen{} --- this is a formalization plan, not a claim that the modules
exist.  The statements are chosen so that the most reusable new facts --- full spark,
superfactorial divisibility, and primitive determinantal normalization --- are independent of
the Alaoglu--Erd\H{o}s-specific notation.}
\end{statusbox}

\section{Conclusions of the third continuation}
\label{sec:thirdconclusion}

The Alaoglu--Erd\H{o}s conjecture remains open.  The literature search found no valid proof
through August 21, 2026, and the central recent framework of Dasgupta--Kakde explicitly leaves
open the auxiliary-polynomial implication that would prove four exponentials.  What changes
is the precision with which the special $2,3$ instance can now be located inside that
framework.

First, the conjecture is exactly the Matrix Coefficient Conjecture on the matrices
\[
 \begin{pmatrix}\log2&\log U\\\log3&\log V\end{pmatrix}
 \quad(U,V\in\N,\ \det=0).
\]
The general conjecture allows a rational coefficient after changing bases on both sides.  In
this family the left alternative is impossible by unique factorization, so the desired
coefficient is exactly a rational right-kernel vector, equivalently $\beta\in\Q$, and then
integrality upgrades it to $\beta\in\Z$.

Second, an irrational power curve is maximally hostile to sparse algebraic interpolation.
Every finite monomial evaluation matrix is full spark.  Therefore $M$ points require exactly
$M+1$ monomials for their first relation; the critical relation is unique, has no zero
coefficient, and alternates in spectral order.  The total-degree, bidegree, multiplicity, and
multi-relation costs are all exact.  In coding language the relation space is MDS.

Third, the same rigidity has an exact Newton-polygon form.  The least BK-degree of a
polynomial through $M\ge2$ points is $M-1$, attained by $c_YY-Q(X)$.  On the
Dasgupta--Kakde grid $\mathcal X(2N)$ this gives lower bounds $4N^2+1$ for support and
$4N^2-1$ for BK-degree, against their sufficient budget $N^2$.  Ordinary interpolation is not
merely inefficient; it is exactly optimal on the wrong side of the needed threshold.

Fourth, the arbitrary-node determinant has more universal arithmetic content than the earlier
report recorded.  It is divisible by $\SF r=\prod_{j<r}j!$, which improves every fixed-order
repulsion constant from order three onward.  The best exponent remains the order-four value
$0.2415037499\ldots$, with constant improved to $1.8721429996\ldots$.  The Barnes asymptotic
also proves why this cannot be the missing mechanism: even with growing order, the universal
content creates only a logarithmic span scale.

Finally, the primitive interpolation polynomial is controlled by the last determinantal
divisor $\Delta_M$, the product of the Smith invariant factors.  Its node-only part $G_M$
has an exact deleted-Vandermonde formula; on an $L\times L$ grid, both
$v_2(G_M)$ and $v_3(G_M)$ are
$L^5/6-L^4/4+O(L^3)$.  This is the correct meeting
point of all places.  Archimedean divided differences bound its maximal minors; $p$-adic rank
defects enlarge their gcd; and an additional candidate would force the nonzero integer
$P_M(u,u^\beta)$.  The rectangular-grid primitive interpolant also admits the exact dilation factorizations
\eqref{c3:eq-2-defect}--\eqref{c3:eq-defect-syzygy}: its two degree-$(L^2-1)$ scaling
defects collapse to coupled degree-$(L-1)$ integer boundary polynomials.  This offers a new,
low-dimensional place to search for the output-dependent Smith excess.

A complete proof can now be stated in exact units: construct an auxiliary polynomial one
monomial below interpolation, one unit below the BK frontier, or make the primitive remainder
one unit smaller than integrality permits.

\begin{resultbox}
\textbf{Bottom line of this continuation.}
No complete proof was obtained, but the modern matrix-coefficient/fewnomial program can be
specialized exactly.  The conjecture sits at a rigid one-monomial interpolation boundary.  A
successful proof must supply something not present in ordinary linear algebra: adelic
smallness before values are known to vanish, leading-order Smith divisibility that survives
all maximal minors, or an archimedean cancellation estimate beyond the total-positive
largest-term scale.  The report now isolates those possibilities with sharp thresholds and
provides several unconditional theorems that can be formalized independently.
\end{resultbox}
...
Restricted Matrix Coefficient equivalence & \statusnew & Section~\ref{sec:mcc} \\
Full-spark theorem; exact support, multiplicity, degree, bidegree, and MDS frontiers &
\statusnew & Section~\ref{sec:fullspark} \\
Exact Bernstein--Kushnirenko frontier & \statusnew & Section~\ref{sec:bkfrontier} \\
Universal superfactorial divisor and strengthened arbitrary-node repulsion & \statusnew &
Section~\ref{sec:superfactorial} \\
Primitive cofactor polynomial, node-deletion divisor, and Smith-normal-form target &
\statusnew & Section~\ref{sec:smithinterp} \\
2023--2026 literature reconnaissance and endpoint audit & \statusnew & Section~\ref{sec:recentlit} \\
\bottomrule
\end{longtable}
}
...
\begin{description}[leftmargin=3.2cm,style=nextline,itemsep=0.9em]
...
Karatarakis and Wiedijk's 2026 preprint documents their Lean~4 formalization of the
Gelfond--Schneider theorem, including a formalized Siegel lemma for algebraic integers and the
auxiliary-function machinery~\cite{KaratarakisWiedijk2026}; that development is the basis of
the pinned port used by this corpus and cited in Appendix~\ref{app:classical}.  Mathlib's
Lindemann--Weierstrass development supplies analytic scaffolding only.  No formalization of
Baker's theorem, of the four- or six-exponentials machinery, or of the
Waldschmidt--Masser logarithm-matrix theorem was found, so the higher-rank transcendence inputs
isolated behind interfaces in this report remain greenfield work.
...
% --- imported with the third continuation ---
...
\newblock Essential Number Theory \textbf{2} (2023), 93--138; arXiv:2303.02037.
...
\newblock Advances in Mathematics \textbf{487} (2026), article 110753; arXiv:2408.08178.
...
\newblock \emph{A new transcendence measure for the values of the exponential function at
algebraic arguments},
\newblock arXiv:2502.17992v1, 2025.

\bibitem{Vermeulen2025}
F.~Vermeulen,
\newblock \emph{Counting rational points on transcendental curves in valued fields},
\newblock arXiv:2506.19411, 2025 (version 2).

\bibitem{Harrison2026}
J.~Harrison,
\newblock \emph{Additive relations in irrational powers},
\newblock arXiv:2512.04081, 2025 (version 3, 2026).

\bibitem{Massold2025}
H.~Massold,
\newblock \emph{Transcendence degrees of fields generated by exponentials of products},
\newblock arXiv:2506.01123v1, 2025.

\bibitem{KaratarakisWiedijk2026}
M.~Karatarakis and F.~Wiedijk,
\newblock \emph{A formalization of the Gelfond--Schneider theorem},
\newblock arXiv:2603.24823, 2026.

\bibitem{BustosGajardoFokkinkYassawi2026}
