...
$\log2/\log3$ is only polynomial in $s_j$.  This produces a sharp ``height exponent zero''
...
At the same time
\[
 \sigma_j-\frac{\log2}{\log3}=\Theta(\eps_j),
...
\[
 \phi(t)=\frac{2^t-1}{3^t-1},\qquad \phi(0)=\frac{\log2}{\log3},
\]
and, for fixed $m\ge1$,
\[
...
\subsection{The normalized $S$-unit point}
...
For every rational prime $p$, put
\[
 m_p=v_p(M),\qquad a_p=v_p(A),
\]
and let $\mathbf 1_{p=q}$ denote the indicator of $p=q$.
...
\begin{equation}
 ...
 \max\!\left\{
 0,
 m_p-\beta\mathbf1_{p=2},
 a_p-\beta\mathbf1_{p=3}
 \right\}\log p.}
 \label{eq:kappa}
\end{equation}
Then
\begin{equation}
 h(1:u_j:v_j)=\kappa(M,A;\beta)s_j+o(s_j).
 \label{eq:projective-asymptotic}
\end{equation}
Moreover
\begin{equation}
 \kappa\ge\max\{\log M_0,\log A_0\}>0.
...
\end{align*}
The local projective contribution is therefore
\[
 \max\{0,-v_p(u_j),-v_p(v_j)\}\log p
 =s_j\max\!\left\{
 0,
 m_p-\frac{r_j}{s_j}\mathbf1_{p=2},
 a_p-\frac{r_j}{s_j}\mathbf1_{p=3}
 \right\}\log p.
\]
Only primes dividing $6MA$ occur.  Since $r_j/s_j\to\beta$, summing the finitely many
terms gives $s_j\kappa+o(s_j)$.  At the archimedean place,
$u_j=2^{\eps_j}$ and $v_j=3^{\eps_j}$, so the contribution is $O(|\eps_j|)=o(1)$.
This proves~\eqref{eq:projective-asymptotic}.

The inequalities $h(1:u:v)\ge h(u)$ and $h(1:u:v)\ge h(v)$, together with
\eqref{eq:hu}--\eqref{eq:hv}, imply~\eqref{eq:kappa-positive}.
\end{proof}
...
\begin{equation}
 ...
 \sum_p\left(
 m_p+a_p-
 \max\{0,m_p-\beta\mathbf1_{p=2},a_p-\beta\mathbf1_{p=3}\}
 \right)\log p.
 \label{eq:lambda-local}
\end{equation}
For $p\ne2,3$, the local coefficient is simply $\min(m_p,a_p)$.  Thus away from the two
structural primes, the linear cancellation rate is exactly the logarithm of the common prime
part of $M$ and $A$.
\end{remark}

\section{$S$-unit secants and exact primitive height}
\label{sec:secant}
...
Finally,
\begin{equation}
 \sigma_j=\frac{\log2}{\log3}
 +\frac{\log2(\log2-\log3)}{2\log3}\,\eps_j
 +O(\eps_j^2).
...
$\sigma_j\to\log2/\log3\in(0,\infty)$, the reduced numerator and denominator differ by only
...
The limit $\log2/\log3$ is transcendental: if it were algebraic irrational,
Gelfond--Schneider applied to $3^{\log2/\log3}=2$ would give a contradiction; if it were
...
\begin{equation}
 s_j^{-C}\ll
 \left|\sigma_j-\frac{\log2}{\log3}\right|
 \ll s_j^{-1},
 \label{eq:sigma-poly}
\end{equation}
and
\begin{equation}
 \frac{-\log\left|\sigma_j-\frac{\log2}{\log3}\right|}
...
\[
 \frac{\log(2^{c_1}3^{c_2})}{\log(2^{d_1}3^{d_2})}
\]
...
denominator in~\eqref{eq:sigma} possess a common divisor of exponential size, but that size
is not free arithmetic information: after division, the surviving height still has the
positive linear slope $\kappa$.
...
$\theta=\log3/\log2$ transcendental, a nonzero rational polynomial in $\theta$ cannot vanish.
\end{proof}
...
Then
\begin{equation}
 K\le\left\lfloor\frac D2\right\rfloor.
 \label{eq:half-bound}
\end{equation}
Equality can occur only if $D$ is even and
\begin{equation}
 P(Z)-Q(Z)=cZ^{D/2}.
 \label{eq:middle-monomial}
\end{equation}
Conversely,~\eqref{eq:middle-monomial} gives equality.
\proved
\end{theorem}
...
$K\le D/2$.  Equality forces the smallest and largest nonzero indices of $H$ both to be
...
\end{equation}
where $\theta=\log3/\log2$ and $A=M^\theta$ under the counterexample hypothesis.
...
\subsection{Finite scan through $50000$}
...
\[
 \mathcal A_j\!\left(\frac{\log2}{\log3}\right)\longrightarrow0.
\]
