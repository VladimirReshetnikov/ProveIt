...
$\mathcal P_r(M,A)$ as in Theorem~\ref{thm:kummer-separators}.  For all nonnegative integers
$p$ and all positive integers $q$ with $r\nmid q$,
\begin{equation}
...
$\log_2 M=\log_3 A$, every odd prime $r$, and every positive-density Chebotarev set
$\mathcal C$ in a Kummer extension generated by $r$th roots of the primes dividing $6MA$,
there are integers $p\ge0$, $q>0$ with $r\nmid q$ and a prime $\ell\in\mathcal C$ such that
\[
 \ell\mid M^q-2^p,
 \qquad
 \ell\mid A^q-3^p.
\]
It is enough to require $p/q$ to range over convergents to $\log_2 M$.
...
such a bridge is established, the correct status is substantial new local-global
