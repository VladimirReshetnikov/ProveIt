...
$\theta=\log 3/\log 2$.  The supplied unified report reduces the problem to a
...
We use the following inherited facts, and only these facts, from the large
source report.
...
\]
The source gives stronger common-perfect-power exclusions, but the displayed
dichotomy is the only part needed below.
...
In rank four, the external valuation vectors are independent.  In rank three,
they are collinear and the second-iterate kernel is nonzero; equivalently,
...
If $A\wedge B=0$, the nonzero vectors $A,B$ are rationally proportional.
Because their coordinates are nonnegative integers, there is a unique
...
\code{AlaogluErdos/Theta.lean}
 & define $\theta=\log3/\log2$; prove positivity, irrationality, and
   $\theta\log2=\log3$ \\
...
The actual implementation needs a separate origin case before \code{field_simp}.
This is a routine but important guard: an automated proof that silently assumes
\code{max u v \ne 0} would formalize a stronger and false statement.
