...
Throughout, put
\[
 a=2^x,\qquad b=3^x,\qquad \alpha=\log_2 3.
\]
...
Let $F$ be analytic on a disk centered at the origin with radius greater than $6$, and
suppose every Maclaurin coefficient of $F$ is nonnegative.  If $F$ interpolates
\[
 (1,1),\quad (2,a),\quad (3,b),\quad (6,ab)
\]
then $(a,b)=(2^k,3^k)$ for an integer $k\ge0$.
\end{corollary}
...
The exact identity has a useful stability form.  Let $(p_n)_{n\ge0}$ be a probability
distribution with $M(6)<\infty$, and put $M(t)=\sum p_nt^n$ and
\[
 \Delta=M(6)-M(2)M(3).
\]
...
Let $D\ge1$.  If every $p_n$ is an integral multiple of $1/D$, then $D^2\Delta$ is a nonnegative integer.  Hence either $\Delta=0$ or $\Delta\ge D^{-2}$.
\end{corollary}
\begin{proof}
...
\[
 \varepsilon\ge \frac{1}{3^{k+1}+2^k+1}.
\]
\end{corollary}
\begin{proof}
...
\end{equation}
If the support is contained in $\{0,\dots,N\}$ and $H_{k,N}=\max_{0\le n\le N}h_k(n)$,
then the representation forces $H_{k,N}>0$, and the total negative mass
$\eta=\sum_{c_n<0}|c_n|$ obeys
\[
...
Let $(p_n)$ be a probability distribution on $\N$ with $\sum_np_n3^n<\infty$.  Then
...
\[
 [1,q,\dots,q^n]g=\frac{\prod_{j=0}^{n-1}(r-q^j)}{\prod_{j=0}^{n-1}(q^n-q^j)}.
\]
...
Let $k\ge1$, and let $A$ be an integer with $2^k<A<2^{k+1}$.  Then every Newton summand
\[
...
\begin{equation}\label{eq:Rn-expansion}
 R_n=\alpha+\frac12\log_2 3\,(\log3-\log2)\,\theta_n+O(\theta_n^2).
\end{equation}
This follows by dividing the Taylor expansions of $e^{\theta\log3}-1$ and $e^{\theta\log2}-1$.

Dirichlet approximation gives infinitely many $n$ with $\theta_n\ll1/n$.  But the displayed
unreduced denominator and height in \eqref{eq:Rn} are exponential in $m_n\asymp n$, while
the error in \eqref{eq:Rn-expansion} is merely $O(1/n)$.  Reduced-denominator cancellation
is not controlled here.  Without such cancellation, the quality expressed in terms of the
available height is only logarithmic, far weaker than an irrationality-measure contradiction.
Higher-order Richardson combinations replace $O(\theta_n)$ by $O(\theta_n^r)$ for fixed
$r$, but still do not overcome the visible exponential height.
...
The finite covariance and support core is formalized in
\texttt{PositiveCoefficientClosure.lean}; its public endpoints are
\texttt{two\_mul\_covariance\_identity} and
\texttt{finite\_nonnegative\_rankOne\_eq\_single}.  The passage from finite support to
the convergent infinite series remains the short truncation argument described above.
