...
Let
\[
  \thetaAE=\frac{\log 3}{\log 2},\qquad
  \alphaAE=\frac{1}{\thetaAE}=\frac{\log 2}{\log 3}.
\]
...
ghas a ternary expansion whose first nonzero digit is known exactly.  Its
...
strict monotonicity of `reinterpret b c` under $b<c$.
...
\midrule
\endhead
`phi_strictMono` & \cref{prop:order} for bases $2,3$.\\
`padicVal_phi_sub_eq` & \cref{thm:isometry}.\\
`phi_add_eq_add_carry` & \cref{thm:add-carry}.\\
`phi_mul_eq_mul_carry` & \cref{thm:mul-carry}.\\
`psi_add_eq_sub_carry` & addition part of \cref{thm:reverse-carry}.\\
`psi_mul_eq_sub_carry` & multiplication part of \cref{thm:reverse-carry}.\\
`phi_le_rpow` & $\PhiMap(n)\le n^{\thetaAE}$.\\
`phi_eq_rpow_iff` & equality iff $n$ is a power of $2$.\\
`psi_ge_rpow` & $\PsiMap(n)\ge n^{\alphaAE}$.\\
`candidate_defect_pos` & \cref{prop:strict-sandwich}.\\
`defect_mul_recurrence` & \eqref{eq:D-recurrence}.\\
`squareCarry_padicVal` & \cref{thm:first-collision}.\\
`candidate_defect_rotation_bounds` & \eqref{eq:defect-bounds-linear}.\\
`radixSpectralRadius` & \cref{thm:spectral-radius}.\\
`alaogluErdos_iff_integer_radixRadius` & \cref{cor:spectral-AE}.\\
\bottomrule
\end{longtable}
...
module plan of the previous section, not imported through `native_decide` as a
