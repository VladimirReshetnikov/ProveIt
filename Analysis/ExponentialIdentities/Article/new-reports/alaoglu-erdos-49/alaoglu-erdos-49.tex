...
$3$-smooth numbers preserves congruences.  Second, every additive relation in that monoid is
classified, and all polynomial additive defects are shown to generate only the single ideal
\[
...
rigorous no-go result: additive identities alone cannot manufacture the exponential support
needed by the local criterion.
...
\]
Set $\theta=\log3/\log2$.  The number $\theta$ is transcendental: it is not rational,
...
Equivalently, $C$ is the largest positive integer $D$ for which
\[
...
Since $M\equiv2$ and $A\equiv3\pmod C$, every monomial satisfies
$M^aA^b\equiv2^a3^b\pmod C$, proving \eqref{eq:largestmod} for $D=C$.  Any modulus with
that property must divide both $M-2$ and $A-3$, hence divide $C$.
\end{proof}
...
\]
is exactly $C_n\Z$.  Equivalently, $C_n$ is the largest modulus $D$ for which
\[
...
$\log M/\log2=\log A/\log3$, then its finite Kummer images
...
\item \textbf{Candidate normalization.}  Rational candidate implies integral; rank-two exclusion
      assuming the transcendence of $\log3/\log2$ as an external theorem.
