...
\hypersetup{
  ...
  citecolor=newgreen,
  urlcolor=leanblue,
  pdftitle={The Alaoglu--Erdos Integer-Exponent Conjecture: Unified Report with Measure-Theoretic Continuation},
  pdfauthor={The ProveIt project},
  pdfsubject={Machine-checked partial results, exact conditional structure, determinant and matrix-coefficient methods, measure theory, probability, and research directions}
}
\urlstyle{same}
...
% The merged report uses appendix letters and R/M-prefixed continuation numbers.
% Widen the table-of-contents number boxes so labels such as R10 and M12.6
% do not collide with their titles.
\setlength{\cftsecnumwidth}{3.5em}
\setlength{\cftsubsecindent}{1.2em}
\setlength{\cftsubsecnumwidth}{4.4em}
\setlength{\cftsubsubsecindent}{2.0em}
...
\vspace{0.12cm}
{\LARGE Determinant, Matrix-Coefficient, and Measure-Theoretic Methods\par}
\vspace{1.0cm}
...
follows.  The present continuation further identifies the conjecture with a fixed-base case of
the Matrix Coefficient Conjecture, proves an exact $K^2-1$ Newton-polytope interpolation
threshold on the conditional $K\times K$ grid, and extends the directed finite exclusion
through $2^{29}$.  A fourth continuation develops a systematic measure-theoretic and
probabilistic audit.  It proves a joint triangle--Haar law, a profinite Beatty lift,
fixed-modulus and compact-unit independence, noncompact adelic escape with an exact
moving-frame joining, rigid Sturmian fluctuation laws, logarithmic Haar measure on the
power curve, metric exceptional-set bounds, and a finite-rank near-Sidon theorem.  These
results do not settle the conjecture; they show that every fixed congruence/profinite factor
considered here, including the natural compact unit factors, is already maximally distributed,
and that the unresolved information lives at exponentially growing local or Fourier scales.
\end{minipage}
\vfill
...
prioritized research program and a formalization blueprint.  Fifth, it develops a
measure-theoretic and probabilistic audit of the exact conditional model, identifying
which compact statistics are provably blind to a counterexample and which noncompact,
moving-scale data retain arithmetic information.


A third continuation connects the problem to the Matrix Coefficient Conjecture of
Dasgupta--Kakde.  On the conditional $K\times K$ multiplicative grid it proves that the
minimum Bernstein--Kushnirenko degree of a nonzero rational vanishing polynomial is exactly
$K^2-1$; in the support-based Matrix Coefficient strategy this exposes an exact factor-four
gap.  The directed-rounding computation is independently extended through $2^{29}$, so every
hypothetical nonintegral solution satisfies $x>29$ at the stated software-trust level.

A fourth continuation proves a joint triangle--Haar limit law for all conditional
solutions, effective discrepancy from Baker--Matveev, equidistribution in
$\widehat\Z^2\times\Torus$, fixed-congruence and compact-unit independence, and an
exact renormalized adelic joining after the uncentered $2$- and $3$-adic masses escape.
It also identifies the denominator jump process as a rigid Sturmian coboundary, proves
logarithmic Haar measure for the full curve $u\mapsto(\{u\},\{u^{\thetaq}\})$, gives
Borel--Cantelli and Hausdorff-dimension bounds in the exponent parameter, and derives a
near-Sidon law from finite-rank unit equations.  The synthesis is negative but precise:
fixed-scale measure theory cannot see the conjectural exact hits; a successful proof must
retain phase at an exponentially growing denominator or Fourier scale, or cross the
factor-four support threshold in the Matrix Coefficient program.
\end{abstract}
...
scan, including the third continuation, extends the finite barrier through $2^x<2^{29}$, i.e.\ $x>29$.  \statuscomp
...
\item $2^x>2^{29}$, hence $x>29$, by the directed-rounding scans through the third
continuation; this raises the barrier only under the stated software trust boundary and is not
a kernel certificate.  \statuscomp
...
layer and finds its valuation to be $O\bigl(T^{3/2}(\log T)^2\bigr)$ against a block budget
$H_T\asymp T^{5/2}$, hence $o(1)$ of the budget.
...
\addlinespace
\lean{TropicalInitialFibers} & Factorization of the first $2$- and $3$-adic initial forms into ordinary Vandermondes on column fibers. & Determinants over valued rings; stable sorting by valuations. \\
\addlinespace
\lean{InitialFiberValuationBound} & Under a standard $p$-adic logarithm hypothesis, the first tied layer contributes $o(H_T)$. & \lean{TropicalInitialFibers}; an abstract two-logarithm valuation bound. \\
\addlinespace
...
\begin{statusbox}{\statuslean{} --- \lean{im_eq_zero_of_complexTwoBaseIntegralSolution} and
\lean{twoBaseIntegralSolution_re_of_complex} in \lean{ComplexSolutions}.  Only irrationality of
$\thetaq$ enters; no transcendence input is used.}
\end{statusbox}
...
Reading & Status \\
\midrule
Exponent in $\C$, principal branch, values in $\Z$ or $\Q$ &
equivalent to the original, \statuslean \\
...
the aggregate import surface, so the tag survives only as a definition for future use.
...
No nonintegral candidate with $2^x<2^{26}$ & \statuscomp &
Section~\ref{sec:finitecheck}, Appendix~\ref{app:scanner} \\
\midrule
...
\lean{LeanProofs}.  Every module listed below is reachable from the aggregate import
surface, and the merged tree builds with no errors.  None of them contains \lean{sorry}, an
extra axiom, or \lean{native_decide}; certificate tables are replayed by the kernel.
...
\lean{.../AuditNewModules} & \texttt{\#print axioms} audit of the headline theorems of the
recent formalization rounds \\
\bottomrule
\end{longtable}
}
...
There are none.  Earlier revisions of this report listed nine modules whose Lean sources
existed in the repository but had not been accepted:
\begin{center}
...
downstream of them is unconditional.  \lean{ExponentialPolynomialZeros} still delivers
nonvanishing rather than the ordered-chamber positivity proved on paper in
Appendix~\ref{app:gvproof}.  Second, the finite scans through $2^{20}$ and $2^{26}$ are
\statuscomp{}, not kernel certificates; the kernel-verified exponent bound is still
...
\section{Third continuation: outcome and claim ledger}
\label{sec:third-continuation}

This continuation starts from the complete inherited report rather than replacing it.  In
particular, it treats the exact free-monoid theorem, the determinant ceilings, the
second-iterate kernel dichotomy, and the software verification through $2^{27}$ as established
inputs with the trust labels already assigned there.  The primary goal was again an
unconditional proof of the Alaoglu--Erd\H{o}s conjecture.  No such proof was obtained.  The
new work does, however, isolate the conjecture inside the recent Matrix Coefficient framework
for logarithm matrices, determine an exact Newton-polytope interpolation threshold for the
hypothetical counterexample grid, and extend the directed-rounding exclusion range.

The most useful conceptual change is that the four-exponentials wall can now be stated as an
\emph{exact factor-four auxiliary-polynomial gap}.  A hypothetical counterexample gives a
singular $2\times2$ matrix of logarithms satisfying the ``weak'' condition of
Dasgupta--Kakde, while its irrational rank-one factorization forbids every rational matrix
coefficient from vanishing.  On the associated $(2N)\times(2N)$ multiplicative grid, the
least possible Bernstein--Kushnirenko degree of a nonzero vanishing polynomial is exactly
$4N^2-1$.  The auxiliary-polynomial condition that would imply the Matrix Coefficient
Conjecture asks for degree below $N^2$.  Thus the missing implication must gain a full factor
of four over exact interpolation; no rearrangement of a generic interpolation determinant can
supply it.

\begin{center}
\begin{tabularx}{0.96\textwidth}{@{}>{\raggedright\arraybackslash}p{0.27\textwidth}X>{\raggedright\arraybackslash}p{0.16\textwidth}@{}}
\toprule
Result & Content & Status\\
\midrule
Fixed-base matrix-coefficient equivalence & The Alaoglu--Erd\H{o}s conjecture is exactly the Matrix Coefficient Conjecture restricted to logarithm matrices whose first column is $(\log2,\log3)^T$ and whose second column consists of logarithms of positive integers. & \statusnew\\
\addlinespace
No rational coefficient & For a nonintegral solution $\beta$, the singular logarithm matrix factors as $(\log2,\log3)^T(1,\beta)$, and every rational matrix coefficient factors into two nonzero terms. & \statusnew\\
\addlinespace
Exact sparse-interpolation threshold & On any $R\ge2$ rational points of the irrational power curve $y=x^\beta$, the minimum two-dimensional BK-degree of a nonzero rational polynomial vanishing at all points is exactly $R-1$. & \statusnew\\
\addlinespace
Boundary-defect compression & The degree-$K^2-1$ graph interpolant has dyadic and triadic dilation defects divisible by the entire interior grid polynomial, leaving two residual polynomials of degree only $K-1$ and an exact boundary commutator identity. & \statusnew\\
\addlinespace
Factor-four obstruction & On the Dasgupta--Kakde grid $X(2N)$ attached to a counterexample, the exact minimum is $4N^2-1$, whereas their condition~(m) requires a polynomial of BK-degree $<N^2$. & \statusnew\\
\addlinespace
Directed finite extension & The same MPFR directed-rounding verifier used in the inherited report was replayed on the next dyadic ranges; the precise endpoint, chunk audit, and extremal records are stated in Section~\ref{sec:new-finite-extension}. & \statuscomp\\
\bottomrule
\end{tabularx}
\end{center}

The first four statements are conventional proofs in this report.  They use no unproved
transcendence conjecture.  Their role is diagnostic: they do not prove that the singular matrix
cannot exist, but they identify exactly what a successful Matrix Coefficient or auxiliary
polynomial argument would have to manufacture.

\section{The fixed-base Matrix Coefficient formulation}
\label{sec:matrix-coefficient}

\subsection{The logarithm matrix of a hypothetical counterexample}

Let $M,A\in\N$ and put
\begin{equation}
 L(M,A):=
 \begin{pmatrix}
  \log2 & \log M\\
...
\begin{equation}
 ...
 \qquad
 \det L(M,A)=0.
 \label{eq:mc-rank-one}
\end{equation}
The matrix has entries in the vector space of logarithms of algebraic numbers.  It is therefore
a particularly rigid $2\times2$ instance of the Matrix Coefficient Conjecture of
Dasgupta and Kakde~\cite{DasguptaKakdeMCC}.  That conjecture says that a singular square
matrix $L$ of logarithms should admit nonzero rational vectors $w,v$ with
$w^TLv=0$; in dimension two it is equivalent to the Four Exponentials Conjecture.

The rank-one factorization in \eqref{eq:mc-rank-one} makes the coefficient condition completely
explicit.

\begin{proposition}[Exact coefficient factorization]
\label{prop:mc-factorization}
Let $\beta\in\R$, $M=2^\beta$, and $A=3^\beta$.  For
$w=(r,s)^T$ and $v=(u,z)^T$ one has
\begin{equation}
 w^T L(M,A)v
 =\bigl(r\log2+s\log3\bigr)\bigl(u+z\beta\bigr).
 \label{eq:mc-coefficient-product}
\end{equation}
If $\beta\notin\Q$, then this number is nonzero for every pair of nonzero vectors
$w,v\in\Q^2$.
\end{proposition}

\begin{proof}
The displayed identity follows immediately from \eqref{eq:mc-rank-one}.  If
$r\log2+s\log3=0$ with $r,s\in\Q$, clearing denominators and exponentiating gives a
multiplicative relation $2^a3^b=1$, hence $a=b=0$ by unique factorization; thus $r=s=0$.
Likewise $u+z\beta=0$ with $u,z\in\Q$ and $(u,z)\ne(0,0)$ would make $\beta$ rational.
\end{proof}

A counterexample is irrational by the elementary rational-exponent argument (and
transcendental by Gelfond--Schneider, as formalized in the inherited corpus).  It would
therefore give a singular logarithm matrix with \emph{no} vanishing rational coefficient: not
merely an application of the Matrix Coefficient Conjecture, but a literal counterexample to its
$2\times2$ case.

\subsection{An exact equivalence, not only a conditional implication}

Define the fixed-base matrix-coefficient statement $\mathrm{MC}_{2,3}$ as follows:

\begin{quote}
For all positive integers $M,A$ with $\det L(M,A)=0$, there exist nonzero
$w,v\in\Q^2$ such that $w^TL(M,A)v=0$.
\end{quote}

\begin{theorem}[Fixed-base Matrix Coefficient equivalence]
\label{thm:mc-equivalence}
The Alaoglu--Erd\H{o}s conjecture is equivalent to $\mathrm{MC}_{2,3}$.
\end{theorem}

\begin{proof}
Assume first $\mathrm{MC}_{2,3}$.  Let $2^x=M\in\N$ and $3^x=A\in\N$.  Then
$\det L(M,A)=0$.  By \eqref{eq:mc-coefficient-product}, a vanishing rational coefficient
forces either a rational relation between $\log2$ and $\log3$, which is impossible, or a
relation $u+zx=0$ with $(u,z)\ne(0,0)$, which makes $x\in\Q$.  Writing $x=p/q$ in lowest
terms, integrality of both $2^{p/q}$ and $3^{p/q}$ forces $q=1$, so $x\in\Z$.

Conversely assume the Alaoglu--Erd\H{o}s conjecture and suppose $M,A\in\N$ make
$L(M,A)$ singular.  Put $x=\log M/\log2$.  Singularity gives
$x=\log A/\log3$, hence $2^x=M$ and $3^x=A$.  The conjecture gives $x\in\Z$, and then
$v=(-x,1)^T$ is a nonzero rational vector satisfying $L(M,A)v=0$; take any nonzero $w$.
(The degenerate case $M=A=1$ has $x=0$ and is included.)
\end{proof}

This equivalence narrows the open transcendence input more sharply than the usual statement
``Four Exponentials implies the conjecture.''  Only one fixed first column is needed, and the
second column is restricted to logarithms of positive integers.  Conversely, even this very
thin positive-integral rank-one family already contains the full Alaoglu--Erd\H{o}s problem.

\subsection{The multiplicative pairing and the absence of orthogonality}

The Matrix Coefficient strategy is formulated multiplicatively.  Let
\begin{equation}
 x_1=(2,M),\qquad x_2=(3,A)\in(\Q_{>0}^{\times})^2
 \label{eq:mc-generators}
\end{equation}
and, for $a=(a_1,a_2)$ and $b=(b_1,b_2)$ in $\Z^2$, set
\begin{equation}
 \langle a,b\rangle_X
 :=\prod_{i,j=1}^{2}x_{i,j}^{a_i b_j}.
 \label{eq:mc-pairing}
\end{equation}
...
\begin{equation}
 \langle a,b\rangle_X
 =\exp\!\left(
   (a_1\log2+a_2\log3)(b_1+\beta b_2)
  \right).
 \label{eq:mc-pairing-factor}
\end{equation}
Thus, when $\beta\notin\Q$, no nonzero $a,b\in\Z^2$ are orthogonal.  In the notation of
Dasgupta--Kakde, a hypothetical counterexample satisfies condition~(w) (the logarithm matrix is
singular) and fails condition~(o) (there is no nontrivial multiplicative orthogonality).  Their
general theorem proves $(\mathrm m)\Rightarrow(\mathrm o)$; the missing implication
$(\mathrm w)\Rightarrow(\mathrm m)$ is exactly what would rule out the counterexample.

The next section specializes their support-based condition~(m) to the present rank-one grid and
computes its exact interpolation threshold.

\section{Exact Newton-polytope complexity of the counterexample grid}
\label{sec:newton-threshold}

\subsection{The grid as an irrational power curve}

Assume for this section that $\beta\notin\Q$ and that
$M=2^\beta$, $A=3^\beta$ are positive rational numbers; in the conjectural application they
are integers.  For $K\ge1$ define
\begin{equation}
 X_\beta(K):=
 \left\{
   (2^i3^j,M^iA^j):0\le i,j<K
 \right\}.
 \label{eq:mc-grid}
\end{equation}
Writing $t_{ij}=i\log2+j\log3$ gives
\begin{equation}
 (2^i3^j,M^iA^j)=\bigl(e^{t_{ij}},e^{\beta t_{ij}}\bigr).
 \label{eq:mc-grid-curve}
\end{equation}
The $K^2$ parameters $t_{ij}$ are distinct, because equality would give
$2^{i-i'}3^{j-j'}=1$.

For a nonzero polynomial
\[
 P(X,Y)=\sum_{(u,v)\in S(P)}c_{u,v}X^uY^v
\]
its restriction to the curve is the exponential polynomial
\begin{equation}
 F_P(t):=P(e^t,e^{\beta t})
 =\sum_{(u,v)\in S(P)}c_{u,v}e^{(u+\beta v)t}.
 \label{eq:mc-restriction}
\end{equation}
Irrationality of $\beta$ makes all frequencies $u+\beta v$ distinct.

\begin{lemma}[Support controls zeros on an irrational power curve]
\label{lem:support-zero-count}
Let $\beta\notin\Q$ and let $P\in\C[X^{\pm1},Y^{\pm1}]$ be nonzero with $r$ nonzero
monomials.  Then $F_P$ has at most $r-1$ distinct real zeros.  Consequently, if $P$ vanishes
on $R$ distinct positive points of $y=x^\beta$, then
\begin{equation}
 |S(P)|\ge R+1.
 \label{eq:mc-support-lower}
\end{equation}
\end{lemma}

\begin{proof}
Order the distinct real frequencies as $\lambda_1<\cdots<\lambda_r$.  The functions
$e^{\lambda_1t},\ldots,e^{\lambda_rt}$ form an extended complete Chebyshev system on
$\R$: for any $t_1<\cdots<t_r$, the generalized exponential Vandermonde determinant
$\det(e^{\lambda_jt_i})$ is strictly positive.  If a nontrivial linear combination vanished at
$r$ distinct real points, its coefficient vector would lie in the kernel of this invertible
evaluation matrix.  This is impossible.  The same argument works for complex coefficients.
\end{proof}

This is the one-dimensional zero estimate hidden inside the generalized-Vandermonde machinery
of the inherited report.  Here it is used not to estimate gaps between nodes but to constrain
the Newton support of an auxiliary polynomial.

\subsection{BK-degree and a sharp lattice-point bound}

Following Dasgupta--Kakde, divide $P$ by its greatest common monomial, which does not change its
zero set in $(\C^\times)^2$.  Let $\Delta(P)$ be the convex hull of the remaining support and
write
\begin{equation}
 \operatorname{bkd}(P)
 :=\max\left\{\deg P,\ 2\operatorname{Area}\Delta(P)\right\}.
 \label{eq:mc-bkd}
\end{equation}
For two variables this is exactly their Bernstein--Kushnirenko degree: the first term is the
mixed volume with one generic linear polynomial, and the second is the normalized area
$\BK(P)=2!\operatorname{Area}\Delta(P)$.

\begin{lemma}[Support cardinality versus BK-degree]
\label{lem:support-bkd}
Every nonzero two-variable polynomial satisfies
\begin{equation}
 |S(P)|\le \operatorname{bkd}(P)+2.
 \label{eq:mc-support-bkd}
\end{equation}
If $\Delta(P)$ is one-dimensional, the sharper bound
$|S(P)|\le\operatorname{bkd}(P)+1$ holds.
\end{lemma}

\begin{proof}
Suppose first that $\Delta(P)$ is a two-dimensional lattice polygon.  Let $I$ and $B$ be its
numbers of interior and boundary lattice points.  Pick's theorem gives
\[
 2\operatorname{Area}\Delta(P)=2I+B-2.
\]
The support is contained in the lattice points of its convex hull, so
\[
 |S(P)|\le I+B
 =2\operatorname{Area}\Delta(P)+2-I
 \le2\operatorname{Area}\Delta(P)+2
 \le\operatorname{bkd}(P)+2.
\]
If the hull is a segment, the number of lattice points on it is its lattice length plus one.
After monomial normalization both endpoint coordinates are nonnegative and bounded by the total
degree; hence the lattice length is at most $\deg P$.  The point case is immediate.
\end{proof}

The inequality is nearly best possible for the precise reason relevant here: a thin triangle
with vertices $(0,0)$, $(d,0)$, $(0,1)$ has normalized area and degree both equal to $d$ and
contains $d+2$ lattice points.

\subsection{The exact interpolation threshold}

\begin{theorem}[Exact BK interpolation threshold]
\label{thm:exact-bkd-threshold}
Let $R\ge2$, let $\beta\notin\Q$, and let
$\xi_1,\ldots,\xi_R$ be distinct positive rational numbers such that
$\xi_i^\beta\in\Q$ for every $i$.  Among all nonzero polynomials
$P\in\Q[X,Y]$ vanishing at all points $(\xi_i,\xi_i^\beta)$, the minimum possible
BK-degree is exactly
\begin{equation}
 \min_P\operatorname{bkd}(P)=R-1.
 \label{eq:mc-exact-threshold}
\end{equation}
After clearing denominators, the same minimum is attained by a polynomial in $\Z[X,Y]$.
\end{theorem}

\begin{proof}
For the lower bound, Lemma~\ref{lem:support-zero-count} gives
$|S(P)|\ge R+1$.  Lemma~\ref{lem:support-bkd} therefore gives
$R+1\le\operatorname{bkd}(P)+2$, or
$\operatorname{bkd}(P)\ge R-1$.

For the upper bound, let $Q\in\Q[X]$ be the Lagrange interpolation polynomial satisfying
$Q(\xi_i)=\xi_i^\beta$ for all $i$.  It has degree at most $R-1$, and
$P(X,Y)=Y-Q(X)$ vanishes at all the required points.  Its Newton polygon lies in the thin
triangle with vertices $(0,0)$, $(R-1,0)$, and $(0,1)$, so both its degree and normalized area
are at most $R-1$.  Hence $\operatorname{bkd}(P)\le R-1$, proving equality.  Rational
coefficients may be cleared without changing the support.
\end{proof}

The theorem is an exact result, not an asymptotic estimate.  It also explains why the constant
cannot be improved by a more clever choice of support: the elementary interpolant
$Y-Q(X)$ already attains the optimum.  Total positivity says that every polynomial with smaller
Newton complexity has too few independent frequencies to vanish at all the points.

\begin{corollary}[Full support and alternating coefficient signs]
\label{cor:full-support-interpolant}
Under the hypotheses of Theorem~\ref{thm:exact-bkd-threshold}, the interpolation polynomial
\[
 Q(X)=q_0+q_1X+\cdots+q_{R-1}X^{R-1}
\]
has degree exactly $R-1$ and every coefficient $q_k$ is nonzero.  Moreover, if the frequencies
$0,1,\ldots,R-1,\beta$ are put in increasing order, then the coefficients of
$Y-Q(X)$, put in the corresponding order, alternate in sign.
\end{corollary}

\begin{proof}
The polynomial $Y-Q(X)$ has at most $R+1$ monomials, while
Lemma~\ref{lem:support-zero-count} says that every vanishing polynomial has at least $R+1$.
Thus all $R$ possible coefficients of $Q$ occur and the leading one is nonzero.  For the sign
statement, form the $R\times(R+1)$ evaluation matrix whose columns are the functions
$x^\lambda$ for the ordered frequencies.  Every maximal minor is strictly positive by total
positivity.  The signed maximal minors give a generator of its one-dimensional kernel, and
therefore alternate in sign.
\end{proof}

This sign statement is the support-theoretic shadow of the arbitrary-node divided-difference
identities in the inherited report.  Here it follows without estimating any gap: the entire
coefficient vector is forced by strict total positivity.

\begin{corollary}[Exact threshold on the counterexample grid]
\label{cor:grid-bkd-threshold}
For $K\ge2$, if $M=2^\beta$ and $A=3^\beta$ are rational and $\beta\notin\Q$, then
\begin{equation}
 \min\left\{
  \operatorname{bkd}(P):
  0\ne P\in\Q[X,Y],\ P|_{X_\beta(K)}=0
 \right\}=K^2-1.
 \label{eq:mc-grid-threshold}
\end{equation}
\end{corollary}

\begin{proof}
The first coordinates $2^i3^j$ are $K^2$ distinct positive rationals and their
$\beta$-th powers are the rational second coordinates $M^iA^j$.  Apply
Theorem~\ref{thm:exact-bkd-threshold} with $R=K^2$.
\end{proof}


\subsection{Boundary compression of the multiplicative interpolation problem}
\label{subsec:boundary-compression}

The optimal interpolant contains more structure than its Newton polygon records.  Let $Q_K$ be
the unique polynomial of degree at most $K^2-1$ satisfying
\begin{equation}
 Q_K(2^i3^j)=M^iA^j,
 \qquad 0\le i,j<K.
 \label{eq:boundary-Q}
\end{equation}
Under the irrational power-curve hypothesis, Corollary~\ref{cor:full-support-interpolant}
shows that $\deg Q_K=K^2-1$.  Define the rectangular node polynomials
\begin{equation}
 \Pi_{a,b}(X):=
 \prod_{0\le i<a}\prod_{0\le j<b}(X-2^i3^j),
 \qquad \Pi_{0,b}=\Pi_{a,0}=1.
 \label{eq:boundary-Pi}
\end{equation}

\begin{theorem}[Dilation-defect factorization]
\label{thm:dilation-defect}
For every $K\ge2$ there are unique polynomials $U_K,V_K\in\Q[X]$ of degree exactly $K-1$
such that
\begin{align}
 Q_K(2X)-M Q_K(X)&=\Pi_{K-1,K}(X)U_K(X),
 \label{eq:boundary-U}\\
 Q_K(3X)-A Q_K(X)&=\Pi_{K,K-1}(X)V_K(X).
 \label{eq:boundary-V}
\end{align}
...
\begin{equation}
 ...
 \qquad
 q_K(3^{K^2-1}-A),
 \label{eq:boundary-leading}
\end{equation}
respectively.
\end{theorem}

\begin{proof}
For $0\le i<K-1$ and $0\le j<K$, equation~\eqref{eq:boundary-Q} gives
\[
 Q_K(2\cdot2^i3^j)-M Q_K(2^i3^j)
 =M^{i+1}A^j-MM^iA^j=0.
\]
The $K(K-1)$ roots are distinct, proving divisibility by $\Pi_{K-1,K}$.  The left side of
\eqref{eq:boundary-U} has degree $K^2-1$, because its leading coefficient is
$q_K(2^{K^2-1}-M)$ and $M=2^\beta$ with $\beta\notin\Z$.  Subtracting the divisor degree
$K(K-1)$ leaves degree $K-1$.  The triadic identity is symmetric.  Uniqueness follows because
the divisors are nonzero.
\end{proof}

The two dilation operators commute.  Cancelling the common $(K-1)\times(K-1)$ interior node
polynomial turns this tautological commutation into a low-degree boundary identity.  Put
\begin{align*}
 L_3^-(X)&:=\prod_{i=0}^{K-2}\left(X-\frac{2^i}{3}\right),
 &L_3^+(X)&:=\prod_{i=0}^{K-2}\left(X-2^i3^{K-1}\right),\\
 L_2^-(X)&:=\prod_{j=0}^{K-2}\left(X-\frac{3^j}{2}\right),
 &L_2^+(X)&:=\prod_{j=0}^{K-2}\left(X-2^{K-1}3^j\right).
\end{align*}

\begin{theorem}[Boundary commutator identity]
\label{thm:boundary-commutator}
The residual polynomials of Theorem~\ref{thm:dilation-defect} satisfy
\begin{align}
 &3^{K(K-1)}L_3^-(X)U_K(3X)-A L_3^+(X)U_K(X)
 \notag\\
 &\hspace{2.2cm}=
 2^{K(K-1)}L_2^-(X)V_K(2X)-M L_2^+(X)V_K(X).
 \label{eq:boundary-commutator}
\end{align}
Both sides have degree at most $2K-2$.
\end{theorem}

\begin{proof}
Let $D_2f(X)=f(2X)-Mf(X)$ and $D_3f(X)=f(3X)-Af(X)$.  Direct expansion gives
$D_2D_3=D_3D_2$.  Apply this to $Q_K$ and substitute
\eqref{eq:boundary-U}--\eqref{eq:boundary-V}.  The four node polynomials factor as
\begin{align*}
 \Pi_{K-1,K}(3X)
 &=3^{K(K-1)}\Pi_{K-1,K-1}(X)L_3^-(X),\\
 \Pi_{K-1,K}(X)
 &=\Pi_{K-1,K-1}(X)L_3^+(X),\\
 \Pi_{K,K-1}(2X)
 &=2^{K(K-1)}\Pi_{K-1,K-1}(X)L_2^-(X),\\
 \Pi_{K,K-1}(X)
 &=\Pi_{K-1,K-1}(X)L_2^+(X).
\end{align*}
...
This compression is exact: $K^2$ graph values leave only two degree-$(K-1)$ boundary defects.
It does not by itself give a contradiction---the leading coefficients of the two sides of
\eqref{eq:boundary-commutator} agree identically---but it exposes a smaller arithmetic object
than the full interpolation determinant.  A promising next step is to study the primitive
integer contents and the $2$- and $3$-adic Newton polygons of $U_K$ and $V_K$.  Any
leading-order divisibility forced on these residuals would be genuinely boundary-sensitive and
would not be covered by the inherited termwise min-plus ceiling.

\subsection{The factor-four obstruction to the support strategy}

For a $2\times2$ logarithm matrix, condition~(m) in
Dasgupta--Kakde~\cite{DasguptaKakdeMCC} asks, for all sufficiently large $N$, for a polynomial
vanishing on $X(2N)$ and satisfying
\begin{equation}
 \operatorname{bkd}(P)<N^2.
 \label{eq:mc-condition-m}
\end{equation}
Under a hypothetical Alaoglu--Erd\H{o}s counterexample, however,
Corollary~\ref{cor:grid-bkd-threshold} gives the exact minimum
\begin{equation}
 \operatorname{bkd}_{\min}(X_\beta(2N))=4N^2-1.
 \label{eq:mc-factor-four}
\end{equation}
Thus condition~(m) lies asymptotically a factor of four below the exact interpolation floor.
This is the quantitative form, for this problem, of the missing implication
$(\mathrm w)\Rightarrow(\mathrm m)$.

There is no paradox.  Dasgupta--Kakde prove $(\mathrm m)\Rightarrow(\mathrm o)$, while
Proposition~\ref{prop:mc-factorization} proves that condition~(o) fails for an irrational
counterexample.  Therefore condition~(m) must fail, and
\eqref{eq:mc-factor-four} says exactly how badly it fails.  A proof of
$(\mathrm w)\Rightarrow(\mathrm m)$ would derive a polynomial below this sharp floor from the
singularity of the logarithm matrix, thereby contradicting the existence of the counterexample.

\begin{resultbox}
\textbf{Interpretation.}
Exact interpolation, Siegel-lemma dimension counting, changing the shape of the Newton polygon,
and adding generic monomials cannot bridge the factor four: the optimum $4N^2-1$ is already
attained by the thin interpolant $Y-Q(X)$.  The missing gain must be genuinely analytic or
adelic.  It must turn the vanishing determinant into simultaneous archimedean and
non-archimedean smallness strong enough that a product-formula argument forces an auxiliary
polynomial to vanish, rather than merely choosing a polynomial that vanishes by linear
algebra.  This is precisely the distinction emphasized by the ``analytic reasons'' question in
Dasgupta--Kakde.
\end{resultbox}

\subsection{Relation to the inherited determinant ceilings}

The new threshold and the inherited min-plus ceiling are complementary.

\begin{itemize}
\item The min-plus ceiling says that entrywise $p$-adic divisibility of an integer determinant
cannot, by itself, beat a correct archimedean determinant bound.  Any successful determinant
must create additional cancellation.
\item The exact BK threshold says that support counting cannot, by itself, create the sparse
auxiliary polynomial required by the Matrix Coefficient strategy.  Any successful support
argument must create analytic smallness that the product formula upgrades to exact vanishing.
\item Both failures are sharp.  The termwise divisor is an assignment minimum that is attained
by a Leibniz term, while the BK threshold is attained by the Lagrange interpolant
$Y-Q(X)$.
\end{itemize}

The two diagnostics therefore point to the same surviving object: a support-controlled
interpolation determinant with \emph{cancellation beyond tropical minima} at selected finite
places and a simultaneous archimedean small-value estimate.  Merely enlarging the node grid or
repacking the exponent support cannot help; both operations scale the two sides together.

\section{Research targets exposed by the exact threshold}
\label{sec:mc-research-targets}

\subsection{A support-preserving analytic determinant}

The most direct target is a two-variable analogue of the classical auxiliary-function
construction in which the Newton support is fixed first and coefficient heights are controlled
second.

\begin{target}[Analytic factor-four theorem]
\label{target:analytic-factor-four}
Let $L(M,A)$ be singular.  Construct, for all sufficiently large $N$, a nonzero
$P_N\in\Z[X,Y]$ with $\operatorname{bkd}(P_N)<N^2$ such that a product-formula estimate forces
$P_N$ to vanish on $X(2N)$.
\end{target}

The word ``forces'' is essential.  Theorem~\ref{thm:exact-bkd-threshold} proves that no such
polynomial exists under an irrational counterexample, so the construction must use singularity
to derive an impossible object.  A linear-algebra interpolant begins at BK-degree $4N^2-1$ and
therefore cannot be massaged into the target without a new source of equations or smallness.

A plausible architecture is:
\begin{enumerate}
\item choose a Newton polygon of normalized area $<N^2$ and solve a Siegel-lemma problem for a
polynomial whose values, rather than being zero, are extremely small on $X(2N)$;
\item use the rank-one real logarithm relation to amplify archimedean smallness along the
ordered curve;
\item use the exact prime-valuation grid to obtain $p$-adic smallness from cancellation, not
from the termwise assignment divisor excluded by the min-plus ceiling;
\item apply the product formula to each algebraic value $P(x)$.
\end{enumerate}
The difficult step is the third.  The inherited report's first tied-layer calculation shows
that the most obvious cancellation is lower order, so a successful construction must align
many tied layers coherently.

\subsection{A coefficient-height version of the exact threshold}

The exact interpolant $P=Y-Q(X)$ has optimal BK-degree but potentially enormous coefficient
height.  The next quantitative invariant to compute is therefore
\begin{equation}
 H_K:=\min\left\{h(P):
 P\in\Z[X,Y]\setminus\{0\},\ P|_{X_\beta(K)}=0,\
 \operatorname{bkd}(P)=K^2-1\right\},
 \label{eq:mc-min-height}
\end{equation}
where $h(P)$ is logarithmic projective coefficient height.  The leading coefficient of the
Lagrange interpolant is the arbitrary-node divided difference already studied in the inherited
report, so its denominator is controlled by a generalized Vandermonde product.  What is new in
\eqref{eq:mc-min-height} is the optimization over \emph{all} thin Newton polygons and all
integer kernel vectors, not only the standard monomial basis.

A lower bound of order $K^3$ or larger, uniform over supports, would make precise why exact
interpolation is arithmetically expensive.  More importantly, a support for which the height
drops anomalously could identify the cancellation geometry needed in
Target~\ref{target:analytic-factor-four}.

\subsection{Formalization targets}

The matrix-coefficient reduction is elementary enough for immediate formalization.  A natural
module order is:
\begin{enumerate}
\item \lean{MatrixCoefficientReduction}: define $L(M,A)$, prove
\eqref{eq:mc-coefficient-product}, and prove Theorem~\ref{thm:mc-equivalence};
\item \lean{IrrationalPowerCurveZeros}: package the existing generalized-Vandermonde
positivity as the support zero bound of Lemma~\ref{lem:support-zero-count};
\item \lean{NewtonPolygonSupport}: formalize the lattice-point estimate
$|S(P)|\le\operatorname{bkd}(P)+2$, using Pick's theorem or an equivalent lattice-polygon
count;
\item \lean{ExactBKInterpolation}: formalize Lagrange interpolation and
Theorem~\ref{thm:exact-bkd-threshold};
\item \lean{MatrixCoefficientGrid}: instantiate the theorem on $X_\beta(K)$ and record the
factor-four corollary.
\end{enumerate}
The only potentially substantial library gap is Pick's theorem.  For the exact threshold it can
be avoided by proving the weaker but sufficient inequality directly for convex lattice
polygons through a triangulation into primitive lattice triangles; the rest is elementary
linear algebra and the Chebyshev-system theorem already represented in the corpus.


\section{Directed-rounding extension through \texorpdfstring{$2^{29}$}{2\^{}29}}
\label{sec:new-finite-extension}

The inherited report certified all non-power inputs $m<2^{27}$ at the MPFR software-trust
level.  I extracted the literal C verifier reproduced in Appendix~\ref{app:scanner}, verified
that its SHA-256 digest is
\begin{center}
\sha{f888c0d89be9366f01e2d26f339dcbe446556d39d4c5dbcf8f579ec0d0052e1d} ,
\end{center}
compiled it against MPFR~4.2.2, and scanned the two next dyadic ranges.  The first range
$[2^{27},2^{28})$ was run at $128$ bits; the second, $[2^{28},2^{29})$, at $96$ bits.
Both precisions are logically exact for the claim being made: MPFR rounds every endpoint in the
specified outward direction, and an input is counted only after both strict logarithmic
inequalities are positive.  Reducing the precision can widen an enclosure and create a reported
failure, but cannot turn an uncertified input into a proof.  The ordinary \lean{long double}
value remains only a locator for the neighboring integer and has no deductive role.

The recorded environment was Linux on x86-64, GCC~14.2.0, MPFR~4.2.2, and five OpenMP worker
threads.  All integers passed to MPFR were exactly representable at the selected precisions:
through the endpoint $2^{29}$, the neighboring output integer is below $3^{29}<2^{46}$.

\begin{statusbox}{\statuscomp{} --- the extension is a directed-rounding software proof, not a
Lean kernel theorem.  The trusted base is the displayed C source, GCC, OpenMP, GMP/MPFR, the
minimal MPFR ABI declarations used when the development header is absent, and the host
hardware.  The source digest agrees with the corrected literal-source digest in the inherited
continuation.}
\end{statusbox}

\subsection{Audit of \texorpdfstring{$[2^{27},2^{28})$}{[2\^{}27,2\^{}28)}}

The range contains $2^{27}$ integers and exactly one power of two, its left endpoint.  Thus the
required checked count is $2^{27}-1=134{,}217{,}727$.  Sixteen chunks of width $2^{23}$
were run at $128$ bits.  The concatenated canonical log has SHA-256 digest
\begin{center}
\sha{187d860b93ae9843a466a1613c115f2a4f899db8a00be22b963725953b30eca6} .
\end{center}
Every chunk returned zero failures, and the counts sum exactly to the required total.


{\small
\begin{longtable}{@{}r p{0.28\textwidth} r r r@{}}
\caption{Directed-rounding audit on $[2^{27},2^{28})$.}\label{tab:scan28-new}\\
\toprule
Chunk & Integer range & Checked & Smallest lower gap & Smallest upper gap \\
\midrule
\endfirsthead
...
\midrule
\endhead
0 & 134{,}217{,}728--142{,}606{,}335 & 8{,}388{,}607 & $2.8148\times10^{-20}$ & $2.8468\times10^{-21}$ \\
1 & 142{,}606{,}336--150{,}994{,}943 & 8{,}388{,}608 & $3.5925\times10^{-21}$ & $6.2726\times10^{-22}$ \\
2 & 150{,}994{,}944--159{,}383{,}551 & 8{,}388{,}608 & $3.7823\times10^{-21}$ & $1.6063\times10^{-20}$ \\
3 & 159{,}383{,}552--167{,}772{,}159 & 8{,}388{,}608 & $3.8468\times10^{-21}$ & $7.5693\times10^{-21}$ \\
4 & 167{,}772{,}160--176{,}160{,}767 & 8{,}388{,}608 & $8.6652\times10^{-21}$ & $1.4331\times10^{-20}$ \\
5 & 176{,}160{,}768--184{,}549{,}375 & 8{,}388{,}608 & $1.8762\times10^{-20}$ & $5.4449\times10^{-22}$ \\
6 & 184{,}549{,}376--192{,}937{,}983 & 8{,}388{,}608 & $3.0851\times10^{-22}$ & $7.8715\times10^{-21}$ \\
7 & 192{,}937{,}984--201{,}326{,}591 & 8{,}388{,}608 & $1.6385\times10^{-21}$ & $1.1766\times10^{-20}$ \\
8 & 201{,}326{,}592--209{,}715{,}199 & 8{,}388{,}608 & $9.3056\times10^{-22}$ & $1.3699\times10^{-21}$ \\
9 & 209{,}715{,}200--218{,}103{,}807 & 8{,}388{,}608 & $4.1009\times10^{-22}$ & $1.6788\times10^{-21}$ \\
10 & 218{,}103{,}808--226{,}492{,}415 & 8{,}388{,}608 & $3.6380\times10^{-21}$ & $2.8733\times10^{-21}$ \\
11 & 226{,}492{,}416--234{,}881{,}023 & 8{,}388{,}608 & $1.9530\times10^{-22}$ & $6.3473\times10^{-21}$ \\
12 & 234{,}881{,}024--243{,}269{,}631 & 8{,}388{,}608 & $3.8124\times10^{-21}$ & $3.7502\times10^{-21}$ \\
13 & 243{,}269{,}632--251{,}658{,}239 & 8{,}388{,}608 & $3.9710\times10^{-21}$ & $5.0056\times10^{-21}$ \\
14 & 251{,}658{,}240--260{,}046{,}847 & 8{,}388{,}608 & $6.9901\times10^{-21}$ & $8.3073\times10^{-22}$ \\
15 & 260{,}046{,}848--268{,}435{,}455 & 8{,}388{,}608 & $1.3740\times10^{-21}$ & $9.4093\times10^{-22}$ \\
\midrule
\multicolumn{2}{@{}r}{\textbf{total}} & \textbf{134{,}217{,}727} & & \\
\bottomrule
\end{longtable}
}

\subsection{Audit of \texorpdfstring{$[2^{28},2^{29})$}{[2\^{}28,2\^{}29)}}

This range contains $2^{28}$ integers and again exactly one power of two.  The required checked
count is $2^{28}-1=268{,}435{,}455$.  Sixteen chunks of width $2^{24}$ were run at
$96$ bits.  The concatenated log has SHA-256 digest
\begin{center}
\sha{5edd9a63f1229ad688be78faeff3b120bfcf557444194c5882f451ef3f68477c} .
\end{center}
Every chunk returned zero failures, and the checked counts again agree with the combinatorial
total.


{\small
\begin{longtable}{@{}r p{0.28\textwidth} r r r@{}}
\caption{Directed-rounding audit on $[2^{28},2^{29})$.}\label{tab:scan29-new}\\
\toprule
Chunk & Integer range & Checked & Smallest lower gap & Smallest upper gap \\
\midrule
\endfirsthead
...
\midrule
\endhead
0 & 268{,}435{,}456--285{,}212{,}671 & 16{,}777{,}215 & $1.3357\times10^{-21}$ & $2.8468\times10^{-21}$ \\
1 & 285{,}212{,}672--301{,}989{,}887 & 16{,}777{,}216 & $1.7754\times10^{-21}$ & $6.2726\times10^{-22}$ \\
2 & 301{,}989{,}888--318{,}767{,}103 & 16{,}777{,}216 & $5.1906\times10^{-22}$ & $3.6531\times10^{-22}$ \\
3 & 318{,}767{,}104--335{,}544{,}319 & 16{,}777{,}216 & $3.8468\times10^{-21}$ & $1.9512\times10^{-21}$ \\
4 & 335{,}544{,}320--352{,}321{,}535 & 16{,}777{,}216 & $1.4372\times10^{-22}$ & $1.9892\times10^{-21}$ \\
5 & 352{,}321{,}536--369{,}098{,}751 & 16{,}777{,}216 & $5.1428\times10^{-22}$ & $1.3160\times10^{-22}$ \\
6 & 369{,}098{,}752--385{,}875{,}967 & 16{,}777{,}216 & $3.0851\times10^{-22}$ & $8.1046\times10^{-22}$ \\
7 & 385{,}875{,}968--402{,}653{,}183 & 16{,}777{,}216 & $4.9130\times10^{-24}$ & $8.1143\times10^{-22}$ \\
8 & 402{,}653{,}184--419{,}430{,}399 & 16{,}777{,}216 & $9.3056\times10^{-22}$ & $4.7107\times10^{-22}$ \\
9 & 419{,}430{,}400--436{,}207{,}615 & 16{,}777{,}216 & $4.1009\times10^{-22}$ & $2.5129\times10^{-22}$ \\
10 & 436{,}207{,}616--452{,}984{,}831 & 16{,}777{,}216 & $1.2750\times10^{-21}$ & $2.8218\times10^{-22}$ \\
11 & 452{,}984{,}832--469{,}762{,}047 & 16{,}777{,}216 & $1.9530\times10^{-22}$ & $7.4586\times10^{-22}$ \\
12 & 469{,}762{,}048--486{,}539{,}263 & 16{,}777{,}216 & $4.3653\times10^{-22}$ & $2.1994\times10^{-22}$ \\
13 & 486{,}539{,}264--503{,}316{,}479 & 16{,}777{,}216 & $5.7804\times10^{-22}$ & $5.2103\times10^{-23}$ \\
14 & 503{,}316{,}480--520{,}093{,}695 & 16{,}777{,}216 & $1.1154\times10^{-21}$ & $4.2736\times10^{-22}$ \\
15 & 520{,}093{,}696--536{,}870{,}911 & 16{,}777{,}216 & $6.0230\times10^{-22}$ & $9.3067\times10^{-22}$ \\
\midrule
\multicolumn{2}{@{}r}{\textbf{total}} & \textbf{268{,}435{,}455} & & \\
\bottomrule
\end{longtable}
}

\subsection{Extremal records and high-precision replay}

Across both new dyadic ranges, the smallest lower logarithmic margin recorded by the
range scan is
\begin{equation}
 4.9130004741674945\times10^{-24}
 \quad\text{at}\quad
 (m,n)=(388{,}909{,}543,\,41{,}170{,}668{,}399{,}174),
 \label{eq:new-global-lower}
\end{equation}
and the smallest upper margin is
\begin{equation}
 5.2102685068037388\times10^{-23}
 \quad\text{at}\quad
 (m,n)=(501{,}373{,}708,\,61{,}578{,}600{,}753{,}798).
 \label{eq:new-global-upper}
\end{equation}
Both one-input cases were replayed at $512$ bits.  The lower-record replay gives the sharper
directed margin
\[
 4.9141462461940518\times10^{-24},
\]
and a $140$-decimal evaluation gives
\[
 (388{,}909{,}543)^{\vartheta}-41{,}170{,}668{,}399{,}174
 =2.9188416434683008767103555\ldots\times10^{-10}.
\]
The upper-record replay gives
\[
 5.2103670350428438\times10^{-23},
\]
with actual gap
\[
 61{,}578{,}600{,}753{,}799-(501{,}373{,}708)^{\vartheta}
 =4.6288453654600268035074950\ldots\times10^{-9}.
\]
Thus the higher-precision runs preserve the neighboring integer and the strict sign while
refining the last digits of the coarser $96$-bit enclosures.  Both replays report zero
failures.

\begin{theorem}[Software-level finite exclusion through $2^{29}$]
\label{thm:finite29}
At the stated MPFR software-trust level, for every integer $1\le m<2^{29}$,
\[
 m^{\vartheta}\in\N
 \quad\Longrightarrow\quad
 m\text{ is a power of two}.
...
\begin{proof}
The inherited scans cover $m<2^{27}$, Table~\ref{tab:scan28-new} covers the next dyadic range,
and Table~\ref{tab:scan29-new} covers the following one.  At each non-power input the directed
interval for $\vartheta\log m$ lies strictly between the directed logarithms of two consecutive
integers.  If $m=2^x$ came from a nonintegral solution with $x<29$, then
$m<2^{29}$ and $m^\vartheta=3^x$ would be an integer at a non-power input, contradicting the
scan.  Equality $m=2^{29}$ forces $x=29$ by injectivity of the exponential, so a nonintegral
solution must satisfy $x>29$.
\end{proof}

The improvement from $27$ to $29$ remains computational rather than conceptual.  Its value is
that every hypothetical primitive generator in the exact conditional theory now begins beyond
$29$, which can be inserted into all finite-order spacing and determinant optimizations.  It
does not change the kernel-verified floor $x\ge12$ until the directed inequalities are converted
to exact rational certificates and replayed in Lean.


\section{Conclusions of the third continuation}
\label{sec:third-conclusion}

The Alaoglu--Erd\H{o}s conjecture remains open.  The work in this continuation does not hide
that fact behind a conditional theorem or a large finite computation.  Its strongest
unconditional mathematical conclusions are instead structural.

First, the conjecture is exactly the positive-integral, fixed-first-column instance
$\mathrm{MC}_{2,3}$ of the Matrix Coefficient Conjecture.  A hypothetical counterexample
produces the rank-one matrix
\[
 \begin{pmatrix}\log2\\\log3\end{pmatrix}(1,\beta),
\]
and every rational matrix coefficient splits into two factors that are separately nonzero.
This pinpoints the obstruction more narrowly than the generic phrase ``a case of Four
Exponentials.''

Second, the support-based auxiliary-polynomial strategy now has an exact calibration.  On
$R$ rational points of the irrational power curve, the minimum BK-degree of a rational
vanishing polynomial is $R-1$, attained by the thin Lagrange interpolant $Y-Q(X)$.  On the
$(2N)^2$ Matrix Coefficient grid this is $4N^2-1$, while the desired support condition is
$<N^2$.  The factor four is therefore not a defect of a particular determinant estimate; it
is the exact interpolation barrier.  Any proof along this route must use analytic or adelic
smallness to manufacture an impossible polynomial, not merely optimize the monomial set.

Third, the optimal graph interpolant has a new boundary normal form.  Its dyadic and triadic
dilation defects contain the complete interior grid polynomial, leaving residuals of degree
only $K-1$ tied by the boundary commutator identity
\eqref{eq:boundary-commutator}.  This is the most concrete new arithmetic object produced by
the continuation.  It separates the exact multiplicative recurrence in the interior from the
$O(K)$ boundary data where finite interpolation must fail.  The primitive contents and local
Newton polygons of these residuals should be computed before returning to larger determinant
families.

Finally, the directed-rounding scan moves the software-level lower bound from $x>27$ to
$x>29$.  This is useful for calibrating finite-order inequalities and future certificates, but
it remains qualitatively separate from the transcendence problem and does not alter the
kernel-verified bound $x\ge12$.

The resulting priority order is:
\begin{enumerate}
\item determine exact or asymptotic $p$-adic valuations of the primitive integer forms of
$U_K$ and $V_K$, especially whether the boundary commutator forces cancellation beyond the
termwise min-plus ceiling;
\item attack the singular-to-sparse implication of the Matrix Coefficient program with a
support-preserving product-formula construction, using the exact factor-four threshold as the
required quantitative benchmark;
\item formalize the fixed-base matrix-coefficient equivalence, the zero/support theorem, and
the exact BK interpolation threshold in Lean;
\item convert the finite MPFR intervals into compact exact rational certificates and replay
them in the kernel rather than extending the raw scan indefinitely.
\end{enumerate}

\begin{resultbox}
\textbf{Bottom line.}
No complete proof was found.  The continuation nevertheless adds three exact pieces of
mathematics---the fixed-base Matrix Coefficient equivalence, the sharp Newton-polytope
interpolation threshold, and the boundary-defect compression---together with a reproducible
finite exclusion through $2^{29}$.  The remaining wall is now visible both algebraically and
quantitatively: one must turn logarithmic singularity into an auxiliary object lying a factor
of four below the ordinary interpolation floor, or extract leading $p$-adic cancellation from
the $O(K)$ boundary residuals.
\end{resultbox}

\clearpage
\setcounter{section}{0}
\renewcommand{\thesection}{M\arabic{section}}
\renewcommand{\theHsection}{M.\arabic{section}}
\renewcommand{\theHsubsection}{\theHsection.\arabic{subsection}}
\renewcommand{\theHsubsubsection}{\theHsubsection.\arabic{subsubsection}}
\renewcommand{\theHtheorem}{\theHsection.\arabic{theorem}}

\section{Fourth continuation: measure theory, probability, and compact factors}
\label{mt:sec-scope}

This continuation starts from all results and audits in the inherited unified report.  In
particular, it assumes the exact conditional structure theorem: if the conjecture fails, there
is a least nonintegral solution $\beta>0$, with
\[
  M=2^\beta\in\N,
  \qquad A=3^\beta\in\N,
\]
...
\[
  ...
  \Cand=\{2^nM^k:n,k\in\Nzero\},
\]
and all coordinates are unique.  Write
\begin{equation}
  \beta=B+\alpha,
  \qquad B=\lfloor\beta\rfloor,
  \qquad 0<\alpha<1,
  \label{mt:eq-beta-alpha}
\end{equation}
and use the simultaneous primitive normalization
\begin{equation}
  M=2^aW,
  \quad W>1\text{ odd},
  \qquad
  ...
  \label{mt:eq-WZ}
\end{equation}
The inherited leastness theorem gives $\min(a,b)=0$.  The compact orbit and its denominator
exponents are
\begin{align}
  R_k&=\frac{M^k}{2^{\lfloor k\beta\rfloor}}
      =\frac{W^k}{2^{q^{(2)}_k}}
      =2^{\fract{k\alpha}},
  &q^{(2)}_k&=k(B-a)+\lfloor k\alpha\rfloor,
  \label{mt:eq-Rq}\\
  S_k&=\frac{A^k}{3^{\lfloor k\beta\rfloor}}
      =\frac{Z^k}{3^{q^{(3)}_k}}
      =3^{\fract{k\alpha}},
  &q^{(3)}_k&=k(B-b)+\lfloor k\alpha\rfloor.
  \label{mt:eq-Sq}
\end{align}

The purpose here is not to repackage equidistribution as a proof.  It is to determine, as
precisely as possible, what ordinary measure theory, ergodic theory, probability, profinite
harmonic analysis, and additive-combinatorial averaging can and cannot detect.  The outcome is
surprisingly rigid but negative: all natural fixed congruence and profinite-unit factors are asymptotically maximally distributed,
whereas the genuinely arithmetic information escapes to infinity in the $2$- and $3$-adic
valuation directions.  This produces several exact theorems and a sharply delimited new
research program, but no unconditional proof of the conjecture.

\subsection{Claim ledger}

Every new mathematical statement in Sections~\ref{mt:sec-difference-group}--
\ref{mt:sec-sumproduct} carries the label \statusnew.  The only external inputs are the
inherited exact conditional structure, Weyl's criterion, the inherited Baker--Matveev finite
irrationality measure, and, for the near-Sidon theorem, the finite-rank unit-equation theorem of
Evertse--Schlickewei--Schmidt.  The following table separates the conclusions from their
limitations.

\begin{longtable}{@{}p{0.27\textwidth}p{0.38\textwidth}p{0.28\textwidth}@{}}
\caption{New results in the measure-theoretic continuation.}
\label{mt:tab-ledger}\\
\toprule
Result & Exact conclusion & Why it does not prove the conjecture \\
\midrule
\endfirsthead
\toprule
Result & Exact conclusion & Why it does not prove the conjecture \\
\midrule
\endhead
Difference-group dichotomy & The group generated by differences of solutions is either
$\Z$ or $\Z+\Z\beta$; modulo $1$, failure creates a dense cyclic Haar-null subgroup. & Closure
and Haar measure cannot distinguish a dense null subgroup from the full circle. \\
Triangle$\times$Haar law & Conditional solutions become jointly uniform in macroscopic
barycentric coordinates and in the phase $\fract{k\beta}$. & The integrality event is an exact,
zero-measure condition, not a positive-measure target. \\
Effective discrepancy & Baker--Matveev gives $D_K\ll K^{-1/\mu_M}$ and an effective weighted
counting law. & The exponent is finite but far too weak for exponentially small targets. \\
Profinite Beatty lift & $(k,\lfloor k\alpha\rfloor,\fract{k\alpha})$ is Haar distributed in
$\widehat\Z^2\times\Torus$; fixed congruence data are independent of phase. & The theorem is
blind to moduli growing at the exponential denominator scale. \\
Adelic anatomy & Compact unit parts are Haar distributed, while $2$- and $3$-adic sizes escape
linearly; after centering, both valuation defects equal $-\fract{k\alpha}$. & Compactifying the
escape collapses precisely the information one needs. \\
Sturmian probability law & Prefix fluctuations are bounded, correlations do not decay, and
the process has pure point spectrum and zero entropy. & Bernoulli, Poisson, central-limit, and
independent-event heuristics are inapplicable. \\
Logarithmic torus law & The full curve $u\mapsto(\fract u,\fract{u^{\thetaq}})$ is Haar at fixed
scale, even if it contains the conditional exact-hit semigroup. & Exact hits become visible only
at shrinking scales beyond weak convergence. \\
Metric parameter theorem & For almost every exponent, prescribed near-integrality occurs only
as permitted by Borel--Cantelli; exceptional limsup sets have explicit dimension bounds. & A
named exponentially algebraic exponent can lie in a null, even zero-dimensional, set. \\
Near-Sidon theorem & Every finite subset of a fixed finite-rank positive multiplicative group
has additive energy $2N^2-N+O(N)$; conditional candidate sets have asymptotically maximal
sumsets. & This is compatible with, rather than hostile to, a rank-two counterexample. \\
\bottomrule
\end{longtable}

\begin{statusbox}{\statusnew{} throughout this continuation unless an external theorem is
explicitly cited.  No claim below is represented as Lean-kernel verified.}
\end{statusbox}

\section{The difference group: a dense null arithmetic skeleton}
\label{mt:sec-difference-group}

The exact conditional monoid has a useful measure-theoretic shadow which is easy to miss if one
looks only at positive solutions.  Form the difference group
\begin{equation}
  \mathcal D:=\Sol-\Sol
  =\{x-y:x,y\in\Sol\}\subset\R.
  \label{mt:eq-difference-group}
\end{equation}
Every $t\in\mathcal D$ has $2^t,3^t\in\Q_{>0}$, because it is a quotient of two integral
output pairs.  Thus $\mathcal D/\Z$ is an arithmetic subgroup of the circle lying inside the
simultaneous rational-power locus, without requiring the unproved denominator-support lemma
for arbitrary rational powers.

\begin{theorem}[Difference-group dichotomy]
\label{mt:thm-difference-group}
For the solution monoid $\Sol$ exactly one of the following holds.
\begin{enumerate}[label=\textup{(\roman*)}]
\item The Alaoglu--Erd\H{o}s conjecture holds, $\Sol=\Nzero$, and
\[
  \mathcal D=\Z,
  \qquad \mathcal D/\Z=\{0\}.
\]
\item The conjecture fails with least generator $\beta$, and
\[
  \mathcal D=\Z+\Z\beta,
  \qquad
  \mathcal D/\Z=\{k\alpha\bmod1:k\in\Z\}.
\]
In the second case $\mathcal D/\Z$ is countable, dense in $\Torus=\R/\Z$, and has Haar measure
zero.
\end{enumerate}
\end{theorem}

\begin{proof}
If the conjecture holds, every nonnegative solution is an integer, and every integer is a
difference of two nonnegative integers.  If it fails, the exact monoid theorem gives
\[
 (\Nzero+\Nzero\beta)-(\Nzero+\Nzero\beta)=\Z+\Z\beta.
\]
Modulo $\Z$ this is the cyclic group generated by $\alpha=\beta-B$.  Irrationality of $\alpha$
makes the subgroup dense by Kronecker's theorem.  It is countable, hence Haar-null.
\end{proof}

\begin{corollary}[Closure is maximally discontinuous]
\label{mt:cor-closure-jump}
The closure of $\mathcal D/\Z$ is either the one-point set $\{0\}$ or the whole circle.  In the
failure branch, averaging the first $K$ positive generators gives Haar measure:
\[
  \frac1K\sum_{k=0}^{K-1}\delta_{\fract{k\beta}}
  \stackrel{*}{\longrightarrow}m_{\Torus},
\]
although every sampled phase belongs to a countable simultaneous rational-power set.
\end{corollary}

This is the first precise warning for measure-theoretic arguments.  Failure creates an
arithmetic set that is simultaneously negligible for Haar measure, dense for topology, and
perfectly equidistributed under its natural enumeration.  Any argument using only closure,
support, or weak limits has already discarded the distinction that must be proved.

The same phenomenon explains why a measurable extension argument is unavailable.  On the
dense cyclic subgroup one has homomorphisms
\[
  k\alpha\longmapsto M^k\in\Q_{>0}^{\times},
  \qquad
  k\alpha\longmapsto A^k\in\Q_{>0}^{\times}.
\]
They cannot extend continuously to the circle: a sequence $k_j\alpha\to0$ can have
$M^{k_j}\to\infty$.  Since the domain subgroup is Haar-null, measurable-extension theorems do
not apply either.  The discontinuity is not a technical nuisance; it is the arithmetic
content.

\section{A joint triangle--Haar limit law for all conditional solutions}
\label{mt:sec-triangle-haar}

The inherited report proves exact counts and blockwise phase equidistribution.  The next result
combines the macroscopic lattice shape and the microscopic phase into one weak limit.  Put
\begin{equation}
  L_T:=\{(n,k)\in\Nzero^2:n+\beta k\le T\},
  \qquad N_T:=\#L_T.
  \label{mt:eq-LT}
\end{equation}
The elementary lattice estimate is
\begin{equation}
  N_T=\frac{T^2}{2\beta}+O_\beta(T).
  \label{mt:eq-NT}
\end{equation}
Let
\[
  \Delta:=\{(u,v)\in\R_{\ge0}^2:u+v\le1\}.
\]

\begin{theorem}[Conditional triangle$\times$Haar law]
\label{mt:thm-triangle-haar}
Assume the conjecture fails with least generator $\beta$.  For every continuous
$F:\Delta\times\Torus\to\C$,
\begin{equation}
 \frac1{N_T}\sum_{(n,k)\in L_T}
 F\!\left(\frac nT,\frac{\beta k}{T},\fract{k\beta}\right)
 \longrightarrow
 2\int_\Delta\int_\Torus F(u,v,t)\,dt\,du\,dv.
 \label{mt:eq-triangle-haar}
\end{equation}
Equivalently, the empirical measures converge to uniform area measure on $\Delta$ (density
$2$) times Haar measure on the circle.
\end{theorem}

\begin{proof}
It suffices first to take
$F(u,v,t)=f(u,v)\exp(2\pi iht)$ with $f$ Lipschitz and $h\in\Z$.  For $h=0$, the sum is an
anisotropic Riemann sum: one lattice cell has area $\beta/T^2$, so
\[
  \sum_{(n,k)\in L_T}f(n/T,\beta k/T)
  =\frac{T^2}{\beta}\int_\Delta f(u,v)\,du\,dv+O_f(T).
\]
Division by \eqref{mt:eq-NT} gives the asserted density $2$.

Let $h\ne0$ and define
\[
  w_k:=\sum_{0\le n\le T-\beta k}f(n/T,\beta k/T),
  \qquad 0\le k\le T/\beta.
\]
The maximum of $|w_k|$ is $O_f(T)$ and its total variation is $O_f(T)$: increasing $k$ by one
moves every surviving summand by $O_f(T^{-1})$ in the second coordinate and deletes only
$O_\beta(1)$ boundary summands.  Since $\beta$ is irrational, the geometric partial sums
\[
  \sum_{k=0}^{K}\exp(2\pi ihk\beta)
\]
are bounded by a constant depending on $h$ and $\beta$.  Abel summation therefore gives
\[
  \sum_k w_k\exp(2\pi ihk\beta)=O_{f,h,\beta}(T)=o(N_T).
\]
Finite Fourier sums in $t$ and Lipschitz functions in $(u,v)$ are uniformly dense in
$C(\Delta\times\Torus)$, so the general case follows by approximation.
\end{proof}

\subsection{Probabilistic coordinates}

The theorem has a compact probabilistic interpretation.  Choose $(n,k)$ uniformly from $L_T$
and define
\begin{equation}
  H_T=\frac{n+\beta k}{T},
  \qquad
  C_T=\frac{\beta k}{n+\beta k}
  \quad (C_T:=0\text{ at }(0,0)),
  \qquad
  P_T=\fract{k\beta}.
  \label{mt:eq-prob-coords}
\end{equation}

\begin{corollary}[Independent height, composition, and phase]
\label{mt:cor-beta-uniform}
As $T\to\infty$,
\[
  (H_T,C_T,P_T)\Longrightarrow(H,C,P),
\]
where $H$ has density $2h$ on $[0,1]$, while $C$ and $P$ are independent uniform variables on
$[0,1]$, independent also of $H$.  Equivalently,
\[
  \Pr(H\le s)=s^2,
  \qquad
  (u,v,1-u-v)\sim\operatorname{Dirichlet}(1,1,1).
\]
\end{corollary}

\begin{proof}
In the limit law of Theorem~\ref{mt:thm-triangle-haar}, set $h=u+v$ and $c=v/(u+v)$.  The
Jacobian is $h$, so the density $2\,du\,dv$ becomes $2h\,dh\,dc$.  The phase factor is already
independent Haar measure.
\end{proof}

Thus a random conditional solution has a macroscopic height biased toward the outer boundary,
but its split between the free integer generator and the hypothetical generator is uniform.
There is no concentration near either coordinate axis that a density argument could exploit.

\subsection{A synchronized Benford law on the compact arc}

For $x=n+k\beta$ one has $\fract x=\fract{k\beta}$.  The normalized output pair is therefore
\[
 \left(\frac{2^x}{2^{\lfloor x\rfloor}},
       \frac{3^x}{3^{\lfloor x\rfloor}}\right)
 =\left(2^{\fract{k\beta}},3^{\fract{k\beta}}\right).
\]

\begin{corollary}[Arc law and marginal densities]
\label{mt:cor-arc-law}
Under uniform sampling from $L_T$, the normalized pair converges to the pushforward of Lebesgue
measure by
\[
  t\longmapsto(2^t,3^t),\qquad 0\le t<1.
\]
Its first marginal has density $dr/(r\log2)$ on $[1,2)$ and its second has density
$ds/(s\log3)$ on $[1,3)$.  The two marginals are not independent: they are perfectly coupled
by $s=r^{\thetaq}$.
\end{corollary}

This is a genuine Benford-type law, but it is a law for conditional solutions themselves.  It
shows that the exact integrality structure is compatible with the most featureless possible
phase distribution.

\section{Effective discrepancy inherited from logarithmic forms}
\label{mt:sec-effective}

The qualitative limit above uses only irrationality.  The inherited
Theorem~\ref{thm:baker-beta} supplies effective constants $c_M>0$ and $\mu_M<\infty$ such that
\begin{equation}
  \distZ{h\beta}\ge c_M h^{1-\mu_M}
  \qquad(h\ge1).
  \label{mt:eq-finite-mu}
\end{equation}
The constants depend effectively on the hypothetical integer $M$ and are not uniform in an
unknown counterexample.

\begin{theorem}[Effective discrepancy of the conditional rotation]
\label{mt:thm-discrepancy}
Let
\[
 D_K(\beta):=\sup_{I\subset\Torus}
 \left|\frac1K\#\{0\le k<K:\fract{k\beta}\in I\}-|I|\right|,
\]
where the supremum is over intervals.  Under a counterexample,
\begin{equation}
  D_K(\beta)\ll_M K^{-1/\mu_M}.
  \label{mt:eq-discrepancy}
\end{equation}
\end{theorem}

\begin{proof}
The Erd\H{o}s--Tur\'an inequality and the geometric-sum bound give, for every $H\ge1$,
\[
 D_K(\beta)
 \ll \frac1H+\frac1K\sum_{h\le H}
       \frac1h\min\!\left(K,\frac1{\distZ{h\beta}}\right).
\]
Using \eqref{mt:eq-finite-mu}, the sum is
$O_M(H^{\mu_M-1}/K)$.  Taking $H\asymp K^{1/\mu_M}$ proves the claim.
\end{proof}

\begin{corollary}[Effective height--phase counting]
\label{mt:cor-weighted-count}
Fix $0\le c<d$ and an interval $I\subset\Torus$.  Then
\begin{align}
 &\#\bigl\{(n,k)\in\Nzero^2:
      cT<n+\beta k\le dT,
      \ \fract{k\beta}\in I\bigr\}
 \notag\\
 &\hspace{3cm}=
 |I|\frac{d^2-c^2}{2\beta}T^2
 +O_{M,c,d}\!\left(T^{2-1/\mu_M}\right).
 \label{mt:eq-weighted-count}
\end{align}
\end{corollary}

\begin{proof}
For each $k$, let $w_k$ be the number of permitted integers $n$.  The weights are $O(T)$,
have support of length $O(T)$, and total variation $O(T)$.  The discrepancy theorem says that
every prefix phase count has error $O_M(T^{1-1/\mu_M})$.  Abel summation gives the stated
weighted error.  The unweighted lattice count contributes the main term and an $O(T)$ boundary
error, absorbed by the displayed bound.
\end{proof}

The gain is polynomial, which is useful for every fixed-scale statistic developed below.  It
still cannot reach the exponentially small neighborhoods naturally associated with exact
integrality of numbers of exponential height.  The distinction between ``effective'' and
``sufficiently effective'' is essential here.

\section{The profinite Beatty lift and fixed-modulus independence}
\label{mt:sec-profinite}

The denominator exponents in \eqref{mt:eq-Rq}--\eqref{mt:eq-Sq} contain the same Beatty term
$\lfloor k\alpha\rfloor$.  It is therefore natural to retain, simultaneously, the real phase
and every fixed congruence class of both $k$ and $\lfloor k\alpha\rfloor$.  The resulting
limit law is stronger than ordinary equidistribution on the circle.

\begin{theorem}[Profinite Beatty lift]
\label{mt:thm-profinite-beatty}
Let $\alpha$ be irrational.  Then
\begin{equation}
 k\longmapsto
 \bigl(k,\lfloor k\alpha\rfloor,\fract{k\alpha}\bigr)
 \label{mt:eq-profinite-sequence}
\end{equation}
is equidistributed in
$\widehat{\Z}^{\,2}\times\Torus$ with respect to the product of Haar measures.  Concretely,
for every $d\ge1$, residues $r,j\pmod d$, and interval $I\subset\Torus$,
\begin{equation}
 \frac1K\#\left\{0\le k<K:
  k\equiv r\pmod d,\quad
  \lfloor k\alpha\rfloor\equiv j\pmod d,\quad
  \fract{k\alpha}\in I
 \right\}
 \longrightarrow \frac{|I|}{d^2}.
 \label{mt:eq-profinite-cell}
\end{equation}
\end{theorem}

\begin{proof}
Fix $d$ and first restrict to $k=d\ell+r$.  Put
\[
 x_\ell=\fract{\ell\alpha+r\alpha/d}.
\]
The sequence $(x_\ell)$ is equidistributed on the circle.  Moreover,
\begin{equation}
 \left(\lfloor(d\ell+r)\alpha\rfloor\bmod d,
       \fract{(d\ell+r)\alpha}\right)
 =\left(\lfloor d x_\ell\rfloor,\fract{d x_\ell}\right).
 \label{mt:eq-digit-phase-map}
\end{equation}
The map
\[
 x\longmapsto\bigl(\lfloor dx\rfloor,\fract{dx}\bigr)
\]
sends Lebesgue measure on $[0,1)$ to the product of uniform measure on
$\Z/d\Z$ and Lebesgue measure on $[0,1)$.  Thus the conditional frequency for a fixed $r$ is
$|I|/d$; the residue class $r$ itself has density $1/d$.  This proves
\eqref{mt:eq-profinite-cell}.  Such cylinder sets determine Haar measure on the inverse limit.
\end{proof}

The theorem says more than ``no congruence obstruction was found.''  It says that every
\emph{fixed} congruence observation is asymptotically independent of the Archimedean phase,
even though both are generated by the same integer $k$.

\subsection{An effective range for a growing modulus}

The argument also quantifies exactly how far the inherited finite irrationality measure can
push a modulus that is allowed to grow.

\begin{theorem}[Effective profinite cells]
\label{mt:thm-effective-profinite}
Assume \eqref{mt:eq-finite-mu}.  Let $d\ge1$, let
$E\subset(\Z/d\Z)^2$, and let $I\subset\Torus$ be an interval.  Then
\begin{align}
 &\#\left\{0\le k<K:
    \bigl(k,\lfloor k\alpha\rfloor\bigr)\bmod d\in E,
    \ \fract{k\alpha}\in I\right\}
 \notag\\
 &\hspace{2cm}=
 K\frac{|E|}{d^2}|I|
 +O_M\!\left(K^{1-1/\mu_M}d^{1+1/\mu_M}+d^2\right).
 \label{mt:eq-effective-profinite}
\end{align}
Consequently the normalized error tends to zero uniformly in $E$ and $I$ whenever
\begin{equation}
 d^{\mu_M+1}=o(K).
 \label{mt:eq-growing-modulus-range}
\end{equation}
\end{theorem}

\begin{proof}
For a fixed first residue $r$, write $k=d\ell+r$ as above.  A condition
$\lfloor k\alpha\rfloor\equiv j\pmod d$ together with
$\fract{k\alpha}\in I$ pulls back under \eqref{mt:eq-digit-phase-map} to at most two intervals
of total length $|I|/d$ for the rotation $x_\ell$.  There are $K/d+O(1)$ available values of
$\ell$.  Theorem~\ref{mt:thm-discrepancy} therefore gives an error
\[
 O_M\!\left((K/d)^{1-1/\mu_M}+1\right)
\]
for each pair $(r,j)$.  Summing over $E$, and using $|E|\le d^2$, yields
\eqref{mt:eq-effective-profinite}.  Dividing by $K$ proves the final assertion.
\end{proof}

This is a concrete threshold, not merely a qualitative caveat.  The theorem controls
polynomially growing moduli of sufficiently small degree.  By contrast, the exact denominator
powers in \eqref{mt:eq-Rq}--\eqref{mt:eq-Sq} are exponential in $k$.  The missing range is
therefore not a logarithmic loss but an exponential gulf.

\subsection{The normalized output pair modulo a fixed integer}

Let $q$ be coprime to $6WZ$, and let $d$ be the exponent of the finite group
$(\Z/q\Z)^\times$.  In that group define
\begin{equation}
 c_2=W2^{a-B},
 \qquad
 c_3=Z3^{b-B}.
 \label{mt:eq-c2c3}
\end{equation}
Negative powers are interpreted using inverses modulo $q$.  Equations
\eqref{mt:eq-Rq}--\eqref{mt:eq-Sq} give
\begin{equation}
 R_k\equiv c_2^k2^{-\lfloor k\alpha\rfloor},
 \qquad
 S_k\equiv c_3^k3^{-\lfloor k\alpha\rfloor}
 \pmod q.
 \label{mt:eq-RS-modq}
\end{equation}
Let
\begin{equation}
 H_q:=\left\{
   \bigl(c_2^r2^{-j},c_3^r3^{-j}\bigr):
   r,j\in\Z/d\Z
 \right\}
 \subset\bigl((\Z/q\Z)^\times\bigr)^2.
 \label{mt:eq-Hq}
\end{equation}
It is the image of a homomorphism from $(\Z/d\Z)^2$, hence a subgroup.

\begin{theorem}[Fixed congruence factors are independent of phase]
\label{mt:thm-fixed-congruence}
For every $q$ coprime to $6WZ$,
\begin{equation}
 \bigl([R_k]_q,[S_k]_q,\fract{k\alpha}\bigr)
 \quad\text{is equidistributed in}\quad
 H_q\times\Torus
 \label{mt:eq-fixed-congruence-law}
\end{equation}
with uniform measure on $H_q$ and Lebesgue measure on the circle.  In particular, if
\begin{equation}
 \varepsilon_k:=\lfloor(k+1)\alpha\rfloor-\lfloor k\alpha\rfloor
 =\mathbf1_{[1-\alpha,1)}(\fract{k\alpha}),
 \label{mt:eq-sturmian-bit}
\end{equation}
then for each $\epsilon\in\{0,1\}$ and each $h\in H_q$,
\begin{equation}
 \frac{\#\{0\le k<K:([R_k]_q,[S_k]_q)=h,
                       \ \varepsilon_k=\epsilon\}}
      {\#\{0\le k<K:\varepsilon_k=\epsilon\}}
 \longrightarrow \frac1{|H_q|}.
 \label{mt:eq-bit-congruence-independent}
\end{equation}
\end{theorem}

\begin{proof}
By Theorem~\ref{mt:thm-profinite-beatty}, the triple
\[
 \bigl(k\bmod d,\lfloor k\alpha\rfloor\bmod d,
       \fract{k\alpha}\bigr)
\]
has product-uniform limiting distribution.  Push the first two coordinates forward by the
homomorphism
\[
 (r,j)\longmapsto(c_2^r2^{-j},c_3^r3^{-j}).
\]
Uniform measure on a finite group maps to uniform measure on its image, and the phase remains
independent.  Finally, $\varepsilon_k$ is the indicator of one fixed phase interval, so
conditioning on either value of the bit does not change the limiting distribution on $H_q$.
\end{proof}

Taking compatible products over finitely many $q$ gives the same statement for every finite
quotient of the prime-to-$6WZ$ profinite unit group.  Equivalently, the normalized pair has a
Haar-distributed compact profinite factor independent of the irrational phase.  Thus no
argument based on a fixed modulus, or on any fixed finite collection of moduli, can distinguish
the hypothetical orbit from its Haar model.  A successful congruence argument must let the
modulus grow with $k$ and must cross far beyond the range
\eqref{mt:eq-growing-modulus-range}.

\section{Adelic anatomy: compact randomness and noncompact escape}
\label{mt:sec-adelic}

The fixed-modulus theorem may look paradoxical because the same rational numbers have
increasingly large denominator powers.  The paradox disappears once compact unit coordinates
and noncompact valuation coordinates are separated.

Put $t_k=\fract{k\alpha}$.  The denominator exponents satisfy the exact centered identities
\begin{equation}
 q^{(2)}_k=k(\beta-a)-t_k,
 \qquad
 q^{(3)}_k=k(\beta-b)-t_k.
 \label{mt:eq-centered-q}
\end{equation}
Consequently
\begin{equation}
 \left(
 ...
 \right)
 =(t_k,t_k,-t_k,-t_k).
 \label{mt:eq-centered-fourfold}
\end{equation}
The Archimedean phase and both centered valuation defects are therefore one and the same
circle coordinate.

\begin{theorem}[Escape of $p$-adic mass]
\label{mt:thm-padic-escape}
Under a counterexample,
\begin{equation}
 |R_k|_2=2^{q^{(2)}_k}\longrightarrow\infty,
 \qquad
 |S_k|_3=3^{q^{(3)}_k}\longrightarrow\infty.
 \label{mt:eq-padic-escape}
\end{equation}
Hence the empirical measures of $(R_k,S_k)$ on $\Q_2\times\Q_3$ are not tight: for every
compact $C\subset\Q_2\times\Q_3$,
\begin{equation}
 \frac1K\#\{0\le k<K:(R_k,S_k)\in C\}\longrightarrow0.
 \label{mt:eq-escape-compact}
\end{equation}
\end{theorem}

\begin{proof}
Because $W$ is odd and $3\nmid Z$,
\[
 |W^k/2^{q^{(2)}_k}|_2=2^{q^{(2)}_k},
 \qquad
 |Z^k/3^{q^{(3)}_k}|_3=3^{q^{(3)}_k}.
\]
The slopes $\beta-a=\log_2W$ and $\beta-b=\log_3Z$ are positive, while $0\le t_k<1$, so
\eqref{mt:eq-centered-q} implies divergence.  A compact subset of a non-Archimedean local
field is bounded, and therefore every compact $C$ is eventually avoided.
\end{proof}

There is nevertheless a compact unit part.  Let
\begin{equation}
 \mathcal H:=\overline{\langle(W,Z)\rangle}
 \subset\Z_2^\times\times\Z_3^\times,
 \label{mt:eq-unit-H}
\end{equation}
where the closure is taken in the profinite topology.

\begin{theorem}[Product law for phase and unit parts]
\label{mt:thm-unit-product}
The sequence
\begin{equation}
 \bigl(t_k,W^k,Z^k\bigr)
 \label{mt:eq-unit-sequence}
\end{equation}
is equidistributed in $\Torus\times\mathcal H$ with respect to Lebesgue measure times Haar
measure.  Equivalently, after removing the escaping denominator powers,
\begin{equation}
 2^{q^{(2)}_k}R_k=W^k,
 \qquad
 3^{q^{(3)}_k}S_k=Z^k
 \label{mt:eq-unit-removal}
\end{equation}
have maximally distributed compact unit coordinates independent of $t_k$.
\end{theorem}

\begin{proof}
Apply the character criterion on the compact abelian group
$\Torus\times\mathcal H$.  A character has the form
$e^{2\pi iht}\chi(u,v)$.  Along \eqref{mt:eq-unit-sequence} its value is
\[
 \left(e^{2\pi ih\alpha}\chi(W,Z)\right)^k.
\]
If $h=0$ and $\chi$ is nontrivial, then $\chi(W,Z)\ne1$, because powers of $(W,Z)$ are dense
in $\mathcal H$.  If $h\ne0$, the number $e^{2\pi ih\alpha}$ is not a root of unity, whereas
$\chi(W,Z)$ is a root of unity because $\mathcal H$ is profinite.  Thus the displayed ratio is
never $1$ for a nontrivial character, and its Ces\`aro average vanishes.
\end{proof}

This is the central measure-theoretic diagnosis.  The compact coordinates are not merely
unobstructed; they are Haar-random in the exact deterministic sense of equidistribution.  The
only data that do not wash out are the linearly escaping valuations and their exact coupling to
the real phase in \eqref{mt:eq-centered-fourfold}.  One-point compactification would turn this
escape into a single atom at infinity and erase the coupling.  Any promising adelic measure
must therefore be renormalized or nonstationary: it must retain both the moving valuation
frame and the phase.

\section{The Sturmian probability model is rigid, not random}
\label{mt:sec-sturmian-probability}

A probabilistic vocabulary is useful only if it respects the dependence structure.  Give the
initial phase $T_0$ the uniform distribution on $\Torus$ and define the stationary binary
process
\begin{equation}
 \xi_j(T_0):=
 \mathbf1_{[1-\alpha,1)}\bigl(\fract{T_0+j\alpha}\bigr).
 \label{mt:eq-sturmian-process}
\end{equation}
The deterministic word $\varepsilon_k$ in \eqref{mt:eq-sturmian-bit} is one orbit of this
process.

\begin{theorem}[Bounded fluctuations and exact covariance]
\label{mt:thm-sturmian-probability}
For every $N\ge1$,
\begin{equation}
 \sum_{j=0}^{N-1}\xi_j(T_0)
 =\lfloor T_0+N\alpha\rfloor-\lfloor T_0\rfloor.
 \label{mt:eq-sturmian-prefix}
\end{equation}
Thus the sum takes only the two values $\lfloor N\alpha\rfloor$ and
$\lceil N\alpha\rceil$, has mean $N\alpha$, and has variance
\begin{equation}
 \fract{N\alpha}\bigl(1-\fract{N\alpha}\bigr)\le\frac14.
 \label{mt:eq-sturmian-variance}
\end{equation}
Moreover, with $\delta_\ell=\fract{\ell\alpha}$,
\begin{equation}
 \operatorname{Cov}(\xi_0,\xi_\ell)
 =g_\alpha(\delta_\ell)-\alpha^2,
 \qquad
 g_\alpha(\delta)=
 \max(0,\alpha-\delta)+\max(0,\alpha+\delta-1).
 \label{mt:eq-sturmian-covariance}
\end{equation}
In particular, the correlations do not tend to zero.
\end{theorem}

\begin{proof}
Each bit is the carry in adding $\alpha$:
\[
 \xi_j(T_0)=
 \lfloor T_0+(j+1)\alpha\rfloor-
 \lfloor T_0+j\alpha\rfloor.
\]
Telescoping proves \eqref{mt:eq-sturmian-prefix}; averaging over $T_0\in[0,1)$ gives the stated
two-point distribution and variance.  For the covariance, the joint success probability is
the length of the intersection of two circular intervals of length $\alpha$ separated by
$\delta_\ell$, which is exactly $g_\alpha(\delta_\ell)$.  There are infinitely many $\ell$
with $\delta_\ell\to0$, and along them the covariance tends to
$\alpha(1-\alpha)>0$.
\end{proof}

\begin{corollary}[No Bernoulli or central-limit heuristic]
\label{mt:cor-no-bernoulli}
The Sturmian process is a factor of an irrational rotation, hence has zero entropy and pure
point dynamical spectrum.  Its centered partial sums are uniformly bounded, so normalization
by $\sqrt N$ converges to zero rather than to a nondegenerate Gaussian.  It is not mixing, and
Poisson or independent-event estimates for its jump pattern have no probabilistic basis.
\end{corollary}

The dependence is even stronger at the level of exact integrality.  A single nonintegral hit
$\beta$ forces every $n+k\beta$ to be a hit.  By uniqueness of conditional coordinates,
\begin{equation}
 \#\{m\in\Cand:m\le X\}
 =\frac{(\log X)^2}{2\beta(\log2)^2}+O_\beta(\log X).
 \label{mt:eq-candidate-log-count}
\end{equation}
Thus the events are not rare independent accidents: one seed deterministically creates a
rank-two semigroup with quadratic logarithmic growth.  A second-moment argument that treats
them as weakly dependent is aimed at the wrong stochastic model.

\section{Logarithmic Haar measure on the power curve}
\label{mt:sec-log-haar}

The original conjecture can be viewed without selecting a counterexample.  Put
\begin{equation}
 \thetaq=\log_2 3
 \qquad\text{and}\qquad
 \Phi(u)=\bigl(\fract u,\fract{u^{\thetaq}}\bigr)
 \quad(u>0).
 \label{mt:eq-power-curve}
\end{equation}
For an integer $m$, the point $\Phi(m)$ is the origin exactly when $m^{\thetaq}$ is an integer.
The conjecture says that these exact hits are precisely $m=2^n$.

The curve has the unconditional semiconjugacy
\begin{equation}
 \Phi(2u)=T_{2,3}\Phi(u),
 \qquad
 T_{2,3}(x,y)=(2x,3y)\pmod1.
 \label{mt:eq-dyadic-semiconjugacy}
\end{equation}
If a counterexample $M=2^\beta$, $A=3^\beta$ exists, it gains a second semiconjugacy
\begin{equation}
 \Phi(Mu)=T_{M,A}\Phi(u),
 \qquad
 T_{M,A}(x,y)=(Mx,Ay)\pmod1.
 \label{mt:eq-counter-semiconjugacy}
\end{equation}
These relations tempt one to seek a rigidity theorem for invariant measures.  The next result
shows why the most natural measure is already too featureless.

\begin{theorem}[Logarithmic Haar law]
\label{mt:thm-log-haar}
Let $m_{\Torus^2}$ denote Haar probability measure on the two-torus.  Then
\begin{equation}
 \frac1{\log X}\int_1^X\delta_{\Phi(u)}\,\frac{du}{u}
 \stackrel{*}{\longrightarrow}m_{\Torus^2}
 \qquad(X\to\infty).
 \label{mt:eq-cumulative-log-haar}
\end{equation}
The local dyadic version also holds:
\begin{equation}
 \frac1{\log2}\int_X^{2X}\delta_{\Phi(u)}\,\frac{du}{u}
 \stackrel{*}{\longrightarrow}m_{\Torus^2}.
 \label{mt:eq-dyadic-log-haar}
\end{equation}
\end{theorem}

\begin{proof}
By Weyl's criterion it is enough to consider a nonzero Fourier mode $(h,\ell)\in\Z^2$ and show
that
\begin{equation}
 \int_X^{2X}\exp\bigl(2\pi i(hu+\ell u^{\thetaq})\bigr)\,\frac{du}{u}=o(1).
 \label{mt:eq-oscillatory-shell}
\end{equation}
If one of $h,\ell$ vanishes, one integration by parts (or the substitution $v=u^{\thetaq}$)
gives the claim.  If both are nonzero, the derivative
\[
 h+\ell\thetaq u^{\thetaq-1}
\]
has at most one zero.  Beyond a fixed compact interval it has constant sign, is monotone in
absolute value, and has size $\asymp_{h,\ell}X^{\thetaq-1}$ on $[X,2X]$.  The first derivative
test, with the smooth weight $1/u$, proves \eqref{mt:eq-oscillatory-shell}.  This yields
\eqref{mt:eq-dyadic-log-haar}.  Summing dyadic shells, or applying the same integration-by-parts
argument on $[1,X]$, gives a bounded nonzero-mode integral; division by $\log X$ proves
\eqref{mt:eq-cumulative-log-haar}.
\end{proof}

Haar measure is invariant under every integer toral endomorphism, including both maps in
\eqref{mt:eq-dyadic-semiconjugacy} and \eqref{mt:eq-counter-semiconjugacy}.  Therefore adding
the hypothetical second semiconjugacy does not contradict the limiting measure.  Moreover,
under a counterexample every
\begin{equation}
 u=2^nM^k
 \label{mt:eq-origin-hit-semigroup}
\end{equation}
is an exact origin hit, but a countable set has zero logarithmic measure and cannot alter
Theorem~\ref{mt:thm-log-haar}.

\subsection{The scale at which exact hits become visible}

The failure is quantitative.  Let $u_0\asymp X$ be an exact hit and write $u=u_0+s$.  For
$|s|=o(X)$,
\[
 u^{\thetaq}=u_0^{\thetaq}
 +\thetaq u_0^{\thetaq-1}s+O(X^{\thetaq-2}s^2).
\]
Thus a small torus box of side widths $\varepsilon_1,\varepsilon_2$ around the origin captures,
near this hit, a logarithmic interval of size
\begin{equation}
 \asymp
 \min\!\left(\frac{\varepsilon_1}{X},
              \frac{\varepsilon_2}{X^{\thetaq}}\right).
 \label{mt:eq-one-hit-log-mass}
\end{equation}
Under the conditional semigroup there are $\asymp_\beta\log X$ exact hits in a multiplicative
shell $[X,2X]$.  Their combined local mass is therefore of order
\begin{equation}
 (\log X)
 \min\!\left(\frac{\varepsilon_1}{X},
              \frac{\varepsilon_2}{X^{\thetaq}}\right),
 \label{mt:eq-all-hit-log-mass}
\end{equation}
whereas Haar predicts box mass $\asymp\varepsilon_1\varepsilon_2$.  For the hits to compete with
the background, both coordinate widths must enter the critical shrinking regime:
\begin{equation}
 \varepsilon_2\lesssim\frac{\log X}{X}
 \qquad\text{and}\qquad
 \varepsilon_1\lesssim\frac{\log X}{X^{\thetaq}}.
 \label{mt:eq-visibility-scale}
\end{equation}
This scale calculation is the real-variable counterpart of the growing-modulus barrier in
Section~\ref{mt:sec-profinite}.  Fixed boxes and weak convergence necessarily miss the exact
hits; the relevant targets shrink essentially exponentially in the natural exponent height.

\section{A metric theorem in the exponent parameter}
\label{mt:sec-metric-parameter}

One can also randomize the exponent rather than the input.  This gives strong almost-everywhere
statements, but it exposes a familiar logical limitation of metric number theory: the exponent
$\thetaq=\log_2 3$ is a distinguished named point, not a random parameter.

Fix $0<a<b$, integers $m\ge2$, and $0<\psi_m\le1/2$.  Define
\begin{equation}
 E_m(\psi):=
 \{c\in[a,b]:\distZ{m^c}<\psi_m\}.
 \label{mt:eq-Em}
\end{equation}

\begin{theorem}[Borel--Cantelli and dimension bounds for irrational powers]
\label{mt:thm-metric-parameter}
There is a constant $C_{a,b}$ such that
\begin{equation}
 \lambda(E_m(\psi))\le C_{a,b}\psi_m
 \qquad(m\ge2).
 \label{mt:eq-Em-measure}
\end{equation}
Consequently, if $\sum_m\psi_m<\infty$, then for Lebesgue-almost every $c\in[a,b]$ the
inequality $\distZ{m^c}<\psi_m$ holds for only finitely many $m$.

For $\psi_m=m^{-\tau}$ with $\tau>0$, the limsup set
\begin{equation}
 \mathcal E_\tau(a,b):=
 \{c\in[a,b]:\distZ{m^c}<m^{-\tau}
   \text{ for infinitely many }m\}
 \label{mt:eq-Etau}
\end{equation}
satisfies
\begin{equation}
 \dim_{\mathrm H}\mathcal E_\tau(a,b)
 \le \min\!\left(1,\frac{b+1}{b+\tau}\right).
 \label{mt:eq-Etau-dimension}
\end{equation}
\end{theorem}

\begin{proof}
Use $y=m^c$, so $dc=dy/(y\log m)$.  In the interval
$[m^a,m^b]$, the condition $\distZ y<\psi_m$ is covered by intervals of total weighted length
\[
 \ll \frac{\psi_m}{\log m}
 \sum_{m^a+O(1)\le n\le m^b+O(1)}\frac1n
 \ll_{a,b}\psi_m,
\]
which proves \eqref{mt:eq-Em-measure}; the first Borel--Cantelli lemma gives the almost-everywhere
claim.

For the dimension estimate, the component around $m^c=n$ pulls back to an interval of length
\[
 \ll\frac{m^{-\tau}}{n\log m}.
\]
For $0<s<1$, the $s$-cost of this cover, summed over $n\le m^b+O(1)$, is
\[
 \ll
 \frac{m^{-\tau s}}{(\log m)^s}
 \sum_{n\le m^b+O(1)}n^{-s}
 \ll
 \frac{m^{b(1-s)-\tau s}}{(\log m)^s}.
\]
The sum over $m$ converges whenever
$s>(b+1)/(b+\tau)$.  The Hausdorff--Cantelli lemma gives
\eqref{mt:eq-Etau-dimension}; the ambient bound $1$ handles the remaining range.
\end{proof}

Under a counterexample, every $m\in\Cand$ satisfies $m^{\thetaq}\in\N$.  Hence, on any compact
interval containing $\thetaq$,
\begin{equation}
 \thetaq\in\bigcap_{\tau>0}\mathcal E_\tau(a,b).
 \label{mt:eq-theta-all-tau}
\end{equation}
The right-hand side has Hausdorff dimension zero by
\eqref{mt:eq-Etau-dimension}.  This is perfectly consistent: a prescribed number may belong to
a null set, a meagre set, or even a zero-dimensional set.  Metric rarity becomes a proof only
when supplemented by arithmetic information excluding the named parameter.

\section{Finite-rank unit equations force a near-Sidon law}
\label{mt:sec-sumproduct}

A different probabilistic intuition asks whether a hypothetical rank-two multiplicative
semigroup would create too many additive coincidences.  The finite-rank unit-equation theorem
shows the opposite: such sets are almost as additively collision-free as possible.

For a finite set $A\subset\R$, write
\begin{equation}
 E_+(A):=\#\{(a,b,c,d)\in A^4:a+b=c+d\}
 \label{mt:eq-additive-energy}
\end{equation}
for its ordered additive energy.

\begin{theorem}[Near-Sidon law in a fixed finite-rank multiplicative group]
\label{mt:thm-near-sidon}
Let $\Gamma\subset\R_{>0}^{\times}$ be a multiplicative group of finite rank.  Uniformly for
finite $A\subset\Gamma$ with $|A|=N$,
\begin{align}
 E_+(A)&=2N^2-N+O_\Gamma(N),
 \label{mt:eq-energy-near-sidon}\\
 |A+A|&=\frac{N(N+1)}2+O_\Gamma(N).
 \label{mt:eq-sumset-near-sidon}
\end{align}
\end{theorem}

\begin{proof}
The two trivial families $(a,b)=(c,d)$ and $(a,b)=(d,c)$ contribute exactly $2N^2-N$ ordered
solutions.  For every other solution divide $a+b=c+d$ by $d$ to obtain
\begin{equation}
 x+y-z=1,
 \qquad x,y,z\in\Gamma.
 \label{mt:eq-unit-equation-three}
\end{equation}
A degenerate solution of this unit equation has $x=z,y=1$ or $y=z,x=1$, corresponding exactly
to a trivial additive solution.  By the theorem of Evertse--Schlickewei--Schmidt~\cite{EvertseSchlickeweiSchmidt2002}, the number of
nondegenerate triples $(x,y,z)$ depends only on the rank of $\Gamma$.  For each such triple
there are at most $N$ choices of the scale $d\in A$.  This proves
\eqref{mt:eq-energy-near-sidon}.

There are $N(N+1)/2$ unordered pairs from $A$.  Two distinct unordered pairs with the same sum
produce a nontrivial energy solution; therefore the total excess of representations over
one per attained sum is $O_\Gamma(N)$.  This gives \eqref{mt:eq-sumset-near-sidon}.
\end{proof}

Apply the theorem to the conditional truncation
\begin{equation}
 \Cand_T:=\{2^nM^k:(n,k)\in L_T\},
 \qquad |\Cand_T|=N_T.
 \label{mt:eq-CandT}
\end{equation}
It lies in the rank-two group generated by $2$ and $M$.

\begin{corollary}[Conditional sum--product profile]
\label{mt:cor-conditional-sumproduct}
Under a counterexample,
\begin{align}
 |\Cand_T+\Cand_T|
   &=\frac{N_T^2}{2}+O_M(N_T),
 \label{mt:eq-CandT-sum}\\
 |\Cand_T\Cand_T|
   &=4N_T+O_\beta(T).
 \label{mt:eq-CandT-product}
\end{align}
In particular,
\begin{equation}
 \frac{|\Cand_T+\Cand_T|}{N_T^2}\longrightarrow\frac12,
 \qquad
 \frac{|\Cand_T\Cand_T|}{N_T}\longrightarrow4.
 \label{mt:eq-sumproduct-ratios}
\end{equation}
\end{corollary}

\begin{proof}
The additive statement is Theorem~\ref{mt:thm-near-sidon}.  Multiplication adds the unique
coordinate pairs, so $|\Cand_T\Cand_T|=|L_T+L_T|$.  The coordinatewise floor/ceiling split gives
\begin{equation}
 L_{2T-(1+\beta)}\subset L_T+L_T\subset L_{2T}.
 \label{mt:eq-Minkowski-sandwich}
\end{equation}
Indeed, if $n+\beta k\le2T-(1+\beta)$, split each coordinate into its floor and ceiling halves;
the heavier half has weight at most $T$.  Now use
$|L_s|=s^2/(2\beta)+O_\beta(s)$ and $N_T=T^2/(2\beta)+O_\beta(T)$.
\end{proof}

The conditional set is therefore simultaneously close to a Sidon set for addition and a
rank-two generalized progression for multiplication.  This is not an inconsistency.  It is a
particularly clean sum--product configuration, exactly of the kind permitted by finite-rank
unit equations.  Additive energy can become useful only if one extracts extra additive
relations tied specifically to the second-base identity $m^{\thetaq}\in\N$, rather than using
the multiplicative rank alone.

\section{Universal saturation of fixed compact observables}
\label{mt:sec-compact-saturation}

The preceding limit laws can be compressed into one statement.  It formalizes a useful
``no-go'' principle: a hypothetical counterexample is statistically generic on every compact
factor that is fixed before the orbit is allowed to grow.  The qualifier \emph{fixed} is
indispensable.  Moving moduli, shrinking targets, and unbounded valuations lie outside the
statement and are exactly where the unresolved arithmetic survives.

Let $G$ be a profinite group, let
\begin{equation}
 \pi:\widehat\Z^{\,2}\longrightarrow G
 \label{mt:eq-profinite-observable}
\end{equation}
be a continuous homomorphism, and put $H=\pi(\widehat\Z^{\,2})$.  For $k\ge0$ define
\begin{equation}
 g_k:=\pi\bigl(k,\lfloor k\alpha\rfloor\bigr)\in H.
 \label{mt:eq-gk}
\end{equation}
Every fixed collection of congruence classes is obtained this way.  So are the compact unit
coordinates $(W^k,Z^k)$: continuity of exponentiation extends
$k\mapsto(W^k,Z^k)$ from $\Z$ to $\widehat\Z$ with image $\mathcal H$.

\begin{theorem}[Fixed compact-factor saturation]
\label{mt:thm-compact-saturation}
Assume the conjecture fails.  For every continuous
$F:\Delta\times H\times\Torus\to\C$,
\begin{align}
 &\frac1{N_T}\sum_{(n,k)\in L_T}
 F\!\left(\frac nT,\frac{\beta k}{T},g_k,\fract{k\alpha}\right)
 \notag\\
 &\hspace{2cm}\longrightarrow
 2\int_\Delta\int_H\int_\Torus F(u,v,g,t)\,dt\,dm_H(g)\,du\,dv,
 \label{mt:eq-compact-saturation}
\end{align}
where $m_H$ is Haar probability measure.  Thus macroscopic lattice position, every fixed
profinite observable, and the Archimedean phase become mutually independent.
\end{theorem}

\begin{proof}
Locally constant functions are uniformly dense in $C(H)$.  Since $\pi$ is continuous, every
locally constant pullback through $\pi$ factors through
$(\Z/d\Z)^2$ for some $d$.  Fourier polynomials are uniformly dense in $C(\Torus)$, and
Lipschitz functions are uniformly dense in $C(\Delta)$.  It is therefore enough to take
\[
 F(u,v,g,t)=f(u,v)\,\mathbf1_C(g)\,e^{2\pi iht},
\]
where $f$ is Lipschitz, $h\in\Z$, and the pullback of the clopen set $C\subset H$ is a union of
residue cells modulo some fixed $d$.

For one such cell define
\[
 a_k=
 \mathbf1_{(k,\lfloor k\alpha\rfloor)\equiv(r,j)\, (\mathrm{mod}\ d)}
 e^{2\pi ih\fract{k\alpha}},
 \qquad
 \eta=\frac{\mathbf1_{h=0}}{d^2}.
\]
The profinite Beatty law, applied also to the real and imaginary parts of the Fourier
character, gives
\[
 A(K):=\sum_{0\le k<K}(a_k-\eta)=o(K).
\]
Consequently
$\max_{K\le cT}|A(K)|=o(T)$ for every fixed $c>0$.

Set
\[
 w_{T,k}:=\sum_{0\le n\le T-\beta k}
 f\!\left(n/T,\beta k/T\right),
 \qquad 0\le k\le T/\beta.
\]
As in the proof of Theorem~\ref{mt:thm-triangle-haar}, one has
$\max_k|w_{T,k}|=O_f(T)$ and total variation $O_f(T)$.  Abel summation therefore yields
\[
 \sum_k w_{T,k}(a_k-\eta)=o(T^2).
\]
The term with $\eta$ is the anisotropic Riemann sum over $\Delta$.  After division by
$N_T$, it gives the product of uniform area measure, the residue-cell mass $d^{-2}$, and the
circle Fourier integral.  Summing residue cells, then using finite linear combinations and
uniform approximation, proves the theorem.
\end{proof}

\begin{corollary}[Fixed-observable blindness]
\label{mt:cor-fixed-observable-blindness}
Consider a bounded continuous statistic built from finitely many of the following data:
\begin{enumerate}[label=\textup{(\alph*)}]
\item the scaled coordinates $n/T$ and $\beta k/T$;
\item the phase $\fract{k\alpha}$ and any fixed finite block of Sturmian jump bits;
\item residues modulo any fixed finite collection of moduli;
\item compact $p$-adic unit coordinates, or any fixed finite quotient of them.
\end{enumerate}
Its limit is obtained by sampling the triangle coordinate, the profinite coordinates, and the
phase from their product Haar law, and then deriving the Sturmian bits from that phase.  Thus
the congruence and unit factors are independent of the phase and of every fixed bit block; the
bits are, correctly, not asserted to be independent of the phase that determines them.  The
same statement holds for indicator functions whose discontinuity set has limiting measure
zero.
\end{corollary}

This is not a philosophical impossibility claim; it is a theorem about a large and natural
class of attempted witnesses.  A contradiction cannot come from the limiting expectation of
such an observable, because the hypothetical orbit already realizes the product-Haar answer.
One must let the observable itself vary with height, retain an unbounded coordinate, or use an
exact arithmetic property discontinuous at every relevant scale.

\subsection{The renormalized adelic joining}

The noncompact data can nevertheless be placed in a stationary moving frame.  Put
\begin{equation}
 \lambda_2=\beta-a=\log_2W,
 \qquad
 \lambda_3=\beta-b=\log_3Z.
 \label{mt:eq-lambda23}
\end{equation}
The common-floor structure gives not only
\eqref{mt:eq-centered-fourfold}, but also the exact linear relation
\begin{equation}
 q^{(2)}_k-q^{(3)}_k=k(b-a).
 \label{mt:eq-q-difference}
\end{equation}

\begin{corollary}[Stationary moving-frame limit]
\label{mt:cor-moving-frame}
The empirical measures of
\begin{equation}
 \Xi_k:=
 \bigl(t_k,
       q^{(2)}_k-k\lambda_2,
 ...
 \label{mt:eq-Xik}
\end{equation}
converge to the pushforward of
$m_\Torus\times m_{\mathcal H}$ under
\begin{equation}
 (t,u,v)\longmapsto(t,-t,-t,u,v).
 \label{mt:eq-moving-frame-map}
\end{equation}
In particular, the two centered valuation defects are perfectly correlated with each other
and with the Archimedean phase, while the compact unit pair is independent Haar measure.
\end{corollary}

\begin{proof}
The centered valuation identities are exact, and
Theorem~\ref{mt:thm-unit-product} supplies the product law for $(t_k,W^k,Z^k)$.
\end{proof}

There is a useful cocycle interpretation.  Let
$T_\alpha(t)=\fract{t+\alpha}$ and
$\varepsilon(t)=\lfloor t+\alpha\rfloor$.  Then
\begin{equation}
 \varepsilon(t)-\alpha=t-T_\alpha(t).
 \label{mt:eq-coboundary}
\end{equation}
Thus the centered valuation increment is a bounded coboundary, not a random walk.  Iterating
\eqref{mt:eq-coboundary} telescopes to the bounded Sturmian fluctuations of
Section~\ref{mt:sec-sturmian-probability}.  Any probabilistic argument that models the
valuation error as accumulated independent noise destroys this exact cohomological structure.

The moving-frame limit is the most faithful probability measure produced by this continuation.
It keeps precisely the phase information lost by one-point compactification.  It still does
not yield a contradiction: the graph measure in \eqref{mt:eq-moving-frame-map} exists and is
fully compatible with all presently known local and Archimedean constraints.  Its value is as
a target for an \emph{arithmetic} rigidity theorem, not as a rigidity theorem by itself.

\section{Literature update through August 21, 2026}
\label{mt:sec-literature-update}

A targeted search of arXiv, journal metadata, current references, and the exact statement of
the problem found no paper claiming an unconditional proof of the Alaoglu--Erd\H{o}s
conjecture.  The search did, however, locate several developments after the earlier literature
appendix that materially sharpen the strategic picture.  They fall into five groups.

\subsection{Matrices of logarithms: the direct frontier remains the same}

Dasgupta's first paper on ranks of logarithm matrices gives a modern account of Baker's theorem,
the Waldschmidt--Masser rank bound, and the Structural Rank and Matrix Coefficient
conjectures~\cite{DasguptaRanksI}.  Dasgupta and Kakde's second paper, now published in
\emph{Advances in Mathematics}, develops the Matrix Coefficient Conjecture and its
support-sensitive auxiliary-polynomial strategy~\cite{DasguptaKakdeMCC}.  In dimension two,
the conjecture is equivalent to Four Exponentials.  Consequently the fixed-base equivalence
proved in the third continuation remains located exactly on a live frontier, not behind a
recent theorem that the inherited report missed.

The most important negative fact from the literature search is also precise.  No subsequent
result was found that converts the weak singularity condition into the sparse auxiliary
polynomial required by the Matrix Coefficient strategy.  In the notation of the third
continuation, nothing currently bridges the exact factor-four gap between the ordinary
$4N^2-1$ interpolation floor and the $<N^2$ support threshold.  The measure-theoretic audit
reinforces this diagnosis: all fixed Fourier moments and compact residue factors are already
Haar, so any successful auxiliary object must use frequencies, supports, or local valuations
that grow with its order.

\subsection{Additive relations in irrational powers and sparse prime support}

Harrison's 2026 preprint proves that, uniformly in an irrational exponent $c$, sufficiently
large subsets of arithmetic progressions have asymptotically maximal iterated sumsets after
applying $x\mapsto x^c$; it also proves near-optimal bounds for additive relations by combining
functional transcendence with modern Pila--Wilkie counting in
$\R_{\exp}$~\cite{Harrison2026}.  This is highly relevant in method and conclusion.  It says
that irrational powers tend to erase additive structure.  It does not prevent the output of
an irrational power from being integral on a sparse multiplicative semigroup.

The near-Sidon theorem of Section~\ref{mt:sec-sumproduct} is complementary.  It uses the
finite-rank unit-equation theorem of Evertse--Schlickewei--Schmidt
\cite{EvertseSchlickeweiSchmidt2002} and applies directly to the hypothetical candidate group.
Its conclusion agrees with Harrison's broad picture: additive collisions are scarce, not
abundant.  Therefore additive expansion by itself is evidence neither for nor against the
conjecture.

Agrawal, Bloom, and Petridis prove strong sum--product bounds for sets of integers or rationals
whose elements involve only boundedly many prime divisors~\cite{AgrawalBloomPetridis2026}.
The conditional set $\Cand_T$ has fixed prime support, so it lies in their regime; the exact
finite-rank argument above is stronger for ordinary addition, while their results indicate
which more elaborate expressions, such as $A+AA$, might still carry information.  Wang and
Xiao obtain degeneracy results for parameterized families of unit equations and related
exponential Diophantine problems via integral-point methods~\cite{WangXiao2026}.  Those results
suggest studying families created by the boundary residuals of the third continuation, but no
direct specialization presently supplies the missing contradiction.

\subsection{Shrinking targets, recurrence, and the price of nonmixing}

Recent work makes the contrast between mixing systems and the Sturmian model especially clear.
Levesley, Li, Simmons, and Velani prove quantitative recurrence counts with square-root-scale
errors for expanding piecewise linear maps~\cite{LevesleyLiSimmonsVelani2025}; He, Li, and
Velani survey the distinction between shrinking targets and recurrence and the role of mixing
hypotheses~\cite{HeLiVelani2025}.  Baker and Todd obtain i.i.d.-type pair-correlation laws for
classes of sufficiently mixing interval maps, and also exhibit slowly mixing behavior that
departs from the independent model~\cite{BakerTodd2026}.  None of these theorems transfers to
an irrational rotation with pure point spectrum and the exact coboundary
\eqref{mt:eq-coboundary}.

Hauke proves Kurzweil-type twisted approximation statements along primes and other
multiplicatively structured times for badly approximable rotations, generally for almost every
inhomogeneous parameter~\cite{Hauke2026}.  This is adjacent to the desired blend of a fixed
rotation and arithmetic times, but two mismatches remain: no bounded-partial-quotient theorem
is known for the relevant logarithmic slopes, and the target in the Alaoglu--Erd\H{o}s problem
is coupled to the same orbit rather than supplied by a generic inhomogeneous parameter.

Fang's 2026 work on self-referential leading digits is methodologically still closer.  It treats
a prescribed algebraic exponential sequence as a fixed-parameter shrinking-target problem,
derives exact signed-discrepancy identities, obtains deterministic estimates only below the
critical target scale, and uses continued fractions for certified localization
\cite{Fang2026}.  The central lesson agrees with Sections~\ref{mt:sec-effective} and
\ref{mt:sec-log-haar}: ordinary equidistribution may be perfectly true while remaining too
coarse by an exponential factor.

Two 2026 papers on dyadic approximation in the middle-third Cantor set obtain new zero--one
ranges using averaged Fourier estimates for Cantor measure
\cite{DaiLiWangWu2026,Bandi2026}.  They are notable because the multiplicatively independent
bases $2$ and $3$ appear in a genuinely measure-theoretic interaction.  Their conclusions are
metric statements for polynomially shrinking targets on a fractal measure, however, not exact
integrality at a named exponentially algebraic exponent.  They demonstrate that mixed-base
Fourier information can be extracted when an ambient measure supplies averaging; the present
problem supplies only one rigid orbit.

\subsection{Automaticity, common divisibility, and adjacent rigidity}

Bustos-Gajardo, Fokkink, and Yassawi prove a Gaussian-integer Cobham theorem that removes a
previous Four-Exponentials assumption in their automata setting
\cite{BustosFokkinkYassawi2026}.  This is encouraging as a general pattern: sufficiently rich
combinatorial structure can sometimes bypass a transcendence conjecture.  The additional
recognizability is doing real work, though.  The irrational Sturmian jump word in the
conditional model is nonperiodic and carries a rotation phase rather than two compatible
finite automata, so the result is not a black-box route to the present conjecture.

Nguyen-Dang proves that for multiplicatively independent integers $a,b\ge2$, every integer
linear divisibility sequence dividing both $a^n-1$ and $b^n-1$ is periodic
\cite{NguyenDang2026}.  This creates a concrete possible bridge.  The hypothetical integers
$M=2^\beta$ and $A=3^\beta$ are multiplicatively independent.  Therefore it would suffice to
extract from the boundary commutator or an adelic determinant an unbounded linear divisibility
sequence dividing both $M^n-1$ and $A^n-1$.  No such extraction is presently known, but the
conclusion to aim for is now a theorem rather than a vague gcd heuristic.

\subsection{Formal transcendence theory in Lean}

Karatarakis and Wiedijk have completed a Lean~4 formalization of the
Gelfond--Schneider theorem, including the auxiliary-function and algebraic-integer machinery
needed for Hilbert's seventh problem~\cite{KaratarakisWiedijk2026}.  The inherited corpus
already ports and uses this development at its Gelfond--Schneider boundary.  The new paper
confirms that deep classical transcendence proofs can be brought into the kernel with explicit
constant management.  Baker's theorem, the six exponentials theorem, and the Matrix
Coefficient framework remain substantially larger formalization projects, but they are no
longer separated from Mathlib by a complete absence of transcendence infrastructure.

\subsection{What changed, and what did not}

The recent literature changes the menu of techniques but not the logical status of the
conjecture.  The strongest direct framework remains Matrix Coefficient/Four Exponentials.
Modern o-minimality gives powerful uniform control of additive relations, while finite-rank
unit equations explain why the hypothetical semigroup is nearly Sidon.  Modern shrinking-
target theory gives sharp laws in mixing or metric settings, while the conditional orbit is a
zero-entropy rotation at a much smaller critical scale.  Mixed-base harmonic analysis is
making progress on fractal measures, but no theorem reaches the exact named orbit here.

The search therefore supports a sharper strategic conclusion than ``more equidistribution is
needed.''  The fixed-scale distributions treated here are completely understood and maximally generic.
What is missing is a theorem that simultaneously sees
\begin{equation}
 \text{phase} \quad+\quad \text{exponentially growing denominator scale}
 \quad+\quad \text{fixed prime support}.
 \label{mt:eq-missing-triple}
\end{equation}
None of the located results contains all three ingredients.

\section{Concrete research targets after the measure-theoretic audit}
\label{mt:sec-research-targets}

The negative results above are useful only if they narrow future work.  The following targets
are stated so that success or failure can be checked mathematically; they are not generic calls
for ``stronger estimates.''

\begin{target}[Critical-frequency cancellation on a rank-two shell]
\label{mt:target-critical-fourier}
For an integer $M>1$ that is not a power of two, put $\beta_M=\log_2M$ and
\begin{equation}
 \mathcal C_M(T)=
 \{2^nM^k:n,k\in\Nzero,\ n+k\beta_M\le T\}.
 \label{mt:eq-CMT}
\end{equation}
Find a positive trigonometric minorant or a range of frequencies growing with $T$ for which
\begin{equation}
 \sum_{m\in\mathcal C_M(T)}e^{2\pi ihm^{\thetaq}}
 \label{mt:eq-critical-fourier-sum}
\end{equation}
has enough cancellation to exclude the possibility that every summand equals $1$.  A bound
$o(T^2)$ for one admissible nonzero frequency would already prove
$M^{\thetaq}\notin\N$; a uniform family reaching frequencies comparable to the inverse target
width would connect directly to \eqref{mt:eq-visibility-scale}.
\end{target}

For fixed $h$, ordinary lacunary methods do not automatically apply to a prescribed number
$M^{\thetaq}$, and proving equidistribution of its powers would be much stronger than needed.
The useful formulation is therefore one-sided: manufacture just enough cancellation, perhaps
through averaging over both semigroup coordinates, to rule out the identically integral
phase.  The triangle--Haar theorem shows that averaging only the logarithmic coordinate will
not do this; the phase in \eqref{mt:eq-critical-fourier-sum} is the nonlinear output phase.

\begin{target}[A moving-scale adelic determinant]
\label{mt:target-adelic-determinant}
Use consecutive normalized points
\begin{equation}
 P_k=\left(\frac{W^k}{2^{q^{(2)}_k}},
           \frac{Z^k}{3^{q^{(3)}_k}}\right)
     =(2^{t_k},3^{t_k})
 \label{mt:eq-Pk-moving}
\end{equation}
to construct an integer determinant or auxiliary polynomial whose Archimedean smallness and
$2$- and $3$-adic divisibility are estimated in the \emph{same moving frame}.  The target must
beat the termwise min-plus ceiling proved in the inherited determinant audit, so the required
gain has to come from cancellation, a sparse support theorem, or a boundary identity rather
than from automatic divisibility of individual monomials.
\end{target}

Corollary~\ref{mt:cor-moving-frame} identifies the natural limiting measure against which such
a construction should be normalized.  In particular, replacing the valuations by points at
infinity is fatal, while subtracting their linear drift retains the exact phase as a graph
joining.  A promising determinant should depend on that centered graph coordinate and on the
unit orbit simultaneously.

\begin{target}[Extract a forbidden common divisibility sequence]
\label{mt:target-divisibility-sequence}
Starting from the boundary residuals $U_K,V_K$ and their commutator in the third continuation,
construct a nonperiodic integer linear divisibility sequence $(D_n)$ such that
\begin{equation}
 D_n\mid M^n-1,
 \qquad
 D_n\mid A^n-1
 \qquad(n\ge1).
 \label{mt:eq-common-LDS}
\end{equation}
Nguyen-Dang's theorem~\cite{NguyenDang2026} would then contradict the multiplicative
independence of $M$ and $A$.
\end{target}

This target is deliberately stronger than asking for sporadically large gcds.  The recent gcd
theorem applies to a structured sequence, so the missing step is to turn the boundary
commutator's coherent family of integer identities into recurrence and divisibility.  Resultants
of shifted residuals, primitive contents, and local Newton polygons are natural objects to
compute first.

\begin{target}[Support-sensitive rational-point counting]
\label{mt:target-support-pila}
Prove a rational-point theorem for the compact arc
$\{(2^t,3^t):0\le t<1\}$ that uses not only height but the restricted forms
\begin{equation}
 r=\frac{W^k}{2^q},
 \qquad
 s=\frac{Z^k}{3^{q'}},
 \qquad
 q-q'=k(b-a).
 \label{mt:eq-supported-points}
\end{equation}
A bound $o(K)$ for points with $0\le k<K$ would contradict the conditional orbit.  Ordinary
Pila--Wilkie bounds in terms of total height allow $K\asymp\log H$ points and therefore do not
suffice; the theorem must exploit prime support and the shared Beatty denominator.
\end{target}

Harrison's use of modern $\R_{\exp}$ point counting shows that irrational-power relations can
be controlled uniformly once enough variables create functional transcendence.  Here the
point set has only one real parameter and unusually rigid denominators.  The needed refinement
is arithmetic rather than purely definable.

\begin{target}[A factor-four Matrix Coefficient bridge]
\label{mt:target-factor-four}
Construct, from singularity of the fixed-base logarithm matrix, a nonzero rational auxiliary
polynomial on the Dasgupta--Kakde grid with Bernstein--Kushnirenko degree $<N^2$, or prove a
special fixed-base variant of the Matrix Coefficient Conjecture by another method.  Any
interpolation-only construction has minimum degree $4N^2-1$ and cannot succeed.
\end{target}

The compact-factor theorem suggests a Fourier reformulation of this target.  Fixed characters
average to zero, so the auxiliary support must expand with $N$; but unrestricted expansion
returns to the $4N^2$ interpolation floor.  The desired factor-four gain must therefore arise
from a structured family of growing characters tied to the rank-one logarithm factorization,
not from generic moment vanishing.

\begin{target}[Kernel-checkable critical-scale certificates]
\label{mt:target-certificates}
Convert the MPFR scan through $2^{29}$ into compact rational certificates and formalize the
finite verifier.  In parallel, use continued-fraction block localization, in the spirit of
fixed-parameter certified shrinking-target algorithms~\cite{Fang2026}, to extend the finite
range without linearly scanning every integer.
\end{target}

This target cannot prove an infinite theorem by itself.  It can, however, test proposed
inequalities in the only regime where the target is exact, discover extremal near misses, and
ensure that future formal statements do not hide a false finite lemma.  The distinction
between a larger software scan and a kernel-certified bound should remain explicit.

\section{Formalization blueprint for the new results}
\label{mt:sec-formalization-blueprint}

Most of the fourth continuation separates naturally into small Lean modules.  The following
order minimizes dependence on sophisticated measure theory.

\begin{longtable}{@{}p{0.27\textwidth}p{0.39\textwidth}p{0.27\textwidth}@{}}
\caption{Suggested formalization order for the measure-theoretic continuation.}
\label{mt:tab-formalization}\\
\toprule
Module & Principal statements & Main dependencies or trust boundary \\
\midrule
\endfirsthead
\toprule
Module & Principal statements & Main dependencies or trust boundary \\
\midrule
\endhead
\lean{DifferenceGroup} &
$\mathcal D=\Z$ or $\Z+\Z\beta$; density of the irrational cyclic subgroup modulo one. &
Existing exact monoid theorem; elementary additive groups and irrational rotations. \\
\lean{SturmianProbability} &
Carry identity, two-valued prefix sums, exact variance, covariance formula, coboundary identity. &
Floor arithmetic and interval measure; no external transcendence input. \\
\lean{AdelicDrift} &
Formulas for $q^{(2)}_k,q^{(3)}_k$, valuation escape, common centered defect, and
$q^{(2)}_k-q^{(3)}_k=k(b-a)$. &
Existing primitive normalization and basic $p$-adic valuations. \\
\lean{FiniteProfiniteBeatty} &
For each fixed $d$, uniform distribution of
$(k,\lfloor k\alpha\rfloor,\{k\alpha\})$ modulo $d$. &
A finite-quotient Weyl criterion; avoids constructing $\widehat\Z$ initially. \\
\lean{FixedCongruenceFactor} &
Pushforward to $H_q$ and independence from the phase and jump bit. &
Previous module plus finite-group Haar counting. \\
\lean{TriangleHaar} &
The weighted triangular lattice limit and probabilistic coordinate corollaries. &
Riemann sums, Abel summation, and equidistribution of irrational rotations. \\
\texttt{EffectiveRotation}\newline\texttt{Discrepancy} &
$D_K\ll K^{-1/\mu_M}$ and weighted counting errors. &
An abstract finite irrationality-measure hypothesis; Baker--Matveev remains behind an
interface. \\
\texttt{CompactFactor}\newline\texttt{Saturation} &
Finite-quotient version of Theorem~\ref{mt:thm-compact-saturation}; inverse-limit statement
later. &
Density of locally constant functions on profinite groups. \\
\lean{LogarithmicPowerCurve} &
Dyadic-shell and cumulative logarithmic Haar laws. &
Oscillatory-integral estimates and Fourier approximation on the torus. \\
\texttt{MetricExponent}\newline\texttt{Parameter} &
Measure estimate, Borel--Cantelli corollary, and Hausdorff-dimension upper bound. &
Measure-theoretic limsup machinery and Hausdorff content. \\
\lean{FiniteRankNearSidon} &
Energy and sumset consequences from a finite-rank unit-equation bound. &
Formalize the deduction conditionally first; Evertse--Schlickewei--Schmidt is a major external
number-theoretic input. \\
\bottomrule
\end{longtable}

The first three modules are especially attractive: their proofs are exact algebraic or floor
identities, and they expose several common informal errors about independence.  The
finite-quotient Beatty theorem is a better first target than an abstract Haar theorem on
$\widehat\Z^2$; the inverse limit can be added after the usable congruence statements exist.

The recent Gelfond--Schneider formalization~\cite{KaratarakisWiedijk2026} supplies a model for
handling the deeper classical boundary.  For the present continuation, however, the cleanest
architecture is to parameterize effective statements by a hypothesis of the form
\eqref{mt:eq-finite-mu}.  This keeps every new measure-theoretic theorem kernel-checkable
without first formalizing Matveev.  Later one can instantiate the interface with progressively
stronger transcendence measures.

A second design principle is to formalize the limiting laws first in test-function form and
derive probability-language corollaries afterwards.  Weak convergence, independence, and
marginal distributions then become explicit pushforward theorems rather than informal appeals
to randomness.  This matters here because the same process is uniform in phase, nonmixing in
time, and bounded in its centered partial sums---properties that are easy to conflate in prose.

\section{Final synthesis of the fourth continuation}
\label{mt:sec-final-synthesis}

No proof of the Alaoglu--Erd\H{o}s conjecture was obtained.  The measure-theoretic investigation
nevertheless changes the shape of the remaining problem in several exact ways.

First, a counterexample would not be statistically conspicuous on compact spaces.  Its full
solution lattice has the product limit
\[
 \text{uniform triangle}\times\text{Haar phase},
\]
and every fixed congruence or compact unit factor is independent Haar measure.  Even the full
real power curve is logarithmically Haar on the two-torus.  Weak convergence, fixed Fourier
modes, fixed moduli, and ordinary invariant-measure arguments are therefore exhausted rather
than merely untried.

Second, the arithmetic signal is noncompact and multiscale.  The $2$- and $3$-adic norms escape
at deterministic linear rates, while their centered defects remain exactly equal to the
negative rotation phase.  The relevant target boxes shrink at essentially exponential scale,
and the effective irrationality measure controls only a polynomially growing modulus.  This
is the sharpest measure-theoretic formulation of the gap: the desired theorem must retain
phase while following a moving local scale.

Third, the probability model is rigid.  The denominator jump process is Sturmian, has zero
entropy and pure point spectrum, and its centered increments form a coboundary.  A single hit
creates the entire rank-two semigroup.  Bernoulli, Poisson, central-limit, and second-moment
heuristics do not merely lack proof; they model the wrong dependence structure.

Fourth, additive combinatorics does not produce an automatic contradiction.  Finite-rank unit
equations force the hypothetical candidate set to be asymptotically Sidon for addition while
its product set remains linear in its size.  Recent work on additive relations in irrational
powers points in the same direction.  Any additive attack must derive relations specific to
the simultaneous identities at bases $2$ and $3$, not relations forced by multiplicative rank
alone.

Finally, metric rarity is logically insufficient.  Almost every exponent obeys strong
Borel--Cantelli restrictions, and the exponents with arbitrarily strong near-integrality form a
zero-dimensional exceptional set.  The named number $\thetaq=\log_2 3$ can still lie in that
set.  A proof needs arithmetic exclusion of this point, not a statement about almost every
point.

The revised priority order is therefore:
\begin{enumerate}
\item build a moving-scale adelic or Fourier construction that retains the common phase while
using exponentially growing denominator information;
\item cross the factor-four Matrix Coefficient threshold with a support-preserving auxiliary
object rather than a generic interpolant;
\item compute primitive contents, local Newton polygons, and resultants of the boundary
residuals, with the explicit goal of extracting a forbidden common divisibility sequence;
\item seek support-sensitive rational-point bounds for the compact arc, where height, prime
support, and the shared Beatty denominator are all visible;
\item formalize the exact floor, profinite, and moving-frame theorems before importing deeper
transcendence estimates.
\end{enumerate}

\begin{resultbox}
\textbf{Bottom line of the fourth continuation.}
The conjecture remains open.  New conventional proofs establish a joint triangle--Haar law for
all conditional solutions, effective rotation discrepancy, a profinite Beatty lift, fixed-
modulus and compact-unit independence, non-tight adelic escape with an exact renormalized
joining, rigid Sturmian fluctuation laws, logarithmic Haar measure on the full power curve,
metric exceptional-set bounds, and a finite-rank near-Sidon theorem.  Together they rule out a
large class of fixed-scale measure and probability strategies.  The surviving problem is now
sharply localized: preserve the rotation phase while working at a denominator or Fourier scale
that grows exponentially with height, or obtain the missing support gain in the Matrix
Coefficient program.
\end{resultbox}
...
% --- imported with the fourth continuation ---
...
\newblock Annals of Mathematics (2) \textbf{155}(3) (2002), 807--836.
\newblock DOI: \href{https://doi.org/10.2307/3062133}{10.2307/3062133}.

\bibitem{DasguptaRanksI}
S.~Dasgupta,
\newblock \emph{Ranks of matrices of logarithms of algebraic numbers I: the theorems of Baker and Waldschmidt--Masser},
\newblock Essential Number Theory \textbf{2} (2023), 93--138.
\newblock DOI: \href{https://doi.org/10.2140/ent.2023.2.93}{10.2140/ent.2023.2.93};
\href{https://arxiv.org/abs/2303.02037}{arXiv:2303.02037}.

\bibitem{Harrison2026}
J.~Harrison,
\newblock \emph{Additive relations in irrational powers},
\newblock arXiv:2512.04081v3, July 31, 2026.
\newblock \href{https://arxiv.org/abs/2512.04081}{arXiv:2512.04081}.

\bibitem{AgrawalBloomPetridis2026}
R.~Agrawal, T.~F.~Bloom, and G.~Petridis,
\newblock \emph{More on the sum--product problem for integers with few prime factors},
\newblock arXiv:2512.04931v2, January 6, 2026.
\newblock \href{https://arxiv.org/abs/2512.04931}{arXiv:2512.04931}.

\bibitem{WangXiao2026}
J.~T.-Y.~Wang and Z.~Xiao,
\newblock \emph{Families of unit equations and exponential Diophantine problems via integral points},
\newblock arXiv:2604.26497, April 29, 2026.
\newblock \href{https://arxiv.org/abs/2604.26497}{arXiv:2604.26497}.

\bibitem{LevesleyLiSimmonsVelani2025}
J.~Levesley, B.~Li, D.~Simmons, and S.~Velani,
\newblock \emph{Shrinking targets versus recurrence: the quantitative theory},
\newblock Mathematika \textbf{71}(4) (2025), Article~e70039.
\newblock DOI: \href{https://doi.org/10.1112/mtk.70039}{10.1112/mtk.70039};
\href{https://arxiv.org/abs/2410.22993}{arXiv:2410.22993}.

\bibitem{HeLiVelani2025}
Y.~He, B.~Li, and S.~Velani,
\newblock \emph{Shrinking targets versus recurrence: a brief survey},
\newblock arXiv:2511.02377, November 4, 2025.
\newblock \href{https://arxiv.org/abs/2511.02377}{arXiv:2511.02377}.

\bibitem{BakerTodd2026}
S.~Baker and M.~Todd,
\newblock \emph{Pair correlation statistics for dynamical systems},
\newblock arXiv:2606.17880v2, August 6, 2026.
\newblock \href{https://arxiv.org/abs/2606.17880}{arXiv:2606.17880}.

\bibitem{Hauke2026}
M.~Hauke,
\newblock \emph{The prime times of twisted Diophantine approximation},
\newblock arXiv:2603.25291, March 26, 2026.
\newblock \href{https://arxiv.org/abs/2603.25291}{arXiv:2603.25291}.

\bibitem{Fang2026}
Z.~Fang,
\newblock \emph{Self-referential leading digits of exponential sequences: arithmetic structure and certified search},
\newblock arXiv:2607.23662, July 26, 2026.
\newblock \href{https://arxiv.org/abs/2607.23662}{arXiv:2607.23662}.

\bibitem{DaiLiWangWu2026}
X.-R.~Dai, B.~Li, B.~Wang, and Y.-F.~Wu,
\newblock \emph{Metric results for dyadic approximation on the middle-third Cantor set},
\newblock arXiv:2606.25305, June 24, 2026.
\newblock \href{https://arxiv.org/abs/2606.25305}{arXiv:2606.25305}.

\bibitem{Bandi2026}
P.~Bandi,
\newblock \emph{Averaged Fourier estimates and dyadic approximation on the Cantor set},
\newblock arXiv:2606.27034v2, July 7, 2026.
\newblock \href{https://arxiv.org/abs/2606.27034}{arXiv:2606.27034}.
...
\newblock arXiv:2510.01440v3, August 8, 2026.
\newblock \href{https://arxiv.org/abs/2510.01440}{arXiv:2510.01440}.

\bibitem{NguyenDang2026}
K.-H.~Nguyen-Dang,
\newblock \emph{On the sequence $\gcd(a^n-1,b^n-1)$},
\newblock arXiv:2606.07959, June 6, 2026.
\newblock \href{https://arxiv.org/abs/2606.07959}{arXiv:2606.07959}.
...
\newblock \href{https://arxiv.org/abs/2603.24823}{arXiv:2603.24823}.
