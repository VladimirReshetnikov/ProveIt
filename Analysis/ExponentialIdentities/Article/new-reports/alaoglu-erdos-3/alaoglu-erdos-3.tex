...
\hypersetup{
  ...
  pdfsubject={Machine-checked results, exact conditional structure, determinants,
matrix coefficients, sparse annihilators, and research directions}
}
\urlstyle{same}
...
\newcommand{\statusknown}{\textcolor{classicalgray}{\textbf{[classical/\allowbreak open background]}}}
...
\vspace{0.65cm}
{\Large Machine-Checked Results and Exact Conditional Structure\par}
\vspace{0.10cm}
{\Large Determinants, Matrix Coefficients, Sparse Annihilators,\par}
\vspace{0.10cm}
...
including an independent $O((\log X)^2)$ dyadic bound and an exact diagnosis of their
one-logarithm deficit.  It also gives a sufficient criterion from $\Q$-linear independence of
products $\log p\log q$.

The third continuation incorporates the 2024--2026 matrix-coefficient program and proves a
sharp finite support spectrum.  On an $M\times M$ orbit grid the least annihilating-polynomial
support is exactly $M^2+1$ for irrational $x$; rationality is equivalent to saving even one
monomial on one grid.  Total positivity, adelic criteria, Smith-normal-form targets, and
matching no-go theorems narrow the remaining construction problem, but do not solve it.
\end{minipage}
\vfill
...
Dasgupta--Kakde Matrix Coefficient Conjecture to this orbit and proves the exact fewnomial
support law
\[
  \mu_x(M)=M^2+1\quad(x\notin\Q),
\]
with the rational collision defect
$(M-r)_+(M-s)_+$ for $x=r/s$.  Consequently the conjecture is equivalent to a single finite
one-monomial support saving.  Strict total positivity, adelic vanishing criteria, primitive
Smith-normal-form targets, generic pigeonhole obstructions, and the exact
$o((\log H)^2)$ point-counting frontier then isolate the remaining coefficient-construction
problem.
\end{abstract}
...
A fourth line, developed in Section~\ref{sec:sparse-continuation}, specializes the recent
matrix-coefficient framework and computes the exact sparse-annihilator threshold: on an
$M\times M$ irrational orbit grid it is $M^2+1$, so the missing rationality certificate is
precisely a one-monomial saving.  None of these lines closes the final gap.
...
\item \textbf{New: matrix coefficients and exact sparse support.}  The rank-one logarithm
matrix attached to $(2,A;3,B)$ has a nonzero rational matrix coefficient exactly when
$x\in\Q$.  For every finite orbit grid, the minimum support of a nonzero polynomial
annihilator is one plus the number of distinct orbit parameters.  Hence an $M\times M$ grid
has minimum support $M^2+1$ for irrational $x$, while for $x=r/s\in\Q_{\ge0}$ in lowest terms
the exact value is
\[
  M^2-(M-r)_+(M-s)_++1.
\]
Under $A,B\in\Z$, the conjecture is therefore equivalent to producing, on one finite grid, an
annihilator with at most $M^2$ monomials.  The same continuation proves strict total
positivity, an adelic divisibility criterion, primitive Smith-normal-form reductions, generic
single- and rowwise-modulus no-go theorems, and the exact $o((\log H)^2)$ point-counting
frontier (Section~\ref{sec:sparse-continuation}).  \statusnew{}  The one-monomial construction
itself remains open.  \statusopen
...
\begin{resultbox}
\textbf{Exact sparse-annihilator spectrum (Section~\ref{sec:sparse-continuation}).}
For an $M\times M$ orbit grid generated by $x$, the minimum number of monomials in a nonzero
annihilating polynomial is $M^2+1$ when $x$ is irrational.  If $x=r/s\ge0$ in lowest terms,
the exact collision defect is $(M-r)_+(M-s)_+$.  Thus rationality is equivalent to a finite
one-monomial support saving; with $2^x\in\Z$ it is equivalent to integrality.  The theorem is
exact, but it converts rather than solves the transcendence barrier: the missing task is to
construct the saving from the integer-output hypotheses.  \statusnew{} \statusopen
\end{resultbox}
...
Karatarakis's Mathlib formalization (Mathlib PR \#42911, Apache~2.0); see also the public
Lean~4 account by Karatarakis and Wiedijk \cite{KaratarakisWiedijk2026}.
...
next idea precisely.
\clearpage
% Preserve continuous Arabic pagination across the merged continuation; restarting at 1
% would create duplicate PDF destinations and ambiguous page references.
...
\section{Third continuation: matrix coefficients and sharp sparse annihilators}
\label{sec:sparse-continuation}

\subsection{Scope, outcome, and claim ledger}
\label{sec:sparse-scope}

This continuation was written after a fresh literature search through August 21, 2026, with
particular attention to arXiv, recent journal publications in transcendence theory, effective
point counting in $\R_{\exp}$, $p$-adic logarithmic forms, and automaticity.  Its most direct
new input is the Dasgupta--Kakde matrix coefficient framework
\cite{Dasgupta2023,DasguptaKakde2026}.  That framework does not solve the present problem,
but it changes the right language for the remaining step: instead of asking only for a
nonzero $2\times2$ logarithm determinant, one asks for a rational row vector and a rational
column vector whose matrix coefficient vanishes.  In dimension two this is equivalent to
Four Exponentials, but in the present rank-one family it admits an especially concrete
fewnomial interpretation.

The principal paper-level result of this continuation is an exact support theorem.  For a
finite set of orbit parameters $t=i+jx$, let
\[
  X=\{(2^t,3^t)\}.
\]
Among all nonzero two-variable polynomials vanishing on $X$, the minimum possible number of
monomials is exactly
\[
  1+\#\{t\}.
\]
For the square grid $0\le i,j<M$, this minimum is $M^2+1$ when $x$ is irrational.  If
$x=r/s\in\Q_{\ge0}$ in lowest terms, it is instead
\[
  M^2-(M-r)_+(M-s)_++1,
  \qquad y_+:=\max\{y,0\}.
\]
Thus irrationality is characterized by a sharp absence of even a one-monomial support
saving.  In the original integer-output problem, the conjecture is equivalent to producing,
for one sufficiently large finite orbit grid, a polynomial with at most as many monomials as
grid positions.  This is a sharper specialized target than the factor-four support saving in
the general Dasgupta--Kakde condition $(m')$.

The continuation also proves an exact rectangular collision formula, a full-spark and strict
total-positivity theorem for all monomial evaluation matrices on the orbit, a multiplicity
version excluding confluent shortcuts, an adelic sparse-certificate criterion, a generic
single-modulus no-go theorem, and a logarithmic-square lower bound for rational points that
identifies the exact exponent needed from effective Wilkie-type counting.

\begin{statusbox}{All new theorems in Sections~\ref{sec:sparse-matrix}--\ref{sec:sparse-pointcount}
are conventional paper proofs, labeled \statusnew.  The literature statements are
\statusknown.  No new Lean build was run for this continuation, and no claim is made that the
new support-spectrum, total-positivity, adelic, or point-count statements have been accepted
by the Lean kernel.  Most importantly, this continuation does \emph{not} claim an
unconditional proof of the Alaoglu--Erd\H{o}s conjecture.}
\end{statusbox}

Throughout this continuation we write
\[
  A=2^x,\qquad B=3^x,
\]
when discussing an alleged integer-output instance.  Since $2^x\in\Z_{>0}$, necessarily
$x\ge0$.  A nonintegral instance is irrational, indeed transcendental, by the inherited
Gelfond--Schneider argument.  The irrationality is all that is needed for the sparse-support
results below.

\subsection{Recent literature: what changed, and what did not}
\label{sec:sparse-literature}

The literature audit found one development that is directly structural and several that are
adjacent but do not presently bridge the final gap.

\paragraph{Matrices of logarithms and the matrix coefficient conjecture.}
Dasgupta's 2023 exposition of matrices of logarithms gives a modern account of Baker's
theorem, the Structural Rank Conjecture, and the Waldschmidt--Masser method
\cite{Dasgupta2023}.  Dasgupta and Kakde then introduced the Matrix Coefficient Conjecture:
if a square matrix of logarithms of algebraic numbers is singular, then there are nonzero
rational vectors $w,v$ with $w^{\mathsf T}Mv=0$.  Their paper first appeared as
arXiv:2408.08178 and was published in \emph{Advances in Mathematics} in 2026
\cite{DasguptaKakde2026}.  They prove that Structural Rank implies Matrix Coefficient, show
that the $2\times2$ case is equivalent to Four Exponentials, and develop a support-sensitive
auxiliary-polynomial strategy.  In their notation the unresolved direction is
\[
  (w)\Longrightarrow(m),
\]
and for rank-one $2\times2$ matrices it is enough to establish the weaker fewnomial condition
$(w)\Longrightarrow(m')$.  They explicitly describe this construction problem as still
mysterious.  Sections~\ref{sec:sparse-matrix}--\ref{sec:sparse-dk} specialize that framework
to the Alaoglu--Erd\H{o}s orbit and compute the exact support threshold.

\paragraph{Effective point counting.}
Binyamini, Novikov, and Zak proved an effective form of Wilkie's conjecture for restricted
sub-Pfaffian structures and deduced Wilkie's conjecture for $\R_{\exp}$: the transcendental
part has at most a fixed power of $\log H$ rational points of height at most $H$
\cite{BinyaminiNovikovZak2024}.  This is a major advance, but the available exponent is not a
subquadratic exponent tailored to the fixed curve $v=u^{\log_2 3}$.  The present conjecture
would follow from $o((\log H)^2)$ on that curve, as proved in
Section~\ref{sec:sparse-pointcount}.  Pila's earlier explicit work on a related
exponential-algebraic surface gives an important benchmark, not a direct estimate for this
curve; the 2017 correction leaves all of that paper's theorems valid
\cite{Pila2010Surface,Pila2017Correction}.  Pila's work on Pfaff curves and the general
Pila--Wilkie theorem remain part of the background
\cite{Pila2007Pfaff,PilaWilkie2006}.

\paragraph{Additive relations in irrational powers.}
Harrison's arXiv paper, revised to version~3 on July 31, 2026, proves strong bounds for
nontrivial additive relations among irrational powers of dense subsets of arithmetic
progressions, using functional transcendence and the Binyamini--Novikov--Zak counting theorem
\cite{Harrison2026}.  It also gives a Diophantine approximation criterion, based on linear
forms in logarithms, that rules out certain linear relations among irrational powers of
multiplicatively independent integers.  These results are genuinely close in theme, but the
counterexample orbit here is a sparse rank-two multiplicative semigroup, not a dense subset
of an arithmetic progression, and its image under the power map need not have abnormally
large additive energy.  Section~\ref{sec:sparse-adjacent} records the exact mismatch.

\paragraph{$p$-adic logarithmic forms.}
Chim obtained improved explicit lower bounds for linear forms in two $p$-adic logarithms,
with logarithmic rather than squared-logarithmic dependence on the exponent parameter
\cite{Chim2025}.  This strengthens an important quantitative tool.  It does not, by itself,
turn the archimedean identity
\[
  \log A\,\log3-\log B\,\log2=0
\]
into a singular $p$-adic logarithm matrix.  The missing transfer is discussed in
Section~\ref{sec:sparse-adjacent}.

\paragraph{Automaticity without Four Exponentials in a stronger setting.}
Bustos-Gajardo, Fokkink, and Yassawi prove a Cobham theorem for Gaussian integer numeration
without assuming Four Exponentials, under hypotheses formulated in terms of multiplicatively
independent Gaussian bases and automatic configurations; arXiv version~3 appeared on
August 8, 2026 \cite{BustosFokkinkYassawi2026}.  This is instructive because it shows that
stronger symbolic structure can sometimes bypass the transcendence conjecture.  The
synchronized Sturmian coding inherited earlier in this report is not presently known to be a
finite-kernel automatic configuration in either of two independent numeration systems, so
the new theorem does not directly apply.

A targeted search for the exact integer-output statement, the Four Exponentials special case
it represents, and the matrix-coefficient construction problem found no unconditional
resolution through August 21, 2026.  The correct conclusion is therefore not that recent
work makes the problem routine, but that it identifies a much more precise auxiliary object
to construct.

\subsection{The specialized matrix coefficient conjecture}
\label{sec:sparse-matrix}

The logarithm matrix attached to an integer-output instance is
\[
  L_x=
  \begin{pmatrix}
    \log2 & \log A\\
...
\]
It has real rank one.  The determinant identity alone is the familiar Four Exponentials
barrier.  The matrix coefficient formulation exposes exactly which rational relation would
finish the problem.

\begin{theorem}[Specialized matrix coefficient equivalence]
\label{thm:specialized-mcc}
Let $x\in\R$, $A=2^x$, and $B=3^x$, with $A,B$ algebraic.  The following are equivalent.
\begin{enumerate}
\item $x\in\Q$.
\item There exist nonzero vectors $p,q\in\Q^2$ such that
\[
  p^{\mathsf T}L_xq=0.
\]
\item There exist nonzero vectors $a,b\in\Z^2$ such that
\[
  2^{a_1b_1}A^{a_1b_2}3^{a_2b_1}B^{a_2b_2}=1.
\]
\end{enumerate}
If in addition $A\in\Z$, these conditions are equivalent to $x\in\Z$.
\end{theorem}

\begin{proof}
Write $p=(r,s)^{\mathsf T}$ and $q=(u,v)^{\mathsf T}$.  The rank-one factorization gives
\[
  p^{\mathsf T}L_xq
  =(r\log2+s\log3)(u+vx).
\]
The first factor cannot vanish for nonzero $p\in\Q^2$.  Indeed, after clearing
denominators, a relation $r\log2+s\log3=0$ would give $2^m3^n=1$ with integers $m,n$, and
unique factorization forces $m=n=0$.  Hence a vanishing matrix coefficient forces
$u+vx=0$.  Since $q\ne0$, necessarily $v\ne0$, and $x=-u/v\in\Q$.  Conversely, if
$x=c/d\in\Q$, then $q=(-c,d)^{\mathsf T}$ is a nonzero rational right-kernel vector of
$L_x$, so any nonzero rational $p$ gives a vanishing coefficient.

For integer vectors $a,b$, taking real logarithms of the positive quantity in item~3 gives
\[
  (a_1\log2+a_2\log3)(b_1+b_2x)=0.
\]
The first factor is nonzero when $a\ne0$, so item~3 is again equivalent to a rational
relation $b_1+b_2x=0$.  Clearing denominators shows equivalence with item~2.

Finally, suppose $A=2^x\in\Z$ and $x=c/d$ in lowest terms with $d>0$.  Then
$A^d=2^c$.  Unique factorization gives $A=2^k$ and $kd=c$.  Since $\gcd(c,d)=1$, one has
$d=1$, so $x\in\Z$.
\end{proof}

\begin{corollary}[Alaoglu--Erd\H{o}s as a restricted matrix coefficient conjecture]
\label{cor:ae-restricted-mcc}
The Alaoglu--Erd\H{o}s conjecture is equivalent to the Matrix Coefficient Conjecture restricted
to matrices
\[
  \begin{pmatrix}\log2&\log A\\ \log3&\log B\end{pmatrix},
  \qquad A,B\in\Z_{>0},\qquad
  \det\begin{pmatrix}\log2&\log A\\ \log3&\log B\end{pmatrix}=0.
\]
\end{corollary}

\begin{proof}
The restricted matrix coefficient statement implies $x\in\Q$ by
Theorem~\ref{thm:specialized-mcc}, hence $x\in\Z$.  Conversely, if the Alaoglu--Erd\H{o}s
conjecture holds, then $x=m\in\Z$ and $(-m,1)^{\mathsf T}$ is a rational right-kernel vector.
\end{proof}

The multiplicative pairing in item~3 is precisely the pairing used by Dasgupta and Kakde.
For the two generators
\[
  g_1=(2,3),\qquad g_2=(A,B),
\]
and $a,b\in\Z^2$, put
\[
  \langle a,b\rangle_X
  :=\prod_{i,j=1}^2 x_{ij}^{a_i b_j}
  =2^{a_1b_1}A^{a_1b_2}3^{a_2b_1}B^{a_2b_2}.
\]
Their condition $(o)$ is the existence of nonzero $a,b$ with
$\langle a,b\rangle_X=1$.  In this family, therefore,
\[
  (o)\quad\Longleftrightarrow\quad x\in\Q.
\]
Their condition $(w)$, singularity of the logarithm matrix, holds identically.  The entire
problem is to extract $(o)$ from this rank-one real matrix using the algebraicity and, here,
integrality of the four exponentials.

\subsection{Finite orbit grids and the support minimization problem}
\label{sec:sparse-grid}

The fewnomial formulation is most transparent for an arbitrary finite index set.  Let
$\Omega\subset\Z^2$ be finite and nonempty, and define
\[
  T_x(\Omega):=\{i+jx:(i,j)\in\Omega\}\subset\R,
\]
\[
  X_x(\Omega):=
  \{(2^{i+jx},3^{i+jx}):(i,j)\in\Omega\}\subset\R_{>0}^2,
\]
\[
  D_x(\Omega):=\#T_x(\Omega)=\#X_x(\Omega).
\]
For a nonzero Laurent polynomial
\[
  P(T_1,T_2)=\sum_{(a,b)\in S}c_{a,b}T_1^aT_2^b,
  \qquad c_{a,b}\ne0,
\]
write $\supp(P)=S$ and $|\supp(P)|=|S|$.  Define
\[
  \mu_x(\Omega):=
  \min\bigl\{|\supp(P)|:
     0\ne P\in\C[T_1^{\pm1},T_2^{\pm1}],\ 
     P|_{X_x(\Omega)}=0\bigr\}.
\]
Multiplying a Laurent polynomial by one monomial neither changes its support size nor its
zero set in $(\C^*)^2$.  Consequently the same minimum is obtained if one restricts to
ordinary polynomials $\C[T_1,T_2]$.

For a finite monomial support $S\subset\Z^2$, enumerate the distinct parameters in increasing
order as $t_1<\cdots<t_D$ and write
\[
  \lambda_{a,b}:=a\log2+b\log3.
\]
The monomial evaluation matrix is
\[
  E_{x,\Omega,S}
   =\bigl(\exp(t_i\lambda_{a,b})\bigr)_
      {1\le i\le D,\ (a,b)\in S}.
\]
Distinct monomials have distinct frequencies: if
$\lambda_{a,b}=\lambda_{a',b'}$, then
$2^{a-a'}3^{b-b'}=1$, hence $(a,b)=(a',b')$.

The exact support theorem follows from the elementary zero bound for real exponential
polynomials.  We state the multiplicity form because it also settles the confluent variant.

\begin{lemma}[Extended Chebyshev zero bound]
\label{lem:exp-zero-bound}
Let $\lambda_1<\cdots<\lambda_K$ be real numbers and let
\[
  F(t)=\sum_{j=1}^K c_j\ee^{\lambda_jt},
  \qquad c_j\in\C,
\]
be nonzero.  Then the total multiplicity of the real zeros of $F$ is at most $K-1$.  In
particular, $F$ has at most $K-1$ distinct real zeros.
\end{lemma}

\begin{proof}
For real coefficients, this is proved by induction on $K$.  The case $K=1$ is immediate.
Multiplying by $\ee^{-\lambda_1t}$ does not change the zeros.  If the resulting function has
$R$ real zeros counted with multiplicity, Rolle's theorem in its multiplicity form implies
that its derivative has at least $R-1$ real zeros counted with multiplicity.  That derivative
is a nonzero exponential polynomial with at most $K-1$ distinct frequencies, so induction
gives $R-1\le K-2$.

For complex coefficients, the real and imaginary parts are real exponential polynomials.  At
least one is nonzero, and every zero of $F$ with its multiplicity is a zero of that nonzero
component with at least the same multiplicity.  The real-coefficient bound applies.
\end{proof}

Equivalently, the functions $\ee^{\lambda_1t},\dots,\ee^{\lambda_Kt}$ form an extended
complete Chebyshev system.  Their Wronskian is
\[
  \det\bigl(\lambda_j^{i-1}\ee^{\lambda_jt}\bigr)_{i,j=1}^K
  =\ee^{(\lambda_1+\cdots+\lambda_K)t}
    \prod_{1\le i<j\le K}(\lambda_j-\lambda_i),
\]
which never vanishes.

\begin{theorem}[Exact support spectrum on a finite orbit]
\label{thm:exact-support-general}
For every real $x$ and every finite nonempty $\Omega\subset\Z^2$,
\[
  \boxed{\ \mu_x(\Omega)=D_x(\Omega)+1.\ }
\]
More precisely, for every finite monomial support $S$,
\[
  \rank E_{x,\Omega,S}=\min\{D_x(\Omega),|S|\}.
\]
Hence the vector space of polynomials supported in $S$ and vanishing on
$X_x(\Omega)$ has dimension
\[
  \max\{0,|S|-D_x(\Omega)\}.
\]
\end{theorem}
...
\begin{proof}
Let $D=D_x(\Omega)$ and suppose that a nonzero polynomial $P$ with $K$ monomials vanishes on
the orbit.  Restrict $P$ to the one-parameter curve:
\[
  F_P(t):=P(2^t,3^t)
  =\sum_{(a,b)\in\supp(P)}c_{a,b}
       \ee^{(a\log2+b\log3)t}.
\]
The $K$ frequencies are distinct.  The function $F_P$ vanishes at the $D$ distinct elements
of $T_x(\Omega)$, so Lemma~\ref{lem:exp-zero-bound} gives $D\le K-1$.  Therefore
$K\ge D+1$.

For the reverse inequality, let the distinct parameters be $t_1,\dots,t_D$ and define
\[
  Q(T_1):=\prod_{\nu=1}^D(T_1-2^{t_\nu}).
\]
All roots are positive, so every elementary symmetric coefficient is nonzero.  Thus $Q$ has
exactly $D+1$ monomials and vanishes on every point of $X_x(\Omega)$.  Hence
$\mu_x(\Omega)\le D+1$.

For the rank statement, if $|S|\le D$, a nontrivial linear dependence among the columns of
$E_{x,\Omega,S}$ would give a polynomial with at most $D$ monomials vanishing on all $D$
distinct parameters, contrary to the lower bound just proved.  Thus the columns are
independent.  If $|S|>D$, any $D$ columns are independent, so the rank is $D$.  The kernel
dimension follows from rank-nullity.
\end{proof}

\begin{corollary}[No support gain from multiplicities]
\label{cor:confluent-support}
Let $t_1,\dots,t_h$ be distinct real numbers and let $m_1,\dots,m_h$ be positive integers.
If a nonzero polynomial $P$ has the property that $F_P(t)=P(2^t,3^t)$ vanishes to order at
least $m_i$ at $t_i$, then
\[
  |\supp(P)|\ge 1+\sum_{i=1}^h m_i.
\]
Thus replacing distinct orbit points by confluent derivative conditions cannot reduce the
support cost: every additional unit of prescribed multiplicity consumes at least one
additional monomial.
\end{corollary}

\begin{proof}
Apply Lemma~\ref{lem:exp-zero-bound} to $F_P$.
\end{proof}

The theorem is stronger than the existence of one canonical annihilator.  It says that the
interpolation matroid is uniform: every chosen set of at most $D$ monomials is independent on
the $D$ distinct orbit points, while every chosen set of $D+1$ monomials has a one-dimensional
space of annihilators.

\subsection{Exact collision geometry on rectangles}
\label{sec:sparse-collisions}

For positive integers $M,N$, let
\[
  \Omega_{M,N}:=\{0,\dots,M-1\}\times\{0,\dots,N-1\},
\]
and abbreviate
\[
  D_x(M,N):=D_x(\Omega_{M,N}),\qquad
  \mu_x(M,N):=\mu_x(\Omega_{M,N}).
\]
The support theorem reduces the problem to counting the values of the linear form
$i+jx$ on a lattice rectangle.  That count is exact.

\begin{theorem}[Rectangular collision formula]
\label{thm:rectangular-collision}
Let $M,N\ge1$.
\begin{enumerate}
\item If $x\notin\Q$, then
\[
  ...
  \mu_x(M,N)=MN+1.
\]
\item If $x=r/s\in\Q_{\ge0}$ in lowest terms, with $r\ge0$ and $s\ge1$, then
\[
  D_x(M,N)=MN-(M-r)_+(N-s)_+,
\]
\[
  \boxed{\ 
  \mu_x(M,N)=MN-(M-r)_+(N-s)_++1.
  \ }
\]
\end{enumerate}
\end{theorem}
...
\begin{proof}
If two grid points satisfy
\[
  i+jx=i'+j'x
\]
and $j\ne j'$, then $x=(i'-i)/(j-j')\in\Q$.  Thus for irrational $x$ all $MN$
values are distinct.

Now let $x=r/s$ in lowest terms.  Equality is equivalent to
\[
  s(i-i')+r(j-j')=0.
\]
Because $\gcd(r,s)=1$, two grid points are equivalent exactly when their difference is an
integer multiple of the primitive vector $(r,-s)$.  Form a graph on $\Omega_{M,N}$ by joining
$(i,j)$ to $(i+r,j-s)$ whenever both vertices lie in the rectangle.  Every equivalence class
is one path in this graph: any two equivalent vertices differ by $k(r,-s)$, and all
intermediate vertices remain in the rectangle.  There are no cycles, because along an edge
$i$ increases when $r>0$, while for $r=0$ the coordinate $j$ strictly decreases.

The number of vertices is $MN$.  The number of edges is
\[
  (M-r)_+(N-s)_+,
\]
since the initial vertex must satisfy $0\le i<M-r$ and $s\le j<N$.  A finite forest has
number of connected components equal to vertices minus edges.  The components are the
collision classes, giving the formula for $D_x(M,N)$.  The formula for $\mu_x(M,N)$ follows
from Theorem~\ref{thm:exact-support-general}.
\end{proof}

For squares write $D_x(M):=D_x(M,M)$ and $\mu_x(M):=\mu_x(M,M)$.

\begin{corollary}[Exact square support spectrum]
\label{cor:square-support-spectrum}
For $M\ge1$,
\[
  \mu_x(M)=
  \begin{cases}
    M^2+1,&x\notin\Q,\\[2mm]
    M^2-(M-r)_+(M-s)_++1,
      &x=r/s\in\Q_{\ge0},\ \gcd(r,s)=1.
  \end{cases}
\]
For $M\ge\max\{r,s\}$ this becomes
\[
  \mu_{r/s}(M)=M(r+s)-rs+1.
\]
The exact support saving from the irrational value is
\[
  M^2+1-\mu_{r/s}(M)=(M-r)_+(M-s)_+.
\]
\end{corollary}

The formula contains a useful geometric interpretation.  The defect is not an asymptotic
artifact; it is exactly the number of primitive collision edges in the $M\times M$ grid.  A
rational relation creates one-dimensional fibers, and every edge removed from the free grid
removes exactly one monomial from the minimum annihilator.

For orientation, the first values are as follows.
\[
\begin{array}{c|c|c|c|c}
M&x\notin\Q&x=1&x=2&x=3/2\\ \hline
1&2&2&2&2\\
2&5&4&5&5\\
3&10&6&8&10\\
4&17&8&11&15\\
...
\end{array}
\]
Here each entry is $\mu_x(M)$.  In particular, at the Dasgupta--Kakde scale $M=2N$,
\[
  \mu_x(2N)=
  \begin{cases}
    4N^2+1,&x\notin\Q,\\
  ...
  \end{cases}
\]
for the displayed rational cases once the linear formula applies.

\begin{corollary}[Rationality detected by support growth]
\label{cor:rationality-support}
For $x\in\R_{\ge0}$ the following are equivalent.
\begin{enumerate}
\item $x\in\Q$.
\item There exists $M\ge1$ with $\mu_x(M)\le M^2$.
\item $\mu_x(M)=O_x(M)$ as $M\to\infty$.
\item $\displaystyle\liminf_{M\to\infty}\frac{\mu_x(M)}{M^2}=0$.
\end{enumerate}
If $x\notin\Q$, then $\mu_x(M)=M^2+1$ for every $M$ and the normalized limit is $1$.
\end{corollary}

\begin{proof}
The irrational case is Corollary~\ref{cor:square-support-spectrum}.  If $x=r/s$, then for
$M\ge\max\{r,s\}$,
\[
  \mu_x(M)=M(r+s)-rs+1=O_x(M),
\]
and for $M>\max\{r,s\}$ the defect $(M-r)(M-s)$ is positive, so
$\mu_x(M)\le M^2$.  The implications among the asymptotic conditions are immediate.
\end{proof}

\begin{theorem}[Sharp sparse-annihilator criterion]
\label{thm:sharp-sparse-criterion}
Let $x\in\R_{\ge0}$.  Then $x\in\Q$ if and only if there are an integer $M\ge1$ and a
nonzero polynomial $P\in\C[T_1,T_2]$ such that
\[
  |\supp(P)|\le M^2
\]
and
\[
  P(2^i(2^x)^j,\,3^i(3^x)^j)=0
  \qquad(0\le i,j<M).
\]
If $2^x$ and $3^x$ are integers, the coefficient field $\C$ may equivalently be replaced by
$\Q$ or $\Z$.
\end{theorem}

\begin{proof}
The first assertion is exactly item~2 of Corollary~\ref{cor:rationality-support}.  If the
orbit points are integral and a fixed support admits a nonzero complex coefficient vector,
its integer evaluation matrix has rank smaller than the number of columns over $\C$.  The
same rank deficiency holds over $\Q$, so the kernel contains a nonzero rational vector.
Clearing denominators gives an integer polynomial with no larger support.
\end{proof}

The universal interpolation construction always supplies an annihilator with $M^2+1$
monomials when the grid is collision-free.  Theorem~\ref{thm:sharp-sparse-criterion} says that
rationality is equivalent to saving \emph{one} monomial.  This is the exact threshold; there
is no hidden constant and no asymptotic loss.

\begin{corollary}[A fixed finite certificate for each integer-output instance]
\label{cor:fixed-M-certificate}
Suppose $A=2^x\in\Z_{>0}$ and $B=3^x\in\Z_{>0}$, and put
\[
  M_A:=\lfloor\log_2A\rfloor+2=\lfloor x\rfloor+2.
\]
Then $x\in\Z$ if and only if there is a nonzero $P\in\Z[T_1,T_2]$ with
$|\supp(P)|\le M_A^2$ vanishing at
\[
  (2^iA^j,3^iB^j),\qquad 0\le i,j<M_A.
\]
Consequently the Alaoglu--Erd\H{o}s conjecture is equivalent to the assertion that every
integer-output instance admits this explicitly sized sparse certificate.
\end{corollary}

\begin{proof}
A certificate forces $x\in\Q$ by Theorem~\ref{thm:sharp-sparse-criterion}, and integrality of
$A$ then forces $x\in\Z$.  Conversely, if $x=m\in\Z_{\ge0}$, then $M_A=m+2>m$ and
Corollary~\ref{cor:square-support-spectrum} gives
\[
  \mu_m(M_A)=M_A(m+1)-m+1\le M_A^2.
\]
The integer-coefficient assertion follows from Theorem~\ref{thm:sharp-sparse-criterion}; it
is also explicit by taking the product over the distinct integer first coordinates.
\end{proof}

For $x=m\in\Z_{\ge0}$ and $M=m+2$, the values $i+jm$ fill the entire interval
\[
  0,1,\dots,(m+1)^2.
\]
Thus one explicit certificate is
\[
  \prod_{t=0}^{(m+1)^2}(T_1-2^t),
\]
which has $(m+1)^2+2\le(m+2)^2$ monomials.  The point is not that this polynomial can be
written without knowing $m$; the point is that the original conjecture has now been converted
into the demand for one rigorously quantified support saving on one finite integer matrix.

\subsection{The Dasgupta--Kakde logical square, specialized exactly}
\label{sec:sparse-dk}

For the generator matrix
\[
  X=\begin{pmatrix}2&A\\3&B\end{pmatrix},
\]
Dasgupta and Kakde define
\[
  X(M)=\{g_1^ig_2^j:0\le i,j<M\}
      =\{(2^iA^j,3^iB^j):0\le i,j<M\}.
\]
Their rank condition $(w)$ says that the logarithm matrix is singular.  It holds because
$L_x$ has rank one.  Their condition $(o)$ is the multiplicative orthogonality condition in
Theorem~\ref{thm:specialized-mcc}, hence is equivalent here to $x\in\Q$.

Their fewnomial condition $(m')$ asks that, for all sufficiently large $N$, there exist a
polynomial $P$ with
\[
  |\supp(P)|<N^2
\]
vanishing on $X(2N)$.  Their real exponential-polynomial lemma proves
$(m')\Rightarrow(o)$ for rank-one $2\times2$ matrices.  The exact support spectrum gives both
directions and the precise minimum.

\begin{theorem}[Exact specialization of $(o)$, $(w)$, and $(m')$]
\label{thm:dk-specialization}
For the rank-one family above, the following are equivalent.
\begin{enumerate}
\item Dasgupta--Kakde condition $(o)$ holds.
\item $x\in\Q$.
\item For all sufficiently large $N$, there exists
$0\ne P_N\in\C[T_1,T_2]$ with $|\supp(P_N)|<N^2$ vanishing on $X(2N)$.
\item For some $M$, there exists a nonzero polynomial with at most $M^2$ monomials vanishing
on $X(M)$.
\end{enumerate}
Moreover, condition $(w)$ holds for every $x$.  Under $A,B\in\Z$, the four equivalent
conditions are equivalent to $x\in\Z$.
\end{theorem}

\begin{proof}
The equivalence of items~1 and~2 is Theorem~\ref{thm:specialized-mcc}.  Item~4 is
Theorem~\ref{thm:sharp-sparse-criterion}.  If $x\notin\Q$, then
\[
  \mu_x(2N)=4N^2+1,
\]
so item~3 is impossible.  If $x=r/s\in\Q_{\ge0}$, then for large $N$,
\[
  \mu_x(2N)=2N(r+s)-rs+1<N^2,
\]
so item~3 holds.  The final assertion is rational denominator descent.
\end{proof}

The general Dasgupta--Kakde implication $(w)\Rightarrow(m')$ would imply Four Exponentials.
For this integral rank-one subfamily, Theorem~\ref{thm:dk-specialization} shows something
sharper:
\[
  \boxed{\text{The implication $(w)\Rightarrow(m')$ is exactly the
  Alaoglu--Erd\H{o}s conjecture on this family.}}
\]
There is no residual zero-counting problem.  The zero-count side is exact, and the entire
missing step is the construction of a support-deficient auxiliary polynomial from the
singular logarithm matrix and the arithmetic of its exponentials.

One can also sharpen $(m')$ for this family.  Define $(m^{\sharp})$ to mean that for all
sufficiently large $N$ there is a nonzero polynomial with
\[
  |\supp(P)|\le4N^2
\]
vanishing on $X(2N)$.  Since the collision-free minimum is $4N^2+1$, while the rational
minimum is eventually linear, one has
\[
  (m^{\sharp})\quad\Longleftrightarrow\quad(o).
\]
Thus the specialized target is a one-monomial saving below universal interpolation, not the
factor-four saving required by $(m')$.  The stronger condition $(m')$ remains valuable
because it fits the general Matrix Coefficient strategy; $(m^{\sharp})$ records the exact
threshold available only after exploiting the one-parameter rank-one geometry.

\subsection{Strict total positivity and full-spark integer matrices}
\label{sec:sparse-total-positive}

The zero-count proof has an equivalent determinant form that connects directly to the
interpolation determinants developed earlier in this report.

\begin{theorem}[Strict total positivity of the exponential kernel]
\label{thm:exp-total-positive}
If
\[
  t_1<\cdots<t_K,
  \qquad
...
then
\[
  \det\bigl(\ee^{t_i\lambda_j}\bigr)_{i,j=1}^K>0.
\]
Consequently every square minor of a matrix
$(\ee^{t_i\lambda_j})$ with increasing row parameters and increasing frequencies is
positive.
\end{theorem}

\begin{proof}
The determinant is nonzero by Lemma~\ref{lem:exp-zero-bound}: a linear dependence among its
columns would give a nonzero exponential polynomial with $K$ distinct zeros.  Its sign is
constant on the connected chamber
$t_1<\cdots<t_K$, $\lambda_1<\cdots<\lambda_K$.

To determine the sign, fix increasing real $y_1<\cdots<y_K$ and set
$\lambda_j=\lambda_0+\varepsilon y_j$.  Expanding each exponential in powers of
$\varepsilon$ gives, as $\varepsilon\to0^+$,
\[
\begin{split}
  \det(\ee^{t_i\lambda_j})
  &={\ee^{\lambda_0(t_1+\cdots+t_K)}
       \varepsilon^{K(K-1)/2}\over\prod_{m=0}^{K-1}m!}
    \prod_{1\le i<j\le K}(t_j-t_i)
    \prod_{1\le i<j\le K}(y_j-y_i)\\
  &\qquad+O\bigl(\varepsilon^{K(K-1)/2+1}\bigr).
\end{split}
\]
The leading coefficient is positive, so the determinant is positive throughout the chamber.
\end{proof}

This is the classical strict total positivity of the kernel $\ee^{uv}$; see also
\cite{KarlinStudden}.  The self-contained sign argument is included to make the arithmetic
consequence transparent.

\begin{corollary}[Full-spark integral orbit matrices]
\label{cor:integer-full-spark}
Assume $A=2^x$ and $B=3^x$ are positive integers, and let
$\Omega\subset\Nzero^2$ be finite.  Let $t_1<\cdots<t_D$ be distinct elements of
$\{i+jx:(i,j)\in\Omega\}$, and let $S\subset\Nzero^2$ be a finite monomial support.  Order $S$ so that
$a\log2+b\log3$ increases.  Then every square minor of
\[
  E=\bigl((2^{t_i})^a(3^{t_i})^b\bigr)_{i,(a,b)\in S}
\]
is a positive integer.  In particular every nonzero square minor is at least $1$, and
\[
  \rank_{\Q}E=\min\{D,|S|\}.
\]
\end{corollary}
...
\begin{proof}
For $t_i=i_0+j_0x$ one has
\[
  2^{t_i}=2^{i_0}A^{j_0}\in\Z,
  \qquad
  3^{t_i}=3^{i_0}B^{j_0}\in\Z.
\]
Thus every matrix entry and every minor is an integer.  Positivity follows from
Theorem~\ref{thm:exp-total-positive}.  The rank statement follows by taking a square minor of
maximal possible size.
\end{proof}

If one begins with Laurent monomials, multiplication of each column by a suitable positive
integer clears all negative exponents without changing ranks or signs.  Hence the same
full-spark statement holds after a harmless diagonal scaling.

\begin{corollary}[Alternating signs of every minimal annihilator]
\label{cor:alternating-minimal}
Let $D$ distinct orbit parameters and $D+1$ distinct monomials be ordered by increasing
frequency.  The space of coefficient vectors annihilating all $D$ orbit points is
one-dimensional.  A generator is
\[
  c_j=(-1)^{j-1}\Delta_j,
\]
where $\Delta_j>0$ is the $D\times D$ minor obtained by deleting column $j$.  Hence every
coefficient is nonzero and the signs alternate in frequency order.  In the integer-output
case the primitive generator may be chosen in $\Z^{D+1}$.
\end{corollary}

\begin{proof}
The rank is $D$, so the kernel is one-dimensional.  The displayed cofactor vector lies in the
kernel by Laplace expansion.  Theorem~\ref{thm:exp-total-positive} gives $\Delta_j>0$.
Integer entries give an integer cofactor vector, which may be divided by the gcd of its
coordinates.
\end{proof}

This sign rigidity is a useful warning for auxiliary-polynomial searches.  At minimal
support, cancellation is not optional and its orientation is completely prescribed.  A
candidate coefficient vector with a different sign pattern cannot vanish on all distinct
orbit points.

\begin{corollary}[Modular rank drops and determinantal divisors]
\label{cor:modular-rank-drop}
Let $E\in\Z^{D\times K}$ be such an evaluation matrix with $K\le D$, and let
$\delta_K(E)$ be the gcd of its $K\times K$ minors.  For a prime $p$,
\[
  \rank_{\mathbb F_p}(E)<K
  \quad\Longleftrightarrow\quad
  p\mid\delta_K(E).
\]
In particular, if one selected $K$ rows whose determinant is $\Delta$ and $p\nmid\Delta$,
then a congruence $Ec\equiv0\pmod p$ forces $c\equiv0\pmod p$.
\end{corollary}

\begin{proof}
This is the standard determinantal-divisor criterion, or equivalently the Smith normal form
of $E$.  Full rank over $\Q$ is guaranteed by Corollary~\ref{cor:integer-full-spark}.
\end{proof}

Corollary~\ref{cor:modular-rank-drop} gives a concrete computational target: rather than
searching indiscriminately for modular coefficient vectors, compute the Smith invariants or
determinantal divisors of thin-support evaluation matrices and identify primes at which the
rank genuinely drops.  Such primes are the only places where a modular auxiliary relation can
begin.

\subsection{The canonical annihilator and its exact coefficient cost}
\label{sec:sparse-height}

The exact support theorem separates two issues that are often conflated:
\begin{enumerate}
\item the number of monomials needed for exact vanishing;
\item the arithmetic height of the coefficients of a polynomial that achieves that support.
\end{enumerate}
The first issue is now completely solved.  The second is where product-formula constructions
must pay their archimedean cost.

Assume for this subsection that $x$ is irrational and $A=2^x\in\Z_{>0}$.  The $M^2$ first
coordinates
\[
  z_{i,j}=2^iA^j,
  \qquad 0\le i,j<M,
\]
are distinct positive integers.  The canonical one-variable annihilator is
\[
  Q_M(T):=\prod_{i,j=0}^{M-1}(T-2^iA^j)\in\Z[T].
\]
It is monic, primitive, and has exactly $M^2+1$ nonzero coefficients.

For an integer polynomial $Q=\sum q_kT^k$, write
\[
  H(Q):=\max_k|q_k|,
  \qquad
  \|Q\|_1:=\sum_k|q_k|.
...
\begin{proposition}[Exact cubic coefficient scale]
\label{prop:canonical-height}
...
and
\[
  {C_M\over M^2+1}\le H(Q_M)\le2^{M^2}C_M.
\]
Consequently
\[
  \log H(Q_M)
  ={(1+x)\log2\over2}M^2(M-1)+O(M^2+\log M).
\]
In particular, at the Dasgupta--Kakde scale $M=2N$, the coefficient height is
$\exp(\Theta_x(N^3))$.
\end{proposition}

\begin{proof}
The absolute value of the constant coefficient is
\[
  \prod_{i,j=0}^{M-1}2^iA^j
  =2^{M\sum_{i=0}^{M-1}i}A^{M\sum_{j=0}^{M-1}j}
  =(2A)^{M^2(M-1)/2}=C_M.
\]
Because all roots are positive, the coefficients alternate in sign and their absolute values
are the elementary symmetric functions of the roots.  Hence
\[
  \|Q_M\|_1=|Q_M(-1)|
  =\prod_{i,j=0}^{M-1}(1+2^iA^j).
\]
Every root is at least $1$, so
\[
  2^iA^j\le1+2^iA^j\le2\cdot2^iA^j.
\]
Multiplying gives the claimed bounds for $\|Q_M\|_1$.  Since $Q_M$ has $M^2+1$
coefficients,
\[
  {\|Q_M\|_1\over M^2+1}\le H(Q_M)\le\|Q_M\|_1.
\]
Finally, $\log A=x\log2$.
\end{proof}

This calculation reproduces, in the present two-dimensional orbit, the cubic coefficient
scale highlighted by Dasgupta and Kakde in their discussion of auxiliary polynomials.  A
naive $p$-adic gain obtained from only $O(N^2)$ roots competes against an archimedean
coefficient cost of order $\exp(cN^3)$.  The support theorem adds a second obstruction: if one
tries to increase the $p$-adic gain by imposing high multiplicity at the orbit points, the
confluent bound in Corollary~\ref{cor:confluent-support} forces the support to increase by the
same total multiplicity.  A successful construction must therefore exploit arithmetic
cancellation, not merely append roots or derivatives.

There is also a useful distinction between the canonical support and the sharp target.  On
$X(2N)$, the canonical annihilator has $4N^2+1$ monomials.  The specialized condition
$(m^{\sharp})$ asks for at most $4N^2$: a saving of exactly one monomial.  Dasgupta--Kakde
condition $(m')$ asks for fewer than $N^2$: a saving of more than three quarters of the
canonical support.  The former is logically exact for this family; the latter is more robust
but arithmetically much more demanding.

\subsection{Adelic sparse certificates}
\label{sec:sparse-adelic}

A product-formula argument can force exact vanishing without first constructing an exact
kernel.  The support theorem shows precisely how strong such a certificate must be.
Normalize $|p|_p=p^{-1}$.

\begin{theorem}[Adelic vanishing criterion]
\label{thm:adelic-vanishing}
Let $Z\subset\Z^2$ be finite, let $P\in\Z[T_1,T_2]$, and let $S$ be a finite set of primes.
Suppose that for every $z\in Z$,
\[
  |P(z)|_\infty\prod_{p\in S}|P(z)|_p<1.
\]
Then $P$ vanishes identically on $Z$.
\end{theorem}

\begin{proof}
If $P(z)=n\ne0$, then the product formula for the nonzero integer $n$ gives
\[
  |n|_\infty\prod_p|n|_p=1.
\]
For primes outside $S$ one has $|n|_p\le1$, so
\[
  1\le |n|_\infty\prod_{p\in S}|n|_p,
\]
contrary to the hypothesis.  Hence $P(z)=0$ for every $z\in Z$.
\end{proof}

A divisibility formulation is often easier to engineer.

\begin{corollary}[Divisibility outruns size]
\label{cor:divisibility-outruns-size}
In the setting of Theorem~\ref{thm:adelic-vanishing}, suppose that for each $z\in Z$ there is
a positive integer $q_z$, all of whose prime factors lie in $S$, such that
\[
  q_z\mid P(z),
  \qquad
  |P(z)|<q_z.
\]
Then $P(z)=0$ for all $z\in Z$.
\end{corollary}

\begin{proof}
The divisibility gives
\[
  \prod_{p\in S}|P(z)|_p\le q_z^{-1},
\]
so the product in Theorem~\ref{thm:adelic-vanishing} is smaller than $1$.
\end{proof}

\begin{corollary}[Adelic sparse certificate for rationality]
\label{cor:adelic-sparse-rationality}
Let
\[
  Z_M=\{(2^iA^j,3^iB^j):0\le i,j<M\},
\]
where $A=2^x$ and $B=3^x$ are integers.  Suppose there is a nonzero
$P\in\Z[T_1,T_2]$ with
\[
  |\supp(P)|\le M^2
\]
and a finite set of primes $S$ such that
\[
  |P(z)|_\infty\prod_{p\in S}|P(z)|_p<1
  \qquad(z\in Z_M).
\]
...
\begin{proof}
Theorem~\ref{thm:adelic-vanishing} makes $P$ an exact sparse annihilator.  Then
Theorem~\ref{thm:sharp-sparse-criterion} gives $x\in\Q$, and denominator descent gives
$x\in\Z$.
\end{proof}

This is the two-dimensional integer-orbit analogue of the ``analytic vanishing'' question
posed by Dasgupta and Kakde.  It cleanly separates the task into three quantitative pieces:
\begin{enumerate}
\item choose a support with at most $M^2$ monomials;
\item force large divisibility of every evaluation $P(z)$ at one or more finite places;
\item control the archimedean values strongly enough that the divisibility exceeds the size.
\end{enumerate}
The support theorem guarantees that success is impossible for an irrational $x$, so any
successful construction proves the conjecture rather than merely producing another partial
bound.

\subsection{Why generic one-modulus pigeonhole cannot close the sparse regime}
\label{sec:sparse-pigeonhole}

The first temptation is to choose bounded coefficients, reduce all evaluations modulo one
large integer, and apply the pigeonhole principle.  The following elementary lemma shows why
that strategy, combined only with the triangle inequality, is dimensionally incapable of
reaching exact vanishing at support size at most the number of constraints.

\begin{lemma}[Generic modular kernel]
\label{lem:generic-modular-kernel}
Let $E\in\Z^{R\times K}$, let $Q\ge2$, and let $H\ge0$ be an integer.  If
\[
  (H+1)^K>Q^R,
\]
then there exists a nonzero $d\in\Z^K$ such that
\[
  |d_j|\le H,
  \qquad
  Ed\equiv0\pmod Q.
...
\begin{proof}
Map the $(H+1)^K$ vectors in $\{0,\dots,H\}^K$ to the $Q^R$ residue classes of their images
under $E$.  Two distinct vectors have the same image; their difference is the required $d$.
\end{proof}

\begin{proposition}[Single-modulus triangle-inequality no-go]
\label{prop:single-modulus-no-go}
Assume $K\le R$.  Any choice of $H,Q$ satisfying the pigeonhole inequality in
Lemma~\ref{lem:generic-modular-kernel} necessarily has $H\ge Q$.  If the entries in row $r$
of $E$ have absolute value at most $M_r$, with $M_r\ge1$, then the generic estimate
\[
  |(Ed)_r|\le KHM_r
\]
can never certify $|(Ed)_r|<Q$.
\end{proposition}

\begin{proof}
The pigeonhole inequality gives
\[
  H+1>Q^{R/K}\ge Q,
\]
so $H\ge Q$.  Since $K\ge1$ and $M_r\ge1$, the right side of the triangle estimate is at
least $Q$, not strictly smaller than it.
\end{proof}

The proposition does \emph{not} say that the actual values $Ed$ cannot be small or zero.
They may be, through highly structured cancellation.  It says that a black-box count of
coefficient vectors followed by the crude triangle inequality cannot prove that smallness.
At least one additional ingredient is necessary:
\begin{itemize}
\item a support-dependent cancellation theorem;
\item a rank drop modulo carefully selected primes, rather than a generic modulus;
\item several places with a weighted lattice argument;
\item common factors in the orbit values that make the rowwise divisibility much larger than
the modulus used to choose coefficients;
\item or an archimedean basis in which the relevant linear combinations are intrinsically
small.
\end{itemize}

This no-go theorem explains why the Smith-normal-form direction in
Corollary~\ref{cor:modular-rank-drop} is more promising than uniform modular pigeonhole.  A
prime dividing a large determinantal divisor represents genuine arithmetic degeneracy; an
arbitrary prime does not.

\subsection{A lattice formulation of the remaining auxiliary-polynomial problem}
\label{sec:sparse-lattice}

Fix a support $S=\{m_1,\dots,m_K\}$ and distinct grid points
$z_1,\dots,z_R$.  Let $E\in\Z^{R\times K}$ be the evaluation matrix.  For a coefficient
vector $c\in\Z^K$, the vector $Ec$ lists the polynomial values.  The sharp support regime is
$K\le R$; over $\Q$, Corollary~\ref{cor:integer-full-spark} says that $E$ has trivial kernel.
The goal is therefore not to find a rational kernel directly, but to find an integer vector
$c$ such that $Ec$ is simultaneously
\begin{enumerate}
\item very divisible at selected finite places, and
\item very small in the archimedean norm.
\end{enumerate}
The product formula would then force $Ec=0$, contradicting full spark unless the orbit has a
collision.

One convenient abstract target is the following.

\begin{target}[Weighted adelic lattice target]
\label{target:weighted-adelic}
For some $M$, choose a monomial support $S$ with $|S|\le M^2$, an integer coefficient vector
$c\ne0$, and positive integers $q_z$ indexed by the grid points, such that
\[
  q_z\mid (Ec)_z,
  \qquad
  |(Ec)_z|<q_z
...
By Corollary~\ref{cor:divisibility-outruns-size}, Target~\ref{target:weighted-adelic} proves
$x\in\Z$.  The quantities $q_z$ need not be equal, and their prime supports need not be the
same.  This row-dependent freedom is important: the orbit has strongly nonuniform
valuations, and forcing a common modulus discards most of that information.

The earlier determinant sections of this report computed several tropical and transport
ceilings for product divisibility.  The support-spectrum theorem adds a complementary exact
constraint: any method that reaches the weighted target with $K\le M^2$ has crossed the full
conjectural boundary.  There is no intermediate irrational example on which such a
certificate could survive.

\subsection{The rational-point exponent frontier}
\label{sec:sparse-pointcount}

Let
\[
  \thetaq=\log_2 3,
  \qquad
  C_{\thetaq}:=\{(u,u^{\thetaq}):u>0\}.
\]
The curve is fixed; it does not depend on a hypothetical counterexample.  Its integer points
are precisely the candidate pairs $(m,m^{\thetaq})$ with $m\in\Cand$.

\begin{lemma}[The curve has no algebraic arc]
\label{lem:no-algebraic-arc}
No nonempty open arc of $C_{\thetaq}$ is contained in an algebraic curve.  Consequently the
one-dimensional semialgebraic part of $C_{\thetaq}$ is empty.
\end{lemma}

\begin{proof}
Suppose a nonzero polynomial $P(U,V)=\sum c_{a,b}U^aV^b$ vanished on an open arc.  With
$u=\ee^t$, the exponential polynomial
\[
  P(\ee^t,\ee^{\thetaq t})
  =\sum c_{a,b}\ee^{(a+b\thetaq)t}
\]
would vanish on an interval and hence identically.  The frequencies $a+b\thetaq$ are distinct
because $\thetaq$ is irrational, so Lemma~\ref{lem:exp-zero-bound} forces every coefficient
to vanish, a contradiction.  A one-dimensional semialgebraic arc has a one-dimensional
algebraic Zariski closure, so none can occur.
\end{proof}

Assume now that a least nonintegral solution $\beta$ exists, as in the inherited conditional
classification.  Put
\[
  A=2^\beta\in\Z,
  \qquad
  B=3^\beta\in\Z.
\]
For every $n,k\ge0$ the point
\[
  (2^nA^k,3^nB^k)
  =(2^{n+k\beta},3^{n+k\beta})
\]
is an integral point of $C_{\thetaq}$, and the points are distinct because $\beta$ is
irrational.

Let $N_{\thetaq}(H)$ denote the number of rational points on $C_{\thetaq}$ of naive height at
most $H$.  Integral points with both coordinates at most $H$ are included in this count.

\begin{theorem}[A counterexample forces logarithmic-square point growth]
\label{thm:log-square-points}
Under a counterexample with least generator $\beta$,
\[
  N_{\thetaq}(H)
  \ge
  { (\log H)^2\over 2\beta(\log3)^2}
  +O_\beta(\log H)
\]
as $H\to\infty$.
\end{theorem}
...
\begin{proof}
Since $\thetaq>1$ and $u\ge1$, the larger coordinate on the curve is
$u^{\thetaq}$.  The orbit point indexed by $(n,k)$ has height at most $H$ whenever
\[
  3^{n+k\beta}\le H,
\]
or equivalently
\[
  n+k\beta\le T:=\log_3H.
\]
The number of such lattice points is
\[
  \sum_{k=0}^{\lfloor T/\beta\rfloor}
       \bigl(\lfloor T-k\beta\rfloor+1\bigr).
\]
Replacing the inner floor by its argument incurs an error
$O(T/\beta+1)=O_\beta(T)$.  If $K=\lfloor T/\beta\rfloor$, the main sum is
\[
  (K+1)(T+1)-{\beta K(K+1)\over2}
  ={T^2\over2\beta}+O_\beta(T).
\]
Substitute $T=(\log H)/(\log3)$.
\end{proof}

\begin{corollary}[Point-counting criterion]
\label{cor:point-count-criterion}
Any upper bound
\[
  N_{\thetaq}(H)=o((\log H)^2)
\]
proves the Alaoglu--Erd\H{o}s conjecture.  More quantitatively, a bound
\[
  N_{\thetaq}(H)=O((\log H)^{2-\delta})
\]
for any $\delta>0$ is sufficient.
\end{corollary}

The exponent $2$ is the exact frontier produced by a rank-two conditional semigroup.  The
unconditional integral points $(2^n,3^n)$ already contribute $\asymp\log H$, so an exponent
below $1$ is impossible and unnecessary.  The problem lies between the known linear family
and the hypothetical quadratic-logarithmic family.

The Binyamini--Novikov--Zak theorem supplies a bound by some power of $\log H$ for definable
sets in $\R_{\exp}$ \cite{BinyaminiNovikovZak2024}.  Lemma~\ref{lem:no-algebraic-arc} removes
the algebraic-part issue for this curve.  What is not supplied is a complexity estimate with
power strictly smaller than $2$ for this particular unbounded graph.  Pila's explicit
$(\log H)^{44+\varepsilon}$-type bound on a related exponential-algebraic surface
\cite{Pila2010Surface,Pila2017Correction} illustrates both the strength of polylogarithmic
counting and the size of the exponent gap.  It should be treated as a benchmark, not as a
bound that can simply be restricted to $C_{\thetaq}$.

The compact normalized orbit from earlier sections asks for a denominator-sensitive count on
a fixed compact arc.  There a counterexample produces $\asymp\log H$ points with controlled
$S$-integral denominators, so an $o(\log H)$ estimate respecting those denominators would also
finish the problem.  The unbounded formulation in Corollary~\ref{cor:point-count-criterion}
is denominator-free but asks for a subquadratic logarithmic exponent.

\subsection{Two exact refinements of the auxiliary-polynomial target}
\label{sec:sparse-refinements}

The preceding arguments leave two possible sources of false optimism.  First, one might hope
that a specially shaped Newton polygon, rather than support cardinality, can evade the zero
bound.  Second, one might hope that allowing a different modulus at every row rescues the
pigeonhole construction.  Neither hope survives exact analysis.

\begin{corollary}[Newton geometry cannot change the exact threshold]
\label{cor:newton-geometry-no-shortcut}
Fix $D$ distinct points on the curve $t\mapsto(2^t,3^t)$.  For every monomial support
$S\subset\Z^2$ with $|S|\le D$, the restriction map
\[
  \Span_{\C}\{T_1^aT_2^b:(a,b)\in S\}
  \longrightarrow \C^D
\]
is injective.  This conclusion is independent of the degree, diameter, convex hull, lattice
width, or other geometry of $S$.
\end{corollary}

\begin{proof}
It is the rank statement in Theorem~\ref{thm:exact-support-general}.  All such geometric data
can affect coefficient height and local divisibility, but exact linear independence depends
only on the number of distinct frequencies.
\end{proof}

Thus thin rectangles, simplices, random supports, and supports concentrated on a curve all
face the same exact support wall.  Geometry still matters---often decisively---for the
\emph{quantitative} problem of constructing small coefficients, but it cannot by itself save
the one monomial required by Theorem~\ref{thm:sharp-sparse-criterion}.

The one-modulus no-go theorem also has a row-dependent form.

\begin{proposition}[Rowwise-modulus box-pigeonhole no-go]
\label{prop:rowwise-modulus-no-go}
Let $E\in\Z^{R\times K}$ with $1\le K\le R$.  For row $r$, suppose
$|E_{rj}|\le M_r$ for all $j$, where $M_r\ge1$.  Let $H\ge1$ and let
$q_1,\dots,q_R\ge2$ be integers.

The standard cardinality-only pigeonhole guarantee using the vectors in
$\{0,\dots,H\}^K$ is
\[
  (H+1)^K>\prod_{r=1}^R q_r;
\]
under this inequality it produces a nonzero $d$ with
\[
  |d_j|\le H,
  \qquad q_r\mid(Ed)_r\quad(1\le r\le R).
\]
On the other hand, a uniform use of the rowwise triangle bounds to certify
$|(Ed)_r|<q_r$ for every such difference vector requires
\[
  q_r>KHM_r\qquad(1\le r\le R).
\]
These two sufficient requirements are incompatible.
\end{proposition}

\begin{proof}
The first inequality is the ordinary cardinality-only pigeonhole condition for the map
\[
  \{0,\dots,H\}^K\longrightarrow
  \prod_{r=1}^R\Z/q_r\Z,
  \qquad c\longmapsto Ec.
\]
The triangle inequality gives
\[
  |(Ed)_r|\le H\sum_{j=1}^K|E_{rj}|\le KHM_r.
\]
If the strict upper bound is to imply $|(Ed)_r|<q_r$, then, because $q_r$ is an integer,
\[
  q_r\ge KHM_r+1\ge H+1.
\]
Therefore
\[
  \prod_{r=1}^Rq_r\ge(H+1)^R\ge(H+1)^K,
\]
contradicting the strict pigeonhole inequality.
\end{proof}

The proposition leaves open lattice reduction in a non-box body, determinant-sensitive
selection of a short vector, and cancellation estimates for the \emph{actual} vector $Ed$.
It rules out only the generic architecture ``count coefficient vectors, obtain congruences,
then use the triangle inequality.''  Crucially, row-dependent moduli do not repair that
architecture.

A second normalization is needed before Smith invariants are interpreted arithmetically.
Let $E\in\Z^{R\times K}$ be an evaluation matrix, and let
\[
  g_r:=\gcd(E_{r1},\dots,E_{rK})>0.
\]
Write
\[
  E=G E^{\mathrm{prim}},
  \qquad
  G=\operatorname{diag}(g_1,\dots,g_R),
\]
so every row of $E^{\mathrm{prim}}$ is primitive.

\begin{proposition}[Row-content invariance]
\label{prop:row-content-invariance}
For every $c\in\Z^K$ and every row $r$,
\[
  (Ec)_r=g_r(E^{\mathrm{prim}}c)_r.
\]
If a finite set of primes $S$ contains every prime divisor of $g_r$, then
\[
 |(Ec)_r|_\infty\prod_{p\in S}|(Ec)_r|_p
 =|(E^{\mathrm{prim}}c)_r|_\infty
   \prod_{p\in S}|(E^{\mathrm{prim}}c)_r|_p.
\]
Hence common row divisibility contributes no net product-formula gain.  In particular,
translating a monomial support by $(u,v)\in\Nzero^2$, which multiplies row $z$ by
$z_1^uz_2^v$, changes only such tautological row content.
\end{proposition}

\begin{proof}
The first identity is the definition of $E^{\mathrm{prim}}$.  For the second, the product
formula for the positive integer $g_r$ gives
\[
  |g_r|_\infty\prod_{p\in S}|g_r|_p=1
\]
when $S$ contains its full prime support.  Multiplicativity of all absolute values proves the
claim.  A support translation factors the same monomial value from every entry in a fixed
row.
\end{proof}

This identifies the meaningful modular object: not the raw evaluation matrix, whose rows may
be divisible for trivial monomial reasons, but its row-primitive normalization.  A useful
computational invariant is therefore the determinantal divisor
\[
  \delta_K(E^{\mathrm{prim}}),
\]
or, more generally, the complete Smith profile of $E^{\mathrm{prim}}$.  Only primes surviving
this normalization encode cancellation among distinct monomials.  Primes appearing solely in
$G$ are exactly canceled by their archimedean contribution.

The same support theorem is not peculiar to the bases $2$ and $3$.

\begin{proposition}[Base-independent support spectrum]
\label{prop:base-independent-support}
Let $a_1,\dots,a_d>0$ be multiplicatively independent, and let
$t_1,\dots,t_D$ be distinct real numbers.  On the curve
\[
  t\longmapsto(a_1^t,\dots,a_d^t),
\]
the minimum support of a nonzero Laurent polynomial vanishing at all $D$ displayed points is
$D+1$.  Every monomial-evaluation matrix has rank equal to the smaller of its number of rows
and columns.
\end{proposition}

\begin{proof}
A Laurent monomial with exponent vector $\nu\in\Z^d$ restricts to
\[
  \exp\!\left(t\sum_{\ell=1}^d\nu_\ell\log a_\ell\right).
\]
Multiplicative independence makes the frequencies distinct.  Lemma~\ref{lem:exp-zero-bound}
and the same interpolation construction as in Theorem~\ref{thm:exact-support-general} apply
verbatim.
\end{proof}

This generalization clarifies what is special about Alaoglu--Erd\H{o}s.  The support spectrum
is a one-parameter Chebyshev phenomenon and is completely understood.  The exceptional
arithmetic content is the demand that the rank-one curve pass through a rank-two semigroup of
\emph{integer} points.

\subsection{Adjacent approaches: exact points of contact and exact mismatches}
\label{sec:sparse-adjacent}

The literature developments in Section~\ref{sec:sparse-literature} suggest several routes.
For each route, it is important to record not merely that it ``does not apply,'' but the
specific statement that would make it apply.

\subsubsection{$p$-adic logarithms}

Chim's bounds concern a nonzero $p$-adic form built from two algebraic logarithms
\cite{Chim2025}.  The integer-output hypothesis instead gives the archimedean identity
\[
  \log A\,\log3-\log B\,\log2=0.
\]
There is no functorial operation sending this equality to
\[
  \log_p A\,\log_p3-\log_p B\,\log_p2=0;
\]
indeed, choices of $p$-adic logarithm, Teichm\"uller factors, and domains of convergence must
be addressed before the expression is even normalized.  More fundamentally, equality of two
ratios in one completion does not imply equality in another completion.

The usable $p$-adic target is therefore not a direct transplantation of the logarithm
identity.  It is to construct a row-primitive polynomial evaluation vector
$E^{\mathrm{prim}}c$ that is $p$-adically small at selected places and archimedeanly small at
the same time.  Chim's theorem could then control how much $p$-adic smallness is possible
unless a multiplicative relation occurs.  The missing input is the construction of that
specific $p$-adic form; the lower-bound theorem comes only afterward.

\subsubsection{Additive relations in irrational powers}

Harrison proves near-maximal sumset growth for $A^{[c]}$ when $c$ is irrational and $A$ is a
sufficiently large subset of an arithmetic progression, and bounds nontrivial additive
relations by functional transcendence and effective point counting \cite{Harrison2026}.  A
counterexample here supplies, up to height $X$, the set
\[
  \mathcal A_X=\{2^nA^k\le X:n,k\ge0\},
  \qquad
  |\mathcal A_X|\asymp_x(\log X)^2,
\]
and
\[
  \mathcal A_X^{[\thetaq]}
   =\{3^nB^k:2^nA^k\le X\}.
\]
This is a sparse multiplicative semigroup contained in the ambient progression
$\{1,\dots,\lfloor X\rfloor\}$.  Its size is only logarithmic-square, and the theorem's
required power of $\log X$ depends on the number of summands.  Even if the cardinality
threshold were met in a particular case, maximal additive expansion of both
$\mathcal A_X$ and its power image would be compatible with the counterexample.  No large
family of nontrivial additive relations is forced by multiplicative closure alone.

A direct application would require an additional lemma of one of the following forms:
\begin{enumerate}
\item the rank-two semigroup $\mathcal A_X$ has additive energy exceeding Harrison's
irrational-power bound; or
\item the synchronized map $2^nA^k\mapsto3^nB^k$ preserves enough additive configurations to
create forbidden relations.
\end{enumerate}
Neither statement is currently known, and simple numerical models of generic rank-two
multiplicative semigroups suggest that the first should not be expected without further
arithmetic structure.

\subsubsection{Automaticity and Cobham-type rigidity}

The Gaussian Cobham theorem of Bustos-Gajardo, Fokkink, and Yassawi is especially instructive:
it bypasses Four Exponentials because automatic configurations provide a strong finite-kernel
hypothesis \cite{BustosFokkinkYassawi2026}.  The present orbit does carry a synchronized
Sturmian coding, developed earlier in this report, but Sturmian complexity is not the same as
recognizability by finite automata in two multiplicatively independent integer numeration
systems.

A viable automatic route must first produce a finite-state object.  One exact target is:
prove that the indicator of the candidate set, or a finite alphabet decoration of the
conditional orbit, has finite kernels in two independent bases.  Cobham--Semenov rigidity
would then force eventual periodicity, which conflicts with the irrational slope.  At
present, the known coding has linear factor complexity but an infinite kernel; that is the
precise missing upgrade.

\subsubsection{Effective Pfaffian and o-minimal counting}

The 2024 Binyamini--Novikov--Zak theorem proves a polylogarithmic rational-point bound in
$\R_{\exp}$ \cite{BinyaminiNovikovZak2024}.  A complementary effective Pila--Wilkie theorem
for Pfaffian-definable sets by Binyamini, Jones, Schmidt, and Thomas was published online in
2026 \cite{BinyaminiJonesSchmidtThomas2026}.  The latter gives effective control of constants
and complexity in the $H^\varepsilon$ regime; the former gives the qualitatively stronger
$\operatorname{poly}(\log H)$ growth needed to approach the present frontier.

Theorem~\ref{thm:log-square-points} identifies the exact required exponent for the unbounded
curve: any $o((\log H)^2)$ estimate suffices.  Neither general theorem currently supplies a
subquadratic logarithmic exponent for this particular graph.  The homogeneity
\[
  (u,u^{\thetaq})\longmapsto(2u,3u^{\thetaq})
\]
suggests that a specialized argument should track the dyadic pieces with uniform complexity,
but ordinary height changes anisotropically under this map.  This is why denominator-sensitive
counting on the compact normalized arc remains a separate and potentially sharper target.

\subsubsection{Structural rank and matrix coefficients}

The Dasgupta--Kakde framework is the closest route because it starts from exactly the singular
logarithm matrix present here \cite{DasguptaKakde2026}.  The exact support theorem removes the
zero-estimate side of their strategy in this family.  It also shows that neither higher
multiplicity nor a more ingenious Newton polygon can substitute for the missing construction.
The remaining implication can now be written with no asymptotic slack:
\[
  \det L_x=0\text{ and }A,B\in\Z
  \quad\Longrightarrow\quad
  \exists M,\ \exists P\ne0,
  \quad |\supp P|\le M^2,
  \quad P|_{X(M)}=0.
\]
This implication is equivalent to the original conjecture.  It is therefore the natural
central target against which all proposed auxiliary-polynomial methods should be measured.

\subsection{Exact computational reconnaissance}
\label{sec:sparse-computation}

The new formulas are proved symbolically, so computation is not part of their logical
foundation.  A small exact check is nevertheless useful for detecting indexing errors in the
rectangular defect formula.  The following program enumerates every collision class directly
using the integer representative $si+rj$ for $x=r/s$.

\begin{lstlisting}[style=pythonstyle,caption={Exact check of the rectangular collision formula.},label={lst:collision-check}]
...
The output certifies $36{,}800$ finite instances with exact integer arithmetic.  It includes
the degenerate case $r=0$, highly rectangular boxes, boxes smaller than one or both primitive
steps, and the eventual linear regime.  The check is a regression test for the formula, not a
replacement for Theorem~\ref{thm:rectangular-collision}.

For the next computational stage, raw modular ranks should not be tabulated.  The correct
pipeline is:
\begin{enumerate}
\item normalize the support up to monomial translation;
\item build the exact integer evaluation matrix;
\item divide every row by its content;
\item compute the Smith normal form or at least the determinantal divisors of the primitive
matrix;
\item identify primes producing a rank drop after this normalization;
\item use those primes in a weighted lattice whose archimedean norm is computed in the same
basis.
\end{enumerate}
This pipeline filters out automatic powers of primes dividing $2AB$ and records only genuine
inter-monomial cancellation.  It is also compatible with exact certificate replay: every
claimed Smith invariant, divisibility, and final inequality can be checked over the integers.

\subsection{Formalization blueprint after the 2026 Gelfond--Schneider development}
\label{sec:sparse-formalization}

Karatarakis and Wiedijk's 2026 Lean~4 formalization of Gelfond--Schneider provides an external,
publicly documented mechanization of the principal transcendence input already ported into the
corpus \cite{KaratarakisWiedijk2026}.  This changes the formalization landscape in a useful
way: the algebraic-irrational case is no longer a greenfield transcendence project.  The
remaining new results in this continuation are mostly elementary real analysis, finite
combinatorics, and integer linear algebra, and are therefore realistic near-term Lean targets.

A modular formalization plan is as follows.

\begin{description}[leftmargin=4.2cm,style=nextline,itemsep=0.7em]
\item[\lean{OrbitFrequencies.lean}]
Prove $\Q$-linear independence of $\log2$ and $\log3$ from unique factorization, and injectivity
of $(a,b)\mapsto a\log2+b\log3$ on $\Z^2$.

\item[\lean{ExponentialChebyshev.lean}]
Formalize Lemma~\ref{lem:exp-zero-bound}, including multiplicities.  The existing exponential-
polynomial zero-count material in the corpus should be reused rather than duplicated.

\item[\lean{OrbitSupportSpectrum.lean}]
Define finite supports and restriction matrices; prove
$\rank E=\min(D,K)$ and $\mu=D+1$ by the zero bound and rank--nullity.

\item[\lean{RationalGridCollisions.lean}]
Formalize equivalence classes under the primitive step $(r,-s)$, the forest count, and the
formula $MN-(M-r)_+(N-s)_+$.

\item[\lean{ExponentialTotalPositivity.lean}]
First formalize nonsingularity from the zero bound.  The positivity sign can then be proved by
the confluent limit used in Theorem~\ref{thm:exp-total-positive}, or imported from a general
Chebyshev-system lemma if one becomes available.

\item[\lean{AdelicSparseCertificate.lean}]
Formalize the integer product formula in the elementary divisibility form of
Corollary~\ref{cor:divisibility-outruns-size}; then derive the sparse rationality and
integer-exponent criteria.

\item[\lean{PrimitiveEvaluationMatrix.lean}]
Implement row-content normalization, determinantal divisors, and the box-pigeonhole no-go
Propositions~\ref{prop:rowwise-modulus-no-go} and~\ref{prop:row-content-invariance}.

\item[\lean{OrbitPointCountingLower.lean}]
Count lattice points below $n+k\beta\le T$ and formalize the explicit quadratic main term with
a linear error bound.
\end{description}

The recommended dependency order is the order displayed.  The support spectrum and collision
formula can be accepted before total positivity; positivity is needed only for signs of
minors, not for the exact rank theorem.  No new Lean build was run in preparing this
continuation, so this subsection is a blueprint, not a claim of kernel acceptance.

\subsection{Synthesis: the narrowed research frontier}
\label{sec:sparse-synthesis}

The different approaches can now be compared on a common quantitative scale.

\begin{center}
\small
\begin{tabularx}{0.98\textwidth}{@{}p{0.18\textwidth}X X@{}}
\toprule
\textbf{Route} & \textbf{Exact result now available} & \textbf{Missing statement that would finish} \\
\midrule
Matrix coefficient
& $L_x$ is rank one; a rational matrix coefficient exists exactly when $x\in\Q$.
& Derive one rational matrix coefficient from singularity plus integrality of the four exponentials. \\
\addlinespace
Sparse annihilator
& The minimum support on an $M\times M$ irrational grid is exactly $M^2+1$.
& Construct any annihilator with at most $M^2$ monomials for one explicitly bounded $M$. \\
\addlinespace
Adelic lattice
& Divisibility exceeding archimedean size forces exact vanishing; generic box pigeonhole fails even with rowwise moduli.
& Produce cancellation in a primitive evaluation lattice strong enough to meet the rowwise strict inequalities. \\
\addlinespace
Smith normal form
& Only primes dividing primitive determinantal divisors can create genuine modular rank drops.
& Prove that sufficiently strong primitive rank drops occur and combine them without losing archimedean control. \\
\addlinespace
Point counting
& A counterexample forces at least $c_x(\log H)^2+O_x(\log H)$ points on the fixed curve.
& Prove $o((\log H)^2)$ for that curve, or $o(\log H)$ in the compact denominator-sensitive formulation. \\
\addlinespace
Automaticity
& Strong automatic configurations can bypass Four Exponentials in Gaussian numeration.
& Upgrade the synchronized Sturmian orbit coding to a finite-kernel configuration in two independent bases. \\
\addlinespace
Additive relations
& Irrational powers of dense progression subsets have near-minimal additive energy.
& Force excess additive energy, or preserved additive configurations, from the rank-two multiplicative orbit. \\
\addlinespace
$p$-adic logarithms
& Strong explicit two-logarithm lower bounds are available.
& Manufacture the relevant nonzero $p$-adic form from the archimedean rank-one identity. \\
\bottomrule
\end{tabularx}
\end{center}

The following priority order is supported by both the exact results and the recent literature.

\begin{enumerate}
\item \textbf{Primitive Smith profiles for thin supports.}  Search supports with
$K=M^2$ or slightly smaller, remove row content, and compute exact determinantal divisors.
The purpose is not blind experimentation but discovery of a repeatable source of genuine
local rank loss.

\item \textbf{Weighted adelic lattice reduction.}  Incorporate those primitive local losses
as row-dependent congruences, and use an archimedean basis adapted to the total-positive
kernel.  The alternating cofactor sign pattern should be built into the basis rather than
rediscovered numerically.

\item \textbf{A specialized Dasgupta--Kakde construction.}  Exploit the one-parameter orbit
and the exact one-monomial threshold.  General constructions aim for fewer than $N^2$
monomials on $X(2N)$; here it is enough to remove one monomial from the universal
$4N^2+1$ support.

\item \textbf{Subquadratic logarithmic point counting.}  Use the homogeneity of the fixed
power curve and the 2024--2026 effective Pfaffian machinery to seek an exponent below two,
with explicit attention to the change of height between dyadic pieces.

\item \textbf{Finite-kernel and additive-configuration tests.}  Attempt to prove, rather than
assume, the additional structure needed by Cobham or Harrison.  Negative results here would
also be valuable because they would retire two broad classes of indirect attacks.

\item \textbf{Formalize the exact paper-level layer.}  The support spectrum, collision count,
rowwise no-go, and adelic criterion are sufficiently elementary that formalization should
precede large computational searches; this prevents a subtle rank or indexing mistake from
propagating into expensive experiments.
\end{enumerate}

Several approaches should be deprioritized unless they acquire a new ingredient.  Appending
more zeros or derivatives cannot improve support.  Translating supports to manufacture
$p$-divisibility changes only row content.  Generic coefficient boxes cannot cross the
constraint/support boundary.  General polylogarithmic point counting without an exponent
below two does not contradict the conditional orbit.  And a $p$-adic logarithm estimate is
not actionable until the archimedean identity has been converted into a specific $p$-adic
form.

\begin{resultbox}
\textbf{Strongest exact conclusion of this continuation.}
For the integer orbit generated by a putative instance $(2,A;3,B)$, rationality of the
exponent is equivalent to a single finite event: on some $M\times M$ grid, the universal
interpolation support $M^2+1$ can be reduced to $M^2$.  Under $A,B\in\Z$, that event is
equivalent to $x\in\Z$.  Every irrational exponent has the exact, rigid value
$\mu_x(M)=M^2+1$ for every $M$.
\end{resultbox}

\begin{statusbox}{\statusnew{} for the exact support, collision, total-positivity,
rowwise-pigeonhole, row-content, adelic, and point-growth results; \statusknown{} for the
cited literature; \statusopen{} for every construction target.  The conjecture remains
unproved.}
\end{statusbox}

\subsection{Conclusion of the third continuation}
\label{sec:sparse-conclusion}

The recent Matrix Coefficient program does not remove the Four Exponentials barrier, but it
allows the barrier to be resolved into a finite and exact combinatorial statement for this
problem.  The zero-estimate side is complete: the orbit evaluation matroid is uniform, the
minimum support is known for every finite index set, and rational collisions are counted by a
closed formula.  The coefficient side remains open.

The new results sharply constrain what a proof can look like.  It cannot arise from a support
shape with too few frequencies, from repeated zeros, from unnormalized modular divisibility,
or from generic pigeonhole plus triangle bounds.  It may arise from a structured primitive
Smith degeneration, an adelic short vector with genuine cancellation, a specialized
matrix-coefficient auxiliary polynomial, or a rational-point theorem crossing the exact
logarithmic-square frontier.  These are materially narrower targets than the undifferentiated
instruction to ``prove Four Exponentials in this case.''

No unconditional proof of the Alaoglu--Erd\H{o}s conjecture is obtained here.  What is
obtained is an exact one-monomial certificate equivalent to the conjecture, a proof that the
standard generic constructions cannot produce that certificate, and a research program tied
to the strongest relevant developments available through August 21, 2026.
...
formalized Siegel lemma for algebraic integers and the auxiliary-function machinery
\cite{KaratarakisWiedijk2026}; that development is the basis of the port used by this corpus
and cited in Appendix~\ref{app:classical}.  Mathlib's Lindemann--Weierstrass development supplies analytic
...
% --- added with the third continuation (literature through August 21, 2026) ---
...
\newblock Annals of Mathematics \textbf{199}(2) (2024), 795--821.
\newblock DOI: \href{https://doi.org/10.4007/annals.2024.199.2.5}{10.4007/annals.2024.199.2.5}.

\bibitem{BinyaminiJonesSchmidtThomas2026}
G.~Binyamini, G.~O.~Jones, H.~Schmidt, and M.~E.~M.~Thomas,
\newblock \emph{An effective Pila--Wilkie theorem for sets definable using Pfaffian
functions, with some diophantine applications},
\newblock Journal of the European Mathematical Society, online first, April 15, 2026.
\newblock DOI: \href{https://doi.org/10.4171/JEMS/1761}{10.4171/JEMS/1761}.

\bibitem{Pila2007Pfaff}
J.~Pila,
\newblock \emph{The density of rational points on a Pfaff curve},
\newblock Annales de la Facult\'e des Sciences de Toulouse \textbf{16}(3) (2007), 635--645.
\newblock DOI: \href{https://doi.org/10.5802/afst.1162}{10.5802/afst.1162}.

\bibitem{Pila2010Surface}
J.~Pila,
\newblock \emph{Counting rational points on a certain exponential-algebraic surface},
\newblock Annales de l'Institut Fourier \textbf{60}(2) (2010), 489--514.
\newblock DOI: \href{https://doi.org/10.5802/aif.2530}{10.5802/aif.2530}.

\bibitem{Pila2017Correction}
J.~Pila,
\newblock \emph{A correction to ``Counting rational points on a certain
exponential-algebraic surface''},
\newblock Annales de l'Institut Fourier \textbf{67}(3) (2017), 1277--1278.
\newblock DOI: \href{https://doi.org/10.5802/aif.3109}{10.5802/aif.3109}.
...
\bibitem{Chim2025}
K.~Chim,
\newblock \emph{Lower bounds for linear forms in two $p$-adic logarithms},
\newblock Journal of Number Theory \textbf{266} (2025), 295--349.
\newblock DOI: \href{https://doi.org/10.1016/j.jnt.2024.07.012}{10.1016/j.jnt.2024.07.012}.
