...
\begin{equation}\label{eq:MA-beta}
  M=2^x,\qquad A=3^x,
  \qquad \vartheta=\frac{\log 3}{\log 2},
  \qquad \varrho=\frac{\log 2}{\log 3}.
\end{equation}
Then
...
Let \(s_k=\lceil\log_2E_k\rceil\).  If the borrow block in
...
For every \(q,p\), the cleared norm of \(\eta^q-3^p\) is the nonzero integer
...
\begin{equation}\label{eq:Qn-cancel}
 Q_n=\frac{3^{D_n\epsilon_n}-1}
           {D_n(2^{\epsilon_n}-1)}
 \longrightarrow\frac{\log3}{\log2}=\vartheta.
\end{equation}
...
enquote{in the ratio sense} along this subsequence.
\end{equation}
More precisely, each quotient of the left side by the right side tends to
\(1\).
\end{theorem}
