...
\AEname{} conjecture asserts that $x$ is an integer.  This report does not claim
...
For a putative \AEname{} counterexample, \eqref{eq:dd-power-scaled} specializes
...
\begin{corollary}[A concrete sufficient condition for \AEname]
\label{cor:compat-AE}
Let $x>0$, and let $M=2^x$ and $N=3^x$ be integers.  If
...
$m\in\N$.  In the first case $x=\log_2 M=m$; in the second,
$x=\log_3 N=m$.
\end{proof}
...
\begin{corollary}
\Cref{conj:MP-bridge} implies the \AEname{} conjecture.
\end{corollary}
...
For the \AEname{} data, one may use the same exponent pairs $(a_j,b_j)$ in the
two semigroups
\[
...
\begin{theorem}[Conditional \AEname{} theorem]\label{thm:conditional-QPLI}
...
$k\in\N_0$.  Therefore $x=\log_2 M=k$.
\end{proof}
...
\begin{enumerate}
 ...
 \item The common slope
       $\log_2 M=\log_3 N$ cancels the leading archimedean growth
...
\[
 \begin{gathered}
 \text{finite products}\longrightarrow q\text{-binomial divisibility}
 \longrightarrow p\text{-ordering}\longrightarrow\text{local valuations},\\
 \text{local valuations}\longrightarrow\text{mass laws}
...
\begin{enumerate}
 ...
 \item A positive combined compatibility mass for $(2,M)$ and $(3,N)$ is an
       unconditional sufficient condition for the \AEname{} conclusion.
...
\begin{enumerate}
 \item For candidate integer pairs $(M,N)$ with
       $\log_2 M\approx\log_3 N$, compute the finite compatibility
       profiles
       \[
       r\longmapsto
