...
\hypersetup{
  ...
  pdfsubject={Machine-checked structure, determinant and probability methods, sparse
certificates, and research directions}
}
\urlstyle{same}
...
\vspace{0.65cm}
{\LARGE Machine-Checked Structure, Determinants, and Probability,\par}
\vspace{0.12cm}
{\LARGE Sparse Certificates and the Remaining Barrier\par}
\vspace{1.0cm}
...
\small
\textbf{Bottom line.}
The conjecture --- that $2^x,3^x\in\Z$ forces $x\in\Z$ --- remains open, and its exact
reduction to the $2\times2$ Four Exponentials problem explains the depth of the last step.
The kernel-verified corpus nevertheless gives a nearly complete conditional anatomy: a
zero-free initial range, transcendence of every hypothetical counterexample, a canonical
primitive odd core, and the exact free-monoid classification
\[
  \Sol=\{n+k\beta:n,k\in\Nzero\},
  \qquad
  \Cand=\{2^nM^k:n,k\in\Nzero\},
\]
with unique coordinates, exact block counts, and quantitative equidistribution.

The report develops arbitrary-node and mixed-semigroup determinant methods, all-prime
valuation transport, and several exact equivalent criteria.  This continuation adds two new
interfaces.  Probability theory gives a uniform Borel--Cantelli theorem and a rigorous
random-phase model whose expected synchronized near-hit count is $O((2/3)^N)$, while a
counterexample would force the distinguished phase zero to contain $\sim N/\beta$ exact hits;
a Fej\'er identity states the missing derandomization as one explicit exponential-sum target.
The 2024 Matrix Coefficient Conjecture of Dasgupta--Kakde yields a complementary sparse
certificate program: on the conditional $4N^2$-point integer grid, every nonzero vanishing
Laurent polynomial needs at least $4N^2+1$ monomials, sharply.  Constructing any subcritical
polynomial whose values fall below the integer threshold would prove the conjecture.
\end{minipage}
\vfill
...
prioritized research program and a formalization blueprint.  Fifth, the third continuation
adds a probability-theoretic control experiment and a sparse-polynomial reformulation.  It
proves a uniform Borel--Cantelli theorem for near-integral powers, shows that almost every
random phase has no late synchronized dyadic--triadic near hits while a counterexample would
force phase zero to carry a linear population, and converts the needed derandomization into a
Fej\'er-weighted exponential-sum criterion.  It also specializes the 2024 Matrix Coefficient
Conjecture to the logarithm matrix of a putative counterexample and proves the sharp support
threshold $4N^2+1$ for polynomials vanishing on its $4N^2$-point integer grid.
\end{abstract}
...
\item \textbf{New: probability and sparse certificates.}  For almost every exponent,
near-integral powers at any summable sequence of radii occur only finitely often.  More
pointedly, after a uniform phase shift the expected number of synchronized near hits in the
$N$-th dyadic block is less than $2(2/3)^N$, so almost every phase has no late hits; a
counterexample would instead force phase zero to contain $\lfloor(N+1)/\beta\rfloor$ exact
hits.  A Fej\'er-kernel identity turns the missing derandomization into a finite weighted
average of exponential sums up to frequency $3^N$.  On the algebraic side, the
Dasgupta--Kakde Matrix Coefficient Conjecture specializes directly to this problem, and any
nonzero Laurent polynomial vanishing on the associated $4N^2$-point integer grid needs at
least $4N^2+1$ monomials, sharply.  Thus any subcritical analytic fewnomial certificate would
prove the conjecture (Sections~\ref{sec:metric-probability}--\ref{sec:sparse-small-value-target}).
\statusnew{} \statusknown
...
\begin{resultbox}
\textbf{Probability and sparse certificates
(Sections~\ref{sec:metric-probability}--\ref{sec:sparse-small-value-target}).}
A random phase has expected synchronized near-hit count $<2(2/3)^N$ and almost surely no late
hits, whereas a counterexample forces phase zero to have $\sim N/\beta$ exact hits.  The
phase-zero anomaly is exactly a positive Fej\'er average of the sums
$\sum_{2^N<a<2^{N+1}}e(h a^{\thetaq})$ for $h<3^N$.  Independently, the conditional integer
grid has a sharp fewnomial threshold: $4N^2$ grid points cannot be annihilated by a nonzero
Laurent polynomial with at most $4N^2$ monomials.  A subcritical polynomial made uniformly
smaller than its integrality denominator would therefore prove the conjecture.  \statusnew
\end{resultbox}
...
\clearpage
\section{Third continuation: probability, sparse certificates, and the matrix-coefficient viewpoint}
\label{sec:continuation3}

This continuation takes two routes that are complementary to the determinant audits above.
The first asks what probability theory says about the apparently random event
\(\|m^{\thetaq}\|_{\Z}\ll1\), and then isolates the exact derandomization statement that would
be needed at the fixed exponent \(\thetaq=\log_2 3\).  The second imports the 2024
matrix-coefficient formulation of Dasgupta and Kakde into the present integral problem and
turns it into a sharp sparse-polynomial certificate problem on the conditional integer grid.
The two routes meet at the same point: a generic or randomly shifted configuration is easy,
but the arithmetic phase and the arithmetic coefficients may be exceptionally coherent.

No proof of Conjecture~\ref{conj:AE} is claimed.  The new unconditional statements are:
\begin{enumerate}
\item a uniform metric estimate and a Borel--Cantelli theorem for near-integral powers;
\item an exact random-phase model in which the expected number of synchronized dyadic--triadic
      near hits in the \(N\)-th block is less than \(2(2/3)^N\), hence almost every phase has
      no sufficiently late hits;
\item a Fej\'er-kernel identity that converts the required derandomization at phase zero into
      an explicit average of exponential sums with frequencies up to \(3^N\);
\item a direct specialization of the Matrix Coefficient Conjecture to the logarithm matrix of
      a putative counterexample;
\item a sharp fewnomial theorem: on the \(4N^2\)-point conditional integer grid, a nonzero
      Laurent polynomial that vanishes everywhere needs at least \(4N^2+1\) monomials, and
      this threshold is attained by rational-coefficient interpolation;
\item an integrality amplifier and a random-sign second-moment audit that state exactly what
      a probabilistic auxiliary-polynomial construction would have to improve.
\end{enumerate}
...
\midrule
Metric near-integrality Borel--Cantelli theorem & proved & change of variables; first Borel--Cantelli lemma \\
Random-phase synchronized-grid extinction & proved & first moment; first Borel--Cantelli lemma \\
Phase-zero Fej\'er criterion & proved & positivity and Fourier expansion of the Fej\'er kernel \\
Matrix-coefficient specialization & proved conditionally & Matrix Coefficient Conjecture \\
Sharp \(4N^2+1\) sparse-vanishing threshold & proved & Chebyshev zero bound for exponential sums \\
Construction of a subcritical analytic certificate & open & the missing \((w)\Rightarrow(m')\) mechanism \\
Transfer from almost every phase to phase zero & open & arithmetic anti-concentration at exponential resolution \\
\bottomrule
\end{tabular}
\end{center}

\section{A metric probability theorem for near-integral powers}
\label{sec:metric-probability}

The exact event \(m^\alpha\in\Z\), with both \(m\) and the integer value allowed to vary, is
supported on a countable set of exponents.  That observation is true but too brittle to be
useful.  A more informative question thickens the integer lattice and asks whether
\(m^\alpha\) can enter a prescribed sequence of shrinking neighborhoods infinitely often.
The following estimate is elementary, uniform in the base, and already captures the first
Borel--Cantelli scale.

\begin{theorem}[Uniform metric bound for one power]
\label{thm:metric-one-power}
Fix \(0<a<b<\infty\).  For every integer \(m\ge2\) and every
\(0<\varepsilon\le1/4\), put
\[
 E_m(\varepsilon)
...
Then
\[
 \operatorname{Leb}\bigl(E_m(\varepsilon)\bigr)
 \le C_b\varepsilon,
 \qquad
 C_b:=\frac{8}{3}\left(b+\frac{1+\log3}{\log2}\right).
\]
In particular the constant is independent of \(m\).  \statusnew
\end{theorem}

\begin{proof}
Make the monotone change of variables \(y=m^\alpha\).  Since
\(d\alpha=dy/(y\log m)\),
\[
 \operatorname{Leb}\bigl(E_m(\varepsilon)\bigr)
 =\frac1{\log m}\int_{m^a}^{m^b}
       \mathbf 1_{\{\distZ y<\varepsilon\}}\,\frac{dy}{y}.
\]
Only neighborhoods of positive integers occur.  Enlarging the range of summation and then
using \(k-\varepsilon\ge3k/4\) for \(k\ge1\) gives
\begin{align*}
 \operatorname{Leb}\bigl(E_m(\varepsilon)\bigr)
 &\le \frac1{\log m}
   \sum_{1\le k\le\lceil m^b\rceil+1}
   \int_{k-\varepsilon}^{k+\varepsilon}\frac{dy}{y} \\
 &\le \frac{8\varepsilon}{3\log m}
   \sum_{1\le k\le\lceil m^b\rceil+1}\frac1k .
\end{align*}
The harmonic sum is at most
\(1+\log(\lceil m^b\rceil+1)\le1+b\log m+\log3\).  Finally
\(\log m\ge\log2\), which yields the displayed constant.
\end{proof}

\begin{corollary}[Borel--Cantelli for powers]
\label{cor:metric-bc}
Let \((\varepsilon_m)_{m\ge2}\) be nonnegative and summable.  Then for Lebesgue-almost every
\(\alpha\in[a,b]\),
\[
  \distZ{m^\alpha}<\varepsilon_m
\]
holds for only finitely many integers \(m\).  In particular, for every \(\delta>0\), almost
every \(\alpha>0\) satisfies
\[
  \distZ{m^\alpha}<m^{-1-\delta}
\]
for only finitely many \(m\).  \statusnew
\end{corollary}

\begin{proof}
Theorem~\ref{thm:metric-one-power} makes
\(\sum_m\operatorname{Leb}(E_m(\varepsilon_m))\) finite.  The first Borel--Cantelli lemma
requires no independence and gives the assertion on each compact interval \([a,b]\).  A
countable union of such intervals covers \(\R_{>0}\).  Finitely many terms for which the
chosen radius exceeds \(1/4\) are harmless.
\end{proof}

The theorem also has the finite expectation form
\[
 \E_{\alpha\sim\mathrm{Unif}[a,b]}
 \#\{2\le m\le M:\distZ{m^\alpha}<\varepsilon_m\}
 \le \frac{C_b}{b-a}\sum_{m=2}^{M}\varepsilon_m,
\]
so Markov's inequality supplies quantitative tail bounds without any independence assumption.
For \(k=1\), this is the elementary core of a broader 2025 theorem of Iyer on sums of
\(\theta\)-powers: under the corresponding summability hypothesis, the relevant
near-integrality inequalities have only finitely many solutions for almost every exponent
\cite{Iyer2025}.

\subsection{Why the metric theorem does not specialize}

The exponent in the conjecture is not random.  It is the single transcendental number
\(\thetaq=\log_2 3\), and a null exceptional set can contain highly structured numbers.
This is not a merely philosophical warning.  Iyer constructs a transcendental number \(\tau\)
for which, for every real \(v\), the inequality
\[
  \distZ{n^\tau}\le n^v
\]
has infinitely many solutions \cite{Iyer2025}.  Taking \(v\) arbitrarily negative shows that
transcendence alone does not prevent superpolynomially accurate near-integrality.  Thus the
kernel-verified transcendence of \(\thetaq\) and of a hypothetical \(\beta\) cannot be combined
with a generic Borel--Cantelli statement by any formal specialization argument.

The useful output of probability theory is instead diagnostic.  It predicts the correct
scale, identifies the exceptional event, and tells us exactly which deterministic
anti-concentration estimate would settle the problem.  The next section makes that statement
literal.

\section{The synchronized grid with a random phase}
\label{sec:random-phase}

For \(N\ge1\), let
\[
  D_N:=\{a\in\Z:2^N<a<2^{N+1}\},
  \qquad |D_N|=2^N-1.
\]
If \(x\in(N,N+1)\) and \(a=2^x\in\Z\), then
\[
  3^x=(2^x)^{\thetaq}=a^{\thetaq}.
\]
Equivalently, writing \(a=2^N u\) with \(1<u<2\),
\[
  3^N u^{\thetaq}=a^{\thetaq},
\]
because \(2^{N\thetaq}=3^N\).  Thus one unit exponent block is exactly one dyadic input block
sampled against the integer lattice by the fixed phase zero.

For \(u\in\R/\Z\) and \(0<\eta\le1/2\), define the shifted near-hit count
\[
  Z_N(u;\eta)
  :=\#\{a\in D_N:\distZ{a^{\thetaq}-u}<\eta\}.
\]
The natural synchronized resolution is \(\eta_N=3^{-N}\), one unit in the denominator scale of
the second coordinate.

\begin{theorem}[Random-phase extinction]
\label{thm:random-phase-extinction}
Let \(U\) be uniform on \(\R/\Z\).  Then
\[
  \E Z_N(U;3^{-N})
  =2(2^N-1)3^{-N}
  <2\left(\frac23\right)^N.
\]
Consequently, for almost every single phase \(U\),
\[
  Z_N(U;3^{-N})=0
\]
for all sufficiently large \(N\).  No independence between blocks is used.  \statusnew
\end{theorem}

\begin{proof}
For each fixed \(a\), the set of phases \(u\) with
\(\distZ{a^{\thetaq}-u}<3^{-N}\) is an arc of Haar measure \(2\cdot3^{-N}\).  Summing the
indicators gives the expectation.  Since
\(\sum_N\E Z_N(U;3^{-N})<\infty\), Markov's inequality gives
\[
 \sum_N\Pr\bigl(Z_N(U;3^{-N})\ge1\bigr)<\infty.
\]
The first Borel--Cantelli lemma finishes the proof.
\end{proof}

This is the clean probabilistic version of the naive ``\((2/3)^N\) expected intersections''
heuristic.  It is rigorous because the random object is the phase, not the values
\(a^{\thetaq}\), and it needs no fiction of independence among those values.

\subsection{Phase zero is exactly the conjectural exceptional phase}

Let
\[
 H_N:=\#\{a\in D_N:a^{\thetaq}\in\Z\}.
\]
The map \(a\mapsto\log_2 a\) is a bijection from the set counted by \(H_N\) to the
nonintegral solutions in \([N,N+1)\).  Hence the exact conditional classification of
Theorem~\ref{cd:block-count} yields the following dichotomy.

\begin{proposition}[Exact phase-zero dichotomy]
\label{prop:phase-zero-dichotomy}
If Conjecture~\ref{conj:AE} holds, then \(H_N=0\) for every \(N\).  If it fails and \(\beta\)
is the primitive least nonintegral solution, then
\[
  H_N=\left\lfloor\frac{N+1}{\beta}\right\rfloor,
  \qquad
  \frac{H_N}{N}\longrightarrow\frac1\beta.
\]
Moreover
\[
  Z_N(0;3^{-N})\ge H_N.
\]
Thus a counterexample would make the phase zero count grow linearly even though its random
phase expectation decays exponentially.  \statuslean{} \statusnew
\end{proposition}

The exact formula is kernel-verified through \lean{ExactCounting}; only its repackaging as a
phase-zero statement is new.  In particular, any deterministic estimate
\[
  Z_N(0;3^{-N})=o(N)
\]
along an unbounded sequence of \(N\)'s would prove the conjecture.  This target is much stronger
than fixed-scale equidistribution, but it is also much more concrete than ``show phase zero is
generic.''

\subsection{Second moment and pair-correlation energy}

Probability also isolates the only obstruction to a second-moment argument.  Put
\(y_{N,a}=\{a^{\thetaq}\}\), let
\(d_{a,a'}=\distZ{y_{N,a}-y_{N,a'}}\), and assume \(0<\eta\le1/4\).  Two radius-\(\eta\) arcs
on the circle have overlap \((2\eta-d_{a,a'})_+\).  Therefore
\begin{equation}
 \E Z_N(U;\eta)^2
 =2\eta|D_N|
  +2\sum_{\substack{a,a'\in D_N\\a<a'}}(2\eta-d_{a,a'})_+ .
 \label{eq:phase-second-moment}
\end{equation}
The second term is a precise microscopic pair-correlation energy.  At
\(\eta=3^{-N}\), a random-points model predicts it to be negligible, but proving that at the
fixed exponent would already require control of differences
\(a^{\thetaq}-a'^{\thetaq}\) modulo one on a scale exponentially smaller than the reciprocal
sample size.  A counterexample creates an explicit coherent cluster at zero, so the
pair-correlation term must then be large.  Equation~\eqref{eq:phase-second-moment} is therefore
not a shortcut; it is a clean measurement device for the obstruction.

\section{Fej\'er derandomization and the exact exponential-sum target}
\label{sec:fejer-target}

A positive trigonometric kernel turns the phase-zero question into one exact Fourier identity.
Write \(e(t)=e^{2\pi i t}\).  For an integer \(H\ge2\), let
\[
 F_H(t):=\frac1H\left|\sum_{j=0}^{H-1}e(jt)\right|^2
 =\sum_{|h|<H}\left(1-\frac{|h|}{H}\right)e(ht),
 \qquad
 \Phi_H(t):=\frac{F_H(t)}{H}.
\]
...
Define
\[
  S_N(h):=\sum_{a\in D_N}e\bigl(h a^{\thetaq}\bigr),
  \qquad
  Q_N(H):=\sum_{a\in D_N}\Phi_H\bigl(a^{\thetaq}\bigr).
\]

\begin{theorem}[Positive-kernel certificate]
\label{thm:fejer-certificate}
For every \(N\) and \(H\ge2\),
\begin{align}
  H_N&\le Q_N(H), \label{eq:HN-QN}\\
  Q_N(H)
  &=\frac{|D_N|}{H}
   +\frac{2}{H}\sum_{h=1}^{H-1}
       \left(1-\frac{h}{H}\right)\Re S_N(h). \label{eq:QN-Fourier}
\end{align}
For a uniform random phase \(U\), the shifted score satisfies
\[
  \E_U\sum_{a\in D_N}\Phi_H(a^{\thetaq}-U)=\frac{|D_N|}{H}.
\]
Consequently, if for some unbounded sequence of \(N\)'s and some \(H_N^*\) with
\(|D_N|/H_N^*=o(N)\) one has
\[
 \frac{2}{H_N^*}\sum_{h=1}^{H_N^*-1}
       \left(1-\frac{h}{H_N^*}\right)\Re S_N(h)=o(N),
 \label{eq:fejer-sufficient}
\]
then Conjecture~\ref{conj:AE} holds.  \statusnew
\end{theorem}

\begin{proof}
Every exact hit has fractional part zero and contributes \(\Phi_H(0)=1\); all other terms are
nonnegative, proving \eqref{eq:HN-QN}.  Equation~\eqref{eq:QN-Fourier} is the finite Fourier
expansion of the Fej\'er kernel summed over \(D_N\), and translation invariance of Haar measure
gives the random-phase mean.  Under a counterexample,
Proposition~\ref{prop:phase-zero-dichotomy} gives \(H_N/N\to1/\beta>0\).  The two terms on the
right of \eqref{eq:QN-Fourier} would instead be \(o(N)\) along the chosen sequence, a
contradiction.
\end{proof}

The natural bandwidth is \(H=3^N\).  Its random-phase mean is
\[
  \frac{2^N-1}{3^N}<\left(\frac23\right)^N,
\]
matching Theorem~\ref{thm:random-phase-extinction}.  The cost is that
\eqref{eq:fejer-sufficient} requires information on frequencies all the way to \(3^N\).
Fixed-frequency Weyl convergence, even with a power-saving discrepancy estimate, is far too
coarse.  A robust but stronger sufficient condition is
\[
 \frac1{H_N^*}\sum_{1\le |h|<H_N^*}|S_N(h)|=o(N),
\]
and even square-root-size bounds for the individual sums would miss this by an exponential
factor.  The probabilistic formulation therefore exposes, rather than hides, the same
one-logarithm deficit seen by the determinant hierarchy: the conjectural population is only
linear in \(N\), while standard harmonic-analysis errors at \(2^N\) sample points are
exponential in \(N\).

\section{The Matrix Coefficient Conjecture specialized to the integer problem}
\label{sec:matrix-coefficient}

Dasgupta and Kakde introduced the Matrix Coefficient Conjecture in 2024: a singular square
matrix of logarithms of algebraic numbers should admit nonzero rational vectors on the left
and right whose resulting scalar coefficient vanishes \cite{DasguptaKakde2024}.  They prove
that their conjecture follows from the Structural Rank Conjecture, show that in dimension two
it is equivalent to the Four Exponentials Conjecture, and reduce a proposed proof strategy to
the construction of sparse auxiliary polynomials.  The present problem gives an unusually
transparent integral specialization.

Let \(A,B\in\N_{>0}\) and write
\[
 L(A,B):=
 \begin{pmatrix}
   \log2&\log3\\
 ...
 \end{pmatrix}.
\]
If \(A=2^x\) and \(B=3^x\), then
\begin{equation}
  L(A,B)=
  \begin{pmatrix}1\\x\end{pmatrix}
  \begin{pmatrix}\log2&\log3\end{pmatrix}.
  \label{eq:AE-log-rank-one}
\end{equation}

\begin{theorem}[Matrix-coefficient implication]
\label{thm:MCC-implies-AE}
Assume the real Matrix Coefficient Conjecture for \(2\times2\) matrices.  Then
Conjecture~\ref{conj:AE} holds.  More precisely, for a pair \((A,B)\) satisfying
\(\det L(A,B)=0\), any nonzero \(w,v\in\Q^2\) with
\(w^{\mathsf T}L(A,B)v=0\) forces
\[
  \frac{\log A}{\log2}=\frac{\log B}{\log3}\in\Q.
\]
If \(A,B\) are integers arising from the original antecedent, the common quotient is an
integer.  \statusknown{} \statusnew
\end{theorem}

\begin{proof}
Put \(x=\log A/\log2=\log B/\log3\).  From
\eqref{eq:AE-log-rank-one},
\[
 w^{\mathsf T}L(A,B)v
 =(w_0+xw_1)(v_0\log2+v_1\log3).
\]
The second factor cannot vanish for nonzero \(v\in\Q^2\), because clearing denominators and
exponentiating would give a nontrivial relation \(2^r3^s=1\), contrary to unique
factorization.  Hence \(w_0+xw_1=0\).  Nonzero \(w\) then has \(w_1\ne0\), so
\(x=-w_0/w_1\in\Q\).  Finally, a rational exponent whose power of the power-primitive base
\(2\) is an integer is itself an integer, the elementary valuation descent already
kernel-verified in \lean{IntegerExponent}.
\end{proof}

Thus a hypothetical nonintegral solution supplies not merely a counterexample to the desired
integer statement, but an explicit singular integral logarithm matrix that violates the
Matrix Coefficient Conjecture.  The point of the reformulation is not logical weakening---in
size two it is still the Four Exponentials wall---but a new proof interface: construct a
sparse polynomial on a finite algebraic grid.

\section{A sharp sparse-vanishing theorem on the conditional grid}
\label{sec:sparse-grid}

Assume temporarily that \(A=2^x\) and \(B=3^x\) are positive integers with \(x\notin\Q\).
For \(N\ge1\), define the integer grid
\[
 \mathcal G_N(A,B)
 :=\{(2^iA^j,3^iB^j):0\le i,j<2N\}.
\]
The corresponding real parameters are
\(t_{i,j}=i+jx\), and irrationality of \(x\) makes all \(4N^2\) parameters distinct.
...
\[
 ...
 \qquad (r_\nu,q_\nu)\in\Z^2,
\]
restricts to the exponential polynomial
\begin{equation}
 f_P(t):=P(2^t,3^t)
 =\sum_{\nu=1}^{s}c_\nu
   \exp\bigl((r_\nu\log2+q_\nu\log3)t\bigr).
 \label{eq:laurent-exponential-polynomial}
\end{equation}
Distinct Laurent monomials give distinct real frequencies because \(\log2\) and \(\log3\)
...
\[
 f(t)=\sum_{\nu=1}^{s}c_\nu e^{\lambda_\nu t}
\]
with distinct real \(\lambda_\nu\) has at most \(s-1\) distinct real zeros.  The statement
holds for complex coefficients as well.  \statusknown
\end{lemma}

\begin{proof}
For real coefficients, order the frequencies and divide by the smallest exponential.  Rolle's
theorem and induction on \(s\) give the claim.  For complex coefficients choose a real linear
functional on \(\C\) whose composition with \(f\) is not identically zero.  Every real zero
of \(f\) remains a zero of the resulting real exponential polynomial, which has at most
\(s\) nonzero terms.
\end{proof}

This is the real zero-counting input used in the fewnomial section of
Dasgupta--Kakde \cite{DasguptaKakde2024}.  In the present grid it yields an exact threshold,
not merely an asymptotic support condition.

\begin{theorem}[Sharp sparse-vanishing threshold]
\label{thm:sharp-sparse-threshold}
Let \(x\notin\Q\), \(A=2^x\in\N\), and \(B=3^x\in\N\).  If a nonzero Laurent polynomial
\(P\in\C[X^{\pm1},Y^{\pm1}]\) vanishes at every point of \(\mathcal G_N(A,B)\), then
\[
  |\supp P|\ge4N^2+1.
\]
The bound is sharp in the following strong sense: for any chosen
\(4N^2+1\) distinct ordinary monomials, there is a nonzero polynomial with rational
coefficients, supported on exactly those monomials, that vanishes on all of
\(\mathcal G_N(A,B)\).  \statusnew
\end{theorem}

\begin{proof}
If \(P\) has \(s\) monomials, then \eqref{eq:laurent-exponential-polynomial} is a nonzero
exponential polynomial with \(s\) distinct frequencies.  Vanishing on the grid gives zeros at
the \(4N^2\) distinct parameters \(i+jx\), so Lemma~\ref{lem:exp-zero-bound} gives
\(4N^2\le s-1\).

For sharpness, choose \(4N^2+1\) ordinary monomials and form their evaluation matrix on the
\(4N^2\) integer points of the grid.  It is an integer matrix with more columns than rows, so
it has a nonzero rational kernel vector.  The corresponding polynomial vanishes on the grid.
If one coefficient were zero, its support would have size at most \(4N^2\), contradicting the
first part; hence every chosen monomial occurs and the support size is exactly
\(4N^2+1\).
\end{proof}

\begin{corollary}[A finite sparse certificate would prove integrality]
\label{cor:sparse-certificate}
Let \(A,B\in\N_{>0}\) satisfy \(\det L(A,B)=0\), and let
\(x=\log A/\log2=\log B/\log3\).  If for some \(N\) there is a nonzero Laurent polynomial
with at most \(4N^2\) monomials that vanishes on \(\mathcal G_N(A,B)\), then \(x\in\Q\); if
\(A=2^x\) and \(B=3^x\) satisfy the original antecedent, then \(x\in\Z\).  \statusnew
\end{corollary}

Dasgupta and Kakde formulate a general condition \((m')\) asking, for large \(N\), for a
polynomial with fewer than \(N^2\) monomials vanishing on the analogous \(X(2N)\) grid.  They
prove that, for a real rank-one \(2\times2\) matrix, this forces the desired rational
coefficient relation; the missing implication is the construction of the polynomial from
singularity \cite{DasguptaKakde2024}.  Theorem~\ref{thm:sharp-sparse-threshold} specializes and
calibrates that mechanism here: the zero-counting wall is exactly \(4N^2+1\), while any
subcritical certificate---in particular their much sparser one---would settle the integer
problem.

\section{Integrality amplification and a probabilistic audit of sparse polynomials}
\label{sec:probabilistic-fewnomials}

The conditional grid consists of integer points.  This removes one layer of number-field
bookkeeping from the usual auxiliary-polynomial strategy.

\begin{proposition}[Integrality amplifier]
\label{prop:integrality-amplifier}
Let \(0\ne P\in\Q[X,Y]\), and let \(D\ge1\) be an integer such that \(DP\in\Z[X,Y]\).  If
\[
  |P(g)|<D^{-1}
  \qquad\text{for every }g\in\mathcal G_N(A,B),
\]
then \(P\) vanishes on the whole grid.  Consequently, if
\(|\supp P|\le4N^2\), the same inequality forces \(x\in\Q\) and hence, in the original
integer problem, \(x\in\Z\).  \statusnew
\end{proposition}

\begin{proof}
For every integer grid point \(g\), the number \(DP(g)\) is an integer of absolute value
strictly less than one, hence zero.  Apply Corollary~\ref{cor:sparse-certificate}.
\end{proof}

This is a particularly clean target: construct a rational fewnomial with subcritical support,
controlled denominator, and values uniformly below the denominator threshold.  It is the
integral analogue of making the product of archimedean and nonarchimedean norms less than one.
The difficulty is that positive monomials at the grid points vary over enormous scales; random
signs do not by themselves create the required cancellation.

\begin{proposition}[Random-sign energy identity]
\label{prop:random-sign-energy}
Fix distinct monomials \(M_1,\ldots,M_s\), real coefficients
\(c_1,\ldots,c_s\), and independent Rademacher signs
\(\epsilon_1,\ldots,\epsilon_s\).  Put
\[
  P_\epsilon=\sum_{\nu=1}^{s}\epsilon_\nu c_\nu M_\nu.
\]
For every finite set \(G\subset\R_{>0}^2\),
\[
 \E\sum_{g\in G}|P_\epsilon(g)|^2
 =\sum_{\nu=1}^{s}|c_\nu|^2\sum_{g\in G}|M_\nu(g)|^2.
 \label{eq:random-sign-energy}
\]
At a fixed \(g\), if
\(\sigma_g^2=\sum_\nu|c_\nu M_\nu(g)|^2\), then
\[
  \Pr\left(|P_\epsilon(g)|\ge\frac{\sigma_g}{\sqrt2}\right)\ge\frac1{12}.
\]
\statusnew
\end{proposition}

\begin{proof}
Independence and \(\E(\epsilon_\mu\epsilon_\nu)=0\) for \(\mu\ne\nu\) give
\eqref{eq:random-sign-energy}.  For the pointwise estimate, write
\(X=P_\epsilon(g)\).  The Rademacher fourth-moment identity gives
\(\E X^4\le3(\E X^2)^2=3\sigma_g^4\).  Apply the Paley--Zygmund inequality to
\(X^2\) at level \(\sigma_g^2/2\).
\end{proof}

Thus unpreconditioned random signs are not a plausible source of uniform values smaller than
one: with fixed positive probability, even one evaluation has the natural root-mean-square
size.  A viable probabilistic construction must randomize more than signs.  Three concrete
possibilities remain:
\begin{enumerate}
\item choose the support from a valuation-adapted distribution and work in an integer-valued
      Newton or Hasse basis rather than the monomial basis;
\item precondition by rational monomials so that archimedean magnitudes are comparable, while
      tracking the exact denominator network through the product formula;
\item use dependent coefficients produced by a discrepancy or vector-balancing theorem, then
      exploit extra prime divisibility to cross the strict \(D^{-1}\) threshold.
\end{enumerate}
The first two are compatible with the integer-jet and rational-basis programs above.  The
third would need a balancing theorem with exponentially stronger accuracy than ordinary
Spencer-type square-root cancellation, because the final target is an exact integer zero.

\section{A unified sparse-small-value research target}
\label{sec:sparse-small-value-target}

The Matrix Coefficient and probability viewpoints can be compressed into one finite target.
Assume \(A=2^x\), \(B=3^x\) are integers and \(\det L(A,B)=0\).  For each \(N\), seek data
\[
  (S_N,c_N,D_N),
  \qquad
  S_N\subset\Nzero^2,
  ...
  \quad c_N:S_N\to\Z,
\]
not all zero, and put
\[
 P_N(X,Y)=\frac1{D_N}\sum_{(r,q)\in S_N}c_N(r,q)X^rY^q.
\]
It would suffice to prove
\begin{equation}
 \max_{0\le i,j<2N}
 \left|
   \sum_{(r,q)\in S_N}c_N(r,q)
   (2^iA^j)^r(3^iB^j)^q
 \right|<1.
 \label{eq:sparse-integer-target}
\end{equation}
Because the quantity is an integer, \eqref{eq:sparse-integer-target} makes it zero; the sharp
fewnomial theorem then forces \(x\in\Q\).  The denominator \(D_N\) cancels from this integer
form, so it is often better to regard it as a normalization used only in the archimedean
construction.

There are three logically distinct ways to reach \eqref{eq:sparse-integer-target}:
\begin{description}
\item[Archimedean.] Construct an exponential polynomial with a very small sup norm on the
parameter set \(\{i+jx\}\), while keeping integer coefficients after evaluation.
\item[Adelic.] Prove that every value has a large universal divisor and that the quotient has
absolute value less than one.  This is the determinant/product-formula route in scalar form.
\item[Probabilistic.] Produce a distribution on supports and dependent coefficients for which
the probability of satisfying all \(4N^2\) inequalities is positive.  The random-sign audit
shows that any such distribution must encode the arithmetic geometry rather than treat the
monomials as exchangeable.
\end{description}

\begin{target}[Subcritical analytic fewnomial]
\label{target:analytic-fewnomial}
Derive, from \(\det L(A,B)=0\) and the integrality of \(A,B\), a sequence of nonzero integer
polynomials satisfying \(|\supp P_N|\le4N^2\) and
\(\max_{g\in\mathcal G_N}|P_N(g)|<1\).
Any one value of \(N\) for which this succeeds proves that the common exponent is rational.
\end{target}

Dasgupta--Kakde's open implication \((w)\Rightarrow(m')\) asks for a stronger support saving
in a much more general setting.  The present target is weaker in support but stronger in
arithmetic input: all grid coordinates are positive integers and the first row is the fixed
pair \((2,3)\).  This makes it a plausible test case for the general strategy and a natural
formalization boundary for ProveIt.

\section{Recent literature after the earlier reconnaissance}
\label{sec:recent-literature3}

The literature search was extended through August 21, 2026.  Four developments sharpen the
map without closing the problem.

\begin{enumerate}
\item \textbf{Matrix coefficients and fewnomials.}
Dasgupta and Kakde's 2024 Matrix Coefficient Conjecture is the most directly relevant new
reformulation \cite{DasguptaKakde2024}.  In size two it is equivalent to Four Exponentials.
Their condition \((m')\) replaces degree by support and reduces the real rank-one case to the
zero bound for exponential sums; the construction \((w)\Rightarrow(m')\) remains open.  The
specialization in Sections~\ref{sec:matrix-coefficient}--\ref{sec:sparse-grid} identifies the
exact support threshold for the Alaoglu--Erd\H{o}s grid.

\item \textbf{Metric powers and exceptional transcendental exponents.}
Iyer's 2025 work proves an almost-everywhere finiteness theorem for near-integral sums of
powers under a convergent-volume hypothesis and, in the opposite direction, constructs an
exceptional transcendental exponent with arbitrarily strong repeated near-integrality
\cite{Iyer2025}.  This validates both halves of the probability diagnosis: the random model is
correct metrically, and transcendence is insufficient to derandomize it at a named exponent.

\item \textbf{Valued-field determinant counting.}
Vermeulen's 2025 determinant-method bounds for rational points on transcendental curves in
arbitrary \(1\)-\(h\)-minimal valued fields broaden the nonarchimedean counting framework
\cite{Vermeulen2025}.  They do not impose the synchronized pure-power denominator condition
\((2^N,3^N)\) needed here, so the compact-arc orbit remains far below the threshold at which
those counting theorems force algebraic structure.

\item \textbf{An automata-theoretic bypass in a neighboring problem.}
Bustos-Gajardo, Fokkink, and Yassawi proved a Gaussian-integer Cobham theorem without the Four
Exponentials assumption previously used in that setting \cite{BustosFokkinkYassawi2026}.
Their classification has an explicit exceptional branch when the numeration bases are roots
of integers.  The positive integer bases \(2\) and \(3\) lie precisely in that branch, so the
result does not transfer to the present conjecture.  It nevertheless supports a useful
methodological lesson: a theorem formerly routed through Four Exponentials can sometimes be
reproved by exploiting exact arithmetic structure, but the root-of-integer case is the hard
residue rather than a solved one.
\end{enumerate}

No arXiv paper or published source located in this update claims an unconditional resolution
of the real \(2\times2\) Four Exponentials problem or of this integral special case.  The
current status therefore remains exactly as in the earlier conclusion: the conjecture is
open, but its remaining obstruction can now be stated simultaneously as a matrix-coefficient
problem, a subcritical fewnomial construction, and a phase-zero anti-concentration problem.

\section{Prioritized probability-and-fewnomial program}
\label{sec:probability-program}

The new routes suggest the following order of attack.

\begin{enumerate}
\item \textbf{Compute the Fej\'er score, not only exact hits.}
For \(H=3^N\), evaluate
\[
 Q_N(H)=\sum_{2^N<a<2^{N+1}}\Phi_H(a^{\thetaq})
\]
with certified interval arithmetic for moderate \(N\).  This records the whole near-hit
cluster seen by the positive kernel and can reveal whether the phase-zero anomaly is caused by
a few isolated terms or a broader coherent packet.

\item \textbf{Search valuation-adapted sparse supports.}
Instead of rectangles of monomials, optimize supports of size at most \(4N^2\) against the
integer grid using exact rational linear algebra, Hasse bases, and tropical valuation costs.
The objective is not exact interpolation---which requires \(4N^2+1\) terms under a
counterexample---but an integer vector whose evaluated values have a common divisor larger
than their archimedean maximum.

\item \textbf{Use randomization only after preconditioning.}
Normalize monomials by a rational change of basis so that their values have comparable
archimedean scale, then use dependent randomized rounding or discrepancy minimization.  Record
all denominators as additional finite-prime costs.  Proposition~\ref{prop:random-sign-energy}
provides the baseline any improvement must beat.

\item \textbf{Exploit the least-generator valuation constraints inside the support search.}
The simultaneous primitive gcd condition and the base-adic depth dichotomy should be imposed
before optimization.  A certificate valid only for primitive valuation patterns is enough,
because every hypothetical counterexample admits that normalization.

\item \textbf{Formalize the elementary bridge first.}
Theorem~\ref{thm:sharp-sparse-threshold}, Corollary~\ref{cor:sparse-certificate},
Theorem~\ref{thm:random-phase-extinction}, and the Fej\'er identity use only finite sums,
calculus at an elementary level, and the existing irrationality of \(\thetaq\).  They are
low-risk Lean targets and would make the new analytic interfaces explicit before any
large computation is trusted.
\end{enumerate}

\section{Conclusion of the third continuation}
\label{sec:conclusion3}

Probability theory does not prove the conjecture by declaring \(\log_2 3\) typical.  What it
does provide is a rigorous control experiment: after a uniform phase shift, the expected
number of synchronized near hits in block \(N\) is \(O((2/3)^N)\), and almost every phase has
none eventually; a counterexample would force phase zero to carry \(\sim N/\beta\) exact hits.
The Fej\'er identity expresses the entire derandomization gap as one finite weighted average of
exponential sums up to frequency \(3^N\).

The Matrix Coefficient Conjecture provides the algebraic counterpart.  A counterexample gives
a singular rank-one matrix of logarithms for which every rational left-right coefficient is
nonzero.  On its associated integer grid, zero counting is exact: \(4N^2\) nodes require at
least \(4N^2+1\) monomials.  Therefore any subcritical polynomial whose values are forced
below the integer threshold would settle the problem immediately.

These are not two unrelated restatements.  Both ask for controlled cancellation at an
arithmetic configuration that is overwhelmingly nonexceptional after randomization but could
be coherent at the distinguished phase.  The next genuinely decisive advance would be either
an arithmetic phase-zero anti-concentration estimate of order \(o(N)\), or an analytic
fewnomial construction with at most \(4N^2\) terms.  Each target is finite, quantitative,
computationally testable, and strictly stronger than the generic probabilistic heuristics that
precede it.
...
Section~\ref{lp:twoprime} \\
\midrule
\multicolumn{3}{@{}l}{\emph{Probability and sparse certificates}}\\
Uniform metric estimate and Borel--Cantelli theorem for near-integral powers &
\statusnew & Theorem~\ref{thm:metric-one-power}, Corollary~\ref{cor:metric-bc} \\
Almost every random phase has no late synchronized near hits & \statusnew &
Theorem~\ref{thm:random-phase-extinction} \\
Exact phase-zero count under a counterexample & \statuslean{} \statusnew &
Proposition~\ref{prop:phase-zero-dichotomy}; \lean{ExactCounting} \\
Positive Fej\'er-kernel sufficient criterion & \statusnew &
Theorem~\ref{thm:fejer-certificate} \\
Matrix Coefficient Conjecture in size two implies Conjecture~\ref{conj:AE} &
\statusknown{} \statusnew & Theorem~\ref{thm:MCC-implies-AE} \\
Sharp $4N^2+1$ support threshold on the conditional integer grid & \statusnew &
Theorem~\ref{thm:sharp-sparse-threshold} \\
Subcritical small-value fewnomial certificate implies integrality & \statusnew &
Corollary~\ref{cor:sparse-certificate}, Proposition~\ref{prop:integrality-amplifier} \\
Random-sign energy and anti-concentration audit & \statusnew &
Proposition~\ref{prop:random-sign-energy} \\
\midrule
...
\newblock arXiv:2408.08178 [math.NT], submitted August 15, 2024.
\newblock \url{https://arxiv.org/abs/2408.08178}.

\bibitem{Iyer2025}
S.~Iyer,
\newblock \emph{Distribution of $\theta$-powers and their sums},
\newblock arXiv:2503.15789 [math.NT], submitted March 20, 2025.
\newblock \url{https://arxiv.org/abs/2503.15789}.

\bibitem{Vermeulen2025}
F.~Vermeulen,
\newblock \emph{Counting rational points on transcendental curves in valued fields},
\newblock arXiv:2506.19411 [math.NT], version 2, July 4, 2025.
...
\newblock arXiv:2510.01440 [math.NT], version 3, August 8, 2026.
