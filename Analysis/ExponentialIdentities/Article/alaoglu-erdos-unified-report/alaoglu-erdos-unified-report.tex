...
would close the conjecture from the four values at $1,2,3,6$; fresh-prime cancellation in a
nested Newton table would have to synchronize base and output relations locally; and a
partial norm preserving the exponentially small shifted Newton--Kummer cocycle would have to
avoid the output-erasing ordinary trace.  None of these bridges follows from the present
hypotheses, and the four-exponentials barrier remains intact.
\end{minipage}
\vfill
...
nonnegative-coefficient representative; a fresh-prime Newton theorem converts primitive
denominator cancellation into local relation transfer; and a shifted higher Newton--Kummer
cocycle gives an exponentially small algebraic integer while exposing exact norm and trace
deficits.  Complete synchronized gap products give integer orbit polynomials, exact
...
\end{equation}
This converts the reversed integer table into an entire positive-definite lattice, rather
than only one divided difference.  Closure would require an integer polynomial whose
analytic $L^2$ norm falls below $1/k_0$; ordinary localization estimates have not reached
that scale.  The finite cofactor, integral-Hankel, and quadratic-lattice transport is
\statuslean{} in \lean{ReciprocalMomentLatticeFinite}; the Stieltjes representation and
positivity are paper inputs.
...
therefore in the harder rank-four Kummer branch.  In the ring of integers of $K_N$, the next
concrete local object is the ideal sum
\begin{equation}
 (D_m(2^{1/N}))+(D_m(3^{1/N})),
 \label{crit:eq-newton-kummer-ideal}
\end{equation}
whose prime-ideal size could in principle supply the divisor missing from the raw norm.
...
\statusnew{} paper mathematics.  No partial norm preserving the real cancellation is known.
...
\statusnew{} paper arguments.  Together with the refined
Stieltjes/Jensen, Hermite, reciprocal-Hankel, BCZ, and pure-denominator results, they isolate
three sharp surviving interfaces: a positive arithmetic interpolant, aggregate fresh-prime
synchronization, or a partial norm that preserves the shifted Newton--Kummer cancellation.
None is supplied by the original hypotheses.}
\end{statusbox}
...
One-Vandermonde reciprocal moments, integral positive-definite Hankel lattice, and normalized
$1/k_0$ floor & finite arithmetic core \statuslean; analytic representation \statusnew &
...
canonical alternating tail, and boundary-quotient lower bounds & \statusnew &
...
Fresh-prime pole, local synchronization, translated-block warning, and BCZ ray divisor &
...
Shifted Newton--Kummer exponential smallness, norm deficit, and output-erasing traces &
...
Hankel moments, determinant and quadratic-form floors under an explicit paper-supplied
