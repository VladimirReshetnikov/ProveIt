...
four-exponentials barrier intact.  Reports 21--27 add independent moment, rank-geometry,
unimodular-gap, orbit-entropy, Pad\'e-defect, carry-language, and canonical-product audits.
Their formal cores certify exact denominator, matrix, collision, entropy, defect,
carry-word, and D-finite identities; their strongest paper endpoints still require a new
transcendence, height, uniformity, or interpolation-regularity bridge.  A subsequent
closure audit constructs the canonical critical-type Stieltjes interpolant, sharpens the
canonical-product threshold through its quadratic term, and proves exact Lambert, Hankel,
fixed-prime-support, and pure-denominator no-go theorems.
\end{minipage}
\vfill
...
in all twenty-seven reports under \lean{Article/new-reports/}.  The reports share this unified
...
Report 21 adds a Hausdorff moment representation, exact structural-prime moment defects,
finite rational-quadrature failure, and M\"untz-invisible positive perturbations.  Report 22
classifies the depth-two logarithmic tensor by hyperbolic rank and supplies an exact rational
symmetrization.  Report 23 proves determinant-distance gap gcds, transported rational
secants, a finite order-signature no-go, and an entrywise jet ceiling.  Report 24 computes
the conditional orbit heights exactly and turns Farey counting into simultaneous worst-case
denominator entropy.  Report 25 optimizes Kawashima's projective Pad\'e defect and proves that
no positive-defect rational tuple can recover the fixed prime logarithm.  Report 26 develops
distinguished-descendant carry languages, exact multinomial fibers, and their finite and
unrestricted pressure laws.  Report 27 identifies the sharp cubic logarithmic type of the
$3$-smooth canonical product, proves subcritical interpolation rigidity, and isolates an
exact critical boundary cocycle.
The post-report closure audit then determines the canonical entire interpolant selected by
finite Lagrange interpolation, its exact second-order critical growth, and the corresponding
positive Stieltjes denominator target.  It also shows that the natural Lambert/Hadamard,
fixed-$S$, reciprocal-Hankel, and pure-$2/3$-denominator amplifications fail at explicitly
quantified scales, while leaving one output-specific primitive-denominator cancellation as a
sharp finite arithmetic target.
Fourth, it
...
\label{r14:cross-resultant-smith}
...
\begin{equation}
 ...
 \label{r14:eq-simultaneous-favorable}
\end{equation}
...
\begin{equation}
 ...
 \label{r14:eq-grid-transform-nogo}
\end{equation}
...
\begin{equation}
 ...
 \qquad(0\le r\le n).
 \label{r14:eq-opposite-residual-profile}
\end{equation}
These complement the direct floors
\[
 v_2(f_r)\ge n(n-r),\qquad v_3(g_r)\ge n(n-r)
\]
from \eqref{r14:eq-dyadic-scaling}--\eqref{r14:eq-triadic-scaling}.
...
\begin{equation}
 ...
 \label{r14:eq-cross-profile-gap}
\end{equation}
...
\begin{equation}
 ...
 \label{r14:eq-cross-smith-profile}
\end{equation}
...
\begin{equation}
 ...
 \label{r14:eq-cross-determinantal-divisors}
\end{equation}
...
\begin{equation}
 ...
 =\frac{n^2(n-1)}2.
 \label{r14:eq-cross-resultant-exact}
\end{equation}
...
\]
give \eqref{r14:eq-cross-smith-profile}.  The convex matching inequality and the universal
...
\begin{align}
 ...
 \label{r14:eq-eventual-content}
\end{align}
Writing $F_K^{\rm pr}=\sum f_rX^r$ and $G_K^{\rm pr}=\sum g_rX^r$, one has
\begin{align}
 v_2(g_r)&=v_3(f_r)=\binom{n-r}{2} &&(0\le r\le n),
 \label{r14:eq-eventual-opposite}\\
 v_2(f_{n-e})&\ge e(n-1)+1,&
 v_3(g_{n-e})&\ge e(n-1)+1 &&(1\le e\le n).
 \label{r14:eq-eventual-direct}
\end{align}
Consequently the primitive cross-Sylvester matrix has, at both $2$ and $3$, the full Smith
profile \eqref{r14:eq-cross-smith-profile} and determinantal-divisor orders
\eqref{r14:eq-cross-determinantal-divisors}.  Thus content normalization enlarges the exact
...
\eqref{r14:eq-eventual-opposite}, while the direct residual has the stronger descending
floor \eqref{r14:eq-eventual-direct}.  Pivoting the Sylvester matrix again gives
...
\eqref{r14:eq-cross-resultant-exact} are \statuslean{} in
\lean{CrossResidualScaling}, \lean{ResultantScaling},
\lean{Report14NodeDivisorScaling}, and \lean{Report14CrossResultantBridge}.  The strongest
kernel endpoint, \lean{report14_actual_divisors_favorable_cross_resultant}, keeps the primary
interpolant scalings and the two nonzero residue reductions as explicit hypotheses; the
barycentric derivation of those hypotheses from \eqref{r14:eq-simultaneous-favorable} remains
the paper layer.
...
\begin{equation}
 ...
 =\mu_s=\frac{s^2(s-1)}2.
 \label{r14:eq-deep-subresultant-chain}
\end{equation}
...
literal application of the all-nonzero-minor inequality of Section~\ref{r17:smith-profile}.
...
\begin{equation}
 ...
 \le \binom{2n}{n+m}.
 \label{r14:eq-cross-support-aware-bound}
\end{equation}
...
\begin{equation}
 ...
 \qquad(0\le\rho\le1).
 \label{r14:eq-cross-compound-deficit}
\end{equation}
...
Section~\ref{r9:common-divisor}, and the associated Barnes product is an exact common
...
\section{Twenty-first report: Hausdorff moments and M\"untz invisibility}
\label{sec:report21}

Report 21 is independent of the previous continuations.  Its minimal transcendence and
fractional-normalization steps overlap the accepted corpus; its new direction is the additive
...
\begin{equation}
 ...
 \label{r21:eq-complete-monotone}
\end{equation}
...
\begin{equation}
 ...
 \label{r21:eq-defect-denominators}
\end{equation}
...
are \statuslean{} in \lean{Report21MomentArithmetic}; positivity comes from
\eqref{r21:eq-hausdorff} and remains the paper layer.
...
\begin{equation}
 ...
 \label{r21:eq-two-atom}
\end{equation}
is a positive rational two-atom measure reproducing $m_0,m_1,m_2$.  This identity is also
\statuslean{} in \lean{Report21MomentArithmetic}.  More generally, a finite family of
...
\begin{statusbox}{Report 21 was audited independently.  The exact two-atom and denominator
arithmetic is \statuslean{} in \lean{Report21MomentArithmetic}.  The Hausdorff integral,
...
\section{Twenty-second report: depth-two rank geometry and rational symmetrization}
\label{sec:report22}

Report 22 is another independent sibling of the common base.  Its restricted matrix
coefficient, formal prime-symbol, and external-rank reductions overlap
\lean{RestrictedMatrixCoefficient}, \lean{FormalPrimeDeterminant}, and the accepted
external-rank modules.  The genuinely new layer is a quadratic-rank classification and a
...
\lean{Report22RankGeometry}.  Baker's theorem and the passage from the actual two-star tensor
to the rank classification are the paper/classical layer.  The source report wrote the
...
normal form are \statuslean{} in \lean{Report22RankGeometry}; algebraicity and the application
to the counterexample remain the \statusnew{} paper layer.

\begin{statusbox}{Report 22 was audited independently.  Its low-rank exclusion is a classical
Baker consequence, and its hyperbolic pullback and rational symmetrization are new exact
reductions.  The finite matrix identities are kernel-checked in
\lean{Report22RankGeometry}.  The remaining rank-three and rank-four depth-two period
nonvanishing statements are open; neither the conic nor the symmetric orbit proves the
conjecture.}
\end{statusbox}

\section{Twenty-third report: unimodular gaps, transported secants, and finite-order barriers}
\label{sec:report23}

Report 23 is an independent continuation from the common unified base.  Its elementary and
Four/Six Exponentials reductions coincide with accepted material and are not repeated.  Its
new contribution is a determinant-distance law for binomial gaps, an associated family of
...
\begin{equation}
 ...
 \label{r23:eq-decoration-budget}
\end{equation}
...
\begin{statusbox}{Report 23 was reconciled as an independent sibling.  The genuinely new
determinant-distance collision core is \statuslean{} in \lean{UnimodularGapCollision}; the
...
\section{Twenty-fourth report: exact orbit heights and Farey entropy}
\label{sec:report24}

Report 24 is another independent continuation from the common unified base.  Its compact
orbit, recurrence, contact-rigidity, and Four Exponentials ingredients overlap earlier
sections.  Its genuinely new contribution is quantitative: it computes every reduced orbit
...
\]
combines with~\eqref{r24:eq-height-law} as follows.  If
\[
...
\begin{statusbox}{Report 24 was audited as an independent sibling.  The exact floor identities
and finite worst-case entropy mechanism are \statuslean{} in \lean{OrbitHeightEntropy}.
...
\section{Twenty-fifth report: projective Pad\'e defect and rational-transfer no-go}
\label{sec:report25}

Report 25 independently tests the new weight-two polylogarithmic criterion of
Kawashima~\cite{Kawashima2026} against the logarithm determinant.  Its tensor-rank
reduction overlaps the stronger rank geometry in Section~\ref{sec:report22}, and its
directed-rounding scan repeats an unbundled computation already separated from the certified
finite frontier.  Neither is counted again.  The new contribution is an optimized,
...
\begin{equation}
 ...
 \label{r25:eq-projective-defect}
\end{equation}
...
\eqref{r25:eq-projective-defect}.  This piecewise optimization and its abstract height-budget
wrapper are \statuslean{} in \lean{Report25ProjectiveDefect}; the number-field height
...
\begin{equation}
 ...
 \label{r25:eq-efficiency}
\end{equation}
These implications are also \statuslean{} in \lean{Report25ProjectiveDefect}.  Thus mere
...
\begin{equation}
 ...
 \label{r25:eq-no-transfer}
\end{equation}
The same proof applies to a fixed $q\in\Q_{>0}\setminus\{1\}$ with a changed constant.
Indeed~\eqref{r25:eq-efficiency} makes every $\ell_i=O(e^{-a_v})$ while bounding all prime
...
Section~\ref{sec:report22}, whereas every tensor built from these rational logarithms has
...
\begin{statusbox}{Report 25 was audited independently.  The finite projective optimization
and efficiency implications are \statuslean{} in \lean{Report25ProjectiveDefect}; the
number-field specialization, rational-transfer no-go, and fixed-cardinality group finiteness
are \statusnew{} paper results using the cited external theorems.  The repeated $2^{29}$ scan
is not promoted, and no mixed-denominator construction or proof of the conjecture is claimed.}
\end{statusbox}

\section{Twenty-sixth report: carry languages and fixed-target entropy}
\label{sec:report26}

Report 26 is an independent sibling.  Its logarithmic cone, one-orbit equivalence,
one/two/three-base threshold, $3$-smooth lattice count, and residual compact orbit largely
overlap the accepted corpus.  Its distinct contribution is additive: a hypothetical
...
\begin{equation}
 ...
 \label{r26:eq-path-representation}
\end{equation}
...
\begin{equation}
 ...
 \label{r26:eq-multinomial-fiber}
\end{equation}
...
are \statuslean{} in \lean{Report26CarryPath}.  Recovering the word from the unordered
...
\begin{equation}
 ...
 \label{r26:eq-fixed-target-fiber}
\end{equation}
...
$O(n^2)$ endpoints yields~\eqref{r26:eq-fixed-target-fiber}.  The matching upper rate is
...
\begin{statusbox}{Report 26 was reconciled independently.  The bounded carry-word sum,
length, suffix-degree, and ordered reconstruction identities are \statuslean{} in
\lean{Report26CarryPath}.  Multiset injectivity, multinomial fibers, character balancing,
and the finite and unrestricted pressure laws are \statusnew{} paper results.  Every
...
\section{Twenty-seventh report: canonical-product threshold and critical defect cocycle}
\label{sec:report27}

Report 27 independently studies the power function on the universal cover of
$\C^\times$ and the arithmetic sampling set $\{2^a3^b:a,b\ge0\}$.  Its Six Exponentials
monodromy and algebraic-multiplier consequences overlap Sections~\ref{mc:counting-monodromy} and
\ref{r16:spectrum}; they are not repeated.  Its new core is an exact analytic growth
...
\begin{equation}
 ...
 \label{r27:eq-jensen-threshold}
\end{equation}
The constant is sharp by~\eqref{r27:eq-cubic-growth}.  These self-contained statements are
...
\eqref{r27:eq-jensen-threshold} annihilates it; coefficient comparison then leaves a single
...
\begin{equation}
 ...
 \label{r27:eq-edge-factorization}
\end{equation}
where
\[
...
\eqref{r27:eq-edge-growth}, evaluated on the negative real axis, first forces $U=0$ and
...
\begin{statusbox}{Report 27 was audited as an independent sibling.  Its D-finite
noncancellation core is \statuslean{} in \lean{DfiniteExponentCore}.  The universal-cover
...
\section{Post-report closure audit: critical interpolation and exact no-go barriers}
\label{sec:post27}

This section records the direct closure attacks carried out after the reconciliation of the
twenty-seven independent reports.  It is not a twenty-eighth source report.  The strongest
positive result is an explicit entire interpolant at the exact critical type, together with a
...
\label{post27:critical-interpolant}
...
\]
For the canonical product $E$ from~\eqref{r27:eq-canonical-product}, irrational-rotation
equidistribution sharpens the lattice count to
\begin{equation}
 \#\{(a,b)\in\Nzero^2:a\alpha+b\gamma\le T\}
 =\frac{T^2}{2\alpha\gamma}
  +\left(\frac1{2\alpha}+\frac1{2\gamma}\right)T+o(T).
 \label{post27:eq-refined-count}
\end{equation}
...
\begin{equation}
 ...
 \label{post27:eq-refined-product}
\end{equation}
The same Jensen argument as in Section~\ref{r27:canonical-product} now says that every
nonzero entire function vanishing on all $2^a3^b$ has maximum-modulus logarithm at least the
right side of~\eqref{post27:eq-refined-product}, up to $o(T^2)$.
...
\begin{equation}
 ...
 \label{post27:eq-stieltjes-interpolant}
\end{equation}
...
\begin{equation}
 ...
 \label{post27:eq-exact-interpolation}
\end{equation}
...
The Cauchy transform in~\eqref{post27:eq-stieltjes-interpolant} is bounded in the two
...
\begin{equation}
 ...
 \label{post27:eq-interpolant-type}
\end{equation}
...
\begin{equation}
 ...
 \label{post27:eq-quadratic-rigidity}
\end{equation}
...
\]
then the cocycle~\eqref{r27:eq-defect-cocycle} and the global Nevanlinna inequalities give
\begin{equation}
 U,V\ne(0,0)\quad\Longrightarrow\quad u+v\ge\delta_{\mathrm{quad}}.
 \label{post27:eq-nevanlinna-gap}
\end{equation}
...
\label{post27:denominator-target}
...
\begin{equation}
 ...
 \label{post27:eq-stieltjes-divdiff}
\end{equation}
...
\begin{equation}
 ...
 \label{post27:eq-positive-residue}
\end{equation}
...
\eqref{post27:eq-positive-residue} imply
\begin{equation}
 \boxed{
 \liminf_{F\nearrow\{2^a3^b\}}
 \frac{\mathfrak d_F}{\prod_{s\in F}s}\ge\frac1{C_x}>0.}
 \label{post27:eq-denominator-lower}
\end{equation}
...
\begin{equation}
 ...
 \label{post27:eq-higher-jets}
\end{equation}
and therefore
\begin{equation}
 L^3-4r_3L+r_4=0.
 \label{post27:eq-jet-cubic}
\end{equation}
...
\begin{equation}
 ...
 \label{post27:eq-hermite-defect}
\end{equation}
...
\begin{equation}
 ...
 \label{post27:eq-lambert-cocycle}
\end{equation}
More importantly,
\begin{equation}
 \boxed{
 ...
 \label{post27:eq-universal-double-cocycle}
\end{equation}
...
\end{split}
 \label{post27:eq-hankel-product}
\end{equation}
...
\begin{align}
 ...
 \label{post27:eq-hankel-denominator}\\
 \log|\mathcal D_n(s,r)|
 &=-\frac16n^3\log s+\frac12n^2\log(r/s)+O_{s,r}(n).
 \label{post27:eq-hankel-smallness}
\end{align}
...
\begin{equation}
 \log B_n=n\log a-o(n).
 \label{post27:eq-bcz-denominator}
\end{equation}
...
\begin{statusbox}{The post-report closure audit makes one formal addition and several exact
paper-level advances.  The universal Lambert double-difference identity is \statuslean{} in
\lean{LambertCocycleFinite}; because it holds for arbitrary rational $M,N$, it is a certified
no-go rather than a closing bridge.  The refined canonical-product coefficient, Stieltjes
interpolant and denominator criterion, quadratic Jensen/Nevanlinna rigidity, half-density
Hermite ceiling, reciprocal-Hankel product, BCZ fixed-support consequence, and pure-denominator
Ridout barrier are \statusnew{} paper results.  They isolate output-specific primitive
denominator cancellation, or equivalently a strict second-order boundary saving, as the
sharpest surviving interface.}
\end{statusbox}
...
\lean{CrossResidualScaling}, \lean{ResultantScaling},
\lean{Report14NodeDivisorScaling}, \lean{Report14CrossResultantBridge}; primary interpolant
...
simultaneous favorable branch & \statusnew & Section~\ref{r14:cross-resultant-smith}; branch is
...
$n>\max(v_2(M),v_3(A))$ & \statusnew & Section~\ref{r14:cross-resultant-smith}; finite cost
...
Section~\ref{r14:cross-resultant-smith}; simultaneous favorable coefficient profiles \\
...
\statusnew & Section~\ref{r14:cross-resultant-smith}; exact paper consequence of the
...
Section~\ref{r14:cross-resultant-smith}; finite $K=2,3,4$ core factorizations are unbundled
...
\lean{.../Report21MomentArithmetic} & rational two-atom moment reproduction; four exact
common-denominator identities and their structural-prime local-unit conclusions \\
\lean{.../Report22RankGeometry} & hyperbolic normal identity, ternary-conic determinant,
...
\lean{.../Report25ProjectiveDefect} & exact piecewise-linear optimization of the abstract
weight-two defect and the resulting depth and arithmetic-efficiency thresholds \\
\lean{.../Report26CarryPath} & exact distinguished-descendant carry-word sum and length;
...
\lean{.../Report14NodeDivisorScaling} & exact dyadic and triadic normalization of the two
report-14 boundary node divisors, with structural-prime-unit constant terms \\
\lean{.../Report14CrossResultantBridge} & conditional composition of the report-14 primary
...
\lean{Report21MomentArithmetic}, \lean{Report22RankGeometry},
\lean{UnimodularGapCollision}, \lean{OrbitHeightEntropy},
\lean{Report25ProjectiveDefect}, \lean{Report26CarryPath},
\lean{DfiniteExponentCore},\\
\lean{CrossResidualScaling}, \lean{ResultantScaling},
\lean{Report14NodeDivisorScaling}, \lean{Report14CrossResultantBridge},\\
