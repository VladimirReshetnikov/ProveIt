...
\begin{equation}
 \mathcal G_N=
 \frac{\log\mathcal Q_{3,A}(N)}{\log3}
 -\frac{\log\mathcal Q_{2,M}(N)}{\log2}.
...
\begin{align}
 \mathcal G_N
 &=\frac{\log P_{3,A}(N)}{\log3}-\frac{\log P_{2,M}(N)}{\log2}\nonumber\\
 &=\frac{\log\Delta_{3,N}}{\log3}-\frac{\log\Delta_{2,N}}{\log2}
...
\begin{equation}
 ...
 \kappa_{\wedge}:=\frac{\log5\log7}{\log2\log3}.
...
\end{equation}
Here one uses $r\le x\log2/\log5$, $s\le x\log3/\log5$ and the corresponding
...
$\thetaq,\eta$, where
$\eta=\log3/\log C=|r\thetaq-s|/d$: the four values
...
\begin{equation}
 ...
 =1+\left\lfloor\frac{\log H}{\log C_0}\right\rfloor\qquad(H\ge1).
...
$\lambda_j=-\log|\det L_j|/\log V_j$ only on the eventually nonzero tail.  A theorem saying
...
\statuslean{} in \lean{MixedGapKummerFinite}.  The same module verifies the abstract
retention bound
\[
 n_2=0\ \text{or}\ n_3=0
 \quad\Longrightarrow\quad
...
counts are \statuslean{} in \lean{KummerCompatibilityFinite}; the mixed-gap determinant and
same-residue obstruction are \statuslean{} in \lean{MixedGapKummerFinite}.  Chebotarev
densities, mantissa entropy, collision valuation, ray divisor, and the finite-box bridge are
\statusnew{} paper deductions.  They strengthen the denominator lower bound without
supplying the missing upper bound.
...
The four adjacent smooth pairs
\[
 (1,2),\qquad(2,3),\qquad(3,4),\qquad(8,9)
\]
support a Kummer construction whose distinguished embedding has unit rather than
...
resultant & \statuslean & \lean{DenominatorCancellationCounterexample}; synthetic
...
Support classification, exact Chebotarev compatibility densities, mantissa entropy,
...
\lean{.../DenominatorCancellationCounterexample} & exact smallest-grid certificate that the
...
which are techniques that would need adaptation.  Two tempting claims are retracted here;
...
A frequently repeated count of colossally abundant numbers below
