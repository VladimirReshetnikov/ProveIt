...
the $q$-hypergeometric limit, and the infinite defect formula are \statusnew{} paper results.
All broader
...
continued-fraction gcd, transported-secant, finite-signature, and entrywise jet packages are
paper proofs.  Existing transcendence reductions are cross-referenced, and no fixed-order
...
$u_n=2^{t_n}-1$, define its Cantor reevaluation
$\mathcal C(\sum_{j\ge1}\varepsilon_j2^{-j})=\sum_{j\ge1}\varepsilon_j3^{-j}$.
Then
\begin{equation}
 \frac{L(M^n)}{A^n}=3^{-t_n}\bigl(1+\mathcal C(u_n)\bigr),
 \qquad
...
\lean{MixedRadixCarryFinite}, and the finite radix-shadow congruence and valuation-index
bridge are \statuslean{} in \lean{RadixShadow}.  Multiset injectivity, multinomial fibers,
...
mantissa gauge, and square-collision formula are \statusnew{} paper results.  Every
...
The support classification is the cited \statusknown{} input.  The Kummer density,
collision valuation, ray divisor, and finite-box bridge are \statusnew{} paper deductions.
They strengthen the denominator lower bound without supplying the missing upper bound.
...
normalization-defect formulas & \statusnew & Sections~\ref{carry:floor-rectangles} and
\ref{carry:digit-lifts} \\
...
Infinite positive-series closure, canonical alternating tail, and boundary-quotient lower
bounds & \statusnew & Section~\ref{crit:positive-closure} \\
...
Support classification, Kummer defect density, collision divisor, and finite-box detection &
\statusnew & Section~\ref{crit:support-defects}; support and Chebotarev inputs classical \\
...
\lean{RadixShadow}, \lean{DfiniteExponentCore},\\
\lean{CrossResidualScaling}, \lean{ResultantScaling},
