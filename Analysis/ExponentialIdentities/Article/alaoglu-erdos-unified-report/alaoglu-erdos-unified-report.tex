...
have to preserve signed cross-base cancellation beyond ordinary traces, character
projections, ideal gcds, and output partial norms, all of which now have exact deficits.
...
A four-value covariance theorem closes any
nonnegative-coefficient representative; reciprocal Stieltjes moments give a full
Vandermonde-divisible Hankel determinant whose exact $T^5$ asymptotic nevertheless remains
strictly positive; and sparse fresh-prime spikes can force almost the full node-product mass
...
projections and the exact common-ideal norm expose a one-factor partial-norm deficit.
...
\end{equation}
Thus infinitely many Maclaurin coefficients are negative.  Similarly,
\[
...
These stronger floors still miss at the exact leading scale.  Take the complete input
triangle
\[
 \mathcal S_T=\{v=2^a3^b:\log v\le T\},\qquad u=v^x,
 \qquad \mathcal L=\log2\log3,
\]
and let $m/N\to\varrho\in(0,1/2]$.  With $\Delta_T,h_{0,T}$ and
$G_d=(h_{i+j}/h_0)_{0\le i,j< m}$ attached to this table, lattice integration and
Andr\'eief's identity give
\begin{align}
...
\end{equation}
The function in parentheses decreases on $[0,1/2]$ and has minimum $1/20$ at
$\varrho=1/2$.  Thus the supposedly dangerous integer alternant instead grows at a strictly
positive $T^5$ rate for every admissible matrix size.  At the endpoint,
$\log\det G_d\sim xT^5/(30\mathcal L^2)$, whereas the logarithm of the lower floor in
\eqref{crit:eq-reciprocal-strong-floor} is
$-xT^5/(60\mathcal L^2)+o(T^5)$.  At the exact endpoint $N=2m$, for the control $x=2$ the columns in
\eqref{crit:eq-reciprocal-alternant} become the consecutive powers of $v$, so
$|\det\mathcal B|$ is exactly the ordinary Vandermonde in the input nodes; this realizes the
$1/20$ coefficient.  At the same endpoint, the case $x=3$ is the Vandermonde times a
positive Schur factor.

The first two estimates in~\eqref{crit:eq-reciprocal-triangle-asymptotics} follow from
triangular lattice integration.  After writing $s=e^{xt}$, the normalized Andr\'eief
integral has ordered-particle exponent
$x(2jt-t^3/(6\mathcal L))$; its maximizer is
$t_j=2\sqrt{\mathcal Lj}$, which stays inside the triangle precisely for
$\varrho\le1/2$ and yields the displayed $2\sqrt2/15$ constant.  Boundary and separation
errors are $o(T^5)$.  This converts the reversed integer table into an entire
positive-definite lattice and closes the most direct determinant attack negatively: neither
the full Vandermonde divisor nor the optimal smooth triangle becomes subunit.
...
Stieltjes representation, positivity, and asymptotic audit are paper results.
...
A successful algebraic operation would have to retain signed cross-base cancellation among
nontrivial characters, not merely a distinguished branch or a common ideal.

The product/cocycle identities and integral-exponent calibrations are \statuslean{} in
\lean{RankOneNewtonKummer}; the sign, asymptotic, Kummer-degree, norm, and trace analysis is
\statusnew{} paper mathematics, as are the ideal-norm and projection calculations above.
...
\lean{RankOneNewtonKummer}.  Private dyadic order, cleaned-gap capture, and adjacent-smooth
...
bound on a balanced exhaustion, rather than more fresh divisors; or a signed Kummer operation
that couples nontrivial characters across both bases.  The natural Stieltjes transfer family,
sparse-spike aggregation, reciprocal Hankel determinants, ordinary traces, character
projections, common ideals, and subgroup output partial norms do not improve the relevant
bounds at their exact scales.  None of the surviving bridges is supplied by the original
hypotheses.}
\end{statusbox}
...
Canonical critical Stieltjes interpolant, refined quadratic threshold, and positive reduced
denominator target & \statusnew & Sections~\ref{crit:critical-interpolant} and
\ref{crit:denominator-target} \\
One-Vandermonde reciprocal moments, integral positive-definite Hankel lattice,
common-content floors, full alternant divisor, and sharp $T^5$ scale audit & finite arithmetic
and common-content core \statuslean; alternant/asymptotic layer \statusnew &
...
canonical alternating tail, boundary-quotient lower bounds, and Stieltjes-transfer no-go & \statusnew &
...
norm, and one-factor partial-norm deficit &
...
\lean{PositiveCoefficientClosure}, \lean{NewtonCollisionCore},
\lean{RankOneNewtonKummer}, \lean{DyadicPrivateSupport},
