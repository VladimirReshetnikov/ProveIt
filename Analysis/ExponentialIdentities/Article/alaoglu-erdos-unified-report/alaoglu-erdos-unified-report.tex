...
$\log2\,\log u+\log3\,\log v=0$ for positive rationals $u,v$, i.e.\ as
$\thetaq=\log u/\log(1/v)$, which says precisely that some rational has a rational
$\thetaq$-th power.  Hence $\Q$-linear independence of $F$ alone is \emph{equivalent} to
Conjecture~\ref{conj:AE}, not weaker than it.  Theorem~\ref{lp:thm-main} is therefore a
correct sufficient condition whose surplus strength --- independence of pairs $\log p\log q$
with $p,q\ge5$ --- is never used by the problem.  Its genuine dividend is the two-prime case
of Section~\ref{lp:twoprime}, which \emph{is} a theorem and does yield an unconditional
exclusion.  One should not expect the criterion to be an easier route in general.
