...
Each sharpens a finite interface while leaving the same four-exponentials barrier intact.
\end{minipage}
\vfill
...
in all six reports under \lean{Article/new-reports/}.  Reports 1--2 show that the
...
for the exceptional phase zero.
Fourth, it
...
hypotheses.  The generic scaled consecutive-power wiring is now formalized below; only the
construction of the explicit fiber certificates identifying the globally first mixed-semigroup
columns of the report's named determinant $D_T$ remains a finite paper-level specialization.
What remains globally is still the valuation and
cancellation of the residual low-excess sums.  No leading-order bound follows from the exact
expansion.
\end{statusbox}
...
\lean{MixedDeterminantUnitBridge}.  Constructing those certificates for the global first-$N$
mixed-semigroup enumeration defining $D_T$ remains paper-level.  The resulting all-layer
gain neither supplies nor upper-bounds the remaining leading-order cancellation excess.
\end{statusbox}
