...
$M=2^x\in\N$, $M>1$ has $x>0$, and since $\thetaq=\log3/\log2$ is irrational, $r=s=0$.
\end{proof}
...
\begin{statusbox}{\statusnew{} for the kernel formalism, with one exception: the input that
$M$ and $A$ cannot both be $3$-smooth is \statuslean{} here
(\lean{integer_of_threeSmooth_outputs} in \lean{SmoothOutputs}), so the dimension bound rests
on a machine-checked foundation rather than an asserted one.}
\end{statusbox}
...
\begin{statusbox}{\statusnew{} --- the equivalence of (i), (iii) and (iv) is the M\"obius
criterion of the fourth source report; the equivalence with (ii) is recorded here.  It is
elementary and self-contained, and is a natural Lean target: it needs only unique
factorization, irrationality of $\thetaq$, and the rational-exponent lemma.}
\end{statusbox}
...
The exponent $\thetaq=\log3/\log2$ and its reciprocal
\[
...
\statusnew & second-iterate kernel (this report) \\
Kernel invariance $K_x=K_y$ across nonintegral solutions & \statusnew &
second-iterate kernel (this report) \\
M\"obius criterion: $K\ne\{0\}$ iff $x\in\PGL_2(\Q)\cdot\thetaq$, iff
$1,\thetaq,x,x\thetaq$ are $\Q$-dependent & \statusnew &
second-iterate kernel (this report) \\
Multiplicative-dependence form: $K\ne\{0\}$ iff $M^eA^{-d}\in\GammaSat$ for some
$(d,e)\ne(0,0)$ & \statusnew & second-iterate kernel (this report) \\
...
$2^{x^2},2^{x^3}\in\Qbar$ & \statusnew & depth-three tower (this report) \\
...
Arbitrary-node repulsion theorem with its analytic input, and corollaries & \statusnew &
...
Semigroup gap theorem in quantitative form & \statusnew & Section~\ref{sec:semigroup} \\
...
Candidate gcd/lcm lattice and ambient root saturation index & \statusnew &
Section~\ref{sec:lattice} \\
Operator formulation on the prime-logarithm space & \statusnew &
Section~\ref{rf:operator} \\
\bottomrule
\end{longtable}
}
...
Two cautions survive that promotion, and the status matrix reflects both.  First, a module
being accepted certifies exactly the theorems it states: \lean{ArbitraryNodeDeterminant} and
\lean{SemigroupDeterminant} deliberately contain no analysis, so the analytic halves of the
repulsion and gap theorems remain \statusnew{}, and \lean{ExponentialPolynomialZeros}
delivers nonvanishing rather than the ordered-chamber positivity proved on paper in
