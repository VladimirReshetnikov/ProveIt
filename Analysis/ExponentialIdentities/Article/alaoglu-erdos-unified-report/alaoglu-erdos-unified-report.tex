...
positive Stieltjes denominator target.  A four-value covariance theorem closes any
...
\begin{equation}
 ...
 \qquad \frac{\log3}{\log2}<\rho_n<2.
 \label{sturmian:eq-return-ratio}
\end{equation}
The ratio on the right is strictly increasing, and at zero has the expansion
\[
 \frac{3^t-1}{2^t-1}
 =\frac{\log3}{\log2}\left(1+\frac{\log3-\log2}{2}t
 +\frac{(\log3-\log2)(2\log3-\log2)}{12}t^2+O(t^3)\right).
\]
Each return therefore gives a rational approximation to $\log3/\log2$ from above, but its
...
passed.  The concatenated extension log has SHA-256
\[
 \sha{b11843448cd8946442aba13758b5b666e4ac6eae50d2243fbdc32d83274cea8d}.
\]
The extracted verifier source has SHA-256
\[
 \sha{f888c0d89be9366f01e2d26f339dcbe446556d39d4c5dbcf8f579ec0d0052e1d};
\]
this differs from an older printed digest, so semantic replay rather than that stale
digest is the source-identity check.  The verifier and invocation details are in
Appendix~\ref{app:scanner}.
...
including chunk tables, SHA--256 identifiers, extremal records, high-precision replays, the
literal C source, and a full manifest.  The logical consequence would be that a nonintegral
solution has $x>29$.  However, the repository contains no standalone scanner, canonical chunk
logs, or replay artifacts named by those hashes, and the long run was not independently
repeated during this audit.  It is therefore retained as a documented \statuscomp{} result,
...
\end{center}
An independent rerun reproduced every mathematical field.  The source JSON digest
...
$a/b\to\sqrt{x\log3/\log2}$ and
$c/d\to\sqrt{x\log2/\log3}$.  If $V\ge2$ and a set $\mathcal P_m$ of good primes admits
moving slopes
\[
...
geometric Smith calculation: one-axis integer-valued interpolation is abundant, but it supplies no
...
continued-fraction convergents to $\log3/\log2$ have determinant distance one; hence their
...
Finally, let $p_n/q_n$ be the continued-fraction convergents to $\log3/\log2$ and choose
...
$\log3/\log2=s/r\in\Q$, impossible.  Every hypothetical cross-coprime counterexample is
