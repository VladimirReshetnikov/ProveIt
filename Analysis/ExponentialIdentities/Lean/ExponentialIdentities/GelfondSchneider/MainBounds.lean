...
public import ExponentialIdentities.GelfondSchneider.MainHol
...
lemma abs_Rb : norm (((R α β σ α' β' γ' hirr htriv habc) q hq0 h2mq) z) ≤ (q * q) * (((c₄ α' β' γ')
  ...
  calc _ ≤ ∑ t, ((GelfondSchneiderHouse.house ((((algebraMap (𝓞 K) K)
  ...
  · unfold R
    ...
    apply GelfondSchneiderHouse.norm_embedding_le_house
  · refine sum_le_sum ?_
    intro i hi
  ...
  · apply sum_le_sum
    ...
    · have lemma82 := house_eta_le_c₄_pow α β σ α' β' γ' hirr htriv habc q hq0 i h2mq
      unfold GelfondSchneiderHouse.house at lemma82
...
lemma use6and8 : norm ((Algebra.norm ℚ (rho α β σ α' β' γ' hirr htriv habc q hq0 h2mq))) ≤ ((c₁₄ α β
  ...
  calc _ ≤  ‖ρᵣ α β σ α' β' γ' hirr htriv habc q hq0 h2mq‖ * (GelfondSchneiderHouse.house (rho α β σ α' β' γ' hirr htriv
  ...
  · have := GelfondSchneiderHouse.norm_norm_le_norm_mul_house_pow (K := K) (α := ((rho α β σ α' β' γ' hirr htriv habc) q
      hq0 h2mq)) σ
    rw [← rho_eq_ρᵣ]
    unfold h
  ...
  · nth_rw 2 [mul_comm]
    ...
    · have := (eq6 α β σ α' β' γ' hirr htriv habc) q hq0 h2mq
      ...
      · exact this
      · simp only [Nat.cast_nonneg]
    · apply pow_nonneg; exact GelfondSchneiderHouse.house_nonneg ((rho α β σ α' β' γ' hirr htriv habc) q hq0 h2mq)
    · apply mul_nonneg
      · apply Real.rpow_nonneg
