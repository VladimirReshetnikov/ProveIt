...
public import ExponentialIdentities.GelfondSchneider.House
public import ExponentialIdentities.GelfondSchneider.Denominator
...
- `house_matrixA_le`: an upper bound on the GelfondSchneiderHouse.house norm of entries of the Siegel matrix `A`.
- `house_eta_le_c₄_pow`: the resulting bound on the GelfondSchneiderHouse.house norm of the solution vector `η`.
...
`GelfondSchneiderHouse.house.exists_ne_zero_int_vec_house_le` that there is a non-trivial set of integer
solutions `η₁, …, η₂` in `K`.
-/
...
lemma house_bound_c₁α :
    GelfondSchneiderHouse.house (c₁ α' β' γ' • α') ^ (a q t * l q u) ≤
      GelfondSchneiderHouse.house (c₁ α' β' γ' • α') ^ (m K * q) := by
  refine Bound.pow_le_pow_right_of_le_one_or_one_le (Or.inl ⟨GelfondSchneiderHouse.one_le_house_of_isIntegral
...
lemma bound_c₁β : 1 ≤ GelfondSchneiderHouse.house (c₁ α' β' γ' • ((q : K) + q • β')) :=
  GelfondSchneiderHouse.one_le_house_of_isIntegral (isIntegral_c₁_smul_q_β' _ _ _ _)
    (smul_ne_zero (c₁_ne_zero _ _ _) (b_sum_ne_zero α β σ α' β' γ' hirr habc q hq0))

include α β σ hirr htriv habc in
lemma one_le_house_c₁γ : 1 ≤ GelfondSchneiderHouse.house (c₁ α' β' γ' • γ') :=
  GelfondSchneiderHouse.one_le_house_of_isIntegral (isIntegral_c₁γ α' β' γ')
...
def c₃ : ℝ := c₂ α' β' γ' * (1 + GelfondSchneiderHouse.house β') * sqrt (2 * m K) *
  (max 1 ((GelfondSchneiderHouse.house α' ^ (2 * m K ^ 2)) * GelfondSchneiderHouse.house γ' ^ (2 * m K ^ 2)))
...
lemma one_le_c₃ : 1 ≤ c₃ α' β' γ' :=
  ...
  le_add_of_nonneg_right <| GelfondSchneiderHouse.house_nonneg _) <|
...
lemma house_c₁_smul_le (x : K) :
    GelfondSchneiderHouse.house (c₁ α' β' γ' • x) ≤ ↑|c₁ α' β' γ'| * GelfondSchneiderHouse.house x := by
  rw [zsmul_eq_mul]; exact (GelfondSchneiderHouse.house_mul_le _ _).trans_eq (by simp)

include α' β' γ' in
lemma house_cCoeffs_smul_eq_factorized :
    GelfondSchneiderHouse.house (cCoeffs α' β' γ' q • systemCoeffs α' β' γ' q u t) =
    GelfondSchneiderHouse.house ((c₁ α' β' γ' ^ ((n K q - 1) - k q u) *
...
lemma house_factorized_le_prod :
    GelfondSchneiderHouse.house ((c₁ α' β' γ' ^ ((n K q - 1) - k q u) *
        ...
        α' ^ (a q t * l q u) * ((c₁ α' β' γ' : K) ^ (b q t * l q u)) *
        γ' ^ (b q t * l q u))) ≤
    GelfondSchneiderHouse.house (((c₁ α' β' γ' : K) ^ (n K q - 1 - k q u) * (c₁ α' β' γ' : K) ^
        (m K * q - a q t * l q u) *
        (c₁ α' β' γ' : K) ^ (m K * q - b q t * l q u))) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' ^ (k q u) • (↑(a q t) + (b q t) • β') ^ (k q u)) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' ^ (a q t * l q u) • α' ^ (a q t * l q u)) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' ^ (b q t * l q u) • γ' ^ (b q t * l q u)) := by
  simp only [nsmul_eq_mul, zsmul_eq_mul, Int.cast_pow, mul_assoc]
  refine (GelfondSchneiderHouse.house_mul_le _ _).trans (mul_le_mul_of_nonneg_left ?_ (GelfondSchneiderHouse.house_nonneg _))
  rw [← mul_assoc, ← mul_assoc, ← mul_assoc]
  refine (GelfondSchneiderHouse.house_mul_le _ _).trans ?_
  rw [← mul_assoc]
  exact mul_le_mul (by grind [mul_assoc, GelfondSchneiderHouse.house_mul_le]) le_rfl (GelfondSchneiderHouse.house_nonneg _)
    (mul_nonneg (GelfondSchneiderHouse.house_nonneg _) (GelfondSchneiderHouse.house_nonneg _))

include K α' β' γ' u t q in
lemma GelfondSchneiderHouse.house_prod_le_smul_pow :
    GelfondSchneiderHouse.house (((c₁ α' β' γ' : K) ^ (n K q - 1 - k q u) * (c₁ α' β' γ' : K) ^
        (m K * q - a q t * l q u) *
        (c₁ α' β' γ' : K) ^ (m K * q - b q t * l q u))) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' ^ (k q u) • (↑(a q t) + (b q t) • β') ^ (k q u)) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' ^ (a q t * l q u) • α' ^ (a q t * l q u)) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' ^ (b q t * l q u) • γ' ^ (b q t * l q u)) ≤
    GelfondSchneiderHouse.house (((c₁ α' β' γ' : K) ^ (n K q - 1 - k q u) * (c₁ α' β' γ' : K) ^
        (m K * q - a q t * l q u) *
        (c₁ α' β' γ' : K) ^ (m K * q - b q t * l q u))) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' • (↑(a q t) + (b q t) • β')) ^ (k q u) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' • α') ^ (a q t * l q u) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' • γ') ^ (b q t * l q u) := by
  simp only [nsmul_eq_mul, zsmul_eq_mul, Int.cast_pow, ← mul_pow]
  gcongr <;> exact GelfondSchneiderHouse.house_pow_le _ _
...
lemma house_smul_pow_le_abs :
    GelfondSchneiderHouse.house (((c₁ α' β' γ' : K) ^ (n K q - 1 - k q u) * (c₁ α' β' γ' : K) ^
        ...
        GelfondSchneiderHouse.house (c₁ α' β' γ' • (↑(a q t) + (b q t) • β')) ^ (k q u) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' • α') ^ (a q t * l q u) *
        GelfondSchneiderHouse.house (c₁ α' β' γ' • γ') ^ (b q t * l q u) ≤
    |(((c₁ α' β' γ') ^ (n K q - 1 - k q u) *
        (c₁ α' β' γ') ^ (m K * q - a q t * l q u) *
        (c₁ α' β' γ') ^ (m K * q - b q t * l q u)))| * (|c₁ α' β' γ'| *
        (|(q : ℤ)| * (1 + GelfondSchneiderHouse.house (β')))) ^ (n K q - 1) * (|c₁ α' β' γ'| * GelfondSchneiderHouse.house (α')) ^
        (mTwoMnq K q) * (|c₁ α' β' γ'| * GelfondSchneiderHouse.house (γ')) ^
        (mTwoMnq K q) := by
  have hbd : ∀ x : K, GelfondSchneiderHouse.house (c₁ α' β' γ' • x) ≤ ↑|c₁ α' β' γ'| * GelfondSchneiderHouse.house x :=
    fun _ ↦ by rw [zsmul_eq_mul]; exact (GelfondSchneiderHouse.house_mul_le _ _).trans_eq (by simp)
  gcongr
  · rw [← @GelfondSchneiderHouse.house_intCast K _]; simp
  · refine (pow_le_pow_right₀
        (GelfondSchneiderHouse.one_le_house_of_isIntegral (isIntegral_c₁_smul_a_b_β' _ _ _ q t)
          (smul_ne_zero (c₁_ne_zero α' β' γ')
            (β'_ne_zero α β σ α' β' γ' hirr habc q t))) (k_le_n_sub_one q u)).trans
      (pow_le_pow_left₀ (GelfondSchneiderHouse.house_nonneg _) ((hbd _).trans
        (mul_le_mul_of_nonneg_left ?_ (by positivity))) _)
    rw [show (↑(a q t) + b q t • β' : K) =
          ((a q t : ℤ) : K) + ((b q t : ℤ) : K) * β' by push_cast [nsmul_eq_mul]; rfl,
        mul_one_add (((|(q : ℤ)| : ℤ) : ℝ)) (GelfondSchneiderHouse.house β')]
    refine (GelfondSchneiderHouse.house_add_le _ _).trans (add_le_add ?_ ((GelfondSchneiderHouse.house_mul_le _ _).trans ?_))
    · simp only [GelfondSchneiderHouse.house_intCast, a]
      gcongr
      exact_mod_cast Nat.succ_le_of_lt (finProdFinEquiv.symm t).1.isLt
    · simp only [GelfondSchneiderHouse.house_intCast, b]
      gcongr
      exact_mod_cast Nat.succ_le_of_lt (finProdFinEquiv.symm t).2.isLt
  · exact (pow_le_pow_left₀ (GelfondSchneiderHouse.house_nonneg _) (hbd _) _).trans (pow_le_pow_right₀
      ((GelfondSchneiderHouse.one_le_house_of_isIntegral (isIntegral_c₁α α' β' γ')
...
lemma abs_bound_le_c₂ :
    |(((c₁ α' β' γ') ^ (n K q - 1 - k q u) *
        ...
        (mTwoMnq K q) ≤
    ↑(c₂ α' β' γ') ^ (n K q) *
      (↑|↑q| ^ ((n K q ) - 1) * (1 + GelfondSchneiderHouse.house β') ^ (n K q - 1) *
      GelfondSchneiderHouse.house α' ^ (mTwoMnq K q) *
      GelfondSchneiderHouse.house γ' ^ (mTwoMnq K q)) := by
  calc
    _ = |(c₁ α' β' γ')| ^ (n K q - 1 - k q u) *
          ...
          (↑|↑q| ^ ((n K q) - 1) * (1 + GelfondSchneiderHouse.house β') ^ (n K q - 1) *
          GelfondSchneiderHouse.house α' ^ (mTwoMnq K q) *
          GelfondSchneiderHouse.house γ' ^ (mTwoMnq K q)) := ?_
    _ ≤ ↑(c₂ α' β' γ') ^ (n K q) *
          (↑|↑q| ^ ((n K q ) - 1) * (1 + GelfondSchneiderHouse.house β') ^ (n K q - 1) *
          GelfondSchneiderHouse.house α' ^ (mTwoMnq K q) *
          GelfondSchneiderHouse.house γ' ^ (mTwoMnq K q)) := ?_
...
lemma c₂_bound_le_c₃ :
    ↑(c₂ α' β' γ') ^ (n K q) *
      (↑|↑q| ^ ((n K q ) - 1) * (1 + GelfondSchneiderHouse.house β') ^ (n K q - 1) *
      GelfondSchneiderHouse.house α' ^ (mTwoMnq K q) *
      GelfondSchneiderHouse.house γ' ^ (mTwoMnq K q)) ≤
    c₃ α' β' γ' ^ (n K q : ℝ) *
  ...
  calc
    _ ≤ ↑(c₂ α' β' γ') ^ (n K q) *
          (√(2 * m K) ^ (n K q - 1) *
          √((n K q : ℝ)) ^ ((n K q : ℝ) - 1) *
          ((1 + GelfondSchneiderHouse.house β') ^ (n K q - 1) *
          (GelfondSchneiderHouse.house α' ^ (mTwoMnq K q) *
           GelfondSchneiderHouse.house γ' ^ (mTwoMnq K q)))) := ?_
    _ ≤ ↑(c₂ α' β' γ') ^ (n K q) *
          (√(2 * m K) ^ (n K q) *
          √((n K q : ℝ)) ^ ((n K q : ℝ) - 1) *
          ((1 + GelfondSchneiderHouse.house β') ^ (n K q) *
          (max 1 (GelfondSchneiderHouse.house α' ^ (2 * m K ^ 2) *
            GelfondSchneiderHouse.house γ' ^ (2 * m K ^ 2))) ^ (n K q))) := ?_
    _ = c₃ α' β' γ' ^ (n K q : ℝ) *
  ...
  · gcongr <;> first
      ...
      | (rw [show mTwoMnq K q = 2 * m K ^ 2 * n K q by simp [mTwoMnq]; ring,
            pow_mul (GelfondSchneiderHouse.house α'), pow_mul (GelfondSchneiderHouse.house γ'), ← mul_pow]
...
lemma house_matrixA_le :
    GelfondSchneiderHouse.house ((algebraMap (𝓞 K) K) ((A α' β' γ' q) u t)) ≤
  ...
  have h₂ := @GelfondSchneiderHouse.house_prod_le_smul_pow K _ α' β' γ' _ q u t
...
abbrev η : Fin (q * q) → 𝓞 K :=
  (GelfondSchneiderHouse.house.exists_ne_zero_int_vec_house_le K (A α' β' γ' q)
...
/-- A real-valued bounding constant used to bound the norm (GelfondSchneiderHouse.house) of the
solution vector `η`. -/
def c₄ : ℝ := (max 1 ((GelfondSchneiderHouse.house.c₁ K) * GelfondSchneiderHouse.house.c₁ K * 2 * (m K))) * (c₃ α' β' γ' : ℝ)

include hq0 h2mq in
open GelfondSchneiderHouse.house in
lemma η_spec (t : Fin (q * q)) :
    GelfondSchneiderHouse.house (algebraMap (𝓞 K) K
        (η (K := K) α β σ α' β' γ' hirr htriv habc q hq0 h2mq t)) ≤
      GelfondSchneiderHouse.house.c₁ K * (GelfondSchneiderHouse.house.c₁ K * ↑(q * q) *
        (c₃ α' β' γ' ^ (n K q : ℝ) *
          (n K q : ℝ) ^ (((n K q : ℝ) - 1) / 2))) ^
        ((m K * n K q : ℝ) / (↑(q * q : ℝ) - ↑(m K * n K q))) := by
  dsimp [η]
  exact_mod_cast (Exists.choose_spec (GelfondSchneiderHouse.house.exists_ne_zero_int_vec_house_le K (A α' β' γ' q)
    (A_ne_zero α β σ α' β' γ' hirr htriv habc q hq0 h2mq)
    (Nat.mul_pos (one_le_m K) (n_one_le q hq0 h2mq))
    ((mul_assoc 2 _ _).symm ▸ lt_mul_of_one_lt_left
      (Nat.mul_pos (one_le_m K) (n_one_le q hq0 h2mq)) Nat.one_lt_two
...
lemma house_eta_le_c₄_pow :
  ...
  calc _ ≤ GelfondSchneiderHouse.house.c₁ K * (GelfondSchneiderHouse.house.c₁ K * ↑(q * q) *
            (c₃ α' β' γ' ^ (n K q : ℝ) *
            (n K q : ℝ) ^ (((n K q : ℝ) - 1) / 2))) ^
            ((m K * n K q : ℝ) /
              (↑(q * q : ℝ) - ↑(m K * n K q))) := ?_
      _ = GelfondSchneiderHouse.house.c₁ K * (GelfondSchneiderHouse.house.c₁ K * 2 * m K *
  ...
  · rw [show ((↑(q * q : ℝ) : ℝ) - ↑(m K * n K q)) =
    ...
    push_cast; linear_combination
      (GelfondSchneiderHouse.house.c₁ K ^ 2 * c₃ α' β' γ' ^ (n K q : ℝ) *
