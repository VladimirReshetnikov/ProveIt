...
\item The active corpus has 30 validated \texttt{LOCAL-NON-FOURIER-HAT:}
      marker/declaration pairs representing 30 distinct document-local command names
      (re-derived on 2026-09-03 from the audit's registry after the transseries
      consolidation retired four document-local hats whose macros no longer exist;
      the register held 34 rows before that retirement, so the previously stated
      35 was already stale by one).
