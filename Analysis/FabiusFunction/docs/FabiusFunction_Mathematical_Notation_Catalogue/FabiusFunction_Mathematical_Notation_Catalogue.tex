...
% Major catalogue divisions always open on a fresh page.  Besides improving
% navigation, this keeps the running section mark faithful to the page's
% opening content rather than to a section introduced near its foot.
\let\cataloguesection\section
\renewcommand{\section}{\clearpage\cataloguesection}
...
\entryfield{Lean crosswalk}{\nolinkurl{primePowerChoose_padicValNat_add} and
\nolinkurl{primePowerChoose_padicValNat} in
\nolinkurl{PrimePowerBinomialValuation.lean} prove the arbitrary-prime identities;
\nolinkurl{twoPowChoose_padicValNat} is the strict dyadic specialization.  Lean's
...
\nolinkurl{legendreGaunt_eq_two_mul_wignerThreeJZeroSqRat}, and
\nolinkurl{legendreProductLinearizationCoeffRat_eq_mul_wignerThreeJZeroSqRat}.
...
\entryfield{Lean crosswalk}{The displayed rule is
\nolinkurl{integral_polynomial_mul_rvachevUp_eq_dyadic_tsum} in
\modulepath{PolynomialCombExactness.lean}; its normalized source identity is
\nolinkurl{tsum_shifted_polynomial_eq_integral}.  Both retain the bounded-Fabius and
...
coefficient/deconvolution, full-interpolation, and row-sum results are its named
...
\nolinkurl{sum_Ioo_lagrangeRvachevAtomCoefficient_mul_shifted_rvachevUp}, and
\nolinkurl{sum_lagrangeRvachevDecoder_eq_one} theorems.  Lean's definitions remain total at
...
and 33 distinct command names; the two Bailey-pair commands are each declared in two
...
\nolinkurl{centeredSincProduct_eq_rvachevFourierProduct} & The bridge is
\(\operatorname{centeredSincProduct}(z)=
\operatorname{rvachevFourierProduct}(z/(2\pi))\).  Identifying the product with the
integral transform uses \nolinkurl{rvachevFourier_eq_product} and retains
...
\modulepath{drafts/exponents-and-q-series/} & 11 & 16 & 27\\
...
\item The active corpus has 35 validated \texttt{LOCAL-NON-FOURIER-HAT:}
      marker/declaration pairs representing 33 distinct document-local command names.
