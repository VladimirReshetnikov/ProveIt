...
\small Active corpus at the validation snapshot: 162 recursively inventoried
\texttt{.tex} sources}
...
current 162-source active \texttt{.tex} corpus.  Each entry states its type, domain,
...
\commandname{GeometricBernoulliPolynomial}\texttt{\{n\}\{q\}} &
\(\GeometricBernoulliPolynomial{n}{q}(x)\) & Monic Appell polynomial that
inverts averaging by the geometric uniform convolution; the evaluation
argument follows the command.\\
...
$(-1)^{\BinaryDigitSum(n)}$.\\
\commandname{PadicValuation}\texttt{\{p\}} & $\PadicValuation{p}$ & Finite
$p$-adic valuation on nonzero integers or rationals; the operand domain determines the
codomain $\NaturalNumbers$ or $\IntegerNumbers$.  Zero is undefined unless an explicitly
named extended or software-totalized convention is in force.\\
\commandname{TwoAdicValuation} & $\TwoAdicValuation$ & The specialization
$\PadicValuation{2}$, with the same operand-domain and zero-convention requirements.\\
...
\commandname{GeneralizedQPochhammer}\texttt{\{a\}\{q\}\{\string\nu\}} &
$\GeneralizedQPochhammer{a}{q}{\nu}$ & Integer- or complex-order
$q$-shifted factorial with an explicit logarithm branch and nonvanishing
denominator when the order is not a natural number.\\
\commandname{GaussianBinomial}\texttt{\{n\}\{k\}\{q\}} &
$\GaussianBinomial{n}{k}{q}$ & Gaussian binomial coefficient with explicit base.\\
\commandname{GeneralizedGaussianBinomial}\texttt{\{\string\alpha\}
\{\string\beta\}\{q\}} &
$\GeneralizedGaussianBinomial{\alpha}{\beta}{q}$ & Generalized Gaussian
coefficient in the explicitly declared finite-product/rational,
branch-dependent, or meromorphic regime.\\
\commandname{QMultinomial}\texttt{\{n\}\{n\string_1,\string\ldots,n\string_r\}\{q\}} &
$\QMultinomial{n}{n_1,\ldots,n_r}{q}$ & Gaussian multinomial coefficient;
the complete lower list and base are explicit.\\
\commandname{QInteger}\texttt{\{n\}\{q\}} & $\QInteger{n}{q}$ & $q$-integer with
explicit base.\\
\commandname{GeneralizedQNumber}\texttt{\{z\}\{q\}} &
$\GeneralizedQNumber{z}{q}$ & Branch-dependent generalized $q$-number for
a declared scalar or complex exponent.\\
...
$\mu,\nu$.}
\entryfield{Document-local dyadic uniform laws}{In the New Frontiers report,
\commandname{DyadicUniformLaw}\texttt{\{n\}} has TeX arity one and prints
$\mathsf U_n^{\mathrm{dyad}}$.  Its mathematical argument is
$n\in\NaturalNumbers$, and it denotes the probability measure
\[
 \mathsf U_n^{\mathrm{dyad}}
 \DefinitionEquals
 \operatorname{Unif}\!\left[-2^{-n-1},2^{-n-1}\right],
 \qquad
 \frac{\Differential\mathsf U_n^{\mathrm{dyad}}}{\Differential x}
 =2^n\IndicatorOf{[-2^{-n-1},2^{-n-1}]}(x).
\]
The interval has length $2^{-n}$, so the displayed density has total mass one;
endpoint values of the density are immaterial.  If the coordinate variables are
independent with these laws, the finite convolution
$\mathsf U_0^{\mathrm{dyad}}\Convolution\cdots\Convolution
\mathsf U_N^{\mathrm{dyad}}$ is their sum law and converges weakly as
$N\to\infty$ to the Rvachev probability law, supported on $[-1,1]$.  This local
family is a sequence of measures, not a valuation or an unnamed $U$-variable.}
\entryfield{Document-local reciprocal-integer law}{In the reciprocal-integer-divisor
report, \commandname{ReciprocalIntegerLaw}\texttt{\{M\}} has TeX arity one and
prints $\mathsf R_M^{\mathrm{law}}$.  Fix an integer $M\ge2$, let $D_M$ be uniform on
the explicitly listed $M$-point set
\[
 \SetOf{(M-1-2j)/M:j\in\NaturalNumbers,\ 0\le j<M},
\]
let $(D_{M,n})_{n\in\NaturalNumbers}$ be independent copies of $D_M$, and define
\[
 Y_M\DefinitionEquals\sum_{n=0}^{\infty}\frac{D_{M,n}}{2^{n+1}},
 \qquad
 \mathsf R_M^{\mathrm{law}}\DefinitionEquals\LawOf{Y_M}.
\]
The series converges absolutely, the law is symmetric and atomless, and
\[
 \SupportOf{\mathsf R_M^{\mathrm{law}}}
 =\left[-\frac{M-1}{M},\frac{M-1}{M}\right].
\]
Under the report's positive-sign $2\pi$ characteristic-function convention, put
$\Phi(z)=\int\EulerE^{2\pi\ImaginaryUnit zx}\,\mu_{\RvachevUp}(\Differential x)$.
The characteristic function of $\mathsf R_M^{\mathrm{law}}$ is the unique entire
continuation of
\[
...
\]
the quotient notation alone is not used at a common zero before this removable
continuation is specified.  Let $X$ and $X'$ be random variables satisfying
\(\LawOf{X}=\LawOf{X'}=\mu_{\RvachevUp}\), on a coupling for which the pair
\((X,X')\) is independent of \(Y_M\).  Then
$X\EqualInLaw M^{-1}X'+Y_M$; only the independence of $X'$ and $Y_M$ is needed
for this distributional decomposition.  With the boundary definitions
$Y_1=0$, $\mathsf R_1^{\mathrm{law}}=\delta_0$, and $Q_1=1$, the multiplicative
cocycle is, for $M,N\in\PositiveIntegers$,
\[
 \mathsf R_{MN}^{\mathrm{law}}
 =\mathsf R_M^{\mathrm{law}}\Convolution
   (x\mapsto x/M)_{\#}\mathsf R_N^{\mathrm{law}},
 \qquad Q_{MN}(z)=Q_M(z)Q_N(z/M).
\]
These are manuscript-level human-proved claims in that report, not a claimed Lean
formalization.}
\entryfield{Conjectural automatic-scale factor pair}{The automatic-scale report uses
exactly the local pair $\rho_1,\rho_2$ for centered, symmetric, compactly supported
probability measures: centered means $\int x\,\rho_j(\Differential x)=0$, and
symmetric means $(x\mapsto-x)_{\#}\rho_j=\rho_j$, for $j\in\{1,2\}$.  Its
factorization hypothesis is
$\rho_1\Convolution\rho_2=\mu_{\RvachevUp}$.  Each angular-frequency characteristic
function $\phi_j(z)=\int\EulerE^{\ImaginaryUnit zx}\,\rho_j(\Differential x)$ is
assumed entire and in the Cartwright class---that is, of finite exponential type with
$\int_{\RealNumbers}\log^+|\phi_j(x)|/(1+x^2)\,\Differential x<\infty$---and its zeros
are assumed to belong to the zero multiset of the Rvachev characteristic function.
The additional completely-regular-growth hypothesis is part of the stated conjecture,
not a consequence of compact support.  Under those hypotheses the conjecture predicts a
factorization by whole dyadic sinc factors, up to a finite B-spline factor and interchange
of $\rho_1$ and $\rho_2$.  The subscripts enumerate two law factors; they are neither
$2$-adic valuations nor the spectral radii denoted by other local $\rho$-symbols.}
...
\begin{catalogueentry}{base-q-digits}{Base-\(q\) digits, digit sums, digit characters,
and digit power sums}
\entryfield{Digits}{For an integer base $q\ge2$ and $n\in\NaturalNumbers$, the $j$-th
base-$q$ digit is
\[
 d_j(n)\DefinitionEquals\Floor{n/q^{j}}\bmod q\in\{0,\dots,q-1\},\qquad j\in\NaturalNumbers,
\]
so that $n=\sum_{j\ge0}d_j(n)\,q^{j}$.  At $q=2$ this is the bit $b_j(n)$ of the
\hyperlink{notation:binary-weight}{binary-weight entry}.  The base is always written
explicitly; it is never inferred from context.}
\entryfield{Digit sum}{$s_q(n)\DefinitionEquals\sum_{j\ge0}d_j(n)$.  At $q=2$ normative
prose writes $\BinaryDigitSum(n)$, not $s_2(n)$; a document that abbreviates
$s_2\DefinitionEquals\BinaryDigitSum$ must say so once.  These are plain-math
conventions, not shared commands.}
\entryfield{Digit window}{A statement over the block $0\le n<q^{m}$ may use the window
product $\prod_{j<m}w\bigl(d_j(n)\bigr)$ for a digit weight $w$; leading zeros inside
the window are weighted by $w(0)$, which is why identities stated with the window need
no normalisation $w(0)=1$, whereas $s_q$ ignores leading zeros.}
\entryfield{Digit character}{For $\zeta$ in a commutative ring, $u_n\DefinitionEquals
\zeta^{s_q(n)}$.  The recursion $u_{qn+r}=\zeta^{r}u_n$ ($r<q$) and the digit product
$\sum_{n<q^{m}}u_nz^{n}=\prod_{j<m}\sum_{r<q}\zeta^{r}z^{rq^{j}}$ hold for every $\zeta$;
the hypothesis that $\zeta$ be a nontrivial $q$-th root of unity, $\sum_{r<q}\zeta^{r}=0$,
enters only through the vanishing of each factor at $z=1$.  At $q=2$, $\zeta=-1$ gives
$u_n=\ThueMorseSign n$.}
\entryfield{Digit power sums}{$P_d(m)\DefinitionEquals\sum_{n<q^{m}}u_n\,n^{d}$.  Under
$\sum_{r<q}\zeta^{r}=0$, $P_d(m)=0$ for $d<m$ (Prouhet cancellation), and with
$\zeta^{q}=1$ the sharp value is
$(\zeta-1)^{m}P_m(m)=m!\,q^{m(m+1)/2}$, stated division-free so that it holds in any
commutative ring.}
\entryfield{Lean crosswalk}{\lean{digitAt} is $d_j(n)$, \lean{windowWeight} the window
product, \lean{digitPowerSum} is $P_d(m)$; the digit product is
\lean{sum_zeta_pow_digits_sum_mul_pow} and its arbitrary-weight form
\lean{sum_windowWeight_mul_pow} (\modulepath{BaseDigitProduct.lean}); cancellation and the
sharp moment are \lean{digitPowerSum_eq_zero_of_lt} and
\lean{sub_one_pow_mul_digitPowerSum_self} (\modulepath{BaseDigitProuhet.lean}).  The
digit sum itself is Mathlib's \lean{(Nat.digits q n).sum}.}
\end{catalogueentry}
...
empty.}
\entryfield{Rademacher--sine representation}{For every $n\in\NaturalNumbers$,
\[
 \ThueMorseSign n=\prod_{j=0}^{\infty}
 \Sign\Bigl(\sin\frac{\pi(n+\frac12)}{2^{j}}\Bigr),
\]
and the $j$-th factor equals $(-1)^{b_j(n)}$ exactly.  Every factor with $2^{j}>n$ is
$1$, so the product has finite multiplicative support and is asserted without a
convergence hypothesis.  The half-unit shift is essential: the unshifted product
$\prod_{j}\Sign(\sin(\pi n/2^{j}))$ contains the factor $\Sign(\sin\pi n)=0$ at $j=0$
and is identically zero.}
\entryfield{Square wave}{For every $x\in\RealNumbers$,
$\sin(\pi x)=(-1)^{\Floor{x}}\sin(\pi\FractionalPart{x})$; the identity holds at the
integers as well, where both sides vanish.  Off the integers
$\sin(\pi\FractionalPart{x})>0$, hence $\Sign(\sin\pi x)=(-1)^{\Floor{x}}$ there.  At the
Rademacher points $x=(n+\frac12)/2^{j}$ one has $\Floor{x}=\Floor{n/2^{j}}$ and
$\FractionalPart{x}\ne0$.}
...
The Rademacher--sine representation is \lean{thueMorseSign_eq_tprod_sign_sin} with factor
law \lean{sign_sin_rademacherPoint}, and the factored square wave is
\lean{sin_pi_mul_eq_neg_one_zpow_floor}, all in \modulepath{RademacherSine.lean}.}
\end{catalogueentry}

\begin{catalogueentry}{thue-morse-autocorrelation}{Thue--Morse autocorrelations, normalized
and limiting}
\entryfield{Window and dyadic autocorrelation}{For $N,k\in\NaturalNumbers$,
$R_N(k)\DefinitionEquals\sum_{n<N}\ThueMorseSign n\,\ThueMorseSign{n+k}$, and at the dyadic
window $A_m(k)\DefinitionEquals R_{2^{m}}(k)$.  These are integers; $R_N(0)=N$ and
$|R_N(k)|\le N$.}
\entryfield{Exact recursions}{$R_{2K}(2r)=2R_K(r)$ and $R_{2K}(2r+1)=-(R_K(r)+R_K(r+1))$
for every even window; nothing uses that the window is a power of two.}
\entryfield{Normalized and limiting values}{$\eta_m(k)\DefinitionEquals A_m(k)/2^{m}\in[-1,1]$,
so that $\eta_{m+1}(2r)=\eta_m(r)$ and $\eta_{m+1}(2r+1)=-(\eta_m(r)+\eta_m(r+1))/2$
exactly.  The limit $\eta(k)\DefinitionEquals\lim_{m\to\infty}\eta_m(k)$ exists for every
$k$ and is the unique solution of $\eta(0)=1$, $\eta(2r)=\eta(r)$,
$\eta(2r+1)=-(\eta(r)+\eta(r+1))/2$; in particular $\eta(1)=\eta(2)=-\tfrac13$,
$\eta(3)=\tfrac13$, $\eta(5)=0$.  The symbol $\eta$ is document-local to the Thue--Morse
spectral discussion; it is not a shared command and must not be reused for a different
sequence in the same document.}
\entryfield{Lean crosswalk}{\lean{thueMorseWindowAutocorrelation} is $R_N(k)$,
\lean{thueMorseAutocorrelation} is $A_m(k)$ (\modulepath{ThueMorseAutocorrelation.lean});
\lean{normalizedAutocorrelation} is $\eta_m(k)$, \lean{limitingAutocorrelation} is
$\eta(k)$ defined by the recursion, and \lean{tendsto_normalizedAutocorrelation} is the
convergence (\modulepath{ThueMorseAutocorrelationLimit.lean}).}
\end{catalogueentry}

\begin{catalogueentry}{thue-morse-block-sum}{Thue--Morse block sums and the nested dyadic
integral}
\entryfield{Block sum}{For $f:\RealNumbers\to\RealNumbers$, $x,h\in\RealNumbers$ and
$m\in\NaturalNumbers$, the block sum is
$\sum_{n<2^{m}}\ThueMorseSign n\,f(x+nh)$.  It is the $m$-fold dyadic difference
$\prod_{j<m}(I-T_{2^{j}h})f$ evaluated at $x$, with $T_a$ the translation by $a$.}
\entryfield{Nested dyadic integral}{The iterated integral
$\int_0^{h}\int_0^{2h}\cdots\int_0^{2^{m-1}h}g(x+t_0+\cdots+t_{m-1})\,dt_{m-1}\cdots dt_0$
is written with the outermost variable on $[0,h]$ and the step doubling inward; its box
volume is $h^{m}2^{0+1+\cdots+(m-1)}=h^{m}2^{\binom m2}$.  Because the integrand depends
only on the sum of the variables, any other nesting order gives the same value, but the
displayed order is the one the recursion produces.}
\entryfield{Cubature identity}{For $f$ of class $C^{m}$ and every real $x$, $h$, the block
sum equals $(-1)^{m}$ times the nested dyadic integral of $f^{(m)}$; no sign condition on
$h$ is needed for the identity.  Consequences stated with $h\ge0$: the block sum is
nonnegative when $(-1)^{m}f^{(m)}\ge0$ on $[x,x+(2^{m}-1)h]$, and is bounded by
$C\,h^{m}2^{\binom m2}$ when $|f^{(m)}|\le C$ there.}
\entryfield{Lean crosswalk}{\lean{thueMorseBlockSum}, \lean{nestedDyadicIntegral},
\lean{thueMorseBlockSum_eq_nestedDyadicIntegral}, \lean{thueMorseBlockSum_nonneg},
\lean{abs_thueMorseBlockSum_le}, all in \modulepath{ThueMorseCubature.lean}.}
\end{catalogueentry}

\begin{catalogueentry}{thue-morse-iterated-prefix}{Exclusive and iterated inclusive
Thue--Morse prefixes}
\entryfield{Exclusive prefix}{$S(N)\DefinitionEquals\sum_{n=0}^{N-1}\ThueMorseSign n$
for $N\in\NaturalNumbers$, so $S(0)=0$; it is the weight-one case $S_N(1)$ of the
arbitrary prefix in the \hyperlink{notation:dyadic-prefix}{dyadic-prefix entry}.  Pair
cancellation gives $S(2q)=0$ and $S(2q+1)=\ThueMorseSign q$.}
\entryfield{Iterated inclusive prefix}{For $k,n\in\NaturalNumbers$,
\[
 S^{(0)}_n\DefinitionEquals\ThueMorseSign n,
 \qquad
 S^{(k+1)}_n\DefinitionEquals\sum_{j=0}^{n}S^{(k)}_j,
\]
an integer sequence with inclusive upper limit; the superscript is the number of
summations and the subscript the position.  For $k\ge1$,
$S^{(k)}_n=\sum_{j=0}^{n}\binom{n-j+k-1}{k-1}\ThueMorseSign j$, and the ordinary
generating function is $\sum_{n\ge0}S^{(k)}_nz^n=E(z)/(1-z)^{k}$ with
$E(z)=\sum_n\ThueMorseSign n\,z^n=\prod_{j\ge0}(1-z^{2^j})$ in $\IntegerNumbers[[z]]$.
The exclusive prefix is not $S^{(1)}$: $S(N)=S^{(1)}_{N-1}$ for $N\ge1$.}
\entryfield{Dyadic facts}{For $1\le k\le r$ the run $S^{(k)}_{2^r-k+i}=0$, $0\le i<k$,
with $S^{(k)}_{2^r-k-1}=(-1)^{r-k}$ and $S^{(k)}_{2^r}=-1$; and the signed reversal
$S^{(k)}_{2^r-k-1-d}=(-1)^{r-k}S^{(k)}_d$ for $k+d<2^r$.}
\entryfield{Collision}{The Thue--Morse atlas writes these as $S(N)$ and $S^{(k)}_n$ in
Parts~I and~III; the three diagonal-polynomial reports it absorbed wrote
$T^{(h)}_r$, $S^{(\ell)}_r$ and $\sigma_h(r)$ for the same object.  The letter $S$ retains
its other uses (support, Stieltjes transform, random series) under the collision
register; the superscript in parentheses is what marks the iterated prefix.}
\entryfield{Status}{Lean-proved.}
\entryfield{Lean crosswalk}{The definition is \lean{iteratedPrefix}, with arguments in
the order $k$, $n$, and the binomial convolution is
\leanline{iteratedPrefix_convolution}
both in \modulepath{ThueMorsePrefix.lean}, which also holds the dyadic theorems
\leanline{iteratedPrefix_dyadic_zero_run}
\leanline{iteratedPrefix_before_dyadic_run}
\leanline{iteratedPrefix_at_dyadic}
\leanline{iteratedPrefix_dyadic_reverse_window}
The exclusive collapse is the declaration
\leanline{sum_thueMorseSign_range}
of \modulepath{ThueMorseEulerTransform.lean}, and the generating identity below every
dyadic truncation is
\leanline{coeff_thueMorseBlockPolynomial_mul_invOneSubPow_eq_iteratedPrefix}
of \modulepath{ThueMorseGenerating.lean}.}
\end{catalogueentry}

\begin{catalogueentry}{thue-morse-summation-table}{The repeated weighted summation
table \(s_{n,k}\)}
\entryfield{Definition}{For $n,k\in\NaturalNumbers$, the table is defined by the row
$s_{0,k}=S(k)$ (the exclusive prefix) and, for $n\ge1$, the weighted recurrence
$s_{n,k}=\sum_{j=0}^{k-1}(k-j)\,s_{n-1,j}$, so that $s_{n,k}=0$ for $k\le n$.  Row
$n$ is an integer sequence indexed by $k$; the subscripts are always in the order
(row, column).}
\entryfield{Identification}{$s_{n,k}=S^{(2n+1)}_{k-n-1}$ for $k\ge n+1$: row $n$ is the
$(2n+1)$-fold inclusive prefix shifted right by $n+1$ columns.  The row generating
function is $A_n(z)=\sum_ks_{n,k}z^k=z^{n+1}E(z^2)/(1-z)^{2n}=z^{n+1}E(z)/(1-z)^{2n+1}$,
and the shifted array $(s_{n,n+1+r})_{n,r}$ is the proper Riordan array
$\bigl(E(z^2),\,z/(1-z)^2\bigr)$.}
\entryfield{Diagonal coordinate}{$r\DefinitionEquals k-n-1\ge0$ is the offset from the
first nonzero entry of the row; the originating program's ``diagonal'' $d=k-n$ is $r+1$.}
\entryfield{Status}{Frontier-derived (Part~III of the Thue--Morse atlas).  The prefix
side of the identification is Lean-proved through the previous entry; the table
recurrence itself has no Lean definition.}
\entryfield{Scope}{The letters $s_{n,k}$, $A_n(z)$, and $r$ are local to Part~III of the
Thue--Morse atlas and to its retained verification programs.}
\end{catalogueentry}

\begin{catalogueentry}{thue-morse-diagonal-polynomial}{Diagonal polynomials of repeated
Thue--Morse summation}
\entryfield{Definition}{For $r\in\NaturalNumbers$, $D_r\in\RationalNumbers[x]$ is
\[
 D_r(x)\DefinitionEquals\sum_{q=0}^{\Floor{r/2}}
 \ThueMorseSign q\binom{2x+r-2q-1}{r-2q}
 =\CoefficientExtraction{z}{r}\,\frac{E(z^2)}{(1-z)^{2x}},
\]
where $\binom{2x+r-2q-1}{r-2q}=\RisingFactorial{2x}{r-2q}/(r-2q)!$ is a polynomial in
$x$ and the generating function is expanded in $\RationalNumbers[x][[z]]$.  Then
$s_{n,k}=D_{k-n-1}(n)$ for $k\ge n+1$; $D_r$ has exact degree $r$, leading coefficient
$2^r/r!$, and $D_0=1$.}
\entryfield{Normalizations}{Two document-local rescalings carry their own accents.  The
Sheffer normalization is $r!\,D_r(x)$, printed with a wide hat over $D$ by the
document-local command in the
\hyperlink{notation:local-hat-ThueMorseDiagonalSheffer}{accent register}; it is a
Sheffer sequence for the pair $\bigl(E(z^2),\,-2\log(1-z)\bigr)$ and satisfies the
half-step law $D_r(x)-D_r(x-\tfrac12)=D_{r-1}(x)$.  The primitive normalization is
$r!\,D_r(x)/2^{\Ceiling{r/2}}$, printed with a wide tilde over $D$; it is a primitive
polynomial in $\IntegerNumbers[x]$, so the exact common denominator of the coefficients
of $D_r$ is $r!/2^{\min(\TwoAdicValuation(r!),\,\Ceiling{r/2})}$.  Neither accent is a
Fourier transform.}
\entryfield{Attached local symbols}{$G_x(z)\DefinitionEquals\sum_{r\ge0}D_r(x)z^r$
(diagonal generating function); the ruler cumulants
$\lambda_m(x)\DefinitionEquals2x+2-2^{\TwoAdicValuation(m)+1}$ for $m\ge1$, with
$\log G_x(z)=\sum_{m\ge1}\lambda_m(x)z^m/m$; and the systematic nonnegative factor
$\Phi_r(x)\DefinitionEquals\prod(2x-m)$ over those $m\ge0$ with
$r\bmod2^{m+1}\ge2^{m+1}-(m+1)$.  Index letters $r$, $m$, $q$ for the diagonal are
interchangeable in Part~III of the atlas; the polynomial is the same.}
\entryfield{Exceptional cases}{$D_r(m/2)=S^{(m+1)}_r$ for $m\in\NaturalNumbers$, so every
nonnegative half-integer value is an iterated prefix; $D_r(m/2)=0$ if and only if
$r\bmod2^{m+1}\ge2^{m+1}-(m+1)$; every rational zero of $D_r$ lies in
$\tfrac12\IntegerNumbers$.}
\entryfield{Status}{Frontier-derived (Part~III of the Thue--Morse atlas); no Lean
definition of $D_r$ exists.  The half-integer values inherit the Lean-proved prefix
facts.}
\entryfield{Collision}{$D$ elsewhere denotes derivatives, dilations, and dyadic
denominators; the diagonal polynomial always carries the subscript $r$ (or $m$, $q$)
and a polynomial argument.}
\end{catalogueentry}

\begin{catalogueentry}{thue-morse-block-polynomial}{Dyadic block polynomials of iterated
prefixes}
\entryfield{Definition}{For $h\in\PositiveIntegers$,
\[
 Q_h(z)\DefinitionEquals\prod_{j=1}^{h-1}\bigl(1+z+\cdots+z^{2^j-1}\bigr)
 =\sum_{a=0}^{2^h-h-1}q_{h,a}z^a\in\IntegerNumbers[z],
\]
an empty product at $h=1$.  Its coefficients are positive on $0\le a\le2^h-h-1$, the
degree is $L-h-1$ with $L\DefinitionEquals2^h$, the mass is $Q_h(1)=2^{\binom h2}$, and
the maximum coefficient is $M_h\DefinitionEquals2^{\binom{h-1}2}$, attained on the central
plateau of exactly $h$ positions.}
\entryfield{Block law}{$S^{(h)}_{aL+b}=\ThueMorseSign a\,q_{h,b}$ for $0\le b<L$ (with
$q_{h,b}=0$ for $b>L-h-1$): the $h$-fold prefix is a signed repetition of one
nonnegative block.  In row $n$ of the summation table the block is
$B_n(z)\DefinitionEquals Q_{2n+1}(z)$ with coefficients $b_{n,a}$, length $L_n=2^{2n+1}$,
and maximum $M_n=2^{\binom{2n}2}$.}
\entryfield{Approximation layer}{$Q_k$ is the integer approximation polynomial
$p_{k-1}$ of the Lean development, and $q_{k,m}/2^{\binom{k-1}2}$ is the step-approximant
value at the centered cell $m$; for $k=2n+1$ these are the level-$2n$ Rvachev histogram
samples whose corrected dyadic-floor samples converge to $\RvachevUp(2x-1)$ and, after
the substitution $x\mapsto x/2$, to $\FabiusBounded(x)$.}
\entryfield{Status}{Definition, and Lean-proved on the approximation side by the
declarations
\leanline{thueMorseBlockPolynomial_mul_invOneSubPow_eq_approximationPolynomialInt}
\leanline{iteratedPrefix_eq_approximationPolynomial_coeff_all}
\leanline{correctedPrefixCoefficient_eq_stepApproximant_all}
\leanline{correctedPrefixGridSample_tendsto_rvachevUp}
\leanline{correctedPrefixGridSample_tendsto_fabius}
in \modulepath{ThueMorseApproximation.lean}.  The plateau, reflection, complement,
multiplicity and distinct-value statements are frontier-derived in Part~III of the
atlas.}
\entryfield{Collision}{The three absorbed reports wrote $Q_h$, $H_{h-1}$ and
$\mathcal A_h$ for this product; $Q$ elsewhere is the rational field, a Fabius
quantile, or a quadrature rule, and $B_n$ elsewhere is a dyadic index block.  Both
letters carry the order subscript and the variable $z$ when they mean the block.}
\end{catalogueentry}

\begin{catalogueentry}{padic-valuation}{\(p\)-adic and \(2\)-adic valuations}
\entryfield{Nonzero integers}{Fix a prime $p$.  Every nonzero
$n\in\IntegerNumbers$ has a unique factorization
$n=p^ku$ with $k\in\NaturalNumbers$, $u\in\IntegerNumbers$, and $p\nmid u$.
The finite integer valuation is
\[
 \PadicValuation{p}(n)\DefinitionEquals
 k
 =\max\SetOf{r\in\NaturalNumbers:p^r\mid n}
 \in\NaturalNumbers.
\]
The value depends only on $|n|$, so signs contribute no power of $p$.}
\entryfield{Nonzero rationals}{For $x\in\RationalNumbers^\times$ choose any
representation $x=a/b$ with nonzero $a,b\in\IntegerNumbers$ and define
\[
 \PadicValuation{p}(x)
 \DefinitionEquals
 \PadicValuation{p}(a)-\PadicValuation{p}(b)
 \in\IntegerNumbers.
\]
This integer is independent of the chosen fraction: multiplying numerator and
denominator by the same nonzero integer adds and subtracts the same valuation.
Equivalently, $x=p^ku/v$ for a unique $k\in\IntegerNumbers$ after requiring
$u,v\in\IntegerNumbers\setminus\{0\}$ and $p\nmid uv$.  Negative values therefore
record powers of $p$ in the denominator.}
\entryfield{Algebraic laws}{For nonzero rationals $x,y$,
\[
 \PadicValuation{p}(xy)=\PadicValuation{p}(x)+\PadicValuation{p}(y),
 \qquad
 \PadicValuation{p}(x/y)=\PadicValuation{p}(x)-\PadicValuation{p}(y).
\]
If also $x+y\ne0$, then
$\PadicValuation{p}(x+y)\ge
\min\{\PadicValuation{p}(x),\PadicValuation{p}(y)\}$, with equality when the two
valuations differ.}
\entryfield{Zero conventions}{The classical finite maps above have domains
$\IntegerNumbers\setminus\{0\}$ and $\RationalNumbers^\times$; their value at zero is
undefined.  An explicitly extended valuation instead has codomain
$\IntegerNumbers\cup\{+\infty\}$ and is defined by
$\overline{\PadicValuation{p}}(0)=+\infty$, with $k<+\infty$ and
$k+(+\infty)=+\infty$ for every $k\in\IntegerNumbers$.  A finite value at zero is
never inferred from the printed glyph: it belongs only to a named software API or another
locally declared totalization.}
\entryfield{Two-adic specialization}{$\TwoAdicValuation(x)=\PadicValuation{2}(x)$ on
the declared nonzero integer or rational domain.  For a nonzero integer,
$n=2^{\TwoAdicValuation(n)}n_{\mathrm{odd}}$ with $n_{\mathrm{odd}}$ odd and
$\TwoAdicValuation(n)\in\NaturalNumbers$.  On rationals its codomain is
$\IntegerNumbers$; for example,
$\TwoAdicValuation(3/8)=-3$ and $\TwoAdicValuation(-12)=2$.}
...
Mathlib's three relevant functions are distinct totalized APIs:
\(\lean{padicValNat}:\NaturalNumbers\to\NaturalNumbers\),
\(\lean{padicValInt}:\IntegerNumbers\to\NaturalNumbers\), and
\(\lean{padicValRat}:\RationalNumbers\to\IntegerNumbers\) after the prime parameter
has been supplied.  Each assigns zero the finite value \(0\); on nonzero inputs its
codomain agrees with the corresponding finite valuation above.  Thus a theorem using one
of these Lean names does not use the \(+\infty\) convention, and the three codomains are not
interchangeable.  Status of the displayed project identities: \textbf{Lean-proved}.}
\entryfield{Scoped historical spelling}{The source-faithful transcription
\modulepath{docs/papers/arXiv-1702.06487v3/157-Arithmetic-v3.tex} carries the explicit
\texttt{HISTORICAL-NOTATION-SCOPE:} marker and retains the raw source form
\texttt{\textbackslash nu\_2(r)}.  In that paper
the operand is a nonzero rational number and the value is the exponent of \(2\) in its
prime factorization, hence an element of \(\IntegerNumbers\) that may be negative.  That
spelling is interpretive historical evidence only; active normative prose uses
\commandname{TwoAdicValuation} and states its domain and zero convention.}
...
\entryfield{Document-local Bell block multiplicity}{In the q-Pochhammer and
q-binomial monograph, \commandname{BellBlockMultiplicity}\texttt{\{r\}} has TeX
arity one and prints $m_r^{\mathrm{Bell}}$.  For a fixed
$n\in\NaturalNumbers$, the complete-Bell summation variable is the entire vector
\[
 (m_r^{\mathrm{Bell}})_{1\le r\le n}\in\NaturalNumbers^n,
 \qquad
 \sum_{r=1}^{n}r\,m_r^{\mathrm{Bell}}=n.
\]
Here $m_r^{\mathrm{Bell}}$ is the number of size-$r$ blocks, or equivalently the
multiplicity with which the $r$th cumulant input is selected.  In the monograph's
Fabius-value specialization its contribution is
\[
 \prod_{r=1}^{n}
 \frac{1}{m_r^{\mathrm{Bell}}!}
 \left(\frac{\kappa_r}{r!}\right)^{m_r^{\mathrm{Bell}}}.
\]
For $n=0$ the index set has the unique empty vector in $\NaturalNumbers^0$, the
weighted sum is zero, and the empty product is one.  The semantic superscript prevents
this nonnegative multiplicity from being mistaken for a valuation or an unrestricted
integer index; the command is local to that monograph and is not part of the shared API.}
...
\begin{catalogueentry}{geometric-bernoulli-appell-family}
  {Geometric Bernoulli polynomials for geometric uniform convolution}
\entryfield{Probability data}{Fix \(q\in(0,1)\), put \(a=1-q\), and let
\(Z_0,Z_1,\ldots\) be independent random variables, each uniformly
distributed on \([0,1]\).  Define
\[
 V_q\DefinitionEquals a\sum_{j=0}^{\infty}q^jZ_j.
\]
The series converges absolutely almost surely, satisfies \(0\le V_q\le1\),
and has total deterministic weight
\(a\sum_{j\ge0}q^j=1\).  Its moment-generating function is the entire
function
\[
 M_q(t)\DefinitionEquals\Expectation\EulerE^{tV_q}
 =\prod_{j=0}^{\infty}
   \frac{\EulerE^{a q^jt}-1}{a q^jt},
\]
where every factor has its removable value \(1\) at \(t=0\).}
\entryfield{Defining exponential generating function}{For
\(n\in\NaturalNumbers\) and \(x\in\ComplexNumbers\), the canonical symbol
\(\GeometricBernoulliPolynomial{n}{q}(x)\) is defined by
\[
 \frac{\EulerE^{xt}}{M_q(t)}
 =\sum_{n=0}^{\infty}
   \GeometricBernoulliPolynomial{n}{q}(x)\frac{t^n}{n!}.
\]
The identity may be read analytically near \(t=0\), where \(M_q(0)=1\), or
formally in \(\ComplexNumbers[x][[t]]\).  It defines a monic polynomial of
degree \(n\), with
\(\GeometricBernoulliPolynomial{0}{q}(x)=1\).  The command arguments are the
degree \(n\) and deformation parameter \(q\); the polynomial evaluation
argument \(x\) follows the command.}
\entryfield{Appell and averaging-inverse laws}{For \(n\ge1\),
\[
 \frac{\Differential}{\Differential x}
 \GeometricBernoulliPolynomial{n}{q}(x)
 =n\GeometricBernoulliPolynomial{n-1}{q}(x).
\]
For every \(n\in\NaturalNumbers\),
\[
 \Expectation\,
 \GeometricBernoulliPolynomial{n}{q}(x+V_q)=x^n.
\]
Thus this family is the polynomial inverse of the averaging operator
\(T_qp(x)=\Expectation p(x+V_q)\); it is not a Bell-polynomial family.}
\entryfield{Difference and reflection laws}{The product decomposition
\(M_q(t)=((\EulerE^{at}-1)/(at))M_q(qt)\) gives, for \(n\ge1\),
\[
 \GeometricBernoulliPolynomial{n}{q}(x+a)
 -\GeometricBernoulliPolynomial{n}{q}(x)
 =anq^{n-1}\GeometricBernoulliPolynomial{n-1}{q}(x/q).
\]
Since \(V_q\) and \(1-V_q\) have the same distribution,
\[
 \GeometricBernoulliPolynomial{n}{q}(1-x)
 =(-1)^n\GeometricBernoulliPolynomial{n}{q}(x).
\]}
\entryfield{Cumulant representation}{Write the cumulant expansion as
\[
 \log M_q(t)=\sum_{j\ge1}\kappa_j(V_q)\frac{t^j}{j!}.
\]
Then
\[
 \GeometricBernoulliPolynomial{n}{q}(x)
 =\ExponentialCompleteBellPolynomial{n}
   \bigl(x-\kappa_1(V_q),-\kappa_2(V_q),\ldots,-\kappa_n(V_q)\bigr),
 \qquad \kappa_1(V_q)=\tfrac12.
\]
This formula uses a complete exponential Bell polynomial to represent the
Appell family; it does not identify the two families.}
\entryfield{Classical limit}{As \(q\downarrow0\), \(a\to1\),
\(V_q\) converges to one uniform random variable, and
\[
 \frac{\EulerE^{xt}}{M_q(t)}
 \longrightarrow \frac{t\EulerE^{xt}}{\EulerE^t-1}.
\]
Consequently
\(\GeometricBernoulliPolynomial{n}{q}(x)\) converges coefficientwise to the
classical Bernoulli polynomial \(B_n(x)\) in this generating-function
convention.  The endpoint \(q=0\) is a limit statement, not part of the
declared parameter domain \((0,1)\).}
\end{catalogueentry}
...
\begin{catalogueentry}{restricted-and-extended-combinatorial-families}
  {Roman Stirling extensions, restricted partitions, Fubini numbers, Narayana
   numbers, and generalized factorial coefficients}
\entryfield{Why the qualifiers are structural}{Each family in this entry
changes either the index domain, the admissible blocks, the order carried by
blocks, or the coefficient basis.  Its decoration is therefore mathematical
data.  None is an unqualified Stirling number, Bell number, Catalan number, or
Gaussian binomial coefficient.}
\entryfield{Roman Stirling extension}{The ordinary combinatorial triangles in
\notelink{stirling-and-lah-families}{the Stirling-and-Lah entry} have
nonnegative indices.  A report that
explicitly announces the \emph{Roman extension} instead continues the
triangular recurrences to integer indices and imposes the dualities
\[
 \UnsignedStirlingFirstKind{n}{k}
 =\StirlingSecondKind{-k}{-n},
 \qquad
 \StirlingSecondKind{n}{k}
 =\UnsignedStirlingFirstKind{-k}{-n}.
\]
These are defining equations for a reciprocal integer-indexed array.  When an
index is negative, neither side retains its permutation-cycle or set-partition
counting interpretation.  In the mixed quadrant \(n\ge1\), \(k\ge0\), the
reports use
\[
 \UnsignedStirlingFirstKind{-n}{k}
 =\frac1{n!}\sum_{j=1}^{n}
   (-1)^{n-j}\binom{n}{j}\frac1{j^k}.
\]
The phrase \emph{Roman extension} must occur in the same scope; negative
arguments do not silently change the default domain of the shared Stirling
commands.}
\entryfield{Negative-index Bell values}{Only after declaring that Roman
extension, the reports define, for \(k\in\PositiveIntegers\),
\[
 \BellNumber{-k}
 \DefinitionEquals
 \sum_{n\ge1}\UnsignedStirlingFirstKind{-n}{k}
 =\EulerE^{-1}\sum_{j\ge1}\frac1{j^k j!}.
\]
Both series converge absolutely.  This analytically continued sequence is not
the cardinality of partitions of a negative-size set, and it is not obtained
by substituting a negative integer into the Dobinski formula without the
displayed definition.}
\entryfield{\(r\)-Stirling numbers of the second kind}{For fixed
\(r\in\NaturalNumbers\),
\[
 \StirlingSecondKind{n}{k}_{\!r}
\]
counts partitions of \(\{1,\ldots,n\}\) into \(k\) nonempty blocks such that
the distinguished labels \(1,\ldots,r\) lie in pairwise distinct blocks.
For \(n,k\in\NaturalNumbers\), its nonzero counting domain is
\(n\ge r\) and \(r\le k\le n\); its boundary value is
\(\StirlingSecondKind{r}{r}_{\!r}=1\); and it is zero outside that domain,
including when \(n<r\), \(k<r\), or \(k>n\).  For \(n>r\),
\[
 \StirlingSecondKind{n}{k}_{\!r}
 =k\StirlingSecondKind{n-1}{k}_{\!r}
  +\StirlingSecondKind{n-1}{k-1}_{\!r}.
\]
The mixed bivariate EGF, including its shift by \(r\), is
\[
 \sum_{n\ge r}\sum_{k\ge r}
 \StirlingSecondKind{n}{k}_{\!r}
 y^k\frac{z^{n-r}}{(n-r)!}
 =y^r\EulerE^{rz}\exp\!\bigl(y(\EulerE^z-1)\bigr).
\]}
\entryfield{Associated Stirling numbers}{For \(r\in\PositiveIntegers\),
\(\mathsf{S}^{\ge r}(n,k)\) counts partitions of an \(n\)-element labelled
set into \(k\) blocks, every block having size at least \(r\).  Set
\(\mathsf{S}^{\ge r}(0,0)=1\); it is zero if \(n<rk\), if \(k<0\), if
\(n<0\), or if \(k=0<n\).  Thus the empty set has one zero-block partition,
whereas a nonempty set has none.  For \(n\ge0\),
\[
 \mathsf{S}^{\ge r}(n+1,k)
 =k\mathsf{S}^{\ge r}(n,k)
  +\binom{n}{r-1}\mathsf{S}^{\ge r}(n-r+1,k-1),
\]
where the zero extension interprets every out-of-domain term.  Equivalently,
\[
 \sum_{n\ge0}\mathsf{S}^{\ge r}(n,k)\frac{z^n}{n!}
 =\frac1{k!}
  \left(\EulerE^z-\sum_{j=0}^{r-1}\frac{z^j}{j!}\right)^k.
\]
The historical source glyph \(\mathsf{S}_r(n,k)\) is read with this
minimum-block-size meaning only inside its declared section.}
\entryfield{Reduced or gap Stirling numbers}{For \(d\in\PositiveIntegers\),
\(\mathsf{S}^{[\mathrm{gap}\,d]}(n,k)\) counts partitions of
\(\{1,\ldots,n\}\) into \(k\) blocks in which two labels in the same block
differ by at least \(d\).  In the source range \(n\ge k\ge d\),
\[
 \mathsf{S}^{[\mathrm{gap}\,d]}(n,k)
 =\StirlingSecondKind{n-d+1}{k-d+1}.
\]
The \(d=1\) specialization is the ordinary second-kind number.  Outside the
stated range the combinatorial definition, not the displayed shifted formula,
governs; no additional boundary values are inferred.}
\entryfield{Fubini or ordered Bell numbers}{The visibly qualified number
\(\mathsf{F}^{\mathrm{Fub}}_n\) counts linearly ordered lists of the blocks of
a set partition.  For \(n\in\NaturalNumbers\),
\[
 \mathsf{F}^{\mathrm{Fub}}_n
 =\sum_{k=0}^{n}k!\StirlingSecondKind{n}{k},
 \qquad
 \sum_{n\ge0}\mathsf{F}^{\mathrm{Fub}}_n\frac{z^n}{n!}
 =\frac1{2-\EulerE^z},
\]
and
\[
 \mathsf{F}^{\mathrm{Fub}}_0=1,
 \qquad
 \mathsf{F}^{\mathrm{Fub}}_n
 =\sum_{j=1}^{n}\binom{n}{j}\mathsf{F}^{\mathrm{Fub}}_{n-j}
 \quad(n\ge1).
\]
It is not the unlabelled Bell number \(\BellNumber{n}\): ordering the blocks is
the extra structure.}
\entryfield{Narayana numbers}{For \(n\in\PositiveIntegers\) and
\(1\le k\le n\), the Narayana number is
\[
 \mathsf{N}^{\mathrm{Nar}}_{n,k}
 \DefinitionEquals
 \frac1n\binom{n}{k}\binom{n}{k-1}.
\]
It counts Dyck paths of semilength \(n\) with exactly \(k\) peaks and satisfies
\[
 \sum_{k=1}^{n}\mathsf{N}^{\mathrm{Nar}}_{n,k}
 =\CatalanNumber{n},
 \qquad
 \mathsf{N}^{\mathrm{Nar}}_{n,k}
 =\mathsf{N}^{\mathrm{Nar}}_{n,n+1-k}.
\]
No value \(\mathsf{N}^{\mathrm{Nar}}_{0,0}\) is introduced by this convention;
an empty Dyck path, when needed, is represented separately by the constant
term of the relevant generating function.}
\entryfield{Generalized factorial coefficients}{Let
\(f\colon\NaturalNumbers\to\ComplexNumbers\), let
\(x,t\in\ComplexNumbers\) with \(t\ne0\), and let \(n\ge1\) be an integer.
The report-local product
\[
 (x)_{n;f,t}
 \DefinitionEquals
 \prod_{j=1}^{n-1}\left(x+\frac{f(j)}{t^j}\right)
\]
has degree \(n-1\), with the \(n=1\) empty product equal to \(1\).  Its
coefficient triangle is
\[
 \genfrac{[}{]}{0pt}{}{n}{k}_{\!f,t}
 \DefinitionEquals
 \CoefficientExtraction{x}{k-1}(x)_{n;f,t},
 \qquad 1\le k\le n.
\]
The four data \(n,k,f,t\) are all part of the object.  These brackets are not
unsigned Stirling brackets, Gaussian binomials, or a \(q\)-Pochhammer symbol,
and no \(n=0\) value is inferred without a separate declaration.}
\end{catalogueentry}
...
\[
 ...
 \DefinitionEquals\frac{1}{\QPochhammer{aq^{-m}}{q}{m}}
 =\frac{(-q/a)^m q^{\binom m2}}{\QPochhammer{q/a}{q}{m}},
\]
...
for integer indices whenever all three factors are defined.  For
\(n\in\NaturalNumbers\),
\(\GeneralizedQPochhammer{a}{q}{n}=\QPochhammer{a}{q}{n}\); the two command
names record whether the governing domain is finite-natural or generalized.}
...
\entryfield{Complex concatenation and unit shift}{With the logarithm branch fixed
as above, and whenever every displayed factor is defined,
\[
...
\]
Both identities use the same chosen value of \(\log q\); changing branches in
only one factor would change the statement.}
...
and is used for a finite-natural length or the literal infinite-product index.
The shared \commandname{GeneralizedQPochhammer} has the same argument order and
visible glyph, but its third argument is a declared integer or complex order
governed by the preceding branch and denominator conditions.}
\end{catalogueentry}
...
\(L_q=\log q<0\).}
\entryfield{Definition}{The shared command
\commandname{GeneralizedQNumber}\texttt{\{z\}\{q\}} prints
\[
 \GeneralizedQNumber{z}{q}\DefinitionEquals\frac{1-q^z}{1-q}.
\]
At \(q=1\), the separately assigned continuous value is
\(\GeneralizedQNumber{z}{1}=z\).  For
\(z=n\in\NaturalNumbers\), this agrees with \(\QInteger n q\).}
\entryfield{The excluded base \(q=0\)}{For an arbitrary complex exponent \(z\),
the branch formula requires \(q\ne0\), and this catalogue assigns no canonical
value to \(0^z\) or to \(\GeneralizedQNumber{z}{0}\).  If
\(z=n\in\NaturalNumbers\), the finite polynomial convention belongs instead to
\commandname{QInteger}: \(\QInteger{0}{0}=0\) and
\(\QInteger{n}{0}=1\) for \(n\ge1\).}
\entryfield{Zeros and branch dependence}{For \(q\ne1\),
\(\GeneralizedQNumber{z}{q}=0\) exactly when
...
lattice.}
\entryfield{Inverse on a chosen branch}{If
\(w=\GeneralizedQNumber{z}{q}\), then
\[
 q^z=1-(1-q)w,\qquad
...
\(q=1\), \(\GeneralizedQNumber{z}{q}\to z\).  A uniform limit over unbounded
\(z\)-sets is not implicit.}
\end{catalogueentry}
...
\begin{catalogueentry}{macmahon-q-catalan}{MacMahon's \(q\)-Catalan polynomial}
\entryfield{Document-local command and type}{The \(q\)-Pochhammer monograph's local
\commandname{MacMahonQCatalanPolynomial}\texttt{\{n\}\{q\}} has TeX arity two and
prints \(C_n^{\mathrm{Mac}}(q)\).  The first argument is
\(n\in\NaturalNumbers\).  The second argument names the displayed polynomial
indeterminate---normally \(q\), or \(X\) when matching the Lean polynomial
variable---rather than supplying a denominator at which pointwise division is to be
performed.  Intrinsically, the family is
\(C_n^{\mathrm{Mac}}(X)\in\IntegerNumbers[X]\); scalar values are obtained afterward by
polynomial evaluation.}
\entryfield{Exact polynomial quotient}{It is the unique polynomial satisfying
\[
 \QInteger{n+1}{q}\,C_n^{\mathrm{Mac}}(q)
 =\GaussianBinomial{2n}{n}{q},
 \qquad
 C_n^{\mathrm{Mac}}(q)
 =\frac{\GaussianBinomial{2n}{n}{q}}{\QInteger{n+1}{q}}
 \quad\hbox{in }\IntegerNumbers[q].
\]
The second display is exact polynomial division: cyclotomic divisibility first proves
that \(\QInteger{n+1}{q}\) divides the central Gaussian polynomial, and the monicity of
the divisor gives the quotient in \(\IntegerNumbers[q]\).  It is not the pointwise
quotient of two scalar-valued functions.  In particular, at a zero of
\(\QInteger{n+1}{q}\), its value is the evaluation of the already constructed quotient
polynomial---the removable value after cancellation---and not an undefined \(0/0\).}
\entryfield{Degree and boundary value}{For every \(n\in\NaturalNumbers\),
\[
 \deg C_n^{\mathrm{Mac}}=n(n-1),
 \qquad C_0^{\mathrm{Mac}}(q)=1.
\]
The degree formula uses natural-number subtraction at \(n=0\).  The explicit boundary
value confirms that the polynomial is the nonzero constant \(1\), so its degree is \(0\)
rather than an exceptional zero-polynomial degree.}
\entryfield{Classical specialization}{Evaluation of the quotient polynomial at \(q=1\)
gives
\[
 C_n^{\mathrm{Mac}}(1)=\CatalanNumber{n}
 =\frac1{n+1}\binom{2n}{n}.
\]
This is polynomial evaluation after cancellation, since the defining exact identity
specializes to
\((n+1)C_n^{\mathrm{Mac}}(1)=\binom{2n}{n}\).}
\entryfield{Coefficient signs}{The coefficients are nonnegative integers, by the
separate classical combinatorial interpretation using an appropriate major-index
statistic on Dyck paths of semilength \(n\).  Integrality from cyclotomic divisibility
alone does not prove nonnegativity.  The current Lean module proves the exact integral
quotient, degree, and specialization at one, but does not prove coefficientwise
nonnegativity; that last assertion therefore has classical/cited, not Lean-proved,
status in this catalogue.}
\entryfield{Collision policy}{The ordinary Catalan number remains
\(\CatalanNumber{n}\).  The bare notation \(C_n(q)\) is not canonical: the literature
contains inequivalent \(q\)-Catalan families, and the corpus also assigns \(C_n\) to
unrelated coefficient sequences.  The semantic superscript and the full local command
identify specifically MacMahon's quotient convention.  This command is local to the
monograph and is intentionally absent from the shared quick-reference and alphabetical
command indexes.}
\entryfield{Lean crosswalk}{In \modulepath{QCatalan.lean},
\lean{qCatalan}~\(n\) is the polynomial in \(\IntegerNumbers[X]\);
\lean{qInt_X_dvd_gaussianBinomial_int} proves the required divisibility,
\lean{qInt_X_mul_qCatalan} is the exact product identity,
\lean{qCatalan_natDegree} proves degree \(n(n-1)\), and
\lean{qCatalan_eval_one} identifies evaluation at one with Mathlib's
\lean{catalan}~\(n\).  These named results have status \textbf{Lean-proved}; none of
them states coefficientwise nonnegativity.}
\end{catalogueentry}

\begin{catalogueentry}{generalized-gaussian-binomial}{Generalized Gaussian binomial
coefficients}
\entryfield{Finite-product/rational definition}{Fix a nonzero complex base
\(q\), a logarithm value \(L_q\), and
\(q^\alpha=\exp(\alpha L_q)\).  For
\(\alpha\in\ComplexNumbers\) and \(k\in\NaturalNumbers\), define
\[
 \GeneralizedGaussianBinomial{\alpha}{k}{q}
 \DefinitionEquals
 \frac{\QPochhammer{q^{\alpha-k+1}}{q}{k}}
      {\QPochhammer{q}{q}{k}},
\]
where the denominator is nonzero.  The shared
\commandname{GeneralizedGaussianBinomial} receives the upper parameter, lower
parameter, and base, in that order.  For an integral upper parameter this is a
rational identity in \(q\); for a nonintegral upper parameter it also depends on
the chosen logarithm branch.}
\entryfield{Integer specialization}{When \(\alpha=n\in\NaturalNumbers\) and
\(k\in\NaturalNumbers\), the generalized expression equals
\(\GaussianBinomial n k q\) for every \(k\), including \(k>n\), where both sides
are the zero polynomial.  At a base where the displayed rational quotient is
formally \(0/0\), this is polynomial evaluation after cancellation, not literal
division.  The out-of-range zero convention is not silently extended to an
arbitrary complex upper parameter.}
\entryfield{Two complex arguments}{For the two-complex-argument definition, fix
\(0<q<1\) and use the real logarithm, so
\(q^z=\exp(z\log q)\).  For
\(\alpha,\beta\in\ComplexNumbers\), define
\[
 \GeneralizedGaussianBinomial{\alpha}{\beta}{q}
 \DefinitionEquals
 \frac{
   \QPochhammer{q^{\beta+1}}{q}{\infty}
   \QPochhammer{q^{\alpha-\beta+1}}{q}{\infty}}
  {\QPochhammer{q}{q}{\infty}
   \QPochhammer{q^{\alpha+1}}{q}{\infty}},
\]
whenever the denominator is nonzero.  Equivalently, using the
\notelink{q-gamma-beta}{catalogue's \(q\)-gamma convention},
\[
 \GeneralizedGaussianBinomial{\alpha}{\beta}{q}
 =\frac{\Gamma_q(\alpha+1)}
 {\Gamma_q(\beta+1)\Gamma_q(\alpha-\beta+1)}
\]
wherever that meromorphic quotient is defined.  This is not a polynomial in
\(q\), and it has no automatic out-of-range zero rule.  When
\(\beta=k\in\NaturalNumbers\), tail cancellation recovers the preceding
...
\[
 ...
 \frac{\QPochhammer{q}{q}{n}}
      {\QPochhammer{q}{q}{n_1}\cdots\QPochhammer{q}{q}{n_r}},
\]
interpreted as its polynomial in \(q\).  For \(r=0\), the constraint forces \(n=0\) and
the value is \(1\); for \(r=1\), the constraint forces \(n_1=n\) and the value is
again \(1\).  The quotient represents an element of \(\IntegerNumbers[q]\), so
evaluation at a root of unity or any other base means polynomial evaluation
after cancellation, not literal division by possibly vanishing factors.  If an
input is negative or the sum constraint fails, the expression is undefined
unless that source separately declares a zero extension.}
\entryfield{Factorization}{For a valid composition,
\[
 \QMultinomial{n}{n_1,\ldots,n_r}{q}
 =\GaussianBinomial{n}{n_1}{q}
  \QMultinomial{n-n_1}{n_2,\ldots,n_r}{q}.
\]
The shared \commandname{QMultinomial} receives \(n\), the complete lower list,
...
\[
 ...
 =\int_0^1 t^{x-1}
   \frac{\QPochhammer{qt}{q}{\infty}}
        {\QPochhammer{q^y t}{q}{\infty}}\,\mathrm d_qt.
\]
...
\entryfield{Lean crosswalk}{In \modulepath{QBetaIntegral.lean}, the total real-valued
definition is \lean{qBeta}~\(q\,x\,y\).  Under the explicit hypotheses
\(0<q<1\) and \(x,y>0\), \lean{qBeta_eq_prod} proves the infinite-product
evaluation, \lean{qBeta_eq_qGamma} proves the displayed gamma quotient,
\lean{qBeta_comm} and \lean{qBeta_pos} prove symmetry and positivity, and
\lean{qBeta_add_one_left} and \lean{qBeta_add_one_right} prove the two positive-domain
recurrences.  The definition is total because Lean's real operations and
\lean{jacksonIntegral} are total; the named evaluation theorems do not silently extend
beyond their positivity hypotheses.  Status: \textbf{Lean-proved with the displayed
hypotheses}.  This module does not formalize the meromorphic continuation or the
classical \(q\uparrow1\) limits.}
...
established.}
\entryfield{Lean crosswalk: carry and divisibility}{In
\modulepath{CyclotomicDivisibility.lean},
\lean{cyclotomic_exponent_eq_one_iff} proves, for \(k\le n\) and \(0<d\), that
the Gaussian cyclotomic exponent is one exactly when
\(n\bmod d<k\bmod d\).  The theorem
\lean{cyclotomic_dvd_gaussianBinomial_iff} proves the corresponding divisibility
criterion for \(\Phi_d\) and the universal Gaussian polynomial over
\(\RationalNumbers[X]\).  These theorems formalize the displayed exponent and carry
criterion, but they do not define the document-local three-argument
\commandname{CyclotomicValuation} as a general Lean function.  Status:
\textbf{Lean-proved with the stated natural-number hypotheses}.}
\entryfield{Lean crosswalk: primitive-root blocks and \(q\)-Lucas}{For a primitive
\(d\)th root \(\zeta\) in an integral domain,
\modulepath{PrimitiveRootBlock.lean} proves the interior vanishing theorem
\lean{gaussianBinomial_isPrimitiveRoot_eq_zero} and the complete block identity
\lean{finiteQPochhammerIn_isPrimitiveRoot}, including the hypothesis \(0<d\):
\[
 \GaussianBinomial{d}{k}{\zeta}=0\quad(0<k<d),
 \qquad
 \prod_{j=0}^{d-1}(1-y\zeta^j)=1-y^d\quad(y\text{ in the coefficient ring}).
\]
In \modulepath{QLucas.lean}, \lean{gaussianBinomial_q_lucas} proves, for
\(a,r,b,s\in\NaturalNumbers\) with \(b,s<d\),
\[
 \GaussianBinomial{ad+b}{rd+s}{\zeta}
 =\binom ar\GaussianBinomial{b}{s}{\zeta}.
\]
Finally, \lean{gaussianBinomial_mul_isPrimitiveRoot} in
\modulepath{CyclotomicDivisibility.lean} specializes this to
\(\GaussianBinomial{an}{bn}{\zeta}=\binom ab\) for a primitive \(n\)th root and
\(0<n\).  All three modules work over the algebraic hypotheses written in their
declarations; they do not by themselves authorize evaluation of an uncancelled rational
\(q\)-series quotient at a root of unity.  Status: \textbf{Lean-proved with the displayed
hypotheses}.}
\end{catalogueentry}
...
\entryfield{Document-local Fabius geometric-Newton coefficient}{In the geometric-comb
synthesis volume, \commandname{FabiusGeometricNewtonCoefficient}
\texttt{\{k\}\{q\}} has TeX arity two and prints
$\mathsf c_k^{\mathrm{F,Newt}}(q)$.  Its mathematical arguments satisfy
$k\in\NaturalNumbers$ and $q\in(0,1)$.  With the bounded Fabius function
$\FabiusBounded$ and the ordered, pairwise distinct nodes
$1,q,\ldots,q^k$, it is the divided difference
\[
 \mathsf c_k^{\mathrm{F,Newt}}(q)
 \DefinitionEquals
 [\FabiusBounded;1,q,\ldots,q^k]
\]
and hence the coefficient of
\[
 N_k(x;1,q)\DefinitionEquals\prod_{r=0}^{k-1}(x-q^r)
\]
in the Newton interpolant.  At $k=0$ the product is empty,
$N_0=1$, and
$\mathsf c_0^{\mathrm{F,Newt}}(q)=\FabiusBounded(1)=1$.
The exact coefficient table in that volume specializes the second argument to
$q=1/2$; retaining $q$ in the command distinguishes the general geometric comb from
this half-base table.  The subscript is a Newton-basis degree, not a valuation, and the
command is document-local rather than a shared Fabius API.}
\entryfield{Generic Newton Lean crosswalk}{In
\modulepath{NewtonInterpolation.lean}, \lean{newtonCoeff} and \lean{newtonPoly}
define the triangular Newton coefficients and their degree-\(n\) polynomial over an
arbitrary field.  The theorem \lean{eval_newtonPoly} proves interpolation at node \(i\)
when the product of its differences from the earlier nodes is nonzero;
\lean{newtonPoly_eq_interpolate} and \lean{newtonCoeff_eq_sum} use injectivity on the
finite node range to identify the Lagrange interpolant and the divided-difference sum.
For geometric nodes, \lean{nodal_range_pow} proves the \(q\)-Pochhammer form of the
Newton basis under \(q\ne0\), \lean{prod_erase_pow_sub_pow} evaluates the nodal
denominator for \(j\le k\), and \lean{newtonCoeff_pow_eq_sum} proves the explicit
coefficient sum under injectivity of \(j\mapsto q^j\) on the required range.
Instantiating the generic data by
\(v(j)=q^j\) and \(y(j)=\FabiusBounded(q^j)\) gives the mathematical object printed by
\commandname{FabiusGeometricNewtonCoefficient}; there is no separate Lean declaration
with that document-local command name.  Lean's definitions remain total when a
denominator vanishes, while the interpolation and divided-difference theorems retain
their nonvanishing or injectivity hypotheses.  Status: \textbf{Lean-proved at the
generic level with the stated hypotheses}.}
...
scope.}
\entryfield{Dyadic-chaos multi-index}{In the dyadic-chaos report,
$V_1,V_2,\ldots$ are independent uniform coordinates on $[-1,1]$, and
$\ell_n(v)=\sqrt{2n+1}\,P_n(v)$ is the orthonormal Legendre basis in
$L^2([-1,1],\Differential v/2)$.  A tensor index is
\[
 \alpha=(\alpha_1,\alpha_2,\ldots),
 \qquad \alpha_j\in\NaturalNumbers\quad(j\ge1).
\]
Its support, total degree, and tensor basis element are, without suppressed bounds,
\[
 \SupportOf{\alpha}\DefinitionEquals\SetOf{j\ge1:\alpha_j>0},
 \qquad
 \CardinalityOf{\SupportOf{\alpha}}<\infty,
 \qquad
 \Norm{\alpha}_1\DefinitionEquals\sum_{j\ge1}\alpha_j,
 \qquad
 \ell_\alpha(V)\DefinitionEquals
 \prod_{j\in\SupportOf{\alpha}}\ell_{\alpha_j}(V_j).
\]
The product is finite; equivalently one may include every $j\ge1$ because
$\ell_0=1$.  The coefficients $c_\alpha(t)$ in
$\EulerE^{tX}=\sum_\alpha c_\alpha(t)\ell_\alpha(V)$ inherit exactly this index
set.  In finite-set monomial expansions the same letter denotes a finite multi-index
$(\alpha_j)_{j\in S}$ and
$|\alpha|=\sum_{j\in S}\alpha_j$, with the displayed summation bounds determining
whether zero components are permitted.  The letter $\alpha$ replaces the former
$\nu$-family so a tensor degree can never be read as a \(2\)-adic valuation.}
...
$Q$ & Rational numbers, Fabius quantile, quadrature rule, polynomial, matrix, a
$q$-dependent quantity, and the dyadic block polynomial $Q_h(z)$ of iterated
Thue--Morse prefixes. & Use $\RationalNumbers$ for the number field and
...
$q$ and always explicit; the block polynomial carries its order subscript and the
variable $z$ (\hyperlink{notation:thue-morse-block-polynomial}{block-polynomial
entry}).\\
...
sequence, domain, and the diagonal polynomial $D_r(x)$ of repeated Thue--Morse
summation. & The upright differential is $\Differential$; derivatives are
$f'$ or $D^nf$ after declaring $D$.  Dilations use $D_a$ or $D_A$, and dyadic
denominators retain the superscript $D_n^{\mathrm{dyad}}$.  The diagonal polynomial
always carries its index subscript and a polynomial argument, and its hatted and
tilde-accented normalizations are declared in the
\hyperlink{notation:thue-morse-diagonal-polynomial}{diagonal-polynomial entry}.\\
...
$S$ & Sum, support, Stieltjes transform, saddle action, spline space, random series,
and the Thue--Morse prefixes $S(N)$ and $S^{(k)}_n$. &
Use $\SupportOf{f}$ for support and $S_\mu$ for Stieltjes transforms.  Saddle actions,
spline spaces, and random series carry semantic subscripts and declared domains.  The
exclusive prefix has a parenthesized argument and the iterated inclusive prefix a
parenthesized order superscript
(\hyperlink{notation:thue-morse-iterated-prefix}{iterated-prefix entry}).\\
...
$X,Y,Z$ & The geometric-uniform random variable in three different affine
normalizations, together with a generic random variable, a state space, and a
spatial coordinate. & The three normalizations are genuinely different random
variables and must never be identified by their letter.  A source using seeds
uniform on $[0,1]$ and writing $X_q=(1-q)\sum_jq^jU_j$ has support $[0,1]$ and
mean $\tfrac12$; a source using seeds uniform on $[-1,1]$ and writing the same
formula obtains twice the centering of the first, with support $[-1,1]$; a
standardized variable divides the centering by $\sigma_q$.  Consequently the
same symbol $Y_q$ denotes the uncentered, the centered, and the doubled centered
variable in three different frontier reports.  Any document using more than one
of them declares the seed interval, the centering, and the scaling at first use,
and states the affine map to the others; the consolidated
\modulepath{geometric_q_fabius_frontiers/} volume fixes one canonical
normalization in sans serif and carries the dictionary as a proved
proposition.\\
$\nu,\nu_j$ & Finite $p$-adic valuation, a signed or probability measure, an integer or
complex order in a generalized q-series, a fractional order, and formerly several local
indexed families. & Valuations use $\PadicValuation{p}$ or $\TwoAdicValuation$ and state
whether the operand is a nonzero integer or rational and whether zero is undefined,
extended to $+\infty$, or software-totalized.  A measure is declared as such and appears
with measurable-set or differential arguments; q-series and fractional orders state their
parameter domains.  The former indexed families now print
$m_r^{\mathrm{Bell}}$, $\mathsf c_k^{\mathrm{F,Newt}}(q)$,
$\mathsf U_n^{\mathrm{dyad}}$, $\alpha$, $\rho_1,\rho_2$, or
$\mathsf R_M^{\mathrm{law}}$ according to their full local definitions.  Outside the
canonical implementation of \commandname{TwoAdicValuation}, no raw
\texttt{\textbackslash nu\_2} remains in active normative prose; the spelling is
permitted only in the explicitly marked source-faithful arithmetic paper described in
the valuation entry.\\
...
in the same scope.\\
$\varphi,\phi,\Phi$ & Characteristic function, standard normal density and
distribution function, a Fourier product in a rescaled frequency, an Euler
totient, and a refinable scaling function. & Use
$\CharacteristicFunction{Z}$ for the characteristic function of a named
variable; the subscript is the variable, not the parameter, unless the source
says otherwise.  A bare $\varphi$ or $\Phi$ with no subscript is the standard
normal density or distribution function only where a source declares it so.
The most dangerous case in the active corpus is a characteristic function of a
\emph{rescaled} variable printed with a parameter subscript: a source writing
$\phi_q(t)$ for the transform of the $[-1,1]$-normalized geometric-uniform
variable means $\CharacteristicFunction{q}(2t)$ in the $[0,1]$ normalization,
and the two are distinguished in print only by $\phi$ against $\varphi$.
Such a source states the argument scaling explicitly; the glyph is not
sufficient qualification.  A totient is $\varphi(n)$ on integer argument, and a
refinable scaling function is declared with its mask.\\
\bottomrule
\end{longtable}
\endgroup

\begin{catalogueentry}{geometric-q-fabius-canonical-namespace}
{The canonical geometric-uniform family of the consolidated \texorpdfstring{$q$}{q}-Fabius volume}
\entryfield{Scope}{Local to the standalone exponents-and-$q$-series volume
\modulepath{geometric_q_fabius_frontiers/}.  The symbols $\mathsf X_q$,
$\mathsf X^{\circ}_q$, and $\mathsf X^{\ast}_q$ are declared in that volume's
Part~0 and are not shared-API commands.  They are typeset in sans serif
precisely so that they cannot be read as any of the roman $X_q$, $Y_q$, $Z_q$
that the volume's twelve absorbed parts use for three different affine images of
the same random variable.}
\entryfield{Formula}{For independent $U_j$ uniform on $[0,1]$ and real
$\AbsoluteValue q<1$,
\[
 \mathsf X_q=(1-q)\sum_{j\ge0}q^{j}U_j,
 \qquad
 \mathsf X^{\circ}_q=\mathsf X_q-\tfrac12,
 \qquad
 \mathsf X^{\ast}_q=\mathsf X^{\circ}_q/\sigma_q,
\]
with $\sigma_q^{2}=(1-q)/\bigl(12(1+q)\bigr)$.}
\entryfield{Normalization}{The weights sum to one, so $\Expectation\mathsf X_q=\tfrac12$
and the support is $[0,1]$ for $0\le q<1$ and
$[-a/(1-a),\,1/(1-a)]$ for $q=-a$ with $0<a<1$.  At $q=\tfrac12$ the distribution
function is $\FabiusBounded$ exactly, which is why this normalization and not one of
the $[-1,1]$ variants is canonical.  The characteristic function of the centered
variable is $\prod_{j\ge0}\SincRad\bigl((1-q)q^{j}t/2\bigr)$.}
\entryfield{Relation to the parts}{A part using seeds uniform on $[-1,1]$ works with
$2\mathsf X^{\circ}_q$, so its cumulants carry an extra $2^{2m}$ and its variance is
$(1-q)/\bigl(3(1+q)\bigr)$; a part working in the standardized scale uses
$\mathsf X^{\ast}_q$, whose normalizing constant $\sqrt{3(1-q^{2})}$ equals
$(1-q)/(2\sigma_q)$.  The volume states the whole dictionary as a proved
proposition rather than leaving it to be read off letter by letter.}
\entryfield{Status}{Definition, with the convergence, support, uniqueness,
transform, and cumulant statements proved in the volume's Part~0.  The $q=\tfrac12$
identification with $\FabiusBounded$ and $\RvachevUp$ is classical.}
\entryfield{Scope end}{The standalone volume.  Outside it the letters $X$, $Y$, $Z$
carry no geometric-uniform meaning without a local declaration, per the collision
register above.}
\end{catalogueentry}
...
% ed.: the three fields below were added on 2026-09-02.  The subscript rule above
% stated only half of the constraint and read as a style preference; the omitted
% half is a build-stopping TeX error that had silently broken six active sources.
\entryfield{Script-final commands: the rule is a build constraint}{Many shared
commands expand to a base atom that already carries a script.  TeX permits at
most one superscript and one subscript per atom, so a directly following script
of a type the atom already carries is the fatal error \texttt{Double
superscript} or \texttt{Double subscript}, not a typographic infelicity.  A
prime counts as a superscript.  \emph{Mixed types are legal}: since
\commandname{NaturalNumbers} expands to \(\NaturalNumbers\), which carries only
a subscript, \(\NaturalNumbers^{2}\) is correct, and so are
\(\FabiusQuantile'\) and \(\SincPi^{(r)}\).  Any audit of this rule that
searches for a command followed by a script, without first asking which script
types that command's trailing atom carries, reports roughly ten false positives
for every real one.}
\entryfield{Classification of the shared commands}{Twenty-seven commands expand
to an atom carrying \emph{both} scripts, so any following \texttt{\textasciicircum},
\texttt{\_} or prime is fatal:
\commandname{AssociahedronByDimension},
\commandname{BellHessenbergMatrixH}, \commandname{BellHessenbergMatrixK},
\commandname{BellNumber}, \commandname{CatalanNumber},
\commandname{ComplementaryBellNumber},
\commandname{ExponentialCompleteBellPolynomial},
\commandname{ExponentialPartialBellPolynomial},
\commandname{GenocchiNumber}, \commandname{GenocchiPolynomial},
\commandname{GeometricBernoulliPolynomial},
\commandname{GregoryCoefficient},
\commandname{InverseFourierAngularOperator},
\commandname{InverseFourierTwoPiOperator}, \commandname{LahNumber},
\commandname{LambertPolynomial}, \commandname{LinearizedDecayObjective},
\commandname{NormalizedReversionCoefficient},
\commandname{OrdinaryGeneratingCoefficient},
\commandname{OrdinaryPartialBellPolynomial},
\commandname{ScaledLambertPolynomial},
\commandname{SecondOrderEulerianPolynomial},
\commandname{TouchardPolynomial}, \commandname{TraceBellArgument},
\commandname{TypeAEulerianPolynomial}, \commandname{TypeBEulerianNumber}, and
\commandname{TypeBEulerianPolynomial}.
Seven are superscript-only and break on a following \texttt{\textasciicircum} or prime:
\commandname{CompositionalInverseOf}, \commandname{MultiplicativeInverseOf},
\commandname{FormalBigO}, \commandname{FallingFactorial},
\commandname{RisingFactorial},
\commandname{ExponentialGeneratingFunctionOf}, and
\commandname{OrdinaryGeneratingFunctionOf}.
The remaining thirty-eight script-final commands are subscript-only and break
only on a following \texttt{\_}.
This classification is derived from the expansions, so it must be recomputed
whenever a command's definition changes rather than trusted as a fixed list.}
\entryfield{Remedy, and document-local aliases}{Prefer the command whose
meaning is the one intended: writing the power-series ring as
\commandname{PolynomialLogPowerSeriesRing} is better than decorating the
Laurent-ring command with a range subscript.  When a genuine extra script is
needed, brace-wrap the call so the script attaches to the whole group, as in
\texttt{\{\textbackslash LambertPolynomial\{n\}\}'}, which is valid and renders as intended.  A
derivative may instead be printed in Leibniz form.

The constraint propagates through \emph{document-local aliases}.  A definition
such as \texttt{\textbackslash newcommand\{\textbackslash PL\}\{\textbackslash PolynomialLogLaurentRing\{\textbackslash K\}\}} makes
\texttt{\textbackslash PL} script-final too, so \texttt{\textbackslash PL\_\{\textbackslash ge0\}} is fatal even though the
shared command never appears with a script beside it.  An audit that inspects
only the shared command names will not see this; alias chains must be resolved
first.}
\end{catalogueentry}
...
\texttt{LOCAL-NON-FOURIER-HAT:} declaration.  There are 32 marked declarations
and 32 distinct command names; each is declared in exactly one active document scope.
...
\subsubsection{Diagonal-law and Riordan Sheffer normalizations}

\Needspace{14\baselineskip}
\paragraph{\localaccentname{ThueMorseDiagonalSheffer}}
\hypertarget{notation:local-hat-ThueMorseDiagonalSheffer}{}%
TeX arity is zero; the command prints a wide hat over \(D\), and the caller
appends the diagonal index as a subscript and the argument in parentheses.
Its mathematical value is the factorial-normalized diagonal polynomial of
the \hyperlink{notation:thue-morse-diagonal-polynomial}{diagonal-polynomial
entry},
\[
 r!\,D_r(x)\in\RationalNumbers[x],\qquad r\in\NaturalNumbers,
\]
the Sheffer sequence for the pair \(\bigl(E(z^2),\,-2\log(1-z)\bigr)\), so that
\(\sum_{r\ge0}r!\,D_r(x)\,z^r/r!=E(z^2)(1-z)^{-2x}\) and the delta operator
\(1-\EulerE^{-\partial_x/2}\) lowers the index by one.  The scope is Part~III of
the Thue--Morse atlas.  The hat means factorial normalization and never a
Fourier transform; the tilde-accented primitive normalization
\(r!\,D_r(x)/2^{\Ceiling{r/2}}\) of the same part is a different
polynomial and is not a hat.

\Needspace{12\baselineskip}
\paragraph{\localaccentname{RiordanSubdiagonalSheffer}}
TeX arity is zero; the command prints a wide hat over \(P\), and the caller
appends the subdiagonal index \(m\) as a subscript.  Its mathematical value is
\(m!\,P_m(x)\), where \(P_m(x)=\CoefficientExtraction{z}{m}\,g(z)\phi(z)^{x}\) is the
formal continuation of the \(m\)-th subdiagonal of a proper Riordan array
\((g(z),\,z\phi(z))\) with \(g(0)\ne0\) and \(\phi(0)\ne0\), as declared in the
Riordan section of Part~III of the Thue--Morse atlas.  The hatted sequence is
Sheffer for \((g,\log\phi)\); at \(g(z)=E(z^2)\), \(\phi(z)=(1-z)^{-2}\) it is
the previous entry.  The scope is that section.  The hat means factorial
normalization and never a Fourier transform.
...
\commandname{vtwo}, \commandname{vTwo}, \commandname{nuTwo},
\commandname{nuu}, \commandname{vtwoPartii}, \commandname{vtwoPartcc} &
\commandname{TwoAdicValuation} & Preserve the operand when replacing any of these
source-local aliases.  A nonzero integer operand gives a value in
$\NaturalNumbers$; a nonzero rational operand gives a value in $\IntegerNumbers$ and may
have negative valuation.  Zero is outside both finite mathematical domains.  If zero can
occur, explicitly select either the extended value $+\infty$ or a named totalized API such
as \lean{padicValNat}, \lean{padicValInt}, or \lean{padicValRat}; all three Lean APIs
assign zero the finite value \(0\), but their codomains differ.\\
...
\commandname{pospart}, \commandname{pos} & \commandname{PositivePart} & Ordered-scalar positive part with
...
\commandname{fall}, \commandname{falling} & \commandname{FallingFactorial} & Supply the base and
nonnegative length as two explicit arguments; the length-zero product is one.\\
\commandname{rise}, \commandname{rising} & \commandname{RisingFactorial} & Supply the base and
nonnegative length as two explicit arguments; the length-zero product is one.\\
\commandname{Lap} & \commandname{LaplaceTransformSymbol} & Bare transform
symbol only; use \commandname{LaplaceTransformOf} when the transformed operand
belongs inside the semantic command.\\
\commandname{Mell} & \commandname{MellinTransformSymbol} & Bare transform
symbol only; use \commandname{MellinTransformOf} when the transformed operand
belongs inside the semantic command.\\
\commandname{Mellin} & \commandname{MellinTransformSymbol} or
\commandname{MellinTransformOf} & Use the symbol command only for a bare operator token
or transform-arrow label.  When the operand is present---whether formerly bracketed,
postfixed, or encoded as a subscript---put that operand inside
\commandname{MellinTransformOf}.\\
\commandname{stirone} & \commandname{UnsignedStirlingFirstKind} & Preserve the
upper and lower triangular indices; this is the unsigned cycle-count triangle.\\
\commandname{stirtwo} & \commandname{StirlingSecondKind} & Preserve the upper
and lower triangular indices; this is the set-partition triangle.\\
\commandname{eulerian} & \commandname{TypeAEulerianNumber} & The legacy
single-angle array is type A, not type B or second order.\\
\commandname{eulertwo} & \commandname{SecondOrderEulerianNumber} & The legacy
double-angle array counts descents of Stirling permutations.\\
\commandname{BellP} & \commandname{ExponentialPartialBellPolynomial} & Supply
the total degree and block/derivative count; retain the exponential factorial
normalization.\\
\commandname{BellC} & \commandname{ExponentialCompleteBellPolynomial} & Supply
the total degree; do not replace it by a Bell number.\\
\commandname{Bell} & \commandname{BellNumber},
\commandname{ExponentialCompleteBellPolynomial},
\commandname{ExponentialPartialBellPolynomial}, or a semantically named
non-Bell family & Inspect the defining equation before using arity: a scalar
indexed Bell sequence is a Bell number, while genuine one- and two-index Bell
polynomial families are complete and partial exponential, respectively.  If
the legacy glyph instead names another object---for example report H's
geometric Bernoulli Appell family---rename that object
\commandname{GeometricBernoulliPolynomial}; do not recast it as a Bell object.\\
\commandname{coeff} & \commandname{CoefficientExtraction} & Supply the
coefficient variable and exponent as the command's two explicit arguments;
the series, polynomial, or other operand follows the command.\\
\commandname{OrdinaryBellPolynomial} &
\commandname{OrdinaryPartialBellPolynomial} & Preserve both indices and the
ordinary-series, rather than exponential-series, normalization.\\
\commandname{OrdinaryBellSeriesOf},
\commandname{OrdinaryGeneratingSeriesOf} &
\commandname{OrdinaryGeneratingFunctionOf} & Supply the coefficient-family
argument and keep the ordinary generating convention explicit.\\
\commandname{NormalizedInverseCoefficient} &
\commandname{NormalizedReversionCoefficient} & Supply both the series family
and coefficient index; this is compositional reversion, not a reciprocal.\\
\commandname{NormalizedOrdinaryCoefficient} &
\commandname{OrdinaryGeneratingCoefficient} & Supply both the coefficient
family and index; this is not a reversion coefficient.\\
\commandname{PosetMinimum}, \commandname{PosetMaximum} &
\commandname{PartitionLatticeMinimum},
\commandname{PartitionLatticeMaximum} & These aliases denoted the discrete and
one-block partitions of a partition lattice, not numeric bounds or endpoints of
an arbitrary poset.\\
\commandname{TracePowerSum} & \commandname{TraceBellArgument} & Preserve the
index and the scaling \(-(k-1)!\TraceOperator(A^k)\), not merely the raw trace.\\
\commandname{cyc} & \commandname{CycleCountOperator} & Permutation cycle count
only; a cyclic group or cycle-index polynomial remains locally typed.\\
\commandname{setpart} & \commandname{SetPartitionOperator} & The set of set
partitions of the following finite-set operand.\\
\commandname{bitand} & \commandname{BitwiseAnd} & Bitwise AND of declared
nonnegative integers, not set intersection or multiplication.\\
\bottomrule
\end{longtable}
\endgroup
...
\(\GeometricBernoulliPolynomial{n}{q}(x)\) &
\commandname{GeometricBernoulliPolynomial} & Geometric-uniform-convolution
Appell family; degree and deformation parameter are explicit.\\
...
$\NaturalNumbers$-valued on nonzero integers and $\IntegerNumbers$-valued on nonzero
rationals.  Zero is undefined unless an extended or named totalized convention is stated.\\
...
\(\GeneralizedQPochhammer{a}{q}{\nu},
  \GeneralizedGaussianBinomial{\alpha}{\beta}{q},
  \GeneralizedQNumber{z}{q}\) &
\commandname{GeneralizedQPochhammer},
\commandname{GeneralizedGaussianBinomial},
\commandname{GeneralizedQNumber} & Generalized \(q\)-families whose integer or complex
order, analytic regime, and branch data are declared locally.\\
\(\QMultinomial{n}{n_1,\ldots,n_r}{q}\) &
\commandname{QMultinomial} & Gaussian multinomial with the complete lower list and base
explicit.\\
...
\commandname{GeneralizedGaussianBinomial} & 3 &
\notelink{generalized-gaussian-binomial}{Generalized Gaussian binomial}.\\
\commandname{GeneralizedQNumber} & 2 &
\notelink{generalized-q-number}{Generalized \(q\)-numbers}.\\
\commandname{GeneralizedQPochhammer} & 3 &
\notelink{q-pochhammer}{Generalized \(q\)-Pochhammer symbols}.\\
...
\commandname{GeometricBernoulliPolynomial} & 2 &
\notelink{geometric-bernoulli-appell-family}{Geometric Bernoulli polynomials}.\\
...
\commandname{QMultinomial} & 3 &
\notelink{q-multinomial-path-area}{\(q\)-multinomial coefficients}.\\
...
$S^{(k)}_n$ & \lean{iteratedPrefix} with \lean{iteratedPrefix_convolution},
\lean{iteratedPrefix_dyadic_zero_run}, and
\lean{iteratedPrefix_dyadic_reverse_window} in \modulepath{ThueMorsePrefix.lean} &
Argument order is (order, position); the generating identity is formal only below each
dyadic truncation (\modulepath{ThueMorseGenerating.lean}).  The summation table
$s_{n,k}$ and the diagonal polynomials $D_r$ built on it have no Lean definition.\\
$Q_k(z)$, $q_{k,m}/2^{\binom{k-1}2}$ &
\lean{approximationPolynomialInt}, \lean{correctedPrefixCoefficient},
\lean{correctedPrefixGridSample},
\lean{iteratedPrefix_eq_}\allowbreak\lean{approximationPolynomial_}\allowbreak\lean{coeff_all},
\lean{correctedPrefixGridSample_}\allowbreak\lean{tendsto_rvachevUp}, and
\lean{correctedPrefixGridSample_}\allowbreak\lean{tendsto_fabius} in
\modulepath{ThueMorseApproximation.lean} &
The block coefficients below the cutoff $m<2^k$ are those of $p_{k-1}$; the
convergence theorems retain \lean{IsFabius} and assert no rate.\\
...
MacMahon's \(q\)-Catalan polynomial & \lean{qCatalan},
\lean{qInt_X_dvd_gaussianBinomial_int}, \lean{qInt_X_mul_qCatalan},
\lean{qCatalan_natDegree}, and \lean{qCatalan_eval_one} in
\modulepath{QCatalan.lean} & An exact quotient in \(\IntegerNumbers[X]\), its product
identity, degree \(n(n-1)\), and evaluation at one are proved.  Coefficientwise
nonnegativity is not a theorem of this module.\\
Jackson \(q\)-beta function & \lean{qBeta}, \lean{qBeta_eq_prod},
\lean{qBeta_eq_qGamma}, \lean{qBeta_comm}, \lean{qBeta_pos},
\lean{qBeta_add_one_left}, and \lean{qBeta_add_one_right} in
\modulepath{QBetaIntegral.lean} & The function is real-valued.  Product, gamma-quotient,
symmetry, positivity, and recurrence theorems retain \(0<q<1\) and \(x,y>0\); no
meromorphic continuation or \(q\uparrow1\) theorem is inferred.\\
Cyclotomic carry, primitive-root blocks, and \(q\)-Lucas &
\lean{cyclotomic_exponent_eq_one_iff} and
\lean{cyclotomic_dvd_gaussianBinomial_iff} in
\modulepath{CyclotomicDivisibility.lean};
\lean{finiteQPochhammerIn_isPrimitiveRoot} in
\modulepath{PrimitiveRootBlock.lean}; \lean{gaussianBinomial_q_lucas} in
\modulepath{QLucas.lean} & The carry criterion is over \(\RationalNumbers[X]\); the
root-block and \(q\)-Lucas theorems assume a primitive root in an integral domain and
retain \(0<d\) and the stated residue bounds.  They do not define a general
\commandname{CyclotomicValuation} function.\\
...
Generic Newton interpolation & \lean{newtonCoeff}, \lean{newtonPoly},
\lean{eval_newtonPoly}, \lean{newtonPoly_eq_interpolate},
\lean{newtonCoeff_eq_sum}, \lean{nodal_range_pow},
\lean{prod_erase_pow_sub_pow}, and \lean{newtonCoeff_pow_eq_sum} in
\modulepath{NewtonInterpolation.lean} & Definitions are total over a field; interpolation
and divided differences retain their product-nonzero or node-injectivity hypotheses.
The geometric formulas do not create a separately named Fabius-specific coefficient.\\
...
The inventory below is the recursive active tree observed on 2026-09-03, after the
consolidation of the six exponents-and-$q$-series drafts into one volume and of the six
polynomial--logarithmic transseries drafts into one volume, and after the arrival of the
special-function-inversion intake.  It excludes every path below \modulepath{docs/archive/} and
counts this catalogue exactly once.  It contains 170 mathematical content sources: 72
standalone documents and 98 included or generated fragments.  The separate shared
contract \modulepath{docs/fabius-notation.tex} raises the total before this catalogue to 171
active TeX files.  Including this standalone catalogue gives 172 active TeX files and
73 standalone documents.  ``Standalone'' means that the uncommented source
...
\modulepath{drafts/combinatorial-coefficient-calculus/} & 1 & 0 & 1\\
\modulepath{drafts/exponents-and-q-series/} & 2 & 20 & 22\\
\modulepath{drafts/frontier-compilations/} & 3 & 1 & 4\\
\modulepath{drafts/integration-and-transforms/} & 1 & 7 & 8\\
\modulepath{drafts/inverse-and-sampling/} & 9 & 30 & 39\\
\modulepath{drafts/lambert-w/} & 1 & 4 & 5\\
\modulepath{drafts/representations/} & 12 & 26 & 38\\
\modulepath{drafts/rvachev_up_fourier_decay/} & 1 & 0 & 1\\
\modulepath{drafts/series-and-transseries/} & 20 & 0 & 20\\
\modulepath{drafts/spectra-and-arithmetic/} & 10 & 10 & 20\\
\modulepath{drafts/thue-morse/} & 1 & 0 & 1\\
\midrule
Mathematical content subtotal & 72 & 98 & 170\\
Shared notation contract & 0 & 1 & 1\\
This catalogue & 1 & 0 & 1\\
\midrule
Active TeX total including catalogue & 73 & 99 & 172\\
\bottomrule
\end{longtable}
\endgroup
...
\begin{itemize}
\item \modulepath{docs/fabius-notation.tex} defines exactly 146 shared commands, not
      counting the version stamp \commandname{FabiusNotationVersion}: the 91 of the
      original contract, the combinatorial coefficient-calculus block, and the
      polynomial--logarithmic transseries block.
\item Every one of those 146 command names occurs in the canonical quick reference and
      in the alphabetical command index, with its exact arity.  This was re-derived on
      2026-09-03 by comparing the \commandname{newcommand} names of the shared file
      against the \commandname{commandname} entries of both sections, not by
...
\item The active corpus has 32 validated \texttt{LOCAL-NON-FOURIER-HAT:}
      marker/declaration pairs representing 32 distinct document-local command names
      (re-derived on 2026-09-05 from the audit's registry: the 30 that remained after
      the transseries consolidation of 2026-09-03 retired four document-local hats,
      plus the two Sheffer-normalization hats declared by Part~III of the Thue--Morse
      atlas when the diagonal-laws companion was folded in).
