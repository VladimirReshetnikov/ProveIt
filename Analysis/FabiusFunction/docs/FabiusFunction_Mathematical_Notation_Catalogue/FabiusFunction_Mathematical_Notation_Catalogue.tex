...
\newcommand{\localaccentname}[1]{\commandname{#1}}
...
denoting this inverse on a stated inversion domain is
\begin{center}
...
signs.  For $r=0$ the empty intersection is $\IntegerNumbers$.  Explicitly,
...
\texttt{LOCAL-NON-FOURIER-HAT:} declaration.  There are 33 marked declarations
and 31 distinct command names; the two Bailey-pair commands are each declared in two
document scopes.  The phrase \emph{prints a wide hat over \(X\)} describes literal output
...
\subsubsection{Bailey-pair transforms}

\paragraph{\localaccentname{BaileyTransformedAlpha}}
TeX arity is zero.  The command prints a wide hat over \(\alpha\); the index
\(n\) is an external source-level subscript rather than a TeX argument.
Let \(a,q\in\ComplexNumbers\) and let
\(\alpha,\beta:\NaturalNumbers\to\ComplexNumbers\) be a Bailey pair
relative to \(a\), so
\[
 \beta_n
 =
...
\]
Both active document scopes define the transformed alpha sequence by
\[
 \alpha_n^{\mathrm B}
 \DefinitionEquals
 a^nq^{n^2}\alpha_n.
\]
The \(q\)-series monograph calls this the alpha component of a Bailey-chain
step and also uses it for the transformed unit pair in the
Rogers--Ramanujan proof.  The proof-oriented article calls the same formula
the alpha component of the weak Bailey-lemma transformed pair and abbreviates
\((z;q)_n\) by \((z)_n\) locally.  Status is an exact finite transformation;
in analytic limiting statements the standing convention is
\(q\in\ComplexNumbers\), \(0<\AbsoluteValue{q}<1\), with all displayed
denominators nonzero.  The source pair, \(a\), \(q\), coefficient field,
denominator hypotheses, and index are suppressed.  The accent means Bailey
transformation, not Fourier transformation.  Appending the single external
subscript is currently valid because the zero-argument expansion itself owns
no script, but it is fragile: no second subscript or simultaneous replacement
superscript may be appended, and any future scripted macro body would require
an indexed wrapper.

\paragraph{\localaccentname{BaileyTransformedBeta}}
TeX arity is zero.  The command prints a wide hat over \(\beta\); the index
\(n\) is appended externally and is not a TeX argument.  For a Bailey pair
\((\alpha,\beta)\) relative to \(a\), with \(a,q\in\ComplexNumbers\), its exact transformed beta component is
\[
 \beta_n^{\mathrm B}
 \DefinitionEquals
 \sum_{j=0}^{n}
...
\]
Together with
\(\alpha_n^{\mathrm B}=a^nq^{n^2}\alpha_n\), this is again a Bailey pair
relative to \(a\).  In the monograph it appears in the Bailey-chain step, in
the limiting weak Bailey lemma through \(\beta_N^{\mathrm B}\), and after
transforming the unit pair, where
\(\beta_n^{\mathrm B}=1/(q;q)_n\).  In the proof-oriented article the same
formula is the beta component of its weak Bailey-lemma corollary, with
\((z)_n=(z;q)_n\) in that local section.  Status is an exact finite
transformation, with convergence assumptions added only when a subsequent
infinite limit is taken.  The input pair, \(a\), \(q\), coefficient field,
nonvanishing denominators, and index are suppressed.  The accent means Bailey
transformation.  The external \texttt{\_n} or \texttt{\_N} convention is safe
only while the zero-argument expansion remains unscripted; callers must not
compose it with a second raw subscript or conflicting superscript.
...
The inventory below is the recursive active tree observed on 2026-08-31 at the
source-validation checkpoint.  It excludes every path below \path{docs/archive/} and
counts this catalogue exactly once.  It contains 160 mathematical content sources: 59
standalone documents and 101 included or generated fragments.  The separate shared
contract \path{docs/fabius-notation.tex} raises the total before this catalogue to 161
active TeX files.  Including this standalone catalogue gives 162 active TeX files and
60 standalone documents.  ``Standalone'' means that the uncommented source
...
\path{drafts/inverse-and-sampling/} & 8 & 37 & 45\\
...
\midrule
Mathematical content subtotal & 59 & 101 & 160\\
Shared notation contract & 0 & 1 & 1\\
This catalogue & 1 & 0 & 1\\
\midrule
Active TeX total including catalogue & 60 & 102 & 162\\
\bottomrule
\end{longtable}
\endgroup
...
\begin{itemize}
\item \path{docs/fabius-notation.tex} defines exactly 91 shared commands.
...
\item The active corpus has 33 validated \texttt{LOCAL-NON-FOURIER-HAT:}
      marker/declaration pairs representing 31 distinct document-local command names.
