...
\hypersetup{
  ...
  citecolor=linkblue,
  urlcolor=linkblue,
  pdftitle={The small-argument asymptotic expansion of the Fabius function},
  pdfsubject={A general formula for every coefficient of the endpoint expansion},
  pdfkeywords={Fabius function, asymptotic expansion, Lambert W, saddle point, Stirling numbers, Gamma-zeta Fourier coefficients}
}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Small-argument asymptotics of the Fabius function}
\fancyhead[R]{\small\nouppercase{\leftmark}}
\fancyfoot[C]{\thepage}
...
\title{\bfseries The Small-Argument Asymptotic Expansion of the Fabius Function\\[0.35em]
\large A general formula for every coefficient}
...
the differential recursion in closed form using signed Stirling numbers of the
first kind and harmonic numbers, collapse the exponent coefficients to a single
...
\end{aligned}
\]
where $c(n,m)=[z^m]\prod_{k=1}^n(z-k)$ is a signed Stirling number of the first
kind, so that $c(n,n)=1$ and $c(n,m)=0$ for $m>n$.
\end{theorem}
...
\item The mean of $A_j$ differs from its ``$\Psi$-free part'' only by mean
  values of products of derivatives of $\Psi$, all of which are of size
  $|\Psihat(1)|^2$ or smaller, hence below $10^{-11}$.  For every practical
  purpose the mean of $A_j$ \emph{is} its $\Psi$-free part.
\end{itemize}
...
\end{aligned}
\end{equation}
The corresponding form of $A_3$ is long; its \emph{linear part} --- the only
part that matters numerically, since products of derivatives of $\Psi$ are
below $10^{-10}$ --- is
...
an explicit Gamma--zeta Fourier object.  Discarding products of derivatives of
$\Psi$, which contribute below $10^{-10}$, the linearized Fourier form of $A_j$
is
\[
...
\[
\begin{aligned}
  \text{mean}&&\langle A_2\rangle&=\frac1{4L^2}=0.5203422452514019,\\
  k=1\ \text{amplitude}&&2|c_2(1)\Psihat(1)|&=1.939878\times10^{-3}
    \quad(\text{peak-to-peak }3.879756\times10^{-3}),\\
  \text{maximum at}&&\{\lambda\}&=0.1011297035,\\
  \text{minimum at}&&\{\lambda\}&=0.6011297035.
\end{aligned}
\]
...
These are estimates from the top term alone.  The exact $k=1$ amplitudes
$2|c_j(1)\Psihat(1)|$, which use the whole of $c_j$, are
$8.73\times10^{-5}$ at $j=1$ --- the top term is the only term there ---
and $1.94\times10^{-3}$ at $j=2$, above the estimate $1.79\times10^{-3}$
because the $w^3$ term of $c_2$ adds in phase.  The gap widens with $j$:
the measured peak-to-peak swings of \cref{sec:numerics} are
$6\times10^{-2}$ at $j=3$ against the estimate $4.9\times10^{-2}$, and
$0.82$ at $j=4$ against $0.50$.  So \cref{eq:amplitude} gives the growth
rate, not the constant.  So the periodic parts of
...
$3\times10^{-3}$; \cref{eq:A2psi} predicts a minimum at $0.6011297$ and a
...
\item Formalize \cref{thm:mass,thm:log} for the generic
  $\operatorname{expCoeff}$ and $\operatorname{logCoeff}$ recursions.  Mathlib
  has \texttt{Composition n} with a \texttt{Fintype} instance but no
  first-block decomposition equivalence, which is the missing combinatorial
  ingredient.
...
\item State the mirrored expansion at the right endpoint, $1-F(1-x)$, which
  follows from $F(x)+F(1-x)=1$ but is nowhere displayed.
\end{enumerate}
