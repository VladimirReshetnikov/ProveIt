...
Fourier analysis, asymptotics, and approximation theory\par}
\vfill
{\large Prepared from the Lean 4 development in\par}
\vspace{0.2cm}
{\ttfamily\large Analysis/FabiusFunction\par}
\vspace{0.8cm}
{\large Repository snapshot inspected 24 August 2026\par}
\vspace{0.2cm}
...
The top-level \file{FabiusFunction.lean} does not contain the development itself.  It is an import façade.  Its principal imports collect four broad products:
\begin{enumerate}
  \item the formalizations of the two 2017 papers, through \file{Paper05442.lean} and \file{Paper06487.lean};
  \item the independent asymptotic project, through \file{PaperFabiusAsymptotic.lean};
  \item the audit and repair of a k-fold Thue--Morse draft, through \file{PaperKFoldThueMorse.lean}; and
  \item later analytic and approximation developments, including negative-Laplace periodic corrections, Mellin finite parts, translated Legendre series, and least-squares optimality.
\end{enumerate}
...
would be false because its fold never inspects values on $(1,\infty)$.
...
\midrule
\endhead
Theorem 1: structure and fold & \lean{IsOriginalFabius}; \lean{IsFabius.isOriginalFabius_rvachevUp} & \file{OriginalCharacterization.lean} \\
Theorem 1: uniqueness & \lean{isOriginalFabius_iff_eq_canonical}; \lean{isOriginalFabius_iff_existsUnique_isFabius}; \lean{existsUnique_originalFabius} & \file{OriginalUniqueness.lean} \\
...
\modulefile{OriginalCharacterization.lean} & Rvachev's original compact-support predicate, the forced scale and normalization, and the general fold from bounded Fabius solutions. \\
\modulefile{OriginalUniqueness.lean} & Fourier-refinement uniqueness and the exact equivalence with unique bounded Fabius witnesses. \\
