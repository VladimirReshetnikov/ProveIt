...
\vspace{0.15cm}
{\small\ttfamily commit 10202bbce8513c2655f615da16a4d1ff90d0a2b4\par}
\vspace{0.2cm}
...
\texttt{\seqsplit{10202bbce8513c2655f615da16a4d1ff90d0a2b4}}.  Before reproducing a
...
\begin{itemize}
  ...
  \item \file{Existence.lean} constructs \lean{Existence.boundedCandidate} and proves unique existence through \lean{existsUnique_fabius} and \lean{IsFabius.eq}; and
  \item \file{PaperStatements.lean} selects the canonical term \lean{fabius} from that witness and exports \lean{fabius_spec}, letting later files forget the implementation again.
\end{itemize}
...
The exact sequences \lean{moment} and \lean{halfMoment} are defined recursively as rational numbers.  The first models the even moments \(\int_{\R}x^{2n}\upf(x)\,\dd x\).  The second has normalized base value \(d_0=1\) and, for \(n\geq0\), models
\[
  d_{n+1}=(n+1)\int_0^1 t^n\upf(t)\,\dd t.
\]
...
\lean{Array.getElem?} lookup, together with compatibility of
\lean{Array.push} and the defining recurrence.
...
smoothness.  On \(0<y<1\),
\lean{deriv_fabiusInv_eq_inv_two_mul_rvachevUp}, together with
\lean{deriv_fabiusInv_pos}, gives
\[
  (F^{-1})'(y)
    =\frac{1}{F'(F^{-1}(y))}
    =\frac{1}{2\upf(2F^{-1}(y)-1)}>0,
\]
and \lean{fabiusInv_contDiffOn_Ioo} proves that the totalized inverse is
...
\lean{deriv_fabiusInv_hasDerivAt} and \lean{deriv_deriv_fabiusInv} prove
\[
  (F^{-1})''(y)
...
\lean{strictConcaveOn_fabiusInv_firstHalf} proves strict concavity on the
closed interval \([0,1/2]\), while
\lean{strictConvexOn_fabiusInv_secondHalf} proves strict convexity on
...
The same method identifies the normalized integrals of \(\upf\) over \([0,1]\) with \lean{halfMoment}.  The
...
\file{FourierProduct.lean} develops the technical product independently.  It proves that complex sinc is entire, establishes pointwise multipliability and the finite-product recurrence, and identifies the product with the analytic Fourier transform.  Continuity then follows through that identification rather than from a separate locally uniform product theorem in this module.
...
\[
  ...
  =\frac1u\sum_{k\in\Z}
    \widehat{\upf}(k/u)e^{2\pi\ii kt/u}.
\]
...
\file{ProbabilityRepresentation.lean} uses the product probability space
...
The self-similarity of \(X\) is the probabilistic counterpart of the fixed-point integral equation.  It supplies a second route to the same canonical bounded function once uniqueness of the \lean{IsFabius} specification is available.
...
\file{WeakConvergence.lean} proves convergence of the finite convolution
...
\file{ProbabilityRepresentation.lean} has CDF \(F\) globally.  The folded
identity is stated directly as
\[
  \upf(x)=\operatorname{weightedSumCDF}(1-|x|)
         =\Prob[X\leq1-|x|].
\]
...
\begin{enumerate}
  ...
  \item \file{ProbabilityRepresentation.lean} for the random-series model;
...
in \file{ThueMorseExponential.lean}:
\[
...
\section{Polynomial constancy and Taylor reduction}
In \file{FabiusQBinomialTaylor.lean}, \(q\) is a common translation indeterminate, not an asymptotic expansion parameter.  Complete-block Thue--Morse cancellation proves that the finite numerator is a constant polynomial in \(q\).  Evaluation is therefore independent of the translation over every characteristic-zero field.  After normalization and reindexing, the constant is identified exactly with \lean{fabiusReductionSum}.

This algebraic constancy is distinct from the discrete-limit approximation \lean{fabiusDiscreteLimitApproximation K q x n}.  A finite approximation there can genuinely depend on a complex \(q\); later complex-shift estimates prove only that this dependence vanishes in the prescribed limit.
...
summable, giving \lean{HasSum} and \lean{tsum} identities.
\file{FabiusGlobalQBinomialSeries.lean} proves that its literal real or complex
...
\file{FabiusParityPowerSeries.lean} rewrites the corrected scale-zero
...
\lean{fabiusReductionSum} and then reuses the already proved binary telescope.
...
\[
  \lean{fabiusDiscreteLimitRangeLength}(x,p)
...
The generic theorem \lean{tendsto_weighted_rows_of_tendsto} isolates four
hypotheses:
\begin{enumerate}
...
points are \lean{sum_range_discreteLimitWeight},
\lean{one_fourth_le_halfQPochhammer},
\lean{sum_abs_discreteLimitWeight_le}, and
\lean{tendsto_weighted_rows_of_tendsto}.

\section{Final reindexing and identification}
\file{FabiusDiscreteLimitIntegration.lean} performs the exact outer
...
\[
  G(z)=1+z\int_0^1\upf(t)e^{zt}\,\dd t.
\]
For \(s>0\), \file{NegativeLaplace.lean} identifies its negative-axis value with the exact product
\[
  e^{\operatorname{negativeLaplaceLog}(s)}=G(-s)
  =\prod_{j\geq1}
    \frac{1-e^{-s/2^j}}{s/2^j}.
\]
The companion \file{LaplaceTransform.lean} proves the equivalent bounded-function formula
\[
...
\section{Mellin--Bose analysis and finite parts}
\file{MellinBose.lean} proves, first for real \(a>0\),
\[
 \int_0^\infty x^{a-1}\log(1-e^{-x})\,\dd x
 =-\Gamma(a)\zeta(a+1),
\]
by expanding the logarithm into an absolutely integrable exponential series
and integrating term by term.  \file{MellinFinitePart.lean} regularizes the
double pole of \(\Gamma(s)\zeta(1+s)\) at \(s=0\), while
\file{BoseFinitePartIntegral.lean} realizes the finite part as convergent
split integrals after subtracting the small-\(x\) singular term.

\file{PeriodicFourier.lean} treats the zero mode through the mean calculation
...
For every \(r>0\), \file{FabiusBromwichInput.lean} proves the exact
...
\file{BromwichSaddle.lean} supplies the generic quantitative framework and
\file{FabiusBromwichInput.lean} discharges its Fabius-specific integrability
...
\file{FabiusLambertPhase.lean}, \file{FabiusLambertRates.lean}, and \file{FabiusLambertSaddle.lean} establish the domain, monotonicity, growth, derivative bounds, and exact lower-Lambert identities.  Separate modules for small-argument expansions later translate \(\lambda\) into elementary logarithms to any fixed order.
...
\file{FabiusSharpAsymptotic.lean} states the sharp logarithmic asymptotic in the repository's exact normalization.  Writing \(\Psi\) for the bounded nonconstant period-one correction and \(C_*\) for the proved normalization constant, its main term is
\[
  \log F(x)=
...
\file{FabiusSaddleJetClosedForm.lean} solves the differential recurrence for
every periodic saddle jet, while \file{FabiusSaddleExponentClosedForm.lean}
...
\file{FabiusSaddleJetStirling.lean} identifies the weight at \(X^m\) as
...
\file{FabiusSaddleLeadingCoefficient.lean} additionally isolates the top-jet
...
\file{FabiusSecondSaddleCorrection.lean} computes \(d_2\) explicitly as a
finite polynomial in the first four bounded exponent jets, and
\file{FabiusSecondSaddleExpansion.lean} substitutes it into the public
...
\[
  ...
  =\lVert\upf-S_N^{\mathrm{even}}\rVert_2^2
   +\lVert S_N^{\mathrm{even}}-p\rVert_2^2.
\]
This Pythagorean identity follows because
\(\upf-S_N^{\mathrm{even}}\) is orthogonal to every polynomial in that
degree class.
...
\section{The failed polygon and the corrected grid samples}
...
formalized by \lean{paperPrefixGridValue_endpoint_not_tendsto_one},
\lean{paperPrefixPolygon_endpoint_not_tendsto_one}, and
\lean{paperPrefixPolygonReal_endpoint_not_tendsto_one} in
\file{ThueMorseGenerating.lean}.

The repaired theorem uses the shifted, dyadic-floor samples
\lean{correctedPrefixGridSample}, not a continuous polygon.  Away from the
right endpoint each sample is exactly \lean{stepApproximant n} at
\lean{correctedPrefixCellCenter n x}; those centers tend to \(2x-1\).  The
endpoint is handled separately, giving pointwise convergence on \([0,1]\) to
\(\upf(2x-1)\), and sampling at \(x/2\) recovers \(F(x)\).  No quantitative
uniform rate or continuity of the sample function is asserted.

\section{Formal counterexamples}
\file{DraftCounterexamples.lean} gives exact witnesses for three distinct
...
unit-interval side condition remains.  Importing \file{FabiusFunction.lean}
...
\longrightarrow
\boxed{\text{\file{Existence.lean}}}\\[1mm]
\longrightarrow
\boxed{\text{\file{PaperStatements.lean}}}.
\end{gathered}
\]
This display omits two important side inputs.  \file{DyadicClosedForm.lean} supplies the exact finite polynomial matched by \file{DyadicAnalytic.lean}, while \file{DyadicCorrectness.lean} supplies evaluator correctness and representation invariance.  \file{Existence.lean} proves unique existence; \file{PaperStatements.lean} makes the canonical choice and publishes \lean{fabius_spec}.  The resulting canonical function is then consumed by moment, Fourier, probability, global-extension, and asymptotic modules.
...
Lemma 1 & \lean{finiteConvolutionProbability_tendsto} and bounded-continuous integral corollaries & \file{EarlyApproximants.lean} (finite convolution measures), \file{WeakConvergence.lean} (characteristic functions and Lévy convergence) \\
...
Theorem 4 & \lean{original_theorem_four_a}, \lean{original_theorem_four_b}, \lean{original_theorem_four_c} & \file{OriginalPaperSupplement.lean}, Fourier and differential helpers \\
Nonanalyticity & \lean{rvachev_not_analyticAt} & \file{OriginalPaperSupplement.lean}, strengthened to the exact locus in \file{NowhereAnalytic.lean} \\
Theorem 5 & Poisson summation and real-axis Fourier rapid decay & \file{PoissonSummation.lean} \\
Theorem 6 & \lean{original_theorem_six}; moment formulas & \file{AnalyticMoments.lean}, \file{OriginalPaperSupplement.lean} \\
Theorem 7 & \lean{original_theorem_seven_global}; global rationality/dyadic statement & \file{GlobalDyadic.lean}, \file{OriginalPaperSupplement.lean} \\
\bottomrule
\end{longtable}
...
Proposition 2 & \lean{proposition_two}; \lean{proposition_two_real} & \file{AnalyticMoments.lean}, \file{PaperStatements.lean}; complex and real removable-singularity forms \\
Proposition 3 & \lean{proposition_three} & \file{MomentPowerSeries.lean}; even-binomial transform of half moments \\
Proposition 4 & \lean{proposition_four} & \file{NormalizedMoments.lean}; natural normalization of half moments \\
...
Proposition 8 & \lean{proposition_eight} & \file{PaperStatements.lean}; even-index Reshetnikov formula and power-of-two denominator bound \\
Theorem 9 & \lean{theorem_nine}, strengthened all-parameter form & \file{Paper06487Supplement.lean}, global/differential helpers \\
Lemma 1 & \lean{lemma_one}, with corrected order condition & \file{TaylorReduction.lean}, \file{PaperStatements.lean} \\
Proposition 10 & \lean{proposition_ten} & \file{TaylorReduction.lean}; recursive signed-extension reduction \\
Definition 12 & \lean{dyadicDenominator} & \file{Arithmetic.lean} \\
Theorem 13 & \lean{theorem_thirteen} and denominator bound & \file{DenominatorBound.lean} \\
Proposition 15 & \lean{proposition_fifteen} & \file{PaperStatements.lean}; inverse-power denominator divides the level dyadic denominator \\
Conjecture 16 & \lean{conjecture_sixteen} & recorded without asserting a proof \\
Theorem 17 & \lean{theorem_seventeen} & \file{PaperStatements.lean}; Lucas digit-product congruence \\
Proposition 18 & \lean{proposition_eighteen} & \file{Parity.lean}, \file{PaperStatements.lean}; count of odd binomial coefficients \\
Proposition 19 & \lean{proposition_nineteen} & \file{Parity.lean}; oddness of \lean{momentNumerator} \\
Theorem 20 & \lean{theorem_twenty} & \file{TwoAdic.lean}; odd reduced numerator/denominator and valuation of \(2d_n\); corrected proof-internal product \(2d_kN_n\) in \file{Paper06487Supplement.lean} \\
Theorem 21 & \lean{theorem_twenty_one} & \file{TwoAdic.lean}; oddness of \(R_n\) and exact inverse-dyadic two-adic valuation \\
Proposition 22 & \lean{proposition_twenty_two_initial}; \lean{proposition_twenty_two} & \file{BernoulliRecurrences.lean}, \file{PaperStatements.lean} \\
\bottomrule
\end{longtable}
...
\chapter{Mathematical object--Lean crosswalk}\label{app:crosswalk}
...
\midrule
\endhead
Abstract bounded Fabius specification & \lean{IsFabius} & \file{Basic.lean} \\
Canonical bounded function \(F\) & \lean{fabius}, \lean{fabius_spec} & chosen in \file{PaperStatements.lean}; existence and uniqueness proved in \file{Existence.lean} \\
Rvachev up-function \(\upf\) & \lean{rvachevUp} & \file{Basic.lean}; analytic properties in \file{Differential.lean} and later shape modules \\
Signed global extension \(\widetilde F\) & \lean{extendedFabius} & defined in \file{Basic.lean}; cell and smoothness theory in \file{GlobalExtension.lean} \\
Exact even moment \(c_n\) & \lean{moment}, \lean{momentIntegral} & \file{Arithmetic.lean}, \file{Basic.lean}, and \file{AnalyticMoments.lean} \\
Normalized half moment \(d_n\) & \lean{halfMoment}, \lean{halfMomentIntegral} & \file{Arithmetic.lean}, \file{Basic.lean}, and \file{AnalyticMoments.lean} \\
Canonical moment numerators \(F_n,G_n\) & \lean{momentNumerator}, \lean{halfMomentNumerator} & \file{Arithmetic.lean}, \file{NormalizedEvenMoments.lean}, and \file{NormalizedMoments.lean} \\
Inverse-power table \(F(2^{-n})\) & \lean{fabiusInversePowTwoTable}, \lean{halfMomentFabiusValue} & \file{Arithmetic.lean} and \file{ExactInversePower.lean} \\
Total dyadic evaluator & \lean{fabiusDyadicValue} & \file{Arithmetic.lean}; analytic correctness wrapper in \file{PaperStatements.lean} \\
Dyadic common denominator \(D_n\) & \lean{dyadicDenominator} & \file{Arithmetic.lean} and \file{DenominatorBound.lean} \\
Reshetnikov quantity \(R_n\) & \lean{reshetnikov} & \file{Arithmetic.lean} and \file{PaperStatements.lean} \\
Weighted-sum CDF & \lean{weightedSumCDF} & \file{ProbabilityRepresentation.lean} \\
Totalized inverse \(F^{-1}\) & \lean{fabiusInv}, \lean{fabiusOrderIso}, \lean{fabiusInv_contDiffOn_Ioo} & \file{FabiusInverse.lean} \\
Inverse curvature & \lean{deriv_deriv_fabiusInv}, \lean{strictConcaveOn_fabiusInv_firstHalf}, \lean{strictConvexOn_fabiusInv_secondHalf} & \file{FabiusInverse.lean} \\
Uniform spline \(S_p\) & \lean{fabiusUniformSpline} & \file{FabiusUniformSpline.lean} and \file{FabiusComputability.lean} \\
Logarithmic profile & \lean{fabiusLogProfile} & \file{FabiusLogScale.lean} \\
Lower-Lambert reference coordinate \(\lambda\) & \lean{fabiusLambertPhase} & \file{FabiusLambertSaddle.lean} \\
Periodic correction \(\Psi\) & \lean{negativeLaplacePsi} & \file{PeriodicCorrection.lean} \\
Even Legendre truncation & \lean{rvachevLegendrePartialSumPolynomial} & \file{FabiusLegendreLeastSquares.lean} \\
\bottomrule
\end{longtable}
...
\lean{reshetnikovQuestion} and \lean{conjecture_sixteen} record open
...
\modulefile{Existence.lean} & Banach fixed-point construction of \lean{Existence.boundedCandidate}, calculus bootstrap, unique existence, and generic equality of all \lean{IsFabius} candidates.  The canonical choice itself is made in \file{PaperStatements.lean}. \\
...
\modulefile{ScalingRecurrence.lean} & Generic chain-rule lemmas for differentiating multiplicative and parameter scaling recurrences, with dyadic specializations used by the negative-Laplace derivative layer. \\
\modulefile{Monotonicity.lean} & Strict increase of \(F\) on \([0,1]\), endpoint range, and order consequences. \\
\modulefile{Regularity.lean} & Optimal global Lipschitz constants for \(F\) and \(\upf\). \\
\modulefile{Convexity.lean} & Convexity before the midpoint and concavity afterward. \\
\modulefile{GlobalBounds.lean} & Sharp value and iterated-derivative bounds for the signed global extension and \(\upf\), including attainment and derivative-support control. \\
\modulefile{BoundedDerivatives.lean} & Exact global sup-norm bounds for every derivative, including attainment. \\
\modulefile{EffectiveFlatness.lean} & Explicit power bounds at the flat endpoint. \\
\modulefile{SharpFlatness.lean} & Factorial-strengthened endpoint power bounds for \(F\) and \(\upf\). \\
\modulefile{NowhereAnalytic.lean} & Exact real-analytic loci: off the closed transition supports and nowhere on them. \\
\modulefile{FabiusInverse.lean} & Totalized inverse, order isomorphism on the unit interval, exact positive reciprocal derivative, reciprocal-cubic second derivative, interior \(C^\infty\) smoothness, strict closed-half curvature, and endpoint steepness. \\
\modulefile{InverseBranch.lean} & Local inverse-branch regularity on the open range. \\
\modulefile{AlgebraicBranch.lean} & Algebraic-function branch interface used in non-elementarity arguments. \\
\modulefile{ElementaryFunction.lean} & Formal expression language and semantics for elementary real functions. \\
\modulefile{NotElementary.lean} & Non-elementarity theorem for the bounded Fabius function on its transition interval. \\
\modulefile{InverseNotElementary.lean} & Transfer of non-elementarity to the inverse branch. \\
\modulefile{FabiusComputability.lean} & Uniform spline error \(\leq2^{-p}\), an effective uniform-continuity modulus, and the computable-real-function bridge. \\
\modulefile{FabiusComputableSpline.lean} & Primitive-recursive dyadic spline evaluator and computable-real interface. \\
\addlinespace[0.45em]
...
\modulefile{MomentPowerSeries.lean} & Exact \lean{PowerSeries} algebra over \(\Q\), including \(M(4X)=M(X)S(X)\); analytic evaluation is deferred to later modules. \\
...
\modulefile{ProbabilityLaplaceMoments.lean} & Moment and Laplace identities connecting the weighted-sum probability model with the analytic generating function. \\
...
\modulefile{FourierProduct.lean} & Entire sinc, pointwise multipliability, finite-product recurrence, and equality of the product with the analytic Fourier transform. \\
\modulefile{FourierAnalytic.lean} & Entire Fourier transform of \(\upf\), Fourier inversion consequences, and its imaginary-axis relation to the half-moment generating function. \\
...
\modulefile{ProbabilityRepresentation.lean} & Product measure on \([0,1]^{\N}\), weighted coordinate sum, self-similarity, and exact CDF/\(\upf\) identifications. \\
\modulefile{WeakConvergence.lean} & Characteristic-function convergence, Lévy continuity, and the resulting convergence against bounded continuous tests. \\
...
\modulefile{FabiusQBinomialTaylor.lean} & Constancy in the common translation indeterminate and identification, after normalization, with \lean{fabiusReductionSum}. \\
...
\modulefile{FabiusUniformSpline.lean} & Centered finite Thue--Morse splines; empty-prefix support, block translation, finite-CDF identification, monotonicity/range bounds, and pointwise convergence. \\
...
\modulefile{FabiusDiscreteLimitIntegration.lean} & Exact outer q-binomial reindexing, Toeplitz passage for complex-shift splines, identification with the binary/literal global series, and all-real extension by empty-prefix vanishing. \\
\addlinespace[0.45em]
...
\modulefile{BromwichSaddle.lean} & Generic Fourier--Bromwich inversion and exact change to dimensionless saddle coordinates, without contour shifting. \\
\modulefile{FabiusLambertPhase.lean} & Exact lower-Lambert reference coordinate and its domain/inversion identities. \\
\modulefile{FabiusLambertRates.lean} & Growth, comparison, and eventual positivity estimates for the exact phase. \\
\modulefile{FabiusLambertSaddle.lean} & Exact algebraic identities at the lower-Lambert reference radius, without claiming it solves the full residual phase's critical equation. \\
...
\modulefile{FabiusSaddleJetClosedForm.lean} & Closed finite formula for periodic saddle jets in derivatives of the periodic correction. \\
\modulefile{FabiusSaddleExponentClosedForm.lean} & Closed decomposition of exponent coefficients into a jet term and universal tail. \\
\modulefile{FabiusSaddleJetStirling.lean} & Identification of jet weights with shifted signed first-kind Stirling numbers. \\
\modulefile{FabiusSaddleLeadingCoefficient.lean} & Exact leading coefficient of the Fabius-specific saddle polynomial. \\
\modulefile{FabiusSecondSaddleCorrection.lean} & Closed period-one coefficient at order \(\lambda^{-2}\). \\
\modulefile{FabiusSecondSaddleExpansion.lean} & Public expansion through the second saddle correction with \(O(\lambda^{-3})\) remainder. \\
...
\modulefile{ThueMorseApproximation.lean} & Exact identification of corrected grid samples with moving-center step approximants and pointwise convergence, including the endpoint. \\
\modulefile{ThueMorseGenerating.lean} & Formal power-series identities, shifted grid recurrence, and exact endpoint refutation of the literal grid and polygonal limits. \\
...
\modulefile{FabiusFunction.lean} & Umbrella import exposing the two paper audits, shape and inverse/non-elementarity theory, computability, negative-Laplace and all-order/closed-form saddle APIs, k-fold audit, and Legendre developments. \\
\addlinespace[0.45em]
\bottomrule
\end{longtable}
...
Poisson equations~(25) and~(32) & Equation~(25) omits the translation variable from the exponential, or equation~(32) retains a factor \(1/a\) after the equation has been scaled by \(a\). & Use the proved normalized identity with \(e^{2\pi\ii kt/u}\); after multiplying both sides by the spacing, cancel the reciprocal factor. \\
...
Theorem 20 naturality & A proof-internal sentence asserts that \(d_kN_n\) is natural. & The corrected integral quantity is \(2d_kN_n\); \lean{two_mul_halfMoment_mul_reshetnikovMultiplier_isNatural} and its oddness strengthening retain the missing factor \(2\). \\
...
\chapter{Reproduction and trust checks}\label{app:reproduction}

\section{Pin the inspected tree}
...
git rev-parse HEAD
source_pin=10202bbce8513c2655f615da16a4d1ff90d0a2b4
...
\section{Build and inspect trust}
...
\lean{propext}, \lean{Classical.choice}, and \lean{Quot.sound}; an individual
theorem may use only a subset.

\section{Rebuild this document}
...
\renewcommand{\bibname}{Bibliographic and repository notes}
\begin{thebibliography}{9}
\addcontentsline{toc}{chapter}{Bibliographic and repository notes}

\bibitem{ProveIt}
Vladimir Reshetnikov,
\emph{ProveIt: Analysis/FabiusFunction},
GitHub repository, source tree and README inspected 25 August 2026.
\url{https://github.com/VladimirReshetnikov/ProveIt/tree/10202bbce8513c2655f615da16a4d1ff90d0a2b4/Analysis/FabiusFunction}
...
Source snapshot & commit \texttt{\seqsplit{10202bbce8513c2655f615da16a4d1ff90d0a2b4}} on \texttt{main}, inspected 25 August 2026 \\
