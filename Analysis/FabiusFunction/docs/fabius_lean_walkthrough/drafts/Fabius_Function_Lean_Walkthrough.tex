...
\]
Because $H$ vanishes on the nonpositive axis, this collapses for every
$y\leq1/2$ to
\[
 H(y)=\int_0^{2y}H(t)\,dt.
\]
The statement remains valid for negative $y$: the interval integral has
reversed orientation, but its integrand vanishes throughout.  This sharp
domain is the public theorem
\pathbreak{weightedSumCDF_eq_intervalIntegral_of_le_half}; the older
\pathbreak{weightedSumCDF_left_formula} remains as its $[0,1/2]$ wrapper.
