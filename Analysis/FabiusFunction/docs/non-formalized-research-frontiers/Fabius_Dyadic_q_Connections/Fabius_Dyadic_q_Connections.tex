...
composition formula \eqref{eq:composition-formula}, the centered and shifted
...
\begin{enumerate}
\item the $q=2$ comparison \eqref{eq:qbinomial-inversion}, the operator
  factorizations \eqref{eq:TM-factorization} and \eqref{eq:TM-sinhc}, and the
  generic-$q$ interpolation identities
  \eqref{eq:q-leading-extractor-general}--\eqref{eq:q-survivor};
\item the Appell package \eqref{eq:An-definition}--\eqref{eq:R-superbound},
...
\item the coefficient-extraction and geometric-simplex derivation
  \eqref{eq:an-coeff}--\eqref{eq:geometric-simplex}, the determinant, and the
...
\item the Toeplitz row polynomial, signed moments and limiting filter, together
  with the explicit rates \eqref{eq:Dn-rate}--\eqref{eq:Dn-centered-rate};
\item the literal CDF sine integral \eqref{eq:F-Fourier} and truncated vertical
  product integral \eqref{eq:Bromwich}, including their convergence and
...
{The finite $q$-formula as an Appell identity}
...
{From exact dyadic values to the arbitrary-argument $q$-series}
...
{The discrete limit as a $q$-Toeplitz transform}
...
\subsection[The discrete q-sum]{The discrete $q$-sum}
