...
\subsection{Layer 2: locally finite zero factorization}

The exact local-jet theorem can be formalized directly from the sinc product by splitting it at $r=\nu_2(m)$.  The crucial finite identity is
\[
 \prod_{h=0}^{r}(-1)^{m/2^h}=-1.
\]
The analytic tail is just a rescaled copy of $\Phi$.  This route avoids a general theory of Fredholm determinants.
...
The resulting program is concrete.  Its finite arithmetic core is ready for Lean formalization; its local jets provide stable numerical tools; its Mellin theory supplies exact asymptotics; and its unresolved comparison with the exact dyadic-value $q$-calculus offers a sharply defined next frontier.
