...
improved inverse theory.  Provenance of the absorbed sources, with
SHA-256 hashes and the cross-validation record, is in
\cref{sec:provenance-inv}.
\end{abstract}
...
This volume is an editorial consolidation (28 August 2026) of two
drafts of the fourth and fifth waves that attack the same problem and
overlap heavily: both prove the closed Lagrange--B\"urmann coefficient
formula, the universal highest logarithm, the explicit third
correction, and the all-orders elasticity recurrence.  Each shared
theorem is stated once; where the sources' formulas could be compared
they were verified to agree exactly (notably the third correction
$d_3$, derived independently in different groupings, and the
elasticity recurrence); and each source's unique material --- the
structure laws, Fourier algebra, tomography, and derivative hierarchy
of one, the saddle partition engine and the $2$-adic completion of
the other --- keeps its own chapter.
...
\prod_j\phi_{r_j,s_j}$ --- convenient for proofs, while
...
$[\ell^{\,n-1}]d_n
=(-1)^{n-1}\Psi^{(n-1)}(\theta)/(2^{n-1}L^{n}(n-1)!)$ for
$n\ge4$.  For the logarithmic coefficients,
...
$d_n$ formula (proved in both, one statement here); the universal
top-log (two proofs, both retained in
\cref{thm:top-log}); the third correction (independently derived,
verified algebraically identical --- the merge's cross-validation);
the Bell conversion; the elasticity recurrence (identical in both);
and the audit/status scaffolding.  Nothing mathematical was
discarded; where the sources' groupings differ, the compacter form
is displayed and the other recorded in the adjacent remark.  Exact
