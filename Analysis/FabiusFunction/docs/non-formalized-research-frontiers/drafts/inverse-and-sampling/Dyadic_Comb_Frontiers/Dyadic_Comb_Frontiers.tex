...
$\sgn\defect_{m,m}=(-1)^{(m+1)/2}$.  Verified for $r\le6$ by the
exact values \eqref{eq:D-values} and
\eqref{eq:D-values-higher}.  The summands alternate in sign by
...
\emph{Formal crosswalk.}  The monomial core of this theorem is kernel-verified: for
