...
conjectures are labeled as such.  Provenance of the six absorbed
...
\eqref{eq:D-first-harmonic} predicts.  The values $D(2)$ and $D(4)$
were derived in one absorbed source and re-derived here; $D(6)$ and
$D(8)$ were computed for this consolidation by an independent
implementation of \eqref{eq:D-rational} (bit-recursion evaluator
\eqref{eq:bit-recursion}, exact \texttt{Fraction} arithmetic) that
first reproduced all eight first-defect values of
...
\cref{tab:first-defects} is nonzero.  The following two conjectures,
stated in the absorbed sources in several equivalent forms, remain the
central open problems of this part.
...
\begin{conjecture}[Spectral positivity]
\label{conj:positivity}
$D(2r)>0$ for every $r\ge1$; equivalently, by
\eqref{eq:D-rational}, the odd-level defects satisfy
$\sgn\defect_{m,m}=(-1)^{(m+1)/2}$.  Verified for $r\le4$ by the
exact values \eqref{eq:D-values}.  The summands alternate in sign by
\eqref{eq:half-sign}, so this is not termwise trivial; the first
harmonic dominates for large $r$ by \eqref{eq:D-first-harmonic}, and
the open content is a uniform estimate at small $r$.
\end{conjecture}
...
closed recurrence would in particular settle
\cref{conj:positivity}.
\end{problem}
...
(\cref{conj:exact-degree,conj:positivity,conj:jets,conj:phase-classification}
and \cref{prob:D-recurrence,prob:sharp-valuations}), the absorbed
sources converge on the following programs.
...
This volume absorbs the following six drafts (all dated 28 August
