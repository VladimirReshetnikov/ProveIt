...
conjectures are labeled as such.  Provenance of the eight absorbed
...
\begin{conjecture}[Phase classification]
\label{conj:phase-classification}
Modulo $1$, the parity-forced zeros of \cref{cor:phase-super} are the
only shifts that are superconvergent at degree $m+1$ for \emph{every}
level of the corresponding parity; individual levels may have
accidental extra zeros.
\end{conjecture}
...
dyadic endpoint comb.  This section, an editorial merge of the
tenth-wave Euler--Maclaurin report and its independently written
eleventh-wave twin (\emph{Exhaustion, Euler--Maclaurin, and Spectral
Exactness for Fabius--Rvachev Monomial Integrals}; overlapping
theorems stated once, each twin's unique layers kept, shared constants
verified identical), extends the same spectral mechanism in four
independent directions: general polynomial weights at \emph{every real
...
defect--quadrature duality.  For general odd $q$ the nonvanishing of
$\E\widetilde B_{2m}(qY)$ --- equivalently of the twisted series
$\sum_{k\,\mathrm{odd}}\Phi(kq/2)k^{-2m}$ --- remains open
(\cref{q:twisted-D-positivity}): for $q\ge3$ the first harmonic
$\Phi(q/2)$ is small and dominance fails.
...
For dyadic meshes the monomial core of \eqref{eq:up-composite-mesh} is
kernel-verified
(\nolinkurl{Fabius.tsum_shifted_monomial_eq_integral_real} in
\nolinkurl{MonomialCombExactness.lean}); the composite-mesh statement
is the same Poisson argument with the coarser valuation count.
...
\begin{conjecture}[Fabius-side phase classification]
\label{conj:F-phase-zeros}
For every $r\ge1$ the only zeros modulo one of
$D_{\cos}(2r;\theta)$ are $\theta=\tfrac14,\tfrac34$, and the only
zeros of $D_{\sin}(2r+1;\theta)$ are $\theta=0,\tfrac12$ --- the
Fabius-side companion of \cref{conj:phase-classification}; first-%
harmonic dominance controls the total mass but not neighbourhoods of
the forced zeros.
\end{conjecture}
...
\begin{question}[Twisted spectral positivity]
\label{q:twisted-D-positivity}
For odd $q\ge3$ and $m\ge1$, is
$\sum_{k\,\mathrm{odd}}\Phi(kq/2)k^{-2m}\ne0$ --- equivalently
$\E\widetilde B_{2m}(qY)\ne0$?  The $q=1$ case is
\cref{thm:D-positive}; for $q\ge3$ the leading harmonic is small and
a genuinely different mechanism (perhaps the Thue--Morse sign
statistics of $\Phi(kq/2)$ along $k$) is needed.  A positive answer
would make the sufficient thresholds of
\cref{thm:composite-termination,thm:reflection-reduction} sharp at
every odd part.
\end{question}
...
with a quantitative alias remainder; increasing $d$ with the mesh
suggests an optimally truncated scheme whose truncation index should
exhibit the lower-Lambert scale of the endpoint asymptotics.  Prove a
uniform ``first surviving alias + Lambert-controlled remainder''
theorem; fractional powers $x^{\alpha}$ ($\alpha>-1$), regularized at
$0$ by the extreme flatness of $F$, are the natural test family, and
quantile transport of the exact $PF$ rules through $F^{-1}$ the
natural application.
\end{question}
...
(\cref{conj:exact-degree,conj:jets,conj:phase-classification,conj:F-phase-zeros,conj:odd-base}
and \cref{prob:D-recurrence,prob:sharp-valuations}; the former
spectral-positivity conjecture is now \cref{thm:D-positive}), the
...
This volume absorbs the following eight drafts (all dated 28 August
