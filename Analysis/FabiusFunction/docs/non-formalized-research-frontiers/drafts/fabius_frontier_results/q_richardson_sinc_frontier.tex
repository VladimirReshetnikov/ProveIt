...
Thus the denominator-free Gaussian identity \eqref{eq:Mrqbinom} and its
mass/cancellation consequences in \cref{lem:qmoments} now have exact Lean
counterparts.  This does not claim a new theorem whose statement is literally
the quotient presentation \eqref{eq:Mrpoch} or the auxiliary polynomial
identity \eqref{eq:MrW}.  The analytic sinc-product acceleration, error series
and bounds, positivity and stability, spline limits, and inverse-function
consequences remain unformalized.
...
\path{GeometricResidualMoments.lean}.  In particular the cancellations
\(1\leq r\leq m\) and \eqref{eq:Mrqbinom} are available at source checkpoint
\path{1e761ed77583e9870ceeaeb2c74ac824094f0f3f}.
...
\item \textbf{Interpolation and positivity.} Reuse the compiled Lagrange
weights and finite moment identities; formalize the divided-difference remainder
and absolute monotonicity of a power series with nonnegative coefficients. The
sign theorem can also be proved directly from the
