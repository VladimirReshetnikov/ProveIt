...
\begin{equation}
 ...
 \label{eq:DN}
\end{equation}
The complete and partial relative tails are
\begin{equation}
 T_N(t)=\frac{P_b(t)}{P_{b,N}(t)}
       =P_b\!\left(\frac{t}{b^N}\right),
 \qquad
 T_{N,j}(t)=\frac{P_{b,N+j}(t)}{P_{b,N}(t)},\quad j\ge0,
 \label{eq:tails}
\end{equation}
with $T_{N,0}=1$.
...
estimate to $\exp(E_{N,r})-1$ and \cref{prop:disk-bound} on that disk.  In particular, the
entire local jet converges uniformly as either $N\to\infty$ at fixed $r$ or $r\to\infty$ at
fixed interior scale.
\end{corollary}
...
\begin{equation}
 ...
 \qquad
 \mathcal T_N(x)=\frac{\Phi(x)}{\Phi_N(x)}
 =\Phi\!\left(\frac{x}{2^{N+1}}\right).
 \label{eq:Phi-prefix-tail}
\end{equation}
The tail is zero-free and positive on $|x|<2^{N+1}$.  Define the factorized transforms
...
For every $N\ge0$ and every real $|x|<2^{N+1}$,
\begin{equation}
...
\end{equation}
with the evident finite interpretation.  Multiplication by $|\Phi_N(x)|$ gives the same
nested inequalities for absolute values of the full products.  Every transform has exactly
the same integer zeros and multiplicities as $\Phi$ inside the interval.
\end{theorem}

\begin{proof}
The exact error is $(-1)^rM_r(1/4,a)$ with
$a=(x/2^{N+1})^2$.  Opposite parities lie on opposite sides of one.  Since
$(1/4)^{r+1}(1+1/4)<1$ for every $r$, \cref{thm:nesting-classification} makes each parity
monotone.  Zero preservation follows from \cref{thm:zero-preserving}.
\end{proof}
...
\input{corpus_manifest_table.tex}
