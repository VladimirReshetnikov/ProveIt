...
The current inventory in \cref{sec:formalized-modules} consists of seventy-two compiled
...
commutative \(\mathbb Q\)-algebras.  This remains generic formal algebra:
it supplies no Thue--Morse-specific probabilistic interpretation.

For \(E_j=-a_j/j\), the ruler convolution
(\cref{thm:ruler-convolution}) says that the sign sequence obeys the same
...
Equation~\eqref{eq:arbitrary-weight-sharp} is an unconditional identity, but
...
\label{thm:prouhet-block-extraction}
...
\begin{equation}
 ...
 \label{eq:prouhet-block-extraction}
\end{equation}
...
\begin{equation}
 ...
 \label{eq:prouhet-block-cancellation}
\end{equation}
...
below \(m\) vanish by \eqref{eq:arbitrary-weight-cancel}, and
\eqref{eq:arbitrary-weight-sharp} gives the degree-\(m\) moment for the steps
...
\label{cor:iterated-prefix-reverse-window}
...
\begin{equation}
 ...
 \label{eq:iterated-prefix-reverse-window}
\end{equation}
...
\begin{equation}
 ...
 \quad\Longleftrightarrow\quad
 S_d^{(k)}=0.
 \label{eq:iterated-prefix-reverse-window-zero-iff}
\end{equation}
...
\eqref{eq:finite-prefix-polynomial}.  The only multiplier is
...
\eqref{eq:iterated-prefix-reverse-window} is
\lean{Fabius.iteratedPrefix\_dyadic\_reverse\_window}, and its exact
zero-locus consequence
\eqref{eq:iterated-prefix-reverse-window-zero-iff} is
...
A product-to-sum recurrence gives the following exact real-variable identity
in the normalization used by the formal development.
...
\begin{equation}
 ...
 \label{eq:formal-lacunary-sine-product}
\end{equation}
\end{theorem}

\emph{Formal versions.}  The displayed theorem is exactly
\lean{Fabius.prod\_sin\_two\_pow\_eq\_thueMorse\_sum} in
\path{LacunaryProductToSum.lean}.  The same module records the two binary
reflection laws
\begin{equation}
 \wt(2^m-1-n)+\wt(n)=m,\qquad
 \eps_{2^m-1-n}=(-1)^m\eps_n
 \quad(n<2^m),                                             \label{eq:formal-bit-complement-laws}
\end{equation}
...
\begin{equation}
 ...
 =\cos\!\left(\frac{m\pi}{2}+x\right)                    \label{eq:formal-quarter-turn-parity}
\end{equation}
for real \(x\), as \lean{Fabius.neg\_one\_pow\_mul\_cos\_sub}.

Putting \(w=z/2^m\) in \eqref{eq:formal-lacunary-sine-product} and reversing
...
\emph{Exact status boundary.}  The Lean module proves
\eqref{eq:formal-lacunary-sine-product} over \(\R\) and the three supporting
identities named above.  It does \emph{not} contain a declaration for the
complex analytic continuation of \eqref{eq:sine-product-to-sum}, nor for the
cosine formula \eqref{eq:cosine-product-to-sum}.  Those adjacent analytic
statements remain natural-language results in this frontier atlas.  Together
they make explicit the Fourier route from finite Thue--Morse blocks to the
sinc products in the Fabius/Rvachev analysis.
...
The first tranches of the formalization now exist as seventy-two
...
\path{ThueMorseSparseProuhet.lean} & Arbitrary-weight Prouhet annihilation and exact top-moment formula (\cref{thm:arbitrary-weights}), degree-valued wrappers covering the zero polynomial on empty supports, the direct affine block extraction and cancellation API (\cref{thm:prouhet-block-extraction}), bit supports, and all-index sparse submask identities. \\
...
\nolinkurl{24da985eba70e46d862acdaddd476793a428213e}, 2026.
