...
The current inventory in \cref{sec:formalized-modules} consists of seventy-seven compiled
...
\label{thm:affine-real-sine-form}
...
\begin{equation}
 ...
 \right).}
 \label{eq:affine-real-sine-form}
\end{equation}
The product is empty at \(m=0\).
\end{theorem}

\emph{Formal version.}
Equation \eqref{eq:affine-real-sine-form} is exactly
...
\eqref{eq:affine-real-sine-form} and
\eqref{eq:formal-lacunary-sine-product} over \(\R\), together with the three
...
\eqref{eq:sine-product-to-sum}, nor for the cosine formula
\eqref{eq:cosine-product-to-sum}.  Those adjacent analytic statements remain
...
The first tranches of the formalization now exist as seventy-seven
...
\path{FiniteQBinomialCore.lean} & Denominator-free Gaussian coefficients by \(q\)-Pascal recursion over arbitrary semirings, finite \(q\)-Pochhammer products over commutative rings, all triangular boundary cases, and the finite \(q\)-binomial theorem at every base, including zero and roots of unity. \\
...
\nolinkurl{252d22694ac58a7ed54854cbec38960166257889}, 2026.
