...
now formal on the whole real interval, not merely on the exponential
ray: for every real \(x\) with \(|x|<1\) the signed series converges
absolutely (\lean{Fabius.summable\_thueMorseSign\_mul\_pow}), the
lacunary product converges
(\lean{Fabius.multipliable\_one\_sub\_pow\_two\_pow}), and
\begin{equation}
 \sum_{n\ge0}\eps_nx^n=\prod_{j\ge0}\bigl(1-x^{2^j}\bigr)
 \qquad(-1<x<1),
 \label{eq:infinite-product-analytic}
\end{equation}
negative \(x\) and \(x=0\) included
(\lean{Fabius.tsum\_thueMorseSign\_mul\_pow},
\path{ThueMorseInfiniteProduct.lean}), with the unordered and
...
The full complex-disc form of \eqref{eq:infinite-product} is not
carried in this shape; what the boundary theory actually uses is the
holomorphic sum \lean{Fabius.thueMorseDiscSeries} with its
finite-level factorization \(F(z)=P_k(z)F(z^{2^k})\)
(\lean{Fabius.thueMorseDiscSeries\_iterate},
\path{ThueMorseDiscSeries.lean}).  The bit Mahler equation
...
The first tranches of the formalization now exist as fifty-seven
...
\path{ThueMorseInfiniteProduct.lean} & The analytic infinite-product identity \(\sum\eps_nx^n=\prod_j(1-x^{2^j})\) on the whole real interval \(|x|<1\): absolute convergence, multipliability, and identification of the limits along the dyadic scale; the exponential ray \(\sum\eps_n\e^{-nt}=\mathcal E(t)\), \(t>0\), is now a specialization. \\
