...
(\path{ThueMorseRarefied.lean}); the Fourier inversion formula
\eqref{eq:DFT-inversion} remains unformalized.
...
\eqref{eq:Riesz-mass} awaits the integration layer.
...
The first tranches of the formalization now exist as twenty-three
...
\path{ThueMorseFourier.lean} & Finite DFT, Walsh transform, product-to-sum identities.  \emph{Delivered} except Fourier inversion and the integral layer (Walsh, Riesz factorization, and the sine-product DFT closed form are formal). \\
...
\path{ThueMorseWords.lean} & Uniform recurrence, aperiodicity, and the elementary complexity bounds.  \emph{Delivered} except the complexity bounds (\path{ThueMorseAperiodicity.lean}, \path{ThueMorseOverlapFree.lean}, \path{ThueMorseRecurrence.lean}). \\
\bottomrule
\end{tabularx}
\end{center}

Of the planned layers, only the Dirichlet--Mellin and factor-complexity
rows remain open (plus the integral and inversion parts of the Fourier
row); every finite or formal layer has been delivered, in most cases in
greater generality than proposed.
