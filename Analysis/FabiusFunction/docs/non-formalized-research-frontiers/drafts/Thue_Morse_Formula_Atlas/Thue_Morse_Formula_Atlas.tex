...
because addition and remainder are.  \Cref{lem:descent-recursion} then
...
\Cref{lem:descent-recursion} are fully formal
...
\lean{Fabius.thueMorseBool\_two\_mul\_add\_one}.
...
sign \eqref{eq:sinc-interval-sign} is not yet formal, but the
quantitative face of the same product is.  \path{SincProductShells.lean}
proves the exact dyadic shell factorization of its modulus: for real
\(y\ne0\) and every \(k\ge0\),
\begin{equation}
 |\Phi(2^ky)|
...
factors of the corresponding dyadic shell.  Bounding the lacunary sine
numerator by the formal Gelfond bound
\lean{Fabius.abs\_prod\_sin\_two\_pow\_le} --- the sine-product form
...
at least the stated form: the upper bound
\lean{Fabius.thueMorseComplexity\_lt} needs only \(\ell\ge1\), which
is why \cref{prop:complexity-bound} is stated there, while
\lean{Fabius.le\_thueMorseComplexity} carries no side condition at
all and holds at \(\ell=0\) too, where \(p(0)=1\).  No literature
import is needed.
...
The first tranches of the formalization now exist as fifty-three
...
\path{ThueMorseBlockProducts.lean} & The three dyadic block-product limits: the block collapse onto the Woods--Robbins partials, the alternating telescoping, and the identification of the half-shifted block with the master series at \((\frac14,\frac34)\). \\
\path{ThueMorseEntireContinuation.lean} & The entire continuation: flatness of every order at zero, exponential decay, local integrability, entirety of \(\Gamma^{-1}\cdot\)Mellin, and agreement with \(D(\sigma,a)\) on \(\sigma>1\). \\
...
\path{ThueMorseDiscSeries.lean} & The Thue--Morse power series as a holomorphic function on the unit disc: locally uniform convergence, the analytic Mahler equation and its iterate, and the real-ray bound \(|F(x)|\le1-x\). \\
