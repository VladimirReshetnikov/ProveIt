...
The current inventory in \cref{sec:formalized-modules} consists of eighty-two compiled
...
zero produces an analytic nonvanishing cofactor, and Lean proves
\[
...
\path{IntegerZeroAnalyticOrder.lean}.  The exact leading coefficient is
extracted by the field-generic local lemma in \path{LeadingJet.lean}.
...
The Glaisher--Fine count \eqref{eq:glaisher-count}
...
\cref{thm:moment-composition}.  The arbitrary-support moment statement and
the master product live one level
down, in \path{ThueMorseMoments.lean} --- an ancestor module cannot import a
descendant, and \cref{thm:moment-composition} is itself derived from them.
...
The first tranches of the formalization now exist as eighty-two
...
\path{LeadingJet.lean} & Field-generic extraction of the first potentially nonzero jet of \(z^nU(z)\), assuming only \(C^n\) regularity of \(U\) at zero, together with the germ-level eventually-equal form requiring no separate regularity hypothesis on the factored function. \\
\path{IntegerZeroAnalyticOrder.lean} & Entirety of the standalone sinc product; nonvanishing at odd half-integers; the total integer-zero local cofactor, its divided pointwise and germ factorizations, analyticity and central value; exact complex analytic order at every nonzero integer; all lower-jet vanishings and the exact nonzero leading derivative. \\
...
\nolinkurl{7b05fce557a326c435f9521ee1a55039429ef116}, 2026.
