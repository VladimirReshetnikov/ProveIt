...
The Euler transform \eqref{eq:log-E} itself and the partition formula
remain analytic and unformalized.
...
\lean{Fabius.integerLift\_modEq}).  The construction of the specific
solution \eqref{eq:integer-lift-recurrence}--\eqref{eq:integer-lift-closed}
and the equivalence of its three descriptions remain unformalized.
...
\emph{Formal versions.}  The algebraic skeleton of the filter's
vanishing is
\lean{Fabius.sum\_thueMorseSign\_mul\_pow\_eq\_zero\_of\_pow\_eq\_one}
...
The complex filter identity \eqref{eq:rarefied-filter} itself remains
unformalized; modulo three it is subsumed by the closed vectors.
...
The first tranches of the formalization now exist as twenty-six
