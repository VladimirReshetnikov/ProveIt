...
below.  In most of them the formal statement is \emph{more general} than the one printed here
...
arbitrary ratio $0<q<1$, so the $q=1/4$ instance is covered.  These labels concern
representation only, not correctness: no result of this report is formalized end
to end, and every result below that carries no such citation is unformalized.  In particular
the transformed infinite series, Richardson tomography and its analytic remainder, the
infinite condition-number limit, the entire probabilistic layer, the entire analytic
layer---Fourier envelopes for the Pascal hierarchy of rank at least two, Mellin contour
displacement, exponential Tauberian transfer, Berry--Esseen for non-identically-distributed
summands, total-variation and Wasserstein distances---and the automatic-sequence machinery
have no Lean counterpart at all.
\end{statusbox}
...
Fourier transform of the Rvachev up-function with the sinc product \eqref{eq:Phi-a} at
$a_h\equiv1$, in the same normalization \eqref{eq:sinc-normalized}, and
...
\begin{equation}
 ...
 \label{eq:zeta-a}
\end{equation}
Both statements are \statusproved.  The multiplicity function \eqref{eq:m-a} is formalized as
...
The canonical product \eqref{eq:canonical-a} is formalized only at $a_h\equiv1$, as
...
\eqref{eq:zeta-a} are not formalized.
\end{theorem}
...
\begin{align}
 ...
 \label{eq:M-bit}
\end{align}
This is \statusproved.  The floor form \eqref{eq:M-floor} is formalized as
...
strictly more general than \eqref{eq:M-floor}.  The two defect forms
\eqref{eq:M-defect}--\eqref{eq:M-bit} are not formalized.
\end{theorem}
...
$a_h\equiv1$ the sign law \eqref{eq:sign-master} is formalized as
...
general parity character \eqref{eq:epsilon-a} is not formalized, and neither is the automaticity
...
$(-1)^{\omega_L(a)}\omega_L(a)!\,2^{\sum_{h\in O_L(a)}h}$ of \eqref{eq:first-surviving},
independently of $x$, and the companion
\lean{Fabius.sum_powerset_neg_one_pow_eval_of_degree_lt} is the vanishing
\eqref{eq:Prouhet-vanish} for every polynomial of degree below $|S|$;
\lean{Fabius.sum_submask_neg_one_pow_pow} is the binary specialization in which $S$ is the bit
support of an integer.  What is not formalized is the factorization \eqref{eq:T-a-L} itself, nor
...
formula.  The conversion itself is formalized: \eqref{eq:bell-moments} is
\lean{Fabius.completeBellPolynomial} (\lean{MomentCumulantAlgebra.lean}), the explicit partition
sum \eqref{eq:partition-moments} is \lean{Fabius.partitionExpSum}
...
this application.  Its cumulant input \cref{thm:cumulants} is not formalized, so the corollary
...
\begin{equation}
 ...
 \label{eq:q-survivor}
\end{equation}
This is \statusproved.  The weights \eqref{eq:lagrange-weight-product} are formalized as
\lean{Fabius.geometricLagrangeWeight} (\lean{GeometricLagrange.lean}); the annihilation
\eqref{eq:q-annihilation} is \lean{Fabius.sum_geometricLagrangeWeight_mul_pow}, with the
normalization at $h=0$ recorded as the genuine boundary value $0^0=1$, and the case $h=r+1$ of
\eqref{eq:q-survivor} is \lean{Fabius.sum_geometricLagrangeWeight_mul_pow_succ}.  Both are
proved over an arbitrary field, for every $q$ whose powers $q^0,\ldots,q^r$ are distinct, so
neither uses the hypothesis $q\in(0,1)$ imposed here.  The closed Gaussian form
\eqref{eq:q-weights} and the range $h>r+1$ of \eqref{eq:q-survivor} are not formalized for
general $q$; what is available for the latter is
\lean{Fabius.sum_lagrangeEvalWeight_mul_pow_card_add} (\lean{LagrangeResidualMoments.lean}),
which expresses every higher moment of an arbitrary distinct-node Lagrange row through a
complete homogeneous polynomial, leaving exactly the principal specialization
$H_n(1,q,\ldots,q^r)=\qbinom{n+r}{r}{q}$ to be supplied.
\end{lemma}
...
\begin{equation}
 ...
 \label{eq:Rrm-bound}
\end{equation}
All three statements are \statusproved.  The exactness half is formalized:
\lean{Fabius.geometricLagrange_richardson_exact} (\lean{GeometricLagrange.lean}) proves that the
weights \eqref{eq:lagrange-weight-product} annihilate the first $r$ geometric error modes of a
...
hypothesis, no nonzero condition on $q$ and no lower bound on the starting index, so it covers
more than the real nonnegative-coefficient setting used here.  The exact tail
\eqref{eq:Rrm-exact}, the sign \eqref{eq:Rrm-sign} and the bound \eqref{eq:Rrm-bound} are not
formalized.
\end{theorem}
...
\end{equation}
This is \statusproved.  Identity \eqref{eq:condition-number} is formalized only at the half
...
reversal $j\mapsto r-j$, with the weights \eqref{eq:lagrange-weight-product}.  General $q$, and
hence both the cumulant grid $q=1/4$ and the limiting value \eqref{eq:qquarter-condition}, is
...
content of \eqref{eq:shift-refinement}--\eqref{eq:mshift-refinement}---that passing to $S^ma$
...
\eqref{eq:finite-difference-factorization} is not formalized.
\end{theorem}
...
\begin{align}
 ...
 \label{eq:kappaP}
\end{align}
All formulas are \statusproved.  Of these, \eqref{eq:mP} is formalized one Newton component at
...
\eqref{eq:generalized-lambert} itself has no Lean counterpart.
...
\end{equation}
These are \statusproved.  The hockey-stick step behind \eqref{eq:Psi-multiplicity} is
...
\eqref{eq:Psi-multiplicity} apart from the exceptional $h=0$ term.  The spectral zeta
\eqref{eq:Psi-zeta}, the cumulants \eqref{eq:Psi-cumulants} and the variance
\eqref{eq:Psi-variance} are not formalized.
\end{theorem}
...
the binary cylinder density selected by $d$.  The Lucas input \eqref{eq:Pd-lucas} is formalized
...
\eqref{eq:Pd-prouhet-density} is not formalized.
\end{corollary}
...
framework of \cite{flajolet1995}.  This is \statusproved.  Neither \eqref{eq:kernel-mellin} nor
\eqref{eq:La-mellin} is formalized.  The library does prove the sibling real-variable integral
...
the right of the strip in \eqref{eq:kernel-mellin}, so it is a companion identity and not
\eqref{eq:kernel-mellin} itself.  The double pole of $\Gamma(w)\zeta(w+1)$ at $w=0$ that governs
...
\item The geometric Lagrange weights, Gaussian-binomial closed form, exact
      survivor, and finite residual sign are now formalized; transferring this
      finite kernel to the report's analytic Richardson remainder remains open.
\end{enumerate}
...
exactness.  What genuinely remains in Phase~I is the binary defect \eqref{eq:M-bit}, the
automaticity equivalence of step~3, the sign-product factorization \eqref{eq:T-a-L} and its
free-even-layer count, the closed Gaussian-binomial form \eqref{eq:q-weights}, and the
general-$q$ survivor \eqref{eq:q-survivor} with the sign-certified remainder built on it.
...
\path{GeometricLagrangeQMoments.lean}.  The tomography interpretation,
one-sided analytic remainder, and infinite condition-number limit remain
