...
Items labeled \statusopen{} are conjectures or proposed programs.  The finite
rational geometric-Lagrange kernel used in the Gaussian-\(q\) rows---closed
weights, all residual moments and their sign, and the finite exact
\(\ell^1\) norm, including \(q=1/4\)---now has matching Lean declarations.
The transformed infinite series, Richardson tomography and analytic
remainder, infinite condition-number limit, and the other new theorem
families in this report are not thereby claimed formalized.
\end{statusbox}
