...
geometric ratio instead of $q\in(0,1)$); in a few it is a strict specialization, such as the
$\ell^1$ weight norm, which is formalized only at $q=1/2$.  Each citation says which.  These
labels concern representation only, not correctness: no result of this report is formalized end
to end, and every result below that carries no such citation is unformalized.  In particular
the entire probabilistic layer, the entire analytic layer---Fourier envelopes for the Pascal
hierarchy of rank at least two, Mellin contour displacement, exponential Tauberian transfer,
Berry--Esseen for non-identically-distributed summands, total-variation and Wasserstein
distances---and the automatic-sequence machinery have no Lean counterpart at all.
\end{statusbox}
...
\item Add the geometric Lagrange weights, Gaussian-binomial closed form, exact survivor, and
