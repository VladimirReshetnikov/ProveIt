...
\begin{equation}\label{p3:eq:headline}
\boxed{
...
\end{equation}
Here $\G$ denotes the signed global Fabius extension.  On $0\le m\le 2^n$, it agrees with the usual bounded Fabius function $\F$ on $[0,1]$.  The formula was proposed experimentally in 2019 \cite{p3:ReshetnikovMSE}; the cited Lean development proves it for all $m,n\in\N$, including all edge cases and all non-reduced representations of a dyadic number.
...
\begin{align}
  \F(x)&=0 &&(x\le 0),\label{p3:eq:Fleft}\\
  \F(x)&=1 &&(x\ge 1),\label{p3:eq:Fright}\\
  \F(1-x)&=1-\F(x) &&(0\le x\le1),\label{p3:eq:Fsym}\\
  \F'(x)&=2\F(2x) &&(0\le x\le\tfrac12).\label{p3:eq:Fder}
\end{align}
...
\begin{equation}\label{p3:eq:updef}
\upfun(x)=
\begin{cases}
 \F(x+1),&x\le0,\\
 \F(1-x),&x>0.
...
\begin{equation}\label{p3:eq:globalext}
  \G(x)=\sum_{j=0}^{\infty}\tm_j\,
  \upfun(x-2j-1).
\end{equation}
Because $\upfun$ has compact support, only finitely many summands can be nonzero at a fixed $x$.  Thus \eqref{p3:eq:globalext} is pointwise a finite sum.  It agrees with $\F$ on $[0,1]$ but can be negative outside that interval.  For example, $\G(3)=-1$.

This distinction matters in \eqref{p3:eq:headline}.  When $m\le2^n$, the argument $m/2^n$ lies in $[0,1]$ and the right-hand side equals $\F(m/2^n)$.  For arbitrary $m$, the same finite expression computes the signed extension $\G(m/2^n)$.
...
\begin{equation}\label{p3:eq:halfmoments}
 d_0=1,
 \qquad
...
\begin{equation}\label{p3:eq:arec}
  a_n=\frac{1}{2^n-1}
  \sum_{j=0}^{n-1}\frac{a_j}{(n-j+1)!}
  \qquad(n\ge1).
\end{equation}
The bridge to the Fabius function is
\begin{equation}\label{p3:eq:Fainverse}
 \boxed{\F(2^{-n})=2^{-\binom n2}a_n.}
\end{equation}
This relation is one of the principal endpoints of the arithmetic development of Arias de Reyna and of its Lean formalization \cite{p3:AriasArithmetic,p3:ProveIt}.

\begin{table}[H]
\centering
\caption{The first inverse-dyadic values obtained from \eqref{p3:eq:arec} and \eqref{p3:eq:Fainverse}.}
\label{p3:tab:firstvalues}
\begin{tabular}{@{}ccll@{}}
\toprule
$n$ & $2^{-n}$ & $a_n$ & $\F(2^{-n})$\\
\midrule
...
The recurrence \eqref{p3:eq:arec} becomes conceptually clearer after passing to a formal power series.  Set
\begin{equation}\label{p3:eq:Adef}
...
\]
For $n\ge1$, recurrence \eqref{p3:eq:arec} says that this is $2^na_n$; the same identity is immediate for $n=0$.  Hence
\begin{equation}\label{p3:eq:Arefine}
  \boxed{A(2X)=E(X)A(X).}
\end{equation}

Equation \eqref{p3:eq:Arefine} is the algebraic core of the later $q$-binomial argument.  Iterating it at the negative dyadic arguments $-X,-2X,-4X,\ldots$ gives
\begin{equation}\label{p3:eq:Aiterate}
 A(-2^kX)
...
\begin{equation}\label{p3:eq:qpochdef}
  \qpoch{a}{q}{n}=\prod_{j=0}^{n-1}(1-aq^j),
  \qquad \qpoch{a}{q}{0}=1.
\end{equation}
When $|q|<1$, its infinite counterpart is
\begin{equation}\label{p3:eq:qpochinf}
...
The terminology can be misleading to a newcomer.  The ordinary Pochhammer symbol $(a)_n=a(a+1)\cdots(a+n-1)$ is an additive shifted product, whereas \eqref{p3:eq:qpochdef} is multiplicative.  The common name reflects the fact that both are the fundamental coefficient packages for their respective hypergeometric theories.

The elementary recurrence
\begin{equation}\label{p3:eq:qpochrec}
...
\begin{equation}\label{p3:eq:qpochsplit}
...
\begin{equation}\label{p3:eq:qpochzero}
 \qpoch{q^{-d}}{q}{n}=0\qquad(0\le d<n).
\end{equation}
At $q=1/2$, this says
\begin{equation}\label{p3:eq:halfzeros}
...
For $q\ne1$, define
\begin{equation}\label{p3:eq:qinteger}
 [n]_q=\frac{1-q^n}{1-q}=1+q+\cdots+q^{n-1},
 \qquad
 [n]_q!=\prod_{j=1}^{n}[j]_q.
\end{equation}
Then
\begin{equation}\label{p3:eq:qfactorialpoch}
...
\begin{equation}\label{p3:eq:qgamma}
 \Gamma_q(z)=(1-q)^{1-z}
 \frac{\qpoch{q}{q}{\infty}}{\qpoch{q^z}{q}{\infty}},
\end{equation}
which satisfies $\Gamma_q(z+1)=[z]_q\Gamma_q(z)$ and tends to the ordinary gamma function as $q\uparrow1$.  The Fabius formulas do not require $q$-gamma functions, but this context explains why the finite product in \eqref{p3:eq:headline} belongs to a substantial special-function theory rather than being an isolated notation \cite{p3:DLMF17,p3:GasperRahman,p3:KacCheung}.

\subsection{The half-base specialization and Mersenne products}

Write
\begin{equation}\label{p3:eq:pn}
 p_n=\qpoch{\frac12}{\frac12}{n}
     =\prod_{j=1}^{n}(1-2^{-j})
\end{equation}
and introduce the Mersenne product
\begin{equation}\label{p3:eq:Mn}
 M_n=\prod_{j=1}^{n}(2^j-1),
 \qquad M_0=1.
\end{equation}
Factoring $2^{-j}$ from the $j$th term yields
\begin{equation}\label{p3:eq:pMersenne}
 \boxed{
 p_n=2^{-\binom{n+1}{2}}M_n.}
\end{equation}
Consequently the apparently mixed denominator in \eqref{p3:eq:headline} has the purely arithmetic form
\begin{equation}\label{p3:eq:fullDenM}
 \boxed{
...
\]
Only finite $p_n$ occur in the exact dyadic formulas, but the nonzero limit helps explain why the dominant growth in \eqref{p3:eq:fullDenM} comes from the explicit power $2^{n^2}$ rather than from an exponentially small $q$-product.
...
\begin{equation}\label{p3:eq:rphis}
 {}_r\phi_s\!\left(
...
\begin{equation}\label{p3:eq:infqbinomial}
 \sum_{n=0}^{\infty}
 \frac{\qpoch{a}{q}{n}}{\qpoch{q}{q}{n}}z^n
 =\frac{\qpoch{az}{q}{\infty}}{\qpoch{z}{q}{\infty}},
 \qquad |q|<1,\quad |z|<1.
\end{equation}
Our Fabius argument uses the finite rather than infinite $q$-binomial theorem.  Nevertheless, \eqref{p3:eq:rphis}--\eqref{p3:eq:infqbinomial} place the notation within the standard hierarchy of $q$-special functions catalogued by the DLMF \cite{p3:DLMF17}.
...
\begin{equation}\label{p3:eq:qbindef}
 \qbinom{n}{k}{q}
 =\frac{\qpoch{q}{q}{n}}
...
Despite the quotient in \eqref{p3:eq:qbindef}, $\qbinom{n}{k}{q}$ is a polynomial in $q$ with nonnegative integer coefficients.  It is symmetric in $k$ and $n-k$,
\begin{equation}\label{p3:eq:qbinsymm}
 \qbinom{n}{k}{q}=\qbinom{n}{n-k}{q},
\end{equation}
and it tends to the ordinary binomial coefficient as $q\to1$:
\begin{equation}\label{p3:eq:qbinlimit}
 \lim_{q\to1}\qbinom{n}{k}{q}=\binom nk.
\end{equation}
Two useful $q$-Pascal recurrences are
\begin{align}
 \qbinom{n+1}{k}{q}
 &=\qbinom{n}{k}{q}+q^{n+1-k}\qbinom{n}{k-1}{q},\label{p3:eq:qpascal1}\\
 \qbinom{n+1}{k}{q}
 &=q^k\qbinom{n}{k}{q}+\qbinom{n}{k-1}{q}.
 \label{p3:eq:qpascal2}
\end{align}
...
\begin{theorem}[Finite $q$-binomial theorem]\label{p3:thm:finiteqbinomial}
For every $n\in\N$,
\begin{equation}\label{p3:eq:finiteqbinomial}
 \boxed{
 \qpoch{z}{q}{n}
...
\]
which is the desired coefficient by \eqref{p3:eq:qpascal1}.
\end{proof}

At $q\to1$, \eqref{p3:eq:finiteqbinomial} reduces to
\[
...
\begin{equation}\label{p3:eq:finitehalf}
 \qpoch{z}{\frac12}{n}
...
\begin{align}
 ...
 \label{p3:eq:verticalzero}\\
 \sum_{k=0}^{n}(-1)^k2^{-\binom{k}{2}}
   \halfqbinom{n}{k}(2^n)^k
 &=(-1)^nM_n.
 \label{p3:eq:verticalendpoint}
\end{align}
...
\begin{equation}\label{p3:eq:reciprocalqbin}
 \qbinom{n}{k}{q^{-1}}
 =q^{-k(n-k)}\qbinom{n}{k}{q}.
\end{equation}
Taking $q=2$ yields
\begin{equation}\label{p3:eq:halfvs2}
 \boxed{
 \halfqbinom{n}{k}
 =2^{-k(n-k)}\qbinom{n}{k}{2}.}
\end{equation}
Thus every half-base coefficient is a power-of-two rescaling of the number of $k$-planes in $\mathbb F_2^n$.  Equivalently, using the Mersenne products from \eqref{p3:eq:Mn},
\begin{equation}\label{p3:eq:halfqMersenne}
 \boxed{
 \halfqbinom{n}{k}
...
\begin{equation}\label{p3:eq:nodesweights}
 x_k=-2^k,
 \qquad
 w_{n,k}=(-1)^k2^{-\binom{k}{2}}\halfqbinom{n}{k}
 \qquad(0\le k\le n).
\end{equation}
Then \eqref{p3:eq:verticalzero}--\eqref{p3:eq:verticalendpoint} become
\begin{equation}\label{p3:eq:momentannihilation}
 ...
 \end{cases}
\end{equation}
Indeed, $x_k^d=(-1)^d(2^d)^k$, so the extra sign changes the endpoint $(-1)^nM_n$ in \eqref{p3:eq:verticalendpoint} into $+M_n$.

Equation \eqref{p3:eq:momentannihilation} says that the functional
\begin{equation}\label{p3:eq:Dndef}
...
\begin{proposition}[Barycentric form of the half-base weights]\label{p3:prop:barycentric}
For $x_k=-2^k$,
\begin{equation}\label{p3:eq:barycentric}
 \boxed{
 \frac{w_{n,k}}{M_n}
...
\]
Substituting \eqref{p3:eq:halfqMersenne} into \eqref{p3:eq:nodesweights} gives the reciprocal of this product after division by $M_n$.
\end{proof}

The right-hand side of \eqref{p3:eq:barycentric} is the standard coefficient of $f(x_k)$ in the formula for the leading coefficient of the Lagrange interpolant through the nodes $x_0,\ldots,x_n$.  Thus the $q$-binomial coefficients in the Fabius formula can be read as \emph{barycentric weights for a geometric grid}.  This viewpoint will turn a complicated double sum into a coefficient extractor.
...
\begin{equation}\label{p3:eq:tmdef}
 \tm_r=(-1)^{s_2(r)}.
\end{equation}
The first terms are
\[
...
From \eqref{p3:eq:tmrec}, one obtains by induction
\begin{equation}\label{p3:eq:tmgeneratingpoly}
...
\begin{equation}\label{p3:eq:Ckdef}
 C_k(X)=\sum_{r=0}^{2^k-1}\tm_r\e^{(r-2^k)X}
       =\sum_{d=0}^{\infty}T_{k,d}\frac{X^d}{d!}.
\end{equation}
Applying \eqref{p3:eq:tmgeneratingpoly} with $z=\e^X$ gives
\begin{align}
 C_k(X)
 &=\e^{-2^kX}\prod_{j=0}^{k-1}(1-\e^{2^jX})\notag\\
 &=(-1)^k\e^{-X}\prod_{j=0}^{k-1}(1-\e^{-2^jX}).
 \label{p3:eq:Ckfactor}
\end{align}
The second form uses $\sum_{j=0}^{k-1}2^j=2^k-1$.

By \eqref{p3:eq:Tvanish}, $C_k$ has an exact zero of order $k$ at $X=0$.  Remove it by defining
\begin{equation}\label{p3:eq:Hkdef}
 H_k(X)=\frac{C_k(X)}{X^k}
       =\sum_{n=0}^{\infty}
         \frac{T_{k,n+k}}{(n+k)!}X^n.
\end{equation}
Factoring $2^jX$ from the $j$th term in \eqref{p3:eq:Ckfactor} yields
\begin{equation}\label{p3:eq:Hkfactor}
 \boxed{
...
\end{equation}
Since
\begin{equation}\label{p3:eq:Enegative}
 E(-aX)=\frac{\e^{-aX}-1}{-aX}
       =\frac{1-\e^{-aX}}{aX},
\end{equation}
we can rewrite \eqref{p3:eq:Hkfactor} as
\begin{equation}\label{p3:eq:HkE}
 \boxed{
 H_k(X)=(-1)^k2^{\binom{k}{2}}\e^{-X}
 \prod_{j=0}^{k-1}E(-2^jX).}
\end{equation}
Compare this with the iterated Fabius refinement identity \eqref{p3:eq:Aiterate}: the products are identical.

\subsection{Translated blocks}

For a common shift $c$, set
\begin{equation}\label{p3:eq:Ttrans}
 T_{k,d}(c)=\sum_{r=0}^{2^k-1}\tm_r(r-2^k+c)^d.
\end{equation}
Its exponential series is simply
\begin{equation}\label{p3:eq:Ctrans}
 \sum_{d=0}^{\infty}T_{k,d}(c)\frac{X^d}{d!}
 =\e^{cX}C_k(X).
\end{equation}
After removing $X^k$,
\begin{equation}\label{p3:eq:Htrans}
 H_k^{(c)}(X)
...
Define
\begin{equation}\label{p3:eq:Rkdef}
 R_k(X)=\e^{-X}\prod_{j=0}^{k-1}E(-2^jX).
\end{equation}
Then \eqref{p3:eq:HkE} becomes
\begin{equation}\label{p3:eq:HkRk}
 H_k(X)=(-1)^k2^{\binom{k}{2}}R_k(X).
\end{equation}
On the other hand, multiplying \eqref{p3:eq:Aiterate} by $\e^{-X}$ gives
\begin{equation}\label{p3:eq:RkA}
 \boxed{
...
\begin{equation}\label{p3:eq:Rparam}
 R(x;X)=\frac{\e^{-X}A(xX)}{A(-X)}.
\end{equation}
Then $R_k(X)=R(-2^k;X)$.

For each fixed $n$, the coefficient
\begin{equation}\label{p3:eq:rnpoly}
...
\begin{proposition}[Coefficient polynomial]\label{p3:prop:coefficientpoly}
For every $n\ge0$,
\[
...
Apply the leading-coefficient functional \eqref{p3:eq:Dndef} to the polynomial $r_n(x)$.  Using \eqref{p3:eq:momentannihilation} or, equivalently, the Lagrange weights \eqref{p3:eq:barycentric}, we obtain
\begin{equation}\label{p3:eq:extractan}
 \frac1{M_n}\sum_{k=0}^{n}w_{n,k}
 \coeff{X}{n}R_k(X)=a_n.
\end{equation}
Thus
\begin{equation}\label{p3:eq:weightedR}
 \boxed{
 \sum_{k=0}^{n}w_{n,k}\coeff{X}{n}R_k(X)=M_na_n.}
\end{equation}

This is the decisive coefficient-isolation identity.  It can also be proved without explicitly introducing $R(x;X)$: one expands \eqref{p3:eq:RkA}, uses the moment annihilation \eqref{p3:eq:momentannihilation}, and inducts on the coefficient degree.  That is the style formalized in \texttt{FabiusQBinomialFormula.lean}.  The divided-difference viewpoint explains what the induction is doing.
...
\begin{equation}\label{p3:eq:Nncdef}
 \mathcal N_n(c)=
 \sum_{k=0}^{n}
 \frac{\halfqbinom{n}{k}}
      {2^{k(k-1)}(n+k)!}
...
\eqref{p3:eq:Ttrans}.  The centered numerator is $\mathcal N_n(0)$, and the
...
\begin{align}
 ...
 &=\sum_{k=0}^{n}
   ...
   \coeff{X}{n}R_k(X)\notag\\
 &=\sum_{k=0}^{n}w_{n,k}\coeff{X}{n}R_k(X).\label{p3:eq:Nweighted}
\end{align}
...
Combining \eqref{p3:eq:Nweighted} with the geometric divided-difference
identity \eqref{p3:eq:weightedR} yields the compact evaluation
\begin{equation}\label{p3:eq:Ncenteredvalue}
...
\begin{equation}\label{p3:eq:Mnpnagain}
...
\begin{align}
 ...
 &=2^{-\binom n2}a_n\notag\\
 &=\F(2^{-n}),\label{p3:eq:normalizationfinish}
\end{align}
where the last step is \eqref{p3:eq:Fainverse}.  We have proved the centered
version of the exact formula.

\begin{theorem}[Inverse-dyadic $q$-binomial--Thue--Morse formula]
\label{p3:thm:inverseqformula}
For every $n\in\N$,
\begin{equation}\label{p3:eq:inverseqformula}
 \boxed{
 \F(2^{-n})=
 \frac{1}{2^{n^2}\qpoch{\frac12}{\frac12}{n}}
 \sum_{k=0}^{n}
...
this strengthening is proved in \cref{p3:sec:translation}.
\end{theorem}

\begin{proof}
The inner sum divided by $(n+k)!$ is the coefficient of $X^n$ in
$H_k(X)$.  After \eqref{p3:eq:HkRk}, the outer coefficients become the
...
\eqref{p3:eq:weightedR}.  The normalization \eqref{p3:eq:normalizationfinish}
...
\begin{equation}\label{p3:eq:proofdiagram}
\begin{aligned}
...
\section{Translation invariance and scalar extension}\label{p3:sec:translation}

\subsection{Why a common shift disappears}

The half shift in \eqref{p3:eq:headline} is aesthetically natural because it
centers the arithmetic around half-integers, but it is not mathematically
essential.  In fact, the complete numerator \eqref{p3:eq:Nncdef} is independent
of $c$.  The reason is another triangular vanishing argument.

Set
\begin{equation}\label{p3:eq:Snc}
 S_n^{(c)}(X)=
 \sum_{k=0}^{n}
 \frac{\halfqbinom{n}{k}}{2^{k(k-1)}}H_k^{(c)}(X).
\end{equation}
Then by \eqref{p3:eq:Htrans},
\begin{equation}\label{p3:eq:Sntranslate}
...
\begin{equation}\label{p3:eq:Snorder}
...
\begin{equation}\label{p3:eq:Nconstant}
...
For rational $c$, identity \eqref{p3:eq:Sntranslate} already gives a direct
...
\begin{equation}\label{p3:eq:tmreflection}
 \tm_{2^k-1-r}=(-1)^k\tm_r
 \qquad(0\le r<2^k).
\end{equation}
...
\begin{align}
 ...
 &=(-1)^n
   \sum_{r=0}^{2^k-1}\tm_r
   (r-2^k+1-q)^{n+k}.\label{p3:eq:rawreflection}
\end{align}
...
\begin{corollary}[Raw-coordinate identity]\label{p3:cor:rawformula}
For every $n\in\N$ and every rational, real, or complex $q$,
\begin{equation}\label{p3:eq:rawformula}
 \boxed{
 \F(2^{-n})=
 \frac{1}{(-2)^{n^2}\qpoch{\frac12}{\frac12}{n}}
 \sum_{k=0}^{n}
...
Formula \eqref{p3:eq:rawformula} is not a distinct phenomenon.  It is the
centered formula viewed through the binary complement symmetry of a complete
block.

\section{Every dyadic numerator}\label{p3:sec:arbitrarym}
...
\begin{equation}\label{p3:eq:Bmdef}
...
\begin{equation}\label{p3:eq:Dmkdef}
 D_{m,k}(X)=
...
\begin{equation}\label{p3:eq:tmblockmultiplicative}
 \tm_{h2^k+s}=\tm_h\tm_s.
\end{equation}
A direct calculation then gives the factorization
\begin{equation}\label{p3:eq:Dfactor}
...
therefore
\begin{equation}\label{p3:eq:Dshiftedfactor}
...
Introduce
\begin{equation}\label{p3:eq:Smparam}
...
not merely $a_n$ but
\begin{equation}\label{p3:eq:rmnleading}
...
\begin{equation}\label{p3:eq:generalextract}
 \sum_{k=0}^{n}w_{n,k}
...
the long-block moments in \eqref{p3:eq:Dmkdef}.  The right-hand side is the
...
that identity is
\begin{equation}\label{p3:eq:dyadicprefixcoefficient}
 \boxed{
 \G\!\left(\frac{m}{2^n}\right)
 =2^{-\binom n2}\coeff{X}{n}\bigl(B_m(X)A(X)\bigr).}
\end{equation}
For $m=1$, $B_1=1$, and \eqref{p3:eq:dyadicprefixcoefficient} reduces to
\eqref{p3:eq:Fainverse}.

Equations \eqref{p3:eq:generalextract}, \eqref{p3:eq:Mnpnagain}, and
\eqref{p3:eq:dyadicprefixcoefficient} prove the full formula.

\begin{theorem}[All nonnegative dyadic numerators]\label{p3:thm:alldyadic}
For all $m,n\in\N$ and every common translation $c$ in $\Q$, $\R$, or
$\C$,
\begin{equation}\label{p3:eq:alldyadic}
\boxed{
\G\!\left(\frac{m}{2^n}\right)
=
\frac{1}{2^{n^2}\qpoch{\frac12}{\frac12}{n}}
...
\eqref{p3:eq:generalextract} still has no coefficients below degree $n$,
...
\cref{p3:sec:translation} proves that the coefficient of degree $n$ is
...
\]
The right side of \eqref{p3:eq:alldyadic} changes drastically when $(m,n)$ is
...
When $0\le m\le2^n$, formula \eqref{p3:eq:alldyadic} computes a number in
$[0,1]$, namely the usual Fabius value.  When $m>2^n$, it computes the signed
global extension from \eqref{p3:eq:globalext}.  This is not a cosmetic domain
...
\begin{equation}\label{p3:eq:Nnqscalar}
 \mathcal N_n^{(q)}=
 \sum_{k=0}^{n}
 \frac{\halfqbinom{n}{k}}
      {2^{k(k-1)}(n+k)!}
...
\begin{equation}\label{p3:eq:Ntaylornormalization}
 \boxed{
 \frac{\mathcal N_n^{(q)}}
      {2^{\binom n2}\qpoch{\frac12}{\frac12}{n}}
 =2^na_n=\frac{2^nd_n}{n!}.}
\end{equation}
Notice that the power of two in \eqref{p3:eq:Ntaylornormalization} is
...
For $m\in\N$, define
\begin{equation}\label{p3:eq:Qmqdef}
 \mathcal Q_m^{(q)}(y)=
 \frac1{2^{\binom{m+1}{2}}}
 \sum_{n=0}^{m}
 ...
 \frac{\mathcal N_n^{(q)}}
      {2^{\binom n2}\qpoch{\frac12}{\frac12}{n}}.
\end{equation}
Substitution of \eqref{p3:eq:Ntaylornormalization}, followed by the change of
index $j=m-n$, gives
\begin{equation}\label{p3:eq:QTaylor}
 \mathcal Q_m^{(q)}(y)
 =\sum_{j=0}^{m}
...
\eqref{p3:eq:Qmqdef} is merely an alternative exact presentation of a familiar
finite polynomial.

\begin{theorem}[$q$-binomial Taylor identity]\label{p3:thm:qTaylor}
For every $m\in\N$, every scalar $q$ in a characteristic-zero field, and
every scalar $y$,
\begin{equation}\label{p3:eq:qTayloridentity}
...
\begin{equation}\label{p3:eq:epsilonm}
 \epsilon_m(x)=
 (-1)^{s_2(p_m(x))}
...
\end{equation}
For every $N\ge0$,
\begin{equation}\label{p3:eq:finitetelescope}
 \G(x)=
...
$N\ge1$
\begin{equation}\label{p3:eq:remainderbound}
 |\rho_N(x)|\le2^{1-N}.
\end{equation}
Moreover, the individual terms satisfy a geometric majorant; for $m\ge2$,
\begin{equation}\label{p3:eq:summandbound}
 \left|\epsilon_m(x)\mathcal T_m(y_m(x))\right|
 \le4\,2^{-(m-1)}.
\end{equation}
Thus the resulting series is absolutely convergent.

\begin{theorem}[Global binary-reduction series]\label{p3:thm:globalbinary}
For every $x\ge0$,
\begin{equation}\label{p3:eq:globalbinary}
 \boxed{
 \G(x)=\sum_{m=0}^{\infty}
...
By \cref{p3:thm:qTaylor}, one may replace $\mathcal T_m$ in
\eqref{p3:eq:globalbinary} by $\mathcal Q_m^{(q)}$ for any real or complex
...
sums:
\begin{equation}\label{p3:eq:globalqcompact}
 \boxed{
 \G(x)=\sum_{m=0}^{\infty}
...
&\hspace{4.5em}\times
 ...
 \sum_{r=0}^{2^k-1}
 \tm_r(r-2^k+q)^{n+k}.
 \label{p3:eq:globalqliteral}
\end{align}
Formula \eqref{p3:eq:globalqliteral} is close to the literal nested expression
formalized in \path{FabiusGlobalQBinomialSeries.lean}.  The compact
notation in \eqref{p3:eq:globalqcompact} reveals its architecture more clearly:
...
\begin{equation}\label{p3:eq:squaresplit}
 n^2=\binom{n+1}{2}+\binom n2.
\end{equation}
The first triangle converts $p_n$ to $M_n$; the second is the inverse-dyadic
Fabius scaling in \eqref{p3:eq:Fainverse}.  Thus the apparently abrupt $n^2$
...
functional
\begin{equation}\label{p3:eq:PTMfunctional}
...
\begin{equation}\label{p3:eq:PTMoperators}
 \mathcal P_k[f]
...
\eqref{p3:eq:PTMoperators}, its geometry is multiplicative: moving from one
...
\begin{equation}\label{p3:eq:dependencygraph}
\begin{gathered}
 \texttt{Basic}
 \longrightarrow
...
\caption{Principal Lean modules and their roles.}\label{p3:tab:leanmodules}\\
\toprule
...
\begin{equation}\label{p3:eq:abstractPk}
 P_k(X)A(-X)=\e^{-X}A(x_kX),
\end{equation}
and weights $w_k$ obey
\begin{equation}\label{p3:eq:abstractmoments}
 \sum_kw_kx_k^d=0\qquad(d<n).
\end{equation}
Then
\begin{equation}\label{p3:eq:abstractisolation}
 \sum_kw_k\coeff{X}{n}P_k(X)
 =\left(\sum_kw_kx_k^n\right)a_n.
\end{equation}
The proof is a strong induction on $n$.  Extract the coefficient of $X^n$
from \eqref{p3:eq:abstractPk}; the top coefficient of $P_k$ is expressed as a
...
\eqref{p3:eq:abstractmoments}; every lower convolution collapses by the
...
\eqref{p3:eq:halfqMersenne} gives
\[
 \halfqbinom{n}{k}
 =\frac{M_n}{M_kM_{n-k}2^{k(n-k)}}.
\]
Substituting this into the outer coefficient of
\eqref{p3:eq:inverseqformula} shows that, before cancellation with the global
...
The recurrence \eqref{p3:eq:arec} is usually the efficient route for computing
...
For moderate $n$, the terms in \eqref{p3:eq:inverseqformula} can be much larger
...
From \eqref{p3:eq:Ncenteredvalue}, the full centered numerator for $n=2$ is
\[
...
The notation in \eqref{p3:eq:headline} resembles a terminating basic
...
\begin{equation}\label{p3:eq:qderivative}
 D_qf(x)=\frac{f(x)-f(qx)}{(1-q)x}.
\end{equation}
Repeated $q$-differentiation samples a multiplicative grid.  Our operator
$\mathcal D_n$ is not exactly an iterate of \eqref{p3:eq:qderivative}, because
...
\label{p3:tab:notationcrosswalk}\\
\toprule
