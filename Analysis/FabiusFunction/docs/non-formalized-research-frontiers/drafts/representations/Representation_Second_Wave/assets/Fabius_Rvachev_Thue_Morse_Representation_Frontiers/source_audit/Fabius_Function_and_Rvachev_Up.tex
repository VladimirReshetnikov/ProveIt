...
\begin{align}
 F(x)&=0 &&(x\le 0),\label{p3:eq:F-left-clamp}\\
 F(x)&=1 &&(x\ge 1),\label{p3:eq:F-right-clamp}\\
 F(x)+F(1-x)&=1 &&(0\le x\le1),\label{p3:eq:F-symmetry}\\
 F'(x)&=2F(2x) &&(0\le x\le\tfrac12),\label{p3:eq:F-diff}
\end{align}
and by the requirement $F\in C^\infty(\R)$.  The canonical existence and uniqueness theorem is proved in \cref{p3:sec:existence}.
\end{definition}

Thus $F$ is a genuine cumulative distribution function.  On the right half of the unit interval, differentiating \eqref{p3:eq:F-symmetry} gives the companion formula
\begin{equation}\label{p3:eq:F-right-derivative}
...
\begin{equation}\label{p3:eq:thue-morse-sign}
 \eps_n:=(-1)^{s_2(n)}\qquad(n\ge0).
\end{equation}
The \emph{signed global Fabius extension} is
\begin{equation}\label{p3:eq:global-F-def}
...
\begin{equation}\label{p3:eq:up-basic}
 \supp(\upf)=[-1,1],\qquad \upf>0\text{ on }(-1,1),\qquad \upf(0)=1,
\end{equation}
and
\begin{equation}\label{p3:eq:up-differential}
...
\begin{equation}\label{p3:eq:F-up-relations}
 \upf(t)=\GF(t+1)=
 \begin{cases}
   F(t+1),&-1\le t\le0,\\
   F(1-t),&0\le t\le1,
...
The infinite Fourier product underlying $\upf$ appeared in work of Jessen and Wintner in 1935 \cite{p3:JessenWintner}.  Fabius used the associated distribution in 1966 to exhibit a nowhere analytic $C^\infty$ function \cite{p3:Fabius1966}.  Rvachev developed the compactly supported function and related ``atomic functions'' as tools in approximation and numerical analysis \cite{p3:Rvachev1990}.  A systematic account of the defining equation, probability interpretation, Fourier formulas, moment identities, Poisson summation, and dyadic rationality was written by Arias de Reyna in 1980 and later made available in English \cite{p3:Arias05442}.  His arithmetic sequel \cite{p3:Arias06487} studies the rational sequences hidden in the moments and answers an integrality question raised by Reshetnikov.

The formal development \cite{p3:ProveIt} verifies all mathematically substantive statements in those two papers and adds, among other results, an executable exact evaluator, a Fourier--Legendre expansion, finite $q$-binomial/Thue--Morse formulas, and a corrected all-orders endpoint asymptotic.  This article is organized around the mathematics rather than the order of the source files.
...
\section{Existence, uniqueness, and the infinite Fourier product}\label{p3:sec:existence}
...
\begin{equation}\label{p3:eq:unknown-k}
...
\begin{align*}
 ...
 \mathcal F_x[u(2x-1)](z)&=\tfrac12\e^{-\pi\ii z}\widehat u(z/2).
\end{align*}
Taking Fourier transforms in \eqref{p3:eq:unknown-k} therefore yields
\begin{equation}\label{p3:eq:Fourier-refinement-k}
...
\begin{proposition}\label{p3:prop:k-two}
Any nonzero integrable solution of \eqref{p3:eq:unknown-k} with nonzero integral has $\kappa=2$.  Its Fourier transform satisfies
\begin{equation}\label{p3:eq:Fourier-refinement}
...
Iterating \eqref{p3:eq:Fourier-refinement} $N$ times gives
\begin{equation}\label{p3:eq:finite-Fourier-product}
...
\begin{theorem}[Fourier product and uniqueness]\label{p3:thm:Fourier-product}
After normalizing $\int_\R u=1$, every solution of \eqref{p3:eq:up-differential} has
\begin{equation}\label{p3:eq:up-Fourier-product}
 \widehat{\upf}(z)=\prod_{j=0}^{\infty}\frac{\sin(\pi z/2^j)}{\pi z/2^j}.
\end{equation}
Equivalently,
\begin{equation}\label{p3:eq:cos-product}
...
\begin{proof}
The first assertion follows from \eqref{p3:eq:finite-Fourier-product}.  For the second, repeatedly use
\[
...
\begin{equation}\label{p3:eq:finite-convolution}
...
\]
and its transform is the $N$th partial product in \eqref{p3:eq:up-Fourier-product}.
...
\begin{equation}\label{p3:eq:rapid-decay}
...
\begin{theorem}[Existence and positivity]\label{p3:thm:existence-positivity}
The inverse Fourier transform of \eqref{p3:eq:up-Fourier-product} is an even nonnegative $C^\infty$ function supported on $[-1,1]$, with integral one.  It is positive on $(-1,1)$, satisfies \eqref{p3:eq:up-differential}, and has $\upf(0)=1$.  It is therefore the unique function in \cref{p3:eq:up-basic,p3:eq:up-differential}.
\end{theorem}

\begin{proof}
Evenness, nonnegativity, support, and unit integral follow from the convolution construction.  Fourier-transforming \eqref{p3:eq:up-Fourier-product} back through \eqref{p3:eq:Fourier-refinement} proves the differential equation.  Positivity in the open support can be seen already at a finite stage: for any $x\in(-1,1)$ choose $N$ so that $x$ lies in the interior of the support of $\nu_N$; convolution with the remaining nonnegative measures preserves strict positivity near $x$.

It remains to normalize the height.  Integrating \eqref{p3:eq:up-differential} from $-\infty$ to $0$ and using evenness and support gives
\[
 \upf(0)=2\int_{-\infty}^{0}\upf(2x+1)\dd x
       -2\int_{-\infty}^{0}\upf(2x-1)\dd x
       =\int_{-1}^{1}\upf(t)\dd t=1.
\]
The uniqueness statement follows from \cref{p3:thm:Fourier-product}.
\end{proof}

\subsection{Existence and uniqueness of the bounded CDF}

Let $X_j$ be independent uniform random variables on $[0,1]$, and define
\begin{equation}\label{p3:eq:random-S}
...
\begin{equation}\label{p3:eq:F-probability}
 F(x)=\mathbb P(S\le x).
\end{equation}
...
\begin{theorem}[Canonical bounded Fabius function]\label{p3:thm:bounded-existence}
The CDF \eqref{p3:eq:F-probability} is the unique bounded $C^\infty$ function satisfying \eqref{p3:eq:F-left-clamp}--\eqref{p3:eq:F-diff}.  It is strictly increasing on $[0,1]$, with $F(1/2)=1/2$, and is flat at both endpoints:
\begin{equation}\label{p3:eq:F-flat}
...
\begin{equation}\label{p3:eq:uN-derivative}
 u_{N+1}'(x)=2^N\bigl(u_N(x+2^{-N-1})-u_N(x-2^{-N-1})\bigr).
\end{equation}
Rescaling the finite densities to a common dyadic mesh produces the polynomial step approximants used in \cite{p3:Arias05442}.  The endpoint convention matters: closed interval indicators double-count shared mesh points.  Giving each shared endpoint weight $1/2$ preserves the normalization and pointwise convergence, exactly as recorded in the formal proof \cite{p3:ProveIt}.

\begin{proposition}[Approximate identity]\label{p3:prop:approximate-identity}
For every continuous $g$ on $[-1,1]$,
\begin{equation}\label{p3:eq:approx-identity}
...
The random series \eqref{p3:eq:random-S} has a useful self-similar decomposition.  Let $S'$ be an independent copy of $S$.  Then
\begin{equation}\label{p3:eq:S-fixed-point}
 S\ \stackrel{d}{=}\ \frac{X+S'}2,
 \qquad X\sim U[0,1].
\end{equation}
For $0\le x\le1$, conditioning on $S'$ gives
\begin{equation}\label{p3:eq:F-integral-fixed-point}
 F(x)=\int_0^1\mathbb P(X\le2x-s)\dd F(s).
\end{equation}
On $0\le x\le1/2$, $2x-s\le1$, and integration by parts reduces this to
\begin{equation}\label{p3:eq:F-integral-simple}
...
\begin{equation}\label{p3:eq:T-fixed-point}
 T\ \stackrel{d}{=}\ \frac{U+T'}2,
 \qquad U\sim U[-1,1].
\end{equation}
The density equation associated with \eqref{p3:eq:T-fixed-point} is
\begin{equation}\label{p3:eq:up-integral-equation}
 \upf(x)=\int_{2x-1}^{2x+1}\frac12\,\upf(t)\dd t.
\end{equation}
Differentiating gives \eqref{p3:eq:up-differential}.  Conversely, integrating \eqref{p3:eq:up-differential} from $-\infty$ to $x$ and using the support condition recovers \eqref{p3:eq:up-integral-equation}.

\begin{corollary}[Probability interpretation of the bump]\label{p3:cor:up-probability}
For $-1\le x\le0$,
\begin{equation}\label{p3:eq:up-CDF}
...
The integral equation immediately shows that $F$ is flatter than any power.  Iterating \eqref{p3:eq:F-integral-simple} and using $0\le F\le1$ gives, for $0\le x\le2^{-n}$,
\begin{equation}\label{p3:eq:basic-flat-bound}
...
\begin{corollary}[Qualitative flatness]\label{p3:cor:superpolynomial}
For every $A>0$,
\begin{equation}\label{p3:eq:F-superpolynomial}
 F(x)=\littleoh(x^A)\qquad(x\downarrow0).
\end{equation}
On the other hand, for every $c>0$,
\begin{equation}\label{p3:eq:F-slower-exp}
...
\begin{proof}
Choose an integer $n>A$ in \eqref{p3:eq:basic-flat-bound}; this proves \eqref{p3:eq:F-superpolynomial}.  The second assertion follows already from the quadratic logarithmic estimate proved in \cref{p3:thm:coarse-asymptotic}:
$\log F(x)\sim-(\log x)^2/(2\log2)$, whose magnitude is $o(1/x)$.
\end{proof}

\section{The global extension and exact derivative identities}\label{p3:sec:global}

\subsection{Thue--Morse translation and the dilation equation}

The Thue--Morse signs satisfy
\begin{equation}\label{p3:eq:TM-recursion}
 \eps_{2n}=\eps_n,\qquad \eps_{2n+1}=-\eps_n.
\end{equation}
This two-scale relation is precisely what is needed to turn the translated bump sum \eqref{p3:eq:global-F-def} into a global solution of a simple differential equation.

\begin{theorem}[Global differential equation]\label{p3:thm:global-differential}
The signed extension $\GF$ is $C^\infty$, vanishes on $(-\infty,0]$, agrees with $F$ on $[0,1]$, and satisfies
\begin{equation}\label{p3:eq:global-diff}
 \GF'(x)=2\GF(2x)\qquad(x\in\R).
\end{equation}
More generally,
\begin{equation}\label{p3:eq:global-higher-derivatives}
 \GF^{(m)}(x)=2^{\binom{m+1}{2}}\GF(2^m x)
 \qquad(m\ge0,\ x\in\R).
\end{equation}
On $[-1,1]$,
\begin{equation}\label{p3:eq:up-higher-derivatives}
 \upf^{(m)}(x)=2^{\binom{m+1}{2}}\GF\bigl(2^m(x+1)\bigr).
\end{equation}
\end{theorem}

\begin{proof}
Local finiteness permits termwise differentiation in \eqref{p3:eq:global-F-def}.  By \eqref{p3:eq:up-differential},
\begin{align*}
 \GF'(x)
 ...
 &=2\GF(2x),
\end{align*}
where \eqref{p3:eq:TM-recursion} was used in the second line.  Iterating gives \eqref{p3:eq:global-higher-derivatives}.  On $[-1,1]$, $\upf(x)=\GF(x+1)$, so \eqref{p3:eq:up-higher-derivatives} follows by differentiation.
\end{proof}
...
Let $x=a/2^n$ with $a\in\Z$.  If $m>n$, then $2^m x$ is an even integer, hence $\GF(2^m x)=0$.  By \eqref{p3:eq:global-higher-derivatives},
\begin{equation}\label{p3:eq:dyadic-derivative-vanishing}
 \GF^{(m)}(a/2^n)=0\qquad(m>n).
\end{equation}
The same statement holds for $\upf$ at dyadic points of $[-1,1]$, after translating by one.

\begin{theorem}[Exact dyadic Taylor polynomial]\label{p3:thm:dyadic-Taylor}
...
\begin{equation}\label{p3:eq:dyadic-Taylor}
 \GF(a/2^n+h)=
 \sum_{m=0}^{n}\frac{2^{\binom{m+1}{2}}\GF(2^{m-n}a)}{m!}h^m
 +\mathcal R_{n,a}(h),
\end{equation}
where the remainder can be reflected, by the global dilation identity, into a signed copy of $\GF$ at a smaller argument.  In the highest-bit situation of \cref{p3:thm:highest-bit}, it is exactly $-F(h)$.
\end{theorem}
...
\begin{theorem}[Fabius's nowhere-analytic example]\label{p3:thm:nowhere-analytic}
Neither $F$ on $(0,1)$ nor $\upf$ on $(-1,1)$ is real analytic at any point.
\end{theorem}

\begin{proof}
At a dyadic point $x=a/2^n$, \eqref{p3:eq:dyadic-derivative-vanishing} says that the Taylor series is a polynomial.  If the function were analytic there, it would equal that polynomial in a neighborhood.  Repeatedly applying the dilation equation to such a polynomial identity propagates a polynomial identity to an interval reaching a flat endpoint.  A polynomial whose derivatives of every order vanish at an endpoint is identically zero, contradicting strict positivity in the interior.

Now suppose the function were analytic at a nondyadic point $x_0$.  Analyticity holds on some open interval about $x_0$, and that interval contains a dyadic point.  The restriction to that dyadic point would then also be analytic, contradicting the first paragraph.  The argument for $\upf$ and for $F$ is the same after the translation/reflection relations \eqref{p3:eq:F-up-relations}.
\end{proof}

\section{Fourier analysis, Poisson summation, and orthogonal expansions}\label{p3:sec:fourier}

\subsection{Zeros, entire continuation, and inversion}

The product \eqref{p3:eq:up-Fourier-product} converges locally uniformly in $z\in\C$, so $\widehat{\upf}$ is entire.  Its zeros are transparent: every nonzero integer is a zero, with a multiplicity determined by its $2$-adic valuation.  More precisely, if $m\ne0$ and $m=2^r q$ with $q$ odd, then the factors indexed by $j=0,\ldots,r$ vanish, so the zero at $m$ has multiplicity $r+1$.

Fourier inversion gives
\begin{equation}\label{p3:eq:up-inversion}
 \upf(x)=\int_{\R}\left(\prod_{j=0}^{\infty}\sinc(t/2^j)\right)
                    \e^{2\pi\ii xt}\dd t.
\end{equation}
Because the product is real and even on the real axis,
\begin{equation}\label{p3:eq:up-cosine-integral}
 \upf(x)=2\int_0^\infty\left(\prod_{j=0}^{\infty}\sinc(t/2^j)\right)
                    \cos(2\pi xt)\dd t.
\end{equation}
The rapid decay \eqref{p3:eq:rapid-decay} justifies differentiating under the integral any number of times.
...
\begin{equation}\label{p3:eq:Legendre-orthogonality}
 \int_{-1}^{1}P_m(x)P_n(x)\dd x=\frac{2}{2n+1}\delta_{mn}.
\end{equation}
Since $\upf$ is even, only even degrees occur.  Define
\begin{equation}\label{p3:eq:Legendre-coeff-def}
 u_n:=\frac{4n+1}{2}\int_{-1}^{1}\upf(x)P_{2n}(x)\dd x.
\end{equation}

\begin{theorem}[Exact Legendre coefficients]\label{p3:thm:Legendre-coefficients}
For $n\ge0$,
\begin{equation}\label{p3:eq:Legendre-coefficients}
\begin{split}
...
\end{equation}
Moreover,
\begin{equation}\label{p3:eq:Legendre-series}
...
\]
and the flatness of $\upf$ at $\pm1$ permit $2n$ integrations by parts in \eqref{p3:eq:Legendre-coeff-def}.  The derivative identity \eqref{p3:eq:up-higher-derivatives} converts $\upf^{(2n)}$ into a scaled global Fabius value.  Expanding $(x^2-1)^{2n}$ and reducing the resulting even moments by \cref{p3:thm:moment-identities} gives the finite sum \eqref{p3:eq:Legendre-coefficients}.  A direct rearrangement using the explicit monomial formula for $P_{2n}$ gives the displayed binomial factors.
...
\begin{proposition}[Least-squares optimality]\label{p3:prop:Legendre-least-squares}
Let $S_N(x)=\sum_{n=0}^{N}u_nP_{2n}(x)$.  For every polynomial $q$ of degree at most $2N+1$,
\begin{equation}\label{p3:eq:Legendre-Pythagorean}
 \int_{-1}^{1}(\upf-q)^2
...
\begin{proof}
The difference $\upf-S_N$ is orthogonal to every $P_j$ with $j\le2N+1$: the even coefficients through $2N$ have been projected out, and all odd coefficients vanish by parity.  Expanding $\upf-q=(\upf-S_N)+(S_N-q)$ proves \eqref{p3:eq:Legendre-Pythagorean}.
\end{proof}


\section{Moments and entire generating functions}\label{p3:sec:moments}

\subsection{Even moments of the up function}

Define
\begin{equation}\label{p3:eq:cndef}
...
\begin{equation}\label{p3:eq:Fourier-moment-series}
...
\begin{theorem}[Moment recurrence]\label{p3:thm:c-recurrence}
...
\begin{equation}\label{p3:eq:c-values}
...
$T\stackrel d=(U+T')/2$ from \eqref{p3:eq:T-fixed-point}.  Since odd moments vanish,
\begin{align*}
 c_n
...
Separating the term $k=n$, equal to $(2n+1)c_n$, yields \eqref{p3:eq:c-recurrence}.  Rationality follows by induction.
\end{proof}

\subsection{Half moments and inverse dyadic values}

Define the exponential generating function
\begin{equation}\label{p3:eq:f-generating}
...
\begin{equation}\label{p3:eq:f-refinement}
 f(2z)=\frac{\e^z-1}{z}f(z),
\end{equation}
where the quotient is interpreted as $1$ at $z=0$.

Write
\begin{equation}\label{p3:eq:d-generating}
 f(z)=\sum_{n=0}^{\infty}\frac{d_n}{n!}z^n.
\end{equation}

\begin{theorem}[Half-moment identities]\label{p3:thm:moment-identities}
The coefficients $d_n$ satisfy
\begin{align}
 d_0&=1,\label{p3:eq:d0}\\
 (n+1)(2^n-1)d_n
 &=\sum_{k=0}^{n-1}\binom{n+1}{k}d_k\qquad(n\ge1),\label{p3:eq:d-recurrence}\\
 d_n&=n\int_0^1t^{n-1}\upf(t)\dd t\qquad(n\ge1),\label{p3:eq:d-half-moment}\\
 d_n&=n!\,2^{\binom n2}F(2^{-n}),\label{p3:eq:d-inverse-dyadic}\\
 d_n&=2^{-n}\sum_{k=0}^{\floor{n/2}}\binom n{2k}c_k.\label{p3:eq:d-from-c}
\end{align}
In particular all $d_n$ and all inverse-dyadic Fabius values are rational.
\end{theorem}

\begin{proof}
Comparing coefficients in \eqref{p3:eq:f-refinement} gives \eqref{p3:eq:d-recurrence}.  Expanding \eqref{p3:eq:f-generating} term by term gives \eqref{p3:eq:d-half-moment}.

To prove \eqref{p3:eq:d-inverse-dyadic}, use the derivative identity for $F$:
$F^{(n)}(x)=2^{\binom{n+1}{2}}F(2^nx)$ whenever $0\le x\le2^{-n}$.  Integrating by parts repeatedly in \eqref{p3:eq:d-half-moment}, or using the moment identity in Arias de Reyna's first paper, yields
\[
...
For \eqref{p3:eq:d-from-c}, integrate by parts once and use \eqref{p3:eq:up-differential}:
\begin{align*}
 d_n
...
\begin{proposition}\label{p3:prop:moment-endpoint}
For every integer $n\ge1$,
\begin{equation}\label{p3:eq:moment-endpoint}
 \int_0^1t^{n-1}\upf(t)\dd t
 =(n-1)!\,2^{\binom n2}\upf(1-2^{-n}).
\end{equation}
Consequently
\begin{equation}\label{p3:eq:even-moment-endpoint}
 \frac{c_n}{2}=(2n)!\,2^{\binom{2n+1}{2}}F(2^{-2n-1}).
\end{equation}
\end{proposition}

\begin{proof}
Equation \eqref{p3:eq:moment-endpoint} is \eqref{p3:eq:d-half-moment} combined with \eqref{p3:eq:d-inverse-dyadic}, divided by $n$.  Taking $n\mapsto2n+1$ and using evenness gives \eqref{p3:eq:even-moment-endpoint}.
\end{proof}

\subsection{Bernoulli recurrences}

Let $B_n$ be defined by
\begin{equation}\label{p3:eq:Bernoulli-gen}
...
\begin{proposition}[Bernoulli recurrences]\label{p3:prop:Bernoulli-recurrences}
For $n\ge1$,
\begin{equation}\label{p3:eq:c-Bernoulli}
 c_n=\frac{1}{2^{2n}-1}\sum_{k=1}^{n}
 2^{2n-2k}(2^{2k}-2)\binom{2n}{2k}B_{2k}c_{n-k},
\end{equation}
and
\begin{equation}\label{p3:eq:d-Bernoulli}
...
\begin{proof}
From \eqref{p3:eq:Fourier-refinement},
\[
 \frac{\pi z}{\sin\pi z}\widehat{\upf}(z)=\widehat{\upf}(z/2),
\]
and from \eqref{p3:eq:f-refinement},
\[
...
\]
and \eqref{p3:eq:Bernoulli-gen}; coefficient comparison and separation of the leading term give \eqref{p3:eq:c-Bernoulli} and \eqref{p3:eq:d-Bernoulli}.
\end{proof}

\section{Exact values at dyadic rationals}\label{p3:sec:dyadic}
...
\begin{theorem}[Moment--Thue--Morse dyadic formula]\label{p3:thm:closed-dyadic}
For $0\le a\le2^n$,
\begin{equation}\label{p3:eq:closed-dyadic}
\boxed{
...
\begin{equation}\label{p3:eq:F-integral-Taylor}
 F(x)=\frac{1}{n!}\int_0^x(x-t)^nF^{(n+1)}(t)\dd t.
\end{equation}
Set $x=a/2^n$ and split the integral into the $2a$ half-unit cells that arise after the substitution $u=2^{n+1}t$.  On each cell, the locally finite representation \eqref{p3:eq:global-F-def} reduces $\GF(u)$ to a translate of $\upf$, with sign $\eps_h$.  Pair the two half-cells around the center $2h+1$, translate to $[-1,1]$, and expand the polynomial kernel.  Odd moments vanish by evenness of $\upf$; the surviving moment of degree $2k$ is $c_k$.  Collecting the scale factor from \eqref{p3:eq:F-integral-Taylor} gives exactly \eqref{p3:eq:closed-dyadic}.  The endpoint cases also follow from $F(0)=0$ and $F(1)=1$; the finite sum reproduces them because the same derivation remains valid.
\end{proof}

\begin{example}\label{p3:ex:five-sixteenths}
With $a=5$, $n=4$, and $c_0=1$, $c_1=1/9$, $c_2=19/675$, formula \eqref{p3:eq:closed-dyadic} gives
\begin{equation}\label{p3:eq:five-sixteenths}
...
\subsection{The highest-bit recursion}\label{p3:subsec:highest-bit}
...
\begin{equation}\label{p3:eq:highest-bit-range}
 2^{-n}\le x<2^{-n+1}.
\end{equation}
...
\begin{equation}\label{p3:eq:derivative-at-inverse-dyadic}
 F^{(k)}(2^{-n})
...
\begin{theorem}[Highest-bit Taylor identity]\label{p3:thm:highest-bit}
Under \eqref{p3:eq:highest-bit-range},
\begin{equation}\label{p3:eq:highest-bit}
\boxed{
 F(x)=-F(y)+
 \sum_{k=0}^{n}
 2^{\binom{k+1}{2}-\binom{n-k}{2}}
...
\begin{equation}\label{p3:eq:highest-bit-Taylor-proof}
 F(y+2^{-n})=
 \sum_{k=0}^{n}\frac{F^{(k)}(2^{-n})}{k!}y^k
 +\frac1{n!}\int_{2^{-n}}^{2^{-n}+y}
       (2^{-n}+y-t)^nF^{(n+1)}(t)\dd t.
\end{equation}
The derivative formula \eqref{p3:eq:derivative-at-inverse-dyadic} gives the finite sum.  For the remainder, substitute $t=2^{-n}+u$ and use the global dilation identity at order $n+1$.  On $0<u<y<2^{-n}$, the scaled arguments lie in adjacent unit cells, where the derivative of the signed extension changes sign under translation by one.  This turns the remainder into
\[
 -\frac1{n!}\int_0^y(y-u)^nF^{(n+1)}(u)\dd u.
\]
All derivatives of $F$ vanish at $0$, so Taylor's theorem at $0$ identifies the last integral with $-F(y)$.  This proves \eqref{p3:eq:highest-bit}.
\end{proof}

For $x=a/2^N\in(0,1)$, the exponent $n$ in \eqref{p3:eq:highest-bit-range} identifies the highest set bit of $a$.  Replacing $x$ by $y$ clears that bit and leaves all lower bits unchanged.  Repetition reaches zero after exactly $s_2(a)$ steps.

\begin{algorithm}[Exact dyadic evaluator]\label{p3:alg:dyadic-evaluator}
...
\item While the numerator is nonzero, remove its highest set bit.  Evaluate the polynomial in \eqref{p3:eq:highest-bit} by Horner's rule, and replace the unknown $F(x)$ by the resulting polynomial minus the smaller unknown $F(y)$.
\item Back-substitute the accumulated alternating affine expressions.
\end{enumerate}
The number of rational operations is on the order of
\begin{equation}\label{p3:eq:dyadic-complexity}
...
\caption{Selected exact values of the bounded Fabius function.}\label{p3:tab:dyadic-values}\\
\toprule
$x$ & $F(x)$ & decimal value\\
\midrule
\endfirsthead
...
\section{Arithmetic structure: hidden integers, denominators, and parity}\label{p3:sec:arithmetic}

\subsection{The integer numerators $F_n$ and $G_n$}

The recurrences for $c_n$ and $d_n$ hide two natural integer sequences.  Define
\begin{equation}\label{p3:eq:Fn-def}
 F_n:=c_n(2n+1)!!\prod_{j=1}^{n}(2^{2j}-1).
\end{equation}
Substitution into \eqref{p3:eq:c-recurrence} gives a division-free recurrence:
\begin{equation}\label{p3:eq:Fn-recurrence}
...
\begin{equation}\label{p3:eq:Fn-values}
 1,\ 1,\ 19,\ 2915,\ 2788989,\ 14754820185,\ldots
\end{equation}

Similarly define
\begin{equation}\label{p3:eq:Gn-def}
 G_n:=d_n(n+1)!\prod_{j=1}^{n}(2^j-1).
\end{equation}
Then
\begin{equation}\label{p3:eq:Gn-recurrence}
 G_n=\sum_{k=0}^{n-1}G_k\binom{n+1}{k}
 \frac{n!}{(k+1)!}\prod_{j=k+1}^{n-1}(2^j-1),
\end{equation}
which proves $G_n\in\N$.  The first terms are
\begin{equation}\label{p3:eq:Gn-values}
 1,\ 1,\ 5,\ 84,\ 4004,\ 494760,\ 150120600,\ldots
\end{equation}
Combining \eqref{p3:eq:d-inverse-dyadic} and \eqref{p3:eq:Gn-def} gives
\begin{equation}\label{p3:eq:F-inverse-Gn}
 F(2^{-n})=
 \frac{2^{-\binom n2}G_n}
 {n!(n+1)!\displaystyle\prod_{j=1}^{n}(2^j-1)}.
\end{equation}

\begin{proposition}\label{p3:prop:F-G-relation}
For $n\ge0$,
\begin{equation}\label{p3:eq:F-G-relation}
 \frac{G_{2n+1}}{2n+1}
...
\[
 ...
 =\frac{2n+1}{2}c_n.
\]
Insert the definitions \eqref{p3:eq:Fn-def} and \eqref{p3:eq:Gn-def} and simplify the even and odd factors of $\prod_{j=1}^{2n+1}(2^j-1)$.
\end{proof}

\subsection{Reshetnikov's integers}

For $n\ge1$, define
\begin{equation}\label{p3:eq:Rn-def}
...
\begin{theorem}[Integrality of Reshetnikov's numbers]\label{p3:thm:Rn-integral}
Every $R_n$ is a positive integer.  The sequence begins
\begin{equation}\label{p3:eq:Rn-values}
 1,\ 5,\ 15,\ 1001,\ 5985,\ 2853675,\ 26261235,\ldots
\end{equation}
\end{theorem}

\begin{proof}
The odd case is \eqref{p3:eq:Rodd}.  By \eqref{p3:eq:Rn-dn}, the $2$-adic valuation of $R_n$ equals that of $2d_n$, because every other factor is odd.  We prove $\nu_2(2d_n)\ge0$ by induction.  Odd indices are already covered.  For an even index $2n$, multiply \eqref{p3:eq:d-recurrence} by two:
\begin{equation}\label{p3:eq:even-d-recurrence}
 (2n+1)(2^{2n}-1)\,2d_{2n}
 =\sum_{k=0}^{2n-1}\binom{2n+1}{k}\,2d_k.
\end{equation}
The multiplier on the left is odd.  By the induction hypothesis, every summand on the right is $2$-adically integral, hence so is $2d_{2n}$.  Since \eqref{p3:eq:Reven-sum} shows that no odd prime occurs in a denominator, $R_{2n}$ is an integer.  Positivity follows from $d_n>0$.
\end{proof}
...
\begin{theorem}[Oddness theorem]\label{p3:thm:oddness}
Every integer $F_n$ is odd.  In lowest terms, both numerator and denominator of $2d_n$ are odd.  Consequently every $R_n$ is odd.
\end{theorem}

\begin{proof}
Reduce \eqref{p3:eq:Fn-recurrence} modulo two.  All product factors $2^{2j}-1$ and odd-double-factorial quotients are odd.  Lucas's theorem determines the parity of $\binom{2n+1}{2k}$ and causes the recurrence to reduce to a sum whose parity is one; induction gives $F_n\equiv1\pmod2$.  For odd $m=2n+1$, \eqref{p3:eq:Rodd} is therefore odd, hence $2d_m$ is a quotient of odd integers by \eqref{p3:eq:Rn-dn}.  The even case follows inductively from \eqref{p3:eq:even-d-recurrence}; modulo two, precisely the terms needed to leave an odd numerator survive.  Finally \eqref{p3:eq:Rn-dn} multiplies $2d_n$ only by odd integers.
\end{proof}

\begin{corollary}[Valuation at inverse powers of two]\label{p3:cor:inverse-dyadic-valuation}
For $n\ge1$,
\begin{equation}\label{p3:eq:inverse-dyadic-valuation}
 \nu_2\bigl(F(2^{-n})\bigr)
 =-\binom n2-1-\nu_2(n!).
\end{equation}
...
\begin{proof}
From \eqref{p3:eq:d-inverse-dyadic},
$F(2^{-n})=d_n/(n!2^{\binom n2})$.  Theorem \ref{p3:thm:oddness} says $\nu_2(d_n)=-1$.
\end{proof}
...
\[
 ...
 \qquad |2k+1|<2^n.
\]
The closed formula \eqref{p3:eq:closed-dyadic} expresses every such value as an integer linear combination of $c_0,\ldots,c_{\floor{n/2}}$, after multiplication by $n!2^{\binom{n+1}{2}}$.  Clearing the known denominators of the $c_k$ gives the following uniform bound.

\begin{theorem}[Dyadic denominator bound]\label{p3:thm:denominator-bound}
For $n\ge1$ and every integer $a$ with $|a|<2^n$,
\begin{equation}\label{p3:eq:denominator-bound}
 \upf(a/2^n)\,n!\,2^{\binom{n+1}{2}}
 (2\floor{n/2}+1)!!
 \prod_{k=1}^{\floor{n/2}}(2^{2k}-1)
 \in\N.
\end{equation}
Therefore $D_n$ divides the integer multiplying $\upf(a/2^n)$ in \eqref{p3:eq:denominator-bound}.
\end{theorem}

\begin{proof}
Put $m=\floor{n/2}$.  Formula \eqref{p3:eq:closed-dyadic}, translated from $F$ to $\upf$, shows that
$n!2^{\binom{n+1}{2}}\upf(a/2^n)$ is an integer linear combination of $c_0,\ldots,c_m$.  From \eqref{p3:eq:Fn-def},
\[
...
The exact pattern of the least common denominators is subtler.  Arias de Reyna formulated a three-part conjecture concerning normalized values of $D_n$; the Lean development formalizes the conjecture as a statement but, correctly, does not treat it as proved \cite{p3:Arias06487,p3:ProveIt}.

\section{A finite $q$-binomial--Thue--Morse formula}\label{p3:sec:qbinomial}
...
\begin{equation}\label{p3:eq:q-notation}
...
\begin{theorem}[Arbitrary-numerator finite formula]\label{p3:thm:qbinomial-dyadic}
For every $m,n\in\N$ and every common translation $c\in\Q$,
\begin{equation}\label{p3:eq:qbinomial-dyadic}
\boxed{
\begin{aligned}
...
\begin{equation}\label{p3:eq:q-refinement-invariance}
 Q(m,n;c)=Q(2m,n+1;c),
\end{equation}
where $Q$ denotes the finite expression on the right of \eqref{p3:eq:qbinomial-dyadic}.

\subsection{Thue--Morse annihilation}

The engine of the proof is the elementary product
\begin{equation}\label{p3:eq:TM-exponential-product}
 \sum_{r=0}^{2^j-1}\eps_r\e^{rz}
 =\prod_{h=0}^{j-1}(1-\e^{2^h z}).
\end{equation}
It follows by induction from \eqref{p3:eq:TM-recursion}.  The right side has a zero of order $j$ at $z=0$, hence
\begin{equation}\label{p3:eq:TM-power-annihilation}
 \sum_{r=0}^{2^j-1}\eps_r r^\ell=0
 \quad(0\le\ell<j),
\end{equation}
and differentiation at order $j$ gives
\begin{equation}\label{p3:eq:TM-leading-power}
 \sum_{r=0}^{2^j-1}\eps_r r^j
 =(-1)^j j!\,2^{\binom j2}.
\end{equation}
More generally, the same vanishing applies to any translated power $(r+c)^\ell$ of degree $<j$.

For an arbitrary prefix length $m2^j$, split into $m$ blocks of length $2^j$.  Since $\eps_{b2^j+r}=\eps_b\eps_r$ when $0\le r<2^j$, one gets a product of a prefix polynomial in the block index and the basic factor \eqref{p3:eq:TM-exponential-product}.  This is the reason that arbitrary numerators $m$ remain tractable.

\subsection{The $q$-binomial cancellation}

The finite $q$-binomial theorem states
\begin{equation}\label{p3:eq:finite-q-binomial}
 \prod_{j=0}^{n-1}(1-q^jX)
 =\sum_{k=0}^{n}(-1)^k q^{\binom k2}
   \GaussBinom{n}{k}{q}X^k.
\end{equation}
At $q=1/2$, this identity supplies exactly the weights in \eqref{p3:eq:qbinomial-dyadic} after a harmless reversal of index and extraction of powers of two.

A convenient proof organization uses exponential generating functions.  For fixed $m,n$, define
\begin{equation}\label{p3:eq:prefix-exp}
 H_{m,k}(z;c):=
 \sum_{r=0}^{m2^k-1}\eps_r\e^{(r-m2^k+c)z}.
\end{equation}
Then the inner power sum in \eqref{p3:eq:qbinomial-dyadic} is $H_{m,k}^{(n+k)}(0;c)$.  Form the weighted combination
\begin{equation}\label{p3:eq:weighted-H}
 \mathcal H_{m,n}(z;c):=
 \sum_{k=0}^{n}
 \frac{\GaussBinom{n}{k}{1/2}}{2^{k(k-1)}}
 z^{-k}H_{m,k}(z;c),
\end{equation}
where the apparent singularities are removable because $H_{m,k}$ vanishes to order $k$ at zero.  The Thue--Morse product and the finite $q$-binomial theorem collapse this sum to the $n$th coefficient of the product generating function $f$ in \eqref{p3:eq:f-mgf}, evaluated over the prefix determined by $m$.  Extracting the coefficient of $z^n$ yields \eqref{p3:eq:qbinomial-dyadic}.

\begin{proof}[Proof of \cref{p3:thm:qbinomial-dyadic}]
We spell out the three algebraic points that make the formal proof work.

\emph{Step 1: divisibility at the origin.}
By block decomposition and \eqref{p3:eq:TM-exponential-product}, $H_{m,k}(z;c)$ is divisible as a formal power series by
$\prod_{h=0}^{k-1}(1-\e^{2^hz})$, hence by $z^k$.  Therefore $z^{-k}H_{m,k}$ is a genuine power series.

\emph{Step 2: coefficient isolation.}
Insert the product representation into \eqref{p3:eq:weighted-H} and apply \eqref{p3:eq:finite-q-binomial} with $q=1/2$.  Terms of total degree below $n$ cancel, while the degree-$n$ term is multiplied by
$2^{n^2}(1/2;1/2)_n$.  The surviving coefficient is the coefficient that, through \eqref{p3:eq:d-generating} and the dyadic derivative identity, equals $\GF(m/2^n)$.
...
Formula \eqref{p3:eq:closed-dyadic} uses only the even moments $c_k$ and an outer sum of length $a$.  Formula \eqref{p3:eq:qbinomial-dyadic} uses $q$-binomial weights and inner prefixes of lengths $m2^k$.  They arise from different eliminations of the same generating-series identity.  The former is usually better for hand calculation and denominator bounds; the highest-bit recursion is much faster computationally; the $q$-binomial formula is structurally strongest because it is global, representation-independent, and translation-invariant.


\section{The small-argument problem and its natural scales}\label{p3:sec:asymptotic}

The flatness statement \eqref{p3:eq:F-superpolynomial} leaves a large gap between
...
\begin{equation}\label{p3:eq:L-def}
 L:=\log 2.
\end{equation}
For $0<x<1/(\e L)$ define
\begin{equation}\label{p3:eq:lambda-def}
 \lambda(x):=-\frac{1}{L}W_{-1}(-Lx),
 \qquad r(x):=2^{\lambda(x)},
\end{equation}
where $W_{-1}$ is the lower real branch of the Lambert $W$ function\cite{p3:CorlessLambertW}.  The
defining identities are
\begin{equation}\label{p3:eq:lambda-identities}
 \lambda 2^{-\lambda}=x,
 \qquad r x=\lambda,
 \qquad \log r=L\lambda.
\end{equation}
Thus $\lambda(x)\to\infty$ as $x\downarrow0$.  If $x=2^{-t}$, then
\begin{equation}\label{p3:eq:lambda-dyadic-fixed-point}
...
Recall the entire moment-generating function $f$ from \eqref{p3:eq:f-mgf}.  On
the negative real axis,
\begin{equation}\label{p3:eq:negative-Laplace-product}
 f(-s)=\mathbb E\e^{-sS}
 =\prod_{j=1}^{\infty}
...
\end{equation}
Write
\begin{equation}\label{p3:eq:ell-def}
 \ell(s):=\log f(-s).
\end{equation}
Splitting off the first factor gives the exact dilation relation
\begin{equation}\label{p3:eq:ell-dilation}
...
Define
\begin{equation}\label{p3:eq:forward-tail-error}
 E(s):=-\sum_{n=0}^{\infty}\log(1-\e^{-s2^n}).
\end{equation}
Every summand is positive.  For $s>0$,
\begin{equation}\label{p3:eq:tail-error-bound}
 0\le E(s)\le
 \frac{\e^{-s}}{(1-\e^{-s})^2},
 \qquad
...
\begin{theorem}[Exact quadratic--periodic decomposition]\label{p3:thm:Laplace-periodic}
There is a smooth one-periodic real function $R$ such that, for every $s>0$,
\begin{equation}\label{p3:eq:Laplace-periodic}
\boxed{
...
\begin{equation}\label{p3:eq:R-construct}
...
Using \eqref{p3:eq:ell-dilation},
\begin{align*}
 R(t+1)-R(t)
...
\end{align*}
Hence $R$ is one-periodic.  Solving \eqref{p3:eq:R-construct} for $\ell$ gives
\eqref{p3:eq:Laplace-periodic}, because the last sum is $-E(2^t)$.
The estimates \eqref{p3:eq:tail-error-bound} follow from
...
\begin{proof}
The construction \eqref{p3:eq:R-construct} periodizes a regularized version of
...
\begin{equation}\label{p3:eq:Bose-Mellin}
...
\eqref{p3:eq:Bose-Mellin} then gives \eqref{p3:eq:Psi-Fourier-coeff}.  The Gamma
...
The mean formula \eqref{p3:eq:R-mean} is the constant term of the same
finite-part calculation: the Laurent expansions of $\Gamma$ and $\zeta$ at
zero and one give \eqref{p3:eq:gmathfrak}, while integration of the removed
...
Combining the mean \eqref{p3:eq:R-mean} with the Gaussian normalization in the
saddle integral produces the constant
\begin{equation}\label{p3:eq:Csharp}
 C_\sharp:=
 \frac{6\gamma^2+12\gamma_1-\pi^2}{12L}
 -\frac{7L}{12}-\frac12\log\pi.
\end{equation}
Define
\begin{equation}\label{p3:eq:Msharp}
\boxed{
...
\begin{theorem}[Corrected sharp asymptotic]\label{p3:thm:sharp-asymptotic}
As $x\downarrow0$,
\begin{equation}\label{p3:eq:sharp-asymptotic}
 \boxed{\log F(x)=M_\sharp(x)+\Oh(\lambda(x)^{-1}).}
\end{equation}
Equivalently,
\begin{equation}\label{p3:eq:sharp-F-equivalent}
...
\begin{equation}\label{p3:eq:Msharp-W}
 M_\sharp(x)=
...
\begin{equation}\label{p3:eq:scaled-kernel}
 K_{x}(v):=
 \e^{\ii\sqrt\lambda v}
...
\eqref{p3:eq:Bromwich} is exact and gives
\begin{equation}\label{p3:eq:exact-saddle-factorization}
\boxed{
...
The exact decomposition \eqref{p3:eq:Laplace-periodic} gives, after elementary
algebra,
\begin{equation}\label{p3:eq:exact-saddle-action}
 rx+\ell(r)-\frac12\log(2\pi\lambda)
 =M_\sharp(x)+E(r).
\end{equation}
Thus all of the remaining analytic work is concentrated in
\begin{equation}\label{p3:eq:kernel-target}
 \mathcal K(x)=1+\Oh(\lambda^{-1}).
\end{equation}
Once this is proved, taking logarithms in
\eqref{p3:eq:exact-saddle-factorization}, using
$E(r)=\Oh(\e^{-r})$, gives \eqref{p3:eq:sharp-asymptotic}.

\begin{proof}[Proof of \eqref{p3:eq:kernel-target}]
Write $u=\lambda\bmod1$.  On a central range
$|v|\le\lambda^\eta$ with a small fixed $\eta>0$, expand the logarithm of
\eqref{p3:eq:scaled-kernel}.  The linear term from $\e^{\ii\sqrt\lambda v}$
cancels the dominant linear term from $\ell$; the quadratic term is
$-v^2/2$.  Uniform smoothness of $\Psi$ gives
\begin{equation}\label{p3:eq:kernel-central-expansion}
...
\eqref{p3:eq:kernel-central-expansion} leaves $1+\Oh(\lambda^{-1})$ after
integration.
...
on the far tail, the denominator in \eqref{p3:eq:scaled-kernel} and repeated
...
\begin{equation}\label{p3:eq:A1-def}
\boxed{
 A_1(u):=\frac1{24}+\frac1{2L}
 -\frac{\Psi''(u)+(\Psi'(u))^2}{2L^2}.}
\end{equation}

\begin{theorem}[First saddle correction]\label{p3:thm:first-correction}
As $x\downarrow0$,
\begin{equation}\label{p3:eq:first-correction}
 \log F(x)=M_\sharp(x)
...
\eqref{p3:eq:kernel-central-expansion}, the relative mass coefficient is
\begin{equation}\label{p3:eq:gaussian-contraction}
 \mathbb E\left[P_2(Z;u)+\frac12P_1(Z;u)^2\right].
\end{equation}
Use
$\mathbb EZ^2=1$, $\mathbb EZ^4=3$, and $\mathbb EZ^6=15$.
Substitution of \eqref{p3:eq:P1}--\eqref{p3:eq:B-Psi} and simplification gives
exactly \eqref{p3:eq:A1-def}.  Since the leading normalized mass is one, the
...
\begin{theorem}[All-orders endpoint expansion]\label{p3:thm:all-orders}
For every integer $N\ge1$, there exist smooth bounded one-periodic functions
$A_1,\ldots,A_{N-1}$, with $A_1$ given by \eqref{p3:eq:A1-def}, such that
\begin{equation}\label{p3:eq:all-orders}
\boxed{
 \log F(x)=M_\sharp(x)
...
\begin{equation}\label{p3:eq:all-order-kernel-exp}
 \log K_x(v)=-\frac12v^2+
...
\begin{equation}\label{p3:eq:formal-log-coeff}
...
\eqref{p3:eq:exact-saddle-action} adds $E(2^\lambda)$, which is exponentially
...
\eqref{p3:eq:lambda-expansion} may be substituted into the nonoscillatory
...
write
\begin{equation}\label{p3:eq:elementary-log-notation}
 \ell_x:=\log x<0,
 \qquad q_x:=\log(-\ell_x),
 \qquad a:=\log L.
\end{equation}
...
\begin{align}
 ...
 \label{p3:eq:M-elementary}
\end{align}

\begin{corollary}[Corrected elementary expansion]\label{p3:cor:elementary-expansion}
As $x\downarrow0$,
\begin{equation}\label{p3:eq:elementary-expansion}
\boxed{
 \log F(x)=M_{\mathrm{elem}}(x)+\Psi(\lambda(x))
 +\Oh\!\left(\frac1{-\log x}\right).}
\end{equation}
Consequently
\begin{equation}\label{p3:eq:elementary-F-equivalent}
 F(x)\sim
...
\begin{proof}
Expand the lower Lambert branch in \eqref{p3:eq:Msharp} through the fourth phase
...
$q_x$ gives \eqref{p3:eq:M-elementary}; the discarded terms are
$\Oh(1/(-\log x))$.  The periodic term is left at the exact phase.  Combining
this algebraic estimate with \eqref{p3:eq:sharp-asymptotic} proves the claim.
\end{proof}
...
\begin{align}
 ...
 &-\frac{(\log t)^2}{2L^2t}
 +\Psi(\lambda_t)+\Oh(t^{-1}).
 \label{p3:eq:dyadic-explicit-asymptotic}
\end{align}
Substitution of $x=2^{-t}$ into the literal expression
\eqref{p3:eq:M-elementary} adds the explicit term
...
Delete $\Psi(\lambda(x))$ from \eqref{p3:eq:elementary-expansion}.  The resulting
error does not tend to zero.  The obstruction can be seen without numerical
computation.

\begin{proposition}[Periodic obstruction]\label{p3:prop:periodic-obstruction}
There is no estimate
\begin{equation}\label{p3:eq:false-uncorrected}
...
\begin{equation}\label{p3:eq:phase-lock-sequence}
 x_m:=(m+u)2^{-(m+u)}.
\end{equation}
Then $x_m\downarrow0$ and the defining equation
\eqref{p3:eq:lambda-identities} gives $\lambda(x_m)=m+u$.  By
\eqref{p3:eq:elementary-expansion},
\begin{equation}\label{p3:eq:phase-lock-limit}
 \log F(x_m)-M_{\mathrm{elem}}(x_m)\longrightarrow\Psi(u).
\end{equation}
The function $\Psi$ is nonconstant by \cref{p3:thm:Psi-Fourier}; choosing two
phases with different values gives two different limits.  But the right side
of \eqref{p3:eq:false-uncorrected} would tend to zero, a contradiction.
\end{proof}
...
\begin{theorem}[Quadratic logarithmic law]\label{p3:thm:coarse-asymptotic}
As $x\downarrow0$,
\begin{equation}\label{p3:eq:coarse-asymptotic}
 \boxed{
 \log F(x)\sim-\frac{(\log x)^2}{2\log2}.}
\end{equation}
Equivalently, for $t\to\infty$,
\begin{equation}\label{p3:eq:coarse-dyadic}
 \log F(2^{-t})\sim-\frac{\log2}{2}t^2.
\end{equation}
\end{theorem}

\begin{proof}
Either divide \eqref{p3:eq:elementary-expansion} by $(\log x)^2$ and observe that
all other terms are lower order, or use \eqref{p3:eq:sharp-asymptotic} together
with $\lambda(x)\sim-\log x/L$.
\end{proof}

Combining the exact bridge \eqref{p3:eq:d-inverse-dyadic} with
\eqref{p3:eq:dyadic-explicit-asymptotic} and Stirling's formula yields a sharp
asymptotic for the half moments.

\begin{corollary}[Asymptotic of $d_n$]\label{p3:cor:dn-asymptotic}
Let $\lambda_n$ be the solution of
$\lambda_n-(\log\lambda_n)/L=n$.  Then
\begin{equation}\label{p3:eq:dn-asymptotic}
 d_n\sim\sqrt{2\pi n}\,
 \exp\!\left(
...
\cref{p3:thm:all-orders} with the Stirling series.
\end{corollary}

\begin{proof}
Take logarithms in
$d_n=n!2^{\binom n2}F(2^{-n})$.  In
\eqref{p3:eq:dyadic-explicit-asymptotic}, the quadratic, $n\log n$, and linear
terms cancel exactly against $\binom n2L+\log n!$.  What remains is
\[
 \log d_n=\frac12\log(2\pi n)+C_\sharp+\Psi(\lambda_n)
 -\frac{(\log n)^2}{2L}+o(1),
\]
which exponentiates to \eqref{p3:eq:dn-asymptotic}.
\end{proof}


\section{A global binary-reduction series}\label{p3:sec:binary-series}
...
\begin{equation}\label{p3:eq:reduction-polynomial}
 P_m(y):=
 \sum_{k=0}^{m}
 2^{\binom{k+1}{2}-\binom{m-k}{2}}
 \frac{d_{m-k}}{(m-k)!}\frac{y^k}{k!}.
\end{equation}
This is exactly the polynomial in \eqref{p3:eq:highest-bit}.  For $x\ge0$ put
\begin{align}
 p_m(x)&:=\floor{2^m x},\label{p3:eq:binary-prefix}\\
 t_m(x)&:=x-\frac{p_m(x)}{2^m}.\label{p3:eq:binary-tail}
\end{align}
so that $0\le t_m(x)<2^{-m}$.  To handle the zeroth scale literally, set
\begin{equation}\label{p3:eq:previous-prefix}
 p_{-1}(x):=\floor{x/2}.
\end{equation}
Finally define
\begin{equation}\label{p3:eq:binary-series-coefficient}
...
\begin{equation}\label{p3:eq:alpha-one-bit}
 \alpha_m(x)=(-1)^{s_2(p_{m-1}(x))}.
\end{equation}
Thus the series really visits only the nonzero binary digits.

\begin{theorem}[Exact finite binary telescope]\label{p3:thm:binary-finite-telescope}
For every $x\ge0$ and every $N\ge0$,
\begin{equation}\label{p3:eq:binary-finite-telescope}
\boxed{
 \GF(x)=\sum_{m=0}^{N}\alpha_m(x)P_m(t_m(x))
 +(-1)^{s_2(p_N(x))}\GF(t_N(x)).}
\end{equation}
For $N\ge1$, the remainder satisfies
\begin{equation}\label{p3:eq:binary-remainder-bound}
...
proves \eqref{p3:eq:binary-finite-telescope} for $N=0$.
...
\begin{equation}\label{p3:eq:binary-tail-step}
 t_{N-1}=2^{-N}+t_N.
\end{equation}
Apply the signed version of \cref{p3:thm:highest-bit} at scale $N$:
\begin{equation}\label{p3:eq:binary-reduction-step}
 \GF(t_{N-1})=-\GF(t_N)+P_N(t_N).
\end{equation}
The parity identity
$s_2(p_N)=s_2(p_{N-1})+1$ changes the sign of the residual, while
\eqref{p3:eq:alpha-one-bit} gives precisely the coefficient of the new
...
\begin{equation}\label{p3:eq:F-linear-tail-bound}
 0\le F(y)\le2y\qquad(0\le y\le1/2).
\end{equation}
Therefore $|\GF(t_N)|\le2t_N<2^{1-N}$.
\end{proof}

\begin{theorem}[Absolutely convergent binary series]\label{p3:thm:binary-series}
For every $x\ge0$,
\begin{equation}\label{p3:eq:binary-series}
\boxed{
...
\begin{proof}
The remainder in \eqref{p3:eq:binary-finite-telescope} tends to zero by
\eqref{p3:eq:binary-remainder-bound}, so the partial sums converge to $\GF(x)$.
...
for $m\ge2$,
\begin{equation}\label{p3:eq:binary-summand-bound}
 |\alpha_m(x)P_m(t_m(x))|
 \le |\GF(t_{m-1})|+|\GF(t_m)|
 \le 4\,2^{-(m-1)},
\end{equation}
...
\begin{remark}[The missing zeroth scale]\label{p3:rem:missing-zero-scale}
...
\begin{equation}\label{p3:eq:zero-scale-at-one}
 \alpha_0(1)P_0(0)=1.
\end{equation}
...
the right of \eqref{p3:eq:qbinomial-dyadic}.

\begin{corollary}[Exact dyadic-row limit]\label{p3:cor:dyadic-row-limit}
Let $x\ge0$, let $c\in\Q$, and put $m_N=\floor{2^Nx}$.  Then
\begin{equation}\label{p3:eq:dyadic-row-limit}
...
\begin{proof}
The equality is \cref{p3:thm:qbinomial-dyadic}.  Since
...
\section{Practical exact and numerical evaluation}\label{p3:sec:computation}
...
\midrule
Exact dyadic value & Highest-bit recursion, \cref{p3:alg:dyadic-evaluator} &
Uses only rational arithmetic and visits one stage per set bit.\\
Many values with one denominator & Closed moment formula
\eqref{p3:eq:closed-dyadic} & Reuses one table of moments and common powers.\\
Global dyadic value & $q$-binomial formula \eqref{p3:eq:qbinomial-dyadic} or
global evaluator & No unit-interval restriction and automatic sign handling.\\
Moderate-precision real value & Binary telescope
\eqref{p3:eq:binary-finite-telescope} & Explicit geometric residual bound.\\
High-precision value near $0$ or $1$ & Lambert expansion
\eqref{p3:eq:all-orders} & Avoids catastrophic underflow and resolves the true
logarithmic scale.\\
...
\eqref{p3:eq:up-Fourier-product} and Poisson summation & Zeros and lattice
...
exact identity \eqref{p3:eq:highest-bit}.  This distinction is valuable in a
...
\begin{align}
 F(a/2^n)+F((2^n-a)/2^n)&=1,\label{p3:eq:test-symmetry}\\
 F(2a/2^{n+1})&=F(a/2^n),\label{p3:eq:test-refinement}\\
 d_n&=n!2^{\binom n2}F(2^{-n}),\label{p3:eq:test-inverse}\\
 \sum_{k\in\Z}\upf(x+k)&=1.\label{p3:eq:test-partition}
\end{align}
...
\section{Corrections certified by the formal development}\label{p3:sec:corrections}
...
Fourier convention \eqref{p3:eq:Fourier-convention} and from the fact that the
...
The factor $(\e^z-1)/z$ in \eqref{p3:eq:f-refinement} is entire after assigning
...
values is \eqref{p3:eq:Rn-def}.  A negative exponent in one source display is a
typographical error.

\item \textbf{The global binary series begins at zero.}
As \cref{p3:rem:missing-zero-scale} shows, starting at $m=1$ is valid on
$0\le x<1$ but fails at $x=1$ and on general global blocks.  The correct
all-$x$ series is \eqref{p3:eq:binary-series}.
...
error; \cref{p3:prop:periodic-obstruction} supplies a direct contradiction.
...
\section{Concluding synthesis}\label{p3:sec:conclusion}
...
\section{Notation dictionary}\label{p3:app:notation}
...
products in $c_n,d_n$.\\
$R_n$ & Reshetnikov's integer, defined in \eqref{p3:eq:Rn-def}.\\
...
$P_m(y)$ & Finite Taylor-reduction polynomial \eqref{p3:eq:reduction-polynomial}.\\
$p_m(x),t_m(x)$ & Binary prefix and tail at scale $m$.\\
$Q(m,n;c)$ & Finite $q$-binomial/Thue--Morse expression in
\eqref{p3:eq:qbinomial-dyadic}.\\
\bottomrule
\end{longtable}

\section{A source-to-theorem map}\label{p3:app:source-map}
...
functions & \cref{p3:sec:existence,p3:sec:global}.\\
Fourier product, convolution measures, random series, weak convergence, step
approximants & \cref{p3:sec:existence} and the section on finite approximants.\\
