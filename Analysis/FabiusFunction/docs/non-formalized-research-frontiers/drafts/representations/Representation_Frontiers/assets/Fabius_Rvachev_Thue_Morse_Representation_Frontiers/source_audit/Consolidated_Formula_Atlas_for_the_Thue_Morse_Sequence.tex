...
\begin{equation}\label{p3:eq:two-normalizations}
  \tau(n)=\bitsum(n)\bmod 2\in\{0,1\},
  \qquad
  \eps_n=(-1)^{\bitsum(n)}=1-2\tau(n)\in\{1,-1\}.
\end{equation}
Thus
\begin{equation}\label{p3:eq:tau-eps-conversion}
  \tau(n)=\frac{1-\eps_n}{2},
  \qquad
  \eps_n=1-2\tau(n).
\end{equation}
...
\begin{equation}\label{p3:eq:master-generating-notation}
...
\begin{theorem}[Dyadic recursion]\label{p3:thm:dyadic-recursion}
For every $n\ge0$,
\begin{equation}\label{p3:eq:dyadic-recursion}
 \tau(2n)=\tau(n),\qquad \tau(2n+1)=1-\tau(n),
\end{equation}
and equivalently
\begin{equation}\label{p3:eq:signed-dyadic-recursion}
 \eps_{2n}=\eps_n,\qquad \eps_{2n+1}=-\eps_n.
\end{equation}
\end{theorem}
...
\begin{equation}\label{p3:eq:morphism}
...
\]
satisfy
\begin{equation}\label{p3:eq:block-recursion}
  W_{m+1}=W_m\,(-W_m).
\end{equation}
...
\begin{corollary}[Complementation on dyadic blocks]\label{p3:cor:block-complement}
For $0\le r<2^m$,
\begin{equation}\label{p3:eq:block-complement}
  \eps_{2^m+r}=-\eps_r,
  \qquad
  \tau(2^m+r)=1-\tau(r).
\end{equation}
...
\begin{proposition}[No-carry concatenation]\label{p3:prop:no-carry-concat}
If $0\le r<2^m$, then for every $q\ge0$,
\begin{equation}\label{p3:eq:no-carry-concat}
...
\begin{equation}\label{p3:eq:xor-character}
 \boxed{\qquad \eps_{a\xor b}=\eps_a\eps_b.\qquad}
\end{equation}
Thus $\eps$ is a character of the Boolean group of finite binary strings.

\begin{remark}
The correction to \cref{p3:eq:no-carry-concat} when ordinary addition creates
carries is exactly the Kummer factor in \cref{p3:thm:carry-law}.  Thus the
...
\begin{proposition}[Minimal kernel]\label{p3:prop:kernel}
...
\begin{proof}
Iterating \cref{p3:eq:no-carry-concat} gives
...
classical theorem of Thue says that the fixed point of \cref{p3:eq:morphism} is
...
For every $j\ge0$,
\begin{equation}\label{p3:eq:binary-digit-floor}
 b_j(n)=\floor{\frac{n}{2^j}}-2\floor{\frac{n}{2^{j+1}}}
       =\floor{\frac{n}{2^j}}\bmod 2.
\end{equation}
Summing over $j$ yields several exact formulae.

\begin{theorem}[Floor and fractional-part formulae]\label{p3:thm:floor-formulas}
For every $n\ge0$,
\begin{align}
 \bitsum(n)
 &=n-\sum_{j\ge1}\floor{\frac{n}{2^j}},\label{p3:eq:legendre-floor}\\
 &=\sum_{j\ge1}\frac{[n]_{2^j}}{2^j},\label{p3:eq:remainder-sum}\\
 &=\sum_{j\ge1}\fracp{\frac{n}{2^j}}.\label{p3:eq:fractional-parts}
\end{align}
...
\begin{proof}
Summing \cref{p3:eq:binary-digit-floor} over $j\ge0$ telescopes:
\[
 \sum_{j\ge0}b_j(n)
...
Consequently
\begin{equation}\label{p3:eq:floor-sign}
...
formula is
\begin{equation}\label{p3:eq:single-cosine}
...
\begin{equation}\label{p3:eq:sinc-product}
...
\begin{proposition}[Signs between consecutive integers]\label{p3:prop:sinc-sign}
For every integer $N\ge0$ and every $x\in(N,N+1)$,
\begin{equation}\label{p3:eq:sinc-sign-interval}
 \sgn\Phi(x)=\eps_N.
\end{equation}
In particular,
\begin{equation}\label{p3:eq:sinc-half-integer}
...
\cref{p3:eq:signed-dyadic-recursion}.  Since the initial interval $(0,1)$ has
...
\begin{theorem}[Legendre]\label{p3:thm:legendre}
For every $n\ge0$,
\begin{equation}\label{p3:eq:legendre}
 \vtwo(n!)=\sum_{j\ge1}\floor{\frac n{2^j}}=n-\bitsum(n).
\end{equation}
Therefore
\begin{equation}\label{p3:eq:factorial-sign}
...
The second equality is \cref{p3:eq:legendre-floor}.
\end{proof}

\subsection{A single central binomial coefficient}

\begin{theorem}[Central-binomial valuation]\label{p3:thm:central-binomial}
For every $n\ge0$,
\begin{equation}\label{p3:eq:central-binomial-v2}
 \vtwo\binom{2n}{n}=\bitsum(n).
\end{equation}
Consequently
\begin{equation}\label{p3:eq:central-binomial-thue-morse}
...
\begin{proposition}[Catalan valuation]\label{p3:prop:catalan}
For $n\ge0$,
\begin{equation}\label{p3:eq:catalan-v2}
 \vtwo(\Cat_n)=\bitsum(n+1)-1.
\end{equation}
Hence for $n\ge0$,
\begin{equation}\label{p3:eq:catalan-thue-morse}
...
\begin{proof}
By \cref{p3:thm:central-binomial},
...
\begin{theorem}[Kummer in base two]\label{p3:thm:kummer}
The number of carries produced when adding $a$ and $b$ in base two is
\begin{equation}\label{p3:eq:kummer}
...
\begin{proof}
The valuation identity follows from \cref{p3:eq:legendre}:
\begin{align*}
 \vtwo\binom{a+b}{a}
...
\begin{theorem}[Carry-corrected multiplicativity]\label{p3:thm:carry-law}
For all $a,b\ge0$,
\begin{equation}\label{p3:eq:carry-law}
 \boxed{\ \eps_{a+b}=\eps_a\eps_b
     (-1)^{\vtwo\binom{a+b}{a}}.\ }
\end{equation}
Equivalently,
\begin{equation}\label{p3:eq:tau-carry-law}
 \tau(a+b)\equiv \tau(a)+\tau(b)+\vtwo\binom{a+b}{a}\pmod2.
\end{equation}
\end{theorem}

\begin{proof}
Reduce \cref{p3:eq:kummer} modulo $2$ and exponentiate $-1$.
\end{proof}
...
\begin{equation}\label{p3:eq:borrow-law}
 \vtwo\binom ab=\bitsum(b)+\bitsum(a-b)-\bitsum(a),
\end{equation}
which counts borrows in the subtraction $a-b$.  Thus
\begin{equation}\label{p3:eq:subtraction-thue-morse}
 \eps_{a-b}=\eps_a\eps_b(-1)^{\vtwo\binom ab}.
\end{equation}
This is the subtraction analogue of \cref{p3:eq:carry-law}.
...
\begin{theorem}[Lucas in base two]\label{p3:thm:lucas}
For $0\le k\le n$,
\begin{equation}\label{p3:eq:lucas-submask}
 \binom nk\equiv1\pmod2
 \quad\Longleftrightarrow\quad
 k\submask n.
\end{equation}
Consequently the number of odd entries in row $n$ of Pascal's triangle is
\begin{equation}\label{p3:eq:glaisher}
...
Taking a base-two logarithm in \cref{p3:eq:glaisher} gives
\begin{equation}\label{p3:eq:binomial-log}
...
\begin{align}
 \mathcal O(n)
 &=\sum_{k=0}^n \frac{1-(-1)^{\binom nk}}2,\label{p3:eq:odd-count-sign}\\
 &=\sum_{k=0}^n\sin^2\!\left(\frac\pi2\binom nk\right),\label{p3:eq:odd-count-trig}\\
 &=\frac12\left(n+1-\sum_{k=0}^n\cos\!\left(\pi\binom nk\right)\right).
 \label{p3:eq:odd-count-cos}
\end{align}
...
even or odd, \cref{p3:eq:glaisher} immediately removes the logarithm.

\begin{theorem}[Log-free Pascal formula]\label{p3:thm:log-free-pascal}
For every $n\ge0$,
\begin{equation}\label{p3:eq:log-free-pascal}
 \boxed{\quad
...
\begin{equation}\label{p3:eq:signed-row-statistic}
 \Sigma(n)=\sum_{k=1}^{n}(-1)^{\binom nk},
\end{equation}
with the empty sum $\Sigma(0)=0$.

\begin{proposition}[Signed-row residue]\label{p3:prop:signed-row-residue}
For every $n\ge0$,
\begin{equation}\label{p3:eq:signed-row-residue}
 \boxed{\qquad \tau(n)=[2n+\Sigma(n)]_3.\qquad}
\end{equation}
Equivalently,
\begin{equation}\label{p3:eq:signed-row-trig}
...
\begin{equation}\label{p3:eq:Ru-definition}
 R_u(n)=\sum_{k=0}^{n}u^{\binom nk\bmod2}.
\end{equation}

\begin{identity}[Pascal row partition function]\label{p3:id:Ru}
\begin{equation}\label{p3:eq:Ru}
...
$M\ge0$:
\begin{equation}\label{p3:eq:nested-bit-probe}
 b_r(M)\equiv\binom{M}{2^r}\pmod2.
\end{equation}
Taking $M=\binom nk$ yields nested binomial coefficients
\begin{equation}\label{p3:eq:nested-pascal-probe}
...
\begin{equation}\label{p3:eq:nested-digit-sum}
...
$\Ftwo$
\begin{equation}\label{p3:eq:freshman-dream}
 (1+x)^{2^m}=1+x^{2^m}.
\end{equation}
More generally, if $n=\sum_{j\in J(n)}2^j$, then
\begin{equation}\label{p3:eq:lucas-polynomial}
 (1+x)^n
...
sparse Prouhet identities of \cref{p3:sec:sparse-prouhet}.
...
\begin{theorem}[Odd multinomial support]\label{p3:thm:odd-multinomial}
The number of odd multinomial coefficients in the full $r$-nomial layer of
weight $n$ is
\begin{equation}\label{p3:eq:odd-multinomial-count}
 \mathcal O_r(n)
...
\begin{equation}\label{p3:eq:multinomial-support-polynomial}
...
\begin{corollary}[Multinomial recovery of Thue--Morse]\label{p3:cor:multinomial-recovery}
For any fixed integer $r\ge2$,
\begin{equation}\label{p3:eq:multinomial-log-recovery}
 \tau(n)=\left[\log_r\mathcal O_r(n)\right]_2,
 \qquad
 \eps_n=(-1)^{\log_r\mathcal O_r(n)}.
\end{equation}
When $r\equiv-1\pmod3$, the logarithm can again be removed:
\begin{equation}\label{p3:eq:multinomial-logfree}
...
\begin{proof}
The logarithmic formula is immediate from \cref{p3:eq:odd-multinomial-count}.  If
...
\begin{equation}\label{p3:eq:multinomial-v2}
...
Thus \cref{p3:thm:odd-multinomial} is a carry-free allocation theorem: each
occupied bit of $n$ chooses exactly one component.

\subsection{A multivariate Boolean lattice}

The support in \cref{p3:eq:multinomial-support-polynomial} is the Cartesian
...
\begin{theorem}[Exact enumerators]\label{p3:thm:evil-odious-enumerators}
For every $j\ge0$,
\begin{equation}\label{p3:eq:evil-odious-enumerators}
 \boxed{\quad E_j=2j+\tau(j),\qquad
 O_j=2j+1-\tau(j).\quad}
\end{equation}
In particular,
\begin{equation}\label{p3:eq:evil-odious-gap}
...
\cref{p3:eq:signed-dyadic-recursion}.  Therefore exactly one member of the pair is
...
\begin{equation}\label{p3:eq:prefix-sum-def}
 S(N)=\sum_{n=0}^{N-1}\eps_n,
 \qquad S(0)=0.
\end{equation}

\begin{theorem}[Exact prefix discrepancy]\label{p3:thm:prefix-discrepancy}
For every $q\ge0$,
\begin{equation}\label{p3:eq:prefix-discrepancy}
 S(2q)=0,
 \qquad
 S(2q+1)=\eps_q.
\end{equation}
...
\]
Then
\begin{equation}\label{p3:eq:evil-odious-counts}
 E(N)=\frac{N+S(N)}2,
 \qquad
 O(N)=\frac{N-S(N)}2.
\end{equation}
...
\begin{corollary}[Uniform interval bound]\label{p3:cor:interval-discrepancy}
For all integers $0\le A<B$,
\begin{equation}\label{p3:eq:interval-discrepancy}
 |D(A,B)|\le2.
\end{equation}
More precisely, for $a,b\ge0$,
\begin{align}
 D(2a,2b)&=0,\label{p3:eq:interval-even-even}\\
 D(2a,2b+1)&=\eps_b,\label{p3:eq:interval-even-odd}\\
 D(2a+1,2b)&=-\eps_a,\label{p3:eq:interval-odd-even}\\
 D(2a+1,2b+1)&=\eps_b-\eps_a.\label{p3:eq:interval-odd-odd}
\end{align}
...
uniform exponent is discussed in \cref{p3:sec:gelfond}.
\end{remark}
...
\begin{theorem}[Finite product identity]\label{p3:thm:finite-product}
For every $m\ge0$,
\begin{equation}\label{p3:eq:finite-product}
 \boxed{\quad
 P_m(z):=\sum_{n=0}^{2^m-1}\eps_nz^n
...
\begin{align}
 P_{m+1}(z)&=(1-z^{2^m})P_m(z),\label{p3:eq:P-adjoin}\\
 P_m(z)&=(1-z)P_{m-1}(z^2)\qquad(m\ge1).\label{p3:eq:P-mahler-finite}
\end{align}
...
\begin{equation}\label{p3:eq:P-reciprocity}
 z^{D_m}P_m(z^{-1})=(-1)^mP_m(z).
\end{equation}
Equating coefficients gives the reflection rule
\begin{equation}\label{p3:eq:reflection-rule}
...
\begin{equation}\label{p3:eq:cyclotomic-factorization}
 P_m(z)=(1-z)^m
...
\begin{align}
 \eps_n
 &=\coeff n{\prod_{j\ge0}(1-z^{2^j})},\label{p3:eq:coefficient-infinite-formal}\\
 \eps_n
 &=\sum_{J\subseteq\{0,\ldots,m-1\}}
    (-1)^{|J|}\ind\!\left(n=\sum_{j\in J}2^j\right)
    \quad(0\le n<2^m),\label{p3:eq:subset-coefficient}\\
 \tau(n)&=\frac12\left(1-\coeff n{P_m(z)}\right)
    \qquad(0\le n<2^m).
\end{align}
The sum in \cref{p3:eq:subset-coefficient} has exactly one nonzero term.  Its
...
\begin{equation}\label{p3:eq:Fu}
 F_u(z)=\sum_{n\ge0}u^{\bitsum(n)}z^n
       =\prod_{j\ge0}(1+u z^{2^j}),
 \qquad |z|<1.
\end{equation}
It satisfies
\begin{equation}\label{p3:eq:Fu-mahler}
 F_u(z)=(1+uz)F_u(z^2).
\end{equation}
The cases $u=1$ and $u=-1$ are
\begin{equation}\label{p3:eq:Fu-special}
 F_1(z)=\frac1{1-z},
 \qquad
 F_{-1}(z)=E(z).
\end{equation}
...
\begin{equation}\label{p3:eq:digit-sum-ogf}
...
Using \cref{p3:eq:tau-eps-conversion},
\begin{equation}\label{p3:eq:T-from-E}
 \boxed{\qquad
...
\begin{equation}\label{p3:eq:T-mahler}
 \boxed{\qquad
 T(z)=(1-z)T(z^2)+\frac{z}{1-z^2}.
 \qquad}
\end{equation}
...
Taking a logarithm in \cref{p3:eq:E-product},
\begin{align}
 \log E(z)
 &=\sum_{j\ge0}\log(1-z^{2^j})\notag\\
 &=-\sum_{j\ge0}\sum_{q\ge1}\frac{z^{q2^j}}q
 =-\sum_{r\ge1}\frac{a_r}{r}z^r,\label{p3:eq:log-E}
\end{align}
where
\begin{equation}\label{p3:eq:ruler-coeff}
 a_r=\sum_{2^j\mid r}2^j=2^{\vtwo(r)+1}-1.
\end{equation}
Thus
\begin{equation}\label{p3:eq:log-derivative-E}
 \frac{zE'(z)}{E(z)}=-\sum_{r\ge1}a_rz^r.
\end{equation}
The coefficient $a_r$ is an affine function of the ruler sequence
$2^{\vtwo(r)}$.

\begin{theorem}[Convolution recurrence]\label{p3:thm:convolution-recurrence}
With $\eps_0=1$, the signed Thue--Morse coefficients satisfy
\begin{equation}\label{p3:eq:convolution-recurrence}
 \boxed{\qquad
 n\eps_n=-\sum_{r=1}^{n}
...
\begin{proof}
Multiply \cref{p3:eq:log-derivative-E} by $E(z)$ and compare the coefficient of
$z^n$.
\end{proof}
...
defined by
\begin{equation}\label{p3:eq:Bell-definition}
 \exp\!\left(\sum_{r\ge1}x_r\frac{z^r}{r!}\right)
 =\sum_{n\ge0}\Bell_n(x_1,\ldots,x_n)\frac{z^n}{n!}.
\end{equation}
Set
\begin{equation}\label{p3:eq:Bell-xr}
 x_r=-(r-1)!\bigl(2^{\vtwo(r)+1}-1\bigr).
\end{equation}
Then \cref{p3:eq:log-E} gives the exact formula
\begin{equation}\label{p3:eq:Bell-epsilon}
 \boxed{\qquad
...
\begin{theorem}[Hessenberg determinant]\label{p3:thm:Hessenberg}
For every $n\ge1$,
\begin{equation}\label{p3:eq:Hessenberg-epsilon}
 \boxed{\qquad
 \eps_n=\frac{(-1)^n}{n!}\det H_n.
 \qquad}
\end{equation}
In particular,
\begin{equation}\label{p3:eq:Hessenberg-factorial}
 |\det H_n|=n!.
\end{equation}
\end{theorem}
...
\cref{p3:eq:Bell-epsilon}.  The absolute-value statement follows from
$\eps_n\in\{\pm1\}$.
\end{proof}

\subsection{Euler-transform form}

The product \cref{p3:eq:E-product} is also an Euler transform with exponents
supported at powers of two:
\begin{equation}\label{p3:eq:euler-transform}
...
that divisor sum collapses to \cref{p3:eq:ruler-coeff}.  This explains why the
...
\cref{p3:eq:T-mahler} gives
\begin{equation}\label{p3:eq:theta-mod2-mahler}
 \Theta(z)=(1+z)\Theta(z)^2+\frac{z}{1+z^2}.
\end{equation}
After clearing denominators,
\begin{equation}\label{p3:eq:theta-algebraic}
 \boxed{\quad
 (1+z)^3\Theta(z)^2+(1+z)^2\Theta(z)+z=0.
 \quad}
\end{equation}
If
\begin{equation}\label{p3:eq:Y-definition}
 Y(z)=(1+z)\Theta(z),
\end{equation}
then \cref{p3:eq:theta-algebraic} becomes the Artin--Schreier equation
\begin{equation}\label{p3:eq:Artin-Schreier}
...
\begin{equation}\label{p3:eq:integer-lift-recurrence}
 nA_n=(5-2n)A_{n-1}+3(n-1)A_{n-2}+1.
\end{equation}
Their generating function is
\begin{equation}\label{p3:eq:integer-lift-generating}
 A(z)=\sum_{n\ge0}A_nz^n
 =\frac1{2(1-z)}
  \left(\sqrt{\frac{1+3z}{1-z}}-1\right).
\end{equation}
It satisfies
\begin{equation}\label{p3:eq:integer-lift-algebraic}
 \boxed{\qquad
 (1-z)^3A(z)^2+(1-z)^2A(z)=z.
 \qquad}
\end{equation}

\begin{proposition}[Parity lift]\label{p3:prop:integer-lift-parity}
For every $n\ge0$,
\begin{equation}\label{p3:eq:integer-lift-parity}
 A_n\equiv\tau(n)\pmod2.
\end{equation}
\end{proposition}

\begin{proof}
Reducing \cref{p3:eq:integer-lift-algebraic} modulo $2$ changes $1-z$ to $1+z$
and gives exactly \cref{p3:eq:theta-algebraic}.  Both formal solutions have
...
giving \cref{p3:eq:first-nonzero-moment}.
\end{proof}

\subsection{The finite-difference identity}

For $h\in\R$, let
\begin{equation}\label{p3:eq:finite-difference-def}
 \Delta_hf(x)=f(x+h)-f(x).
\end{equation}

\begin{theorem}[Thue--Morse as an iterated difference]\label{p3:thm:finite-difference}
For every function $f$ on which the expressions are defined,
\begin{equation}\label{p3:eq:finite-difference}
 \boxed{\qquad
...
\begin{equation}\label{p3:eq:one-difference-integral}
 \Delta_hf(x)=h\int_0^1f'(x+hu)\dd u.
\end{equation}
Iterating gives an exact integral representation.

\begin{theorem}[Cubature identity]\label{p3:thm:cubature}
For $f\in C^m$,
\begin{equation}\label{p3:eq:cubature}
 \boxed{\quad
 \sum_{n=0}^{2^m-1}\eps_nf(x+n)
 =(-1)^m2^{\binom m2}
  \int_{[0,1]^m}
...
\begin{proof}
Apply \cref{p3:eq:one-difference-integral} successively to
\cref{p3:eq:finite-difference}; the product of the step sizes is
$\prod_{j=0}^{m-1}2^j=2^{\binom m2}$.
\end{proof}

\begin{corollary}[Mean-value form]\label{p3:cor:mean-value}
If $f\in C^m$ on $[x,x+2^m-1]$, there exists
$\xi\in[x,x+2^m-1]$ such that
\begin{equation}\label{p3:eq:mean-value}
 \sum_{n=0}^{2^m-1}\eps_nf(x+n)
...
Define
\begin{equation}\label{p3:eq:moment-def}
 M_{m,r}=\sum_{n=0}^{2^m-1}\eps_nn^r.
\end{equation}
The exponential generating function in $r$ is
\begin{equation}\label{p3:eq:moment-egf}
...
\]
Then
\begin{equation}\label{p3:eq:moment-factorization}
 P_m(\e^t)=(-1)^m2^{\binom m2}t^m
 \prod_{j=0}^{m-1}\varphi(2^jt).
\end{equation}

\begin{theorem}[Composition formula for every moment]\label{p3:thm:all-moments}
For $r<m$, $M_{m,r}=0$.  For $r\ge m$,
\begin{equation}\label{p3:eq:all-moments-composition}
 \boxed{\quad
...
\cref{p3:eq:moment-factorization}.
\end{proof}
...
\begin{align}
 M_{m,m}
 &=(-1)^m m!2^{\binom m2},\label{p3:eq:moment-m}\\
 M_{m,m+1}
 &=(-1)^m(m+1)!2^{\binom m2}\frac{D_m}{2},\label{p3:eq:moment-m1}\\
 M_{m,m+2}
 &=(-1)^m(m+2)!2^{\binom m2}
   \left(\frac{D_m^2}{8}+\frac{Q_m}{24}\right).
 \label{p3:eq:moment-m2}
\end{align}

\subsection{Centered moments and parity}

Let $c_m=(2^m-1)/2$.  By reciprocity,
\begin{equation}\label{p3:eq:centered-egf}
...
\begin{corollary}[Centered parity cancellation]\label{p3:cor:centered-parity}
For every $r\ge0$,
\begin{equation}\label{p3:eq:centered-parity}
...
Set
\begin{equation}\label{p3:eq:binomial-moment-def}
 B_{m,r}=\sum_{n<2^m}\eps_n\binom nr.
\end{equation}
Then
\begin{equation}\label{p3:eq:binomial-moment-generating}
 \sum_{r\ge0}B_{m,r}u^r
 =\sum_{n<2^m}\eps_n(1+u)^n
 =P_m(1+u).
\end{equation}
Therefore
\begin{equation}\label{p3:eq:binomial-moment-coeff}
 B_{m,r}=\coeff r{\prod_{j=0}^{m-1}
        \left(1-(1+u)^{2^j}\right)}.
\end{equation}
In particular,
\begin{equation}\label{p3:eq:binomial-moment-first}
 B_{m,r}=0\quad(r<m),
 \qquad
...
\begin{equation}\label{p3:eq:stirling-transform}
 n^r=\sum_{q=0}^{r}\left\{\begin{matrix}r\\q\end{matrix}\right\}
       \fall nq.
\end{equation}

\section{Sparse Pascal-support Prouhet identities}\label{p3:sec:sparse-prouhet}

The full dyadic block is only one member of a larger family.  Fix $N\ge0$ and
let
\begin{equation}\label{p3:eq:occupied-bits}
 J(N)=\{j:b_j(N)=1\},
 \qquad s=|J(N)|=\bitsum(N),
 \qquad \beta(N)=\sum_{j\in J(N)}j.
\end{equation}

\begin{theorem}[Signed submask polynomial]\label{p3:thm:signed-submask-product}
\begin{equation}\label{p3:eq:signed-submask-product}
 \boxed{\quad
...
\begin{corollary}[Sparse Prouhet cancellation]\label{p3:cor:sparse-prouhet}
For $0\le r<s$,
\begin{equation}\label{p3:eq:sparse-prouhet}
 \sum_{k\submask N}\eps_kk^r=0,
\end{equation}
and the first nonzero moment is
\begin{equation}\label{p3:eq:sparse-first-moment}
...
\begin{equation}\label{p3:eq:sparse-finite-difference}
...
\begin{equation}\label{p3:eq:iterated-prefix-definition}
 S_n^{(0)}=\eps_n,
 \qquad
 S_n^{(k+1)}=\sum_{j=0}^{n}S_j^{(k)}.
\end{equation}
The first level is explicit:
\begin{equation}\label{p3:eq:first-inclusive-prefix}
 S_{2m}^{(1)}=\eps_m,
 \qquad
 S_{2m+1}^{(1)}=0.
\end{equation}

\begin{theorem}[Binomial kernel for repeated summation]\label{p3:thm:iterated-prefix}
For $k\ge1$,
\begin{equation}\label{p3:eq:iterated-prefix-binomial}
 \boxed{\quad
 S_n^{(k)}=\sum_{j=0}^{n}
...
\begin{equation}\label{p3:eq:iterated-prefix-generating}
 \boxed{\qquad
...
\begin{theorem}[Dyadic terminal zero run]\label{p3:thm:terminal-zero-run}
If $1\le k\le r$, then
\begin{equation}\label{p3:eq:terminal-zero-run}
 S_{2^r-k+i}^{(k)}=0,
 \qquad 0\le i<k,
\end{equation}
while
\begin{equation}\label{p3:eq:terminal-boundaries}
 S_{2^r-k-1}^{(k)}=(-1)^{r-k},
 \qquad
 S_{2^r}^{(k)}=-1.
\end{equation}
...
\cref{p3:eq:iterated-prefix-generating} by $P_r(z)$.  Then
\begin{equation}\label{p3:eq:prefix-polynomial-degree}
 \frac{P_r(z)}{(1-z)^k}
 =(1-z)^{r-k}
...
\begin{equation}\label{p3:eq:fabius-chain}
...
Walsh transform
\begin{equation}\label{p3:eq:walsh-transform-def}
 \mathcal W_m(a)=\sum_{x\in(\Ftwo)^m}
  (-1)^{\langle\mathbf1,x\rangle}(-1)^{\langle a,x\rangle}.
\end{equation}
Character orthogonality gives
\begin{equation}\label{p3:eq:walsh-one-point}
 \boxed{\qquad
 \mathcal W_m(a)=
 \begin{cases}
 2^m,&a=\mathbf1,\\
...
\begin{proposition}[Exact DFT support and sine product]\label{p3:prop:dft-sine}
If $k$ is even, then $\widehat\eps_m(k)=0$.  If $k$ is odd, then
\begin{equation}\label{p3:eq:dft-sine}
 \widehat\eps_m(k)
 =(2\ii)^m\e^{-\pi\ii k(1-2^{-m})}
 \prod_{r=1}^{m}\sin\!\left(\frac{\pi k}{2^r}\right).
\end{equation}
In particular,
\begin{equation}\label{p3:eq:dft-magnitude}
 |\widehat\eps_m(k)|
 =2^m\prod_{r=1}^{m}
...
\begin{equation}\label{p3:eq:dft-parseval}
...
$n\ge0$,
\begin{equation}\label{p3:eq:rademacher-sign}
 \eps_n=\prod_{j\ge0}
...
product \cref{p3:eq:dft-magnitude}.

\section{Gelfond sums and exact residue-class rarefaction}\label{p3:sec:gelfond}

\subsection{The sharp uniform exponent}

Put
\begin{equation}\label{p3:eq:thue-morse-polynomial-circle}
 p_m(x)=P_m(\e^{2\pi\ii x})
 =\sum_{n<2^m}\eps_n\e^{2\pi\ii nx}.
\end{equation}
Then
\begin{equation}\label{p3:eq:p-magnitude-sine}
 |p_m(x)|=2^m\prod_{j=0}^{m-1}|\sin(\pi2^jx)|.
\end{equation}
The following is Gelfond's sharp uniform estimate
\cite{p3:gelfond1968,p3:queffelec2018}.

\begin{theorem}[Gelfond]\label{p3:thm:gelfond}
For every $m\ge1$,
\begin{equation}\label{p3:eq:gelfond-bound}
 \sup_{x\in\R}|p_m(x)|
 \le\frac{2}{\sqrt3}\,2^{m\log 3/\log4}
 =\frac{2}{\sqrt3}\,3^{m/2}.
\end{equation}
Moreover,
\begin{equation}\label{p3:eq:gelfond-lower}
 \sup_{x\in\R}|p_m(x)|\ge3^{m/2}.
\end{equation}
Hence the optimal uniform growth exponent is
\begin{equation}\label{p3:eq:gelfond-exponent}
...
\begin{equation}\label{p3:eq:gelfond-general-N}
 \sup_{x\in\R}\left|
...
\begin{theorem}[Exact dyadic rarefaction modulo three]\label{p3:thm:mod3-rarefaction}
Let
\begin{equation}\label{p3:eq:A-mr3}
 A_{m,r}=\sum_{\substack{0\le n<2^m\\n\equiv r\pmod3}}\eps_n.
\end{equation}
For $s\ge1$,
\begin{equation}\label{p3:eq:mod3-even-block}
 \bigl(A_{2s,0},A_{2s,1},A_{2s,2}\bigr)
 =3^{s-1}(2,-1,-1).
\end{equation}
For $s\ge0$,
\begin{equation}\label{p3:eq:mod3-odd-block}
...
\begin{equation}\label{p3:eq:mod3-filter-proof}
 A_{m,r}=\frac13\left(
...
\cref{p3:eq:mod3-filter-proof}, using $1+\omega+\omega^2=0$, gives the two
vectors.
\end{proof}
...
\cref{p3:eq:gelfond-lower}; the rarefied sums and the extremal Fourier values are
...
\begin{equation}\label{p3:eq:rarefied-matrix-recursion}
 A_{m+1,r}^{(q)}
 =A_{m,\,2^{-1}r}^{(q)}
  -A_{m,\,2^{-1}(r-1)}^{(q)},
\end{equation}
...
transform reproduces \cref{p3:eq:rarefied-root-filter}.  This is a convenient
...
\begin{equation}\label{p3:eq:autocorrelation-def}
...
\begin{theorem}[Autocorrelation recursion]\label{p3:thm:autocorrelation-recursion}
For $n\ge0$,
\begin{align}
 \eta(2n)&=\eta(n),\label{p3:eq:eta-even}\\
 \eta(2n+1)&=-\frac12\bigl(\eta(n)+\eta(n+1)\bigr).
 \label{p3:eq:eta-odd}
\end{align}
In particular,
\begin{equation}\label{p3:eq:eta-one}
 \eta(1)=-\frac13.
\end{equation}
\end{theorem}
...
Putting $n=0$ in \cref{p3:eq:eta-odd} gives
$\eta(1)=-(1+\eta(1))/2$.
\end{proof}
...
Define
\begin{equation}\label{p3:eq:C-definition}
 C(z)=\sum_{n\ge0}\eta(n)z^n,
 \qquad |z|<1.
\end{equation}

\begin{theorem}[Autocorrelation Mahler equation]\label{p3:thm:C-functional}
The autocorrelation generating series satisfies
\begin{equation}\label{p3:eq:C-functional}
 \boxed{\qquad
 2zC(z)=1-(1-z)^2C(z^2).
 \qquad}
\end{equation}
...
\cref{p3:eq:eta-even,p3:eq:eta-odd}:
\begin{align*}
 C(z)
...
\begin{align}
 ...
 \label{p3:eq:Riesz-density}
\end{align}
Define the probability measures
\begin{equation}\label{p3:eq:Riesz-measures}
 \dd\mu_m(x)=
...
\begin{theorem}[Classical spectral limit]\label{p3:thm:riesz-limit}
The measures $\mu_m$ converge weakly to a probability measure $\mu$ whose
Fourier coefficients are
\begin{equation}\label{p3:eq:mu-fourier}
 \widehat\mu(k)=\eta(k).
\end{equation}
...
\cite{p3:queffelec2010,p3:baakegrimm2008,p3:alloucheshallit2003}.  The finite identity
\cref{p3:eq:Riesz-density}, however, is elementary and exact.  It shows that the
...
for its Fourier coefficients through \cref{p3:eq:eta-even,p3:eq:eta-odd}.  The
...
For $t>0$,
\begin{equation}\label{p3:eq:laplace-discrete}
 \sum_{n\ge0}\eps_n\e^{-nt}
 =E(\e^{-t})
 =\prod_{j\ge0}\left(1-\e^{-2^jt}\right).
\end{equation}
The zero--one version is
\begin{equation}\label{p3:eq:laplace-tau}
...
\begin{equation}\label{p3:eq:shifted-dirichlet}
...
\begin{proposition}[Mellin representation]\label{p3:prop:mellin-dirichlet}
For $a>0$ and $\Re s>0$,
\begin{equation}\label{p3:eq:mellin-dirichlet}
 \boxed{\qquad
 D(s,a)=\frac1{\Gamma(s)}
...
\end{equation}
Equivalently,
\begin{equation}\label{p3:eq:unit-interval-dirichlet}
 D(s,a)=\frac1{\Gamma(s)}
 \int_0^1(-\log x)^{s-1}x^{a-1}E(x)\dd x.
\end{equation}
For $s=1$ this reduces to
\begin{equation}\label{p3:eq:integral-D1}
...
\begin{equation}\label{p3:eq:dirichlet-functional}
 D(s,a)=2^{-s}
...
\begin{equation}\label{p3:eq:positive-reciprocal-block}
 \boxed{\quad
...
For $a>0$, apply \cref{p3:thm:cubature} to $f(x)=\log(x+a)$.  Since for $m\ge1$
\[
 f^{(m)}(x)=(-1)^{m-1}(m-1)!(x+a)^{-m},
\]
we get
\begin{equation}\label{p3:eq:finite-log-block}
...
\end{equation}
Exponentiating,
\begin{equation}\label{p3:eq:finite-product-less-one}
...
\begin{equation}\label{p3:eq:xi-b-def}
 \xi_b=\sum_{n\ge0}\frac{\tau(n)}{b^{n+1}}.
\end{equation}
From \cref{p3:eq:T-from-E},
\begin{equation}\label{p3:eq:xi-b-product}
 \boxed{\quad
 \xi_b=\frac1{2(b-1)}
...
\begin{equation}\label{p3:eq:signed-mahler-number}
 \chi_b=\sum_{n\ge0}\frac{\eps_n}{b^{n+1}}
 =\frac1bE(1/b),
\end{equation}
and
\begin{equation}\label{p3:eq:xi-chi}
...
\cite{p3:mahler1929}.  A much finer theorem of Bugeaud determines their rational
approximation exponent \cite{p3:bugeaud2011}.

\begin{theorem}[Mahler; Bugeaud]\label{p3:thm:mahler-bugeaud}
For every integer $b\ge2$, the Thue--Morse--Mahler number $\xi_b$ is
transcendental.  Its irrationality exponent is exactly
\begin{equation}\label{p3:eq:irrationality-exponent-two}
 \mu(\xi_b)=2.
\end{equation}
\end{theorem}
...
\begin{equation}\label{p3:eq:G-ab}
 G(a,b)=\prod_{n\ge0}
...
\begin{proposition}[Even--odd product equation]\label{p3:prop:G-functional}
\begin{equation}\label{p3:eq:G-functional}
 \boxed{\qquad
...
\begin{equation}\label{p3:eq:woods-robbins}
 \boxed{\qquad
 \prod_{n\ge0}
 ...
 \qquad}
\end{equation}
A short proof uses only even--odd splitting \cite{p3:woods1978,p3:robbins1979}.
Put
\begin{equation}\label{p3:eq:A-B-products}
...
\begin{align}
 ...
 &=\frac12,\label{p3:eq:block-product-1}\\
 \lim_{m\to\infty}
 \prod_{k=0}^{2^m-1}(k+1)^{\eps_k}
 &=\frac1{\sqrt2},\label{p3:eq:block-product-2}\\
 \lim_{m\to\infty}
 \prod_{k=0}^{2^m-1}(k+1)^{(-1)^k\eps_k}
 &=\frac1{2\sqrt2}.
 \label{p3:eq:block-product-3}
\end{align}
...
\cite{p3:alloucheriasatshallit2019}, and the second is
\cref{p3:eq:woods-robbins}.  For $m\ge2$, pairing the third product gives the
...
\begin{equation}\label{p3:eq:baseq-character}
 u_n=\zeta^{s_q(n)}.
\end{equation}
If $0\le r<q$, then
\begin{equation}\label{p3:eq:baseq-recursion}
 u_{qn+r}=\zeta^r u_n.
\end{equation}
...
For $m\ge0$,
\begin{equation}\label{p3:eq:baseq-finite-product}
...
\begin{equation}\label{p3:eq:baseq-infinite}
 U(z)=\sum_{n\ge0}u_nz^n
\end{equation}
satisfies
\begin{equation}\label{p3:eq:baseq-mahler}
...
At $z=1$, every factor in \cref{p3:eq:baseq-finite-product} vanishes because
...
\begin{theorem}[Root-of-unity Prouhet identity]\label{p3:thm:baseq-prouhet}
For $0\le r<m$,
\begin{equation}\label{p3:eq:baseq-prouhet}
 \sum_{n=0}^{q^m-1}\zeta^{s_q(n)}n^r=0.
\end{equation}
\end{theorem}

Choose a primitive $q$th root $\omega$ and apply
\cref{p3:thm:baseq-prouhet} successively to $\zeta=\omega^\ell$ for
...
\begin{equation}\label{p3:eq:general-digit-weight-product}
 \sum_{n<q^m}u_nz^n
 =\prod_{j=0}^{m-1}
...
product in \cref{p3:eq:general-digit-weight-product} has a zero of order $dm$.
...
\section{Hankel determinants, Pad\'e approximation, and natural boundaries}\label{p3:sec:hankel}

\subsection{Hankel matrices}

For a sequence $c=(c_n)_{n\ge0}$, its order-$r$ Hankel determinant is
\begin{equation}\label{p3:eq:hankel-def}
...
\begin{theorem}[Allouche--Peyri\`ere--Wen--Wen]\label{p3:thm:APWW}
For the signed Thue--Morse sequence $\eps=(\eps_n)_{n\ge0}$,
\begin{equation}\label{p3:eq:hankel-nonzero}
 H_r(\eps)\ne0
...
\cite{p3:apww1998}; a later combinatorial proof was given by Bugeaud and Han
\cite{p3:bugeaudhan2014}.  It is one of the striking places where automatic
...
\begin{equation}\label{p3:eq:pade-order}
...
P\'olya--Carlson theorem \cite{p3:carlson1921} therefore implies:

\begin{theorem}[Natural boundary]\label{p3:thm:natural-boundary}
The unit circle is a natural boundary for $E(z)$.  Hence $E(z)$ is
transcendental over $\C(z)$.
\end{theorem}

The same conclusion holds for $T(z)$ because of \cref{p3:eq:T-from-E}.  This
sharp contrast is worth emphasizing:
\begin{equation}\label{p3:eq:char-contrast}
 \begin{array}{c|c}
...
matrix
\begin{equation}\label{p3:eq:substitution-matrix}
...
\cref{p3:thm:prefix-discrepancy}.

\subsection{Overlap-freeness}

A word is an overlap if it has the form $aXaXa$.  Thue's classical theorem
states that the infinite fixed point of \cref{p3:eq:morphism} is overlap-free.
...
\subsection{Aperiodicity}\label{p3:subsec:aperiodicity}
...
\cref{p3:eq:reflection-rule} imply
\[
...
instances of \cref{p3:eq:block-recursion,p3:eq:reflection-rule}.
...
Rarefaction mod $3$ & \cref{p3:eq:mod3-even-block,p3:eq:mod3-odd-block} & cubic-root filter \\
