...
\cite{p3:proveit-primary,p3:proveit-frontiers,p3:proveit-atlas,p3:proveit-fourier,p3:proveit-archive}.
...
\end{enumerate}
A complete source ledger appears in \cref{p3:app:ledger}.
...
Lambert-$W$ sources used for context \cite{p3:arias1982,p3:arias2017arith,p3:aistleitner2015,p3:toth2022,p3:corless1996,p3:allouche2003}, but did not constitute an exhaustive priority search.  Accordingly, every novelty statement in this report is
corpus-relative.

\section{Objects, normalizations, and exact background}
\label{p3:sec:objects}
...
\cite{p3:proveit-primary,p3:arias1982}.
...
saddle expansion.  Our use of \eqref{p3:eq:s0} below is computational: it measures how many
...
\begin{equation}
 ...
 \label{p3:eq:normalized-TM-Fourier}
\end{equation}
\newresult
\end{theorem}
...
\eqref{p3:eq:finite-sinc}; the sum form follows from \eqref{p3:eq:Pm}.
\end{proof}

\begin{remark}[Prouhet cancellation becomes the normalization]
The factor $t^{-m}$ in \eqref{p3:eq:normalized-TM-Fourier} is harmless precisely because
...
\label{p3:prop:tail-self}
For every $m\ge0$,
\begin{align}
 Q_\infty(t)&=Q_m(t)Q_\infty(t/2^m),
 \label{p3:eq:Q-tail-self}\\
 G(-s)&=G_m(-s)G(-s/2^m).
 \label{p3:eq:G-tail-self}
\end{align}
\newresult
\end{proposition}
...
\label{p3:thm:all-orders-Q}
Let $m\ge0$.  For
\begin{equation}
 |t|<\pi2^{m+1},
 \label{p3:eq:Q-domain}
\end{equation}
...
\begin{equation}
 ...
 t^{2r}4^{-rm}.}
 \label{p3:eq:Q-all-orders}
\end{equation}
The disk in \eqref{p3:eq:Q-domain} is maximal for this Taylor logarithm: its boundary contains
the nearest zeros $t=\pm\pi2^{m+1}$ of the tail.
\newresult
\end{theorem}

\begin{proof}
By \cref{p3:prop:tail-self}, the quotient is $Q_\infty(t/2^m)$.  Apply
\cref{p3:lem:logsinc} to every factor and sum:
\begin{align*}
 \log Q_\infty(t/2^m)
...
\begin{equation}
 ...
 -\cdots\right).
 \label{p3:eq:Q-first-terms}
\end{equation}

\begin{theorem}[Certified tail remainder]
\label{p3:thm:Q-remainder}
Let $N\ge0$ and put
\begin{equation}
...
\eqref{p3:eq:Q-all-orders} with $r\ge N+1$, then
\begin{equation}
 \boxed{
 |R_{N,m}(t)|
 \le \frac{4\zeta(2)}{3}
 \frac{x_m(t)^{2N+2}}{(N+1)(1-x_m(t)^2)}.}
 \label{p3:eq:Q-remainder-bound}
\end{equation}
...
\begin{equation}
 ...
 \label{p3:eq:Qcorrected}
\end{equation}
For $N=0$, this is $Q_m$.
\end{definition}

\begin{corollary}[Arbitrary-order convergence]
\label{p3:cor:arbitrary-Q}
For every compact set $K\subset\C$ and every fixed $N\ge0$,
\begin{equation}
 Q_m^{[N]}(t)=Q_\infty(t)
 \left(1+\bigO_K(4^{-(N+1)m})\right)
 \qquad(t\in K,\ m\to\infty).
 \label{p3:eq:arbitrary-Q}
\end{equation}
Moreover, after inserting \eqref{p3:eq:normalized-TM-Fourier}, the left side is a normalized
length-$2^m$ Thue--Morse exponential sum multiplied by only $N$ explicit zeta corrections.
\newresult
\end{corollary}

\begin{proof}
For all sufficiently large $m$, $K$ lies in the disk \eqref{p3:eq:Q-domain}.  The exact quotient
$Q_\infty/Q_m^{[N]}$ is $\exp(-R_{N,m})$.  The bound
\eqref{p3:eq:Q-remainder-bound}, uniformly on $K$, is $\bigO_K(4^{-(N+1)m})$; exponentiation
preserves that order.
\end{proof}

\begin{corollary}[Fourier approximation to the up-function]
\label{p3:cor:up-Fourier-block}
For fixed $\xi\in\C$,
\begin{align}
 \widehat\Up(\xi)
...
Put $q=1/4$.  Then \eqref{p3:eq:Q-all-orders} becomes
\begin{equation}
 \log\frac{Q_\infty(t)}{Q_m(t)}
 =-\sum_{r=1}^{\infty}
 \frac{\zeta(2r)}{r\pi^{2r}}t^{2r}
 \frac{q^{r(m+1)}}{1-q^r}.
 \label{p3:eq:Q-Lambert}
\end{equation}
...
\begin{proof}
Repeat the proof of \cref{p3:thm:all-orders-Q}, replacing the geometric sum in $4^{-r}$ by the
one in $b^{-2r}$.
\end{proof}

\section{An all-orders finite-block formula for the endpoint Laplace product}
\label{p3:sec:Laplace-tail}
...
\begin{equation}
 ...
 \label{p3:eq:centered-G}
\end{equation}
Then $C(-s)=Q_\infty(\ii s/2)$ and
\begin{equation}
 C(-s)=C_m(-s)C(-s/2^m).
 \label{p3:eq:centered-tail}
\end{equation}
...
\begin{proof}
Use \cref{p3:lem:GQ} and \cref{p3:thm:all-orders-Q} with $m=0$ and $t=\ii u/2$:
\[
 \log G(-u)=-\frac u2
...
\label{p3:cor:G-remainder}
Let $y=|u|/(4\pi)<1$.  If the series in \eqref{p3:eq:G-all-orders} is stopped after $u^{2N}$,
...
\begin{proof}
This is \cref{p3:thm:Q-remainder} at $m=0$, $t=\ii u/2$.
\end{proof}
...
\begin{equation}
 ...
 \end{aligned}}
 \label{p3:eq:G-TM-all}
\end{equation}
Stopping the correction series after $r=N$ incurs the explicit logarithmic error
\eqref{p3:eq:G-rem}.  In sum form, $P_m(\e^{-u})=\sum_{n<2^m}\eps_n\e^{-nu}$.
\newresult
\end{theorem}

\begin{proof}
Combine the exact factorization \eqref{p3:eq:full-G-TM} with
\cref{p3:thm:G-all-orders} applied to the tail $G(-u)$.
\end{proof}
...
saddle.  Formula \eqref{p3:eq:G-TM-all} gives an intermediate object: a single finite
...
\label{p3:sec:richardson}

\subsection{An analytic error germ in $q^m$}

Put $q=1/4$.  From \eqref{p3:eq:Q-tail-self},
\begin{equation}
...
\label{p3:lem:weights}
The numbers in \eqref{p3:eq:weights} are the Lagrange weights for evaluating at $r=0$ from the
geometric nodes $1,q,\ldots,q^p$:
\begin{equation}
 w_{p,j}=\prod_{\substack{0\le\ell\le p\\\ell\ne j}}
 \frac{-q^\ell}{q^j-q^\ell}.
 \label{p3:eq:weights-Lagrange}
\end{equation}
Consequently,
\begin{equation}
 \sum_{j=0}^{p}w_{p,j}=1,
 ...
 \label{p3:eq:moment-cancel}
\end{equation}
and
\begin{equation}
 \sum_{j=0}^{p}w_{p,j}q^{j(p+1)}
 =(-1)^pq^{p(p+1)/2}.
 \label{p3:eq:first-uncancelled}
\end{equation}
\end{lemma}

\begin{proof}
Split the denominator in \eqref{p3:eq:weights-Lagrange} into $\ell<j$ and $\ell>j$, factor the
powers of $q$, and collect the factors $1-q^k$.  This gives \eqref{p3:eq:weights}.  The moment
...
$(-1)^{p+1}q^{p(p+1)/2}$ for $f-P$, hence \eqref{p3:eq:first-uncancelled} for $P-f$.
\end{proof}

\begin{theorem}[$q$-binomial spectral Richardson extrapolation]
\label{p3:thm:qRichardson}
Define
\begin{equation}
...
\eqref{p3:eq:normalized-TM-Fourier}; hence \eqref{p3:eq:Rmp} is a finite digital extrapolation
formula with Gaussian-binomial coefficients.
\newresult
\end{theorem}

\begin{proof}
Insert \eqref{p3:eq:Hseries} into \eqref{p3:eq:Rmp}:
\begin{align*}
...
\eqref{p3:eq:first-uncancelled} at $n=p+1$.  Local uniform convergence of the analytic $H_t$
series gives the uniform remainder on compact $t$-sets.
\end{proof}

\begin{corollary}[A derivative bound version]
\label{p3:cor:Richardson-bound}
...
\begin{equation}
 ...
 H_t^{(p+1)}(\theta).
 \label{p3:eq:Richardson-remainder}
\end{equation}
...
The zeta correction \eqref{p3:eq:Qcorrected} uses one block level and analytic coefficients.
The $q$-Richardson formula \eqref{p3:eq:Rmp} uses $p+1$ consecutive block levels but no zeta
...
the corresponding densities gives \eqref{p3:eq:density-deconv}.
\end{proof}
...
\]
Set $m=0$ in \eqref{p3:eq:Q-all-orders} and identify coefficients.  The Bernoulli form follows
...
\begin{equation}
 ...
 \label{p3:eq:weak-bound}
\end{equation}
\newresult
\end{theorem}

\begin{proof}
Condition on $Y_m$ in \eqref{p3:eq:random-tail} and Taylor-expand
...
\begin{equation}
 ...
 \label{p3:eq:cumulant-operator}
\end{equation}
Expanding the exponential changes cumulants into moments and recovers
\eqref{p3:eq:distributional}.  This is a deconvolution calculus for the finite uniform splines
...
\label{p3:sec:Lambert-complexity}

\subsection{The canonical scale and the tail variable}

For $0<x<1/(\e L)$, let $\lambda(x)$ and $s_0(x)$ be given by
\eqref{p3:eq:lambda}--\eqref{p3:eq:s0}.  At an integer truncation level $m$, the entire omitted
...
\begin{proof}
Equation \eqref{p3:eq:tail-orders} is the beginning of \eqref{p3:eq:G-all-orders}:
\[
...
Combine these inequalities with \eqref{p3:eq:u-window}.  Solving $2^{-h}\asymp\eta$ and
$4^{-h}\asymp\eta$, then adding the base depth $\lambda(x)$ and absorbing the ceiling into
$\bigO(1)$, gives \eqref{p3:eq:mraw}--\eqref{p3:eq:mctr}.
\end{proof}
...
\]
where $c_N$ is a small safety margin determined from \eqref{p3:eq:G-rem}.
\item Set $u=s_0/2^m$ and evaluate the finite digital sum
\[
 P_m(\e^{-u})=\sum_{n=0}^{2^m-1}\eps_n\e^{-nu}
\]
or its product form.
\item Reconstruct $G(-s_0)$ from \eqref{p3:eq:G-TM-all}, using the first $N$ Bernoulli
...
\]
The Mellin integral in \eqref{p3:eq:D-Mellin} is finite there, so differentiation gives
\eqref{p3:eq:gamma-Mellin}.

Differentiate \eqref{p3:eq:D-dyadic} with respect to $s$ and set $s=-r$.  The derivative of
$2^{-s}$ is multiplied by the bracket of values at $s=-r$, which vanishes by
\eqref{p3:eq:Dzeros}.  Hence
\[
 ...
 \right].
\]
Exponentiation gives \eqref{p3:eq:gamma-dyadic}.
...
\]
At $s=-r$ with $r\ge1$, the first term vanishes by \eqref{p3:eq:Dzeros}, leaving
$r\partial_s\widetilde D(1-r,a)$.  This proves \eqref{p3:eq:gamma-differential}; iteration gives
\eqref{p3:eq:gamma-iterated}.
\end{proof}

\begin{corollary}[The Woods--Robbins layer]
\label{p3:cor:Woods-Robbins}
...
to the derivative ladder \eqref{p3:eq:gamma-differential}: higher levels integrate logarithms of
...
\midrule
oscillatory \eqref{p3:eq:finite-sinc} & $m=1$ & $t=0.37$ & $7.3\times10^{-102}$\\
oscillatory \eqref{p3:eq:finite-sinc} & $m=3$ & $t=2.125$ & $1.9\times10^{-101}$\\
oscillatory \eqref{p3:eq:finite-sinc} & $m=7$ & $t=5.75$ & $7.0\times10^{-101}$\\
oscillatory \eqref{p3:eq:finite-sinc} & $m=10$ & $t=-3.4$ & $3.8\times10^{-99}$\\
Laplace \eqref{p3:eq:finite-Laplace} & $m=4$ & $s=3.2$ & $5.6\times10^{-101}$\\
Laplace \eqref{p3:eq:finite-Laplace} & $m=8$ & $s=11.5$ & $3.0\times10^{-100}$\\
Laplace \eqref{p3:eq:finite-Laplace} & $m=12$ & $s=0.019$ & $9.4\times10^{-96}$\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.97\textwidth]{assets/Fabius_Rvachev_Thue_Morse_Representation_Frontiers/figures/spectral_convergence.png}
...
\cref{p3:cor:arbitrary-Q,p3:thm:qRichardson}.}
\label{p3:fig:spectral-convergence}
\end{figure}
...
\cref{p3:thm:weak-expansion} becomes a direct moment correction to the finite moment-generating
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.97\textwidth]{assets/Fabius_Rvachev_Thue_Morse_Representation_Frontiers/figures/lambert_truncation.png}
...
as proved in \cref{p3:thm:depth-law}.}
\label{p3:fig:Lambert-depth}
\end{figure}
...
products.  Theorem \ref{p3:thm:finite-sinc} shows these are not merely related by analogy:
...
\]
The weak expansion \eqref{p3:eq:weak-expansion} then supplies every correction coefficient and a
...
negative-Laplace product and its derivatives.  Formula \eqref{p3:eq:G-TM-all} suggests a hybrid
implementation:
\begin{enumerate}
\item use the existing lower-Lambert phase and periodic saddle jets to choose $s$;
\item choose $m$ from the centered depth law \eqref{p3:eq:mctr};
...
complex-analysis layer.  This makes the package a plausible next formalization target.

\section{Lean formalization roadmap}
\label{p3:sec:formalization}
...
\eqref{p3:eq:Pm} & finite products, binary recursion, geometric sums & Prove
$P_{m+1}(z)=P_m(z)(1-z^{2^m})$ and identify coefficients by induction.  This should be almost
entirely algebraic. \\
Finite sinc bridge
\eqref{p3:eq:finite-sinc} & complex exponential identities, finite-product reindexing & Rewrite
...
\eqref{p3:eq:finite-Laplace} & real or complex exponentials, finite products & Substitute
$z=\e^{-s/2^m}$ in the block product and reindex.  This is the simplest new theorem in the
package. \\
Exact tail self-similarity
\eqref{p3:eq:Q-tail-self}--\eqref{p3:eq:G-tail-self} & convergence of infinite products, shifted
...
\eqref{p3:eq:logsinc} & local analytic logarithm, Euler's sine product, normal convergence & This is
the main library-level analytic obligation.  One may alternatively prove the power series by
differentiating the cotangent expansion and fixing the constant at zero. \\
Certified zeta remainder
\eqref{p3:eq:Q-remainder-bound} & positivity, monotonicity of $\zeta(2r)$, geometric series & Once
\eqref{p3:eq:Q-all-orders} exists, the bound is elementary and avoids delicate asymptotic
machinery. \\
$q$-Richardson weights
\eqref{p3:eq:weights}--\eqref{p3:eq:first-uncancelled} & finite Lagrange interpolation,
$q$-Pochhammer algebra & First formalize the interpolation characterization
\eqref{p3:eq:weights-Lagrange}; derive the Gaussian-binomial form as a normalization lemma. \\
Random-tail law
\eqref{p3:eq:random-tail} & independent random variables, almost-sure summation, equality in law &
...
Weak expansion
\eqref{p3:eq:weak-expansion} & Taylor theorem with remainder, expectation inequalities, symmetry &
Condition on the finite sum; use bounded support to make the remainder estimate immediate. \\
Lambert depth law
\eqref{p3:eq:mraw}--\eqref{p3:eq:mctr} & branch $W_{-1}$, ceilings, local exponential estimates & The
...
\eqref{p3:eq:gamma-Mellin}--\eqref{p3:eq:gamma-iterated} & holomorphic parameter differentiation,
...
of new complex analysis that must be trusted at any one time.
...
\label{p3:conj:endpoint-evaluator}
...
The depth estimate itself is proved in \cref{p3:thm:depth-law}.  What remains is to propagate the
...
\begin{equation}
 ...
 \label{p3:eq:cumulant-depth-conj}
\end{equation}
...
This is already a theorem for the isolated product, by \eqref{p3:eq:G-rem}; the conjectural part
...
expansion \eqref{p3:eq:distributional} admits a pointwise or Hölder-norm version through order
...
\label{p3:conj:automatic-Barnes}
The functions $\Gamma_r^{\rm TM}$ possess canonical Weierstrass products and asymptotic
normalizations for which the dyadic law \eqref{p3:eq:gamma-dyadic} and differential ladder
\eqref{p3:eq:gamma-differential} uniquely characterize the hierarchy, analogously to the role of
...
The base-$b$ theorem \eqref{p3:eq:base-b} shows that the zeta--Lambert mechanism does not depend
...
all leading constants in \eqref{p3:eq:Richardson-leading} are exact rational multiples of
$Q_\infty(t)t^{2p+2}$.
