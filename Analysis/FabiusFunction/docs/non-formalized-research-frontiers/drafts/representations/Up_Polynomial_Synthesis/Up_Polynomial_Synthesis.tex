...
fourth-wave drafts that solve the same problem by different routes;
shared foundations are stated once, each construction keeps its own
chapter, their overlapping material (Appell calculus, Thue--Morse
products, rationality, tensor products, extension operators,
conditioning, Lean roadmaps) is merged, and the comparison chapter and
the cross-volume defect identities are new to the consolidation.
...
$\E\,\e^{z(L-R_d/2)}
=\prod_{m=1}^{d}\frac{\sinh(2^{m-1}z)}{2^{m}\sinh(z/2)}$ and
$\MM_u(Mz)=\MM_u(z)\prod_{r=0}^{d-1}\frac{\sinh(2^{r}z)}{2^{r}z}$
cancel factor by factor to \eqref{eq:H-def}.  The logarithm follows
from $\log\bigl[2\sinh(z/2)/z\bigr]
=\sum_mB_{2m}z^{2m}/(2m\,(2m)!)$ and the cumulant series of
$\MM_u$.
\end{proof}
...
\eqref{eq:appell-pair}.  The comparison also settles more of the
minimal-atom question than either source alone.
...
\eqref{eq:H-def}.  Steps (1)--(3) and (5)--(8) are finite algebra;
only (4) needs Schwartz-class Poisson summation.
\end{statusbox}
...
\[
 ...
 H_2=1-\tfrac5{36}z^{2}+\bigO(z^{4}),\quad
...
The exact SHA-256 hashes of the two absorbed \texttt{.tex} sources
...
calculus, the Thue--Morse products, rationality and affine
covariance, tensor products, extension-operator remarks, conditioning
warnings, and the Lean roadmaps, each of which both sources develop.
