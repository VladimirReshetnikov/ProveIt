...
The first two items in the original algebraic plan are now discharged
generically; the remaining order is:
\begin{enumerate}
...
exact total variation.  The generic condition-number identity
\cref{eq:qcondition} and all analytic error, remainder, and convergence
claims remain frontier.
...
longer formalization obligations.  The Bell identity \eqref{eq:h-Bell} for the
reciprocal Lambert nodes, the transformed sinc-tail identities, the
Richardson remainder and sign estimates, and every analytic convergence claim
in this report remain distinct frontier obligations.
...
\cref{thm:Psi-extraction}.  The arbitrary-node interpolation foundation and
its all-order residual representation are already available in
\path{CompleteHomogeneous.lean} and
\path{LagrangeResidualMoments.lean}.  The closed reciprocal Lambert-grid
specialization in \cref{thm:lambert-moments}, unlike the geometric
\(q^j\)-grid specialization, remains to be formalized together with its
phase-locked use of the uniform remainders.
