...
The finite (q)-filter algebra and reciprocal-grid phase locking, closed
weights, cancellation, and all-order residual moments are now machine-checked.
\texttt{CompleteHomogeneous.lean} evaluates Mathlib's complete homogeneous
symmetric polynomial on arbitrary finite families, while
\texttt{LagrangeResidualMoments.lean} identifies the corresponding monomial
quotient and remainder by a monic nodal polynomial and proves every higher
moment of an arbitrary polynomially exact finite row.  Its Lagrange
specialization is
\path{sum_lagrangeEvalWeight_mul_pow_card_add}, and the geometric-grid
specialization is
\path{sum_geometricLagrangeWeight_mul_pow_card_add}.

The denominator-free bridge \texttt{GeometricQBinomialLagrange.lean} proves
the arbitrary-ratio numerator identity over commutative rings.  Its theorem
\texttt{reversed\_finite\_qBinomial\_theorem} identifies the generating
polynomial with \(\prod_{k<r}(z-q^{k+1})\), and
\texttt{sum\_geometricLagrangeWeight\_mul\_pow\_eq\_gaussianBinomial}
computes every positive residual moment over a field from finite-node
injectivity.  The reversed index in its numerator is deliberate; no
unproved Gaussian symmetry is used.

The modules \texttt{GeometricRichardson.lean},
\texttt{GeometricLagrangeWeights.lean},
\texttt{GeometricLagrangeQBinomial.lean}, and
\texttt{GeometricLagrangeQMoments.lean} identify the finite geometric
Richardson polynomial and its Lagrange weights, prove the q-Pochhammer and
Gaussian-binomial weight formulas, and compute every rational \(q\)-moment
under a nonzero-base and nonvanishing finite-denominator hypothesis.  For
\(0<q<1\) they give every positive residual, establish its exact sign, and
compute the finite exact \(\ell^1\) norm.  In particular,
\path{quarterGeometricLagrangeQMoment_eq_residual_qBinomial} and
\path{sum_abs_quarterGeometricLagrangeWeight_eq_qPochhammer_ratio} supply the
report-facing \(q=1/4\) algebra.  At \(q=1/2\),
\texttt{FabiusDiscreteLimitToeplitz.lean} identifies the same weights with
the normalized Toeplitz row and supplies its reverse moments and exact total
variation.

Independently, \texttt{ExponentialPartition.lean} and
...
\(\mathbb Q\)-algebra.  The module \texttt{MomentCumulantAlgebra.lean}
factorially normalizes that engine into named complete-Bell and inverse
moment--cumulant transforms.  Thus the coefficient mechanism behind
\eqref{eq:h-Bell} is available generically, although its displayed
complete-homogeneous power-sum specialization has not yet been packaged as a
theorem.

Finally, \texttt{LambertPhaseLockedRichardson.lean} proves exact lower-Lambert
phase locking, the factorial and binomial reciprocal-grid weights, their
cancellation moments, and the report-shaped all-order identity
\path{sum_shiftedReciprocalLagrangeWeight_mul_invPow_eq_completeHomogeneous}.
The proof is a specialization of the universal Lagrange residual theorem, so
it requires neither rational-function differentiation nor analytic
coefficient extraction.

These declarations do not prove the generating-series characterization
\eqref{eq:complete-homogeneous}, the Bell/harmonic-sum identity
\eqref{eq:h-Bell}, the infinite limit or decimal value in
\eqref{eq:qcondition-limit}--\eqref{eq:qcondition-dyadic}, the Lambert-row
conditioning asymptotics and Bernoulli diagnostics, the derivative-extractor
weights, any sinc-tail transformed-error or bracketing theorem, or any
analytic phase-extraction estimate.
