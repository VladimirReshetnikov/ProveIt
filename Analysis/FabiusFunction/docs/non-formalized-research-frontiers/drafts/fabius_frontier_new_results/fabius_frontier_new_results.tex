...
\item The phase-locked Lambert transform in Section~\ref{sec:lambert} is \statusconditional: the interpolation algebra is proved here, while the input all-orders expansion is inherited from the frontier analysis.
...
\begin{proof}
For $k\ge N+1$ we have $|t|/b^k\le\rho_N<\pi$, so every omitted sinc factor is positive. Hence truncation and limit have the same sign. Applying \cref{lem:logsinc} and using absolute convergence,
\begin{align*}
...
\item formalize the Bell-polynomial conversions as finite coefficient identities.
\end{enumerate}
...
specializations.  It does not yet give the closed Gaussian--\(q\)-binomial
weights, all higher residual moments, or the exact \(\ell^1\) condition number
used above.  Independently, \texttt{ExponentialPartition.lean} and
...
\(\mathbb Q\)-algebra.  In particular, the coefficient mechanism behind
\eqref{eq:h-Bell} can reuse that bridge with
\(E_k=k^{-1}\sum_i a_i^k\) for \(k\ge1\).  The displayed
complete-homogeneous/Bell
specialization, the Richardson error formulas, and every analytic estimate in
this report remain distinct formalization obligations.
...
The Lambert extraction theorem should remain in the frontier namespace until the exact all-orders endpoint expansion, including uniformity in phase and the precise main-term normalization, has a matching Lean theorem. The interpolation algebra itself can be formalized independently now.
...
\begin{conjecture}[Nested parity brackets]
For fixed admissible $(b,N,t)$,
\[
|\widehat P_{b;N,0}|>|\widehat P_{b;N,2}|>
|\widehat P_{b;N,4}|>\cdots>|P_b|
\]
and
\[
|\widehat P_{b;N,1}|<|\widehat P_{b;N,3}|<
|\widehat P_{b;N,5}|<\cdots<|P_b|.
\]
\end{conjecture}
The sign theorem proves only that each parity lies on the correct side. Nestedness would turn the transform into a canonical sequence of shrinking intervals and appears approachable from total positivity of the geometric interpolation kernel.
