...
Since that pinned snapshot, \path{WeightedUniformSeries.lean} has formalized the reusable
product space and absolutely summable Banach-valued uniform-coordinate series: pointwise
summability, continuity and measurability, arbitrary-weight head--tail and reflection
identities, the norm bound, and the nonnegative and unit-mass range bounds.  The central
declarations include \path{weightedUniformSeries}, \path{weightedUniformSeries_split},
\path{weightedUniformSeries_reflect}, and \path{weightedUniformSeries_mem_unitInterval}.
This module does not define the geometric weights $(1-q)q^j$ or a $q$-specific law,
pushforward/CDF, refinement equation, characteristic-function product, moments, cumulants,
full-support theorem, or density.  Thus all report-specific $q$-family statements remain
frontier claims.
\end{boxedremark}
...
Since the pinned snapshot, item 2 has acquired reusable generic construction infrastructure
in \path{WeightedUniformSeries.lean}: \path{weightedUniformSeries} constructs a pointwise
summable series for arbitrary norm-summable weights on the independent-uniform product
space, but the geometric weights and every $q$-specific specialization in items 1--4 remain
open.
