...
Equation \eqref{eq:local-factorization} is now formal in a stronger total,
...
The cofactor is analytic and nonzero, so Lean proves the exact order, every
lower-jet vanishing, and the leading identity
\eqref{eq:first-zero-jet}.  The generic leading-jet engine is
\path{LeadingJet.lean}.  The logarithmic coefficients
\eqref{eq:gamma1}--\eqref{eq:gamma-general} and the Bell-polynomial formula
\eqref{eq:Bell-zero-jets} for \(s>0\) remain human-readable frontier
...
\input{quadrature_table_display.tex}
\end{table}
...
\input{spectral_check_display.tex}
\end{table}
