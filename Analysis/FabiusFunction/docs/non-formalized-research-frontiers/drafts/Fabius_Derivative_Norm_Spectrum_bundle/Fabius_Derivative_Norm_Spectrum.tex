...
The exact statements in \Cref{sec:equimeasurability,sec:beta-transform,sec:stieltjes-wigert}
are unusually suitable for Lean because their proofs reduce to finite sums, affine changes of
variables, Parseval, and determinant algebra.  A practical order is:
\begin{enumerate}
\item expose the derivative tiling as a public theorem with a cellwise corollary;
\item prove the pushforward/equimeasurability identity for arbitrary measurable test
      functions;
\item derive $L^p$, sup-norm, total-variation, and level-set corollaries;
\item formalize the Stieltjes substitution and beta-transform identity for the totalized
      inverse;
\item prove the spectral moments by the existing Fourier differentiation API;
\item formalize the Vandermonde determinant and finite q-binomial orthogonality;
\item defer the all-orders asymptotic operator until the inverse derivative-jet bounds have a
...
The most promising next step is to identify the sinc-square measure inside the Nevanlinna
family and to formalize the exact equimeasurability/beta-transform layer.  The first is a
substantive analytic frontier; the second is a compact, high-value Lean target that would make
several apparently numerical phenomena exact library theorems.
