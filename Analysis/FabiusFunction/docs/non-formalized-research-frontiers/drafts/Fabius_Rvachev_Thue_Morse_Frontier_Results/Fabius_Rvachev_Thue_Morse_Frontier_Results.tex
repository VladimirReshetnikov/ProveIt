...
\lean{Fabius.sum_geometricLagrangeWeight_mul_pow_succ_product}.  What is \emph{not}
formalized is the closed form \eqref{eq:weights} itself: the passage from the Lagrange
quotient to the $q$-Pochhammer and Gaussian-binomial expression has no counterpart in the
library at present, at $q=1/4$ or at any other ratio.
...
in part it is not a target at all but already done: as the \emph{Formal version} paragraphs
throughout \cref{sec:objects,sec:bridges,sec:richardson,sec:gamma-tower} record, the block
identity, both finite bridges in their unnormalized form, both halves of tail
self-similarity, the Fourier--Laplace rotation, the Lagrange characterization of the
$q$-Richardson weights with all three of its moment identities, and the $r=0$ layer of the
gamma tower already have Lean counterparts, several of them more general than the statements
printed here.
...
\eqref{eq:logsinc} & local analytic logarithm, Euler's sine product, normal convergence & This is
...
\eqref{eq:Q-remainder-bound} & positivity, monotonicity of $\zeta(2r)$, geometric series & Once
\eqref{eq:Q-all-orders} exists, the bound is elementary and avoids delicate asymptotic
machinery. \\
...
\eqref{eq:weights-Lagrange}; derive the Gaussian-binomial form as a normalization lemma. \\
Random-tail law
