...
characteristic-function route anticipated in the promotion table below.  The
random-variable phrasing \eqref{eq:random-tail}, over an explicit product probability
space, is not formalized; the measure identity is its full analytic content.
...
\cref{sec:probability}).  The obligation is retained because the random-series and
cumulant-generating instances --- \eqref{eq:random-tail} as a statement about random
variables, and \eqref{eq:cumulant-operator} --- have no
