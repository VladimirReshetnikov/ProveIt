...
(\lean{Fabius.sinc\_pair\_powerSum}).  Composed with the integral-to-product identity, the
same series is the Fourier transform itself:
$\widehat{\Up}(z)=\exp\bigl(-\sum_{r\ge1}\zeta(2r)4^rz^{2r}/(r(4^r-1))\bigr)$ on $|z|<1$
(\lean{Fabius.rvachevFourier\_eq\_cexp}).  Maximality of the disk is not formalized.
...
\emph{Formal versions.}  The measure form of \eqref{eq:density-deconv} is formalized in
\path{MeasureRefinement.lean}: with $\mu_{\Up}$ the Borel probability measure with density
$\Up$ (\lean{Fabius.rvachevMeasure}), the one-step refinement equation
$\mu_{\Up}=\mathrm{Unif}[-\tfrac12,\tfrac12]\ast\bigl(\mu_{\Up}\circ(\cdot/2)^{-1}\bigr)$ is
\lean{Fabius.rvachevMeasure\_refinement}, and the $m$-fold form matching the split in this
proof --- the first $m$ uniform digits convolved with a $2^{-m}$-scale copy of the whole ---
is \lean{Fabius.rvachevMeasure\_eq\_prefix\_conv}, with the digit-prefix law
\lean{Fabius.uniformDigitPrefix} defined by the digit recursion and its characteristic
function evaluated as the finite sinc product
(\lean{Fabius.charFun\_uniformDigitPrefix}); the uniform-digit integral itself is the
reusable \lean{Fabius.charFun\_volume\_restrict\_Icc}.  The proof runs by exactly the
...
remains unformalized of \eqref{eq:random-tail} is only its equality-in-law splitting as a
...
$m$-fold iterate, via \lean{charFun\_conv} and the formal Fourier product. \\
Weak expansion
...
instance \eqref{eq:density-deconv} is now formal as well, in measure form
(\path{MeasureRefinement.lean}, see the \emph{Formal version} paragraph in
\cref{sec:probability}), and the random-variable representation underlying
\eqref{eq:random-tail} is \path{ProbabilityRepresentation.lean}.  The obligation is
retained because the equality-in-law splitting of the random series and the
cumulant-generating instance \eqref{eq:cumulant-operator} have no
