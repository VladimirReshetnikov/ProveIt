...
with the equality-in-law splitting in \path{RandomSeriesLaw.lean}.  The obligation is
retained only for its cumulant quarter: the cumulant-generating instance
\eqref{p2:eq:cumulant-operator} has no formal counterpart yet, and a single statement
instantiating to all four faces at once remains open --- though the product and
convolution faces now each have the abstract library the obligation asked for.
