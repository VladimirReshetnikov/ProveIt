...
\clearpage
\section*{Provenance}
This volume consolidates the following drafts content-preservingly (labels, citation keys,
and asset paths mechanically prefixed per part; wrapper metadata and counter state
normalized; no mathematical content altered).
...
The current inventory in \cref{p1:sec:formalized-modules} consists of eighty-nine compiled
...
The first tranches of the formalization now exist as eighty-nine
...
Formal power series & Finite/infinite products and coefficient-diagonal filters & Coefficients, Mahler equations, and exact finite filtering become algebraic; analytic evaluation and convergence remain separate obligations. \\
...
\emph{Formal version.}  The unnormalized half of \eqref{p2:eq:finite-sinc} is
\leanPartx{Fabius.sum_thueMorseSign_exp_eq_sin_prod} (\path{ThueMorseSineProduct.lean}), which
...
\leanPartx{Fabius.one_sub_exp_mul_I}.  Setting $x=t/2^{m-1}$ and extracting the linear factor from
each $\sin(t/2^k)$ turns it into \eqref{p2:eq:finite-sinc}.  The Lean statement is therefore this
...
\emph{Formal version.}  The block identity underlying \eqref{p2:eq:finite-Laplace} is
\leanPartx{Fabius.sum_range_two_pow_thueMorseSign_exp} (\path{ThueMorseInfiniteProduct.lean}):
\[
...
content of \eqref{p2:eq:finite-Laplace} \emph{before} division by
...
\eqref{p2:eq:GQ}.  The finite form \eqref{p2:eq:GmQm} has no separate Lean statement.
...
\[
 \texttt{geometricQBinomialWeightNumerator}\ q\ p\ j
 =(-1)^{p-j}q^{\binom{p-j+1}{2}}
   \texttt{gaussianBinomial}\ q\ p\ j
\]
as \texttt{geometricQBinomialWeightNumerator\_eq\_forward}; thus the
...
\texttt{geometricLagrangeSeriesFilter\_eq\_residual\_mk} packages degree
zero and every positive Gaussian residual multiplier in one formal-series
identity, while \texttt{map\_geometricSeriesFilter} transports the entire
identity along a commutative-semiring homomorphism.  These are identities in
\texttt{PowerSeries}; they do not identify formal substitution with analytic
evaluation, prove convergence of the functions \(Q_m\), or supply a uniform
weighted remainder.  Consequently the analytic convergence and remainder
claims in \cref{p2:thm:qRichardson} remain frontier.
...
the random series $Y_\infty$, and the identifications
$F(x)=\Prob[Y\le x]$ and $\Up(x)=\Prob[Y\le1-|x|]$ (Arias de Reyna's Theorem~3).  What
remains unformalized of \eqref{p2:eq:random-tail} is only its equality-in-law splitting as a
statement about those random variables; its analytic content is the measure identity
above.
...
-\partial_s\widetilde D(0,a)$ --- is \leanPartx{Fabius.mpLimit_eq_deriv_sub}
(\path{ThueMorseGDirichlet.lean}), for all $a,b>0$, with $\log G_{\rm WR}(a,b)$ realized as
\leanPartx{Fabius.mpLimit}, the limit of the convergent series
...
\leanPartx{Fabius.hasDerivAt_dirichletMellinContinuation_zero}, itself resting on
$(1/\Gamma)'(0)=1$ (\leanPartx{Fabius.hasDerivAt_Gamma_inv_zero}).  Since
$\Gamma_0^{\rm TM}=\exp\bigl(\partial_s\widetilde D(0,\cdot)\bigr)$ by
\eqref{p2:eq:gammaTM}, \eqref{p2:eq:WR-gamma} is that theorem exponentiated.  The layers $r\ge1$ of
\cref{p2:thm:gamma-tower} are not formalized: the missing ingredient is the derivative of the
...
\eqref{p2:eq:finite-sinc} and the finite Laplace bridge \eqref{p2:eq:finite-Laplace} exist as
\leanPartx{Fabius.sum_thueMorseSign_exp_eq_sin_prod} and
\leanPartx{Fabius.sum_range_two_pow_thueMorseSign_exp}, in each case for real argument and in the
...
\path{GeometricPowerSeriesFilter.lean}; it does not evaluate a formal series as
an analytic function.  The Euler--zeta logarithm and certified zeta remainder
...
\path{MeasureRefinement.lean}.  What remains includes the analytic
Richardson evaluation, weighted uniform remainder and convergence theorem,
the random-variable weak expansion, the Lambert depth law, and the levels
$r\ge1$ of the gamma tower.  In particular, neither the formal filter identity
nor its Gaussian coefficient multipliers imply the analytic bounds or
...
entirely algebraic. \\
Finite sinc bridge
\eqref{p2:eq:finite-sinc} & complex exponential identities, finite-product reindexing & Rewrite
...
\eqref{p2:eq:finite-Laplace} & real or complex exponentials, finite products & Substitute
...
coefficientwise filter; analytic evaluation and its remainder are not part of
this row.  The case \(q=1/2\) is linked to the Toeplitz row. \\
Random-tail law
...
\eqref{p2:eq:gamma-Mellin}--\eqref{p2:eq:gamma-iterated} & holomorphic parameter differentiation,
...
the probabilistic splitting in measure form (\path{MeasureRefinement.lean}); of the
splitting, only the random-variable representation itself is unformalized.
...
convention unambiguously: \leanPartx{Fabius.rvachevFourier} is
...
\leanPartx{Fabius.complexGeneratingFunction_neg_eq_tprod} carries the rotation to $G$.  What the
obligation still asks for is the remaining leg --- the $F\leftrightarrow\Up$ rescaling
\eqref{p2:eq:Fup} and the characteristic function of $Y_\infty$ --- packaged with the others as a
...
instance \eqref{p2:eq:density-deconv} is now formal as well, in measure form
(\path{MeasureRefinement.lean}, see the \emph{Formal version} paragraph in
\cref{p2:sec:probability}), and the random-variable representation underlying
\eqref{p2:eq:random-tail} is \path{ProbabilityRepresentation.lean}.  The obligation is
retained because the equality-in-law splitting of the random series and the
cumulant-generating instance \eqref{p2:eq:cumulant-operator} have no
