...
Fourier convention $\widehat{\Up}(x)=\int_{\R}\Up(t)\e^{-2\pi\ii xt}\dd t$.
...
Rvachev's up-function $\Up$ is an even, nonnegative $C^\infty$ function
supported on $[-1,1]$, normalized by $\int_{\R}\Up=1$.  It is the density of an
...
that $f=\widehat{\Up}$ decreases faster than every inverse power on the real
...
\begin{equation}
 f(z)=\prod_{n=0}^{\infty}\sinc\!\left(\frac{\pi z}{2^n}\right),
 \qquad \sinc w=\begin{cases}\sin w/w,&w\ne0,\\1,&w=0.\end{cases}
 \label{eq:f-product}
\end{equation}
The product converges locally uniformly because
$\log\sinc w=-w^2/6+\bigO(w^4)$ near the origin.
...
\begin{align}
 ...
 &=\prod_{m=1}^{\infty}
   \left(1-\frac{z^2}{m^2}\right)^{1+\vtwo(m)}.             \label{eq:weier-product}
\end{align}
Moreover,
\begin{align}
 f(z)&=\sinc(\pi z)f(z/2),                                  \label{eq:feq-down}\\
 f(2z)&=\sinc(2\pi z)f(z).                                  \label{eq:feq-up}
\end{align}
\end{proposition}

\begin{proof}
The identity $\sinc w=\prod_{j\ge1}\cos(w/2^j)$ follows by iterating the
...
$m\in\Z\setminus\{0\}$ has multiplicity $1+\vtwo(|m|)$.  If $x>0$ is not an
integer and $n=\lfloor x\rfloor$, then
\begin{equation}
 \sgn f(x)=(-1)^{\wt(n)},                                   \label{eq:sign-tm}
\end{equation}
where $\wt(n)$ is the sum of the binary digits of $n$.
\end{proposition}
...
\[
 \sum_{m=1}^{n}(1+\vtwo(m))
 =n+\vtwo(n!)=n+(n-\wt(n))=2n-\wt(n).
\]
Its parity is the parity of $\wt(n)$, proving \eqref{eq:sign-tm}.
\end{proof}
...
\begin{equation}
 \sum_{m=1}^{\infty}\frac{1+\vtwo(m)}{m^2-x^2}=0            \label{eq:critical-equation}
\end{equation}
...
\[
 ...
 H(x)=\sum_{m=1}^{\infty}\frac{1+\vtwo(m)}{m^2-x^2}.
\]
The series and its derivative converge locally uniformly off the integers,
because $\vtwo(m)=\bigO(\log m)$.  For $x>0$,
\[
...
\[
 f(2^Ny)=f(y)\prod_{k=1}^{N}\sinc(\pi2^ky).
\]
...
\begin{equation}
 \log|f(x_N)|
 =-\frac{(\log x_N)^2}{2a}
  -\kappa(\alpha(y))\log x_N+\smallo(\log x_N),            \label{eq:ray-law}
\end{equation}
where
\begin{equation}
...
\[
 \log|f(2^Ny)|
 =-\frac a2N^2+
 N\bigl(\alpha(y)-\log(\pi y)-a/2\bigr)+\smallo(N).
\]
...
\begin{equation}
 \int_0^1\phi(t)\dd t=-\log2=-a.                            \label{eq:phi-mean}
\end{equation}
...
\begin{equation}
 ...
 -\kappa_{\mathrm{typ}}\log(2^Ny)+\smallo(N),
...
\begin{equation}
 ...
 (2^Ny)^{-\kappa_{\mathrm{typ}}+\smallo(1)}.               \label{eq:typical-multiplicative}
\end{equation}
\end{corollary}
...
\begin{equation}
 ...
 =\sum_{n=0}^{2^N-1}(-1)^{\wt(n)}\e^{2\pi\ii nt}           \label{eq:tm-poly}
\end{equation}
...
\begin{equation}
 \sup_{t\in\R}Q_N(t)\le C_G(\sqrt3)^N.                     \label{eq:gelfond}
\end{equation}
...
One has
\begin{equation}
 \int_0^1d(x)^2\dd x=\frac{\pi^2}{4}.                      \label{eq:variance}
\end{equation}
\end{lemma}
...
$\int_0^{\pi/2}\log^2(\tan u)\dd u=\pi^3/8$, yielding
...
Fix $c>0$ and $A\in\R$.  As $N\to\infty$,
\begin{equation}
 \min_{r\in\N_0}
...
\begin{align}
 ...
 +\sum_{j=0}^{r-1}\log\sinc\!\left(\frac{\pi}{3\,2^j}\right)\notag\\
 & =r\log(\pi/3)-\frac{ar(r-1)}2+\log f(1/3)+\smallo(1).   \label{eq:Ar-asymp}
\end{align}
...
\begin{equation}
 ...
 \sinc\!\left(\frac{\pi x}{q^n}\right).                    \label{eq:fq}
\end{equation}
...
\begin{equation}
 ...
 \qquad
 G(x)=\prod_{m=2}^{\infty}
 \left(1-\frac{x^2}{m^2}\right)^{1+\vtwo(m)}.              \label{eq:G-near-one}
\end{equation}
