...
(\nolinkurl{Analysis/FabiusFunction/Lean/FabiusFunction/}, forty-seven
...
(\lean{TransferPositivity}, \lean{PerronRootExistence}).
\end{itemize}
The sentence above about the exact variance is therefore superseded:
the Clausen expansion in $L^2$ is formalized, and of the fluctuation
program only the limit theorems themselves (the martingale central
limit theorem and iterated-logarithm law) and the Hoischen
approximation import remain outside the formalized perimeter, both
blocked on machinery currently absent from Mathlib.
