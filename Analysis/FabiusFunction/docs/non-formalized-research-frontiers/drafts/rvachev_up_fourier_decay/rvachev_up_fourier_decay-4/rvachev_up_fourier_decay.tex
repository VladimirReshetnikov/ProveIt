...
\text{arithmetic mean of }\abs{\Phiup}
...
\begin{equation}
 ...
 \label{eq:g1}
\end{equation}
makes the mean of $\abs{\Phiup}/g_1$ on each dyadic shell tend to one.  This gives the
...
of $\abs{\Phiup}$, it must be replaced by $x^{-\kappa_1}$ with
...
\[
 \abs{\Phiup(x)}\sim C(\log x)^{-p}x^{-\kappa}
 \exp\!\left(-\frac{(\log x)^2}{2\log2}\right)
\]
cannot hold globally, for any $C>0,p,\kappa$, because $\Phiup$ vanishes at every integer.
It also cannot be repaired by replacing $\abs{\Phiup(x)}$ with the sequence of local maxima:
...
$d=1+\vtwo(r+1)$ and it accompanies the much faster factor $(b/N)^n$.  It is not a universal
...
\url{https://github.com/VladimirReshetnikov/ProveIt/blob/main/Analysis/FabiusFunction/docs/non-formalized-research-frontiers/drafts/Decay%20rate%20conjecture/Decay%20rate%20conjecture.tex}.
