...
\(\e^{-2\pi\ii x\xi}\) convention.  This article audits the original question,
...
arithmetic mean is replaced by the logarithmic mean \(\dd x/x\), as in
\eqref{eq:logmean}.
...
\(\sinc z=1-z^2/6+\bigO(z^4)\) at the origin.  Hence \(\Phi\) is even and
...
\begin{align}
 ...
 H(y)&:=\prod_{r=1}^{\infty}
 \left|\sinc\!\left(\frac{\pi y}{2^r}\right)\right|.
 \label{eq:H}
\end{align}
...
\[
 \prod_{h=0}^{k}
 \left|\sinc\!\left(\frac{\pi2^ky}{2^h}\right)\right|
...
\(1+\vtwo(|m|)\).  On every interval \((N,N+1)\),
\begin{equation}
 \frac{\dd^2}{\dd x^2}\log|\Phi(x)|
 =-2\sum_{m=1}^{\infty}(1+\vtwo(m))
...
\begin{equation}
 ...
 \label{eq:X-S}
\end{equation}
Since \(\int_0^1\log|\sin\pi t|\dd t=-\log2\), one has
\(\int_0^1X=0\).  At \(\kappa=\kappa_0\),
\eqref{eq:master-normalization} becomes the exact formula
\begin{equation}
 |\Phi(2^ky)|
 =\Ecal_{\kappa_0}(2^ky)D_0(y)\e^{S_{k+1}(y-1)},
 \qquad
 D_0(y)=\frac{H(y)}{2\pi}
 y^{\kappa_0-1}\e^{(\log y)^2/(2a)}.
...
\[
 \Var(S_n)=nc_0+2\sum_{r=1}^{n-1}(n-r)c_r.
\]
...
\begin{equation}
 ...
 \label{eq:d-variance}
\end{equation}
Since \(\Leb(|X|>u)=\bigO(\e^{-u})\), Borel--Cantelli implies
...
\begin{equation}
 (\Lop_qh)(x)=\frac12\sum_{\T y=x}s(y)^q h(y),
 \qquad s(y)=|\sin\pi y|.
 \label{eq:Lq}
\end{equation}
Thus
\begin{equation}
 \int_0^1P_n(t)^qh(t)\dd t
 =\int_0^1\Lop_q^nh(x)\dd x.
 \label{eq:Lq-integral}
\end{equation}
Let \(\rho_q\) be the spectral radius of \(\Lop_q\), and put
\begin{equation}
 \Lambda_q:=\rho_q^{1/q},
 \qquad
...
\begin{equation}
 \int_0^1\log P_n(t)\dd t
 =n\int_0^1\log|\sin\pi t|\dd t=-na.
...
\begin{proof}
The RPF eigenmeasure \(\nu\) for \(\Lop_1^*\) obeys, at a point \(z\),
\(
 \rho_1\nu\{z\}=\tfrac12s(z)\nu\{\T z\}\).  Since
...
part \(r\dd z\).  The dual eigen-equation gives almost everywhere
\begin{equation}
 s(z)r(\T z)=\rho_1r(z).
...
\[
 \Leb\{r\circ\T^n>\e^{\varepsilon n}\}
 =\Leb\{r>\e^{\varepsilon n}\}
 \le\norm{r}_1\e^{-\varepsilon n}.
\]
...
\(q\dd\mu\), where \(q\) is continuous and strictly positive on \([0,1)\).
Uniform linearity would therefore imply \(q\dd\mu=\dd z\).  This is impossible
...
\[
 ...
 \quad(k\ge K_j^{(0)}).
\]
Let \(r_{k,j}=\e^{\ell_{k,j}}\).  Away from \(U_j\), the weighted mass of a
...
\[
 \sup_z\left|\int_0^zr_{k,j}\dd\mu_k-z\right|
 \le 3\varepsilon_j.
\]
...
\(\norm{(\log r_k)'}_\infty=o(k)\); hence \(h\) is eventually strictly
...
\begin{equation}
 ...
 \frac{|\Phi(x)|}{g_\infty(x)}\dd x\longrightarrow1
 \qquad(t\to\infty).
...
\[
 \int_{t_0}^{t}\frac{|\Phi(x)|}{g_\infty(x)}\dd x
...
for every real-valued \(F\in C^1(\R)\) and every positive continuous
\(\epsilon\), there is an entire function \(G\), real on \(\R\), such that
\begin{equation}
 |G(x)-F(x)|<\epsilon(x),
 \qquad
 |G'(x)-F'(x)|<\epsilon(x)
...
\(C^1\) function on all of \(\R\).  Choose a continuous
...
\(\widetilde h>0\), \(\widetilde h'>0\) on \(\R\), and
...
\[
 \left|\int_X^t|\Phi|(\widetilde h-h)\dd x\right|
 \le\varepsilon\int_X^t|\Phi|h\dd x
 =\varepsilon(t+o(t))
\]
...
\(P_m(s)\le C\e^{m\beta}\).  Conversely, the \(q\)-fold preimages of
\(1/3\) and \(2/3\) form an \(O(2^{-q})\)-net.  For example, if
\(s_q=(j+1/3)/2^q\), then \(\T^qs_q=1/3\), and for \(0\le \ell<q\)
\[
 \operatorname{dist}(\T^\ell s_q,\Z)
 \ge \frac1{3\,2^{q-\ell}}.
\]
Since \(|\sin\pi u|\ge2\operatorname{dist}(u,\Z)\), the first \(q\)
...
\begin{equation}
 ...
 H(1+\theta)(1+\theta)^{-n}P_n(\theta)
 =n\beta+\sup_{r,s}\mathcal F_n(r,s)
 +\bigO((\log\log n)^2).
...
\(\max_{0\le t\le1}t^p(1-t)\sim1/(\e p)\) now gives the exact valley law.
