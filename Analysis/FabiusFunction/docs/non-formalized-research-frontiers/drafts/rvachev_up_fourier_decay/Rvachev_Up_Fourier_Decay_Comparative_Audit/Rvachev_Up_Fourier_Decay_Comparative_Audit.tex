...
The reduction the certificate rests on is itself now formal, even though
the certificate is not.  \lean{CollatzWielandt}'s
...
positive homogeneity.  What remains outside the library is the numerical
half: exhibiting the explicit test function and discharging the two
rational-polynomial positivity checks by Sturm sequences.
...
machine-readable.
\textbf{Half done.}  The reduction is formal ---
\lean{CollatzWielandt}'s \lean{perron\_root\_mem\_of\_two\_sided}
brackets the Perron root from any continuous positive test function
$h$ with $m\,h\le\Lop_1h\le M\,h$ on $[0,1]$, no eigenfunction needed,
and \lean{collatzWielandt\_bracket} proves it abstractly in the operator
and the set.  What is left is exhibiting the explicit $h$ and
discharging the two rational positivity checks; no Sturm module exists
yet (\cref{ssec:certificate}).
