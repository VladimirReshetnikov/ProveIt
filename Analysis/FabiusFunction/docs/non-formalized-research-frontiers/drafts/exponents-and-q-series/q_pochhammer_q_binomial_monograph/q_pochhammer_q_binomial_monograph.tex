...
% ---- merged block M11 ----
\begin{remark}[The shift must be common, with one systematic exception]\label{rem:dyadic-shift-common}
\index{translation invariance!of the dyadic formula}
The freedom in $\theta$ in \eqref{eq:dyadic-gaussian-thue-morse}, \eqref{eq:fabius-inverse-dyadic-closed} and \eqref{eq:fabius-centered-formula} is \Cref{cor:transfer-translation} at $q=1/2$: one exponential $\e^{\alpha z}$ is pulled out of all $n+1$ blocks \emph{simultaneously}, after which \eqref{eq:transfer-lower-zero} annihilates every convolution term but one.  It is not a symmetry of an individual block.  To see what is and is not allowed, give each block its own shift and set
...
\end{equation}
Two one-parameter families of shifts leave this constant, and they are exactly the two displays above:
\[
 \mathcal N_n(\theta,\ldots,\theta)=\mathfrak m_na_n,
 \qquad
 \mathcal N_n\bigl(2^0+\theta,\ldots,2^n+\theta\bigr)=(-1)^n\mathfrak m_na_n ,
\]
the first by \Cref{cor:fabius-centered-formula}, the second by \eqref{eq:dyadic-gaussian-thue-morse} after cancelling $-2^k$ against $2^k$.  The second family is genuinely block-dependent, and it survives only because digit complementation converts it into the first, at the cost of the $k$-independent sign $(-1)^n$ computed in the proof of \Cref{cor:fabius-centered-formula}.  Apart from these, unequal shifts destroy the cancellation: for $n=3$ and $(\theta_0,\theta_1,\theta_2,\theta_3)=(0,1,2,3)$ one finds
\[
 \mathcal N_3(0,1,2,3)=\frac1{32},
 \qquad\text{against}\qquad
 \mathfrak m_3a_3=21\cdot\frac1{36}=\frac7{12}.
\]
...
% ed.: kept; the assertion is corrected, since the shifts $\theta_k=2^k+\theta$
% ed.: do preserve the sum.
\end{remark}
