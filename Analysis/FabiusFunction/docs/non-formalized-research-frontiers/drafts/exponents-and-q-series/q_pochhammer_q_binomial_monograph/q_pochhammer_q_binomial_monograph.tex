...
\end{equation}
an entire function with $\Lambda(z)=\sum_{r\ge0}z^r/(r+1)!$, and set
\begin{equation}\label{eq:geometric-uniform-mgf-def}
 \mathcal A_q(z)\defeq\mathbb E\,\e^{zX_q}=\sum_{n\ge0}\alpha^{(q)}_nz^n,
 \qquad
 \alpha^{(q)}_n\defeq\frac{\mathbb EX_q^{\,n}}{n!}.
\end{equation}
...
Let $0<q<1$.  Then $\mathcal A_q$ is entire, $\varphi_q(t)=\mathcal A_q(it)$ for all
real $t$,
\begin{align}
 \mathcal A_q(z)&=\prod_{j=0}^{\infty}\Lambda\bigl((1-q)q^jz\bigr),
 \label{eq:mgf-product}\\
 \mathcal A_q(z)&=\Lambda\bigl((1-q)z\bigr)\,\mathcal A_q(qz),
 \label{eq:mgf-refinement}
\end{align}
the product converging locally uniformly on $\C$.  Moreover $\mathcal A_q$ is the unique
formal power series with constant term $1$ satisfying \eqref{eq:mgf-refinement}, and its
coefficients satisfy $\alpha^{(q)}_0=1$ and
...
\begin{proof}
By \Cref{thm:geometric-uniform-basic} we have $0\le X_q\le1$, so
$\mathbb EX_q^{\,n}\le1$ and $\sum_n\abs z^n\mathbb EX_q^{\,n}/n!\le\e^{\abs z}$.  Hence
$\mathcal A_q$ is entire and term-by-term integration is legitimate; substituting $z=it$
reproduces $\varphi_q$.

For the truncation $X_{q,N}$ of \eqref{eq:finite-geometric-uniform-sum}, independence
gives
\[
 \mathbb E\e^{zX_{q,N}}
...
\]
On $\abs z\le R$ the arguments satisfy $\sum_{j\ge0}(1-q)q^j\abs z\le R$ and
$\Lambda(w)=1+\OO(w)$ near $0$, so the standard criterion for infinite products gives
local uniform convergence.  By \eqref{eq:finite-uniform-tail-bound},
$0\le X_q-X_{q,N}\le q^N$ pointwise, so $\e^{zX_{q,N}}\to\e^{zX_q}$ uniformly on the
sample space while remaining bounded by $\e^{R}$; dominated convergence identifies the
limit and proves \eqref{eq:mgf-product}.

Taking expectations in \eqref{eq:geometric-uniform-fixed-point} and using independence of
$U_0$ and $X_q'$ yields
$\mathcal A_q(z)=\mathbb E\e^{z(1-q)U_0}\cdot\mathbb E\e^{zqX_q'}$, which is
\eqref{eq:mgf-refinement}.  Comparing the coefficient of $z^n$ in
\[
 \sum_{n\ge0}\alpha^{(q)}_nz^n
 =\left(\sum_{s\ge0}\frac{(1-q)^sz^s}{(s+1)!}\right)
  \left(\sum_{r\ge0}\alpha^{(q)}_rq^rz^r\right)
\]
gives $\alpha^{(q)}_n=\sum_{r=0}^{n}(1-q)^{n-r}q^r\alpha^{(q)}_r/(n-r+1)!$; the term $r=n$
on the right is $q^n\alpha^{(q)}_n$, and moving it to the left proves
...
\begin{example}[The first coefficients]\label{ex:fabius-first-coefficients}
...
\begin{remark}[The same value by cutting a cube]\label{rem:half-moment-cube-cut}
There is a second proof of \eqref{eq:fabius-half-moment} which uses neither
\Cref{prop:up-tail} nor the smoothness of $F$.  Write
$X=\sum_{j\ge1}2^{-j}V_j$ with $V_1,V_2,\ldots$ independent and uniform on $[0,1]$,
multiply by $2^n$ and split the series after $n$ terms:
...
$X'=x$, the event $2^nX\le1$ reads $\sum_jc_jV_j\le t$ with $t\defeq1-x\in[0,1]$.  Each
$c_j$ is an integer with $c_j\ge1$, so any $u\in[0,\infty)^n$ with $\sum_jc_ju_j\le t$
automatically satisfies $u_j\le c_ju_j\le t\le1$: no upper constraint is active, the
admissible region is the entire simplex, and its volume is
$t^n/\bigl(n!\prod_jc_j\bigr)=(1-x)^n/\bigl(n!\,2^{\binom n2}\bigr)$, since
$\prod_{j=1}^{n}2^{\,n-j}=2^{\binom n2}$.  Averaging over $X'$ and using
$X\overset d=1-X$ from \eqref{eq:geometric-uniform-mean-symmetry} turns
$\mathbb E[(1-X')^n]$ into $\mathbb E(X^n)$ and returns \eqref{eq:fabius-half-moment}.
\end{remark}
...
\[
 A(z)=\sum_{m\ge0}a_mz^m\in K[[z]],
 \qquad a_0=1 .
\]
For each $0\le k\le n$ there is exactly one series $\Psi_k(z)=\sum_{m\ge0}c_{k,m}z^m\in K[[z]]$ satisfying the bridge relation
\begin{equation}\label{eq:general-bridge}
 \Psi_k(z)A(-z)=\e^{-z}A\bigl(-q^{-k}z\bigr),
\end{equation}
namely $\Psi_k(z)=\e^{-z}A(-q^{-k}z)/A(-z)$, the quotient being legitimate in $K[[z]]$ because $A(-z)$ has constant term $a_0=1$.  For that family, with the weights of \eqref{eq:reciprocal-nodes-weights},
\begin{align}
 \sum_{k=0}^{n}W_{n,k}(q)\,c_{k,d}&=0
 \qquad(0\le d<n),\label{eq:transfer-lower-zero}\\
 \sum_{k=0}^{n}W_{n,k}(q)\,c_{k,n}
 &=q^{-\binom{n+1}{2}}(q;q)_n\,a_n.\label{eq:transfer-top}
\end{align}
\end{theorem}
...
\begin{equation}\label{eq:bridge-coeff}
 \sum_{i=0}^{d}(-1)^{d-i}a_{d-i}\,c_{k,i}
 =\sum_{j=0}^{d}\frac{(-1)^j}{j!}\,a_{d-j}\bigl(-q^{-k}\bigr)^{d-j}.
\end{equation}
...
\begin{equation}\label{eq:bridge-coeff-summed}
 \sum_{j=0}^{d}\frac{(-1)^j}{j!}\,a_{d-j}
 \sum_{k=0}^{n}W_{n,k}(q)\,x_k^{\,d-j},
\end{equation}
and the left-hand side becomes $\sum_{i=0}^{d}(-1)^{d-i}a_{d-i}S_i$.

Let $0\le d<n$ and induct on $d$.  Every exponent occurring in \eqref{eq:bridge-coeff-summed} satisfies $0\le d-j\le d<n$, so the entire right-hand side vanishes by \eqref{eq:geometric-moment-zero}.  On the left, the terms with $i<d$ vanish by the induction hypothesis (for $d=0$ there are none), and the term $i=d$ equals $a_0S_d=S_d$.  Hence $S_d=0$, which is \eqref{eq:transfer-lower-zero}.
...
\[
 a_n\sum_{k=0}^{n}W_{n,k}(q)\,x_k^{\,n}
 =q^{-\binom{n+1}{2}}(q;q)_n\,a_n
\]
by \eqref{eq:geometric-moment-top}.  On the left, the terms with $i<n$ vanish by \eqref{eq:transfer-lower-zero} and the term $i=n$ equals $a_0S_n=S_n$.  This proves \eqref{eq:transfer-top}.
\end{proof}

\begin{remark}[On the normalization $a_0=1$]\label{rem:transfer-normalization}
The proof uses $a_0=1$ three times: once for the invertibility of $A(-z)$ and once at the last step of each induction stage.  Only invertibility of $a_0$ is actually needed: for arbitrary $a_0\ne0$ the same argument gives $S_d=0$ for $d<n$ and $S_n=q^{-\binom{n+1}{2}}(q;q)_na_n/a_0$.  This is not a restriction, because \eqref{eq:general-bridge} is unchanged when $A$ is multiplied by a nonzero constant.
\end{remark}

\begin{remark}[The dyadic reading]\label{rem:transfer-dyadic-reading}
At $q=1/2$ the data \eqref{eq:reciprocal-nodes-weights} are the half-base Prouhet data \eqref{eq:halfbase-prouhet-data}: the nodes are $x_k=-2^{k}=\xi_k$, the weights are
\[
 W_{n,k}(1/2)=(-1)^k2^{-\binom k2}\Qbinom nk{1/2}=w_{n,k},
\]
and the constant on the right of \eqref{eq:transfer-top} is $2^{\binom{n+1}{2}}\left(\tfrac12;\tfrac12\right)_n=\mathfrak m_n$ by \eqref{eq:halfbase-factorial}.  In that reading the theorem says: a family $\Psi_0,\ldots,\Psi_n$ tied to $A$ by $\Psi_k(z)A(-z)=\e^{-z}A(-2^kz)$ has $\sum_kw_{n,k}c_{k,d}=0$ for $d<n$ and $\sum_kw_{n,k}c_{k,n}=\mathfrak m_na_n$.  This is the form used throughout \Cref{chap:geometric-uniform-fabius}.
% ed.: an incoming draft carried exactly this dyadic reading as a separate
% ed.: theorem, together with the existence-and-uniqueness clause now in the
% ed.: statement above; both are recorded here rather than duplicated.
\end{remark}
...
\begin{corollary}[Centred form]\label{cor:fabius-centered-formula}
...
\begin{equation}\label{eq:dyadic-numerator}
 ...
 \qquad \tau_r=(-1)^{s_2(r)} .
\end{equation}
Then $\mathcal N_n(\theta)=\mathfrak m_n a_n$ for every $\theta$, and
\begin{equation}\label{eq:fabius-centered-formula}
 ...
 \sum_{r=0}^{2^k-1}\tau_r\,\bigl(r-2^k+\theta\bigr)^{\,n+k},
\end{equation}
with no sign $(-1)^n$ in front.
\end{corollary}

\begin{proof}
Fix $\theta$ and substitute $r\mapsto2^k-1-r$ in the inner sum of $\mathcal N_n(1-\theta)$.  Complementing all $k$ binary digits gives $s_2(2^k-1-r)=k-s_2(r)$, hence $\tau_{2^k-1-r}=(-1)^k\tau_r$, while
\[
 (2^k-1-r)-2^k+(1-\theta)=-(r+\theta).
\]
Therefore
\[
 \sum_{r=0}^{2^k-1}\tau_r\bigl(r-2^k+1-\theta\bigr)^{\,n+k}
 =(-1)^k\sum_{r=0}^{2^k-1}\tau_r\bigl(-(r+\theta)\bigr)^{\,n+k}
 =(-1)^{n+2k}\sum_{r=0}^{2^k-1}\tau_r(r+\theta)^{\,n+k},
\]
and $(-1)^{n+2k}=(-1)^n$ does not depend on $k$, so it factors out of the sum over $k$:
\[
 \mathcal N_n(1-\theta)
 =(-1)^n\sum_{k=0}^{n}
 \frac{\Qbinom nk{1/2}}{4^{\binom k2}(n+k)!}
 \sum_{r=0}^{2^k-1}\tau_r(r+\theta)^{\,n+k}.
\]
By \eqref{eq:dyadic-gaussian-thue-morse} the sum on the right is $(-1)^n\mathfrak m_na_n$, so $\mathcal N_n(1-\theta)=\mathfrak m_na_n$; as $\theta$ runs over $\C$ so does $1-\theta$.  Now \eqref{eq:fabius-half-moment} gives $F(2^{-n})=2^{-\binom n2}a_n=\mathcal N_n(\theta)/(2^{\binom n2}\mathfrak m_n)$, and the second expression in \eqref{eq:fabius-centered-formula} follows from $2^{\binom n2}\mathfrak m_n=2^{\,n^2}\left(\tfrac12;\tfrac12\right)_n$, as in the proof above.
\end{proof}
...
\begin{theorem}[Row mass and total variation]\label{thm:geometric-row-variation}
Let $0<q<1$.  Then, for every $n\ge0$,
\begin{align}
 \sum_{k=0}^{n}\lambda_{n,k}^{(q)}&=1,
 \label{eq:geometric-row-mass}\\
 \sum_{k=0}^{n}\bigl|\lambda_{n,k}^{(q)}\bigr|
 &=\frac{(-q;q)_n}{(q;q)_n}
 \ \le\ \frac{(-q;q)_\infty}{(q;q)_\infty}<\infty .
 \label{eq:geometric-row-variation}
\end{align}
At $q=1/2$ the supremum equals $(-1/2;1/2)_\infty/(1/2;1/2)_\infty
=8.25598793577825\ldots$, and the elementary bound
...
\eqref{eq:geometric-row-mass}.  For $0<q<1$ the sign of $\lambda_{n,k}^{(q)}$ is
$(-1)^{n-k}$, so $\bigl|\lambda_{n,k}^{(q)}\bigr|=(-1)^{n-k}\lambda_{n,k}^{(q)}$ and
\[
 \sum_{k=0}^{n}\bigl|\lambda_{n,k}^{(q)}\bigr|
 =(-1)^n\sum_{k=0}^{n}\lambda_{n,k}^{(q)}(-1)^k
 =\frac{(-1)^n}{(q;q)_n}\prod_{j=1}^{n}(-1-q^j)
 =\frac{(-q;q)_n}{(q;q)_n},
\]
by \eqref{eq:geometric-row-polynomial} at $z=-1$.  The quotient increases in $n$ and
converges to $(-q;q)_\infty/(q;q)_\infty$, finite because both products converge for
$0<q<1$.

At $q=1/2$ the factor with $r=1$ contributes $(1+1/2)/(1-1/2)=3$.  For $0\le x\le1/4$,
\[
 \log\frac{1+x}{1-x}=\int_{-x}^{x}\frac{\dd t}{1+t}\le\frac{2x}{1-x}\le\frac83x,
\]
so the logarithm of the remaining product is at most
$\tfrac83\sum_{r\ge2}2^{-r}=4/3$.
\end{proof}
