...
A final part develops three further connections motivated by the Fabius/Rvachev documentation in the ProveIt repository: a denominator-free definition of Gaussian coefficients over arbitrary commutative semirings and rings; exact divided-difference and Richardson filters on geometric grids, including closed residual moments and stability norms; and a complete half-base derivation of inverse-dyadic Fabius values from Thue--Morse cancellation and the finite $q$-binomial theorem.
...
The live \texttt{docs} tree of the ProveIt Fabius project was also surveyed, including its mathematical glossary, primary Fabius/Rvachev exposition, Lean walkthrough, non-elementarity article, canonical non-formalized frontier volume, and archive catalog.  The material most directly relevant here is reorganized and reproved in \Cref{chap:universal-gaussian,chap:qfilters,chap:fabius-probability,chap:fabius-qformula}; exploratory claims not needed for those proofs are not imported.
...
\part{Universal specializations, geometric filters, and the dyadic bridge}

\chapter{Denominator-free Gaussian algebra over rings}\label{chap:universal-gaussian}
\index{Gaussian coefficient!over a ring}
\index{specialization!denominator-free}
\index{semiring}

The factorial quotient
\[
  \qbinom nk=\frac{(q;q)_n}{(q;q)_k(q;q)_{n-k}}
\]
is ideal when $q$ is an indeterminate or a complex number away from the zeros of the displayed denominator.  It is not, however, a satisfactory \emph{definition} at a root of unity, in a ring with zero divisors, or in a proof assistant whose algebraic layer should not carry avoidable invertibility hypotheses.  The correct universal object is the polynomial determined by Pascal recursion.  This point is elementary, but it has far-reaching consequences: every identity proved without division can be transported to finite characteristic, quotient rings, truncated power-series rings, and roots of unity without first proving a cancellation theorem.

\section{The universal Pascal construction}

\begin{definition}[Universal Gaussian polynomials]\label{def:universal-gaussian}
Let $u$ be an indeterminate.  Define $G_{n,k}(u)\in\N[u]$ by
\[
 G_{0,0}(u)=1,
 \qquad
 G_{n,k}(u)=0\quad(k<0\text{ or }k>n),
\]
and
\begin{equation}
 G_{n+1,k}(u)=G_{n,k}(u)+u^{n+1-k}G_{n,k-1}(u).\label{eq:universal-pascal}
\end{equation}
For an element $a$ of a commutative semiring $R$, write
\[
 G^{R}_{n,k}(a):=G_{n,k}(a)\in R.
\]
When no confusion is possible, the superscript $R$ will be suppressed and the usual Gaussian notation $\Qbinom nk a$ will be used.
\end{definition}

\begin{theorem}[Existence, uniqueness, and functoriality]\label{thm:universal-functoriality}
The family in \Cref{def:universal-gaussian} is the unique family of polynomials satisfying the boundary conditions and \eqref{eq:universal-pascal}.  If $\phi:R\to S$ is a homomorphism of commutative semirings, then
\[
 \phi\!\left(G^R_{n,k}(a)\right)=G^S_{n,k}(\phi(a)).
\]
In particular, all coefficients of $G_{n,k}(u)$ are nonnegative integers, and specialization requires no division.
\end{theorem}

\begin{proof}
Starting with row $n=0$, recurrence \eqref{eq:universal-pascal} determines row $n+1$ from row $n$, so existence and uniqueness follow by induction.  The recurrence uses only $0$, $1$, addition, multiplication, and powers of $u$; hence it produces a polynomial in $\N[u]$.  Apply $\phi$ to the recurrence.  Because a semiring homomorphism preserves every operation appearing there, the image family satisfies the same boundary conditions and recurrence in $S$.  Uniqueness in $S$ gives the asserted identity.
\end{proof}

\begin{proposition}[Symmetry and the second Pascal recurrence]\label{prop:universal-symmetry}
For all $0\le k\le n$,
\begin{equation}
 G_{n,k}(u)=G_{n,n-k}(u).\label{eq:universal-symmetry}
\end{equation}
Consequently,
\begin{equation}
 G_{n+1,k}(u)=u^kG_{n,k}(u)+G_{n,k-1}(u).\label{eq:universal-second-pascal}
\end{equation}
Both identities remain valid after specialization to every commutative semiring.
\end{proposition}

\begin{proof}
As proved combinatorially in \Cref{chap:combinatorics}, $G_{n,k}(u)$ is the generating polynomial for partitions whose Ferrers diagrams fit in a $k\times(n-k)$ rectangle, with exponent equal to the number of cells.  Transposition of the diagram is a weight-preserving bijection to partitions in an $(n-k)\times k$ rectangle, proving \eqref{eq:universal-symmetry} in $\N[u]$.  Replacing $k$ by $n+1-k$ in \eqref{eq:universal-pascal} and then using symmetry gives
\[
 G_{n+1,k}=G_{n,k-1}+u^kG_{n,k},
\]
which is \eqref{eq:universal-second-pascal}.  The specialization statement follows from \Cref{thm:universal-functoriality}.
\end{proof}

\begin{remark}
The proof separates two logically different facts.  Pascal recursion is meaningful in a semiring, where subtraction need not exist.  Product expansions containing $(-1)^k$ require a ring.  This distinction is important in formal developments of $q$-series, where the algebraic layer should be stated at the weakest useful level; compare the formalization architecture in \cite{LauLeeOno2026}.
\end{remark}

\section{Finite products without quotients}

\begin{theorem}[Universal finite $q$-binomial theorem]\label{thm:universal-finite-qbinomial}
Let $R$ be a commutative ring and let $a,z\in R$.  Then, for every $n\ge0$,
\begin{align}
 \prod_{j=0}^{n-1}(1+za^j)
 &=\sum_{k=0}^n a^{\binom{k}{2}}\Qbinom nk a z^k,\label{eq:universal-positive-finite}\\
 \prod_{j=0}^{n-1}(1-za^j)
 &=\sum_{k=0}^n(-1)^ka^{\binom{k}{2}}\Qbinom nk a z^k.\label{eq:universal-negative-finite}
\end{align}
No factor $1-a^j$ is assumed invertible.
\end{theorem}

\begin{proof}
We prove \eqref{eq:universal-negative-finite}; replacing $z$ by $-z$ gives \eqref{eq:universal-positive-finite}.  The assertion is clear for $n=0$.  Assume it for $n$ and multiply by $1-za^n$.  The coefficient of $z^k$ in the resulting product is
\[
 (-1)^ka^{\binom{k}{2}}\Qbinom nk a
 +(-1)^ka^{n+\binom{k-1}{2}}\Qbinom n{k-1}a.
\]
Since
\[
 n+\binom{k-1}{2}=\binom{k}{2}+n+1-k,
\]
the expression equals
\[
 (-1)^ka^{\binom{k}{2}}
 \left(\Qbinom nk a+a^{n+1-k}\Qbinom n{k-1}a\right)
 =(-1)^ka^{\binom{k}{2}}\Qbinom{n+1}k a
\]
by \eqref{eq:universal-pascal}.  This proves the induction step.
\end{proof}

\begin{theorem}[Denominator-free factorial identities]\label{thm:universal-factorial-identities}
In every commutative ring and for $0\le k\le n$,
\begin{align}
 (a;a)_k\Qbinom nk a&=(a^{n-k+1};a)_k,\label{eq:universal-short-factorial}\\
 (a;a)_n&=(a;a)_k(a;a)_{n-k}\Qbinom nk a.\label{eq:universal-full-factorial}
\end{align}
Thus the familiar factorial quotient is a consequence of a polynomial identity, not a prerequisite for defining the Gaussian coefficient.
\end{theorem}

\begin{proof}
Work first in the fraction field $\Q(u)$ of $\Z[u]$.  The usual product quotient
\[
 H_{n,k}(u)=\frac{(u;u)_n}{{(u;u)_k(u;u)_{n-k}}}
\]
satisfies the boundary conditions and the Pascal recurrence \eqref{eq:universal-pascal}; this follows by putting the two terms over the common denominator and using
\[
 (1-u^{n+1-k})+u^{n+1-k}(1-u^k)=1-u^{n+1}.
\]
By uniqueness, $H_{n,k}(u)=G_{n,k}(u)$ in $\Q(u)$.  Multiplying through by the nonzero polynomial denominators gives \eqref{eq:universal-full-factorial} in $\Z[u]$.  Cancelling the common factor $(u;u)_{n-k}$ in the integral domain $\Z[u]$ gives \eqref{eq:universal-short-factorial}.  Finally, the unique ring homomorphism $\Z[u]\to R$ with $u\mapsto a$ transports both polynomial identities to $R$.
\end{proof}

\begin{example}[A root of unity]
Let $a$ be a primitive $d$th root of unity.  The quotient defining $\Qbinom nk a$ may display $0/0$, but the polynomial $G_{n,k}(a)$ is unambiguous.  Equations \eqref{eq:universal-short-factorial}--\eqref{eq:universal-full-factorial} remain true, including in cases where every displayed product on the right vanishes.  This is the algebraic reason that the $q$-Lucas and cyclic-sieving specializations of \Cref{chap:cyclotomic} are safe.
\end{example}

\section{The reversed Gaussian row as a spectral annihilator}

\begin{definition}
For $0\le k\le n$, define the reversed Gaussian coefficient
\begin{equation}
 N_{n,k}(u)=(-1)^{n-k}u^{\binom{n-k+1}{2}}G_{n,k}(u).\label{eq:reversed-gaussian-def}
\end{equation}
\end{definition}

\begin{theorem}[Characteristic polynomial of a geometric row]\label{thm:reversed-gaussian-polynomial}
For every $n\ge0$,
\begin{equation}
 \sum_{k=0}^nN_{n,k}(u)z^k
 =\prod_{j=1}^n(z-u^j).\label{eq:reversed-gaussian-polynomial}
\end{equation}
Consequently, for every integer $d\ge0$,
\begin{equation}
 \sum_{k=0}^nN_{n,k}(u)u^{kd}
 =\prod_{j=1}^n(u^d-u^j).\label{eq:reversed-gaussian-moments}
\end{equation}
In particular, with the convention $0^0=1$,
\begin{equation}
 \sum_{k=0}^nN_{n,k}(u)u^{kd}=(u;u)_n0^d,
 \qquad 0\le d\le n.\label{eq:reversed-low-moments}
\end{equation}
\end{theorem}

\begin{proof}
By \eqref{eq:universal-negative-finite},
\[
 \prod_{j=1}^n(z-u^j)
 =z^n\prod_{j=1}^n\left(1-\frac{u^j}{z}\right)
 =\sum_{r=0}^n(-1)^ru^{\binom{r+1}{2}}G_{n,r}(u)z^{n-r}.
\]
Set $k=n-r$ and use symmetry $G_{n,n-k}=G_{n,k}$.  The coefficient becomes exactly \eqref{eq:reversed-gaussian-def}, proving \eqref{eq:reversed-gaussian-polynomial}.  Substitution $z=u^d$ gives \eqref{eq:reversed-gaussian-moments}.  If $1\le d\le n$, one factor on the right is $u^d-u^d=0$; if $d=0$, the product is $\prod_{j=1}^n(1-u^j)=(u;u)_n$.
\end{proof}

\begin{corollary}[All residual moments]\label{cor:reversed-residual-moments}
For $d\ge n+1$,
\begin{equation}
 \sum_{k=0}^nN_{n,k}(u)u^{kd}
 =(-1)^nu^{\binom{n+1}{2}}(u;u)_n
   \Qbinom{d-1}n u.\label{eq:reversed-residual-moments}
\end{equation}
\end{corollary}

\begin{proof}
Factor $u^j$ from the $j$th factor in \eqref{eq:reversed-gaussian-moments}:
\[
 \prod_{j=1}^n(u^d-u^j)
 =u^{\binom{n+1}{2}}\prod_{j=1}^n(u^{d-j}-1)
 =(-1)^nu^{\binom{n+1}{2}}(u^{d-n};u)_n.
\]
Apply \eqref{eq:universal-short-factorial} with upper index $d-1$ and lower index $n$.
\end{proof}

\begin{remark}
Equations \eqref{eq:reversed-low-moments} and \eqref{eq:reversed-residual-moments} say that one Gaussian row is a finite spectral filter on the geometric modes $1,u^k,u^{2k},\ldots$.  The first $n$ nonconstant modes are annihilated, while every later mode has an explicit residual.  This interpretation is the bridge from Gaussian polynomials to extrapolation and, at $u=\tfrac12$, to the Fabius function.
\end{remark}

\section{M\"obius inversion on subspace lattices}

The same coefficients arise from incidence algebra.  This is not merely another interpretation: it explains why the inverse of a Gaussian summation matrix has the sign and quadratic exponent occurring in \eqref{eq:reversed-gaussian-def}.

\begin{theorem}[M\"obius function of the subspace lattice]\label{thm:subspace-mobius}
Let $V$ be a $d$-dimensional vector space over $\F_Q$, where $Q$ is a prime power.  In the lattice of subspaces of $V$,
\begin{equation}
 \mu(0,V)=(-1)^dQ^{\binom d2}.\label{eq:subspace-mobius}
\end{equation}
More generally, if $U\subseteq W$, then
\[
 \mu(U,W)=(-1)^rQ^{\binom r2},\qquad r=\dim(W/U).
\]
\end{theorem}

\begin{proof}
The interval $[U,W]$ is isomorphic to the subspace lattice of $W/U$, so it suffices to take $U=0$.  We argue by induction on $d$.  The assertion is clear for $d=0$.  For $d>0$, the defining relation for the M\"obius function gives
\[
 \mu(0,V)=-\sum_{X<V}\mu(0,X)
 =-\sum_{j=0}^{d-1}\Qbinom djQ(-1)^jQ^{\binom j2},
\]
because there are $\Qbinom djQ$ subspaces of dimension $j$.  The finite $q$-binomial theorem at $z=1$ gives
\[
 \sum_{j=0}^{d}\Qbinom djQ(-1)^jQ^{\binom j2}
 =(1;Q)_d=0.
\]
Therefore the negative of the sum through $d-1$ is the missing $j=d$ term, namely $(-1)^dQ^{\binom d2}$.
\end{proof}

\begin{corollary}[$q$-binomial inversion revisited]\label{cor:qbinomial-inversion-mobius}
If
\[
 g_n=\sum_{k=0}^n\qbinom nk f_k,
\]
then
\begin{equation}
 f_n=\sum_{k=0}^n(-1)^{n-k}q^{\binom{n-k}{2}}\qbinom nk g_k.\label{eq:qbinomial-inversion-mobius}
\end{equation}
Whenever $q=Q$ is a prime power, this is M\"obius inversion on the subspace lattice; as a polynomial identity, it holds for an indeterminate $q$ and hence under every safe specialization.
\end{corollary}

\begin{proof}
For $q=Q$, define a function on subspaces that depends only on dimension.  Summation over all subspaces becomes the first displayed transform, and M\"obius inversion with \eqref{eq:subspace-mobius} gives \eqref{eq:qbinomial-inversion-mobius}.  Both sides are polynomials in $q$ with integer coefficients.  Since they agree for infinitely many prime powers, they agree identically in $\Z[q]$ and therefore under every specialization.  Alternatively, a direct proof follows by substituting the first transform into the second, using
\[
 \qbinom nk\qbinom kj=\qbinom nj\Qbinom{n-j}{k-j}q,
\]
and applying the finite $q$-binomial theorem at $z=1$ to the inner sum.
\end{proof}


\chapter{Geometric divided differences and exact \texorpdfstring{$q$}{q}-filters}\label{chap:qfilters}
\index{divided difference!geometric grid}
\index{Richardson extrapolation!$q$-geometric}
\index{Lagrange interpolation!evaluation at zero}

A Gaussian row can be read as an interpolation stencil.  The resulting formulas give exact annihilation of low-order geometric errors and explicit expressions for every residual term.  This chapter develops that statement in full generality before specializing to $q=\tfrac12$.

\section{All monomial moments of a divided difference}

Let $x_0,\ldots,x_n$ be pairwise distinct complex numbers and put
\[
 \Delta_k=\prod_{\substack{0\le j\le n\\j\ne k}}(x_k-x_j).
\]
For a function specified on the nodes, define
\[
 [x_0,\ldots,x_n]f=\sum_{k=0}^n\frac{f(x_k)}{\Delta_k}.
\]

\begin{theorem}[Leading-coefficient extraction]\label{thm:leading-coefficient-divdiff}
If $P$ is a polynomial of degree at most $n$, then
\begin{equation}
 [x_0,\ldots,x_n]P=[t^n]P(t).\label{eq:leading-coefficient-divdiff}
\end{equation}
\end{theorem}

\begin{proof}
The Lagrange interpolant of $P$ is
\[
 P(t)=\sum_{k=0}^nP(x_k)
 \prod_{j\ne k}\frac{t-x_j}{x_k-x_j}.
\]
The coefficient of $t^n$ in the $k$th basis polynomial is $1/\Delta_k$.  Taking leading coefficients gives \eqref{eq:leading-coefficient-divdiff}.
\end{proof}

Let $h_r(x_0,\ldots,x_n)$ denote the complete homogeneous symmetric polynomial, determined by
\begin{equation}
 \sum_{r\ge0}h_r(x_0,\ldots,x_n)s^r
 =\prod_{j=0}^n\frac1{1-sx_j}.\label{eq:complete-homogeneous-gen}
\end{equation}
We set $h_0=1$.

\begin{theorem}[Complete residual moment formula]\label{thm:complete-residual-moments}
For every $m\ge0$,
\begin{equation}
 [x_0,\ldots,x_n]t^m=
 \begin{cases}
 0,&0\le m<n,\\
 h_{m-n}(x_0,\ldots,x_n),&m\ge n.
 \end{cases}\label{eq:complete-residual-moments}
\end{equation}
Equivalently,
\begin{equation}
 \sum_{k=0}^n\frac{x_k^{n+r}}{\Delta_k}
 =h_r(x_0,\ldots,x_n),\qquad r\ge0.\label{eq:complete-residual-moments-r}
\end{equation}
\end{theorem}

\begin{proof}
We first prove the formal generating identity
\begin{equation}
 [x_0,\ldots,x_n]\frac1{1-st}
 =\frac{s^n}{\prod_{j=0}^n(1-sx_j)}.\label{eq:divdiff-resolvent}
\end{equation}
For $n=0$ this is immediate.  Assume it for $n-1$.  The recursive definition of divided differences gives
\[
 [x_0,\ldots,x_n]f
 =\frac{[x_1,\ldots,x_n]f-[x_0,\ldots,x_{n-1}]f}{x_n-x_0}.
\]
For $f(t)=(1-st)^{-1}$, the induction hypothesis turns the numerator into
\[
 s^{n-1}\left(
 \frac1{\prod_{j=1}^n(1-sx_j)}
 -\frac1{\prod_{j=0}^{n-1}(1-sx_j)}
 \right).
\]
After taking a common denominator, its numerator is
$ s^{n-1}\bigl((1-sx_0)-(1-sx_n)\bigr)
 =s^n(x_n-x_0)$, which proves \eqref{eq:divdiff-resolvent}.  Expand the left side as
\[
 \sum_{m\ge0}s^m[x_0,\ldots,x_n]t^m
\]
and the right side by \eqref{eq:complete-homogeneous-gen}.  Equality of coefficients proves \eqref{eq:complete-residual-moments}.
\end{proof}

\section{Evaluation at zero and the first omitted term}

Define the Lagrange weights for evaluation at zero by
\begin{equation}
 \lambda_k=\prod_{j\ne k}\frac{-x_j}{x_k-x_j}.\label{eq:zero-lagrange-weights}
\end{equation}
Then $P(0)=\sum_k\lambda_kP(x_k)$ for every polynomial of degree at most $n$.

\begin{theorem}[Exact residuals for the zero-evaluation filter]\label{thm:zero-filter-residuals}
Assume none of the $x_k$ is zero and put $e_{n+1}=x_0x_1\cdots x_n$.  For every integer $d\ge0$,
\begin{equation}
 \sum_{k=0}^n\lambda_kx_k^d=
 \begin{cases}
 1,&d=0,\\
 0,&1\le d\le n,\\
 (-1)^ne_{n+1}h_{d-n-1}(x_0,\ldots,x_n),&d\ge n+1.
 \end{cases}\label{eq:zero-filter-residuals}
\end{equation}
\end{theorem}

\begin{proof}
The cases $0\le d\le n$ are exactly the polynomial reproduction property.  For $d\ge1$, rewrite \eqref{eq:zero-lagrange-weights} as
\[
 \lambda_k=(-1)^n\frac{e_{n+1}}{x_k\Delta_k}.
\]
Therefore
\[
 \sum_{k=0}^n\lambda_kx_k^d
 =(-1)^ne_{n+1}\sum_{k=0}^n\frac{x_k^{d-1}}{\Delta_k}.
\]
Apply \Cref{thm:complete-residual-moments}.  When $1\le d\le n$, the divided difference vanishes; when $d\ge n+1$, it equals $h_{d-n-1}$.
\end{proof}

\begin{corollary}[First omitted monomial]\label{cor:first-omitted-monomial}
For $d=n+1$,
\begin{equation}
 \sum_{k=0}^n\lambda_kx_k^{n+1}=(-1)^nx_0x_1\cdots x_n.\label{eq:first-omitted-monomial}
\end{equation}
Equivalently, the interpolation error at zero for $P(t)=t^{n+1}$ is the constant term of the node polynomial $\prod_{k=0}^n(t-x_k)$.
\end{corollary}

\begin{proof}
Set $d=n+1$ in \eqref{eq:zero-filter-residuals} and use $h_0=1$.
\end{proof}

\section{Closed forms on a geometric grid}
...
\[
 ...
 \qquad 0\le k\le n,
\]
where $c\ne0$ and $1,q,\ldots,q^n$ are distinct.  The scale $c$ cancels from the weights.

\begin{theorem}[Gaussian Lagrange weights]\label{thm:gaussian-lagrange-weights}
The weights for evaluation at zero on the nodes $c,cq,\ldots,cq^n$ are
\begin{equation}
 \lambda_{n,k}^{(q)}
 =\frac{(-1)^{n-k}q^{\binom{n-k+1}{2}}}{(q;q)_n}
   \qbinom nk
 =\frac{(-1)^{n-k}q^{\binom{n-k+1}{2}}}
 {(q;q)_k(q;q)_{n-k}}.\label{eq:gaussian-lagrange-weights}
\end{equation}
For every $d\ge1$,
\begin{equation}
 \sum_{k=0}^n\lambda_{n,k}^{(q)}(cq^k)^d
 =(-1)^nc^dq^{\binom{n+1}{2}}
   \Qbinom{d-1}n q,\label{eq:geometric-filter-residual}
\end{equation}
where the Gaussian coefficient is understood to be zero when $d-1<n$.
\end{theorem}

\begin{proof}
Formula \eqref{eq:gaussian-lagrange-weights} is the normalized reversed row of \Cref{thm:reversed-gaussian-polynomial}:
\[
 \lambda_{n,k}^{(q)}=\frac{N_{n,k}(q)}{(q;q)_n}.
\]
Indeed, \eqref{eq:reversed-low-moments} shows that these weights reproduce the constant mode and annihilate $q^{kd}$ for $1\le d\le n$, which uniquely characterizes evaluation at zero on the $n+1$ distinct nodes.  The second expression follows from the factorial identity \eqref{eq:universal-full-factorial}.

For the residual, \Cref{thm:zero-filter-residuals} gives
\[
 (-1)^n(c^{n+1}q^{\binom{n+1}{2}})
 h_{d-n-1}(c,cq,\ldots,cq^n).
\]
The reciprocal finite $q$-binomial theorem yields
\[
 \prod_{k=0}^n\frac1{1-cq^kt}
 =\sum_{r\ge0}c^r\Qbinom{n+r}nq t^r,
\]
so
\[
 h_r(c,cq,\ldots,cq^n)=c^r\Qbinom{n+r}nq.
\]
Put $r=d-n-1$ to obtain \eqref{eq:geometric-filter-residual}.  For $1\le d\le n$, both sides vanish by the zero-extension convention.
\end{proof}

\begin{example}[The first three filters]
For nodes $1,q,q^2,\ldots$, the first filters are
\begin{align*}
 n=0:&\qquad (1),\\
 n=1:&\qquad \frac1{1-q}(-q,1),\\
 n=2:&\qquad \frac1{(1-q)(1-q^2)}
 \bigl(q^3,-q(1+q),1\bigr).
\end{align*}
Their dot product with $(1,1,\ldots)$ is $1$, while their dot product with $(1,q^d,\ldots,q^{nd})$ vanishes for $1\le d\le n$.
\end{example}

\begin{theorem}[Exact $q$-Richardson extrapolation]\label{thm:q-richardson}
Let $0<|q|<1$, and suppose a sequence has an absolutely convergent geometric expansion
\begin{equation}
 s_j=L+\sum_{r=1}^{\infty}a_rq^{rj}\label{eq:geometric-error-expansion}
\end{equation}
for $j\ge\ell$.  Define
\[
 \mathcal R_{n,\ell}[s]
 =\sum_{k=0}^n\lambda_{n,k}^{(q)}s_{\ell+k}.
\]
Then
\begin{equation}
 \mathcal R_{n,\ell}[s]-L
 =(-1)^nq^{\binom{n+1}{2}}
 \sum_{r=n+1}^{\infty}a_rq^{r\ell}\Qbinom{r-1}nq.\label{eq:q-richardson-residual}
\end{equation}
Thus the first $n$ geometric error modes are removed exactly.
\end{theorem}

\begin{proof}
Insert \eqref{eq:geometric-error-expansion} into the definition of $\mathcal R_{n,\ell}$.  Absolute convergence and the finiteness of the outer sum permit interchange of sums.  The constant term contributes $L\sum_k\lambda_{n,k}^{(q)}=L$.  For the $r$th mode, apply \eqref{eq:geometric-filter-residual} with $c=q^\ell$.  The modes $1\le r\le n$ vanish, and the remaining modes give \eqref{eq:q-richardson-residual}.
\end{proof}

\begin{corollary}[A quantitative tail bound]\label{cor:q-richardson-bound}
Assume $0<q<1$ and $|a_r|\le AR^{-r}$ for $r\ge1$, where $q^\ell<R$.  Then
\begin{equation}
 \left|\mathcal R_{n,\ell}[s]-L\right|
 \le
 \frac{Aq^{\binom{n+1}{2}}}{(q;q)_n}
 \frac{(q^\ell/R)^{n+1}}{1-q^\ell/R}.\label{eq:q-richardson-bound}
\end{equation}
\end{corollary}
...
\[
 0<\Qbinom{r-1}nq
 =\prod_{j=1}^n\frac{1-q^{r-j}}{1-q^j}
 \le\frac1{(q;q)_n}.
\]
Apply this estimate termwise in \eqref{eq:q-richardson-residual} and sum the resulting geometric series.
\end{proof}

\section{Stability and exact total variation}

Exact cancellation can be numerically useless if the weights are enormous.  On a fixed geometric grid with $0<q<1$, the total variation has an unexpectedly clean closed form.

\begin{theorem}[Norm of the geometric filter]\label{thm:q-filter-norm}
For $0<q<1$,
\begin{equation}
 \sum_{k=0}^n\left|\lambda_{n,k}^{(q)}\right|
 =\frac{(-q;q)_n}{(q;q)_n}.\label{eq:q-filter-norm}
\end{equation}
Consequently,
\[
 \sup_{n\ge0}\sum_{k=0}^n|\lambda_{n,k}^{(q)}|
 =\frac{(-q;q)_\infty}{(q;q)_\infty}<\infty.
\]
\end{theorem}

\begin{proof}
The denominator $(q;q)_n$ is positive, and the sign of \eqref{eq:gaussian-lagrange-weights} is $(-1)^{n-k}$.  Put $j=n-k$ and use symmetry of the Gaussian coefficient:
\[
 \sum_{k=0}^n|\lambda_{n,k}^{(q)}|
 =\frac1{(q;q)_n}
 \sum_{j=0}^nq^{\binom{j+1}{2}}\qbinom nj.
\]
The positive finite $q$-binomial theorem with $z=q$ gives
\[
 \sum_{j=0}^nq^{\binom j2}\qbinom njq^j=(-q;q)_n,
\]
proving \eqref{eq:q-filter-norm}.  Both finite products converge to nonzero limits when $0<q<1$, so the uniform bound follows.
\end{proof}

\begin{remark}
This boundedness is a genuinely geometric phenomenon.  As $q\to1^-$ with $n$ fixed, the nodes coalesce and the weights diverge; but for a fixed $q<1$, increasing the order does not force unbounded total variation.  This helps explain why dyadic filters are effective in exact arithmetic and in high-precision computations.
\end{remark}

\section{The reciprocal grid and a leading-coefficient formula}

The nodes $1,q^{-1},\ldots,q^{-n}$ give a second useful form, adapted to extracting the leading coefficient of a polynomial from values at an expanding geometric grid.

\begin{theorem}[Reciprocal-grid coefficient extractor]\label{thm:reciprocal-grid-extractor}
Assume $q\ne0$ and $1,q,\ldots,q^n$ are distinct.  If $P$ has degree at most $n$, then
\begin{equation}
 [t^n]P(t)=
 \frac{(-1)^nq^{\binom{n+1}{2}}}{(q;q)_n}
 \sum_{k=0}^n(-1)^kq^{\binom k2}\qbinom nkP(q^{-k}).\label{eq:reciprocal-grid-extractor}
\end{equation}
\end{theorem}

\begin{proof}
Apply \Cref{thm:leading-coefficient-divdiff} to $x_k=q^{-k}$.  Splitting the barycentric denominator at $j=k$ gives
\begin{align*}
 \prod_{j\ne k}(q^{-k}-q^{-j})
 &=\left(\prod_{j=0}^{k-1}q^{-k}(1-q^{k-j})\right)
   \left(\prod_{j=k+1}^{n}(-q^{-j})(1-q^{j-k})\right)\\
 &=(-1)^{n-k}q^{-\binom{n+1}{2}-\binom k2}
   (q;q)_k(q;q)_{n-k}.
\end{align*}
Taking reciprocals and using
\[
 \frac1{(q;q)_k(q;q)_{n-k}}=\frac1{(q;q)_n}\qbinom nk
\]
turns the divided-difference formula into \eqref{eq:reciprocal-grid-extractor}.
\end{proof}

\section{The dyadic filter and Mersenne products}

For $q=\tfrac12$, define
\begin{equation}
 \mathfrak m_n=\prod_{j=1}^n(2^j-1),\qquad \mathfrak m_0=1.\label{eq:mersenne-product-filter}
\end{equation}

\begin{corollary}[Integer-normalized dyadic weights]\label{cor:dyadic-lagrange-weights}
On the nodes $1,2^{-1},\ldots,2^{-n}$,
\begin{equation}
 \lambda_{n,k}^{(1/2)}
 =(-1)^{n-k}\frac{2^{\binom{k+1}{2}}}
 {\mathfrak m_k\mathfrak m_{n-k}}.\label{eq:dyadic-lagrange-weights}
\end{equation}
Moreover, for every $d\ge1$,
\begin{equation}
 \sum_{k=0}^n\lambda_{n,k}^{(1/2)}2^{-kd}
 =(-1)^n2^{-\binom{n+1}{2}}
 \Qbinom{d-1}n{1/2}.\label{eq:dyadic-filter-residual}
\end{equation}
\end{corollary}
...
\]
follow by factoring $2^{-j}$ from each $1-2^{-j}$ in the factorial quotient.  Substitute them into \eqref{eq:gaussian-lagrange-weights}; the power of $2$ simplifies because
\[
 \binom{n+1}{2}-\binom{n-k+1}{2}-k(n-k)=\binom{k+1}{2}.
\]
This proves \eqref{eq:dyadic-lagrange-weights}.  Equation \eqref{eq:dyadic-filter-residual} is \eqref{eq:geometric-filter-residual} with $c=1$ and $q=1/2$.
\end{proof}


\chapter{Geometric uniform convolutions and the Fabius function}\label{chap:fabius-probability}
\index{Fabius function}
\index{uniform convolution!geometrically weighted}
\index{moment generating function!Fabius distribution}

The Fabius function enters $q$-analysis through an infinite convolution whose scales form a geometric progression.  At the special ratio $q=\tfrac12$, the moment-generating function obeys the same dilation law that drives the half-base Gaussian filters.  This chapter develops the probabilistic model, its smoothness, its moments, and its inverse-dyadic values from first principles.  Historical background and arithmetic refinements can be found in \cite{JessenWintner1935,Fabius1966,AriasDeReyna2018}; the repository exposition \cite{ReshetnikovProveIt2026} is the immediate source of the dyadic $q$-connections developed in the next chapter.

\section{A one-parameter family of geometric convolutions}

Let $U_0,U_1,\ldots$ be independent random variables, each uniformly distributed on $[0,1]$.

\begin{definition}
For $0<\rho<1$, define
\begin{equation}
 X_\rho=(1-\rho)\sum_{j=0}^{\infty}\rho^jU_j.\label{eq:geometric-uniform-variable}
\end{equation}
Let $F_\rho(x)=\Prob(X_\rho\le x)$.
\end{definition}

\begin{theorem}[Existence, symmetry, and self-similarity]\label{thm:geometric-uniform-basic}
The series in \eqref{eq:geometric-uniform-variable} converges for every outcome and satisfies $0\le X_\rho\le1$.  Furthermore,
\begin{align}
 X_\rho&\mathrel{\overset{\mathrm d}{=}}1-X_\rho,\label{eq:geometric-uniform-symmetry}\\
 X_\rho&\mathrel{\overset{\mathrm d}{=}}(1-\rho)U+\rho X_\rho',\label{eq:geometric-uniform-fixedpoint}
\end{align}
where $U$ is uniform on $[0,1]$, $X_\rho'$ is an independent copy of $X_\rho$, and $\overset{\mathrm d}{=}$ denotes equality in distribution.  Consequently,
\[
 F_\rho(1-x)=1-F_\rho(x)
\]
at every continuity point of $F_\rho$.
\end{theorem}

\begin{proof}
Every summand is nonnegative, and the partial sums are bounded by
\[
 (1-\rho)\sum_{j=0}^{\infty}\rho^j=1,
\]
so the series converges pointwise and has values in $[0,1]$.  Since $1-U_j$ is again uniform on $[0,1]$ and the variables remain independent,
\[
 1-X_\rho=(1-\rho)\sum_{j=0}^{\infty}\rho^j(1-U_j)
 \mathrel{\overset{\mathrm d}{=}}X_\rho,
\]
which proves \eqref{eq:geometric-uniform-symmetry}.  Separating the first summand gives
\[
 X_\rho=(1-\rho)U_0+\rho(1-\rho)\sum_{j=0}^{\infty}\rho^jU_{j+1},
\]
and the tail is an independent copy of $X_\rho$, proving \eqref{eq:geometric-uniform-fixedpoint}.  The distributional symmetry gives the stated relation for the distribution function.
\end{proof}
...
\begin{theorem}[Entire moment-generating product]\label{thm:geometric-uniform-mgf}
The moment-generating function $A_\rho(z)=\E(\e^{zX_\rho})$ is entire and has the locally uniformly convergent product
\begin{equation}
 A_\rho(z)=\prod_{j=0}^{\infty}
 E\bigl((1-\rho)\rho^jz\bigr).\label{eq:geometric-uniform-mgf}
\end{equation}
It obeys the refinement equation
\begin{equation}
 A_\rho(z)=E((1-\rho)z)A_\rho(\rho z).\label{eq:geometric-uniform-mgf-refine}
\end{equation}
\end{theorem}

\begin{proof}
For a uniform random variable $U$,
\[
 \E(\e^{tU})=\int_0^1\e^{tu}\,\dd u=E(t).
\]
Independence gives the finite product for the partial sums in \eqref{eq:geometric-uniform-variable}.  On every compact set $|z|\le M$,
\[
 E((1-\rho)\rho^jz)=1+O_M(\rho^j),
\]
and $\sum_j\rho^j<\infty$; hence the infinite product converges locally uniformly to an entire function.  Since the partial sums converge boundedly to $X_\rho$, dominated convergence identifies this product with $A_\rho$.  Separating the factor $j=0$ proves \eqref{eq:geometric-uniform-mgf-refine}.
\end{proof}

Write
\begin{equation}
 A_\rho(z)=\sum_{n=0}^{\infty}a_n(\rho)z^n,
 \qquad
 a_n(\rho)=\frac{\E(X_\rho^n)}{n!}.\label{eq:geometric-uniform-moments}
\end{equation}

\begin{corollary}[Triangular moment recurrence]\label{cor:geometric-uniform-moment-recurrence}
For $n\ge1$,
\begin{equation}
 (1-\rho^n)a_n(\rho)
 =\sum_{j=0}^{n-1}
 \frac{\rho^j(1-\rho)^{n-j}}{(n-j+1)!}a_j(\rho),
 \qquad a_0(\rho)=1.\label{eq:geometric-uniform-moment-recurrence}
\end{equation}
\end{corollary}

\begin{proof}
Expand both factors in \eqref{eq:geometric-uniform-mgf-refine}:
\[
 \sum_{n\ge0}a_nz^n
 =\left(\sum_{r\ge0}\frac{(1-\rho)^rz^r}{(r+1)!}\right)
  \left(\sum_{j\ge0}a_j\rho^jz^j\right).
\]
Equating the coefficient of $z^n$ and moving the $j=n$ term to the left gives \eqref{eq:geometric-uniform-moment-recurrence}.
\end{proof}

\section{The dyadic law and smoothness}

Set $\rho=\tfrac12$, write $X=X_{1/2}$, $F=F_{1/2}$, $A=A_{1/2}$, and $a_n=a_n(1/2)$.  Then
\begin{equation}
 X=\sum_{j=1}^{\infty}2^{-j}U_{j-1},
 \qquad
 A(z)=\prod_{j=1}^{\infty}E(z/2^j).\label{eq:fabius-random-variable}
\end{equation}

\begin{theorem}[The Fabius integral and differential equations]\label{thm:fabius-equations}
Extend $F$ by $F(x)=0$ for $x\le0$ and $F(x)=1$ for $x\ge1$.  Then
\begin{equation}
 F(x)=\int_0^1F(2x-u)\,\dd u
     =\int_{2x-1}^{2x}F(t)\,\dd t
 \qquad(x\in\R).\label{eq:fabius-integral-equation}
\end{equation}
The function $F$ belongs to $C^\infty(\R)$, is constant outside $[0,1]$, and satisfies
\begin{equation}
 F'(x)=2\bigl(F(2x)-F(2x-1)\bigr).\label{eq:fabius-global-differential}
\end{equation}
In particular,
\begin{equation}
 F'(x)=2F(2x),\qquad 0\le x\le\tfrac12,\label{eq:fabius-left-differential}
\end{equation}
and
\begin{equation}
 F(1-x)=1-F(x),\qquad 0\le x\le1.\label{eq:fabius-symmetry}
\end{equation}
Every derivative of positive order vanishes at $0$ and at $1$.
\end{theorem}

\begin{proof}
The fixed-point identity \eqref{eq:geometric-uniform-fixedpoint} becomes $X\overset{\mathrm d}{=}(U+X')/2$.  Conditioning on $U$ gives
\[
 F(x)=\int_0^1\Prob(X'\le2x-u)\,\dd u,
\]
which is \eqref{eq:fabius-integral-equation}.  If $g$ is bounded, then
\[
 (Tg)(x)=\int_{2x-1}^{2x}g(t)\,\dd t
\]
is continuous; indeed, translating the two endpoints by $2h$ changes the integral by at most $4\|g\|_\infty|h|$.  Thus $F=TF$ is continuous.  If $g\in C^r$, the fundamental theorem of calculus gives
\[
 (Tg)'(x)=2g(2x)-2g(2x-1),
\]
so $Tg\in C^{r+1}$.  Starting from continuity and iterating this observation proves $F\in C^\infty$ and gives \eqref{eq:fabius-global-differential}.  The left-hand formula follows because $F(2x-1)=0$ for $0\le x\le1/2$.  Symmetry follows from \eqref{eq:geometric-uniform-symmetry}; continuity removes the qualification about continuity points.  Finally, $F$ is constant on each side of the endpoints and is globally smooth, so all one-sided derivatives match and every positive-order derivative at $0$ and $1$ is zero.
\end{proof}

\begin{corollary}[Moment refinement at half base]\label{cor:fabius-moment-refinement}
The entire function $A$ satisfies
\begin{equation}
 A(2z)=E(z)A(z),\label{eq:fabius-mgf-refinement}
\end{equation}
and its coefficients obey
\begin{equation}
 (2^n-1)a_n=\sum_{j=0}^{n-1}\frac{a_j}{(n-j+1)!},
 \qquad a_0=1.\label{eq:fabius-moment-recurrence}
\end{equation}
\end{corollary}

\begin{proof}
Equation \eqref{eq:fabius-mgf-refinement} is \eqref{eq:geometric-uniform-mgf-refine} with $\rho=1/2$ and $z$ replaced by $2z$.  Equation \eqref{eq:fabius-moment-recurrence} follows from \eqref{eq:geometric-uniform-moment-recurrence} after multiplication by $2^n$.
\end{proof}

\section{Inverse-dyadic values as normalized moments}

\begin{theorem}[Endpoint-moment identity]\label{thm:fabius-endpoint-moment}
For every $n\ge0$,
\begin{equation}
 F(2^{-n})=2^{-\binom n2}a_n.\label{eq:fabius-endpoint-moment}
\end{equation}
Thus every inverse-dyadic value of $F$ is rational.
\end{theorem}

\begin{proof}
The case $n=0$ is $F(1)=a_0=1$.  Let $n\ge1$.  Iterating \eqref{eq:fabius-left-differential} on $0\le x\le2^{-n}$ gives
\begin{equation}
 F^{(n)}(x)=2^{1+2+\cdots+n}F(2^nx)
 =2^{\binom{n+1}{2}}F(2^nx).\label{eq:fabius-iterated-derivative}
\end{equation}
All derivatives of $F$ vanish at zero, so Taylor's formula with integral remainder gives
\[
 F(2^{-n})
 =\frac1{(n-1)!}\int_0^{2^{-n}}(2^{-n}-t)^{n-1}F^{(n)}(t)\,\dd t.
\]
Substitute \eqref{eq:fabius-iterated-derivative} and then $u=2^nt$:
\begin{equation}
 F(2^{-n})
 =\frac{2^{-\binom n2}}{(n-1)!}
 \int_0^1(1-u)^{n-1}F(u)\,\dd u.\label{eq:fabius-endpoint-integral}
\end{equation}
By symmetry, $X\overset{\mathrm d}{=}1-X$, and Stieltjes integration by parts yields
\[
 \E(X^n)=\E((1-X)^n)
 =\int_0^1(1-u)^n\,\dd F(u)
 =n\int_0^1(1-u)^{n-1}F(u)\,\dd u.
\]
The integral in \eqref{eq:fabius-endpoint-integral}, divided by $(n-1)!$, is therefore $\E(X^n)/n!=a_n$, proving \eqref{eq:fabius-endpoint-moment}.  Rationality follows inductively from \eqref{eq:fabius-moment-recurrence}.
\end{proof}

\begin{theorem}[Nonrecursive composition sum]\label{thm:fabius-composition-sum}
Let $\rho=(r_1,\ldots,r_\ell)\vDash n$ denote an ordered composition of $n$, and put $s_j=r_1+\cdots+r_j$.  For $n\ge1$,
\begin{align}
 a_n&=\sum_{\rho\vDash n}
 \prod_{j=1}^{\ell}
 \frac1{(2^{s_j}-1)(r_j+1)!},\label{eq:fabius-composition-a}\\
 F(2^{-n})&=2^{-\binom n2}
 \sum_{\rho\vDash n}
 \prod_{j=1}^{\ell}
 \frac1{(2^{s_j}-1)(r_j+1)!}.\label{eq:fabius-composition-F}
\end{align}
\end{theorem}

\begin{proof}
Rewrite \eqref{eq:fabius-moment-recurrence} as
\begin{equation}
 a_n=\sum_{r=1}^{n}\frac{a_{n-r}}{(2^n-1)(r+1)!}.\label{eq:fabius-moment-recurrence-r}
\end{equation}
We prove \eqref{eq:fabius-composition-a} by induction on $n$.  It is immediate for $n=1$.  In \eqref{eq:fabius-moment-recurrence-r}, choose the last part $r=r_\ell$ of a composition of $n$; the preceding parts form an arbitrary composition of $n-r$, and their partial sums are unchanged.  Appending $r$ introduces exactly the additional factor
\[
 \frac1{(2^n-1)(r+1)!}.
\]
Thus the recurrence enumerates every ordered composition exactly once, proving \eqref{eq:fabius-composition-a}.  Equation \eqref{eq:fabius-composition-F} follows from \Cref{thm:fabius-endpoint-moment}.
\end{proof}
...
\[
 F(1)=1,
 \qquad F(1/2)=\frac12,
 \qquad F(1/4)=\frac5{72},
 \qquad F(1/8)=\frac1{288}.
\]
For $n=2$, the two compositions $(2)$ and $(1,1)$ contribute $1/18$ and $1/12$, respectively, giving $a_2=5/36$.
\end{example}


\chapter{Thue--Morse cancellation and a half-base \texorpdfstring{$q$}{q}-formula}\label{chap:fabius-qformula}
\index{Thue--Morse sequence}
\index{Prouhet cancellation}
\index{Fabius function!$q$-binomial formula}

The dyadic Fabius formula is generated by two independent cancellation devices.  A length-$2^k$ Thue--Morse block removes the first $k$ ordinary polynomial degrees.  A length-$(n+1)$ Gaussian row at $q=\tfrac12$ then removes the remaining dilation modes below degree $n$.  Their composition leaves one coefficient.  This section proves that mechanism in a form that is both translation-invariant and suitable for exact computation.

\section{The signed binary product}

Let $\wt(r)$ denote the number of $1$'s in the binary expansion of $r$, and set
\begin{equation}
 \tau_r=(-1)^{\wt(r)}.\label{eq:thue-morse-sign}
\end{equation}

\begin{theorem}[Finite Thue--Morse product]\label{thm:thue-morse-product}
For every $k\ge0$,
\begin{equation}
 \sum_{r=0}^{2^k-1}\tau_rz^r
 =\prod_{j=0}^{k-1}(1-z^{2^j}).\label{eq:thue-morse-product}
\end{equation}
\end{theorem}

\begin{proof}
Every integer $0\le r<2^k$ has a unique binary expansion
$r=\sum_{j=0}^{k-1}\varepsilon_j2^j$ with $\varepsilon_j\in\{0,1\}$.  Expanding the product on the right and choosing either $1$ or $-z^{2^j}$ from its $j$th factor produces the monomial
\[
 (-1)^{\varepsilon_0+\cdots+\varepsilon_{k-1}}
 z^{\sum_j\varepsilon_j2^j}=\tau_rz^r.
\]
Summing over all choices proves the identity.
\end{proof}

\begin{theorem}[Prouhet cancellation with translation]\label{thm:thue-morse-prouhet}
For every complex $c$ and every polynomial $P$ of degree less than $k$,
\begin{equation}
 \sum_{r=0}^{2^k-1}\tau_rP(r+c)=0.\label{eq:thue-morse-prouhet}
\end{equation}
For the first nonvanishing degree,
\begin{equation}
 \sum_{r=0}^{2^k-1}\tau_r(r+c)^k
 =(-1)^k2^{\binom k2}k!,\label{eq:thue-morse-first-moment}
\end{equation}
which is independent of $c$.
\end{theorem}

\begin{proof}
By \Cref{thm:thue-morse-product},
\begin{align*}
 \sum_{r=0}^{2^k-1}\tau_r\e^{(r+c)t}
 &=\e^{ct}\prod_{j=0}^{k-1}(1-\e^{2^jt})\\
 &=\e^{ct}\prod_{j=0}^{k-1}
 \left(-2^jt+O(t^2)\right)\\
 &=(-1)^k2^{0+1+\cdots+(k-1)}t^k+O(t^{k+1}).
\end{align*}
The coefficient of $t^d/d!$ on the left is
$\sum_r\tau_r(r+c)^d$.  The coefficients through degree $k-1$ vanish, and the coefficient at degree $k$ is $(-1)^k2^{\binom k2}$, proving both claims for monomials.  Linearity gives \eqref{eq:thue-morse-prouhet} for every polynomial of degree below $k$.
\end{proof}

\begin{corollary}[Difference-operator factorization]\label{cor:thue-morse-difference-operator}
If $D=\frac{\dd}{\dd c}$ and $\e^{hD}P(c)=P(c+h)$, then
\begin{equation}
 \sum_{r=0}^{2^k-1}\tau_rP(c+r)
 =\prod_{j=0}^{k-1}(1-\e^{2^jD})P(c).\label{eq:thue-morse-difference-operator}
\end{equation}
Each factor lowers polynomial degree by at least one.
\end{corollary}

\begin{proof}
Expand the commuting product of translation operators exactly as in the binary proof of \Cref{thm:thue-morse-product}.  The operator $1-\e^{hD}$ sends $P(c)$ to $P(c)-P(c+h)$, whose degree is at most $\deg P-1$.
\end{proof}

\section{Half-base Gaussian coefficients and Mersenne factors}

Recall $\mathfrak m_n$ from \eqref{eq:mersenne-product-filter}.

\begin{proposition}[The half-base dictionary]\label{prop:half-base-dictionary}
For $0\le k\le n$,
\begin{align}
 (1/2;1/2)_n
 &=2^{-\binom{n+1}{2}}\mathfrak m_n,\label{eq:halfbase-pochhammer-mersenne}\\
 \Qbinom nk{1/2}
 &=\frac{\mathfrak m_n}
 {\mathfrak m_k\mathfrak m_{n-k}2^{k(n-k)}}.\label{eq:halfbase-gaussian-mersenne}
\end{align}
Moreover, for integers $m,n\ge0$,
\begin{equation}
 (2^m;1/2)_n=
 \begin{cases}
 0,&m<n,\\
 (-1)^n\dfrac{\mathfrak m_m}{\mathfrak m_{m-n}},&m\ge n.
 \end{cases}\label{eq:halfbase-termination}
\end{equation}
\end{proposition}
...
\[
 (1/2;1/2)_n
 =\prod_{j=1}^n(1-2^{-j})
 =2^{-\sum_{j=1}^nj}\prod_{j=1}^n(2^j-1).
\]
Substitute this formula into the factorial quotient for the Gaussian coefficient.  The power of $2$ in the denominator is
\[
 \binom{n+1}{2}-\binom{k+1}{2}-\binom{n-k+1}{2}=k(n-k),
\]
which proves \eqref{eq:halfbase-gaussian-mersenne}.  Finally,
\[
 (2^m;1/2)_n=\prod_{j=0}^{n-1}(1-2^{m-j}).
\]
If $m<n$, the factor $j=m$ is zero.  If $m\ge n$, put $s=m-j$; the product is
\[
 (-1)^n\prod_{s=m-n+1}^{m}(2^s-1)
 =(-1)^n\frac{\mathfrak m_m}{\mathfrak m_{m-n}}.
\]
\end{proof}

\begin{theorem}[The half-base $q$-Prouhet functional]\label{thm:q-prouhet-functional}
Define
\begin{equation}
 w_{n,k}=(-1)^k2^{-\binom k2}\Qbinom nk{1/2}.\label{eq:q-prouhet-weights}
\end{equation}
For every polynomial $P$ of degree at most $n$,
\begin{equation}
 \sum_{k=0}^nw_{n,k}P(-2^k)
 =\mathfrak m_n[t^n]P(t).\label{eq:q-prouhet-functional}
\end{equation}
\end{theorem}

\begin{proof}
By linearity it is enough to take $P(t)=t^d$, $0\le d\le n$.  Then the left side is
\[
 (-1)^d\sum_{k=0}^n(-1)^k2^{-\binom k2}
 \Qbinom nk{1/2}2^{kd}.
\]
The finite $q$-binomial theorem \eqref{eq:universal-negative-finite}, with $q=1/2$ and $z=2^d$, identifies the sum as
$(-1)^d(2^d;1/2)_n$.  By \eqref{eq:halfbase-termination}, this is zero for $d<n$ and equals
\[
 (-1)^n(2^n;1/2)_n=\mathfrak m_n
\]
for $d=n$.  These are exactly the values of the right side.
\end{proof}

\section{The translation-invariant Fabius sum}

For $n,k\ge0$ and $Q\in\C$, define
\begin{equation}
 M_{k,d}(Q)=\sum_{r=0}^{2^k-1}\tau_r(r+Q)^d.\label{eq:thue-morse-power-sum}
\end{equation}
By \Cref{thm:thue-morse-prouhet}, $M_{k,d}(Q)=0$ for $d<k$.

\begin{theorem}[Half-base Gaussian--Thue--Morse synthesis]\label{thm:fabius-q-synthesis}
For every $n\ge0$ and every $Q\in\C$, put
\begin{equation}
 S_n(Q)=\sum_{k=0}^n
 \frac{\Qbinom nk{1/2}}
 {4^{\binom k2}(n+k)!}
 M_{k,n+k}(Q).\label{eq:fabius-Sn}
\end{equation}
Then $S_n(Q)$ is independent of $Q$ and
\begin{equation}
 S_n(Q)=(-1)^n\mathfrak m_na_n.\label{eq:fabius-Sn-value}
\end{equation}
Consequently,
\begin{equation}
 \boxed{
 ...
 \frac{\Qbinom nk{1/2}}
 {4^{\binom k2}(n+k)!}
 \sum_{r=0}^{2^k-1}(-1)^{\wt(r)}(r+Q)^{n+k}}\label{eq:fabius-q-formula}
\end{equation}
for every choice of the translation parameter $Q$.
\end{theorem}

\begin{proof}
Coefficient extraction from the exponential gives
\begin{align}
 \frac{M_{k,n+k}(Q)}{(n+k)!}
 &=[z^{n+k}]\e^{Qz}
   \sum_{r=0}^{2^k-1}\tau_r\e^{rz}\\
 &=[z^{n+k}]\e^{Qz}
   \prod_{j=0}^{k-1}(1-\e^{2^jz}).\label{eq:thue-egf-coefficient}
\end{align}
From \eqref{eq:fabius-mgf-refinement}, iterated $k$ times,
\begin{equation}
 A(2^kz)=A(z)\prod_{j=0}^{k-1}E(2^jz).\label{eq:fabius-mgf-iterated}
\end{equation}
Since $1-\e^{2^jz}=-2^jzE(2^jz)$, equations \eqref{eq:thue-egf-coefficient}--\eqref{eq:fabius-mgf-iterated} give
\begin{equation}
 \frac{M_{k,n+k}(Q)}{(n+k)!}
 =(-1)^k2^{\binom k2}
 [z^n]\frac{\e^{Qz}A(2^kz)}{A(z)}.\label{eq:thue-fabius-coefficient}
\end{equation}
Insert this into \eqref{eq:fabius-Sn}.  Because
$4^{-\binom k2}2^{\binom k2}=2^{-\binom k2}$,
\begin{equation}
 S_n(Q)=[z^n]\frac{\e^{Qz}}{A(z)}
 \sum_{k=0}^n(-1)^k2^{-\binom k2}
 \Qbinom nk{1/2}A(2^kz).\label{eq:fabius-Sn-filtered}
\end{equation}
Expand $A(2^kz)=\sum_{j\ge0}a_j2^{kj}z^j$ and use the finite $q$-binomial theorem:
\begin{align}
 &\sum_{k=0}^n(-1)^k2^{-\binom k2}
 \Qbinom nk{1/2}A(2^kz)\\
 &\qquad=\sum_{j\ge0}a_jz^j
 \sum_{k=0}^n(-1)^k2^{-\binom k2}
 \Qbinom nk{1/2}(2^j)^k\\
 &\qquad=\sum_{j\ge0}a_j(2^j;1/2)_nz^j.\label{eq:fabius-mode-filter}
\end{align}
To extract $[z^n]$ in \eqref{eq:fabius-Sn-filtered}, only $j\le n$ can contribute because $\e^{Qz}/A(z)$ is a power series with constant term $1$.  Formula \eqref{eq:halfbase-termination} shows that every term with $j<n$ vanishes.  The sole survivor is $j=n$, and its contribution is
\[
 a_n(2^n;1/2)_n=(-1)^n\mathfrak m_na_n.
\]
This proves \eqref{eq:fabius-Sn-value}, and it also proves independence of $Q$: the translation factor contributes only its constant term.

Finally, by \eqref{eq:halfbase-pochhammer-mersenne},
\[
 (-2)^{n^2}(1/2;1/2)_n
 =(-1)^{n^2}2^{n^2-\binom{n+1}{2}}\mathfrak m_n
 =(-1)^{n^2}2^{\binom n2}\mathfrak m_n.
\]
Since $n-n^2$ is even, dividing \eqref{eq:fabius-Sn-value} by this quantity gives
$2^{-\binom n2}a_n=F(2^{-n})$ by \Cref{thm:fabius-endpoint-moment}.  This is \eqref{eq:fabius-q-formula}.
\end{proof}

\begin{example}[The first two nontrivial cases]\label{ex:fabius-q-small}
For $n=1$,
\[
 S_1(Q)=Q+\frac{Q^2-(Q+1)^2}{2}=-\frac12.
\]
The denominator in \eqref{eq:fabius-q-formula} is
$(-2)(1/2;1/2)_1=-1$, so $F(1/2)=1/2$.

For $n=2$, the theorem gives $S_2(Q)=\mathfrak m_2a_2=3\cdot5/36=5/12$ for every $Q$.  Since
\[
 (-2)^4(1/2;1/2)_2=16\cdot\frac12\cdot\frac34=6,
\]
we obtain $F(1/4)=5/72$.
\end{example}

\section{Polynomial-time evaluation of the signed moments}

The innermost sum in \eqref{eq:fabius-q-formula} has $2^k$ terms, but the binary self-similarity removes the apparent exponential cost.

\begin{proposition}[Recurrence for Thue--Morse power sums]\label{prop:thue-morse-power-recurrence}
For $k,d\ge0$,
\begin{equation}
 M_{0,d}(Q)=Q^d,\label{eq:thue-morse-power-base}
\end{equation}
and
\begin{equation}
 M_{k+1,d}(Q)
 =-\sum_{j=1}^d\binom dj2^{kj}M_{k,d-j}(Q).\label{eq:thue-morse-power-recurrence}
\end{equation}
In particular, all $M_{k,d}(Q)$ with $0\le k\le n$ and $0\le d\le2n$ can be computed with a polynomial number of arithmetic operations.
\end{proposition}

\begin{proof}
The signs satisfy $\tau_{r+2^k}=-\tau_r$ for $0\le r<2^k$.  Hence
\begin{align*}
 M_{k+1,d}(Q)
 &=\sum_{r=0}^{2^k-1}\tau_r
 \bigl((r+Q)^d-(r+Q+2^k)^d\bigr)\\
 &=-\sum_{j=1}^d\binom dj2^{kj}
 \sum_{r=0}^{2^k-1}\tau_r(r+Q)^{d-j},
\end{align*}
which is \eqref{eq:thue-morse-power-recurrence}.  Filling a triangular table in increasing $k$ and $d$ uses finitely many inner sums of length at most $2n$, so the operation count is polynomial in $n$.
\end{proof}

\begin{corollary}[Degree consumption]
For every $k$, $M_{k,d}(Q)=0$ when $d<k$, and $M_{k,k}(Q)=(-1)^k2^{\binom k2}k!$.
\end{corollary}

\begin{proof}
This follows from \Cref{thm:thue-morse-prouhet}; it also follows inductively from \eqref{eq:thue-morse-power-recurrence}, which makes the triangular vanishing explicit in computation.
\end{proof}

\section{What the formula is really doing}

Equation \eqref{eq:fabius-q-formula} is not a disguised classical hypergeometric summation.  It is a composition of three exact operations:
\begin{enumerate}
\item the Thue--Morse block contributes the factor
$\prod_{j<k}(1-\e^{2^jz})$, consuming $k$ ordinary Taylor degrees;
\item the Fabius refinement law converts that product into the dilation ratio $A(2^kz)/A(z)$;
\item the finite Gaussian row at $q=1/2$ multiplies the $j$th dilation mode by $(2^j;1/2)_n$, annihilating every $j<n$ and retaining only $j=n$ at the required coefficient.
\end{enumerate}
The free translation $Q$ disappears because, after both cancellation stages, it can contribute only the constant coefficient of $\e^{Qz}/A(z)$.  Thus translation independence is not a coincidental polynomial simplification; it is a spectral statement.

The same architecture suggests a broader research program.  Whenever an entire function satisfies a multiplicative dilation law
\[
 B(bz)=H(z)B(z)
\]
with $H$ admitting a finite signed-product realization, one may combine a digital cancellation sequence with a Gaussian filter at $q=b^{-1}$.  The universal ring formulation of \Cref{chap:universal-gaussian} is particularly useful here: the finite filter can be established algebraically before any analytic convergence argument is introduced.

\section{Formal certification and automated identity discovery}

Two recent computational directions complement the proof-oriented development above.  First, the Lean project of Lau, Lee, and Ono formalizes $q$-Pochhammer symbols, Gaussian coefficients, Bailey pairs, Jacobi's triple product, and the Rogers--Ramanujan identities while carefully separating formal algebra from analytic convergence \cite{LauLeeOno2026}.  Second, Zhong and Zhao's $q$-binomial identity finder systematically translates basic hypergeometric identities into Gaussian-coefficient identities and emits proof-producing transformation data \cite{ZhongZhao2024}.  The present chapters indicate a third target for automation: finite geometric filters whose correctness reduces to the universal Pascal recurrence and coefficient extraction.  In such a pipeline, a computer may discover a stencil or a dyadic formula, but the certificate remains a short polynomial identity of the kind proved in \Cref{thm:universal-finite-qbinomial,thm:complete-residual-moments,thm:fabius-q-synthesis}.
...
V.~Reshetnikov,
\emph{ProveIt: Fabius Function and Rvachev Up documentation},
source repository, 2026,
\url{https://github.com/VladimirReshetnikov/ProveIt/tree/main/Analysis/FabiusFunction/docs},
accessed August 2026.

\bibitem{ZhongZhao2024}
H.~Zhong and L.~Zhao,
$q$-Binomial identities finder,
preprint, arXiv:2408.00186, version 2, 2024.
