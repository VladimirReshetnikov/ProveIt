...
(\lean{Fabius.fract\_natCast\_div\_two\_pow\_eq\_sum\_range}); what is not formalized is the
exchange of summation that converts it into \eqref{p1:eq:M-bit} itself.
\end{theorem}
...
the general ones
(\lean{Fabius.sum\_abs\_discreteLimitWeight\_of\_qToeplitz}, \lean{Fabius.sum\_range\_discreteLimitWeight\_mul\_pow\_of\_qToeplitz}).  What is still not
formalized is the numerical limiting value \eqref{p1:eq:qquarter-condition} at the cumulant grid,
and the uniform-in-$q$ bound: the ratio is not bounded as $q\to1^-$, so the half-base constant
$16$ has no general-$q$ analogue --- only the lower bound $\ge1$ is proved
(\lean{Fabius.one\_le\_sum\_abs\_qToeplitzWeight}).
\end{theorem}
