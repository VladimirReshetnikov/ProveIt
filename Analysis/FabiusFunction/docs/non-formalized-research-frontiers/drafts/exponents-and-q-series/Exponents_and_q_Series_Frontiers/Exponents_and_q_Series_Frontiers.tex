...
The locally uniform convergence the volume states alongside this display is not
formalized, and neither is the consequence $\ord_{z=\pm n}\Phi_a=m_a(n)$ at a general
weight --- that is a statement about \lean{analyticOrderAt} and so needs analyticity of
$\Phi_a$, which needs the locally uniform convergence.  At the \emph{constant} weight,
however, the corpus has the whole story, and the display is a theorem:
...
with the exponent identity.  For a general admissible $a$ the algebraic input is in place
--- the factorization $\Phi_a(z)=(1-z^2/n^2)^{m_a(n)}R_n(z)$ with $R_n(\pm n)\ne0$
(\lean{Fabius.generalizedRvachevProduct\_eq\_canonicalComplexFactor\_mul},
\lean{Fabius.canonicalCofactor\_natCast\_ne\_zero}) --- and only the analyticity is missing.  The spectral zeta \eqref{p1:eq:zeta-a} is now
...
above.  Negative arguments are not treated at all: $\Phi_a$ is even, so the law there reads
$\varepsilon_a(\lfloor\lvert x\rvert\rfloor)$ rather than $\varepsilon_a(\lfloor x\rfloor)$, and neither
file states it.
\end{theorem}
