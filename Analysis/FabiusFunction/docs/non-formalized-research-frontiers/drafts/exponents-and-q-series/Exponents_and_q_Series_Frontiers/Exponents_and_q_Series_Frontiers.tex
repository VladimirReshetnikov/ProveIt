...
\path{source_rvachev_scan.pdf} is archived verbatim under
\path{assets/Atomic_Functions_Beyond_Dyadic_Report-2/}, so the translation
layers of this part can be checked against the primary source inside the
repository.  (The raw OCR transcription that accompanied it was deleted
after its recoverable content had been merged --- three independent
reconstructions agree on every recoverable equation, and the merged repair
ledger in the concordance appendix records every correction; the OCR file's
SHA-256 remains in the provenance list and git history archives it.)  A revised edition of the \emph{fifteenth}-wave report is merged
