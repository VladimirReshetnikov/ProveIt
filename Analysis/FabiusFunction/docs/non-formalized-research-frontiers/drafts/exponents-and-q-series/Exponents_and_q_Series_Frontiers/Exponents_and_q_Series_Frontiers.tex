...
\cref{p1:lem:q-weights} is formal as well (see the crosswalk paragraphs there).  These labels
...
\eqref{p1:eq:q-survivor} is \lean{Fabius.sum_geometricLagrangeWeight_mul_pow_succ}.  Both are
...
\eqref{p1:eq:q-weights} and the range $h>r+1$ of \eqref{p1:eq:q-survivor} are not formalized for
...
\eqref{p1:eq:Rrm-exact}, the sign \eqref{p1:eq:Rrm-sign} and the bound \eqref{p1:eq:Rrm-bound} are not
formalized.
\end{theorem}
...
free-even-layer count, the closed Gaussian-binomial form \eqref{p1:eq:q-weights}, and the
general-$q$ survivor \eqref{p1:eq:q-survivor} with the sign-certified remainder built on it.
...
\eqref{p2:eq:alpharec}; then prove the coefficientwise filtered identity
\eqref{p2:eq:exacterror}. This avoids analytic convergence in the first pass.
