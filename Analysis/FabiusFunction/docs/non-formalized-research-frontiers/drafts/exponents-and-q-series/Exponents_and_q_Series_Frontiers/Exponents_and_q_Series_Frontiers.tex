...
their verification packages are preserved verbatim under \path{assets/}.  Later edits add status-only Lean
...
$\rho_r(4\pi)^{2r}$ & $0.40698$ & $3.58279$ & $3.611538$
 & $3.6115993859$ & $3.6115993860280563$ & $3.61159938602805644868323101$\\
\bottomrule
\end{tabular}
\end{center}
...
-\tfrac{64^{-n}}{2679075}\Up^{(6)}+\cdots$ is simultaneously an asymptotic
...
\item \conjflag\ The endpoint $b-X_a$ has a multiplicatively periodic saddle
structure.  The lower branch $W_{-1}$ of Lambert's function again supplies the
natural inverse coordinate, now with period one in $\log_a s$.  A precise
all-orders expansion is formulated as a research target.
\end{enumerate}
...
\begin{equation}\label{p6:eq:up-Fup1}
 ...
 \right).
\end{equation}
Nothing beyond this identity survives in the supplied files.  A systematic
factor-redistribution hierarchy is proposed in \cref{p6:sec:future-directions}, but
it is not presented as translated source material.
...
in the repository's binary endpoint asymptotics.
...
\begin{conjbox}
\textbf{All-orders endpoint expansion for $h_a$.}
...
The main unresolved tasks are to identify $c_a,A_a,B_a$ without convention
ambiguity, construct the periodic saddle jets, and prove a uniform remainder.
Mellin--Poisson analysis of \eqref{p6:eq:Ya-Laplace} is a promising route.
...
Questions include support, smoothness, exact dyadic values, polynomial
reproduction order, and whether the resulting family forms a useful multiscale
basis.

\subsubsection{Mellin analysis of the endpoint product}
Take the logarithm of \eqref{p6:eq:Ya-Laplace}, separate the main integral in the
scale variable $j$, and apply Poisson summation to the residual function of
$\log_a s$.  This should isolate the periodic phase before saddle inversion.
The binary repository already contains the appropriate lower-Lambert and
periodic-jet technology; the new work is to retain the parameter $a$ and control
uniformity.
