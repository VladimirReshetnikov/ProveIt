...
product identity.  What remains unformalized is the sign law \eqref{p1:eq:sign-master}
itself at general $a$, relating the character to $\sgn\Phi_a$, and the automaticity
...
commutative monoid for an arbitrary summand depending on the two parts, so it serves
both displays; a constant-weight corollary recovers the classical Prouhet--Thue--Morse
evaluation
...
a theorem.  What is still not formalized is the numerical limiting
value \eqref{p1:eq:qquarter-condition} at the cumulant grid: the limit
is identified as a product, but its decimal expansion is not certified.
\end{theorem}
