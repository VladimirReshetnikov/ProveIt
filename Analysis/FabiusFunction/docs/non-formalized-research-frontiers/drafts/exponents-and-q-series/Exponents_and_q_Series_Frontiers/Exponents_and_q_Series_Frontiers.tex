...
The verification packages of all merged drafts are preserved verbatim under \path{assets/}.  Later edits add status-only Lean
...
\item \path{Atomic_Functions_Beyond_Dyadic_Report-2/Atomic_Functions_Beyond_Dyadic.tex} \\ {\footnotesize SHA-256 \path{c560d350d80655759a463c7930b8d1131137d214c40d834f3b7e95d2ca253b11}} (revised fourteenth-wave edition; merged into Part~VI --- Orlicz/rearrangement equimeasurability and the spectral Stieltjes--Wigert bridge; ships the Russian source scan \path{source_rvachev_scan.pdf}, SHA-256 \path{ad4b0078dc1d5de133e5f2bf8f673ea080197c16a624d4fec7cb5126c59c6320}, and its OCR \path{source_rvachev_ocr.tex}, SHA-256 \path{c1a5074f2b3099f5d48b166ab43bc2726139282fe174537c60bc1ae4010be46b})
...
\end{equation}
both locally uniformly in their natural domains (legitimate because
$\sum_m\vtwo(m)m^{-2}=\sum_{j\ge1}\sum_{2^j\mid m}m^{-2}<\infty$).
\end{theorem}
...
$\Phi(w)=\bigl(\prod_{k=1}^{r-1}\sinc(w/2^k)\bigr)\Phi(w/2^{r-1})$; at
$w=w_0$ every displayed factor vanishes linearly, all the associated integer
multiples are even except the last, so the product of their sign slopes is
$-1$ and
...
\end{equation}
(The two groupings, from the factorizations
$F_n=P_n\cdot\sinc(t/2^n)$ and
$F_n=\bigl(\prod_{k<n}\sinc(t/2^k)\bigr)\sinc^2(t/2^n)$ respectively, were
derived independently in the two sources and agree in all regimes.)
...
\begin{equation}
 ...
 \quad I\approx11.8007105434191,
 \quad J\approx1619.745753585,
 \quad L\approx485.548634253.
...
of (3.51)--(3.58) into an explicit binary-encoded form.  The two packages'
OCR-reconstruction ledgers are preserved in
\path{assets/Atomic_Functions_Beyond_Dyadic_Report/} and
\path{assets/Rvachev_Atomic_Functions_Report/}.
...
Decisively for provenance, the first revised edition ships the surviving
\emph{Russian source itself}: the page scan
\path{source_rvachev_scan.pdf} and its OCR transcription
\path{source_rvachev_ocr.tex} are archived verbatim under
\path{assets/Atomic_Functions_Beyond_Dyadic_Report-2/}, so the translation
layers of this part can now be checked against the primary source inside the
repository.  A revised edition of the \emph{fifteenth}-wave report is merged
...
distribution function is the exact tube formula.  The Russian source scan and
OCR are archived under \path{assets/}.
...
derivatives (revised fifteenth wave), and the resulting Hilbert-space
geometry is exactly solvable.
...
Asymptotically
$\log\det G_N=(N+1)\log(q^2;q^2)_\infty-\sum_{d\ge1}d\log(1-q^{2d})
+\bigO(q^{2N})$.  The theorem ties the Rvachev derivative tower to Gaussian
...
Arias de Reyna and Haugland~\cite{p6:arias-arithmetic,p6:haugland}.
\end{conjecture}
...
crosswalk table (which additionally records the chapter-number typo
``(8.85)'' for (3.85) and rebuilds the damaged endpoint indexing of
(3.51)--(3.58) in binary-encoded form).  Both ledgers are preserved verbatim
in
\path{assets/Atomic_Functions_Beyond_Dyadic_Report/Atomic_Functions_Beyond_Dyadic.tex}
and
\path{assets/Rvachev_Atomic_Functions_Report/Atomic_Functions_Rvachev_Report.tex}.
...
(\path{assets/Atomic_Functions_Beyond_Dyadic_Report/}) contains the merged
report source \texttt{Atomic\_Functions\_Beyond\_Dyadic.tex}, the script
...
(\path{assets/Rvachev_Atomic_Functions_Report/}) contains the merged report
source \texttt{Atomic\_Functions\_Rvachev\_Report.tex}, the deterministic
...
log-concave by Pr\'ekopa--Leindler; the class is weakly closed, and the limit
density is smooth.  Plateau: the fixed point \eqref{p7:eq:affine-fixed-point}
