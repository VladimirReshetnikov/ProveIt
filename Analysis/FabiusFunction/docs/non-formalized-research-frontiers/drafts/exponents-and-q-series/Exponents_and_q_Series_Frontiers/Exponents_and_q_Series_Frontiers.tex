...
One scope caveat, stated because the volume's phrasing is wider than what is proved:
the display above is qualified here \emph{away from the integer points altogether}, whereas
\eqref{p1:eq:sign-master} is stated "away from the integer zeros".  An integer $x$ at
which no factor of $\Phi_a$ vanishes --- possible when $a$ has zero layers --- falls inside
the volume's claim and outside the formalized one.
\end{theorem}
