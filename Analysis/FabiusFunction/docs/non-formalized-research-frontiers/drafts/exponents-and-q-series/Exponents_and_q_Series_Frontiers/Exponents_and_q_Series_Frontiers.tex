...
the \emph{analytic} half, that this sign is $\sgn\Phi_a$ on the lobe $(N,N+1)$, which
needs the canonical product at a general weight.
\end{theorem}
...
number as the \emph{support radius} of $\Phi_P$ is not formalized: it needs the canonical
product at a general weight, which the corpus does not carry.
