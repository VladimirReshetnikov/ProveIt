...
What is \emph{not} formalized is the analytic and probabilistic content of the theorem:
locally uniform convergence and hence entirety, the support statement (i), the random
variable $X_a$ and the convolution reading of (iii), and the smoothness dichotomy (iv).
The module proves the algebraic and convergence skeleton on which those would rest, and
...
The canonical \emph{form} \eqref{p1:eq:canonical-a}, which regroups the scale product
into one factor per integer zero, is formalized only at $a_h\equiv1$, as
\lean{Fabius.rvachevFourierProduct_eq_canonical} (\lean{SincCanonicalProduct.lean}), whose
exponent is the classical $\vtwo(n)+1$; the underlying scale product $\Phi_a$ itself is now
formal at a general admissible $a$ (\path{GeneralizedRvachevProduct.lean}, see
\cref{p1:thm:master-probability}), so what the regrouping still needs is the
multiplicative analogue of the fibre argument used for the spectral zeta below, not the
product itself.  The spectral zeta \eqref{p1:eq:zeta-a} is now
...
(\lean{Fabius.tsum\_padicValNat\_succ\_rpow}).  The general-$a$ product remains
unformalized.
\end{theorem}
...
(\lean{Fabius.neg\_one\_pow\_sum\_div\_two\_pow\_const\_one}).  What remains unformalized is
the \emph{analytic} half, that this sign is $\sgn\Phi_a$ on the lobe $(N,N+1)$.  The
product $\Phi_a$ is itself now available (\path{GeneralizedRvachevProduct.lean}); what is
still needed is the sign analysis on a lobe --- that every factor of index at most $N$
is negative there and the rest positive, which is the content of \eqref{p1:eq:lobe-sign-count}.
\end{theorem}
...
\eqref{p1:eq:finite-difference-factorization} is not formalized \emph{as an identity of products},
for the same reason: it quantifies over canonical products at general weights.  Its
exponent-level content is formal, however, in
...
(\lean{Fabius.weight\_fwdDiff\_eq\_sum\_shift}).  So what is missing is exactly the passage
from the exponent identity to the product identity, not the combinatorics.
\end{theorem}
...
\lean{Fabius.weight\_eq\_sum\_choose\_fwdDiff}; that it \emph{forces} the coefficients ---
that $\Delta^s$ applied to a Newton sum returns $c_s$ --- is not formalized, so the module
exhibits $\Delta^rP(0)$ as a valid choice rather than as the unique one.
...
coefficient sum (\lean{Fabius.tsum\_newtonPoly}, \lean{Fabius.half\_mul\_tsum\_newtonPoly\_half}).  Reading that
number as the \emph{support radius} of $\Phi_P$ is not formalized.  The product $\Phi_P$ is
available (\path{GeneralizedRvachevProduct.lean}); what is missing is the \emph{support}
statement of \cref{p1:thm:master-probability}, an analytic and probabilistic fact about
the associated random variable rather than a property of the product itself.
...
\newblock \texttt{ProveIt} repository, \repo{}/\texttt{non-formalized-research-frontiers}, same commit.
