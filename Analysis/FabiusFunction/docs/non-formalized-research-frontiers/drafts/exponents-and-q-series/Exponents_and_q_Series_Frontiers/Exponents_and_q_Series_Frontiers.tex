...
overlap-regime theta conjecture in the register.
\end{scopebox}
...
derivatives (revised and expanded fifteenth-wave editions), and the
resulting Hilbert-space geometry is exactly solvable --- down to a closed
Gram--Schmidt process and an explicit unitary circle model.
...
fifteenth wave): the orthogonal directions, the Cholesky factorization, and
the inverse Gram matrix are all single Gaussian-binomial sums.

\begin{theorem}[Closed $q$-Gram--Schmidt]\label{p6:thm:q-gram-schmidt}
Fix a parity and abbreviate $e_j=\eta_j$.  The vectors
...
kernel and its inverse explicitly:
\[
 C\,G_N\,C^{\mathsf T}
...
$CG_NC^{\mathsf T}=D$ and $G_N^{-1}=C^{\mathsf T}D^{-1}C$.
\end{proof}
