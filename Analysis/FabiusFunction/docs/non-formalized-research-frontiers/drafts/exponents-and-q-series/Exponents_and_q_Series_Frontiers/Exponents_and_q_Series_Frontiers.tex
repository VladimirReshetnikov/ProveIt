...
character.  What remains unformalized is the sign law \eqref{p1:eq:sign-master} itself at
general $a$, relating the character to $\sgn\Phi_a$, which needs the canonical product at
a general weight.
\end{theorem}
...
(\lean{Fabius.sum\_range\_two\_pow\_thueMorseSign\_mul\_pow}).  What is still not
formalized is the exact order $\omega_L(a)$ of the zero of $T_{a,L}$ at $z=1$, which is
the only remaining step of this theorem.
\end{theorem}
...
product at a general weight, which the corpus does not carry.  Neither the spectral zeta
\eqref{p1:eq:ZP} nor the cumulants \eqref{p1:eq:kappaP} are formalized.
\end{theorem}
