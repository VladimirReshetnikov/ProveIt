...
Two things the volume states alongside this display are \emph{not} formalized: the
locally uniform convergence, and the consequence $\ord_{z=\pm n}\Phi_a=m_a(n)$.  The
latter is a statement about an order of vanishing, and nothing in the corpus yet
relates $m_a$ to \lean{analyticOrderAt}; the canonical form is its natural input.  The spectral zeta \eqref{p1:eq:zeta-a} is now
