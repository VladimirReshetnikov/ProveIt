...
\hypersetup{
  ...
  pdfsubject={A proof-oriented monograph on q-shifted factorials, Gaussian coefficients, q-series, geometric interpolation, digital cancellation, and arithmetic},
  pdfkeywords={q-Pochhammer, q-binomial, Gaussian coefficient, basic hypergeometric series, q-series, geometric interpolation, Thue-Morse, Fabius function}
}
...
\Large Algebra, Combinatorics, Analysis, Arithmetic, Summation, and Dyadic Applications}
...
This monograph develops those themes from first principles.  It proves the algebraic laws of shifted factorials; the polynomial, reciprocal, and cyclotomic structure of Gaussian coefficients; finite and infinite $q$-binomial theorems; weighted generalizations through elementary and complete symmetric functions; combinatorial and finite-geometric models; unimodality by $\mathfrak{sl}_2$ representation theory; the $q$-Gauss, $q$-Chu--Vandermonde, and $q$-Pfaff--Saalsch\"utz summations; Jackson calculus and the $q$-gamma and $q$-beta functions; Jacobi's triple product, Euler's pentagonal theorem, and the Rogers--Ramanujan identities through Bailey pairs; cyclotomic factorization, $q$-Lucas and $q$-Babbage congruences; negative upper indices and $q$-Newton interpolation; geometric Gaussian finite differences and Lagrange interpolation on geometric nodes; digital Prouhet--Thue--Morse cancellation; the half-base $q=1/2$ coefficient-extraction mechanism behind exact dyadic Fabius formulae; and a recent family of central $q$-binomial/Lambert-series evaluations.  Every displayed theorem used in the main development is accompanied by a proof, and the final appendices collect formulas, limiting transitions, proof dependencies, and computational recipes.
...
The online source audit also included selected mathematically relevant \texttt{.tex} documents in the \texttt{Analysis/FabiusFunction/docs} tree of the ProveIt repository that could illuminate the present subject, especially the q-calculus article \texttt{Fabius\_q\_Special\_Functions.tex}, the main Fabius/Rvachev exposition, the mathematical glossary, and the non-formalized research-frontiers notes~\cite{ReshetnikovProveIt2026}.  Those sources motivated the independent development in \Cref{chap:dyadic-q}: its geometric interpolation theorem, Thue--Morse factorization, arbitrary-radix Prouhet filter, and dyadic coefficient formula are stated with explicit hypotheses and proved from the finite $q$-binomial theorem.  Online references and preprints were checked in their versions available on August 28, 2026.
...
\part{Geometric interpolation and digital cancellation}

\chapter{Geometric Gaussian differences, Thue--Morse blocks, and the Fabius half-base}\label{chap:dyadic-q}
\index{Fabius function}
\index{Thue--Morse sequence}
\index{geometric interpolation}
\index{finite difference!geometric}

The preceding chapters show that Gaussian coefficients arise from finite products,
weighted subsets, partitions, vector spaces, and basic hypergeometric series.  A
further appearance, central in the Fabius-function material of the accompanying
ProveIt repository~\cite{ReshetnikovProveIt2026}, comes from polynomial interpolation on a geometric grid.  Ordinary
finite differences use equally spaced nodes and ordinary binomial coefficients;
geometric finite differences use multiplicatively spaced nodes and Gaussian
coefficients.  Independently, the Thue--Morse signs create zeros of high order in
finite exponential sums.  When the two cancellations are combined, one obtains a
coefficient-extraction mechanism that explains why $q=1/2$ occurs in exact dyadic
formulae.

This chapter develops that mechanism in a general base and then specializes it to
$q=1/2$.  The algebraic identities are valid over a field or a formal power-series
ring and need no convergence assumptions.  Analytic hypotheses enter only when an
infinite product or an infinite Taylor series is evaluated.

\section{The arithmetic of the half-base}

We begin with two elementary specializations that will repeatedly normalize the
dyadic formulae.

\begin{proposition}[Mersenne normalization and base reciprocity]\label{prop:half-base-arithmetic}
For $n\ge0$ put
\[
 P_n=\left(\frac12;\frac12\right)_n
     =\prod_{j=1}^{n}(1-2^{-j}).
\]
Then
\begin{equation}
 P_n=2^{-\binom{n+1}{2}}\prod_{j=1}^{n}(2^j-1).\label{eq:half-mersenne}
\end{equation}
Moreover, for $0\le k\le n$,
\begin{equation}
 \Qbinom nk{1/2}=2^{-k(n-k)}\Qbinom nk2.\label{eq:half-two-reciprocity}
\end{equation}
In particular, $\Qbinom nk2$ is odd and
\[
...
\begin{proof}
For the first formula, write each factor as
\[
 1-2^{-j}=2^{-j}(2^j-1)
\]
and use $1+2+\cdots+n=\binom{n+1}{2}$.

For the second, apply reciprocity to the Gaussian polynomial:
\[
 \Qbinom nk{q^{-1}}=q^{-k(n-k)}\Qbinom nkq.
\]
Taking $q=2$ gives \eqref{eq:half-two-reciprocity}.  Finally,
$\Qbinom nkq$ has constant coefficient $1$, so its value at $q=2$ is
congruent to $1$ modulo $2$ and is therefore odd.  The valuation statement
follows immediately.
\end{proof}

When $2$ is viewed as the cardinality of $\mathbb F_2$, the integer
$\Qbinom nk2$ counts $k$-dimensional subspaces of $\mathbb F_2^n$.
Thus the rational number $\Qbinom nk{1/2}$ is a precise power-of-two
normalization of a finite-geometric count.  Likewise, $P_n$ is a
power-of-two normalization of a product of Mersenne numbers.  Both facts are
structural rather than mnemonic.

\section{Gaussian weights on a geometric grid}

Let $K$ be a field, let $q\in K^\times$, and fix $n\ge0$.  We assume
$q^j\ne1$ for $1\le j\le n$ whenever a denominator is used.  Define
\begin{equation}
 x_k=-q^{-k},
 \qquad
 W_{n,k}(q)=(-1)^kq^{\binom{k}{2}}\qbinom nk,
 \qquad 0\le k\le n.\label{eq:geometric-nodes-weights}
\end{equation}
The finite $q$-binomial theorem evaluates every power moment of this row.
...
\begin{equation}
 ...
 =(-1)^d(q^{-d};q)_n.\label{eq:geometric-moment-all}
\end{equation}
Consequently,
\begin{equation}
 \sum_{k=0}^{n}W_{n,k}(q)x_k^d=0
 \qquad(0\le d<n),\label{eq:geometric-moment-zero}
\end{equation}
and
\begin{equation}
 \sum_{k=0}^{n}W_{n,k}(q)x_k^n
 =q^{-\binom{n+1}{2}}(q;q)_n.\label{eq:geometric-moment-top}
\end{equation}
\end{theorem}

\begin{proof}
Since $x_k^d=(-1)^dq^{-kd}$, the left side of
\eqref{eq:geometric-moment-all} is
\[
 (-1)^d\sum_{k=0}^{n}(-1)^kq^{\binom{k}{2}}
 \qbinom nk(q^{-d})^k.
\]
The finite $q$-binomial theorem identifies the sum as $(q^{-d};q)_n$.
If $0\le d<n$, the factor with index $j=d$ in
\[
 (q^{-d};q)_n=\prod_{j=0}^{n-1}(1-q^{j-d})
\]
is zero, proving \eqref{eq:geometric-moment-zero}.  For $d=n$,
\begin{align*}
 (q^{-n};q)_n
 &=\prod_{j=0}^{n-1}(1-q^{j-n})\\
 &=(-1)^nq^{-\binom{n+1}{2}}\prod_{r=1}^{n}(1-q^r)\\
 &=(-1)^nq^{-\binom{n+1}{2}}(q;q)_n.
\end{align*}
Multiplication by the outer sign $(-1)^n$ gives
\eqref{eq:geometric-moment-top}.
\end{proof}

\begin{corollary}[Leading-coefficient functional]\label{cor:geometric-leading-coefficient}
If $p(x)$ has degree at most $n$, then
\begin{equation}
 [x^n]p(x)
 =\frac{q^{\binom{n+1}{2}}}{(q;q)_n}
  \sum_{k=0}^{n}(-1)^kq^{\binom{k}{2}}
  \qbinom nk p(-q^{-k}).\label{eq:geometric-leading-functional}
\end{equation}
\end{corollary}

\begin{proof}
Write $p(x)=\sum_{d=0}^{n}a_dx^d$ and apply the functional on the
right.  Every term with $d<n$ vanishes by
\eqref{eq:geometric-moment-zero}, while the term $d=n$ equals $a_n$ by
\eqref{eq:geometric-moment-top}.
\end{proof}

The same identity follows from Lagrange interpolation.  The next result makes
the equality of the two viewpoints explicit.

\begin{theorem}[Lagrange denominators on geometric nodes]\label{thm:geometric-lagrange-denominator}
For $x_k=-q^{-k}$,
\begin{equation}
 \prod_{\substack{0\le j\le n\\j\ne k}}(x_k-x_j)
 =(-1)^kq^{-\binom{n+1}{2}-\binom{k}{2}}
   (q;q)_k(q;q)_{n-k}.\label{eq:geometric-lagrange-denominator}
\end{equation}
Hence
\begin{equation}
 \frac1{\displaystyle\prod_{j\ne k}(x_k-x_j)}
 =\frac{q^{\binom{n+1}{2}}}{(q;q)_n}
  (-1)^kq^{\binom{k}{2}}\qbinom nk.\label{eq:lagrange-gaussian-weight}
\end{equation}
\end{theorem}
...
For $j<k$,
\[
 x_k-x_j=q^{-k}(q^{k-j}-1)=-q^{-k}(1-q^{k-j}),
\]
so
\[
 \prod_{j=0}^{k-1}(x_k-x_j)
 =(-1)^kq^{-k^2}(q;q)_k.
\]
For $j>k$,
\[
...
and therefore
\[
 \prod_{j=k+1}^{n}(x_k-x_j)
 =q^{-\sum_{j=k+1}^{n}j}(q;q)_{n-k}.
\]
Because
\[
 k^2+\sum_{j=k+1}^{n}j
 =\binom{n+1}{2}+\binom{k}{2},
\]
multiplication gives \eqref{eq:geometric-lagrange-denominator}.
Taking the reciprocal and using the factorial definition of $\qbinom nk$
gives \eqref{eq:lagrange-gaussian-weight}.
\end{proof}

\begin{remark}[Uniqueness]
There is only one linear functional on values at the $n+1$ distinct nodes
$x_0,\ldots,x_n$ that returns the leading coefficient of every polynomial of
degree at most $n$.  Thus the Gaussian row in
\eqref{eq:geometric-leading-functional} is forced by interpolation; it is not
one convenient choice among many.
\end{remark}

\begin{corollary}[Vandermonde determinant]\label{cor:geometric-vandermonde}
For $x_i=-q^{-i}$,
\begin{equation}
 \det(x_i^j)_{0\le i,j\le n}
 =(-1)^{\binom{n+1}{2}}
 q^{-\sum_{j=1}^{n}j^2}
 \prod_{d=1}^{n}(1-q^d)^{n+1-d}.\label{eq:geometric-vandermonde}
\end{equation}
\end{corollary}

\begin{proof}
The Vandermonde determinant is $\prod_{0\le i<j\le n}(x_j-x_i)$.
For $i<j$,
\[
 x_j-x_i=-q^{-j}(1-q^{j-i}).
\]
There are $\binom{n+1}{2}$ pairs, the exponent of $q^{-j}$ summed over
all pairs is $\sum_{j=1}^{n}j^2$, and a fixed gap $d=j-i$ occurs
$n+1-d$ times.  Multiplying the factors proves the formula.
\end{proof}

\section{The complete head--tail identity}

The leading-coefficient formula is the degree-$n$ part of a stronger exact
identity.  It quantifies, without approximation, the contribution of every
Taylor coefficient above degree $n$.

\begin{theorem}[Complete Gaussian residual]\label{thm:complete-gaussian-residual}
Let $f(x)=\sum_{m\ge0}a_mx^m$ be a polynomial, or a formal power series for
which the expressions below are interpreted coefficientwise.  Define
\begin{equation}
 \mathscr D_{n,q}f
 =\frac{q^{\binom{n+1}{2}}}{(q;q)_n}
  \sum_{k=0}^{n}W_{n,k}(q)f(-q^{-k}).\label{eq:normalized-geometric-difference}
\end{equation}
Then
\begin{equation}
 \boxed{\displaystyle
 \mathscr D_{n,q}f
 =\sum_{r\ge0}(-1)^rq^{-nr}\Qbinom{n+r}{n}q\,a_{n+r}.}\label{eq:complete-gaussian-residual}
\end{equation}
In particular, $\mathscr D_{n,q}f=a_n$ whenever $\deg f\le n$.
\end{theorem}

\begin{proof}
By linearity and \Cref{thm:geometric-moment-transform},
\[
 \mathscr D_{n,q}f
 =\frac{q^{\binom{n+1}{2}}}{(q;q)_n}
  \sum_{m\ge0}a_m(-1)^m(q^{-m};q)_n.
\]
The terms $m<n$ vanish.  Put $m=n+r$.  Reversing the factors gives
\begin{align*}
 (q^{-n-r};q)_n
 &=(-1)^nq^{-n(n+r)+\binom n2}(q^{r+1};q)_n.
\end{align*}
Therefore the sign becomes $(-1)^r$, the total power of $q$ becomes
$q^{-nr}$, and
\[
 ...
 =\Qbinom{n+r}{n}q.
\]
This proves \eqref{eq:complete-gaussian-residual}.
\end{proof}

\begin{corollary}[An analytic tail bound]\label{cor:geometric-tail-bound}
Assume $0<q<1$ and that $f(z)=\sum_{m\ge0}a_mz^m$ is analytic on
$\abs z\le R$, where $R>q^{-n}$.  Put
$M_R=\max_{\abs z=R}\abs{f(z)}$.  Then
\begin{equation}
 \abs{\mathscr D_{n,q}f-a_n}
 \le
 \frac{M_R}{(q;q)_nR^n}
 \frac{q^{-n}/R}{1-q^{-n}/R}.\label{eq:geometric-tail-bound}
\end{equation}
\end{corollary}
...
\[
 ...
 \le\frac1{(q;q)_n}.
\]
Cauchy's estimate gives $\abs{a_{n+r}}\le M_RR^{-n-r}$.  Apply these
bounds to the terms with $r\ge1$ in
\eqref{eq:complete-gaussian-residual} and sum the resulting geometric
series.
\end{proof}

\section{Integer bases and the dyadic stencil}

Set $q=b^{-1}$, where $b>1$.  Then the nodes are $-b^k$, and the top
moment becomes an integer product.

\begin{corollary}[Geometric difference in base $b$]\label{cor:base-b-stencil}
Let $b\ne0,1$ and assume $b^j\ne1$ for $1\le j\le n$.  Put
\[
 w_{n,k}^{(b)}=(-1)^kb^{-\binom{k}{2}}\Qbinom nk{1/b},
 \qquad x_k=-b^k.
\]
Then
\begin{equation}
 \sum_{k=0}^{n}w_{n,k}^{(b)}x_k^d=0\quad(0\le d<n),\label{eq:base-b-zero}
\end{equation}
and
\begin{equation}
 \sum_{k=0}^{n}w_{n,k}^{(b)}x_k^n
 =\prod_{j=1}^{n}(b^j-1).\label{eq:base-b-top}
\end{equation}
Consequently, for $\deg p\le n$,
\begin{equation}
 [x^n]p(x)
 =\frac{1}{\prod_{j=1}^{n}(b^j-1)}
  \sum_{k=0}^{n}w_{n,k}^{(b)}p(-b^k).\label{eq:base-b-leading}
\end{equation}
\end{corollary}

\begin{proof}
Substitute $q=b^{-1}$ in
\Cref{thm:geometric-moment-transform,cor:geometric-leading-coefficient}.
The top moment simplifies as
\[
 q^{-\binom{n+1}{2}}(q;q)_n
 =b^{\binom{n+1}{2}}\prod_{j=1}^{n}(1-b^{-j})
 =\prod_{j=1}^{n}(b^j-1).
\]
\end{proof}

The dyadic case $b=2$ is particularly clean.  Define
\begin{equation}
 w_{n,k}=(-1)^k2^{-\binom{k}{2}}\Qbinom nk{1/2},
 \qquad
 C_n=\prod_{j=1}^{n}(2^j-1)
     =2^{\binom{n+1}{2}}P_n.\label{eq:dyadic-weights-Cn}
\end{equation}
Then
\begin{equation}
 \sum_{k=0}^{n}w_{n,k}(-2^k)^d
 =\begin{cases}
 0,&0\le d<n,\\
 C_n,&d=n.
 \end{cases}\label{eq:dyadic-moment-stencil}
\end{equation}
Thus $C_n^{-1}\sum_kw_{n,k}p(-2^k)$ is the leading coefficient of
$p$.  This is the exact geometric analogue of
$\Delta^np(0)/n!$ on an equally spaced grid.

\section{Thue--Morse factorization and Prouhet cancellation}

Let $s_2(r)$ denote the number of $1$'s in the binary expansion of $r$, and
put
\[
 \eps_r=(-1)^{s_2(r)}.
\]
The relations $\eps_{2r}=\eps_r$ and $\eps_{2r+1}=-\eps_r$ encode the
entire sequence.  For the broader theory of automatic sequences and the
Prouhet--Thue--Morse construction, see Allouche and Shallit~\cite{AlloucheShallit2003}.

\begin{theorem}[Finite Thue--Morse product]\label{thm:finite-thue-morse-product}
For every $k\ge0$,
\begin{equation}
 \sum_{r=0}^{2^k-1}\eps_rz^r
 =\prod_{j=0}^{k-1}(1-z^{2^j}).\label{eq:finite-thue-morse-product}
\end{equation}
Consequently, for every $\tau\in\C$,
\begin{equation}
 \sum_{r=0}^{2^k-1}\eps_r\e^{(r+\tau)z}
 =\e^{\tau z}\prod_{j=0}^{k-1}(1-\e^{2^jz}).\label{eq:thue-morse-exponential-product}
\end{equation}
\end{theorem}

\begin{proof}
For $k=0$, both sides of \eqref{eq:finite-thue-morse-product} equal
$1$.  Split the sum for $k+1$ into numbers whose highest binary digit is
$0$ and $1$.  Adding $2^k$ changes the parity of the binary digit sum,
so
\begin{align*}
 \sum_{r=0}^{2^{k+1}-1}\eps_rz^r
 &=\sum_{r=0}^{2^k-1}\eps_rz^r
   -z^{2^k}\sum_{r=0}^{2^k-1}\eps_rz^r\\
 &=(1-z^{2^k})\prod_{j=0}^{k-1}(1-z^{2^j}).
\end{align*}
This proves the polynomial identity by induction.  Substitute
$z\mapsto\e^z$ and multiply by $\e^{\tau z}$ to obtain
\eqref{eq:thue-morse-exponential-product}.
\end{proof}

\begin{corollary}[Translated Prouhet identities]\label{cor:translated-prouhet}
For $m\ge0$ and $\tau\in\C$, define
\[
 T_{k,m}(\tau)=\sum_{r=0}^{2^k-1}\eps_r(r+\tau)^m.
\]
Then
\begin{equation}
 T_{k,m}(\tau)=0\qquad(0\le m<k),\label{eq:prouhet-lower}
\end{equation}
and
\begin{equation}
 T_{k,k}(\tau)=(-1)^kk!\,2^{\binom{k}{2}}.\label{eq:prouhet-top}
\end{equation}
The top value is independent of $\tau$.
\end{corollary}

\begin{proof}
The left side of \eqref{eq:thue-morse-exponential-product} is the
exponential generating function
\[
 \sum_{m\ge0}T_{k,m}(\tau)\frac{z^m}{m!}.
\]
Every factor on the right has expansion
\[
 1-\e^{2^jz}=-2^jz+O(z^2).
\]
Hence the product has a zero of exact order $k$, with leading term
\[
 (-1)^k2^{0+1+\cdots+(k-1)}z^k
 =(-1)^k2^{\binom{k}{2}}z^k.
\]
The factor $\e^{\tau z}=1+O(z)$ does not change this leading term.
Comparing coefficients proves both statements.
\end{proof}

\begin{corollary}[Normalized entire block]\label{cor:normalized-thue-morse-block}
The apparent singularity at $z=0$ in
\begin{equation}
 \widehat\Theta_k(z;\tau)
 =\frac{(-1)^k}{2^{\binom{k}{2}}z^k}
  \sum_{r=0}^{2^k-1}\eps_r\e^{(r+\tau)z}\label{eq:normalized-thue-morse-block}
\end{equation}
is removable, and $\widehat\Theta_k(0;\tau)=1$.  Moreover,
\begin{equation}
 [z^m]\widehat\Theta_k(z;\tau)
 =\frac{(-1)^k}{2^{\binom{k}{2}}}
  \frac{T_{k,m+k}(\tau)}{(m+k)!}.\label{eq:normalized-TM-coeff}
\end{equation}
\end{corollary}

\begin{proof}
By \Cref{cor:translated-prouhet}, the numerator vanishes to exact order
$k$ and its coefficient of $z^k$ is
$(-1)^k2^{\binom{k}{2}}$.  Division gives an entire function with
constant term $1$.  Shifting the index in the exponential generating
series gives \eqref{eq:normalized-TM-coeff}.
\end{proof}

\section{Digital cancellation in an arbitrary radix}

The binary signs are one member of a broader roots-of-unity construction.
This extension separates the role of the geometric base from the special
real-valued signs available when $b=2$.

\begin{theorem}[Radix-$b$ Prouhet filter]\label{thm:radix-b-prouhet}
Let $b\ge2$, let $\omega$ be a primitive $b$th root of unity, and let
$s_b(r)$ be the sum of the base-$b$ digits of $r$.  Put
$\eps_r^{(b)}=\omega^{s_b(r)}$.  Then
\begin{equation}
 \sum_{r=0}^{b^k-1}\eps_r^{(b)}\e^{rz}
 =\prod_{j=0}^{k-1}
   \left(\sum_{a=0}^{b-1}\omega^a\e^{ab^jz}\right).\label{eq:radix-b-product}
\end{equation}
For every $\tau\in\C$,
\begin{equation}
 \sum_{r=0}^{b^k-1}\eps_r^{(b)}(r+\tau)^m=0
 \qquad(0\le m<k),\label{eq:radix-b-lower}
\end{equation}
and
\begin{equation}
 \sum_{r=0}^{b^k-1}\eps_r^{(b)}(r+\tau)^k
 =k!\frac{b^{k(k+1)/2}}{(\omega-1)^k}.\label{eq:radix-b-top}
\end{equation}
\end{theorem}

\begin{proof}
Write each $r<b^k$ uniquely as
$r=a_0+a_1b+\cdots+a_{k-1}b^{k-1}$ with $0\le a_j<b$.  Then
\[
 \omega^{s_b(r)}\e^{rz}
 =\prod_{j=0}^{k-1}\omega^{a_j}\e^{a_jb^jz}.
\]
Summing independently over the digits proves
\eqref{eq:radix-b-product}.

Each factor on the right vanishes at $z=0$ because
$\sum_{a=0}^{b-1}\omega^a=0$.  Its derivative there is
\[
 b^j\sum_{a=0}^{b-1}a\omega^a.
\]
Differentiating the finite geometric sum, or summing it directly, gives
\[
 \sum_{a=0}^{b-1}a\omega^a=\frac{b}{\omega-1}.
\]
Thus the product has a zero of exact order $k$, and the coefficient of
$z^k$ is
\[
 \prod_{j=0}^{k-1}\frac{b^{j+1}}{\omega-1}
 =\frac{b^{k(k+1)/2}}{(\omega-1)^k}.
\]
Multiplication by $\e^{\tau z}$ leaves this leading coefficient
unchanged.  Comparing derivatives at zero gives
\eqref{eq:radix-b-lower} and \eqref{eq:radix-b-top}.
\end{proof}

For $b=2$ and $\omega=-1$, the top value in
\eqref{eq:radix-b-top} becomes
$(-1)^kk!2^{\binom{k}{2}}$, exactly
\eqref{eq:prouhet-top}.  When $b$ is a prime power, the same base also has
a finite-field interpretation through $\Qbinom nkb$; thus digital
cancellation, geometric interpolation, and subspace enumeration meet at one
parameter.

\section{A general coefficient-transfer theorem}

The next theorem isolates the double-cancellation principle used in the
dyadic Fabius formulae.  One cancellation takes place in the geometric node
variable; the other transfers it recursively to coefficients of a family of
power series.

\begin{theorem}[Gaussian coefficient transfer]\label{thm:gaussian-coefficient-transfer}
Let
\[
 A(z)=\sum_{m\ge0}a_mz^m,\qquad a_0=1,
\]
and, for $0\le k\le n$, let
$\Psi_k(z)=\sum_{m\ge0}c_{k,m}z^m$.  Suppose that
\begin{equation}
 \Psi_k(z)A(-z)=\e^{-z}A(-q^{-k}z).\label{eq:general-bridge}
\end{equation}
With the weights $W_{n,k}(q)$ from
\eqref{eq:geometric-nodes-weights}, one has
\begin{equation}
...
\begin{proof}
Take the coefficient of $z^d$ in \eqref{eq:general-bridge}.  The left
side is
\[
...
\[
 \sum_{j=0}^{d}\frac{(-1)^j}{j!}a_{d-j}(-q^{-k})^{d-j}.\label{eq:bridge-coeff}
\]
Multiply by $W_{n,k}(q)$ and sum over $k$.

Suppose first that $d<n$.  Every node power on the right side of
\eqref{eq:bridge-coeff} has degree $d-j<n$, so it vanishes by
\eqref{eq:geometric-moment-zero}.  We prove
\eqref{eq:transfer-lower-zero} by induction on $d$.  After summation, all
left-side terms with $i<d$ vanish by the induction hypothesis.  The
remaining term is
\[
 a_0\sum_{k=0}^{n}W_{n,k}(q)c_{k,d}
 =\sum_{k=0}^{n}W_{n,k}(q)c_{k,d},
\]
which must therefore be zero.

Now set $d=n$.  On the right of \eqref{eq:bridge-coeff}, every term with
$j>0$ contains a node power of degree $n-j<n$ and vanishes.  The term
$j=0$ is
\[
 a_n\sum_{k=0}^{n}W_{n,k}(q)(-q^{-k})^n
 =q^{-\binom{n+1}{2}}(q;q)_na_n
\]
by \eqref{eq:geometric-moment-top}.  On the left, all terms with $i<n$
vanish by \eqref{eq:transfer-lower-zero}; the remaining term is the left
side of \eqref{eq:transfer-top}.  This proves the theorem.
\end{proof}

\begin{corollary}[Common-translation invariance]\label{cor:transfer-translation}
Under the hypotheses of \Cref{thm:gaussian-coefficient-transfer}, let
$\lambda\in\C$ and put
\[
 \widetilde\Psi_k(z)=\e^{\lambda z}\Psi_k(z).
\]
Then
\begin{equation}
 \sum_{k=0}^{n}W_{n,k}(q)[z^n]\widetilde\Psi_k(z)
 =\sum_{k=0}^{n}W_{n,k}(q)c_{k,n}.\label{eq:transfer-translation-invariance}
\end{equation}
\end{corollary}
...
\[
 ...
 =\sum_{j=0}^{n}\frac{\lambda^j}{j!}c_{k,n-j}.
\]
After weighting and summing over $k$, every term with $j>0$ vanishes by
\eqref{eq:transfer-lower-zero}; only $j=0$ remains.
\end{proof}

\begin{remark}[Triangular cancellation]
The proof contains two nested triangular systems.  First, the finite
$q$-binomial theorem annihilates every geometric-node power below degree
$n$.  Second, the coefficient identity transfers those vanishing moments
successively to $c_{k,0},c_{k,1},\ldots,c_{k,n-1}$.  The first surviving
term is the desired coefficient $a_n$.  This explains the ``double
cancellation'' terminology used in the Fabius-function development.
\end{remark}

\section{The half-moment dilation series}

We now construct the formal series that appears in the dyadic Fabius and
Rvachev analysis.  Let
\[
 \Phi(z)=\frac{\e^z-1}{z},\qquad \Phi(0)=1.
\]

\begin{theorem}[The dyadic dilation series]\label{thm:dyadic-dilation-series}
There is a unique formal power series
\[
 A(z)=\sum_{n\ge0}a_nz^n,\qquad a_0=1,
\]
satisfying
\begin{equation}
 A(2z)=\Phi(z)A(z).\label{eq:A-dilation}
\end{equation}
Its coefficients obey
\begin{equation}
 (2^n-1)a_n
 =\sum_{j=0}^{n-1}\frac{a_j}{(n-j+1)!}
 \qquad(n\ge1).\label{eq:A-coefficient-recurrence}
\end{equation}
Moreover,
\begin{equation}
 A(z)=\prod_{j=1}^{\infty}\Phi\!\left(\frac{z}{2^j}\right),\label{eq:A-infinite-product-dyadic}
\end{equation}
and the product converges locally uniformly, so $A$ is entire.
\end{theorem}

\begin{proof}
Since
\[
 \Phi(z)=\sum_{m\ge0}\frac{z^m}{(m+1)!},
\]
comparison of the coefficient of $z^n$ in \eqref{eq:A-dilation} gives
\[
 2^na_n=\sum_{j=0}^{n}\frac{a_j}{(n-j+1)!}.
\]
The term $j=n$ is $a_n$, and moving it to the left yields
\eqref{eq:A-coefficient-recurrence}.  Because $2^n-1\ne0$ for $n\ge1$,
this recurrence uniquely determines every coefficient from the earlier
ones, proving formal existence and uniqueness.

For the analytic product, $\Phi(w)=1+O(w)$ locally uniformly near $0$.
On every compact set $\abs z\le R$ there is a constant $C_R$ such that
\[
 \abs{\Phi(z/2^j)-1}\le C_R2^{-j}
\]
for all sufficiently large $j$.  The Weierstrass criterion therefore gives
local uniform convergence of the product to an entire function with value
$1$ at the origin.  Shifting the index shows
\begin{align*}
 A(2z)
 &=\prod_{j=1}^{\infty}\Phi\!\left(\frac{2z}{2^j}\right)
 =\Phi(z)\prod_{j=1}^{\infty}\Phi\!\left(\frac{z}{2^j}\right)
 =\Phi(z)A(z).
\end{align*}
By formal uniqueness, this entire product has the coefficients determined
above.
\end{proof}
...
\[
 ...
 \quad a_5=\frac{19}{32400}.
\]
In the Fabius/Rvachev setting these are normalized half moments.

\section{The exact dyadic coefficient formula}

Iterating \eqref{eq:A-dilation} at negative arguments gives the bridge
required by \Cref{thm:gaussian-coefficient-transfer}.

\begin{lemma}[Thue--Morse/dilation bridge]\label{lem:TM-dilation-bridge}
For $k\ge0$, define
\begin{equation}
 \Psi_k(z)=\e^{-z}\prod_{j=0}^{k-1}\Phi(-2^jz).\label{eq:Psi-k-definition}
\end{equation}
Then
\begin{equation}
 \Psi_k(z)A(-z)=\e^{-z}A(-2^kz).\label{eq:TM-A-bridge}
\end{equation}
Furthermore,
\begin{equation}
 \Psi_k(z)
 =\frac{1}{2^{\binom{k}{2}}z^k}
  \sum_{r=0}^{2^k-1}\eps_r\e^{-(r+1)z}.\label{eq:Psi-TM-expansion}
\end{equation}
The apparent singularity in \eqref{eq:Psi-TM-expansion} is removable.
\end{lemma}

\begin{proof}
Iterating \eqref{eq:A-dilation} gives
\[
 A(-2^kz)=A(-z)\prod_{j=0}^{k-1}\Phi(-2^jz).
\]
Multiplication by $\e^{-z}$ proves \eqref{eq:TM-A-bridge}.  Also,
\[
 \Phi(-2^jz)=\frac{1-\e^{-2^jz}}{2^jz}.
\]
Therefore
\[
 \Psi_k(z)
 =\frac{\e^{-z}}{2^{\binom{k}{2}}z^k}
   \prod_{j=0}^{k-1}(1-\e^{-2^jz}).
\]
Apply \eqref{eq:finite-thue-morse-product} with the variable
$\e^{-z}$ to obtain \eqref{eq:Psi-TM-expansion}.  The product form shows
that the numerator has a zero of order $k$, so the singularity is
removable.
\end{proof}

\begin{theorem}[Dyadic Gaussian--Thue--Morse coefficient formula]\label{thm:dyadic-Gaussian-TM-formula}
Let $a_n$ be determined by \eqref{eq:A-dilation}, and let
$\tau\in\C$ be arbitrary.  Then, for every $n\ge0$,
\begin{equation}
 \boxed{\displaystyle
 a_n=\frac{(-1)^n}{C_n}
 \sum_{k=0}^{n}
 \frac{\Qbinom nk{1/2}}
      {4^{\binom{k}{2}}(n+k)!}
 \sum_{r=0}^{2^k-1}(-1)^{s_2(r)}(r+\tau)^{n+k},}\label{eq:dyadic-Gaussian-TM-formula}
\end{equation}
where
\[
 C_n=\prod_{j=1}^{n}(2^j-1)
     =2^{\binom{n+1}{2}}\left(\frac12;\frac12\right)_n.
\]
Although every inner power sum depends on $\tau$, the complete expression is
independent of $\tau$.
\end{theorem}

\begin{proof}
Write $\Psi_k(z)=\sum_{m\ge0}c_{k,m}z^m$.  From
\eqref{eq:Psi-TM-expansion},
\begin{equation}
 c_{k,m}
 =\frac{(-1)^{m+k}}{2^{\binom{k}{2}}(m+k)!}
  \sum_{r=0}^{2^k-1}\eps_r(r+1)^{m+k}.\label{eq:ckm-TM}
\end{equation}
Apply \Cref{thm:gaussian-coefficient-transfer} with $q=1/2$.  Its top
identity becomes
\[
 \sum_{k=0}^{n}(-1)^k2^{-\binom{k}{2}}
 \Qbinom nk{1/2}c_{k,n}=C_na_n.
\]
Substitution of \eqref{eq:ckm-TM} gives
\eqref{eq:dyadic-Gaussian-TM-formula} with $\tau=1$: the two $k$-dependent
signs cancel, leaving the common sign $(-1)^n$, and the two triangular
powers of $2$ combine to $4^{\binom{k}{2}}$.

For a general $\tau$, multiply every $\Psi_k$ by
$\e^{(1-\tau)z}$.  By \Cref{cor:transfer-translation}, the weighted
coefficient of degree $n$ is unchanged.  On the other hand,
\[
 \e^{(1-\tau)z}\Psi_k(z)
 =\frac{1}{2^{\binom{k}{2}}z^k}
   \sum_{r=0}^{2^k-1}\eps_r\e^{-(r+\tau)z},
\]
so the same coefficient calculation replaces $r+1$ by $r+\tau$.
This proves the formula and its translation invariance.
\end{proof}

\begin{example}[The first nontrivial cases]
For $n=1$, choose $\tau=1$.  The two inner sums are $1$ and
$1^2-2^2=-3$, so
\[
 a_1=-\left(1-\frac32\right)=\frac12.
\]
For $n=2$, the three Gaussian coefficients at $1/2$ are
$1,3/2,1$; substitution in \eqref{eq:dyadic-Gaussian-TM-formula} gives
$a_2=5/36$.  These agree with the recurrence
\eqref{eq:A-coefficient-recurrence}.
\end{example}

\begin{corollary}[Inverse-dyadic Fabius normalization]\label{cor:Fabius-inverse-dyadic-q}
Suppose a sequence $F(2^{-n})$ is normalized by
\begin{equation}
 F(2^{-n})=2^{-\binom n2}a_n.\label{eq:Fabius-half-moment-normalization}
\end{equation}
This is the half-moment normalization of the bounded Fabius function in the
repository sources.  Then, for every $\tau\in\C$,
\begin{equation}
 \boxed{\displaystyle
 F(2^{-n})
 =\frac{(-1)^n}
 {2^{n^2}\left(\frac12;\frac12\right)_n}
 \sum_{k=0}^{n}
 \frac{\Qbinom nk{1/2}}
      {4^{\binom{k}{2}}(n+k)!}
 \sum_{r=0}^{2^k-1}(-1)^{s_2(r)}(r+\tau)^{n+k}.}\label{eq:Fabius-inverse-dyadic-q}
\end{equation}
\end{corollary}

\begin{proof}
Multiply \eqref{eq:dyadic-Gaussian-TM-formula} by
$2^{-\binom n2}$.  The denominator becomes
\begin{align*}
 2^{\binom n2}C_n
 &=2^{\binom n2+\binom{n+1}{2}}
   \left(\frac12;\frac12\right)_n\\
 &=2^{n^2}\left(\frac12;\frac12\right)_n.
\end{align*}
This is exactly \eqref{eq:Fabius-inverse-dyadic-q}.
\end{proof}

\begin{remark}[What the formula explains]
The factors in \eqref{eq:Fabius-inverse-dyadic-q} have distinct origins.
The Gaussian coefficient and $2^{-\binom{k}{2}}$ form the geometric
finite-difference stencil.  A second factor $2^{-\binom{k}{2}}$ comes from
removing the order-$k$ Thue--Morse zero.  The product
$(1/2;1/2)_n$ is the normalized geometric Vandermonde denominator.  Finally,
translation invariance is not a lucky power-sum identity: it follows because
the Gaussian stencil has already killed every lower coefficient.
\end{remark}

\section{Algebraic, analytic, and formal layers}

The mechanism above divides naturally into three layers.
\begin{enumerate}
\item The finite $q$-binomial theorem, the moment transform
\eqref{eq:geometric-moment-all}, Lagrange interpolation, and all finite
Thue--Morse identities are polynomial or formal identities.
\item The coefficient-transfer theorem is an identity in $\C[[z]]$.  It
uses only coefficient comparison and the triangular vanishing moments.
\item The product \eqref{eq:A-infinite-product-dyadic} and any evaluation of
an infinite Taylor series require local uniform or absolute convergence.
\end{enumerate}
This separation is useful both mathematically and in formal proof systems.
It prevents analytic convergence hypotheses from obscuring finite algebra,
while also preventing a formal infinite product from being used as though it
were automatically an analytic function.

The same architecture appears throughout $q$-series: first prove a finite
identity in a polynomial ring, then pass to formal series, and only afterward
supply the estimates needed for analytic convergence.  In the present
dyadic application, that architecture makes the occurrence of
$q$-Pochhammer symbols and Gaussian coefficients completely transparent.
...
Geometric moment transform & $\sum_k(-1)^kq^{\binom k2}\qbinom nk(-q^{-k})^d=(-1)^d(q^{-d};q)_n$ & \Cref{thm:geometric-moment-transform}\\[1pt]
Geometric leading coefficient & Weighted values on $-1,-q^{-1},\ldots,-q^{-n}$ recover $[x^n]p(x)$ & \Cref{cor:geometric-leading-coefficient}\\[1pt]
Thue--Morse product & $\sum_{r<2^k}(-1)^{s_2(r)}z^r=\prod_{j<k}(1-z^{2^j})$ & \Cref{thm:finite-thue-morse-product}\\[1pt]
Dyadic coefficient extraction & Gaussian--Thue--Morse formula for the Taylor coefficient $a_n$ & \Cref{thm:dyadic-Gaussian-TM-formula}\\[1pt]
...
\section*{Route to geometric interpolation and digital cancellation}
\[
\begin{gathered}
 \text{finite }q\text{-binomial theorem}
 \longrightarrow \text{geometric Gaussian moments}
 \longrightarrow \text{Lagrange extraction}\\
 \longrightarrow \text{Prouhet--Thue--Morse zeros}
 \longrightarrow \text{dyadic Fabius coefficients}.
\end{gathered}
\]
Read \Cref{chap:finite-qbinomial,chap:qnewton,chap:dyadic-q}.
...
GitHub repository, especially the q-calculus, mathematical-glossary, and Fabius/Rvachev expositions under
\texttt{Analysis/FabiusFunction/docs},
\url{https://github.com/VladimirReshetnikov/ProveIt}, accessed August 28, 2026.
