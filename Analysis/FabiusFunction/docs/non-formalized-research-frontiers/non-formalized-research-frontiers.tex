...
approximants.  Two reusable discrete engines follow: weighted path sums for triangular
...
arbitrary-family Toeplitz wording, and generic Gaussian coefficient bounds.  The inverse
...
asymptotic synthesis.  Independence of the coordinates gives, for \(s>0\),
\begin{equation}
 G(-s)=M_0(s)=\E[\e^{-sX}]
 =\prod_{j\ge1}\frac{1-\e^{-s/2^j}}{s/2^j}.
 \label{integration:eq:probability-product-handoff}
\end{equation}
Thus the exact negative-Laplace product decomposed in \cref{part:dyadicasym} is the
Laplace transform of the same product-space law, rather than a second independent model.
...
This final part retains the compact symbols of the inverse/saddle source.  They are local to
this part: in particular, its \(d_n\) is a bounded saddle jet and is not the half-moment
sequence \(d_n=\E X^n\) used in the probability and dyadic parts.  The exact correspondence
is
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
...
\end{tabular}
\end{center}
When numerical work later returns to half moments, it writes them as \(h_n\) to keep the
two roles visibly separate.
...
\begin{enumerate}
\item package the ordered-composition formula \eqref{inverse:eq:mass-composition} and the
  corresponding logarithmic composition formula as exact general Lean declarations;
\item formalize the transfer of the mass top-jet coefficient through the logarithm and the
  resulting highest-\(\Psi\)-derivative coefficient at every order;
...
\(F(2^{-n})\).  The deduction combining those facts is not yet a named theorem.
\begin{candidate}
For every \(n\ge1\),
\[
...
\end{candidate}
\begin{obligation}
Formalize the bridge from denominator divisibility and rational two-adic valuation to the
natural valuation of \(\Delta_n\), and expose the displayed inequality with its exact
\(n\ge1\) hypothesis.  The relevant formal inputs are \lean{proposition_fifteen},
\lean{dyadicDenominator_eq_fabiusDyadicDenominator}, and
...
and the signed global extension, but only an already decoded exponent--integer-numerator
evaluator for \(\Up\).
\begin{candidate}
...
\end{candidate}
\begin{obligation}
Define the wrapper and prove both an \texttt{eq\_none\_iff} recognition theorem and a
successful-evaluation theorem identifying the result with analytic \(\Up\).  Reuse
\lean{dyadicExponent?_exists_iff}, \lean{rvachevDyadic}, and
\lean{rvachevDyadic_cast_global}; the existing wrappers \lean{evalFabiusDyadic} and
\lean{evalExtendedFabiusDyadic} are the intended interface model.
\end{obligation}
...
\end{candidate}
\begin{obligation}
Package the two displayed certified inequalities and their direct factor-two comparison.
No claim about the ratio of the actual pointwise errors or about optimal uniform errors is
intended.  The sandwich theorems
\lean{uniformPartialCDF_sandwich} and \lean{uniformCenteredPartialCDF_sandwich} prove the
threshold displacements, but do not yet state these Lipschitz corollaries.
\end{obligation}
...
\end{candidate}
\begin{obligation}
Export the eventual-zero or finite-support statement used internally by the signed-extension
development.  The public summability theorem and the public single-block evaluation identify
the value of the series, but neither has local finiteness as its conclusion.
\end{obligation}
...
\section{Public packaging of convergent series}

\subsection{The binary-series right endpoint}

Let \(c_m(x)\) and \(B_m(x)\) denote the coefficient and summand of the global binary
reduction series.
\begin{candidate}
At \(x=1\), \(B_0(1)=1\), while for every \(m\ge1\),
\[
 c_m(1)=0,\qquad B_m(1)=0.
\]
Thus the binary-reduction series at the right endpoint is supported only at scale zero.
\end{candidate}
\begin{obligation}
Expose the positive-scale endpoint vanishing as public theorems.  Prove the corresponding
binary-prefix identity, use
\lean{globalBinaryReductionCoefficient_eq_zero_of_mod_two_eq_zero}, and unfold the summand.
The existing theorem \lean{globalBinaryReductionSummand_one_zero} states only the scale-zero
case.
\end{obligation}

\subsection{Absolute convergence of the parity-power presentation}

Let \(\Lambda_m(x)\) be the corrected parity-power summand, which is proved pointwise equal
to the corresponding global binary-reduction summand.
\begin{candidate}
For every \(x\ge0\),
\[
 \sum_{m=0}^{\infty}|\Lambda_m(x)|<\infty.
\]
Consequently the corrected parity-power expansion converges absolutely to \(\mathcal F(x)\).
\end{candidate}
\begin{obligation}
Expose the norm-summability corollary by rewriting termwise with
\lean{fabiusParityPowerSummand_eq_globalBinaryReductionSummand} and applying
\lean{summable_norm_globalBinaryReductionSummand}.  The currently public \texttt{HasSum} and
\texttt{tsum} theorems do not themselves state absolute convergence of the parity-power
presentation.
\end{obligation}
...
Binary-series endpoint inputs &
\lean{globalBinaryReductionCoefficient_eq_zero_of_mod_two_eq_zero} and
\lean{globalBinaryReductionSummand_one_zero} in \path{FabiusBinaryReductionSeries.lean}\\
Parity-power term identity and neighboring norm summability &
\lean{fabiusParityPowerSummand_eq_globalBinaryReductionSummand} and
\lean{summable_norm_globalBinaryReductionSummand} in
\path{FabiusParityPowerSeries.lean} and \path{FabiusBinaryReductionSeries.lean}\\
\bottomrule
\end{tabularx}
\endgroup
