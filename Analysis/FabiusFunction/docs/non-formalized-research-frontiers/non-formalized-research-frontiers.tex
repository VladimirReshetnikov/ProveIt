...
The merge is intentionally conservative.  Eleven source notebooks are represented in seven
core or synthesized parts, followed by an eighth, post-audit gap register.  Distinct proof
routes, sharper variants, useful warnings, tables, algorithms, plots, and open problems remain
visible, while genuinely redundant repetitions are compressed and cross-referenced.
Standalone preambles, title pages, local tables of contents, and conflicting labels have
been normalized.  Editorial corrections made during consolidation are stated locally rather
than silently changing the mathematical record.
\end{abstract}
...
The first seven parts below either preserve a source dossier or synthesize overlapping
dossiers; the eighth records claims removed by the primary exposition's exact-counterpart
audit.  Source-path banners and SHA-256 comments preserve all eleven notebook provenances and
the register provenance.  Cross-reference and bibliography keys retain dossier prefixes so
that formerly standalone material can coexist.  Repeated material is retained when it
supplies an independent derivation, a stronger later form, a sharper constant, or a useful
warning, and otherwise compressed behind an explicit cross-reference.
...
Those four clusters receive complete mathematical chapters below.  The inverse and
non-elementarity developments were likewise absent from that earlier version; the current
main article now summarizes them, while this frontier retains their longer proof-oriented
account.  The current snapshot also adds a proof that positivity of the
...
\midrule
\endhead
\textbf{Expanded branch} & A current Lean module whose mathematical content is treated more
fully here than in the main article: principally \path{FabiusInverse.lean},
\path{ElementaryFunction.lean}, \path{NotElementary.lean}, and
...
More precisely,
\begin{equation}
 I(1/2)=1/2,\qquad I'(1/2)=1/2,\qquad I''(1/2)=0,
 \label{integration:eq:inverse-midpoint-jet}
\end{equation}
and for \(0<y<1\),
\begin{equation}
 \begin{aligned}
 ...
 I''(y)=0&\iff y=1/2.
 \end{aligned}
 \label{integration:eq:inverse-curvature-loci}
\end{equation}
\end{corollary}
...
The midpoint identities are \path{fabiusInv_half},
\path{deriv_fabiusInv_half}, and \path{deriv_deriv_fabiusInv_half}; the three
interior equivalences are \path{deriv_deriv_fabiusInv_neg_iff},
\path{deriv_deriv_fabiusInv_pos_iff}, and
\path{deriv_deriv_fabiusInv_eq_zero_iff}.  Their explicit (0<y<1)
hypothesis prevents the total derivative convention at nondifferentiable clamp points from
being mistaken for geometric curvature.
\end{remark}
...
\begin{theorem}[Interior-derivative blow-up; \Formalized]
The positive interior derivative itself diverges at both clamping endpoints:
\[
 I'(y)\longrightarrow+\infty\quad(y\downarrow0),\qquad
 I'(y)\longrightarrow+\infty\quad(y\uparrow1).
\]
The named Lean statements are
\path{tendsto_deriv_fabiusInv_atTop_at_zero_right} and
\path{tendsto_deriv_fabiusInv_atTop_at_one_left}.  They are limits through
\((0,1)\); the totalized inverse has no finite derivative at either endpoint.
\end{theorem}
...
\lean{deriv\_deriv\_fabiusInv}, \lean{deriv\_fabiusInv\_half},
\lean{deriv\_deriv\_fabiusInv\_half},
\lean{deriv\_deriv\_fabiusInv\_eq\_zero\_iff},
\lean{strictConcaveOn\_fabiusInv\_firstHalf},
\lean{strictConvexOn\_fabiusInv\_secondHalf}, the transported root-free bounds,
\lean{mul\_lt\_fabiusInv}, \lean{tendsto\_fabiusInv\_div\_atTop},
\lean{tendsto\_deriv\_fabiusInv\_atTop\_at\_zero\_right}, and reflected companions;
...
Total inverse $I$ & Absent from the earlier article snapshot; now summarized & Global clamped construction, order calculus, reflection, exact
dyadic inverse values, interior calculus consequences, endpoint steepness, and non-H\"older
corollaries.\\
Elementary function class & Absent from the earlier article snapshot; now summarized & Exact inductive closure, totalization convention,
finite-bad-value composition, and dense-open analytic locus.\\
Non-elementarity & Absent from the earlier article snapshot; now summarized & No local agreement on any set with nonempty interior for $F$,
...
Second saddle correction & Absent from the earlier article snapshot & Complete $E_1$--$E_4$ expansion,
...
\begin{equation}
 ...
 =\frac{(-1)^nq^{\binom{n+1}{2}}}{(q;q)_n}
  \sum_{k=0}^{n}(-1)^kq^{\binom k2}\qbinom nkq P(q^{-k}).
...
\begin{equation}
 ...
 {(q;q)_n}\qbinom nkq.                                   \label{dyadicq:eq:barycentric-denominator}
\end{equation}
...
\begin{equation}
 (\Toperator_kP)(c)=\sum_{r=0}^{2^k-1}\tm_rP(c+r).         \label{dyadicq:eq:TM-operator}
\end{equation}
Writing $D=\dd/\dd c$, the binary digits of $r$ give
\begin{equation}
...
moment is
\begin{equation}
 \sum_{r=0}^{2^k-1}\tm_rr^k=(-1)^k2^{\binom k2}k!.       \label{dyadicq:eq:first-TM-moment}
\end{equation}
More generally,
\begin{equation}
 c\longmapsto\frac1{(n+k)!}\sum_{r<2^k}\tm_r(r+c)^{n+k} \label{dyadicq:eq:TM-degree-n}
\end{equation}
has degree at most $n$, with leading coefficient
\begin{equation}
 [c^n]\left(\frac1{(n+k)!}\sum_{r<2^k}\tm_r(r+c)^{n+k}\right)
...
\begin{equation}
 \operatorname{App}_m(\theta)=\E(Y+\theta)^m
...
\begin{equation}
 ...
 \sum_{r=0}^{2^k-1}\tm_r\bigl(r-2^k(1+\theta)\bigr)^{n+k}.}
...
For every $n\ge0$ and $\theta,\eta\in\C$,
\begin{equation}
 \boxed{
 \frac{\operatorname{App}_n(\theta)}{n!}
 =\frac{(-1)^n2^{-\binom{n+1}{2}}}{(1/2;1/2)_n}
 \sum_{k=0}^{n}\frac{\qbinom nk{1/2}}{4^{\binom k2}(n+k)!}
 \sum_{r=0}^{2^k-1}\tm_r\bigl(r+\eta+2^k\theta\bigr)^{n+k}.}
...
\begin{align}
 ...
 \sum_{k=0}^{m}\frac{\qbinom mk{1/2}}{4^{\binom k2}(m+k)!}\notag\\[-0.2em]
 &\hspace{5em}\times
 \sum_{r=0}^{2^k-1}\tm_r\bigl(r-2^k(1+\theta_m)\bigr)^{m+k},
 \label{dyadicq:eq:centered-digit-q}
\end{align}
and, for any fixed $\eta\in\C$,
\begin{align}
 F(x)&=\sum_{\substack{m\ge1\\\delta_m=1}}
 (-1)^{s_{m-1}}
 \frac1{(-2)^{m^2}(1/2;1/2)_m}
 \sum_{k=0}^{m}\frac{\qbinom mk{1/2}}{4^{\binom k2}(m+k)!}\notag\\[-0.2em]
 &\hspace{5em}\times
 \sum_{r=0}^{2^k-1}\tm_r\bigl(r+\eta+2^k\theta_m\bigr)^{m+k}.
...
$p_m=\lfloor2^mM/2^N\rfloor$.  For any function $P$ on the nonnegative integers and
$k\ge0$,
\begin{align}
 \sum_{r=0}^{M2^k-1}\tm_rP(r)
 &=\sum_{\substack{1\le m\le N\\\delta_m=1}}
 \tm_{p_{m-1}}
 \sum_{s=0}^{2^{N-m+k}-1}\tm_s
...
\begin{equation}
 ...
 \le2^{-(p-1)}\e^{|Q-1/2|}.                              \label{dyadicweb:eq:shifted-spline-bound-formal}
\end{equation}
...
\begin{equation}
 \widehat\Up(\xi)=\prod_{j=0}^{\infty}
 \sinc\!\left(\frac{\pi\xi}{2^j}\right),
 \qquad
 \widehat f(\xi)=\int_\R f(t)\e^{-2\pi\ii\xi t}\dd t.
...
\begin{equation}
 L_Y(s)=\E e^{-sY}=\mathscr A(-s)
 =\prod_{j=1}^{\infty}\frac{1-e^{-s/2^j}}{s/2^j},         \label{dyadicq:eq:LX}
\end{equation}
then, for $\sigma>0$ and $0<x<1$, the frontier Bromwich formula is
\begin{equation}
 \boxed{
 F(x)=\frac1{2\pi\ii}\lim_{T\to\infty}
 \int_{\sigma-\ii T}^{\sigma+\ii T}\frac{e^{sx}}s
 \prod_{j=1}^{\infty}\frac{1-e^{-s/2^j}}{s/2^j}\dd s.} \label{dyadicq:eq:Bromwich}
\end{equation}
...
\begin{align}
 ...
 A(z)&=\prod_{j\ge1}\frac{\e^{z/2^j}-1}{z/2^j},          \label{dyadicasym:eq:A-product}\\
 A(2z)&=\frac{\e^z-1}{z}A(z),\qquad A(z)=\e^zA(-z).     \label{dyadicasym:eq:A-reflection}
\end{align}
...
\begin{align}
 \widetilde\kappa(z)
 &=\int_0^\infty\kappa(u)u^{z-1}\dd u
   =-\Gamma(z)\zeta(1+z),
...
factors in the exact formulas.  Its poles at $2\pi\ii k/L$ give the Fourier modes; the
...
$n\ge1$ and every $c>0$,
\begin{equation}
 a_n=\frac1{2\pi\ii}\int_{c-\ii\infty}^{c+\ii\infty}
 \frac{A(z)}{z^{n+1}}\dd z.                               \label{dyadicasym:eq:vertical-extraction}
\end{equation}
The integral is absolutely convergent because
$|A(c+\ii t)|\le A(c)$ and $|c+\ii t|^{-n-1}$ is integrable.  Fubini and the
inverse-Laplace kernel recover $\E X^n/n!=a_n$, so this is an exact consequence of the
moment law rather than an inversion assumption for $F$.

Set $c=\lambda_n$ and write $z=\lambda_n(1+\ii\theta)$.  Define
\begin{equation}
 \mathcal C_n=\sqrt{\frac{\lambda_n}{2\pi}}
 \int_{-\infty}^{\infty}
 \frac{A(\lambda_n(1+\ii\theta))}{A(\lambda_n)}
 (1+\ii\theta)^{-n-1}\dd\theta.                           \label{dyadicasym:eq:Cn-def}
\end{equation}
...
Put $u=\Log(1+\ii\theta)$.  On a fixed central neighborhood, the exponent in
\eqref{dyadicasym:eq:Cn-def} satisfies
\begin{align}
 \Phi_{\lambda}^{\mathrm{coef}}(\theta)
 :={}&\log\frac{A(\lambda(1+\ii\theta))}{A(\lambda)}-(n+1)u\notag\\
...
\end{align}
where every fixed derivative of the last term is $O(\e^{-\lambda})$.  This is the same
scaled local phase as in the continuous Bromwich analysis.  Globally,
\begin{equation}
 \left|\frac{A(\lambda(1+\ii\theta))}{A(\lambda)}
 (1+\ii\theta)^{-n-1}\right|
...
Factor
\begin{equation}
 B_m(z)=\e^{(m-1)z}\mathcal S_m(z),\qquad
 \mathcal S_m(z)=\sum_{h=0}^{m-1}\tm_h\e^{-hz}.
 \label{dyadicasym:eq:fixed-prefix-factor}
\end{equation}
For $\Re z=r>0$,
$|\mathcal S_m(z)-1|\le\e^{-r}/(1-\e^{-r})$; thus a fixed prefix is dominated
...
\end{equation}
Vertical extraction of $[z^n](B_mA)$ at $z=r(1+\ii\theta)$ gives a normalized mass
$\mathcal C_{m,n}$ and the exact separation
\begin{equation}
 \boxed{
...
$O_m(\e^{-\lambda/m})$.  The same vertical-tail bound therefore yields
\begin{equation}
 \log\mathcal C_{m,n}
 =\sum_{j=1}^{N-1}\frac{A_j(\lambda)}{\lambda^j}
 +O_{m,N}(\lambda^{-N}).
...
$O_N(r^{K_N}\e^{-r})$, smaller than every required power of $1/\lambda$ in this
...
\begin{equation}
 ...
 =\sum_{h=0}^{2^n-1}\tm_h\e^{-hz}.
 \label{dyadicasym:eq:Bromwich-TM-product}
\end{equation}
Starting from the Laplace--Stieltjes inversion formula for $F$, putting
$x=2^{-n}$, substituting $s=2^nz$, and using $A(z)=\e^zA(-z)$ yields
\begin{equation}
 F(2^{-n})=\frac{2^{-\binom n2}}{2\pi\ii}
 \int\frac{A(z)P_n(z)}{z^{n+1}}\dd z.
 \label{dyadicasym:eq:Bromwich-rescaled}
\end{equation}
With $A(z)=\E\e^{zX}$, inverse Laplace transforms this exactly into
\begin{equation}
 F(2^{-n})=2^{-\binom n2}\E\left[
 \sum_{h=0}^{2^n-1}\tm_h\frac{(X-h)_+^n}{n!}\right].
...
$P_n(z)=1+O(\e^{-\lambda_n})$, while the continuous radius
...
\begin{equation}
 \mathcal S_{m_n}(r_n)=\sum_{h<m_n}\tm_h\e^{-hr_n},
...
\((1/2,1)\), and \(\R\setminus(0,1)\cup\{1/2\}\), by
...
part of the statement: \(\InvF\) is not differentiable at the clamping points.
\end{proposition}
...
\[
 \InvF'(y)\longrightarrow+\infty\quad(y\downarrow0),\qquad
 \InvF'(y)\longrightarrow+\infty\quad(y\uparrow1).
\]
...
midpoint jet, exact interior curvature loci, strict closed-half curvature,
transported flatness, effective steepness, exact global differentiability and
$C^\infty$ loci, and endpoint quotient and derivative divergence. & \Formalized\\
...
tendsto_deriv_fabiusInv_atTop_at_zero_right
...
$\InvF''$, the midpoint jet, exact interior curvature sign and zero loci, and
strict concavity on $[0,1/2]$ and strict convexity on $[1/2,1]$;
\item the effective and sharp inverse power inequalities, effective
linear steepness, the two endpoint quotient limits, and divergence of the
positive interior derivative at both endpoints;
...
&\InvF'(y)\to+\infty\quad(y\downarrow0),\qquad
 \InvF'(y)\to+\infty\quad(y\uparrow1),\\
&\InvF'(1/2)=1/2,\qquad \InvF''(1/2)=0,\\
&\InvF(y)\sim
 \frac{\sqrt{\log(1/y)}}{\e\sqrt{\log2}}\,
 \exp\!\left[-\sqrt{2\log2\,\log(1/y)}\right].
\end{align*}
