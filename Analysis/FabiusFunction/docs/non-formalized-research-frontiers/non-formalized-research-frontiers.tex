...
\hypersetup{
  ...
  citecolor=linkblue,
  urlcolor=linkblue,
  pdftitle={Non-Formalized Research Frontiers for the Fabius Function},
  pdfsubject={Consolidated natural-language research dossiers awaiting Lean verification},
...
\title{\bfseries Non-Formalized Research Frontiers for the Fabius Function\\[0.35em]
\large Consolidated natural-language dossiers awaiting Lean verification}
\author{Research quarantine for the \texttt{ProveIt} Fabius development\\
\small Eleven source notebooks merged without promotion to the formalization-backed exposition}
\date{Consolidated repository snapshot: 25 August 2026}

\begin{document}
% Compatibility macros used by the consolidated source dossiers.  They are
...
\begin{abstract}
This volume consolidates every mathematical document formerly stored in a
subdirectory of \path{Analysis/FabiusFunction/docs/non-formalized-research-frontiers/}.
The dossiers contain natural-language proofs, symbolic derivations, numerical checks,
conjectural extensions, and explanations built on top of the current Lean library.  They
are gathered here so that no unformalized claim can be mistaken for part of the verified
primary exposition or lost while awaiting a machine-checked proof.

The merge is intentionally conservative.  Overlapping dossiers and alternative proof
routes remain visible, with editorial precedence notes where one source supersedes a weaker
one.  Standalone preambles, title pages, local tables of contents, and conflicting labels
have been normalized, but mathematical bodies, local status warnings, citations, tables,
algorithms, plots, and open-problem lists are retained.  Editorial corrections made during
consolidation are stated locally rather than silently changing the mathematical record.
\end{abstract}
...
paper citation is not sufficient.  Some dossiers quote formalized inputs; that does not
...
Each part below is a source dossier.  The source-path banner and SHA-256 comment preserve
provenance.  Cross-reference and bibliography keys have a dossier prefix solely to make the
formerly standalone documents coexist.  Repeated material is retained when it supplies an
independent derivation, a stronger later form, a sharper constant, or a useful warning.

Notation is local to each dossier unless an explicit crosswalk says otherwise.  In
particular, the Appell polynomials and the saddle logarithmic coefficients both use an
\(A\)-symbol in their source notebooks; \(c_n\) denotes a centered moment in the dyadic
web but a finite Thue--Morse row in the repeated-integration dossier; and the signed global
extension, Toeplitz weights, and Taylor blocks also have source-local spellings.  The
Fourier convention printed in each dossier controls its own factors of \(2\pi\).  These
collisions are preserved as provenance, not intended as global identifications.

\begin{longtable}{@{}p{0.21\textwidth}p{0.33\textwidth}p{0.40\textwidth}@{}}
\toprule
...
in the corresponding dossier is formalized.  In particular, the sharp quantitative
...
Lean declaration is audited.
\end{boxedremark}


% BEGIN MERGED SOURCE: Fabius_Integration_Research_Frontiers/Fabius_Integration_Research_Frontiers.tex
% Original SHA-256: 21222ae5a8c64cf556dac562fd66943ae0b6ed881408e23e37edfa9113bdecbd
\clearpage
\part{Fabius Integration Research Frontiers}
\label{part:integration}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Fabius_Integration_Research_Frontiers/Fabius_Integration_Research_Frontiers.tex}.  Broad mixed-status dossier.  It supplies inverse, probability, discrete-engine, and all-orders saddle machinery.  Claims already represented in Lean are inputs; the dossier-level generalizations and consequences remain quarantined here.\par\smallskip Original source SHA-256: \nolinkurl{21222ae5a8c64cf556dac562fd66943ae0b6ed881408e23e37edfa9113bdecbd}.
\end{boxedremark}
...
\begin{center}
 \texttt{531338d79f2f640041562138c7b80772ee2f0490}.
\end{center}
...
\cite{integration:arias05442} assumes a smooth function $\phi:\R\to\R$, a real dilation
parameter $k$, and
\begin{equation}
...
\eqref{integration:eq:original-characterization-derivative} for some $k\in\R$.  Then $k>0$.
...
\subsubsection{Order-isomorphism construction and totalization}
...
\begin{equation}
 ...
 \label{integration:eq:fabius-order-iso}
\end{equation}
Define the clamping map
\begin{equation}
 \pi(y)=\min(1,\max(0,y))
 \label{integration:eq:clamp}
\end{equation}
and the total inverse
\begin{equation}
 I(y)=\operatorname{val}\bigl(\mathfrak F^{-1}(\pi(y))\bigr).
 \label{integration:eq:total-inverse}
\end{equation}
The declarations \nolinkurl{fabiusOrderIso} and \nolinkurl{fabiusInv} formalize this
construction.  Lean uses \nolinkurl{Set.IccExtend} rather than spelling out
\eqref{integration:eq:clamp}.

\begin{theorem}[Global inverse package]
\label{integration:thm:global-inverse}
...
\begin{enumerate}
 ...
 \begin{equation}
  ...
  \qquad
  I(F(x))=x.
  \label{integration:eq:two-sided-inverse}
 \end{equation}
 ...
 \begin{equation}
  ...
  \qquad
  I(y)=1\quad(y\ge1).
  \label{integration:eq:inverse-tails}
 \end{equation}
\end{enumerate}
\end{theorem}
...
nondecreasing, so the composite \eqref{integration:eq:total-inverse} is continuous and nondecreasing on
...
\label{integration:prop:inverse-comparison}
For $x,y\in[0,1]$,
\begin{align}
 I(y)\le x &\iff y\le F(x),
 & I(y)<x &\iff y<F(x),                                    \label{integration:eq:I-left-compare}\\
 x\le I(y) &\iff F(x)\le y,
 & x<I(y) &\iff F(x)<y.                                    \label{integration:eq:I-right-compare}
\end{align}
\end{proposition}

\begin{proof}
Apply the strictly increasing map $F$ to the first comparison, or the strictly increasing map
$I$ to the second, and use \eqref{integration:eq:two-sided-inverse}.  The strict versions are the same
argument with $<$ in place of $\le$.
\end{proof}

\subsubsection{Reflection and exact inverse values}

The reflection symmetry of $F$ survives totalization without a domain restriction.

\begin{theorem}[Global reflection]
\label{integration:thm:inverse-reflection}
For every $y\in\R$,
\begin{equation}
 I(1-y)=1-I(y).
 \label{integration:eq:inverse-reflection}
\end{equation}
In particular
\begin{equation}
 I(0)=0,
 ...
 \label{integration:eq:inverse-basic-values}
\end{equation}
\end{theorem}
...
\]
Injectivity of $F$ on $[0,1]$ proves \eqref{integration:eq:inverse-reflection}.  Substituting
...
\begin{equation}
 ...
 \label{integration:eq:dyadic-evaluator-input}
\end{equation}
The evaluator is clamped for $a>2^n$.

\begin{theorem}[Inverting the exact dyadic evaluator]
\label{integration:thm:inverse-dyadic-evaluator}
For all $n,a\in\N$,
\begin{equation}
 I(D_{n,a})=\frac{\min(a,2^n)}{2^n}.
 \label{integration:eq:inverse-dyadic-evaluator}
\end{equation}
In particular,
\begin{equation}
 ...
 \label{integration:eq:inverse-five-seventy-two}
\end{equation}
\end{theorem}

\begin{proof}
For $a\le2^n$, combine \eqref{integration:eq:dyadic-evaluator-input} with the left inverse identity in
\eqref{integration:eq:two-sided-inverse}.  For $a\ge2^n$, the clamped evaluator equals $1$ and the result
is $I(1)=1$.
\end{proof}

\subsubsection{Formal interior calculus and curvature}
...
\label{integration:cor:inverse-derivative}
For $0<y<1$,
\begin{equation}
 I'(y)=\frac{1}{F'(I(y))}
      =\frac{1}{2\Up(2I(y)-1)}.
 \label{integration:eq:inverse-derivative}
\end{equation}
...
\eqref{integration:eq:basic-inputs}.
\end{proof}
...
\label{integration:cor:inverse-curvature}
The inverse $I$ is strictly concave on $[0,1/2]$ and strictly convex on $[1/2,1]$.  For every
$0<y<1$,
\begin{equation}
 I''(y)=-\frac{F''(I(y))}{F'(I(y))^3}.
 \label{integration:eq:inverse-second-derivative}
\end{equation}
The midpoint is the reflection-symmetric change of curvature.
\end{corollary}

\begin{remark}
The first identity in \cref{integration:cor:inverse-derivative}, its explicit \(\Up\)-form, strict
...
\cref{integration:cor:inverse-curvature} is \path{deriv_fabiusInv_hasDerivAt} together with
...
\eqref{integration:eq:inverse-derivative} ceases to have a finite limit.  The formal development proves
...
\subsubsection{Root-free inverse bounds}

For $n\in\N$, the effective flatness theorem for $F$ says that on the dyadic window
$0\le x\le2^{-n}$,
\begin{equation}
 F(x)\le 2^{\binom{n+1}{2}}x^n.
 \label{integration:eq:effective-flatness-input}
\end{equation}
The sharp factorial refinement is
\begin{equation}
 F(x)\le \frac{2^{\binom{n+1}{2}}}{n!}x^n.
 \label{integration:eq:sharp-flatness-input}
\end{equation}
Substitute $x=I(y)$ and use $F(I(y))=y$.

\begin{theorem}[Flatness transported through the inverse]
\label{integration:thm:inverse-root-free}
Let $n\in\N$ and $y\in[0,1]$.  If
\begin{equation}
 2^n I(y)\le1,
 \label{integration:eq:inverse-scale-hypothesis}
\end{equation}
then
\begin{align}
 y&\le 2^{\binom{n+1}{2}}I(y)^n,                            \label{integration:eq:inverse-basic-root-free}\\
 y&\le \frac{2^{\binom{n+1}{2}}}{n!}I(y)^n.                \label{integration:eq:inverse-sharp-root-free}
\end{align}
It is enough to assume the argument-side condition
\begin{equation}
 0\le y\le F(2^{-n}).
 \label{integration:eq:inverse-argument-window}
\end{equation}
\end{theorem}

\begin{proof}
Under \eqref{integration:eq:inverse-scale-hypothesis}, the point $x=I(y)$ belongs to the window of
\eqref{integration:eq:effective-flatness-input} and \eqref{integration:eq:sharp-flatness-input}.  Their left sides
become $F(I(y))=y$, giving the two conclusions.  Under
\eqref{integration:eq:inverse-argument-window}, the comparison calculus
\eqref{integration:eq:I-left-compare} gives $I(y)\le2^{-n}$, which is exactly
\eqref{integration:eq:inverse-scale-hypothesis}.
\end{proof}

If roots are allowed, \eqref{integration:eq:inverse-sharp-root-free} reads
\begin{equation}
 I(y)\ge
 \left(\frac{n!}{2^{\binom{n+1}{2}}}y\right)^{1/n}
 \qquad\bigl(0\le y\le F(2^{-n})\bigr).
 \label{integration:eq:inverse-root-form}
\end{equation}
...
\subsubsection{An effective infinite-slope estimate}

The case $n=2$ already forces the difference quotient to diverge.  Since
$2^{\binom32}=8$ and $F(1/4)=5/72$, \eqref{integration:eq:inverse-basic-root-free} gives
\begin{equation}
 y\le8I(y)^2
 \qquad\left(0\le y\le\frac5{72}\right).
 \label{integration:eq:inverse-square-bound}
\end{equation}

\begin{theorem}[Effective domination of every slope]
\label{integration:thm:inverse-effective-slope}
Let $M\in\R$ and $y>0$.  If
\begin{equation}
 y\le F(1/4)=\frac5{72},
 \qquad
 8M^2y<1,
 \label{integration:eq:effective-slope-hypotheses}
\end{equation}
then
\begin{equation}
 My<I(y).
 \label{integration:eq:effective-slope-conclusion}
\end{equation}
\end{theorem}
...
\eqref{integration:eq:inverse-square-bound} yields
\[
 y\le8M^2y^2.
\]
Since $y>0$, division by $y$ gives $1\le8M^2y$, contradicting
\eqref{integration:eq:effective-slope-hypotheses}.
\end{proof}
...
\label{integration:thm:inverse-infinite-slope}
The endpoint difference quotients diverge:
\begin{align}
 \frac{I(y)}{y}&\longrightarrow+\infty
   &&(y\downarrow0),                                       \label{integration:eq:inverse-slope-zero}\\
 \frac{1-I(y)}{1-y}&\longrightarrow+\infty
   &&(y\uparrow1).                                         \label{integration:eq:inverse-slope-one}
\end{align}
\end{theorem}

\begin{proof}
Fix a target slope $M$.  Replace it by $|M|+1$ and choose $y>0$ below both
$F(1/4)$ and $[8(|M|+1)^2]^{-1}$.  Then \cref{integration:thm:inverse-effective-slope} gives
...
\begin{proof}
The Lipschitz claim follows immediately from \eqref{integration:eq:inverse-slope-zero}.  For the H\"older
claim, choose $n$ with $1/n<\alpha$.  On the shrinking window
$0<y\le F(2^{-n})$, \eqref{integration:eq:inverse-root-form} gives $I(y)\ge c_ny^{1/n}$ with
$c_n>0$.  If \eqref{integration:eq:holder-false} held, then
...
not read \eqref{integration:eq:inverse-root-form} as a lower bound uniform in $n$.  Its strength is
...
\end{equation}
Conditioning on the tail \(X'\), the head coordinate \(U_0\) is uniform, so
\((U_0+X')/2\) has no atoms; equivalently, the law is absolutely continuous after
convolution with the head uniform law.  Thus \(H\) is continuous.  Reflection invariance
then gives
\begin{equation}
 H(1-y)=1-H(y).
...
For $t\ge0$, support and \eqref{integration:eq:CDF-is-F} give
\begin{equation}
...
\begin{equation}
 ...
 \label{integration:eq:Fubini-kernel}
\end{equation}
...
\eqref{integration:eq:g-FTC-hypothesis} gives, for $x\in[0,1]$,
\begin{equation}
 \int_0^1K(t,x)\dd t=\int_0^xg'(t)\dd t=g(x)-g(0).
 \label{integration:eq:Fubini-first-order}
\end{equation}
Integrating first in $x$ gives
\begin{equation}
 \int K(t,x)\dd\mu(x)=g'(t)\mu((t,\infty))=g'(t)\Up(t)
 \label{integration:eq:Fubini-second-order}
\end{equation}
by \eqref{integration:eq:survival-is-up}.  Fubini equates the two iterated integrals.  Since $\mu$ is a
probability measure supported on $[0,1]$, the integral of the constant $g(0)$ is $g(0)$,
and rearrangement yields \eqref{integration:eq:survival-Fubini}.
\end{proof}
...
\begin{align}
 ...
 J_k(s)&=\int_0^1t^k\Up(t)\e^{-st}\dd t.                 \label{integration:eq:survival-tilt-def}
\end{align}
...
Apply \cref{integration:thm:survival-Fubini} to $g(x)=\e^{-sx}x^k$.  For $k=0$ one obtains
\begin{equation}
 L_0(\mu;s)=1-sJ_0(s).
...
\begin{align}
 0&\le\int_a^b\Up-\int_{a+\delta}^{b-\delta}\Up\le2\delta, \label{integration:eq:up-shrink-error}\\
 0&\le\int_{a-\delta}^{b+\delta}\Up-\int_a^b\Up\le2\delta. \label{integration:eq:up-expand-error}
\end{align}
...
\begin{equation}
 ...
 \label{integration:eq:inverse-dyadic-normalization}
\end{equation}
Its triangular recurrence has edge weights
\begin{equation}
 w(k,n)=\frac{1}{(2^n-1)(n-k+1)!}
 \qquad(0\le k<n).
 \label{integration:eq:fabius-recurrence-edge}
\end{equation}
Because $a_0=1$, \cref{integration:thm:triangular-path-solver} gives $a_n=P_w(n)$.
...
\begin{equation}
 ...
 \label{integration:eq:fabius-composition-edge}
\end{equation}
Therefore:

\begin{theorem}[Nonrecursive inverse-dyadic formula]
\label{integration:thm:composition-inverse-dyadic}
For every $n\in\N$,
\begin{equation}
 F(2^{-n})=
 2^{-\binom n2}
 ...
 \label{integration:eq:composition-inverse-dyadic}
\end{equation}
At $n=0$, the unique empty composition contributes the empty product $1$.
\end{theorem}

\begin{proof}
Apply \cref{integration:thm:triangular-path-solver} with \eqref{integration:eq:fabius-recurrence-edge}, translate
paths into compositions using \eqref{integration:eq:composition-partial-sums}, and multiply by the outer
normalization in \eqref{integration:eq:inverse-dyadic-normalization}.
\end{proof}
...
\begin{equation}
 ...
 \frac{1}{(2^{r_1+\cdots+r_j}-1)(r_j+1)!},
 \qquad n>0.
 \label{integration:eq:composition-by-length}
\end{equation}

\subsubsection{Small cases as structural checks}
...
\begin{equation}
 F(1/4)=\frac5{72}.
 \label{integration:eq:F-quarter-path-check}
\end{equation}
\end{example}
...
\begin{equation}
 F(1/8)=\frac1{288}.
 \label{integration:eq:F-eighth-path-check}
\end{equation}
\end{example}
...
\subsubsection{The half-base \texorpdfstring{$q$}{q}-binomial weights}
...
\begin{equation}
 ...
 \label{integration:eq:fabius-toeplitz-weight}
\end{equation}
The half-base finite $q$-binomial theorem evaluated at $z=1/2$ gives
\begin{equation}
 \sum_{j=0}^{n}w_{n,j}=1.
 \label{integration:eq:fabius-toeplitz-mass}
\end{equation}
...
\begin{equation}
 ...
 \label{integration:eq:fabius-toeplitz-exact-variation}
\end{equation}
The formal proof uses the elementary uniform lower bound
\begin{equation}
 (1/2;1/2)_n\ge\frac14
 \label{integration:eq:half-q-lower-quarter}
\end{equation}
and the product comparison
\begin{equation}
 (-1/2;1/2)_n(1/2;1/2)_n\le1.
 \label{integration:eq:q-product-comparison}
\end{equation}
Hence
\begin{equation}
 \sum_{j=0}^{n}|w_{n,j}|\le16.
 \label{integration:eq:fabius-toeplitz-variation-16}
\end{equation}
...
\begin{equation}
 q(n,j)=2n-j.
 \label{integration:eq:fabius-toeplitz-index}
\end{equation}
Since $0\le j\le n$,
\begin{equation}
 q(n,j)=2n-j\ge n.
 \label{integration:eq:fabius-toeplitz-index-tail}
\end{equation}
Thus every index in row $n$ lies beyond the common threshold $n$.

\begin{corollary}[Specialized Fabius row convergence]
\label{integration:cor:fabius-toeplitz-convergence}
For every convergent sequence $H_m\to L$ in $\R$ or $\C$,
\begin{equation}
 \sum_{j=0}^{n}w_{n,j}H_{2n-j}\longrightarrow L.
 \label{integration:eq:fabius-toeplitz-convergence}
\end{equation}
\end{corollary}

\begin{proof}
Apply \cref{integration:thm:finite-row-toeplitz} with $C=16$ and use
\eqref{integration:eq:fabius-toeplitz-mass}, \eqref{integration:eq:fabius-toeplitz-variation-16}, and
\eqref{integration:eq:fabius-toeplitz-index-tail}.
\end{proof}

\subsubsection{Why the inner range uses a half-cell correction}
...
\begin{equation}
 ...
 \label{integration:eq:inclusive-upper-bound}
\end{equation}
...
\begin{equation}
 \ell_p(x)=0
 \iff 2^px<\frac12.
 \label{integration:eq:range-length-zero-iff}
\end{equation}
...
Replacing \eqref{integration:eq:corrected-range-length} by $\lfloor2^px\rfloor$ shifts every jump and
...
\begin{equation*}
 H_p(x)\to L(x)
 \quad+\quad
 \text{\eqref{integration:eq:fabius-toeplitz-mass}, \eqref{integration:eq:fabius-toeplitz-variation-16},
 \eqref{integration:eq:fabius-toeplitz-index-tail}}
 \quad\Longrightarrow\quad
 A_n(x)\to L(x).
\end{equation*}
All Fabius-specific work lies in identifying $H_p$ and its limit; all row summation is
contained in \cref{integration:thm:finite-row-toeplitz}.
...
\begin{align}
 Z_0(t)&=-\frac12+\frac{\Psi'(t)}{L},                         \label{integration:eq:Z0-recurrence}\\
 Z_{n+1}(t)&=\frac{Z_n'(t)}{L}-(n+1)Z_n(t)
              +\frac{(-1)^{n+1}n!}{L}.                      \label{integration:eq:Z-recurrence}
\end{align}

Define integers $c(n,m)$ by
\begin{equation}
 \prod_{k=1}^{n}(z-k)=\sum_{m=0}^{n}c(n,m)z^m.
 \label{integration:eq:falling-shift-coeff}
\end{equation}
...
\label{integration:thm:closed-saddle-jet}
For every $n\ge0$,
\begin{equation}
 Z_n(t)=(-1)^nn!\left(\frac{H_n}{L}-\frac12\right)
 +\sum_{m=0}^{n}c(n,m)
   \frac{\Psi^{(m+1)}(t)}{L^{m+1}}.
 \label{integration:eq:closed-saddle-jet}
\end{equation}
Moreover
\begin{equation}
 c(n,m)=s(n+1,m+1),
 \label{integration:eq:shifted-stirling}
\end{equation}
...
\eqref{integration:eq:Z-recurrence} is generated by the operator product
\begin{equation}
 (D-n)(D-(n-1))\cdots(D-1)\frac{\Psi'}L.
 \label{integration:eq:jet-operator-product}
\end{equation}
Expanding the product gives the sum in \eqref{integration:eq:closed-saddle-jet}.  The constant part
$C_n$ satisfies
\begin{equation}
 C_{n+1}=-(n+1)C_n+\frac{(-1)^{n+1}n!}{L},
 \quad C_0=-\frac12,
 \label{integration:eq:jet-constant-recurrence}
\end{equation}
...
\begin{equation}
 ...
 \label{integration:eq:c-stirling-recurrence}
\end{equation}
...
\begin{align}
 Z_0={}&-\frac12+\frac{\Psi'}L,                              \label{integration:eq:Z0-explicit}\\
 Z_1={}&\frac12-\frac1L-\frac{\Psi'}L+\frac{\Psi''}{L^2},  \label{integration:eq:Z1-explicit}\\
 Z_2={}&-1+\frac3L+\frac{2\Psi'}L-\frac{3\Psi''}{L^2}
             +\frac{\Psi^{(3)}}{L^3},                       \label{integration:eq:Z2-explicit}\\
 Z_3={}&3-\frac{11}{L}-\frac{6\Psi'}L+\frac{11\Psi''}{L^2}
             -\frac{6\Psi^{(3)}}{L^3}+\frac{\Psi^{(4)}}{L^4}.
                                                                    \label{integration:eq:Z3-explicit}
\end{align}
...
\subsubsection{The exponent coefficient at arbitrary order}
...
\right).
 \label{integration:eq:closed-exponent-coefficient}
\end{equation}
...
By \eqref{integration:eq:closed-saddle-jet}, each $E_m$ is therefore completely explicit in terms of
$L^{-1}$ and $\Psi',\ldots,\Psi^{(m)}$.  It has the parity law
\begin{equation}
 E_m(t,-v)=(-1)^mE_m(t,v).
 \label{integration:eq:exponent-coefficient-parity}
\end{equation}
...
\begin{equation}
 ...
 \label{integration:eq:top-jet-monomial}
\end{equation}
Gaussian contraction multiplies by $(2j-1)!!=(2j)!/(2^jj!)$, giving
\eqref{integration:eq:top-jet-mass-coefficient}.
...
$\Psi^{(2j)}/L^{2j}$ in \eqref{integration:eq:closed-saddle-jet} yields the ordinary-analysis
corollary
\begin{equation}
 [\Psi^{(2j)}]A_j
 =\frac{(-1)^j}{2^jj!L^{2j}}.
 \label{integration:eq:top-periodic-derivative-coefficient}
\end{equation}
...
\begin{equation}
 z_j=Z_j(t).
 \label{integration:eq:zj-abbreviation}
\end{equation}
From \eqref{integration:eq:closed-exponent-coefficient}, the first four exponent polynomials are
\begin{align}
 E_1={}&\ii\left(z_0X+\frac{X^3}{3}\right),                 \label{integration:eq:E1-explicit}\\
 E_2={}&-\frac{z_1}{2}X^2+\frac14X^4,                       \label{integration:eq:E2-explicit}\\
 E_3={}&\ii\left(-\frac{z_2}{6}X^3-\frac15X^5\right),      \label{integration:eq:E3-explicit}\\
 E_4={}&\frac{z_3}{24}X^4-\frac16X^6.                      \label{integration:eq:E4-explicit}
\end{align}
At order four in $\varepsilon$, \eqref{integration:eq:a4} gives
\begin{equation}
 a_4=E_4+E_3E_1+\frac12E_2^2
     +\frac12E_2E_1^2+\frac1{24}E_1^4.
 \label{integration:eq:a4-saddle}
\end{equation}
...
\begin{equation}
 ...
 \label{integration:eq:a4-even-polynomial}
\end{equation}
where
\begin{align}
 c_4={}&\frac{z_0^4}{24}+\frac{z_0^2z_1}{4}
       +\frac{z_0z_2}{6}+\frac{z_1^2}{8}+\frac{z_3}{24},     \label{integration:eq:c4}\\
 c_6={}&\frac{z_0^3}{18}-\frac{z_0^2}{8}+\frac{z_0z_1}{6}
       +\frac{z_0}{5}-\frac{z_1}{8}+\frac{z_2}{18}-\frac16, \label{integration:eq:c6}\\
 c_8={}&\frac{z_0^2}{36}-\frac{z_0}{12}+\frac{z_1}{36}
       +\frac{47}{480},                                      \label{integration:eq:c8}\\
 c_{10}={}&\frac{z_0}{162}-\frac1{72},                       \label{integration:eq:c10}\\
 c_{12}={}&\frac1{1944}.                                     \label{integration:eq:c12}
\end{align}
The five rational constants in these formulas are the visible trace of the universal
jet-free monomials in \eqref{integration:eq:closed-exponent-coefficient}.

\subsubsection{Gaussian contraction}

Using \eqref{integration:eq:gaussian-moments-second-correction}, the second mass coefficient is
\begin{equation}
 B_2=3c_4+15c_6+105c_8+945c_{10}+10395c_{12}.
 \label{integration:eq:B2-contraction}
\end{equation}
...
\begin{align}
 ...
 +\frac{5z_2}{6}+\frac{z_3}{8}+\frac1{288}.                \label{integration:eq:B2-explicit}
\end{align}
The first mass coefficient, already logarithmic at first order, is
\begin{equation}
 B_1=A_1=-\frac{z_0^2}{2}-z_0-\frac{z_1}{2}-\frac1{12}.
 \label{integration:eq:A1-jets}
\end{equation}
Substitution of \eqref{integration:eq:Z0-explicit}--\eqref{integration:eq:Z1-explicit} reduces this to the compact
formula from the main article,
\begin{equation}
 A_1(t)=\frac1{24}+\frac1{2L}
 -\frac{\Psi''(t)+\Psi'(t)^2}{2L^2}.
 \label{integration:eq:A1-Psi}
\end{equation}

\subsubsection{Taking the logarithm}

By \eqref{integration:eq:first-log-mass-relations},
\begin{equation}
 A_2=B_2-\frac12B_1^2.
 \label{integration:eq:A2-from-B}
\end{equation}
The quartic and many lower-degree terms cancel.  The result is

\begin{theorem}[Closed second periodic correction]
\label{integration:thm:closed-second-correction}
With $z_j=Z_j(t)$,
\begin{align}
 A_2(t)=\frac1{24}\bigl(&8z_0^3+12z_0^2z_1+12z_0^2
 +48z_0z_1+12z_0z_2\notag\\
 &+6z_1^2+24z_1+20z_2+3z_3\bigr).                          \label{integration:eq:A2-jets}
\end{align}
It is a bounded smooth one-periodic function.
\end{theorem}

\begin{proof}
Insert \eqref{integration:eq:B2-explicit} and \eqref{integration:eq:A1-jets} into
\eqref{integration:eq:A2-from-B}, expand the square, and collect.  Periodicity and smoothness follow from
the same properties of $Z_0,\ldots,Z_3$; boundedness follows from periodic continuity.
\end{proof}

Substitution of \eqref{integration:eq:Z0-explicit}--\eqref{integration:eq:Z3-explicit} makes
$A_2$ an explicit differential polynomial in
\begin{equation}
 \Psi',\ \Psi'',\ \Psi^{(3)},\ \Psi^{(4)}
 \label{integration:eq:A2-derivatives}
\end{equation}
with coefficients in $\Q[L^{-1}]$.  The jet form \eqref{integration:eq:A2-jets} is shorter, more stable
under formal manipulation, and displays the triangular structure that generalizes to all
orders.

\begin{boxedremark}
At order two the mass coefficient \eqref{integration:eq:B2-explicit} contains a quartic term
...
\subsubsection{Exact Lambert phase and exact normalized mass}

Let $\lambda=\lambda(x)$ be the large positive solution of
\begin{equation}
 \lambda2^{-\lambda}=x,
 \qquad x\downarrow0,
 \label{integration:eq:exact-lambert-phase}
\end{equation}
so
\begin{equation}
 r=2^\lambda=\frac{\lambda}{x}.
 \label{integration:eq:saddle-radius-r}
\end{equation}
...
\begin{equation}
 ...
 +C_F+\Psi(\lambda).
 \label{integration:eq:sharp-main-supp}
\end{equation}
The exact negative-Laplace decomposition gives
\begin{equation}
 rx+\Lambda(r)-\frac12\log(2\pi\lambda)
 =\mathcal M(x)+E(r),
 \label{integration:eq:exact-main-tail-supp}
\end{equation}
where the forward tail satisfies
\begin{equation}
 0\le E(2^\lambda)\le4\exp(-2^\lambda).
 \label{integration:eq:forward-tail-flat-supp}
\end{equation}
Therefore
\begin{equation}
 \log F(x)-\mathcal M(x)=\log\mathcal S(x)+E(2^\lambda).
 \label{integration:eq:exact-error-separation-supp}
\end{equation}
The final term is flat relative to $\lambda^{-1}$ by elementary exponential domination and
will be reattached using \cref{integration:thm:flat-invisible}.

\subsubsection{The central integral and the moving window}
...
\begin{equation}
 ...
 \label{integration:eq:normalized-saddle-integral}
\end{equation}
...
\begin{equation}
 ...
 \label{integration:eq:central-exponent-expansion}
\end{equation}
inside an order-dependent central window
\begin{equation}
 |v|\le A_N(\lambda)=\sqrt{32(N+1)\log\lambda}.
 \label{integration:eq:central-window-lambda}
\end{equation}
...
\begin{equation}
 ...
 \label{integration:eq:epsilon-log-radius-decay}
\end{equation}

\subsubsection{Finite exponential substitution}
...
\eqref{integration:eq:finite-remainder-evaluated} pointwise with
\begin{equation}
 x=\varepsilon,
 \qquad
 E_m=E_m(\lambda,v).
 \label{integration:eq:finite-exp-saddle-substitution}
\end{equation}
...
\begin{equation}
 ...
 \label{integration:eq:central-reference-expansion}
\end{equation}
...
\subsubsection{Gaussian contraction and the disappearance of half-orders}

Multiply \eqref{integration:eq:central-reference-expansion} by $\e^{-v^2/2}$ and integrate.  Parity gives
\begin{equation}
 \Gcal[a_{2j+1}(\lambda,\cdot)]=0.
 \label{integration:eq:odd-saddle-contractions-zero}
\end{equation}
For the even terms,
\begin{equation}
 \frac1{\sqrt{2\pi}}
 \int_\R\e^{-v^2/2}a_{2j}(\lambda,v)\dd v=B_j(\lambda).
 \label{integration:eq:even-saddle-contraction}
\end{equation}
The central-window integral may be replaced by the whole-line integral because
\cref{integration:thm:all-orders-polynomial-tail} makes the complementary Gaussian-polynomial tail
$O(\lambda^{-N})$.
...
\eqref{integration:eq:normalized-saddle-integral} is also $O(\lambda^{-N})$ for every fixed $N$.
Consequently
\begin{equation}
 \mathcal S(x)=
 1+\sum_{j=1}^{N-1}\frac{B_j(\lambda)}{\lambda^j}
 +O(\lambda^{-N}).
 \label{integration:eq:full-mass-expansion-supp}
\end{equation}
This is the full mass expansion in the sense of \cref{integration:def:parameter-expansion}.

\subsubsection{Logarithmic transfer and restoration of the flat tail}

The ratio in \eqref{integration:eq:full-mass-expansion-supp} tends to $1$ and is positive.  Apply the
formal logarithm recurrence and its finite analytic remainder theorem:
\begin{equation}
 \log\mathcal S(x)=
 \sum_{j=1}^{N-1}\frac{A_j(\lambda)}{\lambda^j}
 +O(\lambda^{-N}).
 \label{integration:eq:full-log-mass-expansion-supp}
\end{equation}
Now restore $E(2^\lambda)$ in \eqref{integration:eq:exact-error-separation-supp}.  It is flat by
\eqref{integration:eq:forward-tail-flat-supp}, so \cref{integration:thm:flat-invisible} leaves every coefficient
unchanged.

\begin{theorem}[Full exact-phase logarithmic expansion]
\label{integration:thm:full-exact-phase-expansion}
...
\begin{equation}
 ...
 \label{integration:eq:full-exact-phase-expansion}
\end{equation}
The coefficient functions $A_j$ are bounded, smooth, and one-periodic; $A_1$ and $A_2$ are
given by \eqref{integration:eq:A1-jets} and \eqref{integration:eq:A2-jets}.
\end{theorem}

\begin{proof}
The preceding five subsections establish the expansion on the dyadic logarithmic coordinate.
Use \cref{integration:thm:small-log-expansion-equivalence} to transport it to the small-positive filter.
...
\begin{equation}
 O\bigl(({-\log x})^{-N}\bigr),
 \label{integration:eq:full-expansion-neglog-rate}
\end{equation}
...
\eqref{integration:eq:full-exact-phase-expansion}: every periodic coefficient is evaluated at the exact
...
transparent.  For any fixed $u\in\R$ and all sufficiently large integers \(n\), the sequence
\begin{equation}
 x_n=(n+u)2^{-(n+u)}
 \label{integration:eq:phase-locked-sequence-supp}
\end{equation}
has \(n+u>0\) on the large lower-Lambert branch and therefore has exact phase
$\lambda(x_n)=n+u$.  Hence every periodic coefficient is literally constant
along it:
\begin{equation}
 A_j(\lambda(x_n))=A_j(u).
 \label{integration:eq:phase-locked-coefficients}
\end{equation}
...
\addcontentsline{toc}{section}{Supplementary material from this dossier}
...
\cref{integration:sec:inverse,integration:sec:inverse-steepness}.\\
...
\eqref{integration:eq:closed-exponent-coefficient}; \cref{integration:sec:saddle-jets}.\\
\path{FabiusSaddleLeadingCoefficient.lean} &
The unique newest-jet monomial and coefficient $(-1)^j/(2^jj!)$ in the $j$th mass
coefficient; \cref{integration:sec:saddle-coeff-engine}.\\
...
closed $B_2$, and closed $A_2$; \cref{integration:sec:second-correction}.\\
\path{FabiusSecondSaddleExpansion.lean} &
The explicit expansion through $A_2/\lambda^2$ with $O(\lambda^{-3})$ and the weakened
$O(({-\log x})^{-3})$ form; \cref{integration:sec:second-correction,integration:sec:full-assembly}.\\
...
\cref{integration:sec:full-assembly}.\\
...
\cref{integration:sec:full-assembly,integration:sec:exact-phase-boundary}.\\
\bottomrule
\end{longtable}
\endgroup
...
\renewcommand{\refname}{Dossier references}
...
\texttt{531338d79f2f640041562138c7b80772ee2f0490}, 25 August 2026,
\newblock \url{https://github.com/VladimirReshetnikov/ProveIt/tree/531338d79f2f640041562138c7b80772ee2f0490/Analysis/FabiusFunction}.

\end{thebibliography}


\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Fabius_Integration_Research_Frontiers/Fabius_Integration_Research_Frontiers.tex


% BEGIN MERGED SOURCE: Repeated_Integration_and_Rvachev_Up/Repeated_Integration_and_Rvachev_Up.tex
% Original SHA-256: e3a7fcad3d4076a1cfea098defe9b922b1dd9910691b1d575e365597d415e8ed
\clearpage
\part{Repeated Integration and Rvachev's Up-Function}
\label{part:repeated}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Repeated_Integration_and_Rvachev_Up/Repeated_Integration_and_Rvachev_Up.tex}.  Distinct box-spline and repeated-integration dossier preserving the 2012 Mathematics Stack Exchange provenance, exact finite-stage formulas, sharp-error claims, and formalization roadmap.  Its new mathematical statements are natural-language proofs awaiting formalization; the all-orders theorem is supported here by a proof sketch, and the variance-matched \(16^{-n}\) scheme is only a conjectural direction.\par\smallskip Original source SHA-256: \nolinkurl{e3a7fcad3d4076a1cfea098defe9b922b1dd9910691b1d575e365597d415e8ed}.
\end{boxedremark}
\section*{Dossier abstract}
...
\begin{boxedremark}
\textbf{Formalization boundary.}
This is a research-frontier article.  The Lean development supplies exact
probability, centered-spline, Thue--Morse, derivative, and convergence inputs
used here.  It does not yet contain declaration-by-declaration counterparts for
this article's repeated-integration factorization and finite-stage formulae,
the sharp $L^\infty$ and derivative-error constants, the Peano-kernel and
convex-order argument, or the all-orders expansion.  Those results are
conventional mathematical claims awaiting Lean certificates, even when their
ingredients resemble or specialize existing formal theorems.
\end{boxedremark}


\section{The overlooked construction}
\label{repeated:sec:introduction}
...
bounds \cite{repeated:companion,repeated:proveit}.  It also records exact finite atomic laws
and centered finite spline approximants.  A closely related construction, however, begins from a
single piecewise constant function and repeatedly integrates a rescaled difference.  It was
posed independently on Mathematics Stack Exchange in 2012 \cite{repeated:msequestion}; Nathan Grigg
found an exact Heaviside/Thue--Morse expression for every finite stage \cite{repeated:mseanswer}, and
...
\begin{equation}
 ...
 \qquad \sinc z=\frac{\sin z}{z},                           \label{repeated:eq:up-fourier}
\end{equation}
and it is the density of
\begin{equation}
 Y=\sum_{k=1}^{\infty}2^{-k}U_k,
 \qquad U_k\overset{\mathrm{iid}}\sim\Unif[-1,1].          \label{repeated:eq:Y-series}
\end{equation}
This is the centered normalization used in the companion article \cite{repeated:companion}; on
$0\le x\le1$ it is related to the bounded Fabius function by
\begin{equation}
 \Up(x)=F(1-x),\qquad \Up(-x)=\Up(x).                       \label{repeated:eq:F-up}
\end{equation}

Define the initial density
\begin{equation}
 \rho(x)=\frac12\mathbf 1_{(-1,1)}(x)                       \label{repeated:eq:rho}
\end{equation}
and, for a function $g$ supported on $[-1,1]$, define
\begin{equation}
 (Tg)(x)=2\int_{-1}^{x}\bigl(g(2t+1)-g(2t-1)\bigr)\dd t.   \label{repeated:eq:T-original}
\end{equation}
Endpoint values of $\rho$ are immaterial for integration and convolution; the convention in
\eqref{repeated:eq:rho} agrees with the original question.

\begin{lemma}[The recursive integral is a moving integral]
\label{repeated:lem:moving-integral}
For every integrable $g$ supported on $[-1,1]$,
\begin{equation}
 (Tg)(x)=\int_{2x-1}^{2x+1}g(t)\dd t.                       \label{repeated:eq:T-moving}
\end{equation}
\end{lemma}

\begin{proof}
In the first term of \eqref{repeated:eq:T-original}, put $u=2t+1$; in the second, put $u=2t-1$.
This gives
\[
...
the left of $-1$.  Subtracting the two primitives gives \eqref{repeated:eq:T-moving}, with the usual
oriented-integral interpretation when the endpoints are reversed.
\end{proof}

\begin{proposition}[Elementary invariants]
\label{repeated:prop:invariants}
...
\begin{equation}
 (Tg)'(x)=2\bigl(g(2x+1)-g(2x-1)\bigr)                     \label{repeated:eq:T-derivative}
\end{equation}
at every point at which the right side is continuous.
\end{proposition}

\begin{proof}
Nonnegativity is immediate from \eqref{repeated:eq:T-moving}.  If $|x|\ge1$, the integration interval
...
\Cref{repeated:prop:affine-map} below.  Continuity is absolute continuity of the indefinite integral,
and differentiating its two affine endpoints gives \eqref{repeated:eq:T-derivative}.
\end{proof}

The repeated-integration sequence is therefore
\begin{equation}
 f_0=\rho,\qquad f_n=T^n\rho.                               \label{repeated:eq:fn-def}
\end{equation}
Every $f_n$ is an even probability density supported on $[-1,1]$.  Also
$f_n(0)=1$ for every $n\ge1$, since
\begin{equation}
 f_n(0)=\int_{-1}^{1}f_{n-1}(t)\dd t=1.                    \label{repeated:eq:center-one}
\end{equation}

\section{The affine random recursion}
\label{repeated:sec:probability}

The most economical interpretation of $T$ is probabilistic.

\begin{proposition}[The operator as an affine smoothing map]
\label{repeated:prop:affine-map}
Let $X$ have density $g$, let $U\sim\Unif[-1,1]$ be independent of $X$, and put
\begin{equation}
 Z=\frac{X+U}{2}.                                           \label{repeated:eq:affine-Z}
\end{equation}
...
\]
Use \Cref{repeated:lem:moving-integral}.
\end{proof}

Thus the original iteration is the law recursion
\begin{equation}
 X_0\sim\Unif[-1,1],\qquad
 X_n\overset d=\frac{X_{n-1}+U_n}{2}.                      \label{repeated:eq:Markov-recursion}
\end{equation}
...
\label{repeated:thm:random-sum}
For every $n\ge1$,
\begin{equation}
 X_n\overset d=\sum_{k=1}^{n}2^{-k}U_k+2^{-n}V,            \label{repeated:eq:Xn-sum}
\end{equation}
where $U_1,\ldots,U_n,V$ are independent and uniform on $[-1,1]$.
Consequently
\begin{equation}
 X_n\longrightarrow Y=\sum_{k=1}^{\infty}2^{-k}U_k        \label{repeated:eq:Xn-coupling}
\end{equation}
almost surely under the coupling in which the same $U_k$ are used and $V$ is fixed.
\end{theorem}

\begin{proof}
Repeated substitution in \eqref{repeated:eq:Markov-recursion} gives weights
...
\eqref{repeated:eq:Xn-sum}.  The difference between its right side and \eqref{repeated:eq:Y-series} is bounded by
\[
...
nested series coupling \eqref{repeated:eq:Xn-coupling}.
\end{remark}

The stationary law of \eqref{repeated:eq:Markov-recursion} satisfies
$Y\overset d=(Y+U)/2$.  At the density level this is $T\Up=\Up$.  Differentiating recovers
Rvachev's equation
\begin{equation}
 \Up'(x)=2\bigl(\Up(2x+1)-\Up(2x-1)\bigr).                \label{repeated:eq:up-diffeq}
\end{equation}
...
\begin{proof}
The convolution identity is the density form of \eqref{repeated:eq:Xn-sum}.  A convolution of $n+1$
...
\begin{align}
 f_0(x)&=\frac12\mathbf 1_{(-1,1)}(x),                                      \label{repeated:eq:f0}\\
 f_1(x)&=\pos{1-|x|},                                                        \label{repeated:eq:f1}\\
 f_2(x)&=
 \begin{cases}
  1-2x^2,&|x|\le\tfrac12,\\
  2(1-|x|)^2,&\tfrac12\le|x|\le1,\\
  0,&|x|\ge1.
 \end{cases}                                                               \label{repeated:eq:f2}
\end{align}
...
\label{repeated:fig:iterates}
\end{figure}

\section{Fourier product and exact identification of the limit}
\label{repeated:sec:fourier}

Under the Fourier convention fixed above,
\begin{equation}
 \widehat{\beta_a}(\xi)=\sinc(a\xi).                       \label{repeated:eq:box-fourier}
\end{equation}
Therefore \Cref{repeated:thm:box-convolution} gives the transform conjectured in the original question,
without any induction through piecewise polynomials.

\begin{theorem}[Finite Fourier product]
\label{repeated:thm:finite-fourier}
For every $n\ge1$,
\begin{align}
 \widehat f_n(\xi)
 &=\sinc(2^{-n}\xi)\prod_{k=1}^{n}\sinc(2^{-k}\xi)         \label{repeated:eq:fn-fourier}\\
 &=\left(\prod_{k=1}^{n-1}\sinc(2^{-k}\xi)\right)
   \sinc^2(2^{-n}\xi).                                     \label{repeated:eq:fn-fourier-duplicate}
\end{align}
As $n\to\infty$, this converges pointwise to \eqref{repeated:eq:up-fourier}.
\end{theorem}
...
\begin{equation}
 \widehat{Tg}(\xi)=\sinc(\xi/2)\widehat g(\xi/2).           \label{repeated:eq:T-fourier}
\end{equation}
Iterating \eqref{repeated:eq:T-fourier} from $\widehat\rho(\xi)=\sinc\xi$ gives
\eqref{repeated:eq:fn-fourier} directly.  Passing to the limit gives the unique continuous transform
...
\label{repeated:cor:Cm-convergence}
For every fixed $m\ge0$,
\begin{equation}
 \max_{0\le r\le m}\norm{f_n^{(r)}-\Up^{(r)}}_\infty\longrightarrow0,        \label{repeated:eq:Cm-convergence}
\end{equation}
where $n$ is taken large enough that the indicated finite-stage derivatives exist.
\end{corollary}

\begin{proof}
Fix $M>m+2$.  For $n\ge M$, the first $M$ sinc factors in \eqref{repeated:eq:fn-fourier} supply an
...
\label{repeated:lem:general-box-spline}
Let $a_1,\ldots,a_N>0$, put $A=\sum_{i=1}^{N}a_i$, and let
$\beta_{a_i}$ be as in \eqref{repeated:eq:beta}.  Then
\begin{equation}
 (\beta_{a_1}*\cdots*\beta_{a_N})(x)
 =\frac{1}{(N-1)!\prod_{i=1}^{N}(2a_i)}
 \sum_{S\subseteq\{1,\ldots,N\}}(-1)^{|S|}
 \pos{x+A-2\sum_{i\in S}a_i}^{N-1}.                       \label{repeated:eq:general-box-spline}
\end{equation}
\end{lemma}
...
\eqref{repeated:eq:general-box-spline}.  The same proof can be written as an induction by integrating
one more moving interval at each step.
\end{proof}

For the dyadic widths in \eqref{repeated:eq:fn-box}, the total half-width is $A=1$ and
\begin{equation}
 \left(\prod_{k=1}^{n}2\cdot2^{-k}\right)(2\cdot2^{-n})
 =2^{1-\binom{n+1}{2}}.                                    \label{repeated:eq:normalization-product}
\end{equation}
This gives a direct subset formula.

\begin{theorem}[Exact subset-sum formula]
\label{repeated:thm:subset-formula}
For $n\ge1$, let
\[
...
\begin{equation}
 ...
 \pos{x+1-2\sum_{i\in S}a_i}^{n}.                         \label{repeated:eq:subset-formula}
\end{equation}
\end{theorem}
...
\eqref{repeated:eq:subset-formula}.
\end{proof}

Formula \eqref{repeated:eq:TM-formula} is algebraically equivalent to the 2012 answer
\cite{repeated:mseanswer}.  The present derivation shows why the coefficient is a \emph{difference} of
...
\begin{equation}
 x_j=-1+\frac{j}{2^{n-1}},\qquad 0\le j\le2^n.             \label{repeated:eq:knot-grid}
\end{equation}
Some grid points are inactive because $\Delta c_n(j)$ can vanish.  Differentiating
\eqref{repeated:eq:TM-formula} $r$ times for $0\le r\le n-1$ gives
\begin{equation}
 f_n^{(r)}(x)=
 \frac{2^{\binom{n+1}{2}-1}}{(n-r)!}
 \sum_{j=0}^{2^n}\Delta c_n(j)
 \pos{x+1-\frac{j}{2^{n-1}}}^{n-r}.                        \label{repeated:eq:TM-derivatives}
\end{equation}
...
\label{repeated:cor:nth-derivative}
If $-1<x<1$ and $x$ is not on the grid \eqref{repeated:eq:knot-grid}, put
\begin{equation}
 J=\left\lfloor2^{n-1}(x+1)\right\rfloor.
\end{equation}
Then
\begin{equation}
 f_n^{(n)}(x)=2^{\binom{n+1}{2}-1}(-1)^{\wt(J)}.            \label{repeated:eq:nth-derivative}
\end{equation}
\end{corollary}
...
\begin{equation}
 \E V^2=\frac13,
 \qquad
 \E Y^2=\frac13\sum_{k=1}^{\infty}4^{-k}=\frac19.          \label{repeated:eq:tail-variances}
\end{equation}
Consequently
\begin{equation}
 \Var(X_n)-\Var(Y)=\frac29\,4^{-n}.                        \label{repeated:eq:variance-defect}
\end{equation}
...
\label{repeated:sec:Peano}

Let
\begin{equation}
 h=\rho-\Up.                                                \label{repeated:eq:h}
\end{equation}
...
\begin{equation}
 K(x)=\frac12\int_{\R}|x-t|h(t)\dd t.                      \label{repeated:eq:K-def}
\end{equation}
...
\label{repeated:prop:K-properties}
...
\item
\begin{equation}
 \int_{\R}K(x)\dd x=\frac19;                              \label{repeated:eq:K-mass}
\end{equation}
\item
\begin{equation}
 K(0)=\frac19.                                             \label{repeated:eq:K-zero}
\end{equation}
\end{enumerate}
\end{proposition}

\begin{proof}
Evenness and $C^1$ regularity follow directly from \eqref{repeated:eq:K-def}; two distributional
...
functions:
\begin{equation}
 K'(x)=\frac{x}{2}-\int_0^x\Up(t)\dd t.                    \label{repeated:eq:K-prime}
\end{equation}
For $0<x<1$, the differential equation \eqref{repeated:eq:up-diffeq} and the support of $\Up$ give
...
\]
which proves \eqref{repeated:eq:K-mass}.
...
\begin{equation}
 Y\preceq_{\mathrm{cx}}V,                                  \label{repeated:eq:convex-order}
\end{equation}
...
\begin{equation}
 h_n(t)=2^nh(2^nt),
 \qquad
 K_n(t)=2^{-n}K(2^nt).                                     \label{repeated:eq:scaled-K}
\end{equation}
Then
\begin{equation}
 K_n''=h_n,
 ...
 K_n\ge0,
 \qquad
 \int K_n=\frac{4^{-n}}9.                                 \label{repeated:eq:scaled-K-properties}
\end{equation}
The two exact tail factorizations \eqref{repeated:eq:Y-tail}--\eqref{repeated:eq:X-tail} now give
\begin{equation}
 f_n-\Up=p_n*h_n=p_n*K_n''=p_n''*K_n                       \label{repeated:eq:error-factorization}
\end{equation}
...
\begin{proof}
Pair \eqref{repeated:eq:error-factorization} with $\phi$, move the two derivatives onto $\phi$, and use
that $p_n$ is a probability density and $K_n$ is nonnegative of mass $4^{-n}/9$.
For $\phi(x)=x^2$, equality is exactly the variance defect \eqref{repeated:eq:variance-defect}.
\end{proof}

\section{A finite-prefix derivative plateau}
\label{repeated:sec:prefix-derivatives}

To turn \eqref{repeated:eq:error-factorization} into a sharp uniform estimate, one needs an exact bound
...
\begin{equation}
 ...
 \begin{cases}
  \eps_{J/2},&J\text{ even},\\
  0,&J\text{ odd}.
 \end{cases}                                                \label{repeated:eq:TM-prefix}
\end{equation}
...
\label{repeated:thm:prefix-plateau}
Let $r\ge0$ and $n\ge r+1$.  Put
\begin{equation}
 C_r=2^{\binom{r+1}{2}},
 \qquad
 x_r=-1+2^{-r}.                                             \label{repeated:eq:Cr-xr}
\end{equation}
Then the $r$th derivative of $p_n$ exists as a bounded piecewise function and satisfies
\begin{equation}
 \norm{p_n^{(r)}}_\infty=C_r.                              \label{repeated:eq:pn-derivative-norm}
\end{equation}
More precisely,
\begin{equation}
 p_n^{(r)}(x)=C_r
 \quad\text{for almost every }x\in[x_r-2^{-n},x_r+2^{-n}]. \label{repeated:eq:pn-plateau}
\end{equation}
\end{theorem}

\begin{proof}
First take $n=r+1$.  Applying \Cref{repeated:lem:general-box-spline} to the $r+1$ distinct boxes in
...
\]
By \Cref{repeated:lem:TM-prefix}, its absolute value is at most $C_r$.  On the first cell the prefix
...
$\norm{\Up^{(r)}}_\infty=C_r$, attained at the same dyadic point $x_r$ \cite{repeated:companion}.
\Cref{repeated:thm:prefix-plateau} explains this without taking a limit: every sufficiently long finite
...
\label{repeated:sec:exact-rate}

We now combine the positive kernel \eqref{repeated:eq:error-factorization} with the saturated derivative
interval \eqref{repeated:eq:pn-plateau}.

\begin{theorem}[Exact $C^m$ error]
\label{repeated:thm:exact-Cm-error}
Let $m\ge0$ and $n\ge m+3$.  Then
\begin{equation}
 \boxed{\displaystyle
 \norm{f_n^{(m)}-\Up^{(m)}}_\infty
 =\frac{2^{\binom{m+3}{2}}}{9}\,4^{-n}.}                  \label{repeated:eq:exact-Cm-error}
\end{equation}
The positive maximum is attained at
\begin{equation}
 x=-1+2^{-(m+2)}.                                          \label{repeated:eq:error-extremal-point}
\end{equation}
\end{theorem}

\begin{proof}
Differentiate \eqref{repeated:eq:error-factorization} $m$ times and put $r=m+2$:
\begin{equation}
 f_n^{(m)}-\Up^{(m)}=p_n^{(r)}*K_n.                        \label{repeated:eq:error-r}
\end{equation}
The kernel is nonnegative and has mass $4^{-n}/9$, while
$\norm{p_n^{(r)}}_\infty=C_r=2^{\binom{m+3}{2}}$ by
\Cref{repeated:thm:prefix-plateau}.  Therefore
\[
 \norm{f_n^{(m)}-\Up^{(m)}}_\infty
 \le C_r\int K_n
 =\frac{C_r}{9}4^{-n}.
...
interval \eqref{repeated:eq:pn-plateau}.  Hence the integrand in \eqref{repeated:eq:error-r} equals $C_r$ almost
...
\label{repeated:prop:n2-error}
One has
\begin{equation}
 \norm{f_2-\Up}_\infty=\frac1{18}=\frac89\,4^{-2}.         \label{repeated:eq:n2-error}
\end{equation}
\end{proposition}
...
derivative is
\begin{equation}
 p_2''=2\bigl(\delta_{-3/4}-\delta_{-1/4}-\delta_{1/4}+\delta_{3/4}\bigr).    \label{repeated:eq:p2-second}
\end{equation}
Thus \eqref{repeated:eq:error-factorization} gives
\begin{align}
 f_2(x)-\Up(x)=2\bigl(&K_2(x+3/4)-K_2(x+1/4)\notag\\
                     &-K_2(x-1/4)+K_2(x-3/4)\bigr).        \label{repeated:eq:e2-K}
\end{align}
...
$\max K_2=2^{-2}K(0)=1/36$.  Therefore the maximum absolute value in \eqref{repeated:eq:e2-K} is
...
$f_1$, use \eqref{repeated:eq:F-up}: on $0\le x\le1$,
\begin{equation}
 f_1(x)-\Up(x)=1-x-F(1-x)=F(x)-x.                          \label{repeated:eq:e1-F}
\end{equation}
...
\label{repeated:thm:complete-error}
For every $n\ge0$,
\begin{equation}
 \boxed{\displaystyle
 \norm{f_n-\Up}_\infty=
 \begin{cases}
  ...
  \frac89\,4^{-n},&n\ge2.
 \end{cases}}                                              \label{repeated:eq:complete-error}
\end{equation}
For $n\ge2$,
\begin{equation}
 \begin{aligned}
 ...
 \end{aligned}                                             \label{repeated:eq:four-extrema}
\end{equation}
\end{theorem}

\begin{proof}
The cases $n=0,1$ were just discussed, $n=2$ is \Cref{repeated:prop:n2-error}, and $n\ge3$ is
\Cref{repeated:thm:exact-Cm-error} with $m=0$.  The four signed extrema follow from the four plateaux
of height $\pm8$ in the second derivative of the three-box prefix, preserved by all later
probability convolutions; equivalently they follow directly from \eqref{repeated:eq:e2-K} and the same
plateau argument used in \Cref{repeated:thm:exact-Cm-error}.
\end{proof}
...
\begin{equation}
 ...
 \qquad(n\ge2).                                            \label{repeated:eq:extremal-values}
\end{equation}
For example,
\begin{center}
...
\label{repeated:fig:scaled-errors}
\end{figure}
...
\begin{equation}
 n\ge\left\lceil\log_4\frac{8}{9\eta}\right\rceil.         \label{repeated:eq:stopping-C0}
\end{equation}
For the $m$th derivative, once $n\ge m+3$, the corresponding condition is
\begin{equation}
 n\ge\left\lceil
 \log_4\frac{2^{\binom{m+3}{2}}}{9\eta}
 \right\rceil.                                             \label{repeated:eq:stopping-Cm}
\end{equation}
...
\label{repeated:sec:asymptotic-expansion}

The exact norm theorem has a complementary Fourier interpretation.  Put
\begin{equation}
 P_n(\xi)=\prod_{k=1}^{n}\sinc(2^{-k}\xi),
 \qquad z=2^{-n}\xi,
 \qquad \Phi(z)=\prod_{k=1}^{\infty}\sinc(2^{-k}z).         \label{repeated:eq:Pn-Phi}
\end{equation}
Then
\begin{equation}
 \widehat f_n(\xi)=P_n(\xi)\sinc z,
 \qquad
 \widehat\Up(\xi)=P_n(\xi)\Phi(z).                       \label{repeated:eq:tail-fourier-factor}
\end{equation}
Near the origin, consider the analytic germ
\begin{equation}
 R(z)=\frac{\sinc z}{\Phi(z)}.                             \label{repeated:eq:R-def}
\end{equation}
Its logarithm is
\begin{equation}
 \log R(z)=-\sum_{r=1}^{\infty}
 \frac{2^{2r-1}|B_{2r}|}{r(2r)!}
 \frac{4^r-2}{4^r-1}\,z^{2r},                             \label{repeated:eq:log-R}
\end{equation}
where $B_{2r}$ are Bernoulli numbers.  Exponentiation gives
\begin{equation}
 R(z)=1-\frac{z^2}{9}+\frac{2z^4}{2025}
      +\frac{z^6}{2679075}
      +\frac{58z^8}{10247461875}+\cdots.                   \label{repeated:eq:R-series}
\end{equation}

\begin{theorem}[All-orders expansion; proof sketch and formal obligation]
\label{repeated:thm:all-orders}
...
\begin{equation}
 ...
 =\bigO_{m,J}(4^{-Jn}).                                    \label{repeated:eq:all-orders}
\end{equation}
In particular,
\begin{equation}
 ...
 +\bigO_{C^m}(256^{-n})                                    \label{repeated:eq:first-corrections}
\end{equation}
for every fixed $m$.
\end{theorem}

\begin{proofidea}
Subtract the truncated series from \eqref{repeated:eq:tail-fourier-factor} without dividing by
$\Phi$ globally:
\[
...
decay.  Fourier inversion then gives \eqref{repeated:eq:all-orders}.
\end{proofidea}

The leading term gives
\begin{equation}
 4^n\bigl(f_n^{(m)}-\Up^{(m)}\bigr)
 \longrightarrow\frac19\Up^{(m+2)}                       \label{repeated:eq:scaled-limit}
\end{equation}
uniformly.  Since the companion development proves
\begin{equation}
 \norm{\Up^{(m+2)}}_\infty=2^{\binom{m+3}{2}},             \label{repeated:eq:up-sharp-bound}
\end{equation}
the asymptotic constant predicted by \eqref{repeated:eq:scaled-limit} agrees with the exact finite-$n$
constant in \eqref{repeated:eq:exact-Cm-error}.

There is also an operator-theoretic explanation.  From \eqref{repeated:eq:T-fourier} and $T\Up=\Up$,
\begin{equation}
 T\bigl(\Up^{(r)}\bigr)=2^{-r}\Up^{(r)}.                   \label{repeated:eq:T-eigenfunctions}
\end{equation}
...
\label{repeated:cor:Lp}
For every fixed $m\ge0$ and $1\le p<\infty$,
\begin{equation}
 \lim_{n\to\infty}4^n\norm{f_n^{(m)}-\Up^{(m)}}_{L^p}
 =\frac19\norm{\Up^{(m+2)}}_{L^p}.                         \label{repeated:eq:Lp-asymptotic}
\end{equation}
For $p=\infty$, the corresponding equality is already exact for every $n\ge m+3$.
\end{corollary}

\begin{proof}
Uniform convergence in \eqref{repeated:eq:scaled-limit}, together with the common compact support,
gives the finite-$p$ limits.  The $p=\infty$ statement is \Cref{repeated:thm:exact-Cm-error}.
\end{proof}

\section{Why the exact constant is visible at dyadic points}
\label{repeated:sec:dyadic-extrema}

The extremal point in \eqref{repeated:eq:error-extremal-point} is dyadic of level $m+2$.  The companion
...
$x=-1+2^{-(m+2)}$,
\begin{equation}
 \Up^{(m+2j)}(x)=0\qquad(j\ge2).                           \label{repeated:eq:high-derivatives-vanish}
\end{equation}
The all-orders series \eqref{repeated:eq:all-orders} consequently has only its first correction term at
...
produce a $16^{-n}$ scheme.  This conjecturally suggests a general family of
higher-order tail corrections; no such positive compactly supported scheme is
constructed here, and the additional moment constraints remain open.

\section{Computational forms}
\label{repeated:sec:computation}
...
$A_{n-1}$ and use
\begin{equation}
 f_n(x)=A_{n-1}(2x+1)-A_{n-1}(2x-1).                       \label{repeated:eq:primitive-algorithm}
\end{equation}
...
For a single rational $x$, \eqref{repeated:eq:TM-formula} is finite and exact.  One may stop at the last
...
\begin{equation}
 \Up(x)\in\left[f_n(x)-\frac8{9\,4^n},
                 f_n(x)+\frac8{9\,4^n}\right]             \label{repeated:eq:certified-band}
\end{equation}
for every $x$ and every $n\ge2$.  At the four extremal dyadic points, one endpoint of this
interval is exact.  Derivative bands follow from \eqref{repeated:eq:exact-Cm-error}.

\section{A roadmap for Lean formalization}
\label{repeated:sec:Lean-roadmap}
...
$\Up^{(m)}$ \cite{repeated:proveit}.  The present argument can be added in a relatively modular way.
The following is a proof-dependency outline rather than a claim that these new statements are
already formalized.

\begin{enumerate}
\item Define the operator $T$ and prove \eqref{repeated:eq:T-moving} for functions supported on
      $[-1,1]$.
\item Identify $T$ with the pushforward of the product law under $(x,u)\mapsto(x+u)/2$.
\item Prove the finite random-sum identity \eqref{repeated:eq:Xn-sum} and derive the box convolution
      \eqref{repeated:eq:fn-box}.
\item Instantiate a general truncated-power formula for convolutions of interval indicators;
      coalesce the duplicated last weight to obtain \eqref{repeated:eq:TM-formula}.
\item Define $K$ by \eqref{repeated:eq:K-def}.  Use the established monotonicity of $\Up$ to prove
      $K\ge0$, and calculate its mass from the second moments.
\item Prove the summatory Thue--Morse lemma \eqref{repeated:eq:TM-prefix} and the finite-prefix plateau
      \eqref{repeated:eq:pn-plateau}.
\item Combine convolution differentiation, positivity, and the plateau to prove the exact
      norm identity \eqref{repeated:eq:exact-Cm-error}; handle $n=2$ through the atomic second
      derivative \eqref{repeated:eq:p2-second}.
\end{enumerate}
...
\item A saturated Thue--Morse plateau in the appropriate derivative of the finite prefix turns
      ...
      \eqref{repeated:eq:exact-Cm-error} once enough integrations have been performed.
\end{enumerate}
...
\label{repeated:app:equivalence}

The formula displayed in \cite{repeated:mseanswer} is
\begin{equation}
 \sum_{j=0}^{2^n}
 \frac{c_n(j)-c_n(j-1)}{2}
 \frac{\pos{2^nx+2^n-2j}^{n}}{n!\,2^{n(n-1)/2}}.           \label{repeated:eq:MSE-form}
\end{equation}
Because
\[
 2^nx+2^n-2j=2^n\left(x+1-\frac{j}{2^{n-1}}\right),
\]
the total power of two in each term of \eqref{repeated:eq:MSE-form} is
\[
 2^{n^2-1-n(n-1)/2}=2^{n(n+1)/2-1}.
\]
This is exactly the prefactor in \eqref{repeated:eq:TM-formula}.  The two displays therefore agree
term by term, including normalization.

\section{A few exact finite-stage formulas}
\label{repeated:app:first-formulas}

The positive-part formula gives compact global expressions.  In addition to
\eqref{repeated:eq:f1}--\eqref{repeated:eq:f2},
\begin{align}
 f_3(x)=\frac{16}{3}\bigl(&\pos{x+1}^{3}-2\pos{x+3/4}^{3}
 +2\pos{x+1/4}^{3}-2\pos{x}^{3}\notag\\
 &+2\pos{x-1/4}^{3}-2\pos{x-3/4}^{3}+\pos{x-1}^{3}\bigr). \label{repeated:eq:f3-global}
\end{align}
...
\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Repeated_Integration_and_Rvachev_Up/Repeated_Integration_and_Rvachev_Up.tex

% BEGIN MERGED SOURCE: Rvachev_Up_from_Repeated_Integration-2/Rvachev_Up_from_Repeated_Integration.tex
% Original SHA-256: fad16072df30d9e6eb5df03da57dd217769180730581f1dfd19fa5be0d16b262
\clearpage
\part{Repeated Integration and Rvachev's Up-Function: Sharp Thresholds}
\label{part:repeated2}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Rvachev_Up_from_Repeated_Integration-2/Rvachev_Up_from_Repeated_Integration.tex}.  Later, sharper treatment of the repeated-integration construction.  It improves the derivative-norm threshold from the earlier dossier's \(n\ge r+3\) presentation to \(n\ge r+2\), while separately resolving the first admissible stage.  Prefer this threshold where the dossiers overlap.  Its new sharp identities and all-orders expansions remain natural-language proofs and formal obligations.\par\smallskip Original source SHA-256: \nolinkurl{fad16072df30d9e6eb5df03da57dd217769180730581f1dfd19fa5be0d16b262}.
\end{boxedremark}
\section*{Dossier abstract}
...
\eqref{repeated2:eq:up-differential-equation} normalized by $\Up(0)=1$ is Rvachev's up-function.
...
\end{equation}
Then $\mathscr F u_a(t)=\sinc(at)$.  From \eqref{repeated2:eq:fn-box-convolution},
\begin{equation}
 \mathscr F f_n(t)
...
\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Rvachev_Up_from_Repeated_Integration-2/Rvachev_Up_from_Repeated_Integration.tex



% BEGIN MERGED SOURCE: Fabius_Dyadic_q_Connections/Fabius_Dyadic_q_Connections.tex
% Original SHA-256: 8776344b6f214e656345070af0dc69d29239b67ed44cdba4173dac2ead19c80d
\clearpage
\part{Dyadic Calculus for the Fabius Function}
\label{part:dyadicq}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Fabius_Dyadic_q_Connections/Fabius_Dyadic_q_Connections.tex}.  Earlier dyadic/Appell/q-binomial dossier.  Its coarse shifted-spline and \(O(2^{-n})\) Toeplitz estimates remain valid historical corollaries; the following Dyadic Web dossier's sharper shifted estimate and explicit \(O_Q(4^{-n})\) row estimate are preferred when the constructions overlap.  The determinant, Appell compression, superexponential tail, prefix rearrangement, limiting-filter, and integral-transform displays remain formal obligations unless an exact declaration is named locally.\par\smallskip Original source SHA-256: \nolinkurl{8776344b6f214e656345070af0dc69d29239b67ed44cdba4173dac2ead19c80d}.
\end{boxedremark}
\section*{Dossier abstract}
...
\begin{equation}\label{dyadicq:eq:Mersenne-Pochhammer}
 \poch{1/2}{1/2}{n}
 =\prod_{j=1}^{n}(1-2^{-j})
 =2^{-\binom{n+1}{2}}\prod_{j=1}^{n}(2^j-1).
\end{equation}
In particular,
\begin{equation}\label{dyadicq:eq:main-denom-Mersenne}
...
Likewise,
\begin{equation}\label{dyadicq:eq:qbinomial-inversion}
 \qbinom{n}{k}{1/2}
...
\begin{equation}\label{dyadicq:eq:X-definition}
...
\begin{equation}\label{dyadicq:eq:dn-definition}
  d_n=\E X^n,
  \qquad d_0=1,
\end{equation}
and their exponential generating function
\begin{equation}\label{dyadicq:eq:G-definition}
  G(z)=\E e^{zX}=\sum_{n=0}^{\infty}d_n\frac{z^n}{n!}.
\end{equation}
Independence in \eqref{dyadicq:eq:X-definition} gives the entire product
\begin{equation}\label{dyadicq:eq:G-product}
  G(z)=\prod_{j=1}^{\infty}E\!\left(\frac{z}{2^j}\right),
  \qquad
  E(w)=\frac{e^w-1}{w}.
\end{equation}
Shifting the product by one factor yields
\begin{equation}\label{dyadicq:eq:G-dilation}
  G(2z)=E(z)G(z).
\end{equation}
Comparing coefficients gives the basic moment recurrence
\begin{equation}\label{dyadicq:eq:moment-recurrence}
  (2^n-1)\frac{d_n}{n!}
...
\begin{equation}\label{dyadicq:eq:inverse-dyadic-moment}
...
\begin{boxedremark}
\textbf{Formalization boundary.}
This is a research-frontier article.  Lean formally proves the underlying
probability/CDF and moment identities, the finite translated $q$-binomial
formulae, the source-literal binary and global $q$-series, the ordered
composition formula \eqref{dyadicq:eq:composition-formula}, the centered and shifted
spline bounds, the exact Toeplitz reindexing, row mass, total variation and
qualitative limit, the sinc product and Fourier inversion for $\Up$, and an
equivalent real-line Bromwich formula for $F$.

The exact displayed forms still requiring Lean counterparts are:
\begin{enumerate}
\item the $q=2$ comparison \eqref{dyadicq:eq:qbinomial-inversion}, the operator
  factorizations \eqref{dyadicq:eq:TM-factorization} and \eqref{dyadicq:eq:TM-sinhc}, and the
  generic-$q$ interpolation identities
  \eqref{dyadicq:eq:q-leading-extractor-general}--\eqref{dyadicq:eq:q-survivor};
\item the Appell package \eqref{dyadicq:eq:An-definition}--\eqref{dyadicq:eq:R-superbound},
  its digit formula, uniform and superexponential tail bounds, dyadic
  termination, Appell-compressed finite and global $q$-formulae, and the
  occupied-bit prefix regrouping;
\item the coefficient-extraction and geometric-simplex derivation
  \eqref{dyadicq:eq:an-coeff}--\eqref{dyadicq:eq:geometric-simplex}, the determinant, and the
  complete Bernoulli--cumulant--Bell and partition formulae; the final
  composition formula itself is already formalized;
\item the Toeplitz row polynomial, signed moments and limiting filter, together
  with the explicit rates \eqref{dyadicq:eq:Dn-rate}--\eqref{dyadicq:eq:Dn-centered-rate};
\item the literal CDF sine integral \eqref{dyadicq:eq:F-Fourier} and truncated vertical
  product integral \eqref{dyadicq:eq:Bromwich}, including their convergence and
  substitution steps; and
\item the $O(N^2)$ rational-operation count and the floating-point conditioning
  claims, which need an explicit cost or numerical-error model.
\end{enumerate}
Arguments for those items below are ordinary mathematics, not Lean
certificates.
\end{boxedremark}
...
For $n\ge0$, define
\begin{equation}\label{dyadicq:eq:Rn-definition}
 R_n(y)=
 \sum_{k=0}^{n}
...
Putting $y=0$ in \eqref{dyadicq:eq:Rn-definition} gives
\[
...
\begin{definition}[Moment Appell polynomials]\label{dyadicq:def:Appell}
For $n\ge0$ and $\theta\in\C$, let
\begin{equation}\label{dyadicq:eq:An-definition}
  A_n(\theta)=\E(X+\theta)^n
  =\sum_{k=0}^{n}\binom nk d_{n-k}\theta^k.
\end{equation}
Equivalently,
\begin{equation}\label{dyadicq:eq:Appell-egf}
...
\begin{equation}\label{dyadicq:eq:Appell-derivative}
  A_n'(\theta)=nA_{n-1}(\theta),
  \qquad
  A_n(\theta+c)=\sum_{j=0}^{n}\binom nj A_{n-j}(\theta)c^j.
\end{equation}
Since $X$ and $1-X$ have the same distribution,
\begin{equation}\label{dyadicq:eq:Appell-reflection}
  A_n(\theta)=(-1)^nA_n(-1-\theta).
\end{equation}

\begin{theorem}[Appell compression of the reduction block]\label{dyadicq:thm:Appell-compression}
For every $n\ge0$ and $\theta\in\C$,
\begin{equation}\label{dyadicq:eq:Appell-compression}
  R_n(2^{-n}\theta)
  =2^{-\binom n2}\frac{A_n(\theta)}{n!}.
\end{equation}
Equivalently, for arbitrary $y$,
\begin{equation}\label{dyadicq:eq:R-Appell-general}
  R_n(y)=2^{-\binom n2}\frac{A_n(2^ny)}{n!}.
\end{equation}
\end{theorem}

\begin{proof}
Substitute $y=2^{-n}\theta$ into \eqref{dyadicq:eq:Rn-definition}.  The power of $2$
...
\begin{equation}\label{dyadicq:eq:Appell-bound}
  0\le A_n(\theta)\le 2^n.
\end{equation}
Consequently,
\begin{equation}\label{dyadicq:eq:R-superbound}
  0\le R_n(2^{-n}\theta)
...
all $1$'s,
\begin{equation}\label{dyadicq:eq:binary-x}
  x=0.b_1b_2b_3\ldots{}_2,
  \qquad b_m\in\{0,1\}.
\end{equation}
Let
\begin{equation}\label{dyadicq:eq:prefix-tail}
...
\end{equation}
Then
\begin{equation}\label{dyadicq:eq:bit-prefix-relation}
  a_m=2a_{m-1}+b_m,
  \qquad
...
\begin{equation}\label{dyadicq:eq:source-coefficient}
  c_m(x)=\eps_{a_m}\bigl(2a_{m-1}-a_m\bigr)
  \qquad(m\ge1).
\end{equation}
Using \eqref{dyadicq:eq:bit-prefix-relation}, this simplifies completely:
\begin{equation}\label{dyadicq:eq:coefficient-simplified}
  c_m(x)=
  \begin{cases}
   0,&b_m=0,\\
   \eps_{a_{m-1}},&b_m=1.
...
\begin{theorem}[Digit-sparse Appell expansion]\label{dyadicq:thm:digit-Appell}
For every $x\in[0,1)$,
\begin{equation}\label{dyadicq:eq:digit-Appell}
 \boxed{
 F(x)=
 \sum_{m=1}^{\infty}
...
\]
Insert \eqref{dyadicq:eq:coefficient-simplified}, use
$\eps_{a_{m-1}}=(-1)^{b_1+\cdots+b_{m-1}}$, and then apply
\cref{dyadicq:thm:Appell-compression} with $y_m=2^{-m}\theta_m$.
Absolute and uniform convergence follow from
\eqref{dyadicq:eq:Appell-bound} and the Weierstrass test.
\end{proof}

Formula \eqref{dyadicq:eq:digit-Appell} is a compact arbitrary-argument representation
...
Let $S_N(x)$ denote the truncation of \eqref{dyadicq:eq:digit-Appell} after $m=N$.
From \eqref{dyadicq:eq:Appell-bound},
\[
 |F(x)-S_N(x)|
...
\]
Therefore, for $N\ge0$,
\begin{equation}\label{dyadicq:eq:digit-tail-bound}
 \boxed{
  |F(x)-S_N(x)|
...
\[
 ...
 \qquad 0\le M<2^N.
\]
Then $b_m=0$ for $m>N$, and \eqref{dyadicq:eq:digit-Appell} terminates:
\begin{equation}\label{dyadicq:eq:dyadic-digit-Appell}
 \boxed{
...
\begin{example}[The value at $5/16$]\label{dyadicq:ex:five-sixteen}
...
\]
Since every factor in \eqref{dyadicq:eq:TM-factorization} begins with
$-2^jD$, the operator annihilates all polynomials of degree less than $k$ and
satisfies
\begin{equation}\label{dyadicq:eq:first-TM-moment}
  \sum_{r=0}^{2^k-1}\eps_rr^k
  =(-1)^k2^{\binom k2}k!.
\end{equation}
More generally, for fixed $n$,
\begin{equation}\label{dyadicq:eq:TM-degree-n}
  c\longmapsto
  \frac1{(n+k)!}
  \sum_{r<2^k}\eps_r(r+c)^{n+k}
\end{equation}
has degree at most $n$, and its leading coefficient is
\begin{equation}\label{dyadicq:eq:TM-leading-coeff}
  \coeff{c}{n}\left(
...
\begin{equation}\label{dyadicq:eq:TM-sinhc}
 \Toperator_k
 =(-1)^k2^{\binom k2}D^k
 e^{(2^k-1)D/2}
 \prod_{j=0}^{k-1}
...
\begin{equation}\label{dyadicq:eq:q-leading-extractor-general}
 \coeff{t}{n}P(t)
...
\begin{equation}\label{dyadicq:eq:barycentric-denominator}
...
\begin{equation}\label{dyadicq:eq:finite-q-binomial}
 \sum_{k=0}^{n}(-1)^kq^{\binom k2}\qbinom{n}{k}{q}z^k
 =\poch{z}{q}{n}.
\end{equation}
If $z=q^{-m}$, the right-hand side is zero for $m<n$, while
\begin{equation}\label{dyadicq:eq:q-survivor}
  \poch{q^{-n}}{q}{n}
...
The inner sum \eqref{dyadicq:eq:TM-degree-n} first lowers degree by $k$.  Its remaining
dependence is a polynomial of degree at most $n$.  The outer geometric stencil
\eqref{dyadicq:eq:q-leading-extractor-general} then kills all but one prescribed
...
For every $n\ge0$ and $\theta\in\C$,
\begin{equation}\label{dyadicq:eq:centered-q-Appell}
\boxed{
 \frac{A_n(\theta)}{n!}
...
\cref{dyadicq:thm:Appell-compression}.  Multiplying by $2^{\binom n2}$ changes the
outer power $2^{-n^2}$ into $2^{-\binom{n+1}{2}}$, while the inner shift becomes
$-2^k(1+\theta)$.
\end{proof}

At $\theta=0$, multiplying \eqref{dyadicq:eq:centered-q-Appell} by
$2^{-\binom n2}$ gives the exact centered inverse-dyadic formula
\begin{equation}\label{dyadicq:eq:centered-inverse-dyadic}
 \boxed{
 F(2^{-n})
 =\frac1{2^{n^2}\poch{1/2}{1/2}{n}}
 \sum_{k=0}^{n}
...
\begin{theorem}[Raw translated representation]\label{dyadicq:thm:raw-q-Appell}
For every $n\ge0$ and every $\theta,\eta\in\C$,
\begin{equation}\label{dyadicq:eq:raw-q-Appell}
\boxed{
 \frac{A_n(\theta)}{n!}
...
\begin{equation}\label{dyadicq:eq:raw-inverse-dyadic}
 F(2^{-n})
 =\frac1{(-2)^{n^2}\poch{1/2}{1/2}{n}}
 \sum_{k=0}^{n}
 \frac{\qbinom{n}{k}{1/2}}
...
Because \eqref{dyadicq:eq:raw-q-Appell} is an identity separately for each $n$, one
...
The formula \eqref{dyadicq:eq:centered-inverse-dyadic} treats the numerator $1$.
There is also a one-shot formula for a general numerator.  For
$0\le M<2^N$,
\begin{equation}\label{dyadicq:eq:one-shot-general-dyadic}
\boxed{
 F\!\left(\frac{M}{2^N}\right)
 =\frac1{2^{N^2}\poch{1/2}{1/2}{N}}
 \sum_{k=0}^{N}
...
outer numerator is unchanged.  Formula \eqref{dyadicq:eq:one-shot-general-dyadic}
uses the denominator exponent $N$ once and for all.  In contrast,
\eqref{dyadicq:eq:dyadic-digit-Appell} uses one lower-order block for each $1$-bit of
...
\begin{align}\label{dyadicq:eq:prefix-block-decomposition}
...
\[
 ...
 =\eps_{a_{m-1}}\eps_s.
\]
This proves \eqref{dyadicq:eq:prefix-block-decomposition}.

Substituting \eqref{dyadicq:eq:prefix-block-decomposition} into the one-shot formula
...
produces \eqref{dyadicq:eq:dyadic-digit-Appell}.  Thus:
\begin{itemize}
\item \eqref{dyadicq:eq:one-shot-general-dyadic} groups the computation by polynomial
  degree $N+k$;
\item \eqref{dyadicq:eq:dyadic-digit-Appell} groups it by the occupied binary scales of
  the numerator;
\item \eqref{dyadicq:eq:prefix-block-decomposition} is the combinatorial bridge between
  the two groupings.
\end{itemize}
...
\cref{dyadicq:ex:five-sixteen}.  Their equality is a nontrivial finite rearrangement,
...
\cref{dyadicq:thm:centered-q-Appell} or \cref{dyadicq:thm:raw-q-Appell} into the digit--Appell
series term by term.

\subsection{The centered nested series}

\begin{theorem}[Centered digit--$q$ expansion]\label{dyadicq:thm:centered-digit-q}
For $x\in[0,1)$, with notation \eqref{dyadicq:eq:binary-x}--\eqref{dyadicq:eq:prefix-tail},
\begin{align}\label{dyadicq:eq:centered-digit-q}
 F(x)
 &=\sum_{m=1}^{\infty}
 b_m\eps_{a_{m-1}}
...
\end{align}
The equality holds term by term with \eqref{dyadicq:eq:digit-Appell}.
\end{theorem}

\begin{proof}
In the $m$th term of \eqref{dyadicq:eq:digit-Appell}, multiply the prefactor
$2^{-\binom m2}$ by the prefactor
$2^{-\binom{m+1}{2}}$ in \eqref{dyadicq:eq:centered-q-Appell}.  Their product is
$2^{-m^2}$.  Everything else is a direct substitution.
\end{proof}

If $x=M/2^N$ has a terminating binary expansion, then
\eqref{dyadicq:eq:centered-digit-q} terminates after $m=N$.  The ``infinite'' global
...
\begin{align}\label{dyadicq:eq:raw-digit-q}
 F(x)
 &=\sum_{m=1}^{\infty}
 b_m\eps_{a_{m-1}}
 \frac1{(-2)^{m^2}\poch{1/2}{1/2}{m}}
...
in \eqref{dyadicq:eq:digit-Appell}; no interchange of infinite sums is needed to prove
the identity.
...
$F(2^{-n})$.  The moment product \eqref{dyadicq:eq:G-product} leads to several others,
...
\end{equation}
Then
\begin{equation}\label{dyadicq:eq:an-coeff}
...
\begin{align}\label{dyadicq:eq:scale-selection}
 a_n
 &=\sum_{\ell=1}^{n}
...
\begin{equation}\label{dyadicq:eq:geometric-simplex}
 \sum_{1\le j_1<\cdots<j_\ell}
 2^{-\sum_i j_ir_i}
...
\begin{theorem}[Composition representation]\label{dyadicq:thm:composition}
Let $\rho=(r_1,\ldots,r_\ell)\vDash n$ run over all ordered compositions of
$n$, and let $s_j=r_1+\cdots+r_j$.  Then
\begin{equation}\label{dyadicq:eq:composition-formula}
 \boxed{
 F(2^{-n})
 =2^{-\binom n2}
 \sum_{\rho\vDash n}
...
\eqref{dyadicq:eq:G-product}.

\subsection{A lower-Hessenberg determinant}

The recurrence \eqref{dyadicq:eq:moment-recurrence} may be written
\begin{equation}\label{dyadicq:eq:an-recurrence}
 a_n=\sum_{k=0}^{n-1}\beta_{n,k}a_k,
 \qquad
 \beta_{n,k}=\frac1{(2^n-1)(n-k+1)!}.
\end{equation}
Define the $n\times n$ lower-Hessenberg matrix
\begin{equation}\label{dyadicq:eq:Hn-matrix}
 H_n=
 \begin{pmatrix}
...
\begin{proposition}[Determinant representation]\label{dyadicq:prop:determinant}
For $n\ge1$,
\begin{equation}\label{dyadicq:eq:determinant-formula}
 \boxed{
...
\]
This is exactly \eqref{dyadicq:eq:an-recurrence}, with the same initial value; hence
$D_n=a_n$.
\end{proof}

Expanding the determinant by paths through the nonzero Hessenberg band
recovers the composition sum \eqref{dyadicq:eq:composition-formula}.  Thus the
...
Taking the logarithm of \eqref{dyadicq:eq:G-product} gives another exact description.
The classical expansion
\begin{equation}\label{dyadicq:eq:log-E}
 \log\frac{e^z-1}{z}
 =\frac z2+\sum_{m=1}^{\infty}
 \frac{B_{2m}}{2m(2m)!}z^{2m}
\end{equation}
uses the Bernoulli numbers $B_{2m}$.  Summing over the dyadic scales yields
\begin{equation}\label{dyadicq:eq:log-G}
 \log G(z)
 =\frac z2+
 \sum_{m=1}^{\infty}
...
\begin{equation}\label{dyadicq:eq:cumulants}
 \kappa_1=\frac12,
 \qquad
...
\begin{equation}\label{dyadicq:eq:Bell-definition}
...
\begin{theorem}[Bernoulli--Bell representation]\label{dyadicq:thm:Bell}
\begin{equation}\label{dyadicq:eq:Bell-dyadic}
 \boxed{
 F(2^{-n})
 =\frac{2^{-\binom n2}}{n!}
 \Bcal_n(\kappa_1,\ldots,\kappa_n),}
\end{equation}
where the $\kappa_j$ are given by \eqref{dyadicq:eq:cumulants}.
\end{theorem}
Equivalently, expanding the complete Bell polynomial gives the finite partition
sum
\begin{equation}\label{dyadicq:eq:cumulant-partition}
 F(2^{-n})
 =2^{-\binom n2}
 \sum_{\substack{\nu_1,\ldots,\nu_n\ge0\\
                    \nu_1+2\nu_2+\cdots+n\nu_n=n}}
...
\begin{equation}\label{dyadicq:eq:shifted-spline}
 S_{p,\eta}(x)=
...
\begin{equation}\label{dyadicq:eq:centered-spline-error}
 |S_{p,1/2}(x)-\Fglobal(x)|\le2^{-p}
 \qquad(x\in\R),
\end{equation}
and, for fixed complex shift and $p\ge1$,
\begin{equation}\label{dyadicq:eq:shift-spline-error}
...
Define
\begin{align}\label{dyadicq:eq:Dn-definition}
 D_n(\eta,x)
 &=\frac1{2^{n^2}\poch{1/2}{1/2}{n}}
 \sum_{k=0}^{n}
 \frac{\qbinom{n}{k}{1/2}}
...
\eqref{dyadicq:eq:centered-inverse-dyadic}.  The decisive simplification is an exact
reindexing.

\begin{theorem}[Exact Toeplitz decomposition]\label{dyadicq:thm:Toeplitz-decomp}
Put
\begin{equation}\label{dyadicq:eq:wnj-definition}
 w_{n,j}=
...
\end{equation}
Then
\begin{equation}\label{dyadicq:eq:Toeplitz-decomp}
 \boxed{
...
\begin{proof}
Set $j=n-k$ and $p=n+k=2n-j$ in \eqref{dyadicq:eq:Dn-definition}.  Replacing the
...
\begin{equation}\label{dyadicq:eq:Wn-definition}
  W_n(z)=\sum_{j=0}^{n}w_{n,j}z^j.
\end{equation}
The finite $q$-binomial theorem gives the compact product
\begin{theorem}[Toeplitz row polynomial]\label{dyadicq:thm:row-polynomial}
\begin{equation}\label{dyadicq:eq:row-polynomial}
 \boxed{
...
\begin{theorem}[Uniform rate for fixed shift]\label{dyadicq:thm:Dn-rate}
For $n\ge1$, $x\ge0$, and $\eta\in\C$,
\begin{equation}\label{dyadicq:eq:Dn-rate}
 \boxed{
 |D_n(\eta,x)-\Fglobal(x)|
 \le V_n\left(2^{-n}+2^{1-n}e^{|\eta-1/2|}\right),}
\end{equation}
where $V_n$ is given by \eqref{dyadicq:eq:row-TV}.  In particular,
\begin{equation}\label{dyadicq:eq:Dn-rate-16}
 |D_n(\eta,x)-\Fglobal(x)|
...
\begin{proof}
By \eqref{dyadicq:eq:row-mass} and \cref{dyadicq:thm:Toeplitz-decomp},
\[
 D_n(\eta,x)-\Fglobal(x)
 =\sum_{j=0}^{n}w_{n,j}
...
\eqref{dyadicq:eq:centered-spline-error} and \eqref{dyadicq:eq:shift-spline-error}:
\[
 |S_{p,\eta}(x)-\Fglobal(x)|
 \le2^{-p}+2^{1-p}e^{|\eta-1/2|}
 \le2^{-n}+2^{1-n}e^{|\eta-1/2|}.
\]
Sum against $|w_{n,j}|$ and use \eqref{dyadicq:eq:row-TV}.
\end{proof}
...
sharper bound is
\begin{equation}\label{dyadicq:eq:Dn-centered-rate}
...
\]
independence gives
\begin{equation}\label{dyadicq:eq:up-Fourier}
 \widehat{\Up}(\xi)
 =\prod_{j=0}^{\infty}
...
\begin{equation}\label{dyadicq:eq:F-Fourier}
 \boxed{
...
\eqref{dyadicq:eq:F-Fourier}; their rationality results from highly nonlocal
...
\begin{equation}\label{dyadicq:eq:LX}
 L_X(s)=\E e^{-sX}=G(-s)
 =\prod_{j=1}^{\infty}
...
function gives
\begin{equation}\label{dyadicq:eq:Bromwich}
 \boxed{
 F(x)=\frac1{2\pi i}
 \lim_{T\to\infty}
...
\item compute $d_0,\ldots,d_N$ from the triangular recurrence
  \eqref{dyadicq:eq:moment-recurrence};
\item form $A_m(\theta)$ by Horner evaluation or by the binomial formula
  \eqref{dyadicq:eq:An-definition};
\item sum only over the $1$-bits in \eqref{dyadicq:eq:dyadic-digit-Appell}.
\end{enumerate}

This uses $O(N^2)$ rational arithmetic operations in a straightforward implementation; it
is not a bit-complexity bound.  The method
...
\eqref{dyadicq:eq:dyadic-digit-Appell} collapses at once to
\eqref{dyadicq:eq:inverse-dyadic-moment}.  The recurrence, composition sum,
...
\eqref{dyadicq:eq:digit-Appell} is attractive because:
\begin{itemize}
\item zero bits cost nothing;
\item each active term needs one polynomial value $A_m(\theta_m)$;
\item the uniform remainder bound \eqref{dyadicq:eq:digit-tail-bound} is
  superexponentially small;
\item no cancellation inside a $2^k$-term Thue--Morse block is required.
\end{itemize}

The centered nested $q$-series \eqref{dyadicq:eq:centered-digit-q} is mathematically
...
\eqref{dyadicq:eq:raw-digit-q} can improve conditioning, although exact rational or
ball arithmetic is preferable when evaluating the nested sums directly.

The discrete limit \eqref{dyadicq:eq:Dn-definition} supplies an independent numerical
route and a global approximation to $\Fglobal$.  Its error is controlled by
\eqref{dyadicq:eq:Dn-rate}; the centered shift $\eta=1/2$ removes the shift-error term
entirely, giving the sharper estimate \eqref{dyadicq:eq:Dn-centered-rate}, and is
...
The random series \eqref{dyadicq:eq:X-definition} gives the entire moment product
\eqref{dyadicq:eq:G-product}.  Its coefficients are $d_n$, its logarithm gives the
cumulants \eqref{dyadicq:eq:cumulants}, and its dilation gives the recurrence
\eqref{dyadicq:eq:moment-recurrence}.
...
\item Follow the binary digits of $x$ and sum exact Appell blocks.  This yields
  \eqref{dyadicq:eq:digit-Appell}; substitution of the finite $q$-identity yields
  \eqref{dyadicq:eq:centered-digit-q} and \eqref{dyadicq:eq:raw-digit-q}.  Dyadic rationals are
...
\item Truncate Thue--Morse power sums at the spatial point $x$, interpret them
  ...
  \eqref{dyadicq:eq:row-polynomial} makes the $q$-Pochhammer structure explicit, while
  \eqref{dyadicq:eq:Dn-rate} proves convergence quantitatively.
\end{enumerate}

The central identities can be compressed into the following chain:
\begin{equation}\label{dyadicq:eq:final-chain}
\boxed{
\begin{aligned}
 F(2^{-n})
...
\]
which is \eqref{dyadicq:eq:geometric-simplex}.

\section{First moments and inverse-dyadic values}

The recurrence \eqref{dyadicq:eq:moment-recurrence} gives:
\begin{center}
...
source files in this \texttt{ProveIt} repository directory:

\begin{center}
\path{Analysis/FabiusFunction}
\end{center}

\renewcommand{\arraystretch}{1.16}
\begin{longtable}{@{}>{\ttfamily\raggedright\arraybackslash}p{0.47\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}@{}}
\toprule
...
ProbabilityRepresentation.lean
 & random dyadic series and the exact identification of its CDF with the
   Fabius function\\
FabiusComplexMGF.lean
 & probabilistic moment-generating function and its identification with the
   canonical complex generating function\\
NegativeLaplaceVertical.lean
 & the canonical dyadic infinite product and its negative-Laplace sign
   structure\\
FabiusRecurrenceSequence.lean
 & moment recurrence and exact inverse-dyadic values\\
FabiusInverseDyadicClosedForm.lean
 & path sums, ordered compositions, and the exact theorem
   \path{fabiusAtInverseTwoPow_eq_composition_formula}\\
HalfQBinomial.lean
...
FabiusQBinomialScalarFormula.lean
 & scalar-valued centered formulae over real and complex targets\\
FabiusRawQBinomialScalar.lean
 & scalar-valued raw translated formulae\\
FabiusDyadicQBinomialScalar.lean
 & general dyadic formulae with arbitrary common real or complex translation\\
...
FabiusComputability.lean
 & the exact global centered bound
   \path{abs_fabiusUniformSpline_sub_extendedFabius_le}\\
...
FabiusDiscreteLimitToeplitz.lean
 & exact Toeplitz weights, row mass, and total variation; not the article's
   row polynomial, limiting filter, or quantitative rate\\
FabiusDiscreteLimitIntegration.lean
 & exact reindexing of the discrete formula and convergence to the global
   Fabius extension, qualitatively rather than with the displayed rate\\
FourierProduct.lean
 & the sinc product for the Rvachev density\\
FourierAnalytic.lean
 & rapid decay of the Fourier transform\\
PoissonSummation.lean
 & Fourier inversion for the density; not the article's literal CDF sine
   integral\\
FabiusBromwichInput.lean
 & an equivalent exact real-line Bromwich formula; not the displayed
   truncated vertical-contour product formula\\
\end{longtable}

\renewcommand{\refname}{Dossier references}
\begin{thebibliography}{9}

\bibitem{dyadicq:Companion}
V.~Reshetnikov,
\emph{The Fabius Function and Rvachev's up-Function},
companion article in the \texttt{ProveIt} repository, 2026.

\bibitem{dyadicq:ProveIt}
V.~Reshetnikov,
\emph{ProveIt: formal developments for the Fabius function},
\url{https://github.com/VladimirReshetnikov/ProveIt/tree/main/Analysis/FabiusFunction}.

\bibitem{dyadicq:GasperRahman}
G.~Gasper and M.~Rahman,
\emph{Basic Hypergeometric Series}, 2nd ed.,
Cambridge University Press, 2004.

\bibitem{dyadicq:Comtet}
L.~Comtet,
\emph{Advanced Combinatorics},
D.~Reidel, 1974.

\bibitem{dyadicq:Hardy}
G.~H.~Hardy,
...
\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Fabius_Dyadic_q_Connections/Fabius_Dyadic_q_Connections.tex


% BEGIN MERGED SOURCE: Fabius_Dyadic_Formulae_and_Alternative_Representations/Fabius_Dyadic_Formulae_and_Alternative_Representations.tex
% Original SHA-256: 462276b10fcd32b0446deb7cfedc4ec07c2ae55dbd333d5ff9b1d98f07df89e1
\clearpage
\part{The Dyadic Web of the Fabius Function}
\label{part:dyadicweb}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Fabius_Dyadic_Formulae_and_Alternative_Representations/Fabius_Dyadic_Formulae_and_Alternative_Representations.tex}.  Preferred dyadic backbone: alternative exact representations, q-Richardson filtering, quantitative limits, and exact stabilization.  All claims lacking an exact audited Lean declaration remain frontier obligations.\par\smallskip Original source SHA-256: \nolinkurl{462276b10fcd32b0446deb7cfedc4ec07c2ae55dbd333d5ff9b1d98f07df89e1}.
\end{boxedremark}
\section*{Dossier abstract}
...
\begin{align}
 ...
 &\qquad\times
 \sum_{r=0}^{m2^k-1}\tm_r(r-m2^k+Q)^{n+k},               \label{dyadicweb:eq:input-q-dyadic}
\end{align}
...
Consequently
\begin{equation}
 d_n=\E(Y^n),\qquad c_k=\E(Z^{2k}),\qquad \E(Z^{2k+1})=0. \label{dyadicweb:eq:prob-moments}
\end{equation}
...
\begin{equation}
 \boxed{\mathscr A(X)=\e^{X/2}\mathscr C(X/2).}           \label{dyadicweb:eq:A-C-transform}
\end{equation}
...
\begin{align}
 ...
 &=\frac{2^{-\binom n2}}{n!}
   \sum_{h=0}^{m-1}\tm_h\,
   \E\bigl(m-1-h+Y\bigr)^n                              \label{dyadicweb:eq:expectation-Y}\\
 &=\frac{2^{-\binom{n+1}{2}}}{n!}
   \sum_{h=0}^{m-1}\tm_h\,
   \E\bigl(2m-2h-1+Z\bigr)^n.                            \label{dyadicweb:eq:expectation-Z}
\end{align}
\end{corollary}
...
\begin{equation}
 ...
 =\frac{2^{-\binom n2}}{2\pi\ii}
  \int_{|X|=\rho}\frac{B_m(X)\mathscr A(X)}{X^{n+1}}\dd X. \label{dyadicweb:eq:cauchy-X}
\end{equation}
\end{corollary}
...
\begin{equation}
 d_n=\lim_{J\to\infty}\int_{[0,1]^J}
       ...
       \dd u_1\cdots\dd u_J.                              \label{dyadicweb:eq:cube-d}
\end{equation}
\end{proposition}
...
\begin{equation}
 ...
 \qquad
 N_p(x)=\max\!\left(0,\left\lfloor2^px+\frac12\right\rfloor\right). \label{dyadicweb:eq:centered-spline-local}
\end{equation}
At degree zero set
\begin{equation}
 S_0(x)=\sum_{r=0}^{N_0(x)-1}\tm_r.                        \label{dyadicweb:eq:S0-local}
\end{equation}
The companion article proves the global estimate
\begin{equation}
 \sup_{x\in\R}|S_p(x)-\mathcal F(x)|\le2^{-p}.             \label{dyadicweb:eq:centered-global-error-input}
\end{equation}
At the matched grid point $m/2^p$ the cutoff is $m$ and
\begin{equation}
 S_p(m/2^p)=\frac{1}{2^{\binom{p+1}{2}}p!}
 \sum_{h=0}^{m-1}\tm_h(2m-2h-1)^p.                       \label{dyadicweb:eq:matched-spline}
\end{equation}
...
\begin{equation}
 ...
 \mathcal F\!\left(\frac{m}{2^{n-2k}}\right).}            \label{dyadicweb:eq:reciprocal-spline}
\end{equation}
\end{theorem}
...
\begin{equation}
 ...
 \qbinom nk{1/2}z^k.                                      \label{dyadicweb:eq:finite-q-binomial-local}
\end{equation}
Put
\begin{equation}
 x_k=-2^k,
...
\begin{equation}
 [z^n]P(z)=\Dq_n[P]
 =\frac{1}{2\pi\ii}\int_\Gamma\frac{P(z)}{\Pi_n(z)}\dd z. \label{dyadicweb:eq:q-residue-extractor}
\end{equation}
\end{proposition}
...
\begin{equation}
 ...
 =2^{\binom n2}\mathcal F(m/2^n).                         \label{dyadicweb:eq:Rmn-leading}
\end{equation}
\end{lemma}
...
\begin{equation}
 ...
 \frac{2^{-\binom n2}}{2\pi\ii}
 \int_\Gamma
 \frac{r_{m,n}(z)}{\prod_{k=0}^{n}(z+2^k)}\dd z,}          \label{dyadicweb:eq:q-node-contour}
\end{equation}
...
\begin{equation}
 C_k(X)=\sum_{r=0}^{2^k-1}\tm_r\e^{(r-2^k)X}.              \label{dyadicweb:eq:Ck-local}
\end{equation}
...
\begin{equation}
 ...
 \e^{-X}\prod_{j=0}^{k-1}E_0(-2^jX).                     \label{dyadicweb:eq:Ck-factor-local}
\end{equation}
...
\begin{equation}
 ...
 =\e^{-X}\prod_{j=0}^{k-1}E_0(-2^jX).                    \label{dyadicweb:eq:A-geometric-bridge}
\end{equation}
...
\begin{equation}
 ...
 \sum_{r=0}^{m2^k-1}\tm_r(r-m2^k)^{n+k}.                 \label{dyadicweb:eq:node-value-power-sum}
\end{equation}
...
\begin{equation}
 ...
 \sum_{k=0}^{n}
 \frac{\qbinom nk{1/2}}{4^{\binom k2}(n+k)!}
 \sum_{r=0}^{m2^k-1}\tm_r(r-m2^k+Q)^{n+k}                 \label{dyadicweb:eq:Q-polynomial}
\end{equation}
is constant.
\end{proposition}
...
\begin{equation}
 ...
 \sum_{k=0}^{n}
 \frac{\qbinom nk{1/2}}{4^{\binom k2}(n+k)!}
 \sum_{r=0}^{2^k-1}\tm_r(r-2^k+Q)^{n+k}.                  \label{dyadicweb:eq:NnQ-local}
\end{equation}
...
\begin{equation}
 S_p^{(Q)}(x)=
 \frac{(-1)^p}{2^{\binom p2}p!}
 \sum_{r=0}^{N_p(x)-1}\tm_r(r-2^px+Q)^p.                  \label{dyadicweb:eq:shifted-spline-local}
\end{equation}
...
\begin{equation}
 ...
 \le2^{-(p-1)}\bigl(\e^{|Q-1/2|}-1\bigr).                 \label{dyadicweb:eq:shifted-spline-bound-local}
\end{equation}
Together with \eqref{dyadicweb:eq:centered-global-error-input},
\begin{equation}
 \bigl|S_p^{(Q)}(x)-\mathcal F(x)\bigr|
 \le(2\e^{|Q-1/2|}-1)2^{-p}.                              \label{dyadicweb:eq:shifted-total-error}
\end{equation}
...
\begin{equation}
 \mathcal D_n^{(Q)}(x)\longrightarrow\mathcal F(x).       \label{dyadicweb:eq:DnQ-limit-local}
\end{equation}
...
\begin{equation}
 \boxed{\mathcal D_n^{(Q)}(x)=\mathcal F(x).}              \label{dyadicweb:eq:dyadic-stabilization}
\end{equation}
...
\begin{equation}
 ...
 \le H_n\bigl(2\e^{|Q-1/2|}-1\bigr)4^{-n}.}              \label{dyadicweb:eq:D-error}
\end{equation}
Moreover $H_n\le64$, so the convergence is uniformly $\bigO_Q(4^{-n})$ on the whole real
line.  At the centered shift,
\begin{equation}
 |\mathcal D_n^{(1/2)}(x)-\mathcal F(x)|\le H_n4^{-n}.     \label{dyadicweb:eq:D-centered-error}
\end{equation}
\end{theorem}
...
\begin{equation}
 ...
 \le2H_n\bigl(\e^{|Q-1/2|}-1\bigr)4^{-n}.                \label{dyadicweb:eq:D-Q-decay}
\end{equation}
\end{corollary}
...
\begin{equation}
 \boxed{
 \widetilde{\mathcal D}_n^{(Q)}(x)
 =\mathcal F\!\left(\frac{M_n(x)}{2^n}\right).}           \label{dyadicweb:eq:synchronized-exact}
\end{equation}
...
\begin{equation}
 N_{n+k}(x)
 =M_n(x)2^k+\left\lfloor2^k\theta_n(x)+\frac12\right\rfloor, \label{dyadicweb:eq:cutoff-defect}
\end{equation}
...
\eqref{dyadicweb:eq:DnQ-limit-local} both evaluate $\mathcal F$, but their relation is not that of an
...
\begin{align}
 ...
 &=\frac{1}{2^{s^2}(1/2;1/2)_s}
   ...
   \sum_{r=0}^{m2^k-1}\tm_r(r-m2^k+Q)^{s+k}              \label{dyadicweb:eq:chain-q}\\
 &=\sum_{k=0}^{\lfloor s/2\rfloor}
...
\begin{equation}
 B_5(X)=\e^{4X}-\e^{3X}-\e^{2X}+\e^X-1.                  \label{dyadicweb:eq:B5}
\end{equation}
Using
\(
...
\path{FabiusQBinomialFormula.lean} & Centered and translated finite $q$-binomial formulas, arbitrary numerators, translation invariance, block/refinement normal forms, and representation independence. \\
...
\item the explicit $\bigO_Q(4^{-n})$ bound \eqref{dyadicweb:eq:D-error};
...
\label{dyadicweb:app:identity-sheet}
...
\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Fabius_Dyadic_Formulae_and_Alternative_Representations/Fabius_Dyadic_Formulae_and_Alternative_Representations.tex

% BEGIN MERGED SOURCE: Fabius_Dyadic_Asymptotic_Bridge/Fabius_Dyadic_Asymptotic_Bridge.tex
% Original SHA-256: c5ad7c5298d958ab63f459a3e246c2293d3020525223bc05ef2d0e1edab58f10
\clearpage
\part{From Exact Dyadic Arithmetic to Endpoint Asymptotics}
\label{part:dyadicasymbridge}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Fabius_Dyadic_Asymptotic_Bridge/Fabius_Dyadic_Asymptotic_Bridge.tex}.  Coefficient-extraction and fixed-numerator bridge from exact arithmetic to the lower-Lambert expansion.  It overlaps both the Dyadic Web and the later dyadic-asymptotic dossier below; retain it for its vertical-saddle, phase-locking, and Bromwich-to-coefficient derivations.  Claims lacking an exact named declaration remain formal obligations.\par\smallskip Original source SHA-256: \nolinkurl{c5ad7c5298d958ab63f459a3e246c2293d3020525223bc05ef2d0e1edab58f10}.
\end{boxedremark}
\section*{Dossier abstract}
...
\label{dyadicasymbridge:sec:introduction}

The formal development in the repository \cite{dyadicasymbridge:proveit} contains two powerful bodies of results.
On the arithmetic side, it proves exact rational formulae for $F(m/2^n)$, develops half moments and their recurrences, gives a finite composition formula for $F(2^{-n})$, and derives a global Gaussian-binomial formula at base $q=1/2$.  On the analytic side, it derives a corrected sharp expansion of $F(x)$ as $x\downarrow0$, including a nonconstant periodic function with explicit Gamma--zeta Fourier coefficients and an all-orders saddle series.  The accompanying exposition \cite{dyadicasymbridge:masterdoc} develops both sides in detail, but it does not turn the exact dyadic identities themselves into a derivation of the endpoint asymptotic.
...
\end{aligned}}
\label{dyadicasymbridge:eq:bridge-chain}
\end{equation}
...
\label{dyadicasymbridge:eq:lambda-def}
\end{equation}
where $W_{-1}$ is the lower real branch of Lambert's $W$-function \cite{dyadicasymbridge:corless1996}.  Define
\begin{equation}
 \Mmain(x)=
 -\frac L2\lambda(x)^2+
 \left(1+\frac L2\right)\lambda(x)
 -\frac12\log\lambda(x)+C_F+\Psi(\lambda(x)),
\label{dyadicasymbridge:eq:main-M}
\end{equation}
where $C_F$ and the smooth mean-zero $1$-periodic function $\Psi$ are specified in
\cref{dyadicasymbridge:sec:periodic-decomposition}.
...
\label{dyadicasymbridge:thm:main-inverse}
Let $\lambda_n=\lambda(2^{-n})$, equivalently
\begin{equation}
 \lambda_n-\log_2\lambda_n=n.
\label{dyadicasymbridge:eq:lambda-n-equation}
\end{equation}
Then, as $n\to\infty$,
\begin{equation}
 \log F(2^{-n})=\Mmain(2^{-n})+\bigO(\lambda_n^{-1}).
\label{dyadicasymbridge:eq:main-inverse-leading}
\end{equation}
...
\label{dyadicasymbridge:eq:random-series}
\end{equation}
...
\begin{equation}
 ...
 \qquad a_0=1.
\label{dyadicasymbridge:eq:a-recurrence}
\end{equation}
...
\begin{equation}
 ...
 =2^{-\binom n2}a_n.}
\label{dyadicasymbridge:eq:inverse-exact}
\end{equation}
...
\label{dyadicasymbridge:eq:normalized-triangular}
\end{equation}
...
\begin{equation}
 ...
 \frac1{(2^{s_j}-1)(r_j+1)!},
\label{dyadicasymbridge:eq:composition-formula}
\end{equation}
where $\rho=(r_1,\ldots,r_\ell)$ is an ordered composition of $n$ and
$s_j=r_1+\cdots+r_j$.  Formula \eqref{dyadicasymbridge:eq:composition-formula} is exact and positive, but it contains $2^{n-1}$ terms.  Its asymptotic structure becomes visible only after the path sum is resummed into a generating function; see \cref{dyadicasymbridge:subsec:composition-resummation}.

\subsection{The global \texorpdfstring{$q$}{q}-binomial formula}
\label{dyadicasymbridge:subsec:qformula}

For
\begin{equation}
 (a;q)_n=\prod_{j=0}^{n-1}(1-aq^j)
\label{dyadicasymbridge:eq:qpochhammer}
\end{equation}
...
\begin{align}
 ...
 (r-m2^k+Q)^{n+k},
\label{dyadicasymbridge:eq:qbinomial-global}
\end{align}
independently of the complex translation parameter $Q$.  For
$0\le m\le2^n$, the left side is the bounded Fabius value $F(m/2^n)$.

At first sight \eqref{dyadicasymbridge:eq:qbinomial-global} seems to offer a direct route to an asymptotic: the prefactor contains $2^{-n^2}$ and the finite product
...
\begin{equation}
 ...
 \qquad B_0=0,
\label{dyadicasymbridge:eq:Bm-def}
\end{equation}
and the Mersenne product
\begin{equation}
 \mathfrak m_n=\prod_{\ell=1}^n(2^\ell-1).
\label{dyadicasymbridge:eq:mersenne-product}
\end{equation}
The exact finite $q$-binomial extraction theorem gives
\begin{equation}
 \boxed{
 F(m/2^n)=2^{-\binom n2}[z^n]\bigl(B_m(z)G(z)\bigr).}
\label{dyadicasymbridge:eq:prefix-coefficient}
\end{equation}
For $m=1$, $B_1=1$ and this reduces to \eqref{dyadicasymbridge:eq:inverse-exact}.  The reason the normalizer disappears is
\begin{equation}
 (1/2;1/2)_n=2^{-\binom{n+1}{2}}\mathfrak m_n,
\label{dyadicasymbridge:eq:q-mersenne}
\end{equation}
and the centered numerator of \eqref{dyadicasymbridge:eq:qbinomial-global} is exactly
\begin{equation}
 \mathfrak m_n[z^n]\bigl(B_m(z)G(z)\bigr).
\label{dyadicasymbridge:eq:q-numerator-collapse}
\end{equation}
Thus
\begin{equation}
 \frac{\mathfrak m_n}{2^{n^2}(1/2;1/2)_n}
 =2^{-\binom n2}.
\label{dyadicasymbridge:eq:q-normalizer-cancel}
\end{equation}
The $q$-Pochhammer factor supplies the same triangular power already present in
\eqref{dyadicasymbridge:eq:inverse-exact}; the remaining endpoint asymptotic resides in a coefficient of the entire product $G$.

\begin{remark}[Why termwise estimates fail]
The individual terms in \eqref{dyadicasymbridge:eq:qbinomial-global} are vastly larger than their sum.  The inner Thue--Morse block annihilates low polynomial degrees, and the outer Gaussian row annihilates the remaining lower powers on the geometric node set $-1,-2,\ldots,-2^n$.  Estimating before these exact cancellations discards the quantity one is trying to compute.  Formula \eqref{dyadicasymbridge:eq:prefix-coefficient} is therefore not a cosmetic rewrite; it is the asymptotically meaningful normal form.
\end{remark}

\section{Resumming the exact recurrences}
\label{dyadicasymbridge:sec:resummation}

\subsection{From the triangular recurrence to the entire product}
\label{dyadicasymbridge:subsec:composition-resummation}

Let
\begin{equation}
 \mathscr A(z)=\sum_{n\ge0}a_nz^n=G(z).
\label{dyadicasymbridge:eq:A-series}
\end{equation}
Multiplying \eqref{dyadicasymbridge:eq:a-recurrence} by $z^n$ and summing over $n\ge1$ gives
\begin{align*}
...
\label{dyadicasymbridge:eq:A-functional}
\end{equation}
This is exactly \eqref{dyadicasymbridge:eq:G-functional}.  Iteration toward the origin yields the locally uniformly convergent product
\begin{equation}
 \boxed{
 G(z)=\prod_{j=1}^{\infty}
 \frac{\e^{z/2^j}-1}{z/2^j}.}
\label{dyadicasymbridge:eq:G-product}
\end{equation}

The path sum \eqref{dyadicasymbridge:eq:composition-formula}, the recurrence
\eqref{dyadicasymbridge:eq:normalized-triangular}, the functional equation \eqref{dyadicasymbridge:eq:A-functional}, and the product \eqref{dyadicasymbridge:eq:G-product} are four presentations of one object.  The first two are finite and exact at each $n$; the last two are the forms suited to asymptotics.
...
\label{dyadicasymbridge:eq:uniform-mgf}
\end{equation}
Thus \eqref{dyadicasymbridge:eq:G-product} is precisely the moment-generating function of the random series \eqref{dyadicasymbridge:eq:random-series}.  This gives several useful estimates for free.  For real $r>0$ and real $t$,
\begin{equation}
 |G(r+\ii t)|\le G(r),
\label{dyadicasymbridge:eq:G-vertical-bound}
\end{equation}
...
\label{dyadicasymbridge:eq:Lambda-def}
\end{equation}
Using \eqref{dyadicasymbridge:eq:G-symmetry},
\begin{equation}
 \log G(s)=s+\Lambda(s).
\label{dyadicasymbridge:eq:positive-negative-log}
\end{equation}
At the product level,
\begin{align}
 G(-s)
 &=\prod_{j=1}^{\infty}
 \frac{1-\e^{-s/2^j}}{s/2^j},                              \label{dyadicasymbridge:eq:negative-product}\\
 \Lambda(s)
 &=\sum_{j=1}^{\infty}\kappa(s/2^j),
 \qquad
 \kappa(u)=\log\frac{1-\e^{-u}}u.                         \label{dyadicasymbridge:eq:Lambda-harmonic}
\end{align}
...
\label{dyadicasymbridge:sec:periodic-decomposition}

This section records the analytic information about \eqref{dyadicasymbridge:eq:Lambda-harmonic} needed by the coefficient saddle.  The derivation is included because it identifies exactly where the $q=1/2$ geometry reappears in the asymptotic.
...
\begin{equation}
 ...
 =-\Gamma(z)\zeta(1+z).
\label{dyadicasymbridge:eq:kappa-mellin}
\end{equation}
Indeed, $\kappa(u)=-u/2+\bigO(u^2)$ at the origin and
$\kappa(u)=-\log u+\bigO(\e^{-u})$ at infinity.  Integration by parts, followed by the subtracted Bose integral, gives \eqref{dyadicasymbridge:eq:kappa-mellin}.
...
\begin{equation}
 ...
 \qquad -1<\Re z<0.
\label{dyadicasymbridge:eq:Lambda-mellin}
\end{equation}
The factor
\begin{equation}
 \frac{2^z}{1-2^z}
\label{dyadicasymbridge:eq:geometric-mellin-factor}
\end{equation}
...
\begin{equation}
 ...
 \qquad k\in\Z.
\label{dyadicasymbridge:eq:chi-k}
\end{equation}
...
\begin{equation}
 ...
 +P(\log_2s)+E(s),
\label{dyadicasymbridge:eq:Lambda-periodic}
\end{equation}
where
\begin{equation}
 0\le E(s)\le4\e^{-s}
\label{dyadicasymbridge:eq:E-bound}
\end{equation}
...
\label{dyadicasymbridge:eq:Psi-def}
\end{equation}
Then
\begin{equation}
...
\label{dyadicasymbridge:eq:Psi-Fourier}
\end{equation}
...
\label{dyadicasymbridge:sec:vertical-coeff}
...
\label{dyadicasymbridge:lem:vertical-extraction}
...
\begin{equation}
 ...
 \frac{G(z)}{z^{n+1}}\dd z.
\label{dyadicasymbridge:eq:vertical-extraction}
\end{equation}
The integral is absolutely convergent.
\end{lemma}

\begin{proof}
On the line $\Re z=c$, \eqref{dyadicasymbridge:eq:G-vertical-bound} gives
...
Combining \eqref{dyadicasymbridge:eq:inverse-exact} and \eqref{dyadicasymbridge:eq:vertical-extraction} gives the exact contour formula
\begin{equation}
 F(2^{-n})=
 \frac{2^{-\binom n2}}{2\pi\ii}
 \int_{c-\ii\infty}^{c+\ii\infty}
 \frac{G(z)}{z^{n+1}}\dd z.
\label{dyadicasymbridge:eq:inverse-vertical}
\end{equation}
...
\label{dyadicasymbridge:sec:inverse-saddle}

\subsection{The Lambert coordinate is forced by coefficient balance}

Let $\lambda=\lambda_n$ be the large solution of \eqref{dyadicasymbridge:eq:lambda-n-equation}.  It is not introduced by analogy with the continuous proof.  It is the leading solution of the coefficient saddle equation.

Indeed, by \eqref{dyadicasymbridge:eq:positive-negative-log} and \eqref{dyadicasymbridge:eq:Lambda-periodic},
\begin{equation}
 \log G(r)=
 r-\frac{(\log r)^2}{2L}+\frac{\log r}{2}
 +P(\log_2r)+E(r).
\label{dyadicasymbridge:eq:logG-positive}
\end{equation}
The logarithmic derivative satisfies
\begin{equation}
 r\frac{G'(r)}{G(r)}
 =r-\log_2r+\frac12+\frac{\Psi'(\log_2r)}L+\bigO(r\e^{-r}).
\label{dyadicasymbridge:eq:coefficient-saddle-exactish}
\end{equation}
...
Set $c=\lambda$ in \eqref{dyadicasymbridge:eq:inverse-vertical} and write
...
\label{dyadicasymbridge:eq:Cn-def}
\end{equation}
Then
\begin{equation}
 F(2^{-n})=
...
\label{dyadicasymbridge:eq:exact-coefficient-saddle}
\end{equation}
...
\label{dyadicasymbridge:eq:exact-log-coefficient}
\end{equation}

\subsection{Phase locking}
\label{dyadicasymbridge:subsec:phase-locking}

The Lambert equation gives the exact integer relation
\begin{equation}
 \log_2\lambda=\lambda-n.
\label{dyadicasymbridge:eq:phase-lock}
\end{equation}
Since $\Psi$ is $1$-periodic,
\begin{equation}
 \boxed{\Psi(\log_2\lambda)=\Psi(\lambda).}
\label{dyadicasymbridge:eq:Psi-phase-lock}
\end{equation}
...
Substitute \eqref{dyadicasymbridge:eq:Lambda-periodic} into \eqref{dyadicasymbridge:eq:exact-log-coefficient}.  Put
$q=\log\lambda=L(\lambda-n)$.  The nonconstant part becomes $\Psi(\lambda)$ by \eqref{dyadicasymbridge:eq:Psi-phase-lock}, and the polynomial terms satisfy the exact cancellation
\begin{align}
 &-\frac L2(n^2-n)+\lambda-
 \frac{q^2}{2L}+\frac q2-nq                         \notag\\
 &\hspace{4em}=-\frac L2\lambda^2+
 \left(1+\frac L2\right)\lambda.
\label{dyadicasymbridge:eq:key-algebra-cancellation}
\end{align}
Together with \eqref{dyadicasymbridge:eq:CF}, this gives an exact separation analogous to the one in the continuous saddle proof.

\begin{proposition}[Exact dyadic separation]
\label{dyadicasymbridge:prop:exact-dyadic-separation}
For $\lambda=\lambda_n$,
\begin{equation}
 \boxed{
 \log F(2^{-n})-\Mmain(2^{-n})
 =E(\lambda)+\log\CoeffMass_n.}
\label{dyadicasymbridge:eq:exact-dyadic-separation}
\end{equation}
\end{proposition}
...
\label{dyadicasymbridge:subsec:phase-identity}

Let
\begin{equation}
 u=\Log(1+\ii\theta).
\label{dyadicasymbridge:eq:u-def}
\end{equation}
On a fixed neighborhood of $\theta=0$, the complex continuation of
\eqref{dyadicasymbridge:eq:Lambda-periodic} is valid.  The exponent in \eqref{dyadicasymbridge:eq:Cn-def} is
\begin{equation}
 \Phi^{\mathrm{coef}}_\lambda(\theta)=
 \log\frac{G(\lambda(1+\ii\theta))}{G(\lambda)}-(n+1)u.
\label{dyadicasymbridge:eq:coef-phase-def}
\end{equation}
...
\label{dyadicasymbridge:eq:coefficient-local-phase}
\end{align}
...
Formula \eqref{dyadicasymbridge:eq:coefficient-local-phase} is the decisive identity.  Apart from an exponentially small tail, it is exactly the phase obtained by scaling the continuous Bromwich integral at its saddle.  Therefore every local Gaussian polynomial and every all-orders correction coefficient must coincide.
...
\label{dyadicasymbridge:eq:log-expansion}
\end{equation}
the leading term in \eqref{dyadicasymbridge:eq:coefficient-local-phase} is
$-\lambda\theta^2/2$.

The tails are especially simple in the vertical representation.  By
\eqref{dyadicasymbridge:eq:G-vertical-bound},
\begin{equation}
 \left|
 \frac{G(\lambda(1+\ii\theta))}{G(\lambda)}
 (1+\ii\theta)^{-n-1}
 \right|
 \le(1+\theta^2)^{-(n+1)/2}.
\label{dyadicasymbridge:eq:coefficient-tail-bound}
\end{equation}
Since $n\sim\lambda$, this gives Gaussian decay near the origin and exponential decay outside any fixed neighborhood.  Taylor expansion on a window growing more slowly than $\lambda^{1/6}$, followed by \eqref{dyadicasymbridge:eq:coefficient-tail-bound}, gives
\begin{equation}
 \CoeffMass_n=1+\bigO(\lambda^{-1}).
\label{dyadicasymbridge:eq:Cn-leading}
\end{equation}
The order $\lambda^{-1/2}$ term vanishes after integration because it is odd and purely imaginary.

Combining \eqref{dyadicasymbridge:eq:exact-dyadic-separation}, \eqref{dyadicasymbridge:eq:E-bound}, and
\eqref{dyadicasymbridge:eq:Cn-leading} proves \eqref{dyadicasymbridge:eq:main-inverse-leading}.

\subsection{First correction}

Substitute $\theta=v/\sqrt\lambda$ into \eqref{dyadicasymbridge:eq:coefficient-local-phase}.  Through order
$\lambda^{-1}$,
\begin{equation}
...
\label{dyadicasymbridge:eq:phase-first-expansion}
\end{equation}
where
\begin{align}
 P_1(v;u)
 ...
 \right],                                                   \label{dyadicasymbridge:eq:P1}\\
 P_2(v;u)
 &=\frac{v^4}{4}+
 \left[-\frac14+\frac1{2L}+\frac{\Psi'(u)}{2L}
 -\frac{\Psi''(u)}{2L^2}\right]v^2.                       \label{dyadicasymbridge:eq:P2}
\end{align}
...
\begin{equation}
 ...
 =\frac1{24}+\frac1{2L}
 -\frac{\Psi''(u)+\Psi'(u)^2}{2L^2}.
\label{dyadicasymbridge:eq:first-Gaussian-contraction}
\end{equation}
This is precisely $A_1(u)$ in \eqref{dyadicasymbridge:eq:A1}.  Since the logarithm of
$1+A_1/\lambda+\bigO(\lambda^{-2})$ has the same first coefficient,
\begin{equation}
 \log\CoeffMass_n=
 \frac{A_1(\lambda_n)}{\lambda_n}+\bigO(\lambda_n^{-2}).
\label{dyadicasymbridge:eq:Cn-first}
\end{equation}
...
At order $N$, expand \eqref{dyadicasymbridge:eq:coefficient-local-phase} after the central scaling through
...
\end{equation}
After exponentiation, all odd powers of $\lambda^{-1/2}$ integrate to zero by parity.  Gaussian moments convert the remaining terms into integer powers of $1/\lambda$.  The tail bound \eqref{dyadicasymbridge:eq:coefficient-tail-bound} justifies termwise integration and gives an arbitrarily small remainder.  Consequently
\begin{equation}
 \log\CoeffMass_n=
 \sum_{j=1}^{N-1}\frac{A_j(\lambda_n)}{\lambda_n^j}
 +\bigO_N(\lambda_n^{-N}),
\label{dyadicasymbridge:eq:Cn-all-orders}
\end{equation}
with the same $A_j$ as in the continuous saddle expansion, because the local phase is the same.  This completes the proof of \cref{dyadicasymbridge:thm:main-inverse}.

\section{The elementary \texorpdfstring{$n$}{n}-scale expansion}
\label{dyadicasymbridge:sec:n-scale}
...
\eqref{dyadicasymbridge:eq:lambda-n-equation} satisfies
\begin{equation}
 \lambda_n=n+\frac{\log n}{L}
 +\frac{\log n}{L^2n}
 +\frac{\log n-\tfrac12\log^2n}{L^3n^2}
 +\bigO\!\left(\frac{\log^3n}{n^3}\right).
\label{dyadicasymbridge:eq:lambda-n-expansion}
\end{equation}
Substitution into \eqref{dyadicasymbridge:eq:main-inverse-leading}, with the exact
...
\label{dyadicasymbridge:eq:integer-dyadic-expansion}
\end{align}
...
Formula \eqref{dyadicasymbridge:eq:integer-dyadic-expansion} also clarifies the roles of the factors in
\eqref{dyadicasymbridge:eq:inverse-exact}:
\begin{itemize}
...
\label{dyadicasymbridge:sec:fixed-numerator}

The coefficient form \eqref{dyadicasymbridge:eq:prefix-coefficient} makes it possible to repeat the derivation for $m2^{-n}$ with fixed $m$.
...
\label{dyadicasymbridge:eq:Sm-def}
\end{equation}
For $\Re z=r>0$,
\begin{equation}
 |\Prefix_m(z)-1|
 \le\sum_{h\ge1}\e^{-hr}
 =\frac{\e^{-r}}{1-\e^{-r}}.
\label{dyadicasymbridge:eq:Sm-bound}
\end{equation}
...
\label{dyadicasymbridge:eq:Bm-dominant}
\end{equation}
...
\begin{equation}
 ...
 \qquad
 r=\frac\lambda m.
\label{dyadicasymbridge:eq:fixed-m-parameters}
\end{equation}
The Lambert equation implies
\begin{equation}
 n=\lambda-\log_2\lambda+\log_2m
 =mr-\log_2r.
\label{dyadicasymbridge:eq:fixed-m-saddle-relation}
\end{equation}
Equivalently,
\begin{equation}
 \log_2r=\lambda-n,
...
\label{dyadicasymbridge:eq:fixed-m-phase-lock}
\end{equation}
...
\label{dyadicasymbridge:eq:Hm-vertical}
\end{equation}
...
\begin{equation}
 ...
 {B_m(r)G(r)}
 (1+\ii\theta)^{-n-1}\dd\theta.
\label{dyadicasymbridge:eq:Cmn-def}
\end{equation}
...
\label{dyadicasymbridge:eq:fixed-m-exact-log}
\end{align}
Using \eqref{dyadicasymbridge:eq:Sm-def} and \eqref{dyadicasymbridge:eq:positive-negative-log},
\begin{equation}
 \log B_m(r)+\log G(r)
 =\lambda+\Lambda(r)+\log\Prefix_m(r).
\label{dyadicasymbridge:eq:BmG-real}
\end{equation}
The same algebra as \eqref{dyadicasymbridge:eq:key-algebra-cancellation}, now with
$\log r=L(\lambda-n)$, yields the exact separation
\begin{equation}
 \boxed{
 \log F(m2^{-n})-\Mmain(m2^{-n})
 =E(r)+\log\Prefix_m(r)+\log\CoeffMass_{m,n}.}
\label{dyadicasymbridge:eq:fixed-m-exact-separation}
\end{equation}

\subsection{Identity of local phases}

On the central window, \eqref{dyadicasymbridge:eq:Bm-dominant} gives
\begin{align*}
...
\eqref{dyadicasymbridge:eq:coefficient-local-phase}.  The vertical tails are bounded by the denominator
$(1+\theta^2)^{-(n+1)/2}$ times a fixed $m$-dependent constant.  Therefore
\begin{equation}
 \log\CoeffMass_{m,n}=
 \sum_{j=1}^{N-1}\frac{A_j(\lambda)}{\lambda^j}
 +\bigO_{m,N}(\lambda^{-N}).
\label{dyadicasymbridge:eq:Cmn-all-orders}
\end{equation}
The two additional terms in \eqref{dyadicasymbridge:eq:fixed-m-exact-separation} are exponentially small for fixed $m$.  This proves \cref{dyadicasymbridge:thm:main-fixed-m}.

\subsection{A moderate-numerator extension}

The estimate \eqref{dyadicasymbridge:eq:Sm-bound} is independent of $m$.  Its derivatives on the central scale introduce only fixed powers of the reduced saddle
...
\label{dyadicasymbridge:prop:moderate-m}
Let $m=m(n)$, put $\lambda=\lambda(m2^{-n})$, and suppose
\begin{equation}
 \frac{\lambda/m}{\log\lambda}\longrightarrow\infty.
\label{dyadicasymbridge:eq:moderate-m-condition}
\end{equation}
Then, for every fixed $N\ge1$, the expansion in \eqref{dyadicasymbridge:eq:main-fixed-m} holds through order $\lambda^{-N}$ uniformly along that sequence.
\end{proposition}

Indeed, every fixed collection of derivatives of the prefix correction is bounded by
$\bigO_N(r^{K_N}\e^{-r})$ for some finite $K_N$, and \eqref{dyadicasymbridge:eq:moderate-m-condition} makes this smaller than every power of $1/\lambda$.  The condition is equivalent to
...
\cref{dyadicasymbridge:sec:dedydadicization}.

\section{An exact Bromwich-to-coefficient collapse}
\label{dyadicasymbridge:sec:bromwich-collapse}
...
From \eqref{dyadicasymbridge:eq:G-functional},
\begin{equation}
 G(-2z)=\frac{1-\e^{-z}}zG(-z).
\label{dyadicasymbridge:eq:negative-refinement}
\end{equation}
After $n$ iterations,
\begin{equation}
 G(-2^nz)=
...
\begin{equation}
 ...
 \qquad R>0.
\label{dyadicasymbridge:eq:Bromwich-F}
\end{equation}
...
\label{dyadicasymbridge:eq:Bromwich-rescaled}
\end{align}
where \eqref{dyadicasymbridge:eq:G-symmetry} was used in the second line.

\subsection{Why the finite product disappears exactly}

Insert \eqref{dyadicasymbridge:eq:Pn-TM} and $G(z)=\E\e^{zX}$ into
\eqref{dyadicasymbridge:eq:Bromwich-rescaled}.  The inverse-Laplace kernel gives
\begin{align}
 F(2^{-n})
 &=2^{-\binom n2}
 \E\left[
 ...
 \right].
\label{dyadicasymbridge:eq:positive-part-collapse}
\end{align}
Because $0\le X\le1$, all terms with $h\ge1$ vanish almost surely.  The $h=0$ term is
$\E X^n/n!=a_n$, and \eqref{dyadicasymbridge:eq:inverse-exact} follows.  Thus the exact reciprocal-power bridge itself can be read as a support-induced collapse of the rescaled Bromwich integral.

Near the saddle this collapse has an even simpler form.  On the line $\Re z=\lambda_n$,
\begin{equation}
 P_n(z)=1+\bigO(\e^{-\lambda_n})
\label{dyadicasymbridge:eq:Pn-near-one}
\end{equation}
...
\label{dyadicasymbridge:eq:continuous-vs-coeff-saddle}
\end{equation}
where the last equality is \eqref{dyadicasymbridge:eq:lambda-n-equation}.  The substitution
$s=2^nz$ maps $R$ exactly to $z=\lambda_n$, and
\eqref{dyadicasymbridge:eq:negative-refinement-iterated} maps the continuous integrand to the coefficient integrand, up to the beyond-all-orders factor $P_n(z)$.  This is the most literal form of the connection sought in this report.

\section{What the different exact formulae contribute}
\label{dyadicasymbridge:sec:formula-audit}
...
\eqref{dyadicasymbridge:eq:normalized-triangular}
...
\eqref{dyadicasymbridge:eq:composition-formula}
...
$q$-binomial formula
\eqref{dyadicasymbridge:eq:qbinomial-global}
& Makes the geometric interpolation and two levels of exact cancellation explicit.
& Perform the finite Gaussian-binomial extraction first.  The Mersenne and $q$-Pochhammer factors then cancel, leaving \eqref{dyadicasymbridge:eq:prefix-coefficient}.\\[0.6em]
...
\eqref{dyadicasymbridge:eq:qbinomial-global} it is paired with a numerator containing the same Mersenne product.  Their exact cancellation occurs before any limit.  The nonconstant periodic term instead comes from the \emph{infinite} product for $G$ and the nonzero imaginary-axis poles of the Mellin factor \eqref{dyadicasymbridge:eq:geometric-mellin-factor}.
...
\label{dyadicasymbridge:sec:numerics}

The following computations use the recurrence \eqref{dyadicasymbridge:eq:a-recurrence} in exact rational arithmetic.  Floating-point conversion occurs only after the rational value has been formed.  The periodic function and its derivatives are evaluated from the Fourier series \eqref{dyadicasymbridge:eq:Psi-Fourier}.

Let
\begin{equation}
 R_n=\log F(2^{-n})-\Mmain(2^{-n}).
\label{dyadicasymbridge:eq:Rn-def}
\end{equation}
...
\label{dyadicasymbridge:tab:inverse-numerics}
\end{table}
...
\cref{dyadicasymbridge:thm:main-fixed-m}.}
\label{dyadicasymbridge:tab:fixed-m-numerics}
\end{table}
...
\label{dyadicasymbridge:sec:dedydadicization}
...
\label{dyadicasymbridge:eq:monotone-squeeze}
\end{equation}
From \eqref{dyadicasymbridge:eq:integer-dyadic-expansion},
\begin{equation}
 \log F(2^{-n})=-\frac L2n^2+\bigO(n\log n).
\label{dyadicasymbridge:eq:dyadic-leading-bound}
\end{equation}
The same estimate holds with $n+1$, and $t=n+\bigO(1)$.  Dividing the logarithm of
\eqref{dyadicasymbridge:eq:monotone-squeeze} by $t^2$ gives
\begin{equation}
 \log F(2^{-t})\sim-\frac L2t^2.
\end{equation}
Since $\log x=-Lt$, this is \eqref{dyadicasymbridge:eq:global-quadratic-law}.  Thus the exact reciprocal-power sequence does recover the universal leading endpoint behavior for every real argument.
...
\label{dyadicasymbridge:eq:neighbor-gap}
\end{equation}
...
To approximate $x$ finely enough for an additive $o(1)$ error in $\log F(x)$, a dyadic grid needs relative spacing much smaller than $1/\lambda$.  At level $n$, that means a reduced numerator of size much larger than $\lambda$.  The fixed- or moderate-numerator analysis in \cref{dyadicasymbridge:sec:fixed-numerator} instead assumes that $r=\lambda/m$ is large.  The two regimes do not overlap.
...
\label{dyadicasymbridge:eq:large-prefix-problem}
\end{equation}
...
\label{dyadicasymbridge:sec:formalization}
...
\eqref{dyadicasymbridge:eq:vertical-extraction} from the bounded-support moment representation.  For $n\ge1$, absolute convergence follows from
$|G(c+\ii t)|\le G(c)$.

\item \textbf{Exact dyadic saddle mass.}  Define $\CoeffMass_n$ by
\eqref{dyadicasymbridge:eq:Cn-def} and prove the exact factorization
\eqref{dyadicasymbridge:eq:exact-coefficient-saddle}.
...
\eqref{dyadicasymbridge:eq:key-algebra-cancellation} and derive
\eqref{dyadicasymbridge:eq:exact-dyadic-separation}.

\item \textbf{Reuse the generic saddle machinery.}  Prove
\eqref{dyadicasymbridge:eq:coefficient-local-phase}.  Its main part is literally the phase already used by the all-orders endpoint modules; the only new tail is exponentially small in $\lambda$ rather than in $2^\lambda$.

\item \textbf{Prefix rays.}  Use the elementary bound
\eqref{dyadicasymbridge:eq:Sm-bound} to lift the same proof to fixed $m$, yielding
\eqref{dyadicasymbridge:eq:fixed-m-exact-separation} and \cref{dyadicasymbridge:thm:main-fixed-m}.

\item \textbf{Optional rescaled-Bromwich theorem.}  Formalize
\eqref{dyadicasymbridge:eq:negative-refinement-iterated} and the support collapse
\eqref{dyadicasymbridge:eq:positive-part-collapse}.  This provides an exact commutative square between the moment and endpoint inversion proofs.
\end{enumerate}
...
\section{Conclusions}
\label{dyadicasymbridge:sec:conclusions}
...
\label{dyadicasymbridge:app:algebra}
...
\label{dyadicasymbridge:app:numerics}
...
\begin{align}
 ...
 &=\sum_{h=0}^{m-1}\TM_h
 \sum_{j=0}^{n}a_j
 \frac{(m-1-h)^{n-j}}{(n-j)!},                             \label{dyadicasymbridge:eq:exact-prefix-coeff-compute}\\
 F(m2^{-n})
 &=2^{-\binom n2}[z^n](B_mG).                              \label{dyadicasymbridge:eq:exact-prefix-value-compute}
\end{align}
Equations \eqref{dyadicasymbridge:eq:exact-prefix-coeff-compute}--\eqref{dyadicasymbridge:eq:exact-prefix-value-compute} involve rational arithmetic only.  The Lambert coordinate is then evaluated at high precision from
\eqref{dyadicasymbridge:eq:lambda-def}; $\Psi$ and its derivatives are evaluated from
\eqref{dyadicasymbridge:eq:Psi-Fourier}.  Because the Fourier coefficients decay exponentially, a small number of modes suffices for the digits printed in \cref{dyadicasymbridge:tab:inverse-numerics,dyadicasymbridge:tab:fixed-m-numerics}.

\section{Bibliographic note}

The exact arithmetic identities used here are developed in the ProveIt repository and its accompanying exposition \cite{dyadicasymbridge:proveit,dyadicasymbridge:masterdoc}, building in particular on the compact-support and arithmetic papers of Arias de Reyna \cite{dyadicasymbridge:arias1982,dyadicasymbridge:arias2018}.  Haugland \cite{dyadicasymbridge:haugland2016} gives another exact-evaluation perspective.  The Mellin treatment of harmonic sums follows the general framework of Flajolet, Gourdon, and Dumas \cite{dyadicasymbridge:flajolet1995}, while the Lambert coordinate uses the branch conventions of Corless et al. \cite{dyadicasymbridge:corless1996}.  The contribution of this report is the explicit coefficient-saddle bridge, its phase-locking identity, its extension to fixed dyadic numerators, and the exact rescaled-Bromwich collapse.

\renewcommand{\refname}{Dossier references}
\begin{thebibliography}{99}

\bibitem{dyadicasymbridge:proveit}
V.~Reshetnikov,
...
\bibitem{dyadicasymbridge:masterdoc}
V.~Reshetnikov,
...
\bibitem{dyadicasymbridge:arias1982}
J.~Arias de Reyna,
...
\bibitem{dyadicasymbridge:arias2018}
J.~Arias de Reyna,
\newblock Arithmetic of the Fabius function,
\newblock \emph{INTEGERS} \textbf{18} (2018), Article A51; arXiv:1702.06487, version~3.

\bibitem{dyadicasymbridge:haugland2016}
J.~K.~Haugland,
\newblock Evaluating the Fabius function,
\newblock arXiv:1609.07999 (2016).

\bibitem{dyadicasymbridge:flajolet1995}
...
\bibitem{dyadicasymbridge:corless1996}
...
\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Fabius_Dyadic_Asymptotic_Bridge/Fabius_Dyadic_Asymptotic_Bridge.tex

% BEGIN MERGED SOURCE: Fabius_Dyadic_Formulae_to_Asymptotics/Fabius_Dyadic_Formulae_to_Asymptotics.tex
% Original SHA-256: 6e98efd3afc402159a7f080e8604c951d5bc51be4a73383da67c1241d46d54ca
\clearpage
\part{From Exact Dyadic Formulae to the Small-Argument Asymptotic}
\label{part:dyadicasym}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Fabius_Dyadic_Formulae_to_Asymptotics/Fabius_Dyadic_Formulae_to_Asymptotics.tex}.  Newest and preferred dyadic-to-asymptotic synthesis.  It decompresses the half-base \(q\)-binomial formula, gives a rigorous moment-to-Laplace route, and separates formalized inputs from its independent coefficient-saddle derivation.  Prefer its normalizations and status ledger where it overlaps the preceding bridge.\par\smallskip Original source SHA-256: \nolinkurl{6e98efd3afc402159a7f080e8604c951d5bc51be4a73383da67c1241d46d54ca}.
\end{boxedremark}
\section*{Dossier abstract}
...
\label{dyadicasym:sec:exact-sequence}
...
\begin{equation}
 \Up(t)=F(1-|t|),\qquad |t|\le1.                             \label{dyadicasym:eq:up-fold}
\end{equation}
Define the half moments
\begin{equation}
 d_0=1,
 \qquad
 d_n=n\int_0^1t^{n-1}\Up(t)\dd t\quad(n\ge1).               \label{dyadicasym:eq:dn}
\end{equation}
They are rational and positive.  Their first few values are
\begin{equation}
 d_0=1,\qquad d_1=\frac12,\qquad d_2=\frac5{18},\qquad
 d_3=\frac16,\qquad d_4=\frac{143}{1350}.                  \label{dyadicasym:eq:dn-first}
\end{equation}

\begin{theorem}[Exact endpoint--moment bridge]
\label{dyadicasym:thm:endpoint-moment}
For every $n\ge0$,
\begin{equation}
 d_n=n!\,2^{\binom n2}F(2^{-n}),
 \qquad
 F(2^{-n})=\frac{d_n}{n!\,2^{\binom n2}}.                  \label{dyadicasym:eq:endpoint-moment}
\end{equation}
\end{theorem}
...
\begin{equation}
 ...
 \qquad -1\le t\le1.                                       \label{dyadicasym:eq:primitive-chain}
\end{equation}
The functional--differential equation of $\Up$ implies
\begin{equation}
 f_{r+1}'(t)=f_r(t),\qquad f_r(-1)=0.                        \label{dyadicasym:eq:primitive-chain-derivative}
\end{equation}
Put $J_n=\int_{-1}^0t^{n-1}\Up(t)\dd t$.  Repeated integration by parts, using
\eqref{dyadicasym:eq:primitive-chain-derivative}, gives
\begin{equation}
 J_n=(-1)^{n-1}(n-1)!f_n(0).                                \label{dyadicasym:eq:J-chain}
\end{equation}
...
\begin{equation*}
 f_n(0)=2^{\binom n2}\Up(2^{-n}-1)
       =2^{\binom n2}F(2^{-n}).
\end{equation*}
Multiplication by $n$ proves \eqref{dyadicasym:eq:endpoint-moment}.  The case $n=0$ is
$F(1)=d_0=1$.
\end{proof}
...
measure gives
\begin{equation}
 d_n=\E(1-X)^n.                                              \label{dyadicasym:eq:dn-endpoint-probability}
\end{equation}
...
\begin{equation}
 a_n=\frac{d_n}{n!},
 \qquad
 A(z)=\sum_{n=0}^{\infty}a_nz^n.                            \label{dyadicasym:eq:an-A}
\end{equation}
Then \Cref{dyadicasym:thm:endpoint-moment} becomes the particularly economical identity
\begin{equation}
 \boxed{F(2^{-n})=2^{-\binom n2}a_n.}                       \label{dyadicasym:eq:F-an}
\end{equation}
The half-moment recurrence is equivalent to
\begin{equation}
 (2^n-1)a_n=\sum_{k=0}^{n-1}\frac{a_k}{(n-k+1)!},
 \qquad n\ge1,
 \qquad a_0=1.                                              \label{dyadicasym:eq:an-recurrence}
\end{equation}
Equivalently, directly in terms of endpoint values,
\begin{equation}
 F(2^{-n})=
 \frac{2^{-\binom n2}}{2^n-1}
 \sum_{k=0}^{n-1}\frac{2^{\binom k2}}{(n-k+1)!}F(2^{-k}).  \label{dyadicasym:eq:F-recurrence}
\end{equation}

The recurrence is not merely an evaluation algorithm.  It is already a coefficient equation.
Let
\begin{equation}
 E(z)=\frac{\e^z-1}{z},\qquad E(0)=1.                       \label{dyadicasym:eq:E}
\end{equation}
Multiplying \eqref{dyadicasym:eq:an-recurrence} by $z^n$ and summing gives
\begin{equation}
 \boxed{A(2z)=E(z)A(z).}                                    \label{dyadicasym:eq:A-functional}
\end{equation}
Conversely, coefficient comparison in \eqref{dyadicasym:eq:A-functional} recovers
\eqref{dyadicasym:eq:an-recurrence}.  Thus the exact recurrence and the analytic refinement equation are
the same statement.

\subsection{Composition sum}

Repeated substitution in \eqref{dyadicasym:eq:an-recurrence} gives a finite closed form.  A composition
...
\label{dyadicasym:prop:composition}
For $n\ge1$,
\begin{equation}
 a_n=
 \sum_{\rho\vDash n}
 \prod_{j=1}^{\ell(\rho)}
 \frac{1}{(2^{s_j}-1)(r_j+1)!}.                             \label{dyadicasym:eq:composition-an}
\end{equation}
Consequently
\begin{equation}
 F(2^{-n})=2^{-\binom n2}
 \sum_{\rho\vDash n}
 \prod_{j=1}^{\ell(\rho)}
 \frac{1}{(2^{s_j}-1)(r_j+1)!}.                             \label{dyadicasym:eq:composition-F}
\end{equation}
\end{proposition}

\begin{proof}
Every use of \eqref{dyadicasym:eq:an-recurrence} replaces a current index $s_j$ by a smaller prefix
...
gives \eqref{dyadicasym:eq:composition-an}.
\end{proof}
...
\label{dyadicasym:sec:q-decompression}
...
For $q=1/2$, write
\begin{equation}
 (a;1/2)_n=\prod_{j=0}^{n-1}(1-a2^{-j}),                    \label{dyadicasym:eq:qpoch}
\end{equation}
and let $\qbinom nk{1/2}$ denote the Gaussian binomial coefficient
\cite{dyadicasym:andrews1976,dyadicasym:gasper2004,dyadicasym:kac2002,dyadicasym:dlmf17}.  The finite
$q$-binomial theorem is
\begin{equation}
 (z;1/2)_n=\sum_{k=0}^n
 (-1)^k2^{-\binom k2}\qbinom nk{1/2}z^k.                   \label{dyadicasym:eq:q-binomial-theorem}
\end{equation}
Define
\begin{equation}
 x_k=-2^k,
 ...
 w_{n,k}=(-1)^k2^{-\binom k2}\qbinom nk{1/2},
 \qquad
 \M_n=\prod_{j=1}^n(2^j-1).                                \label{dyadicasym:eq:q-weights}
\end{equation}

\begin{lemma}[Leading-coefficient extraction]
\label{dyadicasym:lem:q-leading}
If $R$ is a polynomial of degree at most $n$, then
\begin{equation}
 \sum_{k=0}^nw_{n,k}R(-2^k)=\M_n[z^n]R(z).                 \label{dyadicasym:eq:q-leading}
\end{equation}
Equivalently,
\begin{equation}
 ...
 0,&0\le d<n,\\
 \M_n,&d=n.
 \end{cases}                                                \label{dyadicasym:eq:q-monomials}
\end{equation}
\end{lemma}
...
$x_0,\ldots,x_n$:
\begin{equation}
 \frac{w_{n,k}}{\M_n}=\frac1{\prod_{j\ne k}(x_k-x_j)}.      \label{dyadicasym:eq:barycentric}
\end{equation}
The coefficient of $z^n$ in the Lagrange interpolation formula for $R$ is therefore the left
side of \eqref{dyadicasym:eq:q-leading} divided by $\M_n$.  Formula \eqref{dyadicasym:eq:barycentric} can also be
checked directly from \eqref{dyadicasym:eq:q-binomial-theorem}.
\end{proof}

The Pochhammer normalization is exactly the same Mersenne product:
\begin{equation}
 (1/2;1/2)_n=2^{-\binom{n+1}{2}}\M_n.                      \label{dyadicasym:eq:q-mersenne}
\end{equation}
...
Let
\begin{equation}
 \epsTM_r=(-1)^{\wt(r)}.                                    \label{dyadicasym:eq:tm-sign}
\end{equation}
For $k,N\ge0$ and a scalar translation $Q$, define
\begin{equation}
 S_{k,N}(Q)=\sum_{r=0}^{2^k-1}\epsTM_r(r-2^k+Q)^N.          \label{dyadicasym:eq:Skn}
\end{equation}
The complete-block exponential generating function is
\begin{equation}
 C_k(X)=\sum_{r<2^k}\epsTM_r\e^{(r-2^k)X}.                 \label{dyadicasym:eq:Ck}
\end{equation}
The elementary binary product
\begin{equation}
 \sum_{r<2^k}\epsTM_rz^r=\prod_{j=0}^{k-1}(1-z^{2^j})     \label{dyadicasym:eq:tm-product}
\end{equation}
implies
\begin{equation}
 \frac{C_k(X)}{X^k}
 =(-1)^k2^{\binom k2}\e^{-X}
   \prod_{j=0}^{k-1}E(-2^jX).                              \label{dyadicasym:eq:Ck-factor}
\end{equation}
In particular, $C_k$ has a zero of order $k$ at the origin.  Equivalently,
$S_{k,N}(Q)=0$ for $N<k$, independently of $Q$.

Iterating \eqref{dyadicasym:eq:A-functional} on the negative dyadic orbit gives
\begin{equation}
 \frac{A(-2^kX)}{A(-X)}=\prod_{j=0}^{k-1}E(-2^jX).          \label{dyadicasym:eq:A-ratio}
\end{equation}
Translation invariance permits the especially convenient choice $Q=1$.  Since
\begin{equation}
 \sum_{N\ge0}S_{k,N}(1)\frac{X^N}{N!}=\e^XC_k(X),          \label{dyadicasym:eq:S-egf}
\end{equation}
\eqref{dyadicasym:eq:Ck-factor} and \eqref{dyadicasym:eq:A-ratio} yield the exact coefficient identity
\begin{equation}
 \boxed{
 \frac{S_{k,n+k}(1)}{(n+k)!}
 =(-1)^k2^{\binom k2}
 [X^n]\frac{A(-2^kX)}{A(-X)}.}                             \label{dyadicasym:eq:S-ratio-coeff}
\end{equation}
...
\begin{equation}
 ...
 {2^{n^2}(1/2;1/2)_n},                                     \label{dyadicasym:eq:q-F-N}
\end{equation}
...
\begin{equation}
 ...
 \sum_{k=0}^n
 \frac{\qbinom nk{1/2}}{4^{\binom k2}(n+k)!}
 S_{k,n+k}(1).                                              \label{dyadicasym:eq:q-numerator}
\end{equation}

\begin{theorem}[$q$-binomial decompression]
\label{dyadicasym:thm:q-decompression}
For every $n\ge0$,
\begin{equation}
 \boxed{\mathcal N_n=\M_na_n.}                             \label{dyadicasym:eq:N-m-a}
\end{equation}
...
\begin{equation}
 R_n(z)=[X^n]\frac{A(zX)}{A(-X)}.                           \label{dyadicasym:eq:Rn}
\end{equation}
It has degree at most $n$.  Since $A(-X)^{-1}=1+\bigO(X)$ and
$A(zX)=\sum_{j\ge0}a_jz^jX^j$, its leading coefficient is
\begin{equation}
 [z^n]R_n(z)=a_n.                                           \label{dyadicasym:eq:Rn-leading}
\end{equation}
Substitution of \eqref{dyadicasym:eq:S-ratio-coeff} into \eqref{dyadicasym:eq:q-numerator} gives
\begin{align}
 \mathcal N_n
 &=\sum_{k=0}^n
 \qbinom nk{1/2}2^{-2\binom k2}
 (-1)^k2^{\binom k2}R_n(-2^k)                              \notag\\
 &=\sum_{k=0}^n w_{n,k}R_n(-2^k).                          \label{dyadicasym:eq:N-interpolation}
\end{align}
By \Cref{dyadicasym:lem:q-leading}, this is $\M_n[z^n]R_n(z)=\M_na_n$, proving
\eqref{dyadicasym:eq:N-m-a}.  Finally, \eqref{dyadicasym:eq:q-mersenne} gives
\begin{equation*}
...
\begin{align}
 ...
 \frac{\qbinom nk{1/2}}{4^{\binom k2}(n+k)!}                \notag\\
 &\qquad\times
 \sum_{r=0}^{m2^k-1}\epsTM_r(r-m2^k+Q)^{n+k}.              \label{dyadicasym:eq:q-global}
\end{align}
...
quadratic law.  Taking logarithms in \eqref{dyadicasym:eq:endpoint-moment},
\begin{equation}
 \log F(2^{-n})=\log d_n-\log(n!)-\binom n2L.               \label{dyadicasym:eq:log-F-exact}
\end{equation}
Because $0\le\Up\le1$,
\begin{equation}
 d_n\le n\int_0^1t^{n-1}\dd t=1.                           \label{dyadicasym:eq:dn-upper}
\end{equation}
...
\begin{equation}
 B(s)=\prod_{j=1}^{\infty}
 \frac{1-\e^{-s/2^j}}{s/2^j},                               \label{dyadicasym:eq:B-product}
\end{equation}
and hence
\begin{equation}
 \Lam(s)=\sum_{j=1}^{\infty}\kappa(s/2^j),
 \qquad
 \kappa(u)=\log\frac{1-\e^{-u}}u.                          \label{dyadicasym:eq:Lambda-harmonic}
\end{equation}
This is a dyadic harmonic sum.  Its dilation equation is exact:
\begin{equation}
 \Lam(2s)=\Lam(s)+\log(1-\e^{-s})-\log s.                  \label{dyadicasym:eq:Lambda-dilation}
\end{equation}
...
\begin{equation}
 0\le\mathcal E(s)\le
 \frac{\e^{-s}}{(1-\e^{-s})^2},                            \label{dyadicasym:eq:tail-bound-report}
\end{equation}
...
\begin{equation}
 \chi_k=\frac{2\pi\ii k}{L},                                \label{dyadicasym:eq:chi}
\end{equation}
...
\begin{equation}
 ...
 \le16\e^{-nx}(nx^3+n^2x^4).                               \label{dyadicasym:eq:kernel-comparison}
\end{equation}
Integrating against the law of $X$ gives
\begin{align}
 d_n={}&B(n)-\frac n2\E\bigl(X^2\e^{-nX}\bigr)+R_n,        \label{dyadicasym:eq:dn-B-second}\\
 |R_n|\le{}&16\left(
 n\E(X^3\e^{-nX})+n^2\E(X^4\e^{-nX})\right).              \label{dyadicasym:eq:Rn-bound}
\end{align}

Let $q(s)=\Lam(s)=\log B(s)$.  The normalized tilted second moment is
\begin{equation}
 \frac{\E(X^2\e^{-sX})}{B(s)}=q''(s)+q'(s)^2.               \label{dyadicasym:eq:tilted-second}
\end{equation}
...
\begin{equation}
 q'(s)=\frac{-\ell/L+1/2+\Psi'(u)/L}{s}+\bigO(\e^{-s}),     \label{dyadicasym:eq:qprime}
\end{equation}
and
\begin{equation}
 q''(s)=\frac{\ell/L-1/2-1/L-\Psi'(u)/L+\Psi''(u)/L^2}{s^2}
 +\bigO(\e^{-s}).                                           \label{dyadicasym:eq:qsecond}
\end{equation}
...
\begin{equation}
 ...
 +\bigO(r\e^{-r}).                                          \label{dyadicasym:eq:exact-saddle-shift}
\end{equation}
...
\begin{equation}
 ...
 \frac{A(r\e^{\ii\theta})}{A(r)}\e^{-\ii n\theta}\dd\theta.
                                                                    \label{dyadicasym:eq:Cauchy-an}
\end{equation}
At $r=\lambda_n$, split the product \eqref{dyadicasym:eq:A-product} near the dyadic index
...
arbitrary polynomial decay.  The standard coefficient-saddle estimate \cite{dyadicasym:hayman1956,dyadicasym:flajolet2009} is therefore
\begin{equation}
 a_n=\frac{A(\lambda_n)}{\lambda_n^n\sqrt{2\pi\lambda_n}}
 \left(1+\bigO(\lambda_n^{-1})\right).                     \label{dyadicasym:eq:an-saddle}
\end{equation}
The use of the reference rather than exact saddle is absorbed by the same relative error.

Taking logarithms and applying \eqref{dyadicasym:eq:K-positive},
\begin{equation}
 \log a_n=\lambda_n-n\log\lambda_n
 -\frac{\log^2\lambda_n}{2L}
 +P(\log_2\lambda_n)-\frac12\log(2\pi)
 +\bigO(n^{-1}).                                            \label{dyadicasym:eq:loga-pre}
\end{equation}
...
\begin{equation}
 ...
 +\bigO(n^{-1}),                                            \label{dyadicasym:eq:loga-lambda}
\end{equation}
where periodicity was used in the form
$P(\log_2\lambda_n)=P(\lambda_n-n)=P(\lambda_n)$.

Finally, subtract $\binom n2L$ according to \eqref{dyadicasym:eq:F-an}.  Writing
...
\end{align*}
Adding \eqref{dyadicasym:eq:loga-lambda} makes the terms
$\lambda_n\ell_n$ and $\ell_n^2/(2L)$ cancel exactly.  Using
\eqref{dyadicasym:eq:CF-from-P} gives
\begin{equation}
 \log F(2^{-n})=
 -\frac L2\lambda_n^2+
 \left(1+\frac L2\right)\lambda_n-\frac12\log\lambda_n
 +C_F+\Psi(\lambda_n)+\bigO(n^{-1}).                        \label{dyadicasym:eq:saddle-final}
\end{equation}
This is \eqref{dyadicasym:eq:headline-compact}.

\begin{boxedremark}
The coefficient saddle provides a particularly direct answer to the original question.
Starting from the exact $q$-binomial value, \Cref{dyadicasym:thm:q-decompression} produces $a_n$; Cauchy
...
\end{align}
Thus the exact composition sum \eqref{dyadicasym:eq:composition-an} has the asymptotic
\begin{align}
 &\sum_{\rho\vDash n}
 \prod_{j=1}^{\ell(\rho)}
...
By \Cref{dyadicasym:thm:q-decompression},
\begin{equation}
...
$\exp(Ln^2/2)$, while the explicit denominator in \eqref{dyadicasym:eq:q-F-N} contains
...
\begin{equation}
 ...
 =-Ln^2-\log(1/2;1/2)_\infty+\smallo(1).                   \label{dyadicasym:eq:q-den-leading}
\end{equation}
...
The density result does not by itself extend \eqref{dyadicasym:eq:headline-compact} uniformly to all
...
The global formula \eqref{dyadicasym:eq:q-global} contains every dyadic rational and could, in principle,
...
The recurrence \eqref{dyadicasym:eq:an-recurrence} computes $a_n$, hence $F(2^{-n})$, in exact rational
...
argument in \Cref{dyadicasym:sec:q-decompression} is the ordinary-mathematics unpacking of this layer.\\
\addlinespace
...
\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Fabius_Dyadic_Formulae_to_Asymptotics/Fabius_Dyadic_Formulae_to_Asymptotics.tex




% BEGIN MERGED SOURCE: Fabius_Thue_Morse_Convergence_Rate/Fabius_Thue_Morse_Convergence_Rate.tex
% Original SHA-256: 577c5f3426def68a774fedf3fce61552d32200c8da526ace44178f2a8995a6d3
\clearpage
\part{Iterated Thue--Morse Prefix Sums and the Fabius Function}
\label{part:tmrate}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Fabius_Thue_Morse_Convergence_Rate/Fabius_Thue_Morse_Convergence_Rate.tex}.  Elementary quantitative route through discrete curvature and cardinal moments.  It overlaps the next dossier but retains distinct finite bounds, binary-phase expansions, exact special values, plots, and a formalization roadmap.  The exact discrete bridge has Lean-backed ingredients; every quantitative rate, constant, and interpolation estimate in this dossier remains a natural-language proof and formal obligation.\par\smallskip Original source SHA-256: \nolinkurl{577c5f3426def68a774fedf3fce61552d32200c8da526ace44178f2a8995a6d3}.
\end{boxedremark}
\section*{Dossier abstract}
...
\cite{tmrate:proveit,tmrate:synthesis}.  The corresponding Lean module
...
\begin{equation}
 \Up(y)=F(1-|y|),\qquad
 F(x)=\Up(x-1)\quad(0\le x\le1).                 \label{tmrate:eq:fold}
\end{equation}
We use the Fourier transform
\begin{equation}
 \widehat f(\xi)=\int_{\R}f(x)\e^{-2\pi\ii x\xi}\dd x,
 \qquad \sinc z=\frac{\sin z}{z}.               \label{tmrate:eq:fourier-convention}
\end{equation}
The derivative bounds proved sharply in the formal development are
\begin{equation}
 \norm{F^{(r)}}_\infty\le 2^{\binom{r+1}{2}},
 \qquad
 \norm{\Up^{(r)}}_\infty\le 2^{\binom{r+1}{2}}.  \label{tmrate:eq:derivative-bounds}
\end{equation}
We shall use, in particular,
\begin{equation}
 0\le F'\le2,
 \qquad \norm{F''}_\infty\le8,
 \qquad \norm{F'''}_\infty\le64,                \label{tmrate:eq:low-derivative-bounds}
\end{equation}
and $F'(1/2)=2$.
...
\label{tmrate:sec:discrete}

\subsection{Signed Thue--Morse and inclusive summation}

Let
\begin{equation}
 \varepsilon_m=(-1)^{\wt(m)},                              \label{tmrate:eq:tm-sign}
\end{equation}
where $\wt(m)$ is the number of $1$'s in the binary expansion of $m$.  Define
\begin{equation}
 \sigma_0(m)=\varepsilon_m,
 \qquad
 \sigma_{k+1}(m)=\sum_{j=0}^{m}\sigma_k(j).                \label{tmrate:eq:prefix-def}
\end{equation}
The summation is inclusive.  Thus
\begin{equation}
 \sigma_{k+1}(m+1)-\sigma_{k+1}(m)=\sigma_k(m+1),           \label{tmrate:eq:forward-difference}
\end{equation}
...
\begin{equation}
 ...
 \qquad k\ge1.                                             \label{tmrate:eq:binomial-convolution}
\end{equation}
Equivalently,
\begin{equation}
 \sigma_{k+1}(m)=\sum_{r=0}^{m}
 \binom{m-r+k}{k}\varepsilon_r.                            \label{tmrate:eq:binomial-convolution-shift}
\end{equation}
...
\begin{equation}
 K_r(z)=\sum_{m=0}^{2^r-1}\varepsilon_m z^m
       =\prod_{j=0}^{r-1}(1-z^{2^j}),                      \label{tmrate:eq:tm-block-product}
\end{equation}
and coefficientwise the infinite signed series is
\begin{equation}
 K(z)=\prod_{j\ge0}(1-z^{2^j}).                            \label{tmrate:eq:tm-infinite-product}
\end{equation}
Hence the $k$-fold prefix series is formally
\begin{equation}
 S_k(z)=\sum_{m\ge0}\sigma_k(m)z^m=\frac{K(z)}{(1-z)^k}.  \label{tmrate:eq:prefix-series}
\end{equation}
No analytic convergence is involved in this identity.

\subsection{Dyadic annihilation}

The product in \eqref{tmrate:eq:tm-block-product} has a zero of order $r$ at $z=1$.
Equivalently, a complete block of length $2^r$ annihilates every polynomial in
$m$ of degree below $r$.  Applied to \eqref{tmrate:eq:binomial-convolution}, this gives
the exact zero run
\begin{equation}
 \sigma_k(m)=0
 \quad\text{for}\quad
 2^r-k\le m\le2^r-1,
 \qquad 1\le k\le r,                                     \label{tmrate:eq:zero-run}
\end{equation}
with sharp boundary values
\begin{equation}
 \sigma_k(2^r-k-1)=(-1)^{r-k},
 \qquad
 \sigma_k(2^r)=-1.                                        \label{tmrate:eq:zero-run-boundary}
\end{equation}
...
\label{tmrate:sec:polynomial-bridge}
...
\begin{equation}
 P_n(z)=\prod_{j=1}^{n}(1+z+\cdots+z^{2^j-1})
       =\sum_{m=0}^{g_n}a_{n,m}z^m,                        \label{tmrate:eq:Pn}
\end{equation}
with $P_0=1$.  Its degree and total mass are
\begin{equation}
 g_n=\sum_{j=1}^{n}(2^j-1)=2^{n+1}-n-2,
 \qquad
 P_n(1)=2^{\binom{n+1}{2}}.                                \label{tmrate:eq:degree-mass}
\end{equation}
The coefficients count restricted sums
\begin{equation}
 a_{n,m}=
 \#\left\{(s_1,\ldots,s_n):
 0\le s_j<2^j,\ \sum_{j=1}^{n}s_j=m\right\}.              \label{tmrate:eq:coefficient-count}
\end{equation}
They are nonnegative and symmetric: $a_{n,m}=a_{n,g_n-m}$.

The denominator-cleared product identity is
\begin{equation}
 (1-z)^{n+1}P_n(z)=\prod_{j=0}^{n}(1-z^{2^j}).              \label{tmrate:eq:cleared-product}
\end{equation}
Comparing \eqref{tmrate:eq:cleared-product} with
\eqref{tmrate:eq:prefix-series} gives the exact coefficient bridge
\begin{equation}
 \boxed{\displaystyle
 \sigma_{n+1}(m)=a_{n,m}
 \quad\text{whenever}\quad 0\le m<2^{n+1}.}               \label{tmrate:eq:prefix-coefficient}
\end{equation}
The restriction is real: at $m=2^{n+1}$ the left side is $-1$ by
\eqref{tmrate:eq:zero-run-boundary}, while the polynomial coefficient is zero.
...
\begin{equation}
 ...
 \xi_{n,m}=\frac{2m-g_n}{2^{n+1}}
          =h_n\left(m-\frac{g_n}{2}\right).                \label{tmrate:eq:centers}
\end{equation}
...
\begin{equation}
 ...
 \qquad
 p_{n,m}=\frac{a_{n,m}}{2^{\binom{n+1}{2}}},               \label{tmrate:eq:mu-n}
\end{equation}
where $a_{n,m}=0$ outside $0\le m\le g_n$.  Let
\begin{equation}
 \beta_h(x)=\frac1h\indicator_{[-h/2,h/2]}(x)              \label{tmrate:eq:box-density}
\end{equation}
with the harmless half-weight convention at the two endpoints.  The polynomial
histogram is
\begin{equation}
 \mathcal W_n=\mu_n*\beta_{h_n}.                            \label{tmrate:eq:histogram-convolution}
\end{equation}
At a center only its own atom contributes, and therefore
\begin{equation}
 \mathcal W_n(\xi_{n,m})
 =\frac{p_{n,m}}{h_n}
 =\frac{a_{n,m}}{2^{\binom n2}}.                           \label{tmrate:eq:center-value}
\end{equation}
Combining \eqref{tmrate:eq:prefix-coefficient} and \eqref{tmrate:eq:center-value} yields the
formalized exact identification
\begin{equation}
 \frac{\sigma_{n+1}(m)}{2^{\binom n2}}
 =\mathcal W_n(\xi_{n,m}),
 \qquad 0\le m<2^{n+1}.                                   \label{tmrate:eq:exact-grid-histogram}
\end{equation}

\section{What the floor index actually samples}
\label{tmrate:sec:centering}

The corrected prefix sample for the bump is
\begin{equation}
 \mathcal J_n(t)=
 \frac{\sigma_{n+1}(\floor{2^{n+1}t})}{2^{\binom n2}},
 \qquad 0\le t\le1.                                      \label{tmrate:eq:J-def}
\end{equation}
For $t<1$, \eqref{tmrate:eq:exact-grid-histogram} gives
\begin{equation}
 \mathcal J_n(t)=
 \mathcal W_n\!\left(\xi_{n,\floor{2^{n+1}t}}\right).    \label{tmrate:eq:J-histogram}
\end{equation}
The Fabius version is the first-half rescaling
\begin{equation}
 \mathcal A_n(x)=\mathcal J_n(x/2)
 =\frac{\sigma_{n+1}(\floor{2^n x})}{2^{\binom n2}},
 \qquad 0\le x\le1.                                     \label{tmrate:eq:A-def}
\end{equation}

\begin{lemma}[Exact center drift]
\label{tmrate:lem:center-drift}
For $x\in[0,1]$, put
\begin{equation}
 \theta_n(x)=\fracpart{2^n x},
 \qquad
 \delta_n(x)=\frac{n+2-2\theta_n(x)}{2^{n+1}}.            \label{tmrate:eq:delta-def}
\end{equation}
Then
\begin{equation}
 \xi_{n,\floor{2^n x}}=x-1+\delta_n(x),                   \label{tmrate:eq:center-drift}
\end{equation}
and
\begin{equation}
 \delta_n(x)=\frac{n}{2^{n+1}}+
              \frac{1-\theta_n(x)}{2^n},
 \qquad
 \frac{n}{2^{n+1}}<\delta_n(x)\le\frac{n+2}{2^{n+1}}.    \label{tmrate:eq:delta-decompose}
\end{equation}
\end{lemma}

\begin{proof}
Use $g_n=2^{n+1}-n-2$ and
$\floor{2^n x}=2^n x-\theta_n(x)$ in \eqref{tmrate:eq:centers}:
\begin{align*}
 \xi_{n,\floor{2^n x}}
...
The dominant term in \eqref{tmrate:eq:delta-decompose} is $n2^{-n-1}$.  It comes from
...
\begin{equation}
 ...
 =\prod_{r=0}^{j-1}\cos(\pi 2^r h_n\xi).                  \label{tmrate:eq:finite-uniform-transform}
\end{equation}
Multiplying over $j=1,\ldots,n$ and collecting equal cosine factors gives
\begin{equation}
 \widehat\mu_n(\xi)
 =\prod_{\ell=1}^{n}
   \cos\!\left(\frac{\pi\xi}{2^\ell}\right)^\ell.       \label{tmrate:eq:mu-cosine-product}
\end{equation}
The Rvachev bump has the infinite product
\begin{equation}
 \widehat\Up(\xi)
 =\prod_{\ell=1}^{\infty}
   \cos\!\left(\frac{\pi\xi}{2^\ell}\right)^\ell.      \label{tmrate:eq:up-cosine-product}
\end{equation}
This is equivalent to the more familiar infinite sinc product.

Set $h=h_n$.  Reindexing the tail of \eqref{tmrate:eq:up-cosine-product} gives
\begin{align}
 \prod_{\ell=n+1}^{\infty}
 ...
 &=\widehat\Up(h\xi)\sinc(\pi h\xi)^n.                  \label{tmrate:eq:tail-factorization}
\end{align}
...
\begin{equation}
 q_n=\beta_h^{*n}*D_h\Up.                                  \label{tmrate:eq:q-n}
\end{equation}
The Fourier identity \eqref{tmrate:eq:tail-factorization} is precisely
\begin{equation}
 \boxed{\Up=\mu_n*q_n.}                                    \label{tmrate:eq:up-mu-q}
\end{equation}
...
\begin{equation}
 \lambda_{n,k}=h q_n(kh),\qquad k\in\Z.                    \label{tmrate:eq:lambda-def}
\end{equation}
...
\begin{equation}
 h\sum_{k\in\Z}\beta_h(kh-s)=1                            \label{tmrate:eq:box-partition}
\end{equation}
for almost every $s$.  Integration against $\rho_n$ yields
\begin{equation}
 \sum_{k\in\Z}\lambda_{n,k}=1,
 \qquad \lambda_{n,-k}=\lambda_{n,k}.                     \label{tmrate:eq:lambda-probability}
\end{equation}
Equation \eqref{tmrate:eq:up-mu-q}, together with
$p_{n,m}=h\mathcal W_n(\xi_{n,m})$, now becomes the exact cardinal averaging
identity
\begin{equation}
 \Up(\xi_{n,m})=
 \sum_{k\in\Z}\lambda_{n,k}
 \mathcal W_n(\xi_{n,m-k}).                               \label{tmrate:eq:cardinal-average}
\end{equation}
...
\label{tmrate:lem:cardinal-second-moment}
For $n\ge3$,
\begin{equation}
 \sum_{k\in\Z}k^2\lambda_{n,k}
 =\frac{n}{12}+\frac19.                                   \label{tmrate:eq:lambda-second-moment}
\end{equation}
\end{lemma}
...
product \eqref{tmrate:eq:up-cosine-product} represents the Rvachev law as the sum of
...
\begin{equation*}
 ...
 =\frac19.
\end{equation*}
Variances add under convolution, giving \eqref{tmrate:eq:lambda-second-moment}.
\end{proof}

\section{Discrete curvature of the coefficient rows}
\label{tmrate:sec:curvature}

Write
\begin{equation}
 w_{n,m}=\frac{a_{n,m}}{2^{\binom n2}},                    \label{tmrate:eq:w-def}
\end{equation}
...
\begin{equation}
 P_r(z)=P_{r-1}(z)(1+z+\cdots+z^{2^r-1})                  \label{tmrate:eq:P-recursion}
\end{equation}
shows that every coefficient of $P_r$ is a partial sum of coefficients of
$P_{r-1}$.  Consequently
\begin{equation}
 0\le a_{r,m}\le P_{r-1}(1)=2^{\binom r2}.                 \label{tmrate:eq:coefficient-max}
\end{equation}
...
\label{tmrate:lem:discrete-curvature}
For $n\ge3$ and every $m\in\Z$,
\begin{equation}
 \abs{w_{n,m}-2w_{n,m-1}+w_{n,m-2}}
 \le 32\,4^{-n}.                                          \label{tmrate:eq:curvature-bound}
\end{equation}
\end{lemma}
...
\begin{equation}
 (1-z)^2P_n(z)
 =(1-z^{2^{n-1}})(1-z^{2^n})P_{n-2}(z).                   \label{tmrate:eq:second-difference-product}
\end{equation}
The coefficient on the left is
$a_{n,m}-2a_{n,m-1}+a_{n,m-2}$.  The coefficient on the right is a signed sum
of four coefficients of $P_{n-2}$.  By \eqref{tmrate:eq:coefficient-max}, each has
...
\label{tmrate:thm:center-estimate}
For every $n\ge3$ and every $m\in\Z$,
\begin{equation}
 \boxed{\displaystyle
 \abs{\mathcal W_n(\xi_{n,m})-\Up(\xi_{n,m})}
 \le
 \left(\frac{4n}{3}+\frac{16}{9}\right)4^{-n}.}            \label{tmrate:eq:center-estimate}
\end{equation}
\end{theorem}

\begin{proof}
Use \eqref{tmrate:eq:cardinal-average}, symmetry of $\lambda_{n,k}$, and
$\sum_k\lambda_{n,k}=1$:
\begin{align*}
 \Up(\xi_{n,m})-w_{n,m}
...
\end{align*}
By \cref{tmrate:lem:discrete-curvature,tmrate:lem:symmetric-discrete-taylor}, the absolute
...
\end{align*}
which is \eqref{tmrate:eq:center-estimate}.
\end{proof}
...
If one avoids the Poisson calculation in \cref{tmrate:lem:cardinal-second-moment}, the
...
\label{tmrate:sec:main-rate}

We now combine the exact drift with the center estimate.  Recall
$\mathcal A_n$ from \eqref{tmrate:eq:A-def} and $\delta_n$ from \eqref{tmrate:eq:delta-def}.

\begin{theorem}[Uniform second-order expansion]
\label{tmrate:thm:second-order-expansion}
For every $n\ge3$ and every $x\in[0,1]$,
\begin{equation}
 \boxed{\displaystyle
 \mathcal A_n(x)
 =F(x)+F'(x)\delta_n(x)
 +\frac12F''(x)\delta_n(x)^2+\rho_n(x),}                   \label{tmrate:eq:second-order-expansion}
\end{equation}
where
\begin{equation}
 \abs{\rho_n(x)}\le
 \left(\frac{4n}{3}+\frac{16}{9}\right)4^{-n}
 +\frac43(n+2)^3 8^{-n}.                                  \label{tmrate:eq:second-order-remainder}
\end{equation}
\end{theorem}

\begin{proof}
Put $m=\floor{2^n x}$ and $y=x-1$.  Since $m<2^{n+1}$,
\eqref{tmrate:eq:exact-grid-histogram} and \cref{tmrate:lem:center-drift} give
\begin{equation*}
 \mathcal A_n(x)=\mathcal W_n(\xi_{n,m}),
 \qquad
 \xi_{n,m}=y+\delta_n(x).
\end{equation*}
By \cref{tmrate:thm:center-estimate}, replacing $\mathcal W_n$ by $\Up$ at this center
...
\end{equation*}
By \eqref{tmrate:eq:delta-decompose},
$\delta^3\le(n+2)^3/2^{3n+3}$, so
$\abs R\le\frac43(n+2)^3 8^{-n}$.  Finally
$\Up^{(r)}(x-1)=F^{(r)}(x)$ on $[0,1]$ by \eqref{tmrate:eq:fold}.
\end{proof}

For the leading rate it is convenient to absorb the quadratic Taylor term.

\begin{corollary}[Uniform first-order expansion]
\label{tmrate:cor:first-order-expansion}
For $n\ge3$ and $x\in[0,1]$,
\begin{equation}
 \mathcal A_n(x)=F(x)+F'(x)\delta_n(x)+r_n(x),              \label{tmrate:eq:first-order-expansion}
\end{equation}
with
\begin{equation}
 \abs{r_n(x)}\le
 \left(n^2+\frac{16}{3}n+\frac{52}{9}\right)4^{-n}
 <(n+3)^2 4^{-n}.                                         \label{tmrate:eq:first-order-remainder}
\end{equation}
\end{corollary}
...
$(n+2)^2 4^{-n}$.  Add \eqref{tmrate:eq:center-estimate}.
\end{proof}

Substituting \eqref{tmrate:eq:delta-decompose} into
\eqref{tmrate:eq:second-order-expansion} exposes three distinct scales.

\begin{corollary}[Binary-phase expansion]
\label{tmrate:cor:binary-phase-expansion}
...
\begin{align}
 ...
 &\quad+\frac18F''(x)\,n^2 4^{-n}
   +\bigO(n4^{-n}).                                       \label{tmrate:eq:binary-phase-expansion}
\end{align}
...
\eqref{tmrate:eq:second-order-expansion}.  The cross term in $\delta_n^2$ is
$\bigO(n4^{-n})$, the constant term is $\bigO(4^{-n})$, and the remainder in
\eqref{tmrate:eq:second-order-remainder} is uniformly $\bigO(n4^{-n})$ because
$n^3 8^{-n}=o(n4^{-n})$.
\end{proof}

The second term in \eqref{tmrate:eq:binary-phase-expansion} is not periodic in $n$ for
...
\begin{equation}
 ...
 \abs{\frac{2^n}{n}\bigl(\mathcal A_n(x)-F(x)\bigr)
       -\frac12F'(x)}
 \le \frac2n+
 \left(n+\frac{16}{3}+\frac{52}{9n}\right)2^{-n}.}          \label{tmrate:eq:uniform-profile-bound}
\end{equation}
Consequently
\begin{equation}
 \frac{2^n}{n}\bigl(\mathcal A_n-F\bigr)
 \longrightarrow\frac12F'
 \qquad\text{uniformly on }[0,1].                           \label{tmrate:eq:uniform-profile-limit}
\end{equation}
In particular, for every fixed $x\in(0,1)$,
\begin{equation}
 \lim_{n\to\infty}
 \frac{2^n}{n}\bigl(\mathcal A_n(x)-F(x)\bigr)
 =\frac12F'(x).                                             \label{tmrate:eq:pointwise-rate}
\end{equation}
...
\begin{proof}
From \eqref{tmrate:eq:delta-decompose},
\begin{equation*}
 \frac{2^n}{n}\delta_n(x)
 =\frac12+\frac{1-\theta_n(x)}{n}.
\end{equation*}
Insert this in \eqref{tmrate:eq:first-order-expansion}.  Since $0\le F'\le2$ and
$0<1-\theta_n(x)\le1$, the phase term is bounded by $2/n$.  Multiplying
\eqref{tmrate:eq:first-order-remainder} by $2^n/n$ gives the second term on the
right of \eqref{tmrate:eq:uniform-profile-bound}.
\end{proof}

At the endpoints the limiting profile vanishes.  Their unscaled errors obey a
different, much faster regime; see \cref{tmrate:sec:special-values}.

\subsection{The exact uniform constant}

The upper bound follows immediately from $0\le F'\le2$ and
\cref{tmrate:cor:first-order-expansion}:
\begin{equation}
 \norm{\mathcal A_n-F}_{L^\infty[0,1]}
 \le (n+2)2^{-n}+(n+3)^2 4^{-n}.                           \label{tmrate:eq:uniform-upper}
\end{equation}
To prove that the coefficient is not an artifact of this estimate, we evaluate
the midpoint exactly in \cref{tmrate:prop:midpoint-exact}.  That formula gives
\begin{equation}
 \norm{\mathcal A_n-F}_{\infty}
 \ge (n+2)2^{-n}-2^{-\binom n2}.                           \label{tmrate:eq:uniform-lower}
\end{equation}

\begin{theorem}[Sharp uniform convergence rate]
\label{tmrate:thm:uniform-rate}
...
\begin{equation}
 ...
 \le (n+3)^2 4^{-n}.}                                      \label{tmrate:eq:uniform-finite-bracket}
\end{equation}
Consequently
\begin{equation}
 \boxed{\displaystyle
 ...
 \qquad
 \lim_{n\to\infty}\frac{2^n}{n}
 \norm{\mathcal A_n-F}_{L^\infty[0,1]}=1.}                \label{tmrate:eq:uniform-rate}
\end{equation}
\end{theorem}

\begin{proof}
Subtract $(n+2)2^{-n}$ from \eqref{tmrate:eq:uniform-upper} and
\eqref{tmrate:eq:uniform-lower}.  This gives \eqref{tmrate:eq:uniform-finite-bracket}; the
...
\eqref{tmrate:eq:uniform-profile-limit}, since $\norm{F'/2}_\infty=1$.
\end{proof}
...
\end{axis}
\end{tikzpicture}
\caption{The three scales in \eqref{tmrate:eq:binary-phase-expansion}.  The deterministic
center drift $n2^{-n}$ dominates the one-cell scale $2^{-n}$ and the quadratic
Taylor scale $n^2 4^{-n}$.}
\label{tmrate:fig:three-scales}
\end{figure}

\section{Exact special values}
\label{tmrate:sec:special-values}
...
Since $a_{n,0}=1$,
\begin{equation}
 \mathcal A_n(0)=2^{-\binom n2}.                            \label{tmrate:eq:A-zero}
\end{equation}
...
\begin{equation}
 \mathcal A_n(1)=1-2^{-\binom n2}.                          \label{tmrate:eq:A-one}
\end{equation}
Hence
\begin{equation}
 \mathcal A_n(0)-F(0)=2^{-\binom n2},
 \qquad
 \mathcal A_n(1)-F(1)=-2^{-\binom n2}.                     \label{tmrate:eq:endpoint-errors}
\end{equation}
...
\label{tmrate:prop:midpoint-exact}
For every $n\ge2$,
\begin{equation}
 \boxed{\displaystyle
 \mathcal A_n\!\left(\frac12\right)
 =\frac12+\frac{n+2}{2^n}-2^{-\binom n2}.}                 \label{tmrate:eq:midpoint-exact}
\end{equation}
Consequently
\begin{equation}
 \frac{2^n}{n}
 \left(\mathcal A_n\!\left(\frac12\right)-\frac12\right)
 =1+\frac2n-\frac{2^{n-\binom n2}}{n}.                    \label{tmrate:eq:midpoint-scaled}
\end{equation}
\end{proposition}
...
\eqref{tmrate:eq:midpoint-scaled}.

\section{The corresponding result for Rvachev's bump}
\label{tmrate:sec:up-rate}

For $0\le t<1$, let
\begin{equation}
 \vartheta_n(t)=\fracpart{2^{n+1}t},
 \qquad
 \Delta_n(t)=\frac{n+2-2\vartheta_n(t)}{2^{n+1}}.          \label{tmrate:eq:Delta-up}
\end{equation}
Then
\begin{equation}
 \xi_{n,\floor{2^{n+1}t}}=2t-1+\Delta_n(t).                \label{tmrate:eq:up-center-drift}
\end{equation}
The proof of \cref{tmrate:thm:second-order-expansion} gives, uniformly for $0\le t<1$,
\begin{align}
 \mathcal J_n(t)
 &=\Up(2t-1)+\Up'(2t-1)\Delta_n(t) \notag\\
 &\quad+\frac12\Up''(2t-1)\Delta_n(t)^2+\rho_n^{\Up}(t), \label{tmrate:eq:up-second-order}
\end{align}
with the same bound as \eqref{tmrate:eq:second-order-remainder}.  The right endpoint is
exceptional because the coefficient identity stops one index earlier:
\begin{equation}
 \mathcal J_n(1)=-2^{-\binom n2},
 \qquad \Up(1)=0.                                         \label{tmrate:eq:J-right-endpoint}
\end{equation}
...
\label{tmrate:cor:up-rate}
For fixed $t\in(0,1)$,
\begin{equation}
 \frac{2^n}{n}
 \bigl(\mathcal J_n(t)-\Up(2t-1)\bigr)
 \longrightarrow\frac12\Up'(2t-1).                       \label{tmrate:eq:up-pointwise-rate}
\end{equation}
At $t=1/2$, both the derivative term and the actual error collapse:
\begin{equation}
 \mathcal J_n(1/2)=1-2^{-\binom n2}.                       \label{tmrate:eq:J-half}
\end{equation}
Moreover,
\begin{equation}
 \lim_{n\to\infty}\frac{2^n}{n}
 \norm{\mathcal J_n-\Up(2\,\cdot-1)}_{L^\infty[0,1]}=1.  \label{tmrate:eq:up-uniform-rate}
\end{equation}
\end{corollary}
...
\label{tmrate:sec:other-rates}

The sharp $n2^{-n}$ rate belongs to the particular moving sample
\eqref{tmrate:eq:A-def}.  It should not be attributed to the coefficient histogram
itself.
...
\cref{tmrate:thm:center-estimate},
\begin{equation}
 \norm{\mathcal W_n-\Up}_\infty
 \le 2^{-n}+
 \left(\frac{4n}{3}+\frac{16}{9}\right)4^{-n}.             \label{tmrate:eq:histogram-upper}
\end{equation}
...
\begin{equation}
 \boxed{\displaystyle
 \lim_{n\to\infty}2^n\norm{\mathcal W_n-\Up}_\infty=1.}   \label{tmrate:eq:histogram-sharp}
\end{equation}
...
\begin{equation}
 ...
 \qquad(m\in\Z).                                          \label{tmrate:eq:linear-interpolant}
\end{equation}
...
\cref{tmrate:thm:center-estimate} give
\begin{equation*}
...
\begin{equation}
 ...
 \le\left(\frac{4n}{3}+\frac{25}{9}\right)4^{-n}.}        \label{tmrate:eq:linear-rate}
\end{equation}
...
\eqref{tmrate:eq:second-difference-product}.
...
\eqref{tmrate:eq:derivative-bounds}, not fitted numerical estimates.
\end{enumerate}
...
\begin{equation}
 ...
 \;\gg\;
 \underbrace{n4^{-n}}_{\text{intrinsic center error}}.     \label{tmrate:eq:hierarchy}
\end{equation}
For fixed $x$, the first two terms are explicitly visible in
\eqref{tmrate:eq:binary-phase-expansion}.  The third is the first smooth nonlinear
...
A formal proof of \cref{tmrate:thm:uniform-rate} can be organized into the following
small additions.

\begin{enumerate}
\item Define the zero-extended normalized coefficient row $w_{n,m}$ on $\Z$ and
      prove \eqref{tmrate:eq:curvature-bound} by coefficient extraction from
      \eqref{tmrate:eq:second-difference-product}.  This is algebraic and should require
      no measure theory.

\item Reuse the existing cosine-product theorem to define $q_n$ and prove
      $\Up=\mu_n*q_n$.  The identity \eqref{tmrate:eq:tail-factorization} is a direct
...
\item Prove the cardinal partition
      ...
      identity \eqref{tmrate:eq:cardinal-average} then follows from convolution with an
      atomic measure.
...
\item In a second refinement, invoke the repository's Poisson-summation layer to
      prove the exact second moment \eqref{tmrate:eq:lambda-second-moment}; this upgrades
      the center estimate to \eqref{tmrate:eq:center-estimate}.

\item Formalize the exact drift identity \eqref{tmrate:eq:center-drift}, apply the
      repository's derivative bounds, and finish with a uniform Taylor estimate.
      The exact midpoint formula \eqref{tmrate:eq:midpoint-exact} supplies the matching
...
$\delta_n(x)$ & exact center displacement in \eqref{tmrate:eq:delta-def}\\
...
\bibitem{tmrate:arias2018}
J.~Arias de Reyna,
\newblock Arithmetic of the Fabius function,
\newblock \emph{INTEGERS} \textbf{18} (2018), Article A51;
  arXiv:\nolinkurl{1702.06487}, version~3.

\bibitem{tmrate:allouche2003}
J.-P.~Allouche and J.~Shallit,
\newblock \emph{Automatic Sequences: Theory, Applications, Generalizations},
\newblock Cambridge University Press, 2003.

\bibitem{tmrate:proveit}
V.~Reshetnikov,
\newblock \emph{ProveIt: Analysis/FabiusFunction},
\newblock Lean~4 formal development, main branch studied 25 August 2026.
\newblock \url{https://github.com/VladimirReshetnikov/ProveIt/tree/main/Analysis/FabiusFunction}.

\bibitem{tmrate:synthesis}
V.~Reshetnikov,
\newblock \emph{The Fabius Function and Rvachev's Up-Function: Exact arithmetic,
  probability, Fourier analysis, and corrected sharp asymptotics},
...
\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Fabius_Thue_Morse_Convergence_Rate/Fabius_Thue_Morse_Convergence_Rate.tex


% BEGIN MERGED SOURCE: Fabius_Thue_Morse_Convergence_Rate-2/Fabius_Thue_Morse_Convergence_Rates.tex
% Original SHA-256: 384d69b461cb94af33f1c080703e269ea77104405d1940413f1944172b3312c0
\clearpage
\part{Signed Thue--Morse Prefix Sums and the Fabius Function}
\label{part:tmrates}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Fabius_Thue_Morse_Convergence_Rate-2/Fabius_Thue_Morse_Convergence_Rates.tex}.  Sharper Fourier/deconvolution treatment of the same approximation family, including center-grid expansions, sharp constants, all-orders patterns, and accelerated reconstructions.  Its qualitative bridge reuses formal inputs, but its deconvolution estimates, quantitative rates, sharp constants, all-orders pattern, and acceleration results remain natural-language proofs and formal obligations.\par\smallskip Original source SHA-256: \nolinkurl{384d69b461cb94af33f1c080703e269ea77104405d1940413f1944172b3312c0}.
\end{boxedremark}
\section*{Dossier abstract}
Let $\varepsilon_m=(-1)^{\wt(m)}$ be the signed Thue--Morse sequence and let
...
Let
\begin{equation}
 \varepsilon_m=(-1)^{\wt(m)},\qquad m\ge0,                 \label{tmrates:eq:tm-sign}
\end{equation}
...
\begin{equation}
 ...
 \qquad
 \mu_n=\sum_{m\in\Z}p_{n,m}\,\delta_{\lambda_{n,m}},    \label{tmrates:eq:atomic-law}
\end{equation}
...
\begin{equation}
 \mathcal W_n(x)=\sum_m H_n(\lambda_{n,m})\mathbf 1_{I_{n,m}}(x), \label{tmrates:eq:Wn}
\end{equation}
...
repository proves
\begin{equation}
 \mathcal W_n(x)\longrightarrow\Up(x)                     \label{tmrates:eq:qualitative-W}
\end{equation}
...
\begin{equation}
 \Up(x)\dd x=\mu_n*\mathcal L(T_n).                        \label{tmrates:eq:tail-convolution}
\end{equation}
...
\]
with $q=1/2$ gives \eqref{tmrates:eq:rn}.  Independence and $\Var(R_{\ell,r})=1$ give the variance
...
\begin{equation}
 \supp\mu_n=[-1+r_n,1-r_n]\cap\bigl(h_n(\Z-g_n/2)\bigr),  \label{tmrates:eq:support-mun}
\end{equation}
...
\begin{equation}
 ...
 =-\frac{3n+4}{72\,4^n}\Up''(\lambda).                    \label{tmrates:eq:heuristic-leading}
\end{equation}
...
\begin{equation}
 \frac{p_m}{h}
 =\int_{-1/(2h)}^{1/(2h)}
   \widehat\mu(\xi)\e^{2\pi\ii(a+hm)\xi}\dd\xi.          \label{tmrates:eq:lattice-inversion}
\end{equation}
\end{lemma}
...
\begin{equation}
 ...
 =\int_{B_n}\widehat{\mu_n}(\xi)
   \e^{2\pi\ii\lambda_{n,m}\xi}\dd\xi.                  \label{tmrates:eq:Hn-inversion}
\end{equation}
...
\begin{equation}
 H_n=\Up-\frac{v_n}{2}\Up''+\cdots,                        \label{tmrates:eq:variance-form}
\end{equation}
...
Uniformly on $\R$,
\begin{equation}
 \frac{4^n}{n}(L_n-\Up)\longrightarrow-\frac1{24}\Up'',   \label{tmrates:eq:linear-profile}
\end{equation}
in the supremum norm.  In particular,
\begin{equation}
 \|L_n-\Up\|_\infty=\frac n{3\,4^n}(1+o(1)).              \label{tmrates:eq:linear-sharp}
\end{equation}
\end{corollary}
...
\begin{equation}
 \mathcal W_n(x)-\Up(x)
 =h_n\rho_n(x)\Up'(x)+\bigO(nh_n^2).                      \label{tmrates:eq:hist-local}
\end{equation}
...
Equivalently,
\begin{equation}
 \lim_{n\to\infty}2^n\|\mathcal W_n-\Up\|_\infty=1.      \label{tmrates:eq:hist-limit-constant}
\end{equation}
\end{theorem}
...
Let
\begin{equation}
 \theta_n(x)=\fract{2^nx}=2^nx-\lfloor2^nx\rfloor.        \label{tmrates:eq:theta}
\end{equation}
...
\begin{equation}
 H_n(-1/2)=\frac12=\Up(-1/2) \qquad(n\ \text{even}).       \label{tmrates:eq:midpoint-centered-exact}
\end{equation}
...
\begin{equation}
 ...
 =-\frac{n^2}{1152\,16^n}\Up^{(4)}(\lambda)
  +\smallo\!\left(\frac{n^2}{16^n}\right).                \label{tmrates:eq:accelerated-profile}
\end{equation}
Consequently,
\begin{equation}
...
\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Fabius_Thue_Morse_Convergence_Rate-2/Fabius_Thue_Morse_Convergence_Rates.tex


% BEGIN MERGED SOURCE: Fabius_Inverse_and_Saddle_Research_Frontiers/Fabius_Inverse_and_Saddle_Research_Frontiers.tex
% Original SHA-256: f9d8605761aaaa1b2c2af83e3c5c55dcd6acfc847402163458b03b68c7b35ff8
\clearpage
\part{Fabius Inverse and Saddle Research Frontiers}
\label{part:inverse}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Fabius_Inverse_and_Saddle_Research_Frontiers/Fabius_Inverse_and_Saddle_Research_Frontiers.tex}.  Specialist inverse and saddle dossier.  Formalized inputs are distinguished from derived consequences; inverse endpoint asymptotics and the logarithmic highest-derivative transfer remain explicit obligations.\par\smallskip Original source SHA-256: \nolinkurl{f9d8605761aaaa1b2c2af83e3c5c55dcd6acfc847402163458b03b68c7b35ff8}.
\end{boxedremark}
\section*{Dossier abstract}
...
\begin{align}
 ...
 F'(x)&=2\Up(2x-1),&&&0<x<1, \label{inverse:eq:Fprime-up}
\end{align}
...
For $x,y\in[0,1]$,
\begin{align}
 \InvF(y)\leq x&\iff y\leq F(x), \label{inverse:eq:inv-le}\\
 F(x)\leq y&\iff x\leq \InvF(y), \label{inverse:eq:F-le}\\
 \InvF(y)<x&\iff y<F(x), \label{inverse:eq:inv-lt}\\
 F(x)<y&\iff x<\InvF(y). \label{inverse:eq:F-lt}
\end{align}
\end{theorem}
...
\begin{align}
 ...
 \InvF(1-y)&=1-\InvF(y). \label{inverse:eq:inv-reflection}
\end{align}
In particular,
\[
...
then
\begin{align}
 y&\leq C_n\InvF(y)^n, \label{inverse:eq:inverse-effective}\\
 y&\leq \frac{C_n}{n!}\InvF(y)^n. \label{inverse:eq:inverse-sharp}
\end{align}
...
\eqref{inverse:eq:inv-le}, $\InvF(y)\leq2^{-n}$, which is exactly the required
scale condition.
\end{proof}
...
\begin{align}
 \InvF(y)&\geq C_n^{-1/n}y^{1/n}, \label{inverse:eq:root-effective}\\
 \InvF(y)&\geq
...
\[
 y\leq8\InvF(y)^2. \label{inverse:eq:quadratic-inv}
\]
...
\]
By \cref{inverse:thm:effective-linear}, $By<\InvF(y)$, and hence
\[
...
\begin{proof}
For $x\in(0,1)$, \eqref{inverse:eq:Fprime-up} and positivity of $\Up$ on
...
$F(\InvF(y))=y$ and substituting \eqref{inverse:eq:Fprime-up} proves
...
\eqref{inverse:eq:Gprime} gives $\InvF'(1/2)=1/2$.  The up-function is even, so
$\Up'(0)=0$, hence $F''(1/2)=4\Up'(0)=0$; now use
\eqref{inverse:eq:Gsecond}.
\end{proof}
...
The secant slopes from the origin diverge by \cref{inverse:thm:inverse-quotient}, and
...
\begin{equation}
 x=\lambda2^{-\lambda}
   =\lambda\e^{-L\lambda}. \label{inverse:eq:lambert-phase}
\end{equation}
Equivalently,
\[
 \lambda(x)=-\frac1L W_{-1}(-Lx),
...
\begin{align}
 y^\alpha&=o(\InvF(y)), \label{inverse:eq:power-below-G}\\
...
In particular,
\begin{align}
 E_1&=\ii v\,\frac{3d_0+v^2}{3}, \label{inverse:eq:E1}\\
 E_2&=v^2\,\frac{-2d_1+v^2}{4}, \label{inverse:eq:E2}\\
 E_3&=\ii v^3\left(-\frac{d_2}{6}-\frac{v^2}{5}\right), \label{inverse:eq:E3}\\
...
\]
because $\ii^2=-1$.  This is \eqref{inverse:eq:E-closed}.  Substitution of
...
\begin{proof}
Differentiate \eqref{inverse:eq:B-definition} with respect to $\varepsilon$.
...
$\varepsilon^{n-1}$ gives \eqref{inverse:eq:B-recursion}.  Expanding the
exponential as
\[
 \sum_{r\geq0}\frac1{r!}
...
\cref{inverse:prop:exp-recursion} for $B_{2j}$.  Each factor
\[
 E_{m_i}
 =\ii^{m_i}\left(
...
\begin{proof}
Differentiate \eqref{inverse:eq:log-coeff-definition} logarithmically:
\[
...
\[
 ...
 \qquad a_0=1,
\]
and solving for $A_n$ proves \eqref{inverse:eq:A-recursion}.  Formula
\eqref{inverse:eq:A-compositions} is the coefficient of $z^n$ in the ordinary
series
\[
...
\label{inverse:app:inverse-reference}
...
\label{inverse:app:coefficient-reference}
...
\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Fabius_Inverse_and_Saddle_Research_Frontiers/Fabius_Inverse_and_Saddle_Research_Frontiers.tex


% BEGIN MERGED SOURCE: Small_Argument_Asymptotics/Small_Argument_Asymptotics.tex
% Original SHA-256: 85f51a20fc7b6bdf3b1d049ec4506f508aee3c4cd70554e6a68cbcc30977cb0b
\clearpage
\part{Small-Argument Asymptotics Research Frontiers}
\label{part:smallarg}
\begin{boxedremark}
\textbf{Source provenance and status.} Former path: \nolinkurl{Small_Argument_Asymptotics/Small_Argument_Asymptotics.tex}.  Specialist explicit-coefficient dossier.  Closed higher coefficients, ordered-composition formulas, Fourier-amplitude laws, numerical tables, traps, and open problems remain research targets unless an exact Lean theorem is cited.\par\smallskip Original source SHA-256: \nolinkurl{85f51a20fc7b6bdf3b1d049ec4506f508aee3c4cd70554e6a68cbcc30977cb0b}.
\end{boxedremark}
\section*{Dossier abstract}
As $x\to0^+$ the Fabius function $F$ is flat to infinite order.  Its Taylor
series at \(0\) is identically zero and does not represent \(F\) on any right
neighborhood; the correct local description is an exponentially small
...
constants.  At the historical starting point of this dossier, those coefficients existed
only as the output of a chain
...
only $A_1$ had been written down.  This note closes the chain.  We solve
...
the pages below do not.  In particular, the ordered-composition formulas, general
logarithmic highest-derivative law,
expanded higher coefficients, Fourier-amplitude estimates, uniqueness
...
vanishes at the origin, so $F$ is flat there.  Its identically-zero Taylor series fails to
represent \(F\) on every right neighborhood of the origin.
...
The development snapshot from which this dossier began already contained the corrected sharp main term
...
recursion}.  At that historical stage only $A_1$ was displayed.

What this note adds:

\begin{enumerate}
\item A closed form for the saddle jets (\cref{smallarg:sec:jets}).  The differential
...
\item A closed form for the exponent coefficients (\cref{smallarg:sec:exponent}), with
...
\item A fully explicit, non-recursive formula for every coefficient
  (\cref{smallarg:sec:general}): $A_j$ is a finite triple sum over compositions, subsets
  and Gaussian moments.
\item The explicit coefficients $A_2$ and $A_3$ (\cref{smallarg:sec:explicit}), which did
  not exist anywhere before, in two equivalent forms.
\item The leading-derivative law (\cref{smallarg:sec:leading}): the top term of $A_j$ is
...
\item The Gamma--zeta Fourier form of every coefficient and the resulting
  amplitude law (\cref{smallarg:sec:fourier}).
\item An independent end-to-end numerical verification (\cref{smallarg:sec:numerics})
...
$\Psi$ & the centered one-periodic correction of \cref{smallarg:sec:main}\\
$\Psi^{(m)}$ & $m$-th derivative of $\Psi$\\
$\lambda=\lambda(x)$ & the lower-Lambert phase of \cref{smallarg:sec:lambert}\\
$\mathcal M(x)$ & the sharp main term of \cref{smallarg:sec:main}\\
$A_j$ & the $j$-th expansion coefficient, a one-periodic function\\
$a_j$ & the $j$-th \emph{mass} coefficient, before taking the logarithm\\
$d_n$ & the $n$-th bounded exponent jet of \cref{smallarg:sec:jets}\\
$p_n$ & the $n$-th periodic jet of \cref{smallarg:sec:jets}\\
...
$\lambda$ solves
\begin{equation}\label{smallarg:eq:lambda}
  \lambda\,2^{-\lambda}=x,
  \qquad\text{equivalently}\qquad
...
order after that; \cref{smallarg:sec:notclaimed} explains why substituting an expansion
of $\lambda$ into the coefficients is not a legitimate step.

For dyadic arguments $x=2^{-n}$, equation \cref{smallarg:eq:lambda} reads
...
\begin{equation}\label{smallarg:eq:psihat}
  \Psihat(0)=0,\qquad
...
\end{aligned}
\]
so \(\Psi\) itself has amplitude only about \(2\times10^{-6}\) (roughly
\(4\times10^{-6}\) peak to peak).  \Cref{smallarg:sec:traps}
...
\begin{equation}\label{smallarg:eq:main}
...
\begin{equation}\label{smallarg:eq:jetrec}
...
Write $D$ for $\dd/\dd t$.  The recursion \cref{smallarg:eq:jetrec} reads
...
\begin{theorem}[Closed-form jets]\label{smallarg:thm:jets}
...
\begin{remark}
\Cref{smallarg:thm:jets} is formalized as
\nolinkurl{Fabius.negativeLaplacePeriodicJet_eq_closedForm} and
\nolinkurl{Fabius.negativeLaplaceBoundedExponentJet_eq_closedForm}
...
\begin{theorem}[Closed-form exponent]\label{smallarg:thm:exponent}
...
\begin{remark}
\Cref{smallarg:thm:exponent} is formalized as
\nolinkurl{Fabius.negativeLaplaceExponentCoefficient_succ_eq}
and
\nolinkurl{Fabius.negativeLaplaceExponentPolynomial_succ_eq}
...
\begin{theorem}[Explicit mass coefficients]\label{smallarg:thm:mass}
For every $j\ge1$,
\[
...
\begin{theorem}[Explicit logarithmic coefficients]\label{smallarg:thm:log}
...
Substituting \cref{smallarg:thm:jets} into \cref{smallarg:thm:mass,smallarg:thm:log} expresses $A_j(u)$ as
...
\item The mean of $A_j$ is its ``$\Psi$-free part'' plus the mean of the
  ...
  which carries the amplification $(2\pi k)^{a+b}$ of \cref{smallarg:sec:fourier}.
  Applying this to \cref{smallarg:eq:A2psi} and to $A_1$ leaves
  \begin{align*}
...
\]
\begin{equation}\label{smallarg:eq:A2jets}
...
fourth independent check of \cref{smallarg:thm:leading}.
...
\begin{theorem}[Leading term]\label{smallarg:thm:leading}
...
\cref{smallarg:eq:A2psi,smallarg:eq:A3psi}.
...
\cref{smallarg:sec:general} together with \cref{smallarg:eq:psihat} turns every coefficient into
...
\cref{smallarg:sec:general} records the same caution for the means.
...
\]
Consequences for $A_2$, all confirmed numerically in \cref{smallarg:sec:numerics}.
For the first Fourier mode, the linearized amplitude is
$2|c_2(1)\Psihat(1)|$.  The left column is what the linearized form gives, the right what the exact
...
The top term of \cref{smallarg:thm:leading} dominates the oscillation, and it multiplies
...
\cref{smallarg:thm:leading} and the reason the coefficient of the top jet is the mild
...
implementation with \cref{smallarg:sec:numerics}.
...
Neither this table nor \cref{smallarg:sec:numerics} is machine-checked.  What is
machine-checked is listed in \cref{smallarg:sec:lean}.

\section{Map to the Lean development}\label{smallarg:sec:lean}

\begingroup\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.15\textwidth}>{\raggedright\arraybackslash}p{0.40\textwidth}>{\raggedright\arraybackslash}p{0.37\textwidth}@{}}
\toprule
...
$P$ & \nolinkurl{negativeLaplacePeriodicCorrection} & \nolinkurl{PeriodicCorrection.lean}\\
$\Psi$ & \nolinkurl{negativeLaplacePsi} & \nolinkurl{PeriodicCorrection.lean}\\
$\Psihat(k)$ & Fourier-coefficient theorems & \nolinkurl{PeriodicFourier.lean}\\
$C_F$ & \nolinkurl{fabiusSharpAsymptoticConstant} & \nolinkurl{FabiusSharpConstant.lean}\\
$\mathcal M(x)$ & \nolinkurl{fabiusSharpLambertMain} & \nolinkurl{FabiusSharpLambertMain.lean}\\
$s_n$ & \nolinkurl{negativeLaplaceJetSlope} & \nolinkurl{FabiusSaddleCoefficientRecurrence.lean}\\
$p_n$ & \nolinkurl{negativeLaplacePeriodicJet} & \nolinkurl{FabiusSaddleCoefficientRecurrence.lean}\\
$d_n$ & \nolinkurl{negativeLaplaceBoundedExponentJet} & \nolinkurl{FabiusSaddleCoefficientRecurrence.lean}\\
$c(n,m)$ & \nolinkurl{negativeLaplaceJetStirling} & \textbf{\nolinkurl{FabiusSaddleJetClosedForm.lean}}\\
$\Psi^{(m+1)}/L^{m+1}$ & \nolinkurl{negativeLaplacePsiScaledDeriv} & \textbf{\nolinkurl{FabiusSaddleJetClosedForm.lean}}\\
closed-form $p_n$ & \nolinkurl{negativeLaplacePeriodicJet_eq_closedForm} & \textbf{\nolinkurl{FabiusSaddleJetClosedForm.lean}}\\
closed-form $d_n$ & \nolinkurl{negativeLaplaceBoundedExponentJet_eq_closedForm} & \textbf{\nolinkurl{FabiusSaddleJetClosedForm.lean}}\\
$E_m$ & \nolinkurl{negativeLaplaceExponentCoefficient} & \nolinkurl{FabiusSaddleCoefficientRecurrence.lean}\\
closed-form $E_m$ & \nolinkurl{negativeLaplaceExponentCoefficient_succ_eq} & \textbf{\nolinkurl{FabiusSaddleExponentClosedForm.lean}}\\
$\operatorname{expCoeff}$ & \nolinkurl{SaddleExpansion.expCoeff} & \nolinkurl{SaddleExpansionAlgebra.lean}\\
contraction & \nolinkurl{SaddleExpansion.gaussianPolynomialContraction} & \nolinkurl{GaussianPolynomialContraction.lean}\\
$a_j$ & \nolinkurl{fabiusSaddleMassCoefficient} & \nolinkurl{FabiusSaddleExpansionCoefficients.lean}\\
$\operatorname{logCoeff}$ & \nolinkurl{SaddleExpansion.logCoeff} & \nolinkurl{SaddleLogExpansionAlgebra.lean}\\
$A_j$ & \nolinkurl{fabiusSaddleLogCoefficient} & \nolinkurl{FabiusSaddleExpansionCoefficients.lean}\\
$A_1$ & \nolinkurl{fabiusFirstSaddleCorrection} & \nolinkurl{FabiusFirstSaddleCorrection.lean}\\
$A_2$ & \nolinkurl{fabiusSecondSaddleCorrection} & \textbf{\nolinkurl{FabiusSecondSaddleCorrection.lean}}\\
$a_2$ & \nolinkurl{fabiusSaddleMassCoefficient_two} & \textbf{\nolinkurl{FabiusSecondSaddleCorrection.lean}}\\
$c(n,m)$ identified & \nolinkurl{negativeLaplaceJetStirling_eq_coeff} & \textbf{\nolinkurl{FabiusSaddleJetStirling.lean}}\\
$\operatorname{expCoeff}$ at $3,4$ & \nolinkurl{expCoeff_three}, \nolinkurl{expCoeff_four} & \textbf{\nolinkurl{FabiusSecondSaddleCorrection.lean}}\\
order-three expansion & \nolinkurl{log_fabius_sub_twoSaddleCorrections_isBigO} & \textbf{\nolinkurl{FabiusSecondSaddleExpansion.lean}}\\
the expansion & \nolinkurl{log_fabius_sub_sharpLambertMain_hasAsymptoticExpansion} & \nolinkurl{FabiusFullAsymptoticExpansion.lean}\\
truncation & \nolinkurl{log_fabius_sub_sharpLambertExpansion_isBigO} & \nolinkurl{FabiusFullAsymptoticExpansion.lean}\\
exact $F(2^{-n})$ & \nolinkurl{fabiusInversePowTwoTable}, \nolinkurl{evalFabiusDyadic} & \nolinkurl{Arithmetic.lean}\\
\bottomrule
\end{longtable}
\endgroup
...
\emph{Verified symbolically but not formalized.}  \Cref{smallarg:thm:mass,smallarg:thm:log},
...
The coefficients $A_3$ and $A_4$ of \cref{smallarg:eq:A3jets,smallarg:eq:A4jets} were derived
...
\emph{Verified numerically only.}  Everything in \cref{smallarg:sec:numerics}.
...
\item \textbf{No elementary all-orders form.}  The coefficients are evaluated at
  ...
  $p_j(z)=\sum_{m=1}^{j}\frac{s(j,j-m+1)}{m!}z^m$.  But
...
\item \textbf{No convergence}; see \cref{smallarg:sec:fourier}.
...
the arguments \cref{smallarg:eq:psihat} needs.  The singularity is removable in $\zeta$
...
$C^\infty$, one-periodic and bounded, and \cref{smallarg:sec:general} shows that they are
...
\begin{enumerate}
\item Formalize \cref{smallarg:thm:mass,smallarg:thm:log} for the generic
...
\item Formalize \cref{smallarg:thm:leading} for general $j$.
\item Surface effective constants in the $\bigO(\lambda^{-N})$ remainders.
\item Prove or refute convergence of the series in \cref{smallarg:sec:fourier}.
\item Formalize or independently certify the displayed \(A_3\) and \(A_4\)
  formulas and their numerical checks.  The order-six
  \(\operatorname{expCoeff}\), jets \(d_0,\dots,d_5\), and Gaussian moments
  through \(v^{18}\) described above are the data used for \(A_3\), not a recipe
  for an as-yet uncomputed \(A_4\).
\end{enumerate}


\renewcommand{\arraystretch}{1}
% END MERGED SOURCE: Small_Argument_Asymptotics/Small_Argument_Asymptotics.tex
...
dossier-by-dossier.  The required evidence is an exact current Lean declaration whose
