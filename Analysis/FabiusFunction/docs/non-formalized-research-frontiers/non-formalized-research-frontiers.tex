...
identity $T\Up=\Up$ gives
\begin{equation}
 T\bigl(\Up^{(r)}\bigr)=2^{-r}\Up^{(r)}.
 \label{repeated2:eq:T-eigenfunctions}
\end{equation}
Thus $\Up''$ is an eigenfunction with eigenvalue $1/4$.  Matching the tail mass and mean
...
compactly supported law with matching variance, the $\Up''$ term would disappear and the
...
$\beta_h=h^{-1}\mathbf 1_{[-h/2,h/2]}$, put
\(q_n=\beta_{h_n}^{*n}*D_{h_n}\Up\), and rename the cardinal weights
\begin{equation}
 \pi_{n,k}=h_nq_n(kh_n),\qquad k\in\Z.
 \label{tmrates:eq:cardinal-weights}
\end{equation}
They form a symmetric probability distribution and satisfy the exact identities
\begin{align}
 \Up(\lambda_{n,m})
 &=\sum_{k\in\Z}\pi_{n,k}H_n(\lambda_{n,m-k}),
 \label{tmrates:eq:cardinal-average}\\
 \sum_{k\in\Z}k^2\pi_{n,k}&=\frac n{12}+\frac19.
...
\begin{equation}
 |H_n(\lambda_{n,m})-\Up(\lambda_{n,m})|
