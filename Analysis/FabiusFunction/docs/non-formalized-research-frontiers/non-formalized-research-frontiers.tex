...
\small Eleven source notebooks and a post-audit gap register, without unproved promotion}
...
\begin{abstract}
This volume consolidates the eleven mathematical notebooks formerly stored in
subdirectories of \path{Analysis/FabiusFunction/docs/non-formalized-research-frontiers/},
together with the later primary-exposition gap register.
...
Primary-exposition gap register & Smaller claims removed by the exact-counterpart audit & Post-consolidation disposition record; every candidate remains open. \\
\bottomrule
\end{longtable}
...
than recomputing the same formula in a rearranged order.  The last item is now more than a
check: \path{qBinomialThueMorseDyadicFormula_refine} proves it exactly.  The stronger
...
Most algebraic ingredients are already formalized in the repository.  The two items marked
Lean-backed below are now discharged inputs; the remaining items describe the genuinely new
analytic layer.

\begin{enumerate}
\item \textbf{Normalized coefficient sequence (Lean-backed input).}  Package
$a_n=d_n/n!=2^{\binom n2}F(2^{-n})$ and identify its ordinary generating function with $G$.
This package is supplied by
...
\item \textbf{Phase locking (Lean-backed input).}  From
$\lambda_n2^{-\lambda_n}=2^{-n}$ prove
$\log_2\lambda_n=\lambda_n-n$, and use periodicity to rewrite
$\Psi(\log_2\lambda_n)$ as $\Psi(\lambda_n)$.
The exact fixed-point identity is \path{dyadicLambertPhase_fixedPoint} in
\path{FabiusLambertPhase.lean}; the periodic rewrite uses
\path{negativeLaplacePsi_periodic} in \path{PeriodicMean.lean}.
...
exposition's exact-counterpart audit.  Nothing in this part is claimed as a Lean theorem
unless it is explicitly identified as formal background; every candidate below still needs
a public theorem with the stated hypotheses and conclusion.
\end{boxedremark}
...
up-function.  Write \(c_n=\int_{-1}^{1}x^{2n}\Up(x)\dd x\).  The general dyadic evaluator
is proved correct, but these specializations have no named theorems:
\begin{candidate}
\[
 c_1=\frac19,\qquad
...
The proved estimate has the factor \(\exp(|Q-\tfrac12|)\).  Put
\[
...
The Small-Argument part of this volume preserves the removed raw Mellin-inversion
derivation, exponential Gamma--zeta decay estimates, numerical periodic profiles, proposed
...
the dyadic, \(q\)-binomial, inverse, integration, repeated-integration, and small-argument
parts of this same research-frontier volume.
