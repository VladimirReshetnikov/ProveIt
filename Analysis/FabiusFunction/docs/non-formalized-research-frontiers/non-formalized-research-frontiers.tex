...
The post-consolidation audit also records the newly formalized generic naturality and
conditional affine-recursion API for weighted uniform series and the named
geometric-\(q\) weights, series, probability law, recurrence, reflection, and carried-set
results.  Equality of topological support, atomlessness, CDF/density identities, transforms,
moments/cumulants, and fixed-point uniqueness remain in the frontier.
...
\nolinkurl{weightedSumDistribution_eq_geometricUniformDistribution_one_half} &
...
reflection, and carried-\([0,1]\) statements are now exact formal inputs.  Equality of
topological support with \([0,1]\), atomlessness, \(q\)-specific CDF or density equations,
transforms, moments/cumulants, and uniqueness of a measure fixed point are not.
\end{boxedremark}
...
approximants, together with the post-snapshot continuous-linear naturality and conditional
operator/scalar affine-recursion API for arbitrary weighted uniform laws and its named
geometric-\(q\) specialization.  Two reusable
...
arbitrary-family Toeplitz wording, generic Gaussian coefficient bounds, and the topological
support, atomlessness, CDF/density, transform, moment/cumulant, and uniqueness layers beyond
the formalized named geometric-\(q\) measure recurrence.  The inverse
...
zero to its complement.

The nineteen declarations are
...
\lean{ProbabilityRepresentation.geometricUniformDistribution_compl_Icc} in
...
identify the dyadic series and law with this specialization.

These are carried-set and fixed-point identities, not equality of topological support or
uniqueness.  Atomlessness, \(q\)-specific CDF and density identities, transforms, moments
and cumulants, and uniqueness of the affine measure fixed point remain outside the formal
boundary.
...
beta kernel, including at a coincident endpoint.  When \(s<x\),
...
\end{equation}
No order-zero operator law, reversed-endpoint fractional interpretation, semigroup theorem
for merely interval-integrable inputs or noncomplete targets, separately packaged general
power/constant evaluation, Caputo or other fractional derivative, complex-order theorem,
or Fabius-, Rvachev-, or inverse-function specialization is included in these modules.
\end{remark}
...
\lean{ProbabilityRepresentation.weightedSumDistribution_eq_weightedUniformDistribution},
the two half-base geometric bridges, head--tail split, coordinate reflection, and
identification of the CDF with $F$;
...
The compatibility head--tail, probability, scalar-law, and unit-interval surface;
\cref{integration:sec:weighted-naturality}.\\
\path{GeometricUniformLaw.lean} &
The nineteen-declaration named geometric weight, series, and law API: summability and mass,
continuity and measurability, affine splitting, reflection, and carried-\([0,1]\) results;
\cref{integration:sec:weighted-naturality}.\\
...
for positive orders and non-strict endpoint order; the raw and normalized values under a
strict endpoint inequality; and additive composition of positive orders for continuous
Banach-valued inputs on ordered compact intervals.  No corresponding semigroup for merely
interval-integrable inputs or noncomplete targets, separately packaged power/constant
specialization, fractional derivative, complex order, order-zero law, reversed-endpoint
fractional interpretation, or hierarchy-specific specialization;
...
Generic weighted-law naturality & Not recorded in the pinned companion snapshot &
...
the seven-name compatibility surface.\\
...
reflection, carried-\([0,1]\) laws, and both half-base bridges; equality of topological
support, atomlessness, CDF/density identities, transforms, moments/cumulants, and
...
formula does not yet have an exact public counterpart.
\begin{candidate}
