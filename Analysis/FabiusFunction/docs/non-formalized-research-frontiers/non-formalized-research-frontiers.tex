...
\hypersetup{
  ...
  pdfsubject={Six deduplicated thematic syntheses from eleven research notebooks awaiting Lean verification},
...
\large Six thematic syntheses awaiting Lean verification}
...
\date{Base research snapshot: 25 August 2026; formal-status audit updated 28 August 2026\\
\small current compiler-validated checkpoint \nolinkurl{e6f535e5d}}
...
\path{Analysis/FabiusFunction/docs/non-formalized-research-frontiers/}.  Eleven source
...
bounded Fabius function and the affine Rvachev bump.  The negative- and zero-parameter
density interpretation, the general-\(q\) Fourier route, and the further centered, shape,
moment, transform, derivative-hierarchy, endpoint-flatness, and fixed-point layers remain in
the frontier on the precise boundary recorded below.
...
\nolinkurl{ProbabilityRepresentation.lean} \\
Weighted survival and inverse layer cake &
...
\(C^\infty\) regularity.  What remains here is the density interpretation at \(q=0\) and at
negative \(q\), the general-\(q\) characteristic-function product, rapid decay and Fourier
inversion route, named centered packaging, a closed higher-derivative hierarchy and endpoint
flatness wrappers, shape theorems, transforms, moments/cumulants, and uniqueness of the
...
transform, moment/cumulant, and fixed-point-uniqueness layers beyond the formalized CDF and
positive-ratio smooth-density API.  The explicit density's interpretation at zero and
negative parameters is also not supplied by that API.  The inverse
...
\path{AnalyticSeriesFilter.lean}, together with the fractional family
...
\lean{weightedSumDistribution_Icc}, \lean{weightedSumDistribution_compl_Icc}, and the exact
topological-support theorem \lean{weightedSumDistribution_support_eq_Icc}.
...
At compiler-validated checkpoint \texttt{e6f535e5d}, two further modules promote the
...
\begin{itemize}
\item the general-\(q\) characteristic-function product, quantitative rapid decay, and the
...
\item transform formulas, Bernoulli cumulants, Bell moments, and related asymptotic or
      Gaussian-limit statements; and
\item uniqueness of the affine measure fixed point.
\end{itemize}
\end{boxedremark}
...
The main article's probability chapter records the scalar differentiable bridge between
integration against the Fabius law and an ordinary integral against $\Up$.  The formal
development now also contains the stronger Banach-valued partial and full layer-cake forms
recorded below.
...
The same subgraph engine proves a real- or complex-weighted, Banach-valued inverse-clock
identity on compact rectangles.  For the Fabius clocks its explicit-primitive and
pointwise-\(C^1\) forms are
...
The real weighted-area specialization is
\lean{intervalIntegral_mul_fabiusInv_eq}.  The module also exports
...
unit-interval $\Up$ area and with \(1/2\).  These are full-interval theorems.  They do not
package an arbitrary absolutely continuous primitive, a variable-upper-endpoint area
identity, higher or fractional inverse primitives, or complex Mellin continuation.
\end{remark}

\begin{remark}[Formalized positive-real fractional Volterra boundary]
\label{integration:rem:fractional-volterra-boundary}
At compiled full-facade checkpoint \texttt{e6f535e5d}, the definition
...
\end{equation}
The reusable companion \path{FractionalVolterraCalculus.lean} adds two further layers.
For every real order, positive scale $c$, and ordered endpoint $a\le x$,
\lean{fractionalVolterra_affine} gives positive affine covariance and
\lean{fractionalVolterra_comp_affine} gives its inverse-smul form.  This identity is
algebraic for the totalized Lean definition; only the positive-order regime has the usual
classical Riemann--Liouville interpretation.  For \(\alpha>0\), a continuous Banach-valued
function $g$, an interval-integrable $g'$, and the stated right-hand derivative on the
interior, \lean{fractionalVolterra_add_one_deriv} proves
\begin{equation}
 I_{a+}^{\alpha+1}g'(x)
 =I_{a+}^{\alpha}g(x)
  -\frac{(x-a)^\alpha}{\Gamma(\alpha+1)}g(a).
 \label{integration:eq:fractional-volterra-order-raising}
\end{equation}
The declaration \lean{fractionalVolterra_add_one_deriv_of_eq_zero} removes the boundary
term when $g(a)=0$.

Finally, \path{FabiusFractionalVolterra.lean} specializes that generic calculus.  For real
\(\alpha>0\) it proves
\begin{align}
 I_{0+}^{\alpha+1}\widetilde F(x)
 &=2^\alpha I_{0+}^{\alpha}\widetilde F(x/2), &&x\ge0,
 \label{integration:eq:fractional-volterra-fabius-signed-shift}\\
 I_{0+}^{\alpha+1}F(x)
 &=2^\alpha I_{0+}^{\alpha}F(x/2), &&0\le x\le1,
 \label{integration:eq:fractional-volterra-fabius-bounded-shift}\\
 I_{-1+}^{\alpha+1}\Up(x)
 &=2^\alpha I_{0+}^{\alpha}F((x+1)/2), &&x\ge-1.
 \label{integration:eq:fractional-volterra-up-shift}
\end{align}
...
\lean{fractionalVolterra_add_one_rvachevUp}.

No order-zero classical operator law, reversed-endpoint fractional interpretation,
semigroup theorem for merely interval-integrable inputs or noncomplete targets, separately
packaged general power/constant evaluation, Caputo or other fractional derivative,
complex-order theorem, iterated fractional shift, endpoint-moment or dyadic-lattice wrapper,
support-truncated hierarchy semigroup, full two-branch refinement, signed recurrence for
negative endpoints, Up exterior/refinement formula, probability/stop-loss representation,
or inverse-function fractional specialization is included in these modules.
\end{remark}

\begin{remark}[Formalized unconditional analytic-filter boundary]
\label{integration:rem:analytic-series-filter-boundary}
At the same compiled checkpoint, \path{AnalyticSeriesFilter.lean} turns the finite moment algebra of a row
\((\xi_i,w_i)\) into an exact pointwise series theorem.  If each weighted sampled series
\(
...
row over a commutative ring with continuous constant scalar multiplication (each fixed
scalar acts continuously), the finite row-moment factor in
degree \(p+1+r\) is
\begin{equation}
...
corresponding weight is nonzero.

These are Mathlib \lean{Summable}/\lean{tsum} identities and hence unconditional and
pointwise.  They do not cover conditionally-only convergent boundary series, prove uniform
convergence or quantitative bounds, determine residual signs or asymptotic acceleration
rates, instantiate the row with coefficients of an actual sinc product, or supply a
formal-\lean{PowerSeries}-evaluation bridge.
\end{remark}
...
\lean{integral_smul_setIntegral_subgraph}, the real- or complex-weighted Banach-valued
subgraph exchange over arbitrary s-finite measures; \cref{integration:sec:survival-fubini}.\\
...
Generic inverse-clock and Fabius explicit-primitive/pointwise-\(C^1\) layer cake, the full
weighted inverse-area identity, equality with the $\Up$ area, and the inverse mean \(1/2\);
\cref{integration:sec:survival-fubini}.\\
\path{FractionalVolterra.lean}, \path{FractionalVolterraSemigroup.lean},
\path{FractionalVolterraCalculus.lean} &
...
Banach-valued inputs on ordered compact intervals.  Positive affine covariance is formal for
arbitrary real order and positive scale, and positive-order integration by parts raises the
order of a right interior derivative with the exact left-boundary term.  No corresponding
semigroup for merely interval-integrable inputs or noncomplete targets, separately packaged
power/constant specialization, fractional derivative operator, complex order, order-zero
classical law, or reversed-endpoint fractional interpretation;
\cref{integration:rem:fractional-volterra-boundary}.\\
\path{FabiusFractionalVolterra.lean} &
The real-positive one-step fractional shift for the signed extension on $x\ge0$, the
bounded function on \([0,1]\), and the \(-1\)-based Up-to-Fabius affine bridge on
$x\ge-1$.  Iterated shifts, endpoint moments, dyadic-lattice wrappers, negative-endpoint
signed branches, Up exterior/refinement laws, complex orders, fractional derivatives, and
inverse/quantile formulas remain outside this module;
\cref{integration:rem:fractional-volterra-boundary}.\\
\path{AnalyticSeriesFilter.lean} &
Unconditional pointwise diagonalization of finite coefficient filters under weighted
sample-series summability, exact reproduced heads and explicit tails, and the Gaussian
residual tail for geometric and geometric-Lagrange rows, including shifted scales.  No
conditional boundary summation, uniform estimate, sign/rate theorem, sinc-product
instantiation, or formal-power-series evaluation bridge;
\cref{integration:rem:analytic-series-filter-boundary}.\\
\path{ProbabilityLaplaceMoments.lean} &
Almost-sure support, restriction to $[0,1]$, survival function, the partial/full Rvachev
survival layer cake, the differentiable compact-support Fubini identity, raw and endpoint
moment identifications, zero-tilt normalization, and positivity;
...
regularity.  Series, law, CDF, and density half-base bridges are formal.  The zero/negative
parameter density interpretation, general-\(q\) Fourier route, centered package, closed
higher-derivative and endpoint-flatness wrappers, shape, transforms, moments/cumulants, and
fixed-point uniqueness remain outside the formal boundary.\\
