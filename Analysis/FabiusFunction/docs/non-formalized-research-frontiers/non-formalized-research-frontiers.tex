...
\paragraph{Current formal status.}
The exact rational-sequence specialization is
\lean{discreteLimitWeight_qRichardson_exact} in
\path{FabiusDiscreteLimitToeplitz.lean}.  It uses coefficients indexed by
$0\le r<n$, with its coefficient $a_r$ multiplying the same mode
$2^{-(r+1)p}$.  A named scalar-extension form for complex model sequences is
not asserted by that theorem.
