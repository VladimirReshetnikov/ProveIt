...
subset.  Among the obligations retained here are inverse curvature and H\"older claims,
...
ordinary analysis}.  This distinction matters most for the second-derivative curvature
formula of the inverse and for the transfer of the highest-jet coefficient from mass
coefficients to logarithmic coefficients.
\end{boxedremark}
...
\nolinkurl{fabiusInv}; Lean uses \nolinkurl{Set.IccExtend} rather than spelling
out \eqref{integration:eq:clamp}.
...
\subsubsection{Formal interior calculus and an ordinary-analysis curvature corollary}
...
Because $F$ is convex on $(0,1/2)$ and concave on $(1/2,1)$, inversion reverses those
curvature signs.

\begin{corollary}[Curvature of the inverse]
\label{integration:cor:inverse-curvature}
The inverse $I$ is concave on $(0,1/2)$ and convex on $(1/2,1)$.  Where second derivatives
are evaluated,
\begin{equation}
...
\path{fabiusInv_contDiffOn_Ioo}.  The second-derivative curvature formula in
\cref{integration:cor:inverse-curvature} remains an ordinary-analysis corollary rather than a separate
declaration.
\end{remark}
...
\lean{deriv\_fabiusInv\_pos}, \lean{fabiusInv\_contDiffOn\_Ioo}, the transported root-free bounds,
...
bounds \cite{repeated:companion,repeated:proveit}.  It also records the standard finite convolution obtained by
...
% Original SHA-256: 81a0a911ac7ab28e12c6b87ccaf76ffccaf32e48f873abff18fb7a2d01bcf3e5
\clearpage
...
\begin{boxedremark}
The product \eqref{dyadicq:eq:G-product}, the recurrence
\eqref{dyadicq:eq:moment-recurrence}, the dyadic identity
\eqref{dyadicq:eq:inverse-dyadic-moment}, the finite $q$-binomial formulae, and the
...
\begin{tabularx}{0.96\textwidth}{>{\raggedright\arraybackslash}p{0.39\textwidth}X}
\toprule
\normalfont File & Mathematical role\\
\midrule
\nolinkurl{HalfQBinomial.lean}
...
\nolinkurl{FabiusQBinomialTaylor.lean}
...
certify that every displayed consequence has been formalized.  In particular, inverse
curvature and H\"older consequences, inverse endpoint asymptotics, the ordered-composition coefficient formulas and the logarithmic highest-derivative law remain research obligations.
\end{boxedremark}
...
and positivity are now marked \Formalized; the second and higher derivative
formulas remain \Derived.
...
\begin{proposition}[Higher inverse derivatives; \Derived]
\label{inverse:prop:inverse-higher-derivatives}
For $0<y<1$,
\begin{equation}
 \InvF''(y)
   =-\frac{F''(\InvF(y))}
...
\end{equation}
More generally, every derivative of $\InvF$ is a rational polynomial in
the derivatives of $F$ evaluated at $\InvF(y)$, with a power of
$F'(\InvF(y))$ in the denominator.
\end{proposition}
...
\[
 ...
 F'(\InvF(y))\InvF''(y)=0.
\]
Substitution of \eqref{inverse:eq:Gprime} yields \eqref{inverse:eq:Gsecond}.  Repeated
differentiation gives the higher formulas, equivalently the
Fa\`a di Bruno inverse-derivative polynomials.
\end{proof}
...
\begin{proposition}[Concavity and convexity of the inverse; \Derived]
\label{inverse:prop:inverse-shape}
The function $\InvF$ is strictly concave on $(0,1/2)$ and strictly convex
on $(1/2,1)$.  Its tangent slope decreases from $+\infty$ to $1/2$ on the
left half, then increases from $1/2$ to $+\infty$ on the right half.
\end{proposition}

\begin{proof}
For $0<y<1/2$, strict increase gives $0<\InvF(y)<1/2$, where
$F''>0$.  Equation \eqref{inverse:eq:Gsecond} and $F'>0$ imply
$\InvF''(y)<0$.  Similarly, $y>1/2$ gives $\InvF(y)>1/2$, where
$F''<0$, and therefore $\InvF''(y)>0$.

The endpoint divergence of the derivative is understood as a one-sided
slope statement: by \cref{inverse:thm:inverse-quotient}, the secant slopes from
the origin diverge, and concavity makes the right derivative at the
origin at least as large as every such limiting secant slope.  Reflection
gives the right endpoint.  The midpoint slope is
\cref{inverse:cor:midpoint-inverse}.
\end{proof}
...
inversion, transported flatness, effective steepness, exact global
differentiability and $C^\infty$ loci, and endpoint quotient divergence. & \Formalized\\
...
\begin{itemize}
\item the formula for $\InvF''$ obtained from the formalized reciprocal
first derivative;
\item strict concavity on $(0,1/2)$ and strict convexity on $(1/2,1)$;
