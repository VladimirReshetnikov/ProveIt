...
0+7, 4+16, and 0+9), its subsequent zero-definition/four-theorem real
Bernoulli branch-gap companion crosswalking only
\nolinkurl{eq:pair-Bernoulli-general}.  Those four Lambert modules total four
definitions and 36 theorems, 40 declarations; the new complex theorem makes
the exact radius and boundary/exterior divergence formal while the complete
complex value identity and higher Puiseux claims remain open.  The publication
...
2+24.  The origin-side source census is 903 facade-reachable modules and
...
census of exactly 903 modules and 11,448 unique public declarations, with no
missing module headers or declaration documentation.  The retained PDF
predates the Lambert--Bernoulli source update and remains an explicitly
...
The real Bernoulli branch-gap series and its exact complex convergence radius
(zero definitions and four theorems), crosswalking exactly
\nolinkurl{eq:pair-Bernoulli-general} and no other labelled formula in its
surrounding Guide corollary.  The complex Bernoulli exponential generating
series is summable if and only if \(\lVert z\rVert<2\pi\), so it diverges on
the boundary and throughout the exterior; on the real line it has its actual
all-index sum \(z/(e^z-1)\) for \(z\ne0\).  On
...
The convergence theorem includes \(z=0\), but the quotient theorem excludes
the removable value because Lean's totalized quotient is zero there.  The
radius and boundary divergence are Exact, while the full complex value
identity is Partial and all higher or convergent Puiseux claims remain open. \\
...
\path{LambertWBranchGapBernoulli.lean} leaf brings the origin-side live source
census to 903/11,447 and crosswalks only
\nolinkurl{eq:pair-Bernoulli-general}.  Its
...
on and outside the radius.  The real value theorem still assumes \(z\ne0\),
and the complete complex quotient evaluation, removable-origin theorem,
remainders, and higher Puiseux claims remain open.  Retaining the unconditional
public \lean{complexQPochhammerInf_eq_qPochhammerInfIn} bridge yields the final
merged live union 903/11,448 with both documentation-gap counts zero.  The
retained PDF remains explicitly historical until rebuild and does not claim
parity with the current TeX.  Exact source and PDF receipts are kept externally
...
Partial, 18 None, and 8 interface rows.  The evaluated q-Lucas theorem remains
