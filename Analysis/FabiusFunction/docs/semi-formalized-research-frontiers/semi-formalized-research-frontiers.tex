...
\begin{align}
 ...
 \widehat\lambda(t)
   &=\left(\prod_{k=0}^{n-1}\widehat{d_*\rho}(q^kt)\right)
      \widehat\lambda(q^nt) \qquad(n\in\NaturalNumbers).
...
Consequently
\begin{equation}
 \int_\RealNumbers\mathbf1_{[a,b]}^*(x)\Differential x=\max(b-a,0),
 \label{integration:eq:half-indicator-total-integral}
\end{equation}
and, for $c\le d$,
\begin{equation}
 \int_c^d\mathbf1_{[a,b]}^*(x)\Differential x
...
$Q_d=\mathcal D_{\RvachevUp}P_d$, the first two prove exact natural degree $d$ and the explicit unchanged
...
$C_N=\mathcal D_{\RvachevUp}S_N$ with the finite sum of deconvolved modes, prove that $C_N$ has the
...
\begin{equation}
 \widehat\RvachevUp(\xi)=\prod_{j=0}^{\infty}
 \SincRad\!\left(\frac{\pi\xi}{2^j}\right),
 \qquad
 \widehat f(\xi)=\int_\RealNumbers f(t)\EulerE^{-2\pi\ImaginaryUnit\xi t}\Differential t.
...
$\widehat\mu(\xi)=\sum_kp_k\EulerE^{-2\pi\ImaginaryUnit(a+hk)\xi}$.  Then, for every $m\in\IntegerNumbers$,
\begin{equation}
 \frac{p_m}{h}
 =\int_{-1/(2h)}^{1/(2h)}
   \widehat\mu(\xi)\EulerE^{2\pi\ImaginaryUnit(a+hm)\xi}\Differential\xi.          \label{tmrates:eq:lattice-inversion}
\end{equation}
\end{lemma}
...
\begin{equation}
 \widehat{\mu_n}(\xi)=\widehat\RvachevUp(\xi)R_n(\xi),
 \qquad
 R_n(\xi)=\prod_{\ell>n}
...
\end{equation}
This exponential growth is harmless because $\widehat\RvachevUp$ decays faster than every power.
...
$\int_{\RealNumbers}|\xi|^6|\widehat\RvachevUp(\xi)|\Differential\xi<\infty$, its integrated remainder is
...
\begin{equation*}
 \int_{\RealNumbers}(\pi\xi)^2\widehat\RvachevUp(\xi)\EulerE^{2\pi\ImaginaryUnit\lambda\xi}\Differential\xi
 =-\frac14\RvachevUp''(\lambda),
 \qquad
 \int_{\RealNumbers}(\pi\xi)^4\widehat\RvachevUp(\xi)\EulerE^{2\pi\ImaginaryUnit\lambda\xi}\Differential\xi
...
\[
\begin{aligned}
  \widehat\Psi(1)&=(7.55864754098-7.47010355898\,\ImaginaryUnit)\,10^{-7},
    &|\widehat\Psi(1)|&=1.06271162519\,10^{-6},\\
  \widehat\Psi(2)&=(3.24812347723+5.85175898712\,\ImaginaryUnit)\,10^{-13},
    &|\widehat\Psi(2)|&=6.69278636792\,10^{-13},\\
  \widehat\Psi(3)&=(-2.81236523199-2.58674397916\,\ImaginaryUnit)\,10^{-19},
...
Because $\Psi^{(m)}(u)=\sum_{k\ne0}(2\pi\ImaginaryUnit k)^m\Psihat(k)\EulerE^{2\pi\ImaginaryUnit ku}$,
...
\[
  A_j(u)=A_j^{(0)}
   +2\operatorname{Re}\Bigl(\sum_{k\ge1}c_j(k)\Psihat(k)\EulerE^{2\pi\ImaginaryUnit ku}\Bigr)
   +R_j(u),
\]
...
\[
\begin{aligned}
  \texttt{mp.dps}=30:&\quad \Psihat(1)=7.55865005\times10^{-7}-7.47011001\times10^{-7}\,\ImaginaryUnit,\\
  \texttt{mp.dps}=50:&\quad \Psihat(1)=7.55864674\times10^{-7}-7.47009636\times10^{-7}\,\ImaginaryUnit,\\
  \texttt{mp.dps}=80:&\quad \Psihat(1)=7.55865060\times10^{-7}-7.47008702\times10^{-7}\,\ImaginaryUnit,
\end{aligned}
\]
...
\[
 |\widehat{\RvachevUp}(\xi)|\le C_N(1+|\xi|)^{-N}\qquad(\xi\in\RealNumbers).
\]
\end{candidate}
