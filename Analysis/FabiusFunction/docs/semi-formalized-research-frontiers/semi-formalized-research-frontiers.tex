...
The zero-definition, two-theorem finite companion: exact residue-class
dissection and its \(u\le r\) remainder form over every commutative ring, with
no nome, analytic, division, or nonvanishing hypothesis.\\
\path{QPochhammerInfinite.lean} &
The one-definition, twenty-seven-theorem general infinite interface:
strict-contraction complete-normed-ring product, shift, factor removal,
dissection, and raw zeros; inverse-power field zeros under \(q\ne0\);
fixed-\(q\) local uniformity and continuity; and complex entire and explicit
nonzero derivative formulas.  It asserts no joint-\(q\) holomorphy or global
growth/order/type/asymptotics.\\
\path{QPochhammerEntire.lean} &
The zero-definition, four-theorem analytic companion: for each fixed complex
\(\lVert q\rVert<1\), locally uniform convergence on the whole \(a\)-plane,
entireness in \(a\), the raw factor-zero locus including \(q=0\), and analytic
order one at every zero.  It gives no division-based zero parametrization,
joint-\(q\) holomorphy, outer-product local uniformity, or global
...
with no converse or nonvanishing theorem.  There is no Wigner \(3j\) identification,
factorial formula, or nonnegativity-by-Wigner result in this predecessor module; the
closed-form leaf immediately below supplies those square-level conclusions.\\
\path{LegendreGauntClosedForm.lean} &
Exactly two public definitions,
\lean{legendreGauntAdmissible} and
\lean{legendreWignerThreeJZeroSqRat}, and twenty-five public theorems:
...
equivalently
\(i=b+c\), \(j=a+c\), \(k=a+b\).  With \(s=a+b+c\), the total rational square datum is
\[
 W_{ijk}^{2}=
...
\]
with equivalent pairwise-coordinate and half-sum factorial formulas.  It is positive
exactly on the admissible support and zero otherwise; the rational and real Gaunt
coefficients have the same sharp support and are nonnegative.  The coefficient of
\(P_k\) in \(P_iP_j\) is \((2k+1)W_{ijk}^{2}\).  The definition is deliberately the
total square of the integer-index zero-row datum: it makes no signed-symbol or
Condon--Shortley phase choice and proves no half-integer, nonzero-magnetic-index,
general \(3j/6j/9j\), Wigner orthogonality, or recoupling theorem.\\
...
supplies the finite Gaunt entry formulas, and the closed-form leaves adjacent to the Gaunt
rows identify their zero-row Wigner-square form.  Signed or general Wigner theory,
...
Gram-entry expansion.  By themselves they prove no Wigner/$3j$ identification or
factorial formula and no nonnegativity by that route; the two closed-form leaves adjacent
to this row close precisely that finite square-level gap.  Infinite
Legendre-series/integral interchange, Christoffel reconstruction, and named values for the
displayed \(G_3\) entries remain outside this module.\\
...
Wigner symbols, Christoffel reconstruction, roots or quadrature, infinite Jacobi
products/continued fractions, and asymptotics remain open.\\
...
$\mathcal D_{\RvachevUp}P\bigl(x-(\theta+k)/M\bigr)$ against
$\RvachevUp\bigl((\theta+k)/M\bigr)$ and recovers $P(x)$.  Its exact range is
$\deg P\leq v_2(M)$; it does not prove the separate degree-$(N+1)$
