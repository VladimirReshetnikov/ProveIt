...
\lean{iteratedDeriv_rvachevUp_dyadic_below}.  The live source census is
exactly 901 facade-reachable modules and 11,419 unique public declarations,
with no missing module headers or declaration documentation.  The final-source
...
\nolinkurl{QLucas.lean}; zero definitions and eight theorems \\
...
\path{QLucas.lean} is 0+8: \lean{two_mul_choose_two},
...
\path{DyadicDerivativeFiltration.lean} surface.  The live source census is
therefore 901/11,419 with both documentation-gap counts zero.  These additions
...
\path{QLucas.lean} &
The exhaustive zero-definition, eight-theorem natural quadratic, block-coefficient, phase, and q-Lucas interface.\\
...
\path{QLucas.lean} & Exactly eight public theorems and no definitions: \lean{two_mul_choose_two}, \lean{add_mul_add_sub_one}, \lean{choose_two_add}, \lean{coeff_finiteQPochhammerIn_neg_X}, \lean{finiteQPochhammerIn_neg_X_block}, \lean{coeff_block_pow_mul}, \lean{pow_choose_two_add_mul_eq}, and \lean{gaussianBinomial_q_lucas}.  The final theorem assumes only positive primitive-root order and the residue bounds \(b,s<d\), not \(r\le a\) or \(s\le b\). \\
