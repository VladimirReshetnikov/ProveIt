...
theta, derivative, and Gaussian-polynomial closures.  The live facade
contains exactly 657 source modules and 8,756 public declarations, with no
...
\lean{analyticOrderAt_qPochhammerInfIn_of_eq_zero} gives analytic order one
at every zero, including the nome \(q=0\).  The same conclusion for the
historical wrapper name is
\lean{analyticOrderAt_complexQPochhammerInf_of_eq_zero} in the distinct
five-theorem \path{QPochhammerEntire.lean} compatibility leaf.  Conversely
that leaf contains no generic ring-valued concatenation or dissection
theorem.  Neither API proves joint holomorphy in \(q\) or an outside-disk
reciprocal theory.
...
six-declaration overlay is part of the live 657-module, 8,756-declaration
