...
\hypersetup{
  ...
  pdfauthor={Boundary layer between prose and proof in the ProveIt Fabius development},
...
locally uniform theorem for the full phase-bearing prefix sequence, joint normality in
\((q,z)\), uniformity up to \(\lVert q\rVert=1\), general-\(q\) rapid decay
...
records.  Where a printed SHA-256 value cannot be reproduced from a reachable
...
\begin{boxedremark}
\textbf{Current source checkpoint.}
The current source retains the 31 August 2026 Legendre Gaunt--Wigner-square
closed-form overlay and additionally contains the 1 September 2026
...
0+7, 4+16, and 0+9), the three-definition/seven-theorem geometric Richardson
generating-function module, the three-theorem second-moment strengthening that
enlarges the Gaussian-binomial cumulant module from 2+21 to 2+24, the
zero-definition/five-theorem Bernoulli branch-gap companion, and the local
five-theorem effective fixed-column q-binomial closure.  The Bernoulli leaf's earlier four
theorems prove the exact complex convergence radius and boundary divergence,
the nonzero-real sum, and the paired real branch formulas; its added fifth gives the
complex removable-origin sum given by the inverse of
\nolinkurl{complexExpm1Div} at \(z\), exactly when
\(\lVert z\rVert<2\pi\).  It therefore crosswalks
...
definitions and 37 theorems, 41 declarations.  Their completed local
checkpoint is 903/11,448; the five-theorem fixed-column extension gives
903/11,453.  Independently, the one-definition/eight-theorem geometric-uniform
moment-polynomial branch gives 904/11,457 from the Lambert checkpoint, and the
first union of those parallel extensions is the historical 904/11,462 receipt.
\end{boxedremark}

\begin{boxedremark}
\textbf{Current merged census.}
The current source also includes the exact inverse-recursion
...
one-theorem regular central q-binomial sum leaf; the deduplicated zero-definition/
eight-theorem effective fixed-column Gaussian-rate leaf, with the shared
generalized product bound and arbitrary-shift convergence retained upstream in
the one-definition/twenty-seven-theorem
\path{QBinomialTheoremInfinite.lean} surface; and the one-definition/
...
\path{GeometricResidualMoments.lean} module.
\end{boxedremark}

\begin{boxedremark}
\textbf{Artifact status: later source-only leaves and live census.}
The zero-definition/one-
...
\path{RvachevLegendreCentralSum.lean}; and the one-definition/three-theorem
...
\path{GeometricUniformMomentRatFunc.lean} leaf.
\end{boxedremark}

\begin{boxedremark}
\textbf{Current merged census (later closures).}
The subsequent two-theorem extension of the existing
\path{ProbabilityLaplaceMoments.lean} module and the exhaustive
one-definition/one-theorem
\path{RvachevLegendreBiorthogonality.lean} leaf; and, after the later
merged-main additions, the newly public complex-product differentiability
theorem and the exhaustive one-definition/five-theorem
\path{GeometricUniformMomentReciprocity.lean} leaf.  That reciprocity leaf
gave the historical 931/11,685 census checkpoint.  The
subsequently merged upstream \path{DyadicBoundaryIdentity.lean} and
...
theorem \path{TransseriesFlat.lean} leaf and three new integer-Laurent
theorems in \path{TransseriesDifferentialBlock.lean} give the historical
...
\path{WrightOmega.lean} (1+12), totaling 21 definitions and 115 theorems.
...
\path{RvachevPochhammerFactorization.lean} retains one additional public
bridge.  The modified \path{LaplaceMomentBoundsSharp.lean} is excluded from
the new-module count.  Thus that historical source census is 960 facade-reachable
modules and 11,966 public declarations, with zero missing module headers and
zero missing declaration comments.

The retained source-only \path{StirlingCompleteHomogeneous.lean} leaf adds one
module and eight public theorems, with no new definitions, to the existing
960/11,966 checkpoint; the historical local source inventory was therefore 961/11,974.
The coefficient-calculus crosswalk uses
\lean{Fabius.stirlingSecond_add_eq_completeHomogeneousEvalOn} and
\lean{Fabius.stirlingSecond_add_eq_sum_finsuppAntidiag}, alongside its
generating-series, symmetric-polynomial, and factorial-normalization
interfaces.  The focused leaf compiled warning-free; this source inventory
does not claim an aggregate build or a rebuilt companion PDF.
\end{boxedremark}

\begin{boxedremark}
\textbf{Merged branch inventory (continued).}
...
is Neighboring.  The current union has 1004 modules and 12,500 public
declarations, with zero documentation gaps.  The
Legendre-biorthogonality 925/11,619 census remains an explicit historical
checkpoint.
\end{boxedremark}

\begin{boxedremark}
\textbf{Artifact status: exactness, chronology, and integration scope.}
The sharp
...
normalization subclaim is Exact for real \(\lvert q\rvert<1\), but the full MGF
row remains Partial and no analytic value at a genuine unit-root pole is
asserted.  The incoming bridge branch recorded the
...
925/11,619 union.  The subsequent merged-main additions give the
pre-reciprocity 930/11,678 checkpoint; public complex-product
differentiability gives 930/11,679, and the reciprocity leaf gives the
historical 931/11,685 union.  The subsequently merged upstream
\path{DyadicBoundaryIdentity.lean} and
\path{FinitePrefixThueMorseCollapse.lean} modules add two modules and ten
public declarations, giving the historical dyadic/finite-prefix 933/11,695
union.  The incoming union adds one module and fourteen public declarations:
the new zero-definition/six-theorem \path{ProuhetBaseTwoBridge.lean} module,
one theorem added to \path{DyadicBoundaryIdentity.lean}, and seven theorems
added to \path{ThueMorseNewmanSelfSimilarity.lean}.  This gives the historical
...
historical.
Exact source and PDF receipts remain external in the repository documentation.
No \nolinkurl{SHA256SUMS*} ledger participates in validation.
\end{boxedremark}
...
\nolinkurl{iteratedDeriv_rvachevUp_eq_extendedFabius},
\nolinkurl{iteratedDeriv_rvachevUp_dyadic_below} &
\nolinkurl{DyadicDerivativeFiltration.lean} (zero definitions and six theorems:
support, above/critical/depth detection, and the all-real and dyadic
below-depth formulas) \\
...
generic-name analytic-order theorem belongs to \nolinkurl{QPochhammerInfinite.lean} \\
...
\nolinkurl{QBinomialTheoremInfinite.lean}; one definition and twenty-seven
...
\nolinkurl{GaussianBinomialFixedColumnRate.lean}; zero definitions and nine
theorems, inventoried below. The normed-field product bound and
arbitrary-shift convergence live upstream in the 1+27 q-binomial module;
...
\nolinkurl{twelve_mul_varianceNumerator_gaussianBinomial_eval_one} &
Three-theorem delta in the 2+24
\nolinkurl{GaussianBinomialCumulants.lean} surface; the first uses a
characteristic-zero field and \(k\le n\), while the latter two are
division-free and total in \(n,k\) over every commutative semiring \\
...
exterior; precisely on that disk its sum is the inverse of
\nolinkurl{complexExpm1Div} at \(z\), which is \(1\) at zero and rewrites to
...
Formal arbitrary-power recurrence (0+3).  From \(FG'=\beta F'G\), Euler-
derivative coefficient extraction gives
\(na_0c_n=\sum_{j=1}^n((\beta+1)j-n)a_jc_{n-j}\) over every commutative ring.
The formal falling-factorial substitution supplies the unit-constant power
case over a rational algebra, making the power clause of
\nolinkurl{alg:merged-exp-log-power} Exact.  No analytic branch is asserted;
the 207-row coefficient-calculus concordance is 65 Lean, 35 Partial, and
107 None. &
\nolinkurl{Fabius.coeff_recurrence_of_mul_derivative_eq},
\nolinkurl{Fabius.mul_derivative_fallingSeries_subst_sub_one},
\nolinkurl{Fabius.coeff_fallingSeries_subst_sub_one_recurrence} &
\nolinkurl{UnitSeriesPowerRecurrence.lean} \\
Ordinary composition in multiplicity form (one upstream theorem plus 1+3).
A \(\lean{Fin}\,n\)-profile records part count and weighted sum, with
\nolinkurl{Nat.multinomial} counting its orderings.  The formal identity
includes \(n=0\) through the unique empty profile and has an empty \(k=0\)
term for \(n>0\), so \nolinkurl{thm:ordinary-composition} is Exact.  It does
not prove the separate labelled-set-partition interpretation. &
...
\nolinkurl{Fabius.coeff_subst_eq_sum_multinomial} &
\nolinkurl{OrdinaryBellComposition.lean},
\nolinkurl{OrdinaryBellMultinomial.lean} \\
Formal rescaling (0+4).  Coefficient comparison proves
\((f(cX))'=cf'(cX)\) over a commutative semiring; the exponential cases use a
commutative rational algebra.  Three helpers moved from the N\"orlund module
without changing the global declaration total. &
...
\nolinkurl{ExponentialRescaling.lean} \\
Abel polynomials (2+9).  The closed polynomial formula is division-free; over
a commutative rational algebra, Lagrange--B\"urmann proves the exponential
generating function for every solution of \(T=X\exp(-aT)\), including its
constant term, and multiplication proves the all-index binomial law.  Thus
\nolinkurl{prop:merged-abel} is Exact. &
\nolinkurl{Fabius.abelPolynomial},
\nolinkurl{Fabius.abelPolynomial_zero},
\nolinkurl{Fabius.abelPolynomial_succ},
\nolinkurl{Fabius.abelPolynomial_succ_eval},
\nolinkurl{Fabius.abelSeries},
\nolinkurl{Fabius.abelSeries_eq},
\nolinkurl{Fabius.hasSubst_abelSeries},
...
\nolinkurl{AbelPolynomialSeries.lean} \\
Formal Bernoulli-kernel logarithm (0+5).  The constructed reciprocal and the
logarithm product law give
\(\log(X/(\exp X-1))=-X/2-\sum_{n\ge2}\beta_nX^n/(n\,n!)\) over
\(\RationalNumbers\).  Degree one is separated because the Bernoulli
conventions differ; no analytic logarithm is asserted. &
\nolinkurl{Fabius.bernoulliPowerSeries_mul_massSeries_expm1Div},
\nolinkurl{Fabius.logOf_bernoulliPowerSeries},
\nolinkurl{Fabius.coeff_logOf_bernoulliPowerSeries},
\nolinkurl{Fabius.coeff_one_logOf_bernoulliPowerSeries},
\nolinkurl{Fabius.coeff_logOf_bernoulliPowerSeries_of_two_le} &
\nolinkurl{BernoulliFormalLog.lean} \\
Raney numbers (4+12).  Lagrange--B\"urmann gives both the denominator-cleared
and divided coefficient formulas, including \(n=0\), for the canonically
constructed \(T_p\).  No uniqueness theorem transports them to an arbitrary
solution of \(T=1+XT^p\), so the printed row remains Partial. &
...
\nolinkurl{Fabius.coeff_raneyT} &
\nolinkurl{RaneyNumbers.lean} \\
Kirkman--Cayley and associahedral arithmetic (4+23).  Binomial absorption
proves the determinant, exact quotient, divisibility, support, and boundary
values; reindexing and existing Narayana identities prove the face- and
\(h\)-array formulas.  The arrays are definitions, with no dissection type,
face lattice, bijection, or face-to-\(h\) transform, so all three geometric
rows remain Partial.  The current 204-row concordance is 62 Lean, 36 Partial,
and 106 None. &
\nolinkurl{Fabius.dissectionNumber},
\nolinkurl{Fabius.dissectionNumber_mul},
\nolinkurl{Fabius.succ_dvd_choose_mul_choose},
\nolinkurl{Fabius.dissectionNumber_eq_div},
\nolinkurl{Fabius.dissectionNumber_nonneg},
...
\nolinkurl{Fabius.sum_associahedronH},
\nolinkurl{Fabius.associahedronHPoly_one} &
\nolinkurl{AssociahedronFaceNumbers.lean} \\
Weighted binomial translation and inversion (+11).
...
\nolinkurl{Appell.binomialConv_translate_neg_translate} &
\nolinkurl{AppellSequence.lean} \\
Formal unit-series Bell calculus (0+16).  The labels
\nolinkurl{p0:lem:bell-conversion}, \nolinkurl{p0:lem:power-log}, and
\nolinkurl{p0:cor:exp-log-jets} are Exact as coefficient algebra over the
indicated rational algebras/fields.  No analytic convergence or logarithm
branch is asserted. &
\nolinkurl{Fabius.ordPartialBell_eq_factorialRatio_partialBell},
\nolinkurl{Fabius.factorial_mul_ordPartialBell_eq_factorial_mul_partialBell},
\nolinkurl{Fabius.coeff_fallingSeries_subst_eq_sum_ordPartialBell},
\nolinkurl{Fabius.coeff_fallingSeries_subst_eq_sum_ordPartialBell_of_pos},
\nolinkurl{Fabius.coeff_fallingSeries_subst_eq_sum_partialBell},
\nolinkurl{Fabius.coeff_negBinomSeries_subst_eq_sum_ordPartialBell},
\nolinkurl{Fabius.coeff_negBinomSeries_subst_eq_sum_ordPartialBell_of_pos},
\nolinkurl{Fabius.coeff_logOf_eq_sum_ordPartialBell},
\nolinkurl{Fabius.egfA_factorialDenormalize_coeff_eq},
\nolinkurl{Fabius.bellWeightSeries_factorialDenormalize_coeff_eq},
\nolinkurl{Fabius.coeff_logOf_eq_sum_partialBell},
\nolinkurl{Fabius.coeff_exp_subst_eq_completeBell},
\nolinkurl{Fabius.coeff_exp_subst_eq_partitionExpSum},
\nolinkurl{Fabius.coeff_exp_subst_eq_sum_weightedPartitions},
\nolinkurl{Fabius.coeff_exp_subst_eq_sum_div_weightedPartitions},
\nolinkurl{Fabius.coeff_exp_subst_recurrence} &
\nolinkurl{UnitSeriesBellCoefficients.lean} \\
Dickson/Neumann well-basedness (0+7 written, two generated).
\nolinkurl{q0:lem:dickson} remains Exact.  \nolinkurl{q0:lem:neumann} is Exact
for the manuscript's totally ordered commutative monomial group after its
reverse-well-order/strict-growth convention is represented by
\nolinkurl{Set.IsPWO} on \nolinkurl{OrderDual}; the two named wrappers state
both clauses.  They work more generally for a partially ordered cancel monoid.
This is the order-theoretic mechanism, not a Hahn-series construction. &
\nolinkurl{Fabius.dickson_isPWO},
\nolinkurl{Fabius.dickson_antichain_finite},
\nolinkurl{Fabius.dickson_isPWO_pi},
\nolinkurl{Fabius.neumann_isPWO},
\nolinkurl{Fabius.neumann_finite_factorizations},
\nolinkurl{Fabius.neumann_isPWO_orderDual},
\nolinkurl{Fabius.neumann_finite_factorizations_orderDual}; generated
\nolinkurl{Fabius.neumann_add_isPWO},
\nolinkurl{Fabius.neumann_finite_decompositions} &
\nolinkurl{TransseriesWellBased.lean} \\
Elementary height comparisons (0+3).  The two printed real
\nolinkurl{atTop} comparisons of \nolinkurl{q0:prop:height} are Exact; no
recursive height/depth order on arbitrary nested transmonomials is defined. &
\nolinkurl{Fabius.isLittleO_log_pow_rpow},
\nolinkurl{Fabius.isLittleO_log_pow_id},
\nolinkurl{Fabius.isLittleO_pow_mul_log_pow_exp} &
\nolinkurl{TransseriesHeight.lean} \\
Sequence-indexed scale and Poincar\'e coefficients (one structure, two
definitions, six theorems).  The sequence forms of
...
Uniqueness requires \nolinkurl{[l.NeBot]}; the first-omitted-term remainder
convention carries no convergence, flat-remainder recovery, unordered-set, or
maximal-scale claim. &
\nolinkurl{Fabius.IsAsymptoticScale},
\nolinkurl{Fabius.poincarePartialSum},
\nolinkurl{Fabius.poincarePartialSum_zero},
\nolinkurl{Fabius.poincarePartialSum_succ},
\nolinkurl{Fabius.IsPoincareExpansion},
\nolinkurl{Fabius.IsPoincareExpansion.isLittleO_succ_remainder},
\nolinkurl{Fabius.IsPoincareExpansion.tendsto_coeff},
\nolinkurl{Fabius.IsPoincareExpansion.tendsto_coeff_div},
\nolinkurl{Fabius.IsPoincareExpansion.coeff_unique} &
\nolinkurl{TransseriesScale.lean} \\
Real power--logarithmic dominance (1+7).  The analytic zero/one/
\nolinkurl{atTop} trichotomy and integer-generator rule of
\nolinkurl{plt:lem:mot-dominance} are Exact; the full unordered set and maximal
package are not asserted. &
\nolinkurl{Fabius.plMonomial},
\nolinkurl{Fabius.tendsto_plMonomial_atTop_zero},
\nolinkurl{Fabius.plMonomial_div_eventuallyEq},
\nolinkurl{Fabius.tendsto_plMonomial_div_atTop_zero},
\nolinkurl{Fabius.tendsto_plMonomial_div_atTop_one},
\nolinkurl{Fabius.plMonomial_pos},
\nolinkurl{Fabius.tendsto_plMonomial_div_atTop},
\nolinkurl{Fabius.plMonomial_generators_dominance} &
\nolinkurl{TransseriesScaleDominance.lean} \\
Power--logarithmic scale sequences (0+4).  Strictly decreasing exponent
sequences and the inverse-power and fixed-power/logarithmic ladders are exact;
the unordered-set and finite-maximal clauses of
\nolinkurl{plt:def:mot-scale} remain outside the API. &
\nolinkurl{Fabius.isLittleO_plMonomial},
\nolinkurl{Fabius.isAsymptoticScale_plMonomial},
\nolinkurl{Fabius.isAsymptoticScale_plMonomial_pow},
\nolinkurl{Fabius.isAsymptoticScale_plMonomial_log} &
\nolinkurl{TransseriesPolyLogScale.lean} \\
Polynomial logarithmic-block antidifferentiation (3+12).  The polynomial
operator dichotomy in \nolinkurl{plt:lem:mot-block-antiderivative} is Exact:
an explicit degree-preserving inverse off resonance and an explicit
degree-raising primitive at resonance.  This does not construct the full
Laurent ambient ring. &
\nolinkurl{Fabius.blockOperator},
\nolinkurl{Fabius.blockAntiderivative},
\nolinkurl{Fabius.resonantAntiderivative},
\nolinkurl{Fabius.sum_sub_sum_shift},
\nolinkurl{Fabius.blockOperator_zero},
\nolinkurl{Fabius.blockOperator_sub},
\nolinkurl{Fabius.blockOperator_blockAntiderivative},
\nolinkurl{Fabius.blockOperator_surjective},
\nolinkurl{Fabius.natDegree_C_mul_of_ne_zero},
\nolinkurl{Fabius.natDegree_blockOperator},
\nolinkurl{Fabius.blockOperator_injective},
\nolinkurl{Fabius.blockOperator_bijective},
\nolinkurl{Fabius.derivative_resonantAntiderivative},
\nolinkurl{Fabius.derivative_surjective},
\nolinkurl{Fabius.natDegree_resonantAntiderivative} &
\nolinkurl{TransseriesBlockAntiderivative.lean} \\
Abstract differential-block bridge (0+12).  It transports the polynomial
calculation through a derivation satisfying \(d(t)=-t^{2}\), \(d(L)=t\), for
natural and integer exponents.  The source-shaped Laurent law and its degree-
preserving nonresonant primitive use an ambient field and \(t\ne0\);
uniqueness also requires injective evaluation at \(L\), while the older
integer form packages \(t\) as a unit.  Thus
\nolinkurl{plt:eq:mot-block-derivative} is Exact.  Only item~(v) of
\nolinkurl{plt:thm:mot-smallest-differential-algebra} is closed; that compound
theorem remains Partial because its minimal-ring, growth,
algebraic-independence, and concrete Laurent-ring clauses are absent. &
...
\nolinkurl{Fabius.derivation_block_zpow} &
\nolinkurl{TransseriesDifferentialBlock.lean} \\
Catalan quadratic core (3+8).  \nolinkurl{p6:prop:quadratic-core-catalan} is
Exact.  \nolinkurl{p6:lem:quadratic-core} is Partial at the coefficient-family
and coefficient-recursion layer only; packaged power-series
existence/uniqueness, the square-root identity, and the exact denominator
exponent are absent.  \nolinkurl{p6:thm:deepest-pole} is Absent because no
Gamma/Barnes identification is formalized. &
\nolinkurl{Fabius.quadHalf}, \nolinkurl{Fabius.halfBinom},
\nolinkurl{Fabius.quadCoef}, \nolinkurl{Fabius.catalan_two_step},
\nolinkurl{Fabius.quadHalf_zero},
\nolinkurl{Fabius.quadHalf_antidiagonal},
\nolinkurl{Fabius.halfBinom_step}, \nolinkurl{Fabius.quadHalf_rat},
\nolinkurl{Fabius.quadCoef_rat}, \nolinkurl{Fabius.quadCoef_zero},
\nolinkurl{Fabius.quadCoef_rec} &
\nolinkurl{QuadraticCoreCatalan.lean} \\
Derangement nearest-integer surface (1+7).  The integer identity, strict
defect bounds, and rounding conclusion of
\nolinkurl{p8:cor:nearest-integer} are Exact.  The compound printed result is
Partial at arbitrary real \(x>-1\); \nolinkurl{p8:thm:branch-splitting} and
its branch family are Absent. &
...
\nolinkurl{Fabius.round_factorial_mul_exp_neg_one} &
\nolinkurl{DerangementNearestInteger.lean} \\
Linear--logarithmic core inversion (4+18).  Parts (2)--(4) of
\nolinkurl{p0:thm:lambert-core} are Exact over the reals, including the
sign-free equivalence, the positive-branch uniqueness result, and both
negative-\(b\) roots with threshold and order.  The compound theorem remains
Partial because the asymptotic clause and complex general-\(W_k\) reading are
absent. &
...
\nolinkurl{Fabius.linLogCoreRoot_ne_linLogCoreRootLower} &
\nolinkurl{LinLogCoreInversion.lean} \\
Ordinary partial Bell normalization (2+4).  The generating
characterization, factorial normalization bridge, and vanishing clause of
...
\nolinkurl{Fabius.ordinaryPartialBell_eq_zero_of_lt} &
\nolinkurl{OrdinaryPartialBell.lean} \\
Power--logarithmic core inversion (3+6).  The real \(r=1\) inversion and
slope clauses of \nolinkurl{p6:lem:core} are Exact.  The compound lemma
remains Partial because general \(r\), its root determination, and the
complex general-\(W_k\) reading are absent. &
...
\nolinkurl{Fabius.hasDerivAt_powerLogCore_root} &
\nolinkurl{PowerLogCoreInversion.lean} \\
Remainder transport (0+3).  Part (1),
\nolinkurl{p0:eq:transport-bound}, of
\nolinkurl{p0:thm:remainder-transport} is Exact and keeps its one-sided
derivative and two-sided Lipschitz hypotheses distinct.  The compound theorem
is Partial because part (2), the first-order law requiring a second-order
Taylor estimate, is absent. &
\nolinkurl{Fabius.lipschitzOn_of_abs_deriv_le},
\nolinkurl{Fabius.transport_bound_mul},
\nolinkurl{Fabius.transport_bound} &
\nolinkurl{RemainderTransport.lean} \\
Staircase inversion (0+7).  All five clauses of
\nolinkurl{p0:thm:staircase} are Exact at the strict-monotonicity and inverse
equation interface, and the separation failure at arbitrarily small scale is
formal too.  The module does not construct admissible interpolations or prove
the periodic layer's Fourier expansion. &
...
\nolinkurl{Fabius.exists_half_error_of_jump} &
\nolinkurl{StaircaseInversion.lean} \\
Flatness and invisible remainders (4+22).  \nolinkurl{q0:def:flat} and all
three clauses of \nolinkurl{q0:prop:invisible} are Exact for vector-valued
functions and coefficients on a scalar-valued scale, with the same-coefficient
...
\nolinkurl{Fabius.isPoincareExpansion_add_exp_neg} &
\nolinkurl{TransseriesFlat.lean} \\
Harmonic-increment leading order (0+2).  The \(w_n/n\to c_0\) clause of
\nolinkurl{plt:lem:mot-harmonic} is Exact.  The compound lemma remains Partial
because the logarithmic correction, constant, and \(o(1)\) remainder are
absent. &
\nolinkurl{Fabius.tendsto_div_atTop_of_tendsto_sub},
\nolinkurl{Fabius.tendsto_div_atTop_of_harmonic_increment} &
\nolinkurl{TransseriesHarmonicIncrement.lean} \\
Stirling-series coefficients (3+12).  The formal definition, recurrence, and
the displayed values \(c_0=1\), \(c_1=1/12\), \(c_2=1/288\), and
\(c_3=-139/51840\) in \nolinkurl{q2:eq:stirling-cj} are Exact.  The
surrounding Fubini-sector corollary remains Partial because this leaf proves no
...
\nolinkurl{Fabius.stirlingCoeff_three} &
\nolinkurl{StirlingSeriesCoefficients.lean} \\
Wright omega basics (1+13).  The complete real proposition
...
\nolinkurl{AnalyticAt} over \(\RealNumbers\) at every real input; it asserts no
...
\nolinkurl{Fabius.tendsto_wrightOmega_atTop} &
\nolinkurl{WrightOmega.lean} \\
First two Wright-omega orders (0+8).  The two concluding real asymptotic
...
\nolinkurl{Fabius.wrightOmega_residual_isEquivalent} &
\nolinkurl{WrightOmegaTwoOrders.lean} \\
\bottomrule
\end{longtable}
...
The adjacent one-definition, ten-theorem module
...
The new bridge is an unconditional definitional equality for every complex
\(a,q\): the historical and generic names unfold to the identical total
Mathlib \texttt{tprod}.  It therefore also holds when the factors are not
multipliable and both names take the total product's junk value; it makes no
convergence assertion in that regime.
...
compound spectral clauses.  This overlay is included in the finalized
canonical-frontier publication; exact frozen-source and final-PDF receipts are
kept externally in the repository documentation.  The generic-name
analytic-order theorem is owned by
\path{QPochhammerInfinite.lean}, and the unconditional public bridge
\lean{complexQPochhammerInf_eq_qPochhammerInfIn} remains available.
...
the raw equation \(a_0q^j=1\), including \(q=0,j=0\), while
\lean{deriv_qPochhammerInfIn_ne_zero_of_mul_pow_eq_one} proves that derivative
nonzero.  With \(q\ne0\),
...
The \path{CentralQBinomialReduction.lean} has no definitions and
...
seventy theorems, seventy-four public declarations in total.  The small public
inventories are
...
This promotes the fixed-column row from Partial to Exact.  With the later
one-definition/four-theorem RatFunc closure, the moment-polynomial row is also
Exact while the stronger MGF row remains Partial.  The authoritative
282-result forward q ledger is therefore 181 Exact, 79 Partial, 14 None, and
8 N/A.
The subsequent zero-definition/nine-theorem
...
\(\lVert q\rVert\le1\), the generalized product bound upstream and the first
theorem here bound
\(\lVert(q^m;q)_k-1\rVert\) by
\(\exp(k\lVert q\rVert^m)-1\) and
\(k e^k\lVert q\rVert^m\); the second theorem here gives
...
fixed and shifted nonasymptotic additive estimates; the arbitrary-shift
convergence theorem remains upstream.  The final four declarations give the
relative/additive \texttt{IsBigO} laws at rates \(q^{n-k+1}\) and
...
\lean{Fabius.Appell.polynomialTransform}; its theorems are
\lean{Fabius.Appell.polynomialTransform_apply},
\lean{Fabius.Appell.polynomialTransform_monomial},
\lean{Fabius.Appell.polynomialTransform_eq_sum_hasseDeriv_of_natDegree_lt},
\lean{Fabius.Appell.polynomialTransform_eq_sum_hasseDeriv},
...
\lean{hasSum_rogersSzego_generating}.  This source inventory is included in
the finalized canonical-frontier publication; exact frozen-source and
...
chronology first combines the exact 36-declaration Lambert branch-coordinate
tranche with the twelve-declaration terminating-basic-hypergeometric closure
at 901/11,430.  The 3+7
\path{GeometricRichardsonGenerating.lean} leaf gives 902/11,440, and the three
second-moment additions to \path{GaussianBinomialCumulants.lean} give
902/11,443.  The successive three-, four-, and five-theorem
Lambert--Bernoulli receipts are 903/11,446, 903/11,447, and 903/11,448.
The local five-theorem effective fixed-column q-binomial closure then enlarges
\path{QBinomialTheoremInfinite.lean} to 1+27 and gives 903/11,453.
Independently, the 1+8
\path{GeometricUniformMomentPolynomial.lean} branch gives 904/11,457 from the
Lambert checkpoint, and its first union with the local fixed-column branch is
the historical 904/11,462 receipt.  At that intermediate receipt the
polynomial row was still Partial because its later analytic and sharp-degree
closures had not yet landed.  The later merged
...
the incoming-branch \path{GaussianBinomialFixedColumnRate.lean} (0+10), and
...
\path{ThueMorseGammaTowerDifferential.lean} (0+3) leaf brings the historical
...
925/11,619 union.  Subsequent merged-main additions give the pre-reciprocity
930/11,678 checkpoint.  Making
\lean{differentiable_geometricUniformComplexMomentProduct} public gives
930/11,679, and the \path{GeometricUniformMomentReciprocity.lean} (1+5) leaf
gives the historical 931/11,685 union.  The subsequently merged upstream
\path{DyadicBoundaryIdentity.lean} and
\path{FinitePrefixThueMorseCollapse.lean} modules add two modules and ten
public declarations, giving the historical dyadic/finite-prefix 933/11,695
union.  The incoming union adds one module and fourteen public declarations:
...
this does not classify all roots over every extension field.  The rigorous q
forward ledger is 181 Exact / 79 Partial / 14 None / 8 interface rows;
q-Lucas remains Partial.  Its source concordance is 103 Lean-proved / 375
human-proved frontier / 60 non-applicable / 9 conjectures.
...
identity; it does not promote the larger right-inverse row.
...
\leansubtwo{geometricUniformMomentPolynomial_eval}{_eq_mgf_taylorCoefficient}, which
...
public surface one definition and three theorems:
...
prove its complex differentiability on the whole plane, and give the
finite-q-Pochhammer normalization of its Taylor coefficient.  The
...
analytic continuation, so the broader MGF, germ-uniqueness, and pole-divisor
claims remain Partial.

The exhaustive public surface of
\path{GeometricUniformMomentReciprocity.lean} is the definition
...
\lean{geometricUniformComplexMomentGerm_moment_convolution}.  The definition
selects the inner entire product when \(\lVert q\rVert<1\) and the exterior
reciprocal germ when \(1<\lVert q\rVert\).  For
\(q\ne0\) and \(\lVert q\rVert\ne1\), the fourth theorem is the
\texttt{EventuallyEq} at zero
\[
 M_q(z)M_{q^{-1}}(-z)=1.
\]
This is equality of analytic germs, not a global pointwise identity.  In the
inner regime, continuity and \(M_q(0)=1\) give a neighbourhood on which the
inner product is nonzero, while the exterior definition rewrites
\(M_{q^{-1}}(-z)\) as its inverse.  In the outer regime the same proof with
\(q^{-1}\) reverses the factors.  Differentiating the local equality and
using the product and negation rules gives
\[
 \sum_{k=0}^{n}\binom nk(-1)^{n-k}
 M_q^{(k)}(0)M_{q^{-1}}^{(n-k)}(0)=\delta_{n0}.
\]
The derivatives are exactly the manuscript coefficients \(d_n\), so
\nolinkurl{thm:qF-reciprocity} is Exact.  Neither theorem assigns a boundary
value on \(\lVert q\rVert=1\), includes \(q=0\), chooses an explicit maximal
disc, cancels globally across product zeros, proves normalized Mahler-germ
uniqueness, or describes the exterior pole divisor.
...
biorthogonality leaf gives the historical 925/11,619 census.  Subsequent
merged-main additions give the pre-reciprocity 930/11,678 checkpoint; public
complex-product differentiability gives 930/11,679, and the reciprocity leaf
gives the historical 931/11,685 census.  The subsequently merged upstream
\path{DyadicBoundaryIdentity.lean} and
\path{FinitePrefixThueMorseCollapse.lean} modules add two modules and ten
public declarations, giving the historical dyadic/finite-prefix 933/11,695
census.  The incoming union adds one module and fourteen public declarations:
the new zero-definition/six-theorem \path{ProuhetBaseTwoBridge.lean} module,
one theorem added to \path{DyadicBoundaryIdentity.lean}, and seven theorems
added to \path{ThueMorseNewmanSelfSimilarity.lean}.  This gives the historical
934/11,709 checkpoint.  The local branch then recorded historical
...
The rigorous q forward ledger is 181 Exact / 79 Partial / 14 None / 8
interface rows: q-Lucas remains Partial;
...
The reciprocity leaf additionally moves
\nolinkurl{thm:qF-reciprocity} None-to-Exact.
These additions are incorporated in this merged source edition.  The retained
257-page PDF and the accepted 273-page preintegration PDF are explicitly
historical receipts for the editions preceding this merged source.
...
The final five q-series modules contribute exactly two definitions and
twenty-nine new theorems.  \path{GaussianBinomialInteger.lean} defines
\lean{gaussianBinomialZ} and proves
\lean{finiteQPochhammerIn_inv_base_eq},
\lean{finiteQPochhammerIn_mul_pow_inv_base},
\lean{finiteQPochhammerIn_pow_div},
\lean{gaussianBinomialZ_zero_right},
\lean{gaussianBinomialZ_natCast}, \lean{gaussianBinomialZ_succ},
\lean{gaussianBinomialZ_succ'}, \lean{gaussianBinomialZ_neg_natCast},
\lean{hasSum_reciprocal_finiteQPochhammerIn}, and
\lean{hasSum_reciprocal_finiteQPochhammerIn_neg}.  The algebraic declarations
hold over a field.  Inverse-base and natural-index comparison require
\(q\ne0\); the quotient and Pascal forms retain their displayed nonzero
denominators.  The two \texttt{HasSum} results use a complete normed field,
\(\lVert q\rVert<1\), and \(\lVert z\rVert<1\), with \(q\ne0\) additionally
required in the negative-index form.

\path{GaussianBinomialComplexOrder.lean} defines the principal-power
\lean{gaussianBinomialC} and proves
\lean{gaussianBinomialC_intCast}, \lean{gaussianBinomialC_natCast},
\lean{hasSum_gaussianBinomialC_add}, \lean{hasSum_qPochhammerC_inv}, and
\lean{hasSum_qPochhammerC}.  Integer agreement is unconditional; natural
agreement retains \(q\ne0\) and the nonzero finite denominator.  The first two
series assume \(\lVert q\rVert<1\) and \(\lVert z\rVert<1\); the last assumes
\(q\ne0\) and \(\lVert zq^\alpha\rVert<1\).

\path{QPfaffSaalschutz.lean} has no definitions and exactly the three theorems
\lean{finiteQPochhammerIn_ne_zero_of_le},
\lean{q_pfaff_saalschutz_term}, and \lean{q_pfaff_saalschutz}.  The
terminating identity is algebraic over a field under nonzero \(q,a,b,c\) and
the four displayed nonzero finite-product denominators; its term theorem also
assumes \(k\le n\).  It makes no convergence or limiting claim.

\path{QuantumMultinomial.lean} has no definitions and exactly the five
theorems \lean{sum_antidiagonalTuple_succ},
\lean{gaussianBinomial_eq_evalRingHom_quantum},
\lean{gaussianBinomial_symm'}, \lean{Commute.qMultinomial_left}, and
\lean{quantum_multinomial}.  The ordered expansion holds in every semiring
when \(q\) commutes with each variable and \(x_jx_i=q(x_ix_j)\) for \(i<j\).
It uses \texttt{Nat.antidiagonalTuple} and an ordered list product and requires
neither centrality of \(q\), ambient commutativity, division, nor convergence.

\path{GaussianBinomialBounds.lean} has no definitions and exactly the six new
theorems \lean{gaussianBinomial_inv}, \lean{one_le_gaussianBinomial},
...
\lean{gaussianBinomial_le_pow_div_of_one_lt}.  Reciprocity holds over a field
for \(q\ne0\) and \(k\le n\); the lower bound is ordered-field algebra for
\(q\ge0\); and the remaining bounds are real under the displayed
\(0\le q<1\) or \(Q>1\) assumptions.  Positivity of \((q;q)_k\) reuses the
established \lean{finiteQPochhammerIn_self_pos} theorem from
\path{GeneralQConditionNumber.lean} rather than redeclaring it.  No asymptotic
error term is proved.
Consequently the growth, uniform-bound, and extreme-specialization rows are
Exact; the later greater-than-one asymptotics leaf also makes the \(q>1\)
transfer row Exact.

The rigorous 282-row forward q-series ledger is therefore 181 Exact, 79
Partial, 14 None, and 8 interface rows.  The evaluated q-Lucas theorem remains
Partial because its polynomial-congruence lift is absent; the Gaussian
majorant and the \(Q>1\) asymptotic row are Exact.  The
...
\path{QChuVandermonde.lean} (0+10).  The two definitions of the first are
\lean{twoPhiOneFinite} and \lean{twoPhiOneReflection}; its twelve theorems are
...
\lean{twoPhiOneReflection_involutive}, \lean{twoPhiOneFinite_reversal},
\lean{twoPhiOneFinite_reversal_twice},
\lean{twoPhiOneFinite_eq_sum_twoPhiOneTerm},
\lean{twoPhiOne_eq_twoPhiOneFinite_inv_pow}, \lean{twoPhiOne_reversal},
\lean{twoPhiOne_reversal_twice}, and
\lean{twoPhiOne_one_eq_twoPhiOneFinite_zero}.
The ten theorems of \path{QChuVandermonde.lean} are
...
\lean{twoPhiOne_q_chu_vandermonde_second}.  Together they prove the
finite-to-tsum termination bridge, reflected-parameter involution, one- and
two-step reversal formulas, both q-Chu--Vandermonde evaluations, and
full-domain actual-tsum wrappers.
...
The three later additions to \path{GaussianBinomialCumulants.lean}, now 2+24,
are \lean{eval_one_derivative_derivative_gaussianBinomial_X},
\lean{twelve_mul_secondMoment_gaussianBinomial_eval_one}, and
\lean{twelve_mul_varianceNumerator_gaussianBinomial_eval_one}.  For
\(P=[n,k]_X\), \(D=k(n-k)\), and \(C=\binom nk\), the first theorem assumes a
...
The one-definition, ten-theorem complex Pochhammer interface: the unconditional
definitional bridge \lean{complexQPochhammerInf_eq_qPochhammerInfIn}, including
nonmultipliable total-product values; strict-contraction product convergence,
absolute paired perturbation summability, the global general-\(q\)
...
The zero-definition, five-theorem compatibility interface for
\(a\mapsto(a;q)_\infty\): locally uniform convergence on \(\ComplexNumbers\),
entireness, the exact division-free factor-zero locus, the reciprocal-power
lattice for \(q\ne0\), and analytic order one at every zero for complex
\(\lVert q\rVert<1\), including \(q=0\), under the historical wrapper name;
...
The exhaustive one-definition, twenty-nine-theorem generic infinite-product
...
nonzero factor-zero derivative, plus
\lean{analyticOrderAt_qPochhammerInfIn_of_eq_zero}, including \(q=0\); the
closed reciprocal-zero derivative additionally assumes \(q\ne0\).  It asserts no joint-\(q\)
holomorphy or global growth, order, type, or asymptotic theorem.\\
...
The one-definition, twenty-nine-theorem Euler/Cauchy interface: a Banach-valued
Tannery lemma, real comparison products, normed-field bounds and fixed-column
rates, and, over complete normed fields, Euler product and reciprocal
expansions and the infinite q-binomial theorem under the stated strict nome
and series-variable bounds.\\
\path{GaussianBinomialFixedColumnRate.lean} &
The exhaustive zero-definition, nine-theorem effective fixed-column interface:
two finite-product defect bounds, a denominator-free relative Gaussian bound,
fixed and shifted nonasymptotic additive estimates, and four relative/additive
geometric \texttt{IsBigO} results. The normed-field product bound and
arbitrary-shift convergence live upstream in
\path{QBinomialTheoremInfinite.lean}.  The ring layer uses
\(\lVert q\rVert\le1\); the normed-field layer uses only
\(\lVert q\rVert<1\), without completeness or \(q\ne0\).\\
\path{GaussianBinomialGreaterOneAsymptotics.lean} &
The exhaustive zero-definition, two-theorem real-\(q>1\) interface: reciprocity
transfers the fixed-column relative-error rate and central equivalence, making
\nolinkurl{cor:qgreaterone} Exact without a shifted-central or wider-domain
claim.\\
\path{JacobiTripleProduct.lean} &
The two-definition, twenty-five-theorem Jacobi/pentagonal interface: an exact
finite polynomial identity over commutative rings, its nonzero-variable field
Laurent form, and complete-normed-field bilateral and pentagonal sums for
\(\lVert q\rVert<1\); no unit-circle or modular-transformation claim.\\
\path{ThetaQuasiPeriodicity.lean}, \path{JacobiCubic.lean} &
The one-definition/six-theorem bilateral-theta interface, including its exact
quasi-periodicity and zero set \(z=-q^m\) under the stated nonzero-variable,
nonzero-nome, and strict-contraction hypotheses, followed by the
zero-definition/two-theorem Jacobi cubic summation interface.\\
\path{QPascalSummation.lean}, \path{QuantumBinomial.lean} &
Four finite q-Pascal row-splitting/commutation theorems, followed by the two
noncommutative-semiring quantum-plane identities under exactly the displayed
commutation relations; neither interface uses division or convergence.\\
...
The complete 2+24 mean/variance surface, including the characteristic-zero
field second derivative under \(k\le n\) and the two all-row, arbitrary-
commutative-semiring, division-free raw-second-moment and variance-numerator
identities.\\
...
The one-definition, nine-theorem Rogers--Szeg\H{o} interface: finite
commutative-(semi)ring boundary and recurrence laws and the complete-normed-field
generating series under \(\lVert q\rVert<1\), \(\lVert t\rVert<1\), and
\(\lVert zt\rVert<1\); no orthogonality or root-location theorem.\\
\path{PolynomialQDerivative.lean} &
The exhaustive two-definition, seventeen-theorem coefficientwise polynomial
Jackson calculus.  Q-integers are semiring-valued; their geometric identity
and the division-free evaluation law use a commutative ring; scaling and both
product rules use a commutative semiring.  Arbitrary \(q\), including zero and
one; no analytic-function derivative or \(q\to1\) limiting theorem.\\
\path{PolynomialQLeibniz.lean} &
The exhaustive zero-definition, four-theorem commutative-semiring interface
for composing scalings, constant linearity, iterated scaling, and the finite
Gaussian-coefficient q-Leibniz rule at every natural order.  No division,
nonzero nome, topology, or convergence hypothesis.\\
\path{QExponential.lean}, \path{JacksonIntegral.lean} &
The three-definition/eight-theorem q-exponential interface and the
one-definition/seven-theorem Jackson-integral interface.  The q-product rule
is unconditional; the analytic series, eigenfunction, fundamental-theorem,
and integration-by-parts statements retain their distinct nome, disc,
pointwise, convergence, or summability hypotheses.\\
\path{QPochhammerDerivative.lean} &
The exhaustive zero-definition, three-theorem finite-parameter derivative
interface over a nontrivially normed field: unconditional cofactor sum,
logarithmic rewrite under exact factorwise nonvanishing, and chain rule under
the supplied pointwise derivative.  It does not differentiate the infinite
product.\\
\path{LambertSeriesLog.lean} &
The exhaustive zero-definition, four-theorem complex interface: an
unconditional natural-cast norm helper and, under exactly
\(\lVert a\rVert<1\), \(\lVert q\rVert<1\), absolute Lambert summability,
principal-factor-log \texttt{HasSum}, and the branch-free exponential product.
The norm-one boundary, a principal log of the product, continuation, and
differentiated series remain open.\\
\path{QPochhammerLogDerivative.lean},
\path{QPochhammerOrderDerivative.lean} &
The zero-definition ten-theorem logarithmic/Lambert derivative interface under
complex \(\lVert q\rVert<1\), \(\lVert a\rVert<1\), followed by the
zero-definition three-theorem complex-order derivative interface under
\(q\ne0\), \(\lVert q\rVert<1\), and
\(\lVert aq^\alpha\rVert<1\).\\
\path{QGamma.lean} &
The exhaustive two-definition, ten-theorem real q-gamma interface:
positivity, value one, recurrence and natural factorial product for
\(0<q<1\) under their displayed \(x>0\); natural q-number/q-integer agreement
for \(q\ne1\); totalized reflection for \(q<1\); and its theta
\texttt{HasSum} for \(0<q<1\), \(0<x<1\).  Complex continuation, poles,
the classical-gamma limit, log-convexity/uniqueness, and q-digamma calculus
remain outside Lean.\\
...
only simple zeros.  The separate normal-convergence leaf proves locally
uniform convergence on \(\ComplexNumbers\) of the outer pure Pochhammer
factorization for each fixed strict-contraction nome, with dyadic Rvachev and
bounded-Fabius specializations.\\
\emph{Named geometric-\(q\) law (continued)} & --- &
With \(z_q(t)=(1-q)t/(2\pi)\), exact named bridges prove
\(\phi_q(t)=e^{\ImaginaryUnit t/2}S_q(z_q(t))
...
joint normality of the outer spectral Pochhammer product in \((q,z)\) or
uniformity up to \(\lVert q\rVert=1\),
...
and proves no maximality, positivity, or rationality theorem.  This module
changes the facade census but no row in either the forward q-series ledger or
the source concordance.
...
\leansubtwo{Fabius.geometricUniformMomentPolynomial_eval}{_eq_mgf_taylorCoefficient}.
...
\path{Fabius.geometricUniformComplexMomentProduct} and the three theorems
\path{Fabius.hasProdLocallyUniformly_geometricUniformComplexMomentProduct},
\path{Fabius.differentiable_geometricUniformComplexMomentProduct}, and
\leansubtwo{Fabius.geometricUniformMomentPolynomial_eval}{_eq_complexMomentProduct_taylorCoefficient}.
They construct the actual locally uniform product, prove its complex
differentiability, and give its normalized Taylor-coefficient bridge for
every complex \(\lVert q\rVert<1\).  This is an
...
\leansubtwo{Fabius.geometricUniformMomentPolynomial_eval}{_eq_exteriorComplexMomentGerm_taylorCoefficient}.
...
The exhaustive \(1+5\) companion
\path{GeometricUniformMomentReciprocity.lean} consists of
\path{Fabius.geometricUniformComplexMomentGerm},
\path{Fabius.geometricUniformComplexMomentGerm_of_norm_lt_one},
\path{Fabius.geometricUniformComplexMomentGerm_of_one_lt_norm},
\path{Fabius.analyticAt_geometricUniformComplexMomentGerm},
\path{Fabius.geometricUniformComplexMomentGerm_reciprocity}, and
\path{Fabius.geometricUniformComplexMomentGerm_moment_convolution}.  For
\(q\ne0\) and \(\lVert q\rVert\ne1\), its last two declarations prove the
local analytic-germ reciprocity and its all-degree binomial derivative
convolution.  The reciprocal equality is an \path{EventuallyEq} at zero, not
a global cancellation across product zeros.  It makes
\nolinkurl{thm:qF-reciprocity} Exact and adds no unit-circle, maximal-disc,
uniqueness, or pole-divisor claim.
...
\item \path{TwoPhiOneReversal.lean} has exactly two public definitions,
\path{Fabius.twoPhiOneFinite} and \path{Fabius.twoPhiOneReflection}, and twelve
public theorems:
\path{Fabius.choose_two_add_succ_choose_two},
\path{Fabius.finiteQPochhammerIn_sub_eq},
\path{Fabius.finiteQPochhammerIn_reversal_ne_zero},
\path{Fabius.finiteQPochhammerIn_inv_pow_self},
\path{Fabius.twoPhiOneReflection_involutive},
\path{Fabius.twoPhiOneFinite_reversal},
\path{Fabius.twoPhiOneFinite_reversal_twice},
\path{Fabius.twoPhiOneFinite_eq_sum_twoPhiOneTerm},
\path{Fabius.twoPhiOne_eq_twoPhiOneFinite_inv_pow},
\path{Fabius.twoPhiOne_reversal},
\path{Fabius.twoPhiOne_reversal_twice}, and
\path{Fabius.twoPhiOne_one_eq_twoPhiOneFinite_zero}.  This is the exhaustive
finite/actual-tsum reversal and reflected-parameter involution surface.

\item \path{QChuVandermonde.lean} has no public definitions and exactly ten
public theorems:
\path{Fabius.two_mul_choose_two},
\path{Fabius.mul_sub_one_eq_mul_sub_add},
\path{Fabius.finiteQPochhammerIn_div_eq_sum_chu},
\path{Fabius.q_chu_vandermonde_first},
\path{Fabius.finiteQPochhammerIn_div_eq_sum_chu_second},
\path{Fabius.twoPhiOneFinite_mul_finiteQPochhammerIn_eq_chu_second},
\path{Fabius.q_chu_vandermonde_second},
\path{Fabius.q_chu_vandermonde_second_by_reversal},
\path{Fabius.twoPhiOne_q_chu_vandermonde_first}, and
\path{Fabius.twoPhiOne_q_chu_vandermonde_second}.  It contains both cleared
and quotient q-Chu--Vandermonde forms, the separately named reversal route,
and the two full-domain actual-tsum wrappers with their exact hypotheses.
...
3 & $0.147505374$ & $0.147501988$ & $0.125788484$ \\
...
\path{GaussianBinomialContinuity.lean} & Zero definitions and exactly three theorems: \lean{continuous_gaussianBinomial}, \lean{tendsto_gaussianBinomial_nhds_one}, and \lean{gaussianBinomial_eq_finiteQPochhammerIn_div}.  The first two hold for all \(n,k\) in every topological semiring with continuous operations.  The quotient identity needs only a field, \(k\le n\), and \((q;q)_k\ne0\), with no topology. \\
\path{JacobiTripleProduct.lean} & Two definitions, \lean{thetaExponent} and \lean{pentagonalExponent}, and exactly twenty-five theorems: \lean{two_dvd_mul_sub_one}, \lean{mul_sub_one_nonneg}, \lean{two_mul_thetaExponent}, \lean{thetaExponent_natCast}, \lean{thetaExponent_neg_natCast}, \lean{choose_two_add_choose_two_succ_eq}, \lean{sum_mul_sum_eq_sum_antidiagonal}, \lean{thetaCoeff_pair_eq}, \lean{sum_antidiagonal_thetaCoeff}, \lean{finite_triple_product_poly}, \lean{pow_mul_finiteQPochhammerIn_div}, \lean{neg_one_zpow_natCast_sub}, \lean{finite_triple_product}, \lean{norm_gaussianBinomialInt_le}, \lean{tendsto_gaussianBinomialInt_central}, \lean{summable_pow_thetaExponent_mul_zpow}, \lean{hasSum_jacobi_triple_product}, \lean{hasSum_jacobi_triple_product'}, \lean{mul_three_mul_sub_one_nonneg}, \lean{two_dvd_mul_three_mul_sub_one}, \lean{two_mul_pentagonalExponent}, \lean{pentagonalExponent_eq}, \lean{pentagonalExponent_pos}, \lean{hasSum_pentagonal}, and \lean{hasSum_pentagonal_pairs}.  Exponent arithmetic is unconditional; the finite polynomial block is over commutative rings, while the Laurent block is over fields and its divided-product formulas assume \(z\ne0\).  The norm bound needs a normed field and \(\lVert q\rVert<1\); the central limit, Jacobi, and pentagonal identities add completeness, with \(z\ne0\) exactly for Jacobi.  The pentagonal results include \(q=0\); the real theta majorant assumes \(0\le r<1\) and \(0<s\). \\
\path{QBinomialTheoremInfinite.lean} & One definition, \lean{gaussianMajorant}, and exactly twenty-nine theorems: \lean{hasSum_of_tendsto_of_dominated}, \lean{one_le_qPochhammerInfIn_neg}, \lean{finiteQPochhammerIn_neg_le_qPochhammerInfIn}, \lean{qPochhammerInfIn_nonneg}, \lean{qPochhammerInfIn_le_finiteQPochhammerIn}, \lean{qPochhammerInfIn_pos_of_lt_one}, \lean{qPochhammerInfIn_zero_left}, \lean{norm_finiteQPochhammerIn_le_neg_norm}, \lean{finiteQPochhammerIn_norm_le_norm}, \lean{norm_finiteQPochhammerIn_le}, \lean{qPochhammerInfIn_norm_le_norm_finiteQPochhammerIn}, \lean{finiteQPochhammerIn_ne_zero_of_norm_lt_one}, \lean{gaussianBinomial_eq_div}, \lean{gaussianMajorant_nonneg}, \lean{norm_gaussianBinomial_le}, \lean{tendsto_finiteQPochhammerIn_pow_atTop}, \lean{norm_finiteQPochhammerIn_pow_sub_one_le_exp_of_norm_le_one}, \lean{isBigO_finiteQPochhammerIn_pow_sub_one}, \lean{tendsto_gaussianBinomial_atTop}, \lean{tendsto_gaussianBinomial_add_const_atTop}, \lean{isBigO_gaussianBinomial_sub_inv}, \lean{isBigO_gaussianBinomial_add_sub_inv}, \lean{summable_pow_choose_two_mul_pow}, \lean{hasSum_euler_product}, \lean{qPochhammerInfIn_ne_zero_of_norm_lt_one}, \lean{hasSum_qBinomial_theorem}, and \lean{hasSum_euler_reciprocal}.  The Tannery theorem is Banach-valued with its common summable bound, row limits, row \texttt{HasSum}s, and sum limit.  The real comparison block uses exactly \(0\le r<1\) and its displayed hypotheses on \(x\); the infinite zero-left theorem is unconditional over a topological commutative ring.  Norm bounds and fixed-column limits need a normed field and their displayed contraction/unit-ball hypotheses.  Over a complete normed field Euler's product needs only \(\lVert q\rVert<1\) and arbitrary \(z\), while the infinite q-binomial and reciprocal identities add \(\lVert z\rVert<1\), with arbitrary \(a\) and no \(q\ne0\).  The finite zero-left theorem belongs to \path{GaussianBinomialAtOne.lean}. \\
\path{QPascalSummation.lean} & Zero definitions and exactly four theorems: \lean{sum_gaussianBinomial_succ_mul}, \lean{sum_gaussianBinomial_succ_mul'}, \lean{Commute.gaussianBinomial_left}, and \lean{Commute.gaussianBinomial_right}.  The first summation and both commutation laws hold in every semiring, with only the indicated \texttt{Commute} hypothesis for the latter; the second summation assumes a commutative semiring.  There is no division, order, topology, or restriction on \(q,n\), or the weight sequence. \\
\path{QuantumBinomial.lean} & Zero definitions and exactly two theorems, \lean{quantumPlane_mul_pow} and \lean{quantum_binomial}, over an arbitrary possibly noncommutative semiring.  The first assumes that \(q\) commutes with \(X\) and \(YX=q(XY)\); the second additionally assumes that \(q\) commutes with \(Y\).  No division or topology is used. \\
\path{RogersSzegoPolynomial.lean} & One definition, \lean{rogersSzego}, and exactly nine theorems: \lean{rogersSzego_zero}, \lean{rogersSzego_one}, \lean{rogersSzego_succ}, \lean{one_sub_pow_succ_mul_gaussianBinomial_succ_succ}, \lean{rogersSzego_dilation}, \lean{rogersSzego_three_term}, \lean{summable_norm_pow_div_finiteQPochhammerIn_self}, \lean{sum_antidiagonal_euler_mul_euler}, and \lean{hasSum_rogersSzego_generating}.  The definition and first three theorems use a commutative semiring; the cross, dilation, and three-term laws use a commutative ring and are total in their natural indices.  The summability theorem needs a normed field, \(\lVert q\rVert<1\), and \(\lVert z\rVert<1\); the antidiagonal identity needs only the contraction.  The generating function adds completeness and assumes exactly \(\lVert q\rVert<1\), \(\lVert t\rVert<1\), and \(\lVert zt\rVert<1\). \\
\path{PolynomialQDerivative.lean} & Two definitions, \lean{qInt} and \lean{qDerivative}, and exactly seventeen theorems: \lean{qInt_zero}, \lean{qInt_one}, \lean{qInt_succ}, \lean{qInt_succ'}, \lean{qInt_add}, \lean{qInt_one_left}, \lean{one_sub_mul_qInt}, \lean{qDerivative_apply}, \lean{qDerivative_monomial}, \lean{qDerivative_C}, \lean{qDerivative_X_pow}, \lean{qDerivative_X}, \lean{qDerivative_C_mul_X_pow}, \lean{eval_qDerivative_mul}, \lean{qDerivative_comp_C_mul_X}, \lean{qDerivative_mul}, and \lean{qDerivative_mul'}.  The q-integer layer is semiring-valued; its geometric identity and the division-free evaluation law use a commutative ring; the other derivative laws use a commutative semiring.  Arbitrary \(q\), including zero and one.  No analytic-function derivative or \(q\to1\) limit is asserted. \\
\path{PolynomialQLeibniz.lean} & Zero definitions and exactly four theorems: \lean{comp_C_mul_X_comp_C_mul_X}, \lean{qDerivative_C_mul}, \lean{qDerivative_iterate_comp_C_mul_X}, and \lean{qDerivative_iterate_mul}.  All hold over a commutative semiring for arbitrary \(q\) and every natural order including zero.  No division, nonvanishing, topology, or convergence is used. \\
\path{QPochhammerDerivative.lean} & Zero definitions and exactly three theorems: \lean{hasDerivAt_finiteQPochhammerIn}, \lean{hasDerivAt_finiteQPochhammerIn_of_ne_zero}, and \lean{hasDerivAt_finiteQPochhammerIn_comp}.  Over a nontrivially normed field and arbitrary \(q,a,n\), the cofactor formula is unconditional; the logarithmic form assumes exactly every finite factor is nonzero; the chain rule assumes exactly the supplied pointwise derivative.  No infinite-product derivative, log branch, multiplicity, or joint-parameter theorem is asserted. \\
\path{LambertSeriesLog.lean} & Zero definitions and exactly four theorems: \lean{one_le_norm_natCast_add_one}, \lean{summable_lambert_series}, \lean{hasSum_lambert_log_complex}, and \lean{exp_neg_tsum_lambert_eq_qPochhammerInfIn}.  Apart from the unconditional norm helper, they assume exactly complex \(\lVert a\rVert<1\) and \(\lVert q\rVert<1\), and prove absolute summability, a principal-factor-log \texttt{HasSum}, and the branch-free exponential identity.  No norm-one boundary, principal log of the product, analytic continuation, or differentiated Lambert series is asserted. \\
\path{QGamma.lean} & Two total real definitions, \lean{qGamma} and \lean{qNumber}, and exactly ten theorems: \lean{norm_lt_one_of_pos_of_lt_one}, \lean{qPochhammerInfIn_rpow_pos}, \lean{qPochhammerInfIn_self_pos}, \lean{qGamma_pos}, \lean{qGamma_one}, \lean{qGamma_add_one}, \lean{qGamma_nat_succ}, \lean{qNumber_natCast}, \lean{qGamma_mul_qGamma_one_sub}, and \lean{hasSum_theta_qGamma_reflection}.  The positive recurrence API uses \(0<q<1\) and displayed \(x>0\); the natural bridge only \(q\ne1\); reflection only \(q<1\) and arbitrary real \(x\); theta form \(0<q<1\), \(0<x<1\).  Complex continuation, poles, the classical-gamma limit, log-convexity/uniqueness, and q-digamma calculus remain outside Lean. \\
...
\path{GaussianBinomialCumulants.lean} & Exactly two public definitions and twenty-four theorems.  Its three newest theorems are \lean{eval_one_derivative_derivative_gaussianBinomial_X} over a characteristic-zero field under \(k\le n\), and the all-\(n,k\), arbitrary-commutative-semiring, division-free \lean{twelve_mul_secondMoment_gaussianBinomial_eval_one} and \lean{twelve_mul_varianceNumerator_gaussianBinomial_eval_one}; the latter include the zero, diagonal, and above-row boundaries. \\
...
\begin{equation}
 ...
 \qquad
 C_F=-2.02798889863885942\ldots.                            \label{dyadicasym:eq:constants-numeric}
\end{equation}
...
\sectionmark{Inverse-dyadic sampling}
...
\addplot+[mark=triangle*] coordinates {
 ...
 (14,0.0000007301568984988352) (15,0.00000020954757928848267)
...
$150$ & $0.297351$ & $0.5209883273$ & $0.5203979284$ & $0.5209087825$\\
$155$ & $0.342900$ & $0.5204425007$ & $0.5198167900$ & $0.5203665207$\\
$160$ & $0.387044$ & $0.5199082072$ & $0.5192522062$ & $0.5198837121$\\
\bottomrule
\end{tabular}
\end{center}
...
$200$ & $0.698846$ & $-1.853347748$ & $-1.840001848$\\
\bottomrule
\end{tabular}
\end{center}
...
$150$ & $157.29735$ & $0.2974$ & $0.52039793$ & $0.52098833$ & $-0.09240$\\
$175$ & $182.51185$ & $0.5119$ & $0.51801879$ & $0.51869969$ & $-0.12427$\\
$200$ & $207.69885$ & $0.6984$ & $0.51821775$ & $0.51875313$ & $-0.11120$\\
...
\texttt{EffectiveMonotone}\newline\texttt{Inverse.lean} &
