...
28 August 2026; formal-status audit updated 29 August 2026\\
...
and negative \(q\), with no support, density, or moment hypothesis.  The negative- and
zero-parameter density interpretation remains open.  The existing
\texttt{geometricSincProduct} packages the rescaled sinc factors as a \texttt{tprod}; a
named characteristic-function bridge to it (or explicit \texttt{HasProd} theorem) and
compact-uniform convergence beyond the formal pointwise sinc-prefix limit, general-\(q\) rapid decay and Fourier
inversion, and the further centered, shape, explicit Bernoulli-cumulant/Bell-moment,
transform, derivative-hierarchy, and endpoint-flatness layers remain in the frontier on the
precise boundary recorded below.
...
prefixes are formal, with exact dyadic wrappers; \texttt{geometricSincProduct} already
packages the same factors after frequency rescaling as a \texttt{tprod}.  What remains
here is the density interpretation at \(q=0\) and at negative \(q\), a named
characteristic-function bridge to \texttt{geometricSincProduct} (or explicit
\texttt{HasProd} theorem), compact-uniform convergence for the characteristic prefixes, rapid decay and Fourier
inversion, named centered packaging, a closed higher-derivative hierarchy and
endpoint-flatness wrappers, shape theorems, further transforms, and explicit
Bernoulli-cumulant/Bell-moment formulas and asymptotics.  Finite characteristic-function,
MGF, and CGF tail factorizations are already part of the geometric tail dictionary.
...
smooth-density API.  The explicit density's interpretation at zero and negative parameters
is also not supplied by that API.  The probability-law fixed point, by contrast, is now
...
The totalized inverse---written \(I\) in this source's audit tables and \(\InvF\) in the
...
The formal mechanism is now independent of the dyadic weights.  For weights \(w:\N\to E\)
write
\[
...
measurable, and let \(q\in\R\).  For a probability measure \(\lambda\) on \(E\), set
\begin{equation}
...
\end{equation}
Consequently, if \(\widehat\lambda\) denotes the characteristic function of \(\lambda\),
\begin{equation}
...
\begin{align}
 ...
 \widehat\lambda(t)
   &=\left(\prod_{k=0}^{n-1}\widehat{d_*\rho}(q^kt)\right)
      \widehat\lambda(q^nt) \qquad(n\in\N).
...
\[
 \left|\widehat\lambda(t)-\widehat\nu(t)\right|
 \le
 \left|\widehat\lambda(qt)-\widehat\nu(qt)\right|.
\]
Iterating sends \(t\) to \(q^nt\to0\).  Continuity at the origin and
\(\widehat\lambda(0)=\widehat\nu(0)=1\) make the right-hand side tend to zero, after which
...
Taking \(D=[0,1]\), \(\rho=\operatorname{Unif}[0,1]\), \(E=\R\), and
...
\(|q|<1\) gives, for every \(m\in\N\) and \(t\in\R\),
\begin{equation}
...
\texttt{Tendsto} theorem.  The existing \texttt{geometricSincProduct} packages the same
sinc factors, after frequency rescaling, as a \texttt{tprod}; a named
characteristic-function bridge to it (or explicit \texttt{HasProd} theorem) and uniform
convergence of the characteristic prefixes on compact frequency sets are not yet present.
...
\begin{align}
 F_q(x)&=\int_0^1F_q\!\left(\frac{x-(1-q)u}{q}\right)\dd u,
                                                                  \label{integration:eq:geometric-cdf-conditioning}\\
 &=\frac{q}{1-q}\int_{(x-(1-q))/q}^{x/q}F_q(t)\dd t.             \label{integration:eq:geometric-cdf-ordinary-integral}
\end{align}
...
\begin{equation}
 ...
 \qquad F_q,f_q\in C^\infty(\R).                            \label{integration:eq:geometric-density-with-density}
\end{equation}
...
\begin{equation}
 f_{1/2}(x)=2\Up(2x-1).                                    \label{integration:eq:geometric-density-half-bridge}
\end{equation}
...
Radon--Nikodym density only when \(0<q<1\); its density interpretation at \(q=0\) and at
negative \(q\) remains open in this interface.  The following report layers also remain in
the research quarantine:
\begin{itemize}
\item a named characteristic-function bridge to the existing
      \texttt{geometricSincProduct} \texttt{tprod} (or explicit \texttt{HasProd} theorem)
      and compact-uniform convergence beyond the formal pointwise phase-bearing sinc-prefix
      limit, quantitative rapid decay, and the
      Fourier-inversion construction of smooth densities;
\item separately named centered variables and densities and their affine transport package;
...
now supply the geometric sinc specialization.  They do not supply the compact-uniform or
Fourier estimates in the first item.
...
with an ordinary integral against $\Up$.  It has now been promoted in the
...
Then
\begin{equation}
 \int_{[0,1]}\left(\int_0^x k(t)\dd t\right)\dd\mu(x)
...
$K(x)=\int_0^xk(t)\dd t$,
\[
 \int_0^1k(t)\Up(t)\dd t=\E[K(X)].
\]
The choice $E=\C$, regarded as a real Banach space, gives the same identity
for complex kernels.
\end{theorem}
...
\[
 ...
 \qquad
 H(t,x)=\mathbf1_A(t,x)k(t).
\]
Integrability of $k$ and finiteness of the probability law make $H$
Bochner integrable.  Integrating first in $t$ gives
$\int_0^xk(t)\dd t$ for $x\in[0,1]$; integrating first in $x$ gives
$\mu((t,\infty))k(t)=\Up(t)k(t)$.  Bochner--Fubini equates the two
iterated integrals.
\end{proof}
...
Let \(E\) be a real Banach space and \(0\le c\le1\).  If \(k:\R\to E\) is
interval-integrable on \([0,c]\), then
\begin{equation}
 \int_0^c \Up(t)k(t)\dd t
 =\int_{\R}\left(\int_0^{\min\{z,c\}}k(t)\dd t\right)\dd\mu(z).
 \label{integration:eq:survival-layer-cake-partial}
\end{equation}
If \(k\) is interval-integrable on \([0,1]\), then
\begin{equation}
 \int_0^1 \Up(t)k(t)\dd t
 =\int_{\R}\left(\int_0^z k(t)\dd t\right)\dd\mu(z).
 \label{integration:eq:survival-layer-cake-full}
\end{equation}
No positivity hypothesis is imposed on \(k\); in particular \(E=\C\) gives the
complex-kernel form.
\end{theorem}
...
For a finite measure \(\nu\) on \(\R\), \(a\le b\), and a Banach-valued map \(k\)
...
For the Fabius law \(\mu\), every \(\alpha>0\), and every \(x\in\R\),
\begin{equation}
 I_{0+}^{\alpha}F(x)
 =\frac{1}{\Gamma(\alpha+1)}\int_{\R}(x-z)_+^\alpha\dd\mu(z)
...
\begin{equation}
 I_{0+}^{\alpha}F(1)
 =\frac{1}{\Gamma(\alpha+1)}\int_{\R}z^\alpha\dd\mu(z)
 =\frac{1}{\Gamma(\alpha+1)}
...
unit-interval $\Up$ area and with \(1/2\).  These eleven declarations exhaust the public
...
\begin{equation}
 I_{a+}^{\alpha}f(x)
 =\int_a^{\min(x,b)}
   \frac{(x-t)^{\alpha-1}}{\Gamma(\alpha)}f(t)\dd t.
...
\(U_\beta(x)=I_{(-1)+}^{\beta}\Up(x)\).  Its three causal declarations prove
\begin{align}
 U_\beta(x)
 &=\frac1{\Gamma(\beta)}
...
\begin{align}
 ...
 &=2^\alpha I_{0+}^{\alpha}\mathcal F(x/2), &&\alpha>0,\ x\ge0,
...
half-cell cutoff
\[
 \left\lfloor 2^p x-\frac12\right\rfloor+1
...
\(0\le q<1\), and, for \(0<q<1\), conditioning and interval-integral refinement,
...
full weighted inverse-area identity, equality with the $\Up$ area, and the inverse mean
...
objects and do not themselves evaluate $H_4$ or $\beta_4$; the downstream executable
tranche below does.  This module proves no root, Christoffel, quadrature,
...
genuine nonsingular Jacobi formula.  It proves no finite
Gaunt/Wigner/$3j$ entry formula, entry rationality by that route, Christoffel reconstruction,
or infinite product.  The shifted-atom kernel \lean{rvachevTranslateGram} is a distinct
...
\lean{legendrePolynomialCoeffRat} and the noncomputable convenience wrapper
...
\lean{coeff_legendrePolynomialRat_self_div_succ}.  The coefficient theorem identifies
the wrapper coefficientwise with the finite rational evaluator; the remaining results give
its real Legendre cast, exact degree, explicit nonzero leading coefficient, and consecutive
leading-coefficient quotient.\\
\path{FabiusLegendreRationalGram.lean} &
Exactly three public executable definitions,
...
\lean{rvachevLegendreGramDetRat_zero}, \lean{rvachevLegendreGramDetRat_pos},
\lean{rvachevOrthoNormRat_eq_rvachevLegendreGramDetRat_ratio}, and
\lean{rvachevJacobiSubdiagonalRat_eq_rvachevLegendreGramDetRat_ratio}.  The bounded
double moment sum closes entrywise rationality directly, and the cast theorems recover the
up-law Gram objects for every genuine Fabius representative.  Positivity makes the rational
norm and zero-based Jacobi formulas genuine determinant ratios.  The definitions provide an
exact evaluator route for the report's \(H_4\) and \(\beta_4\); the next module supplies
their named values.  Equality with the finite Gaunt/Wigner/\(3j\) entry formula,
...
No public definitions and exactly eleven public theorems:
\lean{moment_four},
...
\lean{hankelRatio_four_div_three}.  These exact finite native-computation certificates
give the raw eighth moment and the rational Legendre Gram determinants through order five.
The first eight identities are unconditional and, more explicitly, prove
\lean{moment_four} $=132809/32531625$ together with
\[
 \begin{aligned}
 D_1&=1,& D_2&=\frac19,& D_3&=\frac8{2025},\\
 D_4&=\frac{39616}{602791875},\\
 D_5&=\frac{16544275456}{27453718922765625}.
 \end{aligned}
\]
They also prove $H_4=26727424/55791736875$; the declaration
\lean{rvachevJacobiSubdiagonalRat_three} gives the zero-based value
\(\beta_4=835232/4640643\).
Exactly the three real/integral consequences require
$F:\lean{BoundedFabius}$ and $h_F:\lean{IsFabius}\,F$.  The finite
Hankel and integral forms identify $H_4$ with the real Hankel ratio and squared integral
and $\beta_4$ with the quotient of the fourth and third real Hankel ratios.  Together with
the preceding four finite-algebra modules, these three downstream modules bring the
complete seven-module inventory to thirteen definitions and sixty-one theorems, hence
seventy-four public declarations.  This is not a general native evaluator; the finite
Gaunt/Wigner/$3j$ entry expansion, roots, Christoffel reconstruction, quadrature,
continued-fraction/Pad\'e theory, and asymptotics remain outside this tranche.\\
...
single-step order shifts for the signed and bounded Fabius functions and \(\Up\).  These
...
$\Up$, the signed extension, or the total inverse, including the enlarged admissible
...
\texttt{geometricSincProduct} packages the rescaled sinc factors as a \texttt{tprod}.  The
zero/negative parameter density interpretation, a named characteristic-function bridge to
that product (or explicit \texttt{HasProd} theorem), compact-uniform convergence,
general-\(q\) rapid decay and Fourier inversion, centered package, closed higher-derivative
and endpoint-flatness wrappers, shape, further transforms, and explicit
Bernoulli-cumulant/Bell-moment formulas and asymptotics remain outside the formal boundary.
In contrast, the geometric finite residual sinc formula and pointwise prefix limit, generic
...
Legendre series for $A_2=\int_0^1F(t)^2\dd t$.
...
$\prod_{j\ge0}\sinc^2(\pi\xi/2^j)$ formula and its $1/(2\pi)$-scaled sine-quotient formula
indexed from one.

At compiled source checkpoint \texttt{a3854643d}, the Rvachev moment--Appell concordance is
...
\lean{integral_eval_rvachevDeconvolvedPolynomial_add_mul_rvachev}.  They package the full
...
underlying operation.  Rvachev smoothing recovers the original polynomial.  This reciprocal-moment
...
\path{RvachevPolynomialSynthesis.lean} exports no public definitions and exactly four public
...
\lean{normalized_sum_Ioo_rvachevDeconvolvedPolynomial_mul_shifted_rvachevUp}.  For every
nonzero natural mesh $M$ and polynomial of degree at most $v_2(M)$, these give the global
...
degree $d$ holds for a nonzero natural mesh $M$ exactly when $d\le v_2(M)$, equivalently
when $2^d\mid M$.  The monomial of degree $v_2(M)+1$ fails at some real shift, $2^d$ is
...
$Q_d=\mathcal D_{\Up}P_d$, the first two prove exact natural degree $d$ and the explicit unchanged
...
$C_N=\mathcal D_{\Up}S_N$ with the finite sum of deconvolved modes, prove that $C_N$ has the
...
The bundled common-mesh partial trains converge to $\Up$ in $C([-1,1])$, hence in the
...
These five modules have public definition/theorem inventories $6/30$, $0/4$, $1/7$,
$6/7$, and $5/25$, respectively, for exactly $91$ public declarations.  Universal
...
Let $m\in\N$ and $u,a,z\in\C$, and write
$\varepsilon_n=(-1)^{\wt(n)}$.  Then
\begin{align}
 \sum_{n=0}^{2^m-1}u^{\wt(n)}\exp(nz)
 &=\prod_{j=0}^{m-1}\left(1+u\exp(2^jz)\right),
 \label{repeated2:eq:weighted-complex-block}\\
 \sum_{n=0}^{2^m-1}u^{\wt(n)}\exp(a+nz)
...
\[
 K^{-1}_{q,s}(n,k)
 =(-s)^{n-k}q^{\binom{n-k}{2}}\qbinom{n}{k}{q}.
\]
...
commutative monoid carrying a $\Q$-module structure.  It needs no target
...
The theorem \lean{discreteLimitWeight_qRichardson_exact} is the $M=\Q$
...
\([z^n](B_mA)\) at \(z=r(1+\ii\theta)\) gives the normalized mass
\begin{equation}
 \mathcal C_{m,n}
 =\sqrt{\frac{\lambda}{2\pi}}
 \int_{-\infty}^{\infty}
 \frac{B_m(r(1+\ii\theta))A(r(1+\ii\theta))}
      {B_m(r)A(r)}
...
$\beta_h=h^{-1}\mathbf 1_{[-h/2,h/2]}$ and define the mass-preserving dilation
\[
 (D_h\Up)(x)=h^{-1}\Up(x/h).
\]
Put \(q_n=\beta_{h_n}^{*n}*D_{h_n}\Up\), and rename the cardinal weights
\begin{equation}
...
\begin{proof}
Replace \(A_n(x)=H_n(x-1+\delta_n(x))\) by \(\Up\) using
...
synthesis otherwise writes that variable as \(u\) and defines \(D=\dd/\dd u\).  In this
final part, \(d_n\) is a bounded saddle jet and is not the half-moment sequence
\(d_n=\E X^n\) used in the probability and dyadic parts.  When numerical work later returns
...
\path{not_eqOn_fabiusInv_of_dense_analyticLocus} excludes agreement with \(\InvF\) on any
...
\(\langle H\rangle=\int_0^1H(u)\,\dd u\).  Fourier multiplication gives the exact identity
\begin{equation}
 \bigl\langle\Psi^{(a)}\Psi^{(b)}\bigr\rangle
 =\sum_{k\ne0}(2\pi\ii k)^a(-2\pi\ii k)^b
   |\widehat\Psi(k)|^2.
...
\[
\begin{aligned}
  \widehat\Psi(1)&=(7.55864754098-7.47010355898\,\ii)\,10^{-7},
    &|\widehat\Psi(1)|&=1.06271162519\,10^{-6},\\
  \widehat\Psi(2)&=(3.24812347723+5.85175898712\,\ii)\,10^{-13},
    &|\widehat\Psi(2)|&=6.69278636792\,10^{-13},\\
  \widehat\Psi(3)&=(-2.81236523199-2.58674397916\,\ii)\,10^{-19},
    &|\widehat\Psi(3)|&=3.82107872359\,10^{-19}.
\end{aligned}
\]
...
Here $c_j(k)$ is the polynomial in $w=2\pi\ii k$ read off from
...
\(D(e,a)=\wt(a)\) for every \(a<2^e\), the endpoint \(a=0\) included, since each interior
...
Let \(n\in\N\), put \(s=2^n\), and for \(a\in\N\) write
\[
...
\[
 ...
 =(-1)^{\wt(B)}F\!\left(\frac C{2^n}\right).
\]
\end{candidate}
\begin{obligation}[Discharged]
...
\(\mathcal F(a/2^n)=(-1)^{\wt(B)}F(C/2^n)\), for every bounded Fabius function, is
...
\(\Up(a/2^n)\) over odd support numerators \(a\).  Lean proves that the reduced denominator
...
and the signed global extension.  For \(\Up\) it once had only an already decoded
exponent--integer-numerator evaluator; the matching wrapper now exists.
\begin{candidate}[Discharged]
Define an exact rational-input evaluator for \(\Up\) that returns ``not dyadic'' exactly when
...
the rational value and its identification with analytic \(\Up\) for every dyadic rational
...
\(\N\to\Z\to\Q\) evaluator, so the same lemma also yields
...
The identity
\[
 S_{p+1}(x)\stackrel{?}{=}\int_0^1S_p(2x-u)\,\dd u
\]
...
\[
 ...
 \sum_{r\ge0}(-1)^{\wt(r)}\left(2^px-\tfrac12-r\right)_+^{p},
\]
...
\(\{r<\lceil 2^px\rceil\}\).  The proof needs no fixed-range detour: left of the support both
...
\begin{candidate}[Discharged]
For every \(p\in\N\) and \(x\in\R\), the respective sandwich-and-Lipschitz arguments certify
\[
 |U_p(x)-F(x)|\le 2^{1-p},
 \qquad
 |C_p(x)-F(x)|\le 2^{-p}.
...
each stated for every \(p\in\N\) and every real \(x\), and each deduced there from the
...
\(\Up\), not the continuous uniform-coordinate partial laws whose centered CDF is \(C_p\).
\end{obligation}

\subsection{Local finiteness of the signed translate family}

For fixed \(x\in\R\), consider
\[
 n\longmapsto(-1)^{\wt(n)}\Up(x-2n-1).
\]
\begin{candidate}[Discharged]
Its support in \(\N\) is finite; equivalently, only finitely many translates in the defining
series for \(\mathcal F(x)\) are nonzero.
\end{candidate}
...
no \(p\ge1\) hypothesis, holding for every \(p\in\N\) and every real \(x\), with the
...
Mathlib's coefficientwise topology on \(\Z[[X]]\)
