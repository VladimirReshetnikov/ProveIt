...
\lean{iteratedDeriv_rvachevUp_dyadic_below}.  The finalized union also contains
the exhaustive 2+12 \path{TwoPhiOneReversal.lean}, 0+10
\path{QChuVandermonde.lean}, and 3+7
\path{GeometricRichardsonGenerating.lean} surfaces while retaining the public
\lean{complexQPochhammerInf_eq_qPochhammerInfIn} bridge.  The live source
census is exactly 902 facade-reachable modules and 11,441 unique public
declarations, with no missing module headers or declaration documentation.  The final-source
publication is validated by exactly three successful serial halt-on-error
passes and the repository's PDF gates.  Exact page-count, byte-count, and
SHA-256 receipts for the frozen TeX and resulting PDF are kept externally in
the repository documentation so that neither artifact embeds a
...
902/11,440.  Retaining the unconditional public
\lean{complexQPochhammerInf_eq_qPochhammerInfIn} bridge yields the final live
union 902/11,441 with both documentation-gap counts zero.  These additions
are included in the finalized canonical-frontier publication; exact
frozen-source and final-PDF receipts are kept externally in the repository
documentation so that no artifact embeds a mutable self-receipt.
