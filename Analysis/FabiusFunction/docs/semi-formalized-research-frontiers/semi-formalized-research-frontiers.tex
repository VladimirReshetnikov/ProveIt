...
The finalized publication contains the 31 August 2026 Legendre
...
0+7, 4+16, and 0+9), its subsequent zero-definition/five-theorem real and
complex Bernoulli branch-gap companion crosswalking
\nolinkurl{eq:pair-Bernoulli-general} and, under the canonical removable-origin
convention, \nolinkurl{eq:bernoulli-gen}.  Those four Lambert modules total four
definitions and 37 theorems, 41 declarations.  The new
\lean{hasSum_bernoulli_mul_pow_div_factorial_complex_iff} theorem gives the
complex \(\operatorname{HasSum}\) value
\(\lean{complexExpm1Div}(z)^{-1}\) exactly for \(\lVert z\rVert<2\pi\),
including value one at \(z=0\).  This is the canonical removable
representation, not an equality with Lean's literal totalized quotient at
zero, and it asserts no holomorphy; higher Puiseux claims remain open.  The publication
also contains the completed zero-definition/six-theorem dyadic derivative
filtration.  The first three Lambert surfaces contain 36 declarations, 13 of
them supplied by the earlier upstream delta; the filtration delta is exactly
\lean{iteratedDeriv_rvachevUp_eq_extendedFabius} and
\lean{iteratedDeriv_rvachevUp_dyadic_below}.  The union also contains the
exhaustive 2+12 \path{TwoPhiOneReversal.lean}, 0+10
\path{QChuVandermonde.lean}, and 3+7
\path{GeometricRichardsonGenerating.lean} surfaces, and the three-theorem
second-moment extension that brings \path{GaussianBinomialCumulants.lean} to
2+24, followed by the one-definition/eight-theorem
\path{GeometricUniformMomentPolynomial.lean} algebraic surface.  The latter
supplies the recursive rational polynomial, residual-product recurrence,
triangular degree bound, reciprocal-factorial value at zero, and
\(P_0,\ldots,P_4\), but not the analytic normalization or the leading-coefficient
and strict odd-degree conclusions; \nolinkurl{p7:thm:Pn} therefore remains
Partial.  The origin-side source census is 904 facade-reachable modules and
11,457 public declarations; retaining the unconditional public
\lean{complexQPochhammerInf_eq_qPochhammerInfIn} bridge gives the merged live
census of exactly 904 modules and 11,458 unique public declarations, with no
missing module headers or declaration documentation.  The companion PDF is a
synchronized render of this source.
...
\nolinkurl{QLucas.lean}; zero definitions and seven theorems \\
...
The identically named \lean{two_mul_choose_two} helper is private here; its
public owner is \path{QChuVandermonde.lean}.
...
union includes the exact current Lambert surfaces
\path{LambertWBranchPairing.lean} (0+7),
\path{LambertWGapBijection.lean} (4+16), and
\path{LambertWBranchSymmetry.lean} (0+9), with 13 declarations supplied by
the earlier upstream delta.  It also adds exactly
\lean{iteratedDeriv_rvachevUp_eq_extendedFabius} and
\lean{iteratedDeriv_rvachevUp_dyadic_below} to the now 0+6
\path{DyadicDerivativeFiltration.lean} surface, and contains the exhaustive
2+12 \path{TwoPhiOneReversal.lean} and 0+10
\path{QChuVandermonde.lean} surfaces.  Those additions and the later
terminating-basic-hypergeometric closure bring the preceding origin-side
checkpoint to 901/11,430.  The 3+7
\path{GeometricRichardsonGenerating.lean} leaf raised that census to
902/11,440.  Three later theorems expand the existing 2+21
\path{GaussianBinomialCumulants.lean} surface to 2+24 and bring the origin-side
census to 902/11,443.  They are
\lean{eval_one_derivative_derivative_gaussianBinomial_X},
\lean{twelve_mul_secondMoment_gaussianBinomial_eval_one}, and
\lean{twelve_mul_varianceNumerator_gaussianBinomial_eval_one}; their exact
hypotheses and totality boundaries are recorded below.  The subsequent
zero-definition/four-theorem
\path{LambertWBranchGapBernoulli.lean} tranche brings the exact-radius origin
checkpoint to 903/11,447.  Its fifth theorem,
\lean{hasSum_bernoulli_mul_pow_div_factorial_complex_iff}, makes the exhaustive
leaf 0+5 and raises the origin-side historical checkpoint to 903/11,448.  It crosswalks
\nolinkurl{eq:pair-Bernoulli-general} and, under the canonical removable-origin
convention, \nolinkurl{eq:bernoulli-gen}.  Its
\lean{summable_bernoulli_mul_pow_div_factorial_iff} theorem proves that the
complex series is summable exactly for \(\lVert z\rVert<2\pi\), hence diverges
on and outside the radius.  The fifth theorem gives the actual complex
...
representation, not the literal Lean quotient at zero, and it asserts no
holomorphy.  The real value theorem still assumes \(z\ne0\); remainders and
higher Puiseux claims remain open.  The subsequent one-definition/eight-theorem
\path{GeometricUniformMomentPolynomial.lean} leaf raises the origin-side live
census to 904/11,457.  It supplies the recursively defined rational polynomial,
...
not prove the analytic normalization identifying that family with the source
moment coefficients, nor the leading-coefficient formula and consequent
strict odd-degree drop, so \nolinkurl{p7:thm:Pn} remains Partial.  Retaining
the unconditional public
\lean{complexQPochhammerInf_eq_qPochhammerInfIn} bridge yields the final merged
live union 904/11,458 with both documentation-gap counts zero.  The companion
PDF is synchronized with the current TeX.  Exact source and PDF receipts are kept externally
so that no artifact embeds a mutable self-receipt.
...
Partial, 17 None, and 8 interface rows.  The evaluated q-Lucas theorem remains
Partial because its polynomial-congruence lift is absent; the Gaussian
majorant is Exact; and the \(Q>1\) asymptotic row remains Partial.  The
...
\path{QLucas.lean} &
The exhaustive zero-definition, seven-theorem natural quadratic, block-coefficient, phase, and q-Lucas interface.\\
...
at zero, and calculate \(P_0,\ldots,P_4\).  They do not identify the family
with the normalized analytic moment coefficients in
\nolinkurl{p7:eq:Pn-def}, and they prove neither the leading-coefficient
formula nor the strict degree drop for odd \(n>1\).  Therefore the compound
frontier label \nolinkurl{p7:thm:Pn} is Partial, not Exact.
