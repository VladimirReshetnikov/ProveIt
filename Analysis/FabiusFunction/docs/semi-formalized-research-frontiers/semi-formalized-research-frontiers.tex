...
\(C^\infty\) regularity.  The affine measure fixed point is also unique for every
\(|q|<1\), including negative \(q\) and \(q=0\), by
\nolinkurl{eq_geometricUniformDistribution_of_selfSimilar}.  The generic and geometric
Cauchy rows above additionally make the transform DDE and adjacent-power hierarchy direct
named theorems, including negative nonzero ratios.  What remains here is the density
interpretation at \(q=0\) and at negative \(q\), the general-\(q\)
characteristic-function product, rapid decay and Fourier inversion route, named centered
packaging, a closed density higher-derivative hierarchy and endpoint-flatness wrappers,
shape theorems, and general-\(q\) moments/cumulants.  Here ``closed density hierarchy''
means the report's real-space derivatives and endpoint wrappers; it does not deny the
proved Cauchy-power recurrence.
\end{boxedremark}
...
\(q\) characteristic-function/Fourier, centered-density, higher-density-derivative,
endpoint-flatness, shape, and moment/cumulant layers beyond the formalized CDF and
positive-ratio smooth-density API.  Fixed-point uniqueness and the oriented Cauchy-transform
DDE/adjacent-power hierarchy are now formal; the latter should not be confused with the
still-open Fourier transform.  The explicit density's interpretation at zero and negative
parameters is also not supplied by that API.  The inverse
...
\item characteristic-function products, Bernoulli cumulants, Bell moments, and related
      asymptotic or Gaussian-limit statements.
\end{itemize}
The affine measure fixed point is unique by the direct theorem
\lean{eq_geometricUniformDistribution_of_selfSimilar}.  The transform DDE and
adjacent Cauchy-power hierarchy are also direct named results in
\path{GeometricUniformCauchy.lean}; the transform bullet above refers only to the report's
still-open Fourier/Laplace and moment formulas.
\end{boxedremark}
...
regularity.  Series, law, CDF, density, and all-power half-base bridges are formal; so are
affine fixed-point uniqueness and the transform DDE/adjacent-power hierarchy for every
nonzero \(|q|<1\).  The zero/negative-parameter density interpretation, general-\(q\)
Fourier route, centered density package, closed real-space derivative and endpoint-flatness
wrappers, shape, and general-\(q\) moment/cumulant formulas remain outside the formal
boundary.\\
