...
925 facade-reachable modules and 11,622 public declarations, with no missing
...
give 924/11,617, and the Legendre biorthogonality leaf gives the incoming-branch
925/11,619 checkpoint.  Merged reconciliation retains all five local
fixed-column declarations in \path{QBinomialTheoremInfinite.lean} and removes
the two shared declarations from the incoming rate leaf, leaving that leaf at
0+8 and adding three unique declarations overall.  This gives the authoritative
925/11,622 union.  The incoming-branch
...
the \path{RvachevLegendreBiorthogonality.lean} (1+1) leaf gives the
incoming-branch 925/11,619 checkpoint.  Merged reconciliation keeps the five
local fixed-column declarations in the 1+27
\path{QBinomialTheoremInfinite.lean} surface and deduplicates the generalized
product bound and arbitrary-shift convergence from the incoming rate leaf,
leaving \path{GaussianBinomialFixedColumnRate.lean} at 0+8.  The three net new
declarations give the authoritative 925/11,622 union, with every module
facade-reachable and no documentation gaps.
...
biorthogonality leaf gives the incoming-branch 925/11,619 census; the
fixed-column reconciliation described above gives the final 925/11,622
census.
The existing
...
257-page receipt records only the pre-merge rendering.
