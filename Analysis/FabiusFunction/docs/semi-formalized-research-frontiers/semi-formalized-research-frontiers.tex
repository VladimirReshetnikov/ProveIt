...
\begin{boxedremark}
\textbf{Artifact status (3 September 2026).}
...
0+7, 4+16, and 0+9), and the completed zero-definition/six-theorem dyadic
derivative filtration.  The Lambert surfaces contain 36 current declarations,
13 of them supplied by the upstream delta; the filtration delta is exactly
...
2+24.  The origin-side source census is 902 facade-reachable modules and
...
census of exactly 902 modules and 11,444 unique public declarations, with no
missing module headers or declaration documentation.  The retained PDF
predates the Gaussian-cumulant source update and remains an explicitly
...
and no Bernoulli-gap series, convergence theorem, or higher Puiseux/logarithmic
...
the upstream delta.  It also preserves the unconditional public Pochhammer
bridge and adds exactly
\lean{iteratedDeriv_rvachevUp_eq_extendedFabius} and
\lean{iteratedDeriv_rvachevUp_dyadic_below} to the now 0+6
\path{DyadicDerivativeFiltration.lean} surface.  It also contains the
exhaustive 2+12 \path{TwoPhiOneReversal.lean} and 0+10
\path{QChuVandermonde.lean} surfaces.  The origin-side checkpoint before
Richardson was recorded as 901/11,430; the 3+7
\path{GeometricRichardsonGenerating.lean} leaf raised that origin census to
...
census to 902/11,443.  Retaining the unconditional public
\lean{complexQPochhammerInf_eq_qPochhammerInfIn} bridge yields the final merged
live union 902/11,444 with both documentation-gap counts zero.  The three new
...
hypotheses and totality boundaries are recorded below.  The retained PDF
remains explicitly historical until rebuild and does not claim parity with
