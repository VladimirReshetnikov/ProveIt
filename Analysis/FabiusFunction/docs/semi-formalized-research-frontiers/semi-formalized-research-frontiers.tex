...
\begin{boxedremark}
\textbf{Source/PDF parity convention.}
This source contains the Legendre Gaunt--Wigner-square closed-form overlay,
the generic and compatibility q-Pochhammer layers, the complete
Pochhammer-limit, normal-convergence, universal-polynomial, q-Taylor,
partial-fraction, integer-index, generalized-identifiability, q-calculus,
theta, derivative, Gaussian-polynomial, central-reduction, cyclotomic,
primitive-root, q-Lucas, q-Catalan, finite-Newton, and q-beta overlays, and
the final integer/complex Gaussian, q-Pfaff--Saalsch\"utz,
quantum-multinomial, Gaussian-bound, and Rvachev-superconvergent surfaces
described below.  The live facade contains exactly 671 source modules and
8,859 public declarations, with no documentation gaps; the rigorous
282-row q-series ledger is 90 Exact, 85 Partial, 99 None, and 8 interface
rows.  The retained 253-page A4 PDF, SHA-256
\nolinkurl{cd2a099e1f517d17ca345a52cdac5753899364652223ed8b7802c52d38c2a299},
is the validated pre-union receipt for the 659-module/8,769-declaration
source.  It is historical and rebuild-pending; its page count and digest do
not certify source/PDF parity for the present TeX file.
\end{boxedremark}
...
report's Thue--Morse corner theorem, but not its repeated-integral equality
under \(C^N\) hypotheses; the following Walsh conditional-expectation
corollary also remains outside this module. &
...
\nolinkurl{GaussianBinomialPalindromic.lean}; zero definitions and twelve theorems \\
...
implication; it does not derive that modulus from the paper's gap sequence. &
...
the series range and the support are exactly \([0,1]\).

The twenty-three declarations are
...
253-page pre-union canonical PDF remains a historical render and does not certify
source/PDF parity until a fresh rebuild is performed.  The generic-name
analytic-order theorem is owned by \path{QPochhammerInfinite.lean}.
...
mean identity.  Exact degree and monicity require only nontriviality.  With
existing row symmetry and reciprocity, this keeps the canonical forward
q-binomial structure theorem exact.  It does not prove that the coefficient
of \(q\) is one.
...
this closes the canonical forward q-binomial structure theorem.  It does not
prove that the coefficient of \(q\) is one.
...
that the historical, rebuild-pending PDF has parity with the merged source.
...
six-declaration overlay is part of the live 671-module, 8,859-declaration
source union.  The retained 253-page pre-union PDF is explicitly historical and does
not certify parity with this source union.

\paragraph{Exact seven-module q-Pochhammer closure overlay.}
Together with the preceding zero-definition/six-theorem identifiability leaf,
the following six modules exhaust the cumulative closure API.

\path{GeometricPochhammerNormalConvergence.lean} has no definitions and exactly
three theorems:
\lean{hasProdLocallyUniformly_geometricSincProduct_complexQPochhammerInf},
\lean{hasProdLocallyUniformly_rvachevFourierProduct_complexQPochhammerInf}, and
\lean{hasProdLocallyUniformly_rvachevFourier_complexQPochhammerInf}.
The first assumes exactly a complex strict contraction
\(\lVert q\rVert<1\) and proves locally uniform convergence on all of
\(\ComplexNumbers\) of the outer factors
\((z^2/(k+1)^2;q^2)_\infty\), including \(q=0\).  The second is the
unconditional dyadic nome-\(1/4\) specialization; the third assumes exactly a
bounded Fabius witness and \lean{IsFabius}.  There is no joint normality in
\((q,z)\), boundary theorem at \(\lVert q\rVert=1\), or outside-disk
continuation.

\path{ClassicalPochhammerLimit.lean} has no definitions and exactly five
theorems: \lean{ascPochhammer_eval_eq_prod_range},
\lean{tendsto_one_sub_div_one_sub_of_hasDerivAt},
\lean{tendsto_finiteQPochhammerIn_div_pow_of_hasDerivAt},
\lean{tendsto_finiteQPochhammerIn_cpow_div_pow}, and
\lean{tendsto_finiteQPochhammerIn_rpow_div_pow}.  The rising-product identity
holds over every commutative semiring.  Over a nontrivially normed field,
\lean{HasDerivAt} \(f\ c\ 1\) and \(f(1)=1\) give the punctured
\(q\to1\) finite-product limit for every natural order; the last two results
specialize it to complex principal powers and real powers.  No rate,
uniformity in order or exponent, infinite-product limit, or classical-gamma
limit is asserted.

\path{GaussianBinomialUniversal.lean} has no definitions and exactly two
theorems,
\texttt{gaussianBinomial\_\allowbreak{}eq\_\allowbreak{}eval\textsubscript{2}\_\allowbreak{}universal}
and
\lean{gaussianBinomial_eq_eval_map_universal}.  Over every commutative
semiring and for arbitrary \(q,n,k\), they identify the Gaussian coefficient
with evaluation of its universal \(\NaturalNumbers[X]\) polynomial, including
out-of-range columns.  They add no degree, coefficient formula, unimodality,
log-concavity, or cyclotomic-factorization theorem.

\path{PolynomialQTaylor.lean} has the two definitions \lean{qFactorial} and
\lean{qFallingPower} and exactly eighteen theorems:
\lean{qFactorial_zero}, \lean{qFactorial_succ}, \lean{qDerivative_one},
\lean{qDerivative_iterate_add}, \lean{qDerivative_iterate_C_mul},
\lean{qDerivative_iterate_sum},
\lean{qDerivative_iterate_C_mul_X_pow},
\lean{qDerivative_iterate_eq_zero_of_natDegree_le},
\lean{qFallingPower_zero}, \lean{qFallingPower_succ},
\lean{qFallingPower_succ'}, \lean{qFallingPower_monic},
\lean{qFallingPower_natDegree}, \lean{qFallingPower_eval_self},
\lean{qDerivative_qFallingPower_succ},
\lean{qDerivative_iterate_qFallingPower},
\lean{prod_qInt_sub_eq_qFactorial}, and \lean{qTaylor}.  The factorial and
iterated-derivative algebra is semiring-level; falling powers and their
Jackson derivative use a commutative ring, and exact degree also requires
nontriviality.  The field-valued Taylor theorem assumes exactly
\([j]_q\ne0\) for \(1\le j\le N\) and natural degree at most \(N\).
It is a finite polynomial identity, not an analytic expansion, convergence
theorem, remainder estimate, or \(q\to1\) result.

\path{QPartialFractions.lean} has the definition
\lean{partialFractionCoeff} and exactly five theorems:
\lean{prod_erase_one_sub_inv_pow_mul_pow},
\lean{finiteQPochhammerIn_self_ne_zero_of_le},
\lean{partialFractionCoeff_mul_prod_eq_one},
\lean{sum_partialFractionCoeff_mul_prod_erase}, and
\lean{one_div_finiteQPochhammerIn_eq_sum}.  Over a field,
\((q;q)_n\ne0\) gives the division-free interpolation identity and includes
\(q=0\); the pointwise reciprocal formula additionally assumes that every
displayed pole factor is nonzero.  The erased-factor lemmas state their
nonzero-nome and index bounds explicitly.  No infinite expansion,
repeated-root case, residue theorem, or convergence claim is present.

\path{QPochhammerIntegerIndex.lean} has the two definitions
\lean{qIntervalProd} and \lean{finiteQPochhammerZ} and exactly fifteen
theorems: \lean{Ico_int_eq_image_range}, \lean{prod_Ico_int_eq_prod_range},
\lean{qIntervalProd_of_le}, \lean{qIntervalProd_of_lt},
\lean{qIntervalProd_self}, \lean{qIntervalProd_symm},
\lean{qIntervalProd_trans_of_le}, \lean{qIntervalProd_trans},
\lean{finiteQPochhammerZ_natCast}, \lean{finiteQPochhammerZ_neg_natCast},
\lean{prod_Ico_add_zpow}, \lean{qIntervalProd_add_eq},
\lean{finiteQPochhammerZ_add}, \lean{finiteQPochhammerZ_add_one}, and
\lean{finiteQPochhammerZ_neg_natCast_eq}.  Over a field, the interval product
is forward on increasing integer intervals and the inverse reversed product
on decreasing ones.  The ordered cocycle is unconditional; the all-order
cocycle assumes its two factors nonzero.  Translation and concatenation
require \(q\ne0\), with the additional displayed factor hypotheses; the
closed negative-index formula also assumes \(a\ne0\).  No infinite-index
limit, meromorphic continuation in the index, or root-of-unity regularization
is asserted.

The modified companion \path{PrimePowerBinomialValuation.lean} has no
definitions and exactly six theorems:
\lean{primePowerChoose_padicValNat_add},
\lean{primePowerChoose_padicValNat},
\lean{primePowerSubOneChoose_padicValNat},
\lean{primePowerSubTwoChoose_padicValNat},
\lean{twoPowChoose_padicValNat}, and
\lean{twoPowSubTwoChoose_padicValNat}.  For every prime \(p\), the first two
give the additive and natural-subtraction row-\(p^m\) identities under
\(j\le p^m\) and \(j\ne0\), including \(j=p^m\) and \(m=0\).  Every column
\(j<p^m\) in row \(p^m-1\) is a \(p\)-adic unit, and under exactly
\(0<j<p^m\) the row-\(p^m-2\), column-\(j-1\) valuation is \(v_p(j)\); the
last two theorems are the strict dyadic specializations.  No valuation-
histogram count is asserted.

The next six q-calculus module counts are
...
\(z=-q^m\), \(m\in\IntegerNumbers\), under the strict nome contraction.
\path{JacobiCubic.lean} has no definitions and exactly
\lean{two_mul_add_one_le_three_pow} and \lean{hasSum_jacobi_cubic}, the latter
under complex \(\lVert q\rVert<1\).
...
\path{CentralQBinomialReduction.lean} has no definitions and exactly six
theorems: \lean{finiteQPochhammerIn_mul_neg},
...
commutative-ring identities and make the central Gaussian reduction without
division.  The quotient wrapper is over a field and assumes exactly that the
two displayed finite q-Pochhammer denominators are nonzero.

\path{CyclotomicFactorization.lean} has no definitions and exactly seven
theorems: \lean{div_add_div_le_div},
...
\lean{gaussianBinomial_X_eq_prod_cyclotomic}.  The two floor inequalities put
every Gaussian cyclotomic exponent in \(\{0,1\}\).  The shifted-factorial
...
domain, and all Gaussian statements retain \(k\le n\).

The following six facade-reachable modules add exactly four definitions and
forty-six theorems.  \path{PrimitiveRootBlock.lean} has no definitions and the
three theorems \lean{gaussianBinomial_isPrimitiveRoot_eq_zero},
\lean{neg_one_pow_mul_pow_choose_two}, and
\lean{finiteQPochhammerIn_isPrimitiveRoot}.  They work over a commutative
integral domain at a primitive \(d\)-th root: interior vanishing assumes
exactly \(0<k<d\), and the phase and full-block formulas explicitly assume
\(0<d\).

\path{QLucas.lean} has no definitions and the eight theorems
...
identities and the two coefficient formulas need only a commutative ring.
The block, phase, and q-Lucas statements use an integral domain, \(0<d\), and
a primitive root; q-Lucas also assumes \(b,s<d\).  Lean proves the evaluated
primitive-root identity, not a named polynomial congruence modulo \(\Phi_d\),
so the manuscript q-Lucas row is Partial.

\path{CyclotomicDivisibility.lean} has no definitions and the three theorems
\lean{cyclotomic_exponent_eq_one_iff},
\lean{cyclotomic_dvd_gaussianBinomial_iff}, and
\lean{gaussianBinomial_mul_isPrimitiveRoot}.  Carry and divisibility assume
\(k\le n\) and \(0<d\), with divisibility specifically in
\(\RationalNumbers[X]\).  The multiple-index value works over a commutative
integral domain and assumes \(0<n\).  Together with the earlier exponent bound
these declarations prove the squarefreeness row, but there is no separately
named squarefree theorem.  The Babbage-derivative row remains Partial because
only its value is formalized.

\path{QCatalan.lean} has the definition \lean{qCatalan} and the eleven
theorems \lean{map_qInt}, \lean{qInt_X_monic}, \lean{qInt_X_natDegree},
...
\lean{qCatalan_eval_one_mul}, and \lean{qCatalan_eval_one}.  Functoriality is
semiring-level; monicity and degree use a nontrivial commutative ring.  The
noncomputable \(\IntegerNumbers[X]\) quotient exists for every \(n\), including
zero, has degree \(n(n-1)\), and evaluates at one to the Catalan number.  No
coefficient-nonnegativity or unimodality theorem is asserted.

\path{NewtonInterpolation.lean} has the definitions \lean{newtonCoeff} and
\lean{newtonInterpolant} and the thirteen theorems
...
\lean{newtonCoeff_pow_eq_sum}.  This is finite interpolation over a field.
Evaluation needs the relevant earlier-node difference product nonzero;
uniqueness and divided differences use finite-range injectivity.  The
geometric basis assumes \(q\ne0\), and its coefficient sum assumes injectivity
of \(j\mapsto q^j\).  The name \lean{newtonInterpolant} preserves the established
scalar-sequence \lean{newtonPoly} API.  There is no topology or convergence
claim.

\path{QBetaIntegral.lean} has the total real definition \lean{qBeta} and the
eight theorems \lean{qNumber_pos}, \lean{qBeta_term_eq},
\lean{qBeta_eq_prod}, \lean{qBeta_eq_qGamma}, \lean{qBeta_comm},
\lean{qBeta_pos}, \lean{qBeta_add_one_left}, and
\lean{qBeta_add_one_right}.  Evaluation, the q-gamma identity, symmetry,
positivity, and both recurrences assume \(0<q<1\) and \(x,y>0\); term
cancellation assumes \(y>0\) and arbitrary real \(x\).  Complex continuation
and the classical limit remain outside Lean.

The 282-row forward ledger is therefore 90 Exact, 85 Partial, 99 None, and
8 N/A.  Seven rows move from None to Exact---the primitive-root block,
squarefreeness, q-Catalan, q-beta evaluation and recurrence, geometric Newton,
and its triangular corollary---while q-Lucas and the Babbage derivative move
from None to Partial at the boundaries just stated.  The evaluated q-Lucas
identity remains Partial because no Lean declaration supplies the
minimal-polynomial lift to the manuscript congruence modulo \(\Phi_d\).
The independent 547-row source concordance is unchanged: 73 Lean-proved,
...
\path{NewtonInterpolation.lean} (2+13).  Their three definitions and
twenty-one theorems evaluate the Jackson q-beta integral as both an
...
uniqueness, divided differences, and the explicit geometric-grid formula.
No endpoint or \(q\to1\) limit theorem is included.
...
established \lean{GeneralQConditionNumber.finiteQPochhammerIn_self_pos}
theorem rather than redeclaring it.  No asymptotic error term is proved.
Consequently the growth, uniform-bound, and extreme-specialization rows are
Exact, while the \(q>1\) transfer row remains Partial because only its
reciprocity clause is formalized.
...
\lean{integral_unit_eq_zero_add_integral_deriv_mul_rvachevUp}, all in
...
The compatibility head--tail, probability, scalar-law, and unit-interval surface, followed by
the four-declaration s-finite uniform-smoothing, sharp weighted-law absolute-continuity, and
...
The twenty-three-declaration named geometric weight, series, and law API: summability and
mass, continuity and measurability, affine splitting, reflection, absolute continuity, null
singletons, and support equal to the series range for \(|q|<1\), with exact support \([0,1]\)
for \(0\le q<1\);
\cref{integration:sec:weighted-naturality}.\\
...
The zero-definition, three-theorem interface for topological-semiring
continuity and the classical \(q\to1\) limit, plus the field quotient form
under its explicit nonzero finite-Pochhammer denominator.\\
...
The exhaustive zero-definition, twelve-theorem commutative-semiring interface
for degree bounds, reflection, coefficient symmetry, boundary coefficients,
and the division-free first moment; exact degree and monicity add
nontriviality.\\
\path{CentralQBinomialReduction.lean} &
The exhaustive zero-definition, six-theorem sign-pairing, even--odd
dissection, ring-hom naturality, and central-Gaussian-reduction interface.  Its
multiplication form is division-free over commutative rings; only the field
quotient wrapper assumes the two finite q-Pochhammer denominators nonzero.\\
\path{CyclotomicFactorization.lean} &
The exhaustive zero-definition, seven-theorem floor-exponent,
divisor-indexing, shifted-factorial, and Gaussian cyclotomic-factorization
interface.  The factorial layer is commutative-ring-level; the final Gaussian
cancellation assumes an integral domain and \(k\le n\).\\
\path{PrimitiveRootBlock.lean} &
The exhaustive zero-definition, three-theorem primitive-root interface:
interior Gaussian vanishing for \(0<k<d\), the top phase, and the complete
q-Pochhammer block over a commutative integral domain; the latter two state
\(0<d\) explicitly.\\
\path{QLucas.lean} &
The exhaustive zero-definition, eight-theorem arithmetic, coefficient,
root-block, phase, and evaluated q-Lucas interface.  Its final theorem assumes
an integral domain, \(0<d\), a primitive \(d\)-th root, and \(b,s<d\); it does
not state the polynomial congruence modulo \(\Phi_d\).\\
\path{CyclotomicDivisibility.lean} &
The exhaustive zero-definition, three-theorem carry, rational-polynomial
cyclotomic-divisibility, and multiple-index primitive-root interface.  The
first two retain \(k\le n\), \(0<d\); the last uses an integral domain and
\(0<n\).\\
\path{QCatalan.lean} &
The exhaustive one-definition, eleven-theorem q-integer factorization and
integral q-Catalan quotient interface, for every natural index including zero;
degree and evaluation at one are exact, while coefficient positivity and
unimodality are not asserted.\\
\path{NewtonInterpolation.lean} &
The exhaustive two-definition, thirteen-theorem finite field-interpolation
interface.  It uses the compatibility name \lean{newtonInterpolant}; node
evaluation and uniqueness expose their nonvanishing/injectivity hypotheses,
and the geometric formulas require \(q\ne0\) or finite-range power
injectivity as stated.\\
\path{QBetaIntegral.lean} &
The exhaustive one-definition, eight-theorem real Jackson q-beta interface:
product and q-gamma evaluations, symmetry, positivity, and recurrences for
\(0<q<1\), \(x,y>0\), with no complex continuation or classical-limit claim.\\
...
identifications, zero-tilt normalization, and positivity; the dyadic almost-sure
...
bounded-Fabius specializations.  With
...
\path{FabiusLegendreTranslateSeries.lean} have public definition/theorem
inventories $6/33$, $0/5$, $1/8$, $2/7$, $1/7$, $6/7$, and $5/25$,
respectively, for exactly $113$ public declarations.  Universal
...
\path{GaussianBinomialPalindromic.lean} & Exactly twelve public theorems and no definitions: generic reflection helpers, Gaussian degree bound, zero/diagonal values, reflection, constant/top coefficients, coefficient symmetry, division-free mean, and exact degree/monicity over a nontrivial commutative semiring; no coefficient-of-\(q\) theorem is asserted. \\
\path{GaussianBinomialPolynomialStructure.lean} & Exactly five public theorems and no definitions: exact universal natural degree, monicity, constant coefficient one, reflection at degree \(k(n-k)\), and bounded-index coefficient palindromicity; no coefficient-of-\(q\) theorem is asserted. \\
...
modulus.  The modulus is assumed, not derived from a gap sequence. & \Formalized\\
...
it does not construct \(d\) from the gap sequence in the paper.  The separate
...
Together the two modules expose exactly these nine declarations and no
input-bit or running-time asymptotic.
