...
\(\beta_{n+1}\), with the latter prefactor \(((n+1)/(2n+1))^2\).  This module itself
asserts no Gaunt entry expansion; the downstream modules below supply the finite formulas.
A Wigner/$3j$ or factorial closed form, Christoffel reconstruction, root or quadrature
theorem, infinite Jacobi product/continued fraction, and asymptotics remain outside this
layer.\\
...
For every \(n\in\NaturalNumbers\), the rational Lebesgue moment is \(2/(n+1)\) at even
\(n\), zero at odd \(n\), and casts to \(\int_{-1}^{1}x^n\,dx\).  For arbitrary natural
\(i,j,k\), \lean{legendreGauntRat} is the bounded rational coefficient--moment sum,
\lean{legendreGaunt} is \(G_{ijk}=\int_{-1}^{1}P_iP_jP_k\,dx\), and, writing
\(a_{ijk}=((2k+1)/2)G_{ijk}^{\mathbb Q}\), the coefficient \(a_{ijk}\) is exactly
\lean{legendreProductLinearizationCoeffRat}\((i,j,k)\).
The sum equals the rational moment functional and casts to the integral, obeys the
displayed first-two and last-two swaps, and gives over both \(\mathbb R[X]\) and
\(\mathbb Q[X]\)
\[
 P_iP_j=\sum_{k=0}^{i+j}a_{ijk}P_k.
\]
Only the zero theorems have hypotheses: \(\operatorname{Odd}(i+j+k)\), \(i+j<k\), or
\(i+j<k\lor i+k<j\lor j+k<i\), respectively.  All definitions and other statements are
total at arbitrary natural indices.  No converse support theorem, Wigner/$3j$
identification, factorial closed form, positivity theorem, or asymptotic claim is made.\\
\path{FabiusLegendreGaunt.lean} &
Exactly one public definition,
\lean{canonicalRvachevFullLegendreCoefficientRat}, and eight public theorems:
...
\(M_{\mathbb Q}\) for the rational moment functional.  Unconditionally in \(k\),
\[
 c_k^{\mathrm{full}}=\frac{2k+1}{2}\,M_{\mathbb Q}(P_k).
\]
For all natural \(i,j\), write \(H_{ij}^{\mathbb Q}\) for the rational Gram entry,
\(c_r\) for the canonical even coefficient, and \(G_{ijk}^{\mathbb Q}\) for the
...
finite polynomial and finite-sum identities, with no Wigner/$3j$ or factorial formula,
infinite Legendre-series interchange, Christoffel reconstruction, or new positivity or
asymptotic result.\\
...
Together, the nine-module finite-algebra inventory through this values leaf has eighteen
public definitions and eighty-one public theorems, hence ninety-nine public declarations.
This values leaf itself has no Gaunt entry expansion; the preceding two Gaunt modules
supply the finite formulas.  A Wigner/$3j$ or factorial closed form, Christoffel
reconstruction, root or quadrature theorem, infinite Jacobi product/continued fraction,
and asymptotics remain outside the combined layer.\\
