...
integer-index, and generalized-identifiability closure.  The live facade
contains exactly 641 source modules and 8,652 public declarations, with no
documentation gaps.  The committed PDF is a historical validation receipt and
is rebuild-pending; its historical page count and digest do not certify
...
\nolinkurl{QPochhammerEntire.lean}; zero definitions and exactly six theorems,
under exactly \(\lVert q\rVert<1\); the division-free factor-zero theorem
includes \(q=0\), the reciprocal-power form assumes \(q\ne0\), and analytic
order one is exposed under both public product names \\
...
\nolinkurl{qPochhammerInfIn}; all twenty-seven public theorem names and their
hypothesis boundaries are listed in the exact crosswalk below &
\nolinkurl{QPochhammerInfinite.lean}; one definition and exactly twenty-seven
...
still lacks a named centered/MGF wrapper and the outside-disk reciprocal formula
and zero--pole exchange.  Locally uniform convergence of the outer Pochhammer
product over the spectral index is now supplied by the separate
\path{GeometricPochhammerNormalConvergence.lean} leaf for every fixed strict-
contraction nome.
...
They are fixed-\(q\), single-symbol results, with the last theorem transporting
analytic order one to the generic public name across the unconditional bridge.
They prove no joint holomorphy in \((a,q)\), outside-disk reciprocal formula,
meromorphic reciprocal, or centered characteristic-function/MGF package.  The
separate \path{GeometricPochhammerNormalConvergence.lean} leaf closes locally
uniform convergence of the outer product in
\eqref{integration:eq:geometric-sinc-pochhammer} for fixed strict-contraction
nome.  The committed PDF is a historical validation receipt and remains
rebuild-pending; it does not certify parity with this six-theorem source API.
...
definition \lean{qPochhammerInfIn} and twenty-seven theorems.  In an arbitrary
...
The remaining four theorems are complex.  Under \(\lVert q\rVert<1\),
...
identification.  No theorem in \path{QPochhammerInfinite.lean} itself states
\texttt{analyticOrderAt}.  The downstream \path{QPochhammerEntire.lean} leaf
states that conclusion both for \lean{complexQPochhammerInf} and, through
\lean{analyticOrderAt_qPochhammerInfIn_of_eq_zero}, for the generic public
name, including \(q=0\).  Conversely the six-theorem analytic leaf contains
no generic ring-valued concatenation or dissection theorem.  Neither API
proves joint holomorphy in \(q\) or an outside-disk reciprocal theory.
...
six-declaration overlay is part of the live 641-module, 8,652-declaration
source union.  The committed PDF remains explicitly historical and
rebuild-pending.
...
The zero-definition, six-theorem interface for
...
\(\lVert q\rVert<1\), including \(q=0\), under both the historical and generic
public names via
\lean{analyticOrderAt_qPochhammerInfIn_of_eq_zero};
...
nonzero reciprocal-zero derivative when \(q\ne0\).  This module itself supplies no
\texttt{analyticOrderAt} theorem; the generic-name result occurs in the
preceding downstream analytic leaf.  It also asserts no joint-\(q\)
