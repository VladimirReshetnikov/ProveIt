...
\(|q|<1\) and \(t\in\R\), including \(q=0\) and negative \(q\),
\[
 \phi_q(t)=e^{\ii t/2}S_q(z_q(t))
 =e^{\ii t/2}G_q(z_q(t))G_q(-z_q(t)).
\]
It also gives locally uniform convergence on \(\C\) of the defining products for \(S_q\),
their pointwise multipliability and exact \texttt{HasProd} identity, and entireness of
\(S_q\).  A named compact-uniform theorem for the full phase-bearing characteristic
prefixes, general-\(q\) rapid decay and Fourier inversion, and the further centered/MGF,
shape, explicit Bernoulli-cumulant/Bell-moment, transform, derivative-hierarchy, and
endpoint-flatness layers remain in the frontier on the precise boundary recorded below.
...
product and paired reciprocal-Gamma factors for every real \(\lvert q\rvert<1\) &
...
\texttt{geometricSincProduct} now converge locally uniformly on \(\C\), have named
...
\(\phi_q(t)=e^{\ii t/2}S_q(z_q(t))
=e^{\ii t/2}G_q(z_q(t))G_q(-z_q(t))\) throughout \(|q|<1\), including zero
...
\(q\le0\), hence not the probability law on the nonpositive contraction range.  What
remains here is Lean formalization of the paper-level corrected signed-density construction
for \(q\le0\), a named
compact-uniform theorem for the full phase-bearing characteristic prefixes, rapid decay and
Fourier inversion, named centered/MGF packaging, a closed
...
The zero-definition, two-theorem module
...
\end{align}
The scope again includes \(q=0\) and negative \(q\).  The locally uniform theorem is for
the complex-variable sinc product \(S_q\); no named theorem yet packages compact-uniform
convergence of the full phase-bearing characteristic prefixes.
...
\begin{itemize}
\item a named compact-uniform theorem for the full phase-bearing characteristic prefixes,
      beyond both the formal pointwise characteristic-prefix limit and the locally uniform
      convergence of the unphased complex sinc product, quantitative rapid decay, and the
...
\texttt{HasProd} theorem controls \(S_q\) as a complex-variable product; these declarations
do not supply the named full phase-bearing characteristic-prefix compact-uniform theorem
or the Fourier estimates in the first item.
...
Locally uniform convergence on \(\C\), pointwise multipliability and exact
...
The zero-definition, two-theorem exact bridge from the geometric-uniform characteristic
function to the rescaled geometric sinc product and to the paired reciprocal-Gamma factors
for every real \(|q|<1\), including zero and negative ratios;
...
\texttt{geometricSincProduct} is now backed by locally uniform convergence on \(\C\),
pointwise multipliability and an exact \texttt{HasProd} theorem, and entireness.  With
\(z_q(t)=(1-q)t/(2\pi)\), exact named bridges prove
\(\phi_q(t)=e^{\ii t/2}S_q(z_q(t))
=e^{\ii t/2}G_q(z_q(t))G_q(-z_q(t))\) throughout \(|q|<1\), including
zero and negative ratios.  Failure of the selected total density formula on \(q\le0\) is
formal; Lean formalization of the paper-level corrected signed-density replacement, a named compact-uniform
theorem for the full phase-bearing characteristic prefixes, general-\(q\) rapid decay and
Fourier inversion, centered/MGF package, closed higher-derivative and endpoint-flatness
wrappers, shape, further transforms, and explicit
...
decreases; for \(0<h\le1\), the maximal support-meeting branches
\([-h,(1-h)/2]\) and \([(1-h)/2,1]\) are strict.  On the unit interval, the endpoint
minimum has exact strict-interior and degenerate/endpoint equality loci, and constrained
superadditivity has its full equality classification.  The ordered and order-free
inverse-gap bounds include complete ordered and absolute equality loci on \([0,1]\), while
the global clamped tails deliberately remain outside that equality classification.
Subadditivity, absolute and metric self-moduli, saturation at one, and attained exact
\path{IsGreatest}/\path{sSup} moduli hold globally and on the unit interval.  The final
four declarations are the strict and closed effective-injectivity bounds, their
contrapositive separation form, and the uniform strict-threshold biconditional.  This
module does not supply the report's closed
