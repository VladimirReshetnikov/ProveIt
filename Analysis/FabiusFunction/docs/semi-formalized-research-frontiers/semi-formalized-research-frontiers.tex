...
28 August 2026; formal-status audit updated 31 August 2026\\
...
described in the audit map below.  Source/PDF parity is certified only by the
repository metadata for a successful final-source build of this exact TeX
checkpoint; the page count, checksum, and validation facts in that build receipt
are authoritative.
\end{boxedremark}
...
Entire complex q-Pochhammer factors and simple zeros &
...
\nolinkurl{QPochhammerEntire.lean}; zero definitions and exactly four theorems,
under exactly \(\lVert q\rVert<1\), including the degenerate nome \(q=0\) \\
...
still lacks a named centered/MGF wrapper, the outside-disk reciprocal formula and
zero--pole exchange, and a named local-uniform/normal-convergence theorem for the
outer Pochhammer product over the spectral index.
...
\]
and the fourth gives analytic order one at every such zero.  This
division-free form includes \(q=0\), with its unique zero \(a=1\).  There is no
division-based parametrization \(a=q^{-j}\), joint holomorphy theorem in \(q\),
locally uniform theorem for the outer spectral product, or global
entire-function growth, order, type, or asymptotic claim.
...
\path{QPochhammerInfinite.lean} has one public definition,
...
The definition is total in a topological commutative ring.  For
\(\lVert q\rVert<1\), the complete commutative normed-ring layer with
\(\lVert1\rVert=1\) proves the product, finite-prefix limit, shift,
factor-removal, and positive-modulus dissection laws; adding a multiplicative
norm gives the raw zero locus \(aq^j=1\).  Complete normed fields admit the
inverse-power locus when \(q\ne0\), and local compactness gives locally uniform
convergence and continuity in \(a\) for fixed \(q\).  The complex layer gives
entireness and the factor-zero derivative; for \(q\ne0\), it gives the closed
inverse-power derivative and its nonvanishing.  There is no joint-\(q\)
holomorphy or global growth/order/type/asymptotic claim.  The preceding
\path{QPochhammerEntire.lean} remains the separate
\lean{complexQPochhammerInf} analytic-order-one API, including \(q=0\).

\paragraph{Exact entire q-Pochhammer and simple-zero crosswalk.}
The downstream \path{QPochhammerEntire.lean} leaf has no public definitions and
exactly four public theorems:
\lean{hasProdLocallyUniformly_complexQPochhammerInf},
\lean{complexQPochhammerInf_differentiable},
\lean{complexQPochhammerInf_eq_zero_iff}, and
\lean{analyticOrderAt_complexQPochhammerInf_of_eq_zero}.
For complex \(q\) with exactly \(\lVert q\rVert<1\), they prove that the finite
products \(a\mapsto\prod_{j<n}(1-aq^j)\) converge locally uniformly on the
whole complex plane to \(a\mapsto(a;q)_\infty\), that the limit is entire, and
that
\begin{equation}
 (a;q)_\infty=0
 \quad\Longleftrightarrow\quad
 \exists j\in\NaturalNumbers,\;1-aq^j=0.                 \label{integration:eq:qpochhammer-zero-locus}
\end{equation}
At every such zero the analytic order is one.  Indeed, on each compact set the
factor perturbations are dominated by a constant times
\(\sum_j\lVert q\rVert^j\), which proves local uniform convergence.  A product
with summable perturbations and no zero factor is nonzero; strict contraction
makes the vanishing factor unique, and removing its linear term leaves a
nonzero holomorphic cofactor.  Thus the four Lean theorems cover the complete
human argument.  The division-free factor form includes \(q=0\), whose unique
zero is \(a=1\); when \(q\ne0\), it is equivalent to the familiar list
\(a=q^{-j}\).  This leaf does not treat \(\lVert q\rVert\ge1\), joint
holomorphy in \(q\), a closed derivative formula, reciprocal continuation, or
local-uniform/normal convergence of the outer product over spectral indices in
\eqref{integration:eq:geometric-sinc-pochhammer}.
...
identification.  No theorem in this generic module states
\texttt{analyticOrderAt}.  That conclusion for the separate public name
\lean{complexQPochhammerInf}, including \(q=0\), remains exactly the final
theorem of \path{QPochhammerEntire.lean}; conversely the four-theorem wrapper
contains no generic ring-valued concatenation or dissection theorem.  Neither
API proves joint holomorphy in \(q\) or an outside-disk reciprocal theory.
...
The zero-definition, four-theorem interface for
\(a\mapsto(a;q)_\infty\): locally uniform convergence on \(\ComplexNumbers\),
entireness, the exact division-free factor-zero locus, and analytic order one at
every zero for complex \(\lVert q\rVert<1\), including \(q=0\);
\cref{integration:eq:qpochhammer-zero-locus}.\\
...
it also asserts no joint-\(q\) holomorphy or global growth/order/type/asymptotics.\\
...
The one-definition, twenty-three-theorem Euler/Cauchy interface over complete
normed fields: comparison products, a uniform Gaussian majorant, fixed-column
and Tannery limits, Euler product and reciprocal expansions, and the infinite
q-binomial theorem under the stated strict nome and series-variable bounds.\\
...
(lVert qVert<1); no unit-circle or modular-transformation claim.\\
...
generating series under (lVert qVert<1), (lVert tVert<1), and
(lVert ztVert<1); no orthogonality or root-location theorem.\\
