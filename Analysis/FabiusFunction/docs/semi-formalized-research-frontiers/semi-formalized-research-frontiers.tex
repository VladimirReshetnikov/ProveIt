...
algebra modules, the completed Gaussian linear-coefficient classifier, and
the eight-declaration rational-gap inverse module.  The live source census is
augmented by the four-module, 26-declaration primitive-root/q-Lucas/q-Catalan
tranche: 666 facade-reachable modules and 8,828 public declarations.  The retained
...
\nolinkurl{NewtonInterpolation.lean}; two definitions and thirteen theorems \\
...
eight public declarations).  A computable sequence of positive natural
...
extension \(x\mapsto g(\nolinkurl{unitClamp}(x))\), not a claim about the
...
\path{NewtonInterpolation.lean} is 2+13: definitions \lean{newtonCoeff} and
\lean{nodeNewtonPoly}; theorems \lean{newtonCoeff_eq},
...
\lean{prod_erase_pow_sub_pow}, and \lean{newtonCoeff_pow_eq_sum}.
...
eight-declaration rational-gap inverse leaf, and the sixteen-module
q-series/q-calculus union, including its two newest finite-q algebra modules
and the two added Gaussian linear-coefficient theorems, followed by the
four-module, 26-declaration root-of-unity/q-Catalan tranche,
and the two-module, 24-declaration Newton-interpolation/q-beta tranche,
the live source census is 666/8,828.  These additions
...
Their one definition and twenty-five theorems give complete primitive-root
blocks, q-Lucas, the cyclotomic carry criterion, and the integral q-Catalan
polynomial with exact degree and Catalan value at one.

The final two module counts are \path{NewtonInterpolation.lean} (2+13) and
\path{QBetaIntegral.lean} (1+8), adding three definitions and twenty-one
theorems.
...
The exhaustive two-definition, thirteen-theorem finite-node and geometric-grid Newton interface.\\
...
The exhaustive public surface of \path{EffectiveGapInverse.lean} is
...
\(x\mapsto g(\path{unitClamp}(x))\), which agrees with \(g\) on \([0,1]\) but
