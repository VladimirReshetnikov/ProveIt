...
28 August 2026; formal-status audit updated 30 August 2026\\
...
and gives the empty, singleton, insert, and powerset laws.  Over a commutative
ring, for a polynomial of degree at most \(|s|\), it extracts the degree-\(|s|\)
...
power.  The dyadic block is group-valued.  Its increasing-grid wrappers use a
characteristic-zero field: the all-order form, including \(m=0\), has points
\(x-(1-2^{-m})+2n2^{-m}\).  At positive order \(N=m+1\), the full point is
\(x-(1-2^{-(m+1)})+n/2^m\); its varying term is \(n/2^m\), its mesh spacing
is \(1/2^m\), and its complement sign is \((-1)^N\).  This closes the Boolean-cube,
polynomial, and affine-grid clauses of the continuous-chaos report's
Thue--Morse corner theorem, but not its repeated-integral equality under
\(C^N\) hypotheses; the following Walsh conditional-expectation corollary
...
branch-point derivative, vertical-tangent limit, or Puiseux law. &
...
If \(m\in\N\setminus\{0\}\), \(A>0\), \(\beta>0\), and \(0<x<P\), both
...
\(\beta_{n+1}\), with the latter prefactor \(((n+1)/(2n+1))^2\).  No
Gaunt/Wigner/$3j$ entry expansion, Christoffel reconstruction, root or quadrature theorem,
infinite Jacobi product/continued fraction, or asymptotic result is asserted.\\
...
These first eight computations are unconditional over $\Q$.  The final three theorems
...
Together with the preceding five finite-algebra modules, these two modules complete a
seven-module inventory of thirteen public definitions and sixty-one public theorems,
hence seventy-four public declarations.
No Gaunt/Wigner/$3j$ entry expansion, Christoffel reconstruction, root or quadrature
theorem, infinite Jacobi product/continued fraction, or asymptotic result is asserted.\\
...
zero-constant solution of \(D+4D^2=(4/9)X\) in \(\Q[[X]]\).  Rescaling the
...
\path{Fabius.fabiusIntervalMass}, and twenty theorems,
...
increases toward the centered interval, and then decreases.  The endpoint minimum yields
the ordered and order-free inverse-gap bounds, subadditivity, absolute and metric
self-moduli, saturation at one, and attained exact \path{IsGreatest}/\path{sSup} moduli
globally and on the unit interval.  The final four declarations are the strict and closed
