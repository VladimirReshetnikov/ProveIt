...
\path{ThueMorseNewmanSelfSimilarity.lean}.  That tranche reached the
historical 934/11,709 checkpoint.  The subsequent q-series, Lambert,
transseries, inversion, and Newman additions in the frozen merge target bring
that target to 957 facade-reachable modules and 11,920 public declarations.
Retaining the unconditional public
\lean{complexQPochhammerInf_eq_qPochhammerInfIn} bridge gives the historical
zero-gap merge-union checkpoint of 957 modules and 11,921 public declarations.
The residual-existence certificate \lean{exists_eq_in_residual_interval} is
included in the current zero-gap semantic union computed by
...
934/11,709 checkpoint.  The frozen merge target contains 957 modules and
11,920 public declarations; the retained public q-Pochhammer bridge gives the
historical 957/11,921 zero-gap merge-union checkpoint, and the
residual-existence certificate is included in the current zero-gap union
computed by \path{scripts/doc_audit.py} and pinned in
\path{docs/doc_audit_baseline.json}.  The incoming-branch
906/11,461 and earlier exterior-branch 907/11,464 censuses remain explicitly
historical.
This integration was first recorded source-only across the primary exposition,
Lean walkthrough, canonical frontier, geometric-q and canonical q-series
syntheses, inverse-theory and comb syntheses, Lambert Guide, Up Polynomial
Synthesis, and Thue--Morse Atlas.  At that checkpoint the preceding PDFs were
historical and rebuild-pending, and no render parity was claimed.  Current
rendered publications are certified by external current source/PDF receipts,
alongside the exact historical receipts, so this artifact embeds no mutable
...
one theorem added to \path{DyadicBoundaryIdentity.lean}, and seven theorems
added to \path{ThueMorseNewmanSelfSimilarity.lean}.  This gives the historical
934/11,709 checkpoint, with zero missing module headers and zero missing declaration
comments.  The frozen merge target itself contains 957 modules and 11,920
public declarations; retaining the unconditional public
\lean{complexQPochhammerInf_eq_qPochhammerInfIn} bridge gives the historical
957/11,921 zero-gap merge-union checkpoint.  The residual-existence certificate
is included in the current zero-gap semantic union computed by
\path{scripts/doc_audit.py} and pinned in
...
one theorem added to \path{DyadicBoundaryIdentity.lean}, and seven theorems
added to \path{ThueMorseNewmanSelfSimilarity.lean}.  This gives the historical
934/11,709 census, with zero documentation gaps.
The frozen merge target now contains 957 modules and 11,920 public
declarations; retaining the unconditional public q-Pochhammer bridge gives
the historical 957/11,921 zero-gap merge-union checkpoint, and the
residual-existence certificate is included in the current zero-gap semantic
union computed by \path{scripts/doc_audit.py} and pinned in
\path{docs/doc_audit_baseline.json}.  The existing
