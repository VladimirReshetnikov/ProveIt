...
\(S_q\).  For every real \(|q|<1\), the full phase-bearing prefixes now converge locally
...
locally uniform theorem for the full phase-bearing prefix sequence, general-\(q\) rapid
decay and Fourier inversion, and the further centered/MGF, shape, explicit
...
line and, explicitly, uniformly on every compact \(K\subset\R\).  This is stronger than the
...
For nonnegative length the fixed-length Fabius increment is reflection-invariant, vanishes
on both constant tails, and has the global weak increase--decrease shape.  For
\(0<h\leq1\), it is strictly increasing on \([-h,(1-h)/2]\) and strictly decreasing on
\([(1-h)/2,1]\), the maximal strict branches before the constant tails.  Inside
\([0,1]\), only the two endpoint intervals minimize nondegenerate fixed-length mass; the
new equality theorems classify that least-mass inequality and constrained
superadditivity.  The endpoint minimum also yields the ordered and order-free inverse-gap
bounds, subadditivity, absolute and metric self-moduli, saturation at one, and attained
exact \path{IsGreatest}/\path{sSup} moduli globally and on the unit interval.  On the unit
interval, equality in the ordered inverse-gap bound occurs exactly for a degenerate
interval or one meeting an endpoint, and equality in the absolute bound occurs exactly
for coincident inputs or an endpoint input.  Clamping creates extra equality cases outside
\([0,1]\), so no corresponding global classification is claimed.  The final four
declarations are the strict and closed effective-injectivity bounds, their contrapositive
separation form, and the uniform strict-threshold biconditional.  This module does not
supply the report's closed \(\Delta_r\) lower bound, recursive-modulus wrapper,
tolerant-bisection realizer, sequential computability theorem, combined
computable-real-function theorem, or input-bit asymptotics.
