...
objects; no explicit $H_4$ or $\beta_4$ value, evaluator, root, Christoffel, quadrature,
continued-fraction, Pad\'e, or asymptotic theorem is asserted.\\
\path{PolynomialMomentGramDeterminant.lean},
\path{FabiusLegendreHankelDeterminant.lean} &
The first module has two public definitions,
...
For every polynomial family with degree of member $k$ at most $k$, they give the
coefficient-matrix congruence over a commutative semiring and determinant transport over a
commutative ring.  The field-level ratio additionally assumes every diagonal coefficient
is nonzero, but no Hankel nonvanishing; it is total finite algebra with no measure,
positivity, or analytic claim.  The second module has two public definitions,
...
Its determinant identity and up-moment ratio need only a bounded Fabius representative;
the integral interpretation, positivity, and rational-cast ratio require
\lean{IsFabius}.  The index is zero-based, hence represents $\beta_{n+1}$.  Neither module
proves a Gaunt/Wigner-symbol expansion, Christoffel reconstruction, root or quadrature
theorem, infinite Jacobi product/continued fraction, or asymptotics.\\
