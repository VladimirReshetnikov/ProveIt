...
\hypersetup{
  pdftitle={Asymptotic Transseries and Inversion of Eight Sequences and Special Functions},
...
\title{\textbf{Asymptotic Transseries and Inversion\\ of Eight Sequences and Special Functions}\\[0.35em]
  \Large Rooted trees, the double factorial, the partition numbers, the swing
  factorial, Gamma and Barnes $G$, the hyperfactorial $K$, the subfactorial,
  and a real-argument Fibonacci function:\\
  Lambert-cored reversion and its limits}
...
\begin{abstract}
Eight subjects are studied here to all orders and then inverted.  The
first four are combinatorial sequences: the numbers \(a_n\) of unlabelled rooted trees on \(n\) vertices
...
\(\operatorname{sw}(n)=n!/\lfloor n/2\rfloor!^{2}\) (A056040).  The next three are special
functions whose logarithms have a power--logarithmic phase: Euler's
\(\Gamma\), the Barnes \(G\)-function, and the hyperfactorial
\(K\)-function.  The last two are outliers, included because they mark the
boundary of the method: the subfactorial \(!n\), whose inverse borrows its
carrier from \(\Gamma^{-1}\) and whose own content is superexponentially
small, and a real-argument Fibonacci function, whose inverse has no Lambert
core at all and whose expansion converges.  For each we give the complete
forward asymptotic transseries and then the asymptotic inverse: given a
value, recover the index, to all orders and with an explicit error theory.

The subjects look unrelated and their forward analyses genuinely are --- a
Pólya-tree singularity analysis, two unrelated pairs of gamma-ratio branches,
Rademacher's convergent root-of-unity series, three Stirling-type expansions,
a Bell-number tail, and a Binet formula.  Their \emph{inversions} are not.
Six of the eight, after a monomial change of variable, are the inversion of
an exponential--power model
\[
  F(x)=C\exp(ax^{\alpha})\,x^{b}\exp Q(x^{-\delta}),
\]
or of a power--logarithmic block \(a x^{p}(\log x-\beta)\), whose dominant
phase is inverted exactly by the Lambert \(W\)-function and whose remaining
corrections are generated by one triangular recurrence.  Chapter~0 develops
that apparatus once, with complete proofs; Chapters~1--8 supply the parameter
dictionary for each subject and the mathematics genuinely specific to it.  The
synthesis chapter compares all eight and records what the common frame does
and does not explain --- including four square roots, arising in two
apparently different ways, that are shown to be one lemma applied four
times.

This volume is a consolidation of twenty-four independently written
articles, three on each subject, filed in this repository on 3 September
2026.  The second twelve were written without knowledge of the first twelve
and reproduce much of their apparatus independently; the duplication is
quantified in the synthesis chapter.  Where
...
\begin{sourcebox}
All of the first twelve filed articles carry a private copy of the material of this
...
the greatest generality of the first twelve.  Four things have been changed.
...
The first four subjects of this volume were written about independently, and their
...
Each of the first twelve articles verifies by hand, for its own case, which branch
...
Several statements in this volume are not in any of its first twelve sources.  They
...
directories below are relative to that path.  Nothing in the twenty-four was
...
the twenty-four articles where a printed numeral is wrong; it is a precision
