...
\item\label{plt:thm:alg-precision-calculus:deriv}
      ...
      sound, and it is attained if and only if
      \((\partial_L-m_1)v_1(m_1)\ne0\), that is (for \(v_1\ne0\)) unless
      \(m_1=0\) and \(v_1(0)\in\K^{\times}\); in that exceptional case the true
...
\texttt{Fabius.lowerLambertW\_near\_zero\_bounds} proves, for
...
\texttt{Fabius.SubLog.bracket\_of\_sub\_log\_eq} brackets the inverse of
\(y\mapsto y-\log y\) for \(y>1\) with no Lambert content at all, and
\texttt{Fabius.SubLog.strictMonoOn\_sub\_log} supplies the monotonicity that
...
(\texttt{Fabius.one\_lt\_log\_one\_div\_neg}) rather than a hypothesis the
...
\texttt{Fabius.principalLambertW\_elementary\_bounds} gives
\(x/(1+x)\le\LambertW_{0}(x)\le\log(1+x)\le x\) for \(x\ge0\), resting on the
Lambert-free inequality
\texttt{Fabius.exp\_le\_one\_add\_mul\_exp\_self}, valid for every real
argument.
