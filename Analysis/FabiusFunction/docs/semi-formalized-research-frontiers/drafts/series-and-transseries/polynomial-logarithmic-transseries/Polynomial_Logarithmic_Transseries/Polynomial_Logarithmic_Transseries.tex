...
The volume is arranged so that each part depends only on those before it, with
one deliberate exception: Part~V may be read directly after Part~IV by a reader
who wants only the Lambert expansion.
...
\item \(X^{1/2}\) arises from inverting a non-integer leading power, and
...
belong in one expansion.  The \(t\)-adic completion of the Laurent ring in
(iii) is the ambient object \(\PLlau=\K[L]((t))\) of Part~I.
\end{remark}
...
and using \(\log x=-\ell\),
\[
  \log\FabiusBounded(x)\le A+B,
  \qquad
  A\DefinitionEquals\frac{an(n+1)}{2}-n\ell,
  \qquad
...
\(n!\ge n^{n}\EulerE^{-n}\), hence \(B\le-n\log n+n\).  As \(m\ge2\) and
...
\[
  B\le-\frac{\ell\log\ell}{a}+\frac{3\ell}{a}.
\]
...
% is exact at z = e.  We keep the exactness, which is verifiable in one line,
% and defer the convergence domain to Part VI with its own citation.
What is elementary, and worth isolating, is that
\eqref{plt:eq:mot-W-two-orders} is \emph{exact} at \(z=\EulerE\): there
\(L_{1}=1\) and \(L_{2}=0\), and \(\LamP{n}(0)=0\) for every
\(n\in\NaturalNumbers\), so every correction block vanishes and the display
returns \(\LambertW_{0}(\EulerE)=1\).  A convergence threshold is a different
assertion and is not proved in this volume; Part~VI states what is proved and
cites what is not.
...
coefficientwise Laurent expansion at \(L=\infty\).  All four inclusions are
strict, and:
...
\begin{proof}
\emph{The first two inclusions.}  Both are inclusions of coefficient rings
\(\K[L]\subset\K[L]\subset\K(L)\) extended blockwise, hence injective ring
homomorphisms.  Strictness: \(t^{-1}\in\PLlau\setminus\PLpow\), and
...
series \(\sum_{r\ge0}(-w)^r\), which is \(u\)-adically summable.  By the
...
Let \(\alpha,\alpha',\beta,\beta'\) be fixed reals with \(\alpha>\alpha'\).
Then, on the positive ray,
\[
  \frac{X^{\alpha}(\log X)^{\beta}}{X^{\alpha'}(\log X)^{\beta'}}
  \longrightarrow+\infty
  \qquad(X\to+\infty),
\]
and if \(\alpha=\alpha'\) and \(\beta>\beta'\) the same conclusion holds.
...
\]
Since \(\log\log X=\LittleOAt{\log X}{X\to+\infty}\) and
\(\alpha-\alpha'>0\), the right-hand side tends to \(+\infty\).  If
\(\alpha=\alpha'\) and \(\beta>\beta'\) the expression reduces to
...
\end{equation}
where \(q_n\) denotes the blocks of \(G\).  In particular \(\ordt\) is a
discrete valuation on \(R\) with valuation ring
\(\SetOf{F:\ordt(F)\ge0}=E[[t]]\).
\end{theorem}
...
Finally, (i)--(iii) are the axioms of a valuation with value group
\(\IntegerNumbers\); its valuation ring is \(\SetOf{\ordt\ge0}\), which by
\eqref{plt:eq:lead-ordt} is the set of series with no negative blocks, namely
\(E[[t]]\).  The argument used about \(E\) only that it is an integral domain,
so the same laws hold in \(E[[t]]\) and \(E((t))\) for every integral domain
\(E\); this is the form in which they are applied in
\cref{plt:prop:ring-chain}.
...
Let \(F\in R\) be nonzero, \(\nu=\ordt(F)\), and let
\(p_{\nu}=aL^{d}+(\text{lower powers of }L)\) with \(a\in\K^\times\) and
\(d=\degL p_\nu\).  Then:
\begin{itemize}
...
For \(F,G\in R\),
\begin{enumerate}[label=\textup{(\roman*)}]
\item \(v(FG)=v(F)+v(G)\); equivalently, for nonzero \(F,G\),
  \[
...
\item if \(\lm(F)\ne\lm(G)\), then \(\lm(F+G)\) is the \(\succdom\)-greater of
  ...
  scalar.
\end{enumerate}
Consequently \(v\) is a valuation of rank two on \(R\), with value group
\(\Gamma\) as in \cref{plt:lem:ring-rank-two-group}.
\end{theorem}
...
vanishes then \(v(F+G)>v(F)\).  Rank two is
...
multiplication from \eqref{plt:eq:lead-slope-bound}, whose right-hand side is
again affine of slope \(\max(\alpha,\beta)\le\sigma\); the constants
\(A,B,r\) are fixed for given \(F,G\).
...
The analytic counterpart, proved for actual functions in Part~V, reads: for
every \(N>n_0\) there is an exponent \(M_N\) with
\[
...
% be \(\LittleO\) of the last retained term.  That condition does not determine
...
% displayed form, and S6's \(\LittleO(X^{-N})\) form, both do determine them.
...
\label{plt:box:trunc-replacements}
...
\label{plt:box:trunc-log-cutoff}
...
\(\mu\DefinitionEquals\inf_i\ordt(S_i)=\min\SetOf{0,\min_{\ordt S_i<0}\ordt
S_i}\) is an integer (or \(+\infty\) if all \(S_i=0\)).  For \(n<\mu\) every
...
Let \(E\) be any of \(\K[L]\), \(\K(L)\), \(\K((L^{-1}))\), and let
\(U\in tE[[t]]\), i.e.\ \(\ordt U\ge1\).  Then \((U^r)_{r\ge0}\) is formally
summable, \(\sum_{r\ge0}U^r\in E[[t]]\), and
\[
...
\begin{proof}
\(\ordt(U^r)=r\,\ordt U\ge r\to+\infty\) by
\cref{plt:thm:ring-valuation-laws}\,(i), so
...
=\sum_{r\ge0}\bigl(U^r-U^{r+1}\bigr)\), and the \(N\)-th truncation of the
last sum telescopes to \(\Trunc{(1-U^{R})}{N}=\Trunc{1}{N}\) for
\(R\ge N\).  Since this holds for every \(N\),
...
they hold verbatim in \(\PLpow\), \(\PLrat\) and \(\PLit\).  In \(\PLit\),
...
\end{equation}
the pair $(P,Q)=(N-b,\,N-a)$ is the smallest admissible one.  If $N>a+b$,
neither entry can be lowered by one: there exist $F,G$ with these valuations
...
\]
whose three terms have $\ordt$ at least $P+b\ge N$, $a+Q\ge N$ and
$P+Q\ge N$ respectively.  Hence the two products agree below $t^N$, which is
\eqref{plt:eq:mul-truncated-product}.  Minimality of $(N-b,N-a)$ is
\cref{plt:thm:mul-finite-determination}: the block $[t^{N-1}](FG)$ already
depends on $p_{N-1-b}$ and on $q_{N-1-a}$.

For the sharpness, assume $N>a+b$.  If $N-b-1>a$ put $F=t^a+t^{N-b-1}$; if
...
subquadratic and loses to it by \(N-1\) block multiplications when
multiplication is schoolbook.  Unrestricted division of coefficient blocks forces \(\PLrat\),
...
kind.  More generally \(\shiftop{n}{k}=\sum_{j=0}^{k}\stirlingsigned{k}{j}
(\partial_{L}-n)^{j}\).
\end{corollary}
...
\begin{proof}
Closure is \cref{plt:prop:dif-block} together with the facts that
\(\partial_{L}\) preserves \(\K[L]\), \(\K(L)\) and \(\K((L^{-1}))\); for
\(\EulerOp\) multiply by \(t^{-1}\), which preserves every class listed except
\(\PLpow\).  The inclusion \(\DX(\PLpow)\subseteq t\PLpow\) is
\cref{plt:eq:dif-block}; the reverse inclusion is
\cref{plt:thm:dif-antiderivative} below, whose construction applied to
\(F\in t\PLpow\) produces blocks only at indices \(\ge0\).  Likewise for
\(t\PLpow\), all indices involved being nonresonant.
\end{proof}
...
\begin{equation}\label{plt:eq:dif-anti-with-Lambda}
  ...
  \qquad\text{so every }F\in\PLit\text{ has an antiderivative in }
  \PLit\oplus\K\Lambda .
\end{equation}
\end{theorem}
...
\item \(t^{2}/L=1/(X^{2}\log X)\) has no antiderivative in \(\PLrat\), although
      it has one in \(\PLit\), namely
...
\cref{plt:lem:cmp-summable} gives (i) the sum
\eqref{plt:eq:cmp-master-sum} and compatibility with summable families.
...
% differentiation in the *argument* of the coefficient polynomial.  For rho != 1
% the two differ by a factor rho^k and the formula is then wrong.  We print
...
harmless only when \(\rho=1\); for \(\rho\ne1\) the chain rule inserts a
spurious factor \(\rho^{k}\) at the \(k\)-th term.
\end{warningbox}
...
\(\tilde a=\ordt\tilde F\), fix \(N\) and \(\varepsilon>0\), and write
...
\(\BigOAt{X^{-N+\varepsilon}}{X\to\infty}\).  The remaining product of two
...
\(\mathcal T_{H}(1)=1\) because \(\DX1=0\).  Compatibility with summable
families follows from \cref{plt:lem:cmp-summable}: the double family
\(\bigl(\tfrac{H^{k}}{k!}\DX^{k}(p_nt^{n})\bigr)_{n,k}\) has
\(\ordt\ge n+k\) and so is summable, and summing it in the two possible orders
gives \(\mathcal T_{H}\bigl(\sum_np_nt^n\bigr)=\sum_n\mathcal T_{H}(p_nt^n)\).
...
\emph{(I1) Block derivative.}  For \(n\in\IntegerNumbers\) and \(p\in\K[L]\),
...
\emph{(I2) Exact formal Taylor formula.}  For \(S\in\PLlau\) and
...
\emph{(I3) Associativity and chain rule.}  \((S\compose H_1)\compose H_2
...
\emph{(I4) Completeness.}  \(\PLpow\) is complete for the \(t\)-adic
...
direction: take \(\sigma_R=\sigma_C=6\) but \(\sigma_K=2\), so that \(J\) is
under-resolved.  Predict the resulting residual order from
\cref{plt:lem:rev-precision-propagation} and verify that the prediction is
attained.
\end{exercise}
...
\item[(F4)] \(\partial X=1\).
\end{enumerate}
We write \(\mathfrak A_{\ge0}=\SetOf{f:\nu(f)\ge0}\), a subring of
\(\mathfrak A\) by (F1).
\end{definition}
...
\begin{enumerate}
\item the family in \cref{plt:eq:lag-shift} is summable, and
      \(\nu(T_Hf)\ge\nu(f)\);
...
\item for \(H,H'\in\mathfrak A_{\ge0}\),
      \(T_{H'}\compose T_H=T_{H''}\) with \(H''=H'+T_{H'}(H)\);
\item \(\Gnear(\mathfrak A)\DefinitionEquals\SetOf{X+H:H\in\mathfrak A_{\ge0}}\)
      is a group under
...
to \(+\infty\); summability and the valuation bound follow from (F2).
...
A\langle z\rangle_{a+b}\), and for each \(g\in\mathfrak A_{\ge0}\) the
...
\(\nu(P_mg^m)\ge m-a\) makes the family summable and (F2) makes the product
rule a rearrangement of a summable double family.  Put
\(\tau(f)=\sum_k(\partial^kf)z^k/k!\); by the computation in (1),
...
Because \(\tau(H)\in\mathfrak A\langle z\rangle_0\), for any
\(P\in\mathfrak A\langle z\rangle_a\) the family \((P_k\tau(H)^k)_{k\ge0}\)
has \([z^m]\bigl(P_k\tau(H)^k\bigr)\) of valuation at least \((k-a)+m\), so it
may be summed coefficientwise; the resulting substitution
\(P\mapsto P(\tau(H))\) is again a ring homomorphism.  Now
\[
  \tau\bigl(T_H f\bigr)
  =\sum_{j\ge0}\frac{\tau(H)^j}{j!}\,\tau(\partial^jf)
  =\sum_{j\ge0}\frac{\tau(H)^j}{j!}\,\partial_z^{\,j}\tau(f)
  =\sum_{k\ge0}\frac{\partial^kf}{k!}\bigl(z+\tau(H)\bigr)^k ,
\]
where the last step is the binomial identity
\(\sum_jw^j\partial_z^jP/j!=P(z+w)\) applied to \(P=\tau(f)\).  Evaluating at
\(z=H'\) and using \(\mathrm{ev}_{H'}\tau(H)=T_{H'}(H)\) gives
\(T_{H'}(T_Hf)=\sum_k(\partial^kf)(H'+T_{H'}H)^k/k!=T_{H''}(f)\).

(5)  Part (4) says exactly that \(\compose\) is associative on
\(\Gnear(\mathfrak A)\) and that \(T_{F\compose G}=T_G\compose T_F\); the
...
\(A\in\mathfrak A_{\ge0}\), and \(F=X+A\).  There is exactly one
\(B\in\mathfrak A_{\ge0}\) with
\begin{equation}
...
\emph{Uniqueness.}  Suppose \(B,B'\in\mathfrak A_{\ge0}\) both solve
\cref{plt:eq:lag-fixedpoint}.  Then
\(B-B'=T_{B'}(A)-T_B(A)=\sum_{k\ge1}\bigl(B'^{\,k}-B^k\bigr)\partial^kA/k!\), the
\(k=0\) terms having cancelled.  For \(k\ge1\),
\(B'^{\,k}-B^k=(B'-B)\sum_{i=0}^{k-1}B'^{\,i}B^{k-1-i}\) has valuation at least
\(\nu(B'-B)\), while \(\nu(\partial^kA)\ge\nu(A)+k\ge k\).  Hence
\(\nu(B-B')\ge\nu(B'-B)+1\), which forces \(\nu(B-B')=+\infty\).

\emph{Existence.}  Set \(B_0=0\) and \(B_{n+1}=-T_{B_n}(A)\); each
\(B_n\in\mathfrak A_{\ge0}\) by \cref{plt:lem:lag-substitution}(1).  The
estimate just made gives
\(\nu(B_{n+1}-B_n)\ge\nu(B_n-B_{n-1})+1\), hence
\(\nu(B_{n+1}-B_n)\ge n+\nu(A)\).  The family \((B_{n+1}-B_n)_{n\ge0}\) is
therefore summable, and \(B\DefinitionEquals\sum_{n\ge0}(B_{n+1}-B_n)\) satisfies
\(\nu(B-B_n)\ge n+\nu(A)\).  Applying the same estimate once more,
\(\nu\bigl(T_B(A)-T_{B_n}(A)\bigr)\ge n+\nu(A)+1\), so
\(\nu\bigl(B+T_B(A)\bigr)\ge\min\SetOf{\nu(B-B_{n+1}),\nu(T_{B_n}A-T_BA)}\to+\infty\)
and \(B=-T_B(A)\).  Finally \(\nu(B)=\nu(T_B(A))\ge\nu(A)\).
\end{proof}
...
\(r-1\).  If \(\nu(A)\ge0\) the right-hand side of \cref{plt:eq:lag-order} is at
least \(r-1\to+\infty\), so the family is summable by (F2); the bound on
\(\nu(S)\) is the case \(r=1\) of \cref{plt:eq:lag-order}, which is the minimum
of \(r\nu(A)+r-1\) over \(r\ge1\).
\end{proof}

The hypothesis \(\nu(A)\ge0\) is not merely convenient: in the
polynomial--logarithmic frame it is exactly the summability condition.

\begin{proposition}[Sharp summability criterion]
\label{plt:prop:lag-summability-sharp}
Let \(A\in\PLlau\) be nonzero.  The family
\(\bigl((-1)^r\DX^{\,r-1}(A^r)/r!\bigr)_{r\ge1}\) is summable in \(\PLlau\) if
and only if \(\ordt(A)\ge0\), that is, if and only if \(A\in\PLpow\).  Indeed,
if \(\ordt(A)=-k\) with \(k\ge1\), then
\begin{equation}
\label{plt:eq:lag-order-exact}
  \ordt\Bigl(\DX^{\,r-1}\bigl(A^r\bigr)\Bigr)=r(1-k)-1
  \qquad\text{for every }r\ge1,
\end{equation}
which is nonincreasing in \(r\) and hence never tends to \(+\infty\).
\end{proposition}

\begin{proof}
Sufficiency is \cref{plt:lem:lag-order}.  For necessity write
\(A=t^{-k}p(L)+A_1\) with \(p\ne0\) and \(\ordt(A_1)>-k\).  Then
\(A^r=t^{-rk}p^r+(\text{terms of }\ordt>-rk)\).  Applying the repeated-derivative
formula
\(\DX^{\,r-1}\bigl(t^np(L)\bigr)=t^{n+r-1}\shiftop{n}{r-1}p(L)\) with
\(n=-rk\),
\[
  \DX^{\,r-1}\bigl(A^r\bigr)
  =t^{-rk+r-1}\prod_{j=0}^{r-2}\bigl(\partial_L+rk-j\bigr)p(L)^r
   +\bigl(\text{terms of }\ordt>-rk+r-1\bigr).
\]
For \(0\le j\le r-2\) one has \(rk-j\ge r-(r-2)=2>0\), so each factor
\(\partial_L+(rk-j)\) multiplies the leading coefficient of a nonzero polynomial
by the nonzero scalar \(rk-j\) and in particular is injective on
\(\K[L]\setminus\SetOf{0}\).  Hence the displayed leading block is nonzero and
\cref{plt:eq:lag-order-exact} holds.  Since \(k\ge1\), the exponent
\(r(1-k)-1\) is at most \(-1\) for all \(r\), so
...
% r-th term has t-order at least r-1"); none observes that the condition is
% also necessary, which is what makes the misuse in
...
\(A=L=\log X\) one has \(A\to+\infty\) yet \(\ordt(A)=0\), and the theory
applies; for \(A=X/L\) one has \(A=\LittleOAt{X}{X\to+\infty}\) yet
\(\ordt(A)=-1\), and it does not.  The decisive quantity is the outer order,
equivalently the dominance comparison \(A\precdom X\) \emph{refined to the
outer filtration}: logarithmic gains are invisible to \(\ordt\).
\end{remark}
...
\begin{proof}
(1)  If \(\nu(u_m)\ge m-a\) and \(\nu(v_m)\ge m-b\) then
\(\nu\bigl(\sum_{i+j=m}u_iv_j\bigr)\ge\min_{i+j=m}(i-a+j-b)=m-a-b\).  Stability
under \(\partial\) follows from (F3), which gives
\(\nu(\partial u_m)\ge m-(a-1)\).

(2)  For \(u\in\mathfrak A\langle\varepsilon\rangle_a\) the family
\((u_m)_{m\ge0}\) is summable, since \(\nu(u_m)\ge m-a\to+\infty\); the sum
exists by (F2) and has \(\nu\ge\inf_m(m-a)=-a\).  Additivity is clear, and
multiplicativity is the product rule of (F2):
\(\mathrm{ev}(uv)=\sum_m\sum_{i+j=m}u_iv_j=\sum_{i,j}u_iv_j
=\bigl(\sum_iu_i\bigr)\bigl(\sum_jv_j\bigr)\), the regrouping being licensed
because \(\nu(u_iv_j)\ge i+j-a-b\) makes the double family summable.
...
\(k>m\), and \(\nu\bigl([\varepsilon^m]f^{(k)}\bigr)\ge m-a\), so
\(\SetOf{(k,m):\nu<\gamma}\subseteq\SetOf{(k,m):m<\gamma+a,\ k\le m}\) is
finite.  Both iterated sums therefore equal the unordered sum, by the
regrouping clause of (F2).
\end{proof}
...
Let \((\mathfrak A,\nu,\partial,X)\) be a reversion frame and let
\(A\in\mathfrak A_{\ge0}\), \(F=X+A\).  Then the family
...
\(\nu(S)\ge\nu(A)\ge0\).  By \cref{plt:prop:lag-fixed-point} it suffices to
prove \(S=-T_S(A)\).
...
\(\nu\bigl([\varepsilon^m]U\bigr)\ge m\nu(A)+m-1\ge m-1\), so
\(U\in\mathfrak A\langle\varepsilon\rangle_1\) and
\(\mathrm{ev}(U)=S\) by construction.  Since \(U\) has zero constant term,
\([\varepsilon^m](U^k)=0\) for \(m<k\) and
\(\nu\bigl([\varepsilon^m](U^k)\bigr)\ge m-k\); with
\(\nu(\partial^kA)\ge\nu(A)+k\ge k\) this gives
\[
  \nu\Bigl([\varepsilon^m]\Bigl(\tfrac{\partial^kA}{k!}U^k\Bigr)\Bigr)\ \ge\ m,
  \qquad
...
\(a=0\), and applying \(\mathrm{ev}\) to \cref{plt:eq:lag-Ufix} gives
\[
...
Why powers and logarithms belong in one  & source draft 1 asserts convergence "on the principal real branch for z >= e", supported only by a general citation. That threshold is not established anywhere in the six drafts, and the assertion also conflates a convergence claim with the separate, elementary observation that the series is exact at z = e. We keep the exactness, which is verifiable in one line, and defer the convergence domain to Part VI with its own citation.\\
...
\cref{plt:ex:lead-strictness} & S1 and S5 record the strictness of (ii) as happening ``exactly when the leading blocks cancel'', omitting the hypothesis \textbackslash (\textbackslash ordt F=\textbackslash ordt G\textbackslash ); with unequal valuations no cancellation is possible and equality always holds.\\
...
\cref{plt:prop:trunc-not-ideal} & S1 offers instead the minimal Poincar\textbackslash 'e requirement that the remainder be \textbackslash (\textbackslash LittleO\textbackslash ) of the last retained term. That condition does not determine the blocks: with \textbackslash (p\_n=L\textasciicircum 5\textbackslash ) and \textbackslash (q\_n=L\textasciicircum 5+L\textasciicircum 3\textbackslash ) the two candidate blocks differ by \textbackslash (L\textasciicircum 3=\textbackslash LittleOAt\{L\textasciicircum 5\}\{X\textbackslash to+\textbackslash infty\}\textbackslash ), so uniqueness fails. The displayed form, and S6's \textbackslash (\textbackslash LittleO(X\textasciicircum \{-N\})\textbackslash ) form, both do determine them.\\
...
\cref{plt:rmk:mul-not-an-ideal} & S6, Theorem "Closure and finite determination", eq. (mult-error-rule). The source states the conclusion as O(t\textasciicircum \{min(p+m\_0, q+n\_0)\}) without any hypothesis relating p,q to the valuations n\_0,m\_0, and its proof silently bounds ord(E*Gtilde) by p+m\_0 and ord(Ftilde*H) by q+n\_0, which requires ord(Gtilde) >= m\_0 and ord(Ftilde) >= n\_0. Neither follows from the stated hypotheses. The corrected statement carries the third entry p+q, and the hypotheses p >= n\_0, q >= m\_0 under which the source's form is recovered.\\
...
\cref{plt:prop:cmp-block-recurrence} & S2 writes the operator in (its) power-leading formula as ":e\textasciicircum \{delta(D-n)\}:" with D previously declared to be d/dL, while the identity requires differentiation in the *argument* of the coefficient polynomial. For rho != 1 the two differ by a factor rho\textasciicircum k and the formula is then wrong. We print partial\_z for the dummy variable and evaluate afterwards.\\
...
\cref{plt:rmk:lag-nearid-meaning} & S1, S2, S3, S4 and S5 all assert only the sufficiency direction ("the r-th term has t-order at least r-1"); none observes that the condition is also necessary, which is what makes the misuse in plt:warn:lag-scale-trap detectable.\\
