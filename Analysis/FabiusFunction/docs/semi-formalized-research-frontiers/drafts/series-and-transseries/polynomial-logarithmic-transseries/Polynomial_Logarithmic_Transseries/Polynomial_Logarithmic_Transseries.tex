...
\cref{plt:eq:dif-block}.  For the reverse inclusion, run the construction of
\cref{plt:thm:dif-antiderivative} below on an \(F\) with \(\ordt F\ge1\): it
produces blocks only at indices \(n\ge0\), so the antiderivative lies in
\(\PLpow\).  The same run on an \(F\) with \(\ordt F\ge2\) meets only
nonresonant indices \(n\ge1\), so its antiderivative lies in \(t\PLpow\).
\end{proof}
