...
> 695-page A4 PDF is a historical publication checkpoint from 4 September
> 2026; the canonical TeX has since changed and no current TeX/PDF parity is
> claimed.
...
The current corpus contains 943 documented Lean modules and 11,787 explicit
public declarations, with no documentation or module-header gaps. Nine focused
modules (72 explicit public commands, plus two named declarations generated by
`to_additive`) now crosswalk the following parts of this volume:

- Exact: the sequence-scale/Poincaré definitions, coefficient limits and
  uniqueness; Dickson's lemma; the analytic power–log dominance trichotomy;
...
- Partial with an explicit boundary: Neumann's lemma needs an `OrderDual`
  ...
  height/depth prose has no datatype; block antidifferentiation is complete on
  the polynomial coefficient operator but not yet in the full integer
  Laurent-block ring; and the quadratic coefficient recurrence is checked
  without the assembled square-root/deepest-pole construction.
