...
> 55,005-line source; the canonical TeX has since grown to 55,300 lines and no
> current TeX/PDF parity is claimed.
...
The current corpus contains 952 documented Lean modules and 11,881 explicit
public declarations, with no documentation or module-header gaps. Eighteen
focused modules contain 166 explicit public commands; two named declarations
generated by `to_additive` bring that inventory to 168 named entries. They now
crosswalk the following parts of this volume:
...
- Exact: the sequence-scale/Poincaré definitions, coefficient limits and
  ...
  lemma; the analytic power–log dominance trichotomy; the ordinary/exponential
  partial-Bell normalization; `p0:lem:bell-conversion`,
  `p0:lem:power-log`, and `p0:cor:exp-log-jets`; the integer block derivative
  identity for a unit power generator; and `p6:prop:quadratic-core-catalan`.
- Partial with an explicit boundary: Neumann's lemma needs an `OrderDual`
  specialization to match the manuscript's greatest-element convention; the
  two displayed height estimates are exact but the general nested
  height/depth prose has no datatype; Wright omega lacks only the real-
  analyticity clause; the harmonic-increment theorem has only its leading
  term; block antidifferentiation and the integer derivative law are proved
  without the concrete Laurent-series ambient/uniqueness layer; the real
