...
\(0<W'<1\) once \(W>0\) (\path{Fabius.inv_exp_mul_add_one_lt_one}).  The convexity of
\(g\) is formal as well: \path{Fabius.strictConvexOn_lambertShiftInv} is its strict convexity on
the open half-line, obtained from the strict growth of \(g'\)
(\path{Fabius.deriv_lambertShiftInv_strictMonoOn}), which in turn is the strict decay of \(W'\)
(\path{Fabius.inv_exp_mul_add_one_strictAntiOn}).  The concavity of \(f\) is not stated as such;
what the corpus carries is the strict concavity of the branch itself,
\path{Fabius.strictConcaveOn_principalLambertW} in \path{LambertWCurvature.lean}, proved there
from \(W''<0\).  The endpoint value \(g'(0)=1/2\), a one-sided derivative, is not formalized.
...
\paragraph{Lean status.}
The polynomial half is formal in \path{LambertInverseCoefficients.lean}: with
\path{Fabius.lambertNumerator} the two-index family generated by the recurrence
(\path{Fabius.lambertNumerator_succ_eq_draft}) from the initial value \(n\), the results
\path{Fabius.natCast_dvd_lambertNumerator}, \path{Fabius.natDegree_lambertNumerator},
\path{Fabius.leadingCoeff_lambertNumerator} and \path{Fabius.eval_zero_lambertNumerator} give
integrality, degree \(2n-2\), leading coefficient \((n-1)!\) and the constant term of \(R_n\),
which is the whole content of the displayed structure.  The identification of that family with
\(A_n\), and hence the limit \(A_n(L)\to1/n\), rest on the operator calculus and are not
formalized.
