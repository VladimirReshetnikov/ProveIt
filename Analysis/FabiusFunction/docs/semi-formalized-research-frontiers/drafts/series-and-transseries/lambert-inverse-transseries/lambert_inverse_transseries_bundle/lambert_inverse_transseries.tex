...
The exact reduction \eqref{eq:exactinverse} is formal in \path{LambertShiftInverse.lean}
(\path{Fabius.lambertShift_lambertShiftInv}, \path{Fabius.lambertShiftInv_eq_mul_exp}, with
\(\rLambert_1\) as \path{Fabius.shiftedLambertW}); the expansion
\eqref{eq:mainexpansion}--\eqref{eq:Akleading} is not formalized.
