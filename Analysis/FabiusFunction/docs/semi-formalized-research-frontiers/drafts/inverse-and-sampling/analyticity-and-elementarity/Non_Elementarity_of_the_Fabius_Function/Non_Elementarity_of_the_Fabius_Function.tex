...
dense open subset of $\RealNumbers$.}
\end{center}
...
is that the one-sided identity $h(g(x))=x$ makes $g$ a right inverse of $h$
and an implicit branch of the simplest possible equation, $h(z)-x=0$, so the
analytic inverse theorem is the implicit function theorem already in hand.
\end{abstract}
...
from Rvachev's $\RvachevUp$ to $F$ and completed at the right endpoint; in the
...
If $f\colon\RealNumbers\to\RealNumbers$ is elementary, then $\An(f)$ is a dense open subset
of~$\RealNumbers$.
\end{theorem}

\begin{corollary}[Main result]\label{cor:main}
Let $U\subseteq[0,1]$ have nonempty interior and let $g\colon\RealNumbers\to\RealNumbers$ be
...
The class $\Elem$ of \emph{elementary functions} $\RealNumbers\to\RealNumbers$ is the smallest
...
Warning~\ref{warn:oddroot}, the classical odd-root functions on all of $\RealNumbers$.
...
Nothing is lost for Corollary~\ref{cor:main}, and this is
...
\emph{Constants and the identity.}  Here $\An(f)=\RealNumbers$.
...
already exists --- yields
 ...
 $C^\infty$ throughout $(0,1)$.  \Cref{thm:nowhere} then supplies the
 contradiction.
...
Variable exponents at a positive base, and at any nonvanishing base
  & \path{IsElementary.rpow_of_pos}, \path{IsElementary.rpow_of_ne_zero}, \path{eqOn_rpow_of_pos}
  & \path{ElementaryFunction} \\
...
Branches of polynomial equations (\Cref{thm:algebraic})
  & \path{analyticDenseOn_of_algebraic}, \path{dense_setOf_analyticAt_of_algebraic}, \path{branchPoly}, \path{AnalyticDenseOn}, \path{analyticDenseOn_of_inter_open_dense}, \path{AnalyticDenseOn.not_eqOn_of_forall_not_analyticAt}
  & \path{AlgebraicBranch} \\
Algebraic functions over $\Elem$ (Corollary~\ref{cor:algebraic})
  & \path{analyticDenseOn_of_isElementary_coeffs}, \path{exists_analyticAt_of_isElementary_coeffs}, \path{not_algebraicBranch_eqOn}, \path{canonical_fabius_not_algebraicBranch_on_Ioo}
  & \path{NotElementary} \\
...
Inverse branches are analytic somewhere (\Cref{thm:invbranch})
  & \path{exists_analyticAt_of_rightInverse}, \path{analyticDenseOn_of_rightInverse}
  & \path{InverseBranch} \\
...
Conditional Lambert-$W$ inverse-branch criterion
  ...
  & \path{InverseBranch} \\
Interior calculus and smoothness of $F^{-1}$
  & \path{fabiusInv_mem_Ioo}, \path{fabiusInv_hasDerivAt}, \path{hasDerivAt_fabiusInv}, \path{deriv_fabiusInv}, \path{deriv_fabiusInv_eq_inv_two_mul_rvachevUp}, \path{deriv_fabiusInv_pos}, \path{fabiusInv_contDiffOn_Ioo}
  & \path{FabiusInverse} \\
Exact global differentiability and smoothness locus of the totalized $F^{-1}$
  & \path{fabiusInv_contDiffAt}, \path{fabiusInv_not_differentiableAt_zero}, \path{fabiusInv_not_differentiableAt_one}, \path{fabiusInv_differentiableAt_iff}, \path{fabiusInv_contDiffAt_iff}, \path{fabiusInv_contDiffAt_infty_iff}, \path{fabiusInv_contDiffOn_compl_endpoints}
  & \path{FabiusInverse} \\
$F^{-1}$ has no critical point, and is nowhere analytic (\Cref{thm:invnowhere})
  & \path{deriv_fabiusInv_ne_zero}, \path{fabiusInv_not_analyticAt}, \path{fabiusInv_analyticAt_iff}
  & \path{InverseNotElementary} \\
$F^{-1}$ is not elementary (Corollary~\ref{cor:invmain})
  & \path{canonical_fabiusInv_not_isElementary_on_Ioo}, \path{fabiusInv_not_isElementary}, \path{not_eqOn_fabiusInv_of_analyticDenseOn}, \path{not_eqOn_fabiusInv_of_dense_analyticLocus}
  & \path{InverseNotElementary} \\
Neither is reachable by the inverse-branch closure (Corollary~\ref{cor:plusmain})
  & \path{not_isElementaryOrInverse_eqOn_fabius}, \path{not_isElementaryOrInverse_eqOn_fabiusInv} (general $U$), \path{canonical_fabius_not_isElementaryOrInverse_on_Ioo}, \path{canonical_fabiusInv_not_isElementaryOrInverse_on_Ioo} ($U=(0,1)$)
  & \path{InverseNotElementary} \\
...
Main result (Corollary~\ref{cor:main})
  & \path{canonical_fabius_not_isElementary_eqOn_of_interior_nonempty} (general $U$), \path{canonical_fabius_not_isElementary_on_Ioo} ($U=(0,1)$)
  & \path{NotElementary} \\
...
General open subset of $[0,1]$
  ...
  & \path{NotElementary} \\
Rvachev's $\RvachevUp$ and the signed extension
...
\item \texttt{AnalyticAt.eventually\_eq\_zero\_or\_eventually\_ne\_zero} and
      ...
      Lemma~\ref{lem:key}.  It applies verbatim to $f\colon\RealNumbers\to\RealNumbers$.
...
\item \texttt{ContDiffAt.analyticAt} together with the family of
      ...
      $\NaturalNumbers\cup\{\infty,\omega\}$: this is how Lemma~\ref{lem:generators} is
      obtained, including the two facts that are otherwise awkward over $\RealNumbers$,
...
The Fabius function is $C^\infty$ on $\RealNumbers$, and its derivatives satisfy the
...
need them.  A set that is open in $\RealNumbers$ and contained in $[0,1]$ is
automatically contained in $(0,1)$ --- an $\RealNumbers$-open set containing $0$
...
contains a nonempty $\RealNumbers$-open subinterval of $(0,1)$.

What the endpoints do buy is exactness.  $\An(F)$ is precisely
$\RealNumbers\setminus[0,1]$: outside the unit interval $F$ is locally constant, hence
