...
% LOCAL-NON-FOURIER-HAT: \FormalTaylorGerm denotes the formal Taylor series of a smooth map at the center stated in context
\newcommand{\FormalTaylorGerm}[1]{\widehat{#1}}
...
\[
 \FormalTaylorGerm{g}(z)=a_1z+a_2z^2+\cdots,
 \qquad
 \FormalTaylorGerm{k}(w)=b_1w+b_2w^2+\cdots,
 \qquad a_1b_1=1.
\]
...
\begin{equation}
 \FormalTaylorGerm{g}\comp\FormalTaylorGerm{k}=w,
 \qquad
 \FormalTaylorGerm{k}\comp\FormalTaylorGerm{g}=z.
 \label{eq:formal-inverse-identities}
\end{equation}
Thus each is the unique formal compositional inverse of the other.

Assume $R_k(y_0)>0$.  Then $\FormalTaylorGerm{k}$ defines an analytic germ with nonzero linear
...
$\FormalTaylorGerm{k}$, hence equals $\FormalTaylorGerm{g}$.  Therefore $R_g(x_0)>0$.  Interchanging $g$
...
\[
 \FormalTaylorGerm{G_n}(\half+h)=\half+2^nh.
\]
...
$\FormalTaylorGerm{G_4}\comp\FormalTaylorGerm{K_4}=\id$ is below $2\times10^{-65}$.
...
\[
 \FormalTaylorGerm{F_{1/4}}(z)=\frac5{72}+z+4z^2,
\]
