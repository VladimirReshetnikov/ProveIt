...
\[
 ...
 \qquad
 \Gzero(y)=\frac{\sqrt T}{\EulerE\sqrt L}\,\EulerE^{-\sqrt{2LT}},
\]
...
analyticity strip $\AbsoluteValue{\Im\theta}<\pi/(2L)$; phase-locked quantile
...
$K=\log\Expectation\,\EulerE^{-tX}$ admits the \emph{exact} decomposition
\[
 K(t)=-\frac{\log^2t}{2L}+\frac12\log t-\CB
 +\Psi(\log_2t)+B(t),
 \qquad
 B(t)=\sum_{n\ge1}\frac{2^{\TwoAdicValuation(n)+1}-1}{n}\,\EulerE^{-nt},
\]
...
$\lambda(x)$ & $-\LambertW_{-1}(-Lx)/L$, so $x=\lambda\EulerE^{-L\lambda}$\\
...
$K(t)$ & $\log\Expectation\,\EulerE^{-tX}$ (Laplace exponent) &
$B(t)$ & dyadic Bose tail \eqref{eq:B-2adic}\\
$\varkappa_r$ & $s^{r}K^{(r)}(s)$ at the saddle &
$\varrho_r$ & $\varkappa_r/\varkappa_2$\\
$\ExponentialCompleteBellPolynomial{n}$, $\ExponentialPartialBellPolynomial{n}{k}$ & complete/partial exponential Bell &
...
\begin{equation}
 \Psi(u)=\sum_{k\ne0}\FourierSeriesCoefficient{\Psi}{k}\,\EulerE^{2\pi\ImaginaryUnit ku},
 \qquad
...
\end{equation}
$\FourierSeriesCoefficient{\Psi}{0}=0$.  The explicit first correction and the forward
top-jet law are
\begin{equation}
...
$(\sum_jd_jz^{j})^{k}
=\sum_{n\ge k}\frac{k!}{n!}\ExponentialPartialBellPolynomial{n}{k}(x_1,x_2,\dots)z^{n}$,
\begin{equation}
 \boxed{\;
 g_n=\frac1{n!}\sum_{k=1}^{n}(-L)^{k}\ExponentialPartialBellPolynomial{n}{k}
 +\frac{\beta}{(n-1)!}\sum_{k=0}^{n-1}(-L)^{k}\ExponentialPartialBellPolynomial{n-1}{k}
 +\frac1{(n-1)!}\sum_{k=1}^{n-1}k\,(-L)^{k-1}\ExponentialPartialBellPolynomial{n-1}{k},
 \;}
 \label{eq:gn-direct}
\end{equation}
all $\ExponentialPartialBellPolynomial{m}{k}=\ExponentialPartialBellPolynomial{m}{k}(x_1,x_2,\dots)$; combined with
...
identity $g_n=\frac1{n!}\ExponentialCompleteBellPolynomial{n}(1!h_1,\dots,n!h_n)$, i.e.\
...
\begin{equation}
 ...
 (2\pi\ImaginaryUnit k_{r,s})^{r}\,\FourierSeriesCoefficient{\Psi}{k_{r,s}},
...
$\AbsoluteValue{\FourierSeriesCoefficient{\Psi}{k}}\le C_\eps\exp[-(\pi^{2}/L-\eps)\AbsoluteValue k]$, and
...
$\AbsoluteValue{\FourierSeriesCoefficient{\Psi}{1}}=1.06271\ldots\times10^{-6}$.
\end{proposition}
...
\begin{equation}
 ...
 \qquad
 \frac{y^{r}G^{(r)}(y)}{G(y)}
 =\ExponentialCompleteBellPolynomial{r}\bigl(H_1,H_2,\dots,H_r\bigr),
...
$G/G_W=\bigl(1+\tfrac{zD}{1+\beta z}\bigr)\EulerE^{-LD}$.
...
\begin{equation}
 ...
 2\AbsoluteValue{\FourierSeriesCoefficient{\Psi}{1}}\,
...
If the $A_j$ extend to a strip $\AbsoluteValue{\Im u}<\sigma$ with
$\Norm{A_j}_\sigma\le CK^{j}j!$, then on every smaller strip the
W-scale $d_m,h_m,g_m$ satisfy
$\Norm\cdot_{\sigma'}\le C'(K')^{m}m!$: W-resummed inversion does not
...
$\BigO(w^{-1}\EulerE^{-2^{\vartheta}})$ (a forward perturbation of size
$\EulerE^{-2^{\lambda}}$, divided by the steep phase derivative $Lw$) and
the saddle-resurgent scale.  For the latter, optimizing the
amplitude law \eqref{eq:amplitude-law} against the Fourier decay
$\AbsoluteValue{\FourierSeriesCoefficient{\Psi}{k}}\asymp\EulerE^{-\pi^{2}\AbsoluteValue k/L}$ places the dominant
...
$\limsup_m(\Norm{g_m}/m!)^{1/m}=8/\pi^{2}$ (after removing finitely
...
\begin{equation}
 G(y)=\frac{\sqrt T}{\EulerE\sqrt L}\,\EulerE^{-\sqrt{2LT}}
 \exp\Bigl\{\sum_{n=1}^{N}
...
$b_n$ shares its statistic $\TwoAdicValuation(n)+1$ with the sinc zero orders;
...
strip, with weight $1+\AbsoluteValue\ell$ for the polynomial part),
$\AbsoluteValue{h_n}+\AbsoluteValue{g_n}\le CA^{n}n!$.  The top-jet transfer
...
a certified proof could isolate the $\AbsoluteValue k=1$ modes against the
enormous decay gap to $\AbsoluteValue k\ge2$.)  At higher orders the
...
$b_n=(2^{\TwoAdicValuation(n)+1}-1)/n$, where $q_\star$ is the \emph{exact}
...
$\inf\AbsoluteValue{\partial_w\mathscr F}\ge L/(2z)$ throughout the relevant
$\BigO(\ell z)$ neighborhood for small $z$; hence
$\AbsoluteValue{D-D^{[N]}}\le(2z/L)\AbsoluteValue{\mathscr F(D^{[N]})}
