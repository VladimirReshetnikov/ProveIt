...
$\overline G:\RealNumbers\to[0,1]$, obtained by clamping the argument to $[0,1]$.
...
modulus, effective uniform continuity, the tolerant-bisection name
transformer, and the final computable-real-function package.  An
...
A post-publication Lean development now formalizes five layers.  The structural
...
subadditivity, attained exact moduli on both $[0,1]$ and $\RealNumbers$, and the exact
...
The generic module
\path{Analysis/FabiusFunction/Lean/FabiusFunction/EffectiveMonotoneInverse.lean}
formalizes computable unit-interval clamping, all three certified comparison
branches, a fixed-depth natural-state bisection realizer, its uniform
dyadic-name error, and restricted sequential inversion from a computable
inverse modulus.  The concrete module
\path{Analysis/FabiusFunction/Lean/FabiusFunction/FabiusInverseComputable.lean}
instantiates that construction with the Fabius spline oracle and Delta
denominator, then combines it with the logarithmic modulus to prove
\texttt{IsComputableRealFunction (fabiusInv F hF)}.
...
minimality, the stronger abstract principle that derives an inverse modulus
from a computable positive gap sequence, and the input-precision asymptotics remain
...
module headers.  The present merged tree now scans 629 modules and 8,546
...
Its centered density is Rvachev's $\RvachevUp$ function, with Fourier transform an
...
requested definition at the paper level.  The post-publication modules now
formalize those clauses exactly; \cref{sec:lean-status} records the complete
name-level crosswalk.
...
denominators.  \path{EffectiveMonotoneInverse.lean} now supplies the
tolerant-bisection realizer and its computable name transformer, while
\path{FabiusInverseComputable.lean} proves the final totalized inverse theorem.
The report's probabilistic event proof, exact ceiling modulus, and stronger
gap-derived abstract inversion principle remain on the human-proof side of the
boundary recorded in \cref{sec:lean-status}.
...
$\FabiusGlobal$, agrees with $F$ on $[0,1]$ but oscillates outside it and obeys
global dilation/translation identities.  Rvachev's $\RvachevUp$ is an even compactly
...
\eqref{eq:absolute-inverse-gap-bound}.  If $\AbsoluteValue{u-v}\le\eta$, monotonicity
...
\eqref{eq:absolute-inverse-gap-bound}; then $v-u=\AbsoluteValue{v-u}$.  If $v<u$, the
...
For $N=8$, interval length $h=1/8$, and the dyadic grid $2^{-8}\IntegerNumbers$, the script
...
$(-1)^{\BinaryDigitSum(j)}$ the exact inclusion--exclusion coefficient of the finite
...
Equation \eqref{eq:sinc-product} gives the Fourier transform of $\RvachevUp$, in
...
The corpus expands $\RvachevUp$ in even Legendre polynomials and represents
...
$r(n)=\min\{r\in\NaturalNumbers:n<2^r\}$ from \cref{thm:reciprocal-modulus}.\\
\addlinespace
...
the exact ceiling $d_*$, or input-bit asymptotics.  Tolerant bisection and the
combined computable-real-function theorem are supplied by the two downstream
modules below.

\subsection{The completed tolerant name-transformation layer}

The representation-generic realizer is
\begin{center}
\path{Analysis/FabiusFunction/Lean/FabiusFunction/}\\
\path{EffectiveMonotoneInverse.lean}.
\end{center}
Its two definitions and six theorems are exhausted by the following table.

\begin{longtable}{@{}p{0.43\textwidth}p{0.52\textwidth}@{}}
\toprule
Lean declaration & Exact role \\
\midrule
\endhead
\path{Fabius.SequentiallyComputableOn} &
Sequential computability restricted to a specified set.\\
\path{Fabius.unitClamp} &
The canonical projection of a real input into $[0,1]$.\\
\path{Fabius.unitClamp_sequentiallyComputable} &
Clamping transforms every computable real sequence into a computable one.\\
\path{Fabius.tolerantDifference_error} &
Two $\delta/8$ oracle errors give a $\delta/4$ signed-difference certificate.\\
\path{Fabius.tolerantDifference_safe_updates} &
Either strict comparison beyond $\delta/2$ selects a sound half-bracket.\\
\path{Fabius.tolerantDifference_inconclusive} &
The third comparison branch is already within the requested inverse modulus.\\
\path{Fabius.tolerantBisection_correct} &
The fixed-depth natural-state loop computes one uniform dyadic output name and
proves error at most $2^{-p}$ at precision $p$.\\
\path{Fabius.effectiveInversionOn_Icc} &
A computably approximable strictly increasing bijection of $[0,1]$, supplied
with a computable positive reciprocal inverse modulus, has a sequentially
computable inverse on $[0,1]$.\\
\bottomrule
\end{longtable}

The Fabius-specific wrapper is
\begin{center}
\path{Analysis/FabiusFunction/Lean/FabiusFunction/}\\
\path{FabiusInverseComputable.lean}.
\end{center}
Its sole public theorem,
\path{Fabius.fabiusInv_isComputableRealFunction}, instantiates the generic
loop with the centered-spline oracle and the Delta denominator, composes input
names with computable clamping, and uses the logarithmic Delta theorem for the
effective-uniform-continuity field.  Thus it proves both clauses of
\texttt{IsComputableRealFunction (fabiusInv F hF)} for every bounded Fabius
solution $F$ satisfying the defining equations.
...
\path{FabiusInverseLogarithmicModulus.lean}, and the generic and concrete
name-transformer steps are complete in the two modules above.  The remaining
order is:
...
\item Strengthen \path{effectiveInversionOn_Icc} by deriving its computable
      inverse-modulus argument directly from a computable positive
      lower-increment sequence, thereby matching
      \cref{thm:abstract-inversion} without a prepackaged modulus.
\item Formalize the flatness bounds
...
The representation-oriented realizer now lives in
\path{EffectiveMonotoneInverse.lean}; its theorem accepts an already packaged
computable inverse modulus.  The remaining strengthening required for the
full abstract \cref{thm:abstract-inversion} is to derive that argument from an
oracle for $f$ and a recursive lower-separation function
\[
  \alpha(p)>0,
  \qquad x+2^{-p}\le b
  \Longrightarrow f(x+2^{-p})-f(x)\ge\alpha(p).
\]
The existing realizer can then be reused without changing its loop or
correctness invariant.  Tolerant bisection is already library infrastructure usable for distribution quantiles,
...
$F(2^{-p})\in\RationalNumbers$ make the sequence amenable to arbitrary-precision
...
The current Lean development now closes both clauses of computability.
...
reciprocal moduli. \path{EffectiveMonotoneInverse.lean} certifies the
tolerant-bisection name transformer, and
\path{FabiusInverseComputable.lean} proves the final
computable-real-function package. The probabilistic box-event proof,
exact-ceiling modulus, gap-derived abstract inversion strengthening, and
precision asymptotics remain to be formalized.
...
$\AbsoluteValue{F(c)-y_i}<3\delta/4<\delta\le F(h)$, so
\cref{cor:effective-injectivity} gives
$\AbsoluteValue{c-G(y_i)}<h<2^{-p}$.  If no acceptance occurs, every update is
...
\cref{thm:bisection-correct,cor:GT-sequential}.  Exact counterparts are
\path{Fabius.tolerantBisection_correct},
\path{Fabius.effectiveInversionOn_Icc}, and the sequential field of
\path{Fabius.fabiusInv_isComputableRealFunction}.\\
\addlinespace
...
witness.  It does not itself formalize the exact ceiling $d_*$, tolerant
bisection, a sequential realizer, or input-bit asymptotics.  Every public declaration is
crosswalked in \cref{sec:lean-status}.\\
\addlinespace
\path{EffectiveMonotoneInverse.lean} & Generic computable unit-interval
clamping, certified tolerant comparison, a fixed-depth natural-state
bisection name transformer, its uniform $2^{-p}$ error theorem, and restricted
sequential inversion from a computable reciprocal inverse modulus.\\
\addlinespace
\path{FabiusInverseComputable.lean} & Fabius-specific instantiation of the
generic realizer and the final theorem
\path{Fabius.fabiusInv_isComputableRealFunction}, combining sequential
computability with the logarithmic Delta effective-continuity witness.\\
\addlinespace
