...
The inverse-computability report's retained PDF predates its current merged
source and is explicitly stale. The other retained PDFs are likewise not
claimed to render their current listed sources.
...
| `inverse-asymptotics-and-computability/Inverse_Fabius_Computability_Report` | 2,992 | `fde44c6ddf4f0e58a813e39a22a46eac62f71ea2f444fad74530d268b9cd9f00` | retained 42 pages; explicitly stale, rebuild pending | all six current payload rows pass without asserting synchronization |
...
- [`Inverse_Fabius_Computability_Report/`](inverse-asymptotics-and-computability/Inverse_Fabius_Computability_Report/),
  *Computability of the Inverse Fabius Function* (retained historical
  42-page A4 PDF; current 2,992-line source), arrived on
  ...
  The generic
  [`EffectiveMonotoneInverse.lean`](../../../../Lean/FabiusFunction/EffectiveMonotoneInverse.lean)
  layer now supplies computable unit-interval clamping, all three certified
  comparison branches, a fixed-depth natural-state tolerant-bisection
  realizer, its uniform dyadic error theorem, and restricted sequential
  computability. The concrete
  [`FabiusInverseComputable.lean`](../../../../Lean/FabiusFunction/FabiusInverseComputable.lean)
  instantiation combines that realizer with the report-exact Delta modulus
  and proves `IsComputableRealFunction (fabiusInv F hF)`. The probabilistic
  box-event proof itself, exact ceiling `d_*`, the stronger abstract theorem
  deriving an inverse modulus from a computable positive gap sequence, and
  precision asymptotics remain open Lean work. The `d_*` claim
  remains denominator-minimal only for the fixed dyadic proxy `2^{-r(n)}`, not
  the weaker target `1/n`. The current 2,992-line source has SHA-256
  `fde44c6ddf4f0e58a813e39a22a46eac62f71ea2f444fad74530d268b9cd9f00`;
  the retained 42-page A4 PDF has 712,447 bytes and SHA-256
  ...
  source now carries the 643/8,661 census. The retained PDF is explicitly
  stale against that merged source and requires a fresh render. The immutable
...
bytes; SHA-256
`755d77354490d25d4f327419d0345623e91ea49dd4ba681ba97c84a0b686b8c1`).
The current canonical source has 2,992 lines (SHA-256
`fde44c6ddf4f0e58a813e39a22a46eac62f71ea2f444fad74530d268b9cd9f00`).
The retained historical 42-page A4/27 mm/Libertinus PDF has 712,447 bytes
(SHA-256
`dc8da9d476d6b5330aad0ac3253bab162ddcb944653b038bc0aede4be4fc3fe1`).
It was built from an earlier source checkpoint in exactly three clean
strict serial passes with all 22 font rows embedded/subset, six Libertinus
rows, and no Type 3 fonts; it is explicitly stale and the current source
requires a fresh render. The report gives
...
`Delta_r` estimate and recursive-modulus packaging are joined by the generic
`EffectiveMonotoneInverse` realizer and the concrete
`fabiusInv_isComputableRealFunction` theorem. The exact ceiling modulus, the
stronger abstract gap-to-modulus inversion principle, and input-bit
asymptotics remain paper-level. The
...
643 Lean modules and 8,661 public declarations with zero documentation/header
gaps.
