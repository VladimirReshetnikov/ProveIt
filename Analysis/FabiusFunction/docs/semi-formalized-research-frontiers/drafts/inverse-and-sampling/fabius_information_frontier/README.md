...
> A refreshed 19-entry `SHA256SUMS` verified those package bytes at the recorded
> merge checkpoint. Package-local ledgers were retired repository-wide on
> 2026-09-01 and remain recoverable from Git history; that historical receipt
> did not assert source/PDF synchronization.
...
ledger verified 18/18 before normalization and was preserved byte-for-byte as
`SHA256SUMS.arrival.txt`; a refreshed 19-entry operational ledger then covered
the normalized package while retaining the submitted PDF as a distinct
historical payload. Both package-local ledger files were retired on 2026-09-01
and remain recoverable from Git history. Four CSV files are stored with
repository-standard LF endings.
