...
% LOCAL-NON-FOURIER-HAT: \GeometricPrefixReproduction denotes the deterministic finite-prefix reproduction in the squared-error test channel
\newcommand{\GeometricPrefixReproduction}[1]{\widehat{X}_{#1}}
...
\[
 ...
 \qquad
 \operatorname{mmse}(X_q\mid S_{q,m})=q^{2m}\Variance(X_q),
\]
and, strikingly,
\[
...
\[
 h(X_q)-h(S_{q,m})
 =\frac{\Variance(X_q)\,\mathcal J(f_q)}2q^{2m}
  +o(q^{2m}),
\]
...
\begin{equation}
 \phi_q(t)=\CharacteristicFunction{X_q}(t)
 =\Expectation\EulerE^{\ImaginaryUnit tX_q}
...
Use the deterministic reproduction $\GeometricPrefixReproduction{m}=S_{q,m}$.  Its rate and
distortion are
\begin{equation}
 R_m:=I(X_q;\GeometricPrefixReproduction{m})=m\log(1/q),
 \qquad
 D_m:=\Expectation(X_q-\GeometricPrefixReproduction{m})^2=q^{2m}\sigma_q^2.
 \label{eq:RD-points}
\end{equation}
Therefore
\begin{equation}
...
\begin{proof}
The upper bound is achieved by $\GeometricPrefixReproduction{m}=S_{q,m}$.  Shannon's lower bound for
squared error gives
\[
...
finite sinc product
\begin{equation}
 \CharacteristicFunction{S_{q,m}}(t)
...
For $0\le j<2^m$, let
\begin{equation}
 \epsTM_j=(-1)^{\BinaryDigitSum(j)},
 \label{eq:TM-sign}
\end{equation}
where $\BinaryDigitSum(j)$ is the binary digit sum.
...
\begin{equation}
 \CharacteristicFunction{S_m}(t)=\prod_{k=1}^{m}\SincRad(t/2^k).
...
$2\sum_{k\in E}2^{-k}=j/2^{m-1}$ and $(-1)^{|E|}=(-1)^{\BinaryDigitSum(j)}$.
...
\[
 \CharacteristicFunction{S_{q,m}}(t)=\frac{\phi_q(t)}{\phi_q(q^mt)}.
\]
Near the origin,
\[
...
$\FourierAngular{f_q''}(t)=-t^2\phi_q(t)$ give
...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{entropy_deficit.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{prefix_entropy_gap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{information_spectrum.png}
