...
\hypersetup{
  ...
  pdfsubject={inverse Fabius function, compositional iterates, formal power-series reversion, Rvachev up-function, Thue--Morse sequence},
...
The requested theorem is proved in full.  The proof is self-contained after the
repository-established derivative atlas, shape theory, endpoint asymptotic, and base
inverse-function facts.  Its new ingredients are a composition-stable Fa\`a di Bruno
``defect'' estimate, the resulting forward orbit-spine asymptotic, and a formal
power-series reversion theorem that transfers zero Taylor radius to inverse iterates.
The argument and numerical diagnostics have been checked, but the new iterate theorems
...
No audited canonical source states the all-$n$ theorem for $F^{\comp n}$ or for
$I^{\comp n}$.  The new proof has two layers.  Sections
\ref{sec:defect}--\ref{sec:proof-main} prove the forward iterate theorem by a
composition-stable orbit-spine asymptotic.  Section \ref{sec:inverse-transfer} then
proves a general theorem that convergence of a formal Taylor series is invariant under
smooth local inversion with nonzero derivative.  This immediately transfers the dense
zero-radius set from $G_n$ to $K_n$ and gives a second, stronger route to the requested
conclusion.
...
$D(\pi)\geq e(3m/4-1)$.  For fixed $K$, the logarithm of the counting and weight
factor is $\bigO(e\log m)$, whereas the negative dyadic exponent is
$-\Omega(em)$.  Summing over $e$ therefore gives $\bigO(\rho^m)$ for some
$\rho<1$.
...
$D(\pi)\geq t(3m/4-1)$.  The same comparison, $\bigO(t\log m)$ versus
$-\Omega(tm)$, gives another exponentially decaying sum.
...
\label{lem:two-spine}
Let $f$ and $h$ be smooth near $x$, and put $y=h(x)$, $B=\abs{h'(x)}$.  Suppose
...
\begin{equation}
 T_n:=\bigcup_{k=0}^{n-2}G_k^{-1}\set{u,1-u}.
 \label{eq:tie-set}
\end{equation}
Each $G_k$ is a homeomorphism of $[0,1]$, so $T_n$ is finite, with at most
$2(n-1)$ points.
...
recovers $F(x)=\Up(x-1)$ on $(0,1)$ and the signed blocks in
...
$\Up$.  Forward iterates become increasingly step-like about $1/2$ as $n$ grows, while
...
homeomorphic image of the forward classification.

\begin{conjecture}[Exact Taylor-radius classification]
\label{conj:trichotomy}
For each fixed $n$, the positive-radius locus of $G_n$ is precisely the set of points at
which the high-order jet is eventually zero.  Equivalently, no point has a genuinely
infinite convergent Taylor series that fails to represent $G_n$.  By
\cref{thm:formal-reversion}, the same statement then holds for $K_n$, with the loci
transported by $G_n$.
\end{conjecture}

The center belongs to the eventually-zero class: both forward and inverse formal series
are affine.  For $n=1$, dyadic points give further finite Taylor polynomials.  For
$n\geq2$, exact cancellation among several orbit levels makes the arithmetic
classification substantially more delicate.

\begin{conjecture}[No persistent cancellation at a forward tie]
\label{conj:ties}
Suppose $x\in T_n$, so two adjacent forward spine weights are maximal.  Then their two
leading symbolic amplitudes do not cancel to exponential accuracy for all sufficiently
large derivative orders.  More precisely, along an infinite sequence $m_k$ their sum
has absolute value at least $cH_n(x)^{m_k}$ for some $c>0$.
\end{conjecture}

A proof would resolve the most visible exceptional mechanism in the forward theorem
and, by formal reversion, the corresponding inverse points.  The two orbit points are
related by one application of $F$, while both amplitudes sample the same dyadic shift;
this is a nonlinear Thue--Morse correlation problem.
...
to explicit derivative lower bounds for the inverse iterates.
...
A sharp version would give an explicit rate in the two-spine lemma, uniform estimates
away from tie surfaces, and the first correction term for both forward and prospective
inverse spines.
...
\begin{itemize}[leftmargin=2em]
 ...
 \item \path{Convexity.lean}, \path{StrictMonotonicity.lean}, and the order-isomorphism
