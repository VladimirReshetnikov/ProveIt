...
\begin{align}
 ...
 &=\prod_{j=1}^n\frac{z-q^j}{1-q^j},
 \label{g1:eq:row-polynomial}\\
 \Gc_{n,q}=W_{n,q}(T_q)
 &=\frac{1}{(q;q)_n}\prod_{j=1}^n(T_q-q^jI),
...
\begin{equation}
 ...
 =A^\alpha q^{\alpha n}\frac{(q^{1-\alpha};q)_n}{(q;q)_n}.
...
\begin{equation}
 ...
 \le Cq^{\binom{n+1}{2}}\frac{t^{n+1}}{(t;q)_{n+1}}.
...
\subsection{The singular regime \texorpdfstring{$q\uparrow1$}{q approaching 1}}
...
\subsection{A \texorpdfstring{$q$}{q}-Pochhammer scaling limit}
...
\subsection{The boundary B-spline and its \texorpdfstring{$q$}{q}-exponential scale}
...
\subsection{Application to \texorpdfstring{$\RvachevUp$}{the Rvachev up function}}
...
\subsection{The double-scaling transition
\texorpdfstring{$q\uparrow1$, $n\to\infty$}{q approaching 1, n approaching infinity}}
...
\end{equation}
has an even $C^\infty$ density $u_q$ supported on $[-1,1]$.  Use the Fourier
convention
\[
 \FourierTwoPi{f}(\xi)
 :=\int_\RealNumbers f(x)\EulerE^{-2\pi\ImaginaryUnit\xi x}\Differential x.
\]
Then
\begin{equation}
