...
\begin{equation}
 h^{n}\sum_{k=-M}^{M}\binom{M-k+n-1}{n-1}(kh)^{p}\,\RvachevUp(kh)
 =\frac{1}{(n-1)!}
 \sum_{i=0}^{n-1}(-1)^{i}
...
$\Norm{F-P_{2^{m}}}_\infty\to\infty$; more strongly,
$\tfrac1M\log_2\Norm{F-P_M}_\infty\to1$ along dyadic $M$, possibly
