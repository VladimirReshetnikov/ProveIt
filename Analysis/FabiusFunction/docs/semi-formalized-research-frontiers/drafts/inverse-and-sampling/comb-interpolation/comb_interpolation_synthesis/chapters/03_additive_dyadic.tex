...
With $\widehat f(\xi)=\int f(x)\EulerE^{-2\pi\ImaginaryUnit\xi x}\Differential x$ and
...
$\widehat g_p(n)=\int_0^1g_p(x)\EulerE^{-2\pi\ImaginaryUnit nx}\Differential x$.  The periodic
...
\begin{equation}
 T_{m,p}=\sum_{\ell\in\IntegerNumbers}\widehat g_p(M\ell),
 \label{eq:trap-alias}
\end{equation}
...
$\widehat g_p(n)=\Expectation\int_Y^1x^{p}\EulerE^{-2\pi\ImaginaryUnit nx}\Differential x$.  Integrating by
...
$\widehat f_p(\xi)=(-2\pi\ImaginaryUnit)^{-p}\Phi^{(p)}(\xi)$, gives the exact
...
Schwartz map with $(-2\pi\ImaginaryUnit)^{p}\widehat{f_p}=(\widehat{f_0})^{(p)}$,
...
with $\widehat{\RvachevUp_b}(\xi)=\prod_{j\ge0}\SincPi(\xi/b^{j})$ (an
...
\begin{align}
 ...
 &=\sum_{\ell\ne0}\EulerE^{2\pi\ImaginaryUnit\ell\theta}\,\widehat{R_P}(M\ell),
 \notag\\
 \widehat{R_P}(q)
...
$F(x)=\Expectation\,\mathbf 1_{Y\le x}$ and Faulhaber's formula for the random
...
\(1-2t^2/9\le\prod_{n}\operatorname{sinc}(t/2^n)\) for \(t^2\le6\)
...
\(\lvert\operatorname{sinc}(mx)\rvert\le\lvert\operatorname{sinc}x\rvert\) for \(m\ge1\)
...
\(\Phi(x)=\prod_{n\ge0}\operatorname{sinc}(\pi x/2^n)\)
...
\begin{align}
 ...
 &=b^{-N}\sum_{j=0}^{b^N-1}
\left(1-b\,\mathbf 1_{\{j\equiv\varepsilon_N\ (\mathrm{mod}\ b)\}}\right)
 f\!\left(\frac{j+\theta_N}{b^N}\right)
...
indicator gives
$1-b\mathbf 1_{\{j\equiv\varepsilon_N\}}=
