...
\begin{equation}
 \FourierSeriesCoefficient{g_p}{n}
 =\sum_{j=0}^{p}
 \frac{\fall pj}{(2\pi\ImaginaryUnit n)^{j+1}}
 \left(
...
\[
 ...
 =\sum_{j=0}^{p}\frac{\fall pj}{(2\pi\ImaginaryUnit n)^{j+1}}
...
\begin{equation}
 ...
 \sum_{j=0}^{p}
 \frac{\fall pj}{(2\pi\ImaginaryUnit M\ell)^{j+1}}\,
...
\[
 -\frac{\fall p{2r-1}}{(2\pi\ImaginaryUnit M)^{2r}}\sum_{\ell\ne0}\ell^{-2r}
...
finite-rank form
\[
 \sum_{r\le d+1}\frac{\fall d{r-1}}{r!}\,B_r(\theta)\,2^{-mr}.
\]
Thus phase
...
\begin{equation}
 ...
 =\sum_{r=s}^{d+1}{\fall d{r-1}\over r!}
...
\begin{equation}
 ...
 \mathcal A^{\mathrm{raw}}_{M,j/b^s}[x^d]
 =\sum_{r=\rho s}^{d+1}{\fall d{r-1}\over r!}
 \delta_{b,s,r}M^{-r}.
...
\[
 Q_{M,\theta,p}=I_p+
 \sum_{r=1}^{p+1}{\fall p{r-1}\over r!}B_r(\theta)M^{-r}.
\]
...
\[
 ...
 {B_{2r}\over(2r)!}\fall p{2r-1}h^{2r}.
\]
...
the one-periodic extension is $\fall\alpha{r-1}$, because all positive
...
$\fall{Mx}{k}=\sum_{j=0}^k s(k,j)M^jx^j$, its $r$th derivative at zero
