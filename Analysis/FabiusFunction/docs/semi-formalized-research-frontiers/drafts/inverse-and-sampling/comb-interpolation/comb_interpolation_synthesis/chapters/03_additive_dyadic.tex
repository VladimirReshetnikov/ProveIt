...
\begin{equation}
 ...
 ={}&\sum_{j=0}^{\Floor{m/2}}\binom m{2j}c_j
     ...
     \mathcal K_{m-2j,r}^{(M)}(a), \\
 \mathcal K_{d,r}^{(M)}(a)
 :={}&\sum_{s=0}^{d}\binom ds2^{s}(-1)^{d-s}
...
\begin{proof}
Solve \eqref{eq:odd-defect-series} for $D(m+1)$ and set $m=2r-1$.
The prefactor then becomes
\begin{equation*}
 \begin{aligned}
 &\frac{(-1)^{(m+1)/2}(2\pi)^{m+1}2^{m(m+1)}}{2\,m!} \\
 &\qquad=\frac{(-1)^{r}2^{(m+1)^2}\pi^{m+1}}{2\,m!}.
 \end{aligned}
\end{equation*}
The defect on the right of \eqref{eq:odd-defect-series} is rational by
...
\[
 0.548311\ldots,\qquad 0.553187\ldots,\qquad
 0.553707\ldots,\qquad 0.553764\ldots;
\]
they converge to $\Phi(1/2)=0.553771\ldots$, as
...
($\OrderOperator_{2^{m}\ell}\Phi=v_2(2^{m}\ell)+1>m$),
...
\begin{proof}
Put $x=2\pi\theta$.  For even $d$ the defect is, up to a nonzero
constant,
\[
 S(x)=\Phi(q/2)\sin x
 +\sum_{\substack{\ell\ge3\\ \ell\ \mathrm{odd}}}
   \Phi(q\ell/2)\,\ell^{-(d+1)}\sin(\ell x).
\]
Since $\AbsoluteValue{\sin(\ell x)}\le
\ell\AbsoluteValue{\sin x}$, the tail has modulus at most
\[
 \AbsoluteValue{\sin x}
 \sum_{\ell\ge3}\AbsoluteValue{\Phi(q\ell/2)}\ell^{-d}
 <\AbsoluteValue{\Phi(q/2)}\AbsoluteValue{\sin x}
\]
by \cref{lem:multiple-angle}.  Thus $S$ has the sign of
$\Phi(q/2)\sin x$ wherever $\sin x\ne0$, and its zeros are exactly
those of $\sin x$.  For odd $d$, replace sines by cosines and use
$\AbsoluteValue{\cos(\ell x)}\le
\ell\AbsoluteValue{\cos x}$, valid for odd $\ell$ after shifting $x$
by $\pi/2$.  The Rvachev comb case is the $q=1$ series
\eqref{eq:first-failure}, with the same parities.
\end{proof}
...
\[
 Q_{M,\theta}[f]:=\frac{1}{M}\sum_{a=0}^{M-1}
 f\!\left(\frac{a+\theta}{M}\right).
\]
Then
\begin{equation}
...
($0,\tfrac12,1$; degrees $\le3$), Boole
($0,\tfrac14,\tfrac12,\tfrac34,1$; degrees $\le5$), and shifted
Gauss--Legendre rules ($s$ points; degrees $\le2s-1$) are immediate
...
At dyadic resolution $M=2^{m}$, the error has the exact separated
finite-rank form
\[
 \sum_{r\le d+1}
 \frac{\fall d{r-1}}{r!}\,B_r(\theta)\,2^{-mr}.
\]
Thus phase acts on the Bernoulli basis while each resolution step
scales the $r$th mode by $2^{-r}$ (and a Ruffa layer scales it by
$1-2^{r}$ at the fine mesh).  Phase filters and resolution filters
therefore commute and can be designed independently.

On the resolution side, general-phase rules carry \emph{all} modes
$2^{-r}$ for $1\le r\le d+1$.  The $q=\tfrac12$ Lagrange weights at
geometric nodes are
\[
 w_j^{(R)}
 =\frac{(-1)^{R-j}2^{-\binom{R-j+1}2}}
        {(\tfrac12;\tfrac12)_R}
   \genfrac{[}{]}{0pt}{}{R}{j}_{1/2};
\]
they delete those modes exactly.  This is the general-$\theta$
companion of the parity-restricted annihilator
\eqref{eq:annihilator}, and it is the $q$-binomial machinery of the
exponents volume specialized to this finite mode set.
...
Then
\begin{equation}
 \boxed{\sum_{r\ge0}\gamma_{b,s,r}^{(\omega)}\frac{z^r}{r!}
 =\frac{z}{\EulerE^z-1}
...
\begin{equation}
 ...
 =\sum_{r=s}^{d+1}\frac{\fall d{r-1}}{r!}
...
\begin{align*}
 \sum_{r\ge0}\gamma_{b,s,r}^{(\omega)}\frac{z^r}{r!}
 &=\frac{z}{\EulerE^z-1}
...
Then
\begin{equation}
 \sum_{r\ge0}\delta_{b,s,r}\frac{z^r}{r!}
 =\frac{z}{\EulerE^z-1}\prod_{\ell=1}^s C(z/b^\ell),
...
\begin{equation}
 ...
 \mathcal A^{\mathrm{raw}}_{M,j/b^s}[x^d]
 =\sum_{r=\rho s}^{d+1}\frac{\fall d{r-1}}{r!}
 \delta_{b,s,r}M^{-r}.
...
\begin{equation}
 Q_N^{(b,\alpha)}[f]
 :=b^{-N}\sum_{j=0}^{b^N-1}
 f\!\left(\frac{j+\theta_N}{b^N}\right).
...
\[
 \frac{k+\theta_{N-1}}{b^{N-1}}
 =\frac{bk+\varepsilon_N+\theta_N}{b^N}.
\]
...
\[
 Q_{M,\theta,p}:=\frac{1}{M}\sum_{j=0}^{M-1}
 \left(\frac{j+\theta}{M}\right)^p
 F\!\left(\frac{j+\theta}{M}\right),
 \qquad h_N:=b^{-N}.
\]
...
\[
 Q_{M,\theta,p}=I_p+
 \sum_{r=1}^{p+1}\frac{\fall p{r-1}}{r!}B_r(\theta)M^{-r}.
\]
...
\begin{align}
 ...
 \Aboxed{I_{\bm p}^{(d)}
 &=\prod_{j=1}^d\left(Q_{M_j,\theta_j,p_j}
...
\begin{proof}
The one-dimensional stable identity is
...
\subsection{A \texorpdfstring{$2$}{2}-adic law for the barycentric weights}
...
\[
 ...
 \text{more strongly,}\qquad
 \frac1M\log_2\Norm{F-P_M}_\infty\longrightarrow1
 \end{gathered}
\]
along dyadic $M$, possibly with a subexponential, dyadically
...
\subsection{Lambert-\texorpdfstring{$W$}{W} appearances}
