\part{Additive dyadic foundations}
...
\begin{equation}
 F(x)=\Probability(Y\le x)\qquad(0\le x\le1),
 \label{eq:F-def}
\end{equation}
and Rvachev's function $\RvachevUp$ is the density of $X$: the unique even
$C^\infty$ function supported on $[-1,1]$ with
$\int_{-1}^1\RvachevUp=1$.  The two are folds of each other,
\begin{equation}
 \RvachevUp(x)=F(1-\AbsoluteValue x)\quad(\AbsoluteValue x\le1),
 \qquad\text{so}\qquad
 \RvachevUp(x)=1-F(x)\quad(0\le x\le1),
 \label{eq:up-fold}
\end{equation}
and the Fabius density is a rescaled copy of $\RvachevUp$:
\begin{equation}
 \rho(x):=F'(x)=2\RvachevUp(2x-1).
...
\begin{equation}
 ...
 \quad(q\ge1),
 \qquad
 \RvachevUp^{(q)}(\pm1)=0,\quad
 (\RvachevUp-1)^{(q)}(0)=0\quad(q\ge0).
 \label{eq:flatness}
\end{equation}
...
$\FabiusGlobal$ of $F$ (the solution of $\FabiusGlobal'(x)=2\FabiusGlobal(2x)$ on
all of $\RealNumbers$) gives the derivative tower and the sharp norms
\begin{equation}
 \FabiusGlobal^{(q)}(x)=2^{\binom{q+1}{2}}\FabiusGlobal(2^qx),
 \qquad
 \Norm{F^{(q)}}_{\infty,[0,1]}
 =\Norm{\RvachevUp^{(q)}}_{\infty,[-1,1]}
...
With $\FourierTwoPi{f}(\xi)=\int f(x)\EulerE^{-2\pi\ImaginaryUnit\xi x}\Differential x$ and
...
\begin{equation}
 \OrderOperator_{\xi=M\ell}\chi=m+\TwoAdicValuation(\ell)
 \qquad(\ell\ne0),
...
\begin{equation}
 c_r:=\int_{-1}^{1}x^{2r}\RvachevUp(x)\Differential x=\Expectation X^{2r},
 \qquad c_0=1;
 \label{eq:c-def}
\end{equation}
...
\begin{equation}
 ...
 \qquad
 \Expectation X^{n}
 =\ExponentialCompleteBellPolynomial{n}(0,\kappa_2,0,\kappa_4,\dots),
 \label{eq:cumulants}
\end{equation}
one of several places where complete Bell polynomials organize the
dyadic structure.

\emph{Fabius-law moments.}  For $s\in\ComplexNumbers$ put
\begin{equation}
 d_s:=\Expectation Y^{s}=\int_0^1x^{s}\rho(x)\Differential x.
 \label{eq:d-def}
\end{equation}
...
\emph{Fabius monomial integrals.}  For $p\in\NaturalNumbers$, integration by parts
gives
\begin{equation}
 I_p:=\int_0^1x^pF(x)\Differential x
 =\frac{1-d_{p+1}}{p+1}\in\RationalNumbers,
 \label{eq:Ip}
\end{equation}
and the same formula defines the entire continuation
\begin{equation}
 I(\alpha):=\int_0^1x^\alpha F(x)\Differential x
 =\frac{1-d_{\alpha+1}}{\alpha+1}
 \qquad(\alpha\in\ComplexNumbers),
 \label{eq:Ialpha}
\end{equation}
the singularity at $\alpha=-1$ being removable with
\begin{equation}
 I(-1)=-\left.\frac{\Differential}{\Differential s}d_s\right|_{s=0}
 =\Expectation\bigl[-\log Y\bigr]
 =0.7582400\ldots.
 \label{eq:Iminus1}
\end{equation}
...
\begin{equation}
 ...
 =\frac{\EulerE^{t}}{t}-\frac{\EulerE^{t}\MM(t)}{\EulerE^{t}-1},
 \label{eq:I-egf}
\end{equation}
and consequently
\begin{equation}
 I_p=\frac1{p+1}\left[
 1-\sum_{j=0}^{p+1}\binom{p+1}{j}\Expectation X^{j}\,B_{p+1-j}(1)
 \right].
 \label{eq:I-closed}
\end{equation}
\end{theorem}
...
\begin{proof}
Set $A(t)=\int_0^1\RvachevUp(x)\EulerE^{tx}\Differential x$.  Evenness of $\RvachevUp$ gives
$A(t)+A(-t)=\MM(t)$, while the partition identity
$\RvachevUp(x)+\RvachevUp(1-x)=1$ on $[0,1]$ (a restatement of
\eqref{eq:up-fold} and \eqref{eq:F-symmetry}) gives
$A(t)+\EulerE^{t}A(-t)=(\EulerE^{t}-1)/t$.  Solving the linear system,
$A(-t)=1/t-\MM(t)/(\EulerE^{t}-1)$.  By \eqref{eq:up-fold},
$\int_0^1F(x)\EulerE^{tx}\Differential x=\EulerE^{t}A(-t)$, which is \eqref{eq:I-egf}.
Expanding $t\EulerE^{t}/(\EulerE^{t}-1)=\sum_nB_n(1)t^n/n!$ and
$\MM(t)=\sum_j\Expectation X^j\,t^j/j!$ gives \eqref{eq:I-closed}.
\end{proof}

The positive-half Rvachev moments follow from the same computation:
\begin{equation}
 a_r:=\int_0^1x^r\RvachevUp(x)\Differential x
 =\frac1{r+1}-I_r
 =\frac{d_{r+1}}{r+1},
 \label{eq:a-r}
\end{equation}
...
Rvachev moments are
\begin{equation}
 \int_{-1}^{1}\AbsoluteValue x^{\,\alpha}\,\RvachevUp(x)\Differential x
...
For every $m\in\NaturalNumbers$ and $0\le k\le M=2^m$,
\begin{equation}
 \boxed{\;
...
\]
The self-similarity $2^{-n}\RvachevUp(y/2^{n})=\Expectation f_n(y-X')$ of the scaled
density, evaluated at the dyadic point $y=2(k-M)$ and combined with
$\RvachevUp(k/M-1)=F(k/M)$ from \eqref{eq:up-fold}, turns each truncated power
into the expectation $\Expectation(2(k-a)-1-X')^{m}=G_m(2(k-a)-1)$ precisely for
$a\le k-1$ (for larger $a$ the truncated power vanishes because
$\AbsoluteValue{X'}\le1$), and the constants assemble to $C_m$.
\end{proof}
...
\part{Additive dyadic comb sums and quadrature}
\label{part:sums}
...
For $\alpha\in\ComplexNumbers$ define the \emph{punctured left Fabius comb}
\begin{equation}
 L_{m,\alpha}
 :=h\sum_{k=1}^{M-1}
 ...
 \label{eq:L-def}
\end{equation}
with the real logarithm on $(0,1]$.  For $\alpha=p\in\NaturalNumbers$ this agrees
...
For a phase $\theta\in\RealNumbers$ define
\begin{equation}
 L_{m,\alpha}^{(\theta)}
 :=h\sum_{\substack{k\in\IntegerNumbers\\0<k+\theta<M}}
...
$\omega=\exp(2\pi\ImaginaryUnit/2^{s})$, and let $A_D$ be the row polynomial of
...
\begin{equation}
 Q_{m,p}(\theta)
 :=h\sum_{k\in\IntegerNumbers}\bigl(h(k+\theta)\bigr)^{p}\,
 \RvachevUp\bigl(h(k+\theta)\bigr),
 \label{eq:Qmp}
\end{equation}
finite sums because $\SupportOperator\RvachevUp=[-1,1]$, and the punctured absolute combs
\begin{equation}
 U_m(\alpha)
 :=h\sum_{k\in\IntegerNumbers\setminus\{0\}}\AbsoluteValue{kh}^{\alpha}\,\RvachevUp(kh).
...
\begin{equation}
 ...
 \label{eq:AN}
\end{equation}
Let $\Dop=z\,\Differential/\Differential z$ be the Euler operator and recall the finite
Thue--Morse product
...
$z=1$, and
\begin{equation}
 \Frow_m(z)=\sum_{k\ge0}\ClampedDyadicSample_m(k)z^{k},
 \qquad
 \ClampedDyadicSample_m(k)=
 \begin{cases}
 F(k/M),&0\le k\le M,\\
 1,&k\ge M.
 \end{cases}
...
\begin{proof}
By definition, the coefficient of $z^k$ in $A_m$ is $F(k/M)$.
The Euler operator satisfies $\Dop^p z^k=k^p z^k$ for natural $p$,
so evaluation at $z=1$ gives
$M^{-p-1}\sum_k k^pF(k/M)=L_{m,p}$.  The spectral definition of
$\Dop^\alpha$ makes the identical coefficientwise calculation for every
$\alpha\in\ComplexNumbers$; the sum is finite, so no convergence
...
$\sum_{n}T_{m,n}t^{n}/n!=\ThetaTM_m(\EulerE^{t})
=\prod_{j=0}^{m-1}\bigl(1-\EulerE^{2^{j}t}\bigr)$.
...
\begin{equation}
 ...
 \frac{(m+s)!}{s!}\,
 \ExponentialCompleteBellPolynomial{s}\bigl(\lambda_{m,1},\dots,\lambda_{m,s}\bigr).
 \;}
 \label{eq:TN-Bell}
\end{equation}
\end{theorem}
...
\begin{proof}
Factor $1-\EulerE^{x}=-x\,\EulerE^{x/2}\,\dfrac{\sinh(x/2)}{x/2}$ and use the
classical expansion
$\log\bigl[(\EulerE^{x}-1)/x\bigr]
...
$\exp(\sum_r x_rt^r/r!)=\sum_s\ExponentialCompleteBellPolynomial{s}(x_1,\dots,x_s)t^{s}/s!$ with the
...
\begin{equation}
 ...
 \label{eq:B-polylog}
\end{equation}
each $\Polylogarithm_{-i}(z)=z\,\mathcal A_i(z)/(1-z)^{i+1}$ rational with
...
$\FourierSeriesCoefficient{g_p}{n}
=\int_0^1g_p(x)\EulerE^{-2\pi\ImaginaryUnit nx}\Differential x$.  The periodic
...
\begin{equation}
 T_{m,p}=\sum_{\ell\in\IntegerNumbers}\FourierSeriesCoefficient{g_p}{M\ell},
 \label{eq:trap-alias}
\end{equation}
...
\label{lem:gp-hat}
For $n\in\IntegerNumbers\setminus\{0\}$,
\begin{equation}
 \FourierSeriesCoefficient{g_p}{n}
 =\sum_{j=0}^{p}
 \frac{\FallingFactorial{p}{j}}{(2\pi\ImaginaryUnit n)^{j+1}}
 \left(
 \frac{\chi^{(p-j)}(n)}{(-2\pi\ImaginaryUnit)^{p-j}}-1
 \right).
 \label{eq:gp-hat-chi}
\end{equation}
...
\begin{equation}
 \FourierTwoPi{g_0}(\xi)
 =-\frac{\EulerE^{-2\pi\ImaginaryUnit\xi}}{2\pi\ImaginaryUnit\xi}
 +\frac{\chi(\xi)}{2\pi\ImaginaryUnit\xi}
 \qquad(\xi\ne0),
 \label{eq:g0-hat}
\end{equation}
...
\begin{proof}
Since $F(x)=\Probability(Y\le x)$, Fubini gives the layer representation
$\FourierSeriesCoefficient{g_p}{n}
=\Expectation\int_Y^1x^{p}\EulerE^{-2\pi\ImaginaryUnit nx}\Differential x$.  Integrating by
parts $p+1$ times, using $\EulerE^{-2\pi\ImaginaryUnit n}=1$ at the fixed endpoint,
\[
 \int_Y^1x^{p}\EulerE^{-2\pi\ImaginaryUnit nx}\Differential x
 =\sum_{j=0}^{p}\frac{\FallingFactorial{p}{j}}{(2\pi\ImaginaryUnit n)^{j+1}}
 \Bigl(Y^{\,p-j}\EulerE^{-2\pi\ImaginaryUnit nY}-1\Bigr),
\]
and $\Expectation\bigl[Y^{r}\EulerE^{-2\pi\ImaginaryUnit nY}\bigr]=\chi^{(r)}(n)/(-2\pi\ImaginaryUnit)^{r}$.
For \eqref{eq:g0-hat}, one integration by parts of
$\int_0^1F\,\EulerE^{-2\pi\ImaginaryUnit\xi x}\Differential x$ with $F'=\rho$,
$\FourierTwoPi{\rho}=\chi$ suffices.
\end{proof}
...
For every $m\ge0$ and $p\in\NaturalNumbers$,
\begin{equation}
 L_{m,p}=I_p-\frac h2+
 \sum_{r=1}^{\Floor{(p+1)/2}}
 \frac{B_{2r}}{(2r)!}\,\FallingFactorial{p}{2r-1}\,h^{2r}
 +\defect_{m,p},
...
\begin{equation}
 ...
 \sum_{j=0}^{p}
 \frac{\FallingFactorial{p}{j}}{(2\pi\ImaginaryUnit M\ell)^{j+1}}\,
 \frac{\chi^{(p-j)}(M\ell)}{(-2\pi\ImaginaryUnit)^{p-j}}.
 \label{eq:R-alias}
\end{equation}
\end{theorem}
...
\[
 -\frac{\FallingFactorial{p}{2r-1}}{(2\pi\ImaginaryUnit M)^{2r}}\sum_{\ell\ne0}\ell^{-2r}
 =\frac{B_{2r}}{(2r)!}\,\FallingFactorial{p}{2r-1}M^{-2r}
\]
...
$m+\TwoAdicValuation(\ell)\ge m$.  If $p<m$, every derivative order $p-j\le p$
...
still vanish ($\TwoAdicValuation(\ell)\ge1$), and at odd $\ell$ only the $j=0$
term, carrying the derivative order exactly $m$, can survive.  From
$\chi(\xi)=\EulerE^{-\pi\ImaginaryUnit\xi}\Phi(\xi/2)$ and the vanishing of all lower
derivatives of $\Phi(\cdot/2)$ at $M\ell$, the Leibniz rule leaves a
single term,
\[
 \chi^{(m)}(M\ell)
 =\EulerE^{-\pi\ImaginaryUnit M\ell}\,2^{-m}\,
 \Phi^{(m)}\!\left(\frac{M\ell}{2}\right),
 \qquad
 \EulerE^{-\pi\ImaginaryUnit M\ell}=1\ \ (m\ge1).
\]
Since $\Phi$ is even, $\Phi^{(m)}$ is even for even $m$, so
$\chi^{(m)}(M\ell)$ is then even in $\ell$, whereas the accompanying
factor $(2\pi\ImaginaryUnit M\ell)^{-1}$ is odd; the aliases $\ell$ and $-\ell$
...
For $p\in\NaturalNumbers$ and $m\ge\tau(p)$,
\begin{equation}
 \boxed{\;
 R_{m,p}
...
(\nolinkurl{assets/companion-evidence/Fabius_Dyadic_Comb_Sums_Report_Package/}), are:
...
\begin{equation}
 \Sign\Phi\!\left(\frac{2t+1}{2}\right)=\eps_t
 \qquad(t\ge0).
 \label{eq:half-sign}
\end{equation}
\end{theorem}
...
For every $r\ge1$,
\begin{equation}
 \boxed{\;\frac{D(2r)}{\pi^{2r}}\in\RationalNumbers,\;}
...
prefactor simplifies as
\[
 \frac{(-1)^{(m+1)/2}(2\pi)^{m+1}2^{m(m+1)}}{2\,m!}
 =\frac{(-1)^{r}2^{(m+1)^2}\pi^{m+1}}{2\,m!}.
\]
The defect on the right of
...
$\TwoAdicValuation(\ell)=1$ alias layer (order-$(m{+}1)$ zeros).  Each piece is a
...
\begin{conjecture}[Even-level sharp threshold]
\label{conj:exact-degree}
For every even $m\ge2$, the first degree beyond the proved exactness
range is genuinely inexact:
$\defect_{m,m+1}\ne0$.  Equivalently, the two surviving alias-jet
contributions described in \cref{rem:even-m-reduction} never cancel.
This is verified exactly for even $m\le8$.  The corresponding odd-level
statement is already \cref{cor:odd-sharp}, not part of this conjecture.
\end{conjecture}
...
$\Sign\defect_{m,m}=(-1)^{(m+1)/2}$ for all odd $m$: although the
...
$\RationalNumbers\,\pi^{\,s-j}$.  (Rationality of each \emph{total} defect is a
...
The two-sided comb sums of $\RvachevUp$ upgrade the Fabius collapse to a
...
$f_p(x)=x^{p}\RvachevUp(x)$, together with
$\FourierTwoPi{f_p}(\xi)=(-2\pi\ImaginaryUnit)^{-p}\Phi^{(p)}(\xi)$, gives the exact
absolutely convergent representation
\begin{equation}
 Q_{m,p}(\theta)
 =\int_{-1}^{1}x^{p}\,\RvachevUp(x)\Differential x
 +\Bigl(-\frac{1}{2\pi\ImaginaryUnit}\Bigr)^{\!p}
 \sum_{\ell\in\IntegerNumbers\setminus\{0\}}
 \EulerE^{2\pi\ImaginaryUnit\ell\theta}\,\Phi^{(p)}(M\ell).
 \label{eq:Q-Poisson}
\end{equation}

\begin{theorem}[Shifted dyadic exactness]
\label{thm:shifted-exactness}
For every level $m\in\NaturalNumbers$, every $\theta\in\RealNumbers$, and every polynomial
$P$ with $\deg P\le m$,
\begin{equation}
 \boxed{\;
 h\sum_{k\in\IntegerNumbers}P\bigl(h(k+\theta)\bigr)\,
 \RvachevUp\bigl(h(k+\theta)\bigr)
 =\int_{-1}^{1}P(x)\,\RvachevUp(x)\Differential x.
 \;}
 \label{eq:polynomial-exactness}
\end{equation}
\end{theorem}

\emph{Formal crosswalk.}  The unscaled polynomial identity underlying this display is
...
as \nolinkurl{Fabius.finite_support_comb}), by linearity from the monomial core.  Namely, for
...
$\sum_{k\in\IntegerNumbers}(\theta+k)^{p}\,\RvachevUp\bigl(2^{-m}(\theta+k)\bigr)=\int_\RealNumbers x^{p}\,\RvachevUp(2^{-m}x)\Differential x$
(the display above follows by applying the polynomial theorem to $P(hX)$, multiplying
by $h$, and making the real change of variables $y=h x$).  This normalization step is
not yet exported as its own named Lean theorem.  The proof of the compiled core is the aliasing argument of this
section made formal: \nolinkurl{MonomialCombFourier.lean} packages $x^{p}\RvachevUp(2^{-m}x)$ as a
Schwartz map with
$(-2\pi\ImaginaryUnit)^{p}\FourierTwoPi{f_p}=(\FourierTwoPi{f_0})^{(p)}$,
\nolinkurl{MonomialCombDerivatives.lean} evaluates $(\FourierTwoPi{f_0})^{(p)}$ through the entire
transform via the real--complex derivative bridge, \nolinkurl{MonomialCombAlias.lean} kills
every nonzero alias by the integer-zero order theorem
($\OrderOperator_{2^{m}\ell}\Phi=\TwoAdicValuation(2^{m}\ell)+1>m$),
...
$\OrderOperator_{\xi=M\ell}\Phi=m+\TwoAdicValuation(\ell)+1\ge m+1$ for
$\ell\ne0$, so every
...
unity, $h\sum_k\RvachevUp(h(k+\theta))=1$.  For the centered comb
($\theta=0$),
\begin{equation}
 h\sum_{k}(kh)^{2r}\RvachevUp(kh)=c_r\quad(2r\le m),
 \qquad
 h\sum_{k}(kh)^{2r+1}\RvachevUp(kh)=0\quad(\text{all }m),
 \label{eq:centered-moments}
\end{equation}
and for $0<2r\le m$ the half-grid identity
$h\sum_{k=0}^{M}(kh)^{2r}\RvachevUp(kh)=c_r/2$ follows by evenness.
Tensor products give exact cubature on dyadic lattices in any
dimension for all coordinatewise degrees at most $m$.
\end{corollary}

\begin{proof}
Apply \cref{thm:shifted-exactness} first to the constant polynomial and
then to $x^{2r}$ when $2r\le m$.  On the centered comb, odd moments
cancel termwise because $\RvachevUp$ is even.  The even half-grid sum is
half of the centered full-grid sum: the two halves pair under $k\mapsto-k$
and the $k=0$ term vanishes for $r>0$.  Finally, both the compactly
supported density and a coordinatewise polynomial factor over Cartesian
products, so repeated application of the one-dimensional identity proves
the tensor statement.
\end{proof}

\begin{remark}[Base-$b$ extension]
\label{rem:base-b}
Only the alignment of the sinc zeros with the dual lattice is used.
For an integer base $b\ge2$, the compactly supported density $\RvachevUp_b$
with $\FourierTwoPi{\RvachevUp_b}(\xi)=\prod_{j\ge0}\SincPi(\xi/b^{j})$ (an
infinite convolution of uniform laws at geometrically decreasing
scales) satisfies the same theorem on the comb $h\IntegerNumbers+\theta$,
...
For every $p\in\NaturalNumbers$,
\begin{equation}
 Q_{m,p}(\theta)-\int_{-1}^{1}x^{p}\RvachevUp(x)\Differential x
 =\Bigl(-\frac{1}{2\pi\ImaginaryUnit}\Bigr)^{\!p}
 \sum_{v=0}^{p-m-1}\;
 \sum_{\substack{q\in\IntegerNumbers\\ q\ \mathrm{odd}}}
 \EulerE^{2\pi\ImaginaryUnit2^{v}q\theta}\,
...
Let $n\ne0$ be an integer, $d=\TwoAdicValuation(n)+1$, $n=\sigma2^{d-1}q$ with $q$
...
\begin{equation}
 ...
 P^{\mathrm{sinc}}_d(w)
 &:=\prod_{j=0}^{d-1}\SincPi\!\left(\frac{w}{2^{j}}\right).
 \end{aligned}
...
\begin{equation}
 \Phi^{(d+r)}(n)
 =\frac{(d+r)!}{r!}\,\Psi_n(0)\,
 \ExponentialCompleteBellPolynomial{r}\bigl(\lambda_1(n),\dots,\lambda_r(n)\bigr),
 \label{eq:Bell-jets}
\end{equation}
with
\begin{equation}
...
\begin{equation}
 ...
 =-\frac{2^{2j-1}\AbsoluteValue{B_{2j}}}{j}\,\pi^{2j}\,
 \frac{1-2^{-2jd}}{1-2^{-2j}}.
...
$\Psi_n^{(r)}(0)=\Psi_n(0)\ExponentialCompleteBellPolynomial{r}(\lambda_1,\dots,\lambda_r)$ for
...
\frac{2^{2j-1}\AbsoluteValue{B_{2j}}}{j\,(2j)!}(\pi z)^{2j}$
...
For $p=m+1$,
\begin{equation}
 Q_{m,m+1}(\theta)-\int_{-1}^{1}x^{m+1}\RvachevUp(x)\Differential x
 =-\Bigl(-\frac{1}{2\pi\ImaginaryUnit}\Bigr)^{\!m+1}
 \frac{(m+1)!}{2^{m(m+1)}}
 \sum_{\substack{\ell\in\IntegerNumbers\\ \ell\ \mathrm{odd}}}
 \frac{\Phi(\AbsoluteValue\ell/2)}{\ell^{\,m+1}}\,
 \EulerE^{2\pi\ImaginaryUnit\ell\theta}.
...
\nolinkurl{assets/companion-evidence/fabius_dyadic_comb_report_final/}) illustrate both
...
\toprule
$m$ & $p=m+1$ & shift $\theta$ & $Q_{m,p}(\theta)-\int x^{p}\RvachevUp$ \\
\midrule
1 & 2 & $0$ & $1/72$ \\
1 & 2 & $1/4$ & $0$ \\
2 & 3 & $0$ & $0$ \\
...
the partition of unity minus the central sample $h\RvachevUp(0)=h$.
\end{proof}
...
The corpus's self-weighted shifted quadrature (samples of $\RvachevUp$ used as
...
\begin{equation}
 ...
 &=\frac{2C_m}{M^{p+1}}
   \sum_{a=0}^{M-2}\eps_a
   \sum_{j=0}^{\Floor{m/2}}\binom m{2j}c_j\\
 &\quad\times\sum_{u=0}^{p}(-1)^{u}\binom pu L^{p-u}
...
(not necessarily a power of two) and $s=\TwoAdicValuation(M)$; the dyadic case
...
$t\EulerE^{xt}/(\EulerE^t-1)=\sum_kB_k(x)t^k/k!$.  For a polynomial $P$ of
degree $d$ put
\begin{equation}
...
For $\theta\in\RealNumbers$ and $x_a=(a+\theta)/M$,
\begin{equation}
 \mathcal A_{M,\theta}[P]
...
\begin{align}
 ...
 &=\sum_{\ell\ne0}\EulerE^{2\pi\ImaginaryUnit\ell\theta}\,
 \FourierSeriesCoefficient{R_P}{M\ell},
 \notag\\
 \FourierSeriesCoefficient{R_P}{q}
 &=\sum_{j=0}^{d}\frac{1}{(2\pi\ImaginaryUnit q)^{j+1}}
 \int_0^1P^{(j)}(x)\,\rho(x)\,\EulerE^{-2\pi\ImaginaryUnit qx}\Differential x
 \quad(q\ne0),
...
\begin{equation}
 C_{a,r}(M,\theta):=\Expectation\bigl[Y^{a}\,\widetilde B_r(MY-\theta)\bigr]
 \label{eq:alias-correlation}
\end{equation}
for the Fabius variable $Y$, the layer-cake representation
$F(x)=\Expectation\,\IndicatorOf{Y\le x}$ and Faulhaber's formula for the random
threshold sum $\sum_{a\ge\Ceiling{MY-\theta}}(a+\theta)^{n}$ give,
...
$\sum_{r}\frac{(-1)^{r+1}}{r!M^{r}}\FallingFactorial{n}{r-1}\,C_{n+1-r,r}(M,\theta)$.
...
$I_n=(1-\Expectation Y^{n+1})/(n+1)$, and no limit is taken.  The Fourier series
of $\widetilde B_r$ together with
$\Expectation[Y^{a}\EulerE^{2\pi\ImaginaryUnit tY}]=\chi^{(a)}(-t)/(-2\pi\ImaginaryUnit)^{a}$ shows that
$C_{a,r}(M,\theta)=0$ whenever $a<\TwoAdicValuation(M)$ (the correlation
...
If $s=\TwoAdicValuation(M)\ge d+1$, then
$\mathcal A_{M,\theta}[P]=I_P$ for \emph{every} shift $\theta\in\RealNumbers$.
\end{theorem}

\begin{proof}
Each integral in \eqref{eq:alias-general} is a combination of
derivatives of $\chi$ of order at most $d-j\le d$ at $q=M\ell$, and
$\chi$ vanishes there to order $\TwoAdicValuation(M\ell)=s+\TwoAdicValuation(\ell)\ge d+1$ (the
...
\begin{equation}
 \mathcal A_{M,0}[P]+\mathcal A_{M,0}[P^*]=\int_0^1P(x)\Differential x
 =I_P+I_{P^*},
...
$\TwoAdicValuation(M)\ge\deg P_{\mathrm a}+1$ suffices.
\end{theorem}
...
$\TwoAdicValuation(M)\ge p$ (even $p$) or $\TwoAdicValuation(M)\ge p+1$ (odd $p$): on dyadic
meshes this recovers the threshold $\tau(p)=2\Ceiling{p/2}$ of
\eqref{eq:tau} and extends it verbatim to every integer mesh with the
same $2$-adic valuation.  Explicitly, whenever
$\TwoAdicValuation(M)\ge\tau(p)$,
\begin{equation}
...
becomes visible; for $\RvachevUp$ all endpoint jets vanish and no corrector is
...
\cref{thm:D-rational}: the same values $D(2r)/\pi^{2r}\in\RationalNumbers$ emerge
...
\begin{equation}
 ...
 \Expectation\bigl[\widetilde B_{d+1}(qY+\theta)\bigr]
 =a_d\,\frac{(-1)^{d+1}d!}{(2\pi\ImaginaryUnit M)^{d+1}}
 \sum_{\substack{k\in\IntegerNumbers\setminus\{0\}\\ k\ \mathrm{odd}}}
 \frac{\Phi(kq/2)\,\EulerE^{2\pi\ImaginaryUnit k\theta}}{k^{d+1}}
...
rule is already exact at $\TwoAdicValuation(M)=d$, because $Y\overset d=1-Y$ and
...
\begin{proof}
Equation \eqref{eq:beta-D} and \cref{thm:D-positive} give
$\Sign(\beta_{2m})=(-1)^m$.  Substituting $2m=d+1$ into
\eqref{eq:beta-defect} yields the asserted nonzero defect and its sign.
\end{proof}
...
\begin{equation}
 \boxed{\;
 \AbsoluteValue{\Phi(q\ell/2)}\;\le\;\frac{\AbsoluteValue{\Phi(q/2)}}{\ell}.\;}
 \label{eq:multiple-angle}
\end{equation}
Consequently, for every $s\ge2$,
\begin{equation}
 \sum_{\substack{\ell\ge3\\ \ell\ \mathrm{odd}}}
 \frac{\AbsoluteValue{\Phi(q\ell/2)}}{\ell^{\,s-1}}
 \;\le\;\AbsoluteValue{\Phi(q/2)}
 \sum_{\substack{\ell\ge3\\ \ell\ \mathrm{odd}}}\ell^{-s}
 \;\le\;\AbsoluteValue{\Phi(q/2)}\Bigl(\frac{\pi^{2}}8-1\Bigr)
 \;<\;\AbsoluteValue{\Phi(q/2)}.
 \label{eq:relative-dominance}
\end{equation}
\end{lemma}

\begin{proof}
$\AbsoluteValue{\sin(\ell x)}\le\ell\,\AbsoluteValue{\sin x}$ gives
$\AbsoluteValue{\SincPi(\ell y)}\le\AbsoluteValue{\SincPi(y)}$ factorwise in
\eqref{eq:Phi-product} at $y=q/2^{\,j+1}$.  At the leading factor
$j=0$ the numerators are exact: $q\ell$ and $q$ are odd, so
$\AbsoluteValue{\sin(\pi q\ell/2)}=\AbsoluteValue{\sin(\pi q/2)}=1$ and the ratio of the
...
\begin{equation}
 ...
 \;\ne\;0,
 \qquad
 \Sign D_q(s)=\Sign\Phi(q/2)=\eps_{(q-1)/2}\,;
 \label{eq:twisted-positive}
\end{equation}
equivalently $\Expectation\widetilde B_{2m}(qY)\ne0$ for every odd $q$ and
...
odd $d$).  By $\AbsoluteValue{\sin\ell x}\le\ell\AbsoluteValue{\sin x}$ the tail has
modulus at most
$\AbsoluteValue{\sin x}\sum_{\ell\ge3}\AbsoluteValue{\Phi(q\ell/2)}\ell^{-d}
<\AbsoluteValue{\Phi(q/2)}\AbsoluteValue{\sin x}$ by
\cref{lem:multiple-angle}, so $S$ has the sign of
$\Phi(q/2)\sin x$ wherever $\sin x\ne0$; its zeros are exactly those
of $\sin x$.  For odd $d$ use $\AbsoluteValue{\cos\ell x}\le\ell\AbsoluteValue{\cos x}$
...
\[
 ...
 \;>\;\frac49.
\]
On the other hand $\AbsoluteValue{\Phi(q/2)}\le1$ and $q^{2r}\ge q^{2}$ give
\[
 \sum_{\substack{q\ge3\\q\ \mathrm{odd}}}
 \frac{\AbsoluteValue{\Phi(q/2)}}{q^{2r}}
...
$\Sign\defect_{m,m}=(-1)^{(m+1)/2}$; the odd-$m$ half of
...
$p=m+1$ at $\TwoAdicValuation(M)=m$, correlation annihilation
...
\begin{equation}
 ...
 \qquad
 C_{a,r}=\Expectation\bigl[Y^{a}\,\widetilde B_r(MY-\theta)\bigr].
...
$\Expectation\bigl[Y^{m+1}(\{MY\}-\tfrac12)\bigr]/M$ does not exactly cancel
...
requires $\TwoAdicValuation(b)>0$) the level increment is
...
$s>d+1$,
$\AbsoluteValue{\mathcal A_{b^{N},\theta}[P]-I_P}
...
the first $N$ with $\sum_{j\le N}\TwoAdicValuation(b_j)>d$.
\end{theorem}
...
$P$ with $\deg P\le\TwoAdicValuation(M)$,
\begin{equation}
 \frac1M\sum_{k\in\IntegerNumbers}
 P\!\Bigl(\frac{k+\theta}{M}\Bigr)
 \RvachevUp\!\Bigl(\frac{k+\theta}{M}\Bigr)
 =\int_{-1}^{1}P(x)\,\RvachevUp(x)\Differential x.
...
order $1+\TwoAdicValuation(M\ell)\ge1+\TwoAdicValuation(M)$, killing all required derivatives.
\end{proof}
...
radius-to-spacing ratio $M$ is exactly $\TwoAdicValuation(M)$, with the general-%
...
proves that at degree $p=\TwoAdicValuation(M)+1$ the first Poisson alias survives ---
...
the transform of $x^{\TwoAdicValuation(M)+1}\RvachevUp(x/M)$ does \emph{not} vanish at odd integer
frequencies, so the quadrature order of the mesh-$M$ comb is exactly $\TwoAdicValuation(M)$.
...
(\nolinkurl{Fabius.fourier_monomialRvachevSchwartz_neg},
$\FourierTwoPi{f}(-\ell)=(-1)^p\FourierTwoPi{f}(\ell)$)
makes the odd-frequency defect series an odd function of the frequency whenever the
threshold degree $\TwoAdicValuation(M)+1$ is odd, so at the endpoint shift $\theta=0$ the
...
fully formal: super-exactness when $\TwoAdicValuation(M)+1$ is odd, the doubled Dirichlet fold
...
The quantitative input these defect series need --- that the first harmonic is not
swallowed by the tail --- is now formal too.  The sinc factor obeys the quadratic lower
bound \(1-x^2/6\le\SincRad x\) (\nolinkurl{Fabius.one_sub_sq_div_six_le_sinc}),
and the Weierstrass product inequality \(1-\sum u_i\le\prod(1-u_i)\)
(\nolinkurl{Fabius.one_sub_sum_le_prod_one_sub}, \nolinkurl{Fabius.one_sub_tsum_le_tprod_one_sub}) ---
neither of which Mathlib carried --- turns it into a bound on the whole dyadic product:
\(1-2t^2/9\le\prod_{n}\SincRad(t/2^n)\) for \(t^2\le6\)
(\nolinkurl{Fabius.one_sub_le_prod_sinc_two_pow}, \nolinkurl{Fabius.one_sub_le_tprod_sinc_two_pow};
the family is multipliable for \emph{every} real \(t\),
\nolinkurl{Fabius.multipliable_sinc_two_pow}).  At the half-integer this gives the
explicit constant the comb volume assumes,
\(\Phi(1/2)>4/9\)
(\nolinkurl{Fabius.four_ninths_lt_tprod_sinc_pi_div_two}, and for the corpus transform
itself \nolinkurl{Fabius.four_ninths_lt_re_rvachevFourierProduct_half}).  The companion
\emph{relative} bound is formal as well: every dilation contracts the sinc factor,
\(\lvert\SincRad(mx)\rvert\le\lvert\SincRad x\rvert\) for \(m\ge1\)
(\nolinkurl{Fabius.abs_sinc_nat_mul_le}), whence the dilated-product comparison at every
odd \(\ell\) (\nolinkurl{Fabius.prod_abs_sinc_odd_dilate_le}), with sharpness witnesses
showing the hypothesis \(m\ge1\) cannot be dropped.

The same bound gives an explicit \emph{nonvanishing window}.  At every real point the
transform is the real dyadic sinc product,
\(\Phi(x)=\prod_{n\ge0}\SincRad(\pi x/2^n)\)
(\nolinkurl{Fabius.rvachevFourierProduct_ofReal_eq_tprod_sinc}), so
\(1-2(\pi x)^2/9\le\Phi(x)\) for \((\pi x)^2\le6\)
(\nolinkurl{Fabius.one_sub_le_re_rvachevFourierProduct_ofReal}) and hence \(\Phi(x)>0\),
with no zero at all, as soon as \(2(\pi x)^2<9\)
(\nolinkurl{Fabius.re_rvachevFourierProduct_ofReal_pos},
\nolinkurl{Fabius.rvachevFourierProduct_ofReal_ne_zero}); concretely on \(\AbsoluteValue{x}\le2/3\)
(\nolinkurl{Fabius.re_rvachevFourierProduct_ofReal_pos_of_abs_le}), which needs only
\(\pi<3.15\).  The corpus already knew \(\Phi\) is nonzero there; what is new is the
explicit constant, which is what the alias estimates actually consume.
Rescaling by \(x=t/2\pi\) carries the window to the up-law's characteristic function
itself: \(\varphi_{\RvachevUp}(t)=\Phi(t/2\pi)\) is real and strictly positive --- in particular nonzero ---
for \(t^2<18\), concretely on \(\lvert t\rvert\le4\)
(\nolinkurl{Fabius.charFun_rvachevMeasure_eq_ofReal},
\nolinkurl{Fabius.re_charFun_rvachevMeasure_pos},
\nolinkurl{Fabius.charFun_rvachevMeasure_ne_zero},
\nolinkurl{Fabius.charFun_rvachevMeasure_ne_zero_of_abs_le_four}).
The unscaled polynomial core of \cref{thm:up-composite-mesh} is kernel-verified:
\nolinkurl{Fabius.tsum_shifted_polynomial_eq_integral_nat} in
\nolinkurl{CompositeMeshExactness.lean} proves
\(\sum_{k\in\IntegerNumbers}P(\theta+k)\,\RvachevUp((\theta+k)/M)=\int P(x)\RvachevUp(x/M)\Differential x\) for every mesh
\(M\ge1\), every real shift, and every \(\deg P\le\TwoAdicValuation(M)\) --- the derivative
vanishing at the alias frequencies \(M\ell\) is exactly the valuation count
\(\TwoAdicValuation(M\ell)+1\ge\TwoAdicValuation(M)+1\) (\nolinkurl{Fabius.iteratedDeriv_rvachevFourier_nat_mul_int_eq_zero}),
...
The normalized display in \cref{thm:up-composite-mesh} follows by applying that theorem
to $P(X/M)$, multiplying by $1/M$, and changing variables; this final normalization is
not yet a named Lean wrapper.

\subsection{Phase cubature and the phase--resolution separation}

The phase parameter is itself an exact integration variable; this fact is
independent of smoothness, Fabius structure, and dyadic divisibility.

\begin{theorem}[Exact phase average]
\label{thm:phase-average}
For every integrable $f$ on $[0,1]$ and every $M\ge1$, put
\[
 Q_{M,\theta}[f]:={1\over M}\sum_{a=0}^{M-1}
 f\!\left({a+\theta\over M}\right).
\]
Then
\begin{equation}
 \boxed{\int_0^1Q_{M,\theta}[f]\Differential\theta
 =\int_0^1f(x)\Differential x.}
 \label{eq:phase-average}
\end{equation}
\end{theorem}

\begin{proof}
In the $a$th summand set $x=(a+\theta)/M$.  Its factor $1/M$ is the
Jacobian, and its phase interval maps to $[a/M,(a+1)/M]$.  Summing those
adjacent intervals partitions $[0,1]$ and proves
\eqref{eq:phase-average}.
\end{proof}

In the stable range $\TwoAdicValuation(M)\ge d+1$ the shifted rule
\eqref{eq:shifted-rule} makes
$\theta\mapsto\mathcal A^{\mathrm{raw}}_{M,\theta}[P]-I_P$ an exact
polynomial of degree at most $d+1$ whose nonconstant part is a zero-mean
combination of Bernoulli polynomials (\cref{thm:composite-termination}).
Combining that polynomial structure with \cref{thm:phase-average} yields a
finite phase rule.

\begin{theorem}[Finite phase cubature]
\label{thm:phase-cubature}
If $\TwoAdicValuation(M)\ge d+1$ and $(\theta_j,w_j)_{j\le s}$ is any quadrature
...
\begin{proof}
For fixed $P$ and $M$, \cref{thm:composite-termination} writes the
phase-dependent sample rule as $I_P+R(\theta)$, where $R$ is a
polynomial of degree at most $d+1$ and each of its nonconstant
Bernoulli components has zero integral on $[0,1]$; equivalently,
\cref{thm:phase-average} gives
$\int_0^1R(\theta)\,\Differential\theta=0$.  Exactness of the phase
quadrature gives $\sum_jw_jR(\theta_j)=0$ and
$\sum_jw_j=1$, which is precisely \eqref{eq:phase-cubature}.
Dyadic phases preserve dyadic sample arguments; exact dyadic Fabius
values and rational quadrature weights therefore give rational
certificates.
\end{proof}
...
finite-rank form
\[
 \sum_{r\le d+1}\frac{\FallingFactorial{d}{r-1}}{r!}\,B_r(\theta)\,2^{-mr}.
\]
Thus phase
...
$t_j=(-1)^{\BinaryDigitSum(j)}=\eps_j$ and
...
\begin{equation}
 ...
 =\frac{z}{\EulerE^{z}-1}\prod_{\ell=1}^{s}
 \bigl(1-\EulerE^{z/2^{\ell}}\bigr),
 \qquad\text{so}\qquad
 \beta_{s,r}=0\ (r<s),
 \quad
...
\begin{proof}
$\sum_j t_j\EulerE^{zj/2^{s}}=\prod_{\ell=1}^{s}(1-\EulerE^{z/2^{\ell}})$ by
...
\begin{align}
 \Theta_{s;M,d}
 &=\sum_{r=s}^{d+1}\frac{\FallingFactorial{d}{r-1}}{r!}\,\beta_{s,r}\,M^{-r},
 \qquad
...
Bernoulli mode by finite back substitution (phase spectroscopy).  The
binary signs are one instance of a complete radix and digit-mask calculus.

\begin{theorem}[Root-of-unity Bernoulli filter]
\label{thm:root-unity-filter}
Let $b\ge2$, let $\omega^b=1$ with $\omega\ne1$, and let $s_b(j)$ be the
sum of the base-$b$ digits of $j$.  Define
\[
 \gamma_{b,s,r}^{(\omega)}
 :=\sum_{j=0}^{b^s-1}\omega^{s_b(j)}B_r(j/b^s).
\]
Then
\begin{equation}
 \boxed{\sum_{r\ge0}\gamma_{b,s,r}^{(\omega)}{z^r\over r!}
 ={z\over\EulerE^z-1}
  \prod_{\ell=1}^s\left(\sum_{a=0}^{b-1}
  \omega^a\EulerE^{az/b^\ell}\right).}
 \label{eq:root-unity-egf}
\end{equation}
Consequently
\begin{equation}
 \gamma_{b,s,r}^{(\omega)}=0\quad(r<s),
 \qquad
 \gamma_{b,s,s}^{(\omega)}
 =(-1)^ss!\,b^{-s(s-1)/2}(1-\omega)^{-s}.
 \label{eq:root-unity-leading}
\end{equation}
If $\TwoAdicValuation(M)\ge d+1$, then the stable shifted Fabius rules obey
\begin{equation}
 \sum_{j=0}^{b^s-1}\omega^{s_b(j)}
 \mathcal A^{\mathrm{raw}}_{M,j/b^s}[x^d]
 =\sum_{r=s}^{d+1}{\FallingFactorial{d}{r-1}\over r!}
 \gamma_{b,s,r}^{(\omega)}M^{-r}.
 \label{eq:root-unity-phase-filter}
\end{equation}
For $s=d+1$ this is the single term
$(-1)^{d+1}d!\,b^{-d(d+1)/2}(1-\omega)^{-(d+1)}M^{-(d+1)}$.
\end{theorem}

\begin{proof}
The Bernoulli generating function and the base-$b$ digit expansion give
\begin{align*}
 \sum_{r\ge0}\gamma_{b,s,r}^{(\omega)}{z^r\over r!}
 &={z\over\EulerE^z-1}
   \sum_{j<b^s}\omega^{s_b(j)}\EulerE^{zj/b^s}\\
 &={z\over\EulerE^z-1}
   \prod_{\ell=1}^s\sum_{a=0}^{b-1}
   \omega^a\EulerE^{az/b^\ell}.
\end{align*}
Because $\sum_a\omega^a=0$ and
$\sum_a a\omega^a=-b/(1-\omega)$, the $\ell$th factor begins with
$-b^{1-\ell}z/(1-\omega)$.  Multiplying the leading terms proves
\eqref{eq:root-unity-leading}.  Finally substitute the exact stable
Bernoulli expansion of \cref{thm:composite-termination} and interchange
the two finite sums.  The constant term vanishes because the character
weights sum to zero, giving \eqref{eq:root-unity-phase-filter} and its
top-mode specialization.
\end{proof}

\begin{theorem}[Digital masks of arbitrary cancellation order]
\label{thm:general-digit-mask}
Let $c_0,\ldots,c_{b-1}\in\ComplexNumbers$ and
$C(z):=\sum_{a=0}^{b-1}c_a\EulerE^{az}$.  Suppose
$C(z)=c_\rho^*z^\rho+\BigO(z^{\rho+1})$ at the origin, with $\rho\ge1$.
For $j=\sum_{\nu=0}^{s-1}a_\nu b^\nu$ put
$w_s(j):=\prod_{\nu=0}^{s-1}c_{a_\nu}$ and
\[
...
Then
\begin{equation}
 \sum_{r\ge0}\delta_{b,s,r}{z^r\over r!}
 ={z\over\EulerE^z-1}\prod_{\ell=1}^s C(z/b^\ell),
 \label{eq:general-mask-egf}
\end{equation}
and therefore
\begin{equation}
 \delta_{b,s,r}=0\quad(r<\rho s),
 \qquad
 \delta_{b,s,\rho s}
 =(\rho s)!(c_\rho^*)^s b^{-\rho s(s+1)/2}.
 \label{eq:general-mask-leading}
\end{equation}
Moreover, in the stable range $\TwoAdicValuation(M)\ge d+1$,
\begin{equation}
 \sum_{j=0}^{b^s-1}w_s(j)
 \mathcal A^{\mathrm{raw}}_{M,j/b^s}[x^d]
 =\sum_{r=\rho s}^{d+1}{\FallingFactorial{d}{r-1}\over r!}
 \delta_{b,s,r}M^{-r}.
 \label{eq:general-mask-phase-filter}
\end{equation}
Thus $\rho s=d+1$ isolates the top Bernoulli mode, while
$\rho s>d+1$ annihilates the entire filtered rule.
\end{theorem}

\begin{proof}
The same Bernoulli generating function turns the digit sum into
$\prod_{\ell=1}^sC(z/b^\ell)$, proving
\eqref{eq:general-mask-egf}.  Its leading product is
$(c_\rho^*)^sz^{\rho s}b^{-\rho(1+\cdots+s)}$, which proves
\eqref{eq:general-mask-leading}.  Since $C(0)=\sum_ac_a=0$, the mask
has zero total mass.  Applying it to the exact finite Bernoulli expansion
and interchanging finite sums proves \eqref{eq:general-mask-phase-filter}.
\end{proof}

This gives a constructive design principle: choose a short digit symbol
with several vanishing power moments and tensor it across digits.  The
resulting exponentially long phase mask has cancellation order linear in
the digit depth; \eqref{eq:tm-phase-filter} is its binary one-state case.

\subsection{Coherent radix exhaustion and tensor shells}

Fix a radix $b\ge2$ and $\alpha\in[0,1)$.  Put
\begin{equation}
 \theta_N:=b^N\alpha-\Floor{b^N\alpha},
 \qquad
 \varepsilon_N:=\Floor{b\theta_{N-1}},
 \label{eq:coherent-radix-orbit}
\end{equation}
so $b\theta_{N-1}=\varepsilon_N+\theta_N$, and define the coherent tagged
sum
\begin{equation}
 Q_N^{(b,\alpha)}[f]
 :=b^{-N}\sum_{j=0}^{b^N-1}
 f\!\left({j+\theta_N\over b^N}\right).
 \label{eq:coherent-tagged-sum}
\end{equation}

\begin{theorem}[Coherent radix exhaustion]
\label{thm:coherent-exhaustion}
Let $\zeta_b=\EulerE^{2\pi\ImaginaryUnit/b}$.  For every $N\ge1$,
\begin{align}
 \mathcal E_N^{(b,\alpha)}[f]
 &:=Q_N^{(b,\alpha)}[f]-Q_{N-1}^{(b,\alpha)}[f] \notag\\
 &=b^{-N}\sum_{j=0}^{b^N-1}
\left(1-b\,\IndicatorOf{j\equiv\varepsilon_N\ (\mathrm{mod}\ b)}\right)
 f\!\left({j+\theta_N\over b^N}\right)
 \label{eq:coherent-residue-layer}\\
 &=-b^{-N}\sum_{a=1}^{b-1}\zeta_b^{-a\varepsilon_N}
 \sum_{j=0}^{b^N-1}\zeta_b^{aj}
 f\!\left({j+\theta_N\over b^N}\right).
 \label{eq:coherent-character-layer}
\end{align}
If $f$ is Riemann integrable, then
\begin{equation}
 \boxed{\int_0^1f(x)\Differential x
 =f(\alpha)+\sum_{N=1}^{\infty}\mathcal E_N^{(b,\alpha)}[f].}
 \label{eq:coherent-exhaustion}
\end{equation}
\end{theorem}
...
\[
 {k+\theta_{N-1}\over b^{N-1}}
 ={bk+\varepsilon_N+\theta_N\over b^N}.
\]
Thus the coarse grid is exactly the residue class
$j\equiv\varepsilon_N\pmod b$ in the fine grid; subtracting it changes
the fine weight from $1$ to $1-b$ on precisely that class.  This proves
\eqref{eq:coherent-residue-layer}.  Finite Fourier inversion of the residue
indicator gives
$1-b\IndicatorOf{j\equiv\varepsilon_N}=
-\sum_{a=1}^{b-1}\zeta_b^{a(j-\varepsilon_N)}$, proving
\eqref{eq:coherent-character-layer}.  The partial sum of
\eqref{eq:coherent-exhaustion} telescopes to $Q_N^{(b,\alpha)}[f]$ because
$Q_0^{(b,\alpha)}[f]=f(\alpha)$; tagged Riemann-sum convergence finishes
the proof.
\end{proof}

For $p\in\NaturalNumbers$ write
\[
 Q_{M,\theta,p}:={1\over M}\sum_{j=0}^{M-1}
 \left({j+\theta\over M}\right)^p
 F\!\left({j+\theta\over M}\right),
 \qquad h_N:=b^{-N}.
\]

\begin{theorem}[Coherent Fabius--Bernoulli layer and tail]
\label{thm:coherent-Fabius-layer}
If both $b^N$ and $b^{N-1}$ have $2$-adic valuation at least $p+1$, then
\begin{align}
 \mathcal E_{N,p}^{(b,\alpha)}
 &:=Q_{b^N,\theta_N,p}-Q_{b^{N-1},\theta_{N-1},p} \notag\\
 &=\sum_{r=1}^{p+1}{\FallingFactorial{p}{r-1}\over r!}h_N^r
 \left[B_r(\theta_N)-b^rB_r(\theta_{N-1})\right],
 \label{eq:coherent-Bernoulli-layer}\\
 I_p-Q_{b^N,\theta_N,p}
 &=-\sum_{r=1}^{p+1}{\FallingFactorial{p}{r-1}\over r!}
 B_r(\theta_N)h_N^r.
 \label{eq:coherent-Bernoulli-tail}
\end{align}
If $b$ is even, both identities therefore hold for all $N$ satisfying
$(N-1)\TwoAdicValuation(b)\ge p+1$.
\end{theorem}

\begin{proof}
At either stable mesh, \cref{thm:composite-termination} and the definition
\eqref{eq:shifted-rule} give
\[
 Q_{M,\theta,p}=I_p+
 \sum_{r=1}^{p+1}{\FallingFactorial{p}{r-1}\over r!}B_r(\theta)M^{-r}.
\]
Subtract the formula at $M=b^{N-1}$ from the one at $M=b^N$ and use
$b^{-(N-1)r}=b^rh_N^r$ to obtain
\eqref{eq:coherent-Bernoulli-layer}.  Moving the fine-grid correction to
the other side gives \eqref{eq:coherent-Bernoulli-tail}.  Finally
$\TwoAdicValuation(b^{N-1})=(N-1)\TwoAdicValuation(b)$, while the fine
grid has at least as much valuation.
\end{proof}
...
\begin{equation}
 \mathcal P_{p,M}(\theta)
 :=\sum_{r=1}^{p+1}{\FallingFactorial{p}{r-1}\over r!}B_r(\theta)M^{-r}.
 \label{eq:tensor-one-dimensional-corrector}
\end{equation}

\begin{theorem}[Exact anisotropic tensor correction]
\label{thm:tensor-correction}
Let $\bm p=(p_1,\ldots,p_d)$, $\bm M=(M_1,\ldots,M_d)$, and
$\bm\theta=(\theta_1,\ldots,\theta_d)$, with
$\TwoAdicValuation(M_j)\ge p_j+1$ for every $j$.  For the tensor-product
rectangle rule and moment,
\[
 Q_{\bm M,\bm\theta,\bm p}^{(d)}
 :=\prod_{j=1}^dQ_{M_j,\theta_j,p_j},
 \qquad I_{\bm p}^{(d)}:=\prod_{j=1}^dI_{p_j},
\]
one has
\begin{align}
 Q_{\bm M,\bm\theta,\bm p}^{(d)}
 &=\prod_{j=1}^d\left(I_{p_j}+\mathcal P_{p_j,M_j}(\theta_j)\right),
 \label{eq:tensor-product-correction}\\
  I_{\bm p}^{(d)}
  &=\boxed{\prod_{j=1}^d\left(Q_{M_j,\theta_j,p_j}
  -\mathcal P_{p_j,M_j}(\theta_j)\right)}.
 \label{eq:tensor-exact-moment}
\end{align}
By linearity the same anisotropic rule integrates every polynomial times
$\prod_jF(x_j)$ once each coordinate meets the largest exponent occurring
in that coordinate.
\end{theorem}

\begin{proof}
The one-dimensional stable identity used in the preceding proof is
$Q_{M_j,\theta_j,p_j}=I_{p_j}+\mathcal P_{p_j,M_j}(\theta_j)$.
Multiplying these $d$ identities proves the first formula.  Solving each
one for $I_{p_j}$ and multiplying proves the second.  Expansion into
monomials and linearity give the polynomial statement.
\end{proof}

For the open dyadic rule define, after its stable threshold,
\[
 T_p(h):=I_p-Q_{1/h,0,p}
 ={h\over2}-\sum_{r=1}^{\Floor{(p+1)/2}}
 {B_{2r}\over(2r)!}\FallingFactorial{p}{2r-1}h^{2r}.
\]

\begin{corollary}[Exact tensor Ruffa shell and remaining tail]
\label{cor:tensor-Ruffa-shell}
Let $h=2^{-N}$ and suppose both consecutive grids are stable in every
coordinate.  With
$L_{N,\bm p}^{(d)}:=\prod_j(I_{p_j}-T_{p_j}(h))$, the synchronous shell is
\begin{equation}
 \boxed{\mathcal E_{N,\bm p}^{(d)}
 =\prod_j\left(I_{p_j}-T_{p_j}(h)\right)
 -\prod_j\left(I_{p_j}-T_{p_j}(2h)\right),}
 \label{eq:tensor-Ruffa-shell}
\end{equation}
and its entire remaining tail is the finite polynomial
\begin{equation}
 \boxed{I_{\bm p}^{(d)}-L_{N,\bm p}^{(d)}
 =\prod_jI_{p_j}
 -\prod_j\left(I_{p_j}-T_{p_j}(h)\right).}
...
\begin{proof}
The definition of $T_p$ gives the displayed product for the level-$N$
tensor rule.  The level $N-1$ rule is the same product with $2h$; their
difference is \eqref{eq:tensor-Ruffa-shell}.  Subtracting the level-$N$
product from $\prod_jI_{p_j}$ proves \eqref{eq:tensor-tail}.
\end{proof}

The raw tensor rule remains first order, with boundary term
$\tfrac h2\sum_j\prod_{k\ne j}I_{p_k}$; phase averaging or coordinatewise
Bernoulli subtraction removes it exactly.  The Fabius moments also have a
correction-free full-support realization: from
$I_p=(1-a_{p+1})/(p+1)$ and the binomial transfer to Rvachev moments,
$\TwoAdicValuation(M)\ge p+1$ gives
$I_p=\tfrac1{p+1}[1-2^{-(p+1)}\sum_{j\le p+1}\binom{p+1}j
U_{M,\theta,j}]$ with $U$ the full-support comb of
\cref{thm:up-composite-mesh}.  Thus the same integral has a one-sided
Bernoulli-corrected lattice rule and a full-support correction-free one.

\subsection{Half-interval Rvachev rules and the boundary-topology
trichotomy}

Substituting $x\mapsto1-x$ turns $x^{n}\RvachevUp(x)$ on $[0,1]$ into
$(1-x)^{n}F(x)$, whose endpoint jet at $1$ is concentrated in the
single order $n$.  The whole Bernoulli corrector therefore collapses
to one term: for $\TwoAdicValuation(M)>n$ and every shift $\eta$,
\begin{equation}
 \boxed{\;
 \int_0^1x^{n}\RvachevUp(x)\Differential x
 =\frac1M\sum_{a=0}^{M-1}
 \Bigl(\frac{a+\eta}{M}\Bigr)^{n}
 \RvachevUp\!\Bigl(\frac{a+\eta}{M}\Bigr)
...
series of $x^{n}\RvachevUp$ on $[0,1]$ accordingly either \emph{terminates}
...
\begin{equation}
 \int_0^1x^{n}\RvachevUp
 =\frac{2^{\,n+1}\,Q_{N+1}[x^{n}\RvachevUp]-Q_N[x^{n}\RvachevUp]}{2^{\,n+1}-1}
 \qquad(n\ \text{odd},\ N>n).
...
$\RvachevUp$ keeps exactly one; and the full symmetric support keeps none ---
...
\begin{align}
 ...
 \frac{B_{2r}}{(2r)!}\,\FallingFactorial{n}{2r-1}\,2^{-2rN},
...
bounds $\AbsoluteValue{\sin\ell x}\le\ell\AbsoluteValue{\sin x}$ and (odd $\ell$)
$\AbsoluteValue{\cos\ell x}\le\ell\AbsoluteValue{\cos x}$ against
...
For $\TwoAdicValuation(M)=v<d$, several jet layers survive; reduce all
...
\begin{proposition}[All-orders complex-power Euler--Maclaurin expansion]
\label{prop:complex-alpha-EM}
Fix $\alpha\in\ComplexNumbers$ and use the real logarithm to define
$x^\alpha$ for $0<x\le1$.  For every $0<\theta<1$ and $K\ge1$, with
$x_a=(a+\theta)/M$,
\begin{equation}
 \frac1M\sum_{a=0}^{M-1}x_a^{\alpha}F(x_a)
 =I(\alpha)
 +\sum_{r=1}^{K}\frac{\FallingFactorial{\alpha}{r-1}}{r!}B_r(\theta)M^{-r}
 +\BigO_{\alpha,K,\theta}\bigl(M^{-K-1}\bigr)
 \qquad(M\to\infty),
 \label{eq:complex-alpha-EM}
\end{equation}
where
$I(\alpha)=\int_0^1x^\alpha F(x)\Differential x$ is entire in $\alpha$.
For $\alpha=p\in\NaturalNumbers$ the endpoint series terminates after
$p+1$ terms, and the remaining alias is the finite-order phenomenon
studied above.
\end{proposition}

\begin{proof}
For every compact set of exponents and every integer $m$, flatness gives
$F(x)=\BigO(x^N)$ near zero with $N$ large enough that all parameter and
$x$-derivatives of $x^\alpha F(x)$ through order $m$ extend continuously
and vanish at zero.  At $x=1$, the jump of the $(r-1)$st derivative of
the one-periodic extension is $\FallingFactorial{\alpha}{r-1}$, because all positive
derivatives of $F$ vanish there and $F(1)=1$.  The shifted
Euler--Maclaurin formula through order $K+1$ therefore gives the displayed
Bernoulli terms and a remainder of order $M^{-K-1}$.  Finally, on each
compact set of $\alpha$, the same flatness dominates
$x^\alpha(\log x)^mF(x)$ for every $m$; differentiation under the integral
is valid to all orders.  Hence $I(\alpha)$ is entire.
\end{proof}

\begin{question}[Optimal truncation and Lambert-controlled aliases]
\label{q:analytic-weights}
For a general nonpolynomial weight, quantify the alias remainder after
subtracting a finite endpoint-jet corrector.  For the proved complex-power
expansion \eqref{eq:complex-alpha-EM}, determine whether increasing $K$
with the mesh yields a first-surviving-alias plus Lambert-controlled
remainder split whose optimal truncation index follows the lower-Lambert
scale of the endpoint asymptotics.  Does the alias term dominate the
exponentially small endpoint resurgence along suitable dyadic rays?  In
the companion regime $p\to\infty$ with
$\TwoAdicValuation(M)-p$ bounded, match the endpoint-localized Bernoulli
terms to the order-$p$ sinc-product alias jets through a Lambert--$W$
saddle.  Quantile transport of the exact $PF$ rules through $F^{-1}$ is a
natural test case.
\end{question}
...
$\theta\leftrightarrow1-\theta$, and minimize $\sum\AbsoluteValue{w_j}$:
...
(\nolinkurl{assets/companion-evidence/Fabius_Euler_Maclaurin_Report_Package/}) serve as
regression data.
...
For every $m\in\NaturalNumbers$ and $\alpha\in\ComplexNumbers$,
\begin{equation}
\begin{split}
 L_{m,\alpha}
 ={}&\frac{C_m}{M^{\alpha+1}}
...
two Hurwitz zetas cancel).  At $\alpha=p\in\NaturalNumbers$ this reduces to
...
For $q\in\NaturalNumbers$,
\begin{equation}
 L_{m,-q}
 =M^{\,q-1}\sum_{k=1}^{M-1}\frac{F(k/M)}{k^{q}}\in\RationalNumbers;
...
\begin{proof}
Every summand in \eqref{eq:negative-rational} is rational: $M^{q-1}$,
$k^{-q}$, and the exact dyadic value $F(k/M)$ are rational, and the sum is
finite.  The displayed identity is the definition of $L_{m,-q}$ with
the vanishing endpoint omitted.
\end{proof}
...
continuous side, expanding $F(x)=\RvachevUp(1-x)$ around the flat point gives
...
\end{equation}
locally uniformly in $\alpha$: flatness of $\RvachevUp$ at $1$ makes the
half-moments $a_r$ decay faster than every power, so the generalized
binomial series converges normally.  For $\alpha=p\in\NaturalNumbers$ it
...
For $\alpha$ in a compact $K\subset\ComplexNumbers$ and every $R\ge1$,
\begin{equation}
 L_{m,\alpha}
 =I(\alpha)-\frac h2
 +\sum_{r=1}^{R-1}\frac{B_{2r}}{(2r)!}\,
 \FallingFactorial{\alpha}{2r-1}\,h^{2r}
...
$\bigl(x^{\alpha}F(x)\bigr)^{(n)}\bigl|_{x=1}=\FallingFactorial{\alpha}{n}$.  The
classical Euler--Maclaurin formula with $R$ terms then has only the
upper-endpoint corrections shown and a remainder
$\BigO_{K,R}(h^{2R})$.
\end{proof}

When $\alpha=p\in\NaturalNumbers$ the falling factorials terminate, and
...
\subsection{The Mellin transform of \texorpdfstring{$\RvachevUp$}{up}}
\label{sec:mellin-up}

For $\Re\alpha>-1$ let
$\MellinTransformSymbol(\alpha):=\int_{-1}^{1}\AbsoluteValue x^{\alpha}\RvachevUp(x)\Differential x
=2\int_0^1x^{\alpha}\RvachevUp(x)\Differential x$.  Since $\RvachevUp-1$ is flat at $0$,
\begin{equation}
 \MellinTransformSymbol(\alpha)
 =\frac{2}{\alpha+1}
 +2\int_0^1x^{\alpha}\bigl(\RvachevUp(x)-1\bigr)\Differential x,
 \label{eq:Mellin-continuation}
\end{equation}
with an entire second term: $\MellinTransformSymbol$ continues meromorphically to $\ComplexNumbers$
with a \emph{single} simple pole at $\alpha=-1$, of residue $2$, and
\begin{equation}
 \MellinTransformSymbol(\alpha)=\frac{2d_{\alpha+1}}{\alpha+1}
 \label{eq:Mellin-d}
\end{equation}
...
\begin{equation}
 ...
 =\int_0^{\infty}x^{\beta}\chi(x)\Differential x
...
\begin{proof}
Let $\chi^{*}(s)=\int_0^\infty\chi(x)x^{s-1}\Differential x$; integration by
parts, $\chi^{*}(s)=-s^{-1}\int_0^\infty\chi'(x)x^{s}\Differential x$, shows
...
\[
 ...
 \chi\bigl(h(k+\theta)\bigr)
 =\frac{1}{2\pi\ImaginaryUnit}\int_{(c)}
 \chi^{*}(s)\,\zeta(s-\beta,\theta)\,h^{\beta+1-s}\Differential s.
\]
...
\end{equation}
For $\alpha=p\in\NaturalNumbers$ the remainder vanishes identically once
$m\ge\tau(p)$, by \cref{thm:absolute-integer}.
\end{theorem}

\begin{proof}
Split $x^{\alpha}\RvachevUp(x)=x^{\alpha}\chi(x)
+x^{\alpha}(\RvachevUp(x)-\chi(x))$ on $[0,\infty)$ with $\chi$ as in the
lemma, supported inside $[0,1)$.  The second summand extends by zero
to $C_c^\infty(0,\infty)$ --- flatness of $\RvachevUp-1$ removes the origin,
flatness of $\RvachevUp$ the support endpoint --- so Poisson summation makes
...
Euler--Maclaurin tail: flatness of $\RvachevUp-1$ kills every higher local
...
\begin{align}
 h\sum_{k\in\IntegerNumbers}\AbsoluteValue{h(k+\theta)}^{\alpha}\,
 \RvachevUp\bigl(h(k+\theta)\bigr)
 &=\MellinTransformSymbol(\alpha)\notag\\
 &\quad+h^{\alpha+1}
   \bigl[\zeta(-\alpha,\theta)+\zeta(-\alpha,1-\theta)\bigr]
   +\BigO(h^{A}),
 \label{eq:shifted-abs-Hurwitz}\\
 h\sum_{k\in\IntegerNumbers}\Sign(k+\theta)\,\AbsoluteValue{h(k+\theta)}^{\alpha}\,
 \RvachevUp\bigl(h(k+\theta)\bigr)
 &=h^{\alpha+1}
   \bigl[\zeta(-\alpha,\theta)-\zeta(-\alpha,1-\theta)\bigr]
   +\BigO(h^{A}).
...
$x^{\alpha}
=\tfrac{1+\EulerE^{\pi\ImaginaryUnit\alpha}}2\AbsoluteValue x^{\alpha}
+\tfrac{1-\EulerE^{\pi\ImaginaryUnit\alpha}}2\Sign(x)\AbsoluteValue x^{\alpha}$.
...
\begin{equation}
 ...
 \longrightarrow \MellinTransformSymbol(\alpha)
 \qquad(m\to\infty),
 \label{eq:negative-renormalized}
\end{equation}
with superalgebraic convergence, $\MellinTransformSymbol(\alpha)$ denoting the
meromorphic value \eqref{eq:Mellin-continuation}.  At the pole,
\begin{equation}
 \boxed{\;
 U_m(-1)
 =2\log M+2\gamma
 +2\int_0^1\frac{\RvachevUp(x)-1}{x}\Differential x
...
\[
 \MellinTransformSymbol(-1+\eps)=\frac2\eps
 +2\int_0^1\frac{\RvachevUp(x)-1}{x}\Differential x+\BigO(\eps),
 \qquad
 2\zeta(1-\eps)h^{\eps}
...
$\FallingFactorial{-1}{2r-1}=-(2r-1)!$ makes the corrections
...
$\int_0^1\frac{\RvachevUp(x)-1}{x}\Differential x=-I(-1)$, i.e.\ the constant is
$\gamma-\Expectation[-\log Y]$.  For integer $q>1$ the same cancellation gives
...
For $n\ge1$ and every $\alpha\in\ComplexNumbers$,
\begin{equation}
 \boxed{\;
 L^{[n]}_{m,\alpha}
 =\frac{M^{\,1-n}}{(n-1)!}
...
$\prod_{r=1}^{n-1}(M-k-1+r)=\RisingFactorial{M-k}{\,n-1}
...
For $p\in\NaturalNumbers$ and $m\ge\tau(p+n-1)$, every shifted comb in
...
\begin{equation}
  \begin{aligned}
  &h^{n}\sum_{k=-M}^{M}\binom{M-k+n-1}{n-1}
    (kh)^{p}\,\RvachevUp(kh)\\
 &\qquad=\frac{1}{(n-1)!}
    \sum_{i=0}^{n-1}(-1)^{i}
    e_{\,n-1-i}\bigl(1{+}h,\dots,1{+}(n{-}1)h\bigr)\,\mu_{p+i},
  \end{aligned}
...
\begin{proof}
Insert the terminating formula \eqref{eq:EM-collapse} for each shifted
comb in the finite linear combination \eqref{eq:iterated-symmetric};
the largest degree is $p+n-1$, so the stated threshold applies to all
terms simultaneously.  For the full-support formula, expand the rising
factor as in the proof of \cref{thm:iterated-reduction} and apply
\cref{thm:shifted-exactness} to each monomial.  Evenness of
$\RvachevUp$ then replaces its moments by $\mu_{2r}=c_r$ and
$\mu_{2r+1}=0$.
\end{proof}

The continuum limits are the classical Volterra kernels: for every
$\alpha\in\ComplexNumbers$,
\begin{equation}
 \lim_{m\to\infty}L^{[n]}_{m,\alpha}
 =\frac{1}{(n-1)!}\int_0^1(1-x)^{n-1}x^{\alpha}F(x)\Differential x
 =\frac{1}{(n-1)!}
...
For $\Re\rho>0$, locally uniformly in $\alpha\in\ComplexNumbers$,
\begin{equation}
 \lim_{m\to\infty}L^{[\rho]}_{m,\alpha}
 =\frac{1}{\Gamma(\rho)}
 \int_0^1(1-x)^{\rho-1}x^{\alpha}F(x)\Differential x.
...
\begin{equation}
 ...
 \qquad
 \Delta_m\,F(k/M)\in\IntegerNumbers\ \ \forall k,
 \label{eq:DeltaN}
\end{equation}
one gets immediately $\Delta_mM^{\,p+1}L_{m,p}\in\IntegerNumbers$ and, with
$\Lambda_M=\LeastCommonMultiple(1,\dots,M-1)$,
$\Delta_m\Lambda_M^{\,q}L_{m,-q}\in\IntegerNumbers$; the factorials and powers of
...
Determine $\TwoAdicValuation(\operatorname{den}L_{m,p})$ and the odd-prime
...
only the shells $\TwoAdicValuation(\ell)\le p-m-1$
...
$\prod_j\SincPi(q^{j}\xi)$ with $1/q\notin\NaturalNumbers$, exact lattice
...
$\Norm{F^{(M+1)}}_\infty=2^{\binom{M+2}2}$ from
...
\end{equation}
and the interpolant of $\RvachevUp$ on the symmetric comb
$t_j=-1+2j/M$ of $[-1,1]$ is even:
$\Lag_M\RvachevUp(t)=(1-t^{2})\,T_M(t^{2})$ with $\deg T_M\le M/2-1$.
\end{proposition}
...
polynomial in $x(1-x)$.  The $\RvachevUp$ statement is the same argument
...
\begin{equation}
 \TwoAdicValuation\binom Mj=m-\TwoAdicValuation(j).
...
The strict-interior formula \eqref{eq:weight-valuation} has the exact Lean
counterpart \path{Fabius.twoPowChoose_padicValNat} in
\path{FabiusFunction.PrimePowerBinomialValuation}.  The same module proves a
stronger arbitrary-prime identity for $0<j\le p^m$, including the positive
right endpoint $j=p^m$ that is excluded from the displayed dyadic theorem.

\begin{proposition}[Prime-power companion rows]
\label{prop:prime-power-companion-rows}
Let $p$ be prime and $m\ge0$.  Writing $v_p$ for the $p$-adic valuation,
\begin{align*}
 v_p\binom{p^m-1}{r}&=0 &&(0\le r<p^m),\\
 v_p\binom{p^m-2}{j-1}&=v_p(j) &&(0<j<p^m).
\end{align*}
In particular, for $M=2^m$ and $1\le j\le M-1$,
\[
 \TwoAdicValuation\binom{M-2}{j-1}=\TwoAdicValuation(j),
\]
so the interior weight $\binom{M-2}{j-1}$ of
\cref{sec:endpoint-flat} is odd exactly when $j$ is odd.
\end{proposition}

\begin{proof}
For $0\le r<p^m$, combine
\[
 p^m\binom{p^m-1}{r}=(r+1)\binom{p^m}{r+1}
\]
with the prime-power row identity
$v_p\binom{p^m}{k}+v_p(k)=m$ for $0<k\le p^m$; taking $k=r+1$
shows that the valuation on the left is $m$ plus the first displayed
valuation, while the valuation on the right is $m$.

For $0<j<p^m$, necessarily $p^m\ge2$, and
\[
 (p^m-1)\binom{p^m-2}{j-1}=j\binom{p^m-1}{j}.
\]
The first part, used at columns $1$ and $j$, says that both $p^m-1$ and
$\binom{p^m-1}{j}$ are $p$-adic units.  Taking valuations gives the second
formula.  Specializing to $p=2$ gives the dyadic statement and its parity
consequence.
\end{proof}

The two generic statements in \cref{prop:prime-power-companion-rows} have the
exact Lean counterparts
\path{Fabius.primePowerSubOneChoose_padicValNat} and
\path{Fabius.primePowerSubTwoChoose_padicValNat}; the displayed dyadic
specialization is \path{Fabius.twoPowSubTwoChoose_padicValNat}.  The latter two
declarations deliberately assume the strict range $0<j<p^m$.  At the omitted
upper endpoint $j=p^m$, the binomial coefficient in row $p^m-2$ vanishes, so
the valuation identity cannot be extended there.
...
$\FallingFactorial{Mx}{k}=\sum_r s(k,r)M^{r}x^{r}$ ($s(k,r)$ the signed
Stirling numbers):
...
\begin{proof}
Differentiate the Newton formula \eqref{eq:newton-forward}.  Since
$\FallingFactorial{Mx}{k}=\sum_{j=0}^k s(k,j)M^jx^j$, its $r$th derivative at zero
is $r!M^rs(k,r)$, which proves \eqref{eq:jet-defect}.  For $r=1$ use
$s(k,1)=(-1)^{k-1}(k-1)!$, expand
$\Delta^kF(0)=\sum_{j=0}^k(-1)^{k-j}\binom{k}{j}F(j/M)$, and note
$F(0)=0$.  The coefficient of $F(j/M)$ becomes
\[
 M(-1)^{j-1}\sum_{k=j}^M\frac1k\binom{k}{j}
 =M(-1)^{j-1}\frac1j\sum_{k=j}^M\binom{k-1}{j-1}
 =M(-1)^{j-1}\frac1j\binom{M}{j},
\]
the hockey-stick identity, giving \eqref{eq:endpoint-slope}.
\end{proof}
...
\begin{tabular}{rrrrr}
\toprule
$M$ & $\Norm{F-P_M}_\infty$ (sampled) & leftmost maximizer &
sampled $\Lambda_M$ & $\log_2\Norm{F-P_M}_\infty$ \\
\midrule
...
\begin{tabular}{rrrr}
\toprule
$M$ & sign & $\log_2\AbsoluteValue{(P_M)'(0)}$ & $M-\log_2\AbsoluteValue{(P_M)'(0)}$ \\
\midrule
16   & $+$ & $-0.681$   & $16.681$ \\
32   & $-$ & $10.816$   & $21.184$ \\
64   & $+$ & $34.823$   & $29.177$ \\
...
$\log_2\Norm{F-P_M}_\infty\approx M-o(M)$ at the tested sizes and an
...
\[
  \Norm{F-P_{2^{m}}}_\infty\longrightarrow\infty.
 \]
 More strongly, along dyadic $M$,
\[
  \frac1M\log_2\Norm{F-P_M}_\infty\longrightarrow1,
 \]
...
\begin{equation}
 S_r(x):=I_x(r+1,r+1)
 =\frac{(2r+1)!}{(r!)^{2}}\int_0^{x}t^{r}(1-t)^{r}\Differential t
 =\frac{(2r+1)!}{(r!)^{2}}
...
Two distinct $\RvachevUp$ problems reduce to the same machinery.
\emph{(i) Half comb.}  On $[-1,0]$, $\RvachevUp(t)=F(1+t)$, so
...
\begin{equation}
 H^{\RvachevUp}_{M,r}(t)=(1-t^{2})^{\,r+1}\,T_{M,r}(t^{2}),
 \qquad
 \deg T_{M,r}\le \tfrac M2-1,
 \label{eq:up-factor}
\end{equation}
with $T_{M,r}$ interpolating
$\RvachevUp(t_j)/(1-t_j^{2})^{r+1}$; evenness is forced by uniqueness.
\emph{(ii) Full integer comb.}  For the comb $y_j=j/M$,
$-M\le j\le M$, of $[-1,1]$, the degree-$2M$ interpolant $R_M$ of
$\RvachevUp$ is even, so $R_M(y)=Q_M(y^{2})$ where $Q_M$ interpolates the
data $\RvachevUp(j/M)$ at the \emph{quadratically spaced} nodes
...
\begin{equation}
 \AbsoluteValue{\ell^{\circ}_j(x)}
 =\frac{\Gamma(y)\,\Gamma(M-y)\,\AbsoluteValue{\sin\pi y}}
 {\pi\,\AbsoluteValue{y-j}\,\Gamma(j)\,\Gamma(M-j)}.
...
$\AbsoluteValue{\omega(y)}=\Gamma(y)\Gamma(M-y)\AbsoluteValue{\sin\pi y}/\pi$, and
$\AbsoluteValue{\omega'(j)}=(j-1)!\,(M-1-j)!=\Gamma(j)\Gamma(M-j)$; divide.
\end{proof}
...
\begin{equation}
 \frac1M\log\,\AbsoluteValue{\ell^{\circ}_{M/2}(a)}
...
\begin{equation}
 \AbsoluteValue{L_{M/2,r}(\tfrac13)}
 =\Bigl(\frac89\Bigr)^{\!r+1}
 \frac{3\sqrt3}{\pi M}\,
 \frac{\Gamma(M/3)\,\Gamma(2M/3)}{\Gamma(M/2)^{2}}
 =\Theta\!\Bigl(M^{-1}\EulerE^{\delta M}(8/9)^{r}\Bigr),
...
$\Leb_{M,r}\ge A_{M,r}
=(1+\LittleO(1))\,2^{M+r+1}\bigl(\pi\sqrt2\,M^{r+2}\bigr)^{-1}$
grows exponentially in $M$.  Each additional endpoint derivative
buys only one algebraic factor $\asymp M^{-1}$ against $2^{M}$.
\end{theorem}

\begin{proof}
The half-cell mode in \cref{prop:modes} is one of the cardinal terms
whose absolute sum defines $\Leb_{M,r}$, so
$\Leb_{M,r}\ge A_{M,r}$.  For fixed $r$, the exact gamma product in
\eqref{eq:A-mode} and Stirling's formula give the displayed asymptotic;
its exponential factor is $2^M$, while increasing $r$ contributes only
the fixed power $M^{-r}$.
\end{proof}
...
\[
 M\bigl(\alpha+\LittleO(1)\bigr)
 +(r+1)\log\bigl[4a(1-a)\bigr]
 \ge M\alpha-\eta M\log\tfrac1{4a(1-a)}+\LittleO(M)
 =\tfrac{\alpha}2M+\LittleO(M),
\]
so the mode is at least $\EulerE^{c_1M}$ for large $M$.
...
\FabiusGlobal(2^{s}j/M)$ an explicit rational number.  It fails faster.
...
\begin{equation}
 \AbsoluteValue{\ell_{M/2}(\tfrac1{2M})}
 =\frac{\Gamma(M+\tfrac12)}
...
\begin{equation}
 \Lambda^{C^{1}}_M
 \ge\AbsoluteValue{h_{M/2}(\tfrac1{2M})}
...
$x_{M,r}=-\dfrac{r/M}{W_{-1}(-r/M)}\bigl(1+\LittleO(1)\bigr)$, up to a
...
\midrule
$\RvachevUp$ & 8  & $2.02\times10^{-1}$ & $3.07\times10^{-2}$ & $1.96\times10^{-2}$ & $1.34\times10^{-2}$ & $2.55\times10^{-1}$\\
$\RvachevUp$ & 16 & $1.97\times10^{-1}$ & $2.67\times10^{-2}$ & $4.17\times10^{-3}$ & $9.85\times10^{-4}$ & $1.06\times10^{-2}$\\
$\RvachevUp$ & 32 & $4.50\times10^{2}$ & $1.46\times10^{1}$ & $6.76\times10^{-1}$ & $4.45\times10^{-3}$ & $4.08\times10^{-5}$\\
$\RvachevUp$ & 64 & $1.05\times10^{10}$ & $1.75\times10^{8}$ & $4.52\times10^{6}$ & $7.72\times10^{3}$ & $9.49\times10^{-1}$\\
\bottomrule
\end{tabular}
\captionof{table}{Sampled maximum errors at fixed jet orders
($\RvachevUp$ on the symmetric comb of $[-1,1]$; level-$11$/-$12$ check
...
\begin{tabular}{@{}crrr@{}}
\toprule
$r$ & error for $F$ & error for $\RvachevUp$ & sampled $\Leb_{64,r}$ \\
\midrule
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\textwidth]{fabius_error_profile_N64.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\textwidth]{fabius_N32_interpolants.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\textwidth]{cardinal_modes_N64.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\textwidth]{conditioning_window_N64.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{balancing_law.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{endpoint_derivative_deficit.png}
\caption{The binary deficit $M-\log_2\AbsoluteValue{(P_M)'(0)}$ of
...
\begin{equation}
 ...
 F\!\left(\frac jM\right)t^{\,j-1}\Differential t,
...
For $f\in C^{2s+2}$,
\begin{equation}
 \Norm{f-\mathcal H^{\mathrm{loc}}_{M,s}f}_\infty
 \le\frac{\Norm{f^{(2s+2)}}_\infty}{(2s+2)!}
...
\begin{equation}
 ...
 \qquad
 \Norm{\RvachevUp-\mathcal H^{\mathrm{loc}}_{M,s}\RvachevUp}_\infty
...
$\binom{2s+3}2$ to $(s+1)(2s+1)$; for $\RvachevUp$ on $[-1,1]$, $h_0=2/M$
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\textwidth]{global_vs_local.png}
...
translates of $\RvachevUp$ itself (the refinement equation makes the dyadic
...
Let $r^{F}_M$ and $r^{\RvachevUp}_M$ minimize the true uniform errors of the
endpoint-flat families, and let $r^{\Leb}_M$ minimize
$\Leb_{M,r}$.  All three satisfy
\begin{equation}
 r_M=\frac{M\log2}{\log M}\bigl(1+\LittleO(1)\bigr),
...
$E^{*}_M:=\min_{r\ge0}\Norm{F-H_{M,r}}_\infty$ does not tend to zero;
more strongly $\log E^{*}_M$ is eventually linear in $M$ up to
$\LittleO(M)$, and for the Rvachev version
$\limsup E^{*,\RvachevUp}_M=+\infty$.  The measured sequence
