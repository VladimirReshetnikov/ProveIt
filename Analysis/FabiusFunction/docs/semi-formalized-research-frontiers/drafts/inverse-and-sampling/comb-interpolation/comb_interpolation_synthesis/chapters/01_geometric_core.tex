...
\path{FabiusFunction.GeometricRichardsonGenerating}; that current formal API is
...
gap-growth bound, or the closed geometric Gaussian--Appell decoder formula and
prefactor.  The scalar and Matrix Lagrange--Rvachev APIs cover the generic
synthesis, coefficient, nodewise-biorthogonality, decoder-row-sum, and finite
Markov/right-inverse layers.  Some remaining statements follow on paper from
formalized finite ingredients, but
...
\item \textbf{Atomic synthesis.}  The generic scalar decoder synthesis,
      ...
      remaining task for the preceding compound decoder theorem is to derive
      the geometric elementary-symmetric decoder and Gaussian
      prefactor in
      \eqref{gq:eq:decoder-entry-explicit}--\eqref{gq:eq:decoder-gaussian-prefactor}
      in Lean.  Decoder optimization remains the open problem
