...
audits the later source modules
\path{FabiusFunction.LagrangeRvachevSynthesis},
\path{FabiusFunction.LagrangeRvachevMatrix}, and
\path{FabiusFunction.GeometricRichardsonGenerating}, and
\path{FabiusFunction.RvachevAppellHasse}; that current formal API is
...
\item finite Appell--Hasse and explicit geometric-decoder formulas, including
      \path{rvachevReciprocalMomentRat_odd},
      \path{rvachevDeconvolvedPolynomial_eq_sum_even_hasseDeriv},
      \path{eval_rvachevDeconvolvedPolynomial_qFallingPower},
      \path{geometric_nodalWeight_eq_geometricQPochhammer}, and
      \path{geometric_lagrangeRvachevDecoder_eq};
\item the corresponding typed finite matrices
      \path{lagrangeRvachevEncoderMatrix} and
      \path{lagrangeRvachevDecoderMatrix}, with encoder nonnegativity and row
      stochasticity, decoder row unitality, and the exact right inverse in
      \path{lagrangeRvachevEncoderMatrix_nonneg},
      \path{sum_lagrangeRvachevEncoderMatrix_row_eq_one},
      \path{sum_lagrangeRvachevDecoderMatrix_row_eq_one}, and
      \path{lagrangeRvachevEncoderMatrix_mul_decoderMatrix}.
\end{enumerate}
...
or the gap-growth bound.  The scalar, Hasse, and Matrix Lagrange--Rvachev APIs
cover the generic synthesis, q-falling and geometric closed formulas,
coefficient, nodewise-biorthogonality, decoder-row-sum, and finite
Markov/right-inverse layers.  They do not add an analytic reciprocal-MGF
power-series identity or decoder optimization.  Some remaining statements
follow on paper from formalized finite ingredients, but
...
Finite interpolation requires the nodes to be pairwise distinct.  For
$q\ne0$, this is equivalent to $q^d\ne1$ for $1\le d\le n$.  The exceptional
base $q=0$ gives a distinct grid only for $n\le1$ and repeated zero nodes
thereafter.  Most algebraic formulae remain valid over an arbitrary field
under the direct distinctness hypothesis.  The convergence and Lebesgue
theory will assume
\begin{equation}
 0<q<1,
 \qquad c>0,
...
The exact formal-series counterpart is
\path{Fabius.geometricLagrangeRichardson_generating} in
\path{FabiusFunction.GeometricRichardsonGenerating}.  It holds over a field
for $q\ne0$; no root-of-unity exclusion is needed for this algebraic identity
with Lean's totalized inverses.  Genuine interpolation still requires the
nodes to be pairwise distinct, equivalently for nonzero $q$,
$q^d\ne1$ for $1\le d\le n$ in each row.  The separate explicit-convolution
theorem \path{Fabius.geometricRichardsonTransform_generating} is unconditional
over commutative rings, including $q=0$, but does not supply a Lagrange
interpolation interpretation there.  In the formal theorem,
$(qz;q)_\infty$ denotes the Euler-coefficient series
\path{qPochhammerSeries} after \path{PowerSeries.rescale} by $q$, not a claim
that a literal fixed-$q$ product converges $z$-adically.  The analytic companion
\path{Fabius.hasSum_geometricLagrangeRichardson_mul_pow} is a
\path{HasSum} statement over a complete normed field under
$\lVert q\rVert<1$, $q\ne0$, and absolute summability of the normalized data
series at $z$.
...
Let $\FabiusGeometricNewtonCoefficient{k}{q}$ be the coefficient of
$N_k(x;1,q)$ in the Newton interpolant to $F$ at $1,q,\ldots,q^k$:
\begin{equation}
 \FabiusGeometricNewtonCoefficient{k}{q}=[F;1,q,\ldots,q^k].
...
\begin{equation}
 \boxed{
 \FabiusGeometricNewtonCoefficient{k}{q}=\frac1{(q;q)_k}
 \sum_{j=0}^{k}(-1)^j
...
\begin{equation}
 \boxed{
 (\Interp_nF)(x)=\sum_{k=0}^{n}\FabiusGeometricNewtonCoefficient{k}{q}
 \prod_{r=0}^{k-1}(x-2^{-r}).}
...
\begin{align}
 \FabiusGeometricNewtonCoefficient{0}{1/2}&=1,
 &\FabiusGeometricNewtonCoefficient{1}{1/2}&=1,\notag\\
 \FabiusGeometricNewtonCoefficient{2}{1/2}&=-\frac{26}{27},
 &\FabiusGeometricNewtonCoefficient{3}{1/2}&=-\frac{128}{27},
 \label{gq:eq:first-fabius-newton-1}\\
 \FabiusGeometricNewtonCoefficient{4}{1/2}&=-\frac{4253248}{637875},
 &\FabiusGeometricNewtonCoefficient{5}{1/2}&=\frac{189673472}{19774125}.
...
\begin{equation}
 \OrderOperator_{\xi=m}\FourierTwoPi{u}(\xi)=1+\TwoAdicValuation(m).
...
\path{eval_rvachevDeconvolvedPolynomial_qFallingPower} proves
\eqref{gq:eq:q-Appell-falling} with the finite Bell/binomial-convolution
reciprocal Rvachev moments.  Combined with
\path{normalized_sum_Ioo_rvachevDeconvolvedPolynomial_mul_shifted_rvachevUp}
at $M=2^n$, the exact degree of the $q$-falling power, and $k\le n$, it also
proves \eqref{gq:eq:q-falling-up-synthesis}.  Thus
\cref{gq:prop:q-Appell-falling} is recorded as Lean-proved.  This finite
identity does not separately formalize the analytic reciprocal-MGF expansion
\eqref{gq:eq:up-inverse-moments}.
...
node families by
\path{normalized_sum_Ioo_lagrangeRvachevDecoder_mul_shifted_rvachevUp}.
The downstream theorem \path{geometric_lagrangeRvachevDecoder_eq} proves
the geometric elementary-symmetric formula
\eqref{gq:eq:decoder-entry-explicit} together with the Gaussian prefactor
\eqref{gq:eq:decoder-gaussian-prefactor}.  Lean writes the latter as division
by $c^nq^{\binom{j}{2}+j(n-j)}$, which under $c>0$ and $0<q<1$ is exactly
the displayed $c^{-n}q^{-nj+\binom{j+1}{2}}$ form.  Combined with the generic
synthesis theorem at $M=2^n$, these declarations prove all three formulas,
so \cref{gq:thm:gaussian-Appell-decoder} is recorded as Lean-proved.  The
algebraic coefficient theorem is totalized even at zero or colliding nodes;
only the manuscript's distinct positive nodes carry the cardinal
interpretation.
...
\paragraph{Formalization boundary.}
In \path{FabiusFunction.LagrangeRvachevMatrix}, the definitions
\path{lagrangeRvachevEncoderMatrix} and
\path{lagrangeRvachevDecoderMatrix} specialize here to $hB$ and $D$.
The declarations \path{lagrangeRvachevEncoderMatrix_nonneg},
\path{sum_lagrangeRvachevEncoderMatrix_row_eq_one},
\path{sum_lagrangeRvachevDecoderMatrix_row_eq_one}, and
\path{lagrangeRvachevEncoderMatrix_mul_decoderMatrix} prove, respectively,
all four assertions above under the displayed admissibility hypotheses.  This
theorem is therefore recorded as Lean-proved in the concordance; the separate
closed Gaussian formula for the decoder remains manuscript-only.

Thus a nonnegative averaging map has an exact polynomial decoder.  Whenever
two distinct encoder rows overlap in a strictly positive column,
\path{exists_lagrangeRvachevDecoderMatrix_entry_neg_of_row_overlap} produces
a negative decoder entry; this is the precise condition under which the
formal API forces signs.  The size of that cancellation reflects the geometric
Lebesgue growth.
...
The matrix $hB$ in \Cref{gq:thm:gaussian-Appell-biorthogonality} is a
nonnegative row-stochastic sampling operator.  Its right inverse $D$ has row
sum one and, whenever two distinct encoder rows overlap in a strictly positive
column, must have a negative entry.  The size of its row variation controls
amplification from nodal perturbations to atom amplitudes.
...
\item \textbf{Atomic synthesis.}  The generic scalar decoder synthesis,
      ...
      \path{FabiusFunction.LagrangeRvachevSynthesis}; the typed Markov encoder,
      decoder, row laws, exact right inverse, and conditional sign obstruction
      are formalized in \path{FabiusFunction.LagrangeRvachevMatrix}.  The
      finite Appell--Hasse, q-falling, geometric elementary-symmetric decoder,
      and Gaussian prefactor formulas are formalized in
      \path{FabiusFunction.RvachevAppellHasse}.  Decoder optimization remains the open problem
