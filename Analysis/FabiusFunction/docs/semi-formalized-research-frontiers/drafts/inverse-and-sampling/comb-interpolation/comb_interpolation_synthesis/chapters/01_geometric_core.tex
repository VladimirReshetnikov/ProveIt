...
\path{normalized_sum_Ioo_rvachevDeconvolvedPolynomial_mul_shifted_rvachevUp}
at (M=2^n), the exact degree of the q-falling power, and (kle n), it also
...
by
(c^nq^{inom{j}{2}+j(n-j)}), which under (c>0) and (0<q<1) is exactly
the displayed (c^{-n}q^{-nj+inom{j+1}{2}}) form.  Combined with the generic
