...
\[
 F_q(q^n)=\frac{q^{n(n+1)/2}}{(1-q)^n n!}\,\Expectation X_q^n.
\]
...
\[
 ...
 \sum_{j=0}^n(-1)^j\GaussianBinomial nj2\frac{\Expectation X^j}{j!}
 =2^{n(n+1)/2}\sum_{j=0}^n
 \frac{(-1)^j\Expectation X^j}{j!\,\mathfrak M_j\mathfrak M_{n-j}},
\]
...
Fix $a\in\ComplexNumbers\setminus\{0\}$ and a base $q\in\ComplexNumbers$ such that
...
\begin{equation}
 \Phi_k(aq^j)=
 \begin{cases}
 (-a)^kq^{\binom k2}(q;q)_k\GaussianBinomial jkq,&j\ge k,\\
 0,&j<k.
 \end{cases}
...
\begin{equation}
 ...
 \qquad
 (P_q)_{j,k}=\GaussianBinomial jkq,
 \qquad
...
\begin{equation}
 (P_q^{-1})_{j,k}
 =(-1)^{j-k}q^{\binom{j-k}{2}}\GaussianBinomial jkq
 \qquad (j\ge k).
...
\begin{align*}
 ...
 &=(-a)^kq^{\binom k2}(q;q)_k\GaussianBinomial jkq.
\end{align*}
...
\(
 \GaussianBinomial jrq\GaussianBinomial rkq=\GaussianBinomial jkq\GaussianBinomial{j-k}{r-k}q
\), the off-diagonal entry is a scalar multiple of
\[
 \sum_{s=0}^{j-k}(-1)^sq^{\binom s2}\GaussianBinomial{j-k}{s}q
 =(1;q)_{j-k}=0.
\]
...
\begin{align}
 ...
 q^{\binom{k-j}{2}}\GaussianBinomial kjq x^j,
 \label{eq:newton-to-monomial}\\
 x^m
 &=\sum_{k=0}^m\GaussianBinomial mkq a^{m-k}\Phi_k(x).
...
below gives $a^{m-k}\GaussianBinomial mkq$.
\end{proof}
...
\begin{equation}
 A_k=\sum_{r\ge0}c_{k+r}\GaussianBinomial{k+r}{k}q a^r.
 \label{eq:formal-Ak}
\end{equation}
...
\[
 (-1)^{n-k}q^{\binom{n-k+1}{2}}\GaussianBinomial nkq,
\]
...
in $\IntegerNumbers[q,z]$.  It remains meaningful at roots of unity even when the normalized
...
For $0<q<1$,
\begin{equation}
 \sum_{k=0}^n\AbsoluteValue{\lambda_{n,k}^{(q)}}
 =\frac{(-q;q)_n}{(q;q)_n}
...
\begin{align*}
 ...
 q^{\binom{r+1}{2}}\GaussianBinomial nrq\\
 &=\frac{(-q;q)_n}{(q;q)_n},
\end{align*}
...
\[
 (\Mellin f)(z)=\int_0^\infty f(x)x^{z-1}\Differential x,
\]
one has
\[
...
\begin{equation}
 ...
 =\sum_{s=0}^r x^s a^{r-s}\GaussianBinomial{n+r-s}{n}q.
...
\[
 ...
 =\sum_{m\ge0}\GaussianBinomial{n+m}{n}q a^mt^m
\]
...
\begin{equation}
 ...
 =\Omega_{n+1}(x)
 \sum_{r\ge0}c_{n+1+r}
 \sum_{s=0}^r x^sa^{r-s}\GaussianBinomial{n+r-s}{n}q.
...
\begin{equation}
 ...
 \sum_{r\ge0}c_{n+1+r}a^r\GaussianBinomial{n+r}{n}q.
...
$|a|<R$, then
\begin{equation}
 \AbsoluteValue{(\Gop_{n,q}f)(a)-f(0)}
 \le
...
$\GaussianBinomial{n+r}{n}q\le1/(q;q)_n$ for $0<q<1$.  Apply these estimates to
...
\(
 \GaussianBinomial kj{q^{-1}}=q^{-j(k-j)}\GaussianBinomial kjq
\).  Division by
...
\begin{equation}
 ...
 =\sum_{r\ge0}c_{k+r}\GaussianBinomial{k+r}{k}q a^r.
...
$\GaussianBinomial{k+r}{k}q\le1/(q;q)_\infty$,
\[
 |A_k|\le C_1\rho^{-k}.
\]
...
More precisely,
\begin{equation}
 \AbsoluteValue{R_{n,q}(\xi)-\frac1{\widehat u_q(\xi)}}
 \le
...
\[
 h_\xi(t)=\widehat u_q(t\xi)=\Expectation\EulerE^{-2\pi\ImaginaryUnit t\xi Z_q}.
\]
...
the following finite synthesis theorem.  Let $u=\RvachevUp$ be the classical up
density, let
\begin{equation}
 \mathsf M_u(z)=\int_{-1}^1\EulerE^{zx}u(x)\Differential x,
 \qquad
...
For the full set,
$e_s(1,q,\ldots,q^n)=q^{\binom s2}\GaussianBinomial{n+1}{s}q$ by
