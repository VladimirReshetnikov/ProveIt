...
function $F$ or Rvachev's compactly supported function $\RvachevUp$ on the
...
$m\ge\tau(p):=2\Ceiling{p/2}$.  The engine is the order-$m+\TwoAdicValuation(\ell)$
...
Dirichlet series
$D(s)=\sum_{q\ \mathrm{odd}}\widehat{\RvachevUp}(q/2)\,q^{-s}$, and the defect
identity forces $D(2r)/\pi^{2r}\in\RationalNumbers$ with
\[
...
$\Leb_{M,r}\ge C\EulerE^{cM}$ uniformly in $r$, and by $M=256$ even the
...
\begin{tabular}{@{}ll@{}}
\toprule
$m\in\NaturalNumbers$ & the dyadic level\\
$M=2^m$, $h=1/M$ & the comb size and mesh\\
$x_k=k/M=kh$ & the comb nodes\\
$F$ & the Fabius function on $[0,1]$, $F(0)=0$, $F(1)=1$\\
$\RvachevUp$ & Rvachev's even function on $[-1,1]$, $\RvachevUp(0)=1$\\
$\rho=F'$ & the Fabius probability density\\
$\eps_a=(-1)^{\BinaryDigitSum(a)}$ & the Thue--Morse sign, $\BinaryDigitSum$ the binary digit sum\\
$\ThetaTM_m(z)=\prod_{j<m}(1-z^{2^j})$ & the finite Thue--Morse product\\
$\Phi=\widehat{\RvachevUp}$ & the infinite sinc product \eqref{eq:Phi-product}\\
...
$\int_0^1\bigl(M^{-1}\sum_{a<M}f((a+\theta)/M)\bigr)\Differential\theta
=\int_0^1f$.  Hence:
...
$C(z)=\sum_{a<b}c_a\EulerE^{az}$ with a zero of order $\rho$ at the
...
$(N-1)\TwoAdicValuation(b)\ge d+1$ each layer and the whole remaining tail are
...
thresholds $\TwoAdicValuation(M_j)\ge p_j+1$ --- so a coordinate of low degree
...
proved for complex powers: for every fixed $\alpha\in\ComplexNumbers$ and
$0<\theta<1$, since $x^{\alpha}F(x)$ is $C^\infty$ and flat at $0$,
\begin{equation}
 \frac1M\sum_{a=0}^{M-1}x_a^{\alpha}F(x_a)
 =\int_0^1x^{\alpha}F(x)\Differential x
...
problem: for $p\to\infty$ with $\TwoAdicValuation(M)-p$ bounded, match the
...
$\beta_{2m}=\Expectation B_{2m}(Y)$ with the spectral identity
...
$\TwoAdicValuation(M)>d$ threshold, the dyadic first defect, the $F$-side Ruffa
...
lemma $\AbsoluteValue{\Phi(q\ell/2)}\le\AbsoluteValue{\Phi(q/2)}/\ell$
