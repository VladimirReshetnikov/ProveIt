...
With $X=\sum_{j\ge1}2^{-j}V_j$ ($V_j$ i.i.d.\ uniform on $[0,1]$) and
...
\begin{equation}
 \boxed{\;D=z\,\EndpointKernel(z,D),\;}
 \qquad
 \EndpointKernel(z,w)=-\frac1L\mathcal R(z,w),
...
\begin{equation}
 ...
 [z^{\,n-k}w^{\,k-1}]\;\EndpointKernel(z,w)^{k}
 =\sum_{k=1}^{n}\frac{(-1)^{k}}{k\,L^{k}}\,
 [z^{\,n-k}w^{\,k-1}]\;\mathcal R(z,w)^{k},
 \;}
 \label{ao:eq:diagonal-dn}
\end{equation}
a finite formula: only the $z^{\,n-1}$-truncation of $\EndpointKernel$, the jets
...
\begin{equation}
 ...
 +\sum_{k=1}^{n}\frac1k\,
 [z^{\,n-k}w^{\,k-1}]\;
 \partial_wK(z,w)\,\EndpointKernel(z,w)^{k},
...
\begin{proof}
Let $W(t,z)$ solve $W=t\,\EndpointKernel(z,W)$.  For fixed $z$, ordinary
Lagrange--B\"urmann inversion in $t$ gives
$[t^{k}]W=\tfrac1k[w^{k-1}]\EndpointKernel(z,w)^{k}$ and the composition form
$K(z,W)=K(z,0)+\sum_k\tfrac{t^{k}}k[w^{k-1}]\partial_wK\cdot\EndpointKernel^{k}$.
...
Expanding $\EndpointKernel=\sum\phi_{r,s}z^{r}w^{s}$ gives the fully unrolled
partition form
\[
 d_n=\sum_k\tfrac1k
...
only as far as the bidegrees require; truncate $\EndpointKernel$ at
...
independent of $\ell$, so the $z^{0}$ slice of $\EndpointKernel$ is
$\EndpointKernel(0,w)=a\ell+\phi(w)$ with
\begin{equation}
 a=-\frac1{2L},
 \qquad
...
\begin{proof}
In \eqref{ao:eq:diagonal-dn} a product $\EndpointKernel^{k}$ has $\ell$-degree at
most $k\le n$, and for $k=n$ the coefficient requested is
$[w^{n-1}]\EndpointKernel(0,w)^{n}/n$, whose nominal $\ell^{n}$ term carries no
...
\begin{equation}
 ...
 &=g_N(\log\rho_{m,u},u)
   +\BigO_N\Bigl(
     \frac{(1+\log\rho_{m,u})^{N+1}}{\rho_{m,u}}
   \Bigr).
 \end{aligned}
...
\begin{equation}
 ...
 \mathcal R_W(z,u)
 &=\frac L2u^{2}+\frac12\log(1+\beta z+zu)-\Psi(\vartheta+u)\\
 &\quad-\sum_{j\ge1}
   \frac{A_j(\vartheta+u)\,z^{j}}{(1+\beta z+zu)^{j}}.
 \end{aligned}
 \label{ao:eq:RW}
\end{equation}
...
$\EndpointKernel_W=-\mathcal R_W/L$, and the ratio identity becomes
...
$D_W=z\EndpointKernel_W(z,D_W)$ therefore has unique coefficients
...
\subsection{First coefficients and the universal
\texorpdfstring{$5/24$}{5/24}}
...
$\EndpointKernel_W(z,u)$ is bounded on the $\sigma_1$ strip by
\[
 C_0K_0^{r+s}(r+s)!.
\]
...
\bottomrule
\end{tabular}
\caption{Relative errors of the W-resummed approximants
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{assets/endpoint/wright-omega/figures/carrier_comparison.png}\\[0.3em]
\includegraphics[width=0.78\textwidth]{assets/endpoint/wright-omega/figures/scaled_residuals.png}
...
\bottomrule
\end{tabular}
\caption{Exact-quantile tests: the new third correction
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{assets/endpoint/all-orders/figures/endpoint_error_plot.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{assets/endpoint/dyadic-completion/figures/psi_periodic.png}\\[0.35em]
\includegraphics[width=0.72\textwidth]{assets/endpoint/dyadic-completion/figures/dyadic_tail_convergence.png}
...
summand requests $z^{\,n-k}w^{\,k-1}$: no $\EndpointKernel$-power beyond
...
$[t^{k}]W=\tfrac1k[w^{k-1}]\EndpointKernel^{k}$ and the diagonal specialization
