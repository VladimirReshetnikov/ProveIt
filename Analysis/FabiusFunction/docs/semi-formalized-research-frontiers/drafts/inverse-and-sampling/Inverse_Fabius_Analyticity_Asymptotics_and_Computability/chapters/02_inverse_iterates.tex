...
the totalized $K_n$ is identically zero on a left-hand interval.  An analytic
germ there would therefore vanish on a two-sided neighbourhood by the
identity theorem, contradicting strict increase on $(0,1)$.  Reflection,
$K_n(1-y)=1-K_n(y)$, gives the same contradiction at $1$.
\end{proof}
...
$Y_n=G_n(E_n)$.  The homeomorphism $G_n:(0,1)\to(0,1)$ preserves density and
...
\begin{proof}[Proof of \cref{ii:thm:strong}]
At every $x\in E_n$, \cref{ii:prop:dominant-lower-bound} and $m!\leq m^m$ give along an
...
\end{align}
This proves \eqref{ii:eq:infinite-limsup}.  The Cauchy--Hadamard formula then gives zero
radius of convergence for the Taylor series at $x$.
\end{proof}

\begin{proof}[Proof of \cref{ii:thm:main}]
...
\begin{proof}[First proof of \cref{ii:thm:inverse-main} on $(0,1)$]
Suppose $K_n$ were real analytic at some $y_0\in(0,1)$.  Its derivative is nonzero by
\eqref{ii:eq:inverse-iterate-derivative}.  The real-analytic inverse-function theorem then
...
\begin{proof}[Proof of \cref{ii:thm:inverse-strong}]
Let $E_n$ be the co-countable dense set in \eqref{ii:eq:En} and define
\begin{equation}
 Y_n:=G_n(E_n).
 \label{ii:eq:Yn}
\end{equation}
...
$y=G_n(x)$ with $x\in E_n$, then $R_{G_n}(x)=0$ by \cref{ii:thm:strong};
\cref{ii:thm:formal-reversion} gives $R_{K_n}(y)=0$.  The Cauchy--Hadamard formula is
exactly the limsup assertion in \eqref{ii:eq:inverse-zero-radius}.
\end{proof}
...
$1$.  Together with the interior proof, this completes \cref{ii:thm:inverse-main} under
...
term is \eqref{ii:eq:iterated-inverse-endpoint}, and each iteration transports rather than
...
\cref{ii:prop:fixed-k-minimum} gives the two profiles.
\end{proof}
