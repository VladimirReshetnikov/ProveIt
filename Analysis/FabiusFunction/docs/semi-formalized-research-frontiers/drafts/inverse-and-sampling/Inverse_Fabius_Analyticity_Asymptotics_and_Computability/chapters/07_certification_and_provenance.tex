...
The machine-readable raw source inventory records every theorem-style source
environment at the immutable pre-retirement revision.  Its canonical
disposition fields are still blank, and the reviewed theorem concordance is a
publication-gate deliverable that does not yet exist.  When that review is
complete, several source rows may map to one canonical theorem when one
statement is a specialization or a later duplicate of a stronger result.
Such a many-to-one mapping will be deduplication, not loss: the source
statement, source line, surviving label, logical status, and reason for the
disposition must remain explicit.
