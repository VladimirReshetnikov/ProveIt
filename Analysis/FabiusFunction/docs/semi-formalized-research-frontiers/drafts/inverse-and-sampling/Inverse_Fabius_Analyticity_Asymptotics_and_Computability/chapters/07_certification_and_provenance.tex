...
classification is \(39\) Lean-proved rows, \(106\) human-proved frontier
...
Three further source rows now have exact compiled counterparts.  The centered
...
Three results in the self-sampling chapter have substantial formal kernels
...
\item[\cref{is:p3:thm:Appell-lattice-reproduction}.]
Specializing
\path{Fabius.normalized_tsum_shifted_rvachevDeconvolvedPolynomial_mul_rvachevUp}
to \(M=2^N\) and \(P=X^n\) proves the arbitrary-phase formula for
\(0\le n\le N\).  The same paper theorem additionally asserts degree
\(N+1\) at the parity-selected superconvergent phases.  That stronger clause
has no exact Lean counterpart, so the whole labelled result remains
human-proved.
\end{description}
...
(748) distinct labels, (591) references, and all (194) immutable source
