...
\begin{equation}
 \OrderOperator_{\xi=m}\PhiCyc=1+\TwoAdicValuation(m).
 \label{is:p3:eq:integer-multiplicity}
\end{equation}
Indeed, the factors indexed by $0\le j\le\TwoAdicValuation(m)$ vanish simply and all later factors are nonzero.
...
$N+1+\TwoAdicValuation(\ell)$, so the full sum begins at order $N+1$.
\end{proof}
...
$N+1+\TwoAdicValuation(\ell)\le r$.  Write $\ell=2^aq$ with $q$ odd.
\end{proof}
...
Let $m=2^{d-1}q\ne0$, where $q$ is odd and $d=1+\TwoAdicValuation(m)$.  Then, for $w$ near $0$,
\begin{equation}
 \boxed{\displaystyle
 \PhiCyc(m+w)
...
\(U_m\) and \(d=1+\TwoAdicValuation(|m|)\), Lean proves every higher jet as
\[
...
\[
 \sum_{m=1}^{j}\bigl(1+\TwoAdicValuation(m)\bigr)
 =j+\TwoAdicValuation(j!)
 =2j-\BinaryDigitSum(j),
\]
where the last equality is Legendre's formula
$\TwoAdicValuation(j!)=j-\BinaryDigitSum(j)$.  Its parity is therefore
...
Only the principal \(0\le n\le N\) clause of
\cref{is:p3:thm:Appell-lattice-reproduction} is formalized: specialize the
last declaration to \(M=2^N\) and \(P=X^n\).  The same paper theorem also
asserts degree \(N+1\) at the parity-selected superconvergent phases, and that
stronger clause has no exact Lean counterpart.  The whole labelled lattice
theorem therefore remains a human-proved frontier result.
\end{statusbox}
