...
The fixed-target and modulus parts of this proposition now have an exact
formal kernel in \path{FabiusInverseExactDyadicModulus.lean}.  At a fixed
...
$r(n)$ at positive inputs.  Its positive-input equation is
...
The concordance row for \cref{co:prop:exact-dyadic-modulus} nevertheless
remains a \emph{human-proved frontier result}.  The proposition also says that
the logarithmic function $d_*$ is recursive, whereas the new module exports no
named \texttt{Primrec} or computability theorem for either exact denominator.
Thus Lean now proves every fixed-dyadic leastness clause and the logarithmic
witness, but not that separate recursion clause; promoting the whole paper
proposition to Lean-proved would overstate the formal surface.
