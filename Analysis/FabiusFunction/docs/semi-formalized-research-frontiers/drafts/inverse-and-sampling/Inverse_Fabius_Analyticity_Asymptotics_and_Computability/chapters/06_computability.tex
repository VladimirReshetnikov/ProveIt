...
This proposition now has an exact Lean counterpart in
\path{FabiusInverseExactDyadicModulus.lean}.  At a fixed
order $r$, \path{Fabius.inverseFabiusExactDyadicDenominator} defines
$\Ceiling{1/F(2^{-r})}$ through the exact rational evaluator;
\path{Fabius.inverseFabiusExactDyadicDenominator_primrec} certifies this
fixed-order denominator as primitive recursive;
\path{Fabius.inverseFabiusExactDyadicDenominator_pos},
\path{Fabius.inv_inverseFabiusExactDyadicDenominator_le_fabiusAtInverseTwoPow},
and \path{Fabius.inverseFabiusExactDyadicDenominator_isLeast} prove its
positivity, endpoint-mass bound, and arithmetic leastness.  The strict modulus,
the endpoint counterexample for every smaller positive denominator, and
leastness among strict integer moduli are respectively
\path{Fabius.abs_fabiusInv_sub_lt_inverse_two_pow_of_lt_exactDyadicDenominator},
\path{Fabius.exists_fabiusInv_gap_of_lt_exactDyadicDenominator}, and
\path{Fabius.inverseFabiusExactDyadicDenominator_isLeast_strictModulus}.

The definition \path{Fabius.inverseFabiusExactLogarithmicDenominator} then
sets the value at zero to $1$ and composes the fixed-target denominator with
$r(n)$ at positive inputs.  The theorem
\path{Fabius.inverseFabiusExactLogarithmicDenominator_primrec} proves that
this totalized function is primitive recursive.  Its positive-input equation is
\path{Fabius.inverseFabiusExactLogarithmicDenominator_of_pos}, and
\path{Fabius.abs_fabiusInv_sub_lt_inv_nat_of_lt_exactLogarithmicDenominator}
is exactly the implication to the weaker $1/n$ target.  That last theorem is
only a witness: it does not assert leastness for $1/n$, and its zero branch is
only the convention $d_*(0)=1$ with no modulus conclusion there.

The two recursion certificates use a natural code rather than a computability
instance for normalized rationals.  If
$F(2^{-r})=H_r/D_r$ is the formalized exact quotient, then the ceiling is
computed as $(D_r+H_r-1)/H_r$.  The development proves $H_r$ and $D_r$
primitive recursive, and then composes the resulting fixed-order denominator
with the already primitive-recursive $r(n)$.  Consequently every clause of
\cref{co:prop:exact-dyadic-modulus} now has a compiled Lean counterpart, and
its concordance row is classified as \emph{Lean-proved}.
...
fixed-dyadic denominator minimality, is therefore translated into Lean.  The
Delta and stronger factorial denominators remain useful closed-form witnesses
that avoid invoking the exact dyadic evaluator at the formalized least order
$r(n)$.
