...
$\widehat c_m:=m!c_m(y)$.  With $\mathcal B_{m,k}$ denoting the partial exponential Bell
polynomial and $\mathcal B_{0,0}=1$, the coefficients obey
\begin{align}
 c_m(y)=-\frac1{F'(x)}\Bigg[&
 \frac1{m!}\sum_{k=2}^{m}F^{(k)}(x)
 \mathcal B_{m,k}(\widehat c_1,\ldots,\widehat c_{m-k+1})
 \notag\\
 &+\sum_{j=1}^{m}\frac1{(m-j)!}
 \sum_{k=0}^{m-j}a_j^{(k)}(x)
 \mathcal B_{m-j,k}(\widehat c_1,\ldots,\widehat c_{m-j-k+1})
 \Bigg].
...
\[
 \delta_n^{[J]}(y):=\sum_{m=1}^{J-1}c_m(y)Q_n^m.
\]
Substitute $x+\delta_n^{[J]}$ into the finite expansion
\eqref{is:p1:eq:an-def}.  The coefficient of $Q_n^m$ in
...
$\operatorname{Var}(\Lambda)=\pi^2/3$ give
$\operatorname{Var}(Z)=\sum_{j\ge1}(3\cdot4^j)^{-1}=1/9$.
