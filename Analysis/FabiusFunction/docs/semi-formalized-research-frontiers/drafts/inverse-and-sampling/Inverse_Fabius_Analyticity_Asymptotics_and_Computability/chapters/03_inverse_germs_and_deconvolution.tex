...
Let $q=1/4$ and abbreviate $Q_n=q^n$.  From \eqref{is:p1:eq:forward-prefix-expansion},
\begin{equation}
...
\]
Substitute $x+\delta_n$ into \eqref{is:p1:eq:an-def} and use
...
through order $J-1$ into the uniform remainder in \eqref{is:p1:eq:Gn-expansion}.
\end{proof}
...
\eqref{is:p2:eq:w-annihilation,is:p2:eq:w-residual} become exactly
...
$M(t)=\Phi(\ImaginaryUnit t)$, \eqref{is:p1:eq:b-def} gives
\begin{equation}
...
\eqref{is:p1:eq:A3local}, while $G_n(143/2073600)$ is governed by the quartic
...
\begin{align}
 G_n(5/72)-\frac14&\sim\frac49\,4^{-n},\label{is:p1:eq:quarter-raw-law}\\
...
directly by repeated integration by parts and \eqref{is:p2:eq:mean-value-inversion}.
\end{proof}
...
\begin{proof}
Independence and \eqref{is:p2:eq:logistic-cf} give
\[
 \Expectation\EulerE^{\ImaginaryUnit tZ}
 =\prod_{j\ge1}
...
\eqref{is:p2:eq:A-logistic-moment}.  The affine identity \eqref{is:p2:eq:A-P-affine} gives
\eqref{is:p2:eq:P-logistic-moment}.  Setting $x=0$ and using symmetry of $Z$ proves
...
Thus $M(t)^{-1}=\Phi(-\ImaginaryUnit t)^{-1}$ is not merely a formal
...
product to the imaginary Fourier axis.  The function $1/\Phi(\xi)$ on the
...
\begin{equation}
 ...
 =S_n^p\int_{-1}^{1}u(y)^p\Differential y.
...
\[
 ...
 =2^{-n}\sum_{k=0}^{2^n-1}
   \int_{-1}^{1}H\!\left(S_n(-1)^{\BinaryDigitSum(k)}u(y)\right)\Differential y.
\]
...
$\fall nN$ is zero for $n<N$.  At $n=N$ it equals $N!$ and $x^{n-N}=1$,
...
\cref{is:p2:thm:exact-recovery} are not consequences
