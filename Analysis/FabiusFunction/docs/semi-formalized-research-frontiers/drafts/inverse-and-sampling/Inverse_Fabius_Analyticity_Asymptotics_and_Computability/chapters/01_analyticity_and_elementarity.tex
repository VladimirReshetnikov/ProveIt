...
\begin{proof}
Openness is \Cref{ne:lem:open}.  For density, induct on the derivation in
\Cref{ne:def:elem}.  Constants and the identity are analytic everywhere.
Finite intersections of dense open sets are dense, so sums, products, and
negation preserve the assertion.  For every unary constructor,
\Cref{ne:lem:generators} gives a finite set of exceptional input values and
\Cref{ne:lem:key} proves that composing with that constructor preserves a
dense analytic locus.  These are exactly the constructors of
\Cref{ne:def:elem}, completing the induction.  The expanded case-by-case
version appears in \Cref{ne:sec:density}.
\end{proof}
...
\begin{proof}[Proof of \Cref{ne:thm:density}]
Openness of $\An(f)$ is Lemma~\ref{ne:lem:open} and holds for every $f$.  Density is
proved by induction on the derivation of $f$ in Definition~\ref{ne:def:elem}.
...
\emph{The unary clauses.}  Apply Lemma~\ref{ne:lem:key} with the pair $(f,g)$ and the
finite bad set $B$ read off from Lemma~\ref{ne:lem:generators}: $B=\varnothing$ for
