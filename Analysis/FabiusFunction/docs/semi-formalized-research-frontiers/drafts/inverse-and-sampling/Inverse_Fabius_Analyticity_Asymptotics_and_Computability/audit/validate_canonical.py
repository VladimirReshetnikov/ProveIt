#!/usr/bin/env python3
"""Static publication and coverage gate for the canonical inverse-Fabius volume.
...
MASTER = PACKAGE / "inverse_fabius_theory.tex"
...
PROVED_ENVS = {"theorem", "proposition", "lemma", "corollary", "identity", "claim"}
...
def check_proof_coverage(path: Path, text: str, failures: list[str]) -> Counter[str]:
    ...
    for index, (name, _begin, end, line) in enumerate(blocks):
        ...
        if r"\begin{proof}" not in between and r"\begin{proofidea}" not in between:
            failures.append(
...
def check_concordance(
    ...
    if len(rows) != 194:
        failures.append(f"concordance has {len(rows)} rows, expected 194")
    ...
    for row in rows:
        ...
        if status == "conjecture" and label and ":conj:" not in f":{label}":
            failures.append(f"{key}: conjecture maps to non-conjecture label {label}")
        if status == "open problem" and label and ":prob:" not in f":{label}":
            failures.append(f"{key}: open problem maps to non-problem label {label}")
        if status in {"Lean-proved", "human-proved frontier result"} and (
            ":conj:" in f":{label}:" or ":prob:" in f":{label}:"
        ):
