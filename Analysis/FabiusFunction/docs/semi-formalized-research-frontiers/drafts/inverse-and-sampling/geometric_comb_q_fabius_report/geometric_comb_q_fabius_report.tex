...
\newcommand{\halfbinom}[2]{\Qbinom{#1}{#2}{1/2}}
\newcommand{\twobinom}[2]{\Qbinom{#1}{#2}{2}}
...
selected by the Fabius function.  Writing $a_j=\E X^j/j!$ for the Fabius
...
the Rvachev function $\Up(t)=F(1-|t|)$ on its boundary comb
$t_j=1-2^{-j}$.  Finally, exact polynomial reproduction by translates of
$\Up$ is composed with the geometric Lagrange operator, producing a finite
Gaussian--Appell decoder and a closed interpolation/synthesis loop.

Statements labeled theorem, proposition, lemma, or corollary carry
manuscript-level proofs in this report.  That label does not by itself assert
a Lean proof: the exact formal boundary is recorded in
\Cref{sec:lean-boundary,app:status-ledger}.  Unproved asymptotic claims are
explicitly labeled conjectures.  The bundled Python program uses exact
rational arithmetic wherever possible and logarithmic floating-point
arithmetic only for the enormous Lebesgue constants.
\end{quote}
...
\begin{align*}
 ...
 x^m&=\sum_{k=0}^m c^{m-k}\qbinom{m}{k}N_k(x),\\
 N_k(x)&=\sum_{m=0}^k(-1)^{k-m}c^{k-m}
 q^{\binom{k-m}{2}}\qbinom{k}{m}x^m.
\end{align*}
...
\[
 ...
 =\sum_{s=0}^r x^s c^{r-s}\qbinom{n+r-s}{n}.
\]
...
synthesis of polynomials by translates of $\Up$.  Results imported from those
...
\path{ffcdbdac47246bd553f272bd1ceac441f8dd4f0c}.
...
gap-growth bound, or Gaussian--Appell cardinal/biorthogonality packaging.
Some of these follow on paper from formalized finite ingredients, but that is
not the same as a checked Lean theorem.  The conjectures and research
problems have no proof status in either layer.

\section{The affine geometric comb}

\begin{definition}[Affine geometric comb]
Let $\alpha,\beta,q\in\C$ with $\beta\ne0$.  The degree-$n$ affine geometric
comb is
\[
...
boundary normalization used for $\Up$ is $\alpha=1$, $\beta=-1$.
\end{definition}
...
\begin{align}
 ...
 \qbinom{n}{k}&:=\frac{(q;q)_n}{(q;q)_k(q;q)_{n-k}}
 &&(0\le k\le n).
\end{align}
...
\begin{equation}
 \Qbinom{n}{k}{q^{-1}}=q^{-k(n-k)}\qbinom{n}{k}.
...
\begin{equation}
 h_r(c,cq,\ldots,cq^m)=c^r\qbinom{m+r}{m}.
...
\begin{align}
 ...
 &=\frac{x^{-k}}{(1-q)^k}
   \sum_{j=0}^k(-1)^j
   q^{-jk+\binom{j+1}{2}}\qbinom{k}{j}f(q^jx).
...
\begin{align}
 x^m
 &=\sum_{k=0}^m c^{m-k}\qbinom{m}{k}N_k(x;c,q),
...
\begin{align}
 \gamma_k
 &=\sum_{m=k}^M a_mc^{m-k}\qbinom{m}{k},
 \label{eq:coeff-forward}\\
 a_m
 &=\sum_{k=m}^M\gamma_k(-1)^{k-m}c^{k-m}
 q^{\binom{k-m}{2}}\qbinom{k}{m}.
...
\begin{equation}
 H_m(z;q):=\sum_{k=0}^m\qbinom{m}{k}z^k.
...
\begin{align*}
 T_{k,m}&=\begin{cases}c^{m-k}\qbinom{m}{k},&k\le m,\\0,&k>m,\end{cases}\\
 L_{j,k}&=\begin{cases}
...
\begin{equation}
 \qbinom{n}{k}=b^{-k(n-k)}\Qbinom{n}{k}{b}.
 \label{eq:reciprocal-integer-base}
\end{equation}
For prime-power $b$, $\Qbinom{n}{k}{b}$ counts $k$-dimensional subspaces of
...
\begin{equation}
 ...
 \sum_{r\ge0}a_{n+1+r}c^r\qbinom{n+r}{n}z^{n+1+r}.
...
$c^r\qbinom{n+r}{n}$ for every higher moment.
\end{proof}
...
\begin{equation}
 \boxed{
 h_r(x,c,cq,\ldots,cq^n)
 =\sum_{s=0}^rx^sc^{r-s}\qbinom{n+r-s}{n}.}
...
\begin{align*}
 ...
 &=\left(\sum_{s\ge0}x^st^s\right)
   \left(\sum_{u\ge0}c^u\qbinom{n+u}{n}t^u\right).
\end{align*}
...
\begin{align*}
 ...
 x^{n+3}-\Interp_nx^{n+3}
 &=N_{n+1}(x)
 \left(x^2+c[n+1]_qx+c^2\qbinom{n+2}{n}\right).
\end{align*}
...
\begin{equation}
 ...
 =\sum_{m\ge k}a_mc^{m-k}\qbinom{m}{k}.
...
\begin{equation}
 \boxed{
 \sup_{|x|\le r}\abs{f(x)-\Interp_nf(x)}
 \le
...
\[
 |\gamma_k|
 \le\sum_{m\ge k}M_RR^{-m}c^{m-k}\qbinom{m}{k}.
\]
For $0<q<1$,
\[
 \qbinom{m}{k}
...
\begin{align*}
 ...
 &=\frac1{(q;q)_n}\sum_{r=0}^n
   q^{\binom{r+1}{2}}\qbinom{n}{r}.
\end{align*}
The finite $q$-binomial theorem with parameter $q$ gives
\[
 \sum_{r=0}^nq^{\binom r2}\qbinom{n}{r}q^r=(-q;q)_n.
\]
...
\end{equation}
The first two factors tend uniformly to $\alpha\e^{-\alpha}$.  For the
...
\[
 \qbinom{n}{r}\le\frac1{(q;q)_r}\le\frac1{(q;q)_\infty}.
\]
...
$1-1/n$ tends to the positive constant $2(-q;q)_\infty/\e$.  Thus
$x_n^*\to1$.
...
\[
 ...
 =2(-q;q)_\infty\alpha\e^{-\alpha}
 \le\frac{2(-q;q)_\infty}{\e}.
\]
The lower bound at $\alpha=1$ forces equality.  Since
$\alpha\e^{-\alpha}$ has a unique maximum at $\alpha=1$, every subsequential
...
\[
 \Qbinom{k}{j}{q^{-1}}
 =q^{-j(k-j)}\qbinom{k}{j}=q^{j(j-k)}\qbinom{k}{j}.
\]
...
\end{equation}
Then $J_n(1-2^{-j})=\Up(1-2^{-j})$ for $0\le j\le n$, and for every fixed
$m$,
\begin{equation}
 J_n^{(m)}(1)\longrightarrow\Up^{(m)}(1)=0.
...
$\Up$.  Because the geometric cardinal coefficients are Gaussian and the
...
\begin{equation}
 \MGF_u(z)=\int_{-1}^{1}\e^{zx}u(x)\,\dd x
 =\prod_{r=1}^{\infty}
...
\begin{equation}
 \boxed{
 p(x)=h\sum_{k\in\Z}(\App p)(kh)u(x-kh),
 \qquad x\in\R,}
...
\[
 (S_hv)(x)=h\sum_{k\in\Z}v(kh)u(x-kh).
\]
...
coincide with those of $\e^{zx}\MGF_u(z)$.  Equating the coefficient of
$z^r/r!$ gives
\begin{equation}
...
$(-q;q)_\infty/(q;q)_\infty$ and $2(-q;q)_\infty/\e$ are computed with
...
\begin{align}
 x^m&=\sum_{k=0}^{m}c^{m-k}\qbinom{m}{k}N_k(x;c,q),
 &&0\le m\le9,
...
\begin{align}
 ...
 \frac{2(-q;q)_\infty}{\e}
 &=1.75421915716734780836138830414\ldots.
 \label{eq:numeric-global-prefactor}
\end{align}
Selected computed values are shown in \Cref{tab:lebesgue-numerics}.  The
...
in \eqref{eq:half-global-asymptotic}.}
...
$1$ is the maximizer of $\alpha\e^{-\alpha}$ in
...
\item direct approximation in scaled translates of $\Up$ rather than first
...
\item \textbf{Boundary profile.}  Isolate the elementary limits
      $(1-\alpha/n)^n\to\e^{-\alpha}$, dominated convergence for the Euler
      cloud, and the tail bound from $q^{\binom r2}$.
\item \textbf{Fabius arithmetic.}  Reuse the existing exact dyadic-value and
      moment development to prove \Cref{thm:fabius-gaussian-transform}, the
      residue generating function, and the fixed-jet bound.
\item \textbf{Atomic synthesis.}  Reuse the repository's already formalized
      Rvachev Appell deconvolution and finite polynomial-synthesis theorem;
      specialize it to \eqref{eq:cardinal-up-decoder}.  Matrix
...
Monomial to Newton
& $x^m=\sum_{k=0}^{m}c^{m-k}\qbinom{m}{k}N_k(x)$\\[4pt]
Newton to monomial
& $N_k(x)=\sum_{m=0}^{k}(-1)^{k-m}c^{k-m}
 q^{\binom{k-m}{2}}\qbinom{k}{m}x^m$\\[6pt]
Barycentric weight
...
& $\Leb_n^*\sim2(-q;q)_\infty q^{-\binom n2}/(\e n)$\\[4pt]
Monomial residual
& $x^{n+1+r}-\Interp_nx^{n+1+r}
 =N_{n+1}(x)h_r(x,c,cq,\ldots,cq^n)$\\[6pt]
Geometric specialization
& $h_r(c,cq,\ldots,cq^n)=c^r\qbinom{n+r}{n}$\\[4pt]
Jackson moment
& $\int_0^cN_k(x)\,\dd_qx=(-1)^kc^{k+1}
...
For $q=1/2$, $d_n=\E X^n$, and $a_n=d_n/n!$,
\begin{longtable}{@{}p{0.28\textwidth}p{0.64\textwidth}@{}}
...
The normalized package contains the research payload plus intake records.
No source ZIP attributable to this package was present in the audited
workspace, so \path{ARRIVAL_SHA256SUMS.txt} preserves the verified 16-file
arrival ledger rather than asserting an unobserved archive hash.
...
\path{ARRIVAL_SHA256SUMS.txt}
& Verbatim, verified checksum ledger for the arrival payload.\\
\path{INTAKE_AUDIT.md}, \path{NUMERICAL_REPLAY.txt}
& Provenance, mathematical/Lean-status audit, and numerical replay record.\\
\path{pdf_preflight.json}
& Machine-readable PDF geometry, font, text, build, and visual checks.\\
\path{SHA256SUMS.txt}
& Exhaustive final-package ledger; by convention it excludes itself.\\
\bottomrule
\end{longtable}
...
\end{lstlisting}
The script requires Python 3.10 or later and the exact package versions in
\path{requirements.txt}.  Rebuild the PDF from a clean auxiliary state with
...
pdflatex -halt-on-error -file-line-error -interaction=nonstopmode \
  geometric_comb_q_fabius_report.tex
\end{lstlisting}
The checked archive records final-pass diagnostics in
\path{pdf_preflight.json}; build auxiliaries are not package contents.

\section{Source boundary}

The primary source monograph and neighboring reports were read to retain
notation and avoid duplicating already consolidated results.  The report was
written against
\begin{equation}
...
\end{equation}
Its formal-status boundary was rechecked at intake head
\path{ffcdbdac47246bd553f272bd1ceac441f8dd4f0c}.  The relevant Lean files
named in \Cref{sec:lean-boundary} have no diff between those two commits.
...
decoder are manuscript deductions beyond the explicitly identified source
baseline.  No wider priority claim is made.
