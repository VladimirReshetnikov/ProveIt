...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Interpolation on a Geometric Comb}
\fancyhead[R]{\thepage}
...
All statements advertised as theorems are proved.  Conjectures and open
problems are separated explicitly.  A commented Python program reproduces the
numerical checks and figures and is included with the source and rendered PDF.
\end{abstract}
...
\[
  N_{n,k}=(-1)^{n-k}q^{\binom{n-k+1}{2}}\qbinom nk;
\]
...
Throughout the main body, $0<q<1$, $A>0$, and $n\in\N_0$.  Most algebraic
...
and
\[
 \qbinom mk:=\frac{(q;q)_m}{(q;q)_k(q;q)_{m-k}}
 \quad(0\le k\le m).
\]
We also write
\[
...
\begin{align}
 (z;q)_m
 &=\sum_{k=0}^m(-1)^kq^{\binom{k}{2}}\qbinom mk z^k,
 \label{eq:finite-q-binomial}\\
 \frac1{(z;q)_{m+1}}
 &=\sum_{r=0}^{\infty}\qbinom{m+r}{m}z^r,
 \qquad |z|<1.
...
\begin{equation}
 h_r(A,Aq,\ldots,Aq^m)=A^r\qbinom{m+r}{m}.
...
\begin{align}
 ...
 &=x^{n+1+r}-\omega_{n,A,q}(x)
   \sum_{j=0}^r x^jA^{r-j}\qbinom{n+r-j}{n}.
...
\begin{equation}
 (\Gc_{n,q}f)(A)-f(0)
 =(-1)^nq^{\binom{n+1}{2}}
  \sum_{r=0}^{\infty}\qbinom{n+r}{n}a_{n+1+r}A^{n+1+r}.
...
\[
 \sum_{r\ge0}\qbinom{n+r}{n}t^r=\frac1{(t;q)_{n+1}}.
\]
\end{proof}
...
\[
 h_r(A,Aq,\ldots,Aq^k)=A^r\qbinom{k+r}{k}.
\]
On the other hand,
\[
...
\begin{align}
 ...
 x^m
 &=\sum_{k=0}^mA^{m-k}\qbinom{m}{k}\phi_k(x).
...
\begin{equation}
 ...
 =\sum_{r=0}^{\infty}a_{k+r}A^r\qbinom{k+r}{k}.
...
Let $\alpha_1,\ldots,\alpha_J\in\C$ have pairwise distinct spectral
points $q^{\alpha_j}$, none equal to $1$, and let $m_1,\ldots,m_J\in\N$, and put $N=\sum_jm_j$.  Define
\begin{equation}
 W_{\bm\alpha,\bm m}(z)
 :=\prod_{j=1}^J
...
For any $\alpha\in\C$ and $s\in\N_0$,
\begin{equation}
 \Rc_{\bm\alpha,\bm m}
 [x^\alpha(\log x)^s]
...
A function $f$ is regularly varying at $0+$ with index $\alpha\in\R$ if
...
\[
 f[0,A,Aq,\ldots,Aq^n]
 =\frac1{(n+1)!}
 \E f^{(n+1)}\!\left(A\sum_{k=0}^nq^kT_k\right).
\]
...
Then
\[
 (\Gc_{n,q}f)(A)-f(0)=\int_0^Af^{(n+1)}(t)K_{n,A,q}(t)\dd t.
\]
Normalize it by
\begin{equation}
 B_{n,A,q}(t):=
...
\begin{align}
 \E Z_q^r&=\frac{r!}{(q;q)_r},
 \label{eq:Z-moments}\\
 \E e^{-sZ_q}&=\frac1{(-s;q)_\infty},\qquad s\ge0.
 \label{eq:Z-Laplace}
\end{align}
...
\[
 \E e^{-sZ_q}=\prod_{k\ge0}(1+sq^k)^{-1}=(-s;q)_\infty^{-1}.
\]
...
For the density, decompose the Laplace transform into simple fractions at its
poles $s=-q^{-k}$:
\[
 \frac1{(-s;q)_\infty}
 =\sum_{k\ge0}
...
\]
Termwise inversion gives \eqref{eq:Z-density}; for each $y>0$ the Gaussian
factor in $k$ and the term $e^{-q^{-k}y}$ ensure absolute convergence.
\end{proof}
...
then
\begin{equation}
 F_b(x)=\int_0^1F_b\!\left(\frac{x-(1-b)u}{b}\right)\dd u,
...
convention
\[
 \widehat u_b(z)=\E e^{-2\pi izZ_b},
\]
its characteristic function is
\begin{equation}
 \widehat u_b(z)
 =\prod_{j=0}^{\infty}
 \sinc\!\bigl(2\pi(1-b)b^jz\bigr),
 \qquad \sinc w:=\frac{\sin w}{w}.
 \label{eq:generic-sinc-product}
\end{equation}
At $b=1/2$, this is
\begin{equation}
 \widehat\Up(z)=\prod_{j=0}^{\infty}\sinc(\pi z/2^j),
 \label{eq:up-sinc}
\end{equation}
...
\begin{equation}
 ...
 \label{eq:Up-F}
\end{equation}
with $\Up(x)=0$ for $|x|\ge1$.  These normalizations agree with the primary
...
\begin{equation}
 ...
 =\sum_{k\ge0}2^{-k}E_k,
 \qquad
 \E e^{-sZ_{1/2}}=\frac1{(-s;1/2)_\infty}.
 \label{eq:Sn-limit}
\end{equation}
...
$\Phi(z)=\E e^{-2\pi izX}$ with a symmetric random variable $X$,
\begin{equation}
...
Let $\Phi$ be even entire, fix $z\in\C$, and set $H_z$ by
\eqref{eq:Hzt}.  Then for every $t\in\C$,
\begin{equation}
 \Phi(z\sqrt t)
 =\sum_{j=0}^{\infty}
...
\]
The theorem gives $(-1)^nR_n>0$ and $|R_n|/B_n=\E F(S_n)\in(0,1)$.  Exact
...
$|R_n|/B_n=\E F(S_n)$ through order $36$.  The decrease is much slower than
...
The most suitable theorem sequence for Lean or another proof assistant is:
\begin{enumerate}[leftmargin=2em]
\item finite $q$-product identities and the geometric denominator
\eqref{eq:bary-denominator};
\item row polynomial factorization and moment residuals;
\item Jackson divided differences on monomials, extended by polynomial
linearity;
\item operator recursion and total variation;
\item divided-difference remainder plus a library Hermite--Genocchi theorem;
\item Dirichlet moment calculation and the scaling limit via exponential
variables;
\item Fabius endpoint derivative induction and boundary sign identity.
\end{enumerate}
The analytic infinite Newton theorem requires more infrastructure, but the
finite algebraic and probabilistic core is comparatively modular.
...
\begin{align}
 ...
 \sum_{r=0}^{\infty}
 \frac{A^r\qbinom{n+r}{n}}{(n+r+1)!}t^r.
