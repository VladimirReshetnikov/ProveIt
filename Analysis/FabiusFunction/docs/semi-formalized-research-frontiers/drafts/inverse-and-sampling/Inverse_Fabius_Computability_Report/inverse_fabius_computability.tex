...
\commit.  At that snapshot, the main computability theorem, the least
...
A post-publication Lean development now formalizes two layers.  The structural
inverse-modulus layer in
\path{Analysis/FabiusFunction/Lean/FabiusFunction/InverseModulus.lean} proves
the global shape of fixed-length increments, the endpoint lower bound,
inverse-gap inequalities, attained exact moduli, and the exact
...
\texttt{EffectivelyUniformContinuous}.  Exhaustive declaration-level
crosswalks are given in \cref{sec:lean-status}.
...
route.  The logarithmic choice $r(n)$, the exact ceiling denominator $d_*$,
tolerant bisection, sequential computability, the combined computable-real-
function theorem, and the input-precision asymptotics also remain outside the
current formalization.  Those claims retain the complete human proofs and
imported-source qualifications stated below.  ``New'' means new relative to
the pinned corpus; no unqualified worldwide priority claim is made.
Conjectures and research problems are explicitly labelled and are not used in
the proof.
\end{statusbox}
...
proves the first inequality and its exact transfer through the clamped inverse,
including sharpness and attainment.
...
simple primitive-recursive reciprocal modulus.  The report's probabilistic
event proof, its smaller logarithmic-$r(n)$ packaging, and the
...
by a different and stronger route.  The exact inverse-dyadic recurrence gives
the one-term estimate
...
that probabilistic proof remains report-level.
...
$[(1-h)/2,1-h]$.  Here ``increasing'' and ``decreasing'' are understood in
the non-strict sense.  Its two endpoint values are equal to $F(h)$.
\end{theorem}
...
\cref{prop:density-shape}, $p(x+h)\ge p(x)$, and hence
\begin{equation}\label{eq:M-derivative}
 M_h'(x)=p(x+h)-p(x)\ge0.
\end{equation}
The function $M_h$ is differentiable, so the mean-value theorem makes it
nondecreasing on the first half of its domain.  The reflection identity then
makes it nonincreasing on the second half.  Finally,
\[
 M_h(0)=F(h)-F(0)=F(h),
\]
and
\[
 M_h(1-h)=F(1)-F(1-h)=1-\bigl(1-F(h)\bigr)=F(h).
\]
Consequently every admissible increment is at least $F(h)$ and both endpoint
increments attain that value.
\end{proof}
...
\texttt{EffectivelyUniformContinuous} for the totalized inverse directly
from this primitive-recursive witness and $n<2^n$.  Thus effective uniform
continuity itself is formalized.  The smaller logarithmic choice $r(n)$ in
\cref{thm:reciprocal-modulus} is still only packaged in the report.
\end{remark}
...
effective-continuity theorem deliberately avoids the ceiling by using the
explicit factorial denominator at order $n$.
...
the heavier random-series and computable-analysis interfaces.  Its complete
public theorem surface was compiler-checked as one focused target; each theorem
uses only the standard logical principles already present in its dependencies.
The declaration-level correspondence is as follows.
...
\path{Fabius.antitoneOn_fabiusIntervalMass_secondHalf}.  These formal
half-line statements strengthen the interval-restricted shape in
\cref{thm:least-mass}.\\
\addlinespace
...
The formal least-mass API is intentionally expressed as an inequality plus
explicit endpoint attainment rather than through a separate minimum operator.
The \texttt{IsGreatest} declarations record both the sharp upper bound and an
...
\end{center}
It should import
\path{FabiusFunction.FabiusInverseEffectiveContinuity}.  It no longer needs to
...
complete in \path{FabiusInverseEffectiveContinuity.lean}.  The remaining order
is:
...
\item Formalize the logarithmic scale $r(n)$ and the smaller report modulus
      from \cref{thm:reciprocal-modulus}; this is an efficiency refinement, not
      a prerequisite for effective uniform continuity.
...
stronger than the report's box bound.  Tolerant bisection, sequential
computability, and the final computable-real-function package remain to be
formalized; the logarithmic-$r(n)$ and exact-ceiling moduli remain optional
sharpenings of the already certified effective-continuity witness.
...
proof in \cref{thm:least-mass}; reflection, global non-strict shape, and the
lower bound are formalized in \path{InverseModulus.lean}.  Exact names appear
in \cref{sec:lean-status}.\\
\addlinespace
...
simple primitive-recursive factorial denominator at $r=n$; the report's
smaller logarithmic-$r(n)$ packaging remains human-only.\\
\addlinespace
...
sequential-computability declaration is supplied by
...
Fabius increments, their global unimodal shape, the least endpoint increment,
constrained forward superadditivity, local and global inverse-gap inequalities,
global inverse subadditivity, absolute and metric pointwise moduli, attained
exact unit-interval and totalized suprema, and the exact effective-injectivity
threshold.  Exact declaration names appear in \cref{sec:lean-status}.\\
\addlinespace
