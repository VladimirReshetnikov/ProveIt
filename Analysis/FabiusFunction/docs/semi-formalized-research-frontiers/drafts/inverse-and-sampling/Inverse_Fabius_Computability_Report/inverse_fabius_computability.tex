...
\[
  \sup_{\lvert u-v\rvert\le\eta}\lvert G(u)-G(v)\rvert
  =G\bigl(\min(\eta,1)\bigr).
\]
...
the completed post-publication Lean crosswalk for the structural inverse
modulus and the remaining roadmap for the computability layer.  An
...
A post-publication Lean development now formalizes the structural
inverse-modulus layer in
\path{Analysis/FabiusFunction/Lean/FabiusFunction/InverseModulus.lean}.  It
proves the global unimodal shape of fixed-length Fabius increments, the
endpoint lower bound, constrained forward superadditivity, local and global
inverse-gap inequalities, global inverse subadditivity, attained exact moduli
on both $[0,1]$ and $\R$, and the exact effective-injectivity threshold.  The
declaration-level crosswalk is given in \cref{sec:lean-status}.

This update does \emph{not} by itself formalize the closed bound $\Delta_r$,
recursive-modulus packaging, tolerant bisection, sequential computability, the
combined computable-real-function theorem, or the input-precision asymptotics.
Those claims retain the complete human proofs and imported-source
qualifications stated below.  ``New'' means new relative to the pinned corpus;
no unqualified worldwide priority claim is made.  Conjectures and research
problems are explicitly labelled and are not used in the proof.
\end{statusbox}
...
including sharpness and attainment.  The second inequality, its recursive
packaging, and the tolerant-bisection realizer remain on the human-proof side
of the boundary recorded in \cref{sec:lean-status}.
...
\end{center}
It should import \path{FabiusFunction.InverseModulus}.  It no longer needs to
reprove interval shape or inverse-gap estimates.  Its remaining public surface
is the effective layer; the following Lean is schematic rather than a claim
about declarations already present.
...
\path{InverseModulus.lean}.  The remaining order is:

\begin{enumerate}
\item Connect the exact dyadic evaluator to the positive rational
      $F(2^{-r})$ and package the reciprocal-ceiling modulus, with the
      qualified minimality statement of
      \cref{prop:exact-dyadic-modulus}.
\item Formalize the elementary box-event bound $\Delta_r\le F(2^{-r})$ as a
      portable closed alternative.
...
\item Package sequential computability and effective uniform continuity into
      the repository's computable-real-function interface.
...
Proved in \cref{thm:reciprocal-modulus}.  Its structural pointwise estimate is
formalized in \path{InverseModulus.lean}; recursive-modulus packaging is not.\\
\addlinespace
