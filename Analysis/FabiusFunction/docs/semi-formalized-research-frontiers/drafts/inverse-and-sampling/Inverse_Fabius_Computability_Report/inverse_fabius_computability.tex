...
\begin{statusbox}{Paper theorem status versus Lean status}
The theorem, proposition, lemma, and corollary labels below denote
paper-level claims.  The least-interval-mass identity, exact inverse modulus,
elementary endpoint-mass bound, tolerant-bisection construction,
inverse-computability packaging, and optimal input-bit law are proved in this
report, but none currently has a matching public Lean declaration.  The live
formal corpus already proves the forward spline certificate and forward
computability, strict density shape, the clamped inverse and its inverse
identities, exact dyadic evaluation, and the leading inverse endpoint
equivalent.  The explicit periodic inverse correction and explicit
all-orders inverse reversion cited later remain frontier-document results,
not Lean theorems.  Proposed Lean names are unqualified and schematic.
Conjectures and research problems are labelled and are not used in the proof;
no unqualified worldwide priority claim is made.
\end{statusbox}
...
module headers, while a separate inventory found 147 prior \texttt{.tex}
...
No public theorem named or formulated as sequential computability, effective
uniform continuity, or computable-real-function status for \lean{fabiusInv}
occurs in the live formal corpus.  The missing bridge is not a formal
triviality.  A
...
the roadmap in \cref{sec:lean-roadmap}.
...
\toprule
Claim & Input & Paper/Lean status after intake audit \\
\midrule
\endhead
...
\addlinespace
Least interval mass $F(h)$ & Symmetric unimodality of $p$ & Paper-level proof
in \cref{thm:least-mass}; no direct public Lean theorem.\\
\addlinespace
Exact inverse modulus $G(\eta)$ & Least interval mass and inverse identities &
Paper-level proof in \cref{thm:exact-inverse-modulus}; no direct public Lean
theorem.\\
\addlinespace
Closed modulus $\Delta_r$ & Explicit positive box event in the random series &
Paper-level proof in \cref{thm:closed-dyadic-modulus}; no direct public Lean
theorem.\\
\addlinespace
Effective uniform continuity & Closed modulus and recursive integer arithmetic &
Paper-level proof in \cref{thm:reciprocal-modulus}; no inverse-side public
Lean theorem.\\
\addlinespace
Sequential computability & Forward rational oracle, closed inverse modulus,
and tolerant bisection & Paper-level proof in
\cref{thm:bisection-correct,cor:GT-sequential}; no inverse-side public Lean
theorem.\\
\addlinespace
