...
\definecolor{warmshade}{RGB}{250,247,240}
...
\small Repository snapshot: \commit}
...
increasing distribution function on $[0,1]$, and let $G=F^{-1}$.  We prove
constructively that $G$ is a computable real function in the
...
subintervals of $[0,1]$ of a fixed length $h$, an endpoint interval has the
least Fabius mass:
\[
...
\[
  \sup_{\lvert u-v\rvert\le\eta}\lvert G(u)-G(v)\rvert=G(\eta).
\]
...
name-transformer and proves sequential computability.

The report audits the recursive LaTeX corpus under \repo{}, separates imported
...
Legendre expansions, and Bell/Bernoulli coefficient calculi, and gives a Lean
formalization roadmap.  An exact-rational Python supplement reproduces the
finite-spline and modulus calculations.
\end{abstract}

\begin{statusbox}{Status and novelty discipline}
Every statement labelled theorem, proposition, lemma, or corollary below is
proved in this report or explicitly imported with a source pointer.  The main
computability theorem, the exact interval-mass identity, the exact inverse
modulus, and the closed elementary modulus are absent from the audited
repository snapshot under these formulations.  ``New'' means new relative to
that pinned corpus; no unqualified worldwide priority claim is made.
Conjectures and research problems are labelled and are not used in the proof.
\end{statusbox}
...
\subsection{Pinned repository snapshot}

The recursive audit was pinned to commit \commit\ \cite{ProveIt}.  The GitHub code index at
its parent snapshot enumerated 94 files with extension \texttt{.tex} below
\repo{}; the final merge added two further LaTeX sources, for a total of 96 in
the pinned tree.  All source paths were inventoried.  Primary and consolidated
...
The following inputs are established in the pinned development and are not
claimed as new here.
...
proved computable real functions.  The relevant Lean modules are
\path{FabiusComputability.lean} and \path{FabiusComputableSpline.lean}; they
expose the sequential-computability and effective-uniform-continuity clauses,
including a primitive-recursive spline evaluator.
\item The module \texttt{FabiusInverse.lean} constructs the clamped totalized
inverse $\texttt{fabiusInv}$, proves both inverse identities, continuity,
monotonicity, interior $C^\infty$ regularity, derivative and curvature laws,
and endpoint steepness.
\item The inverse/sampling reports derive exact finite-prefix quantile germs,
$q$-binomial Richardson filters, and all-orders endpoint asymptotics for
$G$, including the nonconstant log-periodic Gamma--zeta fluctuation.
\end{enumerate}

\subsection{The gap closed here}

No theorem named or formulated as sequential computability, effective uniform
continuity, or computable-real-function status for \texttt{fabiusInv} occurs
in the pinned snapshot.  The missing bridge is not a formal triviality.  A
...
\end{align}
The first is exact and geometric; the second is elementary and recursive.
...
For $0\le h\le1$, define the mass of the interval $[x,x+h]$ by
\begin{equation}\label{eq:interval-mass}
 M_h(x):=F(x+h)-F(x),
 \qquad 0\le x\le1-h.
\end{equation}

\begin{theorem}[Endpoint intervals have least Fabius mass]\label{thm:least-mass}
For every $0\le h\le1$,
\begin{equation}\label{eq:least-mass}
 \min_{0\le x\le1-h}\bigl(F(x+h)-F(x)\bigr)=F(h).
\end{equation}
More precisely, when $0<h<1$, $M_h$ is increasing on
$[0,(1-h)/2]$, symmetric about $(1-h)/2$, and decreasing on
$[(1-h)/2,1-h]$.  Its two endpoint values are equal to $F(h)$.
\end{theorem}

\begin{proof}
The cases $h=0$ and $h=1$ are immediate.  Suppose $0<h<1$.  Reflection gives
\begin{align*}
 M_h(1-h-x)
 &=F(1-x)-F(1-h-x)\\
 &=(1-F(x))-(1-F(x+h))=M_h(x),
...
For $0\le x\le(1-h)/2$, compute
\begin{equation}\label{eq:distance-comparison}
 \left(x+h-\frac12\right)^2-
 \left(x-\frac12\right)^2
 =h(2x+h-1)\le0.
\end{equation}
Thus $x+h$ is at least as close to $1/2$ as $x$ is.  By the unimodality in
\cref{prop:density-shape}, $p(x+h)\ge p(x)$, and therefore
\begin{equation}\label{eq:M-derivative}
 M_h'(x)=p(x+h)-p(x)\ge0.
\end{equation}
So $M_h$ increases on the first half of its domain; symmetry makes it decrease
on the second.  Its minima are the endpoints, and
\[
 M_h(0)=F(h)-F(0)=F(h),
\]
while symmetry gives the same value at $x=1-h$.
\end{proof}

\begin{remark}[A general principle]\label{rem:general-principle}
The proof used only that a probability density on $[0,1]$ is symmetric and
unimodal.  Hence \cref{thm:least-mass} holds for every continuous symmetric
unimodal distribution, with strictness adjusted when the density has plateaus.
The Fabius system is special not because the geometric lemma is exotic, but
because $F(h)$ has unusually rich exact arithmetic and endpoint asymptotics.
\end{remark}
...
\begin{proof}
Let $0\le u<v\le1$ and put
\[
 a=G(u),\qquad b=G(v),\qquad h=b-a.
\]
By the inverse identities and \cref{thm:least-mass},
\begin{equation}\label{eq:mass-gap-proof}
 v-u=F(b)-F(a)\ge F(h).
\end{equation}
Applying the increasing inverse $G$ gives
$h\le G(v-u)$, proving \eqref{eq:inverse-gap-bound}.  If $v-u\le\eta$, then
$h\le G(\eta)$, so $\omega_G(\eta)\le G(\eta)$.  Equality is attained by
$u=0$, $v=\eta$ because $G(0)=0$.

For the totalization, clamping is $1$-Lipschitz:
\[
 \abs{\Clamp(u)-\Clamp(v)}\le\min(\abs{u-v},1).
\]
Applying the first part yields the upper bound in
\eqref{eq:exact-total-modulus}; pairs inside $[0,1]$ attain it.
\end{proof}

\begin{corollary}[Effective-injectivity form]\label{cor:effective-injectivity}
For $0<h\le1$ and $u,v\in\R$,
\begin{equation}\label{eq:effective-injectivity}
 \abs{u-v}<F(h)
 \quad\Longrightarrow\quad
 \abs{\InvT(u)-\InvT(v)}<h.
\end{equation}
The same implication holds for $G$ on $[0,1]$.
\end{corollary}

\begin{proof}
For $0\le\abs{u-v}<F(h)$, \cref{thm:exact-inverse-modulus} gives
\[
 \abs{\InvT(u)-\InvT(v)}
...
The threshold $F(h)$ is exact.  At input separation $F(h)$, the pair
$u=0$, $v=F(h)$ has output separation exactly $h$.  Thus any effective
modulus can improve the lower bound used for $F(h)$, but it cannot replace
$F(h)$ by a larger universal threshold.
\end{remark}
...
The corpus proves that every $F(2^{-r})$ is an exactly computable positive
rational.  Therefore one can replace the elementary lower bound $\Delta_r$ by
the exact endpoint mass.
...
\begin{equation}\label{eq:d-star}
 ...
 \quad d_*(0):=1.
\end{equation}
Then $d_*$ is recursive and is an effective uniform-continuity modulus for
$\InvT$.  Among moduli constrained to use the spatial scale $2^{-r(n)}$, it
is the smallest integer threshold allowed by the exact inverse-modulus law.
\end{proposition}

\begin{proof}
The exact dyadic evaluator computes the rational $F(2^{-r})$, and the latter
is positive by \cref{lem:box-event}; rational reciprocal and ceiling are
computable integer operations.  Since $1/d_*(n)\le F(2^{-r(n)})$, strict input
inequality and \cref{cor:effective-injectivity} yield output separation
strictly below $2^{-r(n)}<1/n$.  Minimality follows because any smaller
integer $d$ has $1/d>F(2^{-r(n)})$; then the endpoint pair can be chosen with
input separation between these two numbers and output separation at least
$2^{-r(n)}$.
\end{proof}
...
choice $\alpha_p=F(2^{-p})$, while \cref{lem:box-event} supplies a closed
lower substitute requiring no invocation of the exact dyadic evaluator.
...
The inverse endpoint reports in the corpus give much more than computability\ \cite{ProveIt}.
Let
...
\end{equation}
Suppressing the explicitly known first log-periodic correction, their leading
equivalent is
...
\begin{corollary}[Sharp asymptotic modulus]\label{cor:sharp-asymptotic-modulus}
The expressions \eqref{eq:G-leading} and
\eqref{eq:G-periodic-correction} are, without any further inversion, the
leading and first-correction asymptotics of the exact continuity modulus of
$G$ and $\InvT$:
\begin{equation}\label{eq:asymptotic-modulus}
 \omega_G(\eta)=\omega_{\InvT}(\eta)=G(\eta)
 \qquad(0\le\eta\le1).
\end{equation}
Every proved all-orders endpoint expansion for the inverse therefore transfers
verbatim to an all-orders modulus-of-continuity expansion.
\end{corollary}
...
\]
with minimizers $x=0$ and $x=7/8$.  This finite-convolution experiment mirrors
\cref{thm:least-mass}; the theorem itself is the continuous unimodality
argument, not a grid inference.
...
$F(x)$ as $x\downarrow0$.  The repository then uses Bell-polynomial reversion to transport
all-orders forward expansions to $G(y)$ \cite{ProveIt}; Bernoulli numbers and
polynomials enter through logarithms of the Laplace/sinc products and through
formal composition.

The exact identity $\omega_G(\eta)=G(\eta)$ gives these expansions a second
interpretation: they are not only quantile asymptotics but optimal continuity-
modulus asymptotics.  In particular, the nonconstant periodic Gamma--zeta term
cannot be discarded from a genuinely sharp effective modulus.
...
The least-mass theorem used only symmetric unimodality, while several corpus
reports investigate stronger total-positivity and variation-diminishing
...
A natural new file is
\begin{center}
\path{Analysis/FabiusFunction/Lean/FabiusFunction/FabiusInverseComputability.lean}.
\end{center}
It would import the existing inverse, shape, exact-dyadic, and forward
computability interfaces, but should avoid depending on endpoint asymptotics.
The core public declarations could have the following mathematical content;
the displayed Lean is schematic, not claimed to compile verbatim against a
future moving version of Mathlib.

\begin{lstlisting}[style=pseudo,language={}]
/-- Every interval of length h has at least the endpoint mass F h. -/
theorem fabiusReal_add_sub_ge_fabiusReal
    {x h : Real} (hx : x in Icc 0 1) (hh : 0 <= h)
    (hxh : x + h <= 1) :
    fabiusReal F (x + h) - fabiusReal F x >= fabiusReal F h

/-- Exact modulus of the inverse on [0,1]. -/
theorem sup_inverse_increment_eq
    {eta : Real} (heta : eta in Icc 0 1) :
    sSup {z | exists u v in Icc (0 : Real) 1,
      abs (u - v) <= eta /\
      z = abs (fabiusInv F hF u - fabiusInv F hF v)}
      = fabiusInv F hF eta
...
The exact statement of the supremum theorem may be heavier than is useful in
Lean.  For all subsequent proofs, the following pointwise form is sufficient
and easier to reuse:
\begin{equation}
 \abs{u-v}<F(h)\quad\Longrightarrow\quad
 \abs{\InvT(u)-\InvT(v)}<h.
 \label{eq:lean-pointwise-modulus}
\end{equation}
The exact supremum identity can then be derived as a mathematical corollary.

\subsection{A low-risk formalization order}

The proof dependencies suggest the following sequence.

\begin{enumerate}
\item Prove the interval-mass inequality \eqref{eq:gap-interval-preview} from
      the existing symmetry and monotonicity of the density.  This is purely
      analytic and does not mention computability.
\item Transport it through the two inverse identities in
      \path{FabiusInverse.lean} to obtain
      \eqref{eq:lean-pointwise-modulus}, including the clamped tails.
\item Initially define a recursive modulus using exact dyadic values:
      compute $F(2^{-r})\in\Q$, then take a natural ceiling of its reciprocal.
      This uses machinery already formalized in the exact dyadic modules.
...
\item Add the elementary closed bound $\Delta_r$ as a second modulus.  This
      removes the exact-dyadic table from the logical dependency graph and
      makes the theorem more portable, but is not needed for the first
      complete formal proof.
\end{enumerate}

The exact-dyadic-first route is likely shortest because it reuses an executable
positive rational already present in the library.  The event-bound route is
conceptually cleaner and gives the closed expression needed for
\cref{thm:mu-asymptotic}; it can be added after the main theorem is accepted.
...
law.  In higher dimensions, inverse CDFs are replaced by triangular
Rosenblatt maps or optimal-transport maps.  Effective invertibility would
require computable lower bounds on masses of suitable boxes or sections.
...
The inverse Fabius function is computable in precisely the two-part sense
requested.  The effective-uniform-continuity clause follows from the exact
geometric identity
\[
 \omega_G(\eta)=G(\eta),
\]
...
at the flat endpoints, and turns the repository's lower-Lambert quantile
asymptotics into sharp continuity-modulus asymptotics.  The same proof covers
the inverse on $[0,1]$ and the canonical clamped totalization on all of
$\R$.

Conceptually, the decisive bridge is simple: for a symmetric unimodal density,
endpoint intervals have the least mass.  In the Fabius setting that shape fact
connects probability, computable analysis, exact dyadic arithmetic, and
inverse asymptotics.  It is also the part most likely to generalize beyond the
...
\texttt{TargetName(i,s)} returns a rational within $2^{-s}$ of the clamped
input $y_i$.  The function \texttt{CenteredSpline(N,c)} is the exact rational
...
\toprule
Claim & Input & Status in this report \\
\midrule
\endhead
...
of uniforms & Proved in \cref{prop:finite-cdf}.\\
\addlinespace
Uniform centered-spline error $2^{-N}$ & Tail support and the global density
bound $p\le2$ & Proved in \cref{thm:centered-error}; also formalized forward
in the repository.\\
\addlinespace
Strict increase of $F$ & Positivity of finite-convolution densities and a
short tail & Proved in \cref{prop:strict-increase}.\\
\addlinespace
Symmetric strict unimodality of $p=F'$ & Differential equation, symmetry, and
strict increase of $F$ & Proved in \cref{prop:density-shape} from imported
Fabius identities.\\
\addlinespace
Least interval mass $F(h)$ & Symmetric unimodality of $p$ & Proved in
\cref{thm:least-mass}.\\
\addlinespace
Exact inverse modulus $G(\eta)$ & Least interval mass and inverse identities &
Proved in \cref{thm:exact-inverse-modulus}.\\
\addlinespace
Closed modulus $\Delta_r$ & Explicit positive box event in the random series &
Proved in \cref{thm:closed-dyadic-modulus}.\\
\addlinespace
Effective uniform continuity & Closed modulus and recursive integer arithmetic &
Proved in \cref{thm:reciprocal-modulus}.\\
\addlinespace
Sequential computability & Forward rational oracle, closed inverse modulus,
and tolerant bisection & Proved in
\cref{thm:bisection-correct,cor:GT-sequential}.\\
\addlinespace
Optimal bit asymptotic & Closed lower bound plus iterated-flatness upper bound &
Proved in \cref{thm:mu-asymptotic}.\\
\addlinespace
Sharp periodic refinement & Repository's all-orders endpoint inverse theory &
Imported and reinterpreted in
\cref{cor:sharp-asymptotic-modulus}; not rederived.\\
\bottomrule
\end{longtable}
...
$q$-Richardson filters, Bell-polynomial inversion, and all-orders endpoint
quantile asymptotics.\\
\addlinespace
...
independent mathematical sources.  The repository was pinned because its
\texttt{main} branch advanced during the audit; all path and count statements
refer to commit \commit.
...
\newblock repository snapshot \commit,
