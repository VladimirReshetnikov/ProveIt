...
\lean{Fabius.two\_pow\_mod\_three\_eq\_mod\_two\_add\_one}, \(2^r\bmod3=(r\bmod2)+1\).
...
\emph{Formal status.}  Neither display of \cref{p1:cor:mod-three-sine} is formalized: the
signed row sum \(S(n)\) of \eqref{p1:eq:S-pascal-exact} has no Lean counterpart yet, and
both formulas are rewritings of \cref{p1:thm:mod-three} through it.  What is formal is
everything the proof uses apart from \(S(n)\) itself --- the residue law of
\cref{p1:thm:mod-three} and the two-cycle
\lean{Fabius.two\_pow\_mod\_three\_eq\_mod\_two\_add\_one}.
