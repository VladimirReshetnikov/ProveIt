...
formal logarithm carries the denominators \(1/k\) and so lives over \(\Q\); differentiating
\eqref{p1:eq:log-E} and clearing \(E\) gives the equivalent identity
\[
  z\,E'(z)=-A(z)\,E(z),\qquad A(z)=\sum_{k\ge1}a_kz^k,
\]
which holds over \(\Z\), and that is
...
\(\Q\)-logarithm itself, and the analytic identity for \(|z|<1\) --- are not formalized:
