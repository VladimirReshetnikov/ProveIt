...
Weighted complex block & \(\sum_{n<2^m}u^{\wt(n)}\e^{a+nz}=\e^a\prod_{j<m}(1+u\e^{2^jz})\), including the base case \(a=0\). \\
Signed complex block & \(\sum_{n<2^m}\eps_n\e^{a+nz}=\e^a\prod_{j<m}(1-\e^{2^jz})\), including the base case \(a=0\). \\
...
there and the sum here agree, and \(S_2\) is \(\wt\) by definition of the latter.  It
...
hold for every \(n,j\in\N\), while the last assumes exactly \(n<2^j\) and
...
stated additively as \(\wt(n)+\vtwo(n!)=n\) so that no subtraction in \(\N\) is needed.
...
\(\wt(2n)=\wt(n)\) cancelling one copy of the weight.  Thus the Lean proof does
...
\(\wt(a+b)+\vtwo\binom{a+b}a=\wt(a)+\wt(b)\); the sign cocycle
...
\(\prod_{j<m}(1+uz^{2^j})=\sum_{n<2^m}u^{\wt(n)}z^n\), from which the finite
...
\[
  \sum_{\substack{n<2^{N}\\ \wt(n)=r}} q^{\sigma(n)}
...
over an \emph{arbitrary} semiring --- by an induction over \(\N\), where the \(q\)-Pascal
...
version was confined to \(\Z\)).  Alongside it the \(q\)-Pochhammer symbol acquires the same
...
For every \(m\in\N\) and \(u,a,z\in\C\),
\begin{align}
 \sum_{n<2^m}u^{\wt(n)}\e^{nz}
  &=\prod_{j<m}\left(1+u\e^{2^jz}\right),
  \label{p1:eq:complex-weighted-block}\\
 \sum_{n<2^m}u^{\wt(n)}\e^{a+nz}
  &=\e^a\prod_{j<m}\left(1+u\e^{2^jz}\right),
  \label{p1:eq:complex-weighted-block-affine}\\
 \sum_{n<2^m}\eps_n\e^{nz}
  &=\prod_{j<m}\left(1-\e^{2^jz}\right),
  \label{p1:eq:complex-signed-block}\\
 \sum_{n<2^m}\eps_n\e^{a+nz}
  &=\e^a\prod_{j<m}\left(1-\e^{2^jz}\right).
...
\(u\e^{2^jz}\) term at the bit positions of \(n<2^m\) contributes
\(u^{\wt(n)}\e^{nz}\), because the selected powers of two sum to \(n\).
The exponential addition law factors the common translate \(\e^a\) and gives
\eqref{p1:eq:complex-weighted-block-affine}.  Finally put \(u=-1\) and use
\((-1)^{\wt(n)}=\eps_n\) to obtain
...
\(z=\e^{t}\in\mathbb Q[[t]]\).  No named Lean theorem in this crosswalk
...
\[
 ...
 =\e^{-c_mt}\prod_{j<m}\left(1-\e^{2^jt}\right)
\]
...
\(1-\e^u=-2\e^{u/2}\sinh(u/2)\), and hence the centered sinh product
...
\path{ThueMorseComplexProductBridge.lean}.  Over \(\R\), Lean proves
...
\(\int_0^1\cos(2\pi t)\,(1+\cos 6\pi t)(1+\cos 8\pi t)\dd t=\tfrac14\)
...
$\ii t/2^{m-1}$ in \cref{p2:thm:finite-sinc} and $-s/2^m$ in
...
every nonzero integer, its $j=0$ factor being $\sinc(\pi n)$, so both sides vanish together on
