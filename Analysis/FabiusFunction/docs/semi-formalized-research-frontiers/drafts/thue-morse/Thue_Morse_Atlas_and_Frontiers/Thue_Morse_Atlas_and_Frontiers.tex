...
\emph{Formal versions.}  The digit extraction \eqref{p1:eq:bit-floor} is
\lean{Fabius.div\_two\_pow\_sub\_two\_mul\_div\_two\_pow\_succ}, with the bit itself on
the left in \lean{Fabius.testBit\_toNat\_eq\_div\_sub\_two\_mul\_div}, both in
\path{BinaryDigitFloor.lean}.  The corpus carried the equivalent \emph{residue} form
first --- \lean{Fabius.testBit\_iff\_div\_two\_pow\_mod\_two} in
...
\emph{Formal versions.}  Equation \eqref{p1:eq:central-v2} is
\lean{Fabius.padicValNat\_two\_centralBinom} and the sign form
\eqref{p1:eq:central-thue-morse} is
\lean{Fabius.thueMorseSign\_eq\_neg\_one\_pow\_centralBinom}, both in
\path{CentralBinomialValuation.lean}.  The Lean proof does not pass through the
factorial valuation as the proof above does: it is the \(a=b=n\) case of the additive
Kummer identity \lean{Fabius.binaryWeight\_add\_addChoose\_padicValNat}, with
\(\wt(2n)=\wt(n)\) cancelling one copy of the weight.  Nothing about central binomial
coefficients as such is used.
...
\emph{Formal versions.}  The multi-variable identity is formal in two pieces, and the
split is the interesting part.  Over any commutative semiring,
...
\(\{0,\dots,2^m-1\}\).  Composing them is \eqref{p1:eq:boolean-mother}.  The corpus
states the composite itself only in the one-variable specialization
...
Thue--Morse product and the weight enumerator both follow by choosing \(u\).
...
\emph{Formal version, and where it stops.}  The \emph{exact finite} form --- the
display above truncated at \(M\) with its remainder retained --- is
\lean{Fabius.thueMorseBitZSeries\_lacunary\_quotient} in
\path{ThueMorseLacunaryQuotient.lean}, stated over the formal power series ring \(\Z[[X]]\) for
every \(M\), with the remainder written as the \(M\)-fold
\lean{PowerSeries.expand} of the series.  That is the identity in the line below,
proved as an identity of formal power series and so with no hypothesis on \(z\).  The
passage to the limit \(M\to\infty\) that produces the infinite sum
\eqref{p1:eq:lacunary-quotient} is not formalized: it needs \(|z|<1\) and a convergence
argument for \(P_M(z)T(z^{2^M})\to0\), neither of which a formal series carries.
...
the corpus develops.  What is formal is each of the two ingredients their proofs use,
and in both cases as a statement about a \emph{formal power series}, which by itself
says nothing about values: the Mahler equation \eqref{p1:eq:E-Mahler} is
...
\path{ThueMorseHankel.lean}, proved through the sharper
...
\emph{Formal versions, and where they stop.}  \path{ThueMorseAutocorrelation.lean}
formalizes the \emph{finite-level} structure exactly, on the unnormalized
\(A_m(k)=\sum_{n<2^m}\eps_n\eps_{n+k}\): the zero shift
\lean{Fabius.thueMorseAutocorrelation\_zero\_shift} (\(A_m(0)=2^m\)), the even branch
\lean{Fabius.thueMorseAutocorrelation\_succ\_even} (\(A_{m+1}(2r)=2A_m(r)\)), the odd
branch \lean{Fabius.thueMorseAutocorrelation\_succ\_odd}
(\(A_{m+1}(2r+1)=-(A_m(r)+A_m(r+1))\)), and the bound
\lean{Fabius.abs\_thueMorseAutocorrelation\_le}, which is what puts every normalized
value in \([-1,1]\).  These are \eqref{p1:eq:eta-even} and \eqref{p1:eq:eta-odd} before
the limit is taken.

The \emph{limit} is formalized only at the first three shifts:
\lean{Fabius.tendsto\_thueMorseAutocorrelation\_zero\_shift},
\lean{Fabius.tendsto\_thueMorseAutocorrelation\_one} and
\lean{Fabius.tendsto\_thueMorseAutocorrelation\_two}, giving
\(\eta(0)=1\) and \(\eta(1)=\eta(2)=-\tfrac13\).  Those three go through closed forms
that leave \(\Z\) only at the last step --- \(3A_m(1)=-2^m-2(-1)^m\)
(\lean{Fabius.three\_mul\_thueMorseAutocorrelation\_one}) and its shift-two companion
--- so the geometric error term \((-\tfrac23)(-\tfrac12)^m\) is explicit rather than
asymptotic.  Existence of \(\eta(k)\) at a general \(k\), and the remaining samples in
\eqref{p1:eq:eta-samples} and \eqref{p1:eq:eta-first-values}, are not formalized; this
is the analytic frontier flagged above the theorem, and the corpus stops exactly
there.
