...
\emph{Formal status.}  Only the degree-zero case of \eqref{p1:eq:base-q-Prouhet} is
formal so far: \lean{Fabius.sum\_zeta\_pow\_digits\_sum\_eq\_zero} in
\path{BaseDigitProduct.lean} gives \(\sum_{n<q^m}\zeta^{s_q(n)}=0\) for \(m\ge1\) under
the single hypothesis \(\sum_{r<q}\zeta^r=0\), by evaluating the digit product at \(z=1\).
The cancellation for polynomials of positive degree below \(m\), and the sharp moment
\eqref{p1:eq:base-q-sharp-moment}, are not yet formalized.
