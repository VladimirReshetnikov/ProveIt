...
Ordinary DFT & \(\widehat\eps_m(k)=\prod_{j<m}(1-\EulerE^{-2\pi\ImaginaryUnit k2^j/2^m})\). \\
...
the digit sum, so
\begin{equation}
  \BinaryDigitSum(n)=\BinaryDigitSum\bigl(\lfloor n/2\rfloor\bigr)+(n\bmod2),
...
\(\BinaryDigitSum(n)=\sum_{i<n}\bigl(\lfloor n/2^i\rfloor\bmod2\bigr)\) yields a
...
\(\lfloor n/2^j\rfloor\) is odd --- and the difference form was added so that this
...
For nonintegral \(x\),
\begin{equation}
  \Sign(\sin\pi x)=(-1)^{\lfloor x\rfloor}.
...
For every \(n\ge0\),
\begin{equation}
  \TwoAdicValuation(n!)=\sum_{j\ge1}\left\lfloor\frac n{2^j}\right\rfloor
           =n-\BinaryDigitSum(n).
...
\[
  \sum_{h\ge0}a_h\Bigl\lfloor\frac{N}{2^{h}}\Bigr\rfloor
  \;=\;N\,A(\tfrac12)-\rho_a(N),
  \qquad
...
identity reduces to Legendre's \(\sum_h\lfloor N/2^h\rfloor=2N-s_2(N)\), which is carried as a
...
\(z=\EulerE^{t}\in\mathbb Q[[t]]\).  No named Lean theorem in this crosswalk
...
\begin{equation}
  ...
  \eps_n=\frac1{2^m}
   ...
   \widehat\eps_m(k)\EulerE^{2\pi\ImaginaryUnit kn/2^m},
  \qquad 0\le n<2^m.}
...
\(\sum_{n<2^m}\ThueMorseSign{a\mathbin{\&}n}=2^m\ind_{a=0}\) as
...
Fourier analysis & $\widehat{\RvachevUp}$ as a dyadic sinc product; shell factorization; pointwise,
...
\[
 \widehat h(\xi)=\int_{\RealNumbers}h(x)\EulerE^{-2\pi\ImaginaryUnit x\xi}\Differential x,
\]
the normalized Rvachev density satisfies
\begin{equation}
 \widehat{\RvachevUp}(\xi)=Q_\infty(2\pi\xi)
...
$\widehat{\RvachevUp}(z)=\exp\bigl(-\sum_{r\ge1}\zeta(2r)4^rz^{2r}/(r(4^r-1))\bigr)$ on $|z|<1$
...
states that generating function to be $\EulerE^{z/2}\widehat\RvachevUp(\ImaginaryUnit z/(4\pi))$ for every
$z\in\ComplexNumbers$; and \leanPartx{Fabius.rvachevFourier_eq_product} (\path{FourierProduct.lean}) replaces
$\widehat\RvachevUp$ by its sinc product.  Since $\widehat\RvachevUp(\ImaginaryUnit z/(4\pi))=Q_\infty(\ImaginaryUnit z/2)$ in the
...
\(\mathbb{E}[e^{itX_\infty}]=e^{it/2}\,\widehat{\RvachevUp}(t/(4\pi))\)
...
convention \(\widehat{\RvachevUp}(z)=\int_\RealNumbers\RvachevUp(t)\EulerE^{-2\pi\ImaginaryUnit tz}\Differential t\) and derive every rescaling
...
\(\widehat{\RvachevUp}=Q_\infty\) on all of \(\ComplexNumbers\), the centered characteristic function
\(\mathbb{E}[e^{itY_\infty}]=\widehat{\RvachevUp}(t/(2\pi))\), and the uncentered
\(\mathbb{E}[e^{itX_\infty}]=e^{it/2}\,\widehat{\RvachevUp}(t/(4\pi))\).  The individual legs that fix the
...
\(\mathbb{E}[e^{itX_\infty}]=e^{it/2}\,\widehat{\RvachevUp}(t/(4\pi))\) for the uncentered
\(X_\infty=(Y_\infty+1)/2\), while
\leanPartx{Fabius.rvachevMeasure\_charFun\_pos} gives the centered identity
\(\mathbb{E}[e^{itY_\infty}]=\widehat{\RvachevUp}(t/(2\pi))\).  These fix the rescaling between
