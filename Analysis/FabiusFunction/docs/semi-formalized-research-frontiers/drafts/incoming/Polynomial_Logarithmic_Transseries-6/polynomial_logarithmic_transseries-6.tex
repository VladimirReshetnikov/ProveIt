...
\]
Consequently, if
$F=\ApproximationOf{F}+\FormalBigO_t(t^{p})$ and
$G=\ApproximationOf{G}+\FormalBigO_t(t^{q})$, then
...
Consider
\begin{align*}
 A&=1+(L+1)t+(L^2-L)t^2+\FormalBigO_t(t^3),\\
 B&=1+(2L-1)t+(3-L)t^2+\FormalBigO_t(t^3).
\end{align*}
...
\begin{align*}
 ...
 &\quad+\bigl[(L^2-L)+(L+1)(2L-1)+(3-L)\bigr]t^2
   +\FormalBigO_t(t^3)\\
 &=1+3Lt+(3L^2-L+2)t^2+\FormalBigO_t(t^3).
\end{align*}
For the quotient, write
\[
 \frac AB=1+Q_1t+Q_2t^2+\FormalBigO_t(t^3).
\]
...
\begin{equation}\label{eq:worked-quotient}
 \frac AB
 =1+(2-L)t+(3L^2-5L-1)t^2+\FormalBigO_t(t^3).
\end{equation}
...
\end{equation}
If $F(G_j)-Y=\FormalBigO_t(t^m)$ and
$F'(G_j)=1+\FormalBigO_t(t)$, the next residual is typically
$\FormalBigO_t(t^{2m})$. Thus the working precision can be doubled at
each stage:
\[
...
\[
 T=\sum_{n=n_0}^{N}p_n(L)t^n+\FormalBigO_t(t^{N+1})
\]
