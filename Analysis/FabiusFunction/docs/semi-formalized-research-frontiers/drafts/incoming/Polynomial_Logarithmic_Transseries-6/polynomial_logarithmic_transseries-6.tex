...
\newcommand{\PL}{\mathscr{PL}}
\newcommand{\PLpoly}{\mathscr{PL}_{\mathrm{poly}}}
\newcommand{\PLfield}{\widehat{\mathscr{PL}}}
...
\newcommand{\compinv}{^{\langle-1\rangle}}
\newcommand{\compose}{\mathbin{\circ}}
\newcommand{\trunc}[2]{\left[#1\right]_{\le #2}}
\newcommand{\coeff}[2]{\left[#1\right]_{#2}}
...
\large with Lambert's \texorpdfstring{$W$}{W} function as the guiding example}
...
\[
 ...
 \frac{\dd^{\,n-1}}{\dd Y^{\,n-1}}A(Y)^n,
\]
whenever the polynomial--logarithmic filtration makes the family summable.
Taking $A(X)=\log X$ yields an all-orders derivation of the Lambert-$W$
...
on Lambert $W$, the NIST Digital Library of Mathematical Functions,
...
for the full theory.  It is large enough for Lambert $W$, many inverse
...
\item Read Part~\ref{part:lambert} for the complete Lambert-$W$ calculation.
...
field $\K$, usually $\R$ or $\C$.  For an analytic interpretation, $X$ tends
...
\begin{equation}\label{eq:W-log-intro}
 W(x)+\log W(x)=\log x.
\end{equation}
The dominant balance is $W(x)\sim\log x$, but substitution of this first
approximation leaves the smaller error $\log\log x$.  Thus the first two
scales are forced:
\[
 W(x)=\log x-\log\log x+\hbox{smaller terms}.
\]
If
\[
...
\begin{equation}\label{eq:W-shape-intro}
 W(x)=L_1-L_2+\sum_{n\ge1}\frac{P_n(L_2)}{L_1^n}.
\end{equation}
...
Lambert $W$ is unusually friendly: its standard double logarithmic series is
...
\[
 \ord_tF=n_0.
\]
Write $F=\bigO_t(t^N)$ when $\ord_tF\ge N$.  This is a formal block statement,
not the analytic estimate $|F|\le C|t|^N$.
\end{definition}
...
and
\[
 F\equiv G\pmod{t^{N+1}}
 \quad\Longleftrightarrow\quad
 \trunc{F}{N}=\trunc{G}{N}.
\]
All finite-order algorithms below take place exactly in the quotient algebra
\[
 \K[L]((t))/t^{N+1}\K[L][[t]],
\]
with the finite principal part retained when needed.
...
\]
To make division systematic, enlarge the coefficient ring to the Laurent
series field in $L^{-1}$:
...
$X^{-1}=o((\log X)^{-m})$ for every fixed $m$.
\end{remark}
...
$\Gamma\subset\R$ one may consider
\[
...
\begin{equation}\label{eq:nested-field}
 \K((\eps_d))\cdots((\eps_2))((\eps_1)).
\end{equation}
The rightmost variable $\eps_1$ is the outermost and smallest scale.  In
particular,
\[
 \eps_1\precdom \eps_2^m\precdom\eps_3^n\precdom1
 \qquad(m,n>0).
\]
...
\begin{equation}\label{eq:block-asymptotic}
 f(X)\sim_{\PL}\sum_{n\ge n_0}P_n(\log X)X^{-n}
\end{equation}
if for every integer $N\ge n_0$,
\[
 f(X)-\sum_{n=n_0}^{N}P_n(\log X)X^{-n}=o(X^{-N})
 \qquad(X\to+\infty).
\]
\end{definition}
...
$X^{-n}$ is a nonzero polynomial $Q(\log X)+o(1)$.  A nonzero polynomial in
...
\]
Consequently, if $F\equiv\widetilde F\pmod{t^{p}}$ and
$G\equiv\widetilde G\pmod{t^{q}}$, then
\begin{equation}\label{eq:mult-error-rule}
 FG-\widetilde F\widetilde G
 =\bigO_t\!\left(t^{\min(p+m_0,q+n_0)}\right).
\end{equation}
\end{theorem}
\begin{proof}
...
$F=\widetilde F+E$ and $G=\widetilde G+H$, with
$\ord_tE\ge p$ and $\ord_tH\ge q$.  Then
\[
 FG-\widetilde F\widetilde G=E\widetilde G+\widetilde F H+EH,
\]
...
Take
\begin{align*}
 F&=1+(L+1)t+(L^2-L)t^2+(2L+3)t^3+\bigO_t(t^4),\\
 G&=1-Lt+(2L-1)t^2+(L^2+1)t^3+\bigO_t(t^4).
\end{align*}
...
\begin{equation}\label{eq:product-example}
 FG=1+t+(L-1)t^2+(-L^3+4L^2+2L+3)t^3+\bigO_t(t^4).
\end{equation}
...
\KwOut{Blocks $C_0,\ldots,C_N$ of $FG\bmod t^{N+1}$}
...
coefficient degree is $d$, the direct cost is $O(N^2d^2)$ scalar operations.
...
For example, discarding all terms with $|k|>10$ may remove $t^2L^{100}$ while
...
Assume
\[
 f(X)=\sum_{n=n_0}^{N}A_n(L)X^{-n}+o(X^{-N}),
 \quad
 g(X)=\sum_{m=m_0}^{M}B_m(L)X^{-m}+o(X^{-M}),
\]
...
$X^{-p}$ times a polynomial in $L$ is $o(X^{-p+\delta})$ for every
...
Let
\[
 B=1-Lt+(2L-1)t^2+(L^2+1)t^3+\bigO_t(t^4).
\]
...
\begin{align}\label{eq:reciprocal-example}
 ...
 +(L^3-5L^2+2L-1)t^3\notag\\
 &+(L^4-8L^3+7L^2-6L+1)t^4+\bigO_t(t^5).
\end{align}
...
\end{equation}
Over $\R$, a real value for $c^\alpha$ requires the usual sign conditions.
Over $\C$, a branch of $\Log c$ must be selected.  Rational $\alpha$ may
...
If $S=\bigO_t(t^q)$ with $q>0$, then the coefficient of $t^N$ in
$(1+S)^\alpha$ receives contributions only from powers
$r\le\lfloor N/q\rfloor$.  Hence \eqref{eq:general-power} is
...
and small part, then
\[
 \exp F=\e^c\exp(F_{\mathrm{large}})\exp S.
\]
...
\begin{equation}\label{eq:derivative-block}
 \D\bigl(P(L)t^n\bigr)
 :=\frac{\dd}{\dd X}\bigl(P(L)X^{-n}\bigr)
...
\begin{equation}\label{eq:euler-derivation}
 \Thetaop=X\frac{\dd}{\dd X}=t^{-1}\D.
\end{equation}
Then
\begin{equation}\label{eq:euler-block}
 \Thetaop\bigl(P(L)t^n\bigr)=t^n(P'-nP).
\end{equation}
Thus each outer block is invariant under $\Thetaop$, and its coefficient is
...
If $A=\bigO_t(1)$ contains no $X$ term, then
\begin{equation}\label{eq:A-derivative-small}
 \D A=\bigO_t(t).
\end{equation}
Consequently, changing an argument by an error $E=\bigO_t(t^r)$ changes
$A$ by roughly $A'E=\bigO_t(t^{r+1})$.  This one-block gain is the mechanism
...
\end{theorem}
\begin{proof}
Because $U=\bigO_t(t)$, the coefficient of $t^r$ in $(1+U)^{-n}$ or
...
\begin{align*}
 ...
 &=L+Lt-\frac12L^2t^2+\frac13L^3t^3
   -\frac14L^4t^4+\bigO_t(t^5),\\
 \frac1G
 &=t(1+Lt)^{-1}\\
 &=t-Lt^2+L^2t^3-L^3t^4+\bigO_t(t^5).
\end{align*}
Therefore
...
\KwOut{$F\compose G\pmod{t^{N+1}}$}
$U\leftarrow t\Delta$\;
$R\leftarrow(1+U)^{-1}\pmod{t^{N-n_0+1}}$\;
$V\leftarrow\log(1+U)\pmod{t^{N-n_0+1}}$\;
$H\leftarrow0$\;
\For{$n\leftarrow n_0$ \KwTo $N$}{
  $H\leftarrow H+t^nR^nP_n(L+V)$\;
  $H\leftarrow H\bmod t^{N+1}$\;
}
\Return{$H$}\;
\end{algorithm}
...
$\D^r(Pt^n)$ has outer order $n+r$.  Since $\ord_t\Delta\ge0$, the $r$th
...
\begin{align}\label{eq:F-G-coeff-notation}
 F(X)&=X+f_0(L)+t f_1(L)+t^2f_2(L)+t^3f_3(L)+\bigO_t(t^4),\\
 G(X)&=X+g_0(L)+t g_1(L)+t^2g_2(L)+t^3g_3(L)+\bigO_t(t^4).
\end{align}
Let
\[
 H=F\compose G
 =X+h_0+t h_1+t^2h_2+t^3h_3+\bigO_t(t^4).
\]
...
\begin{equation}\label{eq:composition-noncommutative}
 F\compose G-G\compose F=-L^2t+\bigO_t(t^2).
\end{equation}
...
\begin{equation}\label{eq:near-id-group}
 \mathcal G_{\PL}
 =\left\{F=X+A(X):A\in\mathcal C_L[[t]]\right\}.
\end{equation}
Every such $F$ satisfies $F/X=1+tA=1+o(1)$ formally.  The identity is $X$.
Closure under composition follows from the preceding chapters.

\begin{theorem}[Near-identity group]\label{thm:near-id-group}
The set $\mathcal G_{\PL}$ is a group under composition.  Every
...
$A-B=\bigO_t(t^r)$.  If $H=X+C$ is fixed, then
\begin{equation}\label{eq:composition-continuity-left}
 F-G=\bigO_t(t^r)
 \quad\Longrightarrow\quad
 H\compose F-H\compose G=\bigO_t(t^r),
\end{equation}
and, because $H'=1+\bigO_t(t)$,
\begin{equation}\label{eq:composition-continuity-right}
 F\compose H-G\compose H=\bigO_t(t^r).
\end{equation}
...
\eqref{eq:h1},
\begin{align*}
 F\compose G&=X+f+g+t\,g f'+\bigO_t(t^2),\\
 G\compose F&=X+f+g+t\,f g'+\bigO_t(t^2).
\end{align*}
Hence
...
$\mathcal G_{\PL}$ is filtered and pro-unipotent at finite truncation, one can
...
\begin{equation}\label{eq:contraction-general}
 U-V=\bigO_t(t^r)
 \quad\Longrightarrow\quad
 \Phi(U)-\Phi(V)=\bigO_t(t^{r+1}).
\end{equation}
Then the iterates $U_{n+1}=\Phi(U_n)$ stabilize one additional block per step:
$U_{n+1}-U_n=\bigO_t(t^n)$ after a suitable initialization.  This is the
...
$\log G=\rho L+\sigma\log L+\log c+o(1)$.  Fractional powers and deeper
...
If $B-C=\bigO_t(t^r)$, with $B,C\in\mathcal C_L[[t]]$, then
\begin{equation}\label{eq:inverse-contraction}
 A(Y+B)-A(Y+C)=\bigO_t(t^{r+1}).
\end{equation}
\end{lemma}
\begin{proof}
...
$B-C=\Phi(B)-\Phi(C)=\bigO_t(t^{r+1})$, a contradiction.
\end{proof}

\section{Why a bad rearrangement may fail}
For Lambert inversion one can rearrange the equation in more than one way.
If $X=W\e^W$, then $W=\log X-\log W$, which is contractive after the dominant
term is removed.  The equivalent equation $Q=\e^{-Q}-\log X$ for a correction
...
\KwOut{$G=F\compinv\pmod{t^{N+1}}$}
$B\leftarrow0$\;
\For{$j\leftarrow0$ \KwTo $N$}{
  $B\leftarrow-\,A\compose(Y+B)\pmod{t^{N+1}}$\;
}
\Return{$Y+B$}\;
\end{algorithm}
...
\begin{summarybox}
For $F=X+A$ with $A=o(X)$ in the polynomial--logarithmic scale, the equation
$B=-A(Y+B)$ is contractive in outer order because $A'=\bigO_t(t)$.  Simple
...
\[
 F\compose G_{<n}=Y+r_nt^n+\bigO_t(t^{n+1}).
\]
Adding $g_nt^n$ to $G$ changes $F(G)$ by
\[
 F'(G_{<n})g_nt^n+\bigO_t(t^{n+1})
 =g_nt^n+\bigO_t(t^{n+1}),
\]
because $F'=1+\bigO_t(t)$.  Hence
...
\For{$n\leftarrow0$ \KwTo $N$}{
  $R\leftarrow F\compose G-Y\pmod{t^{n+1}}$\;
  $r_n\leftarrow[t^n]R$\;
...
\begin{equation}\label{eq:parametric-F}
 F(X)=X+aL+b+(cL+d)t+(eL^2+fL+g)t^2+\bigO_t(t^3).
\end{equation}
The inverse has the form
\[
 G(Y)=Y+g_0(L)+g_1(L)t+g_2(L)t^2+\bigO_t(t^3),
\]
with
\begin{align}
...
\begin{equation}\label{eq:inverse-log-q}
 ...
 +\bigO_t(t^3).
\end{equation}
At $q=0$ this begins the Wright omega, hence Lambert-$W$, expansion developed
...
\begin{summarybox}
Series reversal is triangular because $F'=1+\bigO_t(t)$.  Once an approximate
...
\end{equation}
The denominator is a unit because $F'=1+\bigO_t(t)$.

\begin{theorem}[Precision doubling]\label{thm:newton-doubling}
If $R=\bigO_t(t^m)$, then the residual after
\eqref{eq:newton-compositional} is at least
$\bigO_t(t^{2m+2})$ when $m\ge0$.  In particular, the number of correct outer
blocks approximately doubles at each step.
\end{theorem}
\begin{proof}
Put $C=-R/(F'\compose G)$.  Then $C=\bigO_t(t^m)$.  Taylor expansion gives
\begin{align*}
 F\compose(G+C)-Y
 &=R+(F'\compose G)C
...
\While{$p\le N$}{
  ...
  $R\leftarrow F\compose G-Y\pmod{t^q}$\;
  $J\leftarrow F'\compose G\pmod{t^q}$\;
  $G\leftarrow G-R/J\pmod{t^q}$\;
  $p\leftarrow q$\;
}
\Return{$G$}\;
\end{algorithm}
...
$N$ compositions at growing precision, while Newton performs $O(\log N)$
...
The preceding chapters assume $F(X)=X+o(X)$.  A general asymptotic inverse may
...
\begin{align}\label{eq:u-power-log-inverse}
 ...
 +\bigO\!\left(\frac{(\log S)^2}{S^2}\right)\biggr].\notag
\end{align}
...
\begin{align}\label{eq:X-power-log-inverse}
 ...
 \frac{\sigma^2\log S+\sigma\log c-\sigma^2\log\rho}
      {\rho S}
 +\bigO\!\left(\frac{(\log S)^2}{S^2}\right)\right].\notag
\end{align}
...
\[
 ...
 +\bigO\left(\frac{(\log S)^2}{S^2}\right).
\]
...
\[
 X\e^{(\beta/\alpha)X}=Y^{1/\alpha}.
\]
Thus
\begin{equation}\label{eq:power-exp-W}
 X=\frac{\alpha}{\beta}
 W_k\!\left(\frac{\beta}{\alpha}Y^{1/\alpha}\right).
\end{equation}
The large-$Y$ expansion therefore inherits exactly the
polynomial--logarithmic structure of $W$ after scaling.
...
Lambert $W$ near $-\e^{-1}$ is the important case $m=2$: its branch-point
...
\begin{equation}\label{eq:omega-def}
 \omega(Y)+\Log\omega(Y)=Y,
 \qquad
 \omega(Y)=W(\e^Y)
\end{equation}
where branches are chosen consistently.  The DLMF treats both $W$ and
$\omega$ and records their branch structure and asymptotic expansions
\cite{DLMF413}.  For formal calculation, \eqref{eq:omega-def} simply says
\begin{equation}\label{eq:omega-inverse}
 \omega=(X+\log X)\compinv.
\end{equation}
...
\begin{equation}\label{eq:omega-fixed}
 \omega=Y-\log\omega.
\end{equation}
The dominant term is $Y$.  Starting with $\omega_0=Y$ gives
\[
 \omega_1=Y-L,
 \qquad L=\log Y.
\]
...
\begin{equation}\label{eq:omega-P-expansion}
 \omega(Y)=Y-L+\sum_{r\ge1}\frac{P_r(L)}{Y^r},
\end{equation}
where $P_r$ is a polynomial of degree $r$.  The next chapter develops these
...
\]
into $\omega+\log\omega=Y$.  Since
\[
 \log\omega=z+\log(1-sz+sQ),
\]
...
Let $Y=100$, so the corresponding Lambert argument is $z=\e^{100}$.  The exact
value is approximately
\[
 W(\e^{100})=95.44148664557583184017204\ldots.
\]
...
$Y-L$ & $4.67\times10^{-2}$ \\
$P_1/Y$ & $6.05\times10^{-4}$ \\
$P_2/Y^2$ & $5.27\times10^{-6}$ \\
$P_3/Y^3$ & $8.15\times10^{-8}$ \\
$P_4/Y^4$ & $5.60\times10^{-9}$ \\
$P_5/Y^5$ & $1.54\times10^{-10}$ \\
$P_6/Y^6$ & $2.03\times10^{-12}$ \\
\bottomrule
\end{tabular}
\end{center}
...
\item a different scale, namely a square-root Puiseux series, takes over at
      the branch point $-\e^{-1}$.
\end{itemize}

\begin{summarybox}
The Wright omega function is the compositional inverse of $X+\log X$.
Lambert $W_k(z)$ is obtained by evaluating this inverse at
$L_{1,k}=\Log z+2\pi i k$.  Lagrange inversion contributes exactly one term
...
\begin{align*}
 P_1(z)={}&z,\\
 P_2(z)={}&\frac12z^2-z,\\
 P_3(z)={}&\frac13z^3-\frac32z^2+z,\\
 P_4(z)={}&\frac14z^4-\frac{11}{6}z^3+3z^2-z,\\
 P_5(z)={}&\frac15z^5-\frac{25}{12}z^4
              +\frac{35}{6}z^3-5z^2+z,\\
 P_6(z)={}&\frac16z^6-\frac{137}{60}z^5
              +\frac{75}{8}z^4-\frac{85}{6}z^3
              +\frac{15}{2}z^2-z,\\
 P_7(z)={}&\frac17z^7-\frac{49}{20}z^6
              +\frac{203}{15}z^5-\frac{245}{8}z^4
              +\frac{175}{6}z^3-\frac{21}{2}z^2+z,\\
 P_8(z)={}&\frac18z^8-\frac{363}{140}z^7
...
\begin{align*}
 P_9(z)={}&\frac19z^9-\frac{761}{280}z^8
 +\frac{29531}{1260}z^7-\frac{1869}{20}z^6
 +\frac{7483}{40}z^5\\
 &-189z^4+91z^3-18z^2+z,\\
 P_{10}(z)={}&\frac1{10}z^{10}-\frac{7129}{2520}z^9
...
\begin{enumerate}
\item $P_r(0)=0$.
\item $\deg P_r=r$ and the leading coefficient is $1/r$.
...
\item verify $P_r(0)=0$, degree $r$, and leading coefficient $1/r$;
\item compare coefficients with \eqref{eq:P-stirling};
\item substitute the truncated $W$ expansion into $W+\log W=L_1$ and check
...
degree-$r$ polynomial $P_r(L_2)$.  It has equivalent differential-operator,
...
\chapter{Branches, convergence, and error control for Lambert \texorpdfstring{$W$}{W}}
...
$|L_{1,k}|\to\infty$ in an admissible sector,
\begin{equation}\label{eq:Wk-expansion}
 W_k(z)\sim L_{1,k}-L_{2,k}
 +\sum_{r\ge1}\frac{P_r(L_{2,k})}{L_{1,k}^r}.
\end{equation}
The branch number enters first through the additive term $2\pi i k$ in
...
Let $0<\eps\to0$ and write
\[
 W_{-1}(-\eps)=-u,
 \qquad u>0.
\]
The equation $W\e^W=-\eps$ becomes
\[
 u\e^{-u}=\eps.
\]
With
...
\begin{equation}\label{eq:u-lower-branch}
 u=S+L+\sum_{r\ge1}
 \frac{(-1)^{r+1}P_r(L)}{S^r}.
\end{equation}
Therefore
\begin{align}\label{eq:Wminus1-expansion}
 W_{-1}(-\eps)={}&-S-L-\frac{L}{S}
...
At $z=-\e^{-1}$, the derivative of $w\mapsto w\e^w$ vanishes at $w=-1$.
Put
\[
 p=\sqrt{2(1+\e z)}.
\]
...
\begin{equation}\label{eq:W-branchpoint}
 W_0(z)=-1+p-\frac13p^2+\frac{11}{72}p^3
...
$|s|$.  Along the Lambert path $s=1/L_1$, $z=L_2$, the quantity
$|s z|=|L_2/L_1|$ tends to zero.  Hence the series is genuinely convergent in
...
branch qualifications are given in the classical Lambert-$W$ literature
\cite{Corless1996,DLMF413}.

\section{Residual-based error estimation}
Let
\begin{equation}\label{eq:W-truncation}
 w_N=L_1-L_2+\sum_{r=1}^{N}\frac{P_r(L_2)}{L_1^r}
\end{equation}
and define the additive residual
\begin{equation}\label{eq:W-residual}
 R_N=w_N+\Log w_N-L_1.
\end{equation}
The exact solution $w$ satisfies $F(w)=L_1$ for $F(u)=u+\Log u$.  Since
...
\begin{equation}\label{eq:W-error-from-residual}
 w-w_N=\delta_N+\bigO\!\left(\frac{R_N^2}{|w_N|^2}\right)
\end{equation}
...
For the formal truncation,
\[
 R_N=\bigO\!\left(\frac{L_2^{N+1}}{L_1^{N+1}}\right),
\]
so the inverse error has the same leading order because $F'(w)=1+o(1)$.
...
\begin{enumerate}
\item the branch of $\Log z$ used to form $L_{1,k}$;
\item the integer $k$;
\item the branch of $\Log L_{1,k}$ used for $L_{2,k}$;
\item the sector in which $|L_{1,k}|\to\infty$;
...
$L_{1,k}=\Log z+2\pi i k$.  The lower real branch near zero is the inverse of
$X-\log X$ and uses alternating polynomial blocks.  Near $-\e^{-1}$ the scale
...
\section{Equations reducible exactly to \texorpdfstring{$W$}{W}}
A surprising number of inverse problems reduce to $W$ after elementary
normalization.
...
\begin{align}\label{eq:XlogX-asymptotic}
 ...
 +\bigO\!\left(\frac{L_2^4}{L_1^4}\right)\biggr].\notag
\end{align}
...
$\Pr(Z>q)=p$.  The classical tail expansion has the form
\[
 p=\frac{\e^{-q^2/2}}{\sqrt{2\pi}\,q}
...
\begin{equation}\label{eq:normal-quantile-first}
 ...
 +\bigO\!\left(\frac{(\log S)^2}{S^{3/2}}\right).
\end{equation}
...
Suppose $N$ is large and one wants to solve $\Gamma(X+1)=N$.  Stirling's
formula gives
\[
 \log N=X\log X-X+\frac12\log(2\pi X)
 +\frac1{12X}+\cdots.
\]
The dominant inverse is controlled by $X\log X$ and hence by Lambert $W$;
...
\begin{summarybox}
Lambert $W$ is both a special function and a normal form for asymptotic
...
Consider
\begin{align*}
 A&=1+(L+1)t+(L^2-L)t^2+\bigO(t^3),\\
 B&=1+(2L-1)t+(3-L)t^2+\bigO(t^3).
\end{align*}
...
\begin{align*}
 ...
 &\quad+\bigl[(L^2-L)+(L+1)(2L-1)+(3-L)\bigr]t^2
   +\bigO(t^3)\\
 &=1+3Lt+(3L^2-L+2)t^2+\bigO(t^3).
\end{align*}
For the quotient, write
\[
 \frac AB=1+Q_1t+Q_2t^2+\bigO(t^3).
\]
...
\begin{equation}\label{eq:worked-quotient}
 \frac AB
 =1+(2-L)t+(3L^2-5L-1)t^2+\bigO(t^3).
\end{equation}
...
$A=\bigO(L)$ and each application of $\D$ raises the outer $t$-order by one.
A direct calculation gives
\begin{align}
 G(Y)=Y&-2L-1+\frac{L+3}{Y}
 +\frac{-3L^2+7L-3}{Y^2}\notag\\
 &+\frac{-\frac{32}{3}L^3+25L^2-6L-\frac{59}{6}}{Y^3}
 +\bigO\!\left(\frac{L^4}{Y^4}\right).
...
\end{equation}
If $F(G_j)-Y=\bigO(t^m)$ and $F'(G_j)=1+\bigO(t)$, the next residual is
typically $\bigO(t^{2m})$. Thus the working precision can be doubled at
each stage:
\[
...
finite: they compute the image of a transseries in a quotient by a prescribed
filtration ideal. No machine is asked to store an infinite object.

\section{Sparse block representation}
A truncated one-logarithm transseries
\[
 T=\sum_{n=n_0}^{N}P_n(L)t^n+\bigO(t^{N+1})
\]
...
\KwOut{$F\compose G\bmod t_Y^{N+1}$}
$R\leftarrow(1+U)^{-1}\bmod t_Y^{N+1}$\;
$H\leftarrow\log(1+U)\bmod t_Y^{N+1}$\;
$S\leftarrow0$\;
\ForEach{retained block $(n,P_n)$ of $F$}{
  $S\leftarrow S+
  t_Y^n R^n\,P_n(L_Y+H)\pmod{t_Y^{N+1}}$\;
}
\Return $S$\;
\end{algorithm}
...
A safe schedule is:
\[
 1,\;2,\;4,\;8,\;\ldots,\;2^{\lceil\log_2N\rceil},
\]
...
Suppose $x_\ast=f^{-1}(y)$ and an approximation $x_N\in I$ satisfy
\[
 |f'(x)|\ge m>0
\]
...
For $f(X)=X+\bigO(\log X)$, one has $f'(X)=1+o(1)$ under the relevant
...
scale $\varepsilon$ is changed to scale $\varepsilon^2$ after normalization
...
The Lambert-$W$ large-argument expansion is commonly used asymptotically,
...
\[
 L=\Log X
\]
on a fixed branch in a sector
\[
 |\arg X-\theta_0|\le\Theta<\pi.
\]
...
\item prove a lower bound for $|f'|$ near $G_N$;
...
\[
 B=b_\lambda t^\lambda(1+R),\qquad \ord_tR>0.
\]
...
$(Y-Y_0)^{1/m}$. The square-root expansion of Lambert $W$ at
$-\e^{-1}$ is the canonical example. No amount of integer-power
coefficient matching repairs a wrong local scale.

\section{Suppressing branches of logarithms}
For complex $X$, both $\Log X$ and $\Log\Log X$ are branch dependent.
Changing the branch shifts the first logarithm by $2\pi i k$ and propagates
through every later block. A formula for $W_k$ must state which
$\log_k z$ is being used.
...
\[
 \bigO\!\left(\frac{(\log X)^d}{X^3}\right)
\]
...
term beyond every monomial in the chosen scale, such as $\e^{-X}$ relative
...
\[
 \frac{\dd}{\dd x}\bigl(x^nP(\lambda)\bigr)
...
For $x\to0^+$, the solution $W_{-1}(-x)$ tends to $-\infty$. Put
\[
...
\[
 P_1=z,\qquad
 P_2=\frac{z^2}{2}-z,\qquad
 P_3=\frac{z^3}{3}-\frac32z^2+z.
\]
...
If $x_N$ approximates $f^{-1}(y)$ and
$|f'|\ge m>0$ between $x_N$ and the exact inverse, then
\[
 |x_N-f^{-1}(y)|
 \le\frac{|f(x_N)-y|}{m}.
\]
...
$\K$&Characteristic-zero coefficient field, commonly $\R$ or $\C$.\\
$\PLpoly$&Polynomial-coefficient Laurent algebra $\K[L]((t))$.\\
$\PLfield$&Iterated Laurent completion, typically
$\K((L^{-1}))((t))$.\\
$\ord_tT$&Least outer exponent with nonzero coefficient.\\
$\supp T$&Set of monomials or outer exponents occurring in $T$.\\
...
filtration&Nested ideals or precision levels used to define truncation and
coefficientwise convergence.\\
near identity&A map $F(X)=X+o(X)$ in the selected scale.\\
...
$W$ is the canonical example because its logarithmic defining equation is
...
\newblock Convergence in $\mathbb{C}$ of series for the Lambert $W$ function,
