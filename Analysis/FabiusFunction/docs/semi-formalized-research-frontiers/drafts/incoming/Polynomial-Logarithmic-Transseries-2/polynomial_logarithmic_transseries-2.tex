...
\[
 f^{[-1]}(x)=x-\alpha\log x
 +\sum_{n\ge1}\frac{\LambertPolynomial{n}(\log x)}{x^n}.
\]
...
\[
 A=B+\FormalBigO_t(t^N),\qquad t=X^{-1},
\]
...
\end{proposition}
\begin{proof}
For each integer \(n\), choose \(N>n\).  Eventual stabilization modulo \(t^N\)
...
Take
\[
 A=1+(L-1)t+(L^2+2)t^2+\FormalBigO_t(t^3),
\]
\[
 B=1-(L+2)t+(3L-1)t^2+\FormalBigO_t(t^3).
\]
...
Therefore
\[
 AB=1-3t+(2L+3)t^2+\FormalBigO_t(t^3).
\]
...
Let
\[
 A=1+2Lq+(L^2+1)t^2+\FormalBigO_t(t^3).
\]
Using \((1+U)^{1/2}=1+U/2-U^2/8+\cdots\),
\[
 A^{1/2}=1+Lq+\frac12t^2+\FormalBigO_t(t^3),
\]
...
Let
\[
 A=1+Lq+(L^2+1)t^2+\FormalBigO_t(t^4),
\]
...
Hence
\[
 \frac1A=1-Lq-t^2+(L^3+2L)t^3+\FormalBigO_t(t^4).
\]
...
Thus
\[
 \frac CA=1+(L+2)t+(1-L)t^2+\FormalBigO_t(t^3).
\]
\end{example}
...
Let
\[
 A=1+Lq+(2-L)t^2+\FormalBigO_t(t^3).
\]
Then \(U=Lt+(2-L)t^2\), and
\[
 \log A=U-\frac{U^2}{2}+\FormalBigO_t(t^3)
 =Lq+\left(2-L-\frac{L^2}{2}\right)t^2+\FormalBigO_t(t^3).
\]
\end{workedbox}
...
\begin{equation}\label{eq:recip-residual-test}
 AB_N-1=\FormalBigO_t(t^N).
\end{equation}
For \(Q_N=C/A\), verify
\[
 AQ_N-C=\FormalBigO_t(t^N).
\]
...
\[
 \delta=\log(1+Lq)=Lq-\frac{L^2}{2}t^2+\FormalBigO_t(t^3).
\]
Directly,
\[
 \log G=L+\log2+\delta.
...
\[
 ...
 =\frac t2\left(1-Lq+\frac{3L^2}{2}t^2+\FormalBigO_t(t^3)\right).
\]
...
\[
 ...
 +\frac{h_3(L)}{X^3}+\FormalBigO_t(t^4).
\]
...
\begin{equation}\label{eq:leading-correction}
 F(G+H)=F(G)+C(L)X^{-N}+\FormalBigO_t(t^{N+1}).
\end{equation}
\end{lemma}
\begin{proof}
...
\]
For \(F\in\NearIdentityPolynomialLogGroup{\K}\), \(F'=1+\FormalBigO_t(t)\), while \(F''=\FormalBigO_t(t^2)\).  Thus the
linear term is \(H+\FormalBigO_t(t^{N+1})\), and all higher Taylor terms have t-order
...
\[
 R_n=F\compose G_{n-1}-X\pmod{t^{n+1}}.
\]
...
\]
\end{enumerate}
Then \(F\compose G_N=X+\FormalBigO_t(t^{N+1})\).
\end{algorithm}
...
\begin{equation}\label{eq:param-inverse-example}
 F(X)=X+\alpha L+\frac{\beta}{X}+\FormalBigO_t(t^2),
\end{equation}
...
\begin{equation}\label{eq:two-residuals}
 F\compose G_N-X=\FormalBigO_t(t^{N+1}),
 \qquad
 G_N\compose F-X=\FormalBigO_t(t^{N+1}).
\end{equation}
...
\end{equation}
Formal construction gives \(\mathcal R_N=\FormalBigO_t(t^{N+1})\).  Since the derivative
...
\begin{equation}\label{eq:multiseries-form}
 \sum_{n\ge0}\frac{\LambertPolynomial{n}(\log\log X)}{(\log X)^n}
\end{equation}
...
Verify
\[
 A\recip{A}=1+\FormalBigO_t(t^N),
 \qquad
 A(C/A)=C+\FormalBigO_t(t^N).
\]
...
Let
\begin{align*}
 N={}&1+(2L-1)t+(L^2+3)t^2+(L^3-L)t^3+\FormalBigO_t(t^4),\\
 D={}&1+(L+1)t+(2L-1)t^2+L^2t^3+\FormalBigO_t(t^4).
\end{align*}
Seek \(Q=N/D=\sum_{n=0}^3d_nt^n+\FormalBigO_t(t^4)\).  Since the leading coefficient of
...
\begin{equation}\label{eq:worked-quotient}
 ...
 +(L^3-2L^2-L-8)t^3+\FormalBigO_t(t^4).
\end{equation}
...
Take
\[
 A=1+Lq+(2-L)t^2+(L^2+1)t^3+\FormalBigO_t(t^4).
\]
Set \(U=A-1\).  Since
\[
 \log(1+U)=U-\frac{U^2}{2}+\frac{U^3}{3}+\FormalBigO_t(t^4),
\]
we find
...
\[
 \EulerE^V=1+V+\frac{V^2}{2}+\frac{V^3}{6}+\FormalBigO_t(t^4)
\]
...
\begin{equation}\label{eq:composition-commutator}
 ...
 =(Aa'-aA')X^{-1}+\FormalBigO_t(t^2).
\end{equation}
For \(a=L^2\) and \(A=L\), this becomes
\[
 F\compose G-G\compose F=L^2X^{-1}+\FormalBigO_t(t^2).
\]
...
and
\[
 G(X)=X+(1-L)+\frac{L^2}{X}+\FormalBigO_t(t^2).
\]
Thus
\[
 a=L^2-1,
...
\begin{equation}\label{eq:worked-F-perturbed}
 F(X)=X+L+\frac{L^2+1}{X}+\FormalBigO_t(t^2).
\end{equation}
Here
\[
 a=L,
...
\[
 F(G_1)-X=\left(L-\frac{L^2}{2}\right)X^{-2}+\FormalBigO_t(t^3).
\]
Also
\[
 F'(G_1)=1+\frac1{G_1}=1+X^{-1}+\FormalBigO_t(t^2).
\]
The Newton correction is therefore
\begin{align*}
 -\frac{F(G_1)-X}{F'(G_1)}
 &=-\left(L-\frac{L^2}{2}\right)X^{-2}+\FormalBigO_t(t^3)\\
 &=\frac{L(L-2)}{2X^2}+\FormalBigO_t(t^3),
\end{align*}
...
\(\BigO(u^{M+1})\) when \(u\) stays in a compact subset avoiding the branch cut and
...
\item whether \(\FormalBigO_t(t^N)\) is formal or analytic;
...
\[
 W=X-L+\sum_{n=1}^N\LambertPolynomial{n}(L)t^n+\FormalBigO_t(t^{N+1})
\]
...
\[
 \frac{U(t)^k}{k!}
 =\sum_{n\ge k}\frac1{n!}
 \Bell_{n,k}(1!u_1,2!u_2,\ldots)t^n.
\]
Therefore
...
Let
\[
 G=X+A+Bq+Ct^2+Dt^3+\FormalBigO_t(t^4)
 =X(1+u),
\]
where
\[
 u=Aq+Bt^2+Ct^3+Dt^4+\FormalBigO_t(t^5).
\]
...
\begin{align}
 ...
 +(-A^3+2AB-C)t^4+\FormalBigO_t(t^5),\label{eq:Ginv-app}\\
 G^{-2}={}&t^2-2At^3+(3A^2-2B)t^4+\FormalBigO_t(t^5),\label{eq:Ginv2-app}\\
 G^{-3}={}&t^3-3At^4+\FormalBigO_t(t^5).\label{eq:Ginv3-app}
\end{align}
...
\(G^{-1},G^{-2},G^{-3}\):
\begin{align*}
 b(L+\delta)&=b+b'\delta+\frac12b''\delta^2+\FormalBigO_t(t^3),\\
 c(L+\delta)&=c+c'\delta+\FormalBigO_t(t^2),\\
 d(L+\delta)&=d+\FormalBigO_t(t).
\end{align*}
...
\[
 F(X)=X+2L+\frac3X+\FormalBigO_t(t^2)
\]
through t-order two.
\end{exercise}
...
Thus the product is
\[
 1+3t^2+(L^3+2L)t^3+\FormalBigO_t(t^4).
\]
...
\[
 \frac1{1+(L+1)t+Lt^2}
 =1-(L+1)t+(L^2+L+1)t^2
 -(L^3+L^2+L+1)t^3+\FormalBigO_t(t^4).
\]
...
\[
 \log(1+U)=U-\frac12U^2+\FormalBigO_t(t^3),
\]
so
\[
 \log(1+U)
 =(L-1)t+
 \left(L+2-\frac{(L-1)^2}{2}\right)t^2+\FormalBigO_t(t^3).
\]
...
\[
 ...
 +\frac{-L^3+L^2}{X^2}+\FormalBigO_t(t^3).
\]
...
\[
 ...
 +\frac{4L^2-14L+6}{X^2}+\FormalBigO_t(t^3).
\]
...
\(\FormalBigO_t(t^N)\) & Formal t-adic remainder of valuation at least \(N\). \\
