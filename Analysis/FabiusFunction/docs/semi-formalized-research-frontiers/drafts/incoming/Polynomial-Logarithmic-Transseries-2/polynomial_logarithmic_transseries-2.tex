...
\newcommand{\Bell}{\operatorname{B}}
\newcommand{\rev}[1]{#1^{[-1]}}
\newcommand{\recip}[1]{#1^{\langle-1\rangle}}
\newcommand{\stirlingone}[2]{\genfrac{[}{]}{0pt}{}{#1}{#2}}
\newcommand{\PL}{\mathscr{P}}
\newcommand{\RL}{\mathscr{R}}
\newcommand{\GL}{\mathscr{G}}
...
\[
 ...
 \qquad X=\Log z+2\pi \ii k,
\]
which inverts \(w\mapsto w\e^w\) on the \(k\)-th branch.  These expansions sit
...
Lambert equation \(Y\e^Y=z\), with \(X=\log z\).  Thus
...
Ordinary power series use one small monomial, say \(q\):
\[
 a_0+a_1q+a_2q^2+\cdots.
\]
At infinity we may take \(q=X^{-1}\).  A polynomial--logarithmic series allows
the coefficient of each \(q^n\) to be a polynomial in the slower variable
\(L=\log X\):
...
\[
 A=B+\Oq(q^N),\qquad q=X^{-1},
\]
when the coefficients of \(q^n\) agree for every \(n<N\).  This is a
...
\begin{equation}\label{eq:analytic-remainder}
 ...
 =\bigO\!\left(X^{-N-1}(\log X)^{M_N}\right)
\end{equation}
...
\begin{equation}\label{eq:derivative-preview}
 \frac{\dd}{\dd X}\left(a(L)X^{-n}\right)
...
\midrule
Addition, multiplication & \(\K[L]((q))\) & \(\K[L]((q))\) \\
Differentiation & \(\K[L]((q))\) & \(q\K[L]((q))\) up to the leading power \\
Reciprocal & leading coefficient a nonzero constant & \(\K[L]((q))\) \\
General reciprocal & leading coefficient a nonzero polynomial & \(\K(L)((q))\) \\
Near-identity composition & inner series \(X+o(X)\) of controlled form & usually \(\K[L]((q))\) \\
General composition & inner leading term contains \(L^\beta\) & may require \(\log L\) \\
Near-identity reversal & \(F(X)=X+o(X)\) with polynomial--log blocks & \(\K[L]((q))\) \\
\bottomrule
\end{tabularx}
\end{center}
...
normal form is a Laurent series in \(q=X^{-1}\) with coefficients in
\(\K[L]\), where \(L=\log X\).  Formal truncation is q-adic, while analytic
meaning requires a separate remainder statement.  The variables \(q\) and
...
\begin{equation}\label{eq:qL-def}
 q=X^{-1},\qquad L=\log X.
\end{equation}
The coefficient field \(\K\) may be \(\Q\), \(\R\), or \(\C\).  For symbolic
...
\begin{equation}\label{eq:PL-ring}
 \PL_{\K}=\K[L]((q))
 =\left\{\sum_{n\ge n_0}a_n(L)q^n:
 n_0\in\Z,\ a_n\in\K[L]\right\}.
\end{equation}
Only finitely many negative powers of \(q\) are allowed, but infinitely many
nonnegative powers may occur.
\end{definition}
Thus \(X-L+L/X\) is represented as \(q^{-1}-L+Lq\).  The notation
\(\K[L]((q))\) emphasizes that the outer formal-series variable is \(q\), while
\(L\) belongs to the coefficient ring.

\section{Valuation and truncation}
For nonzero
\[
 A=\sum_{n\ge n_0}a_n(L)q^n,
 \qquad a_{n_0}\ne0,
\]
define
\begin{equation}\label{eq:qvaluation}
 ...
 \qquad \val_q(0)=+\infty.
\end{equation}
The q-adic valuation satisfies
\[
 \val_q(AB)=\val_q(A)+\val_q(B),
 \qquad
 \val_q(A+B)\ge\min\{\val_q(A),\val_q(B)\}.
\]
The congruence
\[
 A\equiv B\pmod{q^N}
\]
means \(\val_q(A-B)\ge N\).  The truncation operator is
\begin{equation}\label{eq:truncation}
 [A]_{<N}=\sum_{n_0\le n<N}a_n(L)q^n.
\end{equation}
Every algorithm below takes \(N\) as input and performs only finitely many
coefficient operations to compute \([A]_{<N}\).

\begin{proposition}[q-adic completeness]\label{prop:qcomplete}
Let \((A_j)\) be a sequence in \(\PL_\K\) such that for every \(N\), the
truncations \([A_j]_{<N}\) eventually stabilize.  Then there is a unique
\(A\in\PL_\K\) whose truncation at every order is that stable value.
\end{proposition}
\begin{proof}
For each integer \(n\), choose \(N>n\).  Eventual stabilization modulo \(q^N\)
...
Within a fixed q-block, powers of \(L\) are ordered by degree.  For monomials
\(q^nL^m=X^{-n}(\log X)^m\), the asymptotic order at \(+\infty\) is
lexicographic:
\begin{equation}\label{eq:lex-order}
 q^{n_1}L^{m_1}\succ q^{n_2}L^{m_2}
...
The block notation \(a_n(L)q^n\) deliberately groups finitely many adjacent
monomials.

Cancellation can lower the degree inside a block or remove the block entirely.
For example,
\[
 (L^2+L)q^3+(-L^2+2)q^3=(L+2)q^3.
\]
An implementation must normalize polynomial coefficients before deciding
whether a q-block vanishes.

\section{Why the polynomial ring is not a field}
The coefficient ring \(\K[L]\) has only the nonzero constants as units.  Hence
\(\PL_\K\) is a ring but not a field.

\begin{proposition}[{Units in \(\K[L]((q))\)}]\label{prop:units}
Let
\[
 A=q^r\bigl(a_0(L)+a_1(L)q+a_2(L)q^2+\cdots\bigr),
 \qquad a_0\ne0.
\]
Then \(A\) has a reciprocal in \(\PL_\K\) if and only if \(a_0(L)\) is a
nonzero constant.
\end{proposition}
\begin{proof}
The monomial \(q^r\) is invertible, so it suffices to consider \(r=0\).  If
\(AB=1\), the constant q-coefficient gives \(a_0(L)b_0(L)=1\) in
...
\begin{equation}\label{eq:rational-log-field}
 \RL_\K=\K(L)((q)).
\end{equation}
Every nonzero element of \(\RL_\K\) is invertible because \(\K(L)\) is a
field.  A smaller extension, \(\K[L,L^{-1}]((q))\), suffices when denominators
are monomials in \(L\), but not for a factor such as \(L+1\).

\begin{workedbox}
The series \(A=L+q\) has no reciprocal in \(\PL_\K\).  In \(\RL_\K\),
\[
 \frac1{L+q}
 =\frac1L\frac1{1+q/L}
 =\frac1L-\frac{q}{L^2}+\frac{q^2}{L^3}-\cdots.
\]
The failure is not caused by the infinite q-expansion; it is already present in
the leading coefficient \(L\).
\end{workedbox}

\section{Affine and monomial prefactors}
Many useful expansions do not begin at q-order zero.  Write a nonzero series as
\begin{equation}\label{eq:prefactor-form}
 A=cX^\rho L^\beta(1+U),
 \qquad U\prec1,
\end{equation}
whenever such a factorization is available.  In the strict ring
\(\PL_\K\), \(\rho\in\Z\), \(\beta\in\N\), and the factor \(L^\beta\) may
...
\item truncate q-adically, using that \(U\) has positive valuation.
\end{enumerate}
...
The Lambert polynomials satisfy \(\deg P_n=n\).  More generally, many
...
q-adic remainder into a useful analytic estimate and limits coefficient size.
...
\[
 ...
 \deg b_n\le B+\beta n
\]
for \(n\ge0\).  If \(C=AB=\sum c_nq^n\), then
\[
...
For some problems integer powers of \(q\) are too restrictive.  Let
\(S\subset\R\) be a well-ordered set with the property that every real bound
...
\[
 S\subset\lambda_0+\N\omega_1+\cdots+\N\omega_r,
 \qquad \omega_j>0,
\]
are particularly convenient.  The integer q-adic theory developed below then
...
\begin{summarybox}
The core ring is \(\PL_\K=\K[L]((q))\) with \(q=X^{-1}\) and
\(L=\log X\).  Its q-adic valuation makes truncation and convergence purely
coefficientwise.  It is not a field: a series is a unit only when its first
nonzero q-coefficient is a nonzero constant.  General division naturally leads
to \(\K(L)((q))\).  Generalized real exponents are possible, but only with
...
Let
\[
 A=\sum_{n\ge r}a_n(L)q^n,
 \qquad
 B=\sum_{n\ge s}b_n(L)q^n.
\]
After inserting zero coefficients where necessary,
\[
 A+B=\sum_{n\ge\min(r,s)}\bigl(a_n(L)+b_n(L)\bigr)q^n.
\]
...
\[
 A=q^{-1}-L+(L^2-1)q,
 \qquad
 B=-q^{-1}+L+(2-L^2)q+3q^2,
\]
then
\[
 A+B=q+3q^2,
\]
so two leading q-blocks and the quadratic logarithmic terms all cancel.
\end{example}
...
For
\[
 A=\sum_{i\ge r}a_i(L)q^i,
 \qquad
 B=\sum_{j\ge s}b_j(L)q^j,
\]
the product is
\begin{equation}\label{eq:product-convolution}
 AB=\sum_{n\ge r+s}c_n(L)q^n,
 \qquad
...
To compute modulo \(q^N\), discard every pair with \(i+j\ge N\).  No higher
...
Take
\[
 A=1+(L-1)q+(L^2+2)q^2+\Oq(q^3),
\]
\[
 B=1-(L+2)q+(3L-1)q^2+\Oq(q^3).
\]
...
If \(U\in q\K[L][[q]]\), then \(U^m\in q^m\K[L][[q]]\).  Consequently, for a
fixed cutoff \(q^N\), only \(m<N\) can contribute to a series in powers of
...
\end{equation}
Modulo \(q^N\), the sum stops at \(m=N-1\).

\begin{example}[Square root]
Let
\[
 A=1+2Lq+(L^2+1)q^2+\Oq(q^3).
\]
Using \((1+U)^{1/2}=1+U/2-U^2/8+\cdots\),
\[
 A^{1/2}=1+Lq+\frac12q^2+\Oq(q^3),
\]
because the \(L^2q^2\) contributions cancel.
\end{example}

\section{Differentiation and the shifted derivative}
Let \(\Dop=\dd/\dd L\).  Since \(L=\log X\),
\[
 \frac{\dd L}{\dd X}=X^{-1}.
\]
For a single block,
\begin{equation}\label{eq:block-derivative}
 \frac{\dd}{\dd X}\bigl(a_n(L)X^{-n}\bigr)
...
\[
 \frac{\dd}{\dd X}\left(a_n(\log X)X^{-n}\right)
...
\[
 \frac{\dd}{\dd X}\left(Q(L)X^{-n+1}\right)
...
polynomial \(\Bell_{n,r}\),
\begin{equation}\label{eq:bell-coeff}
 [q^n]U^r
 =\frac{r!}{n!}\,
 \Bell_{n,r}\bigl(1!u_1,2!u_2,\ldots,(n-r+1)!u_{n-r+1}\bigr).
\end{equation}
We will use this identity to expand \(\log(1+U)\), exponentials, and composition
systematically.

\section{Complexity at fixed truncation}
With dense q-coefficients, schoolbook multiplication through order \(N\) uses
\(O(N^2)\) polynomial products.  If the logarithmic degree is \(O(N)\), naive
...
transparent.  At high orders one may use fast convolution in \(q\), fast
...
\begin{summarybox}
Multiplication is Cauchy convolution in the q-index and ordinary polynomial
multiplication in \(L\).  Positive q-valuation makes binomial and other
...
\begin{equation}\label{eq:A-recip-general}
 A=a_0+a_1q+a_2q^2+\cdots,
 \qquad a_0\in\K^\times,
\end{equation}
and seek
\[
 \recip{A}=B=b_0+b_1q+b_2q^2+\cdots
\]
such that \(AB=1\).  Comparing q-coefficients gives
\[
 a_0b_0=1,
\]
and, for \(n\ge1\),
...
defines the unique \(B\in\K[L][[q]]\) satisfying \(AB=1\).
\end{proposition}
\begin{proof}
Existence follows by induction: once \(b_0,\ldots,b_{n-1}\) are known, the
coefficient equation at q-order \(n\) has the unique solution shown in
\cref{eq:reciprocal-recurrence}.  The resulting product has constant
coefficient one and every positive q-coefficient zero.  If \(B\) and
...
\[
 A=q^r\widetilde A,
 \qquad \val_q(\widetilde A)=0,
\]
then
\[
 \recip{A}=q^{-r}\recip{\widetilde A}.
\]

\begin{workedbox}
Let
\[
 A=1+Lq+(L^2+1)q^2+\Oq(q^4),
\]
with the q-cubic coefficient taken to be zero.  The recurrence gives
\[
 b_0=1,
 \qquad b_1=-L,
\]
...
Hence
\[
 \frac1A=1-Lq-q^2+(L^3+2L)q^3+\Oq(q^4).
\]
Direct multiplication is the quickest verification.
\end{workedbox}

\section{Geometric normalization}
When \(A=a_0(1+U)\) with \(U\in q\K[L][[q]]\), the recurrence is equivalent to
\begin{equation}\label{eq:geometric-recip}
 \frac1A=a_0^{-1}\sum_{m\ge0}(-U)^m.
\end{equation}
At precision \(q^N\), only \(m<N\) contributes.  The recurrence is usually
...
Thus
\[
 \frac CA=1+(L+2)q+(1-L)q^2+\Oq(q^3).
\]
\end{example}
...
For instance,
\[
 \frac1{L+q}=L^{-1}-L^{-2}q+L^{-3}q^2-\cdots.
\]
...
\[
 ...
 \qquad \val_q(1-AB_{\mathrm{new}})\ge2N.
\]
\end{proposition}
\begin{proof}
Substitute \(AB=1-E\):
...
\]
The valuation is multiplicative, so \(\val_q(E^2)\ge2N\).
\end{proof}
...
Let
\[
 A=1+Lq+(2-L)q^2+\Oq(q^3).
\]
Then \(U=Lq+(2-L)q^2\), and
\[
 \log A=U-\frac{U^2}{2}+\Oq(q^3)
 =Lq+\left(2-L-\frac{L^2}{2}\right)q^2+\Oq(q^3).
\]
\end{workedbox}

\section{Formal exponentials}
If \(V\in q\K[L][[q]]\), define
\begin{equation}\label{eq:formal-exp-small}
 \exp V=\sum_{m\ge0}\frac{V^m}{m!}.
\end{equation}
It lies in \(1+q\K[L][[q]]\).  At fixed q-precision, only finitely many powers
...
\[
 ...
 \log(\exp V)=V
\]
follow from the corresponding identities in the complete q-adic algebra.
...
\]
The logarithmic derivative often has higher q-valuation than \(A'\) itself and
...
\(B_N\) is intended as the reciprocal of \(A\) through order \(q^{N-1}\), verify
...
doubles q-adic precision.  Logarithms and exponentials are defined on small
q-adic perturbations, while prefactors determine whether new monomials such as
...
\begin{equation}\label{eq:normal-ordered-def}
 \mathopen{:}\e^{\delta(\Dop-n)}\mathclose{:}\,f
...
q-adically meaningful because \(\val_q(\delta^r)\ge r\).
...
\begin{equation}\label{eq:regular-composition-formula}
 ...
 \mathopen{:}\e^{\delta(\Dop-n)}\mathclose{:}\,
 f_n(L+\log\lambda).
\end{equation}
At every finite q-precision, the right side requires only finitely many terms.
\end{theorem}
\begin{proof}
For one outer block,
\[
 f_n(\log G)G^{-n}
 =\lambda^{-n}X^{-n}
 f_n(L+\log\lambda+\delta)\e^{-n\delta}.
\]
...
\]
Multiplying by
\(\e^{-n\delta}=\sum_{k\ge0}(-n)^k\delta^k/k!\) and collecting the total
power \(r=j+k\) gives
\[
...
\cref{eq:regular-composition-formula}.  To compute modulo \(q^N\), only outer
...
Write
\[
 \delta(q)=\sum_{j\ge1}\delta_j(L)q^j.
\]
...
\]
Consequently, the coefficient of q-order \(N\) in
...
\end{split}
\end{equation}
with \(\Bell_{0,0}=1\) and \(\Bell_{m,0}=0\) for \(m>0\).  This is a closed,
finite formula for every coefficient.

\begin{algorithm}[Regular composition to q-order \(N\)]\label{alg:regular-composition}
\leavevmode
\begin{enumerate}
...
\item Multiply them by \(\delta^r/r!\), the factor
      \(\lambda^{-n}q^n\), and truncate after every multiplication.
...
\]
Here \(\lambda=2\), \(u=Lq\), and
\[
 \delta=\log(1+Lq)=Lq-\frac{L^2}{2}q^2+\Oq(q^3).
\]
Directly,
\[
 \log G=L+\log2+\delta.
...
Also,
\[
 G^{-1}=\frac{q}{2}\e^{-\delta}
 =\frac q2\left(1-Lq+\frac{3L^2}{2}q^2+\Oq(q^3)\right).
\]
Therefore, through q-order two,
\begin{align*}
 F(G)
={}&L+\log2+Lq-\frac{L^2}{2}q^2\\
&+\frac q2\bigl(L+\log2+\delta\bigr)^2\e^{-\delta}
 +\Oq(q^3)\\
={}&L+\log2
 +\left[L+\frac12(L+\log2)^2\right]q\\
&+\left[-\frac{L^2}{2}
 +L(L+\log2)-\frac L2(L+\log2)^2\right]q^2
 +\Oq(q^3).
\end{align*}
...
\[
 G=X\bigl(1+Aq+Bq^2+Cq^3+Dq^4+\cdots\bigr).
\]
Thus
\begin{equation}\label{eq:delta-near-identity}
\begin{split}
 \delta=\log(G/X)
 ={}&Aq+\left(B-\frac{A^2}{2}\right)q^2
 +\left(C-AB+\frac{A^3}{3}\right)q^3\\
 &+\left(D-AC-\frac{B^2}{2}+A^2B-\frac{A^4}{4}\right)q^4
 +\Oq(q^5).
\end{split}
\end{equation}
...
Thus
\[
 F(G)=X+\left(L-\frac{L^2}{2}\right)X^{-2}+\bigO(X^{-3}).
\]
Adding
\[
...
For a near-identity substitution \(G=X+H\) with \(H/X\in q\K[L][[q]]\), one
may also write
\begin{equation}\label{eq:formal-taylor-composition}
 F(X+H)=\sum_{r\ge0}\frac{F^{(r)}(X)}{r!}H^r.
\end{equation}
This is q-adically meaningful for polynomial--logarithmic \(F\): each
derivative of a block raises its q-order by one, while \(H\) has q-valuation at
least zero, so the r-th Taylor term gains at least r q-orders relative to the
same outer block.

\begin{proposition}[Taylor formula in the near-identity class]\label{prop:taylor-composition}
If \(F\in\PL_\K\) and \(H/X\in q\K[L][[q]]\), then
...
chain rule \cref{eq:repeated-derivative}.  Modulo \(q^N\), only finitely many
...
\]
These statements may be checked at every finite q-precision, where all
...
\end{equation}
Again, the proof holds blockwise and then extends by q-adic completeness.
...
integer, generalized q-exponents are required, but no new logarithmic level
appears.
...
creates \(\log L=\log\log X\).  The one-level ring \(\K[L]((q))\) is therefore
...
\item the substituted support may cease to be well ordered in the asymptotic
      direction.
\end{enumerate}
The q-adic class with \(G\sim\lambda X\) avoids these problems because, below
any cutoff, only finitely many outer q-blocks and Taylor orders contribute.
...
\item Do the output q-exponents remain in the chosen support grid?
...
\begin{equation}\label{eq:near-id-group}
 ...
 \right\}.
\end{equation}
Every \(F\in\GL_\K\) satisfies \(F/X=1+o(1)\), so the composition formulas of
...
\]
For \(F\in\GL_\K\), \(F'=1+\Oq(q)\), while \(F''=\Oq(q^2)\).  Thus the
linear term is \(H+\Oq(q^{N+1})\), and all higher Taylor terms have q-order
strictly greater than \(N\).
\end{proof}

\begin{theorem}[Existence and uniqueness of near-identity reversal]\label{thm:reversion-exists}
Every \(F\in\GL_\K\) has a unique compositional inverse
\(\rev{F}\in\GL_\K\).
\end{theorem}
\begin{proof}
Construct
\[
 G_N=X+A_0(L)+A_1(L)q+\cdots+A_N(L)q^N
\]
inductively so that
\[
 F\compose G_N=X+\Oq(q^{N+1}).
\]
At order zero, choose \(A_0\) to cancel the q-zero residual.  Suppose the
coefficients through \(A_{N-1}\) have been chosen and
\[
 F\compose G_{N-1}=X+r_N(L)q^N+\Oq(q^{N+1}).
\]
By \cref{lem:leading-correction}, replacing \(G_{N-1}\) by
\(G_N=G_{N-1}-r_Nq^N\) cancels the residual at order \(N\) without altering
lower orders.  Q-adic completeness produces a formal right inverse \(G\).

For uniqueness, if \(G\) and \(\widetilde G\) first differ by
\(C(L)q^N\), then \cref{lem:leading-correction} shows that
...
To compute \(G=\rev{F}\) through q-order \(N\):
\begin{enumerate}
\item Start with \(G_{-1}=X\).
\item For \(n=0,1,\ldots,N\), compute the residual
\[
 R_n=F\compose G_{n-1}-X\pmod{q^{n+1}}.
\]
\item Extract its q-order \(n\) coefficient \(r_n(L)=[q^n]R_n\).
\item Set
\[
 G_n=G_{n-1}-r_n(L)q^n.
\]
\end{enumerate}
Then \(F\compose G_N=X+\Oq(q^{N+1})\).
\end{algorithm}
...
\begin{equation}\label{eq:param-inverse-example}
 F(X)=X+\alpha L+\frac{\beta}{X}+\Oq(q^2),
\end{equation}
...
\begin{equation}\label{eq:F-x-plus-h}
 ...
 \qquad h(X)=o(X).
\end{equation}
For our core class, \(h\in\K[L][[q]]\): it begins at q-order zero, but is small
relative to \(X=q^{-1}\).
...
\begin{equation}\label{eq:lagrange-infinity}
 ...
 \left(\frac{\dd}{\dd X}\right)^{m-1}\!\bigl(h(X)^m\bigr).
\end{equation}
The m-th summand has q-valuation at least \(m-1\), so only finitely many
summands contribute at any fixed precision.
\end{theorem}
\begin{proof}
Introduce a formal parameter \(t\) and consider
\(F_t(Y)=Y+t h(Y)\).  The classical Lagrange--Bürmann formula for the inverse
of a perturbation of the identity gives
\[
 F_t^{[-1]}(X)
 =X+\sum_{m\ge1}\frac{(-t)^m}{m!}
 \left(\frac{\dd}{\dd X}\right)^{m-1}h(X)^m.
\]
For polynomial--logarithmic \(h\), each derivative raises q-order by at least
one, so the coefficient of any fixed q-power is a finite polynomial in \(t\).
Setting \(t=1\) is therefore formally legitimate and yields
...
\]
Since the denominator is a unit, \(\val_q(\Delta)\ge N\).  Taylor expansion
around \(G\) gives
\[
 F(G+\Delta)-X
 =E+(F'\compose G)\Delta
...
r at least two, hence has q-valuation at least \(2N\); in fact the derivatives
...
\begin{enumerate}
\item determine the q-zero correction by residual lifting;
...
\(U\).  Fractional q-powers generally appear.
...
through q-order three follow from the composition formulas.  The
Lagrange--Bürmann identity
\[
 \rev{(X+h)}=X+\sum_{m\ge1}\frac{(-1)^m}{m!}
 \left(\frac{\dd}{\dd X}\right)^{m-1}h^m
\]
...
\begin{equation}\label{eq:Lambert-def}
 ...
 \qquad k\in\Z.
\end{equation}
The principal branch is \(W_0\).  On the real interval \((-\e^{-1},0)\), there
is a second real branch \(W_{-1}\).  The branch geometry, singularity at
\(-\e^{-1}\), numerical evaluation, and standard asymptotic formulas are
...
\begin{equation}\label{eq:W-fixed-point}
 R=-\log\bigl(1-q(L-R)\bigr),
 \qquad q=X^{-1}.
\end{equation}
This single equation determines all Lambert polynomials \(P_n\).

\begin{proposition}[Triangular Lambert recurrence]\label{prop:Lambert-triangular}
Set \(R_{n-1}=\sum_{j=1}^{n-1}P_j(L)q^j\).  Then
\begin{equation}\label{eq:Lambert-triangular-recurrence}
 P_n(L)=[q^n]\left[-\log\bigl(1-q(L-R_{n-1})\bigr)\right].
\end{equation}
The right side involves only \(P_1,\ldots,P_{n-1}\).
\end{proposition}
\begin{proof}
In \(q(L-R)\), the occurrence of \(P_nq^n\) is shifted to q-order \(n+1\).
Therefore the coefficient of \(q^n\) on the right of
...
\[
 R=q(L-R)+\frac{q^2}{2}(L-R)^2
   +\frac{q^3}{3}(L-R)^3+\cdots.
\]
At q-order one,
\[
 P_1=L.
...
At q-order three,
\[
 P_3=-P_2-LP_1+\frac{L^3}{3}
 =\frac{L(2L^2-9L+6)}6.
\]
A further collection gives
\[
 P_4=\frac{L(3L^3-22L^2+36L-12)}{12}.
\]
\end{workedbox}
...
\begin{equation}\label{eq:W-lagrange}
 W(\e^X)
 =X+\sum_{m\ge1}\frac{(-1)^m}{m!}
 \left(\frac{\dd}{\dd X}\right)^{m-1}L^m.
\end{equation}
...
\begin{equation}\label{eq:Pn-operator}
 P_n(L)=\frac{(-1)^{n+1}}{(n+1)!}
 \prod_{j=0}^{n-1}(\Dop-j)\,L^{n+1},
 \qquad \Dop=\frac{\dd}{\dd L}.
\end{equation}
This formula computes \(P_n\) independently of all earlier polynomials.
...
\[
 \prod_{r=0}^{n-1}(t-r)=\sum_{j=0}^ns(n,j)t^j,
 \qquad
...
\begin{equation}\label{eq:W-all-orders}
 ...
 \stirlingone{n}{n-m+1}\frac{L^m}{m!},
\end{equation}
where \(X=\Log z+2\pi\ii k\) and \(L=\log X\).
\end{theorem}
\begin{proof}
...
\(y^\alpha\e^y\) and associated Stirling numbers
...
For \(n\ge1\):
\begin{enumerate}
\item \(P_n\) has degree \(n\) and is divisible by \(L\);
...
P_1={}&L,\\
P_2={}&\frac{L(L-2)}2,\\
P_3={}&\frac{L(2L^2-9L+6)}6,\\
P_4={}&\frac{L(3L^3-22L^2+36L-12)}{12},\\
P_5={}&\frac{L(12L^4-125L^3+350L^2-300L+60)}{60},\\
P_6={}&\frac{L}{120}\bigl(20L^5-274L^4+1125L^3
          -1700L^2+900L-120\bigr),\\
P_7={}&\frac{L}{840}\bigl(120L^6-2058L^5+11368L^4-25725L^3\\[-0.2ex]
&\hspace{7.4em}{}+24500L^2-8820L+840\bigr),\\
P_8={}&\frac{L}{2520}\bigl(315L^7-6534L^6+45962L^5-142149L^4\\[-0.2ex]
&\hspace{7.4em}{}+205800L^3-135240L^2+35280L-2520\bigr),\\
P_9={}&\frac{L}{2520}\bigl(280L^8-6849L^7+59062L^6-235494L^5\\[-0.2ex]
&\hspace{7.4em}{}+471429L^4-476280L^3+229320L^2\\[-0.2ex]
&\hspace{7.4em}{}-45360L+2520\bigr),\\
P_{10}={}&\frac{L}{10080}\bigl(1008L^9-28516L^8+293175L^7
 -1447360L^6\\[-0.2ex]
...
series for sufficiently large \(|z|\), and convergence on the principal real
branch for \(z\ge\e\) \cite{dlmf-lambert}.  The formal derivation remains useful
...
\(W_N=X-L+\sum_{n=1}^NP_n(L)X^{-n}\) on the principal real branch.}
\label{tab:W-errors}
\begin{tabular}{@{}rccc@{}}
\toprule
...
\end{equation}
Formal construction gives \(\mathcal R_N=\Oq(q^{N+1})\).  Since the derivative
of \(w+\log w-X\) is \(1+1/w\), one Newton correction is
\begin{equation}\label{eq:W-newton-correction}
 W_N^{\mathrm{new}}
 =W_N-\frac{W_N+\log W_N-X}{1+1/W_N}.
\end{equation}
As a formal series, this doubles the q-adic precision.  Numerically, the
residual also provides an inexpensive a posteriori diagnostic.  On a domain
where \(|1+1/w|\) is bounded away from zero, the true error is approximately
\(-\mathcal R_N/(1+1/W_N)\).

One may instead use the original residual \(W_N\e^{W_N}-z\), but
...
\begin{equation}\label{eq:Wm1-expansion}
 W_{-1}(x)
 \sim -T-L+\sum_{n\ge1}\frac{(-1)^nP_n(L)}{T^n}.
\end{equation}
Thus
...
\begin{equation}\label{eq:alpha-log-inversion}
 ...
 \qquad \alpha\ne0.
\end{equation}
Exponentiating gives \(y^\alpha\e^y=\e^X\), and an exact solution is
\begin{equation}\label{eq:alpha-exact-W}
 y=\alpha W\!\left(\frac{\e^{X/\alpha}}{\alpha}\right),
\end{equation}
...
\begin{equation}\label{eq:alpha-expansion}
 y=X-\alpha L+
 \sum_{n\ge1}\frac{\alpha^{n+1}P_n(L)}{X^n}.
\end{equation}
The first terms are
...
\item set \(X=\Log z+2\pi\ii k\) using a specified base branch of \(\Log\);
...
\(X=\Log z+2\pi\ii k\).  Its coefficients satisfy a triangular fixed-point
recurrence, an all-orders differential formula, and the closed Stirling-number
identity
\[
 P_n(L)=\sum_{m=1}^n(-1)^{n+m}
 \stirlingone{n}{n-m+1}\frac{L^m}{m!}.
\]
...
The core ring \(\K[L]((q))\) is deliberately economical.  Its limitations are
...
\begin{equation}\label{eq:iterated-log-derivatives}
 \frac{\dd L_1}{\dd X}=\frac1X,
 \qquad
 \frac{\dd L_2}{\dd X}=\frac1{XL_1},
 \qquad
 \frac{\dd L_3}{\dd X}=\frac1{XL_1L_2},
\end{equation}
and in general
...
\begin{equation}\label{eq:y-over-log-expansion}
 y\sim X\left[
 T+\Lambda+\sum_{n\ge1}\frac{(-1)^{n+1}P_n(\Lambda)}{T^n}
 \right].
\end{equation}
The first terms are
...
The ring \(\K[L]((q))\) has three strong virtues:
\begin{enumerate}
\item every coefficient operation is finite polynomial algebra;
\item q-adic truncation makes recursive algorithms elementary;
...
\begin{equation}\label{eq:Dulac-series}
 \widehat f(x)=\sum_{j\ge0}x^{\lambda_j}P_j(\ell),
 \qquad
...
Since \(\ell=-\log x\),
\[
 \frac{\dd\ell}{\dd x}=-\frac1x.
\]
Therefore
\begin{equation}\label{eq:Dulac-derivative}
 \frac{\dd}{\dd x}\bigl(P(\ell)x^\lambda\bigr)
...
\begin{equation}\label{eq:Dulac-repeated-derivative}
 \frac{\dd^r}{\dd x^r}\bigl(P(\ell)x^\lambda\bigr)
...
identical to \cref{eq:reciprocal-recurrence}, with \(x\) replacing \(q\).
...
\[
 ...
 \qquad
 g^n=\lambda^nx^n\e^{n\eta}.
\]
Consequently,
\begin{equation}\label{eq:small-normal-ordered}
 f\compose g
 =\sum_n\lambda^nx^n
 \mathopen{:}\e^{-\eta(\Dop-n)}\mathclose{:}
 f_n(\ell-\log\lambda),
\end{equation}
...
\begin{equation}\label{eq:small-Lagrange}
 ...
 \left(\frac{\dd}{\dd x}\right)^{m-1}h(x)^m,
\end{equation}
provided \(h=o(x)\) and the support conditions make each truncation finite.
...
The second is
\[
 \frac12\frac{\dd}{\dd x}h^2
 =\frac{a^2}{2}\frac{\dd}{\dd x}(x^4\ell^2)
 =a^2x^3(2\ell^2-\ell).
\]
...
gaps occur.  For the integer q-adic core, either representation works.
...
the input precisions.  If both inputs are known modulo \(q^N\) and have
nonnegative valuation, their sum and product are known modulo \(q^N\).  If
...
finite---usually only the block \(q^{-1}\)---so guard precision is easy to
calculate.

\section{Core pseudocode}
The following routines assume integer q-exponents and exact coefficient
arithmetic.
...
\[
 G=X+A_0+A_1q+A_2q^2+\cdots,
\]
form
\begin{equation}\label{eq:implementation-u}
 u=qG-1=A_0q+A_1q^2+A_2q^3+\cdots
\end{equation}
...
function reverse_triangular(F, N):
    ...
    for n from 0 to N-1:
        ...
        r_n := coefficient(R, q^n)
        G := G - r_n*q^n
    return G
\end{lstlisting}
At the end of the n-th pass, the residual vanishes below q-order \(n+1\).
...
\end{lstlisting}
The current approximation \(G\) should already be correct to positive q-order.
If it is correct modulo \(q^M\), request the new result modulo
\(q^{\min(2M,N)}\).
...
\(q^n=X^{-n}\).  It is intended as transparent reference code rather than a
...
def shift_q(a: Series, s: int, cutoff: int) -> Series:
...
def reciprocal_unit(a: Series, cutoff: int) -> Series:
    ...
    if a0 == 0:
        raise ValueError("q^0 coefficient must be nonzero")
    b: Series = {0: 1/a0}
...
def log1p(u: Series, cutoff: int) -> Series:
    if u and min(u) <= 0:
        raise ValueError("u must have positive q-valuation")
    out: Series = {}
...
def near_identity_u(g: Series, cutoff: int) -> Series:
    """For G=X+..., return u=G/X-1=q*G-1."""
    u = shift_q(g, 1, cutoff + 1)
    u[0] = u.get(0, 0) - 1
...
Generate \(P_n\) by three independent methods:
\begin{enumerate}
...
For hundreds or thousands of q-orders, the following improvements matter:
\begin{itemize}
\item Newton reciprocal and reversion instead of coefficientwise recurrences;
\item FFT-based multiplication in q and in \(L\);
...
\item whether the truncation is blockwise in q or monomialwise in q and \(L\);
...
because it determines what the displayed \(\bigO\)-term actually means.

\begin{summarybox}
A polynomial--logarithmic series can be implemented as a sparse map from
q-exponents to normalized coefficient polynomials.  Precision and coefficient
...
Let
\begin{align*}
 N={}&1+(2L-1)q+(L^2+3)q^2+(L^3-L)q^3+\Oq(q^4),\\
 D={}&1+(L+1)q+(2L-1)q^2+L^2q^3+\Oq(q^4).
\end{align*}
Seek \(Q=N/D=\sum_{n=0}^3d_nq^n+\Oq(q^4)\).  Since the leading coefficient of
...
\begin{equation}\label{eq:worked-quotient}
 \frac ND=1+(L-2)q+(6-L)q^2
 +(L^3-2L^2-L-8)q^3+\Oq(q^4).
\end{equation}
...
Take
\[
 A=1+Lq+(2-L)q^2+(L^2+1)q^3+\Oq(q^4).
\]
Set \(U=A-1\).  Since
\[
 \log(1+U)=U-\frac{U^2}{2}+\frac{U^3}{3}+\Oq(q^4),
\]
we find
\begin{equation}\label{eq:worked-log}
\begin{split}
 \log A={}&Lq+
 \left(2-L-\frac{L^2}{2}\right)q^2\\
 &+\left(2L^2-L-2+\frac{L^3}{3}\right)q^3
 +\Oq(q^4).
\end{split}
\end{equation}
Let the right side be \(V\).  Expanding
\[
 \e^V=1+V+\frac{V^2}{2}+\frac{V^3}{6}+\Oq(q^4)
\]
...
\[
 F\compose G-G\compose F=L^2X^{-1}+\Oq(q^2).
\]
...
Let
\[
 F(Y)=Y+(\log^2Y-1)+\frac{2\log Y}{Y}+\frac1{Y^2}+\bigO(Y^{-3})
\]
and
\[
 G(X)=X+(1-L)+\frac{L^2}{X}+\Oq(q^2).
\]
Thus
\[
 a=L^2-1,
...
\(\dd M/\dd X=1/X\), the Lagrange formula gives
\begin{equation}\label{eq:constant-log-shift-inverse}
 \rev F(X)
 =X-\alpha M+
 \sum_{n\ge1}\frac{\alpha^{n+1}P_n(M)}{X^n}.
\end{equation}
Equivalently,
\[
 \rev F(X)=X-c-\alpha L
...
\[
 F(G_1)-X=\left(L-\frac{L^2}{2}\right)X^{-2}+\Oq(q^3).
\]
Also
\[
 F'(G_1)=1+\frac1{G_1}=1+X^{-1}+\Oq(q^2).
\]
The Newton correction is therefore
\begin{align*}
 -\frac{F(G_1)-X}{F'(G_1)}
 &=-\left(L-\frac{L^2}{2}\right)X^{-2}+\Oq(q^3)\\
 &=\frac{L(L-2)}{2X^2}+\Oq(q^3),
\end{align*}
which inserts \(P_2/X^2\).  If the residual and denominator are carried to
...
\[
 \log\frac1g=\ell+a x\ell+\bigO(x^2\ell^2).
\]
Also
\[
 g^2=x^2-2a x^3\ell+\bigO(x^4\ell^2).
\]
Therefore
\begin{align*}
 f(g)&=g+a g^2\log(1/g)\\
...
\item Make a dependency table: which outer blocks and derivative orders can
      reach each q-order?
\item Keep each q-block as a polynomial in \(L\) until the end.
\item Normalize and collect after every q-order, not after every elementary
      multiplication.
...
\(L^{r-s}/X\to0\).  Thus a finite polynomial q-block is asymptotically
well separated from the next q-block.

If logarithmic degrees grow with q-order, a blockwise statement such as
...
are controlled at the next q-order.  Then
\[
...
If \(u(X)=o(1)\), then for sufficiently large positive \(X\), or uniformly in
a suitable complex sector,
\[
 \log(1+u)=u-\frac{u^2}{2}+\frac{u^3}{3}-\cdots.
\]
The analytic Taylor remainder after \(M\) terms is
\(O(u^{M+1})\) when \(u\) stays in a compact subset avoiding the branch cut and
\(-1\).  Since positive q-valuation implies \(u^M\) gains at least M q-orders,
...
Let \(G(X)=\lambda X(1+u(X))\), where \(u=o(1)\), and suppose the outer
...
\[
 F'(y)=1+\frac1y>1.
\]
Hence the absolute residual \(|y_N+\log y_N-X|\) is already an upper bound for
...
\end{theorem}
\begin{proof}
Because \(F'(y)=1+o(1)\), there is an eventual interval on which
\(|F'(y)|\ge1/2\).  Both \(G(X)\) and \(G_N(X)\) lie in that interval for large
...
In the complex plane, a lower bound on \(|F'|\) along a line segment does not by
...
new scales such as \(\e^{-X}\).  Different analytic sectors can then differ by
...
\end{enumerate}
At very large \(X\), evaluating a polynomial \(P_n(L)\) by Horner's rule is
...
\item whether \(\Oq(q^N)\) is formal or analytic;
\item any logarithmic factor hidden in an analytic \(\bigO\)-bound;
\item whether the series is known to converge or is only asymptotic.
\end{enumerate}
For example,
\[
 W_0(z)=X-L+\sum_{n=1}^N\frac{P_n(L)}{X^n}
 +\bigO\!\left(\frac{L^{N+1}}{X^{N+1}}\right),
\]
with \(X=\log z\), \(L=\log X\), and \(z\to+\infty\), is an analytic
statement.  The notation
\[
 W=X-L+\sum_{n=1}^NP_n(L)q^n+\Oq(q^{N+1})
\]
...
is bounded away from zero.  Near-identity inverses with \(F'=1+o(1)\) inherit
...
\[
 \frac{\dd}{\dd X}\bigl(a(L)X^{-n}\bigr)
 =(a'-na)X^{-n-1}
\]
...
\]
not merely \(L\).  A correct implementation never substitutes only the q-part
of an inner series.

\section{Assuming the polynomial coefficient ring is a field}
The reciprocal of \(L+q\) begins with \(L^{-1}\), so it is not in
\(\K[L]((q))\).  If a division unexpectedly produces denominators in \(L\),
this may be mathematically necessary rather than a simplification failure.

Perform the unit test before recurrence:
\begin{quote}
Is the first nonzero q-coefficient a nonzero constant in the chosen coefficient
ring?
\end{quote}
If not, enlarge the domain or keep the factor unexpanded.

\section{Truncating without guard terms}
Suppose an outer series contains \(q^{-1}\).  To compute a product modulo
\(q^N\), the other factor may be needed through q-order \(N\), not merely
\(N-1\), because \(q^{-1}q^N=q^{N-1}\).  Similar shifts occur in composition.
...
\(\delta=\delta(L,q)\), because \(\Dop\delta\ne0\).  A naive symbolic operator
...
\[
 \sum_{r\ge0}\frac{\delta^r}{r!}(\Dop-n)^rf.
\]
It is unambiguous and finite at fixed q-precision.
...
\]
Trying to force the result back into \(\K[L]((q))\) will either fail or silently
...
\[
 A=B+\Oq(q^N)
\]
means coefficient agreement below q-order \(N\).  It does not by itself assert
an analytic bound with a constant.  Conversely,
\[
 A(X)-B(X)=\bigO(X^{-N}L^M)
\]
is an analytic statement and may hide a nontrivial logarithmic exponent.
Always label the formal q-adic remainder distinctly.
...
\(z=-\e^{-1}\), where \(W\) has a square-root branch point.  There the natural
small variable is
\[
 p=\sqrt{2(\e z+1)},
\]
...
\(X=\Log z+2\pi\ii k\) intact rather than simplifying it through identities
...
For Lambert \(W(z)\) at infinity, the fast coordinate is \(X=\Log z\), not
...
\[
 F\compose G
 =\sum_n\lambda^{-n}X^{-n}
 \mathopen{:}\e^{\log(1+u)(\Dop-n)}\mathclose{:}
 f_n(L+\log\lambda).
\]
...
\(\Bell_{n,k}(x_1,x_2,\ldots)\) are defined by
...
\[
 \Bell_{0,0}=1,
 \qquad
 \Bell_{n,0}=0\quad(n>0).
\]

Their exponential generating function is
\begin{equation}\label{eq:bell-egf}
 \frac1{k!}\left(\sum_{j\ge1}x_j\frac{t^j}{j!}\right)^k
 =\sum_{n\ge k}\Bell_{n,k}(x_1,x_2,\ldots)\frac{t^n}{n!}.
\end{equation}
This identity is exactly what is needed for powers of a q-series.
...
\begin{equation}\label{eq:bell-recurrence}
 \Bell_{n,k}
 =\sum_{j=1}^{n-k+1}
 \binom{n-1}{j-1}x_j\Bell_{n-j,k-1}.
\end{equation}
...
\[
 \frac{U(q)^k}{k!}
 =\sum_{n\ge k}\frac1{n!}
 \Bell_{n,k}(1!u_1,2!u_2,\ldots)q^n.
\]
Therefore
\begin{equation}\label{eq:bell-U-power}
 [q^n]U^k
 =\frac{k!}{n!}\Bell_{n,k}(1!u_1,2!u_2,\ldots).
\end{equation}
...
\begin{equation}\label{eq:bell-log-coeff}
 ...
 \Bell_{n,k}(1!u_1,2!u_2,\ldots).
\end{equation}
Similarly, if \(V=\sum_{j\ge1}v_jq^j\),
\begin{equation}\label{eq:bell-exp-coeff}
 [q^n]\e^V
 =\frac1{n!}\sum_{k=0}^n
 \Bell_{n,k}(1!v_1,2!v_2,\ldots).
\end{equation}
...
\begin{equation}\label{eq:bell-normal-block}
 ...
 \right)
 =\frac1{m!}\sum_{r=0}^m
 \Bell_{m,r}(1!\delta_1,2!\delta_2,\ldots)
 (\Dop-n)^rf.
\end{equation}
After multiplying by \(\lambda^{-n}q^n\) and summing over outer blocks, one
...
\begin{equation}\label{eq:faa-di-bruno}
 \frac{\dd^n}{\dd x^n}f(g(x))
 =\sum_{k=1}^nf^{(k)}(g(x))
 \Bell_{n,k}\bigl(g'(x),g''(x),\ldots,g^{(n-k+1)}(x)\bigr).
\end{equation}
...
Let
\[
 G=X+A+Bq+Cq^2+Dq^3+\Oq(q^4)
 =X(1+u),
\]
where
\[
 u=Aq+Bq^2+Cq^3+Dq^4+\Oq(q^5).
\]
The logarithmic increment is
\begin{equation}\label{eq:appendix-delta}
\begin{split}
 \delta={}&Aq+\left(B-\frac{A^2}{2}\right)q^2
 +\left(C-AB+\frac{A^3}{3}\right)q^3\\
 &+\left(D-AC-\frac{B^2}{2}+A^2B-\frac{A^4}{4}\right)q^4
 +\Oq(q^5).
\end{split}
\end{equation}
The reciprocal powers needed through q-order three are
\begin{align}
 G^{-1}={}&q-Aq^2+(A^2-B)q^3
 +(-A^3+2AB-C)q^4+\Oq(q^5),\label{eq:Ginv-app}\\
 G^{-2}={}&q^2-2Aq^3+(3A^2-2B)q^4+\Oq(q^5),\label{eq:Ginv2-app}\\
 G^{-3}={}&q^3-3Aq^4+\Oq(q^5).\label{eq:Ginv3-app}
\end{align}
...
\(G^{-1},G^{-2},G^{-3}\):
\begin{align*}
 b(L+\delta)&=b+b'\delta+\frac12b''\delta^2+\Oq(q^3),\\
 c(L+\delta)&=c+c'\delta+\Oq(q^2),\\
 d(L+\delta)&=d+\Oq(q).
\end{align*}
...
\]
At q-order zero, only \(G\) and \(a\) contribute, giving
\[
 h_0=A+a.
\]
At q-order one, contributions are
\[
 B+Aa'+b,
\]
which gives \cref{eq:h1}.  At q-order two, use
\cref{eq:Ginv-app,eq:Ginv2-app} and the Taylor expansions to obtain
\[
 h_2=-\frac12A^2a'+\frac12A^2a''-Ab+Ab'+Ba'+C+c.
\]
At q-order three, the contributions are:
\begin{itemize}
\item \(D\) from \(G\);
\item the q-cubic Taylor block of \(a(L+\delta)\);
\item the product of \(b(L+\delta)\) with \(G^{-1}\) through q-cubic order;
\item the product of \(c(L+\delta)\) with \(G^{-2}\) through q-cubic order;
...
Compute through q-order three:
\[
 (1+Lq+q^2)(1-Lq+(L^2+2)q^2+Lq^3).
\]
\end{exercise}

\begin{exercise}[Reciprocal]
Find \((1+(L+1)q+Lq^2)^{-1}\) through q-order three.
\end{exercise}
...
Compute
\[
 \log\left(1+(L-1)q+(L+2)q^2\right)
\]
through q-order two.
\end{exercise}
...
\[
 ...
 G(X)=X-L.
\]
Compute \(F\compose G\) through q-order two.
\end{exercise}
...
coefficient of \(X^{-n}\) is \(\alpha^{n+1}P_n(L)\).
\end{exercise}

\begin{exercise}[Lambert polynomial]
Compute \(P_4(L)\) from the Stirling formula, using
\[
 \stirlingone{4}{4}=1,
...
The q coefficient vanishes.  The q-squared coefficient is
\[
 (L^2+2)-L^2+1=3.
\]
The q-cubic coefficient is
\[
 L+L(L^2+2)-L=L^3+2L.
\]
Thus the product is
\[
 1+3q^2+(L^3+2L)q^3+\Oq(q^4).
\]
...
Put \(U=(L-1)q+(L+2)q^2\).  Then
\[
 \log(1+U)=U-\frac12U^2+\Oq(q^3),
\]
so
\[
 \log(1+U)
 =(L-1)q+
 \left(L+2-\frac{(L-1)^2}{2}\right)q^2+\Oq(q^3).
\]
...
two-log-level coefficient class, not in \(\K[L]((q))\).
...
\[
 ...
 \left(\frac{\dd}{\dd X}\right)^{m-1}L^m.
\]
It has exactly q-order \(m-1\).  Put \(n=m-1\) and compare with
\cref{eq:Pn-operator}; the coefficient is \(\alpha^{n+1}P_n(L)\).
...
the second term:
\[
 \frac12(h^2)'=a^2x^3(2\ell^2-\ell)+\bigO(x^4\ell^2).
\]
Thus
\[
 \rev f(x)=x-a x^2\ell+
...
\(q=X^{-1}\) & Small q-adic variable. \\
\(L=\log X\) & First slow logarithmic variable. \\
\(L_j\) & Iterated logarithm, \(L_{j+1}=\log L_j\). \\
\(\PL_\K\) & Polynomial--logarithmic Laurent ring \(\K[L]((q))\). \\
\(\RL_\K\) & Rational-log coefficient field \(\K(L)((q))\). \\
\(\val_q A\) & Lowest q-exponent of a nonzero series. \\
\([A]_{<N}\) & Truncation to q-exponents below \(N\). \\
\(\Oq(q^N)\) & Formal q-adic remainder of valuation at least \(N\). \\
\(\Dop\) & Derivative \(\dd/\dd L\). \\
...
\(G=\lambda X(1+u)\). \\
\(\mathopen{:}\e^{\delta(\Dop-n)}\mathclose{:}\) & Normal-ordered shifted
Taylor operator. \\
\(\Bell_{n,k}\) & Partial exponential Bell polynomial. \\
\(\stirlingone{n}{k}\) & Unsigned Stirling number of the first kind. \\
\(P_n(L)\) & n-th Lambert polynomial in the expansion of \(W\). \\
\(X=\xi_k\) & Branch coordinate \(\Log z+2\pi\ii k\) for \(W_k(z)\). \\
\bottomrule
\end{longtable}
...
``On the inversion of $y^\alpha e^y$ in terms of associated Stirling
numbers,''
