...
\[
  \Phi_{q,N}(z)=\prod_{j=1}^{N}\sinc(q^jz),\qquad 0<q<1,
\]
we prove that the minimal order over $\C(z)$ of a homogeneous linear
...
$\Phi_b(z)=\prod_{j\ge1}\sinc(z/b^j)$, $b\in\{2,3,\ldots\}$, the familiar
...
finite trigonometric rational field $\C(z,\e^{\ii z/2^M})$, although each finite
...
We use the bounded Fabius function $F:\R\to[0,1]$, clamped to $0$ on
...
\begin{equation}
  \upf(t)=F(t+1)\quad(-1\le t\le0),\qquad
  \upf(t)=F(1-t)\quad(0\le t\le1).
  \label{eq:F-up}
\end{equation}
...
\end{equation}
whose density is $\upf$ in the chosen scaling.  Its characteristic function is
\begin{equation}
 \Phi(z)=\E\e^{\ii zX}
 =\prod_{j=1}^{\infty}\sinc\!\left(\frac{z}{2^j}\right),
 \qquad \sinc w:=\begin{cases}\sin w/w,&w\ne0,\\1,&w=0.\end{cases}
 \label{eq:dyadic-infinite-product}
\end{equation}
The product converges locally uniformly on $\C$ and hence defines an entire
function.
...
\begin{equation}
 ...
 \qquad
 \Phi_{q,N}(z)=\E\e^{\ii zX_{q,N}}
 =\prod_{j=1}^{N}\sinc(q^jz).
...
The dyadic densities converge to $\upf$ in the standard senses.
...
\begin{equation}
 ...
 \qquad \Prob(\eps_j=1)=\Prob(\eps_j=-1)=\frac12.
...
A germ $f$ is \emph{D-finite over $\C(z)$} if there are polynomials
$p_0,\ldots,p_r\in\C[z]$, with $p_r\ne0$, such that
\begin{equation}
 p_r(z)f^{(r)}(z)+\cdots+p_1(z)f'(z)+p_0(z)f(z)=0.
 \label{eq:Dfinite}
\end{equation}
Equivalently, the $\C(z)$-vector space spanned by
...
\end{equation}
Since
$\sin x=(2\ii)^{-1}\sum_{\eps=\pm1}\eps\e^{\ii\eps x}$,
\begin{equation}
 Y_{q,N}(z)=\frac{q^{-N(N+1)/2}}{(2\ii)^N}
...
\begin{equation}
 ...
 \prod_{j=1}^{N}\eps_j,
 \qquad
 \Omega^*_{q,N}:=\set{\omega:c_{q,N}(\omega)\ne0},
 \qquad r_N(q):=\abs{\Omega^*_{q,N}}.
...
$\e^{\lambda_1z},\ldots,\e^{\lambda_hz}$ are linearly independent over
$\C(z)$.
\end{lemma}
\begin{proof}
...
relation
\begin{equation}
 \sum_{j=1}^{h}p_j(z)\e^{\lambda_jz}=0,
 \qquad p_j\in\C[z].
...
$p_h(z)\e^{\lambda_hz}$.  For $j<h$ it produces
\[
 \e^{\lambda_jz}(\D+\lambda_j-\lambda_h)^Mp_j(z).
\]
Because $\lambda_j-\lambda_h\ne0$, the operator
$\D+\lambda_j-\lambda_h$ is injective on $\C[z]$: its action preserves the
...
Let
\[
 H(z)=\sum_{j=1}^{h}a_j\e^{\lambda_jz},
 \qquad a_j\ne0,
\]
with distinct $\lambda_j$.  Then the differential rank of $H$ over $\C(z)$ is
...
If $H,H',\ldots,H^{(h-1)}$ were dependent over $\C(z)$, then for some rational
functions $b_0,\ldots,b_{h-1}$,
\[
 0=\sum_{k=0}^{h-1}b_k(z)H^{(k)}(z)
  =\sum_{j=1}^{h}a_j
    \left(\sum_{k=0}^{h-1}b_k(z)\lambda_j^k\right)\e^{\lambda_jz}.
\]
...
by an invertible triangular matrix over $\C(z)$ with diagonal $r$.  Applying
...
$\Phi_{q,N}$ over $\C(z)$ is exactly
\begin{equation}
 \boxed{\rank_{\C(z)}\Phi_{q,N}=r_N(q)=\abs{\Omega^*_{q,N}}.}
 \label{eq:rank-support}
\end{equation}
A minimal constant-coefficient annihilator of $Y_{q,N}=z^N\Phi_{q,N}$ is
\begin{equation}
 L_{q,N}(\D)=\prod_{\omega\in\Omega^*_{q,N}}(\D-\ii\omega),
...
Let
\begin{equation}
 E_{q,N}(z):=\prod_{j=1}^{N}2\ii\sin(q^jz).
 \label{eq:EqN}
\end{equation}
...
\begin{equation}
 \Phi_N(z):=\prod_{j=1}^{N}\sinc\!\left(\frac{z}{2^j}\right),
...
\begin{equation}
 ...
 =\frac{2^N-1-2k}{2^N},
 \qquad
 \prod_{j=1}^{N}\eps_j=(-1)^{s_2(k)}=:\TM_k.
...
\begin{equation}
 ...
 \TM_k\exp\!\left(\frac{\ii(2^N-1-2k)z}{2^N}\right),
 \qquad C_N\ne0.
 \label{eq:TM-exponential}
\end{equation}
The familiar finite product
$\sum_{k<2^N}\TM_kx^k=\prod_{j=0}^{N-1}(1-x^{2^j})$ is therefore not only a
...
\(\ii(2^N-1-2k)/2^N\), $0\le k<2^N$, the distinct exponents in
...
\begin{equation}
 ...
 \mathcal P_N(\D)=
 2^{-(2N-2)m}
 \left(\frac12+\ii2^{N-1}\D\right)_m
 \left(\frac12-\ii2^{N-1}\D\right)_m.}
...
\begin{align*}
 &\left(\ell-\frac12+\ii2^{N-1}\D\right)
  \left(\ell-\frac12-\ii2^{N-1}\D\right)\\
...
\[
 \prod_{0\le j<k<n}\abs{h(k-j)}^2
 =\abs h^{n(n-1)}\prod_{k=1}^{n-1}(k!)^2.
\]
Here $\abs h=2/n$, which proves the first equality.  The defining integer
...
\begin{equation}
 ...
 \left(\lambda-\ii\sum_{j=1}^{N}\eps_jq^j\right).
...
\begin{equation}
 ...
 \mathscr P_{q,N}(\lambda-\ii q^{N+1})
 \mathscr P_{q,N}(\lambda+\ii q^{N+1}).}
...
$\Z[q]/(q^2+q-1)$, representing every frequency as $a+bq$ with integers
...
\begin{equation}
 \Phi_b(z):=\prod_{j=1}^{\infty}\sinc\!\left(\frac{z}{b^j}\right).
 \label{eq:base-b-product}
\end{equation}
The product converges locally uniformly because
$\sinc(z/b^j)=1+\Oh_K(b^{-2j})$ on every compact set $K\subset\C$.  It obeys
the first-order dilation equation
\begin{equation}
 \boxed{\Phi_b(bz)=\sinc(z)\Phi_b(z).}
...
$\C(z)$.  In particular, it is transcendental over $\C(z)$.
\end{theorem}
\begin{proof}
...
non-D-finiteness implies transcendence over $\C(z)$.
\end{proof}
...
$\Phi_N\to\Phi_2$ locally uniformly on $\C$, but the limit is not D-finite.
...
algebra, while no finite-order linear differential operator over $\C(z)$
...
For $M\ge0$, let
\begin{equation}
 \mathcal T_M:=\C\!\left(z,\e^{\ii z/2^M}\right).
 \label{eq:trig-field}
\end{equation}
...
Fix $M\ge0$ and let
$R(z,w)\in\C(z,w)$.  Suppose $R(z,\e^{\ii z/2^M})$ has a simple pole at
...
function $S(n)\in\C(n)$.  On all sufficiently large odd integers $n$, the
...
\end{theorem}
\begin{proof}
The MGF $M(t)=\Phi_2(-\ii t)$ is not D-finite by
...
containing a nontrivial subinterval of $(0,1)$.  The same is true for $\upf$ on
...
precisely, with
$M_c(r)=\max_{\abs z=r}\abs{A_c(z)}$,
\begin{equation}
 \log M_c(r)
 \le\frac{(\log r)^2}{2\log2}+\Oh(\log r)
 \qquad(r\to\infty).
...
$\Q$ for rational $q$, and integer pairs $a+bq$ modulo $q^2+q-1$ for the golden
...
\begin{equation}
 \Phi_b(z)=\prod_{j\ge1}\sinc(z/b^j)
 \label{eq:DT-conjecture-function}
\end{equation}
is differentially transcendental over $\C(z)$; that is, it satisfies no
nonzero algebraic differential equation with coefficients in $\C(z)$.
...
coefficient $\sinc z$ naturally lives in a trigonometric extension rather than
$\C(z)$.  The residue obstruction in \cref{thm:field-escape} is evidence that no
...
$z$ and all $\e^{\ii z/2^M}$.  The divisor orbit under $z\mapsto2z$ should enter
