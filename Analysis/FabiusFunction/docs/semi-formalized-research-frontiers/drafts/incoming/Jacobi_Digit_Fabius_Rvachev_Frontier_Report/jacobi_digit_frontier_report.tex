...
the repository; \(q=1/2,\alpha=1\) is the normalized Rvachev \(\upf\) law.
...
& Bessel formulas used to represent or transform the fixed \(\upf\) law
& a Bessel product that \emph{defines} a new probability family, with Rayleigh cumulants and Bessel-zero resonances \\
\addlinespace
Orthogonal polynomials
& Legendre, native \(\upf\)-law, and Hermite limits for established laws
...
\[
 ...
 \qquad L:=\log\frac1q.
\]
The Fourier transform of a probability measure is its characteristic
function \(\widehat\mu(t)=\E e^{itX}\).

\begin{definition}[Jacobi digit and Jacobi-digit cascade]
Let \(U_\alpha\) have density
\begin{equation}
 \rho_\alpha(u)
 =c_\alpha(1-u^2)^{\alpha-1}\mathbf1_{|u|<1},
 \qquad
...
\begin{equation}
 \E U_\alpha^{2m}=\frac{\poch{1/2}{m}}{\poch{\alpha+1/2}{m}},
 \qquad
 \E U_\alpha^{2m+1}=0,
...
\end{equation}
In particular, \(\Var(U_\alpha)=1/(2\alpha+1)\).
\end{lemma}
...
Then
\begin{align}
 \E\|\bm X_{q,d}\|^2&=S_2,\label{eq:radial-second}\\
 \E\|\bm X_{q,d}\|^4&=S_2^2+\frac2d(S_2^2-S_4).
...
fourth moment, and
\(\E(\Omega_i\cdot\Omega_j)^2=1/d\).  This gives
\[
...
\begin{align}
 \widehat\rho_\alpha(t)&=\Jcal_{\alpha-1/2}(t),
 \label{eq:digit-cf}\\
 \widehat f_{q,\alpha}(t)
 &=\prod_{n=0}^{\infty}
 \Jcal_{\alpha-1/2}(aq^nt).
 \label{eq:full-bessel-product}
\end{align}
The product converges normally on compact subsets of \(\C\) and is the unique
...
normalized Rvachev \(\upf\) density.
\end{theorem}
...
zero.  Consequently,
\[
 f_{q,\alpha}\in C_c^\infty(\R),\qquad
 \supp(f_{q,\alpha})=[-1,1],
\]
...
The same argument in \(\R^d\) proves that the spherical random-flight law in
...
\(f_{q,\alpha}\) is log-concave on \(\R\).  Hence it is unimodal and its
...
\]
Comparison with
\(\log\E e^{itU}=\sum_r\kappa_r(U)(it)^r/r!\) yields
...
\begin{equation}
 ...
 a^{2r}q^{2N-2r}
 \frac{\poch{1/2}{r}}{\poch{\alpha+1/2}{r}}
 \mu_{2N-2r}.
...
\begin{equation}
 \chi_\ell=\frac{2\pi i\ell}{L},\qquad \ell\in\mathbb Z,
...
\(\widehat f_{q,\alpha}\) is simple.  If \(q\in\mathcal E_\nu\), at least one
...
\(j_{\nu,k_\ell}/j_{\nu,\ell}\to r\).  Choosing
\(k_\ell=\lfloor r\ell\rfloor\) proves density.
\end{proof}
...
For every integer \(m\ge1\), in the classical sense on \(\R\),
\begin{equation}
\boxed{
\begin{aligned}
 f_{q,m}^{(2m-1)}(x)
...
\begin{equation}
 ...
 \qquad
 G_{q,\alpha}(x):=\Prob(D_{q,\alpha}\le x).
...
\boxed{
 ...
 \sum_{r=0}^{\infty}
 \frac{\poch{\alpha}{r}\poch{1-\alpha}{r}}
 {r!(2a)^r}
...
\(\alpha=m\in\N\), it terminates after \(r=m-1\).  For \(\alpha=1\),
\begin{equation}
...
\[
 \left(1-\frac{x-y}{2a}\right)^{\alpha-1}
 =\sum_{r\ge0}\frac{\poch{1-\alpha}{r}}{r!(2a)^r}(x-y)^r.
\]
...
\(\Dop^\alpha\), use
\(\Gamma(\alpha+r)=\Gamma(\alpha)\poch{\alpha}{r}\), and simplify the beta
...
\(2a\).  If \(\alpha=m\), \(\poch{1-m}{r}=0\) for \(r\ge m\).
\end{proof}

Under the common normalization
\(\Fab(x)=\upf(x-1)\) on \([0,1]\), symmetry gives
...
\begin{proof}
Write \(\log z=L(N+\theta)\), where \(N=\lfloor\log z/L\rfloor\), and
reindex \(m=N-n\):
\[
...
satisfies \(c_1y^\alpha\le\Prob(V\le y)\le c_2y^\alpha\).
...
\[
 \frac{d}{ds}\log\E e^{-sD_{q,\alpha}}+x=0,
 \qquad T_x=\log(2as_x).
\]
Then
\begin{equation}
...
\begin{proof}
Couple \(\varepsilon_n=\sgn(U_{\alpha,n})\).  By symmetry it is Rademacher,
and
\[
 \E|X_{q,\alpha}-B_q|
 \le a\sum_{n\ge0}q^n\E(1-|U_\alpha|)
 =1-\E|U_\alpha|.
\]
...
\(\alpha=1\) is the Rvachev \(\upf\) law.
...
\[
 \frac{\sum a^3q^{3n}\E|U|^3}
 {\left(\sum a^2q^{2n}\Var U\right)^{3/2}}
 =\rho_3(\alpha)\frac{(1-q^2)^{3/2}}{1-q^3}.
\]
...
\begin{enumerate}[label=(\alph*)]
 ...
 \item geometric-comb interpolation in the basis
       ...
       Legendre and native-\(\upf\) constructions; and
