...
\newcommand{\HL}{\mathscr H}
\newcommand{\Gnear}{\mathscr G}
\newcommand{\ord}{\operatorname{ord}}
\newcommand{\ordx}{\operatorname{ord}_{x^{-1}}}
...
\newcommand{\as}{\sim}
\newcommand{\precasy}{\prec}
\newcommand{\preceqasy}{\preccurlyeq}
\newcommand{\coeff}[2]{[#1]\,#2}
\newcommand{\trunc}[2]{\mathop{\mathrm{T}}_{<#1}\!\left(#2\right)}
...
\vspace{0.45cm}
{\LARGE Algebra, Composition, Reversion, and the Lambert \(W\) Function\par}
\vspace{1.2cm}
...
running example is the large-argument expansion of the Lambert \(W\) function.
...
large-argument expansions of Lambert \(W\), Wright's omega function, inverse
...
For Lambert \(W\), the article derives the complete expansion in Lambert
...
branch \(W_{-1}\), and gives numerical and implementation guidance.  Throughout,
...
Lambert \(W\), Cohen on Lambert polynomials, Gessel on formal Lagrange inversion,
...
\begin{equation}\label{eq:pl-shape}
  ...
  \qquad p_n\in\K[X].
\end{equation}
Here \(n_0\in\Z\), so finitely many positive powers of \(x\) are permitted.
...
\[
  y+\log y=x.
\]
The inverse is the Wright omega function \(\omega(x)=W(e^x)\), and its expansion
begins
\begin{equation}\label{eq:omega-preview}
\begin{split}
  \omega(x)={}&x-\ell+\frac{\ell}{x}
  +\frac{\ell^2/2-\ell}{x^2}
  +\frac{\ell^3/3-3\ell^2/2+\ell}{x^3}\\
  &+\frac{\ell^4/4-11\ell^3/6+3\ell^2-\ell}{x^4}+\cdots.
\end{split}
\end{equation}
Every coefficient is a polynomial in \(\ell\).  For \(W(z)\), take
\(x=\Log z\) and \(\ell=\Log\Log z\), with branch choices recorded explicitly.
...
when its coefficients are compared algebraically.  Lambert \(W\) is unusually
...
real asymptotics one can take \(\K=\R\); for branch-sensitive analytic work,
\(\K=\C\); and for exact symbolic computation, \(\K=\Q\) or a finite algebraic
...
\begin{equation}\label{eq:basic-derivation}
  \frac{\dd x}{\dd x}=1,
  \qquad \frac{\dd t}{\dd x}=-t^2,
  \qquad \frac{\dd\ell}{\dd x}=t.
\end{equation}
...
\(p_{\ordx F}\), then
\[
  \operatorname{lm}(F)=t^{\ordx F}\ell^{d(F)},\qquad
  \lc(F)=a.
\]
\end{definition}
...
The support of \(F\) is
\[
  \supp F=\{(n,j):\coeff{t^n\ell^j}{F}\ne0\}.
\]
For every fixed \(n\), its fiber in \(j\) is finite, and the set of occurring
\(n\)'s is bounded below.  Consequently every nonempty subset of \(\supp F\)
...
For \(N\in\Z\), define the inverse-power truncation
\begin{equation}\label{eq:truncation}
  \trunc{N}{F}=\sum_{n<N}p_n(\ell)t^n,
  \qquad F\equiv G\pmod{t^N}
  \Longleftrightarrow F-G\in t^N\K[\ell][[t]].
\end{equation}
A computation ``through order \(x^{-N+1}\)'' means modulo \(t^N\).
...
Suppose only \(FG\bmod t^N\) is required.  Terms of \(F\) with exponent
...
\begin{align}
 ...
 e^U&=\sum_{r\ge0}\frac{U^r}{r!}.\label{eq:exp-U}
\end{align}
...
then \(e^{P(\ell)}\) is generally not a PL series: for example,
\(e^{\ell^2}=x^{\ell}\) introduces a new exponential monomial.  By contrast,
\(e^{a\ell+b}=e^b x^a\) remains in a generalized-power extension, and remains in
\(\PL\) when \(a\in\Z\).
...
\begin{equation}\label{eq:one-derivative}
  \frac{\dd}{\dd x}\bigl(t^n p(\ell)\bigr)
  =t^{n+1}(\dlog-n)p(\ell).
\end{equation}
...
\begin{equation}\label{eq:repeated-derivative}
  \frac{\dd^k}{\dd x^k}\bigl(t^n p(\ell)\bigr)
...
\[
  \frac{\dd}{\dd x}\bigl(x^{1-n}q(\ell)\bigr)
...
If \(n=1\), then
\[
  \int x^{-1}p(\log x)\dd x=\int p(\ell)\dd\ell,
\]
...
\begin{equation}\label{eq:admissible-G}
  G(x)=c x^q(1+U(x)),
  \qquad c\in\K^\times,\quad q\in\N_{>0},\quad U\in\mathfrak m.
\end{equation}
Choose a value of \(\log c\) if \(\K\subseteq\C\).  Then
...
If \(q=r/s\in\Q_{>0}\), the same proof works after adjoining
...
For \(G=cx+d+\bigO(x^{-1})\), factor
\[
  G=cx\left(1+\frac{d}{cx}+\bigO(x^{-2})\right).
\]
Then
\begin{align*}
  \log G
...
\(e^x\) & exponential monomials & logarithmic-exponential transseries\\
...
\begin{equation}\label{eq:near-group}
  \Gnear=\left\{x+A(x):A\in\PL_{\ge0}\right\}.
\end{equation}
...
\(\PL_{\ge0}\).  For existence, solve the fixed-point equation
\begin{equation}\label{eq:fixed-point-B}
  B=\Phi(B):=-A\circ(x+B).
\end{equation}
If \(B-C\in t^N\PL_{\ge0}\), the formal mean-value/Taylor identity gives
\[
  A\circ(x+B)-A\circ(x+C)
  =(B-C)\int_0^1 A'\circ(x+C+s(B-C))\dd s.
\]
Since \(A'\in t\PL_{\ge0}\), the right side lies in
\(t^{N+1}\PL_{\ge0}\).  Thus \(\Phi\) improves the agreement order by at least
...
contraction is visible in the elementary fact \(A'=\bigO(t)\).
...
\end{equation}
The denominator is a unit because \(F'=1+\bigO(t)\).  If
\(F\circ G-x\in t^N\PL_{\ge0}\), Taylor expansion around \(G\) shows that the
new residual is in \(t^{2N+1}\PL_{\ge0}\) (the exact exponent can improve with
...
Let \(R=F\circ G-x\) and \(C=-R/(F'\circ G)\).  Since \(C=\bigO(t^N)\),
\[
 F\circ(G+C)-x
 =R+(F'\circ G)C+\frac12(F''\circ G)C^2+\cdots.
\]
The first two terms cancel.  Now \(F''=A''=\bigO(t^2)\), so the quadratic
remainder is at least \(\bigO(t^{2N+2})\); allowing coarse bounds and Laurent
...
Let \(F=x+A\) with \(A\in\PL_{\ge0}\).  Its compositional inverse is
...
Using a dot for \(\dd/\dd\ell\), the first three coefficients are
\begin{align}
 b_0={}&-a_0,\label{eq:b0}\\
...
\section{Lambert \texorpdfstring{\(W\)}{W} and the Lambert polynomials}\label{sec:lambert}
...
\begin{equation}\label{eq:W-definition}
  W(z)e^{W(z)}=z.
\end{equation}
It has branches \(W_k\), \(k\in\Z\).  Taking a consistent logarithm converts
\eqref{eq:W-definition} to
\begin{equation}\label{eq:W-log-equation}
  w+\log w=\Log z+2\pi i k.
\end{equation}
...
\begin{equation}\label{eq:omega-lagrange}
  \omega(x)=x+\sum_{r\ge1}\frac{(-1)^r}{r!}
  D^{r-1}\ell^r.
\end{equation}
...
Define \(L_n(X)\in\Q[X]\), \(n\ge0\), by
\begin{equation}\label{eq:Lambert-polynomial-operator}
\boxed{
  L_n(X)=\frac{(-1)^{n+1}}{(n+1)!}
  \left[\prod_{j=0}^{n-1}(\partial_X-j)\right]X^{n+1},
}
\end{equation}
with an empty product when \(n=0\).  Thus \(L_0(X)=-X\).
\end{definition}

Then \eqref{eq:omega-lagrange} is
\begin{equation}\label{eq:omega-Ln}
\boxed{
  \omega(x)=x+\sum_{n\ge0}\frac{L_n(\ell)}{x^n}.
}
\end{equation}
The degree of \(L_n\) is \(n\) for \(n\ge1\), its leading coefficient is
...
\boxed{
  L_{n+1}(X)=-L_n(X)+n\int_0^X L_n(u)\dd u,
  \qquad n\ge0.
}
\end{equation}
\end{theorem}
...
\begin{equation}\label{eq:Lambert-generating-F}
  \mathcal L(T,X)=\sum_{n\ge0}L_n(X)T^n.
\end{equation}
Substitute \(\omega(x)=x+\mathcal L(t,\ell)\) into
\(\omega+\log\omega=x\).  Since
\[
...
gives
\[
  L_{n+1}'(X)+L_n'(X)=nL_n(X),
  \qquad L_{n+1}(0)=L_n(0)=0,
\]
...
The recurrence computes \(L_{n+1}\) using one exact polynomial integral and one
subtraction.  It is usually the fastest way to generate a table over \(\Q\).
...
\]
Let \(\StirlingFirst{n}{k}=(-1)^{n-k}s(n,k)\) be the unsigned Stirling cycle
numbers.
...
\begin{align}
  ...
  &=\sum_{m=1}^{n}(-1)^{n+m}
       \StirlingFirst{n}{n-m+1}\frac{X^m}{m!}.
...
\(-L_n+n\int_0^X L_n\) becomes, after shifting the integral index, exactly the
Stirling recurrence
\(s(n+1,k)=s(n,k-1)-ns(n,k)\).  The initial case
\(L_1=X\) is immediate.  Equation \eqref{eq:Ln-unsigned-Stirling} follows from
...
standard DLMF all-branch expansion.  It also shows that a common denominator of \(L_n\) divides \(n!\); more precisely, the
coefficient of \(X^m\) is an integer divided by \(m!\).

\subsection{Table of Lambert polynomials}

\begin{longtable}{@{}c>{\(\displaystyle}l<{\)}@{}}
\caption{Lambert polynomials \(L_n(X)\) through \(n=10\).}\label{tab:Lambert-polynomials}\\
\toprule
\(n\)&\multicolumn{1}{c}{\(L_n(X)\)}\\
\midrule
\endfirsthead
\toprule
\(n\)&\multicolumn{1}{c}{\(L_n(X)\)}\\
...
The factored forms can be preferable numerically because every \(L_n\),
...
\begin{equation}\label{eq:xi-k}
  \xi_k=\Log z+2\pi i k,
\end{equation}
with \(\Log\) the chosen principal logarithm and with the branch of
...
\boxed{
  W_k(z)\as \xi_k+
  \sum_{n\ge0}\frac{L_n(\log\xi_k)}{\xi_k^n}.
}
\end{equation}
Equivalently,
\begin{equation}\label{eq:W-Stirling-DLMF}
  W_k(z)\as\xi_k-\log\xi_k
  +\sum_{n\ge1}\frac{(-1)^n}{\xi_k^n}
    \sum_{m=1}^n
      \StirlingFirst{n}{n-m+1}
      \frac{(-\log\xi_k)^m}{m!}.
\end{equation}
\end{theorem}
...
sectors follows from the standard Lambert \(W\) asymptotic theory
\cite{Corless1996,DLMFLambert}.
\end{proof}

Writing \(L_1=\Log z\) and \(L_2=\Log L_1\) on the principal branch,
\eqref{eq:W-Lambert-polynomial} begins
\begin{align}
 W_0(z)\as{}&L_1-L_2+\frac{L_2}{L_1}
...
\subsection{The real nonprincipal branch \(W_{-1}\)}

Let \(X\to+\infty\) and set \(z=-e^{-X}\).  Write
...
\[
 M=K+\log K-\Log z-2\pi i k,
\]
one has
\begin{equation}\label{eq:Cohen-K-expansion}
  W_k(z)=K+\sum_{n\ge0}\frac{L_n(M)}{K^n}
\end{equation}
...
\begin{equation}\label{eq:omega-N}
  \omega_N(x)=x+\sum_{n=0}^N\frac{L_n(\log x)}{x^n}.
\end{equation}
...
Differentiating \(W(z)e^{W(z)}=z\) gives
\begin{equation}\label{eq:W-derivative}
  W'(z)=\frac{W(z)}{z(1+W(z))}.
\end{equation}
For \(\omega(x)=W(e^x)\),
\begin{equation}\label{eq:omega-derivative}
  \omega'(x)=\frac{\omega(x)}{1+\omega(x)}
  =1-\frac{1}{1+\omega(x)}.
\end{equation}
Once \(\omega\) is known as a PL series, the right side is computed by the
reciprocal recurrence of \cref{sec:mul-div}.  Since
\(1+\omega=x\bigl(1+\bigO(t\ell)\bigr)\), its reciprocal lies in \(\PL\).
...
\begin{equation}\label{eq:W-denominator-Ln}
  \frac{1}{1+W_k(z)}
  =\sum_{n\ge1}\frac{L_n'(M)}{K^n}
\end{equation}
under the corresponding convergence conditions.  The fact that \(L_n'\) is
...
\begin{equation}\label{eq:y-a-log-y-solution}
  y=aW_k\!\left(\frac{e^{x/a}}{a}\right),
\end{equation}
...
\begin{equation}\label{eq:power-exponential-solution}
  y=\frac{\alpha}{\beta}
  W_k\!\left(\frac{\beta}{\alpha}z^{1/\alpha}\right).
\end{equation}
...
then operate over \(\Q[\ell]\), \(\Q(a,b)[\ell]\), algebraic-number fields, or
...
example, to compute \(D^kF\bmod t^N\), only coefficients of \(F\) below
...
polynomials, producing \(L_0,\ldots,L_N\) takes \(O(N^2)\) scalar arithmetic
...
\item[Coefficient-ring model.]  Treat addition, multiplication, division, and
  ...
  multiplication and unit inversion require \(O(N^2)\) coefficient operations.
  Differentiation is \(O(N)\).  Straight Taylor composition is typically
  \(O(N^3)\) if every power and product is formed naively, though degree/order
  bounds often reduce the constant substantially.
\item[Scalar model.]  Dense multiplication of two degree-\(M\) polynomials costs
  \(O(M^2)\) scalar operations naively.  Hence naive PL multiplication costs
  \(O(N^2M^2)\) in a uniform-degree bound.  Using the actual profile
  \(d_F(n)\) gives a much sharper sum
  \[
    \sum_{r<N}\sum_{i+j=r}
      O\bigl((d_F(i)+1)(d_G(j)+1)\bigr).
  \]
\end{description}
...
\begin{enumerate}
\item \(\trunc{N}{F(GH)}=\trunc{N}{(FG)H}\) for randomly generated truncated
  series.
\item \(\trunc{N}{G\,G^{-1}}=1\) for units with random polynomial coefficients.
...
\begin{equation}\label{eq:strong-remainder}
  R_N(x)=O\bigl(x^{-N-1}(1+\log x)^{m_N}\bigr)
\end{equation}
...
Lambert \(W\), the integer \(k\) and the logarithm branches in \(\xi_k\) carry the
...
\(t=e^{-\ell}\).  The formal PL topology deliberately treats them as an ordered
...
\(y^2=x\) has inverse \(x^{1/2}\), and the Lambert \(W\) function near its branch
point \(-e^{-1}\) has a square-root local parameter.  Such expansions are not a
...
of \(\R\) or \(\C\) equipped with an asymptotically meaningful real part.  One
...
\(e^F\) is a power of \(x\) times a small exponential and can remain in a
power-log extension.  If \(F_{\mathrm{large}}\) contains \(x\), \(\ell^2\), or
more general large terms, \(e^F\) creates a genuinely new monomial such as
\(e^x\) or \(e^{\ell^2}\).  Repeated closure under exponentials and logarithms
...
\[
  \Phi(x,Y_0)\in t^N\PL_{\ge0}
\]
and the partial derivative
\[
  \Phi_y(x,Y_0)=c+\bigO(t),
  \qquad c\in\K^\times,
\]
...
\(\ell+1+O(t)\), the logarithm would begin with \(\log\ell\) and leave the
one-log ring.  The difference is entirely in the leading coefficient.

\subsection{Triangular construction of \(\omega(x)\)}
...
\begin{align*}
 ...
 &R_3&=-\left(\frac{\ell^3}{3}-\frac{3\ell^2}{2}+\ell\right)t^3
      +\cdots.
\end{align*}
The next correction is therefore \(L_3(\ell)t^3\).  In general, if
\[
  G_N=x+\sum_{n=0}^{N-1}L_n(\ell)t^n,
\]
then
\begin{equation}\label{eq:Lambert-residual-staircase}
  f(G_N)-x=-L_N(\ell)t^N+O(t^{N+1}\ell^{N+1}).
\end{equation}
...
\item \textbf{Lambert \(W\) is the canonical model.}
...
Let \(F=x+A\) with \(A\in\PL_{\ge0}\), and let
\[
...
\[
  F\circ G_N-F\circ G
  =(F'\circ G)q(\ell)t^m+\bigO(t^{m+1}).
\]
Because \(F'\circ G=1+\bigO(t)\), the coefficient at level \(m\) would be
\(q\ne0\), contradicting \eqref{eq:inverse-certificate}.  Hence
\(G_N-G\in t^{N+1}\PL_{\ge0}\).
\end{proof}
...
\begin{equation}\label{eq:ramified-leading-map}
  ...
  \qquad U(y)\prec1,
\end{equation}
where \(q\in\Q_{>0}\).  Choose a branch of
\(d=c^{-1/q}\) and write
...
\(y^2+y\).  If \(q\) is irrational, a Hahn group containing \(q^{-1}\Z\) is
...
\begin{equation}\label{eq:Lambert-coefficient-triangle}
  L_n(X)=\sum_{m=1}^{n}\lambda_{n,m}X^m,
  \qquad n\ge1.
\end{equation}
...
At the level of \(\omega\), implicit differentiation of
\(\omega+\log\omega=x\) yields
\begin{equation}\label{eq:omega-derivative-app}
  \omega'(x)=\frac{\omega(x)}{1+\omega(x)}.
\end{equation}
Substitution of \(\omega=x+\sum L_n(\ell)x^{-n}\) into
...
\begin{equation}\label{eq:scaled-omega-exact}
  y=a\,\omega\!\left(\frac{x}{a}-\log a\right).
\end{equation}
Expanding the right side in the original variable \(x\) recovers the powers
\(a^{n+1}L_n(\log x)\) at their highest logarithmic degree, together with the
...
For direct evaluation of \(W_k(z)\), it is usually preferable to retain
\[
  \xi_k=\Log z+2\pi i k,
...
and evaluate
\[
  W_k(z)\approx \xi_k+\sum_{n=0}^{N}\frac{L_n(\eta_k)}{\xi_k^n}
\]
rather than fully expand in powers of \((\Log z)^{-1}\) and \(2\pi i k\).
The nested form preserves branch information, has smaller intermediate
expressions, and permits Horner evaluation of each \(L_n\).  A further
...
\(L_n(X)\) & Lambert polynomial at inverse-power level \(n\)\\
\(s(n,k)\) & signed Stirling number of the first kind\\
\(\StirlingFirst{n}{k}\) & unsigned Stirling cycle number\\
\(\as\) & Poincar\'e asymptotic expansion in the stated scale\\
\(\bigO(t^N\ell^d)\) & analytic remainder bound when explicitly justified\\
\bottomrule
\end{longtable}
...
\item Attach the exponent group and coefficient domain to every series value;
  never silently coerce \(\K[\ell]\) to \(\K(\ell)\) or \(\Z\) exponents to
  \(\Q\) exponents.
...
``\S4.13 Lambert \(W\)-Function,'' in \emph{DLMF}, version 1.2.7,
