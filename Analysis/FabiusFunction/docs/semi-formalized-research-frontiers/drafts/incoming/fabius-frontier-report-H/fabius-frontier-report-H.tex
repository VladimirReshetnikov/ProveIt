...
\hypersetup{colorlinks=true,linkcolor=deepolive,citecolor=deepolive,urlcolor=softcoral}
...
Let
\[
 F_q(x)=\Pp(V_q\le x),\qquad f_q=F_q',
\]
...
\begin{align}
 ...
 u_q(x)&=\frac1{2a}\int_{(x-a)/q}^{(x+a)/q}u_q(y)\dd y,\label{eq:urefinement}\\
...
up-function in the normalization $\supp u_q=[-1,1]$.
\end{proposition}
...
With $\sinc z=\sin z/z$ and $\sinc0=1$,
\begin{align}
 \widehat u_q(t)&=\E e^{itX_q}
   =\prod_{k=0}^{\infty}\sinc(aq^kt),
 \label{eq:qfourier}\\
 M_q(t)&=\E e^{tV_q}
 ...
 \label{eq:qlaplace}
\end{align}
Each product converges locally uniformly.  In particular, $\widehat u_q$ is entire of
...
$F_q\in C^\infty(\R)$ after constant extension outside $[0,1]$, and every derivative
of positive order is flat at both endpoints.
\end{theorem}

\begin{proof}
Put $T=a|t|$ and $R=\log T$.  For $T\ge1$, choose
$N=\lfloor R/r\rfloor+1$.  For $0\le k<N$, $Tq^k\ge1$, and therefore
\[
 |\widehat u_q(t)|
...
\begin{equation}
 \log|\widehat u_q(t)|\le-\frac{R^2}{2r}+O_q(R).
 \label{eq:lognormal-fourier-bound}
\end{equation}
Hence $|t|^m\widehat u_q(t)\in L^1(\R)$ for every $m$.  Fourier inversion gives a
...
\end{equation}
At $q=1/2$, if $\tm_j=(-1)^{s_2(j)}$ is the $\{\pm1\}$ Thue--Morse sequence, this
reduces to
\begin{equation}
 \up^{(n)}(x)=(-1)^n2^{n(n+1)/2}
 \sum_{j=0}^{2^n-1}\tm_j\,
 \up\bigl(2^nx+2j-(2^n-1)\bigr).
...
For the ordinary Thue--Morse signs $\tm_j=(-1)^{s_2(j)}$,
\begin{equation}
 \sum_{j=0}^{2^n-1}\tm_j\bigl(2j-(2^n-1)\bigr)^m
...
For example,
\begin{align}
 \Var(V_q)&=\frac{a}{12(1+q)},\label{eq:Vvariance}\\
...
\eqref{eq:fixedpoints}.  If $m_n(q)=\E V_q^n$, then $m_0=1$ and
\begin{equation}
 (1-q^n)m_n(q)=
...
$\E[B_n(x+Z)]=x^n$.  The entire geometric convolution has a natural analogue.
...
\begin{align}
 \frac{\dd}{\dd x}\Bell_{n,q}(x)&=n\Bell_{n-1,q}(x),
 \label{eq:appell}\tag{A}\\
 \E\,\Bell_{n,q}(x+V_q)&=x^n,
...
$M_q(t)=\E e^{tV_q}$ proves \eqref{eq:averaging-inverse}.  The fixed-point product
satisfies
\[
...
Writing $y=x-1/2$, $\sigma_q^2=\Var(V_q)$, and $\kappa_{4,q}=\kappa_4(V_q)$ gives
\begin{align}
 \Bell_{0,q}(x)&=1,\\
 \Bell_{1,q}(x)&=y,\\
...
$\int_{-\infty}^y\phi(t)H_n(t)\dd t=-\phi(y)H_{n-1}(y)$ gives
...
\begin{equation}
 \Psi_q(z)=\prod_{k=0}^{\infty}\sinc(q^kz).
 \label{eq:Psiq}
\end{equation}
Then $\widehat u_q(t)=\Psi_q(at)$.  The zero multiset is
\begin{equation}
 \mathcal Z_q=\{\,\pi n q^{-k}:n\in\Z\setminus\{0\},\ k\ge0\,\},
...
$q^d\notin\mathbb Q$ for every integer $d\ge1$.  If $q^d\in\mathbb Q$ for some
...
$q^{|k-\ell|}\in\mathbb Q$.  One collision can be multiplied by arbitrary nonzero
...
\begin{proof}
Since $v_b(m)+1=\sum_{k\ge0}\mathbf1_{b^k\mid m}$,
\[
...
\[
 h_b(x)=\frac12\int_{b(x-1)}^{b(x+1)}h_b(y)\dd y,
\]
so
\[
...
\[
 ...
 T_b=\prod_{j=1}^{\infty}\sinc(c_b/b^j)>0.
\]
Then, for every $m\ge0$,
\begin{equation}
 \left|\widehat h_b(c_bb^m)\right|
...
\begin{proof}
We have $\widehat h_b=\Psi_{1/b}$.  For $0\le j\le m$, the congruence
$b^j\equiv(-1)^j\pmod{b+1}$ gives
$|\sin(c_bb^j)|=\sin c_b$.  Hence
\[
 \prod_{j=0}^{m}|\sinc(c_bb^j)|
...
\begin{equation}
 \log_2\|\up^{(n)}\|_p=\frac{n^2}{2}+O_p(n).
...
\[
 |t|^n|\widehat h_b(t)|=|\widehat{h_b^{(n)}}(t)|
 \le\|h_b^{(n)}\|_1.
\]
...
\begin{equation}
 g^*(s)=\int_0^\infty g(x)x^{s-1}\dd x
...
\[
 \int_0^\infty g'(x)x^s\dd x=\Gamma(1+s)\zeta(1+s).
\]
...
\begin{equation}
 \boxed{
 \log H_q(z)=
 -\frac{L^2}{2r}-\frac L2-rac r{12}-\frac{c_1}{r}
 +\cP_r(L)+O_{q,N}(z^{-N}).}
...
\begin{equation}
 ...
 F_q(x_q(L))=
 \frac{\exp\!\left(
 \cA(L)-\frac{L^2}{2r}-\frac L2-rac r{12}-\frac{c_1}{r}
...
$\dd\log F_q/\dd\log x\sim\cA(L)\asymp L$.  Thus a phase-free inverse can still have
...
\[
 z^j\frac{\dd^j}{\dd z^j}
 =D(D-1)\cdots(D-j+1),\qquad D=\frac\dd{\dd L},
\]
...
\begin{equation}
 ...
 \qquad
 a_n^{(b)}=\frac{2n+1}{2}\int_{-1}^1h_b(x)P_n(x)\dd x.
...
$k=\lfloor\log n/\log b\rfloor$ yields \eqref{eq:legendre-upper}; tracking the
...
\[
 m_q(z)=\#\{k\ge0:zq^k/\pi\in\Z\setminus\{0\}\}.
\]
