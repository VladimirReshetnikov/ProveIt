...
All joint cumulants are explicit Bernoulli rational functions.  Hyperbolic translation $u\mapsto T+u$ suppresses every cumulant of order $m>2$ at rate $\e^{-(m-2)T}$, proving convergence to the stationary Gaussian process with covariance $\sech(u-v)$ and providing a Bell-partition expansion beyond Wick's rule.  Divided-difference confluence gives the exact support volume of the parameter jet
\[
...
\end{equation}
In particular $\E X_q=1/2$.  Direct summation gives
\begin{equation}
  \Var(X_q)
...
For $z\in\C$, write $\sinc z=\sin z/z$ with $\sinc0=1$.  The characteristic function is
\begin{equation}
 \phi_q(t)
 =\exp(\ii t/2)\prod_{n\ge0}
   \sinc\!\left(\frac{t(1-q)q^n}{2}\right).
...
Let $F_q(x)=\Prob(X_q\le x)$ and let $f_q$ denote its density.  Under the normalization used in the repository,
\begin{equation}
  F_{1/2}=F,
\end{equation}
...
\begin{equation}
  f_{1/2}(x)=2\,\operatorname{up}(2x-1).
\end{equation}
...
\begin{equation}
  ...
  \label{eq:zonoid}
\end{equation}
Here convergence is in Hausdorff distance because $\sum_n\norm{v_n}<\infty$.

\begin{theorem}[Common-digit support zonoid]
\label{thm:support-zonoid}
For \eqref{eq:ordered-q}, $\supp(\vct X_Q)=Z_Q$.  The set $Z_Q$ is a compact, convex, full-dimensional zonoid contained in $[0,1]^d$, centrally symmetric about $(1/2,\ldots,1/2)$.
\end{theorem}

\begin{proof}
The product measure on $[0,1]^{\N_0}$ has full support.  The map
...
For $t=(t_1,\ldots,t_d)\in\R^d$,
\begin{equation}
 \Phi_Q(t)
 =\exp\!\left(\frac{\ii}{2}\sum_{j=1}^dt_j\right)
  \prod_{n=0}^{\infty}
  \sinc\!\left(\frac12\sum_{j=1}^dt_j(1-q_j)q_j^n\right).
 \label{eq:joint-sinc}
\end{equation}
...
\begin{equation}
  ...
  \qquad n\ge0,\quad k\in\mathbb Z\setminus\{0\}.
...
$\exp(\ii t\cdot\sum_nv_n/2)=\exp(\ii\sum_jt_j/2)$, proving \eqref{eq:joint-sinc}.  For fixed $t$, the arguments in the tail are exponentially small and their squares are summable.  Since $\log\sinc z=-z^2/6+O(z^4)$ near zero, the tail product converges to a nonzero number.  Therefore the product vanishes precisely when one finite factor vanishes, giving \eqref{eq:zero-hyperplanes}.
\end{proof}
...
For a multi-index $\alpha=(\alpha_1,\ldots,\alpha_d)\in\N_0^d\setminus\{0\}$, set $|\alpha|=\sum_j\alpha_j$.  Let $\kappa_\alpha(\vct X_Q)$ denote the joint cumulant in which coordinate $j$ occurs $\alpha_j$ times.
...
The law of $\vct X_Q$ has a density $f_Q\in C_c^\infty(\R^d)$ supported on $Z_Q$.  Every derivative of $f_Q$ vanishes on $\partial Z_Q$.  Moreover,
\begin{equation}
  \norm{f_Q}_{\infty}
  \le
  \frac{1}{\displaystyle
...
\]
and put $Q=\e^{-2h}$.  Then \cref{thm:exact-volume} and the hyperbolic subtraction law give
\begin{equation}
 \Vol Z_Q
...
\[
 ...
 \qquad D=\operatorname{diag}\sqrt{1-q_i^2},
\]
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{hyperbolic_geometry.pdf}
...
\[
  \widehat Y_q=\operatorname*{argmin}_{Z\in\operatorname{span}(Y_{q_1},\ldots,Y_{q_m})}
      \E(Y_q-Z)^2
\]
...
\begin{equation}
 \inf_{\beta\in\R^m}
   \E\left(Y_q-\sum_{j=1}^m\beta_jY_{q_j}\right)^2
  =\prod_{j=1}^m
...
\begin{equation}
  ...
  \qquad \E\epsilon_n=0,\quad\E\epsilon_n^2=1.
  \label{eq:epsilon}
\end{equation}
Then
\begin{equation}
...
\begin{equation}
  \E\bigl[G(z)\overline{G(w)}\bigr]=\frac1{1-z\bar w}.
...
For $x=q^{m+\theta}$ with $m\in\N_0$ and $0\le\theta\le1$, linear interpolation gives
\begin{equation}
 g_{q,r}(x)
  =r^m\left[
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{support_zonotope_q050_q080.pdf}
...
With the convention
\[
  K(h)=\int_{\R}\e^{\ii\omega h}s(\omega)\dd\omega,
\]
...
\end{equation}
Let $u_j=T+t_j$, put $a_j=\e^{-2t_j}$ and $z=\e^{-2T}$.  Then the same cumulant has the exact finite symmetric-polynomial form
\begin{equation}
...
For $T>0$, define the shifted process on a fixed compact interval $I\subset\R$ by
\begin{equation}
...
$c_n(u)=\sech u\tanh^nu$ a fixed number of times produces $c_n(u)$ times a polynomial in $n\sech^2u/\tanh u$ and bounded hyperbolic functions.  Uniformly for $t\in I$, its largest absolute coefficient is $O_k(\e^{-T})$: the elementary bound $n^a\e^{-cn\e^{-2T}}=O_a(\e^{2aT})$ is exactly canceled by the derivative factors $\e^{-2aT}$, leaving the leading $\sech(T+t)$.
...
\[
  \E\abs{Y_{u+h}^{(a)}-Y_u^{(a)}}^2
   \le |h|^2\sup_v\E\abs{Y_v^{(a+1)}}^2.
\]
...
Let $2p$ time points $t_1,\ldots,t_{2p}$ be fixed.  The moment--cumulant formula and \eqref{eq:cumulant-leading-asymptotic} show that pair partitions survive at order one, a single four-block contributes at order $\e^{-2T}$, and every remaining partition contributes $O(\e^{-4T})$.  If $\mathfrak M(S)$ denotes pairings of a finite even set $S$ and
\begin{equation}
 C_4(t_i,t_j,t_k,t_\ell)
  =-\frac{48}{5}\,
    \frac{\e^{-(t_i+t_j+t_k+t_\ell)}}
         {\e^{-2t_i}+\e^{-2t_j}+\e^{-2t_k}+\e^{-2t_\ell}},
 \label{eq:C4-leading}
\end{equation}
then
\begin{align}
 \E\prod_{a=1}^{2p}Y_{T+t_a}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{gaussianization_cumulants.pdf}
...
Their norms are
\begin{equation}
  \int_{\R}p_m(\omega)p_n(\omega)s(\omega)\dd\omega
   =(n!)^2\delta_{mn}.
  \label{eq:MP-norms}
\end{equation}
...
\end{equation}
Equivalently, $Q_n(D)=\ii^np_n(-\ii D)$.  The first operators are
\begin{align}
 Q_0&=1,&
 Q_1&=D,&
 Q_2&=D^2+1,\\
...
\begin{proof}
In the spectral representation, $Q_n(D)$ has multiplier $\ii^np_n(\omega)$.  Formula \eqref{eq:MP-norms} gives zero covariance for unequal degrees and variance $(n!)^2$ for degree $n$.  Division by $n!$ proves \eqref{eq:innovation-orthonormality}.
\end{proof}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{jet_volume_convergence.pdf}
...
They satisfy
\[
  \int_0^1\mathcal P_m(x)\mathcal P_n(x)\dd x
    =\frac{\delta_{mn}}{2m+1}.
\]
Therefore
\begin{equation}
...
where
\begin{equation}
  L_{mn}(q,r)=\E[\mathcal P_m(X_q)\mathcal P_n(X_r)].
  \label{eq:Lmn}
\end{equation}
...
\end{equation}
Since the zero extension of $f_{q,r}$ is $C^\infty$ on $\R^2$, repeated integration by parts with the Legendre Sturm--Liouville operator gives, for every $A>0$,
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:tau}
\end{equation}
where $s_2(n)$ is the binary digit sum.  The finite product identity
\begin{equation}
...
\begin{equation}
  \frac{\dd}{\dd x}\log\tanh x=\frac{2}{\sinh2x},
  \qquad
  \frac{\dd^2}{\dd x^2}\log\tanh x
...
The large-$d$ limit is a nonclassical logarithmic-energy problem with pair interaction $-\log\tanh|u-v|$.  Near the diagonal it has the usual $-\log|u-v|$ singularity, but its long-range interaction decays exponentially.  Determining the equilibrium density and edge scaling is posed in \cref{question:design-limit}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{volume_convergence.pdf}
...
The exact cumulants \eqref{eq:Yq-cumulant} were compared with their leading hyperbolic asymptotics.  At $T=3$, the ratios exact/leading are $0.99999386$ for order four and $0.99995699$ for order eight.  This rapid agreement reflects the absence of an $\e^{-2T}$ correction inside the individual cumulant formula \eqref{eq:cumulant-elementary-symmetric}; after the leading factor, the first denominator correction is $\e^{-4T}$.
...
\begin{equation}
  ...
  \label{eq:complex-X}
\end{equation}
A $d$-tuple has a real zonoid support in $\C^d\cong\R^{2d}$.  Its covariance Gram matrix is still governed by the Szeg\H{o} kernel, but real $2d$-volume involves $2d\times2d$ minors rather than the complex $d\times d$ determinant.
...
The natural language is exterior algebra over $\R$ applied to Hardy kernel vectors.  The exact real identity \eqref{eq:support-correlation} is the rank-one boundary case of this proposed theory.
...
\begin{equation}
  \int_0^L\log\tanh|u-v|\dd\mu(v)=\text{constant}
...
For vectors $g_1,\ldots,g_M\in\R^d$, the zonotope
\[
  Z=\sum_{k=1}^M[0,g_k]
\]
has volume
...
\[
  ...
  \qquad \ell=1,\ldots,M.
\]
Let $V_\ell$ be the $d\times d$ matrix with columns $v_n$, $n\in B_\ell$.  It is invertible.  Hence for every $t\in\R^d$,
\begin{equation}
  \max_{n\in B_\ell}|t\cdot v_n|
   \ge\frac{\sigma_{\min}(V_\ell)}{\sqrt d}\norm t.
  \label{eq:block-projection}
\end{equation}
Since $|\sinc x|\le\min(1,1/|x|)$, the product of characteristic factors over block $B_\ell$ is at most
\begin{equation}
  \frac{C_\ell}{1+\norm t}.
...
\begin{equation}
  \partial^\beta f_Q(x)
   =(2\pi)^{-d}\int_{\R^d}
      (-\ii t)^\beta\e^{-\ii t\cdot x}\Phi_Q(t)\dd t.
