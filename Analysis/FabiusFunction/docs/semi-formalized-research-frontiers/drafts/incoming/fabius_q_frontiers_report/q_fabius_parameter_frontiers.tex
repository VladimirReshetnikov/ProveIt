...
$\Up$-function, and hence lies on the usual Fabius--Rvachev bridge.  The
...
Second, writing $q=e^{-\delta}$, we prove a full large-deviation principle at
speed $1/\delta$.  Its limiting scaled cumulant transform is
\[
 \Lambda(\theta)=\int_0^\infty
     \log\!\left(\frac{\sinh(\theta e^{-t})}{\theta e^{-t}}\right)\dd t
 =\int_0^\theta\frac{\log(\sinh u/u)}{u}\dd u,
\]
...
& A complete continuous-parameter Edgeworth hierarchy, uniform on $\R$ and
...
$\frac12\1_{[-1,1]}$.  For $0<q<1$ define
\begin{equation}
  X_q=(1-q)\sum_{j=0}^{\infty}q^jU_j.
  \label{eq:Xq}
\end{equation}
The series converges absolutely almost surely, $\E X_q=0$, and
...
$\prod_{k\ge1}\sinc(t/2^k)$, the standard Fourier transform of the centered
Rvachev $\Up$-function.  Thus $X_{1/2}$ is exactly the centered up-law; the
...
Set
\begin{equation}
 q=e^{-\delta},\qquad \delta=-\log q.
 \label{eq:delta}
\end{equation}
Then
\begin{equation}
 1-q=\delta s_\delta e^{-\delta/2},
 \qquad
...
\label{thm:kernel}
For $0<r<s<1$, define a probability mass function on $\N_0$ by
\begin{equation}
...
\begin{equation}
 P_{r,s}(z)=
 \exp\left\{\int_r^s\sum_{k\ge1}u^{k-1}(z^k-1)\dd u\right\}.
...
jumps occur with total intensity $(1-u)^{-1}\dd u$, and conditional on a
jump the size has geometric law
\begin{equation}
 \Pp\{J=k\mid u\}=(1-u)u^{k-1},\qquad k\ge1.
...
$u^{k-1}\dd u$ at size $k$.  Summing over $k$ gives total rate
...
\begin{equation}
 ...
 \qquad
 \E\!\left[Y_{r,s}\mid V_0,V_1,\ldots\right]=X_s^{(V)}.
...
\begin{equation}
 \Lambda_\delta(\theta)
 :=\delta\log\E\exp\!\left(\frac{\theta X_{e^{-\delta}}}{\delta}\right).
 \label{eq:scaled-cgf}
\end{equation}
Using \eqref{eq:sdelta}, the $j$th argument of $g$ is
\[
 \frac{\theta(1-e^{-\delta})e^{-j\delta}}{\delta}
 =\theta s_\delta e^{-(j+1/2)\delta}.
\]
...
\begin{equation}
 ...
 g\!\left(\theta s_\delta e^{-(j+1/2)\delta}\right).
 \quad}
...
Uniformly for $\theta$ in compact subsets of $\R$,
\begin{equation}
 \Lambda_\delta(\theta)\longrightarrow
 \Lambda(\theta):=\int_0^\infty g(\theta e^{-t})\dd t
 =\int_0^\theta\frac{g(u)}{u}\dd u.
 \label{eq:Lambda}
\end{equation}
...
bounded by $C\theta^2e^{-2t}$ after the identification
...
of variables $u=\theta e^{-t}$ yields the second.  Differentiation under the
...
$g''(u)>0$; therefore
\[
 \Lambda''(\theta)=\int_0^\infty e^{-2t}g''(\theta e^{-t})\dd t>0.
\]
The explicit formulas follow by differentiating the second integral.  Finally
$g(\theta)=\theta-\log(2\theta)+O(e^{-2\theta})$ as
...
$D=\theta\frac{\dd}{\dd\theta}$ be the Euler operator.

\begin{theorem}[All-order midpoint expansion]
\label{thm:midpoint-EM}
For every integer $N\ge1$ and every compact $K\subset\R$,
\begin{align}
 \Lambda_\delta(\theta)
...
\begin{proof}
For
$f_{\delta,\theta}(t)=g(\theta s_\delta e^{-t})$, all derivatives decay
...
\begin{equation}
 ...
 =\int_0^\infty f_{\delta,\theta}(t)\dd t
 -\sum_{m=1}^{N-1}
   \frac{B_{2m}(1/2)}{(2m)!}\delta^{2m}
...
\label{thm:LDP}
The family $X_{e^{-\delta}}$ satisfies a large-deviation principle on $\R$
...
differentiable function $\Lambda$ on all of $\R$.  Since every
$X_{e^{-\delta}}$ is supported on the common compact interval $[-1,1]$,
...
For $u>0$,
\begin{equation}
 g(u)=u-\log(2u)+\log(1-e^{-2u}).
 \label{eq:g-large}
\end{equation}
Define the convergent constant
\begin{equation}
 \cL=
 \int_0^1\frac{\log(\sinh u/u)}{u}\dd u-1
 +\int_1^\infty\frac{\log(1-e^{-2u})}{u}\dd u.
 \label{eq:CLambda}
\end{equation}
Numerically,
\begin{equation}
...
\begin{align}
 ...
 &=\theta-\frac12(\log\theta)^2
   -(\log2)\log\theta+\cL
   +O\!\left(\frac{e^{-2\theta}}{\theta}\right),
...
\begin{equation}
 ...
 \left(1+O\!\left(\frac{e^{-2\theta_0(\eta)}}{\log\theta_0(\eta)}\right)\right),
...
$\phi(x)=(2\pi)^{-1/2}e^{-x^2/2}$, and let a multi-index
$\bm{k}=(k_2,k_3,\ldots)$ have weight
\begin{equation}
 \wt(\bm{k})=\sum_{m\ge2}(m-1)k_m,
 \qquad
...
$|\sinc u|\le e^{-c_0u^2}$ for $|u|\le1$.  If
...
\]
which is bounded by $e^{-c/(1-q)}$.  On the far part of the tail, the first
$L$ factors also satisfy $|\sinc u|\le|u|^{-1}$ and supply the stated
...
uniform Gaussian envelope $|\chi_q(t)|\le e^{-ct^2}$ in this zone.
...
\begin{equation}
 \left|\chi_q(t)-e^{-t^2/2}\mathcal P_{R,q}(it)\right|
 \le C_R\delta^{R+1}(1+|t|^{M_R})e^{-ct^2},
 \qquad |t|\le T_\delta,
...
$O(e^{-c/(1-q)})$ times a power of $b_q^{-1}$ and is again smaller than every
...
\end{equation}
The first exponential correction $e^{-\delta z^4/20}$ matches the leading
...
$e^{-I(x)/\delta}$, with Lambert-$W_{-1}$ behavior as $x$ approaches the
support edge.
\end{enumerate}
\end{principle}
...
For every compact $K\Subset(-1,1)$, the density of $X_{e^{-\delta}}$ has an
...
\[
 f_q(x)=\frac1\pi\int_0^\infty\chi_q(t)\cos(tx)\dd t.
\]
...
Let $V$ be bounded and let $K_V(\theta)=\log\E e^{\theta V}$.  For
$X_q^{(V)}$ from \eqref{eq:general-seed},
\begin{equation}
 \delta\log\E\exp\!\left(\frac{\theta X_{e^{-\delta}}^{(V)}}{\delta}\right)
 \longrightarrow
 \Lambda_V(\theta):=\int_0^\infty K_V(\theta e^{-t})\dd t.
...
calculation gives
\[
 \delta\sum_{j\ge0}K_V(\theta s_\delta e^{-(j+1/2)\delta}),
\]
...
in inverse powers of $\theta_0$ and exponentials $e^{-2k\theta_0}$, with no
...
For which probability metrics $d$ does the map $q\mapsto\Law(X_q)$ admit a
