...
\newcommand{\PLL}{\mathscr P_{\mathrm{Laur}}}
\newcommand{\QPL}{\mathscr Q}
\newcommand{\Gnear}{\mathcal G}
...
\newcommand{\D}{\frac{\dd}{\dd x}}
\newcommand{\DL}{\partial_{L}}
...
\Large Algebra, Composition, Series Reversal, and the Lambert \(W\) Function}
\author{}
\date{August 31, 2026}
...
\(x=\log z\), the classical large-argument expansion of the Lambert \(W\) function is the canonical example:
\[
 W(z)=x-\log x+\frac{P_1(\log x)}{x}
      +\frac{P_2(\log x)}{x^2}+\cdots .
\]
...
The inverse of \(x+\log x\) leads to a distinguished family of Lambert polynomials.  We derive their implicit generating equation, differential recurrence, operator formula, explicit expression through unsigned Stirling cycle numbers, Laplace transform, and numerical behavior.  The same polynomials govern every branch of the large-argument Lambert \(W\) expansion, the real branch \(W_{-1}\) near the origin, and the generalized inverse of \(y^\alpha e^y\).  Throughout, formal identities are separated from analytic asymptotics and actual convergence, and the exact points at which one must enlarge the one-logarithm class are made explicit.
\end{abstract}
...
\subsection{\texorpdfstring{From Lambert \(W\) to a one-logarithm algebra}{From Lambert W to a one-logarithm algebra}}

The Lambert function is defined branchwise by
\begin{equation}\label{eq:W-definition}
 W_k(z)\e^{W_k(z)}=z.
\end{equation}
Taking a logarithm in a sector where branches have been fixed gives
\begin{equation}\label{eq:W-log-equation}
 W_k(z)+\Log W_k(z)=\Log z+2\pi\ii k.
\end{equation}
The dominant term is therefore
\(\xi_k=\Log z+2\pi\ii k\), and the first correction is
\(-\Log\xi_k\).  Subsequent corrections are inverse powers of \(\xi_k\) with polynomial coefficients in \(\Log\xi_k\).  In other words, after renaming
\[
 x=\xi_k,\qquad L=\Log x,
\]
...
\begin{equation}\label{eq:power-log-monomials}
 x^\alpha L^\beta,
\end{equation}
usually with \(\alpha\in\Z\) and \(\beta\in\N\).  Its restricted form permits elementary proofs and direct algorithms while retaining the nested-scale behavior characteristic of transseries.
...
\begin{equation}\label{eq:P-def}
 \PL:=\K[L][[t]]
 =\set{\sum_{n=0}^{\infty}P_n(L)t^n:P_n\in\K[L]}.
\end{equation}
Its Laurent extension is
\begin{equation}\label{eq:PLaur-def}
 \PLL:=\K[L]((t))
 =\bigcup_{m\in\Z}t^m\PL.
\end{equation}
...
\begin{equation}\label{eq:truncation}
 [F]_{<N}:=\sum_{n<N}P_n(L)t^n.
\end{equation}
Then \(F=[F]_{<N}+\bigO(t^N)\).
...
For orientation, let
\[
 \mathfrak M_{\mathrm{pl}}=\set{x^\alpha L^\beta:(\alpha,\beta)\in\R^2}
\]
...
\begin{equation}\label{eq:linear-degree-profile}
 ...
 \le \max(a,c)N+b+d.
\end{equation}
For Lambert's expansion, \(\deg P_n=n\); products of finitely many such series therefore retain at most linear logarithmic-degree growth.

\subsection{A worked multiplication}

Consider
\begin{align*}
 F&=1+(L+1)t+(L^2-2L)t^2+(3-L)t^3+\bigO(t^4),\\
 G&=2-Lt+(L^2+1)t^2+2Lt^3+\bigO(t^4).
\end{align*}
...
\begin{equation}\label{eq:worked-product}
 FG=2+(L+2)t+(2L^2-5L+1)t^2+(3L^2+L+7)t^3+\bigO(t^4).
\end{equation}
...
\begin{equation}\label{eq:reciprocal-example-C}
 \frac1B=1-Lt+t^2+(L^3-2L)t^3+(-L^4+L^2+1)t^4+\bigO(t^5).
\end{equation}
...
The result is stronger than one might expect from elementary integration tables.  The obstruction \(\int \dd x/(x\log x)=\log\log x\) does not contradict it, because \(1/(x\log x)=t/L\) is already outside \(\PLL\).  Once rational or negative logarithmic powers are admitted, new logarithmic levels can indeed be forced.
...
\begin{equation}\label{eq:lambda-def}
 \lambda:=\log(1+u)\in t\PL.
\end{equation}
Then
\begin{equation}\label{eq:logG-and-inverseG}
 \log G=L+\lambda,
 \qquad
 G^{-n}=t^n(1+u)^{-n}=t^n\e^{-n\lambda}.
\end{equation}
...
Let
\begin{align}
 F&=x+A_0(L)+A_1(L)t+A_2(L)t^2+A_3(L)t^3+\bigO(t^4),
 \label{eq:F-near-low}\\
 G&=x+B_0(L)+B_1(L)t+B_2(L)t^2+B_3(L)t^3+\bigO(t^4),
 \label{eq:G-near-low}
\end{align}
and write
\begin{equation}\label{eq:H-near-low}
 F\compose G=x+C_0(L)+C_1(L)t+C_2(L)t^2+C_3(L)t^3+\bigO(t^4).
\end{equation}
Primes below denote \(\dd/\dd L\).  Expanding either
...
\begin{equation}\label{eq:E-n-r-def}
 E_n(t,L):=\e^{-n\lambda(t,L)}P_n\bigl(L+\lambda(t,L)\bigr)
...
\begin{equation}\label{eq:rho-composition}
 \rho(t,L):=\frac{\partial_t\lambda}{1+\partial_L\lambda}
...
Indeed,
\[
 \partial_tE_n=\lambda_t\e^{-n\lambda}
 \bigl(P_n'-nP_n\bigr)(L+\lambda),
\]
whereas
\[
 \Delta_nE_n=(1+\lambda_L)\e^{-n\lambda}
 \bigl(P_n'-nP_n\bigr)(L+\lambda).
\]
...
If \(G=\e^x\), then inverse powers of \(G\) become exponentially small monomials \(\e^{-nx}\), and \(\log G=x\).  Such substitutions belong naturally to the full logarithmic-exponential transseries field, not to \(\PLL\).
...
\begin{equation}\label{eq:triangular-perturbation}
 ...
 =F\compose G+c(L)t^N+\bigO(t^{N+1}).
\end{equation}
\end{lemma}
...
\[
 ...
 \qquad \delta=c(L)t^N.
\]
Since \(F'=1+\bigO(t)\), the linear term is
\(c(L)t^N+\bigO(t^{N+1})\).  Also \(F''=\bigO(t^2)\), so every term of degree at least two in \(\delta\) has order greater than \(N\).
\end{proof}
...
\begin{equation}\label{eq:reversion-residual-N}
 F\compose G_{<N}=x+r_N(L)t^N+\bigO(t^{N+1}).
\end{equation}
...
Write
\begin{align}
 F={}&x+A_0+A_1t+A_2t^2+A_3t^3+\bigO(t^4),\label{eq:F-inverse-low}\\
 F^{\rev}={}&x+B_0+B_1t+B_2t^2+B_3t^3+\bigO(t^4).
...
\end{equation}
The denominator is a unit because \(F'=1+\bigO(t)\).
...
\begin{equation}\label{eq:Lagrange-reversion}
 ...
 \left(\frac{\dd}{\dd y}\right)^{k-1}\!\bigl(\phi(y)^k\bigr).
\end{equation}
The sum is well defined in \(y+\K[\log y][[y^{-1}]]\).
\end{theorem}

\begin{proof}
Insert an auxiliary parameter \(\varepsilon\) and solve
\[
 y=x+\varepsilon\phi(x),
 \qquad x=y+w.
\]
Then \(w=-\varepsilon\phi(y+w)\).  Ordinary formal Lagrange inversion in \(\varepsilon\) gives
\[
 w=\sum_{k\ge1}\frac{(-\varepsilon)^k}{k!}
 \left(\frac{\dd}{\dd y}\right)^{k-1}\phi(y)^k.
\]
For \(\phi\in\PL\), the \((k-1)\)-st derivative raises \(t\)-order by at least \(k-1\), so the coefficient of each fixed inverse power receives contributions from only finitely many \(k\).  Therefore setting \(\varepsilon=1\) is legitimate in the \(t\)-adic topology and yields \cref{eq:Lagrange-reversion}.
\end{proof}
...
For real \(x\) sufficiently large, let \(\omega(x)\) be the positive solution of
...
\begin{equation}\label{eq:omega-W}
 \omega(x)=W_0(\e^x).
\end{equation}
Thus \(\omega\) is exactly the compositional inverse of
...
\begin{equation}\label{eq:omega-expansion}
 \omega(x)=x+\mathcal P(t,L)
 =x-L+\sum_{n=1}^{\infty}P_n(L)t^n,
...
\begin{equation}\label{eq:P0-def}
 P_0(L):=-L,
 \qquad
 \mathcal P(t,L):=\sum_{n=0}^{\infty}P_n(L)t^n.
\end{equation}
We call \(P_n\) the \emph{Lambert polynomials}.  The terminology is not completely standardized; the explicit coefficients are the classical Stirling-number coefficients in the large-argument expansion of \(W\) \cite{comtet1970,jeffrey-et-al-1995,corless-et-al-1996,dlmf-W}.
...
Substitute \(\omega=x+\mathcal P\) into \cref{eq:omega-equation}:
\[
...
\begin{align}
 P_0(L)&=-L,\label{eq:P-rec-P0}\\
 P_{n+1}'(L)&=nP_n(L)-P_n'(L),\qquad n\ge0,
 \label{eq:P-rec-diff}\\
 P_n(0)&=0,\qquad n\ge0.
 \label{eq:P-zero}
\end{align}
...
\begin{equation}\label{eq:P-rec-integral}
 \boxed{\quad
 P_{n+1}(L)=n\int_0^L P_n(s)\dd s-P_n(L),
 \qquad n\ge0.
 \quad}
\end{equation}
\end{theorem}
...
\begin{proof}
Write \(\mathcal P=\sum_{n\ge0}P_nt^n\) in \cref{eq:P-PDE}.  The coefficient of \(t^0\) gives \(P_0'+1=0\), and the coefficient of \(t^{n+1}\) gives
\(nP_n=P_{n+1}'+P_n'\).  At \(L=0\), the implicit equation becomes
\[
 \mathcal P(t,0)+\log(1+t\mathcal P(t,0))=0.
\]
Its unique \(t\)-adic solution is zero, hence \(P_n(0)=0\).  Integrating \cref{eq:P-rec-diff} from \(0\) to \(L\) gives \cref{eq:P-rec-integral}.
\end{proof}

The first steps are
\begin{align*}
 P_1&=-P_0=L,\\
 P_2&=\int_0^L s\dd s-L=\frac{L^2}{2}-L,\\
 P_3&=2\int_0^L\left(\frac{s^2}{2}-s\right)\dd s
...
For \(n\ge1\):
\begin{enumerate}[label=\textup{(\roman*)}]
\item \(P_n\) has degree \(n\), zero constant term, and alternating nonzero coefficients;
...
\begin{proof}
Let the integral be \(I_n(s)\).  Since \(P_n(0)=0\), integration by parts gives
\[
 \int_0^\infty\e^{-sL}P_n'(L)\dd L=sI_n(s).
\]
...
\]
Because \(I_1(s)=\int_0^\infty\e^{-sL}L\dd L=s^{-2}\), iteration gives
\cref{eq:P-Laplace}.
\end{proof}
...
\begin{equation}\label{eq:P-moment-zero}
 \int_0^\infty\e^{-rL}P_n(L)\dd L=0.
\end{equation}
\end{corollary}

Thus \(P_n\) is orthogonal, in a finite and non-Hilbert-space sense, to the first \(n-1\) exponential weights.  These cancellations encode the same falling-factorial operator that produced the Stirling numbers.

\subsection{Residual verification}

Let
\begin{equation}\label{eq:omega-trunc}
 \omega_N(x)=x-L+\sum_{n=1}^{N}P_n(L)x^{-n}.
\end{equation}
Triangular reversion implies
\begin{equation}\label{eq:omega-residual-formal}
 \omega_N+\log\omega_N-x
 =-P_{N+1}(L)x^{-N-1}+\bigO(x^{-N-2}).
\end{equation}
Indeed, the omitted coefficient \(P_{N+1}\) enters the next residual block with coefficient one.  Formula \cref{eq:omega-residual-formal} is an efficient symbolic unit test for any implementation of the recurrence.

\section{\texorpdfstring{Lambert \(W\), its branches, and generalized Lambert equations}{Lambert W, its branches, and generalized Lambert equations}}
\label{sec:Lambert-W}

\subsection{The standard large-argument expansion}

Fix a branch \(W_k\) and define
\begin{equation}\label{eq:xi-k}
 \xi_k=\Log z+2\pi\ii k,
\end{equation}
where \(\Log\) is the principal logarithm and subsequent logarithms are taken consistently in a sector.  Equation \cref{eq:W-log-equation} is the omega equation with \(x=\xi_k\).  Therefore
\begin{theorem}[Lambert \(W\) expansion]\label{thm:W-expansion}
...
\cref{eq:W-expansion-Stirling} and notes that the series is absolutely convergent for sufficiently large \(\abs z\); for \(k=0\) and real \(z\), it converges for \(z\ge\e\) \cite{dlmf-W}.  Thus this famous example is simultaneously a formal transseries, a Poincar\'e asymptotic expansion, and, in a substantial domain, an actually convergent series.

\subsection{\texorpdfstring{Why the expansion is exact at \(z=\e\)}{Why the expansion is exact at z = e}}

At \(z=\e\), one has \(L_1=1\) and \(L_2=0\).  Since
\(P_n(0)=0\) for every \(n\), every truncation of
\cref{eq:W-first-terms} equals \(1\), which is exactly
\(W_0(\e)\).  This special point does not mean every derivative of the original Stirling series behaves equally well there; alternative convergent rearrangements based on associated Stirling numbers improve local convergence properties \cite{jeffrey-et-al-1995}.

\subsection{\texorpdfstring{The real branch \(W_{-1}\) near zero}{The real branch W-1 near zero}}
...
\begin{equation}\label{eq:Wminus1-expansion}
 \boxed{\quad
 W_{-1}(x)\sim-\eta-\ell
 +\sum_{n=1}^{\infty}(-1)^n\frac{P_n(\ell)}{\eta^n}.
 \quad}
\end{equation}
The first terms are
\begin{align}
 W_{-1}(x)\sim{}&-\eta-\ell-\frac{\ell}{\eta}
...
\subsection{\texorpdfstring{The generalized equation \(y^\alpha\e^y=X\)}{The generalized equation y to the alpha times exp(y) = X}}
...
\cref{eq:generalized-Lambert-exact}, take the \(1/\alpha\)-th power of \cref{eq:generalized-Lambert} and apply the defining equation of \(W\).
\end{proof}
...
\(X>(\alpha\e)^\alpha\) when \(\alpha\ge1\), and \(X>\e\) when \(\alpha<1\) \cite{jeffrey-et-al-1995}.  They also derived rearrangements using associated Stirling numbers that converge more rapidly on larger domains.
...
\begin{equation}\label{eq:WN-def}
 \widehat W_N(z)=L_1-L_2+\sum_{n=1}^{N}\frac{P_n(L_2)}{L_1^n},
\end{equation}
with \(\widehat W_0=L_1-L_2\).  The following absolute errors illustrate both the usefulness of modest order and the nonmonotonicity that may occur at moderate \(z\).

\begin{table}[htbp]
\centering
\small
\caption{Absolute error \(\abs{\widehat W_N(z)-W_0(z)}\), where \(W_0\) denotes the exact principal Lambert branch.}
\label{tab:W-errors}
\begin{tabular}{@{}rccc@{}}
\toprule
...
\begin{equation}\label{eq:W-log-residual}
 \rho_N=\widehat W_N+\log \widehat W_N-L_1
\end{equation}
is often preferable to \(\widehat W_N\e^{\widehat W_N}-z\), because it avoids overflow.  Since the derivative of \(w+\log w-L_1\) is \(1+1/w\), the actual error is approximately
\(-\rho_N/(1+1/\widehat W_N)\) when the residual is small.
...
Let
\begin{align*}
 A&=1+(2L-1)t+(L^2+3)t^2+(L^3-L)t^3+\bigO(t^4),\\
 B&=1+Lt+(L^2-1)t^2+\bigO(t^4).
\end{align*}
Using the reciprocal from \cref{eq:reciprocal-example-C} and multiplying,
\begin{align}
 \frac AB={}&1+(L-1)t+(-L^2+L+4)t^2
 +(L^3-4L-1)t^3+\bigO(t^4).
 \label{eq:worked-quotient}
\end{align}
The same result follows directly from \cref{eq:quotient-recurrence}.  A useful implementation test is to multiply the computed quotient by \(B\) and check equality with \(A\) modulo the requested precision.
...
\begin{equation}\label{eq:xlogx-equation}
 ...
 \qquad y\to+\infty.
\end{equation}
With \(u=\log x\), one has \(u\e^u=y\), hence
\begin{equation}\label{eq:xlogx-exact}
 u=W_0(y),
 \qquad
 x=\e^{W_0(y)}=\frac{y}{W_0(y)}.
\end{equation}
Let \(L_1=\log y\), \(L_2=\log L_1\).  Dividing by the expansion of \(W_0(y)\) gives
\begin{align}
...
\end{align}
This example illustrates a common workflow: series reversal first produces \(W\), and ordinary division then transforms it into the desired inverse.
...
\begin{equation}\label{eq:powerlog-inverse-exact}
 ...
 W_k\!\left(\frac{\rho}{\sigma}
 \left(\frac ya\right)^{1/\sigma}\right),
 \qquad
 x=\exp\!\left[\frac{\sigma}{\rho}W_k(\cdots)\right].
 \quad}
\end{equation}
Expanding \(W_k\) generates nested polynomials in \(\log y\) and \(\log\log y\).  The formula also makes the closure boundary transparent: the inverse is naturally one logarithmic level deeper than the original monomial when expressed in the variable \(y\).
...
\begin{equation}\label{eq:omega3-residual}
 ...
 +\bigO(t^5).
\end{equation}
The residual coefficient is \(-P_4\), exactly as predicted by
\cref{eq:omega-residual-formal}.  Appending \(P_4t^4\) cancels it without changing any lower block.
...
For some functions and domains, the infinite sum converges to the function.  The standard Lambert \(W\) series is an unusually favorable example: on the principal real branch it converges for \(z\ge\e\) \cite{dlmf-W}.  This analytic fact is additional information, not a prerequisite for the formal algebra.
...
\begin{enumerate}
\item \(G(x)=x+A(x)\) with \(A(x)=o(x)\) and a differentiable polynomial-logarithmic expansion;
...
\begin{equation}\label{eq:fprime-to-one}
 f'(x)=1+o(1).
\end{equation}
Let \(g=f^{-1}\), and let \(G_N(y)\to\infty\) satisfy
...
For \(f(x)=x+\log x\), this proves the asymptotic validity of every truncation \(\omega_N\) directly from
...
For complex \(x\), the symbols \(L=\Log x\), powers \(x^\alpha\), and the branch index of \(W_k\) carry analytic metadata not visible in a bare formal coefficient array.  The formal calculation is local to a chosen sector.  When an argument crosses a branch cut, the appropriate logarithm changes by an integer multiple of \(2\pi\ii\), and the representation must be continued accordingly.  In \cref{eq:W-expansion-P}, this information is concentrated in
\(\xi_k=\Log z+2\pi\ii k\) and the subsequent choice of \(\Log\xi_k\).
...
\item store \(n!P_n\) as an integer polynomial when generating Lambert polynomials at high order;
...
\begin{align}
 F\compose G
 &=x+A_0+B_0+B_0A_0't+\bigO(t),\notag\\
 G\compose F
 &=x+A_0+B_0+A_0B_0't+\bigO(t).
\end{align}
Therefore
\begin{equation}\label{eq:composition-commutator-leading}
 F\compose G-G\compose F
 =\bigl(B_0A_0'-A_0B_0'\bigr)t+\bigO(t^2).
\end{equation}
...
\(\frac1q\Z\) or \(\R\).  Multiplication uses the same convolution over
...
\begin{enumerate}
\item \textbf{Find the dominant scale.}  Solve the leading equation, possibly using a power, logarithm, exponential, or Lambert \(W\) function.
...
\begin{align*}
 \omega(x)&=x-L+\sum_{n\ge1}P_n(L)x^{-n},\\
 \mathcal P+L+\log(1+t\mathcal P)&=0,\\
 P_{n+1}&=n\int_0^LP_n(s)\dd s-P_n(L),\\
 P_n(L)&=\sum_{m=1}^n(-1)^{n+m}
 \stirlingone{n}{n-m+1}\frac{L^m}{m!},\\
 W_k(z)&\sim\xi_k-\Log\xi_k+
 \sum_{n\ge1}\frac{P_n(\Log\xi_k)}{\xi_k^n}.
\end{align*}
The same \(P_n\) govern \(W_{-1}\) near zero and the inverse of
\(y^\alpha\e^y\).
\end{keybox}
...
\(\D\) & total derivative \(\dd/\dd x=t\partial_L-t^2\partial_t\)\\
\(\Delta_n\) & shifted coefficient operator \(\partial_L-n\)\\
\(F^{\rev}\) & compositional inverse of \(F\)\\
\(P_n(L)\) & \(n\)-th Lambert polynomial, with \(P_0=-L\)\\
\(\stirlingone{n}{k}\) & unsigned Stirling cycle number of the first kind\\
\(\xi_k\) & \(\Log z+2\pi\ii k\), the large branch parameter for \(W_k(z)\)\\
\bottomrule
\end{longtable}
