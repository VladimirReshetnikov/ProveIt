...
\newcommand{\Plog}{\mathscr{P}}
\newcommand{\Rlog}{\mathscr{R}}
\newcommand{\Hlog}{\mathscr{H}}
\newcommand{\Lclass}{\widehat{\mathcal{L}}}
...
Finally, the Lambert \(W\) expansion is derived in several ways. Its coefficient polynomials are obtained recursively, by Lagrange inversion, and in closed form through unsigned Stirling cycle numbers. The treatment covers all complex branches, the real branch \(W_{-1}\) near the origin, the different Puiseux scale at the branch point \(-e^{-1}\), algebraic operations on truncated \(W\)-transseries, numerical residuals, and implementable sparse algorithms. Appendices provide coefficient tables through order twelve, proof templates, pseudocode, and a notation concordance.
...
\]
For Lambert \(W(z)\), the same algebra is used one level down by taking \(X=L_1=\Log z\) and \(\lambda=L_2=\Log L_1\). Near a small positive variable \(x\to0^+\), one may instead put \(X=x^{-1}\), or use the Dulac convention \(\ell=-1/\log x\).
...
\begin{equation}\label{eq:Wlog-preview}
 W+\Log W=\Log z.
\end{equation}
Set \(L_1=\Log z\). Since \(\Log W\) is smaller than \(W\), dominant balance gives \(W\sim L_1\). Feeding that approximation into the smaller term gives the improved balance
\[
 W\sim L_1-L_2,
 \qquad L_2=\Log L_1.
\]
...
\begin{equation}\label{eq:preview-ansatz}
 u=\sum_{n\ge1}\frac{P_n(L_2)}{L_1^n},
 \qquad P_n\in\Q[L_2].
\end{equation}
...
\end{equation}
This inverse is the Wright \(\omega\) function. The coefficient polynomials in \cref{eq:omega-preview} are precisely the same universal polynomials that occur in the large-argument expansion of Lambert \(W\).
...
\end{equation}
with a finite lower bound \(n_0\in\Z\).
\end{definition}
...
\end{equation}
where \(S\subset\R\) is well ordered and each \(P_\alpha\) is a polynomial or Laurent series. This is a Hahn-type power--log series. Dulac transseries near \(x=0^+\) are often written
\[
...
\foreach \i/\lab in {0/{X},1/{(\log X)^N},2/{1},3/{(\log X)^{-N}},4/{X^{-1}},5/{X^{-2}},6/{e^{-X}}} {
...
\[
 \widehat f(X)=\sum_{n\ge n_0}A_n(\log X)X^{-n}.
\]
To say that \(\widehat f\) is a Poincar\'e asymptotic expansion in this scale means, at minimum, that for every \(N\),
...
\[
 \mathcal E(f_{<N},X)=\bigO(t^N Q_N(\lambda)).
\]
...
Over \(\C\), logarithms and fractional powers require branch choices. A formula such as
\[
 \Log\bigl(cX^\beta(\log X)^\gamma\bigr)
 =\Log c+\beta\Log X+\gamma\Log\Log X
\]
...
\]
The same universal coefficient polynomials apply on every branch, but \(\Log\xi_k\) and the sector of validity depend on \(k\).
...
Consider
\begin{align*}
 F&=1+(\lambda+1)t+(2\lambda^2-\lambda)t^2+\bigO(t^3),\\
 G&=1+(2-\lambda)t+(\lambda^2+3)t^2+\bigO(t^3).
\end{align*}
...
\begin{equation}\label{eq:worked-product}
 FG=1+3t+(2\lambda^2+5)t^2+\bigO(t^3).
\end{equation}
...
Let \(S,T\subset\R\) be well-ordered supports and consider
\[
...
Return to
\begin{align*}
 A&=1+(\lambda+1)t+(2\lambda^2-\lambda)t^2+\bigO(t^3),\\
 B&=1+(2-\lambda)t+(\lambda^2+3)t^2+\bigO(t^3).
\end{align*}
...
\begin{equation}\label{eq:worked-quotient}
 \frac AB
 =1+(2\lambda-1)t+(3\lambda^2-6\lambda-1)t^2+\bigO(t^3).
\end{equation}
...
\end{align}
No new scale appears: the leading block of \(W/L\) is the scalar one. This simple observation underlies expansions for \(e^W=z/W\), \(W'\), and rational functions of \(W\).
...
function divide(A, B, cutoff N):
    ...
    return C
\end{lstlisting}
For dense inputs and naive polynomial arithmetic, the block convolution costs \(O(N^2)\) polynomial multiplications. Newton inversion of formal power series can reduce asymptotic complexity when fast polynomial multiplication is available, but the triangular recurrence is transparent and usually preferable at the moderate orders encountered in asymptotics.
...
The algebra \(\Plog\) is closed under \(\dd/\dd X\). For
\[
 F=\sum_{n\ge n_0}A_n(\lambda)t^n,
\]
we have
...
\begin{equation}\label{eq:repeated-derivative-block}
 \frac{\dd^k}{\dd X^k}\bigl(t^nP(\lambda)\bigr)
 =t^{n+k}
...
\(\log(1+U)\), \(e^U\), \((1+U)^\alpha\), \(U=O(t)\) & closed & closed & none\\
...
Suppose \(F\) has lower valuation \(n_0\), \(U=O(t)\), and we seek \(F\circ G\pmod{t^N}\). In \cref{eq:exact-block-composition}, the term indexed by \(n\) begins at \(t^n\). Hence only \(n<N\) are needed. Within that term, \(V=\log(1+U)\) and \(R=(1+U)^{-1}\) are needed only through order \(t^{N-n-1}\). This gives an exact guard-term policy.
...
When \(c\in\C^\times\), choose a value of \(\Log c\) and a sector on which \(G\) avoids the logarithmic cut. Formula \cref{eq:lambda-compose-G} then uses that chosen branch. Changing \(\Log c\) by \(2\pi\ii j\) changes the constant part of the composed logarithm and can move the result between branches of a multivalued inverse.

The formal block manipulations are identical after the branch is fixed. A symbolic implementation should therefore carry the branch constant as data rather than silently replacing \(\Log c\) by a principal value.
...
\begin{equation}\label{eq:taylor-difference}
 F(X+H)-F(X+K)
 =(H-K)\int_0^1F'\bigl(X+K+s(H-K)\bigr)\dd s
\end{equation}
...
\begin{warningbox}
For polynomial--log blocks, \(H\in\K[\lambda][[t]]\) is a robust sufficient condition. In a deeper transseries containing, for example, \(e^{e^X}\), a perturbation that is small compared with \(X\) may still be enormous on the variation scale of the outer monomial.
\end{warningbox}
...
\end{exercise}
\begin{exercise}
For \(F=e^{-X}\) in the larger transseries field and \(H=\log X\), show that the Taylor series is summable and agrees with \(e^{-X}X^{-1}\). Compare the relevant logarithmic derivative with the case \(F=e^{e^X}\).
\end{exercise}

\begin{summarybox}
For the near-identity increment \(H\in\K[\lambda][[t]]\), formal Taylor expansion is an exact identity because each derivative raises the primary power order. The difference estimate gains one order when the outer derivative is \(O(t)\), providing the contraction needed for series reversal. Deeper transseries require a logarithmic-derivative test.
\end{summarybox}
...
\begin{equation}\label{eq:inverse-fixed-point}
 K=-H(X+K).
\end{equation}
The derivative \(H'\) is \(O(t)\), so the right side is contractive in the \(t\)-adic sense.
...
\[
 K_{j+1}=-H(X+K_j)
\]
is correct through \(t^N\). More precisely, if \(K_*-K_j=O(t^N)\), where \(K_*\) is the exact fixed point, then
\[
 K_*-K_{j+1}
 =\Phi(K_*)-\Phi(K_j)=O(t^{N+1}).
\]
...
\]
\item stop when \(K_{j+1}=K_j\pmod{t^N}\);
\item return \(G=X+K_j\) and verify \(F\circ G-X=O(t^N)\).
\end{enumerate}
\end{algorithm}
...
\]
The block \(K_n\) appears on the right side of \cref{eq:inverse-fixed-point} only after multiplication by \(H'=O(t)\), so it cannot affect the coefficient of \(t^n\). Therefore the equation for \(K_n\) depends only on \(K_0,\ldots,K_{n-1}\). This is the compositional analogue of the triangular quotient recursion.
...
\end{exercise}
\begin{exercise}
Show that the inverse of \(X+H\), where \(H=O(t^q)\) for \(q\ge1\), has the form \(X-H+O(t^{2q+1})\).
\end{exercise}
...
\[
 Y=X-\lambda+U
\]
and deriving an equation for \(U=O(t\lambda)\) begins one scale closer to the answer.
...
\end{equation}
For \(F=X+H\), we have \(F'=1+O(t)\), so the denominator is a unit.

\begin{proposition}[Quadratic valuation improvement]\label{prop:newton-improvement}
Suppose \(F=X+H\), \(H\in\K[\lambda][[t]]\), and \(R=F\circ G-X=O(t^q)\) with \(q\ge0\). Then the Newton update satisfies
\begin{equation}\label{eq:newton-improvement}
 F\circ G_{\mathrm{new}}-X=O(t^{2q+2}).
\end{equation}
...
\begin{proof}
The correction \(\Delta\) has valuation at least \(q\). The constant and linear residual terms cancel by construction. Since \(F''=H''=O(t^2)\), the quadratic Taylor term has valuation at least \(2+2q\); all higher terms are deeper.
\end{proof}
...
\begin{equation}\label{eq:lagrange-infinity}
 ...
 \frac{\dd^{n-1}}{\dd X^{n-1}}\bigl(H(X)^n\bigr).
\end{equation}
...
\end{align}
The function \(\omega\) is Wright's omega function, characterized by
\begin{equation}\label{eq:omega-equation}
 \omega(X)+\log\omega(X)=X.
\end{equation}
On compatible branches, \(\omega(X)=W(e^X)\).
...
\begin{equation}\label{eq:affine-log-F}
 F(X)=X+a\lambda+b+(c\lambda+d)t+\bigO(t^2).
\end{equation}
...
\[
 K_1=-\lambda-\lambda t.
\]
To extract the second correct block, put \(K=-\lambda+A(\lambda)t+B(\lambda)t^2+\bigO(t^3)\), expand the right side, and compare coefficients. One obtains
\[
 A(\lambda)=0,
 \qquad
...
\[
 F\invcomp
 =X-\lambda+\left(\lambda-\frac12\lambda^2\right)t^2+\bigO(t^3).
\]
...
\section{Exercises}
\begin{exercise}
Derive the \(X^{-6}\) block of \(\omega(X)\) using \cref{eq:omega-lagrange}.
\end{exercise}
\begin{exercise}
...
\begin{summarybox}
The inverse of \(X+\log X\) is Wright \(\omega\), and its block polynomials recur throughout Lambert \(W\) theory. Affine-log perturbations admit explicit parameter formulas, pure reciprocal examples connect to Catalan numbers, and tangent-to-identity Dulac transseries reverse within their power--log class.
\end{summarybox}
...
\begin{equation}\label{eq:dulac-leading}
 ...
 \qquad x\to0^+.
\end{equation}
Put \(s=-\log x\), so \(x=e^{-s}\) and \(\ell=s^{-1}\). Then
\[
 y/a=e^{-\alpha s}s^{-m}.
\]
For \(m\ne0\), raise to the power \(-1/m\):
\[
 (y/a)^{-1/m}=s e^{(\alpha/m)s}.
\]
Therefore
...
\item signs of \(a/b\) and the power \((Y/c)^{1/b}\) determine whether the argument crosses \(-e^{-1}\);
...
\[
 w\longmapsto we^w.
\]
Its branches \(W_k\), \(k\in\Z\), satisfy
\begin{equation}\label{eq:W-definition}
 W_k(z)e^{W_k(z)}=z.
\end{equation}
On \([0,\infty)\), the principal branch \(W_0\) is real, nonnegative, and increasing. On \((-e^{-1},0)\), there are two real values: \(W_0\in(-1,0)\) and \(W_{-1}\in(-\infty,-1)\) \cite{corless-W,dlmf-W}.
...
Fix a branch of \(\Log z\). For the branch \(W_k\), define
\begin{equation}\label{eq:xi-k}
 \xi_k=\Log z+2\pi\ii k,
 \qquad
 \ell_k=\Log\xi_k.
\end{equation}
Taking a compatible logarithm of \(W_ke^{W_k}=z\) gives
\begin{equation}\label{eq:W-log-branch}
 W_k+\Log W_k=\xi_k.
\end{equation}
As \(\abs{z}\to\infty\) in an admissible sector, \(W_k\sim\xi_k\), then \(\Log W_k\sim\ell_k\), so
...
\begin{equation}\label{eq:omega-def}
 \omega(s)+\Log\omega(s)=s.
\end{equation}
On compatible branches,
\begin{equation}\label{eq:omega-W}
 \omega(s)=W(e^s).
\end{equation}
Thus \(\omega\) is the compositional inverse of \(y\mapsto y+\Log y\), while \(W\) is the inverse of \(y\mapsto ye^y\). The substitution \(s=\Log z\) turns one inversion problem into the other.

The near-identity reversal formula of \cref{eq:omega-expansion} applies directly to \(\omega(s)\), because \(s+\log s\) has identity leading term. This is the cleanest route to the coefficient polynomials of \(W\).
...
The identity \(W(ze^z)=z\) holds only when the branch of \(W\) is chosen so that \(z\) lies in the corresponding inverse sheet. Similarly, \(W(e^s)=\omega(s)\) is a local identity with an unwinding-number convention. The formal expansion records one chosen sheet through the integer \(k\); crossing a branch cut changes that data.

\begin{summarybox}
Every large branch of Lambert \(W\) is governed by \(W+\Log W=\xi_k\), with \(\xi_k=\Log z+2\pi i k\). Removing the balances \(\xi_k\) and \(-\Log\xi_k\) leaves a contractive equation in \(1/\xi_k\). Wright omega is the near-identity inverse that exposes this structure directly.
\end{summarybox}
...
\end{equation}
The coefficient of \(r^n\) depends only on \(P_1,\ldots,P_{n-1}\). A compact recursion is
\begin{equation}\label{eq:Pn-coefficient-recursion}
 P_n(\ell)=
 [r^n]\left\{-\Log\left(1-r\left(\ell-
 \sum_{q=1}^{n-1}P_q(\ell)r^q\right)\right)\right\}.
\end{equation}
Terms of degree \(r^n\) and higher inside the provisional \(u\) cannot affect \(P_n\).

At the first two orders,
\begin{align*}
 P_1&=\ell,\\
 P_2&=-P_1+\frac12\ell^2
 =\frac12\ell(\ell-2).
\end{align*}
Continuing gives
\begin{align}
 P_3(\ell)&=\frac{\ell(2\ell^2-9\ell+6)}6,\label{eq:P3}\\
 P_4(\ell)&=\frac{\ell(3\ell^3-22\ell^2+36\ell-12)}{12},\label{eq:P4}\\
...
\begin{equation}\label{eq:omega-lagrange-again}
 \omega(s)=s+\sum_{n\ge1}\frac{(-1)^n}{n!}
 \frac{\dd^{n-1}}{\dd s^{n-1}}(\log s)^n.
\end{equation}
...
\begin{equation}\label{eq:Pn-derivative}
 P_q(\ell)=
 \frac{(-1)^{q+1}}{(q+1)!}
 s^q\frac{\dd^q}{\dd s^q}(\log s)^{q+1},
 \qquad \ell=\log s.
\end{equation}
...
\section{Structural properties of \texorpdfstring{\(P_n\)}{Pn}}
...
\begin{equation}\label{eq:Pn-edge-coefficients}
 P_n(\ell)=\frac{\ell^n}{n}-H_{n-1}\ell^{n-1}
 +\cdots+(-1)^{n-1}\ell.
\end{equation}
...
\begin{equation}\label{eq:W-Pn-expansion}
 W_k(z)=\xi_k-\ell_k+
 \sum_{n\ge1}\frac{P_n(\ell_k)}{\xi_k^n},
 \qquad
 \xi_k=\Log z+2\pi\ii k,
 \quad \ell_k=\Log\xi_k.
\end{equation}
...
\begin{align}
 ...
 +\bigO\left(\frac{\ell^6}{L^6}\right),
\label{eq:W-five-corrections}
\end{align}
where \(L=\xi_k\) and \(\ell=\Log L\).

\section{Residual order}
Let
\begin{equation}\label{eq:WN}
 W_{k,N}=L-\ell+\sum_{n=1}^{N}\frac{P_n(\ell)}{L^n}.
\end{equation}
By construction,
...
\end{equation}
The leading coefficient of the residual can be computed by carrying one additional block. Since the derivative of \(w+\Log w-L\) with respect to \(w\) is \(1+1/w=1+O(L^{-1})\), the solution error has the same leading order away from the branch point.
...
\end{equation}
with \(\stirlingone{0}{0}=1\). Therefore all \(P_1,\ldots,P_N\) can be generated by one triangular integer table followed by division by \(m!\). This is usually faster and more stable than repeated symbolic differentiation.
...
\section{Exercises}
\begin{exercise}
Use \cref{eq:Pn-coefficient-recursion} to derive \(P_3\) and \(P_4\) without Stirling numbers.
\end{exercise}
\begin{exercise}
Prove the leading, next-to-leading, and linear coefficient statements in \cref{eq:Pn-edge-coefficients}.
\end{exercise}
\begin{exercise}
Show directly from \cref{eq:Pn-derivative} that \(P_n\) has rational coefficients and no constant term.
\end{exercise}
...
Let \(L=\xi_k\), \(\ell=\Log L\), and \(r=L^{-1}\). Factoring \(W=L(1+O(r\ell))\) and using geometric division gives
\begin{align}
 \frac1W={}&r+\ell r^2+(\ell^2-\ell)r^3
 +\left(\ell^3-\frac52\ell^2+\ell\right)r^4\notag\\
 &+\left(\ell^4-\frac{13}{3}\ell^3+\frac92\ell^2-\ell\right)r^5\notag\\
...
\begin{equation}\label{eq:expW}
 e^{W_k(z)}=\frac{z}{W_k(z)}.
\end{equation}
Thus \cref{eq:W-reciprocal} immediately yields a transseries for \(e^W\) without exponentiating a long series.
...
\begin{align}
 W_k(z)e^{W_k(z)}&=z,\label{eq:W-check1}\\
 W(ye^y)&=y,\label{eq:W-check2}\\
 \omega(s)+\Log\omega(s)&=s,\label{eq:W-check3}\\
 \omega(s)&=W(e^s),\label{eq:W-check4}
\end{align}
...
\[
 R_N=W_N+\Log W_N-L
\]
is usually numerically preferable to the multiplicative residual \(W_Ne^{W_N}-z\), which may overflow when \(z\) is large. The two are related by
\[
 W_Ne^{W_N}=z e^{R_N}.
\]
...
\end{exercise}
\begin{exercise}
Use \(\Log W=L-W\) to verify \cref{eq:logW-series} without taking a logarithm of a series.
\end{exercise}
\begin{exercise}
...
\begin{summarybox}
Once the base Lambert expansion is available, reciprocals, powers, logarithms, derivatives, and rational functions follow from the block arithmetic already developed. Exact identities such as \(\Log W=L-W\) and \(e^W=z/W\) minimize unnecessary expansion and provide strong residual checks.
\end{summarybox}
...
\begin{equation}\label{eq:Wminus1-Pn}
 W_{-1}(-e^{-\eta})
 \sim-\eta-L+
 \sum_{n\ge1}\frac{(-1)^nP_n(L)}{\eta^n}.
\end{equation}
The first terms are
\begin{align}
 W_{-1}(-e^{-\eta})={}&-\eta-L-\frac{L}{\eta}
 +\frac{L(L-2)}{2\eta^2}\notag\\
...
The same \(P_n\) occur, multiplied by \((-1)^n\). The reason is structural: \(X+\log X\) and \(X-\log X\) differ only by the sign of the perturbation in Lagrange inversion. This universality lets one generate both branches from one coefficient table.
...
\section{The branch point \texorpdfstring{\(-e^{-1}\)}{-1/e}}
At \(z=-e^{-1}\), both real branches meet at \(W=-1\), and \cref{eq:W-derivative-exact} has a pole because \(1+W=0\). The logarithmic expansion is not the correct local scale.
...
\end{align}
The branches are related by \(p\mapsto-p\). A square root, not an iterated logarithm, is forced by the quadratic critical point of \(we^w\) at \(w=-1\).
...
Therefore
\[
 1+ez=\frac12q^2+O(q^3),
\]
so \(q\sim\pm\sqrt{2(1+ez)}=\pm p\). Ordinary series reversion in \(p\) produces \cref{eq:W0-branchpoint,eq:Wm1-branchpoint}.
...
\end{equation}
Its radius of convergence is \(e^{-1}\), set by the branch point. Thus three different regimes of the same function require three different scales:
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
Regime & Natural scale & Expansion type\\
\midrule
\(z\to\infty\) & \(1/\Log z\), \(\Log\Log z\) & polynomial--log\\
\(z\to0\) on \(W_0\) & \(z\) & ordinary power series\\
\(z\to-e^{-1}\) & \(\sqrt{1+ez}\) & Puiseux series\\
...
\end{exercise}
\begin{exercise}
Substitute \(W=-1+q\) into \(We^W=z\) and derive the coefficients through \(p^3\) in \cref{eq:W0-branchpoint}.
\end{exercise}
\begin{exercise}
...
Formula \cref{eq:W-DLMF-form} is a Poincar\'e asymptotic expansion as \(\abs{z}\to\infty\) on a fixed branch. In addition, the standard series is absolutely convergent for sufficiently large \(\abs z\); for \(k=0\) and real \(z\), it converges for \(z\ge e\) \cite{dlmf-W,corless-W}. These are analytic facts beyond the purely formal derivation.
...
For real \(z=e^L\), define
\[
 \widehat W_N=L-\ell+\sum_{n=1}^N\frac{P_n(\ell)}{L^n},
 \qquad \ell=\log L.
\]
The logarithmic residual is
\begin{equation}\label{eq:numeric-residual}
 \rho_N=\widehat W_N+\log\widehat W_N-L.
\end{equation}
...
\end{equation}
for some \(\theta_N\) between \(W\) and \(\widehat W_N\). For large positive \(L\), the denominator is close to one, so the residual is an excellent error proxy.

One Newton correction,
\begin{equation}\label{eq:W-one-Newton}
 \widehat W_N^{\mathrm{new}}
 =\widehat W_N-
 \frac{\widehat W_N+\log\widehat W_N-L}
 {1+1/\widehat W_N},
\end{equation}
often squares the number of accurate digits.

\section{Numerical examples on the principal branch}
The following values were computed at high precision from the exact \(W_0(e^L)\). Here \(N=0\) means the two-term approximation \(L-\log L\).

\begin{table}[htbp]
\centering
\caption{Absolute error of the polynomial--log truncation for \(W_0(e^L)\)}\label{tab:W-errors}
\footnotesize
...
\begin{align*}
 W_0(e^{10})&=7.9294200950196973486,\\
 W_0(e^{50})&=46.167719165492089670,\\
 W_0(e^{100})&=95.441486645575831840.
\end{align*}
...
\begin{align*}
 \abs{\rho_5}&=5.7811\times10^{-6},&&L=10,\\
 \abs{\rho_5}&=2.3115\times10^{-9},&&L=50,\\
 \abs{\rho_5}&=1.5584\times10^{-10},&&L=100.
\end{align*}
These are close to the actual errors because \(1+1/W\) is close to one. The multiplicative relative residual is
\[
 \frac{\widehat W_Ne^{\widehat W_N}}{e^L}-1=e^{\rho_N}-1,
\]
which is approximately \(\rho_N\) when the residual is small.

\section{The lower branch}
For \(x=-e^{-\eta}\), the expansion \cref{eq:Wminus1-Pn} behaves similarly. At \(\eta=10\), the absolute errors for \(N=0,1,3,5,8\) are approximately
\[
...
\item Near \(-e^{-1}\), use the Puiseux expansion to initialize Newton or Halley iteration.
\item For large \(\abs z\), use \cref{eq:W-Pn-expansion} on the desired branch.
\item Compute the logarithmic residual \(w+\Log w-\xi_k\).
\item Apply one or two Newton or Halley corrections if numerical accuracy beyond the formal truncation is desired.
\end{enumerate}

Halley's method for \(we^w-z=0\) is commonly used because it has cubic local convergence. The transseries expansion supplies an excellent branch-aware starting value.
...
\item Branch constants, such as \(2\pi \ii k\), are never silently discarded.
\end{enumerate}
...
If \(s_A\) and \(s_B\) are the numbers of nonzero blocks, the naive sparse cost is \(O(s_As_B)\) block multiplications. Dense convolution, Karatsuba methods, or FFT multiplication become relevant only when the exponent interval is long and most blocks are nonzero. In typical asymptotic work the logarithmic blocks grow rapidly in size while the number of power blocks remains modest, so coefficient simplification is often more expensive than the convolution itself.
...
\begin{equation}\label{eq:Taylor-budget}
 k\le \left\lfloor\frac{N-r}{h+1}\right\rfloor,
\end{equation}
...
\end{enumerate}
In particular, identities such as \(\Log(e^X)=X\) are branch-sensitive over \(\C\) and should not be applied by a purely algebraic simplifier without hypotheses.
...
\[
 R_W=w+\Log w-\xi_k
\]
is usually better conditioned than evaluating \(we^w-z\), which may overflow even when \(w\) is accurate.
...
Let
\[
 u_N=\sum_{n=1}^{N}P_n(\ell)r^n,
 \qquad
 \Phi(u)=-\log(1-r(\ell-u)).
\]
The recurrence is verified by checking
\begin{equation}\label{eq:Lambert-coeff-residual}
 u_N-\Phi(u_N)=\bigO(r^{N+1}).
\end{equation}
This single identity simultaneously verifies every coefficient polynomial through \(P_N\). A second independent check uses the closed Stirling formula. Agreement of two derivations is valuable, but \cref{eq:Lambert-coeff-residual} is the direct defining certificate.
...
Discarding branch constants & A missing \(2\pi\ii k\) changes the constant block and invalidates every subsequent coefficient.\\
...
\end{equation}
where \(S\subset\R\) or an ordered abelian group. Multiplication still uses convolution,
\[
...
\begin{equation}\label{eq:Dulac-derivative}
 \frac{\dd}{\dd x}\bigl(x^\alpha\ell^m\bigr)
...
\end{equation}
with \(a,b,c\ne0\). Let \(X_0\) denote the exact inverse of the leading factor, expressed by Lambert \(W\). Set \(X=X_0e^\eta\). Taking logarithms of \cref{eq:powerlog-corrected} and using the defining equation for \(X_0\) gives
\[
...
\begin{equation}\label{eq:powerlog-correction}
 \eta=-\frac{d}{a\log X_0}+\bigO\left((\log X_0)^{-2}\right).
\end{equation}
Thus
\[
...
\end{itemize}
It does not by itself contain exponentially small terms such as \(e^{-X}\), nested exponentials such as \(e^{e^X}\), or arbitrary mixtures of exponential scales. Those belong to broader logarithmic--exponential transseries fields.
...
\begin{equation}\label{eq:Pn-skeleton}
 P_n(\ell)=\frac{\ell^n}{n}+\cdots+(-1)^{n-1}\ell.
\end{equation}
...
\]
Using \(1-r(\ell-u)=e^{-u}\), we obtain
\begin{equation}\label{eq:Lambert-generating-differential}
 u_\ell=\frac{re^u}{1+re^u}.
\end{equation}
Together with the original fixed-point equation, this is an efficient bivariate generating relation for the derivatives \(P_n'(\ell)\). Equating coefficients gives a triangular recurrence requiring only products of already known blocks. It also proves \(P_n(0)=0\) for every \(n\ge1\).
...
\item \textbf{Coefficient combinatorics.} Develop analogous Stirling- or Bell-polynomial formulas for inverses of \(X+a\log X+\sum_jP_j(\log X)X^{-j}\).
...
\end{exercise}
\begin{exercise}
Expand \cref{eq:Lambert-generating-differential} through \(r^4\) and compare with the derivatives of the displayed polynomials \(P_1,\ldots,P_4\).
\end{exercise}
\begin{exercise}
...
\begin{equation}\label{eq:appendix-Pn}
 P_n(\ell)=\sum_{m=1}^{n}\frac{c_{n,m}}{m!}\ell^m,
 \qquad
 c_{n,m}=(-1)^{n-m}\stirlingone{n}{n-m+1}.
\end{equation}
The integer row \((c_{n,1},\ldots,c_{n,n})\) therefore reconstructs \(P_n\) exactly after division of the \(m\)-th entry by \(m!\).
...
P_5(\ell)={}&\frac{\ell(12\ell^4-125\ell^3+350\ell^2-300\ell+60)}{60},\\
P_6(\ell)={}&\frac{\ell(20\ell^5-274\ell^4+1125\ell^3-1700\ell^2+900\ell-120)}{120}.
\end{align*}
...
\begin{equation}\label{eq:order12-template}
 W_k(z)=\xi_k-\ell_k+
 \sum_{n=1}^{12}\frac{P_n(\ell_k)}{\xi_k^n}
 +\bigO\left(\frac{\ell_k^{13}}{\xi_k^{13}}\right),
\end{equation}
with branch-compatible logarithms. The displayed remainder indicates formal scale; a uniform analytic error bound requires a specified sector and distance from branch cuts.
...
\]
\[
 W_k(z)\sim\xi_k-\ell_k+\sum_{n\ge1}\frac{P_n(\ell_k)}{\xi_k^n},
\]
\[
 P_n(\ell)=\sum_{m=1}^{n}
 \frac{(-1)^{n-m}}{m!}\stirlingone{n}{n-m+1}\ell^m.
...
\(L_1=\Log z\) & First logarithm in the Lambert \(W\) expansion.\\
\(L_2=\Log L_1\) & Second logarithm in the Lambert \(W\) expansion.\\
\(\xi_k=\Log z+2\pi\ii k\) & Branch-adjusted large parameter for \(W_k\).\\
\(\ell_k=\Log\xi_k\) & Logarithm of the branch-adjusted parameter.\\
...
\(\supp(F)\) & Set of exponents or monomials with nonzero coefficients.\\
\(F^{-1}\) & Multiplicative reciprocal.\\
\(F\invcomp\) & Compositional inverse or series reversal.\\
\(P_n\) & Universal coefficient polynomial in the large-argument expansion of Lambert \(W\).\\
...
\(\bigO(t^N)\) & Formal or analytic remainder, according to the explicitly stated context.\\
\bottomrule
\end{longtable}
