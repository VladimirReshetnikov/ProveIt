...
For \(G=cx+d+\FormalBigO_t(t)\), factor
\[
  G=cx\left(1+\frac{d}{cx}+\FormalBigO_t(t^2)\right).
\]
Then
\begin{align*}
  \log G
...
contraction is visible in the elementary fact \(A'=\FormalBigO_t(t)\).
...
\begin{equation}\label{eq:residual-order-N}
  F\circ G_N-x=r_N(\ell)t^N+\FormalBigO_t(t^{N+1}).
\end{equation}
Because \(F'\circ G_N=1+\FormalBigO_t(t)\), adding
\(-r_Nt^N\) to \(G_N\) removes the residual at level \(N\):
\begin{equation}\label{eq:triangular-update}
  G_{N+1}=G_N-r_N(\ell)t^N
  \quad\Longrightarrow\quad
  F\circ G_{N+1}=x+\FormalBigO_t(t^{N+1}).
\end{equation}
...
\end{equation}
The denominator is a unit because \(F'=1+\FormalBigO_t(t)\).  If
...
Let \(R=F\circ G-x\) and \(C=-R/(F'\circ G)\).  Since \(C=\FormalBigO_t(t^N)\),
\[
 F\circ(G+C)-x
 =R+(F'\circ G)C+\frac12(F''\circ G)C^2+\cdots.
\]
The first two terms cancel.  Now \(F''=A''=\FormalBigO_t(t^2)\), so the quadratic
remainder is at least \(\FormalBigO_t(t^{2N+2})\); allowing coarse bounds and Laurent
...
\(1+\WrightOmega=x\bigl(1+\FormalBigO_t(t)\bigr)\), its reciprocal lies in \(\PL\);
...
\(\ell+1+\FormalBigO_t(t)\), the logarithm would begin with \(\log\ell\) and leave the
...
\begin{equation}\label{eq:Lambert-residual-staircase}
  f(G_N)-x=-\LambertPolynomial{N}(\ell)t^N+\FormalBigO_t(t^{N+1}).
\end{equation}
...
\[
  F\circ G_N-F\circ G
  =(F'\circ G)q(\ell)t^m+\FormalBigO_t(t^{m+1}).
\]
Because \(F'\circ G=1+\FormalBigO_t(t)\), the coefficient at level \(m\) would be
