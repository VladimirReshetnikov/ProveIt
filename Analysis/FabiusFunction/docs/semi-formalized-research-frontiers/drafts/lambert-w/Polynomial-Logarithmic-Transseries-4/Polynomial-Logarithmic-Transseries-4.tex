...
The support of \(F\) is
\[
  \SupportOf{F}=\{(n,j):\TransseriesCoefficientOf{t^n\ell^j}{F}\ne0\}.
\]
For every fixed \(n\), its fiber in \(j\) is finite, and the set of occurring
\(n\)'s is bounded below.  Consequently every nonempty subset of \(\SupportOf{F}\)
...
\(1+\WrightOmega=x\bigl(1+\BigO(t\ell)\bigr)\), its reciprocal lies in \(\PL\).
...
polynomials, producing \(\LambertPolynomial{0},\ldots,\LambertPolynomial{N}\) takes \(O(N^2)\) scalar arithmetic
...
\begin{equation}\label{eq:Lambert-residual-staircase}
  f(G_N)-x=-\LambertPolynomial{N}(\ell)t^N+O(t^{N+1}\ell^{N+1}).
\end{equation}
