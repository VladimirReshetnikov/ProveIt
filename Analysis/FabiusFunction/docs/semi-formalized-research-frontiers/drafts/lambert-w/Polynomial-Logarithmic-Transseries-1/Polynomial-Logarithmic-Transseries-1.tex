...
\(\log(1+U)\), \(\EulerE^U\), \((1+U)^\alpha\), \(U=\BigO(t)\) & closed & closed & none\\
...
Suppose \(F\) has lower valuation \(n_0\), \(U=\BigO(t)\), and we seek \(F\circ G\pmod{t^N}\). In \cref{eq:exact-block-composition}, the term indexed by \(n\) begins at \(t^n\). Hence only \(n<N\) are needed. Within that term, \(V=\log(1+U)\) and \(R=(1+U)^{-1}\) are needed only through order \(t^{N-n-1}\). This gives an exact guard-term policy.
...
\begin{summarybox}
For the near-identity increment \(H\in\K[\lambda][[t]]\), formal Taylor expansion is an exact identity because each derivative raises the primary power order. The difference estimate gains one order when the outer derivative is \(\BigO(t)\), providing the contraction needed for series reversal. Deeper transseries require a logarithmic-derivative test.
\end{summarybox}
...
\begin{equation}\label{eq:inverse-fixed-point}
 K=-H(X+K).
\end{equation}
The derivative \(H'\) is \(\BigO(t)\), so the right side is contractive in the \(t\)-adic sense.
...
\[
 K_{j+1}=-H(X+K_j)
\]
is correct through \(t^N\). More precisely, if \(K_*-K_j=\BigO(t^N)\), where \(K_*\) is the exact fixed point, then
\[
 K_*-K_{j+1}
 =\Phi(K_*)-\Phi(K_j)=\BigO(t^{N+1}).
\]
...
\item return \(G=X+K_j\) and verify \(F\circ G-X=\BigO(t^N)\).
\end{enumerate}
\end{algorithm}
...
\]
The block \(K_n\) appears on the right side of \cref{eq:inverse-fixed-point} only after multiplication by \(H'=\BigO(t)\), so it cannot affect the coefficient of \(t^n\). Therefore the equation for \(K_n\) depends only on \(K_0,\ldots,K_{n-1}\). This is the compositional analogue of the triangular quotient recursion.
...
\end{exercise}
\begin{exercise}
Show that the inverse of \(X+H\), where \(H=\BigO(t^q)\) for \(q\ge1\), has the form \(X-H+\BigO(t^{2q+1})\).
\end{exercise}
...
\[
 Y=X-\lambda+U
\]
and deriving an equation for \(U=\BigO(t\lambda)\) begins one scale closer to the answer.
...
\end{equation}
For \(F=X+H\), we have \(F'=1+\BigO(t)\), so the denominator is a unit.

\begin{proposition}[Quadratic valuation improvement]\label{prop:newton-improvement}
Suppose \(F=X+H\), \(H\in\K[\lambda][[t]]\), and \(R=F\circ G-X=\BigO(t^q)\) with \(q\ge0\). Then the Newton update satisfies
\begin{equation}\label{eq:newton-improvement}
 F\circ G_{\mathrm{new}}-X=\BigO(t^{2q+2}).
\end{equation}
...
\begin{proof}
The correction \(\Delta\) has valuation at least \(q\). The constant and linear residual terms cancel by construction. Since \(F''=H''=\BigO(t^2)\), the quadratic Taylor term has valuation at least \(2+2q\); all higher terms are deeper.
\end{proof}
...
\end{equation}
The leading coefficient of the residual can be computed by carrying one additional block. Since the derivative of \(w+\PrincipalLogarithm w-L\) with respect to \(w\) is \(1+1/w=1+\BigO(L^{-1})\), the solution error has the same leading order away from the branch point.
...
Let \(L=\xi_k\), \(\ell=\PrincipalLogarithm L\), and \(r=L^{-1}\). Factoring \(W=L(1+\BigO(r\ell))\) and using geometric division gives
\begin{align}
 \frac1W={}&r+\ell r^2+(\ell^2-\ell)r^3
 +\left(\ell^3-\frac52\ell^2+\ell\right)r^4\notag\\
 &+\left(\ell^4-\frac{13}{3}\ell^3+\frac92\ell^2-\ell\right)r^5\notag\\
...
\item \textbf{Coefficient combinatorics.} Develop analogous Stirling- or Bell-polynomial formulas for inverses of \(X+a\log X+\sum_j\LambertPolynomial{j}(\log X)X^{-j}\).
...
\(\SupportOf{F}\) & Set of exponents or monomials with nonzero coefficients.\\
