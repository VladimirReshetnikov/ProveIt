...
Consider
\begin{align*}
 F&=1+(\lambda+1)t+(2\lambda^2-\lambda)t^2+\FormalBigO_t(t^3),\\
 G&=1+(2-\lambda)t+(\lambda^2+3)t^2+\FormalBigO_t(t^3).
\end{align*}
...
\begin{equation}\label{eq:worked-product}
 FG=1+3t+(2\lambda^2+5)t^2+\FormalBigO_t(t^3).
\end{equation}
...
Return to
\begin{align*}
 A&=1+(\lambda+1)t+(2\lambda^2-\lambda)t^2+\FormalBigO_t(t^3),\\
 B&=1+(2-\lambda)t+(\lambda^2+3)t^2+\FormalBigO_t(t^3).
\end{align*}
...
\begin{equation}\label{eq:worked-quotient}
 \frac AB
 =1+(2\lambda-1)t+(3\lambda^2-6\lambda-1)t^2+\FormalBigO_t(t^3).
\end{equation}
...
\(\log(1+U)\), \(\EulerE^U\), \((1+U)^\alpha\), \(U=\FormalBigO_t(t)\) & closed & closed & none\\
...
Suppose \(F\) has lower valuation \(n_0\), \(U=\FormalBigO_t(t)\), and we seek \(\trunc_{<N}(F\circ G)\). In \cref{eq:exact-block-composition}, the term indexed by \(n\) begins at \(t^n\). Hence only \(n<N\) are needed. Within that term, \(V=\log(1+U)\) and \(R=(1+U)^{-1}\) are needed only through order \(t^{N-n-1}\). This gives an exact guard-term policy.
...
\begin{summarybox}
For the near-identity increment \(H\in\K[\lambda][[t]]\), formal Taylor expansion is an exact identity because each derivative raises the primary power order. The difference estimate gains one order when the outer derivative is \(\FormalBigO_t(t)\), providing the contraction needed for series reversal. Deeper transseries require a logarithmic-derivative test.
\end{summarybox}
...
\begin{equation}\label{eq:inverse-fixed-point}
 K=-H(X+K).
\end{equation}
The derivative \(H'\) is \(\FormalBigO_t(t)\), so the right side is contractive in the \(t\)-adic sense.
...
\[
 K_{j+1}=-H(X+K_j)
\]
is correct through \(t^N\). More precisely, if \(K_*-K_j=\FormalBigO_t(t^N)\), where \(K_*\) is the exact fixed point, then
\[
 K_*-K_{j+1}
 =\Phi(K_*)-\Phi(K_j)=\FormalBigO_t(t^{N+1}).
\]
...
\item return \(G=X+K_j\) and verify \(F\circ G-X=\FormalBigO_t(t^N)\).
\end{enumerate}
\end{algorithm}
...
\]
The block \(K_n\) appears on the right side of \cref{eq:inverse-fixed-point} only after multiplication by \(H'=\FormalBigO_t(t)\), so it cannot affect the coefficient of \(t^n\). Therefore the equation for \(K_n\) depends only on \(K_0,\ldots,K_{n-1}\). This is the compositional analogue of the triangular quotient recursion.
...
\end{exercise}
\begin{exercise}
Show that the inverse of \(X+H\), where \(H=\FormalBigO_t(t^q)\) for \(q\ge1\), has the form \(X-H+\FormalBigO_t(t^{2q+1})\).
\end{exercise}
...
\[
 Y=X-\lambda+U
\]
and deriving an equation for \(U=\FormalBigO_t(t)\) begins one outer block closer to the answer; its leading coefficient may grow polynomially in \(\lambda\).
...
\end{equation}
For \(F=X+H\), we have \(F'=1+\FormalBigO_t(t)\), so the denominator is a unit.

\begin{proposition}[Quadratic valuation improvement]\label{prop:newton-improvement}
Suppose \(F=X+H\), \(H\in\K[\lambda][[t]]\), and \(R=F\circ G-X=\FormalBigO_t(t^q)\) with \(q\ge0\). Then the Newton update satisfies
\begin{equation}\label{eq:newton-improvement}
 F\circ G_{\mathrm{new}}-X=\FormalBigO_t(t^{2q+2}).
\end{equation}
...
\begin{proof}
The correction \(\Delta\) has valuation at least \(q\). The constant and linear residual terms cancel by construction. Since \(F''=H''=\FormalBigO_t(t^2)\), the quadratic Taylor term has valuation at least \(2+2q\); all higher terms are deeper.
\end{proof}
...
\begin{equation}\label{eq:affine-log-F}
 F(X)=X+a\lambda+b+(c\lambda+d)t+\FormalBigO_t(t^2).
\end{equation}
...
\[
 K_1=-\lambda-\lambda t.
\]
To extract the second correct block, put \(K=-\lambda+A(\lambda)t+B(\lambda)t^2+\FormalBigO_t(t^3)\), expand the right side, and compare coefficients. One obtains
\[
 A(\lambda)=0,
 \qquad
...
\[
 F\invcomp
 =X-\lambda+\left(\lambda-\frac12\lambda^2\right)t^2+\FormalBigO_t(t^3).
\]
...
\end{enumerate}
The result is \(FG+\FormalBigO_t(t^{N+1})\).
\end{algorithm}
...
\(\FormalBigO_t(t^N)\) & Formal tail: the omitted difference has \(t\)-valuation at least \(N\); no analytic bound is asserted.\\
