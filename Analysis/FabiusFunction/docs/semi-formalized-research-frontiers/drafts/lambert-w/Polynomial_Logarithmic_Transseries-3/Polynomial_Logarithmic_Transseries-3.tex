...
\usepackage{mathtools,amsthm,amssymb,mathrsfs,bm}
...
\newcommand{\Rring}{\PolynomialLogPowerSeriesRing{\K}}
\newcommand{\Pring}{\PolynomialLogLaurentRing{\K}}
\newcommand{\Fring}{\IteratedLaurentLogField{\K}}
\newcommand{\Ggroup}{\NearIdentityPolynomialLogGroup{\K}}
\newcommand{\ordt}{\nu_t}
\newcommand{\TransseriesTruncateBelow}[2]{%
  \operatorname{Trunc}_{<#2}\!\left(#1\right)%
}
\newcommand{\UnsignedStirlingFirstKind}[2]{%
  \genfrac{[}{]}{0pt}{}{#1}{#2}%
}
...
\newcommand{\ReversionResidual}[1]{%
  \operatorname{Err}_{\mathrm{rev},F}\!\left(#1\right)%
}
\begin{document}
...
\vspace{0.08in}
{\LARGE and the Lambert \(\LambertW\) Expansion\par}
\vspace{0.55in}
...
Lambert's \(\LambertW\) function is derived from formal reversion in three independent
ways.
\end{minipage}
\vfill
...
\[
  \LambertW(z)=L_1-L_2+\sum_{n\geq 1}\frac{\LambertPolynomial{n}(L_2)}{L_1^n},
  \qquad L_1=\log z,\quad L_2=\log\log z,
\]
where each \(\LambertPolynomial{n}\) is a polynomial of degree \(n\).  These expressions are
...
polynomials
\[
 \LambertPolynomial{n}(u)=\frac{(-1)^{n+1}}{(n+1)!}
   \D_u(\D_u-1)\cdots(\D_u-n+1)u^{n+1},
\]
where the product is empty at \(n=0\), so
\(\LambertPolynomial{0}(u)=-u\).  Thus the canonical family is indexed by
\(n\in\NaturalNumbers\), even when a displayed expansion separates the
index-zero logarithmic block and sums only over \(n\ge1\).
The subsequent sections give their Stirling-number formula,
...
A reader mainly interested in Lambert \(\LambertW\) can read
...
Lambert \(\LambertW\).  Thus, in the Lambert application, \(X=\log z\).
...
\chapter{Lambert \texorpdfstring{\(\LambertW\)}{W} as the motivating inverse problem}
\label{ch:motivation}
...
\end{equation}
On the positive real axis, \(\LambertW(z)\to+\infty\) with \(z\), so taking logarithms
is legitimate and gives
\begin{equation}\label{eq:Wlogeq}
  \LambertW(z)+\log \LambertW(z)=\log z.
\end{equation}
Put
\[
  L_1=\log z.
...
balance is \(\LambertW\sim L_1\).  Feeding this approximation back into the smaller
term gives
\[
  \log \LambertW\sim\log L_1=:L_2,
  \qquad
  \LambertW\sim L_1-L_2.
\]
...
polynomials \(\LambertPolynomial{n}(L_2)\).  Treating \(L_1^{-1}\) and \(L_2\) as two independent
...
\begin{equation}\label{eq:scalehierarchyW}
  L_1^{-1}L_2^a=o(L_2^{-b})
  \quad\text{for every fixed }a,b\in\RealNumbers.
\end{equation}
...
Let \(\K\) be a field of characteristic zero, usually \(\RealNumbers\) or
\(\ComplexNumbers\).  Introduce two formal symbols \(t\) and \(\ell\), later
...
The support of \(A\) is
\[
  \CoefficientSupportOf{A}=\{(n,k)\in\IntegerNumbers\times\NaturalNumbers:a_{n,k}\neq0\}.
\]
...
\[
  A=B+\FormalBigO_t(t^N)
  \quad\Longleftrightarrow\quad
  \ordt(A-B)\geq N.
\]
The truncation below order \(N\) is
\begin{equation}\label{eq:truncdef}
  \TransseriesTruncateBelow{A}{N}:=\sum_{n<N}p_n(\ell)t^n.
\end{equation}
...
\(\TransseriesTruncateBelow{\sum_jS_j}{N}
=\TransseriesTruncateBelow{\sum_{j<J}S_j}{N}\), so each finite truncation is
...
\[
  ...
  \qquad
  \frac{\Differential\ell}{\Differential X}=t.
\]
...
\begin{equation}\label{eq:integrationclosed}
  \int \Pring\,\Differential X\subseteq\Pring+\K.
\end{equation}

\begin{example}
Using \cref{eq:integrateformula},
\[
 \int X^{-3}(\log X)^2\,\Differential X
 =-\frac{X^{-2}}{2}
...
\begin{align}\label{eq:multworked}
 ...
 &+(3\ell^3-\ell^2-3\ell+3)t^3+\FormalBigO_t(t^4).
\end{align}
...
\begin{equation}\label{eq:truncmult}
 \TransseriesTruncateBelow{AB}{N}
 =\TransseriesTruncateBelow{%
    \TransseriesTruncateBelow{A}{N}\,
    \TransseriesTruncateBelow{B}{N}%
  }{N}.
\end{equation}
...
\end{equation}
Unless \(\alpha r\in\IntegerNumbers\), this leaves \(\Pring\) and belongs to a
Puiseux or Hahn extension.  Even when \(\alpha r\) is integral, a branch for
\(a^\alpha\) must be selected over \(\ComplexNumbers\).
...
\[
 U=(\ell+1)t+\ell^2t^2.
\]
Since \(U^4=\FormalBigO_t(t^4)\),
\begin{align*}
 \log(1+U)
 & =U-\frac{U^2}{2}+\frac{U^3}{3}+\FormalBigO_t(t^4)\\
 & =(\ell+1)t
...
\end{equation}
Thus, if \(BV=1+\FormalBigO_t(t^m)\), then
\(BV_{\mathrm{new}}=1+\FormalBigO_t(t^{2m})\).  Each step is performed only to the
...
\[
 V\leftarrow\TransseriesTruncateBelow{V(2-BV)}{\min(2m,N)}
\]
...
\begin{equation}\label{eq:generalinner}
  ...
  \quad h\prec1.
\end{equation}
In the discrete ring, take \(\rho\in\PositiveIntegers\) and
\(h\in t\Rring\).  Then
\begin{align}
 t\compose G
 &=G^{-1}=c^{-1}t^\rho(1+h)^{-1},
...
If \(F\in\Pring\), \(\rho\in\PositiveIntegers\), and
...
\begin{align*}
 t\compose G
 &=t-\ell t^2+\ell^2t^3+\FormalBigO_t(t^4),\\
 \ell\compose G
 &=\ell+\ell t-\frac{\ell^2}{2}t^2
   +\frac{\ell^3}{3}t^3+\FormalBigO_t(t^4).
\end{align*}
...
\[
 A_{j+1}=\TransseriesTruncateBelow{A_jA}{N},
 \qquad
 D_{j+1}=\TransseriesTruncateBelow{\D_XD_j}{N}.
\]
...
If \(G=\EulerE^X\), then \(t\compose G=\EulerE^{-X}\), an exponentially small
...
\end{enumerate}
Then \(F\compose H=X+\FormalBigO_t(t^N)\).
\end{algorithm}
...
\[
 \ReversionResidual{H}:=F\compose H-X=0
\]
by Newton's method.  Linearizing in a correction \(\Delta\) gives
\[
 F\compose(H+\Delta)-X
 =\ReversionResidual{H}+(F'\compose H)\Delta+\text{higher order}.
\]
Hence
...
\(F\compose H-X=\FormalBigO_t(t^N)\).
\end{algorithm}
...
\begin{align}
 ...
 H\compose F-X&=\FormalBigO_t(t^N).
...
\(F(H_N(X))-X=\BigOAt{X^{-N}(\log X)^M}{X\to+\infty}\), the mean value theorem transfers the
same order to \(H_N-F\rev\), because \(F'\) stays bounded away from zero.
...
\(\WrightOmega(X)=\LambertW(\EulerE^X)\).  Globally in the complex plane, branch and
...
\begin{equation}\label{eq:omegaform}
 \WrightOmega(X)=X-\ell+\sum_{n\geq1}\LambertPolynomial{n}(\ell)t^n,
 \qquad t=X^{-1}.
\end{equation}
The polynomials \(\LambertPolynomial{n}\) are universal: the same sequence appears in every
...
\begin{equation}\label{eq:Poperator}
 ...
 \qquad D=\frac{\Differential}{\Differential u}.
 }
\end{equation}
...
\begin{align*}
 \LambertPolynomial{1}(u)
 &=\frac12Du^2=u,\\
 \LambertPolynomial{2}(u)
 &=-\frac16D(D-1)u^3
   =\frac{u^2}{2}-u,\\
 \LambertPolynomial{3}(u)
...
\begin{equation}\label{eq:unsignedstirling}
 \UnsignedStirlingFirstKind{n}{k}=(-1)^{n-k}s(n,k).
\end{equation}
...
\begin{equation}\label{eq:Pstirling}
 \boxed{
 \LambertPolynomial{n}(u)=\sum_{m=1}^{n}
 (-1)^{n-m}\UnsignedStirlingFirstKind{n}{n-m+1}\frac{u^m}{m!}.
 }
\end{equation}
\end{theorem}
...
For \(n\geq1\):
\begin{enumerate}
\item \(\LambertPolynomial{n}(0)=0\) and \(\LambertPolynomial{n}'(0)=(-1)^{n-1}\).
\item \(\deg \LambertPolynomial{n}=n\), with leading coefficient \(1/n\).
\item The nonzero coefficients alternate in sign:
\([u^m]\LambertPolynomial{n}\) has sign \((-1)^{n-m}\).
\item The coefficient of \(u^{n-1}\) is \(-H_{n-1}\), where
\(H_r=1+1/2+\cdots+1/r\).
\item The coefficient of \(u^2\) is
\((-1)^{n-2}n(n-1)/4\).
\item \(n!\LambertPolynomial{n}(u)\in\IntegerNumbers[u]\).
\end{enumerate}
\end{proposition}

\begin{proof}
The constant term is absent.  For \(m=1\), use
\(\UnsignedStirlingFirstKind{n}{n}=1\).  For \(m=n\), use
\(\UnsignedStirlingFirstKind{n}{1}=(n-1)!\), giving \((n-1)!/n!=1/n\).
Positivity of unsigned Stirling numbers proves the sign assertion.  For
\(m=n-1\), use
\(\UnsignedStirlingFirstKind{n}{2}=(n-1)!H_{n-1}\).  For \(m=2\), use
\(\UnsignedStirlingFirstKind{n}{n-1}=\binom n2\).  Finally, \(m!\mid n!\) for \(m\leq n\),
...
Starting with \(\LambertPolynomial{1}(u)=u\),
\begin{align}
 \LambertPolynomial{n+1}'(u)&=n\LambertPolynomial{n}(u)-\LambertPolynomial{n}'(u),
 \label{eq:Pderivrec}\\
 \LambertPolynomial{n+1}(0)&=0.
 \label{eq:Pzero}
\end{align}
Equivalently,
\begin{equation}\label{eq:Pintegralrec}
 \boxed{
 \LambertPolynomial{n+1}(u)=-\LambertPolynomial{n}(u)+n\int_0^u\LambertPolynomial{n}(v)\Differential v.
 }
\end{equation}
\end{theorem}
...
\begin{equation}\label{eq:Ugen}
 U(t,u)=\sum_{n\geq1}\LambertPolynomial{n}(u)t^n.
\end{equation}
...
\(\LambertPolynomial{n+1}'=n\LambertPolynomial{n}-\LambertPolynomial{n}'\).  Since every \(\LambertPolynomial{n}\) has zero constant term by
...
\begin{equation}\label{eq:omegaU}
 \WrightOmega(X)=X-u+U,
 \qquad u=\log X,
 \quad U=\sum_{n\geq1}\LambertPolynomial{n}(u)X^{-n}.
\end{equation}
Then
\[
...
\end{equation}
This one equation recursively generates every \(\LambertPolynomial{n}\).  Expanding the right
side,
\begin{equation}\label{eq:Uexpanded}
 U=\sum_{k\geq1}\frac{t^k(u-U)^k}{k}.
\end{equation}
At order \(t^n\), only \(\LambertPolynomial{1},\ldots,\LambertPolynomial{n-1}\) occur on the right, giving a
...
\begin{align*}
 [t]U&=u,\\
 [t^2]U&=-\LambertPolynomial{1}+\frac{u^2}{2},\\
 [t^3]U&=-\LambertPolynomial{2}+\frac{\LambertPolynomial{1}^2}{2}-u\LambertPolynomial{1}+\frac{u^3}{3}.
\end{align*}
Using \(\LambertPolynomial{1}=u\), this gives
\[
 \LambertPolynomial{2}=\frac{u^2}{2}-u,
 \qquad
 \LambertPolynomial{3}=\frac{u^3}{3}-\frac32u^2+u.
\]
\end{example}
...
For reference,
\begin{align}
 \LambertPolynomial{1}(u)={}&u,\notag\\
 \LambertPolynomial{2}(u)={}&\frac{u^2}{2}-u,\notag\\
 \LambertPolynomial{3}(u)={}&\frac{u^3}{3}-\frac32u^2+u,\notag\\
 \LambertPolynomial{4}(u)={}&\frac{u^4}{4}-\frac{11}{6}u^3+3u^2-u,\notag\\
 \LambertPolynomial{5}(u)={}&\frac{u^5}{5}-\frac{25}{12}u^4
             +\frac{35}{6}u^3-5u^2+u,\notag\\
 \LambertPolynomial{6}(u)={}&\frac{u^6}{6}-\frac{137}{60}u^5
             +\frac{75}{8}u^4-\frac{85}{6}u^3
             +\frac{15}{2}u^2-u,\notag\\
 \LambertPolynomial{7}(u)={}&\frac{u^7}{7}-\frac{49}{20}u^6
             +\frac{203}{15}u^5-\frac{245}{8}u^4
             +\frac{175}{6}u^3-\frac{21}{2}u^2+u,\notag\\
 \LambertPolynomial{8}(u)={}&\frac{u^8}{8}-\frac{363}{140}u^7
...
For a truncation
\[
 \WrightOmega_N=X-u+\sum_{n=1}^{N}\LambertPolynomial{n}(u)X^{-n},
\]
define
...
\(\LambertPolynomial{n}\).  The operator formula explains its differential origin, the Stirling
...
\chapter{Lambert \texorpdfstring{\(\LambertW\)}{W} from the omega reverse}
\label{ch:lambert}
...
\begin{equation}\label{eq:WlargePn}
 ...
 \xi_k-\log\xi_k+
 \sum_{n=1}^{\infty}
 \frac{\LambertPolynomial{n}(\log\xi_k)}{\xi_k^n},
 \qquad
 \xi_k=\PrincipalLogarithm z+2\pi\ImaginaryUnit k.
 }
\end{equation}
...
\begin{equation}\label{eq:WlargeStirling}
 \LambertW_k(z)\sim\xi_k-\log\xi_k+
...
Lambert \(\LambertW\) literature \cite{Corless1996,DLMF,JeffreyCorlessKnuth1997}.
...
\begin{equation}\label{eq:Wansatzdirect}
 \LambertW=L_1-L_2+U,
 \qquad
 U=\sum_{n\geq1}\LambertPolynomial{n}(L_2)L_1^{-n}.
\end{equation}
Since
\[
 \LambertW=L_1\left(1-\frac{L_2-U}{L_1}\right),
\]
we have
\[
 \log \LambertW=L_2+
...
\]
Substitution into \(\LambertW+\log \LambertW=L_1\) yields
...
sectors.  Moreover, for sufficiently large \(\AbsoluteValue z\), the series in
\cref{eq:WlargeStirling} is absolutely convergent to \(\LambertW_k(z)\); for the
principal real branch it converges for \(z\geq\EulerE\)
...
\[
 L_1=1,
 \qquad L_2=0,
 \qquad \LambertW_0(\EulerE)=1.
\]
Because \(\LambertPolynomial{n}(0)=0\) for every \(n\), the entire correction series vanishes and
the expansion gives the exact value at the endpoint.

\section{Numerical behavior}
Let
\begin{equation}\label{eq:WNdef}
 \LambertW_N(z)=L_1-L_2+\sum_{n=1}^{N}\frac{\LambertPolynomial{n}(L_2)}{L_1^n},
\end{equation}
with \(\LambertW_0=L_1-L_2\).  The following absolute errors were computed at high
precision against the defining principal branch.

\begin{center}
\begin{tabular}{@{}rccc@{}}
\toprule
\(N\) & \(z=10\) & \(z=10^6\) & \(z=\EulerE^{100}\) \\
\midrule
...
astronomically large \(z\), because it never forms \(\EulerE^w\).

\section{Derivative and arithmetic closure}
Differentiating \(\LambertW\EulerE^\LambertW=z\) gives
\begin{equation}\label{eq:Wderivative}
 \LambertW'(z)=\frac{\LambertW(z)}{z(1+\LambertW(z))}.
\end{equation}
Once \(\LambertW\) is represented as a polynomial-logarithmic transseries in
\(L_1^{-1}\), the derivative follows from the differential rule, and the
rational factor \(\LambertW/(1+\LambertW)\) follows from division.  For example,
\begin{equation}\label{eq:Wderivativefirst}
 \LambertW'(z)=\frac1z\left(
 1-\frac1{L_1}+
 \frac{1-L_2}{L_1^2}+
...
\section{The real branch \texorpdfstring{\(\LambertW_{-1}\)}{W-1} near zero}
For \(-\EulerE^{-1}<x<0\), there are two real branches.  The lower branch satisfies
\(\LambertW_{-1}(x)\to-\infty\) as \(x\to0^-\).  Define
\begin{equation}\label{eq:etaWm}
 \eta=\log(-1/x),
 \qquad
 L=\log\eta.
\end{equation}
Writing \(x=-\EulerE^{-\eta}\) and \(\LambertW_{-1}(x)=-v\) gives
\[
 v\EulerE^{-v}=\EulerE^{-\eta},
 \qquad
 v-\log v=\eta.
\]
...
\begin{equation}\label{eq:WminusPn}
 ...
 \LambertW_{-1}(x)
 \sim-\eta-L+
 \sum_{n=1}^{\infty}(-1)^n\frac{\LambertPolynomial{n}(L)}{\eta^n}
 \qquad(x\to0^-).
 }
\end{equation}
The first terms are
\begin{align}
 \LambertW_{-1}(x)\sim{}&-\eta-L-\frac{L}{\eta}
 +\frac{L^2/2-L}{\eta^2}\notag\\
...
The inverse-logarithmic expansion is adapted to \(\LambertW\to\pm\infty\).  Near the
branch point \(z=-\EulerE^{-1}\), one instead has \(\LambertW\to-1\), and the derivative
\[
 \LambertW'(z)=\frac{\LambertW}{z(1+\LambertW)}
\]
blows up because \(1+\LambertW=0\).  Set
...
\(w\mapsto w\EulerE^w\).  This is a canonical example of scale selection:
polynomial-logarithmic transseries are correct at infinity and near the
logarithmic singular limit \(x\to0^-\) on \(\LambertW_{-1}\), while a Puiseux series is
correct near \(-\EulerE^{-1}\).

\section{A practical high-precision recipe}
For a large complex \(z\) on branch \(k\):
\begin{enumerate}
\item Compute \(\xi_k=\PrincipalLogarithm z+2\pi\ImaginaryUnit k\) and a consistent
\(u=\log\xi_k\).
\item Generate \(\LambertPolynomial{1},\ldots,\LambertPolynomial{N}\) by
\(\LambertPolynomial{n+1}=-\LambertPolynomial{n}+n\int_0^u\LambertPolynomial{n}\), or use the Stirling formula.
\item Sum
\[
 w_N=\xi_k-u+\sum_{n=1}^{N}\LambertPolynomial{n}(u)\xi_k^{-n}
\]
...
with \(\AbsoluteValue{\xi_k}\); Newton then removes the remaining local error.
...
Lambert \(\LambertW\) at large argument is the reverse of \(X+\log X\) evaluated at a
branch logarithm of the original argument:
\[
 \LambertW_k(z)=\WrightOmega(\xi_k),
 \qquad \xi_k=\PrincipalLogarithm z+2\pi\ImaginaryUnit k.
\]
...
\begin{equation}\label{eq:conjugateproblem}
 F=H\rev\compose(\IdentityOperator+\phi)\compose H,
\end{equation}
where \(H\) and \(H\rev\) are known on the relevant transseries class.  Then
\begin{equation}\label{eq:conjugateinverse}
 F\rev=H\rev\compose(\IdentityOperator+\phi)\rev\compose H.
\end{equation}
...
\begin{equation}\label{eq:XlogXleading}
 X\sim\frac{Y}{\log Y},
\end{equation}
not \(Y\).  Substituting the Lambert expansion into \(Y/\LambertW(Y)\) and using the
...
fractional power, \(\log\log X\), or \(\EulerE^{-X}\) appears, enlarge the monomial
...
\end{equation}
where \(n\in\IntegerNumbers\) is the \(t\)-exponent and \(p_n\) is a canonical
...
\item the coefficient domain, such as \(\RationalNumbers[\ell]\),
\(\RationalNumbers(\ell)\), or \(\RationalNumbers((\ell^{-1}))\);
...
\begin{equation}\label{eq:exclusiveprecision}
 \TransseriesTruncateBelow{A}{N}=\sum_{n<N}p_n(\ell)t^n.
\end{equation}
Thus ``precision \(N\)'' means equality after exclusive truncation below
outer \(t\)-order \(N\).  Every operation should
specify how much input precision it consumes and should return a value tagged
with the precision it actually guarantees.

For Laurent inputs with lower order \(r\), multiplication truncated below
\(t\)-order \(N\)
...
\begin{equation}\label{eq:algtaylor}
 \TransseriesTruncateBelow{F\compose(X+A)}{N}=
 \TransseriesTruncateBelow{
   \sum_{j=0}^{N-1}\frac{A^j}{j!}\D_X^jF}{N}.
\end{equation}
...
\begin{equation}\label{eq:algreverseLagrange}
 \TransseriesTruncateBelow{\psi}{N}=
 \TransseriesTruncateBelow{
   \sum_{m=1}^{N}\frac{(-1)^m}{m!}\D_X^{m-1}(\phi^m)}{N}.
\end{equation}
...
below outer \(t\)-order \(N\).  Expanding an intermediate through \(t^{2N}\) and truncating at
...
def reverse_lagrange(phi, order: int):
    """Return psi with compose(X + phi, X + psi) = X below t**order."""
    psi = 0
...
\[
 ...
 =\frac{\Differential L}{\Differential X}\partial_L
  +\frac{\Differential t}{\Differential X}\partial_t
...
\begin{enumerate}
\item \(\TransseriesTruncateBelow{A(B^{-1})-1}{N}=0\) for random unit series;
\item generator composition equals Taylor composition;
\item the exclusive truncations of \((F\compose G)\compose H\) and
      \(F\compose(G\compose H)\) agree below the cutoff;
...
order;
\item differentiation obeys product and chain rules after exclusive
      truncation at the cutoff.
\end{enumerate}

Randomized property tests are particularly effective.  Generate low-degree
polynomial coefficients over \(\RationalNumbers\), use small cutoffs, and compare
...
\begin{enumerate}
\item the branch index \(k\) of \(\LambertW_k\);
\item the branch used for \(\PrincipalLogarithm z\);
...
\begin{equation}\label{eq:Puiseuxderivative}
 ...
 \qquad \alpha\in q^{-1}\IntegerNumbers.
\end{equation}
...
Let \(\Gamma\subseteq\RealNumbers\) be an ordered additive group.  A Hahn
...
Lambert \(\LambertW\) itself uses two logarithmic levels in the original argument
...
Power-log series cannot represent \(\EulerE^{-X}\), \(\EulerE^{-X^2}\), or
\(\EulerE^{\sqrt X}\).  Full logarithmic-exponential transseries adjoin monomials
of the form
...
composition with \(\EulerE^X\) asks for exponential height.  Promotion should be
...
\begin{align}
 ...
 &=\sum_{m=1}^{n}(-1)^{n-m}
   \UnsignedStirlingFirstKind{n}{n-m+1}\frac{u^m}{m!},
 \label{eq:sheetPnstirling}\\
 \LambertPolynomial{n+1}(u)
 &=-\LambertPolynomial{n}(u)+n\int_0^u\LambertPolynomial{n}(v)\Differential v.
 \label{eq:sheetPnrec}
\end{align}

\section{Lambert \texorpdfstring{\(\LambertW\)}{W}}
\begin{align}
 \xi_k&=\PrincipalLogarithm z+2\pi\ImaginaryUnit k,
 \label{eq:sheetxi}\\
 \LambertW_k(z)&\sim\xi_k-\log\xi_k+
 \sum_{n\geq1}\frac{\LambertPolynomial{n}(\log\xi_k)}{\xi_k^n},
 \label{eq:sheetW}\\
 \LambertW_{-1}(x)&\sim-\eta-\log\eta+
 \sum_{n\geq1}(-1)^n
 \frac{\LambertPolynomial{n}(\log\eta)}{\eta^n},
 \quad \eta=\log(-1/x).
...
Let \(C\) be a commutative \(\RationalNumbers\)-algebra.  For a Laurent series
\[
 A(z)=\sum_{j\in\IntegerNumbers}a_jz^j\in C((z)),
\]
define
\[
 \ResidueOperator_z A(z)\,\Differential z=a_{-1}.
\]
Formal residues obey
\begin{equation}\label{eq:formalresderivative}
 \ResidueOperator_z \frac{\Differential}{\Differential z}B(z)\,\Differential z=0
\end{equation}
...
\begin{align*}
 [\eps^m]H(u(\eps))
 &=\ResidueOperator_\eps H(u(\eps))\eps^{-m-1}\Differential\eps\\
 &=\ResidueOperator_z H(z)z^{-m-1}\Phi(z)^{m-1}
       \bigl(\Phi(z)-z\Phi'(z)\bigr)\Differential z.
\end{align*}
By applying \cref{eq:formalresderivative} to
\(H(z)\Phi(z)^mz^{-m}\), one obtains
\begin{align*}
 0={}&\ResidueOperator_z\Bigl(
 H'(z)\Phi(z)^mz^{-m}
...
\LambertPolynomial{12}(u)={}&\frac{u^{12}}{12}-\frac{83711}{27720}u^{11}
...
\(\LambertPolynomial{n+1}=-\LambertPolynomial{n}+n\int_0^u\LambertPolynomial{n}\).

\section{General coefficient diagonals}
Write
\[
 \LambertPolynomial{n}(u)=\sum_{m=1}^np_{n,m}u^m.
\]
...
\begin{equation}\label{eq:upperdiagonal}
 [u^{n-r}]\LambertPolynomial{n}(u)
 =(-1)^r\frac{1}{(n-r)!}\UnsignedStirlingFirstKind{n}{r+1}.
\end{equation}
The first four are
\begin{align}
 [u^n]\LambertPolynomial{n}&=\frac1n,
 \label{eq:diagtop0}\\
 [u^{n-1}]\LambertPolynomial{n}&=-H_{n-1},
 \label{eq:diagtop1}\\
 [u^{n-2}]\LambertPolynomial{n}
 &=\frac{n-1}{2}\left(H_{n-1}^2-H_{n-1}^{(2)}\right),
 \label{eq:diagtop2}\\
 [u^{n-3}]\LambertPolynomial{n}
 &=-\frac{(n-1)(n-2)}{6}
...
\(\UnsignedStirlingFirstKind{n}{2}\), \(\UnsignedStirlingFirstKind{n}{3}\), and
\(\UnsignedStirlingFirstKind{n}{4}\).

At the bottom of the coefficient triangle,
\begin{align}
 [u]\LambertPolynomial{n}&=(-1)^{n-1},
 \label{eq:diagbottom1}\\
 [u^2]\LambertPolynomial{n}&=(-1)^{n-2}\frac{n(n-1)}4,
 \label{eq:diagbottom2}\\
 [u^3]\LambertPolynomial{n}&=(-1)^{n-3}
 \frac{n(n-1)(n-2)(3n-1)}{144}.
 \label{eq:diagbottom3}
\end{align}
For the last identity, use
\[
 \UnsignedStirlingFirstKind{n}{n-2}
...
\[
 \LambertPolynomial{n+1}'=n\LambertPolynomial{n}-\LambertPolynomial{n}'
\]
yields
...
\[
 n!\LambertPolynomial{n}(u)\in\IntegerNumbers[u].
\]
For exact symbolic computation, it is often advantageous to store the scaled
polynomials
\[
 \ScaledLambertPolynomial{n}(u)=n!\LambertPolynomial{n}(u).
\]
Their coefficients are integers, and the recurrence becomes
\begin{equation}\label{eq:scaledPrec}
 \ScaledLambertPolynomial{n+1}
 =-(n+1)\ScaledLambertPolynomial{n}
   +n(n+1)\int_0^u\ScaledLambertPolynomial{n}(v)\Differential v.
\end{equation}
...
\[
 \bigl(1+(\ell-1)t+\ell t^2\bigr)C=1+\FormalBigO_t(t^4).
\]
...
Use
\(\LambertPolynomial{n+1}=-\LambertPolynomial{n}+n\int_0^u\LambertPolynomial{n}(v)\Differential v\) to generate \(\LambertPolynomial{2},\LambertPolynomial{3},\LambertPolynomial{4}\) from
\(\LambertPolynomial{1}=u\).
\end{exercise}
...
\(L_1^{-3}\) and show that
\[
 \LambertW'(z)=\frac1z\left(1-\frac1{L_1}
 +\frac{1-L_2}{L_1^2}
 +\frac{-L_2^2+3L_2-1}{L_1^3}
 +\BigOAt{L_2^3/L_1^4}{|L_1|\to\infty,\ L_1\in S}\right).
\]
\end{exercise}

\begin{exercise}[The lower real branch]
Starting from \(\LambertW_{-1}(x)\EulerE^{\LambertW_{-1}(x)}=x\), set
\(x=-\EulerE^{-\eta}\) and derive the first three correction blocks in
\cref{eq:WminusPn}.
\end{exercise}
...
Hence
\[
 AB=1+3t+(2\ell+1)t^2+\FormalBigO_t(t^3).
\]

\subsection*{Solution 2}
Write \(C=1+c_1t+c_2t^2+c_3t^3+\FormalBigO_t(t^4)\).  Triangular coefficient
comparison gives
\begin{align*}
 c_1&=1-\ell,\\
...
\[
 ...
 +(-\ell^3+5\ell^2-5\ell+1)t^3+\FormalBigO_t(t^4).
\]
...
\(Q=-\ell-2\), and hence
\[
 \int t^2(\ell+1)\Differential X=-t(\ell+2)+C.
\]
Indeed,
...
\begin{align*}
 ...
 =t-at^2+a^2t^3+\FormalBigO_t(t^4),\\
...
\[
 F\rev=X-P+Q_1t+Q_2t^2+\FormalBigO_t(t^3),
\]
where
\[
 Q_1=\frac12D(P^2)=PP'
...
\begin{align*}
 \LambertPolynomial{2}&=-u+\int_0^u v\Differential v=\frac{u^2}{2}-u,\\
 \LambertPolynomial{3}&=-\LambertPolynomial{2}+2\int_0^u\LambertPolynomial{2}(v)\Differential v
 =\frac{u^3}{3}-\frac32u^2+u,\\
 \LambertPolynomial{4}&=-\LambertPolynomial{3}+3\int_0^u\LambertPolynomial{3}(v)\Differential v
...
Because \(L_1=\log z\),
\[
 \frac{\Differential}{\Differential z}=\frac1z\frac{\Differential}{\Differential L_1},
 \qquad
 \frac{\Differential L_2}{\Differential L_1}=\frac1{L_1}.
\]
Differentiate
\[
 \LambertW=L_1-L_2+\frac{L_2}{L_1}
 +\frac{L_2^2/2-L_2}{L_1^2}
 +\frac{L_2^3/3-3L_2^2/2+L_2}{L_1^3}+\cdots
\]
...
follows by expanding \(\LambertW/[z(1+\LambertW)]\).

\subsection*{Solution 11}
Put \(\LambertW_{-1}(x)=-v\).  Since \(x=-\EulerE^{-\eta}\), the defining equation is
\[
 v\EulerE^{-v}=\EulerE^{-\eta},
 \qquad v-\log v=\eta.
\]
...
\]
requires a second logarithmic level.  Finally, \(\EulerE^{-X}\) is beyond every
...
\section{Lambert \texorpdfstring{\(\LambertW\)}{W} sources}
The modern reference on Lambert \(\LambertW\), including branches, asymptotics, and
...
\(F\rev\) for compositional inverses.  The phrase \emph{series reversal}
