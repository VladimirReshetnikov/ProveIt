...
The last synchronized source checkpoint was validated by exactly three
successful serial halt-on-error passes and the repository's PDF gates.  The
merged source now postdates that checkpoint, so the retained PDF is historical
until a fresh three-pass parity rebuild.  Exact page-count, byte-count, and
SHA-256 receipts are kept externally in the repository documentation so that
neither artifact embeds a self-invalidating fingerprint. No
...
\((0,P_{m,A,\beta}]\); all three are in
\path{PowerExponentialLambertCalculus.lean}.  Moreover, if
...
\path{PowerExponentialLambertFabius.lean} identifies the lower generic phase
...
\lean{complexQPochhammerInf_eq_qPochhammerInfIn} bridge gives the semantic
live union 923 modules and 11,611 public declarations, with both documentation
gap counts zero.  The q-Lucas manuscript result remains Partial: Lean proves
...
\path{ThueMorseNewmanSelfSimilarity.lean}.  This made 934 modules and 11,709
...
eleven-theorem \path{TransseriesFlat.lean} module and three integer-zpow
theorems in \path{TransseriesDifferentialBlock.lean} gave the historical census 944
modules and 11,806 public declarations; the merged live census is 985/12,199.  The q forward ledger is 181 Exact /
79 Partial / 14 None / 8 interface rows, the relevant Dyadic
