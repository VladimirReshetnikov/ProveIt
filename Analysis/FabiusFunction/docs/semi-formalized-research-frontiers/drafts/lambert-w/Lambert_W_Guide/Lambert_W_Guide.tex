...
\lean{LambertWBranchGapBernoulli} has \(0+4\).  Their four-module union is
therefore four definitions and 36 theorems, 40 public declarations in all.
...
\(0<\Delta<2\pi\) on \(-1/\EulerE<x<0\), together with the exact complex
convergence radius and boundary divergence &
...
Exact for the compact real branch pair and for the convergence radius.  The
...
fourth specializes both real sums on the strict common branch domain when the
positive branch gap is below \(2\pi\).  At \(z=0\) convergence still holds,
but the quotient theorem excludes the removable value because Lean's
totalized quotient is zero there.  This makes
\eqref{eq:pair-Bernoulli-general} Exact and proves the printed radius claim;
the full complex value identity in \eqref{eq:bernoulli-gen} remains Partial.
The surrounding expanded displays and every Puiseux claim remain unpromoted.\\
\addlinespace
...
Not formalized, in the order they appear above: the full complex value form of
\eqref{eq:bernoulli-gen}, the moments of \Cref{sec:integrals}, the local Taylor
expansion, convergence of the Maclaurin series, the full Puiseux and
