...
A statement about the \emph{formal} series over \(\mathbb{Q}\): the
...
\lean{LambertWLowerBranchLogBounds}) &
...
Not formalized, in the order they appear above: the addition identity of
\Cref{prop:addition}, the exact branch pairing of \Cref{sec:pairing}, the
polynomial recurrence for higher derivatives (\Cref{thm:higherder}), the
antiderivatives and moments of \Cref{sec:integrals}, the local Taylor
...
\Cref{sec:bounds} (the logarithmic bounds for large positive arguments, the
square-root bounds on the negative part of the principal branch, and the
two-sided \(W_{-1}\) bound of \Cref{thm:chatzbound}), and everything from
