...
For the principal branch assume \(x>-1/\ee\), and for the lower real
branch assume \(-1/\ee<x<0\).  On these respective smooth real domains,
\begin{align}
 W_k'(x)
   &=\frac{1}{\ee^{W_k(x)}(1+W_k(x))},\label{eq:firstder}\\
 W_k''(x)
...
\[
 \ee^w(1+w)w'=1,
\]
which proves \eqref{eq:firstder}.  Since \(\ee^{-w}=w/x\) when
...
\]
When \(x\neq0\), substituting \(\ee^{-w}=w/x\) proves
\eqref{eq:secondder}.
...
unique-zero classification at \(-2/\ee^2\).  Finally,
...
The two branch-point vertical-tangent limits above are still not packaged as
Lean declarations, and neither is the square-root Puiseux package of
...
\[
 L=\log2,\qquad P=\frac{\ee^{-1}}L,
 \qquad x_*=\frac{2\ee^{-2}}L.
\]
...
\begin{equation}\label{eq:fabius-principal-phase-second-derivative}
 \lambda_0''(x)
 =L\frac{\ee^{-2\Wz(-Lx)}\bigl(\Wz(-Lx)+2\bigr)}
...
\addlinespace
Derivative & On \(W_0\) for \(x>-1/\ee\), and on \(W_{-1}\) for
\(-1/\ee<x<0\),
\(W_k'(x)=\ee^{-W_k(x)}/[1+W_k(x)]\).  When \(x\neq0\), this is also
\(W_k(x)/\{x[1+W_k(x)]\}\); separately, \(W_0'(0)=1\).\\
\addlinespace
Second derivative & On the same smooth domains,
\(W_k''(x)=-\ee^{-2W_k(x)}[W_k(x)+2]/[1+W_k(x)]^3\).  When
\(x\neq0\), this is also
