...
Ordinary Taylor series fail at \(x=-1/\ee\): from the right the principal
...
\[
 ...
 \quad(x\downarrow-1/\ee).
\]

This formalizes the signed leading equivalent, but not the stronger
\(\bigO(x+\ee^{-1})\) remainder in \eqref{eq:branchleading}, any displayed
...
\cref{thm:puiseux}.  The derivative equivalents \(W_k'\sim\pm\ee/q\) shown
...
attracting exactly when \(\abs W>1\).  Thus the principal iteration attracts
only for \(x>\ee\), while the lower one attracts throughout
\(-1/\ee<x<0\).  Contraction fails at \(W_0(\ee)=1\) and at the branch point.
