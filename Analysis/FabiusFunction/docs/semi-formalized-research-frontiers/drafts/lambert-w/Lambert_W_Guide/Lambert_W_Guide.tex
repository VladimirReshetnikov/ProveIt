...
\(w\mapsto w\ee^w\).  It is the natural language for equations in which an
...
Thus
\[
       w=W(z)\quad\Longleftrightarrow\quad w\ee^w=z.
\]
Because the complex exponential is periodic and the map \(w\mapsto w\ee^w\)
is not one-to-one, \(W\) has branches.  The branches are denoted \(W_k\),
where \(k\in\mathbb Z\).  On the real line only \(W_0\) and \(W_{-1}\) can
take real values.
\end{definition}
...
The equation \(w\ee^w=1\) has a unique real solution
\[
      \Omega:=W_0(1)=0.567143290409783872999968\ldots,
\]
sometimes called the \emph{omega constant}.  Indeed,
\(\Omega\ee^\Omega=1\), or equivalently \(\Omega=\ee^{-\Omega}\).
...
positive.  The complex principal logarithm is written \(\Log\).  At the point
\(-1/\ee\), both real branches equal \(-1\).  The notation
...
The real solutions of \(w\ee^w=x\) are as follows.
\begin{enumerate}[label=\textup{(\roman*)}]
\item If \(x<-1/\ee\), there is no real solution.
\item If \(x=-1/\ee\), there is one real solution, \(w=-1\).
...
More precisely,
\[
 \Wz:[-1/\ee,\infty)\longrightarrow[-1,\infty)
\]
is a strictly increasing bijection, while
\[
 \Wm:[-1/\ee,0)\longrightarrow(-\infty,-1]
\]
is a strictly decreasing bijection.
\end{theorem}

\begin{proof}
Because \(\ee^w>0\), the sign of \(f(w)=w\ee^w\) is the sign of \(w\).
Moreover,
\[
...
\[
 \lim_{w\to-\infty}w\ee^w=0^-,
 \quad f(-1)=-1/\ee,\quad f(0)=0,\quad
 \lim_{w\to\infty}w\ee^w=\infty.
\]
Consequently the restriction of \(f\) to \([-1,\infty)\) maps that interval
bijectively onto \([-1/\ee,\infty)\), and its inverse is \(\Wz\).  The
restriction to \((-\infty,-1]\) maps that interval bijectively, with reversed
orientation, onto \([-1/\ee,0)\), and its inverse is \(\Wm\).  The listed
...
continuity declarations do not include the vertical-tangent limits or the
square-root (Puiseux) expansion at the branch point.
\end{remark}
...
For \(-1/\ee<x<0\), the notation \(W(x)\) is incomplete unless the branch is
understood.  For example,
\[
 \Wz(-0.1)\approx-0.1118326,\qquad
 \Wm(-0.1)\approx-3.5771521.
\]
Both numbers satisfy \(w\ee^w=-0.1\).
\end{warning}

\begin{corollary}[Branch-aware inverse identities]\label{cor:inverse}
For real \(a\),
\[
 \Wz(a\ee^a)=a\quad\text{when }a\geq-1,
\]
and
\[
 \Wm(a\ee^a)=a\quad\text{when }a\leq-1.
\]
...
\(f(a)=a\ee^a\) to \([-1,\infty)\); the second says the same about the
...
\[
 \Wz(x\log x)=\log x\quad\text{if }x\geq\ee^{-1},
\]
whereas
\[
 \Wm(x\log x)=\log x\quad\text{if }0<x\leq\ee^{-1}.
\]
The same expression \(x\log x\) therefore selects different inverse branches
on opposite sides of \(x=\ee^{-1}\).
\end{example}
...
\begin{equation}\label{eq:basicidentities}
 \ee^w=\frac{x}{w},\qquad
 \ee^{-w}=\frac{w}{x},\qquad
 \log\abs{x}=w+\log\abs{w}
\end{equation}
for real \(x\neq0\).  The last identity is often more stable numerically than
\(w\ee^w=x\), because it avoids overflow and underflow.
...
This reparametrization is valuable when \(\ee^t\) is too large to store.
...
\begin{proof}
Writing \(w=-v\) in \(w\ee^w=-\ee^{-u}\) gives
\(v\ee^{-v}=\ee^{-u}\).  Taking logarithms gives
...
On the interval \((-1/\ee,0)\), the two real branches are not independent.
They are the two preimages of the same number under \(w\mapsto w\ee^w\).
...
Let \(x\in(-1/\ee,0)\), and define the positive branch gap
\begin{equation}\label{eq:branchgap}
       \Delta=\Wz(x)-\Wm(x)>0.
\end{equation}
Then
\begin{align}
 \Wz(x)&=-\frac{\Delta}{\ee^\Delta-1},
 \label{eq:W0-gap}\\
 \Wm(x)&=-\frac{\Delta}{1-\ee^{-\Delta}}
       =-\frac{\Delta\ee^\Delta}{\ee^\Delta-1},
 \label{eq:Wm-gap}\\
 x&=-\frac{\Delta}{\ee^\Delta-1}
       \exp\left(-\frac{\Delta}{\ee^\Delta-1}\right).
 \label{eq:x-gap}
\end{align}
Conversely, every \(\Delta>0\) inserted into these formulas produces one
and only one \(x\in(-1/\ee,0)\), with the displayed values equal to
\(\Wz(x)\) and \(\Wm(x)\).

Equivalently, if \(t=\ee^\Delta>1\), then
\begin{align}
 \Wz(x)&=-\frac{\log t}{t-1},&
...
\(a-b=\Delta\), and the equality \(a\ee^a=b\ee^b=x\) gives
\[
       \frac{a}{b}=\ee^{b-a}=\ee^{-\Delta}.
\]
Thus \(a=b\ee^{-\Delta}\), and subtraction yields
\[
 \Delta=a-b=b(\ee^{-\Delta}-1).
\]
Solving first for \(b\), and then for \(a\), proves
\eqref{eq:W0-gap} and \eqref{eq:Wm-gap}.  Substitution into
\(x=a\ee^a\) proves \eqref{eq:x-gap}.  Replacing
\(\ee^\Delta\) by \(t\) gives \eqref{eq:t-pair-W} and
\eqref{eq:t-pair-x}.

It remains to justify the converse and the stated range of \(x\).  For
\(\Delta>0\), the elementary inequality \(\ee^\Delta>1+\Delta\)
gives
\[
       -1<-\frac{\Delta}{\ee^\Delta-1}<0.
\]
Also
\[
 -\frac{\Delta}{1-\ee^{-\Delta}}<-1,
\]
because \(1-\ee^{-\Delta}<\Delta\).  The two displayed numbers have
...
\((-1/\ee,0)\) by \cref{thm:realbranches}, and the branch ranges identify
...
For \(-1/\ee<x<0\), with \(\Delta\) as in
\eqref{eq:branchgap},
\begin{align}
 \frac{\Wm(x)}{\Wz(x)}&=\ee^\Delta,
 \label{eq:pair-ratio}\\
...
formulas, and using
\[
 \coth(y)=\frac{\ee^{2y}+1}{\ee^{2y}-1},
 \qquad
 4\sinh^2(y)=\ee^{2y}-2+\ee^{-2y},
\]
...
\[
       \frac{\Delta}{1-\ee^{-\Delta}}
       =\Delta+\frac{\Delta}{\ee^\Delta-1}
\]
...
nonzero zeros of \(\ee^z-1\) are \(z=\pm2\pi\ii\).
\end{proof}
...
\(\Delta\sim2\sqrt{2(1+\ee x)}\).  The square-root variable is best for
evaluating a single branch near \(-1/\ee\); the gap variable is best for
...
At every real point where \(W_k(x)\neq-1\),
\begin{align}
 W_k'(x)
   &=\frac{1}{\ee^{W_k(x)}(1+W_k(x))}
     =\frac{W_k(x)}{x(1+W_k(x))},\label{eq:firstder}\\
 W_k''(x)
   &=-\frac{\ee^{-2W_k(x)}(W_k(x)+2)}
              {(1+W_k(x))^3}\label{eq:secondder-exp}\\
   &=-\frac{W_k(x)^2(W_k(x)+2)}
              {x^2(1+W_k(x))^3}.\label{eq:secondder}
\end{align}
The second forms are interpreted by limits at \(x=0\) on the principal
branch.  In particular, \(\Wz'(0)=1\) and \(\Wz''(0)=-2\).
\end{theorem}

\begin{proof}
Write \(w=W_k(x)\).  Differentiating \(w\ee^w=x\) gives
\[
 \ee^w(1+w)w'=1,
\]
which proves the first expression in \eqref{eq:firstder}.  Since
\(\ee^{-w}=w/x\), the second expression follows when \(xw\neq0\), and then
extends by continuity wherever appropriate.
...
\[
 w''=\left(-\frac{\ee^{-w}}{1+w}
           -\frac{\ee^{-w}}{(1+w)^2}\right)
           \frac{\ee^{-w}}{1+w}
     =-\frac{\ee^{-2w}(w+2)}{(1+w)^3}.
\]
Substituting \(\ee^{-w}=w/x\) proves \eqref{eq:secondder}.
The limits at zero follow either from these formulas or from the power series
proved in \cref{sec:maclaurin}.
\end{proof}

\begin{corollary}[Monotonicity recovered from the derivative]
On \((-1/\ee,\infty)\), \(\Wz'(x)>0\).  On
\((-1/\ee,0)\), \(\Wm'(x)<0\).  Moreover,
\[
 \lim_{x\downarrow-1/\ee}\Wz'(x)=+\infty,\qquad
 \lim_{x\downarrow-1/\ee}\Wm'(x)=-\infty.
\]
\end{corollary}
...
\begin{remark}[Formalized first-order calculus]
...
\(\Wz'(0)=1\) is \path{deriv_principalLambertW_zero}.  The two
vertical-tangent limits above, the second-derivative formulas
\eqref{eq:secondder-exp}--\eqref{eq:secondder} (including
\(\Wz''(0)=-2\)), and the full curvature classification below are not yet
...
\((-1/\ee,\infty)\).  The branch \(\Wm\) is
\begin{itemize}
\item strictly convex on \((-1/\ee,-2/\ee^2)\),
\item has an inflection point at \(x=-2/\ee^2\), where \(\Wm(x)=-2\),
\item strictly concave on \((-2/\ee^2,0)\).
\end{itemize}
\end{theorem}
...
\(x=w\ee^w=-2/\ee^2\).  The branch is decreasing, so the stated
\(x\)-intervals follow.
\end{proof}
...
\]
Multiplication by
\(\dd w/\dd x=\ee^{-w}/(1+w)\) yields
\[
 \frac{\ee^{-(n+1)w}P_{n+1}(w)}
 {(1+w)^{2n+1}},
\]
...
\begin{proof}
Insert \eqref{eq:firstder} into the definition of relative condition number.
\end{proof}

Thus both real branches are badly conditioned near \(-1/\ee\), where
...
\[
 \frac{\dd}{\dd w}
 \left[\ee^w(w^2-w+1)\right]
 =\ee^w(w+w^2),
\]
which proves \eqref{eq:intW}.  Since \(\ee^w=x/w\), it also proves
...
\begin{align}
 \int_{-1/\ee}^{0}\Wz(x)\dd x&=1-\frac3\ee,\label{eq:defintW0}\\
 \int_{-1/\ee}^{0}\Wm(x)\dd x&=-\frac3\ee.\label{eq:defintWm}
\end{align}
...
\(F(w)=\ee^w(w^2-w+1)\).  On the principal branch,
\(w\) runs from \(-1\) to \(0\), so
\[
 F(0)-F(-1)=1-\frac3\ee.
\]
On the nonprincipal branch, \(w\) runs from \(-1\) to \(-\infty\).
Since \(\ee^w\) decreases faster than every polynomial grows,
...
\[
 x^mW_k(x)^n\dd x
 =w^{m+n}\ee^{mw}\ee^w(1+w)\dd w
 =\ee^{aw}(w^q+w^{q+1})\dd w.
\]
...
integrals.  Recall
\[
 \Gamma(s)=\int_0^\infty t^{s-1}\ee^{-t}\dd t
 \quad(s>0),
\]
and define the upper incomplete gamma integral by
\[
 \Gamma(s,c)=\int_c^\infty t^{s-1}\ee^{-t}\dd t
 \quad(c>0).
\]
...
\begin{align}
 \int_0^\infty \frac{\Wz(x)^\alpha}{x^\beta}\dd x
...
\[
 x=w\ee^w,\qquad \dd x=\ee^w(1+w)\dd w.
\]
...
\begin{align*}
 \int_{-1/\ee}^{0}[-\Wm(x)]^\alpha(-x)^\beta\dd x
 &=\int_1^\infty y^p(y-1)\ee^{-cy}\dd y\\
 &=\int_1^\infty
   \left(y^{p+1}-y^p\right)\ee^{-cy}\dd y.
\end{align*}
In each term let \(t=cy\).  This gives \eqref{eq:mellin-Wm}.
Finally, integration by parts proves
\[
       \Gamma(s+1,c)=s\Gamma(s,c)+c^s\ee^{-c},
\]
...
\begin{equation}\label{eq:classic-mellin-example}
       \int_0^\infty\frac{\Wz(x)}{x\sqrt{x}}\dd x
       =2\sqrt{2\pi}.
\end{equation}
...
inverse of \(w\ee^w\) at \(w_0\), because
...
\[
 W_0(1+h)=\Omega+
 \frac{\ee^{-\Omega}}{1+\Omega}h
 -\frac{\ee^{-2\Omega}(\Omega+2)}
        {2(1+\Omega)^3}h^2+\bigO(h^3).
\]
Since \(\ee^{-\Omega}=\Omega\), every coefficient can be written as a
...
\end{equation}
The radius of convergence is exactly \(1/\ee\).
\end{theorem}
...
\]
The ratio test therefore gives radius \(1/\ee\).  Within that disk, the
series produced by Lagrange inversion satisfies \(W=x\ee^{-W}\) and has
\(W(0)=0\).  The local inverse is unique, so it is the principal branch.
\end{proof}

\begin{remark}[What the radius means]
The nearest singularity to the origin is the branch point \(-1/\ee\).
...
\begin{equation}\label{eq:termratio}
 \abs{\frac{T_{n+1}(x)}{T_n(x)}}
 =\abs{x}\left(1+\frac1n\right)^{n-1}
 <\ee\abs{x}.
\end{equation}

\begin{theorem}[Geometric tail bound]\label{thm:taylortail}
If \(\abs{x}<1/\ee\), \(q=\ee\abs{x}<1\), and
\(S_N(x)=\sum_{n=1}^NT_n(x)\), then
\begin{equation}\label{eq:tailbound}
 \abs{\Wz(x)-S_N(x)}
 \leq\frac{\abs{T_{N+1}(x)}}{1-q}.
\end{equation}
If \(0\leq x\leq1/\ee\), the terms alternate with decreasing magnitudes, so
the sharper estimate
\begin{equation}\label{eq:alternatingtail}
 \abs{\Wz(x)-S_N(x)}\leq\abs{T_{N+1}(x)}
\end{equation}
...
because \(3/2>1\).  Moreover, for every \(x\in[-1/\ee,0]\), the magnitude
...
continuous there, agrees with \(W_0\) on \((-1/\ee,0]\), and hence agrees at
the left endpoint by continuity.  Consequently
\[
 \sum_{n=1}^{\infty}
 \frac{(-n)^{n-1}}{n!}\left(-\frac1\ee\right)^n
 =\Wz(-1/\ee)=-1.
\]
...
There is also a structural explanation of \(T=x\ee^T\).  A rooted labelled
...
Ordinary Taylor series fail at \(x=-1/\ee\) because the derivative of the
inverse is infinite there.  The correct local variable is a square root.
...
\begin{equation}\label{eq:signedp}
 p_0(x)=+\sqrt{2(1+\ee x)},\qquad
 p_{-1}(x)=-\sqrt{2(1+\ee x)}.
\end{equation}
...
\begin{proof}
The identity \(B'=-S'B\), where \(B=\ee^{-S}\), gives
...
\begin{equation}\label{eq:branchleading}
 W_{0,-1}(x)
 =-1\mathbin{\pm}\sqrt{2\ee\left(x+\frac1\ee\right)}
 +\bigO\left(x+\frac1\ee\right),
\end{equation}
where the plus sign is for \(W_0\).  It also explains the vertical tangents:
\[
 \Wz'(x)\sim\frac{\ee}{q},\qquad
 \Wm'(x)\sim-\frac{\ee}{q}.
\]
...
\]
If \(\abs W\) is large, then \(W\) is approximately \(\log\abs x\), and a
...
\begin{align}
 ...
 [t^{m-1}]
 \left(\frac{t}{1-\ee^{-t}+\sigma t}\right)^m\notag\\
 &=\sum_{m=1}^{\infty}\sum_{k=0}^{\infty}
 \frac{(-1)^k}{m}
 \binom{m+k-1}{k}\sigma^k\tau^m
 [t^{m-1}]
 \left(\frac{t}{1-\ee^{-t}}\right)^{m+k}.
...
\[
 \Res_{t=0}\frac{t^k}{(1-\ee^{-t})^{m+k}}\dd t.
\]
Make the substitution \(z=1-\ee^{-t}\), so
\(t=-\log(1-z)\) and \(\dd t=\dd z/(1-z)\).  The residue becomes the
...
\begin{proof}
Let \(w=\Wz(x)\geq0\), so \(x=w\ee^w\).  The elementary inequality
\(1-\ee^{-w}\leq w\) is equivalent both to
\[
       \frac{x}{1+x}\leq w
\]
and to
...
\begin{equation}\label{eq:W0logbounds}
 ...
 <L.
\end{equation}
At \(x=\ee\), the first lower bound is an equality:
\(\Wz(\ee)=1\).
\end{theorem}

\begin{proof}
The function \(f(w)=w\ee^w\) is increasing for \(w>0\).  Since
\[
       L\ee^L=Lx>x,
\]
...
\(1+s<\ee^s\), which is strict for \(s>0\).

For the upper square-root bound, define
\[
 H(s)=2[1-(1-s)\ee^s]-s^2.
\]
Then \(H(0)=0\) and
\[
       H'(s)=2s(\ee^s-1)>0
\]
for \(s>0\).  Thus
\(s^2<2(1+\ee x)\), proving the upper bound.

Finally \(x=w\ee^w\).  Since \(w<0\) and \(0<\ee^w<1\), multiplication of
the negative number \(w\) by \(\ee^w\) moves it toward zero, so \(w<x\).
\end{proof}
...
\(t=\log(1+v)>0\), so \(v=\ee^t-1\) and
\(u=\ee^t-1-t\).  The strict exponential inequality
\(\ee^t>1+t+t^2/2\) gives
\[
       t<\sqrt{2u},
\]
...
\[
       (v+2t)^2>18u.
\]
Substituting \(v=\ee^t-1\) and \(u=\ee^t-1-t\), define
\[
 F(t)=(\ee^t-1+2t)^2-18(\ee^t-1-t).
\]
...
and
\[
       F^{(4)}(t)=4\ee^t(t+4\ee^t-1)>0\qquad(t>0).
\]
...
\[
       \Wm(x)<-1-\sqrt{2(1+\ee x)}.
\]
Indeed, with \(w=-1-v\),
\[
 v^2-2[1-(1+v)\ee^{-v}]
\]
has derivative \(2v(1-\ee^{-v})>0\) and vanishes at \(v=0\).
...
\[
       \frac{-7\pm\sqrt{19}}5,
\]
and both are less than \(-1/\ee\).

For the cubic denominator, write
\[
       D(x)=665x^3+3579x^2+3756x+1020.
\]
The exponential series gives \(\ee>8/3\), hence
\(-1/\ee>-3/8\).  On \([-3/8,\infty)\), the quadratic
\[
...
\]
It follows that \(D(x)>0\) for every \(x\geq-1/\ee\).  Hence the cubic
...
converge to \(\Wz(x)\) for \(\abs{x}<1/\ee\).
\end{proof}
...
Let \(z\) be a regular value of \(f(\zeta)=\zeta\ee^\zeta\), and let
...
\begin{equation}\label{eq:contour-all}
 W_k(z)^m
 =\frac{1}{2\pi\ii}\int_{\Gamma_k}
 \frac{\zeta^m\ee^\zeta(1+\zeta)}
      {\zeta\ee^\zeta-z}\dd\zeta.
\end{equation}
...
\(\sin v\sim v\) and \(v\cot v\to1\) give \(a(v)\to1/\ee\).
...
cut, write
\[
       W_0(-a+\ii0)=u+\ii v,\qquad 0<v<\pi.
\]
The condition that \((u+\ii v)\ee^{u+\ii v}\) be negative real forces
\[
       u=-v\cot v,
\]
and its real value is
\[
       -v\ee^{-v\cot v}\csc v=-a(v).
\]
Thus \(v\) parametrizes the entire cut from \(-1/\ee\) to \(-\infty\).

\begin{theorem}[Stieltjes and logarithmic integrals]\label{thm:stieltjes}
For \(z\in\mathbb C\setminus(-\infty,-1/\ee]\),
\begin{align}
 W_0'(z)
 &=\frac1\pi\int_0^\pi\frac{\dd v}{z+a(v)},\label{eq:Wprime-stieltjes}\\
 W_0(z)
 &=\frac1\pi\int_0^\pi
   \Log\left(1+\frac{z}{a(v)}\right)\dd v,\label{eq:W-log-integral}\\
 W_0(z)
 &=\frac{z}{\pi}\int_0^\pi
 \frac{(1-v\cot v)^2+v^2}{z+a(v)}\dd v.
 \label{eq:W-stieltjes}
\end{align}
For real \(z>-1/\ee\), all three integrals are real and the ordinary real
...
singularity at \(-1/\ee\).  On the upper edge of the cut,
\[
       W_0(-a(v)+\ii0)=-v\cot v+\ii v.
\]
...
\[
 F(z)=\frac1{2\pi\ii}
 \int_{-\infty}^{-1/\ee}
 \frac{F_+(t)-F_-(t)}{t-z}\dd t.
\]
Set \(t=-a(v)\).  As \(t\) runs from \(-\infty\) to \(-1/\ee\),
\(v\) runs from \(\pi\) to \(0\).  Using \eqref{eq:Fjump}, the integral
becomes
\[
       F(z)=\frac1\pi\int_0^\pi\frac{\dd v}{z+a(v)},
\]
as claimed.
...
\[
 ...
 \left(\int_0^z\frac{\dd s}{s+a(v)}\right)\dd v
 =\frac1\pi\int_0^\pi
 \Log\left(1+\frac{z}{a(v)}\right)\dd v.
\]
This proves \eqref{eq:W-log-integral}.

Finally let
\(h(v)=\Log(1+z/a(v))\).  The boundary term
\([vh(v)]_0^\pi\) vanishes: the factor \(v\) handles \(v=0\), and
\(a(v)\to\infty\) handles \(v=\pi\).  Integration by parts gives
\[
 \int_0^\pi h(v)\dd v
 =z\int_0^\pi\frac{v a'(v)}{a(v)[z+a(v)]}\dd v.
\]
...
For every integer \(n\geq0\) and every real \(x>-1/\ee\),
\begin{equation}\label{eq:complete-monotone}
 (-1)^n W_0^{(n+1)}(x)
 =\frac{n!}{\pi}\int_0^\pi
 \frac{\dd v}{[x+a(v)]^{n+1}}>0.
\end{equation}
...
For \(x>-1/\ee\), \cref{lem:av} gives \(x+a(v)>0\), so the integrand on
...
\[
 ...
 \frac{(-1)^{m-1}}m\frac{z^m}{a(v)^m}\dd v
 =
...
Let \(r=W_k(x)\), let \(\widehat w\) be an approximation on the same real
inverse interval, and let \(I\) be the closed interval joining \(r\) and
\(\widehat w\).  If \(I\) does not contain \(-1\), put
\[
       m_I=\min_{t\in I}\abs{\ee^t(1+t)}.
\]
Then
\begin{equation}\label{eq:residual-bound}
 \abs{\widehat w-r}
 \leq
 \frac{\abs{\widehat w\ee^{\widehat w}-x}}{m_I}.
\end{equation}
For \(x\neq0\), an overflow-resistant alternative uses
\[
       G_x(w)=w+\log\abs w-\log\abs x.
\]
If, in addition, \(I\) avoids zero and
\[
       \mu_I=\min_{t\in I}\abs{1+1/t}>0,
\]
then
\begin{equation}\label{eq:log-residual-bound}
       \abs{\widehat w-r}
       \leq\frac{\abs{G_x(\widehat w)}}{\mu_I}.
\end{equation}
\end{theorem}
...
\(\abs{f'(\xi)}\geq m_I\), \eqref{eq:residual-bound} follows.
...
\(f'(-1)=0\).  One should return \(-1\) exactly when \(x=-1/\ee\), or use
...
\begin{proof}
Put \(t=r/w>0\).  Since
\(\log\abs x=r+\log\abs r\), direct algebra gives the exact identities
\begin{align}
 N_x(w)-w
...
\begin{equation}\label{eq:globalstarts}
 w_0=
 \begin{cases}
  x/(1+x),&k=0,\quad 0<x<\ee,\\
  \log x-\log\log x,&k=0,\quad x\geq\ee,\\
  x,&k=0,\quad -1/\ee<x<0,\\
  -1-\sqrt{2u}-u,&k=-1,\quad
       -1/\ee<x<0,\quad u=-1-\log(-x).
 \end{cases}
\end{equation}
...
\begin{proof}
For \(0<x<\ee\), \cref{thm:W0positivebounds} gives
\(0<w_0<W_0(x)\), so case (i) of
\cref{thm:lognewton-order} applies.  For \(x>\ee\),
\cref{thm:W0logbounds} gives the same ordering; at \(x=\ee\), the start is
the exact value \(1\).

For \(-1/\ee<x<0\) on \(W_0\), \eqref{eq:W0xbound} gives
...
\[
       \log\abs{x/\ell}=\ell,
\]
or \(x=\ell\ee^\ell\).  Uniqueness on the selected inverse interval gives
\(\ell=W_k(x)\).
\end{proof}
...
\(x=-1/\ee\), and return \(0\) for \(W_0(0)\).
\item If \(1+\ee x\) is tiny, evaluate the signed Puiseux series
...
Either branch, \(x\approx-1/\ee\) &
signed Puiseux series & \(p=\pm\sqrt{2(1+\ee x)}\)\\
\(W_0\), \(x\gg1\) &
...
\(-W_0(-\ee^{-c})\in(0,1)\) and
\(-W_{-1}(-\ee^{-c})\in(1,\infty)\).  At \(c=1\), they coalesce at
...
\(x\ee^{(q/p)x}=A^{1/p}\).  Multiplication by \(q/p\) produces the defining
...
For \(m\in\N\setminus\{0\}\), \(A>0\), and \(\beta>0\), the project sets
\[
 S_{m,A,\beta}(t)=At^m\ee^{-\beta t},
 \qquad
 P_{m,A,\beta}=A\left(\frac m\beta\ee^{-1}\right)^m
\]
...
\(t2^{-t}\), its peak is \(\ee^{-1}/\log2\), and the principal specialization
...
\[
 ...
 =
 -\frac{\alpha}{\beta}\ee^{-\alpha\gamma/\beta}.
\]
Apply \(W_k\), solve for \(y\), and then for \(x\).
\end{proof}

\begin{example}
The equation \(\ee^x=3x\) has argument \(-1/3\in(-1/\ee,0)\), so it has
...
\item two if \(\ee^{-1/\ee}<A<1\);
\item one double minimum solution if \(A=\ee^{-1/\ee}\), namely
\(x=\ee^{-1}\);
\item none if \(0<A<\ee^{-1/\ee}\).
\end{itemize}
\end{theorem}

\begin{proof}
Put \(y=\log x\), so \(x=\ee^y\).  Taking logarithms of \(x^x=A\) gives
\[
       y\ee^y=\log A.
\]
Thus \(y=W_k(\log A)\), proving \eqref{eq:xtox}.  If \(\log A>0\), only
\(W_0\) is real.  If \(-1/\ee<\log A<0\), both \(W_0\) and \(W_{-1}\)
are real.  At \(\log A=-1/\ee\), they meet.  Below that value no real branch
exists.

For a direct calculus check, the derivative of
\(\log(x^x)=x\log x\) is \(1+\log x\), so \(x^x\) has its positive minimum
at \(x=\ee^{-1}\), with value \(\ee^{-1/\ee}\).
\end{proof}
...
fixed point.  For \(1<x<\ee^{1/\ee}\), the argument lies in
\((-1/\ee,0)\), so there are two positive fixed points.  At
\(x=\ee^{1/\ee}\), they merge at \(y=\ee\).  This statement concerns
...
\[
       F_n(x)=\frac{x^n}{\ee^x-1},\qquad x>0,
\]
...
\[
       y\ee^y=-n\ee^{-n},
\]
so \(x=n+W_k(-n\ee^{-n})\).  Since \(-n\leq-1\),
\cref{cor:inverse} gives
\(W_{-1}(-n\ee^{-n})=-n\), hence the boundary value \(x=0\).
...
\[
       I=I_S\left(\ee^{V_D/(nV_T)}-1\right),
       \qquad V=V_D+IR_S.
\]
...
Exponential modes \(y(t)=\ee^{\lambda t}\) have exponents
...
\[
       ...
       =a\ee^{\lambda(t-\tau)},
\]
so \(\lambda=a\ee^{-\lambda\tau}\).  Hence
\[
...
\(-1/\ee<a\tau<0\), the two real branches give two real characteristic
...
\[
       s_\infty=s_0\ee^{-R_0}\ee^{R_0s_\infty}.
\]
Thus
\[
 (-R_0s_\infty)\ee^{-R_0s_\infty}
 =-R_0s_0\ee^{-R_0},
\]
...
\]
arises when inverting the prime number theorem.  For \(n\geq\ee\), solving
...
\begin{proof}
Put \(y=\log x\), so \(x=\ee^y\).  The equation
\(x/\log x=n\) becomes
\[
       \frac{\ee^y}{y}=n,
\]
...
\begin{theorem}[Expansion centered at \(x=\ee\)]\label{thm:omega-e-series}
Let \(h=\log x-1\).  For \(x>0\) sufficiently close to \(\ee\),
\begin{align}
 W_0(x)
...
The complex derivative of \(f(w)=w\ee^w\) is
\[
       f'(w)=\ee^w(1+w).
\]
Thus \(w=-1\) is the only finite critical point, and its critical value is
\(-1/\ee\).  At every other preimage \(w_*\), the complex inverse function
theorem gives a unique analytic local branch through \(w_*\).  Near
\(-1/\ee\), the same calculation as in \cref{sec:branchpoint} gives the
...
For nonzero \(z\), choose a determination of \(\Log z\) and define
\[
       L_{1,k}=\Log z+2\pi\ii k,\qquad
       L_{2,k}=\Log L_{1,k}.
\]
For large \(\abs{k}\), a solution is close to
...
Whenever \(\abs{L_{1,k}}\to\infty\) while the logarithms are chosen
consistently,
...
\[
       \tau=1-\ee^{-v}+\sigma v,\qquad
       \sigma=1/L_{1,k},\quad \tau=L_{2,k}/L_{1,k}.
\]
The Lagrange-inversion calculation and coefficient identity are algebraic
and remain valid over \(\mathbb C\).  This proves
\eqref{eq:complex-asym}.
\end{proof}

This proof also shows that, for sufficiently large \(\abs{k}\), there is a
...
\(w\ee^w=x\).  Consistency with the asymptotic indexing gives
\[
...
\(\mathbb C\setminus(-\infty,-1/\ee]\); the other branches are organized
...
Elementary bounds for \(W_{-1}(-\ee^{-1-u})\).  A self-contained proof and
...
For \(-1/\ee<x<0\), there are two values.  The value in \((-1,0)\) is
...
\(p=\pm\sqrt{2(1+\ee x)}\).  At the unbounded ends, use logarithmic
coordinates.

\item \textbf{Keep branch qualifications in inverse identities.}
The equation \(W(w\ee^w)=w\) is true only on the inverse interval belonging
...
from \(w\ee^w=x\) and therefore apply on every regular branch.  Monotonicity,
...
\begin{equation}\label{cmp:eq:tower-interval}
 \ee^{-\ee}\le a\le\ee^{1/\ee},
\end{equation}
and throughout this interval its limit is
\(h=-W_0(-\log a)/\log a\) (with \(h=1\) at \(a=1\)).  For
\(1<a<\ee^{1/\ee}\), the second real branch \(W_{-1}\) gives another positive
...
attraction requires \(1/\ee<h<\ee\); mapping the endpoints through
\(a=h^{1/h}\) produces \eqref{cmp:eq:tower-interval}.  For
\(1<a\le\ee^{1/\ee}\), \(h^{1/h}\) increases on \([1,\ee]\), so there is a
unique fixed point \(h\in[1,\ee]\), and since \(a\le h\) and \(g\) is
increasing with \(g([a,h])\subseteq[a,h]\), the towers increase to \(h\)
(including the neutral endpoint \(a=\ee^{1/\ee}\)).  For
\(\ee^{-\ee}\le a<1\), \(g\) is decreasing: the odd and even subsequences
are monotone with limits \(p\le h\le q\) forming a two-cycle
\(p=g(q)\), \(q=g(p)\).  Writing \(u=-\log p\), \(v=-\log q\), a nontrivial
two-cycle forces \(A:=-\log a=u\ee^v=v\ee^u\), hence, with \(r=u/v>1\),
\[
 A=\frac{r\log r}{r-1}\,r^{1/(r-1)},
\]
whose logarithm is strictly increasing in \(\log r\) (this reduces to
\(2\sinh(s/2)>s\)) with infimum \(1\); so \(A>\ee\), contradicting
\(A\le\ee\).  Hence \(p=q=h\), including the neutral endpoint
\(a=\ee^{-\ee}\).  Outside the interval no convergence occurs: for
\(a>\ee^{1/\ee}\) there is no fixed point, and for \(0<a<\ee^{-\ee}\) the
unique fixed point has \(\abs{g'(h)}>1\) and the injectivity of \(g\)
prevents the iterates from reaching it exactly.  The \(W_{-1}\) fixed point
\(H>\ee\) has \(g'(H)=\log H>1\).
\end{proof}
...
\begin{equation}\label{cmp:eq:xyyx}
 y=-\frac1aW_k(-a),
\end{equation}
one branch reproducing \(y=x\) and, for \(0<a<1/\ee\), the other giving the
...
\begin{proposition}[Logarithmic fixed-point iteration; third treatment]
...
attracting precisely when \(\abs W>1\) --- on \(W_0\) only for \(x>\ee\),
on \(W_{-1}\) for every \(-1/\ee<x<0\) --- and it loses contraction at
\(W_0(\ee)=1\) and at the branch point.  Where it converges it is only
linear.
\end{proposition}

\begin{theorem}[Inverse-Taylor (Schr\"oder-type) corrections; monograph
treatment]\label{cmp:thm:inverse-taylor}
Let \(y\ne-1\) approximate a branch value, \(z_0=y\ee^y\), and
\(\Delta=x-z_0\).  Then
\begin{equation}\label{cmp:eq:schroeder}
 W(x)=y+\frac{\ee^{-y}}{1+y}\Delta
 -\frac{\ee^{-2y}(y+2)}{2(1+y)^3}\Delta^2
 +\frac{\ee^{-3y}(2y^2+8y+9)}{6(1+y)^5}\Delta^3
 +\bigO(\Delta^4),
\end{equation}
the coefficients being \(W^{(j)}(z_0)/j!\) from
\cref{thm:higherder}.  The linear truncation is exactly Newton's method for
\(w\ee^w=x\); truncation after \(\Delta^m\) turns an error
...
\cref{thm:W0positivebounds}); \(y_0=x\) for \(W_0\) on \((-1/\ee,0)\);
...
\(\sigma(t)=-W_{-1}(-t\ee^{-t})\ge1\) satisfies
\(t\ee^{-t}=\sigma(t)\ee^{-\sigma(t)}\), and
\(s\mapsto-W_0(-s\ee^{-s})\) inverts it: the two real branches encode the
two inverse sides of the unimodal map \(q\mapsto q\ee^{-q}\).  (The exact
pair parametrization of \cref{sec:pairing} is the same involution in the
gap coordinate \(\Delta\).)
\item\textbf{Scaling.}  If \(w=W_k(x)\) and \(n\in\N\), then
\[
 (nw)\ee^{nw}=\frac{nx^n}{w^{\,n-1}},
 \qquad\text{so}\qquad
 nW_k(x)=W_\ell\!\left(\frac{nx^n}{W_k(x)^{n-1}}\right)
\]
on the branch containing the value; similarly for complex \(\alpha\) with
consistent powers.
\item\textbf{Fixed points of \(W\).}  \(W_k(z)=z\) forces \(z\ee^z=z\),
i.e.\ \(z=0\) or \(z=2\pi\ii n\); on the real principal branch the only
fixed point is \(0\).
\item\textbf{Unwinding.}  The frequently written identity
\(W_k(z)=\Log z-\Log W_k(z)+2\pi\ii m\) carries a mandatory integer \(m\)
recording the logarithm discrepancy; on the real branches with absolute
values, \(W+\log\abs W=\log\abs x\) holds unambiguously.
\item\textbf{Closed Puiseux coefficients.}  Writing the branch-point
relation as \(u=q\,u/G(u)\) with
\(G(u)=u\sqrt{2\bigl(1+(u-1)\ee^u\bigr)/u^2}\), Lagrange inversion gives
...
\end{equation}
\item\textbf{Monodromy.}  Circling once around \(-1/\ee\) changes
\(p=\sqrt{2(1+\ee z)}\) to \(-p\) and interchanges the two local sheets ---
...
Lindemann--Weierstrass makes \(\ee^w\) transcendental, contradicting
\(\ee^w=z/w\) algebraic.
\end{proof}
...
\(W_0(\ee)=1\) and every manufactured value \(W_k(a\ee^a)=a\).
...
\(H\),
\(\tfrac{\dd}{\dd x}H(W(x))=H'(W(x))\,\ee^{-W(x)}/(1+W(x))\); computing
\(\dd H/\dd w\) first usually avoids nested quotient rules.
\item\textbf{Parametrize by \(w\).}  The substitution \(x=w\ee^w\) traces
...
\(A(w\ee^w)\le w\le B(w\ee^w)\).
\end{itemize}

Floating-point hazards (monograph and fourth treatments): near the branch
point, never form \(1+\ee x\) by naive subtraction when \(x\) is given as
\(-1/\ee+h\) --- compute \(p=\sqrt{2\ee h}\) from \(h\) directly or use
fused operations; for inputs so extreme that \(x\) overflows or underflows,
carry \(\log\abs x\) and use the logarithmic Newton map
\eqref{eq:loginput-newton}; and prefer the residual
\(w+\log\abs w-\log\abs x\) to \(w\ee^w-x\) as a stopping test.

\subsection{Further worked applications}

\begin{itemize}
\item\textbf{Optimal patch residence (marginal value theorem).}  With gain
\(G(t)=A(1-\ee^{-bt})\) and travel time \(\tau\), the rate-maximizing
residence time solves \(\ee^{bt}=1+b(t+\tau)\), and with \(y=1+b(t+\tau)>1\),
...
\cite{lehtonen2016}.
\item\textbf{Wien displacement.}  Maximizing \(x^5/(\ee^x-1)\) gives
\((5-x)\ee^x=5\), so \(x_*=5+W_0(-5\ee^{-5})\approx4.9651142317\); the
lower branch returns the boundary root \(x=0\) via
\(W_{-1}(-5\ee^{-5})=-5\).  (Compare \cref{thm:planckmax} with \(n=5\).)
\item\textbf{Fall time with linear drag.}  From
\(s(t)=v_\infty[t-(1-\ee^{-\gamma t})/\gamma]\) and
\(A=1+\gamma s/v_\infty\ge1\),
\(t=[A+W_0(-\ee^{-A})]/\gamma\); the lower branch gives the negative-time
...
treatment): \((ax+b)\ee^{cx+d}=r\), solved by \(W\) directly;
...
The \emph{\(r\)-Lambert function} solves \(w\ee^w+rw=z\)
\cite{mezobaricz2017}; it has
\(W_r'(z)=\bigl[\ee^{W_r}(1+W_r)+r\bigr]^{-1}\), and its branch points are
images of the critical points \(\ee^w(1+w)+r=0\), themselves expressible by
ordinary \(W\).  More generally, equations
\(\ee^x\prod_i(x-t_i)/\prod_j(x-s_j)=a\) define generalized Lambert
...
\path{LambertPhaseLockedRichardson.lean}.  The downstream modules
...
not convergence of an infinite residual series or uniformity in growing
orders.  The two-term inversion
...
For a Laurent series \(A(t)=\sum_{j\in\mathbb Z}a_jt^j\), define
\[
       \Res_{t=0} A(t)\dd t=a_{-1}.
\]
...
\[
       y(x)^m y'(x)
       =\frac1{m+1}\frac{\dd}{\dd x}y(x)^{m+1},
\]
whose residue is zero by part (i); the residue of \(y^m\dd y\) is also zero.
If \(m=-1\), write \(y(x)=c_1x[1+xB(x)]\).  Then
\[
       \frac{y'(x)}{y(x)}
       =\frac1x+\frac{\dd}{\dd x}\log[1+xB(x)],
\]
so its residue is \(1\), exactly the residue of \(\dd y/y\).  This proves
...
\begin{equation}\label{eq:coeff-residue}
       [x^n]H(y(x))
       =\Res_{x=0}\frac{H(y(x))}{x^{n+1}}\dd x.
\end{equation}
From \eqref{eq:appendix-implicit},
\[
       x=\frac{y}{\phi(y)},
       \qquad
       \dd x=\frac{\phi(y)-y\phi'(y)}{\phi(y)^2}\dd y.
\]
...
\item Prove that the equation \(w\ee^w=-1/10\) has exactly two real
solutions, and locate each one in an interval of length \(1\).

\item Let \(a\geq-1\).  Prove \(W_0(a\ee^a)=a\).  Let \(a\leq-1\).
Prove \(W_{-1}(a\ee^a)=a\).  Explain why the branch restrictions cannot be
removed.

\item Solve \(x+2\log x=5\) for \(x>0\).

\item Evaluate
\(\displaystyle\int_{-1/\ee}^{0}W_{-1}(x)\dd x\).

\item Show directly from the defining equation that
\[
       \frac{\dd}{\dd x}\ee^{W_0(x)}
       =\frac1{1+W_0(x)}
       \qquad(x>-1/\ee).
\]

\item For \(-1/\ee<x<0\), prove
...
\item Let \(x>\ee\), \(L=\log x\).  Prove that
\(L-\log L<W_0(x)<L\).
...
\item Since \(-1/\ee<-1/10<0\), \cref{thm:realbranches} gives exactly two
solutions.  On \((-1,0)\),
\((-1)\ee^{-1}<-1/10<0\), so the principal solution lies in \((-1,0)\).
For the lower branch,
\[-4\ee^{-4}\approx-0.0733>-0.1,
  \qquad -3\ee^{-3}\approx-0.1494<-0.1.
\]
Because \(w\ee^w\) is decreasing on \(( -\infty,-1)\), the second solution
lies in \((-4,-3)\).

\item The map \(w\mapsto w\ee^w\) is one-to-one from \([-1,\infty)\) onto
\([-1/\ee,\infty)\), so its inverse there sends \(a\ee^a\) back to \(a\).
This is \(W_0\).  The same argument on \(( -\infty,-1]\), where the map is
one-to-one onto \([-1/\ee,0)\), gives the \(W_{-1}\) identity.  Without the
range restriction, an input in \((-1/\ee,0)\) has two preimages, so a single
inverse formula cannot select both.

\item Divide by \(2\): \(x/2+\log x=5/2\).  Exponentiation gives
\(x\ee^{x/2}=\ee^{5/2}\).  Hence
\[
       \frac{x}{2}\ee^{x/2}=\frac12\ee^{5/2},
       \qquad
       x=2W_0\left(\frac12\ee^{5/2}\right).
\]
The argument is positive, so only \(W_0\) is real.

\item By \eqref{eq:defintWm}, the answer is \(-3/\ee\).  Directly, put
\(w=W_{-1}(x)\).  Then \(w\) runs from \(-1\) to \(-\infty\), and
\[
 \int w\ee^w(1+w)\dd w
 =\left[\ee^w(w^2-w+1)\right]_{-1}^{-\infty}
 =-\frac3\ee.
\]
...
\[
 \frac{\dd}{\dd x}\ee^{W_0(x)}
 =\ee^{W_0(x)}W_0'(x)
 =\ee^{W_0(x)}\frac{\ee^{-W_0(x)}}{1+W_0(x)}.
\]
The factors cancel.
...
\(\ee^{-1/\ee}<A<1\), then \(-1/\ee<\log A<0\) and there are two
solutions, from \(W_0\) and \(W_{-1}\).  If
\(A=\ee^{-1/\ee}\), the two coalesce at \(x=\ee^{-1}\).  If
\(0<A<\ee^{-1/\ee}\), there is no positive real solution.

\item Expand \(\ee^W=1+W+W^2/2+\bigO(x^3)\).  Then
\[
 W\ee^W=W+W^2+\frac12W^3+\bigO(x^4).
\]
Substitution gives
\[
 a_1x+(a_2+a_1^2)x^2+
...
\item Since \(L\ee^L=Lx>x\) and \(w\ee^w\) is increasing for
\(w>0\), one has \(W_0(x)<L\).  Put \(a=L-\log L\).  Then
\[
       a\ee^a=x\left(1-\frac{\log L}{L}\right)<x,
\]
...
\midrule
\endhead
Definition & \(W_k(x)\ee^{W_k(x)}=x\).\\
\addlinespace
Real domains & \(W_0:[-1/\ee,\infty)\to[-1,\infty)\), increasing;
\(W_{-1}:[-1/\ee,0)\to(-\infty,-1]\), decreasing.\\
\addlinespace
Derivative & \(W_k'(x)=W_k(x)/\{x[1+W_k(x)]\}\), with the limiting value
\(W_0'(0)=1\).\\
\addlinespace
Antiderivative &
\(\int W_k(x)\dd x=\ee^{W_k(x)}[W_k(x)^2-W_k(x)+1]+C\).\\
\addlinespace
Maclaurin &
\(W_0(x)=\sum_{n\geq1}(-n)^{n-1}x^n/n!\), \(|x|<1/\ee\).\\
\addlinespace
Branch point &
...
\(W_0=-\Delta/(\ee^\Delta-1)\) and
\(W_{-1}=-\Delta/(1-\ee^{-\Delta})\).\\
\addlinespace
Logarithmic Newton &
\(w^+=\dfrac{w}{1+w}\left(1+\log\left|x/w\right|\right)\).\\
\addlinespace
Residual certificate &
\(|w-W_k(x)|\leq|w\ee^w-x|/\min_I|\ee^t(1+t)|\), provided the interval
...
\midrule
\endhead
Real branches, domains, curvature, inflection of \(\Wm\) at \(-2/\ee^2\) &
...
\addlinespace
Maclaurin \((-n)^{n-1}/n!\), radius \(1/\ee\), tree function, Cayley &
All four; the spine adds the endpoint convergence via Raabe.\\
\addlinespace
Signed Puiseux series at \(-1/\ee\) &
