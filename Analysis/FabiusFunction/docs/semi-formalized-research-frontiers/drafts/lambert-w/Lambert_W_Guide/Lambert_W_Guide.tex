...
\(\Wm=-\Delta\EulerE^\Delta/(\EulerE^\Delta-1)\),
...
Forward direction only: the guide's converse, that every \(\Delta>0\) arises
from exactly one \(x\in(-1/\EulerE,0)\), is not formalized.  The whole
computation is \(\Wm=\Wz\EulerE^\Delta\) from dividing the two defining
equations.\\
\addlinespace
...
Not formalized, in the order they appear above: the converse half of
\Cref{thm:pairing} (that every gap \(\Delta>0\) arises from exactly one
\(x\)), the moments of \Cref{sec:integrals}, the local Taylor
