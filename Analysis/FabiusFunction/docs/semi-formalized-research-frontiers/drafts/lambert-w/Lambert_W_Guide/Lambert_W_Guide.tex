...
the exterior reciprocal-germ coefficient nor the global rational/removable
continuation at roots is formalized.  The leading-coefficient and strict
odd-degree clauses remain absent, so the compound external label
\nolinkurl{p7:thm:Pn} remains Partial.  The complex leaf raised the historical
incoming-branch checkpoint to 906/11,461, and later merged tranches formed the
historical 918/11,568 checkpoint.  The subsequent
\path{HalfQBinomialRootSimplicity.lean},
\path{RvachevLaurentLeading.lean}, and
\path{FinitePrefixAppellRecovery.lean} leaves add three modules and 36 public
declarations, so the current semantic union has 921 modules and 11,604 public
declarations.
