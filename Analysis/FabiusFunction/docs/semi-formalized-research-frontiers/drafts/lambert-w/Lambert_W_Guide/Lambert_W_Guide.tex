...
\lean{LambertWBranchGapBernoulli} has \(0+3\).  Their four-module union is
therefore four definitions and 35 theorems, 39 public declarations in all.
...
\addlinespace
\eqref{eq:pair-Bernoulli-general}: the compact paired Bernoulli series for
\(0<\Delta<2\pi\) on \(-1/\EulerE<x<0\) &
\decl{summable\_norm\_bernoulli\_mul\_pow\_div\_factorial},
\decl{hasSum\_bernoulli\_mul\_pow\_div\_factorial}, and
\decl{principalLambertW\_lowerLambertW\_eq\_bernoulliSeries} (module
\lean{LambertWBranchGapBernoulli}) &
Exact.  The first theorem proves absolute summability for every real
\(z\) with \(\lvert z\rvert<2\pi\); the second proves the actual all-index
sum \(z/(\exp z-1)\) under the additional hypothesis \(z\ne0\); and the
third specializes both sums on the strict common branch domain when the
positive branch gap is below \(2\pi\).  At \(z=0\) the convergence theorem
still applies, but the quotient theorem excludes the removable value because
Lean's totalized quotient is zero there.  This row promotes only
\eqref{eq:pair-Bernoulli-general}: it proves neither the boundary
\(\lvert z\rvert=2\pi\) nor exact maximality of the radius, and it does not
promote the surrounding expanded displays or any Puiseux claim.\\
\addlinespace
...
Not formalized, in the order they appear above: exact maximality of the
Bernoulli-series radius and its \(\lvert z\rvert=2\pi\) boundary, the moments
of \Cref{sec:integrals}, the local Taylor expansion, convergence of the
