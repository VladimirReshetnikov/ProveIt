...
\]
Consequently, if $F\equiv\ApproximationOf{F}\pmod{t^{p}}$ and
$G\equiv\ApproximationOf{G}\pmod{t^{q}}$, then
\begin{equation}\label{eq:mult-error-rule}
 FG-\ApproximationOf{F}\ApproximationOf{G}
 =\FormalBigO_t\!\left(t^{\min(p+m_0,q+n_0)}\right).
\end{equation}
\end{theorem}
\begin{proof}
...
Take
\begin{align*}
 F&=1+(L+1)t+(L^2-L)t^2+(2L+3)t^3+\FormalBigO_t(t^4),\\
 G&=1-Lt+(2L-1)t^2+(L^2+1)t^3+\FormalBigO_t(t^4).
\end{align*}
...
\begin{equation}\label{eq:product-example}
 FG=1+t+(L-1)t^2+(-L^3+4L^2+2L+3)t^3+\FormalBigO_t(t^4).
\end{equation}
...
Let
\[
 B=1-Lt+(2L-1)t^2+(L^2+1)t^3+\FormalBigO_t(t^4).
\]
...
\begin{align}\label{eq:reciprocal-example}
 ...
 +(L^3-5L^2+2L-1)t^3\notag\\
 &+(L^4-8L^3+7L^2-6L+1)t^4+\FormalBigO_t(t^5).
\end{align}
...
If $S=\FormalBigO_t(t^q)$ with $q>0$, then the coefficient of $t^N$ in
...
If $A=\FormalBigO_t(1)$ contains no $X$ term, then
\begin{equation}\label{eq:A-derivative-small}
 \D A=\FormalBigO_t(t).
\end{equation}
Consequently, changing an argument by an error $E=\FormalBigO_t(t^r)$ changes
$A$ by roughly $A'E=\FormalBigO_t(t^{r+1})$.  This one-block gain is the mechanism
...
\end{theorem}
\begin{proof}
Because $U=\FormalBigO_t(t)$, the coefficient of $t^r$ in $(1+U)^{-n}$ or
...
\begin{align*}
 ...
 &=L+Lt-\frac12L^2t^2+\frac13L^3t^3
   -\frac14L^4t^4+\FormalBigO_t(t^5),\\
 \frac1G
 &=t(1+Lt)^{-1}\\
 &=t-Lt^2+L^2t^3-L^3t^4+\FormalBigO_t(t^5).
\end{align*}
Therefore
...
\begin{align}\label{eq:F-G-coeff-notation}
 F(X)&=X+f_0(L)+t f_1(L)+t^2f_2(L)+t^3f_3(L)+\FormalBigO_t(t^4),\\
 G(X)&=X+g_0(L)+t g_1(L)+t^2g_2(L)+t^3g_3(L)+\FormalBigO_t(t^4).
\end{align}
Let
\[
 H=F\compose G
 =X+h_0+t h_1+t^2h_2+t^3h_3+\FormalBigO_t(t^4).
\]
...
\begin{equation}\label{eq:composition-noncommutative}
 F\compose G-G\compose F=-L^2t+\FormalBigO_t(t^2).
\end{equation}
...
$A-B=\FormalBigO_t(t^r)$.  If $H=X+C$ is fixed, then
\begin{equation}\label{eq:composition-continuity-left}
 F-G=\FormalBigO_t(t^r)
 \quad\Longrightarrow\quad
 H\compose F-H\compose G=\FormalBigO_t(t^r),
\end{equation}
and, because $H'=1+\FormalBigO_t(t)$,
\begin{equation}\label{eq:composition-continuity-right}
 F\compose H-G\compose H=\FormalBigO_t(t^r).
\end{equation}
...
\eqref{eq:h1},
\begin{align*}
 F\compose G&=X+f+g+t\,g f'+\FormalBigO_t(t^2),\\
 G\compose F&=X+f+g+t\,f g'+\FormalBigO_t(t^2).
\end{align*}
Hence
\begin{equation}\label{eq:first-commutator}
 F\compose G-G\compose F
 =t(gf'-fg')+\FormalBigO_t(t^2).
\end{equation}
...
\begin{equation}\label{eq:contraction-general}
 U-V=\FormalBigO_t(t^r)
 \quad\Longrightarrow\quad
 \Phi(U)-\Phi(V)=\FormalBigO_t(t^{r+1}).
\end{equation}
Then the iterates $U_{n+1}=\Phi(U_n)$ stabilize one additional block per step:
$U_{n+1}-U_n=\FormalBigO_t(t^n)$ after a suitable initialization.  This is the
...
If $B-C=\FormalBigO_t(t^r)$, with $B,C\in\mathcal C_L[[t]]$, then
\begin{equation}\label{eq:inverse-contraction}
 A(Y+B)-A(Y+C)=\FormalBigO_t(t^{r+1}).
\end{equation}
\end{lemma}
\begin{proof}
...
$B-C=\Phi(B)-\Phi(C)=\FormalBigO_t(t^{r+1})$, a contradiction.
\end{proof}
...
$B=-A(Y+B)$ is contractive in outer order because $A'=\FormalBigO_t(t)$.  Simple
...
\[
 F\compose G_{<n}=Y+r_nt^n+\FormalBigO_t(t^{n+1}).
\]
Adding $g_nt^n$ to $G$ changes $F(G)$ by
\[
 F'(G_{<n})g_nt^n+\FormalBigO_t(t^{n+1})
 =g_nt^n+\FormalBigO_t(t^{n+1}),
\]
because $F'=1+\FormalBigO_t(t)$.  Hence
...
\begin{equation}\label{eq:parametric-F}
 F(X)=X+aL+b+(cL+d)t+(eL^2+fL+g)t^2+\FormalBigO_t(t^3).
\end{equation}
The inverse has the form
\[
 G(Y)=Y+g_0(L)+g_1(L)t+g_2(L)t^2+\FormalBigO_t(t^3),
\]
with
\begin{align}
...
\begin{summarybox}
Series reversal is triangular because $F'=1+\FormalBigO_t(t)$.  Once an approximate
...
\end{equation}
The denominator is a unit because $F'=1+\FormalBigO_t(t)$.

\begin{theorem}[Precision doubling]\label{thm:newton-doubling}
If $R=\FormalBigO_t(t^m)$, then the residual after
\eqref{eq:newton-compositional} is at least
$\FormalBigO_t(t^{2m+2})$ when $m\ge0$.  In particular, the number of correct outer
blocks approximately doubles at each step.
\end{theorem}
\begin{proof}
Put $C=-R/(F'\compose G)$.  Then $C=\FormalBigO_t(t^m)$.  Taylor expansion gives
\begin{align*}
 F\compose(G+C)-Y
 &=R+(F'\compose G)C
...
\begin{equation}\label{eq:W-error-from-residual}
 w-w_N=\delta_N+\BigO\!\left(
 \frac{R_N^2}{\AbsoluteValue{w_N}^2}\right)
\end{equation}
...
For $f(X)=X+\BigO(\log X)$, one has $f'(X)=1+\LittleO(1)$ under the relevant
...
$\SupportOf{T}$&Set of monomials or outer exponents occurring in $T$.\\
