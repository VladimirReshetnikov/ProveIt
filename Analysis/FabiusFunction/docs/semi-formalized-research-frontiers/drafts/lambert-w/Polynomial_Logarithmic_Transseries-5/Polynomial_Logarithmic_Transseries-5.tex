...
\begin{equation}\label{eq:support-Minkowski}
 \SupportOf{FG}\subseteq \SupportOf{F}+\SupportOf{G}.
\end{equation}
...
Consider
\begin{align*}
 F&=1+(L+1)t+(L^2-2L)t^2+(3-L)t^3+\FormalBigO_t(t^4),\\
 G&=2-Lt+(L^2+1)t^2+2Lt^3+\FormalBigO_t(t^4).
\end{align*}
...
\begin{equation}\label{eq:worked-product}
 FG=2+(L+2)t+(2L^2-5L+1)t^2+(3L^2+L+7)t^3+\FormalBigO_t(t^4).
\end{equation}
...
\begin{equation}\label{eq:reciprocal-example-C}
 \frac1B=1-Lt+t^2+(L^3-2L)t^3+(-L^4+L^2+1)t^4+\FormalBigO_t(t^5).
\end{equation}
...
Let
\begin{align}
 F&=x+A_0(L)+A_1(L)t+A_2(L)t^2+A_3(L)t^3+\FormalBigO_t(t^4),
 \label{eq:F-near-low}\\
 G&=x+B_0(L)+B_1(L)t+B_2(L)t^2+B_3(L)t^3+\FormalBigO_t(t^4),
 \label{eq:G-near-low}
\end{align}
and write
\begin{equation}\label{eq:H-near-low}
 F\compose G=x+C_0(L)+C_1(L)t+C_2(L)t^2+C_3(L)t^3+\FormalBigO_t(t^4).
\end{equation}
...
\begin{equation}\label{eq:triangular-perturbation}
 ...
 =F\compose G+c(L)t^N+\FormalBigO_t(t^{N+1}).
\end{equation}
\end{lemma}
...
\[
 ...
 \qquad \delta=c(L)t^N.
\]
Since \(F'=1+\FormalBigO_t(t)\), the linear term is
\(c(L)t^N+\FormalBigO_t(t^{N+1})\).  Also \(F''=\FormalBigO_t(t^2)\), so every term of degree at least two in \(\delta\) has order greater than \(N\).
\end{proof}
...
\begin{equation}\label{eq:reversion-residual-N}
 F\compose G_{<N}=x+r_N(L)t^N+\FormalBigO_t(t^{N+1}).
\end{equation}
...
Write
\begin{align}
 F={}&x+A_0+A_1t+A_2t^2+A_3t^3+\FormalBigO_t(t^4),\label{eq:F-inverse-low}\\
 F^{\rev}={}&x+B_0+B_1t+B_2t^2+B_3t^3+\FormalBigO_t(t^4).
...
\begin{equation}\label{eq:omega-residual-formal}
 ...
 =-\LambertPolynomial{N+1}(L)x^{-N-1}+\FormalBigO_t(t^{N+2}).
\end{equation}
...
Let
\begin{align*}
 A&=1+(2L-1)t+(L^2+3)t^2+(L^3-L)t^3+\FormalBigO_t(t^4),\\
 B&=1+Lt+(L^2-1)t^2+\FormalBigO_t(t^4).
\end{align*}
Using the reciprocal from \cref{eq:reciprocal-example-C} and multiplying,
\begin{align}
 \frac AB={}&1+(L-1)t+(-L^2+L+4)t^2
 +(L^3-4L-1)t^3+\FormalBigO_t(t^4).
...
\[
 F=x+A_0(L)+\FormalBigO_t(t),
 \qquad
 G=x+B_0(L)+\FormalBigO_t(t).
\]
From \cref{eq:C1-compose},
\begin{align}
 F\compose G
 &=x+A_0+B_0+B_0A_0't+\FormalBigO_t(t),\notag\\
 G\compose F
 &=x+A_0+B_0+A_0B_0't+\FormalBigO_t(t).
\end{align}
Therefore
\begin{equation}\label{eq:composition-commutator-leading}
 F\compose G-G\compose F
 =\bigl(B_0A_0'-A_0B_0'\bigr)t+\FormalBigO_t(t^2).
\end{equation}
