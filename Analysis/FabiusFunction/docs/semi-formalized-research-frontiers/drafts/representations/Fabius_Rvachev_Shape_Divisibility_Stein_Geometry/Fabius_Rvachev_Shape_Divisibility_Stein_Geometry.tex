...
factors indexed by $0\le n\le\TwoAdicValuation(m)$ vanish simply and all later factors are
nonzero.  Hence
\begin{equation}\label{eq:zero-order}
 \OrderOperator_{\xi=m}\Phi=1+\TwoAdicValuation(m).
\end{equation}
...
zero of the entire characteristic function $\CharacteristicFunction{\mu}$ has multiplicity
divisible by $r$.
\end{lemma}
...
identity $\CharacteristicFunction{\mu}=\CharacteristicFunction{\nu}^{\,r}$ extends to $\ComplexNumbers$.  At every zero, the order
...
\[
 \CharacteristicFunction{\alpha}(\xi)=\prod_{(m,j)\in A}
 \cos\!\left(\frac{\pi\xi}{2^m}\right),\qquad
 \CharacteristicFunction{\beta}(\xi)=\prod_{(m,j)\in B}
...
If $x\in(-1,1)$ is dyadic rational, then $\tau(x)\in\RationalNumbers$.
\end{corollary}
...
Given an integer-valued zero divisor bounded by $1+\TwoAdicValuation(m)$, characterize when
...
\[
 H(x)=\int_{\RealNumbers}\IndicatorOf{t\le x}p(t)\,dt.
\]
...
$0\le n\le\TwoAdicValuation(m)$.  Each zero is simple because $\sin z$ has simple zeros at
nonzero integer multiples of $\pi$, and the nonvanishing denominator does not
change the order.  The local product therefore has zero order $1+\TwoAdicValuation(m)$.
