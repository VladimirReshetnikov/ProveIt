...
\hypersetup{
  ...
  citecolor=deepgreen,
  urlcolor=linkblue,
  pdftitle={Exact Rvachev Up-Function Polynomial Synthesis},
  pdfsubject={Finite exact polynomial synthesis, Lagrange cardinal loops,
    Legendre spectral closure, operator transmutation, and self-reconstruction
    by Rvachev up-functions},
  pdfauthor={Consolidated frontier volume for Vladimir Reshetnikov's ProveIt project},
...
\title{\bfseries Exact Rvachev Up-Function Polynomial Synthesis\\[0.4em]
\large Dictionaries, Lagrange cardinal loops, Legendre spectral closure,\\
operator transmutation, and self-reconstruction}
...
\begin{abstract}
\noindent
Let $u=\RvachevUp$ be Rvachev's even $C^\infty$ probability density supported
on $[-1,1]$.  This volume consolidates thirteen independently developed
frontier reports: three exact-polynomial synthesis packages, six
Lagrange--Rvachev cardinal-loop packages, and four Legendre--Rvachev
spectral/self-reconstruction packages.  Their shared problem is to represent
polynomials exactly on a compact interval by finite signed sums of shifted and
scaled copies of $u$, then close the representation with interpolation or
orthogonal analysis.  Repeated foundations and equivalent matrix notations
are stated once; genuinely different constructions and consequences are
proved in a common notation.
...
$A_d(z)=\prod_{m=1}^{d}\QInteger{2^{m}}{z}$ of degree $2M-d-2$.  The one-cell
...
$\TwoAdicValuation(m)$, the physical scale is free, and an explicit interval
...
=h^{-d}2^{-\binom d2}\sum_k\ThueMorseSign{k}\,u(\cdots)$ --- Thue--Morse signs,
...
\emph{Lagrange and Legendre closure.}  Exact cardinal synthesis produces a
canonical right inverse and oblique coefficient projector, a one-dimensional
Thue--Morse defect completion, nested detail idempotents, signed-decoder and
Lebesgue barriers, Appell--Vandermonde determinant growth, and exact
Gaussian-binomial rows.  Fourier--Legendre analysis produces finite
biorthogonal up-atom blocks, reverse local closure, rational spectral
identities, positive energy series, refinement null trains, and exact boundary
jets.  The deconvolved family $Q_n^-=\MM_u(D)^{-1}P_n$ and the distinct
smoothed family $R_n=\MM_u(D)P_n$ carry dual pullback geometries with
coordinates $X_\mp=x\mp K'(D)$; their operator recurrences,
Christoffel--Darboux kernels, Legendre--Appell connection determinant,
central reflected-pair $q$-determinant, factorial/Lambert asymptotics, and
non-flattening theorem are proved below.  Provenance of every absorbed source
and the exact-computation boundary are recorded in
\cref{sec:provenance-poly}.
\end{abstract}
...
\markboth{Provenance and merge conventions}{Provenance and merge conventions}
\addcontentsline{toc}{section}{Provenance and merge conventions}

This volume is an editorial consolidation (30 August 2026) of thirteen
source packages: the three polynomial-synthesis drafts already merged here,
six Lagrange--Rvachev reports, and four Legendre--Rvachev reports.  Shared
foundations are stated once, each genuinely different construction keeps a
coherent chapter, and overlapping Appell calculus, Thue--Morse products,
rationality, projector identities, conditioning statements, and Lean
roadmaps are deduplicated.
...
\begin{statusbox}{Status discipline}\small
...
and problems are labeled.  Most of the finite dictionaries, local dimensions,
algorithms, and atom-count results are not formalized in Lean.  The focused
2026-08-30 module \path{CompositeMeshSharpness.lean} now formalizes only the
natural-mesh, universally quantified shifted-comb threshold described below;
its claim boundary is recorded after \cref{thm:scale-classification}.
The existing module \path{LagrangeRvachevSynthesis.lean} makes
\cref{thm:lag-cardinal} Exact/Complete by assembly, without a redundant
report-shaped wrapper:
\path{normalized_sum_Ioo_lagrangeRvachevDecoder_mul_shifted_rvachevUp}
is the individual-cardinal identity, and
\path{sum_Ioo_lagrangeRvachevAtomCoefficient_mul_shifted_rvachevUp}
is its arbitrary-data counterpart.  The Lean hypotheses match the positive
mesh, two-adic degree admissibility, $[-1,1]$ evaluation domain, open lattice
block $\lvert k\rvert<2M$, spacing $h=M^{-1}$, and literal unit-radius
translates of the theorem.  Injectivity is unnecessary for the polynomial
identities and is supplied by the report's distinct-node assumption for the
cardinal reading.  This does not promote \cref{thm:lag-right-inverse}.
The current source API \path{LagrangeRvachevMatrix.lean} exactly formalizes
the finite Markov encoder, decoder row-unit law, right inverse, and the
strictly-positive-overlap sign obstruction of \cref{prop:lag-markov}.  For the
larger \cref{thm:lag-right-inverse}, it formalizes only the boxed $UB=I$
clause; the $BU$ projector and the remaining basis, spectral, moment,
intertwining, and Cauchy--Binet clauses remain human proofs.
The subsequent source-only module \path{RvachevLagrangeNodesOnly.lean}
formalizes \cref{cor:lag-nodes-only} completely: its one definition and
fourteen theorems give the ordinary-derivative and raw-node Appell forms,
coefficientwise rational descent and rational lattice samples, and the formal
complete-Bell/Bernoulli--Mersenne description of the reciprocal coefficients.
This is a compositional Exact result, not a claim of analytic reciprocal-MGF
convergence; it asserts value rationality only at rational evaluation points
and leaves the unformalized clauses of \cref{thm:lag-right-inverse}
unchanged.  Its promotion is independent of the preceding exact synthesis
assembly for \cref{thm:lag-cardinal}.
\end{statusbox}

\begin{statusbox}{Status discipline: matrix and source-only leaves}
The compiled module \path{RvachevLegendreCentralSum.lean} makes
\cref{cor:leg-central-sum} Exact/Complete.  Its exhaustive public surface has
zero definitions and three theorems:
\path{eval_legendrePolynomial_even_zero},
\path{eval_rvachevLegendreDeconvolutionPolynomial_even}, and
\path{rvachevLegendreCentralSum}.  These respectively supply the normalized
central Legendre value, the pointwise evenness of the deconvolved even mode,
and the literal finite identity at $M=4^n$ (including $n=0$), using the
existing synthesis theorem and compact support.  The theorem allows any
bounded Fabius realization satisfying the canonical axioms.  It does not
formalize the Jacobi formula, all-degree parity or rationality in
\cref{thm:leg-mode-synthesis}, reverse closure, target-specific mesh
minimality, or any clause of \cref{thm:lag-right-inverse}.
The compiled source-only module
\path{RvachevLegendreBiorthogonality.lean} makes only
\cref{thm:leg-biorthogonality} Exact/Complete.  Its one definition,
\path{rvachevLegendreAnalysisKernel}, is the displayed
$(2m+1)/2$-normalized integral, and its one theorem,
\path{rvachevLegendreBiorthogonality}, has exactly the positive-mesh condition
\path{M != 0}, admissibility \path{l <= padicValNat 2 M}, open block
$\lvert k\rvert<2M$, factor $M^{-1}$, and Kronecker normalization.  It
includes degree zero and allows any bounded Fabius realization satisfying the
canonical axioms.  It does not formalize the other support, smoothness,
Fourier--Bessel, or rationality clauses of \cref{thm:leg-Lambda}, nor the
matrix/projector conclusions of \cref{cor:leg-biorthogonal-matrices}, reverse
closure, the full mode theorem, or a Lagrange right inverse.
Independently, the existing \path{TwoAdic.lean} declarations
\path{moment_padicVal_two} and \path{moment_num_den_odd} make
\cref{thm:leg2-moment-units} Exact/Complete.  Lean's \path{moment m} is
$\mu_{2m}$: the first declaration gives valuation zero, and the second gives
odd reduced numerator and denominator, equivalently residue one modulo two
for a rational two-adic unit.  Both include $m=0$.  This is the report's
rational valuation/residue convention, not a new construction in a separate
topological $\RationalNumbers_2$ type, and it promotes no neighboring Legendre row.
\end{statusbox}

\subsection*{Notation}

$u=\RvachevUp$ as in the corpus; $\FourierTwoPi{f}(\xi)=\int f\,\EulerE^{-2\pi\ImaginaryUnit\xi x}\Differential x$;
$\FourierTwoPi{u}(\xi)=\prod_{j\ge0}\SincPi(\xi/2^{j})$ with
$\OrderOperator_{\xi=n}\FourierTwoPi{u}=1+\TwoAdicValuation(n)$ at nonzero integers $n$.  The
normalized Fabius function on the unit interval is
$\FabiusBounded(t):=u(1-t)$ for $0\le t\le1$; hence
$u(x)=\FabiusBounded(1-|x|)$ on $[-1,1]$.  Where an endpoint estimate evaluates
$\FabiusBounded(t)$ for $t\ge1$, we use the explicit constant-extension convention
$\FabiusBounded(t)=1$.  The
probability model is $Z=\sum_{j\ge1}2^{-j}V_j$, $V_j$ i.i.d.\ uniform
on $[-1,1]$, with density $u$, moment generating function
$\MM_u(z)=\Expectation\,\EulerE^{zZ}=\prod_{j\ge1}\frac{\sinh(z/2^{j})}{z/2^{j}}$,
...
\begin{equation}
 \begin{aligned}
 G_n(y)&:=\Expectation\,(y-Z)^{n}=\MM_u(D)\,y^{n}
...
$\gamma_r=\ExponentialCompleteBellPolynomial{r}(-\kappa_1,\dots,-\kappa_r)$ (complete Bell
polynomials in the negated cumulants).  Thue--Morse signs are
$\ThueMorseSign{r}=(-1)^{\BinaryDigitSum(r)}$, the finite product is
$\ThetaTM_m(z)=\prod_{j<m}(1-z^{2^{j}})=\sum_{r<2^{m}}\ThueMorseSign{r}z^{r}$,
and $p_m(N)$ denotes the restricted binary partition numbers
\eqref{eq:pm-def}.  $\PiD$ is the space of real polynomials of degree
at most $d$, and $\QInteger{q}{z}=(1-z^{q})/(1-z)$ the $q$-integer.

\section{The synthesis problem and its boundary conditions}
\label{sec:problem}

Given $p\in\RealNumbers[x]$ of degree at most $d$ and a nondegenerate interval
...
\end{equation}
with real amplitudes and shifts and positive scales.  Three structural
...
so no nonzero polynomial admits a global identity on $\RealNumbers$; the
...
even for constants.}  Negative target polynomials force signed amplitudes in
the unrestricted problem because every up-atom is nonnegative.  Independently,
...
orders of $\FourierTwoPi{u}$, and the corpus's non-elementarity theorem,
...
functions are
$\sum_nG_n(y)t^{n}/n!=\EulerE^{yt}\MM_u(t)$ and
$\sum_n\Gd_n(y)t^{n}/n!=\EulerE^{yt}/\MM_u(t)$; both satisfy the Appell
...
\subsection{Thue--Morse products, \texorpdfstring{$q$}{q}-integers, and their Green
sequences}
...
\begin{equation}
 ...
 \qquad
 A_d(z)=\prod_{m=1}^{d}\QInteger{2^{m}}{z}
...
$k_j\in\NaturalNumbers$; equivalently \eqref{eq:three-products} holds, and
\begin{equation}
...
$\sum_{r}\ThueMorseSign{r}\,p_m(N-r)=\delta_{N0}$, i.e.\
...
\label{thm:A-factor}
With $M=2^{d}$ and $\omega=\EulerE^{2\pi\ImaginaryUnit/M}$,
\begin{equation}
 A_d(z)=\prod_{j=1}^{M-1}\bigl(z-\omega^{j}\bigr)^{\TwoAdicValuation(j)+1}
...
$\TwoAdicValuation(j)=d-\ell$, required with multiplicity $d-\ell+1$; since
$\QInteger{2^{m}}{z}=\prod_{\ell\le m}(z^{2^{\ell-1}}+1)$, the multiplicities
...
\begin{equation}
 ...
 =(-1)^{d+1}(1-z)\sum_{r=0}^{M-1}\ThueMorseSign{r}\,z^{2r}.
 \label{eq:TM-factor}
\end{equation}
...
\begin{equation}
 ...
 \qquad
 \FourierTwoPi{\psi_{\rho,k}}(\xi)=\EulerE^{-2\pi\ImaginaryUnit k\xi}\,\FourierTwoPi{u}(\rho\xi),
 \label{eq:psi-def}
\end{equation}
supported on $[k-\rho,k+\rho]$ with unit integral.

\subsection{Why the minimal cardinal ratio is
\texorpdfstring{$2^{d}$}{2 to the d}}

\begin{theorem}[Scale classification]
\label{thm:scale-classification}
The integer translates $\{\psi_{\rho,k}\}_{k\in\IntegerNumbers}$ form a partition
of unity iff $\rho\in\PositiveIntegers$; for positive integer $\rho$ the Strang--Fix
polynomial reproduction order of the system is exactly
$\TwoAdicValuation(\rho)$.  Hence the smallest ratio reproducing all of $\PiD$ is
...
$\FourierTwoPi{u}(\rho n)$, which vanishes for all $n\ne0$ iff $\rho n$ is
always a nonzero integer, i.e.\ $\rho\in\PositiveIntegers$.  For integer $\rho$ and
$n\ne0$, $\OrderOperator_{\xi=n}\FourierTwoPi{u}(\rho\,\cdot)
=1+\TwoAdicValuation(\rho)+\TwoAdicValuation(n)$, minimal at odd $n$; the classical
Strang--Fix equivalence \cite{StrangFix1973,Jia1998} gives order
$\TwoAdicValuation(\rho)$.
\end{proof}

The exact Lean anchor for the natural-mesh universal slice is
\path{rvachevCombExactThrough} in \path{CompositeMeshSharpness.lean}.
The focused build exports that one definition and the seven theorems
...
\path{isLeast_rvachevCombExactThrough_even}.  They prove, for nonzero natural
$M$, that shifted-comb exactness at every real shift for every real polynomial
of degree at most $d$ is equivalent to $d\le\TwoAdicValuation(M)$, equivalently
$2^d\mid M$; the monomial of degree $\TwoAdicValuation(M)+1$ fails at some shift; and
$2^d$ is the least such universal mesh, with $4^N$ least for the complete
space through degree $2N$.  This does \emph{not} formalize the full theorem
above for arbitrary real $\rho$, the Strang--Fix equivalence as such, an
individual target polynomial, a Legendre mode or partial sum, the local
dictionary dimension, or any atom-count minimality claim.
...
For every $Q\in\PiD$,
\begin{equation}
 \sum_{k\in\IntegerNumbers}Q(k)\,\psi_k(t)
 =\int_{-1}^{1}Q(t-My)\,u(y)\Differential y
 =\MM_u(MD)\,Q(t),
...
sample $\FourierTwoPi{f_t}(n)$ is a combination of derivatives of
$\FourierTwoPi{u}(M\xi)$ through order $d$ at $\xi=n$, and
$\OrderOperator=1+\TwoAdicValuation(Mn)=d+1+\TwoAdicValuation(n)>d$, so every alias vanishes.  Evenness
...
\begin{proof}
On $\PiD$ the constant-coefficient operator $\MM_u(MD)$ is unipotent
triangular in degree, so its inverse is the displayed terminating series.
Apply \cref{lem:poisson-collapse} to this $Q$:
\[
 \frac1M\sum_{k\in\IntegerNumbers}Q(k)u\!\left(\frac{t-k}{M}\right)
 =\MM_u(MD)Q(t)=P(t).
\]
For $P(t)=t^n$, the defining Appell identity
$\MM_u(D)\Gd_n=t^n$ and the change of variable $t\mapsto t/M$ give
$\MM_u(MD)\bigl(M^n\Gd_n(t/M)\bigr)=t^n$, which proves the final
assertion by uniqueness of the inverse image.
\end{proof}
...
Let $\omega=\EulerE^{2\pi\ImaginaryUnit/M}$.  For every $1\le j<M$ and
$0\le s\le\TwoAdicValuation(j)$,
\begin{equation}
 \sum_{k\in\IntegerNumbers}k^{s}\,\omega^{jk}\,\psi_k(t)=0
 \qquad(t\in\RealNumbers),
 \label{eq:alias-zero}
\end{equation}
and these
$N_d^{\rm rel}:=\sum_{j=1}^{M-1}(\TwoAdicValuation(j)+1)=2M-d-2$ relations are
linearly independent on any $N_d^{\rm rel}$ consecutive integers.  The solution
...
order $s$, by the coset $j/M+\IntegerNumbers$; there the multiplier
$\FourierTwoPi{u}(M\xi)$ vanishes to order
$1+\TwoAdicValuation(j+Mn)=1+\TwoAdicValuation(j)>s$, since $0<j<M$.  Independence is the
confluent Vandermonde theorem for the distinct roots
$\omega^{j}$ with multiplicities $\TwoAdicValuation(j)+1$; the count is
$\sum_{r<d}2^{d-r-1}(r+1)=2M-d-2$.
\end{proof}

\begin{theorem}[Exact local dimension]
\label{thm:local-dim}
Let $V_d=\SpanOperator\{\psi_k|_{[0,1]}:-M+1\le k\le M\}$.  Then
\begin{equation}
 \boxed{\;\dim V_d=d+2,
 \qquad
 V_d\cap\RealNumbers[t]=\PiD.\;}
 \label{eq:local-dim}
\end{equation}
...
\begin{proof}
The $N_d^{\rm rel}$ relations give
$\dim V_d\le2M-N_d^{\rm rel}=d+2$;
...
$V_d\subseteq\RealNumbers[t]$ would make every atom polynomial, the same
...
$\SpanOperator\{f_r\}\cap\PiD$ is a finite union of proper subspaces, of
...
$N_d^{\rm rel}$ values $q_k=Q(k)$,
$k=-M+1,\dots,-M+N_d^{\rm rel}$, by the monic integer
...
vanishes; $q-n$ is supported on the last
$2M-N_d^{\rm rel}=d+2$ centers, so
...
\tfrac{661024}{135},-\tfrac{2567968}{675}\bigr)
\]
at centers
$11,\dots,16$.
...
The recurrence route touches $N_d^{\rm rel}\asymp2^{d+1}$ values to produce
...
$L=\sum_mU_m$, so $(A_d(E)Q)(x)=A_d(1)\,\Expectation\,Q(x+L)$.  Centering $L$
at its mean $N_d^{\rm rel}/2$ and using
$K+N_d^{\rm rel}/2=-d/2$,
\[
 g_r=A_d(1)\Bigl[
 \Expectation\,\EulerE^{(L-N_d^{\rm rel}/2)D}\;\MM_u(MD)^{-1}P
 \Bigr]\!\left(r-\tfrac d2\right),
\]
and the two product formulas
\begin{gather*}
 \Expectation\,\EulerE^{z(L-N_d^{\rm rel}/2)}
...
=\sum_mB_{2m}z^{2m}/(2m\,(2m)!)
\]
together with the cumulant series
of $\MM_u$.
\end{proof}
...
For $P\in\IntegerNumbers[t]$ the denominators of all $b_k$ divide
$\LeastCommonMultiple (\operatorname{den}\gamma_0,\dots,
...
\label{thm:multicell}
For $J\ge1$, the space
$V_{d,J}=\SpanOperator\{\psi_k|_{[0,J]}:-M+1\le k\le J+M-1\}$ has
\begin{equation}
 \dim V_{d,J}=J+d+1,
 \qquad V_{d,J}\cap\RealNumbers[t]=\PiD,
...
\begin{proof}
The same $N_d^{\rm rel}$ global relations stay independent, so
$\dim\le J+2M-1-N_d^{\rm rel}=J+d+1$; the first-cell endpoint basis plus the
...
cell into $V_d\cap\RealNumbers[t]=\PiD$.  Recurrence elimination leaves exactly
...
\begin{equation}
 ...
 (-1)^{d+1}\sum_{r=0}^{M-1}\ThueMorseSign{r}\,
 \bigl(q_{2r}-q_{2r+1}\bigr).
 \label{eq:certificate-functional}
\end{equation}
Then $\sum_jq_j\psi_{-M+1+j}$ is polynomial on $[0,1]$ \emph{iff}
$\mathfrak C_d(q)=0$; and for the test sequence
$q_j=(-M+1+j)^{d+1}$,
$\mathfrak C_d(q)=A_d(1)\,(d+1)!$.
\end{theorem}
...
$\mathfrak C_d$ vanishes on the $(2M-1)$-dimensional polynomial
...
\begin{equation}
 \boxed{\;
 \mathcal E_d(t)=
 -\frac{(d+1)!}{(2\pi\ImaginaryUnit)^{d+1}}
 \sum_{\substack{n\in\IntegerNumbers\\ n\ \mathrm{odd}}}
 \frac{\FourierTwoPi{u}(n/2)}{n^{d+1}}\,\EulerE^{2\pi\ImaginaryUnit nt},
 \;}
 \qquad
...
\begin{equation}
 V_d=\PiD\oplus\SpanOperator\{\mathcal D_d\},
 \qquad
 c_{\mathrm{def}}(q)
 =\frac{\mathfrak C_d(q)}{A_d(1)\,(d+1)!}
...
(zero order $1+\TwoAdicValuation(Mn)>m$) and at odd $n$ only the leading jet of
$\FourierTwoPi{u}(M\xi)$ survives; by the corpus's leading-jet formula it
equals $-m!\,\FourierTwoPi{u}(n/2)/n^{m}$, giving
...
$V_d\cap\RealNumbers[t]=\PiD$); the corrected $\mathcal D_d=S_m-(\mathcal
...
For every $d\ge0$ and $t\in\RealNumbers$,
\begin{equation}
 \boxed{\;
 \mathcal E_d(t)
 =(-1)^{d+1}\,2^{\,d(d+1)}\,
 \Bigl(Q_{d,d+1}(t)-\int_{-1}^{1}x^{d+1}u(x)\Differential x\Bigr).
 \;}
...
$\FourierTwoPi{u}$ at the order-$(d{+}1)$ zeros: the companion volume's
first-failure theorem gives
\[
 Q_{d,d+1}(\theta)-\mu_{d+1}
 =-\Bigl(-\frac1{2\pi\ImaginaryUnit}\Bigr)^{d+1}
 \frac{(d+1)!}{2^{d(d+1)}}
 \sum_{\ell\ \mathrm{odd}}
 \frac{\FourierTwoPi{u}(\AbsoluteValue\ell/2)}{\ell^{\,d+1}}\,
...
$D(s)=\sum_{q\ \mathrm{odd}}\FourierTwoPi{u}(q/2)\,q^{-s}$ of the companion
volume,
\begin{equation}
 \mathcal E_d(0)
 =(-1)^{(d-1)/2}\,
 \frac{2\,(d+1)!}{(2\pi)^{d+1}}\;D(d+1)\in\RationalNumbers,
 \label{eq:defect-value-D}
\end{equation}
rational because $D(2r)/\pi^{2r}\in\RationalNumbers$ is proved there.  The proved
...
$\sum_{q\ \mathrm{odd}}\FourierTwoPi{u}(q/2)\chi(q)q^{-(d+1)}$ over
...
\begin{proof}
By the refinement derivative,
$\tfrac{\Differential}{\Differential x}u\bigl(\tfrac{x-(2k+1)}2\bigr)
...
$\SupportOperator u_{\rho,c'}=[c'-\rho,c'+\rho]$.
\end{proof}
...
\Differential t$ over the full support once $x\ge c+h$; substitute $t=c+hz$.
\end{proof}
...
Let $a<b$, $L=b-a$, $h>0$, $K=\Ceiling{L/(2h)}$, and for $n\ge0$
\begin{equation}
 \boxed{\;
 \Psi_{n,h}(x)
 =n!\,2^{n(n+1)/2}
...
\begin{proof}
If $h\ge L/2$, then $0<L/(2h)\le1$, hence $K=1$.  The binary-partition
series has $p_{n+1}(0)=p_{n+1}(1)=1$, so
\cref{thm:finite-window} has precisely the displayed two summands.  Substituting
$h=L/2$ in its polynomial argument gives
$(x-a+h)/h=1+2(x-a)/L$.  Expanding $p$ in the resulting $d+1$
triangular blocks uses two atoms per block; for $n=0$ the formula is
exactly \eqref{eq:partition-interval}.
\end{proof}
...
and use \eqref{eq:finite-Psi-def}.  For the monomial form: with $c=a-h$,
...
On $[a,b]$,
$\Psi_{n,h}^{(k)}=\FallingFactorial{n}{k}\,h^{-k}\,\Psi_{n-k,h}$; the Wronskian is
constant,
...
$\PiD\to C_c^\infty(\RealNumbers)$ that restricts to the identity on
...
\begin{equation}
 \SupportOperator E_{d,a,b,h}\,p\subseteq
...
\begin{proof}
The coefficients in \cref{thm:coefficient-transform} depend linearly on
$p$, so $E_{d,a,b,h}$ is linear, and \cref{thm:finite-window} gives
$E_{d,a,b,h}p=p$ on $[a,b]$.  This restriction identity also proves
injectivity.  Every summand is a translate and dilation of the compactly
supported $C^\infty$ function $u$, hence the finite sum is compactly
supported, smooth, and flat at the two outer endpoints.  The leftmost
support edge occurs for $(n,N)=(0,0)$ and equals $a-2h$; the rightmost
occurs for $(n,N)=(d,K)$ and equals
$a+h(2^{d+2}-2+2K)$.  Finally $2hK<L+2h$ gives the strict second
inclusion in \eqref{eq:support}.
\end{proof}
...
$(d+1)\bigl(1+\Ceiling{2^{d+1}L/\rho}\bigr)$ atoms.  Conversely the
sparse $h=L/2$ choice has linear atom count $2d+2$ but an
exponentially long one-sided scale ladder,
$\SupportOperator\subseteq[a-L,\;a+2^{d+1}L]$.  A reflected anticausal train
...
(\cref{prob:conditioning-scale}).  The same warning applies to the
...
The third of the three exact-polynomial sources,
\nolinkurl{Rvachev_Up_Exact_Polynomial_Representation_Report}, detaches the
dictionary construction from
...
For $m\in\PositiveIntegers$, $t\in\RealNumbers$, $z\in\ComplexNumbers$, with
$\MM_u$ the moment generating function,
\begin{equation}
 \sum_{k\in\IntegerNumbers}\EulerE^{kz}\,u\!\left(\frac{t-k}{m}\right)
 =m\sum_{n\in\IntegerNumbers}\EulerE^{(z-2\pi\ImaginaryUnit n)t}\,
 \MM_u\!\bigl(-mz+2\pi\ImaginaryUnit mn\bigr),
...
$x\mapsto\EulerE^{zx}u((t-x)/m)$, whose $n$-th Fourier coefficient is
$m\,\EulerE^{(z-2\pi\ImaginaryUnit n)t}\MM_u(-mz+2\pi\ImaginaryUnit mn)$.
\end{proof}

At a nonzero frequency $n$, the alias factor vanishes at $z=0$ to
order $1+\TwoAdicValuation(mn)\ge\TwoAdicValuation(m)+1$.  Dividing
...
\end{equation}
one has, for every $m\in\PositiveIntegers$ and $0\le r\le\TwoAdicValuation(m)$,
\begin{equation}
 t^{\,r}=\sum_{k\in\IntegerNumbers}A^{(m)}_r(k)\,
 u\!\left(\frac{t-k}{m}\right)
 \qquad(t\in\RealNumbers),
 \label{eq:general-reproduction}
\end{equation}
locally finite, and the order $\TwoAdicValuation(m)$ is \emph{sharp}
...
$\Expectation\,q^{\sharp}(t+mZ)=q(t)$ --- the coefficients undo smoothing by a
random up-distributed displacement.

The first failure at degree $L=\TwoAdicValuation(m)+1$ generalizes the defect
series \eqref{eq:defect-series} beyond dyadic ratios: for
$m=2^{\nu}q$ with $q$ odd,
\begin{equation}
 \sum_{k\in\IntegerNumbers}A^{(m)}_L(k)\,u\!\left(\frac{t-k}{m}\right)-t^{L}
 =\frac{(-1)^{L+1}L!}{(2\pi\ImaginaryUnit)^{L}}
 \sum_{\substack{n\in\IntegerNumbers\\ n\ \mathrm{odd}}}
 \frac{\FourierTwoPi{u}(qn/2)}{n^{L}}\,\EulerE^{-2\pi\ImaginaryUnit nt},
 \label{eq:general-defect}
\end{equation}
nonzero because $\FourierTwoPi{u}(q/2)\ne0$: an odd factor $q$ in the ratio
replaces the half-integer values $\FourierTwoPi{u}(n/2)$ by
$\FourierTwoPi{u}(qn/2)$ --- the arithmetic face of the odd-overscaling
...
and any ratio $m$ with $\TwoAdicValuation(m)\ge d$ (canonically $m=2^{d}$), put
$\rho=mh$ and
...
\begin{equation}
 ...
 D^{\,d}\,U_{2^{d}h,\,c}(x)
 =h^{-d}\,2^{-\binom d2}
 \sum_{k=0}^{2^{d}-1}\ThueMorseSign{k}\,
 U_{h,\;c+(2k-(2^{d}-1))h}(x),
 \;}
 \label{eq:tm-stencil}
\end{equation}
with the signed Thue--Morse signs $\ThueMorseSign{k}$: each differentiation
...
$\prod_{j<d}(1-yq^{j})
=\sum_r(-1)^{r}q^{\binom r2}\GaussianBinomial{d}{r}{q}\,y^{r}$
...
reproduction degree $\min(N-1,\TwoAdicValuation(m))$, whose cumulants obey the
...
$2^{-s}\IntegerNumbers[1/O]$ ($O$ odd) the coefficients lie in
$2^{-[s(d+1)+d+\nu]}\IntegerNumbers[1/O']$, all further growth odd
...
Certificate / defect & one-scalar Thue--Morse test; explicit
 ...
 $G_n$)\\
Discrete engine & alias annihilator $A_d=\prod\QInteger{2^{m}}{z}$
...
(\cref{thm:scale-classification} at $\TwoAdicValuation(\rho)=0$).  Both
...
\input{chapters/Lagrange_Cardinal_Loops}
\input{chapters/Legendre_Spectral_Closure}
\input{chapters/Legendre_Transmutation_Arithmetic}
...
$1+\min\{\TwoAdicValuation(j),d\}$; the one-cell dimension is
...
On a box in $\RealNumbers^{s}$, both constructions tensorize: coordinatewise
...
of \cref{thm:alias-family} for $\IntegerNumbers^{s}$-cosets) is an attractive
frontier problem.
...
\subsection{Base \texorpdfstring{$b$}{b} and
\texorpdfstring{$q$}{q}-ary ladders}
Both engines are dyadic only through the zero set of
$\FourierTwoPi{u}$.  For an integer base $b\ge2$, the compactly supported
...
\begin{problem}[Make support--atom Pareto optimality precise]
\label{prob:pareto}
...
\emph{Problem:} specify the admissible causal construction class, the block
norm and condition-number bound, and then prove or refute this proposed lower
bound.
\end{problem}

\begin{problem}[Define and optimize the ladder scale]
\label{prob:conditioning-scale}
...
adjacent $K$-regimes.
\emph{Problem:} choose an explicit operator norm and target normalization,
define the treatment of the discontinuous truncation index $K$, and determine
the minimizers for that precise functional.
\end{problem}
...
$\TwoAdicValuation(m)\ge d$: long plateaux cannot beat the stationary binary
...
\begin{problem}[Specify stable radii, ghost limits, and local Chebyshev data]
\label{prob:lattice-analytic}
...
\cref{prob:conditioning-scale}).  The $\ell^{2}$-optimal
...
system.
\emph{Problem:} define the coefficient norm and critical-scale rule, the
scale-matching topology for ghost iterates, and the quantitative genericity
and interval-length hypotheses before treating these three proposals as
conjectures.
\end{problem}
...
saddle $z_d$ of $\Gd_d(u)=[z^{d}]d!\,\EulerE^{uz}/\MM_u(z)$ should obey
...
\GaussianBinomial{d}{r}{q}$, interacting at $q=2$ with the
...
\begin{statusbox}{Compiled Poisson and sharpness updates}
...
(quadrature) face
$\sum_{k}(\theta+k)^{p}\RvachevUp(2^{-m}(\theta+k))
=\int x^{p}\RvachevUp(2^{-m}x)\Differential x$ for every real shift and $p\le m$,
...
\emph{Update (30 Aug 2026):} \path{CompositeMeshSharpness.lean} closes
the natural-mesh universal exactness threshold itself: one definition and
seven theorems give the real first defect, the equivalent valuation and
divisibility classifications, canonical membership, the lower bound, and both
\texttt{IsLeast} forms.  This closure is for the complete polynomial space
through the cutoff only; target-specific synthesis, local dictionaries, and
atom counts remain outside it.
\end{statusbox}
...
\medskip
\noindent\textbf{Lagrange and Legendre source packages.}
The ten later reports contribute 176 labeled proved results.  Equivalent
polynomial-synthesis, right-inverse, projector, and finite-loop statements
are represented once by the stronger canonical forms in
\cref{sec:lagrange-loops,sec:legendre-closure}; the distinct operator,
arithmetic, asymptotic, parity, and conditioning results are retained in
\cref{sec:legendre-transmutation}.  Hashes below are SHA-256 over the
repository-normalized main source bytes at the consolidation baseline.  The
directory names are historical identities at pre-retirement commit
\nolinkurl{443793e846934e7363e314ea01129b9f50197a58}, not current navigation.

\begin{center}
\scriptsize
\begin{longtable}{@{}p{0.43\textwidth}p{0.52\textwidth}@{}}
\toprule
Historical source directory, main-source SHA-256 & Distinct contribution retained here\\
\midrule
\endfirsthead
\toprule
Historical source directory, main-source SHA-256 & Distinct contribution retained here\\
\midrule
\endhead
\nolinkurl{rvachev_lagrange_loop_report}\\
\nolinkurl{5d17373e36f6b12576a9add8c46fde57ae2e59b2b93410930fe64fb39f6dd647}
& Base cardinal factorization, exact interpolation loop, and nested
self-expansion.\\
\addlinespace
\nolinkurl{Lagrange_Rvachev_Loop_Package}\\
\nolinkurl{978aeadec8087c5c44881ccc22b87fa2c3c5579aa539a462621c5418d066e4fb}
& Minimal defect-completed inverse, nodal ghost, root-rate obstruction, and
endpoint amplitude barrier.\\
\addlinespace
\nolinkurl{lagrange_rvachev_loop_report_v3}\\
\nolinkurl{07587a69794e72f18af9075eae545f0cd5ae1694f318b961d287713a088a1465}
& Basis-independent matrix factorization, $q$-geometric row lift,
Legendre deconvolution, weighted kernel geometry, factorial asymptotic, and
Lambert threshold.\\
\addlinespace
\nolinkurl{Lagrange_Rvachev_Closed_Loop_Report}\\
\nolinkurl{727ccd7491844ffc4f2d0a82f351e052e83f50acbfcb76f7d12aad0059f95099}
& Appell--Vandermonde inversion, sample-to-atom volume bound, the
parity-adapted four-atom ladder, prime support, exterior-anchor instability,
and normal-convergence obstruction.\\
\addlinespace
\nolinkurl{Rvachev_Lagrange_Loop_Report_v5}\\
\nolinkurl{118a09539feafbccf24e353e6e6c0c2e647236fb77822eadba721adc4c38808d}
& Markov encoder/signed decoder, variation and Lebesgue lower bounds,
alias/sample exact sequence, and coefficient projector flag.\\
\addlinespace
\nolinkurl{Rvachev_Lagrange_Loop_Report_v6}\\
\nolinkurl{e11a2c460ae2b5f33b80217a681dfae874398b8f22c8edcd8d351b5eaeeac0b2}
& Undilated fixed-radius cardinal synthesis, sharp mesh interpretation, and
interlevel dyadic null rows.  The source arrived without its advertised
scripts or machine-readable data; that limitation is preserved.\\
\addlinespace
\nolinkurl{legendre_rvachev_closed_loop}\\
\nolinkurl{bebd04fe47a308328d717028876e78f93c89d6d33238e54690c16d32bfb6a2db}
& Translated analysis kernel, exact biorthogonality, reverse spectral closure,
flatness and derivative-cell Thue--Morse identities, log-square coefficient
bound, and two-adic moment units.\\
\addlinespace
\nolinkurl{Legendre_Rvachev_Closed_Loop_Report_v3}\\
\nolinkurl{8bbd922a51dbc6a132892c43c226d57fda220f0b41550c2d27cd90f97ed324d2}
& Pullback geometry and Favard obstruction, operator
Christoffel--Darboux identity, parity split, central $q$-determinant and Gram
basis, and the exact degree-twelve Sturm transition.\\
\addlinespace
\nolinkurl{Legendre_Rvachev_Closed_Loop_Report_v4}\\
\nolinkurl{7eff3989000e82f72f4b9e9761f75b807ec95bf369f46b67d96abce207594274}
& Moment-smoothed Legendre/Appell matrices and universal determinant,
$X_+$ transmutation, Legendre moments completed by a Thue--Morse row, and the
compressed $2N+2$-atom one-scale closure.\\
\addlinespace
\nolinkurl{Legendre_Rvachev_Self_Reconstruction}\\
\nolinkurl{23180ac9e6ed8bfc9c9d0e7e01d97890b2414768fef982d7a1563083fdba8376}
& Orthogonal self-reconstruction blocks and rank-one detail projectors,
rational and nested dyadic-Fabius square energies, refinement null trains,
exact lifting, derivative energies, and central non-flattening.\\
\bottomrule
\end{longtable}
\end{center}

The historical hashes and bytes of the three earlier absorbed \texttt{.tex}
sources remain recoverable from Git history.  After the ten later reports were
completely absorbed, their individual prose/PDF directories were retired.  The
113-row companion disposition records 106 retained payloads with live canonical
destinations under \nolinkurl{assets/companion-evidence/} and
\nolinkurl{assets/evidence/}; the other seven rows are retired checksum ledgers
with no live destinations.  Their hashes and mappings are recorded in
\nolinkurl{assets/provenance/COMPANION_PAYLOADS.csv}.  The ten hashes above,
historical source snapshots, companion-asset roles, and the exact
computer-assisted certificate boundary are mirrored in the canonical
provenance files under \nolinkurl{assets/provenance/}.  Historical arrival
ledgers are not rewritten to masquerade as live checksums.
\paragraph{Exact replay of the degree-twelve root transition.}
The compact certificate, the complete table through degree twenty, and the
focused exact verifier used in \cref{prop:leg2-Q12-nonreal} live under
\nolinkurl{assets/evidence/legendre/root-geometry/}.  The verifier reconstructs
the rational polynomials and their Sturm chains rather than trusting the text
summary; approximate root locations and plots are diagnostic only.

\emph{What was deduplicated}: the corpus background (normalization,
probability model, MGF/cumulants, Fourier zeros), the Appell
calculus (the third exact-polynomial source's up-Appell family
$A^{(m)}_r(u)=m^{r-1}\Gd_r(u/m)$ is the same deconvolution family in
a scaled normalization --- verified on $P_2,P_4$), the Thue--Morse
products, the sharp-order statement (that source's exact
stationary order $\TwoAdicValuation(m)$ is
\cref{thm:scale-classification}), rationality and affine covariance,
tensor products (that source adds per-coordinate ratios),
...
companion volume's independently computed phase-error table.  Equivalent
source theorems are represented once by their strongest canonical form;
distinct proved results are retained, while genuinely open assertions are
separated into explicitly labeled conjectures and problems.  The first two
exact-polynomial sources audited the repository corpus (67-path inventory) and
agreed that the explicit finite constructions were absent from it; that
corpus-relative novelty standard is inherited here without being promoted to
a claim about the later ten packages.
...
V.~Reshetnikov,
\newblock \emph{Inverse Fabius Theory: Analyticity, Asymptotics,
Computability, and Dyadic Sampling},
\newblock canonical frontier synthesis,
\nolinkurl{drafts/inverse-and-sampling/Inverse_Fabius_Analyticity_Asymptotics_and_Computability/},
2026.
