...
$F(t):=u(1-t)$ for $0\le t\le1$; hence
$u(x)=F(1-|x|)$ on $[-1,1]$.  Where an endpoint estimate evaluates
$F(t)$ for $t\ge1$, we use the explicit constant-extension convention
$F(t)=1$.  The
...
of unity iff $\rho\in\N_{>0}$; for positive integer $\rho$ the Strang--Fix
...
of degree at most $d$ is equivalent to $d\le\vtwo(M)$, equivalently
$2^d\mid M$; the monomial of degree $\vtwo(M)+1$ fails at some shift; and
...
\[
 \frac1M\sum_{k\in\Z}Q(k)u\!\left(\frac{t-k}{M}\right)
 =\MM_u(MD)Q(t)=P(t).
\]
...
\end{equation}
and these
$N_d^{\rm rel}:=\sum_{j=1}^{M-1}(\vtwo(j)+1)=2M-d-2$ relations are
