...
of unity iff $\rho\in\NaturalNumbers$; for integer $\rho$ the Strang--Fix
...
\begin{equation}
 ...
 \label{eq:alias-zero}
\end{equation}
and these $R_d:=\sum_{j=1}^{M-1}(\TwoAdicValuation(j)+1)=2M-d-2$ relations are
...
\[
 g_r=A_d(1)\Bigl[
 \Expectation\,\EulerE^{(L-R_d/2)D}\;\MM_u(MD)^{-1}P
 \Bigr]\!\left(r-\tfrac d2\right),
\]
and the two product formulas
\begin{gather*}
 \Expectation\,\EulerE^{z(L-R_d/2)}
