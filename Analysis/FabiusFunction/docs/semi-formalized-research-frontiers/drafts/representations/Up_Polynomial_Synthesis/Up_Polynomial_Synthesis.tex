...
$u=\RvachevUp$ as in the corpus; $\widehat f(\xi)=\int f\,\EulerE^{-2\pi\ImaginaryUnit\xi x}\Differential x$;
$\widehat u(\xi)=\prod_{j\ge0}\SincPi(\xi/2^{j})$ with
$\ord_{\xi=n}\widehat u=1+\TwoAdicValuation(n)$ at nonzero integers $n$.  The
...
$\eps_r=(-1)^{\BinaryDigitSum(r)}$, the finite product is
...
\begin{equation}
 ...
 \qquad
 \widehat{\psi_{\rho,k}}(\xi)=\EulerE^{-2\pi\ImaginaryUnit k\xi}\,\widehat u(\rho\xi),
 \label{eq:psi-def}
\end{equation}
...
$\ord=1+\TwoAdicValuation(Mn)=d+1+\TwoAdicValuation(n)>d$, so every alias vanishes.  Evenness
...
\begin{equation}
 ...
 \frac{\widehat u(n/2)}{n^{d+1}}\,\EulerE^{2\pi\ImaginaryUnit nt},
 \;}
 \qquad
...
\[
 ...
 \frac{(d+1)!}{2^{d(d+1)}}
 \sum_{\ell\ \mathrm{odd}}
 \frac{\widehat u(\AbsoluteValue\ell/2)}{\ell^{\,d+1}}\,
...
\begin{equation}
 ...
 \sum_{\substack{n\in\IntegerNumbers\\ n\ \mathrm{odd}}}
 \frac{\widehat u(qn/2)}{n^{L}}\,\EulerE^{-2\pi\ImaginaryUnit nt},
