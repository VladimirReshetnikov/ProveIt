...
\begin{equation}
 \langle f,g\rangle_-=
 \int_{-1}^{1}(\MM_u(D)f)(x)(\MM_u(D)g)(x)\dd x,
 \qquad
 \langle f,g\rangle_+=
 \int_{-1}^{1}(\mathscr D_uf)(x)(\mathscr D_ug)(x)\dd x.
...
that
\[
 \langle f,g\rangle_-=\int f(x)g(x)\dd\rho(x)
\]
...
\[
 \langle x,x\rangle_-=\int_{-1}^{1}x^2\dd x=\frac23,
 \qquad
 \langle1,x^2\rangle_-=
 \int_{-1}^{1}\left(x^2+\frac19\right)\dd x=\frac89.
\]
...
\end{equation}
Let $m\in\N_{>0}$ satisfy $\vtwo(m)\ge d$, put
$h=m^{-1}$ and $K_m=\{k\in\Z:|k|<2m\}$, and define
$\mathbf D_{kn}=hQ_n^-(kh)$ for $k\in K_m$, $0\le n\le d$,
$V_{jn}=P_n(\xi_j)$ for $0\le j,n\le d$,
$W=\operatorname{diag}(\omega_0,\ldots,\omega_d)$, and
$H=\operatorname{diag}(h_0,\ldots,h_d)$.
...
\begin{equation}
 (\mathsf L_d)_{n,r}=
 \frac{2n+1}{2}\int_{-1}^{1}\Phi_{d,r}(x)P_n(x)\dd x
 \quad(0\le n\le d),
 \qquad
...
of the frequencies and evenness of $\widehat u$ leave the factor
$(-1)^{d+1}$.  Moreover,
\[
...
$V_d=\Pi_d\oplus\Span\{\mathcal D_d\}$ makes $V_d/\Pi_d$
...
\begin{equation}
 A_d(z)=\prod_{m=1}^{d}[2^m]_z,\qquad
...
monic over $\Z$.  Thus $\sigma_N\in\Z$; as $C_N$ is even, the final factor is
...
\[
 \mathcal G_N=\int_{-1}^{1}\mathbf S_N\mathbf S_N^T\dd x,
 \qquad
 H_N^+=\operatorname{diag}(h_0,h_2,\ldots,h_{2N}).
\]
Then
\begin{equation}
...
There exist constants $C>0$ and $n_0\in\N$ such that, for every $n\ge n_0$,
\begin{equation}
 \boxed{|a_n|\le
...
\[
 \int_X^\infty
 \exp\!\left(-\frac{(\log x)^2}{2a}\right)x^{-\kappa}\dd x
 \le
 \frac{aX^{1-\kappa}}
 {\log X+(\kappa-1)a}
...
For every $m\ge0$,
\begin{equation}
 \boxed{\vtwo(\mu_{2m})=0,\qquad \mu_{2m}\equiv1\pmod2}
...
contributes $o(N^2)$ along the limsup.  Stirling shows that the factorial and
...
$G_n(z+2)=\E(z+2-Z)^n$ and $|Z|\le1$.  Choose $\varepsilon>0$ with
...
$w=W(H/(\pi\e))$.  Then $n_0=H/(2w)$ solves
$2n_0\log(2n_0/(\pi\e))=H$.  The logarithm of the prefactor in
\eqref{eq:leg2-Q0-root} changes the continuous solution by $o(1)$ because the
...
\begin{proof}
The retained exact verifier constructs every $Q_n^-$ over $\Q$ from the
...
\[
 ...
 \log\!\left(\frac{(-1)^N\sigma_N}{2^{2N-3}}\right)=o(N),
\]
...
$\R$.  Along the nonzero coefficient indices,
\[
 \lim_{\substack{n\to\infty\\a_n\ne0}}
 \frac{\|B_n\|_{L^2(\R\setminus[-1,1])}}
 {\|B_n\|_{L^2[-1,1]}}=\infty.
\]
Also the exact square energy
$\mathscr E_F=\int_0^1F(x)^2\dd x$ is irrational.
\end{conjecture}
