...
% LOCAL-NON-FOURIER-HAT: \MonicDeconvolvedLegendre denotes the monic normalization of the deconvolved Legendre polynomial
\newcommand{\MonicDeconvolvedLegendre}[1]{\widehat Q_{#1}^{-}}
% LOCAL-NON-FOURIER-HAT: \GenericMonicOrthogonalPolynomial denotes the generic monic orthogonal polynomial used in the Favard recurrence
\newcommand{\GenericMonicOrthogonalPolynomial}[1]{\widehat Q_{#1}}
...
\]
for all polynomials.  Moreover, if $\MonicDeconvolvedLegendre{n}$ denotes the monic
normalization, then
\begin{align}
 \MonicDeconvolvedLegendre{2}&=x^2-\frac49,&
 \MonicDeconvolvedLegendre{3}&=x^3-\frac{14}{15}x,\notag\\
 \MonicDeconvolvedLegendre{4}&=x^4-\frac{32}{21}x^2+\frac{1072}{4725},
 \label{eq:leg2-low-monic-Q}
\end{align}
and
\begin{equation}
 \boxed{\MonicDeconvolvedLegendre{4}
 =x\MonicDeconvolvedLegendre{3}-\frac{62}{105}\MonicDeconvolvedLegendre{2}
 -\frac8{225}\MonicDeconvolvedLegendre{0}.}
...
$\GenericMonicOrthogonalPolynomial{n+1}=x\GenericMonicOrthogonalPolynomial{n}-\lambda_n\GenericMonicOrthogonalPolynomial{n-1}$; the nonzero
$\GenericMonicOrthogonalPolynomial{0}$ term is impossible.
\end{proof}
...
In the executable rational convention of \path{FabiusFunction.TwoAdic},
\path{moment m} is precisely the manuscript moment $\mu_{2m}$.
The theorem \path{moment_padicVal_two} proves
$\TwoAdicValuation(\mu_{2m})=0$ for every $m\ge0$, while
\path{moment_num_den_odd} proves that the reduced numerator and denominator
of $\mu_{2m}$ are both odd.  Reduction of an odd-over-odd rational unit modulo
two is one, so the latter is exactly the second clause of
\eqref{eq:leg2-moment-units}; $m=0$ is included.  Thus
\cref{thm:leg2-moment-units} is Exact/Complete by these two existing
declarations.  The encoding uses \path{padicValRat} and reduced rational
parity; it does not claim that Lean has constructed a separate topological
$\RationalNumbers_2$ representative, and it changes no neighboring asymptotic,
spectral, or matrix status.
\end{statusbox}
...
expansion in the canonical inverse-Fabius synthesis,
\path{chapters/03_inverse_germs_and_deconvolution.tex}.  Its canonical theorem
and equation labels are, respectively,
\path{is:p2:thm:three-shell} and \path{is:p2:eq:alpha-three-shell}.
