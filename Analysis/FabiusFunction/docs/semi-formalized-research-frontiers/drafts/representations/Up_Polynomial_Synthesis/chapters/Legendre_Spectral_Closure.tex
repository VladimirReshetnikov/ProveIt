...
\begin{equation}
 ...
 \IndicatorOf{I(k/M)}(x)P_{2r}(x-k/M).
 \label{eq:leg-H}
\end{equation}
For $m,r\ge0$, define
\begin{equation}
...
\[
 ...
 H=\DiagonalOperator(h_0,h_2,\ldots,h_{2N}).
\]
...
\[
 ...
 \SpanOperator\{P_0,P_2,\ldots,P_{2N}\}
\]
...
For $p\in\Pi_d$ put $p^\sharp=\mathscr D_{\RvachevUp}p$.  At a mesh
...
\begin{equation}
 \boxed{
 \frac1{rm}\sum_{|k|<2rm}
 \bigl(1-r\IndicatorOf{r\mid k}\bigr)
...
\begin{equation}
 \boxed{
 a_k^{(N+1)}-(E_4a^{(N)})_k
 =\frac{1-4\IndicatorOf{4\mid k}}{4m}C_N^-(k/(4m))
...
\]
whereas $(E_4a^{(N)})_k=
4\IndicatorOf{4\mid k}C_N^-(t)/(4m)$.  Subtract.  Apply
...
\begin{equation}
 ...
 (-1)^{\BinaryDigitSum(j_{k,s})}u(y_{k,s})
 =
 \begin{cases}
 \dfrac{(\ell+s)!}{2^ss!(\ell-s)!},&s\le\ell,\\[0.7em]
 0,&s>\ell.
...
\[
 ...
 =2^{\binom{s+1}{2}}(-1)^{\BinaryDigitSum(j)}u(y),
...
\begin{proof}
Evaluate \eqref{eq:leg-mode-synthesis} for $P_{2n}$ at $x=0$ and
$M=4^n$.  Compact support removes the terms with
$M\le \lvert k\rvert<2M$ (including the zero boundary values at
$\lvert k\rvert=M$), leaving the open integer
interval $-M<k<M$.  Both $Q^-_{2n}$ and $u$ are even, so the nonzero terms
pair under $k\leftrightarrow-k$; the unpaired term is
$Q^-_{2n}(0)u(0)=Q^-_{2n}(0)$.  Multiply by $M=4^n$ and use
$P_{2n}(0)=(-1)^n4^{-n}\binom{2n}{n}$.  When $n=0$, $M=1$ and the
positive-index sum is empty, so the same calculation applies.
\end{proof}

\begin{statusbox}{Lean status: Exact/Complete}
The source module \path{FabiusFunction.RvachevLegendreCentralSum} has no public
definitions and exactly three public theorems.  The supporting declarations
\path{eval_legendrePolynomial_even_zero} and
\path{eval_rvachevLegendreDeconvolutionPolynomial_even} prove the two central
normalizations used above; \path{rvachevLegendreCentralSum} is
\eqref{eq:leg-central-sum} itself, with the printed $M=4^n$, the same open
lattice sum, and arbitrary \path{BoundedFabius} data satisfying
\path{IsFabius}.  The proof composes the previously formalized even-mode
synthesis with compact support and parity, and the principal declaration is
exactly the displayed conclusion.  This status does not promote the
compound \cref{thm:leg-mode-synthesis}: its Jacobi closed form, all-degree
parity, and rationality clauses remain outside this module.  Reverse spectral
closure, target-specific mesh minimality, and \cref{thm:lag-right-inverse}
...
$\SpanOperator\{P_{2n}\}$, and it has the finite atomized form
\begin{equation}
 \boxed{
...
$\|u\|_2^2=\int_\RealNumbers|\FourierTwoPi{u}(\xi)|^2\Differential\xi$.  Evenness, division by two,
...
\begin{align}
 ...
 \int_\RealNumbers\FourierTwoPi{u}(\xi)j_m(2\pi\xi)
...
\begin{proof}
Let $\chi_m(x)=P_m(x)\IndicatorOf{[-1,1]}(x)$.  Evenness of $u$ gives
...
$\FourierTwoPi{\chi_m}(\xi)=2(-\ImaginaryUnit)^mj_m(2\pi\xi)$.  The rapid decay of
$\FourierTwoPi{u}$ makes the product integrable, so Fourier inversion of the
...
\path{FabiusFunction.RvachevLegendreBiorthogonality} has exactly one public
definition and one public theorem.  The definition
\path{rvachevLegendreAnalysisKernel} is \eqref{eq:leg-Lambda}, with the same
factor $(2m+1)/2$ and interval $[-1,1]$.  The theorem
\path{rvachevLegendreBiorthogonality} is \eqref{eq:leg-biorthogonality}:
\path{M != 0} is the positive-natural-mesh condition,
\path{l <= padicValNat 2 M} is $\TwoAdicValuation(M)\ge\ell$,
\path{Finset.Ioo (-(2*M)) (2*M)} is exactly $\lvert k\rvert<2M$, and Lean
retains the factors $M^{-1}$ and $(2m+1)/2$ and the value
\path{if m = l then 1 else 0}.  It includes both zero-degree boundary cases
and harmlessly quantifies over every bounded Fabius realization satisfying
the canonical axioms.  This promotion is confined to
\cref{thm:leg-biorthogonality}: it does not prove the remaining clauses of
\cref{thm:leg-Lambda}, the matrix/projector/trace/Cauchy--Binet conclusions of
\cref{cor:leg-biorthogonal-matrices}, reverse closure, the full compound mode
theorem, or any Lagrange right-inverse clause.
\end{statusbox}
