...
\begin{equation}
 h_n=\int_{-1}^{1}P_n(x)^2\dd x=\frac2{2n+1}
 \label{eq:leg-hn}
\end{equation}
and define the even Fourier--Legendre coefficients
\begin{equation}
 a_n:=\frac{4n+1}{2}\int_{-1}^{1}u(x)P_{2n}(x)\dd x.
 \label{eq:leg-an}
\end{equation}
The primary exposition's section on the Fourier--Legendre expansion of
$\Up$ in \path{Fabius_Function_and_Rvachev_Up.tex} proves
\begin{equation}
...
Let $\ell\ge0$ and let $M\in\N_{>0}$ satisfy
$\vtwo(M)\ge\ell$.  Then, for $x\in[-1,1]$,
\begin{equation}
 \boxed{
...
For $\ell,r\ge0$ and $M\in\N_{>0}$ with $\vtwo(M)\ge\ell$, define
\begin{equation}
 H_{\ell r}^{(M)}(x)
...
Let $\ell\ge0$ and $M\in\N_{>0}$ satisfy $\vtwo(M)\ge\ell$.  Then
\begin{equation}
...
and define
\[
 G_{k\ell}=\int_{-1}^{1}\phi_k(x)\phi_\ell(x)\dd x,\qquad
...
$m\in\N_{>0}$ with $\vtwo(m)\ge d$, define the canonical finite coefficient
row
\[
...
Let $d\ge0$, $p\in\Pi_d$, and $r,s,m\in\N_{>0}$ with
$\vtwo(m)\ge d$.  Then, on $[-1,1]$,
\begin{equation}
 \boxed{
 \frac1{rm}\sum_{|k|<2rm}
...
For every $\ell\ge0$, every $M\in\N_{>0}$ with $\vtwo(M)\ge\ell$, and
every $s\ge0$,
\begin{equation}
 \boxed{
...
Let
\begin{equation}
 \mathscr E_F:=\int_0^1F(t)^2\dd t.
 \label{eq:leg-A2}
\end{equation}
Because $u(x)=F(1-|x|)$ on $[-1,1]$, $\|u\|_2^2=2\mathscr E_F$.
...
$\|u\|_2^2=2\mathscr E_F$.  Rationality follows from $a_n\in\Q$.
Parseval for the Fourier convention of \cref{sec:shared} gives
$\|u\|_2^2=\int_\R|\widehat u(\xi)|^2\dd\xi$.  Evenness, division by two,
...
derivation.
Put $\mu_{2j}=\int_{-1}^{1}x^{2j}u(x)\dd x$.  Expanding $P_{2n}$ in
...
For every $m\ge0$, the function $\Lambda_m$ belongs to $C_c^\infty(\R)$,
...
$\widehat\chi_m(\xi)=2(-\ii)^mj_m(2\pi\xi)$.  The rapid decay of
...
If $a,b\in\Z[1/2]$, $a\le b$, and $r\ge0$, then
\begin{equation}
 \int_a^b x^ru(x)\dd x\in\Q.
 \label{eq:leg-dyadic-truncated}
\end{equation}
Consequently $\Lambda_m(c)\in\Q$ for every dyadic $c$.
\end{lemma}
...
Let $\ell\ge0$, $M\in\N_{>0}$ with $\vtwo(M)\ge\ell$, and $m\ge0$.
Then
\begin{equation}
 \boxed{
 \frac1M\sum_{|k|<2M}
...
Let $d\ge0$ and $M\in\N_{>0}$ satisfy $\vtwo(M)\ge d$.  Then the matrices
indexed by $K_M:=\{k\in\Z:|k|<2M\}$,
\[
...
\begin{proof}
Because $\ell\le d$, the hypothesis $\vtwo(M)\ge d$ permits
