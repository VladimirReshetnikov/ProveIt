...
\[
 ...
 m\in\N_{>0},\qquad h=m^{-1},\qquad
 K_m:=\{k\in\Z:|k|<2m\}.
\]
The mesh is called \emph{admissible through degree $d$} when
$\vtwo(m)\ge d$.  The canonical choice is $m=2^d$.  Notice that
...
$\rho=mh=1$.  It applies because $\deg\ell_j=d\le\vtwo(m)$ and gives
...
For every $0\le j\le d$ and every $t\in\R$,
\begin{align}
 (\mathscr D_u\ell_j)(t)
...
\end{align}
Thus rational nodes give rational amplitudes.  More explicitly, the
$\gamma_{2r}$ are complete Bell polynomials in the negated Bernoulli
cumulants of \eqref{eq:appell-pair}, so no implicit linear solve is hidden in
\eqref{eq:lag-cardinal}.
\end{corollary}
...
\begin{equation}
 ...
 \label{eq:lag-UB}
\end{equation}
Thus $U\in\R^{(d+1)\times q_m}$ samples the atoms and
$B\in\R^{q_m\times(d+1)}$ synthesizes the cardinals.
For a coefficient row $c\in\R^{K_m}$ and a continuous function $f$, write
\[
 \operatorname{Synth}c:=\sum_{k\in K_m}c_k\,u(\,\cdot-kh),
 \qquad
 I_Xf:=\sum_{j=0}^{d}f(x_j)\ell_j.
\]
Thus $\operatorname{Synth}\colon\R^{K_m}\to C[-1,1]$ and
...
and
\[
 \operatorname{rank}\mathcal P_X=\operatorname{tr}\mathcal P_X=d+1.
\]
...
\begin{equation}
 V_{in}=p_n(x_i),
 \qquad
 \mathbf D_{kn}=h(\mathscr D_up_n)(kh).
 \label{eq:lag-UVD}
\end{equation}
Then
\begin{equation}
...
\[
 ...
 \operatorname{TV}(a,b)=\tfrac12\|a-b\|_1.
\]
...
\begin{equation}
 ...
 =\frac1{\operatorname{TV}(A_{i,:},A_{j,:})}.}
...
\[
 \mathcal V_d^{\rm lit}
 :=\Span\{u(\,\cdot-k/M)|_{[-1,1]}:k\in K_M\}.
\]
...
\begin{align}
 ...
 &\mathcal V_d^{\rm lit}\cap\R[x]&=\PiD,
...
\[
 \widehat U_{ir}=\Phi_r(x_i)
\]
and let $\widehat B$ contain the unique endpoint-basis coordinates of the
cardinals.  Then $\widehat U\widehat B=I_{d+1}$.

For $0\le r\le d+1$, delete column $r$ from $\widehat U$ and row $r$ from
$\widehat B$, and set
\begin{equation}
 v_r=(-1)^r\det\widehat U_{\widehat r},
 \qquad
 \zeta_r=(-1)^r\det\widehat B_{\widehat r}.
...
\begin{equation}
 \widehat Uv=0,
 \qquad \zeta^T\widehat B=0,
 \qquad \zeta^Tv=1,
 \label{eq:lag-cofactor-null}
\end{equation}
and
\begin{equation}
 \boxed{I_{d+2}-\widehat B\widehat U=v\zeta^T.}
...
matrix, hence $\widehat Uv=0$; applied to $\widehat B^T$ they give
$\zeta^T\widehat B=0$.  Cauchy--Binet and
$\widehat U\widehat B=I$ yield
\[
 1=\sum_r\det\widehat U_{\widehat r}
       \det\widehat B_{\widehat r}=\zeta^Tv.
\]
The complementary idempotent $I-\widehat B\widehat U$ has rank one, range
$\ker\widehat U=\Span\{v\}$, and left range
$\ker\widehat B^T=\Span\{\zeta\}$.  The normalization
$\zeta^Tv=1$ proves \eqref{eq:lag-rank-one}.

The endpoint coefficient vector of $I_XF_a$ is
$\widehat B\widehat Ua$.  Subtracting it from $a$ and applying
...
\begin{equation}
 \mathcal M_X=
 \begin{pmatrix}\widehat U\\ \zeta^T\end{pmatrix}
 \in\R^{(d+2)\times(d+2)}
 \label{eq:lag-square-completion}
\end{equation}
is invertible and
\begin{equation}
 \boxed{\widehat B=\mathcal M_X^{-1}
...
at most $d$.  The equation $\widehat Ua=0$ says that this polynomial has the
...
$\widehat U\widehat B=I$ and $\zeta^T\widehat B=0$ are exactly
\[
 \mathcal M_X\widehat B=\begin{pmatrix}I\\0\end{pmatrix},
\]
...
\begin{equation}
 \widehat{\mathcal P}_q^{(s)}:=E_sI_qT_s
 \label{eq:lag-lifted-projector}
\end{equation}
satisfy the same multiplication law and
$\operatorname{rank}\widehat{\mathcal P}_q^{(s)}=d_q+1$.  Hence the details
\begin{equation}
 \widehat\Delta_0^{(s)}=\widehat{\mathcal P}_0^{(s)},
 \qquad
 \widehat\Delta_q^{(s)}=
 \widehat{\mathcal P}_q^{(s)}-\widehat{\mathcal P}_{q-1}^{(s)}
 \quad(q\ge1)
...
\[
 \widehat{\mathcal P}_q^{(s)}\widehat{\mathcal P}_r^{(s)}
...
$m\in\N_{>0}$ be an admissible integer mesh.
...
annihilator $\prod_{r=1}^{d}[2^r]_z$ controls compression of the resulting
...
\[
 \mathsf T=
 \operatorname{diag}(D_0,\ldots,D_d)\mathsf A^{-1}
\]
therefore satisfies
\begin{equation}
 \boxed{\det\mathsf T=
...
Using the constant-extension convention for $F$ from the parent notation, put
\[
...
$0\le s\le\delta_d\le1$, so monotonicity and $u(-1+s)=F(s)$ give
\[
 0\le
 u\!\left(\frac{x-c_{d,r}}{2^{d+1}}\right)
 \le F(\delta_d)
...
$0\le u\le1=F(\delta_d)$, because $\delta_d\ge1$ under the stated constant
extension.  Hence in every degree
$|P(x)|\le F(\delta_d)\|a\|_1$; taking the supremum proves
\eqref{eq:lag-endpoint-amplitude}.  The established endpoint law
\[
 -\log F(2^{-t})=\frac{\log2}{2}t^2+o(t^2)
 \qquad(t\to\infty)
\]
...
adds only $d\log2=o(d^2)$ to the logarithm.
\end{proof}
...
\begin{equation}
 ...
 \cup
 \bigcup_{1\le r\le\lfloor d/2\rfloor}
...
evaluation at $kh\in\Z[1/2]$ introduce only the already listed prime $2$.
