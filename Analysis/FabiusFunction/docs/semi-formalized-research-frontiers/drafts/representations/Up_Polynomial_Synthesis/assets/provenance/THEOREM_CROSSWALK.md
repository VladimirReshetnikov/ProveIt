...
| theorem `thm:leg-biorthogonality` — Legendre--up dyadic biorthogonality | G1 `thm:biorthogonality` | **Exact/Complete Lean counterpart.** Direct finite analysis/synthesis biorthogonality identity. Lean's `M ≠ 0` is the positive natural mesh, `l ≤ padicValNat 2 M` is the printed two-adic admissibility, `Finset.Ioo (-(2*M)) (2*M)` is exactly `|k| < 2M`, and the factors `M⁻¹`, `(2m+1)/2`, and the Kronecker value are unchanged. Degree zero is included, and quantification over any `BoundedFabius` satisfying `IsFabius` is harmlessly stronger. | `FabiusFunction.RvachevLegendreBiorthogonality` has exactly one definition, `Fabius.rvachevLegendreAnalysisKernel`, and one theorem, `Fabius.rvachevLegendreBiorthogonality`. This promotion does not include the support, smoothness, Fourier--Bessel, or rationality clauses of `thm:leg-Lambda`; the matrix/projector/trace/Cauchy--Binet conclusions of `cor:leg-biorthogonal-matrices`; reverse closure; the complete `thm:leg-mode-synthesis`; or `thm:lag-right-inverse`. |
...
| theorem `thm:leg2-moment-units` — Every even up-moment is a two-adic unit | G1 `thm:moment-units` | **Exact/Complete Lean counterpart by assembly.** The executable sequence `moment m : ℚ` is the printed even moment `μ_(2m)`. `moment_padicVal_two` proves valuation zero, while `moment_num_den_odd` proves odd reduced numerator and denominator, equivalently residue one modulo two for a rational two-adic unit. Both include `m = 0`. | Existing declarations `Fabius.moment_padicVal_two` and `Fabius.moment_num_den_odd` in `FabiusFunction.TwoAdic`. This uses the repository's rational `padicValRat` and reduced-fraction parity convention; it does not construct a separate topological `ℚ₂` object or promote any neighboring Legendre asymptotic, spectral, or matrix row. |
