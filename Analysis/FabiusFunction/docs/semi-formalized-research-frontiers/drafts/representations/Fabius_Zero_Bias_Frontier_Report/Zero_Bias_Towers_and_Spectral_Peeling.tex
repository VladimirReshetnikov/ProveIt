...
\author{Frontier research report for Vladimir Reshetnikov's \repo\ corpus}
...
\]
so that $Y$ has Rvachev density $\Up$, while $X=(Y+1)/2$ has the Fabius
...
asymptotics assembled from the repository's imported theorems.
\end{statusbox}
...
$[-1,1]$.  Its density is Rvachev's even up-function $\Up$.  The affine variable
\begin{equation}
 X=\frac{Y+1}{2}
...
\begin{equation}
 \Up(x)=F(1-x).                                      \label{eq:up-F}
\end{equation}
Write
\begin{equation}
 \mu_n=\E|Y|^n\quad(n\ge0),
 \qquad \mu_{2n}=\E Y^{2n}.                         \label{eq:mu-def}
\end{equation}
In particular,
\begin{equation}
 \mu_0=1,
...
\end{align}
where $\sinc z=\sin z/z$ with its removable value at the origin.  The even
cumulants are
\begin{equation}
 \kappa_{2r}
...
\begin{equation}
 \sigma_q^2=\Var(Y_q)=\frac{1-q}{3(1+q)}.            \label{eq:q-variance}
\end{equation}
...
\(
 \widehat{\Up}(z)=\int\Up(x)\e^{-2\pi\ii zx}\dd x
\), define
\begin{equation}
 H(z)=\widehat{\Up}\!\left(\frac{\ii\sqrt z}{\pi}\right)
 =\prod_{j=0}^{\infty}
...
\begin{equation}
 r(N)=\vtwo(N)+1.                                   \label{eq:rN}
\end{equation}
Here $\vtwo$ is the $2$-adic valuation.  Relations
\begin{equation}
 M(t)=H(t^2/4),
...
\begin{equation}
 \E[Wf(W)]=\sigma^2\E[f'(W^z)]                     \label{eq:zero-bias-def}
\end{equation}
...
\begin{equation}
 \Prob(J=j)=(1-q^2)q^{2j},\qquad j=0,1,2,\ldots.     \label{eq:J-law}
\end{equation}
...
\label{cor:q-zero-cf}
With $\phi_q(t)=\prod_{j\ge0}\sinc(c_jt)$,
\begin{equation}
 \phi_q^z(t)
 =\sum_{j\ge0}(1-q^2)q^{2j}\,
   \beta_1(c_jt)\prod_{\ell\ne j}\sinc(c_\ell t).   \label{eq:q-zero-cf-sum}
\end{equation}
...
For $q=1/2$, let $p_0=\Up$ and let $p_1$ be the density of $Y^z$.  Since
$\mu_2=1/9$,
\begin{equation}
 p_1(x)=9\int_{|x|}^{1}y\Up(y)\dd y,
 \qquad p_1'(x)=-9x\Up(x)\quad(-1<x<1).              \label{eq:first-density}
\end{equation}
The exact center value follows without numerical integration.  Indeed,
$\Var(X)=\mu_2/4=1/36$ and $\E X=1/2$, hence
$\E X^2=5/18$.  The moment bridge proved later in
\Cref{lem:moment-bridge} gives $\E|Y|=5/18$, and therefore
\begin{equation}
 p_1(0)=\frac{\E|Y|}{2\mu_2}=\frac54.               \label{eq:p1-center}
\end{equation}
...
Let $W$ be symmetric and let $m_r=\E|W|^r$.  Then, for $m,r\ge0$,
\begin{equation}
 \E\bigl[(W^{\langle m\rangle})^{2r}\bigr]
 =\frac{(2m-1)!!(2r-1)!!}{(2m+2r-1)!!}
  \frac{m_{2m+2r}}{m_{2m}},                         \label{eq:tower-moment-formula}
\end{equation}
with $(-1)!!=1$.  In particular,
\begin{equation}
 \Var(W^{\langle m\rangle})
...
\[
 \E[(W^{\langle m+1\rangle})^{2r}]
 =\frac{\E[(W^{\langle m\rangle})^{2r+2}]}
        {(2r+1)\Var(W^{\langle m\rangle})}.
\]
...
\begin{equation}
 \E g(R_m)
 =\frac{\E\bigl[|W|^{2m}g(|W|)\bigr]}{m_{2m}}.       \label{eq:power-radius}
\end{equation}
...
coordinate of a uniform point on the sphere $S^{2m}\subset\R^{2m+1}$.
...
\begin{equation}
 ...
 \int_{|x|}^{1}y(y^2-x^2)^{m-1}p(y)\dd y,
 \qquad
...
\end{equation}
Consequently, with $\mathscr R=x^{-1}\frac{\dd}{\dd x}$ interpreted on even
...
\begin{equation}
 ...
 =\E\mathcal J_m(tR_m)
 =\frac{1}{\mu_{2m}}
   \E\left[|Y|^{2m}\mathcal J_m(t|Y|)\right].       \label{eq:Bessel-mixture}
\end{equation}
...
Let
\begin{equation}
 \Dop_t=\frac1t\frac{\dd}{\dd t}                  \label{eq:radial-D}
\end{equation}
...
\begin{proof}
Taking $f(x)=\e^{tx}/t$ in the defining zero-bias identity gives
$M_{W^z}(t)=M_W'(t)/(\Var(W)t)$.  At level $m$, use
...
\begin{equation}
 H_m(z)=\sum_{r=0}^{\infty}
        \frac{4^r\E[(\Tower{m})^{2r}]}{(2r)!}z^r.    \label{eq:Hm-coeff}
\end{equation}
...
$t_N=2\pi N$.  Its multiplicity is $r(N)=\vtwo(N)+1$.  Differentiation removes
...
\begin{equation}
 A_u=\prod_{\ell=1}^{\infty}
     \sinc\!\left(\frac{\pi u}{2^\ell}\right)
     =\widehat{\Up}(u/2).                           \label{eq:Au}
\end{equation}
The sign law already present in the repository gives
\begin{equation}
 \sgn(A_u)=(-1)^{\wt((u-1)/2)},                    \label{eq:Au-sign}
\end{equation}
where $\wt(n)$ is the binary digit sum.
...
\begin{equation}
 \sgn\phi_r(2\pi N)
 =(-1)^{r+1+\wt((u-1)/2)}.                          \label{eq:released-sign}
\end{equation}
\end{theorem}
...
\begin{equation}
 \left.\frac{\dd}{\dd t}
 \sinc\!\left(\frac{t}{2^j}\right)\right|_{t=t_N}
...
$t_N^{-m}\frac{\dd^m}{\dd t^m}$; all lower ordinary derivatives vanish to
...
$K$ by
\begin{equation}
 D_m(K)=\sum_{N=1}^{K}\max\{\vtwo(N)+1-m,0\}.        \label{eq:Dm-def}
\end{equation}
...
\begin{equation}
 \boxed{D_m(K)=2M-\wt(M).}                          \label{eq:Dm-formula}
\end{equation}
\end{proposition}

\begin{proof}
A positive contribution requires $2^m\mid N$.  Write $N=2^mk$; the remaining
multiplicity is $\vtwo(k)+1$.  Therefore
\[
 D_m(K)=\sum_{k=1}^{M}(\vtwo(k)+1)
       =M+\vtwo(M!)
       =M+(M-\wt(M)),
\]
...
\begin{equation}
 A_{b,u}=\prod_{\ell\ge1}\sinc(\pi u/b^\ell),
 \qquad
...
\end{equation}
and write $\mu_{i,2k}=\E X_i^{2k}$ and
$\mu_{W,2m}=\E W^{2m}$.  Let $X_i^{\langle k\rangle}$ denote the $k$th
...
\begin{proof}
Let $M_i(t)=\E\e^{tX_i}$ and use the derivation
$\Dop_t=t^{-1}\frac{\dd}{\dd t}$.  By
...
\begin{equation}
 \E\left[(\sqrt{2m}\,W^{\langle m\rangle})^{2r}\right]
 =(2r-1)!!
...
$\varepsilon>0$,
\[
 \Prob(R_m\le1-\varepsilon)
 =\frac{\E[|W|^{2m}\mathbf 1_{\{|W|\le1-\varepsilon\}}]}{m_{2m}}
 \longrightarrow0.
\]
...
$\e^{-t^2/2}$, which has no real zero.  Corollary \ref{cor:LP-hierarchy}
...
\begin{equation}
 \boxed{
 \mu_a=\E|Y|^a
 =\frac{2}{a+1}\E X^{a+1}.}                         \label{eq:moment-CDF-bridge}
\end{equation}
\end{lemma}
...
\begin{align*}
 \mu_a
 &=2\int_0^1r^aF(1-r)\dd r
  =2\int_0^1(1-x)^aF(x)\dd x\\
 &=\frac{2}{a+1}\E(1-X)^{a+1}
  =\frac{2}{a+1}\E X^{a+1}.
\end{align*}
\end{proof}
...
\begin{equation}
 \E g(\mathcal R_n)
 =\frac{\E[|Y|^ng(|Y|)]}{\mu_n}.                   \label{eq:Rn-general}
\end{equation}
...
\begin{equation}
 ...
 =\frac{I_n(u)}{I_n(0)},
 \qquad
 I_n(u)=\int_0^1\e^{-unx}(1-x)^nF(x)\dd x.          \label{eq:In}
\end{equation}
The auxiliary Laplace integral
\begin{equation}
 J(s)=\int_0^1\e^{-sx}F(x)\dd x
 =\frac{L_X(s)-\e^{-s}}{s}                         \label{eq:J-L}
\end{equation}
...
\begin{proofidea}
The inequality $\log(1-x)\le-x$ gives $(1-x)^n\le\e^{-nx}$.  On
$0\le x\le1/2$,
\[
 0\le \e^{-nx}-(1-x)^n
 \le Cnx^2\e^{-nx}.
\]
For the measure proportional to $\e^{-sx}F(x)\dd x$, derivatives of
\eqref{eq:Lambda} and \eqref{eq:J-L} give
\[
 \E_s x^k=\bigO_k\!\left((\log s/s)^k\right).
\]
Consequently, the relative error made by replacing $(1-x)^n$ with
$\e^{-nx}$ is $O((\log n)^2/n)$.  The region $x>1/2$ is exponentially small
...
\begin{align}
 &\frac{\E[(\sqrt{2m}\Tower{m})^{2r}]}{(2r-1)!!}
 \notag\\
 &\quad=1-
 \frac{
...
$\E|B_m|=c_m/m$ gives \eqref{eq:Gm-abs}.
\end{proofidea}
...
Let
\[
 d_m=\E(1-R_m)=1-\frac{\mu_{2m+1}}{\mu_{2m}}.
\]
...
approximately $j_{m-1/2,k}\E(1-R_m)$.  A proof should combine uniform turning
...
Let $\widetilde Y_m=\Tower{m}/\sqrt{\Var(\Tower{m})}$.  A natural quadratic
...
Legendre coefficients of $\Up$ to ultraspherical coefficients of $p_m$.  An
...
primitives of $\Up$.  At dyadic $x$, the repository's exact primitive and
