...
\hypersetup{
  ...
  pdfauthor={Frontier research report for Vladimir Reshetnikov's ProveIt corpus},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Zero-bias towers}
\fancyhead[R]{\small\nouppercase{\leftmark}}
\fancyfoot[C]{\thepage}
...
\[
 H(z)=\FourierTwoPi{\RvachevUp}(\ImaginaryUnit\sqrt z/\pi),
\]
...
$\TwoAdicValuation(N)+1$.  Every tower step removes exactly one inherited order, and at the
first nonzero level the released value has a closed product formula whose sign
is
\(
(-1)^{\TwoAdicValuation(N)+2+\BinaryDigitSum((u-1)/2)}
...
Fourier arithmetic
& Multiplicity $\TwoAdicValuation(N)+1$ and Thue--Morse signs of the original product
...
\(
 \FourierTwoPi{\RvachevUp}(z)=\int\RvachevUp(x)\EulerE^{-2\pi\ImaginaryUnit zx}\Differential x
\), define
\begin{equation}
 H(z)=\FourierTwoPi{\RvachevUp}\!\left(\frac{\ImaginaryUnit\sqrt z}{\pi}\right)
 =\prod_{j=0}^{\infty}
...
\begin{equation}
 f_{U^z}(u)=\frac34(1-u^2)\IndicatorOf{[-1,1]}(u),          \label{eq:uniform-z-density}
\end{equation}
or, equivalently,
\begin{equation}
 U^z\stackrel d=2B-1,
...
\subsection{A \texorpdfstring{$q^2$}{q-squared}-geometric replacement index}
...
\begin{equation}
 b_m(u)=c_m(1-u^2)^{m-1}\IndicatorOf{[-1,1]}(u),
 \qquad
...
\begin{equation}
 A_u=\prod_{\ell=1}^{\infty}
     ...
     =\FourierTwoPi{\RvachevUp}(u/2).                           \label{eq:Au}
\end{equation}
...
\begin{tabular}{@{}rrrrl@{}}
\toprule
$N$ & $r=\TwoAdicValuation(N)+1$ & odd part $u$ & sign & $\phi_r(2\pi N)$ \\
\midrule
...
\subsection{The inherited divisor at level \texorpdfstring{$m$}{m}}
...
\end{equation}
Moreover, with $M=\Floor{K/b^m}$ and $s_b(M)$ the base-$b$ digit sum,
\begin{equation}
 D_{b,m}(K)
 =\frac{bM-s_b(M)}{b-1}.                            \label{eq:baseb-divisor}
\end{equation}
The proof repeats the dyadic argument; the identity
$\sum_{j\ge1}\Floor{M/b^j}=(M-s_b(M))/(b-1)$ replaces Legendre's
binary formula.

\section{Occupancy calculus and the \texorpdfstring{$q$}{q}-Pochhammer layer}
\label{sec:occupancy}
...
\[
 ...
 =\frac{\Expectation[|W|^{2m}\IndicatorOf{|W|\le1-\varepsilon}]}{m_{2m}}
 \longrightarrow0.
\]
...
Let $N\ge1$ and $r=\TwoAdicValuation(N)+1$.  Then
\begin{equation}
