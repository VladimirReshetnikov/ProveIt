...
\begin{abstract}
Let $u=\Up$ be Rvachev's compactly supported $C^\infty$ function,
...
\nolinkurl{drafts/inverse-and-sampling/Dyadic_Comb_Frontiers/} volume), which
...
Throughout, $u=\Up$ is even, nonnegative, supported on $[-1,1]$, and
normalized by
\begin{equation}
 \int_{-1}^{1}u(x)\dd x=1,
 \qquad u(0)=1,
...
\begin{equation}
 ...
 \qquad
 \operatorname{ord}_{\xi=q}\widehat u=1+\vtwo(q)
...
$\vtwo(\rho)$, because the least multiplicity of a nonzero dual-lattice zero
of $\widehat u(\rho\xi)$ is $1+\vtwo(\rho)$.  The smallest common scale that
...
\end{equation}
holds for every $x\in\R$.  On $[-1,1]$ it reduces exactly to the finite sum
\begin{equation}
 \boxed{
...
local shift space generated on $[-1,1]$ by all active $h\Z$-translates.
...
Among uniform lattices $h\Z$ for which $1/h$ is an integer, global
...
$\vtwo(\rho)$, so $\vtwo(1/h)\ge n$ and therefore $1/h$ is divisible by
$2^n$.
\end{proof}
...
sum may instead be synthesized with the coarser grid $2^{-(n-1)}\Z$, halving
...
\begin{equation}
 \norm{u-\text{partial sum through }Q}
 =\norm{u-\Lag^{(Q)}u}
...
Let $N_n=\abs{K_n}=4m-1$.  Define
\begin{align}
 (\Synth_nc)(x)&:=\sum_{k\in K_n}c_k u(x-kh),
 &&\Synth_n:\R^{N_n}\longrightarrow C[-1,1],
 \label{eq:R-def}\\
 (\Coeff_np)_k&:=h(\App p)(kh),
 &&\Coeff_n:\PiN_n\longrightarrow\R^{N_n}.
 \label{eq:S-def}
\end{align}
...
\begin{equation}
 \boxed{
 \Proj_n:=C_nB_n=D_n\widetilde B_n\in\R^{N_n\times N_n}.
 }
 \label{eq:P-def}
\end{equation}
Then:
...
satisfies
\begin{equation}
 \norm{\Proj_n}_2=\frac1{\sin\theta_n},
...
\begin{equation}
 \boxed{
 1=\sum_{\substack{S\subset K_n\\\abs S=n+1}}
...
\[
 2=\norm{e_i-e_j}_1
 \le\sum_k\abs{B_{ik}-B_{jk}}\norm{D_{k,\cdot}}_1
 \le\norm{B_{i,\cdot}-B_{j,\cdot}}_1V(D).
\]
If a row sums to one, its total negative mass is
$(\norm{D_{k,\cdot}}_1-1)/2$, giving \eqref{eq:negative-mass-lower}.
\end{proof}
...
\begin{equation}
 \TV(u(\cdot),u(\cdot-\delta_n))
 =\int_{-\delta_n/2}^{\delta_n/2}u(t)\dd t
 =\delta_n+o(\delta_n),
...
Let
\begin{equation}
 \mathcal V_n:=\Span\{u(\cdot-kh)|_{[-1,1]}:k\in K_n\}.
 \label{eq:Vn}
\end{equation}
...
V_n$ and evaluation is an isomorphism from $\PiN_n$ to $\R^{n+1}$.  Hence
\[
...
Let $\omega=\e^{2\pi\ii/m}$.  The imported dyadic alias theorem supplies the
coefficient sequences
\begin{equation}
 a^{(j,s)}_k=k^s\omega^{jk},
 \qquad 1\le j<m,
 \qquad 0\le s\le\vtwo(j).
...
\begin{equation}
 \rank\widehat\Delta_0^{(s)}=n_0+1,
 \qquad
 \rank\widehat\Delta_q^{(s)}=n_q-n_{q-1}\quad(q\ge1).
...
Let $E_n(u)=\inf_{p\in\PiN_n}\norm{u-p}_\infty$ and let $\Lambda_n$ denote
the Lebesgue constant of the nodes.  The standard estimate is
\begin{equation}
 \norm{u-\Lag_nu}_\infty\le(1+\Lambda_n)E_n(u).
...
\begin{equation}
 \norm{\Synth_nc}_\infty
 \le\norm{c}_\infty\sum_{k\in K_n}u(x-kh)
 =m\norm{c}_\infty,
 \label{eq:R-infty-bound}
\end{equation}
and the actual amplitude matrix is $C_n=hD_n$.  Therefore
\begin{equation}
 \norm{\Lag_n}_{\ell^\infty(X_n)\to L^\infty[-1,1]}
 \le m\norm{C_n}_{\infty\to\infty}
 =V(D_n).
...
Also,
\begin{equation}
 h\norm{D_n}_{\infty\to1}\ge\Lambda_n,
 \label{eq:D-infty-one}
\end{equation}
because $\norm{\Synth_n}_{1\to\infty}=1$.
\end{proposition}
...
$\gamma_r$, and multiplies $\widetilde B_nD_n$ over $\Q$.
...
$1/\TV$ from adjacent kernel rows.}
\label{tab:exact-checks}
\begin{tabular}{rrrrrr}
\toprule
$n$ & $m=2^n$ & shifts & $\widetilde B_nD_n=I$ & $V(D_n)$ & adjacent $1/\TV$ \\
\midrule
...
\begin{tabular}{rrrrr}
\toprule
$n$ & shifts & $\norm{\Proj_n}_2$ equispaced &
$\norm{\Proj_n}_2$ Chebyshev & ratio \\
\midrule
...
with $\log_2\norm{\Proj_n}_2=n+o(n)$, whereas the Chebyshev sequence is much
...
$2m-n-2=\sum_{j=1}^{m-1}(1+\vtwo(j))$ already identifies the projector's
...
\begin{equation}
 \min_{X_n}\norm{D_n(X_n)\widetilde B_n(X_n)}_2,
 \quad
 \min_{X_n}V(D_n(X_n)),
 \quad\text{or}\quad
 \max_{X_n}\theta_n.
...
\midrule
\endhead
Dense fixed-radius coefficient space & $4m-1$ & all active $h\Z$ shifts on $[-1,1]$ \\
...
\item the matrix product is performed over $\Q$.
\end{enumerate}
...
\newblock \nolinkurl{semi-formalized-research-frontiers/drafts/inverse-and-sampling/Dyadic_Comb_Frontiers/},
\texttt{ProveIt} snapshot commit
\nolinkurl{4544e9af6b94820402c69967be1dff3f85e43c1e}, 2026.
