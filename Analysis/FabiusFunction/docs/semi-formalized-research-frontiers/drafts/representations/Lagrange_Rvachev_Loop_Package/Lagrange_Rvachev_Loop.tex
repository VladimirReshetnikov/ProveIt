...
\begin{abstract}
Let $u=\Up$ denote Rvachev's compactly supported $C^\infty$ function,
normalized by $u(x)=F(1-\abs{x})$ on $[-1,1]$, where $F$ is the Fabius
...
\[
 \sum_r\abs{a_{n,j,r}}\ge
 \frac{\norm{\ell_{n,j}}_\infty}{F((n+2)2^{-n})},
\]
so the dyadic endpoint law implies
\[
 \liminf_{n\to\infty}\frac1{n^2}
 \log\sum_r\abs{a_{n,j,r}}\ge\frac{\log2}{2}.
\]
...
$u=\Up$.  The repository proves that, at the sharp dyadic ratio $2^n$, a
...
\[
 u(x)=\Up(x)=F(1-\abs{x}),\qquad -1\le x\le1,
\]
...
\[
 \widehat u(\xi)=\prod_{j=1}^{\infty}\sinc\!\left(\frac{\xi}{2^j}\right),
...
Define
\begin{equation}
 \mathcal P_X=B_X\Phi_X\in\R^{(n+2)\times(n+2)}.
 \label{eq:PX}
\end{equation}
Then
\[
...
frequencies,
\begin{equation}
 E_n(t)=-\frac{(n+1)!}{(2\pi\ii)^{n+1}}
 \sum_{m\in2\Z+1}\frac{\widehat u(m/2)}{m^{n+1}}
 \e^{2\pi\ii mt}.
 \label{eq:En}
\end{equation}
...
$\ker\Phi_X=\Span\{v_X\}$ and annihilates $\ker\lambda_n^T$.  Therefore it
...
$C>0$ satisfied
\[
 \norm{u-I_n^{\mathrm{CL}}u}_\infty\le C\rho^{-n},
\]
...
\begin{equation}
 \limsup_{n\to\infty}
 \norm{u-I_n^{\mathrm{CL}}u}_\infty^{1/n}=1.
 \label{eq:root-rate}
\end{equation}
...
$\sum_m\norm{Q_m}_\infty<\infty$; it does not imply absolute convergence of
...
\begin{equation}
 ...
 \label{eq:n2-growth}
\end{equation}
The same leading lower bound holds for $\norm{b^{(n)}}_1$.
\end{corollary}

\begin{proof}
Theorem \ref{thm:amplitude-barrier} gives
$\log\norm{a^{(n)}}_1\ge-\log F(2^{-t_n})=g(t_n)$.
...
$\widehat u(\xi)=\prod_j\sinc(\xi/2^j)$ have dyadic multiplicities.  At scale
...
\[
 \inf\left\{\sum_s\abs{a_s}:P(x)=\sum_sa_su((x-c_s)/\sigma_s)
 \text{ on }[-1,1]\right\}?
\]
...
$E_r=q^{\binom r2}\qbinom{n+1}{r}$, proving
