...
\item Fourier--Legendre expansions of $\Up$, finite synthesis of polynomials by
      ...
      self-reconstruction.
\item Initial native orthogonal-polynomial data: exact moments, positive Hankel
      determinants, monic recurrence, and low-degree rational Jacobi
      coefficients.
...
\end{equation}
The repository already formalizes the following low-degree data; they are
included to fix every normalization used later:
\begin{align}
 \pi_0(x)&=1, & H_0&=1,\notag\\
 \pi_1(x)&=x, & H_1&=\frac19,
   & \beta_1&=\frac19,\notag\\
...
\end{align}
The last value follows from the same exact moment algorithm and is independently
verified by the archive's Python program.  Numerically,
\begin{equation}
...
low-degree native polynomials, and the Cauchy renormalization.  A realistic
...
eq:legendre-hankel,eq:beta-gaunt-determinants}.  These need only finite
      linear algebra and the existing positivity API.
