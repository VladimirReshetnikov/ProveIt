...
\cref{eq:gaunt-integral}, the finite matrix identity
\cref{eq:finite-gaunt-matrix}, or the Wigner/\(3j\)-symbol factorial form
\cref{eq:wigner-factorial}.  Entry
rationality is now formalized through the executable coefficient--moment double
sum, but rationality by the Gaunt/Wigner route remains paper-only.  Christoffel
reconstruction, root results, quadrature, infinite Jacobi products, and
asymptotics also remain paper-only.  Finally, the existing
...
determinant ratios.  This route is coefficientwise and does not use
\cref{eq:gaunt-integral,eq:finite-gaunt-matrix,eq:wigner-factorial}.
\end{remark}
...
\item \textbf{Finite algebra:} the determinant change-of-basis and
      ...
      \path{FabiusLegendreRationalGram.lean}. Formalize the remaining
