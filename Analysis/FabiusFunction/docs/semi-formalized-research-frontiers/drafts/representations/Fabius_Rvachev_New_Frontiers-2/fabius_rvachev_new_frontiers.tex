...
polynomials, continued-fraction/Pad\'e identification, Legendre--Gaunt determinant
bridge, or Szeg\H{o}/Nevai/Rakhmanov asymptotics.
\end{remark}
...
\(n\) corresponding to \(\beta_{n+1}\).  The remaining formalization sequence is:
\begin{enumerate}
\item \textbf{Finite algebra:} formalize
      \cref{eq:inverse-hankel-christoffel,eq:monic-center-product,%
eq:orthonormal-center-product,eq:harmonic-mean,%
eq:legendre-hankel,eq:beta-gaunt-determinants}.  These need only finite
      linear algebra and the existing positivity and rational-native APIs; none is
      supplied by \path{RvachevRationalJacobi.lean} itself.
