...
\path{LegendrePolynomialRational.lean}, with exactly two public definitions and six
public theorems; \path{FabiusLegendreRationalGram.lean}, with three definitions and
eleven theorems; and \path{FabiusLegendreRationalGramValues.lean}, with eleven
theorems.  Together with the four modules above, the complete seven-module inventory
has thirteen definitions and sixty-one theorems, seventy-four public declarations.
The first downstream module supplies executable rational Legendre coefficients.  The
second supplies executable rational Gram entries, matrices, and determinants.  Its
rational determinant identity, \(D_0=1\), strict positivity, norm quotient, and
zero-based Jacobi ratio are unconditional; only its entry, matrix, and determinant cast
bridges require a \path{BoundedFabius} witness \(F\) and a proof
\path{IsFabius F}.  The value module proves unconditionally that
...
This closure does \emph{not} formalize the finite Gaunt integral,
Wigner/\(3j\)-symbol, or factorial entry formulas
\cref{eq:gaunt-integral,eq:finite-gaunt-matrix,eq:wigner-factorial}; consequently it
does not establish entry rationality by that route.  It also proves no Christoffel
reconstruction.  Finally, the existing \path{rvachevTranslateGram} is a different
object: it pairs shifted-up atoms under unweighted interval integration, whereas
\path{upLegendreGramMatrix} pairs ordinary polynomials under the up-law moment
functional.
\end{remark}
...
exact quotients of those formal norm values.  The downstream value module now also
names and proves the final-row \(H_4\) and \(\beta_4\) fractions in Lean.  The combined
table fixes every normalization used later:
\begin{align}
 \pi_0(x)&=1, & H_0&=1,\notag\\
 \pi_1(x)&=x, & H_1&=\frac19,
   & \beta_1&=\frac19,\notag\\
...
\end{align}
The two final-row fractions are named exact rational Lean computations and are also
independently verified by the archive's Python program.  Numerically,
\begin{equation}
...
recurrence that form the algebraic core of these engines.  It does not package the
three procedures as an executable algorithm, prove their operational equivalence as
this proposition states it, or certify the \(\bigO(n^2)\) operation count.
...
Gaunt/Wigner entry expansion.  The remaining formalization sequence is:
\begin{enumerate}
