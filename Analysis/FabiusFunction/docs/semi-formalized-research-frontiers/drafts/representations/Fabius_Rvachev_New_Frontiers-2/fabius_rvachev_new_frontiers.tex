...
\definecolor{deepred}{RGB}{140,45,45}
\hypersetup{
  colorlinks=true,
  linkcolor=linkblue,
  citecolor=deepgreen,
...
\newsharedtheorem{principle}{Principle}
...
Gaunt/Wigner entry expansion. The determinant change-of-basis and Jacobi
cross-ratio are now formalized downstream. ``New'' therefore applies only to the
...
$\beta_n\to1/4$, arcsine zeros
 & New application here & Rakhmanov/regular OPRL theory \\
...
Gauss--Pad\'e package
 & New synthesis here & Classical Markov/Stieltjes theory \\
...
Finite Gaunt/Wigner entry expansion
 & New finite identity here & Existing exact Legendre coefficients \\
...
\path{hankelRatio_four_div_three}.  Thus this focused determinant/rational-Legendre
inventory contains the preceding 40 declarations plus these eleven, for 51 in all.
The module uses exact finite native computation to certify the fourth nontrivial even-moment
value (the raw eighth moment) and
Gram determinants through order five.  It then proves the printed values
\[
 H_4=\frac{26727424}{55791736875},
 \qquad
 \beta_4=\frac{835232}{4640643}
\]
as named rational theorems, and transports \(H_4\) to the real Hankel ratio and
squared integral and \(\beta_4\) to the quotient of the fourth and third real Hankel
ratios for every \path{IsFabius} representative.  These are exact low-order
certificates, not a general native evaluator.

Together with the four finite-algebra modules above, the three downstream modules bring
the complete seven-module inventory to thirteen definitions and sixty-one theorems,
seventy-four public declarations.  More explicitly, the value module proves
...
\path{BoundedFabius} witness \(F\) and a proof \path{IsFabius F}.

This closure still does \emph{not} formalize equality with the finite Gaunt integral,
Wigner/\(3j\)-symbol, or factorial entry formulas
\cref{eq:gaunt-integral,eq:finite-gaunt-matrix,eq:wigner-factorial}.  Entry
rationality is now proved by the direct moment-sum route, not by those formulas.
Christoffel reconstruction, root and quadrature results, and the infinite continued-
fraction or asymptotic Jacobi layers also remain open.
...
\begin{equation}
 ...
 \qquad m_8=\frac{132809}{32531625}.
 \label{eq:native-first-moments}
\end{equation}
The repository directly formalizes \(\pi_0,\ldots,\pi_4\) and
\(H_0,\ldots,H_4\); the displayed \(\beta_1,\beta_2,\beta_3\) follow as
exact quotients of the earlier formal norm values, while
\path{rvachevJacobiSubdiagonalRat_three} gives the zero-based exact value
\(\beta_4\).  The executable rational Gram determinants give an exact evaluation
route for that final row:
\path{rvachevOrthoNormRat_eq_rvachevLegendreGramDetRat_ratio} at \(n=4\)
computes \(H_4\), and
\path{rvachevJacobiSubdiagonalRat_eq_rvachevLegendreGramDetRat_ratio} at \(n=3\)
computes \(\beta_4\).  The exact evaluations are now named directly as
\path{rvachevOrthoNormRat_four} and
\path{rvachevJacobiSubdiagonalRat_three}; \path{hankelRatio_four},
\path{integral_sq_upOrthoPolynomial_four} identify \(H_4\) with the real Hankel
ratio and squared integral, while \path{hankelRatio_four_div_three} identifies
\(\beta_4\) with the quotient of the fourth and third real Hankel ratios.
The combined table is included to fix every normalization used later:
\begin{align}
 \pi_0(x)&=1, & H_0&=1,\notag\\
 \pi_1(x)&=x, & H_1&=\frac19,
   & \beta_1&=\frac19,\notag\\
...
\end{align}
The two final-row fractions are exact rational Lean computations, follow from the same
exact moment algorithm, are independently verified by the archive's Python program,
and have the named Lean
...
Legendre tranche adds an executable scalar coefficient sum and executable Gram
entry, finite matrix, and determinant.  Together with its positive determinant-ratio
theorems, this is an exact evaluation route for the norm and Jacobi data, including
\(H_4\) and \(\beta_4\); the convenience polynomial wrapper itself remains
noncomputable.  The values module now states both displayed final-row fractions,
the real Hankel and squared-integral forms of \(H_4\), and the real Hankel-ratio
quotient form of \(\beta_4\) as named exact theorems.  Lean still does not
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{zero_quantiles.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{pi_approximants.png}
...
$G=C^{\mathsf T}HC$ determinant argument and does not pass through the still-open
Gaunt/Wigner entry expansion.  The determinant identity, $D_0=1$, the
...
Thus entrywise rationality is now closed by a direct moment-sum argument.  What remains
open is equality of this evaluator with the finite Gaunt integral, Wigner/\(3j\), and
factorial expressions in
\cref{eq:gaunt-integral,eq:finite-gaunt-matrix,eq:wigner-factorial}; none of the
determinant or rational-entry theorems proves those formulas.
\end{remark}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{jacobi_coefficients.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{scaled_jacobi_deficit.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{pi_product_error.png}
...
\cref{eq:legendre-hankel,eq:beta-gaunt-determinants}, without formalizing the
Gaunt/Wigner entry expansion.  The executable rational Legendre coefficient and
...
eq:orthonormal-center-product,eq:harmonic-mean}.  These need only finite
      ...
      formalize \cref{eq:gaunt-integral,eq:finite-gaunt-matrix,eq:wigner-factorial}
      if an entrywise Gaunt/\(3j\)-symbol realization is desired.
...
\end{enumerate}
The finite Gaunt/Wigner/\(3j\) equality and Christoffel identities should not be
