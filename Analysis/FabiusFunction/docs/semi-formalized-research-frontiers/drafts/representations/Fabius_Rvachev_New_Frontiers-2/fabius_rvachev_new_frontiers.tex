...
The current Lean development closes the finite Gaunt-integral and product-linearization
portion of this circuit, but not the Wigner identification or the later
Christoffel and limiting steps.
...
rational/real Rvachev Gram-entry sums are now formalized downstream; the
Wigner/$3j$ identification and its factorial closed form are not.  ``New''
...
Wigner/$3j$ identification and factorial entry formula
 & Classical report-level formula & Not represented in the current Lean API \\
...
Rvachev Gram-entry expansion, but not the Wigner identification/factorial formula,
Christoffel reconstruction, or named values for the displayed \(G_3\) entries.
...
Thus this focused determinant/rational-Legendre and Gaunt inventory contains
the preceding 65 declarations plus these eleven, for 76 in all.  The first
...
Together with the four finite-algebra modules above, the five downstream modules bring
the complete nine-module inventory to eighteen definitions and eighty-one theorems,
ninety-nine public declarations.  More explicitly, the value module proves
...
\path{BoundedFabius} and \path{IsFabius}.  It does \emph{not} formalize the
Wigner/\(3j\)-symbol equalities and factorial formula recorded in
\cref{eq:gaunt-wigner,eq:finite-wigner-matrix,eq:wigner-factorial}, nor an
infinite-series/integral interchange.  No named theorem evaluates the displayed
...
\end{equation}
The current Lean theorem identifies this integral with an executable rational triple
coefficient sum.  It also proves that odd $j+k+\ell$ or violation of any strict triangle
inequality forces $\Gamma_{jk\ell}=0$; these are necessary support conditions, not a
converse or nonvanishing theorem.  The classical Wigner identification used by the report
is the further equality
\begin{equation}
 \Gamma_{jk\ell}
 =2\begin{pmatrix}j&k&\ell\\0&0&0\end{pmatrix}^{\!2}.
 \label{eq:gaunt-wigner}
\end{equation}
This second equation remains outside Lean.
...
The equivalent Wigner form used in the paper is the following classical consequence:
\begin{equation}
 \boxed{
 (G_N)_{jk}=
...
\end{equation}
Its equality with \cref{eq:finite-gaunt-matrix} still depends on the open Lean
identity in \cref{eq:gaunt-wigner}.
...
\end{equation}
otherwise it is zero.  \Cref{eq:wigner-factorial} makes the
rationality in \cref{eq:finite-wigner-matrix} completely explicit.
...
Thus the integral Gaunt form and finite Rvachev Gram-entry expansion are Lean-exact.
The Wigner identities in \cref{eq:gaunt-wigner,eq:finite-wigner-matrix} and factorial
formula \eqref{eq:wigner-factorial} remain open, as do Wigner-based nonnegativity and
any converse to the proved parity/triangle vanishing conditions.  The displayed
...
conditions, and the rational and real forms of \eqref{eq:finite-gaunt-matrix}; it does
not formalize the Wigner forms.  The executable rational Legendre coefficient and
...
eq:orthonormal-center-product,eq:harmonic-mean}.  These need only finite
      linear algebra and the existing positivity and rational-native APIs.  Separately,
      formalize the Wigner identities in
      \cref{eq:gaunt-wigner,eq:finite-wigner-matrix,eq:wigner-factorial}; the
      Gaunt-integral forms no longer belong on the open list.
...
\end{enumerate}
The remaining Wigner/\(3j\) equality and Christoffel identities should not be
blocked on the much larger infinite or asymptotic OPRL formalization.  In
