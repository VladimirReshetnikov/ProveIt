...
The coherent-basis module \path{PolynomialMomentGramDeterminant.lean} contains
two public definitions, \path{polynomialCoefficientMatrix} and
\path{polynomialMomentGramMatrix}, and seven public theorems: the two entry
formulas, \path{polynomialMomentGramMatrix_eq_transpose_mul_hankel_mul},
...
For a degree-coherent family these prove \(G_n=C_n^{\mathsf T}H_nC_n\), its
leading-coefficient determinant formula, and the field-level Jacobi
Gram-determinant cross-ratio under nonzero diagonal coefficients.  No Hankel
nonvanishing hypothesis is needed in the last theorem.  The coefficient-matrix
definition and entry formula require only a semiring; the Gram-matrix definition,
entry formula, and congruence require a commutative semiring; and the determinant
formulas require a commutative ring.
...
The specialization \path{FabiusLegendreHankelDeterminant.lean} contains two
public definitions, \path{upLegendreGramMatrix} and
...
\path{rvachevJacobiSubdiagonalRat_cast_eq_upLegendreGramDet_ratio}.  Thus the
Legendre Gram determinant is the monomial Hankel determinant times the squared
product of Legendre leading coefficients, is positive, and gives both the real
and rational \(\beta_{n+1}\) determinant cross-ratios.
For \(F\) of Lean type \path{BoundedFabius}, the determinant identity, empty value,
...
\(\beta_4\) fractions displayed below.  This tranche also proves no roots,
Christoffel reconstruction or central products, Gauss quadrature, second-kind
polynomials, continued-fraction/Pad\'e identification, finite Gaunt/Wigner
entry formula or rationality by that route, or
...
exact quotients of those formal norm values.  The final-row \(H_4\) and
\(\beta_4\) fractions are instead report-level exact computations, not named
Lean evaluations.  The combined table is included to fix every normalization
used later:
\begin{align}
 \pi_0(x)&=1, & H_0&=1,\notag\\
 \pi_1(x)&=x, & H_1&=\frac19,
...
\end{align}
The two final-row fractions follow from the same exact moment algorithm and are
independently verified by the archive's Python program.  The all-degree rational
objects described above prove their rationality and positivity but do not compute
these explicit values.  Numerically,
\begin{equation}
...
\end{equation}
Let $D_N=\det G_N$.  The triangular change of basis from monomials to
...
\(n\) corresponding to \(\beta_{n+1}\).  The coherent-basis determinant congruence,
the Legendre--Hankel identity \cref{eq:legendre-hankel}, and the Legendre
Gram-determinant form \cref{eq:beta-gaunt-determinants} are now supplied by
\path{PolynomialMomentGramDeterminant.lean} and
\path{FabiusLegendreHankelDeterminant.lean}; the explicit finite
Gaunt/Wigner entry formula \cref{eq:finite-gaunt-matrix} remains open.  The remaining
formalization sequence is:
\begin{enumerate}
\item \textbf{Finite algebra:} formalize
      \cref{eq:inverse-hankel-christoffel,eq:monic-center-product,%
eq:orthonormal-center-product,eq:harmonic-mean,eq:finite-gaunt-matrix}.
      These need only finite linear algebra and the existing positivity,
      rational-native, and determinant-bridge APIs.
