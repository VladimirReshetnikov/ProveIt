...
construct a signed Wigner symbol or phase convention, and the later Christoffel and
limiting steps remain open.
...
Integer-index zero-row square datum, factorial form, and finite Wigner-square matrix
 & Exact Lean counterpart & \path{LegendreGauntClosedForm.lean};
   \path{FabiusLegendreGauntClosedForm.lean} \\
Signed/general Wigner symbols, phase, and recoupling
...
20 definitions and 109 theorems.  Each name appears once in the inventory below.
...
half-integer, nonzero-magnetic-index, general recoupling, or infinite-series/integral
interchange theorem.  No named theorem evaluates the displayed
...
\end{equation}
Lean identifies this integral with an executable rational triple coefficient sum.
It defines \(W^2_{jk\ell}\) to be the total rational integer-index zero-row square
datum: the central-binomial quotient on the even weak-triangle support and zero
off it.  This is not a signed Wigner symbol and makes no phase choice.  With that
precise interpretation, Lean proves the all-degree identity
\begin{equation}
 \Gamma_{jk\ell}
 =2\begin{pmatrix}j&k&\ell\\0&0&0\end{pmatrix}^{\!2}.
 \label{eq:gaunt-wigner}
\end{equation}
as well as \(\Gamma_{jk\ell}>0\) exactly on the admissible support and zero
exactly off it.
...
signed/general Wigner-symbol object.
...
\cref{eq:finite-wigner-matrix} completely explicit inside Lean.
...
nonzero-magnetic-index theory, and general Wigner orthogonality or recoupling remain
open.  The displayed
...
eq:orthonormal-center-product,eq:harmonic-mean}.  These need only finite
      ...
      phase conventions, magnetic indices, orthogonality, and recoupling.
