...
\begin{abstract}
Let $u=\Up$ be Rvachev's even $C^\infty$ probability density supported on
...
Put
\[
 \MM_u(z)=\int_{-1}^{1}\e^{zx}u(x)\dd x
 =\prod_{j\ge1}\frac{\sinh(z/2^j)}{z/2^j},
 \qquad
 \Dscr=\MM_u(D)^{-1},
 \qquad Q_\ell=\Dscr P_\ell.
\]
For any integer $M$ with $\vtwo(M)\ge\ell$,
\[
...
\[
 \Lambda_m(c)=\frac{2m+1}{2}\int_{-1}^{1}u(x-c)P_m(x)\dd x.
\]
It is compactly supported and smooth, satisfies
$\Lambda_{2n}(0)=u_n$, and has the spherical-Bessel representation
\[
 \Lambda_m(c)=(2m+1)(-\ii)^m\int_{\R}
 \widehat u(\xi)j_m(2\pi\xi)\e^{2\pi\ii\xi c}\dd\xi.
\]
...
the new conjecture $\vtwo(u_n)=-1$ for every $n$, while moments through
$\mu_{160}$ suggest
$\vtwo(\mu_{2m}-1)=3+\vtwo(m)$.  All numerical and symbolic experiments are
...
multiplicities
\[
 \ord_{\xi=k}\widehat u(\xi)=1+\vtwo(k)
...
Let $u=\Up$ be the unique even nonnegative $C^\infty$ function supported on
$[-1,1]$, normalized by
\[
 \int_{-1}^{1}u(x)\dd x=1,
 \qquad u(0)=1.
...
\end{equation}
The zero at a nonzero integer $k$ has exact order $1+\vtwo(k)$.
...
\begin{equation}
 \int_{-1}^{1}P_n(x)\e^{-\ii tx}\dd x=2(-\ii)^n j_n(t).
...
integer satisfying $\vtwo(M)\ge d$.  Then, for every $x\in[-1,1]$,
\begin{equation}
...
$1+\vtwo(k)$ annihilates every alias derivative through degree $d$, while
...
Let $M$ be a positive integer with $\vtwo(M)\ge\ell$.  Then
\begin{equation}
 \boxed{
...
\begin{equation}
 ...
 \label{eq:Q-jacobi}
\end{equation}
Consequently $Q_\ell\in\Q[x]$ and
\begin{equation}
...
\begin{center}
\small
\input{legendre_coefficients_table.tex}
\end{center}
...
$\norm{P_{2n}}_\infty=1$ and the canonical expansion is absolutely uniformly
convergent,
\[
 \sum_{n\ge0}\norm{u_nP_{2n}}_\infty=\sum_{n\ge0}|u_n|<\infty.
\]
\end{proof}
...
\label{def:Lambda}
For $m\ge0$ and $c\in\R$, define
\begin{equation}
 \boxed{
 \Lambda_m(c):=\frac{2m+1}{2}
 \int_{-1}^{1}u(x-c)P_m(x)\dd x.}
 \label{eq:Lambda-def}
\end{equation}
\end{definition}
...
For every $m\ge0$:
\begin{enumerate}
  \item $\Lambda_m\in C_c^\infty(\R)$ and
        $\supp\Lambda_m\subseteq[-2,2]$;
  \item if $Z$ has density $u$, then
  \begin{equation}
   \Lambda_m(c)=\frac{2m+1}{2}\,
   \E\!\left[P_m(c+Z)\mathbf1_{\{|c+Z|\le1\}}\right];
...
For all $m\ge0$ and $c\in\R$,
\begin{equation}
 \boxed{
 \Lambda_m(c)=(2m+1)(-\ii)^m
 \int_{\R}\widehat u(\xi)j_m(2\pi\xi)
 \e^{2\pi\ii\xi c}\dd\xi.}
...
\begin{equation}
 \boxed{
 u_n=(4n+1)(-1)^n
 \int_{\R}\widehat u(\xi)j_{2n}(2\pi\xi)\dd\xi.}
 \label{eq:un-bessel}
\end{equation}
\end{theorem}
...
\[
 \widehat\chi_m(\xi)=2(-\ii)^m j_m(2\pi\xi).
\]
...
If $c$ is dyadic rational, then $\Lambda_m(c)\in\Q$ for every $m$.
\end{proposition}
...
\[
 \Lambda_m(c)=\frac{2m+1}{2}
 \int_{[-1,1]\cap[-1-c,1-c]}u(y)P_m(y+c)\dd y.
\]
...
$\int_a^b y^r u(y)\dd y$ at dyadic endpoints in rational Fabius/up values and
...
The up-function is $C^\infty$ on $\R$ and vanishes outside $[-1,1]$.
Therefore
\begin{equation}
 u^{(s)}(1)=u^{(s)}(-1)=0
 \qquad(s\ge0).
...
Let $\vtwo(M)\ge\ell$.  For every $s\ge0$,
\begin{equation}
 \boxed{
...
$2|j_m(t)|\le\int_{-1}^{1}|P_m(x)|\dd x\le2$.
\end{proof}
...
\[
 \int_{\log X}^{\infty}
 \exp\!\left(-\frac{t^2}{2a}-(\kappa-1)t\right)\dd t.
\]
...
\[
 |u_n|\le C_2n^2q^{2n}
 +C_3n\int_{X_n}^{\infty}E_{\kappa_\infty}(x)\dd x.
\]
...
All rational numbers in this subsection are viewed inside $\Q_2$.
...
$|Q_{2n}(0)|^{1/(2n)}\sim2n/(\pi\e)$ implies that the degree at which the
central atom coefficient reaches size $X$ is governed to first order by
inverting
\[
 2n\log\!\left(\frac{2n}{\pi\e}\right)\asymp\log X,
\]
thus
\begin{equation}
 n\asymp\frac{\log X}{2W(\log X/(\pi\e))}.
