...
Let $A\in\mathrm{GL}_d(\R)$ be expansive, let
...
\[
 ...
 \sinc\!\bigl(\ip{\xi}{A^{-n}v_j}\bigr),
 \qquad
 \Phi_{A,V}(A^T\xi)=\prod_j\sinc\!\bigl(\ip{\xi}{v_j}\bigr)\Phi_{A,V}(\xi).
\]
...
$(-1)^{s_2(k)}$.  The same cube gives a tensor Prouhet identity annihilating every
...
Its support has perimeter $4\norm v q/(1-q)$ and area
\[
 \frac{4\norm v^2q^2}{1-q^2}
 \sum_{m\ge1}\abs{\sin(m\theta)}q^m.
\]
...
\]
Its law has support $[-1,1]$, density $\Up$, and characteristic function
$\prod_{n\ge1}\sinc(2^{-n}\xi)$, where throughout
\[
...
Fix integers $d,r\ge1$.  Let $A\in\mathrm{GL}_d(\R)$ be \emph{expansive}:
every eigenvalue of $A$ has modulus greater than one.  Put $B=A^{-1}$, let
$V=[v_1\ \cdots\ v_r]\in\R^{d\times r}$, and let
...
be denoted by $\mu$, $\Phi$, $M$, and $K$ respectively.  For $g\in\R^d$,
...
\begin{equation}
 h_K(w)=\sup_{x\in K}\ip{w}{x}
       =\sum_{n\ge1}\sum_{j=1}^r\abs{\ip{w}{g_{n,j}}}.
...
\end{equation}
Finally, $K$ has nonempty interior in $\R^d$ exactly when
\begin{equation}
 \Span\{B^nv_j:n\ge1,\ 1\le j\le r\}=\R^d.
...
$\norm{B^n}\le C_\rho\rho^n$.  Since
$\abs{U_{n,j}}\le1$,
\[
 \sum_{n,j}\norm{B^nv_jU_{n,j}}
 \le C_\rho\sum_j\norm{v_j}\sum_{n\ge1}\rho^n<\infty.
\]
The convergence is therefore deterministic and uniform on the compact product
space $\Omega=[-1,1]^{\N\times r}$.

The summation map $S:\Omega\to\R^d$ is continuous, and the product measure of
...
if the span is all of $\R^d$, some finite subcollection of generators is a basis;
...
For every $w\in\R^d$, the scalar projection $\ip{w}{X}$ has support
...
\label{thm:mahler}
\New\ The characteristic function extends to an entire function on $\C^d$ and
has the locally uniformly convergent product
\begin{equation}
 \Phi(z)=\E\e^{\ii\ip{z}{X}}
 =\prod_{n=1}^{\infty}\prod_{j=1}^r
   \sinc\!\bigl(\ip{z}{B^nv_j}\bigr).
...
\begin{equation}
 ...
 \qquad
 m_V(z):=\prod_{j=1}^r\sinc\!\bigl(\ip{z}{v_j}\bigr).
...
$\sinc u=1-u^2/6+O(u^4)$ locally and
$\sum_{n,j}\norm{B^nv_j}^2<\infty$, the Weierstrass criterion gives local
uniform convergence on $\C^d$ and hence an entire limit.  Substitution gives
\[
 \Phi(A^Tz)
 =\prod_{n,j}\sinc\!\bigl(\ip{z}{AB^nv_j}\bigr)
 =\prod_j\sinc\!\bigl(\ip{z}{v_j}\bigr)
  \prod_{n,j}\sinc\!\bigl(\ip{z}{B^nv_j}\bigr).
\]
Finally, $|\e^{\ii\ip{z}{x}}|=\e^{-\ip{\operatorname{Im}z}{x}}$ and
...
\begin{equation}
 ...
 \abs{\ip{y}{B^kv_j}}\ge c\norm y
 \qquad(y\in\R^d).
 \label{eq:frame}
\end{equation}
For every $\beta>\rho(A)$ there is $C_\beta\ge1$ such that, for every block
index $m\ge0$ and every $\xi\in\R^d$, one of the factors with
...
\New\ Let
$W=\Span\{B^nv_j:n\ge1,1\le j\le r\}$.  If $W\ne\R^d$, then $\mu$ is
supported on a proper subspace and is singular with respect to $d$-dimensional
Lebesgue measure.  If $W=\R^d$, then $\mu$ has a compactly supported
$C^\infty$ density $f$, and for every $\beta>\rho(A)$,
\begin{equation}
 |\Phi(\xi)|\le
 \exp\left
 \{-\frac{(\log\norm\xi)^2}{2L\log\beta}+O(\log\norm\xi)\right\}
 \qquad(\norm\xi\to\infty),
 \label{eq:log-gaussian}
\end{equation}
where $L$ is any spanning-block length in \cref{lem:block-frame}.  In
particular, $\norm\xi^N\Phi(\xi)$ is bounded for every $N$.
\end{theorem}

\begin{proof}
Only the full-span case remains.  On the real axis,
$|\sinc t|\le\min(1,|t|^{-1})$.  Choose from each block the factor supplied by
\cref{lem:block-frame}.  Put $R=\norm\xi$ and let $M$ be the largest integer for
...
$\R^d$, is strictly positive on $\operatorname{int}K$, and vanishes off $K$.
...
$BV(2\omega-\one)$ with signs $(-1)^{|\omega|}$.  The density of $B_\#\mu$ is
$|\det A|f(Ax)$, so translating and simplifying produces
\eqref{eq:matrix-rvachev}.  In the scalar case the left side is
$D_{1/2}\Up=\Up'/2$, and the displayed specialization follows.
\end{proof}
...
$\norm{f-f_N}_{C^m}=O(\rho^{2N})$ for every $\rho>\rho(B)$.
\end{theorem}

\begin{proof}
Choose a fixed prefix $N_0$ containing sufficiently many spanning blocks that
$|\Phi_{N_0}(\xi)|\le C(1+\norm\xi)^{-m-2k-d-2}$ for the needed value of $k$.
...
\[
 \Psi_N(\xi)=\prod_{n>N,j}\sinc\ip{\xi}{g_{n,j}}.
\]
The cumulant expansion of a uniform digit begins
\[
 \log\sinc t=-\frac{t^2}{6}-\frac{t^4}{180}+O(t^6).
\]
Therefore
\[
...
$|1-\sinc t|\le Ct^2$ and the sixth-order Taylor remainder is bounded by
$C|t|^6$.  Multiply by $\Phi_N$, integrate against $\norm\xi^m$, and use the
fixed integrable majorant.  Under the convention
$\widehat f(\xi)=\int\e^{\ii\ip{\xi}{x}}f(x)\dd x$, multiplication by
...
\begin{equation}
 ...
 =\sum_{m\ge1}\frac{c_{2m}}{(2m)!}
   (\ii)^{2m}\sum_{n>N,j}\ip{\xi}{g_{n,j}}^{2m}.
 \label{eq:tail-log}
\end{equation}
...
\begin{equation}
 \sum_{\omega}\varepsilon_\omega\e^{\ip{z}{s_\omega}}
...
\end{equation}
Equivalently, the signed cube annihilates every polynomial on $\R^d$ of total
degree less than $M$.
\end{corollary}
...
\[
 \prod_{\ell=1}^M
 \left(\e^{-\ip{z}{g_\ell}}-\e^{\ip{z}{g_\ell}}\right),
\]
...
\begin{equation}
 \E Y_w^k=\mathcal B_k(\lambda_1(w),\ldots,\lambda_k(w)).
...
\end{equation}
If $A$ is diagonalizable over $\C$, contraction of $\mathbf A_k$ with symmetric
...
\[
 T\mathcal A(x,t)
 =\frac{\e^{\ip{B^Tt}{x}}}{M(t)}
  \prod_j\frac{\sinh\ip{B^Tt}{v_j}}{\ip{B^Tt}{v_j}}
 =\frac{\e^{\ip{B^Tt}{x}}}{M(B^Tt)}.
\]
...
\begin{equation}
 a_\ell(w)=\frac{2\ell+1}{2}\E P_\ell(Y).
 \label{eq:legendre-coefficient}
\end{equation}
Using
\begin{equation}
 P_\ell(y)=2^{-\ell}\sum_{k=0}^{\lfloor\ell/2\rfloor}
...
\begin{align}
 a_0&=\frac12,\nonumber\\
 a_2&=\frac54\left(3\E Y^2-1\right),\nonumber\\
 a_4&=\frac9{16}\left(35\E Y^4-30\E Y^2+3\right).
...
\begin{equation}
 ...
 \abs{\det(g_{\ell_1},\ldots,g_{\ell_d})}.
...
$|\det(g_{\ell_1},\ldots,g_{\ell_d})|\le\prod_i\norm{g_{\ell_i}}$ and
$\sum_\ell\norm{g_\ell}<\infty$.  The finite zonotopes converge to $K$ in
...
\begin{equation}
 ...
 \abs{\det(B^{n_1}v_{j_1},\ldots,B^{n_d}v_{j_d})}.
...
$\theta\in\pi\Z$; otherwise it has a $C^\infty$ density.
...
\begin{equation}
 h_{q,\theta}(\varphi)
 =a\sum_{n\ge1}q^n
   \abs{\cos(n\theta+\varphi_0-\varphi)}.
...
\begin{equation}
 ...
 =\frac{4a^2q^2}{1-q^2}
  \sum_{m=1}^{\infty}\abs{\sin(m\theta)}q^m.
 \quad}
...
\begin{align}
 ...
 &=\frac{4a^2q^2}{1-q^2}
 \sum_{m=1}^{N-1}q^m\abs{\sin(m\theta)}
...
$[-g,g]$ contributes $4\norm g$; approximation by thin rectangles justifies this
...
\[
 \operatorname{Area}(K)
 =4\sum_{1\le n<k}q^{n+k}a^2\abs{\sin((k-n)\theta)}.
\]
...
$2\norm{g_n}=2aq^n$.  The directions $n\theta\bmod\pi$ are dense by Weyl's
...
$\widehat S(k)=\int\e^{-\ii k\phi}\dd S(\phi)$,
\begin{equation}
 \widehat S(k)
 =4a\cos\frac{k\pi}{2}\,
  \e^{-\ii k\varphi_0}
  \frac{q\e^{-\ii k\theta}}{1-q\e^{-\ii k\theta}}.
...
\begin{equation}
 S_\theta(z)=\sum_{m\ge1}\abs{\sin(m\theta)}z^m,
 \qquad |z|<1.
...
\New\ Let $g(x)=\sum_{k\in\Z}c_k\e^{\ii kx}$ be a continuous periodic
...
\begin{equation}
 \lim_{r\uparrow1}(1-r)G(r\e^{-\ii k\theta})
 =c_k\e^{\ii k\delta}.
 \label{eq:abel-residue}
\end{equation}
Every point $\e^{-\ii k\theta}$ for which $c_k\ne0$ is therefore a singularity
...
\[
 (1-r)G(r\e^{-\ii k\theta})
 =\sum_{\ell\in\Z}c_\ell\e^{\ii\ell\delta}
 \frac{(1-r)r\e^{\ii(\ell-k)\theta}}
      {1-r\e^{\ii(\ell-k)\theta}}.
\]
The $\ell=k$ term tends to $c_k\e^{\ii k\delta}$, and every other term tends to
...
\New\ If $\theta/\pi\in\Q$, the coefficient sequence
...
\end{equation}
If $\theta/\pi\notin\Q$, the unit circle is a natural boundary of
...
points $\e^{-2\ii k\theta}$ are dense because $\theta/\pi$ is irrational.
...
\begin{equation}
 H_{\theta,\varphi}(z)
 =\sum_{n\ge1}\abs{\cos(n\theta+\varphi_0-\varphi)}z^n
...
mode contains
\[
 \frac{(1-q)\e^{\ii k\theta}}{1-q\e^{\ii k\theta}}\longrightarrow0.
\]
...
$\ip{w}{q^nR_{n\theta}v}=a\norm w q^n\cos(n\theta+\delta)$.  Let
$Q=q^{2m}$.
...
\begin{equation}
 ...
 \label{eq:deficit}
\end{equation}
Its distribution function $C_w(x)=\Prob(D_w\le x)$ is a directional analogue of
...
\begin{equation}
 \mathcal L_w(s):=\E\e^{-sD_w}
 =\prod_{n,j:a_{n,j}>0}
  \frac{1-\e^{-2sa_{n,j}}}{2sa_{n,j}}
 \qquad(s\ge0).
...
$(2\pi)^{-1}\int_0^{2\pi}\log(2|\cos x|)\dd x=0$, but close returns to the zeros
...
$\mathcal C_{\lfloor\log s/\lambda\rfloor}(\theta,\delta)$.  After saddle-point
...
identity
\[
 \Phi_N(A^T\xi)=\sinc\ip{\xi}{v}\Phi_{N-1}(\xi)
\]
...
denominators $|1-q\e^{2\ii k\theta}|$.  More precisely, suitable anisotropy norms
...
\[
 \widehat f(\xi)=\int_{\R^d}\e^{\ii\ip{\xi}{x}}f(x)\dd x,
 \qquad
 f(x)=\frac1{(2\pi)^d}\int_{\R^d}
 \e^{-\ii\ip{\xi}{x}}\widehat f(\xi)\dd\xi.
\]
Thus $\widehat{D_gf}(\xi)=-\ii\ip{\xi}{g}\widehat f(\xi)$ and
$\widehat{D_g^{2m}f}=(-1)^m\ip{\xi}{g}^{2m}\widehat f$.
...
factor satisfying $|\ip{\xi}{g}|\ge c_m\norm\xi$.  Hence
\[
 |\Phi_{pL}(\xi)|\le C_p(1+\norm\xi)^{-p}.
\]
If $p>d+k$, then $\norm\xi^k\Phi_{pL}\in L^1$, so the corresponding finite box
