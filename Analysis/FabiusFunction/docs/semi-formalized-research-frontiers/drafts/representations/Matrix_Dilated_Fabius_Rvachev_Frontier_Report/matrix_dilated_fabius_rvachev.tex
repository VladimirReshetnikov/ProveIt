...
$\widehat f(\xi)=\int\EulerE^{\ImaginaryUnit\ip{\xi}{x}}f(x)\Differential x$, multiplication by
...
$\widehat S(k)=\int\EulerE^{-\ImaginaryUnit k\phi}\Differential S(\phi)$,
\begin{equation}
 \widehat S(k)
 =4a\cos\frac{k\pi}{2}\,
...
\[
 \widehat f(\xi)=\int_{\RealNumbers^d}\EulerE^{\ImaginaryUnit\ip{\xi}{x}}f(x)\Differential x,
 \qquad
 f(x)=\frac1{(2\pi)^d}\int_{\RealNumbers^d}
 \EulerE^{-\ImaginaryUnit\ip{\xi}{x}}\widehat f(\xi)\Differential\xi.
\]
Thus $\widehat{D_gf}(\xi)=-\ImaginaryUnit\ip{\xi}{g}\widehat f(\xi)$ and
