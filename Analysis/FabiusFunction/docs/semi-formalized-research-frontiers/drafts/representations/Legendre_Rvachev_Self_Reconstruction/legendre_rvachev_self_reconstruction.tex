...
At compiled source checkpoint \texttt{1f9ce6fd9}, the subsequent module
\path{RvachevMomentAppell.lean} exports five public definitions:
\path{rvachevRawMomentRat}, \path{rvachevReciprocalMomentRat},
\path{rvachevAppellPolynomialRat}, \path{rvachevAppellPolynomial}, and
\path{rvachevDeconvolvedPolynomial}.  Its sixteen public theorems are
...
inverse used below.  This closes the formal coefficient content behind
...
The new APIs do not prove minimality or sharpness of the displayed mesh;
reciprocal/deconvolution parity or the derivative, Gegenbauer, Appell, and
low-degree formulas for $Q_d$; rationality of the atom rows; a named outer
translate-block \texttt{HasSum} for \eqref{eq:block-loop}; the one-mesh
partial-sum identity \eqref{eq:fixed-scale}; or the later refinement,
...
\begin{equation}
 ...
 \label{eq:SN-CN}
\end{equation}
Since $\deg S_N=2N$, the whole partial sum can be synthesized on the single
finest grid $m_N=4^N$:
\begin{equation}
 \boxed{
...
\begin{enumerate}
 ...
 formula.  A named outer translate-block \texttt{HasSum} and the fixed-scale
 partial-sum synthesis remain outside these APIs.
