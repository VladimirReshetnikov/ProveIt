...
\texttt{HasSum} theorem and a \texttt{tsum} equality.  It does not formalize
the blocks' finite up-translate realization or atom-Gram formula, the
Fourier-product and infinite-sinc formulas \eqref{eq:A2-fourier} and
\eqref{eq:A2-sinc}, or the assertion that the partial sums are rational.
\end{statusbox}
...
\begin{enumerate}
 \item \textbf{Polynomial layer.}  Define $\Dscr$ on Lean polynomials by the
 terminating reciprocal-MGF differential sum.  Prove degree preservation,
 parity, and the Bell recurrence for $\beta_r$.
 \item \textbf{Legendre specialization.}  Instantiate the existing synthesis
 theorem with $p=P_d$, $m=2^d$, and interval $[-1,1]$.  Package
 \eqref{eq:Pd-synthesis} and the even specialization.
 ...
 are now formalized in \path{FabiusLegendreEnergy.lean}.  The finite
 up-translate realization and atom-Gram formula remain targets.
 \item \textbf{Energy layer.}  The real-variable equality
 \eqref{eq:A2-legendre} is now formalized.  The Fourier-product and
 infinite-sinc equalities \eqref{eq:A2-fourier}--\eqref{eq:A2-sinc} remain
 targets.
