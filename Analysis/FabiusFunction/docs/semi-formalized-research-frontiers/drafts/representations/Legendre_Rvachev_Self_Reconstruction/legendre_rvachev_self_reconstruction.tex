...
focused-build extension, the subsequent module
...
\path{rvachevDeconvolutionLinearMap}.  Its twenty-two public theorems are
...
multiplication, finite sums, and constant-polynomial multiplication.  This
...
\path{rvachevLegendreTranslateBlock}, and \path{rvachevTranslateGram}.  Its five
public theorems are \path{eval_legendrePolynomial_eq_sum_rvachevUp},
...
The focused-build module \path{FabiusLegendreTranslateSeries.lean} exports four
public definitions:
\path{rvachevLegendrePartialSumDeconvolutionPolynomial},
\path{rvachevLegendrePartialSumAtomCoefficient},
\path{rvachevLegendrePartialSumTranslateBlock}, and
\path{rvachevLegendreTranslateBlockOnInterval}.  Its sixteen public theorems
are \path{summable_norm_rvachevLegendreTranslateBlock},
...
\path{tsum_rvachevLegendreTranslateBlock_uniform}.  They close the pointwise
...
the sum of its separately scaled blocks as functions on $[-1,1]$.

The new APIs still do not prove minimality or sharpness of the displayed mesh;
reciprocal/deconvolution parity or the derivative, Gegenbauer, Appell, and
low-degree formulas for $Q_d$; rationality of the atom rows; equality of the
...
These are function equalities; they do not identify the two coefficient
vectors.  The displayed rationality assertion remains outside the current
API.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.91\textwidth]{energy_convergence.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.91\textwidth]{energy_terms.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{quarter_grid_null_stencil.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{block_cancellation_growth.pdf}
...
\begin{enumerate}
 \item \textbf{Polynomial layer.}  At checkpoint \texttt{1f9ce6fd9},
 ...
 deconvolution with exact smoothing recovery and a degree-nonincrease bound.
 The focused-build extension packages deconvolution as a real linear map and
 proves its zero, addition, scalar, finite-sum, and constant-multiple laws.
 ...
 outside that API.
 \item \textbf{Synthesis and Legendre specialization.}  At checkpoint
 \texttt{c51a41fcf}, \path{RvachevPolynomialSynthesis.lean} formalizes the
 ...
 and \eqref{eq:even-P-synthesis} at meshes $2^d$ and $4^n$.  Minimality and
 sharpness of those meshes and the closed derivative/Gegenbauer/Appell forms
 of $Q_d$ remain targets.
 ...
 formula.  The focused-build \path{FabiusLegendreTranslateSeries.lean} now
 ...
 mode expansion of $C_N=\Dscr S_N$, and both global and finite common-mesh
 synthesis at $4^N$.  Rationality of those atom rows, equality of the two
 coefficient vectors, and exact degree of every partial sum remain outside
 these APIs.
...
The interlevel identities and orthogonal block theorem should be relatively
short formal developments because their analytic content is already isolated
in existing repository theorems.  The only new complex-analytic lemma needed
for the sharp central law is the standard implication from geometric Legendre
coefficient decay to holomorphic extension in a Bernstein ellipse.
