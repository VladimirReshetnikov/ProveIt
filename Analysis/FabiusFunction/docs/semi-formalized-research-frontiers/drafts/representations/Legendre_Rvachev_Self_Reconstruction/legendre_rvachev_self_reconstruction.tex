...
$A_2=\int_0^1F(x)^2\Differential x$, then
\[
...
The lattices $4^n\IntegerNumbers^{-1}$ are nested.  Comparing two admissible lattices gives
...
\[
 \limsup_{n\to\infty}
 \frac{|c_n^{(0)}|^{1/n}}{n^2}=\frac1{\pi^2\EulerE^2}.
\]
...
\(d\le\TwoAdicValuation(M)\), equivalently \(2^d\mid M\); a monomial of degree
\(\TwoAdicValuation(M)+1\) fails at some real shift; and \(2^d\) is the least universally
...
$\ord_{\xi=m}\widehat u=1+\TwoAdicValuation(m)$, the MGF and cumulants, rational moments,
...
$\limsup |c_n^{(0)}|^{1/n}/n^2=(\pi^2\EulerE^2)^{-1}$.\\
\bottomrule
\end{tabularx}
...
Let $u=\RvachevUp$ be the unique even nonnegative $C^\infty$ function such that
\begin{equation}
 \SupportOperator u=[-1,1],\qquad u(0)=1,\qquad
 u'(x)=2u(2x+1)-2u(2x-1).
 \label{eq:up-dde}
\end{equation}
...
\begin{equation}
 \MM(z)=\Expectation e^{zZ}
 =\int_{-1}^{1}e^{zx}u(x)\Differential x
...
Let $p$ be a polynomial of degree at most $d$.  If $m\in\NaturalNumbers$ satisfies
$\TwoAdicValuation(m)\ge d$, put $p^\sharp=\Dscr p$.  Then, for $-1\le x\le1$,
\begin{equation}
 \boxed{
...
$\TwoAdicValuation(m)\ge d$ is sharp within this stationary Poisson-synthesis scheme.
\end{proof}
...
\begin{equation}
 \frac1{m_n}\IntegerNumbers\subset\frac1{m_{n+1}}\IntegerNumbers,
 \qquad m_{n+1}=4m_n.
...
$\mu_{2j}=\int x^{2j}u(x)\Differential x$, then
\begin{equation}
...
$\Norm{P_j}_{L^\infty[-1,1]}\le1$ and the imported expansion satisfies
...
and
\begin{equation}
 \Norm{u}_2^2=\sum_{n=0}^{\infty}\frac{2u_n^2}{4n+1}.
...
orthogonality gives
\[
 \int_{-1}^{1}P_{2n}(x)P_{2m}(x)\Differential x
...
\begin{equation}
 ...
 a_{n,k}a_{m,\ell}
 \int_{-1}^{1}u(x-k/m_n)u(x-\ell/m_m)\Differential x
 =\delta_{nm}\frac{2u_n^2}{4n+1}.}
 \label{eq:atom-gram}
\end{equation}
...
$\SpanOperator\{P_{2n}\}$ and admits the finite atom realization
\begin{equation}
 \boxed{
...
\[
 \Norm{u}_2^2=\int_{\RealNumbers}|\widehat u(\xi)|^2\Differential\xi.
\]
...
Let
\begin{equation}
 \mathcal Lf=-\frac{\Differential}{\Differential x}\left((1-x^2)f'(x)\right).
...
For $n,m\ge0$,
\begin{equation}
 \int_{-1}^{1}(1-x^2)B_n'(x)B_m'(x)\Differential x
...
In particular,
\begin{equation}
 \int_{-1}^{1}(1-x^2)|u'(x)|^2\Differential x
...
$rm$ are admissible, i.e. $\TwoAdicValuation(m),\TwoAdicValuation(rm)\ge d$.  Then
\begin{equation}
 \boxed{
 \frac1{rm}\sum_{|k|<2rm}
...
\begin{equation}
 ...
 \int_{-1}^{1}u(x-k/2^d)P_j(x)\Differential x
...
\begin{equation}
 E_N=\SpanOperator\{P_0,P_2,\ldots,P_{2N}\}.
 \label{eq:EN}
\end{equation}
...
$\tr\PiE_N=\RankOperator\PiE_N=N+1$, proving \eqref{eq:trace-identity}.  Expanding the
...
Let
\begin{equation}
 C=\MM(\pi\ImaginaryUnit)
 =\prod_{j=1}^{\infty}\frac{\sin(\pi/2^j)}{\pi/2^j}.
 \label{eq:C-constant}
\end{equation}
The nearest poles of $1/\MM$ are $\pm2\pi\ImaginaryUnit$.  The recent repository loop
report derives
\begin{equation}
 Q_{2n}(0)=
...
\begin{equation}
 ...
 \qquad
 \frac{\AbsoluteValue{Q_{2n}(0)}^{1/n}}{n^2}\to\frac{4}{\pi^2\EulerE^2}.
 \label{eq:Q0-root}
\end{equation}
...
\]
The second factor tends to $(\pi^2\EulerE^2)^{-1}$ by
...
$L^2$ norm
\begin{equation}
 \Norm{B_n}_2=|u_n|\sqrt{\frac{2}{4n+1}}\longrightarrow0.
...
$|Q_{2n}(0)|\asymp(2n/(\pi\EulerE))^{2n}$ for the first index at which it exceeds
$X$ gives the imported threshold
\begin{equation}
 n_Q(X)=\frac{\log X}
 {2W\!\left(\log X/(\pi\EulerE)\right)}+\BigO(1).
...
Extend the finite train $B_n$ from \eqref{eq:block-def} to all of $\RealNumbers$ by the
same formula and call it $\widetilde B_n$.  Although
$\widetilde B_n=u_nP_{2n}$ on $[-1,1]$, conjecturally
\begin{equation}
 \frac{\Norm{\widetilde B_n}_{L^2(\RealNumbers\setminus[-1,1])}}
 {\Norm{B_n}_{L^2[-1,1]}}\longrightarrow\infty.
...
samples the resulting polynomial $Q_d$ at the grid $2^{-d}\IntegerNumbers$, and attaches
those samples to finitely many literal translates of $u$.  For the even modes
in the canonical expansion, the grids are $4^{-n}\IntegerNumbers$ and nest by quarter-grid
refinement.
