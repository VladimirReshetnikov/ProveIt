...
\begin{abstract}
Let $u=\Up$ be Rvachev's even, compactly supported $C^\infty$ function on
...
$\e^{xz}/M_u(z)=\sum A_n(x)z^n/n!$.  We derive mutually inverse triangular
matrices
\[
...
We use $u=\Up$ normalized by
\begin{equation}
 u\in C_c^\infty(\R),\qquad
 \supp u=[-1,1],\qquad u(-x)=u(x),\qquad
 \int_{-1}^{1}u(x)\dd x=1.
 \label{eq:up-basic}
\end{equation}
With Fourier convention
$\widehat f(\xi)=\int_\R f(x)\e^{-2\pi\ii x\xi}\dd x$, the transform is
\begin{equation}
...
\begin{equation}
 \operatorname{ord}_{\xi=m}\widehat u(\xi)=1+\vtwo(m).
...
\end{equation}
If $P\in\Q[t]$, then $b_r\in\Q$.  Moreover, $d+2$ is the least possible
...
polynomiality hyperplane $\tau_d^\perp\subset\Q^{d+2}$.
\end{theorem}
...
\begin{center}
\input{generated/sign_evidence.tex}
\end{center}
...
Define $A_n$ by
\begin{equation}
 \frac{\e^{xz}}{M_u(z)}=\sum_{n=0}^{\infty}A_n(x)\frac{z^n}{n!}.
 \label{eq:Appell-egf}
\end{equation}
Then
\begin{equation}
...
\small
\input{generated/connection_matrices.tex}
\end{minipage}
\end{center}
...
\begin{equation}
 ...
 \int_{-1}^{1}\Psi_{d,r}(x)P_n(x)\dd x,
 \qquad0\le n\le d.
 \label{eq:L-matrix}
\end{equation}
...
The matrix $\cQ_d\in\R^{(d+2)\times(d+2)}$ is invertible.  If
...
\begin{equation}
 ...
 (\mathsf L_d)_{n,r}
 =(2n+1)\int_{a/M}^{(a+1)/M}
 F(s)P_n\!\left(2Ms-(2a+1)\right)\dd s.}
...
$\dd x=2M\dd y$ leave a factor $2$, which cancels the $1/2$ in the Legendre
...
\resizebox{\textwidth}{!}{\input{generated/fixed_scale_summary.tex}}
\end{table}
...
\begin{equation}
 \alpha_n=\frac{2n+1}{2}\int_{-1}^{1}f(x)P_n(x)\dd x,
 \qquad
...
even moment series $M(z)$, define its Appell family by $\e^{xz}/M(z)$.  The
...
\begin{equation}
 ...
 \kappa_2(\cQ_d),
 \quad\text{or}\quad
 \max_r\operatorname{dist}(\supp\Psi_{d,r},[-1,1]).
...
\begin{align*}
 ...
 \frac1{M_u(z)}&=\sum_{n\ge0}\gamma_n\frac{z^n}{n!},
 &\frac{\e^{xz}}{M_u(z)}&=\sum_{n\ge0}A_n(x)\frac{z^n}{n!}.
\end{align*}
