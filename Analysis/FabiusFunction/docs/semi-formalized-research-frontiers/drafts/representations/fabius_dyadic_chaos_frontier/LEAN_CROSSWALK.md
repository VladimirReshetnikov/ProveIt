...
The activation evidence in row 8 and rows 37--41 tracks the source milestone
`a345425d21d90e680bf15e34093af42c69c08a83`.  This current provenance
...
| 8 | `lem:p-bounds` — lemma | Activation bounds | P | **Complete** | `FabiusFunction.HyperbolicActivation` defines the totalized `activationProbability`, proves its value at zero and the off-zero identity `activationProbability_of_ne_zero`, continuity and evenness, strict positivity exactly away from zero, the global bounds `activationProbability_le_min_one_sq_div_three` and `activationProbability_lt_one`, and the complementary positive-argument bound `one_sub_activationProbability_le_min_one_inv_of_pos`.  Its exact elementary proof API separates the hypotheses: for `0 ≤ x`, `Fabius.tanh_nonneg_of_nonneg`, `Fabius.tanh_le_self_of_nonneg`, and `Fabius.tanh_cubic_lower` prove `0 ≤ tanh x`, `tanh x ≤ x`, and `x - x^3/3 ≤ tanh x`, while `Fabius.activationProbability_le_sq_div_three_of_nonneg` gives the half-line quadratic bound; for `0 < x`, `Fabius.tanh_pos_of_pos`, `Fabius.tanh_lt_self_of_pos`, `Fabius.tanhDiv_le_inv_of_pos`, and `Fabius.one_sub_activationProbability_le_inv_of_pos` give the strict tangent bounds and the two `x⁻¹` bounds.  The theorem `Fabius.activationProbability_le_sq_div_three` extends the quadratic estimate to every real `x`.  The two inverse bounds are asserted only for positive arguments, not at zero or on the negative half-line.  The identity `Fabius.tanhDiv_eq_dslope` identifies the removable quotient with the divided slope of `Real.tanh`.  In `FabiusFunction.ActivationTaylor`, `Fabius.tanh_sub_taylor_nine_isBigO` proves the degree-nine hyperbolic-tangent jet with an `O(||x||^11)` remainder; `Fabius.activationProbability_sub_taylor_eight_isBigO` transfers it to the report's activation polynomial with an `O(||x||^10)` remainder; and `Fabius.activationProbability_sub_taylor_eight_isBigO_pow` gives the literal human-facing `O(x^10)` form.  Together with the already formalized bounds, these declarations cover every clause of the report lemma under no stronger hypotheses.  They prove only this finite jet and do not assert an all-order coefficient formula or a radius-of-convergence theorem. |
...
| 37 | `thm:totalized-activation-dictionary` — theorem | Totalized activation dictionary | P | **Complete** | `FabiusFunction.HyperbolicActivation` supplies the literal real-valued statement.  The unconditional origin laws are `Fabius.realSinhc_zero`, `Fabius.tanhDiv_zero`, `Fabius.activationOdds_zero`, and `Fabius.activationProbability_zero`; the quotient laws `Fabius.realSinhc_of_ne_zero` and `Fabius.tanhDiv_of_ne_zero` assume exactly `x ≠ 0`.  Global continuity is `Fabius.continuous_realSinhc`, `Fabius.continuous_tanhDiv`, and `Fabius.continuous_activationOdds`.  Pointwise negation and bundled evenness are respectively `Fabius.realSinhc_neg` / `Fabius.realSinhc_even`, `Fabius.tanhDiv_neg` / `Fabius.tanhDiv_even`, and `Fabius.activationOdds_neg` / `Fabius.activationOdds_even`; `Fabius.activationProbability_neg` is the pointwise probability law.  In addition to the report's clauses, `Fabius.realSinhc_eq_dslope` canonically identifies the totalized real hyperbolic sinc with the global function `dslope Real.sinh 0`; `Fabius.tanhDiv_eq_dslope` gives the corresponding identification for `Real.tanh`.  The unconditional sinc order API is `Fabius.realSinhc_pos`, `Fabius.realSinhc_ne_zero`, `Fabius.realSinhc_two_mul`, `Fabius.realSinhc_le_cosh`, and `Fabius.realSinhc_lt_cosh_iff`; `Fabius.sinh_lt_mul_cosh` and its specialization `Fabius.realSinhc_lt_cosh_of_pos` assume exactly `0 < x`.  The unconditional quotient order API is `Fabius.tanhDiv_pos`, `Fabius.tanhDiv_ne_zero`, `Fabius.tanhDiv_le_one`, and `Fabius.tanhDiv_lt_one_iff`.  The cancellation law `Fabius.tanhDiv_mul_one_add_activationOdds`, the complement identity `Fabius.one_sub_activationProbability`, and its global positivity and unit bound `Fabius.one_sub_activationProbability_pos` and `Fabius.one_sub_activationProbability_le_one` have no side condition.  The remaining exact odds declarations are `one_add_activationOdds`, `one_add_activationOdds_pos`, `activationOdds_of_ne_zero`, `activationOdds_nonneg`, and `activationOdds_pos_iff`, together with the global odds/probability bridge, sign, and off-zero quotient declarations.  This is a real totalized-kernel API: it does not identify `realSinhc` with a complex sinc function, construct a random variable, prove analyticity, or assert an all-order Taylor series. |
...
| 39 | `prop:geometric-activation-dimension` — proposition | Geometric activation dimension | P | **Complete** | `FabiusFunction.GeometricActivationDimension` proves the unconditional termwise estimate `Fabius.activationProbability_pow_mul_le` for every real `q,t` and natural `n`.  Exactly under `|q| < 1`, `Fabius.summable_activationProbability_pow_mul` gives genuine summability and `Fabius.tsum_activationProbability_pow_mul_le` bounds the sum by `(t^2/3) * (1-q^2)⁻¹`.  The laws `Fabius.geometricActivationDimension_zero` and `Fabius.geometricActivationDimension_nonneg` are unconditional in real `q` (and in `t` for nonnegativity), while `Fabius.geometricActivationDimension_sub_refinement`, `Fabius.activationProbability_scale_le_geometricActivationDimension`, and the unsimplified budget `Fabius.geometricActivationDimension_le_quadratic` assume exactly `|q| < 1`.  The specialization `Fabius.geometricActivationDimension_zero_ratio` holds for every real `t`, and `Fabius.dyadicEffectiveDimension_zero` has no side condition.  The module also proves `hasSum_normalizedGeometricWeight_sq`, the even law, positivity exactly off zero, the simplified geometric budget, one-step and finite-prefix refinements, certified prefix enclosures, and all stated dyadic specializations.  `Fabius.tendsto_geometricActivationDimension_div_sq` and `Fabius.tendsto_dyadicEffectiveDimension_div_sq` in `FabiusFunction.GeometricActivationAsymptotics` prove the exact punctured coefficient `(1-q)/(3*(1+q))` and its dyadic specialization `1/9`.  The definition remains total outside `|q| < 1` through Lean's totalized `tsum`, but no general summability, refinement, or quadratic budget is claimed there.  For negative `q`, this activation result is only a deterministic signed-weight series theorem; the report's positive-weight active-count interpretation remains restricted to `0 < q < 1`, and no Bernoulli construction or equality with `E K_t` is claimed by this result. |
...
1. **Finish the local analytic series.**  The compiler-validated
   `HyperbolicActivation`, `ActivationTaylor`, `ActivationSeries`,
   `ActivationAsymptotics`, `ActivationSeriesAsymptotics`,
   `GeometricActivationDimension`, and `GeometricActivationAsymptotics`
   modules now provide totalized `realSinhc`, `tanhDiv`, odds and probability,
   all elementary activation bounds, the report's finite Taylor jet, the
   ...
   geometric tails, and sharp geometric/dyadic coefficients.  The remaining
   local analytic milestone is the all-order Bernoulli coefficient series and
   its exact convergence-radius statements.  That work closes the analytic
