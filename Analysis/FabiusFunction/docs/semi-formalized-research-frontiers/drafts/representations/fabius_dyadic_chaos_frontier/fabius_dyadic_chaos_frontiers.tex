...
% LOCAL-NON-FOURIER-HAT: \OmittedSymmetricArgument denotes the variable deleted from a symmetric-polynomial argument list
\newcommand{\OmittedSymmetricArgument}[1]{\widehat{#1}}
\newcommand{\InteractionPowerSum}[1]{s_{#1}}
...
\]
we prove an explicit infinite Hoeffding decomposition of $\EulerE^{tX}$.  The
...
The activation analysis also extends beyond the dyadic lattice.  For an
arbitrary square-summable real weight family $(w_i)_{i\in I}$, including
signed and zero weights, we prove genuine summability, the global budget
\[
 \sum_{i\in I}p(w_it)\le \frac{t^2}{3}\sum_{i\in I}w_i^2,
\]
and the exact punctured small-field limit obtained by Tannery's theorem.  The
normalized geometric family then has sharp coefficient
$(1-q)/(3(1+q))$ for $|q|<1$, with dyadic value $1/9$.
...
by a central limit theorem: after centering by $\Floor{\log_2t}$ it
...
\begin{enumerate}
\item an exact infinite Hoeffding decomposition of $\EulerE^{tX}$ and all component
      norms;
\item a Poisson--binomial law for the active set and exact first/total Sobol
      indices;
\item a square-summable activation-series theorem with a global quadratic
      budget and its exact Tannery limit;
...
\begin{equation}
 ...
 \label{eq:X-centered}
\end{equation}
Then $X$ has Rvachev density $\RvachevUp$ on $[-1,1]$, and
\begin{equation}
 \RvachevUp(x)=F(1-|x|),\qquad F(y)=\RvachevUp(y-1),\qquad F'(y)=2\RvachevUp(2y-1)
 \quad(0\le y\le1)
...
For $x\in\ComplexNumbers$, define
\begin{equation}
...
\begin{equation}
 M(t)=\Expectation\EulerE^{tX}=\prod_{j\ge1}m(t2^{-j}).
 \label{eq:M-product}
\end{equation}
...
\end{equation}
On the imaginary axis, $M(2\pi\ImaginaryUnit z)$ becomes the repository's normalized
...
\begin{equation}
 \Expectation X^n=\ExponentialCompleteBellPolynomial{n}(\kappa_1,\ldots,\kappa_n),
 \label{eq:Bell-moments}
\end{equation}
with $\ExponentialCompleteBellPolynomial{n}$ the complete exponential Bell polynomial.  These formulae are
...
Let $\Omega=[-1,1]^{\NaturalNumbers}$ with product probability measure
$\mu=\bigotimes_{j\ge1}(\Differential v_j/2)$.  For each coordinate, let
\begin{equation}
 E_jf=\int_{-1}^1f(v_1,\ldots,v_{j-1},u,v_{j+1},\ldots)\frac{\Differential u}{2},
 \qquad Q_j=I-E_j.
 \label{eq:coordinate-projections}
\end{equation}
For a finite set $S\subset\NaturalNumbers$, let
\(\DyadicSigmaField_S=\sigma(V_j:j\in S)\) and put
\begin{equation}
 Q_S=\prod_{j\in S}Q_j,
 \qquad E_{S^c}f=\Expectation[f\mid\DyadicSigmaField_S],
...
expansion
\begin{equation}
 f=\sum_{S\Subset\NaturalNumbers}f_S,
 \qquad
 \|f\|_2^2=\sum_{S\Subset\NaturalNumbers}\|f_S\|_2^2,
 \label{eq:Hoeffding-general}
\end{equation}
for $f\in L^2(\mu)$, with convergence in $L^2$.  Here $S\Subset\NaturalNumbers$ means that
$S$ is finite.  The empty component is $f_\varnothing=\Expectation f$.
...
Fix $t\in\RealNumbers$ and write
\begin{equation}
 x_j=ta_j=t2^{-j},
 \qquad
 Z_j(t)=\frac{\EulerE^{x_jV_j}}{m(x_j)}-1.
 \label{eq:Z-local}
\end{equation}
Then $\Expectation Z_j(t)=0$ and
\begin{equation}
 \EulerE^{tX}=M(t)\prod_{j\ge1}\bigl(1+Z_j(t)\bigr).
...
For every finite $S\subset\NaturalNumbers$, the Hoeffding component of $\EulerE^{tX}$ indexed by
$S$ is
\begin{equation}
...
The series
\begin{equation}
 \EulerE^{tX}=\sum_{S\Subset\NaturalNumbers}H_S(t)
...
\begin{proof}
Let $\DyadicSigmaField_n=\sigma(V_1,\ldots,V_n)$ and
\[
...
normalization, is
\[
 f_n:=\Expectation[\EulerE^{tX}\mid\DyadicSigmaField_n]
 =M_{>n}(t)\EulerE^{t\sum_{j=1}^na_jV_j}
...
\[
 M(t)^2\sum_{S\Subset\NaturalNumbers}\prod_{j\in S}r(x_j)
...
$\EulerE^{tX}$ by the $L^2$ martingale convergence theorem (and also pointwise by
...
Consequently
\begin{equation}
 \Variance(\EulerE^{tX})=M(t)^2\bigl(R(t)-1\bigr).
...
\begin{equation}
 A_k(t)=\sum_{\substack{S\Subset\NaturalNumbers\\|S|=k}}
            \prod_{j\in S}r(t2^{-j}),
 \qquad A_0(t)=1,
 \label{eq:A-k-def}
\end{equation}
...
\begin{equation}
 A_1(t)=\sum_{j\ge1}r(t2^{-j})
       =t\frac{\Differential}{\Differential t}\log M(t).
 \label{eq:A1-logM}
\end{equation}
\end{proposition}
...
\[
 x\frac{\Differential}{\Differential x}\log m(x)=x\coth x-1=r(x)\qquad(x\ne0),
\]
...
Put
\begin{equation}
 \InteractionPowerSum{\ell}(t)=\sum_{j\ge1}r(t2^{-j})^\ell,
 \qquad \ell\ge1.
 \label{eq:power-sums}
\end{equation}
...
\begin{equation}
 \boxed{
 A_k(t)=\frac1{k!}\ExponentialCompleteBellPolynomial{k}\bigl(\InteractionPowerSum{1}(t),-1!\InteractionPowerSum{2}(t),
 \ldots,(-1)^{k-1}(k-1)!\InteractionPowerSum{k}(t)\bigr).}
 \label{eq:A-Bell}
\end{equation}
The power sums obey
\begin{equation}
 \InteractionPowerSum{\ell}(2t)=\InteractionPowerSum{\ell}(t)+r(t)^\ell.
...
For every finite set $S\subset\NaturalNumbers$,
\begin{equation}
 \frac{\|H_S(t)\|_2^2}{\Expectation\EulerE^{2tX}}
 =\Probability\bigl(\{j:B_j(t)=1\}=S\bigr).
 \label{eq:energy-active-set}
\end{equation}
For $t\ne0$, the normalized nonconstant Hoeffding energy is therefore
\begin{equation}
 \boxed{
 \frac{\|H_S(t)\|_2^2}{\Variance(\EulerE^{tX})}
 =\Probability\bigl(\{j:B_j(t)=1\}=S\mid K_t>0\bigr),
 \quad S\ne\varnothing.}
 \label{eq:variance-active-conditioned}
\end{equation}
In particular, the order-$k$ variance share is
\begin{equation}
 \frac{M(t)^2A_k(t)}{\Variance(\EulerE^{tX})}
 =\Probability(K_t=k\mid K_t>0).
 \label{eq:order-law}
\end{equation}
\end{theorem}
...
\[
 \Probability(\{j:B_j=1\}=S)
 =\prod_{j\in S}\frac{r_j}{1+r_j}
  \prod_{j\notin S}\frac1{1+r_j}
 =\frac{\prod_{j\in S}r_j}{R(t)}.
\]
Use $\Expectation\EulerE^{2tX}=M(t)^2R(t)$ and \eqref{eq:H-S-norm}.  Conditioning away the
...
Equivalently,
\begin{equation}
 R(t)-1=\chi^2(\Probability_t\|\Probability),
 \qquad
 \frac{\Differential\Probability_t}{\Differential\Probability}=\frac{\EulerE^{tX}}{M(t)},
 \label{eq:chi-square}
\end{equation}
...
\[
 \Expectation\left(\frac{\EulerE^{tX}}{M(t)}-1\right)^2
...
For $f_t=\EulerE^{tX}$, define the first-order and total-effect Sobol indices by
\begin{equation}
 S_j^{(1)}(t)=\frac{\|H_{\{j\}}(t)\|_2^2}{\Variance(f_t)},
 \qquad
 S_j^{\mathrm T}(t)=
 \frac{\sum_{S\ni j}\|H_S(t)\|_2^2}{\Variance(f_t)}.
 \label{eq:Sobol-def}
\end{equation}
...
\begin{equation}
 ...
 \qquad
 S_j^{\mathrm T}(t)=
 \frac{p(t2^{-j})}{1-\Probability(K_t=0)}.}
 \label{eq:Sobol-closed}
\end{equation}
More generally, if $\DyadicSigmaField_m=\sigma(V_1,\ldots,V_m)$ and
\begin{equation}
...
then
\begin{align}
 \bigl\|\Expectation[f_t\mid\DyadicSigmaField_m]\bigr\|_2^2&=M(t)^2R_m(t),
 \label{eq:projection-norm}\\
 \bigl\|f_t-\Expectation[f_t\mid\DyadicSigmaField_m]\bigr\|_2^2
...
\[
 ...
 =r_j\prod_{i\ne j}(1+r_i)=p_jR.
\]
Divide by $R-1$ and use $1-\Probability(K_t=0)=(R-1)/R$.
...
$\Probability(K_t=k\mid K_t>0)$ for several values of $t$.  As $t$ increases, the
...
\end{equation}
More generally, every real dilation $a$, including $a=0$, satisfies
\begin{equation}
 \boxed{
 \lim_{\substack{t\to0\\t\ne0}}\frac{p(at)}{t^2}=\frac{a^2}{3}.}
 \label{eq:scaled-sharp-activation-quadratic}
\end{equation}
\end{corollary}
...
For $a\ne0$, compose this punctured limit with the dilation $t\mapsto at$
and multiply by $a^2$; for $a=0$, both sides vanish identically.
\end{proof}

\begin{theorem}[Square-summable activation series; \statusP]
\label{thm:square-summable-activation-series}
Let $I$ be an arbitrary index set and let $(w_i)_{i\in I}$ be a real family
such that $\sum_{i\in I}w_i^2<\infty$, where the sum is the unconditional
nonnegative sum.  The weights may be signed or zero.  For all $a,t\in\RealNumbers$,
\begin{equation}
 0\le p(at)\le \frac{t^2}{3}a^2.
 \label{eq:scaled-activation-budget}
\end{equation}
For every $t\in\RealNumbers$, the activation family is genuinely summable and
\begin{equation}
 0\le D_w(t):=\sum_{i\in I}p(w_it)
 \le \frac{t^2}{3}\sum_{i\in I}w_i^2.
 \label{eq:square-summable-activation-budget}
\end{equation}
Moreover, its quadratic budget is coefficient-sharp:
\begin{equation}
 \boxed{
 \lim_{\substack{t\to0\\t\ne0}}\frac{D_w(t)}{t^2}
 =\frac13\sum_{i\in I}w_i^2.}
 \label{eq:square-summable-activation-limit}
\end{equation}
\end{theorem}

\begin{proof}
Evenness extends the quadratic estimate in \cref{lem:p-bounds} from
$x\ge0$ to every real $x$; substituting $x=at$ gives
\eqref{eq:scaled-activation-budget}.  Its right-hand side is summable in $i$,
so comparison proves both genuine summability and
\eqref{eq:square-summable-activation-budget}.  For $t\ne0$, divide the
pointwise estimate by $t^2$ to obtain the summable majorant $w_i^2/3$.
The scaled limit \eqref{eq:scaled-sharp-activation-quadratic} gives
\[
 \frac{p(w_it)}{t^2}\longrightarrow\frac{w_i^2}{3}
 \qquad (t\to0,\ t\ne0)
\]
for every $i$, including $w_i=0$.  Tannery's theorem now interchanges the
punctured limit and the unconditional sum, proving
\eqref{eq:square-summable-activation-limit}.  Thus no finite-support
approximation or Taylor remainder is required.
\end{proof}
...
For all $t>0$,
\begin{equation}
 \Variance(K_t)=\sum_{j\ge1}p(t2^{-j})\bigl(1-p(t2^{-j})\bigr)
 \le\frac{22}{9}.
...
$\Variance(K_t)\le\sum_jt^2/(3\cdot4^j)=t^2/9<1/9$.
For $t\ge1$, put $m=\Floor{\log_2t}$.  On $j\le m$ use
...
$L^2([-1,1],\Differential v/2)$.  Define the modified spherical-Bessel coefficient
\begin{equation}
 i_n(x)=\frac12\int_{-1}^1\EulerE^{xv}P_n(v)\Differential v.
 \label{eq:i-n}
\end{equation}
Thus $i_0(x)=m(x)$ and the classical plane-wave/Legendre expansion reads
\begin{equation}
 \EulerE^{xv}=\sum_{n\ge0}(2n+1)i_n(x)P_n(v)
        =\sum_{n\ge0}\sqrt{2n+1}\,i_n(x)\ell_n(v).
 \label{eq:local-Legendre-expansion}
\end{equation}

For a finitely supported multi-index
$\alpha=(\alpha_1,\alpha_2,\ldots)\in\NaturalNumbers^{(\NaturalNumbers)}$, put
\begin{equation}
 \ell_\alpha(V)=\prod_{j\ge1}\ell_{\alpha_j}(V_j),
 \qquad \SupportOf{\alpha}=\{j:\alpha_j>0\}.
...
\[
 m(2x)=\frac12\int_{-1}^1\EulerE^{2xv}\Differential v
...
The expansion
\begin{equation}
 \EulerE^{tX}=\sum_{\alpha\in\NaturalNumbers^{(\NaturalNumbers)}}c_\alpha(t)\ell_\alpha(V)
...
$\Probability(N_j=n\mid B_j=1)=q_n(x_j)$, conditioned on at least one activation.
...
\[
 f_N=\Expectation[\EulerE^{tX}\mid\DyadicSigmaField_N]
 =M_{>N}(t)\prod_{j=1}^N\EulerE^{x_jV_j}.
\]
...
limit.  Since
$M_{>N}(t)\prod_{j=1}^Ni_0(x_j)=M(t)$, its coefficient for every $\alpha$ with
$\SupportOf{\alpha}\subset\{1,\ldots,N\}$ is exactly \eqref{eq:c-nu}.

The martingale $f_N$ converges in $L^2$ to $\EulerE^{tX}$.  The nested finite tensor
bases therefore yield \eqref{eq:tensor-expansion} and show that the coefficient
squares sum to $\|\EulerE^{tX}\|_2^2=M(2t)$.  A tensor coefficient lies in the
Hoeffding subspace indexed by $S$ exactly when $\SupportOf{\alpha}=S$; Parseval inside
...
\begin{equation}
 \frac1{M(2t)}
 \sum_{\alpha}|c_\alpha(t)|^2
 u^{\CardinalityOf{\SupportOf{\alpha}}}z^{\Norm{\alpha}_1}
...
much more general nonlinear observables.  Let $g\colon[-1,1]\to\RealNumbers$ and
...
\begin{equation}
 \Expectation\bigl|\Delta_Sh(V_S,V'_S)\bigr|^2
...
For one coordinate,
\[
 \Expectation|h(V)-h(V')|^2=2\Expectation|h-\Expectation h|^2=2\|Qh\|_2^2.
\]
...
Let $S\Subset\NaturalNumbers$ with $|S|=k$, and suppose $g\in C^k([-1,1])$.  Then
\begin{equation}
 \boxed{
 \|g_S\|_2^2
 \le \frac{\|g^{(k)}\|_\infty^2}{3^k}
...
\begin{equation}
 \Ecal_k(g)=\sum_{\substack{S\Subset\NaturalNumbers\\|S|=k}}\|g_S\|_2^2
...
Set
\[
 h_S(v_S)=\Expectation\left[g(X)\mid V_S=v_S\right].
\]
...
\begin{equation}
 ...
 \Expectation\left[g^{(k)}(X)\mid V_S=v_S\right].
...
\begin{align*}
 \|g_S\|_2^2
 &\le2^{-k}\|g^{(k)}\|_\infty^2
       \prod_{j\in S}a_j^2\Expectation(V_j-V'_j)^2\\
 &=2^{-k}\|g^{(k)}\|_\infty^2
...
degree greater than $d$.  Since $\Expectation V_j=0$, this multilinear term is already
pure in every coordinate.  Its squared norm is
$(d!)^2\prod_{j\in S}a_j^2\Expectation V_j^2=(d!)^2 3^{-d}\prod a_j^2$.
...
\begin{equation}
 X_{S^c}=\sum_{j\notin S}a_jV_j,
 \qquad
 m_r^{(S^c)}=\Expectation X_{S^c}^r,
 \qquad
 \mu_r=\Expectation V_1^r=\frac{1+(-1)^r}{2(r+1)}.
...
\begin{equation}
 m_r^{(S^c)}
 =\ExponentialCompleteBellPolynomial{r}\bigl(\kappa_1^{(S^c)},\ldots,\kappa_r^{(S^c)}\bigr),
 \qquad
...
\[
 g_{\{j\}}
 =a_j^3V_j^3+3a_jV_j\Variance(X_{\{j\}^c}),
\]
...
\[
 T_k=\sum_jq_j^2e_{k-1}(q_1,\ldots,\OmittedSymmetricArgument{q_j},\ldots).
\]
...
\begin{align}
 ...
 R(t)
 &=\sum_{n\ge0}\ExponentialCompleteBellPolynomial{n}(\lambda_1,\ldots,\lambda_n)
                   \frac{t^n}{n!},
 \qquad |t|<4\pi,
 \label{eq:R-Bell}\\
 R(t)-1
 &=\sum_{m\ge1}\ExponentialCompleteBellPolynomial{2m}(0,\lambda_2,0,\lambda_4,\ldots)
                   \frac{t^{2m}}{(2m)!},
 \qquad |t|<4\pi.
...
$R(t)=m(t)/M(t)$ at $t=\pm\pi\ImaginaryUnit$, whereas the apparent singularities of $R$
at $t=\pm2\pi\ImaginaryUnit$ are removable: the simple zeros of $m(t)$ and $M(t)$
cancel.  At $t=\pm4\pi\ImaginaryUnit$, $M(t)$ has two zero factors while $m(t)$ has only
...
\begin{equation}
 \mu(t)=\Expectation K_t=\sum_{j\ge1}p(t2^{-j}).
 \label{eq:mu-def}
\end{equation}
...
Let $q,t\in\RealNumbers$ with $|q|<1$, and define
\begin{equation}
...
\begin{align}
 \sum_{n\ge0}\bigl(q^n(1-q)\bigr)^2
 &=\frac{1-q}{1+q},
 \label{eq:normalized-geometric-weight-square-sum}\\
 d_q(t)
...
\end{align}
Its exact punctured quadratic coefficient is
\begin{equation}
 \boxed{
 \lim_{\substack{t\to0\\t\ne0}}\frac{d_q(t)}{t^2}
 =\frac{1-q}{3(1+q)}.}
 \label{eq:geometric-activation-sharp}
\end{equation}
...
\begin{equation}
 ...
 \label{eq:dyadic-effective-dimension-sharp}
\end{equation}
\end{proposition}
...
constant.  The same geometric-series calculation gives the genuine
\emph{HasSum} evaluation
\[
 \sum_{n\ge0}\bigl(q^n(1-q)\bigr)^2
 =(1-q)^2\sum_{n\ge0}(q^2)^n
 =\frac{1-q}{1+q}.
\]
Apply \cref{thm:square-summable-activation-series} to the weights
$w_n=q^n(1-q)$ and this exact sum to obtain
\eqref{eq:geometric-activation-sharp}; $q=1/2$ gives the coefficient $1/9$.
Removing the zeroth term gives the refinement, while removing the
...
$\mu(t)=\Expectation K_t=\sum_{j\ge1}p(t2^{-j})$ still depends on the active-count
...
\begin{equation}
 \Mcal_p(s)=\int_0^\infty p(x)x^{s-1}\Differential x,
 \qquad
 \Ical_p(s)=\int_0^\infty p'(x)x^s\Differential x.
...
$p'(x)=O(x)$ at zero and $p'(x)=x^{-2}+O(\EulerE^{-2x}/x)$ at infinity, giving
$-2<\Re s<1$ for $\Ical_p$.

Put
$J(s)=\int_0^\infty x^{s-1}\sech^2x\Differential x$, which is holomorphic on the
half-plane $\Re s>0$.  The exponential expansion
\[
 \sech^2x=4\sum_{n\ge1}(-1)^{n-1}n\EulerE^{-2nx}
\]
...
For $\theta\in\RealNumbers$, define the absolutely convergent renormalized sum
\begin{equation}
 P(\theta)=
...
\label{thm:mu-phase}
Let $t=2^{n+\theta}$ with $n\in\IntegerNumbers$, $0\le\theta<1$, and $t\ge1$.  Then
\begin{equation}
 \boxed{
...
With the convention
\begin{equation}
 Q(\theta)=\sum_{n\in\IntegerNumbers}\FourierSeriesCoefficient{Q}{n}\EulerE^{2\pi\ImaginaryUnit n\theta},
 \label{eq:Q-Fourier-series}
\end{equation}
one has
\begin{align}
 \boxed{
 \FourierSeriesCoefficient{Q}{0}
 =\frac{11}{6}+\frac{\gamma-12\log A}{\log2},}
 \label{eq:Q0}\\
 \boxed{
 \FourierSeriesCoefficient{Q}{n}
 =\frac{\Mcal_p^{\mathrm{cont}}(-\ImaginaryUnit\tau_n)}{\log2}
 =\frac{\Ical_p(-\ImaginaryUnit\tau_n)}{2\pi\ImaginaryUnit n},
 \qquad n\ne0.}
 \label{eq:Qn}
\end{align}
...
Numerically,
\begin{align}
 \FourierSeriesCoefficient{Q}{0}&=-1.6404426943766166621038729\ldots,
 \label{eq:Q0-num}\\
 \FourierSeriesCoefficient{Q}{1}&=
 (1.7087561142062823+1.3093920503208334\,\ImaginaryUnit)10^{-6},
 \label{eq:Q1-num}\\
 2|\FourierSeriesCoefficient{Q}{1}|&=4.3055104223684092\times10^{-6}.
...
\begin{align*}
 \FourierSeriesCoefficient{Q'}{n}
 &=\int_0^1L\sum_{k\in\IntegerNumbers}2^{\theta-k}
   p'(2^{\theta-k})\EulerE^{-2\pi\ImaginaryUnit n\theta}\Differential\theta\\
 &=\int_{-\infty}^{\infty}\EulerE^yp'(\EulerE^y)
   \EulerE^{-\ImaginaryUnit\tau_ny}\Differential y
 =\int_0^\infty p'(x)x^{-\ImaginaryUnit\tau_n}\Differential x
 =\Ical_p(-\ImaginaryUnit\tau_n).
\end{align*}
Since $\FourierSeriesCoefficient{Q'}{n}=2\pi\ImaginaryUnit n\FourierSeriesCoefficient{Q}{n}$, the nonzero formula follows
from \cref{thm:Mellin-p}.

For the mean, the Mellin transform of the harmonic sum is
\begin{equation}
 \int_0^\infty\mu(t)t^{s-1}\Differential t
...
Finally, Stirling's estimate for $\Gamma(-1-\ImaginaryUnit\tau_n)$ in
...
For $\theta\in[0,1)$, let
\begin{equation}
 p_{\theta,k}=p(2^{\theta-k}),\qquad k\in\IntegerNumbers,
 \label{eq:p-theta-k}
\end{equation}
...
\begin{equation}
 \boxed{
 \TotalVariationDistance\bigl(K_t-n,K_\theta\bigr)\le\frac2t.}
 \label{eq:phase-TV}
\end{equation}
...
\begin{equation}
 \boxed{
 \TotalVariationDistance\bigl(J_t-n,K_\theta\bigr)\le\frac4t.}
 \label{eq:conditioned-phase-TV}
\end{equation}
For every $z\in\ComplexNumbers^\times$, the limiting bilateral probability generating
Laurent product is
\begin{equation}
 \boxed{
 \Expectation z^{K_\theta}
 =\prod_{k\le0}\left[p_{\theta,k}+(1-p_{\theta,k})z^{-1}\right]
 \prod_{k\ge1}\left[(1-p_{\theta,k})+p_{\theta,k}z\right].}
 \label{eq:phase-pgf}
\end{equation}
Both products converge locally uniformly on $\ComplexNumbers^\times$.
It has mean
\begin{equation}
 \Expectation K_\theta=P(\theta).
 \label{eq:phase-mean}
\end{equation}
\end{theorem}
...
\[
 \Probability(K_t=0)=\prod_{j\ge1}(1-p(t2^{-j}))
...
\eqref{eq:K-theta}.  On compact subsets of $\ComplexNumbers^\times$, its coarse-factor
...
$\{\log_2t\}\in[0,1)$,
\begin{equation}
 K_t=\log_2t+O_{\Probability}(1),
 \qquad
 J_t=\log_2t+O_{\Probability}(1).
...
\begin{equation}
 \Variance(K_{2^{n+\theta}})\longrightarrow\Variance(K_\theta).
 \label{eq:variance-phase-limit}
\end{equation}
More precisely, for $t=2^{n+\theta}$ with $n\ge1$,
\begin{equation}
 0\le \Variance(K_\theta)-\Variance(K_t)
...
$\IntegerNumbers$ and strictly positive at every integer.  Its set of modes is therefore a
...
support is all of $\IntegerNumbers$.  A log-concave sequence has a consecutive set of
...
\begin{equation}
 \boxed{
 \Probability(K_t=0)
 =\frac{2t}{1-\EulerE^{-2t}}L_F(2t).}
 \label{eq:P0-Laplace}
\end{equation}
Equivalently,
\begin{equation}
 \boxed{
 \log\Probability(K_t=0)
 =\log(2t)-\log(1-\EulerE^{-2t})+\Lambda(2t).}
...
\[
 M(t)=\Expectation\EulerE^{tX}=\EulerE^t\Expectation\EulerE^{-2tY}=\EulerE^tL_F(2t).
\]
Now use \eqref{eq:P0} and
$m(t)=\sinh t/t=(\EulerE^t-\EulerE^{-t})/(2t)$.
\end{proof}
...
\label{thm:TM-corner}
Let $a_1,\ldots,a_N>0$, let $I\subset\RealNumbers$ be an open interval containing
\[
...
\begin{align}
 ...
 &=\int_{-a_1}^{a_1}\cdots\int_{-a_N}^{a_N}
   g^{(N)}(x+u_1+\cdots+u_N)\Differential u_1\cdots\Differential u_N.
...
\begin{equation}
 ...
 =(-1)^N\sum_{k=0}^{2^N-1}(-1)^{\BinaryDigitSum(k)}
 g\left(x-(1-2^{-N})+\frac{k}{2^{N-1}}\right),}
 \label{eq:TM-corner}
\end{equation}
where $\BinaryDigitSum(k)$ is the binary digit sum.  Hence the corner annihilates every
...
$g(x+a)-g(x-a)=\int_{-a}^ag'(x+u)\Differential u$, iterated $N$ times, gives
...
\[
 ...
 =-(1-2^{-N})+\frac{k}{2^{N-1}},
 \qquad
 \prod_{j=1}^N\varepsilon_j=(-1)^N(-1)^{\BinaryDigitSum(k)}.
\]
...
For $n\ge0$ and $x\in\ComplexNumbers$,
\begin{equation}
 i_n(x)=\frac{x^n}{2^{n+1}n!}
 \int_{-1}^1\EulerE^{xv}(1-v^2)^n\Differential v,
 \label{eq:i-Rodrigues}
\end{equation}
and hence
\begin{equation}
 \boxed{
 |i_n(x)|\le\EulerE^{|x|}\frac{|x|^n}{(2n+1)!!}.}
 \label{eq:i-bound}
\end{equation}
\end{proposition}
...
$P_n(v)=(2^nn!)^{-1}\frac{\Differential^n}{\Differential v^n}(v^2-1)^n$ into
...
\[
 \int_{-1}^1(1-v^2)^n\Differential v
...
\begin{equation}
 b_n(x)=\EulerE^{|x|}\frac{|x|^n}{(2n+1)!!},
 \qquad
 N_x=\Ceiling{\frac{|x|}{2}}.
 \label{eq:degree-envelope}
\end{equation}
For every $\varepsilon>0$, define
\begin{equation}
 n_\varepsilon(x)=
 \min\left\{n\in\NaturalNumbers:n\ge N_x\ \text{and}\ b_n(x)\le\varepsilon\right\}.
...
\begin{equation}
 A_\varepsilon=\log\frac{\EulerE^{|x|}}{\varepsilon}.
 \label{eq:A-epsilon}
\end{equation}
...
\begin{equation}
 ...
 {W_0\!\left(\dfrac{2A_\varepsilon}{\EulerE|x|}\right)}.}
...
\[
 \log\bigl((2n+1)!!\bigr)
 =n\log\frac{2n}{\EulerE}+O(\log n).
\]
Therefore the exact minimal cutoff satisfies
\[
 n\log\frac{2n}{\EulerE|x|}=A_\varepsilon+O(\log n).
\]
Solving the leading equation with $W_0(z)\EulerE^{W_0(z)}=z$ yields
\eqref{eq:Lambert-degree}.  Moreover, the derivative of
$y\mapsto y\log(2y/(\EulerE|x|))$ is
$1+\log(2y/(\EulerE|x|))\to\infty$.  Hence the $O(\log n)$ perturbation moves the
...
For every $t\in\RealNumbers$ and integer $N\ge0$,
\begin{equation}
 d_{1/2}(t)
...
\item independent comparison of the product formula for $\Probability(K_t=0)$ with
...
$2|\FourierSeriesCoefficient{Q}{1}|$
...
\begin{tabular}{@{}rrrr@{}}
\toprule
$t$ & $\Probability(K_t=0)$ & $\log\Probability(K_t=0)$ & absolute discrepancy \\
\midrule
...
equation over $\ComplexNumbers(t)$.
\end{conjecture}
...
fraction of the variance of $\EulerE^{tX}$ with the smallest cardinality.  Compare:
\begin{enumerate}
...
At the activation level, the sharp local coefficient survives arbitrary
square-summable signed weight families: a global $\ell^2$ budget supplies a
summable majorant, and Tannery's theorem gives its exact punctured quadratic
limit.  The normalized geometric weights have squared mass
$(1-q)/(1+q)$, so the geometric and dyadic effective dimensions have sharp
coefficients $(1-q)/(3(1+q))$ and $1/9$, respectively.
...
Square-summable activation budget and limit & \statusP & global quadratic bound and Tannery's theorem \\
...
Sharp geometric and dyadic activation coefficients & \statusP & normalized geometric-weight HasSum evaluation \\
...
Scaled activation limit
& $p(at)/t^2\to a^2/3$ as $t\to0$ through $t\ne0$ \\
Square-summable activation series
& $D_w(t)=\sum_i p(w_it)\le(t^2/3)\sum_iw_i^2$ and
$D_w(t)/t^2\to\frac13\sum_iw_i^2$ \\
Component
& $H_S=M\prod_{j\in S}(\EulerE^{x_jV_j}/m(x_j)-1)$ \\
Component energy
& $\|H_S\|_2^2=M^2\prod_{j\in S}r(x_j)$ \\
Second-moment ratio
& $R=M(2t)/M(t)^2=\prod_j(1+r_j)=m(t)/M(t)$ \\
No-active atom
& $\Probability(K_t=0)=1/R=M(t)/m(t)$ \\
Order product
& $\Ccal(t,z)=\prod_j(1+zr_j)=\sum_kA_k(t)z^k$ \\
Order refinement
& $A_k(2t)=A_k(t)+r(t)A_{k-1}(t)$ \\
Power-sum Bell form
& $A_k=k!^{-1}\ExponentialCompleteBellPolynomial{k}(\InteractionPowerSum{1},-1!\InteractionPowerSum{2},\ldots,(-1)^{k-1}(k-1)!\InteractionPowerSum{k})$ \\
Legendre coefficient
& $c_\alpha=M\prod_{\alpha_j>0}\sqrt{2\alpha_j+1}\,i_{\alpha_j}(x_j)/i_0(x_j)$ \\
...
Geometric activation dimension
& $d_q(t)=\sum_{n\ge0}p(q^n(1-q)t)$ and
$d_q(t)/t^2\to(1-q)/(3(1+q))$ for $|q|<1$ \\
Mellin transform
& $\Mcal_p(s)=2^{2-s}(1-2^{2-s})\Gamma(s-1)\zeta(s-1)$ for $-2<\Re s<0$ \\
Periodic mean
& $\mu(t)=\log_2t+Q(\{\log_2t\})+O(t^{-1})$ \\
Nonzero Fourier modes
& $\FourierSeriesCoefficient{Q}{n}=\Mcal_p^{\mathrm{cont}}(-2\pi\ImaginaryUnit n/\log2)/\log2$ \\
Phase coupling
& $\TotalVariationDistance(K_t-\Floor{\log_2t},K_\theta)\le2/t$ \\
Laplace transfer
& $\Probability(K_t=0)=2t(1-\EulerE^{-2t})^{-1}L_F(2t)$ \\
Thue--Morse corner
& $\Delta_{2^{-1}}\cdots\Delta_{2^{-N}}g=(-1)^N\sum_{k<2^N}(-1)^{\BinaryDigitSum(k)}g(\cdots)$ \\
Legendre envelope
& $|i_n(x)|\le\EulerE^{|x|}|x|^n/(2n+1)!!$ \\
Lambert cutoff
& $n_\varepsilon(x)\sim A_\varepsilon/W_0(2A_\varepsilon/(\EulerE|x|))$ \\
\bottomrule
\end{longtable}
...
Git history & Recoverable former package-checksum receipts; no live ledger \\
