...
$\nu=(\nu_1,\nu_2,\ldots)\in\NaturalNumbers^{(\NaturalNumbers)}$, put
\begin{equation}
 \ell_\nu(V)=\prod_{j\ge1}\ell_{\nu_j}(V_j),
 \qquad \SupportOperator\nu=\{j:\nu_j>0\}.
...
The expansion
\begin{equation}
 \EulerE^{tX}=\sum_{\nu\in\NaturalNumbers^{(\NaturalNumbers)}}c_\nu(t)\ell_\nu(V)
...
Hoeffding subspace indexed by $S$ exactly when $\SupportOperator\nu=S$; Parseval inside
