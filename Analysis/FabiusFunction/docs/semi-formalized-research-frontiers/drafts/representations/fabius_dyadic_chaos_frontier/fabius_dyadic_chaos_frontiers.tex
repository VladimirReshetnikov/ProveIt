...
as $\prod_{j\in S}(x_j\coth x_j-1)$, where $x_j=t2^{-j}$.  Consequently the
...
\item a mixed-replacement inequality for all smooth observables, with an exact
      dyadic $q$-Pochhammer factor and sharp monomial top order;
...
\end{equation}
For a finite set $S\subset\N$, put
\begin{equation}
 Q_S=\prod_{j\in S}Q_j,
 \qquad E_{S^c}=\prod_{j\notin S}E_j,
...
\begin{proof}
For a finite coordinate set $J$, factorization gives
\[
 \e^{t\sum_{j\in J}a_jV_j}
 =\prod_{j\in J}m(x_j)\prod_{j\in J}(1+Z_j).
\]
Applying $E_j$ selects the constant term in coordinate $j$, while $Q_j$
selects $Z_j$.  Thus the finite-dimensional Hoeffding component indexed by
$S\subset J$ is exactly the corresponding product.

For the infinite limit, first compute
\begin{align*}
 \E Z_j^2
 &=\frac{m(2x_j)}{m(x_j)^2}-1\\
 &=\frac{\sinh(2x_j)/(2x_j)}{(\sinh x_j/x_j)^2}-1
 =x_j\coth x_j-1=r(x_j).
\end{align*}
...
\]
Thus the orthogonal series converges unconditionally in $L^2$.  Its finite
partial sums are the finite-coordinate products and converge almost surely and
in $L^2$ to $\e^{tX}$, proving the identity and the norm formula.
\end{proof}
...
\[
 x\frac{\dd}{\dd x}\log m(x)=x\coth x-1=r(x),
\]
and termwise differentiation of the locally uniformly convergent product for
$M$ gives \eqref{eq:A1-logM}.
\end{proof}
...
\begin{equation}
 ...
 \qquad
 0\le1-p(x)=\frac{\tanh x}{x}\le\min\left(1,\frac1x\right).
 \label{eq:p-bounds}
\end{equation}
Also
\begin{equation}
...
\begin{proof}
The lower bounds and $p<1$ follow from $0<\tanh x<x$ for $x>0$.
...
\begin{proof}
Expand every local factor $\e^{x_jV_j}$ by
\eqref{eq:local-Legendre-expansion}.  Factoring out
$\prod_ji_0(x_j)=M(t)$ gives \eqref{eq:c-nu}.  Finite tensor products are
orthonormal.  Local Parseval and the summability established below imply that
the coefficient squares sum to $M(2t)$, so the infinite expansion converges in
$L^2$.

A tensor coefficient has Hoeffding support $S$ exactly when $\nu_j>0$ precisely
for $j\in S$.  Orthogonality of the basis inside the closed Hoeffding subspace
then gives \eqref{eq:support-Legendre-energy}.
\end{proof}
...
\end{equation}
Conditionally on digit $j$ being active, define a mark $N_j\in\{1,2,\ldots\}$
by
\begin{equation}
 \boxed{
 \Prob(N_j=n\mid B_j=1)
 =\frac{(2n+1)i_n(x_j)^2}{m(2x_j)-m(x_j)^2}.}
 \label{eq:degree-mark}
\end{equation}
Then the normalized squared tensor coefficients are generated by independent
active indicators $B_j$ and, on active coordinates, independent marks $N_j$,
conditioned on at least one activation.
\end{proposition}
...
\eqref{eq:local-Parseval}; the denominator in \eqref{eq:degree-mark} is the
same positive nonconstant local energy.  The coefficient factorization in
\eqref{eq:c-nu}, together with \cref{thm:active-set-law}, gives the independent
mark construction.
\end{proof}
...
normalized second-moment generating product is
\begin{equation}
 \frac1{M(2t)}
 \sum_{\nu}|c_\nu(t)|^2u^{|\supp\nu|}z^{|\nu|_1}
...
The local odds have the convergent expansion
\begin{equation}
 r(x)=x\coth x-1
 =\frac{x^2}{3}-\frac{x^4}{45}+\frac{2x^6}{945}
...
\end{equation}
The right sides are understood by analytic continuation at removable
singularities.
\end{theorem}
...
For $0<\Re s<1$, put
$J(s)=\int_0^\infty x^{s-1}\sech^2x\dd x$.  Since
\[
...
$\Ical_p(s)=-s\Mcal_p(s)$ for $-2<\Re s<0$.  The gamma recurrence
...
\end{equation}
The limiting probability generating Laurent product is
\begin{equation}
 \boxed{
 \E z^{K_\theta}
...
\eqref{eq:K-theta}, and summing the expectations gives precisely
\eqref{eq:P-theta}.
\end{proof}
...
\end{equation}
Along each fixed phase $\theta$, all centered moments that are uniformly
integrable converge to the corresponding moments of $K_\theta$.  In
particular,
\begin{equation}
...
bounded variance and the bounded periodic mean correction.  The second moments
are uniformly bounded after centering, so total-variation convergence plus
uniform integrability gives \eqref{eq:variance-phase-limit}.  Reindex
...
$\Z$ and strictly positive at every integer.  It is therefore unimodal,
possibly with a two-point plateau.
\end{corollary}
...
\label{thm:TM-corner}
For $g\in C^N$ and positive $a_1,\ldots,a_N$,
\begin{align}
...
Fix $x\ne0$ and let $\varepsilon\downarrow0$.  Define
\begin{equation}
...
\end{equation}
A rigorous adaptive cutoff is obtained by taking the smallest integer $n$ for
which the exact right side of \eqref{eq:i-bound} is at most $\varepsilon$;
\eqref{eq:Lambert-degree} is its asymptotic initial guess.
\end{corollary}
...
\]
The leading equality in \eqref{eq:i-bound} is therefore
\[
 n\log\frac{2n}{\e|x|}=A_\varepsilon+O(\log n).
\]
Solving the leading equation uses $W_0(z)\e^{W_0(z)}=z$ and yields
\eqref{eq:Lambert-degree}.  The $O(\log n)$ term is lower order relative to
$A_\varepsilon$.
\end{proof}
...
Component norm $r(x)=x\coth x-1$ & \statusP & one-dimensional hyperbolic integral \\
...
MGF
& $M(t)=\prod_{j\ge1}m(t2^{-j})$, $m(x)=\sinh(x)/x$, $M(2t)=m(t)M(t)$ \\
Local Hoeffding odds
& $r(x)=m(2x)/m(x)^2-1=x\coth x-1$ \\
Local activation probability
& $p(x)=r(x)/(1+r(x))=1-\tanh(x)/x$ \\
Component
...
Mellin transform
& $\Mcal_p(s)=2^{2-s}(1-2^{2-s})\Gamma(s-1)\zeta(s-1)$ \\
Periodic mean
& $\mu(t)=\log_2t+Q(\{\log_2t\})+O(t^{-1})$ \\
Nonzero Fourier modes
& $\widehat Q(n)=\Mcal_p(-2\pi\ii n/\log2)/\log2$ \\
Phase coupling
...
Lambert cutoff
& $n_\varepsilon\sim A_\varepsilon/W_0(2A_\varepsilon/(\e|x|))$ \\
\bottomrule
\end{longtable}
...
\path{SHA256SUMS} & Integrity hashes for archive contents \\
