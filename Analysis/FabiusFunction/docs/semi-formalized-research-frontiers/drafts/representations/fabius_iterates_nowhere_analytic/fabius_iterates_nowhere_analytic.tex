...
has amplitude exactly $\Up(1/9)\geq1/2$.  The resulting derivative lower bound grows
...
\[
 x_k\in\set{\tfrac14,\tfrac34},
 \qquad
 x_{k+1}=F(x_k)\in\set{\tfrac5{72},\tfrac{67}{72}}.
\]

For $m_\ell\geq4$, both $2^{m_\ell}/4$ and $3\cdot2^{m_\ell}/4$ are even
integers.  The block formula \eqref{eq:theta-block} and $\Up(\pm1)=0$ hence give
\begin{equation}
...
of $\Up$, and monotonicity on $[0,1]$ show that
\begin{equation}
 \abs{\ThetaF(2^{m_\ell}x_{k+1})}
 =\Up(1/9)\geq\Up(1/2)=\frac12.
...
\begin{equation}
 \min_{\substack{\pi\in\Pi_m\\\abs{\pi}=k}}D(\pi)=(k-1)(m-k).
