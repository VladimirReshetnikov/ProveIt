...
The proof is self-contained and has been checked symbolically and numerically, but it has not yet been translated into Lean.
...
\begin{conjecture}[Complete Taylor-series trichotomy]
\label{conj:trichotomy}
For fixed $n$, every interior point belongs to exactly one of three explicitly
...
\end{enumerate}
The third class is empty, and the second class is a countable orbit-arithmetic set
that can be described using dyadic orbit points and exact tie cancellations.
\end{conjecture}

The correction above illustrates why it is valuable to distinguish nowhere
...
\begin{conjecture}[No persistent cancellation at a tie]
\label{conj:ties}
Suppose $x\in T_n$ so two adjacent spine weights are maximal.  Then the two leading
symbolic amplitudes do not cancel to exponential accuracy for all sufficiently large
orders.  More precisely, there is an infinite sequence $m_k$ and a constant $c>0$
such that the sum of the two tied spine terms has absolute value at least
$cH_n(x)^{m_k}$.
\end{conjecture}

A proof may use the fact that the two orbit points are related by one application of
$F$ while their signed-extension samples are taken at the same dyadic scale.  This
creates a nonlinear symbolic correlation problem for the doubling map.
