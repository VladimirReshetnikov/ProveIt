...
\large Exact finite synthesis of Lagrange cardinals by shifted \(\operatorname{up}\)-functions,\\
...
\item the current \emph{Up Polynomial Synthesis} report, especially exact
      polynomial reproduction, the sharp order \(\vtwo(m)\), the
...
and
\begin{equation}
 \operatorname{ord}_{\xi=n}\widehat u=1+\vtwo(n),\qquad n\in\Z\setminus\{0\}.
 \label{eq:zero-order}
\end{equation}
...
\begin{equation}
 \calD:=M(D)^{-1},\qquad D=\frac{\dd}{\dd x}.
 \label{eq:deconv-def}
\end{equation}
...
atom radius \(\rho=mh\).  If \(\deg p\le\vtwo(m)\), then with
\begin{equation}
...
\begin{equation}
 p(x)=\frac1m\sum_{k\in\Z}
 p_\rho^\sharp(x_0+kh)
 u\!\left(\frac{x-x_0-kh}{\rho}\right).
 \label{eq:repo-synthesis}
\end{equation}
The exact stationary reproduction degree is \(\vtwo(m)\); therefore the smallest
...
\(h=1/m\).  Since \(\vtwo(m)=d\), the degree hypothesis is exact, and
\(p_\rho^\sharp=\calD p\).  This gives the global locally finite identity
\[
 \ell_{j,d}(x)=\frac1m\sum_{k\in\Z}
...
If all \(\tau_j\in\Q\), then every \(c_{j,k}^{(d)}\in\Q\).  If the node set is
...
Let
\begin{equation}
 K_d=\{k\in\Z:|k|<2m\},\qquad m=2^d.
 \label{eq:Kd}
\end{equation}
Define the analysis matrix \(A_d\in\R^{K_d\times(d+1)}\) by
\begin{equation}
...
If \(I_du\to u\) in a norm \(\norm{\cdot}_X\), then the finite shifted-up sums
...
\begin{equation}
 \frac{\e^{xz}}{M(z)}=\sum_{n\ge0}\mathcal A_n(x)\frac{z^n}{n!}.
 \label{eq:appell-egf}
\end{equation}
...
\item geometric Lagrange denominators factor into \(q\)-Pochhammer symbols at
      \(q=1/2\) or \(1/4\);
\item the up Fourier zeros have multiplicity \(1+\vtwo(n)\), so the alias
...
\begin{equation}
 \lambda_d(x)=\sum_{j=0}^d\abs{\ell_{j,d}(x)},
 \qquad
...
\end{equation}
can be far larger than \(\norm{I_d}\).  The compressed \(d+2\)-atom basis reduces
...
\begin{equation}
 u(x)=2\int_0^\infty\widehat u(\xi)\cos(2\pi\xi x)\,\dd\xi
...
\midrule
\(\norm{S_2A_2-I_3}_\infty\) & \(6.25\times10^{-10}\)\\
\(\norm{(A_2S_2)^2-A_2S_2}_\infty\) & \(1.21\times10^{-9}\)\\
...
matrices over \(\R\).  The proof of \(S_dA_d=1\) is pointwise cardinal evaluation.
...
reproduction order \(\vtwo(m)\); the canonical minimum \(m=2^d\); Appell moment
