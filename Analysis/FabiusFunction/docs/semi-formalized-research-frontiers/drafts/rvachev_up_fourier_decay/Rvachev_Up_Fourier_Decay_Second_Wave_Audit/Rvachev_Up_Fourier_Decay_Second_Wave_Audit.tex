...
$f(x)=\prod_{n\ge0}\SincRad(\pi x/2^n)$ arrived after, and in explicit
...
$(\LaplaceTransformSymbol_qh)(x)=\frac12[\sin^q(\tfrac{\pi x}2)h(\tfrac x2)+
...
\begin{equation}
 \frac1{t-t_0}\int_{t_0}^t\frac{|f(x)|}{g(x)}\Differential x\longrightarrow1
 \tag{Q2}\label{eq:Q2}
\end{equation}
and the two-sided bounded relaxation of the same ratio
\begin{equation}
 0<A\le\frac1{t-t_0}\int_{t_0}^t\frac{|f(x)|}{g(x)}\Differential x\le B<\infty.
 \tag{Q3}\label{eq:Q3}
\end{equation}
...
$\alpha_k(\Differential z)=\varrho_1^{-(k+1)}W(z)P_{k+1}(z)\Differential z$ for the exact
...
\begin{equation}
 2^{-k}\int_{2^k}^{2^k(1+z)}\frac{|f|}{g}\Differential x\longrightarrow z
...
measures of $|f|/g$ have the form $q_k\,\Differential\alpha_k/\int q_k\Differential\alpha_k$
...
\begin{equation}
 \frac{\Differential}{\Differential z}\log g_1\bigl(2^k(1+z)\bigr)
...
measure $q\,\Differential\alpha_k$ carries mass exactly $\delta$, so its
...
$\frac{\Differential}{\Differential z}\log(g_1/q_k)\le-2b_k+b_k=-b_k<0$; upward jumps of
...
$F\in C^1(\RealNumbers)$ and any continuous $\epsilon>0$ there is an entire $G$,
real on $\RealNumbers$, with $|G-F|<\epsilon$ and $|G'-F'|<\epsilon$ everywhere.
Document 7 applies it to $F=\log g_0$ with
$\epsilon<\min(-F'/2,\,\text{something}\to0)$, obtaining
$g=\EulerE^G$ zero-free, positive, strictly decreasing, with
$g/g_0\to1$; Document 8 applies it to the increasing reciprocal $h$
with $\epsilon<\frac12\min(h,h',\delta h)$, obtaining
$\widetilde h>0$, $\widetilde h'>0$ on all of $\RealNumbers$ and
...
\item Multiplicative repair: with $\Differential x/x$ averaging, $A_1^{\log}g_1$
...
\]
and $\int_0^1d^2=\frac2\pi\int_0^{\pi/2}\log^2(\tan u)\Differential u=\pi^2/4$
...
$\LaplaceTransformSymbol_1$ on Lipschitz functions ($\|\LaplaceTransformSymbol\varphi\|_\infty\le
...
$E_{m2^k}\sim H(1)|H(m)|(m2^k)^{-(k+1)}/(\EulerE(k+1))$ agree with
...
collocation of $\LaplaceTransformSymbol_q$ at $N=48$ and $N=64$ nodes in 40-digit
...
$\Variance(S_{1000})=2464.1112321386$, and all of Document 8's appendix
...
$\Differential\kappa_1/\Differential\varrho_1=-1/(a\varrho_1)\approx-2.18$, so the
...
$\FabiusGlobal=\sum(-1)^{w(n)}\,\RvachevUp(\cdot-2n-1)$ governs the lobe signs of
...
$\Variance(S_{1000})$, and $\int_0^{\pi/2}\log^2\tan=\pi^3/8$.  Writes
