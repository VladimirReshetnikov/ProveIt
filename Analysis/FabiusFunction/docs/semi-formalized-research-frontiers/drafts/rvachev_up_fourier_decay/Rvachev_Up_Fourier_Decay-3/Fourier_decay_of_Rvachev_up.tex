...
\begin{abstract}
Let $\RvachevUp$ be Rvachev's compactly supported $C^\infty$ function, normalized to have
integral one, and use the Fourier convention
$\widehat h(\xi)=\int_{\RealNumbers}h(t)\EulerE^{-2\pi\ImaginaryUnit\xi t}\Differential t$.  Its Fourier transform is
\[
 \Uhat(\xi)=\prod_{n=0}^{\infty}
 \SincRad\!\left(\frac{\pi\xi}{2^n}\right),
...
\begin{equation}\label{eq:def-U}
 ...
 \SincRad\!\left(\frac{\pi x}{2^n}\right),
 \qquad x\in\RealNumbers.
\end{equation}
It is real, even, entire, and rapidly decreasing on the real axis.  Since $\RvachevUp$ is
...
treatment.  The product converges normally on compact subsets of $\ComplexNumbers$, since
$1-\SincRad z=\BigO(z^2)$ near zero.
...
\end{equation}
where $\TwoAdicValuation(m)$ is the exponent of $2$ in $m$.  Thus the zero at each
$m\in\IntegerNumbers\setminus\{0\}$ has multiplicity $1+\TwoAdicValuation(|m|)$.
\end{proposition}
...
$(n,q)$ satisfying $m=2^nq$ is $1+\TwoAdicValuation(m)$.  Since
$\sum_{m\ge1}(1+\TwoAdicValuation(m))/m^2<\infty$, the collected genus-zero product converges
...
use of $\SincRad(2z)=\SincRad(z)\cos z$:
...
\begin{equation}\label{eq:scaling}
 \Uhat(x)=\SincRad(\pi x)\Uhat(x/2),
 \qquad
 \Uhat(2x)=\SincRad(2\pi x)\Uhat(x).
\end{equation}
...
$\RealNumbers\setminus\IntegerNumbers$.  Put
\[
 A(x):=\sum_{m=1}^{\infty}\frac{1+\TwoAdicValuation(m)}{m^2-x^2}.
\]
...
\begin{align}
 ...
 H(y)&:=\prod_{m=1}^{\infty}
 \left|\SincRad\!\left(\frac{\pi y}{2^m}\right)\right|.
 \label{eq:H}
\end{align}
...
\begin{equation}\label{eq:logsin-mean}
 \int_0^1\log|2\sin(\pi t)|\Differential t=0
\end{equation}
...
\begin{equation}\label{eq:max-average}
 ...
 \int_0^1\log|\sin(\pi t)|\Differential\mu(t)
 =\log\rho.
\end{equation}
...
$\int\log z\Differential\eta=-\infty$, there is nothing to prove.  Otherwise invariance and
$\log R(z)=\log4+\log z+\log(1-z)$ imply
\begin{equation}\label{eq:log-one-minus-z}
 \int\log(1-z)\Differential\eta(z)=-\log4.
\end{equation}
...
\[
 3\int\log z\Differential\eta+\int\log(1-z)\Differential\eta
 \le\log\frac{27}{256}.
\]
Using \eqref{eq:log-one-minus-z} yields
$\int\log z\Differential\eta\le\log(3/4)$.  Since
...
\[
 ...
 =\sup_{\mu\in\mathcal M_\T}
 \int\log|\sin(\pi t)|\Differential\mu(t).
\]
...
$P_{k+1}(t)\le M_{k+1}=\rho^{k+1}\EulerE^{o(k)}$ from
...
\[
 \frac1T\int_{T_0}^{T}\frac{|\Uhat(x)|}{g(x)}\Differential x
\]
...
\begin{equation}\label{eq:transfer-L1}
 (\LaplaceTransformSymbol f)(x)=\frac12\left[
 \sin\!\left(\frac{\pi x}{2}\right)f\!\left(\frac x2\right)
 +\cos\!\left(\frac{\pi x}{2}\right)f\!\left(\frac{x+1}{2}\right)
 \right],\qquad 0\le x\le1.
\end{equation}
Indeed, $I_1(n)=\int_0^1\LaplaceTransformSymbol^n\mathbf1(x)\Differential x$, and $\lambda$ is the dominant
eigenvalue of $\LaplaceTransformSymbol$ on a suitable H\"older space.
...
\begin{equation}\label{eq:cesaro-bounds}
 ...
 \frac{|\Uhat(x)|}{g_1(x)}\Differential x\le B.
\end{equation}
...
\[
 J_k:=\int_{2^k}^{2^{k+1}}
 \frac{|\Uhat(x)|}{g_1(x)}\Differential x.
\]
After $x=2^ky$,
...
$\frac{\Differential}{\Differential x}=x^{-1}\frac{\Differential}{\Differential\log x}$ gives, for fixed $m$,
...
$\LaplaceTransformSymbol h=\lambda h$, $\LaplaceTransformSymbol^*\nu=\lambda\nu$, and $\nu(h)=1$.  Then, for H\"older $q$,
\begin{equation}\label{eq:spectral-gap}
 \lambda^{-n}\LaplaceTransformSymbol^nq
...
\begin{equation}\label{eq:mantissa-profile}
 ...
 \frac{|\Uhat(x)|}{C_1g_1(x)}\Differential x
 =\frac{1+D(y)/C_1}{y},
\end{equation}
where
\begin{equation}\label{eq:D-profile}
 D(y)=\left(\int_0^1h(x)\Differential x\right)
...
\begin{equation}\label{eq:Ip}
 I_p(n):=\int_0^1P_n(t)^p\Differential t
\end{equation}
and the weighted transfer operator
\begin{equation}\label{eq:Lp-op}
 (\LaplaceTransformSymbol_p f)(x):=\frac12\left[
...
\begin{equation}\label{eq:Lp-shell-bound}
 c_p2^k\le
 \int_{2^k}^{2^{k+1}}
 \left|\frac{\Uhat(x)}{g_p(x)}\right|^p\Differential x
 \le C_p2^k.
\end{equation}
...
\begin{equation}\label{eq:L2-exact}
 I_2(n)=2^{-n}.
\end{equation}
Indeed, $\LaplaceTransformSymbol_2\mathbf1=\tfrac12\mathbf1$ because
...
\begin{equation}\label{eq:CW}
 \inf_x\frac{\LaplaceTransformSymbol v(x)}{v(x)}
 \le\lambda\le
 \sup_x\frac{\LaplaceTransformSymbol v(x)}{v(x)}.
\end{equation}
