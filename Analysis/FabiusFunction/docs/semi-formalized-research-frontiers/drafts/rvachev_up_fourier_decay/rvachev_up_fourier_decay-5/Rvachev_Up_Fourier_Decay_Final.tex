...
\[
 f(x)=\widehat{\Up}(x)
 =\prod_{j=0}^{\infty}\sinc\!\left(\frac{\pi x}{2^j}\right),
 \qquad \sinc z=\frac{\sin z}{z},
\]
where the Fourier convention is
$\widehat h(\xi)=\int_{\R}h(t)\e^{-2\pi\ii t\xi}\dd t$.
...
analytic gauges.  A logarithmic Ces\`aro average, with measure $\dd x/x$, does
...
\begin{equation}
 \frac1{t-t_0}\int_{t_0}^{t}\frac{|f(x)|}{g(x)}\dd x
 \longrightarrow1,
 \qquad t\to\infty,
 \label{eq:Q2}
\end{equation}
...
\begin{equation}
 f(z)=\prod_{j=0}^{\infty}\sinc\!\left(\frac{\pi z}{2^j}\right)
 \label{eq:f-def}
\end{equation}
converges locally uniformly on $\C$.  Indeed,
$\log\sinc w=-w^2/6+\bigO(w^4)$ near zero, and
...
\begin{align}
 ...
 f(z)&=\sinc(\pi z)H(z),
 \qquad
 H(z):=\prod_{j=1}^{\infty}\sinc\!\left(\frac{\pi z}{2^j}\right).
 \label{eq:H-def}
\end{align}
Thus every nonzero integer $m$ is a zero of multiplicity $1+\vtwo(|m|)$,
...
\[
 \sinc(\pi z)=\prod_{r=1}^{\infty}\left(1-\frac{z^2}{r^2}\right)
\]
applied at $z/2^j$ shows that a fixed integer $m$ appears once for each
$j\in\{0,\ldots,\vtwo(m)\}$.  This gives \eqref{eq:weierstrass} and the stated
zero multiplicities.
\end{proof}
...
\begin{equation}
 \operatorname{sgn}f(x)=(-1)^{\wt(m)},
 \qquad m<x<m+1,
 \label{eq:TM-sign}
\end{equation}
where $\wt(m)$ is the sum of the binary digits of $m$.
\end{theorem}
...
Crossing the zero at $r$ changes the sign $1+\vtwo(r)$ times.  The sign on
$(m,m+1)$ is therefore
\[
 (-1)^{\sum_{r=1}^{m}(1+\vtwo(r))}.
\]
Legendre's identity $\vtwo(m!)=m-\wt(m)$ gives
$\sum_{r=1}^{m}(1+\vtwo(r))=2m-\wt(m)$, whose parity is $\wt(m)$.
\end{proof}
...
\begin{equation}
 2^nP_n(t)=
 \left|\prod_{j=0}^{n-1}(1-\e^{2\pi\ii2^jt})\right|
 =\left|\sum_{r=0}^{2^n-1}(-1)^{\wt(r)}\e^{2\pi\ii rt}\right|.
...
\[
 f(2^ky)=f(y)\prod_{j=1}^{k}\sinc(\pi2^jy).
\]
...
One has
\begin{equation}
 \int_0^1d(x)^2\dd x=\frac{\pi^2}{4}.
 \label{eq:d-variance}
\end{equation}
Equivalently,
\begin{equation}
 \int_0^1\psi(x)\psi(\T^rx)\dd x
 =\frac{\pi^2}{12\,2^r},
 \qquad r\ge0,
 \label{eq:covariance}
\end{equation}
...
\label{prop:RPF}
For each $q>0$, the operator $\Lop_q$ on a suitable H\"older space has a
...
\begin{equation}
 \mathcal M_{q,k}(F):=
 \left(2^{-k}\int_{2^k}^{2^{k+1}}|F(x)|^q\dd x\right)^{1/q}.
 \label{eq:shell-Lq}
\end{equation}
...
\[
 ...
 =\rho_q^{-n}\int_0^1W_{\Lambda_q}(1+t)^qP_n(t)^q\dd t.
\]
...
\begin{equation}
 ...
 \log\frac{|f(x)|}{E_{\kappa_0}(x)}\dd x\right)
 =\exp\!\left(\int_0^1\log W_{1/2}(1+t)\dd t\right).
...
\begin{equation}
 \int_0^1P_n(t)\dd t=K\rho^n(1+o(1)),
 \qquad K>0,
 \label{eq:FM}
\end{equation}
...
\begin{equation}
 c\le
 \frac1{t-t_0}\int_{t_0}^{t}\frac{|f(x)|}{g_1(x)}\dd x
 \le C
 \label{eq:Q3-result}
\end{equation}
...
\begin{equation}
 ...
 \frac{|f(x)|}{g_1(x)}\dd x\longrightarrow1.
 \label{eq:dyadic-Q2}
\end{equation}
\end{theorem}
...
$r(\T^nx)\le\e^{\varepsilon n}$ eventually for a.e. $x$, because $\T$
...
\begin{equation}
 ...
 \frac{|f(x)|}{E_{\kappa_1}(x)}\dd x
 =\rho^{-n}\int_0^zW_1(1+t)P_n(t)\dd t.
...
up to $1/c_K+o(1)$,
\[
 \rho^{-n}\frac{W_1(1+z)}{u(z)}P_n(z)\dd z.
\]
...
same order as all preceding shells combined.  With the measure $\dd x/x$,
...
\begin{equation}
 B_1:=\lim_{k\to\infty}
 \int_{2^k}^{2^{k+1}}
 \frac{|f(x)|}{E_{\kappa_1}(x)}\frac{\dd x}{x}
...
\begin{equation}
 \frac1{\log(t/t_0)}
 \int_{t_0}^{t}
 \frac{|f(x)|}{A_1^{\log}E_{\kappa_1}(x)}\frac{\dd x}{x}
 \longrightarrow1.
 \label{eq:log-Cesaro}
\end{equation}
\end{corollary}
...
For every $n\ge1$,
\begin{equation}
 \lambda^n\le\norm{P_n}_\infty\le\lambda^{n-1}.
 \label{eq:Pn-sup}
\end{equation}
Hence
\begin{equation}
 \lim_{n\to\infty}\norm{P_n}_\infty^{1/n}=\lambda.
...
\begin{equation}
 ...
 \qquad
 H_r(s):=\prod_{h=1}^{r}\sinc\frac{\pi s}{2^h}.
...
$3\cdot2^{q-j}$.  Since
$|\sin\pi x|\ge2\operatorname{dist}(x,\Z)$, the first $q$ factors have
...
\begin{equation}
 \log A_n
 =n\beta+
 \sup_{\substack{r\in\Z_{\ge0}\\1/4\le s\le1/2}}
 \mathcal F_n(r,s)
 +\bigO((\log\log n)^2).
...
\end{equation}
Choose
$q=\lceil4\log_2\log(n+\e^\e)\rceil$ and replace $s_0$ by the point $s_q$
from \cref{lem:preimages}.  On the localized set, the $s$ derivative of
$\mathcal F_n$ is $\bigO((\log n)^2)$, so this replacement changes the
...
\]
Its critical equation is $z\e^z=an\e^{-C_0}=cn$, hence $z=w_n$.  At this
point $n\e^{-C_0}\e^{-w_n}=w_n/a$, which gives
...
$0\le t\le1$.  Define
\begin{equation}
 f_k(t):=\prod_{j=0}^{k}\sinc\!\left(\frac{\pi t}{2^j}\right).
 \label{eq:fk}
\end{equation}
For $0\le j\le k$, $N/2^j$ is an integer and
\begin{equation}
 \left|\sinc\!\left(\frac{\pi(N+t)}{2^j}\right)\right|
 =\frac{t}{N+t}\sinc\!\left(\frac{\pi t}{2^j}\right).
\end{equation}
...
$p t^pF(t)\to hs\e^{-s}$, whose unique maximum is $h/\e$ at $s=1$.
The bound $t^pF(t)\le C(1-t)\e^{-p(1-t)}$ near $1$, together with exponential
smallness away from $1$, prevents a maximizing $s$ from escaping to zero or
infinity.
\end{proof}

Since $(\sinc(\pi t))'|_{t=1}=-1$, formula \eqref{eq:H-def} gives
\begin{equation}
...
Every zero of $P_n$ lies on the grid $2^{-n}\Z$.  On each of the $2^n$ finest
dyadic cells the absolute product is smooth, so fixed-order Gauss--Legendre
quadrature is highly effective.  Let
\begin{equation}
 I_n:=\int_0^1P_n(t)\dd t,
 \qquad
 \mathcal A_n:=\rho^{-n}\int_0^1W_1(1+t)P_n(t)\dd t.
...
\begin{equation}
 \mathscr F_n(z):=
 \frac{\int_0^zW_1(1+t)P_n(t)\dd t}
...
$(\Lop_1h)(x)/h(x)$ into the rational function $R(t)$.  The two inequalities
...
and the spectral theorem for $\Lop_q$.  No ambiguity about ``the decay rate''
