...
\[
 ...
 \qquad \SincRad u=\frac{\sin u}{u},
\]
be the Fourier transform of Rvachev's compactly supported up-function, with
Fourier convention \(\widehat h(\xi)=\int_{\RealNumbers}h(t)\EulerE^{-2\pi\ImaginaryUnit t\xi}\Differential t\).
...
\(g\) for which
\[
 \lim_{t\to\infty}\frac1{t-t_0}\int_{t_0}^t\frac{|\Phi(x)|}{g(x)}\Differential x=1
\]
...
\[
 x=2^ky,
 \qquad k\in\IntegerNumbers,\quad 1\le y<2,
 \qquad z=y-1\in[0,1).
\]
Define
\begin{equation}
 P_n(z)=\prod_{j=0}^{n-1}\AbsoluteValue{\sin(\pi2^jz)},
 \qquad
 H(y)=\prod_{m=1}^{\infty}\AbsoluteValue{\SincRad\!\left(\frac{\pi y}{2^m}\right)}.
 \label{eq:P-H}
\end{equation}
...
\[
 \AbsoluteValue{\sin(\pi2^jy)}=\AbsoluteValue{\sin(\pi2^j(y-1))}.
\]
...
The Euler product for \(\SincRad\) can be regrouped without convergence
...
\begin{equation}
 ...
 \label{eq:canonical}
\end{equation}
Thus the zeros at \(\pm m\) have multiplicity \(1+\TwoAdicValuation(m)\).
\end{proposition}

\begin{proof}
Expand each factor as
\[
 \SincRad\!\left(\frac{\pi z}{2^h}\right)
 =\prod_{r=1}^{\infty}\left(1-\frac{z^2}{r^2 4^h}\right).
\]
A positive integer \(m\) is representable as \(r2^h\) exactly for
\(0\le h\le\TwoAdicValuation(m)\), giving the stated multiplicity.  Finally,
\(\sum_m(1+\TwoAdicValuation(m))/m^2<\infty\), so the regrouping is legitimate.
\end{proof}
...
\begin{equation}
 ...
 =-2\sum_{r=1}^{\infty}(1+\TwoAdicValuation(r))
...
\begin{equation}
 \int_0^1 h(z)P_n(z)^q\Differential z
 =\int_0^1\LaplaceTransformSymbol_q^n h(x)\Differential x.
 \label{eq:transfer-identity}
\end{equation}
We write \(\LaplaceTransformSymbol=\LaplaceTransformSymbol_1\).

\begin{theorem}[Ruelle--Perron--Frobenius package for \(q=1\)]
\label{thm:RPF}
On the Banach space of Lipschitz functions on \([0,1]\), the operator
\(\LaplaceTransformSymbol\) has a simple positive leading eigenvalue \(\rho\), a strictly
positive Lipschitz eigenfunction \(h\), and a positive eigenprobability
\(\nu\), normalized by \(\nu(h)=1\), such that
\begin{equation}
 \rho^{-n}\LaplaceTransformSymbol^n\varphi
 \longrightarrow h\,\nu(\varphi)
...
\[
 \Lip(\LaplaceTransformSymbol\varphi)
 \le \frac{\sqrt2}{4}\Lip(\varphi)+C\Norm{\varphi}_\infty,
\]
while \(\Norm{\LaplaceTransformSymbol\varphi}_\infty\le2^{-1/2}\Norm{\varphi}_\infty\).
...
\[
 \int_0^1P_n\,\Differential z
 \ge \frac{\int_0^1P_n^2\,\Differential z}{\lVert P_n\rVert_\infty}
...
For later use set
\begin{equation}
 H_0=\int_0^1h(x)\Differential x.
 \label{eq:H0}
\end{equation}
Combining \eqref{eq:transfer-identity} and \eqref{eq:RPF-limit} gives
\begin{equation}
 \rho^{-n}\int_0^1\varphi(z)P_n(z)\Differential z
 \longrightarrow H_0\nu(\varphi)
...
\begin{equation}
 \alpha_k(\Differential z)
 =\rho^{-(k+1)}W(z)P_{k+1}(z)\Differential z,
 \qquad 0\le z\le1.
 \label{eq:alpha-k}
\end{equation}
...
\(\int_0^1\log|\sin\pi z|\Differential z=-\log2\) imply
\[
...
\[
 \sum_{n\ge1}\operatorname{Leb}\{z:r(\T^nz)>\EulerE^{\varepsilon n}\}
 =\sum_{n\ge1}\operatorname{Leb}\{r>\EulerE^{\varepsilon n}\}<\infty.
\]
...
\begin{equation}
 ...
 \frac{|\Phi(x)|}{g_{\mathrm{nat}}(x)}\Differential x
 =\mathscr P(y),
...
\begin{align}
 \int_{2^k}^{2^k(1+z)}\frac{|\Phi(x)|}{A_1g_1(x)}\Differential x
...
\begin{proof}
Apply \(\Differential/\Differential x=x^{-1}\Differential/\Differential(\log x)\) inductively to
...
\begin{equation}
 2^{-k}\int_{2^k}^{2^k(1+z)}\frac{|\Phi(x)|}{g(x)}\Differential x
 \longrightarrow z.
...
\[
 u(z)^{-1}\alpha_k(\Differential z)(1+o(1)).
\]
...
measure as
\[
 \eta_k(\Differential z)=\frac{q_k(z)\alpha_k(\Differential z)}{\int q_k\Differential\alpha_k},
 \qquad M^{-1}\le q_k\le m^{-1}.
\]
Thus \(\eta_k\le C\alpha_k\) in the sense that
\(\int\varphi\Differential\eta_k\le C\int\varphi\Differential\alpha_k\) for every
...
\begin{equation}
 \sup_{0\le z\le1}
 \AbsoluteValue{\int_0^zq_k(u)\alpha_k(\Differential u)-z}
 \longrightarrow0;
 \label{eq:q-equalize}
\end{equation}
...
\begin{equation}
 \frac1{t-t_0}\int_{t_0}^t\frac{|\Phi(x)|}{g_0(x)}\Differential x
 \longrightarrow1.
...
\(z\) gives
\begin{equation}
 \frac{\Differential}{\Differential z}\log g_1(2^k(1+z))
...
\[
 \frac{\Differential}{\Differential z}\log g_0
 =\frac{\Differential}{\Differential z}\log g_1-(\log q_k)'
 \le -b_k<0
\]
...
\[
 \frac{|\Phi(2^k(1+z))|}{g_0(2^k(1+z))}\Differential z
 =q_k(z)\alpha_k(\Differential z).
\]
Consequently \eqref{eq:q-equalize} gives, uniformly in \(z\),
\begin{equation}
 \int_{2^k}^{2^k(1+z)}\frac{|\Phi(x)|}{g_0(x)}\Differential x
 =2^k(z+o(1)).
...
\[
 \int_{t_0}^{t}\frac{|\Phi(x)|}{g_0(x)}\Differential x=t+o(t),
\]
...
\label{thm:Hoischen}
Let \(F\in C^1(\RealNumbers)\) and let \(\varepsilon:\RealNumbers\to(0,\infty)\) be continuous.
There is an entire function \(G\), real-valued on \(\RealNumbers\), such that
\begin{equation}
...
all of \(\RealNumbers\).  On a sufficiently large half-line, \(F'(x)<0\).  Choose a
...
\(G'(x)<F'(x)/2<0\), so \(g(x)=\EulerE^{G(x)}\) is positive and strictly
decreasing.  Also
\[
 \EulerE^{-\varepsilon(x)}
 \le\frac{g(x)}{g_0(x)}\le\EulerE^{\varepsilon(x)}.
\]
...
\[
 \beta_k
 =\int_{2^k}^{2^{k+1}}\frac{|\Phi(x)|}{g_1(x)}\frac{\Differential x}{x}
 =\rho^{-(k+1)}\int_0^1\frac{W(z)}{1+z}P_{k+1}(z)\Differential z.
\]
By \eqref{eq:weighted-P-limit},
...
\(P_n=2^{-n}\EulerE^{S_n}\) and \eqref{eq:factorization} one obtains, for fixed
\(1<y<2\),
\begin{equation}
 \log|\Phi(2^ky)|
...
\begin{equation}
 \int_0^1\psi(z)\psi(2^rz)\Differential z
 =\frac{\pi^2}{12\,2^r}.
 \label{eq:covariance}
\end{equation}
Therefore
...
\begin{equation}
 \int_0^1d(z)^2\Differential z=\frac{\pi^2}{4}.
 \label{eq:d-variance}
\end{equation}
...
\(\operatorname{Leb}\{|\psi|>u\}=O(\EulerE^{-u})\) makes the coboundary terms
...
\(\int_0^1\cos^2(2\pi jx)\Differential x=1/2\).  It consequently prints
...
For \(q=2\),
\begin{equation}
 \int_0^1P_n(z)^2\Differential z=2^{-n}.
 \label{eq:Pn-L2}
\end{equation}
One proof is the identity \(\LaplaceTransformSymbol_2\mathbf1=\tfrac12\mathbf1\); another is
...
\label{prop:gelfond}
For every \(n\ge1\) and \(z\in\RealNumbers\),
\begin{equation}
...
\begin{equation}
 \DecayOptimizationObjective{n}(r,s)
 =-\frac{z^2}{2\lambda}-n\EulerE^{-C_0}\EulerE^{-z}
...
\[
 z\EulerE^z=\lambda n\EulerE^{-C_0}=cn,
\]
...
\(\log A_n-n\beta\) to \(\sup\DecayOptimizationObjective{n}\) with error
\(O((\log\log n)^2)\).  The tiling argument in
\eqref{eq:C0-z}--\eqref{eq:F-z} gives
\[
 \sup\DecayOptimizationObjective{n}
...
\(p(1-t)\to1\) and whose maximum is asymptotic to \(1/(\EulerE p)\).
...
Chebyshev--Lobatto collocation of \(\LaplaceTransformSymbol\), with barycentric evaluation at the
...
\label{thm:complete}
Let
\(\Phi(x)=\prod_{n\ge0}\SincRad(\pi x/2^n)\).  Then:
\begin{enumerate}
...
For \(q>0\), let \(\rho_q\) be the leading eigenvalue of \(\LaplaceTransformSymbol_q\) and set
\begin{equation}
 \Lambda_q=\rho_q^{1/q},
 \qquad
...
\begin{itemize}
\item \(q\downarrow0\):
\(\log\Lambda_0=\int_0^1\log|\sin\pi z|\Differential z=-\log2\), giving
\(\kappa_0\).
...
2745497,
\newblock \url{https://github.com/VladimirReshetnikov/ProveIt/blob/main/Analysis/FabiusFunction/docs/semi-formalized-research-frontiers/drafts/rvachev_up_fourier_decay/asymptotic-decay-rate-of-an-infinite-product-of-sinc-functions/asymptotic-decay-rate-of-an-infinite-product-of-sinc-functions.tex}.
