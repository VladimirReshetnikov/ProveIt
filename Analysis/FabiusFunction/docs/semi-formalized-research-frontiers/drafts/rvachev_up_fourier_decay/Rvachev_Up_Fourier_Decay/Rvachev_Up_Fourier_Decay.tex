...
normalizing constant $A_1^{\log}=0.09386063575678126354\ldots$.
...
\[
 \widehat f(\xi)=\int_{\R}f(t)\e^{-2\pi\ii t\xi}\dd t,
\]
its transform is the entire function
\begin{equation}
 \Phiup(z)=\widehat{\Up}(z)
...
average of $\abs{\Phiup}/g$ tends to one or at least stays between two positive constants.
...
\begin{align*}
 \frac1{t-t_0}\int_{t_0}^{t}\frac{\abs{\Phiup(x)}}{g(x)}\dd x&\longrightarrow1
 &&\text{as }t\to\infty, \tag{Q2}\label{eq:Q2-intro}\\
 0<\liminf_{t\to\infty}\frac1{t-t_0}\int_{t_0}^{t}
 \frac{\abs{\Phiup(x)}}{g(x)}\dd x
 &\le\limsup_{t\to\infty}\frac1{t-t_0}\int_{t_0}^{t}
 \frac{\abs{\Phiup(x)}}{g(x)}\dd x<\infty. \tag{Q3}\label{eq:Q3-intro}
\end{align*}
...
\item The transform has a zero at every nonzero integer, so no asymptotic
      $\abs{\Phiup(x)}\sim g(x)>0$ can hold on the full real ray.
...
$|\Phi(2^ny)|=E_{\kappa_0}(2^ny)\exp(o(n))$ as $n\to\infty$, with asymptotic
...
\[
 ...
 \qquad
 \sum_{h\ge0}\frac{\abs z^2}{4^h}<\infty
\]
uniformly for $z$ in a compact set.  Hence $\Phiup$ is even, real on $\R$, entire, and
$\Phiup(0)=1$.

Vieta's product $\sinc t=\prod_{j\ge1}\cos(t/2^j)$ and Euler's sine product give two other
useful forms:
\begin{equation}
 \Phiup(z)
...
at level $h$ precisely when $2^h\mid m$, so its total multiplicity is $1+\vtwo(m)$.
...
\begin{equation}
 R_m(z)
 =\left(\frac{z^2}{4^m};\frac14\right)_\infty
 \exp\!\left(
   -\sum_{k=1}^{\infty}
...
\begin{equation}
 \Phiup(z)=
 \left(\prod_{n=0}^{m-1}\sinc\!\left(\frac{\pi z}{2^n}\right)\right)
 R_m(z).
...
\[
 \sinc(\pi w)=\frac{1}{\Gamma(1+w)\Gamma(1-w)},
\]
...
$\sinc(\pi w)/(1-w^2)$ is holomorphic and zero-free in $|w|<2$, equals $1$ at $0$, and its
...
not a support statement for $\Phiup$.  Outside it, formula
...
$m\in\Z\setminus\{0\}$ has multiplicity
\[
 d(m)=1+\vtwo(\abs m).
\]
Moreover, if $b\ge1$ is an integer, $d=1+\vtwo(b)$, and $z\uparrow b$, then
\begin{equation}
 \abs{\Phiup(z)}
 =b^{-d}\abs{\Phiup(b/2^d)}(b-z)^d\bigl(1+\bigO(b-z)\bigr).
...
$0\le h\le\vtwo(b)$.  At $z=b$ each has a simple zero and
\[
 \abs{\frac{\dd}{\dd z}\sinc\!\left(\frac{\pi z}{2^h}\right)\Big|_{z=b}}
 =\frac1b.
\]
The product of the remaining factors is
\[
 \prod_{h=d}^{\infty}
 \abs{\sinc\!\left(\frac{\pi b}{2^h}\right)}
 =\abs{\Phiup(b/2^d)}.
\]
...
\begin{equation}
 \frac{\abs{\Phiup(2^n y)}}{E_\kappa(2^n y)}
 =W_\kappa(y)
...
\[
 \Phiup(2^n y)=\Phiup(y)\prod_{j=1}^{n}\sinc(\pi2^j y).
\]
...
\begin{equation}
 P_n(x):=\prod_{j=0}^{n-1}(1-\e^{2\pi\ii2^jx})
 =\sum_{k=0}^{2^n-1}\tau_k\e^{2\pi\ii kx}
 \label{eq:TM-polynomial}
\end{equation}
follows by induction.  Taking moduli yields
\begin{equation}
 \abs{P_n(x)}=2^n\prod_{j=0}^{n-1}s(T^jx).
 \label{eq:TM-modulus}
\end{equation}
Thus the same products governing the decay of $\widehat{\Up}$ are the normalized moduli of
...
$1-\e^{2\ii u}$ to $-2\ii\e^{\ii u}\sin u$, and pair the terms indexed by
$r$ and $2^{m+1}-1-r$.  Binary complementation gives
$\tau_{2^{m+1}-1-r}=(-1)^{m+1}\tau_r$; taking the appropriate real or imaginary part yields
\[
 \prod_{j=0}^{m}\sin(2^j\theta)
 =2^{-m}\sum_{r=0}^{2^m-1}\tau_r
...
\[
 ...
 =\sum_{r=0}^{n}q^{r(r-1)/2}\qbinom{n}{r}{q}z^r
\]
...
coefficient $(-1)^{s_2(k)}$.  This is why Gaussian-binomial arithmetic and Thue--Morse
...
$m\ge1$, $\abs{\Phiup}$ has exactly one critical point
$\xi_m\in(m,m+1)$.  It is the strict maximum of $\abs{\Phiup}$ on that interval.  Moreover,
for every integer $m\ge0$,
\begin{equation}
 \sgn\Phiup(x)=(-1)^{s_2(m)},\qquad m<x<m+1.
 \label{eq:TM-sign}
\end{equation}
For $m\ge1$ the signed extremum is therefore
$\Phiup(\xi_m)=(-1)^{s_2(m)}M_m$, where
\[
 M_m:=\max_{m\le x\le m+1}\abs{\Phiup(x)}.
\]
\end{theorem}
...
\begin{align}
 ...
 &=-2x\sum_{k=1}^{\infty}\frac{1+\vtwo(k)}{k^2-x^2},
 \label{eq:log-derivative}\\
 \frac{\dd^2}{\dd x^2}\log\abs{\Phiup(x)}
 &=-2\sum_{k=1}^{\infty}(1+\vtwo(k))
...
$1+\vtwo(k)$ times.  Therefore the sign on $(m,m+1)$ is
\[
 (-1)^{\sum_{k=1}^{m}(1+\vtwo(k))}.
\]
Legendre's identity $\vtwo(m!)=m-s_2(m)$ gives
\[
 \sum_{k=1}^{m}(1+\vtwo(k))=m+\vtwo(m!)=2m-s_2(m),
\]
...
$\bigO(1+\log n)$.  Thus
\begin{equation}
 |F''(x)|\le\Lambda_n:=C_1(1+\log n)^5.
 \label{eq:peak-curvature-bound}
\end{equation}
Taylor's formula at the critical point gives
$F(\xi_n+s)\ge F(\xi_n)-\Lambda_ns^2/2$.  On the interval
\[
 |s|\le w_n:=\min\{\delta_*/2,\Lambda_n^{-1/2}\}
 \ge c_1(1+\log n)^{-5/2},
\]
we therefore have $|\Phiup(\xi_n+s)|\ge\e^{-1/2}M_n$.  Integration over
...
$M_n/\int_n^{n+1}|\Phiup|$ is far smaller than this worst-case bound and that its shell
...
\begin{equation}
 \mathcal M_{q,n}(H)
 :=\left(\frac1{2^{n-1}}\int_{2^{n-1}}^{2^n}\abs{H(x)}^q\dd x\right)^{1/q}
...
\begin{align}
 ...
 &:=\exp\!\left(\frac1{2^{n-1}}
       \int_{2^{n-1}}^{2^n}\log\abs{H(x)}\dd x\right),
 \label{eq:shell-L0}\\
 \mathcal M_{\infty,n}(H)
 &:=\sup_{2^{n-1}\le x\le2^n}\abs{H(x)}.
 \label{eq:shell-Linf}
\end{align}
...
\begin{equation}
 ...
 =\int_0^1(\mathcal L_q^n f)(x)\dd x.
...
$\|f\|_\infty+[f]_\alpha$, there is a spectral gap and
\begin{equation}
 \rho_q^{-n}\mathcal L_q^nf
 =\nu_q(f)h_q+\bigO(\vartheta_q^n\norm f_{C^\alpha}),
...
$\tau_0(x)=x/2$ and $\tau_1(x)=(x+1)/2$, and their weights as
...
doubling map and
\begin{equation}
 \int_0^1\log\sin(\pi x)\dd x=-\log2=-a.
...
\begin{equation}
 ...
 =\exp\!\left(\int_0^1
    \log W_{\kappa_0}\!\left(\frac{x+1}{2}\right)\dd x\right)
...
Equivalently,
\[
 \abs{\Phiup(2^ny)}=E_{\kappa_0}(2^ny)\e^{o(n)}.
\]
\end{corollary}
...
\begin{proof}
The tail estimate $\operatorname{Leb}\{|d|>u\}=O(\e^{-u})$ shows that
$d\in L^p(0,1)$ for every finite $p$.  With
$\mathcal F_j=T^{-j}\mathcal B$, the variables $d\circ T^j$ form a stationary reverse
martingale-difference sequence.  Moreover,
\[
 \frac1n\sum_{j=0}^{n-1}
 \mathbb E\!\left[(d\circ T^j)^2\mid\mathcal F_{j+1}\right]
...
$d\in L^2$.  For clarity, the extra LIL hypotheses are not being hidden: the predictable
quadratic variation is the displayed ergodic sum and hence is asymptotic to
$n\int d^2$, while the exponential tail and Borel--Cantelli give
$\max_{j\le n}|d\circ T^j|=O(\log n)=o(\sqrt{n/\log\log n})$ almost surely.  Truncating at
$\varepsilon\sqrt{n/\log\log n}$ and using the same tail bound gives the conditional
Lindeberg summability required by Stout's martingale LIL.  Thus the stationary martingale
CLT and the martingale LIL apply to $\sum_{j<n}d\circ T^j$; see Stout
\cite{stout1970} and the invariance-principle formulation of Heyde--Scott
\cite{heyde-scott1973}.  These two classical martingale theorems are the imported
probabilistic input.  The variance is \eqref{eq:d-variance}.  It remains only to remove
the two coboundary terms in \eqref{eq:Gordin-exact}.  The positive part of $\psi$ is bounded
by $\log2$, while near the integers
\[
 \operatorname{Leb}\{\psi<-u\}=O(\e^{-u}).
\]
Hence
...
factor
$\int_0^1\cos^2(2\pi mt)\dd t=1/2$, thereby doubling $\sigma^2$ from
...
\[
 \int_0^1\abs{P_n(x)}^2\dd x=2^n,
\]
while \eqref{eq:TM-modulus} gives
\begin{equation}
 \int_0^1\prod_{j=0}^{n-1}s(T^jx)^2\dd x=2^{-n}.
...
\[
 S_n(t):=\prod_{j=0}^{n-1}\abs{\sin(\pi2^jt)}.
\]
...
\begin{proof}
Let $N=2^n$ and $\omega=\e^{2\pi\ii/N}$.  Differentiating
$z^N-1=(z-1)\prod_{r=1}^{N-1}(z-\omega^r)$ at $z=1$ gives
\[
 N=\prod_{r=1}^{N-1}(1-\omega^r).
\]
Taking absolute values and using $|1-\e^{2\pi\ii r/N}|=2\sin(\pi r/N)$ proves
...
\[
 \sum_{m=0}^{N-1}\abs{P_n(m/N)}^2
 =N\sum_{r=0}^{N-1}|\tau_r|^2=N^2.
\]
...
\begin{align}
 \mathcal L_2\mathbf1&=\tfrac12\mathbf1,
...
\end{align}
For $e_m(x)=\e^{2\pi\ii mx}$ it also obeys
\begin{equation}
...
\begin{proof}
The first equality is Plancherel, using that $\Phiup=\widehat\Up$ is real on the real axis.
Expand the function $\Up$, supported in $[-1,1]$, in its length-two Fourier series.  Its
$k$th coefficient is $\tfrac12\widehat\Up(k/2)=\tfrac12\Phiup(k/2)$, so Parseval on
$[-1,1]$ gives
\[
 \int_{-1}^{1}\Up(t)^2\dd t
 =\frac12\sum_{k\in\Z}\Phiup(k/2)^2.
\]
The term $k=0$ contributes $1/2$.  Every nonzero even term vanishes because $\Phiup$ is
...
\]
The half-integer sampling identity above is a concise human-readable target for completing
that formalized energy layer.
...
growth root defined from $\sup_x\mathcal L_1^n\mathbf1(x)$:
\[
...
\begin{equation}
 ...
 \label{eq:g1}
\end{equation}
makes the mean of $\abs{\Phiup}/g_1$ on each dyadic shell tend to one.  Chebyshev
...
\begin{proof}
Put $u=\log x$ and $G(u)=g_1(\e^u)$.  Then
\[
...
\begin{equation}
 ...
 \frac{\abs{\Phiup(x)}}{E_{\kappa_1}(x)}\dd x<C.
...
\begin{equation}
 ...
 \frac{\abs{\Phiup(x)}}{A_1E_{\kappa_1}(x)}\dd x
...
$r(T^nx)\le\e^{\epsilon n}$ eventually for almost every $x$.  Choosing
...
\begin{equation}
 \frac1{t-X_0}\int_{X_0}^t\frac{|\Phi(x)|}{g(x)}\dd x\longrightarrow1
 \label{eq:Q2-final}
\end{equation}
cannot hold.
\end{theorem}
...
That is impossible.  If $\dd\nu_1=r\dd x$ had a nonzero absolutely continuous part, the
...
$r(T^nz)\le\e^{\varepsilon n}$ eventually for a.e. $z$, for every $\varepsilon>0$.
...
\begin{proof}
Put $a_k:=\mu_k([0,1])$ and $\bar\mu_k:=a_k^{-1}\mu_k$.  Then
...
\]
Since $\int q_k\dd\mu_k\ge a_k/M$, we have the correctly normalized domination
$\eta_k\le(M/m)\bar\mu_k$: for every continuous $\varphi\ge0$,
\[
 \int\varphi\dd\eta_k\le\frac{M}{m}\int\varphi\dd\bar\mu_k.
\]
...
\begin{equation}
 \sup_{0\le z\le1}
 \left|\int_0^zr_k\dd\mu_k-z\right|\longrightarrow0;
...
\[
 \int_I r_{k,j}\dd\mu_k-|I|
 =\int_{I\cap U_j}(r_{k,j}-c_{k,j,i})\dd\mu_k.
\]
...
\[
 \sup_z\left|\int_0^zr_{k,j}\dd\mu_k-z\right|\le3\varepsilon_j.
\]
...
\]
Replacing $C_k(1)$ by $1$ and $C_K(z)$ by $z$ costs $o(2^K)$: indeed
$\sum_{k<K}2^k\epsilon_k=o(2^K)$ whenever $\epsilon_k\to0$.  The main term is
...
We use the following precise classical input: if $F\in C^1(\R)$ and
$\epsilon:\R\to(0,\infty)$ is continuous, there is an entire function $G$, real on the
real axis, such that
\begin{equation}
 |G(x)-F(x)|<\epsilon(x),
 \qquad |G'(x)-F'(x)|<\epsilon(x)
 \quad(x\in\R).
 \label{eq:Hoischen}
\end{equation}
...
Extend $h$ in \eqref{eq:smooth-adaptive-gauge} to a positive, strictly increasing $C^1$
function on $\R$.  Choose a continuous $\delta(x)>0$ with $\delta(x)\to0$ as
$x\to+\infty$ and apply \eqref{eq:Hoischen} with
\begin{equation}
 \epsilon(x)<\frac12\min\{h(x),h'(x),\delta(x)h(x)\}.
 \label{eq:Hoischen-error}
\end{equation}
The real entire approximant $\widetilde h$ is positive and strictly increasing on $\R$,
...
slope are $o(k)$, so the gauge remains decreasing, yet its normalized shell shape never
...
\begin{equation}
 ...
 \frac{|\Phi(x)|}{E_{\kappa_1}(x)}\frac{\dd x}{x}
...
\begin{equation}
 B_1:=\left(\int_0^1h_1(x)\dd x\right)
 \nu_1\!\left(\frac{f_1}{1+\cdot}\right),
 \qquad
 A_1^{\log}:=\frac{B_1}{\log2}.
 \label{eq:A1log}
\end{equation}
Numerically,
\begin{equation}
...
\[
 ...
 \frac{f_1(z)}{1+z}\prod_{j=0}^{n-1}|\sin(\pi T^jz)|\dd z.
\]
...
\[
 s(x)^2s(Tx)=2\abs{\sin^3\theta\cos\theta},
\]
...
Consequently
\begin{equation}
 \lim_{n\to\infty}\norm{\prod_{j=0}^{n-1}s\circ T^j}_{\infty}^{1/n}
 =\lambda=\frac{\sqrt3}{2}.
...
$c>0$, $\e^{cx}\abs{\Phiup(x)}$ is unbounded.  Thus the transform decays faster than every
...
than $\e^{-cx}$ along that subsequence.
\end{proof}
...
\[
 \mathcal H(y):=\prod_{m=1}^{\infty}\left|\sinc\!\left(\frac{\pi y}{2^m}\right)\right|,
 \qquad y\in\R.
\]
Thus $\mathcal H(y)=|\Phiup(y/2)|$ globally.  In particular it is continuous and bounded;
...
\begin{equation}
 \theta=2^{-r}s,
 \qquad r\in\Z_{\ge0},\quad s\in I:=[1/4,1/2].
...
\end{equation}
The remaining tail satisfies $Q_m(s)\le C\e^{m\beta}$ by
...
\begin{equation}
 \log A_n=n\beta+
 \sup_{\substack{r\in\Z_{\ge0}\\s\in I}}\mathcal F_n(r,s)
 +O((\log\log n)^2).
...
$r=\lfloor\log_2n\rfloor$, $s=1/3$, whose tail lies exactly on the maximizing two-cycle and
whose logarithm is $n\beta-O((\log n)^2)$.  This proves that every phase within
$O((\log\log n)^2)$ of the maximum has $r<n$.

The same comparison yields the two uniform localization estimates used below.  Since
$s\in[1/4,1/2]$,
\[
 \mathcal F_n(r,s)\le-\frac a2r^2+C_1r,
 \qquad
 \log(1+s2^{-r})\ge\frac12s2^{-r}.
\]
Comparison with the trial value first gives $r=O(\log n)$ and then, after retaining the
last negative term in $\mathcal F_n$,
\begin{equation}
 ns2^{-r}=O((\log n)^2).
...
$\mathcal F_n$ and put
$q=\lceil4\log_2\log(n+\e^\e)\rceil$.  For large $n$ we have $n-r\ge q$.
Choose $s_q$ by \Cref{lem:dense-preimages-final}.  In the localized region
\[
 \left|\frac{\partial\mathcal F_n}{\partial s}\right|
 \le4r+n2^{-r}=O((\log n)^2),
\]
so $|\mathcal F_n(r,s_q)-\mathcal F_n(r,s)|=O(1)$.  The dense-preimage lower bound gives
\[
...
\[
 n\beta+\sup_{r,s}\mathcal F_n(r,s)-O(q^2).
\]
Since $q^2=O((\log\log n)^2)$, this completes both sides of
...
\begin{equation}
 ...
 \qquad
 c:=a\e^{-C_0}=\frac a\pi\sqrt{\frac32}.
 \label{eq:annular-c}
\end{equation}
...
$A^2/(2a)=O(1)$, the supremum is, up to $O(1)$, the real maximum of
\[
 \phi_n(z)=-\frac{z^2}{2a}-n\e^{-C_0}\e^{-z}.
\]
Its critical equation is $z\e^z=cn$, so the maximizer is $w_n=W(cn)$ and
...
\begin{equation}
 \theta_k=\frac{W(c(k+1))}{a(k+1)}\e^{o(1)}.
...
$\phi_n(w_n)-\phi_n(z)=O((\log\log n)^2)$.  Writing $z=w_n+u$ and using
$n\e^{-C_0-w_n}=w_n/a$, one has
\[
 \phi_n'(w_n+u)=\frac{w_n(\e^{-u}-1)-u}{a}.
\]
...
multiple of $w_n$.  Since $w_n\asymp\log n$ while $(\log\log n)^2=o(w_n)$, every maximizer
therefore satisfies
\[
 z=w_n+O\!\left(\frac{\log\log n}{\sqrt{w_n}}\right)=w_n+o(1).
\]
Since
$\theta=s2^{-r}=\e^{-C_0-z}$ and
$n\e^{-C_0-w_n}=w_n/a$, this gives \eqref{eq:annular-location}.
\end{proof}
...
$(m2^k,m2^k+1)$ as $m2^k+t_k$, $0<t_k<1$.  Then
\begin{equation}
...
\begin{equation}
 \boxed{\displaystyle
 M_{m2^k}\sim
 \frac{\mathcal H(1)\abs{\mathcal H(m)}}{\e(k+1)}
 (m2^k)^{-(k+1)}.}
 \label{eq:odd-multiple-amplitude}
\end{equation}
In particular,
\begin{equation}
 M_{2^k}\sim\frac{\mathcal H(1)^2}{\e(k+1)}2^{-k(k+1)}.
...
\begin{equation}
 ...
 =\left(\frac{t}{N+t}\right)^{k+1}
   \frac{\abs{\Phiup(t)}}{\abs{\Phiup(t/2^{k+1})}}
...
$\Phiup(t)$, the denominator quotient is $(t/(N+t))^{k+1}$, and reindexing the two tails
...
\begin{equation}
 \abs{\Phiup(N+t)}
 =N^{-(k+1)}\abs{\mathcal H(m)}\,t^{k+1}\Phiup(t)(1+o(1))
...
$\bigO((k+1)/N)+\bigO(2^{-k})=o(1)$, by local uniform convergence of the product and its
derivative.  Thus the critical-point equation below is perturbed only by $o(1)$.
On $[0,1)$ the function $\Phiup(t)$ is positive, and
\Cref{prop:zero-divisor} at the simple zero $1$ gives
\begin{equation}
 \Phiup(t)=\mathcal H(1)(1-t)(1+\bigO(1-t)).
 \label{eq:Phi-near-one}
\end{equation}

Let $p=k+1$.  A maximizer of $t^p\Phiup(t)$ must tend to $1$: values on
...
$p/t_p+\Phiup'(t_p)/\Phiup(t_p)=0$.  Equation \eqref{eq:Phi-near-one} implies
$(1-t)\Phiup'(t)/\Phiup(t)\to-1$, so $p(1-t_p)\to1$.  Consequently
$t_p^p\to\e^{-1}$ and
$p\Phiup(t_p)\to\mathcal H(1)$.  For the exact lobe expression, logarithmic
differentiation adds only the $o(1)$ term estimated above, so its critical equation gives
$p(1-t_k)=1+o(1)$ by the same argument.  Substitution into the uniform value asymptotic
...
$M_{N+r}=\abs{\Phiup(\xi_{N+r})}$.  Then, as $n\to\infty$,
\begin{align}
 \xi_{N+r}
 &=N+b-\frac{db}{n}+o(n^{-1}),
...
\begin{equation}
 M_{N+r}
 =\abs{\Phiup(1/2)}N^{-n}
   \max_{r\le z\le b}\bigl(z^n\abs{\Phiup(z)}\bigr)(1+o(1)).
...
$|\Phiup(z)|$ above and below by fixed positive multiples of $(b-z)^d$ on
$[b-\delta_0,b]$.  The trial point $z=b-db/n$ has size at least
$c\,b^{n+d}n^{-d}$ for large $n$, whereas $z\le b-\delta_0$ is exponentially smaller.
On the remaining interval put $u=n(b-z)/b$.  Since $(1-u/n)^n\le \e^{-u}$,
\[
 z^n\abs{\Phiup(z)}\le C b^{n+d}n^{-d}u^d\e^{-u}.
\]
After $U>d$ is chosen large, the supremum of $u^de^{-u}$ over $u\ge U$ is smaller
than the trial-point lower constant.  Thus every maximizer has $u$ in one fixed compact
interval.  Uniformly there,
\[
 z^n\abs{\Phiup(z)}
 =D_b b^{n+d}n^{-d}\,u^d\left(1-\frac un\right)^n(1+o(1))
 \longrightarrow
 D_b b^{n+d}n^{-d}u^d\e^{-u}.
\]
The function $u^d\e^{-u}$ has its unique maximum at $u=d$, with value $d^d\e^{-d}$.
Uniform convergence on the compact interval just obtained therefore gives
\[
...
\[
 \max z^n\abs{\Phiup(z)}
 \sim D_b b^{n+d}n^{-d}d^d\e^{-d}.
\]
Insert $D_b=b^{-d}\abs{\Phiup(b/2^d)}$ into
...
at least $\abs{\Phiup(x_n)}$, and \eqref{eq:exact-peak-ray} gives a positive lower bound
...
\begin{equation}
 ...
 \label{eq:Phi-b}
\end{equation}
Local uniform convergence follows from $\sinc w=1-w^2/6+\bigO(w^4)$ and
...
For every $b>1$ and $z\in\C$,
\begin{equation}
 \boxed{\displaystyle
 \Phi_b(z)=\prod_{\ell=1}^{\infty}
 \left(\frac{z^2}{\ell^2};b^{-2}\right)_\infty.}
 \label{eq:global-spectral-pochhammer}
\end{equation}
More generally, for the tail
$R_{b,m}(z):=\prod_{n=m}^{\infty}\sinc(\pi z/b^n)$,
\begin{equation}
 R_{b,m}(z)=\prod_{\ell=1}^{\infty}
 \left(\frac{z^2}{\ell^2b^{2m}};b^{-2}\right)_\infty.
...
If $b\ge2$ is an integer, the nonzero zeros of $\Phi_b$ are precisely the nonzero integers.
The zero at $M\ne0$ has multiplicity
\[
 1+\nu_b(|M|),
 \qquad
 \nu_b(M):=\max\{j\ge0:b^j\mid M\}.
\]
\end{corollary}
...
\[
 ...
 \qquad d_{J,k}:=\frac{\zeta(2k)-H_J^{(2k)}}{k}.
\]
...
$\zeta(2k)-H_J^{(2k)}=(J+1)^{-2k}(1+o(1))$, so the root test gives the stated exact
...
\[
 \Phi_b(b^Ny)=\Phi_b(y)\prod_{k=1}^{N}\sinc(\pi b^ky).
\]
...
Let $b\ge2$ be an integer and $1\le y<b$.  Assume $\Phi_b(y)\ne0$ and that
\[
...
\]
up to the constant $\log|\Phi_b(y)|$.  Substitute
...
$C_b(y)=\log|\Phi_b(y)|+(\log y)^2/(2\log b)
...
Let $b\ge2$ be an integer and let $S_N(t)=\sum_{j=0}^{N-1}\psi(T_b^jt)$.  Then, for
$j\ge0$,
\begin{equation}
 \int_0^1\psi(t)\psi(T_b^jt)\dd t=\frac{\pi^2}{12b^j},
...
of every finite order and exponential tails, hence the Borel--Cantelli maximal bound and
conditional Lindeberg summability checked in the proof of \Cref{thm:correct-lil}.  The same
imported stationary martingale CLT and Stout LIL therefore apply.  Finally the coboundary
...
\lean{RvachevPochhammerFactorization}; the entire contracting-ratio product is developed in
\lean{GeometricReciprocalGamma}; and the case $b=2$ of the shell and variance algebra is
formalized in \lean{SincProductShells} and \lean{BirkhoffVariance}.  The arbitrary-$b$
Pochhammer grouping, integer-$b$ ray law, and general covariance theorem above are new
human-readable results and explicit Lean targets.
\end{boxedremark}
...
valley theorems are proved here.  The exact $L^2$ row is finite Fourier algebra.  Three
classical analytic packages remain imported with their hypotheses visible:

\begin{enumerate}
\item Ruelle--Perron--Frobenius quasi-compactness and simplicity for
      \eqref{eq:transfer-operator};
\item stationary reverse-martingale CLT/LIL theorems;
...
$(\pi/\sqrt2+o(1))\sqrt{(\log x/\log2)\log\log\log x}$ along the LIL extremal
subsequences.
...
Symbol & Meaning\\
\midrule
$\Phiup=\widehat{\Up}$ & Fourier transform of Rvachev's up-function\\
$a$ & $\log2$\\
...
$M_m$ & unique maximum of $\abs{\Phiup}$ on $[m,m+1]$\\
\bottomrule
\end{tabularx}
\end{center}
...
Finite Thue--Morse product-to-sum identity
 ...
 & \Cref{eq:TM-polynomial,eq:TM-modulus}\\
...
Exact $\kappa$ dictionary, ordering, and closed-form interval certificates
...
Global dyadic Pochhammer and reciprocal-Gamma products (not the extracted tail series)
 ...
 & \Cref{thm:pochhammer-tail,prop:global-spectral-pochhammer}\\
Existence and uniqueness of the transfer-supremum growth root (not yet its RPF spectral
identification)
 & \decl{exists_perron_exponent}, \decl{exists_perron_root},
   \decl{existsUnique_perron_root}
 & \Cref{prop:RPF,prop:rho1-rigorous-bound}\\
Lean Bernstein enclosures of the growth root and its $\kappa$ image
 & \decl{perron_bracket_pointwise}, \decl{perron_root_enclosure},
   \decl{kappa_one_enclosure}
 & Separate-certificate remark after \Cref{prop:rho1-rigorous-bound}\\
\bottomrule
\end{longtable}

\subsection{Exact Lean-source-only results still owed a conventional proof here}
...
\midrule
\endhead
Rapid real-axis decay of the Fourier transform and all of its iterated derivatives
 & \decl{rvachevFourier_real_rapidDecay},
   \decl{rvachevFourier_real_iteratedDeriv_rapidDecay}\\
Typical logarithmic exponent in measure and uniform $\sqrt n$-scale tightness
 & \decl{measureReal_log_prod_ge_le}, \decl{tendsto_measureReal_log_prod_ge},
   \decl{measureReal_log_prod_sqrt_ge_le},
   \decl{integral_log_prod_two_sin}, \decl{integral_abs_log_prod_le}\\
...
\item the half-integer sampling equality in \Cref{prop:total-energy}, if not yet exported as
      a named theorem by the energy module.
\end{enumerate}

Every theorem promoted in the prose now appears in the exact, abstract, or unformalized
ledger above, while the three exact Lean-source-only families are visibly recorded as
