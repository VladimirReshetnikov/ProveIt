...
\(T\) at least the diameter of the support.  Folding the two-sided sum onto
\(m\geq0\) --- discarding the vanishing nonzero integer samples through
\decl{rvachevFourier_int_eq_ite} and reflecting the negative half through
\decl{rvachevFourier_neg} --- is bookkeeping about \(\sum\) over \(\mathbb Z\) that the
module does not yet perform, so the displayed \(\tfrac12+\sum_{m\ge0}\) form remains a
one-step human deduction from a formal identity.
...
as an instance of interval Parseval for any time-limited square-integrable function
 ...
 & \Cref{prop:total-energy} (second equality; the folding onto \(m\ge0\) is not formal)\\
...
\item the folding of the formal two-sided sampling identity of \Cref{prop:total-energy}
      onto the nonnegative half-integers (the \(\tfrac12+\sum_{m\ge0}\) display), which is
      \(\mathbb Z\)-summation bookkeeping on top of
      \decl{integral_sq_rvachevUp_eq_half_tsum_sq_rvachevFourier} rather than new analysis.
\end{enumerate}
