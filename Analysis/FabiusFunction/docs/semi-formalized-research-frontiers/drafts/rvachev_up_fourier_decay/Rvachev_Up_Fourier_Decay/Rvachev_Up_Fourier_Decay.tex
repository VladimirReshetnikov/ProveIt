...
\hypersetup{
  ...
  pdfauthor={A human-readable synthesis for the ProveIt repository},
...
\emph{Formal status.}  Formal, in division-free form, in module
\path{DyadicBoundaryIdentity.lean}:
\decl{rvachevFourierProduct_dyadic_boundary} states
\[
 (N+w)^{n}\,\FourierTwoPi{\RvachevUp}(N+w)\,
   \FourierTwoPi{\RvachevUp}\!\left(\frac{w}{2N}\right)
 =-w^{n}\,\FourierTwoPi{\RvachevUp}(w)\,
   \FourierTwoPi{\RvachevUp}\!\left(\frac12+\frac{w}{2N}\right),
 \qquad N=2^{\,n-1},
\]
for every complex $w$ and with no hypothesis at all, both sides being entire.  The displayed
quotient form is \decl{norm_rvachevFourierProduct_dyadic_boundary}, on the lemma's own range
$0<z<N$: there $\AbsoluteValue{z/(2N)}<1$, so
$\FourierTwoPi{\RvachevUp}(z/(2N))\ne0$ by the central-lobe positivity
\decl{norm_rvachevFourierProduct_pos}, and the division is legitimate.  It is the composite of two facts already in the corpus: the local
factorization at the dyadic center (\decl{rvachevFourierProduct_two_pow_mul_add_factorization}
at $q=1$), which clears the first $n$ sinc denominators and exposes the zero factor $w^{n}$,
and the shell factorization (\decl{rvachevFourierProduct_two_pow_mul}) applied to $w/2^{n}$,
which identifies the same prefix product with
$\FourierTwoPi{\RvachevUp}(w)/\FourierTwoPi{\RvachevUp}(w/2^{n})$ after reflecting the index
(\decl{prod_complexSinc_prefix_mul_rvachevFourierProduct}).  The proof of the lemma above,
which separates finite and infinite parts, is thus replaced by an identity between finite
products.
...
Exact zero-safe dyadic shell factorization and exponent algebra, and the
explicit $K$-shell envelope
$(\pi|y|)^K\,\AbsoluteValue{\FourierTwoPi{\RvachevUp}(y)}
 =2^{K(K-1)/2}\prod_{k<K}|\sin(\pi y/2^k)|\cdot
 \AbsoluteValue{\FourierTwoPi{\RvachevUp}(y/2^K)}$
as a hypothesis-free equality (including $y=0$), with the classical corollary
$\AbsoluteValue{\FourierTwoPi{\RvachevUp}(y)}\le2^{K(K-1)/2}(\pi|y|)^{-K}$
 & \decl{norm_rvachevFourierProduct_two_pow_mul_cross},
   \decl{norm_rvachevFourierProduct_two_pow_mul},
   \decl{shell_exponent_identity},
   \decl{norm_rvachevFourierProduct_shell_cross},
   \decl{norm_rvachevFourierProduct_shell_div},
   \decl{norm_rvachevFourierProduct_le_pow_div}
...
Sharp two-step logarithmic inequality \(\tfrac23\log s(x)+\tfrac13\log s(Tx)\le\log\lambda\),
in its cube form \(s(x)^2s(Tx)\le\lambda^3\), and the equality value along the orbit of
\(1/3\)
 & \decl{abs_sin_sq_mul_abs_sin_two_mul_le}, \decl{abs_sin_two_pow_third}
 & \Cref{lem:sharp-sine-inequality} (the ``precisely when'' clause, that equality forces
   \(x\equiv1/3,2/3\), is not formal)\\
Integer-ratio prefix factorization of \(\GeometricSincProduct{b}\) into its first
\(N\) sinc factors times \(\GeometricSincProduct{b}(b^{-N}y)\), and the all-orders zeta
expansion of that tail on \(|b^{-N}y|<1\)
 & \decl{geometricSincProduct_pow_mul}, \decl{geometricSincProduct_eq_cexp},
   \decl{geometricSincProduct_eq_prefix_mul_cexp},
   \decl{base_geometric_sinc_eq_prefix_mul_cexp}
 & \Cref{thm:b-adic-shell} (the algebraic core only: the potential \(\phi\) and its
   Birkhoff sum along \(T_b\) are not formal)\\
\bottomrule
\end{longtable}
...
The Collatz--Wielandt principle of \Cref{prop:rho1-rigorous-bound}: for any continuous
positive test function $f$ with $mf\le\mathcal L_1f\le Mf$ on $[0,1]$, $m\le\rho_1\le M$
(proved abstractly for every monotone positively homogeneous operator, then instantiated)
 & \decl{iterate_bracket}, \decl{collatzWielandt_bracket},
   \decl{perron_root_mem_of_two_sided}
 & The exact rational test function of \Cref{app:certificate} and the Sturm-theorem
   evaluation of $\inf$ and $\sup$ of $\mathcal L_1f/f$, which supply the concrete
   $m$ and $M$ of \eqref{eq:rho1-rigorous-bound}\\
\bottomrule
\end{longtable}
...
\item the strict human Collatz--Wielandt/Sturm bracket in
      \Cref{prop:rho1-rigorous-bound}: the Collatz--Wielandt \emph{principle} it rests on
      is now formal (\decl{perron_root_mem_of_two_sided}, from the abstract
      \decl{collatzWielandt_bracket}), so what remains is the Lean evaluation of the
      certificate's rational test function, i.e.\ the concrete $m$ and $M$; the existing
      Lean Bernstein certificate is a distinct, wider enclosure;
\item the integer-$b$ shell law, typical exponent, covariance, variance, and general
      martingale decomposition from \Cref{sec:integer-ratios}, and the integer-ratio zero
      divisor of \Cref{cor:integer-ratio-zero-divisor} (its dyadic instance is exact,
      \decl{analyticOrderAt_rvachevFourierProduct_int}; the base-\(b\) multiplicity
      \(1+\BasePowerValuation{b}(|M|)\) is not);
