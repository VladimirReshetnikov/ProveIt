...
\(
 \widehat f(\xi)=\int_{\RealNumbers}f(t)\EulerE^{-2\pi i t\xi}\Differential t,
\)
the normalized Rvachev up-function has Fourier transform
\[
 \PhiF(\xi)=\prod_{h=0}^{\infty}
 \SincRad\!\left(\frac{\pi\xi}{2^h}\right),
...
\[
 ...
 \qquad
 H(z)=\prod_{r=1}^{\infty}\SincRad\!\left(\frac{\pi z}{2^r}\right).
\]
...
Since
\(
 \SincRad w=1-w^2/6+\BigO(w^4)
\)
near the origin, the product in \eqref{eq:Phi-definition} converges locally
uniformly on \(\ComplexNumbers\).  It therefore defines an even entire function.  Shifting
the index gives the exact refinement law
\begin{equation}
 \PhiF(2z)=\SincRad(2\pi z)\PhiF(z),
 \qquad
 \PhiF(z)=\SincRad(\pi z)\PhiF(z/2).
 \label{eq:Phi-scaling}
\end{equation}
Repeated use of the first identity yields
\begin{equation}
 \PhiF(2^k y)=\PhiF(y)
 \prod_{j=1}^{k}\SincRad(\pi2^j y).
...
\begin{equation}
 H(z)=\prod_{r=1}^{\infty}
 \SincRad\!\left(\frac{\pi z}{2^r}\right).
 \label{eq:H-definition}
\end{equation}
Then
\begin{equation}
 \PhiF(z)=\SincRad(\pi z)H(z).
 \label{eq:Phi-H}
\end{equation}
...
\end{equation}
where \(\TwoAdicValuation(m)\) is the exponent of \(2\) in \(m\).  Indeed, a fixed
integer \(m\) occurs once for every \(h\) with \(2^h\mid m\), hence with
multiplicity \(1+\TwoAdicValuation(m)\).  Formula \eqref{eq:canonical-product} appears in
Arias de Reyna's treatment \cite{arias2017}.

Consequently, the real zeros are precisely the nonzero integers, and the zero
at \(m\ne0\) has multiplicity
\begin{equation}
 \operatorname{ord}_{m}\PhiF=1+\TwoAdicValuation(|m|).
...
\(|\sin u|\le1\), while for \(h>k\) use \(|\SincRad u|\le1\).  Then
\begin{equation}
 |\PhiF(x)|
...
\(|\PhiF(x)|=\BigO(\EulerE^{-x^\delta})\) along all sufficiently large real
...
then
\begin{equation}
 \prod_{j=0}^{n-1}(1-\EulerE^{2\pi i2^j t})
 =\sum_{m=0}^{2^n-1}(-1)^{s_2(m)}\EulerE^{2\pi i m t},
 \label{eq:TM-polynomial}
\end{equation}
and therefore
\begin{equation}
 2^nQ_n(t)=
 \left|\sum_{m=0}^{2^n-1}(-1)^{s_2(m)}\EulerE^{2\pi i m t}\right|.
...
and
\begin{equation}
 \int_0^1\log|\sin\pi t|\Differential t=-\log2=-\lambda.
...
For every \(t\in\RealNumbers\) and every integer \(n\ge1\),
\begin{equation}
 Q_n(t)
...
Consequently,
\begin{equation}
 \sup_{t\in\RealNumbers}\limsup_{n\to\infty}\frac1n\log Q_n(t)
 =\beta.
...
\begin{equation}
 ...
 \qquad
 H_r(s)=\prod_{h=1}^{r}
 \SincRad\!\left(\frac{\pi s}{2^h}\right).
...
\[
 \dist(T^j(s_q),\IntegerNumbers)\ge\frac{1}{3\cdot2^{q-j}}.
\]
The elementary inequality
\(
 |\sin\pi t|\ge2\dist(t,\IntegerNumbers)
\)
now gives
\[
...
remaining factor equals \(\sqrt3/2=\EulerE^\beta\).  Taking logarithms and
...
\begin{lemma}[Reduction to \(\DecayOptimizationObjective{n}\)]
\label{lem:An-Fn-reduction}
As \(n\to\infty\),
\begin{equation}
 \log A_n
 =n\beta+
 \sup_{\substack{r\in\IntegerNumbers_{\ge0}\\s\in I}}
 \DecayOptimizationObjective{n}(r,s)
...
\(Q_{n-r_0}(1/3)=\EulerE^{(n-r_0)\beta}\) in
...
\(\DecayOptimizationObjective{n}\).  We first record a quantitative localization.  With
\(r_0=\lfloor(\log n)/\lambda\rfloor\) and \(s=1/3\), one has
\(s2^{-r_0}=\BigO(n^{-1})\), and therefore
\[
 \DecayOptimizationObjective{n}(r_0,1/3)\ge-C_1(\log n)^2
\]
for an absolute constant \(C_1\).  On the other hand, because
\(\log(\pi s)-\lambda/2-\beta\) is bounded on \(I\),
\[
 \DecayOptimizationObjective{n}(r,s)\le-\frac\lambda2r^2+C_2r.
\]
...
Take
\[
 q=\left\lceil4\log_2(\log(n+\EulerE^\EulerE))\right\rceil
\]
...
\(|s_q-s_0|\le2^{-q}\).  The derivative of \(\DecayOptimizationObjective{n}(r,s)\) with
respect to \(s\), throughout \(I\), is bounded by \(\BigO((\log n)^2)\) at
the localized values in \eqref{eq:Fn-localization-crude}.  Hence
\[
 \DecayOptimizationObjective{n}(r,s_q)=\DecayOptimizationObjective{n}(r,s_0)+\BigO(1).
\]
...
resulting function by \(\LinearizedDecayObjective{n}(r,s)\).  At a maximizer of
\(\DecayOptimizationObjective{n}\), the localization argument in
\eqref{eq:Fn-localization-crude} gives
\(ns2^{-r}=\BigO((\log n)^2)\).  Therefore
\[
 0\le n\bigl(s2^{-r}-\log(1+s2^{-r})\bigr)
 \le\frac n2s^22^{-2r}
 =\BigO\!\left(\frac{(\log n)^4}{n}\right)=\LittleO(1).
\]
The same comparison applies to a maximizer of \(\LinearizedDecayObjective{n}\), so the two suprema
differ by \(\LittleO(1)\).
...
\begin{equation}
 ...
 \label{eq:z-variable}
\end{equation}
Since \(s=\EulerE^{A-C_0}\), a direct calculation gives
\begin{equation}
 \LinearizedDecayObjective{n}(r,s)
 =-\frac{z^2}{2\lambda}
 -n\EulerE^{-C_0}\EulerE^{-z}
 +\frac{A^2}{2\lambda}.
 \label{eq:F-z-form}
\end{equation}
...
\begin{equation}
 ...
 \SincRad\!\left(\frac{\pi t}{2^j}\right).
...
\[
 \left|
 \SincRad\!\left(\frac{\pi(N+t)}{2^j}\right)
 \right|
 =\frac{t}{N+t}
 \SincRad\!\left(\frac{\pi t}{2^j}\right).
\]
...
\(H(m)\ne0\).  Finally,
\begin{equation}
 \left(1+\frac{t}{N}\right)^{-(k+1)}=1+\LittleO(1).
...
\eqref{eq:boundary-local-limit}, maximize \(hs\EulerE^{-s}\).  This maximum is
unique and occurs at \(s=1\).  Hence \(s_p\to1\), and the maximum value is
asymptotic to \(h/(\EulerE p)\).
\end{proof}
...
\[
 |\PhiF(m2^k+t)|
 =(m2^k)^{-p}|H(m)|\,t^p\PhiF(t)(1+\LittleO(1)).
\]
...
\(\log|\PhiF(x)|=-(\log x)^2/(2\log2)
 -(3/2+\log\pi/\log2)\log x+\LittleO(\log x)\).\\[0.4em]
...
\begin{equation}
\begin{split}
 \int_{2^k}^{2^{k+1}}|\PhiF(x)|\Differential x
 ={}&2^k\pi^{-n}2^{-k(k+1)/2}\,I_n,\\
 I_n={}&\int_0^1
 H(1+\theta)(1+\theta)^{-n}Q_n(\theta)\Differential\theta.
\end{split}
...
\[
 \sum_{h=0}^{H}\log\left|
 \SincRad\!\left(\frac{\pi x}{2^h}\right)\right|
\]
on \((j,j+1)\).  A tail bound from
\(
 \log\SincRad u=-u^2/6+\BigO(u^4)
\)
