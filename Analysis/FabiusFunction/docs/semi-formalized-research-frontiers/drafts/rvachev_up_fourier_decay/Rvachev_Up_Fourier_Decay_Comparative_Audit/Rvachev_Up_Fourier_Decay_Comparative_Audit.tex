...
exact gaps.  The $\kappa_1$ row is deliberately absent there: the Perron
root exists in the library only existentially, so every Lean statement
about $\kappa_1$ carries it as a hypothesis.  What the dictionary does not
...
The $\kappa_1$ consequence remains unformalized (it needs interval
bounds on $\log\pi$, $\log2$, and $\log\varrho_1$).
...
dictionary (\lean{KappaDictionary}); the two-sided $\kappa_\infty$
