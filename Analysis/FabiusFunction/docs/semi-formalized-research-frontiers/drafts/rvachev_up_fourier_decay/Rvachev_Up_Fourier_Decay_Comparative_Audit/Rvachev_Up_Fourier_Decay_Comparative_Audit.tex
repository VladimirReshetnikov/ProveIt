...
one, where $f(x)=\prod_{n\ge0}\SincRad(\pi x/2^n)$ is the Fourier transform of
...
gauge---can attain the limit along all $t$: the limit
points trace a nonconstant dyadic-mantissa profile (grid-resolved here to
$[0.9249,1.1133]$; finer quadrature at $y=8/7$ and $10/7$ finds narrow
excursions reaching $\approx[0.9163,1.1181]$---see the second-wave
postscript), and constancy of the profile would force the
...
$\varrho_1\ge3^{-1/2}>\tfrac12$.  The second-wave documents later proved
that a deliberately \emph{shell-adaptive} analytic decreasing gauge does
attain the unrestricted limit, so the impossibility is a class
restriction, not an absolute one (\cref{sec:second-wave}).  We add one repair, with proof: with the
same gauge, the \emph{logarithmic} mean
$\frac{1}{\log(t/t_0)}\int_{t_0}^t\frac{|f(x)|}{g(x)}\frac{\Differential x}{x}$
...
$\Variance(S_{1000})=2459.5$ against the predictions $2464.1$ for variance
...
$\int_\RealNumbers f^2=\tfrac12+\sum_mf(m+\tfrac12)^2=0.808873535967972$ closes to
$2\cdot10^{-15}$.
\end{abstract}
...
Throughout, $\SincRad z=\sin z/z$, $\SincRad 0=1$, $a=\log2$, and
\begin{equation}
 f(x)=\prod_{n=0}^{\infty}\SincRad\!\left(\frac{\pi x}{2^n}\right),
 \qquad x\in\RealNumbers.
 \label{eq:f-def}
\end{equation}
Under the convention $\widehat h(\xi)=\int h(t)\EulerE^{-2\pi\ImaginaryUnit\xi t}\Differential t$ one
has $f=\widehat{\RvachevUp}$, the Fourier transform of Rvachev's up-function; in
...
each nonzero integer $m$ with multiplicity $1+\TwoAdicValuation(|m|)$, has exactly one
...
\S``The sign of the Fabius sinc product'').  All three background facts
are formalized in this same normalization, and are cited here rather than
reproved.  \emph{Multiplicity}: the dyadic zero lattice meets the integer
$m\ge1$ exactly $1+\TwoAdicValuation(m)$ times (\lean{LatticeFiber}) and
$\|\Phi(x)\|/|x-m|^{1+\TwoAdicValuation(m)}$ tends to an explicit positive constant
along the punctured neighbourhood of $m$
(\lean{PhiZeroOrder}, \lean{tendsto\_norm\_div\_pow\_zero\_order}), with
the odd and $2^{j}$ extremes as corollaries (\lean{ValleyZeroOrders});
the Lean statements are for $m\ge1$, the negative side following by
evenness.  \emph{One peak}: $\log\|\Phi\|$ is strictly concave on every
lobe (\lean{OnePeakPerLobe}), which with continuity of $\Phi$ on the real
axis gives \emph{exactly one} maximizer of $|\Phi|$ per side lobe
(\lean{RvachevProductContinuity}, \lean{existsUnique\_isMaxOn\_lobe}) and
the central peak exactly at the origin (\lean{CentralLobePeakAtZero}) ---
the equivalent maximizer form of ``one critical point per lobe'', which is
what the library states.  \emph{Sign}:
\lean{rvachevFourierProduct\_eq\_thueMorse\_sign\_mul\_norm}
(\lean{ThueMorseLobeSign}), built on \lean{PhiRealSign},
\lean{LobeSignCount} and \lean{LobeSignSelfSimilarity}.
...
\begin{equation}
 \lim_{t\to\infty}\frac{1}{t-t_0}\int_{t_0}^{t}\frac{|f(x)|}{g(x)}\Differential x=1
 \tag{Q2}\label{eq:Q2}
\end{equation}
for large enough $t_0$, or at least
\begin{equation}
 \exists\,0<A<B\ \forall t>t_0:\quad
 A<\frac{1}{t-t_0}\int_{t_0}^{t}\frac{|f(x)|}{g(x)}\Differential x<B.
 \tag{Q3}\label{eq:Q3}
\end{equation}
...
$\frac{1}{2^k}\int_{2^k}^{2^ky}|f|/g\,\Differential x\to y-1$ for every
...
Lebesgue-typical dyadic ray
 ...
 & holds a.e.\ with $\LittleO(\log x)$ error; the error is genuinely random,
...
root mean square
 ...
 & exact identity $\int_0^1\prod_{j<n}\sin^2(\pi2^jt)\Differential t=2^{-n}$; this is
...
The dictionary map itself is formalized.  \lean{KappaDictionary} defines
$\kappa(\Lambda)=\tfrac12+\log_2(\pi/\Lambda)$ (\lean{kappaOf},
\lean{kappaOf\_eq}), proves it strictly decreasing in the per-shell mean
$\Lambda$ (\lean{kappaOf\_strictAntiOn}), and evaluates it at the three
closed-form means $\Lambda=\tfrac12,\ \sqrt2/2,\ \sqrt3/2$ --- that is, at
$\kappa_0$, $\kappa_2$, $\kappa_\infty$ (\lean{kappaOf\_half},
\lean{kappaOf\_sqrt\_two}, \lean{kappaOf\_sqrt\_three}), together with the
exact gaps.  Since \lean{Fabius.log\_pi\_bracket} became available, all
three also carry numerical enclosures, in
\lean{KappaConstantEnclosures}:
\[
 \begin{aligned}
  3.1514961287&\le\kappa_0     \le3.1514961300,\\
  2.6514961287&\le\kappa_2     \le2.6514961300,\\
  2.3590148784&\le\kappa_\infty\le2.3590148796,
 \end{aligned}
\]
(\lean{Fabius.kappa\_zero\_bracket}, \lean{Fabius.kappa\_two\_bracket},
\lean{Fabius.kappa\_inf\_bracket}), of widths $1.3$, $1.3$ and
$1.2\cdot10^{-9}$, so seven decimals of $\kappa_0$ and $\kappa_2$ and
eight of $\kappa_\infty$ are kernel-checked --- the extra decimal being
where the values fall against a decimal boundary, not extra precision.
Because $\kappa_0$ and $\kappa_2$ differ from $\log_2\pi$ by an additive
rational, that one quantity is bracketed once
(\lean{Fabius.logTwo\_pi\_bracket}) and both are read off with no
further loss.  These use Mathlib's $\log2$ and $\log3$ in their
tightest form, \lean{Real.log\_two\_near\_10} and
\lean{Real.log\_three\_near\_10}, rather than the \lean{\_d9} decimal
corollaries; what dominates the residual width is $\log\pi$, whose own
$5\cdot10^{-10}$ width is inherited from the \lean{\_d9} form of
$\log3$ used to prove it.  The $\kappa_1$ row is deliberately absent
from the dictionary: the Perron root exists in the library only
existentially, so every Lean statement about $\kappa_1$ carries it as a
hypothesis (see \lean{KappaOneEnclosure}, below).  What the dictionary does not
supply is the \emph{attribution} of each $\kappa$ to its interpretation;
that is formal so far only for $\kappa_\infty$, pinned from both sides by
\lean{GlobalDecayEnvelope} and \lean{PeakRayEnvelope}, and, in measure
rather than almost everywhere, for $\kappa_0$
(\lean{KappaZeroInMeasure}, \lean{FluctuationTightness}).

Two further interpretation issues are settled by the problem statement
itself.  The convention $\SincRad z=\sin z/z$ (argument already containing
$\pi$) is forced by the stated zero set $\IntegerNumbers_{>0}$ and by the plot in the
question.  And the normalization matters only through trivial rescalings:
$f(x)=\widehat{\RvachevUp}(x)$ exactly in the $\EulerE^{-2\pi\ImaginaryUnit x t}$ convention, so
...
$f(2x)=\SincRad(2\pi x)f(x)$, and the obstruction to \eqref{eq:Q2} disappears
...
\[
 ...
 \qquad
 H(y)=\prod_{m=1}^{\infty}\Bigl|\SincRad\frac{\pi y}{2^m}\Bigr|.
\]
Then
\begin{equation}
 |f(2^ky)|
...
numerator scale $\EulerE^{-c}$ into the power
$\kappa=\tfrac12+c/a$.  The problem statement's own finite expansion
\[
 \prod_{n=0}^{m}\SincRad\frac{\pi x}{2^n}
...
trigonometric polynomial $\sum_{r<2^n}(-1)^{s_2(r)}\EulerE^{2\pi\ImaginaryUnit rt}$.
The identity is not merely spot-checked but proved: it is
\lean{LacunaryProductToSum}'s
\lean{prod\_sin\_two\_pow\_eq\_thueMorse\_sum},
$2^m\prod_{j\le m}\sin(2^jw)=\sum_{n<2^m}\varepsilon_n
\sin(m\pi/2+(2n+1)w)$, for every $m$ and every real $w$.
...
Let $\LaplaceTransformSymbol_1$ be the weighted doubling transfer operator
\begin{equation}
 (\LaplaceTransformSymbol_1 h)(x)
 =\frac12\Bigl[\sin\Bigl(\frac{\pi x}{2}\Bigr)h\Bigl(\frac x2\Bigr)
 +\cos\Bigl(\frac{\pi x}{2}\Bigr)h\Bigl(\frac{x+1}{2}\Bigr)\Bigr],
 \qquad 0\le x\le1,
 \label{eq:L1}
\end{equation}
so that $\int_0^1P_n(t)q(t)\Differential t=\int_0^1(\LaplaceTransformSymbol_1^nq)(x)\Differential x$, and let
...
\begin{align}
 ...
 \label{eq:gauge}
\end{align}
The displayed Perron value comes from collocation with $N=16$--$40$ and is
stable to $2\cdot10^{-15}$.
Here $A_1$ is the limit of the dyadic-shell means of $|f|/g_1$
...
Lemma~5.1 of \cite{ahl2017}; Document 4's exact-arithmetic
...
by Sturm sequences alone; and the kernel-checked Lean enclosure
$0.66126798\le\varrho_1\le0.66134921$ certifies the same test
function through subdivided Bernstein certificates
(\lean{PerronRootEnclosure}, \cref{ssec:certificate}).
...
$\mathscr P$ of item (iii), numerically containing
$[0.91627,1.11814]$ (the step-$0.025$ grid of \cref{fig:profile}
resolves only $[0.9249,1.1133]$; see \cref{sec:second-wave}).
...
\[
 ...
 \mathscr F(z)
 =\lim_{n\to\infty}
 \frac{\int_0^zW(t)P_n(t)\Differential t}{\int_0^1W(t)P_n(t)\Differential t},
\]
...
eigenmeasure of $\LaplaceTransformSymbol_1$; it is continuous and \emph{singular}, so
$\mathscr F(z)\ne z$ and $\mathscr P\not\equiv1$.  Consequently no gauge
whose within-shell shape stabilizes---in particular no Hardy-field
gauge, with or without a continuous log-periodic
modulation---can satisfy \eqref{eq:Q2}; the second wave strengthened
this to all bounded-distortion gauges, and conversely constructed a
shell-adaptive analytic gauge that does attain \eqref{eq:Q2}
(\cref{sec:second-wave}).  Measured profile:
grid minimum $0.9249$ near $y=1.15$, maximum $1.1132$ near $y=1.43$
(\cref{fig:profile}); narrow excursions near $y=8/7$ and $10/7$ reach
$0.91627$ and $1.11814$.
...
constant $A_1^{\log}=0.0938606357568\ldots$ (corrected here; the value
$0.0938610$ printed in the first edition of this audit was a
finite-shell running mean, wrong in its seventh figure ---
\cref{sec:second-wave}),
\[
 \lim_{t\to\infty}\frac{1}{\log(t/t_0)}
 \int_{t_0}^{t}\frac{|f(x)|}{A_1^{\log}g_1(x)}\,\frac{\Differential x}{x}=1
...
$\LaplaceTransformSymbol_1$ (see \cref{ssec:imported} for the exact division between proved
...
\emph{Formal status.}  The purely limit-theoretic cores of
\cref{thm:audit-answer} are kernel-checked in the Lean development, each
stated for an arbitrary cumulative mass function with the RPF input left
as a hypothesis; they therefore make the theorem \emph{conditionally}
formal, and are not the same thing as the statements above being
formalized.  Item (i) is \lean{BoundednessSkeleton}'s
\lean{eventually\_bounded\_of\_monotone\_of\_dyadic}: a monotone mass $G$
with the dyadic normalization $G(2^K)/2^K\to A>0$ satisfies
$A/4\le G(t)/t\le3A$ for all large $t$ --- a two-sided bracket, weaker
than the sharp compact limit set asserted in (i).  The shell arithmetic of
(iii) is \lean{CesaroProfileSkeleton}'s
\lean{tendsto\_cumulative\_profile}: given the completed-shell limit $A$
and the terminal partial-shell limit $P$ at mantissa $y$, the Ces\`aro
ratio along $t=2^ky$ converges to $(A+P)/y$ --- exactly the profile
$\mathscr P$ whose nonconstancy is the obstruction, with the aggregation
step supplied by \lean{GeometricCesaro}
(\lean{tendsto\_geom\_shell\_average}, itself a corollary of the general
weighted Toeplitz theorem \lean{tendsto\_weighted\_average}).  Item (iv)
is \lean{LogMeanSkeleton}'s \lean{tendsto\_sum\_add\_bdd\_div}.  What none
of them contains is precisely the imported block of \cref{ssec:imported}:
the RPF convergence that supplies the shell limits, and with it the
singularity of $\nu$ that makes the profile nonconstant.  The library's
Perron layer stops short of that --- existence of the root
$\varrho_1\in[\tfrac12,\sqrt2/2]$ by Fekete
(\lean{PerronRootExistence}) and the Lasota--Yorke pair for $\LaplaceTransformSymbol_1$
(\lean{TransferOperatorStep}, \lean{LasotaYorkeIteration}) are formal, the
spectral gap and the eigenfunction are not.
...
\begin{proof}
\eqref{eq:Q2} says $F(t)=(t-t_0)(1+\LittleO(1))$.  Insert $t=2^ky$ and
$t=2^k$ and subtract:
$F(2^ky)-F(2^k)=2^k\bigl(y-1+\LittleO(1)\bigr)$, the error uniform for
$y\in[1,2]$ since $y$ is bounded.
\end{proof}

\Cref{lem:reduction} is formalized, and in greater generality:
\lean{ShellLinearity}'s \lean{tendsto\_shell\_increment} proves
$\bigl(F(2^k(1+z))-F(2^k)\bigr)/2^k\to z$ for an \emph{arbitrary}
$F:\RealNumbers\to\RealNumbers$ and every $z\ge0$, not only for $y=1+z\in[1,2]$, with the ray
normalization $F(2^kc)/2^k\to c$ as
\lean{tendsto\_ray\_div\_two\_pow}.  The uniformity in $y$ noted in the
proof above is not part of the Lean statement, which fixes $z$ and takes
the limit in $k$.
...
Both halves of \cref{prop:bracket} are formalized.  The lower half is
\lean{le\_mul\_integral\_prod\_abs\_sin\_two\_pow}
(\lean{GelfondLogisticBound}), which is the statement above; the upper
half is \lean{SincMeanBracket}'s
\lean{integral\_prod\_abs\_sin\_two\_pow\_le}, which proves the
\emph{sharper} $I_1(n+1)\le(\sqrt3/2)^{n}$ --- its pointwise input being
the sharp uniform bound \lean{abs\_prod\_sin\_two\_pow\_le\_sharp}
(\lean{SharpGelfondBound}) rather than the $\sqrt{5/3}$-constant bound
used here.  So the Lean bracket
$3^{-1/2}\le\liminf I_1(n)^{1/n}\le\limsup I_1(n)^{1/n}\le\sqrt3/2$ is
this proposition's conclusion, reached by a slightly better route.

\begin{proposition}[exact $L^2$ identity]
\label[proposition]{prop:L2}
$\int_0^1P_n(t)^2\Differential t=2^{-n}$ for every $n\ge1$.
\end{proposition}
...
Formalized, and more generally than stated: \lean{LacunaryRieszIntegral}
proves the master orthogonality theorem
\lean{integral\_cos\_mul\_rieszProduct} for arbitrary Hadamard-gap integer
frequencies, amplitudes and phases, with \cref{prop:L2} the corollary
\lean{integral\_prod\_sin\_sq\_two\_pow} --- valid there for every
$n\ge0$ --- and \lean{integral\_prod\_sin\_sq\_pow} the same identity for
an arbitrary integer base $b\ge2$.
...
\[
 ...
 =\frac{1+\LittleO(1)}{c_k\varrho_1^{\,k+1}}
 \int_{z_1}^{z_2}q(z)P_{k+1}(z)\Differential z .
\]
By the RPF convergence for $\LaplaceTransformSymbol_1$ (\cref{ssec:imported}), for every
continuous $\phi$,
...
$P_n(z)=\exp\bigl(-n(\log 2+\LittleO(1))\bigr)$ for Lebesgue-a.e.\ $z$
(the potential $\log s$ is integrable with mean $-\log2$), so on
$\{r>0\}$, a set of positive measure,
$r(\T^nz)=r(z)\exp\bigl(n(\log(2\varrho_1)+\LittleO(1))\bigr)\to\infty$
geometrically, using $\log(2\varrho_1)>0$ from \cref{prop:bracket}.  On
the other hand $\T$ preserves Lebesgue measure, so for any
$\varepsilon>0$,
$\mathrm{Leb}\bigl(r\circ\T^n>\EulerE^{\varepsilon n}\bigr)
=\mathrm{Leb}\bigl(r>\EulerE^{\varepsilon n}\bigr)
\le\|r\|_1\EulerE^{-\varepsilon n}$, which is summable; by Borel--Cantelli
$r(\T^nz)\le\EulerE^{\varepsilon n}$ eventually for a.e.\ $z$.  Choosing
...
$\EulerE^{-(h(2^k(1+z))-h(2^k))}$ degenerates monotonically, and---using that
...
\[
 ...
 \frac{|f(x)|}{g_1(x)}\,\frac{\Differential x}{x}
...
\begin{proof}
Let $\beta_k=\int_{2^k}^{2^{k+1}}\frac{|f|}{g_1}\frac{\Differential x}{x}$.  By the
exact factorization,
$\beta_k=\varrho_1^{-(k+1)}\int_0^1\frac{W(z)}{1+z}P_{k+1}(z)\Differential z$, and
the RPF convergence applied to the continuous weight $W(z)/(1+z)$ gives
\[
 \beta_k\longrightarrow
 \beta_\infty
 :=\nu\Bigl(\frac{W}{1+\cdot}\Bigr)\int_0^1h\,\Differential\mathrm{Leb}
 \;\in(0,\infty);
\]
in particular $(\beta_k)$ is bounded.  For $2^K\le t<2^{K+1}$,
\[
 \int_{t_0}^{t}\frac{|f|}{g_1}\frac{\Differential x}{x}
...
The bookkeeping of this proof --- everything after ``$\beta_k\to
\beta_\infty$'' --- is formalized as \lean{LogMeanSkeleton}'s
\lean{tendsto\_sum\_add\_bdd\_div}: if $\beta_k\to L$ and the terminal
partial shell $\gamma$ and endpoint correction $\delta$ are bounded, then
$\bigl(\sum_{k<K}\beta_k+\gamma_K\bigr)/(K+\delta_K)\to L$.  The
convergence $\beta_k\to\beta_\infty$ itself is the RPF input and is not
formalized, so \cref{thm:logmean} is formally backed only conditionally.
...
$I_1(n)=\kappa\varrho_1^{\,n}(1+\LittleO(1))$
...
allows a subexponential drift $I_1(n)/\varrho_1^{\,n}=\EulerE^{\LittleO(n)}$,
which a fixed power $x^{-\kappa_1}$ cannot absorb.
\item \emph{The one genuinely imported analytic block:}
the RPF spectral picture for $\LaplaceTransformSymbol_1$ (existence of $h>0$, $\nu$, and
$\varrho_1^{-n}\LaplaceTransformSymbol_1^n\phi\to h\,\nu(\phi)$ for continuous $\phi$) on a
...
while the unrestricted \eqref{eq:Q2} is unattainable for every gauge
whose within-shell shape stabilizes or stays within bounded distortion
of $g$: the last dyadic block never forgets the singular way in which
the mass of $|f|$ is distributed inside a block.  (A deliberately
shell-adaptive analytic gauge escapes this, as the second wave later
proved: \cref{sec:second-wave}.)  The asker's guess
...
$0.6613\in(0.5,0.7071)$.  (The Cauchy--Schwarz half is formalized as
\lean{LacunaryMeanSharp}'s
\lean{integral\_prod\_abs\_sin\_two\_pow\_le\_sqrt\_two},
$I_1(n)\le(\sqrt2/2)^n$, proved there for arbitrary Hadamard-gap integer
frequencies and every integer base $b\ge2$; it is strictly better than the
$(\sqrt3/2)$-route upper bound of \cref{prop:bracket}.)  Second, the deleted draft's exponent
...
\[
 ...
 \qquad \xi_n\in(n,n+1),
\]
per lobe, with signed value $(-1)^{s_2(n)}E_n$ (audited, and both halves
formalized --- \lean{existsUnique\_isMaxOn\_lobe} and
\lean{rvachevFourierProduct\_eq\_thueMorse\_sign\_mul\_norm}, see
\cref{sec:object}; the sign law is also the Atlas's ``sign of the Fabius
sinc product'').  Reading the
...
\begin{equation}
 (\log|f|)'(x)=-2x\sum_{k\ge1}\frac{1+\TwoAdicValuation(k)}{k^2-x^2},
 \qquad
 (\log|f|)''(x)=-2\sum_{k\ge1}(1+\TwoAdicValuation(k))\frac{k^2+x^2}{(k^2-x^2)^2}<0,
 \label{eq:lobe-derivatives}
\end{equation}
so every lobe of $|f|$ is strictly log-concave with a unique peak.  That
conclusion is formalized, by a route that does not pass through
\eqref{eq:lobe-derivatives}: \lean{LobeLogFactorization} proves the
lattice log-series identity
$\log\|\Phi(x)\|=\sum_{(h,r)}\log\bigl(1-x^2/(2^h(r+1))^2\bigr)$ at every
non-lattice real $x$, \lean{CentralLobeConcavity} and
\lean{SideLobeConcavity} give strict negativity of its termwise second
derivative on the central and on every side lobe, and
\lean{OnePeakPerLobe} assembles them.  The central-lobe sum and regrouping identities
underlying \eqref{eq:lobe-derivatives} are now stated, for $|x|<1$, in
\lean{LobeDerivativeFormulas}
(\lean{Fabius.tsum\_central\_log\_deriv\_eq},
\lean{Fabius.tsum\_central\_log\_second\_deriv\_eq}), and the gap between
the two indexings turns out to be combinatorial rather than analytic: it
is the same fibre equivalence \lean{Fabius.dyadicFactorEquiv} that turns
the scale product into the canonical product.  The fibre of
$(h,r)\mapsto2^h(r+1)$ over $k$ has exactly $\TwoAdicValuation(k)+1$ elements and the
summand depends only on the fibre base, so the lattice sum collapses to
the integer sum with that multiplicity.  This is stated once for an
arbitrary summand (\lean{Fabius.tsum\_lobeZero\_regroup}), which is the
reusable part: \emph{any} absolutely convergent sum over the dyadic zero
lattice is the sum over the positive integers weighted by $\TwoAdicValuation(k)+1$.
The two central-lobe derivative formulas are instances.  The unrestricted identities away
from all integers remain report-level statements; the all-lobe concavity and one-peak
conclusions above are formalized through the separate factorization route.  Summability of the two
families had to be supplied, since \lean{CentralLobeConcavity} establishes
it only inside a proof and does not export it; both are dominated by
$1/(2^h(r+1))^2$, because $|x|<1$ and $L\ge1$ give
$L^2-x^2\ge L^2(1-x^2)$.  The displayed identities away from every integer,
including a closed side-lobe form, remain prose obligations; the separate
formal side-lobe results establish concavity and the unique peak without
asserting those global sums.
The following comparison is the workhorse of the mean layer; it does not
appear in any of the four documents, and it is not formalized either ---
the log-concavity input is, the quantitative $\delta_{\min}$ and
curvature bookkeeping is not.
...
$d_0=1+\TwoAdicValuation(n)$, $d_1=1+\TwoAdicValuation(n+1)$, $\delta_0=\xi_n-n$,
...
beyond, $1+\TwoAdicValuation(k)\le1+\log_2k$) gives, for $x$ in the middle segment
...
log-concavity, $|f(\xi_n\pm s)|\ge E_n\EulerE^{-\Lambda s^2/2}\ge E_n/\EulerE$ for
...
$E_{m2^k}\sim\frac{H(1)|H(m)|}{\EulerE(k+1)}(m2^k)^{-(k+1)}$, whose leading
...
\[
 \frac{\sum_{2^k\le n<2^{k+1}}E_n}{\int_{2^k}^{2^{k+1}}|f|\,\Differential x}
...
$2^jJ$ keeps distance at least $k^2\,2^{j-k}$ from $\IntegerNumbers$.  For each $j$
...
$j\le k'$ gives at most $C2^k/k^2=\LittleO(2^k)$ irregular cells.  On a
...
\[
 \max_{J}P_{k+1}\le\EulerE^{\pi/k}\,P_{k'}(z_J)
...
the Lebesgue equilibrium; with the RPF package for $\LaplaceTransformSymbol_q$ on
...
$\{P_{k'}\ge\EulerE^{-(1-\varepsilon)k\log2}\}$ by $\EulerE^{-c(\varepsilon)k}$;
...
$2^k\EulerE^{-c(\varepsilon)k}=\LittleO(2^k)$.  For such good cells,
$E_n\le C\cdot\text{shell factor}\cdot\EulerE^{-(1-\varepsilon)k\log2}$ lies
...
ensemble because, outside the $\LittleO(2^k)$ irregular cells, all points
...
Both corrections are $\BigO(\log^2 k)=\LittleO(\sqrt k)$, as is the
...
$\sum_{r<2^{k+1}}(-1)^{s_2(r)}\EulerE^{2\pi\ImaginaryUnit rz}$ near the dyadic frequency
...
$\omega=\EulerE^{2\pi\ImaginaryUnit/N}$, taken in modulus.  It says that the mean of
...
(ii) Let $p(t)=\prod_{j<n}(1-\EulerE^{2\pi\ImaginaryUnit2^jt})$, of degree $2^n-1$ with
...
Both identities are formalized, each in a more general form.  (i) is
\lean{SineProductRootsUnity}'s \lean{prod\_two\_sin\_pi\_div\_two\_pow},
a specialization of \lean{prod\_two\_sin\_pi\_div}, which proves
$\prod_{r=1}^{N-1}2\sin(\pi r/N)=N$ for every $N\ge1$.  (ii) is
\lean{DiscreteLacunaryParseval}'s
\lean{sum\_prod\_sin\_sq\_two\_pow}, a specialization of the master
discrete orthogonality theorem \lean{sum\_cos\_mul\_rieszProduct}, which
holds for arbitrary Hadamard-gap integer frequencies, amplitudes and
phases on any grid of size $N\ge m_n$ --- the dyadic case
$N=2^n$, $m_j=2^j$ sitting at the edge of that admissible resolution.
Both Lean statements are for every $n\ge0$.
...
$x^{-\kappa_0+\LittleO(1)}\mathcal E(x)$ --- two different formulas,
necessarily.
...
$(f/g)^2$ is tame.  Because $f=\widehat\RvachevUp$, the shell RMS of $f$ is the
...
\begin{proposition}[exact subleading spectrum of $\LaplaceTransformSymbol_2$]
\label[proposition]{prop:L2-spectrum}
The operator
$(\LaplaceTransformSymbol_2h)(x)=\tfrac12[\sin^2(\tfrac{\pi x}2)h(\tfrac x2)
+\cos^2(\tfrac{\pi x}2)h(\tfrac{x+1}2)]$ satisfies, exactly,
\[
 \LaplaceTransformSymbol_2\mathbf 1=\tfrac12\,\mathbf 1,
 \qquad
 \LaplaceTransformSymbol_2\bigl[\sin2\pi x\bigr]=-\tfrac14\sin2\pi x,
 \qquad
 \LaplaceTransformSymbol_2\bigl[1+3\cos2\pi x\bigr]=-\tfrac14\bigl(1+3\cos2\pi x\bigr),
\]
and on even Fourier modes it acts by frequency halving,
$\LaplaceTransformSymbol_2\EulerE^{2\pi\ImaginaryUnit\,2m x}\mapsto\tfrac12\EulerE^{2\pi\ImaginaryUnit mx}$.
\end{proposition}

\begin{proof}
With $e_m(x)=\EulerE^{2\pi\ImaginaryUnit mx}$: for even frequency $2m$,
$\EulerE^{2\pi\ImaginaryUnit\,2m\frac{x}{2}}=e_m(x)$ on both branches and
$\sin^2+\cos^2=1$ gives $\LaplaceTransformSymbol_2e_{2m}=\tfrac12e_m$ (the $m=0$ case is
the first display).  For $m=1$,
\[
 \LaplaceTransformSymbol_2e_1
 =\tfrac12\EulerE^{\pi\ImaginaryUnit x}\bigl[\sin^2(\tfrac{\pi x}2)
 -\cos^2(\tfrac{\pi x}2)\bigr]
 =-\tfrac12\EulerE^{\pi\ImaginaryUnit x}\cos\pi x
 =-\tfrac14\bigl(e_1+\mathbf 1\bigr),
\]
and likewise $\LaplaceTransformSymbol_2e_{-1}=-\tfrac14(e_{-1}+\mathbf1)$.  Hence
$\{\mathbf1,e_1,e_{-1}\}$ spans an invariant subspace on which $\LaplaceTransformSymbol_2$
has matrix eigenvalues $\{\tfrac12,-\tfrac14,-\tfrac14\}$; the real
eigenfunctions for $-\tfrac14$ are
$\sin2\pi x=(e_1-e_{-1})/2\ImaginaryUnit$ and $1+3\cos2\pi x$ (direct check:
$\LaplaceTransformSymbol_2[3\cos2\pi x]=-\tfrac34(1+\cos2\pi x)$, so
$\LaplaceTransformSymbol_2[1+3\cos]=\tfrac12-\tfrac34-\tfrac34\cos=-\tfrac14(1+3\cos)$).
\end{proof}

All four displays of \cref{prop:L2-spectrum} are formalized in
\lean{RMSTransferEigenfunctions}, in exactly this normalization of
$\LaplaceTransformSymbol_2$: \lean{rms\_transfer\_const\_eigen},
\lean{rms\_transfer\_sin\_eigen},
\lean{rms\_transfer\_one\_add\_cos\_eigen}, and the mode halving in its
real form, \lean{rms\_transfer\_cos\_even\_mode} together with
\lean{rms\_transfer\_sin\_even\_mode}.  They are stated pointwise in $x$
--- elementary trigonometry, no operator object and no spectral theory ---
so what is formal is the eigen-\emph{identities}, not the claim that
$-\tfrac14$ is the subleading eigenvalue, which rests on the collocation
bound below.
...
$-0.13435\pm0.0500\ImaginaryUnit$ of modulus $0.1434$, rate
...
$t$ fails --- for the same reason as at $q=1$: the $\LaplaceTransformSymbol_2$-eigenmeasure
$\nu_2$ is atomless and singular;
(iii) the logarithmic mean
$\frac1{\log(t/t_0)}\int_{t_0}^t(f/G_2)^2\,\frac{\Differential x}{x}$ converges
for all $t\to\infty$.
\end{theorem}
...
gives $P_n(z)^2=\EulerE^{-2n\log2(1+\LittleO(1))}$, so on $\{r>0\}$ the
density grows like $\EulerE^{n(\log2+\LittleO(1))}$, contradicting
...
$\varrho_q\EulerE^{q\log2}>1$, i.e.\ whenever $\kappa_q<\kappa_0$; at
...
\[
 \int_{\RealNumbers}f(x)^2\Differential x=\int_{\RealNumbers}\RvachevUp(t)^2\Differential t
...
\begin{proof}
The first equality is Plancherel ($f=\widehat\RvachevUp$, real and even).  For
the second, expand $\RvachevUp$, supported in $[-1,1]$, in Fourier series on
$[-1,1]$: the coefficients are $\tfrac12\widehat\RvachevUp(k/2)=\tfrac12f(k/2)$,
and Parseval gives
$\int\RvachevUp^2=\tfrac12\sum_{k\in\IntegerNumbers}f(k/2)^2$; the even $k\ne0$ terms vanish
...
\cref{ssec:variance}); $\int_0^1\log^2|\tan\pi x|\Differential x=\pi^2/4$;
...
\begin{multline*}
 ...
 \qquad c=\tfrac{\lambda}{\pi}\sqrt{\tfrac32}.
\end{multline*}
...
$E_{m2^k}\sim\frac{H(1)|H(m)|}{\EulerE(k+1)}(m2^k)^{-(k+1)}$ for odd $m$, a
...
\emph{Defects.}
(i) \textbf{Its Theorem 4.2 gives the constant $\pi$ in the sharp upper
metric fluctuation:}
\[
 \limsup_n\frac{S_n(t)}{\sqrt{n\log\log n}}=\pi
 \quad\text{a.e.}
\]
The quotation is a
...
every fixed offset $r$ (with the zero-multiplicity $d=1+\TwoAdicValuation(r+1)$
...
$\int_0^1\cos^2(2\pi jx)\Differential x=\tfrac12$,
\[
 c_0:=\int_0^1\psi^2=\frac12\sum_{j\ge1}\frac1{j^2}=\frac{\pi^2}{12},
\]
and for $r\ge1$, since
$\int_0^1\cos(2\pi jx)\cos(2\pi k2^rx)\Differential x=\tfrac12\,
\mathbf 1_{\{j=2^rk\}}$,
\[
 c_r:=\int_0^1\psi(x)\,\psi(2^rx\bmod1)\Differential x
...
\Cref{thm:variance} is formalized exactly as stated, and for every $m$
including the empty sum $m=0$: \lean{BirkhoffVariance}'s
\lean{integral\_sq\_birkhoff\_sum\_all} in Birkhoff-sum form, and
\lean{BirkhoffProductBridge}'s
\lean{integral\_sq\_log\_prod\_two\_sin\_all} in the product form
$\int_0^1\log^2\bigl|\prod_{k<m}2\sin(\pi2^kt)\bigr|$.  Its two inputs are
formal as well: the covariance ladder $c_r=(\pi^2/12)2^{-r}$ for every lag
$r$, over the genuine mod-one doubling map
(\lean{CovarianceLadder}, \lean{integral\_cocycle\_covariance}), and both
Clausen values, $\int_0^1\log^2(2\sin\pi t)\Differential t=\pi^2/12$
(\lean{CocycleParseval}, \lean{integral\_sq\_log\_two\_sin\_pi\_mul}) and
$\int_0^1\log^2|\tan\pi t|\Differential t=\pi^2/4$
(\lean{GordinParseval}, \lean{integral\_sq\_log\_abs\_tan\_pi\_mul}).
This is therefore the finite-$m$ kernel of \cref{find:ahl} in
machine-checked form; the limit theorems that consume it are not (see
after \cref{thm:lil-correct}).
...
\[
 ...
 \text{equivalently,}\qquad \limsup_{L\to\infty}
 \frac{\log\prod_{\ell=0}^{L}|2\sin\pi2^\ell\alpha|}
 {\sqrt{L\log\log L}}&=\frac{\pi}{\sqrt2}=2.2214\ldots .
 \end{aligned}
\]
\end{theorem}
...
$d(x)=\log|\tan\pi x|$, and
$\int_0^1d^2=\tfrac2\pi\int_0^{\pi/2}\log^2\tan u\,\Differential u
...
almost-surely linear growth $T_L/L\to\Expectation d^2$ by the ergodic theorem)
gives
$\limsup\bigl(\sum_{\ell<L}d\circ\T^\ell\bigr)/\sqrt{2L\log\log L}
=\sqrt{\Expectation d^2}=\pi/2$ a.e.  The boundary terms are almost surely
$\LittleO(\sqrt{L\log\log L})$: $\psi\le\log2$ above, while
$\mathrm{Leb}(\psi<-u)\le\tfrac2\pi\EulerE^{-u}(1+\LittleO(1))$ gives
$\sum_L\Probability(\psi\circ\T^L<-2\log L)<\infty$, so by Borel--Cantelli
...
The Gordin algebra of this proof is formalized in
\lean{DoublingCocycleIdentities}: the halving law $P\psi=\psi/2$
(\lean{log\_two\_sin\_half\_add}), the identification
$d=2\psi-\psi\circ\T=\log|\tan\pi t|$
(\lean{two\_log\_two\_sin\_sub}), and the annihilation $Pd=0$
(\lean{log\_abs\_tan\_half\_add}); with
$\int_0^1d^2=\pi^2/4$ (\lean{GordinParseval}) this makes every ingredient
of the variance formal.  \Cref{thm:lil-correct} itself is not: Mathlib has
neither the stationary-ergodic martingale central limit theorem nor its
law of the iterated logarithm, so the step from the exact variance to the
constant $\pi/\sqrt2$ remains a named import.
...
$(\log L)^{-(\pi-\varepsilon)^2/(2\sigma^2)}=(\log L)^{-2+\LittleO(1)}$
...
$\limsup|\sum\EulerE^{2\pi\ImaginaryUnit hX_k}|/\sqrt{2N\log\log N}=1/\sqrt2$
...
\midrule
$\Variance(S_{20})$, $4\cdot10^5$ samples & $45.66$ & $46.06$ & $92.12$\\
$\Variance(S_{34})$, $4\cdot10^5$ samples & $80.41$ & $80.60$ & $161.20$\\
$\Variance(S_{1000})$, $4\cdot10^6$ samples & $2459.5$ & $2464.1$ & $4928.2$\\
\bottomrule
\end{tabular}
\end{center}
The covariance $\int_0^1\psi(x)\psi(2x)\Differential x$ was measured as
...
$\log|{\SincRad}|$ over levels $h\le\log_2(\pi x)+30$ with a quartic Taylor
...
independently, as $\varrho_1^{-n}\int_0^1\LaplaceTransformSymbol_1^n(W)\,$ by collocation
...
$[0.9249,1.1133]$ on a step-$0.025$ grid; exact-point quadrature at
$y=8/7$ and $10/7$ (located by the second wave, confirmed in the
second-wave audit) reaches $0.91627$ and $1.11814$, so the grid
under-resolves the singular profile's narrow excursions.  This is the
numerical content of ``\eqref{eq:Q3} yes, stabilized \eqref{eq:Q2}
no''.

\subsection{The logarithmic mean}
\label{ssec:logmean}

$\frac{1}{\log(t/t_0)}\int_{t_0}^{t}|f|/g_1\cdot x^{-1}\Differential x$ at the same
mantissas:
\begin{center}
\small
\begin{tabular}{@{}rcccc@{}}
...
\end{tabular}
\end{center}
All columns approach the common constant
$A_1^{\log}=0.09386064$ (the first edition printed the running mean
$0.0938610$, whose $\BigO(1/K)$ transient had not died out;
corrected --- \cref{sec:second-wave}), at the
...
The reduction the certificate rests on is formal.  \lean{CollatzWielandt}'s
\lean{perron\_root\_mem\_of\_two\_sided} proves that any continuous $h$
that is positive on $[0,1]$ and satisfies $m\,h\le\LaplaceTransformSymbol_1h\le M\,h$ there
brackets the growth root, $m\le\varrho_1\le M$ --- with no eigenfunction,
$\LaplaceTransformSymbol_1$ being the operator \eqref{eq:L1} verbatim; and in its abstract
form \lean{collatzWielandt\_bracket} the argument mentions neither
$\LaplaceTransformSymbol_1$ nor $[0,1]$ nor sine and cosine, using only monotonicity and
positive homogeneity.  The numerical half is now formal as well, by a
different and Sturm-free route: \lean{PerronRootEnclosure} carries the
same cubic test function through the tangent half-angle dictionary
$t=\tan(\pi x/4)$ and certifies the two degree-$12$ integer-polynomial
inequalities on $[0,1]$ with $32$-piece subdivided Bernstein
certificates (\lean{BernsteinPositivity}), yielding the kernel-checked
enclosure $0.66126798\le\varrho_1\le0.66134921$.  Only the last two
digits of the SymPy enclosure above are given up; recovering them
needs on the order of two thousand pieces, since the endpoints above
approximate the exact range of $\LaplaceTransformSymbol_1h/h$ to within $10^{-8}$.
The $\kappa_1$ consequence is now formalized as well, in
\lean{KappaOneEnclosure}, and it is precisely the three interval bounds
just named that it had to supply.  Two of them were new.  Mathlib
brackets $\log2$ and $\log3$ but carries no bound on $\log\pi$; that is
proved here to eight decimals,
\[
 1.1447298855<\log\pi<1.1447298860
 \qquad(\lean{Fabius.log\_pi\_bracket}),
\]
by anchoring at the \emph{rational} $c=3.1415926535$ --- so that
$3/c=1-283185307/6283185307$ makes the Taylor partial sum of
\lean{Real.abs\_log\_sub\_add\_sum\_range\_le} a numeral --- and closing
the gap from $c$ to $\pi$ with \lean{Real.pi\_gt\_d20} below and
\lean{Real.pi\_lt\_d20} above.  Anchoring at $1-3/\pi$ directly, the
obvious route, is not formalizable at all: that point is irrational, so
the partial sum is not a numeral and no \lean{norm\_num} can evaluate
it.  The logarithms at the two rational Perron endpoints follow from
$\varrho=\tfrac23\cdot\tfrac32\varrho$, which lets Mathlib's $\log2$
carry the scale.  Combining them with the dictionary and its
antitonicity gives
\[
 2.7480118\le\kappa_1\le2.7481893
 \qquad(\lean{Fabius.kappa\_one\_enclosure}),
\]
so three decimals of $\kappa_1$ are kernel-checked.  Of the width
$1.775\cdot10^{-4}$, the $\varrho_1$ bracket contributes $99.84\%$ and
the four logarithm brackets only $0.004\%$: sharpening $\kappa_1$ is
entirely a question of sharpening $\varrho_1$.  The orientation is worth
recording, since it is easy to get backwards --- the dictionary is
antitone, so the \emph{lower} Perron endpoint yields the \emph{upper}
bound on $\kappa_1$.  The audit's own
$\kappa_1=2.74807001487133267\ldots$ lies strictly inside but is not
proved.

\subsection{Exact spot identities}

\begin{itemize}
\item $I_2(n)=2^{-n}$ was verified to $10^{-12}$ for $n=2,5,8,11$.
\item The slow-ray identity
\[
 |f(2^k\tfrac43)|
 =|f(\tfrac43)|\left(\frac{3\sqrt3}{8\pi}\right)^k2^{-k(k+1)/2}
\]
was verified to $10^{-14}$ at $k=6$.
\item Document 4's peak-ray constant
$W_{\kappa_\infty}(2/3)=0.139129774734829$ was reproduced to all digits,
and its ray identity was verified to $10^{-13}$ at $n=9$.
\item The collocation eigenvalues $\varrho_2=\tfrac12$ (exact to
$10^{-15}$), $\varrho_3$, and $\varrho_4$ agree with Document 4.
\end{itemize}
...
recover with \texttt{git show} from commit \texttt{e1cdd6260}; the archived
file is named \texttt{Decay rate conjecture.tex})
claimed
\[
...
$\Sign f|_{(N,N+1)}=\varepsilon_N$, the finite Fourier product, the
...
$E(t)=\prod_j(1-\EulerE^{-2^jt})$, including the remark that a bounded
...
its neighbors carry the small-argument $-\log^2$ layer.  The decay layer
proper is by now substantially covered as well: the shell factorization
and the $\kappa$-algebra (\lean{SincProductShells}); the sharp Gelfond
bound and both mean brackets (\lean{SharpGelfondBound},
\lean{SincMeanBracket}, \lean{LacunaryMeanSharp}); the exact variance
package (\lean{BirkhoffVariance}, \lean{BirkhoffProductBridge},
\lean{CovarianceLadder}, \lean{CocycleParseval}, \lean{GordinParseval});
the lobe geometry --- zero orders, one peak, Thue--Morse sign
(\lean{PhiZeroOrder}, \lean{OnePeakPerLobe},
\lean{RvachevProductContinuity}, \lean{ThueMorseLobeSign}); the exponent
dictionary (\lean{KappaDictionary}) and the certified enclosures of all
four exponents (\lean{KappaOneEnclosure},
\lean{KappaConstantEnclosures}); the two-sided $\kappa_\infty$
envelope (\lean{GlobalDecayEnvelope}, \lean{PeakRayEnvelope}); and the
hypothesis-carrying skeletons of the mean theorems
(\lean{ShellLinearity}, \lean{BoundednessSkeleton},
\lean{CesaroProfileSkeleton}, \lean{LogMeanSkeleton}).  The gap between
that list and this document is the RPF analytic block and the martingale
limit theorems, exactly as \cref{ssec:imported} predicts.
...
\textbf{Done}, in more generality: \lean{GeometricScaleProducts} proves
the renormalization law for geometric-scale infinite products
$\prod_n g(z/c^n)$ over any field into any commutative topological
monoid (\lean{tprod\_geom\_scale}, \lean{tprod\_geom\_scale\_pow},
induction-free), together with the monoid-valued tail-multipliability
direction missing from Mathlib
(\lean{Multipliable.of\_nat\_add}); \lean{SincProductShells}
specializes to the standalone sinc product
(\lean{rvachevFourierProduct\_two\_pow\_mul}), proves the modulus shell
factorization with the triangular power
(\lean{norm\_rvachevFourierProduct\_two\_pow\_mul},
\lean{prod\_range\_two\_pow\_succ}), and formalizes the
$\kappa$-algebra as \lean{shell\_exponent\_identity}.
\item $I_2(n)=2^{-n}$: finite Parseval over the Thue--Morse block,
adjacent to \lean{ThueMorseBlockAlgebra}.
\textbf{Done}, in more generality than stated here: the module
\lean{LacunaryRieszIntegral} proves the master orthogonality theorem
\lean{integral\_cos\_mul\_rieszProduct} for arbitrary gap-$2$ integer
frequencies, amplitudes, and phases (every Fourier coefficient of a
lacunary Riesz product below the spectral floor), with the identity of
\cref{prop:L2} as the corollary \lean{integral\_prod\_sin\_sq\_two\_pow}
via arbitrary bases (\lean{integral\_prod\_sin\_sq\_pow}).
...
\textbf{Done}, in more generality: \lean{GelfondLogisticBound} proves
the product bound along \emph{every} orbit of the full logistic map
(\lean{logistic\_prod\_le}), specializes to the dyadic sine products
(\lean{prod\_sin\_sq\_two\_pow\_le},
\lean{abs\_prod\_sin\_two\_pow\_le}) with sharpness at $t=1/3$
(\lean{prod\_sin\_sq\_two\_pow\_third}), and combines with the $L^2$
identity to formalize the lower bracket of \cref{prop:bracket}
($I_1(n)\ge\sqrt{3/5}\,3^{-n/2}$, hence $\varrho_1\ge3^{-1/2}>\tfrac12$)
as \lean{le\_mul\_integral\_prod\_abs\_sin\_two\_pow} --- the elementary
input to the singularity argument of \cref{thm:impossible}.
...
machine-readable.
\textbf{Done}, at marginally wider constants and with no Sturm module
at all: \lean{PerronRootEnclosure} exhibits Document 4's optimized
cubic test function and certifies
\[
 0.66126798\le\varrho_1\le0.66134921
\]
(\lean{perron\_root\_enclosure}, \lean{exists\_perron\_root\_enclosure})
by discharging the two positivity checks with $32$-piece subdivided
Bernstein certificates --- exact integer coefficients, verified by ring
normalization and literal sign checks alone
(\lean{BernsteinPositivity}, \lean{nonneg\_on\_Icc\_of\_pieces}).  The
reduction underneath is the previously formal
\lean{perron\_root\_mem\_of\_two\_sided}.  The SymPy enclosure of
\cref{ssec:certificate} is ${\approx}10^{-7}$ sharper; its endpoints
sit within $10^{-8}$ of the exact range of $\LaplaceTransformSymbol_1h/h$, so reaching
them by subdivision needs roughly two thousand pieces (the method
converges quadratically in the piece width) --- a mechanical, if bulky,
sharpening.
\item The valley asymptotics (elementary boundary-maximum analysis).
\textbf{The local half is done}: the zero order at $m$ is exactly
$1+\TwoAdicValuation(m)$ with an explicit positive constant
(\lean{LatticeFiber}, \lean{ZeroOrderTail}, \lean{PhiZeroOrder}), and
the two extremes --- simple zeros at odd integers, order $j+1$ at $2^j$,
hence unbounded order along the dyadic integers --- are
\lean{ValleyZeroOrders}.  The asymptotic constant of the valley law
$E_{m2^k}\sim\frac{H(1)|H(m)|}{\EulerE(k+1)}(m2^k)^{-(k+1)}$ is not
formalized.
...
\textbf{Done}, in the exact finite-$m$ form of \cref{thm:variance}
rather than only in the limit: \lean{BirkhoffVariance}
(\lean{integral\_sq\_birkhoff\_sum\_all}) and
\lean{BirkhoffProductBridge}
(\lean{integral\_sq\_log\_prod\_two\_sin\_all}) prove
$\int_0^1S_m^2=\frac{\pi^2}{4}m-\frac{\pi^2}{3}(1-2^{-m})$ for every
$m$, including $m=0$, on the covariance ladder
\lean{CovarianceLadder} and the two Clausen values
\lean{CocycleParseval}, \lean{GordinParseval}, with the Gordin algebra
in \lean{DoublingCocycleIdentities}.  This item was still listed as an
open target in earlier editions of this audit.
...
That recommendation has been carried out for the mean theorems: the
limit-theoretic cores of \cref{thm:audit-answer}(i),(iii) and
\cref{thm:logmean} are formal for an arbitrary cumulative mass with the
RPF layer left as a hypothesis (\lean{BoundednessSkeleton},
\lean{CesaroProfileSkeleton}, \lean{LogMeanSkeleton},
\lean{GeometricCesaro}), as is \cref{lem:reduction}
(\lean{ShellLinearity}).  On the operator side, existence of the growth
root $\varrho_1\in[\tfrac12,\sqrt2/2]$ by Fekete
(\lean{PerronRootExistence}) and the Lasota--Yorke pair
(\lean{TransferOperatorStep}, \lean{LasotaYorkeIteration}) are formal;
the spectral gap, the eigenfunction, the eigenmeasure and its
singularity, and the martingale limit theorems are not, and remain the
frontier.
\end{enumerate}
...
logarithmic mean --- and, as the second wave later proved, exactly and
for all times under the ordinary mean too, by a necessarily
shell-adaptive analytic gauge outside the natural class
(\cref{sec:second-wave}).  This composite answer --- not any single formula ---
...
The OCR copy of \cite{ahl2017} is annotated at (1.4)--(1.7), Lemma 4.1
and (4.5) per the repository's policy on evident errors in stored
papers, and the same directory now carries a corrected critical edition
of the paper whose constants agree with \cref{find:ahl} and whose
further repairs are catalogued in the postscript of \cref{sec:lil};
communicating the correction to the authors remains recommended.
Retire the four drafts after absorption, per the inbox policy; the
accessible companion (\nolinkurl{Rvachev_Up_Fourier_Decay_Gentle_Guide/})
should survive absorption as the entry point for readers.
\end{enumerate}

\section{Postscript: the second wave (27 August 2026)}
\label{sec:second-wave}

After this audit was written, four further documents
(\nolinkurl{rvachev_up_fourier_decay-5} through \nolinkurl{-8}), each
composed with access to it, arrived and were audited separately in
\nolinkurl{../Rvachev_Up_Fourier_Decay_Second_Wave_Audit/}.  Their net
effect on this document is recorded here; the in-text corrections above
are marked with references to this section.

\begin{enumerate}
\item \textbf{The unrestricted \eqref{eq:Q2} is attainable.}  Documents
7 and 8 prove, by two independent constructions (dyadic mass
equalization on slowly refining partitions, riding the within-shell
baseline slope $-(k+\kappa_1+1)/2$; then Hoischen--Carleman entire
approximation of the gauge, or of its reciprocal, together with its
derivative), that a positive, real-analytic, strictly decreasing gauge
attains \eqref{eq:Q2} for all $t$, with
$\log g=-\log^2x/(2\log2)-\kappa_1\log x+o(\log x)$.  Document 7 also
strengthens \cref{thm:impossible} from stabilizing shapes to all
bounded-distortion gauges.  \Cref{thm:impossible} itself stands; this
audit's summary slogan ``unattainable in principle'' did not, and has
been corrected in place.  Any exact gauge necessarily develops
unbounded, ever finer within-shell structure; no stabilized formula
works.
\item \textbf{Profile range.}  The step-$0.025$ grid underlying
\cref{fig:profile} misses narrow excursions of the singular-continuous
profile near $y=8/7$ and $y=10/7$; exact-point quadrature reaches
$\mathscr P\approx0.91627$ and $1.11814$ (values confirmed
independently at levels $17$--$19$ in the second-wave audit, which also
reproduces Document 6's level-18 table to $4\cdot10^{-10}$ and Document
7's level-19 extremes to all printed digits).  The figure's curve is
unchanged --- it is correct for its grid --- but its range is a lower
bound, not the span.
\item \textbf{Constants.}  Forty-digit collocation gives
$\varrho_1=0.66132260206056561507\ldots$,
$A_1=0.09126612413158821229\ldots$,
$A_1^{\log}=0.09386063575678126354\ldots$ (this audit's first edition
printed $A_1^{\log}=0.0938610$, a finite-shell running mean wrong in
the seventh figure --- corrected above), and the resolved
$\kappa_1=2.74807001487133267\ldots$ (the widely copied sixteen-digit
print $2.748070014871334$ inherits the fifteen-digit truncation of
$\varrho_1$; this audit's constants table had bracketed the
uncertainty correctly, its abstract had not).  New exact-ray constant:
$|f(2^n\cdot\tfrac23)|=C_*E_{\kappa_\infty}(2^n\cdot\tfrac23)$ for all
$n\ge0$ with $C_*=0.13912977473482934529\ldots$ (Document 6; verified
to $38$ digits).
\item \textbf{Fluctuation theory.}  Document 6's one-line Gordin
decomposition ($\mathcal P\psi=\psi/2$, $d=2\psi-\psi\circ T
=\log|\tan\pi t|$, $\int d^2=\pi^2/4$ exactly) gives the shortest
self-contained proof of \cref{thm:variance,thm:lil-correct}; the
second wave unanimously confirms \cref{find:ahl}.
\end{enumerate}
\appendix
...
$\kappa_1$ & $2.74807001487133267$ & $\tfrac12+\log_2(\pi/\varrho_1)$;
...
$\varrho_1$ & $0.66132260206056561507$ & collocation (second-wave
precision); certified in $(0.66126807,0.66134891)$\\
$\varrho_2$ & $1/2$ & exact\\
$\varrho_3$ & $0.394896632370234$ & collocation\\
$\varrho_4$ & $0.320194101601104$ & collocation\\
$A_1$ & $0.091266124131588212$ & shell-mean limit (direct = spectral)\\
$A_1^{\log}$ & $0.093860635756781264$ & logarithmic-shell limit
(corrects the first edition's $0.0938610$)\\
$C_*$ & $0.139129774734829345$ & exact peak-ray constant
$|f(2^n\cdot\tfrac23)|/E_{\kappa_\infty}$, second wave\\
$A_2$ & $0.103008918$ & RMS shell constant, $A_2^2=0.0106108372$\\
$\lambda_2(\LaplaceTransformSymbol_2)$ & $-1/4$ (exact, double) & \cref{prop:L2-spectrum};
rest of spectrum $<3\cdot10^{-4}$\\
$\lambda_2(\LaplaceTransformSymbol_1)$ & $-0.13435\pm0.0500\,\ImaginaryUnit$ & collocation;
...
(conjectural limit)\\
$\int_\RealNumbers f^2$ & $0.808873535967972$ & two independent computations,
\cref{prop:energy}\\
$L^2$ profile range & $[0.894,1.192]$ & step-$0.025$ grid, $n=13$\\
profile range & $\supseteq[0.91627,1.11814]$ & narrow features at
$y=8/7,10/7$ (second wave); $0.025$ grid sees $[0.9249,1.1133]$\\
...
statistics, the $\LaplaceTransformSymbol_2$ spectrum, the RMS constants and profile, the
