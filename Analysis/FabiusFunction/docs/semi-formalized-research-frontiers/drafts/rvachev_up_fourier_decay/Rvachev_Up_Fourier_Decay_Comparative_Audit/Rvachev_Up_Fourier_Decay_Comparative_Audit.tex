...
$m\ge1$ exactly $1+\vtwo(m)$ times (\lean{LatticeFiber}) and
$\|\Phi(x)\|/|x-m|^{1+\vtwo(m)}$ tends to an explicit positive constant
...
(\lean{PerronRootExistence}) and the Lasota--Yorke pair for $\Lop_1$
...
$F:\R\to\R$ and every $z\ge0$, not only for $y=1+z\in[1,2]$, with the ray
...
$(h,r)\mapsto2^h(r+1)$ over $k$ has exactly $\vtwo(k)+1$ elements and the
...
lattice is the sum over the positive integers weighted by $\vtwo(k)+1$.
...
$\Lop_2$: \lean{rms\_transfer\_const\_eigen},
...
Clausen values, $\int_0^1\log^2(2\sin\pi t)\dd t=\pi^2/12$
(\lean{CocycleParseval}, \lean{integral\_sq\_log\_two\_sin\_pi\_mul}) and
$\int_0^1\log^2|\tan\pi t|\dd t=\pi^2/4$
...
that is positive on $[0,1]$ and satisfies $m\,h\le\Lop_1h\le M\,h$ there
brackets the growth root, $m\le\varrho_1\le M$ --- with no eigenfunction,
$\Lop_1$ being the operator \eqref{eq:L1} verbatim; and in its abstract
form \lean{collatzWielandt\_bracket} the argument mentions neither
$\Lop_1$ nor $[0,1]$ nor sine and cosine, using only monotonicity and
...
approximate the exact range of $\Lop_1h/h$ to within $10^{-8}$.
...
sit within $10^{-8}$ of the exact range of $\Lop_1h/h$, so reaching
...
$1+\vtwo(m)$ with an explicit positive constant
...
$E_{m2^k}\sim\frac{H(1)|H(m)|}{\e(k+1)}(m2^k)^{-(k+1)}$ is not
formalized.
