...
Pochhammer--Polygamma Calculus, Barnes' $\BarnesGFunction$-Function, and Asymptotics}
...
polygamma multiplication and reflection; Barnes' $\BarnesGFunction$-function and its
...
\[
 n!\CoefficientExtraction z n e^zA(z)=\sum_{k=0}^n\binom nk a_k=b_n,
\]
...
\begin{equation}\label{eq:new-partition-mobius-top}
 \mu(\pi,\PartitionLatticeMaximum)=(-1)^{|\pi|-1}(|\pi|-1)!.
\end{equation}
\end{theorem}
...
\[
 u_m:=\mu_{\Pi_m}(\PartitionLatticeMinimum,\PartitionLatticeMaximum).
\]
For a partition $\tau\in\Pi_m$, the interval $[\PartitionLatticeMinimum,\tau]$ is the
product of $\Pi_{|C|}$ over the blocks $C$ of $\tau$.  Hence
\[
 \mu(\PartitionLatticeMinimum,\tau)=\prod_{C\in\tau}u_{|C|}.
\]
...
\eqref{eq:new-partition-mobius}; taking $\pi=\PartitionLatticeMaximum$ yields
...
\begin{equation}\label{eq:new-binomial-type-bell}
 ...
 \ExponentialPartialBellPolynomial{n}{k}(\beta_1,\beta_2,\ldots,\beta_{n-k+1}).
\end{equation}
\end{theorem}
...
respectively,
\[
 x^n,\qquad \FallingFactorial{x}{n},\qquad \RisingFactorial{x}{n},\qquad
 \TouchardPolynomial n(x)=\sum_{k=0}^n\StirlingSecondKind nk x^k.
\]
...
\begin{equation}\label{eq:new-riordan-entry}
 R_{n,k}=\frac{n!}{k!}\CoefficientExtraction z n g(z)f(z)^k.
\end{equation}
...
\begin{align}
 \StirlingSecondKind nk&=\frac{n!}{k!}\CoefficientExtraction z n(e^z-1)^k,
 \label{eq:new-s2-riordan}\\
 \SignedStirlingFirstKind{n}{k}&=\frac{n!}{k!}\CoefficientExtraction z n\log(1+z)^k,
 \label{eq:new-s1-riordan}\\
 \ExponentialPartialBellPolynomial nk(x_1,x_2,\ldots)
 &=\frac{n!}{k!}\CoefficientExtraction z n
...
\begin{align*}
 \CoefficientExtraction z n e^{xT(z)}
 &=\frac{x}{n}\CoefficientExtraction u{n-1}e^{xu}e^{-anu}\\
 &=\frac{x(x-an)^{n-1}}{n!}.
\end{align*}
Therefore
\[
...
\begin{equation}\label{eq:new-ordinary-composition-algorithm}
 \CoefficientExtraction z n A(B(z))
 =\sum_{k=0}^n a_k\,\CoefficientExtraction z n B(z)^k.
\end{equation}
...
\begin{equation}\label{eq:new-reversion-algorithm}
 \CoefficientExtraction z n G(z)^m
 =\frac{m}{n}\CoefficientExtraction u{n-m}\left(\frac{u}{F(u)}\right)^n,
 \qquad 1\le m\le n,
\end{equation}
...
\begin{align}
 ...
 \int_0^1\beta_n(x)\Differential x&=0, &&n\ge1.
...
\[
 n\int_0^1\beta_{n-1}(x)\Differential x=\beta_n(1)-\beta_n(0).
\]
The right side is zero for $n\ne1$ by
\eqref{eq:new-bernoulli-difference}; the remaining case is
$\int_0^1\beta_0(x)\Differential x=1$, so after reindexing
...
The Stirling EGF is
\[
 (e^t-1)^k=k!\sum_{n\ge k}\StirlingSecondKind nk\frac{t^n}{n!}.
\]
...
For every $n\ge0$,
\begin{align}
 \mathfrak b_n(x)&=\int_x^{x+1}\FallingFactorial{u}{n}\Differential u,
 \label{eq:new-bernoulli-second-integral}\\
 \mathfrak b_n'(x)&=n\FallingFactorial{x}{n-1}, &&n\ge1,
...
\begin{equation}\label{eq:new-cauchy-stirling}
 \boxed{\displaystyle
 \mathfrak b_n=\sum_{k=0}^n\frac{\SignedStirlingFirstKind{n}{k}}{k+1}.}
\end{equation}
\end{theorem}
...
Since
\[
 \int_x^{x+1}(1+z)^u\Differential u
 =\frac{z(1+z)^x}{\log(1+z)},
\]
and $(1+z)^u=\sum_{n\ge0}\FallingFactorial{u}{n}z^n/n!$, coefficient comparison
gives \eqref{eq:new-bernoulli-second-integral}.  Differentiating the integral
with moving endpoints gives
\[
 \FallingFactorial{x+1}{n}-\FallingFactorial{x}{n}=n\FallingFactorial{x}{n-1},
\]
...
$\FallingFactorial{u}{n}=\sum_{k=0}^ns(n,k)u^k$; integrate from $0$ to $1$ to obtain
...
Define the Genocchi polynomials $\GenocchiPolynomial n(x)$ by
\begin{equation}\label{eq:new-genocchi-egf}
 \frac{2t e^{xt}}{e^t+1}
 =\sum_{n\ge0}\GenocchiPolynomial n(x)\frac{t^n}{n!},
 \qquad \GenocchiNumber n=\GenocchiPolynomial n(0).
\end{equation}
Thus $\GenocchiNumber0=0$, $\GenocchiNumber1=1$, $\GenocchiNumber2=-1$,
$\GenocchiNumber4=1$, and $\GenocchiNumber6=-3$.

\begin{theorem}[Genocchi relations]\label{thm:new-genocchi}
For $n\ge1$,
\begin{align}
 \GenocchiPolynomial n(x)&=n\mathcal E_{n-1}(x),
 \label{eq:new-genocchi-euler}\\
 \GenocchiNumber n&=2(1-2^n)\beta_n,
 \label{eq:new-genocchi-bernoulli}\\
 \GenocchiPolynomial n(x+1)+\GenocchiPolynomial n(x)&=2n x^{n-1}.
 \label{eq:new-genocchi-difference}
\end{align}
In particular, $\GenocchiNumber{2m+1}=0$ for $m\ge1$.
\end{theorem}
...
\begin{equation}\label{eq:new-tangent-genocchi}
 \mathsf T_{2m-1}=(-1)^m\frac{2^{2m-2}}{m}\GenocchiNumber{2m}.
\end{equation}
...
Let $\TouchardPolynomial n(x)=\sum_{k=0}^n\StirlingSecondKind nk x^k$ be the Touchard polynomial.
The complementary Bell number is
\begin{equation}\label{eq:new-complementary-bell-def}
 \ComplementaryBellNumber n:=\TouchardPolynomial n(-1)=\sum_{k=0}^n(-1)^k\StirlingSecondKind nk.
\end{equation}
...
\begin{align}
 ...
 &=-\sum_{k=0}^n\binom nk\ComplementaryBellNumber k.
...
\begin{equation}\label{eq:new-bell-complement-inverse}
 \sum_{k=0}^n\binom nk\BellNumber k\ComplementaryBellNumber{n-k}=\delta_{n0}.
\end{equation}
\end{corollary}
...
More generally, $\TouchardPolynomial n(-x)$ is the signed block enumerator with a weight $-x$
...
\begin{align}
 ...
 \int_0^1\phi(t)f^{(n+1)}(z_0+th)\Differential t
 \notag\\
...
$\int_0^1\phi(t)F^{(n+1)}(t)\Differential t$ by parts $n+1$ times.  Because
$\phi^{(n+1)}=0$,
\[
 \int_0^1\phi F^{(n+1)}\Differential t
 =\sum_{j=0}^{n}(-1)^j
...
\begin{equation}\label{eq:new-periodic-bernoulli-derivative}
 \frac{\Differential}{\Differential x}\widetilde\beta_m(x)
...
\[
 \int_k^{k+1}(x-k-\tfrac12)f'(x)\Differential x
...
applied to $f(x)=x\log x$, it produces the Barnes $\BarnesGFunction$ expansion developed
later.
\end{remark}
...
$\alpha\in\ComplexNumbers$.  Suppose
\[
...
\[
 \CoefficientExtraction z n(1-z/\rho)^{-\gamma}
...
\[
 \left|\CoefficientExtraction z n(1-z/\rho)^{M-\alpha}H_M(z)\right|
...
\begin{equation}\label{eq:new-pochhammer-relations}
 (a)_n=\RisingFactorial an=\frac{\Gamma(a+n)}{\Gamma(a)},
 \qquad
 \FallingFactorial{x}{n}=(-1)^n(-x)_n.
\end{equation}
...
\begin{equation}\label{eq:new-pochhammer-stirling}
 (a)_n=\sum_{k=0}^n\UnsignedStirlingFirstKind nk a^k,
\end{equation}
...
\begin{equation}\label{eq:new-pochhammer-parameter-derivatives}
 ...
 =(a)_n\ExponentialCompleteBellPolynomial m
...
\begin{equation}\label{eq:new-reciprocal-pochhammer}
 ...
 =\frac1{(a)_n}\ExponentialCompleteBellPolynomial m
...
\[
 B(z,N+1)=\int_0^1t^{z-1}(1-t)^N\Differential t
 =\frac{N!}{z(z+1)\cdots(z+N)}.
\]
After multiplying by $N^z$ and substituting $u=Nt$,
\[
 N^zB(z,N+1)=\int_0^N u^{z-1}\left(1-\frac uN\right)^N\Differential u.
\]
...
\[
 \frac{\Differential}{\Differential z}\log\left(
...
$\ComplexNumbers\setminus\{0,-1,-2,\ldots\}$ and defines a meromorphic function there;
...
\begin{align}
 ...
 \psi^{(m)}(n+1)&=(-1)^{m+1}m!
 \left(\RiemannZeta(m+1)-H_n^{(m+1)}\right),\qquad m\ge1.
...
$\psi^{(m)}(1)=(-1)^{m+1}m!\RiemannZeta(m+1)$ and iterating the recurrences gives
the integer values.
\end{proof}
...
\begin{equation}\label{eq:new-polygamma-reflection}
 ...
 =\frac{\Differential^m}{\Differential z^m}\bigl(\pi\cot(\pi z)\bigr).
\end{equation}
\end{theorem}
...
\chapter{Barnes' \texorpdfstring{$\BarnesGFunction$}{G}-function and superfactorial asymptotics}
\label{ch:barnes-g}

\section{Canonical product and recurrence}

Barnes' $\BarnesGFunction$-function is the entire function defined by the canonical product
...
\begin{equation}\label{eq:new-barnes-integers}
 \BarnesGFunction(n+1)=\prod_{k=1}^{n-1}k!
 =\prod_{j=1}^{n-1}j^{n-j}.
\end{equation}
\end{theorem}
...
$P(z)=\BarnesGFunction(z+1)$, and let $P_N$ be the product truncated after $k=N$.  Setting
...
$\BarnesGFunction(z+1)=\Gamma(z)\BarnesGFunction(z)$ with $\BarnesGFunction(1)=1$.

Iteration gives
$\BarnesGFunction(n+1)=\Gamma(n)\cdots\Gamma(1)=\prod_{k=1}^{n-1}k!$.  A fixed integer
...
\begin{align}
 \log \BarnesGFunction(N+1)
={}&\frac{N^2}{2}\log N-\frac{3N^2}{4}
 +\frac N2\log(2\pi)-\frac1{12}\log N+\RiemannZeta'(-1)
 \notag\\
 &+\sum_{r=1}^{p-1}
...
\begin{equation}\label{eq:new-barnes-sum}
 \log \BarnesGFunction(N+1)=N\log\Gamma(N)-\sum_{k=1}^{N-1}k\log k.
\end{equation}
...
$\RiemannZeta(s)=\sum_{k\ge1}k^{-s}$, initially for $\Re s>1$ and then by analytic
continuation.  Uniformly for $s$ near $-1$,
\begin{align*}
 \sum_{k=1}^{N-1}k^{-s}
 ={}&\RiemannZeta(s)-\frac{N^{1-s}}{s-1}-\frac12N^{-s}\\
 &-\sum_{r=1}^{p+1}
...
\[
 \left.\frac{\Differential}{\Differential s}(s)_{2r-1}\right|_{s=-1}
 =-(2r-3)!.
\]
...
constant as $1/12-\RiemannZeta'(-1)$, proving
...
\begin{align*}
 \log \BarnesGFunction(N+1)=&\frac{N^2}{2}\log N-\frac{3N^2}{4}
 +\frac N2\log(2\pi)-\frac1{12}\log N+\RiemannZeta'(-1)\\
...
says
\[
 \CoefficientExtraction y n g(y)=\frac1n\CoefficientExtraction x{n-1}
...
For $n\ge1$,
\begin{align}
 \CoefficientExtraction x{n-1}\left(\frac{x}{e^x-1}\right)^n
 &=(-1)^{n-1},
 \label{eq:new-norlund-diagonal}\\
 \CoefficientExtraction x{n-1}\left(\frac{x}{\log(1+x)}\right)^n
 &=\frac1{(n-1)!}.
...
\[
 \frac1n\CoefficientExtraction x{n-1}\left(\frac{x}{e^x-1}\right)^n
 =\frac{(-1)^{n-1}}n.
\]
...
\begin{equation}\label{eq:new-offset-lagrange}
 ...
 =\frac1n\CoefficientExtraction z{n-1}H'(a+z)\Phi(a+z)^n.}
\end{equation}
\end{theorem}
...
\[
 \CoefficientExtraction t n\widetilde H(w(t))
 =\frac1n\CoefficientExtraction z{n-1}\widetilde H'(z)\phi(z)^n,
\]
...
\end{equation}
Thus $\sum_{k=1}^nN(n,k)=\CatalanNumber n$.
\end{theorem}
...
$\boldsymbol n\in\NaturalNumbers^d$,
\begin{align}
...
\[
 ...
 \prod_{i=1}^dt_i^{-n_i-1}\Differential t_1\cdots\Differential t_d.
\]
...
\end{align*}
Let $X=\DiagonalOperator(x_1,\ldots,x_d)$ and
$Y=\DiagonalOperator(\phi_1^{-1},\ldots,\phi_d^{-1})J_\phi$.
...
$\Differential M=(1-e\cos E)\Differential E$,
\begin{align*}
 c_n
 &=\frac1\pi\int_0^{2\pi}q(M)\sin(nM)\Differential M\\
...
polygamma functions, and Barnes' $\BarnesGFunction$-function supplied a normalization check
...
\item Pochhammer symbols, all parameter derivatives through Bell polynomials,
      digamma and polygamma series, and Barnes' $\BarnesGFunction$-function with its
...
Barnes' $\BarnesGFunction$-function \cite{NISTDLMF}.  Gessel's survey supplied a modern
...
\begin{enumerate}
\item $\SignedStirlingFirstKind{n}{k}$ is signed and $\UnsignedStirlingFirstKind nk$ is unsigned.  They are never
      interchanged inside a logarithmic generating function or a factorial
      expansion.
\item Bell numbers are $\BellNumber n$, partial and complete exponential Bell
      polynomials are $\ExponentialPartialBellPolynomial nk$ and $\ExponentialCompleteBellPolynomial n$, Bernoulli numbers are
      $\beta_n$, Euler polynomials are $\mathcal E_n(x)$, and complementary
      Bell numbers are $\ComplementaryBellNumber n$.
