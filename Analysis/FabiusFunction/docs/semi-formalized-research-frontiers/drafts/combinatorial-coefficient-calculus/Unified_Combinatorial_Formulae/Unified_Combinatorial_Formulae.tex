...
Pochhammer parameter derivatives, polygamma and Barnes $\BarnesGFunction$ functions, Kepler's
...
gives
\[
 \sum_{m\ge0}(m+1)^nt^m=\frac{\TypeAEulerianPolynomial n(t)}{(1-t)^{n+1}}.
\]
Comparing this with the defining Ehrhart form
$\sum_{m\ge0}|mC\cap\IntegerNumbers^n|t^m=h^*(t)/(1-t)^{n+1}$ proves
$h^*(t)=\TypeAEulerianPolynomial n(t)$.
\end{proof}
...
\begin{equation}\label{unif:eq-reciprocal-derivative}
 ...
 =\sum_{k=0}^{r}(-1)^k k!\,
 \frac{\ExponentialPartialBellPolynomial{r}{k}(g',g'',\ldots,g^{(r-k+1)})}{g^{k+1}},
\end{equation}
where $\ExponentialPartialBellPolynomial00=1$. Consequently,
\begin{equation}\label{unif:eq-higher-quotient}
 D^n\!\left(\frac fg\right)
 =\sum_{r=0}^{n}\binom nr f^{(n-r)}
 \sum_{k=0}^{r}(-1)^k k!\,
 \frac{\ExponentialPartialBellPolynomial{r}{k}(g',g'',\ldots)}{g^{k+1}}.
\end{equation}
\end{proposition}
\begin{proof}
...
\begin{align}
 ...
 \ExponentialPartialBellPolynomial{n}{k}\bigl((g\circ h)',(g\circ h)'',\ldots\bigr),
...
\begin{equation}\label{unif:eq-inverse-derivative-recursion}
 g^{(n)}=-\frac1{f'(g)}
 \sum_{k=2}^{n}f^{(k)}(g)
 \ExponentialPartialBellPolynomial{n}{k}(g',g'',\ldots,g^{(n-k+1)}).
\end{equation}
The first cases are
\begin{align}
 g'&=\frac1{f'},\\
...
\end{theorem}
\begin{proof}
The interval $[\pi,\PartitionLatticeMaximum]$ is isomorphic to the partition lattice on the
$|\pi|$ blocks of $\pi$, so let
$\mu_m=\mu_{\Pi_m}(\PartitionLatticeMinimum,\PartitionLatticeMaximum)$. Every interval
$[\PartitionLatticeMinimum,\sigma]$ is a product of partition lattices, hence
$\mu(\PartitionLatticeMinimum,\sigma)=\prod_{B\in\sigma}\mu_{|B|}$. The defining M\"obius
...
remain $\BellNumber n$ and Bernoulli numbers remain $\beta_n$; write
$\BerP n x$, $\EulP n x$, and $\GenocchiPolynomial n(x)$ for Bernoulli, Euler, and Genocchi
polynomials.
...
\begin{align}
 ...
 \frac{\Differential}{\Differential x}\BerP n x&=n\BerP{n-1}x,
...
\end{equation}
and for $p\ge0$, $N\in\NaturalNumbers$,
...
\begin{align}
 ...
 \GenocchiPolynomial n(x)&=n\EulP{n-1}x\quad(n\ge1),
 \label{unif:eq-genocchi-euler}\\
 \GenocchiPolynomial n(0)&=2(1-2^n)\beta_n.
...
\end{equation}
Their values $b_n=b_n(0)$ satisfy $b_n=n!\GregoryCoefficient n$ for the Gregory coefficients in
the earlier chapter.
...
\begin{align}
 ...
 b_n&=\sum_{k=0}^{n}\frac{\SignedStirlingFirstKind{n}{k}}{k+1},
 \label{unif:eq-cauchy-stirling}\\
 b_n(x+1)-b_n(x)&=nb_{n-1}(x),
 \label{unif:eq-cauchy-difference}\\
 \frac{\Differential}{\Differential x}b_n(x)&=n\FallingFactorial x{n-1},
 \label{unif:eq-cauchy-derivative}\\
 b_n(x+y)&=\sum_{k=0}^{n}\binom nk b_k(x)\FallingFactorial y{n-k}.
 \label{unif:eq-cauchy-sheffer}
\end{align}
\end{theorem}
\begin{proof}
Integrate
$(1+t)^{x+u}=\sum_n\FallingFactorial{x+u}{n}t^n/n!$ from $u=0$ to $1$. The left side
becomes $(1+t)^xt/\log(1+t)$, proving the integral. At $x=0$, expand
$\FallingFactorial un=\sum_ks(n,k)u^k$ and integrate to get the Stirling sum. The
...
\begin{equation}\label{unif:eq-complementary-bell-definition}
 \ComplementaryBellNumber n=\sum_{k=0}^{n}(-1)^k\StirlingSecondKind nk=\TouchardPolynomial n(-1).
\end{equation}
...
\begin{align}
 \sum_{n\ge0}\ComplementaryBellNumber n\frac{z^n}{n!}&=\exp(1-e^z),
 \label{unif:eq-complementary-bell-egf}\\
 \ComplementaryBellNumber{n+1}&=-\sum_{j=0}^{n}\binom nj\ComplementaryBellNumber j,
...
\begin{equation}\label{unif:eq-em-remainder}
 ...
 \widetilde B_{2p}(x)f^{(2p)}(x)\Differential x.
\end{equation}
Moreover,
\begin{equation}\label{unif:eq-em-bound}
 |R_p|\le\frac{2\RiemannZeta(2p)}{(2\pi)^{2p}}
...
\]
implies
$|\widetilde B_{2p}(x)|/(2p)!\le2\RiemannZeta(2p)/(2\pi)^{2p}$.
...
\begin{align}
 ...
 f^{(n+1)}(z_0+t\Delta)\Differential t
 \notag\\
...
\chapter{Pochhammer symbols, polygamma functions, and Barnes' \texorpdfstring{$\BarnesGFunction$}{G}}
...
\begin{equation}\label{unif:eq-pochhammer-bell-derivative}
 ...
 =(a)_n\ExponentialCompleteBellPolynomial r(L_1(a,n),\ldots,L_r(a,n)).
\end{equation}
\end{theorem}
\begin{proof}
...
\begin{equation}\label{unif:eq-polygamma-hurwitz}
 \psi^{(m)}(z)=(-1)^{m+1}m!\HurwitzZeta{m+1}{z}.
\end{equation}

\section{Barnes' \texorpdfstring{$\BarnesGFunction$}{G}-function}
...
\]
Thus $\BarnesGFunction(z+2)=\Gamma(z+1)\BarnesGFunction(z+1)$, equivalent to the stated recurrence.
...
For an integer $N$,
\[
 \log \BarnesGFunction(N+1)=N\log\Gamma(N)-\sum_{j=1}^{N-1}j\log j.
\]
...
\begin{align}
 ...
 \frac{\Differential^{n-1}}{\Differential M^{n-1}}\sin^nM,
...
\[
 ...
 =\frac1{n!}\frac{\Differential^{n-1}}{\Differential M^{n-1}}\sin^nM.
\]
For the Fourier form, $u(M)$ is odd and $2\pi$-periodic. If
$u=\sum_{n\ge1}b_n\sin(nM)$, integration by parts and $u(\pm\pi)=0$ give
\[
 b_n=\frac1{\pi n}\int_{-\pi}^{\pi}E'(M)\cos(nM)\Differential M.
\]
Change variables using $M=E-e\sin E$ and $E'(M)\Differential M=\Differential E$:
\[
 b_n=\frac1{\pi n}\int_{-\pi}^{\pi}
 \cos(nE-ne\sin E)\Differential E=\frac{2J_n(ne)}n,
\]
...
Use the normalization of the earlier associahedral inversion theorem:
...
\begin{theorem}[Face numbers of $\AssociahedronByDimension{n-1}$]
...
\begin{equation}\label{unif:eq-associahedron-f-vector}
 f_d(\AssociahedronByDimension{n-1})
 =\frac1{n-d}\binom{2n-d}{n-1-d}\binom{n-1}{d}.
\end{equation}
In particular, $f_0(\AssociahedronByDimension{n-1})=\CatalanNumber n$,
$f_{n-2}(\AssociahedronByDimension{n-1})=n(n+1)/2-1$, and
$f_{n-1}(\AssociahedronByDimension{n-1})=1$.
\end{theorem}
\begin{proof}
...
\end{align*}
A face of $\AssociahedronByDimension{n-1}$ has $L=n+1$ leaves. If it has $m$ internal vertices, its
...
For $\AssociahedronByDimension3$, the formula gives $14$ vertices, $21$ edges, $9$ facets, and one
...
Barnes' $\BarnesGFunction$-function. These were integrated in the new parts on Bernoulli
...
multivariate Fa\`a di Bruno, singularity analysis, Barnes' $\BarnesGFunction$-function, the
...
Multiplying
$\GenocchiPolynomial n(x)=n\EulP{n-1}x$ by $t^n/n!$ recovers the Genocchi EGF, while
substitution of \eqref{unif:eq-euler-bernoulli} recovers
$\GenocchiPolynomial n(0)=2(1-2^n)\beta_n$. Thus the two routes through the generating-function
...
selected entries from \emph{Wolfram MathWorld}: Barnes $\BarnesGFunction$-function, Bell and
