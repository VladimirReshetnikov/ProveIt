...
This is a semi-formalized research-frontier document.  Its displayed proofs
are human-readable mathematical arguments: the manuscript has not been
formalized in Lean and is not machine-checked.  Theorem environments therefore
do not, by themselves, assert Lean verification.
...
\begin{enumerate}
\item Bell numbers are $\BellNumber n$, complete exponential Bell polynomials are
      $\ExponentialCompleteBellPolynomial n$, partial exponential Bell polynomials are $\ExponentialPartialBellPolynomial nk$, and
      ordinary partial Bell polynomials are $\OrdinaryPartialBellPolynomial nk$.
...
\item Signed first-kind Stirling numbers are $\SignedStirlingFirstKind{n}{k}$; unsigned first-kind numbers
      are $\UnsignedStirlingFirstKind nk$.  The sign conversion is never left implicit.
...
The ground coefficient ring is usually a commutative $\RationalNumbers$-algebra; analytic
statements specialize to $\RealNumbers$ or $\ComplexNumbers$.  For $n\in\NaturalNumbers$,
\[
...
\[
 \CoefficientExtraction z nF(z)=[z^n]F(z),\qquad
 \ResidueOperator_zF(z)\Differential z=[z^{-1}]F(z).
\]
...
\[
 ...
 \StirlingSecondKind nk,\qquad \TypeAEulerianNumber nk,\qquad \SecondOrderEulerianNumber nk.
\]
Bell numbers are $\BellNumber n$; the complementary Bell number is
$\ComplementaryBellNumber{n}=\TouchardPolynomial{n}(-1)$.  Exponential partial and complete Bell polynomials are
$\ExponentialPartialBellPolynomial nk$ and $\ExponentialCompleteBellPolynomial n$, and $\OrdinaryPartialBellPolynomial nk$ denotes the ordinary partial Bell
...
\begin{align*}
 ...
 &=\left(\sum_{r\ge0}\frac{(cz)^r}{r!}\right)
   \left(\sum_{k\ge0}a_k\frac{z^k}{k!}\right)=\EulerE^{cz}A(z).
\end{align*}
Multiplication by $\EulerE^{-cz}$ gives the inverse transform.
\end{proof}
...
is
\[
 \sum_{S\subseteq U\subseteq T}\mu(S,U)=\IndicatorOf{S=T}.
\]
...
\end{equation}
Thus $\ComplementaryBellNumber{n}$ is the number of partitions of $[n]$ with an even number
...
\begin{align}
 ...
 &=\EulerE\sum_{m=0}^{\infty}\frac{(-1)^m m^n}{m!},
...
\[
 \sum_{n\ge0}\TouchardPolynomial{n}(x)\frac{z^n}{n!}=\exp\!\bigl(x(\EulerE^z-1)\bigr).
\]
Setting $x=-1$ proves \eqref{eq:complementary-bell-egf}.  Expanding the outer
exponential gives
\[
 \EulerE^{1-\EulerE^z}=\EulerE\sum_{m\ge0}\frac{(-1)^m \EulerE^{mz}}{m!},
\]
...
$C(z)=\EulerE^{1-\EulerE^z}$, then $C'(z)=-\EulerE^zC(z)$.  Comparing coefficients of
$z^n/n!$ gives \eqref{eq:complementary-bell-recurrence}.
\end{proof}

More generally, $\TouchardPolynomial{n}(x)$ is the partition enumerator in which every block has
...
\end{theorem}
% CANONICAL-PROOF-STATUS: external-result
...
\begin{align*}
 ...
 &=\frac{te^{xt}}{\EulerE^t-1}\frac{\EulerE^t-1}{\EulerE^{t/q}-1}\\
...
\]
with $u=\EulerE^t-1$ becomes
\[
 \frac{t}{\EulerE^t-1}
 =\sum_{k\ge0}\frac{(-1)^k}{k+1}(\EulerE^t-1)^k.
\]
Now use
$(\EulerE^t-1)^k=k!\sum_{n\ge k}\StirlingSecondKind nk t^n/n!$ and compare
...
\[
 \beta_n=\int_0^1 \TouchardPolynomial{n}(-u)\,\Differential u,
\]
...
inversion applied to the inverse pair $\EulerE^t-1$ and $\log(1+z)$ gives
\[
 [t^{n-1}]\left(\frac{t}{\EulerE^t-1}\right)^n=(-1)^{n-1}.
\]
...
\begin{equation}\label{eq:norlund-complete-bell}
 \beta_n^{(\alpha)}(x)
 =\ExponentialCompleteBellPolynomial n(c_1,c_2,\ldots,c_n),
\end{equation}
where
\[
...
\begin{equation}\label{eq:bernoulli-fourier}
 \widetilde\beta_n(x)
 =-\frac{n!}{(2\pi \ImaginaryUnit)^n}
   \sum_{k\in\IntegerNumbers\setminus\{0\}}\frac{\EulerE^{2\pi ikx}}{k^n}.
\end{equation}
...
\begin{proof}
For $k\ne0$, let
$c_k=\int_0^1\beta_n(x)\EulerE^{-2\pi ikx}\,\Differential x$.  Since
...
$n=2m$ gives
\[
 \beta_{2m}=-\frac{(2m)!}{(2\pi \ImaginaryUnit)^{2m}}2\RiemannZeta(2m),
\]
...
\begin{equation}\label{eq:modified-bernoulli-egf}
 ...
 =\frac12\log\!\left(
   \frac{\EulerE^{x/2}-\EulerE^{-x/2}}{x/2}\right).
\end{equation}
...
the generating functions at $x$ and $x+1$ gives $2\EulerE^{xt}$, proving
...
\begin{align*}
 \frac1t\left[
 \frac{2t \EulerE^{(x+1)t}}{\EulerE^{2t}-1}
 -\frac{2t \EulerE^{xt}}{\EulerE^{2t}-1}
 \right]
 &=\frac{2\EulerE^{xt}(\EulerE^t-1)}{\EulerE^{2t}-1}\\
 &=\frac{2\EulerE^{xt}}{\EulerE^t+1},
\end{align*}
...
For $n\ge1$,
\begin{align}
 \GenocchiPolynomial{n}(x)&=nE_{n-1}(x),
...
\end{align}
Thus $\GenocchiNumber{0}=0,\GenocchiNumber{1}=1,\GenocchiNumber{2}=-1,\GenocchiNumber{3}=0,\GenocchiNumber{4}=1,\GenocchiNumber{6}=-3,\GenocchiNumber{8}=17$.
\end{theorem}
\begin{proof}
...
\[
 \frac{2t}{\EulerE^t+1}
 =\frac{2t}{\EulerE^t-1}-\frac{4t}{\EulerE^{2t}-1}
...
For $n\ge0$,
\begin{align}
 b_n(x)&=\int_0^1\FallingFactorial{x+u}{n}\,\Differential u,
 \label{eq:bernoulli-second-kind-integral}\\
 b_n(x+1)-b_n(x)&=n\FallingFactorial{x}{n-1},
 \label{eq:bernoulli-second-kind-difference}\\
 b_n'(x)&=n\FallingFactorial{x}{n-1},
 \label{eq:bernoulli-second-kind-derivative}\\
 b_n(x)&=b_n+n\sum_{k=1}^{n}\frac{\SignedStirlingFirstKind{n-1}{k-1}}{k}x^k,
...
\begin{proof}
Since
$\sum_{n\ge0}\FallingFactorial{x+u}{n}t^n/n!=(1+t)^{x+u}$,
...
$\FallingFactorial{x}{n-1}=\sum_{j=0}^{n-1}\SignedStirlingFirstKind{n-1}{j}x^j$, integration from $0$ to $x$
gives \eqref{eq:bernoulli-second-kind-stirling}.  Finally use
\[
 \FallingFactorial{z}{n}=(-1)^n\FallingFactorial{n-1-z}{n}
\]
in the integral representation and change $u$ to $1-u$.
\end{proof}

The inverse relation between $\log(1+t)$ and $\EulerE^t-1$ gives the diagonal
coefficient
...
For $r\ge1$, put
$\widetilde\beta_r(x)=\beta_r(\FractionalPart{x})$ away from integers.  At
...
\begin{align}
 ...
 \widetilde\beta_{2m}(x)f^{(2m)}(x)\,\Differential x.
 \label{eq:euler-maclaurin-remainder}
\end{align}
Moreover,
\begin{equation}\label{eq:euler-maclaurin-bound}
 |R_m|\le
 \frac{2\RiemannZeta(2m)}{(2\pi)^{2m}}
 \int_a^b|f^{(2m)}(x)|\,\Differential x.
\end{equation}
\end{theorem}
\begin{proof}
On one interval $[k,k+1]$, integration by parts gives
\[
 \int_k^{k+1}\widetilde\beta_1(x)f'(x)\,\Differential x
...
\[
 ...
 \le\frac{2(2m)!}{(2\pi)^{2m}}\RiemannZeta(2m),
\]
...
\begin{equation}\label{eq:darboux-taylor}
 ...
 +\frac1{(m-1)!}\int_a^x(x-t)^{m-1}f^{(m)}(t)\,\Differential t.
\end{equation}
\end{theorem}
\begin{proof}
...
$u=f^{(m)}(t)$ and
$\Differential v=(x-t)^{m-1}\Differential t/(m-1)!$.  The boundary at $t=x$ vanishes and the
...
For $n\in\NaturalNumbers$, the rising Pochhammer symbol is
\[
 (z)_n=\RisingFactorial zn=z(z+1)\cdots(z+n-1),\qquad (z)_0=1.
\]
...
Since $(z)_n=\sum_k\UnsignedStirlingFirstKind nk z^k$, the unsigned first-kind Stirling triangle
...
\begin{align}
 ...
 \psi^{(m)}(n+1)&=(-1)^{m+1}m!
 \left(\RiemannZeta(m+1)-H_n^{(m+1)}\right),
...
\begin{equation}\label{eq:pochhammer-parameter-derivatives}
 ...
 =(z)_n\ExponentialCompleteBellPolynomial m\bigl(L_1,L_2,\ldots,L_m\bigr),
\end{equation}
where
...
\end{theorem}
\begin{proof}
The value $\BarnesGFunction(1)=1$ follows by setting $z=0$ in the product.  Divide the product
...
\begin{multline*}
 \frac{\BarnesGFunction(z+1)}{\BarnesGFunction(z)}
 =\Gamma(z)\sqrt{2\pi}
 \EulerE^{-z-\gamma z+\gamma/2}\\
 {}\times\lim_{N\to\infty}
...
second line is
\[
 \frac{\EulerE^{z+\gamma(z-1/2)}}{\sqrt{2\pi}},
\]
...
\begin{multline}\label{eq:barnes-integral}
 \log \BarnesGFunction(z+1)=\frac z2\log(2\pi)-\frac12z(z+1)
 +z\log\Gamma(z+1)\\
 {}-\int_0^z\log\Gamma(t+1)\,\Differential t.
\end{multline}
\end{theorem}
\begin{proof}
...
\begin{multline}\label{eq:barnes-asymptotic-elementary}
 ...
 +O(N^{-2m}).
\end{multline}
The constant $\frac1{12}-\log A$ is also $\RiemannZeta'(-1)$.
\end{theorem}
\begin{proof}
...
\]
The equality $\RiemannZeta'(-1)=1/12-\log A$ follows by differentiating the
...
\begin{align}
 D^n g(x)^\alpha
 &=\sum_{k=1}^{n}\FallingFactorial{\alpha}{k}g(x)^{\alpha-k}
   \ExponentialPartialBellPolynomial nk(g',g'',\ldots),
 \label{eq:faa-power}\\
 D^n\log g(x)
 &=\sum_{k=1}^{n}(-1)^{k-1}(k-1)!\,
   \frac{\ExponentialPartialBellPolynomial nk(g',g'',\ldots)}{g(x)^k},
 \label{eq:faa-log}\\
 D^n\frac1{g(x)}
 &=\sum_{k=1}^{n}(-1)^kk!\,
   \frac{\ExponentialPartialBellPolynomial nk(g',g'',\ldots)}{g(x)^{k+1}},
...
\[
 D^ku^\alpha=\FallingFactorial{\alpha}{k}u^{\alpha-k},\qquad
...
\begin{equation}\label{eq:higher-quotient}
 ...
 =\sum_{r=0}^{n}\binom nr f^{(n-r)}(x)
   \sum_{k=0}^{r}(-1)^kk!\,
   \frac{\ExponentialPartialBellPolynomial rk(g',g'',\ldots)}{g(x)^{k+1}},
\end{equation}
where $\ExponentialPartialBellPolynomial00=1$.
\end{corollary}
\begin{proof}
...
because $\ExponentialPartialBellPolynomial nk$ with $k\ge2$ involves derivatives of $g$ of order at most
$n-1$.
\end{proof}
...
For ordinary series, $\OrdinaryPartialBellPolynomial nk$ extracts $[z^n]G(z)^k$; for exponential
series, $\ExponentialPartialBellPolynomial nk$ extracts $n![z^n]G(z)^k/k!$; in derivative formulas the
arguments are the actual derivatives $g^{(j)}(x)$.  The conversion
\[
 \OrdinaryPartialBellPolynomial nk(g_1,g_2,\ldots)
 =\frac{k!}{n!}\ExponentialPartialBellPolynomial nk(1!g_1,2!g_2,\ldots)
\]
...
\end{equation}
Let $\AssociahedronByDimension{n-1}$ denote the associahedron of dimension $n-1$.  Its faces may be
...
\begin{align}
 b_n
 &=\sum_{F\text{ a face of }\AssociahedronByDimension{n-1}}(-1)^{n-\dim F}a_F,
...
with faces of $\AssociahedronByDimension{n-1}$; contraction of an internal edge merges two internal
...
The number $f_d(\AssociahedronByDimension{n-1})$ of $d$-dimensional faces, $0\le d\le n-1$, is
\begin{equation}\label{eq:associahedron-f-vector}
 f_d(\AssociahedronByDimension{n-1})=\frac1{n+1}\binom{n-1}{d}\binom{2n-d}{n}.
\end{equation}
In particular, $\AssociahedronByDimension{n-1}$ has the Catalan number
$\frac1{n+1}\binom{2n}{n}$ of vertices, and
$\sum_d(-1)^df_d(\AssociahedronByDimension{n-1})=1$.
\end{corollary}
\begin{proof}
...
\end{align*}
For $\AssociahedronByDimension{3}$, the coefficients $1,6,3,21,14$ enumerate the whole polytope, the
...
The inverse pair $f(x)=\EulerE^x-1$ and $g(y)=\log(1+y)$ gives
\begin{equation}\label{eq:norlund-lagrange-diagonal}
 [x^{n-1}]\left(\frac{x}{\EulerE^x-1}\right)^n=(-1)^{n-1},
 \qquad n\ge1.
\end{equation}
...
Conversely, $f(x)=\log(1+x)$ and $g(y)=\EulerE^y-1$ give
...
$[y^n](\EulerE^y-1)=1/n!$.  This evaluates a diagonal of the higher powers of the
...
\begin{align*}
 ...
 \sin E\,\sin\bigl(n(E-e\sin E)\bigr)(1-e\cos E)\,\Differential E.
\end{align*}
Now
\[
...
$\frac{\Differential}{\Differential u}\sin(nu-t\sin u)$ over a period to obtain
\[
 n\int_0^{2\pi}\cos(nu-t\sin u)\,\Differential u
 =t\int_0^{2\pi}\cos u\cos(nu-t\sin u)\,\Differential u.
\]
...
$\EulerE^{inu}\exp\{\frac t2(\EulerE^{-iu}-\EulerE^{iu})\}$; matching equal powers of
$\EulerE^{iu}$ and $\EulerE^{-iu}$ gives exactly the displayed sum.  Hence every term in
...
\[
 \ResidueOperator_{\boldsymbol x}H(\boldsymbol x)\,
...
\begin{multline}\label{eq:multivariate-residue-substitution}
 \ResidueOperator_{\boldsymbol t}H(\boldsymbol t)\,
 \Differential t_1\cdots\Differential t_d\\
 =\ResidueOperator_{\boldsymbol x}H(\boldsymbol f(\boldsymbol x))
...
$\boldsymbol t^{\boldsymbol m}\Differential t_1\wedge\cdots\wedge\Differential t_d$.
If some $m_j\ne-1$, that form is exact: up to the sign needed to move
$\Differential t_j$ into position, it is
\[
 \Differential\!\left(
 \frac{t_j^{m_j+1}}{m_j+1}
 \prod_{i\ne j}t_i^{m_i}
 \bigwedge_{\substack{1\le r\le d\\ r\ne j}}\Differential t_r
 \right).
\]
...
$f_i=x_i u_i$,
\[
 \frac{\Differential f_i}{f_i}=\frac{\Differential x_i}{x_i}+\frac{\Differential u_i}{u_i}.
\]
In the wedge product of these $d$ forms, the product of all
$\Differential x_i/x_i$ has residue $1$.  Every other term contains at least one
regular form $\Differential u_i/u_i$, whose coefficients have no negative exponents;
...
For a multi-index $\boldsymbol n=(n_1,\ldots,n_d)\in\NaturalNumbers^d$ and any formal
series $F$,
...
\[
 ...
 \right).
\]
Let $D_x=\DiagonalOperator(x_1,\ldots,x_d)$,
$D_\phi=\DiagonalOperator(\phi_1,\ldots,\phi_d)$, and
...
$n^{-3/2}$ coefficient factor.  The branch point $-\EulerE^{-1}$ of Lambert $W$, the
...
$\mu_{\Pi_m}(\PartitionLatticeMinimum,\PartitionLatticeMaximum)=(-1)^{m-1}(m-1)!$ for every $m$.

We prove this by induction on $m$.  If $\tau<\PartitionLatticeMaximum$ is a partition of
$[m]$, then $[\PartitionLatticeMinimum,\tau]$ is the product of the smaller lattices
$\Pi_{|B|}$ over $B\in\tau$.  The induction hypothesis and multiplicativity
therefore give
\[
 \mu_{\Pi_m}(\PartitionLatticeMinimum,\tau)
...
$\sum_{\tau\le\PartitionLatticeMaximum}\mu(\PartitionLatticeMinimum,\tau)=0$ therefore forces the remaining
...
\end{theorem}
\begin{proof}
Multiply the EGF $A(z)\EulerE^{x\beta(z)}$ by
$\EulerE^{y\beta(z)}$ and compare coefficients to prove the identity.  Conversely,
let $S(x,z)=\sum s_n(x)z^n/n!$ and
$P(y,z)=\sum p_n(y)z^n/n!=\EulerE^{y\beta(z)}$.  The assumed identity is
$S(x+y,z)=S(x,z)P(y,z)$.  Setting $x=0$ gives
$S(y,z)=S(0,z)\EulerE^{y\beta(z)}$; hence $A(z)=S(0,z)$, uniquely.
\end{proof}
...
\end{theorem}
\begin{proof}
Since $D_xe^{x\beta(z)}=\beta(z)\EulerE^{x\beta(z)}$,
\[
 \bar\beta(D_x)\EulerE^{x\beta(z)}
 =\bar\beta(\beta(z))\EulerE^{x\beta(z)}=ze^{x\beta(z)}.
\]
...
\midrule
Monomials & $(1,z)$ & $\EulerE^{xz}$; $Q=D$.\\
Falling factorials & $(1,\log(1+z))$ & $(1+z)^x$;
$Q=\EulerE^D-1=\Delta$.\\
Rising factorials & $(1,-\log(1-z))$ & $(1-z)^{-x}$;
$Q=1-\EulerE^{-D}$.\\
Touchard polynomials & $(1,\EulerE^z-1)$ & $\EulerE^{x(\EulerE^z-1)}$;
$Q=\log(1+D)$.\\
Bernoulli polynomials & $(z/(\EulerE^z-1),z)$ &
$ze^{xz}/(\EulerE^z-1)$; $Q=D$.\\
Euler polynomials & $(2/(\EulerE^z+1),z)$ &
$2\EulerE^{xz}/(\EulerE^z+1)$; $Q=D$.\\
Bernoulli polynomials of the second kind &
$(z/\log(1+z),\log(1+z))$ &
$z(1+z)^x/\log(1+z)$; $Q=\EulerE^D-1$.\\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{equation}\label{eq:riordan-sheffer-row}
 ...
 \qquad
 \sum_ns_n(x)\frac{z^n}{n!}=A(z)\EulerE^{x\beta(z)}.
\end{equation}
...
& Set partitions, Touchard polynomials, Dobi\'nski's formula, Bell-polynomial
  ...
  as the evaluation $\TouchardPolynomial{n}(-1)$.\\
...
\[
 ...
 \OrdinaryGeneratingFunctionOf{X}(z)=\sum_{j\ge1}\OrdinaryGeneratingCoefficient{x}{j}z^j.
\]
The exponential and ordinary partial Bell polynomials are characterized by
\begin{align*}
 \frac{X(z)^k}{k!}
 &=\sum_{n\ge k}\ExponentialPartialBellPolynomial nk(x_1,x_2,\ldots)\frac{z^n}{n!},\\
 \OrdinaryGeneratingFunctionOf{X}(z)^k
 &=\sum_{n\ge k}\OrdinaryPartialBellPolynomial nk(\OrdinaryGeneratingCoefficient{x}{1},\OrdinaryGeneratingCoefficient{x}{2},\ldots)z^n.
\end{align*}
Putting $x_j=j!\OrdinaryGeneratingCoefficient{x}{j}$ gives the conversion
\begin{equation}\label{eq:appendix-bell-normalization}
 \OrdinaryPartialBellPolynomial nk(\OrdinaryGeneratingCoefficient{x}{1},\OrdinaryGeneratingCoefficient{x}{2},\ldots)
 =\frac{k!}{n!}\ExponentialPartialBellPolynomial nk(1!\OrdinaryGeneratingCoefficient{x}{1},2!\OrdinaryGeneratingCoefficient{x}{2},\ldots).
\end{equation}
...
\[
 \FallingFactorial{x}{n}=\sum_{k=0}^n\SignedStirlingFirstKind{n}{k}x^k,
 \qquad
 \RisingFactorial{x}{n}=\sum_{k=0}^n\UnsignedStirlingFirstKind nkx^k,
 \qquad
 \SignedStirlingFirstKind{n}{k}=(-1)^{n-k}\UnsignedStirlingFirstKind nk.
\]
Consequently the inverse matrices are
\[
 (\SignedStirlingFirstKind{n}{k})_{n,k\ge0}^{-1}=(\StirlingSecondKind nk)_{n,k\ge0},
\]
whereas the unsigned array requires the sign diagonal:
\[
 (\UnsignedStirlingFirstKind nk)^{-1}_{n,k}=(-1)^{n-k}\StirlingSecondKind nk.
\]
...
\[
 ...
 \qquad \beta_1=-\frac12.
\]
Euler polynomials are the Appell sequence generated by $2\EulerE^{xt}/(\EulerE^t+1)$.
...
In one variable,
\[
 \ResidueOperator_t H(t)\,\Differential t=\ResidueOperator_x H(f(x))f'(x)\,\Differential x.
\]
...
\[
 ...
 =\sum_{j=1}^{n-k+1}\binom{n-1}{j-1}x_j
   \ExponentialPartialBellPolynomial{n-j}{k-1}(x_1,x_2,\ldots)
\]
...
\begin{align*}
 \ExponentialPartialBellPolynomial nk(cx_1,cx_2,\ldots)&=c^k\ExponentialPartialBellPolynomial nk(x_1,x_2,\ldots),\\
 \ExponentialPartialBellPolynomial nk(c x_1,c^2x_2,\ldots)&=c^n\ExponentialPartialBellPolynomial nk(x_1,x_2,\ldots).
\end{align*}
...
$4$, $3+1$, $2+2$, $2+1+1$, and $1+1+1+1$.  Their sum is $\BellNumber4=15$, checking
...
corresponding face-profile counts of $\AssociahedronByDimension{3}$.

\section{Matrix check for Stirling and Riordan inversion}
Truncating at any order $N$, multiply the signed first-kind matrix
$(\SignedStirlingFirstKind{n}{k})_{0\le n,k\le N}$ by the second-kind matrix
$(\StirlingSecondKind nk)_{0\le n,k\le N}$.  Factorial-basis substitution proves that the
product is the identity over $\IntegerNumbers$; direct multiplication is an independent finite
