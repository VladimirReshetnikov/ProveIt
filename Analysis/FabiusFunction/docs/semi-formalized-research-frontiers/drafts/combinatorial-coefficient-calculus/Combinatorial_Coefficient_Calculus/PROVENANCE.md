...
policy abolishing that filename convention.  The transitional twelve-payload
closure remains the live canonical TeX/PDF pair and the five donor TeX/PDF
pairs.  This file, `SOURCE_DISPOSITION.csv`, and `SOURCE_CLOSURE.sha256` are
consolidation metadata rather than source manuscripts.  At final retirement
the active closure contracts to the one canonical TeX/PDF pair; the commented
historical records continue to identify all six inputs.
