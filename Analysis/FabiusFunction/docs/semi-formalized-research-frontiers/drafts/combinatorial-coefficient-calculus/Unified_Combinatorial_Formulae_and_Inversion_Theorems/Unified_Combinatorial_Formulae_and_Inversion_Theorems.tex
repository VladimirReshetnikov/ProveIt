...
symbols, polygamma differences, the Barnes $G$-function, Darboux's principle, and
...
form is unique, so $h_C^*(t)=A_n(t)$; its coefficients are precisely the
$n$th Eulerian row.
\end{proof}
...
\begin{equation}
 \frac{\dd^n}{\dd x^n}\prod_{r=1}^q f_r(x)
...
\begin{equation}
 \left(\frac1g\right)^{(n)}
 =\sum_{k=0}^n(-1)^k k!\,
   \frac{\BellP nk(g',g'',\ldots,g^{(n-k+1)})}{g^{k+1}}.
...
\begin{equation}
 ...
 =\sum_{r=0}^n\binom nr f^{(n-r)}
   \sum_{k=0}^{r}(-1)^k k!\,
   \frac{\BellP rk(g',g'',\ldots,g^{(r-k+1)})}{g^{k+1}}.
...
\begin{proof}
Since $x=g(y)$ and $\dd y/\dd x=f'(x)$, differentiation along the inverse
curve satisfies
\[
 \frac{\dd}{\dd y}=\frac1{f'(x)}\frac{\dd}{\dd x}.
\]
...
$\hat0$ and its maximum is the one-block partition $\hat1$.
...
\eqref{new:eq:interval-mobius}, and the special case $\sigma=\hat0$ gives
...
\end{align}
Equivalently, $m_n=\BellC n(\kappa_1,\ldots,\kappa_n)$ and
...
\[
 G(\hat1)=\sum_{\pi\le\hat1}\mu(\pi,\hat1)F(\pi)
...
differential operator $b(D)$, where $D=\dd/\dd x$, is the associated delta
operator.
...
\[
 \stirtwo nk=R(1,e^t-1)_{n,k},
\]
while the signed first-kind matrix is
\[
 s(n,k)=R(1,\log(1+t))_{n,k}.
\]
...
\[
 ...
 F(\boldsymbol z)\,\dd\boldsymbol z
...
\end{equation}
For a multi-index $\boldsymbol n=(n_1,\ldots,n_d)\in\N^d$ and a formal series
$F$, one has
\begin{equation}
...
\[
 ...
 \prod_{i=1}^dt_i^{-n_i-1}\,\dd\boldsymbol t.
\]
...
\eqref{new:eq:good-formula}.  Indeed, write $X=\diag(x_i)$,
$\Phi=\diag(\phi_i)$, and $J=(\partial_j\phi_i)$.  The first matrix is
...
\end{equation}
Consequently the number of $(n-k)$-dimensional faces of $K_{n+1}$ is
\begin{equation}
...
\]
which is \eqref{new:eq:kirkman-cayley}.  Finally $K_{n+1}$ corresponds to an
$(n+2)$-gon and codimension $k-1$, so $N=n+2$ and $r=k-1$ give
\eqref{new:eq:associa-face}.
\end{proof}

For $K_5$ the formula gives $1,9,21,14$ faces in dimensions $3,2,1,0$,
...
\begin{align*}
 \frac{te^{xt}}{e^t-1}
 &=e^{xt}\int_0^1\frac{\dd u}{1+u(e^t-1)}\\
...
coefficient is $k!\stirtwo nk$, giving
...
\begin{equation}
 \beta_n(x)=-n!\sum_{k\in\Z\setminus\{0\}}
...
\begin{proof}
Let $c_k^{(n)}=\int_0^1\beta_n(x)e^{-2\pi ikx}\,\dd x$.  For $k\ne0$ and
...
At the last step,
\[
 c_k^{(1)}=\int_0^1\left(x-\frac12\right)e^{-2\pi ikx}\,\dd x
 =-\frac1{2\pi ik}.
\]
Thus $c_k^{(n)}=-n!/(2\pi ik)^n$.  The mean coefficient is zero for $n\ge1$
because $\int_0^1\beta_n(x)\dd x=0$, a consequence of the Appell identity and
...
For all $n\ge0$,
\begin{equation}
 G_n=2(1-2^n)\beta_n=nE_{n-1}(0)\quad(n\ge1),
...
indices gives $G_n=nE_{n-1}(0)$.
\end{proof}

Since $\beta_n=0$ for odd $n>1$, the same is true of $G_n$.  The first values
are
\[
 G_0=0,\quad G_1=1,\quad G_2=-1,\quad G_4=1,\quad
 G_6=-3,\quad G_8=17.
\]
...
Since
\[
 \frac{t}{\log(1+t)}=\int_0^1(1+t)^u\,\dd u,
\]
...
$n\falling{x}{n-1}$; this proves the derivative formula.  Replacing $x$ by
...
At $x=0$, expand $\falling u n=\sum_k s(n,k)u^k$ in the integral to obtain
\eqref{new:eq:bernoulli-second-stirling}.  Finally integrate
$c_n'(x)=n\sum_j s(n-1,j)x^j$ from $0$ to $x$, and put $k=j+1$.
\end{proof}
...
$\falling y n=(-1)^n\falling{n-1-y}{n}$.  Apply this in the integral
...
Define
\begin{equation}
 U_n=\sum_{k=0}^n(-1)^k\stirtwo nk.
 \label{new:eq:complementary-bell-def}
\end{equation}
Thus $U_n$ is the difference between the numbers of set partitions with an even
and an odd number of blocks, with the sign convention in
\eqref{new:eq:complementary-bell-def}.  Equivalently, $U_n=T_n(-1)$ for the
Touchard polynomial $T_n(x)$.

\begin{theorem}[Complementary Bell calculus]
The numbers $U_n$ satisfy
\begin{align}
 \sum_{n\ge0}U_n\frac{t^n}{n!}&=\exp(1-e^t),
 \label{new:eq:complementary-bell-egf}\\
 U_{n+1}&=-\sum_{k=0}^n\binom nkU_k,
 \label{new:eq:complementary-bell-recurrence}\\
 U_n&=e\sum_{m\ge0}\frac{(-1)^m m^n}{m!}.
...
$\sum T_n(x)t^n/n!=\exp(x(e^t-1))$ to obtain
...
$x-\lfloor x\rfloor-1/2$ away from integers.  On every open unit interval,
\[
...
\[
 \int_k^{k+1}f(x)\,\dd x
 =\frac{f(k)+f(k+1)}2
 -\int_k^{k+1}\left(x-k-\frac12\right)f'(x)\,\dd x.
\]
...
For $n\in\N$, the Pochhammer symbol is
\[
 (z)_n=\rising z n=z(z+1)\cdots(z+n-1),\qquad (z)_0=1.
\]
...
\begin{align}
 ...
 (z)_n&=\sum_{k=0}^n\stirone nk z^k,
 \label{new:eq:pochhammer-stirling}\\
 (-z)_n&=(-1)^n\falling z n.
...
\[
 \psi(z)=\frac{\Gamma'(z)}{\Gamma(z)},
 \qquad \psi^{(r)}(z)=\frac{\dd^r}{\dd z^r}\psi(z).
\]
...
\begin{equation}
 ...
 =(z)_n\,
 \BellC m\bigl(\Delta_0(z,n),\Delta_1(z,n),\ldots,
                    \Delta_{m-1}(z,n)\bigr).
...
Bruno states
\[
 \frac{\dd^m}{\dd z^m}e^{L(z)}
 =e^{L(z)}\BellC m(L',L'',\ldots,L^{(m)}),
\]
...
\begin{equation}
 \left.\frac{\dd^m}{\dd z^m}(z)_n\right|_{z=1}
 =n!\BellC m\bigl(H_n,-1!H_n^{(2)},2!H_n^{(3)},\ldots,
...
\chapter{The Barnes \texorpdfstring{$G$}{G}-function}
\label{ch:barnes-g}

The gamma function interpolates factorials; the Barnes $G$-function interpolates
...
\begin{proof}
The value $G(1)=1$ follows by putting $z=0$ in the product.  Form the quotient
...
$G(z+2)/G(z+1)=\Gamma(z+1)$, equivalent to
\eqref{new:eq:barnes-functional}.  Iterating from $G(1)=1$ proves the integer
formula.
\end{proof}
...
integrate as
\[
 \int t\log t\,\dd t=\frac{t^2}{2}\log t-\frac{t^2}{4},
 \qquad
 \int\log t\,\dd t=t\log t-t.
\]
...
$G(n+1)=\prod_{k=1}^{n-1}k!$; Euler--Maclaurin applied to the resulting double
sum identifies the constant as $\zeta'(-1)$.  Equivalently, defining the
...
\end{equation}
Indeed, $(n!)^n$ contains $k$ to exponent $n$, whereas $G(n+1)=1!2!\cdots
...
\[
 ...
 \int_{-\pi}^{\pi}r(\theta)e^{-in\theta}\,\dd\theta.
\]
...
\[
 ...
 \int_{-\pi}^{\pi}r^{(p)}(\theta)e^{-in\theta}\,\dd\theta.
\]
...
\]
Combining the exact formula $C_n=(n+1)^{-1}\binom{2n}{n}$ with Stirling's
...
\[
 [z^n]G(z)^k=\OBellP nk(b_1,b_2,\ldots).
\]
...
\nolinkurl{MathWorld/}
& Reference material on Barnes $G$, Bernoulli numbers and both kinds of Bernoulli
...
selected MathWorld entries supplied with the project: Barnes $G$-function, Bell
