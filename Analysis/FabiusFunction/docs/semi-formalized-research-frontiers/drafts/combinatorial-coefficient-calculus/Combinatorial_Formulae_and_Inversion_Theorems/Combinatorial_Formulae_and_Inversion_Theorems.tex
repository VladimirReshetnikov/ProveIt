...
$A_n(t)/(1-t)^{n+1}$.  The numerator in the Ehrhart-series normal form is
unique, hence $h^*_{[0,1]^n}(t)=A_n(t)$; coefficient comparison gives the
stated $h^*$-vector.
\end{proof}
...
\(\falling{x}{n}\) and \(\rising{x}{n}\) for falling and rising factorials;
when a formula is standardly written with the Pochhammer symbol, we set
\begin{equation}\label{eq:ext-pochhammer-def}
 (x)_n:=\rising{x}{n}=x(x+1)\cdots(x+n-1),\qquad (x)_0=1.
\end{equation}
Thus \(\falling{x}{n}=(-1)^n(-x)_n\).
...
\begin{align}
 ...
 \falling{x}{m+n}&=\falling{x}{m}\falling{x-m}{n},\label{eq:ext-falling-split}\\
...
\(\binom{x}{n}=\falling{x}{n}/n!=(-1)^n(-x)_n/n!\), gives
...
Let \(R\) be a commutative \(\Q\)-algebra and let \(\alpha\in R\).  In
\(R[[z]]\),
\begin{align}
 (1+z)^\alpha
 &=\sum_{n\ge0}\binom{\alpha}{n}z^n
 =\sum_{n\ge0}\frac{\falling{\alpha}{n}}{n!}z^n,\label{eq:ext-binomial-plus}\\
 (1-z)^{-\alpha}
...
\begin{proof}
Let
\(F(z)=\sum_{n\ge0}\falling{\alpha}{n}z^n/n!\).  Since
\(\falling{\alpha}{n+1}=(\alpha-n)\falling{\alpha}{n}\), coefficient
comparison gives
\[
...
\end{equation}
For exponential generating functions
\(\widehat A(z)=\sum a_nz^n/n!\) and
\(\widehat B(z)=\sum b_nz^n/n!\), the equivalent relation is
\begin{equation}\label{eq:ext-binomial-egf}
 \widehat B(z)=e^z\widehat A(z),\qquad
 \widehat A(z)=e^{-z}\widehat B(z).
\end{equation}
\end{corollary}
\begin{proof}
...
For \(n\ge1\),
\begin{align}
 \Delta\falling{x}{n}&=n\falling{x}{n-1},\label{eq:ext-delta-falling}\\
 \nabla\rising{x}{n}&=n\rising{x}{n-1},\label{eq:ext-nabla-rising}\\
 \frac{\dd}{\dd x}(x)_n
...
\[
 \falling{x+1}{n}-\falling{x}{n}
 =\falling{x}{n-1}\bigl((x+1)-(x-n+1)\bigr)
 =n\falling{x}{n-1}.
\]
...
Euler polynomials, \(\mathsf G_n\) for Genocchi numbers, and
...
by \(e^{yt}\) proves \eqref{eq:ext-bernoulli-translation}; taking \(x=0\)
...
polynomials.  Multiplying \eqref{eq:ext-euler-poly-egf} by \(e^t+1\) gives
\[
...
\begin{definition}
The Genocchi numbers \(\mathsf G_n\) are defined by
\begin{equation}\label{eq:ext-genocchi-egf}
 \frac{2t}{e^t+1}=\sum_{n\ge0}\mathsf G_n\frac{t^n}{n!}.
\end{equation}
\end{definition}
...
\end{equation}
In particular, \(\mathsf G_0=0\), \(\mathsf G_1=1\), and
\(\mathsf G_{2m+1}=0\) for \(m\ge1\).
\end{proposition}
\begin{proof}
...
\begin{align}
 ...
 +\sum_{k=1}^n\frac{n}{k}s(n-1,k-1)x^k,
 \label{eq:ext-cauchy-stirling}\\
 \mathsf C_n(x)&=\int_0^1\falling{x+u}{n}\,\dd u.
...
\]
Using
\((1+t)^x=\sum_{m\ge0}\falling{x}{m}t^m/m!\), comparison gives
\begin{equation}\label{eq:ext-cauchy-derivative}
 \mathsf C_n'(x)=n\falling{x}{n-1}.
\end{equation}
Expand \(\falling{x}{n-1}=\sum_{j=0}^{n-1}s(n-1,j)x^j\) and integrate from
...
\(\falling{n-1-y}{n}=(-1)^n\falling{y}{n}\):
\begin{align*}
 \mathsf C_n(n-2-x)
 &=\int_0^1\falling{n-2-x+u}{n}\dd u\\
 &=\int_0^1\falling{n-1-(x+1-u)}{n}\dd u\\
 &=(-1)^n\int_0^1\falling{x+1-u}{n}\dd u
 =(-1)^n\mathsf C_n(x),
\end{align*}
...
\begin{equation}\label{eq:ext-complementary-bell-def}
 \widetilde{\Bell n}:=\sum_{k=0}^n(-1)^k\stirtwo nk.
\end{equation}
\end{definition}
...
One has
\begin{align}
 \sum_{n\ge0}\widetilde{\Bell n}\frac{z^n}{n!}
 &=\exp(1-e^z),\label{eq:ext-complementary-bell-egf}\\
 \widetilde{\Bell n}
 &=e\sum_{m\ge0}\frac{(-1)^m m^n}{m!}.\label{eq:ext-complementary-dobinski}
\end{align}
...
\[
 \sum_{n\ge0}\widetilde{\Bell n}\frac{z^n}{n!}
 =\sum_{k\ge0}\frac{(-1)^k(e^z-1)^k}{k!}
 =e^{-(e^z-1)}.
\]
This is \eqref{eq:ext-complementary-bell-egf}.  Expanding the outer exponential
in the equivalent form \(e\,e^{-e^z}\) gives
\[
 e\sum_{m\ge0}\frac{(-1)^m}{m!}e^{mz}.
\]
...
\[
 A(z)e^{(x+y)\beta(z)}
 =\bigl(A(z)e^{x\beta(z)}\bigr)e^{y\beta(z)}.
\]
Apply the EGF product rule.  The second identity follows from
\(A(z)e^{x\beta(z)}=A(z)\sum p_n(x)z^n/n!\).  Conversely, summing either
...
\(A(z)=z/(e^z-1)\); Euler polynomials correspond to
\(A(z)=2/(e^z+1)\).  Their derivative laws
...
\[
 A(t)e^{x\beta(t)},\qquad
 A(t)=\frac{t}{\log(1+t)},\quad \beta(t)=\log(1+t).
\]
Its delta operator is \(Q=e^{D_x}-1=\Delta\), and hence
...
\(T=z e^{-aT}\).  Lagrange--B\"urmann gives, for \(n\ge1\),
\begin{align*}
 [z^n]e^{xT(z)}
 &=\frac1n[u^{n-1}]x e^{xu}e^{-anu}\\
 &=\frac{x(x-an)^{n-1}}{n!}.
\end{align*}
Thus
\[
 e^{xT(z)}=\sum_{n\ge0}p_n(x)\frac{z^n}{n!}.
\]
This is of the form \(e^{x\beta(z)}\), so
...
that \(e^z-1\) and \(\log(1+z)\) are compositional inverses.  The second is the
...
\begin{equation}\label{eq:ext-reciprocal-derivative}
 D^r\frac1{g(x)}
 =\sum_{k=0}^r(-1)^kk!\,
 \frac{\BellP rk(g'(x),g''(x),\ldots,g^{(r-k+1)}(x))}{g(x)^{k+1}},
\end{equation}
where \(\BellP00=1\).  Hence
\begin{equation}\label{eq:ext-higher-quotient}
 D^n\frac{f}{g}
 =\sum_{r=0}^n\binom nr f^{(n-r)}
 \sum_{k=0}^r(-1)^kk!\,
 \frac{\BellP rk(g',g'',\ldots)}{g^{k+1}}.
\end{equation}
\end{theorem}
\begin{proof}
...
\begin{equation}\label{eq:ext-good-system}
 ...
 \quad 1\le i\le d.
\end{equation}
For multi-indices \(\boldsymbol n\in\N^d\), write
...
\begin{equation}\label{eq:ext-multivariate-residue-substitution}
 \res_{\boldsymbol t} A(\boldsymbol t)\,
   \dd t_1\cdots\dd t_d
 =\res_{\boldsymbol x} A(\boldsymbol T(\boldsymbol x))
...
differential form
\[
 \Omega=\prod_i t_i^{m_i}\,\dd t_1\wedge\cdots\wedge\dd t_d.
\]
...
\[
 ...
 \prod_{j\ne i}t_j^{m_j}
 \dd t_1\wedge\cdots\widehat{\dd t_i}\cdots\wedge\dd t_d
 \right).
\]
...
\[
 \bigwedge_{i=1}^d\frac{\dd T_i}{T_i}
 =\det\left(
 \delta_{ij}+\frac{x_j}{u_i}\frac{\partial u_i}{\partial x_j}
 \right)_{i,j=1}^d
 \bigwedge_{j=1}^d\frac{\dd x_j}{x_j}.
\]
...
\(\Phi=\diag(\phi_1,\ldots,\phi_d)\), and let
...
\[
 0=\res F'(x)\phi(x)^nx^{-n}\dd x
   +n\res F(x)\phi(x)^{n-1}\phi'(x)x^{-n}\dd x
   -n\res F(x)\phi(x)^nx^{-n-1}\dd x.
\]
...
\[
 \res F(x)\phi(x)^nx^{-n-1}\dd x
 -\res F(x)\phi(x)^{n-1}\phi'(x)x^{-n}\dd x
 =\frac1n\res F'(x)\phi(x)^nx^{-n}\dd x.
\]
...
We now use the common convention that \(K_n\) is the \((n-2)\)-dimensional
...
For \(0\le d\le n-2\), the number of \(d\)-faces of \(K_n\) is
\begin{equation}\label{eq:ext-associahedron-face-number}
 f_d(K_n)=\frac1n\binom{n-2}{d}
                \binom{2n-d-2}{n-1}.
\end{equation}
In particular,
\begin{align}
 f_0(K_n)&=\frac1n\binom{2n-2}{n-1}=C_{n-1},
 \label{eq:ext-associahedron-vertices}\\
 f_{n-3}(K_n)&=\frac{(n+1)(n-2)}2,
...
\begin{example}
For \(K_4\), the formula gives \((f_0,f_1,f_2)=(5,5,1)\), the face vector of a
pentagon.  For \(K_5\), it gives \((14,21,9,1)\): fourteen vertices,
...
\begin{equation}\label{eq:ext-associahedron-h}
 h_{K_n}(u)=\sum_{r=0}^{n-2}N(n-1,r+1)u^r
...
The coefficients of \(h_{K_n}(u)\) are palindromic.
\end{corollary}
\begin{proof}
...
Two vertices of \(K_n\) are adjacent exactly when their triangulations differ by
...
lattice.  The pentagonal \(K_4\) is therefore not merely a picture of five
...
\end{equation}
Consequently, for all \(z\in\C\),
\begin{equation}\label{eq:ext-weierstrass-gamma}
 \frac1{\Gamma(z+1)}
 =e^{\gamma z}\prod_{k=1}^\infty
...
\[
 \int_0^1t^{z-1}(1-t)^N\dd t
 =\frac{N!}{z(z+1)\cdots(z+N)}.
\]
Multiply by \(N^z\) and substitute \(u=Nt\):
\[
 \frac{N!N^z}{z(z+1)\cdots(z+N)}
 =\int_0^N u^{z-1}\left(1-\frac uN\right)^N\dd u.
\]
For fixed \(u\), the factor in parentheses tends to \(e^{-u}\); moreover
\((1-u/N)^N\le e^{-u}\) on \(0\le u\le N\).  Dominated convergence on a
...
\[
 \lim_{N\to\infty}e^{\gamma z+o(1)}
 \prod_{k=1}^N\left(1+\frac zk\right)e^{-z/k},
\]
...
\((1+(z+1)/N)^N\to e^{z+1}\).  The displayed expression differs by a factor
tending to one from Euler's limit
\[
 \Gamma(z+1)=\lim_{N\to\infty}
 \frac{N^zN!}{\prod_{k=1}^N(z+k)}.
\]
Therefore \(G(z+2)/G(z+1)=\Gamma(z+1)\), equivalent to
\eqref{eq:ext-barnes-recurrence}.  Setting \(z=0\) in the product gives
\(G(1)=1\).  Iterating the recurrence at positive integers proves the first
...
\begin{equation}\label{eq:ext-alexeiewsky}
 \int_0^x\log\Gamma(1+t)\dd t
 =\frac{x}{2}\log(2\pi)-\frac{x(x+1)}2
  +x\log\Gamma(x+1)-\log G(x+1).
\end{equation}
...
\begin{align}
 \log G(x+1)={}&\frac{x^2}{2}\log x-\frac{3x^2}{4}
 +\frac{x}{2}\log(2\pi)-\frac1{12}\log x+\zeta'(-1)
 \notag\\
 &+\sum_{r=1}^{p-1}
...
\(\zeta(s)=\sum_{k\ge1}k^{-s}\), differentiating the continued formula at
...
\(-\sum_{k\le n}k\log k\) to the other side leaves \(\zeta'(-1)\).
...
Alexeiewsky's identity \eqref{eq:ext-alexeiewsky} for \(\log G(x+1)\), split
...
constant as \(\zeta'(-1)\), completing the proof.
\end{proof}

\begin{remark}
The constant \(A=\exp(1/12-\zeta'(-1))\) is the Glaisher--Kinkelin constant.
...
\begin{equation}\label{eq:ext-periodic-bernoulli-derivative}
 \frac{\dd}{\dd x}\frac{\widetilde{\mathsf B}_m(x)}{m!}
...
\begin{equation}\label{eq:ext-bernoulli-fourier-complex}
 ...
 =-\frac{m!}{(2\pi i)^m}
 \sum_{k\in\Z\setminus\{0\}}\frac{e^{2\pi ikx}}{k^m},
\end{equation}
...
\begin{proof}
For nonzero \(k\), let
\(c_k^{(m)}=\int_0^1\mathsf B_m(x)e^{-2\pi ikx}\dd x\).  Integration by
...
\(\int_0^1\mathsf B_m(x)\dd x=0\) for \(m\ge1\), by
...
\begin{align}
 ...
 \notag\\
 &-\frac1{(2p)!}\int_a^b
   \widetilde{\mathsf B}_{2p}(x)f^{(2p)}(x)\dd x.
...
\[
 \int_k^{k+1}\widetilde{\mathsf B}_1(x)f'(x)\dd x
...
\begin{equation}\label{eq:ext-em-start}
 \sum_{k=a}^{b}f(k)
 =\int_a^b f(x)\dd x+\frac{f(a)+f(b)}2
 +\int_a^b\widetilde{\mathsf B}_1(x)f'(x)\dd x.
\end{equation}
...
\begin{align}
 \sum_{k=a}^{\infty}f(k)
 ={}&\int_a^\infty f(x)\dd x+\frac{f(a)}2
...
\(O(\int_n^\infty x^{-2p-1}\dd x)=O(n^{-2p})\) by
...
\begin{align}
 ...
 \notag\\
 &+(-1)^n(z-a)^{n+1}
 \int_0^1\phi(t)f^{(n+1)}(a+t(z-a))\dd t.
...
\(C_n=\binom{2n}{n}/(n+1)\).  Stirling's expansion gives
\[
...
refinement, with bottom \(\widehat0\) and top \(\widehat1\).
...
\(K(\widehat1)=\sum_{\pi\le\widehat1}
 \mu(\pi,\widehat1)M(\pi)\).  Since
\(K(\widehat1)=\kappa_n\), substitution of
...
\begin{equation}\label{eq:ext-kepler}
 E=M+e\sin E.
\end{equation}
Since \(\dd(E-e\sin E)/\dd E=1-e\cos E>0\), the map from eccentric anomaly
...
Fourier coefficients \(O(e^{-\eta_e n})\) by shifting the coefficient contour
...
\begin{equation}\label{eq:ext-alg-composition}
 [z^n]A(B(z))
 =\sum_{k=0}^na_k\OBellP nk(b_1,\ldots,b_{n-k+1}).
\end{equation}
If \(A(0)=0\), \(a_1\ne0\), and
...
\begin{align}
 ...
 \OBellP nk(b_1,\ldots,b_{n-k+1}),\qquad n\ge2.
