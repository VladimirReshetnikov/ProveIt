...
to the Prouhet--Thue--Morse sign $\ThueMorseSign{n-1}=(-1)^{\BinaryDigitSum(n-1)}$:
\[
 X_+=\sum_{\ThueMorseSign{n-1}=1}U_n,
 \qquad
 X_-=\sum_{\ThueMorseSign{n-1}=-1}U_n.
\]
...
product
\[
 E(q)=\sum_{m\ge0}\ThueMorseSign{m}q^m=\prod_{j\ge0}(1-q^{2^j}),
\qquad
...
\[
 \MellinTransformSymbol J(s)=-\Gamma(s)\zeta(s+1)E(2^s),
 \qquad -2<\Re s<-1,
\]
...
Fourier convention
\(
 \FourierAngular{f}(-z)=\int_\RealNumbers \EulerE^{\ImaginaryUnit zx}f(x)\Differential x
\)
and with $\SincRad z=\sin z/z$, evenness and independence give
\begin{equation}
 \FourierAngular{\RvachevUp}(z)=\prod_{n\ge1}\SincRad\!\left(\frac z{2^n}\right).
 \label{eq:up-product}
\end{equation}
...
Fabius function $\FabiusBounded$.  Thus
\begin{equation}
 \FabiusBounded(x)=\int_{-1}^{2x-1}\RvachevUp(t)\Differential t,
 \qquad 0\le x\le1,
 \label{eq:F-up-cdf}
\end{equation}
...
\begin{equation}
 \RvachevUp(x)=
 \begin{cases}
 \FabiusBounded(1+x),&-1\le x\le0,\\
 \FabiusBounded(1-x),&0\le x\le1.
 \end{cases}
 \label{eq:F-up-pointwise}
\end{equation}
The arithmetic of the dyadic values of $\FabiusBounded$, including integrality phenomena originating
...
For any set $A\subseteq\PositiveIntegers$, let
\begin{equation}
...
For every $A\subseteq\PositiveIntegers$,
\begin{equation}
 \phi_A(z)=\prod_{n\in A}\SincRad\!\left(\frac z{2^n}\right)
 \label{eq:subset-product}
\end{equation}
locally uniformly on $\ComplexNumbers$.  The function is entire of exponential type at most $R_A$,
$\mu_A$ is symmetric, and
\(
 \SupportOperator\mu_A=[-R_A,R_A]
\)
unless $A=\varnothing$.  If $A^c=\PositiveIntegers\setminus A$, then
\begin{equation}
 \mu_A*\mu_{A^c}=\mu_{\PositiveIntegers},
 \qquad
 \phi_A\phi_{A^c}=\FourierAngular{\RvachevUp}.
...
of $\ComplexNumbers$, $\SincRad(z/2^n)=1+\BigO(4^{-n})$, so the product converges locally uniformly and is
...
Let
\begin{equation}
 \ThueMorseSign{m}\DefinitionEquals(-1)^{\BinaryDigitSum(m)},\qquad m\ge0,
 \label{eq:tm-sign}
\end{equation}
where $\BinaryDigitSum(m)$ is the sum of the binary digits of $m$.  Put
\begin{equation}
 A_+=\SetOf{n\ge1:\ThueMorseSign{n-1}=1},
 \qquad
 A_-=\SetOf{n\ge1:\ThueMorseSign{n-1}=-1}.
 \label{eq:Apm}
\end{equation}
...
Mahler product is
\begin{equation}
 E(q)=\sum_{m\ge0}\ThueMorseSign{m}q^m
 =\prod_{j\ge0}(1-q^{2^j}),
 \qquad |q|<1,
 \label{eq:Eq}
\end{equation}
and satisfies $E(q)=(1-q)E(q^2)$.  The product identity follows immediately from
$\ThueMorseSign{2m}=\ThueMorseSign{m}$ and $\ThueMorseSign{2m+1}=-\ThueMorseSign{m}$; it is a standard Thue--Morse identity, surveyed for
...
\begin{equation}
 \boxed{\ \mu_+*\mu_-=\mu_{\RvachevUp},\qquad
 u_+*u_-=\RvachevUp\ },
...
\begin{equation}
 ...
 \SincRad\!\left(\frac z{2^{m+1}}\right)^{(1+\sigma\ThueMorseSign{m})/2},
 \qquad \sigma\in\{+1,-1\}.
 \label{eq:phi-pm}
\end{equation}
\end{theorem}
...
\begin{proof}
The two selectors $(1+\ThueMorseSign{m})/2$ and $(1-\ThueMorseSign{m})/2$ are complementary at every index.  Apply
...
\begin{align}
 \sum_{m\ge0}\frac{1+\sigma\ThueMorseSign{m}}{2}q^{m+1}
 &=\frac q2\left(\sum_{m\ge0}q^m+
       \sigma\sum_{m\ge0}\ThueMorseSign{m}q^m\right)\\
...
is
\begin{equation}
 f(x)=\frac1{2a}\int_{x-a}^{x+a}g(y)\Differential y.
...
\end{equation}
If $g\in C^\infty_c(\RealNumbers)$, then on the right transition interval $a-r<x<a+r$,
\begin{equation}
...
$f(x)=(2a)^{-1}\int_{x-a}^r g(y)\Differential y$.  Differentiation gives
...
\(
 \RvachevUp(0)=\int u_+(-y)u_-(y)\Differential y.
\)
The support of $u_-$ lies in the closed plateau of $u_+$, where $u_+=1$, giving
$\RvachevUp(0)=\int u_-=1$.  Once \cref{thm:smoothness} is available, differentiation under the
integral gives
\[
 \RvachevUp^{(k)}(0)=\int u_+^{(k)}(-y)u_-(y)\Differential y=0.
\]
...
\begin{equation}
 \Phi_\sigma(z;q)=\prod_{m\ge0}
 \SincRad(zq^{m+1})^{a_m^\sigma},
 \qquad
 a_m^\sigma=\frac{1+\sigma\ThueMorseSign{m}}{2}.
 \label{eq:Phiq}
\end{equation}
The product converges locally uniformly on $\ComplexNumbers\times\{q:|q|<1\}$ and is entire in $z$.
...
$\ThueMorseSign{2r}=\ThueMorseSign{r}$ and $\ThueMorseSign{2r+1}=-\ThueMorseSign{r}$, the even-indexed factors are
\[
 \prod_{r\ge0}\SincRad(zq^{2r+1})^{a_r^\sigma}
 =\Phi_\sigma(z/q;q^2),
\]
while the odd-indexed factors are
\[
 \prod_{r\ge0}\SincRad(zq^{2r+2})^{a_r^{-\sigma}}
 =\Phi_{-\sigma}(z;q^2).
\]
...
\begin{equation}
 K_{\sigma,r}(t)=\sum_{m\ge0}\frac{1+\sigma\ThueMorseSign{m+r}}2
...
\begin{equation}
 K_{\sigma,r}(2t)
 =\frac{1+\sigma\ThueMorseSign{r}}{2}h(t)+K_{\sigma,r+1}(t).
...
\begin{equation}
 \log\SincRad w=-\sum_{r\ge1}\frac{\zeta(2r)}{r\pi^{2r}}w^{2r}.
 \label{eq:logsinc}
\end{equation}
...
\begin{equation}
 K_\sigma(t;q)=\log\Expectation\EulerE^{tX_\sigma(q)}
...
Let $m_n^\sigma(q)=\Expectation[X_\sigma(q)^n]$.  Then
\begin{equation}
 m_n^\sigma(q)=\ExponentialCompleteBellPolynomial{n}\bigl(\kappa_1^\sigma(q),\ldots,
 \kappa_n^\sigma(q)\bigr),
 \label{eq:bell-moments}
\end{equation}
where $\ExponentialCompleteBellPolynomial{n}$ is the complete exponential Bell polynomial.  Equivalently,
\begin{equation}
 m_0^\sigma=1,
 \qquad
...
\begin{equation}
 ...
 \qquad
 c_n^\sigma=\frac{2n+1}{2}\Expectation[P_n(Y_\sigma)].
...
\begin{proof}
Use $|\SincRad y|\le\min(1,|y|^{-1})$.  Put
...
\[
 \sum_{m=0}^{2r-1}\ThueMorseSign{m}=0,
 \qquad
 \sum_{m=0}^{2r}\ThueMorseSign{m}=\ThueMorseSign{r}.
\]
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fourier_decay.png}
\caption{Numerical values along $x=\EulerE^L+\sqrt2$.  Exact zeros prevent a global two-sided
...
Each factor $\SincRad(z/2^n)$ has simple zeros at $z=\pi\ell2^n$, $\ell\in\IntegerNumbers\setminus\{0\}$.
The zeros of the full Rvachev transform are therefore $2\pi k$, $k\ne0$, with multiplicity
$1+\TwoAdicValuation(k)$.  The automatic splitting gives a finer formula.

\begin{theorem}[Thue--Morse zero multiplicities]
\label{thm:zero-multiplicity}
Let $k\in\IntegerNumbers\setminus\{0\}$, put $N=1+\TwoAdicValuation(k)$, and define
\begin{equation}
 S_N=\sum_{m=0}^{N-1}\ThueMorseSign{m}.
 \label{eq:SN}
\end{equation}
...
\[
 \sum_{m=0}^{N-1}\frac{1+\sigma\ThueMorseSign{m}}{2}
 =\frac{N+\sigma S_N}{2}.
\]
The prefix identities follow by pairing $\ThueMorseSign{2j}+\ThueMorseSign{2j+1}=0$.
\end{proof}
...
extend to entire functions and $\phi_\mu=\phi_\nu^r$ on $\ComplexNumbers$.  Every zero multiplicity of
...
\begin{remark}
Thus the factorization $\mu_+*\mu_-=\mu_{\RvachevUp}$ is genuinely asymmetric: it does not arise
...
Let
\begin{equation}
 K_\sigma(t)=\log\Expectation\EulerE^{tX_\sigma}
...
\begin{equation}
 ...
 \sum_{n\ge1}h(t/2^n),
 \qquad
 J=K_+-K_-=\sum_{m\ge0}\ThueMorseSign{m}h(t/2^{m+1}).
 \label{eq:K0J}
\end{equation}
...
For $-2<\Re s<-1$,
\begin{equation}
 \MellinTransformSymbol h(s):=\int_0^\infty h(t)t^{s-1}\Differential t
...
\[
 \int_0^\infty y^{s-1}\log(1+y^2)\Differential y
...
For $-2<\Re s<-1$,
\begin{align}
 \MellinTransformSymbol K_0(s)
 &=-\frac{\Gamma(s)\zeta(s+1)}{1-2^s},
 \label{eq:mellin-K0}\\
 \MellinTransformSymbol J(s)
 &=-\Gamma(s)\zeta(s+1)E(2^s),
 \label{eq:mellin-J}\\
 \MellinTransformSymbol K_\sigma(s)
...
\(
 \MellinTransformSymbol[h(t/2^n)](s)=2^{ns}\MellinTransformSymbol h(s).
\)
Therefore
\[
 \sum_{n\ge1}2^{ns}=\frac{2^s}{1-2^s},
 \qquad
 \sum_{m\ge0}\ThueMorseSign{m}2^{(m+1)s}=2^sE(2^s).
\]
...
Let $A\subseteq\PositiveIntegers$ and suppose $A(N)=\delta N+\BigO(1)$.  Write
\begin{equation}
 D_A=R_A-X_A\ge0,
 \qquad
...
\begin{equation}
 ...
 \qquad
 \ell(y)=\log\frac{1-\EulerE^{-y}}{y}.
 \label{eq:ell-sum}
\end{equation}
For $y\ge1$, $\ell(y)=-\log y+\BigO(\EulerE^{-y})$; for $0<y\le1$,
...
$\Expectation\EulerE^{tX_A}=\EulerE^{R_At}\Expectation\EulerE^{-tD_A}$, the second
formula follows.
\end{proof}
...
\[
 G_A(\eps)
 =\Pr(\EulerE^{-tD_A}\ge\EulerE^{-t\eps})
 \le\EulerE^{t\eps}\Expectation\EulerE^{-tD_A}.
\]
...
$N=\Ceiling{\log_2(1/\eps)}+3$.  The deterministic tail after $N$ is at most
\(
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{laplace_mgf_asymptotics.png}
...
\begin{equation}
 \log t
 =-W_{-1}\!\left(-\frac{\eps}{2c}\EulerE^{d/(2c)}\right)-\frac d{2c}.
...
\end{equation}
It is piecewise polynomial of degree $m-1$, with knots on the lattice $2^{1-N}\IntegerNumbers$.
...
\item the complex product identity $\phi_+\phi_-=\FourierAngular{\RvachevUp}$ with residual
...
$s=-1.4+0.3\ImaginaryUnit$, with numerical integration errors about $10^{-22}$ and $10^{-27}$;
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{factor_densities.png}
...
The green curve is the coupled sum, whose exact law is $\RvachevUp$.}
...
\begin{align}
 ...
 \log\Expectation\EulerE^{-t(R_{A,b}-X_{A,b})}
...
$\Floor{\log_2t}$,
\begin{equation}
 \log\Expectation\EulerE^{-tD_\sigma}
 =-\frac{(\log t)^2}{4\log2}
...
\label{conj:rigidity}
Let $\rho_1,\rho_2$ be centered symmetric compactly supported probability measures with
entire characteristic functions of Cartwright class, and suppose
$\rho_1\Convolution\rho_2=\mu_{\RvachevUp}$.  Under the additional condition that every zero of each factor
...
involution induced by $\ThueMorseSign{2m+1}=-\ThueMorseSign{m}$.
\end{conjecture}
...
Formalize $\ThueMorseSign{n}=(-1)^{\BinaryDigitSum(n)}$, the recurrences
$\ThueMorseSign{2n}=\ThueMorseSign{n}$, $\ThueMorseSign{2n+1}=-\ThueMorseSign{n}$, prefix sums \eqref{eq:prefix-exact}, selector counts, and
...
$|\SincRad x|\le\min(1,1/|x|)$.  The weighted selector count then gives rapid decay.  Existing
...
$\ell(y)=\log((1-\EulerE^{-y})/y)$ on $(0,1]$ and $[1,\infty)$.  The small-ball lower bound is a
...
compactly supported laws whose convolution is $\RvachevUp$, whose supports are measured by a
...
Let
\begin{equation}
 T_N=\sum_{m=0}^{N-1}m\ThueMorseSign{m}.
 \label{eq:weighted-prefix-def}
\end{equation}
Pairing even and odd indices gives
\begin{align}
 T_{2M}
 &=\sum_{r=0}^{M-1}\bigl(2r\ThueMorseSign{2r}+(2r+1)\ThueMorseSign{2r+1}\bigr)
 =-\sum_{r=0}^{M-1}\ThueMorseSign{r}=-S_M,
 \label{eq:T-even}\\
 T_{2M+1}
 &=T_{2M}+2M\ThueMorseSign{2M}
 =-S_M+2M\ThueMorseSign{M}.
 \label{eq:T-odd}
\end{align}
...
\begin{equation}
 \Variance(X_+)+\Variance(X_-)=\frac19,
