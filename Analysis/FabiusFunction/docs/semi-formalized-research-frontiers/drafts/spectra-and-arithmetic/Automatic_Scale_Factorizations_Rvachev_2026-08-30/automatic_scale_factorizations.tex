...
\[
 ...
 \SincRad(zq^{m+1})^{(1+\sigma e_m)/2},\qquad \sigma\in\{+1,-1\},
\]
...
Fourier convention
\(
 \widehat f(z)=\int_\RealNumbers \EulerE^{\ImaginaryUnit zx}f(x)\Differential x
\)
and with $\SincRad z=\sin z/z$, independence gives
\begin{equation}
 \widehat\RvachevUp(z)=\prod_{n\ge1}\SincRad\!\left(\frac z{2^n}\right).
 \label{eq:up-product}
\end{equation}
...
\begin{equation}
 ...
 \qquad
 \phi_A\phi_{A^c}=\widehat\RvachevUp.
...
\begin{equation}
 ...
 \SincRad\!\left(\frac z{2^{m+1}}\right)^{(1+\sigma e_m)/2},
 \qquad \sigma\in\{+1,-1\}.
 \label{eq:phi-pm}
\end{equation}
\end{theorem}
...
\item the complex product identity $\phi_+\phi_-=\widehat\RvachevUp$ with residual
