...
function $G_r$ is nonrational, hypertranscendental over $\C(z)$, and has the
...
\[
 L_r(q)=\sum_{n\geq1}\binom{\vtwo(n)+1}{r}q^n
\]
...
on the lattice $Q^{-1}\Z$, the density $f_r$ integrates every polynomial of
degree strictly below $\binom{\vtwo(Q)+1}{r}$ by an exact finite periodized
...
zero multiplicities $1+\vtwo(n)$, and exact zero jets;
\item the Pascal--Rvachev hierarchy, its random-series model, support,
refinement law, multiplicity formula
$\mu_r(n)=\binom{\vtwo(n)+1}{r}$, spectral zeta function, digital sign law,
...
Higher-rank comb exactness & \New & Sharp degree $\binom{\vtwo(Q)+1}{r}-1$ for polynomial cubature and Appell reconstruction on $Q^{-1}\Z$.\\
...
Set $\Phi_0(z)=\sinc(2z)$.  For $r\geq1$, let
\begin{equation}\label{eq:Phi-r}
 a_{r,h}:=\binom{h}{r-1},\qquad
 \Phi_r(z):=\prod_{h\geq0}\sinc\!\left(\frac{z}{2^h}\right)^{a_{r,h}}.
\end{equation}
...
\begin{equation}\label{eq:characteristic}
 \E\e^{-2\pi\ii zX_r}=\Phi_r(z),\qquad \supp X_r=[-1,1].
\end{equation}
...
\begin{equation}\label{eq:mu-r}
 \mu_r(n)=\sum_{h=0}^{\vtwo(|n|)}\binom h{r-1}
 =\binom{\vtwo(|n|)+1}{r}.
\end{equation}
Consequently
...
Let $m_{r,n}=\E X_r^n$.  Symmetry kills odd moments.  Taking logarithms in
...
\begin{equation}\label{eq:A-def-early}
 \Ar_r(w):=\Phi_r(\ii\sqrt w)=\sum_{n\geq0}C_{r,n}w^n.
\end{equation}
Then
...
linear Mahler equation over $\C(z)$ involving $f(z),f(z^p),\ldots,f(z^{p^m})$.
Then either the solution belongs to the ramified rational field
$\bigcup_{d\geq1}\C(z^{1/d})$, or it is hypertranscendental over $\C(z)$.
\end{theorem}
...
ramification forces the rational function to descend to $\C(z)$.
...
\item $G_r(z)$ is hypertranscendental over $\C(z)$;
...
\]
are algebraically independent over $\overline\Q$.
\end{corollary}
...
\begin{proof}
If $v=\vtwo(n)$, only $h\leq v$ contribute, and the hockey-stick identity
gives
...
For every $r\geq1$:
\begin{enumerate}[label=(\roman*)]
\item $L_r$ is hypertranscendental over $\C(q)$;
...
\begin{equation}\label{eq:comb}
 \mathcal C_{r,Q}[p](x):=\frac{1}{Q}\sum_{k\in\Z}
...
\begin{proof}
Apply Poisson summation to $g_j(t)=t^jf_r(t)$ on the lattice $Q^{-1}\Z$.
Our Fourier convention gives
$\widehat g_j(\xi)=\Phi_r^{(j)}(\xi)/(-2\pi\ii)^j$.
At $\xi=0$ this is $\int t^jf_r(t)\dd t=m_{r,j}$.
\end{proof}
...
\begin{equation}\label{eq:M-rQ}
 M_r(Q):=\mu_r(Q)=\binom{\vtwo(Q)+1}{r}.
\end{equation}
If $p$ is a polynomial with $\deg p<M_r(Q)$, then for every real $x$,
\begin{equation}\label{eq:comb-exact}
 \boxed{
 \frac1Q\sum_{k\in\Z}p\!\left(x-\frac{k}{Q}\right)
 f_r\!\left(x-\frac{k}{Q}\right)=\E p(X_r).}
\end{equation}
...
For $m\neq0$,
\[
 \mu_r(Qm)=\binom{\vtwo(Q)+\vtwo(m)+1}{r}\geq M_r(Q).
\]
...
At $r=1$, $M_1(Q)=1+\vtwo(Q)$, exactly the sharp rank-one hierarchy already
proved in the dyadic-comb and frontier-compilation volumes.  The new content is
the Pascal-rank lift and its binomial growth in $\vtwo(Q)$.
\end{remark}
...
\begin{equation}\label{eq:mgf}
 \Mr_r(z):=\E\e^{zX_r}=\Phi_r\!\left(\frac{\ii z}{2\pi}\right).
\end{equation}
...
\begin{equation}\label{eq:Appell-reproduce}
 \boxed{
 \frac1Q\sum_{k\in\Z}P_{r,n}\!\left(\frac{k}{Q}\right)
...
is $\E P_{r,n}(x-X_r)$.  Taking the exponential generating function and using
symmetry,
\[
 \sum_{n\geq0}\E P_{r,n}(x-X_r)\frac{z^n}{n!}
 =\frac{\e^{xz}\E\e^{-zX_r}}{\Mr_r(z)}
 =\e^{xz}.
\]
...
\begin{equation}\label{eq:Legendre-comb}
 \int_{-1}^{1}\mathrm P_n(t)f_r(t)\dd t
...
\begin{equation}\label{eq:A-mgf}
 \Ar_r(w)=\E\e^{2\pi\sqrt w X_r}=\E\e^{-2\pi\sqrt w X_r}
...
$\C(z)$.  More strongly, finite sets of distinct ranks should be
...
\begin{equation}\label{eq:p-n}
 p_{n,\theta}:=\Prob(B_{n,j}=1)=\frac{\theta}{n^2+\theta}.
\end{equation}
Since
...
Then
\begin{align}
 \lambda_{r,\theta}:=\E N_{r,\theta}&=S_{r,1}(\theta),\label{eq:PB-mean}\\
 \Var(N_{r,\theta})&=S_{r,1}(\theta)-S_{r,2}(\theta),\label{eq:PB-var}\\
 \kappa_k^{(F)}(N_{r,\theta})
...
\begin{proof}
Use $\log\E(1+t)^N=\sum_{n,j}\log(1+p_{n,\theta}t)$ and compare powers of
$t$.
\end{proof}
...
\end{equation}
With $\TV$ defined as the supremum over events,
\begin{equation}\label{eq:critical-TV}
 \TV\!\left(N_{r,c3^r},\Po(4c\zeta(2))\right)
...
\[
 \TV(N_{r,\theta_r},\Po(\lambda_{r,\theta_r}))
 \leq2S_{r,2}(\theta_r)
...
\begin{tabular}{@{}rrrr@{}}
\toprule
$r$ & $\E N_{r,3^r}$ & target minus mean & proved TV bound\\
\midrule
...
$\lambda_r=\E N_{r,\theta_r}$ and
$\sigma_r^2=\Var(N_{r,\theta_r})$.
...
\]
Expanding
$\log\E(1+t)^N=\sum_{k\geq1}(-1)^{k-1}S_{r,k}t^k/k$ proves
...
\[
 \lambda_r=\theta_r\ZetaR_r(2)(1+\smallo(1)),\qquad
 S_{r,2}(\theta_r)=\smallo(\lambda_r),
\]
so \eqref{eq:mean-var-asymp} holds and $\sigma_r\to\infty$.  For a Bernoulli
variable $B$ with parameter $p$,
$\E|B-p|^3\leq p(1-p)$.  Hence the sum of third absolute centered moments is
...
\begin{equation}\label{eq:V-mean-var}
 \E V_{r,\tau}=\frac{4\tau\zeta(2)}{3^r},
 \qquad
 \Var(V_{r,\tau})=\frac{16\tau\zeta(4)}{15^r}.
\end{equation}
Therefore
\begin{equation}\label{eq:V-relative}
 \frac{\Var(V_{r,\tau})}{(\E V_{r,\tau})^2}
...
\begin{equation}\label{eq:V-L2}
 \frac{V_{r,\tau}}{\E V_{r,\tau}}\longrightarrow1
 \qquad\text{in }L^2.
\end{equation}
\end{corollary}
...
$\sigma_r^2=\Var(V_{r,\tau})$.  Then
\begin{align*}
 \log\E\e^{\ii tW_{r,\tau}}
 &=-\frac{\ii t\E V_{r,\tau}}{\sigma_r}
 -\tau\sum_n\mu_r(n)
 \log\!\left(1-\frac{\ii t}{\sigma_r n^2}\right).
\end{align*}
...
\begin{equation}\label{eq:Legendre-coeff}
 \ell_{r,n}:=\frac{2n+1}{2}\int_{-1}^{1}f_r(t)\mathrm P_n(t)\dd t
\end{equation}
...
\url{https://github.com/VladimirReshetnikov/ProveIt/blob/main/Analysis/FabiusFunction/docs/semi-formalized-research-frontiers/drafts/inverse-and-sampling/comb-interpolation/Dyadic_Comb_Frontiers/Dyadic_Comb_Frontiers.tex},
retrieved 30 August 2026.
