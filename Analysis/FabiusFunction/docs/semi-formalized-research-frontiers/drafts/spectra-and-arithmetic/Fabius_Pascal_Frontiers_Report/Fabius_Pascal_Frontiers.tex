...
\hypersetup{
  ...
  pdfauthor={Prepared from the ProveIt Fabius-function document corpus},
...
\begin{equation}\label{eq:Bell-coeff}
 C_{r,n}=\frac{1}{n!}\ExponentialCompleteBellPolynomial{n}(x_{r,1},\ldots,x_{r,n}),
 \qquad
 x_{r,k}=(-1)^{k+1}(k-1)!\ZetaR_r(2k),
\end{equation}
where $\ExponentialCompleteBellPolynomial{n}$ is the complete exponential Bell polynomial, and
...
\begin{equation}\label{eq:Pr}
 P_r:=2^{\Ceiling{\log_2r}},\qquad B_r:=2^{P_r}.
\end{equation}
...
\begin{equation}\label{eq:mu-layer}
 \mu_r(n)=\sum_{h\geq r-1}\binom h{r-1}\IndicatorOf{2^h\mid n}.
\end{equation}
Consequently,
...
$\IndicatorOf{2^h\mid n}q^n$ over $n$ gives $q^{2^h}/(1-q^{2^h})$.
\end{proof}
...
$\FourierTwoPi{g_j}(\xi)=\Phi_r^{(j)}(\xi)/(-2\pi\ImaginaryUnit)^j$.
...
\begin{equation}\label{eq:moment-Bell}
 ...
 \ExponentialCompleteBellPolynomial{n}\!\left(
...
\url{https://github.com/VladimirReshetnikov/ProveIt/blob/main/Analysis/FabiusFunction/docs/semi-formalized-research-frontiers/drafts/exponents-and-q-series/geometric-sinc-and-exponent-families/Exponents_and_q_Series_Frontiers/Exponents_and_q_Series_Frontiers.tex},
retrieved 30 August 2026.
