...
\newcommand{\sincp}{\operatorname{sinc}_{\pi}}
\newcommand{\vtwo}{\nu_2}
...
\[
 \Psi_c(t)=\prod_{j\ge0}\sincp(t/2^j)^{a_j},
...
\item the canonical infinite sinc product, integer zero multiplicities
      $1+\vtwo(n)$, arithmetic rays, spectral determinants, and Fourier-decay
      analyses;
...
\begin{equation}
 \sincp z:=\frac{\sin(\pi z)}{\pi z},\qquad \sincp0=1.
...
\end{equation}
The series converges absolutely and $|X|\le1$.  We use $\Up$ for its density.
...
\begin{equation}
 \Phi(t):=\E\e^{2\pi\ii tX}
 =\prod_{j=0}^{\infty}\sincp(t/2^j).
...
Let $F:[0,1]\to[0,1]$ be the standard Fabius function and let
$I=F^{-1}$.  With
\begin{equation}
 G(x):=\Prob(X\le x),\qquad -1\le x\le1,
\end{equation}
...
Fabius dilation equation gives $G'(x)=\Up(x)$, while both sides vanish at
$x=-1$.
...
\begin{proof}
The quantile of $G(x)=F((x+1)/2)$ is $G^{-1}(v)=2I(v)-1$.
\end{proof}
...
$\prod_{h,\ell}\Up(x_{h,\ell})$ along the coefficient vector whose entries are
...
\begin{equation}
 \Psi_c(t):=\E\e^{2\pi\ii tY_c}
 =\prod_{h\ge0}\Phi(t/2^h)^{c_h}
 =\prod_{j\ge0}\sincp(t/2^j)^{a_j}.
...
$\sincp(t/2^j)$: it occurs once for every outer shell $h\le j$, with total
...
\[
 \Psi_c(t)=\prod_{j\ge0}\sincp(t/2^j)^{j+1}.
\]
...
For $n\in\Z\setminus\{0\}$ write $v=\vtwo(|n|)$.  A factor
$\sincp(t/2^j)$ vanishes at $t=n$ exactly when $j\le v$, and then the zero is
simple.
...
\begin{equation}
 \mathcal Z_c(s)
 :=\sum_{n=1}^{\infty}\frac{m_{\vtwo(n)}}{n^s}
...
$2^{-s}=1$, namely $s=2\pi\ii k/\log2$, subject only to explicit numerator
...
\[
 ...
 =\int_0^1\left(\sum_{i=0}^Nu_ix^i\right)^2\dd\mu_c(x)\ge0,
\]
...
Define
\begin{equation}
 \Lambda_n(c):=\int_{-1}^{1}P_n(x)g_c(x)\dd x=\E P_n(Y_c).
...
\begin{proof}
The factor $\sincp(t/2^j)$ crosses one simple zero at every positive multiple
...
$\int_0^1u^d(1-u)\dd u=1/((d+1)(d+2))$.  Euler--Maclaurin or elementary
...
$\log\sincp y=-\pi^2y^2/6+\bigO(y^4)$ near zero.  Since
$a_j=\bigO(j^d)$,
\[
 \sum_{j>H}a_j\abs{\log\sincp(2^{H-j}/3)}
...
\begin{equation}
 \Phi_r(t):=
 \prod_{j=0}^{\infty}\sincp(t/2^j)^{\binom{j}{r-1}},
...
multiplicity
\begin{equation}
 \ord_{t=n}\Phi_r(t)=\binom{\vtwo(|n|)+1}{r},
...
$\theta\in\R$ and every polynomial $p$ of degree strictly less than $m_H$,
\begin{equation}
 \boxed{
 \int_{\R}p(x)g_c(x)\dd x
 =\frac1Q\sum_{k\in\Z}
...
$\int x^jg_c(x)\dd x$.  Linearity gives \eqref{eq:profile-cubature}.  Compact
...
\begin{equation}
 ...
 2^{-h}\bigl(2I(V_{h,\ell})-1\bigr).}
 \label{eq:inverse-Fabius-profile}
\end{equation}
Writing $D_c:=S_c+Y_c\in[0,2S_c]$, one has
\begin{equation}
 D_c\ \stackrel d=\
 2\sum_{h\ge0}\sum_{\ell=1}^{c_h}2^{-h}I(V_{h,\ell}),
 \label{eq:Dc-inverse}
\end{equation}
...
endpoint information is naturally written for $I(y)$ and for the lower branch
$W_{-1}$ of Lambert's function.

\subsection{Laplace factorization}

Let $D:=1+X=2I(V)$ and define
\begin{equation}
 L_1(s):=\E\e^{-sD}
 =\int_0^1\e^{-2sI(v)}\dd v.
 \label{eq:L1-inverse}
\end{equation}
...
\begin{center}
\input{data/fourier_envelope_table.tex}
\end{center}
...
multiplicity depends only on $\vtwo(n)$.  After a common rescaling, every
...
$\rho_{h_0}(x)=2^{h_0}\Up(2^{h_0}x)$, then
...
\begin{align*}
 ...
 R_3-2R_2+R_1&\ge0,\\
 R_1R_3-R_2^2&\ge0,\\
 \det\begin{pmatrix}R_1&R_2&R_3\\R_2&R_3&R_4\\R_3&R_4&R_5\end{pmatrix}&\ge0.
\end{align*}
