...
\usepackage[a4paper,margin=0.92in,headheight=15pt]{geometry}
...
\hypersetup{
  ...
  pdfsubject={Spectral reconstruction, generalized Thue--Morse signs, sampled cumulants, Pascal factorizations, and Lambert frontiers},
  pdfkeywords={Fabius function, Rvachev up-function, Radon profile, Thue--Morse, cumulant, Pascal factorization, Lambert W}
}
...
figures, the source-audit record, and build instructions accompany the
manuscript.  Package-wide checksum ledgers are retired; their historical
provenance remains recoverable from Git history.  The word ``new'' throughout
means apparently new relative to
...
\begin{longtable}{@{}p{0.08\textwidth}p{0.34\textwidth}>{\raggedright\arraybackslash}p{0.50\textwidth}@{}}
...
Let $\ExponentialCompleteBellPolynomial{n}(x_1,\ldots,x_n)$ denote the complete exponential Bell polynomial.
Set
\begin{equation}
 K_{2r}(c):=
...
\begin{equation}
 \Expectation Y_c^n
 =\ExponentialCompleteBellPolynomial{n}\bigl(K_1(c),\ldots,K_n(c)\bigr).
...
\begin{equation}
 ...
 \ExponentialCompleteBellPolynomial{n-2k}\bigl(K_1(c),\ldots,K_{n-2k}(c)\bigr).
...
\end{equation}
Since $g_c$ is even, $\FourierTwoPi{g_c}=\Psi_c$.
...
\begin{equation}
 ...
 \FourierTwoPi{x^jg_c}(Qn).
 \label{eq:Poisson-profile}
\end{equation}
Up to a nonzero constant,
$\FourierTwoPi{x^jg_c}(\xi)$ is the $j$th derivative of $\Psi_c(\xi)$.  For
$n\ne0$,
\[
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/profile_inversion.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/digital_signs.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/cumulant_ratio_moments.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/fourier_envelope.png}
...
\path{figures/*.pdf} & retained vector source figures \\
\path{figures/*.png} & raster figures used in the report \\
...
GitHub repository,
\href{https://github.com/VladimirReshetnikov/ProveIt/tree/main/Analysis/FabiusFunction/docs}%
{\nolinkurl{github.com/VladimirReshetnikov/ProveIt}},
