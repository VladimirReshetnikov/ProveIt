...
% Document-specific settings, notation, and environments not present in the
...
\author{Research report prepared from the \repo\ Fabius-function corpus}
\date{30 August 2026}
...
\[
 \Phi(z)=\prod_{n\geq0}\sinc\!\left(\frac{\pi z}{2^n}\right).
\]
...
change of variables, $\nu_M$ is generated by i.i.d. digits uniform on
...
\item the multiplicative convolution cocycle $\nu_{MN}=\nu_M*\D_{1/M}\nu_N$;
\item a complete odd/even and $2$-adic regularity classification of $\nu_M$;
\item singular continuous, full-interval approximants converging to $\Up$ at exact
...
\[
 ...
 \qquad
 \widehat\mu(z)=\int_{\R}\e^{2\pi\ii zx}\,\mu(\dd x).
\]
Let $X$ be the random variable with density $\Up(x)$, supported on $[-1,1]$.
The repository's primary exposition records the entire characteristic function~\cite{ProveItPrimary}
\begin{equation}
 \Phi(z)=\widehat{\Law(X)}(z)
...
Let $F$ be the standard Fabius function on $[0,1]$.  In this normalization,
\begin{equation}
 \mathbb P(X\le t)=F\!\left(\frac{t+1}{2}\right),
 \qquad -1\le t\le1,
 \label{eq:F-cdf}
\end{equation}
and $F'(x)=2\Up(2x-1)$.  This distinction is important: $F$ is the affine CDF of
the $\Up$ density, while the identity $F(x)=\Up(x-1)$ on $[0,1]$ is a special
...
For $m\in\Z\setminus\{0\}$,
\begin{equation}
 \ord_{z=m}\Phi(z)=1+\vtwo(|m|),
 \label{eq:Phi-zero-order}
\end{equation}
where $\vtwo(m)$ is the exponent of $2$ in $m$.  There are no other zeros.  The
canonical product can therefore be written
\begin{equation}
 \Phi(z)=\prod_{n=1}^{\infty}
 \left(1-\frac{z^2}{n^2}\right)^{1+\vtwo(n)}.
 \label{eq:Phi-canonical}
\end{equation}
The convergence is locally uniform because
$\sum_{n\ge1}(1+\vtwo(n))/n^2<\infty$.
...
\end{equation}
For $y\in\C$ the elementary identity
\begin{equation}
 \frac{\sinc(My)}{\sinc(y)}
 =\frac{\sin(My)}{M\sin y}
 =\frac1M\sum_{j=0}^{M-1}\e^{\ii(M-1-2j)y}
 \label{eq:Dirichlet-kernel}
\end{equation}
holds by analytic continuation at the common zeros.  Applying it with
$y=\pi z/(M2^n)$ gives
\begin{equation}
 \frac{\sinc(\pi z/2^n)}{\sinc(\pi z/(M2^n))}
 =\frac1M\sum_{j=0}^{M-1}
 \exp\!\left(\frac{\pi\ii z(M-1-2j)}{M2^n}\right).
...
compact set $K\subset\C$, so their product converges locally uniformly.
...
\begin{equation}
 \supp(\nu_M)=\left[-\frac{M-1}{M},\frac{M-1}{M}\right],
 \qquad \nu_M:=\Law(Y_M),
 \label{eq:nu-support}
\end{equation}
...
Let $c\in\R$.  There exist independent random variables $X'\overset d=X$ and $Y$ such
that
\begin{equation}
 X\overset d=cX'+Y
 \label{eq:c-decomp}
...
\end{equation}
For $M\ge2$ the nondegenerate remainder is $\nu_M$ up to reflection, while $M=1$
...
\end{equation}
holds first for real $z$ and then on all of $\C$ by the identity theorem.  If $c\ne0$,
...
$\{0\}\cup\{\pm1/M:M\in\N\}$.  It is not self-decomposable in the classical sense,
...
\begin{equation}
 \Up(x)=M\int_{\R}\Up\bigl(M(x-y)\bigr)\,\nu_M(\dd y).
 \label{eq:Up-M-refinement}
\end{equation}
If $F$ is extended by $0$ on $(-\infty,0]$ and by $1$ on $[1,\infty)$, then
\begin{equation}
 F(u)=\int_{\R}
 F\!\left(\frac{M(2u-1-y)+1}{2}\right)\nu_M(\dd y).
...
the CDF identity to $G(t)=F((t+1)/2)$:
$G(t)=\int G(M(t-y))\nu_M(\dd y)$, then set $t=2u-1$.
\end{proof}
...
\begin{equation}
 ...
 \label{eq:Y-Z-affine}
\end{equation}
Let $\eta_M=\Law(Z_M)$.  Removing the first digit gives the exact self-similarity
\begin{equation}
...
\begin{equation}
 Q_{2^r}(z)=\prod_{j=0}^{r-1}\sinc\!\left(\frac{\pi z}{2^j}\right).
...
\begin{proof}
Write the Lebesgue decomposition $\eta_M=f\,\dd x+\sigma$.  The affine operator
\[
...
$f\dd x=\mathcal T(f\dd x)$ and $\sigma=\mathcal T\sigma$.  If both parts had positive
...
\begin{equation}
 R_M(z)=\frac{\sinc(\pi z)}{\sinc(\pi z/M)}.
 \label{eq:RM}
\end{equation}
Then
\begin{equation}
...
If $M\ge3$ is odd, then $\nu_M$ and $\eta_M$ are singular continuous measures with
...
finite measure has a Fourier transform tending to zero at infinity, so $\nu_M$ is not
...
\begin{enumerate}
\item If $r=0$ (equivalently, $M$ is odd), then $\nu_M$ is singular continuous.
\item If $r\ge1$ and $q=1$, then $\nu_M$ is the finite B-spline law $\nu_{2^r}$.
For $r=1$ its density has jump discontinuities; for $r\ge2$ its density is exactly
$C^{r-2}$ and not $C^{r-1}$.
\item If $r\ge1$ and $q>1$, then $\nu_M$ has a compactly supported density $g_M$ that is
exactly
\begin{equation}
 g_M\in C^{r-1}(\R)\setminus C^r(\R).
...
$D^r b_r$ is atomic,
\[
 D^r g_M=(D^r b_r)*\D_{2^{-r}}\nu_q
\]
is a finite linear combination of translates of the atomless measure
$\D_{2^{-r}}\nu_q$.  It has no atoms.  A distribution whose derivative is an atomless
...
nonzero constant multiple of one translate of the singular measure $\nu_q$, with no
...
\label{thm:torus}
Let $Y_M\bmod1$ denote the pushforward of $\nu_M$ to $\T=\R/\Z$.
\begin{enumerate}
\item If $M$ is even, $Y_M\bmod1$ is Haar-uniform on $\T$.
\item If $M$ is odd, its Fourier spectrum is supported on $M\Z$, it is invariant under
translation by $1/M$, and it is not Haar-uniform.
\end{enumerate}
If $M$ is even and $g_M$ is a density, then
\begin{equation}
 \sum_{k\in\Z}g_M(x+k)=1
...
\eqref{eq:nondecay-subsequence}.  Fourier support in $M\Z$ is equivalent to invariance
...
\begin{equation}
 W_p(\mu,\nu_M)
 \le\frac1M\bigl(\mathbb E|X|^p\bigr)^{1/p}.
 \label{eq:Wp-bound}
\end{equation}
Consequently $\nu_M\Rightarrow\mu$ as $M\to\infty$.  Along odd $M$ this is convergence
of singular continuous full-support measures to the $C^\infty$ density $\Up$.
\end{theorem}
...
\label{cor:TV}
For every odd $M\ge3$, $\mu$ and $\nu_M$ are mutually singular.  In the convention
$d_{\TV}(\rho,\sigma)=\sup_A|\rho(A)-\sigma(A)|$,
\begin{equation}
 d_{\TV}(\mu,\nu_M)=1,
 \qquad
 W_\infty(\mu,\nu_M)=\frac1M.
...
\begin{equation}
 ...
 \label{eq:quantile-W}
\end{equation}
By \eqref{eq:F-cdf}, $G_X^{-1}(p)=2F^{-1}(p)-1$.  If $K_M$ is the quantile function of
...
\begin{equation}
 \boxed{
 \left|F^{-1}(p)-\left(\frac{K_M(p)}{M}+\frac1{2M}\right)\right|
 \le\frac1{2M}.}
...
\begin{equation}
 W_\infty(\Law(S_{M,k}),\mu)=M^{-k}.
 \label{eq:iter-W}
\end{equation}
If $L_{M,k}$ is the quantile of $S_{M,k}$,
\begin{equation}
 \left|F^{-1}(p)-\frac{1+L_{M,k}(p)}2\right|
 \le\frac1{2M^k}.
...
\begin{equation}
 ...
 \frac{\kappa_{2r}(X)(2\pi\ii w)^{2r}}{(2r)!}.
...
\begin{equation}
 P_{M,k}(z)=\Phi(z)
 \exp\!\left(-\sum_{r\ge1}
 \frac{\kappa_{2r}(X)(2\pi\ii z)^{2r}}{(2r)!}q^{rk}\right).
...
\begin{equation}
 \delta_M(n)
 =1+\vtwo(n)-\1_{M\mid n}\bigl(1+\vtwo(n/M)\bigr).
...
Thus:
\begin{itemize}
\item if $M$ is odd, the positive zero set is $\N\setminus M\N$;
\item if $M$ is even, every positive integer is a zero;
\item on multiples of $M$, the residual order is the constant $\vtwo(M)$.
\end{itemize}
\end{proposition}
...
$\vtwo(n)=\vtwo(M)+\vtwo(n/M)$, giving the second form.
\end{proof}
...
\end{equation}
For odd $M$, the zero lattice has periodic holes at $M\N$; for even $M$ it remains the
...
\begin{equation}
 ...
 \label{eq:zero-zeta}
\end{equation}
This identity gives a meromorphic continuation to $\C$.  Its residue at $s=1$ is
\begin{equation}
...
\begin{equation}
 s_k=\frac{2\pi\ii k}{\log2},\qquad k\in\Z\setminus\{0\},
...
$Q_M(z)=\prod_{j=0}^{r-1}\sinc(\pi z/2^j)$.
\end{enumerate}
If $M=2^r$, then
\begin{equation}
...
$k\log_2M\in\Z$.  Cancellation at one nonzero $k$ makes $\log_2M$ rational, and
...
holds, characteristic-function uniqueness identifies $\nu_M$ with $\nu_{2^r}$; comparing
...
for its moments $m_n^{(M)}=\mathbb E Y_M^n$:
\begin{equation}
 (2^n-1)m_n^{(M)}
 =\sum_{k=0}^{n-1}\binom nk
 \mathbb E D_M^{n-k}\,m_k^{(M)},
 \qquad m_0^{(M)}=1.
...
Let $\Bell_n$ denote the complete exponential Bell polynomial.  The moment-cumulant
formula specializes to
\begin{equation}
 \mathbb E Y_M^{2n}
 =\Bell_{2n}\bigl(0,\kappa_2(Y_M),0,\kappa_4(Y_M),\ldots,
...
\begin{equation}
 \mathbb P(2^NZ_{M,N}=n)=M^{-N}c_{M,N}(n),
 \qquad
...
\end{equation}
Coefficientwise in the formal power-series ring $\Z[[x]]$,
\begin{equation}
 A_M(x)T(x)=T(x^M),
...
\begin{equation}
 ...
 \qquad
 T(x)=\prod_{j\ge0}(1-x^{2^j})=\sum_{n\ge0}\eps_nx^n.
 \label{eq:AM-T}
\end{equation}
Equivalently,
\begin{equation}
 \sum_{k=0}^{n}c_M(k)\eps_{n-k}
 =\begin{cases}
 \eps_{n/M},&M\mid n,\\
 0,&M\nmid n.
 \end{cases}
...
coefficientwise to $\Z[[x]]$.  Comparing coefficients gives \eqref{eq:TM-convolution-inverse}.
\end{proof}
...
\begin{equation}
 ...
 \left(\sum_{n\ge0}\eps_nx^n\right)
 =\sum_{n\ge0}\eps_nx^{3n}.}
 \label{eq:Stern-TM}
\end{equation}
Equivalently,
\begin{equation}
 \sum_{k=0}^{n}s(k+1)\eps_{n-k}
 =\begin{cases}
 \eps_{n/3},&3\mid n,\\0,&3\nmid n.
 \end{cases}
...
\begin{equation}
 \mathbb P(2^NZ_{3,N}=n)=3^{-N}c_{3,N}(n),
 \qquad
...
Let
\begin{equation}
 H_M(x)=\eta_M(( -\infty,x])=\mathbb P(Z_M\le x).
 \label{eq:HM}
\end{equation}
...
$-\log_2x=n+\theta$ with $n\in\N$ and $0\le\theta<1$.  Iterating
...
\begin{proof}
Apply \eqref{eq:smoothing-operator} to $p=P_n$ and integrate against $\nu_M$.
\end{proof}
...
\end{equation}
On exponentials, $\mathcal C_{\nu_M}$ has multiplier $Q_M$; on polynomials, it is the
...
For $M=6=2\cdot3$, the uniform factor $\nu_2$ smooths the singular ternary core.  The
...
\begin{equation}
 \mathfrak M_M(s)=\int_0^{1/2}H_M(x)x^{s-1}\dd x,
 \qquad
 B_M(s)=\int_{1/2}^{1}H_M(x)x^{s-1}\dd x.
...
\[
 \mathfrak M_M(s)
 =\frac{2^{-s}}M\int_0^1H_M(u)u^{s-1}\dd u
...
$s=-\alpha_M+2\pi\ii k/\log2$ is the Fourier coefficient
$\int_0^1\Omega_M(\theta)\e^{-2\pi\ii k\theta}\,\dd\theta$.  Hence a constant
...
matrices $\mathscr C_q$ such that the global H\"older exponent of the CDF of $\nu_q$, and
...
The local dimension spectrum of $\nu_3$ is the Legendre transform of a pressure function
...
\end{equation}
has the unit circle as a natural boundary and is transcendental over $\mathbb C(x)$.  For
...
\begin{equation}
 \Phi_b(z)=\prod_{n=0}^{\infty}\sinc\!\left(\frac{\pi z}{b^n}\right).
 \label{eq:Phi-b}
\end{equation}
...
$\mathbb EP_n(X)$ than direct integration against $\Up$.  Powers of two optimize symbolic
...
\begin{enumerate}
\item the Lambert-$W$ approximation provides a narrow initial interval for $F^{-1}(p)$;
...
The divisor family changes the usual viewpoint on $\Up$.  Rather than one special dyadic
...
Path & Purpose\\
\midrule
\path{frontier_report.tex} & complete LaTeX source\\
\path{frontier_report.pdf} & compiled report\\
