...
\[
 ...
 \qquad
 \widehat\mu(z)=\int_{\RealNumbers}\EulerE^{2\pi\ImaginaryUnit zx}\,\mu(\Differential x).
\]
...
\begin{equation}
 ...
 \label{eq:F-cdf}
\end{equation}
and $F'(x)=2\RvachevUp(2x-1)$.  This distinction is important: $F$ is the affine CDF of
the $\RvachevUp$ density, while the identity $F(x)=\RvachevUp(x-1)$ on $[0,1]$ is a special
...
\begin{equation}
 \ord_{z=m}\Phi(z)=1+\TwoAdicValuation(|m|),
...
\subsection{New refinement equations for $\RvachevUp$ and $F$}
...
$G(t)=\int G(M(t-y))\nu_M(\Differential y)$, then set $t=2u-1$.
\end{proof}
...
\label{thm:Winfty}
Let $\mu=\LawOperator(X)$ and $\nu_M=\LawOperator(Y_M)$.  Then
\begin{equation}
 W_\infty(\mu,\nu_M)=\frac1M.
...
\begin{equation}
 \delta_M(n)
 =1+\TwoAdicValuation(n)-\1_{M\mid n}\bigl(1+\TwoAdicValuation(n/M)\bigr).
...
\begin{equation}
 \frac{\EulerE^{xt}}{\mathbb E\EulerE^{tY_M}}
...
$\mathbb EP_n(X)$ than direct integration against $\RvachevUp$.  Powers of two optimize symbolic
