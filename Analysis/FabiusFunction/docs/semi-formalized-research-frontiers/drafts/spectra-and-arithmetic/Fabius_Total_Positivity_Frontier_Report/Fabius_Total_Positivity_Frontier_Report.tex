...
\hypersetup{
 ...
 pdfkeywords={Fabius function, Rvachev up-function, total positivity, Cartwright class, Laguerre--Polya, Thue--Morse, Jensen polynomial}
}
\pagestyle{fancy}
\fancyhf{}
...
\[
 \FourierTwoPi{\RvachevUp}(z)=\prod_{j=0}^{\infty}\SincRad\!\left(\frac{\pi z}{2^j}\right).
\]
...
\[
 H(x):=\FourierTwoPi{\RvachevUp}\!\left(\frac{\ImaginaryUnit\sqrt{x}}{\pi}\right)
...
On the original Fourier side, the zero at the nonzero integer $N$ has multiplicity $\TwoAdicValuation(N)+1$.  Consequently the multiplicity-weighted positive zero count is exactly $2N-\BinaryDigitSum(N)$, and, more strikingly,
\[
 \Sign\FourierTwoPi{\RvachevUp}(x)=\ThueMorseSign{\Floor{x}}
 \qquad(x>0,\ x\notin\IntegerNumbers).
\]
...
\[
 \FourierTwoPi{f}(z)=\int_{\RealNumbers}f(t)\EulerE^{-2\pi\ImaginaryUnit zt}\,\Differential t.
\]
...
\begin{equation}\label{eq:fourier-moment-expansion}
 \FourierTwoPi{\RvachevUp}(z)=\sum_{n=0}^{\infty}(-1)^n\frac{c_n}{(2n)!}(2\pi z)^{2n}.
\end{equation}
...
Let $\TwoAdicValuation(N)$ be the exponent of $2$ in the prime factorization of the positive integer $N$, and let $\BinaryDigitSum(N)$ be its binary digit sum.  Write
\[
...
\begin{equation}\label{eq:zero-order}
 \OrderOperator_{z=N}\FourierTwoPi{\RvachevUp}(z)=\TwoAdicValuation(\AbsoluteValue N)+1.
\end{equation}
Equivalently, $H$ has a zero at $-\pi^2N^2$ of multiplicity $\TwoAdicValuation(N)+1$ and has no other zeros.
\end{theorem}
\begin{proof}
The $j$th factor of \eqref{eq:up-product} vanishes at $z=N$ precisely when $N/2^j$ is a nonzero integer, i.e. when $0\le j\le\TwoAdicValuation(N)$.  Each such sine zero is simple, and all other factors are nonzero.  This proves \eqref{eq:zero-order}.  The statement for $H$ follows either by substitution or directly from \eqref{eq:H-double-product}.
\end{proof}

\begin{theorem}[Exact zero count and digit-sum discrepancy]\label{thm:zero-count}
Let $M=\Floor R$ for $R\ge0$, and count zeros with multiplicity.  The number of positive zeros of $\FourierTwoPi{\RvachevUp}$ in $(0,R]$ is
\begin{equation}\label{eq:positive-zero-count}
 Z_+(R)=2M-\BinaryDigitSum(M).
\end{equation}
The number of nonzero zeros in $[-R,R]$ is
\begin{equation}\label{eq:total-zero-count}
 Z(R)=4M-2\BinaryDigitSum(M).
\end{equation}
\end{theorem}
\begin{proof}
By \cref{thm:zero-multiplicity},
\[
 Z_+(R)=\sum_{n=1}^{M}(\TwoAdicValuation(n)+1)=M+\TwoAdicValuation(M!).
\]
Legendre's identity $\TwoAdicValuation(M!)=M-\BinaryDigitSum(M)$ proves \eqref{eq:positive-zero-count}.  Evenness of the product gives \eqref{eq:total-zero-count}.
\end{proof}
...
\begin{equation}\label{eq:sign-interpolation}
 \Sign\FourierTwoPi{\RvachevUp}(x)=\ThueMorseSign{\Floor{x}}=(-1)^{\BinaryDigitSum(\Floor{x})}.
\end{equation}
Thus $\FourierTwoPi{\RvachevUp}$ has constant sign on every open unit interval $(N,N+1)$, and those signs spell the Thue--Morse word exactly.
\end{theorem}
\begin{proof}
...
\[
 (-1)^{Z_+(N)}=(-1)^{2N-\BinaryDigitSum(N)}=\ThueMorseSign{N}.
\]
...
\caption{The exact discrepancy $Z_+(M)-2M=-\BinaryDigitSum(M)$.  The nested sawtooth pattern is not numerical noise: every plotted value is an integer identity.}
...
The entire function $\FourierTwoPi{\RvachevUp}$ is in the Cartwright class, has exponential type $2\pi$, and has indicator
...
\[
 \log\AbsoluteValue{\FourierTwoPi{\RvachevUp}(re^{\ImaginaryUnit\theta})}
...
$\AbsoluteValue{\FourierTwoPi{\RvachevUp}(x)}\le1$.  Therefore
\[
 \int_{\RealNumbers}\frac{\log^+\AbsoluteValue{\FourierTwoPi{\RvachevUp}(x)}}{1+x^2}\,\Differential x=0,
\]
which is the Cartwright integrability condition.  Finally, the zero multiplicities $\TwoAdicValuation(N)+1$ are unbounded.  Standard sine-type functions have a uniformly separated zero divisor with uniformly bounded multiplicities (and in the usual definition, simple zeros), so $\FourierTwoPi{\RvachevUp}$ is not sine-type.
\end{proof}
...
\[
 Z(R)-4\Floor R=-2\BinaryDigitSum(\Floor R).
\]
...
\item It writes the exact zero-count data $Z_+(M)=2M-\BinaryDigitSum(M)$ through $M=4096$ and produces \cref{fig:zero-discrepancy}.
\end{enumerate}
...
\[
 ...
 H(x)=\FourierTwoPi{\RvachevUp}\!\left(\frac{\ImaginaryUnit\sqrt{x}}{\pi}\right)
 =\prod_{n=1}^{\infty}
  \left(1+\frac{x}{\pi^2n^2}\right)^{\TwoAdicValuation(n)+1}
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.43\textwidth}>{\raggedright\arraybackslash}p{0.14\textwidth}>{\raggedright\arraybackslash}p{0.348\textwidth}}
\toprule
Statement & Status & Basis \\
\midrule
\endhead
Zero multiplicity $\TwoAdicValuation(N)+1$ & Proved & Direct factor count.\\
Exact count $2N-\BinaryDigitSum(N)$ & Proved & Legendre digit-sum identity.\\
...
commit \nolinkurl{fc4b8868fcf57cc534d22f438d940ec65ebbe767}, 30 August 2026.
\end{thebibliography}
\endgroup
