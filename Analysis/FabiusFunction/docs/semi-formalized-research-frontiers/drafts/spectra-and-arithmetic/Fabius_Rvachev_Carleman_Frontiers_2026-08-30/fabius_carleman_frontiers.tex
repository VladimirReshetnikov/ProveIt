...
\begin{abstract}
The exact derivative scale of the bounded Fabius function $F$ and the Rvachev
up-function $\Up$ is
\[
  W_n=2^{n(n+1)/2}.
\]
...
on \emph{every} nondegenerate interval, for $f=F$ or $f=\Up$.  Their pushforward
measures converge in total variation, at rate $O(2^{-n}/|J|)$, to the symmetric law
of $S\Up(Y)$, where $S$ is a Rademacher sign and $Y$ is uniform on $[-1,1]$.
...
weight $N$, membership of $F$ or $\Up$ in the Roumieu class $\mathcal E^{\{N\}}$
...
Let $F:[0,1]\to[0,1]$ be the bounded Fabius function and let
$\Up:\R\to\R$ be the compactly supported Rvachev up-function, normalized by
\[
  \supp\Up=[-1,1],\qquad \Up(0)=1,\qquad
  \int_{-1}^{1}\Up(y)\dd y=1.
\]
...
\begin{equation}
  \Up^{(n)}\bigl(c_{n,k}(y)\bigr)
  =W_n\eps_k\Up(y).
  \label{eq:cell-identity}
\end{equation}
In particular,
\begin{equation}
  \norm{\Up^{(n)}}_{L^\infty([-1,1])}=W_n,
  \qquad
  \Up^{(n)}\bigl(c_{n,k}(0)\bigr)=W_n\eps_k.
  \label{eq:sharp-global}
\end{equation}
The shift \eqref{eq:F-up-shift} transfers these identities to $F$ on $[0,1]$.
Equivalent matched-grid identities and the global signed extension of $F$ are
...
Let $P(N)=\sum_{k=0}^{N-1}\eps_k$.  Then
\begin{equation}
  P(2m)=0,
  \qquad
  P(2m+1)=\eps_m.
...
\begin{proof}
The identities $\eps_{2m}=\eps_m$ and $\eps_{2m+1}=-\eps_m$ give
...
each pair $(\eps_{2m},\eps_{2m+1})$ consists of opposite signs, so a run of length
three is impossible.
\end{proof}
...
\end{equation}
For $\Phi=\widehat\Up$, the repository's product formula is
\begin{equation}
...
\begin{equation}
  \norm{\Up^{(n)}}_{L^1(\R)}=W_n.
  \label{eq:l1-derivative}
\end{equation}
Indeed, on every cell the absolute value is $W_n\Up(y)$ and
$\dd x=2^{-n}\dd y$; summing the $2^n$ cells uses $\int\Up=1$.
...
$f\in\{F,\Up\}$, let $\mu_{n,J}^{f}$ be the pushforward of normalized Lebesgue
measure on $J$ by $W_n^{-1}f^{(n)}$.

\begin{theorem}[Quantitative local Thue--Morse universality]
\label{thm:local-universality}
Let $f=\Up$ with $J\subseteq[-1,1]$, or $f=F$ with $J\subseteq[0,1]$.
Assume $|J|>0$.  Then for every bounded Borel $H$,
\begin{equation}
 \left|
   \frac1{|J|}\int_J H\!\left(\frac{f^{(n)}(x)}{W_n}\right)\dd x
   -\int H\dd\mu_*
 \right|
 \le
  6\norm{H}_\infty\frac{h_n}{|J|}.
 \label{eq:local-law-bound}
\end{equation}
Consequently,
\begin{equation}
  \norm{\mu_{n,J}^{f}-\mu_*}_{\TV}
  \le 6\frac{h_n}{|J|}
  =\frac{12}{2^n|J|},
  \label{eq:tv-bound}
\end{equation}
where $\|\nu\|_{\TV}=\sup_{\|H\|_\infty\le1}|\int H\dd\nu|$.
\end{theorem}

\begin{proof}
It suffices to prove the assertion for $\Up$, because $F$ is its translate on
...
Put
\[
 A_+=\int_{-1}^{1}H(\Up(y))\dd y,
 \qquad
 A_-=\int_{-1}^{1}H(-\Up(y))\dd y.
\]
By \eqref{eq:cell-identity}, the contribution of the complete cells is
\[
 I_K=\frac{h_n}{2}\sum_{k\in K}A_{\eps_k}.
\]
Since $q h_n\int H\dd\mu_*=qh_n(A_++A_-)/4$, their difference is
\[
 I_K-qh_n\int H\dd\mu_*
 =\frac{h_n}{4}\left(\sum_{k\in K}\eps_k\right)(A_+-A_-).
\]
...
$r\|H\|_\infty$, while
$r|\int H\dd\mu_*|\le r\|H\|_\infty$.  Thus replacing the fragments by their
...
of a positive or negative cell, \eqref{eq:cell-identity} and $\Up(0)=1$ give the
...
\begin{equation}
 \lim_{n\to\infty}
 \frac{1}{|J|W_n^p}\int_J\abs{f^{(n)}(x)}^p\dd x
 =\frac12\int_{-1}^{1}\Up(y)^p\dd y.
 \label{eq:lp-limit}
\end{equation}
...
\begin{equation}
 \lim_{n\to\infty}
 \frac{1}{|J|W_n^m}\int_J f^{(n)}(x)^m\dd x
 =\begin{cases}
  0,&m\text{ odd},\\[2mm]
  \displaystyle\frac12\int_{-1}^{1}\Up(y)^m\dd y,&m\text{ even}.
 \end{cases}
...
$[-1,1]$.  For $p=1$, use $\int_{-1}^{1}\Up=1$.
\end{proof}

\begin{corollary}[Exact local quadratic derivative order]
\label{cor:local-order}
For $f=F$ or $f=\Up$ and every nondegenerate interval $J$ in its domain,
\begin{equation}
 \lim_{n\to\infty}
 \frac{\log_2\norm{f^{(n)}}_{L^\infty(J)}}{n^2}
 =\frac12.
...
Let $f=F$ or $f=\Up$, and let $J$ be any nondegenerate compact interval in its
...
\label{cor:no-gevery}
For every finite $s\ge1$, neither $F$ nor $\Up$ belongs locally on any nondegenerate
...
\begin{equation}
  \tau_J(g;M)=
  \limsup_{n\to\infty}
  \left(\frac{\norm{g^{(n)}}_{L^\infty(J)}}{n!M_n}\right)^{1/n}.
...
\begin{remark}
Nonquasianalyticity is consistent with the compact support of $\Up$.  Moderate
...
Fa\`a di Bruno expansion of $F^{\comp r}$ and proves that branched partitions are
...
\begin{proof}
The image $J=I(K)$ is compactly contained in $(0,1)$, where $F'>0$.
By \cref{cor:critical-class,cor:stability}, local inversion gives
$I\in\Roumieu{M}(K)$.  If $I\in\Beurling{M}(K)$, Beurling inversion stability would
imply $F\in\Beurling{M}(J)$, contradicting \cref{cor:critical-class}.
...
much finer endpoint asymptotics for $I(y)$ as $y\downarrow0$, where Lambert phases
...
\begin{equation}
 \limsup_{n\to\infty}
 \frac{\log_2\norm{G_r^{(n)}}_{L^\infty(J)}}{n^2}
 =\frac12,
 \qquad
 \limsup_{n\to\infty}
 \frac{\log_2\norm{H_r^{(n)}}_{L^\infty(K)}}{n^2}
 =\frac12.
...
\begin{proof}
Because $F\in\Roumieu{M}$ and the class is composition stable,
...
\[
  \abs{G_r^{(n)}(x)}\le C_\rho\rho^nW_n.
\]
...
$0\le\dist(a,\Z)\le1/2$ gives \eqref{eq:periodic-factor}.
\end{proof}
...
\begin{equation}
  \abs{\Phi(\xi)}\le C E_{\kappa_\infty}(|\xi|),
  \qquad
...
identified with an actual derivative of $F\comp F$.
...
in $\log_2|\xi|$.  For $\Up$, the derivative-lattice profile $P$ and the sinc-product
...
Match the interior class $\Roumieu{M}\setminus\Beurling{M}$ of $I=F^{-1}$ to the
...
$\sup_{y\in[\eta,2\eta]}|I^{(n)}(y)|$ in the joint limit $n\to\infty$,
...
\item Formalize $P(2m)=0$, $P(2m+1)=\eps_m$, and block discrepancy at most $2$.
