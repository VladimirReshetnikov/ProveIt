...
on \emph{every} nondegenerate interval, for $f=F$ or $f=\RvachevUp$.  Their pushforward
...
weight $N$, membership of $F$ or $\RvachevUp$ in the Roumieu class $\mathcal E^{\{N\}}$
...
\begin{equation}
  ...
  \qquad
  \RvachevUp^{(n)}\bigl(c_{n,k}(0)\bigr)=W_n\eps_k.
...
\begin{equation}
  \AbsoluteValue{\sum_{k=a}^{b-1}\eps_k}\le2.
...
$f\in\{F,\RvachevUp\}$, let $\mu_{n,J}^{f}$ be the pushforward of normalized Lebesgue
measure on $J$ by $W_n^{-1}f^{(n)}$.

\begin{theorem}[Quantitative local Thue--Morse universality]
\label{thm:local-universality}
Let $f=\RvachevUp$ with $J\subseteq[-1,1]$, or $f=F$ with $J\subseteq[0,1]$.
...
Consequently,
\begin{equation}
  \Norm{\mu_{n,J}^{f}-\mu_*}_{\TV}
  \le 6\frac{h_n}{|J|}
  =\frac{12}{2^n|J|},
  \label{eq:tv-bound}
\end{equation}
where $\|\nu\|_{\TV}=\sup_{\|H\|_\infty\le1}|\int H\Differential\nu|$.
\end{theorem}

\begin{proof}
It suffices to prove the assertion for $\RvachevUp$, because $F$ is its translate on
...
For $f=F$ or $f=\RvachevUp$ and every nondegenerate interval $J$ in its domain,
\begin{equation}
 \lim_{n\to\infty}
...
Let $f=F$ or $f=\RvachevUp$, and let $J$ be any nondegenerate compact interval in its
...
\label{cor:no-gevery}
For every finite $s\ge1$, neither $F$ nor $\RvachevUp$ belongs locally on any nondegenerate
...
\begin{align}
 ...
 \right),
 \label{eq:CC}\\
 P(a)&=2^{\dist(a,\IntegerNumbers)^2/2},
 \qquad
 1\le P(a)\le2^{1/8}.
...
$0\le\dist(a,\IntegerNumbers)\le1/2$ gives \eqref{eq:periodic-factor}.
\end{proof}
...
\begin{equation}
 ...
 =b^{-a_b^2/2+\dist(a_b,\IntegerNumbers)^2/2},
 \qquad
 a_b=\log_b A+\frac12.
 \label{eq:b-lattice}
\end{equation}
...
\[
  ...
  \qquad d_0=\dist(a_0,\IntegerNumbers).
\]
