...
statement that the periodization is \(\RvachevUp\) on \([-1,1]\)).  Only the final fold of
\(j\) with \(2N-j\) onto \(1\le j\le N-1\) in \eqref{p1:eq:A-Up-DCT}, and the passage to
\(\FabiusBounded=1-\RvachevUp\) in \eqref{p1:eq:A-F-DCT}, are not separately formalized.
...
residues is \path{Fabius.foldedCoefficient_eq_sum_cos}, and its samples are the values
\(\RvachevUp(j/N)\) for \(j\le N\) (\path{Fabius.gridSample_natCast_of_le}).  The fold of
\(j\) with \(2N-j\) into \eqref{p1:eq:A-F-DCT} and the substitution of the \(q\)-binomial
values into it are not formalized, nor is the convergence statement, which is analytic.
