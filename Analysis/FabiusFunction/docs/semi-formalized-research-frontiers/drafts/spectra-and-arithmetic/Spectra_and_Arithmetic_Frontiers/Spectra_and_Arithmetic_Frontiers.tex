...
\hypersetup{
 ...
 pdfauthor={Frontier research report based on Vladimir Reshetnikov's ProveIt repository},
...
\newcommand{\FieldNorm}{\operatorname{N}}
\newcommand{\PhiU}{\Phi}
...
\newcommand{\SpectralPowerSum}[1]{\mathfrak{s}_{#1}}
...
\author{Frontier research report based on Vladimir Reshetnikov's \texttt{ProveIt} repository}
...
Let
\[
 \PhiU(z):=\FourierTwoPi{\RvachevUp}(z)
...
orders.  With $o_k=2k+1$, $\ThueMorseSign{k}=(-1)^{\BinaryDigitSum(k)}$, and
$w_k=|a_k|$, one has
\[
 \sum_{k\ge0}a_k=\frac12,
 \qquad
 \sum_{k\ge0}\ThueMorseSign{k} w_k o_k^{2m}=0\quad(m\ge1).
\]
...
$\sum_k\ThueMorseSign{k}w_kP(o_k^2)=P(0)/2$ for every polynomial $P$.
...
\[
 ...
 \FabiusBounded\!\left(\frac jN\right)\cos\!\left(\frac{\pi rj}{N}\right)\right].
\]
Then the exact alias identity
\[
 A_{n,r}=\sum_{q\in\IntegerNumbers}\PhiU\!\left(qN+\frac r2\right)
\]
...
each $\FabiusBounded(j/N)$ therefore gives superexponentially convergent, finite
...
$2^{n+1}$-st cyclotomic field.  Their orbit polynomial lies in $\RationalNumbers[X]$; all power
...
\begin{enumerate}
 \item The relations between the bounded Fabius function $\FabiusBounded$, its signed global
 extension $\FabiusGlobal$, and Rvachev's even compactly supported function $\RvachevUp$.
 ...
 formula for $\FabiusGlobal(m/2^n)$.
 \item The integer zero set of $\PhiU$, multiplicity $1+\TwoAdicValuation(m)$ at a nonzero
 integer $m$, strict logarithmic concavity between zeros, and the sign law
 $\Sign \PhiU(x)=\ThueMorseSign{M}$ on $M<x<M+1$.
...
amplitudes reconstruct $\RvachevUp$, while its dyadic arithmetic chapters contain
...
Spectral quadrature & Shows $\sum_k\ThueMorseSign{k}w_kP((2k+1)^2)=P(0)/2$ for every polynomial $P$. & \newresult \\
...
Beyond-all-orders remainder & Rewrites $\FabiusBounded(x)$ after subtraction of an arbitrary cosine Taylor polynomial, with all algebraic terms cancelling exactly. & \newresult \\
...
Let $\FabiusBounded:[0,1]\to[0,1]$ be the bounded Fabius function.  Let
\begin{equation}\label{p1:eq:global-F}
 \FabiusGlobal(x):=\sum_{n=0}^{\infty}\ThueMorseSign{n}\RvachevUp(x-2n-1),
 \qquad \ThueMorseSign{n}:=(-1)^{\BinaryDigitSum(n)},
\end{equation}
be its signed global extension; the sum is locally finite.  On $[-1,1]$,
\begin{equation}\label{p1:eq:Up-F-relations}
 \RvachevUp(t)=\FabiusGlobal(t+1)=
 \begin{cases}
  \FabiusBounded(t+1),&-1\le t\le0,\\
  \FabiusBounded(1-t)=1-\FabiusBounded(t),&0\le t\le1.
 \end{cases}
\end{equation}
The function $\RvachevUp$ is even, nonnegative, $C^\infty$, supported on $[-1,1]$,
normalized by $\int_{\RealNumbers}\RvachevUp=1$, and satisfies $\RvachevUp(0)=1$.
Every positive derivative of $\RvachevUp$ vanishes at $0$, because on the right of
zero it is $1-\FabiusBounded(t)$ and $\FabiusBounded$ is flat at the origin.
...
\begin{equation}\label{p1:eq:global-derivatives}
 ...
 \qquad
 \RvachevUp^{(m)}(t)=2^{\binom{m+1}{2}}\FabiusGlobal\bigl(2^m(t+1)\bigr).
\end{equation}
At nonnegative integers,
\begin{equation}\label{p1:eq:global-integers}
 \FabiusGlobal(2b)=0,
 \qquad
 \FabiusGlobal(2b+1)=\ThueMorseSign{b}.
\end{equation}
...
\[
 \FourierTwoPi{f}(z)=\int_{\RealNumbers}f(t)\EulerE^{-2\pi\ImaginaryUnit tz}\Differential t.
\]
Put
...
$1+\TwoAdicValuation(|m|)$.  If $M<x<M+1$, then
\begin{equation}\label{p1:eq:Phi-sign-law}
 \Sign \PhiU(x)=\ThueMorseSign{M}.
\end{equation}
...
$\GaussianBinomial{n}{k}{q}$, one has for $m,n\in\NaturalNumbers$ and every rational translation $c$
\begin{equation}\label{p1:eq:baseline-qbinomial}
\begin{aligned}
 \FabiusGlobal\!\left(\frac{m}{2^n}\right)
 ={}&\frac{1}{2^{n^2}(1/2;1/2)_n}
 \sum_{k=0}^{n}
 \frac{\GaussianBinomial{n}{k}{1/2}}
      {2^{k(k-1)}(n+k)!}\\
 &\qquad\times
 \sum_{s=0}^{m2^k-1}\ThueMorseSign{s}
 \left(s-m2^k+c\right)^{n+k}.
\end{aligned}
\end{equation}
The right side is independent of $c$.  When $0\le m\le2^n$, it equals the
bounded value $\FabiusBounded(m/2^n)$.  This is \imported\ from the main exposition; the
...
\begin{equation}\label{p1:eq:Lambert-baseline}
 \FabiusBounded(x)=
 \frac{\EulerE^{\Psi(-W/L)}}{2^{7/12}\sqrt{\pi x}}
...
an exact half-integer spectral identity whose left side is $\FabiusBounded(x)$ and then
...
\begin{tabular}{@{}r r r@{}}
\toprule
$k$ & $\ThueMorseSign{k}$ & $a_k=\PhiU(k+1/2)$ \\
\midrule
...
Let $P$ denote the two-periodization of $\RvachevUp$,
\begin{equation}\label{p1:eq:P-periodization}
 P(t):=\sum_{\ell\in\IntegerNumbers}\RvachevUp(t-2\ell).
\end{equation}
Because $\RvachevUp$ is supported on $[-1,1]$, $P(t)=\RvachevUp(t)$ for $-1\le t\le1$.
...
\end{equation}
In particular, on $[-1,1]$ the series equals $\RvachevUp(t)$.
\end{proposition}

\begin{proof}
The $m$th complex Fourier coefficient for period two is
\[
 c_m=\frac12\int_{-1}^{1}\RvachevUp(t)\EulerE^{-\pi\ImaginaryUnit mt}\Differential t
 =\frac12\PhiU(m/2).
\]
...
extension of $\RvachevUp$.  Integer Fourier samples disappear because the entire sinc
...
\begin{equation}\label{p1:eq:positive-prouhet}
 \boxed{
 \sum_{\substack{k\ge0\\\BinaryDigitSum(k)\ \mathrm{even}}}
 w_k(2k+1)^{2m}
 =
 \sum_{\substack{k\ge0\\\BinaryDigitSum(k)\ \mathrm{odd}}}
 w_k(2k+1)^{2m}.}
\end{equation}
...
\begin{proof}
Set $t=0$ in \eqref{p1:eq:cosine-spectrum}.  Since $P(0)=\RvachevUp(0)=1$,
\[
...
\]
Every positive derivative of $P=\RvachevUp$ vanishes at zero, hence the left side is
...
Finally substitute $a_k=\ThueMorseSign{k}w_k$ and move the negative-sign terms to the
opposite side.
\end{proof}

\begin{corollary}[Exact spectral quadrature at the origin]\label{p1:cor:spectral-quadrature}
For every polynomial $Q\in\ComplexNumbers[X]$,
\begin{equation}\label{p1:eq:polynomial-quadrature}
 \boxed{
 \sum_{k=0}^{\infty}\ThueMorseSign{k}w_k\,Q\bigl((2k+1)^2\bigr)
 =\frac{Q(0)}2.}
\end{equation}
Thus the signed positive weights $\ThueMorseSign{k}w_k$ evaluate every polynomial in the
...
\begin{equation}\label{p1:eq:nu-moments}
 \int_{\RealNumbers}x^m\Differential\nu(x)=0\qquad(m\ge1).
\end{equation}
...
\[
 \sum_{k=K}^{\infty}(2k+1)^{-p}
 \le (2K+1)^{-p}+\int_K^\infty(2x+1)^{-p}\Differential x.
\]
\end{proof}
...
\begin{equation}\label{p1:eq:spectral-derivative}
 \boxed{
 S_m(t)=\frac{\RvachevUp^{(m)}(t)}{\pi^m}
 =\frac{2^{\binom{m+1}{2}}}{\pi^m}
 \FabiusGlobal\bigl(2^m(t+1)\bigr).}
\end{equation}
...
\begin{proof}
By \cref{p1:thm:spectral-derivative}, the relevant argument of $\FabiusGlobal$ is
\[
...
$\ThueMorseSign{b}=-\ThueMorseSign{(a-1)/2}$, proving \eqref{p1:eq:filtration-critical}.  The remaining
...
\paragraph{Lean status.}
The two exact parts of \cref{p1:thm:dyadic-filtration} are formal in
\path{DyadicDerivativeFiltration.lean}, for the derivatives themselves (the volume's
\(S_m=\RvachevUp^{(m)}/\pi^m\) differs by the fixed factor \(\pi^{-m}\)).  Above the depth,
\path{Fabius.iteratedDeriv_rvachevUp_dyadic_eq_zero} gives
\(\RvachevUp^{(m)}(a/2^n)=0\) for every \(m>n\) and every odd \(a\)
(\eqref{p1:eq:filtration-above}); at the critical order,
\path{Fabius.iteratedDeriv_rvachevUp_dyadic_critical} gives
\(\RvachevUp^{(n)}\bigl((2c+1)/2^n\bigr)=-\ThueMorseSign{c}\,2^{n(n+1)/2}\) for
\(c<2^{n-1}\) (\eqref{p1:eq:filtration-critical}).  Both are read off the corpus's closed
form of the derivatives as finite Thue--Morse sums of translates
(\path{Fabius.iteratedDeriv_rvachevUp_eq_thueMorse_sum}): above the depth every translate
argument is an odd integer, where \(\RvachevUp\) vanishes
(\path{Fabius.rvachevUp_eq_zero_of_one_le_abs}); at the depth every argument is an even
integer and only \(j=c+2^{n-1}\) survives, with the sign
\(\ThueMorseSign{c+2^{n-1}}=-\ThueMorseSign{c}\) (\path{Fabius.thueMorseSign_add_pow_two}).
The remaining case \eqref{p1:eq:filtration-below} is formal as well, and in a stronger
form, by a different route than the proof above takes.  Since
\((2^n+a)/2^{n-m}=2^m(1+a/2^n)\), the display is the composite of two facts that need
neither a dyadic argument nor an order below the depth: on \((-\infty,1)\) the up-function is a
single translate of the signed extension
(\path{Fabius.extendedFabius_add_one_eq_rvachevUp}), so the two have the same derivatives
there, and every iterated derivative of the extension is a rescaled global value
(\path{Fabius.iteratedDeriv_extendedFabius}).  Composing them,
\path{Fabius.iteratedDeriv_rvachevUp_eq_extendedFabius} is
\(\RvachevUp^{(m)}(x)=2^{m(m+1)/2}\FabiusGlobal(2^m(1+x))\) for \emph{every} order \(m\) and
every \(x<1\), and \path{Fabius.iteratedDeriv_rvachevUp_dyadic_below} is the volume's own
dyadic form for \(m<n\), \(a<2^n\).  The global \(q\)-binomial formula, which the proof above
invokes at this point, is not needed for the identity itself.
...
\paragraph{Lean status.}
Formal, for the derivatives themselves:
\path{Fabius.dyadic_depth_eq_max_nonzero_iteratedDeriv} in
\path{DyadicDerivativeFiltration.lean} states, for \(t=(2c+1)/2^n\) with \(c<2^{n-1}\), that
\(\RvachevUp^{(n)}(t)\ne0\) while \(\RvachevUp^{(m)}(t)=0\) for every \(m>n\), which is
\eqref{p1:eq:denominator-detection} with the maximum spelled out as its two defining
clauses; the passage to \(S_m=\RvachevUp^{(m)}/\pi^m\) is the fixed nonzero factor
\(\pi^{-m}\).
...
\begin{equation}\label{p1:eq:F-basic-spectral}
 \FabiusBounded(x)=\sum_{k=0}^{\infty}a_k
 \bigl(1-\cos(\pi o_kx)\bigr),
\end{equation}
and, more strongly,
\begin{equation}\label{p1:eq:F-rho-spectral}
 \boxed{
 \FabiusBounded(x)=\sum_{k=0}^{\infty}\ThueMorseSign{k}w_k\,
 \rho_M(\pi o_kx).}
\end{equation}
...
$\FabiusBounded(x)=1-\RvachevUp(x)$.  Subtract \eqref{p1:eq:cosine-spectrum} from $1$ and use
...
than checking $\FabiusBounded^{(m)}(0)=0$ one derivative at a time: the same exact identity
...
\boxed{
\begin{aligned}
 \sum_{k=0}^{\infty}\ThueMorseSign{k}w_k\rho_M(\pi o_kx)
 ={}&\frac{\EulerE^{\Psi(-W/L)}}{2^{7/12}\sqrt{\pi x}}
 \exp\!\left(\frac{\mathfrak g-W-\tfrac12W^2}{L}\right)\\
...
\begin{proof}
The left side equals $\FabiusBounded(x)$ exactly by
...
For a summable bilateral family \(a:\IntegerNumbers\to E\), define
\[
 \mathcal R_s(a):=\sum_{\substack{\ell\in\IntegerNumbers\\\bar\ell=s}}a(\ell).
\]
If \(E\) is also Hausdorff, then for every natural modulus, including zero,
the residue family
\(s\mapsto\mathcal R_s(a)\) has sum \(\sum_{\ell\in\IntegerNumbers}a(\ell)\).
For a positive modulus \(M\) and a complete Hausdorff uniform complex
module, put
\[
 \mathcal A_k(a):=
 \sum_{\ell\in\IntegerNumbers}\chi_M(-\bar\ell k)a(\ell).
\]
Every twisted series is summable, and the unnormalized DFT satisfies
\[
 \mathcal D_M(\mathcal R_\bullet(a))(k)=\mathcal A_k(a),
 \qquad
 \mathcal R_s(a)=\frac1M\sum_{k\in\IntegerNumbers/M\IntegerNumbers}
...
\path{SummableCyclicAlias.lean}.  That specialization --- inserting the
absolutely convergent Fourier series of \(P\) and identifying its finite DFT
with the sampled values \(A_{n,r}\) --- is now carried out in
\path{HalfIntegerAliasIdentity.lean}; see the Lean status after
\cref{p1:thm:alias-identity}.
...
\begin{equation}\label{p1:eq:A-n-r-def}
 ...
 P\!\left(\frac jN\right)\EulerE^{-\pi\ImaginaryUnit rj/N}.
\end{equation}
...
\begin{align}
 ...
 \RvachevUp\!\left(\frac jN\right)
 \cos\!\left(\frac{\pi rj}{N}\right)\right]
 \label{p1:eq:A-Up-DCT}\\
 &=\frac1N\left[1-2\sum_{j=1}^{N-1}
 \FabiusBounded\!\left(\frac jN\right)
 \cos\!\left(\frac{\pi rj}{N}\right)\right].
 \label{p1:eq:A-F-DCT}
\end{align}
...
\[
 P(t)=\sum_{m\in\IntegerNumbers}c_m\EulerE^{\pi\ImaginaryUnit mt},
...
\[
 ...
 =\sum_{q\in\IntegerNumbers}\PhiU\!\left(\frac{r+2Nq}{2}\right),
\]
...
$j=N$ contributes zero because $P(1)=\RvachevUp(1)=0$, while $P(0)=1$; this gives
\eqref{p1:eq:A-Up-DCT}.  On $0\le j\le N$,
$\RvachevUp(j/N)=1-\FabiusBounded(j/N)$.  Since $r$ is odd,
\[
...
\paragraph{Lean status.}
The boxed identity \eqref{p1:eq:alias-identity} is formal, in
\path{HalfIntegerAliasIdentity.lean}, and for every modulus: with
\path{Fabius.periodizedUp} the two-periodization, \path{Fabius.gridSample} its sample
vector on \(\IntegerNumbers/2N\IntegerNumbers\), and \path{Fabius.foldedCoefficient}
the coefficient \(A_{N,r}=N^{-1}\,\mathcal D_{2N}(\text{samples})(r)\) in Mathlib's DFT
normalization, \path{Fabius.foldedCoefficient_eq_tsum} is
\(A_{N,r}=\sum_{q\in\IntegerNumbers}\PhiU(qN+r/2)\) for every \(N\ge1\) and every residue
\(r\) modulo \(2N\) --- neither \(N=2^n\) nor the oddness of \(r\) is used.  The proof is
the one above, carried out as a chain of rewrites: sampling the period-two Fourier series
\path{Fabius.rvachev_even_translate_fourier} at \(j/N\) turns \(\EulerE^{\pi\ImaginaryUnit
kj/N}\) into the standard character of \(\IntegerNumbers/2N\IntegerNumbers\)
(\path{Fabius.exp_pi_mul_I_eq_stdAddChar}), so the sample vector is \(\tfrac12\) times
the bilateral Fourier sums of \path{SummableCyclicAlias.lean} at \(-j\)
(\path{Fabius.gridSample_eq}); the DFT of those sums is the residue-class sums
(\path{Fabius.dft_intResidueTsum}), and two Mathlib identities, \path{ZMod.dft_comp_neg}
and \path{ZMod.dft_dft}, give \(\mathcal D_{2N}(\text{samples})=N\cdot\mathcal R_\bullet\)
(\path{Fabius.dft_gridSample}).  The residue class is finally re-indexed as the affine
line \(q\mapsto2Nq+r\) (\path{Fabius.residueClassEquiv}).  The cosine form is formal
over the full residue range: \path{Fabius.foldedCoefficient_eq_sum_cos} is
\(A_{N,r}=N^{-1}\sum_{j\in\IntegerNumbers/2N\IntegerNumbers}P(j/N)\cos(\pi rj/N)\), obtained
by re-indexing the character sum through \(j\mapsto-j\), which fixes the sample vector
(\path{Fabius.gridSample_neg}, from the period \(2\) and the evenness of the periodization,
\path{Fabius.periodizedUp_add_two} and \path{Fabius.periodizedUp_neg}), and writing the
character at \(\pm rj\) as \(\EulerE^{\pm\pi\ImaginaryUnit rj/N}\)
(\path{Fabius.stdAddChar_mul_eq_exp}, \path{Fabius.stdAddChar_neg_mul_eq_exp}).  The
identification \(P(j/N)=\RvachevUp(j/N)\) for \(0\le j\le N\) is
\path{Fabius.gridSample_natCast_of_le} (from \path{Fabius.periodizedUp_of_mem_Icc}, the
statement that the periodization is \(\RvachevUp\) on \([-1,1]\)).  The fold of \(j\)
with \(2N-j\) onto \(1\le j\le N-1\) is \path{Fabius.sum_zmod_even_eq_fold} in
\path{AliasDyadicBridge.lean}, a half-range fold for every even function on
\(\IntegerNumbers/2N\IntegerNumbers\) (the sum over \(\IntegerNumbers/2N\IntegerNumbers\) is first
written as a sum over \(0\le k<2N\), \path{Fabius.sum_zmod_eq_sum_range}); applied to the
cosine form, with \(P(0)=1\), \(P(1)=\RvachevUp(1)=0\) and the periodicity of the cosine
factor (\path{Fabius.cos_alias_angle_neg}), it gives \eqref{p1:eq:A-Up-DCT} verbatim,
\path{Fabius.foldedCoefficient_eq_halfRange_cos}, for every \(N\ge1\) and every residue
\(r\).  For \(N=2^n\) the samples are the exact rational dyadic values,
\(\RvachevUp(k/2^n)=\FabiusBounded\bigl((2^n-k)/2^n\bigr)=\mathtt{fabiusDyadic}\,n\,(2^n-k)\)
(\path{Fabius.foldedCoefficient_two_pow_eq_fabiusDyadic}, through
\path{Fabius.rvachevUp_eq_fabiusReal_one_sub_abs} and \path{Fabius.fabiusDyadic_cast}).
The variant \eqref{p1:eq:A-F-DCT} additionally uses
\(\sum_{j=1}^{N-1}\cos(\pi rj/N)=0\) for odd \(r\), which is
\path{Fabius.sum_Ico_cos_alias_angle_odd} (from the vanishing of the full character sum,
\path{Fabius.sum_cos_alias_angle_eq_zero}, folded to the half range); the resulting display,
with \(1-\RvachevUp(j/N)\) in place of \(\FabiusBounded(j/N)\), is
\path{Fabius.foldedCoefficient_eq_halfRange_cos_odd}, for every odd residue \(r\).
...
\end{equation}
In particular, all dyadic values of $\RvachevUp$ at denominator $N$ are reconstructed
...
\[
 P(j/N)=\frac12\sum_{r=0}^{2N-1}A_{n,r}\EulerE^{\pi\ImaginaryUnit rj/N}.
\]
...
\paragraph{Lean status.}
The three facts the proof uses are formal in \path{HalfIntegerAliasIdentity.lean}, for every
\(N\ge1\): the inverse transform in complex form,
\(P(j/N)=\tfrac12\sum_{r\in\IntegerNumbers/2N\IntegerNumbers}A_{N,r}\chi_{2N}(rj)\)
(\path{Fabius.gridSample_eq_sum_foldedCoefficient}, from Mathlib's \path{ZMod.dft_dft});
\(A_{N,0}=1\) (\path{Fabius.foldedCoefficient_zero}); the vanishing of every nonzero even
mode, \(A_{N,2s}=0\) for \(0<s<N\) (\path{Fabius.foldedCoefficient_two_mul_eq_zero}, since
each alias point \(qN+s\) is a nonzero integer, where \(\FourierTwoPi{\RvachevUp}\) vanishes);
and the symmetry \(A_{N,-r}=A_{N,r}\) (\path{Fabius.foldedCoefficient_neg}, from the
evenness of \(\FourierTwoPi{\RvachevUp}\)).  The pairing of \(r\) with \(2N-r\) into the cosine
form \eqref{p1:eq:inverse-DCT} is formal as well, for every even \(N\):
\path{Fabius.gridSample_eq_half_add_sum_odd} in \path{AliasDyadicBridge.lean} is the boxed
display verbatim.  It is the same two steps as on the sample side --- the reconstruction sum is
made real by pairing \(r\) with \(-r\) (\path{Fabius.sum_char_mul_foldedCoefficient_eq_sum_cos},
using \(A_{N,-r}=A_{N,r}\) and \(\chi(rj)+\chi(-rj)=2\cos(\pi rj/N)\)), and the resulting even
sum is folded to \(1\le r<N\) by \path{Fabius.sum_zmod_even_eq_fold}, the cosine factor being
even in the frequency by \path{Fabius.cos_alias_angle_neg_left}.  The constant \(\tfrac12\) is
\(A_{N,0}=1\) halved, the frequency \(r=N\) drops out because \(N\) is then an even residue, and
the surviving even frequencies below \(N\) vanish by
\path{Fabius.foldedCoefficient_two_mul_eq_zero}, which is what leaves the odd ones alone.  The
dual statement on the sample side is \path{Fabius.foldedCoefficient_eq_sum_cos}.
...
\begin{equation}\label{p1:eq:discrete-energy}
 ...
 =\frac1N\left[1+2\sum_{j=1}^{N-1}
 \RvachevUp\!\left(\frac jN\right)^2\right]-\frac12.}
\end{equation}
...
\paragraph{Lean status.}
The energy identity is formal before the odd/even split and the sample pairing:
\path{Fabius.sum_foldedCoefficient_sq} in \path{HalfIntegerAliasIdentity.lean} gives
\(\sum_{r\in\IntegerNumbers/2N\IntegerNumbers}A_{N,r}^2=\tfrac2N\sum_{j<2N}P(j/N)^2\)
for every \(N\ge1\).  It rests on a Parseval-type identity for Mathlib's DFT on
\(\IntegerNumbers/M\IntegerNumbers\) proved for every vector \(\Phi\),
\(\sum_r\mathcal D\Phi(r)\,\mathcal D\Phi(-r)=M\sum_j\Phi(j)^2\)
(\path{Fabius.sum_dft_mul_dft_neg}, from \path{ZMod.dft_apply} and \path{ZMod.dft_dft} alone),
combined with the symmetry \path{Fabius.foldedCoefficient_neg}.  The bookkeeping that turns it
into the displayed \eqref{p1:eq:discrete-energy} is formal too, for every even \(N\):
\path{Fabius.sum_odd_foldedCoefficient_sq} in \path{AliasDyadicBridge.lean} is
\[
  \sum_{\substack{1\le r<N\\ r\ \mathrm{odd}}}A_{N,r}^2
  =\frac1N\Bigl[1+2\sum_{j=1}^{N-1}\RvachevUp(j/N)^2\Bigr]-\frac12 .
\]
Both sides of the Parseval identity are sums of an even function over
\(\IntegerNumbers/2N\IntegerNumbers\), so the same half-range fold applies to each
(\path{Fabius.sum_gridSample_sq_eq_fold} on the samples, using \(P(0)=1\) and \(P(1)=0\);
\path{Fabius.sum_foldedCoefficient_sq_eq_fold} on the coefficients, using \(A_{N,0}=1\) and,
because \(N\) is even, \(A_{N,N}=0\)); the even residues strictly between \(0\) and \(N\) then
drop out by \path{Fabius.foldedCoefficient_sq_eq_zero_of_even}, leaving only the odd ones.  The
hypothesis that \(N\) be even is what makes \(r\mapsto2N-r\) fix no odd residue; it holds in
every dyadic case \(N=2^n\), \(n\ge1\).

\begin{remark}[Continuous limit]
As $n\to\infty$, the right side of \eqref{p1:eq:discrete-energy} tends
\[
 2\int_0^1\RvachevUp(t)^2\Differential t-\frac12
...
\paragraph{Lean status.}
The general bound \eqref{p1:eq:alias-error-general} is formal in
\path{AliasErrorBound.lean}, with a slightly larger constant: for every \(K\ge2\), every
\(N\ge1\) and every residue \(r<N\) (no parity assumption, no restriction to \(N=2^n\)),
\path{Fabius.norm_foldedCoefficient_sub_le'} gives
\[
 |A_{N,r}-\PhiU(r/2)|\le 2^{K+1}C_KN^{-K}\sum_{n\ge1}n^{-K},
\]
that is, \(2^K\zeta(K)\) where the display has \((2^K-1)\zeta(K)\): the formal proof bounds
every alias point by \(|qN+r/2|\ge N|q|/2\) (\path{Fabius.abs_alias_point_ge}) and sums over
all integers (\path{Fabius.norm_foldedCoefficient_sub_le}, \path{Fabius.tsum_abs_int_rpow_eq})
instead of over the odd ones.  The termwise input is the selected-factor bound of
\cref{p1:lem:selected-factor-bound} at the alias point (\path{Fabius.norm_alias_term_le}),
and the alias terms are separated from \(\PhiU(r/2)\) through the formal alias identity
\path{Fabius.foldedCoefficient_eq_tsum}.  The specialization \(K=n\), \(N=2^n\) giving
\eqref{p1:eq:alias-error-n} is not restated.
...
$\FabiusBounded(j/2^n)$ values, with only the final cyclotomic evaluation performed at high
precision.
...
\begin{aligned}
 ...
 \frac{\GaussianBinomial{n}{k}{1/2}}
 {2^{k(k-1)}(n+k)!}
 \sum_{s=0}^{j2^k-1}\ThueMorseSign{s}(s-j2^k)^{n+k}.
\end{aligned}}
\end{equation}
...
\paragraph{Lean status.}
Partial.  The dyadic values that \eqref{p1:eq:q-approximant} feeds into the cosine transform
are formal through the \(q\)-binomial formula
\path{Fabius.qBinomialThueMorseTranslatedFormulaIn_eq_fabiusAtInverseTwoPow}
(\path{FabiusQBinomialScalarFormula.lean}), and the alias identity underlying the left side
is \path{Fabius.foldedCoefficient_eq_tsum} (\path{HalfIntegerAliasIdentity.lean}).  The
assembly is formal up to the half-range fold: the cosine form of \(A_{N,r}\) over all
residues is \path{Fabius.foldedCoefficient_eq_sum_cos}, its half-range fold is
\path{Fabius.foldedCoefficient_eq_halfRange_cos}, and at \(N=2^n\) the samples are the
exact rational values \(\mathtt{fabiusDyadic}\,n\,(2^n-k)\)
(\path{Fabius.foldedCoefficient_two_pow_eq_fabiusDyadic}, \path{AliasDyadicBridge.lean}).
The sign form \eqref{p1:eq:A-F-DCT} is \path{Fabius.foldedCoefficient_eq_halfRange_cos_odd}
(odd \(r\), with \(1-\RvachevUp\) for \(\FabiusBounded\)).  The substitution of the
\(q\)-binomial closed form for those rational values into the transform is formal too, in
\path{AliasQBinomialBridge.lean}: \path{Fabius.foldedCoefficient_two_pow_eq_qBinomial} is
\[
  A_{2^n,r}=\frac1{2^n}\Bigl[1+2\sum_{k=1}^{2^n-1}
    Q(2^n-k,n)\cos\Bigl(\frac{\pi rk}{2^n}\Bigr)\Bigr],
\]
where \(Q(m,n)\) is \path{Fabius.qBinomialThueMorseDyadicTranslatedFormulaIn}, the
\(q\)-binomial closed form of \(\FabiusBounded(m/2^n)\) --- the general dyadic identity
\path{Fabius.fabiusFunction_dyadic_eq_qBinomialThueMorseDyadicTranslatedFormulaIn_real}, valid
for every \(m\le2^n\) and every translation parameter, not only at the single point
\(1/2^n\).  The reflected index \(2^n-k\) is the sample identity
\(\RvachevUp(k/2^n)=\FabiusBounded((2^n-k)/2^n)\)
(\path{Fabius.rvachevUp_dyadic_eq_qBinomial}).  The convergence clause is formal as well:
\path{Fabius.tendsto_foldedCoefficient_two_pow} in \path{AliasErrorBound.lean} is
\(A_{2^n,r}\to\PhiU(r/2)\) as \(n\to\infty\), for every fixed \(r\), obtained from the alias
error bound \path{Fabius.norm_foldedCoefficient_sub_le'} at the fixed order \(K=2\), which
already forces the difference below a multiple of \(4^{-n}\).  The sharp rate \(E_n\) of the
theorem, which optimizes \(K\) with \(n\) and gives \(2^{-n^2/2+\BigO(n\log n)}\), is not
claimed there.  So the whole of \eqref{p1:eq:q-approximant} is now formal, rate excepted: the
folded alias class of the infinite sinc product equals a finite cosine sum of \(q\)-binomial
expressions, and converges to the sinc-product value.
...
\begin{equation}\label{p1:eq:cyclotomic-field}
 \zeta_{2N}=\EulerE^{\pi\ImaginaryUnit/N},
 \qquad
 K_n:=\RationalNumbers(\zeta_{2N}+\zeta_{2N}^{-1}).
\end{equation}
Since every $\FabiusBounded(j/N)$ is rational,
...
\begin{equation}\label{p1:eq:G-n}
 G_n\simeq(\IntegerNumbers/2N\IntegerNumbers)^{\times}/\{\pm1\}.
\end{equation}
...
\begin{equation}\label{p1:eq:orbit-poly-rational}
 \mathcal P_n(X)\in\RationalNumbers[X],
 \qquad
 p_m(n)\in\RationalNumbers\quad(m\ge1).
\end{equation}
...
by $G_n$ and lies in $\RationalNumbers$.  The same argument applies to power sums.  Newton's
...
\begin{equation}\label{p1:eq:P-j-sample}
 ...
 \qquad 0\le j<2N.
\end{equation}
Every $P_j$ is rational, since it is a dyadic value of $\RvachevUp$.
...
\paragraph{Lean status.}
The two finite character sums the proof rests on are formal in
\path{CyclicCharacterSums.lean}, for every modulus rather than only powers of two: the
orthogonality \(\sum_{j\in\IntegerNumbers/M\IntegerNumbers}\chi_M(jk)=M[k=0]\)
(\path{Fabius.sum_stdAddChar_mul}, obtained from Mathlib's \path{ZMod.dft_apply} and
\path{ZMod.dft_dft} applied to the delta at \(0\), with no orthogonality lemma), and the
Ramanujan sum \eqref{p1:eq:Ramanujan-power-two} over the odd residues modulo \(2N\),
\(c_{2N}(S)=N(-1)^{S/N}\) if \(N\mid S\) and \(0\) otherwise
(\path{Fabius.ramanujanOddSum_eq}, with \path{Fabius.ramanujanOddSum} the sum
parametrized as \(r=2s+1\), \(s\in\IntegerNumbers/N\IntegerNumbers\)).  The power-trace
formula \eqref{p1:eq:Ramanujan-traces} itself --- the \(m\)-fold expansion of
\(A_{n,r}^m\) and the resummation over the odd residues --- is not formalized for general
\(m\).  Its first two cases are.  The case \(m=2\), which the theorem records as reducing to
\eqref{p1:eq:discrete-energy}, is \path{Fabius.sum_odd_foldedCoefficient_sq} in
\path{AliasDyadicBridge.lean}, for every even \(N\); it is proved there not by the \(m\)-fold
expansion but by folding both sides of the Parseval identity onto the half range, which for
\(m=2\) replaces the Ramanujan resummation entirely.  At a dyadic modulus the rationality the
theorem advertises is literal:
\path{Fabius.sum_odd_foldedCoefficient_sq_two_pow} states \(p_2(n)\) for \(N=2^n\), \(n\ge1\),
as a finite expression in the exact rational values \(\mathtt{fabiusDyadic}\,n\,(2^n-k)\),
through the sample identity \path{Fabius.rvachevUp_two_pow_eq_fabiusDyadic}.  The universal trace
\eqref{p1:eq:universal-trace} is formal in the form
\(\sum_{s<N}A_{N,2s+1}=1\), for every \(N\ge1\)
(\path{Fabius.sum_foldedCoefficient_odd}, in \path{HalfIntegerAliasIdentity.lean}): the
inverse transform at \(j=0\) gives \(\sum_{r}A_{N,r}=2\)
(\path{Fabius.sum_foldedCoefficient}, using \(P(0)=1\)), the even residues contribute
exactly \(A_{N,0}=1\), and the parity split of a sum over
\(\IntegerNumbers/2N\IntegerNumbers\) (\path{Fabius.sum_zmod_two_mul_eq_sum_even_add_odd})
isolates the odd ones.  Halving to \(p_1(n)=1/2\) by pairing \(r\) with \(2N-r\)
(\path{Fabius.foldedCoefficient_neg}) is the one step not restated.
...
\begin{equation}\label{p1:eq:spectral-power-sums}
 \SpectralPowerSum{m}:=\sum_{k=0}^{\infty}a_k^m
 \qquad(m\ge1).
\end{equation}
They converge absolutely.  The case $m=1$ is $\SpectralPowerSum{1}=1/2$; for $m=2$,
\begin{equation}\label{p1:eq:s2-L2}
 \SpectralPowerSum{2}=2\int_0^1\RvachevUp(t)^2\Differential t-\frac12.
\end{equation}
...
Every $\SpectralPowerSum{m}$ is a limit of explicitly computable rational numbers
...
$2\int_0^1\RvachevUp^2-1/2$ admits a superexponentially convergent rational sequence
...
\end{equation}
This is a probability measure on $\RealNumbers$.
...
\begin{equation}\label{p1:eq:orbit-second-moment}
 \int x^2\Differential\mu_n(x)=\frac2N p_2(n)\le\frac3N.
\end{equation}
Nevertheless the unnormalized trace remains $p_1(n)=1/2$.
\end{theorem}

\begin{proof}
From \eqref{p1:eq:discrete-energy} and $0\le\RvachevUp\le1$,
\[
...
\begin{equation}\label{p1:eq:A-shifted-sum}
 \boxed{
 A_{n,r}=\Gamma_{n,r}
 \sum_{q\in\IntegerNumbers}\frac{\PhiU(q+\alpha)}{(q+\alpha)^n}.}
\end{equation}
...
\[
 \prod_{j=n}^{\infty}
 \SincPi\!\left(\frac{qN+r/2}{2^j}\right)
 =\PhiU(q+\alpha).
\]
For $0\le j<n$,
\[
...
\begin{proof}
For the magnitude, use $|1-\EulerE^{2\pi\ImaginaryUnit x}|=2|\sin\pi x|$ in each factor of
\eqref{p1:eq:T-n-polynomial} and reverse the order of scales.  For the sign,
$\sin(\pi x)$ has sign $(-1)^{\Floor{x}}$ away from integers.  Since
$r<N$, all floors beyond $h=n$ vanish, and
\[
 \sum_{h=1}^{n}\Floor{\frac r{2^h}}
 =r-\BinaryDigitSum(r).
\]
Write $r=2c+1$.  Then $\BinaryDigitSum(r)=\BinaryDigitSum(c)+1$, so
$r-\BinaryDigitSum(r)\equiv \BinaryDigitSum(c)\pmod2$.  Hence the sign is $\ThueMorseSign{c}$ with
$c=(r-1)/2$.
\end{proof}

\paragraph{Lean status.}
Both halves of \cref{p1:prop:TM-sign-carrier} are formal in the Thue--Morse layer of the
corpus.  The magnitude \eqref{p1:eq:T-root-magnitude} is
\path{Fabius.norm_sum_thueMorseSign_exp} in \path{ThueMorseGelfond.lean}, which evaluates
$\bigl|\sum_{m<2^n}\ThueMorseSign m\,\EulerE^{\ImaginaryUnit mx}\bigr|$ to
$\prod_{j<n}2|\sin(2^jx/2)|$ for every real $x$ and every $n$; at $x=2\pi r/N$ with
$N=2^n$ the factors are $2|\sin(\pi r/2^{n-j})|$, the display above with the scales
reversed.  The sign word \eqref{p1:eq:Gamma-sign} is
\path{Fabius.neg_one_pow_sum_floor_odd_div_two_pow} in \path{RademacherSine.lean}:
for $c<2^n$, $(-1)^{\sum_{h=1}^{n}\Floor{(2c+1)/2^h}}=\ThueMorseSign{c}$, obtained from the
corpus identity $(-1)^{\sum_{h<n}\Floor{c/2^h}}=\ThueMorseSign{c}$
(\path{Fabius.neg_one_pow_sum_div_two_pow_const_one},
\path{DigitCharacterCongruence.lean}) through $\Floor{(2c+1)/2^{h+1}}=\Floor{c/2^h}$
(\path{Fabius.two_mul_add_one_div_two_pow_succ}).  The reading of the sign as the
square wave $\Sign\sin(\pi x)=(-1)^{\Floor{x}}$, used in the proof, is
\path{Fabius.sign_sin_pi_mul} in the same file.
...
\begin{equation}\label{p1:eq:observed-A-sign}
 \Sign A_{n,r}=\ThueMorseSign{(r-1)/2}
\end{equation}
...
\begin{equation}\label{p1:eq:shifted-positivity}
 \boxed{
 \sum_{q\in\IntegerNumbers}\frac{\PhiU(q+\alpha)}{(q+\alpha)^n}>0.}
\end{equation}
...
positivity of $\RvachevUp$ nor the elementary alias error bound proves the claim,
...
\begin{enumerate}
 ...
 for $\FabiusBounded(a/2^n)$, including
 $\FabiusBounded(1/4)=5/72$, $\FabiusBounded(1/8)=1/288$,
 $\FabiusBounded(3/8)=73/288$, and
 $\FabiusBounded(5/16)=305857/2073600$.
...
The most stable exact path uses the even moments of $\RvachevUp$.  Set $c_0=1$ and
compute
...
\begin{equation}\label{p1:eq:closed-dyadic-verification}
 \FabiusBounded\!\left(\frac a{2^n}\right)
 =\frac{2^{-\binom{n+1}{2}}}{n!}
 \sum_{h=0}^{a-1}\ThueMorseSign{h}
 \sum_{k=0}^{\Floor{n/2}}
...
# A_{n,r} = 1/N - (2/N) sum_j F_bounded(j/N) cos(pi*r*j/N).
# Use exact Fraction values for F_bounded and a high-precision cyclotomic backend
...
Use the established rapid decay of $\FourierTwoPi{\RvachevUp}$ to formalize:
\begin{enumerate}
...
\begin{enumerate}
 \item Work concretely in $\ComplexNumbers$ with a primitive root $\zeta_{2N}$ and prove
...
The examples $n=2,3$ are irreducible over $\RationalNumbers$.  The exact Galois action makes
...
\subsection{Spectral moments and nonlinear functionals of \texorpdfstring{$\RvachevUp$}{up}}

The trace limits \eqref{p1:eq:trace-limit} define constants
$\SpectralPowerSum{m}=\sum_ka_k^m$.  The cases $m\ge3$ do not have an immediate
...
$\SpectralPowerSum{m}$ for $m\ge3$.  Determine whether the rational trace sequences
...
lattice $2\pi\ImaginaryUnit\IntegerNumbers/\log2$.  The dyadic spectral tower has the same scale lattice,
...
$\RvachevUp$, seven source-paper transcriptions, and thirty-two frozen historical TeX
...
\[
 \mathscr U(x):=\FourierTwoPi{\RvachevUp}(x)
 =\prod_{n=0}^{\infty}\SincRad\!\left(\frac{\pi x}{2^n}\right).
\]
The central result is an exact decay law on every non-dyadic rational ray
$x_N=2^N(1+a/q)$. If $q$ is odd, $d=\OrderOperator_q(2)$, and
\[
...
$h=\prod_{k\ge1}\SincRad(\pi/2^k)=-\mathscr U'(1)$, while the first exponentially small
...
Cyclotomic orbit polynomial & Constructs $\mathscr P_{q,2}(X)\in\IntegerNumbers[X]$ and proves a resultant identity whose right side is a perfect $d$th power. & \newresult \\
...
$\FourierTwoPi{\RvachevUp}$ decreases faster than every power. Across its twelve drafts it
...
contains
\[
 T_n(z)=\sum_{m=0}^{2^n-1}(-1)^{\BinaryDigitSum(m)}z^m
...
function $\FabiusBounded$, the even compactly supported Rvachev function $\RvachevUp$, and the signed
global continuation governed by Thue--Morse signs \cite{p2:proveit,p2:ariascompact,p2:ariasarith}.
For this report only $\RvachevUp$ and its Fourier transform are needed. In the repository
normalization,
\begin{equation}
 \mathscr U(x):=\FourierTwoPi{\RvachevUp}(x)
 =\prod_{n=0}^{\infty}\SincRad\!\left(\frac{\pi x}{2^n}\right),
 \qquad \SincRad u=\frac{\sin u}{u},\quad\SincRad0=1.
 \label{p2:eq:U-product}
\end{equation}
The product is entire and obeys
\begin{equation}
 \mathscr U(x)=\SincRad(\pi x)\mathscr U(x/2).
...
Write $\ThueMorseSign{m}=(-1)^{\BinaryDigitSum(m)}$, where $\BinaryDigitSum(m)$ counts the binary ones in $m$. The
block polynomial is
\begin{equation}
 T_n(z):=\sum_{m=0}^{2^n-1}\ThueMorseSign{m}z^m
       =\prod_{j=0}^{n-1}(1-z^{2^j}).
 \label{p2:eq:TM-poly}
\end{equation}
...
\[
 \sum_{m<2^n}\ThueMorseSign{m}m^r=0\qquad(0\le r<n).
\]
The bridge to the Fourier product is
\begin{equation}
 |T_n(\EulerE^{2\pi\ImaginaryUnit t})|
...
For a primitive $q$th root $\zeta_q$ and $d=\OrderOperator_q(2)$, the sequence
...
and define
\begin{equation}
 H(y):=\prod_{r=1}^{\infty}\SincRad\!\left(\frac{\pi y}{2^r}\right),
 \qquad
...
\begin{equation}
 Q_n(t)=|T_n(\EulerE^{2\pi\ImaginaryUnit t})|.
 \label{p2:eq:Q-as-T}
\end{equation}
...
\[
 \mathscr U(2^Ny)=\prod_{k=0}^{N}\SincRad(\pi2^ky)
 \prod_{r=1}^{\infty}\SincRad\!\left(\frac{\pi y}{2^r}\right).
\]
...
\paragraph{Lean status.}
\Cref{p2:thm:exact-factorization} is the corpus's dyadic shell factorization, formal in
\path{SincProductShells.lean} for every real \(y\ne0\) and every \(N\):
\path{Fabius.norm_rvachevFourierProduct_two_pow_mul} gives
\(|\mathscr U(2^Ny)|=\prod_{j<N}|\sin(2^{j+1}\pi y)|\big/\bigl(2^{N(N+1)/2}(\pi|y|)^N\bigr)
\cdot|\mathscr U(y)|\), and its zero-safe cross-multiplied form
\path{Fabius.norm_rvachevFourierProduct_two_pow_mul_cross} is the assertion that the identity
survives when both sides vanish.  The two readings differ only in bookkeeping: the display
above splits off \(N+1\) factors and writes the tail as \(H(y)=\mathscr U(y)/\SincRad(\pi y)\),
whereas the formal statement splits off \(N\) and keeps \(\mathscr U(y)\); the passage
\(|\sin(\pi2^ky)|=|\sin(\pi2^kt)|\) for \(t=y-1\) and the identification
\(Q_{N+1}(t)=|T_{N+1}(\EulerE^{2\pi\ImaginaryUnit t})|\) are not restated in Lean.  The
exponent algebra behind the master cocycle is \path{Fabius.shell_exponent_identity}.
...
\begin{equation}
 d:=\OrderOperator_q(2),\qquad
 \zeta_q:=\EulerE^{2\pi\ImaginaryUnit/q},\qquad
 H_q:=\langle2\rangle\subseteq(\IntegerNumbers/q\IntegerNumbers)^\times.
 \label{p2:eq:q-d-H}
\end{equation}
Define
\begin{equation}
...
\begin{enumerate}
 ...
 \cref{p2:thm:exact-rational-ray}, with period $\OrderOperator_q(2)$.
\end{enumerate}
...
Let $K_q=\RationalNumbers(\zeta_q)$, $G_q=(\IntegerNumbers/q\IntegerNumbers)^\times$, and let $K_q^{H_q}$ be the fixed
field of $H_q=\langle2\rangle$. Then
\begin{equation}
 \Gamma_{a,q}=\FieldNorm_{K_q/K_q^{H_q}}(1-\zeta_q^a).
...
\begin{equation}
 \mathscr P_{q,2}(X)\in\IntegerNumbers[X]
 \label{p2:eq:orbit-integral}
\end{equation}
and
\begin{equation}
 \boxed{
 \ResidueOperator_z\!\left(\Phi_q(z),X-F_d(z)\right)=\mathscr P_{q,2}(X)^d.}
...
so every coefficient lies in $\RationalNumbers$ and therefore in $\IntegerNumbers$.

Since $\Phi_q$ is monic with roots $\zeta_q^a$, $a\in G_q$,
\[
 \ResidueOperator_z(\Phi_q(z),X-F_d(z))
...
\begin{equation}
 ...
 \ThueMorseSign{m_1}\cdots\ThueMorseSign{m_r}
 c_q(m_1+\cdots+m_r).}
 \label{p2:eq:power-trace}
\end{equation}
In particular,
\begin{equation}
 \TraceOperator_{K_q^{H_q}/\RationalNumbers}(\Gamma_{1,q})
 =\frac1d\sum_{m=0}^{2^d-1}\ThueMorseSign{m}c_q(m),
...
Expand
\[
 \Gamma_{a,q}=T_d(\zeta_q^a)=\sum_{m=0}^{2^d-1}\ThueMorseSign{m}\zeta_q^{am}.
\]
...
\[
 \prod_C\Gamma_{a_C,q}
 =\FieldNorm_{K_q^{H_q}/\RationalNumbers}
 \!\left(\FieldNorm_{K_q/K_q^{H_q}}(1-\zeta_q)\right)
 =\FieldNorm_{K_q/\RationalNumbers}(1-\zeta_q)=\Phi_q(1).
\]
...
\begin{align*}
 A_{a,q}^{2/d}
 &=\left(\prod_{j=0}^{d-1}|1-\EulerE^{2\pi\ImaginaryUnit t_j}|^2\right)^{1/d}\\
...
\paragraph{Lean status.}
The inequality \eqref{p2:eq:sqrt3-bound} is formal, division-free, in
\path{CyclicSineProductBound.lean}: \path{Fabius.prod_norm_one_sub_exp_orbit_le} gives
$\prod_{j<d}|1-\EulerE^{2\pi\ImaginaryUnit a2^j/q}|\le(\sqrt3)^{d}$, that is
$A_{a,q}^{1/d}\le\sqrt3$, for every $a$ and every $d$ with $2^{d}\equiv1\pmod q$ --- the
order of $2$ or any multiple of it --- with no coprimality or oddness hypothesis beyond
what the congruence forces.  The proof is not the cosine averaging above but a cyclic
form of the corpus's sharp Gelfond pairing: the two-step inequality
$|\sin\theta|^{2}|\sin2\theta|\le(\sqrt3/2)^{3}$
(\path{Fabius.abs_sin_sq_mul_abs_sin_two_mul_le}, \path{SharpGelfondBound.lean}) is
applied around the closed orbit, the last factor pairing with the first because
$|\sin(\pi a2^{d}/q)|=|\sin(\pi a/q)|$
(\path{Fabius.abs_sin_pi_mul_two_pow_div_of_mod}), and the general principle
\path{Fabius.prod_le_pow_of_sq_mul_succ_le_cyclic} --- cube the product, regroup as the
$d$ pairs, take cube roots --- gives $\prod_{j<d}|\sin(\pi a2^{j}/q)|\le(\sqrt3/2)^{d}$
(\path{Fabius.abs_prod_sin_orbit_le_sharp}).  The linear (non-cyclic) pairing of
\path{SharpGelfondBound.lean} leaves one factor uncovered and yields only
$2^{1/d}\cdot3^{(d-1)/2d}$; closing the orbit is exactly what removes that loss.  The
equality clause --- equality if and only if $q=3$ --- is half formalized.  The half that
matters for sharpness is: at $q=3$ the bound is attained, so the constant $\sqrt3$ cannot be
lowered.  That is \path{Fabius.abs_prod_sin_orbit_three_eq_sharp} in
\path{CyclicSineProductBound.lean}, with the two orbit points $\pi/3$ and $2\pi/3$ evaluated in
\path{Fabius.abs_sin_orbit_three} and the closed value $3/4$ in
\path{Fabius.abs_prod_sin_orbit_three_eq}.  The converse, that no other odd $q$ attains it, is
not formalized.
...
\paragraph{Lean status.}
The inequality \eqref{p2:eq:kappa-minimum} is \path{Fabius.rational_ray_exponent_ge_min}
in \path{CyclicSineProductBound.lean}, stated with $\kappa_{a,q}$ written out as
$\tfrac12+\log(2\pi)/\log2-\log A_{a,q}/(d\log2)$ and $A_{a,q}$ as the explicit product
of moduli, for odd $q>1$, $(a,q)=1$, and any $d$ with $2^{d}\equiv1\pmod q$.  The
positivity $A_{a,q}>0$ that makes the logarithm meaningful is
\path{Fabius.prod_norm_one_sub_exp_orbit_pos}: no factor vanishes because $a2^{j}/q$ is
never an integer when $q$ is odd and coprime to $a$
(\path{Fabius.orbit_point_not_int}, via $1-\EulerE^{2\pi\ImaginaryUnit x}\ne0$ off the
integers, \path{Fabius.one_sub_exp_two_pi_mul_I_ne_zero}).  The equality clause is not
formalized.
...
One has
\begin{equation}
 \OrderOperator_{q_d^-}(2)=d,
 \qquad
 \OrderOperator_{q_d^+}(2)=2d.
...
\begin{equation}
 ...
 \qquad
 H_d(1):=\prod_{k=1}^{d}\SincRad\!\left(\frac{\pi}{2^k}\right).
 \label{p2:eq:B-Hd}
\end{equation}
Then
\begin{equation}
...
\begin{equation}
 h:=\prod_{k=1}^{\infty}\SincRad\!\left(\frac{\pi}{2^k}\right)
...
\[
 \mathscr U(x)=\SincRad(\pi x)
 \prod_{k=1}^{\infty}\SincRad\!\left(\frac{\pi x}{2^k}\right).
\]
The derivative of $\SincRad(\pi x)$ at $x=1$ is $-1$, proving
\eqref{p2:eq:h-derivative}. Euler's product for the sine gives, for $|z|<\pi$,
\[
 \log\SincRad z=-\sum_{m=1}^{\infty}\frac{\zeta(2m)}{m\pi^{2m}}z^{2m}.
\]
Substitute $z=\pi/2^k$, sum over $k\ge1$, and use
$\sum_{k\ge1}4^{-km}=1/(4^m-1)$. Finally,
$\log\SincRad(\pi/2^k)=\BigO(4^{-k})$, which yields \eqref{p2:eq:Hd-tail}.
\end{proof}
...
resultant or Ramanujan power sums. No numerical integration of $\RvachevUp$ is involved.
...
Given odd $q>1$:
\begin{enumerate}
 \item compute $d=\OrderOperator_q(2)$;
 \item form $F_d(z)=\prod_{j=0}^{d-1}(1-z^{2^j})$;
 \item compute $R(X)=\ResidueOperator_z(\Phi_q(z),X-F_d(z))$ over $\IntegerNumbers[X]$;
...
\toprule
$q$ & $d=\OrderOperator_q(2)$ & $\mathscr P_{q,2}(X)$ \\
\midrule
3  & 2  & $X-3$ \\
5  & 4  & $X-5$ \\
7  & 3  & $X^2+7$ \\
...
\begin{equation}
 ...
 \SincRad\!\left(\frac{\pi x}{b^n}\right).
 \label{p2:eq:Ub}
\end{equation}
For $1<y<2$ and $t=y-1$, put
\begin{equation}
 H_b(y):=\prod_{r=1}^{\infty}\SincRad\!\left(\frac{\pi y}{b^r}\right),
 \qquad
...
\end{equation}
Let $q>1$, $(a,q)=1$, $(b,q)=1$, $d=\OrderOperator_q(b)$, and
\begin{equation}
...
\end{equation}
If $H_{q,b}=\langle b\rangle\subseteq(\IntegerNumbers/q\IntegerNumbers)^\times$, then the mean over its
cosets is
\begin{equation}
 \boxed{
...
\paragraph{Repository status.}  The module boundary below was proposed before
the library's flat layout settled, so the nested paths name nothing that
exists; and part of the plan has since been built under other names.  The
exact ray factorization of \cref{p2:thm:exact-factorization} is
\decl{norm_rvachevFourierProduct_fixedMantissa_of_mem_Ioo} in module
\lean{RvachevFixedMantissaRay}, stated for $N\in\mathbb N$ and mantissa
$y\in(1,2)$; the same module's
\decl{normalizedDyadicSineProduct_add_of_dyadic_period} is the periodic-orbit
engine, the identity $Q_{n+d}(t)=Q_d(t)Q_n(t)$ whenever $2^dt=t+z$ for an
integer $z$, which is the returning-seed hypothesis with no rationality
required.  The cyclotomic layer (the relative norm, the orbit polynomial, and
the mean exponent) has no counterpart in the library; the cyclotomic values it
would use are Mathlib's.
...
represent roots of unity in $\ComplexNumbers$ or in an abstract cyclotomic extension. The
resultant theorem is easiest abstractly; the absolute-value ray law is easiest in
$\ComplexNumbers$ with the standard embedding.
...
\[
 A_d^{\max}:=\max_{\OrderOperator_q(2)=d}A_{a,q},
 \qquad
 A_d^{\min}:=\min_{\OrderOperator_q(2)=d}A_{a,q}.
\]
...
\end{itemize}
A direct finite identity here would join the exact dyadic values of $\FabiusBounded$ to the
arithmetic rays of $\FourierTwoPi{\RvachevUp}$, rather than linking them only asymptotically.

\subsection{Sharp constants in other bases}

For $b\ge3$, determine
\[
 \sup_{q,a}\left(A_{a,q}^{(b)}\right)^{1/\OrderOperator_q(b)}
\]
...
\[
  \Phi_1(z)=\prod_{h\ge 0}\SincPi\!\left(\frac{z}{2^h}\right),
  \qquad \SincPi z=\frac{\sin(\pi z)}{\pi z}.
\]
The present report begins at a boundary not developed in that corpus: the arithmetic of the entire zero divisor and its interaction with repeated dyadic summation.  For the classical product, the cumulative multiplicity of positive zeros up to $N$ is exactly $2N-\BinaryDigitSum(N)$; hence the Thue--Morse sign is the parity of a spectral counting function.  This observation extends to a new Pascal--Rvachev hierarchy
\[
  \Phi_r(z)=\prod_{h\ge0}\SincPi\!\left(\frac z{2^h}\right)^{\binom h{r-1}},
  \qquad r\ge1.
\]
Each member is the Fourier transform of a $C_c^\infty$ probability density on $[-1,1]$ and satisfies the refinement ladder $\Phi_r(2z)=\Phi_r(z)\Phi_{r-1}(z)$, with $\Phi_0(z)=\SincPi(2z)$.  Its zero at an integer $n\ne0$ has multiplicity $\binom{\TwoAdicValuation(n)+1}{r}$, and the corresponding zero-divisor Dirichlet series is
\[
...
\textbf{Known inputs.}  The classical up-function sinc product, its probability interpretation, its canonical integer zero set with multiplicities $1+\TwoAdicValuation(n)$, its Thue--Morse sign law, and its rank-one endpoint/Fourier asymptotics are treated in the surveyed corpus and literature.
...
Throughout,
\[
  \SincPi z:=\begin{cases}
  \dfrac{\sin(\pi z)}{\pi z},&z\ne0,\\[0.4em]
  1,&z=0,
  \end{cases}
  \qquad
  \FourierTwoPi{f}(\xi):=\int_{\RealNumbers}f(x)e^{-2\pi i x\xi}\,\Differential x.
\]
Thus the Fourier transform of the uniform probability density on $[-a/2,a/2]$ is $\SincPi(a\xi)$.  The $2$-adic valuation of a positive integer is $\TwoAdicValuation(n)$, and
\[
  n=\sum_{j\ge0}b_j(n)2^j,
  \qquad b_j(n)\in\{0,1\},
  \qquad \BinaryDigitSum(n):=\sum_{j\ge0}b_j(n).
\]
...
\begin{equation}\label{p3:eq:phi1}
  \Phi_1(z):=\prod_{h=0}^{\infty}\SincPi\!\left(\frac z{2^h}\right).
\end{equation}
...
\begin{equation}\label{p3:eq:fabius-shift}
  \FabiusBounded(x)=f_1(x-1),\qquad 0\le x\le1.
\end{equation}
...
\[
  \Phi_1(z)=\prod_{n=1}^{\infty}\left(1-\frac{z^2}{n^2}\right)^{1+\TwoAdicValuation(n)};
\]
\item the sign law $\Sign\Phi_1(x)=\ThueMorseSign{N}$ for $N<x<N+1$;
...
\begin{equation}\label{p3:eq:classical-count}
  \mathcal N_1(N)=2N-\BinaryDigitSum(N).
\end{equation}
Consequently,
\begin{equation}\label{p3:eq:spectral-thue-morse}
  (-1)^{\mathcal N_1(N)}=\ThueMorseSign{N}
  =(-1)^{\BinaryDigitSum(N)}.
\end{equation}
...
\]
Legendre's digit formula gives $\sum_{h\ge1}\Floor{N/2^h}=N-\BinaryDigitSum(N)$, proving \eqref{p3:eq:classical-count}.  Since $2N$ is even, \eqref{p3:eq:spectral-thue-morse} follows.  Finally, $\Phi_1(0)>0$, and crossing the zero at $n$ changes the sign exactly when $\mu_1(n)$ is odd.  Therefore the sign on $(N,N+1)$ is $(-1)^{\mathcal N_1(N)}$.
\end{proof}

\paragraph{Lean status.}
\Cref{p3:thm:classical-count} is formal through two corpus statements.  The count
\eqref{p3:eq:classical-count} is Legendre's digit formula in the form
\(\sum_{h<M}\Floor{N/2^h}+\BinaryDigitSum(N)=2N\) for every \(M>\log_2N\),
\path{Fabius.sum_range_div_two_pow_add_digitSum} in \path{DigitDefectCounting.lean}; the
finite window is exactly the observation that all floors beyond \(h=\Floor{\log_2N}\)
vanish, so \(\mathcal N_1(N)=\sum_{h<M}\Floor{N/2^h}\).  The parity statement
\eqref{p3:eq:spectral-thue-morse} is
\path{Fabius.neg_one_pow_sum_div_two_pow_const_one} in
\path{DigitCharacterCongruence.lean}, \((-1)^{\sum_{h<m}\Floor{N/2^h}}=\ThueMorseSign N\)
for \(N<2^m\), which is the mod-\(2\) shadow of the count without passing through the
digit sum.  The identification of \(\mathcal N_1\) with the zero counting function of
\(\Phi_1\) itself (each factor \(h\) contributing a simple zero at every positive multiple
of \(2^h\)) is the zero-divisor description
\path{Fabius.analyticOrderAt_generalizedRvachevProduct_int} at unit weight, in
\path{GeneralizedZeroDivisor.lean}; the cumulative sum over \(n\le N\) is not separately
stated.
...
Let
\[
  \Phi_{1,M}(z):=\prod_{h=0}^{M-1}\SincPi\!\left(\frac z{2^h}\right),
  \qquad M\ge0.
\]
At $n\ge1$ its zero multiplicity is
\[
  \mu_{1,M}(n)=\min\{M,1+\TwoAdicValuation(n)\}.
\]
...
\begin{align}
 ...
 &=2N-\BinaryDigitSum(N)-2Q+\BinaryDigitSum(Q).\label{p3:eq:finite-count-digital}
\end{align}
In particular,
\begin{equation}\label{p3:eq:finite-sign}
  (-1)^{\mathcal N_{1,M}(N)}
  =\ThueMorseSign{N}\ThueMorseSign{Q}.
\end{equation}
\end{proposition}
...
\[
 ...
 =2Q-\BinaryDigitSum(Q)
\]
by \cref{p3:thm:classical-count}.  Subtracting it from $2N-\BinaryDigitSum(N)$ proves the result.
\end{proof}

The factor $\ThueMorseSign{Q}$ in \eqref{p3:eq:finite-sign} is a coarse-scale correction: a truncation remembers both the Thue--Morse parity at $N$ and the parity of the block index at scale $2^M$.
...
\[
  \mathcal Z_1(s):=\sum_{n\ge1}\frac{1+\TwoAdicValuation(n)}{n^s},
  \qquad
...
\end{equation}
Both identities extend meromorphically to $\ComplexNumbers$.  The infinite series has a simple pole at $s=1$ with residue $2$, and simple poles at
\begin{equation}\label{p3:eq:dyadic-lattice}
  s_k:=\frac{2\pi i k}{\log2},\qquad k\in\IntegerNumbers.
\end{equation}
At $s=s_k$ its residue is $\zeta(s_k)/\log2$.
\end{theorem}

\begin{proof}
Write $1+\TwoAdicValuation(n)=\sum_{h\ge0}\IndicatorOf{2^h\mid n}$.  Absolute convergence permits interchange:
\[
...
\begin{equation}\label{p3:eq:phir}
  ...
  \SincPi\!\left(\frac z{2^h}\right)^{a_{r,h}}.
\end{equation}
...
For each $r\ge1$, the product \eqref{p3:eq:phir} converges locally uniformly on $\ComplexNumbers$ and defines an even entire function with $\Phi_r(0)=1$.  Let
...
\[
 \log\SincPi(z/2^h)=\BigO_K(4^{-h}).
\]
Since $a_{r,h}=\BigO_r(h^{r-1})$, the series
$\sum_h a_{r,h}\log\SincPi(z/2^h)$ converges normally away from the finitely many zeros of the initial factors; equivalently, the associated Weierstrass product converges locally uniformly.  Evenness and normalization are immediate.
...
Let $r\ge1$ and $n\in\IntegerNumbers\setminus\{0\}$.  The multiplicity of the zero of $\Phi_r$ at $n$ is
...
\begin{proof}
The factor at scale $h$ vanishes at $n$ precisely when $2^h\mid n$, and then contributes multiplicity $\binom h{r-1}$.  Summing over $0\le h\le\TwoAdicValuation(n)$ and applying the hockey-stick identity gives \eqref{p3:eq:mu-r}.  Expanding each sinc by Euler's product and collecting equal zeros yields \eqref{p3:eq:canonical-r}.  Normal convergence follows from
...
$\sum_{h=0}^{\TwoAdicValuation(n)}\binom h{r-1}=\binom{\TwoAdicValuation(n)+1}r$.
It does not by itself prove that this number is the analytic zero order of
$\Phi_r$; that, the canonical product \eqref{p3:eq:canonical-r}, and locally
uniform convergence are now separate machine-checked statements, for an
arbitrary weight sequence $a$ with $\sum_h a_h2^{-h}<\infty$ and hence for the
Pascal weights $a_h=\binom h{r-1}$ (Lean's \decl{weightedScaleMultiplicity}
carries the index $r-1$ as its own parameter):
\decl{analyticOrderAt_generalizedRvachevProduct_int} (module
\lean{GeneralizedZeroDivisor}) identifies the analytic order of
$\Phi_a$ at every nonzero integer $n$ with the weighted scale multiplicity of
$|n|$, with \decl{generalizedRvachevProduct_eq_zero_iff_int} the zero set;
\decl{generalizedRvachevProduct_eq_canonical} (module
\lean{GeneralizedCanonicalForm}) is the canonical product
$\Phi_a(z)=\prod_{m\ge1}(1-z^2/m^2)^{\mu_a(m)}$; and
\decl{hasProdLocallyUniformlyOn_generalizedRvachevProduct} with
\decl{generalizedRvachevProduct_differentiable} (module
\lean{GeneralizedRvachevEntire}) give locally uniform convergence on every
disc and entireness.  Earlier text here placed all three in the analytic
frontier; that is no longer the state of the library.

Thus $\Phi_r$ has a zero at $n$ exactly when $2^{r-1}\mid n$.  Higher rank does not spread the zeros; it thins their locations while increasing their multiplicities according to the $r$th binomial coefficient in $\TwoAdicValuation(n)+1$.
...
\begin{equation}\label{p3:eq:finite-phir}
  ...
  \SincPi\!\left(\frac z{2^h}\right)^{\binom h{r-1}}.
\end{equation}
Its zero multiplicity is
\begin{equation}\label{p3:eq:finite-mu-r}
  \mu_{r,M}(n)=\binom{\min\{M,\TwoAdicValuation(n)+1\}}{r}.
\end{equation}
Let
\[
...
$1-2^{-s}=(s-s_k)\log2+\BigO((s-s_k)^2)$.  As in \cref{p3:thm:zeta1}, $\zeta(s_k)\ne0$, proving exact order and \eqref{p3:eq:laurent-zeta-r}.
\end{proof}
...
\begin{equation}\label{p3:eq:mu-rank-gen}
 ...
 =\frac{(1+u)^{\TwoAdicValuation(n)+1}-1}{u}.
\end{equation}
\end{corollary}
...
\end{equation}
For $r=1$, $Q_1(j)=1$ and $S_1(N)=\BinaryDigitSum(N)$.
\end{theorem}
...
binary digit formula \eqref{p3:eq:Nr-digital} and the sharp discrepancy
statements are not consequences of that generic layer-cake theorem and remain
to be formalized.  The parity consequence that matters downstream --- the sign
law $\operatorname{sgn}\Phi_r(x)=\varepsilon_r(\lfloor x\rfloor)$ on every
open unit interval --- is formal for every admissible weight sequence:
\decl{generalizedRvachevProduct_ofReal_eq_parityCharacter_mul_norm} (module
\lean{LobeSignLaw}) writes $\Phi_a(x)$ as the parity character
\decl{parityCharacter} of $\lfloor x\rfloor$ times $|\Phi_a(x)|$ for
$N<x<N+1$.
...
\begin{equation}\label{p3:eq:sign-law-r}
  \Sign\Phi_r(x)=\varepsilon_r(N).
\end{equation}
Let
\begin{equation}\label{p3:eq:Pr-dr}
 m_r:=\Ceiling{\log_2r},\qquad
 P_r:=2^{m_r},\qquad
 d_r:=2^{m_r-\BinaryDigitSum(r-1)}.
\end{equation}
...
\begin{tabular}{@{}rrrrl@{}}
\toprule
$r$ & $P_r$ & $d_r$ & $\Variance(X_r)$ & $(\sigma_{r,0},\ldots,\sigma_{r,P_r-1})$\\
\midrule
1 & 1 & 1 & $1/9$     & $-$\\
2 & 2 & 1 & $1/27$    & $+-$\\
3 & 4 & 2 & $1/81$    & $++--$\\
...
The largest centered summand divided by the standard deviation is $\BigO(m^{-1/2})$, so Lindeberg's condition holds.  The Berry--Esseen theorem \cite{p3:feller} gives the stated error because
\[
...
\item the full multiplicity $\binom{\TwoAdicValuation(n)+1}{r}$;
...
\end{enumerate}
For $r=1$ these collapse respectively to $1+\TwoAdicValuation(n)$, $\BinaryDigitSum(N)$, and the ordinary Thue--Morse sign.
\end{resultbox}
...
\begin{align}
 ...
 &=\frac{q^{r-1}}{(r-1)!}
   \left(\frac{\Differential}{\Differential q}\right)^{r-1}
...
\begin{equation}\label{p3:eq:q-binomial-theorem}
 ...
 =\sum_{r=0}^{M}q^{\binom r2}\GaussianBinomial{M}{r}{q} u^r,
\end{equation}
where
\[
 \GaussianBinomial{M}{r}{q}
...
\begin{equation}\label{p3:eq:q-sum-stat}
 ...
 =q^{\binom r2}\GaussianBinomial{M}{r}{q}.
\end{equation}
...
\midrule
$h_1+\cdots+h_r$ & $q^{\binom r2}\GaussianBinomial{M}{r}{q}$ & Gaussian $q$-binomial dyadic formulae\\
...
The ordinary Pascal coefficient $\binom h{r-1}$ and the Gaussian coefficient $\GaussianBinomial{M}{r}{q}$ are not rival appearances of ``binomial coefficients.''  They enumerate the same increasing scale tuples with two different statistics.  The Pascal--Rvachev hierarchy weights a tuple only by its terminal scale; the classical Gaussian identity weights it by the sum of all selected scales.  This provides a concrete route for transferring coefficient-extraction techniques from the dyadic Fabius formulae to the new hierarchy.
\end{resultbox}
...
\begin{equation}\label{p3:eq:Yr}
  Y_r:=3^{(r+1)/2}X_r,
\end{equation}
so that $\Variance(Y_r)=1$.  For $k\ge2$,
...
\end{equation}
Equivalently, for every fixed $z\in\RealNumbers$,
...
\begin{equation}\label{p3:eq:berry-esseen-r}
 \sup_x\left|\Probability(Y_r\le x)-\Probability(Z\le x)\right|
...
For a centered uniform variable of width $a$, $\Expectation|U|^3=a^3/32$.  Therefore the sum of third absolute moments before normalization is
\begin{align*}
...
$|x|/2^h\ge1$ and
\[
 \left|\SincPi\!\left(\frac x{2^h}\right)\right|
 \le\frac{2^h}{\pi|x|}.
\]
...
\end{align*}
where \cref{p3:lem:binomial-distance} absorbs the leading distance from $h$ to $H$, and the difference between $\log|x|$ and $H\log2$ contributes only $\BigO_r(H^r)$.  Since $H=(\log|x|)/(\log2)+\BigO(1)$, the result follows.
\end{proof}
...
Consequently,
\[
 \log\left|\SincPi\!\left(\frac{x_m}{2^h}\right)\right|
...
$\log\SincPi y=\BigO(y^2)$.  Hence the tail is
\[
...
For every $r\ge1$, the density $f_r$ belongs to $C_c^\infty(\RealNumbers)$.
\end{corollary}
...
\begin{equation}\label{p3:eq:mellin-rho}
 \int_0^{\infty}\rho(u)u^{s-1}\,\Differential u
 =\Gamma(s)\zeta(s+1).
\end{equation}
\end{lemma}
...
\[
 \int_0^\infty\rho(u)u^{s-1}\,\Differential u
 =-\frac1s\int_0^\infty
 \left(\frac1u-\frac1{e^u-1}\right)u^s\,\Differential u
...
\begin{equation}\label{p3:eq:mellin-bridge}
 ...
 \Gamma(-z)\zeta(1-z)\,
 \mathcal A_r(z)\,t^z\,\Differential z
 \ },
\end{equation}
...
\[
 ...
 \Gamma(-z)\zeta(1-z)u^z\,\Differential z.
\]
...
\begin{equation}\label{p3:eq:omega-zk}
  ...
  \qquad z_k:=ik\omega,\quad k\in\IntegerNumbers.
\end{equation}
...
\begin{equation}\label{p3:eq:residue-sum}
 \Psi_r(t)=\sum_{k\in\IntegerNumbers}
 \ResidueOperator_{z=z_k}\left[
...
\begin{equation}\label{p3:eq:Hr}
  H_r(x):=\Probability(D_r\le x),\qquad 0\le x\le2.
\end{equation}
...
Let $g_1$ be the density of $D_1$, so $g_1(x)=f_1(x-1)$.  On $[0,1]$,
$g_1$ is the standard Fabius function $\FabiusBounded$.  Since $D_0$ is
uniform on $[0,2]$, the density form of \eqref{p3:eq:Dr-refine} gives, for
$0\le x\le1$,
\[
 g_1(x)=2(g_1*g_0)(2x)=\int_0^{2x}g_1(u)\,\Differential u=H_1(2x).
\]
Therefore
\begin{equation}\label{p3:eq:H1-F}
  H_1(x)=\FabiusBounded(x/2),\qquad 0\le x\le2.
\end{equation}
...
\begin{equation}\label{p3:eq:Lambda-expansion}
 ...
 -\log\!\bigl(r!(\log2)^r\bigr)+\LittleO(1).
\end{equation}
\end{proposition}
...
$\log\SincPi(z/2^h)=\BigO_z(4^{-h})$.  Shifting the scale index gives
\begin{align*}
 \log\mathfrak P(u,2z)
 &=\log\SincPi(2z)
...
$\Phi_1(2z)=\SincPi(2z)\Phi_1(z)$.  Differentiating with respect to $u$ at zero successively recovers the logarithmic refinement laws for all higher ranks.  The singular boundary $u=3$ reflects the variance denominator $3^r$ and is the first rank-plane obstruction visible in the local Taylor expansion.
\end{remark}
...
\begin{align}
 \mathscr R_N(u)
 &=\sum_{n=1}^{N}
   \frac{(1+u)^{\TwoAdicValuation(n)+1}-1}{u},\label{p3:eq:rank-count-valuations}\\
...
& $\mu_r(n)=\binom{\TwoAdicValuation(n)+1}{r}$
...
& A family whose efficiency is governed by $\BinaryDigitSum(r-1)$.\\
...
\end{enumerate}
The dependence of the mask density $d_r/P_r=2^{-\BinaryDigitSum(r-1)}$ on the binary weight of the rank predicts sharp arithmetic phase changes as $r$ varies.
\end{frontierproblem}
...
The original rank-one sinc product contains more arithmetic than its pointwise sign pattern suggests.  Counting its zeros with multiplicity gives the exact formula $2N-\BinaryDigitSum(N)$, so the Thue--Morse sequence is the parity boundary of a spectral counting law.  Promoting the constant scale multiplicity to the Pascal column $\binom h{r-1}$ reveals that this is the first case of a coherent hierarchy rather than an isolated coincidence.

The hierarchy is unusually rigid.  Its support remains $[-1,1]$ at every rank; its refinement is exactly Pascal's identity; its zero multiplicities are binomial polynomials in $\TwoAdicValuation$; its zero zeta function is elementary; its finite counts are digital; its signs are automatic; its blocks solve exact Prouhet problems; its moments are rational; and its normalized central laws become Gaussian.  In the two most delicate asymptotic regimes, the same coefficient
...
navigation.  The non-elementarity material is now consolidated in Chapter~1
of the canonical inverse-Fabius synthesis at
...
symbol & meaning\\
\midrule
$\SincPi z$ & $\sin(\pi z)/(\pi z)$, normalized to equal $1$ at zero\\
...
$H_r$ & distribution function $\Probability(D_r\le x)$\\
$\mu_r(n)$ & integer-zero multiplicity $\binom{\TwoAdicValuation(n)+1}{r}$\\
...
$P_r$ & a digit-position period $2^{\Ceiling{\log_2r}}$\\
...
\begin{abstract}
Let $\FabiusBounded$ be the bounded Fabius distribution function,
$\FabiusQuantile=\FabiusBounded^{-1}$ its inverse on $[0,1]$,
$\RvachevUp(x)=1-\FabiusBounded(|x|)$ on $[-1,1]$, and
\[
...
$T_n=n(n+1)/2$, then $2^{-T_n}|\RvachevUp^{(n)}|$ has exactly the same value distribution on
$[-1,1]$ as $\RvachevUp$.  The signs on the $2^n$ cells are the length-$2^n$ Thue--Morse block.
Consequently every rearrangement-invariant norm, including every $L^p$ quasi-norm
$0<p\leq\infty$, obeys
\[
 \Norm{\RvachevUp^{(n)}}_p=2^{T_n}\Norm{\RvachevUp}_p.
\]
The level sets are also exact:
\[
 \bigl|\{x:\AbsoluteValue{\RvachevUp^{(n)}(x)}>2^{T_n}t\}\bigr|
 =2\FabiusQuantile(1-t),
 \qquad 0<t<1.
\]
...
Writing
$A_p=\tfrac12\Norm{\RvachevUp}_p^p$, one obtains the exact beta transform
\[
 A_p=p\int_0^1(1-y)^{p-1}\FabiusQuantile(y)\Differential y.
\]
For $\Dop=y\,\Differential/\Differential y$, the dilation variable $Z_p=pY_p$ with
$Y_p\sim\mathrm{Beta}(1,p)$ has Mellin symbol
\[
 \Expectation Z_p^s=p^s\frac{\Gamma(p+1)\Gamma(1+s)}{\Gamma(p+1+s)}.
\]
This yields a new Euler--Gamma differential calculus
\[
 A_p\sim
 \FabiusQuantile-\gamma\Dop\FabiusQuantile
 +\frac{\gamma^2+\pi^2/6}{2}\Dop^2\FabiusQuantile
 -\frac{\gamma^3+\gamma\pi^2/2+2\zeta(3)}{6}\Dop^3\FabiusQuantile+\cdots
 \quad\text{at }y=p^{-1}.
\]
...
\[
 \Norm{\RvachevUp}_p^p
 =2\FabiusQuantile(p^{-1})\left(1-\frac{\gamma}{\rho}
...
\[
 1-\Norm{\RvachevUp}_p\sim\frac{\sqrt{2\log2\,\log p}}{p}.
\]
...
$|\Phi(\xi)|^2\Differential\xi/\Norm{\RvachevUp}_2^2$ by $z=(2\pi\xi)^2$ has moments
\[
...
\[
 w_{\rm sp}(4z)=\frac12\SincRad^2(\sqrt z)\,w_{\rm sp}(z).
\]
...
\begin{enumerate}[label=(\roman*)]
\item the identities relating $\FabiusBounded$, $\RvachevUp$, and the signed global extension;
\item the differential-refinement law for $\RvachevUp$ and its finite Thue--Morse derivative tiling;
\item the infinite sinc product and its integer zero multiplicities;
\item the exact dyadic values of $\FabiusBounded$, their $q$-Pochhammer and
      Gaussian-binomial forms;
...
of $\FabiusBounded$ from integer derivative norms; the inverse-Fabius beta
transform; an exact Mellin
...
real Banach space, every signed natural-power moment through the exact Boolean-cube formula
and its parity corollaries, the positive-order normalized signed half-mixture for continuous
Banach-valued tests, the normalized absolute continuous-test identity, the absolute and
positive-order symmetric signed Borel pushforward equalities on Lebesgue measure restricted
to \([-1,1]\), and every nonnegative real absolute moment.  This is a post-audit proof-status
update; it does not alter the pinned novelty comparison above.

The report as a whole is not thereby formalized.  The signed Rademacher Borel law is now
closed in symmetric half-mixture form, with the sharp hypothesis \(0<n\); the order-zero
pushforward remains the unsymmetrized \(\RvachevUp\) law.  The \path{eLpNorm} and general
rearrangement-invariant norm ladder, the explicit inverse-Fabius level-set laws, the beta
transform and its asymptotics, and all spectral-moment, Hankel, q-Pearson, and q-binomial
layers below remain without matching declarations.
\end{statusbox}

\section{Normalization and established structural identities}
\label{p4:sec:baseline}

Let $\FabiusBounded:\RealNumbers\to[0,1]$ be the bounded Fabius function, with
$\FabiusBounded=0$ on $(-\infty,0]$, $\FabiusBounded=1$ on $[1,\infty)$,
symmetry
\begin{equation}
 \FabiusBounded(1-x)=1-\FabiusBounded(x),\qquad 0\leq x\leq1,
 \label{p4:eq:F-symmetry}
\end{equation}
and differential law $\FabiusBounded'(x)=2\FabiusBounded(2x)$ on
$0<x<1/2$, continued by symmetry.  Let
\begin{equation}
 \RvachevUp(x)=
 \begin{cases}
  1-\FabiusBounded(|x|),&|x|\leq1,\\
  0,&|x|\geq1.
 \end{cases}
 \label{p4:eq:up-from-F}
\end{equation}
Then $\RvachevUp$ is even, nonnegative, $C^\infty$, supported on $[-1,1]$, satisfies
$\RvachevUp(0)=1$ and $\int\RvachevUp=1$, and obeys
\begin{equation}
...
Write
\begin{equation}
 \ThueMorseSign{k}:=(-1)^{\BinaryDigitSum(k)},\qquad
 T_n:=\binom{n+1}{2}=\frac{n(n+1)}2,
 \label{p4:eq:epsilon-Tn}
\end{equation}
where $\BinaryDigitSum(k)$ is the binary digit sum.  The identities
$\ThueMorseSign{2k}=\ThueMorseSign{k}$ and
$\ThueMorseSign{2k+1}=-\ThueMorseSign{k}$ will be used repeatedly.

\begin{theorem}[Corpus baseline: exact derivative tiling]
\label{p4:thm:derivative-tiling}
For every $n\geq0$ and $x\in\RealNumbers$,
\begin{equation}
 \boxed{
 \RvachevUp^{(n)}(x)=2^{T_n}
 \sum_{k=0}^{2^n-1}\ThueMorseSign{k}\,
 \RvachevUp\!\left(2^n x+2^n-1-2k\right).}
 \label{p4:eq:derivative-tiling}
\end{equation}
Equivalently, for $0\leq k<2^n$ and $-1\leq y\leq1$,
\begin{equation}
 \boxed{
 \RvachevUp^{(n)}\!\left(-1+\frac{2k+1+y}{2^n}\right)
 =2^{T_n}\ThueMorseSign{k}\RvachevUp(y).}
...
the Thue--Morse identities above give exactly the signs $\ThueMorseSign{2k}$ and
$\ThueMorseSign{2k+1}$.  The scalar grows from $2^{T_n}$ to
$2^{T_n+n+1}=2^{T_{n+1}}$.
...
$\Probability(R=1)=\Probability(R=-1)=1/2$.
...
\begin{equation}
 \boxed{
 2^{-T_n}\AbsoluteValue{\RvachevUp^{(n)}(U)}\ \overset{\mathrm d}=\ \RvachevUp(U).}
 \label{p4:eq:absolute-equimeasurable}
\end{equation}
For every $n\geq1$,
\begin{equation}
 \boxed{
 2^{-T_n}\RvachevUp^{(n)}(U)\ \overset{\mathrm d}=\ R\RvachevUp(U).}
 \label{p4:eq:signed-equimeasurable}
\end{equation}
Equivalently, for every Borel function $H$ for which the integrals exist,
\begin{align}
 \frac12\int_{-1}^{1}H\!\left(2^{-T_n}\AbsoluteValue{\RvachevUp^{(n)}(x)}\right)\Differential x
 &=\frac12\int_{-1}^{1}H(\RvachevUp(y))\Differential y,\label{p4:eq:H-absolute}\\
 \frac12\int_{-1}^{1}H\!\left(2^{-T_n}\RvachevUp^{(n)}(x)\right)\Differential x
...
\[
 \int_{I_{n,k}}H\!\left(2^{-T_n}\AbsoluteValue{\RvachevUp^{(n)}(x)}\right)\Differential x
 =2^{-n}\int_{-1}^{1}H(\RvachevUp(y))\Differential y.
\]
...
\begin{statusbox}{Exact Lean parity for the distribution identities and signed moments}
At the continuous-test level, Lean exposes the following all-order signed finite mixture.
If \(E\) is any real Banach space and
\(H:\RealNumbers\to E\) is continuous, then for every \(n\ge0\),
\begin{equation}
 \int_{-1}^{1}H\!\left(\RvachevUp^{(n)}(x)\right)\Differential x
 =
 2^{-n}\sum_{k=0}^{2^n-1}
 \int_{-1}^{1}H\!\left(2^{T_n}\ThueMorseSign{k}\RvachevUp(y)\right)\Differential y.
...
For the sharp positive-order range \(0<n\), Lean also exposes the collapsed symmetric
half-mixture
\begin{equation}
 ...
 \label{p4:eq:formal-normalized-signed-test}
\end{equation}
This is \path{intervalIntegral_comp_normalized_iteratedDeriv_rvachev}.  Its codomain may be
any real Banach space, but its test function must be continuous.  The restriction \(0<n\)
is essential: for \(n=0\) the left side is the unsymmetrized integral of
\(H\circ\RvachevUp\), not the half-mixture.

Specializing to \(H(t)=t^m\) and evaluating the Boolean-cube weight enumerator gives the
separately named all-order identity, valid for every \(n,m\in\NaturalNumbers\),
\begin{equation}
 \int_{-1}^{1}\RvachevUp^{(n)}(x)^m\Differential x
 =2^{-n}\bigl(1+(-1)^m\bigr)^n
   \left(2^{T_n}\right)^m
   \int_{-1}^{1}\RvachevUp(y)^m\Differential y.
 \label{p4:eq:formal-signed-moment-boolean-cube}
\end{equation}
This is \path{intervalIntegral_iteratedDeriv_rvachev_pow}.  Its even-moment corollary,
valid for every derivative order including \(n=0\), is
\path{intervalIntegral_iteratedDeriv_rvachev_pow_of_even}; its odd-moment corollary,
valid only when \(0<n\), is
\path{intervalIntegral_iteratedDeriv_rvachev_pow_eq_zero_of_odd}.  The boundary distinction
is essential: when \(n=0\) and \(m\) is odd, the Boolean factor in
\eqref{p4:eq:formal-signed-moment-boolean-cube} is \(0^0=1\), so the identity recovers the
generally nonzero base moment.

The same Banach-valued continuous-test API proves
\begin{align}
 \int_{-1}^{1}H\!\left(\AbsoluteValue{\RvachevUp^{(n)}(x)}\right)\Differential x
 &=
 \int_{-1}^{1}H\!\left(2^{T_n}\RvachevUp(y)\right)\Differential y,
 \label{p4:eq:formal-absolute-test}\\
 \int_{-1}^{1}H\!\left(2^{-T_n}\AbsoluteValue{\RvachevUp^{(n)}(x)}\right)\Differential x
 &=
 \int_{-1}^{1}H(\RvachevUp(y))\Differential y.
...
At the Borel-measure level, let
\(\lambda_I\) be Lebesgue measure restricted to \(I=[-1,1]\), and write
\(f_{\#}\mu\) for pushforward.  Lean proves the literal Borel-measure equalities
\begin{align}
 \left(x\longmapsto2^{-T_n}\AbsoluteValue{\RvachevUp^{(n)}(x)}\right)_{\#}\lambda_I
 &=\RvachevUp_{\#}\lambda_I,
                                      \label{p4:eq:formal-absolute-pushforward}\\
 \left(x\longmapsto2^{-T_n}\RvachevUp^{(n)}(x)\right)_{\#}\lambda_I
 &=\frac12\left(
   \RvachevUp_{\#}\lambda_I+
   \left(y\longmapsto-\RvachevUp(y)\right)_{\#}\lambda_I\right)
   \qquad(0<n).                     \label{p4:eq:formal-signed-pushforward}
\end{align}
Their declarations are
\path{map_normalized_abs_iteratedDeriv_rvachev_restrict_Icc} and
\path{map_normalized_iteratedDeriv_rvachev_restrict_Icc}, respectively.  The second is a
scalar Borel pushforward equality, not merely another continuous-test theorem: it carries
all Borel distributional consequences.  Conversely,
\eqref{p4:eq:formal-normalized-signed-test} is the directly exposed Banach-valued API.
After normalizing \(\lambda_I\) to the uniform probability law, the pushforward equality is
exactly \eqref{p4:eq:signed-equimeasurable}, equivalently the law of
\(R\RvachevUp(U)\); Lean packages the symmetric measure mixture rather than introducing
random variables \(R,U\).  At \(n=0\), the pushforward is instead the unsymmetrized
\(\RvachevUp_{\#}\lambda_I\).  Thus the Rademacher Borel-law gap is closed without erasing
the order-zero boundary.
\end{statusbox}
...
\(p=0\),
\begin{equation}
 \int_{-1}^{1}\AbsoluteValue{\RvachevUp^{(n)}(x)}^p\Differential x
 =\left(2^{T_n}\right)^p\int_{-1}^{1}\RvachevUp(x)^p\Differential x.
 \label{p4:eq:absolute-moments}
\end{equation}
For every $n,m\in\NaturalNumbers$, the signed natural-power moments satisfy the exact
Boolean-cube identity
\begin{equation}
 \int_{-1}^{1}\RvachevUp^{(n)}(x)^m\Differential x
 =2^{-n}\bigl(1+(-1)^m\bigr)^n
   \left(2^{T_n}\right)^m
   \int_{-1}^{1}\RvachevUp(y)^m\Differential y.
 \label{p4:eq:signed-moments-boolean-cube}
\end{equation}
Consequently,
\begin{align}
 \int_{-1}^{1}\RvachevUp^{(n)}(x)^m\Differential x
 &=\left(2^{T_n}\right)^m
   \int_{-1}^{1}\RvachevUp(x)^m\Differential x
 &&(m\text{ even},\ n\geq0),\label{p4:eq:signed-moments-even}\\
 \int_{-1}^{1}\RvachevUp^{(n)}(x)^m\Differential x
 &=0
 &&(m\text{ odd},\ 0<n).
...
\path{Real.rpow} convention \(0^0=1\).  The signed master identity
\eqref{p4:eq:signed-moments-boolean-cube} is
\path{intervalIntegral_iteratedDeriv_rvachev_pow}; its two parity specializations are
\path{intervalIntegral_iteratedDeriv_rvachev_pow_of_even} and
\path{intervalIntegral_iteratedDeriv_rvachev_pow_eq_zero_of_odd}.  Thus the even formula is
formal for every \(n\ge0\), whereas the zero odd moment is asserted only for \(0<n\); at
\(n=0\), the master formula deliberately retains the Boolean factor \(0^0=1\).
...
\begin{equation}
 \boxed{
 \Norm{\RvachevUp^{(n)}}_X=2^{T_n}\Norm{\RvachevUp}_X,
 \qquad n\geq0.}
...
\begin{equation}
 \boxed{
 \Norm{\RvachevUp^{(n)}}_{L^p(\RealNumbers)}
 =2^{n(n+1)/2}\Norm{\RvachevUp}_{L^p(\RealNumbers)}.}
...
$2^{-T_n}|\RvachevUp^{(n)}|$ and $\RvachevUp$ have the same decreasing rearrangement.  Rearrangement
...
\label{p4:cor:sup-TV}
Since $\Norm{\RvachevUp}_1=\Norm{\RvachevUp}_\infty=1$,
\begin{align}
 \Norm{\RvachevUp^{(n)}}_1&=2^{T_n},\label{p4:eq:L1-exact}\\
 \Norm{\RvachevUp^{(n)}}_\infty&=2^{T_n},\label{p4:eq:Linf-exact}\\
 \TotalVariationOf{\RvachevUp^{(n)}}
 =\Norm{\RvachevUp^{(n+1)}}_1&=2^{T_{n+1}}.
 \label{p4:eq:TV-exact}
\end{align}
For every $0<p\leq\infty$,
\begin{equation}
 \frac{\Norm{\RvachevUp^{(n+1)}}_p}{\Norm{\RvachevUp^{(n)}}_p}=2^{n+1}.
...
\begin{equation}
 \boxed{
 \RvachevUp^{(n)}(x_{n,k})=2^{T_n}\ThueMorseSign{k}.}
 \label{p4:eq:midpoint-values}
\end{equation}
These are precisely the points at which $|\RvachevUp^{(n)}|$ attains its global maximum.
...
\begin{proof}
Set $y=0$ in \eqref{p4:eq:cell-form} and use $\RvachevUp(0)=1$.  Strict decrease of $\RvachevUp$ on
...
Let $\FabiusQuantile=\FabiusBounded^{-1}:[0,1]\to[0,1]$.  Since
$\RvachevUp(y)=1-\FabiusBounded(|y|)$,
\begin{equation}
 \SetOf{y\in[-1,1]:\RvachevUp(y)>t}
 =(-\FabiusQuantile(1-t),\FabiusQuantile(1-t)),\qquad0<t<1.
...
\begin{equation}
 \boxed{
 \left|\SetOf{x\in\RealNumbers:
 \AbsoluteValue{\RvachevUp^{(n)}(x)}>2^{T_n}t}\right|
 =2\FabiusQuantile(1-t).}
 \label{p4:eq:derivative-level-set}
\end{equation}
Consequently, if $V_n=2^{-T_n}\AbsoluteValue{\RvachevUp^{(n)}(U)}$, then
\begin{align}
 \Probability(V_n>t)&=\FabiusQuantile(1-t),\label{p4:eq:V-tail}\\
 \Probability(V_n\leq t)&=1-\FabiusQuantile(1-t),\label{p4:eq:V-cdf}\\
 f_{V_n}(t)&=\FabiusQuantile'(1-t),\qquad0<t<1.
 \label{p4:eq:V-density}
\end{align}
For $n\geq1$, the signed variable $2^{-T_n}\RvachevUp^{(n)}(U)$ has density
\begin{equation}
 f_n(v)=\frac12\FabiusQuantile'(1-|v|),\qquad-1<v<1.
...
and densities explicitly through the totalized inverse \(\FabiusQuantile\), do
not yet have matching Lean
...
\begin{equation}
 \FabiusQuantile\!\left(1-\RvachevUp(U)\right)=|U|.
...
\begin{equation}
 A_p:=\frac12\Norm{\RvachevUp}_p^p
 =2^{-1-pT_n}\Norm{\RvachevUp^{(n)}}_p^p,
 \qquad p>0.
 \label{p4:eq:Ap-definition}
\end{equation}
The second expression is independent of $n$ by \eqref{p4:eq:Lp-norm-law}.
Set $A_0:=1$.  If $W=\RvachevUp(U)$, then $A_p=\Expectation W^p$ for every $p\geq0$.
...
\begin{equation}
 (-1)^rA_p^{(r)}
 =\Expectation\bigl[W^p(-\log W)^r\bigr]>0,
 \qquad r\geq0.
...
\begin{equation}
 (-1)^r\Delta^r A_m
 =\Expectation\bigl[W^m(1-W)^r\bigr]>0,
 \qquad m,r\geq0,
...
The sequence $\{A_m\}_{m\geq0}$ uniquely determines $\FabiusQuantile$, hence
$\FabiusBounded$ and $\RvachevUp$.
...
$\Norm{\RvachevUp^{(n)}}_m$, $m=1,2,\ldots$, for any one derivative order $n$, contains the entire
...
\begin{equation}
 \boxed{A_p=\Expectation \FabiusQuantile(Y_p).}
...
\[
 A_p=\int_0^1(1-\FabiusBounded(x))^p\Differential x.
\]
The monotone substitution $y=\FabiusBounded(x)$, interpreted as a
Lebesgue--Stieltjes substitution at the
flat endpoints, gives
\[
 A_p=\int_0^1(1-y)^p\Differential \FabiusQuantile(y).
\]
Integration by parts has no boundary term because $\FabiusQuantile(0)=0$ and
$(1-y)^p=0$ at $y=1$.
...
\begin{equation}
 \Norm{\RvachevUp^{(n)}}_p^p
 =2^{pT_n+1}p\int_0^1(1-y)^{p-1}\FabiusQuantile(y)\Differential y.
 \label{p4:eq:derivative-beta-transform}
\end{equation}
\end{corollary}

The beta kernel concentrates at $y\asymp p^{-1}$.  Because
$\FabiusQuantile$ is slowly varying at zero,
\eqref{p4:eq:beta-transform} already implies
$A_p\sim\FabiusQuantile(1/p)$.  The next section replaces that
...
Let
\begin{equation}
 \Dop:=y\frac{\Differential}{\Differential y}
...
\begin{equation}
 \boxed{
 M_p(s):=\Expectation Z_p^s
 =p^s\frac{\Gamma(p+1)\Gamma(1+s)}{\Gamma(p+1+s)}.}
 \label{p4:eq:Mellin-symbol}
\end{equation}
For any bounded extension of $\FabiusQuantile$ to $(1,\infty)$,
\begin{equation}
 \boxed{
 A_p=\left[M_p(\Dop)\FabiusQuantile\right](p^{-1}),}
 \label{p4:eq:exact-operator}
\end{equation}
where the operator is rigorously interpreted by the Bochner average
\begin{equation}
 \left[M_p(\Dop)H\right](y)
 :=\Expectation\left[H(Z_py)\right].
 \label{p4:eq:Bochner-dilation}
\end{equation}
The value in \eqref{p4:eq:exact-operator} is independent of how
$\FabiusQuantile$ is extended, because
$0<Z_p/p=Y_p<1$.
\end{proposition}
...
\[
 \Expectation Z_p^s=p^s p\int_0^1y^s(1-y)^{p-1}\Differential y
 =p^s\frac{\Gamma(p+1)\Gamma(1+s)}{\Gamma(p+1+s)}.
\]
The dilation semigroup satisfies $\exp(t\Dop)H(y)=H(\EulerE^t y)$.  Averaging with
\(t=\log Z_p\) and evaluating at $y=p^{-1}$ gives
$\Expectation\FabiusQuantile(Z_p/p)
=\Expectation\FabiusQuantile(Y_p)=A_p$.
\end{proof}
...
\begin{equation}
 ...
 \Dop^mG(p^{-1})
 +\BigO_{N}\!\left(
 \FabiusQuantile(p^{-1})\frac{(\log\rho)^{c_N}}{\rho^N}
 \right),}
...
\[
 \FabiusQuantile(\EulerE^t y)
 =\sum_{m=0}^{N-1}\frac{t^m}{m!}\Dop^m\FabiusQuantile(y)+R_N(t,y).
\]
The all-orders inverse endpoint expansion in the corpus implies, for each fixed $m$,
$\Dop^m\FabiusQuantile(y)/\FabiusQuantile(y)$ is bounded by a power of
$\log\rho$ times $\rho^{-m}$.
Fix a constant $C>L+1$ and split the expectation into $|t|\leq C\rho$ and its complement.
On the central range, every point $\EulerE^u y$ between $y$ and $\EulerE^t y$ has endpoint coordinate
$\rho+\BigO_C(1)$.  Integrating the elasticity shows
$\FabiusQuantile(\EulerE^u y)/\FabiusQuantile(y)=\BigO_C(1)$ there, so
Taylor's integral remainder is
\[
 |R_N(t,y)|\leq C_NG(y)\frac{(\log\rho)^{c_N}}{\rho^N}|t|^N.
\]
The moments $\Expectation|\log Z_p|^N$ are bounded uniformly for $p\geq2$: near zero the density of
$Z_p$ is at most one, while for $z\geq1$ it is bounded by $\EulerE^{-z/2}$.
For the omitted negative tail,
$\Probability(Z_p<\EulerE^{-C\rho})\leq\EulerE^{-C\rho}$; for the positive tail,
$\Probability(Z_p>\EulerE^{C\rho})\leq\exp(-\EulerE^{C\rho})$ once $p$ is large.  Since
$\FabiusQuantile(p^{-1})\asymp\rho\EulerE^{-L\rho}$, both tail contributions
are $\LittleO(\FabiusQuantile(p^{-1})\rho^{-N})$.  Taking expectations gives
coefficients $\mu_{m,p}$; replacing them by $\mu_m$ changes the result by
$\BigO(p^{-1}\FabiusQuantile(p^{-1}))$, which is beyond every power of
$\rho^{-1}$ relative to $\FabiusQuantile(p^{-1})$.  The Bell and Bernoulli descriptions
...
Thus
\begin{align}
 A_p={}&\FabiusQuantile-\gamma\Dop\FabiusQuantile
 +\frac{\gamma^2+\pi^2/6}{2}\Dop^2\FabiusQuantile\nonumber\\
 &-\frac{\gamma^3+\gamma\pi^2/2+2\zeta(3)}{6}\Dop^3\FabiusQuantile\nonumber\\
 &+\frac{\gamma^4+\gamma^2\pi^2+8\gamma\zeta(3)+3\pi^4/20}{24}
 \Dop^4\FabiusQuantile
 +\cdots,
 \qquad y=p^{-1}.
...
\begin{equation}
 G_0(p^{-1})
 :=\rho\EulerE^{-L\rho-L\beta}
 =\frac{\sqrt T}{\EulerE\sqrt L}\EulerE^{-\sqrt{2LT}},
 \label{p4:eq:G0-p}
\end{equation}
and
\begin{equation}
 \frac{\FabiusQuantile(p^{-1})}{G_0(p^{-1})}
...
\begin{equation}
 \frac{y\FabiusQuantile'(y)}{\FabiusQuantile(y)}
...
In particular,
\begin{equation}
 \boxed{A_p\sim\FabiusQuantile(p^{-1}).}
...
\[
 \frac{A_p}{\FabiusQuantile}
 =1-\gamma\frac{\Dop\FabiusQuantile}{\FabiusQuantile}
 +\frac{\gamma^2+\pi^2/6}{2}
  \frac{\Dop^2\FabiusQuantile}{\FabiusQuantile}
 +\BigO(\rho^{-3}\log\rho).
\]
Equation \eqref{p4:eq:imported-elasticity} gives
\[
 \frac{\Dop\FabiusQuantile}{\FabiusQuantile}
 =\frac1\rho+\frac{\Psi'(\theta)-1}{L\rho^2}
 +\BigO\!\left(\frac{\log\rho}{\rho^3}\right).
\]
If $a=\Dop\log\FabiusQuantile$, then
$\Dop^2\FabiusQuantile/\FabiusQuantile
=a^2+\Dop a=\rho^{-2}+\BigO((\log\rho)/\rho^3)$.
...
\begin{equation}
 \boxed{
 \frac{\Norm{\RvachevUp}_p^p}{2G_0(p^{-1})}
...
\eqref{p4:eq:imported-G-expansion} and use $\Norm{\RvachevUp}_p^p=2A_p$.
\end{proof}
...
$-\log\Norm{\RvachevUp}_p$ is $L\rho/p$; since it tends to zero,
$1-\Norm{\RvachevUp}_p\sim-\log\Norm{\RvachevUp}_p$.
\end{proof}
...
Fix a phase $\vartheta\in\RealNumbers/\IntegerNumbers$.  For all sufficiently large integers $j$, define
\begin{equation}
...
\begin{equation}
 ...
 \left[
 \rho_j\left(
 \frac{\Norm{\RvachevUp}_{p_j}^{p_j}}{2G_0(p_j^{-1})}-1
...
\emph{global norm measurements}; pointwise evaluation or inversion of
$\FabiusBounded$ is not required.
\end{theorem}
...
Let
\begin{equation}
 C_2:=\Norm{\RvachevUp}_2^2=\int_{\RealNumbers}|\Phi(\xi)|^2\Differential\xi.
 \label{p4:eq:C2}
\end{equation}
...
\begin{equation}
 \Differential\nu(\xi):=\frac{|\Phi(\xi)|^2}{C_2}\Differential\xi
...
\begin{equation}
 \boxed{
 \int_0^\infty z^n\Differential\mu_{\rm sp}(z)
 =2^{n(n+1)}.}
...
\[
 \int_{\RealNumbers}(2\pi\xi)^{2n}|\Phi(\xi)|^2\Differential\xi
 =\Norm{\RvachevUp^{(n)}}_2^2.
\]
...
\begin{equation}
 \int_0^\infty z^n w_{\rm ln}(z)\Differential z
 =\exp(nL+n^2L)=2^{n(n+1)}.
...
Indeed, $w_{\rm sp}(z)\Differential z$ and $w_{\rm ln}(z)\Differential z$ are distinct probability measures with
...
$z=(2\pi m)^2$, $m\in\NaturalNumbers$, whereas $w_{\rm ln}$ is strictly positive on $(0,\infty)$.
...
\begin{equation}
 \Phi(2\xi)=\SincRad(2\pi\xi)\Phi(\xi).
...
\begin{equation}
 \omega_{n,j}:=\int\ell_{n,j}(z)\Differential\mu(z)
...
\begin{equation}
 \boxed{
 \int_0^\infty Q(z)\Differential\mu_{\rm sp}(z)
 =\sum_{j=1}^{n}\omega_{n,j}Q(\lambda_{n,j})
 =\int_0^\infty Q(z)w_{\rm ln}(z)\Differential z.}
...
\begin{equation}
 ...
 \label{p4:eq:Mn}
\end{equation}
By \eqref{p4:eq:Linf-exact}, $\RvachevUp$ saturates this derivative weight exactly:
\begin{equation}
 \Norm{\RvachevUp^{(n)}}_\infty=M_n.
...
existence of the compactly supported nonzero function $\RvachevUp$ requires.
...
\end{equation}
Hence $\RvachevUp$ belongs to no finite-order Gevrey class on any interval containing its support.
\end{proposition}
...
\begin{equation}
 ...
 -\MetricDistance\!\left(r-\frac12,\NaturalNumbers\right)^2
 \right].}
...
\begin{equation}
 |\Phi(\xi)|\leq
 2^{-\frac12(r-1/2)^2+\frac12\MetricDistance(r-1/2,\NaturalNumbers)^2}.
...
comments in the accompanying archive.  The script reconstructs $\RvachevUp$ by inverse FFT on a
periodic box of length $4$ from $55$ dyadic sinc factors, using $2^{19}$ samples.  Since
$\RvachevUp$ is supported on $[-1,1]$, the box supplies exact geometric zero padding.  The run used
\[
 \Delta x=2^{-17}=7.62939453125\times10^{-6},
 \qquad \Delta\xi=\frac14.
\]
The reconstructed density had integral $1$ and value $\RvachevUp(0)=1$ to displayed precision;
...
\caption{Normalized derivatives $2^{-T_n}\RvachevUp^{(n)}$, vertically offset.  Each row is a
...
$2^{T_n}\Norm{\RvachevUp}_p$.  The largest absolute deviation of the ratio from $1$ was
$2.22\times10^{-16}$.
...
$\FabiusQuantile(1-\RvachevUp(x))=x$ on $0\leq x\leq1$.  Logarithmic finite differences estimated
...
\begin{tabular}{@{}rrrrrr@{}}
\toprule
$p$ & $A_p/\FabiusQuantile(1/p)$ & $\FabiusQuantile$ only &
through $\Dop$ & through $\Dop^2$ & through $\Dop^3$\\
\midrule
...
The inverse report gives an all-orders expansion of $\FabiusQuantile$ in the
differential algebra generated
...
\begin{equation}
 S_{\rm sp}(z)=\int_0^\infty\frac{\Differential\mu_{\rm sp}(t)}{z-t}
...
spectral Mellin transform to the Mellin transform of $\SincRad^2(\sqrt z)$.  Another is to use
...
\[
 \int_0^\infty z^nQ_j(z)w_{\rm sp}(z)\Differential z=0
 \qquad(n\geq0)
\]
...
\item \textbf{Completed.}  The derivative tiling has a public exact cell coordinate,
      ...
      corollary.
\item \textbf{Absolute and signed distribution laws completed.}  The normalized absolute
      law is an exact Borel pushforward equality and has Banach-valued continuous-test forms.
      The signed finite-mixture identity is formal for continuous Banach-valued tests and all
      \(n\); for \(0<n\), both its normalized Banach-valued half-mixture and its symmetric
      Borel pushforward equality are formal.  Its Boolean-cube natural-moment identity and
      the exact even/positive-order odd parity laws are also formal.  At \(n=0\), the signed
      law remains correctly unsymmetrized.
...
by the beta-transform layer.  The signed Rademacher Borel law is now closed in symmetric
half-mixture form; every beta/spectral layer in this report remains a frontier statement.
...
The four parts above cite Lean declarations in their prose where the authors
knew of them, but carry no theorem-level crosswalk.  This appendix records the
correspondence for the results whose formal counterpart was verified by
reading the Lean statement, in the discipline of the Rvachev decay volume's
parity ledger: a row is listed only when the Lean statement has the displayed
mathematical content, and it says where Lean is weaker, more general, or
normalized differently.  All declarations are in namespace \lean{Fabius}.
The convention agrees on both sides: \decl{rvachevFourier} is this volume's
$\FourierTwoPi{\RvachevUp}$, \decl{rvachevFourierProduct} is $\prod_n\SincPi(z/2^n)$, and
\decl{generalizedRvachevProduct} at the weights $a_h=\binom h{r-1}$ is
$\Phi_r$, with \decl{parityCharacter} at those weights the sign character
$\varepsilon_r$.

\begin{longtable}{@{}p{0.29\textwidth}p{0.42\textwidth}p{0.23\textwidth}@{}}
\toprule
Statement in this volume & Exact Lean declaration(s) & Remarks\\
\midrule
\endfirsthead
\toprule
Statement in this volume & Exact Lean declaration(s) & Remarks\\
\midrule
\endhead
\Cref{p1:prop:cosine-spectrum}: the half-integer cosine series of
$\RvachevUp$ on $[-1,1]$ &
\decl{rvachev_cosine_series} (module \lean{PoissonSummation}) &
Exact; stated as a complex-valued series with the real
$\FourierTwoPi{\RvachevUp}(k+\tfrac12)$ coefficients.\\
\addlinespace
\Cref{p1:lem:selected-factor-bound}, \Cref{p4:cor:IBP-envelope}: the
selected-factor envelope $|\Phi(x)|\le 2^{\binom K2}/(\pi|x|)^K$ &
\decl{norm_rvachevFourierProduct_le_prefix_div} (module
\lean{SincPrefixBound}) &
More general: every $K\in\mathbb N$, including $K=0$, for every real
$x\neq0$.\\
\addlinespace
\Cref{p2:thm:exact-factorization}: the exact ray factorization at
$x=2^Ny$, $y\in(1,2)$ &
\decl{norm_rvachevFourierProduct_fixedMantissa_of_mem_Ioo} (module
\lean{RvachevFixedMantissaRay}) &
Exact, in modulus form.  The periodic-orbit identity
$Q_{n+d}(t)=Q_d(t)Q_n(t)$ under $2^dt=t+z$, $z\in\mathbb Z$, is
\decl{normalizedDyadicSineProduct_add_of_dyadic_period}, with no
rationality hypothesis.\\
\addlinespace
\Cref{p3:thm:classical-count}: $\sum_{n\le N}\mu(n)=2N-w(N)$ &
\decl{sum_dyadicZeroMultiplicity_eq} (module
\lean{DyadicZeroMultiplicity}) &
Exact, over $\mathbb N$ with $w$ the binary weight.\\
\addlinespace
\Cref{p3:thm:zero-multiplicity}: the multiplicity
$\binom{\TwoAdicValuation(n)+1}r$, the canonical product, and entireness of
$\Phi_r$ &
\decl{weightedScaleMultiplicity_choose},
\decl{analyticOrderAt_generalizedRvachevProduct_int},
\decl{generalizedRvachevProduct_eq_zero_iff_int},
\decl{generalizedRvachevProduct_eq_canonical},
\decl{hasProdLocallyUniformlyOn_generalizedRvachevProduct},
\decl{generalizedRvachevProduct_differentiable} &
More general: every weight sequence $a$ with $\sum_ha_h2^{-h}<\infty$; the
Pascal case is $a_h=\binom h{r-1}$.  Lean's Pascal index is $r-1$, so its
\texttt{choose r} is this volume's $\binom{\cdot}{r-1}$.\\
\addlinespace
\Cref{p3:thm:sign-law-r}: $\operatorname{sgn}\Phi_r(x)=\varepsilon_r(\lfloor
x\rfloor)$ &
\decl{generalizedRvachevProduct_ofReal_eq_parityCharacter_mul_norm} (module
\lean{LobeSignLaw}), \decl{parityCharacter} &
More general: every admissible weight sequence, on every open unit interval
$(N,N+1)$.\\
\addlinespace
\Cref{p3:thm:prouhet-r}: the generalized Prouhet cancellation
$\sum_{n<2^L}\varepsilon_r(n)n^k=0$ &
\decl{sum_range_two_pow_parityCharacter_mul_pow_eq_zero} (module
\lean{GeneralizedProuhetBlock}) &
More general: every weight sequence, every block length $L$, every
$k$ below the number of odd layers.\\
\addlinespace
\Cref{p3:prop:Lambert-coordinate}: the saddle equation solved by the
generalized Lambert coordinate &
\decl{generalizedLambertCoordinate_solves_saddle} (module
\lean{GeneralizedLambertCoordinate}) &
Exact, under the explicit lower-branch domain hypothesis.\\
\addlinespace
\Cref{p3:cor:rank-one-Psi}: the exact periodic decomposition of the
rank-one Laplace exponent &
\decl{negativeLaplaceLog_exact_periodic_decomposition} (module
\lean{NegativeLaplace}) &
\textbf{Convention differs}: Lean's \decl{negativeLaplaceLog} is
$+\log L$ at argument $2t$, i.e.\ $\operatorname{negativeLaplaceLog}(s)=-\Psi_1(s/2)$
in this volume's notation; the periodic correction and the tail-error term
are separate named objects there.\\
\addlinespace
\Cref{p3:lem:pascal-tail}: \(\sum_{h\le j}\binom h{r-1}2^{j-h}=2^{j+1}-Q_r(j)\) &
\decl{PascalTail.sum_choose_mul_two_pow_eq}, in the additive form
\decl{PascalTail.sum_choose_mul_two_pow_add_tailCount} (module
\lean{PascalTailIdentity}) &
Exact, over \(\mathbb N\): with Lean's index \(k=r-1\),
\decl{PascalTail.tailCount} \(j\,k=\sum_{i\le k}\binom{j+1}i\) is \(Q_r(j)\).
The additive form holds with no side condition; the subtraction form is its
corollary.  Proved by induction with Pascal's rule, not by the subset count.\\
\addlinespace
\Cref{p2:thm:sqrt3-bound}: \(A_{a,q}^{1/d}\le\sqrt3\) for odd \(q\) and
\((a,q)=1\) &
\decl{normalizedDyadicSineProduct_le_sqrt_three_pow_of_dyadic_period} (module
\lean{PeriodicOrbitGelfondBound}), from the one-shot bound
\decl{normalizedDyadicSineProduct_succ_le} and the extraction lemma
\decl{le_of_pow_le_mul_pow} &
More general: for \emph{every} real \(t\) whose orbit returns,
\(2^dt=t+z\) with \(z\in\mathbb Z\), the normalized product
\(\prod_{j<d}|2\sin(\pi2^jt)|\) is at most \(\sqrt3^{\,d}\); the volume's
statement is \(t=a/q\), \(2^d\equiv1\pmod q\), together with the identity
\(|1-e^{2\pi i\theta}|=2|\sin\pi\theta|\), which is not part of the module.
The equality case \(q=3\) is not formal.\\
\addlinespace
\Cref{p3:thm:sign-law-r}, periodicity part: the parity of \(\binom j{r-1}\)
is periodic in \(j\) (Lucas), hence \(\varepsilon_r\) is \(2\)-automatic &
\decl{PascalParity.choose_mod_two_eq_mod_two_pow},
\decl{PascalParity.choose_add_two_pow_mod_two},
\decl{eventuallyPeriodic_parityWord_choose},
\decl{finite_twoKernel_parityCharacter_choose} (module
\lean{PascalParityPeriodic}) &
Lucas mod \(2\), iterated: for \(k<2^m\) the parity of \(\binom hk\) depends
only on \(h\bmod 2^m\), for every \(h,k,m\).  The library takes the period
\(2^{r-1}\), which is a period but not the minimal one; the exact minimal
period \(P_r=2^{m_r}\) and the count \(d_r\) of \(-1\) values per period in
\eqref{p3:eq:Pr-dr} are not formal.  \(2\)-automaticity is the library's
finiteness of the \(2\)-kernel of \(\varepsilon_r\), through
\decl{finite_twoKernel_parityCharacter_iff}.\\
\bottomrule
\end{longtable}

The Part~4 derivative-distribution results already name their declarations in
the status boxes beside each theorem and are not repeated here.  Everything
else in this volume --- in particular the alias identity of Part~1 beyond its
abstract engine, the equality case \(q=3\) of the periodic-orbit bound and the
cyclotomic layer of Part~2, the digital count and discrepancy statements of
Part~3, the rank-$r$ global decay and sharp ray, and the spectral moments of
Part~4 --- is unformalized.
