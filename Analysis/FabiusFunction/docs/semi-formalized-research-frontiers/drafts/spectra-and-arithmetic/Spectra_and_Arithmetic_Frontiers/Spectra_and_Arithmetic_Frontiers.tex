...
orders.  With $o_k=2k+1$, $\ThueMorseSign{k}=(-1)^{s_2(k)}$, and
$w_k=|a_k|$, one has
\[
...
\begin{enumerate}
 ...
 $\Sign \PhiU(x)=(-1)^{s_2(M)}$ on $M<x<M+1$.
...
\[
 \widehat f(z)=\int_{\RealNumbers}f(t)\EulerE^{-2\pi\ImaginaryUnit tz}\Differential t.
\]
Put
...
\begin{equation}\label{p1:eq:s2-L2}
 \mathfrak s_2=2\int_0^1\RvachevUp(t)^2\Differential t-\frac12.
\end{equation}
...
$r-s_2(r)\equiv s_2(c)\pmod2$.  Hence the sign is $\ThueMorseSign{c}$ with
$c=(r-1)/2$.
\end{proof}
...
Use the established rapid decay of $\widehat\RvachevUp$ to formalize:
\begin{enumerate}
...
\[
 \mathscr U(x):=\widehat{\RvachevUp}(x)
...
$\widehat{\RvachevUp}$ decreases faster than every power. Across its twelve drafts it
...
arithmetic rays of $\widehat{\RvachevUp}$, rather than linking them only asymptotically.
...
\[
  ...
  \end{cases}
  \qquad
  \widehat f(\xi):=\int_{\RealNumbers}f(x)e^{-2\pi i x\xi}\,\Differential x.
\]
...
\]
\item the sign law $\Sign\Phi_1(x)=(-1)^{s_2(N)}$ for $N<x<N+1$;
...
\begin{equation}
 ...
 =2^{T_n}\varepsilon_k\RvachevUp(y).}
...
\begin{equation}
 \boxed{
 \RvachevUp^{(n)}(x_{n,k})=2^{T_n}\varepsilon_k.}
