...
\begin{equation}\label{p1:eq:F-rho-spectral}
 \boxed{
 F(x)=\sum_{k=0}^{\infty}\ThueMorseSign{k}w_k\,
 \rho_M(\pi o_kx).}
\end{equation}
...
$F(x)=1-\RvachevUp(x)$.  Subtract \eqref{p1:eq:cosine-spectrum} from $1$ and use
...
$\RvachevUp(j/N)=1-F(j/N)$.  Since $r$ is odd,
\[
...
\begin{equation}\label{p1:eq:observed-A-sign}
 \operatorname{sgn}A_{n,r}=\ThueMorseSign{(r-1)/2}
\end{equation}
...
function $F$, the even compactly supported Rvachev function $\RvachevUp$, and the signed
...
$[0,1]$, $\RvachevUp(x)=1-F(|x|)$ on $[-1,1]$, and
\[
...
\[
 \bigl|\{x:\AbsoluteValue{\RvachevUp^{(n)}(x)}>2^{T_n}t\}\bigr|=2G(1-t),
 \qquad 0<t<1.
\]
...
\[
 A_p=p\int_0^1(1-y)^{p-1}G(y)\Differential y.
\]
...
\begin{enumerate}[label=(\roman*)]
\item the identities relating $F$, $\RvachevUp$, and the signed global extension;
...
Let $F:\RealNumbers\to[0,1]$ be the bounded Fabius function, with $F=0$ on $(-\infty,0]$,
...
Let $G=F^{-1}:[0,1]\to[0,1]$.  Since $\RvachevUp(y)=1-F(|y|)$,
\begin{equation}
 \SetOf{y\in[-1,1]:\RvachevUp(y)>t}=(-G(1-t),G(1-t)),\qquad0<t<1.
...
\begin{align}
 \Probability(V_n>t)&=G(1-t),\label{p4:eq:V-tail}\\
 \Probability(V_n\leq t)&=1-G(1-t),\label{p4:eq:V-cdf}\\
...
The sequence $\{A_m\}_{m\geq0}$ uniquely determines $G$, hence $F$ and $\RvachevUp$.
...
expectation converges to $\Probability(W\leq t)=1-G(1-t)$ by
...
\begin{equation}
 ...
 =2^{pT_n+1}p\int_0^1(1-y)^{p-1}G(y)\Differential y.
...
$\Expectation G(Z_p/p)=\Expectation G(Y_p)=A_p$.
\end{proof}
...
\[
 G(\EulerE^t y)=\sum_{m=0}^{N-1}\frac{t^m}{m!}\Dop^mG(y)+R_N(t,y).
\]
...
$G(\EulerE^u y)/G(y)=\BigO_C(1)$ there, so Taylor's integral remainder is
\[
...
$G(p^{-1})\asymp\rho\EulerE^{-L\rho}$, both tail contributions are
$\LittleO(G(p^{-1})\rho^{-N})$.  Taking expectations gives coefficients $\mu_{m,p}$;
...
\begin{equation}
 ...
 -\dist\!\left(r-\frac12,\NaturalNumbers_0\right)^2
 \right].}
...
\begin{equation}
 |\Phi(\xi)|\leq
 2^{-\frac12(r-1/2)^2+\frac12\dist(r-1/2,\NaturalNumbers_0)^2}.
...
$G(1-\RvachevUp(x))=x$ on $0\leq x\leq1$.  Logarithmic finite differences estimated
