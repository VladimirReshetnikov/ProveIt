...
\lean{Fabius.norm\_sum\_thueMorseSign\_exp} in \path{ThueMorseGelfond.lean}, which evaluates
...
\lean{Fabius.neg\_one\_pow\_sum\_floor\_odd\_div\_two\_pow} in \path{RademacherSine.lean}:
...
(\lean{Fabius.neg\_one\_pow\_sum\_div\_two\_pow\_const\_one},
\path{DigitCharacterCongruence.lean}) through $\Floor{(2c+1)/2^{h+1}}=\Floor{c/2^h}$
(\lean{Fabius.two\_mul\_add\_one\_div\_two\_pow\_succ}).  The reading of the sign as the
square wave $\Sign\sin(\pi x)=(-1)^{\Floor{x}}$, used in the proof, is
\lean{Fabius.sign\_sin\_pi\_mul} in the same file.
