...
evenness of \(\FourierTwoPi{\RvachevUp}\)).  The final pairing of \(r\) with \(2N-r\) into the
cosine form \eqref{p1:eq:inverse-DCT} is not separately formalized; the dual statement on
the sample side, the cosine form of \(A_{N,r}\) over all residues \(j\), is
\path{Fabius.foldedCoefficient_eq_sum_cos}.
