...
\]
Thus the Fourier transform of the uniform probability density on $[-a/2,a/2]$ is $\SincPi(a\xi)$.  The $2$-adic valuation of a positive integer is $\nu_2(n)$, and
\[
...
\begin{proof}
Write $1+\nu_2(n)=\sum_{h\ge0}\IndicatorOf{2^h\mid n}$.  Absolute convergence permits interchange:
\[
...
\end{enumerate}
For $r=1$ these collapse respectively to $1+\nu_2(n)$, $\BinaryDigitSum(N)$, and the ordinary Thue--Morse sign.
\end{resultbox}
