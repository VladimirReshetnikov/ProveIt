...
and its parity corollaries, the normalized absolute continuous-test identity, the equality of
the two absolute Borel pushforward measures on Lebesgue measure restricted to \([-1,1]\), and
every nonnegative real absolute moment.  This is a post-audit proof-status update; it does not
alter the pinned novelty comparison above.
...
not yet its Rademacher Borel simplification.  If \(E\) is any real Banach space and
...
\begin{equation}
 ...
 =\RvachevUp_{\#}\lambda_I.                                      \label{p4:eq:formal-absolute-pushforward}
\end{equation}
Its declaration is
...
\(R\RvachevUp(U)\).  Thus \eqref{p4:eq:signed-equimeasurable} and the full Borel formulation of
...
\item \textbf{Completed.}  The derivative tiling has a public exact cell coordinate,
      ...
      corollary.
\item \textbf{Absolute part and signed natural moments completed.}  The normalized absolute
      ...
      \(n\); its Boolean-cube natural-moment identity and the exact even/positive-order odd
      parity laws are also formal.  The simplified signed Rademacher Borel law remains open.
