...
\(\sum_{j=1}^{N-1}\cos(\pi rj/N)=0\) for odd \(r\); that character sum is not separately
formalized.
...
The substitution of the \(q\)-binomial closed form for those rational values into the
transform, and the sign form \eqref{p1:eq:A-F-DCT}, are not formalized, nor is the
convergence statement, which is analytic.
