...
line \(q\mapsto2Nq+r\) (\path{Fabius.residueClassEquiv}).  The two cosine displays
\eqref{p1:eq:A-Up-DCT}--\eqref{p1:eq:A-F-DCT}, which pair \(j\) with \(2N-j\) and use
\(P=\RvachevUp\) on \([0,1]\), are not separately formalized.
...
cosine form \eqref{p1:eq:inverse-DCT} is not separately formalized.
...
assembly --- the cosine form \eqref{p1:eq:A-F-DCT}, which pairs \(j\) with \(2N-j\), and the
substitution of the \(q\)-binomial values into it --- is not formalized, nor is the
