...
sequence $\FabiusBounded(2^{-n})$, and more generally
$\FabiusBounded(c2^{-n})$ and fixed-order derivative
...
\begin{enumerate}[label=(\roman*)]
  \item the zero set of the dyadic infinite sinc product, the multiplicity
        $1+\TwoAdicValuation(m)$ at $2\pi m$, its spectral zeta function, or the associated
        digit-counting laws;
...
\begin{enumerate}
  ...
  $\FabiusBounded(c2^{-n})$ and their fixed derivative analogues satisfy no polynomial-
...
We use the bounded Fabius function $\FabiusBounded:\RealNumbers\to[0,1]$, clamped to $0$ on
$(-\infty,0]$ and to $1$ on $[1,\infty)$, with
\begin{equation}
 \FabiusBounded'(x)=2\FabiusBounded(2x),\qquad 0\le x\le\frac12,
 \label{eq:fabius-diff}
\end{equation}
and $\FabiusBounded(x)+\FabiusBounded(1-x)=1$ on $[0,1]$.  It is strictly increasing on $[0,1]$,
$C^\infty$, and flat at the endpoints.  Rvachev's even up function is the
compactly supported density on $[-1,1]$ related by
\begin{equation}
  \RvachevUp(t)=\FabiusBounded(t+1)\quad(-1\le t\le0),\qquad
  \RvachevUp(t)=\FabiusBounded(1-t)\quad(0\le t\le1).
  \label{eq:F-up}
\end{equation}
...
\begin{equation}
 ...
 =\frac{2^N-1-2k}{2^N},
 \qquad
 \prod_{j=1}^{N}\eps_j=\ThueMorseSign{k}=(-1)^{\BinaryDigitSum(k)}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{data/dyadic_thue_morse_spectrum.png}
\caption{At level $N=7$, every odd dyadic frequency occurs once, and the
coefficient is the Thue--Morse sign $\ThueMorseSign{k}$.}
...
\end{equation}
We use the complete exponential Bell polynomials $\ExponentialCompleteBellPolynomial{k}$, normalized by
\begin{equation}
 \exp\left(\sum_{j\ge1}x_j\frac{t^j}{j!}\right)
 =\sum_{k\ge0}\ExponentialCompleteBellPolynomial{k}(x_1,\ldots,x_k)\frac{t^k}{k!}.
...
\begin{equation}
 ...
 \ExponentialCompleteBellPolynomial{k}\bigl(S_1(m),-1!S_2(m),\ldots,
...
\begin{equation}
 ...
 \ExponentialCompleteBellPolynomial{k}\bigl(0,\kappa_2,0,\kappa_4,\ldots,\kappa_k\bigr),
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\textwidth]{data/level3_rank_defect.png}
...
\includegraphics[width=\linewidth]{data/rank_growth.png}
\caption{Full, unsigned, and signed ranks.}
\end{subfigure}\hfill
\begin{subfigure}{0.49\textwidth}
\includegraphics[width=\linewidth]{data/golden_entropy.png}
...
and
\begin{equation}
 \OrderOperator_{z=\pi bm}\Phi_b=1+\nu_b(m),
...
\begin{equation}
 1+\TwoAdicValuation(2^Mn)=M+1+\TwoAdicValuation(n).
...
The ruler function $1+\TwoAdicValuation(n)$ is therefore an obstruction to finite-level
...
Furthermore,
\begin{equation}
 \mu_n=\ExponentialCompleteBellPolynomial{n}(\kappa_1,\ldots,\kappa_n),
 \qquad
 \rho_n=\ExponentialCompleteBellPolynomial{n}(-\kappa_1,\ldots,-\kappa_n).
...
\texorpdfstring{$\FabiusBounded$ and $\RvachevUp$}{bounded Fabius and up}
themselves}
...
No nonzero polynomial-coefficient linear ODE can hold for $\FabiusBounded$ on an interval
...
singular.  The function $\FabiusBounded$ is nowhere analytic on $(0,1)$, so such an ODE is
...
\begin{equation}
 a_n(c):=\FabiusBounded(c2^{-n}).
 \label{eq:deep-sample}
\end{equation}
For all sufficiently large $n$, the argument lies in $(0,1/2]$.  Integrating
\eqref{eq:fabius-diff} from $0$ to $x$ and using $\FabiusBounded(0)=0$ gives the exact
Volterra identity
\begin{equation}
 \FabiusBounded(x)=\int_0^{2x}\FabiusBounded(u)\Differential u,
...
\[
 a_{n+1}(c)=\int_0^{c2^{-n}}\FabiusBounded(u)\Differential u.
\]
Since $\FabiusBounded$ is increasing and positive on $(0,1)$, the integral is positive and
at most $c2^{-n}\FabiusBounded(c2^{-n})$.  Iterating this estimate proves
\eqref{eq:sample-ratio-iterated}.
\end{proof}

\begin{theorem}[No P-recursion for deep dyadic samples]
\label{thm:deep-samples-not-P}
For every $c>0$, the sequence $(\FabiusBounded(c2^{-n}))_{n\ge0}$ is not P-recursive.  In
particular, the exact rational sequence
\begin{equation}
 \boxed{\bigl(\FabiusBounded(2^{-n})\bigr)_{n\ge0}}
...
Arias de Reyna proved that $\FabiusBounded$ takes rational values at dyadic arguments and
...
\begin{equation}
 \FabiusBounded^{(m)}(x)=2^{m(m+1)/2}\FabiusBounded(2^mx).
...
\begin{align*}
 \FabiusBounded^{(m+1)}(x)
 &=2^{m(m+1)/2}2^mF'(2^mx)\\
 &=2^{m(m+1)/2+m+1}\FabiusBounded(2^{m+1}x)\\
 &=2^{(m+1)(m+2)/2}\FabiusBounded(2^{m+1}x).
\end{align*}
\end{proof}
...
\begin{equation}
 \bigl(\FabiusBounded^{(m)}(c2^{-n})\bigr)_{n\ge0}
...
\[
 \FabiusBounded^{(m)}(c2^{-n})
 =2^{m(m+1)/2}\FabiusBounded(c2^{-(n-m)}).
\]
...
\begin{equation}
 A_c(z):=\sum_{n=0}^{\infty}\FabiusBounded(c2^{-n})z^n,
 \label{eq:sample-OGF}
\end{equation}
...
estimate.  The repository's corrected endpoint transseries for $\FabiusBounded(x)$ as
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{data/frequency_separation.png}
...
\begin{equation}
 b_n(c):=\FabiusQuantile(c2^{-n})
 \label{eq:inverse-sample}
\end{equation}
for sufficiently large $n$ is not P-recursive.
\end{conjecture}

Unlike $\FabiusBounded(c2^{-n})$, the inverse sequence decays only stretched-exponentially
...
Insert the all-orders endpoint expansion of $\FabiusBounded(c2^{-n})$ into
\begin{equation}
 A_c(r)=\sum_{n\ge0}\FabiusBounded(c2^{-n})r^n
...
This would create a new duality between the endpoint asymptotics of $\FabiusBounded$ and the
...
Field escape, \cref{thm:field-escape}
 & Residues $1+\TwoAdicValuation(m)$; rational Laurent expansion at $w=1$.\\
