...
\nolinkurl{Analysis/FabiusFunction/docs} at repository commit \RepoCommit.  Its purpose is
...
Define $\GeometricBernoulliPolynomial{n}{q}(x)$ by
\begin{equation}
 \frac{e^{xt}}{M_q(t)}
 =\prod_{k=0}^{\infty}\frac{aq^kt}{e^{aq^kt}-1}\,e^{xt}
 =\sum_{n=0}^{\infty}\GeometricBernoulliPolynomial{n}{q}(x)\frac{t^n}{n!}.
...
The polynomials $\GeometricBernoulliPolynomial{n}{q}$ form an Appell sequence and satisfy
\begin{align}
 \frac{\Differential}{\Differential x}\GeometricBernoulliPolynomial{n}{q}(x)&=n\GeometricBernoulliPolynomial{n-1}{q}(x),
 \label{eq:appell}\tag{A}\\
 \Expectation\,\GeometricBernoulliPolynomial{n}{q}(x+V_q)&=x^n,
 \label{eq:averaging-inverse}\tag{B}\\
 \GeometricBernoulliPolynomial{n}{q}(x+a)-\GeometricBernoulliPolynomial{n}{q}(x)
 &=anq^{n-1}\GeometricBernoulliPolynomial{n-1}{q}(x/q),
 \label{eq:q-difference-Bernoulli}\tag{C}\\
 \GeometricBernoulliPolynomial{n}{q}(1-x)&=(-1)^n\GeometricBernoulliPolynomial{n}{q}(x).
 \label{eq:Bernoulli-reflection}\tag{D}
\end{align}
As $q\downarrow0$, $\GeometricBernoulliPolynomial{n}{q}(x)$ tends coefficientwise to the classical Bernoulli
polynomial $B_n(x)$.
\end{theorem}
...
\begin{align}
 \GeometricBernoulliPolynomial{0}{q}(x)&=1,\\
 \GeometricBernoulliPolynomial{1}{q}(x)&=y,\\
 \GeometricBernoulliPolynomial{2}{q}(x)&=y^2-\sigma_q^2,\\
 \GeometricBernoulliPolynomial{3}{q}(x)&=y^3-3\sigma_q^2y,\\
 \GeometricBernoulliPolynomial{4}{q}(x)&=y^4-6\sigma_q^2y^2+3\sigma_q^4-\kappa_{4,q}.
\end{align}
More generally, $\GeometricBernoulliPolynomial{n}{q}(x)$ is the complete Bell polynomial whose first argument
...
\begin{equation}
 \sum_{j=0}^{n}\binom njm_j(q)\GeometricBernoulliPolynomial{n-j}{q}(x)=x^n.
...
\begin{remark}
For $b=2$, the multiplicity is $\TwoAdicValuation(m)+1$, so the zero divisor of the up-function's
...
\[
 |a_n|\le C^k n^{-k+O(1)}\max_{j\le k}\|h_b^{(j)}\|;
\]
the exact polynomial factors in
...
\]
has inverse on polynomials given by $T_q^{-1}x^n=\GeometricBernoulliPolynomial{n}{q}(x)$.  The fixed-point
decomposition factors it as an infinite product of uniform-averaging operators.

\begin{problem}
Develop the spectral and zero geometry of $\GeometricBernoulliPolynomial{n}{q}$.  Questions include the
...
For each fixed $q\in(0,1)$, all real zeros of $\GeometricBernoulliPolynomial{n}{q}$ are simple except for the
