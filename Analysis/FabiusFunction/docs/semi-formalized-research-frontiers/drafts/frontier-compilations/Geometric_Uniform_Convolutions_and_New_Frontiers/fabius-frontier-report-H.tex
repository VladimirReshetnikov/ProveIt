...
\begin{align}
 \widehat u_q(t)&=\Expectation e^{itX_q}
...
\end{equation}
Hence $|t|^m\widehat u_q(t)\in L^1(\RealNumbers)$ for every $m$.  Fourier inversion gives a
...
\end{equation}
At $q=1/2$, if $\ThueMorseSignSymbol_j=(-1)^{s_2(j)}$ is the $\{\pm1\}$ Thue--Morse sequence, this
reduces to
\begin{equation}
...
For the ordinary Thue--Morse signs $\ThueMorseSignSymbol_j=(-1)^{s_2(j)}$,
\begin{equation}
