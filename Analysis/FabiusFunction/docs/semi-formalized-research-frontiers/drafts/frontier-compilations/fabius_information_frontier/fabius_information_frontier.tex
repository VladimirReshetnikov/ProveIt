...
\[
 \mathcal J(\Up)
 =\int_0^1\frac{F'(x)^2}{F(x)(1-F(x))}\dd x
 =\int_0^1\frac{\dd y}{y(1-y)(F^{-1})'(y)}.
\]
...
\item the existing global entropy identity
      $\int_0^1\log (F^{-1})'(y)\dd y=h(F')$ and entropy laws for derivative-energy
...
thus $f_q\in C_c^\infty(\R)$ and is flat at the support endpoints.  These regularity
...
Since $\Var(U_j)=1/3$,
\begin{equation}
 \sigma_q^2:=\Var(X_q)=\frac{(1-q)^2}{3}\sum_{j\ge0}q^{2j}
 =\frac{1-q}{3(1+q)}.
 \label{eq:variance}
\end{equation}
...
Equivalently,
\begin{equation}
 \Up(x)=F(1-\abs{x}),\qquad \abs{x}\le1.
 \label{eq:up-F}
\end{equation}
Let
\begin{equation}
...
\begin{equation}
 ...
 \label{eq:dyadic-var}
\end{equation}
The report will move repeatedly among the centered density $\Up$, the bounded CDF
...
where $Q_q=F_q^{-1}$, and
\begin{equation}
 \E\bigl[\e^{\ii tX_q}\mid S_{q,m}=s\bigr]
 =\e^{\ii ts}\phi_q(q^mt).
...
\begin{equation}
 \E\bigl[(X_q-s)^n\mid S_{q,m}=s\bigr]
...
$\alpha\ne1$, and let
\[
 \mathcal J(f_q)=\int_{-1}^{1}\frac{f_q'(x)^2}{f_q(x)}\dd x,
 \qquad
 V(f_q)=\Var[-\log f_q(X_q)].
\]
Then
\begin{align}
...
The audited repository contains Legendre expansions of $\Up$ and inverse-moment
...
at $q=1/2$ the existing even Legendre expansion of $\Up$ becomes an exact local
...
For every $m\ge1$,
\begin{equation}
 \E[X_q\mid S_{q,m}]=S_{q,m},
 \qquad
 \Var(X_q\mid S_{q,m})=q^{2m}\sigma_q^2,
...
\[
 \Var(S_{q,m})=\sigma_q^2-q^{2m}\sigma_q^2,
 \qquad
 \Cov(X_q,S_{q,m})=\Var(S_{q,m}),
\]
...
\begin{equation}
 ...
 \qquad
 D_m:=\E(X_q-\widehat X_m)^2=q^{2m}\sigma_q^2.
 \label{eq:RD-points}
\end{equation}
Therefore
\begin{equation}
 D_m=\sigma_q^2\e^{-2R_m}.
...
\begin{equation}
 \delta_q
 :=\frac12\log(2\pi\e\sigma_q^2)-h(X_q)
 =\KL\!\left(f_q\,\middle\|\,
...
squared error gives
\[
 R_{X_q}(D)\ge h(X_q)-\frac12\log(2\pi\e D).
\]
...
\[
 R_{X_q}(D)=h(X_q)-\frac12\log(2\pi\e D)+o(1)
 \qquad(D\downarrow0).
\]
...
For every $0<q<1$,
\begin{equation}
 0<\delta_q\le\frac12\log\frac{\pi\e}{6}.
...
\[
 N(X_q):=\frac{\e^{2h(X_q)}}{2\pi\e}
 \ge\sum_{j\ge0}(1-q)^2q^{2j}\frac{2}{\pi\e}
 =\frac{2(1-q)}{\pi\e(1+q)}.
\]
...
$D_m\e^{-2\delta_q}$.  Hence the exact geometric prefix has multiplicative
high-resolution loss $\e^{2\delta_q}$.  Numerically, for $q=1/2$ this factor is
...
\begin{equation}
 \widehat b_{q,m}(t)
 =\prod_{j=0}^{m-1}\sinc\bigl((1-q)q^jt\bigr)
...
\begin{equation}
 ...
 \pospart{s+A_m-\frac{j}{2^{m-1}}}^{m-1}.}
 \label{eq:TM-spline}
\end{equation}
Its Fourier transform is
\begin{equation}
 \widehat b_m(t)=\prod_{k=1}^{m}\sinc(t/2^k).
...
\[
 ...
 \pospart{s+\sum_ka_k-2\sum_{k\in E}a_k}^{m-1}.
\]
For $a_k=2^{-k}$,
\[
...
\label{thm:KL-TM}
For the dyadic law $X\sim\Up$,
\begin{equation}
 p_{S_m,X}(s,x)
 =b_m(s)\,2^m\Up\bigl(2^m(x-s)\bigr).
 \label{eq:joint-TM-up}
\end{equation}
Consequently,
\begin{equation}
 \boxed{
 \iint_{\R^2} b_m(s)\,2^m\Up\bigl(2^m(x-s)\bigr)
 \log\frac{2^m\Up(2^m(x-s))}{\Up(x)}\dd x\dd s
 =m\log2.}
 \label{eq:KL-TM}
\end{equation}
...
\begin{equation}
 \E\bigl[\log f_q(X_q^{(m)})-\log f_q(X_q)\bigr]=0.
...
\begin{equation}
 \Var\bigl(\imath_m(X_q;S_{q,m})\bigr)\longrightarrow2V_q.
...
Let
\begin{equation}
 h_{q,m}:=h(S_{q,m})=-\int b_{q,m}(x)\log b_{q,m}(x)\dd x.
...
\begin{equation}
 \mathcal J(f)=\int\frac{f'(x)^2}{f(x)}\dd x.
 \label{eq:Fisher-def}
\end{equation}
...
$\E[X_q(f_q'/f_q)(X_q)]=-1$.  Cauchy--Schwarz yields
...
$\mathcal J(\Up)/18$.  The next identity places it directly in the Fabius and
...
\begin{align}
 \mathcal J(\Up)
 &=8\int_0^1\frac{\Up(2x-1)^2}{\Up(x)}\dd x,
 \label{eq:Fisher-up-dilation}\\
 &=\int_0^1\frac{F'(x)^2}{F(x)(1-F(x))}\dd x,
 \label{eq:Fisher-F}\\
 &=\int_0^1\frac{\dd y}{y(1-y)G'(y)}.
 \label{eq:Fisher-G}
\end{align}
...
\[
 \mathcal J(\Up)
 =2\int_0^1\frac{F'(x)^2}{1-F(x)}\dd x
 =8\int_0^1\frac{\Up(2x-1)^2}{\Up(x)}\dd x.
\]
By $x\mapsto1-x$ and $F(1-x)=1-F(x)$,
\[
 \int_0^1\frac{F'(x)^2}{1-F(x)}\dd x
 =\int_0^1\frac{F'(x)^2}{F(x)}\dd x.
\]
Adding the two equal integrals gives \eqref{eq:Fisher-F}.  Finally substitute
$y=F(x)$, so $x=G(y)$, $F'(x)=1/G'(y)$, and
$\dd x=G'(y)\dd y$.  This gives \eqref{eq:Fisher-G}.  Finiteness follows from the
...
\begin{equation}
 h(\Up)-h(S_m)
 =\frac{\mathcal J(\Up)}{18}\,4^{-m}+o(4^{-m}).
 \label{eq:dyadic-fisher-gap}
\end{equation}
Equivalently,
\begin{equation}
 \lim_{m\to\infty}4^m\bigl[h(\Up)-h(S_m)\bigr]
 =\frac1{18}\int_0^1\frac{\dd y}{y(1-y)G'(y)}.
...
\end{equation}
For the centered Rvachev density, let $\mathscr H_{\Up}(y)$ denote the entropy
...
\begin{proof}
Write $u=\e^{-t}$.  Equation \eqref{eq:tail-entropy-quantile} becomes
\[
 \mathscr H_F(y)=\int_T^\infty \e^{-t}
 \left(t-\sqrt{2Lt}-1-\frac L2+o(1)\right)\dd t.
\]
Watson integration at the lower endpoint gives
\[
 \int_T^\infty\e^{-t}t\dd t=y(T+1),
 \qquad
 \int_T^\infty\e^{-t}\sqrt{2Lt}\dd t
...
$X\sim\Up$ and $Y=(X+1)/2$, then $F'$ is the density of $Y$ and
$\Up(x)=\tfrac12F'((x+1)/2)$.  Therefore the centered surprisal exceeds the
...
\begin{equation}
 \int_0^y\bigl[\log G'(u)\bigr]^r\dd u
...
\begin{equation}
 \int_0^y\bigl[\log G'(u)\bigr]^2\dd u
...
$\log G'(\e^{-t})=t-\sqrt{2Lt}+O(1)$.  Raise this to the fixed power $r$, retain
...
\begin{equation}
 \lim_{q\downarrow0}\delta_q=\frac12\log\frac{\pi\e}{6},
 \qquad
...
\begin{align}
 h(\Up)&=0.297039912858918,\label{eq:num-hup}\\
 \mathcal J(\Up)&=11.800710562320129,\label{eq:num-Jup}\\
 V(\Up)&=0.259694517369981,\label{eq:num-Vup}\\
...
\begin{equation}
 \frac{\mathcal J(\Up)}{18}=0.655595031240007.
...
\begin{tabular}{@{}rrrrr@{}}
\toprule
$m$ & $h(S_m)$ & $h(\Up)-h(S_m)$ & $4^m[h(\Up)-h(S_m)]$ & exact MMSE\\
\midrule
...
Fisher coefficient $\mathcal J(\Up)/18$.  Depths beyond eight are retained in the
...
\begin{equation}
 \sup_{t\in K}
 \left|\log\E\e^{t(\imath_m-R_m)}-\Lambda_q(t)\right|
 =O_q(q^m),
...
\begin{equation}
 (\mathcal B_mg)(s)
 :=\E\left[g\!\left(\frac{X_q-S_{q,m}}{q^m}\right)
...
\begin{equation}
 (\mathcal C_mg)(s)=\E[g(X_q)\mid S_{q,m}=s]
 =\int_{-1}^{1}g(s+q^mz)f_q(z)\dd z.
 \label{eq:Cm}
\end{equation}
...
\begin{equation}
 ...
 =\frac1{\alpha-1}
 \log\iint b_m(s)
 \left(\frac{2^m\Up(2^m(x-s))}{\Up(x)}\right)^{\alpha}
 \Up(x)\dd x\dd s.
 \label{eq:TM-Renyi}
\end{equation}
...
\end{equation}
with invertible contraction $A\in\R^{d\times d}$ and an absolutely continuous
...
\[
 N(a_jU_j)=a_j^2\frac{\e^{2\log2}}{2\pi\e}
 =a_j^2\frac{2}{\pi\e}.
\]
The entropy-power inequality gives
\[
 N(X_q)\ge\frac{2}{\pi\e}\sum_{j\ge0}a_j^2
 =\frac{2(1-q)}{\pi\e(1+q)}.
\]
Thus
\[
 h(X_q)\ge\frac12\log\left(2\pi\e\,
 \frac{2(1-q)}{\pi\e(1+q)}\right)
 =\log2+\frac12\log\frac{1-q}{1+q}.
\]
Subtracting from the matching Gaussian entropy
\[
 \frac12\log\left(2\pi\e\frac{1-q}{3(1+q)}\right)
\]
gives $\delta_q\le\tfrac12\log(\pi\e/6)$.

\subsection{Exact information and correlation in one line}

The prefix variance and tail variance split orthogonally:
\[
 \Var(S_{q,m})=\sigma_q^2(1-q^{2m}),
 \qquad
 \Var(q^mX_q^{(m)})=\sigma_q^2q^{2m}.
\]
Therefore
\[
 1-\Corr(X_q,S_{q,m})^2
 =\frac{\Var(X_q-S_{q,m})}{\Var(X_q)}=q^{2m},
\]
...
\[
 ...
 =\frac12\E_\gamma u^2-\frac16\E_\gamma u^3+O(\E|u|^4).
\]
