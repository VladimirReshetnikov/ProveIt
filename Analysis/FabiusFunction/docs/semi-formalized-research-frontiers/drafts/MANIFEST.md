...
901 facade-reachable source modules and 11,419 unique public declarations, with
no missing module headers and no missing documentation comments; the checked
baseline records this zero-gap state.

Four publication pairs now have synchronized final-source receipts.  The
primary exposition is a 694,350-byte TeX source (SHA-256
...
The Lean walkthrough is a 456,855-byte TeX source (SHA-256
...
The canonical frontier is an 808,185-byte TeX source (SHA-256
...
Each pair completed an exact clean final three-pass cycle and its recorded
publication gates.
...
provenance receipts for their named historical sources only.  Direct SHA-256
values in this manifest are artifact receipts, not operational
`SHA256SUMS*` verification claims.
...
The canonical q-series synthesis now has the synchronized current receipt
recorded above.  An earlier source checkpoint has 14,516 lines, 682,778 bytes,
and SHA-256
...
Later sources added the Bell-block and MacMahon q-Catalan notation successor
and further formalization crosswalks before the current synchronized build.
The still earlier 348-page artifact checkpoint also remains historical
evidence:
its PDF SHA-256 is
...
results: 166 Exact / 90 Partial / 18 None / 8 N/A interface rows.  The current
395-page receipt above closes the later source-only interval. The comb synthesis also
...
The consolidated geometric q-frontier now also has current-source parity.  Its
808,185-byte TeX source has SHA-256
`7dd140370a0ac68522364a83a3c6423df93570741eafaef2ee8c1fac17670e2f`;
an exact clean final three-pass cycle produced the 260-page, 1,877,159-byte A4
PDF with SHA-256
`9d38ab9d43befd6e26fd06ab9680b4a761365fb9d8a9f0de18489c243bd62d3e`,
and its recorded publication gates are clean.  The earlier 238-page PDF and
the 16,274- and 16,369-line source checkpoints remain historical provenance
only.  The current source includes the exact identifiability and outer
q-Pochhammer normal-convergence crosswalks; its named remaining Partial claims
are unchanged.  This current receipt supersedes lower table prose that still
describes the consolidated volume as source-only or unsynchronized.  Direct
SHA-256 values are artifact receipts, not operational `SHA256SUMS*`
verification claims; immutable arrival fingerprints remain historical
provenance.
...
| `special-function-inversion/Sequence_Inversion_Transseries/` | **Canonical consolidation of eighteen articles across six subject chapters.** *Asymptotic Transseries and Inversion of Four Combinatorial Sequences: Rooted Trees, the Double Factorial, the Partition Numbers, and the Swing Factorial* — 15,093-line/702,309-byte source (SHA-256 `ca8036ea1ad995f37155d20aaf4e5f65f85a3dc3700b8ccce252f5cd87855d53`) and the synchronized 183-page A4 PDF (2,735,461 bytes; SHA-256 `bbe95ae95a135615f9d18390e4b5c37946f0ddc89c7379605b8077c034a59bff`); 212 named results, 207 proof environments, 1,189 labels, no dangling references. Chapter~0 states the shared apparatus once (exponential--power model and axiomatized dominant core, Bell calculus, Lagrange--Bürmann, the Lagrange fixed point and perturbed inversion around an exactly invertible core, the Lambert carrier and branch rule, all-orders reversion, flattening, the three inverse objects and the staircase theorem, backward error, remainder transport, optimal truncation); the six subject chapters cite it rather than restating it. Repairs to the sources are boxed in the text and listed in its Appendix~B; the sharpest is that five source articles derive the Lagrange fixed-point formula from Lagrange--Bürmann in a case where the required invertibility fails, so the identity — on which every reversion in the volume depends — is reproved here by universality. Every printed coefficient and decimal was recomputed independently; the documented inherited-sector exception is corrected in Chapter~1. Comparison against the wider corpus and Lean crosswalking are deferred | consolidation of eighteen filed source articles, 2026-09-03; the sources are retained pending a residue audit |
