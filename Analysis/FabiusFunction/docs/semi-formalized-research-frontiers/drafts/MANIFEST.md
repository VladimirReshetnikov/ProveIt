...
source modules and 11,622 public declarations, with no missing module headers or
declaration comments. The incoming branch's 925/11,619 count is the historical
pre-compatibility checkpoint; preserving three unique local
`QBinomialTheoremInfinite` wrappers gives the current total.

This merge changed the source closure of exactly ten retained publications.
All ten have accepted merged-source PDFs; their compact root / graph-aggregate /
passes / PDF / final-log receipts are:
...
and subset, Libertinus has 11 rows, Type-3 has zero rows, and checksum
basenames and PDF/source references are absent. The dyadic-chaos PDF is not in
this set.
Other already-unsynchronized retained publications below keep their existing
...
The q-series synthesis has the accepted current receipt above and retains its
earlier artifacts explicitly as history. At pre-upstream
...
historical and are superseded by the accepted 401-page current receipt above.
...
the 389-page predecessor are explicit history. The accepted 401-page current
receipt is recorded above.
...
`2d434eec` whole-root receipt recorded below and an accepted current 404-page
merged-source receipt recorded above. Cyclotomic has a
...
accepted post-merge q-series PDF now renders the current source graph. The
...
| `Thue_Morse_Atlas_and_Frontiers/` | *The Thue–Morse Sequence: Formula Atlas and Fabius–Rvachev Frontier Results* (accepted current receipt: root `10557L/482022B/8cef828c3d92a0017e22463ac90878a5e3a98e1138059d5f4793d47c04a88404`; two-file aggregate `10841L/493857B/79a43711c6989336166d6b2ed1faa306cc985b8d70557025ab139791d455723c`; passes `139/144/144`; PDF `144pp/1740015B/deb63fe66fc8f020bb072acbc4301e9b7c9f0559b165cbae2c076f261405c5be`; log `1503L/59417B/1219dbc87bc4f9920c40b24659182220b14e7908a45a9eb33d0f19390148c64b`; all recorded gates clean; former 143-page PDF retained as history) — consolidation (2026-08-28) of the former `Thue_Morse_Formula_Atlas/` (*A Unified Formula Atlas for the Thue–Morse Sequence*) and `Fabius_Rvachev_Thue_Morse_Frontier_Results/` (*A Finite-Block Calculus for the Fabius–Rvachev–Thue–Morse System*, heavily Lean-crosswalked); assets under `assets/`, provenance with SHA-256 in the document | absorbed drafts deleted; git history is the archive |
...
current receipts above identify their current PDFs; the historical `2d434eec`
receipts below and the older
...
historical 924/11,615 checkpoint, the two
`ProbabilityLaplaceMoments.lean` theorems give 924/11,617, and the 1+1
`RvachevLegendreBiorthogonality.lean` leaf gives the incoming 925/11,619
checkpoint. Preserving all five local `QBinomialTheoremInfinite` additions,
with the incoming fixed-column module reusing the two upstream names it had
also supplied, contributes three unique compatibility wrappers, giving the live
925/11,622 semantic union. In that union, the subsequent
...
925/11,622; the RatFunc-only 924/11,615 and incoming pre-compatibility
925/11,619 counts remain historical checkpoints. The q forward totals are 181 Exact / 78
...
It is a fully reviewed historical checkpoint superseded by the accepted
137-page merged-source receipt above. The current 23-input source-only closure
digest is
