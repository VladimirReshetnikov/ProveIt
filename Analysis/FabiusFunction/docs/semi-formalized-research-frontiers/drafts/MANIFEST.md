...
**Current post-merge working-tree checkpoint (2026-09-04).** The authoritative
live facade census and zero-gap result are recorded in the
[documentation audit](../../DOCUMENTATION_AUDIT.md#findings) and pinned in
`docs/doc_audit_baseline.json`; this manifest does not duplicate their live
numeric receipt.  The frozen upstream 957/11,920 inventory, the retained public q-Pochhammer
bridge's 957/11,921 merge-union checkpoint, and
`Fabius.exists_eq_in_residual_interval`'s 957/11,922 residual-certificate
checkpoint are historical waypoints.  Four Bell declarations, nine
repeated-differential declarations, ten Cauchy declarations, and the
reverse-row Stirling theorem are included in that authoritative receipt.  The earlier 923/11,610,
931/11,685, 933/11,695, and 934/11,709 counts likewise remain historical
checkpoints.
The rigorous q ledger is 181 Exact / 79 Partial / 14 None / 8 interface rows,
and its 547-row source concordance is 103 Lean-proved / 375 human-frontier /
60 non-applicable / 9 conjecture.  The inverse ledger is 57/88/10/15/24 and
the comb ledger remains 7/159/20/30/16.

The current publication check contains the 26 roots below: 14 require fresh
build cycles and 12 retain verified pairs from `69b447f7c`.  Any earlier pair for one of those roots is a historical
receipt unless that checkpoint explicitly re-establishes parity.  Receipts
outside the batch remain valid only for their named source states.
Package-wide checksum ledgers are retired and are neither generated nor used
as validation gates; direct historical artifact digests and purpose-specific
provenance records remain permitted.
...
In the driver column, `DOC` abbreviates `Analysis/FabiusFunction/docs` and
`DRAFT` abbreviates
`DOC/semi-formalized-research-frontiers/drafts`.

| Driver | Final pages | Final PDF bytes | Gate result |
| --- | ---: | ---: | --- |
| `DRAFT/exponents-and-q-series/geometric_q_fabius_frontiers/geometric_q_fabius_frontiers.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DRAFT/exponents-and-q-series/q_pochhammer_q_binomial_monograph/q_pochhammer_q_binomial_monograph.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DRAFT/thue-morse/Thue_Morse_Atlas_and_Frontiers/Thue_Morse_Atlas_and_Frontiers.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DOC/Fabius_Function_and_Rvachev_Up/Fabius_Function_and_Rvachev_Up.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DOC/fabius_lean_walkthrough/fabius_lean_walkthrough.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DRAFT/frontier-compilations/Frontier_Compilations/Frontier_Compilations.tex` | 275 | 2,790,721 | Retained verified pair from `69b447f7c`; source/input/PDF unchanged |
| `DRAFT/integration-and-transforms/Integration_and_Transform_Frontiers/Integration_and_Transform_Frontiers.tex` | 377 | 3,292,594 | Retained verified pair from `69b447f7c`; source/input/PDF unchanged |
| `DRAFT/inverse-and-sampling/Inverse_Fabius_Analyticity_Asymptotics_and_Computability/inverse_fabius_theory.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DRAFT/inverse-and-sampling/comb-interpolation/comb_interpolation_synthesis/comb_interpolation_synthesis.tex` | 160 | 2,468,000 | Retained verified pair from `69b447f7c`; source/input/PDF unchanged |
| `DRAFT/inverse-and-sampling/fabius_information_frontier/fabius_information_frontier.tex` | 29 | 790,802 | Retained verified pair from `69b447f7c`; source/input/PDF unchanged |
| `DRAFT/lambert-w/Lambert_W_Guide/Lambert_W_Guide.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DRAFT/representations/Matrix_Dilated_Fabius_Rvachev_Frontier_Report/matrix_dilated_fabius_rvachev.tex` | 29 | 878,932 | Retained verified pair from `69b447f7c`; source/input/PDF unchanged |
| `DRAFT/representations/Representation_Frontiers/Representation_Frontiers.tex` | 301 | 3,609,120 | Retained verified pair from `69b447f7c`; source/input/PDF unchanged |
| `DRAFT/representations/Up_Polynomial_Synthesis/Up_Polynomial_Synthesis.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DRAFT/representations/common_digit_fabius_zonoids_frontier_report/common_digit_fabius_zonoids.tex` | 36 | 1,171,153 | Retained verified pair from `69b447f7c`; source/input/PDF unchanged |
| `DRAFT/representations/fabius_dyadic_chaos_frontier/fabius_dyadic_chaos_frontiers.tex` | 40 | 821,194 | Retained verified pair from `69b447f7c`; source/input/PDF unchanged |
| `DRAFT/rvachev_up_fourier_decay/Rvachev_Up_Fourier_Decay-2/Rvachev_Up_Fourier_Decay-2.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DRAFT/rvachev_up_fourier_decay/Rvachev_Up_Fourier_Decay/Rvachev_Up_Fourier_Decay.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DRAFT/series-and-transseries/Transseries_And_Inversion/transseries_and_inversion.tex` | *(pending)* | *(pending)* | *(pending)* |
| `DRAFT/spectra-and-arithmetic/Dyadic_Radon_Profiles_Fabius_Rvachev_2026-08-30/dyadic_radon_profiles_fabius_rvachev.tex` | 29 | 998,017 | Retained verified pair from `69b447f7c`; source/input/PDF unchanged |
...
The historical `69b447f7c` receipts below use `pages/bytes` pass tuples. All
ten roots were frozen and built in exactly three serial halt-on-error passes
from absent sidecars. They remain exact receipts for that source checkpoint,
not claims of parity after the current merge.
...
At the historical `b899` checkpoint, the B2--B6 and B8--B11 roots had matching
synchronized receipts.  The B7 row below is the preceding historical Up
checkpoint and was superseded at that checkpoint by root 8 above. All passed the same
...
forbidden-basename gates stated above.  Their exact recorded rows are:
...
158-page artifacts were superseded by the later historical 137- and 160-page
`b899` receipts above. Earlier 167-page
...
rigorous forward status ledger covers 282 labelled results:
181 Exact / 79 Partial / 14 None / 8 N/A interface rows; q-Lucas remains
Partial. The
...
The fifteen archives received on 2026-09-04 were filed the same day by a quick
archival intake: six Thue--Morse articles to `thue-morse/`, three
...
found no discrepancy.  Later on 2026-09-05 the companion was folded into
the atlas as Part III: its prefix chapter was replaced by references to
Part I's exact-prefix, iterated-prefix, terminal-zero-run and
signed-reversal theorems, its notation was moved onto the notation
catalogue (with Part I's `S^{(k)}_n` for the iterated prefix and two
declared local hats for the Sheffer normalizations), the Fabius section
now cites the approximation and convergence declarations of
`ThueMorseApproximation.lean` by name, and the verification bundle and
figures moved to `Thue_Morse_Atlas_and_Frontiers/assets/diagonal-laws/`.
The notation catalogue gained entries for the iterated prefixes, the
summation table, the diagonal polynomials and the block polynomials, two
accent-register rows, three collision-register amendments and two Lean
crosswalk rows.  The companion directory `Thue_Morse_Diagonal_Laws/` is
deleted (Git history retains it).  The folded atlas builds with zero
undefined references, zero overfull boxes and zero strict notation-audit
findings.  No Lean proof was added.
...
`Thue_Morse_Frontier_Deductions/`, and the six arrival directories were
deleted (Git history retains them).  The two that treated the same object,
`Thue_Morse_Dyadic_Completion/` and `thue_morse_research_article/` (the
completion `K(t) = P(t)Phi(t)` with `K(2t) = K(t)/t` and its Mellin and
Dirichlet consequences), are fused into one part in one notation, with the
shared theorems stated once and the second report's uniform remainder,
classification and gamma-tower uniqueness added; the other four overlapped
only in the preliminaries (signs, finite product, Prouhet cancellation, the
uniform-sum transform, Bernoulli and Bell expansions), which the volume
states once in a common chapter.  The six exact and high-precision
verification programs are retained unchanged under the package's
`verification/`.  The atlas was not modified: it is under concurrent editing,
so folding the companion into it is a recorded follow-up.  The cross-group
overlap noted at intake (the exact-polynomiality in `4^{-m}` of
`Thue_Morse_Research/`, now Part IV, against the dyadic-value volume under
`inverse-and-sampling/dyadic-up-extraction/`) is recorded in both volumes
and not acted on.  No Lean crosswalking was done.
...
70-page Guide receipt is historical; the later historical `b899` root-7 receipt
...
`ThueMorseNewmanSelfSimilarity.lean`. This gives the historical 934/11,709 census.
...
`cor:qbinom-inversion-law` Partial. The current semantic-union facade census is
computed by `scripts/doc_audit.py` and pinned in
`docs/doc_audit_baseline.json`; the frozen upstream target has 957/11,920, the retained
unconditional public `complexQPochhammerInf_eq_qPochhammerInfIn` bridge gives
the 957/11,921 merge-union checkpoint, and the residual-existence certificate
adds the current final declaration. The historical dyadic/finite-prefix census is 933/11,695 and the
...
checkpoints. The rigorous q forward totals are 181 Exact / 79 Partial / 14 None / 8
N/A; q-Lucas remains Partial. The relevant Dyadic Gaussian--Thue--Morse chapter is 13/43/0/0, and the
...
volume (`combinatorial_transseries_and_inverses/`,
`combinatorial_transseries_extension/`,
`combinatorial_transseries_extension-2/`).  Each said in its own abstract that
it extended the volume's methods to complementary families rather than
restating them.  The comparison was made the same day: the first two overlap
on most of their families and on all of their machinery (the balanced gamma
engine, the Stirling-column inverse, the Motzkin endpoint blocks, the
involution saddles and their unbounded inverse drift, the zigzag Dirichlet
sectors, the necklace divisor sums), and the third overlaps with them only on
central Gaussian binomial coefficients and on necklaces; every shared formula
was compared symbol by symbol and no discrepancy was found.  They were merged
editorially into one companion volume, `Combinatorial_Transseries_Inverses/`,
with the foundations and the three inversion engines stated once, each
family in one chapter, and the three verification programs retained with
their recorded runs.  The three directories and their arrival PDFs were
deleted after a residue audit of every titled result; git history is the
archive.  Nothing of the three is in the canonical volume, whose combinatorial
case studies are disjoint from theirs; the companion is kept beside it
rather than folded in because the canonical volume is under concurrent
formalization edits.

| Directory | Document | Previous path |
| --- | --- | --- |
| `series-and-transseries/Combinatorial_Transseries_Inverses/` | **Canonical companion, consolidation complete:** *Combinatorial Transseries and Their Inverses: Gamma quotients, finite exponential sums, moment sectors, moving saddles, arithmetic sheets, and q-products* -- nine parts, 20 chapters: the four inverse objects and interpolation gauge freedom; the finite partition formulas, phase-coordinate inversion with the all-multiplicative-sector formula in signed Stirling numbers, scaled Lagrange--Buermann reversion, the balanced linear--logarithmic core to all orders, the convergent inverse-transseries theorem for analytic exponential tails with its finite nonrecursive coefficient formula and general-phase derivative form, and the residual certificate; then the families -- balanced gamma quotients (Catalan, Fuss--Catalan, central multinomials, rectangular tableaux) with the `W_{-1}` core; fixed-column Stirling-II and Eulerian numbers (convergent multiexponential inverses) and fixed-cycle Stirling-I numbers (nested logarithms); endpoint moment sequences (Motzkin and trinomial at general exponent, Delannoy, large Schroeder) with exact oscillatory sectors; involutions with two finite saddle formulas, the unbounded `sqrt(X)/log X` inverse drift and the `exp(-2 sqrt x)` lattice; alternating permutations with odd-integer ordered-factorization sectors and prime Euler products; connected labelled graphs with every exponential layer and a range inverse; necklaces, Lyndon words and irreducible polynomials with the finite-grid obstruction, fixed-radical sheets and the two-candidate threshold theorem; the `q`-products of finite-field enumeration (general linear, symplectic, unitary orders, flags, fixed-rank and scaled Gaussian coefficients, `q`-Catalan numbers, Galois numbers with root-lattice theta sectors) and the singular `q -> 1` transition; harmonic and finite-limit inversion; certification, three sets of recorded numerical checks, synthesis; appendices with the coefficient table, the involution coefficient audit, the three Wolfram recipe sets, notation reconciliation, and provenance. 21 theorems, 10 propositions, 4 lemmas, 4 corollaries, every one with a proof; loads `docs/fabius-notation.tex`; labels `ct:`, `t1:`, `t2:`, `t3:`. Source `Combinatorial_Transseries_Inverses.tex`, 5{,}616 lines / 300{,}113 bytes; PDF 127 A4 pages / 1{,}555{,}972 bytes, three passes, 0 errors / undefined references / overfull boxes. | Editorial merge (2026-09-04) of the three 2026-09-04 arrivals `combinatorial_transseries_and_inverses/`, `combinatorial_transseries_extension/`, `combinatorial_transseries_extension-2/` (from `incoming/`); their verification programs and recorded outputs retained under `verification/source1..3/`; absorbed directories and arrival PDFs deleted after a residue audit, git history is the archive, per-source contributions in the volume's provenance chapter |
...
later historical `b899` root-6 receipt above. At that historical checkpoint the driver SHA-256 was
...
| `Matrix_Dilated_Fabius_Rvachev_Frontier_Report/` | *Matrix-Dilated Fabius--Rvachev Laws* (historical synchronized 1,997-line source and 29-page A4 PDF; exact B5 receipt above; with experiment, data, figures, and audits), direct arrival `8a184546747082cbd92ad4675fb61981c6b8c3b6`. Its 27 historical payload hashes reflect seven CSV LF-normalization updates. The self-affine box-spline/zonoid layer remains separate from the common-digit report pending a claim-level crosswalk; manuscript claims do not establish Lean status | `drafts/incoming/Matrix_Dilated_Fabius_Rvachev_Frontier_Report/`; filed here |
