...
**Current artifact checkpoint (2026-09-04).** The merged source audit scans
987 modules and 12,207 explicit public declarations, with no missing module
headers or declaration comments. Every module is facade-reachable. These are
source-inventory results, not a full aggregate-build receipt.
...
Earlier local and incoming branch inventories overlap and are not additive.
Their detailed receipts remain in Git history and the package audit records.
No PDF was rebuilt or checksum ledger regenerated during this integration;
the retained artifacts below are historical unless a package explicitly
records a synchronized source checkpoint.
...
| --- | --- | --- |
| `Combinatorial_Coefficient_Calculus/` | **Canonical, editorial consolidation complete:** *Combinatorial Coefficient Calculus* — the live source contains 207 theorem-like assertions with adjacent proofs and a claim-by-claim formalization register. `UnitSeriesPowerRecurrence.lean` closes the unit-series power recurrence; `OrdinaryBellMultinomial.lean` closes the ordinary-composition multiplicity formula, including degree zero; and `AbelPolynomialSeries.lean` closes the Abel row. The new Lagrange and Raney uniqueness extensions are tracked in `FORMALIZATION_STATUS.md`. The three associahedron rows retain their exact arithmetic scope and remaining geometric obligations. The retained upstream PDF has 208 A4 pages and 2,014,975 bytes; it is a historical render, not a render of the latest source. PDF rebuilding remains deferred by user instruction. | Six-source provenance and disposition records in the package; original non-suffixed arrival was `incoming/Combinatorial_Coefficient_Calculus.zip`; `SOURCE_INVENTORY.csv` records immutable Git locators for all six inputs, including the five retired donors. The mutable `SOURCE_CLOSURE.sha256` ledger is retired and no checksum ledger is required. |
