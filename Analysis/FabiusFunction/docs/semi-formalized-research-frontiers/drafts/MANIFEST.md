...
source modules and 11,884 public declarations, with no missing module headers or
declaration comments. The incoming-only 952/11,881 count is a historical
checkpoint; preserving three unique local `QBinomialTheoremInfinite` wrappers
gives the exact-name-deduplicated current total.
...
exact historical receipts. Up and comb still have current parity. All accepted
current campaign receipts are recorded in the authoritative table below.
All nine campaign roots now have current source/PDF parity; the Transseries
row discloses its remaining layout warnings. The dyadic-chaos PDF is not in
this set. Other
...
### Current post-merge publication receipts

This is the authoritative live source/PDF parity register for the nine roots in
the current rerender campaign. The first-merge tuples above remain immutable
historical provenance. Counts are `lines/bytes/SHA-256`; each aggregate covers
...
PDF was regenerated.  For every affected package, the live-source description
recorded in the thematic checkpoint below supersedes an older inline source
count or any earlier statement that the retained publication PDF renders the
current TeX.  Immutable arrival records and all historical build fingerprints
remain provenance evidence rather than current-source claims.
...
history; its accepted current receipt is in the [authoritative receipt
register](#current-post-merge-publication-receipts). At pre-upstream
...
generated at that source-only checkpoint; its 391-page `2d434eec` artifact and
the 389-page predecessor are explicit history. The accepted 401-page
first-merge receipt is recorded above, and the accepted current receipt is in
the authoritative register linked above.
The comb synthesis also has later chapter-03 notation and
...
it is a validated historical checkpoint superseded by the accepted 160-page
...
changes. The current q-series receipt is accepted in the authoritative register.
The
...
`SHA256SUMS` ledger, where present, was computed by its author on the submitted
bytes, was verified at intake, and is now retired; its bytes remain recoverable
from the filing revision in Git.
...
| --- | --- | --- |
| `Combinatorial_Coefficient_Calculus/` | **Canonical, consolidation complete:** *Combinatorial Coefficient Calculus* — 8,966-line/390,732-byte source and the 174-page A4 PDF built from it in the same run | Six-source provenance, purpose-specific closure, and disposition records remain in the package; original non-suffixed arrival was `incoming/Combinatorial_Coefficient_Calculus.zip` (SHA-256 `a22479ac8f58e1710117af9d0a3f515c7d24ec250548f537520c9f9024f4321a`); the five retired donors are identified in `SOURCE_CLOSURE.sha256` |
...
first-merge receipts above identify historical PDFs for their exact source
snapshots. Both current rerenders are accepted in the authoritative register.
The historical `2d434eec` receipts
below and the older
Parts-I--VII and other artifact
...
census. Later upstream work reaches 952/11,881, and the three unique local
q-binomial compatibility wrappers give the live 952/11,884 census. In the
current semantic union, the subsequent
...
952/11,884; 934/11,709, the dyadic/finite-prefix census 933/11,695, and the
reciprocity census 931/11,685 remain historical, while the RatFunc-only
...
latest retained 391-page `2d434eec` q-series PDF is a historical checkpoint
and does not render this source-only crosswalk; the 389-page PDF is earlier
history.
...
| Directory | Document | Previous path |
| --- | --- | --- |
| `series-and-transseries/Transseries_And_Inversion/` | **Canonical consolidation of the whole `series-and-transseries` group.** *Transseries: the polynomial--logarithmic calculus, series reversal at infinity, and the inversion of rapidly growing functions*. The current source inventory is 55,015 lines / 2,732,554 bytes / SHA-256 `545d4b5ead18831e37a4df0ab7fb6b74a40e1e598144dc7057e8c62aaaa46799`, with 16 parts, 58 chapters, and 3,111 unique labels. Its two-file root-plus-notation closure is 55,299 lines / 2,744,389 bytes / aggregate SHA-256 `499a045ab45fdea7849c888bc8903db0866a2d4ca27eca3db6f62a9db282af30`. The incoming 702-page, 7,172,512-byte A4 PDF (SHA-256 `7cbc430332c78a6f2dcd06513c771e15685d87d3ef3a867f55c2687a8702d067`) remains a structurally validated historical pre-merge checkpoint. The three-pass 704-page current render establishes source/PDF parity; its exact tuple and the retained 114 overfull boxes and 11 duplicate Hyperref targets are recorded in the [authoritative receipt register](#current-post-merge-publication-receipts). The volume merges forty-two articles, and its concordance records the genuine duplicate, strengthening, missing counterpart, and three non-equivalent same-subject comparisons. | Consolidation of forty-two articles filed 2026-09-01 to -03; all replaced source layouts remain recoverable from Git history. |
...
is claimed. The former 134-page, 2,027,726-byte A4 PDF
has SHA-256
`22bc68d855ad04dde9654e9fbd20b3ba7f05a33e3c5df0e5b80bb8991c94b41d`.
It is a fully reviewed historical checkpoint superseded by the later
137-page first-merge receipt above; both are now historical. The retained
23-input source-only closure digest for that later snapshot is
...
gates for its recorded source graph and remains explicit history. The accepted
160-page merged-source receipt above supersedes it. Full numerical replay
