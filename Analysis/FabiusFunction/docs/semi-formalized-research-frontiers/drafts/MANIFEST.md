...
source modules and 11,881 explicit public declarations, with no missing module headers or
...
declarations contribute 94 explicit declarations and give the live
952/11,881 census.
...
transseries modules plus four integer-block declarations give the live
952/11,881 census.
...
952/11,881; the historical 943/11,787, 934/11,709, and dyadic/finite-prefix 933/11,695
...
open: the current source overlay exhausts these eighteen module surfaces and keeps
their remaining boundaries explicit.
...
- `TransseriesWellBased.lean` (0+5 written): `dickson_isPWO`,
  `dickson_antichain_finite`, `dickson_isPWO_pi`, `neumann_isPWO`, and
  `neumann_finite_factorizations`; `to_additive` also generates
  `neumann_add_isPWO` and `neumann_finite_decompositions` outside the lexical
  count.  `q0:lem:dickson` is **Exact**; `q0:lem:neumann` is **Partial** as a
  direct crosswalk because the manuscript's greatest-element/no-increasing
  convention requires an explicit `OrderDual` specialization of Lean's
  `Set.IsPWO` orientation.  No Hahn-series type is constructed.
...
- `TransseriesBlockAntiderivative.lean` (3+12): definitions `blockOperator`,
  ...
  exact, but the compound `plt:lem:mot-block-antiderivative` remains
  **Partial** because the leaf does not construct the Laurent ambient ring,
  kernel/uniqueness package, or every integer-block consequence.
...
- `TransseriesDifferentialBlock.lean` (0+9): `derivation_pow_t`,
  ...
  The abstract integer-exponent block identity is exact when `t` is a unit;
  `plt:prop:dif-block` remains **Partial** because the full Laurent-series,
  general-exponent, uniqueness, and coefficient-field clauses are absent.
...
- `WrightOmega.lean` (1+12): `wrightOmega`, `wrightOmega_pos`,
  ...
  `tendsto_wrightOmega_atTop`.  `plt:prop:mot-omega-basic` is **Partial** only
  because its real-analyticity clause is not formalized.
