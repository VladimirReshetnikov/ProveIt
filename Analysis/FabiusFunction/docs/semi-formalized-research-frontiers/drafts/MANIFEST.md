...
facade-reachable source modules and 11,458 unique public declarations, with no
missing module headers and no missing documentation comments; the checked
baseline records this zero-gap state.  Origin's 11,457-declaration inventory
lacks this tree's retained public q-Pochhammer bridge.  The newest
`GeometricUniformMomentPolynomial.lean` source changes the primary exposition,
Lean walkthrough, canonical frontier, Lambert Guide, geometric-q synthesis,
and canonical q-series synthesis after their earlier renders.  Those historical
checkpoints are preserved below, and all six current source/PDF pairs now have
the synchronized 2026-09-04 receipts recorded after them.  Other packages
retain their own explicit source/PDF status.
...
The synchronized 2026-09-04 publication receipts close the current-source
boundary.  Each pass tuple below is `pages/bytes`.  All six pairs completed
...
The B2--B11 roots have matching synchronized receipts.  They passed the same
...
- B7, Up Polynomial Synthesis: TeX 2,324 lines / 95,757 bytes /
...
catalogue, New Frontiers-2, inverse-computability report, canonical
inverse-theory synthesis, and comb synthesis retain 88-, 41-, 42-, 134-, and
158-page artifacts and still require their own final-source rebuilds where
their package records say so.  Earlier 167-page
...
and twenty-four theorems in all.
...
five theorems), and `QPfaffSaalschutz.lean` (three theorems).  The latter's
surface is exactly `finiteQPochhammerIn_ne_zero_of_le`,
`q_pfaff_saalschutz_term`, and `q_pfaff_saalschutz`.  Its terminating balanced
q-Pfaff--Saalschütz identity is algebraic over a field under nonzero
`q,a,b,c` and the four displayed nonzero finite-product denominators; the term
identity also assumes `k ≤ n`, and no convergence claim is made.
...
results: 165 Exact / 92 Partial / 17 None / 8 interface rows. The
...
provenance theorem retains two auxiliary nonvanishing assumptions. The
395-page receipt predates this source-only merge and remains historical; the
current synchronized 398-page receipt above supersedes it. The
comb synthesis also has later chapter-03 notation and
...
The consolidated geometric q-frontier's synchronized 2026-09-04 receipt names
...
Lambert checkpoint 903/11,448, while the retained public q-Pochhammer bridge
raises that local checkpoint to 903/11,449.  The historical Guide receipt
predates the newest source crosswalk; the synchronized 70-page receipt above
now renders the current source.
...
one-definition/eight-theorem addition gives the semantic-union census
904/11,458 and the q forward totals 165 Exact / 92 Partial / 17 None / 8
interface rows.  The source concordance stays unchanged at 86 Lean-proved /
392 human-proved frontier /
60 N/A / 9 conjectures because its historical-source disposition does not
encode partial Lean coverage.  The intermediate 397-page q-series PDF is a
historical checkpoint; the synchronized 398-page receipt above renders the
current source.
...
purpose-specific 23-input source closure; a fresh three-pass build is still required before the
...
| `fabius_information_frontier/` | *Exact Information Geometry and New Frontiers for the Fabius--Rvachev System* (current synchronized 2,138-line TeX and 29-page A4 PDF; exact B4 receipt above; a 601-line experiment, five data products, and three PDF/PNG figure pairs). Its 18 arrival hashes and 19 later payload hashes distinguish the submitted 30-page PDF from subsequent source changes. The information-geometry, entropy, Fisher-information, prefix-code, Thue--Morse, and endpoint layers remain archival manuscript claims pending hostile audit, numerical replay, and an exact Lean crosswalk; manuscript theorem labels do not establish formal verification. | `frontier-compilations/fabius_information_frontier/`; moved here by the thematic reorganization. |
...
| `Up_Polynomial_Synthesis/` | *Exact Rvachev Up-Function Polynomial Synthesis* (current synchronized 2,324-line master and 60-page A4 PDF; exact B7 receipt above; 5,278 source lines across the master and three chapters; 80 theorem-like assertions, 80 proofs, and 80 one-to-one crosswalk rows: 25 + 16 + 17 + 22). Canonical editorial consolidation (2026-08-30) of three exact-polynomial, six Lagrange--Rvachev, and four Legendre--Rvachev packages, with repeated foundations deduplicated, exact Sturm evidence retained, and Lean anchors scoped claim by claim. The three earlier package payloads are under `assets/`; the disposition table records 113 selected later-report rows: 106 retained payloads have live destinations under `assets/companion-evidence/` and `assets/evidence/`, while seven checksum-ledger rows are retired without live destinations in `assets/provenance/COMPANION_PAYLOADS.csv`. The ten individual report directories were retired on 2026-08-31 after the exact gate passed; their source bytes remain recoverable at immutable commit `443793e846934e7363e314ea01129b9f50197a58`. | canonical volume; ten individual reports retired; current master, chapters, and synchronized PDF are retained; the current inventory lists every retained payload |
