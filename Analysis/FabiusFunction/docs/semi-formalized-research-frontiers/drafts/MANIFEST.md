...
**Current artifact checkpoint (2026-09-04).** The live Lean audit scans 943
source modules and 11,787 explicit public declarations, with no missing module headers or
...
and six concurrent declarations elsewhere give the live 943/11,787 census.
...
`DyadicBoundaryIdentity.lean`, and seven theorems added to
`ThueMorseNewmanSelfSimilarity.lean`. This gives the historical 934/11,709
census. The nine series/transseries modules and six concurrent declarations
then give the live 943/11,787 census.
...
943/11,787; the historical 934/11,709 and dyadic/finite-prefix 933/11,695
...
open: the current source overlay exhausts these nine module surfaces and keeps
...
- `TransseriesWellBased.lean` (0+5 written): `dickson_isPWO`,
  ...
  count.  `q0:lem:dickson` and `q0:lem:neumann` are **Exact** after translating
  the manuscript's reverse-well-order/strict-growth convention to Lean's
  `Set.IsPWO` orientation, equivalently the order dual.  No Hahn-series type is
  constructed.
...
- `TransseriesBlockAntiderivative.lean` (3+12): definitions `blockOperator`,
  ...
  `natDegree_resonantAntiderivative`.  The polynomial-operator dichotomy of
  `plt:lem:mot-block-antiderivative` is **Exact**; it does not construct the
  full Laurent ambient ring.

- `TransseriesDifferentialBlock.lean` (0+5): `derivation_pow_t`,
  `derivation_block`, `exists_block_primitive`, `derivation_block_zero`, and
  `exists_block_primitive_resonant`.  This is an exact abstract bridge for
  natural exponents under `d(t)=-t²`, `d(L)=t`, not the full integer-exponent
  Laurent wrapper.
