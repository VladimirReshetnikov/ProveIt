...
| --- | --- | --- |
| `Combinatorial_Coefficient_Calculus/` | **Canonical, editorial consolidation complete:** *Combinatorial Coefficient Calculus* — synchronized 213-page A4 PDF, 2,063,614 bytes, rebuilt on 2026-09-04 after integrating main `da90f69d1`; 209 register rows: 66 Lean / 35 partial / 108 without a compiled counterpart. The source has 11,290 lines, 555,974 UTF-8 bytes, 712 labels, and 960 references. Retains the stronger ring/semiring interfaces and upstream Laplace, Lagrange, symmetric-function, Newton, Nörlund, grid, CRT, and Bell-partition work. Historical compiler receipts and pending merged proofs are distinguished in `FORMALIZATION_STATUS.md`. | Six-source provenance and disposition records in the package; original non-suffixed arrival was `incoming/Combinatorial_Coefficient_Calculus.zip`; `SOURCE_INVENTORY.csv` records immutable Git locators for all six inputs, including the five retired donors. No checksum ledger is required. |
