...
If \(\Lip(\phi)\le1\), then
\(
 x\mapsto\int_0^1\phi((1-q)u+qx)\Differential u
\)
...
Let
\(
 b(x)=\Expectation[V\mid X=x]
\)
be any regular conditional version.  On \((0,1)\),
\begin{equation}
 b(x)f(x)=-G(x),
 \qquad
 \rho(x)=-\frac{\Differential}{\Differential x}\bigl(b(x)f(x)\bigr),
 \qquad
 b(1-x)=-b(x).
...
\[
 ...
 =\int\varphi'(x)b(x)f(x)\Differential x.
\]
...
should contain small divisors of the simple form \(2^{1-2\pi \ImaginaryUnit k/\log2}-1\), but the exact
