...
\end{equation}
For every \(\phi\in C_c^1(\R)\), this measure has the source formula
\begin{equation}
...
Write
\[
 T_q\phi(x)=\int_0^1\phi((1-q)u+qx)\dd u.
\]
If \(\Lip(\phi)\le1\), then \(T_q\phi\) has Lipschitz constant at most \(q\).
It need not vanish at the KR basepoint.  Since \(\eta(\R)=0\), however,
\[
 \int T_q\phi\dd\eta
 =\int\bigl(T_q\phi-T_q\phi(0)\bigr)\dd\eta,
\]
...
\(q\mapsto f_q(x)\dd x\) in total variation.  Hence \(\nu_q\) and the measure
...
\(\phi\in C_c^1(\R)\).  The derivative hitting the measure gives \(R_q\nu_q\); differentiating
...
\begin{equation}
 b_*(x)f(x)=-G(x),
 \qquad
 \rho(x)=-\frac{\dd}{\dd x}\bigl(b_*(x)f(x)\bigr),
 \qquad b_*(1-x)=-b_*(x).
...
\[
 ...
 \qquad \rho=-(\widetilde b\,f)' \quad\text{in }\mathcal D'(\R).
\]
...
\[
 ...
 =\E[\varphi'(X)V]
 =\int\varphi'(x)\widetilde b(x)f(x)\dd x.
\]
...
\((\widetilde b f+G)'=0\) on \(\R\), and compact support forces
\(\widetilde b f=-G\) a.e.

At half base, \(f(x)=2\Up(2x-1)>0\) for \(0<x<1\).  Thus \(\mu\) and Lebesgue measure have the
...
\begin{equation}
 \boxed{\displaystyle
 1-F^n(x)=
 \exp\left\{-2L\exp\bigl(2^nB_{\mathrm R}(x)+\smallo(1)\bigr)\right\}.}
...
Assume \(\mathrm{(K)}\).  Define, for \(u\in\R\),
\begin{equation}
