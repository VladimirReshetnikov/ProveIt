...
\hypersetup{
  ...
  pdfauthor={Research report prepared from the current ProveIt documentation corpus}
}
...
% Report-local additions after the canonical primary preamble.
...
\newcommand{\statusconditional}{\textsc{Conditional}}
...
The third direction treats composition dynamics.  We isolate a \(C^\infty\) flat Koenigs
linearization statement as an explicit analytic hypothesis \(\mathrm{(K)}\), because the
derivative estimates needed to justify it are not supplied here.  Conditional on
\(\mathrm{(K)}\), the established strict convexity of \(F\), exact midpoint--endpoint transfer,
and complete affine midpoint jet give a unique global \(C^\infty\) diffeomorphism
\(\Theta:(0,1)\to\RealNumbers\) satisfying
\[
 \Theta(F(x))=2\Theta(x),\qquad
 \Theta(F^{-1}(x))=\tfrac12\Theta(x),\qquad
 \Theta'(1/2)=1.
\]
The conditional coordinate differs from \(x-1/2\) by a nonzero flat function.  Consequently
inverse
...
is converted unconditionally into a logarithmic B\"ottcher-type height, giving an exact
doubling cocycle and a double-exponential asymptotic for forward iterates.  Conditional on
\(\mathrm{(K)}\), its quotient by \(-\Theta\) is an exact one-periodic dynamical factor.  The
report ends with explicit conjectures, numerical checks, and a Lean formalization roadmap.
\end{abstract}
...
real/Fourier analysis.  Conditional statements and conjectures are explicitly separated.  No
new theorem in this report has an exact current Lean counterpart; the formal files listed
below certify inputs only.

\subsection{Exact formalization boundary}

All paths in this paragraph are relative to
\path{Analysis/FabiusFunction/Lean/FabiusFunction/}.  The fixed-parameter probability law,
reflection, support, and affine self-similarity are in
\path{GeometricUniformLaw.lean}; the fixed-\(q\) CDF and density and their spatial smoothness
are in \path{GeometricUniformCDF.lean}; the half-base identifications are in
\path{ProbabilityRepresentation.lean}; and the fixed-parameter prefix sinc convergence is in
\path{GeometricSincFactorization.lean}.  The affine operator and uniqueness theorem are in
\path{AffineIndependentCopy.lean}, while the shape and inverse inputs are in
\path{Convexity.lean} and \path{FabiusInverse.lean}.  Exact midpoint transfer and inverse
midpoint defect are in \path{MidpointEndpointTransfer.lean} and
\path{InverseMidpointDefect.lean}; endpoint flatness and the leading quadratic logarithmic law
are in \path{FabiusFlatness.lean} and \path{FabiusLogSquaredAsymptotic.lean}.  Finally,
\path{CenteredMomentCumulants.lean}, \path{ExponentialBell.lean}, and
\path{GeometricUniformDictionary.lean} provide fixed-base algebraic infrastructure.

Not presently formalized are joint parameter--space smoothness, the flat parameter fronts,
the tangent measure and susceptibility/refinement theory, the response sinc/cumulant/moment/
Legendre calculations, the higher-response source regularity, the flat Koenigs hypothesis,
the global coordinate, the endpoint height, or the dynamical periodic factor.  In particular,
fixed-\(q\) spatial smoothness in \path{GeometricUniformCDF.lean} must not be cited as joint
\(q,x\) smoothness.
...
Midpoint--endpoint transfer & Conditional global Schr\"oder coordinate & Flat local linearization hypothesis extended by inverse iteration \tabularnewline
Lambert/periodic endpoint law & Dynamical height \(B\) & Unconditional doubling cocycle; conditional periodic quotient \tabularnewline
Thue--Morse/dyadic structure & Composition clocks & Conditional multiplication by \(2\) in the global coordinate \tabularnewline
\bottomrule
\end{tabularx}
\end{center}
...
\begin{equation}
 ...
 \label{eq:q-law}
\end{equation}
The weights sum to one, so \(X_q\in[0,1]\) and \(\Expectation X_q=1/2\).  We write
\[
 F_q(x)=\Probability(X_q\le x),\qquad f_q(x)=F_q'(x).
\]
...
\begin{equation}
 ...
 =\prod_{j\ge0}\SincRad\!\left(\frac{c_j(q)t}{2}\right),
...
\]
is \(C^\infty\) on \((0,1)\times\RealNumbers\).  More quantitatively, for every compact
\(K\Subset(0,1)\) and every \(a,b,N\in\NaturalNumbers\),
\begin{equation}
 \sup_{q\in K}\sup_{t\in\RealNumbers}
 (1+|t|)^N
...
\(C^m(\RealNumbers)\), and all parameter derivatives are supported in \([0,1]\).
\end{theorem}
...
\(
 \Phi_{q,M}(t)=\prod_{j<M}\SincRad(c_j(q)t/2).
\)
...
\[
 \left|\SincRad\!\left(\frac{c_j(q)t}{2}\right)\right|
...
The Fourier transform of \(f_q\) is \(\EulerE^{\ImaginaryUnit t/2}\Phi_q(t)\).  Fourier inversion, with
...
\(q_c=\min(x,1-x)\);
\item the \(C^\infty(\RealNumbers)\)-valued map \(q\mapsto f_q\) is not real analytic at any
\(q\in(0,1/2]\).
\end{enumerate}
\end{theorem}
...
For every \(\varphi\in C^1(\RealNumbers)\) whose derivative is bounded,
\begin{equation}
 \left.\frac{\Differential}{\Differential q}\Expectation\varphi(X_q)\right|_{q=1/2}
 =\Expectation\bigl[\varphi'(X)V\bigr].
...
\begin{equation}
 G(x)=\left.\partial_qF_q(x)\right|_{q=1/2}
      =\int_{-\infty}^{x}\rho(t)\Differential t
 \label{eq:G-def}
\end{equation}
satisfies
\begin{gather}
 ...
 \int\rho=0,
 \qquad \int\left(x-\frac12\right)\rho(x)\Differential x=0,
...
\begin{equation}
 \|\eta\|_{\mathrm{KR}}
 =\sup\left\{\left|\int\phi\Differential\eta\right|:
...
\begin{equation}
 ...
 =\int_0^1\int\phi((1-q)u+qx)\Differential\eta(x)\Differential u.
 \label{eq:Rq}
\end{equation}

\begin{theorem}[Susceptibility series]
\label{thm:susceptibility}
The operator \(R_q\) preserves \(\mathcal M_0\) and is a contraction there:
\begin{equation}
 \|R_q\eta\|_{\mathrm{KR}}\le q\|\eta\|_{\mathrm{KR}}.
 \label{eq:Rq-contraction}
\end{equation}
Let \(\mu_q\) be the law of \(X_q\).  The derivative
\(\nu_q=\partial_q\mu_q\) exists in total variation (and hence in the KR norm), has smooth
compactly supported density \(\partial_qf_q\), and has total mass zero.  Define the finite
zero-mass measure
\begin{equation}
 \sigma_q:=\nu_q-R_q\nu_q.
 \label{eq:sigmaq-measure}
\end{equation}
For every \(\phi\in C_c^1(\RealNumbers)\), this measure has the source formula
\begin{equation}
 \langle\phi,\sigma_q\rangle
 =\int_0^1\int
 \phi'((1-q)u+qx)(x-u)\Differential\mu_q(x)\Differential u.
 \label{eq:sigmaq}
\end{equation}
It has a smooth compactly supported density.  Moreover, \(\nu_q\) is the unique zero-mass
solution of
\begin{equation}
 \nu_q=R_q\nu_q+\sigma_q,
...
Write
\[
 T_q\phi(x)=\int_0^1\phi((1-q)u+qx)\Differential u.
\]
If \(\Lip(\phi)\le1\), then \(T_q\phi\) has Lipschitz constant at most \(q\).
It need not vanish at the KR basepoint.  Since \(\eta(\RealNumbers)=0\), however,
\[
 \int T_q\phi\Differential\eta
 =\int\bigl(T_q\phi-T_q\phi(0)\bigr)\Differential\eta,
\]
and the parenthesized function does vanish at zero.  Thus
\(
\|R_q\eta\|_{\mathrm{KR}}\le q\|\eta\|_{\mathrm{KR}}
\), proving \eqref{eq:Rq-contraction}; preservation of total mass is immediate.

Joint smoothness and the common compact support \([0,1]\) imply differentiability of
\(q\mapsto f_q(x)\Differential x\) in total variation.  Hence \(\nu_q\) and the measure
\(\sigma_q\) in \eqref{eq:sigmaq-measure} are well defined and have the asserted smooth
densities.  Differentiate the fixed-point identity \(\mu_q=R_q\mu_q\) against
\(\phi\in C_c^1(\RealNumbers)\).  The derivative hitting the measure gives \(R_q\nu_q\); differentiating
the affine argument gives \eqref{eq:sigmaq}.  Since \(\|R_q\|<1\) on \(\mathcal M_0\), the
Neumann series converges in KR and its solution is unique.
\end{proof}

\begin{proposition}[Higher response hierarchy: distributional and conditional]
...
\begin{equation}
 ...
 \qquad\text{in }\mathcal D'(\RealNumbers).
 \label{eq:higher-distributional}
\end{equation}
If the distribution on the right has independently been represented by a finite zero-mass
measure \(S_{q,s}\), then the KR resolvent gives the conditional identity
\begin{equation}
 \nu_q^{(s)}=(I-R_q)^{-1}S_{q,s}
 =\sum_{n=0}^{\infty}R_q^nS_{q,s}.
 \label{eq:higher-resolvent}
\end{equation}
No common measure-space regularity theorem for the displayed differentiated sources is proved
here when \(s\ge2\); the resolvent assertion at those orders is therefore conditional, not a
completed higher-response theorem.
\end{proposition}

\begin{proof}
Differentiating \(\mu_q=R_q\mu_q\) \(s\) times against compactly supported smooth tests and
isolating \(R_q\nu_q^{(s)}\) proves \eqref{eq:higher-distributional}.  Under the additional
measure representation stated above, the contraction argument from
\cref{thm:susceptibility} applies to \(S_{q,s}\) and proves \eqref{eq:higher-resolvent}.
\end{proof}
...
\begin{equation}
 \boxed{\displaystyle
 G(x)=\int_{2x-1}^{2x}G(v)\Differential v
      +4\int_{2x-1}^{2x}(x-v)f(v)\Differential v.}
...
\[
 G(x)=\int_0^1G(2x-u)\Differential u
      +4\int_0^1(u-x)f(2x-u)\Differential u,
\]
which is \eqref{eq:G-refinement}.  Integration by parts in the source and the half-base CDF
identity \(F(x)=\int_{2x-1}^{2x}F(v)\Differential v\) give \eqref{eq:G-source-F}.  Leibniz's rule and
\(
 F(2x)-F(2x-1)=f(x)/2
\)
...
\begin{proposition}[Canonical velocity field and conditional versions]
\label{prop:velocity-field}
Let
\(
 \widetilde b(x)=\Expectation[V\mid X=x]
\)
be any regular conditional version.  The half-base density is strictly positive on
\((0,1)\), and every such version agrees Lebesgue-a.e. there with the canonical representative
\begin{equation}
 b_*(x):=-\frac{G(x)}{f(x)}.
 \label{eq:canonical-velocity}
\end{equation}
The function \(b_*\) is smooth and odd on \((0,1)\), and it satisfies the pointwise identities
\begin{equation}
 b_*(x)f(x)=-G(x),
 \qquad
 \rho(x)=-\frac{\Differential}{\Differential x}\bigl(b_*(x)f(x)\bigr),
 \qquad b_*(1-x)=-b_*(x).
 \label{eq:velocity-form}
\end{equation}
For an arbitrary regular conditional version, the product identity holds only a.e. and the
continuity equation only distributionally:
\[
 \widetilde b\,f=-G\quad\text{a.e. on }(0,1),
 \qquad \rho=-(\widetilde b\,f)' \quad\text{in }\mathcal D'(\RealNumbers).
\]
Thus the \(q\)-response is a one-dimensional continuity equation with canonical velocity
\(b_*\); null-set changes to a conditional version carry no pointwise content.
\end{proposition}

\begin{proof}
For a smooth compactly supported test function,
\[
 \int\varphi(x)\rho(x)\Differential x
 =\Expectation[\varphi'(X)V]
 =\int\varphi'(x)\widetilde b(x)f(x)\Differential x.
\]
Hence \(\rho=-(\widetilde b f)'\) in distributions.  Extend
\(\widetilde b f\) by zero off \([0,1]\).  Since \(V\) is bounded, this product is integrable
and compactly supported; \(G\) is also compactly supported because \(\int\rho=0\).  Therefore
\((\widetilde b f+G)'=0\) on \(\RealNumbers\), and compact support forces
\(\widetilde b f=-G\) a.e.

At half base, \(f(x)=2\RvachevUp(2x-1)>0\) for \(0<x<1\).  Thus \(\mu\) and Lebesgue measure have the
same null sets in the interior, and every conditional version equals
\(b_*=-G/f\) there a.e.  Smoothness follows from smoothness of \(G,f\) and interior positivity.
Finally, \(G(1-x)=-G(x)\) and \(f(1-x)=f(x)\), so the canonical representative is odd.
Its product with \(f\) equals \(-G\) pointwise, and differentiation yields the pointwise
continuity equation.
\end{proof}
...
\begin{equation}
 ...
 \SincRad'\!\left(\frac{t}{2^{j+2}}\right)
 \prod_{k\ne j}\SincRad\!\left(\frac{t}{2^{k+2}}\right),
...
Fourier transform is \(\FourierAngular{\rho}(-t)=\EulerE^{\ImaginaryUnit t/2}\dot\Phi(t)\).
\end{theorem}

\begin{proof}
Differentiate the normally convergent product \eqref{eq:centered-sinc-product}.  The logarithmic
form uses \(\SincRad'(z)/\SincRad(z)=\cot z-z^{-1}\).
\end{proof}
...
\begin{equation}
 ...
 =\frac{B_r}{r}\frac{(1-q)^{r-1}}{\QInteger{r}{q}}.
 \label{eq:q-cumulant}
\end{equation}
At the Fabius point,
\begin{align}
...
Let \(\mu_n(q)=\Expectation Z_q^n\), \(\mu_n=\mu_n(1/2)\), and
...
If
\(
 M_q(z)=\Expectation\EulerE^{zZ_q}=\exp K_q(z),
\)
...
\[
 \int_0^1P_k^*(x)P_\ell^*(x)\Differential x=\frac{\delta_{k\ell}}{2k+1}.
\]
...
Orthogonality gives
\[
 a_k=(2k+1)\int_0^1\rho(x)P_k(2x-1)\Differential x
 =(2k+1)\left.\partial_q\Expectation P_k(2Z_q)\right|_{q=1/2}.
\]
...
midpoint defect.  After the unconditional dynamical preliminaries, the global composition
claims are explicitly conditional on the analytic hypothesis \(\mathrm{(K)}\) isolated below.
...
flatness says \(F(t)=\LittleO(t^N)\) as \(t\downarrow0\) for every \(N\), while
...
The local \(C^\infty\) linearization is the one analytic step not completed in this report.
We isolate it rather than treating a telescoping sketch as a proof.

\begin{definition}[Flat Koenigs analytic hypothesis \(\mathrm{(K)}\)]
\label{hyp:flat-koenigs}
For the map \(g\) in \eqref{eq:g-def}, hypothesis \(\mathrm{(K)}\) is the assertion that on a
neighborhood of zero the limit
\begin{equation}
 \theta(y)=\lim_{n\to\infty}2^n g^n(y)
 \label{eq:local-koenigs-limit}
\end{equation}
exists in \(C^\infty\), is a local increasing diffeomorphism, and satisfies
\begin{equation}
 \theta(g(y))=\frac12\theta(y),
 \qquad \theta'(0)=1,
 \qquad \theta(y)-y\text{ is flat at }0.
 \label{eq:local-koenigs-properties}
\end{equation}
It also asserts uniqueness among local \(C^1\) solutions with derivative one at zero.
\end{definition}

\begin{statusbox}
\textbf{\statusconditional.}
Hypothesis \(\mathrm{(K)}\) is neither proved in this report nor represented by a current Lean
theorem.  Every subsequent assertion involving \(\Theta\), its inverse-iterate expansion, its
escape clock, or the periodic quotient with \(B\) is conditional on \(\mathrm{(K)}\).
\end{statusbox}

\begin{remark}[The missing estimate]
For a general contraction \(g(y)=\lambda y+r(y)\), the telescoping identity
\begin{equation}
 \lambda^{-n}g^n(y)
 =y+\sum_{k=0}^{n-1}\lambda^{-k-1}r(g^k(y))
 \label{eq:koenigs-telescope}
\end{equation}
and flatness of \(r\) readily give local uniform convergence after choosing a sufficiently
large flatness order.  The submitted argument then asserted the same majorant after arbitrary
differentiation.  That step requires uniform Faà di Bruno estimates for derivatives of the
iterates and was not supplied.  Equation \eqref{eq:koenigs-telescope} is therefore motivation
for \(\mathrm{(K)}\), not a complete \(C^\infty\) proof.
\end{remark}

\subsection{Globalization and exact conjugacy}

\begin{theorem}[Global Fabius Schr\"oder coordinate, conditional on \(\mathrm{(K)}\)]
\label{thm:global-theta}
Assume hypothesis \(\mathrm{(K)}\) from \cref{hyp:flat-koenigs}.  Then there is a unique
\(C^\infty\) increasing diffeomorphism
\begin{equation}
 \Theta:(0,1)\longrightarrow\RealNumbers
...
\begin{proof}
Hypothesis \(\mathrm{(K)}\) supplies the local coordinate \(\theta\).  By
\cref{lem:diagonal}, every interior point enters the local neighborhood under
...
must be all of \(\RealNumbers\).  Reflection and local uniqueness give oddness.  Global uniqueness
follows from
\[
 \widetilde\Theta(x)
 =2^n\widetilde\Theta(Q^n(x))
 =2^n(Q^n(x)-p)+\LittleO(1).
\]
Flatness is part of hypothesis \(\mathrm{(K)}\); flatness of the inverse follows
...
\begin{corollary}[Conditional exact iterate formula and inverse asymptotic]
\label{cor:iterate-formula}
Assume \(\mathrm{(K)}\).  For every \(x\in(0,1)\) and \(n\ge0\),
\begin{equation}
...
\begin{corollary}[Conditional nonanalyticity of the linearizing coordinate]
\label{cor:theta-nonanalytic}
Assume \(\mathrm{(K)}\).  Neither \(\Theta\) nor \(\Theta^{-1}\) is real analytic at the
midpoint.  Their Taylor series there are exactly \(x-p\) and \(p+t\), respectively.
\end{corollary}
...
\begin{corollary}[Conditional dyadic escape time]
\label{cor:escape-clock}
Assume \(\mathrm{(K)}\).  Fix \(a\in(0,1/2)\).  For \(x\in(p,p+a)\), let
\[
...
\begin{equation}
 \tau_a(x)=
 \Ceiling{\log_2\frac{\Theta(p+a)}{\Theta(x)}}.
...
Conditional on \(\mathrm{(K)}\), the coordinate \(\Theta\) solves the hyperbolic dynamics
globally, but its endpoint inverse \(\Theta^{-1}(t)\) as \(|t|\to\infty\) is highly
non-elementary.  Independently of that hypothesis, the leading endpoint law implies a natural
doubling height adapted to the endpoint logarithm.
...
\begin{equation}
 -\log F(\EulerE^{-s})=a s^2(1+\LittleO(1))
 \qquad(s\to\infty).
...
\begin{equation}
 -\log F^n(x)
 =2L\exp\bigl(2^nB(x)+\LittleO(1)\bigr),
...
\begin{equation}
 \boxed{\displaystyle
 F^n(x)=
 \exp\left\{-2L\exp\bigl(2^nB(x)+\LittleO(1)\bigr)\right\}.}
 \label{eq:double-exponential-orbit}
\end{equation}
For \(p<x<1\), define \(B_{\mathrm R}(x)=B(1-x)\).  Reflection gives the explicit
right-endpoint formula
\begin{equation}
 \boxed{\displaystyle
 1-F^n(x)=
 \exp\left\{-2L\exp\bigl(2^nB_{\mathrm R}(x)+\LittleO(1)\bigr)\right\}.}
 \label{eq:double-exponential-right-orbit}
\end{equation}
\end{theorem}
...
This proves \eqref{eq:B-limit} and \eqref{eq:B-series}.  On a compact
\(K\Subset(0,p)\), the continuous functions \(F^k|_K\) decrease pointwise to the continuous
limit zero.  Dini's theorem makes this convergence uniform.  Consequently
\(\varepsilon(F^k(x))\to0\) uniformly on \(K\), and the geometrically weighted series in
\eqref{eq:B-series} converges uniformly there.  This proves continuity of \(B\).  Shifting
the orbit proves \eqref{eq:B-doubling}.
...
\]
tends to zero.  Therefore \(z_n=2^nB(x)+\LittleO(1)\), which gives
\eqref{eq:double-exp-log} and \eqref{eq:double-exponential-orbit}.  Also \(z_n\to+\infty\).
If \(B(x)<0\), the tail identity would force \(z_n\to-\infty\); if \(B(x)=0\), it would force
\(z_n\to0\).  Both alternatives are impossible, so \(B(x)>0\).

Finally, reflection iterates exactly:
\(
 1-F^n(x)=F^n(1-x)
\).
Applying \eqref{eq:double-exponential-orbit} to \(1-x\in(0,p)\) proves
\eqref{eq:double-exponential-right-orbit}.
\end{proof}

\subsection{A conditional periodic quotient of the two dynamical coordinates}

Assume \(\mathrm{(K)}\), so that the conditional coordinate \(\Theta\) exists.  Both \(B\) and
\(-\Theta\) then double under \(F\) on the left half.  Their quotient is invariant along each
orbit, and in the linear coordinate invariance becomes periodicity.

\begin{theorem}[Dynamical one-periodic factor, conditional on \(\mathrm{(K)}\)]
\label{thm:dynamical-periodic}
Assume \(\mathrm{(K)}\).  Define, for \(u\in\RealNumbers\),
\begin{equation}
...
orbits and the conditional global Schr\"oder coordinate.  Determining the transform between the two
...
All unconditional theorems above are proved independently of numerics; computation also does
not discharge hypothesis \(\mathrm{(K)}\).  The included Python program uses exact rational
arithmetic for cumulants, moments, and Legendre coefficients, and common-random-number Monte
Carlo only as a consistency check of the pathwise derivative formula.
\end{statusbox}
...
\begin{equation}
 \dot\mu_m=m\Expectation[Z^{m-1}V].
...
\[
 q\longmapsto f_q
\]
is real analytic as a map into \(C^\infty(\RealNumbers)\) on \((1/2,1)\).  On \((0,1/2]\), the flat
...
\label{conj:Pdyn}
Assuming \(\mathrm{(K)}\), the function \(\mathcal P_{\mathrm{dyn}}\) in
\eqref{eq:Pdyn-def} is nonconstant and real analytic.  Its Fourier coefficients can be
obtained by an explicit cohomological transform of the Gamma--zeta Fourier coefficients of
the repository's endpoint periodic function \(\Psi\).
\end{conjecture}

Conditional on \(\mathrm{(K)}\), one route is to insert the sharp endpoint expansion into the
cocycle \(z(F(x))-2z(x)\), sum it along the orbit as in \eqref{eq:B-series}, and then convert
the orbit phase to \(u=\log_2[-\Theta(x)]\).  No Fourier multiplier is asserted here: it must
be derived only after the lower-Lambert phase has been converted to the dynamical phase.  An
unvalidated candidate in the submitted draft simplified to a non-small constant and is
withdrawn.
...
\(
 \partial_q\Expectation\varphi(X_q)=\Expectation[\varphi'(X_q)\partial_qX_q].
\)
...
inverse midpoint.
\item Close the flat Koenigs hypothesis \(\mathrm{(K)}\) from
\cref{hyp:flat-koenigs}, including the missing all-orders derivative estimates, first in a
local one-dimensional real setting.
\item Globalize by finite inverse iteration and prove image \(\RealNumbers\), uniqueness, reflection, and
the limit formula.
...
The local linearization is the only substantial general-purpose analytic component, and it is
an open proof obligation in this report.  It is independent of Fabius-specific arithmetic and
may be useful elsewhere in the library.
...
Conditional on the explicitly isolated hypothesis \(\mathrm{(K)}\), the global coordinate
\(\Theta\) would reveal a simple composition law beneath the nonanalytic graph: the whole open
unit interval would be smoothly conjugate to the real line under multiplication by two.  The
unconditional logarithmic height \(B\) captures the singular endpoint geometry.  Under
\(\mathrm{(K)}\), its exact periodic quotient with \(\Theta\) gives a concrete target for
connecting composition dynamics to the known Lambert--periodic endpoint expansion.
