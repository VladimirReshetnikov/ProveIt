...
\[
 \Lambda(\theta)=\int_0^\infty
     \log\!\left(\frac{\sinh(\theta e^{-t})}{\theta e^{-t}}\right)\Differential t
...
\begin{align}
 \phi_q(t)&=\Expectation e^{itX_q}
   =\prod_{j=0}^{\infty}\SincRad\bigl((1-q)q^jt\bigr),
   \label{eq:cf}\\
 M_q(t)&=\Expectation e^{tX_q}
...
\begin{equation}
 \Lambda_\delta(\theta)
 :=\delta\log\Expectation\exp\!\left(\frac{\theta X_{e^{-\delta}}}{\delta}\right).
 \label{eq:scaled-cgf}
\end{equation}
...
\begin{equation}
 ...
 \Lambda(\theta):=\int_0^\infty g(\theta e^{-t})\Differential t
...
$g''(u)>0$; therefore
\[
 \Lambda''(\theta)=\int_0^\infty e^{-2t}g''(\theta e^{-t})\Differential t>0.
\]
...
\label{thm:LDP}
The family $X_{e^{-\delta}}$ satisfies a large-deviation principle on $\RealNumbers$
...
\begin{equation}
 ...
 +\int_1^\infty\frac{\log(1-e^{-2u})}{u}\Differential u.
 \label{eq:CLambda}
\end{equation}
Numerically,
\begin{equation}
...
$|\SincRad u|\le e^{-c_0u^2}$ for $|u|\le1$.  If
...
Let $V$ be bounded and let $K_V(\theta)=\log\Expectation e^{\theta V}$.  For
$X_q^{(V)}$ from \eqref{eq:general-seed},
\begin{equation}
 \delta\log\Expectation\exp\!\left(\frac{\theta X_{e^{-\delta}}^{(V)}}{\delta}\right)
 \longrightarrow
 \Lambda_V(\theta):=\int_0^\infty K_V(\theta e^{-t})\Differential t.
