...
finite product \(P_n(a,q)=\poch aqn\), the nontrivial parameter for which
...
\[
 \poch{-1/3}{q}{3}=\frac{32}{27}+\frac4{27}(q+1)^3,
\]
...
\((a;\e^{-\varepsilon})_\infty=\e^{-L}\), with \(L>0\) fixed, then
\[
\begin{aligned}
...
\(
 \omega_m^j=\exp(2\pi\ii j/m),\ 0\le j<m
\).
...
\[
 \log F(\e^{-\varepsilon})=-\frac{A}{\varepsilon}+B-C\varepsilon+
...
\(q=\e^{-\varepsilon}\).  If \(\varepsilon\) also occurs inside a logarithm,
...
Let \(\bm F:\C^r\to\C^r\) and suppose
...
For \(n\in\N\), set
\[
 P_n(a,q)=\poch aqn=\prod_{j=0}^{n-1}(1-aq^j),
 \qquad P_0=1.
\]
...
\[
 s=[n]_q a-q\qbinom n2a^2+q^3\qbinom n3a^3+\cdots,
\]
...
\begin{equation}
 \log P_n(a,\e^{-\varepsilon})
 =n\log(1-a)+\sum_{m\ge1}
 \frac{(-1)^{m+1}}{m!}
 \Li_{1-m}(a)S_m(n)\varepsilon^m.
...
Use
\[
 \log(1-a\e^{-j\varepsilon})
 =-\sum_{r\ge1}\frac{a^r}{r}\e^{-jr\varepsilon}
\]
...
\[
 ...
 \qquad
 c_m=\frac{(-1)^{m+1}}{m!}\Li_{1-m}(a)S_m(n).
\]
If \(c_1=S_1(n)a/(1-a)\ne0\), then
\begin{equation}
 \varepsilon=
...
\(P_n(a(\varepsilon),\e^{-\varepsilon})=y\) using
...
\end{equation}
with a chosen branch of \(q^\alpha=\e^{\alpha\Log q}\).  Its Lambert-series
form is
\begin{equation}
 \log(a;q)_\alpha
...
\(\Log q\) by \(2\pi\ii\) changes the global order covering.

\section{Near \(q=1\): Bernoulli-polynomial order jet}
The generating identity
\begin{equation}
 \frac{1-\e^{-\alpha x}}{1-\e^{-x}}
 =\sum_{j\ge0}
...
\[
 ...
 \qquad C=\frac1{12}\Li_0(a),
 \qquad E=\frac1{720}\Li_{-2}(a),
\]
...
\[
 \log(a;\e^{-\varepsilon})_\infty
 =-\sum_{r\ge1}\frac{a^r}{r(1-\e^{-r\varepsilon})}.
\]
...
For a complex target \(y\ne0\), choose \(L=-\Log y\).  Each logarithm branch
produces a formal double-scaling branch.  The condition \(a\to0\) selects the
same local balance, but different values \(L+2\pi\ii k\) lead to different
...
\]
Unless \(\alpha\in\N\), the expansion at \(q=0\) belongs to the generalized
power semigroup generated by \(1\) and \(\alpha\), not to \(\C[[q]]\).  If
...
\begin{align}
 \log(a;q)_{n+\eta}
 ={}&\log(a;q)_n
 +\eta(\Log q)\sum_{r\ge1}
 \frac{a^rq^{nr}}{1-q^r} \notag \\
 &+\frac{\eta^2}{2}(\Log q)^2\sum_{r\ge1}
...
it retains exact branch information through \(\Log q\).
...
\begin{equation}
 H_P(\varepsilon)=
 \frac{\e^{D\varepsilon/2}P(\e^{-\varepsilon})}{P(1)}
 =\E\e^{-\varepsilon(X-D/2)}.
...
\[
 H_P(\varepsilon)=\E\cosh(\varepsilon Z).
\]
It is even and at least one.  For \(\varepsilon>0\),
\[
 H_P'(\varepsilon)=\E[Z\sinh(\varepsilon Z)]>0,
\]
...
\(H_P(\varepsilon)=1+\Var(X)\varepsilon^2/2+\cdots\), so the local inverse is
square-root.
\end{proof}
...
\end{equation}
It agrees with \(\qbinom nk\) at integers.  Let \(\psi_q\) denote the
...
\begin{equation}
 \log\mathcal G_{\e^{-\varepsilon}}(\alpha,\beta)
...
\begin{equation}
 x=\frac{\Log\bigl(1-(1-q)y\bigr)}{\Log q}.
 \label{eq:qnumber-x-inverse}
\end{equation}
The branch points are inherited from both logarithms.  When \(x=m\in\N\),
...
Set \(q=\e^{-\varepsilon}\).  The following coefficient functions are
polynomials in \(x\):
\begin{align}
...
\begin{equation}
 \log\frac{B_{\e^{-\varepsilon}}(x,y)}{B(x,y)}
...
For \(0<\abs q<1\), use the conventions
\[
...
\end{equation}
For fixed \(0<\abs q<1\), its zeros \(z=q^m\), \(m\in\Z\), are simple.  Hence
...
\(\e^{-\pi^2/\varepsilon}\), so a complete inverse is a transseries rather than
...
Let \(\operatorname{sinc}z=\sin z/z\), and for \(0<q<1\) define
\begin{equation}
 \widehat U_q(t)=\prod_{j\ge0}
 \operatorname{sinc}\bigl((1-q)q^jt\bigr).
...
\[
 \widehat U_{1/2}(t)=\prod_{r\ge1}\operatorname{sinc}(t/2^r),
\]
...
\(q\approx1\) & \(\varepsilon=-\Log q\) & logarithmic target \(w=\Log(y/y_0)\) \\
...
\(q\) & \(q=1\) & Taylor & \(\varepsilon=-\Log q\), logarithmic target
\(w\) \\
