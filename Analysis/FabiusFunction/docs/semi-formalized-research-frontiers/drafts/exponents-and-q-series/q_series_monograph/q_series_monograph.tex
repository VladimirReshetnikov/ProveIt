...
$\e^{\pi i\tau}$, others use $q=\e^{2\pi i\tau}$; some suppress the base in
...
at $a=q^{-m}$, $m\in\N_0$, with simple zeros when $q\ne0$.
\end{lemma}
\begin{proof}
...
\begin{align}
\qbinom{n+1}k&=\qbinom nk+q^{n+1-k}\qbinom n{k-1},\label{eq:pascal1}\\
\qbinom{n+1}k&=q^k\qbinom nk+\qbinom n{k-1}.\label{eq:pascal2}
\end{align}
\end{proposition}
\begin{proof}
...
the first after the symmetry $\qbinom nk=\qbinom n{n-k}$.
\end{proof}

\begin{remark}[Combinatorial meaning]
When $q$ is a prime power, $\qbinom nk$ counts the $k$-dimensional subspaces of
...
\[
\qbinom nk+q^{n+1-k}\qbinom n{k-1}=\qbinom{n+1}k
\]
...
\end{equation}
The identity also holds coefficientwise in $\C[a,q][[z]]$.
\end{theorem}
\begin{proof}
Let
...
\[
\frac1{(z;q)_{m+1}}=
\sum_{n=0}^{\infty}\qbinom{m+n}{n}z^n.
\]
\end{corollary}
\begin{proof}
...
=\frac{(q;q)_{m+n}}{(q;q)_m(q;q)_n}=\qbinom{m+n}{n}.
\]
\end{proof}
...
(xy;q)_n=\sum_{k=0}^n\qbinom nk(x;q)_k y^k(y;q)_{n-k}.
\end{equation}
\end{lemma}
\begin{proof}
...
(at;q)_n(bt;q)_n
=\sum_{k=0}^n\qbinom nk(a;q)_k(b;q)_k t^k
...
Put
\[
S_{n,k}=\qbinom nk(a;q)_k(b;q)_k t^k
(t;q)_{n-k}(abtq^k;q)_{n-k},
...
becomes
\[
\qbinom nk(a;q)_k(b;q)_k\left(\frac{c}{ab}\right)^k
...
\end{equation}
Both bilateral series converge normally on compact annuli in $\C^\times$.
\end{theorem}
\begin{proof}
Put
\[
...
\]
It is analytic on $\C^\times$ and has a normally convergent Laurent expansion
$F(z)=\sum_{n\in\Z}c_nz^n$.  Directly from the products,
\[
F(qz)=\frac{1+z^{-1}}{1+z}F(z)=z^{-1}F(z).
\]
Comparing Laurent coefficients gives
$c_{n+1}=q^n c_n$, hence
$c_n=q^{\binom n2}c_0$ for every $n\in\Z$.
...
(q;q)_\infty=\sum_{n\in\Z}(-1)^nq^{n(3n-1)/2}.
\end{equation}
\end{corollary}
\begin{proof}
...
For $|q|<1$ and $z\ne0$,
\begin{align}
&\sum_{n\in\Z}q^{n(3n-1)/2}z^{3n}(1-zq^n)\label{eq:qpp}\\
...
P(qz)=q^{-1}z^{-3}P(z).
\]
If $P(z)=\sum_{m\in\Z}c_mz^m$, then
...
\[
j(z;q)=\sum_{r\in\Z}(-1)^rq^{r(r-1)/2}z^r,
\qquad
...
\]
for all $n\in\Z$.  This is exactly the Laurent series on the left of
\eqref{eq:qpp}.
\end{proof}
...
The coefficient of $q^m$ in $\qbinom{r+s}{r}$ is the number of partitions of
...
$\qbinom{r+s}{r}$ satisfies the same recurrence and boundary values.
\end{proof}
...
\[
\frac{q^{mN}}{(q;q)_N}
\sum_{r=0}^N(-1)^rq^{\binom r2}\qbinom Nr q^{-mr}
...
For a Schwartz function $f\colon\R\to\C$, use the normalization
\[
\widehat f(\xi)=\int_{-\infty}^{\infty}f(x)\e^{-2\pi\ii x\xi}\,dx.
\]
\end{definition}
...
\[
\int_0^1P_f(x)\e^{-2\pi\ii mx}\,dx
=\int_{\R}f(x)\e^{-2\pi\ii mx}\,dx=\widehat f(m).
\]
...
\int_{\R}\e^{-\pi t x^2}\e^{-2\pi\ii x\xi}\,dx
=t^{-1/2}\e^{-\pi\xi^2/t}.
\end{equation}
\end{lemma}
\begin{proof}
...
$G(\xi)=G(0)\e^{-\pi\xi^2}$.  The classical polar-coordinate computation
\[
G(0)^2=\iint_{\R^2}\e^{-\pi(x^2+y^2)}\,dx\,dy
=2\pi\int_0^\infty r\e^{-\pi r^2}\,dr=1
\]
...
\begin{align*}
\vartheta_2(\tau)&=\sum_{n\in\Z}\e^{\pi\ii\tau(n+1/2)^2},\\
\vartheta_3(\tau)&=\sum_{n\in\Z}\e^{\pi\ii\tau n^2},\\
\vartheta_4(\tau)&=\sum_{n\in\Z}(-1)^n\e^{\pi\ii\tau n^2}.
\end{align*}
We take $(-\ii\tau)^{1/2}$ to be the holomorphic square root that is positive
for $\tau=\ii t$, $t>0$.

\begin{theorem}[Theta inversion]\label{thm:thetaS}
For $\operatorname{Im}\tau>0$,
\begin{align}
\vartheta_3(-1/\tau)&=(-\ii\tau)^{1/2}\vartheta_3(\tau),\label{eq:theta3S}\\
\vartheta_2(-1/\tau)&=(-\ii\tau)^{1/2}\vartheta_4(\tau),\label{eq:theta2S}\\
\vartheta_4(-1/\tau)&=(-\ii\tau)^{1/2}\vartheta_2(\tau).\label{eq:theta4S}
\end{align}
\end{theorem}
\begin{proof}
First set $\tau=\ii t$, $t>0$.  Apply Poisson summation to
$f(x)=\e^{-\pi t x^2}$ and use \eqref{eq:gaussianFT}:
\[
\sum_{n\in\Z}\e^{-\pi t n^2}
=t^{-1/2}\sum_{m\in\Z}\e^{-\pi m^2/t}.
\]
...
$q=Q^2=\e^{2\pi\ii\tau}$, the extra factors besides
...
For $\operatorname{Im}\tau>0$,
\begin{align}
\eta(\tau+1)&=\e^{\pi\ii/12}\eta(\tau),\label{eq:etaT}\\
\eta(-1/\tau)&=(-\ii\tau)^{1/2}\eta(\tau).
\label{eq:etaS}
\end{align}
\end{theorem}
\begin{proof}
...
&=\sqrt{\frac{2\pi}{t}}
\exp\!\left(-\frac{\pi^2}{6t}+\frac{t}{24}\right)
\left(1+O(\e^{-4\pi^2/t})\right),\label{eq:qpoch-asymp}\\
\log(q;q)_\infty
&=-\frac{\pi^2}{6t}+\frac12\log\frac{2\pi}{t}
+\frac{t}{24}+O(\e^{-4\pi^2/t}).\label{eq:logqpoch}
\end{align}
\end{theorem}
\begin{proof}
Put $\tau=\ii t/(2\pi)$.  From the definition,
\[
\eta(\tau)=\e^{-t/24}(\e^{-t};\e^{-t})_\infty.
\]
The eta inversion formula gives
\[
\eta\!\left(\frac{\ii t}{2\pi}\right)
=\sqrt{\frac{2\pi}{t}}\,
\eta\!\left(\frac{2\pi\ii}{t}\right).
\]
...
product differs from $1$ by $O(\e^{-4\pi^2/t})$; taking a logarithm preserves
...
\end{theorem}
\begin{proof}
We first prove the formula for $b=q^{m+1}$, $m\in\N_0$.  For these values,
...
$P_N$ has a well-defined coefficientwise limit in $\C[[q]]$.  If in addition
...
\end{equation}
There are unique polynomials $A_n,B_n,C_n\in\Z[x]$ such that
...
$\qbinom nk$ & Gaussian or $q$-binomial coefficient\\
