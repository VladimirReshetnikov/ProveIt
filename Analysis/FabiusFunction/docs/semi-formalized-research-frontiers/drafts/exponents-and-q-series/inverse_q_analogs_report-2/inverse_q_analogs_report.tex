...
\]
\[
 \qbinom{n}{k}{q}=\frac{(q;q)_n}{(q;q)_k(q;q)_{n-k}},\qquad
 [x]_q=\frac{1-q^x}{1-q}.
\]
...
Branches of $\Log$, fractional powers, and polylogarithms are always part of
...
For a Gaussian polynomial, $y=\qbinom{n}{k}{q}$ is an algebraic equation of
...
\[
 \Disc_x(F(x;\lambda)-y)
 =\Res_x(F(x;\lambda)-y,F_x(x;\lambda)).
\]
...
$\qbinom{n}{k}{q}$ in $q$ & $q=0$ & $y-1$ & partition-coefficient reversion\\
$\qbinom{n}{k}{q}$ in $q$ & $q=1$ & $\log\{y/\binom nk\}$ & Bernoulli-cumulant reversion\\
$\qbinom{n}{k}{q}$ in $q$ & $q=\pm\infty$ & $y^{-1/D}$ & reciprocal Puiseux sheets\\
...
local ring $\C[[u]]$ and therefore also proves formal uniqueness.
\end{proof}

\begin{remark}
For $q$-products it is usually better to revert a logarithm.  If $F(x_0)\ne0$,
set $u=\Log(F(x)/F(x_0))$.  Logarithmic derivatives turn products into Lambert
...
\begin{equation}\label{eq:finite-a-expansion}
 ...
 \qquad E_r(n,q)=q^{r(r-1)/2}\qbinom{n}{r}{q}.
\end{equation}
...
={}&-\frac{\Li_2(a)}{t}+\frac12\Log(1-a)\notag\\
 ...
 t^{2r-1}\Li_{2-2r}(a)+\OO(t^{2M-1}).
 \label{eq:mcintosh}
\end{align}
For $0<a<1$, let
\[
 Y=-\log y,\quad
 A=\Li_2(a),\quad B=\frac12\log(1-a),
\]
\[
 C=\frac{\Li_0(a)}{12}=\frac{a}{12(1-a)},
 \qquad E=\frac{\Li_{-2}(a)}{720}
...
inverse requires a branch of $\Li_2(a)$ and $\Log(1-a)$ and, near Stokes
...
\begin{align}
 \Log(a;\zeta e^{-t})_\infty
={}&-\frac{\Li_2(a^d)}{d^2t}+C_{d,\zeta}(a)+\OO(t),
 \label{eq:root-unity-asymptotic}\\
 C_{d,\zeta}(a)
={}&\sum_{r=0}^{d-1}\left(\frac12-\frac rd\right)
       \Log(1-a\zeta^r).\label{eq:root-unity-constant}
\end{align}
\end{theorem}
\begin{proof}
...
\[
 -\frac1{dt}\sum_{r=0}^{d-1}\Li_2(a\zeta^r).
\]
The root-of-unity filter gives
\[
 \sum_{r=0}^{d-1}\Li_2(a\zeta^r)
 =\frac1d\Li_2(a^d).
\]
The parameter $a\zeta^re^{-rt}$ contributes
$-(r/d)\Log(1-a\zeta^r)$ from the first-order expansion of its dilogarithm,
while McIntosh contributes $\tfrac12\Log(1-a\zeta^r)$.  Summing gives
...
\end{corollary}
\begin{proof}
For small $a$, $\Li_2(a^d)=a^d+\OO(a^{2d})$.  Insert this into
...
\end{equation}
with a chosen branch of $q^\alpha=e^{\alpha\Log q}$.
...
\begin{equation}\label{eq:alpha-logder}
 ...
 =\Log q\sum_{j\ge0}\frac{aq^{\alpha+j}}{1-aq^{\alpha+j}}<0.
\end{equation}
...
Put $u=\Log y$ and
\[
 c_m=(\Log q)^m\sum_{j\ge0}\Li_{1-m}(aq^j),\qquad m\ge1.
\]
Then
\[
 \Log(a;q)_\alpha
...
Therefore
\begin{align}
 \alpha={1\over\Log q}\biggl[&\Log\frac{(1-q)w}{a}
 -\frac{w}{1+q}\notag\\
...
\begin{equation}\label{eq:qnumber-xinverse}
 x=\frac{\Log(1-y(1-q))}{\Log q}.
\end{equation}
This exact formula is useful as a model: branch changes in $\Log$ create all
...
\begin{equation}\label{eq:rogers-z-inverse}
 z=\frac{y-1}{[n]_q}
 -\frac{\qbinom{n}{2}{q}}{[n]_q^3}(y-1)^2
 +\OO((y-1)^3).
\end{equation}
...
Define
\[
 \Cat_n(q)=\frac{1}{[n+1]_q}\qbinom{2n}{n}{q}.
\]
For every $n\ge2$,
...
\begin{equation}\label{eq:theta-def}
 ...
 \qquad 0<|q|<1.
\end{equation}
Its zeros $z=q^m$, $m\in\Z$, are simple when $q$ is not a root of unity, so
...
\item use Newton iteration in the truncated ring
      $\C[[u]]/(u^{M+1})$, doubling the number of correct coefficients at each
      step;
...
$\Log(a;\zeta e^{-t})_\infty$ has coefficients in the differential algebra
generated by
\[
 \Li_m(a\zeta^r),\quad \Log(1-a\zeta^r),
 \qquad 0\le r<d,
\]
...
$\Li_2(a^d)$ and on proximity to a critical level.
\end{problem}
...
critical values of $\qbinom{n}{k}{q}$.  Relate it to the limit shape of
...
$q\to1$, finite &
$u=\sum_{m\ge1}-S_m(n)\Li_{1-m}(a)t^m/m!$, followed by universal reversion\\
$q\to\infty$, finite &
...
$q\to\zeta$, order $d$ &
$\log(a;q)_\infty=-\Li_2(a^d)/(d^2t)+C_{d,\zeta}(a)+\OO(t)$\\
$a$ at fixed level near $\zeta$ &
$a\sim\omega(d^2Lt)^{1/d}$\\
complex order $\alpha\to\infty$ &
$\alpha=(\Log((1-q)w/a)-w/(1+q)+\cdots)/\Log q$\\
\bottomrule
\end{longtable}
