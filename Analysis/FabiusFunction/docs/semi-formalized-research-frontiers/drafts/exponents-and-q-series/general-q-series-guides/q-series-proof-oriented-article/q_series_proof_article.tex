...
\usepackage{geometry}
\geometry{a4paper,margin=27mm,headheight=15pt}
...
% LOCAL-NON-FOURIER-HAT: \BaileyTransformedAlpha denotes the alpha component of the weak Bailey-lemma transformed pair.
\newcommand{\BaileyTransformedAlpha}{\widehat{\alpha}}
% LOCAL-NON-FOURIER-HAT: \BaileyTransformedBeta denotes the beta component of the weak Bailey-lemma transformed pair.
\newcommand{\BaileyTransformedBeta}{\widehat{\beta}}
...
\textbf{Standing convention.} Unless a statement is explicitly finite or formal, $q\in\ComplexNumbers$ and $0<|q|<1$. Parameters are assumed to avoid displayed denominator zeros. Equalities first proved in a domain of absolute convergence extend meromorphically wherever both sides are defined.
}}
\end{center}
...
\[
 (a;q)_n\DefinitionEquals\prod_{j=0}^{n-1}(1-aq^j),\qquad (a;q)_0=1,
\]
and set
\[
 (a;q)_\infty\DefinitionEquals\prod_{j=0}^{\infty}(1-aq^j).
\]
Multiple parameters are abbreviated by
\[
 (a_1,\ldots,a_r;q)_n\DefinitionEquals\prod_{j=1}^{r}(a_j;q)_n,
\]
...
For $m\ge1$ define
\[
 (a;q)_{-m}\DefinitionEquals\frac{1}{(aq^{-m};q)_m}.
\]
\end{definition}
...
\begin{definition}
For integers $n\ge0$ and $k\in\IntegerNumbers$, define
\[
...
Despite the quotient notation, $\GaussianBinomial{n}{k}{q}$ is a polynomial in $q$ with nonnegative integer coefficients. This will follow both algebraically and combinatorially.

\begin{proposition}[Pascal recurrences and symmetry]\label{prop:q-pascal}
For $n\ge1$,
\begin{align}
 \GaussianBinomial{n}{k}{q}&=\GaussianBinomial{n-1}{k}{q}+q^{n-k}\GaussianBinomial{n-1}{k-1}{q},\label{eq:qpascal1}\\
 \GaussianBinomial{n}{k}{q}&=q^k\GaussianBinomial{n-1}{k}{q}+\GaussianBinomial{n-1}{k-1}{q},\label{eq:qpascal2}\\
 \GaussianBinomial{n}{k}{q}&=\GaussianBinomial{n}{n-k}{q}.\label{eq:q-symmetry}
\end{align}
\end{proposition}
...
\begin{equation}\label{eq:finite-plus-binomial}
 ...
 \sum_{k=0}^{n}q^{\binom k2}\GaussianBinomial{n}{k}{q} z^k.
\end{equation}
\end{theorem}
...
\begin{align*}
 (-1)^kq^{\binom k2}\GaussianBinomial{n}{k}{q}
 +(-1)^kq^nq^{\binom{k-1}2}\GaussianBinomial{n}{k-1}{q}
 &=(-1)^kq^{\binom k2}
   \left(\GaussianBinomial{n}{k}{q}+q^{n+1-k}\GaussianBinomial{n}{k-1}{q}\right)\\
 &=(-1)^kq^{\binom k2}\GaussianBinomial{n+1}{k}{q}
\end{align*}
...
\begin{align*}
 F_n(z)
 &=\sum_{k\ge0}\left(q^k\GaussianBinomial{n+k-1}{k}{q}
      +\GaussianBinomial{n+k-1}{k-1}{q}\right)z^k\\
 &=F_{n-1}(qz)+zF_n(z).
\end{align*}
...
\begin{equation}\label{eq:q-vandermonde}
 ...
 =\sum_{k\in\IntegerNumbers}q^{k(m-r+k)}
   \GaussianBinomial{m}{r-k}{q}\GaussianBinomial{n}{k}{q}.
\end{equation}
Equivalently,
\begin{equation}\label{eq:q-vandermonde-alt}
 \GaussianBinomial{m+n}{r}{q}
 =\sum_{k\in\IntegerNumbers}q^{(m-k)(r-k)}
   \GaussianBinomial{m}{k}{q}\GaussianBinomial{n}{r-k}{q}.
\end{equation}
\end{theorem}
...
\[
 ...
 =\sum_r q^{\binom r2}\GaussianBinomial{m+n}{r}{q}z^r.
\]
Split the product after $m$ factors. The second block is
\[
 \prod_{j=m}^{m+n-1}(1+zq^j)
 =\sum_k q^{\binom k2+mk}\GaussianBinomial{n}{k}{q} z^k.
\]
...
The coefficient of $q^N$ in $\GaussianBinomial{m+n}{m}{q}$ is the number of partitions of $N$ whose Ferrers diagram fits in an $m\times n$ rectangle.
\end{proposition}
...
\]
The polynomial $\GaussianBinomial{m+n}{m}{q}$ satisfies the same recurrence by \eqref{eq:qpascal1}, with the same boundary values. Induction on $m+n$ proves equality.
\end{proof}
...
\end{equation}
The convergence is locally uniform in $(a,z)$ on compact subsets of $\ComplexNumbers\times\{|z|<1\}$.
\end{theorem}
...
$\IntegerNumbers[A,B,C,D,E,F]$, and division by the common denominator proves
...
\begin{proof}
We first prove the identity at the infinite sequence $b=q^{K+1}$, $K\in\NaturalNumbers$. In that case the terms with $n<-K$ vanish. Put $n=j-K$ and use the shift law:
\begin{align*}
 \sum_{n\in\IntegerNumbers}\frac{(a;q)_n}{(q^{K+1};q)_n}z^n
...
\begin{align*}
 ...
 (q/z;q)_N&=\sum_{j=0}^N(-1)^jq^{\binom j2+j}\GaussianBinomial{N}{j}{q} z^{-j}.
\end{align*}
For a fixed power $z^k$ put $i=j+k$. Its coefficient is
\[
 (-1)^kq^{\binom k2}
 \sum_jq^{j(j+k)}\GaussianBinomial{N}{j+k}{q}\GaussianBinomial{N}{j}{q}.
\]
Use \eqref{eq:q-vandermonde} with $m=n=N$ and $r=N-k$:
\[
 \GaussianBinomial{2N}{N-k}{q}
 =\sum_jq^{j(j+k)}\GaussianBinomial{N}{N-k-j}{q}\GaussianBinomial{N}{j}{q}
 =\sum_jq^{j(j+k)}\GaussianBinomial{N}{j+k}{q}\GaussianBinomial{N}{j}{q}.
\]
Finally $\GaussianBinomial{2N}{N-k}{q}=\GaussianBinomial{2N}{N+k}{q}$ by symmetry.
\end{proof}

\begin{theorem}[Jacobi triple product]\label{thm:jtp}
For $z\ne0$,
\begin{equation}\label{eq:jtp}
 \J(z;q)\DefinitionEquals(q,z,q/z;q)_\infty
 =\sum_{k\in\IntegerNumbers}(-1)^kq^{\binom k2}z^k.
\end{equation}
The bilateral series converges absolutely and locally uniformly on $\ComplexNumbers^\times$.
\end{theorem}
...
\begin{align*}
 \J(-z;q)&=\sum_{r\in\IntegerNumbers}q^{r(r-1)/2}z^r,\\
...
\[
 q=\EulerE^{\pi\ImaginaryUnit\tau},\qquad \zeta=\EulerE^{2\pi\ImaginaryUnit z},
\]
and use the principal branch of $(-\ImaginaryUnit\tau)^{1/2}$.

\begin{definition}[Jacobi theta functions]
\begin{align}
 \vartheta_3(z\mid\tau)&=\sum_{n\in\IntegerNumbers}
   \EulerE^{\pi\ImaginaryUnit n^2\tau+2\pi\ImaginaryUnit nz}
   =\sum_{n\in\IntegerNumbers}q^{n^2}\zeta^n,\label{eq:theta3-def}\\
 \vartheta_4(z\mid\tau)&=\sum_{n\in\IntegerNumbers}
   (-1)^n\EulerE^{\pi\ImaginaryUnit n^2\tau+2\pi\ImaginaryUnit nz},\label{eq:theta4-def}\\
 \vartheta_2(z\mid\tau)&=\sum_{n\in\IntegerNumbers}
   \EulerE^{\pi\ImaginaryUnit(n+1/2)^2\tau+2\pi\ImaginaryUnit(n+1/2)z}.\label{eq:theta2-def}
\end{align}
\end{definition}

Normal convergence on compact subsets of $\ComplexNumbers\times\{\operatorname{Im}\tau>0\}$ permits termwise differentiation.
...
\[
 \vartheta_2(z\mid\tau)
 =q^{1/4}\zeta^{1/2}\sum_{n\in\IntegerNumbers}q^{n(n+1)}\zeta^n,
\]
...
\begin{equation}\label{eq:heat-equation}
 ...
 =\frac{1}{4\pi\ImaginaryUnit}\frac{\partial^2\vartheta_j}{\partial z^2}.
\end{equation}
\end{proposition}

\begin{proof}
For a term $\EulerE^{\pi\ImaginaryUnit(n+\alpha)^2\tau+2\pi\ImaginaryUnit(n+\alpha)z}$,
differentiation in $\tau$ multiplies by $\pi\ImaginaryUnit(n+\alpha)^2$, while
two differentiations in $z$ multiply by $-4\pi^2(n+\alpha)^2$.
Since $-4\pi^2/(4\pi\ImaginaryUnit)=\pi\ImaginaryUnit$, the terms agree. Normal convergence
...
For every Schwartz function $f:\RealNumbers\to\ComplexNumbers$, with Fourier transform
\[
\FourierTwoPi{f}(\xi)=\int_{\RealNumbers}f(x)\EulerE^{-2\pi\ImaginaryUnit x\xi}\,\Differential x,
\]
one has
\begin{equation}\label{eq:poisson}
\sum_{n\in\IntegerNumbers}f(n)=\sum_{k\in\IntegerNumbers}\FourierTwoPi{f}(k).
\end{equation}
\end{lemma}

\begin{proof}
The periodization $P(x)=\sum_{n\in\IntegerNumbers}f(x+n)$ and all its derivatives
...
\begin{align*}
 ...
 &=\sum_{n\in\IntegerNumbers}\int_0^1f(x+n)\EulerE^{-2\pi\ImaginaryUnit kx}\,\Differential x\\
 &=\int_\RealNumbers f(u)\EulerE^{-2\pi\ImaginaryUnit ku}\,\Differential u
=\FourierTwoPi{f}(k),
\end{align*}
because $\EulerE^{2\pi\ImaginaryUnit kn}=1$. The Fourier coefficients decay faster
than any power, so the Fourier series converges absolutely. Evaluating
$P(x)=\sum_k\FourierTwoPi{f}(k)\EulerE^{2\pi\ImaginaryUnit kx}$ at $x=0$ gives
\eqref{eq:poisson}.
\end{proof}
...
\begin{equation}\label{eq:gaussian-ft}
 \int_\RealNumbers \EulerE^{\pi\ImaginaryUnit\tau x^2+2\pi\ImaginaryUnit ux}\,\Differential x
 =(-\ImaginaryUnit\tau)^{-1/2}\EulerE^{-\pi\ImaginaryUnit u^2/\tau}.
\end{equation}
\end{lemma}

\begin{proof}
For $\tau=\ImaginaryUnit t$, $t>0$, and real $u$, complete the square and use the
ordinary Gaussian integral; this gives
$t^{-1/2}\EulerE^{-\pi u^2/t}$. Both sides of \eqref{eq:gaussian-ft} are
...
\begin{equation}\label{eq:theta3-modular}
 ...
 =(-\ImaginaryUnit\tau)^{1/2}\EulerE^{\pi\ImaginaryUnit z^2/\tau}\vartheta_3(z\mid\tau).
\end{equation}
At $z=0$,
\begin{align}
 \vartheta_2(0\mid-1/\tau)&=(-\ImaginaryUnit\tau)^{1/2}\vartheta_4(0\mid\tau),
 \label{eq:theta2-modular}\\
 \vartheta_3(0\mid-1/\tau)&=(-\ImaginaryUnit\tau)^{1/2}\vartheta_3(0\mid\tau),
 \label{eq:theta3-null-modular}\\
 \vartheta_4(0\mid-1/\tau)&=(-\ImaginaryUnit\tau)^{1/2}\vartheta_2(0\mid\tau).
...
$f(x)=\EulerE^{\pi\ImaginaryUnit\tau x^2+2\pi\ImaginaryUnit zx}$. By
...
\[
 \vartheta_3(z\mid\tau)
 =(-\ImaginaryUnit\tau)^{-1/2}\EulerE^{-\pi\ImaginaryUnit z^2/\tau}
 \vartheta_3\!\left(\frac z\tau\middle|-\frac1\tau\right),
\]
which is \eqref{eq:theta3-modular}. Setting $z=0$ gives
\eqref{eq:theta3-null-modular}.

For the shifted Gaussian $f(x)=\EulerE^{\pi\ImaginaryUnit\tau(x+1/2)^2}$, a translation
by $1/2$ multiplies its $k$th Fourier transform by
$\EulerE^{\pi\ImaginaryUnit k}=(-1)^k$. Poisson summation consequently gives
\[
 \vartheta_2(0\mid\tau)
...
\begin{align}
 ...
 \eta(\tau+1)&=\EulerE^{\pi\ImaginaryUnit/12}\eta(\tau),\label{eq:eta-T}\\
 \eta(-1/\tau)&=(-\ImaginaryUnit\tau)^{1/2}\eta(\tau).\label{eq:eta-S}
\end{align}
\end{theorem}
...
the infinite product depends only on $\EulerE^{2\pi\ImaginaryUnit\tau}$, while the
prefactor acquires $\EulerE^{\pi\ImaginaryUnit/12}$. For inversion, apply
...
constant. At $\tau=\ImaginaryUnit$ the transformation fixes $\tau$ and the
principal square root equals $1$, so $h(\ImaginaryUnit)=1$. This proves
\eqref{eq:eta-S}.
\end{proof}
...
\begin{equation}\label{eq:ramanujan-f}
 f(a,b)=\sum_{n\in\IntegerNumbers}
 a^{n(n+1)/2}b^{n(n-1)/2}.
\end{equation}
\end{definition}
...
For $|Q|<1$ define
\[
 \T(x;Q)=\sum_{n\in\IntegerNumbers}Q^{n^2}x^n.
\]
...
\begin{proof}
Expand the left side as a sum over $(n,m)\in\IntegerNumbers^2$. Partition the lattice
according to the unique residue $k\in\{0,\ldots,a+b-1\}$ for which
\[
 m-n=k+(a+b)s
\]
with $s\in\IntegerNumbers$. The change of variables
\[
 n=r-bs,\qquad m=k+r+as
\]
is a bijection from $\IntegerNumbers^2$ onto the lattice points in the $k$th class.
It gives
\begin{align*}
...
in the ring $\IntegerNumbers[[q]]$, where no analytic convergence is needed.
...
\[
 ...
 \left(\sum_{m\in\IntegerNumbers}(-1)^mq^{m(3m-1)/2}\right)=1.
\]
...
For $s\ge1$, let
\[
 r_s(n)=\#\{(x_1,\ldots,x_s)\in\IntegerNumbers^s:x_1^2+\cdots+x_s^2=n\}.
\]
Then, formally and analytically for $|q|<1$,
\begin{equation}\label{eq:rs-theta}
 \left(\sum_{m\in\IntegerNumbers}q^{m^2}\right)^s
...
The ring $\IntegerNumbers[\ImaginaryUnit]$ is a Euclidean domain for the norm
$N(x+\ImaginaryUnit y)=x^2+y^2$, and hence a unique-factorization domain.
For a rational prime $p$:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $2=-\ImaginaryUnit(1+\ImaginaryUnit)^2$;
\item if $p\equiv3\pmod4$, then $p$ is irreducible in $\IntegerNumbers[\ImaginaryUnit]$;
...
\begin{proof}
Given $\alpha,\beta\in\IntegerNumbers[\ImaginaryUnit]$ with $\beta\ne0$, choose a Gaussian integer
...
up to order and the units $\{\pm1,\pm\ImaginaryUnit\}$.
...
$p\mid(t+\ImaginaryUnit)(t-\ImaginaryUnit)$ in $\IntegerNumbers[\ImaginaryUnit]$.  The element $p$ cannot be irreducible:
...
$z=x+\ImaginaryUnit y$ of norm $n$.  By \cref{lem:gaussian-ufd}, the prime
...
where
\[
 A(x)=\sum_{s\in\IntegerNumbers}x^{s^2},\qquad
...
\[
 \prod_{\nu\ge1}\frac{1-x^\nu}{1+x^\nu}
 =\sum_{m\in\IntegerNumbers}(-1)^m x^{m^2}.
\]
...
$(\BaileyTransformedAlpha_n,\BaileyTransformedBeta_n)$, where
\begin{align}
\BaileyTransformedAlpha_n&=a^nq^{n^2}\alpha_n,\label{eq:weak-alpha}\\
\BaileyTransformedBeta_n&=\sum_{j=0}^{n}
...
\begin{align*}
 ...
 &=1+\sum_{n\ge1}(-1)^n
 q^{(5n^2-n)/2}(1+q^n)\\
 &=\sum_{m\in\IntegerNumbers}(-1)^m q^{m(5m-1)/2}.
\end{align*}
...
\[
 \sum_{m\in\IntegerNumbers}(-1)^m q^{m(5m-3)/2}.
\]
...
\begin{align}
 ...
 \RR(q)
 \DefinitionEquals q^{1/5}\frac{F_1(q)}{F_0(q)}
 &=
 q^{1/5}
...
substitution $q=\EulerE^{2\pi\ImaginaryUnit\tau}$.
...
\]
is interpreted in $\IntegerNumbers[[q]]$.  For every $N\ge0$,
...
$\RationalNumbers(A,B,C,D,E,F)$, the assertion proves the formal rational identity.  The
