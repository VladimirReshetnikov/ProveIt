...
% LOCAL-NON-FOURIER-HAT: \BaileyTransformedAlpha denotes the alpha component of the Bailey-chain transformed pair.
\newcommand{\BaileyTransformedAlpha}{\widehat{\alpha}}
% LOCAL-NON-FOURIER-HAT: \BaileyTransformedBeta denotes the beta component of the Bailey-chain transformed pair.
...
\begin{align}
\BaileyTransformedAlpha_n&=a^nq^{n^2}\alpha_n,\label{eq:chainalpha}\\
...
$(\BaileyTransformedAlpha,\BaileyTransformedBeta)$ at index $N$.  Let $N\to\infty$.  From
\eqref{eq:chainbeta},
\[
\BaileyTransformedBeta_N\longrightarrow
...
\[
\BaileyTransformedBeta_n=\frac1{(q;q)_n},\qquad
\BaileyTransformedAlpha_n=a^nq^{n^2}\alpha_n^{\mathrm{unit}}(a).
\]
...
=\int_{\RealNumbers}f(x)\EulerE^{-2\pi\ImaginaryUnit mx}\,\Differential x=\FourierTwoPi{f}(m).
\]
