...
Let $\tau$ lie in the upper half-plane $\mathbb H$ and put $Q=\EulerE^{\pi i\tau}$.  We use the theta constants
\begin{align*}
...
\eta(\tau)=\EulerE^{\pi i\tau/12}\prod_{n=1}^{\infty}(1-\EulerE^{2\pi in\tau})
...
\[
\FourierTwoPi{f}(\xi)=\int_{-\infty}^{\infty}f(x)\EulerE^{-2\pi\ImaginaryUnit x\xi}\,\Differential x.
\]

\begin{theorem}[Poisson summation for Schwartz functions]\label{thm:poisson}
If $f$ is a Schwartz function, then
\begin{equation}\label{eq:poisson}
\sum_{n\in\IntegerNumbers}f(n)=\sum_{m\in\IntegerNumbers}\FourierTwoPi{f}(m),
\end{equation}
...
\begin{align*}
\int_0^1P(x)\EulerE^{-2\pi imx}\,dx
&=\sum_{n\in\IntegerNumbers}\int_0^1f(x+n)\EulerE^{-2\pi imx}\,dx\\
&=\int_{-\infty}^{\infty}f(u)\EulerE^{-2\pi imu}\,du
=\FourierTwoPi{f}(m).
\end{align*}
Repeated integration by parts makes $\FourierTwoPi{f}(m)$ decrease faster than every power of $|m|$, so the Fourier series is absolutely and uniformly convergent.  Evaluating it at $x=0$ gives \eqref{eq:poisson}.
\end{proof}
...
\int_{-\infty}^{\infty}\EulerE^{-\pi tx^2}\EulerE^{-2\pi ix\xi}\,dx
...
\[
I'(\xi)=-2\pi i\int xe^{-\pi tx^2}\EulerE^{-2\pi ix\xi}\,dx
=-\frac{2\pi\xi}{t}I(\xi).
\]
Hence $I(\xi)=I(0)\EulerE^{-\pi\xi^2/t}$.  Squaring the elementary Gaussian integral gives $I(0)=t^{-1/2}$.  Both sides of \eqref{eq:gaussian-ft} are holomorphic for $\Re t>0$, so the identity theorem extends the formula to that half-plane.
\end{proof}
...
\begin{proof}
Take $t=-i\tau$, whose real part is positive.  Applying Poisson summation and \cref{lem:gaussian-ft} to $f(x)=\EulerE^{-\pi tx^2}$ gives
\[
\sum_{n\in\IntegerNumbers}\EulerE^{\pi i\tau n^2}
=(-i\tau)^{-1/2}\sum_{m\in\IntegerNumbers}\EulerE^{-\pi im^2/\tau}.
\]
Rearrangement proves \eqref{eq:theta3-S}.

The shifted Poisson formula
\begin{equation}\label{eq:shifted-poisson}
\sum_{n\in\IntegerNumbers}f(n+a)=\sum_{m\in\IntegerNumbers}\EulerE^{2\pi\ImaginaryUnit ma}\FourierTwoPi{f}(m)
\end{equation}
...
\]
which is equivalent to \eqref{eq:theta4-S}.  Applying ordinary Poisson summation to $\EulerE^{2\pi ix(1/2)}f(x)$ gives
\[
...
For $\tau\in\mathbb H$,
\begin{align}
\eta(\tau+1)&=\EulerE^{\pi i/12}\eta(\tau), \label{eq:eta-T}\\
\eta(-1/\tau)&=(-i\tau)^{1/2}\eta(\tau). \label{eq:eta-S}
\end{align}
\end{theorem}

\begin{proof}
Equation \eqref{eq:eta-T} follows directly from the product definition: $\EulerE^{2\pi in(\tau+1)}=\EulerE^{2\pi in\tau}$, while the initial exponential acquires $\EulerE^{\pi i/12}$.
...
\frac{a^{r_1+\cdots+r_{t-1}}
 q^{r_1^2+\cdots+r_{t-1}^2}}
{(q;q)_{n-r_1}(q;q)_{r_1-r_2}\cdots
 (q;q)_{r_{t-2}-r_{t-1}}(q;q)_{r_{t-1}}},
\end{equation}
...
\[
r_j=N_j\quad(1\le j\le K),
\qquad r_{K+1}=0.
\]
Then $n_j=r_j-r_{j+1}$, and the left side of \eqref{eq:AG} is
\begin{equation}\label{eq:AG-monotone}
\sum_{r_1\ge\cdots\ge r_K\ge0}
\frac{q^{r_1^2+\cdots+r_K^2+r_i+\cdots+r_K}}
{(q;q)_{r_1-r_2}\cdots(q;q)_{r_{K-1}-r_K}(q;q)_{r_K}}.
\end{equation}
We derive this sum from the unit Bailey pair relative to $q$.

\medskip
\noindent\textbf{Case 1: $i=1$.}
Apply $S_q$ exactly $K$ times, and then apply the limiting identity \eqref{eq:limiting-bailey}, still relative to $q$.  By \cref{lem:iterated-S}, the left side is exactly \eqref{eq:AG-monotone}: the final limiting sum supplies the outer variable $r_1$ and its weight $q^{r_1^2+r_1}$, while the preceding $K-1$ nested variables have the same quadratic-plus-linear weights.
...
The first $S_q$ applied to the delta beta sequence creates only the terminal factor $1/(q;q)_n$.  The remaining $t-1$ applications create the innermost variables, each weighted by $q^{r_j^2+r_j}$.  The shift $L_q$ leaves beta unchanged and changes the relative parameter to $1$.  The next $u$ applications create variables weighted by $q^{r_j^2}$, and the final limiting identity creates one more outer variable with weight $q^{r_1^2}$.  Since $u=i-2$, the variables with linear weight begin exactly at position $i$.  Thus the left side is \eqref{eq:AG-monotone}.
