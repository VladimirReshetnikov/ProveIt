...
An identity in $\C[[q]]$ is established once the coefficient of every $q^N$ agrees. In products such as $\prod_{m\ge1}(1-q^m)^{-1}$, a fixed coefficient involves only finitely many factors. Consequently, most partition identities below are simultaneously formal and analytic. Analytic language is needed chiefly when parameters other than $q$ vary or when modular transformations are considered.
\end{remark}
...
\begin{align}
\qbinom{n}{k}
 &=\qbinom{n-1}{k}+q^{n-k}\qbinom{n-1}{k-1}, \label{eq:qpascal1}\\
 &=q^k\qbinom{n-1}{k}+\qbinom{n-1}{k-1}. \label{eq:qpascal2}
\end{align}
In particular, all Gaussian coefficients lie in $\Z_{\ge0}[q]$.
\end{proposition}
...
\]
This proves the first recurrence; the second follows similarly or by the symmetry $\qbinom nk=\qbinom n{n-k}$. Starting with the boundary values $1$, either recurrence proves polynomiality and coefficientwise nonnegativity by induction on $n$.
\end{proof}
...
\prod_{j=0}^{N-1}(1+zq^j)
 &=\sum_{k=0}^{N}q^{\binom{k}{2}}\qbinom Nk z^k, \label{eq:finite-qbin-plus}\\
(z;q)_N
 &=\sum_{k=0}^{N}(-1)^kq^{\binom{k}{2}}\qbinom Nk z^k. \label{eq:finite-qbin-minus}
\end{align}
\end{theorem}
...
\begin{align*}
P_N(z)
&=\sum_k q^{\binom{k}{2}}\qbinom{N-1}{k}z^k
 +\sum_k q^{\binom{k}{2}+N-k}\qbinom{N-1}{k-1}z^k\\
...
\[
\frac1{(z;q)_N}=\sum_{m=0}^{\infty}\qbinom{N+m-1}{m}z^m.
\]
\end{corollary}
...
\begin{align*}
(1-zq^{N-1})R_N(z)
&=\sum_{m\ge0}\left(\qbinom{N+m-1}{m}-q^{N-1}\qbinom{N+m-2}{m-1}\right)z^m\\
&=\sum_{m\ge0}\qbinom{N+m-2}{m}z^m=R_{N-1}(z).
\end{align*}
Induction yields $(z;q)_NR_N(z)=1$.
\end{proof}

The coefficient of $q^m$ in $\qbinom nk$ counts partitions of $m$ whose Ferrers diagram fits inside a $k\times(n-k)$ rectangle. Such diagrams are in bijection with lattice paths from $(0,0)$ to $(n-k,k)$, and the recurrences in \cref{prop:q-pascal} separate the paths by their last step. This gives a second, purely combinatorial proof of positivity.
...
\[
\qbinom Nn=\frac{(q^{N-n+1};q)_n}{(q;q)_n}\longrightarrow\frac1{(q;q)_n}.
\]
...
=(q,z,q/z;q)_\infty.
\end{equation}
The series converges normally on compact subsets of $\C^\times$.
\end{theorem}
...
\]
It has an absolutely convergent Laurent expansion $F(z)=\sum_{n\in\Z}c_nz^n$.  Removing first factors gives
\[
...
c_n=(-1)^nq^{\binom n2}c_0\qquad(n\in\Z).
\end{equation}
...
\]
Substitution in \eqref{eq:jtp-coeff} proves \eqref{eq:jtp} on the annulus.  Both sides are holomorphic on $\C^\times$ and the series is normally convergent there, so analytic continuation proves the identity for all $z\ne0$.
\end{proof}
...
\begin{align*}
\vartheta_2(\tau)&=\sum_{n\in\Z}Q^{(n+1/2)^2},\\
\vartheta_3(\tau)&=\sum_{n\in\Z}Q^{n^2},\\
\vartheta_4(\tau)&=\sum_{n\in\Z}(-1)^nQ^{n^2},
\end{align*}
...
\[
\sum_{n\in\Z}Q^{n(n+1)}=(Q^2,-Q^2,-1;Q^2)_\infty
...
For an integrable function $f$ on $\R$, write
\[
...
\begin{proof}
The periodization $P(x)=\sum_{n\in\Z}f(x+n)$ and all its derivatives converge uniformly on $[0,1]$, because $f$ is rapidly decreasing.  Thus $P$ is a smooth $1$-periodic function.  Its $m$th Fourier coefficient is
\begin{align*}
\int_0^1P(x)e^{-2\pi imx}\,dx
&=\sum_{n\in\Z}\int_0^1f(x+n)e^{-2\pi imx}\,dx\\
...
\[
\sum_{n\in\Z}e^{\pi i\tau n^2}
=(-i\tau)^{-1/2}\sum_{m\in\Z}e^{-\pi im^2/\tau}.
\]
Rearrangement proves \eqref{eq:theta3-S}.

The shifted Poisson formula
\begin{equation}\label{eq:shifted-poisson}
\sum_{n\in\Z}f(n+a)=\sum_{m\in\Z}e^{2\pi ima}\widehat f(m)
\end{equation}
...
Let $r_d(n)$ denote the number of ordered $d$-tuples $(x_1,\ldots,x_d)\in\Z^d$ with $x_1^2+\cdots+x_d^2=n$.
...
\left(\sum_{m\in\Z}q^{m^2}\right)^d
...
\begin{align*}
\sum_{s=0}^{m}(-1)^{m-s}q^{\binom{m-s}{2}}\qbinom ms q^{hs}
&=q^{hm}\sum_{t=0}^{m}(-1)^tq^{\binom t2}\qbinom mt q^{-ht}\\
&=q^{hm}(q^{-h};q)_m=0,
\end{align*}
...
q^{j^2}\qbinom{N+1-j}{j}, \label{eq:P-polynomial}\\
Q_N&=\sum_{j=0}^{\lfloor N/2\rfloor}
q^{j^2+j}\qbinom{N-j}{j}. \label{eq:Q-polynomial}
\end{align}
...
\item \textbf{Jacobi's bilateral series.}  On a compact annulus $r\le|z|\le R$, the positive and negative tails are bounded by geometric multiples of $|q|^{n^2/3}$ for large $|n|$.  Hence normal convergence holds on $\C^\times$.
\item \textbf{Partition products.}  Every coefficient of $q^N$ uses only factors indexed by $m\le N$, so all combinatorial product manipulations are valid in $\Z[[q]]$ independently of analytic convergence.
