...
derivatives under their stated field hypotheses.  The division-free theorem
...
The companion \path{QPochhammerEntire.lean} retains exactly the five legacy
compatibility wrappers
\path{hasProdLocallyUniformly_complexQPochhammerInf},
\path{complexQPochhammerInf_differentiable},
\path{complexQPochhammerInf_eq_zero_iff}, and
...
every zero.  The generic-name analytic-order theorem assumes exactly
$\lVert q\rVert<1$ together with \path{qPochhammerInfIn a q = 0}, and also
includes $q=0$.  Neither API asserts joint holomorphy in \(q\), explicit
uniform-in-\(q\) tail or derivative-kernel bounds, or local uniformity of the
outer spectral product over \(k\).
\end{statusbox}
...
These five theorems concern one fixed Pochhammer symbol as a function of its
argument.  They do not by themselves establish locally uniform (normal)
convergence of the \emph{outer} product over $k$ on the right-hand side of
\eqref{p7:eq:sinc-poch}.  There is also still no named centered $\psi_q$ or
MGF wrapper, and no outside-$\AbsoluteValue q>1$ reciprocal formula or
zero--pole exchange.  Consequently the compound label
\cref{p7:thm:sinc-poch} remains only partially formalized: its fixed-symbol
inside-disk analytic and divisor layer is exact, while the centered/MGF
bridges, the outer-$k$ normal-convergence statement, and the outside-disk
reciprocal layer remain frontier obligations.
\end{statusbox}
