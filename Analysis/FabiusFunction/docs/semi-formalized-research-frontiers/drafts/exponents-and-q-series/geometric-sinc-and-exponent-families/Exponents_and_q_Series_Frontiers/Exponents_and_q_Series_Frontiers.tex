...
There is still no named centered $\psi_q$ or MGF wrapper, no
outside-$\AbsoluteValue q>1$ reciprocal formula or zero--pole exchange, and no named
local-uniform or normal-convergence theorem for the outer Pochhammer product
over the spectral index \(k\).  Consequently the compound label
\cref{p7:thm:sinc-poch} remains only partially formalized.
\end{statusbox}
