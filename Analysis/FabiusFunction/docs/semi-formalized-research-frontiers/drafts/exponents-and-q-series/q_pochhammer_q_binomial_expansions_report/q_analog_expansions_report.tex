...
$G_{n,k}=\qbinom nk|_{q=-1}$ and extend the Gaussian triangle by zero outside
$0\leq k\leq n$.  Reciprocity has the polynomial form
$\qbinom nk=q^{k(n-k)}\qbinombase{n}{k}{q^{-1}}$.  In an even row and an
...
ring in the module
\path|FabiusFunction.GaussianBinomialAtNegOne|.  Its four value theorems are:
\begin{itemize}[nosep,leftmargin=2em]
\item \path|gaussianBinomial_neg_one_even_even|;
\item \path|gaussianBinomial_neg_one_even_odd|;
\item \path|gaussianBinomial_neg_one_odd_even|;
\item \path|gaussianBinomial_neg_one_odd_odd|.
\end{itemize}
The two product identities
in~\eqref{eq:poc-minus-one-paired} are checked there as well.  The first-jet
formulas~\eqref{eq:minus-one-der-nonzero} and
\eqref{eq:minus-one-der-zero} remain the next formalization layer; this report
does not treat numerical checks as a substitute for that proof.
\end{remark}
