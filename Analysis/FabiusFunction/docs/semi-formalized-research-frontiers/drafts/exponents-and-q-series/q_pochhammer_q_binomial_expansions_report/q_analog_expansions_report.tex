...
Double scaling & Expansion for $q=\EulerE^{-\tau/N}$ and $k=\alpha N$ & Paper proof plus one spot check; not Lean\\
...
it, identities of logarithms would be determined only modulo $2\pi\ImaginaryUnit$.
...
\[
 ...
 =(-(1-q)x;q)_\infty.
\]
Let $h=1-q$.  For $0<q<1$ and $\AbsoluteValue{hx}<1$, with logarithms continued from
