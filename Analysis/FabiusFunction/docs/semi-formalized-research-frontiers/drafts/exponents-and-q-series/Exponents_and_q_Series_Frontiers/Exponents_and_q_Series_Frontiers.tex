...
\(m\in\N\setminus\{0\}\), \(A>0\), and \(\beta>0\),
\path{PowerExponentialLambertCurvature.lean} proves that both smooth branches
\(\lambda(x)\) of \(x=A\lambda^m\e^{-\beta\lambda}\) satisfy, for
\(0<x<P_{m,A,\beta}\),
\[
 \lambda''(x)=
...
\(z>-\e^{-1}\), and for \(k=-1\) with \(-\e^{-1}<z<0\), the nonsingular form
\[
 W_k''(z)=-\e^{-2W_k(z)}\frac{W_k(z)+2}{(W_k(z)+1)^3}.
\]
...
the full closed domain \([ -\e^{-1},\infty)\).  On the lower branch,
...
inflection at \(-2\e^{-2}\); the endpoint-inclusive strict convexity and
...
\(\bigO(z+\e^{-1})\) remainder, the subsequent Puiseux coefficients, a
...
\begin{align*}
 (uv;q)_n
 &=\sum_{k=0}^{n}\qbinom nkq (u;q)_k v^k(v;q)_{n-k},\\
 &=\sum_{k=0}^{n}\qbinom nkq (v;q)_k v^{n-k}(u;q)_{n-k},
\end{align*}
...
uncentered general-$q$ Fourier product.  For $q,z\in\C$ with
$\lVert q\rVert<1$, writing
$S_q(z)=\prod_{n\ge0}\sinc(\pi q^nz)$, the theorems
...
and entire-ness.  For real $\abs q<1$, $t\in\R$, and
...
\[
 C'_{n,k}=(-q)^{n-k}\qbinom nk{q^2},
 \qquad
 B_{n,k}=q^{(n-k)^2}\qbinom nk{q^2}
\]
...
$q,z\in\C$, $\lVert q\rVert<1$, the four theorems
\path{hasProdLocallyUniformly_geometricSincProduct},
\path{geometricSincProductFactors_multipliable},
\path{hasProd_geometricSincProduct}, and
...
$S_q(z)=\prod_{n\ge0}\sinc(\pi q^nz)$.  For real $\abs q<1$, $t\in\R$, and
...
outside-$\abs q>1$ reciprocal formula or zero--pole exchange, and no packaged
