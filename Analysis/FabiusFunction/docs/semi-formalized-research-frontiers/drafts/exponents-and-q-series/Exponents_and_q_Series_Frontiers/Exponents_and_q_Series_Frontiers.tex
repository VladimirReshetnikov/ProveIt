...
$\OrderOperator_{z=m}\Phi_{\mathbf1}=m_{\mathbf1}(\lvert m\rvert)$ for every nonzero integer $m$, chaining
...
\begin{equation}
 \boxed{\qquad \Sign\Phi_a(x)=\varepsilon_a(\lfloor x\rfloor).\qquad}
...
\begin{equation}
 \widehat f(\xi)=\int_{\RealNumbers} f(x)\EulerE^{-2\pi\ImaginaryUnit x\xi}\Differential x,
 \qquad
 f(x)=\int_{\RealNumbers}\widehat f(\xi)\EulerE^{2\pi\ImaginaryUnit x\xi}\Differential\xi.
...
Then
\begin{equation}
 \widehat{u_a}(\xi)=\SincRad(2\pi a\xi).
...
\begin{equation}
 \widehat g(t)=\int_{\RealNumbers}g(x)\EulerE^{-\ImaginaryUnit tx}\Differential x,
 \qquad
...
so that
\begin{equation}
 \Phi(t):=\widehat{\RvachevUp}(t)=\prod_{k=1}^{\infty}\SincRad\!\left(\frac{t}{2^k}\right),
 \qquad \Phi(0)=1,
...
converging to $\RvachevUp$.  Writing $\beta_a=\tfrac1{2a}\mathbf 1_{[-a,a]}$ for
...
With $P_n(t)=\prod_{k=1}^{n}\SincRad(t/2^k)=\widehat{p_n}(t)$,
\begin{equation}
 F_n(t):=\widehat{f_n}(t)=P_n(t)\SincRad(ht),
 \qquad
 \Phi(t)=P_n(t)\,\Phi(ht).
...
\begin{proof}
Legendre's formula $\TwoAdicValuation(M!)=M-s_2(M)$ gives \eqref{p4:eq:w-digit-sum}; the
pole count is
$\sum_{m\le M}\max\{\TwoAdicValuation(m)-1,0\}=\sum_{j\ge2}\lfloor M/2^j\rfloor
=\TwoAdicValuation(M!)-\lfloor M/2\rfloor$.  Since
$\TwoAdicValuation(m)=\sum_{j\ge1}\mathbf 1_{2^j\mid m}$, summation in $m$ gives both
...
Since $\widehat{\RvachevUp^{(2j)}}=(-1)^jt^{2j}\Phi$, the $p=2$ case transfers to
...
\begin{equation}
 ...
 \qquad
 f(x)=\frac1{2\pi}\int_{\RealNumbers}\widehat f(\xi)\EulerE^{\ImaginaryUnit\xi x}\Differential\xi.
 \label{p5:eq:fourier}
\end{equation}
...
\[
  \widehat h_a(\xi)=\prod_{j\ge 1}\SincRad(2\pi a^{-j}\xi),\qquad a>1,
\]
...
classical hierarchy $\widehat{\Fup_n}(t/2\pi)=\SincRad(t2^{-n-1})^{\,n}\,
\widehat\RvachevUp\bigl(t2^{-n}/2\pi\bigr)$: $\RvachevUp$ is reconstructed from $\Fup_n$ by
...
\begin{equation}\label{p6:eq:up-fourier-product}
 \widehat\RvachevUp(\xi)=\prod_{k=1}^{\infty}\SincRad(2\pi2^{-k}\xi).
\end{equation}
...
\begin{equation}\label{p6:eq:up-def-again}
 \RvachevUp=h_2,
 \qquad
 \widehat\RvachevUp(\xi)=\prod_{j\ge1}\SincRad(2\pi2^{-j}\xi),
\end{equation}
with
...
\begin{equation}\label{p6:eq:Gn-hat}
 \widehat G_n(t/2\pi)=H(t)\,\SincRad\bigl(t2^{-(n+1)}\bigr).
\end{equation}
...
\begin{equation}\label{p6:eq:Fupn-def}
 ...
 =\SincRad\bigl(t2^{-n-1}\bigr)^{\,n}\,
  \widehat\RvachevUp\bigl(t2^{-n}/2\pi\bigr),
...
\begin{equation}\label{p6:eq:cosine-multiplicity}
 \widehat\RvachevUp(t/2\pi)=\prod_{r=2}^{\infty}\cos\bigl(t2^{-r}\bigr)^{\,r-1}.
\end{equation}
\end{theorem}

\begin{proof}
By \eqref{p6:eq:Fupn-def},
$\widehat\RvachevUp/\widehat{\Fup_n}
...
$\chi_n(t)=\widehat\RvachevUp\bigl(2t/(2\pi s_n)\bigr)\SincRad(t/s_n)^n$ in the volume's
...
$|\widehat\RvachevUp|\le1$, $\max_{\delta\le|u|\le A}|\SincRad u|<1$, and
...
$\widehat\mu_{2,n,p}(t)\to\SincRad t$ by the classical cosine
...
$I_a=\int_\RealNumbers|\widehat h_a(t/2\pi)|^2\Differential t=2\pi\|h_a\|_2^2$, and put
$Z_a=T_a^2$.
...
$\int t^{2n}|\widehat h_a(t/2\pi)|^2\Differential t=2\pi\|h_a^{(n)}\|_2^2$, and the
...
=(-1)^n(2\pi)^{-1}\int t^{2n}|\widehat h_a(t)|^2\Differential t
...
limit,
$\Probability(p_aN_a>x)=(1-p_a)^{\lfloor x/p_a\rfloor+1}\to\EulerE^{-x}$ for every
...
$\Probability(\mathcal J_a\ge x)=(2/a)^{\lceil\nu_a(x)\rceil}$ with
...
\begin{equation}\label{p6:eq:general-gamma-zeta}
 \widehat\Psi_a(k)=\int_0^1\Psi_a(x)\,\EulerE^{-2\pi\ImaginaryUnit kx}\Differential x
