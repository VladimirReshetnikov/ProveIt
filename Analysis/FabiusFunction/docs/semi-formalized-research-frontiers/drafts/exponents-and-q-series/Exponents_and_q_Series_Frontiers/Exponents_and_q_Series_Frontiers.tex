...
\path{drafts/inverse-and-sampling/Non_Elementarity_of_the_Fabius_Function/};
...
require exactly $\lVert q\rVert<1$.  For arbitrary complex $q,z$ with
$\lVert q\rVert<1$, the next three results prove absolute summability of the
paired Euler perturbations, identify the geometric sinc product with the
product indexed by scale--zero pairs, and justify the index exchange
\[
 S_q(z)=\prod_{k\ge0}
 \left(\frac{z^2}{(k+1)^2};q^2\right)_{\!\infty}.
\]
The result includes $q=0$, negative and nonreal contractions, and every zero
of an individual factor.  The final two theorems specialize it to dyadic
scale and nome $1/4$; the Fourier-transform form additionally assumes a
bounded Fabius witness satisfying \path{IsFabius}.

Under $t=4\pi z$, the general Pochhammer-product theorem is exactly the
inside centered factorization in \eqref{p7:eq:sinc-poch}.  The complementary
sinc-product tranche supplies the analytic convergence layer: for
...
outside-$\AbsoluteValue q>1$ reciprocal formula or zero--pole exchange, and no packaged
compact-uniform theorem for the full phase-bearing characteristic prefixes.
Consequently the inside product identity is proved, while the compound label
\cref{p7:thm:sinc-poch} remains partial exactly at those wrappers and the
outside-disk clause.
\end{statusbox}
