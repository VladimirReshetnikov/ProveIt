...
The independent leaf \path{QBinomialCauchy.lean} proves the monograph's finite
q-Cauchy convolution exactly as
\lean{Fabius.finiteQPochhammerIn_mul_eq_sum_gaussianBinomial}.  Its parameters
\(q,u,v\) range over an arbitrary commutative ring and its length over
\(\N\); the statement is denominator-free and assumes no cancellation,
nonvanishing, injectivity, topology, or convergence.  This is an exact
counterpart only for the first q-Cauchy identity: the later Cauchy convolution
II and q-Pfaff--Saalsch\"utz summation remain outside the formalized surface.
...
\lean{Fabius.gaussianBinomial_inversion}.  They give both pointwise
compositions and the exact iff for
\texttt{[CommRing R] [AddCommGroup M] [Module R M]}, with no topology or
convergence.  Finally, \path{QBinomialInversionSpecializations.lean} has the
...
ring acting on an additive commutative group; \(q\) need not be nonzero or
invertible.
