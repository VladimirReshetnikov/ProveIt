...
\begin{statusbox}{Lean crosswalk: the fixed dyadic inside specialization}
...
Under $t=4\pi z$, the standalone-product theorem is exactly the inside
$q=1/2$ specialization of \eqref{p7:eq:sinc-poch}.  The general real
$\abs q<1$ identity, a named bridge to $\psi_q$ or $M_q$, the outside
$\abs q>1$ reciprocal formula and zero--pole exchange, and the asserted
local-uniform/normal convergence are not declarations of this module.
Consequently the full label \cref{p7:thm:sinc-poch} remains only partially
formalized.
\end{statusbox}
