...
The characteristic-function reading of (i) now has all three of its prerequisites, though not
the statement itself: $\Phi_a(0)=1$, continuity from entirety, and
\[
...
(i) --- that $R_a$ is the support radius rather than merely the number $A_a(1/2)$ --- the
...
half is, and needs no probability: by Paley--Wiener a law carried by $[-R,R]$ has a
characteristic function of exponential type at most $R$, and that growth bound follows from the
product directly,
\[
 \lVert\Phi_a(z)\rVert\le\exp\bigl(\pi\lVert z\rVert R_a\bigr),
 \qquad R_a=\sum_{h\ge0}a_h2^{-h},
\]
...
controlled by $R_a$, exactly as the support reading predicts.  The converse half --- that the
...
At the constant weight this says something checkable.  There $R_1=\sum_h2^{-h}=2$, so $\Phi$
...
$x/(C_Pp)\ge0$ with it.  The branch hypothesis is carried rather than derived: $W_{-1}$ satisfies
its defining equation only on $(-\e^{-1},0)$, and which $x$ land there depends on $C_P$, $p$ and
$q$ --- something settled here by context rather than by a formula, so the Lean statement asks
for it.  At $q=1$ the coordinate collapses to $-W_{-1}(-x/(C_Pp))$
...
\lean{Fabius.tendsto\_sum\_abs\_qToeplitzWeight}, with left-endpoint divergence
at one proved by
\lean{Fabius.tendsto\_qConditionNumberLimit\_atTop\_at\_one\_left}.  The conditionally convergent
boundary, the ordered one-sided sign and bound, the actual
exponent-generating-function instantiation, and the uniform estimates remain
results of this report rather than claims about the current Lean corpus.
...
\item \textbf{Analytic sinc logarithm (formalized; concrete quotient bridge open).}
...
with the actual quotient \(P_{b,n}/P_b\) in the analytic filter interface.
