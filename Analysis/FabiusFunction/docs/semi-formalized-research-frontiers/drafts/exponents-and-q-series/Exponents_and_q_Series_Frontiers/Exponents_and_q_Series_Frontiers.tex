...
Fourier envelopes for the Pascal hierarchy of rank at least two, Mellin contour
displacement, exponential Tauberian transfer, Berry--Esseen for
non-identically-distributed summands, total-variation and Wasserstein distances,
and the automatic-sequence machinery have no Lean counterpart here.
\end{statusbox}
...
The characteristic-function reading of (i) now has three of its necessary analytic
properties, though not the statement itself: $\Phi_a(0)=1$, continuity from entirety, and
\[
 \lVert\Phi_a(x)\rVert\le1
 \qquad\text{for every real }x
\]
(\lean{Fabius.norm\_generalizedRvachevProduct\_ofReal\_le\_one},
\path{GeneralizedRealBound.lean}), which had been missing at every weight, the constant one
included.  The bound is attained at $x=0$, so it is sharp.

What remains \emph{not} formalized is the probabilistic content proper: the support statement
(i) --- that the number $R_a=\tfrac12A(1/2)$ is the support radius --- the
...
$\Phi_a(0)=1$, but the hypothesis $m_a(0)=a_0$ need not hold, so that point is closed by a
...
(\lean{Fabius.one\_div\_one\_sub\_le\_qConditionNumberLimit}), hence
unbounded as $q\to1^-$, with $q=999/1000$ exhibited
...
(\lean{Fabius.pow\_div\_one\_sub\_pow\_at\_half}), so the series collapses to the plain
coefficient sum (\lean{Fabius.tsum\_newtonPoly}, \lean{Fabius.half\_mul\_tsum\_newtonPoly\_half}).

Reading that number as the \emph{support radius} of $\Phi_P$ is half formalized.  The analytic
half is, and needs no probability: with the Fourier kernel $\e^{-2\pi\ii zx}$, a law carried
by $[-R,R]$ has a characteristic function of conventional exponential type at most $2\pi R$.
The product directly satisfies the corresponding growth estimate
\[
 \lVert\Phi_a(z)\rVert\le\exp\bigl(2\pi\lVert z\rVert R_a\bigr),
 \qquad R_a=\frac12\sum_{h\ge0}a_h2^{-h}=\frac12A(1/2),
\]
...
at most $2\pi R_a$, exactly as the support reading predicts.  The converse half --- that the
type is no smaller, and that the law really is carried by $[-R_a,R_a]$ --- is the \emph{support}
...
At the constant weight this says something checkable.  There $A_1(1/2)=\sum_h2^{-h}=2$ and
$R_1=1$, so $\Phi$
has exponential type at most $2\pi$ (\lean{Fabius.norm\_rvachevFourierProduct\_le\_exp},
...
(\lean{Fabius.norm\_rvachevFourierProduct\_ofReal\_le\_one}), which the corpus had not had for
$\Phi$ either.
...
(\lean{Fabius.summable\_newtonPoly\_rpow}).  The cumulants \eqref{p1:eq:kappaP} remain
unformalized: they need the probabilistic model behind $\Phi_P$.
\end{theorem}
...
$x/(C_Pp)\ge0$ with it.  The theorem carries the interior hypothesis
$u\in(-\e^{-1},0)$ rather than deriving it, because its proof invokes that version of the
lower-branch defining equation; the branch-point API is available separately.  Which $x$ satisfy
the interior hypothesis depends on $C_P$, $p$ and $q$ --- something settled here by context rather
than by a formula.  At $q=1$ the coordinate collapses to $-W_{-1}(-x/(C_Pp))$
(\lean{Fabius.generalizedLambertCoordinate\_one}), the shape the classical $p=2$ analysis uses:
\lean{Fabius.lowerLambertW} (\lean{LowerLambertW.lean}), the corpus's totalized lower-branch
construction through \lean{Function.invFunOn}, inverts $w\mapsto w\e^w$ on
$(-\infty,-1]$, with
functional equation \lean{Fabius.lowerLambertW_mul_exp} and uniqueness
\lean{Fabius.lowerLambertW_unique}.  Thus the generalized coordinate
\eqref{p1:eq:generalized-lambert} has an exact Lean counterpart; the endpoint transseries
organized by that coordinate remains outside the formal boundary.
...
The cumulants \eqref{p1:eq:Psi-cumulants} and the variance \eqref{p1:eq:Psi-variance} remain
unformalized; both need the probabilistic model behind $\Psi_d$, which the corpus does not
have.
\end{theorem}
...
\begin{equation}
 \begin{array}{ccl}
 \tfrac12A(1/2) &:& \text{support radius},\\
...
and uniform estimates remain results of this report rather than claims about
the current Lean corpus.
...
(\lean{Fabius.one\_div\_one\_sub\_le\_qConditionNumberLimit}); it is unbounded as
$q\uparrow1$, and at $q=999/1000$ is already at least $1000$.

Identifying the coefficients of the transformed-error series with the
sinc-product tail, and proving the report's signs, remainder and bracketing
bounds, uniform spline limits, and inverse-function consequences, remain
unformalized.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{assets/finite_sinc_products_report/finite_sinc_approximants.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{assets/finite_sinc_products_report/scaled_error_profiles.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{assets/finite_sinc_products_report/convergence_comparison.pdf}
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.92\textwidth]{assets/Rvachev_Piecewise_Approximation_Fourier_Images/data/transfer_profile.pdf}
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.9\textwidth]{assets/Rvachev_Piecewise_Approximation_Fourier_Images/data/coefficient_asymptotic.pdf}
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.94\textwidth]{assets/Rvachev_Piecewise_Approximation_Fourier_Images/data/finite_spectra_log.pdf}
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.94\textwidth]{assets/Rvachev_Piecewise_Approximation_Fourier_Images/data/scaled_fourier_error.pdf}
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.94\textwidth]{assets/Rvachev_Piecewise_Approximation_Fourier_Images/data/accelerated_surrogates.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.91\textwidth]{assets/fabius_finite_products_frontier/figures/finite_spline_approximants.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.91\textwidth]{assets/fabius_finite_products_frontier/figures/scaled_density_error.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.91\textwidth]{assets/fabius_finite_products_frontier/figures/scaled_stop_loss.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{assets/fabius_finite_products_frontier/figures/exact_metric_rates.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{assets/fabius_finite_products_frontier/figures/information_transport_ratios.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\textwidth]{assets/fabius_finite_products_frontier/figures/fisher_information.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{assets/Atomic_Functions_Rvachev_Report_Package/figures/fup_edgeworth_error.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{assets/Atomic_Functions_Beyond_Dyadic_Frontiers/figures/derivative_energy_cantor_limit_a_3.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/Atomic_Functions_Beyond_Dyadic_Report-2/figures/spectral_lognormal_general_base.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{assets/Atomic_Functions_Rvachev_Report_Package/figures/derivative_gram_spectrum.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{assets/Atomic_Functions_Beyond_Dyadic_Expanded/figures/orthogonal_derivative_ladder_a_2_6.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{assets/Atomic_Functions_Beyond_Dyadic_Report/figures/tube_oscillation_a_3.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{assets/Atomic_Functions_Beyond_Dyadic_Report/figures/degree_critical_limit.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/Atomic_Functions_Beyond_Dyadic_Report-3/figures/distance_to_singular_set_a_3.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{assets/Atomic_Functions_Beyond_Dyadic_Report/figures/gaussian_cumulant_limit.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{assets/Atomic_Functions_Beyond_Dyadic_Report/figures/periodic_correction_bases.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/Fabius_Q_Connections_Report/figures/physical_densities.pdf}
