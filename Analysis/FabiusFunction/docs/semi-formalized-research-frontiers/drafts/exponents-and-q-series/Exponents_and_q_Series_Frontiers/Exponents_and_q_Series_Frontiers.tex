...
Write
\[
 \mathscr V(a)=\operatorname{span}_{\ComplexNumbers}\{S^ja:j\ge0\}.
\]
...
\[
 \ord_{z=\pm n}\Psi_d(z)=1+\binom{\TwoAdicValuation(n)}{d+1},
 \qquad n\ge1
\]
...
\begin{equation}
 \ord_{z=\pi m}A=1-\TwoAdicValuation(|m|).
...
For every nonzero integer $M$,
$\ord_{t=2^n\pi M}\bigl(F_n/\Phi\bigr)=1-\TwoAdicValuation(|M|)$: at the critical
...
frequency line, and uniformly on every compact subset of \(\R\), to
\(\phi_q\).  Their hypothesis is again exactly \(\abs q<1\), so this includes
...
$\overline{\operatorname{span}}\{\psi_{2j+\epsilon}\}_{j\ge0}\subset L^2(\RealNumbers)$
...
uniform convergence of the full phase-bearing prefixes on \(\R\) and its
...
\end{equation}
In particular $\ord_{z=n}\Phi_{1/4}(z)=1+\Floor{\TwoAdicValuation(n)/2}$: the quartic
