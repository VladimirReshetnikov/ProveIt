...
half is, and needs no probability: by Paley--Wiener a law carried by $[-S,S]$ has a
characteristic function of exponential type at most $2\pi S$ in the present Fourier
normalization, and that growth bound follows from the product directly,
\[
 \lVert\Phi_a(z)\rVert\le\exp\bigl(2\pi\lVert z\rVert S_a\bigr),
 \qquad S_a=\frac12\sum_{h\ge0}a_h2^{-h},
\]
...
controlled by $S_a$, exactly as the support reading predicts.  The converse half --- that the
type is no smaller, and that the law really is carried by $[-S_a,S_a]$ --- is the \emph{support}
...
At the constant weight this says something checkable.  There $S_1=\frac12\sum_h2^{-h}=1$, so $\Phi$
...
\lean{Fabius.lowerLambertW_unique}.
