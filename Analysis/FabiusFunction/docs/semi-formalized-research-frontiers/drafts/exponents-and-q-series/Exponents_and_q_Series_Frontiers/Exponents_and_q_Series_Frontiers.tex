...
\begin{statusbox}{Lean crosswalk: general sinc bridge and fixed dyadic Pochhammer specialization}
...
\path{complexQPochhammerInf} and exposes exactly the six theorems
...
require exactly $\lVert q\rVert<1$.  The spectral theorems themselves fix the
dyadic scale and hence the nome $1/4$, and are global in $z\in\ComplexNumbers$, including
at individual zero factors; the Fourier-transform form additionally assumes
a bounded Fabius witness satisfying \path{IsFabius}.

Under $t=4\pi z$, the standalone Pochhammer-product theorem is exactly the
inside $q=1/2$ specialization of \eqref{p7:eq:sinc-poch}.  A separate final
sinc-product tranche now proves the general uncentered bridge.  For
...
There is still no named centered $\psi_q$ or MGF wrapper, no declaration
identifying this sinc product with the general $q^2$-Pochhammer rearrangement,
and no outside-$\AbsoluteValue q>1$ reciprocal formula or zero--pole exchange.  The
normal convergence asserted for the Pochhammer double product and a packaged
compact-uniform theorem for the full phase-bearing characteristic prefixes
also remain outside the API.  Consequently the compound label
