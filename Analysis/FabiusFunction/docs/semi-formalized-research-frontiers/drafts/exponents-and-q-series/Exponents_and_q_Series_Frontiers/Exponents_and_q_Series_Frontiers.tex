...
The generic inverse package in \path{PowerExponentialLambertInverse.lean} is stronger still:
...
$\operatorname{InvOn}$ law with $\lambda\mapsto C_Pp\lambda^q\e^{-\lambda}$ on
$[q,\infty)$ and $(0,P_q]$.  At $q=1$ the coordinate collapses to
$-W_{-1}(-x/(C_Pp))$
(\lean{Fabius.generalizedLambertCoordinate\_one}), the shape the classical $p=2$ analysis uses:
\lean{Fabius.lowerLambertW} (\lean{LowerLambertW.lean}), the project's totalized construction
of the lower real branch, satisfies $W_{-1}(z)\e^{W_{-1}(z)}=z$ on
$[-\e^{-1},0)$, with functional equation
\lean{Fabius.lowerLambertW_mul_exp_of_mem_Ico} and uniqueness
\lean{Fabius.lowerLambertW_unique_of_mem_Ico}.  Thus the coordinate, its intrinsic
endpoint-inclusive solve law, and its exact inverse geometry have Lean counterparts; the
endpoint transseries organized by that coordinate remains outside the formal boundary.
