...
for an arbitrary base $b>1$ and weights in an arbitrary additive commutative monoid.  At the
product level, \lean{Fabius.generalizedRvachevProduct_two_mul}
(\lean{GeneralizedRvachevProduct.lean}) proves \eqref{p1:eq:shift-refinement}, and
\lean{Fabius.generalizedRvachevProduct_two_pow_mul}
(\lean{WeightLinearityProducts.lean}) proves the full iterated identity
\eqref{p1:eq:mshift-refinement} for every admissible $\NaturalNumbers$-valued exponent sequence
and every $m\ge0$.  The constant sequence $a_h\equiv1$ gives the earlier specializations
\lean{Fabius.rvachevFourierProduct_two_mul} and
\lean{Fabius.rvachevFourierProduct_two_pow_mul} (\lean{SincProductShells.lean}).  The generic
renormalization lemmas \lean{Fabius.tprod_geom_scale} and
\lean{Fabius.tprod_geom_scale_pow} (\lean{GeometricScaleProducts.lean}) remain the common
fixed-factor mechanism behind those specializations.  The finite-difference factorization
...
formalized.  Both dilation identities themselves are formalized for arbitrary admissible
$\NaturalNumbers$-valued exponents; \lean{Fabius.shiftExponent\_iterate} identifies the shifted
tail in the iterated theorem with $h\mapsto a_{h+m}$.
