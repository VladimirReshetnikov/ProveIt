...
\[
 \Phi(q,z)=\prod_{j\ge0}\sinc\bigl((1-q)q^jz\bigr),\qquad |q|<1.
\]
...
\[
 ...
 \sum_{n\ge1}\Li_2\!\left(\left(\frac{(1-\omega)z}{\pi n}\right)^{2d}\right).
\]
...
\end{equation}
has support $[-1,1]$ and density Rvachev's up-function $\upf$.  Its characteristic function, with the convention $\sinc z=\sin z/z$, is
\begin{equation}
 \widehat{\upf}(z)
 =\prod_{j\ge0}\sinc\!\left(\frac{z}{2^{j+1}}\right).
...
\begin{equation}
 \mathbb E\e^{\ii zY_q}
 =\prod_{j\ge0}\sinc\bigl((1-q)q^jz\bigr).
 \label{eq:real-char}
\end{equation}
...
For $q\in\D=\set{q\in\C:|q|<1}$ and $z\in\C$, set
\begin{equation}
 \Phi(q,z)=\prod_{j=0}^{\infty}
 \sinc\bigl((1-q)q^jz\bigr).
 \label{eq:Phi-def}
\end{equation}
\end{definition}

\begin{proposition}[Holomorphy and normalization]
The product in \cref{eq:Phi-def} converges locally uniformly on $\D\times\C$ and defines a holomorphic function there.  Moreover,
\[
 \Phi(q,0)=1,
 \qquad
 \Phi(0,z)=\sinc z,
 \qquad
 \Phi\left(\tfrac12,z\right)=\widehat{\upf}(z).
\]
\end{proposition}

\begin{proof}
On a compact set $|q|\le r<1$, $|z|\le R$, the $j$th argument is $O(r^j)$ uniformly.  Since $\sinc w=1-w^2/6+O(w^4)$, the deviation of the $j$th factor from one is $O(r^{2j})$.  The Weierstrass criterion gives locally uniform convergence of the product and of its holomorphic finite approximants.  The three normalizations follow directly.
\end{proof}
...
\[
 ...
 =\prod_{j\ge0}\sinc\!\bigl(\pi q^j u\bigr).
\]
...
\begin{equation}
 \sinc w=\prod_{n\ge1}\left(1-\frac{w^2}{\pi^2n^2}\right),
 \qquad
 \log\sinc w=-\sum_{k\ge1}c_kw^{2k},
 \label{eq:sinc-log}
\end{equation}
where
\begin{equation}
 ...
 \label{eq:ck}
\end{equation}
The logarithm is the branch normalized by $\log\sinc0=0$ in $|w|<\pi$.

\begin{theorem}[Master logarithm]
On the domain
\[
 \Omega=\set{(q,z)\in\D\times\C: |(1-q)z|<\pi},
\]
...
$q\in\C$ with $|q|<1$ and every $u\in\C$,
\[
 \operatorname{geometricSincProduct}(q,u)
 =\prod_{k\ge0}
...
\begin{equation}
 \omega=\exp\left(\frac{2\pi\ii h}{m}\right),
 \qquad (h,m)=1,
 \qquad m\ge2,
 \label{eq:omega}
\end{equation}
...
Let $K$ be a compact subset of $\set{z:|a_\omega z|<\pi}$.  Uniformly for $z\in K$,
\begin{equation}
 \boxed{
...
$N=\lfloor\delta^{-1/2}\rfloor$.  Uniformly when $k\le N$,
\begin{align*}
...
\[
 1-(1-\delta)^n\ge 1-\e^{-n\delta}
 \ge (1-\e^{-1})\min(1,n\delta)
\]
...
\begin{align*}
 ...
 \Li_2\!\left(\left(\frac{a_\omega z}{\pi n}\right)^{2d}\right).
\end{align*}
\end{proof}
...
\[
 \frac1d\sum_{r=0}^{d-1}\log\sinc(a_\omega\omega^rt)
...
\begin{proof}
Write $\omega=\e^{\ii\theta}$.  Since
\(
 1-\e^{\ii\theta}=-2\ii\e^{\ii\theta/2}\sin(\theta/2)
\),
\[
 a_\omega^{2d}=(-4)^d\e^{\ii d\theta}\sin^{2d}(\theta/2).
\]
If $m$ is odd then $d=m$ and $\e^{\ii d\theta}=1$, while $d$ is odd.  If $m$ is even then $d=m/2$, the primitive numerator $h$ is odd, and $\e^{\ii d\theta}=\e^{\ii\pi h}=-1$.  The signs follow.
\end{proof}
...
\begin{equation}
 \sgn\Aact_\omega(z)=\sgn(a_\omega^{2d}).
...
\[
 ...
 =\sigma x_n,
 \qquad
 \sigma\in\set{-1,1},\quad 0<x_n<1.
\]
For $0<x<1$, $\Li_2(x)>0$ and $\Li_2(-x)<0$.  Every summand in \cref{eq:spectral-dilog} therefore has sign $\sigma$, proving the claim.
\end{proof}
...
\(
 A:=\Aact_\omega(z)\in\R\setminus\set0
...
For every compact set $W\subset\C$ there are $C_W>0$ and $\eps_0>0$ such
...
For $\omega=\e^{2\pi\ii/3}$, $d=3$ and $A_\omega=-1/630$.  Moreover
\[
 C_{1,\omega}=-\frac{\ii}{2\sqrt3},
 \qquad
 C_{2,\omega}=-\frac{\ii}{20\sqrt3}.
\]
Hence
\begin{equation}
 \log\Phi\bigl(\omega(1-\eps^3),\sqrt\eps\,w\bigr)
 =\frac{w^6}{630}
 +\frac{\ii\eps w^2}{2\sqrt3}
 +\frac{\ii\eps^2w^4}{20\sqrt3}
 +O(\eps^3).
 \label{eq:m3}
\end{equation}
...
For $\omega=\ii$, $d=2$, $A_\omega=-1/180$, and $C_{1,\omega}=-\ii/6$.  Therefore
\begin{align}
 \log\Phi\bigl(\ii(1-\eps^2),\sqrt\eps\,w\bigr)
 =&\frac{w^4}{180}+\frac{\ii\eps w^2}{6}
 \label{eq:m4-first}\\
 &-\eps^2\left[
 \left(\frac1{360}-\frac{\ii}{90}\right)w^4
 +\frac{w^8}{18900}
 \right]+O(\eps^3).
 \label{eq:m4-second}
\end{align}
...
For $\omega=\e^{\pi\ii/3}$, $d=3$, $A_\omega=1/17010$,
\[
 C_{1,\omega}=-\frac{\ii}{6\sqrt3},
 \qquad
 C_{2,\omega}=\frac{\ii\sqrt3}{540}.
\]
Thus
\begin{equation}
 \log\Phi\bigl(\omega(1-\eps^3),\sqrt\eps\,w\bigr)
 =-\frac{w^6}{17010}
 +\frac{\ii\eps w^2}{6\sqrt3}
 -\frac{\ii\sqrt3\eps^2w^4}{540}
 +O(\eps^3).
 \label{eq:m6}
\end{equation}
...
For $h(z)=\sinc z$, only even coefficients occur and the first resonance is $r=2d$, recovering \cref{eq:blowup-limit}.  For $h(z)=1-z$ and a primitive $m$th root, $r=m$ and
\begin{equation}
 \prod_{j\ge0}
...
The limiting entire function $\e^{-A_\omega w^{2d}}$ has no zeros.  The zero cloud of the rescaled sinc product escapes to infinity, but its $2d$th reciprocal moment remains finite and becomes the exponential factor in the Hadamard limit.  All other reciprocal moments disappear.
\end{remark}
...
If $A_\omega<0$ (all odd $m$ and all $m\equiv0\pmod4$), the real-axis symbol $\e^{-A_\omega w^{2d}}$ grows.  It should not be interpreted as a tempered Fourier multiplier on $\R$.  It is nevertheless an entire function with canonical decaying sectors
\begin{equation}
 \set{w:\Re(A_\omega w^{2d})>0}.
...
\[
 ...
 \e^{-\tau A_\omega w^{2d}-\ii xw}\dd w
\]
converges absolutely and locally uniformly in $x\in\C$ for $\tau>0$.
...
Fix a nonzero target $y\in\C$ and a logarithm branch $\ell=\Log y$ with $\ell\ne0$:
\begin{equation}
 \ell=\Log y.
 \label{eq:ell}
\end{equation}
...
$w(\eps)\in w_0+\eps\C[[\eps]]$ which solves, coefficient by coefficient in
...
There is a unique formal series
\[
 \rho(s)\in -\frac{A_\omega}{\ell}+s\C[[s]],
...
$\ell+2\pi\ii k$ has $2d$ choices of $w_0$, and each choice produces one formal
...
The inverse $q$ experiment at $\omega=\ii$ fixes $y=1.1$ and solves for
\(q(z)=\ii(1-\delta(z))\).  The leading approximation has error $O(z^6)$; after the $C_{1,\omega}$ correction, the fitted exponent is $7.9960$, matching the predicted $O(z^8)$ remainder.  At $z=0.145$, the computed local $q$-coordinate was
\[
 \delta=2.57314407304007927\times10^{-5}
 +9.47348632511157983\times10^{-7}\ii,
\]
...
At an irrational point $q=\e^{2\pi\ii\alpha}$ there is no exact resonance, but denominators $1-q^{2k}$ can be arbitrarily small.  Diophantine properties of $\alpha$ should govern the boundary growth.

\begin{conjecture}[Diophantine phase transition]
For badly approximable $\alpha$, suitably Abel-regularized logarithms of $\Phi(r\e^{2\pi\ii\alpha},z)$ admit polynomial bounds as $r\uparrow1$ on a small frequency disk.  For Liouville $\alpha$, there exist frequency values with superpolynomial subsequential growth generated by near-resonant cumulants.
\end{conjecture}
...
$\ell\mapsto\ell+2\pi\ii$ caused by winding around $y=0$.  The rotation
$w_0\mapsto\e^{\pi\ii/d}w_0$ permutes the $2d$ roots on a fixed nonzero
...
\item \textbf{Sign and natural boundary.}  Formalize the sign of $a_\omega^{2d}$ and $\Li_2(\pm x)$ on $0<x<1$, then use the finite-order zero/pole contradiction.
...
Let $K\Subset\set{z:|a_\omega z|<\pi}$ and choose $\rho<1$ such that
\(
...
For resonant $k$, the cutoff $N=\lfloor\delta^{-1/2}\rfloor$ is deliberately
...
\begin{table}[H]
\centering
\caption{Leading constants for selected primitive roots $\omega=\e^{2\pi\ii/m}$.}
\begin{tabular}{@{}ccccc@{}}
\toprule
