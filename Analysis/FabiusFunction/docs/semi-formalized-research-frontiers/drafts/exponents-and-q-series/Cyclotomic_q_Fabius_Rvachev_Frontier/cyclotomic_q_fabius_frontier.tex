...
For each root $\omega$, each logarithm value $\ell+2\pi\ii k$, and each $2d$th root in \cref{eq:w0}, one obtains a local branch.  Analytic continuation around $y=0$ permutes these branches.  Thus the root-of-unity boundary is accompanied by a nontrivial inverse monodromy, not merely by a divergent scalar asymptotic.
\end{remark}
...
\(q(z)=\ii(1-\delta(z))\).  The leading approximation has error $O(z^6)$; after the $C_{1,\omega}$ correction, the fitted exponent is $7.9960$, matching the predicted $O(z^8)$ remainder.  At $z=0.145$, the exact radial coordinate was
\[
...
For generic target $y$, continuation around a simple zero of the finite-level sinc product acts by a transposition on inverse branches, while continuation around $y=0$ acts by the cyclic rotation $w_0\mapsto\e^{\pi\ii/d}w_0$.  In the infinite limit these generators produce a nontrivial braid representation compatible with the escaping zero cloud.
\end{conjecture}
