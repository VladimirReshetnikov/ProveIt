...
- [`q_series_and_inverse_analogs/`](q-pochhammer-and-inversion/q_series_and_inverse_analogs/),
  *q-Series and Inverse q-Analogs: A Proof-Oriented Synthesis*, is the one
  ...
  Its
  [`PROVENANCE.md`](q-pochhammer-and-inversion/q_series_and_inverse_analogs/PROVENANCE.md)
  records the five-publication merge surface and the earlier six-package
  inverse lineage. The completed
  [`source_concordance.csv`](q-pochhammer-and-inversion/q_series_and_inverse_analogs/source_concordance.csv)
  ...
  This package deliberately has no canonical publication PDF. All five
  superseded source-publication PDFs were removed instead of being relabelled
  as partial or donor renderings, and no replacement was generated by explicit
  user direction. PDF files retained beneath `assets/` are vector research
  figures, not manuscript builds. Consequently no page-count, font, clean-log,
  or visual-render claim is made for the merged master. Manuscript result
  labels and numerical checks remain distinct from Lean verification.
