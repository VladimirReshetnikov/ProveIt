...
- [`fabius_q_frontiers_report/`](fabius_q_frontiers_report/),
  *Parameter-Flow, Gaussian, and Large-Deviation Frontiers for the
  ...
  supplied. Its active ledger covers all 20 non-ledger payloads and now
  verifies after four CSV entries were refreshed for CRLF-to-LF normalization
  and the current source/PDF/README rows were synchronized. All five committed
  PDFs are readable and unencrypted (26 pages total). The main report has 33
...
- [`Continuous_Parameter_Edgeworth_and_q_Gevrey_Frontier/`](Continuous_Parameter_Edgeworth_and_q_Gevrey_Frontier/),
  *Continuous-Parameter Edgeworth Theory, Large Deviations, and Quadratic
  ...
  source lines), arrived on 2026-08-30 in direct-arrival commit
  ...
  refreshed for CRLF-to-LF normalization and the current
  source/PDF/README rows were synchronized. The main PDF has 33
  embedded/subset font rows, including six Libertinus rows and six Type-3 rows
  inherited from the three included vector figures; the standalone figure
  PDFs contain the same six Type-3 rows. Its title and abstract concern
...
- [`q-series-proof-oriented-article/`](q-series-proof-oriented-article/),
  *A Proof-Oriented Guide to q-Series: Shifted Factorials, Basic
  ...
  or ledger. The repository-generated two-entry `SHA256SUMS` verifies the
  current source and matching PDF; the TeX was already LF.
...
- [`q_series_from_first_principles/`](q_series_from_first_principles/),
  *q-Series from First Principles: Products, Basic Hypergeometric Sums,
  ...
  (30 letter-paper pp and 1,548 source lines at arrival; current artifact:
  28 A4 pp from 1,540 source lines), likewise arrived as a bare
  ...
  current files; its TeX was already LF.

- [`q_series_monograph/`](q_series_monograph/),
  *A Proof-Driven Guide to q-Series, Basic Hypergeometric Identities, Bailey
  Chains, and Rogers--Ramanujan Theory* (1,915 source lines at arrival;
  current artifact: 27 A4 pp from 1,910 source lines), arrived as a source-only
  bare directory in direct-arrival commit
  ...
  repository build. The live ledger now covers and verifies both current
  source and PDF.
...
- [`Cyclotomic_q_Fabius_Rvachev_Frontier/`](Cyclotomic_q_Fabius_Rvachev_Frontier/),
  *Cyclotomic Blow-Ups and Natural Boundaries for the q-Fabius--Rvachev Sinc
  ...
  22-entry active ledger now verifies in full. The five committed PDFs have 32
  pages in total (28 main plus four one-page figures). The main report has 31
  embedded/subset font rows, including four Libertinus rows and nine Type-3
  figure-font rows; the standalone figures contain the same nine Type-3 rows.
  Figure-font normalization remains deferred.
...
1,475-line source and matching 26-page A4 PDF),
...
(current matching artifact: 237 A4 pp, 6,313,577-byte PDF, and 16,235-line TeX
across seven parts; both parent ledgers verify the current source and PDF) — the
...
(current matching artifact: 213-page A4 PDF, book class, 13,117-line source,
and 1,582,997-byte PDF; rebuilt after the merged sinc--Pochhammer crosswalk
update) —
