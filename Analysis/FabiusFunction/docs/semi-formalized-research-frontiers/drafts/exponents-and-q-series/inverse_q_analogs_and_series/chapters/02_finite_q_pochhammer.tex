\part{Finite and infinite \texorpdfstring{\(q\)}{q}-Pochhammer inversion}
...
\end{enumerate}
These intervals exhaust the real inverse branches.
\end{theorem}
...
listed intervals partition the real line after the roots are included as
endpoints, so there are no further real branches.
\end{proof}
...
\end{equation}
Hence, on the ramified cover of a sufficiently small punctured target
neighborhood, the inverse at target zero has exactly \(\nu_s\) local Puiseux
sheets, coalescing at \(a_s\).  Equivalently, after choosing a
\(\nu_s\)-th root on a simply connected punctured target domain, these are the
\(\nu_s\) determinations obtained by multiplying that root by the roots of
unity.
...
\cref{fp-root-multiplicity}.  Removing the nonvanishing factors leaves a
nonzero constant times \((a-a_s)^{\nu_s}\), whose equation with \(y\) has
\(\nu_s\) Puiseux determinations.
...
\subsection{All-order inversion at \(q=0\)}
...
\subsection{The complete logarithmic jet at \(q=1\)}
...
\subsection{The exceptional slice \(q=-1\)}
...
\(P_n(a,-1)=(1-a)^{\lceil n/2\rceil}(1+a)^{\lfloor n/2\rfloor}\) gives the
two critical values.
...
\]
has at \(q=-1\) a zero of exact order \(r=\lfloor n/2\rfloor\), because
exactly the odd-indexed factors vanish and each does so simply.  More
precisely, with \(e=\lceil n/2\rceil\) and
