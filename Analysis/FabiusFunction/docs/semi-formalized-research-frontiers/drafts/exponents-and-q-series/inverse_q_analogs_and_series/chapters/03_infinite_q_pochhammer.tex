...
\section{Inversion in the base at \(q=0\) and on the real interval}
...
\section{Fixed-argument inversion as \(q\to1\)}
...
of \(\zeta(s+1)\) cancels the gamma pole.  Standard vertical bounds for
\(\Gamma(s)\), the Riemann zeta function, and the Hurwitz zeta function bound
the shifted contour by the next power of \(t\), uniformly for \(x\) on the
stated compact sets.  This proves the sectorial Poincaré expansion.
\end{proof}
...
\section{Fixed-target double scaling at \(q=1\)}
...
\begin{proof}
For \(d=2\), split the Lambert indices into even and odd classes.  Applying
the Bernoulli expansion through fifth order and summing the resulting
geometric derivatives gives the displayed rational coefficients; equivalently
one may use the exact pairing
\[
 (a;-\EulerE^{-t})_\infty
 =(a;\EulerE^{-2t})_\infty(-a\EulerE^{-t};\EulerE^{-2t})_\infty
\]
and apply \cref{thm:ip-fixed-a-expansion} to both factors.  The odd powers
shown are the terms that remain after cancellation.  Coefficient comparison
in \(X=A_-/t+D_-t-E_-t^3+\BigO(t^5)\) gives the inverse coefficients, and the
same derivative/Rouché estimate as in
\cref{thm:ip-fixed-a-inverse} improves the forward remainder by two inverse
powers.  The exclusion \(a=0\) is necessary: there the product is identically
one and \(A_-=0\).
\end{proof}
...
solution germ tending to zero lies on one of these circles.  This proves the
exact germ count and both asymptotic relations.
