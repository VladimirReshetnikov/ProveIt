...
\(\widehat Q\in\ComplexNumbers\) and \(\varepsilon_Q\ge0\) with
\(\lvert Q-\widehat Q\rvert\le\varepsilon_Q\).  If the closed disk
\[
 \left\{z:\left\lvert z-\frac{\widehat Q}{2\pi\ImaginaryUnit}\right\rvert
...
\(\lceil x\rceil\); if \(x\in\NaturalNumbers\), the minimizing indices are \(x\) and
\(x+1\).
\end{proposition}
...
\subsection{Generic monodromy of finite \(q\)-products}
...
\subsection{The minimum of the \(q\)-gamma function}
...
\begin{longtable}{@{}p{0.21\textwidth}p{0.45\textwidth}p{0.29\textwidth}@{}}
\toprule
