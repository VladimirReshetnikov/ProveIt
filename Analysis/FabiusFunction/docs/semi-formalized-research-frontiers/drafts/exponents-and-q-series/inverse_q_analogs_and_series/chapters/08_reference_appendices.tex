...
In the checked snapshot those fields remain unpopulated in all \(260\) rows.
Thus the ledger is already exhaustive as a source-result inventory, but its
row-by-row editorial disposition audit is not yet mechanically closed.  A
blank canonical label must therefore be read as \emph{awaiting recorded
disposition}, never as \emph{false}, \emph{deleted}, or \emph{Lean-proved}.
The regenerating audit is
\lean{audit/extract_source_results.py}; regeneration must not overwrite
reviewed disposition data without first preserving and rejoining those
columns.
...
\item selected finite \(q\)-Pochhammer and Gaussian-polynomial identities,
  including reciprocity and exact values at \(q=-1\);
