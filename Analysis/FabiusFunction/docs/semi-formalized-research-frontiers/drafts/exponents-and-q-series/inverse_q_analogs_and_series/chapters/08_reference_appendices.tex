...
paths in \cref{tab:ref-source-packages} are relative to the surrounding
\lean{exponents-and-q-series} directory.  The row counts are the numbers of
theorem-like source environments recorded in the machine-readable concordance.

\begin{table}[p]
\centering
\small
\begin{tabularx}{0.98\textwidth}{@{}p{0.34\textwidth}rX@{}}
\toprule
...
\centering
\small
\begin{tabularx}{0.98\textwidth}{@{}p{0.27\textwidth}X@{}}
\toprule
...
The extraction script deliberately generates the five canonical-disposition
columns empty, because text matching cannot decide mathematical equivalence.
The reviewed ledger is now mechanically closed: all \(260\) source rows have
an explicit status and disposition.  Its status census is five
\emph{Lean-proved}, \(143\) \emph{human-proved frontier result}, fifteen
\emph{conjecture}, and \(97\) \emph{not applicable}.  The historical extractor
\lean{audit/extract_source_results.py} requires a checkout of the six
pre-retirement source packages at repository snapshot \texttt{f46e5d7f6}; it
is not a current-tree regenerator after consolidation.  Any replay must write
to a temporary file and preserve and rejoin the five reviewed editorial
columns rather than overwrite the canonical ledger.
...
\begingroup
\small
\begin{longtable}{@{}p{0.22\textwidth}p{0.29\textwidth}p{0.42\textwidth}@{}}
...
\addlinespace
\(\phi_q(t)\), sinc products
& Characteristic functions of the normalized geometric-uniform law and the
  Fabius--Rvachev member.
...
\centering
\small
\begin{tabularx}{0.98\textwidth}{@{}p{0.24\textwidth}p{0.30\textwidth}X@{}}
\toprule
...
& \Cref{sec:uq-regular-germs,sec:uq-sensitivities,fp-finite-covering,
sec:ip-argument-atlas,sec:certification-frontiers}.\\
\addlinespace
\lean{inverse_q_analog_jet_atlas}
& Triangular coefficient recurrences, Lagrange inversion, Bell polynomials,
mixed jets, multivariate and Good inversion.
& \Cref{sec:uq-formal-inversion,sec:uq-lagrange-burmann,
sec:uq-bell-packaging,sec:uq-multivariate-recursion,
sec:uq-lagrange-good}.\\
\addlinespace
\lean{inverse_q_analogs_extended_report}
& Exceptional finite bases, \(q=0,\pm1\), radial cyclotomic inversion,
\(q\)-special functions, certification, Fabius--Rvachev recovery.
& \Cref{fp-base-inversion,fp-minus-one-section,sec:ip-real-base,
sec:ip-cyclotomic,sec:qs-qgamma,sec:fr-geometric-law,
sec:certification-frontiers}.\\
\addlinespace
\lean{inverse_q_analogs_report}
& Discriminants, real gaps and remote sheets, \(q\)-special inverses,
certification algorithms, conjecture register.
& \Cref{fp-finite-covering,sec:ip-argument-atlas,thm:ip-remote-gaps,
sec:qs-qdigamma,sec:certification-frontiers,sec:precise-open-frontiers}.\\
\addlinespace
\lean{inverse_q_analogs_report-2}
& Reciprocal regimes, roots of unity, compact inverse tables, independent
algorithmic derivations.
& \Cref{fp-base-inversion,sec:ip-fixed-a-qone,sec:ip-cyclotomic,
sec:gaussian-infinity,sec:qs-qexponentials,
sec:certification-frontiers}.\\
\addlinespace
\lean{q_pochhammer_q_binomial_expansions_report}
...
\(q\)-gamma and \(q\)-beta.
& \Cref{fp-finite-covering,sec:ip-fixed-a-qone,sec:ip-qx,
sec:gaussian-one,sec:qmultinomial,sec:qs-qgamma,sec:qs-qbeta}.\\
\bottomrule
\end{tabularx}
...
\item selected finite \(q\)-Pochhammer and Gaussian-polynomial identities,
  including reciprocity, exact values and first derivatives at \(q=-1\), and
  multiplicity one for the alternating zero over characteristic-zero rings;
...
named Bell and quadratic support, or for the human proofs of infinite-product
normal convergence, remote-gap and log-periodic asymptotics, all-order
...
are the six packages in \cref{tab:ref-source-packages}, the package-level
ledger \lean{PROVENANCE.md}, the editorial contract \lean{README.md}, the
row-level ledger \lean{theorem_concordance.csv}, and the reproducible extractor
\lean{audit/extract_source_results.py}.  Repository history is the archival
record for superseded layouts and retired duplicates.
