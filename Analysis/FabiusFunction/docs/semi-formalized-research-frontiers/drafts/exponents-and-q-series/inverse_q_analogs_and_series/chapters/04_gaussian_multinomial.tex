\part{Gaussian and multinomial \(q\)-analogs}
...
\[
  c_2=\mathbf 1_{k\ge2}+\mathbf 1_{b\ge2},
  \qquad
  c_3=\mathbf 1_{b\ge3}
      +\mathbf 1_{k\ge2,\,b\ge2}
      +\mathbf 1_{k\ge3}.
\]
...
\section{Complete expansion and inversion at \(q=1\)}
...
\(\log((1-\EulerE^{-x})/x)\).  Termwise integration from \(0\) to \(x\)
gives the formula; the integration constant is zero because the quotient
tends to one.
\end{proof}

\begin{theorem}[Complete Gaussian logarithmic expansion]
\label{thm:gq-one-log}
Let \(q=\EulerE^{-t}\).  For \(|t|<2\pi/n\),
\[
...
\[
  \left\lfloor\frac{k+b}{m}\right\rfloor
  -\left\lfloor\frac{k}{m}\right\rfloor
  -\left\lfloor\frac{b}{m}\right\rfloor\in\{0,1\}
\]
...
\begin{proposition}[Value and zero derivative at \(q=-1\)]
...
\[
  ...
  =-\frac{D(\bm n)}2t+
   ...
   \frac{B_{2r}\Delta_{2r}(\bm n)}
        {2r(2r)!}\,t^{2r}.
\]
After multiplication by \(\EulerE^{D(\bm n)t/2}\), the normalized polynomial is
even and its inverse at the minimum has the two Puiseux branches of
\cref{prop:gq-centered-puiseux}.
\end{theorem}

\begin{proof}
Apply \cref{lem:gq-bernoulli-factor} to the numerator
\(\QPochhammer{q}{q}{n}\) and every denominator
\(\QPochhammer{q}{q}{n_i}\).  Constants give the ordinary multinomial
...
\]
The coefficient distribution is palindromic with positive equal endpoints,
so \cref{thm:gq-centered-palindromic,prop:gq-centered-puiseux} applies.
\end{proof}

\begin{theorem}[Cyclotomic carry and exact ramification]
\label{thm:qm-carry}
Let \(\zeta\) be a primitive \(m\)-th root of unity and write
\(n_i=m\ell_i+r_i\) with \(0\le r_i<m\).  The multiplicity of \(\zeta\) as
a zero of \(M_{\bm n}\) is
\[
  e_m
  =\left\lfloor\frac nm\right\rfloor
   -\sum_{i=1}^{s}\left\lfloor\frac{n_i}{m}\right\rfloor
  =\left\lfloor\frac{r_1+\cdots+r_s}{m}\right\rfloor.
\]
