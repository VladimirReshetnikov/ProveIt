...
Let $K$ be a field, let $N\in\NaturalNumbers$, and let
$u_0,\ldots,u_N,v_0,\ldots,v_N\in K$.  Put $w_k=u_k-v_k$ and assume
$w_0v_1\cdots v_N\ne0$.  Then
\begin{equation}
...
$\RationalNumbers(A,B,C,D,E,F)$ one has
\begingroup
\small
\begin{align}
&1-
...
\label{qg:eq:jackson-seed}
 J(A;B,C,D,E,F)
 =R(A;B,C,D,E,F)\,J\bigl(\sigma(A;B,C,D,E,F)\bigr).
\end{equation}
...
\[
 R(A;B,C,D,E,F)\frac{D_0}{D_1}=\lambda.
\]
...
\begin{align*}
&R(A;B,C,D,E,F)\,R\bigl(\sigma(A;B,C,D,E,F)\bigr)\\
&\quad=
...
&\qquad\times
 ...
 {(1-A^2/(BDEF))(1-A^2/(CDEF))}.
\end{align*}
Thus $J=R(R\circ\sigma)(J\circ\sigma^2)$ is precisely
...
\begin{proof}
Work first in the field $K=\RationalNumbers(q,a,b,c,d)$.  Let $F(n,k)$ be the
...
\end{align*}
Set $F(n,k)=0$ outside $0\le k\le n$.  Define
\begin{align*}
...
\label{qg:eq:jackson-direct-difference}
 F(n+1,k)-F(n,k)=\mathcal C_{n,k}\mathcal B_{n,k}
 \qquad(0\le k\le n+1).
\end{equation}
...
(1-aq^{n+k+1})}.
\end{align*}
Multiplying by the displayed formula for $F(n,k)$ and cancelling one initial
and one terminal factor in each shifted factorial gives
\eqref{qg:eq:jackson-direct-difference}.  For $k=n+1$ one has
$(q^{-n};q)_{n+1}=0$, so $F(n,n+1)=0$; the second summand of
...
\]
At $n=0$ only $k=0$ occurs and $F(0,0)=1$.  Hence the normalized sum is $1$
