...
the product of these two bilateral series.  Normal convergence on compact
annuli permits coefficient extraction.  The coefficient of $z^m$ is
\begin{equation}
 q^{m(m-1)/2}\sum_{s\in\IntegerNumbers}(-1)^s
 q^{3s^2+(1-2m)s}.
\label{eq:qg-quintuple-coefficient}
\end{equation}
Apply the minus-sign form of the triple product with base $Q=q^6$.
Quasi-periodicity gives, when $m=3n$,
\[
...
calculation gives
$(-1)^nq^{n(3n+1)/2}(q^2;q^2)_\infty$.  When $m=3n+2$, the remaining
Jacobi factor is evaluated at $Q^{-n}$ and vanishes, because the product
contains the factor $1-Q^0$ after an integral quasi-periodic shift.
...
$f(a,b)=1+a+b$.  With the stated zero-base convention, the product side is
\[
...
\[
 \mathcal T(x;Q)=\sum_{n\in\IntegerNumbers}Q^{n^2}x^n.
\]
...
\begin{align}
 \mathcal T(x;q^a)\mathcal T(y;q^b)
 &=\sum_{k=0}^{a+b-1}y^kq^{bk^2}
 \mathcal T(xyq^{2bk};q^{a+b})\notag\\
 &\qquad\qquad\times
 \mathcal T(y^ax^{-b}q^{2abk};q^{ab(a+b)}).
...
\item if $p\equiv1\pmod4$, then $p=\pi\bar\pi$ for a Gaussian prime
      $\pi$, unique up to a unit and conjugation.
\end{enumerate}
\end{lemma}
...
$p=\pi\rho$ with nonunits.  Norms force
$N(\pi)=N(\rho)=p$.  An element of prime norm is irreducible, so $\pi$ and
$\rho$ are Gaussian primes.  Moreover,
\[
 \pi\bar\pi=N(\pi)=p=\pi\rho,
\]
and cancellation gives $\rho=\bar\pi$.  If
$p=\sigma\bar\sigma$ is another such factorization, uniqueness of
factorization applied to $\pi\bar\pi=\sigma\bar\sigma$ shows that $\sigma$
is associated either to $\pi$ or to $\bar\pi$.  This proves uniqueness up to
...
$p_j=\pi_j\bar\pi_j$, the exponent of $\pi_j$ may be any
$e_j\in\{0,\ldots,\alpha_j\}$ and the exponent of $\bar\pi_j$ is then
