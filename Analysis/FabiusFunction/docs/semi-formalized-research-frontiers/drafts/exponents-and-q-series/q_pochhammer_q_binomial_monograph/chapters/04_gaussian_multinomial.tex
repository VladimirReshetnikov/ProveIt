...
\begin{remark}[Partial Lean crosswalk]
The universal-polynomial theorem
\nolinkurl{gaussianBinomial_eq_eval₂_universal} supplies the nonnegative
integral coefficients.  The five public theorems of
...
\nolinkurl{coeff_gaussianBinomial_universal_symm}---give exact degree,
monicity, constant coefficient one, reflection, and coefficient
palindromicity for every admissible column.  No current declaration proves
that the coefficient of \(q\) is one for the strict interior range
\(0<k<n\).  Consequently
\cref{prop:gq-positive-palindromic} remains a human-proved, partially
formalized proposition rather than an exact Lean row.
\end{remark}
