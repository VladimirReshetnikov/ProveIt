...
The law, exact mean, support, CDF refinement, smooth density, and half-base
...
\lean{integral_id_geometricUniformDistribution_eq_one_half} proves the mean
for every real \(\lvert q\rvert<1\), including zero and negative ratios, while
...
For real \(t\),
\begin{equation}
 \CharacteristicFunction{Z_q}(t)\DefinitionEquals
 \Expectation\EulerE^{\ImaginaryUnit tZ_q}
 =\EulerE^{-\ImaginaryUnit t}\varphi_q(2t)
 =\prod_{j=0}^{\infty}\SincRad\bigl((1-q)q^jt\bigr),
 \label{eq:fr-sinc-product}
\end{equation}
The function \(t\mapsto\CharacteristicFunction{Z_q}(t)\) has the log-Gaussian,
hence arbitrary-power, decay of
...
\begin{equation}
 \partial_q\log\CharacteristicFunction{Z_q}(t)
 =\sum_{j\ge0}w_j'(q)
...
\begin{equation}
 \sup_{q\in K}\left|\partial_q^k\CharacteristicFunction{Z_q}(t)\right|
 \le C_{A,k,K}(1+|t|)^{-A}.
