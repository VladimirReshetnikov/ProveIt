...
\(\OrderOperator_q G=\min\{d:g_d\ne0\}\), with
\(\OrderOperator_q 0=\infty\).  The congruence
...
\(\OrderOperator_q S_m\to\infty\), in the precise sense that for each
\(N\) only finitely many \(m\) have
\(\OrderOperator_q S_m\le N\).  Then the \(q\)-adic sum
\(S=\sum_{m\ge0}S_m\) is well defined and
\begin{equation}
 S(q)\equiv
 \sum_{\OrderOperator_q S_m\le N}S_m(q)
 \pmod{q^{N+1}}.
