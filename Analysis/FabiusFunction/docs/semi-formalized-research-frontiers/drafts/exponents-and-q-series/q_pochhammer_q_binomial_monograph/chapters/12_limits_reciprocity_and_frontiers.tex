...
\section{\texorpdfstring{Coefficientwise and \(q\)-adic limits}{Coefficientwise and q-adic limits}}

For a series \(G(q)=\sum_{d\ge0}g_dq^d\), write
\(\OrderOperator_{q=0} G=\min\{d:g_d\ne0\}\), with
\(\OrderOperator_{q=0} 0=\infty\).  The congruence
...
The finite products
\[
 \ProductTruncation{M}{q}=\prod_{j=1}^{M}(1-q^j)^{e_j}
\]
...
\(\OrderOperator_{q=0} S_m\to\infty\), in the precise sense that for each
\(N\) only finitely many \(m\) have
\(\OrderOperator_{q=0} S_m\le N\).  Then the \(q\)-adic sum
\(S=\sum_{m\ge0}S_m\) is well defined and
\begin{equation}
 S(q)\equiv
 \sum_{\OrderOperator_{q=0} S_m\le N}S_m(q)
 \pmod{q^{N+1}}.
...
\[
 \ProductTruncation{M}{q}\equiv\ProductTruncation{N}{q}
 \pmod{q^{N+1}}.
\]
Thus every coefficient of \(\ProductTruncation{M}{q}\) stabilizes; apply
...
For \(n\ge1\), define
\begin{equation}
 \BorweinProduct{n}{q}=\frac{(q;q)_{3n}}{(q^3;q^3)_n}
 =\prod_{j=1}^{n}(1-q^{3j-2})(1-q^{3j-1}).
\label{eq:qg-borwein-product}
\end{equation}
Grouping coefficients by their exponent modulo \(3\) gives unique
polynomials
\(A_n^{\mathrm{Bor}},B_n^{\mathrm{Bor}},C_n^{\mathrm{Bor}}
\in\IntegerNumbers[x]\) such that
\begin{equation}
 \BorweinProduct{n}{q}
 =\BorweinComponentA{n}{q^3}
  -q\BorweinComponentB{n}{q^3}
  -q^2\BorweinComponentC{n}{q^3}.
...
For every \(n\ge1\),
\begin{align}
 \BorweinComponentA{n}{x}
 &=x^{n^2}\BorweinComponentA{n}{x^{-1}},\\
 \BorweinComponentB{n}{x}
 &=x^{n^2-1}\BorweinComponentC{n}{x^{-1}},\\
 \BorweinComponentC{n}{x}
 &=x^{n^2-1}\BorweinComponentB{n}{x^{-1}}.
\label{eq:qg-borwein-reciprocity}
\end{align}
In particular,
\[
 \BorweinComponentB{n}{1}=\BorweinComponentC{n}{1},
 \qquad
 \BorweinComponentA{n}{1}=2\BorweinComponentB{n}{1}.
\]
\end{proposition}

\begin{proof}
The degree of \(\BorweinProduct{n}{q}\) is
\[
...
\[
 \BorweinProduct{n}{q}=q^{3n^2}\BorweinProduct{n}{q^{-1}}.
\]
Insert \eqref{eq:qg-borwein-dissection} on both sides and put \(x=q^3\).
The residue-zero terms give
\(\BorweinComponentA{n}{x}
=x^{n^2}\BorweinComponentA{n}{x^{-1}}\).  The residue-one terms on the left match the
reflected residue-two terms on the right, giving
\(\BorweinComponentB{n}{x}
=x^{n^2-1}\BorweinComponentC{n}{x^{-1}}\); the remaining residue class gives the third
...
Putting \(x=1\) in the last two identities gives
\(\BorweinComponentB{n}{1}=\BorweinComponentC{n}{1}\).  Since
\(\BorweinProduct{n}{1}=0\), equation \eqref{eq:qg-borwein-dissection} at
\(q=1\) gives
\(\BorweinComponentA{n}{1}
=\BorweinComponentB{n}{1}+\BorweinComponentC{n}{1}
=2\BorweinComponentB{n}{1}\).
\end{proof}
...
Write
\[
 \BorweinCoefficientA{n}{r}=[x^r]\BorweinComponentA{n}{x},
 \qquad
 \BorweinCoefficientB{n}{r}=[x^r]\BorweinComponentB{n}{x},
 \qquad
 \BorweinCoefficientC{n}{r}=[x^r]\BorweinComponentC{n}{x}.
\]
For every fixed \(d\ge0\) and every \(n\ge d+1\), the lower-end
coefficients have already stabilized:
\[
 \BorweinCoefficientA{n}{d}=\BorweinCoefficientA{d+1}{d},
 \qquad
 \BorweinCoefficientB{n}{d}=\BorweinCoefficientB{d+1}{d},
 \qquad
 \BorweinCoefficientC{n}{d}=\BorweinCoefficientC{d+1}{d}.
\]
The coefficients at the same fixed distance from the reciprocal ends satisfy
the exact formulas
\begin{align}
 \BorweinCoefficientA{n}{n^2-d}&=\BorweinCoefficientA{d+1}{d},\\
 \BorweinCoefficientB{n}{n^2-1-d}&=\BorweinCoefficientC{d+1}{d},\\
 \BorweinCoefficientC{n}{n^2-1-d}&=\BorweinCoefficientB{d+1}{d}.
...
\[
 \BorweinProduct{n}{q}=\BorweinProduct{N}{q}
...
\(q^{3N+1}\), and
\begin{equation}
 \BorweinProduct{n}{q}\equiv\BorweinProduct{N}{q}
 \pmod{q^{3N+1}}.
\label{eq:qg-borwein-finite-product-stabilization}
\end{equation}
Take \(N=d+1\).  The exponents \(3d\), \(3d+1\), and \(3d+2\) are all less
than \(3N+1=3d+4\), so their coefficients in
\(\BorweinProduct{n}{q}\) and \(\BorweinProduct{d+1}{q}\) agree.
By \eqref{eq:qg-borwein-dissection}, those three coefficients are respectively
\(\BorweinCoefficientA{n}{d}\), \(-\BorweinCoefficientB{n}{d}\), and
\(-\BorweinCoefficientC{n}{d}\).  This proves the three lower-end
stabilization identities.

The reciprocity identities in \Cref{qg:prop-borwein-reciprocity} give, for
every \(n\),
\[
 \BorweinCoefficientA{n}{n^2-d}=\BorweinCoefficientA{n}{d},
 \qquad
 \BorweinCoefficientB{n}{n^2-1-d}=\BorweinCoefficientC{n}{d},
 \qquad
 \BorweinCoefficientC{n}{n^2-1-d}=\BorweinCoefficientB{n}{d}.
\]
...
\(A_n^{\mathrm{Bor}},B_n^{\mathrm{Bor}},C_n^{\mathrm{Bor}}\) as a
Borwein conjecture.  That assertion is not retained here
...
\(\RecordedIdentity\) means ``the source records this
