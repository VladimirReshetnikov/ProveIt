\chapter{Universal inversion calculus}
\label{chap:universal-inversion}
...
Lagrange--B\"urmann determines a germ, not a global sheet.  If $g$ is the
selected inverse germ at the expansion value $w_0$, its Taylor radius is
precisely
\[
 R=\sup\bigl\{r>0:g\text{ extends as a single-valued holomorphic function to
 }\AbsoluteValue{w-w_0}<r\bigr\}.
\]
Thus $R$ is the distance from $w_0$ to the boundary of the maximal centered
disk of single-valued holomorphy of that germ.  A critical value belonging
only to another sheet, or one reachable only after leaving this disk and
crossing a chosen cut, need not control $R$; conversely, any obstruction to
single-valued holomorphic continuation on the centered disk can control it.
The phrase \emph{the nearest critical value} is therefore not a valid
unqualified radius formula.
\end{warning}
...
\section{\texorpdfstring{Asymptotic inverse transfer and Lambert \(W\)}{Asymptotic inverse transfer and Lambert W}}
...
relation \(u \EulerE^u=Z\), and hence for a local branch of \(W\) whenever that
...
The real residual estimate needed here is already the general branchwise
transfer principle \Cref{prop:uq-asymptotic-residual-transfer}.  If
\(\lvert F'\rvert\ge m>0\) on all of an interval, Darboux's theorem forces
the derivative to have constant sign, so \(F\) is injective there; taking
\(m(y)=m\) gives the familiar bound
\(
 \lvert\widetilde x-x_*\rvert
 \le \lvert F(\widetilde x)-y\rvert/m
\)
without a second theorem or proof.
...
\begin{theorem}[Scalar interval-Newton certificate at an arbitrary target]
\label{thm:uq-interval-newton-certificate}
Let \(I=[a,b]\subset\RealNumbers\), let \(F\in C^1(I)\), choose
\(x_0\in I\) and \(y\in\RealNumbers\), and let
\(D=[d_-,d_+]\) be a closed real interval containing \(F'(I)\), with
\(0\notin D\).  Interpret \((F(x_0)-y)/D\) as its interval image and define
\[
 N=x_0-\frac{F(x_0)-y}{D}.
\]
If \(N\subseteq I\), then \(F(x)=y\) has exactly one solution \(x_*\) in
\(I\), and \(x_*\in N\).  If \(N\subseteq\operatorname{int}I\), the
certified solution lies
in \(\operatorname{int}I\).
\end{theorem}

\begin{proof}
Replace \(F\) by \(F-y\).  Its derivative enclosure is still \(D\), its
zero set is the target level set of the original function, and its interval
Newton image is exactly the displayed \(N\).  We may therefore assume
\(y=0\).
