\chapter{Inverse \texorpdfstring{\(q\)}{q}-special functions}
\label{chap:inverse-q-special-functions}

\section{\texorpdfstring{\(q\)-numbers}{q-numbers}}
\label{sec:qs-qnumbers}

Fix a logarithm value \(\ell_q\) of
\(q\in\ComplexNumbers\setminus\{0,1\}\), put
\(q^x=\EulerE^{x\ell_q}\), and define
\[
  \GeneralizedQNumber{x}{q}:=\frac{1-q^x}{1-q}.
\]

\begin{proposition}[Exact inverse in the exponent]
\label{prop:qs-qnumber-x}
Let \(w=1-(1-q)y\ne0\), and fix one logarithm value \(\ell_w\) of \(w\).
Then, for every \(k\in\IntegerNumbers\),
\[
  x_k=\frac{\ell_w+2\pi\ImaginaryUnit k}{\ell_q}
\]
solves \(\GeneralizedQNumber{x_k}{q}=y\), and every solution is obtained in this way.
\end{proposition}

\begin{proof}
The equation \(\GeneralizedQNumber{x}{q}=y\) is equivalent to
\[
  \EulerE^{x\ell_q}=w.
\]
The complete preimage of \(w\) under the exponential is
\(\ell_w+2\pi\ImaginaryUnit\IntegerNumbers\).  Since \(q\ne1\), one has
\(\ell_q\ne0\), so division by \(\ell_q\) gives precisely the displayed
solutions.  Conversely, exponentiating any one of them recovers the equation.
\end{proof}
...
Fix an integer \(m\ge2\), and let \(u=y-1\).  The inverse branch through
\(q=0\) satisfies
\[
\begin{array}{rll}
 m=2:&q=u,&\text{exactly},\\
...
\]
For every integer \(m\ge2\), at infinity,
\[
...
\section{\texorpdfstring{\(q\)-factorials and their exact ramification}{q-factorials and their exact ramification}}
...
For every \(n\ge0\), let \(\zeta\ne1\) be a primitive \(d\)-th root of
unity.  Then
\[
  \OrderOperator_{q=\zeta}[n]_q!
  =\Floor{\frac nd}.
\]
If \(n\ge2\), put \(M=\Floor{n/2}\ge1\).  Then
\[
...
\[
  ...
  \qquad 0\le j<M.
\]
For \(n=0,1\), one has \([n]_q!\equiv1\), so there is no inverse at target
zero.
\end{theorem}
...
\(\Floor{n/d}\) factors therefore vanish, each simply.

For \(d=2\) and \(n\ge2\), the even factors satisfy
\[
...
For \(n\ge2\), write \(N=\binom n2>0\).  One also has
\[
  [n]_q!=q^N\left(1+\frac{n-1}{q}+\BigO(q^{-2})\right),
\]
and the same logarithmic coefficient comparison as in
\cref{thm:gq-large-inverse} yields, on each choice
\(Y=y^{1/N}\to\infty\),
\[
  q=Y-\frac{n-1}{N}+\BigO(Y^{-1})
   =Y-\frac2n+\BigO(Y^{-1}).
\]
For \(n=0,1\), the forward function is constant and has no large-target
inverse.

\section{\texorpdfstring{The \(q\)-gamma function}{The q-gamma function}}
\label{sec:qs-qgamma}
...
ordering.  Before coincident exponents are collected, the \((m,r)=(1,0)\)
term of the first double series contributes \(q^x\), while the two
integer-power series contribute \(-(2-x)q\) at exponent one.  These are
bookkeeping contributions, not necessarily the first two terms in the
ordered generalized-power expansion.  Whenever exponents coincide, all
contributions at that exponent must be collected.
\end{proposition}
...
then immediate.  The \((m,r)=(1,0)\) term and the integer exponent-one terms
give the two stated uncollected contributions; collecting all terms with a
common exponent is essential when collisions occur.
\end{proof}
...
\section{\texorpdfstring{\(q\)-digamma inversion}{q-digamma inversion}}
\label{sec:qs-qdigamma}

\begin{proposition}[Global argument inverse and the exceptional reciprocal fold]
...
\]
so, within the original domain \(0<q<1\), \(q=1\) is a one-sided quadratic
endpoint.  More precisely, if
\[
 \Delta=\psi_{\EulerE^{-t}}(3/2)-\psi(3/2),
\]
then the unique local inverse with \(t\downarrow0\) is
\[
 t=\sqrt{-288\Delta}\,\bigl(1+\BigO(\Delta)\bigr),
 \qquad \Delta\uparrow0,\quad \Delta<0.
\]

There is also a natural genuine two-sided statement.  Extend the standard
normalization to \(q>1\) by
\[
 \Gamma_q(x):=q^{(x-1)(x-2)/2}\Gamma_{1/q}(x),
 \qquad \psi_q(x):=\partial_x\log\Gamma_q(x),
\]
and use the classical functions at \(q=1\).  Then
\[
 \psi_q(x)=\psi_{1/q}(x)+\left(x-\frac32\right)\log q.
\]
In particular, \(t\mapsto\psi_{\EulerE^{-t}}(3/2)\) is reciprocal-even,
is \(C^2\) at \(t=0\), and has a genuine two-sided quadratic fold there.
Its branches are \(t=\pm\sqrt{-288\Delta}(1+\BigO(\Delta))\), equivalently
\(q=\EulerE^{\mp\sqrt{-288\Delta}(1+\BigO(\Delta))}\).
\end{proposition}
...
nonexceptional points.  The differentiated remainder in
\cref{thm:qs-qgamma-one} gives, at the exceptional point,
\[
 \partial_t\psi_{\EulerE^{-t}}(3/2)
 =-\frac{t}{144}+\BigO(t^3)<0
\]
for all sufficiently small \(t>0\).  Hence the physical one-sided inverse
exists and is locally unique.  Factoring the forward relation as
\(\Delta=-t^2(1+\BigO(t^2))/288\) and taking the positive square root gives
its displayed expansion.

The twice-differentiated remainder likewise gives
\[
 \partial_t^2\psi_{\EulerE^{-t}}(3/2)
 =-\frac1{144}+\BigO(t^2).
\]
For the reciprocal extension, logarithmic differentiation in \(x\) gives
the stated identity for \(\psi_q\).  At \(x=3/2\) its correction term
vanishes, so replacing \(q\) by \(1/q\), or \(t\) by \(-t\), changes
nothing.  The one-sided expansion and its differentiated remainder patch
with this exact reflection to a \(C^2\) function at zero, with second
derivative \(-1/144\).  The reflected positive and negative solutions are
therefore the asserted two branches.  This real argument requires no
holomorphic continuation across the boundary \(|q|=1\).
\end{proof}

\section{\texorpdfstring{\(q\)-beta coordinate and base inverses}{q-beta coordinate and base inverses}}
\label{sec:qs-qbeta}

For \(x,y>0\) and initially \(0<q<1\), define
\[
...
\]
For \(x\ne1\), this is a one-sided quadratic endpoint in the original
domain.  If
\[
 \Delta=\log\frac{B_{\EulerE^{-t}}(x,1/x)}{B(x,1/x)},
\]
then its unique local inverse with \(t\downarrow0\) is
\[
 t=\sqrt{-\frac{24x}{(x-1)^2}\Delta}\,
 \bigl(1+\BigO(\Delta)\bigr),
 \qquad \Delta\uparrow0,\quad \Delta<0.
\]
Under the reciprocal extension of \(\Gamma_q\) from
\cref{prop:qs-qdigamma-inverse},
\[
 B_q(x,y)=q^{1-xy}B_{1/q}(x,y).
\]
Consequently, on \(xy=1\) the extended beta function is exactly
reciprocal-even and the one-sided endpoint becomes a genuine two-sided
quadratic fold.  At \((x,y)=(1,1)\), the identity \(B_q(1,1)\equiv1\)
makes the base completely unidentifiable.
\end{theorem}
...
the stated negative \(b_2\), nonzero unless \(x=1\).  Uniform differentiated
remainders give
\[
 \partial_t\Delta
 =-\frac{(x-1)^2}{12x}t+\BigO(t^3)<0
\]
for sufficiently small \(t>0\).  Thus the one-sided inverse exists and is
unique, and taking the positive square root in
\(\Delta=-(x-1)^2t^2/(24x)+\BigO(t^4)\) gives its displayed expansion.
The twice-differentiated remainder is
\[
 \partial_t^2\Delta
 =-\frac{(x-1)^2}{12x}+\BigO(t^2),
\]
which will control the regularity at the reflected endpoint.

For \(q>1\), substitution of the reciprocal \(q\)-gamma normalization into
the definition of \(B_q\) gives an exponent
\[
 \frac{(x-1)(x-2)+(y-1)(y-2)-(x+y-1)(x+y-2)}2
 =1-xy,
\]
which proves the reciprocal identity.  On \(xy=1\) it gives exact evenness
under \(t\mapsto-t\); the differentiated one-sided remainder patches to a
\(C^2\) function with nonzero second derivative, and reflection supplies
the second branch.  At \((1,1)\),
\(\Gamma_q(1)=\Gamma_q(2)=1\) proves the exact constant identity.
\end{proof}
...
\section{\texorpdfstring{Euler \(q\)-exponentials}{Euler q-exponentials}}
...
Let \(|q|<1\).  If \(A_q\) is a selected argument-inverse branch of
...
\]
Writing \(h=1-q\), suppose additionally that \(|hx|<1\), and choose the
logarithms determined by their power series at \(x=0\).  Then the scaled
functions satisfy the absolutely convergent identities
\[
  \log e_q(x)
...
\[
  ...
  =\sum_{m\ge1}\frac{(-1)^{m+1}h^m x^m}
                       {m(1-(1-h)^m)}.
\]
For every fixed \(x\), the condition \(|hx|<1\) holds for all sufficiently
small positive \(h\).  In the resulting \(h\downarrow0\) asymptotic
expansion, only finitely many indices \(m\) contribute at each fixed order.
\end{proposition}
...
affect any prescribed coefficient.  For completeness, fix \(K\ge0\), take
real \(0<h<1\), and shrink \(h\) until \(h|x|\le1/2\).  Since
\(1-(1-h)^m\ge h\), the tail satisfies
\[
 \sum_{m\ge K+2}
 \left|\frac{h^mx^m}{m(1-(1-h)^m)}\right|
 \leq\frac1h\sum_{m\ge K+2}\frac{(h|x|)^m}{m}
 =\BigO(h^{K+1}).
\]
The alternating numerator obeys the same bound.  The finitely many terms
with \(m\le K+1\) may be expanded individually, proving the asserted
coefficientwise asymptotic statement with a genuine remainder estimate.
\end{proof}
...
For \(0<|q|<1\), let
\[
  \MultiplicativeTheta{z}{q}
  :=(z;q)_\infty(q/z;q)_\infty(q;q)_\infty,
  \qquad z\in\ComplexNumbers^\times.
\]
Jacobi's triple product gives the exact normalization crosswalk
\[
 \MultiplicativeTheta{z}{q}=\BilateralTheta{-z}{q}.
\]
Thus changing between the product and plus-sign bilateral conventions
requires the displayed argument negation.

\begin{proposition}[Simple theta zero atlas]
\label{prop:qs-theta-zeros}
The zeros of \(z\mapsto\MultiplicativeTheta{z}{q}\) are exactly
\(z=q^m\), \(m\in\IntegerNumbers\), and every zero is simple.  Hence the inverse at target
zero is ordinary on every sheet:
\[
  z=q^m+\frac{y}{\partial_z\MultiplicativeTheta{q^m}{q}}+\BigO(y^2).
\]
Moreover,
\[
  \MultiplicativeTheta{qz}{q}=-z^{-1}\MultiplicativeTheta{z}{q},
\]
