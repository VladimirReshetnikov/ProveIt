...
\(\prod_{\nu=1}^{s}(a_\nu;q)_n\).  Thus \((q;q)_n\ne0\).
As in \Cref{chap:bailey}, a Bailey pair relative to \(a\) means the single
canonical relation in \Cref{qg:def-bailey-pair}, equivalently
\eqref{eq:bailey-pair}.  We recall the terminology here but do not introduce a
second definition.
...
\end{align}
Then \((\alpha^{\mathrm{BT}},\beta^{\mathrm{BT}})\) is a Bailey pair relative to
\(a\).
\end{theorem}

\begin{proof}
Insert \eqref{eq:bailey-pair} into
...
\(\BaileyTransformAlpha{r}\) from \(\alpha_r\) in
...
\(\BaileyTransformAlpha{r}\).  Summing over \(r\) gives
\[
  \BaileyTransformBeta{n}
  =\sum_{r=0}^{n}
    \frac{\BaileyTransformAlpha{r}}
...
\begin{remark}[A polynomial route to the inner sum]
\label{qg:rem:full-bailey-cauchy-route}
% ed.: added with the formalisation (\texttt{BaileyLemmaFull})
The inner sum \eqref{qg:eq:full-bailey-inner} can also be evaluated without
the reversal \eqref{qg:eq:finite-reversal} and without any temporary
denominator.  Put \(x=\rho_1q^r\), \(y=\rho_2q^r\) and \(c=xyA=aq^{2r+1}\).
Since \(1/((q;q)_k(q;q)_{N-k})=\GaussianBinomial{N}{k}{q}/(q;q)_N\) and
\(1/(c;q)_k=(cq^k;q)_{N-k}/(c;q)_N\), the sum in
\eqref{qg:eq:full-bailey-inner} equals
\[
 \frac{1}{(q;q)_N(c;q)_N}
 \sum_{k=0}^{N}\GaussianBinomial{N}{k}{q}
 (x;q)_k(y;q)_kA^k(A;q)_{N-k}(cq^k;q)_{N-k},
\]
and the second finite Cauchy identity
\((xA;q)_N(yA;q)_N=\sum_{k=0}^{N}\GaussianBinomial{N}{k}{q}(x;q)_k(y;q)_kA^k(A;q)_{N-k}(xyAq^k;q)_{N-k}\)
(\texttt{finite\_qCauchy\_second\_identity}), a polynomial identity valid in
every commutative ring, evaluates it as
\((xA;q)_N(yA;q)_N/((q;q)_N(c;q)_N)\), which is
\eqref{qg:eq:full-bailey-inner-evaluation} since \(xA=aq^{r+1}/\rho_2\) and
\(yA=aq^{r+1}/\rho_1\).  The Zariski-density argument at the end of the proof
is then unnecessary.
\end{remark}
...
\begin{align}
 \BaileyLoweredAlpha{0}&=\alpha_0,
 \label{qg:eq:bailey-lowering-zero}\\
 \BaileyLoweredAlpha{n}
 &=(1-a)\left(
 ...
 \BaileyLoweredBeta{n}&=\beta_n.
 \label{qg:eq:bailey-lowering-beta}
\end{align}
Then \((\alpha^{\mathrm{BL}},\beta^{\mathrm{BL}})\) is a Bailey pair
relative to \(a/q\).
\end{lemma}
...
\(\beta_n=\BaileyLoweredBeta{n}\), proving the claim.
\end{proof}
...
\Cref{thm:bailey-limit-finite}.  The semantic superscript
\(\mathrm{BC}(t)\) records the result of applying \(S_a\) exactly \(t\)
times; it is not derivative notation.
...
\((\alpha^{\mathrm{BC}(0)},\beta^{\mathrm{BC}(0)})=(\alpha,\beta)\).
For \(t\ge1\),
\begin{align}
 \BaileyChainAlpha{n}{t}
 &=a^{tn}q^{tn^2}\alpha_n,
...
\(\BaileyChainBeta{n}{1}=1/(q;q)_n\), while for \(t\ge2\),
\begin{equation}
 \BaileyChainBeta{n}{t}
 =
...
\begin{equation}
  \RogersSeries{m}{q}
  =\sum_{n\ge0}\frac{q^{n^2+mn}}{(q;q)_n}.
  \label{qg:eq:rogers-family}
\end{equation}
Thus \(\RogersSeries{0}{q}=G(q)\) and \(\RogersSeries{1}{q}=H(q)\).
...
\begin{equation}
  \RogersSeries{m}{q}
  =\RogersSeries{m+1}{q}+q^{m+1}\RogersSeries{m+2}{q}.
  \label{qg:eq:rogers-recurrence}
\end{equation}
\end{lemma}

\begin{proof}
The \(n=0\) terms cancel when
\(\RogersSeries{m}{q}-\RogersSeries{m+1}{q}\) is formed.  For
\(n\ge1\), use
\((1-q^n)/(q;q)_n=1/(q;q)_{n-1}\):
\begin{align*}
 \RogersSeries{m}{q}-\RogersSeries{m+1}{q}
 &=\sum_{n\ge1}
   \frac{q^{n^2+mn}(1-q^n)}{(q;q)_n}\\
 &=\sum_{n\ge1}\frac{q^{n^2+mn}}{(q;q)_{n-1}}\\
 &=q^{m+1}\sum_{r\ge0}
   \frac{q^{r^2+(m+2)r}}{(q;q)_r}
 =q^{m+1}\RogersSeries{m+2}{q},
\end{align*}
...
\begin{equation}
 \frac{\RogersSeries{1}{q}}{\RogersSeries{0}{q}}
 =
 \cfrac{1}{
  1+\cfrac{q}{
  1+\cfrac{q^2}{
...
\[
 ...
 =1+\cfrac{q}{
  1+\cfrac{q^2}{
  \ddots+\cfrac{q^N}{1}}}
 =\frac{\ContinuantNumerator{N}}{\ContinuantDenominator{N}}.
\]
Its numerator and denominator satisfy
\begin{align*}
 \ContinuantNumerator{-1}=1,\quad \ContinuantNumerator{0}=1,\quad
 \ContinuantNumerator{N}
 &=\ContinuantNumerator{N-1}+q^N\ContinuantNumerator{N-2},\\
 \ContinuantDenominator{-1}=0,\quad \ContinuantDenominator{0}=1,\quad
 \ContinuantDenominator{N}
 &=\ContinuantDenominator{N-1}+q^N\ContinuantDenominator{N-2}.
\end{align*}
We claim
\begin{align}
 \ContinuantNumerator{N}
...
\(\ContinuantNumerator{N-1}\).  In the second term put \(h=j-1\); its power is
\[
 j^2+N+1-2j=N+h^2,
\]
so it sums to \(q^N\ContinuantNumerator{N-2}\).  This proves
...
\[
  \ContinuantNumerator{N}\longrightarrow\RogersSeries{0}{q},
  \qquad
  \ContinuantDenominator{N}\longrightarrow\RogersSeries{1}{q}
\]
locally uniformly.

The products in \Cref{thm:rogers-ramanujan} show that
\(\RogersSeries{0}{q}\) and \(\RogersSeries{1}{q}\) are nonzero for
\(|q|<1\).  Let \(K_0\) be a compact subset of
the unit disk.  Continuity and zero-freeness give
\[
  \eta_{K_0}=\min_{q\in K_0}|\RogersSeries{0}{q}|>0.
\]
Uniform convergence
\(\ContinuantNumerator{N}\to\RogersSeries{0}{q}\) gives
\(|\ContinuantNumerator{N}-\RogersSeries{0}{q}|<\eta_{K_0}/2\) on
\(K_0\) for all sufficiently large \(N\).  Those
\(\ContinuantNumerator{N}\) are consequently zero-free on \(K_0\), and
\[
  \frac1{\FiniteContinuant{N}}
  =\frac{\ContinuantDenominator{N}}{\ContinuantNumerator{N}}
  \longrightarrow
  \frac{\RogersSeries{1}{q}}{\RogersSeries{0}{q}}.
\]
The convergence is uniform on \(K_0\).  We define each finite truncation by
the rational continuant
\(\ContinuantDenominator{N}/\ContinuantNumerator{N}\); where the nested
...
The polynomials \(\ContinuantNumerator{N}\) and
\(\ContinuantDenominator{N}\) in
\eqref{qg:eq:continuant-P}--\eqref{qg:eq:continuant-Q} are Schur-type
finitizations of \(\RogersSeries{0}{q}\) and \(\RogersSeries{1}{q}\).
Thus the same recurrence has three
