...
Over \(\ComplexNumbers\), the final six theorems are
\lean{Fabius.differentiable_qPochhammerInfIn},
\lean{Fabius.hasDerivAt_qPochhammerInfIn_of_mul_pow_eq_one},
\lean{Fabius.hasDerivAt_qPochhammerInfIn_inv_pow},
\lean{Fabius.deriv_qPochhammerInfIn_inv_pow_ne_zero},
\lean{Fabius.deriv_qPochhammerInfIn_ne_zero_of_mul_pow_eq_one}, and
\lean{Fabius.analyticOrderAt_qPochhammerInfIn_of_eq_zero}.  The raw derivative
theorem assumes \(a_0q^j=1\) and permits \(q=0,j=0\); the reciprocal-power
formula and its nonvanishing theorem assume \(q\ne0\) and give exactly
\cref{eq:ip-zero-derivative}.  The inverse-power nonvanishing theorem literally
proves the displayed coefficient nonzero, while the preceding \texttt{HasDerivAt}
theorem identifies that coefficient with the derivative.  The first new
theorem strengthens this to actual derivative nonvanishing directly from the
division-free raw factor equation \(aq^j=1\), including \(q=0\).  The second
states that every zero of \(a\mapsto(a;q)_\infty\) has analytic order exactly
one for every strict complex contraction, again including \(q=0\).
...
result.  The generic module now states analytic order one directly through
\lean{Fabius.analyticOrderAt_qPochhammerInfIn_of_eq_zero}.  The unchanged
\lean{Fabius.analyticOrderAt_complexQPochhammerInf_of_eq_zero} and the other
three declarations in \path{QPochhammerEntire.lean} are legacy compatibility
wrappers for the older \lean{Fabius.complexQPochhammerInf} names; they transfer
the generic results rather than duplicating their analytic proofs.
