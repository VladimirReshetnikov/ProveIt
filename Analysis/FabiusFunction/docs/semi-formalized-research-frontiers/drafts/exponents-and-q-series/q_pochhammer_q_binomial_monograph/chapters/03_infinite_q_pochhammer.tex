...
For fixed \(q\), \lean{QPochhammerInfinite.lean} proves finite-prefix and
locally uniform convergence in \(a\), complex entire-ness, the exact zeros,
and their simplicity through
...
\lean{qPochhammerInfIn_eq_zero_iff_exists_inv_pow}, and
\lean{deriv_qPochhammerInfIn_inv_pow_ne_zero}.  The stronger joint
\((a,q)\)-holomorphy, explicit uniform-in-\(q\) tail bounds, and derivative
kernels in \cref{thm:ip-normal-convergence} remain human-proved, so the
compound theorem is only partially formalized.
