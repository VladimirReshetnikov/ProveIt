...
ledger, all \(260\) rows now have a canonical status and disposition note.  Its
status census is one \emph{Lean-proved}, \(147\)
\emph{human-proved frontier result}, fifteen \emph{conjecture}, and \(97\)
\emph{not applicable}; \(180\) rows map to canonical labels, while the
remaining \(80\) are explicitly absorbed, retired, or not separately asserted.
Exact Lean module and declaration cells remain empty unless an exact formal
counterpart is claimed.
...
formal surface has six principal islands:
...
\item the generic infinite \(q\)-Pochhammer product at every strict contraction:
  complete-normed-ring convergence, concatenation, factor removal and
  positive-modulus dissection, the multiplicative-norm factor-zero locus,
  locally compact field continuity, and complex entireness with an explicit
  nonzero reciprocal-zero derivative; the separate complex wrapper additionally
  has analytic order one at every zero, including \(q=0\);
\item selected finite \(q\)-Pochhammer and Gaussian-polynomial identities,
  including exact residue-class and remainder dissections over arbitrary
  commutative rings, reciprocity, exact values and first derivatives at
  \(q=-1\), and multiplicity one for the alternating zero over
...
claimed here for the general analytic and formal inverse theorems beyond the
  named Bell and quadratic support.  Beyond the fixed-\(q\) generic
  \(q\)-Pochhammer declarations named above, no Lean status is claimed for the
  uniform-in-\(q\) product and logarithmic tail bounds, joint holomorphy, or
  the differentiated logarithmic kernels of
\cref{thm:ip-normal-convergence}, nor for remote-gap and log-periodic
asymptotics, all-order \(q\to1\) and cyclotomic expansions, Dedekind-eta
completion, Gaussian and \(q\)-special global inverse atlases, complex
certification algorithms, or the conjectures in
