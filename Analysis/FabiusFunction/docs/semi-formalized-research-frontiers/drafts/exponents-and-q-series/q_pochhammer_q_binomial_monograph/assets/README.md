...
absorbed here; source checksum ledgers were retired; the two source requirements
files were replaced by the
...
Package checksum manifests are retired. Canonical destination SHA-256 values
in `ASSET_DISPOSITION.csv` retain the migration receipts without creating a
live checksum gate.
