...
def write_tables(rows: Sequence[tuple[str, str, str, str]]):
    ...
    (HERE/"numerical_results.tex").write_text("\n".join(tex), encoding="utf-8")
...
def make_branch_figure():
    ...
    plt.tight_layout()
    plt.savefig(HERE/"finite_pochhammer_branch_atlas.pdf")
    plt.close()
...
def make_periodic_figure():
    ...
    plt.tight_layout()
    plt.savefig(HERE/"log_periodic_correction.pdf")
    plt.close()
...
def make_qgamma_figure():
    ...
    plt.legend(loc="upper right")
    plt.tight_layout()
    plt.savefig(HERE/"qgamma_inverse_branches.pdf")
    plt.close()
...
def main():
    ...
    make_qgamma_figure()
    print((HERE/"numerical_results.txt").read_text(encoding="utf-8"))
