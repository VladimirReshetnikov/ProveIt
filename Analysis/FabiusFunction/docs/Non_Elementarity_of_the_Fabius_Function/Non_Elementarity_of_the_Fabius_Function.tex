...
in $W$.  The set $W$ absorbs exactly the degenerate locus that a discriminant
argument would otherwise have to control, and the induction terminates because
the degree drops.
...
Let $a_0,\dots,a_n$ be elementary, let $a_n$ vanish nowhere, and let
$y\colon\R\to\R$ be continuous with $\sum_{i\le n}a_i(x)y(x)^i=0$ for every
$x$.  Then $y$ does not agree with the Fabius function on any nonempty open
subset of $[0,1]$; in particular not on $(0,1)$.
\end{corollary}

\begin{proof}
Each $a_i$ has dense open analytic locus by \Cref{thm:density}.  The
intersection of the $n+1$ of them is a finite intersection of dense open sets,
hence dense and open, and \Cref{thm:algebraic} applies to it, giving that the
analytic locus of $y$ is dense in $\R$.  Now argue as in
Corollary~\ref{cor:main}.
\end{proof}

\begin{boxedremark}
Corollary~\ref{cor:algebraic} covers every algebraic function over the
elementary functions, including those whose Galois group is not solvable and
which are therefore not expressible by radicals at all.  It is genuinely
stronger than Corollary~\ref{cor:main} in that direction, and independent of it
in the other: it says nothing about $\exp$ or $\log$ of an algebraic function,
because it is a single algebraic step over elementary coefficients rather than
a closure.  A tower that interleaves algebraic extensions with exponentials
and logarithms at several levels is covered only when it can be presented in
...
Algebraic functions over $\Elem$ (Corollary~\ref{cor:algebraic})
  & \texttt{not\_algebraicBranch\_eqOn}, \texttt{canonical\_fabius\_not\_algebraicBranch\_on\_Ioo}
  & \texttt{NotElementary} \\
