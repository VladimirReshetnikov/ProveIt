...
\large Density of the analytic locus, and what it survives}
...
does not reach.  The first is the algebraic functions: any continuous branch
of a polynomial equation whose coefficients are elementary and whose leading
coefficient vanishes nowhere, which covers those not expressible by radicals,
and which rests on a degree induction that avoids the discriminant --- of
which \texttt{Mathlib} has no usable theory --- in favour of the analytic
implicit function theorem, of which it does.

The second is inverses.  We prove that $F^{-1}$, which exists because $F$ is a
homeomorphism of $[0,1]$, is itself analytic at no point of $[0,1]$ and hence
not elementary either; and then that adjoining continuous inverse branches to
the class, at any depth, changes nothing.  The Lambert $W$ function is such a
branch, so it is covered.  The organizing observation is that a one-sided
inverse is an implicit branch of the simplest possible equation, $h(z)-x=0$, so the
inverse function theorem is the implicit function theorem already in hand and
no new analysis is required at all.
\end{abstract}
...
\begin{boxedremark}
Corollary~\ref{cor:main} is the sharp form of the assertion ``the Fabius function is
...
$\Elem$ is closed under $n$-th roots but not, as defined, under passage to an
arbitrary algebraic function: Definition~\ref{def:elem} has no clause producing a
continuous branch of $P(x,y)=0$ for a polynomial $P$ with elementary
coefficients whose Galois group is not solvable, and Liouville's
differential-field definition of ``elementary'' does admit such branches.
\Cref{sec:algebraic} closes that gap, not by enlarging $\Elem$ but by proving
the conclusion of Corollary~\ref{cor:main} directly for such branches.  What
remains genuinely outside the result is stated there.
...
with a positive lower bound, but not $x\mapsto x^x$, whose base is positive
only on $(0,\infty)$.  That case is handled instead by the region statement
below.

The sign of the base is \emph{not} an obstruction, and it is worth being
precise about what is.  \texttt{Mathlib}'s $x^y$ is sign-dependent at a
...
vanishes.  The genuine obstruction is precisely the zero set of the base: at
$f(x)=0$ the conventions $0^r=0$ for $r\ne0$ and $0^0=1$ are discontinuous in
the exponent, and no continuous correction factor reproduces them --- which is
why $x\mapsto x^x$, and not some negative-base example, is the honest
counterexample.  Nothing is lost for Corollary~\ref{cor:main}, and this is
...
analytic locus omits every $1/(k\pi)$ with $k\in\Z\setminus\{0\}$, together
...
\section{Algebraic functions over the elementary functions}\label{sec:algebraic}

Definition~\ref{def:elem} reaches the $n$-th roots but not a general algebraic
function.  This section removes that restriction.  It does so without
...
\texttt{ContDiffAt.implicitFunction}, which is stated for every smoothness
exponent in $\N\cup\{\infty,\omega\}$; instantiating at $\omega$ and converting
with \texttt{ContDiffAt.analyticAt} gives an analytic implicit function
...
\texttt{HasDerivAt.hasFDerivAt\_equiv} together with uniqueness of the
Fréchet derivative.
\end{proofidea}
...
version with global hypotheses would be much weaker than it looks: requiring
$\sum a_i(x)y(x)^i=0$ for \emph{every} real $x$ forces the polynomial
$w\mapsto\sum a_i(x)w^i$ to be injective along the branch, and for
$y^5-y-x=0$ --- the standard example of an equation with non-solvable Galois
group --- no continuous branch on all of $\R$ exists, since $w\mapsto w^5-w$
is not injective.  The localized statement reaches the branches of that
equation over $(0,1)$, which are exactly the algebraic functions not
...
$F^{-1}$.  Everything above concerned $F$; this section treats $F^{-1}$, and
then closes the class under inversion.

One convention, since the classes of \Cref{sec:elem} consist of functions on
all of $\R$.  Throughout, $F^{-1}$ denotes the extension of the inverse
homeomorphism by its endpoint values, constantly $0$ on $(-\infty,0]$ and
constantly $1$ on $[1,\infty)$; in the formalization this is
\texttt{Fabius.fabiusInv}, defined by \texttt{Set.IccExtend} of the order
isomorphism.  The extension is continuous, so nothing is hidden in the
gluing, and the constancy outside $[0,1]$ is what the endpoint half of
\Cref{thm:invnowhere} exploits.

\subsection{A one-sided inverse is an implicit branch}
...
$F^{-1}(y_0)\in(0,1)$ --- provided $F^{-1}$ has no critical point there.  It
has none.  This is the one place where $F'$ enters at all --- everywhere else
...
obviously an obstruction to being elementary --- the real cube root is steep
at the origin and perfectly elementary.  \Cref{thm:invnowhere} says the hope
...
the invariant of \Cref{thm:density} survives inversion in general.
...
domain need not be open: \texttt{analyticDenseOn\_of\_rightInverse}.  For a
...
One asymmetry with $\Elem$ deserves stating, since \Cref{sec:elem} made a
point of it.  Composition is a \emph{theorem} for $\Elem$
(Proposition~\ref{prop:comp}), proved by inducting on the derivation of the
outer function, because every clause of Definition~\ref{def:elem} commutes with
...
\Cref{thm:plusdense} does not need it --- the induction there is over the
derivation, and every clause of Definition~\ref{def:elemplus} preserves dense
...
are as in \Cref{sec:density}; and the inverse rule splits on whether the test
set meets $U$.  Given a nonempty open $B$: if $B\cap U\ne\emptyset$ then
\Cref{thm:invbranch} applies to the nonempty open set $B\cap U$ and produces a
point of $B$ at which $g$ is analytic; and if $B\cap U=\emptyset$ then
$B\subseteq U^{c}$, so $B\subseteq\operatorname{int}(U^{c})$ because $B$ is
open, and every point of $B$ will do by the rule's analyticity clause.
\end{proof}
...
$(0,1)$ by elementary functions, their inverses, inverses of those, and the
elementary operations --- Lambert $W$ included.
\end{corollary}

\begin{boxedremark}
It is worth being explicit about what Corollary~\ref{cor:plusmain} does and does not
rest on.  It does not rest on any property of $W$ beyond its defining
identity: the formalization states the membership of $W$ in $\Elem^{-1}$ for
an \emph{arbitrary} continuous branch of $y=z\e^{z}$, because \texttt{Mathlib}
does not define $W$, and the statement therefore applies to any construction
of it.  And it does not rest on the inverse being a two-sided one; a
continuous right inverse on an open set is all that is used, which is the most
that a multivalued algebraic or transcendental inverse ever provides.
\end{boxedremark}
...
Derived closures: $-$, $/$, $x^n$, $\sqrt{\ }$, $|\cdot|$, $\tan$, $\sinh$, $\cosh$, $\tanh$, $\arccos$, $\operatorname{arsinh}$, polynomials
  & \texttt{IsElementary.sub}, \texttt{.div}, \texttt{.npow}, \texttt{.sqrt}, \texttt{.abs}, \texttt{.tan}, \texttt{.sinh}, \texttt{.cosh}, \texttt{.tanh}, \texttt{.arccos}, \texttt{.arsinh}, \texttt{.polynomial}
  & \texttt{ElementaryFunction} \\
Analytic locus $\An(f)$
  & \texttt{analyticLocus}
  & \texttt{ElementaryFunction} \\
...
Signed root, and its defining property (Warning~\ref{warn:oddroot})
  ...
  & \texttt{ElementaryFunction} \\
Variable exponents at a positive base, and at any nonvanishing base
  & \texttt{IsElementary.rpow\_of\_pos}, \texttt{IsElementary.rpow\_of\_ne\_zero}, \texttt{eqOn\_rpow\_of\_pos}
  & \texttt{ElementaryFunction} \\
...
Analytic implicit function theorem in two real variables
  & \texttt{analyticAt\_implicitFunction}, \texttt{analyticAt\_of\_continuous\_branch}
  & \texttt{AlgebraicBranch} \\
Branches of polynomial equations (\Cref{thm:algebraic})
  & \texttt{analyticDenseOn\_of\_algebraic}, \texttt{dense\_setOf\_analyticAt\_of\_algebraic}, \texttt{branchPoly}, \texttt{AnalyticDenseOn}, \texttt{analyticDenseOn\_of\_inter\_open\_dense}, \texttt{AnalyticDenseOn.not\_eqOn\_of\_forall\_not\_analyticAt}
  & \texttt{AlgebraicBranch} \\
Algebraic functions over $\Elem$ (Corollary~\ref{cor:algebraic})
  & \texttt{analyticDenseOn\_of\_isElementary\_coeffs}, \texttt{exists\_analyticAt\_of\_isElementary\_coeffs}, \texttt{not\_algebraicBranch\_eqOn}, \texttt{canonical\_fabius\_not\_algebraicBranch\_on\_Ioo}
  & \texttt{NotElementary} \\
The obstruction, stated once (proof of Corollary~\ref{cor:main})
  & \texttt{not\_eqOn\_of\_dense\_analyticLocus}
  & \texttt{ElementaryFunction} \\
Analytic inverse function theorem (\Cref{thm:ift})
  & \texttt{analyticAt\_of\_rightInverse}
  & \texttt{InverseBranch} \\
Inverse branches are analytic somewhere (\Cref{thm:invbranch})
  & \texttt{exists\_analyticAt\_of\_rightInverse}, \texttt{analyticDenseOn\_of\_rightInverse}
  & \texttt{InverseBranch} \\
The class $\Elem^{-1}$ (Definition~\ref{def:elemplus}, \Cref{thm:plusdense})
  & \texttt{IsElementaryOrInverse}, \texttt{IsElementaryOrInverse.dense\_analyticLocus}
  & \texttt{InverseBranch} \\
Lambert $W$ belongs to $\Elem^{-1}$
  & \texttt{isElementaryOrInverse\_of\_lambertW}
  & \texttt{InverseBranch} \\
$F^{-1}$ is differentiable on $(0,1)$ with $(F^{-1})'=1/F'>0$ (\Cref{thm:invnowhere})
  & \texttt{fabiusInv\_hasDerivAt}, \texttt{deriv\_fabiusInv}, \texttt{deriv\_fabiusInv\_pos}, \texttt{fabiusInv\_mem\_Ioo}, \texttt{fabiusInv\_contDiffOn\_Ioo}
  & \texttt{FabiusInverse} \\
Hence no critical point, and nowhere analytic (\Cref{thm:invnowhere})
  & \texttt{deriv\_fabiusInv\_ne\_zero}, \texttt{fabiusInv\_not\_analyticAt}, \texttt{fabiusInv\_analyticAt\_iff}
  & \texttt{InverseNotElementary} \\
$F^{-1}$ is not elementary (Corollary~\ref{cor:invmain})
  & \texttt{canonical\_fabiusInv\_not\_isElementary\_on\_Ioo}, \texttt{fabiusInv\_not\_isElementary}, \texttt{not\_eqOn\_fabiusInv\_of\_analyticDenseOn}, \texttt{not\_eqOn\_fabiusInv\_of\_dense\_analyticLocus}
  & \texttt{InverseNotElementary} \\
Neither is reachable by inverting (Corollary~\ref{cor:plusmain})
  & \texttt{not\_isElementaryOrInverse\_eqOn\_fabius}, \texttt{not\_isElementaryOrInverse\_eqOn\_fabiusInv} (general $U$), \texttt{canonical\_fabius\_not\_isElementaryOrInverse\_on\_Ioo}, \texttt{canonical\_fabiusInv\_not\_isElementaryOrInverse\_on\_Ioo} ($U=(0,1)$)
  & \texttt{InverseNotElementary} \\
...
Main result (Corollary~\ref{cor:main})
  & \texttt{canonical\_fabius\_not\_isElementary\_eqOn\_of\_interior\_nonempty} (general $U$), \texttt{canonical\_fabius\_not\_isElementary\_on\_Ioo} ($U=(0,1)$)
  & \texttt{NotElementary} \\
Interior form of the obstruction
  & \texttt{IsElementary.not\_eqOn\_of\_interior\_nonempty}; \texttt{not\_isElementary\_eqOn\_of\_interior\_nonempty}, \texttt{not\_algebraicBranch\_eqOn\_of\_interior\_nonempty}
  & \texttt{ElementaryFunction}; \texttt{NotElementary} \\
$F$ is not elementary
...
The same is true of the inverse, and for the same reason.  Bootstrapping the
reciprocal formula of \Cref{thm:invnowhere} through its own derivative shows
$F^{-1}$ to be $C^{\infty}$ on $(0,1)$ --- this is
\texttt{fabiusInv\_contDiffOn\_Ioo} --- so $F^{-1}$ is a second function that
is smooth to every order on an interval and real analytic at no point of it.
Smoothness separates neither of them from $\Elem$; analyticity separates both.
