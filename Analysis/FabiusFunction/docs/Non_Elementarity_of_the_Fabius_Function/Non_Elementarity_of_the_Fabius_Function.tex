...
bounded away from zero, but not $x\mapsto x^x$, whose base is positive only on
$(0,\infty)$.  That case is handled instead by the region statement below.
...
Variable exponents at a positive base
  & \texttt{IsElementary.rpow\_of\_pos}, \texttt{eqOn\_rpow\_of\_pos}
  & \texttt{ElementaryFunction} \\
...
Neither is reachable by inverting (Corollary~\ref{cor:plusmain})
  & \texttt{canonical\_fabius\_not\_isElementaryOrInverse\_on\_Ioo}, \texttt{canonical\_fabiusInv\_not\_isElementaryOrInverse\_on\_Ioo}
  & \texttt{InverseNotElementary} \\
