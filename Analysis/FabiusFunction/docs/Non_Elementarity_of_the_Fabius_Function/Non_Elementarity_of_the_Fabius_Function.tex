...
has none, and this is the one place in the whole development where smoothness
of $F$ is used rather than its failure to be analytic: differentiating
...
obviously an obstruction to being elementary --- $x\mapsto x^{1/3}$ is steep
...
that is not slack in the argument: $x\mapsto x^{1/3}$ is a continuous right
inverse of the entire function $x\mapsto x^3$ and is not analytic at the
origin.  Critical points of $f$ really do produce singularities of the
inverse.  It is the same phenomenon that made density, rather than
analyticity everywhere, the right invariant in \Cref{thm:density}.
...
The rule is local in $U$ deliberately.  The Lambert $W$ function satisfies its
defining identity $W(y)\e^{W(y)}=y$ only on $[-1/\e,\infty)$, so a rule
demanding a global identity would not reach it; and the requirement that $g$
be analytic off $U$ is exactly what the junk-value conventions supply, those
values being constants.  Inversion may be applied at any depth: to members of
...
Analytic inverse function theorem (\Cref{thm:ift})
  & \texttt{analyticAt\_of\_leftInverse}
  & \texttt{InverseBranch} \\
