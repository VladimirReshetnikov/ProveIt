...
hypothetical analytic interior point this derivative cannot vanish, and this
is the one place in the whole development where smoothness of $F$ is used
rather than its failure to be analytic: differentiating
$F^{-1}\circ F=\mathrm{id}$ gives
\[
  (F^{-1})'\bigl(F(x)\bigr)\cdot F'(x)=1 ,
\]
and $F$ is $C^\infty$, so $F'(x)$ is a finite real number and the product
could not be $1$ if the first factor vanished.  \Cref{thm:nowhere} then
...
that is not slack in the argument: the signed real cube root
$x\mapsto (x/|x|)|x|^{1/3}$ (with value $0$ at the origin) is a continuous
right inverse of the entire function $x\mapsto x^3$ and is not analytic at the
origin.  Critical points of $f$ really do produce singularities of the
inverse.  It is the same phenomenon that made density, rather than analyticity
everywhere, the right invariant in \Cref{thm:density}.
...
$y\in U$, and $g$ is analytic off $U$, then $g\in\Elem^{-1}$.
\end{definition}

The rule is local in $U$ deliberately: a branch identity may hold only on its
natural domain.  The Lean development records a conditional constructor for
any totalized Lambert-$W$ branch satisfying its inverse identity on an open
set, continuity there, and analyticity at every point outside that set.  It
does not construct a total principal branch satisfying those hypotheses; in
particular the real branch point at $-1/\e$ requires separate treatment.
Inversion may be applied at any depth: to members of $\Elem$, to branches of
those, and so on without limit.
...
inside $U$ together with the hypothesis off $U$ --- given a nonempty open $B$,
either $B$ meets $U$, and \Cref{thm:invbranch} applies to $B\cap U$, or
$B$ misses $U$ and every point of $B$ will do.
\end{proof}
...
constructor also requires the chosen totalization to be analytic off that
open set.  Because \texttt{Mathlib} does not define $W$, the formalization
neither constructs such a totalization nor claims that the standard principal
real branch satisfies the boundary hypothesis.
\end{boxedremark}
...
Analytic inverse function theorem (\Cref{thm:ift})
  & \path{analyticAt_of_leftInverse}
  & \path{InverseBranch} \\
...
Conditional Lambert-$W$ inverse-branch constructor
  ...
  & \path{InverseBranch} \\
Any analytic interior germ of $F^{-1}$ has nonzero derivative; $F^{-1}$ is nowhere analytic (\Cref{thm:invnowhere})
