...
branch, so it is covered.  The organizing observation is that a left inverse
is an implicit branch of the simplest possible equation, $h(z)-x=0$, so the
...
\subsection{A left inverse is an implicit branch}
...
has none.  This is the one place in the whole development where a
...
Branches of polynomial equations (\Cref{thm:algebraic})
  & \texttt{analyticDenseOn\_of\_algebraic}, \texttt{dense\_setOf\_analyticAt\_of\_algebraic}, \texttt{branchPoly}, \texttt{AnalyticDenseOn}
  & \texttt{AlgebraicBranch} \\
Algebraic functions over $\Elem$ (Corollary~\ref{cor:algebraic})
  & \texttt{exists\_analyticAt\_of\_isElementary\_coeffs}, \texttt{not\_algebraicBranch\_eqOn}, \texttt{canonical\_fabius\_not\_algebraicBranch\_on\_Ioo}
  & \texttt{NotElementary} \\
