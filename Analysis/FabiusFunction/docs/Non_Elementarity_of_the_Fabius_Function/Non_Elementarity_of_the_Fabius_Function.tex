...
them here.  The obstruction is purely one of formalization cost: the analogue
of \Cref{thm:density} for algebraic branches is true and classical --- away
from the zero set of the discriminant, the analytic implicit function theorem
applies, and that zero set is finite on each interval --- but the analytic
implicit function theorem over $\R$ is not available in \texttt{Mathlib} at
the time of writing.
...
\item \emph{Liouvillian functions.}  Replacing $\Elem$ by the class of
      ...
      unchanged; the missing ingredient for the algebraic branches is an
      analytic implicit function theorem, as recorded in
      \Cref{sec:notclaimed}.
