...
\eqref{eq:finite-atomic-law}--\eqref{eq:finite-atomic-weak} and the centered finite spline
...
$\int_\R\Up=1$, which makes $\Up$ a probability density.  For a solution of
...
The mass normalization itself is \lean{integral_rvachev_eq_one} in
...
\end{equation}
The transform refinement \eqref{eq:four-refinement}, specialized to \(\Up\), determines
its Fourier transform recursively.
...
\begin{proof}
Iterating \eqref{eq:four-refinement} with \(\varphi=\Up\) gives
\begin{equation*}
 \widehat{\Up}(z)=\widehat{\Up}(z/2^N)
   \prod_{n=0}^{N-1}\sinc\!\left(\frac{\pi z}{2^n}\right).
\end{equation*}
Continuity and the normalization \(\widehat{\Up}(0)=1\) make the first factor tend to one.
...
entire function, and passage to the limit gives the formula for every complex \(z\).
\end{proof}

The exact formal statement is \lean{rvachevFourier_eq_product} in
...
branches.  Even this enlarged class cannot agree with \(F\) or \(I\) on \((0,1)\); the exact
statements are
...
already introduced in \eqref{eq:four-P-product}, whose degree and total mass are recorded in
\eqref{eq:four-degree-mass}; we restate the product and degree here in the index convention
of this section:
\begin{equation}
...
Recall the step approximants \(\mathcal W_n\) from \eqref{eq:four-step-approximant} and
their pointwise convergence \eqref{eq:four-step-limit}.  Their bounded-partition
interpretation can be expressed through an exact finite discrete law: let
$Z_1,\ldots,Z_n$ be independent with $Z_i$ uniform on
...
$\Up$.  This is precisely the finite atomic construction
\eqref{eq:finite-atomic-law}--\eqref{eq:finite-atomic-weak}, whose stage dependence is
...
Why weak convergence can be proved through characteristic functions: the finite product
\eqref{eq:finite-atomic-char} converges to the transform of the probability measure with
density \(\Up\), yielding \eqref{eq:finite-atomic-weak}.\\
Sinc product &
...
$\Up$. &
\Cref{sec:probability}, via the finite atomic laws
\eqref{eq:finite-atomic-law}--\eqref{eq:finite-atomic-weak}.  Corrected: the printed
