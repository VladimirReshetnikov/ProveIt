...
symmetry, and smoothing at once, and which produces the finite atomic laws
\eqref{eq:finite-atomic-law}--\eqref{eq:finite-atomic-tests} and the centered finite spline
of \Cref{sec:centered-splines}.
...
$\int_\R\Up=1$, which makes the measure with density $\Up$ a probability measure and the
target of the weak convergence in \eqref{eq:finite-atomic-weak}.  For a solution of
\eqref{eq:original-rvachev} with $k=2$ and support in
...
Let \(\mu_{\Up}\) be the probability measure having density \(\Up\).
The finite laws converge weakly to \(\mu_{\Up}\):
\begin{equation}
 \nu_n\Longrightarrow\mu_{\Up}.                               \label{eq:finite-atomic-weak}
\end{equation}
Equivalently, for every bounded continuous \(g:\R\to\R\),
\begin{equation}
 \lim_{n\to\infty}\int_{\R}g(x)\,\dd\nu_n(x)
 =\int_{\R}g(x)\Up(x)\,\dd x.                                 \label{eq:finite-atomic-tests}
\end{equation}
...
For the analytic assertions, see
\lean{stepApproximant_nonneg}, \lean{stepApproximant_le_one},
...
\end{equation}
The dyadic Fourier refinement equation \eqref{eq:fourier-refinement} iterates to the product
below.
...
\begin{proof}
Repeated application of \eqref{eq:fourier-refinement} gives, for $N\ge1$,
\[
 \widehat{\Up}(z)=
 \left(\prod_{n=0}^{N-1}\sinc\!\left(\frac{\pi z}{2^n}\right)\right)
 \widehat{\Up}\!\left(\frac z{2^N}\right).
\]
The last factor tends to $\widehat{\Up}(0)=1$.  On compact subsets of $\C$,
\begin{equation*}
...
entire function and justifies the limit.  The exact formal theorem is
...
nothing else.  The bound therefore captures the whole of the doubly
...
The formal development also closes the elementary class under repeated continuous inverse
branches on open domains, provided each selected totalization is analytic on the interior
of the domain's complement.  These are exactly the side conditions of the
\lean{IsElementaryOrInverse.invBranch} constructor.  Even this enlarged class cannot agree
with \(F\) or \(I\) on \((0,1)\); the exact statements are
...
already introduced in \eqref{eq:four-P-product}; their coefficient interpretation and degree
are \eqref{eq:four-restricted-tuples} and \eqref{eq:four-degree-mass}.  We restate the product
here in the index convention of this section:
\begin{equation}
...
Recall from \eqref{eq:finite-atomic-weak}--\eqref{eq:finite-atomic-tests} that the measure
with density $\Up$ is the weak limit of exact finite atomic laws.  The generating-polynomial
construction gives a second exact finite description.
...
$\Up$.  The exact finite-stage formulation used in this article is the atomic law
\eqref{eq:finite-atomic-law}, with characteristic function
\eqref{eq:finite-atomic-char}; its stage dependence is explicit in the notation.
...
Finite atomic approximation &
Why the limiting density is approached by explicit centered Bernoulli convolution stages:
their characteristic functions are \eqref{eq:finite-atomic-char}, and their weak limit is
\eqref{eq:finite-atomic-weak}.\\
Sinc product &
...
$\Up$. &
\Cref{sec:probability}, in the exact finite-atomic form
\eqref{eq:finite-atomic-law}--\eqref{eq:finite-atomic-tests}.  Corrected: the printed
convolution index is finite, see \Cref{sec:aud-source-defects}.\\
