...
The practical consequence is that the $\Psi$-free part of $A_j$ is \emph{not} its mean.  At
$j=2$ the two differ by
\begin{equation*}
 \langle A_2\rangle-\frac1{4L^{2}}
 =-\frac{\langle(\Psi')^{2}\rangle}{2L^{3}}-\frac{\langle(\Psi'')^{2}\rangle}{4L^{4}}
 =-3.946\times10^{-9},
\end{equation*}
using $\langle\Psi'\Psi''\rangle=\langle\Psi''\Psi'''\rangle=0$ by parity and
$\langle\Psi'\Psi'''\rangle=-\langle(\Psi'')^{2}\rangle$.  The gap grows with $j$ like
everything else here: it is $-9.3\times10^{-11}$, $-3.9\times10^{-9}$ and
$-1.14\times10^{-7}$ at $j=1,2,3$, so it is invisible at $j=1$ and reaches the seventh digit
by $j=3$.  The $\Psi$-free part remains the right practical value for the mean, but the
reason is a ratio and not an absolute bound: at each order it sits four to five decades below
the oscillation of $A_j$ itself.
\end{warning}
