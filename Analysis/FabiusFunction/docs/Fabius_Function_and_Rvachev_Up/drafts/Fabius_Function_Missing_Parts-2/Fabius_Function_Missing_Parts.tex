...
interior picture; the formal development now packages its first-order consequences directly.
...
\begin{remark}
The first identity in \cref{cor:inverse-derivative}, its explicit \(\Up\)-form, and strict
positivity are the named Lean declarations \lean{fabiusInv\_hasDerivAt},
\lean{deriv\_fabiusInv}, \lean{deriv\_fabiusInv\_eq\_inv\_two\_mul\_rvachevUp}, and
\lean{deriv\_fabiusInv\_pos}.  The promotion to a named \(C^\infty\) theorem and the
second-derivative curvature formula in \cref{cor:inverse-curvature} remain ordinary-analysis
corollaries rather than separate declarations.
\end{remark}
...
\lean{deriv\_fabiusInv\_pos}, the transported root-free bounds,
