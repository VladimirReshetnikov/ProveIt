...
and rational functions, $\sqrt{\cdot}$ and all $n$-th roots $x\mapsto x^{1/n}$,
...
vanishing denominator, no nonpositive logarithm, no arcsine of a value outside
$[-1,1]$ --- then on $U$ it agrees pointwise with the total function produced
by the same derivation, since on $U$ the junk values are never consulted.
So every classical elementary function on $U$ is the restriction to $U$ of a
member of $\Elem$, and \Cref{cor:main} applies to it.
\end{boxedremark}
...
arbitrary algebraic function: \Cref{def:elem} has no clause producing a
...
is false as stated for the naive reason above, and with a merely $C^\infty$
inner function it is false outright: analyticity of $f$ is what makes the
dichotomy ``locally constant or locally injective away from a discrete set''
available.  This is the only place in the proof where a theorem of substance
about real analytic functions is used.
\end{remark}
...
$\pi\Z$.  It is, however, true and provable by the same induction that the
complement is always \emph{meagre and closed}; density is all \Cref{cor:main}
requires, so we stop there.
\end{remark}
...
Generators (\Cref{lem:generators})
  & \texttt{analyticAt\_log}, \texttt{analyticAt\_rpow\_const}, \texttt{analyticAt\_sqrt}, \texttt{analyticAt\_arctan}, \texttt{analyticAt\_arcsin}
  & \texttt{ElementaryFunction} \\
...
\item \texttt{AnalyticAt.eventually\_analyticAt}: analyticity at a point
      propagates to a neighbourhood, which is \Cref{lem:open}.
...
candidate available from the definition of $\Elem$ alone: the class $\Elem$
contains functions with prescribed $C^\infty$ behaviour of great generality,
and no restriction on growth, moments, or derivative bounds separates it from
$F$.

\subsection{The role of the endpoints}

\Cref{thm:nowhere} includes the endpoints $0$ and $1$, where $F$ is flat to
infinite order; this is what makes \Cref{cor:main} apply to \emph{every}
nonempty open subset of $[0,1]$ rather than to open subsets of $(0,1)$ only.
Note that $\An(F)$ is exactly $\R\setminus[0,1]$: outside the unit interval
$F$ is locally constant, hence analytic, so the nowhere-analytic locus is as
large as it can be and no larger.
...
\item \emph{Liouvillian functions.}  Replacing $\Elem$ by the class of
      functions lying in a Liouville tower over the rational functions ---
      allowing arbitrary algebraic branches, and iterated exponentials and
      logarithms in the differential-field sense --- would subsume
      \Cref{def:elem}.  The analytic content is unchanged; what is missing is
      the analytic implicit function theorem over $\R$, as recorded in
      \Cref{sec:notclaimed}.
