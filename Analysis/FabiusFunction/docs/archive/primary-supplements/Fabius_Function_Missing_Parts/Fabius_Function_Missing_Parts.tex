...
\hypersetup{
  ...
  citecolor=linkblue,
  urlcolor=linkblue,
  pdftitle={Fabius Function -- Missing Parts},
  pdfsubject={The totalized inverse and the hidden machinery of the all-orders endpoint expansion},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Fabius function---missing parts}
\fancyhead[R]{\small\nouppercase{\leftmark}}
\fancyfoot[C]{\thepage}
...
\title{\bfseries Fabius Function---Missing Parts\\[0.35em]
\large The totalized inverse and the hidden machinery of the all-orders endpoint expansion}
\author{Human-readable companion to the formal development\\
...
one-variable analysis.  The individual conclusions are therefore marked
\Derived, even though all Fabius-specific hypotheses have been proved
formally.

\subsection{Smoothness and differential identities in the interior}

\begin{theorem}[Interior inverse calculus; \Derived]
\label{thm:inverse-calculus}
The inverse $\InvF$ is $C^\infty$ on $(0,1)$.  For $0<y<1$,
\begin{align}
 \InvF'(y)
   &=\frac1{F'(\InvF(y))}
     =\frac1{2\Up(2\InvF(y)-1)}>0, \label{eq:Gprime}\\
 \InvF''(y)
   &=-\frac{F''(\InvF(y))}
           {F'(\InvF(y))^3}. \label{eq:Gsecond}
\end{align}
...
Uniqueness identifies all these local inverses with $\InvF$.

Differentiate $F(\InvF(y))=y$:
\[
 F'(\InvF(y))\InvF'(y)=1,
\]
which proves \eqref{eq:Gprime}.  Differentiating once more gives
\[
 F''(\InvF(y))\InvF'(y)^2+
 F'(\InvF(y))\InvF''(y)=0.
\]
Substitution of \eqref{eq:Gprime} yields \eqref{eq:Gsecond}.  Repeated
...
$F''>0$.  Equation \eqref{eq:Gsecond} and $F'>0$ imply
...
slope statement: by \cref{thm:inverse-quotient}, the secant slopes from
