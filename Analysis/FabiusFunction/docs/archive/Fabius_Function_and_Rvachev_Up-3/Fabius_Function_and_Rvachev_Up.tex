...
In fact the convergence is strong in every fixed differentiability order.  The Fourier
transforms are
\begin{equation}
 \widehat u_N(\xi)=\prod_{j=0}^{N}
 \sinc\!\left(\frac{\pi\xi}{2^j}\right),                 \label{eq:finite-box-transform}
\end{equation}
so they converge pointwise to \eqref{eq:sinc-product}.  Fix $m\ge0$ and choose $M>m+1$.
For $N\ge M$, every additional real sinc factor has modulus at most one, and therefore
\[
 |\xi|^m\left|\widehat u_N(\xi)-\widehat\Up(\xi)\right|
 \le2|\xi|^m|\widehat u_M(\xi)|.
\]
The right side is integrable: the $M+1$ finite sinc factors give more than $m+1$ powers of
decay.  Dominated convergence followed by Fourier inversion now gives
\begin{equation}
 \max_{0\le r\le m}\left\|u_N^{(r)}-\Up^{(r)}\right\|_\infty\longrightarrow0.
                                                               \label{eq:finite-box-Cm}
\end{equation}
Thus the same finite convolutions give weak convergence with an explicit tail modulus below
and $C^m$ convergence for every fixed $m$.
...
\begin{align}
 u_N'(x)
 &=2\bigl(u_{N-1}(2x+1)-u_{N-1}(2x-1)\bigr) \notag\\
 &=2^{N}\left(u_{N-1}\!\left(x+2^{-N-1}\right)
             -u_{N-1}\!\left(x-2^{-N-1}\right)\right).
                                                          \label{eq:res-derivative-recursion}
\end{align}
Consequently $u_N\in C^{N-1}(\R)$ for $N\ge1$, and no better: away from its finite knot set,
its piecewise $N$th derivative is a step function with nonremovable jumps.
...
\eqref{eq:res-derivative-recursion} then gives $u_N\in C^{N-1}$ for all $N\ge1$.  On the
complement of the finite knot set, iterating \eqref{eq:res-derivative-recursion} $N$ times
expresses the piecewise $N$th derivative as a signed combination of dilates
$u_0\bigl(2^{N}x+c\bigr)$ with pairwise distinct odd integers $c$; their jump sets are
disjoint, so no cancellation occurs.  Thus no continuous $N$th derivative exists.
...
For $m\ge0$ and $0\le r\le g_m$ put
\begin{equation}
...
$x>0$ and choose $0<h<x$.  Monotonicity of $\mathcal W_m$ on $[0,\infty)$ gives
\begin{equation*}
...
closed indicators the value there would be $2$, not $\Up(0)=1$.  A half-open convention such
as $\mathbf 1_{[\alpha,\beta)}$ avoids the double count and still assigns the origin the
correct value $1$, but it destroys evenness and makes the values at paired breakpoints
asymmetric.  Only the symmetric half weight simultaneously preserves evenness, keeps the
integral equal to $1$, and yields the intended symmetric breakpoint values for every $m$.
\end{warning}
