...
| Normalized Toeplitz q-row polynomial | The complete rational generating polynomial is the normalized finite q-Pochhammer product.  Its roots are exactly `2, 4, ..., 2^n`, including the empty root set at `n = 0`; mass one is the value at `z = 1`.  The row annihilates the first `n` rational dyadic geometric error modes exactly, giving a rational q-Richardson theorem. | `Fabius.sum_range_discreteLimitWeight_mul_pow`, `Fabius.sum_range_discreteLimitWeight_mul_pow_eq_zero_iff`, `Fabius.sum_range_discreteLimitWeight`, `Fabius.discreteLimitWeight_qRichardson_exact` |
