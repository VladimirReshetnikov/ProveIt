...
| Computability of the canonical bounded Fabius function | Proved in the Grzegorczyk sense.  The sequential clause uses uniform fast dyadic names and a certified primitive-recursive centered-spline evaluator with nearest rounding.  Effective uniform continuity uses the primitive-recursive positive modulus `d(n)=2n` for positive precision indices. | `Fabius.ComputableRealSequence`, `Fabius.SequentiallyComputable`, `Fabius.EffectivelyUniformContinuous`, `Fabius.tmBitPR_primrec`, `Fabius.fabiusSplineApproxPR_computable`, `Fabius.fabiusSplineApproxPR_error`, `Fabius.fabius_sequentiallyComputable`, `Fabius.fabius_isComputableRealFunction` |
