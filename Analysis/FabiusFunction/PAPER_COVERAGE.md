...
| Arbitrary-rational translation and dyadic-representation invariance of the finite formula | Proved for every `q : ℚ`; the fully displayed sum with inner power `(j - m * 2^k + q)^(n+k)` computes the same value.  In particular the source's `+1/2` and centered sums agree.  Equal nonnegative dyadic rationals give equal formula values, including the refinement `(m,n) ↦ (2m,n+1)`. | `Fabius.fabiusDyadic_eq_qBinomialThueMorseDyadic_translated_sum`, `Fabius.globalFabius_dyadic_eq_qBinomialThueMorseDyadic_translated_sum`, `Fabius.fabiusFunction_dyadic_eq_qBinomialThueMorseDyadic_translated_sum`, `Fabius.fabius_dyadic_eq_qBinomialThueMorseDyadic_translated_sum`, `Fabius.qBinomialThueMorseDyadicHalfShiftFormula_refine`, `Fabius.qBinomialThueMorseDyadicHalfShiftFormula_eq_of_rat_eq` |
...
| Raw-coordinate inverse-power formula with `(r+q)^(n+k)` and `(-2)^(n^2)` | Proved for every `q : ℚ` and `n : ℕ`; the literal theorem uses the zero-one `thueMorseBit`, and `n = q = 0` follows Lean's `0^0 = 1` convention. | `Fabius.neg_one_pow_thueMorseBit`, `Fabius.thueMorseSign_dyadic_complement`, `Fabius.neg_two_pow_nat_sq`, `Fabius.fabiusAtInverseTwoPow_eq_qBinomialThueMorse_add_sum`, `Fabius.fabius_inverse_two_pow_eq_qBinomialThueMorse_add_sum`, `Fabius.fabiusAtInverseTwoPow_eq_qBinomialThueMorse_uncentered_sum`, `Fabius.fabius_inverse_two_pow_eq_qBinomialThueMorse_uncentered_sum` |
| Infinite binary-reduction q-binomial/Thue--Morse series | Corrected and proved with outer index `m = 0`: for every `x ≥ 0` and every real or complex `q`, the literal nested series converges absolutely to the signed global Fabius extension; on `[0,1]` it equals the bounded Fabius function.  The finite expression is a constant polynomial in `q`, and the scale-zero term uses `Floor[x/2]` and supplies the missing endpoint value at `x = 1`. | `Fabius.qBinomialThueMorseTranslatedNumeratorPolynomial_eq_const`, `Fabius.qBinomialFabiusReductionPolynomial_rclike_eq_fabiusReductionSum`, `Fabius.binaryPreviousPrefix_eq_floor_zpow`, `Fabius.hasSum_globalBinaryReductionSummand`, `Fabius.qBinomialFabiusGlobalSummand`, `Fabius.qBinomialFabiusGlobalSummand_eq`, `Fabius.qBinomialFabiusGlobalSummand_independent`, `Fabius.summable_norm_qBinomialFabiusGlobalSummand`, `Fabius.hasSum_qBinomialFabiusGlobalSummand`, `Fabius.globalFabius_eq_qBinomialThueMorse_global_sum_complex`, `Fabius.globalFabius_eq_tsum_qBinomialFabiusGlobalSummand_real`, `Fabius.fabiusReal_eq_tsum_qBinomialFabiusGlobalSummand` |
| Generalized Wolfram `DiscreteLimit` q-binomial/Thue--Morse formula | Proved for every real `x ≥ 0` and every `q : ℂ`, including Gaussian-rational shifts and arbitrary real shifts (hence irrational ones).  The limit is the signed global Fabius extension, and on `[0,1]` it is the bounded Fabius function.  The safe natural range length `⌊2^(n+k)x+1/2⌋₊` is proved equivalent to the inclusive upper cutoff `Floor[2^(n+k)x-1/2]`, including the empty case.  Finite rows may depend on `q`; their limit does not.  Exact reindexing makes each row a Toeplitz average of convergent centered splines, and separate telescope/`tsum` results identify its limit—not each finite row—with the binary-reduction series. | `Fabius.fabiusDiscreteLimitRangeLength_eq_floor_add_one`, `Fabius.fabiusDiscreteLimitApproximationComplex_eq_weighted_shiftSpline`, `Fabius.fabiusDiscreteLimitApproximationComplex_tendsto_globalFabius`, `Fabius.fabiusDiscreteLimitApproximationGaussianRat_tendsto_globalFabius`, `Fabius.fabiusDiscreteLimitApproximationReal_tendsto_globalFabius`, `Fabius.fabiusDiscreteLimitApproximationRat_tendsto_globalFabius`, `Fabius.fabiusDiscreteLimit_literal_complex_tendsto_globalFabius`, `Fabius.fabiusDiscreteLimitApproximationComplex_tendsto_fabiusReal`, `Fabius.binary_telescope_tendsto_globalFabius`, `Fabius.fabiusDiscreteLimitApproximationComplex_tendsto_binary_tsum`, `Fabius.fabiusDiscreteLimitApproximationComplex_tendsto_literal_tsum` |
