...
| Full sharp expansion to every order | Proved at the exact lower-Lambert phase: after the first `N` periodic saddle coefficients, the remainder is `O(lambda^-N)`; `N = 2` exposes the first nonzero correction explicitly | `Fabius.log_fabius_sub_sharpLambertMain_hasAsymptoticExpansion`, `Fabius.log_fabius_sub_sharpLambertExpansion_isBigO`, `Fabius.fabiusSharpLambertExpansion_two` |
