...
| Finite q-binomial/centered Thue--Morse formula for `F(2^-n)` | Proved exactly for every natural `n`, including `n = 0`; `qPochhammer` and `qBinomial` follow the Wolfram argument order | `Fabius.fabiusAtInverseTwoPow_eq_qBinomialThueMorse_sum`, `Fabius.qBinomialThueMorseFormula`, `Fabius.fabiusFunction_inverse_two_pow_eq_qBinomialThueMorseFormula`, `Fabius.fabius_inverse_two_pow_eq_qBinomialThueMorseFormula` |
| Variant with inner power `(r - 2^k + 1/2)^(n+k)` | Proved exactly; more generally, the outer numerator is invariant under every common rational translation `c` | `Fabius.thueMorseTranslatedShiftedPowerSeries_eq_exp_mul`, `Fabius.qBinomialThueMorseTranslatedNumerator_eq_centered`, `Fabius.fabiusAtInverseTwoPow_eq_qBinomialThueMorse_halfShift_sum`, `Fabius.fabius_inverse_two_pow_eq_qBinomialThueMorseHalfShiftFormula` |
