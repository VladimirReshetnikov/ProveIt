...
| Translation and dyadic-representation invariance of the finite formula | Proved for every common rational translation `c`; in particular the source's `+1/2` and centered sums agree.  Equal nonnegative dyadic rationals give equal formula values, including the refinement `(m,n) ↦ (2m,n+1)`. | `Fabius.qBinomialThueMorseDyadicTranslatedFormula_eq_centered`, `Fabius.fabiusDyadic_eq_qBinomialThueMorseDyadicTranslatedFormula`, `Fabius.qBinomialThueMorseDyadicHalfShiftFormula_refine`, `Fabius.qBinomialThueMorseDyadicHalfShiftFormula_eq_of_rat_eq` |
