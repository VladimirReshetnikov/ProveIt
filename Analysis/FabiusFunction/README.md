...
| Quarter Catalan formal-germ bridge | `FabiusFunction.QuarterCatalanGerm` | Exhaustive public surface: `dyadicGermTwo_functionalEquation`, `rescale_dyadicGermTwo_eq_quadraticInverse`, `dyadicGermTwo_eq_rescale_quadraticInverse`, `coeff_dyadicGermTwo_succ`.  Rescaling the dyadic parameter by `9/4` identifies the distinguished rational germ exactly with the Catalan inverse of `X + 4X²`, and every positive coefficient is `(4/9)^(m+1) (-4)^m C_m`.  This module is formal power-series algebra only; the downstream analytic bridge is supplied separately. |
...
| Finite `q`-Cauchy convolution | `FabiusFunction.QBinomialCauchy` | `finiteQPochhammerIn_mul_eq_sum_gaussianBinomial` proves `(uv;q)_n = Σ_(k=0)^n [n choose k]_q (u;q)_k v^k (v;q)_(n-k)` over every commutative ring.  The identity is finite and denominator-free, so it includes zero divisors, positive characteristic, `q = 0`, and roots of unity; it asserts no infinite-product limit or later Pfaff--Saalschütz summation. |
