...
> `GeometricUniformMomentPolynomial` 1+8, its real-MGF normalization bridge
> `GeometricUniformMomentPolynomialBridge` 0+1, and the inner complex-product
> bridge `GeometricUniformComplexMomentProduct` 1+2.
...
| Geometric-uniform moment polynomials and their real and inner-complex analytic normalizations | `FabiusFunction.GeometricUniformMomentPolynomial`, `FabiusFunction.GeometricUniformMomentPolynomialBridge`, `FabiusFunction.GeometricUniformComplexMomentProduct` | The algebra leaf has the exhaustive public surface of one definition and eight theorems: `geometricUniformMomentPolynomial`, `geometricUniformMomentPolynomial_zero`, `geometricUniformMomentPolynomial_succ`, `geometricUniformMomentPolynomial_natDegree_le`, `geometricUniformMomentPolynomial_eval_zero`, `geometricUniformMomentPolynomial_one`, `geometricUniformMomentPolynomial_two`, `geometricUniformMomentPolynomial_three`, and `geometricUniformMomentPolynomial_four`.  Its total family over `ℚ[X]` satisfies the division-free residual finite q-Pochhammer recurrence, `natDegree P_n ≤ n.choose 2`, `P_n(0)=1/(n+1)!`, and the displayed values `P_0` through `P_4`.  The real bridge has no public definitions and exactly one public theorem, `geometricUniformMomentPolynomial_eval₂_eq_mgf_taylorCoefficient`: for every real `q` with `|q|<1`, including `q=0` and negative contractions, it proves `P_n(q)=((q;q)_n/(1-q)^n)·(iteratedDeriv n M_q 0/n!)`, where `M_q` is the genuine geometric-uniform MGF.  The complex bridge has the exhaustive public surface of one definition and two theorems: `geometricUniformComplexMomentProduct`, `hasProdLocallyUniformly_geometricUniformComplexMomentProduct`, and `geometricUniformMomentPolynomial_eval₂_eq_complexMomentProduct_taylorCoefficient`.  For every complex `q` with `‖q‖<1`, including `q=0`, it constructs `A_q(z)=∏' j, complexExpm1Div ((1-q)*q^j*z)`, proves locally uniform convergence on the whole complex plane, and proves `P_n(q)=((q;q)_n/(1-q)^n)·(iteratedDeriv n A_q 0/n!)`.  This is an analytic product, not a probability MGF when `q` is nonreal.  The normalization in `p7:thm:Pn` is exact on the inner disc, but that label remains **Partial** because its leading-coefficient formula and strict odd-degree drop are absent.  Monograph label `thm:qF-moment-polynomial` remains **Partial** because its source uses one rational-function coefficient object across the inner product and the `|q|>1` reciprocal germ, with no named exterior-germ, `RatFunc`, or root-of-unity continuation identification in Lean.  Label `thm:geometric-uniform-mgf` remains **Partial** because the public API does not package the product's dilation/Mahler law, entireness and normalization, formal uniqueness, coefficient rationality and pole data, or a direct equality with the real MGF or characteristic function.  The leading/subleading coefficients and sharp degree in `prop:qF-P-degree-sharp` remain absent, so that label remains **None**. |
