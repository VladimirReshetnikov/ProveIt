...
> 952 source modules and 11,884 explicit public declarations.  The documentation audit
...
> `GaussianBinomialFixedColumnRate` 0+10,
...
> `to_additive` names), `WrightOmega` to 1+13, and the live census to
> 952/11,884, again with no documentation gaps.  The separate
...
| --- | --- | --- |
| Generic asymptotic scales, vector-valued Poincaré expansions, and corrected flatness (`q0:def:scale`, `q0:def:poincare`, `q0:prop:uniqueness`, `q0:def:flat`, `q0:prop:invisible`) | `FabiusFunction.TransseriesScale`, `FabiusFunction.TransseriesFlat` | **Exact.** `TransseriesScale` is exhaustively 3+6: definitions/structure `IsAsymptoticScale`, `poincarePartialSum`, `IsPoincareExpansion`; theorems `poincarePartialSum_zero`, `poincarePartialSum_succ`, `IsPoincareExpansion.isLittleO_succ_remainder`, `.tendsto_coeff`, `.tendsto_coeff_div`, `.coeff_unique`.  It works over an arbitrary filter and normed scalar field with coefficients in a normed vector space; uniqueness explicitly needs a proper (`NeBot`) filter.  `TransseriesFlat` is exhaustively 1+11: `IsFlat`; `isFlat_zero`, `IsFlat.add`, `.neg`, `.sub`, `.const_smul`, `isFlat_exp_neg_rpow_atTop`, `IsPoincareExpansion.add_flat`, `.sub_same_coeff_isFlat`, `.iff_sub_isFlat`, `IsFlat.smul_of_scale_absorption`, `.smul_of_isBigO_inv_pow`.  Thus the corrected invisible-function proposition includes the concrete `exp (-x)` term, vector-space closure, exact same-coefficient iff, and polynomial-growth closure on the power scale.  A variable multiplier on an arbitrary scale is claimed only under the explicit hypothesis `∀ N, ∃ M, m · φ_M = O(φ_N)`; the power specialization separately assumes eventual nonvanishing. |
| Well-based supports, power--logarithmic scales, and height comparison (`q0:lem:dickson`, `q0:lem:neumann`, `plt:lem:mot-dominance`, `q0:prop:height`) | `FabiusFunction.TransseriesWellBased`, `FabiusFunction.TransseriesScaleDominance`, `FabiusFunction.TransseriesPolyLogScale`, `FabiusFunction.TransseriesHeight` | `TransseriesWellBased` is exhaustively 0+5 in the lexical census: `dickson_isPWO`, `dickson_antichain_finite`, `dickson_isPWO_pi`, `neumann_isPWO`, `neumann_finite_factorizations`; `to_additive` also exports the non-lexically-counted `neumann_add_isPWO` and `neumann_finite_decompositions`.  Dickson and Neumann are **Exact**, with the source monomial order instantiated through `OrderDual` where required.  `TransseriesScaleDominance` is exhaustively 1+7: `plMonomial`; `tendsto_plMonomial_atTop_zero`, `plMonomial_div_eventuallyEq`, `tendsto_plMonomial_div_atTop_zero`, `tendsto_plMonomial_div_atTop_one`, `plMonomial_pos`, `tendsto_plMonomial_div_atTop`, `plMonomial_generators_dominance`.  `TransseriesPolyLogScale` is exhaustively 0+4: `isLittleO_plMonomial`, `isAsymptoticScale_plMonomial`, `isAsymptoticScale_plMonomial_pow`, `isAsymptoticScale_plMonomial_log`.  Together they make the displayed three-way limits and each chosen decreasing sequence scale **Exact**; the compound `plt:lem:mot-dominance` remains **Partial** because no unordered set-indexed scale API or named reverse generator implication is supplied.  `TransseriesHeight` is exhaustively 0+3: `isLittleO_log_pow_rpow`, `isLittleO_log_pow_id`, `isLittleO_pow_mul_log_pow_exp`.  These make the displayed quantitative equation and its power/log dual **Exact**; the broader recursive taxonomy of arbitrary nested exponential height and logarithmic depth remains **Partial**. |
| Polynomial block operators and Laurent-block differentiation (`plt:eq:mot-block-derivative`, `plt:lem:mot-block-antiderivative`) | `FabiusFunction.TransseriesBlockAntiderivative`, `FabiusFunction.TransseriesDifferentialBlock` | `TransseriesBlockAntiderivative` is exhaustively 3+12.  Definitions: `blockOperator`, `blockAntiderivative`, `resonantAntiderivative`.  Theorems: `sum_sub_sum_shift`, `blockOperator_zero`, `blockOperator_sub`, `blockOperator_blockAntiderivative`, `blockOperator_surjective`, `natDegree_C_mul_of_ne_zero`, `natDegree_blockOperator`, `blockOperator_injective`, `blockOperator_bijective`, `derivative_resonantAntiderivative`, `derivative_surjective`, `natDegree_resonantAntiderivative`.  `TransseriesDifferentialBlock` is exhaustively 0+8: `derivation_pow_t`, `derivation_block`, `derivation_zpow_block`, `exists_zpow_block_primitive`, `existsUnique_zpow_block_primitive`, `exists_block_primitive`, `derivation_block_zero`, `exists_block_primitive_resonant`.  The integer Laurent block formula is **Exact**, and the polynomial operator clauses are Exact.  The compound antiderivative lemma remains **Partial**: the generic Laurent existence theorem uses an ambient field and its uniqueness theorem explicitly assumes injectivity of evaluation at `L`; the concrete Laurent-polynomial model, that injectivity in the model, and a resonant uniqueness-up-to-constants wrapper are not constructed here. |
...
| Linear--logarithmic remainder transport | `FabiusFunction.RemainderTransport` | Exhaustive zero-definition/three-theorem surface: `lipschitzOn_of_abs_deriv_le`, `transport_bound_mul`, and `transport_bound`.  The derivative-to-Lipschitz bridge and both the division-free and divided forms of `p0:eq:transport-bound` are exact; they correctly separate the one-sided derivative floor on the core from the two-sided derivative bound on the perturbation.  Part (2) of `p0:thm:remainder-transport`, the first-order law with an explicit second-order error, remains unformalized. |
