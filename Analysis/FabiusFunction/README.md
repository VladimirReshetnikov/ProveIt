...
| Fixed-length Fabius increments and exact inverse modulus | `FabiusFunction.InverseModulus` | Exhaustive public surface (one definition and 20 theorems): `fabiusIntervalMass`; `fabiusIntervalMass_reflect`, `monotoneOn_fabiusIntervalMass_firstHalf`, `antitoneOn_fabiusIntervalMass_secondHalf`, `fabiusReal_sub_le_sub`, `fabiusReal_add_le`, `fabiusInv_sub_le_sub_of_mem_Icc`, `fabiusInv_sub_le_sub_of_le`, `fabiusInv_sub_le_sub`, `fabiusInv_add_le`, `abs_fabiusInv_sub_le`, `dist_fabiusInv_le`, `fabiusInv_min_one`, `isGreatest_abs_fabiusInv_sub`, `sSup_abs_fabiusInv_sub_eq`, `isGreatest_abs_fabiusInv_sub_Icc`, `sSup_abs_fabiusInv_sub_Icc_eq`, `abs_fabiusInv_sub_lt_of_abs_sub_lt_fabiusReal`, `abs_fabiusInv_sub_le_of_abs_sub_le_fabiusReal`, `fabiusReal_le_abs_sub_of_le_abs_fabiusInv_sub`, `forall_abs_fabiusInv_sub_lt_iff`.  For nonnegative interval length, the increment is globally monotone up to its centered position and antitone afterwards; on `[0,1]` this yields least endpoint mass and constrained forward superadditivity.  The clamped inverse has ordered and order-free gap bounds, global subadditivity, absolute and metric self-moduli, saturation at one, and attained exact `IsGreatest`/`sSup` moduli at every nonnegative radius, both over all real inputs and over unit-interval inputs.  The final four declarations give strict and closed effective-injectivity bounds, contrapositive separation, and the exact uniform strict-threshold biconditional.  This structural module alone does not construct an explicit recursive denominator; that effective layer is supplied by `FabiusInverseEffectiveContinuity` below. |
