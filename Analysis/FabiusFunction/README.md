...
> **Artifact status (2026-09-01).**  The authoritative live lexical audit of
> the facade union contains exactly 671 source modules and 8,859 public
> declarations, with no missing module headers or public-declaration doc
> comments.  The union preserves the exhaustive q-series inventories
> `RvachevPochhammerFactorization` 1+10 (including the public
> `complexQPochhammerInf_eq_qPochhammerInfIn` bridge),
> `QPochhammerEntire` 0+5, `QPochhammerInfinite` 1+29,
> `QMultinomial` 1+9, and `GaussianBinomialPolynomialStructure` 0+5.
> It also preserves the six-module 665/8,819 checkpoint
> (`CyclotomicDivisibility` 0+3, `PrimitiveRootBlock` 0+3,
> `QLucas` 0+8, `QCatalan` 1+11, `NewtonInterpolation` 2+13,
> and `QBetaIntegral` 1+8), with Newton definitions `newtonCoeff` and
> `newtonInterpolant`; the latter name avoids collision with the established
> scalar-sequence `Fabius.newtonPoly` API while the `newtonPoly_*`
> theorem spellings remain compatible.  The latest six modules are
> `GaussianBinomialInteger` 1+10, `GaussianBinomialComplexOrder` 1+5,
> `QPfaffSaalschutz` 0+3, `QuantumMultinomial` 0+5,
> `RvachevSuperconvergentSynthesis` 1+8, and
> `GaussianBinomialBounds` 0+6: three definitions and 37 theorems.
> Thus this union adds six modules and 40 unique declarations to 665/8,819,
> and 22 modules and 161 declarations relative to the 649/8,698 checkpoint.
> The rigorous forward q-monograph ledger is 90 Exact, 85 Partial, 99 None,
> and 8 interface rows.  `prop:gaussian-bound` moves from None to Exact,
> witnessed by `one_le_gaussianBinomial` and
> `gaussianBinomial_le_inv_qPochhammerInfIn`.  In particular,
> `thm:q-lucas` is Partial: Lean
> proves an evaluated primitive-root identity, while the manuscript states a
> polynomial congruence modulo `Φ_d`; the minimal-polynomial lift remains
> unformalized.  `cor:babbage-derivative` likewise remains Partial because
> only its value, not its derivative, is formalized.
...
> Integration-and-Transform master, comb-interpolation, and q-series
> synthesis PDFs contain respectively 167, 150, 237, 301, 41, 88, 377, 158,
> and 357 A4 pages.  These are historical validation receipts, not source/PDF
> parity claims; all remain pending fresh uninterrupted three-pass Libertinus
> rebuilds.  Their current TeX sources contain post-render unions, including
> the centered
...
> inverse realizer and total inverse computability theorem.  The reorganized q-series roots whose
...
| Universal Gaussian-polynomial degree and palindromicity | `FabiusFunction.GaussianBinomialPolynomialStructure` | Exhaustive public surface: zero definitions and five theorems, `natDegree_gaussianBinomial_universal`, `gaussianBinomial_universal_monic`, `coeff_zero_gaussianBinomial_universal`, `gaussianBinomial_universal_reflect`, and `coeff_gaussianBinomial_universal_symm`.  For `k ≤ n`, the universal natural-coefficient polynomial `gaussianBinomial (X : ℕ[X]) n k` has exact natural degree `k * (n - k)`, is monic, has constant coefficient one, and is fixed by reflection in that degree; consequently its coefficients at `d` and `k * (n - k) - d` agree whenever `d ≤ k * (n - k)`.  Together with the existing Gaussian symmetry and reciprocal identities this closes the canonical forward `q`-binomial structure theorem.  It does not prove that the coefficient of `q` is one. |
| Generic Gaussian-polynomial palindromicity and the inversion mean | `FabiusFunction.GaussianBinomialPalindromic` | Exhaustive zero-definition/twelve-theorem surface: `reflect_add_of_natDegree_le`, `reflect_one'`, `gaussianBinomial_natDegree_le`, `gaussianBinomial_zero_left`, `gaussianBinomial_diag'`, `reflect_gaussianBinomial`, `coeff_gaussianBinomial_reflect`, `coeff_gaussianBinomial_zero`, `coeff_gaussianBinomial_top`, `gaussianBinomial_natDegree`, `gaussianBinomial_monic`, and `two_mul_derivative_gaussianBinomial_eval_one`.  Over every commutative semiring and under exactly `k ≤ n`, the Gaussian polynomial over an arbitrary coefficient semiring has degree at most `k(n-k)`, constant and top coefficient one, and exact reflection/coefficient symmetry; coefficient reversal additionally uses `j ≤ k(n-k)`, while exact degree and monicity require a nontrivial target.  The final division-free identity is `2 * G'(1) = k(n-k) * choose(n,k)` and needs no inversion of two.  Together with row symmetry and reciprocity this closes the canonical forward structure theorem, but does not identify the coefficient of `q` with one. |
| Function-level q-derivative and the two q-exponentials | `FabiusFunction.QExponential` | Exhaustive public surface (three definitions and eight theorems): `qDeriv`, `qExp`, `qExpBig`; `qFactorial_mul_one_sub_pow`, `qFactorial_ne_zero`, `qDeriv_mul`, `hasSum_qExp`, `hasSum_qExpBig`, `qExp_mul_qExpBig_neg`, `qDeriv_qExp`, and `qDeriv_qExpBig`.  The factorial clearing identity is commutative-ring algebra, and the product rule is total at every `q,x`, including the totalized zero denominator.  Over a complete normed field with `‖q‖ < 1`, the small q-exponential series and product inverse require `‖(1-q)x‖ < 1`, while the big q-exponential series is global in `x`; their eigenfunction laws additionally assume `x ≠ 0`, and only the small law needs the displayed convergence-domain bound. |
| Jackson q-integration, both fundamental theorems, and integration by parts | `FabiusFunction.JacksonIntegral` | Exhaustive public surface (one definition and seven theorems): `jacksonIntegral`; `qDeriv_jacksonIntegral`, `one_sub_mul_pow_mul_qDeriv`, `tendsto_jackson_sum_qDeriv`, `jacksonIntegral_qDeriv`, `tendsto_jackson_sum_parts`, `jackson_parts_of_tendsto`, and `jacksonIntegral_mul_qDeriv`.  The first fundamental theorem assumes exactly `q ≠ 1`, `x ≠ 0`, and summability of the displayed Jackson series.  The termwise telescoping law is unconditional over a normed field; its partial-sum converse assumes the stated limit of `G(aq^n)`, and the `jacksonIntegral` form adds summability.  The product-rule integration-by-parts limit needs only convergence of `f(aq^n)g(aq^n)`; its two convergent-series forms state exactly their partial-sum or summability hypotheses. |
| Bilateral theta product, quasi-periodicity, and zero lattice | `FabiusFunction.ThetaQuasiPeriodicity` | Exhaustive public surface (one definition and six theorems): `bilateralTheta`; `thetaExponent_add_one`, `pow_thetaExponent_add_one`, `hasSum_bilateralTheta`, `bilateralTheta_eq_prod`, `bilateralTheta_mul_left`, and `bilateralTheta_eq_zero_iff`.  The exponent recurrence is unconditional, its power form assumes `q ≠ 0`, and the bilateral sum/product identities assume a complete normed field, `‖q‖ < 1`, and `z ≠ 0`.  Quasi-periodicity and the exact lattice `theta(z;q)=0 ↔ ∃ m : ℤ, z=-q^m` additionally assume `q ≠ 0`; no statement is made at `z=0`, outside the strict-contraction disk, or about zero multiplicities. |
| Jacobi's cubic identity | `FabiusFunction.JacobiCubic` | Exhaustive zero-definition/two-theorem surface: `two_mul_add_one_le_three_pow` and `hasSum_jacobi_cubic`.  The first is the total natural bound `2n+1 ≤ 3^n`; the second proves `∑ n, (-1)^n(2n+1)q^((n+1).choose 2) = (q;q)_∞^3` in `HasSum` form for complex `q` under exactly `‖q‖ < 1`, including `q=0`.  It asserts no norm-one boundary or modular transformation. |
| Logarithmic derivative of the infinite q-Pochhammer symbol | `FabiusFunction.QPochhammerLogDerivative` | Exhaustive zero-definition/ten-theorem surface: `one_sub_le_norm_one_sub_mul_pow`, `summable_pow_div_one_sub_mul_pow`, `summable_log_one_sub_mul_pow`, `one_sub_mul_pow_ne_zero`, `qPochhammerInfIn_eq_cexp_tsum_log`, `hasDerivAt_tsum_log_one_sub_mul_pow`, `hasDerivAt_qPochhammerInfIn`, `hasDerivAt_lambert_series`, `tsum_neg_pow_div_one_sub_mul_pow_eq`, and `hasDerivAt_qPochhammerInfIn_lambert`.  For complex `‖q‖ < 1`, the factor-log series is summable for every `a`; the reciprocal-factor series, exponential product spelling, and product derivative use `‖a‖ < 1`.  Termwise differentiation is proved on any disk `‖a‖ < r < 1`, and the two derivative formulas identify the factor sum with the Lambert sum.  These are interior-disk formulas, not boundary values, joint `(a,q)` derivatives, or logarithmic continuations across zeros. |
| Derivative of the complex-order q-Pochhammer symbol | `FabiusFunction.QPochhammerOrderDerivative` | Exhaustive zero-definition/three-theorem surface: `hasDerivAt_const_cpow'`, `hasDerivAt_qPochhammerInfIn_mul_cpow`, and `hasDerivAt_qPochhammerC`.  Principal complex powers satisfy the first chain rule under exactly `q ≠ 0`.  The shifted infinite-product derivative and the order derivative of `(a;q)_α` assume exactly `‖q‖ < 1`, `q ≠ 0`, and `‖a*q^α‖ < 1`; the final logarithmic form is `qPochhammerC a q α * log(q) * ∑ j, a*q^(α+j)/(1-a*q^(α+j))`.  No branch-independent order coordinate, boundary continuation, or derivative in the nome is claimed. |
...
| Continuity and the classical limit of Gaussian binomial coefficients | `FabiusFunction.GaussianBinomialContinuity` | Exhaustive public surface: zero definitions and three theorems, `continuous_gaussianBinomial`, `tendsto_gaussianBinomial_nhds_one`, and `gaussianBinomial_eq_finiteQPochhammerIn_div`.  Continuity and the limit at `q = 1` hold in every topological semiring, for all natural `n,k`, including out-of-range and degree-zero cases.  The quotient formula instead works over a field and assumes exactly `k ≤ n` and nonvanishing of the finite denominator `(q;q)_k`; no differentiability, rate of convergence, or quotient formula at a vanishing denominator is asserted. |
| q-Pascal row summation and Gaussian-coefficient commutation | `FabiusFunction.QPascalSummation` | Exhaustive public surface: zero definitions and four theorems, `sum_gaussianBinomial_succ_mul`, `sum_gaussianBinomial_succ_mul'`, `Commute.gaussianBinomial_left`, and `Commute.gaussianBinomial_right`.  The first row split holds in every semiring, the second in every commutative semiring, and both are total in the row index, including `n = 0`.  In an arbitrary semiring, a Gaussian coefficient commutes with every element that commutes with `q`, in either orientation.  No division, topology, or convergence is used, and the commutation wrappers require no global centrality of `q`. |
| Noncommutative q-binomial theorem for the quantum plane | `FabiusFunction.QuantumBinomial` | Exhaustive public surface: zero definitions and two theorems, `quantumPlane_mul_pow` and `quantum_binomial`.  Over every possibly noncommutative semiring, `Y * X = q * (X * Y)` and commutation of `q` with the needed variables give the iterated power law and the ordered Gaussian expansion of `(X+Y)^n`; the theorem includes `n = 0` and requires neither inverses nor subtraction.  Centrality of `q` in the whole semiring is not assumed, only its stated commutation with `X` and `Y`. |
| Euler identities, stable-limit bounds, and the infinite q-binomial theorem | `FabiusFunction.QBinomialTheoremInfinite` | Exhaustive public surface (one definition and 22 theorems): `gaussianMajorant`; `hasSum_of_tendsto_of_dominated`, `one_le_qPochhammerInfIn_neg`, `finiteQPochhammerIn_neg_le_qPochhammerInfIn`, `qPochhammerInfIn_nonneg`, `qPochhammerInfIn_le_finiteQPochhammerIn`, `qPochhammerInfIn_pos_of_lt_one`, `qPochhammerInfIn_zero_left`, `norm_finiteQPochhammerIn_le_neg_norm`, `finiteQPochhammerIn_norm_le_norm`, `norm_finiteQPochhammerIn_le`, `qPochhammerInfIn_norm_le_norm_finiteQPochhammerIn`, `finiteQPochhammerIn_ne_zero_of_norm_lt_one`, `gaussianBinomial_eq_div`, `gaussianMajorant_nonneg`, `norm_gaussianBinomial_le`, `tendsto_finiteQPochhammerIn_pow_atTop`, `tendsto_gaussianBinomial_atTop`, `summable_pow_choose_two_mul_pow`, `hasSum_euler_product`, `qPochhammerInfIn_ne_zero_of_norm_lt_one`, `hasSum_qBinomial_theorem`, and `hasSum_euler_reciprocal`.  The finite zero-left lemma is reused from `GaussianBinomialAtOne` rather than redeclared.  The generic Tannery theorem uses a summable dominating family in a complete normed additive commutative group; the real comparison lemmas state their nonnegative and unit-interval hypotheses explicitly.  Over every complete normed field and every `‖q‖ < 1`, including `q = 0`, Euler's product expansion holds for arbitrary `z`, while the infinite q-binomial theorem (with arbitrary `a`) and Euler reciprocal require exactly `‖z‖ < 1`, including `z = 0`.  No `‖q‖ = 1` or `‖z‖ = 1` boundary theorem, analytic continuation, or bilateral series identity is asserted. |
| Rogers--Szegő polynomials, finite recurrences, and generating function | `FabiusFunction.RogersSzegoPolynomial` | Exhaustive public surface (one definition and nine theorems): `rogersSzego`; `rogersSzego_zero`, `rogersSzego_one`, `rogersSzego_succ`, `one_sub_pow_succ_mul_gaussianBinomial_succ_succ`, `rogersSzego_dilation`, `rogersSzego_three_term`, `summable_norm_pow_div_finiteQPochhammerIn_self`, `sum_antidiagonal_euler_mul_euler`, and `hasSum_rogersSzego_generating`.  The definition, zero-degree and `z = 1` evaluations, and two-term recurrence are finite algebra over every commutative semiring; the adjacent-column, dilation, and three-term identities use a commutative ring and include all natural indices.  Over a normed field, the absolute-summability helper assumes `‖q‖ < 1` and norm below one for its series variable, while the finite antidiagonal coefficient identity assumes only `‖q‖ < 1`.  Over a complete normed field the generating function assumes exactly `‖q‖ < 1`, `‖t‖ < 1`, and `‖z*t‖ < 1`, so `q = 0` and `t = 0` are included.  No unit-circle boundary, orthogonality measure, zero-location, or asymptotic theorem is asserted. |
| Jacobi triple product and Euler pentagonal theorem | `FabiusFunction.JacobiTripleProduct` | Exhaustive public surface (two definitions and 25 theorems): `thetaExponent`, `pentagonalExponent`; `two_dvd_mul_sub_one`, `mul_sub_one_nonneg`, `two_mul_thetaExponent`, `thetaExponent_natCast`, `thetaExponent_neg_natCast`, `choose_two_add_choose_two_succ_eq`, `sum_mul_sum_eq_sum_antidiagonal`, `thetaCoeff_pair_eq`, `sum_antidiagonal_thetaCoeff`, `finite_triple_product_poly`, `pow_mul_finiteQPochhammerIn_div`, `neg_one_zpow_natCast_sub`, `finite_triple_product`, `norm_gaussianBinomialInt_le`, `tendsto_gaussianBinomialInt_central`, `summable_pow_thetaExponent_mul_zpow`, `hasSum_jacobi_triple_product`, `hasSum_jacobi_triple_product'`, `mul_three_mul_sub_one_nonneg`, `two_dvd_mul_three_mul_sub_one`, `two_mul_pentagonalExponent`, `pentagonalExponent_eq`, `pentagonalExponent_pos`, `hasSum_pentagonal`, and `hasSum_pentagonal_pairs`.  Both integer exponent functions are total.  The finite polynomial identity holds over every commutative ring with arbitrary `q,z,N`, including `N = 0`; its Laurent form works over a field and assumes exactly `z ≠ 0`.  Over every complete normed field, Jacobi's two bilateral forms assume `‖q‖ < 1` and `z ≠ 0`, while both pentagonal forms assume only `‖q‖ < 1` and explicitly include `q = 0`.  The current source change only renames the unused proof binder `hi` to `_hi` in `thetaCoeff_pair_eq`; its public type and this count are unchanged.  No norm-one boundary, modular transformation, theta functional equation, or analytic continuation is claimed. |
...
| Uniform bounds for finite and infinite `q`-Pochhammer symbols | `FabiusFunction.QPochhammerInfiniteBounds` | Exhaustive zero-definition/five-theorem surface: `qPochhammerInfIn_neg_le_neg`, `norm_qPochhammerInfIn_le`, `finiteQPochhammerIn_ne_zero_of_qPochhammerInfIn_ne_zero`, `inv_norm_finiteQPochhammerIn_le`, and `exists_norm_finiteQPochhammerIn_div_le`.  The first is the real monotonicity `(-x';r)_∞ ≤ (-x;r)_∞` under exactly `0 ≤ x' ≤ x` and `0 ≤ r < 1`.  Over a complete normed field and `‖q‖ < 1`, the remaining results bound `‖(a;q)_∞‖` by `(-‖a‖;‖q‖)_∞`, show every prefix of a nonzero infinite product is nonzero, bound all reciprocal prefix norms uniformly, and bound `(a;q)_n/(c;q)_n` uniformly when `(c;q)_∞ ≠ 0`.  No `‖q‖ = 1`, compactness, or continuity theorem is asserted. |
| The basic hypergeometric series `₂φ₁` and Heine transformation | `FabiusFunction.HeineTransformation` | Exhaustive public surface (two definitions and five theorems): `twoPhiOneTerm`, `twoPhiOne`; `exists_norm_twoPhiOneTerm_le`, `summable_twoPhiOneTerm`, `hasSum_twoPhiOne`, `qPochhammerInfIn_mul_pow_div`, and `heine_transformation`.  Over a complete normed field, term domination assumes `‖q‖ < 1` and `(c;q)_∞ ≠ 0`; summability adds `‖z‖ < 1`.  The shifted-tail quotient also assumes its finite prefix nonzero.  Heine's formula assumes exactly `‖q‖ < 1`, `b ≠ 0`, `‖b‖ < 1`, `(c;q)_∞ ≠ 0`, `‖z‖ < 1`, and `(az;q)_∞ ≠ 0`; it does not assume `a ≠ 0` or provide analytic continuation beyond this domain. |
| q-Gauss summation | `FabiusFunction.QGaussSummation` | Exhaustive zero-definition/two-theorem surface: `twoPhiOne_self_eq` and `q_gauss_summation`.  The cancellation theorem assumes `‖q‖ < 1`, `(a;q)_∞ ≠ 0`, and `‖z‖ < 1`.  The q-Gauss formula assumes exactly `‖q‖ < 1`, `a ≠ 0`, `b ≠ 0`, `‖b‖ < 1`, `(c;q)_∞ ≠ 0`, `(c/b;q)_∞ ≠ 0`, and `‖c/(ab)‖ < 1`.  This is the initial Heine domain, not an analytic-continuation result. |
| General basic hypergeometric series | `FabiusFunction.BasicHypergeometricSeries` | Exhaustive public surface (two definitions and five theorems): `basicHypergeometricTerm`, `basicHypergeometric`; `qPochhammerInfIn_neg_norm_nonneg`, `twoPhiOneTerm_eq_basicHypergeometricTerm`, `norm_basicHypergeometricTerm_le`, `summable_basicHypergeometricTerm_of_le`, and `summable_basicHypergeometricTerm_of_eq`.  Over complete normed fields with `‖q‖ < 1` and every denominator product `(b_j;q)_∞ ≠ 0`, the bound assumes `r ≤ s+1`; absolute convergence is global in `z` for `r ≤ s`, and holds for `‖z‖ < 1` when `r=s+1`.  The `₂φ₁` identification is unconditional at the finite-term level.  No boundary convergence or analytic continuation is claimed. |
| Complex-order `q`-Pochhammer symbol | `FabiusFunction.QPochhammerComplexOrder` | Exhaustive public surface (one definition and four theorems): `qPochhammerC`; `qPochhammerC_zero`, `qPochhammerC_natCast`, `qPochhammerC_add`, and `qPochhammerC_add_one`.  The definition uses principal `Complex.cpow` and totalized division.  The zero law assumes only `(a;q)_∞ ≠ 0`; natural-order agreement assumes `‖q‖ < 1` and `(aq^n;q)_∞ ≠ 0`; concatenation assumes `q ≠ 0` and `(aq^α;q)_∞ ≠ 0`; the unit shift assumes `‖q‖ < 1`, `q ≠ 0`, and `1-aq^α ≠ 0`.  No meromorphic or branch-continuation theory is asserted. |
| Division-free q-multinomial coefficients | `FabiusFunction.QMultinomial` | Exhaustive public surface (one definition and nine theorems): `qMultinomial`; `qMultinomial_nil`, `qMultinomial_cons`, `gaussianBinomial_diag`, `qMultinomial_singleton`, `qMultinomial_pair`, `map_qMultinomial`, `qMultinomial_eq_eval₂_universal`, `finiteQPochhammerIn_self_sum_eq_qMultinomial_mul`, and `qMultinomial_eq_div`.  The definition, empty-list law, and recursion need only an arbitrary semiring.  The diagonal, singleton, pair, naturality, and universal-polynomial laws use commutative semirings; the pair law assumes `k ≤ n`.  The factorial form is division-free over commutative rings; the field quotient form assumes exactly the denominator-control hypothesis `(q;q)_(sum l) ≠ 0`.  Polynomiality has natural coefficients, but no coefficient formula, unimodality, or asymptotic theorem is claimed. |
| Division-free Jackson `q`-derivative on polynomials | `FabiusFunction.PolynomialQDerivative` | Exhaustive public surface (two definitions and 17 theorems): `qInt`, `qDerivative`; `qInt_zero`, `qInt_one`, `qInt_succ`, `qInt_succ'`, `qInt_add`, `qInt_one_left`, `one_sub_mul_qInt`, `qDerivative_apply`, `qDerivative_monomial`, `qDerivative_C`, `qDerivative_X_pow`, `qDerivative_X`, `qDerivative_C_mul_X_pow`, `eval_qDerivative_mul`, `qDerivative_comp_C_mul_X`, `qDerivative_mul`, and `qDerivative_mul'`.  The `q`-integer definition and its first six laws hold over every semiring; `(1-q)[n]_q=1-q^n` uses a commutative ring.  The coefficientwise `qDerivative` is an `R`-linear map over every commutative semiring and satisfies the monomial, constant, scaling, and both product rules there.  The division-free evaluation identity `(D_q p)(x)((1-q)x)=p(x)-p(qx)` uses a commutative ring.  All statements are total in `q`, including `q=0` and `q=1`; no quotient, topology, analytic-function derivative, or `q→1` limit is asserted. |
| Iterated polynomial `q`-Leibniz rule | `FabiusFunction.PolynomialQLeibniz` | Exhaustive zero-definition/four-theorem surface: `comp_C_mul_X_comp_C_mul_X`, `qDerivative_C_mul`, `qDerivative_iterate_comp_C_mul_X`, and `qDerivative_iterate_mul`.  Over every commutative semiring and for arbitrary `q,c,d` and natural iteration order (including zero), scalings compose multiplicatively, `D_q` commutes with constant multiplication, iterated differentiation of `p(c·)` contributes `c^k`, and `D_q^n(fg)` is the finite Gaussian-coefficient Leibniz sum with the first factor evaluated at `q^(n-k)·`.  No division, nonzero-nome, order, topology, or convergence hypothesis is used. |
| Finite `q`-Pochhammer parameter derivatives | `FabiusFunction.QPochhammerDerivative` | Exhaustive zero-definition/three-theorem surface: `hasDerivAt_finiteQPochhammerIn`, `hasDerivAt_finiteQPochhammerIn_of_ne_zero`, and `hasDerivAt_finiteQPochhammerIn_comp`.  Over every nontrivially normed field, for arbitrary `q,a` and natural `n`, the first theorem gives the derivative as the negative finite cofactor sum.  The second rewrites it as `-(a;q)_n * ∑_{j<n} q^j/(1-aq^j)` under exactly `∀ j ∈ range n, 1-a*q^j ≠ 0`.  The third is the pointwise chain rule under exactly `HasDerivAt f f' x`.  There is no hypothesis on `q`, and no derivative of the infinite product, logarithm branch, zero-multiplicity, or joint-parameter theorem is supplied by this leaf. |
| Lambert-series logarithm of the infinite `q`-Pochhammer product | `FabiusFunction.LambertSeriesLog` | Exhaustive zero-definition/four-theorem surface: `one_le_norm_natCast_add_one`, `summable_lambert_series`, `hasSum_lambert_log_complex`, and `exp_neg_tsum_lambert_eq_qPochhammerInfIn`.  The first is the unconditional complex norm bound on a positive natural cast.  For complex `a,q` under exactly `‖a‖<1` and `‖q‖<1`, the remaining theorems prove absolute summability of `a^(m+1)/((m+1)(1-q^(m+1)))`, its `HasSum` equality with the negative sum of the principal factor logarithms, and the branch-free exponential identity with `(a;q)_∞`.  These strict disks include `a=0` and `q=0`; no norm-one boundary, principal-log identity for the product itself, analytic continuation, or differentiation of the Lambert series is asserted. |
| Real `q`-gamma function, recurrence, reflection product, and theta form | `FabiusFunction.QGamma` | Exhaustive public surface (two definitions and ten theorems): `qGamma`, `qNumber`; `norm_lt_one_of_pos_of_lt_one`, `qPochhammerInfIn_rpow_pos`, `qPochhammerInfIn_self_pos`, `qGamma_pos`, `qGamma_one`, `qGamma_add_one`, `qGamma_nat_succ`, `qNumber_natCast`, `qGamma_mul_qGamma_one_sub`, and `hasSum_theta_qGamma_reflection`.  Both definitions are total real-valued functions.  Positivity, `Γ_q(1)=1`, the functional equation `Γ_q(x+1)=[x]_q Γ_q(x)`, and the natural factorial product use exactly `0<q<1`, with `x>0` additionally for positivity and the functional equation.  The natural-cast bridge `[n]_q=qInt q n` assumes only `q≠1`.  The algebraic reflection-product identity is stated for every real `x` under only `q<1`, with field division totalized; its Jacobi-theta `HasSum` form strengthens the hypotheses to `0<q<1` and `0<x<1`.  This is a real API: it proves no complex continuation, pole classification, `q→1` classical-gamma limit, log-convexity, uniqueness, or `q`-digamma calculus. |
...
| Central Gaussian reduction at base `q²` | `FabiusFunction.CentralQBinomialReduction` | Exhaustive public surface: zero definitions and exactly six theorems, `finiteQPochhammerIn_mul_neg`, `finiteQPochhammerIn_two_mul`, `finiteQPochhammerIn_map_ringHom`, `central_gaussianBinomial_sq_mul_int`, `central_gaussianBinomial_sq_mul`, and `central_gaussianBinomial_sq_div`.  The sign-pairing identity `(x;q)_n(-x;q)_n=(x²;q²)_n`, the even--odd dissection `(q;q)_(2k)=(q;q²)_k(q²;q²)_k`, and finite-symbol naturality hold over commutative rings with no nonvanishing, division, or positivity premise, including `n=0`, `k=0`, and `q=0`.  The polynomial cancellation step is proved in `ℤ[X]`, then naturality gives `[2k,k]_(q²)(q²;q²)_k=(q;q²)_k(-q;q)_(2k)` in every commutative ring, including the zero ring.  Its field quotient form assumes exactly `(-q;q)_(2k) ≠ 0` and `(q²;q²)_k ≠ 0`.  No infinite-product, convergence, root-of-unity regularization, or asymptotic statement is asserted. |
| Cyclotomic factorization of finite `q`-factorials and Gaussian polynomials | `FabiusFunction.CyclotomicFactorization` | Exhaustive public surface: zero definitions and exactly seven theorems, `div_add_div_le_div`, `div_le_div_add_div_add_one`, `mem_range_and_mem_divisors_iff`, `finiteQPochhammerIn_X_eq_prod_cyclotomic`, `finiteQPochhammerIn_X_eq_gaussianBinomial_mul`, `prod_cyclotomic_pow_div_extend`, and `gaussianBinomial_X_eq_prod_cyclotomic`.  The two floor-division bounds assume exactly `k ≤ n` and `0 < d` and show that each displayed Gaussian cyclotomic exponent is zero or one; the range/divisor exchange is total in `n,j,d`.  Over every commutative ring, including the zero ring and `n=0`, `(X;X)_n=(-1)^n∏_(1≤d≤n) Φ_d(X)^(n/d)`; the factorial/Gaussian bridge assumes exactly `k ≤ n`, and product-range extension exactly `m ≤ n`.  The terminal factorization `[n,k]_X=∏_(1≤d≤n) Φ_d(X)^(n/d-k/d-(n-k)/d)` assumes exactly `k ≤ n` and that the coefficient ring is an integral domain.  No theorem here treats `k>n`, evaluates at roots of unity, or asserts coefficient unimodality or asymptotics. |
| Cyclotomic carry divisibility and multiple-index primitive-root value | `FabiusFunction.CyclotomicDivisibility` | Exhaustive zero-definition/three-theorem surface: `cyclotomic_exponent_eq_one_iff`, `cyclotomic_dvd_gaussianBinomial_iff`, and `gaussianBinomial_mul_isPrimitiveRoot`.  The first two assume exactly `k ≤ n` and `0 < d`; they identify exponent one with `n % d < k % d` and, over `ℚ[X]`, identify that carry with `cyclotomic d ℚ ∣ gaussianBinomial X n k`.  The last works in a commutative integral domain, assumes `0 < n` and `IsPrimitiveRoot ζ n`, and gives `[a*n,b*n]_ζ=(a.choose b : R)` for arbitrary naturals `a,b`, including `b>a` through Gaussian/binomial zero extension.  The `{0,1}` exponent bound itself remains owned by `CyclotomicFactorization`; there is no coefficient, convergence, or asymptotic claim. |
| Complete primitive-root block | `FabiusFunction.PrimitiveRootBlock` | Exhaustive zero-definition/three-theorem surface: `gaussianBinomial_isPrimitiveRoot_eq_zero`, `neg_one_pow_mul_pow_choose_two`, and `finiteQPochhammerIn_isPrimitiveRoot`.  In every commutative integral domain, the first theorem assumes `IsPrimitiveRoot ζ d` and exactly `0<k<d` (hence `0<d`) and proves `[d,k]_ζ=0`; the phase and complete-block theorems explicitly assume `0<d` and `IsPrimitiveRoot ζ d`, and the latter holds for arbitrary `y`, including `y=0`, as `(y;ζ)_d=1-y^d`.  This is finite algebra only: it asserts no infinite-product convergence or limiting root regularization. |
| Evaluated q-Lucas theorem at a primitive root | `FabiusFunction.QLucas` | Exhaustive zero-definition/eight-theorem surface: `two_mul_choose_two`, `add_mul_add_sub_one`, `choose_two_add`, `coeff_finiteQPochhammerIn_neg_X`, `finiteQPochhammerIn_neg_X_block`, `coeff_block_pow_mul`, `pow_choose_two_add_mul_eq`, and `gaussianBinomial_q_lucas`.  The first three are unconditional natural arithmetic.  Coefficient extraction and `coeff_block_pow_mul` need only a commutative ring; the latter assumes `s<d` and `B.natDegree<d`.  The block factorization and phase identity use a commutative integral domain with `0<d` and `IsPrimitiveRoot ζ d`.  Under those same structural hypotheses, q-Lucas assumes exactly `b<d` and `s<d` and proves `[a*d+b,r*d+s]_ζ=(a.choose r:R)[b,s]_ζ` for arbitrary `a,r`, without `r≤a` or `s≤b` because both coefficients zero-extend.  This proves an evaluated identity only: the manuscript polynomial congruence modulo `Φ_d` and its minimal-polynomial lift remain unformalized, so `thm:q-lucas` is Partial. |
| MacMahon q-Catalan polynomial | `FabiusFunction.QCatalan` | Exhaustive public surface: the definition `qCatalan` and eleven theorems, `map_qInt`, `qInt_X_monic`, `qInt_X_natDegree`, `X_sub_one_mul_qInt`, `qInt_X_eq_prod_cyclotomic`, `qInt_X_dvd_gaussianBinomial_rat`, `qInt_X_dvd_gaussianBinomial_int`, `qInt_X_mul_qCatalan`, `qCatalan_natDegree`, `qCatalan_eval_one_mul`, and `qCatalan_eval_one`.  Naturality is semiring-level; monicity and degree require a nontrivial commutative ring; `(X-1)[n+1]_X=X^(n+1)-1` holds in every commutative ring, including the zero ring.  The cyclotomic product and divisibility steps are fixed in `ℚ[X]` and `ℤ[X]`.  For every `n`, including zero, `qCatalan n` is the monic polynomial quotient `/ₘ`, not field division, and satisfies `[n+1]_X C_n=[2n,n]_X`, degree `n(n-1)`, `(n+1)C_n(1)=choose(2n,n)`, and `C_n(1)=catalan n`.  No coefficient positivity/unimodality, analytic continuation, or asymptotic result is asserted. |
| Newton interpolation and the geometric grid | `FabiusFunction.NewtonInterpolation` | Exhaustive public surface: two definitions, `newtonCoeff` and `newtonInterpolant`, and thirteen theorems, `newtonCoeff_eq`, `newtonCoeff_zero`, `newtonCoeff_mul_prod`, `newtonPoly_succ`, `eval_newtonPoly`, `degree_newtonPoly_lt`, `newtonPoly_eq_interpolate`, `eq_newtonPoly_of_eval_eq`, `coeff_newtonPoly_self`, `newtonCoeff_eq_sum`, `nodal_range_pow`, `prod_erase_pow_sub_pow`, and `newtonCoeff_pow_eq_sum`.  The polynomial definition is named `newtonInterpolant` to preserve the established scalar-sequence `Fabius.newtonPoly` API; the theorem names retain their compatibility spellings.  Everything is finite algebra over an arbitrary field.  Definitions, recurrence, degree, and top-coefficient laws are total even at repeated nodes; the multiplied triangular step assumes its denominator product nonzero, and evaluation at node `i≤n` assumes only the current product `∏j<i(v i-v j)≠0`.  Lagrange equality and divided differences require `InjOn v (range (k+1))`; uniqueness additionally assumes `P.degree< n+1` and the displayed node values.  On the geometric grid, `nodal_range_pow` assumes `q≠0`, the erased-product formula assumes only `j≤k`, and the explicit Newton coefficient formula assumes power-map injectivity on `range (k+1)`.  No infinite interpolation or convergence theorem is present. |
| Jackson q-beta integral | `FabiusFunction.QBetaIntegral` | Exhaustive public surface: the total real definition `qBeta` and eight theorems, `qNumber_pos`, `qBeta_term_eq`, `qBeta_eq_prod`, `qBeta_eq_qGamma`, `qBeta_comm`, `qBeta_pos`, `qBeta_add_one_left`, and `qBeta_add_one_right`.  `qNumber_pos` assumes exactly `0<q<1` and `z>0`; the term identity assumes `0<q<1`, `y>0`, and arbitrary real `x` and natural `n`.  The remaining six theorems assume exactly `0<q<1` and `x,y>0`, giving the infinite-product evaluation, q-gamma quotient, symmetry, positivity, and both unit-shift recurrences.  The definition is total outside this region, but the API claims no `q=0` or `q=1` endpoint, zero parameter, complex continuation, or classical-limit theorem. |
| Integer upper-index Gaussian coefficients and reciprocal finite q-binomial series | `FabiusFunction.GaussianBinomialInteger` | Exhaustive public surface (one definition and ten theorems): `gaussianBinomialZ`; `finiteQPochhammerIn_inv_base_eq`, `finiteQPochhammerIn_mul_pow_inv_base`, `finiteQPochhammerIn_pow_div`, `gaussianBinomialZ_zero_right`, `gaussianBinomialZ_natCast`, `gaussianBinomialZ_succ`, `gaussianBinomialZ_succ'`, `gaussianBinomialZ_neg_natCast`, `hasSum_reciprocal_finiteQPochhammerIn`, and `hasSum_reciprocal_finiteQPochhammerIn_neg`.  The finite identities work over a field: the two inversion formulas require `q ≠ 0`; the quotient bridge requires only `(q;q)_k ≠ 0`; the zero-column law is unconditional; natural-index agreement and negative-index reflection require `q ≠ 0` and `(q;q)_k ≠ 0`; and both integer q-Pascal laws require `q ≠ 0` and `(q;q)_(k+1) ≠ 0`.  Over a complete normed field the reciprocal series assumes exactly `‖q‖ < 1` and `‖z‖ < 1` for every `m`, including `m=0`; its negative-index spelling also requires `q ≠ 0`.  The definition is total, with no meromorphic continuation or unit-circle boundary claim. |
| Complex upper-index Gaussian coefficients | `FabiusFunction.GaussianBinomialComplexOrder` | Exhaustive public surface (one definition and five theorems): `gaussianBinomialC`; `gaussianBinomialC_intCast`, `gaussianBinomialC_natCast`, `hasSum_gaussianBinomialC_add`, `hasSum_qPochhammerC_inv`, and `hasSum_qPochhammerC`.  The definition uses principal complex powers.  Integer agreement is unconditional; natural agreement requires `q ≠ 0` and `(q;q)_k ≠ 0`.  The upper-parameter and reciprocal series require exactly `‖q‖ < 1` and `‖z‖ < 1`; the generalized finite series additionally requires `q ≠ 0` and `‖z*q^α‖ < 1`.  No boundary theorem, branch-independent power, or analytic continuation is asserted. |
| Terminating balanced q-Pfaff--Saalschütz summation | `FabiusFunction.QPfaffSaalschutz` | Exhaustive zero-definition/three-theorem surface: `finiteQPochhammerIn_ne_zero_of_le`, `q_pfaff_saalschutz_term`, and `q_pfaff_saalschutz`.  Over a field, prefix nonvanishing uses only `m ≤ n` and nonvanishing of the longer symbol.  The term and finite sum require `q,a,b,c ≠ 0` and nonvanishing of `(q;q)_n`, `(c;q)_n`, `(abq/(cq^n);q)_n`, and `(c/(ab);q)_n`; the term additionally assumes `k ≤ n`.  The `n=0` case is included.  This is finite algebra and makes no convergence claim. |
| Noncommutative q-multinomial theorem | `FabiusFunction.QuantumMultinomial` | Exhaustive zero-definition/five-theorem surface: `sum_antidiagonalTuple_succ`, `gaussianBinomial_eq_evalRingHom_quantum`, `gaussianBinomial_symm'`, `Commute.qMultinomial_left`, and `quantum_multinomial`.  The tuple decomposition needs only an additive commutative monoid; the remaining results hold in an arbitrary, possibly noncommutative semiring, with symmetry assuming `k ≤ n`.  If `q` commutes with every `x_i` and `x_j*x_i = q*(x_i*x_j)` for `i<j`, the final theorem expands every finite power into ordered monomials weighted by `qMultinomial`, including `r=0` and `n=0`.  It assumes neither invertibility nor global centrality of `q` and makes no convergence claim. |
| Gaussian-binomial inversion and sharp elementary bounds | `FabiusFunction.GaussianBinomialBounds` | Exhaustive zero-definition/six-theorem surface: `gaussianBinomial_inv`, `one_le_gaussianBinomial`, `finiteQPochhammerIn_pow_le_one`, `gaussianBinomial_le_inv_qPochhammerInfIn`, `pow_le_gaussianBinomial_of_one_lt`, and `gaussianBinomial_le_pow_div_of_one_lt`.  The real positivity theorem `finiteQPochhammerIn_self_pos` is imported and reused from `GeneralQConditionNumber`; it is not newly declared here.  The inversion identity holds in every field for `q ≠ 0` and `k ≤ n`; the lower bound one holds in every strict ordered field for `q ≥ 0` and `k ≤ n`.  Over the reals, the reused positivity input and the infinite-product upper bound use `0 ≤ q < 1`, the finite-factor bound uses `0 ≤ q ≤ 1`, and the two large-base estimates use `Q > 1`, always with `k ≤ n` where Gaussian coefficients occur.  No above-diagonal, limiting, or asymptotic estimate is claimed. |
...
`FabiusInverseComputable.lean` applies that construction to `fabiusReal`, its
...
the computable positive reciprocal inverse modulus as input; the canonical
report's stronger abstract theorem that derives a modulus and effective
uniform continuity from a computable positive gap sequence remains outside
Lean.
