...
> union contains exactly 903 source modules and 11,448 public declarations.
...
> origin inventory has 11,447 declarations because it lacks this tree's unique
...
> The Lambert branch-gap union adds `LambertWBranchGapBernoulli` 0+4:
...
> theorem excludes the removable origin, the branch theorem excludes both
> endpoints, and no remainder or higher-Puiseux claim is made.  Together with
> the three finite branch-coordinate modules, the four-module Lambert union is
> 4+36, forty declarations.
...
| Actual Bernoulli series for the real Lambert branch gap and its exact complex convergence radius | `FabiusFunction.LambertWBranchGapBernoulli` | Exhaustive public surface (zero definitions and four theorems): `summable_norm_bernoulli_mul_pow_div_factorial`, `summable_bernoulli_mul_pow_div_factorial_iff`, `hasSum_bernoulli_mul_pow_div_factorial`, and `principalLambertW_lowerLambertW_eq_bernoulliSeries`.  For real `z` with `|z| < 2π`, the first proves absolute convergence of `∑ B_n z^n/n!`.  For every complex `z`, the second proves that the complex series is summable exactly when `‖z‖ < 2π`; thus it diverges throughout the boundary and exterior, with the proof detecting an even-indexed subsequence whose term norms stay at least `2`.  For real `z ≠ 0`, the third gives the actual `HasSum` value `z/(exp z-1)`.  On `x ∈ (-exp(-1),0)` with `Δ=branchGap x<2π`, the final conjunction is exactly `W₀(x)=-∑ B_nΔ^n/n!` and `W₋₁(x)=-Δ-∑ B_nΔ^n/n!`, making only the Lambert Guide label `eq:pair-Bernoulli-general` Exact.  Together with the three finite branch-coordinate modules this is a four-module surface of four definitions and 36 theorems, 40 declarations.  The exact complex convergence theorem includes `z=0`, but no complex `HasSum` quotient evaluation on the punctured disk or separate theorem giving the series value `1` at the removable origin is asserted; the real quotient statement deliberately excludes `z=0`, and neither endpoint of the real branch pair is included.  No remainder estimate or higher/convergent Puiseux expansion is asserted. |
