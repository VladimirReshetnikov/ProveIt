...
> the facade union contains exactly 665 source modules and 8,819 public
> declarations, with no missing module headers or public-declaration doc
> comments.  The union retains the exhaustive finite and infinite q-series,
> unconditional complex/generic Pochhammer bridge, canonical generic
> analytic-order theorem, polynomial Jackson/q-Leibniz, Lambert-series, real
> q-gamma, generalized-product identifiability, Gaunt--Wigner-square, centered
> Appell, arbitrary-phase synthesis, prime-power, normal-convergence,
> classical-limit, q-Taylor, q-partial-fraction, and integer-index APIs.  Its
> q-series census includes `RvachevPochhammerFactorization` 1+10,
> `QPochhammerEntire` 0+5, `QPochhammerInfinite` 1+29,
> `QMultinomial` 1+9, and `GaussianBinomialPolynomialStructure` 0+5.  It also
> includes Heine transformation, q-Gauss summation, complex-order
> q-Pochhammer, infinite-product bounds, general basic hypergeometric series,
> generic Gaussian palindromicity, Jackson integration, both q-exponentials,
> theta quasi-periodicity, Jacobi's cubic identity, parameter/order
> derivatives of the infinite q-Pochhammer symbol, the six-theorem central
> Gaussian reduction, the seven-theorem cyclotomic factorization, cyclotomic
> divisibility, primitive-root blocks, evaluated q-Lucas, q-Catalan, Newton
> interpolation, and Jackson q-beta APIs.  The last six modules are
> `CyclotomicDivisibility` 0+3, `PrimitiveRootBlock` 0+3, `QLucas` 0+8,
> `QCatalan` 1+11, `NewtonInterpolation` 2+13, and `QBetaIntegral` 1+8:
> four definitions and 46 theorems.  Relative to the 649/8,698 checkpoint,
> this exact tree union adds sixteen modules and 121 declarations.  The
> rigorous forward q-monograph ledger is 80 Exact, 86 Partial, 108 None, and
> 8 interface rows.  In particular, `thm:q-lucas` is Partial: Lean proves the
> evaluated primitive-root identity, while the manuscript's polynomial
> congruence modulo `Φ_d` still needs a minimal-polynomial lift.  The new Exact
> rows are `lem:root-block`, `cor:squarefree-cyclotomic`, `cor:q-catalan`,
> `thm:qbeta`, `cor:qbeta-recurrence`, `thm:geometric-newton`, and
> `cor:geometric-triangular`.  `cor:babbage-derivative` also remains Partial:
> its value is formalized, but its derivative is not.  The retained
...
> 167, 150, 237, 301, 41, 88, 377, 158, and 357 A4 pages.  The 150-page
> walkthrough and the other committed PDFs predate this six-module union.
> Their current TeX sources contain post-render unions, including the centered
...
| Generic Gaussian-polynomial degree, palindromicity, and mean | `FabiusFunction.GaussianBinomialPalindromic` | Exhaustive public surface: zero definitions and twelve theorems, `reflect_add_of_natDegree_le`, `reflect_one'`, `gaussianBinomial_natDegree_le`, `gaussianBinomial_zero_left`, `gaussianBinomial_diag'`, `reflect_gaussianBinomial`, `coeff_gaussianBinomial_reflect`, `coeff_gaussianBinomial_zero`, `coeff_gaussianBinomial_top`, `gaussianBinomial_natDegree`, `gaussianBinomial_monic`, and `two_mul_derivative_gaussianBinomial_eval_one`.  Over every commutative semiring it supplies the reflection helpers, the degree bound, zero/diagonal values, reflection in degree `k * (n - k)`, constant and top coefficients one, bounded-index coefficient reversal, and the division-free mean identity.  Exact degree and monicity require only that the semiring be nontrivial.  Together with the existing Gaussian symmetry and reciprocity, this keeps the canonical forward `q`-binomial structure theorem exact.  It does not prove that the coefficient of `q` is one. |
