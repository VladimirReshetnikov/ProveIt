...
| Half-shifted Rademacher sine formula for the Thue--Morse sign | `FabiusFunction.RademacherSine` | Exhaustive public inventory: zero definitions and nine theorems, `sin_pi_mul_eq_neg_one_zpow_floor`, `sin_pi_mul_fract_pos`, `sign_sin_pi_mul`, `floor_rademacherPoint`, `fract_rademacherPoint_ne_zero`, `sign_sin_rademacherPoint`, `sign_sin_rademacherPoint_eq_one_of_lt`, `thueMorseSign_eq_prod_sign_sin`, and `thueMorseSign_eq_tprod_sign_sin`.  The factored square wave is unconditional; its positivity and sign forms assume exactly a nonzero fractional part.  The half-shifted floor, nonintegrality, and bit-sign identities hold for every `n,j`; a factor is one under `n < 2^j`, the finite product assumes `n < 2^m`, and the `tprod` formula is unconditional because all but finitely many factors are one. |
...
| Executable rational Rvachev--Legendre Gram entries, matrices, determinants, and ratios | `FabiusFunction.FabiusLegendreRationalGram` | Exhaustive public inventory: three definitions, `rvachevLegendreGramEntryRat`, `rvachevLegendreGramMatrixRat`, and `rvachevLegendreGramDetRat`; and eleven theorems, `rvachevLegendreGramEntryRat_eq_momentPairing`, `rvachevLegendreGramMatrixRat_apply`, `rvachevLegendreGramMatrixRat_eq_polynomialMomentGramMatrix`, `rvachevLegendreGramEntryRat_cast`, `rvachevLegendreGramMatrixRat_cast`, `rvachevLegendreGramDetRat_cast`, `rvachevLegendreGramDetRat_eq_prod_leadingCoeff_sq_mul_rvachevHankelDetRat`, `rvachevLegendreGramDetRat_zero`, `rvachevLegendreGramDetRat_pos`, `rvachevOrthoNormRat_eq_rvachevLegendreGramDetRat_ratio`, and `rvachevJacobiSubdiagonalRat_eq_rvachevLegendreGramDetRat_ratio`.  All three definitions are finite and executable.  The rational identities and positivity are unconditional; exactly the entry, matrix, and determinant real-cast bridges take `F : BoundedFabius` and `hF : IsFabius F`.  This gives entrywise rationality by bounded coefficient sums and finite determinant/norm/Jacobi ratios, without making `RvachevRationalJacobi` itself computable.  It uses no Gaunt/Wigner/`3j` expansion and proves no roots, quadrature, Christoffel reconstruction, infinite Jacobi product, or asymptotic theorem; two private determinant helpers are excluded. |
| Exact low-order rational Rvachev--Legendre Gram and fourth Jacobi data | `FabiusFunction.FabiusLegendreRationalGramValues` | Exhaustive public inventory: zero definitions and eleven theorems, `moment_four`, `rvachevLegendreGramDetRat_one`, `rvachevLegendreGramDetRat_two`, `rvachevLegendreGramDetRat_three`, `rvachevLegendreGramDetRat_four`, `rvachevLegendreGramDetRat_five`, `rvachevOrthoNormRat_four`, `rvachevJacobiSubdiagonalRat_three`, `hankelRatio_four`, `integral_sq_upOrthoPolynomial_four`, and `hankelRatio_four_div_three`.  Exact rational computation gives the raw eighth moment, Gram determinants of orders one through five, `H_4 = 26727424 / 55791736875`, and the zero-based index-three value `beta_4 = 835232 / 4640643`.  The first eight theorems are unconditional rational identities; exactly the last three take `F : BoundedFabius` and `hF : IsFabius F`.  This closes only the displayed `H_4`/`beta_4` and low-order determinant data, not general Gaunt/Wigner/`3j` entries, roots/quadrature, Christoffel reconstruction, continued fractions/Padé, or Jacobi asymptotics; one private coefficient certificate is excluded. |
...
| Executable rational Legendre coefficients, up-law Gram determinants, and exact low-order values | `FabiusFunction.LegendrePolynomialRational`, `FabiusFunction.FabiusLegendreRationalGram`, `FabiusFunction.FabiusLegendreRationalGramValues` | Exhaustive public inventory: `LegendrePolynomialRational` has the definitions `legendrePolynomialCoeffRat`, `legendrePolynomialRat` and the theorems `legendrePolynomialRat_cast`, `coeff_legendrePolynomialRat`, `natDegree_legendrePolynomialRat`, `coeff_legendrePolynomialRat_self`, `coeff_legendrePolynomialRat_self_ne_zero`, `coeff_legendrePolynomialRat_self_div_succ`; `FabiusLegendreRationalGram` has the definitions `rvachevLegendreGramEntryRat`, `rvachevLegendreGramMatrixRat`, `rvachevLegendreGramDetRat` and the theorems `rvachevLegendreGramEntryRat_eq_momentPairing`, `rvachevLegendreGramMatrixRat_apply`, `rvachevLegendreGramMatrixRat_eq_polynomialMomentGramMatrix`, `rvachevLegendreGramEntryRat_cast`, `rvachevLegendreGramMatrixRat_cast`, `rvachevLegendreGramDetRat_cast`, `rvachevLegendreGramDetRat_eq_prod_leadingCoeff_sq_mul_rvachevHankelDetRat`, `rvachevLegendreGramDetRat_zero`, `rvachevLegendreGramDetRat_pos`, `rvachevOrthoNormRat_eq_rvachevLegendreGramDetRat_ratio`, and `rvachevJacobiSubdiagonalRat_eq_rvachevLegendreGramDetRat_ratio`; `FabiusLegendreRationalGramValues` has no public definitions and the eleven theorems `moment_four`, `rvachevLegendreGramDetRat_one`, `rvachevLegendreGramDetRat_two`, `rvachevLegendreGramDetRat_three`, `rvachevLegendreGramDetRat_four`, `rvachevLegendreGramDetRat_five`, `rvachevOrthoNormRat_four`, `rvachevJacobiSubdiagonalRat_three`, `hankelRatio_four`, `integral_sq_upOrthoPolynomial_four`, and `hankelRatio_four_div_three`.  The scalar coefficients and the rational entry, matrix, and determinant are finite executable definitions; only the convenience polynomial wrapper is noncomputable.  The direct bounded moment sum closes entrywise rationality and casts to every genuine up-law pairing, while the positive rational determinants make the norm and zero-based Jacobi formulas genuine ratios.  The values module supplies exact finite native-computation certificates, including the fourth nontrivial even-moment value (the raw eighth moment) and determinants through order five.  It proves `H_4 = 26727424/55791736875` rationally and as both the real Hankel ratio and squared integral, while `beta_4 = 835232/4640643` is proved rationally and as the quotient of the fourth and third real Hankel ratios.  This is not a general native evaluator.  Equality with the finite Gaunt/Wigner/`3j` formulas, Christoffel reconstruction, and infinite or asymptotic Jacobi theory remain open. |
...
| Formal quarter-anchor Catalan germ | `FabiusFunction.QuarterCatalanGerm` | Exhaustive public surface (two definitions and nine theorems): `quarterCatalanCoefficient`, `quarterCatalanGermSeries`, `quarterCatalanCoefficient_zero`, `quarterCatalanCoefficient_succ_eq_report`, `quarterCatalanGermSeries_coeff`, `quarterCatalanGermSeries_coeff_succ`, `quarterCatalanGermSeries_constantCoeff`, `quarterCatalanGermSeries_equation`, `powerSeries_quadratic_injectiveOn_zeroConstant`, `eq_quarterCatalanGermSeries_of_equation`, and `existsUnique_quarterCatalanGermSeries`.  This uniquely solves `D + 4D^2 = (4/9)X` with zero constant coefficient in `ℚ[[X]]` and proves every coefficient formula; it asserts no analytic convergence or identification with a real inverse germ. |
