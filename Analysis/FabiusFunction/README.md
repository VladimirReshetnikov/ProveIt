...
> 952 source modules and 11,881 explicit public declarations.  The documentation audit
...
> tranche contributes nine modules and 94 lexical declarations, and the live
> census is 952/11,881, again with no documentation gaps.  The separate
...
| Dickson and Neumann well-basedness | `FabiusFunction.TransseriesWellBased` | Exhaustive written zero-definition/five-theorem surface: `dickson_isPWO`, `dickson_antichain_finite`, `dickson_isPWO_pi`, `neumann_isPWO`, and `neumann_finite_factorizations`; `to_additive` additionally generates `neumann_add_isPWO` and `neumann_finite_decompositions`, which the lexical census deliberately does not count.  The manuscript labels `q0:lem:dickson` and `q0:lem:neumann` are **Exact** once its reverse-well-order/strict-growth convention is translated to Lean's `Set.IsPWO` order (equivalently by using the order dual for the manuscript orientation).  The Neumann results use an ordered cancel commutative monoid and do not construct a Hahn-series type. |
...
| Wright omega through the principal real Lambert branch | `FabiusFunction.WrightOmega` | Exhaustive one-definition/twelve-theorem surface: `wrightOmega`, `wrightOmega_pos`, `wrightOmega_add_log`, `principalLambertW_eq_wrightOmega_log`, `wrightOmega_leftInverse`, `wrightOmega_strictMono`, `wrightOmega_one`, `one_le_wrightOmega`, `wrightOmega_le_self`, `sub_log_le_wrightOmega`, `wrightOmega_envelope`, `add_one_div_two_le_wrightOmega`, and `tendsto_wrightOmega_atTop`.  The defining equation, inverse law, monotonicity, positivity, anchor, envelope, Lambert relation, and divergence clauses of `plt:prop:mot-omega-basic` are exact over the reals.  The full proposition remains partial only because real analyticity is not formalized. |
