...
> Appell/deconvolution results, expanded formula-and-proof commentary, and the
> source-only five-theorem fixed-nome complex q-Pochhammer entire-function
> overlay, so both PDFs are historical pending a post-merge three-pass rebuild.
...
> In particular, the filed package's mixed 20-row ledger verifies its current
> source and retained PDF as separate payloads; it does not establish
> source/PDF parity.  Package-local notices record the remaining rebuild debt,
> including the historical q-series renders.
