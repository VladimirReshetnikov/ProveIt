...
> declarations in total, giving the historical 934/11,709 pre-transseries
> checkpoint.  The subsequent transseries tranche brings the frozen upstream
> tree to 957/11,920; retaining the unconditional Pochhammer bridge gives the
> historical merge-union checkpoint 957/11,921, and the residual-existence
> certificate `exists_eq_in_residual_interval` is included in the live semantic
> union computed by `scripts/doc_audit.py` and pinned in
> `docs/doc_audit_baseline.json`.  The separate
...
> At their named historical checkpoints, the retained primary exposition,
> Lean walkthrough, canonical frontier, Representation Frontiers, filed New
> Frontiers, notation catalogue, Integration-and-Transform master,
> comb-interpolation, and q-series synthesis PDFs contained respectively
...
> the merged bytes.  The fixed-26 checkpoint linked above is the sole live
> record of the replacement rebuild and its per-root parity gates.
