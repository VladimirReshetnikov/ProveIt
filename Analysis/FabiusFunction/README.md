...
> semantic union contains exactly 923 source modules and 11,611 public
> declarations.  The merge target contributes 11,610 declarations; the retained
> unconditional public `complexQPochhammerInf_eq_qPochhammerInfIn` bridge in
> `RvachevPochhammerFactorization` 1+10 contributes the additional declaration.
...
> leaf, the centered-MGF Laurent-leading theorem, and finite-prefix Appell
> recovery, are included in
...
> complex-product bridge `GeometricUniformComplexMomentProduct` 1+2, the
...
> declaration count by one and yields the live 923/11,611 semantic union.  The
...
> forward q-monograph ledger is 177 Exact, 82 Partial, 15 None, and 8
> interface rows; its source concordance is 99 Lean, 379 human, 60 N/A, and
...
> All ten current source/PDF rows are synchronized; earlier receipts below
> remain historical provenance for their named checkpoints.
>
...
> receipts certify only their named source checkpoints, and parity rebuilds are
> pending wherever the current sources postdate them.
> Filed New Frontiers and the notation catalogue retain historical 41- and
> 88-page receipts; the comb-interpolation 158-page receipt is historical and is
> superseded by the current 160-page receipt above.  Checksum ledgers remain
