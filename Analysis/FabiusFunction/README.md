...
| Positive-real fractional Volterra calculus over real normed spaces | `FabiusFunction.FractionalVolterra`, `FabiusFunction.FractionalVolterraSemigroup`, `FabiusFunction.FractionalVolterraCalculus` | `fractionalVolterra`, `fractionalVolterra_self`, `fractionalVolterra_congr`, `fractionalVolterra_smul`, `intervalIntegrable_fractionalVolterra_kernel`, `fractionalVolterra_add_input`, `fractionalVolterra_one`, `fractionalVolterra_nat_succ`, `intervalIntegral_fractionalVolterra_betaKernel`, `intervalIntegrable_fractionalVolterra_betaKernel`, `intervalIntegral_fractionalVolterra_normalizedBetaKernel`, `fractionalVolterra_add`, `fractionalVolterra_affine`, `fractionalVolterra_comp_affine`, `fractionalVolterra_add_one_deriv`, `fractionalVolterra_add_one_deriv_of_eq_zero`; the definition and algebraic endpoint/integer rules use oriented interval integrals, while the classical kernel, semigroup, affine, and order-raising results state their ordered-endpoint hypotheses explicitly; affine covariance holds for arbitrary real order at every positive scale `c` when `a ≤ x`, without regularity, integrability, completeness, or cancellation assumptions; on a complete target, order raising assumes `0 < α`, `a ≤ x`, continuity of the primitive on `[a,x]`, an interval-integrable displayed derivative, and only its right derivative on the open interval, and includes the exact Gamma-normalized left-endpoint term and the degenerate interval; no classical order-zero or reversed-endpoint interpretation, Caputo/Riemann--Liouville derivative operator, complex order, or general power/constant evaluation is claimed |
| Exact first fractional shifts for the Fabius--Rvachev system | `FabiusFunction.FabiusFractionalVolterra` | `fractionalVolterra_add_one_extendedFabius_of_nonneg`, `fractionalVolterra_add_one_fabiusReal`, `fractionalVolterra_add_one_rvachevUp`; for every `α > 0`, the signed extension has the dyadic shift at `x ≥ 0`, the bounded function has it at `x ∈ [0,1]`, and `up` based at `-1` bridges to the bounded function at `(x+1)/2` for every `x ≥ -1`; signed negative-endpoint branches, iterated fractional shifts, endpoint-moment and weighted-power formulas, fractional inverse/quantile identities, hierarchy-adapted semigroups, and complex order remain separate frontiers |
...
| Geometrically weighted uniform laws | `FabiusFunction.GeometricUniformLaw` | `geometricUniformWeight`, `hasSum_geometricUniformWeight`, `geometricUniformSeries`, `geometricUniformSeries_split`, `geometricUniformDistribution_selfSimilar`, `geometricUniformDistribution_absolutelyContinuous`, `geometricUniformDistribution_nullSingletonClass`, `geometricUniformDistribution_reflection`, `geometricUniformDistribution_Icc` |
| CDF and explicit density of the geometric uniform law | `FabiusFunction.GeometricUniformCDF` | `geometricUniformCDF`, `monotone_geometricUniformCDF`, `geometricUniformCDF_nonneg`, `geometricUniformCDF_le_one`, `measurable_geometricUniformCDF`, `continuous_geometricUniformCDF`, `geometricUniformCDF_reflection`, `geometricUniformCDF_one_half`, `geometricUniformCDF_zero_of_nonpos`, `geometricUniformCDF_one_of_one_le`, `geometricUniformCDF_eq_integral`, `geometricUniformCDF_eq_intervalIntegral`, `geometricUniformDensity`, `geometricUniformCDF_hasDerivAt`, `deriv_geometricUniformCDF`, `continuous_geometricUniformDensity`, `geometricUniformDensity_nonneg`, `geometricUniformDensity_zero_of_nonpos`, `geometricUniformDensity_zero_of_one_le`, `support_geometricUniformDensity_subset_Ioo`, `support_geometricUniformDensity_subset_Icc`, `tsupport_geometricUniformDensity_subset_Icc`, `geometricUniformDensity_hasCompactSupport`, `geometricUniformDensity_reflection`, `geometricUniformDistribution_eq_withDensity`, `contDiff_geometricUniformCDF`, `contDiff_geometricUniformDensity`; continuity and CDF reflection assume that the absolute value of `q` is less than one, exterior CDF values assume `0 ≤ q < 1`, and the conditioning, density, `withDensity`, compact-support, and `C∞` results assume `0 < q < 1`; `geometricUniformDensity` is a total Lean definition, but only this strict positive range has the classical density interpretation (division is totalized at `q=0`, while negative `q` reverses the affine inequality); general-`q` Fourier products, moments/cumulants, shape theorems, and affine fixed-point uniqueness remain frontier targets |
| Product-probability and CDF representations | `FabiusFunction.ProbabilityRepresentation` | `weightedCoordinateSum_eq_weightedUniformSeries`, `weightedCoordinateSum_eq_geometricUniformSeries_one_half`, `weightedSumDistribution_eq_geometricUniformDistribution_one_half`, `weightedSumCDF_eq_geometricUniformCDF_one_half`, `weightedSumCDF_eq_fabiusReal`, `geometricUniformCDF_one_half_eq_fabiusReal`, `geometricUniformDensity_one_half_eq_rvachevUp`, `fabiusReal_eq_weightedSum_probability`, `rvachevUp_eq_weightedSumCDF`, `rvachevUp_eq_weightedSum_probability_global`; the dyadic smoothing, continuity, exterior-value, and reflection proofs now route through the half-base geometric CDF API |
| Generic finite moment functionals and Hankel Gram forms | `FabiusFunction.FiniteMomentGram` | At source checkpoint `b3720d4b5`: `momentFunctional`, `momentFunctional_apply`, `momentFunctional_monomial`, `momentFunctional_C`, `momentFunctional_X_pow`, `momentFunctional_of_linearMap`, `momentFunctional_injective`, `momentFunctional_eq_sum_support`, `momentFunctional_eq_sum_range`, `momentFunctional_map`, `momentPairing`, `momentPairing_apply`, `momentPairing_monomial`, `momentPairing_X_pow`, `momentPairing_isSymm`, `momentHankelMatrix`, `momentHankelMatrix_apply`, `momentHankelMatrix_transpose`, `momentHankelMatrix_succ_submatrix`, `momentHankelMatrix_map`, `momentHankelDet`, `momentHankelDet_zero`, `map_momentHankelDet`, `finiteMomentPairing`, `finiteMomentPairing_toMatrix`, `finiteMomentPairing_nondegenerate_iff`; the API is finite algebra over the stated semiring/ring/domain hypotheses and does not assert a measure representation, square-positivity, or determinant positivity |
| Fraction-free and normalized finite Gram--Stieltjes polynomials | `FabiusFunction.GramStieltjes` | At source checkpoint `b3720d4b5`: `gramStieltjesNumerator`, `gramStieltjesNumerator_coeff`, `gramStieltjesNumerator_natDegree_le`, `momentPairing_gramStieltjesNumerator_X_pow_eq_zero`, `momentPairing_gramStieltjesNumerator_X_pow_eq_det`, `gramStieltjesNumerator_coeff_top`, `momentPairing_gramStieltjesNumerator_eq_coeff_mul_det`, `momentPairing_gramStieltjesNumerator_self`, `gramStieltjesPolynomial`, `gramStieltjesPolynomial_natDegree_le`, `gramStieltjesPolynomial_coeff_top`, `gramStieltjesPolynomial_isMonicOfDegree`, `momentPairing_gramStieltjesPolynomial_X_pow_eq_zero`, `momentPairing_gramStieltjesPolynomial_eq_zero`, `eq_gramStieltjesPolynomial_of_isMonicOfDegree_of_orthogonal`, `momentPairing_gramStieltjesPolynomial_self`, `momentPairing_gramStieltjesPolynomial_self_ne_zero`; the fraction-free construction works over a commutative ring, while normalization, monicity, uniqueness, and the consecutive-Hankel-determinant self-pairing quotient use a field and the displayed nonzero-minor hypotheses; positivity, three-term recurrences/Jacobi parameters, roots, Gaussian or Lobatto quadrature, finite or infinite Jacobi/Stieltjes fractions, transform identification, and convergence remain open |
| Ordinary Cauchy--Stieltjes transforms of the canonical up and unit-interval laws | `FabiusFunction.CauchyTransform` | `rvachevCauchyTransform_apply`, `rvachevCauchyTransform_eq_integral_rvachevUp`, `hasDerivAt_rvachevCauchyTransform`, `analyticOn_rvachevCauchyTransform`, `fabiusStieltjesTransform_eq_two_mul_rvachevCauchyTransform`; logarithmic fixed points, the resolvent DDE, its measure-specific higher orbit, boundary values, and Jacobi fractions remain open |
...
| Absolutely-continuous two-function calculus for inverse-order pairs | `FabiusFunction.InversePairIntegral` | `intervalIntegral_deriv_mul_comp_add_comp_mul_deriv_of_lt_iff_lt`, `intervalIntegral_deriv_mul_fabiusReal_add_fabiusInv_mul_deriv`, `intervalIntegral_fabiusReal_add_fabiusInv_eq_one`; the generic theorem derives monotonicity and measurable clamped extensions of both clocks from the strict order equivalence and includes degenerate ordered intervals |
