...
> 970 source modules and 12,056 explicit public declarations.  The documentation
> audit reports no missing module headers or declaration comments.  The verified
> zero-definition/eight-theorem `StirlingCompleteHomogeneous` leaf is retained
> from the historical 961/11,974 checkpoint.  The exact-dyadic
...
> deltas.  The merged facade additionally contains `CayleyKernel` 1+14,
...
> Existing-module reconciliation brings `AppellSequence` to 3+34,
> `GaussianBinomialFixedColumnRate` to 0+9, `RemainderTransport` to 0+4,
> `TransseriesWellBased` to 0+7 explicitly written declarations, and
> `WrightOmega` to 1+13; the two `to_additive` products of the well-basedness
> API remain deliberately outside the lexical count.  The authoritative
> merged audit is 970/12,056 with
> no documentation gaps.  This is a conservative API inventory, not an
> automatic promotion of broader manuscript claims.  The separate
...
| --- | --- | --- |
| Generic asymptotic scales, vector-valued Poincaré expansions, and corrected flatness (`q0:def:scale`, `q0:def:poincare`, `q0:prop:uniqueness`, `q0:def:flat`, `q0:prop:invisible`) | `FabiusFunction.TransseriesScale`, `FabiusFunction.TransseriesFlat` | **Exact for the stated generic-vector clauses.** `TransseriesScale` remains exhaustively 3+6.  `TransseriesFlat` is now the semantic union of that generic-vector API (1+11) and a scalar compatibility API (3+11), hence 4+22 overall.  The added definitions are `flatSubmodule`, `AbsorbsScale`, and `powScale`; the added theorems are `mem_flatSubmodule_iff`, `IsFlat.mul_absorbsScale`, `absorbsScale_const`, `IsPoincareExpansion.add_isFlat`, `isFlat_sub_of_isPoincareExpansion`, `isPoincareExpansion_iff_isFlat_sub`, `isPoincareExpansion_zero_iff`, `powScale_eq_rpow`, `absorbsScale_of_isBigO_pow`, `isFlat_exp_neg`, and `isPoincareExpansion_add_exp_neg`.  The original declarations and hypotheses are unchanged: arbitrary-scale multiplication still needs an explicit absorption premise, and the generic power specialization still assumes eventual nonvanishing.  The scalar aliases and submodule packaging do not assert an unrestricted multiplier theorem or promote a further manuscript claim. |
| Well-based supports, power--logarithmic scales, and height comparison (`q0:lem:dickson`, `q0:lem:neumann`, `plt:lem:mot-dominance`, `q0:prop:height`) | `FabiusFunction.TransseriesWellBased`, `FabiusFunction.TransseriesScaleDominance`, `FabiusFunction.TransseriesPolyLogScale`, `FabiusFunction.TransseriesHeight` | `TransseriesWellBased` is exhaustively 0+7 in the lexical census: `dickson_isPWO`, `dickson_antichain_finite`, `dickson_isPWO_pi`, `neumann_isPWO`, `neumann_finite_factorizations`, `neumann_isPWO_orderDual`, and `neumann_finite_factorizations_orderDual`; `to_additive` also exports the non-lexically-counted `neumann_add_isPWO` and `neumann_finite_decompositions`.  Dickson and Neumann are **Exact**, with the source monomial order instantiated through `OrderDual` where required.  `TransseriesScaleDominance` is exhaustively 1+7: `plMonomial`; `tendsto_plMonomial_atTop_zero`, `plMonomial_div_eventuallyEq`, `tendsto_plMonomial_div_atTop_zero`, `tendsto_plMonomial_div_atTop_one`, `plMonomial_pos`, `tendsto_plMonomial_div_atTop`, `plMonomial_generators_dominance`.  `TransseriesPolyLogScale` is exhaustively 0+4: `isLittleO_plMonomial`, `isAsymptoticScale_plMonomial`, `isAsymptoticScale_plMonomial_pow`, `isAsymptoticScale_plMonomial_log`.  Together they make the displayed three-way limits and each chosen decreasing sequence scale **Exact**; the compound `plt:lem:mot-dominance` remains **Partial** because no unordered set-indexed scale API or named reverse generator implication is supplied.  `TransseriesHeight` is exhaustively 0+3: `isLittleO_log_pow_rpow`, `isLittleO_log_pow_id`, `isLittleO_pow_mul_log_pow_exp`.  These make the displayed quantitative equation and its power/log dual **Exact**; the broader recursive taxonomy of arbitrary nested exponential height and logarithmic depth remains **Partial**. |
| Polynomial block operators and Laurent-block differentiation (`plt:eq:mot-block-derivative`, `plt:lem:mot-block-antiderivative`) | `FabiusFunction.TransseriesBlockAntiderivative`, `FabiusFunction.TransseriesDifferentialBlock` | `TransseriesBlockAntiderivative` remains exhaustively 3+12.  `TransseriesDifferentialBlock` is now exhaustively 0+12: its previous eight theorems plus `derivation_val_inv`, `derivation_pow_inv`, `derivation_zpow_t`, and `derivation_block_zpow`.  The integer Laurent block formula and polynomial-operator clauses are **Exact** in their stated abstract settings.  The four additions give a unit-valued formulation of integer powers; they do not construct the source's concrete Laurent-polynomial differential ring.  The compound antiderivative lemma therefore remains **Partial**: generic uniqueness still assumes injectivity of evaluation at `L`, and a concrete-model injectivity proof and resonant uniqueness-up-to-constants wrapper remain absent. |
...
| Sequence-indexed asymptotic scales and Poincaré coefficients | `FabiusFunction.TransseriesScale` | Exhaustive one-structure/one-definition/three-theorem surface: `IsAsymptoticScale`, `IsPoincareExpansion`, `IsPoincareExpansion.isLittleO_succ_remainder`, `IsPoincareExpansion.tendsto_coeff`, and `IsPoincareExpansion.coeff_unique`.  This makes the sequence-indexed content of `q0:def:scale`, `q0:def:poincare`, and `q0:prop:uniqueness` **Exact** over an arbitrary filter and normed field.  Coefficient uniqueness assumes `[l.NeBot]`; the expansion uses the fixed first-omitted-term remainder convention.  It claims no convergence, no recovery modulo flat remainders, and no unordered-set or maximal-scale package. |
...
| Abstract differential-block bridge | `FabiusFunction.TransseriesDifferentialBlock` | Exhaustive zero-definition/nine-theorem surface: the prior natural-exponent API `derivation_pow_t`, `derivation_block`, `exists_block_primitive`, `derivation_block_zero`, and `exists_block_primitive_resonant`, plus `derivation_val_inv`, `derivation_pow_inv`, `derivation_zpow_t`, and `derivation_block_zpow`.  In any commutative algebra with a derivation satisfying `d(t)=-t²` and `d(L)=t`, the first five transport the polynomial operator to blocks `t^n p(L)` and supply natural-exponent primitives; when `t` is represented by a unit, the four new theorems prove the power and block derivative laws for every integer exponent.  Thus the displayed integer block-derivative identity is exact in the abstract unit model.  The module still does not construct the manuscript's full Laurent ring, and its primitive-existence API remains natural-exponent only. |
| Flat functions and the invisible-function problem | `FabiusFunction.TransseriesFlat` | Exhaustive four-definition/sixteen-theorem surface.  Definitions: `IsFlat`, `flatSubmodule`, `AbsorbsScale`, `powScale`.  Theorems: `isFlat_zero`, `IsFlat.add`, `IsFlat.neg`, `IsFlat.sub`, `IsFlat.const_smul`, `mem_flatSubmodule_iff`, `IsFlat.mul_absorbsScale`, `absorbsScale_const`, `IsPoincareExpansion.add_isFlat`, `isFlat_sub_of_isPoincareExpansion`, `isPoincareExpansion_iff_isFlat_sub`, `isPoincareExpansion_zero_iff`, `powScale_eq_rpow`, `absorbsScale_of_isBigO_pow`, `isFlat_exp_neg`, and `isPoincareExpansion_add_exp_neg`.  These make `q0:def:flat` and all three clauses of `q0:prop:invisible` exact, with the last clause strengthened to an iff.  Multiplicative closure is stated through the precise `AbsorbsScale` hypothesis; the fixed-polynomial-growth power-scale result and flatness of `exp (-x)` are explicit corollaries. |
...
| Linear--logarithmic remainder transport | `FabiusFunction.RemainderTransport` | Exhaustive zero-definition/four-theorem surface: `lipschitzOn_of_abs_deriv_le`, `transport_bound_mul`, `transport_first_order`, and `transport_bound`.  The derivative-to-Lipschitz bridge and both forms of `p0:eq:transport-bound` are exact.  `transport_first_order` also proves the displayed first-order displacement law with an explicit error, using a Lipschitz hypothesis on the core derivative (weaker than a second-derivative bound) together with the stated slope and perturbation controls. |
...
| Infinite q-binomial and reciprocal Euler theorems | `FabiusFunction.QBinomialTheoremInfinite` | Exhaustive 1-definition/27-theorem q-facing inventory.  The retained comparison, Gaussian-majorant, Euler-product, q-binomial, and reciprocal-Euler results are joined by `norm_finiteQPochhammerIn_pow_sub_one_le_exp_of_norm_le_one`, `isBigO_finiteQPochhammerIn_pow_sub_one`, `tendsto_gaussianBinomial_add_const_atTop`, `isBigO_gaussianBinomial_sub_inv`, and `isBigO_gaussianBinomial_add_sub_inv`.  These additions give an explicit product defect, fixed-column and fixed-shift convergence, and geometric `IsBigO` estimates under their displayed norm hypotheses; they do not add a norm-one boundary result or analytic continuation.  The shared finite zero-left identity is imported from `GaussianBinomialAtOne`. |
| Effective fixed-column Gaussian convergence | `FabiusFunction.GaussianBinomialFixedColumnRate` | Exhaustive zero-definition/nine-theorem surface: `norm_finiteQPochhammerIn_pow_sub_one_le_exp'`, `norm_finiteQPochhammerIn_pow_sub_one_le`, `norm_finiteQPochhammerIn_self_mul_gaussianBinomial_sub_one_le`, `norm_gaussianBinomial_sub_inv_finiteQPochhammerIn_le`, `norm_gaussianBinomial_add_sub_inv_finiteQPochhammerIn_le`, `gaussianBinomial_fixedColumn_relativeError_isBigO`, `gaussianBinomial_shifted_fixedColumn_relativeError_isBigO`, `gaussianBinomial_fixedColumn_error_isBigO`, and `gaussianBinomial_shifted_fixedColumn_error_isBigO`.  In a normed commutative ring with normalized multiplicative norm and `‖q‖ ≤ 1`, the first two bound `‖(q^m;q)_k-1‖` first by `exp(k‖q‖^m)-1` and then by `(k exp k)‖q‖^m`; the third gives the denominator-free relative bound `‖(q;q)_k[n,k]_q-1‖ ≤ (k exp k)‖q‖^(n-k+1)` for `k≤n`, so it remains meaningful at roots of unity.  Over an arbitrary normed field with `‖q‖<1`, the remaining declarations give fixed and shifted nonasymptotic additive bounds plus relative and additive `IsBigO` estimates with rates `q^(n-k+1)` and `q^(n+1)`.  The shifted limit is the more general `tendsto_gaussianBinomial_add_const_atTop` in `QBinomialTheoremInfinite`; together with `tendsto_gaussianBinomial_atTop`, it closes the four manuscript clauses.  No completeness or `q≠0` assumption is introduced, and the constant is elementary rather than sharp. |
