...
| Positive-real fractional Volterra foundation over real normed spaces | `FabiusFunction.FractionalVolterra` | `fractionalVolterra`, `fractionalVolterra_self`, `fractionalVolterra_congr`, `fractionalVolterra_smul`, `intervalIntegrable_fractionalVolterra_kernel`, `fractionalVolterra_add_input`, `fractionalVolterra_one`, `fractionalVolterra_nat_succ`, `intervalIntegral_fractionalVolterra_betaKernel`; the definition and algebraic endpoint/integer rules use oriented interval integrals, while kernel integrability and input additivity assume `0 < α`, `a ≤ x`, and continuity on `[a,x]`; the shifted beta-kernel value assumes `0 < α`, `0 < β`, and `s < x`; no order-zero law, reversed-endpoint fractional interpretation, order semigroup, general power/constant evaluation, derivative, complex order, or hierarchy-specific specialization is claimed |
