...
> 967 source modules and 12,001 explicit public declarations.  The documentation audit
...
> `GaussianBinomialFixedColumnRate` -1 after ownership transfer).  The live
> union is therefore 967/12,001, again with no documentation gaps.  The separate
...
| Infinite q-binomial and reciprocal Euler theorems | `FabiusFunction.QBinomialTheoremInfinite` | Current 1-definition/27-theorem q-facing inventory: the retained comparison, positivity, uniform Gaussian, Euler-product, q-binomial, and reciprocal-Euler results, plus `norm_finiteQPochhammerIn_pow_sub_one_le_exp`, `isBigO_finiteQPochhammerIn_pow_sub_one`, `tendsto_gaussianBinomial_add_atTop`, `isBigO_gaussianBinomial_sub_inv`, and `isBigO_gaussianBinomial_add_sub_inv`.  The shared finite zero-left identity is imported from `GaussianBinomialAtOne`. |
