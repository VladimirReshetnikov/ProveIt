...
| Coherent polynomial-basis Gram determinants | `FabiusFunction.PolynomialMomentGramDeterminant` | Exhaustive public inventory: two definitions and seven theorems.  `polynomialCoefficientMatrix` and `polynomialMomentGramMatrix` have named entry formulas; a degree-coherent family satisfies the exact congruence `G_n = C_nᵀ H_n C_n`, hence `det G_n = (det C_n)^2 det H_n = (prod_{j<n} coeff (p_j) j)^2 det H_n`.  Over a field, nonzero diagonal coefficients transport the finite Gram--Stieltjes Jacobi subdiagonal to the corresponding Gram-determinant cross-ratio.  The coefficient matrix and its entry formula are semiring-level; the Gram matrix, its entry formula, and the congruence are commutative-semiring-level; determinant identities are commutative-ring-level; and the quotient identity is field-level.  No measure, positivity, orthogonality, root, quadrature, or infinite-fraction claim is added. |
...
| Legendre Gram/Hankel determinant bridge for the up law | `FabiusFunction.FabiusLegendreHankelDeterminant` | Exhaustive public inventory: two definitions and seven theorems.  `upLegendreGramMatrix` and `upLegendreGramDet` identify the finite Legendre-basis Gram entries with integrals, express their determinant as the monomial Hankel determinant times the squared product of Legendre leading coefficients, prove the empty value and strict positivity, compute the consecutive leading-coefficient quotient `(n+1)/(2n+1)`, and transport both the real and exact rational `beta_(n+1)` to the Legendre Gram-determinant cross-ratio.  For `F : BoundedFabius`, the determinant identity, empty value, leading-coefficient quotient, and real Gram cross-ratio are unconditional; the integral-entry, strict-positivity, and rational-cast bridge theorems require `hF : IsFabius F`.  This closes the finite determinant part of the report's Legendre bridge; it does not prove the finite Gaunt/Wigner-symbol entry expansion, rationality by that route, explicit `H_4`/`beta_4` evaluation, roots, Christoffel reconstruction, quadrature, Padé/continued fractions, asymptotics, or an infinite Jacobi product. |
