...
| Executable rational ordinary Legendre polynomials | `FabiusFunction.LegendrePolynomialRational` | Exhaustive public inventory: two definitions, the executable coefficient function `legendrePolynomialCoeffRat` and the noncomputable polynomial wrapper `legendrePolynomialRat`; and six theorems, `legendrePolynomialRat_cast`, `coeff_legendrePolynomialRat`, `natDegree_legendrePolynomialRat`, `coeff_legendrePolynomialRat_self`, `coeff_legendrePolynomialRat_self_ne_zero`, and `coeff_legendrePolynomialRat_self_div_succ`.  The bounded rational sum computes every coefficient, the wrapper casts coefficientwise to the real Rodrigues-normalized `legendrePolynomial`, has exact degree `n` and leading coefficient `2^(-n) * choose (2*n) n`, and consecutive leading coefficients have quotient `(n+1)/(2*n+1)`.  This is exact finite coefficient algebra; it asserts no root, Christoffel, quadrature, orthogonality, or asymptotic result. |
...
| Positive-list partition defect and its sharp first shell | `FabiusFunction.PartitionDefect` | Exhaustive public surface: definitions `pairSum`, `blockPairDefect`, `partitionDefect`; theorems `pairSum_nil`, `pairSum_cons`, `pairSum_one`, `pairSum_add`, `pairSum_congr`, `pairSum_map`, `choose_add_two`, `choose_list_sum_two`, `blockPairDefect_eq_mul_sub_one`, `partitionDefect_eq_pairSum_mul_sub_one`, `partitionDefect_nonneg`, `choose_sum_two_eq_choose_length_add_sum_add_partitionDefect`, `partitionDefect_eq_choose_sum_sub_choose_length_sub_sum`, `length_le_sum_of_pos`, `sum_map_sub_one`, `pairSum_add_eq`, `pairSum_map_add_eq`, `partitionDefect_eq_linear_add_pairwise_excess`, `partitionDefect_lower_bound`, `partitionDefect_fixed_block_bound`, `pairSum_eq_zero_iff_pairwise`, `blockPairDefect_eq_zero_iff`, `partitionDefect_eq_zero_iff`, `sub_one_mul_sub_one_eq_zero_iff`, `partitionDefect_eq_lower_bound_iff`, `partitionDefect_fixed_block_eq_iff`, `add_sub_one_le_mul_of_pos`, `mul_eq_add_sub_one_iff`, `firstShell_le_fixedBlockProduct`, `fixedBlockProduct_eq_firstShell_iff`, `firstShell_le_partitionDefect`, `partitionDefect_eq_firstShell_iff`, and `partitionDefect_twoBlock_firstShell`.  For a positive list of length `k` and sum `m`, the defect is a nonnegative linear term plus pairwise excess, is at least `(k-1)(m-k)`, and under `2 ≤ k < m` is at least `m-2`, with exact zero, fixed-block equality, and first-shell classifiers.  This finite list algebra does not construct set partitions, prove the weighted Bell sum or its asymptotics, or establish a forward/inverse compositional-iterate theorem. |
...
| General-base multiplicity and digit recovery | `FabiusFunction.BaseDigitMultiplicity` | Exhaustive five-theorem surface: `sum_range_weightedScaleMultiplicity_of_log_lt`, `sum_range_weightedScaleMultiplicity_log`, `sum_range_div_pow_log_eq_self_add_tail`, `sub_one_mul_sum_padicValNat_succ_add_digitSum`, and `sum_range_padicValNat_succ_eq_sub_digitSum_div`.  For every integer base `b > 1`, including composite bases and `N = 0`, the sharp logarithmic layer range yields `(b-1) * ∑_(n=1)^N (1+ν_b(n)) + s_b(N) = b*N` and its exact quotient form.  These are finite natural-number identities; identifying analytic zero multiplicities, signs, convergence, or an infinite product remains separate. |
...
| Reciprocal symmetry and the `q = -1` Gaussian zeros | `FabiusFunction.QBinomialReciprocity` | Exhaustive public inventory: zero definitions and four theorems, `gaussianBinomial_reciprocity_units`, `gaussianBinomial_reciprocity`, `gaussianBinomial_neg_one_eq_zero_of_odd_degree`, and `gaussianBinomial_neg_one_even_odd_eq_zero`.  The first proves `q^(k*(n-k)) * [n choose k]_(q⁻¹) = [n choose k]_q` for a unit `q` in every commutative semiring, including the zero-extended region `n < k`; the second is exactly its nonzero-element wrapper over a semifield.  Over every commutative ring, odd degree `k*(n-k)` forces the value at `q = -1` to vanish, and every even-row/odd-column value `[2*a choose 2*b+1]_(-1)` is zero, including above the diagonal.  These are finite algebraic evaluations: no quotient formula, cancellation, domain, characteristic, topology, or analytic palindromicity claim is used. |
| Complete Gaussian and finite q-Pochhammer evaluation at `q = -1` | `FabiusFunction.GaussianBinomialAtNegOne` | Exhaustive five-theorem surface: `gaussianBinomial_neg_one_even_even`, `gaussianBinomial_neg_one_odd_even`, `gaussianBinomial_neg_one_odd_odd`, `finiteQPochhammerIn_neg_one_even`, and `finiteQPochhammerIn_neg_one_odd`.  Together with `gaussianBinomial_neg_one_even_odd_eq_zero`, these give all four Gaussian parity cases and both finite-product parities over every commutative ring, for arbitrary natural indices; Mathlib's Gaussian coefficient supplies total above-row zero extension.  No derivative, simple-root, characteristic-zero jet, or analytic root-of-unity statement is asserted. |
