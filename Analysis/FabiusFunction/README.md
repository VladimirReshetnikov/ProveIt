...
| Laurent, Herglotz, and integrated Stieltjes--Perron layers | `FabiusFunction.StieltjesMomentLaurent`, `FabiusFunction.StieltjesCauchyTransform`, `FabiusFunction.StieltjesHerglotz`, `FabiusFunction.StieltjesInversion`, `FabiusFunction.StieltjesPerron` | Direct named theorems give finite Laurent expansions with exact remainder and convergent all-moment series (real exterior and complex `‖z‖ > 1` forms), conjugation and upper/lower-half-plane sign and quantitative bounds, exact finite-height Poisson/conjugate-Poisson representations, approximate-identity estimates, and integrated interval Stieltjes--Perron inversion.  Pointwise or nontangential Sokhotski--Plemelj boundary values and principal-value Hilbert-transform identities remain open. |
...
| Complete homogeneous evaluations, their finite formal generating series, and the denominator-free geometric principal specialization | `FabiusFunction.CompleteHomogeneous`, `FabiusFunction.CompleteHomogeneousGenerating`, `FabiusFunction.GeometricCompleteHomogeneous` | `completeHomogeneousEval_eq_eval_hsymm`, `completeHomogeneousEval_smul`, `completeHomogeneousEval_option_zero`, `completeHomogeneousEval_fin_succ`, `completeHomogeneousGeneratingSeriesOn`, `completeHomogeneousGeneratingSeriesOn_insert`, `prod_one_sub_mul_completeHomogeneousGeneratingSeriesOn`, `completeHomogeneousGeneratingSeriesOn_eq_invOfUnit_prod`, `completeHomogeneousEval_geometric`, `completeHomogeneousEval_scaled_geometric`, `completeHomogeneousEvalOn_range_pow_eq_gaussianBinomial`; the generating identity is purely formal and holds over commutative rings without distinctness, division, cancellation, or convergence assumptions |
...
| Scaled and classical `q`-binomial transforms of module-valued sequences | `FabiusFunction.QBinomialTransform` | Exhaustive public surface: `scaledGaussianBinomialTransform`, `scaledGaussianBinomialInverseTransform`, `scaledGaussianBinomialInverseTransform_transform`, `scaledGaussianBinomialTransform_inverseTransform`, `scaledGaussianBinomial_inversion`, `gaussianBinomialTransform`, `gaussianBinomialInverseTransform`, `gaussianBinomial_inversion`.  For `[CommRing R] [AddCommGroup M] [Module R M]`, both scaled compositions are pointwise identities and the two triangular sequence relations are equivalent.  These finite algebraic maps require no topology, convergence, division, or invertibility; the unscaled theorem is the classical `q`-binomial inversion. |
| Exact geometric-numerator and q-Gaussian inversion specializations | `FabiusFunction.QBinomialInversionSpecializations` | Exhaustive public surface: `geometricQBinomialWeightNumerator_eq_scaledGaussianBinomialInverseKernel`, `qGaussianResidualCoeff`, `qGaussianReconstructionCoeff`, `qGaussianResidualCoeff_eq`, `qGaussianReconstructionCoeff_eq`, `qGaussianReconstructionCoeff_residualCoeff_delta`, `qGaussianResidualCoeff_reconstructionCoeff_delta`.  The geometric numerator is globally the inverse kernel at base and scale `q`, including indices above the diagonal, without a `k ≤ n` hypothesis.  At Gaussian base `q^2` and scale `-q`, the residual coefficient is `(-q)^(n-k) [n choose k]_(q^2)` and its reconstruction coefficient is `q^((n-k)^2) [n choose k]_(q^2)`; both total `Icc` convolution orders are the Kronecker delta over every commutative ring.  This does not weaken the separate `Field`/finite-node-`InjOn`/`k ≤ n` assumptions used by normalized Lagrange-weight quotient statements. |
...
| Two real Lambert inverses of scaled power--exponential saddles | `FabiusFunction.PowerExponentialLambert`, `FabiusFunction.PowerExponentialLambertCalculus`, `FabiusFunction.PowerExponentialLambertInverse`, `FabiusFunction.PowerExponentialLambertFabius` | `powerExponentialSaddle`, `powerExponentialPeak`, `powerExponentialLambertArgument`, `principalPowerExponentialPhase`, `lowerPowerExponentialPhase`, the three `powerExponentialLambertArgument_mem_*` domain theorems, both branch solve laws and endpoint values, `principalPowerExponentialPhase_mem_Icc`, `lowerPowerExponentialPhase_mem_Ici`, `powerExponentialLambertArgument_strictAntiOn`, `principalPowerExponentialPhase_strictMonoOn`, `lowerPowerExponentialPhase_strictAntiOn`, both branch `HasDerivAt`/`deriv`/derivative-sign and interior-continuity pairs, `powerExponentialLambertArgument_image_Icc`, `powerExponentialLambertArgument_image_Ioc`, `principalPowerExponentialPhase_image_Icc`, `lowerPowerExponentialPhase_image_Ioc`, both branch `LeftInvOn`/`RightInvOn`/`InvOn` packages, `lowerPowerExponentialPhase_rate_one`, `generalizedLambertCoordinate_argument_mem_Ioo`, `generalizedLambertCoordinate_argument_mem_Ico`, `generalizedLambertCoordinate_solves_saddle_of_mem`, and `lowerPowerExponentialPhase_one_one_log_two`; the branch laws assume a nonzero natural power and positive amplitude/rate, include the common peak endpoint, give exact images `[0,m/beta]` and `[m/beta,∞)`, and restrict the derivative formula to the smooth open interval |
...
At compiled source checkpoint `1f9ce6fd9`,
`RvachevMomentAppell.lean` exports five public definitions:
...
`Fabius.rvachevDeconvolvedPolynomial`.  Its sixteen public theorems are
...
families of exact degree, and polynomial deconvolution whose degree does not
increase and whose smoothing by `up` recovers the original polynomial.
...
These modules do not assert the report's minimal-mesh or sharpness claims, an
...
coefficient rationality for the atom rows, a named outer translate-block
`HasSum`, the fixed-scale partial-sum formula, or the later refinement,
projector, and asymptotic layers.
