...
> 933 source modules and 11,695 public declarations.  The documentation audit
...
> Laurent-leading theorem, finite-prefix Appell recovery, complete finite-prefix
> Thue--Morse collapse, the division-free dyadic-boundary identity, and the strengthened
...
> `RvachevLegendreBiorthogonality` 1+1 gives the historical 925/11,619 census.
> Merged upstream work then reached the immediate pre-reciprocity checkpoint
> 930/11,678.  Promoting complex-product differentiability/entireness adds one
> public theorem, and `GeometricUniformMomentReciprocity` 1+5 adds one source
> module and six public declarations, so this reciprocity tranche adds one
> module and seven declarations in total and gives the historical reciprocity
> checkpoint 931/11,685.  The subsequently merged
> `DyadicBoundaryIdentity` 0+2 and `FinitePrefixThueMorseCollapse` 0+8 leaves
> add two modules and ten public declarations, giving the live 933/11,695 census.
> The separate incoming-branch inner/exterior checkpoints
...
| Complete Thue--Morse responses of finite dyadic Appell prefixes | `FabiusFunction.FinitePrefixThueMorseCollapse` | Exhaustive zero-definition/eight-theorem surface: `Appell.sum_thueMorseSign_mul_eval_poly`, `sum_thueMorseSign_mul_uncenteredDyadicPrefixAppellPolynomialRat`, `sum_thueMorseSign_mul_uncenteredDyadicPrefixAppellPolynomialRat_of_lt`, `sum_thueMorseSign_mul_uncenteredDyadicPrefixAppellPolynomialRat_self`, `sum_thueMorseSign_mul_centeredDyadicPrefixAppellPolynomialRat`, `sum_thueMorseSign_mul_centeredDyadicPrefixAppellPolynomialRat_succ`, `sum_thueMorseSign_mul_centeredDyadicPrefixAppellPolynomialRat_of_lt`, and `sum_thueMorseSign_mul_centeredDyadicPrefixAppellPolynomialRat_self`.  The general Appell identity reduces a complete signed block to its Thue--Morse power moments; the rational uncentered and centered specializations include depth zero, with `_of_lt` cancellation and `_self` first-surviving responses.  These are finite coefficient identities with no analytic generating-function or convergence claim.  This census entry records the imported API without changing any source-coverage status. |
| Division-free dyadic-boundary composition for the Rvachev sinc product | `FabiusFunction.DyadicBoundaryIdentity` | Exhaustive zero-definition/two-theorem surface: `prod_complexSinc_prefix_mul_rvachevFourierProduct` and `rvachevFourierProduct_dyadic_boundary`.  The first clears the finite sinc prefix against the rescaled product; the second composes it with the integer-zero factorization to give the entire denominator-cleared dyadic-boundary identity for every `k` and complex displacement, with no nonvanishing hypothesis.  A quotient form still requires the relevant denominator to be nonzero.  This census entry records the imported API without changing any source-coverage status. |
