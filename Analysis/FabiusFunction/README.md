...
> 925 source modules and 11,622 public declarations.  The documentation audit
...
> and the complex geometric-uniform moment-product bridge, the exterior
> reciprocal-germ bridge, the sharp geometric-uniform moment-polynomial degree
...
> complex-product bridge `GeometricUniformComplexMomentProduct` 1+2 and the
> exterior reciprocal-germ bridge
> `GeometricUniformExteriorComplexMomentGerm` 1+2, followed by the sharp
...
> `RvachevLegendreBiorthogonality` 1+1 gives the incoming-target 925/11,619
> checkpoint.  The five local `QBinomialTheoremInfinite` additions overlap in
> exactly two public names with that target.  Keeping those shared declarations
> upstream leaves `QBinomialTheoremInfinite` at 1+27 and
> `GaussianBinomialFixedColumnRate` at 0+8; the other three local declarations
> give the exact-name-deduplicated live union of 925 modules and 11,622 public
> declarations.  The separate incoming-branch inner/exterior checkpoints
...
> / 78 Partial / 15 None / 8 interface rows.  In particular, the 1+4 RatFunc
> leaf makes `thm:qF-moment-polynomial` Exact by assembly; the broader
> `thm:geometric-uniform-mgf` remains Partial.
> Exactly ten retained publication PDFs had source closures changed by this
> merge. All ten now have accepted merged-source artifacts: the 199-page primary
...
> volume. Their current TeX/PDF pairs contain
