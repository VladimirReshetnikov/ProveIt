...
| Actual Bernoulli series for the real Lambert branch gap | `FabiusFunction.LambertWBranchGapBernoulli` | Exhaustive public surface (three theorems): `summable_norm_bernoulli_mul_pow_div_factorial`, `hasSum_bernoulli_mul_pow_div_factorial`, and `principalLambertW_lowerLambertW_eq_bernoulliSeries`.  For real `z` with `|z| < 2π`, the first proves absolute convergence of `∑ B_n z^n/n!`; for `z ≠ 0`, the second gives the actual `HasSum` value `z/(exp z-1)`.  On `x ∈ (-exp(-1),0)` with `Δ=branchGap x<2π`, the final conjunction is exactly `W₀(x)=-∑ B_nΔ^n/n!` and `W₋₁(x)=-Δ-∑ B_nΔ^n/n!`, making only the Lambert Guide label `eq:pair-Bernoulli-general` Exact.  Together with the three finite branch-coordinate modules this is a four-module 4+35 surface, 39 declarations.  The quotient statement deliberately excludes `z=0`, and neither endpoint of the real branch pair is included.  No maximality of the radius `2π`, boundary convergence or divergence, remainder estimate, or higher/convergent Puiseux expansion is asserted. |
