...
> 917 source modules and 11,565 public declarations.  The documentation audit
...
> are included in that clean census.  Its q-series union
> retains
...
| Geometric-uniform moment polynomials and their real-MGF normalization | `FabiusFunction.GeometricUniformMomentPolynomial`, `FabiusFunction.GeometricUniformMomentPolynomialBridge` | The algebra leaf has the exhaustive public surface of one definition and eight theorems: `geometricUniformMomentPolynomial`, `geometricUniformMomentPolynomial_zero`, `geometricUniformMomentPolynomial_succ`, `geometricUniformMomentPolynomial_natDegree_le`, `geometricUniformMomentPolynomial_eval_zero`, `geometricUniformMomentPolynomial_one`, `geometricUniformMomentPolynomial_two`, `geometricUniformMomentPolynomial_three`, and `geometricUniformMomentPolynomial_four`.  Its total family over `ℚ[X]` satisfies the division-free residual finite q-Pochhammer recurrence, `natDegree P_n ≤ n.choose 2`, `P_n(0)=1/(n+1)!`, and the displayed values `P_0` through `P_4`.  The bridge leaf has no public definitions and exactly one public theorem, `geometricUniformMomentPolynomial_eval₂_eq_mgf_taylorCoefficient`: for every real `q` with `|q|<1`, including `q=0` and negative contractions, it proves `P_n(q)=((q;q)_n/(1-q)^n)·(iteratedDeriv n M_q 0/n!)`, where `M_q` is the genuine geometric-uniform MGF.  Thus the analytic normalization in `p7:thm:Pn` is exact on its real probability-law domain; that label remains **Partial** only because its leading-coefficient formula and strict odd-degree drop are absent.  Monograph label `thm:qF-moment-polynomial` remains **Partial** because its source coefficients come from a complex-parameter locally-uniform infinite product not constructed or identified by this real-MGF bridge.  The leading/subleading coefficients and sharp degree in `prop:qF-P-degree-sharp` remain absent, so that label remains **None**. |
