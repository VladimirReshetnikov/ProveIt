...
> 904 source modules and 11,449 public declarations.  The documentation audit
> reports no missing module headers or declaration comments; the exact-dyadic
> inverse and Jacobi two-square leaves are included in that clean census.  Its
> q-series union retains
...
> `QPfaffSaalschutz` 0+3, `TwoPhiOneReversal` 1+6,
> `QChuVandermonde` 0+5, `JacobiTwoSquareCount` 0+4,
> `QuantumMultinomial` 0+5, and
> `GaussianBinomialBounds` 0+6.
> The retained
...
> 183, 130, 257, 301, 41, 88, 377, 158, and 354 A4 pages.  Their current TeX
...
> inverse realizer, total inverse computability theorem, and sharp exact
> endpoint-mass denominator.  These retained PDFs
...
| Terminating two-phi-one reversal and the two q-Chu--Vandermonde sums | `FabiusFunction.TwoPhiOneReversal`, `FabiusFunction.QChuVandermonde` | Exhaustive public surfaces: `TwoPhiOneReversal` has one definition and six theorems (`twoPhiOneFinite`; `choose_two_add_succ_choose_two`, `finiteQPochhammerIn_sub_eq`, `finiteQPochhammerIn_reversal_ne_zero`, `finiteQPochhammerIn_inv_pow_self`, `twoPhiOneFinite_reversal`, and `twoPhiOneFinite_eq_sum_twoPhiOneTerm`), while `QChuVandermonde` has five theorems and no definitions (`two_mul_choose_two`, `mul_sub_one_eq_mul_sub_add`, `finiteQPochhammerIn_div_eq_sum_chu`, `q_chu_vandermonde_first`, and `q_chu_vandermonde_second`).  The finite identities hold over an arbitrary field with their explicit nonzero-parameter and finite-product denominator hypotheses.  The second sum is proved by applying the reversal theorem to the first on the explicit reversal domain; removal of the auxiliary `C ≠ 0` and `(A;q)_n ≠ 0` hypotheses by rational extension is not a separate Lean theorem. |
...
| Terminating two-phi-one reversal and q-Chu--Vandermonde | `FabiusFunction.TwoPhiOneReversal`, `FabiusFunction.QChuVandermonde` | Exhaustive counts are 1+6 and 0+5: one definition and eleven theorems.  They prove the reflected finite-product identity, the terminating reversal, the denominator-cleared first q-Chu sum, both quotient evaluations, and the second evaluation specifically by reversal.  All field, nonzero-parameter, and finite-product denominator hypotheses remain explicit; the broader rational-extension removal of the second theorem's auxiliary `C ≠ 0` and `(A;q)_n ≠ 0` assumptions is not separately formalized. |
