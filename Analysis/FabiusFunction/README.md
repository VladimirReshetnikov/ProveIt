...
> 943 source modules and 11,787 explicit public declarations.  The documentation audit
...
> series/transseries tranche adds nine facade modules carrying 72 explicit
...
> live census is 943/11,787, again with no documentation gaps.  The separate
...
| Abstract differential-block bridge | `FabiusFunction.TransseriesDifferentialBlock` | Exhaustive zero-definition/five-theorem surface: `derivation_pow_t`, `derivation_block`, `exists_block_primitive`, `derivation_block_zero`, and `exists_block_primitive_resonant`.  In any commutative algebra with a derivation satisfying `d(t)=-t²` and `d(L)=t`, these transport the polynomial operator to blocks `t^n p(L)` and supply primitives in the nonresonant and resonant natural-exponent cases.  They do not instantiate the manuscript's full integer-exponent Laurent wrapper. |
