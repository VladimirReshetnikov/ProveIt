...
| Two real Lambert inverses of scaled power--exponential saddles | `FabiusFunction.PowerExponentialLambert`, `FabiusFunction.PowerExponentialLambertCalculus`, `FabiusFunction.PowerExponentialLambertFabius` | `powerExponentialSaddle`, `powerExponentialPeak`, `powerExponentialLambertArgument`, `principalPowerExponentialPhase`, `lowerPowerExponentialPhase`, the three `powerExponentialLambertArgument_mem_*` domain theorems, both branch solve laws and endpoint values, `principalPowerExponentialPhase_mem_Icc`, `lowerPowerExponentialPhase_mem_Ici`, `powerExponentialLambertArgument_strictAntiOn`, `principalPowerExponentialPhase_strictMonoOn`, `lowerPowerExponentialPhase_strictAntiOn`, both branch `HasDerivAt`/`deriv`/derivative-sign and interior-continuity pairs, `lowerPowerExponentialPhase_rate_one`, `generalizedLambertCoordinate_argument_mem_Ioo`, `generalizedLambertCoordinate_solves_saddle_of_mem`, and `lowerPowerExponentialPhase_one_one_log_two`; the branch laws assume a nonzero natural power and positive amplitude/rate, include the common peak endpoint, and the derivative formula is restricted to the smooth open interval |
