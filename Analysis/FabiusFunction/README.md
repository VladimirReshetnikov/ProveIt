...
> 925 source modules and 11,619 public declarations.  The documentation audit
...
> Laurent-leading theorem, finite-prefix Appell
...
> `RvachevLegendreBiorthogonality` 1+1 gives the live 925/11,619 census.  The
> separate incoming-branch inner/exterior checkpoints
> 906/11,461 and 907/11,464 remain historical.
> The final exact-closure tranche consists of `ThueMorseCornerIntegral` 1+4,
...
| Finite Legendre--Rvachev analysis/synthesis biorthogonality (`thm:leg-biorthogonality`) | `FabiusFunction.RvachevLegendreBiorthogonality` | Exhaustive public surface: one definition, `rvachevLegendreAnalysisKernel`, and one theorem, `rvachevLegendreBiorthogonality`.  For every `F : BoundedFabius` satisfying `IsFabius`, every nonzero natural mesh `M`, and natural `l,m` with `l ≤ padicValNat 2 M`, the theorem proves that the normalized finite pairing of the degree-`l` Rvachev deconvolution samples with the degree-`m` Legendre analysis kernel on the literal open block `|k| < 2M` is `if m=l then 1 else 0`.  This makes `thm:leg-biorthogonality` **Exact**.  The broader `thm:leg-Lambda` and `cor:leg-biorthogonal-matrices` remain incomplete: the leaf does not prove the kernel's support, smoothness, parity, origin values, Fourier--Bessel representation, or dyadic rationality, nor does it bundle the stronger matrix projector or reverse spectral closure. |
