...
`FabiusFunction.NegativeLaplace`.  The two infinite sums are treated locally
on compact positive intervals.  The small-scale logarithmic product has a
...
/-- The logarithmic product is continuous on every compact positive interval. -/
theorem continuousOn_negativeLaplaceLog_Icc
    (a b : ℝ) (ha : 0 < a) :
    ContinuousOn negativeLaplaceLog (Icc a b) := by
  change ContinuousOn
    (fun s : ℝ => ∑' n : ℕ, negativeLaplaceTerm s n) _
  apply continuousOn_tsum
  · intro n
    exact (continuousOn_negativeLaplaceTerm n).mono
      (fun _ hs => ha.trans_le hs.1)
  · exact summable_geometric_two' b
  · intro n s hs
    rw [Real.norm_eq_abs]
    refine (abs_negativeLaplaceTerm_le s (ha.trans_le hs.1) n).trans ?_
    gcongr
    exact hs.2
...
/-- The forward tail is continuous on every compact positive interval. -/
theorem continuousOn_negativeLaplaceForwardTail_Icc
    (a b : ℝ) (ha : 0 < a) :
    ContinuousOn negativeLaplaceForwardTail (Icc a b) := by
  let r := Real.exp (-a)
