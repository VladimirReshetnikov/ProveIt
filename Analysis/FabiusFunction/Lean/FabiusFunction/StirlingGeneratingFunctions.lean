...
/-- Coefficients of an exponential generating function carry factorial normalization. -/
...
/-- Explicit coefficients of the formal series `-log(1-X)`. -/
theorem coeff_negLogOneSub (n : ℕ) :
    coeff n (negLogOneSub A) = if n = 0 then 0 else algebraMap ℚ A (1 / n) :=
  coeff_mk _ _

/-- The formal series `-log(1-X)` has zero constant coefficient. -/
