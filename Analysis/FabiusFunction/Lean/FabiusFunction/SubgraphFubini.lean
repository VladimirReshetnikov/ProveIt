...
/-!
# Weighted Fubini transport across a measurable subgraph

This module isolates a reusable Bochner-valued form of the elementary
subgraph argument behind survival integrals and quantile layer-cake formulas.
For a measurable height `Q`, an integrable scalar weight `w`, and an
integrable vector-valued kernel `k`, it exchanges integration over the lower
section `{t | t < Q ω}` with integration over the upper section
`{ω | t < Q ω}`.
