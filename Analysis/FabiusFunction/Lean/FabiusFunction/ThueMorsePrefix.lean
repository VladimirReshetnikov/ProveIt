...
/-- At the first nonvanishing degree, an affine change of variable contributes
only the expected power of its linear coefficient.  In particular, the sum is
independent of the translation `x`. -/
