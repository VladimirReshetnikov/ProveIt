...
branches satisfying an analytic off-domain completion hypothesis at any depth.
That hypothesis deliberately includes branch-domain boundaries; in particular
the class does not automatically contain principal real Lambert `W` at its
nonanalytic branch point.
...
no critical point there.  And `F⁻¹` cannot have one: differentiating
`F⁻¹ ∘ F = id` gives `(F⁻¹)'(F x) · F'(x) = 1`, and `F` is `C^∞`, so `F'(x)`
is a finite real number and the product could not be `1` if the first factor
vanished.  So analyticity of `F⁻¹` anywhere in `(0,1)` would hand back
...
continuous inverse branch, of members, of branches of members, and so on
without limit.  By `Fabius.IsElementaryOrInverse.dense_analyticLocus` every
