...
/-!
# `Φ_a` has exponential type at most `π A_a(1/2)`

Write `A_a(1/2) = ∑_h a_h 2^{-h}` for the weighted dyadic mass.  The
corresponding probabilistic model has support radius `A_a(1/2) / 2`.
...
Paley–Wiener, a compactly supported law on `[-S, S]` has a
characteristic function of exponential type at most `2π S` in the
`e^{-2πixt}` convention — here `π A_a(1/2)`, in the normalization
`sinc(π z / 2^h)` this corpus uses.  That growth bound is proved here,
directly from the product:

`‖Φ_a(z)‖ ≤ exp (π ‖z‖ A_a(1/2))`  for every `z : ℂ`.

So `Φ_a` is entire of order at most `1` and type at most `π A_a(1/2)`, and
the weighted dyadic mass controls it exactly as the support reading predicts.
What is *not* proved is the converse — that the type is not smaller,
and that the law is supported on the interval with radius
`A_a(1/2) / 2` — which does need the
probabilistic side.
...
/-- **`Φ_a` is of exponential type at most `π A_a(1/2)`.**

`‖Φ_a(z)‖ ≤ exp (π ‖z‖ · ∑_h a_h 2^{-h})`.

So `Φ_a` is entire of order at most one, with type controlled by the
weighted dyadic mass `A_a(1/2)`.  Its probabilistic support radius is
half that mass.  The converse support statement is not addressed. -/
