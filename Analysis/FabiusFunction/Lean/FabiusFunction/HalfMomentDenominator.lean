...
theorem rat_den_mul_nat_odd (m : ℕ) (q : ℚ) (hq : Odd q.den) :
    Odd ((m : ℚ) * q).den := by
  exact rat_den_mul_odd (by simp) hq
  ...
  | h n ih =>
      ...
      · obtain ⟨m, hm⟩ := heven
        ...
        · have hmpos : 1 ≤ m := Nat.one_le_iff_ne_zero.mpr hm0
          ...
          have hDodd : Odd D := by
            dsimp [D]
            exact (odd_two_mul_add_one m).mul (two_pow_sub_one_odd (by omega))
          ...
          exact rat_den_div_nat_odd S D hSodd hDodd
      · obtain ⟨m, hm⟩ := hodd
        ...
        have hoddDen :
          ...
          exact (odd_oddDoubleFactorial (m + 1)).mul
            (odd_evenMersenneProduct m)
