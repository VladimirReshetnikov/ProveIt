...
The exact algebraic identity remains valid at the finite lower-Lambert branch
point.
-/
...
theorem fabiusLambertPhase_pos {x : ℝ} (hx : 0 < x)
  ...
  fabiusLambertPhase_pos_of_le hx hsmall.le

/-- The exact saddle prefactor differs from the corrected Lambert main term
only by the exponentially small forward-tail error, including at the finite
lower-Lambert branch point. -/
theorem fabiusLambertSaddleAction_eq_of_le
    {x : ℝ} (hx : 0 < x)
    (hsmall : Real.log 2 * x ≤ Real.exp (-1)) :
  ...
  have hlam : 0 < lam := fabiusLambertPhase_pos_of_le hx hsmall
  have hL : Real.log 2 ≠ 0 := (Real.log_pos (by norm_num)).ne'
  have hrx : r * x = lam :=
    fabiusLambertRadius_mul_argument_of_le hx hsmall
...
theorem fabiusLambertSaddleAction_eq
    ...
    fabiusLambertRadius x * x +
      ...
      fabiusSharpLambertMain x +
        negativeLaplaceTailError (fabiusLambertRadius x) := by
  exact fabiusLambertSaddleAction_eq_of_le hx hsmall.le
