...
/-- The sum of two functions flat relative to a scale is flat. -/
theorem IsFlat.add (hε : IsFlat l φ ε) (hδ : IsFlat l φ δ) : IsFlat l φ (ε + δ) :=
  fun n => (hε n).add (hδ n)

/-- The negation of a flat function is flat. -/
theorem IsFlat.neg (hε : IsFlat l φ ε) : IsFlat l φ (-ε) := fun n => (hε n).neg_left

/-- The difference of two functions flat relative to a scale is flat. -/
...
/-- Membership in `flatSubmodule` is equivalent to flatness. -/
...
/-- The power-scale monomial is the real power `X ^ (-n)`. -/
