...
/-- The sum of two functions flat relative to the same scale is flat. -/
...
/-- The difference of two functions flat relative to the same scale is flat. -/
theorem IsFlat.sub (hε : IsFlat l φ ε) (hδ : IsFlat l φ δ) : IsFlat l φ (ε - δ) :=
  fun n => (hε n).sub (hδ n)

/-- A constant scalar multiple of a flat function is flat. -/
