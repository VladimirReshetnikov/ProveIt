...
/-- The divided logarithm series `log(1+t)/t` has constant coefficient one. -/
...
/-- The series `tOverLog` is the multiplicative inverse of the divided logarithm series. -/
...
/-- Multiplying `log(1+t)` by `t/log(1+t)` gives the series variable. -/
...
/-- The polynomial-coefficient lifts of `log(1+t)` and `t/log(1+t)` multiply to `X`. -/
...
/-- The coefficientwise polynomial derivative annihilates `tOverLogPoly`. -/
...
/-- The `n`th coefficient of `cauchySeries` is `cauchyPoly n / n!`. -/
...
/-- Formal integration sends `a x^k` to `a / (k + 1)`. -/
...
/-- Coefficients of `mapIntPoly f` are obtained by formally integrating those of `f`. -/
theorem coeff_mapIntPoly (f : (Polynomial ℚ)⟦X⟧) (n : ℕ) :
    coeff n (mapIntPoly f) = intPoly (coeff n f) := coeff_mk _ _

/-- Coefficientwise formal integration commutes with multiplication by a rational series. -/
