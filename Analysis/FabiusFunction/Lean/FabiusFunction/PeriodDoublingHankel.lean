...
/-- The left summand of the even/odd interleaving maps to the even index `2i`. -/
@[simp] theorem interleaveEquiv_inl (n : ℕ) (i : Fin n) :
    ((interleaveEquiv n) (Sum.inl i) : ℕ) = 2 * i := rfl

/-- The right summand of the even/odd interleaving maps to the odd index `2i+1`. -/
@[simp] theorem interleaveEquiv_inr (n : ℕ) (i : Fin n) :
    ((interleaveEquiv n) (Sum.inr i) : ℕ) = 2 * i + 1 := rfl

/-- In the odd-size interleaving, the left summand maps to the even index `2i`. -/
@[simp] theorem interleaveOddEquiv_inl (n : ℕ) (i : Fin (n + 1)) :
    ((interleaveOddEquiv n) (Sum.inl i) : ℕ) = 2 * i := rfl

/-- In the odd-size interleaving, the right summand maps to the odd index `2i+1`. -/
...
/-- The period-doubling bit is one at every even index. -/
theorem pdBit_two_mul (k : ℕ) : pdBit (2 * k) = 1 := by
  ...
  rcases Nat.le_one_iff_eq_zero_or_eq_one.mp hb with h | h <;> rw [h] <;>
    decide
/-- At an odd index, the period-doubling bit is one plus the bit at the
half-index, with values in `ZMod 2`. -/
