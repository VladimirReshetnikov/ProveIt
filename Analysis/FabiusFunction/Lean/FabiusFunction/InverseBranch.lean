...
and it is obtained here for free.  In the equation `h (g x) = x`, `h` is a
left inverse of `g`, or equivalently `g` is a right inverse of `h`.  Thus `g`
is a continuous implicit branch of the simplest possible equation:
...
is why the strongest conclusion is density of analytic points rather than
analyticity everywhere; the existence theorem records its single-open-set
specialization.
