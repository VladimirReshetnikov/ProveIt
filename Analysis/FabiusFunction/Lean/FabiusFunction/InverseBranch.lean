...
That closure is what makes the non-elementarity results robust.  It covers
every totalized inverse branch satisfying the constructor's explicit open-set
identity, continuity, and off-domain analyticity hypotheses, as well as
anything assembled from such branches by the elementary operations.  A
function that is analytic at *no* point of a nonempty open interval is
...
and it is obtained here for free.  In its equation `h (g x) = x`, `h` is a
left inverse of `g`, or equivalently `g` is a right inverse of `h`.  The latter
is an implicit branch of the simplest possible equation:
...
`x ↦ x ^ 3` is analytic and invertible, but its signed real inverse
`x ↦ x / |x| * |x| ^ ((3 : ℝ)⁻¹)` is not analytic at `0`.  This is why the
conclusion is "analytic somewhere in `U`" rather than "analytic on `U`", and
it is the same phenomenon that makes density, not analyticity everywhere, the
right invariant throughout.
...
`U` on which the branch identity holds, and explicitly asks that the totalized
function be analytic at every point off `U`.  Constant junk values can often
supply the latter away from a domain boundary, but analyticity at a branch
point still requires separate proof.  Localizing the identity and continuity
hypotheses is nevertheless essential rather than cosmetic.  The theorem
`Fabius.isElementaryOrInverse_of_lambertW` below records a conditional schema
for a Lambert-`W`-shaped inverse branch; it does not construct the standard
principal real branch or discharge those hypotheses for it.
-/
...
"Somewhere" cannot be improved to "everywhere": the signed real cube root
`x ↦ x / |x| * |x| ^ ((3 : ℝ)⁻¹)` is a continuous right inverse of the entire
function `x ↦ x ^ 3` and is not analytic at the origin. -/
...
identity `f (g y) = y` holds, asks that `g` be continuous there, and separately
asks that `g` be analytic at every point off `U`.  The last condition is not
automatic at a branch point, even when a totalization uses constant junk
values farther outside the natural domain.  Localizing the identity is still
essential: `Fabius.isElementaryOrInverse_of_lambertW` records the resulting
conditional schema for any Lambert-`W`-shaped branch meeting all of these
hypotheses, without constructing a standard branch.

`Real.log` already belongs through the elementary-function constructor.  More
generally, the inverse constructor may be iterated at any finite depth and
admits every totalized inverse branch meeting its stated hypotheses.  Yet by
`Fabius.IsElementaryOrInverse.dense_analyticLocus` the resulting functions are
still analytic on a dense set, so the class still cannot reach the Fabius
...
/-- **Conditional Lambert-`W` inverse-branch constructor.**

Any total function satisfying `W y * exp (W y) = y` on an open set, continuous
there and analytic at every point outside it, belongs to the extended class.
This theorem does not construct the principal real branch or prove these
hypotheses for it; in particular, the branch point at `-1/e` requires separate
treatment. -/
