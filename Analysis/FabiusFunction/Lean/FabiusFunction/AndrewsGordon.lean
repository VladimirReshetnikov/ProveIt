...
/-- Every iteration of the Bailey-chain transform on `β` preserves its zeroth coefficient. -/
...
omit [CompleteSpace 𝕜] in
/-- The lowered alpha term along the `t`-fold Bailey chain of the unit pair has norm at most `2`
when `‖q‖ < 1`. -/
