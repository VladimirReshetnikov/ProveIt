...
/-- Exponential generating series preserve pointwise addition of sequences. -/
theorem egfA_add (a b : ℕ → A) : egfA A a + egfA A b = egfA A (a + b) := by
  ext n
  rw [map_add, coeff_egfA, coeff_egfA, coeff_egfA, Pi.add_apply, mul_add]

/-- Multiplication by a constant series scales every coefficient of an EGF. -/
...
/-- The series `invOneAdd` is a right inverse of `1 + X`. -/
...
/-- The constant coefficient of the falling-factorial series is one. -/
...
/-- On constant power series, `Dx` differentiates the polynomial value. -/
...
/-- A power series whose polynomial coefficients are constant is annihilated by `Dx`. -/
...
/-- The coefficientwise derivative of the unit series is zero. -/
theorem Dx_one : Dx (1 : (Polynomial ℚ)⟦X⟧) = 0 := by
  ext n
  rw [coeff_Dx, coeff_one, map_zero]
  split_ifs <;> simp

/-- The coefficientwise polynomial derivative annihilates the series variable `X`. -/
...
/-- The coefficientwise polynomial derivative annihilates `logPoly`. -/
...
/-- The formal derivative of `logPoly` satisfies `(1 + X) logPoly' = 1`. -/
...
/-- The constant coefficient of `fallingPoly` is one. -/
