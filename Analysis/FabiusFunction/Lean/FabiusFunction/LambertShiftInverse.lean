...
/-- Every nonnegative real lies in the closed domain of the principal Lambert branch. -/
theorem neg_exp_neg_one_le_of_nonneg {x : ℝ} (hx : 0 ≤ x) : -Real.exp (-1) ≤ x := by
  have := Real.exp_pos (-1)
  linarith

/-- Every nonnegative real lies strictly to the right of the principal branch point. -/
...
/-- The shifted Lambert map fixes the origin. -/
...
/-- The shifted Lambert map preserves nonnegativity. -/
theorem lambertShift_nonneg {x : ℝ} (hx : 0 ≤ x) : 0 ≤ lambertShift x := by
  unfold lambertShift
  have := principalLambertW_nonneg hx
  linarith

/-- On the nonnegative half-line, the shifted Lambert map is bounded above by `2x`. -/
...
/-- The nonnegative inverse branch `shiftedLambertW` takes nonnegative values. -/
...
/-- Expanded defining equation for the shifted Lambert root. -/
...
/-- The shifted Lambert root is positive at every positive argument. -/
...
/-- The inverse shifted Lambert map preserves nonnegativity. -/
...
/-- The principal-Lambert lower bracket for the inverse shifted map. -/
...
/-- The derivative of `x ↦ x + W(x)` is at least one on the open half-line. -/
...
/-- The derivative of `x ↦ x + W(x)` is at most two on the open half-line. -/
