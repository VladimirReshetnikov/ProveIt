...
/-- Every nonnegative real lies above `-exp (-1)`. -/
theorem neg_exp_neg_one_le_of_nonneg {x : ℝ} (hx : 0 ≤ x) : -Real.exp (-1) ≤ x := by
  have := Real.exp_pos (-1)
  linarith

/-- Every nonnegative real lies strictly above `-exp (-1)`. -/
...
/-- The shifted Lambert map is continuous on the nonnegative reals. -/
...
/-- The shifted Lambert map is nonnegative on nonnegative inputs. -/
theorem lambertShift_nonneg {x : ℝ} (hx : 0 ≤ x) : 0 ≤ lambertShift x := by
  unfold lambertShift
  have := principalLambertW_nonneg hx
  linarith

/-- On nonnegative inputs, the shifted Lambert map is bounded above by `2x`. -/
...
/-- The shifted Lambert function is nonnegative on nonnegative inputs. -/
...
/-- Expanded form of the defining equation for the shifted Lambert function. -/
...
/-- The shifted Lambert function is bounded above by its nonnegative input. -/
...
/-- The shifted Lambert function is positive on positive inputs. -/
...
/-- The inverse of the shifted Lambert map is nonnegative on nonnegative inputs. -/
...
/-- The inverse lies strictly above the lower bracket `z - W(z)` for positive `z`. -/
...
/-- The derivative of the shifted Lambert map is at least one on the positive reals. -/
...
/-- The derivative of the shifted Lambert map is at most two on the positive reals. -/
theorem deriv_lambertShift_le_two {x : ℝ} (hx : 0 < x) : deriv lambertShift x ≤ 2 := by
  rw [(lambertShift_hasDerivAt hx).deriv]
  have := inv_exp_mul_add_one_le_one hx.le
  linarith

/-- The shifted Lambert map is differentiable on the interior of the nonnegative ray. -/
...
/-- Ordered Lipschitz upper bound for the shifted Lambert map on `[0, ∞)`. -/
