...
Everything built on `partitionExpSum` genuinely divides, by the
multiplicity factorials `mⱼ!` (and, for the ruler specialization, by the
part sizes `j`).  So it needs a *commutative* structure in which those
naturals are invertible: `Finset.prod` forces commutativity and `/`
forces inverses, which together mean a field, and invertibility of every
`mⱼ!` in a field means characteristic zero.  `CharZero` alone is not
enough: over `ℤ` the partition sum is not even well defined.  A general
commutative `ℚ`-algebra `A` would do, but only after rewriting each
`x / (m ! : A)` as `algebraMap ℚ A ((m ! : ℚ)⁻¹) * x`; that is a
different development, not a change of typeclass, so it is not done
here.
-/
...
coefficient sequence `x`.  The division by `mⱼ!` is why this lives over
a field rather than over the ring of `eq_of_ruler_recurrence_ring`. -/
...
/-- The partition sum starts at `1`: the empty product. -/
...
Characteristic zero is used, and needed, twice: `(m! : F) ≠ 0` and
`((u : F) + 1) ≠ 0` are what make the removal of a marked part divide
out cleanly. -/
