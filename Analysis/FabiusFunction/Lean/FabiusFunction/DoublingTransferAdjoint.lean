...
theorem integral_mul_comp_doubling' (f g : ℝ → ℝ)
...
/-- The transfer identity for continuous data (the integrability
hypotheses of `integral_mul_comp_doubling'` hold automatically). -/
theorem integral_mul_comp_doubling (f g : ℝ → ℝ) (hf : Continuous f)
    (hg : Continuous g) :
    ((∫ t in (0:ℝ)..(1/2:ℝ), f t * g (2 * t)) +
      ∫ t in (1/2:ℝ)..1, f t * g (2 * t - 1)) =
    ∫ t in (0:ℝ)..1, (f (t / 2) + f ((t + 1) / 2)) / 2 * g t := by
  refine integral_mul_comp_doubling' f g ?_ ?_
