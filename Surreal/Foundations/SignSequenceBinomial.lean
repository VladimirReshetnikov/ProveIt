...
/-!
# Binomial series at actual surreal infinitesimals

For an ordinary real exponent, the formal binomial series at an actual
...
are unique among actual values infinitesimally close to one; the half
power agrees with the independently constructed nonnegative square root.
No global real-power operation or topological convergence is asserted.
-/
...
/-- The ordinary-real binomial series evaluated at an actual infinitesimal. -/
def binomialPower (x : SignSequence.{u}) (hx : IsInfinitesimal x) (r : ℝ) :
    SignSequence.{u} :=
  powerSeriesEvaluation x hx (PowerSeries.binomialSeries ℝ r)

/-- The displayed binomial terms satisfy both strong summability conditions. -/
theorem stronglySummable_binomialTerms (x : SignSequence.{u}) (hx : IsInfinitesimal x)
    (r : ℝ) : StronglySummable (fun n : ℕ => ofReal (Ring.choose r n) * x ^ n) :=
...
theorem binomialPower_eq_strongSum (x : SignSequence.{u}) (hx : IsInfinitesimal x)
    (r : ℝ) : binomialPower x hx r =
      strongSum (fun n : ℕ => ofReal (Ring.choose r n) * x ^ n)
  ...
  simpa only [binomialPower, PowerSeries.binomialSeries_coeff, smul_eq_mul, mul_one] using
    powerSeriesEvaluation_eq_strongSum x hx (PowerSeries.binomialSeries ℝ r)

/-- Every binomial value is finite. -/
theorem isFinite_binomialPower (x : SignSequence.{u}) (hx : IsInfinitesimal x) (r : ℝ) :
    IsFinite (binomialPower x hx r) :=
  isFinite_powerSeriesEvaluation x hx _

@[simp] theorem standardPart_binomialPower (x : SignSequence.{u}) (hx : IsInfinitesimal x)
    (r : ℝ) : standardPart (binomialPower x hx r) = 1 := by
...
theorem isInfinitesimal_binomialPower_sub_one (x : SignSequence.{u})
    (hx : IsInfinitesimal x) (r : ℝ) : IsInfinitesimal (binomialPower x hx r - 1) := by
  have h := (isInfinitesimal_powerSeriesEvaluation_iff x hx
    (PowerSeries.binomialSeries ℝ r - 1)).mpr (by simp)
  simpa only [map_sub, map_one, binomialPower] using h

/-- Exponent addition multiplies the actual binomial values. -/
theorem binomialPower_add (x : SignSequence.{u}) (hx : IsInfinitesimal x) (r s : ℝ) :
...
@[simp] theorem binomialPower_nat (x : SignSequence.{u}) (hx : IsInfinitesimal x) (n : ℕ) :
    binomialPower x hx (n : ℝ) = (1 + x) ^ n := by
  simp only [binomialPower, PowerSeries.binomialSeries_nat, map_pow, map_add, map_one,
    powerSeriesEvaluation_X]

@[simp] theorem binomialPower_zero (x : SignSequence.{u}) (hx : IsInfinitesimal x) :
    binomialPower x hx (0 : ℝ) = 1 := by
  simpa only [Nat.cast_zero, pow_zero] using binomialPower_nat x hx 0

@[simp] theorem binomialPower_one (x : SignSequence.{u}) (hx : IsInfinitesimal x) :
    binomialPower x hx (1 : ℝ) = 1 + x := by
  simpa only [Nat.cast_one, pow_one] using binomialPower_nat x hx 1

@[simp] theorem binomialPower_zero_input (r : ℝ) :
    binomialPower (0 : SignSequence.{u}) infinitesimal_zero r = 1 := by
...
theorem binomialPower_pow (x : SignSequence.{u}) (hx : IsInfinitesimal x) (r : ℝ) (n : ℕ) :
    binomialPower x hx r ^ n = binomialPower x hx ((n : ℝ) * r) := by
  induction n with
  | zero => simp
  | succ n ih =>
    rw [pow_succ, ih, Nat.cast_add, Nat.cast_one, add_mul, one_mul, binomialPower_add]

theorem binomialPower_ne_zero (x : SignSequence.{u}) (hx : IsInfinitesimal x) (r : ℝ) :
...
theorem binomialPower_neg (x : SignSequence.{u}) (hx : IsInfinitesimal x) (r : ℝ) :
...
theorem binomialPower_rat_root (x : SignSequence.{u}) (hx : IsInfinitesimal x)
    (m : ℕ) (hm : m ≠ 0) : binomialPower x hx (1 / (m : ℝ)) ^ m = 1 + x := by
  rw [binomialPower_pow, mul_one_div_cancel (Nat.cast_ne_zero.mpr hm), binomialPower_one]

theorem binomialPower_half_sq (x : SignSequence.{u}) (hx : IsInfinitesimal x) :
    binomialPower x hx (1 / 2 : ℝ) ^ 2 = 1 + x :=
...
/-- Binomial values agree with their evaluation in every small real Hahn workspace. -/
theorem binomialPower_hahnEmbedding (e : Γ →+ SignSequence.{u}) (he : StrictMono e)
    (y : _root_.HahnSeries Γ ℝ) (hy : 0 < y.orderTop) (r : ℝ) :
...
theorem pow_injective_near_one (m : ℕ) (hm : m ≠ 0) :
    Set.InjOn (fun y : SignSequence.{u} => y ^ m) {y | IsInfinitesimal (y - 1)} := by
  intro y hy z hz hpow
  change IsInfinitesimal (y - 1) at hy
  change IsInfinitesimal (z - 1) at hz
  let F : Bool → SignSequence.{u} := fun b => cond b z y
...
theorem binomialPower_rat_root_unique (x : SignSequence.{u}) (hx : IsInfinitesimal x)
    (m : ℕ) (hm : m ≠ 0) (y : SignSequence.{u}) (hy : IsInfinitesimal (y - 1))
    (hpow : y ^ m = 1 + x) : y = binomialPower x hx (1 / (m : ℝ)) :=
...
theorem exists_unique_root_near_one (x : SignSequence.{u}) (hx : IsInfinitesimal x)
    (m : ℕ) (hm : m ≠ 0) :
    ∃! y : SignSequence.{u}, y ^ m = 1 + x ∧ IsInfinitesimal (y - 1) := by
  refine ⟨binomialPower x hx (1 / (m : ℝ)),
...
/-- Every actual value infinitesimally close to one is positive. -/
theorem pos_of_infinitesimal_sub_one (y : SignSequence.{u}) (hy : IsInfinitesimal (y - 1)) :
    0 < y := by
  have h := (isInfinitesimal_iff_forall_real_abs_lt (y - 1)).mp hy 1 _root_.zero_lt_one
  rw [ofReal_one, abs_lt] at h
  linarith [h.1]

/-- The ordinary-real binomial exponent always gives the positive branch near one. -/
theorem binomialPower_pos (x : SignSequence.{u}) (hx : IsInfinitesimal x) (r : ℝ) :
    0 < binomialPower x hx r :=
  pos_of_infinitesimal_sub_one _ (isInfinitesimal_binomialPower_sub_one x hx r)

/-- The binomial half-power agrees with the existing actual nonnegative square root. -/
theorem binomialPower_half_eq_sqrt (x : SignSequence.{u}) (hx : IsInfinitesimal x) :
    binomialPower x hx (1 / 2 : ℝ) = sqrt (1 + x) :=
  (sqrt_eq_of_nonneg_sq (binomialPower_pos x hx _).le (binomialPower_half_sq x hx)).symm
