...
| `GenericQuotationList.v` | `Meta/Qq.lean` | Complete proof-kernel surface: the three list-membership constructors used by quoted proof terms, generalized over arbitrary element types and closed constructively. Lean quotation, expression inspection, and metaprogram monads are intentionally omitted |
...
| `PropositionalLiteral.v` | `Meta/Lit.lean` | Complete mathematical surface: atom-polymorphic literal trees with a distinct explicit biconditional constructor, their primitive connective algebra, structural complexity, and a generalized fold into any target connective algebra. The fold is packaged as a connective homomorphism, commutes with atom maps, and preserves explicit biconditionals; quotation, pretty-printing, and Lean-expression definitional equality are intentionally omitted as elaborator plumbing |
...
| `PropositionalTwoSided.v`, `PropositionalTwoSidedTableau.v` | `Meta/TwoSided.lean` | Complete mathematical surface: generic Type-valued two-sided sequents use the source's bottom-terminated finite succedent disjunction, with inhabited theoremhood wrappers. Proof-relevant context inclusions eliminate decidable formula equality while supporting weakening, rotations, singleton projection, hypothesis discharge, closure, truth/falsity, and every propositional connective rule over the weakest available minimal, intuitionistic, or classical capability. A separate proof-relevant tableau validity layer supplies exact head/tail evidence, membership/subset transport, uppercedent transformation, and all source structural and connective lifts without choice |
| `PropositionalClassicalProverTheorems.v` | `Meta/ClProver.lean` | Complete kernel theorem namespace used by the classical proof-search tactic: provability projection, rotations, weaker-system hypothesis discharge, closures, constants, and all conjunction, disjunction, negation, implication, and biconditional rules. Source-style ordinary list membership is bridged constructively to proof-relevant positional membership, eliminating `DecidableEq`; Lean quotation, elaboration, and tactic control flow are intentionally omitted |
| `PropositionalIntuitionisticProverTheorems.v` | `Meta/IntProver.lean` | Complete 25-theorem kernel namespace used by intuitionistic proof search, each with a Type-valued raw construction and inhabited public wrapper. The source's append-oriented tableau rotation, double-append negation/implication premises, weaker-system hypothesis discharge, closures, constants, and all connective rules are preserved without formula equality or choice; logical hypotheses are weakened to Minimal except where ordinary singleton projection genuinely requires intuitionistic explosion. Lean quotation and tactic control flow are intentionally omitted |
...
| `FoundationPort/Coq/Modal/Kripke.v` | `Modal/Kripke.lean` | Exact declaration-free import facade: all 26 unique imports (including the duplicated upstream `S4Point4` line only once) are re-exported through 29 existing checked modules. It inherits only the documented partial boundaries for `Balloon`, `GrzPoint3`, `KTMk`, and `S4H` |
