...
| `PropositionalEntailmentLukasiewicz.v` | `Propositional/Entailment/Cl/Łukasiewicz.lean` | Generic Type-valued K/S/elimination-of-contraposition capability and its principal classical consequences: implication identity, DNE, DNI, contradiction explosion, and theorem-level inconsistency. The core needs no conjunction, disjunction, truth, bottom abbreviation, formula equality, or full minimal/classical capability; connective reconstruction and presentation aliases remain |
| `PropositionalEntailmentMinimal.v` | `Propositional/Entailment/Minimal/Basic.lean` | Generic Type-valued minimal-entailment capability and a broad constructive derived algebra: implication identity/weakening/composition/exchange/contraction and variance; conjunction/disjunction introduction, elimination, mapping, congruence, symmetry, associativity, and generalized conjunctive/disjunctive cut; biconditional algebra plus theoremhood and unprovability reflection; negation-as-bottom; double-negation introduction, functoriality, and distribution through implication; contraposition; triple-negation collapse; contradiction; implication refutation of an antecedent conjoined with the negated consequent; refutation of an implication from incompatible consequences of one premise; constructive De Morgan laws; truth weakening; currying; finite folds with positional insertion/removal, unique-entry, append, and cons laws; structural finite contexts with an exact normalized-conjunction representation; and proof-relevant predicate contexts with deduction and exact finite-support extraction. Type-valued membership removes `DecidableEq` and choice from context construction, strengthening the source's equality-based finite-set removal theorem to a duplicate-safe positional equivalence; representation-only finite-set conveniences and minor presentation aliases remain |
| `PropositionalEntailmentInt.v` | `Propositional/Entailment/Int/Basic.lean` | Explicit intuitionistic capability combining minimal entailment with ex falso; contradiction-to-arbitrary implication in both orders; proof-relevant context explosion and theorem-level inconsistency; `¬p ∨ q → (p → q)` and disjunctive syllogism; the intuitionistic converse `(¬¬p → ¬¬q) → ¬¬(p → q)` to the minimal stable-implication theorem; duplicate-safe finite-disjunction elimination, append, cons, positional removal, subset, and unique-entry laws; and the exact finite De Morgan equivalence `¬⋁Γ ↔ ⋀(Γ.map ¬)`. Type-valued positional membership removes the source's ambient `DecidableEq`; representation-only finite-set wrappers and downstream context presentation aliases remain |
| `PropositionalEntailmentClassical.v` | `Propositional/Entailment/Cl/Basic.lean` | DNE-based classical algebra with exact and expanded double-negation equivalences, raw and internal inverse contraposition, negation transport across biconditionals, derived ex falso and intuitionistic capabilities, classical negated-conjunction De Morgan equivalence, implication/disjunction equivalence, excluded middle, elimination of contraposition, Dummett and Peirce packages, and the finite mapped-negation De Morgan theorem. The proof-relevant list formulation removes the source's formula equality and finite-set assumptions, while implication refutation, incompatible-consequence refutation, and disjunctive syllogism are factored into their strictly weaker minimal and intuitionistic layers |
