...
| `PropositionalKripkeCanonical.v` | `Propositional/Kripke/{Completeness,AxiomDummett,Hilbert/Int/Basic,Hilbert/LC}.lean` | Enumerated maximal extensions avoiding a target formula, derivably closed prime theories, the canonical preorder/model and truth lemma, completeness for every frame class containing the canonical frame, countermodel extraction, Int soundness/completeness, and Dummett-driven connectedness plus ordinary LC completeness; a compact one-sided construction replaces the source's two-sided saturated tableau |
...
| `StandardModalCompanions.v` | `Modal/ModalCompanion/Standard/{Cl,Int,KC,LC}.lean` | Complete classical companion family; the full Int/S4 and LC/S4.3 modal-companion equivalences via generalized forcing and canonical completeness; reusable S4/S4.2/S4.3 entailment packages; and the forward Int-to-Grz, KC-to-S4.2/Grz.2, and LC-to-Grz.3 translations, including direct convergent-frame WLEM and connected-frame Dummett proofs |
