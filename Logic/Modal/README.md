...
| `PropositionalEntailmentInt.v` | `Propositional/Entailment/Int/Basic.lean` | Explicit intuitionistic capability combining minimal entailment with ex falso; contradiction-to-arbitrary implication in both orders; `¬p ∨ q → (p → q)`; the intuitionistic converse `(¬¬p → ¬¬q) → ¬¬(p → q)` to the minimal stable-implication theorem; and duplicate-safe finite-disjunction elimination, append, cons, and unique-entry laws. Type-valued positional membership removes the source's ambient `DecidableEq`; finite-set wrappers and downstream context conveniences remain |
