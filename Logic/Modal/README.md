...
- the quasinormal logic S as GL plus a single atomic T generator, including
  ...
  inclusion `GL < S`;
- the proof-theoretic core of the quasinormal logic D, generated by GL,
  consistency, and binary Dz: substitution normalization, strict `GL < D`,
  its finite Dz theorem, converse-Four consequence, and a finite
  subformula-indexed Dz conjunction for the continuing semantic reduction;
...
| `QuasiNormalD.v` | `Modal/Logic/D/Basic.lean` | Syntactic tranche: exact P/Dz generators, all substitutions, strict `GL < D`, substitution-free derivation induction, generalized finite Dz theorem, converse-Four, and the finite dz-subformula conjunction; the tail-model reduction, T unprovability, and strict `D < S` remain |
...
The syntactic D tranche similarly normalizes substitution into GL, P, and Dz
cases.  The generalized finite Dz theorem and its converse-Four consequence
use only the existing classical tautology and GL proof interfaces; their
audited assumptions are excluded middle and the standard description
principle inherited from GL completeness.  Its list powerset deliberately
retains duplicate-sensitive presentations and trailing bottom/top connectives
instead of Foundation's deduplicated finite sets; membership is sufficient
for the future semantic filter and every enumerated Dz condition is proved.
The tail-model reduction, T countermodel, and strict `D < S` comparison are
