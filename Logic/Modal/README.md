...
| `PropositionalConsistentTableau.v` | `Propositional/ConsistentTableau.lean` | In progress: duplicate-tolerant two-sided Hilbert consistency, exact positive/negative insertion laws, the shared cut argument showing every consistent pair can decide the next formula, and an atom-polymorphic enumerated Lindenbaum chain with finite-stage extraction and saturated consistent extension. Positional lists and a typed insert split remove the source's finite-set and `DecidableEq` requirements; connective and contextual-provability laws are the next layer |
