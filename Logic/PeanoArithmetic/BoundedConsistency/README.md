...
`CompactPAUniformProvability.v` now also packages the standard orbit by
external induction and proves `compactUniformRestrictedPAConsistencyProvabilityFormula`
in any raw PA model whose carrier is generated by standard numerals.  This
is an explicit diagnostic theorem, not the requested PA theorem: it isolates
the remaining obligation to the nonstandard-carrier proof selector rather
than confusing ordinary numeral induction with represented PA induction.
