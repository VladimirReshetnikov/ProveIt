(**
  Public formula substitution/substitution interchange laws.

  This small final layer runs the represented induction supplied by
  [RawCodedFormulaSubstitutionAtomSubstitutionInterchangeInvariant] and exports the
...
Module PABoundedRawCodedFormulaSubstitutionAtomSubstitutionInterchangeInduction.
...
Theorem raw_codedFormulaSubstitutionAtomSubstitutionInterchangeIndexBelow_all : forall
...
Proof.
  ...
  set (phi :=
    codedFormulaSubstitutionAtomSubstitutionInterchangeIndexBelowTermAt (tVar 0)).
      ...
      raw_formula_sat M (scons M current parameterEnv) phi).
  {
    ...
    - unfold phi.
      apply (proj2
        (raw_sat_codedFormulaSubstitutionAtomSubstitutionInterchangeIndexBelowTermAt_iff
          M (scons M (raw_zero M) parameterEnv) (tVar 0))).
      cbn [raw_term_eval scons].
      intros depth outerReplacement openingDepth replacement
    ...
    - intros current hcurrent.
      ...
      exact (raw_codedFormulaSubstitutionAtomSubstitutionInterchangeIndexBelow_succ
        M hPA current hraw).
  }
...
Theorem raw_codedFormulaSubstitutionAtom_substitution_interchange_at : forall
...
Proof.
  intros M hPA outerReplacement depth openingDepth replacement
    ...
    hleft hright.
  exact (raw_codedFormulaSubstitutionAtomSubstitutionInterchangeIndexBelow_all M hPA
    ...
    houterAtom htopTrace hleft hright).
Qed.

(** Exact contract used three times by the ternary application assembly. *)
Theorem raw_codedFormulaSubstitutionAtom_singleSubstitutionInterchange : forall
    (M : RawPAModel), RawPASatisfies M -> forall outerReplacement,
  RawCodedFormulaOperationSingleSubstitutionInterchange M
    (RawCodedFormulaSubstitutionAtom M) outerReplacement.
Proof.
  intros M hPA outerReplacement depth replacement transformedReplacement
    input transformedInput output transformedOutput
    houterAtom htop hleft hright.
  unfold RawCodedFormulaSingleSubstitution in hleft, hright.
  assert (htopZero :
      RawCodedFormulaOperation M (RawCodedFormulaSubstitutionAtom M)
        outerReplacement
        (raw_succ M (raw_add M depth (raw_zero M)))
        input transformedInput).
  {
    rewrite raw_add_zero_right by exact hPA. exact htop.
  }
  pose proof (raw_codedFormulaSubstitutionAtom_substitution_interchange_at M hPA
    outerReplacement depth (raw_zero M) replacement transformedReplacement
    input transformedInput output transformedOutput
    houterAtom htopZero hleft hright) as hresult.
  rewrite raw_add_zero_right in hresult by exact hPA.
  exact hresult.
Qed.

End PABoundedRawCodedFormulaSubstitutionAtomSubstitutionInterchangeInduction.
