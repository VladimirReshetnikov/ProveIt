...
* **The bounds are numerals of external naturals.**  Two goal-2 lemmas
  used below, `piBounded_of_sigmaBounded_all` and
  `sigmaBounded_of_piBounded_ex`, peel a successor off a `natV` bound and
  are available only in that shape.  The check therefore runs at
  `bnd = natV H m`, `bnd⁺ = natV H (m+1)`.  The internal generalization —
  the same peeling at `vsucc` of an arbitrary element of `omegaV` — is the
  one goal-2-side lemma the nonstandard successor step will additionally
  need.  It is a two-line consequence of `succ_le_lt`, but it lives in
  goal 2 and is not added here.
-/
...
peels a successor. -/
theorem stepGood_of_levelLaws {L : W → W → W → Prop} {m : Nat}
    (hL : LevelLaws H L (natV H m)) {C : W}
    (hC : LevelClosed H L (natV H (m + 1)) C)
    (hind : CodeInduction H (StepGood H L (natV H (m + 1)) C)) :
    ∀ c, IsFormCodeSem H c → StepGood H L (natV H (m + 1)) C c := by
  have hom : mem (natV H (m + 1)) (omegaV H) := natV_mem_omega H (m + 1)
  ...
  · rintro a ⟨hca, iha⟩
    ...
    · rintro ⟨h1, h0⟩
      have k1 : L (allC H a) e (natV H 1) ∨
          (PiBounded H (natV H (m + 1)) (allC H a) ∧
    ...
    · intro hbd hmem
      ...
      · rcases hcase with ⟨-, -, hnp⟩ | ⟨hb0, -⟩
        · rcases hL.decided _ e (isFormCodeSem_all H hca) he
            (quantBounded_of_piBounded H (natV_mem_omega H m)
              (piBounded_of_sigmaBounded_all H hbd)) with k | k
          · exact k
          · exact absurd k hnp
        · exact absurd hb0 (natV_ne H (by decide))
  ...
  · rintro a ⟨hca, iha⟩
    ...
    · rintro ⟨h1, h0⟩
      ...
      have k0 : L (exC H a) e (natV H 0) ∨
          (SigmaBounded H (natV H (m + 1)) (exC H a) ∧
    ...
    · intro hbd hmem
      ...
      · rcases hcase with ⟨hb1, -⟩ | ⟨-, -, hns⟩
        ...
        · rcases hL.decided _ e (isFormCodeSem_ex H hca) he
            (quantBounded_of_sigmaBounded H (natV_mem_omega H m)
              (sigmaBounded_of_piBounded_ex H hbd)) with k | k
          · exact absurd k hns
          · exact k
    · intro hbd hmem
      rcases levelJustified_ex H (hC _ _ _ hmem).2 with hd | hcase
...
single set. -/
theorem exclusivity_step {L : W → W → W → Prop} {m : Nat}
    (hL : LevelLaws H L (natV H m))
    (hind : ∀ C, LevelClosed H L (natV H (m + 1)) C →
      CodeInduction H (StepGood H L (natV H (m + 1)) C))
    {c e : W} (hc : IsFormCodeSem H c) (he : IsUnivEnv H e)
    (h1 : Step H L (natV H (m + 1)) c e (natV H 1)) :
    ¬ Step H L (natV H (m + 1)) c e (natV H 0) := by
  rintro ⟨C0, hC0, mem0⟩
  obtain ⟨C1, hC1, mem1⟩ := h1
  have hCu : LevelClosed H L (natV H (m + 1)) (vcup H C1 C0) :=
    levelClosed_cup H hC1 hC0
  exact ((stepGood_of_levelLaws H hL hCu (hind _ hCu) c hc).2 e he).1
...
theorem piFalse_collapse_step {L : W → W → W → Prop} {m : Nat}
    (hL : LevelLaws H L (natV H m))
    (hind : ∀ C, LevelClosed H L (natV H (m + 1)) C →
      CodeInduction H (StepGood H L (natV H (m + 1)) C))
    {c e : W} (hc : IsFormCodeSem H c) (he : IsUnivEnv H e)
    (hbd : PiBounded H (natV H (m + 1)) c)
    (h : Step H L (natV H (m + 1)) c e (natV H 0)) : L c e (natV H 0) := by
  obtain ⟨C, hC, hm⟩ := h
  exact ((stepGood_of_levelLaws H hL hC (hind _ hC) c hc).2 e he).2.1 hbd hm

/-- **The adjacent-level collapse, Sigma side.**  Goal-2 counterpart:
`sigmaTrue_collapse`. -/
theorem sigmaTrue_collapse_step {L : W → W → W → Prop} {m : Nat}
    (hL : LevelLaws H L (natV H m))
    (hind : ∀ C, LevelClosed H L (natV H (m + 1)) C →
      CodeInduction H (StepGood H L (natV H (m + 1)) C))
    {c e : W} (hc : IsFormCodeSem H c) (he : IsUnivEnv H e)
    (hbd : SigmaBounded H (natV H (m + 1)) c)
    (h : Step H L (natV H (m + 1)) c e (natV H 1)) : L c e (natV H 1) := by
  obtain ⟨C, hC, hm⟩ := h
  exact ((stepGood_of_levelLaws H hL hC (hind _ hC) c hc).2 e he).2.2 hbd hm
