...
still being improved, and Leant keeps the boundary to the engines
narrow on purpose, so each improvement arrives by version bump. The engines speak Haskell types; Leant translates Lean
...
quicker than remembering which library corner it lives in. Take the
...
refutation, it then asks the classical question, and answers with a
term whose spelling shows what was used:
...
middle, one \code{absurd} where the contradiction lands. Less famous
gaps between the two logics come out the same way. Weak excluded
middle, and the G\"odel--Dummett formula:
...
recursion or induction; recursive types contribute constructors only.
It will not analyze dependent structure; atoms are transported, never
opened. Quantifier handling beyond the propositional fragment follows
...
improves; until then, some higher-rank goals answer ``no term found
within bounds'' and nothing stronger. And although Djex can resolve
...
anyway. The fragment is bounded on purpose: inside it, answers are
...
binders named by role, and anonymous-constructor and leading-dot
spellings preferred over fully qualified fallbacks. Finally the
