...
\emph{``show me that no such term exists.''} It is built on two
program-synthesis engines from the vendored Djex library, linked
in-process: \textbf{Djinn}, a complete and terminating proof search
for intuitionistic propositional logic (Dyckhoff's LJT calculus) that
emits programs, and \textbf{Exference}, a ranked heuristic search
under explicit budgets. Both were built for Haskell types; Leant
translates Lean goals into their fragment and their answers back into
Lean.
...
analyze dependent structure (atoms are transported, never opened); its
quantifier handling beyond the propositional fragment is a bounded,
sequent-supplied discipline, so some higher-rank goals answer ``no
term found within bounds'' rather than anything stronger; and
class-constrained goals rely on Lean's elaborator to resolve
instances during verification. The fragment is small on purpose:
inside it, answers are fast, verified, and --- for refutations ---
final.
