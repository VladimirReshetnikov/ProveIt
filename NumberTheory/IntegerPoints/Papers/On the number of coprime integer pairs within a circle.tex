...
$$
\begin{aligned}
\sum_{a<n \leq b} g(n) e(f(n))= & \sum_{\alpha<u \leq \beta} b_{u} \frac{g\left(n_{u}\right)}{\sqrt{\left|f^{\prime \prime}\left(n_{u}\right)\right|}} e\left(f\left(n_{u}\right)-u n_{u}+1 / 8\right) \\
...
\end{aligned}
\right| \\
&\qquad\ll F^{1/2}Q_{1}^{3/2}N^{-1/2}\log^{2}N.
\end{aligned}
\tag{2.20}
\end{equation*}
...
$$
x^{-\varepsilon} T(M, N) \ll x^{1 / 12} N^{11 / 12} M^{1 / 12}+N^{1 / 2} M^{11 / 12},
$$
