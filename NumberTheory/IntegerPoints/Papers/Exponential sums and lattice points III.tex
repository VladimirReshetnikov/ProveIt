...
$$
% ed.: the decimal value of $\Lambda$ was printed as $2.2513\ldots$; $18627/8320 = 2.2388\ldots$ (the fraction is the value of the exponent in (1.22) at $\kappa = 3/10$, $\lambda = 57/140$).
K=\frac{131}{208}=0.6298 \ldots, \quad \Lambda=\frac{18627}{8320}=2.2388 \ldots
$$
...
% ed.: the exponent of $M$ was printed as $(51\kappa-1)/(33\kappa-3)$; the condition $H \leqslant N$ for the choice (3.22) gives $(51\kappa-5)/(33\kappa-3)$, in agreement with (1.43).
H \leqslant \min \left(B_{1} M^{\frac{51 \kappa-5}{33 \kappa-3}} T^{-\frac{20 \kappa-2}{33 \kappa-3}}(\log T)^{\frac{(9 \kappa-1) \lambda}{33 \kappa-3}},\right. \\
...
% ed.: the upper limit of the wider range was printed as $1+\frac{1}{2M}$, which is not a wider range at all; the integral in (1.28) runs over $1-\frac{1}{2M} \leqslant x/M \leqslant (M_2+\frac12)/M \leqslant 2-\frac{1}{2M}$, so the extension is needed on the left only, and we print the symmetric range $[1-\frac{1}{2M},\,2+\frac{1}{2M}]$.
Corollary. If, in addition, $F(x)$ is defined for $1-\frac{1}{2M} \leqslant x \leqslant 2+\frac{1}{2M}$, and has two continuous derivatives in this wider range, then
...
(R-1)^{2} N T<C_{2} M^{3} \leqslant R^{2} N T. \tag{2.2}
\end{gather*}
...
\begin{equation*}
% ed.: the inner sum was printed without absolute value bars, which would bound the real quantity $R(\delta)$ by a complex number; the bars are supplied (Lemma 5.3.2 of [9] bounds the number of points by the Fej\'er-weighted sum of the absolute values of the exponential sums).
R(\delta) \ll \frac{M}{D}+\frac{1}{D}\sum_{h=1}^{D}\left(1-\frac{h}{D}\right) \left|\sum_{m=M}^{2 M-1} e\left(h N F\left(\frac{m}{M}\right)\right)\right|, \tag{3.30}
\end{equation*}
...
\begin{equation*}
N^{45 \kappa-3} \asymp M^{54 \kappa-4} / T^{15 \kappa-1}. \tag{3.37}
\end{equation*}
