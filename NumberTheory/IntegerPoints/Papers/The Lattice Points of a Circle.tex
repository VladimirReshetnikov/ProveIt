...
% ed.: printed text has $u^{-7/(4s)}$; corrected to $u^{-1/(4s)}$
% (the integrand is $u^{1/(2s)}\cdot y^{-3/2}$ with $y=2\pi\sqrt{nx}\,u^{1/(2s)}$,
% so the exponent of $u$ is $1/(2s)-3/(4s)=-1/(4s)$, matching the $u^{1/(4s)}$
% of the main term and of (2.1)).
 \int_0^\infty e^{-u}u^{-1/(4s)}\dd u
 +O\!\left(x^{37/112+\varepsilon}\right) \\
={}&\frac1\pi x^{1/4}\sum_{n=1}^{\infty}\frac{r(n)}{n^{3/4}}a_n
 +O\!\left(x^{37/112+\varepsilon}\right),
\end{align*}
