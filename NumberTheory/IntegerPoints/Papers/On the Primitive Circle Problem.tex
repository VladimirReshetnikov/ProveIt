...
\begin{equation*}
% [Editorial note: the source has plus signs in front of both sums. With
% a_j and b_k as defined below, truncating mu = mu * 1 * mu at U gives, for
% n > U, mu(n) = -sum_{jk=n, j<=U^2} a_j - sum_{jk=n, j>U, k>U} mu(j) b_k
% (checked numerically); the signs are immaterial for the estimates below.]
...
% ed.: the source cites Lemma 2.6 here, but the five terms D_1,...,D_5 below are exactly
% the five terms of Lemma 2.5 (the type I bound) with (X,H,M,N)=(sqrt(xN)/(JK),N,J,K),
% the weights M^{-1/2}N^{-3/4} and J <= M^{28/83}; e.g. (X^6H^{22}M^{22}N^{11})^{1/26}
% gives (x^{498}M^{-712}N^{913})^{1/4316}.  The sum is of type I (a_j, smooth in k),
% so Lemma 2.5 is the lemma actually applied; "Lemma 2.6" corrected to "Lemma 2.5".
We begin by treating $\mathscr R_I(M,N)$. Firstly we remove $1 / \sqrt{k}$ by a partial summation. Then we eliminate the dependence $j k \sim M$ by Lemma 2.6 in [9] at the cost of a factor $\log M$ in the estimate. Finally we use Lemma 2.5 with $(X,H,M,N)=(\sqrt{xN}/(JK),N,J,K)$ to bound the exponential sums obtained. Thus
