...
where $\floor{x}$ is the greatest integer less than or equal to $x$. We divide this region under the hyperbola $yz=x$ into three sections as shown in Figure 2. Hence, if $\{x\}$ denotes the
...
It is natural to ask if lower bounds for $P(x)$ can be obtained. Landau [28] and Hardy [14, pp. 243-267], [16] in papers both published in 1915, obtained the first results in this direction. Employing an identity for the ``smoothed'' sum
...
Improving on Landau's result, but using a completely different method, Hardy [14], [16, pp. 243-267] proved that
...
where $\chi$ is an odd primitive character modulo $q$. We do not state this identity here, but using it, the authors of [8] derived a representation for $\sum_{0 \leq n \leq x}{ }^{\prime} r_{2}(n)$.
