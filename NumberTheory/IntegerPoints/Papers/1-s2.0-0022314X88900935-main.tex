...
so the error term is admissible if $y=M x^{-\theta}$. Our aim (1.2) reduces to showing that
...
where $F_{s}=f * g_{s}$ and $g_{s}(n)=\sigma_{s}\left(n N^{-1}\right)$. Notice that $\sigma_{s}(t)$ is a smooth function in $t$ supported in the interval [1,5]; so that $n \asymp N$. Combining (5.1)-(5.7) we conclude
...
Moreover $(-iy)^{2}\widehat f(y)=\widehat{f^{\prime\prime}}(y)$ and $(-iy)^{3}\widehat f(y)=\widehat{f^{\prime\prime\prime}}(y)$; thus, by the Plancherel theorem
...
\chi\left(\frac{1}{2H\sqrt{\gamma ck}}\right)
\sigma\left(\frac{\sqrt{\gamma ck}}{N}\right)
...
$$
\chi\left(\frac{1}{2 H \sqrt{\gamma c(k-\kappa)}}\right)=\chi\left(\frac{1}{2 H \sqrt{\gamma c k}}\right)+O\left(k^{-1}\right)
$$

and

$$
\sigma\left(\frac{\sqrt{\gamma c(k-\kappa)}}{N}\right)=\sigma\left(\frac{\sqrt{\gamma c k}}{N}\right)+O\left(k^{-1}\right) .
$$
...
\begin{align*}
F(m)= & \operatorname{Im}\sum_{k\asymp K}\sum_{l\asymp L} \frac{1}{\pi l} \chi\left(\left(\frac{x c}{a^{3}}\right)^{1 / 4} \frac{k^{-1 / 2} l}{2 H}\right) \sigma\left(\left(\frac{a^{3}}{x c}\right)^{1 / 4} \frac{k^{1 / 2}}{N}\right) \\
