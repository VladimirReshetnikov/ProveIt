...
% ed.: printed text has no restriction on $\lambda_1,\ldots,\lambda_m$; since the entries of an exponent tuple are non-negative (Section~4), $\widetilde\lambda_j=1-\lambda_j\geqslant0$ requires $\lambda_j\leqslant1$ for $j\leqslant m$ (e.g. $(0\mid2,1,\ldots,1)\in E_n$ trivially, but the printed statement would give the entry $-1$); the hypothesis is added
\textit{Let $m\in[1,n]$, and let $(\lambda_0,\lambda_1,\ldots,\lambda_n)\in E_n$ with $\lambda_j\leqslant1$ $(j=1,\ldots,m)$. Then $(\widetilde\lambda_0,\widetilde\lambda_1,\ldots,\widetilde\lambda_n)\in E_n$, where
\[
...
\]
% ed.: printed text has the denominator $274910$ (three times); but $47728/274910=0.17361\ldots$, whereas $47728/294910=0.16183920\ldots$ agrees with the stated decimal and lies, as it must, between the limit $0.1618$ and the unconditional $35/216=0.162037\ldots$
\[
  \left(\frac{47728}{294910}\,\middle|\,\frac12\right)\in E_1,
  \qquad
  \zeta\left(\tfrac12+it\right)\lll t^{47728/294910},
\]
where $47728/294910=0.16183920$. For comparison, the best published unconditional result is
\[
...
\begin{enumerate}
  ...
  \[
    A_k\leqslant(X_1\cdots X_k)^2F^{1/6-\varepsilon-k}.
  \]}
\end{enumerate}
...
% ed.: both transcriptions read $|\mathscr L|$; the bound $\sqrt{A_k}\,|D_1|\ll\sqrt{A_k}\,|D|/A_k$ shows the intended quantity is $|D|$
\ll{}&|D|A_k^{-1/2}
...
\[
  ...
  \leqslant F^{-\varepsilon_0(K+1)}\ll F^{-n}.
\]
...
\[
\begin{aligned}
% ed.: printed text has $(\alpha_1^{\alpha_1}\cdots\alpha_n^{\alpha_n}/A)^{1/(\alpha_1+\cdots+\alpha_n-1)}$; corrected to $(A\alpha_1^{\alpha_1}\cdots\alpha_n^{\alpha_n})^{-1/(\alpha_1+\cdots+\alpha_n-1)}$: from $m_j=\alpha_jf/x_j$ one gets $f^{1-s}=A\prod\alpha_j^{\alpha_j}\prod m_j^{-\alpha_j}$ with $s=\sum\alpha_j$, and $f_1=(1-s)f$
  B&=(1-\alpha_1-\cdots-\alpha_n)
  \left(A\alpha_1^{\alpha_1}\cdots\alpha_n^{\alpha_n}\right)^{-1/(\alpha_1+\cdots+\alpha_n-1)},\\
...
\]
Then
% ed.: printed text has ``$\leqslant$'' and a single modulus $\bigl|\sum_{(x,h)\in D_1}e(f_1(x,h))\bigr|$ on the right; as printed the inequality is false: for $n=m=1$, $D=[X,2X]$, $f(x)=\theta x$ with $\theta$ chosen so that $\sum_{0<|h|\leqslant q}(X+1-|h|)\cos2\pi\theta h=0$ (possible near $\theta=1/(2q+2)$), one has $S\asymp q$ while the right side is $|D|^2/q$, which is smaller as soon as $q^3\gg X^2$ (checked numerically); the Cauchy--Schwarz proof yields the form below, with an implied constant depending on $m$ (and on $\mathscr D$), and this is the form used throughout Section~5
\[
  |S|^2\equiv\left|\sum_{x\in D}e(f(x))\right|^2
  \ll |D|^2Q^{-1}+|D|Q^{-1}
  \sum_{h}\Bigl|\sum_{x\colon(x,h)\in D_1}e(f_1(x,h))\Bigr|.
\]}
...
% ed.: version-1 OCR reads $\sqrt[19]{\cdots}$ here but $\sqrt[10]{\cdots}$ in its application below; $10$ is kept
...
$C_{30}C_{03}\ne C_{21}C_{12}$,
\[
  \max\{a_1,b_1\}\geqslant\varepsilon_0.
\]
...
\[
% ed.: printed text has $N^{-1}Q_1^{-4}$; corrected to $N^{-1}Q_1^{-7}$, as in every later use of this sum (cases 1)--6))
  (S/N)^8\ll Q_1^{-4}
  +N^{-1}Q_1^{-7}\sum_h
...
% ed.: printed text has $(F_1)^{1/4}$; corrected to $(F_1)^{-1/4}$, the factor carried from the preceding line
&+(F_1)^{-1/4}\gamma_2^{1/2}
 (\gamma_1+Q_1^{-1})(\gamma_1+Q_1^{-2})
 \ll Q_1^{-4}.
\end{aligned}
...
\noindent\textbf{6)}
\[
% ed.: printed text has $Q_1^3$; corrected to $Q_1^9$, the threshold $\sqrt{N^3F^{-2}Q_1^9}$ used in cases 1)--5) and below
  a_3\gg\sqrt{N^3F^{-2}Q_1^9},
...
If
\[
% ed.: printed text has $Q_1^7$; corrected to $Q_1^{31}$ so that this is the complement of the preceding case $a\gg\sqrt[4]{F^2Q_1^{31}N^{-5}Y^{-4}}$
  a\ll\sqrt[4]{F^2Q_1^{31}N^{-5}Y^{-4}},
\]
...
\begin{gathered}
 ...
 \Phi_{0,2}(f)=-f_{x^2},\\
% ed.: printed text names this determinant $M(f)$ and then uses $H(f)$ (once $M(f)$) in the recursion; since $H$ is the Hessian determinant of Section~2, $H(f)$ is used throughout
...
\begin{aligned}
 ...
 &&(j=2,\ldots,k_2+4),\\
% ed.: printed text has $\delta_2^{3k_1k_2-2k_1}$ on the left and $\delta_2^{3k_1k_2-3k_1}$ on the right; the two exponents must agree and the right-hand one is kept
 d_j\delta_2^{3k_1k_2-3k_1}
 &\leqslant\lvert\varphi_j(\theta)\rvert
...
or
\[
% ed.: printed text reads $Q_2^{-1/2(3k_2+1)}$; resolved as $Q_2^{-(3k_2+1)/2}$, which is exactly $a\ll Q_2^{-1}$ for $a$ defined below
  \min\{d_2,\ldots,d_{k_2+4}\}\ll Q_2^{-(3k_2+1)/2},
\]
...
\[
  \left(\frac SN\right)^{K_1}
% ed.: printed text has $Q_1^{K_1/2}$ and $\ll Q_2^{-K_1/2}$; corrected to $Q_1^{-K_1/2}$ in both places (the target bound throughout is $(S/N)^{K_1}\ll Q_1^{-K_1/2}$)
...
\[
  X/Y\geqslant Q_2+NQ_2^{K_2}N_0^{-k_2-1/3},
\]
...
% ed.: printed text reads $Q_3^{2j-1}$; corrected to $Q_3^{2^{j-1}}$ (the $j$th application of Lemma~2 uses $Q_j=Q^{2^{j-1}}$, cf.\ Section~4 and case~II)
...
\begin{aligned}
  ...
  &+N_1^{K_2-1}
% ed.: printed text has $Y^{-k_1}$; corrected to $Y^{-k_2}$ (only $k_2$ differences in $v$ are taken here)
    \left(F_1^{k_2+3}N^{-2}Y^{-k_2}Q_3^{1-K_2}d_{k_2+4}^{-1}\right)^{1/2}\\
...
Suppose that
\[
  \min\{c_{4,1},c_{4,2}\}\geqslant d_{k_2+5}/Q_2
\]
and
\[
  \min\left\{
...
% ed.: printed text has $F_1^{2k_2+8}$; corrected to $F_1^{2k_2+6}$ to match the symmetric $a_5$-term
  +\sqrt[4]{F_1^{2k_2+6}N^{-6}Q_2^{-3}Q^{2-K_2/2}Y^{4-2k_2}b_5^{-2}}
  \Bigg\}\\
  \ll{}&R_0+
...
% ed.: printed text has $\varphi_{0,3},\varphi_{2,3},\varphi_{1,3}$; corrected to $\psi_{0,3},\psi_{2,3},\psi_{1,3}$, following the pattern of $D_5$ and $D_6$
\[
\begin{aligned}
  a_6\delta_2^{3k_2+1}\delta_{1,3}
  &\leqslant|\psi_{0,3}(\theta)|
   \leqslant2a_6\delta_2^{3k_2+1}\delta_{1,3},\\
  b_6\delta_2^{3k_2+1}\delta_{1,3}
  &\leqslant|\psi_{2,3}(\theta)|
   \leqslant2b_6\delta_2^{3k_2+1}\delta_{1,3},\\
  c_6\delta_2^{3k_2+1}\delta_{1,3}
  &\leqslant|\psi_{1,3}(\theta)|
...
% ed.: printed text has $XF_1^{-2}$; corrected to $XF_1^{-1}$ to match the symmetric term $YF_1^{-1}/\sqrt{c_{6,1}}$
      +\frac{XF_1^{-1}}{\sqrt{c_{6,1}}}
  \Bigg\}.
\end{aligned}
\]
...
\[
  |D_2|\leqslant\sqrt{a_{31}}\,N.
\]
This can be done by proving that $H(g(x))\asymp P(\theta_1,\theta_2,\theta_3)$, where $\theta_j=h_j/x_j$ and $P$ is a homogeneous polynomial of degree~$3$ such that the system
\[
...
\]
and $\left|\sum_{h,x}\right|$ is small compared with the estimate we obtain for the first sum. Fix $h=(h_1,h_2,h_3)$, and put
\[
...
\paragraph{1. $a_1\geqslant\varepsilon_0$, $a_2\geqslant\varepsilon_0$.}
...
\paragraph{2. $a_1\leqslant\varepsilon_0$.}
In this case
\[
  |D_2|\leqslant(\Delta+a_2)N,
  \qquad \max\left\{a_2,\max_{i<j}a_{i,j}\right\}\geqslant\varepsilon_0,
\]
and
\[
  \max_{i<j}\bigl(a_{i,j}(a_{i+2}+a_{j+2})\bigr)\geqslant\varepsilon_0.
\]
If $a_2\geqslant F_1^{\varepsilon-1/3}$, we use Lemma~1 with $k=1$ three times, changing the order of summation when necessary:
\[
\begin{aligned}
  S_1
  ...
  &\lll NF_1^{-1/2}+X_iX_jF_1^{-1}
      \sum_{m_1,m_2}\left|\sum_xe(f_2(m,x))\right|\\
  &\lll NF_1^{-1/2}
...
\paragraph{3. $a_1\geqslant\varepsilon_0$, $a_2\leqslant\varepsilon_0$.}
Here
\[
  |D_2|\leqslant(\Delta+\sqrt{a_2})N
\]
(see the comments to Theorem~2), and also
\[
  |D_2|\leqslant\left(\Delta+\max_{i<j}a_{i,j}\right)N,
  \qquad \max_{3\leqslant i\leqslant5}a_i\geqslant\varepsilon_0.
\]
If, say,
\[
  a_{2,3}=\max_{i<j}a_{i,j}\geqslant F_1^{\varepsilon-1/3},
\]
then $a_2\geqslant\varepsilon_0$ (or $a_3\geqslant\varepsilon_0$) and, applying Lemma~1 twice with $k=1$, we get
\[
\begin{aligned}
  S_1
...
\end{aligned}
\]
% ed.: printed text reads ``Lemma 1 with $m=1$''; Lemma 1 is parametrised by $k$
also, using Lemma~1 with $k=1$, we get
\[
  S_1\lll(\Delta+a_{2,3})X_1X_2X_4\cdots X_n\sqrt{F_1}
  +\frac{N}{\sqrt{F_1}}.
\]
If $a_2\geqslant F_1^{\varepsilon_0-1/3}$, then we apply Lemma~1 twice with $k=1$ and once with $k=n-2$:
\[
\begin{aligned}
  S_1
  &\lll X_2F_1^{-1/2}\sum_m
  ...
  &\lll X_2X_3F_1^{-1/2}\sum_{m_1,m_2}
    \left|\sum_{x_1,x_4,\ldots,x_n}e(f_2(m,x))\right|\\
...
\]
(Note that the condition $F\geqslant X_1$ is not essential; we can remove it and substitute~(1) with
\[
  S\ll N_mF^{-m/2}
  S\left(F;F/X_1+1,\ldots,F/X_m+1,X_{m+1},\ldots,X_n\right).
\]
One can however do it better: if $F\leqslant\varepsilon_0X_1$, then
\[
\begin{aligned}
  S&=\left|\sum_{x_2,\ldots,x_n}\sum_{x_1=X}^{X'}e(f(x))\right|\\
  &\ll\left|\sum_{x_2,\ldots,x_n}\int_X^{X'}e(f(x))\,dx_1\right|
...
% ed.: the middle line of this display is garbled in the OCR (a duplicate of the first line); reconstructed by inserting the exponent-tuple bound for the inner sum
\[
\begin{aligned}
  S&\ll N_mF^{-m/2}
  S(F;F/X_1,\ldots,F/X_m,X_{m+1},\ldots,X_n)\\
...
While Theorems~1--3 can be applied to a wide class of functions in $\mathscr F$, they may not give a ``good'' result in some cases. The general strategy in using m.e.p. is the following. Knowing the estimate of an exponential sum one wants to obtain, one needs to apply the algorithm (based on (1) and (4)) described in the Introduction to determine whether it is possible and (if it is possible) a sequence of operations $A$ and $B$ one needs to apply in order to obtain the result. The last step is to prove the result using the ideas contained in the proof of Theorem~1. For a specific function it can be done much more simply than in the general case.
