...
$f=O(g)$ means $f \ll g$.\\
$f \asymp g$ means $c_{1} f<g<c_{2} f$ with some positive, unspecified constants $c_{1}, c_{2}$.
...
These estimates and (1.6) yield the assertion of Theorem 2 provided $x \geqslant 1$. If $x<1$ the assertion is trivial. \(\blacksquare\)
...
$$
% ed.: the original prints the inner condition as $K\leqslant k-q,\ k+q<L$, which for $q<0$ lets the index $k-q$ exceed $L$; both indices must lie in $[K,L)$, i.e. $K\leqslant k-|q|$ and $k+|q|<L$.
...
Proof. It is similar to that of Lemma 5.10 in [6]. \(\blacksquare\)
...
\[
% ed.: the original prints the factor $\frac1{2\pi}$; the correct Perron normalisation is $\frac1{2\pi i}$, since $\frac1{2\pi i}\int_{-L}^{L}\bigl((\nu N/l)^{it}-(N/l)^{it}\bigr)t^{-1}\,dt=\frac1\pi\bigl(\mathrm{Si}(L\log(\nu N/l))-\mathrm{Si}(L\log(N/l))\bigr)\to1$ for $N<l<\nu N$ (and $\to0$ otherwise); with $\frac1{2\pi}$ the right-hand side would be $i\sum a_n+O(\log(2+L))$, impossible e.g. for $a_n=1$, $N=L$, $\nu=2$.
\sum_{N<n<\nu N}a_n
 =\frac1{2\pi i}\int_{-L}^{L}
...
\sum_{M<m<\mu M}m^{-1/2}
 e\left(\alpha^{-1}m^\alpha M^{-\alpha}X\right)
% ed.: the original prints $\frac{\gamma}{2\pi}$; with the corrected Lemma~6 the factor is $\frac{\gamma}{2\pi i}$, $\gamma$ being the constant of Lemma~5 (as printed, the two $\gamma$'s would differ by the factor $i$).
={}&\frac{\gamma}{2\pi i}\int_{-L}^{L}
...
\end{aligned}
\]
% ed.: the original claims here that the last but one term $x^{1/2}H^2M^{1/2}N^3$ is majorized by the first term $x^{1/2}H^2N^4$ and omits it; this holds only when $N\ge M^{1/2}$, which is not assumed. The term is in fact retained in the statement of Theorem~6 (it is the summand $N^{1/4}M^{1/8}$), so nothing is omitted.
Taking the fourth root, all four terms appear in the statement of
Theorem~6. This completes the proof of Theorem~6. \(\blacksquare\)
...
\]
% ed.: as printed, $\widehat f(h/(mn))\ll h^{-2}$ is not uniform in $m,n$: two integrations by parts give $\widehat f(t)\ll t^{-2}(yx^{-2\varepsilon})^{-1}$, hence a tail $O(MNx^{-\varepsilon})$ rather than $O(yx^{-\varepsilon})$. Integrating by parts $j$ times gives $\widehat f(t)\ll_j |t|^{-j}(yx^{-2\varepsilon})^{1-j}$ and a tail $O_j(MNx^{-(j-1)\varepsilon})$; since $MN\le x$ may be assumed (otherwise $|r_{mn}|$ is dominated by the trivial term), $j=\lceil 3/\varepsilon\rceil$ suffices and the constant still depends on $\varepsilon$ alone.
For $|h|>H=MNy^{-1}x^{3\varepsilon}$ we have
$\widehat f(h/(mn))\ll_j (mn/h)^{j}(yx^{-2\varepsilon})^{1-j}$ for every $j\ge2$, so the terms with $|h|>H$ contribute
