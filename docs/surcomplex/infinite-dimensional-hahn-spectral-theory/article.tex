...
set-sized ordered value groups; Part IV assumes $\Gamma$ divisible.
...
\emph{Part IV} returns to $H((t^\Gamma))$ with $\Gamma$ divisible. For an
...
that unused restriction, as explained in \cref{ihs:hh:sec:field}. Part IV
keeps its source's divisibility hypothesis.
...
question (\cref{ihs:hh:thm:spectrum,ihs:fr:thm:main}, and the Volterra
example \cref{ihs:dz:sec:volterra}).
...
$e$ maps a halo onto exactly the infinitesimals with valuation in $e\Gamma$,
...
Throughout Part IV, $H$ is infinite-dimensional and separable, $\Gamma$ is
...
Part II. Each part has its own inner product,
...
(\cref{ihs:dz:lem:straighten}). Part IV assumes it, to diagonalize finite
Hermitian clusters inside the field (\cref{ihs:fr:lem:hermitian}). In Part I all field-valued roots
...
Part~IV. Neither does it speak about bijectivity on a vector space: the
statement $\sigma^{\mathrm{alg}}_\Gamma=\sigma^{\mathrm{adj}}_\Gamma$ is
proved in this report for constant normal operators
(\cref{ihs:hh:thm:spectrum}), for the Volterra operator
(\cref{ihs:dz:sec:volterra}), and for the operators of Part~IV
(\cref{ihs:fr:thm:main}), and nowhere else.
...
normality and rank one are \emph{not} assumed; only
...
invertibility and a ramification test. An isolated spectral point can acquire
...
$P_{\mathrm{inv}}=SS^{\mathrm D}$. These are complementary idempotents
commuting with $S$, $S^{\mathrm D}P_{\mathrm{nil}}=P_{\mathrm{nil}}S^{\mathrm D}=0$,
...
Thus $P_{\mathrm{nil}}$ is the nilpotent part and $P_{\mathrm{inv}}$ the
invertible part. If $\nu\ge1$ is least, then $S^{\nu-1}P_{\mathrm{nil}}\ne0$.
...
$S^{\mathrm D}P_{\mathrm{nil}}=0$. It follows that
...
To see the mechanism, take $S=T-a1$ with Drazin inverse $S^{\mathrm D}$. The
...
coefficient is a Drazin inverse. No accumulation point of the ordinary
...
operator of \cref{ihs:dz:sec:blocks} at monomial displacements). The second is point-spectrum rigidity,
...
On the part for $a$ write $T=a1+N_a$ with $N_a$ of nilpotency index $\nu_a$.
Then $p(a1+N_a)-b1=N_a^{e_a}w_a(N_a)$ with $w_a(0)\ne0$; the factor
$w_a(N_a)$ is invertible and commutes with $N_a$, so the $k$th power vanishes
exactly when $N_a^{e_ak}$ does, giving index $\lceil\nu_a/e_a\rceil$. The
index of a finite direct sum is the maximum. Apply \cref{ihs:dz:thm:scale}
for the valuation.
\end{proof}
...
injective; its range consists of functions with an absolutely continuous
representative vanishing at zero, so the constant function $1$ is not in its
range. Repeated integration gives
...
\end{equation}
There is a direct support witness at $\eps=t^\eta$. The constant vector $y$
whose $n$th block is $2^{-n}e_n$ lies in $H$. A putative solution $x$ of
$(\mathcal N-t^\eta I)x=y$ must, in the $n$th block, equal
$x^{(n)}=-\sum_{j=0}^{n-1}t^{-(j+1)\eta}(2^{-n}J_n)^j2^{-n}e_n$, whose
coefficient at $-n\eta$ is nonzero. The global support of $x$ would contain
...
on infinitesimals consists precisely of those whose valuation has both
coordinates even; inverse descent and the scalar-power criterion answer
...
There are two independent mechanisms. A constant pencil admits an
infinitesimally shifted Hahn inverse exactly when its singular corner is
nilpotent of bounded finite index, and that condition survives all
...
\emph{Standing hypotheses for Part IV.} $H$ is an ordinary
...
of this part only; it is used to make $\K$ algebraically closed and $\Fld$
real closed, so that finite Hermitian clusters diagonalize inside the field
(\cref{ihs:fr:lem:hermitian}). This report has not examined which Part~IV
statements survive without it. ``Spectrum'' in this part means both Part~I
...
Let $X,Y$ be ordinary complex Banach spaces and $f$ holomorphic near
$x_0\in X$. For a positive-support Hahn perturbation
...
above, and \cref{ihs:fr:lem:lift} proves idempotence and commutation. Each
...
and all nonconstant terms have support in the stated monoid.
\end{proof}

This is the classical two-projection calculation of \cite{SimonProjections},
...
The complementary inverse also proves the kernel isomorphism.
\end{proof}
...
\end{equation}
The determinant commutes with coefficient conjugation and adjunction and
agrees with a finite matrix determinant on a finite-dimensional ordinary
summand. If $v(C)>0$,
...
The adjoint and finite-matrix assertions follow by the same comparison. For
...
For $C_0\in\SH$ there is a finite-rank ordinary operator $F=\iota\kappa$,
with $\iota:\C^m\to H$ and $\kappa:H\to\C^m$, such that $I+C_0+F$ is
invertible.
\end{lemma}
\begin{proof}
...
invertible. Any finite-rank operator factors through $\C^m$.
\end{proof}
...
permits); combined with $M_F^{-1}$ this proves the first implication, and an
...
cokernels they are well defined and inverse because $N_FR_F=R_FL_F$ and
$\kappa N_F=L_F\kappa$. Multiplication by the invertible $M_F$ does not change
...
boundary term. Finite-section determinants remove that term and converge
coefficientwise to \eqref{ihs:fr:eq:detsecond}; products of the labelled
branches of the infinite operator do not. There is no conflict with ordinary
...
entire Taylor series. Finite coupling $z$ corresponds to spectral parameter
...
germs. Every coefficient is divisible by $z^m$, so $\mathcal G=z^m\mathcal H$
...
\cref{ihs:fr:lem:nonvanishing}. The zero function fails too: a nonreal
...
Let the first two ordinary coordinates share the eigenvalue $c=1$, let the
...
\item \emph{Constant spectra.} The source describes the bounded-coefficient
 ...
 operator (Part~III).
\item \emph{Divisibility.} The source's divisibility hypothesis is its own
 and is kept. Unlike the Part III source, it makes no claim about Part~I's
 hypotheses on $\Gamma$.
...
\item Parts III and IV are kept as separate parts, not merged into Part~I,
 because Part III's spectrum is relative to an arbitrary algebra and Part IV
 assumes divisibility and a separable infinite-dimensional $H$, which Part~I
 does not.
...
that image into the necessary and sufficient criterion. Only the Banach
corollaries (\cref{ihs:dz:cor:normal,ihs:dz:cor:algebraic,ihs:dz:prop:polemap})
use ordinary analytic spectral theory, and only the surreal specialization
...
finite-dimensional linear algebra; the accumulation obstruction uses only a
...
elements of $\Ahh$ or of compact residues with nontrivial kernel; anything
without divisibility; a positive spectral measure or general
...
their respective proofs; Part IV assumes divisibility, and nothing here
transfers its conclusions to nondivisible groups. The row- and
...
(\cref{ihs:hh:thm:spectrum,ihs:fr:thm:main}, \cref{ihs:dz:sec:volterra}).
