...
spaces and operator algebras are different. Both parts now allow arbitrary
nonzero set-sized ordered value groups.
...
different vector spaces and in different operator algebras. Quoted side
by side without their categories, their conclusions about accumulation read
as a contradiction. They are not one. Both parts allow arbitrary nonzero
...
that unused restriction, as explained in \cref{ihs:hh:sec:field}.
...
The inner-product conventions also differ: Part I is linear in the first
variable, whereas Part II is conjugate-linear in the first. To translate a
scalar pairing, conjugate its value (equivalently, exchange its arguments);
do not copy a variational functional without translating its linearity.
Positivity and the adjoint identity are unchanged by this conversion.
...
products and their operator algebras --- is
...
coefficient-space requirements, and each proves its own positivity statement
from scratch:
...
Neither construction needs divisibility. In Part I all field-valued roots
...
is transferred between them. Both now allow nondivisible $\Gamma$ by
their respective proofs; the row- and column-finiteness of Part~II is
not available in Part~I. The two
