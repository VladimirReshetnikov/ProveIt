...
\addlinespace
Hypothesis on $\Gamma$ &
nonzero, set-sized, ordered abelian and \textbf{divisible} as a standing
convention; the norm itself exists without divisibility, and
...
The stated standing hypotheses are retained separately below; no hypothesis
should be carried across the boundary. In particular, divisibility remains
a Part I convention, but is not necessary for its norm construction.
General positive scalars and squared norms have different root requirements:
...
abelian group. This convention guarantees square roots of all positive
scalars in $\Fld=\R((t^\Gamma))$; the norm construction below needs only
roots of squared norms and works without divisibility.
\Cref{ihs:sec:topology} temporarily strengthens the hypothesis to
$\Gamma\subseteq\R$. ``Spectrum'' in this part means failure
...
the positive root is unique in any ordered field. Divisibility supplies
$\delta/2$ for arbitrary positive scalars. For the norm, however, the
leading exponent is already even, so no group extension is needed.
The next proposition proves this directly. We retain the Part I standing
hypotheses for the subsequent results; they are not a claim that divisibility
is necessary to define the norm.
