...
in~(2). The report is filed under \texttt{surcomplex} although the archive
...
\cref{epd:thm:topology}(a) is the classical rank-one Cantor repeller, which
the manuscript cites from Benedetto's notes \cite[Example~4.39]{Benedetto}.
...
\begin{equation}\label{epd:eq:omega-error}
 ...
 \qquad(n\in\N).
\end{equation}
All conclusions of the non-order-unit case of \cref{epd:thm:topology} hold in
...
(Placement note: the collection's Lean library proves the positive-support
lemma imported here from \cite{Neumann}, over an arbitrary ordered abelian
group, including finiteness of fixed-sum word fibers across all word
lengths. These are the declarations \texttt{neumann\_positive} and
\texttt{finite\_words\_of\_sum\_eq} in \texttt{Surreal/HahnSeries/Neumann.lean},
recorded in \texttt{docs/FORMALIZATION.md}. No other statement of this
report is formalized, and no Lean declaration is attached to any
\texttt{epd:} label.)
...
are only regrouped here. Items P1--P3 were added at placement. Nothing in
...
\item The report is an unrefereed research draft. No result has an
independent peer-review or machine-check certificate (title page,
Appendix~\ref{epd:app:ledger}).
...
\item There is no Lean formalization of the article. No Lean file is
included, no repository build was run, and compatibility with concrete
Lean interfaces was not tested (\cref{epd:subsec:formalization}).
...
complete absence of overlap across every file is made. The repository was
neither rebuilt nor modified, and repository descriptions are provenance,
...
\item No general surreal canonicalizer, floating-point simulation, or
proof-assistant certificate is used (\texttt{data/verification.json}).
