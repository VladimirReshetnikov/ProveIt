...
\hypersetup{colorlinks=true,linkcolor=ink,citecolor=ink,urlcolor=ink,
 ...
 pdfsubject={Exact symbolic fibers, cofinality, Hahn fields, and polynomial iteration}}
\pagestyle{fancy}
\fancyhf{}
...
\begin{titlepage}
\vspace*{12mm}
{\color{ink}\large RESEARCH ARTICLE\par}
\vspace{10mm}
{\color{ink}\huge\bfseries Expanding Polynomial Dynamics\par
 over Surreal and Surcomplex Fields\par}
\vspace{7mm}
{\Large Exact symbolic fibers and the cofinality obstruction\par}
\vspace{14mm}
...
$F=q^{-1}P$, where $v(q)=\kappa>0$ and the degree-$d$ reduction of
$P\in\OO[X]$ splits into distinct roots. We construct, by finite-parameter
...
with open infinitesimal fibers. The distinction is cofinality of the
expansion scale, not valuation rank alone. In the noncofinal case the
...
\end{abstract}
\vfill
\smallbox{Status and scope}{This is an unrefereed research draft with written
proofs and exact finite regression tests. The fiber, compact-subsystem,
...
is claimed solved. No Lean formalization of this article is claimed.}
\end{titlepage}
\pagenumbering{roman}
...
arbitrary infinitesimal coefficient perturbations of any polynomial with
simple split reduction. The main formula is
\begin{equation}\label{eq:intro-fibers}
...
cofinality/topology dichotomy; finiteness of compact forward-invariant
subsets in the noncofinal case; and the scalar-extension and
support-controlled deformation consequences.
...
Throughout Sections~\ref{sec:workspace}--\ref{sec:general}, let
\[
...
\begin{definition}\label{def:orderunit}
A positive $u\in\Gamma$ is an \emph{order unit}, or a
\emph{cofinal scale}, if for every $\gamma\in\Gamma$ there is an
...
If $\kappa$ is cofinal, no finite valuation can exceed all $n\kappa$.
...
\begin{example}\label{ex:ranktwo}
In $\Gamma=\Q\oplus_{\mathrm{lex}}\Q$, the scale $(0,1)$ is not
cofinal, since $(1,0)>n(0,1)$ for every $n$. In the very same group,
$(1,0)$ is cofinal. Consequently ``rank greater than one'' cannot replace
``the chosen scale is not cofinal'' in any theorem below.
\end{example}
...
\begin{lemma}[Finite-parameter Hahn evaluation]\label{lem:evaluation}
...
\begin{remark}\label{rem:no-limits}
...
degree $d$ and unit leading coefficient. Primes below always denote
...
\begin{lemma}[Universal implicit branch]\label{lem:formalbranch}
For each $i$, there exists a unique
$\Psi_i\in k[[E_0,\ldots,E_d,V]]$ such that
\begin{equation}\label{eq:formalbranch}
 \Psi_i(0,0)=c_i,
 \qquad
 P_0(\Psi_i)+\sum_{j=0}^dE_j\Psi_i^j=V.
\end{equation}
Moreover, for any complete formal power-series ring containing the
parameters and any $U,W$ of positive total order,
\begin{equation}\label{eq:formal-difference}
...
having positive valuation. \Cref{lem:evaluation} proves existence and
...
class gives \eqref{eq:forward-metric}. It also proves that two lifts of
the same $w$ coincide.
\end{proof}
...
has zero constant term. By \cref{lem:formalbranch},
...
\begin{theorem}[Universal centers and common support]\label{thm:centers}
There is a formal series $X_s\in R$ for every $s\in\Sig$ with
\begin{equation}\label{eq:formalcenter}
 X_s-X_s^{[N]}\in Q^N R,
 \qquad \mathcal P(X_s)=QX_{\sh s},
 \qquad X_s(0)=c_{s_0}.
\end{equation}
After evaluation $Q=q$, $E_j=e_j$, set $x_s=\ev(X_s)$. Then
\begin{equation}\label{eq:shift-center}
...
\end{theorem}
\begin{proof}
Repeated use of \eqref{eq:q-contraction} shows that for $L\geq N$,
\[
 X_s^{[L]}-X_s^{[N]}\in Q^N R.
\]
The ring $R$ is $Q$-adically complete: it may equally be written
$k[[E_0,\ldots,E_d]][[Q]]$. Hence these expressions determine $X_s$.
...
\eqref{eq:formalcenter}. The other two assertions follow directly from
...
\cref{lem:evaluation}. This proves \eqref{eq:shift-center}. Reduction
at each shifted word gives the itinerary. The same evaluation lemma
gives \eqref{eq:common-support}, with no dependence of $M$ on $s$.
\end{proof}
...
\begin{corollary}[Constant coefficients]\label{cor:constant-center}
...
\begin{remark}[A limit that is not taken]\label{rem:nestedlimit}
...
$N\kappa$: apply \eqref{eq:branch-metric} $N$ times to the tail $x_{\sh^N s}$
and $0$, whose residues differ. When $\kappa$ is noncofinal, these
...
\label{sec:fibers}
\begin{theorem}[Exact fibers and exact separation]\label{thm:fibers}
Under \eqref{eq:polynomial}, the itinerary map is onto and
\begin{equation}\label{eq:fiber-main}
 \pi^{-1}(s)=x_s+\Ik,
 \qquad
 \BI(F)=\coprod_{s\in\Sig}(x_s+\Ik).
\end{equation}
For $h\in\Ik\setminus\{0\}$ and every $n\geq0$,
\begin{equation}\label{eq:fiber-growth}
 v\bigl(F^n(x_s+h)-x_{\sh^n s}\bigr)=v(h)-n\kappa.
\end{equation}
If $s\ne r$ first differ at index $m$, then for every
$x\in\pi^{-1}(s)$ and $y\in\pi^{-1}(r)$,
\begin{equation}\label{eq:first-mismatch}
 v(x-y)=m\kappa.
\end{equation}
...
residue at time $n$, and \eqref{eq:forward-metric} proves the next
instance of \eqref{eq:fiber-growth}. Induction proves both that the
...
\eqref{eq:branch-metric} $N$ times gives
\[
 v(x-x_s)\geq N\kappa.
\]
This holds for every $N$, and \cref{lem:ideal} implies $x-x_s\in\Ik$.
...
common $m$ branches gives \eqref{eq:first-mismatch}. In particular the
cosets in \eqref{eq:fiber-main} are disjoint. The last assertion follows
from \cref{lem:ideal}.
\end{proof}

\begin{corollary}[Exact finite cylinders]\label{cor:cylinders}
...
\begin{corollary}[The action on the fibers]\label{cor:fiber-bijection}
...
\end{corollary}
\begin{proof}
Forward inclusion is \eqref{eq:fiber-growth}. Given a point in the
...
then \eqref{eq:fiber-growth} would imply simultaneously
\[
...
never becomes zero, by \eqref{eq:fiber-growth}, unless it was zero
initially. This proves the first assertion; the converse follows from
\eqref{eq:shift-center}.
...
distinct roots of \eqref{eq:preperiodic-polynomial}. That polynomial
...
\begin{equation}\label{eq:escape-step}
...
\begin{proposition}[Exact multipliers]\label{prop:multiplier}
For $x\in\BI(F)$ and $n\geq1$,
\[
 v\bigl((F^n)'(x)\bigr)=-n\kappa.
\]
At a root of \eqref{eq:preperiodic-polynomial}, its derivative has
valuation $-(m+p)\kappa$.
\end{proposition}
\begin{proof}
At every integral orbit point, $P'$ has residue $P_0'(c_{s_j})\ne0$.
Thus $v(F')=-\kappa$, and the chain rule gives the first formula.
At a root $x$ of \eqref{eq:preperiodic-polynomial}, factor its derivative
as
\[
...
\begin{corollary}[Periodic counts and zeta series]\label{cor:zeta}
...
\begin{theorem}[Cofinality dichotomy]\label{thm:topology}
...
\item In the noncofinal case the family $(F^n)_{n\geq0}$ is uniformly
equicontinuous on $\BI(F)$ in the native valuation uniformity.
\end{enumerate}
\end{theorem}
\begin{proof}
When $\kappa$ is cofinal, \cref{cor:cylinders} and cofinality identify
...
$m\kappa$, arbitrarily large by cofinality; the difference after $m$
...
\cref{lem:ideal}, so their union is open. Also
...
By \cref{cor:cylinders}, every cylinder preimage is open, so $\pi$ is
...
$\Ik$, so they have the same itinerary by \cref{thm:fibers}. Repeated
...
\begin{remark}[Why this is not a contradiction]\label{rem:expansion}
...
\begin{definition}\label{def:scaletop}
...
\begin{proposition}\label{prop:scalequotient}
...
In the noncofinal case this is not the native topological quotient.
\end{proposition}
\begin{proof}
By \cref{cor:cylinders}, $E_N$ is exactly equality of the first $N$
symbols. By \cref{thm:fibers}, the common equivalence relation of all
...
\begin{theorem}[Compact-subsystem rigidity]\label{thm:compact}
...
in \cref{prop:scalequotient} has entropy $\log d$.
\end{theorem}
\begin{proof}
...
ball argument used in \cref{thm:topology} makes it closed and discrete
...
\begin{theorem}[Scalar-extension theorem]\label{thm:extension}
The universal centers over $K$ and over $L$ are the same elements of
$K\subseteq L$. Moreover,
\begin{align}
 \BI(F;L)&=\coprod_{s\in\Sig}\bigl(x_s+\Ik(L)\bigr),\label{eq:extension-fibers}\\
 \BI(F;L)\cap K&=\BI(F;K),\qquad
 \Ik(L)\cap K=\Ik(K).\label{eq:intersection-extension}
\end{align}
Every periodic and preperiodic point over \emph{any field extension}
of $K$ already belongs to $K$. If $\kappa$ is cofinal in $\Gamma$ but
not in $\Delta$, the following additional conclusions hold:
...
\cref{thm:fibers} in $L$ to get \eqref{eq:extension-fibers}.
...
$F^{m+p}-F^m$ for some $m,p$. By \cref{thm:periodic}, that polynomial
...
Assume now that cofinality is lost. The first conclusion is
\cref{thm:topology} in the two fields. For the second, write a new point
as $x_s+h$ with $h\in\Ik(L)\setminus\{0\}$. Because $\kappa$ is
cofinal in $\Gamma$, $v(h)$ exceeds every element of $\Gamma$.
...
\begin{example}[A minimal extension that changes the dynamics]\label{ex:minimal-extension}
Take
\[
 K=\C((q)),\qquad
...
\begin{theorem}[Uniform deformation estimate]\label{thm:deformation}
If $D\ne0$ and $\theta=\min_{d_j\ne0}v(d_j)>0$, then for every
itinerary $s$,
\begin{equation}\label{eq:deformation-bound}
...
\begin{equation}\label{eq:leading-response}
 \coeff{\theta}{(\widetilde x_s-x_s)}
 =-\frac{D_\theta(c_{s_0})}{P_0'(c_{s_0})}.
\end{equation}
If the numerator is nonzero, equality holds in
\eqref{eq:deformation-bound}. If it vanishes, cancellation is allowed;
...
\cref{lem:evaluation} at $Q=q$, $E=e$, $D=d$. The two evaluations
...
\begin{equation}\label{eq:variation-eq}
...
coefficient in \eqref{eq:variation-eq} proves
\eqref{eq:leading-response}.
\end{proof}

\begin{remark}[Why a formal ideal argument is necessary]
...
\Cref{thm:deformation} therefore gives
...
noncofinal workspace. The correction formula shows exactly where the
...
\begin{theorem}[Exact bounded-orbit locus]\label{thm:bounded}
For $F=q^{-1}P$ as in \eqref{eq:polynomial},
\begin{equation}\label{eq:bounded-formula}
 \BV(F)=\BI(F)\ \cup\
 \{x\in K:\kappa+\alpha(x)\text{ is not an order unit in }\Gamma\}.
\end{equation}
Consequently:
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=2em]
\item If $\kappa$ is cofinal, $\BV(F)=\BI(F)$.
\item If $\kappa$ is noncofinal, then
...
Once negative, valuations obey \eqref{eq:escape-step}. After a further
$n$ steps they are
\begin{equation}\label{eq:outside-recurrence}
...
Indeed, if that element is cofinal, powers of $d$ dominate any finite
...
between $\kappa$ and $2\kappa$, so it is cofinal exactly when $\kappa$
is. Otherwise $a+\kappa=\alpha(x)+\kappa$. Points which never leave
are already in $\BI(F)$. This proves \eqref{eq:bounded-formula}.
...
cofinal, some $\eta$ exceeds all $n\gamma$, while $\eta\leq Nu$ for
some $N$. Applying the bound to $Nn\gamma$ gives $n\gamma<u$.
Conversely that last condition prevents cofinality. Any convex subgroup
containing an order unit is all of $\Gamma$, proving maximality.

\begin{corollary}\label{cor:coarsebounded}
If $\Gamma$ has an order unit but $\kappa$ is noncofinal, then
\[
...
is noncofinal exactly when $\alpha\in H$. The formula follows from
\cref{thm:bounded}. Also $v(q)\in H$, so $q$ is a unit for the
...
\begin{example}[Escaping the disk without an unbounded orbit]
...
\eqref{eq:outside-recurrence} gives increasingly negative second
coordinates. All valuations remain greater than $(-1,0)$, so the
orbit is still valuation-bounded. This is not a failure of the
recurrence; it is the distinction between a noncofinal direction and
...
\begin{proposition}[An orbit and difference budget]\label{prop:budget}
...
\]
which is \eqref{eq:differencebudget}. Cancellation can only increase
these valuations.
\end{proof}
...
$D_A$ for which the coefficient bounds in \cref{prop:budget} hold.
\end{theorem}
\begin{proof}
Assume (i). For the $A$ in \cref{prop:budget}, choose
$\eta>rA$ for every ordinary integer $r$. Then
\eqref{eq:orbitbudget} bounds every orbit starting in $D_A$ below by
...
$v(x-y)>\delta$, then \eqref{eq:differencebudget} gives
...
\begin{definition}[A deliberately named affine notion]
...
\begin{corollary}\label{cor:empty-affine}
If $\Gamma$ has no order unit, then $J_{\mathrm{aff},v}(f)=\varnothing$
for every polynomial $f$.
\end{corollary}
\begin{proof}
This is exactly statement (iii) of \cref{thm:general}.
\end{proof}
...
metric, or on a Berkovich space, not by this unspecified affine
...
\begin{example}[No uncountable-cofinality hypothesis is needed]
...
unit. All conclusions of \cref{thm:general} hold. This is a finite
...
additive group generated by its supports. All infinite sums used to
construct the centers have the common set-sized support certificate
\eqref{eq:common-support}.
...
such enlargement by \cref{thm:extension}.

\subsection{An explicit quadratic theorem inside the actual field}
Set
\begin{equation}\label{eq:surreal-F}
...
\begin{equation}\label{eq:surreal-fibers}
...
\begin{equation}\label{eq:omega-error}
 v\bigl(F^n(x_s+h)-x_{\sh^n s}\bigr)=\omega-n
 \qquad(n\in\N).
\end{equation}
All conclusions of the noncofinal case of \cref{thm:topology} hold in
the full native topology, with compactness assertions understood for
set-sized subsets.
\end{theorem}
\begin{proof}
...
natural valuation, so the left side of \eqref{eq:surreal-fibers} is
precisely the integral nonescaping condition.

The set $I$ is nonzero because $\omega>n$ for all $n$. The fiber
formula and \eqref{eq:omega-error} follow from
\cref{thm:fibers}. All coefficients of the binary centers are real.
Any preperiodic surcomplex point is a root of a polynomial which already
splits into distinct real linear factors by \cref{thm:periodic}, so it
is one of those real centers. For topology, the proofs in
\cref{thm:topology,thm:compact} only use balls, a fixed dominating
valuation, and set-indexed compactness; they apply verbatim.
\end{proof}

\begin{remark}[Fine topology versus a workspace topology]
The natural valuation topology of the full surreal field is its order
(fine) topology: monomial scales are cofinal among positive radii,
...
\begin{equation}\label{eq:binary-recursion}
 a_0(s)=s_0,\qquad
...
\end{equation}
In particular,
\begin{equation}\label{eq:first-coeffs}
...
\begin{align}\label{eq:fixed-plus}
...
iterates has residue $+1$. Formula \eqref{eq:omega-error} records its
...
The quantitative estimates in \cref{prop:budget} therefore apply to
...
\begin{corollary}\label{cor:surreal-all}
...
\cref{prop:budget} and then a larger scale $\eta$ dominating all $nA$.
The proof of \cref{thm:general} uses only this particular pair of
...
leading coefficient. The same cofinal comparison of monomial balls
...
bound for one sequence: \eqref{eq:differencebudget} provides a single
...
The principal counts are shown in \cref{tab:checks}. Every count is
computed by the shipped program; the full record is in
\texttt{data/verification.json}.

\begin{table}[ht]
\centering\small
\caption{Exact finite checks in the included run.}\label{tab:checks}
...
second uses \eqref{eq:binary-recursion} directly, constructing the tail
...
fixed point; the exact point was proved in \eqref{eq:fixed-plus}.
...
\subsection{A proof-assistant route, not a formalization claim}
...
infinite summation. This isolates the support-heavy part of a future
Lean development. The repository inventory reports relevant Hahn and
finite-algebra infrastructure \cite{Repo}; compatibility of concrete
Lean interfaces has not been tested here. No Lean file is included,
no repository build was run for this article, and the written proofs
have not been checked by a proof assistant.

\section{Limitations and further research questions}
\label{sec:questions}
The hypotheses in \eqref{eq:polynomial} are doing real work. Simple
...
\begin{question}[Nonuniform branch scales]
...
\begin{question}[A topology retaining both levels of information]
...
\begin{question}[Dynamics inside a single fiber]
...
flat deformations of \cref{cor:flatdeform}?
\end{question}
...
only when the accumulated scale is cofinal. Otherwise it determines
...
Positive-support lemma & Classical imported result; cited to Neumann.\\
Finite-parameter evaluation & Explicit consequence with coefficient-finiteness proof in \cref{lem:evaluation}.\\
Universal centers & Formal implicit recursion, formal $Q$-adic completeness, and strong evaluation; proved in \cref{thm:centers}.\\
Exact fibers & Centers plus the exact divided-difference metric; proposed new arbitrary-rank formulation, \cref{thm:fibers}.\\
Preperiodicity splitting & Exact fibers and finite polynomial degree; proved in \cref{thm:periodic}. Counting formulas are classical consequences.\\
Native topology and compact rigidity & Exact first mismatch, open error ideal, and elementary compactness; proposed new package, \cref{thm:topology,thm:compact}.\\
Scalar extension and transcendence & Universal support, preperiodicity splitting, and one-term valuation dominance; \cref{thm:extension}.\\
Deformation and flat invisibility & Formal perturbation ideal before evaluation; \cref{thm:deformation,cor:flatdeform}.\\
Bounded orbit locus & Exact exterior valuation recurrence and order-unit criterion; \cref{thm:bounded}.\\
General polynomial equicontinuity & Finite-degree growth budget; structural consequence of no order unit, \cref{thm:general}.\\
Surreal realization & Classical Conway normal form plus the set-sized workspace proofs; \cref{thm:surreal,cor:surreal-all}.\\
\bottomrule
\end{tabularx}
\end{table}

All newly proved assertions are mathematical claims of this draft.
None has an independent peer-review or machine-check certificate.
Finite tests support only the finite identities and regression cases
listed in \cref{sec:verification}; they do not establish the general
...
$\kappa$ cofinal in $\Gamma$\\compact Cantor topology};
\node[box,right=of ground] (large) {$\BI(F;L)=\coprod_s(x_s+\Ik)$\\
$\kappa$ noncofinal in $\Delta$\\open fibers; centers discrete};
