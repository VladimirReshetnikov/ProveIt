...
a previously unknown conditioning mechanism. For a single cluster of $n\ge2$
...
When every weight has valuation zero and all $n\ge2$ nodes occupy one equally
spaced valuation cluster, $d_{ij}=s>0$ for $i\ne j$, one has
\[
 d_i=(n-1)s,\qquad \delta_i=s,\qquad \Theta=(2n-1)s.
\]
For a single node there is no pairwise separation: the empty-sum and
empty-maximum conventions give $d_1=\delta_1=0$, so
$\Theta=\varpi_1$, or $\Theta=0$ for a weight of valuation zero.
