...
notation follows the polynomial-algebra report. Otherwise the statements,
proofs, limitations and priority caveats are those of the manuscript, and
...
classical identity is the only thing this report shares with the section on
...
neither square classes nor leading principal minors. The two share no
theorem, neither uses the other, and they are cross-referenced, not merged.
...
$\sum_jc_jx_j^n$ with distinct $x_j$ exactly when a monic polynomial with
distinct roots annihilates it under the shift) and a finite Gaussian
...
moment data, under positivity. This report starts from an $n$-node
...
matrix of the Newton power sums of $P$; this classical identity is the only
point of contact with the Hankel section of the collection's spectral-theory
...
\eqref{prony:eq:Bsystem} and \eqref{prony:eq:graphedges}. Hence $\Id+B'$ and $\Id+B$
...
It follows that $v(b_i)\ge\rho_i-E_i>\delta_i$.

Construct the $n$ roots by Lemma~\ref{prony:lem:localroots} and the weights by
