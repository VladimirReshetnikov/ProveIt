...
The same criterion controls every pairable polynomial observable. When all
...
from the proposed new finite-certificate theorem package.
\end{abstract}
\end{minipage}
\vfill
\begin{minipage}{0.94\textwidth}\small
\textbf{Research status.} Prepared with AI assistance; not refereed and not
proof-assistant verified. This manuscript develops candidate-new theorems,
not a claimed solution of a named longstanding conjecture. The precise
statements were not located in the materials inspected. The repository audit
is pinned to \Commit; compressed archive contents were not exhaustively
accessible. \Cref{sec:provenance} gives the exact novelty and verification
boundaries. Finite computations are checks, not substitutes for the proofs.
\end{minipage}
\end{titlepage}
...
\begin{align}
 \lambda_a+\sum_i\alpha_i^a p_i&>0 &&(1\le a\le s),\label{eq:introvertex}\\
 c_{ij}-p_i-p_j&>0 &&(C_{ij}\ne0).\label{eq:introedge}
\end{align}
...
infinite regrouping. An isotropic example in \Cref{sec:cancellation} has a
...
\label[corollary]{cor:substitution}
...
\Cref{lem:neumann}, the union of these supports is well ordered. A fixed
...
\label[proposition]{prop:embedding}
...
\end{proposition}
\begin{proof}
An increasing well-ordered set of $\gamma$ becomes a decreasing well-ordered
...
For $\Gamma=\Q$, take $\iota(q)=q$. Then $t$ becomes $\omega^{-1}$.
...
\]
The matrix can be singular. Except in \Cref{sec:positive}, it need not be real
...
\eqref{eq:W}. When a loop is not allowed, its count is zero and its factor is
...
cancellation in passing from pairings to \eqref{eq:atom}.
...
\begin{equation}
 ...
 \label{eq:weight}
\end{equation}
The coefficient in \eqref{eq:atom} is nonzero in characteristic zero, so
\begin{equation}
...
\label[proposition]{prop:hilbert}
...
the characterization. Finiteness follows from \Cref{lem:dickson}. If an
...
sector. \Cref{prop:module} alone does not establish summability, because
...
a minimum. All pairwise inequalities \eqref{eq:fm} say that the former is
...
Consequently \Cref{thm:alternative} gives the exact bridge between valuation
...
For the finite interaction and covariance data of \Cref{sec:wick}, the
...
\eqref{eq:leading}, its atom has leading exponent $nL(q)$. If $L(q)<0$,
...
Each case violates \Cref{def:strong}.
...
\Cref{prop:hilbert}. Its weight is a sum of strictly positive elements.

\emph{(ii) is equivalent to (iv).} Apply \Cref{thm:alternative} to
\eqref{eq:wickM}. Clearing denominators identifies every possible
...
\Cref{cor:substitution} applies to the entire family in the last expression.
...
\label[corollary]{cor:obstruction}
...
\label[corollary]{cor:robustness}
...
monomial transformation \eqref{eq:scaleddata}, without requiring that the
...
is $1$, although nonempty observable sectors may contain nontrivial finite
or infinite sums.
...
\label[lemma]{lem:indecompconnected}
...
\label[lemma]{lem:rewire}
Let $G$ be a connected colored Wick graph in which every vertex has valence
...
fails, \Cref{cor:obstruction} gives an $h\in\cH$ with $L(h)\le0$. The
integer incidence equation guarantees that at least one pairing realizes
$h$, by \Cref{lem:wickcount}. It is connected by
\Cref{lem:indecompconnected}. Apply \Cref{lem:rewire}. The amplitudes of the
...
\label[proposition]{prop:connectedlog}
Under the equivalent conditions of \Cref{thm:main}, write $Z=1+z$. Then
...
$z$ and makes \eqref{eq:log} valid by \Cref{cor:substitution}.
...
$u_a,w_e$ of \eqref{eq:scaleddata}. \Cref{cor:substitution} respects the
formal identity, and the vacuum scaling factors cancel. Taking the formal
logarithm, evaluated by \eqref{eq:log}, gives the claimed equality.
\end{proof}

\begin{example}[Why linear vertices are different]
\label[example]{ex:linear}
...
that the valence hypothesis in \Cref{thm:connected} cannot simply be deleted.
...
\begin{equation}
 ...
 \label{eq:explicitpositivebalance}
\end{equation}
whenever \eqref{eq:deltacriterion} holds.
\end{theorem}
\begin{proof}
...
nonzero member of $S$ with weight $2\delta_a$. \Cref{thm:main} requires that
weight to be positive.

For sufficiency, take any $(m,k)\in S$. By \Cref{lem:valCS},
\begin{align*}
...
are positive. Apply \Cref{thm:main}.
...
Finally, \eqref{eq:explicitpositivebalance} gives
\[
 \lambda_a+\alpha^a\cdot p
 =\delta_a-\abs{\alpha^a}\varepsilon>0,
\]
and, by \eqref{eq:valCS},
\[
 c_{ij}-p_i-p_j
...
in \eqref{eq:explicitpositivebalance}.
\end{proof}
...
example in \Cref{sec:cancellation} uses $C=I$ and does not contradict this
...
\label[corollary]{cor:stationary}
Set $h=\val(\hbar)$, $a_i=\val(q_i)$, and $b_a=\val(u_a)$. The normalized
Wick expansion for \eqref{eq:action} is diagramwise strongly summable
exactly when
\begin{equation}
 \Delta_a=b_a+
 ...
 \label{eq:stationarycone}
\end{equation}
The same criterion governs every pairable polynomial insertion and the
connected vacuum expansion.
\end{corollary}
\begin{proof}
Substitute $\lambda_a=b_a-h$ and $c_{ii}=h-a_i$ into
\eqref{eq:delta}. This gives $\delta_a=\Delta_a$. The diagonal part of
\Cref{thm:positive} proves the exact criterion, \Cref{thm:main} gives the
observable statement, and \Cref{thm:connected} applies because all valences
are at least three.
\end{proof}
...
of \eqref{eq:stationarycone} is its exact necessity and sufficiency when the
...
$\sum_j\beta_j C_{ij}\EE_C[x^{\beta-\ve_j}]$, which is the right-hand side.
\end{proof}

\begin{theorem}[Exact perturbative Schwinger--Dyson identities]
\label[theorem]{thm:SD}
...
variables. Apply \eqref{eq:ibp} to each polynomial coefficient of
$f\exp(P)$ in those variables. Formal differentiation gives
\[
 \partial_j(f\exp(P))=(\partial_j f+f\partial_jP)\exp(P).
\]
This yields the unnormalized version of \eqref{eq:SD} coefficientwise.
...
in \eqref{eq:scaledatom}. The extra polynomial derivatives introduce only
...
\eqref{eq:scaledatom}, its valuation is at least
$p\cdot\beta+(\abs m+\abs k)\eta$, hence at least the right-hand side of
\eqref{eq:tail}. Every exponent of the atom is at least its leading exponent.
...
element. Thus \eqref{eq:tail} does not generally imply convergence of the
...
Under the explicit embedding in \Cref{prop:embedding}, these scales become
...
has radius of convergence zero. Nevertheless, \Cref{thm:positive} says that
\eqref{eq:quartic} has a strong Hahn value for every $g$ with $\val(g)>0$.
...
\eqref{eq:cubiccriterion} is satisfied. One coupling is infinite, yet each
...
\label[proposition]{prop:quarticH}
...
\begin{align}
 ...
 w_2&=\lambda_2+2c_{22}>0,\label{eq:quartictests}\\
...
\begin{equation}
 ...
 \label{eq:mixedbad}
\end{equation}
Their leading exponents descend without bound. The solver returns precisely
...
The matrix in \eqref{eq:mixedmodel} is invertible, since its determinant is
...
of \Cref{thm:positive}. All its displayed coefficients are positive, so the
...
\begin{equation}
 ...
 \label{eq:groupedone}
\end{equation}
Each cancellation in \eqref{eq:groupedone} is finite, but there are
...
\label[proposition]{prop:signcoherent}
Suppose every nonzero covariance entry is coefficientwise nonnegative, and
$g_a=\sigma u_a$ for a common $\sigma\in\{1,-1\}$, where each $u_a$ is a
...
\end{enumerate}
For a pairable sector, both are equivalent to \Cref{thm:main}.
\end{proposition}
\begin{proof}
At order $n$, every atom is $\sigma^n$ times a nonzero coefficientwise
nonnegative Hahn series. Its coefficient multiplier in \eqref{eq:atom} is
positive rational. Therefore no coefficient cancels within an order block,
...
claim follows from \Cref{thm:main}.
\end{proof}

The quartic model \eqref{eq:quartic} and the mixed obstruction
\eqref{eq:mixedmodel} satisfy this sign-coherent hypothesis. Their domains
are therefore also exact for the familiar expansion grouped by total
interaction order. The isotropic example fails the common-phase hypothesis
and demonstrates why a general cancellation-free theorem needs it.
...
The archive contains a standard-library Python implementation in
\texttt{code/wick\_certificates.py}. Its implemented value groups are
$\Q^r$ with lexicographic order, for any specified finite $r\ge1$.
...
algorithm of \Cref{thm:alternative}, and returns one of two certificates:
...
For example, the mixed-quartic obstruction can be reproduced by
...
\Cref{thm:connected}. Search the finite box $0\le q'\le q$ for a nonzero
...
\Cref{lem:wickcount}. It is connected automatically. Select a non-bridge
...
This distinction is part of the solver's contract.
...
question, not a proposed resolution of a spacetime singularity.
...
succeed. Targeted repository searches for \texttt{Wick} and
\texttt{Gaussian} returned no matches. These are useful negative search
observations, but they do not prove absence from compressed archives or
unindexed content.
...
searches, the snapshot identifier, and this access boundary.
...
\Cref{thm:main}, with its observable and robustness conclusions.\\
Connected obstruction amplification & Proposed contribution in
\Cref{thm:connected}; the valence boundary is explicit.\\
Positive-covariance collapse and multiscale thresholds & Consequences proved
in \Cref{thm:positive,cor:stationary}, presented as part of the theorem package.\\
Cancellation and sign-coherent boundary & Explicit limitations and exact
comparison results in \Cref{sec:cancellation}.\\
\bottomrule
\end{tabularx}
...
verify the paper in Lean or another proof assistant. No preexisting Lean
coverage in \Repo\ should be read as coverage of this manuscript.
...
through \Cref{prop:embedding}: they are actual normal-form numbers, not
...
This proves summability by \Cref{thm:main}.
...
the two directions of \Cref{thm:main}.
...
The normal-form and summability material used here is in Sections 2.3--2.6
of the arXiv text.
...
and 3.11. The book's printed page numbers and PDF page indices differ.
...
Scope and access limitations are specified in \Cref{sec:provenance}.
