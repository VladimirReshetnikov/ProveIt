...
order three and jet degree $\le3$; and linear mixed rigidity holds at unit
dilations. $\Gamma$ is \emph{not} assumed divisible; divisibility enters only
in the three threshold constructions of Section~\ref{hol:sub:divisibility},
each of which states it.
\end{abstract}
\end{minipage}
\vfill
\begin{minipage}{0.94\textwidth}
...
The full classifications and the theorem packages of the six later parts
remain pending. The
...
This report is built from nine manuscripts, numbered as they arrived in the
...
Section~\ref{hol:ot:sub:predecessors}, and those of sources~18 and~19 in
Sections~\ref{hol:ud:sub:predecessors} and~\ref{hol:nr:sub:predecessors}.
Section~\ref{hol:sec:status} and
Sections~\ref{hol:cf:sub:nonclaims}, \ref{hol:td:sub:nonclaims},
\ref{hol:ot:sub:nonclaims}, \ref{hol:ud:sub:nonclaims}
and~\ref{hol:nr:sub:nonclaims} record exactly what is and is not claimed.
...
unit-dilation and Newton-rigidity parts in
Sections~\ref{hol:ud:sec:setting}--\ref{hol:nr:sec:boundary}, whose
suggested formalization orders are recorded in
Section~\ref{hol:ud:sub:certificates} and
Question~\ref{hol:nr:q:formal},
...
\section{The nine finite check suites}\label{hol:sec:verification}
...
the recorded interpreter version. The suites of sources~08, 09, 10, 12 and~18
use only the Python standard library, integers, and
\texttt{fractions.Fraction}; the suites of sources~11, 13, 14 and~19 use exact
symbolic and rational arithmetic in SymPy, the only third-party package any
suite needs. None uses floating-point mathematics or network access; those of
sources~13 and~14 record their elapsed time.
...
$G_1$ is a polynomial. What the suites
can detect is sign,
...
order-three-threshold, unit-dilation and Newton-rigidity parts. The
coefficient-field and unit-dilation parts import one external recurrence
theorem without proof, the Skolem--Mahler--Lech theorem, used only for
relations involving at least two distinct dilations or multipliers. The equations use the
...
theorem packages remain pending.\\
\bottomrule
\end{longtable}
...
theta-descent, order-three-threshold, unit-dilation and Newton-rigidity parts,
...
and~\ref{hol:nr:sub:predecessors}.
...
source~19 as (R1)--(R16) in Section~\ref{hol:nr:sub:nonclaims}.
...
The last six questions come from source~19 (its Questions~12.2, 12.3, 12.5,
...
\ref{hol:ud:thm:dichotomy} & Theorems~A--K of this report are different
...
and Theorem~\ref{hol:nr:thm:systems} & Theorems~A--K of this report are different
theorems.\\
\bottomrule
\end{longtable}}
...
zero, which it proves in one line.
...
characteristic-zero field in the nonlinear, coefficient-field, theta-descent,
order-three-threshold, unit-dilation and Newton-rigidity parts; the latter
...
jet degree at most three (the Newton-rigidity part). Linear mixed equations in
...
unit-dilation part those of Section~\ref{hol:ud:sub:nonclaims}, and the
Newton-rigidity part those of Section~\ref{hol:nr:sub:nonclaims}; no
...
coefficient-field, theta-descent, order-three-threshold, unit-dilation and
Newton-rigidity theorem packages remain without Lean verification or
...
README, nineteen source audit documents at the top level, twenty files under
\texttt{code/} and thirty-one under \texttt{data/}. Of these, fourteen audit
documents, fourteen files under \texttt{code/} (the nine verification programs
of the sources merged here, the build scripts of sources~10, 11, 13 and~19,
and source~12's \texttt{Makefile}) and twenty-two under \texttt{data/} (the
nine recorded outputs, the source build and layout reports, and the
requirements files of sources~11, 13, 14 and~19) belong to the nine sources
merged into this text. The files prefixed \texttt{15-}, \texttt{16-} and
\texttt{17-} belong to three further manuscripts that have been placed in this
directory but whose merge into the text is pending; the README describes
them. No source document from the
...
path given by \verb|--output|, overwriting it, and otherwise only prints. All
use exact integer and rational arithmetic, those of sources~11, 13, 14 and~19
through SymPy, and exit with a nonzero status if a check fails. The build script
...
manuscript, whose source and PDF, hashed there, are not shipped.
...
Background for source~18's positive-characteristic direction; not an input to
any proof.
