...
dilations; and a dominant argument $f(P(z))$, $\deg P\ge2$, forces
polynomiality for every $\Gamma$. $\Gamma$ is \emph{not} assumed divisible; divisibility enters only
...
The full classifications and the theorem packages of the nine later parts
remain pending. The
...
This report is built from twelve manuscripts, numbered as they arrived in
...
and~\ref{hol:fh:sub:predecessors}, and that of source~15 in
Section~\ref{hol:pc:sub:predecessors}. Section~\ref{hol:sec:status} and
...
\ref{hol:fh:sub:nonclaims} and~\ref{hol:pc:sub:nonclaims} record exactly what
...
\section{The twelve finite check suites}\label{hol:sec:verification}
...
recorded interpreter version. The
...
computed for polynomials over $\Z^2$ only. What the suites
can detect is sign,
...
order-three-threshold, unit-dilation, Newton-rigidity and
polynomial-composition parts. The
...
relations involving at least two distinct dilations or multipliers. The
...
Newton-rigidity, theta-hierarchy, single-scale and polynomial-composition
parts remain pending.\\
\bottomrule
\end{longtable}
...
theta-hierarchy, single-scale and polynomial-composition parts, with their own
search limits, are Sections~\ref{hol:nl:sub:predecessors},
...
\ref{hol:fh:sub:predecessors} and~\ref{hol:pc:sub:predecessors}.
...
constant-term retraction, and its encoding Euler projectors.
...
characteristic-zero field in the nonlinear, coefficient-field, theta-descent,
order-three-threshold, unit-dilation, Newton-rigidity, single-scale and
polynomial-composition parts (with $\R$ or $\C$ for its omnific rings), and
$\R$ or $\C$ in the
...
coefficient-field, theta-descent, order-three-threshold, unit-dilation,
Newton-rigidity, theta-hierarchy, single-scale and polynomial-composition
theorem packages remain without Lean verification or independent refereeing.
...
\texttt{code/} and thirty-two under \texttt{data/}. Of these, nineteen audit
documents, twenty files under \texttt{code/} (the twelve verification programs
of the sources merged here, the build scripts of sources~10, 11, 13, 17
and~19, and the \texttt{Makefile}s of sources~12, 15 and~16) and thirty-one
under \texttt{data/} (the twelve recorded outputs, the source build, layout and
visual reports, and the requirements files of sources~11, 13, 14, 15, 17
and~19) belong to the twelve sources merged into this text. The files
prefixed \texttt{20-} belong to one further manuscript that has been placed in
this directory but whose merge into the text is pending; the README describes
...
given, overwriting it. All use exact integer and rational arithmetic,
