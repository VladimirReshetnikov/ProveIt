...
nonterminating formal solution on a computable exterior valuation region,
...
where it is used, in the four statements listed in
...
resolution of a named published conjecture. No independent refereeing and no
Lean verification is claimed. The variable derivative used throughout is not
...
every nonzero dilation valuation (Theorem~\ref{hol:thm:coarse}); and the exact
...
four places, and each is hypothesised where it is used.
...
\item The ``only if'' half of the torsion eigenspace entireness equivalence,
Proposition~\ref{hol:prop:torsion}\,(c), which extracts an $m$th root of an
arbitrary element of $\K$ and therefore needs $\delta/m\in\Gamma$.
...
proves rigidity in groups of very large rank. It is also why the strict
version below, which divides by $j$, is a refinement of the constant and never
of the conclusion.
...
nonzero for all $n\ge N$ with eventually periodic valuation of common period
$m$, and let $\alpha_{j,r}$ be the valuation of slot $j$ on the residue class
...
$P(q^n)=Q_r(1+V_k)$. The same minimum \eqref{hol:eq:betar} and the same
...
integer beyond the recurrence's starting index and beyond every nonnegative
...
Corollary~\ref{hol:cor:periodic} applies with $m=h_0$.
\end{proof}
...
When $\beta$ is an order unit one may, if desired, push the workspace forward
by the real-valued coarsening
$\pi:\Gamma\to\R$ of~\cite[Lemma~9.1]{repoentire} with $\pi(\beta)=1$, in
which $\Theta_p$ becomes an ordinary rank-one partial theta series. We record
this only as an orientation. It is used in no proof here, and it is
unavailable in exactly the case the classification is about, namely when no
order unit exists.
\end{remark}
...
\item If in addition $\Gamma$ is divisible, the converse of \textup{(b)} also
holds, so $f\in\E$ if and only if $G\in\E$.
\end{enumerate}
Moreover, every root of unity of $\K$ lies in $\C$, so the hypothesis
$q\in\C^\times$ loses no torsion element of the Hahn field. Parts
\textup{(a)}, \textup{(b)} and this last assertion use only
Convention~\ref{hol:conv:group}.
\end{proposition}
...
For (c) assume $\Gamma$ divisible and let $y\in\K^\times$. Write
$y=c\,t^{\delta}(1+u)$ with $c\in\C^\times$ and $u$ zero or of positive
valuation. Choose an $m$th root of $c$ in $\C$, and use $\delta/m\in\Gamma$,
which is exactly where divisibility is needed. For $u\ne0$ the family
$\bigl(\binom{1/m}{k}u^k\bigr)_{k\ge0}$ is strongly summable: its supports lie
in $M(\supp u)$, which is well ordered, and each of its elements has only
finitely many word representations by Lemma~\ref{hol:lem:support}\,(2), so it
lies in $\supp(u^k)$ for only finitely many $k$. Its sum $(1+u)^{1/m}$
satisfies $\bigl((1+u)^{1/m}\bigr)^m=1+u$, the rearrangement at each exponent
being finite; for $u=0$ take the value $1$. This produces $x\in\K^\times$ with
$x^m=y$.
The evaluation family for $G$ at $y$ is then $x^{-r}$ times the subsequence in
degrees $r+mn$ of the evaluation family for $f$ at $x$, hence strongly
summable; at $y=0$ the evaluation is finite.
...
Part (c) is the one place in this report where an entireness \emph{equivalence}
needs divisibility, and the hypothesis is printed in the statement for that
reason. Parts (a) and (b), and the classification of roots of unity, hold
under Convention~\ref{hol:conv:group}.
...
and Lemma~\ref{hol:lem:certificate}\,(a) applies when $w=0$.
...
The most reusable formalization target is Lemma~\ref{hol:lem:escape} together
...
The Python checks distributed with this report are not a Lean formalization,
and the decomposition above is a dependency plan, not a claim that these
statements already occur in the repository's verified code.
...
Priority, peer review, formal verification & Not certified; no Lean proof is
supplied or claimed.\\
\bottomrule
\end{longtable}
...
needed and no machine-checked correctness is asserted.
...
Proposition~\ref{hol:prop:torsion}\,(c), and
Example~\ref{hol:ex:sparse}, each of which states it.
...
is claimed; the finite checks prove no infinite theorem; and no Lean
verification or independent mathematical review is claimed.
...
outputs and both source build reports under \texttt{data/}, and the two source
...
Theorem~1.5 gives the D-finite/P-recursive equivalence.
