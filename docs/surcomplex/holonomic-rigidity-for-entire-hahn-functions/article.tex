...
order three; whether order two occurs stays open. $\Gamma$ is \emph{not}
assumed divisible;
...
The full classifications and the nonlinear, coefficient-field and
theta-descent theorem packages remain pending. The variable derivative is not the
...
Question~\ref{hol:q:nonlinear} is re-scoped to order exactly two.
...
equation over all examples is two or three, and the question is re-scoped to
order two (Section~\ref{hol:sec:status}). The equation of $\mathcal T_p$ is
...
This report is built from six manuscripts, numbered as they arrived in the
...
Section~\ref{hol:cf:sub:predecessors}, and that of source~13 in
Section~\ref{hol:td:sub:predecessors}. Section~\ref{hol:sec:status} and
Sections~\ref{hol:cf:sub:nonclaims} and~\ref{hol:td:sub:nonclaims} record
...
and whether order two occurs is Question~\ref{hol:q:nonlinear}.
\end{remark}
...
row of the second table (Theorem~\ref{hol:td:main:boundary}); this does not
...
Sections~\ref{hol:cf:sec:setting}--\ref{hol:cf:sec:families}, and the
...
Remark~\ref{hol:td:rem:corner}). Whether order two occurs is
...
\section{The six finite check suites}\label{hol:sec:verification}
...
the interpreter version and the elapsed time. The suites of sources~08, 09,
10 and~12 use only the Python standard library, integers, and
\texttt{fractions.Fraction}; the suites of sources~11 and~13 use exact
symbolic and rational arithmetic in SymPy, the only third-party package any
suite needs. None uses floating-point mathematics or network access; source~13's
records its elapsed time.
...
finite truncation of $\mathcal T_p$ is a polynomial. What the suites
can detect is sign,
...
answered in order one, without an order unit in all orders, and with an order
unit by an example of order three, is a question of this report, not a
historical conjecture.\\[4pt]
Priority, peer review, formal verification & Priority and independent
refereeing are not certified. Lean covers support and escape lemmas,
exponential and partial-theta domains, polynomial-orbit valuations,
constant-point avoidance, inward stability and torsion covariance.
The full classifications and the nonlinear, coefficient-field and
theta-descent theorem packages remain pending.\\
\bottomrule
\end{longtable}
...
corresponding records for the nonlinear, coefficient-field and
theta-descent parts, with their own search limits, are
Sections~\ref{hol:nl:sub:predecessors}, \ref{hol:cf:sub:predecessors}
and~\ref{hol:td:sub:predecessors}.
...
separately as (C1)--(C26) in Section~\ref{hol:cf:sub:nonclaims}, and those of
source~13 as (T1)--(T19) in Section~\ref{hol:td:sub:nonclaims}.
...
$\Gamma$ has an order unit; Proposition~\ref{hol:td:prop:corner}.)
...
\begin{question}[Nonlinear differential equations of order two, with an
order unit]\label{hol:q:nonlinear}
...
$\K(z)$ of order $s=2$, that is, are $f$, $D_zf$ and $D_z^2f$ algebraically
dependent over $\K(z)$ for some nonpolynomial $f\in\E$? By
Theorem~\ref{hol:nl:main:first} no equation of order at most one is possible,
and by Theorem~\ref{hol:td:main:boundary} order three occurs, so the least
order over all examples is two or three. More broadly, to what extent do
finite algebraic descents of theta or abelian functions, combined with
elementary operations preserving strong entireness, account for the
nonpolynomial strongly entire series that are differentially algebraic over
$\K(z)$? The same questions are open over $\KK=k((t^\Gamma))$ for every field
...
classification (its Questions~11.1 and~11.2); the question is re-scoped to
these, as printed above. Neither $\Theta_p$ nor $G$ is decided: whether either
is differentially algebraic stays open.
...
The last question comes from source~13 (its Question~11.3).
...
and it is now re-scoped to order two in Section~\ref{hol:sec:status}.
...
series nor the possible minimal orders: whether order two occurs for another
series is Question~\ref{hol:q:nonlinear}.
...
different assertion. The geometric route suggested by
...
Whether some other nonpolynomial entire series satisfies an equation of order
exactly two is the open part of Question~\ref{hol:q:nonlinear}.
...
Theorem~\ref{hol:cf:main:entire}\,(a).
...
order three. No general mixed-jet independence theorem is claimed when
...
shows that universal differential algebraic rigidity fails for them, and
whether minimal order two occurs is not decided. The nonlinear part keeps the
non-claims of Section~\ref{hol:nl:sub:nonclaims}, the no-order-unit results
keep those of Section~\ref{hol:cf:sub:nonclaims}, and the theta-descent part
keeps those of Section~\ref{hol:td:sub:nonclaims}; no complete algorithm for arbitrary
...
coefficient-field and theta-descent theorem packages remain without Lean
verification or independent refereeing.
...
README, ten source audit documents at the top level, ten files under
\texttt{code/} (the six source verification programs, the build scripts of
sources~10, 11 and~13, and source~12's \texttt{Makefile}), and fifteen under
\texttt{data/} (the six recorded outputs, the source build and layout
reports, and the requirements files of sources~11 and~13). No source document from the
...
\texttt{verification.json} in the current directory when no path is given. All use exact integer and
...
by source~13 on 23 September 2026.
...
Publisher abstract and bibliography consulted by source~13; full text not
inspected.
