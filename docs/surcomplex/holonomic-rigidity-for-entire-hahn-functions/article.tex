...
it; the single-scale part uses no divisibility.
...
This report is built from eleven manuscripts, numbered as they arrived in
...
and~\ref{hol:fh:sub:predecessors}. Section~\ref{hol:sec:status} and
...
and~\ref{hol:fh:sub:nonclaims} record exactly what is and is not claimed.
...
unit-dilation, Newton-rigidity, theta-hierarchy and single-scale parts in
Sections~\ref{hol:ud:sec:setting}--\ref{hol:fh:sec:realizations}, whose
suggested formalization orders are recorded in
Sections~\ref{hol:ud:sub:certificates} and~\ref{hol:th:sub:third} and
Questions~\ref{hol:nr:q:formal} and~\ref{hol:fh:q:formal},
...
\section{The eleven finite check suites}\label{hol:sec:verification}
...
suites of sources~08, 09, 10, 12, 16 and~18 use only the Python standard
library, integers, and \texttt{fractions.Fraction}; the suites of sources~11,
13, 14, 17 and~19 use exact symbolic and rational arithmetic in SymPy, the
...
no supports, equations or continuum families. What the suites
can detect is sign,
...
Newton-rigidity, theta-hierarchy and single-scale parts remain pending.\\
\bottomrule
\end{longtable}
...
source~16 as (Th1)--(Th16) in Section~\ref{hol:th:sub:nonclaims}, and those of
source~17 as (F1)--(F19) in Section~\ref{hol:fh:sub:nonclaims}.
...
\ref{hol:ud:thm:dichotomy} & Theorems~A--M of this report are different
...
and Theorem~\ref{hol:nr:thm:systems} & Theorems~A--M of this report are different
theorems.\\
\bottomrule
\end{longtable}}
...
Theorems~1.1--1.3 & Theorem~\ref{hol:th:main:hierarchy} & Theorems~A--M of
...
Theorem~\ref{hol:fh:main:single} collects them; Theorems~A--M of this report
...
Berarducci--Mantova derivation and the normal-form definition of $\Oz$.
...
characteristic-zero field in the nonlinear, coefficient-field, theta-descent,
order-three-threshold, unit-dilation, Newton-rigidity and single-scale parts, and $\R$ or $\C$ in the
...
theta-hierarchy part those of Section~\ref{hol:th:sub:nonclaims}, and the
single-scale part those of Section~\ref{hol:fh:sub:nonclaims}; no
...
coefficient-field, theta-descent, order-three-threshold, unit-dilation,
Newton-rigidity, theta-hierarchy and single-scale theorem packages remain
without Lean verification or independent refereeing.
...
\texttt{code/} and thirty-one under \texttt{data/}. Of these, eighteen audit
documents, eighteen files under \texttt{code/} (the eleven verification programs
of the sources merged here, the build scripts of sources~10, 11, 13, 17
and~19, and the \texttt{Makefile}s of sources~12 and~16) and twenty-eight under
\texttt{data/} (the eleven recorded outputs, the source build, layout and
visual reports, and the requirements files of sources~11, 13, 14, 17 and~19)
belong to the eleven sources merged into this text. The files prefixed
\texttt{15-} belong to one further manuscript that has been placed in this
...
overwrite, so pass new paths. All use exact integer and rational arithmetic,
those of sources~11, 13, 14, 17 and~19 through SymPy, and exit with a nonzero
