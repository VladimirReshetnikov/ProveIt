...
Because $\Phi$ moves the chain by an infinitesimal of strictly positive
support, $\eta^\alpha$ has support bounded below by $|\alpha|\cdot\inf E$, so at
each Hahn exponent only finitely many multi-indices $\alpha$ contribute, and
only finitely many ordinary derivatives $\partial^\alpha a$ are ever read.
...
requiring that much would make the problem unsolvable. Nor is
$Z:\mathsf T\to K^n$ asserted to be continuous for any of the three topologies
of \cref{contours:rem:threetopologies}; by \cref{contours:prop:fullfine} a
nonconstant such map cannot be.
...
no larger than its partner cannot increase the sum, equality of sums forces
identical words because every letter is strictly positive, and both a descending
...
Apply \cref{contours:lem:radiusfreesub} with $P_W=\{\rho\}$; at a fixed output
exponent only finitely many radial degrees $\ell$ occur, by
\cref{contours:lem:support}.
\end{proof}
...
that manifold. Neither statement implies the other, and neither produces a
common ordinary radius for the original coefficient germs.
...
extensions agree on overlaps. Continuity gives $f(Z(q,0))=0$, whence
$Z(q,0)=0$ because $f^{-1}(0)=\{0\}$.
...
Holomorphy in the punctured $s$ disk and uniqueness of the Laurent expansion
upgrade that single value to the coefficient statement.
\end{proof}
...
\emph{larger} infinitesimal, so the cycle must be large in valuation in order to
protect the perturbation. Expanding the ordinary family of
...
flatness theorem or a Nullstellensatz. For the second identity, $f(0)=0$ makes
each ordinary composition with $f$ a convergent germ and the expansion in $E$ is
support-controlled by \cref{contours:lem:radiusfreesub}; on the adapted cycle
...
repository commit \texttt{39f2be66}. Its verbatim source, its exact symbolic
check script and that script's recorded run are kept in this directory under
\texttt{sources/}, \texttt{code/} and \texttt{data/}, all three carrying the
file-name stem \texttt{05-untransformed-\allowbreak leray-\allowbreak cycles}.
It is the source of
