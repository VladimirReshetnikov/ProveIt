...
multiscale contours, and radius-free residues\par}
\vspace{0.6cm}
...
arbitrary isolated complete intersection.
...
explicitly verified overlaps. Root-of-unity sampling is analyzed on both sides: a
coefficientwise recovery theorem under an explicit decay hypothesis, and an exact
example showing that the hypothesis cannot be dropped. Berkovich, tropical,
...
period theory rather than a consequence of fine connectedness. Finally, positive
...
This article merges three manuscripts of September 21, 2026 that share roughly two
thirds of their content: \emph{Stokes Calculus}, \emph{Stokes Integration}, and
...
the first. The separately numbered winding-stability theorem
\cref{contours:thm:winding}, the numbered admissible-deformation definition
...
\cref{contours:app:transform} come from the third.
...
Polynomial-coefficient & $\C[w_1,\ldots,w_n]((t^\Gamma))$
  & Every Hahn coefficient is an ordinary polynomial; equivalently the
    common-domain ring with $U=\C^n$. This is the uniform-support target of
...
Formal & $K[[z]]$, and $\C[[z]]((t^\Gamma))$
  & No convergence of any coefficient is required. Used only for injectivity
    statements and for the formal residue class
    \eqref{contours:eq:formalresclass}.\\
\bottomrule
\end{longtable}
\end{center}
...
\cref{contours:sec:radiusfree,contours:sec:multi}. The sign $(-1)^{|k|}$ and the
...
analytic and the same positive-support substitution hypotheses hold, and it agrees
with canonical real-analytic infinitesimal evaluation of the coefficients at every
ordinary parameter point.
\end{lemma}
\begin{proof}
...
with ordinary derivatives and products, hence coefficientwise. Equality at every
...
deformation, and let $\omega$ be a Hahn $(n-1)$-form with real-analytic coefficient
forms on the ordinary coefficient neighborhood $U$. Then
...
$\sum_ac_{a,1}$. That no other infinitesimal zero exists is
\cref{contours:lem:protect}; agreement with
...
\subsection{Scope: what a general isolated complete intersection still requires}
\label{contours:subsec:scope}
This subsection records a disagreement about \emph{status} among the three
manuscripts merged here. It is the sharpest in the set, and it is not averaged.
...
Suppose a semialgebraic loop $\gamma$ has an affine normalized description
$C=a+r\gamma_R$, where $\gamma$ is an ordinary piecewise-polynomial closed loop and
$\gamma_R$ its semialgebraic extension to field-valued parameters, whose restriction
to ordinary parameters is an admissible Hahn-deformed contour $\gamma_0+\eta$.
Assume that the same description on the full field interval is uniformly
infinitesimally close to its ordinary real-coefficient leading loop. Suppose each
...
\[
  I_{\mathrm{alg}}(\gamma,\omega)=\HInt_\gamma\omega .
\]
The same holds after a polynomial infinitesimal deformation for which a definable
pole-avoiding homotopy to this loop is verified.
\end{proposition}
\begin{proof}
...
$(\gamma,b)$ to $(\gamma_0,c)$ avoid zero differences throughout, so
$\wind_{\mathrm{sa}}(\gamma,b)=\wind(\gamma_0,c)$. \Cref{contours:thm:winding} then
...
\section{Root-of-unity sampling: a positive theorem and its indispensable
hypothesis}
...
positive side. They are compatible, and the hypothesis is doing all the work. This
section therefore states the positive theorem \emph{with} that hypothesis and keeps
the negative example immediately alongside as the demonstration that the hypothesis
cannot be dropped.
...
\end{equation}
the $\xi$ being the ordinary complex roots of unity. Cherry's presentation of the
...
datum for arbitrary functions, not just its absolute value. In a Hahn field with
coefficient field $\C$ the natural valuation has residue characteristic zero and
...
\subsection{Why the hypothesis cannot be dropped}
...
\emph{A single-cycle realization for arbitrary finite Hahn zero schemes.} Given
...
complete intersections. The rational pairing is defined independently from
...
in \cref{contours:thm:septorus}. In particular \cref{contours:thm:polstokes,contours:thm:hahnstokes,contours:thm:cohomology,contours:thm:homotopy,contours:thm:generalrepresentation} and the sampling comparisons stand independently
of those deeper analytic-algebra claims.
...
\bibitem{BR}
M. Baker and R. Rumely, \emph{Potential Theory and Dynamics on the Berkovich
Projective Line}, Mathematical Surveys and Monographs, vol.~159, American
Mathematical Society, 2010.
\href{https://doi.org/10.1090/surv/159}{doi:10.1090/surv/159}.
...
\bibitem{Cherry}
W. Cherry, \emph{Non-Archimedean Analytic Curves in Abelian Varieties} and lecture
notes on non-Archimedean function theory, especially \S1.3 on the Schnirelman
integral.
...
\bibitem{Kaiser}
T. Kaiser, ``Integration of semialgebraic functions and integrated Nash functions''
and related work on tame measure and integration over non-Archimedean real closed
