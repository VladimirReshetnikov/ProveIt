...
one direction, the coefficient field, and that generality is carried into
Theorem 1.2 and its added sections. The proof review also extends modular
recovery (Theorem 11.1) to any characteristic-zero coefficient field.
Source 02 supplies the multivariable theta half of the open question on higher-dimensional
...
1. **Exact domain and uniformization, Theorem 1.2.** The bilateral theta
   family is strongly Hahn-summable exactly on `U_q` (Theorem 3.1); for `u ∈ K^×`
...
5. Support-preserving recovery of `q` from `j` when `v(j) < 0` (Theorem
   11.1, also over any characteristic-zero coefficient field); if `Γ` is
   divisible, every elliptic curve over `K` with `v(j) < 0` is
   ...
   12.2, Corollary 12.3; Theorem 1.3). The torsion and classification statements
   retain complex coefficients; modular recovery does not require them.
...
- A real rank-one value group identifies the last quotient in the
  value-layer sequence with a real circle; the full value quotient still retains the infinitesimal
  subgroup. Even at rank one, field-rational values can form a proper
  subgroup of the full analytic skeleton if the value group is smaller
  than the reals.
...
- Arbitrary characteristic-zero coefficient fields are allowed in Theorem
  1.2, Sections 8–9 and 10.3, and modular recovery (Theorem 11.1).
  Torsion and classification retain their complex-coefficient hypotheses;
  Corollary 11.2 also assumes divisibility.
...
During the arithmetic review, the PDF rebuilt in three passes at 64 pages,
and the catalogue at 22, both without warnings or box issues. Source 01's
unchanged verifier passed on a temporary copy with Python 3.13.14 and
SymPy 1.14.0; only the Python-version field differs from its historical JSON.
