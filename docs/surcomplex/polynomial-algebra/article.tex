...
exactly $k-r$ roots of the $r$th derivative. A finite root-cluster tree, an
...
error term; and the coarser but computable discriminant certificate, shown to
be strictly weaker on an explicit quartic and sharp at equality on an
explicit quadratic.
...
exhaustive. Nothing here has been independently refereed or verified in a
proof assistant; the symbolic checks recorded in the source archives validate
...
Residue Duality'') is the only one of the three that leaves one variable. It
...
$\OO_K$.\\[0.5em]
$\HH(U)=\OO(U)((t^\Gamma))$, with the subrings $\HHn(U)$ and $\HHp(U)$ of
nonnegative and strictly positive support &
...
Consequently an expression indexed by one exponent of a well-ordered set and
one finite word in $S$ is strongly summable whenever each word contributes
finitely many terms.
\end{lemma}
\begin{proof}
...
fixed} ordinary domain $U$, with $\HHn(U)$ and $\HHp(U)$ its subrings of
nonnegative and of strictly positive support. A datum $F=\sum f_\gamma
t^\gamma$ is evaluated at a finite displaced point $c+\varepsilon$ by Taylor
...
Euclidean algorithm then produces $D$ and the B\'ezout coefficients, and a
nonzero member of least degree generates any ideal. The multiplicity
...
Suppose $P\in F[z]$. Every nonreal critical point belongs to at least one
...
In particular a real-rooted polynomial has only real critical points. The
...
Tarski's theorem gives quantifier elimination for ordered fields, so a
...
limiting process and is meaningful for negative, positive, finite, infinite
and nonreal surreal exponents alike. Put
...
Over $K$, at a finite exponent $\rho$, the number of nonzero roots of
...
\end{equation}
strongly summable by \cref{polynomial:lem:support} with support in
$\{\gamma_k-1/2\}^*$. Its use does not require that the original coefficient
...
For finitely many polynomials of bounded finite degree one can choose one
...
$P=z^n+\sum_{j<n}a_jz^j$ choose $\rho\in\Gamma$ with
...
assertion apply \cref{polynomial:thm:initialroots} to $I'$, which is the
initial polynomial of $P'$ up to the nonzero scalar $t^{\rho-\lambda}$.
\end{proof}
...
ball exactly $d_--1$. The indicated restriction of $P$ is bijective exactly
when its mapping degree is one, equivalently when the ball contains no
critical point.
\end{corollary}
\begin{proof}
...
finitely many critical values; such a target exists in the infinite image
ball, and its degree-many preimages are simple and distinct, so the map is not
injective.
\end{proof}

The last qualification prevents confusing multiplicity-counted one-to-one
...
Suppose the closed ball $\Bge a\rho$ over $K$ contains $k$ roots and its
...
Every node $C$ is exactly the set of roots in $\Bge a{\delta_C}$ for
...
$\eta\in\mm_K$ and $\supp\eta\subseteq S^*\setminus\{0\}$. At the least
positive error exponent $e=\min S$ the coefficient is
...
first correction is assumed.
\end{corollary}
\begin{proof}
Apply \cref{polynomial:thm:hensel} to the coprime pair $z-c$ and
$P_0/(z-c)$; the lifted first factor is $z-c-\eta$, and for any root reducing
to $c$, uniqueness of this linear factor identifies it with $c+\eta$.
Expanding $P(c+\eta)=0$ at the least positive input exponent gives
$P_0'(c)\eta_e+E_e(c)=0$.
\end{proof}
...
a single computable number; strictly weaker in general\\
\bottomrule
\end{tabularx}
\end{center}
...
The right side of \eqref{polynomial:eq:newtonerror} is strictly greater than
...
$4,4,6,8$ by \eqref{polynomial:eq:leadingdisplacement}. All three original
cluster scales, and the residual critical directions $3/4$, $2/3$, $1/2$ of
the closely related nested quartic of \cref{polynomial:ex:nested}, survive
such a perturbation.
\end{example}
...
If $H$ vanishes at all common zeros, then in $K[x,y]$ the polynomials
$F_1,\ldots,F_s,1-yH$ have no common zero, so by the weak form there is an
identity $1=\sum_iA_i(x,y)F_i(x)+B(x,y)(1-yH(x))$. If $H=0$ the certificate is
...
just stated, over the fixed-domain ring $\HHp(U)$,
...
exhaustive. Nothing here has been independently refereed or verified in a
proof assistant. The symbolic checks recorded in the source archives use exact
...
ball; branch directions; nearest-neighbour identity &
...
delimited polynomial subclass & Degree bookkeeping plus positive support, over
$\HHp(U)$ for one fixed $U$; \cref{polynomial:thm:analyticcomparison}.
...
ordinary domain $U$, and its nonnegative- and positive-support subrings.\\
\bottomrule
\end{longtable}
...
Rida T. Farouki and Chang Yong Han, ``Root neighborhoods, generalized
lemniscates, and robust stability of dynamic systems,'' with the standard
pseudozero-set formulation used as the ordinary antecedent of
\cref{polynomial:thm:pseudozeros}.

\bibitem{polynomial:BCRS}
Eduardo Cattani, Alicia Dickenstein and collaborators, on residues,
B\'ezoutians and symmetric bilinear forms in elimination theory; standard
reference material for the multivariate duality of
\cref{polynomial:sec:bezoutian} and for the Hermite form of
\cref{polynomial:thm:hermite}.

\bibitem{polynomial:BMP}
Laurent Bus\'e, Houssam Khalil and Bernard Mourrain, on resultants,
B\'ezoutians and the algebra of a zero-dimensional complete intersection;
broader context for \cref{polynomial:thm:multiperfect,polynomial:thm:jacobian}.
