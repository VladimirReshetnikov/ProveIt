...
\hypersetup{%
  ...
  pdfkeywords={surreal numbers, surcomplex numbers, Berarducci--Mantova derivation, H-field, Hahn series, differential Galois theory}}
...
field $\SC$. This report is their union.
...
\textbf{Status.} A merged exposition assembled from seven independent
manuscripts, all pinned to \Repo\ at commit \Commit. Deep structural inputs
...
\subsection{Seven manuscripts, one theory}
\label{diff:sub:seven}

This report is the union of seven independently written manuscripts on the
differential algebra of the surreal and surcomplex numbers. They were prepared
separately, they are all pinned to the same documentation snapshot, and they
prove --- in seven notations --- the same twelve-item spine:
...
Every one of the seven proves items (i)--(x); at least five prove (xi) and
(xii). Keeping them apart would mean printing one theory seven times,
...
any theorem is stated: three different objects in these manuscripts are called
``phase'', and two nearly identical classes are both called the exponential
domain. A merged sentence that unifies either word, or either symbol, proves
false statements. This report fixes three names and three symbols for the
three phases, keeps the two classes visibly distinct, and states the
distinction wherever a reader might carry one sense into another.
...
results of \cite{ADH} are background, not premises. Ordinary complex
...
$\der\mm\subseteq\mm$. Two of the seven source manuscripts import it as a
...
Two source manuscripts instead \emph{import} $\der\mm\subseteq\mm$ and
strengthen it to $\der\Ofin=\der\mm\subseteq\mm$ by a square-root trick: a
...
exactly when the accumulated phase of $b$ happens to be a finite angle, in
which case $\eta=B-\st(B)$ is its infinitesimal residue.
\end{enumerate}
\end{convention}

\begin{table}[htbp]
\centering\small
\caption{The three phases, and where each source's notation lands.}
...
exactly when $\pp(B)\neq0$ &
$B$; $\Izero(b)$; and four different letters for the primitive operator \\
\bottomrule
\end{tabularx}
\end{table}

\begin{warning}[Where the three collide in the sources]
\label{diff:warn:phasecollision}
Two of the seven manuscripts use ``phase'' for sense (a), four use it for sense
(b), and all seven use ``accumulated phase'' for sense (c). One of them prints
...
without \cref{diff:thm:phasecriterion}.
\end{warning}
...
\label{diff:conv:noJ}
This report uses no symbol $J$. The normalized intrinsic primitive is
...
\begin{example}[A genuinely infinite Hahn phase]
...
For $w=x+i\theta\in\SC$ with $x,\theta\in\No$, the following are equivalent.
...
\cref{diff:thm:maximalstrip}(1) is $\der\theta\in\Acl$, and
...
$\Imm(\der w)=\der\theta\in\Acl$. Every accumulated phase of $\der\theta$ is
$\theta+r$ with $r\in\R$, so by \cref{diff:thm:phasecriterion} such a phase is
finite exactly when $\theta$ is finite; equivalently
...
only the first invariant; \cref{diff:sec:nonclaims} records that no
classification of arbitrary nontriangular systems over $\SC$ is offered here.
...
\cref{diff:lem:smallnegative} uses only the displayed positivity rule and
$\der\omega=1$, so it applies in this ordered extension and forces
...
\end{lemma}
\begin{proof}
If $k\sum_{j}n_{j}B_{j}=0$, torsion-freeness of the real vector class $\Pcl$
gives $\sum_{j}n_{j}B_{j}=0$. So the quotient is finitely generated and
...
$B'_{\ell+1},\dots,B'_{m}$ are $\Z$-independent, since any relation among them
...
for triangular extensions. It is \emph{not} a classification of
Picard--Vessiot groups for arbitrary nontriangular matrices over $\SC$. For a
...
fundamental matrix $\Phi_{0}$ of $\Dz\Phi_{0}=A_{0}\Phi_{0}$,
$\Phi_{0}(z_{0})=I_{d}$. Uniqueness holds among \emph{all} Hahn-coherent
matrices, not only those whose support was prescribed in advance.
\end{theorem}
\begin{proof}
Ordinary holomorphic linear theory gives $\Phi_{0}$ on $U$, normalized at
...
\cite[Ch.\ 4]{Teschl}. Put $B=\Phi_{0}^{-1}E\Phi_{0}$, whose support lies in
$S$, and $T H=I_{z_{0}}(BH)$. For each $n$ the support of $T^{n}I_{d}$ lies in
...
$\Dz V=BV$ and $V(z_{0})=I_{d}$; then $X=\Phi_{0}V$ solves
...
support. The equation $\Dz H=BH$ forces $H'_{\gamma}=0$: any contribution on
...
\begin{equation}\label{diff:eq:linearrecursion}
 V'_{\gamma}=\sum_{s+\delta=\gamma}B_{s}V_{\delta},
...
\[
 \det X=\det\Phi_{0}\cdot\Emm\bigl(I_{z_{0}}(\tr E)\bigr).
\]
...
$z_{0}\in U$, and $y_{0}'(z)=F_{0}(z,y_{0}(z))$. Let $\eta\in K_{\Gamma}^{d}$
...
\begin{equation}\label{diff:eq:ivp}
 \Dz Y=F(z,Y),\qquad Y(z_{0})=y_{0}(z_{0})+\eta,
\end{equation}
...
matrix $\Phi$ on $U$ with $\Phi'=A\Phi$, $\Phi(z_{0})=I_{d}$; simple
...
\begin{equation}\label{diff:eq:coefode}
 ...
 \qquad u_{\gamma}(z_{0})=\eta_{\gamma},
\end{equation}
...
\begin{equation}\label{diff:eq:coefsolution}
 u_{\gamma}(z)=\Phi(z)
 \left(\eta_{\gamma}+\int_{z_{0}}^{z}\Phi(\zeta)^{-1}R_{\gamma}(\zeta)\,
 d\zeta\right)
\end{equation}
...
inhomogeneous solve with the \emph{same} fundamental matrix $\Phi$: the
...
operation; they are cited for context and are not a substitute for an
unstated linear-solvability theorem. The $\omega$-series composition results
...
the main proofs here. Three cross-links matter.
...
at least one of the seven source manuscripts; all of them survive the merge.
Several are also stated in place above, and the cross-references say where.
...
a naming convention for this distinguished construction.
...
and the placement suggestions they made were suggestions.
\end{enumerate}
...
that primitives are computable from finite input descriptions.
\item No general inhomogeneous existence theorem is claimed. Outside the
admissible locus $\Phi(q)=0$ the only assertion is \emph{uniqueness}
...
theorem would need is not invoked. Nor is it claimed that such broader
...
\item No classification of arbitrary nontriangular matrix systems over $\SC$
is given. The trace condition is only the first invariant
(\cref{diff:prop:trace}), the torus theorem treats \emph{diagonal} systems
only (\cref{diff:thm:torus}), constant matrices are settled while arbitrary
matrices with surreal entries are not, no general matrix normal form follows
from the scalar result, and higher-rank differential modules are not
classified.
...
\item The seven delivered verification suites check \emph{finite exact
symbolic identities and finite jets}, and nothing else. In total they record
$1{,}395+115+149+258+291+150+259$ passing checks
(\cref{diff:app:verification}); a matrix identity or a whole checked
coefficient prefix counts as one check in two of the suites. They do not
...
Classify logarithmic derivatives in multi-scale $\der$-stable Hahn workspaces
where support shifts lack the uniform rational form of
\cref{diff:eq:hahnderiv}. Determine useful classes of variable-coefficient
matrix equations whose fundamental matrices remain in $\No[i]$, distinguishing
genuine phase obstructions from workspace obstructions, and understand which
nontriangular differential modules admit a filtration whose rank-one factors
have computable obstructions --- which would require genuine module-theoretic
information, not merely eigenvalues of an instantaneous coefficient matrix.
...
domains in the nonlinear case. Connect the explicit phase invariant with the
strongest available linear-solvability theorems for this $H$-field, separating
existence from the dimension of the homogeneous solution space.
...
uses $D$ for no operator at all, and keeps
\cref{diff:tab:derivatives} as the reference.
...
The letter $U$ is used in this report only for an ordinary complex domain
(\cref{diff:part:coordinate}). One source manuscript flagged its own collision
...
The seven source manuscripts are preserved unmodified in the
\path{sources/} directory of this report's package, together with their own
READMEs and audit files. The identifiers below are the merge identifiers; the
original archive names collided, three pairs of them differing only by a
trailing marker, so the mapping is recorded here and in the package README
because it is not recoverable from the filenames.
...
interface (\cref{diff:tab:interface}). \\
\bottomrule
\end{longtable}
...
The seven manuscripts state, between them, $13+16+18+13+24+21+13=118$ explicit
non-claims, with heavy overlap. \Cref{diff:sec:nonclaims} consolidates them
into $52$ numbered items, organized by theme; every one of the $118$ has a
home there, and many items carry several sources' phrasings at once. Two of
the $118$ are artifact-level discrepancies in the source packages rather than
mathematical limitations, and they are recorded as such in item~N52.
\end{remark}
...
not re-run them and does not merge their counts into a single suite: seven
suites with seven scopes remain seven suites.

\begin{longtable}{@{}L{0.09\textwidth}L{0.13\textwidth}L{0.70\textwidth}@{}}
\caption{The seven recorded verification runs.}
...
$\Q(\text{variable})$. Check families include the real power rule over $30$
...
examples $33$; matrix identities $35$. \\
\bottomrule
\end{longtable}

All seven runs were recorded under Python 3.13.5, and the six SymPy-dependent
suites under SymPy 1.14.0; the seventh uses the standard library only. All
seven record zero failures. The consolidated scope limits are item~N48 to
item~N52 of \cref{diff:sec:nonclaims}. Two artifact-level discrepancies are
recorded there and repeated here for completeness: one package's PowerShell
build script was supplied but not executed on Windows in its preparation
environment, and one package's README lists a \path{SHA256SUMS.txt} that is
not present among its delivered files.
...
also contains \path{README.md}, the unmodified \path{sources/}, and the
unmodified \path{code/} and \path{data/} directories of the seven source
packages. To re-run a source suite, run it \emph{on a copy}: several of the
delivered programs rewrite their own evidence files beside themselves, so a
