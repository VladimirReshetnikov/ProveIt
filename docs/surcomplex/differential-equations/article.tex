...
classification in \cref{diff:rs:part} agrees with the one here; every stage
$\alpha\geq1$ has coefficients outside $\KR$, where
\cref{diff:rs:prop:boundary} shows that \cref{diff:rs:part} cannot reach
...
These are imported, not proved here: \cite[Proposition~2.5, Lemmas~2.6--2.8
and the pre-derivation formula on p.~10]{ADH}, where the notation is
...
numbering is that of the arXiv version~3 consulted by member~13. For finite
...
monomial-product logarithm identity of \cite[Section~2]{ADH}. At a limit
...
(\cref{diff:cp:rem:derivation}). Derivatives of monomials with non-real
exponents are computed only through the logarithms of the specific monomials
involved, as \cref{diff:warn:monomialmap} requires. The real powers rest on
two facts recorded by member~13 from \cite[Section~2]{ADH}: the logarithm of a
...
first assertion. A nonempty interval of the positive family
$(\omega^{-\beta})$ has a positive sum, so $\varpi$ increases. By
...
Finally $\omega^{-\lambda}<\omega^{-\beta}$.
\end{proof}
...
families are summable (\cref{diff:def:summable}), and so is
$(\ell_{\beta+1})_{\beta<\alpha}$. The double family is the product family of
the first with itself, summable by the Hahn product theorem
\cite[Section~2.4]{BM}. Since $\ell_{<\alpha}=\exp(\sum_{\beta<\alpha}\ell_{\beta+1})$,
...
and $Q_{\lambda}$ is the Hahn sum of its increments, not a sum or limit of the
partial sums $Q_{\beta}$.
\end{remark}
...
and let $y_{1},y_{2}\in F$ solve $\der^{2}y+Qy=0$ with
$\mathrm{Wr}(y_{1},y_{2})\neq0$. Then $\mathrm{Wr}(y_{1},y_{2})\in C_{F}$, and
every solution in $F$ lies in $C_{F}y_{1}+C_{F}y_{2}$.
\end{lemma}
\begin{proof}
$\der\mathrm{Wr}=y_{1}\der^{2}y_{2}-y_{2}\der^{2}y_{1}=0$. For a third solution
$y$ put $a=\mathrm{Wr}(y,y_{2})/\mathrm{Wr}(y_{1},y_{2})$ and
$b=\mathrm{Wr}(y_{1},y)/\mathrm{Wr}(y_{1},y_{2})$; the same calculation gives
$\der a=\der b=0$, and direct substitution gives $y=ay_{1}+by_{2}$.
\end{proof}
...
\end{corollary}
\begin{proof}
Write $\nu=a+ib$ with $b\neq0$. The imaginary coefficient $b\,s^{\dagger}$ has
the accumulated phase $b\log s$, which is infinite: a real bound on $\log s$
would exponentiate to a real bound on $s$. By \cref{diff:thm:phasecriterion},
whose direction (i)$\Rightarrow$(iii) uses no primitive, there is no nonzero
solution. For real $\nu$, $s^{\nu}$ is a solution by \ref{diff:BM:exp}, and the
quotient of any solution by it has derivative zero.
\end{proof}
...
Let $F\subseteq\SC$ be a differential subfield and let $s,u,Q\in F$ with
$s\neq0$, $\der s\neq0$, $u\neq0$, $\der^{2}u+Qu=0$ and $u^{2}\der s=1$. Then for
every $v\in F$ and every $c\in\C$,
...
The identity treats $v$ as an arbitrary element of the field. It does not
assert that every surreal is a composite $v(s)$ and invokes no substitution
...
direct differentiation gives the pair $s^{1/2}$, $s^{1/2}\log s$ with
$\mathrm{Wr}_{s}=1$. If $c>\frac14$ the roots are
$\nu_{\pm}=\frac12\pm i\sqrt{c-\frac14}$, the operator factors as
$(s\der_{s}-\nu_{+})(s\der_{s}-\nu_{-})$ with commuting constant-coefficient
factors, and each factor is injective by \cref{diff:cp:cor:nonreal}. Real
statements follow by intersecting with $\No$.
\end{proof}
...
$s>0$, $s\der_{s}=\der_{\sigma}$ with $\sigma=\log s$ (\cref{diff:prop:rescale}),
and $\sigma$ is positive infinite when $s>\R$. So
...
$\der_{\sigma}$ and $\sigma$ in place of $\der$ and $\omega$: the roots are real
exactly when $c\leq\frac14$, and $e^{\lambda\sigma}=s^{\lambda}$,
$\sigma=\log s$. \Cref{diff:thm:constant} is stated for $\der$ only; its proof
goes through for $\der_{\sigma}$ with \cref{diff:cp:cor:nonreal} in place of
...
it returns the solutions $Ae^{\kappa\omega}+Be^{-\kappa\omega}$ of
$\der^{2}y-\kappa^{2}y=0$, and $A+B\omega$ at $c=\frac14$, as
...
$\sigma=\log s=\omega$, $\ell_{1}$ and $\ell_{\alpha+1}$, and in every
supercritical case the obstruction is the infinite accumulated phase
$\sqrt{c-\frac14}\,\sigma$ of \cref{diff:cp:cor:nonreal}; the constant-potential
...
$c=0$ the last term of \cref{diff:cp:eq:qnormal} is omitted. In the printed
order, $\mathrm{Wr}(y_{+},y_{-})=-\sqrt{1-4c}$ and $\mathrm{Wr}(y_{0},y_{1})=1$.
...
$c>\frac14$ there is no \emph{real} Riccati solution: from one, a real
primitive and its exponential would give a nonzero real solution of
\cref{diff:cp:eq:main}. That implication uses the surjectivity clause
\ref{diff:BM:surj}, equivalently the Liouville closedness of $\No$ \cite{BM};
...
give, and at $c=\frac14$ the construction itself shows that a smaller
perturbation can matter. No comparison theorem for general surreal Riccati
equations is used.
\end{remark}
...
recorded as agreement. For $\alpha\geq1$ the potential involves $\log\omega$
and lies outside $\KR$, beyond the class of \cref{diff:rs:part}
(\cref{diff:rs:prop:boundary,diff:rs:rem:boundary}). For finite $n$ the
