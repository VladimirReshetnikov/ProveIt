...
\hypersetup{%
  ...
  pdfkeywords={surreal numbers, surcomplex numbers, Berarducci--Mantova derivation, H-field, Hahn series, differential Galois theory, regular-singular systems, autonomous differential equations, abelian varieties}}
...
spine of scalar and workspace theory; the eighth, ninth and tenth joined
later and carry the matrix, regular-singular and autonomous strata.
...
marks the boundary: the non-Hermitian case is not classified. That part also
invokes a separately imported linear-splitting and first-order surjectivity
theorem, which is not a consequence of the scalar criterion and is flagged
wherever it is used.

Two later parts classify regular-singular systems $\der y=-t\,Ay$ with an
...
without the import, and first-order autonomous equations with constant
coefficients, for \emph{every} normalized surreal derivation, together with the
logarithmic-derivative image of a constant abelian variety.

A deliberately separate half treats \emph{coordinate} differential equations
in fixed-common-domain Hahn rings $\Ofin(U)((t^{\Gamma}))$: existence,
...
\textbf{Status.} A merged exposition assembled from ten independent
...
\CommitB; and the ninth and tenth, which supply
\cref{diff:aut:part,diff:rs:part}, pinned to \CommitC. Deep structural inputs
...
\subsection{Ten manuscripts, one theory}
\label{diff:sub:seven}

This report is the union of ten independently written manuscripts on the
...
\cref{diff:app:provenance} --- joined last, arrived together, and are pinned to
...
not premises; in \cref{diff:part:matrix} alone they become a premise, because
...
The two latest parts add no new import of that kind. \Cref{diff:rs:part} uses
...
Berarducci--Mantova derivation; it uses surjectivity in two flagged corollaries
only, and it imports the classical local normal forms and Rosenlicht's
...
Strong summability is a statement about supports, not about convergence. It is
strictly stronger than ``valuations increase''. It licenses coefficientwise
rearrangement of a summable family, and nothing else: it does not authorize
exchanging two infinite sums, and \emph{writing} a formal geometric or
...
estimate about numerical rounding. Smallness is a structural hypothesis about
this derivation.
\end{remark}
...
\section{One derivation, and six other operators}
\label{diff:sec:taxonomy}

Almost every apparent paradox in this subject is a type error. The seven
manuscripts between them separate seven operators; the merged report keeps all
seven distinct, with the symbols of \cref{diff:tab:derivatives}. A formula can
...
\begin{table}[htbp]
\centering\small
\caption{The seven operators. Only the first is ``the derivation''.}
...
$\Obs(i)=\omega$ and $\Obs(i/\omega)=\log\omega$ are purely infinite surreals,
...
infinitesimal'', which would make $\der y=i\omega^{-1}y$ solvable. Every one of
the seven manuscripts explicitly refutes exactly that, and the counterexample
...
$\Izero$ (\cref{diff:prop:normprimitive}); its target class of purely infinite
surreals is $\Pcl$; the inverse of $\der|_{\mm}$ is written
...
nonzero infinitesimal is positive and well ordered. These are Hahn sums, not
limits of their partial sums in any topology. Their domain is
...
coefficient: a real coefficient has zero obstruction however large it is, and
an infinitesimal coefficient may have a nonzero one.
...
zero obstruction, while multiplying one by $i$ may not. An implementation must
...
\end{equation}
Restricted to $\No$ it reads
$0\to\Acl\to\No\xrightarrow{\Obs}\Pcl\to 0$.
\end{corollary}
\begin{proof}
...
logarithm tower is used. The tests resemble familiar convergence thresholds for
...
of constants yields no existence conclusion there. No general inhomogeneous
existence theorem is claimed outside the admissible locus $\Phi(q)=0$; see
\cref{diff:sec:nonclaims}. Note also that having a trivial kernel is not the
...
\end{equation}
More generally, for real $\mu$ and $P\in\C[X]$,
...
counterexample that marks the line; \cref{diff:sub:nc:scalarscope} records what
...
eigenvalues when the coefficient is Hermitian.
...
\end{proposition}
\begin{proof}
By \cref{diff:eq:matintertwine} any change between normalizing gauges is a
constant block-diagonal matrix, so it commutes with every $E_{P}$; the
...
The example also agrees with, rather than contradicts, the hedge already
recorded in \cref{diff:sub:nc:open}: a classification of nontriangular systems
by module-theoretic data cannot be replaced by the eigenvalues of an
instantaneous coefficient matrix. \Cref{diff:thm:matspectral} does not do that
either; it replaces them by the eigenvalues \emph{after} applying $\Obs$, and
only under Hermitian symmetry. The non-Hermitian case remains open, and
\cref{diff:ex:matnonnormal} is what marks the line. (Added with member~12: one
non-Hermitian sector is now computed. For the power-Hahn regular-singular
systems of \cref{diff:rs:part}, $\Ph(-tA)=\{(\Imm\lambda_{j})\log t\}$ with
$\lambda_{j}$ the eigenvalues of the ordinary residual matrix
(\cref{diff:rs:prop:phasespectrum}); \cref{diff:ex:matnonnormal} lies outside that
class, and the general non-Hermitian case stays open.)
\end{warning}
...
\emph{not} addition unless the matrices commute.
\end{warning}
...
a summability certificate, not convergence in any topology on $\SC$.
...
since $u<v$ would give $v-u\in\Lambda^{+}\setminus\{0\}$; Dickson's
finite-antichain lemma for $\N^{n}$ --- the special case of
\cref{diff:app:support} in which the alphabet is $\{1,\dots,n\}$ and words are
unordered --- therefore makes the indecomposables finite, and induction on
total degree writes every member of $\Lambda^{+}$ as a sum of them. Finally a
...
coefficient matrix. Higher-rank differential modules remain outside the
classification. All statements are for the one fixed $\der$ of
...
from the residual matrix, and for finite rational supports computes its Hahn
invariants by a finite procedure (\cref{diff:rs:rem:effective}). Everything in
...
previous stage, together with witnesses for the finitely many
factorizations and first-order inhomogeneous equations of
\cref{diff:thm:matsurjectivity} whose coefficients appear there. The union is a
...
sequence $(y_{N})$ in any surreal topology. A numerical substitution such as
...
$F_{n}$, adjoin the images of all of its elements under the listed unary class
...
manuscripts, over five different base fields and by four different proofs.
...
\subsection{Four set-sized bases, and a non-embedding}
...
simplest oscillatory differential extension. The role of $\mathsf T$ or $\mathsf T$ is not
that of an algebraic root missing from $\No[i]$; it is a new differential
transcendental, and no change of normal-form encoding inside the same field can
make it exist. A symbolic constructor for such an extension is mathematically
...
No ordering of $F(\mathsf v)$ extending the given order of $F$ can satisfy
\[
...
becomes one only in combination with the matrix normal form proved later:
...
continuation \emph{for this class only}; the general non-Hermitian case stays
open (\cref{diff:sub:nc:open}).
...
procedure for finite rational supports (\cref{diff:rs:sub:procedure}). That is
...
$\der=-t\,\derT$ preserves $K_{\Gamma}$ exactly when $1\in\Gamma$
(\cref{diff:prop:realgroup}), whereas $\derT$ preserves every $K_{\Gamma}$.
\end{lemma}
\begin{proof}
...
\ref{diff:BM:additive} extends this to Hahn sums; the support is not changed,
...
\cref{diff:sec:laurent} and the two subrings $K_{\Gamma}^{\geq0}$ and
$K_{\Gamma}^{>0}$ of \cref{diff:rs:lem:euler}; in the embedding into $\SC$,
their elements are finite, respectively infinitesimal.
...
\end{itemize}
For a differential subfield $E\subseteq\SC$ and $G\in\GL_{n}(E)$, the
substitution $y=Gz$ carries \cref{diff:rs:eq:system} to
\begin{equation}\label{diff:rs:eq:gauge}
 \derT z=A^{G}z,\qquad A^{G}=G^{-1}AG-G^{-1}\derT G .
\end{equation}
Multiplying by $-t$ shows that this is exactly \cref{diff:eq:matgauge} for the
$\der$-coefficient $-tA$: $E$-gauge equivalence of $\derT$-systems is
$E$-gauge equivalence of the corresponding $\der$-systems. A morphism from a
system $B'$ to a system $A$, of possibly different sizes, is a rectangular $X$
with $\derT X=AX-XB'$; all morphism spaces are $\C$-vector spaces.
\end{convention}
...
Member~12 proves the first two assertions by a second, Higman-free route that
is available only for real exponents: a sequence in $S^{k}$ has a
componentwise nondecreasing subsequence, so infinitely many distinct words of
one length $k$ with one sum would give a nondecreasing sequence of constant
sum, hence a constant one; the length bound leaves finitely many $k$. The same
extraction applied to bounded-length word representations of a strictly
decreasing sequence in $S^{*}$ gives the well-ordering. The length bound uses
the Archimedean property of $\R$ and has no analogue at higher rank
(\cref{diff:warn:summability}).
...
\end{lemma}
\begin{proof}
The union of the words with sums in the finite set $R$ is finite by
\cref{diff:rs:lem:words}. Expanding a recursively defined coefficient into
input matrices labels its terms by ordered words whose sums equal that
coefficient's weight. The expansion terminates: along any branch the number of
input letters is at most the total weight divided by $\min S$, and each step
has only finitely many predecessors. Fixed linear operators change neither the
weight nor the letters.
\end{proof}
...
\end{equation}
where the sum is finite and every coefficient on the right has smaller weight
(\cref{diff:rs:cor:support}). On resonant blocks impose $G_{\gamma}=0$, which
...
same $A^{\natural}$, then $G_{\natural}(G_{\natural}^{\crit})^{-1}$ is a gauge
from $A^{\crit}$ to $A$. The general assertion has the same proof, and if no
Levelt resonance lies in $S^{*}$ every projection in the recursion vanishes.
\end{proof}
...
maps a smaller value to a larger one, so $N$ is block triangular with nilpotent
diagonal blocks, hence nilpotent of index at most $n$. The gauge identity is
the product rule.
\end{proof}
...
nonnegative real constant; if it is positive, the real and imaginary parts
$u,v$ are finite and $\derT u=-bv$, $\derT v=bu$ make them infinitesimal,
which is impossible. Equivalently $\der z=i(-bt)z$, whose accumulated phase
...
\end{equation}
The operator on the right is nilpotent, so unless $\gamma-i(b-c)=0$ a finite
geometric inverse forces $X_{\gamma}=0$; since $\gamma$ and $b-c$ are real this
happens only for $\gamma=0$ and $b=c$, and the surviving coefficient satisfies
\cref{diff:rs:eq:intertwine}. Conversely every such constant matrix is a
morphism. An invertible morphism is invertible on each frequency space, where
\cref{diff:rs:eq:intertwine} makes the nilpotent matrices similar; similarity of
nilpotent complex matrices is determined by Jordan block sizes. Transport
through the matrices $F$.
\end{proof}
...
$\C[X]/(X^{q})\to\C[X]/(X^{p})$, determined by an image annihilated by $X^{q}$,
a space of dimension $\min(p,q)$. Add over block pairs.
\end{proof}
...
zero and equal ones give arbitrary complex constants. Both $G_{\tau}$ and its
inverse have entries in $\KR[\tau]$.
\end{proof}
...
The normal forms also make tensor constructions explicit. If $A$ and $A'$ have
...
eigenvalues. It answers item~N24 and the open continuation ``extend the phase
spectrum to non-Hermitian coefficients'' for this class and for no other.
...
reduces under $\st$ to the characteristic polynomial of $A_{0}$; so the
eigenvalues $\mu_{j}$ of $A$ lie in $\KR^{\geq0}$ and, after renumbering,
$\mu_{j}-\lambda_{j}\in\KR^{>0}$. The eigenvalues of $-tA$ are $-t\mu_{j}$, and
...
extend the recipe beyond the class, and the general non-Hermitian case remains
open.
\end{remark}
...
$t^{0}$-coefficient of $\derT f$ vanishes by \cref{diff:rs:eq:eulerhahn}. If
$\tau$ were algebraic over $\KR$, the finite extension $L=\KR(\tau)$ would be
separable, the derivation would extend uniquely to it, and differentiation
would commute with the field trace (write the trace as a sum over the distinct
$\KR$-embeddings into an algebraic closure and use uniqueness of the
extension). Then $\derT\bigl(\operatorname{Tr}_{L/\KR}(\tau)/[L:\KR]\bigr)=1$ with
the argument in $\KR$, a contradiction.
\end{proof}
...
$E=\exp(\tau N)$ recovers $\tau$ from a nonzero entry of $N$; the reverse
inclusion is the fundamental-matrix formula. (iii) As a differential subfield of
...
$N_{0}=0$). If $V_{0}=0$ there is a unique solution in $\KR^{n}$, and it is also
...
\end{equation}
with support inside $\supp g_{b}$, so these coefficients form a Hahn vector. At
...
\end{equation}
for which $(\derT-N_{0})p=g_{0,0}$ by cancellation; adding it to the nonresonant
Hahn solution and multiplying by $F$ gives a particular solution in
$\KR[\tau]^{n}$. If $V_{0}=0$ nothing is missing, and the difference of two
ambient solutions is an ambient homogeneous solution, which is zero by
\cref{diff:rs:thm:selection}.
\end{proof}
...
the coefficients of $h$ grow without bound; neither is an obstruction. The only
word in $S$ reaching the resonance $1$ is $(\tfrac12,\tfrac12)$, whose matrix
product $E_{12}^{2}$ vanishes, so $S_{\crit}=\{\tfrac12\}$ is a valid finite
dependence set while the minimal one here is empty.
\end{example}
...
and it is printed only there. Together with \cref{diff:ex:sqrt2}, where $\der$
...
being infinitesimal; a nonzero infinitesimal of $\KR$ has a least positive
exponent $a_{0}$ and leading size a nonzero real multiple of $t^{a_{0}}$, so it
cannot be $1/\log\omega$.
\end{proof}
...
an open continuation in \cref{diff:sub:nc:open}.
\end{remark}

\subsection{An exact finite procedure, and what it does not compute}
\label{diff:rs:sub:procedure}

For finite rational support the proofs are directly executable. Fix $A_{0}^{\circ}$
and a finite rational $S$; construct $\Lev(A_{0})$ and enumerate the monoid
exponents up to $\max\Lev(A_{0})$ when there are positive resonances. There are
finitely many such exponents, since after taking a common denominator they lie
on a lattice in a bounded interval. Apply
\cref{diff:rs:eq:recursion,diff:rs:eq:explicit} through that range, record the
nonzero $A^{\natural}_{r}$, perform the shear and compute the Jordan forms of the
$N_{b}$. This finite calculation determines all the $\KR$-gauge invariants; the
ambient invariants need only the residual eigenvalues. The record shrinks if one
first computes the words reaching $\Lev(A_{0})$ and keeps only their letters and
partial sums: the certificate is then the critical input coefficients, the
finite homological inverses at their partial weights, the resulting
$A^{\natural}_{r}$, and the nilpotent Jordan data, each checked by a finite
identity.
...
recursion. For an arbitrary well-ordered support supplied only indirectly the
...
for finite rational supports with exact coefficients the procedure above
...
Minimal logarithm (\cref{diff:rs:thm:minlog}) & Trace and derivation; finite
matrix logarithm; constants & Algebraic closedness of the Hahn field \\
\bottomrule
\end{tabularx}
\end{table}
...
is flagged. Two statements are flagged: the abelian finite-primitive criterion
(\cref{diff:aut:cor:abeliancriterion}) assumes that $\Dn$ is surjective, and
the speed threshold (\cref{diff:aut:cor:speeds}) is stated for $\Dn=\der$. The
...
\end{equation}
Formal inverse maps with invertible linear part evaluate to mutually inverse
maps on the corresponding infinitesimal tuples.
\end{lemma}
\begin{proof}
Discard zero coordinates. The finite union of the supports of the
$\varepsilon_{j}$ is a well-ordered set of positive exponents in the convention of
\cref{diff:eq:tmonomial}, so \cref{diff:lem:neumann} makes the family
$\varepsilon^{\nu}$, $\nu\in\N^{r}$, summable, with finitely many contributions to
each monomial even after full expansion; ordinary complex coefficients do not
change supports. The same finiteness proves multiplicativity and permits the
regrouping in a zero-constant composition. Strong additivity and the Leibniz
rule give $\Dn(\varepsilon^{\nu})=\sum_{j}\nu_{j}\varepsilon^{\nu-e_{j}}\Dn\varepsilon_{j}$;
each derivative family is again a valid evaluation multiplied by one fixed
element, and there are finitely many $j$. The last assertion follows by
evaluating the two formal composition identities.
\end{proof}
...
$u,v\in\mm_{\C}$, formal subtraction and division by $W-W'$ give
$\Theta(\varepsilon,u)-\Theta(\varepsilon,v)=(u-v)(a+\varepsilon')$ with
$\varepsilon'\in\mm_{\C}$, and the second factor is a unit. The argument also
...
define a global surcomplex exponential.
...
$\Dn\mathbf y=\xi(\mathbf y)$. A nonconstant point of a curve over the
algebraically closed constant field is generic over it, so rational functions
cause no evaluation ambiguity; constant points are treated separately. This
...
\begin{equation}\label{diff:aut:eq:localfield}
 ...
 \qquad c_{k}\neq0,\quad k\in\Z .
\end{equation}
At a simple zero ($k=1$) the coefficient $\lambda=c_{1}$ does not depend on the
local parameter.
...
defined, and morphisms commute with it by evaluating regular coordinate
expressions in the finite ring. This is the usual valuative reading of
properness \cite[Tag~0BX4]{Stacks}. At a smooth point of a curve, shrink to a
standard smooth affine presentation with an invertible Jacobian minor and a
coordinate $\sigma$ with nonzero differential; the completed local ring is
$\C[[\sigma]]$, the other coordinates are formal implicit functions of $\sigma$
with ordinary constant terms, and \cref{diff:aut:lem:evaluation} evaluates them at
every $\varepsilon\in\mm_{\C}$, giving a point reducing to $p$. Conversely, if two
coordinate tuples with the same $\sigma$ and the same reduction solve the affine
equations, the linear system for their differences has as matrix the ordinary
Jacobian minor plus an infinitesimal matrix, of determinant with nonzero
residue, so the differences vanish; no completeness of an ambient valuation
topology is used. A regular denominator nonvanishing at $p$ remains a unit after
evaluation, so the point lies in the chosen chart.
\end{proof}
...
\end{lemma}
\begin{proof}
Put $u=\sigma(\mathbf y)$; since $\mathbf y$ is nonconstant $u\neq0$, and reduction
gives $u\in\mm_{\C}$, so $\Dn u\in\mm_{\C}$ by \cref{diff:aut:lem:small}. The local
...
\end{proposition}
\begin{proof}
$\lambda/f(\sigma)-1/\sigma$ lies in $\C[[\sigma]]$; integrate it with zero
constant term and put
$h_{p}(\sigma)=\sigma\exp\bigl(\int_{0}^{\sigma}(\lambda/f(s)-1/s)\,ds\bigr)$, a
formal exponential of a zero-constant series. Logarithmic differentiation proves
the identity, and conversely the identity determines $h_{p}/\sigma$ from its
logarithmic derivative and constant term $1$. Its formal inverse exists. For a
...
The formal linearization is classical \cite[Lemma~6.1]{NPT}. The exclusion of
...
\end{equation}
because the constant field is $\C$.
...
\end{lemma}
\begin{proof}
Every term of \cref{diff:aut:eq:time} other than $c_{\ast}u^{-m}$ is negligible
relative to $u^{-m}$: this is immediate for the other negative powers and for
$g(u)$, and it is \cref{diff:aut:lem:log} for $\Log u$. So
\cref{diff:aut:eq:timeconstant} gives $c_{\ast}u^{-m}\sim\omega$, that is
$(u/x_{m})^{m}\sim c_{\ast}$ with $x_{m}=\omega^{-1/m}$. Taking moduli shows that
$u/x_{m}$ is a finite unit; its standard part $\beta$ satisfies $\beta^{m}=c_{\ast}$
and is unique.
\end{proof}
...
\end{proposition}
\begin{proof}
With $W=1+U$ the right side of \cref{diff:aut:eq:implicit} is a formal series in
$(X,Y,U)$ with zero constant term and $U$-coefficient $-m\neq0$ at the origin;
\cref{diff:aut:lem:implicit} constructs the unique formal solution and proves
unique evaluated solvability in the monad of $1$. Evaluation is legitimate by
\cref{diff:aut:lem:evaluation}, reversing the algebra gives
$\mathfrak t(u_{\beta,C})=\omega+C$, and differentiation gives $\Dn u_{\beta,C}=f(u_{\beta,C})$;
the leading term $\beta x_{m}$ makes the solution nonzero. Conversely
\cref{diff:aut:lem:leading} fixes $\beta$ for a given solution,
\cref{diff:aut:eq:timeconstant} fixes $C$, and $u/(\beta x_{m})$ solves the same
evaluated implicit equation in $1+\mm_{\C}$, so the uniqueness part of
\cref{diff:aut:lem:implicit} gives equality. Different $\beta$ have different
leading coefficients and different $C$ different values of $\mathfrak t$.
\end{proof}
...
A change of local parameter transports the solutions by an invertible ordinary
formal series, and a change of $\log\beta$ adds an ordinary constant to
$\mathfrak t$ and translates $C$. The parametrizations are therefore not
canonical; the number of branches, the family of points and the existence
criterion are. The leading coefficient $\beta$ is coordinate-dependent at a
multiple zero, and no invariance of it is asserted.
\end{remark}
...
$g$. A nonzero Laurent series with ordinary coefficients evaluated at a nonzero
infinitesimal $u$ is $a u^{r}(1+\varepsilon)$ with $a\neq0$ and cannot vanish, so
the construction gives a field embedding and not only an assignment of
coordinates. A constant point has zero derivative and solves the equation
...
monomials --- $\omega$ is log-atomic and exponentials of purely infinite surreals
are monomials \cite[\S\S3,5]{BM} --- so $\mathfrak G_{\xi}$ is a finitely
...
invert a unit $1+\varepsilon$ by its geometric series (\cref{diff:lem:neumann}).
$\mathfrak G_{\xi}$ is countable, so there are at most $\abs\C^{\aleph_{0}}=2^{\aleph_{0}}$
series, and the constants give the reverse inequality. For a monomial
$\mathfrak g=\omega^{a/M}(\log\omega)^{b}\prod_{j}e^{n_{j}\lambda_{j}\omega}$ with
$a,b,n_{j}\in\Z$, exponential compatibility, the product rule and $\Dn\omega=1$
give the \emph{three-shift formula}
...
\end{equation}
which does not depend on the representation of $\mathfrak g$, being obtained by
differentiating an identity in the field. The supports of the derivative terms
lie in three fixed multiplicative translates of the original support, each
output monomial receives finitely many contributions, and strong additivity
gives $\Dn K_{\xi}\subseteq K_{\xi}$ with a restriction independent of $\Dn$. At a
multiple zero $\omega^{-1/m_{j}}$ and $(\log\omega)/\omega$ lie in $\mathfrak G_{\xi}$
and the support of \cref{diff:aut:eq:master} lies in the group they generate; at a
stable simple zero the one-variable expression in $e^{\lambda_{j}\omega}$ does the
same; and every local algebraic coordinate is an ordinary formal Laurent series
in the parameter, so its evaluation stays in $K_{\xi}$.
\end{proof}
...
\end{corollary}
\begin{proof}
At $u=0$ the vector field has a double zero with leading coefficient $-1$, and
\cref{diff:aut:prop:multiple} gives a distinct solution $u_{C}$ for every $C\in\C$
using only the two stated monomials. A classical theorem originating with
Rosenlicht states that distinct nonconstant solutions of this equation are
algebraically independent over the constant field; it is imported from
\cite[Lemma~7.2 and Proposition~7.3]{NPT} and applied to every finite
subfamily, inside the set-sized differential field containing those solutions
and $\omega$, so no proper-class version is needed.
\end{proof}
...
projective $\mathcal X/\C$ and nonzero meromorphic $\xi$, the differential
...
\end{theorem}
\begin{proof}
For curves, \cref{diff:aut:thm:main} gives formulas that do not involve the
choice of derivation --- their ingredients are ordinary coefficients, $\omega$,
$\log\omega$ and $e^{\lambda\omega}$ --- or alternatively
\cref{diff:aut:thm:scales} places every solution in $K_{\xi}$, where the two
derivations agree. For the scalar assertion the case $F=0$ is immediate, so let
$F\neq0$; a nonzero constant has no solutions. Factor $F$ over $\C$; at a solution
some irreducible factor $R$ vanishes. If $y\in\C$ both derivatives vanish. If $R$
involves only $Y$ it forces $y\in\C$. If it involves only $Z$ then $R=Z-c$ up to
a scalar, and $\Dn_{1}y=c$ gives $y=c\omega+d$ with $d\in\C$, on which $\Dn_{2}$
agrees. Otherwise $y$ is nonconstant and $R$ involves both variables;
$Y\mapsto y$, $Z\mapsto\Dn_{1}y$ embeds the function field of the affine curve
$R=0$, since a larger kernel would give an image of transcendence degree zero,
contrary to $y\notin\C$. On its smooth projective model put
\begin{equation}\label{diff:aut:eq:inducedfield}
 \xi Y=Z,\qquad \xi Z=-\frac{R_{Y}}{R_{Z}}\,Z,
\end{equation}
the unique derivation extending $\xi Y=Z$ (in characteristic zero $R_{Z}\neq0$ in
the function field), nonzero because $Z=0$ would make $R$ a multiple of $Z$.
Differentiating $R(y,\Dn_{1}y)=0$ shows that the embedding is differential for
$\Dn_{1}$; the curve assertion makes it differential for $\Dn_{2}$, so
$\Dn_{2}y=\Dn_{1}y$ and $F(y,\Dn_{2}y)=0$. Interchange the derivations.
\end{proof}
...
\end{corollary}
\begin{proof}
Use the field of \cref{diff:aut:thm:scales} for the curve and vector field of
\cref{diff:aut:eq:inducedfield} attached to each of the finitely many irreducible
factors
involving both variables, and enlarge the monomial group to the group generated
by these groups and by $\omega,\log\omega$; factors in $Y$ alone contribute
constants and factors $Z-c$ contribute $c\omega+d$. The three-shift argument and
the countability argument apply unchanged.
\end{proof}
...
zero, real nonconstant solutions exist exactly when $\lambda<0$, with $c\in\R^{\times}$.
\end{corollary}
\begin{proof}
A real solution has real leading coefficient and real time constant in the real
time primitive; conversely uniqueness of the formal implicit solution over $\R$
keeps the construction real. The roots of $\beta^{m}=-1/(mc)$ have the stated
count, and the simple-zero case follows from the real linearizer and
\cref{diff:aut:lem:eigen}.
\end{proof}

The sign restrictions reflect the orientation $\Dn\omega=1$, not a choice of
complex branch cut (compare \cref{diff:rs:warn:orientation}); using
$\log\abs\sigma$ avoids an imaginary time shift for a negative real solution.
...
$u=c\,e^{-\omega}/(1+c\,e^{-\omega})$ with $c\in\C^{\times}$, as direct
differentiation confirms; the constants $0$ and $1$ are not both obtained by an
ordinary value of $c$.
\end{example}
...
here $m=1$, $\beta=1$, $\varrho=1$. With $X=1/\omega$ and $Y=(\log\omega)/\omega$
the implicit equation is
\[
 W^{-1}+X\Lmm(W)-Y-X\Lmm(1+XW)-CX-1=0,
\]
and with $a=Y+CX$ its first homogeneous terms are
$W=1-a+(a^{2}-Xa-X^{2})+O((X,Y)^{3})$. Thus, with $L=\log\omega$,
\[
 u_{C}=\frac1\omega-\frac{L+C}{\omega^{2}}+\frac{(L+C)^{2}-(L+C)-1}{\omega^{3}}
 +\cdots,
\]
the remainder being the evaluation of a series in $X(X,Y)^{3}\C[[X,Y]]$ --- a
more precise statement than a fine-topological big-$O$. The formula describes all
solutions reducing to $0$. The same equation has a simple zero at $-1$ with
eigenvalue $-1$ and hence also the exponential family of
\cref{diff:aut:thm:main}(ii).
\end{example}
...
\end{proposition}
\begin{proof}
Reduce $\mathbf y$ to $p$ and choose an affine neighbourhood with regular
coordinates; its nonvanishing denominators evaluate to finite units. The
coordinates of $\mathbf y$ are finite, their derivatives infinitesimal
(\cref{diff:aut:lem:small}), and the coefficients of $\varphi$ at $\mathbf y$
finite, so the contraction is a finite sum of products of the two kinds. It is
intrinsic, hence independent of the chart.
\end{proof}
...
Equivalently $\xi$ has no zeros: it is nonzero where $\varphi$ is and has poles at
the zeros of $\varphi$; this agrees with \cref{diff:aut:thm:main}.
...
\end{theorem}
\begin{proof}
These constants solve the equation. Suppose $y$ is nonconstant and put $z=\Dn y$;
then $z\neq0$, and $z^{2}=Q(y)$ defines a nonconstant point on the smooth
projective model of that affine curve. The differential $\varphi=dY/Z$ is
holomorphic there: at a simple root $a$ of $Q$ use $Z$ as parameter, so
$Y-a=Z^{2}/Q'(a)+O(Z^{4})$ and $dY/Z$ is regular; squarefreeness leaves no other
finite singularity. At infinity write $\deg Q=2\mathrm g+1$ or $2\mathrm g+2$ with
$\mathrm g\geq1$: in the odd case a local parameter has $Y\sim\sigma^{-2}$ and
$Z\sim c\,\sigma^{-(2\mathrm g+1)}$, so $dY/Z$ is a unit multiple of
$\sigma^{2\mathrm g-2}d\sigma$; in the even case, at either point above infinity,
$Y=\sigma^{-1}$ and $Z\sim c\,\sigma^{-(\mathrm g+1)}$, so it is a unit multiple of
$\sigma^{\mathrm g-1}d\sigma$. Contraction with the derivative at $(y,z)$ is
$\Dn y/z=1$, which \cref{diff:aut:prop:contraction} makes infinitesimal.
\end{proof}
...
$y=(e^{\omega}+e^{-\omega})/2$ satisfies $(\Dn y)^{2}=y^{2}-1$. Neither hypothesis
of \cref{diff:aut:thm:hyperelliptic} can simply be deleted.
\end{example}
...
and tangent spaces is standard \cite{Milne}; the characteristic-zero logarithm
fact is proved here.
...
\end{lemma}
\begin{proof}
Translate a basis of the cotangent space at the identity to invariant formal
$1$-forms $\varphi_{1},\dots,\varphi_{d}$, whose coefficient matrix has constant term
the identity. Commutativity makes them closed: for invariant tangent fields $v,w$
the formula for $d\varphi_{j}$ reduces to $-\varphi_{j}([v,w])=0$, and these fields
form a basis. A closed formal $1$-form in characteristic zero has a unique
primitive with prescribed constant term --- for a homogeneous coefficient form of
degree $n$, contract with the Euler vector field and divide by $n+1$ --- and
integrating the $\varphi_{j}$ with zero constant term gives $\log_{\mathcal F}$.
Invariance gives
$d\bigl(\log_{\mathcal F}(\mathcal F(T,S))-\log_{\mathcal F}(T)-\log_{\mathcal F}(S)\bigr)=0$,
and the difference has zero constant term. Invertibility follows from the
identity linear part. For uniqueness, compose another such map with
$\exp_{\mathcal F}$: the result is additive with identity linear part, and each
homogeneous term $H_{n}$ of degree $n>1$ vanishes because $H_{n}(2T)=2H_{n}(T)$ and
$2^{n}-2\neq0$.
\end{proof}

By the smooth-coordinate argument of \cref{diff:aut:lem:projective}, now in $d$
variables, the formal completion parametrizes all of $\mathbf A_{1}(\SC)$, and
\cref{diff:aut:lem:evaluation} evaluates $\log_{\mathcal F}$ and $\exp_{\mathcal F}$ on
every infinitesimal tuple as inverse group isomorphisms.
...
\end{proposition}
\begin{proof}
The constant section and reduction split $\mathbf A(\SC)$ into the constant
subgroup and the identity monad; apply the evaluated formal logarithm to the
latter. All group and inverse identities are evaluated formal identities.
\end{proof}
...
\end{theorem}
\begin{proof}
Translating $\mathbf y$ by the constant point $-\st(\mathbf y)$ does not change its
logarithmic derivative and puts it in the identity monad; the differential of
$\log_{\mathcal F}$ is the invariant differential matrix, and the chain rule
(\cref{diff:aut:lem:evaluation}) gives the first identity. As $\mathbf y$ ranges over
$\mathbf A(\SC)$ its logarithm coordinate ranges over all of $\mm_{\C}^{d}$
(\cref{diff:aut:prop:abeliansplit}), so the derivative ranges over
$(\Dn\mm_{\C})^{d}$ and vanishes exactly when that infinitesimal coordinate
vanishes. The splitting is a homomorphism because $(\Dn|_{\mm_{\C}})^{-1}$ and
$\exp_{\mathcal F}$ are, and composing it with $\dlog_{\Dn}$ gives the identity.
\end{proof}
...
For the rest of this subsection assume $\Dn$ is surjective, as $\der$ is. By
...
\end{corollary}
\begin{proof}
The obstruction vanishes exactly when a primitive is finite, and subtracting its
standard part then gives the unique infinitesimal primitive; this is
$\Ker\Obs^{\C}_{\Dn}=\Dn\mm_{\C}$, and \cref{diff:aut:thm:abelianimage} finishes.
A change of ordinary Lie basis is a constant invertible matrix and commutes with
$\Obs^{\C}_{\Dn}$. \Cref{diff:aut:eq:abeliansolutions} is the splitting of
\cref{diff:aut:eq:abelianexact} translated by the kernel.
\end{proof}
...
infinite primitive would not be a valid substitute. Surjectivity is used only
here and in the next corollary.

\begin{corollary}[Sharp one-power speed threshold; $\Dn=\der$]
\label{diff:aut:cor:speeds}
Fix $0\neq v\in\Lie(\mathbf A)(\C)$ and $p\in\R$. Under the Berarducci--Mantova
derivation, $\dlog_{\der}\mathbf y=\omega^{-p}v$ is solvable exactly when $p>1$; in
particular no nonzero constant speed and no speed $\omega^{-1}v$ is realizable.
\end{corollary}
\begin{proof}
A primitive is $\omega^{1-p}v/(1-p)$ for $p\neq1$ and $(\log\omega)v$ for $p=1$;
the first is infinitesimal exactly when $p>1$, and otherwise, like the second, has
a nonzero purely infinite component in some coordinate, which no constant vector
removes. Apply \cref{diff:aut:cor:abeliancriterion}.
\end{proof}
...
For a presented smooth constant curve and meromorphic vector field,
\cref{diff:aut:thm:main} reduces the existence of a nonconstant surcomplex
trajectory to a finite test: compute the zeros of $\xi$, their orders and the
linear coefficients at simple zeros. A multiple zero always contributes
solutions, and a simple zero does so exactly when its coefficient is a negative
real number; the formulas then construct all solutions as exact Hahn
evaluations. This is an effective procedure when the coefficient field has exact
algebraic operations and a decidable test for the relevant real signs, for
example with algebraic complex coefficients. It is not an unconditional
computability claim about arbitrary unencoded complex constants, and producing
an entire formal solution as a finite string requires a representation by its
implicit equation and coefficient recursion, not a list of all coefficients. For
the class it covers it lifts item~N25; beyond the class N25 stands.
\end{remark}
...
finitely many input series and the global sequence is stated pointwise. Member~11
proposes a formalization in four interfaces: a valued differential field with a
residue section, small derivative on its valuation ring and an element of
derivative $1$; a finite-variable strong formal-evaluation interface with
composition and the chain rule; local smooth algebraic geometry identifying the
residue monad with infinitesimal formal coordinates; and the one-variable time
primitive with the constant-field argument --- with commutative formal groups and
invariant differentials for the abelian extension. A proof of a Hahn identity is
not a proof that every surcomplex point admits local coordinates, and a residue
reduction does not justify summing formal series; the two bridges are
\cref{diff:aut:lem:evaluation,diff:aut:lem:projective}. No Lean file is included,
and the repository is not claimed to contain these bridges
...
and not an answer. That report makes no assertion about the Berarducci--Mantova
derivation, and this part is the collection's first contact between surreal
derivations and abelian varieties. The ``autonomous'' flows here are solutions of
autonomous differential equations; they are unrelated to the embedding of
near-identity maps into formal flows in the companion report on dynamics and
normal forms.
\end{remark}
...
construction in this part. Let $U\subseteq\C$ be an ordinary \emph{connected}
...
matrix $\Upsilon$ on $U$ with $\Phi'=A\Phi$, $\Phi(z_{0})=I_{d}$; simple
...
Analytic continuation of the normalized fundamental matrix around an ordinary
based loop $\ell$ yields another fundamental matrix $X_{\ell}$, and their ratio
...
\end{equation}
defines a $\C$-linear derivation of the common-domain Hahn algebra. Every
...
dropped: for $a\in\SC$ and $F(Z)=1/(Z-a)$,
\begin{equation}\label{diff:eq:movingpole}
 \dercoef F=\frac{\der a}{(Z-a)^{2}},\qquad
...
\midrule
Workspace solution &
Rational exponent $r$, a finite $k$ with positive support and
$-rt+\der k=q$ &
...
with verified $\pp(B)=0$, and an admissible infinitesimal-residue expression &
...
\begin{warning}[No formalization is delivered]
\label{diff:warn:noformalization}
No Lean source and no machine-checked formalization is included or asserted to
exist, here or in the repository or in a library. The preceding paragraphs
specify proof obligations and a possible order of development, nothing more.
This report also does not claim that the concrete surreal carrier has already
been connected to every required Hahn, real-closed-field, exponential or
...
at least one of the ten source manuscripts, or is an artifact-level finding of
...
eighth, N65--N79 from member~12 and N80--N94 from member~11; where a later
...
\item \textbf{The interface table describes nothing that exists.}  One source
introduces its certificate and interface contract with the explicit disclaimer
that these are \emph{proposed} interfaces, not claims about functions already
implemented in this repository's symbolic packages.  That applies verbatim to
\cref{diff:tab:interface}: nothing in it is implemented anywhere in the
collection, and its prescriptive voice is a proposal throughout.  The
companion computer-algebra report holds the existing capability taxonomy, and
it implements none of this either.
...
made about surjectivity of all linear differential operators on $\SC$: a
...
limits are items N65--N79.
\item The Hermitian case of the nontriangular matrix problem is settled; the
non-Hermitian case is not. \Cref{diff:thm:matnormal} gives a complete phase
normal form for every finite system, and \cref{diff:thm:matspectral} computes
its invariant from ordinary algebraic eigenvalues \emph{when the coefficient is
$iH$ with $H$ Hermitian}. Outside that hypothesis no spectral formula is
claimed, and \cref{diff:ex:matnonnormal} refutes the obvious candidate: a
...
\cref{diff:thm:matsurjectivity}. Higher-rank differential modules remain
unclassified, and so does the question of which nontriangular differential
modules admit a filtration whose rank-one factors carry computable
obstructions, which needs genuine module-theoretic information rather than the
eigenvalues of an instantaneous coefficient matrix
(\cref{diff:warn:matnonnormal,diff:warn:matlimits}). \emph{Amended with
member~12:} one non-Hermitian sector is computed. For power-Hahn
regular-singular systems, $\Ph(-tA)=\{(\Imm\lambda_{j})\log t\}$ with the
$\lambda_{j}$ the eigenvalues of the ordinary residual matrix, the normalizing
gauge is explicit and lies in $\GL_{n}(\KR[\log t])$, and no import is used
(\cref{diff:rs:prop:phasespectrum}); within that class the module splits into
rank-one factors with obstructions read off from the residual matrix
(\cref{diff:rs:rem:effective}). \Cref{diff:ex:matnonnormal} lies outside the
class. Outside it the item stands: the general non-Hermitian case is not
classified, and the general filtration question is open.
...
order, not convergence of the truncation sequence in any surreal topology, and
...
\item Higher rank is open here. The obstruction interacts with invariant
subspaces and changes of basis in ways this report does not determine.
\end{enumerate}
...
items, N94 and N78--N79; every limitation stated in the rest of this item applies
...
and exact coefficients, \cref{diff:rs:sub:procedure} computes the Hahn invariants
...
support-certified formal sum, not a limit in any topology on $\SC$. This is
...
mathematical statement, but it is the reason no count in this report is
...
used only for $\Obs^{\C}_{\Dn}$, in \cref{diff:aut:cor:abeliancriterion,diff:aut:cor:speeds},
and a standard part is taken only after finiteness of the primitive has been
established.
\item \emph{Effectivity.} The finite geometric decision criterion is an effective
procedure only when the coefficient field has exact algebraic operations and
decidable sign tests, for example algebraic complex coefficients. It is not an
...
review and no Lean verification is claimed; no Lean file is supplied, and the
repository is not claimed to contain the evaluation and reduction bridges in the
generality needed. No named published open problem is claimed solved and no
...
\cref{diff:part:workspaces}, and \cref{diff:cor:matperturbation} shows that the
former is stable under every entrywise perturbation by $\Acl_{\C}$.
...
removing it. Higher-rank differential modules likewise remain unclassified.
...
Three of the remaining continuations are touched by the two latest parts: two
are answered \emph{in part}, and one only has an adjacent result. They are
re-scoped here rather than deleted.
\begin{itemize}[leftmargin=1.6em]
\item \emph{Extend the phase spectrum to non-Hermitian coefficients, or show that
no spectral formula can exist there,} the obstruction being that a nonnormal
...
(\cref{diff:rs:prop:phasespectrum}, member~12). Open for every coefficient outside
that class, and \cref{diff:ex:matnonnormal} still marks the general line.
...
finite rational supports of power-Hahn regular-singular systems
...
The two latest members add continuations of their own. From member~12: a
...
\cref{diff:warn:collapse} insists on, and an additional imported input; and it
needs Hermitian symmetry, which \cref{diff:ex:matnonnormal} shows cannot be
dropped.
...
This report merges ten source manuscripts. The manuscripts themselves are
...
members 11 and 12 joined last, from the archives
...
differential equations, not iteration of maps.
...
each, by this merge's count, giving $160$.
\Cref{diff:sec:nonclaims} consolidates them into $94$ numbered items,
organized by theme; every one of the $160$ has a home there, and many items
...
packages of members~12 and~11 are items N79 and N94.
...
of \cref{diff:thm:matsurjectivity}, and item~N24 now records the Hermitian case
settled and the non-Hermitian case open, keeping the counterexample that marks
the line. Nothing is strengthened beyond what a source proves.
\end{remark}
...
not re-run them and does not merge their counts into a single suite: ten
suites with ten scopes remain ten suites.

\begin{longtable}{@{}L{0.09\textwidth}L{0.13\textwidth}L{0.70\textwidth}@{}}
\caption{The ten recorded verification runs.}
...
Python~3, and its program requires Python 3.10 or later. All ten record zero
failures. The consolidated scope limits are item~N48 to item~N52, item~N63 to
item~N64, items N78--N79 and item~N94 of \cref{diff:sec:nonclaims}.
...
delivered text record; neither of the last two packages carries a checksum
manifest; and member~12's record labels the mixed-frequency ambient dimension
$1$ without computing it.
...
\path{data/} directories of the ten source packages. To re-run a source
...
directory, and the build scripts of members~11 and~12 redirect output onto their
recorded transcripts, still under the packages' original file names; so a
...
Preprint: \href{https://arxiv.org/abs/1503.00315}{arXiv:1503.00315v3}.
...
Preprint: \href{https://arxiv.org/abs/1512.02267}{arXiv:1512.02267v3}.
...
Preprint: \href{https://arxiv.org/abs/1904.08152}{arXiv:1904.08152}. The
...
\url{https://stacks.math.columbia.edu/tag/0BX4}.
