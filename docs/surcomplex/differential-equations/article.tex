...
More explicitly, dividing the remainder after that leading term
by $(\ell_{\beta}^{\dagger})^{2}$ shifts its entire Hahn support
strictly below the monomial $1$. The remainder is therefore
infinitesimal, including at limit $\alpha$; this uses the
well-ordered support of the whole sum, not a limit of finite tails.
For the second assertion, the common initial sum cancels exactly,
and $(\ell_{\alpha}^{\dagger}/\ell_{\beta}^{\dagger})^{2}$ is
infinitesimal. Multiplication by the nonzero ordinary constant
$c-d$ preserves that property.
\end{proof}
...
\begin{proof}
Take the ordinal
$\alpha=\sup\{\beta+1:\beta\in\mathcal B\}+1$.
This is a set ordinal, is at least $1$, and exceeds every member
of $\mathcal B$; for the empty set it gives $\alpha=1$.
Now take $Q_{-}=Q_{\alpha,0}=Q_{\alpha}$
...
$c=\frac14$. The exact Hahn sum in \cref{diff:cp:eq:Qomega}
does determine $Q_{\omega}$ uniquely. What fails to determine
solvability is prescribing its coefficients at all the displayed
finite logarithmic scales while allowing a remainder smaller
than every one of them. The two potentials agree in that data,
but differ at the additional monomial
$(\ell_{\omega}^{\dagger})^{2}$ in their full normal forms.
\end{example}
...
For a fixed set $\mathcal B$, put
\[
 \mathcal E_{\mathcal B}
 =\{e\in\No:e\prec(\ell_{\beta}^{\dagger})^{2}
                      \text{ for every }\beta\in\mathcal B\}.
\]
It is closed under addition and multiplication by ordinary real
constants, since each normalized error is infinitesimal.
The theorem gives $Q_{+}-Q_{-}\in\mathcal E_{\mathcal B}$, so
solvability does not descend to these additive equivalence classes.
For nonempty $\mathcal B$, this error class is nonzero and proper:
the constructed difference belongs to it, whereas $1$ does not.
It is therefore not an ideal of the surreal field, since
multiplying a nonzero member by its inverse would give $1$.
For empty $\mathcal B$ it is all of $\No$, as the vacuous condition
requires. No discontinuity in the fine surreal topology is asserted
...
arbitrary well-ordered \emph{outer} support of $t$-exponents in
that group (equivalently reverse well-ordered Conway exponents). The
...
\end{lemma}
\begin{proof}
Write $\gamma=\sum_{j=1}^{m}r_j\varpi_{\beta_j}$ with
$\beta_j<\lambda$. For a nonempty list choose
$\beta=\max_j\beta_j<\lambda$; for the empty list take $\beta=0$.
Every summand, and therefore their sum after any cancellations,
has zero coefficient at $\omega^{-\zeta}$ for $\zeta\geq\beta$.
This proves the support bound; it does not assert the converse
that every bounded inner support lies in $\Delta_0$.

The coefficient of $\omega^{-\zeta}$ in $\varpi_{\lambda}$ is
$1$ at every $\zeta<\lambda$, so no such bound is possible.
Every element of $\Delta_{1}$ has zero coefficient at
$\omega^{-\lambda}$, since all its generators do. If
$a\varpi_{\lambda}+b\omega^{-\lambda}\in\Delta_{0}$, compare the
coefficient at $\omega^{-\lambda}$ to obtain $b=0$. Next choose
an inner-support bound $\beta<\lambda$ for that element and
compare its coefficient at $\omega^{-\beta}$; this gives $a=0$.
This proves independence modulo $\Delta_0$, not just separate
nonmembership of the two generators.
\end{proof}

All three groups are set-sized: they are images of sets of finite
lists of generators and real coefficients. They are divisible
because they are real vector spaces. The Hahn real-closedness
theorem therefore makes every $H_{\Delta_j}$ real closed, and
adjoining $i$ makes every $K_{\Delta_j}$ algebraically closed
\cite[Section~2.3]{BM}, with the same hypotheses as
\cref{diff:thm:hull}. The complex Hahn field is exactly this
quadratic extension: split each coefficient into its real and
imaginary parts; both supports remain subsets of the original
well-ordered support. The monomials $u_{\lambda}$ and
$\ell_{\lambda}$ witness the two strict inclusions respectively,
by \cref{diff:cp:lem:exponents}.
...
strong additivity \ref{diff:BM:additive}, not continuity.
Explicitly, $\der(\omega^\gamma)=\omega^\gamma
(\omega^\gamma)^\dagger$ shifts the latter support by $\gamma$,
which stays in the same additive group. For an arbitrary Hahn
series, strong additivity gives a summable family of these
termwise derivatives; their union of supports still lies in the
group, so their sum lies in the full Hahn field. A constant of a
subfield is an ambient constant, and the fields contain $\R$,
respectively $\C$, giving equality of constant fields.
\end{proof}

\begin{theorem}[Solution dimensions zero, one and two]
...
either support position: its coefficient at the first exponent
is exactly $a$, and at the second exactly $b$.
Thus membership in $K_{\Delta_0}$ forces $a=b=0$; membership in
$K_{\Delta_1}$ forces only $b=0$; and $K_{\Delta_2}$ permits both.
The real case has the identical support test with $a,b\in\R$.
This rules out hidden solutions with more complicated supports,
since the ambient spanning theorem was applied before restricting
to any of the three fields.
\end{proof}
...
\]
The larger field $H_{\Delta_2}$ is not closed under $\log$:
$\ell_\lambda\in H_{\Delta_2}$ but
$\log\ell_\lambda=\ell_{\lambda+1}\notin H_{\Delta_2}$.
\end{proposition}
\begin{proof}
...
Taylor series in $\varepsilon$ with support in $\Delta_{j}$
(\cref{diff:lem:neumann}); the case $\varepsilon=0$ gives the
zero series. Consequently
\[
 \log\bigl(r\omega^\gamma(1+\varepsilon)\bigr)
 =\log r+\log\omega^\gamma+
   \sum_{n\geq1}\frac{(-1)^{n+1}}{n}\varepsilon^n
 \in H_{\Delta_j}\qquad(j=0,1).
\]
The series is strongly summable inside the fixed group, since its
exponents are finite sums of exponents of $\varepsilon$.
This proves the claimed closure without assuming it in advance. Now $\log u_{\lambda}=\frac12\log\ell_{<\lambda}\in
...
with $r\in\R$, and its coefficient at exponent
$\omega^{-\lambda}\notin\Delta_1$ is still $1$, so no such
primitive lies in $H_{\Delta_1}$.

Finally, every element of $\Delta_2$ has inner support contained
in $\{\omega^{-\zeta}:\zeta\leq\lambda\}$. Hence
$\omega^{-(\lambda+1)}\notin\Delta_2$. The positive monomial
$\ell_\lambda$ lies in $H_{\Delta_2}$, but its logarithm
$\ell_{\lambda+1}=\omega^{\omega^{-(\lambda+1)}}$ does not.
This proves the failure of logarithm closure in the third field.
\end{proof}
...
\cref{diff:ex:logmissing} at a limit stage. The third field
contains both fundamental solutions, but loses logarithm closure:
\cref{diff:cp:prop:gaps} supplies the explicit witness. Thus
solving this equation and being closed under logarithms are
separate properties. $\Delta_{0}$ is divisible and
...
$\Delta_{0}+n\varpi_{\lambda}/2+m\omega^{-\lambda}$.
If two such cosets meet, subtraction gives
$(n-n')\varpi_{\lambda}/2+(m-m')\omega^{-\lambda}\in\Delta_0$,
so independence forces $n=n'$ and $m=m'$. For any nonzero
coefficient $f_{nm}$, choose one monomial in its support. Its
translate has nonzero coefficient in that summand and appears
in no other summand. The finite relation therefore cannot vanish.
This permits arbitrary Hahn coefficients in $k$, not just
constant or finitely supported coefficients.
\end{proof}
...
\end{theorem}
\begin{proof}
The derivatives of both generators lie in $L$ by
\cref{diff:cp:eq:pvder}; the product and quotient rules then
make $L$ a differential field. It contains the matrix
\[
...
\]
satisfying
$\der Y=\left(\begin{smallmatrix}0&1\\-Q_\lambda&0\end{smallmatrix}\right)Y$.
Indeed its columns are the solution pair and their derivatives;
$\der h+h^2=-Q_\lambda$ checks the second row directly.
Its entries generate $u_{\lambda}$ and
$\ell_{\lambda}=(u_{\lambda}\ell_{\lambda})/u_{\lambda}$ over $k$,
and its determinant is already a unit. No new constant occurs, because $L\subseteq\SC$, whose constants are
...
\cref{diff:cp:lem:algind} identifies $L$ with a rational function
field in two independent generators. The substitution in
\cref{diff:cp:eq:pvaction} is invertible there, with inverse
$\psi_{a^{-1},-a^2b}$, so no nonzero denominator is sent to zero.
It commutes with $\der$ on $k$ and on the generators, since
\[
 \der(au_\lambda)=h(au_\lambda),\qquad
 \der(a^{-2}\ell_\lambda+b)=(au_\lambda)^{-2}.
\]
The product and quotient rules extend this compatibility to all
of $L$.

Use the right-matrix convention $\psi(Y)=YM_\psi$. The action on
$u_\lambda\ell_\lambda$ is
$ab\,u_\lambda+a^{-1}u_\lambda\ell_\lambda$, hence
\[
 M_{\psi_{a,b}}=\begin{pmatrix}a&ab\\0&a^{-1}\end{pmatrix},\qquad
 \psi_{a,b}\circ\psi_{a',b'}
 =\psi_{aa',\,a'^{-2}b+b'}.
\]
Composition here applies the right-hand automorphism first; it
agrees with $M_{\psi\circ\phi}=M_\psi M_\phi$, since all matrix
entries are constants. Every upper-triangular determinant-one
matrix occurs, with $b=d/a$. The parameter changes $d=ab$ and
$b=d/a$ are regular on $a\neq0$, giving the claimed algebraic
group identification. In particular the translation subgroup
$a=1$ is additive, the quotient parameter is $a\in\C^\times$,
and
$\psi_{a,0}\psi_{1,b}\psi_{a,0}^{-1}=\psi_{1,a^2b}$.
Taking $a=2$ and $b=1$ explicitly shows noncommutativity.
\end{proof}
...
derivative lies in $K_{\Delta_{1}}$. A differential automorphism
fixing this base sends $\ell_\lambda$ to another primitive of
$u_\lambda^{-2}$, hence to $\ell_\lambda+b$ with $b\in\C$.
Conversely each translation defines a rational-field automorphism
with inverse translation by $-b$, and commutes with the derivation.
The displayed fundamental matrix still generates the extension,
and the ambient constant field again supplies no new constants.
Thus its transcendence degree over the new base is one and its
group is exactly $(\C,+)$.
\end{proof}
...
$y_{1},y_{2}$ is such a pair, all such $s$ are exactly
\[
 s=\frac{m_{11}s_0+m_{12}}{m_{21}s_0+m_{22}},\qquad
 s_0=y_2/y_1,\qquad M=(m_{jk})\in\GL_2(\C).
\]
Every displayed denominator is nonzero.
\end{lemma}
\begin{proof}
Given $s$, algebraic closedness supplies $v$ with
$v^2\der s=1$. This is a choice of either of two algebraic roots,
not a complex logarithm or an analytic branch. Differentiating
that identity gives
\[
 v^\dagger=-\frac12\frac{\der^2s}{\der s},\qquad
 \frac{\der^2v}{v}
 =-\frac12\frac{\der^3s}{\der s}
   +\frac34\left(\frac{\der^2s}{\der s}\right)^2
 =-\frac12\Sch(s).
\]
Thus $v$ solves the equation, and differentiating $v^2\der s=1$
shows that $vs$ does too, as in \cref{diff:cp:thm:critical}.
Their Wronskian is $1$, so they are independent.

Conversely let $\mathcal W=\mathrm{Wr}(y_1,y_2)\neq0$.
It is constant by \cref{diff:cp:lem:span}, and $y_1\neq0$.
For $s_0=y_2/y_1$ the quotient rule gives
$\der s_0=\mathcal W/y_1^2\neq0$ and
$\der^2s_0/\der s_0=-2\der y_1/y_1$. Differentiating the latter
identity in the definition of $\Sch$ yields
$\Sch(s_0)=-2\der^2y_1/y_1=2Q$.

Any other $s$ with this Schwarzian reconstructs an independent
pair $(v,vs)$ in the same two-dimensional constant solution space.
Write it as
$(m_{22}y_1+m_{21}y_2,m_{12}y_1+m_{11}y_2)$ with
$M\in\GL_2(\C)$; then its ratio is the displayed fraction.
Conversely that
constant change of basis has nonzero determinant, so it gives
another fundamental pair with the same Schwarzian. Since
$\der s_0\neq0$, one has $s_0\notin\C$; thus
$m_{21}s_0+m_{22}=0$ would force $m_{21}=m_{22}=0$,
contradicting invertibility. The transformed derivative is
\[
 \der\left(\frac{m_{11}s_0+m_{12}}{m_{21}s_0+m_{22}}\right)
 =\frac{\det(M)\der s_0}{(m_{21}s_0+m_{22})^2}\neq0.
\]
\end{proof}

\begin{corollary}[Transfinite Schwarzian potentials]
\label{diff:cp:cor:schwarzian}
For every ordinal $\alpha$, $\Sch(\ell_{\alpha})=2Q_{\alpha}$.
For every $c\in\R$, the $s$ with
...
\end{corollary}
\begin{proof}
The pair $(u_\alpha,u_\alpha\ell_\alpha)$ has ratio
$\ell_\alpha$. For $c<\frac14$, divide the first basis element
in \cref{diff:cp:eq:basissub} by the second to obtain
$\ell_\alpha^{r_+-r_-}=\ell_\alpha^{\sqrt{1-4c}}$.
Reversing the pair only inverts the ratio, which is itself a
constant fractional linear transformation. At $c=\frac14$ the
ratio of the second basis element in \cref{diff:cp:eq:basiscrit}
to the first is $\ell_{\alpha+1}$. These ratios have nonzero
derivative. The lemma supplies precisely their fractional linear
orbits; if $c>\frac14$, existence of such an $s$ would reconstruct
a nonzero solution, contrary to \cref{diff:cp:thm:main}.
\end{proof}
...
many terms. The equality for $\Delta_0$ follows in both directions
from $\varpi_n=\sum_{j<n}\omega^{-j}$ and
$\omega^{-n}=\varpi_{n+1}-\varpi_n$. In particular
$\varpi_\omega=1/(1-\omega^{-1})\notin\Delta_0$, even though
$1-\omega^{-1}\in\Delta_0$: this additive group is not a field,
and is not the field of Laurent series in $\omega^{-1}$. Every
...
\cite[Section~2, especially (2.5) and Corollary~2.3]{KT}, whose
Schr\"odinger potential has the opposite sign to $Q$ here: their
$-y''+qy=0$ becomes $y''+Qy=0$ with $q=-Q$.
Their finite logarithmic hierarchy is therefore an antecedent,
not a transfinite-surreal existence theorem. Neither the finite
threshold nor reduction of order is new. The
...
nonzero solution of $4y''+ay=0$. In the present normalization the
real equation is therefore tested by $\Omega(4Q)$, not
$\Omega(Q)$; this comparison imports no decision procedure.
The asymptotic differential algebra of
...
A dependency-ordered formalization can start with characteristic-zero
differential-field algebra: Wronskians and spanning, the gauge
identity, rational-field automorphisms, and the Schwarzian
calculation with its explicit square-root hypothesis. These finite
identities do not require the construction of all surreals.
Next isolate Hahn support and exponent-coset arguments, keeping
finite inner spans separate from full outer Hahn summation.
Finally instantiate the intrinsic interface: the exact constant
field, strong additivity, the phase obstruction and the imported
log-atomic identities, including limit ordinals. Normalization
$\der\omega=1$ cannot supply those missing laws by itself.

The current library's actual-field and strong-evaluation results
are reusable prerequisites; its fine-derivative calculus is a
different operator. The live coverage ledger
\path{docs/FORMALIZATION.md} does not claim the intrinsic tower,
critical classification or this Galois computation as Lean proofs.
Member~13 supplied no Lean file, a historical source-package fact
(\cref{diff:warn:noformalization}).
\end{remark}
...
finite angular primitive; exact constant field. The explicit
primitive $b\log s$ gives the contradiction; no surjectivity. \\
...
Schwarzian (\cref{diff:cp:lem:schwarzian}) & Algebraic square root
of $(\der s)^{-1}$, Wronskian spanning and the exact constant
field. No choice of analytic branch or global composition. \\
\bottomrule
\end{tabularx}
\end{table}
...
All three fields are algebraically closed after adjoining $i$ and
stable under the derivation. The first two real fields are closed
under logarithms; the third is not, as witnessed in
\cref{diff:cp:prop:gaps}. No exponential or general solution closure
...
Preprint: \href{https://arxiv.org/abs/1703.01995v3}{arXiv:1703.01995v3}.
...
preprint \href{https://arxiv.org/abs/0709.0127v2}{arXiv:0709.0127v2} (2007).
