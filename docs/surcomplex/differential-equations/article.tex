...
spine of scalar and workspace theory; the eighth carries the finite matrix
stratum and joined later.
...
\textbf{Status.} A merged exposition assembled from eight independent
manuscripts: seven pinned to \Repo\ at commit \Commit, and the eighth,
which supplies \cref{diff:part:matrix}, pinned to the later revision
\CommitB. Deep structural inputs
...
\subsection{Eight manuscripts, one theory}
\label{diff:sub:seven}

This report is the union of eight independently written manuscripts on the
...
\cref{diff:part:foundations,diff:part:scalar,diff:part:workspaces,diff:part:coordinate},
...
\cref{diff:ex:matnonnormal} is what marks the line.
\end{warning}
...
consolidated list is \cref{diff:sub:nc:matrixscope}.
\end{warning}
...
\item The eight delivered verification suites check \emph{finite exact
...
The remaining continuations stand. Classify logarithmic derivatives in
multi-scale $\der$-stable Hahn workspaces where support shifts lack the uniform
rational form of \cref{diff:eq:hahnderiv}. Extend the phase spectrum to
non-Hermitian coefficients, or show that no spectral formula can exist there,
the obstruction being that a nonnormal similarity gauge need not be entrywise
finite (\cref{diff:warn:matnonnormal}). Identify restricted, support-certified
input languages on which the phase map, the purely infinite parts of eigenvalue
primitives and a normalizing unitary gauge become effectively computable.
...
wholly different field. This report fixes $\der$ for the intrinsic derivation and uses $D$ for no
operator on $\No$ or $\SC$ --- its only appearance is as ordinary polynomial
differentiation on $\C[x]$ in the constant-coefficient discussion after
\cref{diff:thm:variation}, where nothing surreal is differentiated --- and keeps
...
The eight source manuscripts are preserved unmodified in the
\path{sources/} directory of this report's package, together with their own
READMEs and audit files. The identifiers below are the merge identifiers; among
the first seven the original archive names collided, three pairs of them
...
eighth states a further $12$, giving $130$.
\Cref{diff:sec:nonclaims} consolidates them into $64$ numbered items,
organized by theme; every one of the $130$ has a home there, and many items
...
self-rewriting verification script, is item~N64.
...
not re-run them and does not merge their counts into a single suite: eight
suites with eight scopes remain eight suites.

\begin{longtable}{@{}L{0.09\textwidth}L{0.13\textwidth}L{0.70\textwidth}@{}}
\caption{The eight recorded verification runs.}
...
symbolic checks; not a proof of general or infinite-support theorems. \\
\bottomrule
\end{longtable}
...
states Python 3.10 or later. All eight record zero failures. The consolidated
scope limits are item~N48 to item~N52 and item~N63 to item~N64 of
\cref{diff:sec:nonclaims}. Three artifact-level discrepancies are recorded
there and repeated here for completeness: one package's PowerShell build script
was supplied but not executed on Windows in its preparation environment; one
package's README lists a \path{SHA256SUMS.txt} that is not present among its
delivered files; and the eighth package's program rewrites its own delivered
\path{data/verification.json}, computing the package root as the parent of its
own directory, so the record reproduced above is the delivered one and a rerun
would overwrite it.
...
unmodified \path{code/} and \path{data/} directories of the eight source
packages. To re-run a source suite, run it \emph{on a copy}: several of the
delivered programs rewrite their own evidence files beside themselves, and the
eighth writes to a \path{data/} directory it creates one level \emph{above} its
own, so a byte-identity check performed after a run compares two equally
modified copies and tells you nothing.
