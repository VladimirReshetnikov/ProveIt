...
independent build of the repository is claimed, and no theorem of this report
is Lean-verified. Repository comparison is
...
($u=C(-1/x)$ in its notation); that report identifies the unit circle with
...
proves the valued-field forms of the rigidity report's displacement
results; its ordered-field \lbl{lem:amplify}, used here, is recorded in
\repo{FORMALIZATION.md} as pending. No statement of this report is covered
by a Lean proof.
...
\eqref{saut:eq:skeleton} leaves exactly $\U_{\mathrm{add}}$. This gives an
...
proposed finite limit. Equivalently, by \cref{saut:thm:derivative} a limit
would have to be zero, which \eqref{saut:eq:dilationquot} rules out.
\end{proof}
...
\item \emph{(Added.)} No statement of this report is covered by a Lean
proof. The Lean modules named in Section~\ref{saut:sec:position} are
interfaces and neighbouring results only; the Lean proof of the
discreteness clauses concerns lower-universe-small subsets of the Lean
carrier.
...
\end{enumerate}
No mathematical statement of the source was changed. Where the edit had to
choose: the results the source reproduces from the rigidity report (its
...
kernel clause) are printed here once, with the
