...
\begin{equation}\label{eq:enumerate}
...
\begin{equation}\label{eq:prefix-intro}
...
The pure complex field language gives a deliberately contrasting answer: there is only one missing one-variable type, and its omission number is $|K_{\kappa,\C}|$ rather than $\mu$; see \cref{thm:pure}. The linear application in \cref{thm:projection} provides an exact bound for every retraction $K+Kx\to K$, not merely an existence or nonexistence assertion.
...
General approximation-type theory already describes immediate extensions by nests of balls. Kuhlmann's account includes their relation to first-order one-types \cite{FK}; the passage from polynomial data to complete types uses classical quantifier elimination \cite{Tarski,Robinson,Yin}. The contribution proposed here is an explicit \emph{cardinal-prefix calculation} for the fields in \eqref{eq:fields}, together with its sharp singular-cardinal and completion consequences. In particular, the exact omission number is proved for arbitrary formulas, not only for the displayed ball conditions.

The repository comparison is with the pinned revision in \cref{sec:audit}. Its report on transcendence over bounded support concerns order-bounded supports and their fraction field, not the cardinal restriction in \eqref{eq:fields}. Its foundations and Hahn-vector-space reports supply a natural context, but the present proofs do not depend on a new, unaudited theorem from those manuscripts. A limited search did not identify the entire theorem package in the material reviewed. This is a scoped novelty assessment, not a guarantee that no equivalent statement exists.
...
\begin{equation}\label{eq:ultra-exact}
 v(f+g)=\min\{v(f),v(g)\}.
\end{equation}
...
\begin{lemma}[Small-workspace lemma]\label{lem:workspace}
...
\begin{proposition}[Algebraic closedness and real closedness]\label{prop:closed}
...
\end{proposition}
\begin{proof}
For two inputs, apply \cref{lem:workspace}. Their sum, product, and, for a nonzero input, inverse lie in the Hahn field $k((t^H))$. This proves the subfield assertion without any union of $\kappa$ many supports.
...
\begin{remark}[Why singular cardinals cause no algebraic failure]\label{rem:singular}
A countable union of sets of size less than a singular $\kappa$ can have size $\kappa$. That fact does not invalidate \cref{prop:closed}: a fixed polynomial has only finitely many coefficients, and all its roots can be found in \emph{one} exponent group of size less than $\kappa$. The relevant bound is not an uncontrolled union along a root construction. This distinction is central later: algebraic equations stay in one small workspace, whereas an infinite type may use parameters from many increasingly large workspaces.
\end{remark}
...
For a missing element $x$ as in \eqref{eq:enumerate}, define
...
\begin{theorem}[Exact approximation values]\label{thm:approx}
...
\begin{lemma}[Prefix fibers]\label{lem:fibers}
...
\begin{theorem}[First-$\kappa$ classification: valued case]\label{thm:valued-type}
For $x,y\in\FC\setminus\KC$,
\begin{equation}\label{eq:valued-type}
...
\end{theorem}
\begin{proof}
Suppose first that the prefixes agree. By \cref{lem:fibers}, either $x=y$ or $v(x-y)$ is greater than every element of $D_x$. For each $a\in\KC$, \cref{thm:approx} gives $v(x-a)\in D_x$, and therefore
\begin{equation}\label{eq:linear-data}
...
\]
with multiplicities retained. Neither $x$ nor $y$ is a root, since $\KC$ is algebraically closed and they do not belong to it. Formula \eqref{eq:linear-data} implies
\[
...
\begin{equation}\label{eq:prefix-formula}
 v(X-b_\alpha)>v(t^{s_\alpha}).
\end{equation}
It holds of $x$, hence of $y$. The two series consequently agree at every exponent at most $s_\alpha$, for all $\alpha<\kappa$. Their prefixes agree. The assertion about the entire realization set follows from \cref{lem:fibers}, which also excludes realizations in $\KC$.
\end{proof}

\begin{corollary}[Rational-function data]\label{cor:rational}
...
\end{corollary}
\begin{proof}
The missing elements are transcendental over their respective base fields. For complex coefficients, factor numerator and denominator into linear factors and apply \eqref{eq:linear-data}; leading coefficients multiply and divide. In the real case, the same identity shows that $x-a$ and $y-a$ have the same sign for every $a$ in the base. Factor real polynomials into real linear factors and monic irreducible quadratic factors, the latter positive everywhere. Signs of nonzero rational functions are therefore preserved.
\end{proof}

\subsection{The real ordered-field language}
\begin{theorem}[First-$\kappa$ classification: ordered case]\label{thm:real-type}
For $x,y\in\FR\setminus\KR$,
\begin{equation}\label{eq:real-type}
...
\end{theorem}
\begin{proof}
Equal prefixes imply equal signs of $x-a$ and $y-a$ for all $a\in\KR$, by the leading-coefficient argument in \eqref{eq:linear-data}. A nonzero real polynomial factors into linear factors and positive monic irreducible quadratic factors, together with a nonzero scalar. Thus its values at $x$ and $y$ have the same sign. Quantifier elimination for real closed fields now gives equal types.

For the reverse direction, set $b_\alpha=x_{\le s_\alpha}\in\KR$. The ordered-ring formula
\begin{equation}\label{eq:ordered-interval}
 b_\alpha-t^{s_\alpha}<X<b_\alpha+t^{s_\alpha}
\end{equation}
holds of $x$: its difference from $b_\alpha$ has valuation strictly greater than $s_\alpha$ and is therefore smaller in absolute value than the positive monomial $t^{s_\alpha}$. Equal types imply that \eqref{eq:ordered-interval} holds of $y$ as well.
...
\begin{corollary}[Counting and identifying the missing types]\label{cor:count}
In either language of \cref{thm:valued-type,thm:real-type}, the missing one-types realized in $F_k$ are in bijection with the Hahn series whose support has order type exactly $\kappa$. The bijection sends a type to its first-$\kappa$ prefix. If $K_{\kappa,k}\ne F_k$, there are at least $2^\kappa$ such types.
\end{corollary}
\begin{proof}
...
\end{equation}
Then
\begin{equation}\label{eq:approx-errors}
...
\begin{lemma}[Eventual elementary agreement]\label{lem:stabilize}
...
\end{lemma}
\begin{proof}
First consider a fixed $b\in\K$. By \cref{thm:approx}, $r=v(x-b)$ belongs to $D_x$. For all sufficiently large $i$, $s_{\alpha_i}>r$. Hence
\[
...
\begin{theorem}[Exact omission number]\label{thm:omission}
For each $x\in F_k\setminus\K$, in the ordered real or valued complex language,
\begin{equation}\label{eq:om-main}
...
\begin{equation}\label{eq:om-interval}
 b_i-t^{s_{\alpha_i}}<X<b_i+t^{s_{\alpha_i}}\qquad(i<\mu).
\end{equation}
These formulas belong to the type of $x$. As in the proof of \cref{thm:real-type}, a common realization must agree with $x$ below every $s_{\alpha_i}$ and hence on all first $\kappa$ terms. Again it cannot lie in $\KR$. This proves the upper bound and the equality.
\end{proof}

\subsection{What forgetting the valuation does}
\begin{theorem}[Pure-field collapse]\label{thm:pure}
...
\begin{equation}\label{eq:pureom}
...
\begin{corollary}[Restriction of types is truncation of prefixes]\label{cor:tower}
...
\end{corollary}
\begin{proof}
The inclusions are elementary by \cref{prop:closed} and quantifier elimination. The assertions about prefixes and omission numbers follow from \cref{thm:valued-type,thm:real-type,thm:omission} applied at the two cardinals.
\end{proof}
...
\begin{lemma}[Coefficient gluing with a cardinal bound]\label{lem:glue}
...
\]
The union on the right has size less than $\kappa$: a union of fewer than $\cf(\kappa)$ sets, each of cardinality less than $\kappa$, still has cardinality less than $\kappa$. Therefore $f\in\K$. For each $j$, it agrees with $a_j$ at every exponent below $\gamma_j$, so \eqref{eq:first-diff} gives $f\in B(a_j,\gamma_j)$.
\end{proof}
...
\begin{theorem}[First empty nests]\label{thm:balls}
...
\end{theorem}
\begin{proof}
The lower bound is \cref{lem:glue}. For the upper bound, choose a missing $x$ and use the approximants from \eqref{eq:approximants}. Let
\[
 B_i=B(a_i,s_{\alpha_i})\quad(i<\mu).
\]
For $i<j$, the center difference has value $s_{\alpha_i}$, and the radius value $s_{\alpha_j}$ is larger. The ultrametric inequality therefore gives $B_j\subseteq B_i$. The inclusion is strict: $a_i\notin B_j$. A common point would satisfy all formulas \eqref{eq:om-ball}, whose common realization in $\K$ was shown impossible. Thus this is an empty nest of cardinality exactly $\mu$.
\end{proof}

\begin{corollary}[Its intersection in the full Hahn field]\label{cor:ballfiber}
...
The ball theorem does not say that every type over fewer than $\mu$ parameters is realized. Our lower bound in \cref{thm:omission} applies to types realized in the fixed full Hahn extension. Other types may demand new values or residues. A field can have all the small ball intersections established here while failing a different saturation condition. Likewise, the pure-field omission number in \cref{thm:pure} is not contradicted by the smaller valued-field ball obstruction.
\end{remark}
...
Define
\begin{equation}\label{eq:localfield}
...
\begin{theorem}[Completion by locally small support]\label{thm:completion}
...
\end{theorem}
\begin{proof}
Suppose $f$ is in the closure. Given $\gamma\in\Gamma$, there is $a\in\K$ with $v(f-a)\ge\gamma$. By \eqref{eq:first-diff}, the support of $f$ below $\gamma$ is contained in the support of $a$, so it has size less than $\kappa$. Thus $f\in L_{\kappa,k}$.
...
\begin{proposition}[Exactly which supports are added]\label{prop:new-supports}
...
\end{proposition}
\begin{proof}
Write the support increasingly as $(s_\alpha)_{\alpha<\lambda}$ with $\lambda\ge\kappa$. If $\lambda>\kappa$, the exponent $s_\kappa$ exists, and the support strictly below it already has cardinality $\kappa$. This violates \eqref{eq:localfield}. Hence membership in $L_{\kappa,k}$ forces $\lambda=\kappa$.
...
\begin{lemma}[Long cofinal supports]\label{lem:long-cofinal}
...
\begin{equation}\label{eq:cofinal-condition}
 \cf(\Gamma)=\cf(\kappa).
\end{equation}
\end{lemma}
\begin{proof}
...
\begin{theorem}[Completeness dichotomy]\label{thm:dichotomy}
...
\begin{equation}\label{eq:complete-dichotomy}
...
By \cref{thm:completion,prop:new-supports}, a proper $\K$ fails to be complete exactly when $\Gamma$ admits a cofinal increasing sequence of order type $\kappa$. By \cref{lem:long-cofinal}, under the properness hypothesis this is equivalent to equality of the two cofinalities. The final assertion is \cref{thm:balls}.
\end{proof}

\begin{remark}[The completion need not be the full Hahn field]
A new completion element is required to have cofinal support of order type exactly $\kappa$. A series with $\kappa$ support positions confined below a fixed exponent is never added by completion. Thus the completion is not obtained simply by replacing $<\kappa$ by $\le\kappa$, nor by replacing $\kappa$ with $\kappa^+$. Example~\ref{ex:partialcompletion} gives an explicit strict chain $K_{\kappa,k}\subsetneq L_{\kappa,k}\subsetneq F_k$.
\end{remark}
...
\begin{theorem}[Exact retraction-loss formula]\label{thm:projection}
Put $r_c=v(x-c)\in D_x$. Then
\begin{equation}\label{eq:projectionexact}
 \boxed{\ P_c\text{ satisfies \eqref{eq:loss}}
 \quad\Longleftrightarrow\quad
...
\end{theorem}
\begin{proof}
Vectors in $K$ satisfy \eqref{eq:loss} automatically. A vector $a+bx$ with $b\ne0$ is $b(x-d)$ for $d=-a/b\in K$. By linearity and multiplicativity of the valuation, it suffices to check
\begin{equation}\label{eq:reducedloss}
 v(c-d)\ge v(x-d)-\delta\qquad(d\in K).
\end{equation}
Write $r_d=v(x-d)$. If $r_d<r_c$, then \eqref{eq:ultra-exact} gives $v(c-d)=r_d$, so \eqref{eq:reducedloss} holds. If $r_d=r_c$, the ultrametric inequality gives $v(c-d)\ge r_c$, so it also holds. If $r_d>r_c$, the exact value is $v(c-d)=r_c$, and \eqref{eq:reducedloss} is equivalent to $r_d\le r_c+\delta$. As the set of all $r_d$ is $D_x$, this is exactly \eqref{eq:projectionexact}.
...
\[
 v(Pz)>v(z)-\beta-\eta.
\]
The same lower bound is automatic if $Pz=0$. Thus $\delta=\max\{0,\beta+\eta\}$ is a loss bound. Applying \eqref{eq:projectionexact} to a continuous retraction forces $D_x$ to be bounded above. Finally, an initial segment of a linear order is unbounded exactly when it is the whole order, and \cref{prop:new-supports} identifies $D_x=\Gamma$ with $x\in L_{\kappa,k}$.
\end{proof}

\begin{corollary}[A zero-or-full continuous-dual dichotomy]\label{cor:dual}
...
\end{corollary}
\begin{proof}
If $x\in L_{\kappa,k}\setminus K$, then $K$ is dense in $E_x$: approximate $x$ by elements of $K$ and multiply and translate to approximate $a+bx$. If a continuous functional $\ell$ had $\ell(1)\ne0$, dividing by $\ell(1)$ would produce a continuous retraction, contrary to \cref{thm:projection}. Hence $\ell$ vanishes on the dense subspace $K$ and therefore vanishes everywhere, since the target is Hausdorff.
...
All algebraic equations appropriate to real closedness or algebraic closedness can still be solved in the smaller field, by \cref{prop:closed}. The omitted type of $x_\kappa$ in the ordered real or valued complex language needs exactly countably many formulas. Its realization set in this particular full Hahn field is a singleton, because $D_{x_\kappa}=\Gamma_\kappa$ and $I_{D_{x_\kappa}}=\{0\}$. This does not mean the type is isolated by a single formula: \cref{thm:omission} proves the opposite.
...
\begin{example}\label{ex:partialcompletion}
...
\]
has a cofinal well-ordered support of order type $\sum_{n<\omega}\aleph_n=\aleph_\omega$. The blocks are strictly separated because every $e_\alpha$ is less than $u$. Therefore $y\in L_{\kappa,k}\setminus K_{\kappa,k}$ by \cref{prop:new-supports}, whereas $x_\kappa\in F_k\setminus L_{\kappa,k}$. This proves the strict chain
\[
...
\]
The nontrivial cardinal-support phenomena in this paper therefore require value groups beyond the Archimedean case. Merely replacing the real coefficients by complex ones in a rank-one Hahn field does not create these missing types.
...
\begin{equation}\label{eq:complexconway}
...
For the groups $\Gamma_\theta$ in \eqref{eq:Gtheta}, define
\[
...
Under this embedding, the missing series in \eqref{eq:xkappa} is the actual surreal
\begin{equation}\label{eq:actualx}
...
Naming complex conjugation would create another language. Its fixed field is the real axis, and positivity on that fixed field is definable by being a nonzero square. Thus the pure-field collapse from \cref{thm:pure} cannot be transferred without qualification to a surcomplex field with named conjugation. We have not classified all one-types in that expanded language: an arbitrary complex element corresponds to a pair of real coordinates, whose joint algebraic relations require a tuple analysis.
...
Three comparisons matter. The report on \emph{transcendence over bounded support} uses supports bounded in exponent order and the fraction field they generate; our base is instead the already-real-closed or algebraically closed field of all supports of cardinality less than $\kappa$. The foundations report treats universes, localization, and the full fine topology; our theorems concern exact one-types and intrinsic completion in a fixed set-sized localization. The Hahn-vector-space report studies strong maps and several duals; \cref{cor:dual} instead uses the inherited valuation on a two-dimensional space over a potentially incomplete cardinally bounded base field. These differences are substantive, not merely changes of notation.
...
See \cref{sec:audit} for the actual inspection boundary.
\par\endgroup
