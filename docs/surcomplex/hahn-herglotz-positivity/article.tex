...
In ordinary complex analysis, a normalized holomorphic function $h$ on the disk with nonnegative real part has a unique representation
...
\begin{equation}\label{eq:basicpositivity}
...
Theorem~\ref{thm:normalization} reduces any coherent Hahn-holomorphic matrix with positive real part to a constant zero block and a normalized block whose ordinary coefficient has strictly positive real part. All higher coefficients are unrestricted apart from normalization. Positivity on ordinary points is equivalent to positivity on their infinitesimal halos. Theorem~\ref{thm:harnack} proves uniform matrix Harnack inequalities with strict slack and gives a counterexample to the unmodified sharp constant.
\item \textbf{An exact criterion for positive Hahn measures.}
Theorem~\ref{thm:nullideal} characterizes positivity by the vanishing of each negative coefficient measure on the common null ideal of all earlier coefficient measures. Countable predecessor sets admit a control measure; genuinely uncountable supports need the null ideal itself. Theorem~\ref{thm:functional} applies the criterion to positive Hahn-linear functionals and proves that even pointwise complete positivity does not force a positive measure representation.
\item \textbf{A fully explicit moment obstruction.}
Theorems~\ref{thm:negativeatom}--\ref{thm:schur} give positive Toeplitz matrices at every size, positive finite quadratures, nonpositive coefficientwise weak-* limits even with fixed Haar leading coefficient, and exact Schur parameters. Theorem~\ref{thm:hierarchy} assembles four proper inclusions, separating measure positivity, signed-measure regularity, distributional regularity, analytic regularity, and halo positivity.
\end{enumerate}
...
Classical constancy of the nullspace of the imaginary part of a matrix Herglotz function is already explicit in Gesztesy--Tsekanovskii, Lemma~5.3 \cite{GT}. We do not claim it as new. The contribution in Section~\ref{sec:normalization} is the arbitrary-rank Hahn normalization, the unrestricted-tail classification, and their halo and sharp-constant consequences. Similarly, ordinary Riesz representation, Fourier uniqueness, the distribution growth criterion, and finite real-closed-field congruence are established ingredients, not new discoveries.
...
Fix a nonzero, divisible, set-sized ordered abelian group $\Gamma$. Put
...
We use the standard facts that $\FG$ is real closed and $\KG$ is algebraically closed, and the standard Hahn support arithmetic. The corresponding surreal normal-form background is described in \cite{Gonshor}. Divisibility is used for finite Hermitian congruence and the modulus. The moment, measure, and explicit counterexample arguments themselves work over an arbitrary ordered value group containing a positive element.
...
\begin{lemma}[Leading-vector test]\label{lem:leadingvector}
...
Let $U\subset\C$ be a connected nonempty ordinary open set. Define
\begin{equation}\label{eq:analyticring}
 \AH(U)=\mathcal O(U)((t^\Gamma)).
\end{equation}
Thus $f=\sum_\gamma f_\gamma t^\gamma$ has a single well-ordered support and each $f_\gamma$ is holomorphic on the \emph{same} domain $U$. There is no requirement of uniform analytic bounds as $\gamma$ varies. Multiplication uses finite coefficient convolutions. Evaluation at an ordinary $a\in U$ is coefficientwise.
...
\begin{equation}\label{eq:tayloreval}
 f(a+\eta)=\sum_{\gamma}\sum_{k\ge0}
 \frac{f_\gamma^{(k)}(a)}{k!}\,t^\gamma\eta^k.
\end{equation}
Here the sum is a strong Hahn sum. The well-ordered support of $f$, together with the positive support of $\eta$, guarantees a well-ordered union and only finitely many contributions to each exponent, by the standard Neumann support lemma. Formula~\eqref{eq:tayloreval} is therefore valid without a common lower bound for the ordinary radii of convergence of the individual coefficients.
...
\item A Taylor series in an \emph{infinitesimal} increment, as in \eqref{eq:tayloreval}, is a strong Hahn sum. It need not be the limit of finite subsums in every topology one could put on the field.
...
\begin{remark}[The halo is not the full internal disk]\label{rem:disk}
...
\begin{lemma}[First nonzero real part]\label{lem:scalar}
Let $f\in\AH(U)$ and suppose $\Rea f(a)\ge0$ for every ordinary $a\in U$.
...
For the converse, Taylor evaluation at $a+\eta$ produces a finite series whose standard part is $f_0(a)$. Its real part is positive because $\Rea f_0(a)>0$. Notice that no size condition on the higher holomorphic coefficients was needed.
\end{proof}

\begin{corollary}[Scalar normalization]\label{cor:scalarnormal}
If $\rho=\Rea f(a_0)>0$, then
\[
 g(z)=\frac{f(z)-i\Ima f(a_0)}{\rho}
\]
has exactly the form described in Lemma~\ref{lem:scalar}(c). Thus every nonconstant positive-real-part function becomes finite, with strictly positive ordinary real part, after subtracting one imaginary constant and dividing by one positive Hahn constant.
\end{corollary}

\subsection{Constant congruence and the common kernel}

\begin{theorem}[Hahn--Herglotz normalization]\label{thm:normalization}
Let $H\in M_n(\AH(U))$ satisfy $\Rea H(a)\succeq0$ at every ordinary $a\in U$. Fix $a_0\in U$ and set
\[
...
\end{equation}
where
\begin{equation}\label{eq:normalizedG}
...
\begin{equation}\label{eq:commonkernel}
 \ker\Rea H(z)=\ker P\qquad(z\in\halo U).
\end{equation}
Conversely, every expression of the form \eqref{eq:normalform}--\eqref{eq:normalizedG}, with constant invertible $L$ and constant Hermitian $B$, defines a function with nonnegative real part on $\halo U$.
\end{theorem}
\begin{proof}
First recall a finite algebraic fact: over $\KG$, if $A\succeq0$ and $v^*Av=0$, then $Av=0$. One can prove it by diagonalizing a finite Hermitian matrix over the real closed base, or by applying positivity to $v+\lambda w$ and varying $\lambda$. This is not a compactness assertion.

Let $W=\ker P$. For a constant vector $v\in W$, the scalar function $v^*H(z)v$ belongs to $\AH(U)$, has nonnegative real part, and has zero real part at $a_0$. Lemma~\ref{lem:scalar}(b) gives
\[
...
Finite Hermitian congruence supplies a constant $L$ with $L^*PL=I_r\oplus0$ and with its last columns spanning $W$. The preceding paragraph gives the block form \eqref{eq:normalform}. The surviving block satisfies $G(a_0)=I_r$ and $\Rea G(a)\succeq0$.

For every nonzero ordinary $u\in\C^r$, apply Lemma~\ref{lem:scalar}(c) to $u^*G(z)u/(u^*u)$. Its negative coefficients vanish. Since this holds for every ordinary $u$, polarization gives $G_\gamma=0$ for every $\gamma<0$. The same scalar argument gives $u^*\Rea G_0(a)u>0$ for every nonzero ordinary $u$, proving $\Rea G_0(a)\succ0$.

For $z=a+\eta\in\halo U$, $G(z)$ is finite and $\st G(z)=G_0(a)$. Lemma~\ref{lem:leadingvector} gives $\Rea G(z)\succ0$. Congruence proves positivity and the exact common kernel on the whole halo. The converse follows by the identical leading-vector test and congruence. No constraint on the higher coefficients enters that test.
\end{proof}
...
\begin{theorem}[Uniform matrix Harnack comparison]\label{thm:harnack}
Under the hypotheses of Theorem~\ref{thm:normalization}, let $E\subset U$ be ordinary compact. There is an ordinary $C>1$ such that
...
\end{theorem}
\begin{proof}
Fix $v\in\KG^n$. If $v^*Pv=0$, the common-kernel conclusion gives $v^*\Rea H(z)v=0$, so the scalar inequalities are equalities. Otherwise put $\rho=v^*Pv>0$ and normalize $v^*Hv$ by Corollary~\ref{cor:scalarnormal}. The resulting $g$ has $g(a_0)=1$ and ordinary positive harmonic real part $u_0=\Rea g_0$. Classical Harnack gives $C_0^{-1}\le u_0(a)\le C_0$ on $E$.
...
\begin{equation}\label{eq:harnackfailure}
...
\begin{definition}[Coefficientwise Hahn measure]\label{def:measure}
A coefficientwise Hahn measure is a series
\begin{equation}\label{eq:measuredef}
 \mu=\sum_{\gamma\in S}\nu_\gamma t^\gamma,
...
For disjoint $E_j$, the identity $\mu(\bigcup_jE_j)=\sum_j\mu(E_j)$ means that its $\gamma$-coefficient is the ordinary identity $\nu_\gamma(\bigcup_jE_j)=\sum_j\nu_\gamma(E_j)$. The scalar sum is absolutely convergent because $\nu_\gamma$ has finite total variation. There is no assertion of strong Hahn summability in the index $j$. This is a definition of a coefficientwise category, not a hidden appeal to fine-topological countable additivity.
...
\begin{theorem}[Exact null-ideal criterion]\label{thm:nullideal}
The Hahn measure \eqref{eq:measuredef} is positive if and only if
\begin{equation}\label{eq:nullcriterion}
 \nu_\gamma^-(E)=0
...
\begin{equation}\label{eq:ACcriterion}
 \nu_\gamma^-\ll\lambda_{<\gamma}.
\end{equation}
No bound on a Radon--Nikodym derivative is required.
\end{theorem}
\begin{proof}
Assume first that $\mu$ is positive. If \eqref{eq:nullcriterion} fails, choose $E\in\mathcal J_{<\gamma}$ with $\nu_\gamma^-(E)>0$. Let $N_\gamma$ be a negative set in a Hahn decomposition of the signed measure $\nu_\gamma$, and set $A=E\cap N_\gamma$. Then every earlier coefficient vanishes on $A$, while
\[
 \nu_\gamma(A)=-\nu_\gamma^-(E)<0.
\]
Consequently $\mu(A)<0$, a contradiction.

Conversely, assume \eqref{eq:nullcriterion}. Fix a Borel set $E$. If every coefficient $\nu_\gamma(E)$ is zero, then $\mu(E)=0$. Otherwise let $\gamma$ be the least exponent with $\nu_\gamma(E)\ne0$. We claim that $|\nu_\beta|(E)=0$ for every $\beta<\gamma$.

Prove this claim by well-founded induction through the predecessor set. If all earlier total variations vanish on $E$, then $E\in\mathcal J_{<\beta}$, so $\nu_\beta^-(E)=0$. Since $\nu_\beta(E)=0$ by choice of $\gamma$, its positive part vanishes as well. Thus $|\nu_\beta|(E)=0$, completing the induction. Now $E\in\mathcal J_{<\gamma}$, so $\nu_\gamma^-(E)=0$. As $\nu_\gamma(E)\ne0$, it is strictly positive. The first nonzero coefficient of $\mu(E)$ is therefore positive.

The measure in \eqref{eq:control} has total mass at most $\sum_j2^{-j}$. Its null sets are exactly the common null sets of all $|\nu_{\beta_j}|$, since each weight is strictly positive. This proves the equivalence with \eqref{eq:ACcriterion}.
\end{proof}

\begin{corollary}[Two-scale criterion]\label{cor:twoscale}
...
\begin{equation}\label{eq:uncountable}
...
No countable collection of earlier point masses controls $m$. Indeed, a countable sum of them is supported on a countable set $A$, whereas $m(\T\setminus A)=1$. Thus replacing the common null ideal by a countable control measure would wrongly reject \eqref{eq:uncountable}. This proves a genuine distinction between the countable and arbitrary-support formulations.
...
\subsection{Positive functionals without positive measures}
...
\begin{theorem}[Infinitesimal saturation of positive functionals]\label{thm:functional}
...
\item $\Lambda(F)>0$ for every nonzero pointwise nonnegative $F\in\CH(X)$.
...
\begin{equation}\label{eq:ordinarybound}
...
A nonzero continuous nonnegative function is bounded below by a positive real number on some nonempty open set. Full support of $\mu_0$ therefore implies $\int F_\gamma\dd\mu_0>0$. All contributions from the second term in \eqref{eq:functional} have exponent at least $\gamma+\eta>\gamma$. Thus the leading coefficient of $\Lambda(F)$ is strictly positive. This proves (b).
...
The signed measure $\mu_0+\eps\nu$ represents the functional by definition. If another coefficientwise Hahn measure did so, testing all ordinary continuous functions and comparing each exponent would give the same ordinary signed measure at that exponent, by uniqueness in ordinary Riesz representation. Any additional coefficients would represent the zero functional and hence vanish. Corollary~\ref{cor:twoscale} now proves (d).

Finally, $\|f\|_\infty\pm f$ are ordinary continuous nonnegative functions. Positivity and normalization give \eqref{eq:ordinarybound}.
\end{proof}

\begin{corollary}[A Riesz-type representation obstruction]\label{cor:rieszfailure}
Take $X=\T$, $\mu_0=m$, and $\nu=m-\delta_1$. Then
\begin{equation}\label{eq:Rieszfailure}
 \Lambda_\eps(F)=(1+\eps)\int_\T F\dd m-\eps F(1)
\end{equation}
is positive, unital, and pointwise completely positive on $\CH(\T)$, but it has no positive coefficientwise Hahn-measure representation on the ordinary circle. Its two ordinary coefficient functionals are bounded in the usual supremum norm, with bounds $1$ and $2$.
\end{corollary}
\begin{proof}
Here $\nu^-=\delta_1$, which is singular with respect to $m$. Apply Theorem~\ref{thm:functional}. The norm estimates follow from $|\int f\dd m|\le\|f\|_\infty$ and $|\int f\dd m-f(1)|\le2\|f\|_\infty$.
\end{proof}

This obstruction is stronger than a failure to represent one formal moment sequence. The functional is already defined on all coherent continuous Hahn functions and is positive there. An ordinary continuous nonnegative function cannot isolate a point of Haar measure zero while keeping zero integral. The first nonzero continuous coefficient of a positive Hahn function has the same limitation. The Borel indicator of $\{1\}$, by contrast, immediately detects the negative atom.

The bound \eqref{eq:ordinarybound} is an order bound, not continuity from the ordinary supremum norm into the fine topology of the surreal field. An ordinary sequence of positive real numbers tending to zero does not tend to zero against every positive surreal radius. No such continuity is used or claimed.
...
\]
where $B_N$ is an ordinary finite Hermitian matrix. Its standard part is the identity, so Lemma~\ref{lem:leadingvector} proves positivity against every Hahn vector, independently of how large the finitely many entries of $B_N$ are.
...
\]
The trigonometric polynomial $q_N$ is real valued and has integral zero. If $m(E)>0$, the leading coefficient of $\mu_N(E)$ is $m(E)>0$; if $m(E)=0$, both coefficients vanish. Thus $\mu_N$ is a positive Hahn probability measure. Orthogonality of the ordinary characters gives
\[
...
The full-support hypothesis in Theorem~\ref{thm:functional} and the infinite-support hypothesis here have different purposes. Continuous nonnegative functions can vanish on an infinite closed set, whereas a nonzero polynomial has only finitely many zeros.
...
\begin{equation}\label{eq:commonmoments}
...
\begin{theorem}[Exact coefficientwise representation test]\label{thm:representation}
The moments \eqref{eq:commonmoments} have a signed coefficientwise Hahn-measure representation on $\T$ if and only if, for every $\gamma\in S$,
\begin{equation}\label{eq:variationtest}
 M_\gamma=
 \sup_{\substack{p\in\trig(\T)\\\|p\|_\infty\le1}}
 |L_\gamma(p)|<\infty.
\end{equation}
The representing coefficient measure $\nu_\gamma$ is unique and satisfies $\|\nu_\gamma\|_{\TV}=M_\gamma$. The resulting Hahn measure is positive if and only if its coefficients satisfy Theorem~\ref{thm:nullideal}.
...
\begin{equation}\label{eq:distributiontest}
...
\end{theorem}
\begin{proof}
A finite signed measure gives \eqref{eq:variationtest} by the total-variation inequality. Conversely, that inequality makes $L_\gamma$ a bounded linear functional on the trigonometric polynomials with their supremum norm. Trigonometric polynomials are uniformly dense in $C(\T)$, so the functional has a unique continuous extension. Ordinary Riesz representation supplies a unique finite regular complex measure with the same norm. The conjugate symmetry in \eqref{eq:commonmoments} makes the functional real on real functions, so this measure is real signed. Assembling these measures over the prescribed well-ordered $S$ gives the Hahn measure. Positivity is exactly the already proved null-ideal condition. This is a coefficientwise use of the classical Riesz theorem \cite{Rudin}, not a new version of that theorem over $\R$.
...
Bounded Fourier coefficients are necessary for a finite measure, but are not asserted to be sufficient. The exact measure condition is the norm bound on \emph{all trigonometric polynomials} in \eqref{eq:variationtest}. Polynomial Fourier growth is sufficient for a distribution because smooth test functions have rapidly decaying Fourier coefficients.

\subsection{Why positive measures really give positive Toeplitz matrices}

\begin{proposition}\label{prop:measuretoeplitz}
...
\begin{theorem}[All Toeplitz tests pass, but no positive measure exists]\label{thm:negativeatom}
Define
\begin{equation}\label{eq:mainmoments}
...
\begin{align}
 ...
 \qquad 1-N\eps\quad\text{with multiplicity }1,\label{eq:eigen}\\
 &\det T_N(c)=(1+\eps)^N(1-N\eps)>0.\label{eq:det}
\end{align}
The unique signed coefficientwise Hahn measure having these moments is
\begin{equation}\label{eq:badmeasure}
...
\end{theorem}
\begin{proof}
The diagonal entries in \eqref{eq:mainmatrix} are $1$, and the off-diagonal entries are $-\eps$, so the matrix formula is exact. The vector of ones has $J_{N+1}$-eigenvalue $N+1$, while its orthogonal complement has eigenvalue zero. This gives \eqref{eq:eigen} and \eqref{eq:det}. Both eigenvalues are positive by infinitesimality and ordinary finiteness of $N$.

The Fourier coefficients of $m$ are $\delta_{n0}$, and those of $\delta_1$ are all $1$. Therefore \eqref{eq:badmeasure} has precisely the moments \eqref{eq:mainmoments}. If another coefficientwise measure had these moments, every coefficient measure of the difference would have all Fourier coefficients zero. Trigonometric density and uniqueness of ordinary finite measures force every such coefficient to be zero. Finally $m(\{1\})=0$, giving the negative value on the singleton.
\end{proof}
...
\begin{theorem}[Nonclosedness of the positive-measure cone]\label{thm:quadrature}
...
\end{equation}
Then $\mu_M$ is a positive coefficientwise Hahn probability measure and represents \eqref{eq:mainmoments} for $|n|<M$. As $M\to\infty$, it converges coefficientwise weak-* to \eqref{eq:badmeasure}. Its two coefficient measures have total variations bounded uniformly by $1$ and $2$.

Thus the positive Hahn-probability cone is not closed in coefficientwise weak-* topology, even with one fixed two-element exponent support and uniformly bounded coefficient total variations.
\end{theorem}
\begin{proof}
...
The Fourier coefficient of $\sigma_M$ at $n$ is $1$ when $M$ divides $n$ and $0$ otherwise. For $0<|n|<M$, the moment of \eqref{eq:quadrature} is therefore $-\eps$, and the zeroth moment is $1$.

For an ordinary trigonometric polynomial $p$, $\int p\dd\sigma_M=\int p\dd m$ once $M$ exceeds its degree in both directions. Uniform approximation of a continuous $f$ by such a $p$, together with unit total masses, shows $\sigma_M\to m$ weak-*. Hence the exponent-zero coefficient of $\mu_M$ tends to $m$, while its exponent-$\eta$ coefficient $\sigma_M-\delta_1$ tends to $m-\delta_1$. Their variations are at most $1$ and $2$. The limit is not positive by Theorem~\ref{thm:negativeatom}.
\end{proof}
...
\begin{theorem}[Nonclosure with a fixed Haar leading coefficient]\label{thm:fejer}
The nonclosedness in Theorem~\ref{thm:quadrature} persists even when the leading coefficient is exactly $m$ throughout and every approximating coefficient measure is absolutely continuous with respect to $m$.
\end{theorem}
\begin{proof}
...
\begin{equation}\label{eq:boundarynegative}
...
\begin{theorem}[Exact Schur iteration and its internal poles]\label{thm:schur}
...
\begin{equation}\label{eq:schurpole}
...
\end{theorem}
\begin{proof}
Put $A=1-n\eps$ and $B=A+\eps$. If $s_n=-\eps/(A-Bz)$, then $\alpha_n=-\eps/A$, and direct substitution into \eqref{eq:schurrec} gives
\[
...
There is a cyclic algebraic unitary system over $\KG$ whose vacuum moments are \eqref{eq:mainmoments}, but which has no positive coefficientwise Hahn spectral measure on the ordinary circle reproducing those moments.
\end{corollary}
\begin{proof}
...
\]
with $\Lambda_\eps$ from \eqref{eq:Rieszfailure}. For $p\ne0$, the coherent function $|p|^2$ is nonzero and pointwise nonnegative on $\T$. Theorem~\ref{thm:functional} gives $\langle p,p\rangle>0$. Thus this is a positive definite Hermitian inner product.
...
\]
A spectral measure in the specified category would in particular be a positive Hahn measure with these moments, contradicting Theorem~\ref{thm:negativeatom}.
\end{proof}
...
Work in the universe of normalized coherent functions $H\in\AH(\D)$ with $H(0)=1$. Define their moments by
\[
...
\end{tabularx}
\end{center}
The letters $\mathsf D$ and $\mathsf A$ here denote classes, not the disk or the ring $\AH$.

\begin{theorem}[Strict Hahn--Herglotz hierarchy]\label{thm:hierarchy}
All four inclusions
...
\begin{equation}\label{eq:n2boundary}
...
Set
\begin{equation}\label{eq:superpolynomial}
 c_0=1,\qquad c_n=\eps e^{\sqrt{|n|}}\quad(n\ne0).
\end{equation}
The ordinary series $\sum_{n\ge1}e^{\sqrt n}z^n$ has radius of convergence exactly one, since $(e^{\sqrt n})^{1/n}\to1$. Its coefficients grow faster than every polynomial, so Theorem~\ref{thm:representation} rules out a periodic distribution at exponent $\eta$. Finite Toeplitz positivity is unaffected by this superpolynomial growth.

\paragraph{A halo-Herglotz function with a negative Toeplitz determinant: $\mathsf H\setminus\mathsf A$.}
Use \eqref{eq:harnackfailure}:
\[
...
\begin{equation}\label{eq:negativeT1}
...
\begin{example}[Zero analytic radius despite all finite tests]\label{ex:factorial}
Set $c_0=1$ and $c_n=\eps |n|!$ for $n\ne0$. Every $T_N(c)$ is strictly positive. Nevertheless the exponent-$\eta$ coefficient of the putative normalized analytic function is $2\sum_{n\ge1}n!z^n$, which has radius of convergence zero. There is no coherent analytic germ with these Taylor coefficients. Every finite prefix still has a positive measure completion by Theorem~\ref{thm:saturation}.
\end{example}

This last example lies outside the universe of \eqref{eq:hierarchy}. It demonstrates an additional distinction between a positive formal moment problem and an analytic Herglotz problem. It must not be called an analytic counterexample on a positive-radius disk.

\subsection{A constructive finite-information barrier}

\begin{theorem}[One finite prefix admits every boundary behavior]\label{thm:prefix}
...
\end{theorem}
\begin{proof}
For (i), use $m+\eps q_Nm$ from Theorem~\ref{thm:saturation}, and take its complete Fourier sequence.

For (ii), begin with the coefficient measure $m-\delta_1$. Add the real trigonometric density
\[
 q(\zeta)=\sum_{0<|k|\le N}(b_k+1)\zeta^k
\]
against $m$. This changes its low nonzero Fourier coefficients from $-1$ to $b_k$ and leaves total mass zero. Its mass at $\{1\}$ remains $-1$, so the resulting Hahn measure is not positive and no other coefficientwise measure can represent the same full sequence.
...
In each case the exponent-zero sequence is the Haar sequence, and all remaining moments are $\eps$ times ordinary numbers with conjugate symmetry. Theorem~\ref{thm:saturation} gives strict Toeplitz positivity at every finite size.
\end{proof}
...
\begin{proposition}[Normal-form transport]\label{prop:transport}
...
\end{equation}
embed $\FG$ as an ordered subfield of $\No$, and $\KG$ as its complexification inside $\No[i]$. They preserve conjugation, positivity, finite matrix inequalities, and all the coefficientwise identities proved above. The scalar and matrix halo conclusions transport under the same map.
\end{proposition}
\begin{proof}
...
For the central examples take $\Gamma=\Q$, $j$ the usual rational embedding, and $\eta=1$. Then $\eps=\omega^{-1}$, and Theorem~\ref{thm:negativeatom} becomes the entirely explicit surreal assertion
\[
...
\]
The positive unital functional is
\[
...
No proper-class support, proper-class integral, or proper-class matrix is used. Every workspace, measure coefficient family, and vector-space construction in the article is set sized. The uncountable example in Section~\ref{sec:measures} still has set-sized support and admits the explicit exponent embedding already given there.

\subsection{A replacement for an unqualified Herglotz theorem}

Within the coefficientwise category, the correct replacement has two independent layers. First, each exponent must satisfy the ordinary bounded-functional criterion \eqref{eq:variationtest}; this is the boundary \emph{regularity} requirement. Second, the unique coefficient measures must satisfy the null-ideal criterion \eqref{eq:nullcriterion}; this is the boundary \emph{positivity} requirement. These conditions are jointly necessary and sufficient. Neither all finite Toeplitz inequalities nor halo positivity substitutes for them.
...
The distinction has practical consequences for symbolic surcomplex computation. A system can certify every finite Toeplitz determinant symbolically and still have no justification for constructing a positive boundary measure. It should record separately the moment-positivity certificate, the coefficient regularity certificate, and the null-ideal certificate. Theorem~\ref{thm:prefix} shows that inspecting more but still finitely many moments cannot eliminate that separation in general.
...
The repository comparison was made on 22 September 2026 against
\begin{center}
\texttt{VladimirReshetnikov/Surreal}\par
\texttt{4cf691c7d951e037739d32d9f5c387dcce724f3c}.
\end{center}
The materials inspected through the GitHub connector were the root README, the maintained documentation reader map, the documentation directory listing, the detailed spectral-theory README, and the tree of the \texttt{docs/new} archive directory \cite{Repo}. The reader map describes 26 maintained reports. The archive directory additionally contained 18 ZIP files.

The maintained material already includes substantial Hahn support arithmetic, local and global surcomplex analysis, polynomial geometry, finite spectral theory, dynamics, and differential equations. We do not present those foundations or finite Hermitian congruence as new. The spectral-theory README distinguishes its finite matrix scope from spectral measures. The archive filenames also show existing infinite-spectral and Hahn-Hilbert work; Corollary~\ref{cor:unitary} is therefore framed narrowly as a moment-measure obstruction, not as the invention of infinite-dimensional surcomplex linear algebra.

Two repository code searches, for \texttt{Herglotz} and \texttt{Toeplitz}, returned no entries but explicitly reported \texttt{incomplete\_results: true}. These are \emph{not} reliable certificates of absence. The ZIP interiors were not available for a complete content audit in this run. No statement in this article should be read as certifying that every source file or archive in the repository lacks a related result.

The precise provenance claim is consequently limited: no matching statement for the combined Hahn normalization, null-ideal representation criterion, explicit negative-atom/Schur package, and five-way finite-prefix extension theorem was located in the inspected repository material or the targeted primary literature. This is evidence for a candidate new contribution, not a priority certificate. A later full-text audit or referee may identify earlier formulations or stronger antecedents.

\subsection{The literature comparison}

The classical comparison used the disk representation discussion in \cite{BBK} and the constant-kernel lemma and matrix-measure background in \cite{GT}. The latter was inspected in its actual PDF, including the relevant proof pages. Our subtraction of the constant $iB$ in \eqref{eq:normalform} is essential: a function with zero real part can be a nonzero imaginary constant. A zero-block assertion without accounting for that constant would be false.
...
Theorem~\ref{thm:normalization} and Theorem~\ref{thm:harnack} & Proved Hahn-level extension and unrestricted-tail classification, with a strict-slack obstruction. Candidate contribution in this precise form.\\[4pt]
Theorem~\ref{thm:nullideal} and Example~\ref{ex:uncountable} & Exact signed-coefficient positivity criterion at arbitrary support length; countable control measures are a corollary, not a substitute. Candidate contribution in this formulation.\\[4pt]
Theorem~\ref{thm:functional} and Corollary~\ref{cor:rieszfailure} & Positive unital, pointwise completely positive Hahn functionals; exact criterion for a positive coefficientwise measure. Candidate representation-obstruction package.\\[4pt]
Theorem~\ref{thm:representation} & Coefficientwise assembly of standard bounded-functional and distribution criteria, with an explicit proof and the new positivity criterion appended. Not a new ordinary Riesz theorem.\\[4pt]
Theorems~\ref{thm:negativeatom}--\ref{thm:schur} & Explicit all-size determinant, negative-atom, quadrature nonclosure, and Schur-boundary package. Candidate contribution; finite algebra is elementary but the infinite compatibility distinction is essential.\\[4pt]
Theorems~\ref{thm:hierarchy} and \ref{thm:prefix} & Strict hierarchy and constructive indistinguishability of finite prefixes. Candidate synthesis and extension theorem.\\
\bottomrule
\end{longtable}
...
Hahn support arithmetic and the ordinary minimum principle give scalar normalization. Finite congruence then gives matrix normalization, and the ordinary Harnack principle gives the strict-slack comparison.

\paragraph{Measure chain.}
Jordan decomposition and well-founded induction give the null-ideal criterion. Combined with full support of an ordinary measure, this yields the exact positive-functional representation test.
...
The only transfinite induction specific to the new measure argument occurs in the sufficiency proof of Theorem~\ref{thm:nullideal}. The uncountable example uses the axiom of choice to well order the ordinary circle. All other explicit counterexamples use a two-element Hahn support.

The accompanying \texttt{verify.py} performs exact symbolic checks of the finite determinant formula, the rational Schur recurrence, both internal boundary evaluations, the negative $2\times2$ determinant, and the algebraic quadrature moments. It also exhaustively compares the null-ideal test with direct positivity on every subset of a finite three-point space for three coefficient levels with entries in $\{-1,0,1\}$. The captured results are in \texttt{verification.txt}.
...
The source is self-contained LaTeX with an inline bibliography. Run \texttt{build.sh} from the archive directory, or run \texttt{pdflatex} repeatedly on \texttt{article.tex} until references stabilize. The verification code requires Python~3 and SymPy. No font files, external datasets, or copies of the cited papers are included.
...
Symbol & Meaning\\
\midrule
$\Gamma$ & A set-sized ordered abelian exponent group; divisible for the standing field convention.\\
$\FG,\KG$ & Real and complex Hahn fields with increasing well-ordered support.\\
...
$\AH(U)$ & Hahn series of holomorphic functions on one fixed ordinary domain $U$.\\
$\halo U$ & Ordinary points of $U$ with arbitrary infinitesimal increments.\\
...
$c_n=\int\zeta^{-n}\dd\mu$ & Fourier moment convention used throughout.\\
$T_N=(c_{j-k})_{0\le j,k\le N}$ & Toeplitz matrix of ordinary finite size $N+1$.\\
$\mathsf P,\mathsf M,\mathsf D,\mathsf A,\mathsf H$ & The five classes in Theorem~\ref{thm:hierarchy}.\\
\bottomrule
\end{tabularx}
\end{center}
...
\texttt{docs/surcomplex/spectral-theory/README.md}, and the \texttt{docs/new} directory tree; see Appendix~\ref{app:audit} for limitations.
