...
That report, assembled from three manuscripts of the same batch, is the measure-theoretic companion of Sections~\ref{herg:sec:measures} and~\ref{herg:sec:representation}. Its central theory concerns \emph{strong} Hahn measures, for which the masses of every disjoint sequence of events are strongly Hahn summable; it also treats the coefficientwise class used here. The measures of this report are coefficientwise and are \emph{not} strongly additive: their leading coefficient is typically Haar measure, which is diffuse, whereas by the atomicity theorem of that report every coefficient of a strong Hahn measure is a finite combination of point masses. Three consequences should be kept in view.
...
The source manuscript made divisibility of $\Gamma$ a standing hypothesis. It is not one here. Divisibility is assumed exactly in Theorem~\ref{herg:thm:normalization} and in Theorem~\ref{herg:thm:harnack}, which is stated under the hypotheses of Theorem~\ref{herg:thm:normalization} and uses its common kernel on the halo; the matrix halo conclusions transported in Proposition~\ref{herg:prop:transport} carry the same hypothesis. There it supplies the standard facts that $\FG$ is real closed and $\KG$ algebraically closed, and with them the finite Hermitian congruence $L^*PL=I_r\oplus0$, which needs square roots of positive elements of $\FG$. The source names one further use, the modulus $\sqrt{z\overline z}$ of a complex Hahn number; no statement below takes the modulus of a non-ordinary element, since the internal disk is written with $z\overline z<1$ and Toeplitz tests with $u^*u$. The source states that the moment, measure, and explicit counterexample arguments work over an arbitrary ordered value group containing a positive element; accordingly Lemma~\ref{herg:lem:scalar}, Corollary~\ref{herg:cor:scalarnormal}, and Sections~\ref{herg:sec:measures}--\ref{herg:sec:transport} (apart from the matrix halo conclusions just named) assume only that $\Gamma$ is nonzero.
...
This subsection and the next are the only places where $\Gamma$ is assumed divisible (see Section~\ref{herg:sec:framework}). Lemma~\ref{herg:lem:scalar} and Corollary~\ref{herg:cor:scalarnormal} above hold for every nonzero $\Gamma$.
...
\end{theorem}
\begin{proof}
First recall a finite algebraic fact: over $\KG$, if $A\succeq0$ and $v^*Av=0$, then $Av=0$. One can prove it by diagonalizing a finite Hermitian matrix over the real closed base, or by applying positivity to $v+\lambda w$ and varying $\lambda$; the second argument uses only the order of $\FG$. This is not a compactness assertion.
...
For every nonzero ordinary $u\in\C^r$, apply Lemma~\ref{herg:lem:scalar}(c) to $u^*G(z)u/(u^*u)$. Its negative coefficients vanish. Since this holds for every ordinary $u$, polarization gives $G_\gamma=0$ for every $\gamma<0$. The same scalar argument gives $u^*\Rea G_0(a)u>0$ for every nonzero ordinary $u$, proving $\Rea G_0(a)\succ0$.
...
Under the hypotheses of Theorem~\ref{herg:thm:normalization}, including divisibility of $\Gamma$, let $E\subset U$ be ordinary compact. There is an ordinary $C>1$ such that
...
\end{theorem}
\begin{proof}
Fix $v\in\KG^n$. If $v^*Pv=0$, the common-kernel conclusion gives $v^*\Rea H(z)v=0$, so the scalar inequalities are equalities. Otherwise put $\rho=v^*Pv>0$ and normalize $v^*Hv$ by Corollary~\ref{herg:cor:scalarnormal}. The resulting $g$ has $g(a_0)=1$ and ordinary positive harmonic real part $u_0=\Rea g_0$. Classical Harnack gives $C_0^{-1}\le u_0(a)\le C_0$ on $E$.
...
Its counterpart in the measures report is a positive-definite Hermitian power-moment form on $[0,1]$ whose multiplication operator has no positive \emph{strong} spectral measure (Section~\ref{herg:sec:position}); the two statements differ in class, sample space and kind of moment. Neither the spectral-theory report nor \repo{surcomplex/infinite-dimensional-hahn-spectral-theory} supplies spectral measures, so the corollary contradicts nothing there; it records a reason why the natural positive candidate can fail to exist.
...
\end{equation}
embed $\FG$ as an ordered subfield of $\No$, and $\KG$ as its complexification inside $\No[i]$. They preserve conjugation, positivity, finite matrix inequalities, and all the coefficientwise identities proved above. The scalar and matrix halo conclusions transport under the same map, the matrix ones under the divisibility hypothesis of Theorem~\ref{herg:thm:normalization}.
\end{proposition}
\begin{proof}
...
\item No named published open conjecture is claimed settled. The package is a candidate original contribution; priority, and absence from every repository file or archive, are not certified. The proofs are not refereed and not checked in Lean.
...
\item Divisibility of $\Gamma$ is assumed exactly where Section~\ref{herg:sec:framework} says. Nothing is claimed about Theorem~\ref{herg:thm:normalization} or Theorem~\ref{herg:thm:harnack} for nondivisible $\Gamma$.
...
\item Every measure here is coefficientwise. Nothing in this report is a statement about strongly additive Hahn measures, and the two-exponent support of Theorem~\ref{herg:thm:negativeatom} does not conflict with the order type $\omega+1$ that is the threshold in the strong class.
\end{enumerate}
...
The classical comparison used the disk representation discussion in \cite{BBK} and the constant-kernel lemma and matrix-measure background in \cite{GT}. The latter was inspected in its actual PDF, including the relevant proof pages. Our subtraction of the constant $iB$ in \eqref{herg:eq:normalform} is essential: a function with zero real part can be a nonzero imaginary constant. A zero-block assertion without accounting for that constant would be false.
...
Theorem~\ref{herg:thm:normalization} and Theorem~\ref{herg:thm:harnack} & Proved Hahn-level extension and unrestricted-tail classification, with a strict-slack obstruction, for divisible $\Gamma$. Candidate contribution in this precise form.\\[4pt]
...
Hahn support arithmetic and the ordinary minimum principle give scalar normalization. Finite congruence, for divisible $\Gamma$, then gives matrix normalization, and the ordinary Harnack principle gives the strict-slack comparison.
...
Symbol & Meaning\\
\midrule
$\Gamma$ & A nonzero set-sized ordered abelian exponent group; divisible only in Theorems~\ref{herg:thm:normalization} and~\ref{herg:thm:harnack}.\\
