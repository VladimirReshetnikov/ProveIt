...
optimal radius for $\rang\ge2$ remains open; an exact radius-collapse law
...
$\rang=1$, which includes every scalar multiplier, and for $\rang\ge2$ the
optimal radius is left open (\cref{dyn:nsc:thm:criterion}). Source 10 read
...
preservation of admissible cuts \cite[Prop.~5.6]{FMS26}, and Carletti's
...
& $\kappa<\infty$, with universal radius $Re^{-\kappa}$, for
 ...
 (\cref{dyn:nsc:thm:criterion}); the optimal radius for $\rang\ge2$ is
...
when $\rang\ge2$, which is \cref{dyn:q:commongerm}.
\end{convention}
...
is the optimal radius when $\rang\ge2$ (\cref{dyn:q:commongerm}).
\end{warning}
...
when $\rang\ge2$), and the absence of machine-checked proofs. The first row
...
sharp, when $\rang=1$ (\cref{dyn:nsc:thm:criterion}); whether it holds when
$\rang\ge2$ is open.
...
rational rank of the multiplier's angles. When $\rang=1$ the loss is exactly
$e^{-\tau}$ and is sharp, in particular for every scalar $\lambda I_d$
...
the $f_s$. Then the unique normalized positive conjugacy $H=z+h$ of
\cref{dyn:thm:main}, $H\circ F=\Lambda H$, and its inverse
...
$(-e_v(k))_v$. Expanding along it bounds the determinant by
$C_1N_k^{q}e^{-b_*N_k/2}$, with $C_1$ independent of $k$. An integer of
...
rational rank at most $q$ for large $k$.
...
Polynomial factors do not change a positive exponential rate, so the rate
of $e_v$ would exceed $b$, a contradiction. Order the distinct rates from
...
integer-determinant argument, not a subspace theorem. Source 10 records two
limits of the lemma. The finiteness of the family is essential: nothing is
asserted for a number of vector sequences growing with $N_k$. And the height
bound must be linear, or at least polynomial; exponentially large heights
would destroy the polynomial Cramer bound.
...
$p'_v=Qp_v-\sum_jQa_{j0}k_{v,j}$ have height $O(N)$ and satisfy
...
the same exponential rates as the circle divisors. Apply
\cref{dyn:nsc:lem:height} with $q=\rang$.
\end{proof}
...
condition on all of $\Z^{d}$. Preservation of admissible cuts in operator
trees has a precedent in \cite[Prop.~5.6]{FMS26}; source 10 claims the rank
estimate, not the use of tree cuts.
...
\]
exists for every internal vertex; the limits are finite and nonnegative by
\eqref{dyn:nsc:eq:Ca} and $|\alpha_v|\le n_k+1$. The rate of
...
the constant independent of $v$ and $k$. Summing over $A_b$ and using the
...
\end{corollary}
\begin{proof}
The upper bound is \cref{dyn:nsc:thm:products}. For each $n$ choose a
monomial and output color realizing $\Delta_{n+1}$; its one-vertex tree has
...
fixed exponent $\gamma$ its expansion uses trees with at most $\ell_S(\gamma)$
...
contributes one Taylor coefficient $f_{s_v,j_v,\mu_v}$ of an input, with
$|\mu_v|=a_v$, and the divisor $\lambda^{\alpha_v}-\lambda_{j_v}$ of
...
directly, by the least-difference argument for
\eqref{dyn:nsc:eq:forward}, which does not assume that a second solution is
supported in $S^{*}$. For the expansion, iterate
$\check h=\Li^{-1}f(z+\check h)$ formally. A coefficient
$f_{s,j,\mu}z^{\mu}e_j$ supplies an internal vertex of output color $j$,
arity $|\mu|\ge2$ and Hahn letter $s$. Expanding a slot by a further copy of
...
Construct $\check H$ by \cref{dyn:nsc:prop:formal}. Fix $\gamma$, put
$m=\ell_S(\gamma)$, and fix $0<r<R$. By Cauchy's estimate there is a finite
$M_\gamma(r)\ge1$ with
...
\end{equation}
The root test gives $\radiso(\check h_\gamma)\ge re^{-\min(\rang,m)\tau-\eps}$;
...
weight). Each such $\check h_\beta$ is holomorphic on the polydisk of
\eqref{dyn:nsc:eq:relative}, and finite products and derivatives keep it
there.
\end{proof}
...
If the input coefficients are holomorphic on $\prod_j\mathbb D_{R_j}$, all
coefficients of $H$ and $H^{-1}$ are holomorphic on
$\prod_j\mathbb D_{R_je^{-\rang\tau}}$. If the $f_s$ also depend
holomorphically on ordinary parameters $w$ in a complex domain $V$, with
$\Lambda$ independent of $w$, the coefficients are jointly holomorphic on
that polydisk times $V$.
\end{corollary}
\begin{proof}
The rescaling $z_j=R_jx_j$ commutes with the diagonal $\Lambda$ and does not
change its divisors; apply \cref{dyn:nsc:thm:main} with radius one and undo
it. For parameters, fix a relatively compact subset of $V$. The finitely
...
$|u\sqrt2+v\sqrt3-p|\ge C(n+1)^{-3}$. With $(u,v)=\alpha-e_j$ this bounds
every homological divisor polynomially from below, so $\tau=0$ and the
...
For an arbitrary well-ordered support, source 10 uses exactly the set $S$
...
summation is the support operation of \cref{dyn:def:summable}, not a limit
in the fine topology. The failure at $\tau=\infty$ concerns ordinary
...
10 except for the optimal radius when $\rang\ge2$
...
$Re^{-\tau}$ when $\rang=1$). The optimal radius when $\rang\ge2$ is still
open.
\end{warning}
...
(\cref{dyn:nsc:thm:main}); only the optimal radius when $\rang\ge2$ remains
...
is covered by source 10 except for the optimal radius when $\rang\ge2$
...
criterion.
\item\label{dyn:nsc:nc:gap} (10) For $\rang>1$ the factor $\rang$ is a
...
comes from a subsequence argument: it is finite but has no computable
formula. No bound uniform in $m$ follows, and replacing $m$ by $n$ in
\eqref{dyn:nsc:eq:products} and dropping the prefactor is invalid.
\item (10) \Cref{dyn:nsc:lem:height} needs a finite family of vector
sequences, and asserts nothing for a number of sequences growing with the
height; and it needs heights growing linearly, since exponentially large
heights would invalidate the polynomial Cramer bound.
...
Preservation of admissible cuts in operator trees \cite[Prop.~5.6]{FMS26};
...
$\Lambda$, whose optimal radius for $\rang\ge2$ is
...
$\nres$; $\tau<\infty$; arities at least two, so every weight is positive
(no infinitesimal linear term). Fixed $m$; the prefactor is not explicit and
not uniform in $m$.\\[3pt]
...
sharp for $\rang=1$; the optimal radius for $\rang\ge2$ is open
...
\href{https://arxiv.org/abs/math/0110135}{arXiv:math/0110135}, 12 October
2001. Source 10 used its abstract and metadata only.
...
version~2 dated 13 September 2026 (first posted 17 July 2025), 41~pp.
