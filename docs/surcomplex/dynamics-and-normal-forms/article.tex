...
\vspace{0.35in}
{\large 21 September 2026; extended 22 September 2026\par}
\vspace{0.40in}
...
Nine of the ten are germ and fixed-point dynamics: multipliers, return
...
\textbf{Status.} This is a merged report assembled from ten AI-assisted,
...
conjecture is claimed solved. The ten sources' own verification programs
...
\subsection{Ten manuscripts, one body of machinery}
...
as merged here. All ten share a single body
...
near-identity inverse --- and each of the ten states that machinery from
scratch. The positive-support lemma is stated in all ten. Between $45$
and $55$ per cent of the combined text of sources 01--06 is the same
theory, and sources 07--10 restate the same foundation again.
...
(\texttt{docs/surcomplex/analysis/}). All ten sources restate it; we cite
...
(\texttt{docs/surcomplex/differential-equations/}). All ten sources
...
and disclaims priority (\ref{dyn:nsc:nc:priority}).
...
All ten sources set up this material independently. It is stated once
here.
...
phase residue of the scalar derivation. All ten sources derive this
...
claimed as a contribution. All ten sources state it; sources 02, 05 and 09
...
material. All ten sources restate that discipline and all ten state
the support lemma. Sources 02, 03, 05 and 09 each reproduce a
...
The ten verification programs are preserved unmodified in \texttt{code/}
...
finite range used. No Lean code.\\
\bottomrule
\end{longtable}
...
\begin{enumerate}[label=(\arabic*)]
\item \textbf{All ten suites pass.} Run on Python 3.14.4 with SymPy 1.14.0
where SymPy is used, every one exits $0$; sources 08 and 09 were run, on a
copy, when they were integrated. Reported totals where a script
...
\item \textbf{Four of the ten scripts rewrite their own evidence.} Source
...
\end{verbatim}
Reproducing the ten suites, each \emph{on a copy}, requires Python 3.10 or
later and, for five of them, SymPy; the pinned versions recorded by the
...
\cref{dyn:conv:merge}(iv). It carries \textbf{one hundred and twenty-three}
distinct limitations stated by the ten sources --- eleven from source 01,
...
source 08, twelve from source 09 and seventeen from source 10 --- with the
originating source named in each case. None has been merged away, softened, or absorbed
...
of \cref{dyn:esm:sec}, which it cites rather than reproves. None uses an
...
README. The reports are AI-assisted, unrefereed research drafts,
