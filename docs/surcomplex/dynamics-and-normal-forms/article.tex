...
\begin{abstract}
Six independently written manuscripts developed local and global dynamics
...
\emph{resonance} across the six sources, and five inequivalent conditions
...
invisible moduli for global near-identity maps; and a gauge-normalized
symplectic Hamiltonian normal form, its full Poisson centralizer, and
support-certified domains reaching infinite phase magnitudes.
\end{abstract}
\end{minipage}
\vfill
\begin{minipage}{0.92\textwidth}\small
\textbf{Status.} This is a merged report assembled from six AI-assisted,
unrefereed research drafts. It is not refereed and not proof-assistant
verified; no Lean or other formal development corresponds to any theorem
below. Priority is not certified for any statement, and no named published
conjecture is claimed solved. The six sources' own verification programs
...
\subsection{Six manuscripts, one body of machinery}
The six source manuscripts preserved in \texttt{sources/} were written
independently on 21 September 2026 against the same pinned repository
snapshot. They share a single body of machinery --- the set-sized Hahn
workspace inside $\No[i]$, strong summability by well-ordered support and
local finiteness, the positive-support lemma of Neumann in finite-word
form, halo evaluation by coefficientwise double Taylor sums, Taylor
substitution and its near-identity inverse --- and each of the six states
that machinery from scratch. The positive-support lemma is stated in all
six. Between $45$ and $55$ per cent of the combined text is the same
theory.
...
(\texttt{docs/surcomplex/analysis/}). All six sources restate it; we cite
...
(\texttt{docs/surcomplex/differential-equations/}). All six sources derive
it independently; we adopt that report's wording once, in
\cref{dyn:warn:derivative}.
\end{itemize}
...
\cref{dyn:thm:moduli,dyn:thm:flat,dyn:thm:slowtime}; and the Hamiltonian
...
\cref{dyn:sec:nonclaims}.

\section{The notion table: four resonances, five small divisors, two radii}
...
The six sources use the word \emph{resonance} for four inequivalent
conditions and the words \emph{small divisor} for five, and two groups of
...
\begin{warning}[The four notions do not intersect in the way the word suggests]
...
\subsection{Five notions of small divisor}
\label{dyn:subsec:smalldivisor}

All five concern \emph{ordinary complex} divisors. Every nonzero ordinary
...
treatment using the decreasing-support Conway convention directly has
every inequality of this report reversed. Silently merging a threshold
across the two conventions inverts it.
\end{warning}
...
All six sources set up this material independently. It is stated once here.
...
phase residue of the scalar derivation. All six sources derive this
...
claimed as a contribution. All six sources state it; sources 02 and 05 each
reproduce a minimal-bad-sequence proof of the underlying word order, which
is the same argument as \cref{dyn:app:higman} and is therefore not printed
again.

For $\gamma\in S^{*}$ let $D_S(\gamma)$ be the finite set of letters
occurring in the words of $\word_S(\gamma)$.

\begin{corollary}[Finite ancestry]\label{dyn:cor:ancestry}
Suppose $T$ is a $\C$-linear operator on coefficient families such that
every term of $T(A)$ adds a nonempty word from $S$ to an input exponent of
$A$, with finitely many coefficient operations per word. Then
$\sum_{k\ge0}c_kT^{k}(A)$ is strongly summable for any ordinary scalars
$c_k$, its support lies in $\supp A+S^{*}$, and at a fixed output exponent
$\gamma$ only finitely many input exponents, word lengths and operations
contribute --- at most $\ell_S(\gamma)$ applications of $T$, and only
input data at letters in $D_S(\gamma)$. Consequently polynomial identities
valid for a nilpotent operator may be used coefficientwise, which is what
justifies the geometric, logarithmic and exponential identities below. The
same statement holds for operators with Hahn coefficients, for vector and
matrix operators, and for a strongly summable input family simultaneously.
\end{corollary}
\begin{proof}
Flatten every intermediate exponent into a word in $S$ and apply
\cref{dyn:lem:neumann}; \cref{dyn:lem:neumann}'s last assertion bounds the
relevant input weights, and local finiteness of the input family bounds the
...
\end{proposition}
\begin{proof}
Strong summability is \cref{dyn:lem:neumann,dyn:cor:ancestry}. At a fixed
...
length. Taylor composition in that finite nilpotent parameter algebra is
associative and multiplicative, and each asserted coefficient identity is
...
\cref{dyn:cor:ancestry}, and every output coefficient is a finite
differential expression with coefficients holomorphic on $U$.
...
letters. \Cref{dyn:lem:neumann,dyn:cor:ancestry} give the support
inclusion, strong summability, \eqref{dyn:eq:word-term-bound}, and the
finiteness of the relevant letters and intermediate sums. Every
...
coefficientwise, \cref{dyn:cor:ancestry} permitting every rearrangement; so
the image equals $\mathscr D\AAA$. The kernel of $\mathscr D$ is $K$ since
...
(\cref{dyn:cor:ancestry}), not a rank-one approximation to the value
group.
...
material. All six sources restate that discipline and all six state the
support lemma, with sources 02, 03 and 05 each reproducing a
...
The six verification programs are preserved unmodified in \texttt{code/}
and their recorded outputs in \texttt{data/}. They supply \emph{finite
exact symbolic} checks. None of them is a proof of any infinite statement
in this report, and no Lean or other proof-assistant development
corresponds to any theorem here.
...
\begin{enumerate}[label=(\arabic*)]
\item \textbf{All six suites pass.} Run on Python 3.14.4 with SymPy 1.14.0
where SymPy is used, every one exits $0$. Reported totals where a script
prints one: $54$ (01), $1{,}109$ (02), $21$ (03), $2{,}764$ (04), $245$
(05), $119$ (06). Source 04's count reproduced the recorded $2{,}764$
exactly; source 05 reproduced its recorded transcript line for line; source
03 reproduced its four recorded example expansions byte for byte; source
06's regenerated JSON matched the shipped one up to line endings only.
\item \textbf{Three of the six scripts rewrite their own evidence.} Source
...
packaging gap, recorded rather than repaired, since
\texttt{sources/} and \texttt{code/} are preserved unmodified.
...
\end{verbatim}
Reproducing the six suites, each \emph{on a copy}, requires Python 3.10 or
...
\cref{dyn:conv:merge}(iv). It carries \textbf{sixty-eight} distinct
limitations stated by the six sources --- eleven from source 01, nine from
source 02, ten from source 03, fourteen from source 04, twelve from source
05 and twelve from source 06 --- with the originating source named in each
case. None has been merged away, softened, or absorbed into a stronger
statement. Where two sources state overlapping limitations, both wordings
are preserved because their scopes differ.
...
\noindent The list above contains $70$ numbered items for the $68$ distinct
...
$11/9/10/14/12/12$ across sources $01$ through $06$; the corresponding
numbered-item counts are $11/9/11/15/12/12$.
...
demanded ($\sddr$, $\sdex$, $\sdfr$); and under $\pares$ there is no
divisor family at all ($\sdno$). The two positions are complementary, not
opposed, and stating them without their coefficient categories is what
would make them look contradictory.
\end{warning}
...
& All six sources cite it at commit
\texttt{39f2be6667ade51bca2b45daa47e289d69c09764}, for the
...
$\resq$, $\mres$/$\nres$, $\fres$, $\pares$ & The four resonance notions
(\cref{dyn:subsec:resonance}). A threshold proved under one is never quoted
under another.\\[3pt]
$\sdz$, $\sddr$, $\sdex$, $\sdfr$, $\sdno$ & The five small-divisor notions
(\cref{dyn:subsec:smalldivisor}).\\[3pt]
...
\bibitem{Bounemoura}
A.~Bounemoura,
\emph{Optimal linearization of vector fields on the torus in non-analytic
Gevrey classes}, 2020.
\href{https://arxiv.org/abs/2006.05673}{arXiv:2006.05673}.
