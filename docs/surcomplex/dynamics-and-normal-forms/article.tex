...
\newcommand{\rang}{r_{\mathrm{ang}}}
\newcommand{\Oz}{\mathbf{Oz}}
\newcommand{\mergetag}{\textup{\textsf{\small[merge]}}}% text added when source 11 was merged (batch 34)
...
\vspace{0.35in}
{\large 21 September 2026; extended 22 and 23 September 2026\par}
\vspace{0.40in}
...
Ten of the eleven are germ and fixed-point dynamics: multipliers, return
iterates, periods, Hamiltonians at the origin of a phase halo. Source 07
...
of any of the others: none of them contains a resonance lattice, a
...
\emph{resonance} across the sources, and six inequivalent conditions
...
witnessed at a single infinitesimal weight; for an exact \emph{scalar}
multiplier $\lambda I_d$, in every dimension, one common polydisk of the
sharp radius $Re^{-\tau}$ for all coefficients of the normalized conjugacy
and its inverse, which answers the common-domain germ question positively
there; for an exact \emph{nonscalar} diagonal unitary multiplier, one common
polydisk of radius $Re^{-\rang\tau}$, where $\rang$ is the rational rank of
the multiplier's angles, sharp when $\rang=1$, so that the common-domain
question is answered positively for every such multiplier and only the
optimal radius for $\rang\ge2$ and $0<\tau<\infty$ remains open; an exact radius-collapse law
$\rad(h_{k,1})=R\,e^{-(k+1)\sigma}$ in the \emph{drifted}-multiplier
category, which answers the same question negatively there, already over the
cyclic value group $\Z$ with a single Hahn monomial; an exact maximal
centered valuation ball read
off a \emph{finite return iterate}, with its shell cycles, its
cancellation phase diagram and its common-domain lifting; for
rotation-equivariant polynomials, the same ball read off the map's own
coefficients, the exclusion of every other period from the closed ball, an
exact algebraicity criterion for the reduced linearizer with a one-way
transcendence certificate, and reciprocal rational maps whose infinitely
large periodic points are prescribed Gaussian omnific monomials; an algebraicity
...
\textbf{Status.} This is a merged report assembled from eleven AI-assisted,
...
conjecture is claimed solved. The eleven sources' own verification programs
...
\subsection{Eleven manuscripts, one body of machinery}
The first seven source manuscripts were all
...
26a87210\allowbreak 292ec19b}. Sources 08 and 09 were written
independently on 22 September 2026 against commit
\texttt{a3124af7\allowbreak 9f66b8b9\allowbreak c196d76b\allowbreak
4cbc5ac3\allowbreak 938907c4}. Source 10 was written independently on
22 September 2026 against commit
\texttt{048b72cf\allowbreak 7cbfc8ab\allowbreak 246e4f73\allowbreak
788c1046\allowbreak 0cb3f6e0}, which already contained sources 08 and 09
as merged here. Source 11 was written in September 2026 against commit
\texttt{9ee2dac0\allowbreak d234c904\allowbreak 15f6b729\allowbreak
0cbe5b4d\allowbreak cf4a4d3e}, at which this report already contained
sources 01--10 as merged here, but it did not read this report. All eleven
share a single body
...
near-identity inverse --- and each of the eleven states that machinery
from scratch. The positive-support lemma is stated in all eleven. Between $45$
and $55$ per cent of the combined text of sources 01--06 is the same
theory, and sources 07--11 restate the same foundation again.
...
Sources 08 and 09 belong to the germ half and were integrated last, as
\cref{dyn:esm:sec} and \cref{dyn:cyc:subsec}. Both answer
\cref{dyn:q:commongerm}, which sources 01--07 had left open for an exact
multiplier. Source 08 answers it for a scalar multiplier $\lambda I_d$ in
every dimension, and source 09 in one variable; both obtain the same sharp
radius $Re^{-\tau}$. Source 08 also answers \cref{dyn:q:cyclic}, negatively,
in the one-variable drifted setting in which it was posed. The two
manuscripts prove the same theorem by different routes, and they contradict
neither each other nor any earlier source. The positive answer lies in the
exact category $\sdex$ and the negative one in the drifted category $\sddr$,
which \cref{dyn:sec:notions} keeps apart. Source 08 is the base of their
merge, because it has the weaker hypotheses (every $d$) and also carries the
drifted theorem. Source 09 contributes the sharper tree bound
\cref{dyn:esm:thm:treebound}, the forward plane-tree route of
\cref{dyn:tree:subsec}, the parameter and higher-rank remarks, and its
hypothesis audit. Where their statements coincide, the result is printed once
and both sources are credited.

Source 10 also belongs to the germ half and was integrated after sources 08
and 09, as \cref{dyn:nsc:subsec} and the five subsections after it, at the
end of \cref{dyn:esm:sec}. It answers the case of \cref{dyn:q:commongerm}
that sources 08 and 09 had left open, a \emph{nonscalar} exact diagonal
unitary multiplier. For every such multiplier with $\tau<\infty$ it gives
one common polydisk of radius $Re^{-\rang\tau}$, where $\rang$ is the
rational rank of the angles (\cref{dyn:nsc:thm:main}); this is sharp when
$\rang=1$, which includes every nonresonant scalar multiplier, and also
when $\tau=0$ at every angular rank. For $\rang\ge2$ and $0<\tau<\infty$
the optimal radius is left open (\cref{dyn:nsc:thm:criterion}). Source 10 read
this report after sources 08 and 09 had been merged, cites their scalar
theorem rather than claiming it, and contradicts no earlier source. Its
proof is a genuinely different route; in the scalar case it is kept as a
third route to the rate of \cref{dyn:esm:cor:chain}(ii)
(\cref{dyn:nsc:rem:scalar}).

Source 11 (batch 34, manuscript 08) also belongs to the germ half and was
integrated last, as \cref{dyn:rd:subsec} and the six subsections after it,
at the end of \cref{dyn:sec:return}. It treats polynomials that commute with
a rotation of order $q$ and have multiplier $\zeta+e$, with $\zeta$ a
primitive $q$-th root of unity and $e$ infinitesimal: a $\resq$ family. For
them it reads the exact ball of \cref{dyn:thm:exact} off the coefficients of
the map instead of its $q$-th return, which is an independent re-derivation
of that theorem and of \cref{dyn:thm:shell} in this case
(\cref{dyn:rd:prop:return}). Beyond that it excludes every other period
from the closed ball, decides algebraicity of the reduced linearizer by a
finite residue criterion that certifies transcendence of the full one,
classifies the infinitesimal periodic points of integral equivariant maps,
and builds reciprocal rational maps with prescribed Gaussian omnific
periodic points. It contradicts no earlier source. In one subcase its
one-way transcendence certificate is superseded by \cref{dyn:thm:dichotomy}
(\cref{dyn:rd:rem:superseded}). Its eleven questions are added to
\cref{dyn:sec:questions}.
...
(\texttt{docs/surcomplex/analysis/}). All eleven sources restate it; we cite
...
(\texttt{docs/surcomplex/differential-equations/}). Sources 01--10 all
derive it independently (source 11 uses only formal coordinate derivatives
and does not discuss the scalar derivation) --- source 07 in the form that its torus
...
\cite{CarlettiMarmi,Yoccoz,BuffCheritat,Marmi}; tree expansions for
linearization and Davie-type small-divisor counting
\cite{FauvetMenousSauzin,FMS26,Marmi}; rank-one ultrametric
...
\cref{dyn:thm:trichotomy} it yields; from sources 08 and 09, the
fixed-complexity tree bound \cref{dyn:esm:thm:treebound}, the sharp common
radius \cref{dyn:esm:thm:main} for an exact scalar multiplier, and the
single-exponent collapse \cref{dyn:cyc:thm} with its collision
noncancellation \cref{dyn:cyc:prop:noncancel}; the exact finite-return threshold
...
and, from source 10, the rate-count lemma \cref{dyn:nsc:lem:height}, the
rational-rank product bound \cref{dyn:nsc:thm:products}, and their
consequence, the nonscalar common domain \cref{dyn:nsc:thm:main} with its
rank-one sharpness \cref{dyn:nsc:thm:criterion} and the two-mode example
\cref{dyn:nsc:prop:twomode}; and, from source 11, the closed-ball period
exclusion \cref{dyn:rd:prop:periodfinite}, the algebraicity criterion and
transcendence certificate \cref{dyn:rd:thm:algebraicity} for Hahn-valued
data, the classification \cref{dyn:rd:thm:allperiods}, and the realization
theorems \cref{dyn:rd:thm:allshells,dyn:rd:thm:omnific}.
...
Sources 08 and 09 credit the Davie estimates discussed by Marmi and the tree
expansions of Fauvet, Menous and Sauzin
\cite{Marmi,FauvetMenousSauzin,FMS26}. Their clustering-and-packing
estimate is of the same kind as that classical counting. Source 08
inspected \cite{FauvetMenousSauzin} at abstract level only, and both sources
disclaim priority (\cref{dyn:esm:rem:antecedents};
\ref{dyn:esm:nc:priority08}, \ref{dyn:esm:nc:priority09}). The priority of
their contributions is correspondingly uncertain. Source 10 credits the
same tree expansions, inspecting the HTML text of their revision of
13 September 2026, including their
admissible-cut closure for forests whose associated operators are not
universally zero, and the resulting homogeneity-degree constraints
\cite[Thm.~E and Prop.~5.6]{FMS26}, and Carletti's
tree formulas for inversion and linearization over non-Archimedean
formal-series structures \cite{CarlettiLagrange}, which it saw at abstract
level only. It claims only the rank count and its common-domain consequence,
and disclaims priority (\ref{dyn:nsc:nc:priority}). Source 11 credits
Lindahl's rank-one linearization and small-divisor dichotomy
\cite{Lindahl}, Poonen's algebraic closedness \cite{Poonen} and Neumann's
lemma in Marker's notes \cite{Marker}, and disclaims priority
(\ref{dyn:rd:nc:priority}); its own novelty assessment did not include this
report.
...
The sources use the word \emph{resonance} for five inequivalent
...
$\nres$ reads $\lambda^{n}\ne\lambda$ for $n\ge2$.
&
This is the hypothesis of \cref{dyn:thm:main,dyn:thm:lifting,dyn:thm:trichotomy,dyn:esm:thm:main,dyn:nsc:thm:main,dyn:cyc:thm}
...
in \cref{dyn:thm:main,dyn:esm:thm:main,dyn:nsc:thm:main} and \emph{drifted} in
\cref{dyn:thm:collapse,dyn:cyc:thm}; see \cref{dyn:warn:drift}.\\[4pt]
...
$v(u)>0$.
&
Sources 02 and 08. The drift is not decoration: it is what creates the
higher reciprocal jets $c_{n,k}$, $k\ge1$, whose poles of order $k+1$ drive
the collapse law. \Cref{dyn:warn:drift} records that removing drift removes
the mechanism. Source 08's \cref{dyn:cyc:thm} needs the drift but not a
second independent exponent.\\[4pt]
...
&
Sources 03, 08, 09 and 10. No infinitesimal linear part is permitted in
\cref{dyn:thm:main} or in \cref{dyn:esm:sec}. Source 10 adds a second
invariant of the same multiplier, the rational angular rank $\rang$ of
\eqref{dyn:nsc:eq:rank}, which bounds how many distinct exponential rates
the divisors can show at fixed tree complexity; it is an arithmetic
invariant of the angles, not a divisor condition. Infinitesimal linear terms
\emph{are} permitted in the identity-reducing flow theorem
\cref{dyn:thm:exp-log}.\\[4pt]
...
\emph{not} share is the sixth category, common-domain germs. At the pins of
sources 01--07 it was decided only on the drifted family. Sources 08 and 09
have since decided it on the exact family for a \emph{scalar} multiplier,
and source 10 for every diagonal unitary exact multiplier; the exact and
drifted answers are different:
\begin{center}
\begin{tabular}{@{}P{0.25\textwidth}P{0.36\textwidth}P{0.30\textwidth}@{}}
\toprule
Sixth category & exact family $\lambda^{\alpha}-\lambda_j$ ($\sdex$)
& drifted family $\lambda^{n}-\lambda$ ($\sddr$)\\
\midrule
common-domain germs $\mathcal R_n$
& $\kappa<\infty$, with universal radius $Re^{-\kappa}$, for
 $\Lambda=\lambda I_d$ (\cref{dyn:esm:thm:main}); $\kappa<\infty$ also for
 nonscalar $\Lambda$, with universal radius between $Re^{-\rang\kappa}$
 and $Re^{-\kappa}$, equal to $Re^{-\kappa}$ when $\rang=1$
 (\cref{dyn:nsc:thm:criterion}), and equal to $R$ for $\kappa=0$ at every
 angular rank; the optimal radius for $\rang\ge2$ and $0<\kappa<\infty$ is
 \textbf{open} (\cref{dyn:q:commongerm})
& $\kappa=0$ (\cref{dyn:thm:trichotomy}), already over any fixed nonzero
 $\Gamma$, including $\Z$ (\cref{dyn:cyc:cor:classification})\\
\bottomrule
\end{tabular}
\end{center}
These two entries belong to different categories and must never be merged
into one row (\cref{dyn:warn:resolution}). No source decides the sixth
category on the frequency family.
...
\cref{dyn:rd:thm:linearization,dyn:rd:prop:return} equivariant ball
& $\resq$, $f(\zeta z)=\zeta f(z)$ & $\sdz$ & formal, $\BB_{\mathsf M}$ & $B_{r}$ (valuation)\\
\cref{dyn:rd:prop:periodfinite,dyn:rd:thm:shell} closed-ball periods, shell
& $\resq$, equivariant & $\sdz$ & ordinary polynomial $R\in\C[U]$ & $\bar B_{r}$, $S_{r}$\\
\cref{dyn:rd:thm:reduction,dyn:rd:thm:algebraicity} boundary reduction
& $\resq$, equivariant & $\sdz$ & $\C[[X]]$; transcendence over $K_\Gamma(z)$ & none\\
\cref{dyn:rd:thm:allperiods} infinitesimal periods
& residue of order $q$; $\lambda=\zeta$ allowed & none used & integral polynomial & $\m_\Gamma$\\
\cref{dyn:rd:thm:allshells,dyn:rd:thm:omnific} prescribed shells
& $\resq$, equivariant & $\sdz$ & polynomial; rational over $\Oz[i]$ & $\m_\Gamma$; infinitely large points\\
...
& $\nres$ & $\sdex$ & germ / fixed polydisk & $\radiso$, \emph{lower bound
only}; superseded for $\Lambda=\lambda I_d$ by \cref{dyn:esm:thm:main}, and
improved to $\min(\rang,\ell_S(\gamma))\tau$ by \cref{dyn:nsc:cor:local}\\
\cref{dyn:esm:thm:treebound} tree product bound
& $\nres$, one rotation & arithmetic only; usable under $\sdex$ & none
& none\\
\cref{dyn:esm:thm:main} sharp common radius
& $\nres$, exact $\lambda I_d$ & $\sdex$ & common-domain germs
$\mathcal R_d$ (sixth category) & $\mathbb D^{d}_{Re^{-\tau}}$,
$\radiso$\\
\cref{dyn:tree:prop:estimate} route D
& $\nres$, exact, $d=1$ & $\sdex$ & $\mathcal R_1$ & $\rad$\\
\cref{dyn:nsc:lem:height,dyn:nsc:cor:angular} rate count
& none & arithmetic only: angles and integer heights & none & none\\
\cref{dyn:nsc:thm:products} colored product bound
& $\nres$, diagonal unitary & $\sdex$ rate, with $\rang$ & none & none\\
\cref{dyn:nsc:thm:main} nonscalar common radius
& $\nres$, exact diagonal $\Lambda$ & $\sdex$ & common-domain germs
$\mathcal R_d$ & $\mathbb D^{d}_{Re^{-\rang\tau}}$, $\radiso$\\
\cref{dyn:nsc:thm:criterion} criterion, rank-one sharpness
& $\nres$, exact diagonal $\Lambda$ & $\sdex$, \textbf{iff} $\tau<\infty$
& $\mathcal R_d$ & radius in $[Re^{-\rang\tau},Re^{-\tau}]$; exact if
$\rang=1$\\
\cref{dyn:nsc:cor:local} input-relative radius
& $\nres$, exact diagonal $\Lambda$ & $\sdex$ & germ & $\radiso$, lower
bound\\
...
\cref{dyn:cyc:thm} single-exponent collapse
& $\nres$ & $\sddr$, \textbf{drift required}; one monomial over $\Z$
& germ yes, common domain no & $\rad$\\
\cref{dyn:thm:trichotomy} trichotomy
& $\nres$ & $\sddr$ & common domain vs germ; over $\Gamma_{*}$, and over
$\Z$ by \cref{dyn:cyc:cor:classification} & $\mathbb D_R$\\
\cref{dyn:cyc:cor:classification} fixed-group criterion
& $\nres$ & $\sddr$ & common domain iff $\sigma=0$, any fixed $\Gamma$
& $\mathbb D_R$\\
\cref{dyn:thm:multicollapse} several-variable collapse
& $\nres$ & $\sddr$, multiplicative detuning & polydisk germ; over
$\Gamma_{*}$ only & $\radiso$\\
...
$0<\sigma<\infty$. Source 08 later removed the second exponent but not the
drift: \cref{dyn:cyc:thm,dyn:cyc:cor:classification,dyn:cyc:cor:onecoef}
work over $\Z$ with one Hahn monomial and still \textbf{require} drift. The
exact laws $\rad(h_{k,1})=R\,e^{-(k+1)\sigma}$ and
$\rad(h^{\mathrm{cyc}}_m)=R\,e^{-m\sigma}$ must therefore never be quoted
without the drift hypothesis. In particular they must not be quoted as an
answer in the exact-multiplier category $\sdex$ of \cref{dyn:thm:main},
where for a scalar multiplier the answer is now known to be the opposite:
one common radius $Re^{-\tau}$ (\cref{dyn:esm:thm:main}). For a nonscalar
diagonal unitary multiplier with $\tau<\infty$ it is also the opposite: one
common radius of at least $Re^{-\rang\tau}$ (\cref{dyn:nsc:thm:main}). See
\cref{dyn:subsec:resolution} for the resolution of this apparent tension
with \cref{dyn:q:commongerm}.
\end{warning}
...
All eleven sources set up this material independently. It is stated once
here.
...
phase residue of the scalar derivation. Sources 01--10 all derive this
...
intermediate regime. For a scalar multiplier $\lambda I_d$ that is done by
\cref{dyn:esm:thm:main}; for a nonscalar diagonal one by
\cref{dyn:nsc:thm:criterion}, except for the value of the optimal radius
when $\rang\ge2$ and $0<\tau<\infty$, which is \cref{dyn:q:commongerm}.
\end{convention}
...
claimed as a contribution. All eleven sources state it; source 11 cites
it from Marker's notes \cite[Lemma~2.36]{Marker}; sources 02, 05 and 09
...
printed again. Source 10 derives it from Higman's lemma, cited as a
classical theorem. Source 08 instead reduces the lemma to Poonen's
positive-support theorem over a free associative algebra
(\cref{dyn:esm:rem:duplicates}). Source 07 uses the \emph{full} finiteness assertion, across
...
\emph{finite-return} threshold \cref{dyn:thm:exact} exact. A fourth route,
source 09's forward plane-tree expansion (route D,
\cref{dyn:tree:subsec}), computes the inverse coordinate $H^{-1}$ directly
as a sum over full plane trees. It exposes the branching case of the tree
bound \cref{dyn:esm:thm:treebound}, and it shares that bound's arithmetic
lemmas with route A as route A is used in \cref{dyn:esm:sec}. Source 10
extends route D to several variables and nonscalar $\Lambda$ by colored
trees (\cref{dyn:nsc:prop:formal}); its arithmetic is its own
(\cref{dyn:nsc:rem:scalar}). Source 02 also
...
radius-collapse phenomenon. In the exact scalar category the interchange
does hold, at the cost of exactly one factor $e^{-\tau}$ of radius
(\cref{dyn:esm:thm:main}), and for every exact diagonal unitary multiplier
it holds at the cost of at most $e^{-\rang\tau}$ (\cref{dyn:nsc:thm:main});
the noncommutation is a phenomenon of drift.
...
collapse occurs. That question is \cref{dyn:q:commongerm}. At the pins of
sources 01--07 the only source that answered it did so in a different
category, the drifted one, and \cref{dyn:sec:drift} keeps the two apart.
For a scalar multiplier $\Lambda=\lambda I_d$, sources 08 and 09 have since
shown that the depth-dependent factor is never needed: every coefficient has
isotropic radius at least $Re^{-\tau}$ (\cref{dyn:esm:thm:main}). For a
nonscalar $\Lambda$ the question remained open at the pins of sources 08
and 09, and this paragraph said so. Source 10 has since shown that every
coefficient has isotropic radius at least $Re^{-\rang\tau}$, with $\rang$
the rational angular rank (\cref{dyn:nsc:thm:main}), and that
\eqref{dyn:eq:radius-depth} holds with $\ell_S(\gamma)$ replaced by
$\min(\rang,\ell_S(\gamma))$ (\cref{dyn:nsc:cor:local}). What remains open
is the optimal radius when $\rang\ge2$ and $0<\tau<\infty$ (\cref{dyn:q:commongerm}).
\end{warning}
...
\section{The exact scalar multiplier: one radius loss}
\label{dyn:esm:sec}

This section is contributed by sources 08 and 09, written on 22 September
2026 against repository commit
\texttt{a3124af7\allowbreak 9f66b8b9\allowbreak c196d76b\allowbreak
4cbc5ac3\allowbreak 938907c4}, after sources 01--07 had been merged. It
answers \cref{dyn:q:commongerm} for a \emph{scalar} exact multiplier
$\Lambda=\lambda I_d$: source 08 in every finite dimension $d$, source 09
in one variable. The answer is positive and comes with a sharp constant. The
common-domain germ category, which \cref{dyn:thm:main} left undecided for
$0<\tau<\infty$, costs exactly one factor $e^{-\tau}$ of radius, however
deep the Hahn words are. Sources 08 and 09 do \emph{not} settle the
nonscalar diagonal case of \cref{dyn:q:commongerm}
(\cref{dyn:esm:subsec:limits}). Source 10, merged later, settles it up to
the optimal radius, in \cref{dyn:nsc:subsec} and the five subsections after
it, which close this section.

\emph{Tags.} Resonance $\nres$, with the multiplier \textbf{exactly}
$\lambda I_d$; divisor notion $\sdex$; category the common-domain germ ring
$\mathcal R_d$ of \eqref{dyn:eq:rings}, with a specified radius; domain
notion $\mathbb D_R^{d}$ and $\radiso$. No drift is permitted anywhere in
this section. The drifted counterpart, which has the opposite answer, is
\cref{dyn:sec:drift}, and in particular \cref{dyn:cyc:subsec}.

\begin{convention}[Setting, rate, and the two coordinate conventions]
\label{dyn:esm:conv}
\begin{enumerate}[label=(\roman*)]
\item Let $\lambda=e^{2\pi i\theta}$ with $\theta$ irrational. For the scalar
matrix $\Lambda=\lambda I_d$ this is exactly $\nres$, because
$\lambda^{\alpha}-\lambda=\lambda(\lambda^{|\alpha|-1}-1)$. Write
\begin{equation}\label{dyn:esm:eq:rate}
 \mathfrak d_n=|\lambda^{n}-1|\quad(n\ge1),\qquad
 \tau=\tau(\lambda I_d)=\limsup_{n\to\infty}
 \frac{\log^{+}\mathfrak d_n^{-1}}{n}\in[0,\infty].
\end{equation}
Since $|\lambda^{\alpha}-\lambda_j|=\mathfrak d_{|\alpha|-1}$ when
$\Lambda=\lambda I_d$, this is the rate $\tau(\Lambda)$ of
\cref{dyn:subsec:smalldivisor}; the index shift does not change the limsup.
In one variable \cref{dyn:prop:cf} makes it numerically equal to
$\sigma(\lambda)$, but the hypotheses that accompany the two rates remain
different (\cref{dyn:warn:drift}). The divisor $d_n=\lambda^{n}-\lambda$ of
\cref{dyn:sec:drift} satisfies $|d_{n+1}|=\mathfrak d_n$.
\item The value group $\Gamma$ is any nonzero set-sized ordered abelian
group. No divisibility, rank, countability or algebraic-closure hypothesis
is made: source 08 notes that algebraic closedness of the workspace is used
nowhere, and source 09 excludes Archimedean, divisibility, countability and
finite-rank hypotheses. With $S\subset\Gamma_{>0}$ well ordered, the input is
\begin{equation}\label{dyn:esm:eq:input}
 F(z)=\lambda z+f(z),\qquad f=\sum_{s\in S}t^{s}f_s,\qquad
 f_s\in\OO(\mathbb D_R^{d})^{d},\quad f_s(0)=0,\quad \mathrm Df_s(0)=0,
\end{equation}
the last two conditions holding coefficientwise. This is
\eqref{dyn:eq:Fintro} with $\Lambda=\lambda I_d$ and
$B=\OO(\mathbb D_R^{d})$. No common bound on the sizes of the $f_s$ is
assumed.
\item \emph{Conjugacy convention.} As everywhere in this report,
$H=z+h$ and $H\circ F=\lambda H$ (\eqref{dyn:eq:Schroeder}); source 08
uses this convention. Source 09 works in the \emph{forward} convention
$F\circ\check H=\check H(\lambda\,\cdot)$. Its forward coordinate is our
$\check H=H^{-1}=z+\check h$, and its inverse coordinate is our $H$.
Every formula taken from source 09 below has been translated into this
notation. In particular its first sharp coefficient is our
$\check h_\eta=-h_\eta$, and what it calls the linearizer is our $H^{-1}$.
\item \emph{Local notation, chosen to avoid collisions.} $\|x\|_{\Z}$ is
the distance from $x$ to the nearest integer. $q_*$ and $p_*/q_*$ are the
cluster denominator and fraction of \cref{dyn:esm:lem:cluster}; they are
neither the order $q$ of $\resq$ nor a convergent denominator $q_j$. A
forest has $m$ vertices, positive integer weights $w_v$, subtree sums
$s_v$ and total weight $n$, so that $n+1$ is a Taylor degree; $n$ is
\emph{not} the nonlinear part $N$ of \cref{dyn:thm:lifting}. The numbers
$a>\tau$ and $C_a$ are those of \eqref{dyn:esm:eq:Ca}, and $r<R$ is an
auxiliary ordinary radius, not the finite-return threshold $r$ of
\cref{dyn:sec:return}. The radius $Re^{-\tau}$ is always written out;
$\rho$ keeps its meaning from \cref{dyn:subsec:radius}.
\end{enumerate}
\end{convention}

\subsection{Exceptionally small divisors share one rational direction}
\label{dyn:esm:subsec:arith}

As in the proof of \cref{dyn:prop:cf},
\begin{equation}\label{dyn:esm:eq:sine}
 4\|n\theta\|_{\Z}\le\mathfrak d_n\le2\pi\|n\theta\|_{\Z}.
\end{equation}

\begin{lemma}[One rational cluster; sources 08 and 09]
\label{dyn:esm:lem:cluster}
Let $n\ge1$, and call $s\in\{1,\dots,n\}$ \emph{marked} if
$\|s\theta\|_{\Z}<1/(4n)$. If some $s$ is marked, there is a reduced
fraction $p_*/q_*$ with $1\le q_*\le n$ such that every marked $s$ is a
multiple $s=kq_*$ whose nearest integer to $s\theta$ is $kp_*$, and
\[
 \|q_*\theta\|_{\Z}<\frac1{4n},\qquad
 \|s\theta\|_{\Z}=k\|q_*\theta\|_{\Z},\qquad
 \mathfrak d_s\ge\mathfrak d_{q_*}.
\]
\end{lemma}
\begin{proof}
Let $p_s$ be the nearest integer to $s\theta$. For marked $s,s'$ the
integer $p_ss'-p_{s'}s$ has absolute value at most
$s'|p_s-s\theta|+s|p_{s'}-s'\theta|<(s+s')/(4n)\le\frac12$, so it vanishes.
All the fractions $p_s/s$ therefore equal one reduced fraction $p_*/q_*$,
and $s=kq_*$, $p_s=kp_*$. Then $|q_*\theta-p_*|=|s\theta-p_s|/k<1/(4n)$,
so $p_*$ is the nearest integer to $q_*\theta$ and
$\|s\theta\|_{\Z}=k\|q_*\theta\|_{\Z}\ge\|q_*\theta\|_{\Z}$. Both distances
are below $\frac14$, where $x\mapsto|e^{2\pi ix}-1|=2\sin(\pi x)$ is
increasing, so $\mathfrak d_s\ge\mathfrak d_{q_*}$.
\end{proof}

Source 09 proves the lemma in this form. Source 08 uses the cutoff
$\mathfrak d_s<n^{-3}$ with the same determinant argument and obtains
$\mathfrak d_{kq_*}\ge(2/\pi)k\,\mathfrak d_{q_*}$. By its own account that
cutoff is convenient rather than optimal, and it costs a factor $n^{3}$ for
each unmarked divisor where the cutoff above costs $n$. It is one argument,
printed once.

\begin{lemma}[Divisible-subtree packing; sources 08 and 09]
\label{dyn:esm:lem:packing}
Let a nonempty finite rooted forest carry positive integer vertex weights
$w_v$, with subtree sums $s_v$ (the subtree of $v$ contains $v$) and total
weight $n$. For every integer $q\ge1$ and every set $A$ of vertices with
$q\mid s_v$ for all $v\in A$,
\begin{equation}\label{dyn:esm:eq:pack}
 q\,|A|\le n .
\end{equation}
\end{lemma}
\begin{proof}
For $v\in A$ let $C(v)$ be the set of closest strict descendants of $v$ that
lie in $A$, and let $B_v$ be the vertex set of the subtree of $v$ with the
subtrees of the members of $C(v)$ removed. $B_v$ contains $v$, so its weight
$b_v=s_v-\sum_{u\in C(v)}s_u$ is positive; it is divisible by $q$, hence
$b_v\ge q$. A vertex lies in $B_v$ only for its closest ancestor-or-self in
$A$, so the sets $B_v$ are pairwise disjoint and $\sum_{v\in A}b_v\le n$.
\end{proof}

Both sources give this proof; source 08 phrases it through the forest on
the marked vertices. It is printed once.

\begin{warning}[Strict positivity is exactly what drift destroys]
\label{dyn:esm:warn:zero}
With a zero weight \cref{dyn:esm:lem:packing} fails. A two-vertex chain with
weights $0$ and $q$ has both subtree sums equal to $q$ and total weight $q$
(source 09). A chain of $m$ vertices whose only positive weight $n$ sits at
its leaf has every subtree sum equal to $n$, so its divisor product is
$\mathfrak d_n^{-m}$, of exponential rate $m\tau$ (source 08). Both sources
identify this as the combinatorial model of a \emph{linear} multiplier
perturbation, since a vertex of arity one consumes no Taylor degree. It is
the mechanism behind \cref{dyn:thm:collapse} and \cref{dyn:cyc:lem:chains}.
\end{warning}

For $\tau<\infty$ and $a>\tau$ put
\begin{equation}\label{dyn:esm:eq:Ca}
 C_a=\max\Bigl(1,\sup_{s\ge1}e^{-as}\mathfrak d_s^{-1}\Bigr)<\infty,
 \qquad\text{so that}\qquad \mathfrak d_s^{-1}\le C_ae^{as}.
\end{equation}

\begin{theorem}[Tree product bound; source 09]\label{dyn:esm:thm:treebound}
Let $\tau<\infty$ and $a>\tau$. For every nonempty finite rooted forest with
$m$ vertices, positive integer weights and total weight $n$,
\begin{equation}\label{dyn:esm:eq:treebound}
 \prod_{v}\mathfrak d_{s_v}^{-1}\le(C_an)^{m}e^{an}.
\end{equation}
\end{theorem}
\begin{proof}
Mark $v$ when $\|s_v\theta\|_{\Z}<1/(4n)$; since $1\le s_v\le n$ this is
the marking of \cref{dyn:esm:lem:cluster}. An unmarked factor is at most $n$
by \eqref{dyn:esm:eq:sine}. If $M\ge1$ vertices are marked,
\cref{dyn:esm:lem:cluster} supplies $q_*$ dividing all their $s_v$ and bounds
each marked factor by $\mathfrak d_{q_*}^{-1}\le C_ae^{aq_*}$, and
\cref{dyn:esm:lem:packing} gives $Mq_*\le n$. Hence the product is at most
$n^{m-M}C_a^{M}e^{aMq_*}\le(C_an)^{m}e^{an}$.
\end{proof}

The small divisors are paid for once, by disjoint blocks of Taylor weight,
and not once per nested subtree along a path. That is the gain over
applying \cref{dyn:lem:radius-loss} at every inversion. The inequality holds
for every $m$, but $(C_an)^{m}$ is polynomial in $n$ only for fixed $m$.
Source 09 makes no assertion that it is harmless uniformly in $m$.

\begin{corollary}[Strict chains and the fixed-complexity rate; sources 08 and 09]
\label{dyn:esm:cor:chain}
\begin{enumerate}[label=(\roman*)]
\item If $\tau<\infty$ and $a>\tau$, every strictly increasing chain
$1\le k_1<\cdots<k_r\le n$ satisfies
\begin{equation}\label{dyn:esm:eq:chain}
 \prod_{j=1}^{r}\max\bigl(1,\mathfrak d_{k_j}^{-1}\bigr)\le(C_an)^{r}e^{an}.
\end{equation}
\item Let $\tau\in[0,\infty]$ and fix $m\ge1$. Let $A_m(n)$ be the maximum of
$\prod_v\mathfrak d_{s_v}^{-1}$ over rooted trees of total weight $n$ with
at most $m$ vertices. Then, in the extended sense,
\begin{equation}\label{dyn:esm:eq:treerate}
 \limsup_{n\to\infty}\frac{\log^{+}A_m(n)}{n}=\tau .
\end{equation}
The same holds for trees with exactly $m$ vertices and $n\ge m$, and for
the maximum over chains of length at most $m$ in \eqref{dyn:esm:eq:chain}.
The exponential rate at fixed complexity is $\tau$, not $m\tau$.
\end{enumerate}
\end{corollary}
\begin{proof}
(i) The factors exceeding one form a subchain
$k_{j_1}<\cdots<k_{j_{r'}}$. It is the list of subtree sums of the path whose
leaf has weight $k_{j_1}$ and whose successive vertices towards the root
have weights $k_{j_{i+1}}-k_{j_i}>0$. Its total weight is at most $n$, so
\cref{dyn:esm:thm:treebound} bounds the product by
$(C_an)^{r'}e^{an}\le(C_an)^{r}e^{an}$.
(ii) If $\tau<\infty$, \cref{dyn:esm:thm:treebound} gives
$n^{-1}\log A_m(n)\le a+m\log(C_an)/n$; let $n\to\infty$ and then
$a\downarrow\tau$. The one-vertex tree of weight $n$ has product
$\mathfrak d_n^{-1}$, whose exponential rate is $\tau$. The star with $m-1$
leaves of weight one and root weight $n-m+1$ has product
$\mathfrak d_n^{-1}\mathfrak d_1^{-(m-1)}$, with the same rate. These lower
bounds also cover $\tau=\infty$. The one-element chain $k_1=n$ does the
same for chains.
\end{proof}

Source 08 proves (i) with a different constant and the polynomial factor
$n^{3r}$, which comes from its cutoff, and proves a tree version with
repeated subtree sums allowed. The forest form and the sharper polynomial
factor are source 09's.

\begin{remark}[Which uniformity is obtained, and which is not]
\label{dyn:esm:rem:uniformity}
The exponential constant $a$ does not depend on the number of vertices; the
polynomial factor does. That suits Hahn coefficients, where the number of
vertices is finite at each exponent but unbounded over the support. It gives
no estimate for summing all depths in an ordinary complex parameter; both
sources say so. Source 09 makes the point concrete. For $\lambda z+t^{\eta}z^{2}$
every internal vertex of the tree expansion of
\cref{dyn:tree:prop:formula} has weight one, so $m=n$ and
$(C_an)^{m}=(C_an)^{n}$. The estimate then gives no ordinary analytic
convergence, and no hidden uniform estimate lets the two quantifier orders
be exchanged. This agrees with \cref{dyn:prop:quadratic-scale}.
\end{remark}

\begin{remark}[Antecedents of the arithmetic]\label{dyn:esm:rem:antecedents}
Grouping the exceptionally small divisors under one rational approximant,
and paying for repeated small divisors with disjoint pieces of degree, is
the same kind of argument as classical small-divisor counting. Examples are
the Davie estimates discussed by Marmi \cite{Marmi} and the tree expansions
of Fauvet, Menous and Sauzin \cite{FauvetMenousSauzin,FMS26} with their
Bruno estimates. Both sources credit these. Neither claims the framework or
the tree method, and neither certifies priority. Source 08 inspected
\cite{FauvetMenousSauzin} at abstract and metadata level only, and both
describe their literature comparison as targeted (\ref{dyn:esm:nc:priority08},
\ref{dyn:esm:nc:priority09}). What they offer is the fixed-complexity form above.
Polynomial losses are allowed there because Hahn support keeps the number of
vertices finite before any ordinary Taylor convergence is considered.
\end{remark}

\subsection{Why the divisors are distinct: strict degree chains}
\label{dyn:esm:subsec:chains}

Use route A, \cref{dyn:prop:linearization}, with $\Lambda=\lambda I_d$. On
a coordinate monomial the homological operator acts by
$\Li(z^{\beta}e_j)=(\lambda^{|\beta|}-\lambda)z^{\beta}e_j$. So an inversion
at total degree $n+1$ divides by a number of modulus $\mathfrak d_n$, and
this number depends only on $n$. An \emph{expanded term} of
\eqref{dyn:eq:linearizer-series} consists of one choice of each of the
following: a support word; a grouping of it into the Taylor blocks of
$\NN_f$ (\eqref{dyn:eq:Noperator}); the coordinate component of every
perturbation factor; and one ordinary Taylor monomial of each such factor.

\begin{lemma}[Strict degree chains; source 08]\label{dyn:esm:lem:degreechain}
Consider an expanded term with $m$ perturbation factors that contributes to
a coordinate monomial of total degree $n+1$.
\begin{enumerate}[label=(\roman*)]
\item Its input Taylor degrees $n_1,\dots,n_m$ are at least two and satisfy
$\sum_jn_j=n+m$.
\item Its total order of differentiation is $m-1$.
\item Its homological divisors have moduli
$\mathfrak d_{k_1},\dots,\mathfrak d_{k_r}$ along a strictly increasing
chain $1\le k_1<\cdots<k_r=n$ with $r\le m$.
\end{enumerate}
\end{lemma}
\begin{proof}
Initially the degree is $n_1$ and one divisor is used. A Taylor block of
order $k=|\kappa|$ differentiates the current monomial $k$ times and
multiplies it by $k$ new perturbation monomials of degrees
$n_{i+1},\dots,n_{i+k}$. The net change of degree is
$\sum_{l=1}^{k}(n_{i+l}-1)\ge k\ge1$, so the next inversion takes place at
a strictly larger degree. At the end
$n+1=n_1+\sum_{j\ge2}(n_j-1)=1+\sum_{j=1}^{m}(n_j-1)$, which is (i). The
orders of all blocks add up to $m-1$, which is (ii). Each new inversion comes
with at least one new letter, which is (iii).
\end{proof}

The hypothesis $\mathrm Df_s(0)=0$ is used exactly in $n_j-1\ge1$ (source
08; source 09's audit, \cref{dyn:esm:tab:audit}). With $f_s(0)=0$ alone,
factors of degree one give zero increments and the chain is no longer
strict. That is \cref{dyn:esm:warn:zero} again.

\begin{example}[Two divisors at the second order; sources 08 and 09]
\label{dyn:esm:ex:second}
Let $d=1$, $F=\lambda z+t^{\eta}g$ with $g=\sum_{k\ge2}g_kz^{k}$, and
$H=z+\sum_{j\ge1}t^{j\eta}u_j$. Then $\Li u_1=-g$,
$\Li u_2=-g\,u_1'(\lambda z)$, and
\begin{equation}\label{dyn:esm:eq:second}
 u_2(z)=\sum_{i,k\ge2}
 \frac{k\lambda^{k-1}g_ig_k}{(\lambda^{k}-\lambda)(\lambda^{i+k-1}-\lambda)}
 \,z^{i+k-1}.
\end{equation}
The two divisors have net indices $k-1$ and $n=i+k-2$, which are distinct
because $i\ge2$. If both are marked at total weight $n$,
\cref{dyn:esm:lem:cluster} gives a $q_*$ dividing both, and hence dividing
$i-1\ge1$ as well; so $2q_*\le(k-1)+(i-1)=n$. The dangerous pair is paid
for by the two positive pieces $k-1$ and $i-1$, not twice by $n$. Iterating
one-step estimates would lose $e^{-\tau}$ twice; the chain estimate loses it
once. Source 09 makes the analogous computation for the forward coordinate
$\check H$, and \cref{dyn:subsec:quadratic} records both sources'
quadratic coefficients.
\end{example}

\subsection{One common radius at every Hahn depth}
\label{dyn:esm:subsec:radius}

\begin{proposition}[A depth-uniform exponential rate; source 08]
\label{dyn:esm:prop:coefbound}
Assume \eqref{dyn:esm:eq:input} and $\tau<\infty$. For every
$\gamma\in S^{*}\setminus\{0\}$, every $0<r<R$ and every $a>\tau$ there is a
finite $A_{\gamma,r,a}$ with
\begin{equation}\label{dyn:esm:eq:coefbound}
 \max_{\substack{|\beta|=n+1\\1\le j\le d}}\bigl|[z^{\beta}]h_{\gamma,j}\bigr|
 \le A_{\gamma,r,a}\,(n+1)^{(d+4)\ell_S(\gamma)}\,r^{-n}e^{an}
 \qquad(n\ge1).
\end{equation}
The exponent $a$ does not depend on $\gamma$ or on its word depth.
\end{proposition}
\begin{proof}
Only the finitely many words of $\word_S(\gamma)$ contribute
(\cref{dyn:prop:linearization}). For each letter $s$ occurring in them put
$M_s(r)=\max_j\sup_{\overline{\mathbb D_r^{d}}}|f_{s,j}|<\infty$, so that
$|[z^{\nu}]f_{s,j}|\le M_s(r)r^{-|\nu|}$ by Cauchy's estimate. Fix a word of
length $m$ and a grouping into Taylor blocks. By
\cref{dyn:esm:lem:degreechain}(i), the input coefficients of a contribution
of final degree $n+1$ have product at most $r^{-(n+m)}\prod_sM_s(r)$, the
product running over the letters of the word with multiplicity. Every
intermediate degree is at most $n+1$, so a derivative of order $k$ together
with its factorial denominator contributes at most $(n+1)^{k}$; by (ii) these
factors multiply to at most $(n+1)^{m-1}$. Composition with $\lambda z$
contributes factors of modulus one. The $m$ input multi-indices have $dm$
entries summing to $n+m$, so there are at most
$\binom{n+m+dm-1}{dm-1}\le C_{d,m}(n+1)^{dm-1}$ of them. The coordinate
choices and the groupings are finite in number, independently of $n$. By
(iii) and \cref{dyn:esm:cor:chain}(i), the inverse divisors multiply to at
most $(C_an)^{m}e^{an}\le C_a^{m}n^{3m}e^{an}$; the right-hand form is the
one source 08 uses. The total polynomial exponent is therefore at most
$(m-1)+(dm-1)+3m=(d+4)m-2$. Summing over the finitely many words and
groupings gives \eqref{dyn:esm:eq:coefbound}. The bound applies to the sum of
absolute values, so it covers cancellations too.
\end{proof}

\begin{corollary}[One open polydisk for every coefficient; source 08]
\label{dyn:esm:cor:commondomain}
Every $h_\gamma$ lies in $\OO(\mathbb D^{d}_{Re^{-\tau}})^{d}$.
\end{corollary}
\begin{proof}
Fix $r<R$, $a>\tau$ and $0<b<re^{-a}$. There are $\binom{n+d}{d-1}$
monomials of total degree $n+1$, polynomially many, so
\eqref{dyn:esm:eq:coefbound} gives a normally convergent Taylor series on the
closed polydisk of radius $b$. Letting $r\uparrow R$ and $a\downarrow\tau$
exhausts $\mathbb D^{d}_{Re^{-\tau}}$. The Taylor coefficients do not
change, so they define one holomorphic function on the union, and the union
does not depend on $\gamma$.
\end{proof}

Only the \emph{open} polydisk is claimed. There is no boundedness and no
continuation across its boundary, and the constants and polynomial exponents
in \eqref{dyn:esm:eq:coefbound} may grow without bound as $\gamma$ varies
(source 08). A common domain of holomorphy is not a common norm bound
(source 09; compare \cref{dyn:warn:nouniform}).

\begin{theorem}[Sharp common domain for an exact scalar multiplier; sources
08 and 09]\label{dyn:esm:thm:main}
Let $\Gamma$ be any nonzero set-sized ordered abelian group, $d\ge1$,
$\lambda=e^{2\pi i\theta}$ with $\theta$ irrational, and
$\tau=\tau(\lambda I_d)<\infty$. For every input \eqref{dyn:esm:eq:input},
the unique normalized positive conjugacy $H=z+h$ of \cref{dyn:thm:main},
$H\circ F=\lambda H$, and its inverse $H^{-1}=z+\check h$ satisfy
\begin{equation}\label{dyn:esm:eq:main}
 h_\gamma,\ \check h_\gamma\in\OO(\mathbb D^{d}_{Re^{-\tau}})^{d},\qquad
 \ord_0h_\gamma,\ \ord_0\check h_\gamma\ge2,\qquad
 \supp h,\ \supp\check h\subseteq S^{*}\setminus\{0\}.
\end{equation}
The radius $Re^{-\tau}$ is the optimal universal common radius. For every
$\eta\in\Gamma_{>0}$ the input $\lambda z+t^{\eta}P_R(z_1)e_1$, with $P_R$ as
in \eqref{dyn:eq:PR}, has coefficient $h_\eta$ of isotropic Taylor radius
exactly $Re^{-\tau}$. If $\tau=\infty$ the same input has $h_\eta$ of radius
zero, so it has no normalized conjugacy with even separately convergent
coefficient germs. When $\tau<\infty$, $H$ and $H^{-1}$ lie in the
common-domain germ ring $\mathcal R_d$ and evaluate to mutually inverse
bijective conjugacies of the finite halo $(\mathbb D^{d}_{Re^{-\tau}})^{\#}_\Gamma$.
\end{theorem}
\begin{proof}
\Cref{dyn:prop:linearization}, applied in the formal row, gives the unique
normalized positive conjugacy with support in $S^{*}\setminus\{0\}$.
\Cref{dyn:esm:cor:commondomain} shows that each of its coefficients is
holomorphic on $\mathbb D^{d}_{Re^{-\tau}}$. At a fixed Hahn exponent,
$H\circ F$ is computed by \eqref{dyn:eq:rotation-substitution} as a finite
expression (\cref{dyn:lem:neumann}) in functions holomorphic on that
polydisk, which $\lambda$ preserves. Its Taylor series agrees with the
formal one, so the conjugacy identity holds there by the identity theorem.
\Cref{dyn:prop:subgroup}, with $U=\mathbb D^{d}_{Re^{-\tau}}$, gives the
inverse on the same connected domain, with no second loss of radius and with
support in $S^{*}\setminus\{0\}$; it is normalized as in
\cref{dyn:prop:linearization}. Source 08's inverse lemma and source 09's
same-domain inversion are this statement and are not reprinted. Uniqueness
holds in the formal category, and therefore a fortiori in the analytic one.

For optimality, \cref{dyn:prop:first-weight} gives
\[
 h_\eta=-\Li^{-1}\bigl(P_R(z_1)e_1\bigr)
 =-\sum_{n\ge2}\frac{R^{2-n}}{\lambda^{n}-\lambda}\,z_1^{n}e_1 ,
\]
whose degree-$n$ coefficient has modulus $R^{2-n}\mathfrak d_{n-1}^{-1}$.
Because $\mathfrak d\le2$, the negative parts of $\log\mathfrak d^{-1}$ are
bounded, and the root test gives radius $Re^{-\tau}$, including the value
$0$ when $\tau=\infty$. A coefficient holomorphic on a larger common
polydisk would restrict on the $z_1$-axis to an extension of this series with
a larger Taylor radius, which is impossible. The halo statement follows from
the proof of \cref{dyn:prop:halo} with $\mathbb D^{d}_{Re^{-\tau}}$ in place
of $\mathbb D^{d}_R$: the reductions of $H$ and $H^{-1}$ are the identity,
and that of $F$ is $z\mapsto\lambda z$, which preserves the polydisk.
\end{proof}

``Unique'' means unique among tangent-to-identity normalized formal
coordinates and their coherent coefficientwise analytic realizations. It does
not refer to arbitrary set-theoretic maps (source 09). In source 09's
normalization $g_R=P_R/R=z^{2}/(R-z)$, the one-variable witness has forward
coefficient
\[
 \check h_\eta=\lambda^{-1}\sum_{n\ge1}\frac{R^{-n}z^{n+1}}{\lambda^{n}-1}
 =-h_\eta,
\]
with the same radius $Re^{-\tau}$. The minus sign is the convention flip of
\cref{dyn:esm:conv}(iii). For unequal input radii $R_1,\dots,R_d$, a diagonal
change of ordinary coordinates commutes with $\lambda I_d$, and the same
proof gives the common polydisk with radii $R_je^{-\tau}$ (source 08). This
is a rescaling corollary. It says nothing about optimal Reinhardt domains.

\begin{corollary}[The sixth category for a scalar exact multiplier; sources 08
and 09]\label{dyn:esm:cor:sixthrow}
Fix a nonzero $\Gamma$, $\lambda=e^{2\pi i\theta}$ with $\theta$ irrational,
$d\ge1$ and $0<R<\infty$. Consider inputs \eqref{dyn:eq:Fintro} with
$\Lambda=\lambda I_d$ whose coefficients lie in the indicated category; for
the second row, holomorphic on one common polydisk $\mathbb D_R^{d}$ as in
\eqref{dyn:esm:eq:input}. The following conditions are necessary and
sufficient for the normalized conjugacy and its inverse of \emph{every} such
input to lie in that category.
{\normalfont
\begin{center}
\begin{tabular}{@{}P{0.60\textwidth}P{0.28\textwidth}@{}}
\toprule
Coefficient category & Exact condition\\
\midrule
the same fixed $\OO(\mathbb D_R^{d})$ (row one of \cref{dyn:thm:main}) & $\tau=0$\\
some common polydisk, i.e.\ $\mathcal R_d$ (\textbf{the sixth category}) &
 $\tau<\infty$; largest universal radius $Re^{-\tau}$\\
separate convergent germs, $\mathcal A_d$ (row two) & $\tau<\infty$\\
entire coefficients (row three) & $\tau<\infty$\\
polynomial and formal coefficients (rows four and five) & $\nres$ only\\
\bottomrule
\end{tabular}
\end{center}}
\end{corollary}
\begin{proof}
The second row is \cref{dyn:esm:thm:main}: sufficiency and the radius come
from the theorem, and necessity from its $\tau=\infty$ witness, whose first
coefficient is not even a germ. The other rows are \cref{dyn:thm:main} for
$\Lambda=\lambda I_d$.
\end{proof}

Only the second row is new, and both sources say so. Source 08 prints the
first three rows in dimension $d$. Source 09 prints a six-row table in one
variable that adds the entire, polynomial and formal rows. Its necessity
witness for the entire row is the one in \cref{dyn:lem:sharp-divisor}, and its
polynomial-row argument is that of \cref{dyn:thm:main}. Neither is reprinted.
Source 08's corollary on entire and polynomial layers restates rows three
and four. The table shows that, for a scalar exact multiplier, requiring one
common germ imposes \emph{no stronger} arithmetic condition than requiring
separate germs (source 08). Under drift the answer is the opposite
(\cref{dyn:cyc:cor:classification}), and the two answers are kept in
separate rows (\cref{dyn:rem:oneinvariant,dyn:cyc:tab:exactdrift}).

\begin{remark}[What is cited rather than reprinted]
\label{dyn:esm:rem:duplicates}
Both sources restate the machinery of
\cref{dyn:sec:framework,dyn:sec:categories}, and it is cited here. That
covers their strong-summability definitions and coefficient-ring tables
(\cref{dyn:def:summable}, \cref{dyn:subsec:rings}); the positive-support
lemma (\cref{dyn:lem:neumann}); the formal existence and uniqueness of the
normalized conjugacy (\cref{dyn:prop:linearization}); same-domain inversion
(\cref{dyn:prop:subgroup,dyn:lem:inverse}); halo evaluation and the bijective
halo conjugacy (\cref{dyn:prop:eval,dyn:prop:halo}); workspace invariance
(\cref{dyn:prop:workspace}); and evaluation on the monad
(\cref{dyn:prop:monad}). Source 09 proves the positive-support lemma by the
minimal-bad-sequence argument of \cref{dyn:app:higman}. Source 08 takes a
second route: it reduces the lemma to Poonen's strong geometric-series
theorem \cite[Thm.~4.1]{Poonen} over the free associative algebra
$\Z\langle X_s:s\in S\rangle$. There every ordered word of weight $\gamma$
occurs in $\sum_ma^{m}$, $a=\sum_sX_st^{s}$, with a positive integer
coefficient, so no cancellation can hide infinitely many words. The
continued-fraction material of both sources is \cref{dyn:prop:cf} and
\cref{dyn:thm:nonbrjuno}(b)--(d). Source 09's rate-zero non-Bruno rotation,
$a_{j+1}=\lceil e^{q_j/(j+1)}\rceil$, is \cref{dyn:thm:nonbrjuno}(b) exactly.
\end{remark}

\begin{remark}[No conflict with Brjuno--Yoccoz or with
\cref{dyn:prop:noncoherent}]\label{dyn:esm:rem:nocontra}
The Brjuno condition implies $\tau=0$, whereas \cref{dyn:esm:thm:main}
allows every finite $\tau$. The two statements concern different kinds of
convergence: Brjuno--Yoccoz concerns an ordinary germ whose whole nonlinear
Taylor series is present at once. Here each Hahn coefficient is
constructed before any attempt to sum over perturbation orders as ordinary
numbers, and its norm may grow arbitrarily fast with the exponent (sources
08 and 09; \cref{dyn:rem:nocontra}). \Cref{dyn:prop:noncoherent} is not a
counterexample either. Its nonlinear part $X^{2}$ sits at Hahn exponent
zero, whereas \eqref{dyn:esm:eq:input} requires strictly positive support.
Neither source reproves or alters Brjuno--Yoccoz theory, and both cite it
as external.
\end{remark}

\subsection{Route D: plane trees in the forward coordinate}
\label{dyn:tree:subsec}

Source 09's main proof is a \emph{fourth} route to a support-controlled
linearizer, kept and marked under \cref{dyn:conv:merge}(ii) because it is
genuinely different from routes A--C (\cref{dyn:rem:tworoutes-lin}). It
works in one variable. It computes the forward coordinate
$\check H=H^{-1}$ directly, organizes it by full plane trees with slot
multiplicities, and uses the branching (forest) case of
\cref{dyn:esm:thm:treebound}, whereas route A uses only the chain case.
Source 09 also gives an operator-chain proof in the inverse convention (its
Appendix~B). That proof is route A combined with the chain case of
\cref{dyn:esm:thm:treebound}: for $d=1$ it gives the bound of
\cref{dyn:esm:prop:coefbound} with polynomial exponent $3m-2$ and
combinatorial constant $2^{m-1}$. It is not printed again. The two routes
\textbf{share} the arithmetic of
\cref{dyn:esm:lem:cluster,dyn:esm:lem:packing}, so they are not two
independent proofs of it (\ref{dyn:esm:nc:routes}). Their value is that they expose
different possible errors: slot multiplicities in the forward tree formula,
and derivative factors and signs in the inverse operator expansion. Source
09's program compares both (\cref{dyn:sec:verification}).

In one variable, $F\circ\check H=\check H(\lambda z)$ with $\check H=z+\check h$
is equivalent to
\begin{equation}\label{dyn:tree:eq:forward}
 \Li\check h=f(z+\check h).
\end{equation}
At Taylor degree $n$ the right side involves only Taylor coefficients of
$\check h$ of degree below $n$. Under $\nres$, $\check h$ therefore exists
and is unique, with support in $S^{*}\setminus\{0\}$ (source 09's formal
proposition, whose Hahn-support part is \cref{dyn:lem:neumann}). Expand
$f_s(z)=\sum_{k\ge2}a_{s,k}z^{k}$. A \emph{full plane tree} is a finite
rooted tree with ordered children in which every internal vertex has at
least two children; its leaves stand for $z$. An internal vertex $v$ with
$k_v$ children carries a label $\sigma_v\in S$. Put $w_v=k_v-1\ge1$, let
$s_v$ be the sum of $w_u$ over the internal vertices $u$ of the subtree of
$v$, and set $n=\sum_vw_v$ and $\gamma(T)=\sum_v\sigma_v$. The subtree at $v$
has $s_v+1$ leaves.

\begin{proposition}[Plane-tree coefficient formula; source 09]
\label{dyn:tree:prop:formula}
\begin{equation}\label{dyn:tree:eq:formula}
 [t^{\gamma}z^{n+1}]\check h
 =\sum_{\substack{T:\ \gamma(T)=\gamma\\ \sum_vw_v=n}}\
 \prod_{v\ \mathrm{internal}}
 \frac{a_{\sigma_v,k_v}}{\lambda^{s_v+1}-\lambda}.
\end{equation}
For fixed $\gamma$ and $n$ the sum is finite, over trees with at most
$\ell_S(\gamma)$ internal vertices.
\end{proposition}
\begin{proof}
A term $a_{\sigma,k}t^{\sigma}(z+\check h)^{k}$ on the right of
\eqref{dyn:tree:eq:forward} is a root with $k$ ordered children, each of
which is either the leaf $z$ or a previously constructed contribution of
$\check h$. The inversion at the resulting degree $s+1$ contributes
$1/(\lambda^{s+1}-\lambda)$. Recursion on the nontrivial children gives the
product. A plane tree has a unique root decomposition, so each tree is
counted exactly once; the children are ordered, so there are no symmetry
factors. Reading the internal labels in preorder gives a word in
$\word_S(\gamma)$. By \cref{dyn:lem:neumann} there are finitely many such
words, of length at most $\ell_S(\gamma)$, and there are finitely many shapes
with $n+1$ leaves.
\end{proof}

This is a direct form of the classical tree formulas
\cite{FauvetMenousSauzin}. Source 09 claims only their regrouping by Hahn
exponent and the estimate below, not the tree representation.

\begin{lemma}[Polynomial tree count; source 09]\label{dyn:tree:lem:count}
For $1\le m\le n$, the number of unlabeled full plane trees of weight $n$
with $m$ internal vertices is at most $4^{m-1}n^{m-1}(n+1)^{m-1}$. With
labels summing to $\gamma$ it is at most that number times
$W_{\gamma,m}=\#\{w\in\word_S(\gamma):|w|=m\}$.
\end{lemma}
\begin{proof}
Deleting the leaves leaves a plane skeleton on the $m$ internal vertices.
There are at most $4^{m-1}$ such skeletons: encode the depth-first traversal
of their $m-1$ edges by $2(m-1)$ up and down symbols. There are
$\binom{n-1}{m-1}\le n^{m-1}$ positive weight assignments summing to $n$.
Given the weights, $k_v=w_v+1$, and the $d_v$ internal children of $v$
occupy one of at most $\binom{w_v+1}{d_v}\le(n+1)^{d_v}$ sets of slots, with
$\sum_vd_v=m-1$. A label word in preorder assigns the labels in one way.
\end{proof}

Only polynomiality at fixed $m$ matters. An overcount exponential in $n$
would lose the sharp radius (source 09).

\begin{proposition}[Finite-depth estimate for the forward coordinate;
source 09]\label{dyn:tree:prop:estimate}
Let $d=1$, $\tau<\infty$, $\gamma\in S^{*}\setminus\{0\}$, $0<r<R$ and
$a>\tau$, and put
$B_\gamma(r)=\max_{\sigma\in D_S(\gamma)}\max_{|z|\le r}|f_\sigma(z)|/r$.
For $n\ge1$,
\begin{equation}\label{dyn:tree:eq:estimate}
 \bigl|[z^{n+1}]\check h_\gamma\bigr|
 \le\Bigl(\frac{e^{a}}{r}\Bigr)^{n}
 \sum_{1\le m\le\min(\ell_S(\gamma),n)}W_{\gamma,m}4^{m-1}
 \bigl(C_aB_\gamma(r)\bigr)^{m}(n+1)^{3m-2}
 \le K_{\gamma,r,a}(n+1)^{3\ell_S(\gamma)}\Bigl(\frac{e^{a}}{r}\Bigr)^{n},
\end{equation}
where $K_{\gamma,r,a}=\sum_{m=1}^{\ell_S(\gamma)}W_{\gamma,m}4^{m-1}(C_aB_\gamma(r))^{m}$.
Consequently $\rad(\check h_\gamma)\ge Re^{-\tau}$ for every $\gamma$, and
$H=\check H^{-1}$ has its coefficients on the same disk
(\cref{dyn:lem:inverse}). This is a second proof of \cref{dyn:esm:thm:main}
for $d=1$.
\end{proposition}
\begin{proof}
By Cauchy's estimate,
$\prod_v|a_{\sigma_v,k_v}|\le\prod_vr^{-k_v}\max_{|z|\le r}|f_{\sigma_v}|\le r^{-n}B_\gamma(r)^{m}$,
because $\sum_vk_v=n+m$. Also $|\lambda^{s_v+1}-\lambda|=\mathfrak d_{s_v}$,
so \cref{dyn:esm:thm:treebound} bounds the divisor product by
$(C_an)^{m}e^{an}$. Multiply by the count of \cref{dyn:tree:lem:count} and
use $n^{2m-1}(n+1)^{m-1}\le(n+1)^{3m-2}$. The root test at fixed $\gamma$,
followed by $r\uparrow R$ and $a\downarrow\tau$, gives the radius.
\end{proof}

The same $r$ and $a$ serve at every exponent, while the constants and the
polynomial degrees depend on $\gamma$. That is exactly the difference
between a common domain of holomorphy and a common norm bound. The root test
is taken \emph{after} a Hahn exponent and its finite word certificate have
been fixed. No bound uniform in all words is summed as an ordinary series
(source 09).

\subsection{Formal parameters, higher rank, and the hypothesis audit}
\label{dyn:esm:subsec:params}

\begin{corollary}[Several formal parameters, one dynamical variable; source 09]
\label{dyn:esm:cor:parameters}
Let $x=(x_1,\dots,x_e)$ be finitely many formal
parameters and
$F(z,x)=\lambda z+\sum_{\nu\in\N^{e}\setminus\{0\}}x^{\nu}f_\nu(z)$
with $f_\nu\in z^{2}\OO(\mathbb D_R)$. If $\tau<\infty$, all the normalized
parameter coefficients of the linearizing coordinate and of its inverse are
holomorphic on $\mathbb D_{Re^{-\tau}}$, whatever $|\nu|$ is.
\end{corollary}
\begin{proof}
Embed the monoid $\N^{e}$ into the lexicographically ordered group $\Z^{e}$.
Its nonzero elements are positive, and it is well ordered, so
\cref{dyn:esm:thm:main} applies to $\Gamma=\Z^{e}$. Source 09 also gives a
direct tree argument: at a fixed $\nu$ only finitely many nonzero
multi-indices, at most $|\nu|$ of them, can label vertices.
\end{proof}

``Several'' here refers to parameters, not to dynamical variables: there is
still one variable $z$ (source 09). Source 08's theorem applies verbatim to
the same embedded input in $d$ variables with multiplier $\lambda I_d$. No
ordinary convergence of the parameter series is asserted.

\begin{example}[A higher-rank support; source 09]\label{dyn:esm:ex:lex}
Take $\Gamma=\Z^{2}$ in lexicographic order, $u=t^{(0,1)}$, $v=t^{(1,0)}$,
and $F(z)=\lambda z+ug(z)+vk(z)$ with $g,k\in z^{2}\OO(\mathbb D_R)$. Every
power $u^{n}$ has smaller valuation than $v$, so an iteration that waits for
powers of $u$ to pass the scale $v$ is inappropriate
(\cref{dyn:warn:contraction}). Yet the coefficient at $u^{i}v^{j}$ uses
exactly $i$ labels of the first kind and $j$ of the second, in finitely
many orders, and for $\tau<\infty$ all these coefficients share the radius
$Re^{-\tau}$. The theorem is therefore not an argument confined to rank one
or to a cofinal sequence of powers.
\end{example}

\begin{longtable}{@{}P{0.28\textwidth}P{0.64\textwidth}@{}}
\caption{Where each hypothesis of \cref{dyn:esm:thm:main} is used (source
09's audit, translated; the first row also records source 08's
several-variable scope).}\label{dyn:esm:tab:audit}\\
\toprule
Hypothesis & Precise role\\
\midrule
\endfirsthead
\toprule
Hypothesis & Precise role\\
\midrule
\endhead
One dynamical variable (source 09), or a scalar multiplier $\lambda I_d$
(source 08) & All divisors are indexed by one integer $|\beta|-1$ for one
irrational rotation, and the determinant argument of
\cref{dyn:esm:lem:cluster} produces one common reduced rational
approximation.\\[3pt]
$|\lambda|=1$ & Identifies divisor moduli with $\mathfrak d_n$, removes
rotation growth from the operator route, and preserves centered
polydisks.\\[3pt]
$\lambda$ not a root of unity ($\nres$) & Makes every homological divisor
nonzero and the normalized formal coordinates unique.\\[3pt]
No coefficientwise constant term & Fixes the ordinary origin and allows
tangent-to-identity formal composition at zero.\\[3pt]
No coefficientwise linear term (exactness, $\sdex$) & Makes every tree
weight $k_v-1$ and every operator degree increment
(\cref{dyn:esm:lem:degreechain}) strictly positive. Dropping it is the drift
of \cref{dyn:sec:drift}.\\[3pt]
Well-ordered positive Hahn support & Makes the output monoid well ordered
and bounds all input word lengths at each fixed exponent
(\cref{dyn:lem:neumann}).\\[3pt]
A common input polydisk & Supplies one ordinary $r<R$ for all relevant input
coefficients, whatever the output exponent.\\[3pt]
$\tau<\infty$ & Supplies $C_a<\infty$ for every $a>\tau$.\\[3pt]
No uniform coefficient-norm requirement & Lets the finite constants and the
polynomial powers depend on the exponent. It does not weaken the
common-radius conclusion.\\[3pt]
Set-sized workspace & Makes all support families and recursions sets.
Embeddings \eqref{dyn:eq:embedding} connect the result to surcomplex
values.\\
\bottomrule
\end{longtable}

The optimality witness satisfies every structural hypothesis and needs no
delicate cancellation. Source 09 lists three substantive limitations,
which are not hidden exceptions to the statement: the absence of ordinary
parameter convergence, the remaining open problem (in this report the
nonscalar case, since answered by source 10 except for the optimal radius
when $\rang\ge2$ and $0<\tau<\infty$), and the absence of machine-checked proofs. The first row
of \cref{dyn:esm:tab:audit} is exactly the hypothesis that source 10
removes, at the price of the factor $\rang$ in the exponent
(\cref{dyn:nsc:thm:main}).

\subsection{What this section does not settle}
\label{dyn:esm:subsec:limits}

\emph{The nonscalar boundary.} For
$\Lambda=\operatorname{diag}(\lambda_1,\dots,\lambda_d)$ the homological
divisor on $z^{\beta}e_j$ is $\lambda^{\beta}-\lambda_j$. It is no longer
controlled by a single integer $|\beta|-1$ and a single rotation number, so
the determinant argument of \cref{dyn:esm:lem:cluster} does not apply as
written. Very small values need not reduce to one rational number $p_*/q_*$.
Positivity still constrains the degree bookkeeping, but that alone does not
prove the same nonaccumulation estimate for these families (source 08). A
replacement would have to respect vector resonance relations and the
geometry of decorated trees. The one-dimensional constant $e^{-\tau}$ may
not be imported unchanged, nor may one simply substitute the minimum divisor
$\Delta_n$ of \eqref{dyn:eq:multisigma} into the scalar proof (sources 08
and 09). Source 09 names the most direct target: a vector analogue of
\cref{dyn:esm:thm:treebound} whose exponential loss stays independent of
fixed tree complexity. Establishing it, or constructing a counterexample,
would address the remaining case of \cref{dyn:q:commongerm}. Neither source
asserts success or failure there.

\emph{Status after source 10.} The analysis above is that of sources 08 and
09 and is kept as they state it. Source 10 supplies the vector analogue that
source 09 named, in a weaker form than a single $e^{-\tau}$: at fixed tree
complexity $m$ the exponential loss is at most $\min(\rang,m)\tau$, which
never exceeds $\rang\tau$ however large $m$ is
(\cref{dyn:nsc:thm:products}). It
does not import the one-dimensional constant and does not substitute
$\Delta_n$ into the scalar proof; it bounds the number of distinct
exponential rates by $\rang$ (\cref{dyn:nsc:cor:angular}) and pays for each
rate once with contracted blocks that stay inside the nonlinear cone
(\cref{dyn:nsc:lem:blocks}). The constant $e^{-\tau}$ is recovered, and is
sharp, when $\rang=1$ (\cref{dyn:nsc:thm:criterion}), and equals $1$ when
$\tau=0$ at every angular rank. Whether it holds when $\rang\ge2$ and
$0<\tau<\infty$ is open.

\emph{Individual inputs.} $Re^{-\tau}$ is a universal guarantee, not a
formula for each input; polynomial inputs already do much better. Criteria
for input-specific improvement, from vanishing coefficients or algebraic
structure, are a different question, and any such criterion must survive the
nonlinear degree interactions of the conjugacy equation (source 08;
\cref{dyn:q:maximal}).

\emph{Depth-uniform and ordinary estimates.} The estimates allow the
constants to depend arbitrarily on a fixed support word. So they do not give
ordinary specialization of a Hahn parameter, Gevrey bounds across depth, or
analytic convergence jointly in a complex parameter and the coordinate
(sources 08 and 09; \cref{dyn:q:realization}). Even in the quadratic example,
where every coefficient is a polynomial, no estimate on the growth of its
scalar coefficient with the depth is asserted
(\cref{dyn:subsec:quadratic}). For a non-Bruno $\lambda$, specializing
$t^{\eta}$ to a nonzero complex number in $\lambda z+t^{\eta}z^{2}$ gives a
germ linearly conjugate to $\lambda w+w^{2}$, whose linearizer diverges. The
Hahn family does not justify the specialization
(\cref{dyn:prop:quadratic-scale}).

\emph{The drifted category.} \Cref{dyn:esm:thm:main} is not a statement
that all small-divisor radius collapse is illusory. It identifies the role
of exactness in preventing collapse (source 09). Admitting one infinitesimal
linear term already brings collapse back over the same cyclic workspace
(\cref{dyn:cyc:thm,dyn:cyc:ex:logtwo}).

\subsection{The nonscalar diagonal multiplier: rational angular rank}
\label{dyn:nsc:subsec}

This subsection and the five after it are contributed by source 10, written
on 22 September 2026 against repository commit
\texttt{048b72cf\allowbreak 7cbfc8ab\allowbreak 246e4f73\allowbreak
788c1046\allowbreak 0cb3f6e0}. That commit already contained sources 08 and
09 as merged above, and at that commit this report's source file was
byte-identical to its state immediately before source 10 was added. Source
10 answers the case of \cref{dyn:q:commongerm} that
\cref{dyn:esm:subsec:limits} leaves open: a \emph{nonscalar} exact diagonal
unitary multiplier. The answer is again positive. Every Hahn coefficient of
the normalized conjugacy and of its inverse is holomorphic on one polydisk,
whose radius loses at most the factor $e^{-\rang\tau}$, where $\rang$ is the
rational rank of the multiplier's angles. When $\rang=1$ the universally
guaranteed factor is $e^{-\tau}$ and is sharp, in particular for every scalar $\lambda I_d$
(\cref{dyn:esm:thm:main}) and for many nonscalar multipliers. For
$\rang\ge2$ and $0<\tau<\infty$ the optimal radius is not determined
(\cref{dyn:nsc:subsec:limits}). Source 10 does not claim the scalar theorem
or the scalar fixed-complexity rate as new; it cites both.

\emph{Tags.} Resonance $\nres$, with the multiplier \textbf{exactly}
$\Lambda=\operatorname{diag}(\lambda_1,\dots,\lambda_d)$, $|\lambda_j|=1$;
divisor notion $\sdex$; category the common-domain germ ring $\mathcal R_d$
of \eqref{dyn:eq:rings}, with a specified radius; domain notion
$\mathbb D_R^{d}$ and $\radiso$. No drift is permitted anywhere in these
subsections.

\begin{convention}[Setting and local notation of source 10]
\label{dyn:nsc:conv}
\begin{enumerate}[label=(\roman*)]
\item Write $\lambda_j=e^{2\pi i\theta_j}$ with $\theta_j\in\R$ and
$\theta=(\theta_1,\dots,\theta_d)$, and assume $\nres$. Repeated eigenvalues
are allowed: $\nres$ is not a simple-spectrum hypothesis. The rate is
$\tau=\tau(\Lambda)$ of $\sdex$ (\cref{dyn:subsec:smalldivisor}); it equals
$\sigma_d(\Lambda)$ of \eqref{dyn:eq:multisigma}, whose minimal divisor
$\Delta_n=\min_{|\alpha|=n,\,j}|\lambda^{\alpha}-\lambda_j|$ is used below.
\item The \emph{rational angular rank} of $\Lambda$ is
\begin{equation}\label{dyn:nsc:eq:rank}
 \rang=\rang(\Lambda)=\dim_{\Q}\operatorname{span}_{\Q}(1,\theta_1,\dots,\theta_d)-1 .
\end{equation}
It does not change when an angle changes by an integer, so it is an
invariant of the eigenvalue list of $\Lambda$. Under $\nres$,
$1\le\rang\le d$: if $\rang=0$ every $\lambda_j$ would be a root of unity,
and $\lambda_j^{q+1}=\lambda_j$ for an order $q$ of $\lambda_j$ would violate
$\nres$ at $\alpha=(q+1)e_j$. For $\Lambda=\lambda I_d$ with $\theta$
irrational, $\rang=1$. Source 10 writes $r$; this report writes $\rang$,
because $r$ is already the finite-return threshold of \cref{dyn:sec:return},
the auxiliary radius of \cref{dyn:esm:conv}(iv), the chain length of
\cref{dyn:esm:cor:chain}(i) and the flag length of \cref{dyn:thm:flag}.
\item $\rang$ is \textbf{not} the valuation rank of $\Gamma$ and has nothing
to do with the Hahn support: it is a property of $d$ ordinary real angles.
Nor is it the flag length of \cref{dyn:thm:flag}. Both are bounded by $d$
through a rank count of integer vectors, but the flag concerns valuations
of the Hahn divisors $k\cdot\Upsilon$ and $\rang$ concerns exponential
rates of ordinary divisors; no statement is transferred between them.
\item \emph{Conjugacy convention.} As everywhere in this report, $H=z+h$ and
$H\circ F=\Lambda H$. Source 10 computes the forward coordinate first. It
writes $G=z+g$ with $F\circ G=G\circ\Lambda$; this is our
$\check H=H^{-1}=z+\check h$ of \cref{dyn:esm:conv}(iii). Its $H$ is ours.
Every formula below has been translated.
\item \emph{Other local symbols.} $\radiso$ is source 10's
$\operatorname{rad}_{\mathrm{iso}}$. An auxiliary ordinary radius is written
$r<R$, as in \cref{dyn:esm:conv}(iv); source 10 writes $\rho$, which this
report reserves (\cref{dyn:subsec:radius}). The divisor at a tree vertex is
$\mathfrak d_v$ (source 10: $\delta_v$); for $\Lambda=\lambda I_d$ it is
$\mathfrak d_{s_v}$ of \eqref{dyn:esm:eq:rate}. For $a>\tau$, $C_a(\Lambda)$
is the least constant $\ge1$ with
\begin{equation}\label{dyn:nsc:eq:Ca}
 \max\bigl(1,|\lambda^{\alpha}-\lambda_j|^{-1}\bigr)\le C_a(\Lambda)\,e^{a|\alpha|}
 \qquad(|\alpha|\ge2,\ 1\le j\le d),
\end{equation}
the vector form of \eqref{dyn:esm:eq:Ca}. Source 10 writes $A_m(n)$ and
$P(T)$ for the quantities called $A^{\Lambda}_m(n)$ and $\mathsf P(T)$ below,
to keep them apart from $A_m(n)$ of \cref{dyn:esm:cor:chain}(ii) and the
shell polynomial $P$.
\end{enumerate}
\end{convention}

The rate constant exists by a finite-prefix argument. Put
$M_n=\max(1,\Delta_n^{-1})$. For $a>\tau$, the definition of the
limsup gives $n_0$ such that $M_n\le e^{an}$ for $n\ge n_0$.
The finitely many earlier nonzero divisors are controlled by
\[
 C_a(\Lambda)=\max\bigl(\{1\}\cup
       \{M_ne^{-an}:2\le n<n_0\}\bigr).
\]
This is the least constant at least one satisfying \eqref{dyn:nsc:eq:Ca}.
No uniform bound in a varying multiplier is asserted.

\begin{theorem}[Common-domain linearization for a nonscalar exact multiplier;
source 10]\label{dyn:nsc:thm:main}
Let $\Gamma$ be any nonzero set-sized ordered abelian group, $d\ge1$,
$\Lambda$ diagonal unitary with $\nres$, and $\tau=\tau(\Lambda)<\infty$. Let
\begin{equation}\label{dyn:nsc:eq:input}
 F(z)=\Lambda z+\sum_{s\in S}t^{s}f_s(z),\qquad
 f_s\in\OO(\mathbb D_R^{d})^{d},\quad f_s(0)=0,\quad \mathrm Df_s(0)=0,
\end{equation}
with $S\subset\Gamma_{>0}$ well ordered and no common bound on the sizes of
the $f_s$. Then the unique normalized positive conjugacy $H=z+h$ from the formal
coefficient case of \cref{dyn:thm:main}, $H\circ F=\Lambda H$, and its inverse
$\check H=H^{-1}=z+\check h$ satisfy
\begin{equation}\label{dyn:nsc:eq:main}
 h_\gamma,\ \check h_\gamma\in\OO\bigl(\mathbb D^{d}_{Re^{-\rang\tau}}\bigr)^{d},
 \qquad \ord_0h_\gamma,\ \ord_0\check h_\gamma\ge2,
 \qquad \supp h,\ \supp\check h\subseteq S^{*}\setminus\{0\}
\end{equation}
for every Hahn exponent $\gamma$. The domain is independent of the exponent,
of the word depth $\ell_S(\gamma)$, of the rank of $\Gamma$ and of the sizes
of the input coefficients. $H$ and $H^{-1}$ lie in $\mathcal R_d$ and evaluate
to mutually inverse bijective conjugacies of the finite halo
$(\mathbb D^{d}_{Re^{-\rang\tau}})^{\#}_\Gamma$, on which $F^{\#}$ is also a
bijection. The construction is unchanged under ordered enlargement of
$\Gamma$.
\end{theorem}

The polydisk is \emph{open}; nothing is asserted about the boundary.
Uniqueness is uniqueness in the normalized formal positive-support class
(\cref{dyn:prop:linearization}), so the analytic coordinate is not chosen
afresh at each smaller radius. When $\rang=1$ the radius is $Re^{-\tau}$,
which \cref{dyn:nsc:thm:criterion} shows is optimal. For a scalar multiplier
this is \cref{dyn:esm:thm:main} again. The proof below is then a third route
to it (\cref{dyn:nsc:rem:scalar}), and it gives nothing new in that case.

\subsection{Exponential approximation rates are bounded by angular rank}
\label{dyn:nsc:subsec:rates}

The arithmetic input is elementary. A fixed finite family of integer vectors
whose heights grow only linearly cannot exhibit more distinct exponentially
small values of one real linear form than the rank allows. The count is of
distinct \emph{rates}, not of vectors or of their multiplicities.

\begin{lemma}[Exponential rates and integer height; source 10]
\label{dyn:nsc:lem:height}
Fix $q\ge1$ and $\psi=(\psi_1,\dots,\psi_q)\in\R^{q}$. Let
$N_k\to\infty$ be integers, and for $v$ in a fixed finite set $V$ let
$a_v(k)=(u_v(k),p_v(k))\in\Z^{q+1}$ satisfy
$\|a_v(k)\|_\infty\le CN_k$. Suppose that
$e_v(k)=\psi\cdot u_v(k)-p_v(k)\ne0$ and that
\[
 b_v=\lim_{k\to\infty}-\frac1{N_k}\log|e_v(k)|\in(0,\infty)
 \qquad(v\in V).
\]
Then $\{b_v:v\in V\}$ has at most $q$ elements.
\end{lemma}
\begin{proof}
The empty family has no rates, so assume $V\ne\emptyset$.
\emph{The vectors eventually span at most $q$ rational dimensions.} Let
$b_*=\min_vb_v>0$; eventually $|e_v(k)|\le e^{-b_*N_k/2}$ for every $v$.
Take any $q+1$ of the vectors as the rows of a square integer matrix and
subtract $\psi_1,\dots,\psi_q$ times its first $q$ columns from its last
column. The determinant does not change, and the last column becomes
$(-e_v(k))_v$. Each cofactor has modulus at most $q!(CN_k)^q$,
so expansion along that column bounds the determinant by
$(q+1)q!C^qN_k^{q}e^{-b_*N_k/2}$. An integer of
modulus less than one is zero, so for large $k$ every such determinant
vanishes. There are finitely many choices of rows, so the whole family has
rational rank at most $q$ for large $k$. If there are fewer than $q+1$
vectors, that rank bound is immediate.

\emph{Each strictly smaller positive rate adds a dimension.} Pass to a
subsequence along which the rank of every subset of the family is constant;
there are finitely many subsets and ranks. Suppose a vector $a_v(k)$ with
rate $b$ lay, along this subsequence, in the rational span of vectors whose
rates all exceed $b$. Choose a basis among those faster vectors, passing to
a further subsequence to fix its indices and a nonsingular coordinate minor.
Cramer's rule writes $a_v(k)$ in that basis with coefficients of modulus at
most $C_2N_k^{q}$: the denominator is a nonzero integer, of modulus at least
one, and the numerators are polynomially bounded in $N_k$. Applying the
linear form $(u,p)\mapsto\psi\cdot u-p$ gives
$|e_v(k)|\le C_3N_k^{q}\max_w|e_w(k)|$, the maximum over the faster basis.
Let $c>b$ be the least rate among this finite faster basis, and put
$\epsilon=(c-b)/3$. For large $k$, the asserted limits give
$|e_v(k)|\ge e^{-(b+\epsilon)N_k}$ and
$\max_w|e_w(k)|\le e^{-(b+2\epsilon)N_k}$. The preceding bound would
force $1\le C_3N_k^qe^{-\epsilon N_k}$, impossible for large $k$.
Thus a slower rate adds a rationally independent direction. Order the distinct rates from
the largest down. The fastest nonzero vector spans a nonzero space, and a
vector at each later rate lies outside the span of all faster ones, so each
rate raises the rank by at least one. The first step bounds that rank by
$q$.
\end{proof}

No coordinate of $\psi$ need be algebraic or Diophantine; the argument is an
integer-determinant argument, not a subspace theorem. The stated linear
height is sufficient. The same proof works for a common bound $H_k\ge1$
with $\log H_k=o(N_k)$: the determinant and Cramer factors are bounded
by a constant times $H_k^q=e^{o(N_k)}$. The distinct-rate conclusion also
extends to a fixed infinite family satisfying the hypotheses at each index:
$q+1$ distinct rates would already occur in a finite subfamily. This gives
neither a uniform eventual rank bound for the entire infinite family nor an
estimate for a sum over a growing number of divisors. The tree application
below keeps its complexity fixed.

Unrestricted exponential height really can fail. Let
$\phi=(1+\sqrt5)/2$ and let $F_0=0,F_1=1,F_{n+1}=F_n+F_{n-1}$ be
the Fibonacci numbers. The recurrence gives
$\phi F_n-F_{n+1}=(-1)^{n+1}\phi^{-n}$. With $q=1$, $\psi=\phi$
and $N_k=k$, the two vectors
$(F_k,F_{k+1})$ and $(F_{2k},F_{2k+1})$ therefore have distinct positive
error rates $\log\phi$ and $2\log\phi$. Their exponential heights lie
outside the hypothesis. These distinctions correct the source's necessity
wording; the numbered lemma is used below exactly as stated.

\begin{corollary}[Angular-rank form; source 10]\label{dyn:nsc:cor:angular}
Let $k_v(N)\in\Z^{d}$, for finitely many indices $v$, satisfy
$\|k_v(N)\|_\infty=O(N)$, and suppose the nonzero circle divisors have
limits
\[
 b_v=\lim_{N\to\infty}\frac1N\log\bigl|e^{2\pi i\theta\cdot k_v(N)}-1\bigr|^{-1}
 \in(0,\infty).
\]
Then there are at most $\rang$ distinct $b_v$. The same holds along any
subsequence $N_k\to\infty$.
\end{corollary}
\begin{proof}
Choose $\psi_1,\dots,\psi_{\rang}$ so that $1,\psi_1,\dots,\psi_{\rang}$ is a
rational basis of $\operatorname{span}_{\Q}(1,\theta_1,\dots,\theta_d)$, write
$\theta_j=a_{j0}+\sum_{\ell}a_{j\ell}\psi_\ell$ with rational $a_{j\ell}$,
and let $Q\ge1$ clear all denominators. With $p_v(N)$ a nearest integer to
$\theta\cdot k_v(N)$, the integers
$u_{v,\ell}=\sum_jQa_{j\ell}k_{v,j}$ and
$p'_v=Qp_v-\sum_jQa_{j0}k_{v,j}$ have height $O(N)$:
$|p_v|\le\sum_j|\theta_j||k_{v,j}|+1/2$. They satisfy
$\psi\cdot u_v-p'_v=Q(\theta\cdot k_v-p_v)$. By \eqref{dyn:esm:eq:sine},
$4\|x\|_{\Z}\le|e^{2\pi ix}-1|\le2\pi\|x\|_{\Z}$, so these real errors have
the same exponential rates as the circle divisors: fixed multiplicative
constants change their logarithms divided by $N$ by $O(1/N)$.
A nonzero circle divisor also makes the real error nonzero.
Apply \cref{dyn:nsc:lem:height} with $q=\rang\ge1$.
\end{proof}

\subsection{Colored trees, block contractions, and the product bound}
\label{dyn:nsc:subsec:trees}

An \emph{admissible colored tree} is a finite rooted substitution tree with
internal vertices and external leaves. Each internal vertex $v$ has an
output color $j_v\in\{1,\dots,d\}$ and $a_v\ge2$ input slots, each with a
coordinate color, and each slot holds either an external leaf of that color
or a child internal vertex of that output color. The internal skeleton is
the tree left after forgetting the leaves. For an internal vertex $v$ let
$T_v$ be the set of internal vertices of its subtree and $\alpha_v\in\N^{d}$
the vector counting the leaves of each color below $v$, and put
\begin{equation}\label{dyn:nsc:eq:weights}
\begin{gathered}
 w_v=a_v-1\ge1,\qquad s_v=\sum_{u\in T_v}w_u=|\alpha_v|-1,\qquad
 k_v=\alpha_v-e_{j_v},\\
 \mathfrak d_v=|\lambda^{\alpha_v}-\lambda_{j_v}|=|e^{2\pi i\theta\cdot k_v}-1|>0 .
\end{gathered}
\end{equation}
A tree with $m$ internal vertices and total weight $n=\sum_vw_v$ has $n+1$
leaves and $\sum_va_v=n+m$, by counting its $m-1$ internal edges. In several
variables a difference of two homological exponent vectors need not be a
homological exponent vector again. The contraction below produces exactly
the differences that are, so the proof never strengthens $\nres$ to a
condition on all of $\Z^{d}$. Admissible-cut closure for forests whose
associated operators are not universally zero, and its homogeneity-degree
consequences, have a precedent in \cite[Thm.~E and Prop.~5.6]{FMS26}; source
10 claims the rank estimate, not the use of tree cuts.

\begin{lemma}[Marked-block contraction; source 10]\label{dyn:nsc:lem:blocks}
Mark any set $A$ of internal vertices. For $v\in A$ let $C_A(v)$ be its
nearest strict marked descendants and
$B_v=T_v\setminus\bigcup_{u\in C_A(v)}T_u$. The $B_v$ are pairwise disjoint
and $v\in B_v$. Contracting each removed subtree to one leaf of its root's
output color leaves a tree at $v$ with leaf vector
\begin{equation}\label{dyn:nsc:eq:beta}
 \beta_v=\alpha_v-\sum_{u\in C_A(v)}\alpha_u+\sum_{u\in C_A(v)}e_{j_u}\in\N^{d},
\end{equation}
and
\begin{equation}\label{dyn:nsc:eq:blockfacts}
 |\beta_v|=1+\sum_{u\in B_v}w_u\ge2,\qquad
 \beta_v-e_{j_v}=k_v-\sum_{u\in C_A(v)}k_u,\qquad
 \sum_{v\in A}|\beta_v|\le n+|A| .
\end{equation}
In particular $|\lambda^{\beta_v}-\lambda_{j_v}|$ is a nonzero divisor of
degree $|\beta_v|\ge2$, covered by $\nres$ and by \eqref{dyn:nsc:eq:Ca}.
\end{lemma}
\begin{proof}
The removed subtrees are disjoint and their leaves form submultisets of the
leaves below $v$; replacing each by one leaf of the stated color gives
\eqref{dyn:nsc:eq:beta} and nonnegativity. A vertex lies in $B_v$ exactly
when $v$ is its closest marked ancestor-or-self, so the blocks are disjoint,
and $v\in B_v$ because only strict descendants were removed. The remaining
internal vertices keep their arities, the edge to a removed root having
become a leaf, and counting leaves of the contracted tree gives the degree
identity. Subtracting $e_{j_v}$ from \eqref{dyn:nsc:eq:beta} gives the
second identity. The positive weights in disjoint blocks sum to at most $n$,
and adding one per block gives the third.
\end{proof}

In two coordinates, let the root have output color $1$ and input monomial
$x^{2}y$, and fill its $y$ slot with a child of output color $2$ and
monomial $xy^{2}$. Then $\alpha_{\mathrm{root}}=(3,2)$,
$\alpha_{\mathrm{child}}=(1,2)$ and both weights are $2$. With both marked,
$\beta_{\mathrm{root}}=(3,2)-(1,2)+(0,1)=(2,1)$, of degree $3=1+w_{\mathrm{root}}$,
and $(2,2)-(1,1)=(1,1)=\beta_{\mathrm{root}}-e_1$; the contracted degrees sum
to $6=n+2$. The subtraction has produced a nonlinear monomial, not an
arbitrary signed lattice vector (source 10's example).

The weight $w_v\ge1$ is where exactness enters. An arity-one vertex, which
is what an infinitesimal linear term creates, would give a block of weight
zero and contracted degree one. Its divisor could be an exact linear
resonance, and small divisors could then be incurred repeatedly without
spending nonlinear degree. This is \cref{dyn:esm:warn:zero} in colored form.
The drifted collapse of \cref{dyn:sec:drift} lies exactly outside this
hypothesis and is not a counterexample to \cref{dyn:nsc:thm:main}
(source 10).

For an admissible colored tree $T$ put
$\mathsf P(T)=\prod_{v}\max(1,\mathfrak d_v^{-1})$, and for integers
$m,n\ge1$ let $A^{\Lambda}_m(n)$ be the maximum of $\mathsf P(T)$ over trees
with at most $m$ internal vertices and total weight $n$. Such trees exist (a
single vertex of arity $n+1$), and after a canonical slot convention there
are finitely many of them, so the maximum is attained. The factor
$\max(1,\cdot)$ only enlarges the divisor product of an actual tree
expansion.

\begin{theorem}[Rational-rank divisor-product bound; source 10]
\label{dyn:nsc:thm:products}
Assume $\nres$ and $\tau<\infty$. For every fixed $m\ge1$,
\begin{equation}\label{dyn:nsc:eq:limsup}
 \limsup_{n\to\infty}\frac{\log A^{\Lambda}_m(n)}{n}\le\min(\rang,m)\,\tau .
\end{equation}
Hence for every $\eps>0$ there is a finite $C_{m,\eps,\Lambda}$ with
\begin{equation}\label{dyn:nsc:eq:products}
 \mathsf P(T)\le C_{m,\eps,\Lambda}\exp\bigl((\min(\rang,m)\tau+\eps)n\bigr)
\end{equation}
for all admissible colored trees with at most $m$ internal vertices and total
weight $n$. No bound uniform in $m$ is asserted for the prefactor.
\end{theorem}
\begin{proof}
Fix $a>\tau$, so that \eqref{dyn:nsc:eq:Ca} holds. Choose maximizing trees
$T_k$ of weights $n_k\to\infty$ along which the limsup in
\eqref{dyn:nsc:eq:limsup} is attained, and pass to a subsequence with a fixed
number $m'\le m$ of internal vertices, a fixed rooted internal skeleton and
fixed output colors; there are finitely many choices. Input arities and leaf
counts still vary with $k$. Pass to a further subsequence along which
\[
 b_v=\lim_{k\to\infty}\frac1{n_k}\log^{+}\bigl(\mathfrak d_v(k)^{-1}\bigr)
\]
exists for every internal vertex. Indeed each sequence is nonnegative
and bounded above by $a(1+1/n_k)+\log C_a(\Lambda)/n_k$, so finitely
many subsequence extractions suffice. The limits are finite and nonnegative. The rate of
$\mathsf P(T_k)$ along the subsequence is $\sum_vb_v$.

\emph{At most $\rang$ positive rates.} For the vertices with $b_v>0$ take
$k_v(k)=\alpha_v(k)-e_{j_v}$, of height $O(n_k)$. Their circle divisors have
positive rates $b_v$, so by \cref{dyn:nsc:cor:angular} there are at most
$\rang$ distinct positive values among the $b_v$. There are also at most
$m'$.

\emph{Each rate value is charged at most $\tau$ in total.} Fix a positive
value $b$ and mark $A_b=\{v:b_v\ge b\}$. For $0<\zeta<b$ and all large $k$,
with $p_v(k)$ a nearest integer to $\theta\cdot k_v(k)$,
$|\theta\cdot k_v(k)-p_v(k)|\le e^{-(b-\zeta)n_k}$ for $v\in A_b$, after
fixed constants are absorbed by starting later. Apply
\cref{dyn:nsc:lem:blocks} to $A_b$. By the second identity of
\eqref{dyn:nsc:eq:blockfacts}, the real number
$\theta\cdot(\beta_v-e_{j_v})$ lies within $|A_b|e^{-(b-\zeta)n_k}$ of an
integer, so the contracted divisor is at most
$2\pi|A_b|e^{-(b-\zeta)n_k}$. Cancellation can only help this upper
bound, and the divisor is nonzero because $|\beta_v|\ge2$. Applying
\eqref{dyn:nsc:eq:Ca} to it gives $(b-\zeta)n_k\le a|\beta_v|+O(1)$, with
the constant independent of $v$ and $k$ (the finite skeleton and $a$ are
fixed). Summing over $A_b$ and using the
third identity of \eqref{dyn:nsc:eq:blockfacts},
$|A_b|(b-\zeta)n_k\le a(n_k+|A_b|)+O(1)$. Divide by $n_k$ and let
$k\to\infty$, $\zeta\downarrow0$ and $a\downarrow\tau$:
\begin{equation}\label{dyn:nsc:eq:budget}
 |A_b|\,b\le\tau .
\end{equation}
Exact cancellation to zero would contradict $\nres$; no lower estimate for a
signed lattice divisor was used.

\emph{Summing the rate values.} Let $b^{(1)},\dots,b^{(q)}$ be the distinct
positive rates, $q\le\min(\rang,m')$, and let $V_i$ be the set of vertices
with rate exactly $b^{(i)}$. Then $|V_i|\le|A_{b^{(i)}}|$, and
\eqref{dyn:nsc:eq:budget} gives
$\sum_vb_v=\sum_i|V_i|b^{(i)}\le\sum_i|A_{b^{(i)}}|b^{(i)}\le q\tau\le\min(\rang,m)\tau$.
This is \eqref{dyn:nsc:eq:limsup}. All sufficiently large $n$ then satisfy
\eqref{dyn:nsc:eq:products} with prefactor one, and the finitely many
remaining maxima are absorbed into $C_{m,\eps,\Lambda}$.
\end{proof}

\begin{corollary}[The fixed-complexity rate in angular rank one; source 10]
\label{dyn:nsc:cor:rankone}
If $\rang=1$, then for every $m\ge1$
\[
 \limsup_{n\to\infty}\frac{\log A^{\Lambda}_m(n)}{n}=\tau .
\]
\end{corollary}
\begin{proof}
For each $n$ choose a monomial and output color realizing $\Delta_{n+1}$;
its one-vertex tree has
weight $n$ and is admissible for $A^{\Lambda}_m(n)$. The resulting lower
bound has limsup $\tau$, since replacing $n+1$ by $n$ does not change the
rate. This already proves the assertion when $\tau=\infty$. For finite
$\tau$, the matching upper bound is \cref{dyn:nsc:thm:products}.
\end{proof}

The argument extracts subsequences, so the prefactor $C_{m,\eps,\Lambda}$ is
finite but not given by a formula. That suffices for the holomorphy of a
fixed Hahn coefficient. A bound uniform as $m\to\infty$ would be a stronger
theorem and does not follow; in particular, replacing $m$ by $n$ in
\eqref{dyn:nsc:eq:products} and discarding the prefactor is invalid
(source 10). Source 10's appendix restates \cref{dyn:nsc:thm:products} in a
form that mentions no Hahn field; it is the same statement and is not
reprinted.

\begin{remark}[The scalar case, and how the routes differ]
\label{dyn:nsc:rem:scalar}
For $\Lambda=\lambda I_d$ every colored tree has $\mathfrak d_v=\mathfrak d_{s_v}$,
so its internal skeleton is a positively weighted rooted tree with the same
subtree sums, and $\max(1,\mathfrak d^{-1})\le2\mathfrak d^{-1}$ because
$\mathfrak d\le2$. Hence $A^{\lambda I_d}_m(n)\le2^{m}A_m(n)$, and
\cref{dyn:nsc:cor:rankone} for a scalar multiplier already follows from the
rate statement of \cref{dyn:esm:cor:chain}(ii). The two proofs are different. Sources 08 and 09
bound each tree explicitly, by $(C_an)^{m}e^{an}$
(\cref{dyn:esm:thm:treebound}), using one rational cluster
(\cref{dyn:esm:lem:cluster}) and divisible-subtree packing
(\cref{dyn:esm:lem:packing}). Source 10 bounds only the limsup along
subsequences, by the rank count \cref{dyn:nsc:cor:angular} and the block
budget \eqref{dyn:nsc:eq:budget}, and its constant is not explicit. The
blocks $B_v$ of \cref{dyn:nsc:lem:blocks} are those of
\cref{dyn:esm:lem:packing}; what is new is that the contracted leaf vector
$\beta_v$ keeps each block's divisor inside the nonlinear cone in several
colors. Both routes are kept under \cref{dyn:conv:merge}(ii). For a scalar
multiplier the explicit bound is strictly more information; for a nonscalar
one only source 10's route is available.
\end{remark}

\subsection{Route D in several variables, and the proof of the common domain}
\label{dyn:nsc:subsec:proof}

Source 10 works with the forward coordinate, as source 09 does in one
variable (\cref{dyn:tree:subsec}). With $\check H=z+\check h$ and
$F=\Lambda z+f$, the equation $F\circ\check H=\check H\circ\Lambda$ reads
\begin{equation}\label{dyn:nsc:eq:forward}
 \Li\check h=f(z+\check h),\qquad \Li\check h=\check h(\Lambda z)-\Lambda\check h(z),
\end{equation}
the several-variable form of \eqref{dyn:tree:eq:forward}, with $\Li$ of
\eqref{dyn:eq:homological}.

\begin{proposition}[Colored tree expansion of the forward coordinate;
source 10]\label{dyn:nsc:prop:formal}
For formal input coefficients of order at least two and well-ordered
$S\subset\Gamma_{>0}$, \eqref{dyn:nsc:eq:forward} has a unique normalized
positive-support formal solution, supported in $S^{*}\setminus\{0\}$. At a
fixed exponent $\gamma\in S^{*}\setminus\{0\}$ its expansion uses trees with
at most $\ell_S(\gamma)$
internal vertices, labelled only by letters in $D_S(\gamma)$, and at a fixed
exponent and spatial multi-index it is a finite sum. Each vertex $v$
contributes one Taylor coefficient $f_{\eta_v,j_v,\mu_v}$ of an input, with
Hahn label $\eta_v\in S$ and $|\mu_v|=a_v$, and the divisor $\lambda^{\alpha_v}-\lambda_{j_v}$ of
modulus $\mathfrak d_v$.
\end{proposition}
\begin{proof}
Existence and uniqueness are \cref{dyn:prop:linearization} for $H$, with
$\check H=H^{-1}$ from \cref{dyn:prop:subgroup}; source 10 also proves them
directly, by a least-difference argument. If two normalized positive
solutions differ first at exponent $\delta$, every term in
$f(z+\check h)-f(z+\check h')$ involving that difference has an additional
positive input weight and therefore has weight greater than $\delta$.
Its coefficient at $\delta$ is zero. The forward equation then gives
$\Li(\check h_\delta-\check h'_\delta)=0$, contradicting nonresonance.
The least differing exponent exists in the union of the two well-ordered
positive supports; neither support need be assumed contained in $S^{*}$.
For the expansion, iterate
$\check h=\Li^{-1}f(z+\check h)$ formally. A coefficient
$f_{s,j,\mu}z^{\mu}e_j$ supplies an internal vertex of output color $j$,
arity $|\mu|\ge2$ and Hahn letter $s=\eta_v$. This label is distinct
from the integer subtree weight $s_v$ in \eqref{dyn:nsc:eq:weights}.
For $z^\mu$, distinguish the $\mu_i$ slots of coordinate $i$, listed in
coordinate order; this records the usual multinomial multiplicities.
Expanding a slot by a further copy of
$\check h$ supplies a child, and an unexpanded slot becomes an external leaf
of its coordinate color. The subtree of $v$ carries the monomial
$z^{\alpha_v}e_{j_v}$, so by \eqref{dyn:eq:homological} the divisor at $v$
is $\lambda^{\alpha_v}-\lambda_{j_v}$. The Hahn exponent is the sum of the
letters, which form a word in $\word_S(\gamma)$, so there are at most
$\ell_S(\gamma)$ vertices (\cref{dyn:lem:neumann}). At a fixed spatial
degree every arity is bounded and there are finitely many trees, monomials
and slot choices.
\end{proof}

This is the several-variable, colored form of source 09's route D
(\cref{dyn:tree:prop:formula}); each slot-labelled term occurs once, so
there are no symmetry factors.

\begin{lemma}[Polynomial diagram count; source 10]\label{dyn:nsc:lem:count}
Fix $m,d\ge1$ and a finite set of input labels. There is a constant $C$
such that the number of slot-labelled expansion terms of total spatial
degree $n+1$ with at most $m$ internal vertices is at most
$C(n+m+1)^{dm+m}$. A term with $m'$ vertices has total input monomial
degree $n+m'$, and the count includes the multinomial multiplicities of
substitution.
\end{lemma}
\begin{proof}
There are finitely many rooted plane skeletons with $m'\le m$ vertices,
output colorings and label assignments. Each vertex's input multi-index has
coordinates at most $n+m$, giving at most $(n+m+1)^{dm'}$ choices. After
that, each of the $m'-1$ children is assigned to a compatible slot of its
parent, in at most $(n+m+1)^{m'-1}$ ways. The remaining slots are leaves
whose colors are already fixed by the input monomial, so there is \emph{no}
independent factor $d^{n+1}$ for them. Ordering children and slots only
overcounts and absorbs the binomial coefficients of the repeated factors
$(z_i+\check h_i)^{\mu_i}$. The degree assertion is $\sum_va_v=n+m'$.
\end{proof}

Only polynomiality in $n$ at fixed $m$ matters, as in
\cref{dyn:tree:lem:count}. Treating leaf colorings as independent would
cost a spurious factor exponential in $n$ and a smaller radius (source 10).

\begin{proof}[Proof of \cref{dyn:nsc:thm:main}]
Construct $\check H$ by \cref{dyn:nsc:prop:formal}. For
$\gamma\notin S^{*}\setminus\{0\}$ its coefficient is zero; this also
settles every coefficient if $S$ is empty. Otherwise put
$m=\ell_S(\gamma)\ge1$ and fix $0<r<R$. By Cauchy's estimate there is a finite
$M_\gamma(r)\ge1$ with
$|f_{s,j,\mu}|\le M_\gamma(r)r^{-|\mu|}$ for the finitely many $s\in D_S(\gamma)$
and all $j,\mu$. A term of total spatial degree $n+1$ with $m'\le m$
vertices has input coefficients of product at most
$M_\gamma(r)^{m'}r^{-(n+m')}\le C_{\gamma,r}r^{-n}$, and by
\cref{dyn:nsc:thm:products} its divisor product is at most
$C_{m,\eps,\Lambda}e^{(\min(\rang,m)\tau+\eps)n}$. With
\cref{dyn:nsc:lem:count}, and the finitely many letter assignments summing
to $\gamma$ absorbed into the constant,
\begin{equation}\label{dyn:nsc:eq:coefbound}
 \max_{j,\ |\mu|=n+1}\bigl|[z^{\mu}]\check h_{\gamma,j}\bigr|
 \le C'(n+m+1)^{dm+m}\,r^{-n}e^{(\min(\rang,m)\tau+\eps)n}.
\end{equation}
There are $\binom{n+d}{d-1}$ multi-indices of degree $n+1$, only
polynomially many in $n$. On any closed polydisk of radius
$q<re^{-\min(\rang,m)\tau-\eps}$, summing the absolute Taylor terms is
therefore bounded by a polynomial times a geometric series with ratio
$q e^{\min(\rang,m)\tau+\eps}/r<1$. This proves normal convergence and
$\radiso(\check h_\gamma)\ge re^{-\min(\rang,m)\tau-\eps}$.
Letting $r\uparrow R$ and $\eps\downarrow0$,
\begin{equation}\label{dyn:nsc:eq:localdepth}
 \radiso(\check h_\gamma)\ge Re^{-\min(\rang,\ell_S(\gamma))\tau}\ge Re^{-\rang\tau}.
\end{equation}
The last bound does not depend on $\gamma$, so every $\check h_\gamma$ is
holomorphic on $U=\mathbb D^{d}_{Re^{-\rang\tau}}$. \Cref{dyn:prop:subgroup},
with this $U$, gives the inverse $H$ on the same domain, with no second loss
of radius and support in $S^{*}\setminus\{0\}$; source 10's own positive
inverse lemma is this statement and is not reprinted. The formal identities
transport \eqref{dyn:nsc:eq:forward} to $H\circ F=\Lambda H$, and uniqueness
is formal. The halo statement is the proof of \cref{dyn:prop:halo} with $U$
in place of $\mathbb D_R^{d}$: the reductions of $H,H^{-1}$ are the identity
and that of $F$ is $z\mapsto\Lambda z$, which preserves $U$ because
$|\lambda_j|=1$. Workspace invariance is \cref{dyn:prop:workspace}: every
formula uses only the input exponents and finite sums of them.
\end{proof}

\begin{corollary}[Input-relative radius at a specified exponent; source 10]
\label{dyn:nsc:cor:local}
Assume $\nres$, $\tau<\infty$ and exactness, with input coefficients
convergent germs. For $\gamma\in S^{*}\setminus\{0\}$ let $R_\gamma>0$ be
the minimum of $\radiso(f_s)$ over the finite set $s\in D_S(\gamma)$. Then
\begin{equation}\label{dyn:nsc:eq:relative}
 \radiso(h_\gamma),\ \radiso(\check h_\gamma)
 \ge R_\gamma\,e^{-\min(\rang,\ell_S(\gamma))\tau}.
\end{equation}
\end{corollary}
\begin{proof}
For $\check h_\gamma$ use the Cauchy bound with any $r<R_\gamma$ in the proof
above. For $h_\gamma$ expand the inverse recursion. Every $\check h_\beta$
that occurs has an input word that extends to one at $\gamma$, so its
letters lie in $D_S(\gamma)$ and $\ell_S(\beta)\le\ell_S(\gamma)$
(concatenate a longest word at $\beta$ with a word for the remaining
weight). Thus $D_S(\beta)\subseteq D_S(\gamma)$ and
$R_\beta\ge R_\gamma$; since $\tau\ge0$, the forward-coordinate bound
for $\beta$ is at least the radius in \eqref{dyn:nsc:eq:relative}.
Only finitely many such weights and word partitions occur at $\gamma$.
Finite sums, products and coordinate derivatives of their holomorphic
coefficients preserve that open polydisk, proving the inverse bound without
another homological inversion.
\end{proof}

Since $\min(\rang,\ell)\le\ell$, \eqref{dyn:nsc:eq:relative} improves the
word-depth bound \eqref{dyn:eq:radius-depth} of \cref{dyn:thm:radius-depth}
whenever $\ell_S(\gamma)>\rang$, and reproduces it otherwise. Arbitrary
radius-free input germs thus acquire a finite input-relative certificate
even when there is no common input radius.

\begin{corollary}[Rectangular polydisks and ordinary parameters; source 10]
\label{dyn:nsc:cor:parameters}
If the input coefficients are holomorphic on $\prod_j\mathbb D_{R_j}$,
where $0<R_j\le\infty$, all
coefficients of $H$ and $H^{-1}$ are holomorphic on
$\prod_j\mathbb D_{R_je^{-\rang\tau}}$. If the $f_s$ also depend
jointly holomorphically on $(z,w)\in(\prod_j\mathbb D_{R_j})\times V$,
where $V$ is an ordinary complex parameter domain, with
$\Lambda$ independent of $w$, the coefficients are jointly holomorphic on
that polydisk times $V$.
\end{corollary}
\begin{proof}
For finite radii the rescaling $z=Dx$, $D=\operatorname{diag}(R_j)$,
commutes with $\Lambda$ and does not change its divisors. The inputs become
$D^{-1}f_s(Dx)$; apply \cref{dyn:nsc:thm:main} with radius one and undo
the rescaling. If some radii are infinite, exhaust those coordinates by
finite radii; formal uniqueness makes the resulting coefficients agree.
For parameters, fix a relatively compact subset of $V$. The finitely
many inputs used at one exponent have Cauchy bounds uniform on it, so
\eqref{dyn:nsc:eq:coefbound} gives normal convergence on compact subsets of
the polydisk, uniformly in the parameter. The Taylor coefficients are
holomorphic in $w$, hence so is the sum; exhaust $V$.
\end{proof}

The rescaling part is the nonscalar form of source 08's unequal-radius
statement after \cref{dyn:esm:thm:main}; like that one, it says nothing
about optimal Reinhardt domains. The joint holomorphy in ordinary
parameters is new to this report. The constants in
\eqref{dyn:nsc:eq:coefbound} may grow arbitrarily fast from one exponent to
another; only their finiteness at each exponent is used. An ordinary
coefficient of size $10^{10^{10}}$ still has valuation zero
(\cref{dyn:rem:invisible}), and a common domain of holomorphy is not a
common norm bound (\cref{dyn:warn:nouniform}).

Source 10 lists the places where a proof of \cref{dyn:nsc:thm:main} could
fail and how its proof avoids each.
\begin{center}
\begin{tabular}{@{}P{0.28\textwidth}P{0.64\textwidth}@{}}
\toprule
Potential failure & How it is handled\\
\midrule
Exponent differences leave the nonlinear cone & Only contracted tree
characters are used; \cref{dyn:nsc:lem:blocks} gives a nonnegative leaf
vector of degree at least two.\\[3pt]
A rational relation with huge coefficients & Integer minors have modulus
at least one, and Cramer numerators grow polynomially with the height.\\[3pt]
Exponential cancellation changes a rate & The block argument needs only an
upper bound on the angular error. Cancellation helps, and exact resonance is
excluded by $\nres$.\\[3pt]
Exponentially many leaf colorings & Leaf colors are fixed by the input
multi-indices; only the $m-1$ internal edges choose slots
(\cref{dyn:nsc:lem:count}).\\[3pt]
A different radius at each Hahn exponent & The prefactor varies with the
exponent, but the exponential rate is bounded by the same $\rang\tau$
everywhere.\\[3pt]
Inverting the conjugacy loses another factor & Positive Hahn inversion
uses only differentiation and multiplication, no homological inversion
(\cref{dyn:prop:subgroup}).\\
\bottomrule
\end{tabular}
\end{center}

\subsection{The exact criterion, and sharpness in angular rank one}
\label{dyn:nsc:subsec:sharp}

\begin{theorem}[Exact criterion and rank-one sharpness; source 10]
\label{dyn:nsc:thm:criterion}
Fix a diagonal unitary $\Lambda$ with $\nres$, a nonzero $\Gamma$ and
$0<R<\infty$. The following are equivalent:
\begin{enumerate}[label=(\roman*)]
\item $\tau(\Lambda)<\infty$;
\item every input \eqref{dyn:nsc:eq:input} with coefficients on
$\mathbb D_R^{d}$ has a normalized conjugacy and inverse whose coefficients
are all holomorphic on one common polydisk of positive radius;
\item every input with one positive exponent and entire coefficient
functions has a normalized conjugacy whose coefficients are convergent
germs.
\end{enumerate}
For $\tau<\infty$ let $R_{\mathrm{univ}}$ be the supremum of the $q>0$ such
that every input on $\mathbb D_R^{d}$ has all coefficients of its normalized
conjugacy and inverse holomorphic on $\mathbb D_q^{d}$. Then
\begin{equation}\label{dyn:nsc:eq:gap}
 Re^{-\rang\tau}\le R_{\mathrm{univ}}\le Re^{-\tau},
\end{equation}
the lower value being attained as a guaranteed open domain. If $\rang=1$,
then $R_{\mathrm{univ}}=Re^{-\tau}$ exactly.
\end{theorem}
\begin{proof}
(i)$\Rightarrow$(ii) and the lower bound are \cref{dyn:nsc:thm:main}. (ii)
implies (iii) by restricting an entire one-exponent input to $\mathbb D_R^{d}$.
(iii)$\Rightarrow$(i) is the necessity half of rows two and three of
\cref{dyn:thm:main}: if $\tau=\infty$, the entire forcing
\eqref{dyn:eq:entire-witness} of \cref{dyn:lem:sharp-divisor} placed at one
weight $\eta$ has, by \cref{dyn:prop:first-weight}, a forced coefficient
$h_\eta=-\Li^{-1}\varphi$ of radius zero. For the upper bound, take the
witness \eqref{dyn:eq:finite-radius-witness} at one weight $\eta$; its
$h_\eta$ has isotropic radius exactly $Re^{-\tau}$, so no larger
$q$ is guaranteed. Source 10 notes that this witness needs no case
distinction at $\tau=0$: choose degrees $n_k$ with
$n_k^{-1}\log\Delta_{n_k}^{-1}\to\tau$, which exist also when $\tau=0$
because $\Delta_n\le2$ bounds the negative parts, and use the coefficients
$R^{-n_k}$; the root test gives the radius $Re^{-\tau}$ directly. When
$\rang=1$ the two bounds of \eqref{dyn:nsc:eq:gap} coincide.
\end{proof}

Source 10 includes the necessity and upper-bound halves only to make the
criterion complete; they are the first-weight obstructions of
\cref{dyn:lem:sharp-divisor,dyn:prop:first-weight}, printed once. For
$\tau=0$, \eqref{dyn:nsc:eq:gap} gives $R_{\mathrm{univ}}=R$, which is row one
of \cref{dyn:thm:main}. For $\rang\ge2$ and $0<\tau<\infty$ the theorem
decides the \emph{existence} of a common positive radius, not its optimal
value.

``Sharp'' refers to the largest \emph{universally guaranteed centered
isotropic polydisk} for the ordinary coefficient functions. A particular
input may have much larger domains, or be polynomial at every exponent, and
a coefficient may continue analytically beyond its centered Taylor
polydisk; none of this contradicts sharpness (source 10). The proof also
gives no classification of nonnormalized, non-Hahn-coherent or purely
set-theoretic conjugacies.

\subsection{Nonscalar examples}
\label{dyn:nsc:subsec:examples}

\begin{proposition}[Two modes in one irrational direction; source 10]
\label{dyn:nsc:prop:twomode}
Let $\lambda=e^{2\pi i\theta}$ with $\theta$ irrational and
$\Lambda=\operatorname{diag}(\lambda^{2},\lambda^{3})$. Then $\Lambda$ is
nonscalar, satisfies $\nres$, has $\rang(\Lambda)=1$, and
\begin{equation}\label{dyn:nsc:eq:twomode}
 \tau(\Lambda)=3\,\tau(\lambda),\qquad
 \tau(\lambda)=\limsup_{k\to\infty}\frac1k\log^{+}\mathfrak d_k^{-1},
\end{equation}
in the extended sense when $\tau(\lambda)=\infty$; here $\tau(\lambda)$ is
\eqref{dyn:esm:eq:rate} and \eqref{dyn:eq:tau-cf}. Consequently, when
$\tau(\lambda)<\infty$, every exact input \eqref{dyn:nsc:eq:input} for this
$\Lambda$, with arbitrary mixed monomials and arbitrary well-ordered positive
support, has all coefficients on the polydisk of radius $Re^{-3\tau(\lambda)}$,
and this radius is the optimal universal one.
\end{proposition}
\begin{proof}
If $a+b\ge2$ then $2a+3b\ge4$, so $2a+3b\notin\{2,3\}$ and irrationality
excludes $\lambda^{2a+3b}=\lambda^{c}$ for $c\in\{2,3\}$; this is $\nres$.
The angles $2\theta,3\theta$ span the same rational space with $1$ as
$\theta$ does, so $\rang=1$. A divisor of degree $n=a+b\ge2$ is
$|\lambda^{k}-1|$ with $k=2a+3b-c$, $c\in\{2,3\}$, and $1\le k\le3n-2$. If
$\tau(\lambda)<\infty$ and $u>\tau(\lambda)$, then
$|\lambda^{k}-1|^{-1}\le C_ue^{uk}\le C_ue^{3un}$, so $\tau(\Lambda)\le3u$.
Conversely, for large $k$ choose $a\in\{0,1,2\}$ with $2a\equiv k\pmod3$
and $b=(k+3-2a)/3$: then $2a+3b-3=k$, so $|\lambda^{k}-1|$ occurs in
component $2$ at degree $n=a+b=k/3+O(1)$. Along a sequence realizing
$\tau(\lambda)$ this gives $\tau(\Lambda)\ge3\tau(\lambda)$, and
$\tau(\Lambda)=\infty$ when $\tau(\lambda)=\infty$. The last assertion is
\cref{dyn:nsc:thm:main,dyn:nsc:thm:criterion} with $\rang=1$.
\end{proof}

The hypothesis is not vacuous at positive rate. \Cref{dyn:thm:nonbrjuno}(c)
gives, for each $s>0$, the explicit continued fraction
$a_{j+1}=\lceil e^{sq_j}\rceil$ with $\tau(\lambda)=s$
(\cref{dyn:prop:cf}). Source 10 constructs the same family, starting from
$a_1=1$, and proves its rate by the same continued-fraction estimates; it
is printed once, and source 10 does not claim it. For
\cref{dyn:nsc:prop:twomode} it gives a nonscalar spectrum with
$\tau(\Lambda)=3s$ and sharp common radius $Re^{-3s}$, so for any
$0<\kappa<1$ the choice $s=-\tfrac13\log\kappa$ gives universal radius
exactly $\kappa R$. The theorem therefore covers genuinely exponential small
divisors, not merely rate zero, and the input may couple the two coordinates
arbitrarily: the example is not two uncoupled scalar maps.

\begin{example}[Angular rank two with zero loss; source 10]
\label{dyn:nsc:ex:ranktwo}
Let $\theta=(\sqrt2,\sqrt3)$. Since $1,\sqrt2,\sqrt3$ are rationally
independent, $\Lambda$ satisfies $\nres$ and $\rang=2$. For integers
$u,v,p$ with $|u|+|v|=O(n)$ and $(u,v)\ne0$, the algebraic integer
$u\sqrt2+v\sqrt3-p$ is nonzero, its norm from $\Q(\sqrt2,\sqrt3)$ is a nonzero
integer, and its three other conjugates are $O(n+1)$ when $p$ is a nearest
integer to $u\sqrt2+v\sqrt3$. Hence
$|u\sqrt2+v\sqrt3-p|\ge C(n+1)^{-3}$. With $(u,v)=\alpha-e_j$, the sine
bound \eqref{dyn:esm:eq:sine} transfers this estimate to every homological
divisor, giving a polynomial lower bound. Thus $\tau=0$ and the
original polydisk is kept. This is row one of \cref{dyn:thm:main}; source
10 gives it only to show that $\rang\ge2$ is compatible with zero loss, and
it is a familiar algebraic-number estimate, not a new Diophantine statement.
\end{example}

As a nongrid well-ordered-support example, source 10 uses the set $S$
and group $\Gamma=\Q+\Q\Omega$ of \cref{dyn:ex:ranktwo}, with any of the
spectra above and independently chosen order-two coefficients $f_s$ on
$\mathbb D_R^{d}$, with no bound uniform in $s$. \Cref{dyn:nsc:thm:main}
gives one common domain for all coefficients; $\check h_\Omega=\Li^{-1}f_\Omega$
and $h_\Omega=-\check h_\Omega$, since the only word at $\Omega$ is $(\Omega)$,
and at $\Omega+2$ only the three permutations of $(\Omega,1,1)$ occur. The
infinitely many input exponents below $\Omega$ do not enter the coefficient
at $\Omega$. The construction is local in the support, not a sequential
limit through all smaller scales (\cref{dyn:warn:contraction}). Source 10's
lexicographic $\Z^{2}$ caution is \cref{dyn:esm:ex:lex}, and its
surcomplex realization through \eqref{dyn:eq:embedding}, with the sign
$t^{\gamma}=\omega^{-\gamma}$ and set-sized workspaces, is
\cref{dyn:subsec:halo,dyn:prop:workspace}; neither is reprinted. All
derivatives are coordinate derivatives of coefficient functions, not the
Berarducci--Mantova derivation (\cref{dyn:warn:derivative}), and strong
summation is the support operation of \cref{dyn:def:summable}; convergence
of partial sums in the fine topology is not assumed. The failure at
$\tau=\infty$ concerns ordinary
coefficient holomorphy only; the formal conjugacy still evaluates on the
monad (\cref{dyn:prop:monad}).

\subsection{What source 10 does not settle}
\label{dyn:nsc:subsec:limits}

\emph{The optimal radius in angular rank at least two.} For $\rang\ge2$ and
$0<\tau<\infty$ the universal radius lies between the two bounds of
\eqref{dyn:nsc:eq:gap} and is not determined. The proof charges up to
$\tau$ to each distinct exponential rate. It does not show that several rate
values can saturate their charges simultaneously, nor that saturation would
survive the coefficient cancellations of an actual conjugacy. A sharper
count might reduce the factor $\rang$; a matching lower example would need
more than large individual tree factors (source 10). This is what remains of
\cref{dyn:q:commongerm}.

\emph{Other multipliers.} No theorem is asserted for infinitesimally drifting
multipliers in several variables, resonant spectra, nontrivial Jordan
blocks, nonunitary spectra in analytic categories, or infinite-dimensional
coordinate spaces. The support method is compatible with many of these
settings, but the admissible-divisor and polydisk-geometry arguments above
do not transfer automatically.

\emph{Ordinary specialization and depth uniformity.} Nothing is asserted
after substituting a nonzero complex number for $t$. A bound on each
coefficient's spatial radius gives no control of its size as the Hahn or
parameter order grows, and the prefactor of \cref{dyn:nsc:thm:products} is
not uniform in $m$. So the theorem neither replaces a Bruno-type theorem for
ordinary germs nor contradicts the known failures of classical analytic
linearization (\cref{dyn:esm:rem:nocontra,dyn:q:realization}).

\section{The drifted category: exact radius collapse and a trichotomy}
\label{dyn:sec:drift}

This section decides the common-domain germ category in the drifted
category $\sddr$, negatively. At the pins of sources 01--07 it was the only
place in the report where that category was decided at all.
\Cref{dyn:esm:sec} now decides it for an exact scalar multiplier, with the
opposite answer. \Cref{dyn:warn:drift} applies to everything in this
section, including the single-exponent theorem of \cref{dyn:cyc:subsec}.
...
separate problem. Source 08 later resolved it in one variable. Over $\Z$ the
colliding blocks cannot cancel the highest pole, and the coefficient radii
are $Re^{-m\sigma}$ (\cref{dyn:cyc:thm}).
\end{remark}
...
This is source 02's statement. Source 08 sharpens it to \emph{one} nonzero
positive Hahn coefficient over $\Z$ (\cref{dyn:cyc:cor:onecoef}).
...
\begin{remark}[One cyclic scale suffices; source 08]
\label{dyn:cyc:rem:trichotomyZ}
Both equivalences of \cref{dyn:thm:trichotomy} also survive replacing the
universal quantifier over $\Gamma$ by \emph{any} single nonzero group, in
particular by $\Z$. See \cref{dyn:cyc:cor:classification} and the paragraph
after it. The negative assertions then use one Hahn monomial instead of the
two exponents of $\Gamma_{*}$. The drift hypothesis is still required.
\end{remark}
...
Negative common-domain assertions use $\Gamma_{*}=\Q+\sqrt2\,\Q$ here; by
source 08 they already hold over $\Z$ with a single Hahn monomial
(\cref{dyn:cyc:thm}). Positive assertions hold for every set-sized ordered
abelian group. The polynomial
...
\subsection{A single cyclic exponent: collisions cannot cancel the highest pole}
\label{dyn:cyc:subsec}

This subsection is source 08's, and \textbf{everything in it requires
drift} ($\sddr$, \cref{dyn:warn:drift}). It answers \cref{dyn:q:cyclic} in
the one-variable setting in which that question is posed. The negative
direction of \cref{dyn:thm:collapse}, proved above with two rationally
independent exponents, already holds over the cyclic value group $\Z$ with
a single Hahn monomial. Rates are written $\sigma=\sigma(\lambda)$, never
$\tau$, because the multiplier drifts. The numerical equality $\sigma=\tau$
of \cref{dyn:prop:cf} does not move a statement from $\sddr$ to $\sdex$.

Work over $K_{\Z}=\C((t))$, $v(t)=1$, and for a fixed finite $R>0$ put
\begin{equation}\label{dyn:cyc:eq:input}
 F_t(z)=(\lambda+t)z+t\,P_R(z),
\end{equation}
with $P_R$ as in \eqref{dyn:eq:PR}. One Hahn monomial, $t$, carries both
the drift $z$ and the nonlinearity $P_R(z)$. \Cref{dyn:thm:lifting}, with
$A=\{1\}$ and $f_1=z+P_R$, gives a unique normalized positive-support
formal linearizer
\begin{equation}\label{dyn:cyc:eq:H}
 H_t(z)=z+\sum_{m\ge1}t^{m}h^{\mathrm{cyc}}_m(z),\qquad
 H_t\circ F_t=(\lambda+t)H_t .
\end{equation}

\emph{Bookkeeping by two formal parameters.} Let $u,v$ be independent
formal parameters and let $F_{u,v}$ be as in \eqref{dyn:eq:familyintro},
now over $\C[[u,v]]$ rather than over $\Gamma_{*}$. Write
$H_{u,v}=z+W$ with $W=\sum_{k\ge0,\ \ell\ge1}u^{k}v^{\ell}h_{k,\ell}(z)$.
The conjugacy equation $H_{u,v}\circ F_{u,v}=(\lambda+u)H_{u,v}$ becomes
\begin{equation}\label{dyn:cyc:eq:master}
 \Li W=-vP_R+uW-\NN_{u,v}W,\qquad
 \NN_{u,v}W=W\bigl(\lambda z+uz+vP_R(z)\bigr)-W(\lambda z).
\end{equation}
The operator $\Li^{-1}(u\,\id-\NN_{u,v})$ raises the total parameter degree
by at least one. So \eqref{dyn:cyc:eq:master} has a unique normalized
solution: the geometric series in that operator applied to
$-\Li^{-1}(vP_R)$. Each parameter degree is a finite sum of operator
patterns, and every term is divisible by $v$. The map
$(k,\ell)\mapsto k+\ell\sqrt2$ is injective on $\N^{2}$, and the coefficient
recursion has a unique solution; so the $h_{k,\ell}$ are exactly the
coefficients at $u^{k}v^{\ell}$ of the linearizer of
\eqref{dyn:eq:familyintro} over $\Gamma_{*}$, and in particular
$h_{k,1}$ is the function of \eqref{dyn:eq:blockformula}. The parameters
$u,v$ are bookkeeping only. They do not assume that the final value group
contains two independent positive elements.

\emph{The fold.} Only finitely many monomials $u^{k}v^{\ell}$ have a given
total degree. So $u=v=t$ defines a homomorphism from formal series in
$(u,v)$ to formal series in $t$ that respects the Taylor substitutions in
\eqref{dyn:cyc:eq:master}. It carries $F_{u,v}$ to $F_t$ and the conjugacy
equation to that of \eqref{dyn:cyc:eq:H}, so by uniqueness
\begin{equation}\label{dyn:cyc:eq:collision}
 h^{\mathrm{cyc}}_m=\sum_{\ell=1}^{m}h_{m-\ell,\ell}.
\end{equation}
The whole sum \eqref{dyn:cyc:eq:collision} must be analyzed, not one of its
summands. Over $\Z$ the blocks that \cref{dyn:rem:twoexponents} keeps apart
collide at a single Hahn exponent. The summand with $\ell=1$ is $h_{m-1,1}$,
whose radius is $Re^{-m\sigma}$ by \eqref{dyn:eq:collapse}; its highest pole
is \eqref{dyn:eq:highestpole} with $k=m-1$. The question is whether the
summands with $\ell\ge2$ can cancel that pole.

\begin{lemma}[Pole multiplicity after collisions; source 08]
\label{dyn:cyc:lem:chains}
At total parameter degree $m$, an expanded term of the geometric-series
solution of \eqref{dyn:cyc:eq:master} that contributes to $z^{n}$ contains at
most $m$ homological inversions. Their Taylor degrees form a nondecreasing
chain in $\{2,\dots,n\}$ whose last entry is $n$. If the term contains at
least two factors $P_R$, at least one inversion has Taylor degree strictly
below $n$. In particular the power of $d_n^{-1}$ in such a term is at most
$m-1$.
\end{lemma}
\begin{proof}
The initial forcing uses one parameter $v$, one $P_R$ and one inversion. The
term $uW$ adds one parameter and one inversion without changing the
coordinate degree. A Taylor block of order $k$ in $\NN_{u,v}$ differentiates
$k$ times and multiplies by $k$ factors, each either $uz$ or $vP_R$. A factor
$uz$ has degree one, so its net increment is zero. A monomial of $P_R$ has
degree at least two, so its net increment is positive. Degrees therefore
never decrease, and any block that contains a further $P_R$ strictly
increases them. Each new inversion costs at least one more parameter
degree, so at most $m$ inversions occur. With two or more factors $P_R$, a
strict increase happens after the initial inversion, so some inversion sits
below the final degree $n$.
\end{proof}

The argument applies term by term, before any cancellation. It is therefore
unaffected by the many parameter blocks that merge when $u=v=t$.

\begin{lemma}[Polynomial numerators at fixed parameter degree; source 08]
\label{dyn:cyc:lem:majorant}
Fix $m\ge1$ and $R>0$. In degree $z^{n}$, the expanded terms of total
parameter degree $m$ are bounded in number by a polynomial in $n$, and so
are the moduli of their derivative and combinatorial factors, with constants
depending only on $m$. If a term contains $\ell$ factors $P_R$, the product
of their Taylor coefficients is exactly $R^{\ell+1-n}$.
\end{lemma}
\begin{proof}
There are finitely many Taylor-block patterns of total parameter degree
$m$, independently of $n$. Coordinate degrees in a nonzero pattern are at
most $n$, so its derivative factors grow polynomially. If the $\ell$
nonlinear factors have degrees $n_1,\dots,n_\ell$, the linear factors
contribute nothing to the degree, so $\sum_{j=1}^{\ell}(n_j-1)=n-1$. There
are at most $n^{\ell-1}$ such degree lists, and the coefficients
$R^{2-n_j}$ of $P_R$ multiply to $R^{2\ell-(n-1+\ell)}=R^{\ell+1-n}$. The
operations $uW$ have coefficient one, and $1\le\ell\le m$, so the powers
$R^{\ell}$ are absorbed into constants depending on $m$ and $R$.
\end{proof}

\begin{proposition}[The upper bound under drift; source 08]
\label{dyn:cyc:prop:upper}
If $\sigma<\infty$, then for every $a>\sigma$ and every fixed $m$ there are a
constant $A_{m,a,R}$ and an integer $B_m$ with
\begin{equation}\label{dyn:cyc:eq:upper}
 \bigl|[z^{n}]h^{\mathrm{cyc}}_m\bigr|\le A_{m,a,R}\,n^{B_m}R^{-n}e^{man}.
\end{equation}
Consequently $\rad(h^{\mathrm{cyc}}_m)\ge Re^{-m\sigma}$.
\end{proposition}
\begin{proof}
By \cref{dyn:cyc:lem:chains} each of the at most $m$ divisors is some
$d_{n'}$ with $n'\le n$. Each reciprocal is at most $A_ae^{an}$, for a
constant $A_a$ supplied by the definition of $\sigma$. So their product is
at most $A_a^{m}e^{man}$. Combine this with \cref{dyn:cyc:lem:majorant},
sum over the finitely many patterns, apply the root test, and let
$a\downarrow\sigma$.
\end{proof}

Repeated indices prevent replacing $ma$ by $a$ here, unlike in the exact
case (\cref{dyn:esm:prop:coefbound}). The next proposition shows that this
worst loss really occurs, after all the collisions.

At a large convergent denominator $q_j$ of $\theta$, every earlier divisor
is only polynomially small. The best-approximation property
$\|s\theta\|_{\Z}\ge\|q_{j-1}\theta\|_{\Z}$ for $0<s<q_j$, combined with
\eqref{dyn:eq:cfbounds} at $j-1$ and \eqref{dyn:esm:eq:sine}, gives
\cite{Marmi,CarminatiMarmi}
\begin{equation}\label{dyn:cyc:eq:earlier}
 \mathfrak d_s\ge4\|q_{j-1}\theta\|_{\Z}>\frac4{q_{j-1}+q_j}>\frac2{q_j}
 \qquad(1\le s<q_j),
\end{equation}
for all sufficiently large $j$. This is source 08's lemma on convergents. Its
other assertions are \eqref{dyn:eq:cfbounds} and \cref{dyn:prop:cf}, and its
corollary on prescribed rates is \cref{dyn:thm:nonbrjuno}(c),(d); they are
not reprinted.

\begin{proposition}[Collision noncancellation at convergent degrees;
source 08]\label{dyn:cyc:prop:noncancel}
Fix $m=k+1\ge1$. Let $n-1=q_j$ be a sufficiently large convergent
denominator with $|d_n|<1$. Then
\begin{equation}\label{dyn:cyc:eq:dominant}
 [z^{n}]h^{\mathrm{cyc}}_{k+1}
 =-R^{2-n}(-1)^{k}\frac{a_{n,1}^{k}}{d_n^{k+1}}+E_{k,n},
 \qquad
 |E_{k,n}|\le C_{k,R}\,n^{B_k}R^{-n}|d_n|^{-k},
\end{equation}
with $a_{n,1}=n\lambda^{n-1}-1$ as in \cref{dyn:lem:jets}, and with finite
constants $C_{k,R}$ and integers $B_k$. For $k=0$ the error is zero.
\end{proposition}
\begin{proof}
Separate the summand $h_{k,1}$ in \eqref{dyn:cyc:eq:collision}. By
\eqref{dyn:eq:blockformula}, $[z^{n}]h_{k,1}=-R^{2-n}c_{n,k}$. By
\eqref{dyn:eq:jetremainder}, $c_{n,k}$ is the highest-pole term
\eqref{dyn:eq:highestpole} plus an error of modulus at most
$C_kn^{k}|d_n|^{-k}$; this is inside the stated bound. Every other summand
has at least two factors $P_R$. By \cref{dyn:cyc:lem:chains}, its final
divisor $d_n$ occurs to power at most $k$, and all its other divisors are
$d_{n'}$ with $2\le n'<n$. By \eqref{dyn:cyc:eq:earlier} each of those has
$|d_{n'}|=\mathfrak d_{n'-1}>2/q_j$, so its reciprocal is below $n/2$.
There are at most $m$ of them, so they cost only a polynomial factor, not an
exponential one. Since $|d_n|<1$, a power $|d_n|^{-i}$ with $i\le k$ is at
most $|d_n|^{-k}$. \Cref{dyn:cyc:lem:majorant} then gives the error bound
after summing \emph{all} colliding contributions. The argument bounds their
absolute values and assumes nothing about their signs or phases.
\end{proof}

\begin{theorem}[Sharp collapse with one cyclic exponent; \textbf{requires
drift}; source 08]\label{dyn:cyc:thm}
Let $\lambda=e^{2\pi i\theta}$ with $\theta$ irrational, $R>0$, and
$0<\sigma=\sigma(\lambda)<\infty$. The coefficients of the normalized
formal linearizer \eqref{dyn:cyc:eq:H} of \eqref{dyn:cyc:eq:input} satisfy
\begin{equation}\label{dyn:cyc:eq:radius}
 \boxed{\;\rad\bigl(h^{\mathrm{cyc}}_m\bigr)=R\,e^{-m\sigma}\qquad(m\ge1).\;}
\end{equation}
So every $h^{\mathrm{cyc}}_m$ is a convergent germ, and $H_t$ lies in the
radius-free ring $\mathcal A_1$ over $\Z$. But no positive disk carries all
of them, so $H_t$ is not in the common-domain germ ring $\mathcal R_1$. If
$\sigma=\infty$, every $h^{\mathrm{cyc}}_m$ has radius zero. The value
group is $\Z$ itself, and the input uses one Hahn monomial.
\end{theorem}
\begin{proof}
First let $0<\sigma<\infty$. By \eqref{dyn:eq:cfbounds} and
\eqref{dyn:esm:eq:sine},
$2/q_{j+1}\le4/(q_j+q_{j+1})<\mathfrak d_{q_j}<2\pi/q_{j+1}$, so
$\log\mathfrak d_{q_j}^{-1}$ and $\log q_{j+1}$ differ by less than
$\log(2\pi)$.
\Cref{dyn:prop:cf} therefore gives convergent denominators $q_j$ along which
$q_j^{-1}\log\mathfrak d_{q_j}^{-1}\to\sigma$. Put $n=q_j+1$, so that
$|d_n|=\mathfrak d_{q_j}$; then $|d_n|\le e^{-cn}$ eventually on this
subsequence for some $c>0$. By \cref{dyn:lem:jets}, $|a_{n,1}|\ge n-1$.
Dividing the error bound of \eqref{dyn:cyc:eq:dominant} by the modulus of
the leading term gives at most $C'_{k,R}n^{B'_k}|d_n|\to0$. Along the
subsequence the full coefficient is therefore the leading term times
$1+o(1)$, and
\[
 \frac{\log\bigl|[z^{n}]h^{\mathrm{cyc}}_m\bigr|}{n}
 =-\log R+m\,\frac{\log|d_n|^{-1}}{n}+o(1)\longrightarrow-\log R+m\sigma .
\]
With \cref{dyn:cyc:prop:upper} this gives \eqref{dyn:cyc:eq:radius}. The
radii tend to zero, so no positive disk works for every coefficient, even
though each coefficient converges separately. When $\sigma=\infty$, choose
convergent denominators with $q_j^{-1}\log\mathfrak d_{q_j}^{-1}\to\infty$.
The same ratio tends to zero, and the displayed growth tends to infinity.
\end{proof}

\begin{remark}[What the lower-bound proof actually excludes; source 08]
\label{dyn:cyc:rem:excludes}
The error may contain many terms, and some of them may be large. No termwise
positivity is claimed. Cancellation is excluded because the leading
contribution carries a full extra power of an exponentially small divisor,
while every earlier divisor at a convergent degree is at worst polynomially
small. A statement about independent parameter blocks alone would not
establish this. At $m=2$ the mechanism is visible directly. The drift block
contributes $R^{2-n}a_{n,1}/d_n^{2}$ to $[z^{n}]h^{\mathrm{cyc}}_2$; this is
$[z^{n}]h_{1,1}$. The block with two factors $P_R$ has two distinct divisor
indices, and at a convergent degree one of them is not exceptional. Higher
$m$ need the full bookkeeping above.
\end{remark}

\begin{corollary}[No independent scales are needed; source 08]
\label{dyn:cyc:cor:classification}
Work in one variable. Fix any nonzero set-sized ordered abelian group
$\Gamma$, in particular $\Z$, and an irrational unit multiplier $\lambda$.
Allow positive Hahn perturbations $F=\lambda z+P$ whose coefficients are
holomorphic on a common $\mathbb D_R$ and vanish at $0$, with infinitesimal
linear terms permitted, and normalize the target multiplier to be $F'(0)$.
Then every such input has a normalized linearizer with a common positive
coefficient disk \emph{if and only if} $\sigma(\lambda)=0$. At
$\sigma=0$ the original disk $\mathbb D_R$ is preserved.
\end{corollary}
\begin{proof}
Necessity for $\sigma>0$ is \cref{dyn:cyc:thm}, already over $\Z$. For
another nonzero $\Gamma$, choose $\eta>0$ and substitute $t\mapsto t^{\eta}$.
Since $n\mapsto n\eta$ is a strictly increasing additive embedding
$\Z\to\Gamma$, the linearizer of $(\lambda+t^{\eta})z+t^{\eta}P_R$ is
$z+\sum_mt^{m\eta}h^{\mathrm{cyc}}_m$, with the same coefficient functions.
No competing normalized solution with extra exponents exists. At the least
exponent of the difference of two solutions, all perturbative terms have
larger weight, since the difference of the target multipliers has positive
weight too. So $\Li$ annihilates the leading coefficient, and $\nres$ makes
it zero. This is source 08's workspace statement for the drifting equation,
the argument of \cref{dyn:prop:linearization,dyn:thm:collapse}. Sufficiency
at $\sigma=0$ is \cref{dyn:thm:lifting}(i), source 02's route B. Source 08
records that this mechanism was already in the repository and that its new
content is the single-scale witness.
\end{proof}

Combined with \cref{dyn:thm:lifting}(ii) and the $\sigma=\infty$ clause of
\cref{dyn:cyc:thm}, the corollary shows that \emph{both} equivalences of
\cref{dyn:thm:trichotomy} survive replacing the universal quantifier over
$\Gamma$ by any single nonzero group, in particular by $\Z$. Extending the
workspace cannot turn the formal linearizer of \eqref{dyn:cyc:eq:input} into
one with a common positive coefficient domain. And collapse is not the
absence of every surcomplex linearization: the formal conjugacy still
evaluates bijectively on the monad (\cref{dyn:prop:monad}, which source 08
restates for the drifting setting). Source 08 constructs no globally
Hahn-entire nonpolynomial map on $\No[i]$.

\begin{corollary}[One positive Hahn coefficient suffices; source 08]
\label{dyn:cyc:cor:onecoef}
For every irrational multiplier with $0<\sigma<\infty$, one nonzero positive
Hahn coefficient already suffices for a holomorphic perturbation of the
rotation to have a radius-free germ linearizer and no common-domain
linearizer. The perturbation can be taken to be $t\,(z+P_R(z))$ over $\Z$.
This sharpens \cref{dyn:cor:finitefailure}, which uses two coefficients over
$\Gamma_{*}$.
\end{corollary}

\begin{table}[ht]
\centering\small
\begin{tabular}{@{}P{0.20\textwidth}P{0.14\textwidth}P{0.22\textwidth}P{0.34\textwidth}@{}}
\toprule
One-variable category & Divisor tag & Universal common-germ condition
& Sharp behavior for a rate in $(0,\infty)$\\
\midrule
Exact multiplier $\lambda$ & $\sdex$ & $\tau<\infty$
& one common radius $Re^{-\tau}$ (\cref{dyn:esm:thm:main})\\[3pt]
Drifting multiplier $\lambda+u$ & $\sddr$, \textbf{drift required}
& $\sigma=0$ & one cyclic example already has radii $Re^{-m\sigma}$
(\cref{dyn:cyc:thm})\\
\bottomrule
\end{tabular}
\caption{The sixth category in one variable, by source 08. The two rows are
different categories and must never be merged into one ``intermediate
case'' (\cref{dyn:warn:resolution}). Positive assertions hold over every
nonzero $\Gamma$, and negative ones already over $\Z$. The nonscalar exact
case in several variables, open when source 08 wrote, is settled by source
10 except for the optimal radius when $\rang\ge2$ and $0<\tau<\infty$
(\cref{dyn:nsc:thm:criterion,dyn:q:commongerm}).}
\label{dyn:cyc:tab:exactdrift}
\end{table}

Source 08 draws the main qualitative conclusion itself. Exactness of the
multiplier changes the universal arithmetic threshold for a common
coefficient germ, even when the Hahn value group is held fixed.

\begin{example}[The same workspace, two answers; source 08]
\label{dyn:cyc:ex:logtwo}
Take $R=1$ and the multiplier of \cref{dyn:thm:nonbrjuno}(c) with
$s=\log2$, so that $\tau=\sigma=\log2$. Over the same $\C((t))$ compare
\[
 F_{\mathrm{ex}}(z)=\lambda z+t\,\frac{z^{2}}{1-z},
 \qquad
 F_{\mathrm{dr}}(z)=(\lambda+t)z+t\,\frac{z^{2}}{1-z}.
\]
For $F_{\mathrm{ex}}$ every coefficient of the linearizer and of its inverse
is holomorphic on $\mathbb D_{1/2}$, and the first one has radius exactly
$\frac12$ (\cref{dyn:esm:thm:main}). For $F_{\mathrm{dr}}$ the coefficient
of $t^{m}$ has radius exactly $2^{-m}$ (\cref{dyn:cyc:thm}). The only change
in the input is the infinitesimal linear term $tz$. The difference is a
common-domain obstruction. It is not a change in the availability of a
formal conjugacy, or of its evaluation on the monad. The infinite continued
fraction specifies $\lambda$; no floating-point approximation to it is
involved.
\end{example}

\emph{What this subsection does not reach.} Source 08 does not address the
several-variable multiplicative-detuning collapse
\cref{dyn:thm:multicollapse} over a fixed cyclic group. That statement is
still asserted only over $\Gamma_{*}$ (\cref{dyn:q:cyclic}). Neither
\cref{dyn:cyc:thm} nor \cref{dyn:esm:thm:main} gives ordinary analytic
convergence after $t$ is replaced by a nonzero complex number
(\ref{dyn:esm:nc:specialize}).

\subsection{Resolution: which category answers which question}
\label{dyn:subsec:resolution}

\begin{question}[Common-domain germs for a finite positive rate;
\emph{settled} for every exact diagonal unitary multiplier, the optimal
radius \emph{open} in angular rank at least two]\label{dyn:q:commongerm}
...
\emph{Status.} For a scalar multiplier $\Lambda=\lambda I_d$ the answer is:
all of them, with the sharp universal radius $Re^{-\tau}$
(\cref{dyn:esm:thm:main}; source 08 for every $d$, source 09 for $d=1$).
For a nonscalar diagonal unitary $\Lambda$ the answer is again: all of them
(\cref{dyn:nsc:thm:main}; source 10). Every coefficient of the normalized
conjugacy and its inverse is holomorphic on the polydisk of radius
$Re^{-\rang\tau}$, where $\rang$ is the rational angular rank
\eqref{dyn:nsc:eq:rank}, and the largest universal radius $R_{\mathrm{univ}}$
satisfies $Re^{-\rang\tau}\le R_{\mathrm{univ}}\le Re^{-\tau}$, with
equality $R_{\mathrm{univ}}=Re^{-\tau}$ when $\rang=1$
(\cref{dyn:nsc:thm:criterion}). Still \textbf{open}: the value of
$R_{\mathrm{univ}}$ when $\rang\ge2$ and $0<\tau<\infty$
(\cref{dyn:nsc:subsec:limits}). At the pins of sources 01--07 the question
was open throughout $\sdex$; at the pins of sources 08 and 09 it was open
for every nonscalar $\Lambda$.
\end{question}

\begin{warning}[The sources answer different questions; do not average
...
\eqref{dyn:eq:collapse} is exact. Source 08's \cref{dyn:cyc:thm} shows the
same failure over $\Z$ with one Hahn monomial.
\item Sources 01--07 left \cref{dyn:q:commongerm} \textbf{open} in the
...
needed. Sources 08 and 09 supply a positive theorem for a scalar multiplier
$\Lambda=\lambda I_d$: there every input has a common-domain linearizer, of
the sharp radius $Re^{-\tau}$ (\cref{dyn:esm:thm:main}). The theorem needs
no cancellation. Positive nonlinear degree makes the divisor indices
strictly increase (\cref{dyn:esm:lem:degreechain}), and a single rational
cluster pays for all the exceptional divisors at once
(\cref{dyn:esm:thm:treebound}). Source 10 supplies the positive theorem for
every nonscalar diagonal unitary $\Lambda$, with radius at least
$Re^{-\rang\tau}$ (\cref{dyn:nsc:thm:main}); again no cancellation is
needed, and the number of distinct exponential rates, at most $\rang$, is
what is paid for (\cref{dyn:nsc:thm:products}). Only the optimal radius for
$\rang\ge2$ and $0<\tau<\infty$ remains \textbf{open}.
\item The two categories must not be averaged into a single ``intermediate
case'' claim (\cref{dyn:cyc:tab:exactdrift}). Neither
\eqref{dyn:eq:collapse} nor \eqref{dyn:cyc:eq:radius} may ever be quoted
without its drift hypothesis.
\end{itemize}
...
first-order equation give failure whenever the rate is positive. The issue
that sources 01--07 left unresolved concerns the existence of \emph{some}
smaller common neighborhood for every input. It is now resolved in the
drifted category over any fixed value group (no such neighborhood exists in
general), for a scalar exact multiplier (it always exists, with radius
$Re^{-\tau}$), and for a nonscalar exact diagonal unitary multiplier (it
always exists, with radius at least $Re^{-\rang\tau}$, and exactly
$Re^{-\tau}$ when $\rang=1$, or $R$ when $\tau=0$ at every angular rank).
The optimal radius when $\rang\ge2$ and $0<\tau<\infty$ is still open.
\end{warning}
...
\subsection{Rotation-equivariant maps: the threshold read off the map itself}
\label{dyn:rd:subsec}

This subsection and the six after it are contributed by source 11,
\emph{Sharp Linearization at Surreal Scales: Hahn summability, resonant
periodic cycles, transcendental conjugacies, and omnific rational maps}
(23 pages, September 2026), delivered in batch 34 as manuscript 08, the
archive \texttt{Surreal\_Resonant\_Dynamics}, whose shipped files carry the
local prefix \texttt{11-resonant-dynamics-}. It was written against
repository commit
\texttt{9ee2dac0\allowbreak d234c904\allowbreak 15f6b729\allowbreak
0cbe5b4d\allowbreak cf4a4d3e}. At that commit this report's source file was
byte-identical to its state immediately before source 11 was added. Source
11 did not read it: its repository inspection covered the catalogue
READMEs, the notation guide, the analysis guide and the omnific-quotient
guide only (\ref{dyn:rd:nc:repo}). It therefore re-derives, independently
and for a special class of maps, the exact finite-return threshold
\cref{dyn:thm:exact} and the shell count of \cref{dyn:thm:shell}. Its
statement that the combined formulation was not identified in the sources
it checked is accurate for those sources and stale against this report
(\ref{dyn:rd:nc:priority}).

Source 11 studies
\begin{equation}\label{dyn:rd:eq:family}
 f(z)=\lambda z+\sum_{j=1}^{J}a_{1+jq}z^{1+jq},\qquad
 \lambda=\zeta+e,\qquad \delta=v(e)>0,
\end{equation}
over $K_\Gamma$ with $\Gamma$ nonzero, set-sized and divisible, where $q\ge1$
is an ordinary integer, $\zeta\in\C$ is a primitive $q$-th root of unity,
$0\ne e\in\m_\Gamma$, and at least one $a_{1+jq}$ is nonzero. Such an $f$
commutes with the rotation: $f(\zeta z)=\zeta f(z)$, and this symmetry is
essential to every theorem below except \cref{dyn:rd:thm:omnific}, which
inherits it. The residue multiplier is $\alpha=\bar\lambda=\zeta$, of exact
order $q$, and $\lambda$ is not a root of unity
(\cref{dyn:rd:lem:divisors}), so $f$ satisfies $\resq$ with the rotation
order as its $q$. What source 11 adds to this section is:
\begin{enumerate}[label=(\alph*)]
\item the exact threshold computed from the coefficients of $f$ itself, with
no return iterate (\cref{dyn:rd:thm:linearization,dyn:rd:prop:return});
\item the exclusion of \emph{every} other period from the closed normalized
ball (\cref{dyn:rd:prop:periodfinite}), whereas \cref{dyn:thm:shell}(i)
counts the solutions of $f^{\circ q}(z)=z$ on the shell;
\item an exact algebraicity criterion for the reduced linearizer and a
one-way transcendence certificate for the full one, for Hahn-valued $e$ and
normalized coefficients (\cref{dyn:rd:thm:reduction,dyn:rd:thm:algebraicity});
\item the periodic points of integral equivariant maps
(\cref{dyn:rd:thm:allperiods}), prescribed finite families of periodic
shells (\cref{dyn:rd:thm:allshells}), and reciprocal rational maps whose
infinitely large periodic points are prescribed Gaussian omnific monomials
(\cref{dyn:rd:thm:omnific}).
\end{enumerate}
Printed once, and credited to source 11 as an independent re-derivation:
the positive-support lemma (\cref{dyn:lem:neumann}); evaluation, inversion
and isometry for one uniform monoid (\cref{dyn:lem:evaluation}); the root
count (\cref{dyn:lem:root-count}); the non-cofinality example
(\cref{dyn:warn:contraction}); the maximal ball and the shell count, which
are the equivariant case of \cref{dyn:thm:exact,dyn:thm:shell}
(\cref{dyn:rd:prop:return}); and the leading differential equation, which
is the equivariant case of \eqref{dyn:eq:ode0}
(\cref{dyn:rd:rem:coherent}). Where source 11's proof takes a different
route, the route is kept and marked. Text added in the merge, and not
present in source 11, is marked \mergetag.

\emph{Tags.} Resonance $\resq$, with $q$ the rotation order; the one
exception is \cref{dyn:rd:thm:allperiods}, which also admits $\lambda=\zeta$
exactly, the finite-order case $\Lambda^{q}=I$ of \cref{dyn:thm:finite-order},
which is neither $\resq$ nor $\mres$ (\cref{dyn:warn:resonance}). Divisor
notion $\sdz$: no arithmetic condition of any kind. Coefficient categories:
the formal row $\mathcal F_1$ with the uniform-monoid algebra
$\BB_{\mathsf M}$ of \cref{dyn:def:BM} for the linearizer, an ordinary
polynomial $R\in\C[U]$ for the shell, and $\C[[X]]$ for the reduced
linearizer. Domains: the valuation ball $B_r$, the closed ball
$\bar B_r=\{v\ge r\}$ and the shell $S_r$ of \cref{dyn:subsec:radius}.

\begin{convention}[Setting and local notation of source 11]
\label{dyn:rd:conv}
Source 11's symbols collide with this section's at almost every letter. The
following renamings are used in \cref{dyn:rd:subsec} and the subsections
after it, in \cref{dyn:rd:q:symmetry} to \cref{dyn:rd:q:effective}, and in
\cref{dyn:rd:subsec:nonclaims}. No normalization changes; each item gives
source 11's symbol first.
\begin{enumerate}[label=(\roman*)]
\item The rotation order $m$ is written $q$: it is the exact order of the
residue multiplier, the $q$ of $\resq$. It is \emph{not} the series index
$m$ of \cref{dyn:sec:drift}, nor source 10's tree complexity $m$.
\item The displacement $d$, with $\lambda=\zeta+d$, is written $e$, as in
the proof of \cref{dyn:lem:nonresonant}. In this report $d$ is a polynomial
degree \eqref{dyn:eq:f-main} or a dimension. Source 11's
$\delta=v(d)$ is $\delta=v(e)$ here. It equals $v(c)$ for
$c=\lambda^{q}-1$ of \eqref{dyn:eq:g-main} (\cref{dyn:rd:lem:divisors}), so
$\delta$ has one meaning in this section.
\item The coefficient $a_j$ of $z^{1+jm}$ is written $a_{1+jq}$, so that
$a_n$ is the coefficient of $z^{n}$, as in \eqref{dyn:eq:f-main}; the
number $r$ of nonlinear terms is written $J$.
\item The threshold exponent $\rho\in\Gamma$ is written $r_{\mathrm{eq}}$
\eqref{dyn:rd:eq:req}, and the monomial $s=t^{\rho}$ is written
$\rho=t^{r_{\mathrm{eq}}}$, as in \eqref{dyn:eq:H-scaled}. \textbf{Tempting
false reading:} source 11's $\rho$ is an exponent; this section's $\rho$ is
a monomial of $K_\Gamma$. \Cref{dyn:rd:prop:return} proves
$r_{\mathrm{eq}}=r(f)$ of \eqref{dyn:eq:exact-r}; before that proposition
the two are not identified.
\item The normalized coefficients $u_j$ are written $\hat a_j$; the
unreduced polynomial $P(U)=1+\sum_ju_jU^{j}$ is written $\widehat R(U)$; the
reduced polynomial $R(U)$ keeps its name, with coefficients
$\varkappa_j=\res(\hat a_j)$. \textbf{Tempting false reading:} this
report's $P$ is the \emph{reduced} shell polynomial in $X$ of
\eqref{dyn:eq:P}; source 11's $P$ is \emph{unreduced} and in the variable
$U=X^{q}$. \Cref{dyn:rd:prop:return} proves $P(X)=R(X^{q})$. Source 11's
$R$ is not the function $R(w)$ of \cref{dyn:q:moving}, not a radius, and
not a coefficient ring.
\item The normalized variable $y$ is written $X$. The normalized linearizer
$h(y)=s^{-1}H(sy)$ is $H_r$ of \eqref{dyn:eq:H-scaled}; the coefficients
$B_n$ of $H$ are written $h_n$, and the coefficients $b_n$ of $h$ are
$h_n\rho^{n-1}$. This section's $b_n$ are the coefficients of the return
$g=f^{\circ q}$ \eqref{dyn:eq:g-main}. The reduction $h_0\in\C[[y]]$ is
written $\bar H_r\in\C[[X]]$: a coefficientwise reduction, not an
evaluation. It is the Taylor expansion at zero of the exponent-zero
coefficient $H_0$ of \cref{dyn:thm:coherent} (\cref{dyn:rd:rem:coherent}).
The series $B(U)$ with $h_0=yB(y^{m})$ is written $\mathfrak h(U)$, with
coefficients $\mathfrak h_k$ (source 11's $\beta_k$); $B_r$ remains a ball
and $\BB$ the algebraicity base \eqref{dyn:eq:base}.
\item A root $r$ of $R$ is written $\beta$, and source 11's residue
$\alpha_r=1/(mrR'(r))$ is written
$\mathfrak r_\beta=1/(q\beta R'(\beta))$. In this report $\alpha$ is the
residue of $\lambda$, which here is $\zeta$, and $r$ is a threshold.
\item The polynomial $q_k(D)$ is written $\varphi_k(E)$; the iterate
quotient $T_n$ is written $\mathcal Q_n$, its divided difference $D_\psi$ is
written $\mathcal D_\psi$, its coefficient ring $A$ is written $\mathsf A$,
and $\mu=\psi'(0)$ is written $\mu_\psi$. The polynomial $T(X)$ of
\cref{dyn:lem:displacement} and $\mu=1+c$ in the proof of
\cref{dyn:thm:exact} are different objects.
\item The support ring $A_S$ is written $\OO_{\mathsf M}$ with
$\mathsf M=S^{*}$, so that $\BB_{\mathsf M}=\OO_{\mathsf M}[[X]]$
(\cref{dyn:def:BM}); source 11's monoid $M(S)$ is $S^{*}$. In this section
$M$ is the degree \eqref{dyn:eq:P}.
\item In \cref{dyn:rd:thm:allshells,dyn:rd:thm:omnific}: the scales
$\rho_\ell$ are written $r_\ell$, the constants $c_\ell$ are written
$\varsigma_\ell$, the elements $b_\ell$ are written $\mathfrak b_\ell$, the
polynomial $P(U)=\prod_\ell(U-b_\ell)$ is written $\mathcal S(U)$ with
coefficients $s_j$, the number $r$ of shells is written $N$, the maximum
$\sigma=\max_\ell\rho_\ell$ is written $r_{\max}$, a solution $\alpha$ of
$\alpha^{m}=c_\ell$ is written $\vartheta$, the reciprocal map $G$ is written
$f^{\vee}$, the clearing exponent $L$ is written $L_{\mathrm{cl}}$, and the
class $\Pi$ of purely infinite forms is written $\mathbb J$. In this report
$c=\lambda^{q}-1$, $G$ is the rescaled return \eqref{dyn:eq:G}, $\sigma$ is
the drifted rate, and $\Pi$ is the action projection of
\cref{dyn:sec:hamiltonian}.
\item In the collision example, source 11's exponents $\epsilon,\gamma$ are
written $\eta,\theta$, as in \cref{dyn:warn:contraction}; the temporary
$t=t^{\epsilon}$ of its explicit quintic is written $t^{\eta}$. Its balls
$B_\rho$, $\overline B_\rho$ and shell $S_\rho$ are $B_r$, $\bar B_r$ and
$S_r$ at $r=r_{\mathrm{eq}}$.
\item Source 11's word ``resonant'' means $\resq$ throughout, never $\mres$
or $\fres$, and its ``boundary'' of a ball is the valuation shell, not a
topological boundary.
\end{enumerate}
\end{convention}

\begin{warning}[Rotation order one meets the parabolic tag]
\label{dyn:rd:warn:qone}
For $q=1$, so $\zeta=1$, a map \eqref{dyn:rd:eq:family} has multiplier
$1+e$ and residue multiplier $1$ of exact order one: it satisfies $\resq$
with $q=1$, because $1+e$ is not a root of unity. Its normalized map
$F(X)=X+eX\widehat R(X)$ of \eqref{dyn:rd:eq:F} reduces to the identity,
which is the shape of $\pares$; when $e=t^{\delta}$ and every $\hat a_j$ is
an ordinary constant it is exactly an Euler map \eqref{dyn:eq:Euler}.
\Cref{dyn:warn:resonance} says that \cref{dyn:thm:exact} needs $\resq$ and
must not be applied under $\pares$; that is a statement about hypotheses. It
forbids quoting the exact ball for a map known only to be parabolic. It does
not forbid it here, where $\resq$ holds. Each theorem below is quoted under
the tag its source proves it under. The two readings meet in
\cref{dyn:thm:sharp} (\cref{dyn:rd:rem:scope}) and in
\cref{dyn:rd:rem:superseded}.
\end{warning}

\subsection{Support rings, and a failure of local finiteness}
\label{dyn:rd:subsec:support}

Source 11 states the positive-support lemma in the finite-word form of
\cref{dyn:lem:neumann}, including finiteness across variable word length,
and cites Marker's lecture notes for it \cite[Lemma~2.36]{Marker}. Its
remark that $n\eta$ need not be cofinal, with $\eta=(0,1)$ and
$\theta=(1,0)$ in lexicographic $\Q^{2}$, where the geometric family
$(t^{n\eta})_{n\ge0}$ is strongly summable but its partial sums are not
Cauchy in the valuation topology, is the example of
\cref{dyn:warn:contraction}. Source 11 draws from it the conclusion that a
rank-one proof cannot be transferred by renaming the absolute value, and it
never writes a real number $|t|^{\gamma}$ for a general exponent. Its
workspace convention $t^{\gamma}=\omega^{-\gamma}$ is
\eqref{dyn:eq:embedding}.

\begin{definition}[A common local coefficient ring]
\label{dyn:rd:def:ring}
For a well-ordered $S\subset\Gamma_{>0}$ and $\mathsf M=S^{*}$, put
$\OO_{\mathsf M}=\{x\in K_\Gamma:\supp x\subseteq\mathsf M\}$. It contains
$0$ and every ordinary constant, lies in $\OO_\Gamma$, and
$\BB_{\mathsf M}=\OO_{\mathsf M}[[X]]$.
\end{definition}

\begin{lemma}[The ring $\OO_{\mathsf M}$; source 11]\label{dyn:rd:lem:ring}
$\OO_{\mathsf M}$ is a subring of $\OO_\Gamma$. An element of
$\OO_{\mathsf M}$ with nonzero residue is invertible in $\OO_{\mathsf M}$.
Every finite collection of elements of $\OO_\Gamma$ lies in some
$\OO_{\mathsf M}$.
\end{lemma}
\begin{proof}
The support of a sum lies in the union of the supports, and that of a
product in their sum, so both operations preserve $\OO_{\mathsf M}$. Write a
residue unit as $b=c_0(1-u)$ with $c_0\in\C^{\times}$ and
$\supp u\subseteq\mathsf M\setminus\{0\}$, a positive well-ordered set.
\Cref{dyn:lem:neumann} makes $\sum_{n\ge0}u^{n}$ strongly summable with
support in $\mathsf M$, and the geometric identity gives the inverse.
Finally, the finite union of the positive supports of finitely many elements
of $\OO_\Gamma$ is positive and well ordered; take it as $S$.
\end{proof}

\begin{lemma}[Uniform support evaluation; source 11]
\label{dyn:rd:lem:evaluation}
Let $h_n\in\OO_{\mathsf M}$ for all $n\ge0$ and $u\in\m_\Gamma$. Then
$\sum_{n\ge0}h_nu^{n}$ is strongly summable. The same holds for a formal
power series in finitely many variables with coefficients in
$\OO_{\mathsf M}$, evaluated at a tuple of infinitesimals. Products, and
formal substitution into series with zero constant coefficient, commute with
these evaluations whenever the substituted series also have coefficients in
$\OO_{\mathsf M}$. The formal derivative series of every fixed finite order
are strongly evaluable on the same infinitesimal domain.
\end{lemma}
\begin{proof}
The one-variable clause and the substitution clause are
\cref{dyn:lem:evaluation}; source 11's proof is the same finite-word count.
For finitely many variables, take the finite union of their positive
supports with $S$. A monomial of total degree $n$ again contributes at least
$n$ positive letters, and there are only finitely many multi-indices of
bounded total degree, so the same proof applies. For a product or a
substitution, expand into the underlying monomials of the infinitesimal
arguments: at a fixed Hahn exponent their total degree is bounded by the
word argument, and at a fixed total degree there are only finitely many
formal composition partitions, because the substituted series have zero
constant coefficient. Formal differentiation multiplies coefficients by
ordinary integers and lowers the degree by a fixed amount, so the same
finiteness proof applies to each fixed derivative.
\end{proof}

\begin{remark}[Inverse and isometry; printed once]\label{dyn:rd:rem:inverse}
Source 11's ``strong inverse and isometry'' proposition is the second and
third clauses of \cref{dyn:lem:evaluation}, proved the same way: the
inverse coefficients are integer polynomials in finitely many $h_n$, and the
divided difference is $1$ plus a strongly summable family of positive
valuation. It adds one clause: for $H=X+\sum_{n\ge2}h_nX^{n}$ with all
$h_n\in\OO_{\mathsf M}$ and $0\ne u\in\m_\Gamma$, either $H(u)=u$ or
$v(H(u)-u)>v(u)$, because $H(u)-u=u^{2}\sum_{n\ge2}h_nu^{n-2}$.
\end{remark}

\begin{example}[Integral coefficients are not enough: local finiteness fails]
\label{dyn:rd:ex:collision}
Let $\eta,\theta\in\Gamma$ be positive with $n\eta<\theta$ for every
integer $n\ge1$; for instance $\eta=(0,1)$ and $\theta=(1,0)$ in
lexicographic $\Q^{2}$. For $n\ge2$ put
\[
 g_n=t^{\theta-n\eta}\in\m_\Gamma,\qquad u=t^{\eta}\in\m_\Gamma .
\]
Every coefficient is infinitesimal, but $g_nu^{n}=t^{\theta}$ for every
$n\ge2$. The family $(g_nu^{n})$ is not strongly summable: the union of its
supports is the well-ordered set $\{\theta\}$, but that one exponent occurs
infinitely often. So the assertion ``$\sum g_nX^{n}\in\OO_\Gamma[[X]]$,
hence it evaluates at every infinitesimal'' is false in arbitrary rank. The
$g_n$ lie in no single $\OO_{\mathsf M}$, since their supports form a
strictly decreasing sequence.

\mergetag\ This is a different failure from \cref{dyn:ex:bad-support}.
There the Taylor exponents $1/n+n\eps_0$ strictly decrease, so
well-ordering fails; here every term sits at one exponent, so local
finiteness fails. The two examples violate the two clauses of
\cref{dyn:def:summable} separately. \Cref{dyn:thm:sharp} exhibits both
failures for a single Taylor family, at and below its threshold.
\end{example}

\subsection{The explicit threshold, and a linearization without the return}
\label{dyn:rd:subsec:threshold}

\begin{lemma}[Equal-characteristic root-of-unity dichotomy]
\label{dyn:rd:lem:divisors}
Let $\lambda=\zeta+e$ with $v(e)=\delta>0$ and $\zeta$ a primitive ordinary
$q$-th root of unity. For every ordinary integer $n\ge1$,
\[
 v(\lambda^{n}-1)=
 \begin{cases}
 0, & q\nmid n,\\
 ...
 \end{cases}
\]
In particular $\lambda$ is not a root of unity. For $k\ge1$,
\begin{equation}\label{dyn:rd:eq:divisorunit}
 \lambda(1-\lambda^{kq})=e\,\varphi_k(e),\qquad
 \varphi_k(E)\in\C[E],\qquad \varphi_k(0)=-kq\ne0 .
\end{equation}
\end{lemma}
\begin{proof}
If $q\nmid n$, the residue of $\lambda^{n}-1$ is $\zeta^{n}-1\ne0$. If
$q\mid n$, write $\lambda=\zeta(1+e/\zeta)$ and expand the ordinary finite
binomial: its first nonzero term is $ne/\zeta$, of valuation $\delta$, and
every other term has valuation at least $2\delta$. This proves the dichotomy
and non-torsion. The polynomial $(\zeta+E)(1-(\zeta+E)^{kq})$ vanishes at
$E=0$ with derivative $-kq$ there, which gives
\eqref{dyn:rd:eq:divisorunit}.
\end{proof}

This agrees with the rank-one small-divisor calculation of
\cite[Lemma~3.3]{Lindahl}, and no real-valued norm is needed. The same
first-term computation is used in the proofs of \cref{dyn:thm:exact} and
\cref{dyn:lem:displacement}. In particular $c=\lambda^{q}-1$ has valuation
$\delta$, so source 11's $\delta$ is the $\delta$ of \eqref{dyn:eq:g-main}.

The normalized formal linearizer $H(z)=z+\sum_{n\ge2}h_nz^{n}$ of any
polynomial with multiplier $\lambda\ne0$ not a root of unity is unique, by
the recurrence \eqref{dyn:eq:recurrence}; source 11 writes the same
recurrence with $T=f$. If $f(\zeta z)=\zeta f(z)$, then
$\zeta^{-1}H(\zeta z)$ satisfies the same equation and normalization, so
\begin{equation}\label{dyn:rd:eq:hsymmetry}
 H(\zeta z)=\zeta H(z),\qquad h_n=0\ \text{unless}\ n\equiv1\pmod q .
\end{equation}
For $q=1$ this is vacuous.

\emph{Cancellation after scaling.} For \eqref{dyn:rd:eq:family} put
\begin{equation}\label{dyn:rd:eq:req}
 r_{\mathrm{eq}}=\max_{a_n\ne0}\frac{\delta-v(a_n)}{n-1}
 =\max_{a_{1+jq}\ne0}\frac{\delta-v(a_{1+jq})}{jq},
 \qquad \rho=t^{r_{\mathrm{eq}}},
\end{equation}
a finite maximum over a nonempty set, lying in $\Gamma$ by divisibility.
Unlike \eqref{dyn:eq:exact-r}, it is read off the coefficients of $f$, not
of its $q$-th return. Put
\begin{equation}\label{dyn:rd:eq:R}
 \hat a_j=\frac{a_{1+jq}\rho^{jq}}{e}\in\OO_\Gamma,\qquad
 \widehat R(U)=1+\sum_{j=1}^{J}\hat a_jU^{j},\qquad
 R(U)=1+\sum_{j=1}^{J}\varkappa_jU^{j},\quad \varkappa_j=\res(\hat a_j).
\end{equation}
Each $\hat a_j$ is integral and at least one is a unit, because
$v(\hat a_j)=v(a_{1+jq})+jq\,r_{\mathrm{eq}}-\delta\ge0$ with equality at a
maximizing $j$; so $R\in\C[U]$ is nonconstant with $R(0)=1$. The rescaled
map is
\begin{equation}\label{dyn:rd:eq:F}
 F(X)=\rho^{-1}f(\rho X)=\zeta X+eX\widehat R(X^{q}),
\end{equation}
with multiplier $\lambda$; it is the $F$ of the proof of
\cref{dyn:thm:exact} at $r=r_{\mathrm{eq}}$.

\begin{theorem}[Strong linearization on the exact candidate ball; source 11]
\label{dyn:rd:thm:linearization}
For $f$ as in \eqref{dyn:rd:eq:family}, the normalized formal linearizer
$H$ and its formal inverse evaluate strongly on $B_{r_{\mathrm{eq}}}$, their
values are mutually inverse bijective isometries of $B_{r_{\mathrm{eq}}}$,
and $H(f(z))=\lambda H(z)$ for $z\in B_{r_{\mathrm{eq}}}$. More precisely,
every coefficient of $H_r(X)=\rho^{-1}H(\rho X)$ and of its inverse lies in
one ring $\OO_{\mathsf M}$, where $S$ may be chosen as the union of the
positive supports of the finite list $e,\hat a_1,\dots,\hat a_J$.
Consequently $f$ is a bijective isometry of $B_{r_{\mathrm{eq}}}$ with no
nonzero periodic point there.
\end{theorem}
\begin{proof}[Proof \textup{(source 11: cancellation in the recurrence of
$f$ itself, with no return iterate)}]
Choose $S$ as stated, omitting zero coefficients; by
\cref{dyn:rd:lem:ring}, $e$ and every $\hat a_j$ lie in $\OO_{\mathsf M}$.
The normalized formal series $H_r(X)=X+\sum_{n\ge2}h_n\rho^{n-1}X^{n}$ is
the unique normalized linearizer of $F$, by
\cref{dyn:rd:lem:divisors} and \eqref{dyn:eq:recurrence}, and by
\eqref{dyn:rd:eq:hsymmetry} only degrees $n=1+kq$ occur. Assume every
earlier coefficient lies in $\OO_{\mathsf M}$. The nonlinear coefficients of
$F$ are $e\hat a_j$. In the numerator of \eqref{dyn:eq:recurrence} every
term is divisible by $e$ in $\OO_{\mathsf M}$: this is evident for a
coefficient of $F$ itself, and a term of degree $n$ in $F^{k}$ with $k<n$
uses at least one nonlinear coefficient. After division by $e$ the numerator
is still in $\OO_{\mathsf M}$. The denominator is $e\varphi_k(e)$ by
\eqref{dyn:rd:eq:divisorunit}, and $\varphi_k(e)$ is a unit of
$\OO_{\mathsf M}$ because its residue is $-kq\ne0$. Cancelling $e$ gives
$h_n\rho^{n-1}\in\OO_{\mathsf M}$, which proves the common-support
assertion by induction.

\Cref{dyn:lem:evaluation} and \cref{dyn:rd:rem:inverse} make $H_r$ and its
inverse mutually inverse isometries of $\m_\Gamma$. The polynomial $F$
preserves $\m_\Gamma$, and \cref{dyn:rd:lem:evaluation} justifies evaluating
the formal conjugacy there. Multiplication by $\rho$ transfers everything to
$B_{r_{\mathrm{eq}}}=\rho\,\m_\Gamma$. Multiplication by $\lambda$ is an
isometry preserving $\m_\Gamma$, so $f$ is a bijective isometry. If
$f^{\circ n}(z)=z$ then $(\lambda^{n}-1)H(z)=0$; non-torsion gives
$H(z)=0$ and injectivity gives $z=0$.
\end{proof}

\begin{remark}[Why the recurrence is stronger than a norm estimate; two
routes]\label{dyn:rd:rem:cancel}
At every surviving degree the denominator has valuation $\delta$, but the
numerator has an actual factor $e$ inside one fixed support ring. That
algebraic cancellation, followed by inversion of a residue unit, keeps every
normalized coefficient in one $\OO_{\mathsf M}$; a sequence of unrelated
valuation bounds would not. The proof yields the estimate
\begin{equation}\label{dyn:rd:eq:coeffbound}
 v(h_n)+(n-1)r_{\mathrm{eq}}\ge0\qquad(n\ge2),
\end{equation}
which alone is not the analytic theorem. \mergetag\ The cancellation is the
one of \cref{dyn:lem:displacement}, which this report performs on the
return $G$ and then transfers to $F$ by commutation. Source 11 performs it
on the recurrence of $F$ itself. That is possible because equivariance
removes every degree $n\not\equiv1\pmod q$, whose divisors
$\lambda-\lambda^{n}$ are units, and leaves only the degrees whose divisors
carry the factor $e$. The two proofs are kept as two routes to the same
support certificate; source 11's composes no iterate.
\end{remark}

\subsection{All periods in the closed normalized ball, and the radius shell}
\label{dyn:rd:subsec:shell}

\begin{lemma}[Universal iterate quotient; source 11]\label{dyn:rd:lem:iterate}
Let $\mathsf A$ be a commutative ring and $\psi(X)\in\mathsf A[X]$. For
every $n\ge1$ there is $\mathcal Q_n(X)\in\mathsf A[X]$ with
\begin{equation}\label{dyn:rd:eq:iteratediv}
 \psi^{\circ n}(X)-X=\bigl(\psi(X)-X\bigr)\mathcal Q_n(X).
\end{equation}
If $\psi(0)=0$ and $\mu_\psi=\psi'(0)$, then
$\mathcal Q_n(0)=1+\mu_\psi+\cdots+\mu_\psi^{n-1}$. If $\mathsf A=\OO_\Gamma$
and $\psi(X)\equiv X\pmod{\m_\Gamma[X]}$, then
$\mathcal Q_n(X)\equiv n\pmod{\m_\Gamma[X]}$.
\end{lemma}
\begin{proof}
Let
$\mathcal D_\psi(X,Y)=(\psi(X)-\psi(Y))/(X-Y)\in\mathsf A[X,Y]$. Put
$\mathcal Q_1=1$ and
$\mathcal Q_{n+1}(X)=\mathcal Q_n(X)\,\mathcal D_\psi(\psi^{\circ n}(X),X)+1$;
substitution proves \eqref{dyn:rd:eq:iteratediv} by induction. If
$\psi(0)=0$ then $\mathcal D_\psi(0,0)=\mu_\psi$, giving the constant term.
If $\psi$ reduces to the identity, its divided difference reduces to $1$,
and reducing the recursion gives $\mathcal Q_n\equiv n$.
\end{proof}

The construction divides by no leading coefficient, so no nonunit is ever
inverted.

\begin{proposition}[No hidden periods in the closed normalized ball;
source 11]\label{dyn:rd:prop:periodfinite}
For $F$ of \eqref{dyn:rd:eq:F}, every nonzero periodic point in $\OO_\Gamma$
satisfies $F(X)=\zeta X$ and has exact period $q$. These points are exactly
the roots in $\OO_\Gamma^{\times}$ of $\widehat R(X^{q})=0$. Equivalently,
every nonzero periodic point of $f$ in the closed ball
$\bar B_{r_{\mathrm{eq}}}$ lies on $S_{r_{\mathrm{eq}}}$, satisfies
$f(z)=\zeta z$ and has exact period $q$.
\end{proposition}
\begin{proof}
Let $\psi=\zeta^{-1}F$. It commutes with multiplication by $\zeta$, and its
reduction is the identity. If $q\nmid n$, the polynomial
$(F^{\circ n}(X)-X)/X$ has reduction the constant $\zeta^{n}-1\ne0$, so it
is a unit at every $X\in\OO_\Gamma$, and no nonzero point of $\OO_\Gamma$
has period dividing $n$. If $q\mid n$, equivariance gives
$F^{\circ n}=\psi^{\circ n}$, and by \cref{dyn:rd:lem:iterate}
$\mathcal Q_n(X)$ has residue $n\ne0$ at every $X\in\OO_\Gamma$. So
$F^{\circ n}(X)=X$ is equivalent to $\psi(X)=X$, which for $X\ne0$ and
$e\ne0$ is $\widehat R(X^{q})=0$. Conversely such a point satisfies
$F(X)=\zeta X$; equivariance gives its whole rotation orbit, of exact
length $q$ because $\zeta$ has exact order $q$ and $X\ne0$. A root with
$v(X)>0$ is impossible, since $\widehat R(X^{q})$ has residue $1$ there.
Scaling by $\rho$ gives the statement for $f$.
\end{proof}

\mergetag\ \Cref{dyn:thm:shell}(i) counts the solutions of
$f^{\circ q}(z)=z$ on $S_r$ and excludes periodic points from the open ball
$B_r$. It does not exclude, on $S_r$, periodic points whose period is not a
divisor of $q$. For equivariant maps \cref{dyn:rd:prop:periodfinite}
excludes all of them. That exclusion is new to this report.

\begin{proposition}[The two thresholds and the two shell polynomials
coincide]\label{dyn:rd:prop:return}
\mergetag\ For $f$ as in \eqref{dyn:rd:eq:family}, the threshold
\eqref{dyn:eq:exact-r} of \cref{dyn:thm:exact}, which is computed from the
$q$-th return, equals $r_{\mathrm{eq}}$, and the shell polynomial
\eqref{dyn:eq:P} is
\begin{equation}\label{dyn:rd:eq:PR}
 P(X)=R(X^{q}),\qquad M=\deg P=q\deg R .
\end{equation}
\end{proposition}
\begin{proof}
Let $\psi=\zeta^{-1}F$, so $\psi(X)-X=\zeta^{-1}eX\widehat R(X^{q})$ and
$\psi$ reduces to the identity. Since $\zeta^{q}=1$ and $\psi$ commutes with
multiplication by $\zeta$, $F^{\circ q}=\psi^{\circ q}$; and rescaling
commutes with iteration, so the rescaled return of \eqref{dyn:eq:G} at
scale $\rho=t^{r_{\mathrm{eq}}}$ is $G=F^{\circ q}$. By
\cref{dyn:rd:lem:iterate},
$G(X)-X=\zeta^{-1}eX\widehat R(X^{q})\mathcal Q_q(X)$ with
$\mathcal Q_q\equiv q\pmod{\m_\Gamma[X]}$. By \cref{dyn:rd:lem:divisors}
write $c=\lambda^{q}-1=q\zeta^{-1}e\,w$, where $w\in\OO_\Gamma$ has residue
$1$. Then
\[
 Q(X)=\frac{G(X)-X}{cX}=\frac{\widehat R(X^{q})\,\mathcal Q_q(X)}{q\,w}
\]
is an integral polynomial with reduction $R(X^{q})$, which is nonconstant.
Its constant term is $1$, as it must be, since
$\mathcal Q_q(0)=\sum_{i<q}(\lambda/\zeta)^{i}=c\zeta/e=qw$. Integrality of
$Q$ with a nonconstant reduction says that
$v(b_n)+(n-1)r_{\mathrm{eq}}-\delta\ge0$ for every nonzero return
coefficient $b_n$, with equality for some $n$. The function
$r'\mapsto\min_n\bigl(v(b_n)+(n-1)r'-\delta\bigr)$ is strictly increasing,
because every $n-1\ge1$, and it vanishes at $r'=r(f)$ by
\eqref{dyn:eq:exact-r}. So $r(f)=r_{\mathrm{eq}}$, the $Q$ above is
\eqref{dyn:eq:Q-shell}, and $P=\bar Q=R(X^{q})$.
\end{proof}

So for equivariant polynomials the threshold of \cref{dyn:thm:exact} is the
explicit expression \eqref{dyn:rd:eq:req} in the coefficients of $f$, and
\cref{dyn:thm:shell}(ii), $P\in\C[X^{q}]$, is visible directly. The
expression is special to equivariant maps. For the non-equivariant tuned
cubic \eqref{dyn:eq:tuned-cubic} it would give
$\max\{\delta/1,\delta/2\}=\delta$, whereas $r(f_e)=\delta/4$
\eqref{dyn:eq:tuned-r}: the cancellation in the return
(\cref{dyn:sec:phase}) is invisible in the coefficients of $f$.

Source 11's finite-root degree lemma is \cref{dyn:lem:root-count}, stated for
a polynomial $\widehat R\in\OO_\Gamma[U]$ with unit constant coefficient,
not constant coefficient $1$. Its proof is the same finite factorization,
organized by Gauss valuation. Factor over the algebraically closed
$K_\Gamma$, using $U-\beta$ for a root with $v(\beta)\ge0$ and $1-U/\beta$
for a root with $v(\beta)<0$. Every factor has Gauss valuation zero, and a
product of such factors has Gauss valuation zero because the reductions are
nonzero polynomials over a field. Reduction then sends each
negative-valuation factor to $1$. It is printed once, as
\cref{dyn:lem:root-count}.

\begin{theorem}[The radius-shell divisor and sharp maximality; source 11]
\label{dyn:rd:thm:shell}
The polynomial $f$ of \eqref{dyn:rd:eq:family} has exactly $q\deg R$
nonzero periodic points in $\bar B_{r_{\mathrm{eq}}}$, counted with
algebraic multiplicity in $f^{\circ q}(z)-z$. All of them lie on
$S_{r_{\mathrm{eq}}}$ and have exact period $q$. Equivalently there are
$\deg R$ cycles, each counted with the common multiplicity of its points.
If $R$ is squarefree, these are $\deg R$ distinct simple cycles, one for
each root of $R$ in the coordinate $(z/\rho)^{q}$. The ball
$B_{r_{\mathrm{eq}}}$ is the maximal centered valuation ball on which an
injective conjugacy fixing $0$ can carry $f$ to multiplication by
$\lambda$, even if the conjugacy is not assumed to be a power series.
\end{theorem}
\begin{proof}[Proof \textup{(source 11)}]
$\widehat R$ reduces to $R$, so \cref{dyn:lem:root-count} gives exactly
$\deg R$ unit roots $U$, with multiplicity. Every such $U$ has $q$ distinct
$q$-th roots in $K_\Gamma$, and $U=X^{q}$ has derivative $qX^{q-1}\ne0$
there, so each multiplicity is preserved. By
\cref{dyn:rd:prop:periodfinite} these $q\deg R$ points are exactly the
nonzero periodic points of $F$ in $\OO_\Gamma$, hence of $f$ in the closed
ball, and they are units in the normalized coordinate, so they lie on the
shell. The quotient $\mathcal Q_q$ of \eqref{dyn:rd:eq:iteratediv} is a unit
at each point of $\OO_\Gamma$, so the multiplicity in $F^{\circ q}-X$
equals that in $\psi-X=\zeta^{-1}eX\widehat R(X^{q})$. Rotation preserves
it and partitions the nonzero roots into orbits of size $q$, which gives
every count, including the squarefree case. Since $\deg R\ge1$ the shell
carries a nonzero $q$-cycle, and every centered ball strictly containing
$B_{r_{\mathrm{eq}}}$ contains the whole shell. An injective $J$ with
$J(0)=0$ would send a point $p$ of that cycle to a solution of
$\lambda^{q}J(p)=J(p)$; non-torsion forces $J(p)=0$, contradicting
injectivity. Existence on $B_{r_{\mathrm{eq}}}$ is
\cref{dyn:rd:thm:linearization}.
\end{proof}

By \cref{dyn:rd:prop:return}, the count and the maximality are
\cref{dyn:thm:exact} and \cref{dyn:thm:shell}(i),(iii) for $P=R(X^{q})$;
they are printed once there and credited to source 11 here. Its proof is a
second route. It uses the iterate quotient on the whole closed ball instead
of the return polynomial $Q$, and it yields the closed-ball exclusion of
\cref{dyn:rd:prop:periodfinite} as well. For a simple root the leading
return-multiplier ratio of \eqref{dyn:eq:multiplier} is
$aP'(a)=qa^{q}R'(a^{q})=q\beta R'(\beta)=1/\mathfrak r_\beta$ at
$\beta=a^{q}$, the reciprocal of the residue in \eqref{dyn:rd:eq:alpha}
below.

\begin{remark}[What maximality does not say]\label{dyn:rd:rem:scope}
Source 11 states independently what \cref{dyn:rem:exact-scope} states. The
theorem does not say that the series $H$ fails to be summable at every
individual shell point. It says only that no injective linearizing chart
extends to a whole larger centered ball; extension to specially chosen sets
omitting the obstructing cycles is a different question
(\cref{dyn:rd:q:continuation}). \mergetag\ In one model the stronger
statement is known. For $q=1$, $e=t^{\delta}$ and the single term $z^{1+k}$,
source 11's map is $(1+t^{\delta})z+z^{1+k}$, the model of
\cref{dyn:thm:sharp} with its $q$ equal to $k$. There the Taylor family of
$H$ fails to be strongly summable at \emph{every} point of
$S_{\delta/k}$, which is consistent with source 11's disclaimer and
strictly stronger than it.
\end{remark}

\subsection{The boundary vector field and algebraicity of the reduced
linearizer}\label{dyn:rd:subsec:reduction}

By \cref{dyn:rd:thm:linearization} every coefficient of $H_r$ is integral,
so its coefficientwise reduction $\bar H_r\in\C[[X]]$ is defined. This is a
formal operation, not the evaluation of $H_r$ at an ordinary point of the
shell.

\begin{theorem}[First-order boundary reduction; source 11]
\label{dyn:rd:thm:reduction}
$\bar H_r$ is the unique formal series $X+O(X^{2})$ with
\begin{equation}\label{dyn:rd:eq:vectorfield}
 XR(X^{q})\,\bar H_r'(X)=\bar H_r(X).
\end{equation}
Writing $\bar H_r(X)=X\mathfrak h(X^{q})$ with $\mathfrak h(0)=1$,
\begin{align}
 \frac{\mathfrak h'(U)}{\mathfrak h(U)}&=\frac{R(U)^{-1}-1}{qU},
 \label{dyn:rd:eq:hlog}\\
 \bar H_r(X)&=X\exp\left(\int_0^{X^{q}}\frac{R(U)^{-1}-1}{qU}\dd U\right),
 \label{dyn:rd:eq:h0integral}
\end{align}
where the integral and the exponential are ordinary formal power-series
operations over $\C$.
\end{theorem}
\begin{proof}[Proof \textup{(source 11: a formal Taylor identity, with no
common disk)}]
In the independent variable $X$,
\[
 H_r\bigl(\zeta X+eV(X)\bigr)=H_r(\zeta X)+eV(X)H_r'(\zeta X)
 +\sum_{\ell\ge2}\frac{e^{\ell}V(X)^{\ell}}{\ell!}H_r^{(\ell)}(\zeta X),
 \qquad V(X)=X\widehat R(X^{q}).
\]
At each degree in $X$ the sum is finite, because $V$ has no constant term
and a fixed monomial has only finitely many nonzero derivatives, and all
coefficients other than the displayed powers of $e$ are integral.
Equivariance gives $H_r(\zeta X)=\zeta H_r(X)$ and $H_r'(\zeta X)=H_r'(X)$.
Subtract these terms from $H_r(F(X))=(\zeta+e)H_r(X)$, divide by $e$ and
reduce coefficientwise: every term with $\ell\ge2$ dies, and
$XR(X^{q})\bar H_r'=\bar H_r$ remains. Equivariance already puts
$\bar H_r$ in the form $X\mathfrak h(X^{q})$, and substitution gives
$R(U)\bigl(\mathfrak h+qU\mathfrak h'\bigr)=\mathfrak h$, hence
\eqref{dyn:rd:eq:hlog}. Since $R^{-1}-1$ has no constant term the quotient
by $U$ is a formal power series; termwise integration with zero constant
term and formal exponentiation give the unique solution with
$\mathfrak h(0)=1$.
\end{proof}

\begin{remark}[The same equation as the coherent leading coefficient; two
routes]\label{dyn:rd:rem:coherent}
\mergetag\ \Cref{dyn:thm:coherent} gives, for every $\resq$ polynomial, the
exponent-zero coefficient $H_0$ of $H_r$ as a holomorphic function on an
ordinary disk, with $V_0H_0'=H_0$ and $V_0=XP(X)$ \eqref{dyn:eq:ode0}. By
\cref{dyn:rd:prop:return}, $V_0=XR(X^{q})$ here, so
\eqref{dyn:rd:eq:vectorfield} is the Taylor expansion at zero of
\eqref{dyn:eq:ode0}, and \eqref{dyn:eq:H0} becomes
\eqref{dyn:rd:eq:h0integral} under the substitution $U=s^{q}$ in its
integral. The equation is printed once, as \eqref{dyn:eq:ode0}. Source 11's
proof is a different route and is kept: a formal Taylor identity in
$\C[[X]]$ after division by $e$, with no ordinary disk and no transfinite
recursion. \Cref{dyn:thm:coherent} gives more, the holomorphy of every
$H_\gamma$ on one disk; source 11 gives the equation from the formal
coefficients alone.
\end{remark}

\begin{corollary}[A finite recurrence for the boundary series; source 11]
\label{dyn:rd:cor:boundaryrec}
If $\mathfrak h(U)=\sum_{k\ge0}\mathfrak h_kU^{k}$, then $\mathfrak h_0=1$ and
\begin{equation}\label{dyn:rd:eq:betarec}
 qk\,\mathfrak h_k=-\sum_{j=1}^{\min(k,\deg R)}
 \varkappa_j\bigl(1+q(k-j)\bigr)\mathfrak h_{k-j}\qquad(k\ge1).
\end{equation}
So every finite jet of the boundary series is computed by a finite recursion
from $q$ and the coefficients of $R$.
\end{corollary}
\begin{proof}
Compare coefficients of $U^{k}$ in
$R(\mathfrak h+qU\mathfrak h')=\mathfrak h$; the term $j=0$ contributes
$(1+qk)\mathfrak h_k$.
\end{proof}

The following argument gives the exact algebraicity test. The observation
about logarithmic derivatives is standard algebra; source 11 claims only its
application to the boundary series.

\begin{lemma}[Norm obstruction; source 11]\label{dyn:rd:lem:norm}
Suppose $0\ne g$ is algebraic over $\C(X)$ and $g'/g=\varpi(X)\in\C(X)$.
Then $\varpi$ has only simple finite poles, and its residue at each finite
pole is rational.
\end{lemma}
\begin{proof}
Let $L_g=\C(X,g)$ and $D=[L_g:\C(X)]$. In characteristic zero the
derivation $d/dX$ extends uniquely to algebraic extensions, so every field
embedding of $L_g$ into an algebraic closure commutes with it. Taking the
norm gives $(N g)'/(N g)=D\varpi$, where $N g=N_{L_g/\C(X)}g$ is a nonzero
rational function, a constant times a finite product $\prod_a(X-a)^{n_a}$
with integers $n_a$. Its logarithmic derivative is $\sum_an_a/(X-a)$;
divide by $D$.
\end{proof}

\begin{theorem}[Exact algebraicity criterion for the reduction; source 11]
\label{dyn:rd:thm:algebraicity}
Let $R\in\C[U]$ be nonconstant with $R(0)=1$, and let $\bar H_r$ be
\eqref{dyn:rd:eq:h0integral}. Then $\bar H_r$ is algebraic over $\C(X)$ if
and only if $R$ is squarefree and
\begin{equation}\label{dyn:rd:eq:alpha}
 \mathfrak r_\beta=\frac1{q\beta R'(\beta)}\in\Q
 \qquad\text{for every root }\beta\text{ of }R.
\end{equation}
In that case
\begin{equation}\label{dyn:rd:eq:h0product}
 \bar H_r(X)=X\prod_{R(\beta)=0}\bigl(1-X^{q}/\beta\bigr)^{\mathfrak r_\beta},
\end{equation}
each power being the formal binomial branch with constant coefficient $1$.
For $f$ as in \eqref{dyn:rd:eq:family}, failure of either condition proves
that the normalized linearizer $H(z)$ is transcendental over $K_\Gamma(z)$.
\end{theorem}
\begin{proof}
By \eqref{dyn:rd:eq:vectorfield}, $\bar H_r'/\bar H_r=1/(XR(X^{q}))$. A
repeated root $\beta$ of $R$ is nonzero because $R(0)=1$, and at each
$q$-th root of $\beta$ the right side has a pole of order at least two,
contradicting \cref{dyn:rd:lem:norm} if $\bar H_r$ is algebraic. If $R$ is
squarefree, the residue at any $a$ with $a^{q}=\beta$ is
$1/(a\,R'(\beta)\,qa^{q-1})=\mathfrak r_\beta$, and \cref{dyn:rd:lem:norm}
proves necessity of \eqref{dyn:rd:eq:alpha}.

For sufficiency, the rational function in \eqref{dyn:rd:eq:hlog} is regular
at $0$, tends to $0$ at infinity, and has simple poles at the roots of $R$
with residues $\mathfrak r_\beta$. Partial fractions give
$(R(U)^{-1}-1)/(qU)=\sum_{R(\beta)=0}\mathfrak r_\beta/(U-\beta)$, and
integration with $\mathfrak h(0)=1$ gives \eqref{dyn:rd:eq:h0product}. If
all $\mathfrak r_\beta$ are rational, a common positive denominator $N_0$
makes $\bar H_r^{N_0}$ rational in $X$, so $\bar H_r$ is algebraic.

Finally suppose $H$ is algebraic over $K_\Gamma(z)$ while $\bar H_r$ is
transcendental. Then $H_r(X)=\rho^{-1}H(\rho X)$ is algebraic over
$K_\Gamma(X)$, so some nonzero $\mathcal P(X,T)\in K_\Gamma[X,T]$ satisfies
$\mathcal P(X,H_r(X))=0$. Multiply $\mathcal P$ by one nonzero scalar so
that its finitely many coefficients all have nonnegative valuation and one
has valuation zero. Since $H_r\in\OO_\Gamma[[X]]$, coefficientwise reduction
gives $\bar{\mathcal P}(X,\bar H_r(X))=0$ with
$0\ne\bar{\mathcal P}\in\C[X,T]$, contradicting transcendence of
$\bar H_r$.
\end{proof}

The last step normalizes a \emph{finite} relation before reducing it, so its
reduction is not the zero polynomial. This is the argument of
\cref{dyn:lem:reduction}, over $K_\Gamma(z)$ instead of $\BB$.

\begin{remark}[The condition is exact at the residue level only]
\label{dyn:rd:rem:residuelevel}
The theorem decides algebraicity of $\bar H_r$ completely from the finite
polynomial $R$. Passing the test does not prove $H$ algebraic, because the
higher Hahn coefficients were discarded by reduction. Failing it is a
conclusive obstruction for $H$, not merely an obstruction to one proposed
formula.
\end{remark}

\begin{remark}[The same criterion under $\pares$; two routes]
\label{dyn:rd:rem:parabolic}
\mergetag\ Put $V(X)=XR(X^{q})$ and $\mu=1$. Its zeros are $0$ and the $a$
with $a^{q}=\beta$ for a root $\beta$ of $R$; they are simple exactly when
$R$ is squarefree; and $V'(a)=q\beta R'(\beta)$, so
$\mu/V'(a)=\mathfrak r_\beta$. The first clause of
\cref{dyn:prop:leadingalgebraic}, and its product formula
\eqref{dyn:eq:h0product}, involve only the polynomial $V$: the coefficient
of $\eps^{0}$ in $\Omega_\eps$ is $\mu/V$. Applied to this $V$ they are
\eqref{dyn:rd:eq:alpha} and \eqref{dyn:rd:eq:h0product}. Equivalently, with
$U=X^{q}$ one has $\bar H_r(X)^{q}=k(U)$, where $k(U)=U\mathfrak h(U)^{q}$
satisfies $k'/k=1/(UR(U))$. This is the leading equation
$h_0'/h_0=\mu/V$ with $V=UR(U)$, whose criterion
$1/(\beta R'(\beta))\in\Q$ is equivalent to \eqref{dyn:rd:eq:alpha}
because $q$ is a nonzero integer. The proofs differ, and both are kept.
Source 11 proves necessity through the norm of an algebraic logarithmic
derivative (\cref{dyn:rd:lem:norm}); source 04 proves it through finite
monodromy orbits and the growth bound \cref{dyn:lem:essential}.

\emph{Every $\resq$ polynomial} \mergetag. This remark is a merge
observation and is claimed by no source. The first two paragraphs of the
proof of \cref{dyn:rd:thm:algebraicity} use only the equation
$XR(X^{q})\bar H_r'=\bar H_r$ with $R(0)=1$ and $R$ nonconstant. By
\eqref{dyn:eq:ode0}, the Taylor expansion of $H_0$ satisfies
$XP(X)H_0'=H_0$ for every polynomial $f$ under $\resq$, with $P$ the shell
polynomial of \eqref{dyn:eq:P}, which is nonconstant with $P(0)=1$. The
same argument, with $XR(X^{q})$ replaced by $XP(X)$, shows that $H_0$ is
algebraic over $\C(X)$ if and only if $P$ is squarefree and
$1/(aP'(a))\in\Q$ for every root $a$ of $P$, and then
$H_0=X\prod_a(1-X/a)^{1/(aP'(a))}$. The ratio $aP'(a)$ is the
return-multiplier ratio of \eqref{dyn:eq:multiplier}. The third paragraph
applies unchanged: $H_r$ is integral by \cref{dyn:thm:exact}, and its
reduction is the Taylor expansion of $H_0$. So failure makes $H$
transcendental over $K_\Gamma(z)$. For example, the tuned cubic
\eqref{dyn:eq:tuned-cubic} has $P=1-2X^{4}$ and $aP'(a)=-4$, and its
$H_0=X(1-2X^{4})^{-1/4}$ is algebraic, in agreement with
\eqref{dyn:eq:phase-H0}.
\end{remark}

\begin{remark}[Where source 11's certificate is superseded]
\label{dyn:rd:rem:superseded}
\mergetag\ Suppose $e=c_0t^{\delta}$ is a single monomial, with
$c_0\in\C^{\times}$, and every $\hat a_j$ is an ordinary constant, so that
$\widehat R=R$. Then $\psi=\zeta^{-1}F$ is the Euler map
\[
 \psi(X)=X+t^{\delta}\widetilde V(X),\qquad
 \widetilde V(X)=c_0\zeta^{-1}XR(X^{q})\in\C[X],\qquad
 \widetilde V'(0)=\mu=c_0\zeta^{-1},
\]
of \eqref{dyn:eq:Euler}. Equivariance gives
$H_r\circ\psi=\zeta^{-1}H_r\circ F=(1+\mu t^{\delta})H_r$, so by formal
uniqueness $H_r$ is the Taylor expansion at zero of the solution $H_\eps$
of \eqref{dyn:eq:schroederintro} for $\widetilde V$. Taylor expansion at
zero is an injective ring homomorphism from $\OO(U)((t^{\Gamma}))$, with
$U$ connected, to power series over $K_\Gamma$, and it carries $K_\Gamma[X]$
identically. So a nonzero relation $\mathcal P(X,H_r)=0$ over
$K_\Gamma[X]$ would give $\mathcal P(w,H_\eps)=0$ on $U$, hence on the
universal cover by continuation, and $H_\eps$ would be algebraic over
$K_\Gamma(w)\subset\BB$. Since $\deg\widetilde V=1+q\deg R\ge2$,
\cref{dyn:thm:dichotomy} excludes this. So in this subcase $H$ is
\textbf{always} transcendental over $K_\Gamma(z)$, whether or not $R$ passes
\eqref{dyn:rd:eq:alpha}. When $\bar H_r$ is algebraic,
\cref{dyn:prop:leadingalgebraic} shows that the first correction, the
coefficient of $t^{\delta}$ in $H_\eps$, is already transcendental over
$\C(w)$. There source 11's one-way certificate is superseded, and its
question on higher boundary obstructions is answered at the first layer
(\cref{dyn:rd:q:higher}). For $q=1$ the map $F$ itself is the Euler map.
Outside this subcase, when $e$ is not a constant times a monomial or some
$\hat a_j$ has a nonzero positive-support part, $\psi$ is not an Euler map
and \cref{dyn:rd:thm:algebraicity} is what is available. It is new to this
report there, for every $q\ge1$. For $q\ge2$ the map $F$ itself has
multiplier near $\zeta\ne1$ and is in no parabolic family of
\cref{dyn:sec:parabolic-monodromy}; the Euler form appears only after
dividing by $\zeta$. The Hahn-valued case is close to the moving and
perturbed Euler maps of \cref{dyn:q:moving} and does not answer that
question.
\end{remark}

\subsection{Examples, an explicit real quintic, and stability}
\label{dyn:rd:subsec:examples}

\begin{example}[A binomial boundary]\label{dyn:rd:ex:binomial}
If $R(U)=1+\varkappa U^{k}$ with $\varkappa\ne0$ and $k\ge1$ (source 11
writes $c$ and $q$), then $\beta R'(\beta)=-k$ at every root and
\[
 \bar H_r(X)=X\bigl(1+\varkappa X^{qk}\bigr)^{-1/(qk)} .
\]
The reduced linearizer is algebraic, and the shell carries $k$ distinct
simple $q$-cycles. This illustrates the algebraic side of the criterion; it
does not assert that $H$ is algebraic. \mergetag\ For $q=1$ this is the
tangent binomial of \cref{dyn:sec:phase}, with $H_0=X(1+AX^{m-1})^{-1/(m-1)}$
there, and the universal profile $w(1+w^{k})^{-1/k}$ of
\cref{dyn:cor:universal} after normalizing $\varkappa=1$; for $q=k=1$ it is
$w/(1+w)$ of \cref{dyn:ex:rational}. Source 11 extends the profile to
rotation order $q$ for polynomial equivariant input.
\end{example}

\begin{example}[Simple roots forcing transcendence]
\label{dyn:rd:ex:quadraticR}
Let $R(U)=1+U+U^{2}$. For $\beta_\pm=(-1\pm i\sqrt3)/2$,
\[
 \mathfrak r_{\beta_\pm}=\frac1q\Bigl(-\frac12\pm\frac{i}{2\sqrt3}\Bigr),
\]
with the sign matched to the root. Neither is rational, so $H$ is
transcendental whenever this $R$ is the boundary polynomial, although the
shell carries exactly two simple $q$-cycles. Transcendence does not require
a singular or multiple periodic point.
\end{example}

\begin{example}[A repeated root and an exponential factor]
\label{dyn:rd:ex:repeated}
For $R(U)=(1-U)^{2}$ direct integration gives
\[
 \bar H_r(X)=X(1-X^{q})^{-1/q}\exp\left(\frac{X^{q}}{q(1-X^{q})}\right).
\]
The repeated root makes $\bar H_r$, hence $H$, transcendental. If the
unreduced $\widehat R$ is exactly $(1-U)^{2}$, its one geometric cycle has
multiplicity two. If it only reduces to $(1-U)^{2}$, it may split into two
cycles at a finer scale, and the multiplicity count of
\cref{dyn:rd:thm:shell} remains valid (\cref{dyn:rem:multiplicity}).
\mergetag\ For $q=1$ this is $-h_0(-X)$ for the $h_0$ of
\cref{dyn:ex:multiplezero}.
\end{example}

\begin{example}[Two simple roots with rational residues]
\label{dyn:rd:ex:tworoots}
For $R(U)=(1-U)(1-2U)$ the roots are $1$ and $1/2$, with
$\mathfrak r_1=1/q$ and $\mathfrak r_{1/2}=-2/q$, so
\[
 \bar H_r(X)=X(1-X^{q})^{1/q}(1-2X^{q})^{-2/q} .
\]
This non-binomial algebraic example separates squarefreeness from the
rational-residue condition. \mergetag\ For $q=1$ it is the leading
coordinate $w(1-w)/(1-2w)^{2}$ of \cref{dyn:ex:trivialroot}. When
$e=t^{\delta}$ and $\hat a_1=-3$, $\hat a_2=2$ are exact, that example
shows the first correction $h_1=\tfrac32h_0\log(1-2w)$ to be transcendental
(\cref{dyn:rd:rem:superseded}).
\end{example}

\begin{example}[A fully explicit real quintic]\label{dyn:rd:ex:quintic}
Fix a positive $\eta\in\Gamma$. The polynomial
\begin{equation}\label{dyn:rd:eq:quintic}
 f(z)=(-1+t^{4\eta})z-2t^{2\eta}z^{3}+z^{5}
\end{equation}
has $q=2$, $\zeta=-1$, $e=t^{4\eta}$ and
$r_{\mathrm{eq}}=\max\{(4\eta-2\eta)/2,\,(4\eta-0)/4\}=\eta$. With
$\rho=t^{\eta}$ its normalized coefficients are $\hat a_1=-2$ and
$\hat a_2=1$, so $R(U)=(1-U)^{2}$. Hence the strong linearization ball is
$v(z)>\eta$, $H$ is transcendental over $K_\Gamma(z)$, and
$\{t^{\eta},-t^{\eta}\}$ is an exact real two-cycle on the shell: the
equation $f(z)=-z$ is $z(z^{2}-t^{2\eta})^{2}=0$, which exhibits the cycle
and its multiplicity directly. Replacing $-2t^{2\eta}z^{3}$ by
$+t^{2\eta}z^{3}$ gives $R=1+U+U^{2}$. The conjugacy is still
transcendental, but the two shell cycles are nonreal, so sharpness of the
complex centered ball does not by itself establish sharpness of a real
interval (\cref{dyn:rd:q:real}); \eqref{dyn:rd:eq:quintic} avoids that
issue. \mergetag\ Both polynomials lie in the monomial subcase of
\cref{dyn:rd:rem:superseded}.
\end{example}

\begin{example}[Higher-rank scale separation]\label{dyn:rd:ex:rank2}
Let $\Gamma=\Q^{2}$ with the lexicographic order, $e=t^{(1,0)}$, and
$f(z)=(\zeta+e)z+t^{(0,1)}z^{1+q}$. The exact threshold is
$r_{\mathrm{eq}}=(1/q,-1/q)$, a positive element of $\Gamma$. The proof uses
this group element literally; it selects no real number to measure it and
stays valid at arguments whose extra infinitesimal scale is smaller than
every rational multiple of another positive value. Compare
\cref{dyn:ex:rank2}.
\end{example}

\begin{proposition}[Stability of the first boundary data; source 11]
\label{dyn:rd:prop:stability}
Fix $q$, $\zeta$, $e$ and $\rho$ in \eqref{dyn:rd:eq:F}. Replace finitely
many $\hat a_j$ by $\hat a_j+\epsilon_j$ with $\epsilon_j\in\m_\Gamma$, and
allow finitely many additional terms $\hat a_jU^{j}$, $j>J$, in
$\widehat R$ with $\hat a_j\in\m_\Gamma$. Then $r_{\mathrm{eq}}$, $R$,
$\bar H_r$ and the multiplicity count on the shell are unchanged. A simple
root of $R$ determines a unique lifted cycle for each perturbed polynomial,
and every transcendence certificate of \cref{dyn:rd:thm:algebraicity}
remains valid.
\end{proposition}
\begin{proof}
Every normalized coefficient stays integral, and a unit coefficient stays a
unit with the same residue, so the candidate threshold is still attained at
$r_{\mathrm{eq}}$ and reduction gives the same $R$. The previous theorems
apply to the new finite list of inputs, possibly with a larger common
support set. The simple-cycle statement is \cref{dyn:lem:root-count}. No
assertion is intended that the lifted points themselves stay fixed.
\end{proof}

\mergetag\ \Cref{dyn:thm:stability} is the general-$\resq$ counterpart. It
perturbs the \emph{return} coefficients, and \cref{dyn:warn:stability}
forbids replacing its hypothesis by the same inequalities on the
coefficients of $f$. For equivariant maps \cref{dyn:rd:prop:return} makes
the threshold a function of the coefficients of $f$. Source 11's hypothesis
on the normalized coefficients of $f$ is therefore legitimate here, but only
for perturbations that stay in the family \eqref{dyn:rd:eq:family}. The
point-distance estimate \eqref{dyn:eq:stable-distance} is proved only for
the return version; source 11 claims no distance estimate.

\subsection{Periodic points of integral equivariant maps, and prescribed
shells}\label{dyn:rd:subsec:periods}

The closed-ball argument of \cref{dyn:rd:prop:periodfinite} has a second
form. It applies on all of $\m_\Gamma$ even when the nonlinear coefficients
are not divisible by $e$, and even when $\lambda=\zeta$.

\begin{theorem}[Infinitesimal periodic points are rotation points; source
11]\label{dyn:rd:thm:allperiods}
Let $f(z)=\lambda z+\sum_{j=1}^{J}a_{1+jq}z^{1+jq}\in\OO_\Gamma[z]$ with
$\res(\lambda)=\zeta$, a primitive ordinary $q$-th root of unity. Every
nonzero infinitesimal periodic point of $f$ satisfies $f(z)=\zeta z$ and
has exact period $q$, and conversely every nonzero infinitesimal solution of
$f(z)=\zeta z$ is periodic of exact period $q$. The hypothesis
$\lambda\ne\zeta$ is not needed.
\end{theorem}
\begin{proof}
$f$ preserves $\m_\Gamma$. Put $\psi=\zeta^{-1}f$; it commutes with the
rotation, and $\mu_\psi=\psi'(0)$ has residue $1$. If $q\nmid n$, the
constant term of $(f^{\circ n}(X)-X)/X$ is $\lambda^{n}-1$, a unit, and all
other coefficients are integral, so at a nonzero infinitesimal the constant
term dominates and $f^{\circ n}(z)\ne z$. If $q\mid n$ then
$f^{\circ n}=\psi^{\circ n}$, and $\mathcal Q_n$ of
\cref{dyn:rd:lem:iterate} is integral with constant term of residue $n$. Its
value at an infinitesimal is therefore a unit, even though the reduction of
$\mathcal Q_n$ as a polynomial need not be constant. So
$\psi^{\circ n}(z)=z$ is equivalent to $\psi(z)=z$, which is
$f(z)=\zeta z$. The converse and the exact period follow from equivariance
and the exact order of $\zeta$.
\end{proof}

\begin{remark}[A stronger classification on a narrower domain]
\label{dyn:rd:rem:narrower}
\Cref{dyn:rd:thm:allperiods} classifies all infinitesimal periodic points of
any integral equivariant polynomial, and makes no claim about periodic points
of valuation zero, where the constant term of $\mathcal Q_n$ need not
dominate. \Cref{dyn:rd:prop:periodfinite} works on the whole closed
normalized ball because there the entire reduction of $\psi$ is the
identity. When $\lambda=\zeta$ exactly, the multiplier has finite order,
which is neither $\resq$ nor $\mres$, and \cref{dyn:thm:finite-order} is
the relevant normal-form theorem; \cref{dyn:rd:thm:allperiods} concerns
periodic points only and linearizes nothing.
\end{remark}

\begin{theorem}[Exact finite shell realization; source 11]
\label{dyn:rd:thm:allshells}
Choose ordinary $q,N\ge1$, a primitive $q$-th root $\zeta\in\C$, positive
$r_1,\dots,r_N\in\Gamma$, and $\varsigma_1,\dots,\varsigma_N\in\C^{\times}$
such that the elements $\mathfrak b_\ell=\varsigma_\ell t^{qr_\ell}$ are
pairwise distinct. Put
\begin{equation}\label{dyn:rd:eq:shellf}
 \mathcal S(U)=\prod_{\ell=1}^{N}(U-\mathfrak b_\ell),\qquad
 f(z)=z\bigl(\zeta+\mathcal S(z^{q})\bigr).
\end{equation}
The nonzero infinitesimal periodic points of $f$ are exactly
\begin{equation}\label{dyn:rd:eq:shellpoints}
 z=t^{r_\ell}\vartheta,\qquad \vartheta^{q}=\varsigma_\ell,\qquad
 1\le\ell\le N .
\end{equation}
They form $N$ distinct simple exact $q$-cycles, and there is no other
nonzero infinitesimal periodic point. The maximal centered strong
linearization ball at $0$ is $v(z)>r_{\max}$, with
$r_{\max}=\max_\ell r_\ell$.
\end{theorem}
\begin{proof}
Every coefficient of $\mathcal S$ is integral, and its constant coefficient
$e=\mathcal S(0)=(-1)^{N}\prod_\ell\mathfrak b_\ell$ is nonzero and
infinitesimal. So the multiplier of $f$ is $\zeta+e$ and $f$ satisfies the
hypotheses of \cref{dyn:rd:thm:linearization,dyn:rd:thm:allperiods}. For
$z\ne0$ the equation $f(z)=\zeta z$ is $\mathcal S(z^{q})=0$, whose roots
are \eqref{dyn:rd:eq:shellpoints}: all infinitesimal, and all simple because
the $\mathfrak b_\ell$ are distinct and nonzero.
\Cref{dyn:rd:thm:allperiods} excludes every other infinitesimal period.

For the threshold, allowing for cancellation among the coefficients of
$\mathcal S$, write $\mathcal S(U)=e+\sum_{j=1}^{N}s_jU^{j}$. Since
$\mathcal S(U)/e=\prod_\ell(1-U/\mathfrak b_\ell)$, each $s_j/e$ is a sum of
products of $j$ reciprocals $\mathfrak b_\ell^{-1}$, so
$v(s_j/e)\ge-jq\,r_{\max}$ and $r_{\mathrm{eq}}\le r_{\max}$. If it were
strictly smaller, every root of valuation $r_{\max}$ would lie in the
linearization ball of \cref{dyn:rd:thm:linearization}, contradicting its
periodicity. So $r_{\mathrm{eq}}=r_{\max}$, and
\cref{dyn:rd:thm:shell} gives maximality.
\end{proof}

Repeated scales are permitted, with different $\varsigma_\ell$ when the
$r_\ell$ agree. The theorem prescribes every \emph{infinitesimal} periodic
point, not every periodic point of the polynomial on $K_\Gamma$: other finite
or infinitely large cycles of $f$ are not excluded.

\subsection{Reciprocal rational maps with Gaussian omnific periodic points}
\label{dyn:rd:subsec:omnific}

An omnific integer has a Conway normal form supported on nonnegative growth
exponents with an ordinary integer constant coefficient, so
$\Oz=\Z\oplus\mathbb J$ with $\mathbb J$ the class of purely infinite forms,
including $0$; its nonzero members may have either sign. The Gaussian
omnific ring is $\Oz[i]=\Z[i]\oplus(\mathbb J+i\mathbb J)$. In particular
$\varsigma\omega^{r}\in\Oz[i]$ for every ordinary $\varsigma\in\C$ and every
positive surreal $r$. These are the normal-form conventions of the
collection's notation guide, which writes $\Pi$ for $\mathbb J$. The
coefficient-extraction retraction on this ring is not the standard-part map
on $\OO_\Gamma$: their support restrictions point in opposite directions
(\cref{dyn:warn:orientation}).

\begin{theorem}[Exact omnific periodic sets for rational maps; source 11]
\label{dyn:rd:thm:omnific}
Use the data of \cref{dyn:rd:thm:allshells}, now with positive surreal
$r_\ell$, and read $t^{\gamma}=\omega^{-\gamma}$. The reciprocal rational map
\begin{equation}\label{dyn:rd:eq:omnificG}
 f^{\vee}(w)=\frac1{f(1/w)}
 =\frac{w^{1+qN}}{\zeta w^{qN}+\prod_{\ell=1}^{N}(1-\mathfrak b_\ell w^{q})}
\end{equation}
has as its infinitely large finite periodic points exactly
\begin{equation}\label{dyn:rd:eq:ompoints}
 w=\omega^{r_\ell}\vartheta^{-1},\qquad \vartheta^{q}=\varsigma_\ell,\qquad
 1\le\ell\le N .
\end{equation}
Every such point lies in $\Oz[i]$, each cycle has exact period $q$, and
$f^{\vee}$ acts on them by $w\mapsto\zeta^{-1}w$. There is a
numerator--denominator presentation of $f^{\vee}$ in which every coefficient
is purely infinite Gaussian omnific; one explicit common clearing factor is
$\omega^{L_{\mathrm{cl}}}$ with
\begin{equation}\label{dyn:rd:eq:clearL}
 L_{\mathrm{cl}}=1+q\sum_{\ell=1}^{N}r_\ell .
\end{equation}
The projective point $\infty$ is an additional fixed point and is not a
``finite periodic point''.
\end{theorem}
\begin{proof}
The coordinate change $w=1/z$ conjugates $f$ on $\mathbb P^{1}(\SC)$ to
$f^{\vee}$, and finite infinitely large nonzero $w$ correspond exactly to
nonzero infinitesimal $z$. A periodic orbit starting at such a $z$ cannot
pass through $0$ and return, since $0$ is fixed. So the classification of
\cref{dyn:rd:thm:allshells} transfers with no missing polar case and gives
\eqref{dyn:rd:eq:ompoints}; reciprocals of $f(z)=\zeta z$ give the action.
Each point is one positive-growth monomial with an ordinary complex
coefficient, hence Gaussian omnific. Every denominator coefficient in
\eqref{dyn:rd:eq:omnificG} is a finite sum of an ordinary constant and terms
$\varsigma\,\omega^{-q\sum_{\ell\in I}r_\ell}$ for subsets
$I\subseteq\{1,\dots,N\}$. After multiplication by
$\omega^{L_{\mathrm{cl}}}$ each has growth exponent
$L_{\mathrm{cl}}-q\sum_{\ell\in I}r_\ell\ge1$, and the numerator coefficient
becomes $\omega^{L_{\mathrm{cl}}}$. So numerator and denominator have purely
infinite Gaussian omnific coefficients and define the same rational map.
Finally $f(0)=0$ gives $f^{\vee}(\infty)=\infty$.
\end{proof}

\begin{remark}[A presentation is not a ring endomorphism]
\label{dyn:rd:rem:presentation}
The theorem concerns a rational map over the fraction field of the omnific
ring, with an explicit presentation over that ring. It does not say that
$f^{\vee}(a)\in\Oz[i]$ for every $a\in\Oz[i]$ at which $f^{\vee}$ is
defined, nor that its denominator is a unit. Omnific integrality is proved
for the whole classified periodic set, not for every orbit
(\cref{dyn:rd:q:omnific}).
\end{remark}

\begin{example}[Two widely separated scales]\label{dyn:rd:ex:twoscale}
Take $q=2$, $\zeta=-1$, $N=2$, $\varsigma_1=\varsigma_2=1$, $r_1=1$ and
$r_2=\omega$, so $\mathfrak b_1=\omega^{-2}$ and
$\mathfrak b_2=\omega^{-2\omega}$. The real polynomial
$f(z)=z\bigl[-1+(z^{2}-\mathfrak b_1)(z^{2}-\mathfrak b_2)\bigr]$ has
exactly the two nonzero infinitesimal two-cycles
$\{\omega^{-1},-\omega^{-1}\}$ and $\{\omega^{-\omega},-\omega^{-\omega}\}$,
and its maximal centered surcomplex linearization ball is $v(z)>\omega$.
The reciprocal map
\begin{equation}\label{dyn:rd:eq:twoscaleG}
 f^{\vee}(w)=\frac{w^{5}}{-w^{4}+(1-\mathfrak b_1w^{2})(1-\mathfrak b_2w^{2})}
\end{equation}
has exactly the infinitely large finite periodic points $\omega$, $-\omega$,
$\omega^{\omega}$ and $-\omega^{\omega}$, paired into those two two-cycles.
Multiplying numerator and denominator of \eqref{dyn:rd:eq:twoscaleG} by
$\omega^{2\omega+3}=\omega^{L_{\mathrm{cl}}}$ puts every coefficient in the
purely infinite real omnific ideal. No limiting process is used to produce
the four points or to exclude further infinite periods.
\end{example}

\begin{proposition}[Class-level interpretation without class sums; source
11]\label{dyn:rd:prop:transfer}
\Cref{dyn:rd:thm:linearization,dyn:rd:thm:shell,dyn:rd:thm:reduction,%
dyn:rd:thm:algebraicity,dyn:rd:thm:allperiods,dyn:rd:thm:allshells,%
dyn:rd:thm:omnific} hold for actual surcomplex coefficients, with the ball
and periodic-point assertions read pointwise over $\SC$. Every evaluation and
coefficient construction involved stays set-sized, and no large-cardinal
axiom is used.
\end{proposition}
\begin{proof}
Take the union of the normal-form supports of the finitely many
coefficients and of any finite collection of arguments under discussion,
adjoin the finitely many scale exponents needed for normalization, and take
the divisible subgroup $\Gamma$ they generate: a set-sized subgroup of
$\No$ whose Hahn field contains the inputs, is algebraically closed, and is
closed under the finite coefficient recursions and the strong sums of
\cref{dyn:rd:subsec:support}. The conjugacy coefficients are uniquely
determined by \eqref{dyn:eq:recurrence}, so enlarging the workspace cannot
change them, and a strongly summable family has the same normal-form sum in
every larger workspace. The root count factors a finite polynomial in one
algebraically closed workspace, and those are all its roots in any larger
field. For an arbitrary further point, enlarge the workspace once more. No
proper class is treated as a set-sized field and no proper-class family is
summed. The set constructions use ordinary choice and set-based Hahn-field
theory; in a class language one may use the definable classes of a
conservative class theory over ZFC. No inaccessible cardinal or stronger
hypothesis enters.
\end{proof}

This is the equivariant case of \cref{dyn:thm:workspace} and
\cref{dyn:cor:surcomplex}, which source 11 re-derives independently and
extends to its periodic-point and omnific statements. The notation guide
and the report on set-sized quotients of omnific integers
(\texttt{docs/\allowbreak surreal/\allowbreak set-sized-quotients-of-\allowbreak omnific-integers/})
were
consulted by source 11 only to fix conventions and to avoid duplicating the
omnific algebraic programme; no theorem of either is used here.

\subsection{Finite certificates, and what source 11 does not settle}
\label{dyn:rd:subsec:limits}

For explicitly represented finite Hahn inputs the theorems give a short
calculation pipeline. It is an algebraic procedure \emph{conditional on
extracting leading valuations and leading coefficients}, not a decision
procedure for arbitrary unnamed surreal numbers. Compute
$\delta=v(\lambda-\zeta)$ and every nonzero $v(a_{1+jq})$, and take the
finite maximum \eqref{dyn:rd:eq:req} in the ordered group. Normalize
$\hat a_j$ and keep exactly their residues. The polynomial $R$ then gives
three separate outputs: the shell divisor, the recurrence
\eqref{dyn:rd:eq:betarec}, and the algebraicity test
\eqref{dyn:rd:eq:alpha}. The support certificate is the finite union of the
positive supports of $e,\hat a_1,\dots,\hat a_J$, closed under the monoid
construction. If the coefficients of $R$ are algebraic numbers,
squarefreeness and rationality of the finitely many algebraic numbers
$1/(q\beta R'(\beta))$ are exact algebraic questions. For complex
coefficients given by an oracle or by non-effective descriptions no uniform
decidability follows, and checking that an infinite support is well ordered
is a separate matter from manipulating a finite support expression.

\begin{proposition}[Finite leading-data certificate; source 11]
\label{dyn:rd:prop:certificate}
Given valid leading data for \eqref{dyn:rd:eq:family}, the scale
$r_{\mathrm{eq}}$, the residue polynomial $R$, and either a repeated root of
$R$ or a root $\beta$ with $1/(q\beta R'(\beta))\notin\Q$ certify that $H$
is transcendental over $K_\Gamma(z)$. No higher Hahn coefficient of $H$ is
needed.
\end{proposition}
\begin{proof}
The leading data determine $r_{\mathrm{eq}}$ and $R$ exactly, and
\cref{dyn:rd:thm:algebraicity} contradicts any algebraic relation for $H$.
Higher coefficients play no role.
\end{proof}

This is the transcendence counterpart of the maximality certificate
\cref{dyn:cor:finite-certificate} and of the procedure of
\cref{dyn:app:certificate}. For equivariant input the return iterate is not
needed, because \cref{dyn:rd:prop:return} reads $P=R(X^{q})$ off $f$.

Source 11 sketches a formalization sequence and supplies no Lean file. It
would start in an abstract algebraically closed valued field, with
\cref{dyn:rd:lem:iterate}, \cref{dyn:rd:thm:allperiods} and the root count,
and handle the formal coefficients and \eqref{dyn:eq:recurrence} separately
over a field with a non-torsion multiplier. The Hahn-specific step is
$\OO_{\mathsf M}$ with \cref{dyn:rd:lem:evaluation}, including the
finite-word bound needed for reassociation; normal-form embeddings would
then transport the fixed-workspace theorem to the surreal carrier. This is a
plan, not an implementation. The repository's
\texttt{Surreal/HahnSeries/Neumann.lean} proves the finite-word support
lemma, but existing formal verification of the coefficient field does not
verify any statement of these subsections. The limitations are
\ref{dyn:rd:nc:status}--\ref{dyn:rd:nc:last} of
\cref{dyn:rd:subsec:nonclaims}, and the eleven questions source 11 raises
are \cref{dyn:rd:q:symmetry} to \cref{dyn:rd:q:effective}.
...
source 07 classifies\\[3pt]
common-domain germs, the sixth category (second table of
\cref{dyn:rem:oneinvariant})
& $\tau<\infty$, radius $Re^{-\tau}$, for $\lambda I_d$
 (\cref{dyn:esm:thm:main}); $\tau<\infty$, radius at least
 $Re^{-\rang\tau}$, for nonscalar $\Lambda$ (\cref{dyn:nsc:thm:main});
 $\sigma=0$ under
 drift (\cref{dyn:thm:trichotomy})
...
same refusal elsewhere in this report. For what the strong and the
coefficientwise notions of Hahn countable additivity give here, see
\texttt{meas:rem:dynamics} in
...
\subsection{The quadratic Schr\"oder recursion, once, in six normalizations}
\label{dyn:subsec:quadratic}

Four of sources 01--07 compute the coefficients of the normalized formal
linearizer of an ordinary quadratic, and sources 08 and 09 compute them
again (below). They are one calculation; the translation between
...
Sources 08 and 09 compute the same recursion twice more. Source 08 works in
this report's convention, with $F=\lambda z+t^{\eta}z^{2}$ and
$H=z+\sum_mt^{m\eta}u_m$, and finds
\[
 u_1=-\frac{z^{2}}{\lambda^{2}-\lambda},\qquad
 u_2=\frac{2\lambda z^{3}}{(\lambda^{2}-\lambda)(\lambda^{3}-\lambda)},\qquad
 u_3=-\frac{(5\lambda^{3}+\lambda)z^{4}}
 {(\lambda^{2}-\lambda)(\lambda^{3}-\lambda)(\lambda^{4}-\lambda)},
\]
which is $u_m=b_mz^{m+1}$ for the first three $b_m$ of
\eqref{dyn:eq:quadratic-bs}. Source 09 works in the forward convention of
\cref{dyn:esm:conv}(iii). Its coordinate
$\check H=z+\sum_{m\ge1}c_mt^{m\eta}z^{m+1}$ is the inverse of
\eqref{dyn:eq:scaled-H}, so its $c_m$ are the coefficients of the
compositional inverse of $L_\lambda$, with $c_0=1$ and
$c_m=\sum_{r+s=m-1}c_rc_s/(\lambda^{m+1}-\lambda)$. Thus
$c_1=1/(\lambda^{2}-\lambda)=-b_1$, $c_2=2c_1/(\lambda^{3}-\lambda)$ and
$c_3=(2c_2+c_1^{2})/(\lambda^{4}-\lambda)$. Neither source asserts any
estimate for the growth of these scalars in $m$. Source 09 adds that at this
input every tree vertex has weight one. The fixed-complexity factor of
\cref{dyn:esm:thm:treebound} then becomes $n^{n}$, and no ordinary
convergence follows (\cref{dyn:esm:rem:uniformity}).
...
coordinate-versus-surreal-derivation distinction ten times.
...
material. All eleven sources restate that discipline and all eleven
state the support lemma. Sources 02, 03, 05 and 09 each reproduce a
minimal-bad-sequence Higman proof, and source 08 reduces the lemma to
Poonen's positive-support theorem.
...
\cref{dyn:def:summable}. The other statements, one per source, are
deleted. This is
...
The eleven verification programs are preserved unmodified in \texttt{code/}
...
any infinite theorem is claimed.\\[4pt]
08 & --- &
No single total; the counts are different units and are not added. Exact
arithmetic in $\Q(i)$ with SymPy~1.14 and the test multiplier
$\lambda=(3+4i)/5$, which is unimodular and not a root of unity. Two
independent finite constructions --- the positive-parameter operator
recursion and a direct solve in increasing total coordinate degree --- are
compared, and the conjugacy identity and both inverse identities are
checked. One variable at parameter degree $4$ and coordinate degree $11$
($7$ component identities). Two variables with mixed monomials at parameter
degree $3$ and coordinate degree $7$ ($14$ component identities). The
cyclic drifting construction \eqref{dyn:cyc:eq:input} and its inverse at
parameter degree $5$ and coordinate degree $13$, with the fold $u=v=t$
keeping every colliding contribution ($6$ polynomial identities).
& Does not prove holomorphy on a common domain or strong summability over
arbitrary ordered groups. The test multiplier is a convenient exact value,
not an approximation to a positive-rate example.\\[4pt]
08, cont. & &
The first nonlinear parameter block against its closed reciprocal formula
($60$ exact scalar equalities). Divisible-subtree packing on $8{,}566$
tree--weight configurations (sizes $1$--$6$, local weights $1$ or $2$,
$q=1,\dots,10$; $85{,}660$ inequalities). Strict-chain packing on
$14{,}892$ chains (total weight $1$--$16$, length at most $6$;
$119{,}136$ inequalities).
& Finite ranges only. Does not prove the asymptotic small-divisor rate, the
existence of irrationals of prescribed rate, or noncancellation along an
infinite convergent subsequence. No novelty, no peer review, no Lean.\\[4pt]
09 & --- &
No single total; status \texttt{PASS}. Standard library,
\texttt{fractions.Fraction}. Packing: all plane rooted skeletons with at
most $5$ vertices and positive weights of total at most $11$ ($8{,}514$
weighted skeletons), every relevant $q$: $83{,}996$ instances of
\cref{dyn:esm:lem:packing}, each with an independent construction of the
disjoint blocks; $45$ aggregate plane-tree count bounds; the zero-weight
counterexample. Clustering: $33{,}030$ pairs of a reduced rational test
value of $\theta$ (denominator at most $60$) and a total weight $n\le30$,
$7{,}075$ of them with a nonempty marked set, checking the determinant,
divisibility and proportional-error identities of
\cref{dyn:esm:lem:cluster}.
& The rational tests infer nothing about an irrational limsup. Does not
prove the support lemma, the general packing inequality, or any
small-divisor rate.\\[4pt]
09, cont. & &
Formal algebra: six exact rational cases (multipliers $2$, $2/3$, $-2$;
one-label geometric data and two colliding integer labels) through
parameter order $4$ and Taylor degree $8$. They compare three independently
organized constructions: Taylor/parameter recursion for $\check H$, the
plane-skeleton formula \eqref{dyn:tree:eq:formula} with explicit slot
multiplicities, and an operator geometric expansion for $H$. That gives
$270$ coefficient comparisons, $2{,}478$ tree--label--weight terms and $36$
groups of identities, including both inverse identities and both
conventions.
& The test multipliers are \emph{not} on the unit circle. They check tree
factors, signs, multiplicities and composition only, and the unit-circle
radius estimate is not tested. Does not prove convergence on a disk,
workspace invariance, or novelty. No Lean code.\\[4pt]
10 & --- &
No single total; status \texttt{PASS}. Standard library only; exact
$\Q(i)$ arithmetic by pairs of \texttt{fractions.Fraction}. Contraction
lemma \cref{dyn:nsc:lem:blocks}: all $2{,}730$ recursively labelled colored
internal-tree configurations with at most three internal vertices, arities
two or three, two colors, every leaf-color count and both output colors,
and every nonempty marked subset, giving $18{,}466$ checks of the
nonnegative contracted leaf vector, the character identity, the degree
identity, disjointness of blocks and the degree budget
\eqref{dyn:nsc:eq:blockfacts}; and $1{,}000$ pseudorandom configurations
with four to nine internal vertices from a fixed seed. Algebra: the
nonscalar unit multipliers $(3+4i)/5$ and $(5+12i)/13$ with
$F_1=\tfrac{3+4i}5x+t(x^{2}+2xy)+t^{2}y^{3}$,
$F_2=\tfrac{5+12i}{13}y+t(x^{2}-y^{2})+t^{2}x^{2}y$; the forward coordinate
by one recursion and the inverse by an independently organized residual
recursion, through parameter degree $3$ and total spatial degree $6$; both
component conjugacy identities and both two-sided inverse identities ($8$
component identities), and the $21$ nonzero divisors used by the forward
recursion. Each coordinate has $23$ nonzero monomials at this truncation.
& Detects transcription, sign, color, multiplicity and truncation errors
only. Does not establish \cref{dyn:nsc:lem:height}, the limsup of
\cref{dyn:nsc:thm:products}, analytic convergence, arbitrary-support
summability or originality. The random configurations are samples, not an
enumeration; nonresonance of the test multipliers is checked only in the
finite range used. No Lean code.\\[4pt]
11 & 1{,}863 &
Six groups, exact SymPy~1.14 arithmetic. Formal conjugacy ($173$): the
Schr\"oder recurrence and both inverse identities for five equivariant
polynomials with non-unimodular test multipliers $2,3,-2,-3/2,2i$ and
$q=1,2,4$, including the vanishing \eqref{dyn:rd:eq:hsymmetry}. Parameter
reduction ($107$): with a symbolic displacement $E$, regularity of every
normalized coefficient at $E=0$, its limit against the boundary series
\eqref{dyn:rd:eq:betarec}, and $\varphi_k(0)=-kq$. Boundary differential
($325$): the recurrence against $R(\mathfrak h+qU\mathfrak h')=\mathfrak h$
for $q\le5$, and the logarithmic derivatives of
\cref{dyn:rd:ex:binomial,dyn:rd:ex:quadraticR,dyn:rd:ex:repeated,dyn:rd:ex:tworoots}.
Iterate quotient ($38$): \eqref{dyn:rd:eq:iteratediv} and its constant
term. Shell and reciprocal ($24$): equivariance, rotation roots and
\eqref{dyn:rd:eq:omnificG} for $N=2$, rational two-cycle samples, and the
factorization of \cref{dyn:rd:ex:quintic}. Lexicographic scales ($1{,}196$):
$80$ seeded random trials in $\Q^{2}$ of the threshold identity
$r_{\mathrm{eq}}=r_{\max}$ and positivity of the clearing exponents, plus
$40$ samples of \cref{dyn:rd:ex:collision}.
& Finite regression checks only. Does not verify arbitrary-rank strong
summability, proper-class compatibility, all-period exclusion as a universal
theorem, or novelty. The rational specialization of the shell test is not a
model of infinitesimal valuations. The scale test models the clearing
exponent by adding $(0,1)$, not $1$. No Lean.\\
\bottomrule
\end{longtable}
...
\begin{enumerate}[label=(\arabic*)]
\item \textbf{All eleven suites pass.} Run on Python 3.14.4 with SymPy 1.14.0
where SymPy is used, every one exits $0$; sources 08 and 09 were run, on a
copy, when they were integrated, and so was source 11 (item~(10)). Reported totals where a script
...
\item \textbf{Five of the eleven scripts rewrite their own evidence}
(source 11's is described in item~(10)). Source
...
\item \textbf{Sources 08 and 09 reproduce and do not overwrite.} Re-run on a
copy with Python 3.14.4 and SymPy 1.14.0, source 08's program produced a
record that agrees with
\texttt{data/\allowbreak 08-exact-and-drifting-multipliers-verification.json}
in every field except the elapsed time and the Python version; the shipped
run used Python 3.13.5. Source 09's regenerated record agrees with
\texttt{data/\allowbreak 09-single-loss-linearization-verification.json}
field for field. Both scripts are overwrite-safe. Without
\texttt{-{}-output} they print only, and they refuse an existing output
path: source 08 unless \texttt{-{}-overwrite} is given, source 09 always.
Source 08's docstring names the pre-placement path
\texttt{data/verification.json}. Source 09's delivered record sat at its
package root, and its \texttt{Makefile} calls the script by its delivered
name \texttt{verify.py}. The placed
\texttt{code/\allowbreak 08-exact-and-drifting-multipliers-build.sh} and the
\texttt{pdf} target of
\texttt{code/\allowbreak 09-single-loss-linearization-Makefile} build the
two standalone source manuscripts, which are not shipped in this directory.
Neither package ships or promises a checksum manifest.
\item \textbf{Source 10 reproduces and does not overwrite.} Its script
\texttt{code/\allowbreak 10-nonscalar-\allowbreak common-domain-\allowbreak verify.py} takes no
arguments, writes no file and prints ten lines to standard output. Run on a
copy with Python 3.14.4 it exits $0$ in under two seconds, and its output
agrees line for line with
\texttt{data/\allowbreak 10-nonscalar-\allowbreak common-domain-\allowbreak verification\_\allowbreak results.txt}
(apart from the platform's line endings). The counts quoted in the table
above are the ones printed. The delivered package also held the source
manuscript, its PDF, a README and a checksum manifest; the manifest was
checked against the delivered files, all six entries matching, and none of
those four files is shipped here. Its provenance note is shipped as
\texttt{10-nonscalar-\allowbreak common-domain-\allowbreak SOURCES.md}; it describes this report as
it stood at source 10's pin, before the nonscalar subsections were added.
\item \textbf{Source 11 reproduces, and rewrites its own record by default.}
Its script \texttt{code/\allowbreak 11-resonant-dynamics-verify.py} writes
\texttt{verification.json} and \texttt{verification.txt} into
\texttt{-{}-output-dir}, whose default is the directory containing the
script. In the delivered layout that overwrote its record. In this tree an
argument-free run writes the two files into \texttt{code/}, beside the
script, and leaves the shipped \texttt{data/} records untouched. Run on a
copy with \texttt{-{}-output-dir} and Python 3.14.4, SymPy 1.14.0, it exits
$0$ with status \texttt{PASS} and the recorded group counts
$173/107/325/38/24/1{,}196$, total $1{,}863$. The regenerated JSON agrees
with \texttt{data/\allowbreak 11-resonant-dynamics-verification.json} in
every field except the Python version (the shipped run used 3.13.5), the
run time and the elapsed seconds; the text record differs only in the
elapsed seconds. The shipped \texttt{code/\allowbreak
11-resonant-dynamics-build.sh} and \texttt{-build.ps1} rerun the suite and
then rebuild the source manuscript. They call \texttt{verify.py} and
\texttt{article.tex} by their delivered names, and the source manuscript is
not shipped here, so they do not run in this tree as shipped. Its build
report records a 23-page PDF with no warnings and states that the
PowerShell script was not executed. The package's manuscript, PDF and
README are not shipped; its source and proof audits are shipped as
\texttt{11-resonant-dynamics-SOURCE\_AUDIT.md} and
\texttt{11-resonant-dynamics-PROOF\_AUDIT.md}, and describe the source as
delivered, with delivery file names.
\end{enumerate}
...
\end{verbatim}
Reproducing the eleven suites, each \emph{on a copy}, requires Python 3.10
or later and, for six of them, SymPy; the pinned versions recorded by the
sources are \texttt{sympy==1.14.0}. Source 10's script needs only the
standard library of Python 3.9 or later.

\section{Open questions}
\label{dyn:sec:questions}

The questions below are exactly those the sources leave open, with the
categories in which they are open stated explicitly. Sources 08 and 09
settle part of two of them, \cref{dyn:q:commongerm-restate,dyn:q:cyclic}.
Both are kept, re-scoped, with the settled part and its source recorded.
The two sources add no new question; their remaining directions are folded
into \cref{dyn:q:commongerm-restate,dyn:q:maximal,dyn:q:realization}.
Source 10 settles most of what sources 08 and 09 left of
\cref{dyn:q:commongerm-restate}; the question is kept once more, re-scoped
to what remains. Source 10 adds no new question either; its remaining
directions are folded into \cref{dyn:q:commongerm-restate,dyn:q:realization}.
Source 11 answers none of these questions and adds eleven of its own,
printed after them as \cref{dyn:rd:q:symmetry} to
\cref{dyn:rd:q:effective}; this report bears on three of them, as their
status notes record.

\begin{question}[Common-domain germs at a finite positive rate: the optimal
radius for a nonscalar multiplier of angular rank at least two]\label{dyn:q:commongerm-restate}
This is \cref{dyn:q:commongerm}, and \cref{dyn:warn:resolution} is its
status. It is settled negatively in the \emph{drifted} category $\sddr$ by
\cref{dyn:thm:collapse,dyn:thm:trichotomy}, and over $\Z$ as well by
\cref{dyn:cyc:thm}. In the exact-multiplier category $\sdex$ it is settled
positively for a scalar $\Lambda=\lambda I_d$ by \cref{dyn:esm:thm:main}
(sources 08 and 09). As recorded at their pins, it was \textbf{open} for a
nonscalar diagonal unitary $\Lambda$. Source 03's remark applied there: an
exact characterization would need to control interactions across unbounded
word depths rather than a single homological inverse. For a scalar
multiplier that control is \cref{dyn:esm:thm:treebound}. For a nonscalar one
the divisors $\lambda^{\beta}-\lambda_j$ are not indexed by a single
integer, and what was missing was a vector analogue of the tree bound, or a
counterexample to one (\cref{dyn:esm:subsec:limits}). At the pins of
sources 01--07 the whole exact-multiplier case was open.

\emph{Status.} Source 10 supplies a vector analogue with loss
$\min(\rang,m)\tau$ at complexity $m$ (\cref{dyn:nsc:thm:products}) and
settles the question positively for every nonscalar diagonal unitary
$\Lambda$: when $\tau<\infty$, every input has a common-domain linearizer,
of radius at least $Re^{-\rang\tau}$ (\cref{dyn:nsc:thm:main}), and when
$\tau=\infty$ not every input does (\cref{dyn:nsc:thm:criterion}). The
largest universal radius $R_{\mathrm{univ}}$ satisfies
$Re^{-\rang\tau}\le R_{\mathrm{univ}}\le Re^{-\tau}$ and equals $Re^{-\tau}$
when $\rang=1$. Still \textbf{open}: the value of $R_{\mathrm{univ}}$ when
$\rang\ge2$ and $0<\tau<\infty$. A sharper accounting might reduce the
factor $\rang$; a matching lower example would have to show several
exponential rates saturating their charges at once, and surviving the
cancellations of an actual conjugacy, which large individual tree factors do
not give (\cref{dyn:nsc:subsec:limits}). Outside the question as posed,
source 10 also leaves drifting multipliers in several variables, resonant
spectra, Jordan blocks, nonunitary spectra and infinite coordinate
dimension untreated.
\end{question}

\begin{question}[The fixed cyclic value group; \emph{settled} in one
variable]\label{dyn:q:cyclic}
As recorded at the pins of sources 01--07: the negative direction of
\cref{dyn:thm:collapse} is established over
...
\emph{Status.} Source 08 settles the question, negatively, in the
one-variable setting in which it is posed. Over $\Z$, with the single Hahn
monomial of \eqref{dyn:cyc:eq:input}, the colliding blocks cannot cancel the
highest pole (\cref{dyn:cyc:prop:noncancel}), and the coefficient radii are
exactly $Re^{-m\sigma}$ (\cref{dyn:cyc:thm}). The universal common-domain
property holds over any fixed nonzero $\Gamma$ exactly when $\sigma=0$
(\cref{dyn:cyc:cor:classification}). Still \textbf{open} is the
several-variable multiplicative-detuning collapse \cref{dyn:thm:multicollapse}
over a fixed cyclic group. Source 08 does not address it, and it remains
asserted only over $\Gamma_{*}$.
\end{question}
...
a first-order divisibility test alone need not be enough. Source 08 poses
the exact-multiplier form of the same question. The radius $Re^{-\tau}$ of
\cref{dyn:esm:thm:main} is a universal guarantee, not a formula for each
input, and polynomial inputs already do much better. Determining
input-specific improvements from vanishing coefficients or algebraic
structure is a separate question, and any criterion must survive the
nonlinear degree interactions of the conjugacy equation.
\end{question}
...
starting point. Sources 08 and 09 add the depth dimension. Their estimates
\cref{dyn:esm:prop:coefbound,dyn:tree:prop:estimate} let the constants
depend arbitrarily on the support word, so they give no uniform-depth or
Gevrey bounds across depth, and no joint analytic convergence in a complex
parameter and the coordinate. Source 09 locates the missing information in
the constants $W_{\gamma,m}$ and $B_\gamma(r)$ and in the polynomial factors
of \eqref{dyn:tree:eq:estimate}. Restrictions on them could support Gevrey
or other weighted estimates, but none follows from support finiteness alone.
In the nonscalar case the prefactor $C_{m,\eps,\Lambda}$ of
\cref{dyn:nsc:thm:products} is not even explicit, and no bound uniform in
$m$ is available (source 10).
\end{question}
...
\subsection*{Questions raised by source 11}
\addcontentsline{toc}{subsection}{Questions raised by source 11}

Source 11 (\cref{dyn:rd:subsec}) raises the eleven questions below and
presents them as a research programme arising from its results, not as
historically established open problems. They are printed in source 11's
order and in the notation of \cref{dyn:rd:conv}. Each carries a
\emph{Status (batch 34)} note, added in the merge, where this report already
bears on it. Source 11 answers none of the questions above.

\begin{question}[Removing exact rotational symmetry]\label{dyn:rd:q:symmetry}
For a general polynomial with $\res(\lambda)$ of order $q$, can a finite
sequence of nonresonant coordinate changes reduce the sharp linearization
problem to a computable resonant boundary invariant? Which support
conditions survive that normalization uniformly on the candidate maximal
ball? Formal solvability alone is not the missing step: nonresonant changes
can alter the scale of the first surviving resonance, and the resulting
infinite normal form may lose the finite support certificate, so an exact
threshold formula must keep both effects.

\emph{Status (batch 34).} \mergetag\ Settled in substance at source 11's
own pin, by a different route. For every polynomial under $\resq$,
\cref{dyn:thm:exact} gives the exact threshold from the finite $q$-th
return, with a uniform support certificate (\cref{dyn:lem:displacement}).
\Cref{dyn:thm:shell} supplies the computable boundary invariant $P$, and
\cref{dyn:thm:coherent} the boundary vector field $XP(X)\partial_X$, whose
leading coordinate is decided by the criterion of
\cref{dyn:rd:rem:parabolic}. Still open: the route the question proposes,
through nonresonant coordinate changes instead of the return; the
closed-ball exclusion of all other periods (\cref{dyn:rd:prop:periodfinite})
for non-equivariant maps; and infinite power series, which
\textbf{N18} excludes.
\end{question}

\begin{question}[A sharp criterion for infinite coefficient families]
\label{dyn:rd:q:infinite}
Characterize the sequences $(h_n)$ in $\OO_\Gamma$ for which
$\sum_nh_nu^{n}$ is strongly summable at every $u\in\m_\Gamma$. How far can
the sufficient common-$\OO_{\mathsf M}$ criterion be weakened?
\Cref{dyn:rd:ex:collision} rules out any answer based only on the
inequalities $v(h_n)\ge0$, and \cref{dyn:ex:bad-support} rules out an
answer based only on positive valuations. An answer in terms of supports
rather than a chosen presentation would extend the polynomial theorems to
much larger classes of germs.
\end{question}

\begin{question}[Higher boundary obstructions to algebraicity]
\label{dyn:rd:q:higher}
When $R$ passes the exact test \eqref{dyn:rd:eq:alpha} for algebraicity of
$\bar H_r$, can the next nonzero Hahn layer produce a second effective
obstruction to algebraicity of $H$? Is there a useful finite hierarchy for
algebraic input polynomials? The reduction test is deliberately one-sided
upstairs, and a hierarchy would separate higher-order obstructions from
those already visible in the rational logarithmic derivative of the first
boundary field.

\emph{Status (batch 34).} \mergetag\ Answered positively at the first
layer when $e$ is a constant times a monomial and every normalized
coefficient is an ordinary constant, for every $q\ge1$
(\cref{dyn:rd:rem:superseded}). There $H$ is always transcendental, by
\cref{dyn:thm:dichotomy}, and when $\bar H_r$ is algebraic the coefficient
of $t^{\delta}$ is already transcendental, by
\cref{dyn:prop:leadingalgebraic}. Open for Hahn-valued displacement or
normalized coefficients, and the finite hierarchy is open everywhere.
\end{question}

\begin{question}[Splitting multiple boundary cycles]
\label{dyn:rd:q:splitting}
When $R$ has repeated roots, can the successive scale structure of
$\widehat R(U)$ near those roots be organized into a canonical finite rooted
tree of periodic clusters, compatible with the linearizer's higher
reductions? Root splitting at finer valuations is accessible to algebraic
methods. The additional problem is to relate it to the singular exponential
factor of \cref{dyn:rd:ex:repeated}. \mergetag\ The same degeneracy for a
non-equivariant return is \cref{dyn:rem:multiplicity} and
\cref{dyn:ex:kappa}, where the finer splitting is likewise left to higher
Hahn terms.
\end{question}

\begin{question}[Real domains versus surcomplex balls]\label{dyn:rd:q:real}
For a real equivariant polynomial whose complex shell cycles are all
nonreal, determine the maximal real centered domain of an injective strong
linearizer. Which real algebraic feature replaces the complex periodic
obstruction? \Cref{dyn:rd:ex:quadraticR} and the variant in
\cref{dyn:rd:ex:quintic} show why the complex sharpness proof does not
settle this: even in a real closed field a complex obstruction need not be a
real periodic point.
\end{question}

\begin{question}[Rational and algebraic germs]\label{dyn:rd:q:rational}
Extend the exact threshold and the boundary reduction to rational or
algebraic germs whose denominators remain units on the closed normalized
ball. Can a finite algebraic presentation replace the finite polynomial
coefficient list in the support argument? Unit denominators are promising
because their geometric inverses stay in the same support monoid, but a
proof must still handle the infinite coefficient family and the periodic
equation at the shell. \mergetag\ This is the infinite-series exclusion
\textbf{N18} of source 01, asked for a specific class.
\end{question}

\begin{question}[Several variables and simultaneous resonances]
\label{dyn:rd:q:several}
For maps on $K_\Gamma^{n}$ with a finite-order residue linear part, identify
a several-variable counterpart of the field $XR(X^{q})\partial_X$ and a
domain on which strong conjugacy holds. What geometric objects replace the
scalar periodic divisor? In several variables resonant monomials need not
be ordered by one degree, and cancelling a single factor $e$ may no longer
control every small divisor. \mergetag\ Compare the several-variable part
of \cref{dyn:q:moving}.
\end{question}

\begin{question}[Beyond finite periodic-shell prescriptions]
\label{dyn:rd:q:shells}
Which infinite sets of positive surreal scales can be realized as exactly
the infinitesimal periodic shells of a strongly analytic map with a fixed
multiplier residue? What degree or support restrictions force finiteness?
\Cref{dyn:rd:thm:allshells} is finite by construction; an infinite version
needs a legitimate infinite product and a proof excluding extra periods, not
a formal interpolation of the desired points.
\end{question}

\begin{question}[Actual omnific self-maps]\label{dyn:rd:q:omnific}
Which polynomial or rational maps preserve $\Oz$ or $\Oz[i]$ on a natural
invariant domain while realizing specified infinite cycles? Can denominator
clearing be replaced by a global integrality theorem?
\Cref{dyn:rd:thm:omnific} gives an omnific coefficient presentation and an
integral periodic set, and deliberately not a self-map of the whole ring;
the distinction poses an arithmetic problem, not a notational one.
\end{question}

\begin{question}[Continuation away from the obstructing cycles]
\label{dyn:rd:q:continuation}
Can the normalized linearizer be continued strongly through parts of the
shell once the periodic divisor is removed? How does the continuation
depend on the rational residues $\mathfrak r_\beta$? The product formula
\eqref{dyn:rd:eq:h0product} suggests ramification data and repeated roots
suggest exponential singularities; this analogy is a source of conjectures,
not a proof of global continuation over the surcomplex class.

\emph{Status (batch 34).} \mergetag\ Partly addressed in two special cases,
in other senses of ``continued''. \Cref{dyn:thm:coherent} continues the
exponent-zero coefficient $H_0$, and every $H_\gamma$, over one ordinary
disk on which $P$ has no zero. In the parabolic model
(\cref{dyn:rd:rem:scope}), \cref{dyn:subsec:boundary} evaluates the
coefficientwise continuation at the rescaled shell, where the Taylor family
fails, and \cref{dyn:thm:monodromy,dyn:thm:modelfactor} show that the
positive part of the exponent survives every finite ramification. Strong
continuation of the evaluated conjugacy through shell points, in the sense
asked, is open.
\end{question}

\begin{question}[Effective and formal verification]\label{dyn:rd:q:effective}
Develop a certified implementation that accepts finite Hahn data, produces
the threshold and the boundary polynomial, and verifies the transcendence
certificate. Can the common-support argument be expressed as a reusable
formal-library interface for strong power-series substitution? That would be
more reusable than checking isolated examples, since the support interface
is the obstruction met in many higher-rank analytic constructions.
\mergetag\ Compare \cref{dyn:app:certificate} and \textbf{N64}; the
repository's Lean support lemma is described in \cref{dyn:sec:verification}.
\end{question}
...
\cref{dyn:conv:merge}(iv). It carries \textbf{one hundred and forty-seven}
distinct limitations stated by the eleven sources --- eleven from source 01,
nine from source 02, ten from source 03, fourteen from source 04, twelve
from source 05, twelve from source 06, ten from source 07, sixteen from
source 08, twelve from source 09, seventeen from source 10 and twenty-four
from source 11 --- with the originating source named in each case. None has been merged away, softened, or absorbed
...
Valuation-expanding periodic points of polynomials $q^{-1}P$ with $v(q)>0$
are studied in the report
\path{docs/surcomplex/expanding-polynomial-dynamics/}; their multipliers are
Hahn elements of negative valuation, not ordinary repelling complex
multipliers, and that report linearizes nothing
(\texttt{epd:prop:multiplier}, \texttt{epd:rem:expansion}).
...
A global symbolic description of a different class of maps, polynomials
$q^{-1}P$ with $v(q)>0$ and simple split reduction, is in
\path{docs/surcomplex/expanding-polynomial-dynamics/}
(\texttt{epd:thm:fibers}, \texttt{epd:thm:topology}). It is not a Julia or
Fatou theory either (\texttt{epd:cor:empty-affine}), and nothing there bears
on the germ linearization of this report.
...
cyclic value group (\cref{dyn:warn:drift,dyn:q:cyclic}). Both are source
02's limitations and are kept as such. Source 08 later supplied the
one-variable fixed-cyclic necessity, still with drift (\cref{dyn:cyc:thm}).
Sources 08 and 09 settled the scalar fixed-multiplier case positively
(\cref{dyn:esm:thm:main}), and source 10 the nonscalar diagonal one, with
radius at least $Re^{-\rang\tau}$ (\cref{dyn:nsc:thm:main}).
...
prove that radius collapse occurs (\cref{dyn:warn:lowerbound}). This is
source 03's limitation and is kept as such. The regime has since been
settled for a scalar multiplier by sources 08 and 09
(\cref{dyn:esm:thm:main}), and for a nonscalar diagonal one by source 10
(\cref{dyn:nsc:thm:main}); only the optimal radius when $\rang\ge2$ and $0<\tau<\infty$ remains
open (\cref{dyn:q:commongerm}).
...
arbitrary subsets and \textbf{not} a real-time ergodic theorem. The report
\path{docs/surreal/hahn-valued-measures-and-probability/} shows that in the
strong sense no invariant probability exists for an ordinary action all of
whose orbits are infinite (\texttt{meas:thm:orbits}), and discusses this
density there (\texttt{meas:rem:dynamics}).
...
\subsection{Sources 08 and 09: exact and drifting multipliers}
\label{dyn:esm:subsec:nonclaims}
Source 08 states sixteen limitations and source 09 twelve. Where they
overlap (status, priority, ordinary specialization, the nonscalar case),
both wordings are kept, because source 09 states its own for one variable.
Source 09's scope limitation is re-scoped rather than dropped: it is the
limitation of source 09, and after the merge the scalar several-variable case
is covered by source 08. The nonscalar case, which both sources leave open,
is covered by source 10 except for the optimal radius when $\rang\ge2$ and $0<\tau<\infty$
(\cref{dyn:nsc:subsec:nonclaims}); \ref{dyn:esm:nc:scalar} and
\ref{dyn:esm:nc:onevar} are kept as sources 08 and 09 state them.
\begin{enumerate}[label=\textbf{N\arabic*.},ref=\textbf{N\arabic*},leftmargin=3.2em,start=81]
\item\label{dyn:esm:nc:scalar} (08) Only the scalar case $\lambda I_d$ of
the exact-multiplier question is proved. The general nonscalar diagonal case
is not resolved, and no assertion of success or failure is made there. The
rational-clustering proof does not apply as written to the divisors
$\lambda^{\beta}-\lambda_j$ (\cref{dyn:esm:subsec:limits}). \emph{Re-scoped
after source 10's merge}: the nonscalar case is \cref{dyn:nsc:thm:main},
proved by a different argument, and only its optimal radius for $\rang\ge2$
and $0<\tau<\infty$ is open (\ref{dyn:nsc:nc:gap}).
\item (08) The cyclic theorem is settled only in the one-dimensional drifting
setting in which the repository question was posed
(\cref{dyn:cyc:subsec}); the several-variable collapse over a fixed cyclic
group is not addressed.
\item\label{dyn:esm:nc:specialize} (08) Neither theorem gives ordinary
analytic convergence after $t^{s}$, or the cyclic parameter, is replaced by
a nonzero complex number. There is no general specialization map from
positive Hahn series to values at a nonzero complex parameter: after the
replacement, infinitely many contributions may enter one ordinary
coefficient and need not be summable.
\item (08) Holomorphy is asserted on the \emph{open} polydisk only. There is
no boundedness and no continuation across its boundary, and the constants
and polynomial exponents of \cref{dyn:esm:prop:coefbound} may grow without
bound as the Hahn weight varies.
\item (08) In the chain estimate the exponential constant is uniform, but
the polynomial factor ($n^{3r}$ in source 08's form) is not uniform in the
chain length. No estimate suitable for summing all depths in an ordinary
complex parameter is provided.
\item (08) Sharpness is for the \emph{coherent coefficient representation}
only. There is no claim that an individual map admits no continuation, or
that no differently defined conjugacy exists on a larger class of surreal
points.
\item (08) Common-domain collapse means failure of a specified analytic
representation on ordinary neighborhood halos. It does \emph{not} mean the
absence of every surcomplex linearization: evaluation on the monad survives
(\cref{dyn:prop:monad}). No globally Hahn-entire nonpolynomial map on all
of $\No[i]$ is constructed.
\item (08) The unequal-radius statement, with polydisk radii $R_je^{-\tau}$,
is a coordinate-rescaling corollary, not a claim about optimal arbitrary
Reinhardt domains.
\item (08) The results do not replace the classical Bruno--Yoccoz theory.
No growth estimate for the quadratic scalars $u_m$ as $m\to\infty$ is
asserted. The bounds are deliberately not uniform in depth, and the
classical tree-expansion and Bruno estimates control a different
accumulation problem.
\item (08) All derivatives act on the ordinary coordinate functions and
annihilate the Hahn coefficient field. Nothing is claimed about the
Berarducci--Mantova derivation, the differentiation of $\omega$, or internal
surreal differential equations (\cref{dyn:warn:derivative}).
\item (08) The executable checks do not prove the asymptotic small-divisor
rate, the existence of irrationals of prescribed rate, holomorphy on a
common domain, strong summability over arbitrary ordered groups, or the
impossibility of cancellation along an infinite convergent subsequence.
They establish no novelty and replace no peer review, and no Lean or other
proof-assistant verification is included. The test multiplier $(3+4i)/5$ is
a convenient exact value, not an approximation to a positive-rate example.
The finite enumeration ranges are stated so that the record cannot be
mistaken for the universal assertions.
\item (08) $Re^{-\tau}$ is a universal guarantee, not a formula for every
particular input. Input-specific improvements are a different question.
\item (08) There is no uniform-depth, Gevrey, or joint analytic convergence
result in a complex parameter and the coordinate. The estimates allow the
constants to depend arbitrarily on a fixed support word.
\item\label{dyn:esm:nc:priority08} (08) Priority is not certified, and the
literature and repository comparisons are targeted. The repository was read
at the pinned commit through selected files and line ranges, not line by
line. \cite{FauvetMenousSauzin} was inspected only at abstract and metadata
level because its full text could not be retrieved, and \cite{Yoccoz} only
at metadata level. A prior theorem on formal families, a refinement of the
tree estimates, or an uninspected paper could subsume a specialization of
the results.
\item (08) The lower-bound proof of \cref{dyn:cyc:thm} claims no termwise
positivity. The error may contain many terms, and some of them may be large
(\cref{dyn:cyc:rem:excludes}).
\item (08) Status: proposed research results with proofs, not refereed or
proof-assistant verified, and not claims of bibliographic priority. The
open status of the two repository questions is relative to the pinned
revision, and neither is called a famous published conjecture.
\item\label{dyn:esm:nc:onevar} (09) One dynamical variable only; source 09
does not settle the several-variable case. ``Several'' in
\cref{dyn:esm:cor:parameters} refers to formal parameters, not dynamical
variables. Source 09 also states that the one-dimensional constant
$e^{-\tau}$ may not be imported unchanged into a nonscalar several-variable
problem. \emph{Re-scoped after the merge}: the scalar case $\lambda I_d$ in
every $d$ is source 08's \cref{dyn:esm:thm:main}, and the nonscalar case
remained open (\ref{dyn:esm:nc:scalar}) until source 10
(\cref{dyn:nsc:thm:main}), which gives the constant $e^{-\tau}$ when
$\rang=1$ and $e^{-\rang\tau}$ in general.
\item (09) The drifted multiplier and the fixed-cyclic-value-group question
are not covered. Source 09 neither reproves the drift result nor extends it
to the fixed cyclic setting; the latter is source 08's
\cref{dyn:cyc:thm}.
\item (09) Not covered: resonant multipliers, arbitrary infinite surcomplex
multipliers, mixed characteristic, moving base points, and nonidentity
ordinary nonlinear reductions.
\item (09) No maximal noncircular continuation domain, no universal boundary
regularity statement, and no classification of the inputs that happen to
linearize when $\tau=\infty$. The optimality witness proves sharpness of the
common \emph{centered Taylor disk} only. It does not claim a natural
boundary, or that continuation through every point of the convergence circle
is impossible.
\item (09) The formal support recursion is not claimed to be uniformly
computable from arbitrary presentations of $\Gamma$, arbitrary real
$\theta$ and arbitrary coefficient functions. The finite sets
$\word_S(\gamma)$ and the constants $C_a$ exist, but a finite algorithm
needs effective access to them or suitable certificates.
\item (09) $(C_an)^{m}$ is not harmless uniformly in $m$. For
$\lambda z+t^{\eta}z^{2}$ one has $m=n$ and the factor is $n^{n}$, so no
ordinary analytic convergence follows. Specialization of $t^{\eta}$ at a
nonzero complex number is not justified; for a non-Bruno $\lambda$ it
produces a divergent linearizer.
\item (09) There are no uniform coefficient-norm bounds: the constants and
polynomial degrees need not be uniform in the exponent. The halo statements
are coefficientwise. They assert no convergence of complex-parameter
specializations and no convergence of partial sums in the fine topology of
$\No[i]$.
\item (09) The tests use the multipliers $2$, $2/3$ and $-2$, which are not
on the unit circle. They check tree factors, signs, multiplicities and
composition formulas only, not the unit-circle radius estimate. The program
proves neither the support lemma, nor the general packing inequality, nor
convergence on a disk, nor any small-divisor rate, nor workspace
invariance, nor novelty. No Lean code is supplied. The four-layer Lean route
the source sketches is an implementation plan, not a completed
formalization, and no existing Lean declaration is certified to have the
interface it needs.
\item\label{dyn:esm:nc:priority09} (09) Novelty was checked by a targeted
comparison only, at the pinned commit and without a line-by-line audit of
the repository. The tree method is credited to classical tree expansions
\cite{FauvetMenousSauzin,FMS26}. A specialist may identify a theorem in the
parameter-dependent linearization literature that subsumes the result. The
draft is AI-assisted and not peer reviewed.
\item (09) ``Unique'' means unique among tangent-to-identity formal
coordinates or their coherent coefficientwise analytic realizations, not
among arbitrary set-theoretic maps.
\item\label{dyn:esm:nc:routes} (09) Its two routes share the essential
arithmetic lemma. They are not two independent proofs of it
(\cref{dyn:tree:subsec}).
\item (09) The result is not a claim that all small-divisor radius collapse
is illusory. It identifies the role of exactness in preventing collapse, in
one variable.
\end{enumerate}

\subsection{Source 10: the nonscalar diagonal multiplier}
\label{dyn:nsc:subsec:nonclaims}
Source 10 states seventeen limitations, collected from its title page,
body, README and provenance note. Where one overlaps a limitation of
source 08 or 09 (status, priority, ordinary specialization, uniformity in
depth), both wordings are kept, because source 10 states its own for a
nonscalar multiplier.
\begin{enumerate}[label=\textbf{N\arabic*.},ref=\textbf{N\arabic*},leftmargin=3.2em,start=109]
\item (10) Status: an AI-assisted, unrefereed research draft. Its new
arguments have not been checked in a proof assistant or independently peer
reviewed, and no Lean formalization, Wolfram verification or external peer
review was performed or is implied. A successful typesetting run verifies
document construction, not the mathematics.
\item (10) The question it answers is a specific question in an unpublished
repository report, this one (\cref{dyn:q:commongerm}), not a named published
conjecture.
\item\label{dyn:nsc:nc:priority} (10) Priority is not certified. The
literature comparison was targeted: the HTML text of the 13 September 2026
revision of \cite{FMS26} was inspected, especially its tree formulas,
admissible cuts and divisor estimates, while \cite{CarlettiLagrange} was
seen only through its abstract and metadata, with no full-text audit. The
rational-rank formulation was not located in that comparison. This is a
bounded novelty assessment, not a bibliographic proof, and an earlier
equivalent result under other terminology remains possible.
\item (10) The repository comparison is bounded: the document catalogue and
selected parts of this report's source at the pin (abstract and provenance,
support and halo machinery, the homological operator and category
classification, the depth-dependent radius theorem with its nonscalar
limitation, the scalar tree bound, and the verification discussion). It is
not an audit of every report, every Lean declaration, or every manuscript
kept only in Git history.
\item (10) Not claimed new: Hahn fields and the positive-support lemma
(Higman's theorem imported, Neumann credited), formal homological
inversion, rooted-tree methods and admissible-cut preservation
\cite{FMS26,CarlettiLagrange}, surreal normal forms \cite{Gonshor}, Cauchy
estimates, the root test, elementary continued fractions including the
prescribed-rate family, and the algebraic-number estimate of
\cref{dyn:nsc:ex:ranktwo}. The scalar common-domain theorem and the scalar
fixed-complexity rate are already in this report and are not claimed; the
general upper bound $Re^{-\tau}$ and the $\tau=\infty$ obstruction are
first-weight obstructions already here, included only to complete the
criterion.
\item\label{dyn:nsc:nc:gap} (10) For $\rang>1$ and $0<\tau<\infty$ the
factor $\rang$ is a
proved bound, not an optimality assertion. The optimal universal radius lies
between $Re^{-\rang\tau}$ and $Re^{-\tau}$ and is undetermined. The proof
does not show that several rate values can saturate their charges at once,
or that saturation survives the cancellations of an actual conjugacy. The
theorem answers the existence of a common radius, not the optimal general
nonscalar radius in this positive-rate regime. When $\tau=0$, both bounds
equal $R$, so the optimal radius is known at every angular rank.
\item (10) Holomorphy is asserted on the \emph{open} polydisk only; nothing is
asserted about continuity up to its boundary.
\item (10) Uniqueness is in the normalized formal positive-support class. No
classification of nonnormalized, non-Hahn-coherent or purely set-theoretic
conjugacies is given.
\item (10) The prefactor $C_{m,\eps,\Lambda}$ of \cref{dyn:nsc:thm:products}
comes from a subsequence argument: it is finite, but this argument supplies
no effective formula. No bound uniform in $m$ follows, and replacing $m$ by $n$ in
\eqref{dyn:nsc:eq:products} and dropping the prefactor is invalid.
\item (10) \Cref{dyn:nsc:lem:height} is stated for a fixed finite family
with linear heights. As explained after its proof, subexponential heights
suffice, and the distinct-rate count extends to a fixed infinite family by
applying the finite result to each finite subfamily. This gives neither a
uniform eventual rank bound for that infinite family nor a bound on a sum
over a growing number of divisors. Unrestricted exponential heights fail,
as the Fibonacci example there shows.
\item (10) ``Sharp'' concerns the largest universally guaranteed centered
isotropic polydisk for the ordinary coefficient functions. A particular
forcing may have much larger domains, and a coefficient may continue
analytically outside its centered Taylor polydisk; the isotropic radius is
not a statement about maximal analytic continuation.
\item (10) Not covered: infinitesimally drifting multipliers, resonant
spectra, nontrivial Jordan blocks, nonunitary spectra in analytic
categories, and infinite-dimensional coordinate spaces. The support method
is compatible with many of these, but its admissible-divisor and geometry
arguments do not transfer automatically.
\item (10) No ordinary analytic convergence is asserted after substituting a
nonzero complex number for $t$. A radius bound for each coefficient gives no
control of its size as the Hahn or parameter order grows. The result neither
replaces a Bruno-type theorem for ordinary germs nor contradicts the known
failures of classical analytic linearization.
\item (10) All derivatives are ordinary coordinate derivatives extended
coefficientwise; the Berarducci--Mantova derivation is not used. Strong
summation is an algebraic support operation: no convergence of partial sums
in the fine topology of $\No[i]$ is assumed, and nothing is asserted about
nonconstant continuous ordinary-time paths. The proof does not reconstruct
the proper class $\No$ as one workspace.
\item (10) The failure at $\tau=\infty$ is a failure of ordinary coefficient
holomorphy on a common domain. It is not a failure of formal conjugacy at
infinitesimal arguments, where a formal series of Taylor radius zero can
still be evaluated (\cref{dyn:prop:monad}).
\item (10) The finite checks detect transcription, sign, color, multiplicity
and truncation errors. They do not establish \cref{dyn:nsc:lem:height}, the
limsup of \cref{dyn:nsc:thm:products}, analytic convergence, summability for
arbitrary supports, or originality. The $1{,}000$ larger configurations are
reproducible samples, not an enumeration, and no global nonresonance or
asymptotic arithmetic claim is inferred from the finite algebra test.
\item (10) The new analytic theorem is not formalized in Lean. The existence
of related support lemmas in the repository's Lean code does not imply
formal verification of it, and source 10's three-interface formalization
outline is a plan, not an implementation.
\end{enumerate}

\noindent The list above contains $125$ numbered items for the $123$ distinct
...
$11/9/10/14/12/12/10/16/12/17$ across sources $01$ through $10$; the
corresponding numbered-item counts are $11/9/11/15/12/12/10/16/12/17$.
...
\subsection{Source 11: rotation-equivariant maps}
\label{dyn:rd:subsec:nonclaims}
Source 11 states the limitations below, collected from its title page,
abstract, introduction, remarks, program section, appendices, README, source
audit, proof audit, build report and recorded verification. Where one
overlaps an earlier limitation (status, priority, equal characteristic zero,
centered balls, finite checks), both wordings are kept, because source 11
states its own for the equivariant family. Items that the merge re-scopes
are annotated, not dropped.
\begin{enumerate}[label=\textbf{N\arabic*.},ref=\textbf{N\arabic*},leftmargin=3.2em,start=126]
\item\label{dyn:rd:nc:status} (11) Status: proposed research
contributions with written proofs, unrefereed and not machine-checked. The
PDF metadata and title page name the author as ``Research manuscript
prepared with ChatGPT''. Novelty is provisional, no resolution of a named
longstanding conjecture is asserted, and no independently certified
``breakthrough'' is claimed.
\item\label{dyn:rd:nc:priority} (11) Priority is not certified. ``Proposed
contribution'' means that an argument is supplied and that the exact
combined formulation was not identified in the sources checked. Targeted
searches for arbitrary-rank equivariant linearization and the combined
boundary-transcendence formulation found no directly matching primary
theorem, but some results were poorly targeted, so the search justifies no
universal absence claim. Independent comparison with the literature on
non-Archimedean dynamics, formal confluence and algebraic Schr\"oder
solutions is called appropriate before publication; the results stand or
fall on their hypotheses and proofs, not on the word ``new''. \emph{Stale
against this report:} the exact threshold and the shell count, for every
$\resq$ polynomial, were already \cref{dyn:thm:exact,dyn:thm:shell} at
source 11's pin (\cref{dyn:rd:prop:return}).
\item\label{dyn:rd:nc:repo} (11) The repository inspection was a targeted
guide and notation audit, not an exhaustive reading: the root and catalogue
READMEs, the notation guide \texttt{docs/NOTATION.md} and the analysis
guide \texttt{docs/\allowbreak surcomplex/\allowbreak analysis/} at the
pin, and the omnific-quotient README. A complete clone was not
available, and no source-code, Lean-build or full-manuscript review is
claimed. The main proofs use no unreviewed repository theorem.
\item (11) Credited, not claimed: Lindahl's rank-one linearization with its
lower and upper disk bounds, the formal Schr\"oder recursion and the
root-of-unity small-divisor dichotomy \cite[Thm.~1.1, Lemma~3.3]{Lindahl};
Hahn algebraic closedness \cite[Cor.~4]{Poonen}; Neumann's lemma
\cite[Lemma~2.36]{Marker}; polynomial iterate divisibility as an algebraic
mechanism; and the norm argument for algebraic logarithmic derivatives.
Source 11 is ``not the first proof of linearization near an indifferent
non-torsion fixed point in equal characteristic zero''.
\item (11) Theorem-level results for a specified family, not a solution of
general non-Archimedean linearization. Arbitrary \emph{non-equivariant}
polynomials are not covered, and exact rotational symmetry is essential to
the sharp theorem as proved. \emph{Re-scoped:} the threshold and shell for
non-equivariant polynomials are \cref{dyn:thm:exact,dyn:thm:shell}; the
closed-ball exclusion of all other periods is not extended to them
(\cref{dyn:rd:q:symmetry}).
\item (11) Residue characteristic $p$ is not covered; compare \textbf{N19}.
\item (11) Multipliers that are themselves roots of unity are not covered,
except by \cref{dyn:rd:thm:allperiods}, which allows $\lambda=\zeta$ and
classifies periodic points only.
\item (11) Unrestricted infinite coefficient families are not covered; the
finite polynomial input is essential to the support argument.
\item\label{dyn:rd:nc:oneway} (11) Algebraicity of the reduction is
\emph{not} claimed to imply algebraicity of the full conjugacy. The criterion
is an equivalence for $\bar H_r$ only; failure certifies transcendence of
$H$, and success does not prove it algebraic. \emph{Re-scoped:} in the
monomial subcase of \cref{dyn:rd:rem:superseded} $H$ is always
transcendental.
\item (11) Maximality is for \emph{centered} valuation balls only. It does not
say that $H$ fails to be summable at every individual shell point, and
extensions to sets omitting the obstructing cycles are a different question
(\cref{dyn:rd:rem:scope}). The shell is radial terminology, not a
topological boundary; valuation balls can be both open and closed.
\item (11) \Cref{dyn:rd:thm:allshells} prescribes every
\emph{infinitesimal} periodic point, not every periodic point of the
polynomial on $K_\Gamma$; other finite or infinitely large cycles of $f$
are not excluded.
\item (11) \Cref{dyn:rd:thm:allperiods} makes no claim about periodic points
of valuation zero, where the constant term of $\mathcal Q_n$ need not
dominate.
\item (11) The rational map $f^{\vee}$ is a presentation over the omnific
ring, not an $\Oz[i]$ self-map. It is not claimed that $f^{\vee}(a)\in\Oz[i]$
for every $a\in\Oz[i]$ at which it is defined, nor that its denominator is a
unit; integrality is proved for the classified periodic set, not for every
orbit. The coefficient-extraction retraction on $\Oz[i]$ is not the standard
part on $\OO_\Gamma$.
\item (11) Sharpness of the complex centered ball does not establish
sharpness of a real-only interval when the shell cycles are nonreal
(\cref{dyn:rd:ex:quintic}, \cref{dyn:rd:q:real}).
\item (11) The calculation pipeline is conditional on extracting leading
valuations and leading coefficients; it is not a decision procedure for
arbitrary unnamed surreal numbers. The algebraicity test is an exact
algebraic question only when the coefficients of $R$ are algebraic
numbers; for coefficients given by oracles or non-effective descriptions no
uniform decidability follows; and checking that an infinite support is well
ordered is separate from manipulating a finite support expression.
\item (11) The reduction $\bar H_r$ is a formal coefficientwise operation,
not the evaluation of $H_r$ at an ordinary point of the shell, and the
reduced equation \eqref{dyn:rd:eq:vectorfield} is not an evaluation on the
shell.
\item (11) Repeated residue roots are counted with algebraic multiplicity,
and unique lifting is claimed only for simple roots; a polynomial reducing
to $(1-U)^{2}$ may split into two cycles at a finer scale. In
\cref{dyn:rd:prop:stability} the lifted points themselves are not claimed to
stay fixed.
\item (11) Strong Hahn summation is never identified with sequential
convergence. No real number $|t|^{\gamma}$ is written for a general
exponent, no radius-of-convergence argument is used, and a rank-one proof is
not transferred by renaming the absolute value.
\item (11) Class level: no proper-class Hahn sum and no fixed workspace
containing every surcomplex point. The set constructions use ordinary choice
and set-based Hahn-field theory, or the definable classes of a conservative
class theory over ZFC, and no inaccessible cardinal or stronger hypothesis.
\item (11) The eleven questions are a research programme proposed by the
source, not historically established named open problems.
\item (11) The finite checks are regression checks of finite consequences,
$1{,}863$ exact assertions in six groups. They do not prove the
positive-support lemma, arbitrary-rank strong summability, proper-class
compatibility, all-period exclusion as a universal theorem, or novelty or
priority. No floating-point evidence substitutes for proof, and the ordinary
rational specialization in the shell test is not a model of infinitesimal
valuations or of the absence of other cycles.
\item (11) No Lean files are supplied as proofs of the new theorems, and
existing formal verification of the coefficient field does not amount to
verification of the article. The formalization sequence of
\cref{dyn:rd:subsec:limits} is a plan.
\item (11) The PowerShell build script was included but not executed in the
source's own build environment. Its build report records document quality
control and finite checks only, not proof review or formal verification.
\item\label{dyn:rd:nc:last} (11) The Wikipedia page supplied with the
request was used for orientation only; no theorem relies on it.
\end{enumerate}

\noindent Source 11 contributes $24$ distinct limitations, numbered
\textbf{N126}--\textbf{N149}. With the $123$ distinct limitations of sources
01--10 (\textbf{N1}--\textbf{N125}), this section records $147$ distinct
source-level limitations in $149$ numbered items. Three of source 11's items
are annotated as re-scoped by this report
(\textbf{N127}, \textbf{N130}, \textbf{N134}); none is dropped.
...
coefficients.\\[3pt]
Davie's small-divisor estimates as discussed by Marmi \cite{Marmi}; tree
expansions for one- and multi-dimensional linearization and their Bruno
estimates \cite{FauvetMenousSauzin,FMS26}; continued-fraction best
approximation \cite{Marmi,CarminatiMarmi}; Poonen's positive-support
geometric-series theorem \cite[Thm.~4.1]{Poonen}
& The classical background of \cref{dyn:esm:sec,dyn:cyc:subsec}. Sources
08 and 09 credit the counting and tree methods and claim only the
fixed-complexity form \cref{dyn:esm:thm:treebound}, the sharp radius
\cref{dyn:esm:thm:main} and the collision noncancellation
\cref{dyn:cyc:prop:noncancel}. Source 08 inspected
\cite{FauvetMenousSauzin} at abstract and metadata level only, \cite{FMS26}
in its version~2 of 13 September 2026, and \cite{Yoccoz} at metadata level.
No priority is certified (\ref{dyn:esm:nc:priority08},
\ref{dyn:esm:nc:priority09}).\\[3pt]
Admissible-cut closure for forests with non-universally-zero associated
operators, and its homogeneity-degree consequences
\cite[Thm.~E and Prop.~5.6]{FMS26};
Carletti's tree formulas for Lagrange inversion and linearization over
non-Archimedean formal-series structures \cite{CarlettiLagrange};
integer minors, Cramer's rule and norms of algebraic integers
& The classical background of \cref{dyn:nsc:subsec,dyn:nsc:subsec:trees,dyn:nsc:ex:ranktwo}.
Source 10 claims only the rate count \cref{dyn:nsc:lem:height}, the product
bound \cref{dyn:nsc:thm:products} and the common domain they give. It
inspected the HTML text of \cite{FMS26} and only the abstract and metadata of
\cite{CarlettiLagrange}. No priority is certified
(\ref{dyn:nsc:nc:priority}).\\[3pt]
...
characteristic warning \cref{dyn:warn:charzero}. Source 11 credits
\cite[Thm.~1.1 and Lemma~3.3]{Lindahl} for the Schr\"oder recursion and the
root-of-unity divisor dichotomy \cref{dyn:rd:lem:divisors}, inspected in its
HTML text and page images.\\[3pt]
Neumann's lemma as stated in Marker's lecture notes
\cite[Lemma~2.36]{Marker}; the norm of an algebraic logarithmic derivative
& Source 11's citation for \cref{dyn:lem:neumann}, and the classical
algebra behind \cref{dyn:rd:lem:norm}; neither is claimed.\\[3pt]
...
coefficient-category distinction only. Sources 08 and 09 cite it at
\texttt{a3124af79f66b8b9c196d76b4cbc5ac3938907c4} for the statements of
\cref{dyn:q:commongerm-restate,dyn:q:cyclic} and the constructions they
reuse. Source 10 cites it at
\texttt{048b72cf7cbfc8ab246e4f73788c10460cb3f6e0} for
\cref{dyn:q:commongerm}, \cref{dyn:thm:radius-depth} and the scalar results
of \cref{dyn:esm:sec}, which it cites rather than reproves. Source 11 cites
it at \texttt{9ee2dac0d234c90415f6b7290cbe5b4dcf4a4d3e} for the notation
and analysis guides only, and did not read this report. None uses an
unrefereed repository theorem as a premise. The
inspections were targeted, not exhaustive (\textbf{N9}, \textbf{N13},
\textbf{N71}, \ref{dyn:esm:nc:priority08},
\ref{dyn:esm:nc:priority09}, \ref{dyn:nsc:nc:priority},
\ref{dyn:rd:nc:repo}).\\
\bottomrule
\end{longtable}
...
$\mathfrak d_n$, $\|x\|_{\Z}$, $q_*$, $C_a$ & $|\lambda^{n}-1|$, so that
$|d_{n+1}|=\mathfrak d_n$; the distance to the nearest integer; the cluster
denominator of \cref{dyn:esm:lem:cluster} (neither the $\resq$ order nor a
convergent); and the constant \eqref{dyn:esm:eq:Ca}.\\[3pt]
$\check H=H^{-1}=z+\check h$ & The forward coordinate of source 09,
$F\circ\check H=\check H(\lambda\,\cdot)$; source 09's $H$ is this report's
$H^{-1}$ (\cref{dyn:esm:conv}). Source 10 writes it $G=z+g$, with
$F\circ G=G\circ\Lambda$ (\cref{dyn:nsc:conv}).\\[3pt]
$\rang(\Lambda)$ & Rational angular rank
$\dim_{\Q}\operatorname{span}_{\Q}(1,\theta_1,\dots,\theta_d)-1$ of an exact
diagonal multiplier \eqref{dyn:nsc:eq:rank}; $1\le\rang\le d$ under $\nres$.
Not the valuation rank of $\Gamma$, not the flag length $r$, not any other
$r$ of this report (\cref{dyn:nsc:conv}).\\[3pt]
$\mathfrak d_v$, $\alpha_v$, $\beta_v$, $\mathsf P(T)$, $A^{\Lambda}_m(n)$,
$C_a(\Lambda)$ & Divisor at a vertex of an admissible colored tree, its
leaf vector, the contracted leaf vector of \cref{dyn:nsc:lem:blocks}, the
tree's divisor product, its maximum at complexity $m$ and weight $n$, and the
constant \eqref{dyn:nsc:eq:Ca}. $A^{\Lambda}_m(n)$ is not $A_m(n)$ of
\cref{dyn:esm:cor:chain}.\\[3pt]
$h_{k,\ell}$, $h^{\mathrm{cyc}}_m$ & The coefficient of $u^{k}v^{\ell}$ in
the two-parameter drifted linearizer, and that of $t^{m}$ in the
single-exponent one; $h^{\mathrm{cyc}}_m=\sum_{\ell}h_{m-\ell,\ell}$
\eqref{dyn:cyc:eq:collision}.\\[3pt]
...
$\zeta$, $e$, $r_{\mathrm{eq}}$, $\hat a_j$, $\widehat R$, $R$, $\varkappa_j$
& Source 11 (\cref{dyn:rd:conv}): the rotation root with $\lambda=\zeta+e$,
the threshold \eqref{dyn:rd:eq:req} read off $f$ (equal to $r$ by
\cref{dyn:rd:prop:return}), normalized coefficients, the unreduced and
reduced boundary polynomials in $U=X^{q}$ \eqref{dyn:rd:eq:R}, and the
coefficients of $R$. Source 11 writes $m,d,\rho,u_j,P$ for $q,e,r_{\mathrm{eq}},\hat a_j,\widehat R$,
and its $s$ is this section's $\rho=t^{r_{\mathrm{eq}}}$.\\[3pt]
$\bar H_r$, $\mathfrak h$, $\beta$, $\mathfrak r_\beta$ & The coefficientwise
reduction of $H_r$ (source 11's $h_0$; the Taylor series of $H_0$), with
$\bar H_r=X\mathfrak h(X^{q})$; a root of $R$; the residue
$1/(q\beta R'(\beta))$ of \eqref{dyn:rd:eq:alpha}.\\[3pt]
$\OO_{\mathsf M}$, $\mathcal Q_n$, $\varphi_k$ & Source 11's support ring
$A_S$, iterate quotient $T_n$ and divisor polynomial $q_k$
(\cref{dyn:rd:def:ring,dyn:rd:lem:iterate,dyn:rd:lem:divisors}).\\[3pt]
$\mathcal S$, $\mathfrak b_\ell$, $\varsigma_\ell$, $r_\ell$, $f^{\vee}$,
$L_{\mathrm{cl}}$, $\Oz$, $\mathbb J$ & The shell-realizing polynomial, its
roots, constants and scales, the reciprocal map $1/f(1/w)$, its clearing
exponent, the omnific integers and their purely infinite part (the
collection's $\Pi$) (\cref{dyn:rd:thm:allshells,dyn:rd:thm:omnific}).\\[3pt]
...
interchangeable; the sixth category is \cref{dyn:esm:thm:main} for
$\Lambda=\lambda I_d$ and \cref{dyn:nsc:thm:criterion} for nonscalar
$\Lambda$, whose optimal radius for $\rang\ge2$ and $0<\tau<\infty$ is
\cref{dyn:q:commongerm}.\\[3pt]
\cref{dyn:thm:radius-depth} &
$\tau<\infty$. \textbf{Lower bound only}
(\cref{dyn:warn:lowerbound}).\\[3pt]
\cref{dyn:esm:thm:treebound,dyn:esm:cor:chain} &
$\tau<\infty$ for the bound; \emph{strictly positive} integer weights
(\cref{dyn:esm:warn:zero}); one irrational rotation. The polynomial factor
$(C_an)^{m}$ is harmless only at fixed $m$.\\[3pt]
\cref{dyn:esm:thm:main} &
$\nres$ with the multiplier \emph{exactly} the scalar $\lambda I_d$;
$f_s(0)=0$ and $\mathrm Df_s(0)=0$ coefficientwise; one common input
polydisk; $\tau<\infty$. Open polydisk only, no uniform norm bound, and a
universal rather than a per-input radius. Nonscalar $\Lambda$ is not
covered (\cref{dyn:esm:subsec:limits,dyn:esm:tab:audit}); it is
\cref{dyn:nsc:thm:main}.\\[3pt]
\cref{dyn:nsc:lem:height,dyn:nsc:cor:angular} &
A \emph{fixed finite} family of integer vector sequences with heights
$O(N)$ and positive limiting rates. No Diophantine or algebraicity
hypothesis on the angles. Counts distinct rates, not vectors.\\[3pt]
\cref{dyn:nsc:thm:products,dyn:nsc:cor:rankone} &
$\nres$; arities at least two, so every weight is positive
(no infinitesimal linear term). The product bound requires $\tau<\infty$;
the rank-one equality also includes $\tau=\infty$. Fixed $m$; the finite-rate
prefactor is not explicit and not uniform in $m$.\\[3pt]
\cref{dyn:nsc:thm:main,dyn:nsc:thm:criterion} &
$\nres$ with the multiplier \emph{exactly} a diagonal unitary $\Lambda$;
$f_s(0)=0$ and $\mathrm Df_s(0)=0$ coefficientwise; one common input
polydisk; $\tau<\infty$. Open polydisk only; a universal radius, proved
sharp for $\rang=1$ and for $\tau=0$ at every angular rank; the optimal
radius for $\rang\ge2$ and $0<\tau<\infty$ is open
(\cref{dyn:nsc:subsec:limits}).\\[3pt]
\cref{dyn:nsc:cor:local} &
Convergent-germ inputs; the radius is relative to the finitely many inputs
at letters of $D_S(\gamma)$. Lower bound only.\\[3pt]
\cref{dyn:tree:prop:formula,dyn:tree:prop:estimate} &
One variable; the forward convention $\check H=H^{-1}$; otherwise the
hypotheses of \cref{dyn:esm:thm:main}. Shares its arithmetic lemmas with
route A.\\[3pt]
...
and equal-radius polydisks. As stated, these three are asserted over
$\Gamma_{*}$. Source 08's \cref{dyn:cyc:thm} removes the second exponent in
one variable, over $\Z$, but still requires drift.
\Cref{dyn:thm:multicollapse} is not asserted for a fixed cyclic value group
(\cref{dyn:warn:drift,dyn:q:cyclic}).\\[3pt]
\cref{dyn:cyc:thm,dyn:cyc:cor:classification} &
\textbf{Multiplier drift required}; one variable; value group $\Z$, or any
fixed nonzero $\Gamma$ by embedding; the explicit input
\eqref{dyn:cyc:eq:input}. No termwise positivity is claimed
(\cref{dyn:cyc:rem:excludes}), and there is no ordinary specialization of
$t$ (\ref{dyn:esm:nc:specialize}).\\[3pt]
...
\cref{dyn:rd:thm:linearization,dyn:rd:thm:shell,dyn:rd:prop:periodfinite} &
Exact rotational symmetry $f(\zeta z)=\zeta f(z)$; $v(e)>0$ with $e\ne0$;
finite polynomial input; one common positive support monoid; divisible
$\Gamma$ and algebraic closure for the root count; equal characteristic
zero. Maximality for centered balls only.\\[3pt]
\cref{dyn:rd:thm:reduction,dyn:rd:thm:algebraicity} &
Integral normalized coefficients before reduction. The criterion decides
$\bar H_r$ only; failure certifies transcendence of $H$, success does not
prove algebraicity (\ref{dyn:rd:nc:oneway}); superseded in the monomial
subcase (\cref{dyn:rd:rem:superseded}).\\[3pt]
\cref{dyn:rd:thm:allperiods} &
Integral coefficients and equivariance; nonzero \emph{infinitesimal} points
only, nothing at valuation zero; $\lambda=\zeta$ allowed.\\[3pt]
\cref{dyn:rd:thm:allshells,dyn:rd:thm:omnific} &
Finitely many prescribed shells with distinct $\mathfrak b_\ell$; only
infinitesimal (respectively infinitely large finite) periodic points are
prescribed; a rational-map presentation, not an $\Oz[i]$ self-map.\\[3pt]
...
21 September 2026 (sources 01--06); commit
\texttt{aa846271b4dcae2c055b216126a87210292ec19b} (source 07); and commit
\texttt{a3124af79f66b8b9c196d76b4cbc5ac3938907c4}, inspected 22 September
2026 (sources 08 and 09); and commit
\texttt{048b72cf7cbfc8ab246e4f73788c10460cb3f6e0}, inspected 22 September
2026 (source 10); in particular
\texttt{docs/surcomplex/dynamics-and-normal-forms/article.tex} and its
README; and commit \texttt{9ee2dac0d234c90415f6b7290cbe5b4dcf4a4d3e}
(source 11), of which it consulted the catalogue READMEs,
\texttt{docs/NOTATION.md}, \texttt{docs/surcomplex/analysis/README.md} and
the README of \texttt{docs/surreal/set-sized-quotients-of-omnific-integers/}. The reports are AI-assisted, unrefereed research drafts,
...
\bibitem{CarlettiLagrange}
T.~Carletti,
\emph{The Lagrange inversion formula on non-Archimedean fields.
Non-Analytical Form of Differential and Finite Difference Equations},
\href{https://arxiv.org/abs/math/0110135}{arXiv:math/0110135}, first submitted
12 October 2001; revised 1 July 2002. Source 10 used its abstract and metadata only.

\bibitem{FMS26}
F.~Fauvet, F.~Menous, and D.~Sauzin,
\emph{Explicit linearization of multi-dimensional germs and vector fields
through Ecalle's tree expansions},
\href{https://arxiv.org/abs/2507.13216v2}{arXiv:2507.13216v2},
version~2 submitted 13 September 2026 (first posted 17 July 2025), 41~pp.
...
\bibitem{Marker}
D.~Marker,
\emph{Model Theory of Valued Fields}, lecture notes, Math~512, University of
Illinois at Chicago, 2018, Chapters~1--2; Lemma~2.36 is Neumann's lemma.
