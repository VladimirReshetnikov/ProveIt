...
Eight of the nine are germ and fixed-point dynamics: multipliers, return
...
there (the nonscalar exact case remains open); an exact radius-collapse law
...
\textbf{Status.} This is a merged report assembled from nine AI-assisted,
...
conjecture is claimed solved. The nine sources' own verification programs
...
\subsection{Nine manuscripts, one body of machinery}
...
4cbc5ac3\allowbreak 938907c4}. All nine share a single body
...
near-identity inverse --- and each of the nine states that machinery from
scratch. The positive-support lemma is stated in all nine. Between $45$
and $55$ per cent of the combined text of sources 01--06 is the same
theory, and sources 07, 08 and 09 restate the same foundation again.
...
(\texttt{docs/surcomplex/analysis/}). All nine sources restate it; we cite
...
(\texttt{docs/surcomplex/differential-equations/}). All nine sources
...
their contributions is correspondingly uncertain.
...
$\nres$ reads $\lambda^{n}\ne\lambda$ for $n\ge2$.
&
This is the hypothesis of \cref{dyn:thm:main,dyn:thm:lifting,dyn:thm:trichotomy,dyn:esm:thm:main,dyn:cyc:thm}
...
in \cref{dyn:thm:main,dyn:esm:thm:main} and \emph{drifted} in
...
&
Sources 03, 08 and 09. No infinitesimal linear part is permitted in
\cref{dyn:thm:main} or in \cref{dyn:esm:sec}; infinitesimal linear terms
...
& $\kappa<\infty$, with universal radius $Re^{-\kappa}$, for
 $\Lambda=\lambda I_d$ (\cref{dyn:esm:thm:main}); \textbf{open} for
 nonscalar $\Lambda$ (\cref{dyn:q:commongerm})
...
only}; superseded for $\Lambda=\lambda I_d$ by \cref{dyn:esm:thm:main}\\
...
one common radius $Re^{-\tau}$ (\cref{dyn:esm:thm:main}). See
...
All nine sources set up this material independently. It is stated once
here.
...
phase residue of the scalar derivation. All nine sources derive this
...
\cref{dyn:esm:thm:main}; for a nonscalar one it is \cref{dyn:q:commongerm}.
\end{convention}
...
claimed as a contribution. All nine sources state it; sources 02, 05 and 09
...
printed again. Source 08 instead reduces the lemma to Poonen's
...
lemmas with route A as route A is used in \cref{dyn:esm:sec}. Source 02 also
...
(\cref{dyn:esm:thm:main}); the noncommutation is a phenomenon of drift.
...
nonscalar $\Lambda$ the question remains open.
\end{warning}
...
deep the Hahn words are. The nonscalar diagonal case of
\cref{dyn:q:commongerm} is \emph{not} settled
(\cref{dyn:esm:subsec:limits}).
...
nonscalar case), and the absence of machine-checked proofs.
...
case in several variables is open (\cref{dyn:q:commongerm}).}
...
\emph{settled} for a scalar exact multiplier, \emph{open} for a nonscalar
one]\label{dyn:q:commongerm}
...
For a nonscalar diagonal unitary $\Lambda$ the question is \textbf{open}
(\cref{dyn:esm:subsec:limits}). At the pins of sources 01--07 it was open
throughout $\sdex$.
\end{question}
...
(\cref{dyn:esm:thm:treebound}). For a nonscalar $\Lambda$ the question
remains \textbf{open}.
...
general) and for a scalar exact multiplier (it always exists, with radius
$Re^{-\tau}$). For a nonscalar exact multiplier it is still open.
\end{warning}
...
& $\tau<\infty$, radius $Re^{-\tau}$, for $\lambda I_d$
 (\cref{dyn:esm:thm:main}); open for nonscalar $\Lambda$; $\sigma=0$ under
...
coordinate-versus-surreal-derivation distinction nine times.
...
material. All nine sources restate that discipline and all nine state
...
The nine verification programs are preserved unmodified in \texttt{code/}
...
workspace invariance, or novelty. No Lean code.\\
\bottomrule
\end{longtable}
...
\begin{enumerate}[label=(\arabic*)]
\item \textbf{All nine suites pass.} Run on Python 3.14.4 with SymPy 1.14.0
...
\item \textbf{Four of the nine scripts rewrite their own evidence.} Source
...
\end{verbatim}
Reproducing the nine suites, each \emph{on a copy}, requires Python 3.10 or
...
nonscalar multiplier]\label{dyn:q:commongerm-restate}
...
(sources 08 and 09), and it is \textbf{open} for a nonscalar diagonal
unitary $\Lambda$. Source 03's remark still applies there: an exact
characterization would need to control interactions across unbounded word
depths rather than a single homological inverse. For a scalar multiplier
that control is \cref{dyn:esm:thm:treebound}. For a nonscalar one the
divisors $\lambda^{\beta}-\lambda_j$ are not indexed by a single integer,
and what is missing is a vector analogue of the tree bound, or a
...
\cref{dyn:conv:merge}(iv). It carries \textbf{one hundred and six}
distinct limitations stated by the nine sources --- eleven from source 01,
...
source 08 and twelve from source 09 --- with the originating source named in
each case. None has been merged away, softened, or absorbed
...
(\cref{dyn:esm:thm:main}); it remains open for a nonscalar one
...
is covered by source 08 while the nonscalar case stays open.
...
$\lambda^{\beta}-\lambda_j$ (\cref{dyn:esm:subsec:limits}).
...
remains open (\ref{dyn:esm:nc:scalar}).
...
\noindent The list above contains $108$ numbered items for the $106$ distinct
...
$11/9/10/14/12/12/10/16/12$ across sources $01$ through $09$; the
corresponding numbered-item counts are $11/9/11/15/12/12/10/16/12$.
...
$\Lambda=\lambda I_d$ and \cref{dyn:q:commongerm} for nonscalar
$\Lambda$.\\[3pt]
...
covered (\cref{dyn:esm:subsec:limits,dyn:esm:tab:audit}).\\[3pt]
...
2026 (sources 08 and 09), in particular
