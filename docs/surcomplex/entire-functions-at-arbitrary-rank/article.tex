...
coefficients, the boundary residue fields then being real closed. The third
...
This report is the union of five manuscripts. Two were written
...
scalar-extension clause of the several-variable question. The arbitrary-rank
...
several-variable replacement are proposed original contributions, not a
claimed solution of any named published conjecture. Classical Hahn-field
inputs and rank-one antecedents are identified and credited. Complete proofs
...
weaker hypotheses, $\Gamma$ is a \emph{set-sized divisible ordered abelian
group}, written additively, and
\[
 \K=\C((t^\Gamma)),\qquad
...
Two further manuscripts were merged later. The \emph{fourth} was written
...
finite-variable criteria that earlier versions kept in an appendix.
...
extension domain or the whole-class rigidity.
\Cref{ent:sec:scope,ent:app:sources} state the limits in full; every
limitation recorded by any of the five source manuscripts is preserved
there.
...
Five manuscripts are merged here, and three of them arrived with letters
...
element of $\Gamma$. Every statement taken from the third, fourth or fifth
manuscript has been rewritten in the convention below; the tables are
...
\section{Several ordinary variables: exact extension domains and
...
\item \emph{Notation.} The fifth source's letters are translated by the last
table of \cref{ent:subsec:notation}. In particular its homogeneous block
...
This report is a merge of five manuscripts, each prepared against a pinned
...
the other sections is transferred to it.
...
theory. The arbitrary-rank preparation theorem is
...
an independent build. None of the five manuscripts was independently
...
that its source typesets. No theorem below depends for its validity on the
...
See \cref{ent:app:checks} for the full statement. In summary: the five
delivered suites verify $831$, $664$, $1958$, $1876$ and $1072$ finite exact
assertions respectively, and they prove \emph{no} infinite theorem. The first
...
Five independent exact-arithmetic suites accompany this report, one from
each source manuscript, preserved unmodified in \texttt{code/} and
\texttt{data/} under the file names given below. All five use only the
Python standard library, with exact integer and \texttt{fractions.Fraction}
arithmetic and no floating point. The first four represent group elements as
tuples of rational coordinates ordered by the largest nonzero coordinate;
the fifth needs no ordered-group tuples beyond integer relation vectors.

\paragraph{Suite A (831 checks).}
The first source's script
\texttt{code/06-cofinality-\allowbreak bezout-trichotomy-\allowbreak
verify-examples.py} checks finite instances of the
...
checks and $175$ inverse-jet checks; the record is
\texttt{data/06-cofinality-\allowbreak bezout-trichotomy-verification.json}. The script
takes a \emph{required} output path argument
...
\texttt{code/03-cofinality-and-scalar-extension-verify.py} checks finite
truncations of
...
reports \textbf{664} passing checks, recorded in
...
All five suites print their own scope disclaimers, and all five are correct
to do so.
...
The first two of this report's five source manuscripts are both dated
