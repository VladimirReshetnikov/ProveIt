...
written additively. Except where the degenerate case $\Gamma=0$ is explicitly
discussed, assume $\Gamma\ne0$. Put
\[
 \K=\C((t^\Gamma)),\qquad
...
Let $\gamma_n>0$ tend cofinally to $+\infty$ (for example, let
$(\gamma_n)$ be increasing and cofinal), and let $\lambda_n>0$ satisfy
...
every odd index. All applications below use the increasing sequence of
...
\end{theorem}
\begin{proof}
Assume strong summability at every diagonal monomial point and $F\ne0$.
At $(1,\ldots,1)$ it gives the attained lower bound
$m=\min_{a_\alpha\ne0}v(a_\alpha)$. If the slope condition fails, some
$h\in\Gamma$ has violating indices of arbitrarily large total degree
$N=|\alpha|$, with $a_\alpha\ne0$ and $v(a_\alpha)\le Nh$.
Choose $q>\max\{0,|m|,|h|\}$. Strong summability at
$(t^{-2q},\ldots,t^{-2q})$ makes the set of leading exponents
$v(a_\alpha)-2|\alpha|q$ well ordered. Its minimum $\mu$ is attained at an
index $\beta$, and
\[
 \mu=v(a_\beta)-2|\beta|q\ge m-2|\beta|q
      >-(2|\beta|+1)q.
\]
A violating index of degree $N>2|\beta|+1$ instead gives
\[
 v(a_\alpha)-2Nq\le Nh-2Nq<-Nq<\mu,
\]
a contradiction. This is the finite-index minimum argument of
\cref{ent:thm:criterion}; it does not require multiples of $q$ to be cofinal.
The zero series satisfies the criterion directly.
...
reports \textbf{664} passing checks. A review run on a temporary copy also
passed all $664$ checks, leaving the historical records unchanged.
