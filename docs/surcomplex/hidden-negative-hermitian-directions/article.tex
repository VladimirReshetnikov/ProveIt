...
at assembly (\texttt{ed88b8f}) and revisited during the main-text proof
review against repository commit \texttt{b6819f7}. The historical search record remains
in Appendix~\ref{hnd:app:audit}.
...
congruence \eqref{hnd:eq:congruence} uses the inertia invariance of
\lbl{prop:positive}. Its triangular matrix $T$ is not unitary, so this
calculation does not supply the unitary eigenbasis of
\lbl{spec:thm:hermitian}. Nothing here extends or contradicts those results. The support-index bound,
...
by it. The null-ideal results compared here use coefficientwise countable
additivity; they do not require strong Hahn additivity.
...
valuation limit of its algebraic truncations when $\Gamma=\Q$.
Those truncations are algebraic over $B_0$: their finitely many
coefficients generate one finite extension of $\Q$. Merely having
algebraic coefficients would not suffice, as the full transcendental
tail itself demonstrates. Here Proposition~\ref{hnd:prop:probeclosed}
and Theorem~\ref{hnd:thm:barrier} exclude convergence even of nets of
elements algebraic over $\C(t^\gamma:\gamma\in\Gamma)$;
Corollary~\ref{hnd:cor:degreebound} specializes to $\C(t^\delta)$.
The base fields in these algebraicity assertions differ. The warning
...
above. For $\Gamma=\Q$ the two bases are incomparable. Every prime tail $h_A$
has support bounded above by $\delta$ and so lies in
$\mathcal B_\Q(\C)$, but not in $\mathscr P_\Q$ by the same fresh-prime
argument as Theorem~\ref{hnd:thm:barrier}. In the reverse direction,
$\sqrt{1+t}\in\C[[t]]\subseteq\mathscr P_\Q$ does not belong to the
bounded-support fraction field. Indeed, \lbl{bst:cor:onescale} gives
\[
 \C((t))\cap\operatorname{Frac}\mathcal B_\Q(\C)=\C(t),
\]
and $1+t$ is not a square in $\C(t)$: the multiplicity of the factor
$t+1$ in a rational square is even.

For this special intersection, the descent argument is short. If
$f=a/b\in\C((t))$ with bounded-support $a,b$ and $b\ne0$, select a
coset $r+\Z$ meeting $\supp b$. Restrict $bf=a$ to that coset and shift
by $-r$. Since $f$ has integer exponents, this gives $b_r f=a_r$.
The shifted supports of $a_r,b_r$ are well-ordered, bounded subsets of
$\Z$, hence finite, and $b_r\ne0$. Thus $f$ is rational in $t$.
In particular, unbounded support alone would not suffice: the geometric
series $\sum_{n\ge1}t^n=t/(1-t)$ is already in the fraction field.
No incomparability for arbitrary $\Gamma$ is asserted. If $\Gamma$ has
no order unit (no $u>0$ whose positive integer multiples are cofinal),
each finitely generated subgroup is bounded above in $\Gamma$.
Indeed, its generators are dominated in absolute value by some $u>0$,
and failure of cofinality supplies a bound above all $nu$.
Thus $\Pfield\subseteq\mathcal B_\Gamma(\C)$.
These distinctions of base and witnesses prevent either independence
theorem from being identified with the other.
...
comparisons involving $+\infty$. The intrinsic $\Gamma$-valuation
topology has neighborhoods $\{x:v(x-a)>\rho\}$ at $a$, for
$\rho\in\Gamma$.
...
When $\Gamma=\Q$, every finitely generated subgroup is contained in
$\frac1n\Z$ for a suitable integer $n\ge1$, by taking a common
denominator of its generators.
Consequently
\[
...
supports. Each nonzero input has entirely positive support because
its least exponent is positive. Their finite union is well ordered.
The positive-support lemma says that the monoid of finite sums from
this union is well ordered, with only finitely many words giving any
prescribed sum, over all word lengths. Tagging the
letters of such a word by the $m$ input variables still gives finitely
many choices. Thus all formal monomial contributions are strongly
summable, regardless of the coefficients of $\Phi$. Every resulting
exponent is a finite sum of input exponents and hence lies in $H$.
The evaluated series therefore belongs to $\C((t^H))$.
Here a zero argument simply kills every monomial using that variable;
the constant monomial contributes at exponent zero. No convergence of
successive total-degree truncations is needed, even when the positive
input valuations are not cofinal in $\Gamma$.
\end{proof}
...
For if $b+ka=b'+k'a$, then $(k-k')a\in B'$. In the first case this
forces $k=k'$ and $b=b'$. In the second, division with remainder by
$n$ shows $k-k'=\ell n$ for some integer $\ell$; consequently
$\chi(b'-b)=\chi(na)^\ell=c^{k-k'}$, which gives the same assigned value.
The formula respects addition because $\chi$ is a homomorphism on $B'$.
...
More explicitly, if $G\cong\Z^k$, this quotient is isomorphic to
$(\Z/D!\Z)^k$: multiplication by $D!$ identifies it with $G/(D!)G$.
This includes $G=0$ and $D=1$.
...
\end{proposition}
\begin{proof}
A nonconstant polynomial over $\Pfield$ has finitely many coefficients. Choose a
...
Fix $\delta>0$ and set $s=t^\delta$. Distinct nonnegative integer
multiples of $\delta$ give distinct exponents, so no nonzero polynomial
over $\C$ vanishes at $s$. Thus $s$ is transcendental and
$\C(t^{\Z\delta})=\C(s)$ is a rational-function field.
Let $\primes$ be the ordinary set
...
These classes generate $(\frac1{P_N}\Z\delta)/(\Z\delta)$: the integers
$P_N/p_1,\ldots,P_N/p_N$ have greatest common divisor one, so B\'ezout's
identity produces $\delta/P_N$ from the $\delta/p_j$. This quotient has
order $P_N$, because $\delta$ has infinite order. Therefore $D\ge P_N$.

On the other hand, put $u=t^{\delta/P_N}$. Then $u^{P_N}=s$ and
$h_{\delta,N}=\sum_{j=1}^N u^{P_N-P_N/p_j}$ lies in $\C(u)$, an
extension of $\C(s)$ of degree at most $P_N$. Its support contains the first $N$
...
For real approximants, adjoining $i$ can only decrease field degree:
an $\R(s)$-basis of $\R(s,a)$ spans $\C(s,a)$ over $\C(s)$.
Thus $[\C(s,a):\C(s)]\le[\R(s,a):\R(s)]$, so the same lower bound applies. The displayed real finite sum has
...
its degrees, converges to $h_\delta$ in the intrinsic $\Gamma$-valuation topology.
\end{corollary}
\begin{proof}
...
\end{proof}
The same fixed neighborhood excludes convergence of any net of these
approximants. Increasing degrees or replacing sequences by nets cannot
cross the missing valuation scale.
...
\end{equation}
These are $p$ distinct elements, since their coefficients at
$q_p\delta$ are the distinct numbers $\zeta^{-k}$. The extension
character need not have order $p$ on all of $\Gamma/J$; only these
$p$ values and the displayed action on $h_r$ are used.
Since $T$ fixes the original coefficients and every previous $h_j$,
it fixes every coefficient of $R$. All the displayed elements are
therefore roots of that same nonzero polynomial. This contradicts
...
$\min_jv(b_j)=m_0$. Indeed, all errors have valuation greater than
$m_0$, so no $b_j$ has valuation below $m_0$, and an $a_j$ with
valuation $m_0$ retains its nonzero leading coefficient. In particular,
$(b_1,\ldots,b_r)\ne0$. Write
...
\end{corollary}
\begin{proof}
Choose the almost-disjoint prime tails with $\Gamma=\Q$ and $\delta=1$.
Theorem~\ref{hnd:thm:inertia} gives the inertia, and
Theorem~\ref{hnd:thm:closedseparation} gives strict positivity on the
closure identified in Proposition~\ref{hnd:prop:lc}.
\end{proof}
...
\end{proposition}
\begin{proof}
Assume density and let $q(x)=x^TAx$ satisfy $q(u)<0$ for some $u\in F^n$.
We can see the needed polynomial continuity directly over any ordered
field. Put
\[
 C=1+\sum_{i,j}|A_{ij}|\bigl(|u_i|+|u_j|+1\bigr),\qquad
 \theta=\min\left(1,\frac{-q(u)}{2C}\right)>0.
\]
If $|x_i-u_i|<\theta$ for every $i$, expanding $q(x)-q(u)$ gives
$|q(x)-q(u)|<C\theta\le-q(u)/2$, so $q(x)<0$.
Density supplies such an $x\in E^n$ by choosing each coordinate in
$(u_i-\theta,u_i+\theta)$. This proves (i)$\Rightarrow$(ii), and
...
$|z/w-h|^2=(x-h)^2+y^2>e^2$. This handles $w\ne0$; for
$w=0$ and $z\ne0$, the quadratic value is $|z|^2>0$.
...
algebraic over rational expressions in surreal monomials. To justify
this without assuming a workspace already contains the root, let $x$
be such a coordinate. Its equation uses finitely many monomials;
let $H$ be the finitely generated group of their Hahn exponents.
Adjoin $1$ and the negatives of all Conway exponents of $x$ to $H$
and take the divisible hull. This is a nonzero set-sized ordered subgroup $\Gamma'$ of
$\No$, and its Hahn workspace contains both $x$ and the equation.
Now apply Lemma~\ref{hnd:lem:supportdegree} inside that workspace with
$G=H$ and $L=\C(t^H)$. If $D=[L(x):L]$, the support of $x$ lies in
$\frac1{D!}H$, which is finitely generated. Thus $x$ is an allowed probe.
For a finite vector take the sum of these finitely many support groups.
This does not include a coordinate whose defining parameters already
contain the prime tail itself.
...
If $b\ne0$, then $|b|^2>0$ because it is the sum of the squares of
its real and imaginary parts. Choose $s=-\lambda b$ with $\lambda>0$
in $L$. Hermitian symmetry gives $u^*Aw=\bar b$, so
\[
 (u+sw)^*A(u+sw)
 =s\bar b+\bar s b+|s|^2c
...
Let $P,Q\in M_d(\C)$ be Hermitian. Write $N=\ker P$ and let
$C_N$ denote the compression of $Q$ to $N$,
...
\begin{equation}\label{hnd:eq:activeblock}
 ...
 \end{pmatrix}.
\end{equation}
Assume first that both active spaces have positive dimension.
Let $U=P_R+\eps A$. Every leading principal minor of $U$ has positive
constant coefficient, the corresponding minor of $P_R$.
For a Hermitian matrix over an ordered field complexification, positive
leading principal minors imply positive definiteness: elimination gives
an $LDL^*$ factorization with diagonal pivots
$\Delta_j/\Delta_{j-1}>0$, where $\Delta_0=1$ and $\Delta_j$ is the
$j$th leading principal minor. Its quadratic value is a sum of positive
pivots times squared moduli after an invertible triangular change of
coordinates. This works for all Hahn vectors and needs no real-closedness.
Thus $U\succ0$.

The adjugate formula shows that $U^{-1}$ has entries rational in $\eps$.
Since $\det U$ has nonzero constant term, its reciprocal has valuation
zero and reciprocal constant coefficient; hence the constant matrix of
$U^{-1}$ is $P_R^{-1}$. Put
\[
 V=\begin{pmatrix}I&-\eps U^{-1}B\\0&I\end{pmatrix}.
\]
A block multiplication gives the explicit congruence
\[
 V^*\begin{pmatrix}U&\eps B\\\eps B^*&\eps C_+\end{pmatrix}V
 =\diag\bigl(U,\eps(C_+-\eps B^*U^{-1}B)\bigr).
\]
The second factor in parentheses is Hermitian and has constant matrix
$C_+$. Every leading principal minor therefore has positive constant
coefficient, so the same pivot argument makes it positive definite.
Since $\eps>0$ and $V$ is invertible, the entire active block is
positive definite. If $R=0$, it is simply $\eps C_+$; if $N_+=0$,
it is $P_R+\eps A$. If both are zero there is no active block.
These cases need no empty inverse.

The only kernel is therefore $N_0\otimes_\C K_\Gamma$.
Within $N$, the condition $Qu=0$ implies $C_Nu=0$ by compression;
the converse is exactly the assumed annihilation clause. Thus
$N_0=\ker P\cap\ker Q$, and the active dimension is
$\dim R+\dim N_+=\rank P+\rank C_N$. This proves \eqref{hnd:eq:kernel} and
...
sets. Finiteness follows directly from the entry measures: for each event $E$,
\[
 |\sigma_u|(E)\le\sum_{i,j}|u_j|\,|(M_1)_{ij}|(E)<\infty.
\]
Indeed, apply the triangle inequality to $\|M_1(B)u\|$ on each piece of
a finite measurable partition of $E$, then take the supremum over partitions.
...
variation through finite partitions. If $\sigma_u(B)=0$ for every
measurable $B\subseteq E$, each partition sum defining $|\sigma_u|(E)$
is zero. Conversely,
$\|\sigma_u(B)\|\le|\sigma_u|(B)\le|\sigma_u|(E)$, so zero variation
on $E$ forces all those vector masses to vanish.
...
For example, intersect $E$ with a negative set in a Hahn decomposition
of $\nu_u$: its mass is $-\nu_u^-(E)$ and must be nonnegative.
...
the comparison uses incomparable bases for $\Gamma=\Q$, with different
witnesses (Section~\ref{hnd:sec:position}).
...
commit \texttt{30dfb4f} as a new report, because it continues themes and
...
in that file. During editorial assembly at \texttt{ed88b8f}, the
manuscript's article and README were rewritten, and a newly built PDF was added.
...
reports. Raw placement \texttt{30dfb4f} added four new reports,
this one among them, bringing the count then to 40.
...
\end{itemize}
Between the pin and assembly at \texttt{ed88b8f}, the spectral-theory,
