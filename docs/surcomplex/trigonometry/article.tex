...
\item \textbf{Asymptotics.} $a\prec b$ means $a/b\in\mm_{\SC}$; $a\sim b$ means
 $a/b-1\in\mm_{\SC}$. Both are exact algebraic relations between surreal numbers, never limits
...
\end{description}
The inclusions $\text{(R1)}\subset\text{(R2)}\subset\text{(R3)}\subset\text{(R4)}$ are all strict.
...
circumcircle, and $R=abc/(4\Delta)$. All denominators are nonzero, and all sides, the area, the
circumradius and the inradius may independently be infinite, infinitesimal, or of mutually
different scales.
\end{theorem}
\begin{proof}
