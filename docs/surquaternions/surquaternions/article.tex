...
noncommutative, and that is the whole difficulty: no commutative argument may be
reused, and every commutative result quoted from a companion report is reproved here
over $\mathbb H$.
...
\emph{radial-exponential-support-lifting}, both dated September 21, 2026 and both
shipped with the source archives now in \texttt{sources/}. Every result either
...
cite it for orientation and reprove what we need over $\mathbb H$; we never transport a
...
positive Cauchy--Binet argument are determinant arguments and are \emph{not} available
...
\]
A Hahn series has a well-ordered support in $\Gamma$, with our increasing-exponent
convention. If $\Gamma$ is divisible, the standard real-closed Hahn-field theorem makes
...
part. It is surjective onto $\SQ^\times$, conjugation-equivariant, and multiplicative
only on commuting pairs; it is \emph{not} an everywhere Hahn-summed power series and
...
Not additive on noncommuting pairs; not an isomorphism onto $K$, the transported law
...
\[
 ...
 \abs{[p,q]}\le2\abs{\Ima p}\,\abs{\Ima q}.
\]
...
$\abs{u\times v}^2=\abs u^2\abs v^2-(u\cdot v)^2$ proves the inequality. In an ordered
...
variable $s$ is nonnegative for every $s\in F$; substituting $s=(p,q)/N(q)$ gives
...
obtainable from its four-coordinate matrix.
\end{proof}
...
Two rational charts on the unit group are available, and both are exact at every scale,
including scales whose radii are infinitesimal.

\begin{proposition}[Two rational charts on the unit group]\label{squat:prop:charts}
...
Independently, the \emph{stereographic chart}
...
\end{proposition}
\begin{proof}
For \cref{squat:eq:cayley}, conjugation gives
$\overline{\operatorname{Cay}(x)}=(1-x)^{-1}(1+x)$ up to the order of the commuting
rational factors in one quaternion, and the modulus is $1$ by multiplicativity; all the
factors used are rational functions of the single element $x$ and therefore commute with
one another where the verification uses that. The displayed inverse is checked by
substitution. For \cref{squat:eq:stereographic}, expand
$N(1-\abs x^2+2x)=(1-\abs x^2)^2+4\abs x^2=(1+\abs x^2)^2$; the displayed inverse is
again checked by substitution. Neither chart is a claim that $\Sp(1,F)$ is topologically
a real three-sphere.
\end{proof}
...
has a root on $[a,r]$ if and only if $\Delta_{a,r}\mid N_P$; if $A=0$ in
...
and such a sum is explicitly not a limit of partial sums.
\end{definition}
...
On a workspace $K_\Gamma$ or $D_\Gamma$ one must distinguish: the \emph{intrinsic} order
or valuation topology, using only radii available in that workspace; the subspace topology
inherited from the full fine topology, which is discrete because the workspace is a set;
and a deliberately coarse \emph{standard-part} topology on finite surquaternions, pulled
back from $\HR\simeq\R^4$ by \cref{squat:eq:residue}, which is not Hausdorff, since points
with the same standard part cannot be separated. In particular no sequence of distinct
...
addition. Every finite unit $u$ factors uniquely as $u=\st(u)\,\omega_1$ with
...
\cref{squat:def:summable}; at $q=\omega\ii$ their exponents are unbounded in the wrong
direction. Neither failure is repaired by citing ordinary real convergence.
\end{warning}
...
For an infinitesimal $x$, differentiating its Hahn exponential gives the valid
noncommutative formula
\begin{equation}\label{squat:eq:dexp}
 \delH\Expm(x)=\sum_{n\ge1}\frac1{n!}\sum_{m=0}^{n-1}x^m\,\delH(x)\,x^{n-1-m},
\end{equation}
whose right-hand family is support-controlled: it is built from words in the positive
support of $x$ with one distinguished factor from $\supp\delH(x)$, and
\cref{squat:lem:neumann} with the finite antidiagonals of sums of well-ordered sets
justifies the regrouping. In general this is \emph{not} $\Expm(x)\delH(x)$; that simpler
expression is valid exactly when $[x,\delH(x)]=0$. Nothing in this local assertion implies
a global commuting chain rule for any extension of the quaternion exponential.
...
positive Cauchy--Binet expansion of the minors of a product. \emph{Neither is available
over the quaternions}, because there is no determinant on $M_n(\HF)$ with the required
multiplicative and positivity properties; the Dieudonn\'e determinant takes values in an
abelianization and does not support that argument. Nothing below reuses either statement,
and the multiscale results of this section are proved by the min--max and projector
arguments instead. That report's explicit warning about Gram squaring applies here
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.4em]
\item Knowing $w(a_n)\ge0$ individually is \emph{not} a substitute for a common $S$. In
the workspace with $\Gamma=\Q$ the series $\sum_{n\ge0}t^{-n^2}x^n$ is not Hahn-summable at
...
exist or compile}. Both source manuscripts proposed a layering, and they differ in
...
topology. Neither rational chart of \cref{squat:prop:charts} claims that $\Sp(1,F)$ is
topologically a real three-sphere, and the polar decomposition
...
determinant arguments and are not available over the quaternions; nothing here reuses them
(\cref{squat:warn:nodet}). More generally no commutative result is reused: every
commutative argument in this subject is reproved over $\mathbb H$.
...
computable; and the top row of \cref{squat:sub:hierarchy} carries no decidability or
...
\paragraph{Sources.} The two source manuscripts and their READMEs are preserved unchanged
in \texttt{sources/}, namely
\begin{itemize}\raggedright
\item \texttt{10-noncommutative-hahn-\allowbreak obstructions.tex} and
\texttt{10-noncommutative-hahn-\allowbreak obstructions-README.md};
\item \texttt{11-radial-exponential-\allowbreak support-lifting.tex} and
\texttt{11-radial-exponential-\allowbreak support-lifting-README.md}.
\end{itemize}
The first was delivered as a $34$-page article with $27$ numbered statements; the second
as a $32$-page article.
...
(\labelcref{squat:nc:selfwriting}). Nothing under \texttt{sources/}, \texttt{code/} or
