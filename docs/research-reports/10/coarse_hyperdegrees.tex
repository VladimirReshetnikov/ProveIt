...
\emph{Formalization.} Theorem~\ref{thm:A} for $Z=\emptyset$, Corollary~\ref{cor:noleast}, and Theorem~\ref{thm:D} for the nonuniform coarse degrees are formalized in the Lean~4 library \path{CoarseDegrees} at the repository root. The budget forcing, Lemma~\ref{lem:bridge} and the construction of Section~\ref{sec:A} are proved there with nothing admitted, for an abstract decision relation on strings that covers both Cohen forcing for ranked sentences and convergence of Turing functionals; the classical inputs, including (H2)--(H5) as a single package, are admitted with their references. Theorem~\ref{thm:C}, Corollary~\ref{cor:infimum} and the relativization to $Z$ are not formalized. See \path{CoarseDegrees/README.md}.
