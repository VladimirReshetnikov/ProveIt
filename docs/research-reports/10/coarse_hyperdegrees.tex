...
All proofs are conventional and have not been refereed or formalized. Novelty has not been established. The facts about Cohen forcing over $L_{\omck}$ that the proof of Theorem~\ref{thm:A} uses are quoted from the literature, from memory; they are isolated in Section~\ref{sec:facts} so that they can be checked.
\end{assessment}
...
The following are standard; see \cite{SacksHRT}, Chapters II--IV, and \cite{Feferman}. They are quoted from memory and should be checked against those sources. All of them relativize to an arbitrary real $Z$, with $\omck$, $\KO$, $L_{\omck}$ replaced by $\omega_1^Z$, $\KO^Z$, $L_{\omega_1^Z}[Z]$ and ``hyperarithmetic'' by ``hyperarithmetic in $Z$''.
...
\item \emph{A cone dichotomy for $\leh$.} For arbitrary $X$ and $A\notin\Coreh(X)$, is $\{D\in\DX X:A\leh D\}$ meager in the prefix-density metric? The set is $\Pi^1_1$ and so has the Baire property; if it is nonmeager it is comeager in a ball around a finite modification of any prescribed $Z\in\DX X$, and translating by $Z$ turns the question into whether ``$A\leh E\oplus Z$ for comeager many density-zero sets $E$ in a ball'' implies $A\leh Z$. For Cantor space this is the classical theorem that a set hyperarithmetic in comeager many reals is hyperarithmetic (Feferman; Hinman; Thomason; Kechris). We expect it to hold for the computably presented Polish space of density-zero sets, which would give exact partners and perfect families for $\leh$ inside \emph{every} description class, as in \cite[Theorems~4.6 and~4.12]{Synthesis}. We have not verified the transfer.
...
\section{Status}
The proofs above are complete relative to the facts (H1)--(H5) and to the results of \cite{Synthesis} that are cited, which are themselves unrefereed. (H1)--(H5) are classical, but they were written from memory, and the exact formulation of (H3) and (H4) in the sources should be compared with the use made of them in Lemma~\ref{lem:bridge} and in step (iii): what is needed is that forcing for the instances of a fixed ranked formula is uniformly $\Dh$, and that sets hyperarithmetic in a generic real are defined by ranked formulas. Remark~\ref{rem:variants} and question (Q1) contain unproved claims and are marked as such. Nothing here has been formalized; the Lean library \path{CoarseDegrees} does not contain hyperarithmetic notions. No claim of novelty is made.

\begingroup\raggedright\small
\begin{thebibliography}{9}
\bibitem{Synthesis}
\emph{Coarse Classes Without Least Turing Degrees: Deduplicated results of the nine research reports on target C1}. 18 September 2026. \path{docs/research-synthesis/Turing_Degrees_Synthesis.tex}.
\bibitem{HJKS}
...
\bibitem{Plan}
\emph{Turing Degrees: A Unified Research Report}, second edition, 18 September 2026, Section 12. \path{docs/research-plan/turing_degrees_unified.tex}.
\end{thebibliography}
\endgroup
\end{document}
