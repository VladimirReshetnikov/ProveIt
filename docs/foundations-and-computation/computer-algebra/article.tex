...
and executed example tests are distinguished throughout. The three original
source packages supplied finite checks, with no Lean files, formalization
blueprints, proof-assistant audits, or pinned Lean dependencies. Current
repository formalization coverage is recorded separately in
\path{docs/FORMALIZATION.md}; it is not supplied by these prototype tests.
No universal surreal simplifier, complete
...
\[
 A=\frac1{1-t},\qquad
 B=1+t+\cdots+t^9+\Ov{10},\qquad
 C=\operatorname{StrongSum}(n\mapsto t^n).
\]
Here $v(t)=1$ and $\Ov{10}$ means an error of valuation at least $10$, as in
\cref{cas:sub-ovconvention}. $A$ names one exact element; it is a finite exact field expression whose
...
be supplied.\\[0.4em]
Finitely described Hahn data, possibly relative to a fixed oracle & A countable
collection of exact values for each fixed oracle & No universal equality, support validation or finite cutoff
materialization.\\
\bottomrule
\end{longtable}
\endgroup
...
capability-tiers archive shipped unchanged copies of SA~(2) and TR inside its
own delivery, so only its provenance was independently inspectable from the
delivery itself. No supplied manuscript is redistributed with this report; which
manuscript, and which revision, supplies each imported result is recorded
throughout the text. Those same two manuscripts were also supplied to two other
merged reports in this collection: TR is the canonical-angles-and-oscillation
manuscript behind
\begin{sloppypar}\noindent
\emph{Trigonometry on the Surcomplex Plane}
(\path{docs/surcomplex/trigonometry/}), and SA~(2) is, after CRLF
normalization, the infinitesimal-calculus-and-contour-theory manuscript behind
\emph{Surcomplex Analysis: Holomorphic Functions on $\mathrm{No}[i]$}
(\path{docs/surcomplex/analysis/}).
...
None of the three original source packages supplied a Lean formalization or
pinned proof-assistant dependencies. Their finite symbolic checks, recorded in
\cref{cas:sec-executed}, do not establish general theorems in Lean. The containing
repository now maintains a separate implementation and coverage ledger in
\path{docs/FORMALIZATION.md}; that ledger, rather than the historical package
inventory, determines which source claims have checked counterparts.
...
\end{proposition}
\begin{proof}
Use exponents in $\Q^2_{\mathrm{lex}}$ and let $a_n$ be the first-halt
coefficients $b_M(n)$ of Reduction~B, with halting steps indexed from $1$; their
support is empty or a singleton, which is what makes the object below finite.
(Reduction~A's coefficients are cumulative --- $a_n=1$ once $M$ has halted
\emph{within} $n$ steps --- so they would give infinitely many terms. The
valuation and sign conclusions would survive, but the count below would not.)
Put
\[
...
represent exactly the numbers with finite sign expansions, namely the dyadic
rationals. Different cut trees can represent the same dyadic value.
...
field together with enough data to select one root. For nonzero $P$, a bare
equation $P(z)=0$ denotes a finite root set, not one selected number; the zero
polynomial imposes no condition. Valid selectors include an
...
independent over $\Z$ (no nonzero integer exponent vector sums to zero).
Then $k(X_1,\dots,X_d)$ embeds in $k((t^{\Gamma}))$,
...
\end{equation}
the last when $k$ is ordered, where $\sgn$ is the sign in $k$. On the Hahn
valuation ring $v(f)\ge0$, the residue in $k$ is $0$ if $v(f)>0$ and
$\lc(f)$ if $v(f)=0$. If, in addition, $k$ is embedded in the ordinary reals
or complexes and the exponent group is interpreted by Conway monomials, this
residue is the ordinary standard part: $v(f)<0$ then means $f$ is infinite
and has no finite standard part. For arbitrary $k$ the residue assertion makes
no claim about ordinary finiteness. The zero case has residue and standard
part $0$ when applicable, with $v(0)=+\infty$ as a distinguished marker.
\end{theorem}
\begin{proof}
Distinct integer exponent vectors give distinct values of $\sum n_j\beta_j$ by
integer independence, so a nonzero polynomial in the $X_j$ maps to a
...
\eqref{cas:eq-leadingrational}; the Hahn ordering gives the sign statement.
For $v(f)\ge0$, extracting the zero-exponent coefficient is the residue map:
it vanishes at positive valuation and is the leading coefficient at valuation
zero. With ordinary coefficients the positive-valuation tail is infinitesimal,
which gives the stated standard-part interpretation. No infinite expansion is
required by any of these algorithms.
\end{proof}
...
\cref{cas:thm-core}, with an effective coefficient subfield of the ordinary
reals and an ordered additive embedding of its exponent group in $\No$.
Interpret its monomials by that embedding. Let $F$ be its relative real
...
and field operations. Every nonconstant univariate polynomial over $C$ splits
into linear factors in $C$. Semialgebraic sentences over these effective coefficient
...
consequently every nonzero polynomial over it factors there and no additional
roots appear in a larger field extension. For a nonzero polynomial, root
specifications can also be obtained by writing its equation in real and
imaginary coordinates and selecting among its finitely many solutions. A
nonzero constant has no roots; the zero polynomial vanishes on all of $C$ and
is represented by that whole solution set, not a finite root list. For a polynomial system the same real-algebraic
...
geometric terms, indexed $0,\dots,N-1$, the remainder has valuation $N\lambda$
and \emph{never}
...
entire purely infinite part as a finite list. The ring $\Pi\oplus\Z$ is Conway's
ring of omnific integers (\nolinkurl{foundations-and-computation/foundations},
\texttt{found:eq:omnific}), so this floor is the omnific floor; its arithmetic
and notations are developed in the omnific-integer reports of the collection.
...
recursion gives
\begin{align*}
 \alpha_+&=t+\tfrac12t^3-\tfrac38t^5+\Ov{7},\\
 \alpha_-&=-t+\tfrac12t^3+\tfrac38t^5+\Ov{7},\\
 \alpha_0&=-t^3-t^7+\Ov{11} .
\end{align*}
...
Two companion reports in this repository bear on the boundary above. The
rigidity theorem for class functions on all of $\No[i]$ is stated in
\path{docs/surcomplex/analysis/}; the sharpness witness below is recorded not
there but in \path{docs/foundations-and-computation/foundations/}, whose
worked example carries the support proof. Over the \emph{fixed} value
...
\[
 \arctan\omega=\frac\pi2-\arctan t
 =\frac\pi2-t+\frac{t^3}{3}-\frac{t^5}{5}+\frac{t^7}{7}+\Ov{9},
\]
...
finite polynomial arithmetic. The engine \texttt{RationalHahn} and
\texttt{HahnRational} actually use; \texttt{SurrealCASCore} uses none of
these, its sparse layer being a hand-built canonical map and its fraction
wrapper deliberately unreduced. &
...
\textbf{Session, in this kernel's own convention.} The opening
\texttt{Get} succeeds only against the one-character repair of \S18.3; the
file as delivered does not parse.
\begin{lstlisting}
Get["code/RationalHahn.wl"];   (* after the repair of 18.3 *)
...
\label{cas:rem-10000}
The suite constructs both inputs explicitly:
\begin{lstlisting}
RHCompare[RHMake[s,1,{t,s}],
          RHMake[t^10000,1,{t,s}]] === -1
RHSign[RHMake[t^10000-s,1,{t,s}]] === 1
\end{lstlisting}
These are finite higher-rank order checks. Bare symbolic expressions are not
arguments of \texttt{RHCompare} or \texttt{RHSign}; the kernel has no automatic
coercion from them. \emph{The checks are not the proof for every $n$}; the universal statement
...
\textbf{Not implemented} (stated in the package comment, the README, the test
report and the article, though no two of them phrase the list identically and
none of them mentions standard part, which is recorded above as absent by
inspection rather than by the package's own disclosure): arbitrary surreal cuts,
algebraic coefficient extensions, algebraic root isolation and selectors,
infinite series generators, certified analytic domains, general Hahn-coherent
functions, the transserial
...
different parent}. \textbf{Confusing the conventions can reverse comparisons
between scales.} For example, $(1,0)$ and $(0,1)$ exchange order when coordinate
significance is reversed, whereas $(1,1)>(0,0)$ holds in both orders.
SurrealCASCore agrees with HahnRational on
...
\begin{itemize}
\item \textbf{RationalHahn}: the fully constructed calls in
\cref{cas:rem-10000} give comparison $-1$ and difference sign $1$.
...
where the suite defines \texttt{h[e\_] := HCreate[\{u,t\},e]}.
\end{itemize}
...
An audit of this merge against its three sources found three scope disclaimers
stated in a source and missing from the merged text. They are restored here.

\begin{itemize}
\item \textbf{Imported proofs were not re-audited.} This report does not
re-audit the companion analytic-geometry manuscript's Noetherian, Jacobson or
deformation proofs, nor the contour manuscript's general
residue-transformation argument. Both are leaned on --- the first for the
normal-form and perturbed-residue material, the second for the torus formula
--- and their computational consequences are conditional on those stated
results \emph{and} on effective input operations. ``Not independently
machine-verified'' is a weaker statement than this one, and was the only form
the merged text carried.
\item \textbf{Every imported algorithm would still need its own review.} A
complete implementation would require a separate engineering and proof review
of each algorithm imported here, beyond anything the executed checks establish.
\item \textbf{An umbrella layer inherits no guarantees.} A general symbolic
layer can compose expressions freely, but it cannot claim the algorithmic
guarantees of its strongest subdomain for every expression it contains. The
architecture proposed in \cref{cas:sec-architecture} is precisely such a layer,
which is where the prohibition most needs stating.
\end{itemize}
...
algebraic coefficient fields, algebraic root selectors, infinite series
generators, certified analytic domains, arbitrary Conway cuts and the transserial
layer; it also has no \emph{standard part}, which is an omission observed by
inspecting its exported symbols rather than one the package declares. HahnRational omits general Hahn coefficient
...
\textbf{None of the three original packages supplied a Lean file, formalization
blueprint, formalization audit, or pinned proof-assistant dependencies.}

\subsection{What was independently reproduced for this report}
\label{cas:sub-reproduced}
The following is the merged report's record of a separate Windows run on
September 21, 2026, distinct from the three archives' recorded runs. It is
preserved as historical verification evidence, not a claim about the current
review environment.
...
The Windows record reports matching check counts once
\texttt{RationalHahn} is repaired; as delivered that package loads in no
kernel at all, so the 22 rational-kernel checks its archive records cannot be
run from the shipped file (\S18.3). The quoted sessions of
...
package as delivered. The recorded Windows run of the unrepaired driver returned
...
collection ships no checksum manifests.

\textbf{SurrealCASCore and HahnRational were recorded as loading cleanly}, with
their harness counts matching the archives. Their source files are also
bracket-balanced, though that syntax check alone does not establish a successful
Wolfram run.
\end{remark}

\subsection{What the executed checks do and do not establish}
\label{cas:sub-checkscope}
During the maintained documentation review, fresh temporary copies of the two
unchanged Python scripts passed their separate suites under Python~3.13.14 and
SymPy~1.14.0: $69/69$ for SurrealCASCore and $323/323$ for HahnRational. The
Wolfram sources were inspected for their public contracts and the documented
packaging defect; no new Wolfram execution is claimed by this review.
...
versions other than 15.0.1, and with Mathics, was not tested in those runs.}
The original packages supplied no proof-assistant verification. Consult
\path{docs/FORMALIZATION.md} for the containing repository's current Lean
coverage, which must be matched to each statement and its hypotheses.
...
Beside this article, the directory carries the executable files of all three
prototypes under \path{code/} and the recorded verification reports under
\path{data/}. \textbf{The files in \path{code/} and
\path{data/} ship verbatim as delivered and are not modified here} --- including
the defective \path{RationalHahn.wl} (see \cref{cas:app-defectnote}). The three
source manuscripts themselves are not redistributed with this report; the
disposition of each source result is recorded in the provenance material of
\cref{cas:sub-provenance} and in the per-result attributions throughout the
text.
...
archives and reported separately on Windows x86-64 at merge time); one date
(September 21, 2026);
...
formal verification of general theorems by these tests; and no Lean files,
blueprints, proof-assistant audits or pinned Lean dependencies in the three
original packages. These are historical package claims, not a description of
the containing repository's current formalization work. The
...
normalization, to the infinitesimal-calculus-and-contour-theory manuscript
behind \emph{Surcomplex Analysis: Holomorphic Functions on $\mathrm{No}[i]$}
(\path{docs/surcomplex/analysis/})
...
Byte-identical to the canonical-angles-and-oscillation manuscript behind
\emph{Trigonometry on the Surcomplex Plane}
(\path{docs/surcomplex/trigonometry/})
