...
what a future reader could actually check.

{\small
\begin{longtable}{@{}P{0.08\textwidth}P{0.25\textwidth}P{0.20\textwidth}P{0.34\textwidth}@{}}
\toprule
Audit & Lean files shipped & Import footprint & State\\
\midrule\endhead
...
\end{equation}
which M2 ships as a Lean one-liner proved \texttt{by ring} (uncompiled; see
\cref{found:sub:archives}). These are order-valued estimates, valid over every
...
typeset listings inside its PDF. None of the five files was compiled.
...
\begin{samepage}
\begin{lstlisting}
-- Subset-based variant (M2), shipped as Lean source, NOT compiled.
...
\begin{samepage}
\begin{lstlisting}
-- Predicate-based variant (M1), shipped as Lean source, NOT compiled.
...
Both are the machine-level form of \cref{found:prop:allcuts}, and neither has
been run (\cref{found:sub:archives}). A smallness test belongs in the statement
...
single sentence that governs everything above: three read-only inspections, zero
compilations, zero rebuilds, zero axiom audits, nothing kernel-checked; the
\LaTeX{} builds are the only executed step. The per-audit build evidence and the
...
environments.
\item No Lean file in any archive was compiled or kernel-checked.
\item The external Lean repository was not rebuilt.
\item No transitive \texttt{\#print axioms} audit was executed. The
\texttt{\#print axioms} commands present in M1's and M2's files are unexecuted
source, and their output is recorded nowhere.
...
\item Only the \LaTeX{} builds were actually executed. That is the only step
common to all three audits.
...
\item No claim in this report was kernel-checked.
