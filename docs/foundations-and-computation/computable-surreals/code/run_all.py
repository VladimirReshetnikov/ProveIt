...
def main() -> int:
    ...
    suites = {
        "A": ["code/original_A/verify.py", "--output", "results/A.json"],
        "B": ["code/original_B/verify.py"],
        "C": ["code/original_C/verify_examples.py"],
        "merged": ["code/verify_reconciliation.py", "--output", "results/merged.json"],
    }
    ...
    for name, args in suites.items():
        ...
        (results/f"{name}.txt").write_text(text, encoding="utf-8")
        status = "PASS" if code == 0 else "FAIL"
        detail: dict[str, object] = {"status": status, "return_code": code,
                                     "log": f"results/{name}.txt"}
        if code == 0 and name in ("A", "merged"):
            payload = json.loads((results/f"{name}.json").read_text(encoding="utf-8"))
