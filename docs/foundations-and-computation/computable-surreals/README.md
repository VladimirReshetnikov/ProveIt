...
- `report.pdf`: the 50-page unified report, with linked contents and bibliography.
- `report.tex`: complete editable LaTeX source; no external bibliography or graphics.
...
Alternatively run `pdflatex -interaction=nonstopmode -halt-on-error report.tex`
...
all subprocesses, records logs under `results`, and exits nonzero if a suite fails.
...
| Reconciliation | 4,321 exact cross-model checks | PASS |
...
`4896a2808ce30e01b1c86ae3ba2295a64762d246`. No repository changes or local Lean
build were performed. The report distinguishes the existing mathematical
infrastructure from the effective-name algorithms proposed here.
