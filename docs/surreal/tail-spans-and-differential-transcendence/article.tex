...
is claimed solved. The article is not independently refereed or
proof-assistant verified. Accompanying exact computations test finite
identities and examples, not the infinite theorems.
\end{minipage}
\end{titlepage}
...
\section{The research question and the contributions}\label{sec:intro}
...
Theorem~\ref{thm:planes} then identifies the full relation ideal after its
...
(Theorem~\ref{thm:mixed}). Theorem~\ref{thm:continuumanalytic} supplies
...
\eqref{eq:intromain}, its elementary polarization proof, and the explicit
...
\section{Hahn fields, coefficient extensions, and summation}\label{sec:hahn}
...
\begin{lemma}[Finite base change]\label{lem:basechange}
...
\begin{warning}\label{warn:infinitebase}
...
the union of the finite constant extensions from Lemma~\ref{lem:basechange};
a Hahn series on the left can involve coefficients generating an infinite
extension. Section~\ref{sec:coefficient} makes the obstruction exact, and
Section~\ref{sec:surreal} shows that the failure can have maximal
...
\begin{lemma}[Independent sign changes]\label{lem:signs}
For finite $J\subseteq I$, the extension $M(r_i:i\in J)/M$ has degree
...
\begin{remark}[No order-preserving automorphism is asserted]\label{rem:noorder}
...
\section{The exact coefficient-field criterion}\label{sec:coefficient}
...
\begin{theorem}[Coefficient-field criterion]\label{thm:coefficient}
...
property for algebraic closures in separable characteristic zero
\cite{StacksGalois}. Distinct embeddings differ on a coefficient of $f$,
...
so $F=F_*$ is finite. Formula~\eqref{eq:relativeunion} follows immediately.
\end{proof}
...
Hahn base change. Section~\ref{sec:surreal} proves the substantially
...
\section{Cofinite spans and the polarization mechanism}\label{sec:mechanism}
...
\begin{lemma}[Stabilization and exceptions]\label{lem:stabilization}
...
\begin{lemma}[Cofinite spanning detects multilinear forms]\label{lem:multilinear}
...
\begin{lemma}[Top-degree mixed difference]\label{lem:difference}
Let $P\in K[X_1,\ldots,X_d]$ have total degree $D$, with homogeneous
part $P_D$. For vectors $u_1,\ldots,u_D\in K^d$, define
...
\end{lemma}
\begin{proof}
Taking one difference lowers total degree by at least one. Its highest
remaining homogeneous part is the directional derivative of the previous
highest part. After $D$ differences, terms of degree below $D$ have
vanished, and only the $D$ successive directional derivatives of $P_D$
remain. This proves the first equality. Expanding
...
\begin{proposition}[Sign-orbit independence]\label{prop:orbit}
Let $B=M((t^\Gamma))$, let $(a_i)_{i\in I}$ have independent square
classes, and let $F\subset I$ be finite. Set
...
Lemma~\ref{lem:difference}, it is
\begin{equation}\label{eq:polarzero}
...
all distinct indexed tuples. Lemma~\ref{lem:multilinear} gives $T=0$,
...
\section{The tail-span classification}\label{sec:classification}

\subsection{The generated algebra and its algebraic part}
\begin{theorem}[Exact tail-span classification]\label{thm:tail}
...
\begin{equation}\label{eq:algebraclassification}
 B[Y_1,\ldots,Y_r]=B_E[s_1,\ldots,s_d],
\end{equation}
and are algebraically independent over $B_E$. In particular,
\begin{align}
 B(Y_1,\ldots,Y_r)&=B_E(s_1,\ldots,s_d),\label{eq:fieldclassification}\\
 \trdeg_B B(Y_1,\ldots,Y_r)&=d,\label{eq:trdegree}\\
 [B_E:B]&=2^{|E|}.\label{eq:exceptiondegree}
\end{align}
The relative algebraic closure of $B$ in $B(Y_1,\ldots,Y_r)$ is exactly
$B_E$.
\end{theorem}
\begin{proof}
By Lemma~\ref{lem:stabilization}, $E$ is finite and the vectors outside
$E$ lie in $W$, with every cofinite subfamily spanning $W$. In coordinates
on $W$, Proposition~\ref{prop:orbit} proves algebraic independence of
...
infinite field $M$. Thus
\begin{equation}\label{eq:primitiveeta}
...
Lemmas~\ref{lem:basechange} and~\ref{lem:signs}. Every sign assignment
...
belong to this generated algebra, and the $s_j$, being $M$-linear
coordinates of $Y-p$, do as well. This proves the inclusion from right
to left in~\eqref{eq:algebraclassification}. The reverse inclusion follows
...
\begin{theorem}[Relation ideal as disjoint affine planes]\label{thm:planes}
In Theorem~\ref{thm:tail}, let
\[
...
\end{theorem}
\begin{proof}
Theorem~\ref{thm:tail} identifies the coordinate algebra over $B$ with
...
\section{Examples, exact relations, and finite witnesses}\label{sec:examples}
...
\begin{equation}\label{eq:quadraticexample}
...
subfamily has cofinite span $M^r$. Theorem~\ref{thm:tail} gives
\[
 \sum_n t^nr_n,\quad
...
\begin{corollary}\label{cor:finitewitness}
In Proposition~\ref{prop:orbit}, suppose $d>0$. For any degree $D\geq1$,
...
Equation~\eqref{eq:diffidentity} then gives a nonzero alternating sum of
...
information. Section~\ref{sec:effective} makes that distinction explicit.

\section{Actual surreal numbers of maximal differential independence}\label{sec:surreal}

\subsection{A specified differential Hahn workspace}
Let $\AR=\overline\Q\cap\R$ be the countable field of real-algebraic
numbers. Via~\eqref{eq:embedding}, regard
\begin{equation}\label{eq:realworkspaces}
...
The normalized Berarducci--Mantova derivation has real constant field,
is strongly additive, and satisfies $\BM\omega=1$~\cite{BM}.
These imported facts determine its restriction to the rational-monomial
workspace. Differentiating integer powers and then the equation defining
a rational power gives
\begin{equation}\label{eq:BMrestriction}
 \BM(t^q)=-q t^{q+1},\qquad
 \BM\big|_{H_0}=-t\EDer.
\end{equation}
Thus both $B_0$ and $H_0$ are differential subfields. Their constant
...
Apply Theorem~\ref{thm:tail} with $W=\Q^{K+1}$ and $E=\varnothing$.
...
of the whole family. Formula~\eqref{eq:BMtriangular} transfers this
...
not extend to $H_0$ or $\No$; Remark~\ref{rem:noorder} applies. Polynomial
...
\begin{equation}\label{eq:binarycode}
...
\begin{theorem}[Explicit maximal differential transcendence]\label{thm:surreal}
Put $\xi_\alpha=\xi_{A_\alpha}$ using~\eqref{eq:xiA} and~\eqref{eq:Aalpha}.
Then
\begin{equation}\label{eq:allrealjets}
 \{\BM^k\xi_\alpha:
...
\begin{equation}\label{eq:maxdtrdeg}
...
of the full combined family is $\Q^{r(K+1)}$. Theorem~\ref{thm:tail}
therefore proves independence of the combined finite jet tuple.
The triangular change~\eqref{eq:BMtriangular} gives the same result for
$\BM$-jets. Since any polynomial involves only finitely many sequences
and jet orders,~\eqref{eq:allrealjets} follows.

There are $\cf$ binary sequences, so this gives differential
transcendence degree at least $\cf$. On the other hand, $\AR$ and $\Q$
are countable. A Hahn series in $H_0$ is in particular a function from
$\Q$ to the countable set $\AR$, so $|H_0|\leq\aleph_0^{\aleph_0}=\cf$.
The independent family proves the reverse cardinal bound and the upper
bound on both transcendence degrees. Finally, a differential polynomial
...
By Theorem~\ref{thm:coefficient}, it is the union of
...
\section{Analytic coefficient fields and two commuting derivations}\label{sec:analyticsetup}
...
\begin{lemma}[Independent analytic square classes]\label{lem:analyticclasses}
...
\begin{equation}\label{eq:analyticfields}
...
made only where justified in Section~\ref{sec:halos}. This distinction
...
\]
Define on $H$
\begin{equation}\label{eq:twoderivations}
...
\begin{equation}\label{eq:mixedcoefficients}
 \partial^j\EDer^kF_A
   =\sum_{n\in A}t^nh_n(z)\,
             c_j\frac{n^k}{(z-2^n)^j}.
\end{equation}

\section{Every mixed jet is algebraically independent}\label{sec:mixed}
...
\begin{lemma}[Hereditary full jet span]\label{lem:jetspan}
...
\begin{equation}\label{eq:jetlinearrelation}
...
Lemma~\ref{lem:expgraph}, because $A$ is infinite. The same proof covers
$j_*=0$: the polynomial identity itself may simply be evaluated.
\end{proof}

\subsection{The mixed-jet theorem}
\begin{theorem}[Hereditary mixed differential independence]\label{thm:mixed}
For every infinite $A\subseteq\N_{>0}$, the family
\begin{equation}\label{eq:mixedindependence}
...
$0\leq j\leq J$, $0\leq k\leq K$. By~\eqref{eq:mixedcoefficients}, the
vector of all jets in the rectangle has the form of
Theorem~\ref{thm:tail}, with $r_n=h_n$, $\gamma_n=n$, and coefficient
vectors from Lemma~\ref{lem:jetspan}. Lemma~\ref{lem:analyticclasses}
supplies square-class independence, and Lemma~\ref{lem:jetspan} supplies
...
is injective exactly when the family~\eqref{eq:mixedindependence} is
...
From~\eqref{eq:hnjet}, coefficientwise Taylor extraction yields
\begin{equation}\label{eq:taylorrational}
...
\begin{equation}\label{eq:formaltaylor}
...
\begin{warning}\label{warn:taylor}
For an ordinary nonzero complex value of $z$, expression
\eqref{eq:formaltaylor} is \emph{not} a Hahn sum indexed by $j$:
...
There is also no contradiction between~\eqref{eq:taylorrational} and
Theorem~\ref{thm:mixed}. The theorem is about functions in $z$ over the
...
Section~\ref{sec:surreal}, their point values are not being asserted
transcendental over that field.

\section{A continuum of jointly free surcomplex analytic jets}\label{sec:analyticcontinuum}

Use the almost disjoint sets $A_\alpha$ from~\eqref{eq:Aalpha} and define
\[
...
\label{thm:continuumanalytic}
The family
\begin{equation}\label{eq:allanalyticjets}
...
jet coefficient vector of Lemma~\ref{lem:jetspan} in block $a$ if
...
$a$. After any finite deletion, Lemma~\ref{lem:jetspan} shows that these
vectors span the entire block. The cofinite span of the combined vectors
is consequently the full combined jet space. Theorem~\ref{thm:tail}
proves algebraic independence of the chosen finite tuple, and hence of
all of~\eqref{eq:allanalyticjets}.
...
\section{From coefficient functions to surcomplex halos}\label{sec:halos}
...
Define
\begin{equation}\label{eq:halovalue}
 F_A(z_0+\varepsilon,t)
  =\sum_{n\in A}\sum_{\ell\geq0}
       t^n\frac{h_n^{(\ell)}(z_0)}{\ell!}\varepsilon^\ell.
\end{equation}

\begin{proposition}[Joint summability of the halo formula]\label{prop:halo}
Formula~\eqref{eq:halovalue} is a Hahn-summable double family and gives
...
\begin{proposition}\label{prop:functionalmeaning}
No nonzero polynomial over $K_\Q(z)$ in finitely many of the mixed jets
of Theorem~\ref{thm:continuumanalytic} vanishes identically on their
...
Theorem~\ref{thm:continuumanalytic}. In fact it lies in
...
In Section~\ref{sec:surreal}, $\BM$ acts on actual surreal numbers and
...
before such substitutions; Section~\ref{sec:surreal} separately supplies
a direct, normalized Berarducci--Mantova result for actual numbers.

\section{Algebraic approximation depends on the value group}\label{sec:approx}

Theorem~\ref{thm:coefficient} also gives a sharp approximation criterion.
...
Theorem~\ref{thm:coefficient}, proving necessity.

Conversely, suppose~\eqref{eq:prefixcriterion}. The truncation
...
For a contrasting example, take $\Gamma=\Z^2$ in lexicographic order and
\[
...
criterion fails. No algebraic element approximates $f$ with error
valuation greater than $(1,0)$.
...
\section{Effective limitations and exact verification}\label{sec:effective}
...
Theorem~\ref{thm:onesurreal}, and in particular transcendental.
...
Corollary~\ref{cor:finitewitness} require structural information about the
...
The supplied \texttt{verify.py} uses exact arithmetic and symbolic
...
Lemma~\ref{lem:expgraph}. Finite sign-orbit examples do not prove the
...
No Lean file is included, and no machine-checked theorem is claimed.
...
\section{Further questions and final assessment}\label{sec:questions}
...
problem of Proposition~\ref{prop:undecidable} does not rule out such
structured subclasses. Corollary~\ref{cor:finitewitness} suggests a
...
The Stacks Project Authors, \emph{Fields, Section 9.21: Galois theory},
...
needed here is proved in Lemma~\ref{lem:signs}.
