...
$\Gamma_a$ agrees with the finite Taylor extension of the real Gamma function,
...
flatness to every algebraic order. At $x=\omega$,
...
$\varepsilon$ an ordinal epsilon-number, as in \cite{vdDE}.
...
\end{equation}
where $S$ is a reverse well-ordered set of surreal exponents. We write
\[
...
sequence in the fine surreal order topology. Throughout the paper a displayed
infinite sum has the strong, coefficientwise meaning just specified.
...
\end{equation}
The gauge is real-valued even though the function's domain is surcomplex.
...
in particular $\Log(nz)=\log n+\Log z$. The real gauge cancels because
$h_a(x+1)=h_a(x)$ and
...
finite, for every real-valued gauge \eqref{eq:complex-gauge}.
\end{lemma}

\begin{proof}
Re-expand \eqref{eq:complex-infinite} around $x$ in the ratio $iy/x$.
All coefficients at a real-surreal $x$ are real, and the imaginary part
...
is finite. The real gauge changes none of this. At infinite real part,
...
\item if $x,y$ are positive infinite and $y/x\sim1$, then
\[
...
The repository inspection used the tree with observed commit
\begin{center}\small\commit\end{center}
and the documentation catalogue \cite{Repo}. The catalogue lists eighteen
...
No Gamma-specific research package was identified in that catalogue.
...
The following commands rebuild the article and reproduce the checks from the
package directory:
\begin{verbatim}
...
\end{verbatim}
The supplied \texttt{Makefile} runs the same operations. The LaTeX source has
an internal bibliography and no external graphics or custom font files.
