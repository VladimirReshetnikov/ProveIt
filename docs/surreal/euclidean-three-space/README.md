...
Research report, 22 September 2026, built from three manuscripts written
independently on the same day: 04 (the base, Part I) and 09 (Part II), merged
at `50cb709`, and 10 (Part III), added at `7b5f934`.
...
09 carries the sub-prefix `e3:so:`, and material from source 10 the sub-prefix
`e3:cut:`. When sources 04 and 09 were merged the report was new, so no earlier
...
ones all `e3:cut:`. The [formalization ledger](../../FORMALIZATION.md) lists
40 statement labels of Parts I and II as pending; no label of this report has
a Lean mapping.
...
22 for source 09 (S1–S22), 18 for source 10 (C1–C18), and 6 for the merge and
the addition (M1–M6). The main ones follow.
...
- The report is AI-assisted and has not been refereed. No statement has a
  ...
  At pin `50cb709`, and still at `7b5f934`, the repository has no
  three-dimensional Euclidean or rotation-group Lean module.
...
- Only `SO(n)` with `n ≥ 3` and `SU(n)` with `n ≥ 2` are treated, in fixed
  rank. There is no higher-rank normal-subgroup classification, no uniform
  reductive theorem, and no higher-rank commutator width at prescribed depths.
  The questions of Section 38.1 remain open.
...
The build gives 96 pages with zero errors, zero warnings, zero overfull or
