...
formulas, and a tree-and-spine construction with integer-valued markers. Both
can be made planar with maximum degree at most three. For the single value
$1/2$ the exact
...
identifiers, never cached rationals. The bundled \code{verify\_sparse.py} suite
applies it to $G$ and $\can(1/2)$: it verifies both $G\leq\can(1/2)$ and
$\can(1/2)\leq G$, and checks all $36$ ordered comparisons among their six
distinct subforms against exact rational ordering. This is a structurally
...
For a finite-birthday value $x$, list its sign-prefix values by length:
\begin{equation}
...
canonical form matters. Integer values follow directly from their unary chains;
negation preserves rank, so it remains to consider $x\in(n,n+1)$ with $n\geq0$
and $\kappa(x)=k\geq1$. For $k=1$ the two
...
rank-minimal and still fails containment even under the unlabelled convention,
so it also rules out any stronger subgraph requirement preserving roots,
directions, option sides, or values. Questions about minors or restricted
arithmetic presentations are different problems; see \cref{q:minors,q:arithmetic}.
The transfinite example of \cref{prop:omega-highgirth} shows that at a limit
...
\section{Arbitrarily large girth for every finite-birthday value}
...
Let $x>0$ be dyadic. Recursively unfold $D(\can(x))$ by giving every occurrence
of an option its own tree vertex, even when that form occurs elsewhere. This produces
...
Enumerate the leaves as $0,1,\ldots,L-1$ in any fixed order; a left-before-right
traversal will also give planarity. Choose a positive
...
For a positive dyadic $x$ and a positive integer $M$ satisfying
\eqref{eq:spacing}, the form $A_{x,M}$ has value
...
\end{equation}
In every leaf order the maximum degree is at most three. If the leaves are
enumerated by a left-before-right traversal, the graph is also planar.
\end{theorem}
...
Every internal tree vertex has at most two children and one parent. Each
attachment has one tree parent and at most two spine neighbours; all other
spine vertices have degree at most two. Thus the maximum degree is at most
three. For planarity, draw the ordered occurrence tree above a horizontal
line with its leaves on the line in their traversal order. Such a drawing is
obtained recursively by placing child subtrees in disjoint horizontal
intervals; unary vertices cause no difficulty. Draw the spine between
consecutive leaves below the line, inserting its intermediate vertices along
each segment. The chosen leaf order prevents crossings.
\end{proof}
...
\cref{thm:high-girth} already gives arbitrarily large girth at every
finite-birthday value, with exact vertex, edge, cycle-rank, and rank formulas.
Its ordered-leaf version is planar and has maximum degree at most three. This
section gives a second construction with those graph properties: it stops the
canonical tree at integer leaves, attaches them to an integer-valued spine,
and uses a different proof of non-collapse. Its spacing requirement depends
only on the requested girth, whereas the first construction requires $2M>b(x)$.
The resulting size and rank bounds are expressed in the integer leaf labels
and the denominator exponent.
...
leaf. The construction below separates them by \emph{integrality} --- internal
vertices are nonintegers, spine vertices are integers --- followed by a descent
through a singleton-option template. Each proof must establish that distinct
occurrences remain distinct forms: without that step, the graph
...
Given a nonempty finite list of integers $n_1,\ldots,n_\ell$ and an integer
$d\geq1$, there is a unary
...
occurrence labelled $r$ is determined by $r$ alone. These occurrences cannot
be ancestors of one another, since the denominator exponent strictly decreases
along internal edges. Their descendant subtrees are therefore disjoint.
Since each rebuilt left and
...
\eqref{eq:output-bound} are stated in $I$, $L$, $b(x)$, and $M$. The cycle rank
$\ell-1$ here equals $L-1$ there: extending each integer leaf by its canonical
unary chain to zero does not change the leaf count. Neither construction is
...
\item \emph{A recursive comparison oracle.} A separate implementation of
 ...
 identifiers, with no cached rationals at all. The bundled suite verifies
 $G\leq\can(1/2)$ and $\can(1/2)\leq G$, and checks all $36$ ordered
 comparisons among their six distinct subforms against exact rational ordering.
 Unlike (i) it never computes a value, so it tests the order-theoretic content
 of this witness rather than the arithmetic kernel.
...
(the other four have value $-1/2$). Every triangle-free noninteger form fails
canonical-subgraph containment, since every noninteger canonical graph has a
triangle by \cref{lem:canonical-triangle}. The converse fails, as witnessed by
the girth-three counterexamples above. Thus the eight forms satisfying the
triangle-free condition form a proper subset of the sixteen containment failures.
...
finite-birthday surreal value, and that the sparse representations can be
taken planar with maximum degree at most three. For the single value $1/2$ the cost of
...
vertices for every integer $g\geq5$. See \cref{rem:cross-girth}. The
counterexample also defeats stronger subgraph conventions; minor containment
and restricted presentations require separate analysis. The optimal girth
cost remains open here for general dyadics.
...
representations of every finite-birthday value can avoid all cycles below an
arbitrary length, and can be taken planar with maximum degree at most three;
at $1/2$ the exact price of girth at least $g$ is $g$ vertices for integers
$g\geq5$. On the rigid side, canonical prefix values
