...
Every label in `article.tex` carries the prefix `odg:` (150 labels).
The earlier base text was already indexed in the formalization ledger under
its unprefixed labels; this rewrite prefixes and consolidates those labels.
The [formalization ledger](../../FORMALIZATION.md) now indexes the rewritten
text with its current labels and source positions. All omnific-integer
statements remain pending in Lean. Further manuscripts placed after this
three-source writing pass are not yet integrated into the text.
...
10. **Families and primitivity (Section 11).** Polynomial lifting (Proposition
    ...
    finite-support solutions specialize to polynomial arcs (Theorem 11.7).
