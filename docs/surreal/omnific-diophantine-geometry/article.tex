...
inspection is pinned to the commit recorded in Appendix~\ref{odg:app:audit}.
\end{minipage}
\end{titlepage}
\tableofcontents
\newpage

\section{Scope, results, and mathematical conventions}\label{odg:sec:scope}
...
The structural results in Sections~\ref{odg:sec:basic}--\ref{odg:sec:class} explain
...
(\cref{odg:thm:binary,odg:thm:norm}).
...
parameterized infinite family (\cref{odg:thm:quadclassification}).
...
$\Z^n$ inside $A^n$ (\cref{odg:thm:guard}).
...
it is rational (\cref{odg:prop:pythagoras,odg:thm:realray}).
\end{enumerate}
...
Quotients by class ideals are read through their quotient maps and
congruence relations. In particular, $A/\J=\Z$ means that $\ct$ is onto
$\Z$ and $x-y\in\J$ exactly when $\ct(x)=\ct(y)$; a residue is represented
by its ordinary integer value. For $0\ne n\in\Z$, the quotient $A/nA$
is represented by the ordinary residue ring $\Z/n\Z$. No proper-class coset is used as an element
of a set. Ideal generation uses finite sums, even when the family of
generators is a class. The notation $\Frac(A)$ below is interpreted through
fractions inside $\No$, not through a set of all pairs from $A$.
...
This is manuscript 01 of the three-source Diophantine report placed in
repository commit \texttt{be06fc8}. The present integration covers its basic
normal-form, residue and global-support arguments, compared with manuscripts
02 and 05. Additional consequences from those sources are identified below.
Their later Diophantine and differential results have not yet all been
integrated. The maintained \texttt{README.md} and \texttt{RECONCILIATION.md}
record this boundary; statements later in this base text retain their
original scope and attribution. Repository placement does not constitute
proof review or Lean formalization.
...
\section{The normal-form ring and its integer-part structure}\label{odg:sec:basic}
...
These are growth-exponent conventions. With $t^\gamma=\omega^{-\gamma}$,
the supports of $\BR$ and $\BC$ have only nonpositive $t$-exponents, and
$\J$ has only strictly negative $t$-exponents. This is a restriction on
every supported exponent, not just the leading one. In the complex ring,
the kernel of $\ct$ is $\J+i\J$, an ideal of $\BC$; it is not the real
ideal $\J$ alone.
...
\begin{proposition}\label{odg:prop:ring}
...
\begin{remark}[A frequently misleading analogy]\label{odg:rem:notvaluation}
...
The retraction is not order preserving: $\omega-1>0$ but
$\ct(\omega-1)=-1$. Thus its use in equational arguments does not transport
inequalities.
\end{remark}
...
For the same nonzero form in $t$-notation, $v(x)=-\ddeg(x)$.

\begin{lemma}\label{odg:lem:degree}
...
\begin{proposition}\label{odg:prop:units}
The unit groups are
\[
...
\begin{theorem}[Integer-part formula]\label{odg:thm:floor}
...
\subsection{A common set-sized workspace}

\begin{lemma}[Localizing the input supports]\label{odg:lem:workspace}
Every set-sized family of real or complex surreal normal forms belongs to
a common Hahn field with a set-sized divisible ordered exponent group
$H\subseteq(\No,+)$. One may take the rational span of the union of their
growth-exponent supports. The nonnegative-support subrings in this field
are the intersections with $\BR$ and $\BC$, respectively.
\end{lemma}
\begin{proof}
The support union is a set; all its finite rational linear combinations
form a set-sized divisible ordered subgroup $H$. A normal form with
support in $H$ is a Hahn series over $H$ with the growth order reversed.
Hahn field operations, including inverses, preserve this exponent group.
Intersecting with the nonnegative growth-support condition gives the
stated subrings. Real and imaginary parts use the same group.
\end{proof}

This is manuscript 05's workspace lemma, an application of the collection's
normal-form localization principle. It does not promise that a later
support bound lies in the chosen $H$: the common-divisor and clearing
arguments in Section~\ref{odg:sec:class} may require enlarging it.

\section{Divisibility, residues, and finite quotients}\label{odg:sec:residues}

\subsection{Division by ordinary integers}

\begin{theorem}\label{odg:thm:finitequotients}
For every nonzero ordinary integer $n$,
\begin{equation}\label{odg:eq:nA}
 nA=\J+n\Z,\qquad
...
can have only constant factors by \cref{odg:lem:degree}.

\begin{corollary}[Mixed gcds with an ordinary integer]\label{odg:cor:mixedgcd}
Let $0\ne n\in\Z$, $x\in A$, and let $d=\gcd(n,\ct(x))>0$ be the
ordinary positive gcd. Then the ideal generated by $n$ and $x$ is $dA$.
Consequently $d$ is a gcd of $n$ and $x$ in $A$. Every ordinary positive
prime $p$ is a prime element of $A$, and $pA$ is maximal.
\end{corollary}
\begin{proof}
Write $x=m+j$, where $m=\ct(x)$ and $j\in\J$. Since $j/n\in\J\subseteq A$,
the identity $x-n(j/n)=m$ gives $(n,x)=(n,m)$. Ordinary B\'ezout
coefficients and the divisibilities $d\mid n,m$ give $(n,m)=dA$.
Every common divisor divides the B\'ezout combination $d$, while $d$
divides both generators, so it is a gcd. Finally
$A/pA\cong\Z/p\Z$ is a field; hence $pA$ is maximal and prime.
\end{proof}

This adds manuscript 02's mixed-gcd statement and the ordinary-prime
consequence also present in manuscript 05. It concerns a pair with one
nonzero ordinary integer; arbitrary pairs need not have a gcd.

\begin{corollary}\label{odg:cor:charideals}
...
Equation~\eqref{odg:eq:nA} identifies its inverse image with $nA$.
...
The first inverse system is indexed by positive ordinary integers ordered
by divisibility, with reduction maps $\Z/n\Z\to\Z/m\Z$ when $m\mid n$.
The second uses the chain of powers of the fixed prime $p$. These are
set-sized diagrams of ordinary residue rings. The compatible quotient
identifications above justify the displayed limits without forming a set
of class ideals or a set of proper-class cosets.

\begin{proposition}\label{odg:prop:canonicalct}
...
\section{Global algebra: common divisors and the full fraction field}\label{odg:sec:class}
...
\begin{lemma}[Set-sized exponent bounds]\label{odg:lem:setbounds}
...
\begin{theorem}[Common monomial divisibility]\label{odg:thm:commondivisor}
...
\cref{odg:lem:setbounds}. Dividing by $\omega^\epsilon$ shifts every exponent
...
\begin{corollary}\label{odg:cor:Jglobal}
...
That factor would be reducible by \cref{odg:thm:commondivisor}.
\end{proof}
...
In particular, $\J^k=\J$ for every ordinary $k\ge1$, so
$\J/\J^2=0$ and the $\J$-adic inverse system is constantly
$A/\J\cong\Z$. Its completion is $\Z$, with kernel $\J$.
These consequences appear in manuscripts 02 and 05; a zero quotient
$\J/\J^2$ does not imply that $\J$ itself is zero.

\begin{corollary}[Maps into a nilpotent ideal]\label{odg:cor:nilpotent}
Let $\phi:A\to B$ be a ring homomorphism and $N$ an ideal of $B$ with
$N^k=0$ for some ordinary $k\ge1$. If $\phi(\J)\subseteq N$, then
$\phi(\J)=0$.
\end{corollary}
\begin{proof}
By \cref{odg:cor:Jglobal}, $\J^k=\J$. The image under $\phi$ of every finite
sum of $k$-fold products from $\J$ belongs to $N^k=0$, which proves the
claim. The image need not itself be an ideal of all of $B$.
\end{proof}

This is manuscript 02's nilpotent-test corollary. Its hypothesis concerns
the image of the whole ideal, not merely finitely many selected elements.

\begin{proposition}[A necessary residue condition for irreducibility]\label{odg:prop:irreduciblect}
...
\cref{odg:thm:commondivisor} is the existence of a common \emph{infinite
...
\begin{theorem}[Global monomial clearing]\label{odg:thm:fractions}
...
\begin{corollary}[Common multiples of every ordinary power]\label{odg:cor:commonmultiples}
Every set-sized family $(a_j)_{j\in T}$ of nonzero omnific integers has a
nonzero monomial $M\in A$ such that $M/a_j^k\in\J$ for every $j\in T$
and every ordinary $k\ge1$. Thus $M$ is a common multiple of all these
powers in $A$.
\end{corollary}
\begin{proof}
The family of inverses $a_j^{-k}$ is set-sized. Apply
\cref{odg:thm:fractions} to it and take $M=\omega^h$ from that theorem.
\end{proof}

This is manuscript 02's strengthened common-multiple statement. It does
not give one element divisible by every nonzero omnific integer: the
whole class of such integers is not an allowed index set.
...
\begin{corollary}\label{odg:cor:projectiveclear}
...
Section~\ref{odg:sec:primitive} will show that some real projective directions
...
\begin{proposition}\label{odg:prop:division}
...
\end{proposition}
\begin{proof}
Take $q=\lfloor a/b\rfloor_A$ from \cref{odg:thm:floor} and
...
\begin{proposition}[An explicit pair without a gcd]\label{odg:prop:nogcd}
...
Manuscript 05 gives a shorter witness to the same conclusion:
$\sqrt2=(\sqrt2\omega)/\omega\in\Frac(A)$ satisfies $T^2-2=0$ but is
not omnific. The proof above retains the base manuscript's witness with
nonconstant normal form; neither assertion depends on a factorization
classification.
...
it. Appendix~\ref{odg:app:audit} records why we make no current open-status
...
\begin{theorem}[Retraction theorem for equations]\label{odg:thm:transfer}
...
\begin{corollary}[Hilbert's tenth problem]\label{odg:cor:H10}
...
\end{corollary}
\begin{proof}
Equation~\eqref{odg:eq:existencetransfer} identifies the two decision problems
...
\begin{proposition}\label{odg:prop:notdiophantine}
...
homomorphism argument in \cref{odg:thm:transfer}. But $\omega$ is nonzero
...
\begin{theorem}[Smith normal form over the omnific integers]\label{odg:thm:smith}
...
possible precisely under \cref{odg:thm:finitequotients}. Zero rows require
...
\section{Rigidity of constant-product and norm equations}\label{odg:sec:rigidity}

\subsection{The constant-product principle}

\begin{lemma}\label{odg:lem:constantproduct}
...
Apply \cref{odg:prop:units}. Equivalently, their nonnegative degrees have
...
\begin{theorem}[Decomposable polynomial fibers]\label{odg:thm:decomposable}
Let
\[
...
\end{theorem}
\begin{proof}
By \cref{odg:lem:constantproduct}, every $L_j(\mathbf x)$ is constant.
...
\begin{theorem}[Binary-form rigidity]\label{odg:thm:binary}
...
vectors. The rank in \cref{odg:thm:decomposable} is therefore two.
\end{proof}
...
\begin{corollary}[Pell rigidity]\label{odg:cor:pell}
...
\begin{proposition}[The zero Pell fiber]\label{odg:prop:zeropell}
...
\begin{corollary}[Power-difference rigidity]\label{odg:cor:powers}
...
\cref{odg:thm:binary}. If a positive ordinary power of an element of $A$
...
\begin{theorem}[Norm-form rigidity]\label{odg:thm:norm}
...
The result now follows from \cref{odg:thm:decomposable}.
\end{proof}
...
\section{A complete dichotomy for nonzero quadratic levels}\label{odg:sec:quadratic}
...
\begin{theorem}[Quadratic-level dichotomy]\label{odg:thm:quadclassification}
...
\begin{lemma}[A polynomial quadratic isometry]\label{odg:lem:transvection}
...
\begin{lemma}\label{odg:lem:nullchoices}
...
two distinct linear factors over $\C$, so \cref{odg:thm:binary} applies.
...
For (iii), choose $e,u$ by \cref{odg:lem:nullchoices} and use
\cref{odg:lem:transvection}. Each coordinate of $T_t(a)$ is an integer
...
\section{Defining the ordinary integers by one quartic equation}\label{odg:sec:guard}
...
\]
implies
\begin{equation}\label{odg:eq:pellmod8}
...
\begin{lemma}[A three-square gap below a Pell coordinate]\label{odg:lem:pellgap}
...
\begin{theorem}[Uniform quartic integrality guard]\label{odg:thm:guard}
...
\cref{odg:prop:units}, each is an ordinary integer.

Conversely, suppose $\mathbf x\in\Z^n$ and put
$S=\sum_i x_i^2\in\N$. Choose an ordinary Pell solution and three
squares from \cref{odg:lem:pellgap}. They satisfy both equations in
\eqref{odg:eq:guardrelations}, and hence make $G_n$ vanish.
\end{proof}
...
\cref{odg:lem:pellgap}, not on these examples or on a finite computation.
...
There is no conflict with \cref{odg:prop:notdiophantine}. The copy of $\Z$
...
\begin{corollary}\label{odg:cor:Jdefinable}
...
integers, then applies \eqref{odg:eq:divkernel}. The second expresses the
...
The proof of \cref{odg:thm:guard} needs less than all surreal numbers.
...
\begin{proposition}[Finite polynomial lifting]\label{odg:prop:polylift}
...
\begin{theorem}[Homogeneous cone criterion]\label{odg:thm:cone}
...
All coordinates in \eqref{odg:eq:fermatscaling} are divisible by $2$ in
$A$, since they belong to $\J$. By \cref{odg:thm:commondivisor}, they
...
\begin{definition}\label{odg:def:primitive}
...
\begin{proposition}[Infinite unimodular Pythagorean triples]\label{odg:prop:pythagoras}
...
\begin{theorem}[Rationality of primitive constant directions]\label{odg:thm:realray}
...
\section{Obstructions from leading terms and bounded geometry}\label{odg:sec:obstructions}
...
\cref{odg:prop:notnormal}. The missing information resides in the full tail
of a potential root.

\subsection{No real point at infinity implies rigidity}

\begin{theorem}[Leading-homogeneous obstruction]\label{odg:thm:infinity}
...
$\mathbf r$ as in \cref{odg:thm:cone}. If $d_j=\deg F_j$, the coefficient
...
\begin{proposition}\label{odg:prop:squaresbound}
...
Use \cref{odg:prop:units}.
\end{proof}

\begin{corollary}\label{odg:cor:orthogonal}
...
\begin{corollary}[Symmetric matrix rigidity]\label{odg:cor:symmetric}
...
\]
so \cref{odg:prop:squaresbound} applies. The first two characteristic
coefficients give
...
$x^4-y^6=c\ne0$ & Only ordinary integer solutions.\\
$x^2-y^2-z^2=1$ & Infinite unimodular family from \eqref{odg:eq:hyperboloid}.\\
$x^2+y^2=z^2$ & Infinite unimodular family from \cref{odg:prop:pythagoras}.\\
$x^n+y^n=z^n$, $n\geq2$ & Positive infinite solutions in \eqref{odg:eq:fermatscaling}; these are not primitive.\\
...
\section{Fixed Hahn workspaces and possible formalization}\label{odg:sec:formal}
...
\section{Further questions and conclusions}\label{odg:sec:questions}
...
The scaled solutions in \eqref{odg:eq:fermatscaling} are never primitive.
...
\appendix
\section{Proof dependencies and verification boundaries}\label{odg:app:dependencies}
...
\section{Repository and source audit}\label{odg:app:audit}
...
trigonometry source-audit file identified in Appendix~\ref{odg:app:audit}.
