...
\]
and put
\begin{equation}\label{eq:summarydegrees}
...
The exact formula is proved in Theorem~\ref{thm:fiber}, using a buffer from
...
Theorem~\ref{thm:hahn} proves existence in every \emph{full}
...
Theorem~\ref{thm:algebraic}. For eventually periodic codes, existence of an
algebraic realization turns this conditional statement into an exact
dichotomy (Theorem~\ref{thm:periodiccriterion}).
...
\begin{equation}\label{eq:positivedegree}
...
Let $\OO$ be the ring of finite surreals and $\mm$ its infinitesimal ideal:
\[
 \OO=\{0\}\cup\{x:v(x)\ge0\},\qquad
 \mm=\{0\}\cup\{x:v(x)>0\}.
\]
...
\eqref{eq:KG} includes $G=\{0\}$, when $K_G=\R$.

The floor in Lemma~\ref{lem:floor} preserves $K_G$, since it deletes terms
...
\begin{definition}[Digits and complete quotients]
...
\begin{align}\label{eq:continuants}
...
\begin{lemma}[Matrix identities]\label{lem:matrix}
For every $n\ge0$,
\begin{align}
 \prod_{j=0}^{n}\begin{pmatrix}a_j&1\\1&0\end{pmatrix}
  &=\begin{pmatrix}p_n&p_{n-1}\\q_n&q_{n-1}\end{pmatrix},\label{eq:matrix}\\
 p_nq_{n-1}-p_{n-1}q_n&=(-1)^{n+1}.\label{eq:det}
\end{align}
...
\eqref{eq:continuants}; each factor has determinant $-1$. For the positive
...
with $k=0,1$, proves \eqref{eq:fib}. No Archimedean hypothesis enters.
\end{proof}
...
\begin{proposition}[Cylinder description]\label{prop:cylinder}
...
\begin{align}
 |x-r_n|&=\frac1{q_n(q_nt+q_{n-1})},\label{eq:distR}\\
 |x-s_n|&=\frac{t-1}{(q_n+q_{n-1})(q_nt+q_{n-1})}.\label{eq:distS}
\end{align}
Finally,
...
two fractional linear expressions and using \eqref{eq:det} proves
\eqref{eq:width}--\eqref{eq:distS}. The determinant also shows strict
...
\begin{corollary}[Realization in the full surreal field]\label{cor:realization}
...
\begin{lemma}[Uniform two-step buffer]\label{lem:buffer}
...
induction proves \eqref{eq:degrees}.

In \eqref{eq:distR}, the complete quotient satisfies
...
\eqref{eq:distR} proves \eqref{eq:exacterror}.
\end{proof}

\begin{definition}[Precision ideal]
...
\begin{theorem}[Exact fiber]\label{thm:fiber}
...
and a proper ideal of $\OO$, contained in $\mm$.
\end{theorem}
\begin{proof}
Suppose $x,y\in\F(\codea)$. For fixed $n$ and every $k\ge0$, both points
belong to $C_{n+k}$. Thus by \eqref{eq:width} and \eqref{eq:fib},
\[
...
by an element of $\OO$ can only increase valuation. Convexity follows
either from \eqref{eq:little} or from the valuation order. Finally $d_0=0$
implies $I_{\codea}\subseteq\mm$, so $1\notin I_{\codea}$.
\end{proof}

\begin{corollary}[No unique unrestricted surreal realization]
\label{cor:neverunique}
...
\begin{proposition}[Order-topological obstruction]\label{prop:topology}
...
We now answer a stronger question than Corollary~\ref{cor:realization}:
...
\begin{theorem}[Hahn realization and exact uniqueness]\label{thm:hahn}
...
Fix $n$. There is $k$ with $d_k>d_n$. By \eqref{eq:exacterror},
\[
...
Assertion (ii) is the intersection of \eqref{eq:fiber} with $K_G$.
...
\begin{remark}[The projection is not a homomorphism]
...
\begin{remark}[Why eventual constancy was separated]
...
\begin{corollary}[Countable cofinality threshold]\label{cor:cofinality}
...
Theorem~\ref{thm:hahn} gives a unique realization.
\end{proof}
...
\begin{corollary}[Convergence in the unique Hahn case]
If $G\ne\{0\}$ and the conditions for uniqueness in
Theorem~\ref{thm:hahn} hold, the continuants converge in the order topology
...
positive element of $G$. For $n\ge n_0$, \eqref{eq:exacterror} gives
...
\begin{proposition}[Prescribed degree cut]\label{prop:prescribed}
...
\end{proposition}
\begin{proof}
Every exponent in \eqref{eq:prescribed} is nonnegative, so each digit is at
least one and belongs to $\Oz$. Formula \eqref{eq:degrees} gives
...
$\gamma_n\in G$. Theorem~\ref{thm:hahn} itself has no such divisibility
requirement.

\begin{theorem}[Prime precision criterion]\label{thm:prime}
The ideal $I_{\codea}$ is prime in $\OO$ if and only if condition
\eqref{cond:P} holds. The same equivalence holds for
$I_{\codea}\cap K_G$ in $\OO\cap K_G$ when the digits lie in $K_G$.
If \eqref{cond:P} fails, there is an explicit element whose image in the
...
\eqref{cond:P} holds and $g,h\in D$, choose $n$ bounding both by $2d_n$
...
$4d_n\le2d_m$, which is \eqref{cond:P}.
...
\begin{corollary}[Radical and nilpotency index]\label{cor:radical}
For $0\ne e\in\OO$, its class modulo $I_{\codea}$ in a set-sized
...
Condition \eqref{cond:P} is thus more than a growth convenience. It says
...
Otherwise iterating \eqref{cond:P} produces an $m$ with
$d_m\ge M d_N$. It follows that $v(z)\le2d_m$, again excluding membership.
This proves \eqref{eq:fieldvisibility}.
\end{proof}

\begin{theorem}[Exact algebraic-fiber trichotomy]\label{thm:algebraic}
For every admissible code the following statements hold.
\begin{enumerate}[label=\textup{(\roman*)}]
\item If \eqref{cond:P} and \eqref{cond:V} both hold, then
$|A_{\codea}|\le1$.
...
\begin{equation}\label{eq:algtranslations}
...
Theorem~\ref{thm:fiber}. Lemmas~\ref{lem:algerror} and
\ref{lem:digitfield} force that difference to vanish when both conditions
hold.

When either fails, Lemma~\ref{lem:digitfield} supplies a nonzero $\delta$
...
in \eqref{eq:algtranslations}. Translation by an element of the base field
...
Condition \eqref{cond:P} is intrinsic to the precision ideal: the observed
denominator degrees must eventually absorb their own doubles. Condition
\eqref{cond:V} is different. The first digit contributes a parameter to
...
of Corollary~\ref{cor:neverunique} is compatible with uniqueness among
...
\eqref{eq:periodpoly} is negative at $1$, and its positive root exceeds
$1$.
...
finite prefix gives $\xi$ with the code \eqref{eq:periodiccode}. The
...
and \eqref{eq:periodpoly} has only one root greater than one.
\end{proof}
...
\eqref{eq:periodpoly}. A periodic itinerary does not force its complete
quotients to recur as equal numbers.

\subsection{The exact algebraic dichotomy}

Define
\begin{equation}\label{eq:periodscales}
\begin{split}
...
\label{thm:periodiccriterion}
For the code \eqref{eq:periodiccode}, exactly one point of $\F(\codea)$ is
...
\end{equation}
In that case the point is $\xi$ from Theorem~\ref{thm:periodroot}.
...
$0\le t<\ell$,
\begin{equation}\label{eq:perioddegree}
 d_{r-1+k\ell+t}=S+kE+E_t.
\end{equation}
If $E=D_*=0$, both conditions \eqref{cond:P} and \eqref{cond:V} hold.
...
multiples $kE$ in \eqref{eq:perioddegree} then absorb both the double of
any specified $d_n$ and $D_0$. Again both conditions hold. Theorem
\ref{thm:algebraic} gives at most one algebraic point and
Theorem~\ref{thm:periodroot} supplies it.

Suppose \eqref{eq:periodcriterion} fails. Then $D_*>0$, and either $E=0$
...
$S\le(r-1)D_*$ and \eqref{eq:perioddegree} show that this valuation exceeds
...
Theorem~\ref{thm:periodiccriterion}.
\end{proof}
...
\label{thm:orderunit}
Let $G\ne\{0\}$ and let \eqref{eq:periodiccode} be a code in $\Oz_G$.
...
\end{theorem}
\begin{proof}
If $E$ is an order unit, the terms $S+kE$ in \eqref{eq:perioddegree} are
cofinal in $G_{\ge0}$. Theorem~\ref{thm:hahn} gives uniqueness.
...
$d_n\ge S+g$. Formula \eqref{eq:perioddegree} implies
$g\le kE+E_t\le(k+1)E$. Thus $E$ is an order unit.

The first existence equivalence is Corollary~\ref{cor:cofinality}. For the
...
degree two over $K$. Here \eqref{cond:P} holds, whereas
\eqref{cond:V} fails: $D_0=1$ but every $d_n=0$.
...
\eqref{cond:V} holds and \eqref{cond:P} fails. A distinguished point is
...
\begin{equation}\label{eq:rankoneboundary}
...
Theorem~\ref{thm:hahn} ensures a realization in $K_\R$ even though the
...
Theorem~\ref{thm:orderunit} excludes uniqueness for every eventually
...
\begin{theorem}[Hidden algebraic independence]\label{thm:independence}
...
\]
Each $\delta_\alpha$ belongs to $I_{\codea}$ by \eqref{eq:betabound}.
...
\begin{theorem}[Exact surcomplex coordinate fiber]\label{thm:complex}
If $x\in\F(\codea)$ and $y\in\F(\codeb)$, then the points with the same
pair of coordinate codes form exactly
\begin{equation}\label{eq:complexfiber}
...
are different, their sum in \eqref{eq:complexfiber} is not invariant under
multiplication by $i$.
\end{theorem}
\begin{proof}
The exact real fiber theorem applied to both coordinates proves
\eqref{eq:complexfiber}. If the ideals agree, then
\[
 (a+ib)(e+if)=(ae-bf)+i(af+be)\in I+iI
 \quad(a,b\in\OO,\ e,f\in I).
\]
...
\begin{corollary}[Surcomplex hidden independence]
...
Use the construction of Theorem~\ref{thm:independence}, bounding the degree
...
\begin{proposition}[Precision transport]\label{prop:mobius}
Let $f(z)=(az+b)/(cz+d)$ over $\SC$, with $ad-bc\ne0$ and $cz+d\ne0$.
For a nonzero perturbation $e$ satisfying
\begin{equation}\label{eq:smalltransport}
 v_\C(ce)>v_\C(cz+d),
\end{equation}
one has $c(z+e)+d\ne0$ and
\begin{align}
 f(z+e)-f(z)
   &=\frac{(ad-bc)e}{(cz+d)(c(z+e)+d)},\label{eq:mobiusdiff}\\
 v_\C(f(z+e)-f(z))
   &=v_\C(ad-bc)+v_\C(e)-2v_\C(cz+d).\label{eq:mobiusscale}
\end{align}
...
$cz+d$. Subtracting the two fractions proves \eqref{eq:mobiusdiff}; taking
valuations proves \eqref{eq:mobiusscale}.
\end{proof}
...
\eqref{eq:smalltransport}, a pole or leading cancellation can invalidate
...
\subsection{The finite verification artifact}
...
Lemma~\ref{lem:algerror}. This keeps the main algebraic classification
...
\begin{problem}[Priority and exact equivalence of formulations]
...
\begin{problem}[Proper subfields of a full Hahn field]
...
Theorem~\ref{thm:hahn}; arbitrary subfields do not follow from its proof.
Determine which support-completeness or truncation assumptions suffice.
\end{problem}

\begin{problem}[Algebraic existence for a nonperiodic code]
Theorem~\ref{thm:algebraic} determines the size of the algebraic fiber
...
\begin{problem}[A correctly scoped Lagrange converse]
...
fixed in advance. Theorem~\ref{thm:periodroot} proves an existence direction
for a given periodic code, not this converse.
\end{problem}

\begin{problem}[Simplicity versus dynamical selection]
...
\begin{problem}[Iteration beyond the first block of length $\omega$]
...
\begin{problem}[Other integer parts]
...
\begin{problem}[Primary precision ideals]
Theorems~\ref{thm:prime} and \ref{cor:radical} classify primality and the
...
\begin{problem}[A genuinely surcomplex continued-fraction algorithm]
...
The coordinatewise result of Section~\ref{sec:complex} does not solve this.
\end{problem}

\begin{problem}[Effective precision and representations]
...
\begin{problem}[Formal proof and reusable interfaces]
...
\appendix
\section{Notation and scope checklist}
...
$\OO$, $\mm$ & Finite surreals and infinitesimals.\\
