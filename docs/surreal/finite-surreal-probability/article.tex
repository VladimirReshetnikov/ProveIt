...
Section~\ref{sec:position}; the delivered article is a new standalone file,
not a modification of that repository.
\end{minipage}
\end{titlepage}
...
in Section~\ref{sec:iid} are elementary special cases consistent with the
...
\begin{lemma}[Local exponential workspace]\label{lem:workspace}
...
One proof takes the supremum in $\R$ of the real numbers below $x$; the
Dedekind completeness used here belongs to $\R$, not to $\No$.

\begin{proposition}[Residue algebra]\label{prop:st}
On limited elements, standard part is an order-preserving ring homomorphism.
If $y$ is limited and $\st y\ne0$, then
$\st(x/y)=\st x/\st y$. If $x>0$ is limited, it is infinitesimal exactly
when $\st x=0$.
\end{proposition}
\begin{proof}
...
Order preservation and the last assertion follow from the definition.
\end{proof}
...
forms identify $K_\Gamma$ with an ordered subfield of $\No$; the field need
...
For positive $x,y$,
\begin{equation}\label{eq:positiveval}
 v(xy)=v(x)+v(y),\qquad
 v(x+y)=\min\{v(x),v(y)\}.
\end{equation}
The second equality has no cancellation because leading coefficients are
positive. For probabilities $0<p\le1$, $v(p)\ge0$.
...
established elementary equivalence with the real exponential field
\cite{vdde,vddeerr}. For each fixed ordinary integer $n$, a universally
quantified real-exponential inequality involving $n$ coordinates can be
used this way. This does \emph{not} transfer a theorem quantifying over
...
Finite additivity implies $P(\varnothing)=0$, $P(A^c)=1-P(A)$,
monotonicity, and $0\le P(A)\le1$. On a finite space
$\Omega=\{1,\ldots,n\}$ it is equivalent to a vector
$p\in K_{\ge0}^n$ with $\sum_i p_i=1$: necessarily
$P(A)=\sum_{i\in A}p_i$. Regularity means exactly that all $p_i>0$.
Thus no extra consistency problem arises merely from replacing real
weights by infinitesimal ones.
...
\end{equation}
Refining partitions proves that this definition is independent of the
representation. It is $K$-linear, positive, and satisfies
$\min X\le\E X\le\max X$. No convergence notion is needed, even if the
finitely many values $x_j$ are infinite surreals.
...
\label{thm:finitebounds}
For finite-range variables over $K$,
\begin{align}
 \Var(X)&=\E(X-\E X)^2\ge0,\\
 (\E XY)^2&\le\E X^2\,\E Y^2,\label{eq:cs}\\
 P(X\ge a)&\le\E X/a\quad(X\ge0,\ a>0),\label{eq:markov}\\
 P(|X-\E X|\ge a)&\le\Var(X)/a^2\quad(a>0).
\end{align}
The thresholds and variable values may be surreal.
\end{theorem}
\begin{proof}
Positivity of squares gives the variance assertion. If $\E Y^2=0$,
each positive-mass partition cell has $Y=0$, so $\E XY=0$.
Otherwise substitute $c=\E XY/\E Y^2$ in
$0\le\E(X-cY)^2$ and multiply by $\E Y^2>0$ to obtain
\eqref{eq:cs}. The pointwise inequality
$a\ind_{\{X\ge a\}}\le X$ proves \eqref{eq:markov}.
Apply it to $(X-\E X)^2$ for the last claim.
\end{proof}
...
weights. One cannot apply this assertion to an arbitrary function
...
Multiplication of denominators proves Bayes' rule and the chain rule.
...
\end{equation}
The second identity follows because an atom of $\mathcal H$ is a finite
union of atoms of $\mathcal G$. Multiplication by a $\mathcal G$-measurable
variable can be taken outside conditional expectation. Expanding the square
and using that property proves the finite law of total variance:
\begin{equation}
 \Var(X)=\E\bigl[\Var(X\mid\mathcal G)\bigr]
       +\Var\bigl(\E[X\mid\mathcal G]\bigr).
\end{equation}
These statements hold on zero-mass atoms up to the usual freedom of versions.
...
Conversely, inconsistent normalization or finite additivity supplies a
constant nonzero gain by a finite combination of tickets, and a price
outside $[0,1]$ supplies a sure loss in one trading direction. Thus the
usual finite coherence argument is genuinely retained. Physical
...
For $P_0(B)>0$, Proposition~\ref{prop:st} gives
\begin{equation}\label{eq:stcondition}
...
\begin{equation}\label{eq:conditionalshadow}
...
\begin{equation}\label{eq:popperproduct}
...
\eqref{eq:conditionalshadow} is supplied by regularity, not by a
purported universal rule for $0/0$.

\subsection{Leading atoms determine every conditional real shadow}

\begin{theorem}[Conditional leading-scale skeleton]\label{thm:skeleton}
...
\end{theorem}
\begin{proof}
Divide numerator and denominator of \eqref{eq:condition} by $t^{a(B)}$.
...
Proposition~\ref{prop:st} now applies. Common rescaling of the coefficients
...
\begin{theorem}[Signed-row lexicographic compression]\label{thm:compression}
...
\end{equation}
Then
\begin{equation}\label{eq:scalebayes}
...
$c_id_i>0$. Equation~\eqref{eq:positiveval} shows that $Z$ has leading
term $Dt^m$. Dividing each numerator by this leading term proves
\eqref{eq:scalebayes}. Nonminimal terms then have positive valuation,
...
\label{ex:bayes}
Take the prior \eqref{eq:intro} and likelihoods $(t^3,t,1)$. The products
are proportional to $(t^3,t^2,t^2)$, so
\begin{equation}\label{eq:bayesexample}
...
\begin{equation}
 ...
 \le\frac{2\delta}{d},
\end{equation}
since $0\le y\le e$. This implies \eqref{eq:conditionalbound}.
For the valuation assertion write $e=d+\Delta_d$ and
$y=x+\Delta_x$. Finite sums in \eqref{eq:precision} give
...
$\beta+\lambda$. Example~\ref{ex:shadowloss} demonstrates the qualitative
...
\begin{equation}\label{eq:logitbayes}
...
ratio, and $\log(xy)=\log x+\log y$ proves \eqref{eq:logitbayes}.
\end{proof}
...
infinitesimal. Then
\begin{equation}\label{eq:logittail}
...
\begin{equation}\label{eq:logitt}
...
\begin{equation}\label{eq:andlogit}
...
\begin{equation}\label{eq:softbound}
...
\begin{theorem}[Finite multiscale softmax limit]\label{thm:multisoft}
...
\begin{equation}\label{eq:multisoft}
 \st p_i=
 \begin{cases}
...
Section~\ref{sec:scalars} is not implicated. The standard part describes
...
\label{thm:kl}
Under the support condition in \eqref{eq:entropy},
\begin{equation}
 D(p\Vert q)\ge0,
...
\begin{equation}\label{eq:rareentropy}
...
\begin{theorem}[Finite data processing]\label{thm:dataprocessing}
...
Equation~\eqref{eq:klchain} and Theorem~\ref{thm:kl} give the inequality.
...
\begin{align}
 D(p\Vert q)
 &=\frac1t+t\log t
   +(1-t)\bigl(\log(1-t)-\log(1-u)\bigr),\label{eq:hugekl}\\
 \st\bigl(tD(p\Vert q)\bigr)&=1.
\end{align}
Thus their relative entropy is positive infinite. In fact the final
term in \eqref{eq:hugekl} is infinitesimal, and $t\log t$ is
infinitesimal, while the first term is $1/t$.
\end{example}

There is no conflict with \eqref{eq:entropybound}. Entropy uses each
...
$q=p$, by Theorem~\ref{thm:kl}. The Brier loss
...
loss is
\begin{equation}\label{eq:logisticloss}
...
\begin{theorem}[Surreal finite Gibbs principle]\label{thm:gibbs}
...
Multiplication by $\tau$ and rearrangement give \eqref{eq:gibbs}.
...
Theorem~\ref{thm:multisoft}. This does not construct a thermodynamic
...
Theorem~\ref{thm:compression} explains how real-valued acts can detect
...
For $N=0$, formula \eqref{eq:smoothing} instead reduces to the ordinary
...
\eqref{eq:logisticloss}; that fact does not establish existence for every
...
\subsection{Finite chains and finite paths}
...
infinite paths; Section~\ref{sec:iid} supplies concrete obstructions to
...
\subsection{Finite martingales and stopping}
...
the repository's foundations discussion \cite{repofound}.
...
\begin{example}[A regular strong law on a countably infinite space]
\label{ex:geometric}
...
supported standard part in that class \cite{repomeasures}. We use the
...
\begin{definition}\label{def:coefficientwise}
...
$\mu_\gamma$ on $\A$, via
\begin{equation}\label{eq:coefficientmeasure}
...
such index, then
\begin{equation}\label{eq:hierconditional}
 \st P(A\mid B)
...
$t=1$ in \eqref{eq:hierarchy} and regard the generally divergent result
...
For the hierarchy in Theorem~\ref{thm:hierarchy} and a bounded measurable
\emph{real-valued} function $f$, define
\begin{equation}\label{eq:hierintegral}
...
\begin{equation}\label{eq:badintegral}
...
\eqref{eq:hierintegral} for real-valued functions.
...
are zero. The failure in \eqref{eq:badintegral} exploits an uncountable
...
\begin{align}
 D(y)&=\sum_{j=0}^m t^{\gamma_j}h_j(y),\label{eq:kerneldensity}\\
...
\begin{equation}\label{eq:kernelreconstruction}
 P(X\in A,Y\in B)
...
Equation~\eqref{eq:kernelreconstruction} is a rigorous posterior
...
for the coefficientwise extension in Definition~\ref{def:coefficientwise}.
...
laws require
\begin{equation}\label{eq:firstsuccess}
 P(B_n)=t(1-t)^{n-1}.
\end{equation}
...
is impossible: Section~\ref{sec:extension} constructs such laws after
...
\begin{equation}\label{eq:coinrow}
 2(2k-n)2^{-n}.
\end{equation}
...
one quarter of those in \eqref{eq:coinrow}, and their variations are
...
\begin{lemma}[Set-sized relative field embedding]\label{lem:embedding}
...
canonical exponential closure from Lemma~\ref{lem:workspace}, but that
is not a proof that two different exponential structures agree.

\subsection{A fine-ultrafilter construction}
...
\begin{equation}\label{eq:ultrafilter}
...
\begin{equation}
 ...
 \quad(a\in\Omega).
\end{equation}
Lemma~\ref{lem:embedding}, applied over $\R$, turns these into surreal
...
The addition implicit in \eqref{eq:ultrafilter} is a snapshot operation:
\begin{equation}
...
produce the strong-summability obstruction of Section~\ref{sec:infinity}.

\subsection{Nonuniqueness and limited symmetry}
...
by their leading terms. Lemma~\ref{lem:embedding} then embeds the full
...
Lemma~\ref{lem:embedding} over $K$; its image remains a set-sized field
...
regular and finitely additive. Apply Theorem~\ref{thm:extension}
over its original scalar field.
\end{proof}

Theorems~\ref{thm:rarecoin} and \ref{thm:faircoin} prove that these
...
\begin{equation}\label{eq:latentlaw}
 P=\frac{\delta_0\otimes\mu_0+t\delta_1\otimes\mu_1}{1+t}.
\end{equation}
Theorem~\ref{thm:hierarchy} makes this a positive coefficientwise
...
\begin{equation}\label{eq:latentcov}
...
marginals do not contradict Theorem~\ref{thm:faircoin} or the analogous
...
\begin{equation}\label{eq:latentposterior}
 M_n=P(H=1\mid\F_n)
...
The ordinary strong laws of the two real component measures
...
\end{equation}
Consequently
\begin{equation}\label{eq:tailposterior}
 P(B)=\frac{t}{1+t}>0,
 \qquad P(H=1\mid B)=1.
\end{equation}

\begin{proposition}[Finite-update shadows need not recover rare tail conditioning]
\label{prop:tailfailure}
In model \eqref{eq:latentlaw}, every finite posterior in
\eqref{eq:latentposterior} has standard part zero, but observing the tail
...
laws give \eqref{eq:tailposterior}. For every ordinary $n$ and every
...
\begin{theorem}[A continuity test for the shadow]\label{thm:shadowcountable}
...
from \eqref{eq:ultrafilter} does not: singleton shadows are zero while
...
\subsection{A Poisson shadow at reciprocal-probability sample size}
...
counts $k$ have the real Poisson masses in \eqref{eq:poisson}, whose
...
standard parts in \eqref{eq:poisson} remain valid. But an internal power
...
and leading-coefficient layer from Theorem~\ref{thm:scalebayes} can be
...
Theorem~\ref{thm:stability} supplies a precision contract. An operation
...
score-based models, Proposition~\ref{prop:softbound} gives a different
...
\subsection{What the accompanying verifier checks}

The file \texttt{verify.py} implements exact arithmetic in $\Q(t)$ with
...
The output \texttt{verification.json} records the actual assertion count
...
using the full fine topology of $\No$; Proposition~\ref{prop:fine}
...
martingale convergence, strong law, central limit, or stochastic calculus
...
and countable-support functions. Theorem~\ref{thm:integrationfailure}
...
\ref{thm:extension} guarantees regular all-subsets extension after field
...
The algebraic results in Sections~\ref{sec:finite}--\ref{sec:bayes} need
only ordered fields plus the explicitly used residue or Hahn structure.
Sections~\ref{sec:logits}--\ref{sec:gibbs} additionally use canonical
exponential and its scalar inequalities. Section~\ref{sec:hierarchy}
uses ordinary real finite measures and Hahn support multiplication.
Section~\ref{sec:extension} uses choice, ordered real closures, the
surreal cut property, and first-order compactness.
Section~\ref{sec:latent} imports two ordinary real product laws and their
strong laws. Section~\ref{sec:bridges} distinguishes ordinary real
...
external images, bibliography processor, or downloaded fonts. Build with
\begin{verbatim}
latexmk -pdf -interaction=nonstopmode -halt-on-error \
  surreal_probability.tex
python3 verify.py --output verification.json
\end{verbatim}
When \texttt{latexmk} is unavailable, run \texttt{pdflatex} repeatedly
until cross-references and the table of contents stabilize. A build
script is included. The delivery was checked for unresolved references
and typesetting errors, and the PDF was rendered for visual inspection.
