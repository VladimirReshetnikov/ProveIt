...
numbers provide the classical background in \cite{kallenberg}. Real
disintegration and product-law existence are imported; the two Bernoulli
strong laws needed in Section~\ref{fsp:sec:latent} are proved there by a
fourth-moment argument.
...
\item \emph{Not asserted anywhere} (Section~\ref{fsp:sub:notasserted}). No general
infinite probability theory using countable additivity in the full fine
topology; finite-support laws are compatible with it. No universal
...
The review of Sections~\ref{fsp:sec:shadows}--\ref{fsp:sec:bayes}
subsequently adds the joint-normalizer hypothesis for commuting likelihood
updates, corrects the neutral-likelihood wording, and separates sufficient
conditioning precision from necessity. The live review record is
\path{docs/REVIEW.md}. The Sections~\ref{fsp:sec:logits}--\ref{fsp:sec:information}
review makes the finite nonempty domains and binary-prior hypotheses explicit,
expands softmax and information inequalities, and separates an attained
interior logarithmic-score minimum from the boundary infimum. The delivered
audit, programs and data remain unchanged. The
Sections~\ref{fsp:sec:gibbs}--\ref{fsp:sec:processes} pass adds the
posterior-support qualification after smoothing, expands the Gibbs and
finite-process proofs, and makes adaptation and stopping-time measurability
explicit. The Sections~\ref{fsp:sec:infinity}--\ref{fsp:sec:hierarchy}
pass expands common-support and integration arguments, distinguishes uniform
bounds from one-component essential bounds, and supplies a posterior with
nonintegrable coefficients before density cancellation. The
Sections~\ref{fsp:sec:iid}--\ref{fsp:sec:extension} pass expands the
coefficient-variation obstructions, the relative field embedding and finite
compactness models, and replaces the logit change-of-variable shortcut by
a calculation in the original Hahn coordinates. The
Section~\ref{fsp:sec:latent} pass derives the martingale updates and
component strong laws, and separates order, coefficientwise and
standard-part convergence. Section~\ref{fsp:sec:bridges} now spells out
the shadow-continuity equivalence, the internal-algebra extension and the
Poisson ultraproduct, including its saturation proof and the measurable
ordinary count. The final implementation and scope pass distinguishes
nonnegative model weights from regular laws, separates algebraic scoring
from exponential hypotheses, and aligns the dependency statements with
these proofs. Source and provenance reconciliation remain separate from
this main-text pass.
...
Since every event mass lies in $[0,1]$, standard part is defined on it.
Its positivity, additivity and value at $1$ show that
$P_0(A)=\st P(A)$ is a real finitely additive probability.
...
The coordinate differences are $(2t,0,-2t)$, so their total variation
distance is $\tfrac12(2t+0+2t)=2t$.
\end{example}

For a regular probability on an event algebra $\A$ there is a richer
real object, defined for $A,B\in\A$ with $B\ne\varnothing$:
...
\end{equation}
Regularity makes $P(D)$ and $P(B\cap D)$ positive, so the ordinary
cancellation identity for these ratios applies before standard part.
Each factor lies in $[0,1]$, and the ring law for standard part gives
\eqref{fsp:eq:popperproduct}, even when $C(B\mid D)=0$.
The conditioning domain is precisely $\A\setminus\{\varnothing\}$.
This has the normalization, finite-additivity and product properties of
conditional probability spaces in \cite[Definition~2.1]{halpern}; compare
also \cite{brickhill}. No convention for the empty conditioning event is
needed. The existence of the ratios comes from regularity of $P$, whereas
the real conditional law itself can vanish on nonempty events.
...
Let a finite regular Hahn probability have atoms indexed by
$\{1,\ldots,n\}$ and weights
\begin{equation}
 p_i=c_i t^{a_i}(1+\eta_i),\qquad c_i>0,\quad v(\eta_i)>0,
\end{equation}
where $c_i\in\R_{>0}$, $a_i\in\Gamma$, and a zero error is allowed
by $v(0)=+\infty$. Identify events with their sets of atom indices.
For a nonempty event $B$, put $a(B)=\min_{i\in B}a_i$. Then
...
\end{theorem}
\begin{proof}
Divide numerator and denominator of \eqref{fsp:eq:condition} by
$t^{a(B)}$. Each resulting summand has the form
$c_i t^{a_i-a(B)}(1+\eta_i)$. It is limited: its residue is $c_i$
when $a_i=a(B)$, and zero when $a_i>a(B)$. Hence the normalized
denominator has positive real residue
$\sum_{i\in B,a_i=a(B)}c_i$, and the normalized numerator has the
residue in the formula, possibly zero. The quotient rule of
Proposition~\ref{fsp:prop:st} applies even if $A\cap B$ is empty.
Rescaling all coefficients in the first group meeting $B$ multiplies
both residues by the same positive real, which cancels.
\end{proof}
...
group. Conversely the conditional shadows recover the ordered groups
and their coefficient ratios. For two distinct atoms $i,j$, the value of
$C(\{i\}\mid\{i,j\})$ is $1$ when $a_i<a_j$, $0$ when $a_i>a_j$,
and $c_i/(c_i+c_j)\in(0,1)$ when $a_i=a_j$. Within a tied group this
last number recovers $c_i/c_j$. Thus the group order and the normalized
coefficients are exactly the information retained, but the numerical gaps
between exponents and the higher coefficients are lost.
...
The row set is nonempty because $\sum_i p_i=1$. Choose its earliest
nonzero row. If the chosen rows do not yet span every coefficient row,
the set of exponents whose rows lie outside their span is a nonempty
subset of $W$, so has a least member. Choose that row next.
The chosen rows are linearly independent, so after at most $n$ choices
they span every coefficient row. In particular, each skipped row lies
in the span of chosen rows with strictly earlier exponents.

For $d\in\R^n$, finite distributivity gives
\[
 [t^\gamma]\sum_i p_i d_i=r_\gamma\cdot d.
\]
If $u_j$ is the first chosen row pairing nontrivially with $d$, then
every earlier coefficient row pairs to zero: the chosen earlier rows do,
and each skipped one is their linear combination. The exponent of $u_j$
is therefore exactly the leading exponent of the expectation, with
coefficient $u_j\cdot d$. If all chosen rows annihilate $d$, so do all
coefficient rows, and the expectation is zero. The converse follows by
...
All weights have nonnegative valuation. Normalization makes the
exponent-zero row a real probability vector and the sum of every later
row zero. Each nonzero later row therefore has both positive and negative
entries. These selected coefficient rows need not themselves be a list of
probability laws; this construction is distinct from the finite-state
probability-system equivalence in \cite[Section~4]{halpern}.
The selection argument establishes existence, not a finite search through
an arbitrary infinite Hahn support.

The real-payoff hypothesis is essential. Take
$p=(1/2+t,1/2-t)$, whose selected rows are $(1/2,1/2)$ and $(1,-1)$.
For the surreal payoff difference $d=(-1+t,1)$, the first row gives
$t/2>0$, but the actual expectation is
\[
 (1/2+t)(-1+t)+(1/2-t)=-3t/2+t^2<0.
\]
Multiplication by surreal payoffs outside $\R$ shifts exponents, invalidating the
coefficient-extraction step used in the theorem.
...
At least one product $p_i\ell_i$ is positive, so $Z>0$; thus every
$q_i\ge0$ and $\sum_iq_i=1$. Multiplying all likelihoods by one positive
factor multiplies $Z$ by that factor and leaves the posterior unchanged. Bayes' formula is exact for either interpretation when the
...
term $Dt^m$, where $D>0$. Write $Z=Dt^m(1+\zeta)$ with
$v(\zeta)>0$. The factor $(1+\zeta)^{-1}$ is a unit with residue one,
so division subtracts $m$ from every numerator valuation and divides
its leading coefficient by $D$. This proves \eqref{fsp:eq:scalebayes}
for every surviving index, not just the minimizers. Nonminimal terms
have positive valuation and residue zero; the minimal ones have the
stated positive residues, summing to one. An exactly zero likelihood
has no leading coefficient and is excluded from the minimum; its
posterior mass is exactly zero.
\end{proof}
...
For finitely many nonnegative likelihood vectors $\ell^{(1)},\ldots,
\ell^{(k)}$, successive updates commute provided their joint normalizer
$Z_* = \sum_i p_i\prod_{j=1}^k\ell_i^{(j)}$ is positive.
Some index then has positive prior and every likelihood factor positive,
so every intermediate normalizer is positive in every update order.
Canceling those normalizers gives
$q_i=p_i\prod_j\ell_i^{(j)}/Z_*$, independent of the order.
Individual updates being defined is not enough: with a positive two-state
prior, likelihoods $(1,0)$ and $(0,1)$ each define an update, but their
joint normalizer is zero and neither second update exists.
Under conditional independence this product is the likelihood of the
combined observations; without it, the factors must be the appropriate
conditional likelihoods. For surviving hypotheses, posterior odds are
prior odds times the product of likelihood ratios. No unspecified
infinite product is defined by this finite identity.
...
\end{equation}
Put $h_i=p_i-q_i$ and $I_+=\{i:h_i>0\}$.
Since $\sum_i h_i=0$, the positive and negative totals satisfy
$\sum_{I_+}h_i=-\sum_{h_i<0}h_i=\tfrac12\sum_i|h_i|$.
Every event sum lies between these two signed extremes; the event $I_+$
attains the positive one. This proves the equality over any ordered field,
including zero differences and nonregular laws.
...
\end{equation}
Separately, for Hahn probabilities assume $P(B)>0$ and put
$\beta=v(P(B))\in\Gamma$ and $\lambda\in\Gamma_{\ge0}$.
If
\begin{equation}\label{fsp:eq:precision}
 v(p_i-q_i)>\beta+\lambda\quad\text{for every atom }i,
\end{equation}
then $Q(B)>0$, it has the same leading term as $P(B)$, and
\begin{equation}
...
The valuation assertion does not require a separately chosen $b$.
Write $e=d+\Delta_d$ and $y=x+\Delta_x$. Each difference is a finite
sum of atom errors, so the valuation inequality for sums gives
$v(\Delta_d),v(\Delta_x)>\beta+\lambda$. Because $\lambda\ge0$,
the perturbation $\Delta_d$ cannot change the leading term of $d$.
Thus $e>0$ and $v(e)=v(d)=\beta$.
Also $0\le x\le d$ implies $v(x)\ge\beta$: a smaller valuation
would make positive $x$ dominate $d$. The case $x=0$ uses $v(0)=+\infty$.
The exact difference is
\[
 \frac{x}{d}-\frac{y}{e}
 =\frac{x\Delta_d-d\Delta_x}{de}.
\]
Both numerator terms have valuation greater than $2\beta+\lambda$,
so their difference does too; cancellation can only raise that valuation.
The denominator has valuation $2\beta$. Division proves the strict
bound, including the case of an identically zero numerator.
\end{proof}

Consequently an approximation suitable for unconditional probabilities
may be unusable after rare conditioning. Input errors beyond the event scale $\beta+\lambda$ are a
\emph{sufficient} contract for preserving a conditional answer through
scale $\lambda$. They are not necessary for a particular pair of laws:
$(1-2t,t,t)$ and $(1-4t,2t,2t)$ have the same conditional law on
$B=\{2,3\}$, although their atom errors already have valuation $1=v(P(B))$.

The strict inequality in the uniform contract cannot simply be weakened.
Choose $\beta>0$, $\lambda\ge0$ in $\Gamma$, set
$\eps=\tfrac12t^{\beta+\lambda}$, and take
\[
 p=(1-2t^\beta,t^\beta,t^\beta),\qquad
 q=(1-2t^\beta,t^\beta+\eps,t^\beta-\eps).
\]
Both are positive laws, and the nonzero atom errors have valuation
$\beta+\lambda$. On $B=\{2,3\}$ their first conditional coordinates
differ by $t^\lambda/4$, of valuation exactly $\lambda$, not greater.
At $\lambda=0$ this is a real discrepancy. These examples isolate the
precision loss without asserting necessity when numerator and denominator
errors happen to cancel.

\subsection{Infinite prior odds are not ordinary dogmatism}

For $0<p<1$ and any positive likelihood ratio $R\in K$,
the posterior odds are $R\,p/(1-p)$. They change if and only if $R\ne1$;
a neutral ratio leaves the prior exactly unchanged. With strictly positive
likelihoods both posterior coordinates remain nonzero. An exact prior
endpoint, by contrast, cannot be revived by multiplying its zero weight.

An infinite prior logit cannot be brought to a limited logit by an
ordinary finite number of log-likelihood increments bounded by real numbers.
If $L$ is positive infinite and $|b_j|\le C\in\R$ for $j\le n$ with
ordinary finite $n$, then $|\sum_{j=1}^n b_j|\le nC$ is limited.
For every real $M$, the inequality $L>M+nC$ gives
$L+\sum_jb_j>M$, so the updated logit remains positive infinite.
For such a logit $L'$, $\exp(-L')$ is a positive infinitesimal:
for every positive real $a$, $L'>-\log a$ implies $\exp(-L')<a$.
Hence the logistic posterior $1/(1+\exp(-L'))$ has shadow one,
even when the exact posterior changes. If the increments sum to zero,
the exact posterior is unchanged as well.
...
For binary hypotheses with $0<P(H)<1$ and both likelihoods
$P(D\mid H),P(D\mid H^c)>0$, all belonging to $E$,
...
\end{proposition}
\begin{proof}
For $z\in E$, positivity of $\exp(-z)$ gives $0<\sigma(z)<1$, and
closure puts $\sigma(z)$ in $E$. Direct division gives
$\sigma(z)/(1-\sigma(z))=\exp z$, hence $\logit(\sigma(z))=z$.
For $0<p<1$, the odds $p/(1-p)$ are positive in $E$; substituting
their logarithm into $\sigma$ gives $\sigma(\logit(p))=p$.
If $z<w$, then $\exp(-z)>\exp(-w)>0$ and taking reciprocals of
$1+\exp(-z)$ and $1+\exp(-w)$ proves $\sigma(z)<\sigma(w)$.
Its inverse is therefore strictly increasing too. Multiplying by
$\exp z$ gives $\sigma(-z)=1-\sigma(z)$.
Both prior weights and likelihoods are positive, so the evidence mass
and both posterior weights are positive. Canceling the common evidence
mass in the posterior odds, then applying $\log(xy)=\log x+\log y$,
proves \eqref{fsp:eq:logitbayes}.
\end{proof}
...
\end{equation}
Here $O(\eta^2)$ means bounded by a real constant times $|\eta|^2$
in this local expansion, including zero remainder at $\eta=0$.
To see the coefficient directly, put $a=\exp(-r)>0$ and use
$\exp(-\eta)=1-\eta+\eta^2R$ with limited $R$. Factoring $1+a$
from $1+a\exp(-\eta)$ leaves a principal unit. Its geometric inverse
has linear term $a\eta/(1+a)$ and a limited quadratic remainder.
Multiplication by $(1+a)^{-1}$ gives $a/(1+a)^2=\sigma(r)(1-\sigma(r))$
as the linear coefficient. These are strong infinitesimal series identities,
not ordinary sequence limits. For $t=\omega^{-1}$,
...
For an ordinary positive integer $n$ and scores $z_1,\ldots,z_n\in E$, define
...
scores multiplies their normalizing sum by $e^c$ and does not change $p$.
Conversely, equal softmax laws satisfy
$z_i-w_i=\LSE(z)-\LSE(w)$ for every $i$, by taking logarithms of
the equal probabilities. Thus a common additive constant is exactly the
ambiguity of the score vector. For $n=1$ the law is $(1)$ and its logit
is not an element of $E$. For $n\ge2$, the one-against-the-rest logit is
\begin{equation}
...
$|z_i-w_i|\le\delta$ for every $i$, where $\delta\in E_{\ge0}$, then
\begin{equation}\label{fsp:eq:softbound}
 e^{-2\delta}p_i\le q_i\le e^{2\delta}p_i.
\end{equation}
\end{proposition}
\begin{proof}
The maximum exists because $n\ge1$ is finite. Factoring $e^M$ from
the sum leaves $\sum_i e^{z_i-M}$, whose terms lie in $(0,1]$ and
at least one of which is $1$. It therefore lies in $[1,n]$; apply the
increasing logarithm and add $M$.
For perturbations, write $Z=\sum_i e^{z_i}$ and $W=\sum_i e^{w_i}$.
Termwise monotonicity and finite summation give
\[
 e^{-\delta}\le\frac{e^{w_i}}{e^{z_i}}\le e^\delta,
 \qquad e^{-\delta}\le\frac WZ\le e^\delta.
\]
All quantities are positive. Invert the second pair of bounds and
multiply it by the first to obtain
$e^{-2\delta}\le q_i/p_i\le e^{2\delta}$.
The same normalizer bounds also give
$|\LSE(w)-\LSE(z)|\le\delta$ after taking logarithms.
\end{proof}

For infinitesimal $\delta$, both $e^{2\delta}-1$ and
$1-e^{-2\delta}$ are infinitesimal by the strong exponential expansion.
Thus the displayed bounds imply $q_i/p_i-1\simeq0$, even when $p_i$
is extremely small. A finite list of infinitesimal score errors admits
such a common $\delta$ by taking their maximum. This is a different guarantee from
...
Let $m\ge1$ be an ordinary integer and
$\Omega_1,\ldots,\Omega_m$ positive surreals in $E$ with
$\Omega_m$ positive infinite and
$\Omega_{r+1}/\Omega_r$ infinitesimal for $r<m$. For an ordinary
positive number $n$ of states suppose
\begin{equation}
 z_i=\sum_{r=1}^m\Omega_r a_{ir}+b_i+\eta_i,
 \quad a_{ir},b_i\in\R,\quad \eta_i\in E,\quad \eta_i\simeq0.
\end{equation}
...
\end{theorem}
\begin{proof}
A finite nonempty set of real tuples has a lexicographic maximum,
so choose $j\in I_*$. All $\Omega_r$ are positive infinite: each
earlier scale exceeds the next. For $s>r$, the ratio
$\Omega_s/\Omega_r$ is a finite product of positive infinitesimals
and is infinitesimal.
If $i\notin I_*$, at the first differing coordinate $r$ write
$a_{ir}-a_{jr}=-c$ with real $c>0$. Dividing $z_i-z_j$ by $\Omega_r$
gives $-c$ plus an infinitesimal. The later scale terms have that property
by the ratio calculation; the real and infinitesimal offsets do as well
because $\Omega_r$ is infinite. Hence
$z_i-z_j<-c\Omega_r/2$ is negative infinite, making $e^{z_i-z_j}$
infinitesimal by comparison with the exponential of every real constant.
For $i,j\in I_*$, the hierarchical terms cancel and
\[
 e^{z_i-z_j}=e^{b_i-b_j}e^{\eta_i-\eta_j}
 \simeq e^{b_i-b_j}.
\]
Every term in $\sum_i e^{z_i-z_j}$ is limited. Its residue is the
positive real $\sum_{i\in I_*}e^{b_i-b_j}$, with the $j$th term equal
to $1$. The standard-part quotient rule applied to this normalizer gives
the asserted formula, after canceling the real factor $e^{-b_j}$.
\end{proof}

The word ``limit'' in the theorem's title refers to this standard-part
formula for one fixed list of surreal scales; no sequence or net is taken.
...
canonical logarithms in $E$. Let $n\ge1$ be an ordinary integer.
For probability vectors $p,q\in E^n$ define
...
\end{equation}
More explicitly, the exact decomposition is
\[
 D(p\Vert q)=\sum_{i\in S}p_i
 \left(-\log\frac{q_i}{p_i}-1+\frac{q_i}{p_i}\right)
 +\sum_{i\notin S}q_i.
\]
Every summand is nonnegative. A finite sum of nonnegative elements of
an ordered field is zero exactly when each summand is zero; the strict
scalar logarithm inequality then forces $q_i=p_i$ on $S$ and zero
mass outside $S$. The converse follows by $\log1=0$. Each $-p_i\log p_i$ is nonnegative. Applying
Gibbs inequality to the uniform law gives
$D(p\Vert(1/n,\ldots,1/n))=\log n-H(p)\ge0$.

For the standard-part identity, if $\st p_i=r_i>0$, write
$p_i=r_i(1+\xi_i)$ with infinitesimal $\xi_i$. Then
$\log p_i=\log r_i+\log(1+\xi_i)\simeq\log r_i$, so the ring
law gives $\st(-p_i\log p_i)=-r_i\log r_i$.
If $p_i>0$ is infinitesimal, apply the transferred real-exponential
inequality $\log x\le\sqrt{x}$ for $x\ge4$ to $x=1/p_i$.
Here the positive root belongs to $E$, since
$\sqrt{x}=\exp(\tfrac12\log x)$. This gives
\[
 0\le p_i\log(1/p_i)\le\sqrt{p_i}\simeq0.
\]
The last infinitesimality follows from $p_i<a^2$ for every positive
real $a$. Zero coordinates contribute the specified zero term. Taking
standard parts of the finite sum now proves the formula. This argument
does not take the standard part of $\log p_i$ when that logarithm is infinite.
\end{proof}
...
\end{equation}
Indeed $-\log(1-t)=\sum_{k\ge1}t^k/k$ strongly; multiplication by
$1-t$ gives $t-\sum_{k\ge2}t^k/(k(k-1))$, whose tail after degree two
is $t^3$ times a limited element. Here the remainder notation again means
a real bound times $t^3$.
...
\end{equation}
If $p^Y_j>0$, some $p_{ij}>0$, so the joint support condition gives
$q_{ij}>0$ and hence $q^Y_j>0$. Both conditional laws are therefore
defined, and their support condition also holds. On a cell with
$p_{ij}>0$, factor the joint ratio as
$(p^Y_j/q^Y_j)(p^{X\mid j}_i/q^{X\mid j}_i)$ and split its
logarithm. Summing the marginal term over $i$ contributes
$p^Y_j\log(p^Y_j/q^Y_j)$, and the remaining terms give the conditional
divergence. Columns with $p^Y_j=0$ contribute nothing; their conditional
laws need not be specified. All regrouping is finite.
...
If $P_{ij}>0$, then $p_i>0$ and $T_{ij}>0$, so $q_i>0$ and
$Q_{ij}>0$. Thus the joint support condition holds, and on this cell
$P_{ij}/Q_{ij}=p_i/q_i$. Summing over $j$ and using
$\sum_jT_{ij}=1$ gives $D(P\Vert Q)=D(p\Vert q)$.
The output marginals are $pT,qT$, with the inherited support condition.
The chain rule now gives the exact nonnegative difference
\[
 D(p\Vert q)-D(pT\Vert qT)
 =\sum_{j:(pT)_j>0}(pT)_j D(P_{X\mid j}\Vert Q_{X\mid j}).
\]
Its weights are positive even when infinitesimal. A finite sum of such
nonnegative terms vanishes precisely when each conditional divergence
vanishes, which is equivalent to equality of the entire two conditional
vectors by Theorem~\ref{fsp:thm:kl}. No condition is imposed at outputs
with zero $pT$ mass; their terms are absent.
\end{proof}
...
\end{equation}
The inequality $\exp x\ge1+x$ at $x=1/t^2$ gives
$0<u\le t^2/(1+t^2)$, so $u$ is positive infinitesimal.
...
term in \eqref{fsp:eq:hugekl} is infinitesimal because both
$\log(1-t)$ and $\log(1-u)$ are infinitesimal and $1-t$ is limited.
The entropy estimate above gives $t\log t\simeq0$.
Thus $D(p\Vert q)=1/t+\eps$ for an infinitesimal $\eps$, which
proves both positive infinitude and the displayed normalized residue.
\end{example}
...
\end{equation}
If the true law $p$ is strictly positive, the loss over strictly
positive predictions is therefore uniquely minimized by $q=p$.
If $p$ has a zero coordinate, $p$ is outside that prediction domain
and no minimizer in it exists. Indeed let $S=\{i:p_i>0\}$ and
$a=\sum_{i\in S}q_i<1$. Choose $a<c<1$, multiply the predicted weights
on $S$ by $c/a$ and those outside $S$ by $(1-c)/(1-a)$.
The new prediction remains strictly positive and normalized, while its
expected loss decreases by $\log(c/a)>0$.

The lower bound $H(p)$ is still the infimum. For
$q^{(\eps)}=(1-\eps)p+\eps(1/n,\ldots,1/n)$ with $0<\eps<1$,
\[
 0\le D(p\Vert q^{(\eps)})\le-\log(1-\eps),
\]
since $q_i^{(\eps)}\ge(1-\eps)p_i$ on $S$. Given any $\delta>0$ in
$E$, take $\eps=1-\exp(-\delta/2)$ to make the excess at most
$\delta/2$. This is an order-infimum argument inside $E$, with no
convergence convention or evaluation of $\log0$.
The Brier loss
...
\end{equation}
For completeness,
$\E_pS(q,i)=\sum_jq_j^2-2\sum_jp_jq_j+1$;
subtracting the same expression at $q=p$ gives the displayed sum of
squares. It is nonnegative, with equality only when every $q_j=p_j$.
Thus Brier loss is strictly proper on the whole finite simplex over any
ordered field, including boundary laws.
...
\end{equation}
Indeed $-\log\sigma(z)=\log(1+e^z)-z$ and
$-\log(1-\sigma(z))=\log(1+e^z)$, giving the displayed expected loss.
Since $q=\sigma(z)$ parametrizes the strictly positive binary simplex,
$\Phi(z;p)-H(p,1-p)$ is exactly the corresponding relative entropy.
Its unique minimizer is therefore $z=\logit(p)$. This conclusion uses an explicit
...
Let $n\ge1$ be an ordinary integer, let $E_1,\ldots,E_n\in E$
be energies, and let $\tau>0$ belong to $E$.
Set
\begin{equation}
 Z=\sum_i e^{-E_i/\tau},\qquad q_i=Z^{-1}e^{-E_i/\tau}.
\end{equation}
For every probability vector $p\in E^n$,
...
\end{theorem}
\begin{proof}
Each summand defining $Z$ is positive and lies in $E$; thus $Z>0$
and $q$ is a strictly positive probability vector in $E^n$.
Consequently $D(p\Vert q)$ is defined for every $p$ on the entire simplex,
even when some $p_i=0$. On its positive support, substitute
$\log q_i=-E_i/\tau-\log Z$ to obtain
\[
 D(p\Vert q)=-H(p)+\frac1\tau\sum_i p_iE_i+\log Z,
\]
using $\sum_i p_i=1$. Multiplication by the positive $\tau$ gives the
identity, and Gibbs inequality makes its excess term nonnegative, with
equality exactly at $p=q$. A positive infinitesimal $\tau$ preserves
strict positivity of a nonzero excess just as any other positive field
element does. At $n=1$, $q=(1)$ and the identity reduces to $E_1=E_1$.
\end{proof}
...
For a positive infinitesimal temperature and real energies, let
$E_* = \min_i E_i$ and let $G=\{i:E_i=E_*\}$. Factor
$e^{-E_*/\tau}$ out of $Z$. Every ground-state term is $1$; every
excited term is $e^{-(E_i-E_*)/\tau}$, infinitesimal because its positive
real gap divided by $\tau$ is infinite. The normalized denominator
therefore has residue $|G|$, giving shadow $1/|G|$ on $G$ and zero
elsewhere. The gap relative to temperature matters: for energies $(0,\tau)$
the law is exactly $(1,e^{-1})/(1+e^{-1})$, even when $\tau$ is
infinitesimal, so both shadows are positive.
Several ordered energy scales can be treated by
...
For a nonempty finite collection of acts on finitely many states, expected
utility is an ordered-field calculation. Comparing the finite nonempty
list of expected utilities successively produces a maximum; no supremum
or compactness argument is required.
...
\end{equation}
Every product $p_iX_i$ and their finite sum are limited, so the ring
property of standard part applies. This explains precisely why the
qualification on the payoffs is indispensable. In the rare-loss example
$X=\ind_A/t$ with $P(A)=t$, the payoff $1/t$ has no real standard part,
although the expectation is the ordinary real number $1$.
...
Suppose a nonempty finite alphabet has ordinary counts $N_i\in\N$
with $N=\sum_iN_i>0$. Choose $a_i\in\R_{>0}$ and a positive
infinitesimal $t$ in the scalar field.
The law
\begin{equation}\label{fsp:eq:smoothing}
 p_i=\frac{N_i+t a_i}{N+t\sum_j a_j}
\end{equation}
is regular: its numerator and denominator are positive, and summing
the numerators gives the denominator. The denominator has residue $N$,
so a seen category has standard probability $N_i/N$. For an unseen
category,
\[
 p_i=t\frac{a_i}{N+t\sum_j a_j},\qquad
 \st(p_i/t)=a_i/N.
\]
In Hahn coordinates this means $v(p_i)=v(t)$ and
$\lc(p_i)=(a_i/N)\lc(t)$; the leading coefficient is literally $a_i/N$
when $t$ is the unit-coefficient monomial $\omega^{-1}$.
Any single nonnegative likelihood vector that is not identically zero
has a positive normalizer for this regular prior. Its posterior is
positive exactly on the positive-likelihood categories. A sequence of
updates needs the positive joint normalizer from
Section~\ref{fsp:sec:bayes}; smoothing cannot make incompatible zero
likelihoods compatible. The constants $a_i$ and the scale $t$ are
modeling choices, not consequences of the data. Distinct choices
can give the same empirical shadow and different answers after rare
conditioning.

For $N=0$, all nonnegative counts vanish. Canceling the nonzero $t$
in \eqref{fsp:eq:smoothing} gives the ordinary prior $a_i/\sum_j a_j$;
this case should be handled explicitly rather
...
\end{proposition}
\begin{proof}
Fix any $\theta\in E^d$ and any $s>0$ in $E$. Each signed margin
increases by $s(y_i x_i\cdot v)>0$ at $\theta+sv$. Its negative
therefore decreases, and strict monotonicity of both $\exp$ and $\log$
gives
\[
 \log(1+e^{-y_i x_i\cdot(\theta+sv)})
 <\log(1+e^{-y_i x_i\cdot\theta}).
\]
The data set is nonempty, so summing its finitely many strict inequalities
strictly lowers $L$. The new parameter still belongs to $E^d$, even when
$\theta$ or $v$ has infinite coordinates. Thus every candidate is improved;
no limiting point, derivative or continuity theorem is used.
\end{proof}

An infinite parameter is not a terminal point at infinity: it can still
be moved in the separating direction. Infinitesimal positive losses are
not zero. Without strict separation a minimizer can exist.
For example, two observations with the same scalar feature $x=1$ and
opposite labels have loss
$\log((1+e^{-\theta})(1+e^\theta))$.
Writing $u=e^\theta>0$, one has
$u+u^{-1}-2=(u-1)^2/u\ge0$, with equality only at $\theta=0$.
Their loss therefore has its unique minimum $\log4$ at zero. Soft targets
...
Let the finite nonempty state set be $S$, let $T$ be a stochastic
matrix over $K$ indexed by $S$, and let $\pi$ be an initial law.
For each ordinary integer horizon $N\ge0$, use all subsets of $S^{N+1}$
as events and define the path weights
\begin{equation}
 P(i_0,\ldots,i_N)=\pi_{i_0}\prod_{k=1}^N T_{i_{k-1},i_k}
\end{equation}
using the empty product $1$ at $N=0$. They are nonnegative.
For any fixed prefix, summing its extensions multiplies its weight by
$\sum_{i_N\in S}T_{i_{N-1},i_N}=1$. Repeatedly eliminate terminal
coordinates to prove normalization and consistency with every shorter
horizon, ending with $\sum_{i_0}\pi_{i_0}=1$.
All initial and transition entries lie in $[0,1]$. Their standard parts
are therefore defined, and the row sums remain $1$. The ring property
of standard part applied to each path product and each finite event sum
gives exactly the real chain with initial law $\st\pi$ and transition
matrix $\st T$.
...
On a finite probability space with a filtration
$\F_0\subseteq\cdots\subseteq\F_N$ of event algebras, a martingale
$(M_k)_{k=0}^N$ is $K$-valued, has $M_k$ measurable with respect to
$\F_k$, and satisfies $\E[M_k\mid\F_{k-1}]=M_{k-1}$ outside a null
event for $1\le k\le N$. The conditional expectation is the one already
constructed by finite partitions; null-atom versions do not affect scalar
expectations. A stopping time $T:\Omega\to\{0,\ldots,N\}$ satisfies
$\{T\le k\}\in\F_k$ for every $k$. Here $T$ denotes the random time,
not the transition matrix of the preceding subsection.
...
\end{equation}
is a pointwise telescoping sum: on $\{T=j\}$ exactly the terms
$1\le k\le j$ occur, including the empty sum when $j=0$.
The stopping-time property gives
$\{T\ge k\}=\Omega\setminus\{T\le k-1\}\in\F_{k-1}$.
The pull-out rule and the martingale identity therefore give
\[
 \E[\ind_{\{T\ge k\}}(M_k-M_{k-1})]
 =\E\!\left[\ind_{\{T\ge k\}}
       \E[M_k-M_{k-1}\mid\F_{k-1}]\right]=0.
\]
Any null-atom version contributes zero. Taking expectations of the
finite sum proves the claim, including $N=0$.
\end{proof}
...
additional integration and convergence results. The stopping-time
hypothesis is also essential: on two equiprobable states take $M_0=0$,
$M_1\in\{-1,1\}$, trivial $\F_0$ and full $\F_1$.
Choosing $T=1$ exactly when $M_1=1$ gives $\E M_T=1/2$,
but is not a stopping time because $\{T\le0\}$ is not in $\F_0$.
It selects whether to stop using information that was unavailable at time zero.

\subsection{Rare transitions and sample size}

Consider a state that changes irreversibly with probability $t>0$
infinitesimal at each step while it remains in its initial state.
At a fixed ordinary horizon $n\ge0$ its hitting probability is
\begin{equation}\label{fsp:eq:hitting}
 1-(1-t)^n\le nt.
\end{equation}
The no-change path has probability $(1-t)^n$, so its complement has
probability $1-(1-t)^n$.
For the bound, start with equality at $n=0$ and use
\[
 1-(1-t)^{n+1}=1-(1-t)^n+t(1-t)^n\le nt+t.
\]
Here $0\le(1-t)^n\le1$. Thus the hitting probability is infinitesimal
at every ordinary horizon; it is strictly positive when $n\ge1$ and zero
when $n=0$. Its standard part is always zero.
...
For an ordinary positive integer $n$, let $X_1,\ldots,X_n$ be
independent Bernoulli variables with parameter $p\in[0,1]_K$ and put
$\overline X_n=n^{-1}\sum_{k=1}^nX_k$. Finite variance algebra gives
\begin{equation}
 \E\overline X_n=p,\qquad
 \Var(\overline X_n)=\frac{p(1-p)}{n},\qquad
 P(|\overline X_n-p|\ge a)\le\frac{p(1-p)}{na^2}.
\end{equation}
Indeed $X_k^2=X_k$ gives $\Var(X_k)=p(1-p)$, and independence
gives $\E[X_jX_k]=p^2$ for $j\ne k$, so all cross covariances vanish.
Expanding the square of the finite sum yields variance $np(1-p)$;
dividing by $n^2$ and applying Chebyshev proves the display.
The inequalities hold for surreal $a>0$, including the deterministic
cases $p=0,1$. Their interpretation depends on scale. For fixed positive real $a$ and a real shadow, they have the
...
$n$ need not make the right-hand side small. At $p=a=t$, for example,
the bound $(1-t)/(nt)$ is infinite for every ordinary $n\ge1$.
In fact $P(|\overline X_n-t|\ge t)=1$: the empirical mean is either
zero, with error exactly $t$, or at least $1/n>2t$, with error greater
than $t$. This is an exact finite experiment at each $n$, not a claim
that an infinite path law has already been constructed. There is no unconditional
...
\label{fsp:prop:fine}
If a net $(x_i)_{i\in I}$ with a nonempty set-sized directed index set
converges to $x$ in
...
construction this forces $x_i=x$. Conversely, a net eventually equal to
$x$ converges to $x$, so this describes all such convergent nets.
\end{proof}
...
$\delta$ chosen in the proof may lie outside that subfield. For a
universe-sized formal carrier, the permitted index sets must be small
relative to that carrier, as in the foundations theorem; quantifying over
all subsets of the carrier would remove the crucial size hypothesis.
This does not make the full fine topology discrete: the ball of radius
$r>0$ about $x$ contains the distinct point $x+r/2$. Nevertheless,
...
Work in one Hahn field $\R((t^\Gamma))$, with $\Gamma$ a set-sized
ordered abelian group. A strong Hahn probability is a nonnegative event
map on a sigma-algebra, with $P(\varnothing)=0$ and $P(\Omega)=1$,
such that for every disjoint sequence $(A_n)$ the family $(P(A_n))$ is
strongly Hahn summable and
\begin{equation}
 P\Bigl(\bigcup_n A_n\Bigr)=\hsum_nP(A_n).
\end{equation}
Here strong summability requires a well-ordered union of supports and
only finitely many nonzero contributions at each exponent. In particular,
finite additivity follows by appending empty events to a finite disjoint list.
The ordinary series $\sum_{n\ge1}2^{-n}$ is not strong: every summand contributes
...
On $\N_{\ge1}$ with all subsets measurable, take $t=\omega^{-1}$ and
\begin{equation}
 w_n=(1-t)t^{n-1},\qquad
 P(A)=\hsum_{n\in A}w_n.
\end{equation}
The supports lie in $\N$, a well-ordered set, and
$w_n=t^{n-1}-t^n$ contributes only at two consecutive exponents.
Every exponent therefore occurs in at most two weights, proving strong
summability of all subfamilies. With $\ind_A(0)=0$, the coefficient of
$t^k$ in $P(A)$ is
\[
 \ind_A(k+1)-\ind_A(k)\qquad(k\ge0).
\]
For the whole space this is one at $k=0$ and zero thereafter, so
$P(\N_{\ge1})=1$. For nonempty $A$, its least element $m$ makes
all coefficients below $m-1$ zero and the coefficient at $m-1$ one;
hence $P(A)>0$. In a disjoint sequence of events, each of the two
indices $k,k+1$ can belong to at most one event. Thus at most two
event probabilities contribute at $k$, and their coefficients sum to
that of the union. This proves strong countable additivity directly.
The constant coefficient is $\ind_A(1)$, so the standard part is the
point mass at $1$. More explicitly,
$P(\{n>N\})=t^N>0$ for every ordinary $N\ge0$; the first $N$
weights sum to $1-t^N$, not to one.
\end{example}
...
set-sized well-ordered set $W\subseteq\Gamma$ and ordinary countably
additive signed measures $\mu_\gamma$ of finite total variation on
$\A$, via
...
\[
 \mu_\gamma\Bigl(\bigcup_n A_n\Bigr)=\sum_n\mu_\gamma(A_n),
 \qquad \sum_n|\mu_\gamma(A_n)|\le|\mu_\gamma|(\Omega)<\infty.
\]
Here $|\mu_\gamma|$ denotes ordinary total variation. The one common
set $W$ controls all events; a different support bound for each event
would not be this definition.
\end{definition}
...
\end{theorem}
\begin{proof}
Put $W_0=\{\gamma_\alpha:\alpha<\kappa\}$. The strictly
increasing image of the ordinal $\kappa$ is well ordered, and each
exponent receives one monomial contribution. Thus both displayed sums
exist, with all numerator supports contained in $W_0$. Write $Z=1+U$,
where $U$ has strictly positive support. The positive-support Neumann
lemma makes $\hsum_{n\ge0}(-U)^n$ a Hahn series and the inverse of
$Z$; when $\kappa=1$, $U=0$ and this is just one. In particular,
$Z$ and $Z^{-1}$ have leading coefficient one at exponent zero.

If every $\mu_\alpha(A)$ is zero, the numerator is zero. Otherwise
its first nonzero coefficient is positive, since every component is a
positive real measure; multiplication by $Z^{-1}>0$ preserves its sign.
The numerator for $\Omega$ is $Z$, giving normalization. Finite
additivity holds in each coefficient, so complements also give $P(A)\le1$.

For the full coefficientwise definition, write
$Z^{-1}=\sum_{\delta\in V}c_\delta t^\delta$, with $V$ well
ordered and extend $c_\delta$ by zero off $V$. The Hahn product lemma
makes $W=V+W_0$ well ordered and gives only finitely many pairs
$(\delta,\gamma_\alpha)$ at each exponent $\eta$. Consequently
\[
 [t^\eta]P(A)=\sum_{\alpha:\,\eta-\gamma_\alpha\in V}
 c_{\eta-\gamma_\alpha}\mu_\alpha(A).
\]
This is a finite real linear combination of ordinary probability
measures, with total variation at most
$\sum_{\alpha:\,\eta-\gamma_\alpha\in V}|c_{\eta-\gamma_\alpha}|$.
The support bound $W$ is independent of $A$. Each coefficient is
therefore an ordinary finite signed measure, proving coefficientwise
countable additivity.

For $P(B)>0$, put $b=\alpha(B)$. Cancel $Z^{-1}$ in the conditional
ratio and divide both numerator and denominator by $t^{\gamma_b}$.
All coefficients before $b$ in the numerator vanish because
$0\le\mu_\alpha(A\cap B)\le\mu_\alpha(B)=0$ there.
The denominator now has positive real residue $\mu_b(B)$; the
numerator has residue $\mu_b(A\cap B)$, possibly zero. Both are
limited, so standard-part division proves \eqref{fsp:eq:hierconditional}.
Taking $B=\Omega$, whose first active component is zero, gives
$\st P=\mu_0$.
\end{proof}
...
as an ordinary mixture. The law is regular precisely when every nonempty
measurable event has positive mass for at least one component. An arbitrary
hierarchy need not satisfy that coverage condition.
...
by $1+t+t^2$ introduces corrections: its coefficient at $t$ is
$\delta_0-\lambda$, negative on any positive-Lebesgue-measure event
that omits zero. Those signed coefficients are
...
\begin{equation}\label{fsp:eq:hierintegral}
 \E_P f=Z^{-1}\hsum_{\alpha<\kappa}t^{\gamma_\alpha}
                  \int f\,d\mu_\alpha.
\end{equation}
Every component integral is a finite real number, and the numerator
support lies in $W_0$ from the proof above, so this defines a Hahn value.
Ordinary linearity and finite Hahn addition give real linearity, and
indicators recover $P(A)$. If $f\ge0$, all numerator coefficients are
nonnegative, proving positivity after division by $Z>0$. Constants are
preserved: $\E_Pc=c$. Applying positivity to $M\pm f$ whenever
$|f|\le M$ gives $|\E_Pf|\le M$, in particular
$|\E_Pf|\le\norm{f}_\infty$, where this norm is the pointwise
supremum bound, not an essential supremum for $\mu_0$ alone.
The constant coefficient gives $\st\E_Pf=\int f\,d\mu_0$.
For example, the endpoint indicator in the preceding example has
$\mu_0$-essential supremum zero but positive surreal expectation.

Let $f_n,f$ be bounded measurable real functions with one uniform real
bound, and suppose $f_n\to f$ almost everywhere for each component
$\mu_\alpha$. The exceptional null set may depend on $\alpha$;
no uncountable intersection of full-measure sets is needed. Ordinary
dominated convergence gives convergence of each component integral.
The coefficient formula in the proof of Theorem~\ref{fsp:thm:hierarchy}
is a finite linear combination of those integrals at every fixed exponent.
Thus $\E_Pf_n$ converges
...
or ordered-field convergence. For example, the constant functions
$f_n=1/n$ have expectations $1/n$, which converge coefficientwise to
zero but stay above any fixed positive infinitesimal. Their errors all
have valuation zero, so they do not converge in the intrinsic valuation
topology either.

For two set-sized hierarchies on two measurable spaces, take both exponent
families in one ordered group $\Gamma$, and let $f$ be a bounded real
function measurable for the product sigma-algebra. Their well-ordered
supports have a well-ordered sum and finitely many decompositions at each
exponent by the Hahn product lemma. Thus the component product measures
and monomial weights define a positive normalized law, with normalizer
$Z_PZ_Q$, even for uncountable hierarchies. Coincident exponents combine
by finite sums. Ordinary Fubini on each component product then proves
the corresponding coefficientwise identity. Explicitly, the iterated
integral is defined through component pairing as
\begin{equation}
 \frac{1}{Z_PZ_Q}\hsum_{\alpha,\beta}
 t^{\gamma_\alpha+\delta_\beta}
 \int\!\int f(x,y)\,d\nu_\beta(y)\,d\mu_\alpha(x).
\end{equation}
Reversing the two ordinary integrals leaves each component unchanged;
Hahn multiplication by the fixed reciprocal normalizers then preserves
equality. The support condition is automatic for these two hierarchies
in a common value group. One does not apply the real-observable definition
unchanged to an arbitrary Hahn-valued inner integral. This construction
does not give a Fubini theorem for all surreal-valued functions.
...
countable. Two cocountable sets cannot be disjoint on the uncountable
space $\omega_1$. These cases give respectively total mass zero and one.

Take the additive subgroup $\Gamma\subset\No$ generated by the
set of surreal ordinals $\xi\le\omega_1$, with its inherited order;
it is set-sized because finite integer combinations of a set form a set.
Use the associated Hahn field, with $t^\gamma=\omega^{-\gamma}$
in the canonical surreal realization. Set
...
write $X=\sum_{\gamma\in W}x_\gamma t^\gamma$ and suppose
first that the integral has a least nonzero coefficient at $\eta$.
For $\gamma<\eta$ in $W$, transfinite induction shows
$x_\gamma=0$ almost everywhere. Indeed, outside the countable union
of the earlier exceptional null sets, all preceding coefficients vanish.
Hahn positivity then forces $x_\gamma\ge0$ there; its zero integral
forces it to vanish almost everywhere. Apply the same argument at
$\eta$: $x_\eta\ge0$ almost everywhere and its nonzero integral
is strictly positive. If there is no nonzero integral coefficient, the
integral is zero. This proves the claim, including finite and empty
supports. Applying it also to $M-X$ gives
$0\le\int X\,d\mu\le M\mu(\Omega)$ when $0\le X\le M$;
for a probability measure this preserves the bound $M$. The failure in \eqref{fsp:eq:badintegral}
occurs at exponent $\omega_1$, where removing the earlier exceptional
singletons would remove the whole sample space: an uncountable union
of null sets need not be null.
...
setting \cite[Theorem~8.5]{kallenberg}, applied with the observation
as the conditioning variable and the state as the Borel-valued variable.
It is an ordinary real disintegration input.

Take $\rho=(m+1)^{-1}\sum_j\mu_j^Y$ and let
$h_j=d\mu_j^Y/d\rho$. Choose versions which are finite and nonnegative
on a common full-$\rho$ measurable set $G$. By uniqueness of
Radon--Nikodym derivatives, $\sum_jh_j=m+1$ almost everywhere;
shrink $G$ to enforce this equality everywhere on $G$. Thus
$0\le h_j\le m+1$ there and at least one $h_j$ is positive.
Set every $h_j$ to zero off $G$. For $y\in G$ and
$0=\gamma_0<\cdots<\gamma_m$, put
\begin{align}
...
\end{align}
For each $y\in G$, let $j$ be the first index with $h_j(y)>0$.
Factoring $t^{\gamma_j}h_j(y)$ out of $D(y)$ leaves one plus a
finite positive-exponent series. The denominator is therefore positive,
and expansion of its inverse makes each coefficient of $P_y$ a finite
real linear combination of the ordinary probability measures
$K_k(y,\cdot)$. This proves coefficientwise countable additivity in
$A$; positivity and normalization follow directly from the quotient.
Its standard part is $K_j(y,\cdot)$.

There is also a common support bound as $y$ varies. On the measurable
piece with first active index $j$, the inverse expansion uses the additive
monoid generated by the positive gaps
$\gamma_k-\gamma_j$ for $k>j$. This monoid is well ordered with
finitely many contributions to each coefficient by the Neumann lemma.
The finite union over $j$ is again well ordered. Every coefficient of
$P_y(A)$ is measurable in $y$, being a finite sum of products and
quotients of measurable functions with $h_j(y)>0$ on that piece.
Extend $P_y$ by any fixed ordinary probability law off $G$; this adds
at most exponent zero and preserves coefficientwise measurability.
...
termwise integration operation. For an explicit example, take
$Y\in(0,1)$, $X\in\{0,1\}$, and two components: under $\mu_0$,
$X=0$ and $Y$ has density $2y$; under $\mu_1$, $X=1$ and $Y$
has density $2(1-y)$. Then $\rho$ is Lebesgue probability,
$h_0(y)=2y$, $h_1(y)=2(1-y)$, and weights $1,t$ give
\[
 P_y(\{1\})=\frac{t(1-y)}{y+t(1-y)}.
\]
Its coefficient at $t$ is $(1-y)/y$, whose real integral diverges at
zero. Yet $D(y)P_y(\{1\})=2t(1-y)$ is integrable coefficientwise,
and reconstruction gives $P(X=1)=t/(1+t)$. Boundedness of the
posterior as a Hahn value does not bound its separate real coefficients.
...
$\{0,1\}^{\N_{\ge1}}$ with its Borel sigma-algebra. Summing over a
new coordinate multiplies any earlier pattern weight by $p+(1-p)=1$,
so the finite laws are consistent; every pattern weight is positive for
both biases. The obstructions below concern a real coefficient measure's
finite total variation. Positivity of the proposed Hahn extension is not
used, so signed extensions are excluded as well. The conclusions are special
...
coordinates do not extend even to a coefficientwise Hahn signed measure
with ordinary finite signed coefficient measures.
\end{theorem}
\begin{proof}
For $n\ge1$, let
$B_n=\{x:x_1=\cdots=x_{n-1}=0,\ x_n=1\}$.
The initial zero string is empty at $n=1$. These measurable cylinder
events are pairwise disjoint: a path cannot have two different first
success times. Their prescribed finite-dimensional laws require
...
In fact the complementary event $C_N=\{x_1=\cdots=x_N=0\}$
has mass $(1-t)^N$ and coefficient $-N$ at $t$.
The finite partition $B_1,\ldots,B_N,C_N$ therefore gives the sharper
bound $|\mu_1|(\Omega)\ge2N$. Given a proposed finite real total
variation $M$, an ordinary integer $N>M/2$ is already a contradiction;
no infinite sum of surreal event masses is taken.
\end{proof}
...
\end{theorem}
\begin{proof}
Fix an ordinary integer $n\ge1$. A length-$n$ pattern with $k$
successes has probability $(1/2+t)^k(1/2-t)^{n-k}$.
To obtain its coefficient at $t$, choose the linear term from exactly
one factor: the $k$ success factors contribute $+2^{-(n-1)}$ each,
and the $n-k$ failure factors contribute $-2^{-(n-1)}$ each. Thus
...
\[
 \sum_{k=0}^n\binom nk\,|2(2k-n)2^{-n}|=2\E|S_n|,
\]
where $S_n=\varepsilon_1+\cdots+\varepsilon_n$ under the ordinary
uniform law on $n$ independent signs $\varepsilon_i\in\{-1,1\}$.
There are $\binom nk$ patterns with $S_n=2k-n$; this explains the
partition multiplicities. Centering and independence make every product
with an odd occurrence of some index have expectation zero. Consequently
\[
 \E S_n^2=n,\qquad
 \E S_n^4=n+6\binom n2=3n^2-2n\le3n^2.
\]
The fourth moment counts either one index four times or two indices
twice each. H\"older with conjugate exponents $3/2$ and $3$, applied to
$|S_n|^2=|S_n|^{2/3}|S_n|^{4/3}$, gives
\begin{equation}
 n\le(\E|S_n|)^{2/3}(\E S_n^4)^{1/3},
 \qquad \E|S_n|\ge\sqrt{n/3}.
\end{equation}
Cubing the first inequality gives
$n^3\le(\E|S_n|)^2\E S_n^4\le3n^2(\E|S_n|)^2$;
divide by $n^2>0$ and take the nonnegative real square root.
A proposed finite variation $M$ is contradicted by any ordinary integer
$n>3M^2/4$, since then $2\sqrt{n/3}>M$. Thus no finite signed
coefficient measure has the required values on all cylinder partitions.
The argument uses finite real moments, not a central limit theorem.
\end{proof}
...
one quarter of those in \eqref{fsp:eq:coinrow}, and their variations
are at least $\tfrac12\sqrt{n/3}$, still unbounded.
Likewise, put $u=\exp(-L)$ for positive infinite $L$. In canonical
Hahn coordinates write its leading term as $a t^\beta$, with
$a>0$ real and $\beta>0$; $u$ need not itself be a monomial.
The shared success probability $\sigma(-L)=u/(1+u)$ has the same
leading term, as does every first-success mass
$u/(1+u)^n$. Its exponent-$\beta$ coefficient is therefore the same
positive real $a$ for all $n$, again forcing unbounded coefficient
variation. This argument uses the original Hahn coordinates and does
not assume that a change of scalar variable preserves coefficientwise
measure structure. Specifying logits does not supply a missing
infinite-addition rule.
...
\end{lemma}
\begin{proof}
Let $M$ be an ordered real closure of $L$, still a set-sized field.
The subfield $K_0$ of elements of $M$ algebraic over $K$ is a real
closure of $K$: positive square roots and roots of odd-degree polynomials
in $M$ remain algebraic over $K$. The same argument in the real closed
field $\No$ gives its real closure of $K$, consisting of the surreals
algebraic over $K$. This is set-sized, since there are set many polynomials
and each has finitely many roots. Uniqueness of ordered real closures
identifies these two fields by an ordered isomorphism fixing $K$.

Choose and well order a transcendence basis of $M$ over $K_0$. Extend
this initial embedding successively, keeping its domain a real closed
subfield of $M$ and its image real closed. At a step let $F$ be the
current domain and $x$ the next basis element. This $x$ is transcendental
over $F$, because earlier real-closure steps added only algebraic elements
over the field generated by earlier basis elements. The sets
\begin{equation}
 \{j(a):a\in F,\ a<x\},\qquad
 \{j(b):b\in F,\ x<b\}
\end{equation}
form a surreal cut: every left entry is smaller than every right entry.
The cut property applies even if one side is empty. Take its simplest
surreal $y$, which lies strictly between the two sides. No element of $j(F)$ can equal $y$, since
every element of $F$ lies strictly on one side of $x$. A real closed
ordered field has no proper ordered algebraic extension, so $y$ is
transcendental over $j(F)$.

Substitution $x\mapsto y$ therefore gives an injective field map on
$F(x)$. It preserves order: a nonzero polynomial over a real closed
field factors into a nonzero constant, linear factors, and monic
irreducible quadratic factors. The latter are positive everywhere
(by completing the square), the linear signs are determined by the cut,
and the constant's sign is already preserved. Apply this to numerator
and denominator to preserve signs of rational functions. The ordered
embedding extends uniquely to the real closure of $F(x)$ inside $M$,
with image its real closure inside $\No$.

At a limit stage take unions. Any positive element or odd-degree
polynomial has all its data in one earlier stage, where its required
root already exists, so the union remains real closed. After the whole
basis has been included, $M$ is algebraic over the resulting real closed
subfield and hence equals it. Restrict the resulting embedding of $M$
to $L$. Every stage, image and cut is a set, since the basis is set-sized
and algebraic closure steps adjoin set many elements. The original
field $K$ is fixed throughout.
\end{proof}
...
construction in the literature: compare the fine-ideal construction
in \cite[Section~4.2]{bhw} and the fine-ultrafilter regularity and
uniformity results in \cite[Definition~4 and Propositions~5--6]{brickhill}.
The sections and numbers refer to the cited arXiv preprints. Let $\Omega$ be
...
cones are cones: the intersection for $F_1,\ldots,F_r$ is the cone
for their finite union. The empty set is in no such filter because every
cone has a nonempty finite member. Extend this proper cone filter to an
ultrafilter $\mathcal U$. It is fine, meaning that every cone containing
one prescribed point belongs to it. It cannot be principal at any
$s\in I$, since a point outside that finite $s$ has a cone omitting $s$.
This use of the ultrafilter lemma is nonconstructive.

The ultrapower $L=\R^I/\mathcal U$ is a set-sized ordered field;
reals embed as constant functions. Its order is defined by
$[f]<[g]$ exactly when $\{s:f(s)<g(s)\}\in\mathcal U$.
In particular a positive class need not have a positive real lower bound.
Define
...
For $a\in A$, every snapshot in its cone has
$|A\cap s|/|s|\ge1/|s|>0$, so $P(A)>0$ in the ultrapower order.
For every ordinary $n$, choose $n+1$ distinct points of the infinite
set $\Omega$; their cone forces $|s|>n$. On the cone containing $a$,
the singleton's snapshot frequency is exactly $1/|s|$. Thus
\begin{equation}
 H>n\quad(n\in\N),\qquad P(\{a\})=1/H
 \quad(a\in\Omega).
\end{equation}
Lemma~\ref{fsp:lem:embedding}, applied over $\R$, turns these into surreal
probability values without changing any field identities or real constants.
The reciprocal $1/H$ is positive infinitesimal, since $H$ exceeds every
ordinary integer and hence every real bound. This yields positive equal
singleton masses even on an uncountable set. Every finite set $F$ has
mass $|F|/H$, by finite additivity.
...
probability exactly $1/2$, because the even count in $s_n$ is $n/2$
on those indices. Such nonprincipal ultrafilters exist by extending the
cofinite filter together with the infinite set of even indices.
Using odd indices instead, the count is $(n-1)/2$, giving probability
\begin{equation}
...
finite support. If a permutation moves only points in a finite set $F$,
it fixes $A\setminus F$ and preserves the cardinality of $A\cap F$;
finite additivity therefore preserves $P(A)$. This does not give invariance
under arbitrary transformations. On $\N_{\ge1}$ the shift
$n\mapsto n+1$ is an injection, not a permutation; invariance under
that injection would give
$P(\N_{\ge1}\setminus\{1\})=P(\N_{\ge1})$,
contradicting $P(\{1\})>0$.
Even invariance under all permutations of $\N_{\ge1}$ is impossible:
partition it into two infinite sets $A,C$ and choose $a\in A$.
Bijections $A\to A\setminus\{a\}$ and
$C\to C\cup\{a\}$ combine into a permutation, under which
$P(A)$ decreases by $P(\{a\})>0$.
...
$K\subseteq\No$ is a set-sized ordered subfield. There is a set-sized ordered field
...
This is a set-sized language because $K$ and $\mathcal P(\Omega)$
are sets. Include the ordered-field axioms and the atomic diagram of
$K$ with inequalities and distinctions between different constants, so
that their interpretations form an ordered copy of $K$. Add
$p_\varnothing=0$, $p_\Omega=1$, every disjoint finite-additivity
identity, $p_A>0$ for every nonempty $A$, and
$p_A=P_0(A)$ for $A\in\A$. This is a set of first-order sentences.
...
mentioned in the fragment, including every union appearing in an
additivity identity. Within a parent atom $C$ of mass $b=P_0(C)>0$,
let $r_C\ge1$ be the number of its nonempty refined children and give
each child mass $b/r_C$. This is positive in $K$, and their total is
$b$. Assign each mentioned event the sum of the masses of its children.
Every nonempty mentioned event contains a nonempty child, so its assigned
mass is positive. Each original event is a union of parent atoms and
therefore retains its $P_0$ mass. Disjoint unions correspond to disjoint
lists of children, satisfying every additivity constraint in the fragment;
the whole space has mass one and the empty set zero. Constants for
unmentioned events may be interpreted arbitrarily for this fragment.
The constants from $K$ retain their given values. The assignments for
different finite fragments need not agree; compactness is what produces
...
Imposing such extra conditions may block the construction. If $\Omega$
is finite, the same splitting can be done once down to singletons, giving
an extension already in $K$; the compactness argument is needed for the
unrestricted infinite case. Regularity is an actual hypothesis on the
original event algebra, not a property supplied by changing scalar fields.
...
probabilities on all subsets of $\{0,1\}^{\N_{\ge1}}$, with the original
$t=\omega^{-1}$ unchanged.
\end{corollary}
\begin{proof}
Every cylinder-algebra event depends on finitely many coordinates and
is a union of full patterns on those coordinates. For either bias,
$0<p<1$, so each pattern has positive mass $p^k(1-p)^{n-k}$.
Refining to any larger finite coordinate set preserves masses by
$p+(1-p)=1$, proving that this assignment is well defined and finitely
additive. A nonempty event contains a pattern and therefore has positive
mass. All these finite probabilities lie in $K=\R(t)\subseteq\No$.
Apply Theorem~\ref{fsp:thm:extension} over this $K$ and use the
embedding that fixes $K$ pointwise, in particular the original $t$.
\end{proof}
...
Index coordinates by the positive integers. Let $\mu_0$ be the ordinary
independent Bernoulli$(1/3)$ law on $\{0,1\}^{\N_{\ge1}}$ and $\mu_1$
the independent Bernoulli$(2/3)$ law, both on the product sigma-algebra.
Attach a latent label $H\in\{0,1\}$ and give the joint space the Hahn law
...
\end{equation}
Indeed, independence within each component gives, for $i\ne j$,
\[
 \E(X_iX_j)=\frac{1+4t}{9(1+t)},\qquad
 \Cov(X_i,X_j)
 =\frac{(1+4t)(1+t)-(1+2t)^2}{9(1+t)^2}
 =\frac{t}{9(1+t)^2}.
\]
Each component is invariant under finite coordinate permutations, so the
mixture is exchangeable. Its positive covariance rules out independence.
Therefore its infinitesimally shifted
...
Let $\F_n=\sigma(X_1,\ldots,X_n)$ for ordinary integers $n\ge0$, with
$\F_0$ trivial, and write $K_n=\sum_{i=1}^nX_i$ (so $K_0=0$).
An observation word of length $n$ with $k$ successes has likelihoods
$(1/3)^k(2/3)^{n-k}$ and $(2/3)^k(1/3)^{n-k}$ in the two regimes.
Both are positive, and their ratio is $2^{2k-n}$. Every atom of $\F_n$
therefore has positive probability. Bayes' rule gives the finite conditional
probability of the rare regime:
\begin{equation}\label{fsp:eq:latentposterior}
 M_n=P(H=1\mid\F_n)
 =\frac{t\,2^{2K_n-n}}{1+t\,2^{2K_n-n}}.
\end{equation}
Every value of $M_n$ is positive infinitesimal for each ordinary $n$:
the factor $r_n=2^{2K_n-n}$ is a positive real number. In particular,
$M_0=t/(1+t)$ and $0<M_n<1$. These adapted finite-range variables
form a bounded martingale by the finite tower property. One can also
check this directly. Given a word with likelihood ratio $r$, the next
success probability and the two next posterior values are
\[
 q=\frac{1+M_n}{3}=\frac{1+2tr}{3(1+tr)},\qquad
 M_{n+1}^{(1)}=\frac{2tr}{1+2tr},\qquad
 M_{n+1}^{(0)}=\frac{tr}{2+tr}.
\]
Then $qM_{n+1}^{(1)}+(1-q)M_{n+1}^{(0)}=tr/(1+tr)=M_n$.
All these conditional expectations use finite partitions; no infinite
surreal sum or martingale convergence theorem is used.
...
\end{equation}
Here an elementary real argument suffices for the imported strong law.
For $h\in\{0,1\}$, write $\E_h$ for expectation under $\mu_h$,
and put $p=(1+h)/3$, $Y_i=X_i-p$.
Independence, centering and $\E_h Y_i^2\le1/4$, $\E_h Y_i^4\le1$ give, for $n\ge1$,
\[
 \E_h\Bigl(\sum_{i=1}^nY_i\Bigr)^4
 =\sum_{i=1}^n\E_h Y_i^4
   +6\sum_{i<j}\E_h Y_i^2\E_h Y_j^2
 \le n+\frac{3n(n-1)}{16}\le n^2.
\]
For each real $\varepsilon>0$, Markov's inequality therefore bounds
the component probability of $|K_n/n-p|\ge\varepsilon$ by
$1/(n^2\varepsilon^4)$.
The probability of a violation at some $n\ge N$ is bounded by the
tail of this convergent real series, which tends to zero. Thus violations
occur only finitely often almost surely. Intersecting these full-measure
events for $\varepsilon=1,1/2,1/3,\ldots$ proves $K_n/n\to p$.
This argument is entirely within the two real component measures.

The convergence condition defining $B$ uses countably many inequalities
between measurable real functions, so $B$ is measurable. For every fixed
$m$, the same limit is obtained from $\sum_{i=m+1}^nX_i/n$, which depends
only on coordinates after $m$. Hence $B$ belongs to every future-coordinate
sigma-algebra and is a tail event. We also write $B$ for its inverse image
in the joint space. Consequently
...
This also has a precise conditional-expectation formulation. Put
$\F_\infty=\sigma(X_1,X_2,\ldots)$. For every observation event $A$,
\[
 P(H=1,A)=\frac{t\mu_1(A)}{1+t}
 =\frac{\mu_0(B\cap A)+t\mu_1(B\cap A)}{1+t}
 =P(B\cap A),
\]
because $\mu_0(B)=0$ and $\mu_1(B^c)=0$. Thus the $\F_\infty$-measurable
variable $\ind_B$ satisfies
$\E[\ind_A\ind_{\{H=1\}}]=\E[\ind_A\ind_B]$ for every
$A\in\F_\infty$. This is precisely the defining integral identity for a
version of conditional expectation, using only finite-range integrals.
Also $P(\{H=1\}\mathbin{\triangle}B)=0$. The martingale $(M_n)$ does not
recover this version on $B$ even at a fixed real resolution.

Off $B$, convergence to zero fails at infinitesimal resolution as well.
In the fixed chart $t=\omega^{-1}$, each $M_n$ has valuation one and
leading coefficient $r_n>0$, so $M_n>t^2$ at every ordinary time and on
every path. More explicitly, if the next observation is $1$ or $0$,
respectively, then
\[
 M_{n+1}-M_n=
 \begin{cases}
 \displaystyle\frac{tr_n}{(1+2tr_n)(1+tr_n)},&X_{n+1}=1,\\[6pt]
 \displaystyle-\frac{tr_n}{(2+tr_n)(1+tr_n)},&X_{n+1}=0.
 \end{cases}
\]
The leading coefficient of the absolute difference is $r_n$ or $r_n/2$,
respectively, and its valuation is one. Consequently
$|M_{n+1}-M_n|>t^2$ everywhere. The single order tolerance $t^2$ therefore
violates the Cauchy condition on every path, regardless of how small or
large the real factors $r_n$ become. These are ordinary integer times;
no transfinite extension of the process is being asserted.
...
Coefficientwise convergence gives a third, different answer. At each
ordinary time the geometric expansion is
\[
 M_n=\hsum_{j\ge1}(-1)^{j-1}r_n^j t^j.
\]
On the event $B_0=\{K_n/n\to1/3\}$, which has $\mu_0$-measure one,
$(2K_n-n)/n\to-1/3$, so $r_n\to0$ in $\R$. Every fixed coefficient
then tends to zero: $M_n$ converges coefficientwise to zero on $B_0$,
although it is nowhere order-Cauchy. On $B$, instead,
$(2K_n-n)/n\to1/3$ and $r_n\to+\infty$; already the coefficient of $t$
has no finite real limit. This coefficientwise statement is a limit in
$\R$ at each fixed exponent, not strong Hahn summation of the time sequence.
...
\end{theorem}
\begin{proof}
Since $0\le P(A)\le1$, every event probability is limited. Standard
part preserves finite sums and order, so $P_0$ is a real finitely additive
probability. Such a probability is countably additive exactly when it
is continuous from above at the empty set. Here are both directions.
If $A_n\downarrow\varnothing$, set $D_n=A_n\setminus A_{n+1}$ for
$n\ge1$. The $D_n$ are disjoint and $A_m=\bigcup_{n\ge m}D_n$.
Countable additivity gives $P_0(A_m)=\sum_{n\ge m}P_0(D_n)\to0$, since
the nonnegative real series has total $P_0(A_1)\le1$.
Conversely, let $B_n$ be disjoint measurable sets and
$B=\bigcup_{n\ge1}B_n$. The measurable remainders
$A_N=B\setminus\bigcup_{n=1}^N B_n$ decrease to the empty set, and
\[
 P_0(B)=\sum_{n=1}^N P_0(B_n)+P_0(A_N).
\]
Continuity from above makes the last term tend to zero and proves
countable additivity.
...
hence zero. Monotonicity then makes the whole sequence tend to zero,
not just a subsequence. This is the required continuity. The condition
uses real tolerances: it does not assert $P(A_n)\to0$ at every positive
surreal tolerance.
\end{proof}

The standard part of the normalized hierarchy is the ordinary measure
$\mu_0$ and satisfies this test. The all-subsets uniform law on
$\N_{\ge1}$ from \eqref{fsp:eq:ultrafilter} does not: singleton shadows are zero while
...
In the finite-mass case used here, Loeb's construction starts with
an internal sample space $\Omega$, an internal algebra $\mathcal I$ of
subsets, and an internal finitely additive probability
$\nu:\mathcal I\to{}^*\R$ in a countably saturated nonstandard setting
\cite{loeb}. Here countable saturation means that a countable family of
internal subsets of $\Omega$ with the finite-intersection property has
nonempty intersection. All values of $\nu$ lie in $[0,1]$, and
$\nu_0(A)=\st\nu(A)$ is a real finitely additive probability on
$\mathcal I$, regarded as an ordinary algebra of subsets of $\Omega$.
It need not be a sigma-algebra.

To check the premeasure property, take disjoint $A_1,A_2,\ldots\in\mathcal I$
whose union $A$ also belongs to $\mathcal I$. The remainders
\[
 R_N=A\setminus\bigcup_{n=1}^N A_n
\]
are internal and decrease to the empty set. If every $R_N$ were nonempty,
countable saturation would give a point in their intersection. Hence some
$R_N$ is empty. Disjointness then forces $A_n=\varnothing$ for $n>N$,
so finite additivity gives $\nu_0(A)=\sum_{n\ge1}\nu_0(A_n)$.
This is exactly the countable-additivity condition required of a
premeasure on an algebra; no claim that arbitrary countable unions
remain internal is needed.

The ordinary finite-measure extension theorem now gives the unique
probability on $\sigma(\mathcal I)$ extending $\nu_0$. Its completion
is the Loeb probability $\nu_L$. Thus countable additivity concerns a
real measure on an enlarged event domain, not a Hahn sum of the original
hyperreal probabilities. Saturation was a statement about internal
\emph{sets}, not merely about the order of the scalar field.

An ordered embedding of the underlying scalar field into $\No$ fixing
$\R$ preserves standard parts. Indeed, if $\st x=r$, then
$r-\varepsilon<x<r+\varepsilon$ for every positive real $\varepsilon$;
the same inequalities hold after embedding. Such an embedding exists
for the set-sized hyperreal field by Lemma~\ref{fsp:lem:embedding}, with
$K=\R$. Transporting the internal probability values therefore leaves
$\nu_0$ and its Loeb extension unchanged on the specified event space.
The embedding alone does not manufacture internal events, saturation,
hyperfinite sets or transferred sums. A bare surreal $H>\N$ is not a
substitute for this package of structure.

\subsection{A Poisson shadow at reciprocal-probability sample size}\label{fsp:sub:poisson}

Fix a positive real $\lambda$ and let $I=\{N\in\N:N>\lambda\}$.
For each $N\in I$, put $\Omega_N=\{0,1\}^N$ and give a word $x$ the
ordinary product probability
\[
 P_N(\{x\})=(\lambda/N)^{S_N(x)}(1-\lambda/N)^{N-S_N(x)},
 \qquad S_N(x)=\sum_{i=1}^N x_i.
\]
Both one-trial weights are positive and sum to one, so the binomial
theorem normalizes $P_N$. Fix a nonprincipal ultrafilter $\mathcal U$
on $I$, and form the set-sized ordered field ${}^*\R=\R^I/\mathcal U$
and the sample space $\Omega=\prod_{N\in I}\Omega_N/\mathcal U$.
An internal event, denoted $[A_N]$, consists of those $[x_N]$ for which
$\{N:x_N\in A_N\}\in\mathcal U$, where $A_N\subseteq\Omega_N$.
These events form an algebra $\mathcal I$, with internal probability
\[
 \nu([A_N])=[P_N(A_N)].
\]
Equality of two internal events forces their representing sets to agree
on a $\mathcal U$-large set: otherwise one can select points in their
symmetric differences and obtain a point belonging to just one event.
Thus $\nu$ is well-defined. Finite set operations and finite additivity
pass coordinatewise to the quotient.

The needed countable saturation has a direct diagonal proof in this
model. Suppose internal events $A_j=[A_{j,N}]$, $j\ge1$, have the
finite-intersection property. Put
\[
 D_m=\{N:\bigcap_{j=1}^m A_{j,N}\ne\varnothing\}\in\mathcal U,
 \qquad
 m(N)=\max\bigl(\{0\}\cup\{m\le N:N\in D_m\}\bigr).
\]
Choose $x_N$ in the intersection of the first $m(N)$ sets, taking any
point of $\Omega_N$ if $m(N)=0$. For fixed $j$, the set
$D_j\cap\{N:N\ge j\}$ belongs to $\mathcal U$ and there $m(N)\ge j$.
Hence $[x_N]\in A_j$ for every $j$. The preceding Loeb construction
therefore applies to this explicit ultraproduct.

Write $H=[N]$ and $S([x_N])=[S_N(x_N)]$ for the hyperinteger sample size
and internal count. Since $\mathcal U$ contains every cofinite subset,
$H$ exceeds every ordinary integer. A trial indexed internally by
$[i_N]$, $1\le i_N\le N$, has probability $[\lambda/N]=\lambda/H$,
a positive infinitesimal. The count $S$ and the internal trial structure
come from this construction, not from the scalar $H$ alone.
...
\begin{equation}
 ...
 \left(1-\frac\lambda N\right)^{N-k}.
\end{equation}
For $k=0$, the product is empty and equals one. For any fixed $k$ it
tends to one in ordinary real analysis. A quantitative logarithm bound
handles the remaining factor: for $0\le u\le1/2$,
\[
 0\le-\log(1-u)-u=\int_0^u\frac{s}{1-s}\,ds\le u^2.
\]
Thus, for $N\ge\max(k,2\lambda)$,
\[
 \bigl|(N-k)\log(1-\lambda/N)+\lambda\bigr|
 \le\frac{k\lambda+\lambda^2}{N}\longrightarrow0.
\]
Continuity of the ordinary real exponential gives the limit
$e^{-\lambda}$ of the last factor. Finally, if a bounded real sequence
$a_N$ tends to $a$, every real tolerance holds on a cofinite, hence
$\mathcal U$-large, set. Therefore $[a_N]-a$ is infinitesimal, proving
the standard-part formula in \eqref{fsp:eq:poisson}.

For each ordinary $k\ge0$, the event $E_k=\{S=k\}$ is internal.
The disjoint union $E_{\rm fin}=\bigcup_{k\ge0}E_k$ is Loeb measurable,
even though one must not assume it internal. Countable additivity gives
\[
 \nu_L(E_{\rm fin})
 =\sum_{k=0}^{\infty}e^{-\lambda}\frac{\lambda^k}{k!}=1.
\]
Here the last equality is the ordinary real exponential series.
There is also a direct check that no mass escapes to infinite counts:
for every positive ordinary integer $m$, finite expectation gives
$\E_{P_N}S_N=\lambda$, and Markov's inequality yields
$\nu_L(S>m)\le\lambda/m$. These events decrease to
$\Omega\setminus E_{\rm fin}$: a hyperinteger bounded by an ordinary
integer takes one of finitely many ordinary values, as the ultrafilter
selects one cell of the finite partition. Continuity from above gives
zero mass to their intersection.

Define an ordinary nonnegative integer-valued variable $Z$ to equal $k$ on $E_k$
and zero on $\Omega\setminus E_{\rm fin}$. Its fibers are Loeb
measurable and the extra part of its zero fiber is null. Consequently
$Z$ has exactly the ordinary Poisson$(\lambda)$ law. This is the precise
sense in which the internal count has a Poisson shadow at sample size
$H=\lambda/(\lambda/H)$, comparable to the reciprocal of its infinitesimal
one-trial probability. It supplies the meaning of the informal scale
calculation $n\asymp1/p$ in this chosen nonstandard experiment.

After an ordered-field embedding into $\No$ fixing $\R$, the probabilities and
...
For a finite model, a useful representation stores nonnegative
unnormalized weights with positive total, retaining exact zeros. Requiring
every weight to be positive restricts this interface to regular laws on
the full event algebra. A conditional query must certify that the total
weight on its conditioning event is positive. Normalization,
...
Theorem~\ref{fsp:thm:stability} supplies a sufficient precision contract:
when conditioning on mass of valuation $\beta$ and requesting output through
scale $\lambda$, require input error beyond $\beta+\lambda$.
This uniform guarantee is not a necessary condition for each query;
specific numerator--denominator cancellations can permit weaker precision.
...
\item An ordered-field layer with an increasing exponential group
isomorphism onto the positive elements, its inverse logarithm, and the
scalar inequalities used in the proofs: logit, softmax, entropy, relative
entropy, logarithmic scoring and the Gibbs identity. The Brier score
belongs already to the first layer.
...
exponential, standard part, strong sums, and internal structure. An ordered
embedding fixing $\R$ preserves standard parts: the inequalities
$r-\varepsilon<x<r+\varepsilon$ for every positive real $\varepsilon$
are preserved. Preservation of exponentials, strong sums or internal
structure requires additional hypotheses.
...
We do not assert a general infinite probability theory using countable
additivity in the full fine topology of $\No$.
Proposition~\ref{fsp:prop:fine} explains why ordinary set-indexed convergence
there is unusually restrictive. Finite-support laws are an elementary
exception: in a disjoint sequence only finitely many events can meet a
fixed finite support, so the partial probability sums eventually stabilize
at the probability of the union. This does not supply a theory of arbitrary
infinite sampling.
...
$H$ in the latent model & The binary latent label; unrelated to internal sample size\\
$\mathcal I,\nu,\nu_L$ & Internal event algebra, internal probability and its completed real Loeb measure\\
\bottomrule
\end{tabularx}
\end{center}
...
Section~\ref{fsp:sec:latent} imports two ordinary real product laws and
proves their strong laws of large numbers by a fourth-moment argument. Section~\ref{fsp:sec:bridges} distinguishes
ordinary real measure extension from the internal-set saturation used in the
Loeb construction; it proves the needed saturation for the Poisson ultraproduct. No infinite result is deduced solely from the finite
