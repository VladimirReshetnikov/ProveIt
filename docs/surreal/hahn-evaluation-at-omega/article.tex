...
\usepackage[colorlinks=true,linkcolor=OliveDark,citecolor=OliveDark,urlcolor=OliveDark,
  ...
  pdfsubject={Negative answer to Lipparini Problem 7.7; Hahn series and surreal numbers;
    nonnegative-coefficient semiring certificate},
...
with a nonnegative-coefficient semiring certificate,\par
...
preservation of infinite sums is assumed. We also give a direct obstruction
on the semiring $\Qnn[[x]]$ of nonnegative-coefficient series, where there
are no additive inverses, excluding every value $u\ge1$. Restriction and
extension of homomorphisms relate the two formulations; the positive witness
supplies a separate elementary proof. We prove that the possible
...
semiring certificate of \cref{sec:semiring}, the general unit square root
...
from the second report, while the infinitesimal evaluation criterion, the
...
Section~\ref{sec:semiring} gives a separate proof using only nonnegative
coefficients and semiring identities. It does not use the proof in
Section~\ref{sec:main}; its obstruction also implies that section's theorem
by restriction of homomorphisms. Sections~\ref{sec:inf}
...
\begin{remark}[How the ring and semiring obstructions relate]
\label{rem:incomparable}
The two arguments give different elementary certificates. Their
homomorphism claims are related by restriction and extension. Every unital ring
homomorphism $\Q[[x]]\to K$ restricts to a unital semiring homomorphism
$\Qnn[[x]]\to K$, with the same image of $x$. Its codomain is still $K$;
no claim that every series has a nonnegative image is needed. Consequently
\cref{thm:semiring} implies \cref{thm:main} and also excludes $u=1$.

In fact, restriction is a bijection between these two kinds of maps into
any fixed commutative ring $K$. To see this without an abstract completion
theorem, write each $f\in\Q[[x]]$ as $f=a-b$ with $a,b\in\Qnn[[x]]$,
using the positive and negative parts of each rational coefficient. Given
a semiring map $\psi$, define
\[
 \overline\psi(f)=\psi(a)-\psi(b).
\]
This is independent of the representation: if $a-b=a'-b'$, then
$a+b'=a'+b$ in the semiring, so applying $\psi$ gives the required equality
of differences in $K$. The formulas
\[
 (a-b)+(c-d)=(a+c)-(b+d),\qquad
 (a-b)(c-d)=(ac+bd)-(ad+bc)
\]
show that $\overline\psi$ preserves addition and multiplication. It preserves
$0$ and $1$, extends $\psi$, and is the only ring map doing so because
every $f$ is such a difference. Conversely, extending the restriction of
a ring map recovers that map.

Thus \cref{thm:main}, applied to the extension, already excludes semiring
maps with $u>1$. The endpoint $u=1$ is excluded directly by
$\psi(G)=1+\psi(G)$. The positive witness in \cref{lem:H} remains useful:
it proves the full $u\ge1$ obstruction from two identities in the semiring,
without introducing a ring extension. The earlier version of this remark
incorrectly described the statements as incomparable; the distinction is
between their explicit proofs, not the existence of the corresponding maps.
\end{remark}
...
once --- an arbitrary unit with constant term $1$ rather than $1+qx$, a field of
...
\end{proposition}
\begin{proof}
The restriction of $\phi$ to $\Q$ is injective, so $F$ has characteristic
zero; identify its prime subfield with $\Q$. Unitality then fixes these
rational constants. If $\phi(x)=0$, write
...
The map just constructed sends $x^m$ to $u^m\ne0$ and sends the unit $v$
to a nonzero element, so it is injective. For $u=0$, constant-term evaluation
is a coefficient-fixing homomorphism. It is strongly additive because only
finitely many members of a source-summable family have a nonzero constant
term, so taking that coefficient commutes with the sum.
\end{proof}
...
\citep[Sections 3 and 4]{KKS}. The construction here is substitution of an
arbitrary infinitesimal series. When $u=\omega^a$ with $a<0$, it reduces to
the exponent change $n\mapsto na$ on monomials; this special case belongs
to the familiar change-of-value-group construction for Hahn fields, written
in the opposite exponent convention; compare \citep[Section 2.3]{KKS}.
For a general $u$, its powers can have many terms, so the substitution is
not just a relabeling of exponents. The finite-word support argument is what
justifies it. In particular, the nonexistence theorem requires far less
structure than the positive construction uses.
...
converge to $s$ in the full surreal order topology.

The ambient field matters. In the ordered subfield
$K=\R((\omega^{-1}))$, the same partial sums do converge to $s$ in its
\emph{intrinsic} order topology. Indeed, every $\eta\in K_{>0}$ has leading
term $c\omega^{-k}$ with $c>0$ and $k\in\Z$. For $N+1>k$, the positive
tail $s-s_N$ has leading exponent $-(N+1)<-k$, so $0<s-s_N<\eta$.
The surreal interval of radius $\omega^{-\omega}$ about $s$ intersects $K$
only in $\{s\}$. Thus the intrinsic order topology on $K$ differs from the
subspace topology inherited from $\No$. \Cref{thm:exact} uses Hahn summability,
whose support criterion specifies the sum without choosing either topology.
...
Its leading coefficient is unchanged, so it also preserves the canonical
field order, even when $G$ is not $2$-divisible.
...
More precisely, the extending embeddings correspond bijectively to those
roots, by the image of the residue class of $Y$.
\end{proposition}
\begin{proof}
...
an embedding. Since $L$ is generated over $\R(x)$ by the residue class of
$Y$, that root determines the extension uniquely, and different roots give
...
Theorem~\ref{thm:semiring} proves the obstruction directly in a semiring
source with no additive inverses and excludes all values $u\ge1$;
...
for the posed prescription; the nonnegative-coefficient semiring
obstruction; the image criterion for a single power-series variable; and the
