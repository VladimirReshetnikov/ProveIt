...
\vspace{0.3in}
{Merged research report built from thirteen manuscripts (03, 04, 06--11 of one batch,\par
12--16 of the next)\par}
\vspace{0.08in}
...
polynomial and rational maps. It is an AI-assisted merge of unrefereed manuscripts; nothing
in it is verified in Lean.
\end{abstract}
\end{minipage}
\vfill
\begin{minipage}{0.9\textwidth}\small
\textbf{Status.} Proof-bearing research report assembled from thirteen AI-assisted
...
rational maps (16; \cref{osq:fs:sub:gaps,osq:fs:sub:tests}). This report prints the common spine
once and keeps every result of every source.
...
while no set-sized sample tests all rational self-maps (16).
(\Cref{osq:pm:thm:numreal,osq:pm:thm:cp,osq:pm:cor:completion,osq:pm:cor:bijections,osq:fs:thm:imagegap,osq:fs:thm:surjective,osq:fs:thm:lower,osq:fs:thm:nosmall}.)
\end{maintheorem}
...
(\Cref{osq:hd:thm:boolean,osq:hd:cor:fd,osq:hd:thm:collapse,osq:hd:thm:fdlattice,osq:tn:thm:syzygy,osq:tn:thm:tensor,osq:tn:thm:rees,osq:tn:thm:hom,osq:tn:thm:pic,osq:tn:thm:duals}.)
\end{maintheorem}

\subsection{Sources, provenance and where the merge chose}\label{osq:sub:sources}

The report is built from thirteen manuscripts. Eight (03, 04, 06--11), dated 22 September 2026
...
programs and data are stored in this directory. None has a reviewer, and none is shipped with this
report.
...
only access to the repository through a GitHub connector on 23 September 2026.
...
(\cref{osq:hd:rem:reconcile}). Each follows from lemmas of the sources; none is a new method.
...
D-ring precedent that 16 itself credits (\cref{osq:fs:thm:collapse}). No source contains a false theorem.
...
\midrule
purely infinite ideal & $\mathcal I$ (03, 06, 09, 14), $\mathcal J_k$ (04), $\mathfrak I$ (07, 08, 11), $\mathfrak P$ (10), $J_k$ (13), $\mathcal I_K$ (15), $\mathcal P_k$ (16) & $\Pi$, $\Pi_k$, $\Pi_\C$\\
...
surcomplex field & $\No(i)$ (04, 07, 08, 09, 12, 15, 16) & $\SC=\No[i]$ \cite{RepoNotation}\\
...
report \cite{RepoOdg}, and 14 and 16 use them; they are background for the new results, not
...
reading of \cref{osq:sub:foundations}. Neither adds a hypothesis-free generalization. The proofs differ, and the
...
(\cref{osq:tn:sub:lattices,osq:tn:sub:tensor,osq:tn:sub:rees,osq:tn:sub:hom}). Labels carry the
sub-prefixes \texttt{osq:hd:} (12) and \texttt{osq:tn:} (14).
...
finite surreal rings); the saturated lattice $L$ of its matrix theorem is $W_D$.
...
\begin{theorem}[Generators and the first syzygy \src{14; 12} \merge]\label{osq:tn:thm:syzygy}
...
\begin{theorem}[Exact tensor normal form \src{14}]\label{osq:tn:thm:tensor}
...
\begin{corollary}[A family of every degree \src{14}]\label{osq:tn:cor:alldegrees}
...
\begin{theorem}[Scalar homomorphisms and isomorphism types \src{14}]\label{osq:tn:thm:hom}
...
\begin{theorem}[Common-scale constant matrices \src{14}]\label{osq:tn:thm:matrix}
...
The matrix theorem concerns constant matrices at one common monomial scale; several support levels need more analysis (14). Its companion,
...
(\cref{osq:fs:sub:gaps,osq:fs:sub:tests}). It uses from the
...
\begin{theorem}[The real numerical-polynomial ring \src{10; 16}]\label{osq:pm:thm:numreal}
...
\begin{theorem}[Gaussian numerical polynomials \src{10; 16}]\label{osq:pm:thm:numgaussian}
...
\begin{theorem}[Polynomiality of rational lattice self-maps \src{10; 16}]
...
\begin{corollary}[All rational bijections of the two lattices \src{10; 16}]
...
For $D=\Z$ and $\Z[i]$ this is \cref{osq:pm:thm:numreal,osq:pm:thm:numgaussian} (10); 16 proves it for $r=1$ and for several variables with
any $D$. The finite-difference identity needs no convergence argument: both sides agree at the grid points (16).

\begin{theorem}[Optimal point tests \src{16}; 10]\label{osq:fs:thm:lower}
...
$\binom{n+d}n$ points) and the optimality statement are 16's. So the binomial polynomials form a basis of the $\Oz$-module $\Num_1(\Oz)$, each
...
functions with finitely many coefficients and use the whole surreal exponent class.
...
only $A^{08}_\kappa$ with $\cf\kappa=\aleph_0$.
...
coordinate-cone cores $d\le\pd_{A_d}D\le d+1$ (\cref{osq:hd:q:cores}).
...
\end{question}
\emph{Status.} Open. 04's question on Diophantine fibers belongs to the sibling report \cite{RepoOdg}, and so does 13's
question on fibers of polynomials with nonlinear factors.
...
for the two merges on copies (Python~3.14.4, SymPy~1.14.0) and reproduce the recorded outputs, up to line
...
$d\le8$, $n\le12$; like the shipped checks it verifies finite cases only.
...
workspace and a fresh exponent). None was executed.
