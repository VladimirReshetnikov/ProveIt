...
\hypersetup{colorlinks=true,linkcolor=blue!45!black,citecolor=blue!45!black,
 ...
 relation modules, ordinary-output loci, normalization fibres}}
...
\vspace{0.3in}
{Merged research report built from sixteen manuscripts (03, 04, 06--11 of one batch,\par
12--16 of the next, 17--19 of a third)\par}
\vspace{0.08in}
...
integral closure of $\Oz$ modulo the ideal generated by $\Pi$ as a Boolean power of the algebraic integers. It is an AI-assisted
merge of unrefereed manuscripts; nothing in it is verified in Lean.
\end{abstract}
\end{minipage}
\vfill
\begin{minipage}{0.9\textwidth}\small
\textbf{Status.} Proof-bearing research report assembled from sixteen AI-assisted
...
\cref{osq:or:sub:outputs}), and the arithmetic fibre of the integral closure (19; \cref{osq:nf:sub:fibre}).
...
$\NN$ or $\NN_\C$ is $\overline\Z$ or $\Fpbar$ (19).
(\Cref{osq:nm:thm:complete,osq:nm:thm:normalization,osq:nm:thm:valuation,osq:nm:thm:invisible,osq:nf:thm:radical,osq:nf:thm:boolean,osq:nf:thm:doubling,osq:nf:thm:domainimages}.)
\end{maintheorem}
...
The report is built from sixteen manuscripts. Eight (03, 04, 06--11), dated 22 September 2026
...
groups and lattices \cite{RepoOpa,RepoOgl}. None has a reviewer, and none is shipped with this
report.
...
files, so 18 and 19 saw 06's text but not 03's normalization, 10's polynomials or the second batch.
...
by 17 and 19, is used only for 19's class ultrafilters, and the results depending on them say so
(\cref{osq:sub:foundations}).
...
(\cref{osq:nf:rem:candidates,osq:nf:rem:rootimages}). Each follows from lemmas of the sources; none is a new method.
...
quotient manuscript'' was 06's text, now this report. The shipped audit files keep these statements verbatim; the
...
optimality theorem; the Gaussian question itself stays open (\cref{osq:or:q:outputs}). No source contains a false theorem.
...
results built on it (\cref{osq:nf:thm:kernel,osq:nf:cor:fieldtargets}) need global choice. The collision step of the
...
\cref{osq:fs:sub:gaps} list the further renamings of 12, 14 and 16, and
\cref{osq:rel:sub:relations,osq:or:sub:outputs,osq:nf:sub:fibre} those of 17, 18 and 19.
...
\midrule
purely infinite ideal & $\mathcal I$ (03, 06, 09, 14), $\mathcal J_k$ (04), $\mathfrak I$ (07, 08, 11), $\mathfrak P$ (10), $J_k$ (13), $\mathcal I_K$ (15), $\mathcal P_k$ (16), $\mathfrak m$ (17), $\mathcal J$ (18, 19) & $\Pi$, $\Pi_k$, $\Pi_\C$\\
...
integral closure & $B,C,\Int_F(A)$ (03), $N_\R,N_\C,\Int_A(F)$ (19) & $\NN$, $\NN_\C$, $\IC_F(A)$\\
...
proving sources; neither adds generality. The proofs differ, and the
...
\begin{lemma}[Contraction \src{19}]\label{osq:nf:lem:contraction}
...
\begin{theorem}[Radicality \src{19}]\label{osq:nf:thm:radical}
...
\begin{theorem}[Rational saturation \src{19}]\label{osq:nf:thm:saturation}
...
\begin{lemma}[A coefficient copy of $\overline\Q$ \src{19}]\label{osq:nf:lem:coefficients}
...
\begin{lemma}[Spectral idempotents \src{19}]\label{osq:nf:lem:spectral}
...
\begin{definition}[Finite-partition Boolean powers \src{19}]\label{osq:nf:def:boolean}
...
\begin{theorem}[The fibre is a Boolean power of the algebraic integers \src{19}]\label{osq:nf:thm:boolean}
...
restriction is part of the theorem, and $\Idem(\mathsf B_\R)$ is not determined (19).
...
\begin{lemma}[Rational comparison \src{19}]\label{osq:nf:lem:rationalcomplex}
...
\begin{theorem}[Integral doubling \src{19}]\label{osq:nf:thm:doubling}
...
\begin{corollary}[Set-sized images factor through the fibre \src{19}]\label{osq:nf:cor:small}
...
\begin{lemma}[Class ultrafilters \src{19}]\label{osq:nf:lem:classultra}
...
\begin{theorem}[Detection by maps to $\overline\Z$ \src{19}]\label{osq:nf:thm:kernel}
...
argument, not a counterexample to any conclusion (19).
...
statement; and the fibre theorem of \cref{osq:nf:sub:fibre} leaves $\Idem(\NN/\Pi\NN)$ undetermined and gives no membership
test for $\NN$.
...
\paragraph{19.} (1) $\Idem(\mathsf B_\R)$ is not determined: its size, its atoms, whether it is trivial and whether it is a set. (2) There is no
membership criterion for $\NN$ by normal forms. (3) $\mathsf B_\bullet$ is neither one finite product nor an unrestricted product of copies of
...
$\mathsf B_\bullet$; not all set-sized images are claimed countable; $\mathsf B_\bullet$ is not asserted to be a set, and no universal object in a
category of set-sized rings is claimed. (8) The kernel detection uses an explicit foundation (class theory with global choice, no class
...
without being a counterexample. (10) Integrality, absolute integral closure, Boolean powers and the B\'ezout property of $\overline\Z$ are
...
(10) The comparison of Gaussian tests concerns the sizes of two explicit certificates, not the minimal size of a test, and the
candidates of \cref{osq:nf:rem:candidates} decide nothing about $\Idem(\mathsf B_\R)$.
...
$\NN/\Pi\NN\cong\overline\Z[\Idem]$ (\cref{osq:nf:cor:small,osq:nf:thm:boolean}); images with zero divisors are not classified, and the Boolean
algebra $\Idem(\NN/\Pi\NN)$ is undetermined (\cref{osq:nf:q:fibre}). The membership test, the fixed exponent subgroup, the descent, the
generation number and the valuative criterion stay open; 19 asks the same membership question, for supports in one finitely generated
exponent subgroup first, allowing integrality witnesses at larger scales. 04's question on Diophantine fibers belongs to the sibling report
...
\end{question}
\emph{Status.} Open. (a) is reduced to instances of the membership clause of \cref{osq:q:normalization} (\cref{osq:nf:rem:candidates}), and
\cref{osq:nf:thm:doubling} holds for every answer. 19's membership question is merged into \cref{osq:q:normalization}.
...
for the three merges on copies (Python~3.14.4, SymPy~1.14.0) and reproduce the recorded outputs, up to line
...
of the Gaussian certificates compared in \cref{osq:or:prop:gausstest} for $n\le3$, $d\le5$.
...
doubling, and keep the class representation theorem and its ultrafilter foundation separate. None was executed.
