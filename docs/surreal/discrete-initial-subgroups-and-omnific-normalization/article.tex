...
of the omnific integers. The construction is not ordinary multiplication
...
\section{The problem and the main results}\label{sec:intro}
...
separate initial-monoid problem.
...
\begin{maintheorem}[Omnific normalization]\label{thm:A}
...
\begin{maintheorem}[Sharp image classification]\label{thm:B}
...
\begin{maintheorem}[Removing the bottom cyclic layer]\label{thm:C}
For every group $A$ in \cref{thm:A}, the subgroup $\Z\varepsilon$ is
...
The main construction is in \cref{sec:surgery,sec:transport}; the proofs
of the three theorems are completed in
\cref{sec:normalization,sec:classification,sec:quotient}, respectively.
...
\section{Foundations and the imported initiality criterion}\label{sec:background}
...
\begin{fact}[Ehrlich--Kaplan, concrete form]\label{fact:EK}
...
\begin{fact}[Real coefficient groups]\label{fact:real}
...
No proof in the present paper replaces \cref{fact:EK} by the weaker
...
\cite[Section~6.2]{Kaplan}. It is an initial additive subgroup: apply
\cref{fact:EK} to $E=\No_{\ge0}$, coefficient group $\Z$ at zero and
...
violation of \eqref{eq:dyadic}. Every positive element of \Oz is at
least $1$.

\begin{lemma}[Isolating a normal-form term]\label{lem:isolate}
...
\section{Locating the least positive scale}\label{sec:bottom}

\begin{lemma}[A minimum in an initial tree]\label{lem:min-tree}
...
\begin{proposition}[Bottom coefficient structure]\label{prop:bottom}
...
\begin{equation}\label{eq:bottomdata}
...
\end{proposition}
\begin{proof}
By \cref{fact:EK}, $A$ is truncation closed. Write the leading term of
...
Thus $p$ is the minimum of $\Gamma(A)$. By \cref{lem:min-tree},
$p=-\alpha$ for a unique ordinal $\alpha$.

The real coefficient group $A_p$ has least positive element $r$.
By \cref{fact:real}, it is $2^{-n}\Z$ for a unique $n\in\N$, and
$r=2^{-n}$. All supports lie in $\Gamma(A)$, proving the last assertion.
\end{proof}

The shape of the least positive element is consistent with
\cite[Proposition~9 and its proof]{EK}; the argument above also records
the support and coefficient data needed later.

\begin{corollary}[Forced ancestors]\label{cor:forced}
Under \eqref{eq:bottomdata}, for every $\beta<\alpha$ one has
\[
...
Initiality gives the exponent, and \eqref{eq:dyadic} gives all dyadic
coefficients there.
\end{proof}
...
\section{Moving a minimum to zero in the sign tree}\label{sec:surgery}
...
\begin{definition}[Root relocation]\label{def:theta}
...
\begin{lemma}[The inverse is well-defined]\label{lem:inverse}
The maps \eqref{eq:theta} and \eqref{eq:psi} are inverse bijections
...
\begin{lemma}[Order preservation]\label{lem:thetaorder}
...
\begin{theorem}[Predecessor and right-ancestor transport]\label{thm:tree}
For every $x\in C_\alpha$ with $x>p$,
\begin{equation}\label{eq:predidentity}
 \Pred(\tht(x))=\tht\bigl[\Pred(x)\cup\{p\}\bigr].
\end{equation}
Consequently,
\begin{align}
 \Rs(\tht(x))&=\tht[\Rs(x)],\label{eq:rightidentity}\\
 \Ls(\tht(x))&=\tht[\Ls(x)\cup\{p\}].\label{eq:leftidentity}
\end{align}
...
The empty word is $\tht(p)$, proving \eqref{eq:predidentity} in this
case.
...
\eqref{eq:predidentity} without a missing limit case.

Filter \eqref{eq:predidentity} according to numerical comparison with
$\tht(x)$. By \cref{lem:thetaorder}, comparison of images is comparison
of preimages. Since $p<x$, the extra element $\tht(p)=0$ is a left
ancestor, never a right ancestor. This gives
\eqref{eq:rightidentity} and \eqref{eq:leftidentity}.
Formula \eqref{eq:lost} follows directly from the all-minus word $p$.
Finally, if $x,y>p$, membership of $\tht(x)$ in the predecessor set of
$\tht(y)$ is, by injectivity and \eqref{eq:predidentity}, exactly
membership of $x$ in $\Pred(y)$.
\end{proof}

\begin{remark}[A stronger identity that would be false]\label{rem:exception}
For $\alpha>0$ one must not assert \eqref{eq:rightidentity} at $x=p$.
Already $p=-1$ has right ancestor $0$, whose image is $1$, while
$\tht(p)=0$ has no ancestors at all. The missing relations at precisely
this one point account for the extra condition in \cref{thm:B}.
\end{remark}

\begin{corollary}[Initiality of the image]\label{cor:initialimage}
...
\eqref{eq:predidentity} lies in $\tht[\Gamma]$, because both
$\Pred(x)$ and $p$ lie in $\Gamma$.
\end{proof}

\begin{proposition}[The exact inverse obstruction]\label{prop:inversetree}
...
$x=\psiA(y)$, where $y>0$, \eqref{eq:predidentity} gives
\[
...
\begin{proposition}[An exponent-birthday bound]\label{prop:length}
...
\section{Strong transport of normal forms}\label{sec:transport}

\begin{definition}[Bottom-normalized transport]\label{def:normalization}
...
\begin{theorem}[Ambient transport]\label{thm:transport}
...
family,
\begin{equation}\label{eq:strongtransport}
 \Rect\left(\sum_{i\in I}x_i\right)
       =\sum_{i\in I}\Rect(x_i).
\end{equation}
For a nonzero $x$,
\begin{equation}\label{eq:leadingtransport}
...
preserve order. This proves \eqref{eq:leadingtransport} as well.
...
When it holds, coefficient extraction reduces \eqref{eq:strongtransport}
...
\begin{remark}[No implicit completion of a subgroup]
The ambient Hahn spaces in \cref{thm:transport} are full spaces. A
...
\eqref{eq:strongtransport} still holds in the ambient spaces, but is not
...
\emph{exponent} groups. Consequently \eqref{eq:leadingtransport} should
not be described as preservation of a multiplicative valuation with the
same value-group structure.
\end{remark}

\section{Proof of omnific normalization}\label{sec:normalization}

We now prove \cref{thm:A}. Let $A$ be as there, and obtain
$\alpha,n,p,c$ from \cref{prop:bottom}. Write
\[
...
both inclusions in the second identity of \eqref{eq:coefftransport}.
At zero the coefficient group is $c^{-1}A_p=\Z$.
\end{proof}

\begin{proof}[Proof of \cref{thm:A}]
By \cref{cor:initialimage}, $\Delta$ is initial with minimum zero.
By \cref{lem:coefficients}, the image is truncation closed and
...
with $\gamma>p$. By \eqref{eq:rightidentity}, there exists
...
in \cref{fact:EK} proves that $H$ is initial in $\No$.

All exponents of $H$ are nonnegative and its zero coefficient is an
integer. Hence $H\subseteq\Oz$ by \eqref{eq:Oz}. Its minimum positive
...
claims are the restrictions of \cref{thm:transport}. Uniqueness of
$\alpha,n$ was established in \cref{prop:bottom}.
\end{proof}

\begin{corollary}[The cited existence question]\label{cor:answer}
Every discrete initial subgroup of $\No$ is isomorphic, as an ordered
abelian group, to an initial subgroup of $\Oz$.
\end{corollary}

\begin{remark}[An initial image is not a simplicity isomorphism]
...
\section{Exact images, extremal groups, and admissible scales}\label{sec:classification}

\subsection{The compulsory dyadic spine}
\begin{lemma}[The spine group is initial]\label{lem:K}
For every ordinal $\alpha$, the group $K_\alpha$ of
\eqref{eq:Kintro} is initial in $\No$, has least positive element $1$,
...
required dyadic containment holds. Apply \cref{fact:EK}. The remaining
claims follow from its normal forms.
\end{proof}

\begin{proposition}[Necessary image data]\label{prop:Knecessary}
If $A$ has least positive element $2^{-n}\omega^{-\alpha}$ and is
initial, then $K_\alpha\subseteq\Rect[A]$.
\end{proposition}
\begin{proof}
The image contains $1$. For each $\beta<\alpha$, \cref{cor:forced}
...
$\Gamma=\psiA[\Delta]$ is initial by \cref{prop:inversetree}.
Its minimum is $p$.
...
covered by \eqref{eq:rightidentity}.

The criterion \cref{fact:EK} now proves $A$ initial. Its least positive
...
\begin{align}
 L_{\alpha,n}
   &=c\Z\omega^p\ \oplus\!
     \bigoplus_{\beta<\alpha}\D\omega^{-\beta},\label{eq:L}\\
 M_{\alpha,n}
   &=\{x\in\Hahn{\R}{C_\alpha}:[\omega^p]x\in c\Z\}.
   \label{eq:M}
\end{align}
Here $[\omega^p]x$ denotes coefficient extraction. The direct sum in
\eqref{eq:L} again permits only finite supports; the class in
\eqref{eq:M} permits every set reverse-well-ordered support in the cone.

\begin{theorem}[Extremal groups]\label{thm:extremal}
...
\]
Moreover,
\begin{equation}\label{eq:extremalimages}
...
\end{theorem}
\begin{proof}
The first two identities in \eqref{eq:extremalimages} follow directly
...
By \cref{thm:B}, the inverse images of $K_\alpha$ and $\Oz$ are
initial and have the required minimum. The lower bound on $A$ follows
from \cref{cor:forced} and $A_p=c\Z$; the upper bound follows from
\cref{prop:bottom}.
...
\begin{corollary}[Sharp size obstruction]\label{cor:size}
...
Its initiality and minimum follow from \cref{thm:extremal}.
\end{proof}
...
\begin{definition}[Dyadic-spine depth]
...
\begin{corollary}[Admissible scales]\label{cor:depth}
...
\end{corollary}
\begin{proof}
By \cref{thm:B}, the condition is exactly that all
...
Thus the endpoint in \cref{cor:depth} can be a limit ordinal and must
...
unbounded depth. It is initial by \cref{fact:EK}, but is much smaller
than $\Oz$, for example it does not contain $\omega^2$.

\section{The quotient by the least positive cyclic subgroup}\label{sec:quotient}
...
\begin{lemma}[Bottom-layer decomposition]\label{lem:split}
...
\begin{lemma}[Positive-root deletion]\label{lem:rho}
...
\begin{equation}\label{eq:Bquotient}
 B=\left\{\sum_{\delta>0}s_\delta\omega^{\rho(\delta)}:
              \sum_{\delta>0}s_\delta\omega^\delta\in H^+\right\}.
\end{equation}

\begin{proposition}[An initial model of the quotient]\label{prop:quotient}
The class $B$ in \eqref{eq:Bquotient} is an initial additive subgroup of
...
\end{corollary}
\begin{proof}
Apply \cref{thm:C} at the first stage and then apply it to the initial
...
\begin{theorem}[Integer-bottom suspension]\label{thm:suspension}
...
\begin{corollary}[A relative criterion for discrete ordered groups]
...
\cref{thm:C} and transport its splitting and quotient realization back.
...
\section{Surcomplex and linear-algebraic consequences}\label{sec:complex}
...
\begin{definition}[Rectangular initiality]
...
\begin{theorem}[Surcomplex additive normalization]\label{thm:complex}
...
\cref{thm:transport} is unchanged; there is no additional infinite sum
...
range assertion is exactly \cref{thm:B} applied to the real factor.
\end{proof}

\subsection{What linear systems can be transported}
\begin{corollary}[Exact transport of integral linear equations]
...
\section{Examples and failure modes}\label{sec:examples}
...
are initial. Hence $A$ is initial by \cref{fact:EK}. Its least positive
element is $1/2$.
...
\begin{equation}\label{eq:limitexample}
...
the group in \eqref{eq:limitexample}. Its image $K_\omega$ consists
only of finite sums of the positive terms appearing in
\eqref{eq:limitimage}, together with an integer constant. The full
...
Here $K_1\not\subseteq H$, so \cref{thm:B} correctly rules out
initiality.
...
constant through \eqref{eq:dyadic}, but its constant group is only
$\Z$. Equivalently, $1/2$ is a missing prefix of $\omega^{-1}$.
Thus the exponent condition in \cref{prop:inversetree} alone does not
...
\section{Verification, provenance, and a formalization route}\label{sec:audit}
...
The accompanying Python program \texttt{verify.py} uses only the standard
library, exact rational arithmetic, and a fixed random seed. Its recorded
run performs \textbf{450,862 assertions}, all passing. The tests cover
...
\item \emph{Cone root relocation.} Define the three-way sign split,
      ...
      \eqref{eq:predidentity} for ordinal-length sequences.
...
\item \emph{Concrete initial-subgroup criterion.} Supply a checked
      version of \cref{fact:EK}, including both its coefficient and
...
\section{Further questions and exact boundaries}\label{sec:further}
...
\begin{question}[Other convex quotients]
...
\begin{question}[Limit iteration]
...
\end{question}
Finite iteration follows from \cref{cor:iteration}; limit iteration
...
\begin{question}[Multiplicative compatibility on restricted classes]
...
\begin{question}[Formal certification and uniqueness principles]
...
\cref{thm:B} concerns the inverse under the fixed map, not uniqueness of
...
\appendix
\section{A compact proof for independent checking}\label{app:compact}
...
\section{Proof-review checklist}\label{app:checklist}
...
Surcomplex scope &
\Cref{thm:complex}: the conclusion is Gaussian-linear and coordinatewise
...
of the review are recorded in \cref{sec:audit} and the accompanying
