...
% Added on placement: statements share the theorem counter, and without these
...
exact, uniform gain in relative valuation at entry $e$: an input change
...
This is an AI-assisted research draft, and no statement of it has a Lean
formalization. It is built from a single
...
\eqref{scale:eq:overviewbound} for a nonbridge $e$. Therefore no larger
uniform gain can replace $\kappa_e$. This conclusion holds even when
...
AI-assisted draft; it has not been refereed; no statement of it has a Lean
formalization, and \cref{scale:sec:verification} does not assert that the
...
topology is claimed. Strong sums are coefficientwise sums, not limits of
partial sums (\cref{scale:sec:boundaries}), and the remainder certificate
...
\item \emph{What ``sharp'' means.} $\kappa_e$ is the largest gain valid
uniformly; equality is not asserted for every input direction
...
depends on this one. The comparisons below are with the articles as committed
at \texttt{52c7ab6}; theorem numbers of other reports are those of their
committed PDFs.
...
\cref{scale:sec:boundaries} make the same point with the same elements
$(0,1)$ and $(1,0)$ of $\Z^2_{\mathrm{lex}}$: strong sums are not limits of
partial sums. Each report keeps its own proofs.
...
(\cref{scale:sec:comparison}). This report does not use the word
\emph{spectrum}.
...
interactions with a positive-cycle hypothesis, and its path minima bound node
errors. It involves no spanning trees and no normalization, and its moment
...
placed at commit \texttt{52c7ab6}. Its own repository comparison is pinned to
\texttt{\pin} (\cref{scale:sec:comparison}).
...
\begin{enumerate}[label=(\alph*)]
\item The source was renamed \texttt{article.tex}. The two audit files, the
...
\texttt{scale:} and are unchanged. The labels added on placement carry the
same prefix.
\item \Cref{scale:sec:nonclaims,scale:sec:neighbours,scale:sec:words,scale:sec:provenance}
were added, and so were \cref{scale:rem:support,scale:rem:trees}, which
...
records a rerun of the checks on placement.
...
\end{enumerate}
Placement left the mathematics unchanged.
...
its Theorem~3.1 gives a general scaling criterion, and its Section~4.3
records continuity results. The present article does not reclaim
existence, uniqueness, or ordinary real continuity.

Sharify, Gaubert and Grigori \cite{SGG}, Theorem~2.6, prove that the
entries obtained by scaling the Hadamard deformation $A^{(p)}$ have
convergent generalized Dirichlet expansions in $t=\exp(-p)$.
Their analysis also gives convergence to the optimal-assignment face.
...
The manuscript was placed in the collection at commit \texttt{52c7ab6},
eight commits after its pin. The statements above remain true of the pin
and are kept as provenance; the following has changed or was checked on
placement. The catalogue \texttt{docs/README.md} did not change between the
two commits. It still lists 36 reports, and it does not yet list this
report or the three other reports placed by the same commit. The directory
...
them; commit \texttt{52c7ab6} unpacked, placed and removed all nine, so the
directory again holds only its README. The positive-control file
\nolinkurl{Surreal/Algebra/MarkovResolvent.lean} is still present. It proves
...
At \texttt{52c7ab6}, a full-text search of every LaTeX source in the
...
sentence does not concern the prescribed-margin problem studied here. No
Lean source mentions Sinkhorn scaling, spanning trees, doubly stochastic
...
well-ordering of the union of supports. Expanding finite polynomial
expressions for the coefficients and applying the same argument to
products and compositions licenses the corresponding rearrangements.
No bound on the polynomial degrees of the individual $b_\nu$ is
required, since there are only finitely many relevant $\nu$ at a
fixed exponent.
\end{proof}
...
it is $\pm1$. These facts follow, for example, by deleting a leaf
and inducting, while a disconnected choice has deficient rank.
...
$L_p=BW_pB^{\mathsf T}$. Finally
$v(w_T/Z)\geq0$ by the determinant formula and no leading
...
$B_T^{\mathsf T}$ has integral entries, so the root-normalized
...
inverse $h\mapsto H_ph$. Its tangent space has dimension
$m-N+1$ over $K$. This is a support-controlled analytic chart in
...
The quantity $\kappa_e$ is the largest gain valid uniformly over all
infinitesimal changes and all centers in this relative polydisc.
...
The argument for the secant identity, now in the formal power-series
ring, gives
\[
 \mathcal H_p(X)_e=
 \sum_{T:e\notin T}
...
The first bound in \eqref{scale:eq:coefficientbound} follows.
...
For a center $g$, uniqueness gives the exact recentering identity
...
If $f\in\mm^E$ is nonzero, $\delta=\min_e v(f_e)$, and
...
exists. These are standard finite minimum-spanning-tree operations;
...
$F$ on the same connected support, with strictly positive feasible
fixed margins. Let $P,P'$ be their positive normalizations.
...
gives an $\eta\in1+\mm_F$ as in the cut bound. Hence
$P'_e/P_e\in1+\mm_F$. The vertex numbers used in that proof may
be divided by a root value; their largest-to-smallest ratio is in
$1+\mm_F$, so all root-normalized vertex numbers lie in
$1+\mm_F$. Their restricted logarithms give an infinitesimal
potential $u$ with
\[
...
\end{align*}
The nonlinear two-point inequality, one-coordinate equality, and
all Taylor-coefficient bounds of \cref{scale:thm:sharp,scale:thm:coeffgain}
...
interpolation coefficients can carry additional valuations.
...
For global positive scaling, include a positive feasible witness and
pass to the divisible hull of the input support group. The resulting
...
\begin{equation}\label{scale:eq:phasequadratic}
 ...
 +\tfrac13(\exp s-1)=0.
\end{equation}
Its discriminant has leading term $-(16/9)t^{2\delta}$.
...
run on a copy of this directory. When the report joined the collection, such
a run under Python~3.14.4 and SymPy~1.14.0 passed in about twenty seconds and
reproduced every count above. The file it wrote differed from the recorded
one only in the Python version field and in line endings.
...
gap is the exact uniform nonlinear gain in each entry, with matching
Taylor-coefficient bounds and one-coordinate leading amplitudes.
...
for matrices and positive maps}, arXiv:1609.06349 (2016).
...
\url{https://arxiv.org/abs/1104.3830}.
...
compared on placement at commit \texttt{52c7ab6}.
\repo.
...
was compared on placement at commit \texttt{52c7ab6}.
\repo.
\end{thebibliography}
\end{document}
