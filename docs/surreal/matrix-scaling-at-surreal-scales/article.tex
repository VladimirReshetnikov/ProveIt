...
\[
 F(X)=\sum_{n\geq1} t^{\beta-n\delta}X^n
\]
...
latexmk -pdf -halt-on-error -interaction=nonstopmode \
  surreal_matrix_scaling.tex
\end{verbatim}
The bibliography is internal; no external database or figure file
is required. The provided \texttt{build.sh} changes to its own
package directory before running these commands, so it also works
when invoked from elsewhere. Compilation verifies typesetting,
...
this one contribution. The package contains no copies of the
repository's existing reports. No change has been made to the
repository itself.
...
Detailed README inspected at commit \texttt{\shortpin}.
\repo.
...
README lines 1--180 inspected at commit \texttt{\shortpin}.
\repo.
\end{thebibliography}
\end{document}
