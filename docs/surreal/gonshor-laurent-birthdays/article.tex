...
nondyadic boundary condition. A positive-power polynomial/bounded-tail decomposition
...
many nonzero terms. The sums are defined by normal forms; no topological limit
is used. Integer exponents are bounded above, so every interval
...
Throughout, $\bd(x)$ is the length of the canonical sign sequence of the
\emph{value} $x$. It is not the recursively computed rank of an arbitrary
cut expression representing that value. For example, the form
$\{-1\mid1\}$, with canonical options, has construction rank $2$ but
represents $0$, whose birthday is $0$. The bounds below concern canonical
birthdays after evaluation, including any cancellations in a product.
...
represent the options canonically and apply the simplicity rule to their cut.
This is an upper bound, not an equality for every displayed cut form.
...
\cite[Definition~2.20 and Theorem~2.21, p.~13]{BG2022}. Here is the version needed for our
calculations.

If $a$ has a sign sequence and $p_\delta$ denotes the ordinal order type of the plus signs
...
already occurs in an earlier exponent. These comparisons use the original
exponent sequences, before any deletions. There is a second rule at successor
term indices: if the current exponent starts with the immediately preceding
exponent followed by a minus,
...
$\omega^{n+1}$. Here the coefficient at $n$ is the collected coefficient
of $z$; if it vanishes after cancellation, the dyadic case applies.
\end{proof}
...
At the highest possible exponent $N_x+N_y$, only the pair $(N_x,N_y)$
contributes. Its coefficient is $c_{N_x}d_{N_y}\ne0$, so the highest exponent
of $xy$ is exactly $N_x+N_y$. If both factors have finite support, with
lowest exponents $m_x,m_y$, the same argument at the other end gives lowest
exponent $m_x+m_y$ and nonzero coefficient $c_{m_x}d_{m_y}$. Thus interior
coefficients may cancel, but the extreme coefficients cannot. In particular,
widths of nonzero finite Laurent polynomials add under multiplication.
...
\]
where $M_j$ equals $m_j$ or $m_j+1$, and $\tau_j$ is finite. In particular,
\[
 H+\omega\cdot m_j\ \leq\ \bd(x_j)
       \ <\ H+\omega\cdot(m_j+2).
\]
Since $m_j$ is unbounded in $\N$, these lengths are cofinal in
$H+\omega^2$, by continuity of ordinary ordinal addition in its right
argument. The sign sequences are nested prefixes by
...
\end{equation}
For instance, $T=\min\{K-N_y,K-N_x\}$ works.
Then
...
in the argument. Coefficient equality includes zeros caused by cancellation;
the conclusion is about the collected normal forms, not only about possible
support exponents.
\end{proof}
...
nonzero monomials. Indeed, define the width as highest exponent minus lowest
exponent. The extreme-coefficient calculation in Section~\ref{sec:infinite}
shows that widths add under multiplication. A product equal to $1$ has width
zero, forcing both factors to have width zero; conversely, a nonzero monomial
has a monomial inverse.
...
$H+\omega^2$. For example,
\[
 f=\omega^{-2}-\omega^{-3},\qquad
 f^{-1}=\sum_{k\leq2}\omega^k,\qquad
 \bd(f^{-1})=(\omega^2+\omega)+\omega^2=\omega^2\cdot2.
\]
...
In particular, Definition~2.20 and Theorem~2.21, p.~13.
