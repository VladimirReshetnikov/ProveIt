...
The report \path{docs/surreal/finite-surreal-probability/} builds
finite-horizon path weights from a stochastic matrix over an ordered field
(\texttt{fsp:sub:paths}); applied to $R_L(s)$ its path shadow is the chain with
transition matrix $K_\alpha(c)$. Neither report constructs an infinite-path law.
