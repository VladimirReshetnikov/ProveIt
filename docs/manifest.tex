...
AI-assisted drafts.  None is refereed or machine-checked, and this catalogue
...
because no commutative argument from \dir{docs/surcomplex} survives the move to
...
a formal radial change $r=s^qw(s)$ with $w(0)\ne0$ the pole order becomes
exactly $6q$ with leading coefficient $48M^2$ \emph{unchanged}, so no radial
...
exact comparisons, 18 named unit tests, 266 exact checks, and 4{,}321 exact
cross-model checks new to the merge, comparing dense common-grid Puiseux
convolution against sparse rational convolution, candidate covers with extra
zeros and nonnested lists, the finite-jet root transformation, derivative and
