...
recurrences and finite sums, and the precise asymptotic
$a_n\sim 0.4139310680\ldots\,(3.5464554447\ldots)^n n^{-3/2}$.
The proof is self-contained, and the accompanying exact-arithmetic programs
check the literal mesh definition independently of the generating function.
\end{abstract}
...
thread~\cite{seqfan}. They also remove any dependence of our proof or
...
empty permutation at $n=0$. Since $\Rtwo=\Rone^{\mathsf T}$, inversion is
a bijection to the class avoiding $(12,\Rtwo)$. Thus both versions in the
OEIS definition have the same counting sequence.
...
The entry retrieved for this note still labels \eqref{eq:main}
...
$a_3=3$. The permutation $1324$ has no upper succession, but its two
...
$Y=x(1+Y)(1+uY)$. Formal Lagrange inversion gives
\[
...
series in $x$. No analytic convergence assertion is involved.
...
unique singleton block when one exists.
...
\begin{equation}
 ...
 \label{eq:Dfactor}
\end{equation}
The first factor has two positive reciprocal roots, and the second has
two nonreal roots on the unit circle. Hence the smallest-modulus zero
is the positive number
\begin{equation}
 \rho=\frac{1+2\sqrt2-\sqrt{5+4\sqrt2}}2
       =0.2819716800611948531\ldots.
 \label{eq:rho}
\end{equation}
...
\begin{align}
 \gamma&=\rho^{-1}
       =\frac{1+2\sqrt2+\sqrt{5+4\sqrt2}}2
        =3.5464554446849952445\ldots,\label{eq:gamma}\\
 \delta&=\sqrt{-\rho D'(\rho)},\qquad
 K=\frac{(\rho^2+5\rho+3)\delta}
          {16\sqrt\pi\,\rho(1+\rho)^2}
       =0.4139310680369706448\ldots.
 \label{eq:K}
\end{align}
Then
\begin{equation}
...
singularity transfer, obtained by deforming a Cauchy coefficient contour
...
\begin{table}[ht]
\centering
\caption{Exact coefficients divided by the leading asymptotic.}
\begin{tabular}{r l}
\toprule
$n$ & $a_n/(K\gamma^n n^{-3/2})$\\
\midrule
20 & $0.951983446713366\ldots$\\
50 & $0.980230192459601\ldots$\\
100 & $0.990017308750539\ldots$\\
200 & $0.994983824600422\ldots$\\
\bottomrule
\end{tabular}
\end{table}
...
$n$. The probability that it has exactly one fixed point tends to
\begin{equation}
 \frac{2\rho H(\rho)}{1+2\rho H(\rho)}
    =0.4288852556670782555\ldots,
 \qquad
...
gives \eqref{eq:fixedprob}.
\end{proof}

\section{Reproducibility and proof audit}
\label{sec:checks}

The archive contains a standard-library Python verifier,
\texttt{code/verify.py}, and an optional SymPy/mpmath script,
\texttt{code/symbolic\_checks.py}. Neither program downloads data or
uses numerical guessing to establish the formula.
...
deleting its maximum. Catalan totals are checked at each length.
For mesh membership, the program tests the nine-rectangle definition
...
that the independent test should be capable of falsifying.

\begin{table}[ht]
\centering
\caption{Executed checks included in the reproducibility logs.}
\begin{tabular}{p{0.69\textwidth}r}
\toprule
Check & Range\\
\midrule
Literal mesh test, both $R_{174}$ and $R_{234}$, against the structural
...
permutations and counts $115049$. The symbolic script verifies seven
algebraic residuals identically equal to zero, including
\eqref{eq:AfromH}, \eqref{eq:Apoly}, and \eqref{eq:Dfactor}. The coefficient
file extends the sequence through $n=200$ and is explicitly identified
as computed from this proof, not as an official OEIS file.
...
pdflatex -interaction=nonstopmode article.tex
\end{verbatim}
The first program writes coefficient tables and a machine-readable
verification report to \texttt{data/}. The second writes exact symbolic
check results and high-precision numerical approximations. This note
provides a conventional mathematical proof; it is not a proof-assistant
formalization, and its numerical approximations are not presented as
...
formulas, and a precise asymptotic expansion.
