...
def main() -> None:
    ...
    # A second, scale-free form of the same certificate: the quotient of the
    # resultant by the displayed quartic is a nonzero constant, here exactly 1.
    ratio=sp.cancel(resultant/equation)
    require(not ratio.free_symbols and ratio!=0,
            "resultant is not a nonzero constant multiple of the quartic")
    # The grouped form printed in the article is the same polynomial.
    grouped=((256*z*z+107*z-32)*(h**4-h**3)
             +(96*z*z+36*z)*h*h-(16*z*z+4*z)*h+z*z)
    require(sp.expand(grouped-equation)==0,"grouped quartic identity")
    # The three parity-weight identities consumed by the quadratic lemma.
    require(sp.expand((alpha+beta)*(alpha-beta))==alpha**2-beta**2,
            "odd parity weight")
    require(sp.expand((alpha+beta)**2)==alpha**2+2*alpha*beta+beta**2,
            "even parity weight, plus")
    require(sp.expand((alpha-beta)**2)==alpha**2-2*alpha*beta+beta**2,
            "even parity weight, minus")
    ...
    lines=["Symbolic checks: PASS",f"SymPy version: {sp.__version__}",
           ...
           "quartic power series through degree 20, the grouped quartic form,",
           "the three parity-weight identities, saddle point, growth constant,",
           "Gaussian/Bell formulas for c1 and c2, and the leading amplitude.","",
           f"resultant divided by the displayed quartic = {ratio}",
