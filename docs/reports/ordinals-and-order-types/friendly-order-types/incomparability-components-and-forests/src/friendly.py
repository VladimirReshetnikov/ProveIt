#!/usr/bin/env python3
"""Exact friendly order type and certified optimal sequences for finite posets.
...
def naturally_labelled_posets(n: int) -> Iterator[tuple[int, ...]]:
    ...
    if n == 0:
        ...
        for lower in naturally_labelled_posets(n - 1):
...
def main() -> None:
    ...
    parser.add_argument("input", type=Path, help='JSON object {"n":..., "relations":[[a,b],...]}')
    parser.add_argument("--dp", action="store_true", help="Also run exponential direct recursion")
    ...
    try:
        ...
        output = {"n": poset.n,
                  "incomparability_components": [list(bits(c)) for c in poset.ordered_components()],
