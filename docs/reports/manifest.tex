...
\hypersetup{%
  ...
  pdfsubject={Catalogue of 105 independent mathematical research packages}}
...
\begin{center}
  ...
  {\large One hundred and five independent mathematical research packages,\\
...
This document catalogues the one hundred and five research packages stored
under \dir{docs/reports}.  They arrived as one hundred and twenty-five ZIP
...
One hundred and twenty-five archives yield one hundred and five reports because
twenty of them duplicated another: sixteen pairs, three three-way clusters and
one four-way cluster.  The seventh delivery of eighteen contained no duplicate
at all, which is worth recording: a sweep that finds nothing is evidence about
the collection, not a sweep that failed.
...
merged report on surreal option graphs keeps two different five-vertex
counterexamples for the same reason, and the merged shuffle report keeps three
independent finite certifications of one theorem.  A merged entry below names both of its
...
\midrule
\dir{ordinals-and-order-types}    & \dir{friendly-order-types}          & 3\\
                                  & \dir{wqo-powersets-and-statures}    & 4\\
                                  & \dir{transfinite-words}             & 2\\
                                  & \dir{ordinal-arithmetic}            & 4\\
                                  & \dir{games-on-ordinals}             & 3\\
                                  & (top level)                         & 4\\
\midrule
\dir{surreal-numbers}             & ---                                 & 6\\
\midrule
\dir{hankel-determinants}         & \dir{somos-and-elliptic}            & 3\\
                                  & \dir{catalan-and-ballot}            & 4\\
                                  & \dir{growth-and-runs}               & 2\\
                                  & (top level)                         & 1\\
\midrule
\dir{congruences-and-valuations}  & \dir{supercongruences}              & 2\\
...
\dir{graph-theory}                      & ---                           & 3\\
\midrule
\dir{enumerative-combinatorics}         & ---                           & 19\\
\midrule
\dir{generating-functions-and-asymptotics}
                                  & \dir{binary-product-coefficient-counts} & 3\\
                                  & \dir{oeis-sequence-asymptotics}     & 9\\
                                  & (top level)                         & 4\\
\midrule
\multicolumn{2}{l}{\textbf{Total}} & \textbf{105}\\
\bottomrule
\end{tabular}
\end{center}
...
\section{OEIS sequence asymptotics and formulas}
...
\entry{Cubic-order growth of the derivatives of $x^{x^x}$}
{generating-functions-and-asymptotics/oeis-sequence-asymptotics/a281434-cubic-growth}
{article.pdf}{A281434\_exact\_algorithm\_and\_cubic\_growth.zip}
{For OEIS A281434, the number of nonzero monomials in the fully collected $n$th
derivative of $x^{x^x}$, gives a canonical three-variable differential-polynomial
representation with a seven-transition integer recurrence, two deterministic
exact algorithms, and the two-sided bound placing $a(n)$ at order $n^3$, between
$(7n^3+39n^2+26n+3\varepsilon)/24$ and $(2n^3+3n^2+4n)/3$.  The OEIS constant is
not pinned down.  One of three entries counting derivative terms of a
power-tower function; the functions, recurrences and answers all differ.}

\entry{The derivative-term conjecture for $x^{x^2}$}
{generating-functions-and-asymptotics/oeis-sequence-asymptotics/a290268-derivative-terms}
{A290268\_partial\_results.pdf}{A290268\_partial\_results.zip}
{\textbf{The OEIS A290268 conjecture is not proved here, and the report says so
first.}  What is proved unconditionally: the conjectured
$U(n)=n^2/2+n+1-\ldots$ is an upper bound for the number of collected terms in
the $n$th derivative of $x^{x^2}$; every predicted cancellation is explained;
positivity holds on a large coefficient region; and there are no further zeros
on the first two logarithmic-deficit diagonals.  The precise remaining
nonvanishing statement is isolated.}

\entry{Counting terms in the derivatives of $x^x$}
{generating-functions-and-asymptotics/oeis-sequence-asymptotics/a293239-derivative-terms}
{article.pdf}{A293239\_research\_report.zip}
{\textbf{The principal all-order closed formula for OEIS A293239 is not proved
here}; its missing nonvanishing statement is isolated.  What the report does
settle: the term count reduces exactly to zeros of the first-kind
Lehmer--Comtet triangle A008296, giving $a(n)=1+n(n+1)/2-Z(n)$; and the
recurrence printed in the entry with range $n>6$ is \emph{disproved} at $n=8$,
its correct eventual range for the proposed sequence being $n\ge14$ ---
conditional, for the actual derivative count, on the principal conjecture.}
...
Six reports on the surreal field itself: birthdays and their behaviour under
multiplication, the option graphs of surreal forms, genetic functions and gaps,
and two questions of Lipparini about evaluating formal sums in the surreals.
The first four arrived as two pairs; each pair was merged.

\entry{A square-root obstruction to evaluating increasing Hahn series at $\omega$}
{surreal-numbers/hahn-evaluation-at-omega}
...
{surreal-numbers/gonshor-product-birthdays}
...
{surreal-numbers/gonshor-laurent-birthdays}
...
{surreal-numbers/canonical-forms-need-not-be-subgraphs}
...
{surreal-numbers/genetic-gaps-and-primitives}
...
{surreal-numbers/broadcast-sum-of-surreal-sequences}
