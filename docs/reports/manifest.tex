...
\hypersetup{%
  ...
  pdfsubject={Catalogue of 58 independent mathematical research packages}}
...
\begin{center}
  ...
  {\large Fifty-eight independent mathematical research packages,\\
...
\dir{docs/reports}.  They arrived as sixty-four ZIP archives, which have been
unpacked and grouped into the subject hierarchy described in
Section~\ref{sec:tree}.  The archives themselves are no longer kept in the
repository.

Sixty-four archives yield fifty-eight reports because six pairs turned out to
prove the same theorem --- in five of the six, by the same argument, down to
shared lemmas and in one case shared worked counterexamples.  Each pair has been
merged into one report that proves the shared theorem once and keeps everything
both originals built on top of it.  Where the two reached a result by genuinely
different routes, both proofs are kept and marked as alternatives, with a
sentence on what each one buys.  A merged entry below names both of its source
archives and says what was doubled.
