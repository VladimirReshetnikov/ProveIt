...
\hypersetup{%
  ...
  pdfsubject={Catalogue of 71 independent mathematical research packages}}
...
\begin{center}
  ...
  {\large Seventy-one independent mathematical research packages,\\
...
This document catalogues the seventy-one research packages stored under
\dir{docs/reports}.  They arrived as eighty ZIP archives, in three deliveries,
which have been unpacked and grouped into the subject hierarchy described in
Section~\ref{sec:tree}.  The archives themselves are no longer kept in the
repository.

Eighty archives yield seventy-one reports because nine pairs turned out to prove
the same theorem.  Each pair has been merged into one report that proves the
...
counterexamples for the same reason.  A merged entry below names both of its
