...
The interval is nonempty in that case. In all other cases put
...
\(\delta\), so both bounding suprema are \(\omega\cdot\delta\).
...
\(a\ns b\leq\alpha\ns\beta\), so
\(\sup S(xy)\leq\alpha\ns\beta\). Now apply
\eqref{eq:support-sandwich}. Coefficient cancellation only removes
...
To see this, compare the aligned Cantor coefficients. The target differs
from \(\gamma\) only by subtracting one from the constant coefficient.
A first decrease at a positive Cantor exponent in either input cannot
be offset by the other input, which is also bounded by its corresponding
maximum. Thus all positive-exponent coefficients must be unchanged, and
exactly one constant coefficient decreases by one. The coefficient at
...
the birthday is requested. For infinite supports with a last limit index,
...
Coefficientwise multiplication is therefore legitimate, and the finite
geometric cancellation at each coefficient shows
\((1+v)\sum_{n\geq0}(-v)^n=1\). Thus the series is an inverse in
