...
Actual sign-field standard part is now constructed using Mathlib’s
`ArchimedeanClass.stdPart` APIs, and the binary coprime factor linearization
is a native Sylvester-map equivalence. The next foundational steps are
monomial scaling of a depressed polynomial to finite coefficients and
coprime factor lifting with strongly summable evaluation inside the sign
field. General finite-family linearization and the infinite lifting remain
