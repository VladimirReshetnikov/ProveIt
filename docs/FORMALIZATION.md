...
| Existence, uniqueness and support clauses of `polynomial:cor:simpleroot` | `Surreal.HahnSeries.exists_supported_root_of_simple_standardPart`, `existsUnique_root_of_simple_standardPart`, `existsUnique_infinitesimal_root_correction`, `existsUnique_supported_root_correction` in [PolynomialSimpleRootLifting.lean](../Surreal/HahnSeries/PolynomialSimpleRootLifting.lean) | Every simple root of the ordinary residue polynomial of a monic Hahn polynomial has a unique finite root with that residue. The equivalent correction has positive order and support in the original error monoid minus zero. Uniqueness compares every positive-order correction, not only candidates satisfying the support bound. The complementary factor has nonzero residue at the selected root; the lifted linear factor determines the root. Neither algebraic closedness nor divisibility of the exponent group is required. **Proved** for these clauses; the explicit first-coefficient formula `polynomial:eq:firstrootcorrection` and actual surreal transfer remain pending. Build and axiom audit pass. |
