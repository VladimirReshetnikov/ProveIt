...
The next foundational steps are actual sign-field standard part (using
Mathlib's `ArchimedeanClass.stdPart` APIs), monomial scaling of a depressed
polynomial to finite coefficients, and coprime factor lifting with strongly
summable evaluation inside the sign field. Existing node-based polynomial CRT
does not yet prove the general coprime factor-linearization statement.
