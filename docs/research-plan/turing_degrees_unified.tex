...
Each input has twenty numbered research entries, but the lists overlap substantially. The unified catalogue has \textbf{39 topics}; it splits the distinct classical and relative dimension questions that one original entry grouped together. It also preserves three bounded programmes from Report A without counting them as established literature-open problems. Some topics contain refinements, restricted versions, or linked conjectures. Thus neither the original sixty entries nor the unified thirty-nine topics are counts of logically independent conjectures.
...
\textbf{Unresolved-target track: M1, M3, and C1.} For a genuinely unresolved bounded attempt, the combined reports favor the five-cycle metric realization, the diameter-three-versus-four question, and a counterexample to least Turing degrees in nonuniform coarse classes. These are speculative choices, not forecasts. A short conventional argument might exist; a complete formal proof could still require a substantial foundation. The local questions Q1--Q3, the effective-dense questions C2--C3, and a sharply defined categoricity subcase are exploratory alternatives. Programmes R1--R3 are more controllable only after fixing a genuinely new, finite or restricted target.

\subsection{Status evidence and feasibility are different axes}
The catalogue separates the following evidence descriptions. \textbf{O26}, \textbf{O25}, \textbf{O24}, and \textbf{O21} mean an explicit open-question statement or listing of that date inherited from the reports. \textbf{S07}, \textbf{S12}, and \textbf{S17} flag older primary-source leads for which current specialist reconfirmation is particularly important. \textbf{Ref} identifies a refinement or a precise component of a broader source question. \textbf{V} flags a supplied proof proposal rather than externally established closure of an open problem. A recent preprint is not silently treated as a peer-reviewed publication.
...
\hyperref[entry:C1]{C1} & Least Turing degree in each nonuniform coarse class & B & O25 \\
\hyperref[entry:C2]{C2} & Least Turing degree in each nonuniform effective-dense class & E & O25 \\
...
\end{longtable}
\normalsize
\noindent R1--R3, discussed in Section~\ref{sec:projects}, are separately proposed programmes: bounded lattice-obstruction classification, a new restricted meet-removal theorem, and finite bookkeeping for measure constructions. Their specification and novelty remain part of the research task.
...
\problemheading{C1}{Must every nonuniform coarse class have a least Turing degree?}{\W{}, counterexample route}
The coarse instance of Gerdes's Question~7 asks whether
\[
 \forall f\in\Baire\ \exists g\equiv_{\mathrm{nc}}f\
 \forall h\equiv_{\mathrm{nc}}f\ (g\leT h).
\]
A small negative example would settle this instance completely. The distinction between a \emph{minimal} element and a \emph{least} element must be preserved: a least representative is below every representative.

A concrete sufficient target is a nonzero coarse class with two representatives whose Turing degrees form a minimal pair. Proposition~\ref{prop:minpair} proves why this suffices. The research difficulty is arranging those properties together. Ordinary Turing noncomputability is far too weak: Proposition~\ref{prop:sparse} gives every Turing degree a representative in the computable coarse class. The \W{} rating is conditional on obtaining a short finite-extension argument, not on importing a large minimal-pair theorem as an unproved Lean axiom. Unlike the tower validation track, this remains a construction-search target; its formal cost depends strongly on the availability of an oracle-construction library.

\problemheading{C2}{Least Turing degrees in effective-dense classes}{\E{}}
...
\end{proof}
This is a useful warning for C1: even the presence of arbitrarily complicated representatives says nothing against the existence of a least one. The same proof of upward closure works for natural-number-valued representatives, with a fixed encoding of the higher oracle.
...
\end{proof}
This gives a sufficient route for C1 or C2, not a necessary characterization. In particular, a proof that no such minimal pair exists would not prove that every class has a least degree.
...
\]
Martin's full conjecture is normally formulated in a determinacy setting, such as $\mathsf{ZF}+\mathsf{AD}+\mathsf{DC}$. Borel restrictions provide targets in ordinary classical mathematics. These are not interchangeable foundational formulations.\cite{MSS}
...
For the tower track, first agree on the exact definitions, array representation, optional $G_0=\N$ convention, and reduction direction. Ask whether the proposed arguments address the intended source questions and whether equivalent constructions already occur in the literature. For an unresolved target, choose \emph{one} of M1, M3, C1, or a precisely stated restriction, and write its complete quantifiers before selecting a proof assistant interface. The first formal pilot should test the semantic bottleneck, not merely establish notation.

For example, the tower pilot is bounded projection and maximum computability with the right uniformity; M1's pilot is a genuinely usable distance estimate; C1's pilot separates coarse descriptions from Turing noncomputability. An early failure to prove these interfaces is evidence to revise the project, not permission to turn the missing result into an axiom.
