...
Alongside them, `docs/reports/` collects seventy-one further research reports on
...
`docs/reports/` holds seventy-one independent reports that belong to neither project
...
graphs, enumerative combinatorics, and generating-function asymptotics.  Most attack a
specific conjecture from the literature or from an OEIS entry; about a quarter are
counterexamples rather than proofs.  They arrived as eighty archives in three
deliveries; nine pairs turned out to prove the same theorem and were merged, each
merged report keeping both originals' extra material and, where the proofs
genuinely differed, both proofs.  `docs/reports/manifest.pdf` catalogues all
seventy-one, and `docs/reports/README.md` is the index.
